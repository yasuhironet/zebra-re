...
void
community_add_val (struct community *com, uint32_t val)
{
  com->size++;
...
void
community_del_val (struct community *com, uint32_t *val)
{
  int i = 0;
  int c = 0;
      ...
      if (memcmp (com->val + i, val, sizeof (uint32_t)) == 0)
        {
          ...
          if (com->size > 0)
            com->val =
                XREALLOC (MTYPE_COMMUNITY_VAL, com->val, com_length (com));
          else
            {
              XFREE (MTYPE_COMMUNITY_VAL, com->val);
            }
...
bool
community_include (struct community *com, uint32_t val)
{
  int i;
...
static void
set_community_string (struct community *com, bool make_json,
  ...
  for (i = 0; i < com->size; i++)
    {
      ...
      switch (comval)
        ...
        case COMMUNITY_ACCEPT_OWN:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("acceptown");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_ROUTE_FILTER_TRANSLATED_v4:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("routeFilterTranslatedV4");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_ROUTE_FILTER_v4:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("routeFilterV4");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_ROUTE_FILTER_TRANSLATED_v6:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("routeFilterTranslatedV6");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_ROUTE_FILTER_v6:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("routeFilterV6");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_LLGR_STALE:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("llgrStale");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_NO_LLGR:
        ...
        case COMMUNITY_ACCEPT_OWN_NEXTHOP:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("acceptownnexthop");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_BLACKHOLE:
          ...
          if (make_json)
            {
              json_string = json_object_new_string ("blackhole");
              json_object_array_add (json_community_list, json_string);
            }
          break;
        case COMMUNITY_NO_EXPORT:
...
struct community *
community_parse (uint32_t *pnt, unsigned short length)
{
...
community_dup (struct community *com)
{
  ...
  if (new->size)
    {
      new->val = XMALLOC (MTYPE_COMMUNITY_VAL, com->size * COMMUNITY_SIZE);
...
char *
community_str (struct community *com, bool make_json, bool translate_alias)
{
  if (! com)
    return NULL;
...
community_cmp (const struct community *com1, const struct community *com2)
{
  ...
  if (com1->size == com2->size)
    if (memcmp (com1->val, com2->val, com1->size * COMMUNITY_SIZE) == 0)
      return true;
  return false;
}
...
static bool
community_valid (const char *community)
{
  int octets = 0;
  char **splits;
  int num;
  ...
  if (isalpha ((unsigned char) *p))
    {
      ...
      if (strncmp (p, "accept-own-nexthop", strlen ("accept-own-nexthop")) ==
          0)
        {
      ...
      if (strncmp (p, "route-filter-v4", strlen ("route-filter-v4")) == 0)
        {
      ...
      if (strncmp (p, "route-filter-v6", strlen ("route-filter-v6")) == 0)
        {
...
static void *
bgp_aggr_community_hash_alloc (void *p)
{
...
static void
bgp_aggr_community_prepare (struct hash_bucket *hb, void *arg)
{
...
void
bgp_aggr_community_remove (void *arg)
{
...
void
bgp_compute_aggregate_community (struct bgp_aggregate *aggregate,
                                 struct community *community)
{
...
void
bgp_compute_aggregate_community_val (struct bgp_aggregate *aggregate)
{
  ...
  if (aggregate->community_hash && aggregate->community_hash->count)
    {
      hash_iterate (aggregate->community_hash, bgp_aggr_community_prepare,
                    &aggregate->community);
...
void
bgp_remove_community_from_aggregate (struct bgp_aggregate *aggregate,
                                     struct community *community)
{
  struct community *aggr_community = NULL;
  ...
  if (aggr_community)
    {
      ...
      if (aggr_community->refcnt == 0)
        {
          ret_comm = hash_release (aggregate->community_hash, aggr_community);
