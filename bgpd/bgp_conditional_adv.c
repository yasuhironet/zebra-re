      ...
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
          ...
          /* Fill temp path_info */
          prep_for_rmap_apply (&path, &path_extra, dest, pi, pi->peer,
                               &dummy_attr);
          ...
          if (ret == RMAP_PERMITMATCH)
            {
              ...
              bgp_cond_adv_debug (
                  "%s: Condition map routes present in BGP table", __func__);
...
static void
bgp_conditional_adv_routes (struct peer *peer, afi_t afi, safi_t safi,
      ...
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
          ...
          ret = route_map_apply (rmap, dest_p, &path);
          if (ret != RMAP_PERMITMATCH ||
              ! bgp_check_selected (pi, peer, addpath_capable, afi, safi))
            {
          ...
          if (update_type == UPDATE_TYPE_ADVERTISE &&
              subgroup_announce_check (dest, pi, subgrp, dest_p, &attr,
                                       &advmap_attr))
            {
 ...
 */
static void
bgp_conditional_adv_timer (struct event *t)
{
  afi_t afi;
  safi_t safi;
  int pfx_rcd_safi;
      ...
      FOREACH_AFI_SAFI (afi, safi)
        {
           ...
           */
          pfx_rcd_safi = (safi == SAFI_LABELED_UNICAST) ? SAFI_UNICAST : safi;
          ...
          if (BGP_DEBUG (cond_adv, COND_ADV))
            {
              if (peer->advmap_table_change)
                zlog_debug ("%s: %s - routes changed in BGP table.", __func__,
                            peer->host);
              if (peer->advmap_config_change[afi][safi])
                zlog_debug (
          ...
          if (filter->advmap.condition == CONDITION_EXIST)
            filter->advmap.update_type = (ret == RMAP_PERMITMATCH)
          ...
          if (paf && (SUBGRP_PEER (PAF_SUBGRP (paf))
                          ->filter[afi][safi]
                          .advmap.update_type != filter->advmap.update_type))
            {
...
void
bgp_conditional_adv_enable (struct peer *peer, afi_t afi, safi_t safi)
{
...
void
bgp_conditional_adv_disable (struct peer *peer, afi_t afi, safi_t safi)
{
  struct bgp *bgp = peer->bgp;
...
static void
peer_advertise_map_filter_update (struct peer *peer, afi_t afi, safi_t safi,
                                  const char *amap_name,
                                  struct route_map *amap,
                                  const char *cmap_name,
                                  struct route_map *cmap, bool condition,
                                  bool set)
{
...
int
peer_advertise_map_set (struct peer *peer, afi_t afi, safi_t safi,
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      peer_advertise_map_filter_update (member, afi, safi, advertise_name,
                                        advertise_map, condition_name,
                                        condition_map, condition, true);
    }
...
int
peer_advertise_map_unset (struct peer *peer, afi_t afi, safi_t safi,
                          const char *advertise_name,
                          struct route_map *advertise_map,
                          const char *condition_name,
                          struct route_map *condition_map, bool condition)
{
  ...
  if (peer_group_active (peer))
    {
      PEER_STR_ATTR_INHERIT (peer, peer->group, filter[afi][safi].advmap.aname,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, peer->group, filter[afi][safi].advmap.amap);
    }
  else
    peer_advertise_map_filter_update (peer, afi, safi, advertise_name,
                                      advertise_map, condition_name,
  ...
  if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      /* Process peer route updates. */
      bgp_cond_adv_debug ("%s: Send normal update to %s for %s", __func__,
                          peer->host, get_afi_safi_str (afi, safi, false));
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      /* Process peer route updates. */
      bgp_cond_adv_debug ("%s: Send normal update to %s for %s ", __func__,
                          member->host, get_afi_safi_str (afi, safi, false));
    }
