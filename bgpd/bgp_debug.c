...
const char *const bgp_type_str[] = { NULL,        "OPEN",
                                     "UPDATE",    "NOTIFICATION",
 ...
 */
static int
bgp_debug_list_conf_print (struct vty *vty, const char *desc,
                           struct list *list)
{
  struct bgp_debug_filter *filter;
  struct listnode *node, *nnode;
      ...
      for (ALL_LIST_ELEMENTS (list, node, nnode, filter))
        {
          ...
          if (filter->p && filter->p->family == AF_EVPN)
            {
              bgp_debug_print_evpn_prefix (vty, desc, filter->p);
              write++;
            }
          else if (filter->p)
            {
...
static void
bgp_debug_list_add_entry (struct list *list, const char *host,
...
static bool
bgp_debug_list_remove_entry (struct list *list, const char *host,
                             const struct prefix *p)
{
  struct bgp_debug_filter *filter;
  struct listnode *node, *nnode;
...
static bool
bgp_debug_list_has_entry (struct list *list, const char *host,
                          const struct prefix *p)
{
  struct bgp_debug_filter *filter;
  struct listnode *node, *nnode;
...
bgp_dump_attr (struct attr *attr, char *buf, size_t size)
{
  ...
  if (attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV6_GLOBAL ||
    ...
    snprintfrr (buf + strlen (buf), size - strlen (buf), ", mp_nexthop %pI6",
  ...
  if (CHECK_FLAG (attr->flag, ATTR_FLAG_BIT (BGP_ATTR_LARGE_COMMUNITIES)))
    snprintf (buf + strlen (buf), size - strlen (buf), ", large-community %s",
  ...
  if (CHECK_FLAG (attr->flag, ATTR_FLAG_BIT (BGP_ATTR_ORIGINATOR_ID)))
    snprintfrr (buf + strlen (buf), size - strlen (buf), ", originator %pI4",
                &attr->originator_id);
  ...
  if (CHECK_FLAG (attr->flag, ATTR_FLAG_BIT (BGP_ATTR_CLUSTER_LIST)))
    {
      ...
      for (i = 0; i < cluster->length / 4; i++)
        snprintfrr (buf + strlen (buf), size - strlen (buf), " %pI4",
                    &cluster->list[i]);
    }
...
const char *
bgp_notify_admin_message (char *buf, size_t bufsz, uint8_t *data,
                          size_t datalen)
{
  memset (buf, 0, bufsz);
  if (! data || datalen < 1)
                  ...
                  const char *direct, bool hard_reset)
{
  ...
  if (BGP_DEBUG (neighbor_events, NEIGHBOR_EVENTS) ||
      CHECK_FLAG (peer->bgp->flags, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
    {
      ...
      subcode_str =
          bgp_notify_subcode_str (bgp_notify->code, bgp_notify->subcode);
      ...
      if (msg_str)
        {
          zlog_info ("%%NOTIFICATION%s: %s neighbor %s %d/%d (%s%s) \"%s\"",
      ...
      else
        {
          ...
          zlog_info (
              ...
              peer->host, bgp_notify->code, bgp_notify->subcode, code_str,
...
static void
bgp_debug_print_evpn_prefix (struct vty *vty, const char *desc,
                             struct prefix *p)
{
      ...
      if (is_evpn_prefix_ipaddr_none ((struct prefix_evpn *) p))
      ...
      else
        {
          ...
          snprintf (evpn_desc, sizeof (evpn_desc),
                    ...
                    prefix_mac2str (&p->u.prefix_evpn.macip_addr.mac, buf2,
                                    sizeof (buf2)),
                    inet_ntop (family, &p->u.prefix_evpn.macip_addr.ip.ip.addr,
...
static int
bgp_debug_parse_evpn_prefix (struct vty *vty, struct cmd_token **argv,
  ...
  if (evpn_type == BGP_EVPN_MAC_IP_ROUTE)
    {
      ...
      build_evpn_type2_prefix ((struct prefix_evpn *) argv_p, &mac, &ip);
    }
  else if (evpn_type == BGP_EVPN_IMET_ROUTE)
    {
      ...
      build_evpn_type3_prefix ((struct prefix_evpn *) argv_p, ip.ipaddr_v4);
    }
  else if (evpn_type == BGP_EVPN_IP_PREFIX_ROUTE)
    {
      ...
      build_type5_prefix_from_ip_prefix ((struct prefix_evpn *) argv_p,
                                         &ip_prefix);
    }
       ...
       "BGP neighbor on interface to debug\n")
{
  ...
  if (bgp_debug_keepalive_peers && ! list_isempty (bgp_debug_keepalive_peers))
    {
      found_peer =
          bgp_debug_list_remove_entry (bgp_debug_keepalive_peers, host, NULL);
       ...
       "IPv6 prefix\n")
{
  ...
  if (bgp_debug_bestpath_prefixes &&
    ...
    {
      found_prefix = bgp_debug_list_remove_entry (bgp_debug_bestpath_prefixes,
                                                  NULL, prefix);
       ...
       "BGP neighbor on interface to debug\n")
{
  ...
  if (inbound && bgp_debug_update_in_peers &&
    ...
    {
      found_peer =
          bgp_debug_list_remove_entry (bgp_debug_update_in_peers, host, NULL);
  ...
  if (! inbound && bgp_debug_update_out_peers &&
    ...
    {
      found_peer =
          bgp_debug_list_remove_entry (bgp_debug_update_out_peers, host, NULL);
       ...
       "IPv6 prefix\n")
{
  ...
  if (bgp_debug_list_has_entry (bgp_debug_update_prefixes, NULL, &argv_p))
    {
      vty_out (vty, "BGP updates debugging is already enabled for %pFX\n",
               &argv_p);
      return CMD_SUCCESS;
    }
       ...
       "IPv6 prefix\n")
{
  ...
  if (bgp_debug_update_prefixes && ! list_isempty (bgp_debug_update_prefixes))
    {
      found_prefix = bgp_debug_list_remove_entry (bgp_debug_update_prefixes,
                                                  NULL, &argv_p);
       ...
       "IPv6 prefix\n")
{
  ...
  if (bgp_debug_zebra_prefixes && ! list_isempty (bgp_debug_zebra_prefixes))
    {
      found_prefix =
          bgp_debug_list_remove_entry (bgp_debug_zebra_prefixes, NULL, prefix);
          ...
          if (no)
            {
              ...
              vty_out (vty, "BGP EVPN-MH ES debugging is off\n");
            }
          else
            {
          ...
          if (no)
            {
              ...
              vty_out (vty, "BGP EVPN-MH route debugging is off\n");
            }
          else
            {
      ...
      if (no)
        {
          ...
          vty_out (vty, "BGP conditional advertisement debugging is off\n");
        }
      else
        {
            ...
            BGP_STR)
{
  ...
  if (BGP_DEBUG (vpn, VPN_LEAK_FROM_VRF))
    vty_out (vty, "  BGP route leak from vrf to vpn debugging is on\n");
  if (BGP_DEBUG (vpn, VPN_LEAK_TO_VRF))
    vty_out (vty, "  BGP route leak to vrf from vpn debugging is on\n");
 ...
 */
static int
bgp_debug_per_prefix (const struct prefix *p,
...
bool
bgp_debug_neighbor_events (const struct peer *peer)
{
  char *host = NULL;
...
bool
bgp_debug_keepalive (const struct peer *peer)
{
  char *host = NULL;
  ...
  return bgp_debug_per_peer (host, term_bgp_debug_keepalive,
                             BGP_DEBUG_KEEPALIVE, bgp_debug_keepalive_peers);
}

bool
bgp_debug_update (const struct peer *peer, const struct prefix *p,
...
bool
bgp_debug_bestpath (struct bgp_dest *dest)
{
  if (BGP_DEBUG (bestpath, BESTPATH))
    {
      if (bgp_debug_per_prefix (bgp_dest_get_prefix (dest),
                                term_bgp_debug_bestpath, BGP_DEBUG_BESTPATH,
                                bgp_debug_bestpath_prefixes))
        return true;
    }
...
bool
bgp_debug_zebra (const struct prefix *p)
{
  if (BGP_DEBUG (zebra, ZEBRA))
    {
...
bgp_debug_rdpfxpath2str (afi_t afi, safi_t safi, const struct prefix_rd *prd,
                         ...
                         uint32_t addpath_id,
                         struct bgp_route_evpn *overlay_index, char *str,
                         int size)
{
  char tag_buf[30];
  ...
  if (overlay_index && overlay_index->type == OVERLAY_INDEX_GATEWAY_IP)
    {
      ...
      if (is_evpn_prefix_ipaddr_v4 (evp))
        inet_ntop (AF_INET, &overlay_index->gw_ip, obuf, sizeof (obuf));
      else if (is_evpn_prefix_ipaddr_v6 (evp))
        inet_ntop (AF_INET6, &overlay_index->gw_ip, obuf, sizeof (obuf));
      ...
      if (safi == SAFI_EVPN)
        {
          ...
          snprintf (tag_buf, sizeof (tag_buf), " label %s", tag_buf2);
        }
      else
        {
          ...
          snprintf (tag_buf, sizeof (tag_buf), " label %u", label_value);
        }
    }
  ...
  if (prd)
    {
      ...
      len += snprintfrr (str + len, size - len,
                         BGP_RD_AS_FORMAT (bgp_get_asnotation (NULL)), prd);
  ...
  else if (safi == SAFI_FLOWSPEC)
    {
      ...
      bgp_fs_nlri_get_string ((unsigned char *) fs->prefix.ptr,
                              fs->prefix.prefixlen, return_string,
                              NLRI_STRING_FORMAT_DEBUG, NULL,
                              family2afi (fs->prefix.family));
      snprintf (str, size, "FS %s Match{%s}", afi2str (afi), return_string);
    }
  else
    snprintfrr (str, size, "%pFX%s%s %s %s", pu.p, tag_buf, pathid_buf,
