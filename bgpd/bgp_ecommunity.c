...
*/
static bool
ecommunity_add_val_internal (struct ecommunity *ecom, const void *eval,
                             bool unique, bool overwrite, uint8_t ecom_size)
{
  ...
  if (ecom->val == NULL)
    {
      ecom->size = 1;
      ecom->val =
          XMALLOC (MTYPE_ECOMMUNITY_VAL, ecom_length_size (ecom, ecom_size));
  ...
  ins_idx = UINT32_MAX;
  for (uint8_t *p = ecom->val; c < ecom->size; p += ecom_size, c++)
    {
      if (unique)
        {
  ...
  memcpy (ecom->val + (ins_idx * ecom_size), eval, ecom_size);
...
bool
ecommunity_add_val_ipv6 (struct ecommunity *ecom,
                         struct ecommunity_val_ipv6 *eval, bool unique,
                         bool overwrite)
{
  return ecommunity_add_val_internal (ecom, (const void *) eval, unique,
...
static struct ecommunity *
ecommunity_parse_internal (uint8_t *pnt, unsigned short length,
...
struct ecommunity *
ecommunity_parse (uint8_t *pnt, unsigned short length,
                  bool disable_ieee_floating)
{
...
struct ecommunity *
ecommunity_parse_ipv6 (uint8_t *pnt, unsigned short length,
                       bool disable_ieee_floating)
{
...
ecommunity_dup (struct ecommunity *ecom)
{
  ...
  if (new->size)
    {
      new->val = XMALLOC (MTYPE_ECOMMUNITY_VAL, ecom->size * ecom->unit_size);
...
ecommunity_cmp (const void *arg1, const void *arg2)
{
  ...
  return (ecom1->size == ecom2->size &&
          memcmp (ecom1->val, ecom2->val, ecom1->size * ecom1->unit_size) ==
              0);
}
...
static const char *
ecommunity_origin_validation_state2str (
...
static void
ecommunity_origin_validation_state_str (char *buf, size_t bufsz, uint8_t *ptr)
{
...
bool
ecommunity_node_target_match (struct ecommunity *ecom,
  ...
  for (i = 0; i < ecom->size; i++)
    {
      ...
      if (type == ECOMMUNITY_ENCODE_IP && sub_type == ECOMMUNITY_NODE_TARGET)
        {
...
static void
ecommunity_node_target_str (char *buf, size_t bufsz, uint8_t *ptr, int format)
{
  /*
   *  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  snprintfrr (
      buf, bufsz, "%s%pI4%s",
      format == ECOMMUNITY_FORMAT_COMMUNITY_LIST ? "nt " : "NT:", &node_id,
...
static int
ecommunity_encode_internal (uint8_t type, uint8_t sub_type, int trans, as_t as,
                            struct in_addr *ip, struct in6_addr *ip6,
                            uint32_t val, void *eval_ptr)
{
  struct ecommunity_val *eval = (struct ecommunity_val *) eval_ptr;
  struct ecommunity_val_ipv6 *eval6 = (struct ecommunity_val_ipv6 *) eval_ptr;
  ...
  else if (type == ECOMMUNITY_ENCODE_IP || type == ECOMMUNITY_ENCODE_AS4)
    {
      if (val > UINT16_MAX)
        return -1;
    }
 ...
 */
static int
ecommunity_encode (uint8_t type, uint8_t sub_type, int trans, as_t as,
                   struct in_addr ip, uint32_t val,
                   struct ecommunity_val *eval)
{
  return ecommunity_encode_internal (type, sub_type, trans, as, &ip, NULL, val,
                                     (void *) eval);
}

/* Get next Extended Communities token from the string. */
static const char *
ecommunity_gettoken (const char *str, void *eval_ptr,
                     enum ecommunity_token *token, int type)
{
  ...
  if (*token == ecommunity_token_rt6 || *token == ecommunity_token_val6)
    {
      char *limit;
      ...
      if (ecommunity_encode_internal (ecomm_type,
                                      ECOMMUNITY_FLOWSPEC_REDIRECT_IPV6, 1, 0,
                                      NULL, &ip6, as, eval_ptr))
        goto error;
...
static struct ecommunity *
ecommunity_str2com_internal (const char *str, int type, int keyword_included,
  ...
  while ((str = ecommunity_gettoken (str, (void *) &eval, &token, type)))
    {
      switch (token)
        ...
        case ecommunity_token_soo:
          ...
          if (token == ecommunity_token_rt || token == ecommunity_token_rt6)
            {
...
{
  return ecommunity_str2com_internal (str, type, keyword_included, false);
}

struct ecommunity *
ecommunity_str2com_ipv6 (const char *str, int type, int keyword_included)
{
  return ecommunity_str2com_internal (str, type, keyword_included, true);
}

static int
ecommunity_rt_soo_str_internal (char *buf, size_t bufsz, const uint8_t *pnt,
  ...
  else if (type == ECOMMUNITY_ENCODE_AS)
    {
      if (ecom_size == ECOMMUNITY_SIZE)
      ...
      else
        {
          ...
          len = snprintf (buf, bufsz, "%s%s:%u", prefix, buf_local, eip6.val);
        }
    }
  else if (type == ECOMMUNITY_ENCODE_IP)
    {
      ...
      len = snprintfrr (buf, bufsz, "%s%pI4:%u", prefix, &eip.ip, eip.val);
    }
...
static int
ecommunity_rt_soo_str (char *buf, size_t bufsz, const uint8_t *pnt, int type,
                       int sub_type, int format)
{
  return ecommunity_rt_soo_str_internal (buf, bufsz, pnt, type, sub_type,
                                         format, ECOMMUNITY_SIZE);
}

/* Helper function to convert IEEE-754 Floating Point to uint32 */
static uint32_t
ieee_float_uint32_to_uint32 (uint32_t u)
{
  union
  {
    float r;
...
static int
ecommunity_lb_str (char *buf, size_t bufsz, const uint8_t *pnt,
                   bool disable_ieee_floating)
{
  int len = 0;
  as_t as;
...
#define ONE_KBPS_BYTES (1000 / 8)
  ...
  bw = disable_ieee_floating ? bw_tmp : ieee_float_uint32_to_uint32 (bw_tmp);
...
/* Convert extended community attribute to string.
   ...
   extcommunity-list
        "rt 100:1 rt 100:2 soo 100:3show [ip] bgp" and extcommunity-list
   regular expression matching "RT:100:1 RT:100:2 SoO:100:3"
  ...
  for (i = 0; i < ecom->size; i++)
    {
      ...
      if (type == ECOMMUNITY_ENCODE_AS || type == ECOMMUNITY_ENCODE_IP ||
          type == ECOMMUNITY_ENCODE_AS4)
        {
          ...
          if (sub_type != ECOMMUNITY_ROUTE_TARGET &&
              sub_type != ECOMMUNITY_SITE_ORIGIN)
            {
              if (sub_type == ECOMMUNITY_FLOWSPEC_REDIRECT_IPV4 &&
                  type == ECOMMUNITY_ENCODE_IP)
                {
                  ...
                  snprintfrr (encbuf, sizeof (encbuf), "NH:%pI4:%d", ipv4,
                              pnt[5]);
                }
                       ...
                       type == ECOMMUNITY_ENCODE_AS)
                {
                  ecommunity_lb_str (encbuf, sizeof (encbuf), pnt,
                       ...
                       type == ECOMMUNITY_ENCODE_IP)
                {
                  ecommunity_node_target_str (encbuf, sizeof (encbuf), pnt,
                                              format);
                }
              else
                unk_ecom = 1;
            ...
            {
              ecommunity_rt_soo_str (encbuf, sizeof (encbuf), pnt, type,
                                     sub_type, format);
            }
        }
      ...
      else if (type == ECOMMUNITY_ENCODE_EVPN)
        {
          ...
          if (*pnt == ECOMMUNITY_EVPN_SUBTYPE_ROUTERMAC)
            {
              ...
              snprintf (encbuf, sizeof (encbuf),
                        ...
                        (uint8_t) rmac.octet[0], (uint8_t) rmac.octet[1],
                        (uint8_t) rmac.octet[2], (uint8_t) rmac.octet[3],
                        (uint8_t) rmac.octet[4], (uint8_t) rmac.octet[5]);
            }
          else if (*pnt == ECOMMUNITY_EVPN_SUBTYPE_MACMOBILITY)
            {
              ...
              snprintf (encbuf, sizeof (encbuf), "MM:%u", seqnum);
          ...
          else if (*pnt == ECOMMUNITY_EVPN_SUBTYPE_ND)
            {
              ...
              if (CHECK_FLAG (flags, ECOMMUNITY_EVPN_SUBTYPE_ND_ROUTER_FLAG))
                strlcpy (encbuf, "ND:Router Flag", sizeof (encbuf));
              if (CHECK_FLAG (flags, ECOMMUNITY_EVPN_SUBTYPE_PROXY_FLAG))
                strlcpy (encbuf, "ND:Proxy", sizeof (encbuf));
            }
          ...
          else if (*pnt == ECOMMUNITY_EVPN_SUBTYPE_ESI_LABEL)
            {
              uint8_t flags = *++pnt;

              snprintf (encbuf, sizeof (encbuf), "ESI-label-Rt:%s",
                        (flags & ECOMMUNITY_EVPN_SUBTYPE_ESI_SA_FLAG) ? "SA"
                                                                      : "AA");
            }
          else if (*pnt == ECOMMUNITY_EVPN_SUBTYPE_DF_ELECTION)
            {
              uint8_t alg;
              uint16_t pref;
              uint16_t bmap;
              ...
              if (bmap)
                snprintf (encbuf, sizeof (encbuf),
                          "DF: (alg: %u, bmap: 0x%x pref: %u)", alg, bmap,
                          pref);
              else
                snprintf (encbuf, sizeof (encbuf), "DF: (alg: %u, pref: %u)",
                          alg, pref);
            }
          else
            unk_ecom = 1;
      ...
      else if (type == ECOMMUNITY_ENCODE_REDIRECT_IP_NH)
        {
          ...
          if (sub_type == ECOMMUNITY_REDIRECT_IP_NH)
            {
              snprintf (encbuf, sizeof (encbuf), "FS:redirect IP 0x%x",
                        *(pnt + 5));
            }
          else
            unk_ecom = 1;
          ...
          if (sub_type == ECOMMUNITY_ROUTE_TARGET)
            {
              ...
              ecommunity_rt_soo_str_internal (
                  ...
                  type & ~ECOMMUNITY_ENCODE_TRANS_EXP, ECOMMUNITY_ROUTE_TARGET,
                  format, ecom->unit_size);
          ...
          else if (sub_type == ECOMMUNITY_FLOWSPEC_REDIRECT_IPV6)
            {
              ...
              ecommunity_rt_soo_str_internal (
                  buf, sizeof (buf), (const uint8_t *) pnt,
                  type & ~ECOMMUNITY_ENCODE_TRANS_EXP, ECOMMUNITY_ROUTE_TARGET,
                  ECOMMUNITY_FORMAT_DISPLAY, ecom->unit_size);
          ...
          else if (sub_type == ECOMMUNITY_TRAFFIC_ACTION)
            {
              ...
              if (*(pnt + 3) == 1 << FLOWSPEC_TRAFFIC_ACTION_TERMINAL)
                strlcpy (action, "terminate (apply)", sizeof (action));
              else
                strlcpy (action, "eval stops", sizeof (action));

              if (*(pnt + 3) == 1 << FLOWSPEC_TRAFFIC_ACTION_SAMPLE)
                strlcat (action, ", sample", sizeof (action));


              snprintf (encbuf, sizeof (encbuf), "FS:action %s", action);
            }
            ...
            {
              snprintf (encbuf, sizeof (encbuf), "FS:marking %u", *(pnt + 5));
            }
          else
            unk_ecom = 1;
        }
      ...
      else if (type == ECOMMUNITY_ENCODE_OPAQUE_NON_TRANS)
        {
          ...
          if (sub_type == ECOMMUNITY_ORIGIN_VALIDATION_STATE)
            ecommunity_origin_validation_state_str (encbuf, sizeof (encbuf),
                                                    pnt);
          else
            unk_ecom = 1;
        }
      ...
      if (unk_ecom)
        snprintf (encbuf, sizeof (encbuf), "UNK:%d, %d", type, sub_type);
...
bool
ecommunity_include (struct ecommunity *e1, struct ecommunity *e2)
{
  uint32_t i, j;
      ...
      for (j = 0; j < e2->size; ++j)
        {
          if (! memcmp (e1->val + (i * e1->unit_size),
                        e2->val + (j * e2->unit_size), e1->unit_size))
            return true;
        }
    }
  return false;
  ...
  while (i < ecom1->size && j < ecom2->size)
    {
      if (memcmp (ecom1->val + i * ecom1->unit_size,
                  ecom2->val + j * ecom2->unit_size, ecom2->unit_size) == 0)
        j++;
      i++;
    }
...
extern struct ecommunity_val *
ecommunity_lookup (const struct ecommunity *ecom, uint8_t type,
                   uint8_t subtype)
{
  uint8_t *p;
  uint32_t c;
 ...
 */
bool
ecommunity_strip (struct ecommunity *ecom, uint8_t type, uint8_t subtype)
{
  uint8_t *p, *q, *new;
  uint32_t c, found = 0;
  /* When this is fist value, just add it.  */
 ...
 */
bool
ecommunity_del_val (struct ecommunity *ecom, struct ecommunity_val *eval)
{
  uint8_t *p;
  uint32_t c, found = 0;
...
int
ecommunity_fill_pbr_action (struct ecommunity_val *ecom_eval,
                            struct bgp_pbr_entry_action *api, afi_t afi)
{
  ...
  else if (ecom_eval->val[1] == ECOMMUNITY_REDIRECT_IP_NH && afi == AFI_IP)
    {
...
static struct ecommunity *
bgp_aggr_ecommunity_lookup (struct bgp_aggregate *aggregate,
...
static void *
bgp_aggr_ecommunty_hash_alloc (void *p)
{
...
static void
bgp_aggr_ecommunity_prepare (struct hash_bucket *hb, void *arg)
{
  struct ecommunity *hb_ecommunity = hb->data;
  struct ecommunity **aggr_ecommunity = arg;

  if (*aggr_ecommunity)
    *aggr_ecommunity = ecommunity_merge (*aggr_ecommunity, hb_ecommunity);
  else
    *aggr_ecommunity = ecommunity_dup (hb_ecommunity);
}

void
bgp_aggr_ecommunity_remove (void *arg)
{
                                       ...
                                       struct ecommunity *ecommunity)
{
  ...
  if (aggregate->ecommunity_hash == NULL)
    aggregate->ecommunity_hash =
        hash_create (ecommunity_hash_make, ecommunity_cmp,
  ...
  if (aggr_ecommunity == NULL)
    {
       ...
       */
      aggr_ecommunity = hash_get (aggregate->ecommunity_hash, ecommunity,
...
void
bgp_compute_aggregate_ecommunity_val (struct bgp_aggregate *aggregate)
{
  ...
  if (aggregate->ecommunity_hash && aggregate->ecommunity_hash->count)
    {
      hash_iterate (aggregate->ecommunity_hash, bgp_aggr_ecommunity_prepare,
...
void
bgp_remove_ecommunity_from_aggregate (struct bgp_aggregate *aggregate,
                                      struct ecommunity *ecommunity)
{
  struct ecommunity *aggr_ecommunity = NULL;
  struct ecommunity *ret_ecomm = NULL;

  if ((! aggregate) || (! aggregate->ecommunity_hash) || (! ecommunity))
    return;
  ...
  if (aggr_ecommunity)
    {
      ...
      if (aggr_ecommunity->refcnt == 0)
        {
          ret_ecomm =
              hash_release (aggregate->ecommunity_hash, aggr_ecommunity);
 ...
 */
const uint8_t *
ecommunity_linkbw_present (struct ecommunity *ecom, uint32_t *bw)
{
  const uint8_t *eval;
  uint32_t i;
