...
mpls_label_t
bgp_adv_label (struct bgp_dest *dest, struct bgp_path_info *pi,
...
static void
bgp_send_fec_register_label_msg (struct bgp_dest *dest, bool reg,
                                 uint32_t label_index)
{
  ...
  have_label_to_reg =
      bgp_is_valid_label (local_label) && label != MPLS_LABEL_IMPLICIT_NULL;
  ...
  if (BGP_DEBUG (labelpool, LABELPOOL))
    zlog_debug ("%s: FEC %sregister %pRN label_index=%u label=%u", __func__,
                reg ? "" : "un", bgp_dest_to_rnode (dest), label_index, label);
 ...
 */
int
bgp_reg_for_label_callback (mpls_label_t new_label, void *labelid,
                            bool allocated)
{
  struct bgp_dest *dest;
      ...
      if (CHECK_FLAG (dest->flags, BGP_NODE_REGISTERED_FOR_LABEL))
        {
          ...
          dest->local_label =
              mpls_lse_encode (MPLS_LABEL_IMPLICIT_NULL, 0, 0, 1);
          bgp_set_valid_label (&dest->local_label);
        }
    }
...
void
bgp_reg_dereg_for_label (struct bgp_dest *dest, struct bgp_path_info *pi,
                         bool reg)
{
  ...
  if (BGP_DEBUG (labelpool, LABELPOOL))
    zlog_debug ("%s: %pFX: %s ", __func__, p, (reg ? "reg" : "dereg"));
          ...
          if (! have_label_to_reg)
            {
              ...
              if (BGP_DEBUG (labelpool, LABELPOOL))
                zlog_debug ("%s: Requesting label from LP for %pFX", __func__,
                            p);
               ...
               */
              bgp_lp_get (LP_TYPE_BGP_LU, dest, bgp_reg_for_label_callback);
              return;
            }
        }
    }
...
static int
bgp_nlri_get_labels (struct peer *peer, uint8_t *pnt, uint8_t plen,
                     mpls_label_t *label)
{
  uint8_t *data = pnt;
  ...
  if (label_depth > 1)
    zlog_info ("%pBP rcvd UPDATE with label stack %d deep", peer, label_depth);
  ...
  for (; pnt < lim; pnt += psize)
    {
      ...
      if (attr)
        {
          bgp_update (peer, &p, addpath_id, attr, packet->afi, safi,
                      ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, &label, 1, 0,
                      NULL);
        }
      else
        {
          bgp_withdraw (peer, &p, addpath_id, packet->afi, SAFI_UNICAST,
                        ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, &label, 1,
                        NULL);
        }
    }
