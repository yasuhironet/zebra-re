...
static int64_t
bgp_clist_new_seq_get (struct community_list *list)
{
  int64_t maxseq;
  int64_t newseq;
...
bgp_clist_seq_check (struct community_list *list, int64_t seq)
{
  struct community_entry *entry;
...
static uint32_t
bgp_clist_hash_key_community_list (const void *data)
{
...
static bool
bgp_clist_hash_cmp_community_list (const void *a1, const void *a2)
{
...
static void
community_list_delete (struct community_list_master *cm,
                       struct community_list *list)
{
...
static void
community_list_entry_delete (struct community_list_master *cm,
                             struct community_list *list,
                             struct community_entry *entry)
{
  if (entry->next)
 ...
 */
static void
community_list_entry_replace (struct community_list *list,
 ...
 */
bool
lcommunity_list_exact_match (struct lcommunity *lcom,
                             struct community_list *list)
{
  struct community_entry *entry;
...
/* Set community-list.  */
int
community_list_set (struct community_list_handler *ch, const char *name,
...
int
community_list_unset (struct community_list_handler *ch, const char *name,
                      const char *str, const char *seq, int direct, int style)
{
...
bool
lcommunity_list_valid (const char *community, int style)
{
  int octets;
          ...
          if (style == LARGE_COMMUNITY_LIST_STANDARD)
            {
              if (*splits[i] == '-')
                ...
                invalid++;
              else if (strtoul (splits[i], &endptr, 10) > UINT_MAX)
...
/* Set lcommunity-list.  */
int
lcommunity_list_set (struct community_list_handler *ch, const char *name,
                     const char *str, const char *seq, int direct, int style)
{
  struct community_entry *entry = NULL;
  struct community_list *list;
  ...
  if (community_list_dup_check (list, entry))
    ...
    {
      community_list_entry_add (list, entry, ch, LARGE_COMMUNITY_LIST_MASTER);
...
int
lcommunity_list_unset (struct community_list_handler *ch, const char *name,
                       const char *str, const char *seq, int direct, int style)
{
  struct community_list_master *cm = NULL;
  struct community_entry *entry = NULL;
...
int
extcommunity_list_set (struct community_list_handler *ch, const char *name,
                       const char *str, const char *seq, int direct, int style)
{
  struct community_entry *entry = NULL;
  struct community_list *list;
  ...
  if (ecom)
    entry->config =
        ecommunity_ecom2str (ecom, ECOMMUNITY_FORMAT_COMMUNITY_LIST, 0);
  else if (regex)
  ...
  if (community_list_dup_check (list, entry))
    ...
    {
      community_list_entry_add (list, entry, ch, EXTCOMMUNITY_LIST_MASTER);
 ...
 */
int
extcommunity_list_unset (struct community_list_handler *ch, const char *name,
...
community_list_init (void)
{
  ...
  ch->community_list.hash = hash_create_size (
      4, bgp_clist_hash_key_community_list, bgp_clist_hash_cmp_community_list,
      "Community List Number Quick Lookup");

  ch->extcommunity_list.hash = hash_create_size (
      4, bgp_clist_hash_key_community_list, bgp_clist_hash_cmp_community_list,
      "Extended Community List Quick Lookup");

  ch->lcommunity_list.hash = hash_create_size (
      4, bgp_clist_hash_key_community_list, bgp_clist_hash_cmp_community_list,
...
static int
bgp_community_list_vector_walker (struct hash_bucket *bucket, void *data)
{
  vector *comps = data;
...
static void
bgp_community_list_cmd_completion (vector comps, struct cmd_token *token)
{
  struct community_list_master *cm;
...
static void
bgp_lcommunity_list_cmd_completion (vector comps, struct cmd_token *token)
{
  struct community_list_master *cm;
...
static void
bgp_extcommunity_list_cmd_completion (vector comps, struct cmd_token *token)
{
  struct community_list_master *cm;
...
void
bgp_community_list_command_completion_setup (void)
{
