...
static int
bgp_isvalid_nexthop_for_ebgp (struct bgp_nexthop_cache *bnc,
  ...
  for (nexthop = bnc->nexthop; nexthop; nexthop = nexthop->next)
    {
      if (nexthop->type == NEXTHOP_TYPE_IFINDEX ||
        ...
        {
          ifp = if_lookup_by_index (bnc->ifindex ? bnc->ifindex
                                                 : nexthop->ifindex,
                                    bnc->bgp->vrf_id);
        }
      if (! ifp)
...
static int
bgp_isvalid_nexthop_for_mplsovergre (struct bgp_nexthop_cache *bnc,
                                     struct bgp_path_info *path)
{
  struct interface *ifp = NULL;
  struct nexthop *nexthop;
  ...
  if (CHECK_FLAG (path->attr->rmap_change_flags, BATTR_RMAP_L3VPN_ACCEPT_GRE))
    return true;
...
static int
bgp_isvalid_nexthop_for_mpls (struct bgp_nexthop_cache *bnc,
                              struct bgp_path_info *path)
{
  /*
  ...
  if (LIST_EMPTY (&(bnc->paths)) && ! bnc->nht_info)
    {
      if (BGP_DEBUG (nht, NHT))
        zlog_debug ("%s: freeing bnc %pFX(%d)(%u)(%s)", __func__, &bnc->prefix,
                    bnc->ifindex, bnc->srte_color, bnc->bgp->name_pretty);
...
void
bgp_unlink_nexthop (struct bgp_path_info *path)
{
...
void
bgp_replace_nexthop_by_peer (struct peer *from, struct peer *to)
{
  struct prefix pp;
  struct prefix pt;
...
bgp_find_ipv6_nexthop_matching_peer (struct peer *peer)
{
  ...
  frr_each (bgp_nexthop_cache, &peer->bgp->nexthop_cache_table[AFI_IP6], bnc)
    {
      if (bnc->nht_info == peer)
        {
          if (BGP_DEBUG (nht, NHT))
            {
              zlog_debug ("Found bnc: %pFX(%u)(%u)(%p) for peer: %s(%s) %p",
                          &bnc->prefix, bnc->ifindex, bnc->srte_color, bnc,
  ...
  if (BGP_DEBUG (nht, NHT))
    zlog_debug ("Could not find bnc for peer %s(%s) %p in v6 nexthop table",
                peer->host, peer->bgp->name_pretty, peer);
 ...
 */
int
bgp_find_or_add_nexthop (struct bgp *bgp_route, struct bgp *bgp_nexthop,
                         ...
                         const struct prefix *orig_prefix)
{
  ...
  if (pi)
    {
      ...
      if (! is_bgp_static_route && orig_prefix &&
          prefix_same (&p, orig_prefix))
        {
          if (BGP_DEBUG (nht, NHT))
            {
              zlog_debug ("%s(%pFX): prefix loops through itself", __func__,
                          &p);
            }
          return 0;
        }
  ...
  if (! bnc)
    {
      ...
      bnc->bgp = bgp_nexthop;
      if (BGP_DEBUG (nht, NHT))
        zlog_debug ("Allocated bnc %pFX(%d)(%u)(%s) peer %p", &bnc->prefix,
                    bnc->ifindex, bnc->srte_color, bnc->bgp->name_pretty,
                    peer);
    }
  else
    {
      if (BGP_DEBUG (nht, NHT))
        zlog_debug (
            ...
            &bnc->prefix, bnc->ifindex, bnc->bgp->name_pretty, bnc->flags,
  ...
  if (is_bgp_static_route)
    {
      ...
      else if ((! CHECK_FLAG (bgp_route->flags, BGP_FLAG_IMPORT_CHECK)) &&
               CHECK_FLAG (bnc->flags, BGP_STATIC_ROUTE_EXACT_MATCH))
        {
  ...
  if (pi && pi->nexthop != bnc)
    {
      ...
      if (CHECK_FLAG (bnc->flags, BGP_NEXTHOP_VALID) && bnc->metric)
        (bgp_path_info_extra_get (bpi_ultimate))->igpmetric = bnc->metric;
  ...
  if (bgp_route->inst_type == BGP_INSTANCE_TYPE_VIEW)
    return 1;
           ...
           pi->extra && pi->extra->num_labels && ! bnc->is_evpn_gwip_nexthop)
...
bgp_delete_connected_nexthop (afi_t afi, struct peer *peer)
{
  ...
  if (afi == AFI_IP6 && IN6_IS_ADDR_LINKLOCAL (&peer->su.sin6.sin6_addr))
    ifindex = peer->su.sin6.sin6_scope_id;
  bnc = bnc_find (&peer->bgp->nexthop_cache_table[family2afi (p.family)], &p,
                  0, ifindex);
  if (! bnc)
    {
      if (BGP_DEBUG (nht, NHT))
  ...
  if (bnc->nht_info != peer)
    {
      if (BGP_DEBUG (nht, NHT))
        zlog_debug ("Connected NHT %p node for peer %s(%s) points to %p", bnc,
                    peer->host, bnc->bgp->name_pretty, bnc->nht_info);
      return;
    }
  ...
  if (LIST_EMPTY (&(bnc->paths)))
    {
      if (BGP_DEBUG (nht, NHT))
        zlog_debug ("Freeing connected NHT node %p for peer %s(%s)", bnc,
                    peer->host, bnc->bgp->name_pretty);
      unregister_zebra_rnh (bnc);
      bnc_free (bnc);
    }
}
...
static void
bgp_process_nexthop_update (struct bgp_nexthop_cache *bnc,
                            struct zapi_route *nhr, bool import_check)
{
  struct nexthop *nexthop;
  ...
  if (BGP_DEBUG (nht, NHT))
    {
      ...
      zlog_debug (
          ...
          bnc->bgp->name_pretty, bnc->bgp->vrf_id, &nhr->prefix, bnc->ifindex,
          bnc->srte_color, nhr->metric, bnc->metric, nhr->nexthop_num,
          bnc->nexthop_num,
          ...
          if (BGP_DEBUG (nht, NHT))
            {
              ...
              zlog_debug ("    nhop via %s (%d labels)",
      ...
      if (bnc->is_evpn_gwip_nexthop)
        {
          ...
          if (BGP_DEBUG (nht, NHT))
            zlog_debug ("EVPN gateway IP %pFX recursive MAC/IP lookup %s",
                        &bnc->prefix,
...
static void
bgp_nht_ifp_table_handle (struct bgp *bgp,
...
static void
bgp_nht_ifp_handle (struct interface *ifp, bool up)
{
  struct bgp *bgp;
  ...
  bgp_nht_ifp_table_handle (bgp, &bgp->nexthop_cache_table[AFI_IP], ifp, up);
  bgp_nht_ifp_table_handle (bgp, &bgp->import_check_table[AFI_IP], ifp, up);
  bgp_nht_ifp_table_handle (bgp, &bgp->nexthop_cache_table[AFI_IP6], ifp, up);
  bgp_nht_ifp_table_handle (bgp, &bgp->import_check_table[AFI_IP6], ifp, up);
}

void
bgp_nht_ifp_up (struct interface *ifp)
{
  bgp_nht_ifp_handle (ifp, true);
}

void
bgp_nht_ifp_down (struct interface *ifp)
{
  bgp_nht_ifp_handle (ifp, false);
}

static void
bgp_nht_ifp_initial (struct event *thread)
{
 ...
 */
void
bgp_nht_interface_events (struct peer *peer)
{
  ...
  if (bnc->ifindex)
    event_add_event (bm->master, bgp_nht_ifp_initial, bnc->bgp, bnc->ifindex,
                     NULL);
}
  ...
  if (nhr.srte_color == 0 && bnc_nhc)
    {
      ...
      frr_each (bgp_nexthop_cache, &bgp->nexthop_cache_table[afi], bnc_iter)
        {
 ...
 */
void
bgp_cleanup_nexthops (struct bgp *bgp)
{
  for (afi_t afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      struct bgp_nexthop_cache *bnc;

      frr_each (bgp_nexthop_cache, &bgp->nexthop_cache_table[afi], bnc)
        {
 ...
 */
static int
make_prefix (int afi, struct bgp_path_info *pi, struct prefix *p)
{
  ...
  if (p_orig->family == AF_FLOWSPEC)
    {
      if (! pi->peer)
        return -1;
      return bgp_flowspec_get_first_nh (pi->peer->bgp, pi, p, afi);
    }
    ...
    case AFI_IP6:
      ...
      if (is_bgp_static)
      ...
      else
        {
           ...
           */
          if (pi->attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV6_GLOBAL_AND_LL &&
...
sendmsg_zebra_rnh (struct bgp_nexthop_cache *bnc, int command)
{
  ...
  ret = zclient_send_rnh (zclient, command, &bnc->prefix, SAFI_UNICAST,
                          exact_match, resolve_via_default, bnc->bgp->vrf_id);
 ...
 */
static void
register_zebra_rnh (struct bgp_nexthop_cache *bnc)
{
 ...
 */
static void
unregister_zebra_rnh (struct bgp_nexthop_cache *bnc)
{
  /* Check if we have already registered */
  ...
  if (BGP_DEBUG (nht, NHT))
    {
      ...
      zlog_debug (
          ...
          &bnc->prefix, bnc->ifindex, bnc->srte_color, bnc->bgp->name_pretty,
          bgp_nexthop_dump_bnc_flags (bnc, bnc_buf, sizeof (bnc_buf)),
  ...
  LIST_FOREACH (path, &(bnc->paths), nh_thread)
    {
      ...
      if (safi == SAFI_UNICAST && path->sub_type == BGP_ROUTE_IMPORTED &&
          ...
          (path->attr->evpn_overlay.type != OVERLAY_INDEX_GATEWAY_IP))
        {
          bnc_is_valid_nexthop =
              bgp_isvalid_nexthop_for_mpls (bnc, path) ? true : false;
        }
      else
        {
          if (bgp_update_martian_nexthop (bnc->bgp, afi, safi, path->type,
          ...
          if (dest->pdest)
            {
              ...
              prefix_rd2str (
                  ...
                  rd_buf, sizeof (rd_buf), bgp_get_asnotation (bnc->bgp));
              zlog_debug ("... eval path %d/%d %pBD RD %s %s flags 0x%x", afi,
                          safi, dest, rd_buf, bgp_path->name_pretty,
                          path->flags);
            }
          else
            zlog_debug ("... eval path %d/%d %pBD %s flags 0x%x", afi, safi,
                        dest, bgp_path->name_pretty, path->flags);
        }
      ...
      if (path->type == ZEBRA_ROUTE_BGP &&
         ...
         */
        vpn_leak_from_vrf_update (bgp_get_default (), bgp_path, path);
      else if (path->sub_type == BGP_ROUTE_REDISTRIBUTE &&
               safi == SAFI_UNICAST &&
          ...
          if (path_valid)
            {
               ...
               */
              bgp_aggregate_decrement (bgp_path, p, path, afi, safi);
              bgp_path_info_unset_flag (dest, path, BGP_PATH_VALID);
              ...
              if (safi == SAFI_UNICAST &&
                ...
                vpn_leak_from_vrf_withdraw (bgp_get_default (), bgp_path,
                                            path);
            }
          else
            {
               ...
               */
              bgp_path_info_set_flag (dest, path, BGP_PATH_VALID);
              bgp_aggregate_increment (bgp_path, p, path, afi, safi);
              if (safi == SAFI_EVPN && bgp_evpn_is_prefix_nht_supported (
                                           bgp_dest_get_prefix (dest)))
 ...
 */
void
path_nh_map (struct bgp_path_info *path, struct bgp_nexthop_cache *bnc,
             bool make)
{
  if (path->nexthop)
    {
 ...
 */
void
bgp_nht_register_nexthops (struct bgp *bgp)
{
  for (afi_t afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      struct bgp_nexthop_cache *bnc;
...
void
bgp_nht_reg_enhe_cap_intfs (struct peer *peer)
{
  struct bgp *bgp;
  struct bgp_nexthop_cache *bnc;
  struct nexthop *nhop;
  ...
  if (! sockunion2hostprefix (&peer->su, &p))
    {
      zlog_warn ("%s: Unable to convert sockunion to prefix for %s", __func__,
                 peer->host);
      return;
    }
  ...
  for (nhop = bnc->nexthop; nhop; nhop = nhop->next)
    {
      ...
      zclient_send_interface_radv_req (zclient, nhop->vrf_id, ifp, true,
                                       BGP_UNNUM_DEFAULT_RA_INTERVAL);
    }
}

void
bgp_nht_dereg_enhe_cap_intfs (struct peer *peer)
{
  struct bgp *bgp;
  struct bgp_nexthop_cache *bnc;
  struct nexthop *nhop;
  ...
  for (nhop = bnc->nexthop; nhop; nhop = nhop->next)
    {
      ...
      zclient_send_interface_radv_req (zclient, nhop->vrf_id, ifp, 0, 0);
    }
}
...
static void
bgp_l3nhg_add_cb (const char *name)
{
}

static void
bgp_l3nhg_modify_cb (const struct nexthop_group_cmd *nhgc)
{
}

static void
bgp_l3nhg_add_nexthop_cb (const struct nexthop_group_cmd *nhgc,
                          const struct nexthop *nhop)
{
}

static void
bgp_l3nhg_del_nexthop_cb (const struct nexthop_group_cmd *nhgc,
                          const struct nexthop *nhop)
{
}

static void
bgp_l3nhg_del_cb (const char *name)
{
}

static void
bgp_l3nhg_zebra_init (void)
{
...
void
bgp_l3nhg_finish (void)
{
  bf_free (bgp_nh_id_bitmap);
}

uint32_t
bgp_l3nhg_id_alloc (void)
{
  uint32_t nhg_id = 0;
...
void
bgp_l3nhg_id_free (uint32_t nhg_id)
{
