...
int
bgp_nexthop_cache_compare (const struct bgp_nexthop_cache *a,
...
bnc_new (struct bgp_nexthop_cache_head *tree, struct prefix *prefix,
...
bool
bnc_existing_for_prefix (struct bgp_nexthop_cache *bnc)
{
...
struct bgp_nexthop_cache *
bnc_find (struct bgp_nexthop_cache_head *tree, struct prefix *prefix,
          uint32_t srte_color, ifindex_t ifindex)
{
  struct bgp_nexthop_cache bnc = {};
...
static void
bgp_nexthop_cache_reset (struct bgp_nexthop_cache_head *tree)
{
  struct bgp_nexthop_cache *bnc;
...
static void
bgp_tip_hash_free (void *addr)
{
  XFREE (MTYPE_TIP_ADDR, addr);
}

static unsigned int
bgp_tip_hash_key_make (const void *p)
{
...
static bool
bgp_tip_hash_cmp (const void *p1, const void *p2)
{
...
void
bgp_tip_hash_init (struct bgp *bgp)
{
  bgp->tip_hash =
      hash_create (bgp_tip_hash_key_make, bgp_tip_hash_cmp, "BGP TIP hash");
}

void
bgp_tip_hash_destroy (struct bgp *bgp)
{
 ...
 */
bool
bgp_tip_add (struct bgp *bgp, struct in_addr *tip)
{
  struct tip_addr tmp;
  struct tip_addr *addr;
...
void
bgp_tip_del (struct bgp *bgp, struct in_addr *tip)
{
  struct tip_addr tmp;
  struct tip_addr *addr;
...
static void
show_address_entry (struct hash_bucket *bucket, void *args)
{
  ...
  vty_out (
      vty, "addr: %s, count: %d : ",
      inet_ntop (addr->p.family, &(addr->p.u.prefix), str, INET6_ADDRSTRLEN),
...
void
bgp_nexthop_show_address_hash (struct vty *vty, struct bgp *bgp)
{
...
static void
bgp_address_hash_string_del (void *val)
{
  char *data = val;
...
static void
bgp_address_hash_free (void *data)
{
...
static void
bgp_address_add (struct bgp *bgp, struct connected *ifc, struct prefix *p)
{
  struct bgp_addr tmp;
...
static void
bgp_address_del (struct bgp *bgp, struct connected *ifc, struct prefix *p)
{
  struct bgp_addr tmp;
  struct bgp_addr *addr;
  struct listnode *node;
...
static void
bgp_connected_cleanup (struct route_table *table, struct route_node *rn)
{
  struct bgp_connected_ref *bc;
...
bool
bgp_nexthop_self (struct bgp *bgp, afi_t afi, uint8_t type, uint8_t sub_type,
                  struct attr *attr, struct bgp_dest *dest)
{
  ...
  bool is_bgp_static_route =
      ((type == ZEBRA_ROUTE_BGP) && (sub_type == BGP_ROUTE_STATIC)) ? true
                                                                    : false;
      ...
      if (is_bgp_static_route)
      ...
      else
        {
          ...
          if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_NEXT_HOP))
            ...
            {
              tmp_addr.p.u.prefix4 = attr->mp_nexthop_global_in;
              tmp_addr.p.prefixlen = IPV4_MAX_BITLEN;
            }
          else
            return false;
...
bool
bgp_multiaccess_check_v6 (struct in6_addr nexthop, struct peer *peer)
{
  struct bgp_dest *dest1;
  struct bgp_dest *dest2;
  struct prefix p;
...
bool
bgp_subgrp_multiaccess_check_v6 (struct in6_addr nexthop,
...
bool
bgp_subgrp_multiaccess_check_v4 (struct in_addr nexthop,
                                 struct update_subgroup *subgrp,
                                 struct peer *exclude)
{
...
static void
bgp_show_bgp_path_info_flags (uint32_t flags, json_object *json)
{
...
static void
bgp_show_nexthop_paths (struct vty *vty, struct bgp *bgp,
                        struct bgp_nexthop_cache *bnc, json_object *json)
{
  struct bgp_dest *dest;
      ...
      if (json)
        {
          ...
          if (dest->pdest)
            json_object_string_addf (
                json_path, "rd", BGP_RD_AS_FORMAT (bgp->asnotation),
...
static void
bgp_show_nexthops_detail (struct vty *vty, struct bgp *bgp,
                          struct bgp_nexthop_cache *bnc, json_object *json)
{
  ...
  for (nexthop = bnc->nexthop; nexthop; nexthop = nexthop->next)
    {
      if (json)
        {
          ...
          switch (nexthop->type)
            ...
            case NEXTHOP_TYPE_IPV6_IFINDEX:
              json_object_string_addf (json_gate, "ip", "%pI6",
                                       &nexthop->gate.ipv6);
              json_object_string_add (json_gate, "interfaceName",
            ...
            case NEXTHOP_TYPE_BLACKHOLE:
              json_object_boolean_true_add (json_gate, "unreachable");
              switch (nexthop->bh_type)
                {
                case BLACKHOLE_REJECT:
                  json_object_boolean_true_add (json_gate, "reject");
                  break;
                case BLACKHOLE_ADMINPROHIB:
                  json_object_boolean_true_add (json_gate, "adminProhibited");
                  break;
                case BLACKHOLE_NULL:
                  json_object_boolean_true_add (json_gate, "blackhole");
                  break;
      ...
      switch (nexthop->type)
        ...
        case NEXTHOP_TYPE_IPV6_IFINDEX:
          ...
          if (nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX && bnc->ifindex)
            vty_out (vty, ", if %s\n",
                     ifindex2ifname (bnc->ifindex, bgp->vrf_id));
        ...
        case NEXTHOP_TYPE_IPV4_IFINDEX:
          ...
          if (nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX && bnc->ifindex)
            vty_out (vty, ", if %s\n",
                     ifindex2ifname (bnc->ifindex, bgp->vrf_id));
          else if (nexthop->ifindex)
            vty_out (vty, ", if %s\n",
        ...
        case NEXTHOP_TYPE_IFINDEX:
          vty_out (
              vty, "  if %s\n",
              ifindex2ifname (bnc->ifindex ? bnc->ifindex : nexthop->ifindex,
                              bgp->vrf_id));
          break;
        case NEXTHOP_TYPE_BLACKHOLE:
...
static void
bgp_show_nexthop (struct vty *vty, struct bgp *bgp,
                  ...
                  json_object *json)
{
  ...
  if (bnc->srte_color)
    {
      if (json)
        json_object_int_add (json_nexthop, "srteColor", bnc->srte_color);
      else
      ...
      if (json)
        {
          ...
          json_object_int_add (json_nexthop, "igpMetric", bnc->metric);
          json_object_int_add (json_nexthop, "pathCount", bnc->path_count);
          if (peer)
            json_object_string_add (json_nexthop, "peer", peer->host);
          if (bnc->is_evpn_gwip_nexthop)
            json_object_boolean_true_add (json_nexthop, "isEvpnGatewayIp");
        }
      else
        {
          vty_out (vty, " %s valid [IGP metric %d], #paths %d", buf,
      ...
      if (json)
        {
          ...
          if (CHECK_FLAG (bnc->flags, BGP_NEXTHOP_CONNECTED))
            json_object_boolean_false_add (json_nexthop, "isConnected");
          if (! CHECK_FLAG (bnc->flags, BGP_NEXTHOP_REGISTERED))
            json_object_boolean_false_add (json_nexthop, "isRegistered");
        }
      else
        {
          vty_out (vty, " %s invalid, #paths %d", buf, bnc->path_count);
          if (peer)
            vty_out (vty, ", peer %s", peer->host);
          if (bnc->is_evpn_gwip_nexthop)
            vty_out (vty, " EVPN Gateway IP");
...
static void
bgp_show_nexthops (struct vty *vty, struct bgp *bgp, bool import_table,
  ...
  if (afi == AFI_IP || afi == AFI_IP6)
    {
      ...
      if (found && json)
        json_object_object_add (json, (afi == AFI_IP) ? "ipv4" : "ipv6",
                                json_afi);
      return;
    }
...
static int
show_ip_bgp_nexthop_table (struct vty *vty, const char *name,
  ...
  if (nhopip_str)
    {
      ...
      tree =
          import_table ? &bgp->import_check_table : &bgp->nexthop_cache_table;
      if (json)
        json_afi = json_object_new_object ();
      frr_each (bgp_nexthop_cache, &(*tree)[family2afi (nhop.family)], bnc)
        {
      ...
      if (json)
        json_object_object_add (
            json, (family2afi (nhop.family) == AFI_IP) ? "ipv4" : "ipv6",
            json_afi);
      if (! found && ! json)
...
static void
bgp_show_all_instances_nexthops_vty (struct vty *vty, json_object *json,
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      ...
      bgp->connected_table[afi] = bgp_table_init (bgp, afi, SAFI_UNICAST);
    }
}
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      ...
      bgp->connected_table[afi]->route_table->cleanup = bgp_connected_cleanup;
...
char *
bgp_nexthop_dump_bnc_flags (struct bgp_nexthop_cache *bnc, char *buf,
                            size_t len)
{
  if (bnc->flags == 0)
    {
  ...
  snprintfrr (
      ...
      CHECK_FLAG (bnc->flags, BGP_STATIC_ROUTE_EXACT_MATCH) ? "Static Exact "
                                                            : "",
      CHECK_FLAG (bnc->flags, BGP_NEXTHOP_LABELED_VALID) ? "Label Valid "
                                                         : "");
...
char *
bgp_nexthop_dump_bnc_change_flags (struct bgp_nexthop_cache *bnc, char *buf,
                                   size_t len)
{
  if (bnc->flags == 0)
    {
