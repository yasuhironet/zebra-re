...
struct lcommunity *
lcommunity_parse (uint8_t *pnt, unsigned short length)
{
...
static void
set_lcommunity_string (struct lcommunity *lcom, bool make_json,
      ...
      if (make_json)
        {
          ...
          json_object_object_add (lcom->json, "list", json_lcommunity_list);
        }
  ...
  for (i = 0; i < lcom->size; i++)
    {
      ...
      snprintf (lcsb, sizeof (lcsb), "%u:%u:%u", global, local1, local2);
      ...
      if (individual_len + len > str_buf_sz)
        {
          ...
          str_buf = XREALLOC (MTYPE_LCOMMUNITY_STR, str_buf, str_buf_sz);
        }
      ...
      if (make_json)
        {
          ...
          json_object_array_add (json_lcommunity_list, json_string);
        }
    }
...
char *
lcommunity_str (struct lcommunity *lcom, bool make_json, bool translate_alias)
{
  if (! lcom)
    return NULL;
...
unsigned int
lcommunity_hash_make (const void *arg)
{
...
bool
lcommunity_include (struct lcommunity *lcom, uint8_t *ptr)
{
  int i;
  uint8_t *lcom_ptr;
  ...
  while (i < lcom1->size && j < lcom2->size)
    {
      if (memcmp (lcom1->val + (i * LCOMMUNITY_SIZE),
                  lcom2->val + (j * LCOMMUNITY_SIZE), LCOMMUNITY_SIZE) == 0)
        j++;
      i++;
    }
...
void
lcommunity_del_val (struct lcommunity *lcom, uint8_t *ptr)
{
  int i = 0;
  int c = 0;
...
static struct lcommunity *
bgp_aggr_lcommunity_lookup (struct bgp_aggregate *aggregate,
...
static void *
bgp_aggr_lcommunty_hash_alloc (void *p)
{
...
static void
bgp_aggr_lcommunity_prepare (struct hash_bucket *hb, void *arg)
{
  struct lcommunity *hb_lcommunity = hb->data;
  struct lcommunity **aggr_lcommunity = arg;

  if (*aggr_lcommunity)
    *aggr_lcommunity = lcommunity_merge (*aggr_lcommunity, hb_lcommunity);
  else
    *aggr_lcommunity = lcommunity_dup (hb_lcommunity);
}

void
bgp_aggr_lcommunity_remove (void *arg)
{
...
void
bgp_compute_aggregate_lcommunity (struct bgp_aggregate *aggregate,
                                  struct lcommunity *lcommunity)
{
                                       ...
                                       struct lcommunity *lcommunity)
{
  ...
  if (aggregate->lcommunity_hash == NULL)
    aggregate->lcommunity_hash =
        hash_create (lcommunity_hash_make, lcommunity_cmp,
  ...
  if (aggr_lcommunity == NULL)
    {
       ...
       */
      aggr_lcommunity = hash_get (aggregate->lcommunity_hash, lcommunity,
...
void
bgp_compute_aggregate_lcommunity_val (struct bgp_aggregate *aggregate)
{
  ...
  if (aggregate->lcommunity_hash && aggregate->lcommunity_hash->count)
    {
      hash_iterate (aggregate->lcommunity_hash, bgp_aggr_lcommunity_prepare,
...
void
bgp_remove_lcommunity_from_aggregate (struct bgp_aggregate *aggregate,
                                      struct lcommunity *lcommunity)
{
  struct lcommunity *aggr_lcommunity = NULL;
  struct lcommunity *ret_lcomm = NULL;

  if ((! aggregate) || (! aggregate->lcommunity_hash) || (! lcommunity))
    return;
  ...
  if (aggr_lcommunity)
    {
      ...
      if (aggr_lcommunity->refcnt == 0)
        {
          ret_lcomm =
              hash_release (aggregate->lcommunity_hash, aggr_lcommunity);
