...
DEFINE_MTYPE_STATIC (BGPD, BGP_RPKI_CACHE_GROUP,
...
static void revalidate_bgp_node (struct bgp_dest *dest, afi_t afi,
                                 safi_t safi);
...
static void *
realloc_wrapper (void *ptr, size_t size)
{
...
static void
init_tr_socket (struct cache *cache)
{
  if (cache->type == TCP)
...
static void
free_tr_socket (struct cache *cache)
{
  if (cache->type == TCP)
    tr_tcp_init (cache->tr_config.tcp_config, cache->tr_socket);
#if defined(FOUND_SSH)
...
static void
ipv6_addr_to_host_byte_order (const uint32_t *src, uint32_t *dest)
{
  int i;
...
route_match (void *rule, const struct prefix *prefix, void *object)
{
  ...
  if (rpki_validate_prefix (path->peer, path->attr, prefix) == *rpki_status)
    {
      return RMAP_MATCH;
    }
...
static void *
route_match_compile (const char *arg)
{
  int *rpki_status;
...
static void
route_match_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}

static struct rtr_socket *
create_rtr_socket (struct tr_socket *tr_socket)
{
  struct rtr_socket *rtr_socket =
...
static void
rpki_delete_all_cache_nodes (void)
{
...
static void
print_record (const struct pfx_record *record, struct vty *vty,
              json_object *json, enum asnotation_mode asnotation)
{
  ...
  if (! json)
  ...
  else
    {
      ...
      json_object_int_add (json_record, "prefixLenMin", record->min_len);
      json_object_int_add (json_record, "prefixLenMax", record->max_len);
...
static void
print_record_by_asn (const struct pfx_record *record, void *data)
{
...
static void
print_record_cb (const struct pfx_record *record, void *data)
{
  struct rpki_for_each_record_arg *arg = data;
  struct vty *vty = arg->vty;
...
static void
pfx_record_to_prefix (struct pfx_record *record, struct prefix *prefix)
{
...
static void
rpki_revalidate_prefix (struct event *thread)
{
...
static void
bgpd_sync_callback (struct event *thread)
{
  struct bgp *bgp;
  ...
  if (atomic_load_explicit (&rtr_update_overflow, memory_order_seq_cst))
    {
      while (read (rpki_sync_socket_bgpd, &rec, sizeof (struct pfx_record)) !=
             -1)
        ;
  ...
  int retval = read (rpki_sync_socket_bgpd, &rec, sizeof (struct pfx_record));
      ...
      for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
        {
          ...
          rrp->afi = afi;
          rrp->safi = safi;
          event_add_event (bm->master, rpki_revalidate_prefix, rrp, 0,
                           &bgp->t_revalidate[afi][safi]);
        }
    }
}

static void
revalidate_bgp_node (struct bgp_dest *bgp_dest, afi_t afi, safi_t safi)
{
...
static void
bgp_rpki_revalidate_peer (struct event *thread)
{
          ...
          FOREACH_AFI_SAFI (afi, safi)
            {
              ...
              rvp = XCALLOC (MTYPE_BGP_RPKI_REVALIDATE, sizeof (*rvp));
              rvp->peer = peer;
              rvp->afi = afi;
              rvp->safi = safi;

              event_add_event (bm->master, bgp_rpki_revalidate_peer, rvp, 0,
...
static void
rpki_update_cb_sync_rtr (struct pfx_table *p __attribute__ ((unused)),
                         ...
                         const bool added __attribute__ ((unused)))
{
  ...
  int retval = write (rpki_sync_socket_rtr, &rec, sizeof (struct pfx_record));
  if (retval == -1 && (errno == EAGAIN || errno == EWOULDBLOCK))
    atomic_store_explicit (&rtr_update_overflow, 1, memory_order_seq_cst);
...
static int
bgp_rpki_init (struct event_loop *master)
{
  rpki_debug = false;
...
start (void)
{
  ...
  ret = rtr_mgr_init (&rtr_config, groups, groups_len, polling_period,
                      expire_interval, retry_interval, rpki_update_cb_sync_rtr,
                      NULL, NULL, NULL);
  if (ret == RTR_ERROR)
    {
...
static void
print_prefix_table_by_asn (struct vty *vty, as_t as, json_object *json)
{
  ...
  if (! json)
    ...
    {
      json_object_int_add (json, "ipv4PrefixCount", number_of_ipv4_prefixes);
      json_object_int_add (json, "ipv6PrefixCount", number_of_ipv6_prefixes);
    }
...
static void
print_prefix_table (struct vty *vty, json_object *json)
{
  struct rpki_for_each_record_arg arg;
...
static int
add_tcp_cache (const char *host, const char *port, const uint8_t preference,
               const char *bindaddr)
{
  struct rtr_socket *rtr_socket;
  struct tr_tcp_config *tcp_config =
...
#if defined(FOUND_SSH)
static int
add_ssh_cache (const char *host, const unsigned int port, const char *username,
               const char *client_privkey_path, const char *server_pubkey_path,
               const uint8_t preference, const char *bindaddr)
{
  struct tr_ssh_config *ssh_config =
  ...
  if (cache->type == TCP)
    {
      ...
      XFREE (MTYPE_BGP_RPKI_CACHE, cache->tr_config.tcp_config->bindaddr);
  ...
  else
    {
      ...
      XFREE (MTYPE_BGP_RPKI_CACHE, cache->tr_config.ssh_config->username);
      XFREE (MTYPE_BGP_RPKI_CACHE,
             cache->tr_config.ssh_config->client_privkey_path);
      XFREE (MTYPE_BGP_RPKI_CACHE,
             cache->tr_config.ssh_config->server_hostkey_path);
      XFREE (MTYPE_BGP_RPKI_CACHE, cache->tr_config.ssh_config->bindaddr);
  ...
  for (ALL_LIST_ELEMENTS_RO (cache_list, cache_node, cache))
    {
      switch (cache->type)
        ...
        case SSH:
          ...
          vty_out (vty, " rpki cache %s %u %s %s %s ", ssh_config->host,
       ...
       "Preference value\n")
{
  ...
  if (ssh_uname)
    {
#if defined(FOUND_SSH)
      return_value = add_ssh_cache (cache, sshport, ssh_uname, ssh_privkey,
    ...
    { // use tcp connection
      return_value = add_tcp_cache (cache, tcpport, preference, bindaddr_str);
    }
       ...
       JSON_STR)
{
  ...
  if (pfx_table_validate_r (rtr_config->pfx_table, &matches, &match_count, asn,
                            &addr, prefix->prefixlen, &result) != PFX_SUCCESS)
    {
      if (! json)
      ...
      if (cache->type == TCP)
          ...
          if (! json)
            {
              vty_out (vty, "host: %s port: %s, preference: %hhu\n",
                       ...
                       cache->tr_config.tcp_config->port, cache->preference);
            }
          else
            {
              json_server = json_object_new_object ();
              json_object_string_add (json_server, "mode", "tcp");
      ...
      else if (cache->type == SSH)
        {
          if (! json)
            ...
            {
              json_server = json_object_new_object ();
              json_object_string_add (json_server, "mode", "ssh");
              json_object_string_add (json_server, "host",
                                      cache->tr_config.ssh_config->host);
        ...
        case TCP:
          ...
          if (! json)
          ...
          else
            {
              ...
              json_object_string_add (json_conn, "host", tcp_config->host);
              json_object_string_add (json_conn, "port", tcp_config->port);
              json_object_int_add (json_conn, "preference", cache->preference);
          ...
          if (! json)
            {
              vty_out (vty, "rpki ssh cache %s %u pref %hhu%s\n",
                       ssh_config->host, ssh_config->port, cache->preference,
                       cache->rtr_socket->state == RTR_ESTABLISHED
                           ? " (connected)"
                           : "");
            }
          else
            {
              ...
              json_object_string_add (json_conn, "host", ssh_config->host);
              json_object_int_add (json_conn, "port", ssh_config->port);
              json_object_int_add (json_conn, "preference", cache->preference);
              json_object_string_add (json_conn, "state",
                                      cache->rtr_socket->state ==
                                              RTR_ESTABLISHED
