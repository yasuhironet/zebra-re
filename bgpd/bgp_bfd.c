...
static void
bfd_session_status_update (struct bfd_session_params *bsp,
                           const struct bfd_session_status *bss, void *arg)
{
  struct peer *peer = arg;

  if (BGP_DEBUG (bfd, BFD_LIB))
    zlog_debug ("%s: neighbor %s vrf %s(%u) bfd state %s -> %s", __func__,
                peer->conf_if ? peer->conf_if : peer->host, bfd_sess_vrf (bsp),
                bfd_sess_vrf_id (bsp),
  ...
  if (bss->state == BSS_DOWN && bss->previous_state == BSS_UP)
    {
      ...
      if (BGP_IS_VALID_STATE_FOR_NOTIF (peer->status))
        bgp_notify_send (peer, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_BFD_DOWN);
...
void
bgp_peer_config_apply (struct peer *p, struct peer_group *pg)
{
  struct listnode *n;
  struct peer *pn;
  struct peer *gconfig;
  ...
  if (pg == NULL || pg->conf->bfd_config == NULL)
    {
      bfd_sess_set_timers (p->bfd_config->session,
                           ...
                           p->bfd_config->min_rx, p->bfd_config->min_tx);
      bfd_sess_set_cbit (p->bfd_config->session, p->bfd_config->cbit);
      bfd_sess_set_profile (p->bfd_config->session, p->bfd_config->profile);
  ...
  if (! p->bfd_config->cbit)
    bfd_sess_set_cbit (p->bfd_config->session, gconfig->bfd_config->cbit);
  else
    bfd_sess_set_cbit (p->bfd_config->session, p->bfd_config->cbit);
  ...
  if (p->bfd_config->detection_multiplier == BFD_DEF_DETECT_MULT ||
    ...
    bfd_sess_set_timers (
        p->bfd_config->session, gconfig->bfd_config->detection_multiplier,
        gconfig->bfd_config->min_rx, gconfig->bfd_config->min_tx);
  else
    bfd_sess_set_timers (p->bfd_config->session,
                         p->bfd_config->detection_multiplier,
                         p->bfd_config->min_rx, p->bfd_config->min_tx);
...
void
bgp_peer_bfd_update_source (struct peer *p)
{
  ...
  if (family == AF_INET)
    {
      if ((source && source->sin.sin_addr.s_addr != src.v4.s_addr) ||
        ...
        {
          if (BGP_DEBUG (bfd, BFD_LIB))
            zlog_debug ("%s: address [%pI4->%pI4] to [%pI4->%pI4]", __func__,
                        &src.v4, &dst.v4,
                        source ? &source->sin.sin_addr : &src.v4,
                        &p->su.sin.sin_addr);

          bfd_sess_set_ipv4_addrs (session,
                                   source ? &source->sin.sin_addr : NULL,
                                   &p->su.sin.sin_addr);
          changed = true;
        }
    }
  else
    {
      if ((source && memcmp (&source->sin6, &src.v6, sizeof (src.v6))) ||
        ...
        {
          if (BGP_DEBUG (bfd, BFD_LIB))
            zlog_debug ("%s: address [%pI6->%pI6] to [%pI6->%pI6]", __func__,
                        &src.v6, &dst.v6,
                        source ? &source->sin6.sin6_addr : &src.v6,
                        &p->su.sin6.sin6_addr);

          bfd_sess_set_ipv6_addrs (session,
                                   source ? &source->sin6.sin6_addr : NULL,
                                   &p->su.sin6.sin6_addr);
          changed = true;
        }
    }
  ...
  if (! PEER_IS_MULTIHOP (p) && bfd_sess_hop_count (session) > 1)
    {
      if (BGP_DEBUG (bfd, BFD_LIB))
        zlog_debug ("%s: TTL %d to 1", __func__, bfd_sess_hop_count (session));
  ...
  if (PEER_IS_MULTIHOP (p) && p->ttl != bfd_sess_hop_count (session))
    {
      if (BGP_DEBUG (bfd, BFD_LIB))
        zlog_debug ("%s: TTL %d to %d", __func__, bfd_sess_hop_count (session),
                    p->ttl);
 ...
 */
static void
bgp_peer_bfd_reset (struct peer *p)
{
  /* Set defaults. */
...
void
bgp_peer_configure_bfd (struct peer *p, bool manual)
{
...
static void
bgp_peer_remove_bfd (struct peer *p)
{
  /* Groups should not call this. */
  assert (! CHECK_FLAG (p->sflags, PEER_STATUS_GROUP));
...
static void
bgp_group_configure_bfd (struct peer *p)
{
  struct listnode *n;
  struct peer *pn;
...
static void
bgp_group_remove_bfd (struct peer *p)
{
  struct listnode *n;
  struct peer *pn;
...
void
bgp_peer_remove_bfd_config (struct peer *p)
{
  if (CHECK_FLAG (p->sflags, PEER_STATUS_GROUP))
 ...
 */
void
bgp_bfd_peer_config_write (struct vty *vty, const struct peer *peer,
                           const char *addr)
{
  /*
 ...
 */
void
bgp_bfd_show_info (struct vty *vty, const struct peer *peer,
...
void
bgp_bfd_init (struct event_loop *tm)
{
