...
static uint8_t *mplsL3vpnConfiguredVrfs (struct variable *, oid[], size_t *,
                                         int, size_t *, WriteMethod **);

static uint8_t *mplsL3vpnActiveVrfs (struct variable *, oid[], size_t *, int,
                                     size_t *, WriteMethod **);

static uint8_t *mplsL3vpnConnectedInterfaces (struct variable *, oid[],
                                              size_t *, int, size_t *,
                                              WriteMethod **);

static uint8_t *mplsL3vpnNotificationEnable (struct variable *, oid[],
                                             size_t *, int, size_t *,
                                             WriteMethod **);
...
static uint8_t *mplsL3vpnIllLblRcvThrsh (struct variable *, oid[], size_t *,
                                         int, size_t *, WriteMethod **);
...
static void
bgp_mpls_l3vpn_update_timeticks (time_t *counter)
{
  struct timeval tv;
...
static int
bgp_mpls_l3vpn_update_last_changed (struct bgp *bgp)
{
  if (bgp->snmp_stats)
...
static uint32_t
bgp_mpls_l3vpn_current_routes (struct bgp *l3vpn_bgp)
{
  uint32_t count = 0;
...
static int
bgp_init_snmp_stats (struct bgp *bgp)
{
  if (is_bgp_vrf_mplsvpn (bgp))
    {
      if (bgp->snmp_stats == NULL)
        {
          bgp->snmp_stats =
              XCALLOC (MTYPE_BGP, sizeof (struct bgp_snmp_stats));
          /* fix up added routes */
          if (bgp->snmp_stats)
            {
...
static int
bgp_snmp_update_route_stats (struct bgp_dest *dest, struct bgp_path_info *pi,
                             bool added)
{
  struct bgp_table *table;
...
static bool
is_bgp_vrf_active (struct bgp *bgp)
{
  struct vrf *vrf;
...
static int
bgp_vrf_check_update_active (struct bgp *bgp, struct interface *ifp)
{
  ...
  if (bgp->snmp_stats->active != new_active)
    {
      ...
      oid_copy_str (trap_index[1].indexname, bgp->name,
                    trap_index[1].indexlen);
      trap_index[0].indexlen = trap_index[1].indexlen + sizeof (ifindex_t);
      ...
      smux_trap_multi_index (
          ...
          mpls_l3vpn_oid, sizeof (mpls_l3vpn_oid) / sizeof (oid), trap_index,
          array_size (trap_index), l3vpn_trap_list,
...
static uint8_t *
mplsL3vpnConfiguredVrfs (struct variable *v, oid name[], size_t *length,
...
static uint8_t *
mplsL3vpnActiveVrfs (struct variable *v, oid name[], size_t *length, int exact,
...
static uint8_t *
mplsL3vpnConnectedInterfaces (struct variable *v, oid name[], size_t *length,
                              int exact, size_t *var_len,
                              WriteMethod **write_method)
{
  struct listnode *node, *nnode;
...
static int
write_mplsL3vpnNotificationEnable (int action, uint8_t *var_val,
...
static uint8_t *
mplsL3vpnNotificationEnable (struct variable *v, oid name[], size_t *length,
                             int exact, size_t *var_len,
                             WriteMethod **write_method)
{
  if (smux_header_generic (v, name, length, exact, var_len, write_method) ==
...
static uint8_t *
mplsL3vpnVrfConfMaxPossRts (struct variable *v, oid name[], size_t *length,
                            int exact, size_t *var_len,
                            WriteMethod **write_method)
{
  if (smux_header_generic (v, name, length, exact, var_len, write_method) ==
...
static uint8_t *
mplsL3vpnVrfConfRteMxThrshTime (struct variable *v, oid name[], size_t *length,
                                int exact, size_t *var_len,
                                WriteMethod **write_method)
{
  if (smux_header_generic (v, name, length, exact, var_len, write_method) ==
...
static uint8_t *
mplsL3vpnIllLblRcvThrsh (struct variable *v, oid name[], size_t *length,
                         int exact, size_t *var_len,
                         WriteMethod **write_method)
{
  if (smux_header_generic (v, name, length, exact, var_len, write_method) ==
      ...
      if (first || strncmp (bgp->name, vrf_name, VRF_NAMSIZ) > 0)
        {
          if (bgp_next == NULL)
            bgp_next = bgp;
          else if (strncmp (bgp->name, bgp_next->name, VRF_NAMSIZ) < 0)
            bgp_next = bgp;
        }
    }
  return bgp_next;
...
static struct bgp *
bgpL3vpnIfConf_lookup (struct variable *v, oid name[], size_t *length,
                       char *vrf_name, ifindex_t *ifindex, int exact)
{
  struct bgp *bgp = NULL;
          ...
          if (ifp)
            {
              ...
              *ifindex = ifp->ifindex;
              ...
              oid_copy_str (name + namelen, bgp->name, vrf_name_len);
              oid_copy_int (name + namelen + vrf_name_len, ifindex);
              *length = len + namelen;
...
static uint8_t *
mplsL3vpnIfConfTable (struct variable *v, oid name[], size_t *length,
                      int exact, size_t *var_len, WriteMethod **write_method)
{
  ...
  l3vpn_bgp =
      bgpL3vpnIfConf_lookup (v, name, length, vrf_name, &ifindex, exact);
  if (! l3vpn_bgp)
    return NULL;
...
static struct bgp *
bgpL3vpnVrf_lookup (struct variable *v, oid name[], size_t *length,
                    char *vrf_name, int exact)
{
  struct bgp *bgp = NULL;
...
static uint8_t *
mplsL3vpnVrfTable (struct variable *v, oid name[], size_t *length, int exact,
                   size_t *var_len, WriteMethod **write_method)
{
  char vrf_name[VRF_NAMSIZ];
  struct bgp *l3vpn_bgp;
  ...
  switch (v->magic)
    ...
    case MPLSL3VPNVRFRD:
      ...
      if (CHECK_FLAG (l3vpn_bgp->vpn_policy[AFI_IP].flags,
        ...
        prefix_rd2str (&l3vpn_bgp->vpn_policy[AFI_IP].tovpn_rd, rd_buf,
                       sizeof (rd_buf), bgp_get_asnotation (l3vpn_bgp));
      else if (CHECK_FLAG (l3vpn_bgp->vpn_policy[AFI_IP6].flags,
                           BGP_VPN_POLICY_TOVPN_RD_SET))
        prefix_rd2str (&l3vpn_bgp->vpn_policy[AFI_IP6].tovpn_rd, rd_buf,
                       sizeof (rd_buf), bgp_get_asnotation (l3vpn_bgp));

      *var_len = strnlen (rd_buf, RD_ADDRSTRLEN);
      return (uint8_t *) rd_buf;
    case MPLSL3VPNVRFCREATIONTIME:
      return SNMP_INTEGER ((uint32_t) l3vpn_bgp->snmp_stats->creation_time);
    ...
    case MPLSL3VPNVRFCONFLASTCHANGED:
      return SNMP_INTEGER ((uint32_t) l3vpn_bgp->snmp_stats->modify_time);
...
static struct bgp *
bgpL3vpnVrfRt_lookup (struct variable *v, oid name[], size_t *length,
                      char *vrf_name, uint32_t *rt_index, uint8_t *rt_type,
                      int exact)
{
  ...
  /* too long ? */
  if (*length - namelen > (VRF_NAMSIZ + sizeof (uint32_t)) + sizeof (uint8_t))
    return NULL;
          ...
          if (*rt_type)
            {
              ...
              import = (! ! l3vpn_bgp->vpn_policy[*rt_index]
                                .rtlist[BGP_VPN_POLICY_DIR_FROMVPN]);
              export = (! ! l3vpn_bgp->vpn_policy[*rt_index]
                                .rtlist[BGP_VPN_POLICY_DIR_TOVPN]);
              if (*rt_type == MPLSVPNVRFRTTYPEIMPORT && ! import)
                continue;
              if (*rt_type == MPLSVPNVRFRTTYPEEXPORT && ! export)
                continue;
              /* ckeck for both */
              if (*rt_type == MPLSVPNVRFRTTYPEIMPORT && import && export &&
                  ecommunity_cmp (l3vpn_bgp->vpn_policy[*rt_index]
                                      .rtlist[BGP_VPN_POLICY_DIR_FROMVPN],
                                  l3vpn_bgp->vpn_policy[*rt_index]
                                      .rtlist[BGP_VPN_POLICY_DIR_TOVPN]))
              ...
              len = vrf_name_len + sizeof (uint32_t) + sizeof (uint8_t);
              oid_copy_str (name + namelen, l3vpn_bgp->name, vrf_name_len);
              oid_copy_int (name + namelen + vrf_name_len, (int *) rt_index);
...
static uint8_t *
mplsL3vpnVrfRtTable (struct variable *v, oid name[], size_t *length, int exact,
                     size_t *var_len, WriteMethod **write_method)
{
  char vrf_name[VRF_NAMSIZ];
  struct bgp *l3vpn_bgp;
  ...
  switch (v->magic)
    ...
    case MPLSL3VPNVRFRT:
      switch (rt_type)
        {
        case MPLSVPNVRFRTTYPEIMPORT:
          rt_b = ecommunity_ecom2str (l3vpn_bgp->vpn_policy[rt_index]
                                          .rtlist[BGP_VPN_POLICY_DIR_FROMVPN],
        ...
        case MPLSVPNVRFRTTYPEBOTH:
          rt_b = ecommunity_ecom2str (
              l3vpn_bgp->vpn_policy[rt_index].rtlist[BGP_VPN_POLICY_DIR_TOVPN],
    ...
    case MPLSL3VPNVRFRTDESCR:
      ...
      snprintf (rt_description, VRF_NAMSIZ + RT_PREAMBLE_SIZE,
                "RT %s for VRF %s", rt_type2str (rt_type), l3vpn_bgp->name);
      *var_len = strnlen (rt_description, VRF_NAMSIZ + RT_PREAMBLE_SIZE);
...
static uint8_t *
mplsL3vpnPerfTable (struct variable *v, oid name[], size_t *length, int exact,
                    size_t *var_len, WriteMethod **write_method)
{
  char vrf_name[VRF_NAMSIZ];
  struct bgp *l3vpn_bgp;
...
bgp_lookup_route (struct bgp *l3vpn_bgp, struct bgp_dest **dest,
                  struct prefix *prefix, uint16_t policy,
  ...
  for (; pi; pi = pi->next)
    {
      switch (nexthop->ipa_type)
        {
        case IPADDR_V4:
          if (nexthop->ip._v4_addr.s_addr == pi->attr->nexthop.s_addr)
            return pi;
          break;
        case IPADDR_V6:
          if (memcmp (&nexthop->ip._v6_addr, &pi->attr->mp_nexthop_global,
...
static struct bgp_path_info *
bgp_lookup_route_next (struct bgp **l3vpn_bgp, struct bgp_dest **dest,
                       struct prefix *prefix, uint16_t *policy,
                       struct ipaddr *nexthop)
{
  struct bgp_path_info *pi = NULL;
  ...
  for (; *l3vpn_bgp; *l3vpn_bgp = bgp_lookup_by_name_next ((*l3vpn_bgp)->name))
    {
      *policy = 0;
      if (! *dest)
        {
...
static struct bgp_path_info *
bgpL3vpnRte_lookup (struct variable *v, oid name[], size_t *length,
                    char *vrf_name, struct bgp **l3vpn_bgp,
                    struct bgp_dest **dest, uint16_t *policy, int exact)
{
  uint8_t i;
  ...
  if (*length - namelen != 0)
    {
      ...
      switch (prefix_type)
        ...
        case INETADDRESSTYPEIPV4:
          ...
          oid2in_addr (&name[i], sizeof (struct in_addr), &prefix.u.prefix4);
      ...
      *policy |= name[i++];
      ...
      switch (nexthop_type)
        ...
        case INETADDRESSTYPEUNKNOWN:
          nexthop.ipa_type =
              (prefix.family == AF_INET) ? IPADDR_V4 : IPADDR_V6;
          break;
        case INETADDRESSTYPEIPV4:
  ...
  if (exact)
  ...
  else
    {
      ...
      pi = bgp_lookup_route_next (l3vpn_bgp, dest, &prefix, policy, &nexthop);
      if (pi)
        {
          uint8_t vrf_name_len = strnlen ((*l3vpn_bgp)->name, VRF_NAMSIZ);
          const struct prefix *p = bgp_dest_get_prefix (*dest);
          uint8_t oid_index;
          bool v4 = (p->family == AF_INET);
          uint8_t addr_len =
              v4 ? sizeof (struct in_addr) : sizeof (struct in6_addr);
          ...
          if (v4)
            {
              ...
              oid_copy_in_addr (&name[oid_index], &p->u.prefix4);
            }
          else
            {
              name[oid_index++] = INETADDRESSTYPEIPV6;
              oid_copy_in6_addr (&name[oid_index], &p->u.prefix6);
            }
          ...
          if (! BGP_ATTR_NEXTHOP_AFI_IP6 (attr))
            {
              if (attr->nexthop.s_addr == INADDR_ANY)
                name[oid_index++] = INETADDRESSTYPEUNKNOWN;
              else
                {
                  name[oid_index++] = INETADDRESSTYPEIPV4;
                  oid_copy_in_addr (&name[oid_index], &attr->nexthop);
                  oid_index += sizeof (struct in_addr);
                }
            }
          else
            {
              if (IN6_IS_ADDR_UNSPECIFIED (&attr->mp_nexthop_global))
                name[oid_index++] = INETADDRESSTYPEUNKNOWN;
              else
                {
                  name[oid_index++] = INETADDRESSTYPEIPV6;
...
static uint8_t *
mplsL3vpnRteTable (struct variable *v, oid name[], size_t *length, int exact,
                   size_t *var_len, WriteMethod **write_method)
{
  char vrf_name[VRF_NAMSIZ];
  struct bgp *l3vpn_bgp;
      ...
      if (pi->nexthop && pi->nexthop->nexthop)
        {
          switch (pi->nexthop->nexthop->type)
            ...
            case NEXTHOP_TYPE_IFINDEX:
              return SNMP_INTEGER (MPLSL3VPNVRFRTECIDRTYPELOCAL);
            case NEXTHOP_TYPE_IPV4:
            case NEXTHOP_TYPE_IPV4_IFINDEX:
            case NEXTHOP_TYPE_IPV6:
            case NEXTHOP_TYPE_IPV6_IFINDEX:
              return SNMP_INTEGER (MPLSL3VPNVRFRTECIDRTYPEREMOTE);
            case NEXTHOP_TYPE_BLACKHOLE:
              switch (pi->nexthop->nexthop->bh_type)
                {
                case BLACKHOLE_REJECT:
                  return SNMP_INTEGER (MPLSL3VPNVRFRTECIDRTYPEREJECT);
                case BLACKHOLE_UNSPEC:
                case BLACKHOLE_NULL:
                case BLACKHOLE_ADMINPROHIB:
                  return SNMP_INTEGER (MPLSL3VPNVRFRTECIDRTYPEBLACKHOLE);
                }
              break;
            }
        }
...
void
bgp_mpls_l3vpn_module_init (void)
{
