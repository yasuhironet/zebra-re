 ...
 */
void
bgp_fs_nlri_get_string (unsigned char *nlri_content, size_t len,
  ...
  while (offset < len - 1 && error >= 0)
    {
      ...
      switch (type)
        ...
        case FLOWSPEC_SRC_PREFIX:
          ret = bgp_flowspec_ip_address (type_util, nlri_content + offset,
                                         len - offset, local_string, &error,
                                         afi, NULL);
          if (ret <= 0)
            break;
          if (json_path)
            {
              json_object_string_add (
                  json_path, lookup_msg (bgp_flowspec_display, type, ""),
                  local_string);
              break;
            }
          FS_STRING_UPDATE (count, ptr, format, len_string);
          len_written = snprintf (ptr, len_string, "%s%s %s%s", pre_extra,
        ...
        case FLOWSPEC_ICMP_CODE:
          ret = bgp_flowspec_op_decode (type_util, nlri_content + offset,
                                        len - offset, local_string, &error);
          if (ret <= 0)
            break;
          if (json_path)
        ...
        case FLOWSPEC_TCP_FLAGS:
          ret =
              bgp_flowspec_bitmask_decode (type_util, nlri_content + offset,
                                           len - offset, local_string, &error);
          if (ret <= 0)
            break;
          if (json_path)
...
void
route_vty_out_flowspec (struct vty *vty, const struct prefix *p,
  ...
  if (path->attr && (bgp_attr_get_ecommunity (path->attr) || ipv6_ecomm))
    {
      ...
      if (ipv6_ecomm)
        s2 = ecommunity_ecom2str (ipv6_ecomm, ECOMMUNITY_FORMAT_ROUTE_MAP, 0);
      if (! s1 && ! s2)
        return;
      if (display == NLRI_STRING_FORMAT_LARGE)
        vty_out (vty, "\t%s%s%s\n", s1 ? s1 : "", s2 && s1 ? " " : "",
      ...
      else if (json_paths)
        {
          ...
          if (s1)
            json_object_string_add (json_ecom_path, "ecomlist", s1);
          if (s2)
            json_object_string_add (json_ecom_path, "ecom6list", s2);
          if (display == NLRI_STRING_FORMAT_JSON)
            json_object_array_add (json_paths, json_ecom_path);
        }
      if (display == NLRI_STRING_FORMAT_LARGE)
        {
      ...
      if (extra->bgp_fs_pbr && listcount (extra->bgp_fs_pbr))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (extra->bgp_fs_pbr, node, bpme))
            {
      ...
      if (extra->bgp_fs_iprule && listcount (extra->bgp_fs_iprule))
        {
          ...
          if (! list_began)
            vty_out (vty, "\tinstalled in PBR");
          for (ALL_LIST_ELEMENTS_RO (extra->bgp_fs_iprule, node, bpr))
            {
              if (! bpr->action)
                continue;
              if (! list_began)
                ...
                vty_out (vty, ", ");
              vty_out (vty, "-ipv4-rule %d action lookup %u-", bpr->priority,
                       bpr->action->table_id);
            }
        }
      if (list_began)
...
int
bgp_show_table_flowspec (struct vty *vty, struct bgp *bgp, afi_t afi,
  ...
  for (dest = bgp_table_top (table); dest; dest = bgp_route_next (dest))
    {
      ...
      for (; pi; pi = pi->next)
        {
          total_count++;
          route_vty_out_flowspec (vty, bgp_dest_get_prefix (dest), pi, display,
                                  json_paths);
        }
      if (use_json)
        {
...
int
bgp_fs_config_write_pbr (struct vty *vty, struct bgp *bgp, afi_t afi,
                         safi_t safi)
{
...
static int
bgp_fs_local_install_interface (struct bgp *bgp, const char *no,
       ...
       "Interface name\n")
{
  ...
  char *ifname =
      argv_find (argv, argc, "INTERFACE", &idx) ? argv[idx]->arg : NULL;

  return bgp_fs_local_install_interface (bgp, no, ifname, bgp_node_afi (vty));
}

extern int
bgp_flowspec_display_match_per_ip (afi_t afi, struct bgp_table *rib,
                                   struct prefix *match, int prefix_check,
                                   struct vty *vty, bool use_json,
                                   json_object *json_paths)
{
  struct bgp_dest *dest;
      ...
      if (bgp_flowspec_contains_prefix (prefix, match, prefix_check))
        {
          route_vty_out_flowspec (
              ...
              use_json ? NLRI_STRING_FORMAT_JSON : NLRI_STRING_FORMAT_LARGE,
              json_paths);
          display++;
        }
    }
