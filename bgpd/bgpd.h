...
#define BGP_FLAG_LU_IPV6_EXPLICIT_NULL (1ULL << 34)
  ...
  enum global_mode GLOBAL_GR_FSM[BGP_GLOBAL_GR_MODE][BGP_GLOBAL_GR_EVENT_CMD];
...
#define BGP_VPNVX_RETAIN_ROUTE_TARGET_ALL (1 << 11)
  ...
  /* Tree for next-hop lookup cache. */
  struct bgp_label_per_nexthop_cache_head mpls_labels_per_nexthop[AFI_MAX];
...
DECLARE_HOOK (bgp_inst_config_write, (struct bgp * bgp, struct vty *vty),
              (bgp, vty));
...
#define BGP_SUPPRESS_FIB_ENABLED(bgp)                                         \
  (CHECK_FLAG (bgp->flags, BGP_FLAG_SUPPRESS_FIB_PENDING) || bm->wait_for_fib)
...
#define PEER_CAP_RESTART_AF_RCV                                               \
  (1U << 6) /* graceful restart afi/safi received */
#define PEER_CAP_RESTART_AF_PRESERVE_RCV                                      \
  (1U << 7) /* graceful restart afi/safi F-bit received */
...
#define PEER_CAP_ENHE_AF_ADV                                                  \
  (1U << 12) /* Extended nexthopi afi/safi advertised */
#define PEER_CAP_ENHE_AF_RCV                                                  \
  (1U << 13) /* Extended nexthop afi/safi received */
#define PEER_CAP_ENHE_AF_NEGO                                                 \
  (1U << 14) /* Extended nexthop afi/safi negotiated */
...
#define PEER_FLAG_DISABLE_CONNECTED_CHECK                                     \
  (1ULL << 6)                                     /* disable-connected-check */
#define PEER_FLAG_LOCAL_AS_NO_PREPEND (1ULL << 7) /* local-as no-prepend */
#define PEER_FLAG_LOCAL_AS_REPLACE_AS                                         \
  (1ULL << 8)                              /* local-as no-prepend replace-as */
...
#define PEER_FLAG_CAPABILITY_ENHE                                             \
  (1ULL << 13) /* Extended next-hop (rfc 5549)*/
...
#define PEER_FLAG_GRACEFUL_RESTART_GLOBAL_INHERIT                             \
  (1ULL << 25)                                 /* Global-Inherit */
...
peer_group_lookup_dynamic_neighbor_range (struct peer_group *,
                                          struct prefix *);
extern struct peer_group *
peer_group_lookup_dynamic_neighbor (struct bgp *, struct prefix *,
                                    struct prefix **);
...
extern struct peer *peer_create (union sockunion *su, const char *conf_if,
                                 struct bgp *bgp, as_t local_as,
                                 as_t remote_as, int as_type,
...
extern void bgp_timers_set (struct bgp *, uint32_t keepalive,
...
extern int peer_remote_as (struct bgp *bgp, union sockunion *su,
                           ...
                           const char *as_str);
extern int peer_group_remote_as (struct bgp *bgp, const char *peer_str,
...
extern int peer_default_originate_set (struct peer *peer, afi_t afi,
...
extern int peer_distribute_set (struct peer *, afi_t, safi_t, int,
                                const char *);
...
extern void peer_maximum_prefix_out_refresh_routes (struct peer *peer,
  ...
  do                                                                          \
    {                                                                         \
      ...
      for (ALL_LIST_ELEMENTS_RO (_peer_list, node, peer_loop))                \
        {                                                                     \
          if (CHECK_FLAG (peer_loop->flags, PEER_FLAG_GRACEFUL_RESTART))      \
...
peer_afi_active_nego (const struct peer *peer, afi_t afi)
{
  if (peer->afc_nego[afi][SAFI_UNICAST] ||
      ...
      peer->afc_nego[afi][SAFI_MPLS_VPN] || peer->afc_nego[afi][SAFI_ENCAP] ||
      peer->afc_nego[afi][SAFI_FLOWSPEC] || peer->afc_nego[afi][SAFI_EVPN])
    return 1;
  return 0;
}
...
peer_group_af_configured (struct peer_group *group)
{
  ...
  if (peer->afc[AFI_IP][SAFI_UNICAST] || peer->afc[AFI_IP][SAFI_MULTICAST] ||
      ...
      peer->afc[AFI_IP][SAFI_FLOWSPEC] || peer->afc[AFI_IP][SAFI_MPLS_VPN] ||
...
static inline bool
peer_dynamic_neighbor_no_nsf (struct peer *peer)
{
...
bgp_vrf_interfaces (struct bgp *bgp, bool active)
{
  struct vrf *vrf;
  struct interface *ifp;
  uint32_t count = 0;
...
static inline bool
bgp_in_graceful_shutdown (struct bgp *bgp)
{
...
/* Hooks */
DECLARE_HOOK (bgp_vrf_status_changed,
              (struct bgp * bgp, struct interface *ifp), (bgp, ifp));
