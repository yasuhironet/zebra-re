      ...
      for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
        {
              ...
              if (use_json)
                {
                  ...
                  if (type == RD_TYPE_AS || type == RD_TYPE_AS4)
                    snprintf (rd_str, sizeof (rd_str), "%u:%d", rd_as.as,
                              rd_as.val);
                  else if (type == RD_TYPE_IP)
                    snprintfrr (rd_str, sizeof (rd_str), "%pI4:%d", &rd_ip.ip,
                                rd_ip.val);
                  json_object_string_add (json_routes, "rd", rd_str);
                }
              else
                {
                  ...
                  if (type == RD_TYPE_AS || type == RD_TYPE_AS4)
                    ...
                    vty_out (vty, "%pI4:%d", &rd_ip.ip, rd_ip.val);
#ifdef ENABLE_BGP_VNC
                  else if (type == RD_TYPE_VNC_ETH)
          ...
          route_vty_out_tmp (vty, rm, bgp_dest_get_prefix (rm), attr, safi,
