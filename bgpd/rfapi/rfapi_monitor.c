 ...
 */
void
rfapiMonitorEthSlCheck (struct agg_node *rn, const char *tag1,
                        const char *tag2)
{
  ...
  if (sl || sl_saved)
    {
      vnc_zlog_debug_verbose (
          "%s[%s%s]: rn=%p, rn->lock=%d, old sl=%p, new sl=%p", __func__,
          (tag1 ? tag1 : ""), (tag2 ? tag2 : ""), rn, rn->lock, sl_saved, sl);
      sl_saved = sl;
    }
}
#endif
  ...
  for (ALL_LIST_ELEMENTS_RO (&bgp->rfapi->descriptors, hnode, rfd))
    {
      ...
      for (rn = agg_route_top (rfd->import_table->imported_vpn[AFI_IP]); rn;
           rn = agg_route_next (rn))
        {

          assert (! RFAPI_MONITOR_VPN (rn));
        }
      for (rn = agg_route_top (rfd->import_table->imported_vpn[AFI_IP6]); rn;
           rn = agg_route_next (rn))
        {
...
void
rfapiMonitorExtraFlush (safi_t safi, struct agg_node *rn)
{
 ...
 */
void
rfapiMonitorExtraPrune (safi_t safi, struct agg_node *rn)
{
  struct rfapi_it_extra *hie;
...
rfapiMonitorGetAttachNode (struct rfapi_descriptor *rfd, struct prefix *p)
{
   ...
   */
  for (rn = agg_node_match (rfd->import_table->imported_vpn[afi], p); rn;)
    {
  ...
  if (! rn)
    {
       ...
       */
      rn = agg_node_get (rfd->import_table->imported_vpn[afi], &pfx_default);
    }
  ...
  if (RFAPI_0_PREFIX (&m->p))
    {
      ...
      vnc_zlog_debug_verbose ("%s: attached monitor %p to vpn0 list", __func__,
                              m);
      return NULL;
    }
 ...
 */
struct agg_node *
rfapiMonitorAdd (struct bgp *bgp, struct rfapi_descriptor *rfd,
                 struct prefix *p)
{
  struct rfapi_monitor_vpn *m;
  struct agg_node *rn;
...
rfapiMonitorDetachImport (struct rfapi_monitor_vpn *m)
{
  ...
  if (RFAPI_0_PREFIX (&m->p))
  ...
  else
    {
      ...
      if (m->node)
        {
          for (prev = NULL, this = RFAPI_MONITOR_VPN (m->node); this;
...
void
rfapiMonitorDel (struct bgp *bgp, struct rfapi_descriptor *rfd,
                 struct prefix *p)
{
  struct agg_node *rn;
  struct rfapi_monitor_vpn *m;
...
static void
rfapiMonitorTimerExpire (struct event *t)
{
 ...
 */
void
rfapiMonitorTimersRestart (struct rfapi_descriptor *rfd,
  ...
  if (AF_ETHERNET == p->family)
    {
       ...
       */
      for (cursor = NULL,
          rc = skiplist_next (rfd->mon_eth, NULL, (void **) &mon_eth, &cursor);
           rc == 0; rc = skiplist_next (rfd->mon_eth, NULL, (void **) &mon_eth,
                                        &cursor))
        {
      ...
      do
        {
           ...
           */
          if (! agg_node_parent (rn) && ! rn->info && it_node->parent)
            break;
 ...
 */
#if DEBUG_L2_EXTRA
              vnc_zlog_debug_verbose ("%s: found L2 all-routes monitor %p",
                                      __func__, e);
#endif
 ...
 */
void
rfapiMonitorMovedUp (struct rfapi_import_table *import_table,
                     ...
                     struct rfapi_monitor_vpn *monitor_list)
{
  ...
  if (! new_node->parent && ! new_node->info)
    {
      vnc_zlog_debug_verbose (
          "%s: new monitor at 0/0 and no routes, no updates", __func__);
      return;
    }
...
static void
rfapiMonitorEthTimerExpire (struct event *t)
{
...
static int
mon_eth_cmp (const void *a, const void *b)
{
  ...
  if (RFAPI_0_ETHERADDR (&mon->macaddr))
    {
      ...
      it->eth0_queries = mon;
#if DEBUG_L2_EXTRA
      vnc_zlog_debug_verbose ("%s: attached monitor %p to eth0 list", __func__,
                              mon);
#endif
      return;
    }
...
#if DEBUG_L2_EXTRA
  vnc_zlog_debug_verbose (
      "%s: rn=%p, rn->lock=%d, sl=%p, attaching eth mon %p", __func__, rn,
      rn->lock, sl, mon);
#endif
...
struct agg_node *
rfapiMonitorEthAdd (struct bgp *bgp, struct rfapi_descriptor *rfd,
                    struct ethaddr *macaddr, uint32_t logical_net_id)
{
...

  {
    ...
    vnc_zlog_debug_verbose (
        "%s: LNI=%d: rfd=%p, pfx=%s", __func__, logical_net_id, rfd,
        rfapi_ntop (pfx_mac_buf.family, pfx_mac_buf.u.val, buf, BUFSIZ));
  }
  ...
  if (! rc)
    {
       ...
       */
      vnc_zlog_debug_verbose (
          "%s: already present in rfd->mon_eth, not adding", __func__);
...
#if DEBUG_L2_EXTRA
  vnc_zlog_debug_verbose ("%s: inserted rfd=%p mon_eth=%p, rc=%d", __func__,
                          rfd, val, rc);
#else
  (void) rc;
#endif
...
void
rfapiMonitorEthDel (struct bgp *bgp, struct rfapi_descriptor *rfd,
                    struct ethaddr *macaddr, uint32_t logical_net_id)
{
  struct rfapi_monitor_eth *val;
  struct rfapi_monitor_eth mon_buf;
