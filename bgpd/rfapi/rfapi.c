...
static int
rfapi_find_node (struct bgp *bgp, struct rfapi_ip_addr *vn_addr,
                 struct rfapi_ip_addr *un_addr, struct agg_node **node)
{
  struct rfapi *h;
  struct prefix p;
  struct agg_node *rn;
...
int
rfapi_find_rfd (struct bgp *bgp, struct rfapi_ip_addr *vn_addr,
 ...
 *------------------------------------------*/
static int
rfapi_find_handle (struct bgp *bgp, struct rfapi_ip_addr *vn_addr,
                   struct rfapi_ip_addr *un_addr, rfapi_handle *handle)
{
...
static int
rfapi_find_handle_vty (struct vty *vty, struct rfapi_ip_addr *vn_addr,
                       struct rfapi_ip_addr *un_addr, rfapi_handle *handle)
{
  struct bgp *bgp;
  struct rfapi_descriptor **rfd;
...
void
del_vnc_route (struct rfapi_descriptor *rfd,
               ...
               struct rfapi_nexthop *lnh, int kill)
{
  ...
  for (bpi = (bn ? bgp_dest_get_bgp_path_info (bn) : NULL); bpi;
       bpi = bpi->next)
    {
      ...
      vnc_zlog_debug_verbose (
          ...
          (bpi->extra ? bpi->extra->vnc.export.rfapi_handle : NULL),
  ...
  if (lnh)
    {
       ...
       */
      if (! bpi || ! bpi->extra || ! bpi->extra->vnc.export.local_nexthops)
        {
          /*
           * no local nexthops
           */
          vnc_zlog_debug_verbose ("%s: lnh list already empty at prefix %pFX",
                                  __func__, p);
          goto done;
        }
      ...
      for (ALL_LIST_ELEMENTS_RO (bpi->extra->vnc.export.local_nexthops, node,
                                 pLnh))
        {
      ...
      if (pLnh)
        {
          listnode_delete (bpi->extra->vnc.export.local_nexthops, pLnh);
        ...
        {
          vnc_zlog_debug_verbose ("%s: desired lnh not found %pFX", __func__,
                                  p);
        }
      goto done;
    }
  ...
  if (bpi)
    {
      vnc_zlog_debug_verbose (
          "%s: Found route (safi=%d) to delete at prefix %pFX", __func__, safi,
          p);
      ...
      if (safi == SAFI_MPLS_VPN)
        {
          ...
          if (table)
            vnc_import_bgp_del_vnc_host_route_mode_resolve_nve (bgp, prd,
                                                                table, p, bpi);
    ...
    {
      vnc_zlog_debug_verbose (
          "%s: Couldn't find route (safi=%d) at prefix %pFX", __func__, safi,
          p);
    }
done:
...
void
add_vnc_route (struct rfapi_descriptor *rfd, /* cookie, VPN UN addr, peer */
               ...
               int flags)
{
  ...
  if (safi == SAFI_ENCAP &&
      ! (bgp->rfapi_cfg->flags & BGP_VNC_CONFIG_ADV_UN_METHOD_ENCAP))
    {
       ...
       */
      vnc_zlog_debug_verbose (
          "%s: encap mode not enabled, not adding SAFI_ENCAP route", __func__);
      return;
    }
  ...
  else
    {
      ...
      if (rfd->default_tunneltype_option.type)
        ...
        if (type == ZEBRA_ROUTE_BGP && sub_type == BGP_ROUTE_RFP)
          TunnelType = rfapi_tunneltype_option_to_tlv (bgp, un_addr, NULL,
                                                       &attr, l2o != NULL);
    }
      ...
      if (safi == SAFI_ENCAP)
        {
          ...
          vnc_zlog_debug_verbose ("%s: mpls tunnel type, encap safi omitted",
                                  __func__);
          ...
          for (hop = rfp_options; hop; hop = hop->next)
            {
              ...
              encaptlv = XCALLOC (MTYPE_ENCAP_TLV,
                                  sizeof (struct bgp_attr_encap_subtlv) + 2 +
                                      hop->length);
              encaptlv->type = BGP_VNC_SUBTLV_TYPE_RFPOPTION; /* RFP
                                                                 option
                                                                 */
  ...
  if (rt_export_list)
    {
      bgp_attr_set_ecommunity (
          &attr,
          ecommunity_merge (bgp_attr_get_ecommunity (&attr), rt_export_list));
    }
  ...
  if (bpi)
    {
          ...
          if (! bpi->extra->vnc.export.local_nexthops)
            {
              ...
              bpi->extra->vnc.export.local_nexthops = list_new ();
              bpi->extra->vnc.export.local_nexthops->del = rfapi_nexthop_free;
            }
          ...
          if (! pLnh)
            {
              ...
              listnode_add (bpi->extra->vnc.export.local_nexthops, pLnh);
            }
        }
      ...
      if (attrhash_cmp (bpi->attr, new_attr) &&
          ! CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED))
        {
          ...
          vnc_zlog_debug_any (
              "%s: Found route (safi=%d) at prefix %s, no change", __func__,
              safi, buf);
      ...
      else
        {
          ...
          if (safi == SAFI_MPLS_VPN)
            {
              ...
              pdest =
                  bgp_node_get (bgp->rib[afi][safi], (struct prefix *) prd);
              table = bgp_dest_get_bgp_table_info (pdest);
              if (table)
          ...
          vnc_zlog_debug_any (
              "%s: Found route (safi=%d) at prefix %s, changed attr", __func__,
              safi, buf);
  ...
  if (safi == SAFI_MPLS_VPN)
    {
      ...
      table = bgp_dest_get_bgp_table_info (pdest);
      if (table)
        vnc_import_bgp_add_vnc_host_route_mode_resolve_nve (bgp, prd, table, p,
                                                            new);
      bgp_dest_unlock_node (pdest);
  ...
  vnc_zlog_debug_any (
      "%s: Added route (safi=%s) at prefix %s (bn=%p, prd=%pRDP)", __func__,
      safi2str (safi), buf, bn, prd);

done:
  /* Loop back to import tables */
  rfapiProcessUpdate (rfd->peer, rfd, p, prd, new_attr, afi, safi, type,
                      sub_type, &label_val);
  vnc_zlog_debug_verbose ("%s: looped back import route (safi=%d)", __func__,
                          safi);
}
...
static void
rfapiTunnelRouteAnnounce (struct bgp *bgp, struct rfapi_descriptor *rfd,
                          uint32_t *pLifetime)
{
  ...
  add_vnc_route (rfd,           /* rfapi descr, for export list  & backref */
                 ...
                 &rfd->un_addr, /* nexthop */
                 &local_pref, pLifetime, /* max lifetime of child VPN routes */
 ...
 */
static int
rfapi_open_inner (struct rfapi_descriptor *rfd, struct bgp *bgp,
                  struct rfapi *h, struct rfapi_nve_group_cfg *rfg)
{
  ...
  rfd->rt_export_list =
      (rfg->rt_export_list) ? ecommunity_dup (rfg->rt_export_list) : NULL;
  ...
  { /* base code assumes have valid host pointer */
    ...
    if (rfd->vn_addr.addr_family == AF_INET)
      {
        inet_ntop (AF_INET, &rfd->vn_addr.addr.v4, buf, sizeof (buf));
      }
    else if (rfd->vn_addr.addr_family == AF_INET6)
      {
        inet_ntop (AF_INET6, &rfd->vn_addr.addr.v6, buf, sizeof (buf));
      }
...
int
rfapi_init_and_open (struct bgp *bgp, struct rfapi_descriptor *rfd,
                     struct rfapi_nve_group_cfg *rfg)
{
  ...
  vnc_zlog_debug_verbose ("%s: new RFD with VN=%s UN=%s cookie=%p", __func__,
                          buf_vn, buf_un, rfd->cookie);
                   ...
                   struct rfapi_next_hop_entry **ppNextHopEntry)
{
  ...
  if (bgp->rfapi->flags & RFAPI_INCALLBACK)
    {
      vnc_zlog_debug_verbose ("%s: Called during calback, returning EDEADLK",
                              __func__);
      return EDEADLK;
    }
  ...
  rfd->ftd_last_allowed_time =
      monotime (NULL) - bgp->rfapi_cfg->rfp_cfg.ftd_advertisement_interval;

  if (l2o)
    {
      if (! memcmp (l2o->macaddr.octet, rfapi_ethaddr0.octet, ETH_ALEN))
        {
          eth_is_0 = 1;
        }
...

  {
    ...
    vnc_zlog_debug_verbose ("%s(rfd=%p, target=%pFX, ppNextHop=%p)", __func__,
  ...
  if (use_eth_resolution)
    {
      ...
      l2com = bgp_rfapi_get_ecommunity_by_lni_label (bgp, 1, logical_net_id,
                                                     l2o->label);
      if (l2com)
        {
          uint8_t *v = l2com->val;
      ...
      if (eth_is_0)
        {
          ...
          pNHE = rfapiEthRouteTable2NextHopList (
              logical_net_id, &rprefix, rfd->response_lifetime, self_vn_addr,
  ...
  if (! rn->info)
    {
      agg_unlock_node (rn);
      vnc_zlog_debug_verbose ("%s: VPN route not found, returning ENOENT",
                              __func__);
      return ENOENT;
    }
  ...
  if (use_eth_resolution)
    {
      ...
      pNHE = rfapiEthRouteNode2NextHopList (
          rn, &rprefix, rfd->response_lifetime, self_vn_addr, rfd->rib[afi],
          &p_original);
    }
  else
    {
       ...
       */
      pNHE =
          rfapiRouteNode2NextHopList (rn, rfd->response_lifetime, self_vn_addr,
                                      rfd->rib[afi], &p_original);
    }
 ...
 *------------------------------------------*/
int
rfapi_open (void *rfp_start_val, struct rfapi_ip_addr *vn,
            struct rfapi_ip_addr *un, struct rfapi_un_option *default_options,
            uint32_t *response_lifetime, void *userdata, /* callback cookie */
...
rfapi_close_inner (struct rfapi_descriptor *rfd, struct bgp *bgp)
{
  ...
  if (rfd->peer)
    {
      vnc_zlog_debug_verbose ("%s: calling peer_delete(%p), #%d", __func__,
      ...
      if (! CHECK_FLAG (rfd->flags, RFAPI_HD_FLAG_CLOSING_ADMINISTRATIVELY))
        {
          ...
          vnc_zlog_debug_verbose (
              "%s: added handle %p to deferred close queue", __func__, handle);
        }
      return 0;
    }
  ...
  if (CHECK_FLAG (rfd->flags, RFAPI_HD_FLAG_CLOSING_ADMINISTRATIVELY))
    {
      ...
      if (h->rfp_methods.close_cb)
        {
          vnc_zlog_debug_verbose ("%s calling close callback rfd=%p", __func__,
                                  rfd);
 ...
 */
int
rfapi_register (void *handle, struct rfapi_ip_prefix *prefix,
                ...
                rfapi_register_action action)
{
  ...
  vnc_zlog_debug_verbose (
      ...
      __func__, rfd, &p, lifetime, options_un, options_vn, action_str);
  ...
  if (! rfd->rfg)
    {
      ...
      vnc_zlog_debug_verbose (
          "%s: rfd=%p, no RF GRP instance: returning ESTALE", __func__, rfd);
      return ESTALE;
    }
  ...
  if (! (RFAPI_0_PREFIX (&p) && RFAPI_HOST_PREFIX (&p)))
  ...
  else
    {
      ...
      if (rfapiRaddr2Qprefix (&rfd->vn_addr, &pfx_vn_buf))
        {
          vnc_zlog_debug_verbose (
              "%s: handle has bad vn_addr: returning EBADF", __func__);
          if (RFAPI_REGISTER_ADD == action)
            {
              ++bgp->rfapi->stat.count_registrations_failed;
            }
  ...
  if (action == RFAPI_REGISTER_WITHDRAW || action == RFAPI_REGISTER_KILL)
    {
      ...
      del_vnc_route (
          ...
          ZEBRA_ROUTE_BGP, BGP_ROUTE_RFP, NULL, action == RFAPI_REGISTER_KILL);
  ...
  else
    {
      ...
      if (pfx_mac)
        {
          ...
          if (l2com)
          ...
          else
            {
              ...
              ecommunity_add_val (rtlist, &ecom_value, false, false);
            }
          if (l2o->tag_id)
            {
              as_t as = bgp->as;
      ...
      add_vnc_route (
          ...
          &prd,          /* route distinguisher to use (0 for ENCAP) */
          &rfd->vn_addr, /* nexthop */
          &local_pref, &lifetime, /* prefix lifetime -> Tunnel Encap attr */
...
int
rfapi_query (void *handle, struct rfapi_ip_addr *target,
             struct rfapi_l2address_option *l2o, /* may be NULL */
             struct rfapi_next_hop_entry **ppNextHopEntry)
{
  struct rfapi_descriptor *rfd = (struct rfapi_descriptor *) handle;
...
DEFUN (debug_rfapi_show_nves_vn_un,
       ...
       "0-255\n")
{
  ...
  rc =
      rfapi_register (handle, &hpfx, lifetime, NULL, NULL, RFAPI_REGISTER_ADD);
  if (rc)
    {
       ...
       "lni value range\n")
{
  ...
  if (rfapiStr2EthAddr (argv[12]->arg, &optary[opt_next].v.l2addr.macaddr))
    {
       ...
       "Remove without holddown\n")
{
  ...
  rfapi_register (
      handle, &hpfx, 0, NULL, NULL,
      (argc == 10 ? RFAPI_REGISTER_KILL : RFAPI_REGISTER_WITHDRAW));
       ...
       "import\n")
{
  ...
  for (it = h->imports; it; it = it->next)
    {
      s = ecommunity_ecom2str (it->rt_import_list, ECOMMUNITY_FORMAT_ROUTE_MAP,
                               0);
  ...
  rfapiShowImportTable (vty, "CE IT - IP VPN", h->it_ce->imported_vpn[AFI_IP],
                        1);
       ...
       "underlay network interface IPv6 address\n")
{
  ...
  rfapiShowImportTable (vty, "IP VPN", rfd->import_table->imported_vpn[AFI_IP],
                        1);
  ...
  if (h && h->descriptors.count)
    {
      ...
      struct rfapi_descriptor *rfd;
#if DEBUG_CLEANUP
      zlog_debug ("%s: descriptor count %u", __func__, h->descriptors.count);
#endif
...
rfapi_set_autord_from_vn (struct prefix_rd *rd, struct rfapi_ip_addr *vn)
{
  ...
  if (vn->addr_family != AF_INET && vn->addr_family != AF_INET6)
    {
      vnc_zlog_debug_verbose (
          "%s: can't auto-assign RD, VN addr family is not IPv4|v6", __func__);
      return EAFNOSUPPORT;
    }
  rd->family = AF_UNSPEC;
  rd->prefixlen = 64;
 ...
 ***********************************************************************/
static void *
rfapi_rfp_get_or_init_group_config_default (struct rfapi_cfg *rfc,
...
static void *
rfapi_rfp_get_or_init_group_config_nve (struct rfapi_cfg *rfc, struct vty *vty,
                                        uint32_t size)
{
...
static void *
rfapi_rfp_get_or_init_group_config_l2 (struct rfapi_cfg *rfc, struct vty *vty,
                                       uint32_t size)
{
                                     ...
                                     struct vty *vty, uint32_t size)
{
  ...
  switch (type)
    ...
    case RFAPI_RFP_CFG_GROUP_DEFAULT:
      ret = rfapi_rfp_get_or_init_group_config_default (bgp->rfapi_cfg, vty,
                                                        size);
      break;
    case RFAPI_RFP_CFG_GROUP_NVE:
      ret = rfapi_rfp_get_or_init_group_config_nve (bgp->rfapi_cfg, vty, size);
      break;
    case RFAPI_RFP_CFG_GROUP_L2:
      ret = rfapi_rfp_get_or_init_group_config_l2 (bgp->rfapi_cfg, vty, size);
      break;
    default:
      flog_err (EC_LIB_DEVELOPMENT, "%s: Unknown group type=%d", __func__,
                type);
      /* should never happen */
...
--------------------------------------------*/
void *
rfapi_rfp_get_group_config_ptr_vty (void *rfp_start_val,
                                    rfapi_rfp_cfg_group_type type,
                                    struct vty *vty)
{
...
--------------------------------------------*/
void *
rfapi_rfp_get_group_config_ptr_name (void *rfp_start_val,
                                     rfapi_rfp_cfg_group_type type,
                                     const char *name, void *criteria,
                                     rfp_group_config_search_cb_t *search_cb)
{
  struct bgp *bgp;
  void *ret = NULL;
  ...
  switch (type)
    ...
    case RFAPI_RFP_CFG_GROUP_NVE:
      ...
      break;
    case RFAPI_RFP_CFG_GROUP_L2:
      ret = rfapi_rfp_get_group_config_name_l2 (bgp->rfapi_cfg, name, criteria,
                                                search_cb);
      break;
    default:
      flog_err (EC_LIB_DEVELOPMENT, "%s: Unknown group type=%d", __func__,
  ...
  for (ALL_LIST_ELEMENTS_RO (bgp->rfapi_cfg->l2_groups, node, rfg))
    {
      if (rfg->logical_net_id == logical_net_id &&
        ...
        {
          if (rfg->rfp_cfg == NULL)
            vnc_zlog_debug_verbose ("%s: returning rfp group config for lni=0",
                                    __func__);
          return rfg->rfp_cfg;
        }
    }
