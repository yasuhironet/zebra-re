      ...
      for (afi = AFI_IP; afi < AFI_MAX; ++afi)
        {
          ...
          if (it->holddown_count[afi] != holddown_count)
            {
              vnc_zlog_debug_verbose (
                  "%s: it->holddown_count %d != holddown_count %d", __func__,
                  it->holddown_count[afi], holddown_count);
              assert (0);
            }
          if (it->remote_count[afi] != remote_count)
            {
              vnc_zlog_debug_verbose (
                  "%s: it->remote_count %d != remote_count %d", __func__,
          ...
          if (it->imported_count[afi] != imported_count)
            {
              vnc_zlog_debug_verbose (
                  "%s: it->imported_count %d != imported_count %d", __func__,
                  it->imported_count[afi], imported_count);
              assert (0);
            }
        }
    }
 ...
 */
void
rfapiCheckRefcount (struct agg_node *rn, safi_t safi, int lockoffset)
{
  ...
  if (count_bpi + count_monitor + lockoffset != agg_node_get_lock_count (rn))
    {
      vnc_zlog_debug_verbose (
 ...
 */
int
rfapiGetUnAddrOfVpnBi (struct bgp_path_info *bpi, struct prefix *p)
{
...
rfapiBgpInfoCreate (struct attr *attr, struct peer *peer, void *rfd,
 ...
 */
static void
rfapiBgpInfoFree (struct bgp_path_info *goner)
{
  if (! goner)
    return;
  ...
  if (goner->peer)
    {
      vnc_zlog_debug_verbose ("%s: calling peer_unlock(%p), #%d", __func__,
...
rfapiMonitorMoveShorter (struct agg_node *original_vpn_node, int lockoffset)
{
  ...
  {
    vnc_zlog_debug_verbose ("%s: called with node pfx=%pFX", __func__,
                            &original_vpn_node->p);
  }
#endif
...
#ifdef DEBUG_MONITOR_MOVE_SHORTER
          vnc_zlog_debug_verbose (
              "%s: have valid UN at original node, no change", __func__);
#endif
          return NULL;
        }
    }
  ...
  if (! par)
    {
      ...
      par = agg_node_get (agg_get_table (original_vpn_node), &pfx_default);
      ++parent_already_refcounted;
    }
...
#ifdef DEBUG_MONITOR_MOVE_SHORTER
  {
    vnc_zlog_debug_verbose ("%s: moved to node pfx=%pFX", __func__, &par->p);
  }
#endif
 ...
 */
static void
rfapiMonitorMoveLonger (struct agg_node *new_vpn_node)
{
  ...
  if (! bpi)
    {
      vnc_zlog_debug_verbose (
          "%s: no valid routes at node %p, so not attempting moves", __func__,
          new_vpn_node);
      return;
    }
   ...
   */
  for (par = agg_node_parent (new_vpn_node); par; par = agg_node_parent (par))
    {
  ...
  if (! par)
    {
      vnc_zlog_debug_verbose ("%s: no parent nodes with monitors, done",
                              __func__);
      return;
    }
...
static void
rfapiBgpInfoChainFree (struct bgp_path_info *bpi)
{
  struct bgp_path_info *next;
  ...
  for (afi = AFI_IP; afi < AFI_MAX; ++afi)
    {
      ...
      if (at)
        {
          for (rn = agg_route_top (at); rn; rn = agg_route_next (rn))
            {
              /*
               * Each route_node has:
 ...
 */
static int
rfapiEcommunitiesMatchBeec (struct ecommunity *ecom, bgp_encap_types type)
{
  int i;
  ...
  for (i = 0; i < (ecom->size * ECOMMUNITY_SIZE); i += ECOMMUNITY_SIZE)
    {
      ...
      if (ep[0] == ECOMMUNITY_ENCODE_OPAQUE &&
          ...
          ep[6] == ((type && 0xff00) >> 8) && ep[7] == (type & 0xff))
        {
...
static int
rfapiVpnBiNhEqualsPt (struct bgp_path_info *bpi, struct rfapi_ip_addr *hpt)
{
  uint8_t family;
  ...
  switch (family)
    {
    case AF_INET:
      if (bpi->attr->mp_nexthop_global_in.s_addr != hpt->addr.v4.s_addr)
        return 0;
      break;
 ...
 */
static int
rfapiVpnBiSamePtUn (struct bgp_path_info *bpi1, struct bgp_path_info *bpi2)
{
      ...
      if (bpi1->extra)
        {
          ...
          switch (bpi1->extra->vnc.import.un_family)
            {
            case AF_INET:
              pfx_un1.u.prefix4 = bpi1->extra->vnc.import.un.addr4;
              break;
            case AF_INET6:
              pfx_un1.u.prefix6 = bpi1->extra->vnc.import.un.addr6;
              break;
            default:
      ...
      if (bpi2->extra)
        {
          ...
          switch (bpi2->extra->vnc.import.un_family)
            {
            case AF_INET:
              pfx_un2.u.prefix4 = bpi2->extra->vnc.import.un.addr4;
              break;
            case AF_INET6:
              pfx_un2.u.prefix6 = bpi2->extra->vnc.import.un.addr6;
              break;
            default:
      ...
      if ((p->type == RFAPI_VN_OPTION_TYPE_L2ADDR) && (p->length >= 8))
        {
          ...
          l2o->label = ((v[6] << 12) & 0xff000) + ((v[7] << 4) & 0xff0) +
                       ((v[8] >> 4) & 0xf);
                             ...
                             struct agg_node *rn) /* req for L2 eth addr */
{
  ...
  if (bpi->extra &&
      decode_rd_type (bpi->extra->vnc.import.rd.val) == RD_TYPE_VNC_ETH)
    {
      /* ethernet */
      ...
      (void) rfapiEcommunityGetEthernetTag (
          bgp_attr_get_ecommunity (bpi->attr), &vo->v.l2addr.tag_id);
  ...
  for (pEncap = bgp_attr_get_vnc_subtlvs (bpi->attr); pEncap;
       pEncap = pEncap->next)
    {
      switch (pEncap->type)
        ...
        default:
          zlog_warn ("%s: unknown VNC option type %d", __func__, pEncap->type);
  ...
  for (pEncap = bpi->attr->encap_subtlvs; pEncap; pEncap = pEncap->next)
    {
      switch (pEncap->type)
        ...
        default:
          zlog_warn ("%s: unknown Encap Attribute option type %d", __func__,
                     pEncap->type);
          break;
        }
    }
  ...
  if (! have_vnc_tunnel_un && bpi->extra)
    {
      ...
      switch (new->un_address.addr_family)
        ...
        case AF_INET6:
          ...
          break;
        default:
          zlog_warn ("%s: invalid UN addr family (%d) for bpi %p", __func__,
                     new->un_address.addr_family, bpi);
          rfapi_free_next_hop_list (new);
          return NULL;
        }
    }
...
int
rfapiHasNonRemovedRoutes (struct agg_node *rn)
{
  struct bgp_path_info *bpi;
 ...
 */
void
rfapiDumpNode (struct agg_node *rn)
{
  struct bgp_path_info *bpi;
  ...
  for (bpi = rn->info; bpi; bpi = bpi->next)
    {
      ...
      vnc_zlog_debug_verbose ("  bpi=%p, nr=%d, flags=0x%x, extra=%p, ctrc=%d",
                              bpi, nr, bpi->flags, bpi->extra, ctrc);
    }
}
#endif
      ...
      if (atable)
        {
          ...
          if (rfapiRibFTDFilterRecentPrefix (rfd, rn, pfx_target_original))
            return 0;
        }
    }

  seen_nexthops = skiplist_new (0, vnc_prefix_cmp, prefix_free_lists);
  ...
  for (bpi = rn->info; bpi; bpi = bpi->next)
    {
      ...
      if (removed && ! CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED))
        {
          ...
          vnc_zlog_debug_verbose (
              "%s: want holddown, this route not holddown, skip", __func__);
#endif
          continue;
        }
      if (! removed && CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED))
      ...
      if (! skiplist_search (seen_nexthops, &pfx_vn, NULL))
        {
#ifdef DEBUG_RETURNED_NHL
          vnc_zlog_debug_verbose ("%s: already put VN/nexthop %pFX, skip",
                                  __func__, &pfx_vn);
#endif
          continue;
        }
...
#ifdef DEBUG_ENCAP_MONITOR
          vnc_zlog_debug_verbose (
              "%s: failed to get UN address of this VPN bpi", __func__);
#endif
          continue;
        }
      ...
      *newpfx = pfx_vn;
      ...
      if (new)
        {
          if (rfapiRibPreloadBi (rfd_rib_node, &pfx_vn, &pfx_un, lifetime,
                                 bpi))
            {
    ...
    struct prefix *pfx_target_original)   /* query target */
{
  ...
  if (agg_node_left (rn) && agg_node_left (rn) != omit_node)
    {
      if (agg_node_left (rn)->info)
        {
          const struct prefix *p = agg_node_get_prefix (agg_node_left (rn));
          int count = 0;
          ...
          count = rfapiNhlAddNodeRoutes (
              agg_node_left (rn), &rprefix, lifetime, 0, head, tail,
              exclude_vnaddr, rib_rn, pfx_target_original);
          if (! count)
            {
              count = rfapiNhlAddNodeRoutes (
                  agg_node_left (rn), &rprefix, lifetime, 1, head, tail,
                  exclude_vnaddr, rib_rn, pfx_target_original);
            }
          rcount += count;
          if (rib_rn)
  ...
  if (agg_node_right (rn) && agg_node_right (rn) != omit_node)
    {
      if (agg_node_right (rn)->info)
        {
          const struct prefix *p = agg_node_get_prefix (agg_node_right (rn));
          int count = 0;
          struct agg_node *rib_rn = NULL;
          ...
          count = rfapiNhlAddNodeRoutes (
              agg_node_right (rn), &rprefix, lifetime, 0, head, tail,
              exclude_vnaddr, rib_rn, pfx_target_original);
          if (! count)
            {
              count = rfapiNhlAddNodeRoutes (
                  agg_node_right (rn), &rprefix, lifetime, 1, head, tail,
                  exclude_vnaddr, rib_rn, pfx_target_original);
            }
          rcount += count;
          if (rib_rn)
  ...
  if (agg_node_left (rn))
    {
      rcount += rfapiNhlAddSubtree (agg_node_left (rn), lifetime, head, tail,
                                    omit_node, exclude_vnaddr, rfd_rib_table,
                                    pfx_target_original);
    }
  if (agg_node_right (rn))
    {
      rcount += rfapiNhlAddSubtree (agg_node_right (rn), lifetime, head, tail,
                                    omit_node, exclude_vnaddr, rfd_rib_table,
                                    pfx_target_original);
    }
  ...
  if (parent)
    {
      ...
      rfapiQprefix2Rprefix (p, &rprefix);
      count +=
          rfapiNhlAddNodeRoutes (parent, &rprefix, lifetime, 0, &answer, &last,
                                 exclude_vnaddr, rib_rn, pfx_target_original);
    ...
    struct prefix *pfx_target_original)   /* query target */
{
...
#ifdef DEBUG_ENCAP_MONITOR
  vnc_zlog_debug_verbose ("%s: node %p: %d non-holddown routes", __func__, rn,
                          count);
#endif
  ...
  if (! count)
    {
      count =
          rfapiNhlAddNodeRoutes (rn, rprefix, lifetime, 1, &answer, &last,
                                 exclude_vnaddr, rib_rn, pfx_target_original);
      vnc_zlog_debug_verbose ("%s: node %p: %d holddown routes", __func__, rn,
                              count);
    }
  ...
  for (rn = agg_route_top (rt); rn; rn = agg_route_next (rn))
    {
      ...
      nhl =
          rfapiEthRouteNode2NextHopList (rn, rprefix, lifetime, exclude_vnaddr,
                                         rfd_rib_table, pfx_target_original);
      if (! tail)
        {
          tail = biglist = nhl;
 ...
 */
static void
rfapiBgpInfoAttachSorted (struct agg_node *rn, struct bgp_path_info *info_new,
  ...
  for (prev = NULL, next = rn->info; next; prev = next, next = next->next)
    {
      ...
      if (! bgp ||
          ...
          bgp_path_info_cmp_compatible (bgp, info_new, next, pfx_buf, afi,
                                        safi, &reason) == -1)
        { /* -1 if 1st is better */
          break;
        }
...
static void
rfapiBgpInfoDetach (struct agg_node *rn, struct bgp_path_info *bpi)
{
  /*
 ...
 */
static int
rfapi_bi_peer_rd_cmp (const void *b1, const void *b2)
{
 ...
 */
static int
rfapi_bi_peer_rd_aux_cmp (const void *b1, const void *b2)
{
  const struct bgp_path_info *bpi1 = b1;
  const struct bgp_path_info *bpi2 = b2;
  int rc;
 ...
 */
static void
rfapiItBiIndexAdd (struct agg_node *rn,       /* Import table VPN node */
...
static void
rfapiItBiIndexDump (struct agg_node *rn)
{
  struct skiplist *sl;
  void *cursor = NULL;
  ...
  for (rc = skiplist_next (sl, (void **) &k, (void **) &v, &cursor); ! rc;
       rc = skiplist_next (sl, (void **) &k, (void **) &v, &cursor))
    {
      ...
      if (k->extra->vnc.import.aux_prefix.family)
        {
          prefix2str (&k->extra->vnc.import.aux_prefix, buf_aux_pfx,
                      sizeof (buf_aux_pfx));
        }
      else
        strlcpy (buf_aux_pfx, "(none)", sizeof (buf_aux_pfx));

      vnc_zlog_debug_verbose ("bpi %p, peer %p, rd %s, aux_prefix %s", k,
                              k->peer, buf, buf_aux_pfx);
    }
}
    ...
    const struct prefix *aux_prefix) /* optional L3 addr for L2 ITs */
{
...
#ifdef DEBUG_BI_SEARCH
  {
    ...
    if (aux_prefix)
      {
        prefix2str (aux_prefix, buf_aux_pfx, sizeof (buf_aux_pfx));
      }
    else
      strlcpy (buf_aux_pfx, "(nil)", sizeof (buf_aux_pfx));

    vnc_zlog_debug_verbose ("%s want prd=%pRDP, peer=%p, aux_prefix=%s",
                            __func__, prd, peer, buf_aux_pfx);
  ...
  if (sl->count < 3)
    {
      ...
      for (bpi_result = rn->info; bpi_result; bpi_result = bpi_result->next)
        {
#ifdef DEBUG_BI_SEARCH
          vnc_zlog_debug_verbose ("%s: bpi has prd=%pRDP, peer=%p", __func__,
                                  &bpi_result->extra->vnc.import.rd,
                                  bpi_result->peer);
#endif
          if (peer == bpi_result->peer &&
              ! prefix_cmp (
                  (struct prefix *) &bpi_result->extra->vnc.import.rd,
                  (struct prefix *) prd))
            {

#ifdef DEBUG_BI_SEARCH
              vnc_zlog_debug_verbose (
                  "%s: peer and RD same, doing aux_prefix check", __func__);
#endif
              if (! aux_prefix ||
                  ! prefix_cmp (aux_prefix,
                                &bpi_result->extra->vnc.import.aux_prefix))
                {
...
static void
rfapiItBiIndexDel (struct agg_node *rn,       /* Import table VPN node */
                      ...
                      struct bgp_path_info *vpn_bpi) /* VPN bpi/route */
{
  ...
  m = XCALLOC (MTYPE_RFAPI_MONITOR_ENCAP, sizeof (struct rfapi_monitor_encap));
...
static void
rfapiMonitorEncapDelete (struct bgp_path_info *vpn_bpi)
{
  /*
  ...
  if (vpn_bpi->extra)
    {
      struct rfapi_monitor_encap *hme = vpn_bpi->extra->vnc.import.hme;
 ...
 */
static void
rfapiWithdrawTimerVPN (struct event *t)
{
  ...
  if (bgp == NULL)
    {
      vnc_zlog_debug_verbose (
          "%s: NULL BGP pointer, assume shutdown race condition!!!", __func__);
      early_exit = true;
    }
  ...
  vnc_zlog_debug_verbose ("%s: removing bpi %p at prefix %pRN", __func__, bpi,
                          wcb->node);
...
static void
rfapiCopyUnEncap2VPN (struct bgp_path_info *encap_bpi,
                      struct bgp_path_info *vpn_bpi)
{
  ...
  switch (BGP_MP_NEXTHOP_FAMILY (encap_bpi->attr->mp_nexthop_len))
    ...
    case AF_INET6:
      ...
      vpn_bpi->extra->vnc.import.un.addr6 = encap_bpi->attr->mp_nexthop_global;
      break;
  ...
  if (! encap_bpi)
    {
      ...
      if (! vpn_bpi || ! vpn_bpi->extra)
        {
          zlog_warn ("%s: missing VPN bpi/extra, can't clear UN addr",
                     __func__);
          return 1;
        }
      vpn_bpi->extra->vnc.import.un_family = AF_UNSPEC;
          ...
          if (rfapiGetVncTunnelUnAddr (vpn_bpi->attr, NULL))
            {
              ...
              vnc_import_bgp_exterior_del_route_interior (
                  bgp_get_default (), import_table, vpn_rn, vpn_bpi);
            }
        }
    }
  else
    {
      if (! vpn_bpi)
        {
          zlog_warn ("%s: missing VPN bpi, can't clear UN addr", __func__);
          return 1;
        }
...
static void
rfapiWithdrawTimerEncap (struct event *t)
{
  struct rfapi_withdraw *wcb = EVENT_ARG (t);
  struct bgp_path_info *bpi = wcb->info;
  ...
  for (em = RFAPI_MONITOR_ENCAP (wcb->node); em; em = em->next)
    {
       ...
       */
      if (rfapiWithdrawEncapUpdateCachedUn (wcb->import_table, wcb->node->info,
                                            em->node, em->bpi))
        continue;
                           ...
                           void (*timer_service_func) (struct event *))
{
  ...
  if (CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED))
    {
       ...
       * delete it.
       */
      vnc_zlog_debug_verbose ("%s: already being withdrawn, do nothing",
                              __func__);
      return;
    }
  ...
  if (lifetime > UINT32_MAX / 1001)
  ...
  else
    {
      ...
      bpi->extra->vnc.import.timer = NULL;
      event_add_timer_msec (bm->master, timer_service_func, wcb, lifetime_msec,
                            &bpi->extra->vnc.import.timer);
    }
...
typedef void (rfapi_bi_filtered_import_f) (
    struct rfapi_import_table *table, int action, struct peer *peer, void *rfd,
    const struct prefix *prefix, const struct prefix *aux_prefix, afi_t afi,
    struct prefix_rd *prd, struct attr *attr, uint8_t type, uint8_t sub_type,
    uint32_t *label);


static void
rfapiExpireEncapNow (struct rfapi_import_table *it, struct agg_node *rn,
                     struct bgp_path_info *bpi)
{
  struct rfapi_withdraw *wcb;
  struct event t;
...
#ifdef RFAPI_REQUIRE_ENCAP_BEEC
      if (! rfapiEcommunitiesMatchBeec (bgp_attr_get_ecommunity (attr)))
        {
          vnc_zlog_debug_verbose (
              "%s: it=%p: no match for BGP Encapsulation ecommunity", __func__,
              import_table);
          return;
        }
#endif
...
#ifdef DEBUG_ENCAP_MONITOR
  vnc_zlog_debug_verbose ("%s: initial encap lookup(it=%p) rn=%p", __func__,
                          import_table, rn);
#endif
      ...
      for (bpi = rn->info; bpi; bpi = bpi->next)
        {
           ...
           */
          vnc_zlog_debug_verbose ("%s: comparing BPI %p", __func__, bpi);
          ...
          if (! bpi->extra)
            {
              vnc_zlog_debug_verbose ("%s: no bpi->extra", __func__);
              continue;
            }
          if (prefix_cmp ((struct prefix *) &bpi->extra->vnc.import.rd,
                          (struct prefix *) prd))
            {
          ...
          vnc_zlog_debug_verbose ("%s: found matching bpi", __func__);
          ...
          if (action == FIF_ACTION_WITHDRAW)
            {
              ...
              vnc_zlog_debug_verbose ("%s: withdrawing at prefix %pRN",
                                      __func__, rn);

              rfapiBiStartWithdrawTimer (import_table, rn, bpi, afi,
                                         SAFI_ENCAP, rfapiWithdrawTimerEncap);
            }
          else
            {
              vnc_zlog_debug_verbose (
                  "%s: %s at prefix %pRN", __func__,
                  ((action == FIF_ACTION_KILL) ? "killing" : "replacing"), rn);
              ...
              if (action == FIF_ACTION_UPDATE)
              ...
              else
                {
                   ...
                   * pretend we're an expiring timer
                   */
                  wcb = XCALLOC (MTYPE_RFAPI_WITHDRAW,
  ...
  for (bpi = info_new->next; bpi; bpi = next)
    {
      ...
      vnc_zlog_debug_verbose (
          "%s: removing holddown bpi matching NVE of new route", __func__);
 ...
 */
      ...
      for (m = RFAPI_MONITOR_ENCAP (rn); m; m = m->next)
        {
          ...
          if (! CHECK_FLAG (m->bpi->flags, BGP_PATH_VALID))
            {
              ...
              vnc_import_bgp_exterior_add_route_interior (bgp, import_table,
                                                          m->node, m->bpi);
            }
          ...
          referenced_vpn_prefix = agg_node_get (referenced_vpn_table, p);
          assert (referenced_vpn_prefix);
          for (mnext = referenced_vpn_prefix->info; mnext; mnext = mnext->next)
            {
       ...
       * for each VPN node referenced in the ENCAP monitors:
       */
      for (referenced_vpn_prefix = agg_route_top (referenced_vpn_table);
           referenced_vpn_prefix;
           referenced_vpn_prefix = agg_route_next (referenced_vpn_prefix))
        {
...
static void
rfapiExpireVpnNow (struct rfapi_import_table *it, struct agg_node *rn,
                   struct bgp_path_info *bpi, int lockoffset)
{
  struct rfapi_withdraw *wcb;
  struct event t;
    ...
    uint32_t *label)   /* part of bgp_path_info */
{
      ...
      if (bpi)
        {
          ...
          if (VNC_DEBUG (VERBOSE) && bpi->type != type)
            ...
            zlog_warn ("%s: type mismatch! (bpi=%d, arg=%d)", __func__,
                       bpi->type, type);
          ...
          if (action == FIF_ACTION_WITHDRAW)
            {
              ...
              int washolddown = CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED);

              vnc_zlog_debug_verbose (
                  "%s: withdrawing at prefix %pRN%s", __func__, rn,
              ...
              if (! washolddown)
                {
                  ...
                  RFAPI_UPDATE_ITABLE_COUNT (bpi, import_table, afi, -1);
              ...
              if (action == FIF_ACTION_UPDATE)
                {
                   ...
                   */
                  rfapiGetNexthop (bpi->attr, &original_nexthop);
  ...
  if (ern)
  ...
  else
    {
      ...
      vnc_zlog_debug_verbose ("%s: no encap route for vn addr %pFX", __func__,
                              &vn_prefix);
  ...
  if ((AFI_L2VPN == afi) && aux_prefix)
    {
      ...
      vnc_zlog_debug_verbose ("%s: setting BPI's aux_prefix", __func__);
      info_new->extra->vnc.import.aux_prefix = *aux_prefix;
    }

  vnc_zlog_debug_verbose ("%s: inserting bpi %p at prefix %pRN #%d", __func__,
                          info_new, rn, agg_node_get_lock_count (rn));
  ...
  vnc_import_bgp_exterior_add_route_interior (bgp, import_table, rn, info_new);
  ...
  for (bpi = info_new->next; bpi; bpi = next)
    {
      ...
      if (un_prefix_valid &&                        /* new route UN addr */
          ! rfapiGetUnAddrOfVpnBi (bpi, &pfx_un) && /* old route UN addr */
          prefix_same (&pfx_un, &un_prefix))
        { /* compare */
          un_match = 1;
        }
...
static void
rfapiBgpInfoFilteredImportBadSafi (
    struct rfapi_import_table *import_table, int action, struct peer *peer,
    void *rfd, /* set for looped back routes */
    const struct prefix *p,
    const struct prefix *aux_prefix, /* AFI_L2VPN: optional IP */
                    ...
                    uint8_t sub_type, uint32_t *label)
{
  ...
  if ((safi == SAFI_MPLS_VPN) &&
      (decode_rd_type (prd->val) == RD_TYPE_VNC_ETH))
    {
      ...
      vnc_zlog_debug_verbose (
          "%s: rfapiEcommunityGetLNI returned %d, lni=%d, attr=%p", __func__,
          rc, lni, attr);
      if (! rc)
        {
          ...
          rfapiBgpInfoFilteredImportVPN (
              it, FIF_ACTION_UPDATE, peer, rfd, &pfx_mac_buf, /* prefix */
  ...
  if (safi == SAFI_MPLS_VPN)
    {
      ...
      rfapiBgpInfoFilteredImportVPN (
          bgp->rfapi->it_ce, FIF_ACTION_UPDATE, peer, rfd, p, /* prefix */
          NULL, afi, prd, attr, type, sub_type, label);
    }
}
...
void
rfapiProcessWithdraw (struct peer *peer, void *rfd, const struct prefix *p,
                      ...
                      safi_t safi, uint8_t type, int kill)
{
      ...
      decode_rd_type (prd->val) == RD_TYPE_VNC_ETH)
    {
      ...
      for (rc = skiplist_next (h->import_mac, NULL, (void **) &it, &cursor);
           ...
           rc = skiplist_next (h->import_mac, NULL, (void **) &it, &cursor))
        {
          ...
          rfapiBgpInfoFilteredImportVPN (
              it, (kill ? FIF_ACTION_KILL : FIF_ACTION_WITHDRAW), peer, rfd,
              &pfx_mac_buf, /* prefix */
              p,            /* aux_prefix: IP */
  ...
  if (safi == SAFI_MPLS_VPN)
    {
      rfapiBgpInfoFilteredImportVPN (
          bgp->rfapi->it_ce, (kill ? FIF_ACTION_KILL : FIF_ACTION_WITHDRAW),
          peer, rfd, p, /* prefix */
          NULL, afi, prd, attr, type, 0,
          NULL); /* sub_type & label unused for withdraw */
    }
...
static void
rfapiProcessPeerDownRt (struct peer *peer,
                        struct rfapi_import_table *import_table, afi_t afi,
                        safi_t safi)
{
  struct agg_node *rn;
  struct bgp_path_info *bpi;
 ...
 */
static void
rfapiBgpTableFilteredImport (struct bgp *bgp, struct rfapi_import_table *it,
                             afi_t afi, safi_t safi)
{
              ...
              for (bpi = bgp_dest_get_bgp_path_info (dest2); bpi;
                   bpi = bpi->next)
                {
                  ...
                  (*rfapiBgpInfoFilteredImportFunction (safi)) (
                      ...
                      FIF_ACTION_UPDATE, bpi->peer, NULL,
                      bgp_dest_get_prefix (dest2), NULL, afi,
                      (struct prefix_rd *) bgp_dest_get_prefix (dest1),
                      bpi->attr, bpi->type, bpi->sub_type, &label);
                }
            }
        }
    }
  ...
  if (! it)
    {
      ...
      it->rfg = rfg;
      it->monitor_exterior_orphans = skiplist_new (0, NULL, prefix_free_lists);
 ...
 */
static void
rfapiDeleteRemotePrefixesIt (struct bgp *bgp, struct rfapi_import_table *it,
                             struct prefix *un, struct prefix *vn,
                             struct prefix *p, int delete_active,
                             int delete_holddown, uint32_t *pARcount,
                             uint32_t *pAHcount, uint32_t *pHRcount,
                             uint32_t *pHHcount,
                             ...
                             struct skiplist *uniq_holddown_nves)
{
...
#ifdef DEBUG_L2_EXTRA
  {
    ...
    vnc_zlog_debug_verbose (
        "%s: entry, p=%s, delete_active=%d, delete_holddown=%d", __func__,
      ...
      for (rn = agg_route_top (rt); rn; rn = agg_route_next (rn))
        {
          ...
          if (p && VNC_DEBUG (IMPORT_DEL_REMOTE))
            vnc_zlog_debug_any ("%s: want %pFX, have %pRN", __func__, p, rn);
          ...
          for (bpi = rn->info; bpi; bpi = next)
            {
              ...
              vnc_zlog_debug_verbose ("%s: examining bpi %p", __func__, bpi);
                  ...
                  if (bpi->extra->vnc.import.timer)
                    {
                      ...
                      RFAPI_UPDATE_ITABLE_COUNT (bpi, wcb->import_table, afi,
                                                 1);
                      XFREE (MTYPE_RFAPI_WITHDRAW, wcb);
                      EVENT_OFF (bpi->extra->vnc.import.timer);
                    }
              ...
              vnc_zlog_debug_verbose (
                  ...
                  __func__, bpi, qct_valid, qpt_valid, delete_holddown,
                  delete_active);
              ...
              if (qct_valid && qpt_valid)
                {
                  ...
                  if (skiplist_search (
                          (is_active ? uniq_active_nves : uniq_holddown_nves),
                          &na, (void **) &nap))
                    {
                      char line[BUFSIZ];
              ...
              vnc_direct_bgp_rh_del_route (bgp, afi, rn_p, bpi->peer);
 ...
 *	void
 --------------------------------------------*/
void
rfapiDeleteRemotePrefixes (struct prefix *un, struct prefix *vn,
                           struct prefix *p, struct rfapi_import_table *arg_it,
  ...
  for (; it;)
    {
      ...
      rfapiDeleteRemotePrefixesIt (
          ...
          &deleted_holddown_route_count, &deleted_holddown_nve_count,
          uniq_active_nves, uniq_holddown_nves);
