 ...
 */
static void
encap_attr_export_ce (struct attr *new, struct attr *orig,
...
getce (struct bgp *bgp, struct attr *attr, struct prefix *pfx_ce)
{
  ...
  for (ecp = ecomm->val, i = 0; i < ecomm->size; ++i, ecp += ECOMMUNITY_SIZE)
    {

      if (VNC_DEBUG (EXPORT_BGP_GETCE))
        {
          vnc_zlog_debug_any ("%s: %02x %02x %02x %02x %02x %02x %02x %02x",
                              __func__, ecp[0], ecp[1], ecp[2], ecp[3], ecp[4],
                              ecp[5], ecp[6], ecp[7]);
        }
...
void
vnc_direct_bgp_add_route_ce (struct bgp *bgp, struct agg_node *rn,
                             struct bgp_path_info *bpi)
{
  ...
  if ((bpi->type != ZEBRA_ROUTE_BGP) ||
      (bpi->sub_type != BGP_ROUTE_NORMAL && bpi->sub_type != BGP_ROUTE_RFP &&
       bpi->sub_type != BGP_ROUTE_STATIC))
    {

      vnc_zlog_debug_verbose (
          "%s: wrong route type/sub_type for export, skipping", __func__);
      return;
    }
  ...
  if (! bgp->redist[afi][ZEBRA_ROUTE_VNC_DIRECT])
    {
      vnc_zlog_debug_verbose (
          "%s: bgp redistribution of VNC direct routes is off", __func__);
      return;
    }
  ...
  if (! VNC_EXPORT_BGP_CE_ENABLED (bgp->rfapi_cfg))
    {
      vnc_zlog_debug_verbose (
          "%s: export-to-bgp ce mode not enabled, skipping", __func__);
      return;
    }
  ...
  if (bgp->rfapi_cfg->plist_export_bgp[afi])
    {
      if (prefix_list_apply (bgp->rfapi_cfg->plist_export_bgp[afi], prefix) ==
          PREFIX_DENY)
        {
          vnc_zlog_debug_verbose ("%s: prefix list denied, skipping",
                                  __func__);
          return;
        }
    }
  ...
  udest = bgp_afi_node_get (bgp->rib[afi][SAFI_UNICAST], afi, SAFI_UNICAST,
                            prefix, NULL);
  for (ubpi = bgp_dest_get_bgp_path_info (udest); ubpi; ubpi = ubpi->next)
    {
      ...
      if (ubpi->type == ZEBRA_ROUTE_VNC_DIRECT &&
          ubpi->sub_type == BGP_ROUTE_REDISTRIBUTE && ubpi->peer == peer &&
  ...
  if (bgp->rfapi_cfg->routemap_export_bgp)
    {
      ...
      info.attr = &hattr;
      ret =
          route_map_apply (bgp->rfapi_cfg->routemap_export_bgp, prefix, &info);
 ...
 */
void
vnc_direct_bgp_del_route_ce (struct bgp *bgp, struct agg_node *rn,
                             struct bgp_path_info *bpi)
{
  ...
  for (vbpi = rn->info; vbpi; vbpi = vbpi->next)
    {
      ...
      if (prefix_same (&ce, &ce_nexthop))
        {
          vnc_zlog_debug_verbose (
              "%s: still have a route via CE, not deleting unicast", __func__);
          return;
        }
    }
...
vnc_direct_bgp_vpn_enable_ce (struct bgp *bgp, afi_t afi)
{
  ...
  if (! VNC_EXPORT_BGP_CE_ENABLED (bgp->rfapi_cfg))
    {
      vnc_zlog_debug_verbose ("%s: export of CE routes not enabled, skipping",
                              __func__);
      return;
    }
  ...
  for (rn = agg_route_top (bgp->rfapi->it_ce->imported_vpn[afi]); rn;
       rn = agg_route_next (rn))
    {
      ...
      vnc_zlog_debug_verbose ("%s: checking prefix %pRN", __func__, rn);
...
vnc_direct_bgp_vpn_disable_ce (struct bgp *bgp, afi_t afi)
{
  ...
  for (dest = bgp_table_top (bgp->rib[afi][SAFI_UNICAST]); dest;
       dest = bgp_route_next (dest))
    {
      ...
      for (ri = bgp_dest_get_bgp_path_info (dest), next = NULL; ri; ri = next)
        {
          ...
          if (ri->type == ZEBRA_ROUTE_VNC_DIRECT &&
              ri->sub_type == BGP_ROUTE_REDISTRIBUTE)
            {
              ...
              bgp_withdraw (
                  ri->peer, bgp_dest_get_prefix (dest), 0, /* addpath_id */
                  AFI_IP, SAFI_UNICAST, ZEBRA_ROUTE_VNC_DIRECT,
                  BGP_ROUTE_REDISTRIBUTE, NULL, /* RD not used for unicast */
                  NULL, 0, NULL);               /* tag not used for unicast */
            }
        }
...
vnc_route_origin_ecom (struct agg_node *rn)
{
  ...
  for (bpi = rn->info; bpi; bpi = bpi->next)
    {
      ...
      switch (BGP_MP_NEXTHOP_FAMILY (bpi->attr->mp_nexthop_len))
        ...
        case AF_INET:
          ...
          roec.val[0] = 0x01;
          roec.val[1] = 0x03;
          memcpy (roec.val + 2, &bpi->attr->mp_nexthop_global_in.s_addr, 4);
          roec.val[6] = 0;
          roec.val[7] = 0;
                                        ...
                                        /* if rn is 0, use route's nexthop */
{
  ...
  if (new_nexthop)
  ...
  else
    {
      ...
      orig_nexthop.family = BGP_MP_NEXTHOP_FAMILY (orig->mp_nexthop_len);
  ...
  if (bgp_attr_get_ecommunity (new))
    {
      if (ecom_ro)
        bgp_attr_set_ecommunity (
            new, ecommunity_merge (ecom_ro, bgp_attr_get_ecommunity (new)));
    }
  else
    {
 ...
 */
void
vnc_direct_bgp_add_prefix (struct bgp *bgp,
                           ...
                           struct agg_node *rn)
{
  ...
  if (! VNC_EXPORT_BGP_GRP_ENABLED (bgp->rfapi_cfg))
    {
      vnc_zlog_debug_verbose (
          "%s: export-to-bgp group mode not enabled, skipping", __func__);
      return;
    }

  if (! listcount (bgp->rfapi_cfg->rfg_export_direct_bgp_l))
    {
      vnc_zlog_debug_verbose ("%s: no bgp-direct export nve group, skipping",
                              __func__);
      return;
    }
  ...
  vnc_zlog_debug_verbose (
      "%s: looping over nve-groups in direct-bgp export list", __func__);

  for (ALL_LIST_ELEMENTS (bgp->rfapi_cfg->rfg_export_direct_bgp_l, node, nnode,
                          rfgn))
    {
      ...
      if (rfgn->rfg->plist_export_bgp[afi])
        {
          if (prefix_list_apply (rfgn->rfg->plist_export_bgp[afi], p) ==
              PREFIX_DENY)
      ...
      if (rfgn->rfg->type == RFAPI_GROUP_CFG_VRF)
        {
          vnc_direct_add_rn_group_rd (bgp, rfgn->rfg, rn, &attr, afi,
                                      rfgn->rfg->rfd);
          /*
           * yuck!
 ...
 */
void
vnc_direct_bgp_del_prefix (struct bgp *bgp,
                           struct rfapi_import_table *import_table,
                           struct agg_node *rn)
{
  struct listnode *node, *nnode;
  ...
  if (! afi)
    {
      flog_err (EC_LIB_DEVELOPMENT, "%s: can't get afi route node", __func__);
      return;
    }
                  ...
                  if (rfgn->rfg->routemap_export_bgp)
                    {
                      ...
                      info.peer = irfd->peer;
                      info.attr = &hattr;
                      ret = route_map_apply (rfgn->rfg->routemap_export_bgp, p,
                                             &info);
                      if (ret == RMAP_DENYMATCH)
                        {
                          bgp_attr_flush (&hattr);
              ...
              if (rn->info)
                {
                  ...
                  bgp_withdraw (irfd->peer, p, /* prefix */
                                0,             /* addpath_id */
                                afi, SAFI_UNICAST, ZEBRA_ROUTE_VNC_DIRECT,
                                BGP_ROUTE_REDISTRIBUTE, NULL, /* RD not used
                                                                 for unicast */
                                NULL, 0, NULL);               /* tag not
                                                              used for
                                                              unicast */
...
static void
vnc_direct_add_rn_group_rd (struct bgp *bgp, struct rfapi_nve_group_cfg *rfg,
                            struct agg_node *rn, struct attr *attr, afi_t afi,
                            struct rfapi_descriptor *irfd)
{
  struct prefix nhp;
  struct bgp_path_info info;
  ...
  if (irfd == NULL && rfg->type != RFAPI_GROUP_CFG_VRF)
    {
      ...
      if (! rfg->rd.prefixlen)
        {
          vnc_zlog_debug_verbose (
              "%s: VRF \"%s\" is missing RD configuration.", __func__,
              rfg->name);
          return;
        }
      ...
      irfd = XCALLOC (MTYPE_RFAPI_DESC, sizeof (struct rfapi_descriptor));
      irfd->bgp = bgp;
      rfg->rfd = irfd;
      /*
      ...
      if (ret == RMAP_DENYMATCH)
        {
          bgp_attr_flush (&hattr);
          vnc_zlog_debug_verbose (
              "%s: route map says DENY, so not calling bgp_update", __func__);
          return;
        }
    }
 ...
 */
static void
vnc_direct_bgp_add_group_afi (struct bgp *bgp, struct rfapi_nve_group_cfg *rfg,
                              afi_t afi)
{
  struct agg_table *rt = NULL;
  struct agg_node *rn;
  ...
  if (! import_table)
    {
      vnc_zlog_debug_verbose ("%s: import table not defined, returning",
                              __func__);
      return;
    }
      ...
      if (rn->info)
        {
          ...
          if (rfg->plist_export_bgp[afi])
            {
              if (prefix_list_apply (rfg->plist_export_bgp[afi], p) ==
                  PREFIX_DENY)
          ...
          if (rfg->type == RFAPI_GROUP_CFG_VRF)
            {
              vnc_direct_add_rn_group_rd (bgp, rfg, rn, &attr, afi, rfg->rfd);
              /*
               * yuck!
               *  - but consistent with rest of function
               */
          ...
          for (ln = listhead (rfg->nves); ln; ln = listnextnode (ln))
            {
              vnc_direct_add_rn_group_rd (bgp, rfg, rn, &attr, afi,
                                          listgetdata (ln));
            }
        }
    }
...
static void
vnc_direct_del_rn_group_rd (struct bgp *bgp, struct rfapi_nve_group_cfg *rfg,
 ...
 */
static void
vnc_direct_bgp_del_group_afi (struct bgp *bgp, struct rfapi_nve_group_cfg *rfg,
                              afi_t afi)
{
  struct agg_table *rt = NULL;
  struct agg_node *rn;
  ...
  for (rn = agg_route_top (rt); rn; rn = agg_route_next (rn))
    if (rn->info)
      {
        if (rfg->type == RFAPI_GROUP_CFG_VRF)
          vnc_direct_del_rn_group_rd (bgp, rfg, rn, afi, rfg->rfd);
        else
          {
            struct listnode *ln;
             ...
             */
            for (ln = listhead (rfg->nves); ln; ln = listnextnode (ln))
              vnc_direct_del_rn_group_rd (bgp, rfg, rn, afi, listgetdata (ln));
          }
      }
}
...
void
vnc_direct_bgp_reexport_group_afi (struct bgp *bgp,
...
static void
vnc_direct_bgp_unexport_table (afi_t afi, struct agg_table *rt,
              ...
              for (ALL_LIST_ELEMENTS_RO (nve_list, hln, irfd))
                {
                  ...
                  bgp_withdraw (irfd->peer, agg_node_get_prefix (rn),
                                0, /* addpath_id */
                                afi, SAFI_UNICAST, ZEBRA_ROUTE_VNC_DIRECT,
                                BGP_ROUTE_REDISTRIBUTE, NULL, /* RD not used
                                                                 for unicast */
...
static void
import_table_to_nve_list_direct_bgp (struct bgp *bgp,
      ...
      if (rfgn->rfg && rfgn->rfg->rfapi_import_table == it)
        {
          ...
          else if (rfgn->rfg->rfd && rfgn->rfg->type == RFAPI_GROUP_CFG_VRF)
            {
              if (! *nves)
                *nves = list_new ();
...
vnc_direct_bgp_vpn_enable (struct bgp *bgp, afi_t afi)
{
   ...
   */
  for (ALL_LIST_ELEMENTS_RO (bgp->rfapi_cfg->nve_groups_sequential, rfgn, rfg))
    {
  ...
  for (it = bgp->rfapi->imports; it; it = it->next)
    {
      ...
      if (nve_list)
        {
          vnc_direct_bgp_unexport_table (afi, it->imported_vpn[afi], nve_list);
 ...
 */
void
vnc_direct_bgp_rh_add_route (struct bgp *bgp, afi_t afi,
                             ...
                             struct attr *attr)
{
  ...
  if (! VNC_EXPORT_BGP_RH_ENABLED (bgp->rfapi_cfg))
    {
      vnc_zlog_debug_verbose (
          "%s: export-to-bgp RH mode not enabled, skipping", __func__);
      return;
    }
  ...
  if (hc->plist_export_bgp[afi])
    {
      if (prefix_list_apply (hc->plist_export_bgp[afi], prefix) == PREFIX_DENY)
        return;
    }
...
static void
vncExportWithdrawTimer (struct event *t)
{
  ...
  bgp_withdraw (eti->peer, p, 0, /* addpath_id */
                family2afi (p->family), SAFI_UNICAST, eti->type, eti->subtype,
                NULL,           /* RD not used for unicast */
 ...
 */
void
vnc_direct_bgp_rh_del_route (struct bgp *bgp, afi_t afi,
                             const struct prefix *prefix, struct peer *peer)
{
  struct vnc_export_info *eti;
  ...
  if (! eti->timer && eti->lifetime <= INT32_MAX)
    {
      eti->timer = NULL;
      event_add_timer (bm->master, vncExportWithdrawTimer, eti, eti->lifetime,
                       &eti->timer);
...
vnc_direct_bgp_rh_vpn_enable (struct bgp *bgp, afi_t afi)
{
  ...
  if (! VNC_EXPORT_BGP_RH_ENABLED (bgp->rfapi_cfg))
    {
      vnc_zlog_debug_verbose ("%s: export of RH routes not enabled, skipping",
                              __func__);
      return;
    }
      ...
      for (dest = bgp_table_top (table); dest; dest = bgp_route_next (dest))
        {
          ...
          vnc_zlog_debug_verbose ("%s: checking prefix %pBD", __func__, dest);
          ...
          if (hc->plist_export_bgp[afi])
            {
              if (prefix_list_apply (hc->plist_export_bgp[afi], dest_p) ==
                  PREFIX_DENY)
                {

                  vnc_zlog_debug_verbose ("%s:   prefix list says DENY",
                                          __func__);
                  continue;
                }
            }
              ...
              if (ri->sub_type == BGP_ROUTE_NORMAL ||
                  ri->sub_type == BGP_ROUTE_RFP)
                {
                  ...
                  if (encap_attr_export (&hattr, ri->attr, NULL, NULL))
                    {
                      vnc_zlog_debug_verbose ("%s:   encap_attr_export failed",
                                              __func__);
                      continue;
                    }
                  ...
                  if (hc->routemap_export_bgp)
                    {
                      ...
                      info.peer = ri->peer;
                      info.attr = &hattr;
                      ret = route_map_apply (hc->routemap_export_bgp, dest_p,
                                             &info);
                      if (ret == RMAP_DENYMATCH)
                        {
                          bgp_attr_flush (&hattr);
                          vnc_zlog_debug_verbose ("%s:   route map says DENY",
                                                  __func__);
                          continue;
                        }
                    }
                   ...
                   * this route
                   */
                  eti = vnc_eti_get (bgp, EXPORT_TYPE_BGP, dest_p, ri->peer,
      ...
      for (ri = bgp_dest_get_bgp_path_info (dest), next = NULL; ri; ri = next)
        {
          ...
          if (ri->type == ZEBRA_ROUTE_VNC_DIRECT_RH &&
              ri->sub_type == BGP_ROUTE_REDISTRIBUTE)
            {
               ...
               */
              eti = vnc_eti_checktimer (bgp, EXPORT_TYPE_BGP, dest_p, ri->peer,
                                        ZEBRA_ROUTE_VNC_DIRECT_RH,
                                        BGP_ROUTE_REDISTRIBUTE);
              if (eti)
                {
