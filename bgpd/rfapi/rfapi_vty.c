...
void
rfapiQprefix2Rprefix (const struct prefix *qprefix,
...
int
rfapiStream2Vty (void *stream,                           /* input */
...
void
rfapi_vty_out_vncinfo (struct vty *vty, const struct prefix *p,
  ...
  if (bpi->extra != NULL)
    {
      if (bpi->extra->label[0] == BGP_PREVENT_VRF_2_VRF_LEAK)
        ...
        vty_out (vty, " label=%u", decode_label (&bpi->extra->label[0]));
...
rfapiPrintAttrPtrs (void *stream, struct attr *attr)
{
  ...
  fp (out, "  ecommunity=%p, refcnt=%d%s", ecomm, (ecomm ? ecomm->refcnt : 0),
      HVTYNL);
 ...
 */
void
rfapiPrintBi (void *stream, struct bgp_path_info *bpi)
{
  char buf[BUFSIZ];
  char *s;
...
#define INCP                                                                  \
  ...
  if (CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED) && bpi->extra &&
      bpi->extra->vnc.import.timer)
    {
      ...
      r = snprintf (p, REMAIN, " [%4lu] ", event_timer_remain_second (t));
      INCP;
    }
  else
    {
  ...
  if (bpi->extra)
    {
      ...
      if (decode_rd_type (bpi->extra->vnc.import.rd.val) == RD_TYPE_VNC_ETH)
        {
          has_macaddr = 1;
          memcpy (macaddr.octet, bpi->extra->vnc.import.rd.val + 2, 6);
  ...
  if (af == AF_INET)
    {
      r = snprintfrr (p, REMAIN, "%pI4", &bpi->attr->mp_nexthop_global_in);
      INCP;
    }
  else if (af == AF_INET6)
    {
      r = snprintfrr (p, REMAIN, "%pI6", &bpi->attr->mp_nexthop_global);
      INCP;
    }
  else
    {
  ...
  if (has_macaddr)
    {
      fp (out, "        RD HID=%d ETH=%02x:%02x:%02x:%02x:%02x:%02x%s", l2hid,
  ...
  if (bpi->extra && bpi->extra->vnc.import.aux_prefix.family)
    {
      ...
      sp = rfapi_ntop (bpi->extra->vnc.import.aux_prefix.family,
                       &bpi->extra->vnc.import.aux_prefix.u.prefix, buf,
                       BUFSIZ);
      buf[BUFSIZ - 1] = 0;
      if (sp)
        {
  ...
  {
    struct rfapi_un_option *uo = rfapi_encap_tlv_to_un_option (bpi->attr);
    if (uo)
      {
...
rfapiMonitorVpn2Str (struct rfapi_monitor_vpn *m, char *buf, int size)
{
  ...
  rc = snprintf (
      buf, size, "m=%p, next=%p, rfd=%p(vn=%s un=%s), p=%s/%d, node=%p", m,
      m->next, m->rfd, buf_vn, buf_un,
      inet_ntop (m->p.family, &m->p.u.prefix, buf_pfx, sizeof (buf_pfx)),
...
void
rfapiShowItNode (void *stream, struct agg_node *rn)
{
...
void
rfapiShowImportTable (void *stream, const char *label, struct agg_table *rt,
                      int isvpn)
{
  struct agg_node *rn;
  char buf[BUFSIZ];
          ...
          for (rn = agg_route_top (rfd->mon); rn; rn = agg_route_next (rn))
            {
              ...
              if (! printedquerier)
                ...
                fp (out, "%-15s %-15s", "", "");
              buf_remain[0] = 0;
              rfapiFormatSeconds (event_timer_remain_second (m->timer),
                                  buf_remain, BUFSIZ);
              fp (out, " %-15s %-10s\n",
      ...
      if (rfd->mon_eth && skiplist_count (rfd->mon_eth))
        {
          ...
          for (cursor = NULL, rc = skiplist_next (rfd->mon_eth, NULL,
                                                  (void **) &mon_eth, &cursor);
                                            ...
                                            (void **) &mon_eth, &cursor))
            {
              ...
              vnc_zlog_debug_verbose ("%s: checking rfd=%p mon_eth=%p",
              ...
              if (! printedheader)
                {
                  ...
                  fp (out, "%-15s %-15s %-17s %10s %-10s\n", "VN Address",
                      "UN Address", "Target", "LNI", "Remaining");
                }
              ...
              if (! printedquerier)
                ...
                fp (out, "%-15s %-15s", "", "");
              buf_remain[0] = 0;
              rfapiFormatSeconds (event_timer_remain_second (mon_eth->timer),
                                  buf_remain, BUFSIZ);
              fp (out, " %-17s %10d %-10s\n",
...
static int
rfapiPrintRemoteRegBi (struct bgp *bgp, void *stream, struct agg_node *rn,
                       struct bgp_path_info *bpi)
{
  int (*fp) (void *, const char *, ...);
  struct vty *vty;
  ...
  if (CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED) && bpi->extra &&
  ...
  else if (RFAPI_LOCAL_BI (bpi))
    {
      ...
      if (bpi->extra && bpi->extra->vnc.import.create_time)
        {
          rfapiFormatAge (bpi->extra->vnc.import.create_time, buf_age, BUFSIZ);
        }
      else
        {
          buf_age[0] = '?';
...
static int
rfapiShowRemoteRegistrationsIt (struct bgp *bgp, void *stream,
          ...
          for (bpi = rn->info; bpi; bpi = bpi->next)
            {
              ...
              if (show_expiring && ! CHECK_FLAG (bpi->flags, BGP_PATH_REMOVED))
                continue;
              ...
              if (! printed_rtlist_hdr)
                {
                  ...
                  if (pLni)
                    ...
                    {
                      fp (out, "%s[%s] Prefix RT={%s}", HVTYNL, type, s);
                    }
                  XFREE (MTYPE_ECOMMUNITY_STR, s);
                  ...
                  if (it->rfg && it->rfg->name)
                    {
                      fp (out, " %s \"%s\"",
                          (it->rfg->type == RFAPI_GROUP_CFG_VRF ? "VRF"
                                                                : "NVE group"),
                          it->rfg->name);
                    }
                  fp (out, "%s", HVTYNL);
                  ...
                  fp (out, "%-20s %-15s %-15s %4s %-10s %-10s%s",
                      (pLni ? "L2 Address/IP" : "Prefix"), "VN Address",
                      "UN Address", "Cost", "Lifetime", agetype, HVTYNL);
                }
              printed += rfapiPrintRemoteRegBi (bgp, stream, rn, bpi);
            }
        }
    }
  ...
  if (printed > 0)
    {
      ...
      fp (out, "Displayed %d out of %d %s%s", printed, total, type, HVTYNL);
#if DEBUG_SHOW_EXTRA
 ...
 */
int
rfapiShowRemoteRegistrations (void *stream, struct prefix *prefix_only,
  ...
  for (it = h->imports; it; it = it->next)
    {
      printed += rfapiShowRemoteRegistrationsIt (
          bgp, stream, it, prefix_only, show_expiring, show_local, show_remote,
          show_imported, NULL);
    }
  ...
  if (h->import_mac)
    {
      ...
      for (rc = skiplist_next (h->import_mac, (void **) &lni_as_ptr,
                               (void **) &it, &cursor);
           ! rc; rc = skiplist_next (h->import_mac, (void **) &lni_as_ptr,
                                     (void **) &it, &cursor))
        {
          pLni = NULL;
...
void
rfapiPrintAdvertisedInfo (struct vty *vty, struct rfapi_descriptor *rfd,
  ...
  for (afi = AFI_IP; afi < AFI_MAX; ++afi)
    {
      ...
      for (rc = skiplist_next (rfd->advertised.ipN_by_prefix, NULL,
                               (void **) &adb, &cursor);
           rc == 0; rc = skiplist_next (rfd->advertised.ipN_by_prefix, NULL,
                                        (void **) &adb, &cursor))
        {
          ...
          vty_out (vty, "  Adv Pfx: %pFX%s", &adb->u.s.prefix_ip, HVTYNL);
          rfapiPrintAdvertisedInfo (vty, rfd, SAFI_MPLS_VPN,
                                    &adb->u.s.prefix_ip);
        }
    }
  for (rc = skiplist_next (rfd->advertised.ip0_by_ether, NULL, (void **) &adb,
                           &cursor);
 ...
 */
void
rfapiPrintMatchingDescriptors (struct vty *vty, struct prefix *vn_prefix,
  ...
  for (ln = listhead (&h->descriptors); ln; ln = listnextnode (ln))
    {
      ...
      if (! printed)
        {
          /* print column header */
          vty_out (vty, "%s %s %s %s %s %s %s %s%s", "descriptor", "un-addr",
                   "vn-addr", "callback", "cookie", "RD", "lifetime", "group",
                   HVTYNL);
        }
...
rfapiCliGetPrefixAddr (struct vty *vty, const char *str, struct prefix *p)
{
  if (! str2prefix (str, p))
    {
      vty_out (vty, "Malformed address \"%s\"%s", str ? str : "null", HVTYNL);
      return CMD_WARNING;
    }
  switch (p->family)
    {
...
int
rfapiCliGetRfapiIpAddr (struct vty *vty, const char *str,
              ...
              switch (vo->type)
                ...
                case RFAPI_VN_OPTION_TYPE_LOCAL_NEXTHOP:
                  fp (out, "%sLNH %pFX cost=%d%s", offset,
                      &vo->v.local_nexthop.addr, vo->v.local_nexthop.cost,
                      HVTYNL);
                  break;

                case RFAPI_VN_OPTION_TYPE_INTERNAL_RD:
                  fp (out, "%svn option type %d (unknown)%s", offset, vo->type,
                      HVTYNL);
                  break;
                }
            }
          ...
          for (uo = nh->un_options; uo; uo = uo->next)
            {
              switch (uo->type)
                ...
                case RFAPI_UN_OPTION_TYPE_PROVISIONAL:
                  fp (out, "%sUN Option type %d%s", offset, uo->type,
                      vty_newline);
                  break;
                }
            }
 ...
 */
static void
rfapiAddDeleteLocalRfpPrefix (struct rfapi_ip_addr *un_addr,
                              struct rfapi_ip_addr *vn_addr,
                              struct rfapi_ip_prefix *rprefix, int is_add,
...
static int
register_add (
    struct vty *vty, struct cmd_token *carg_prefix, struct cmd_token *carg_vn,
    struct cmd_token *carg_un, struct cmd_token *carg_cost, /* optional */
    ...
    int argc, struct cmd_token **argv)
{
  ...
  if (arg_lnh_cost)
    {
      if (! arg_lnh)
        {
          vty_out (vty, "%% %s may only be specified with local-next-hop\n",
                   "local-cost");
          goto fail;
        }
      endptr = NULL;
  ...
  if (arg_lnh)
    {
      ...
      if ((rc = rfapiCliGetPrefixAddr (
               vty, arg_lnh, &optary[opt_next].v.local_nexthop.addr)))
        {
  ...
  if (arg_macaddr)
    {
      if (! arg_vni)
        {
          vty_out (vty, "Missing \"vni\" parameter (mandatory with mac)\n");
          return CMD_WARNING_CONFIG_FAILED;
        }
      optary[opt_next].v.l2addr.logical_net_id = strtoul (arg_vni, NULL, 10);
  ...
  vnc_zlog_debug_verbose (
      "%s: vn=%s, un=%s, prefix=%s, cost=%s, lifetime=%s, lnh=%s", __func__,
      arg_vn, arg_un, arg_prefix, (arg_cost ? arg_cost : "NULL"),
      (arg_lifetime ? arg_lifetime : "NULL"), (arg_lnh ? arg_lnh : "NULL"));
      ...
      if (ENOENT == rc)
        {
          ...
          rc = rfapi_open (rfapi_get_rfp_start_val_by_bgp (bgp), &vn_address,
                           &un_address, &uo, /* flags */
  ...
  rc = rfapi_register (rfd, &rpfx, lifetime, NULL, opt, RFAPI_REGISTER_ADD);
  if (! rc)
    {
      ...
      vnc_zlog_debug_verbose ("%s: rfapi_register succeeded, returning 0",
                              __func__);
      ...
      if (h->rfp_methods.local_cb)
        {
          ...
          rfapiAddDeleteLocalRfpPrefix (&r->un_addr, &r->vn_addr, &rpfx, 1,
                                        lifetime, vn_opt_new, &head, &tail);
          if (head)
            {
...
static int
nve_addr_cmp (const void *k1, const void *k2)
{
...
static int
parse_deleter_args (struct vty *vty, struct bgp *bgp, const char *arg_prefix,
                    const char *arg_vn, const char *arg_un,
                    const char *arg_l2addr, const char *arg_vni,
                    const char *arg_rd, struct rfapi_nve_group_cfg *arg_rfg,
                      ...
                      struct rfapi_local_reg_delete_arg *rcdarg)
{
  ...
  return parse_deleter_args (vty, bgp, arg_prefix, arg_vn, arg_un, arg_l2addr,
                             arg_vni, arg_rd, arg_rfg, rcdarg);
}

static void
record_nve_in_cda_list (struct rfapi_local_reg_delete_arg *cda,
                        struct rfapi_ip_addr *un_address,
                        struct rfapi_ip_addr *vn_address,
                        struct rfapi_descriptor *rfd)
{
  ...
  if (cda->vn_address.addr_family && cda->un_address.addr_family)
    {
       ...
       * Single nve case
       */
      if (rfapi_find_rfd (bgp_default, &cda->vn_address, &cda->un_address,
                          &rfd))
        return;
...
#if DEBUG_L2_EXTRA
                    vnc_zlog_debug_verbose (
                        "%s: adb=%p, prefix doesn't match, skipping", __func__,
                        adb);
#endif
                    continue;
                  }
...
#if DEBUG_L2_EXTRA
                    vnc_zlog_debug_verbose (
                        "%s: adb=%p, RD doesn't match, skipping", __func__,
                        adb);
#endif
                    continue;
                  }
            ...
            if (CHECK_FLAG (cda->l2o.flags, RFAPI_L2O_MACADDR))
              {
                if (memcmp (cda->l2o.o.macaddr.octet,
                            adb->u.s.prefix_eth.u.prefix_eth.octet, ETH_ALEN))
                  {
#if DEBUG_L2_EXTRA
                    vnc_zlog_debug_verbose (
...
#if DEBUG_L2_EXTRA
                    vnc_zlog_debug_verbose (
                        "%s: adb=%p, LNI doesn't match, skipping", __func__,
                        adb);
#endif
                    continue;
                  }
...
#if DEBUG_L2_EXTRA
            vnc_zlog_debug_verbose ("%s: ipN adding adb %p to delete list",
                                    __func__, adb);
#endif
...
#if DEBUG_L2_EXTRA
            vnc_zlog_debug_verbose ("%s: ipN killing reg from adb %p ",
                                    __func__, adb);
#endif

            rc = rfapi_register (rfd, &rp, 0, NULL, (cur_opt ? optary : NULL),
                                 RFAPI_REGISTER_KILL);
            if (! rc)
              {
...
clear_vnc_nve_closer (struct rfapi_local_reg_delete_arg *cda)
{
  ...
  for (rc = skiplist_next (sl, (void **) &pKey, (void **) &pValue, &cursor);
       ...
       rc = skiplist_next (sl, (void **) &pKey, (void **) &pValue, &cursor))
    {
      ...
      if (pValue->rfd)
        {
          pValue->rfd->flags |= RFAPI_HD_FLAG_CLOSING_ADMINISTRATIVELY;
       ...
       "For all NVEs\n")
{
  ...
  if ((rc = parse_deleter_args (vty, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                                NULL, &cda)))
    return rc;
       ...
       "VN IPv6 interface address\n")
{
  ...
  if ((rc = parse_deleter_tokens (vty, NULL, NULL, argv[4], NULL, NULL, NULL,
                                  NULL, NULL, &cda)))
    return rc;
       ...
       "UN IPv6 interface address\n")
{
  ...
  if ((rc = parse_deleter_tokens (vty, NULL, NULL, NULL, argv[4], NULL, NULL,
                                  NULL, NULL, &cda)))
    return rc;
       ...
       "UN IPv6 interface address\n")
{
  ...
  if ((rc = parse_deleter_tokens (vty, NULL, argv[3], argv[5], argv[7], NULL,
                                  NULL, NULL, NULL, &cda)))
    return rc;
  cda.vty = vty;
  clear_vnc_prefix (&cda);
       ...
       "VN IPv6 interface address\n")
{
  ...
  if ((rc = parse_deleter_tokens (vty, NULL, argv[3], argv[7], argv[5], NULL,
                                  NULL, NULL, NULL, &cda)))
    return rc;
  cda.vty = vty;
  clear_vnc_prefix (&cda);
       ...
       "From any NVE\n")
{
  ...
  if ((rc = parse_deleter_tokens (vty, NULL, argv[3], NULL, NULL, NULL, NULL,
                                  NULL, NULL, &cda)))
    return rc;
  cda.vty = vty;
  clear_vnc_prefix (&cda);
       ...
       "UN IPv6 interface address\n")
{
  ...
  /* pfx vn un L2 VNI */
  if ((rc = parse_deleter_tokens (vty, NULL, NULL, argv[7], argv[9], argv[3],
       ...
       "VN IPv6 interface address\n")
{
  ...
  /* pfx vn un L2 VNI */
  if ((rc = parse_deleter_tokens (vty, NULL, NULL, argv[9], argv[7], argv[3],
                                  argv[5], NULL, NULL, &cda)))
    return rc;
  cda.vty = vty;
  clear_vnc_prefix (&cda);
       ...
       "IPv6 Prefix\n")
{
  ...
  /* pfx vn un L2 VNI */
  if ((rc = parse_deleter_tokens (vty, NULL, argv[9], NULL, argv[7], argv[3],
                                  argv[5], NULL, NULL, &cda)))
    return rc;
  cda.vty = vty;
  clear_vnc_prefix (&cda);
       ...
       "IPv6 Prefix\n")
{
  ...
  /* pfx vn un L2 VNI */
  if ((rc = parse_deleter_tokens (vty, NULL, argv[9], argv[7], NULL, argv[3],
                                  argv[5], NULL, NULL, &cda)))
    return rc;
  cda.vty = vty;
  clear_vnc_prefix (&cda);
  ...
  if (is_vnc_running || show_type == SHOW_NVE_SUMMARY_REGISTERED)
    {
      ...
      switch (show_type)
        ...
        case SHOW_NVE_SUMMARY_REGISTERED:
          ...
          if (is_vnc_running)
            {
              ...
              vty_out (vty, "%-8s %-8u ",
                       "Active:", rfapiApCountAll (bgp_default));
              vty_out (vty, "%-8s %-8u ",
                       "Failed:", h->stat.count_registrations_failed);
              vty_out (vty, "%-8s %-8u",
          ...
          rfapiCountAllItRoutes (&active_local_routes, &active_remote_routes,
        ...
        case SHOW_NVE_SUMMARY_QUERIES:
          ...
          vty_out (vty, "%-8s %-8u ", "Failed:", h->stat.count_queries_failed);
...
static int
rfapi_show_nves (struct vty *vty, struct prefix *vn_prefix,
                 struct prefix *un_prefix)
{
  // struct hash                      *rfds;
      ...
      if (! printed)
        {
          ...
          vty_out (vty, "%-15s %-15s %-5s %-5s %-6s %-6s %s\n", "VN Address",
                   "UN Address", "Regis", "Resps", "Reach", "Remove", "Age");
        }

      ++printed;

      vty_out (vty, "%-15s %-15s %-5u %-5u %-6u %-6u %s\n", vn_addr_buf,
               un_addr_buf, rfapiApCount (rfd), rfapi_monitor_count (rfd),
               rfd->stat_count_nh_reachable, rfd->stat_count_nh_removal,
...
static void
rfapi_show_registrations (struct vty *vty, struct prefix *restrict_to,
                          int show_local, int show_remote, int show_holddown,
                          int show_imported)
{
  int printed = 0;
 ...
 ************************************************************************/
void
vnc_add_vrf_opener (struct bgp *bgp, struct rfapi_nve_group_cfg *rfg)
{
  if (rfg->rfd == NULL)
...
static int
vnc_add_vrf_prefix (struct vty *vty, const char *arg_vrf,
                    const char *arg_prefix, const char *arg_rd, /* optional */
      ...
      if (bgp->rfapi->rfp_methods.local_cb)
        {
          ...
          rfapiAddDeleteLocalRfpPrefix (&r->un_addr, &r->vn_addr, &rpfx, 1,
                                        RFAPI_INFINITE_LIFETIME, vn_opt_new,
                                        &head, &tail);
          if (head)
            {
...
clear_vnc_vrf_closer (struct rfapi_nve_group_cfg *rfg)
{
  ...
  for (afi = 0;
       afi < AFI_MAX && rfg->rfapi_import_table->local_count[afi] == 0; afi++)
    ;
  ...
  if (afi == AFI_MAX)
    ...
    {
      vnc_zlog_debug_verbose ("%s: VRF %s afi=%d count=%d", __func__,
                              rfg->name, afi,
...
static int
vnc_clear_vrf (struct vty *vty, struct bgp *bgp, const char *arg_vrf,
