 ...
 */
static int
sl_adb_lifetime_cmp (const void *adb1, const void *adb2)
{
...
rfapiApReadvertiseAll (struct bgp *bgp, struct rfapi_descriptor *rfd)
{
  ...
  for (rc = skiplist_next (rfd->advertised.by_lifetime, NULL, (void **) &adb,
                           &cursor);
          ...
          if (rfapiRaddr2Qprefix (&rfd->vn_addr, &pfx_vn_buf))
            {
              ...
              vnc_zlog_debug_verbose (
                  "%s: BAD: handle has bad vn_addr: skipping", __func__);
              continue;
            }
        }
  ...
  if (find_min || find_max)
    {
      ...
      if (! skiplist_first (rfd->advertised.by_lifetime, (void **) &adb_min,
                            NULL) &&
      ...
      else
        {
          ...
          cursor = NULL;
          for (rc = skiplist_next (rfd->advertised.ip0_by_ether, (void **) &rk,
                                   (void **) &adb, &cursor);
               ! rc;
               rc = skiplist_next (rfd->advertised.ip0_by_ether, (void **) &rk,
                                   (void **) &adb, &cursor))
            {
 ...
 */
int
rfapiApAdd (struct bgp *bgp, struct rfapi_descriptor *rfd,
            ...
            struct rfapi_l2address_option *l2o) /* other options TBD */
{
  ...
  if (RFAPI_0_PREFIX (pfx_ip) && RFAPI_HOST_PREFIX (pfx_ip))
    {
      use_ip0 = 1;
      assert (pfx_eth);
      rc = skiplist_search (rfd->advertised.ip0_by_ether, &rk, (void **) &adb);
    }
  else
    {
  ...
  if (rc)
    {
      ...
      if (use_ip0)
        {
          assert (pfx_eth);
          skiplist_insert (rfd->advertised.ip0_by_ether, &adb->u.key, adb);
        }
      else
        {
          skiplist_insert (rfd->advertised.ipN_by_prefix, &adb->u.key, adb);
        }
  ...
  if (rfapiApAdjustLifetimeStats (rfd, (rc ? NULL : &old_lifetime), &lifetime))
    return 1;
 ...
 */
int
rfapiApDelete (struct bgp *bgp, struct rfapi_descriptor *rfd,
               struct prefix *pfx_ip, struct prefix *pfx_eth,
