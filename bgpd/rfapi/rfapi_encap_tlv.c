...
static void
subtlv_print_encap_gre (void *stream, int column_offset,
...
static void
subtlv_print_encap_pbb (void *stream, int column_offset,
                        struct bgp_tea_subtlv_encap_pbb *st)
{
  ...
  if (st->flag_isid)
    {
      fp (out, "%*s  ISID: %d (0x%x)%s", column_offset, "", st->isid, st->isid,
          vty_newline);
    }
  if (st->flag_vid)
    {
...
static void
subtlv_print_proto_type (void *stream, int column_offset,
...
static void
subtlv_print_color (void *stream, int column_offset,
...
static void
subtlv_print_ipsec_ta (void *stream, int column_offset,
...
static void
print_encap_type_gre (void *stream, int column_offset,
...
static void
print_encap_type_ip_in_ip (void *stream, int column_offset,
...
static void
print_encap_type_pbb (void *stream, int column_offset,
...
static void
print_encap_type_vxlan (void *stream, int column_offset,
...
static void
print_encap_type_nvgre (void *stream, int column_offset,
...
static void
print_encap_type_mpls (void *stream, int column_offset,
...
static void
print_encap_type_mpls_in_gre (void *stream, int column_offset,
...
static void
print_encap_type_vxlan_gpe (void *stream, int column_offset,
...
static void
print_encap_type_mpls_in_udp (void *stream, int column_offset,
...
void
rfapi_print_tunneltype_option (void *stream, int column_offset,
                               struct rfapi_tunneltype_option *tto)
{
  switch (tto->type)
    {
    ...
    case BGP_ENCAP_TYPE_TRANSMIT_TUNNEL_ENDPOINT:
      print_encap_type_transmit_tunnel_endpoint (
          stream, column_offset, &tto->bgpinfo.transmit_tunnel_endpoint);
      break;
