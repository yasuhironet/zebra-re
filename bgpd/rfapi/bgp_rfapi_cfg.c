...
void
nve_group_to_nve_list (struct rfapi_nve_group_cfg *rfg, struct list **nves,
...
bgp_rfapi_cfg_match_group (struct rfapi_cfg *hc, struct prefix *vn,
                           struct prefix *un)
{
  ...
  vnc_zlog_debug_verbose (
      "%s: shouldn't happen, returning NULL when un and vn match", __func__);
                            ...
                            rfapi_group_cfg_type_t type) /* _MAX = any */
{
  ...
  for (ALL_LIST_ELEMENTS (bgp->rfapi_cfg->nve_groups_sequential, node, nnode,
                          rfg))
    {
...
rfapi_group_new (struct bgp *bgp, rfapi_group_cfg_type_t type,
                 const char *name)
{
  struct rfapi_nve_group_cfg *rfg;
...
static int
rfapi_str2route_type (const char *l3str, const char *pstr, afi_t *afi,
                      int *type)
{
  if (! l3str || ! pstr)
    return EINVAL;
       ...
       "Group name\n")
{
  ...
  bgp->rfapi_cfg->rfg_redist =
      bgp_rfapi_cfg_match_byname (bgp, argv[3]->arg, RFAPI_GROUP_CFG_NVE);
       ...
       "Route-map for filtering redistributed routes\n")
{
  ...
  route_map_counter_decrement (rfg->routemap_redist[ZEBRA_ROUTE_BGP_DIRECT]);
       ...
       "route map name\n")
{
  ...
  route_map_counter_increment (rfg->routemap_redist[ZEBRA_ROUTE_BGP_DIRECT]);
       ...
       "Group name\n")
{
  ...
  rfg_new =
      bgp_rfapi_cfg_match_byname (bgp, argv[5]->arg, RFAPI_GROUP_CFG_NVE);
  if (rfg_new == NULL)
    {
      rfg_new =
          bgp_rfapi_cfg_match_byname (bgp, argv[5]->arg, RFAPI_GROUP_CFG_VRF);
      if (rfg_new)
  ...
  if (argv[2]->arg[0] == 'b')
    {
      ...
      if (rfg_new)
        {
          vnc_zlog_debug_verbose ("%s: testing bgp grp mode enabled",
                                  __func__);
          if (VNC_EXPORT_BGP_GRP_ENABLED (bgp->rfapi_cfg))
            vnc_zlog_debug_verbose ("%s: calling vnc_direct_bgp_add_group",
                                    __func__);
      ...
      for (ALL_LIST_ELEMENTS (bgp->rfapi_cfg->rfg_export_direct_bgp_l, node,
                              nnode, rfgn))
        {
          ...
          if (rfgn->name && ! strcmp (rfgn->name, argv[6]->arg))
            {
              vnc_zlog_debug_verbose ("%s: matched \"%s\"", __func__,
                                      rfgn->name);
              if (rfgn->rfg)
  ...
  else
    {
      if (idx == argc ||
          (rfg->plist_export_zebra_name[afi] &&
           strmatch (argv[idx]->arg, rfg->plist_export_zebra_name[afi])))
        {
  ...
  if (is_bgp)
    {
      ...
      rfg->plist_export_bgp[afi] = prefix_list_lookup (afi, argv[idx]->arg);
  ...
  else
    {
      ...
      rfg->plist_export_zebra[afi] = prefix_list_lookup (afi, argv[idx]->arg);
  ...
  if (is_bgp)
    {
      ...
      rfg->routemap_export_bgp = route_map_lookup_by_name (argv[idx]->arg);
  ...
  else
    {
      ...
      rfg->routemap_export_zebra = route_map_lookup_by_name (argv[idx]->arg);
          ...
          if (rfg->routemap_redist_name[i])
            {
              ...
              if (! old)
                route_map_counter_increment (rfg->routemap_redist[i]);
            }
        }
      ...
      if (hc->routemap_redist_name[i])
        {
          ...
          if (! old)
            route_map_counter_increment (hc->routemap_redist[i]);
        }
    }
  ...
  if (! rfg)
    {
      ...
      for (ALL_LIST_ELEMENTS (bgp->rfapi_cfg->rfg_export_zebra_l, node, nnode,
                              rfgn))
        {
          ...
          vnc_zlog_debug_verbose (
              "%s: ezport zebra: checking if \"%s\" == \"%s\"", __func__,
              rfgn->name, rfg->name);
          if (! strcmp (rfgn->name, rfg->name))
            {
              rfgn->rfg = rfg;
...
static void
bgp_rfapi_delete_nve_group (struct vty *vty, /* NULL = no output */
                            struct bgp *bgp, struct rfapi_nve_group_cfg *rfg)
{
...
static void
bgp_rfapi_delete_l2_group (struct vty *vty, /* NULL = no output */
                           struct bgp *bgp, struct rfapi_l2_group_cfg *rfg)
{
  /* delete it */
  free (rfg->name);
  if (rfg->rt_import_list)
...
bgp_rfapi_cfg_init (void)
{
  ...
  install_element (BGP_VNC_NVE_GROUP_NODE,
                   &vnc_nve_group_export_prefixlist_cmd);
  install_element (BGP_VNC_NVE_GROUP_NODE, &vnc_nve_group_export_routemap_cmd);
  install_element (BGP_VNC_NVE_GROUP_NODE,
                   &vnc_nve_group_export_no_prefixlist_cmd);
  install_element (BGP_VNC_NVE_GROUP_NODE,
                   &vnc_nve_group_export_no_routemap_cmd);
  ...
  install_element (BGP_VRF_POLICY_NODE, &vnc_vrf_policy_export_prefixlist_cmd);
  install_element (BGP_VRF_POLICY_NODE, &vnc_vrf_policy_export_routemap_cmd);
...
static void
bgp_rfapi_rfgn_list_delete (void *data)
{
            ...
            if (rfg->vn_prefix.family)
              {
                ...
                inet_ntop (rfg->vn_prefix.family, &rfg->vn_prefix.u.prefix,
                           buf, sizeof (buf));
  ...
  { /* was based on listen ports */
        ...
        for (ALL_LIST_ELEMENTS_RO (hc->l2_groups, gnode, rfgc))
          {
            ...
            if (rfgc->logical_net_id != 0)
              vty_out (vty, "   logical-network-id %u\n",
                       rfgc->logical_net_id);
            if (rfgc->labels != NULL && listhead (rfgc->labels) != NULL)
              {
                vty_out (vty, "   labels ");
                for (ALL_LIST_ELEMENTS_RO (rfgc->labels, lnode, data))
                  {
            ...
            if (rfgc->rt_import_list && rfgc->rt_export_list &&
                ecommunity_cmp (rfgc->rt_import_list, rfgc->rt_export_list))
              {
                char *b = ecommunity_ecom2str (rfgc->rt_import_list,
                                               ECOMMUNITY_FORMAT_ROUTE_MAP,
                                               ECOMMUNITY_ROUTE_TARGET);
                vty_out (vty, "   rt both %s\n", b);
                XFREE (MTYPE_ECOMMUNITY_STR, b);
            ...
            else
              {
                ...
                if (rfgc->rt_export_list)
                  {
                    char *b = ecommunity_ecom2str (rfgc->rt_export_list,
                                                   ECOMMUNITY_FORMAT_ROUTE_MAP,
                                                   ECOMMUNITY_ROUTE_TARGET);
                    vty_out (vty, "  rt export %s\n", b);
                    XFREE (MTYPE_ECOMMUNITY_STR, b);
            ...
            if (bgp->rfapi->rfp_methods.cfg_group_cb)
              write += (bgp->rfapi->rfp_methods.cfg_group_cb) (
                  vty, bgp->rfapi->rfp, RFAPI_RFP_CFG_GROUP_L2, rfgc->name,
                  rfgc->rfp_cfg);
    ...
    if (hc->default_rd.prefixlen ||
        ...
        hc->nve_groups_sequential->count)
      {
        ...
        if (bgp->rfapi->rfp_methods.cfg_group_cb)
          write += (bgp->rfapi->rfp_methods.cfg_group_cb) (
              vty, bgp->rfapi->rfp, RFAPI_RFP_CFG_GROUP_DEFAULT, NULL,
      ...
      if (rfg->type == RFAPI_GROUP_CFG_NVE)
        {
          ...
          for (afi = AFI_IP; afi < AFI_MAX; ++afi)
            {
              ...
              if (rfg->plist_redist_name[ZEBRA_ROUTE_BGP_DIRECT][afi])
                {
                  vty_out (
                      vty, "  redistribute bgp-direct %s prefix-list %s\n",
                      afistr,
                      rfg->plist_redist_name[ZEBRA_ROUTE_BGP_DIRECT][afi]);
                }
              if (rfg->plist_redist_name[ZEBRA_ROUTE_BGP_DIRECT_EXT][afi])
          ...
          if (bgp->rfapi->rfp_methods.cfg_group_cb)
            write += (bgp->rfapi->rfp_methods.cfg_group_cb) (
                vty, bgp->rfapi->rfp, RFAPI_RFP_CFG_GROUP_NVE, rfg->name,
                rfg->rfp_cfg);
          vty_out (vty, " exit-vnc\n");
          vty_out (vty, "!\n");
        }
...
bgp_rfapi_show_summary (struct bgp *bgp, struct vty *vty)
{
      ...
      for (type = 0; type < ZEBRA_ROUTE_MAX; ++type)
        {
          if (hc->redist[afi][type])
            {
              vty_out (vty, "{%s,%s} ", ((afi == AFI_IP) ? "ipv4" : "ipv6"),
                       zebra_route_string (type));
              redist++;
            }
        }
  ...
  vty_out (
      vty, "%-39s %3u%-16s %s\n",
      "RFP Registration Hold-Down Factor:", hc->rfp_cfg.holddown_factor, "%",
      (hc->rfp_cfg.holddown_factor == RFAPI_RFP_CFG_DEFAULT_HOLDDOWN_FACTOR
           ? "(default)"
           : ""));
  ...
  vty_out (
      ...
      (hc->rfp_cfg.download_type == RFAPI_RFP_DOWNLOAD_FULL ? "On" : "Off"),
  ...
  vty_out (vty, "%-39s %d seconds\n",
           "Default RFP response lifetime:", hc->default_response_lifetime);
  vty_out (vty, "\n");
  return;
}
