          ...
          if (pfx_active != rfd->rib_prefix_count)
            {
              vnc_zlog_debug_verbose (
                  "%s: rfd %p actual pfx count %u != running %u", __func__,
  ...
  if (checkstats && bgp->rfapi)
    {
      if (t_pfx_active != bgp->rfapi->rib_prefix_count_total)
        {
          vnc_zlog_debug_verbose (
              "%s: actual total pfx count %u != running %u", __func__,
              t_pfx_active, bgp->rfapi->rib_prefix_count_total);
          assert (0);
        }
    }
 ...
 */
static void
rfapiRibExpireTimer (struct event *t)
{
                    ...
                    int deleted)
{
  ...
  if (ri->timer)
    ...
    {
      tcb = XCALLOC (MTYPE_RFAPI_RECENT_DELETE, sizeof (struct rfapi_rib_tcb));
    }
#if DEBUG_CLEANUP
  zlog_debug ("%s: rfd %p, rn %p, ri %p, tcb %p", __func__, rfd, rn, ri, tcb);
#endif
 ...
 */
int
rfapi_rib_key_cmp (const void *k1, const void *k2)
{
              ...
              if (rn->info)
                {
                  ...
                  while (0 == skiplist_first ((struct skiplist *) rn->info,
                                              NULL, (void **) &ri))
                    {
...
/*
 * Copies struct bgp_path_info to struct rfapi_info, except for rk fields and
 * un
 */
static void
rfapiRibBi2Ri (struct bgp_path_info *bpi, struct rfapi_info *ri,
               uint32_t lifetime)
{
       ...
       pEncap = pEncap->next)
    {
      ...
      switch (pEncap->type)
        ...
        case BGP_VNC_SUBTLV_TYPE_RFPOPTION:
          hop =
              XCALLOC (MTYPE_BGP_TEA_OPTIONS, sizeof (struct bgp_tea_options));
          assert (hop);
          hop->type = pEncap->value[0];
          hop->length = pEncap->value[1];
          hop->value =
              XCALLOC (MTYPE_BGP_TEA_OPTIONS_VALUE, pEncap->length - 2);
          assert (hop->value);
 ...
 */
static void
process_pending_node (struct bgp *bgp, struct rfapi_descriptor *rfd, afi_t afi,
                      ...
                      struct rfapi_next_hop_entry **tail)
{
  ...
  vnc_zlog_debug_verbose ("%s: afi=%d, %pRN pn->info=%p", __func__, afi, pn,
                          pn->info);
          ...
          while (0 == skiplist_first (slRibPt, NULL, (void **) &ri))
            {
              ...
              vnc_zlog_debug_verbose (
                  "%s: after listnode_add, delete_list->count=%d", __func__,
                  delete_list->count);
              ...
              vnc_zlog_debug_verbose (
                  ...
                  __func__, pn, &ri->rk.vn, &ri->un, ri->cost, ri->lifetime,
                  ri->vn_options);
  ...
  vnc_zlog_debug_verbose ("%s:   lPendCost->count=%d, slRibPt->count=%d",
                          __func__, (lPendCost ? (int) lPendCost->count : -1),
                  ...
                  if (lPendCost)
                    {
                       ...
                       * optimization */
                      listnode_delete (lPendCost, ri); /* XXX
                                                          doesn't
                                                          free
                                                          data!
                                                          bug? */
...
#if DEBUG_PROCESS_PENDING_NODE
              vnc_zlog_debug_verbose (
                  "%s:   slRibPt ri %p matched in pending list, %s", __func__,
                  ori, (same ? "same info" : "different info"));
#endif
            }
        }
      /*
      ...
      if (delete_list)
        {
          for (ALL_LIST_ELEMENTS_RO (delete_list, node, ri))
            {
              vnc_zlog_debug_verbose ("%s:   deleting ri %p from slRibPt",
                                      __func__, ri);
      ...
      for (ALL_LIST_ELEMENTS (lPendCost, node, nnode, ri))
        {
          ...
          *tail = new;
          ...
          vnc_zlog_debug_verbose ("%s:   add vn=%s un=%s cost=%d life=%d",
  ...
  if (delete_list)
    {
      ...
      if (! printedprefix)
        {
          vnc_zlog_debug_verbose ("%s: For prefix %pRN (d)", __func__, pn);
        }
      vnc_zlog_debug_verbose ("%s: delete_list has %d elements", __func__,
                              delete_list->count);
      ...
      if (! CHECK_FLAG (bgp->rfapi_cfg->flags,
          ...
          for (ALL_LIST_ELEMENTS (delete_list, node, nnode, ri))
            {
              ...
              *tail = new;
              ...
              rfapiRfapiIpAddr2Str (&new->vn_address, buf, BUFSIZ);
              rfapiRfapiIpAddr2Str (&new->un_address, buf2, BUFSIZ);
              vnc_zlog_debug_verbose ("%s:   DEL vn=%s un=%s cost=%d life=%d",
                                      __func__, buf, buf2, new->prefix.cost,
                                      new->lifetime);
        ...
        {
          vnc_zlog_debug_verbose (
              "%s: response removal disabled, omitting removals", __func__);
        }
 ...
 */
void
rfapiRibUpdatePendingNode (struct bgp *bgp, struct rfapi_descriptor *rfd,
    ...
    uint32_t lifetime)
{
   ...
   * hands in node->link */
  if (agg_node_left (it_node) && (agg_node_left (it_node) != omit_subtree))
    {
      if (agg_node_left (it_node)->info)
        rfapiRibUpdatePendingNode (bgp, rfd, it, agg_node_left (it_node),
                                   lifetime);
      rfapiRibUpdatePendingNodeSubtree (bgp, rfd, it, agg_node_left (it_node),
                                        omit_subtree, lifetime);
    }

  if (agg_node_right (it_node) && (agg_node_right (it_node) != omit_subtree))
    {
      if (agg_node_right (it_node)->info)
        rfapiRibUpdatePendingNode (bgp, rfd, it, agg_node_right (it_node),
                                   lifetime);
      rfapiRibUpdatePendingNodeSubtree (bgp, rfd, it, agg_node_right (it_node),
                                        omit_subtree, lifetime);
    }
}
  ...
  for (nhp = response; nhp; nhp = nhp_next)
    {
...
#if DEBUG_NHL
      {
        ...
        prefix2str (&rk.aux_prefix, str_aux_prefix, sizeof (str_aux_prefix));

        if (! rk.aux_prefix.family)
          {
          }
        vnc_zlog_debug_verbose ("%s:   rk.vn=%pFX rk.aux_prefix=%s", __func__,
                                &rk.vn,
                                (rk.aux_prefix.family ? str_aux_prefix : "-"));
      }
      vnc_zlog_debug_verbose ("%s: RIB skiplist for this prefix follows",
                              __func__);
...
#if DEBUG_NHL
              vnc_zlog_debug_verbose (
                  "%s: allowed due to counter/timestamp diff", __func__);
#endif
              allowed = 1;
            }
        }
...
#if DEBUG_NHL
          vnc_zlog_debug_verbose ("%s: allowed due to not yet in RIB",
                                  __func__);
#endif
          /* not found: add new route to RIB */
          ri = rfapi_info_new ();
      ...
      vnc_zlog_debug_verbose (
          ...
          __func__, &pfx, &rk.vn, nhp->prefix.cost, nhp->lifetime, allowed);
...
void
rfapiRibPendingDeleteRoute (struct bgp *bgp, struct rfapi_import_table *it,
      ...
      if ((sl = RFAPI_MONITOR_ETH (it_node)))
        {
          ...
          vnc_zlog_debug_verbose ("%s: route-specific skiplist: %p", __func__,
                                  sl);

          for (cursor = NULL,
              rc = skiplist_next (sl, NULL, (void **) &m, &cursor);
               ! rc; rc = skiplist_next (sl, NULL, (void **) &m, &cursor))
            {

#if DEBUG_PENDING_DELETE_ROUTE
              vnc_zlog_debug_verbose ("%s: eth monitor rfd=%p", __func__,
                                      m->rfd);
#endif
              /*
      ...
      for (ALL_LIST_ELEMENTS_RO (&bgp->rfapi->descriptors, node, rfd))
        {
          ...
          vnc_zlog_debug_verbose (
              "%s: comparing rfd(%p)->import_table=%p to it=%p", __func__, rfd,
              rfd->import_table, it);
...
static int
print_rib_sl (int (*fp) (void *, const char *, ...), struct vty *vty,
              void *out, struct skiplist *sl, int deleted, char *str_pfx,
              int *printedprefix)
{
  struct rfapi_info *ri;
  int rc;
  ...
  for (rc = skiplist_next (sl, NULL, (void **) &ri, &cursor); ! rc;
       rc = skiplist_next (sl, NULL, (void **) &ri, &cursor))
    {
...
#else
      {
        ...
        rfapiFormatSeconds ((expire > now ? expire - now : 1), str_age,
                            BUFSIZ);
      }
#endif

      fp (out, " %c %-20s %-15s %-15s %-4u %-8s %-8s %pRDP\n",
          deleted ? 'r' : ' ', *printedprefix ? "" : str_pfx, str_vn, str_un,
          ri->cost, str_lifetime, str_age, &ri->rk.rd);
...
rfapiRibShowRibSl (void *stream, struct prefix *pfx, struct skiplist *sl)
{
  ...
  nhs_displayed += print_rib_sl (fp, vty, out, sl, 0, str_pfx, &printedprefix);
}
#endif

void
rfapiRibShowResponses (void *stream, struct prefix *pfx_match,
                       int show_removed)
{
  int (*fp) (void *, const char *, ...);
  struct vty *vty;
      ...
      for (afi = AFI_IP; afi < AFI_MAX; ++afi)
        {
          ...
          for (rn = agg_route_top (rfd->rib[afi]); rn;
               rn = agg_route_next (rn))
            {
              ...
              if (! printedheader)
                {
                  ...
                  fp (out, "   %-20s %-15s %-15s %4s %-8s %-8s\n", "Prefix",
                      "Registered VN", "Registered UN", "Cost", "Lifetime",
#ifdef RFAPI_REGISTRATIONS_REPORT_AGE
                      "Age"
#else
                      "Remaining"
              ...
              routes_displayed++;
              nhs_displayed += print_rib_sl (fp, vty, out, sl, show_removed,
                                             str_pfx, &printedprefix);
            }
        }
    }
