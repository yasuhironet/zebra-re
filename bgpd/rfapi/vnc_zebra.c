 ...
 */
static void
vnc_redistribute_add (struct prefix *p, uint32_t metric, uint8_t type)
{
  ...
  switch (bgp->rfapi_cfg->rfg_redist->vn_prefix.family)
    {
    case AF_INET:
      if (bgp->rfapi_cfg->rfg_redist->vn_prefix.prefixlen != IPV4_MAX_BITLEN)
        {
          vnc_zlog_debug_verbose (
      ...
      break;
    case AF_INET6:
      if (bgp->rfapi_cfg->rfg_redist->vn_prefix.prefixlen != IPV6_MAX_BITLEN)
        {
          vnc_zlog_debug_verbose (
  ...
  add_vnc_route (&vncHD1VR, /* cookie + UN addr */
                 ...
                 &(bgp->rfapi_cfg->redist_lifetime), NULL, /* RFP options */
 ...
 */
static int
vnc_zebra_read_route (ZAPI_CALLBACK_ARGS)
{
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    vnc_zlog_debug_verbose ("%s: Zebra rcvd: route delete %s %pFX metric %u",
                            __func__, zebra_route_string (api.type),
                            &api.prefix, api.metric);
 ...
 */
static void
vnc_zebra_route_msg (const struct prefix *p, unsigned int nhp_count,
                     void *nhp_ary, int add) /* 1 = add, 0 = del */
{
  ...
  for (i = 0; i < api.nexthop_num; i++)
    {
      ...
      switch (p->family)
        {
        case AF_INET:
          memcpy (&api_nh->gate.ipv4, nhp_ary4[i], sizeof (api_nh->gate.ipv4));
          api_nh->type = NEXTHOP_TYPE_IPV4;
          break;
        case AF_INET6:
          memcpy (&api_nh->gate.ipv6, nhp_ary6[i], sizeof (api_nh->gate.ipv6));
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    vnc_zlog_debug_verbose ("%s: Zebra send: route %s %pFX, nhp_count=%d",
                            __func__, (add ? "add" : "del"), &api.prefix,
                            nhp_count);
...
static void
import_table_to_nve_list_zebra (struct bgp *bgp, struct rfapi_import_table *it,
...
static void
vnc_zebra_add_del_prefix (struct bgp *bgp,
                          struct rfapi_import_table *import_table,
                          struct agg_node *rn, int add) /* !0 = add, 0 = del */
{
  struct list *nves;
  ...
  if (p->family != AF_INET && p->family != AF_INET6)
    {
      flog_err (EC_LIB_DEVELOPMENT, "%s: invalid route node addr family",
                __func__);
      return;
    }
  ...
  if (! listcount (bgp->rfapi_cfg->rfg_export_zebra_l))
    {
      vnc_zlog_debug_verbose ("%s: no zebra export nve group, skipping",
                              __func__);
      return;
    }
...
void
vnc_zebra_add_prefix (struct bgp *bgp, struct rfapi_import_table *import_table,
                      struct agg_node *rn)
{
...
void
vnc_zebra_del_prefix (struct bgp *bgp, struct rfapi_import_table *import_table,
                      struct agg_node *rn)
{
...
static void
vnc_zebra_add_del_nve (struct bgp *bgp, struct rfapi_descriptor *rfd,
                       int add) /* 0 = del, !0 = add */
{
  struct listnode *node;
  struct rfapi_rfg_name *rfgn;
  ...
  if (afi != AFI_IP && afi != AFI_IP6)
    {
      flog_err (EC_LIB_DEVELOPMENT, "%s: invalid vn addr family", __func__);
      return;
    }
      ...
      if (rfgn->rfg == rfg)
        {
          ...
          vnc_zlog_debug_verbose (
              "%s: this nve's group is in zebra export list", __func__);
          ...
          for (rn = agg_route_top (rt); rn; rn = agg_route_next (rn))
            {
              ...
              vnc_zlog_debug_verbose ("%s: sending %s", __func__,
                                      (add ? "add" : "del"));
              vnc_zebra_route_msg (agg_node_get_prefix (rn), 1, &pAddr, add);
            }
        }
    }
}
...
static void
vnc_zebra_add_del_group_afi (struct bgp *bgp, struct rfapi_nve_group_cfg *rfg,
                             afi_t afi, int add)
{
  struct agg_table *rt = NULL;
  struct agg_node *rn;
  ...
  if (! family)
    {
      flog_err (EC_LIB_DEVELOPMENT, "%s: computed bad family: %d", __func__,
                family);
      return;
    }
  ...
  if (nves)
    {
      ...
      vnc_zlog_debug_verbose ("%s: family: %d, nve count: %d", __func__,
                              family, nexthop_count);
          ...
          for (rn = agg_route_top (rt); rn; rn = agg_route_next (rn))
            {
              if (rn->info)
                {
                  vnc_zebra_route_msg (agg_node_get_prefix (rn), nexthop_count,
                                       nhp_ary, add);
                }
            }
        }
...
void
vnc_zebra_reexport_group_afi (struct bgp *bgp, struct rfapi_nve_group_cfg *rfg,
                              afi_t afi)
{
  struct listnode *node;
  struct rfapi_rfg_name *rfgn;
...
vnc_redistribute_set (struct bgp *bgp, afi_t afi, int type)
{
  ...
  zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient_vnc, afi, type, 0,
                           VRF_DEFAULT);
      ...
      bgp->rfapi_cfg->redist[AFI_IP6][type] == 0 && zclient_vnc->sock >= 0)
    {
      ...
      if (BGP_DEBUG (zebra, ZEBRA))
        vnc_zlog_debug_verbose ("Zebra send: redistribute delete %s",
                                zebra_route_string (type));
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE, zclient_vnc, afi,
                               type, 0, VRF_DEFAULT);
    }
 ...
 */
void
vnc_zebra_init (struct event_loop *master)
{
  /* Set default values. */
  zclient_vnc = zclient_new (master, &zclient_options_default, vnc_handlers,
