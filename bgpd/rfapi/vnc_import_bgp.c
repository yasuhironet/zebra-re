...
static int
is_host_prefix (const struct prefix *p)
{
  switch (p->family)
    {
    case AF_INET:
...
int
vnc_prefix_cmp (const void *pfx1, const void *pfx2)
{
  int offset;
  int shift;
  uint8_t mask;
  ...
  for (p = sl->header->forward[0]; p; p = p->forward[0])
    {
      ...
      rfapiUnicastNexthop2Prefix (afi, pb->ubpi->attr, &pfx_orig_nexthop);
 ...
 */
static int
process_unicast_route (struct bgp *bgp,                /* in */
                       afi_t afi,                      /* in */
  ...
  if (hc->plist_redist[ZEBRA_ROUTE_BGP_DIRECT][afi])
    {
      ...
      if (prefix_list_apply (hc->plist_redist[ZEBRA_ROUTE_BGP_DIRECT][afi],
                             prefix) == PREFIX_DENY)
        {
          vnc_zlog_debug_verbose (
              "%s: prefix list returns DENY, blocking route", __func__);
          return -1;
        }
      vnc_zlog_debug_verbose ("%s: prefix list returns PASS, allowing route",
                              __func__);
    }
      ...
      if (ret == RMAP_DENYMATCH)
        {
          ...
          vnc_zlog_debug_verbose ("%s: route map \"%s\" says DENY, returning",
                                  __func__, rmap->name);
          return -1;
        }
    }
    ...
    struct ecommunity *ecom)     /* generated ecoms */
{
  ...
  if (bpi->sub_type != BGP_ROUTE_NORMAL && bpi->sub_type != BGP_ROUTE_STATIC &&
    ...
    struct prefix *ubpi_nexthop) /* unicast nexthop */
{
  ...
  if (! bd)
    {
      vnc_zlog_debug_verbose ("%s: no match in RD's table for ubpi_nexthop",
                              __func__);
      return;
    }
    ...
    struct bgp_path_info *info)                   /* unicast info */
{
  ...
  if (VNC_DEBUG (VERBOSE))
    {
      ...
      vnc_zlog_debug_verbose ("%s(bgp=%p, unicast prefix=%pFX, unicast nh=%s)",
  ...
  if (info->type != ZEBRA_ROUTE_BGP)
    {
      vnc_zlog_debug_verbose (
          "%s: unicast type %d=\"%s\" is not %d=%s, skipping", __func__,
          info->type, zebra_route_string (info->type), ZEBRA_ROUTE_BGP,
          "ZEBRA_ROUTE_BGP");
      return;
    }
  ...
  if (! afi)
    {
      flog_err (EC_LIB_DEVELOPMENT, "%s: can't get afi of prefix", __func__);
      return;
    }
  ...
  if (process_unicast_route (bgp, afi, prefix, info, &ecom,
                             &pfx_unicast_nexthop))
    {
      ...
      vnc_zlog_debug_verbose ("%s: process_unicast_route error, skipping",
                              __func__);
      return;
    }
  ...
  if (! bgp->rfapi->resolve_nve_nexthop)
    {
      bgp->rfapi->resolve_nve_nexthop = skiplist_new (
          SKIPLIST_FLAG_ALLOW_DUPLICATES, vnc_prefix_cmp, prefix_bag_free);
    }
  ...
  for (bdp = bgp_table_top (bgp->rib[afi][SAFI_MPLS_VPN]); bdp;
       bdp = bgp_route_next (bdp))
    {
      ...
      vnc_import_bgp_add_route_mode_resolve_nve_one_rd (
          (struct prefix_rd *) bgp_dest_get_prefix (bdp), table, afi, bgp,
          prefix, ecom, &local_pref, med, &pfx_unicast_nexthop);
    }
                                     ...
                                     struct bgp_path_info *info)
{
  ...
  add_vnc_route (&vncHDBgpDirect, bgp, SAFI_MPLS_VPN, prefix, &prd, &vnaddr,
                 &local_pref, &(bgp->rfapi_cfg->redist_lifetime),
                 NULL,                  /* RFP options */
                                        ...
                                        struct rfapi_nve_group_cfg *rfg)
{
   ...
   */
  {
    ...
    if (rfg->plist_redist[ZEBRA_ROUTE_BGP_DIRECT][afi])
      {
        vnc_zlog_debug_verbose ("%s: RFG prefix list is set, checking",
                                __func__);
        if (prefix_list_apply (rfg->plist_redist[ZEBRA_ROUTE_BGP_DIRECT][afi],
                               prefix) == PREFIX_DENY)
          {
            vnc_zlog_debug_verbose (
                "%s: prefix list returns DENY, blocking route", __func__);
    ...
    if (! is_host_prefix (&rfg->un_prefix))
      {
        ...
        vnc_zlog_debug_verbose (
            "%s: redist RFG UN pfx not host pfx (plen=%d), skipping", __func__,
   ...
   */
  {
    ...
    if (iattr && bgp_attr_get_ecommunity (iattr))
      ecom = ecommunity_merge (ecom, bgp_attr_get_ecommunity (iattr));
  }
                                     ...
                                     struct bgp_path_info *info)
{
  ...
  switch (vn_pfx->family)
    ...
    default:
      vnc_zlog_debug_verbose (
          "%s: no redist RFG VN host pfx configured, skipping", __func__);
      return;
    }
  ...
  vnc_zlog_debug_verbose ("%s: setting peer to %p", __func__,
                          vncHDBgpDirect.peer);
  del_vnc_route (&vncHDBgpDirect, info->peer, bgp, SAFI_MPLS_VPN, prefix, &prd,
    ...
    const struct prefix *prefix) /* unicast route prefix */
{
  ...
  del_vnc_route (&vncHDResolveNve, vncHDResolveNve.peer, bgp, SAFI_MPLS_VPN,
                 prefix, /* unicast route prefix */
    ...
    struct bgp_path_info *bpi)              /* new VPN host route */
{
  ...
  if (! sl)
    {
      vnc_zlog_debug_verbose ("%s: no resolve_nve_nexthop skiplist, skipping",
                              __func__);
      return;
    }
  ...
  while (! rc)
    {
      ...
      if (VNC_DEBUG (IMPORT_BGP_ADD_ROUTE))
        vnc_zlog_debug_any (
            ...
            __func__, cursor, &pb->upfx, &pb->hpfx, pb->ubpi);
      ...
      if (pb->ubpi->attr->flag & ATTR_FLAG_BIT (BGP_ATTR_MULTI_EXIT_DISC))
      ...
      {
        vnc_zlog_debug_verbose (
            "%s: advancing past RHN Entry (q=%p): with prefix %pFX", __func__,
            cursor, prefix);
  ...
  while (! rc)
    {
      ...
      vnc_import_bgp_del_route_mode_resolve_nve_one_bi (bgp, afi, bpi, prd,
                                                        &pb->upfx);
...
static int
is_usable_interior_route (struct bgp_path_info *bpi_interior)
{
  ...
  if (! CHECK_FLAG (bpi_interior->flags, BGP_PATH_VALID))
    {
#if DEBUG_IS_USABLE_INTERIOR
      vnc_zlog_debug_verbose ("%s: NO: BGP_PATH_VALID not set", __func__);
#endif
      return 0;
    }
  return 1;
    ...
    struct rfapi_import_table *it_only) /* NULL, or limit to this IT */
{
  ...
  if (! h || ! hc)
    {
      vnc_zlog_debug_verbose (
          "%s: rfapi or rfapi_cfg not instantiated, skipping", __func__);
      return;
    }
  ...
  if (bgp != hc->redist_bgp_exterior_view)
    {
      vnc_zlog_debug_verbose (
          "%s: bgp %p != hc->redist_bgp_exterior_view %p, skipping", __func__,
  ...
  if (! hc->redist[afi][ZEBRA_ROUTE_BGP_DIRECT_EXT])
    {
      vnc_zlog_debug_verbose (
          "%s: redist of exterior routes not enabled, skipping", __func__);
      return;
    }
  ...
  for (it = h->imports; it; it = it->next)
    {
      ...
      if (it_only && (it_only != it))
        {
          vnc_zlog_debug_verbose ("%s: doesn't match it_only %p", __func__,
                                  it_only);
          continue;
        }
           ...
           (! have_usable_route) && rn;)
        {
          ...
          for (bpi_interior = rn->info; bpi_interior;
               bpi_interior = bpi_interior->next)
            {
              ...
              vnc_zlog_debug_verbose ("%s: usable: bpi_interior %p", __func__,
                                      bpi_interior);
              ...
              rfapiBgpInfoFilteredImportVPN (
                  it, FIF_ACTION_UPDATE, bpi_interior->peer, NULL, /* rfd */
                  prefix, NULL, afi, prd, &new_attr,
                  ZEBRA_ROUTE_BGP_DIRECT_EXT, BGP_ROUTE_REDISTRIBUTE, &label);
            }
          ...
          if (have_usable_route)
            {
              ...
              if (! skiplist_insert (RFAPI_MONITOR_EXTERIOR (rn)->source, info,
                                     pfx_mon))
                {
    ...
    struct bgp_path_info *info)                   /* unicast info */
{
  ...
  if (! hc->redist[afi][ZEBRA_ROUTE_BGP_DIRECT_EXT])
    {
      vnc_zlog_debug_verbose (
          "%s: redist of exterior routes no enabled, skipping", __func__);
      return;
    }
          ...
          for (bpi_interior = rn->info; bpi_interior;
               bpi_interior = bpi_interior->next)
            {
              ...
              rfapiBgpInfoFilteredImportVPN (
                  ...
                  prefix, NULL, afi, prd, bpi_interior->attr,
                  ZEBRA_ROUTE_BGP_DIRECT_EXT, BGP_ROUTE_REDISTRIBUTE, &label);
    ...
    struct bgp_path_info *bpi_interior) /* VPN IT route */
{
  ...
  if (! is_usable_interior_route (bpi_interior))
    {
      vnc_zlog_debug_verbose ("%s: not usable interior route, skipping",
                              __func__);
      return;
    }
  ...
  /*debugging */
  vnc_zlog_debug_verbose ("%s: interior prefix=%pRN, bpi type=%d", __func__,
                          rn_interior, bpi_interior->type);

  if (RFAPI_HAS_MONITOR_EXTERIOR (rn_interior))
    {

      vnc_zlog_debug_verbose ("%s: has exterior monitor; ext src: %p",
                              __func__,
      ...
      for (rc = skiplist_next (RFAPI_MONITOR_EXTERIOR (rn_interior)->source,
                               ...
                               (void **) &pfx_exterior, &cursor))
        {
          ...
          new_attr = *bpi_interior->attr;
          if (bpi_exterior &&
              (bpi_exterior->attr->flag & ATTR_FLAG_BIT (BGP_ATTR_LOCAL_PREF)))
            {
              new_attr.local_pref = bpi_exterior->attr->local_pref;
              new_attr.flag |= ATTR_FLAG_BIT (BGP_ATTR_LOCAL_PREF);
            }
   ...
   */
  for (par = agg_node_parent (rn_interior); par; par = agg_node_parent (par))
    {
  ...
  if (par)
    {
      ...
      for (rc = skiplist_next (RFAPI_MONITOR_EXTERIOR (par)->source,
                               (void **) &bpi_exterior,
                               (void **) &pfx_exterior, &cursor);
           ! rc; rc = skiplist_next (RFAPI_MONITOR_EXTERIOR (par)->source,
                                     (void **) &bpi_exterior,
                                     (void **) &pfx_exterior, &cursor))
        {
          ...
          rfapiUnicastNexthop2Prefix (afi, bpi_exterior->attr, &pfx_nexthop);
          ...
          if (prefix_match (p, &pfx_nexthop))
            {
              ...
              for (bpi = par->info; bpi; bpi = bpi->next)
                {
                  ...
                  rfapiBgpInfoFilteredImportVPN (
                      it, FIF_ACTION_KILL, bpi->peer, NULL, /* rfd */
                      pfx_exterior, NULL, afi, prd, bpi->attr,
              ...
              memset (&new_attr, 0, sizeof (struct attr));
              new_attr = *bpi_interior->attr;
              if (bpi_exterior && (bpi_exterior->attr->flag &
      ...
      for (rc = skiplist_next (RFAPI_MONITOR_EXTERIOR (rn_interior)->source,
                               ...
                               (void **) &bpi_exterior, NULL, &cursor))
        {
          ...
          skiplist_delete (RFAPI_MONITOR_EXTERIOR (par)->source, bpi_exterior,
                           NULL);
  ...
  for (rc = skiplist_next (it->monitor_exterior_orphans,
                                 ...
                                 (void **) &pfx_exterior, &cursor))
    {
      ...
      vnc_zlog_debug_verbose ("%s: checking exterior orphan at prefix %pFX",
                              __func__, pfx_exterior);

      if (afi_exterior != afi)
        {
          vnc_zlog_debug_verbose (
              "%s: exterior orphan afi %d != interior afi %d, skip", __func__,
              afi_exterior, afi);
          continue;
        }
  ...
  if (list_adopted)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (list_adopted, node, an_bpi_exterior))
        {
          skiplist_delete (it->monitor_exterior_orphans, an_bpi_exterior,
                           NULL);
        }
    ...
    struct bgp_path_info *bpi_interior) /* VPN IT route */
{
  ...
  for (rc = skiplist_next (RFAPI_MONITOR_EXTERIOR (rn_interior)->source,
                           (void **) &bpi_exterior, (void **) &pfx_exterior,
                           &cursor);
       ! rc; rc = skiplist_next (RFAPI_MONITOR_EXTERIOR (rn_interior)->source,
                                 (void **) &bpi_exterior,
                                 (void **) &pfx_exterior, &cursor))
    {
  ...
  if (RFAPI_MONITOR_EXTERIOR (rn_interior)->valid_interior_count)
    {
      vnc_zlog_debug_verbose ("%s: interior routes still present, skipping",
                              __func__);
      return;
    }
      ...
      if (par)
        {
          ...
          skiplist_insert (RFAPI_MONITOR_EXTERIOR (par)->source, bpi_exterior,
                           pfx_mon);
          ...
          for (bpi = par->info; bpi; bpi = bpi->next)
            {
              ...
              rfapiBgpInfoFilteredImportVPN (
                  it, FIF_ACTION_UPDATE, bpi->peer, NULL, /* rfd */
                  pfx_exterior, NULL, afi, prd, &new_attr,
                  ZEBRA_ROUTE_BGP_DIRECT_EXT, BGP_ROUTE_REDISTRIBUTE, &label);
            }
        }
      else
        {
           ...
           */
          skiplist_insert (it->monitor_exterior_orphans, bpi_exterior,
                           pfx_mon);
        }
...
void
vnc_import_bgp_add_route (struct bgp *bgp, const struct prefix *prefix,
                          struct bgp_path_info *info)
{
  afi_t afi = family2afi (prefix->family);
  ...
  if (VNC_DEBUG (VERBOSE))
    {
      ...
      vnc_zlog_debug_verbose ("%s: pfx %pFX, nh %pFX", __func__, prefix,
                              &pfx_nexthop);
    }
#if DEBUG_RHN_LIST
 ...
 */
void
vnc_import_bgp_del_route (struct bgp *bgp, const struct prefix *prefix,
                          struct bgp_path_info *info) /* unicast info */
{
  afi_t afi = family2afi (prefix->family);
...
vnc_import_bgp_redist_enable (struct bgp *bgp, afi_t afi)
{
  ...
  if (bgp->rfapi_cfg->redist[afi][ZEBRA_ROUTE_BGP_DIRECT])
    {
      vnc_zlog_debug_verbose ("%s: already enabled for afi %d, skipping",
                              __func__, afi);
      return;
    }
      ...
      for (bpi = bgp_dest_get_bgp_path_info (dest); bpi; bpi = bpi->next)
        {
          ...
          vnc_import_bgp_add_route (bgp, bgp_dest_get_prefix (dest), bpi);
        }
    }
  vnc_zlog_debug_verbose (
...
vnc_import_bgp_exterior_redist_enable (struct bgp *bgp, afi_t afi)
{
  ...
  if (! bgp_exterior)
    {
      vnc_zlog_debug_verbose (
          "%s: no exterior view set yet, no routes to import yet", __func__);
      return;
    }
      ...
      for (bpi = bgp_dest_get_bgp_path_info (dest); bpi; bpi = bpi->next)
        {
          ...
          vnc_import_bgp_exterior_add_route (bgp_exterior,
                                             bgp_dest_get_prefix (dest), bpi);
        }
    }
  vnc_zlog_debug_verbose (
      "%s: set redist[afi=%d][type=%d=ZEBRA_ROUTE_BGP_DIRECT] return",
                                          ...
                                          struct rfapi_import_table *it_only)
{
  ...
  if (! bgp->rfapi_cfg->redist[afi][ZEBRA_ROUTE_BGP_DIRECT_EXT])
    {
      vnc_zlog_debug_verbose ("%s: not enabled for afi %d, skipping", __func__,
                              afi);
      return;
    }
      ...
      for (bpi = bgp_dest_get_bgp_path_info (dest); bpi; bpi = bpi->next)
        {
          ...
          vnc_import_bgp_exterior_add_route_it (
              bgp_exterior, bgp_dest_get_prefix (dest), bpi, it_only);
        }
    }
}
...
vnc_import_bgp_redist_disable (struct bgp *bgp, afi_t afi)
{
  ...
  if (! bgp->rfapi_cfg->redist[afi][ZEBRA_ROUTE_BGP_DIRECT])
    {
      vnc_zlog_debug_verbose ("%s: already disabled for afi %d, skipping",
                              __func__, afi);
      return;
    }
  ...
  for (dest1 = bgp_table_top (bgp->rib[afi][SAFI_MPLS_VPN]); dest1;
       dest1 = bgp_route_next (dest1))
    {
      ...
      for (dest2 = bgp_table_top (bgp_dest_get_bgp_table_info (dest1)); dest2;
           dest2 = bgp_route_next (dest2))
        {
          const struct prefix *dest2_p = bgp_dest_get_prefix (dest2);
          struct bgp_path_info *bpi;
          struct bgp_path_info *nextbpi;

          for (bpi = bgp_dest_get_bgp_path_info (dest2); bpi; bpi = nextbpi)
            {
              ...
              del_vnc_route (rfd, bpi->peer, bgp, SAFI_MPLS_VPN, dest2_p,
                             (struct prefix_rd *) dest1_p, bpi->type,
                             bpi->sub_type, NULL, 1); /* kill */
...
vnc_import_bgp_exterior_redist_disable (struct bgp *bgp, afi_t afi)
{
  ...
  if (! bgp_exterior)
    {
      vnc_zlog_debug_verbose ("%s: bgp exterior view not defined, skipping",
                              __func__);
      return;
    }
