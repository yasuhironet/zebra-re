 ...
 */
static void
peer_process (struct hash_bucket *hb, void *arg)
{
  ...
  uint32_t v_ka =
      atomic_load_explicit (&pkat->peer->v_keepalive, memory_order_relaxed);
...
static void
bgp_keepalives_finish (void *arg)
{
  ...
  while (atomic_load_explicit (&fpt->running, memory_order_relaxed))
    {
      if (peerhash->count > 0)
        ...
        while (peerhash->count == 0 &&
               atomic_load_explicit (&fpt->running, memory_order_relaxed))
...
void
bgp_keepalives_on (struct peer *peer)
{
...
void
bgp_keepalives_off (struct peer *peer)
{
  if (! CHECK_FLAG (peer->thread_flags, PEER_THREAD_KEEPALIVES_ON))
    return;
...
int
bgp_keepalives_stop (struct frr_pthread *fpt, void **result)
{
  assert (fpt->running);
