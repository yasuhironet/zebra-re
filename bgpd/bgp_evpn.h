...
static inline int
advertise_type5_routes (struct bgp *bgp_vrf, afi_t afi)
{
  if (! bgp_vrf->l3vni)
    return 0;
...
get_route_parent_evpn (struct bgp_path_info *ri)
{
  ...
  if (ri->sub_type != BGP_ROUTE_IMPORTED || ! ri->extra || ! ri->extra->parent)
    return NULL;
...
/* Flag if the route's parent is a EVPN route. */
static inline int
is_route_parent_evpn (struct bgp_path_info *ri)
{
  struct bgp_path_info *parent_ri;
...
static inline bool
is_pi_family_evpn (struct bgp_path_info *pi)
{
  return is_pi_family_matching (pi, AFI_L2VPN, SAFI_EVPN);
}

static inline bool
evpn_resolve_overlay_index (void)
{
  struct bgp *bgp = NULL;
...
extern char *bgp_evpn_label2str (mpls_label_t *label, uint32_t num_labels,
                                 char *buf, int len);
extern void bgp_evpn_route2json (const struct prefix_evpn *p,
                                 json_object *json);
...
extern void bgp_evpn_show_remote_ip_hash (struct hash_bucket *bucket,
                                          void *args);
extern void bgp_evpn_show_vni_svi_hash (struct hash_bucket *bucket,
                                        void *args);
