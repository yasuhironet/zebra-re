...
static void bgp_evpn_remote_ip_hash_del (struct bgpevpn *vpn,
                                         struct bgp_path_info *pi);
static void
bgp_evpn_remote_ip_hash_iterate (struct bgpevpn *vpn,
                                 void (*func) (struct hash_bucket *, void *),
                                 void *arg);
static void bgp_evpn_link_to_vni_svi_hash (struct bgp *bgp,
                                           struct bgpevpn *vpn);
 ...
 */
static unsigned int
vni_hash_key_make (const void *p)
{
 ...
 */
static unsigned int
vrf_import_rt_hash_key_make (const void *p)
{
 ...
 */
static bool
vrf_import_rt_hash_cmp (const void *p1, const void *p2)
{
 ...
 */
static struct vrf_irt_node *
vrf_import_rt_new (struct ecommunity_val *rt)
{
  ...
  irt = XCALLOC (MTYPE_BGP_EVPN_VRF_IMPORT_RT, sizeof (struct vrf_irt_node));
 ...
 */
static void
vrf_import_rt_free (struct vrf_irt_node *irt)
{
  struct bgp *bgp_evpn = NULL;
...
static void
hash_vrf_import_rt_free (struct vrf_irt_node *irt)
{
  XFREE (MTYPE_BGP_EVPN_VRF_IMPORT_RT, irt);
}
 ...
 */
static struct vrf_irt_node *
lookup_vrf_import_rt (struct ecommunity_val *rt)
{
  struct bgp *bgp_evpn = NULL;
  struct vrf_irt_node *irt;
 ...
 */
static int
is_vrf_present_in_irt_vrfs (struct list *vrfs, struct bgp *bgp_vrf)
{
 ...
 */
static unsigned int
import_rt_hash_key_make (const void *p)
{
...
static void
hash_import_rt_free (struct irt_node *irt)
{
  XFREE (MTYPE_BGP_EVPN_IMPORT_RT, irt);
}
 ...
 */
bool
is_route_injectable_into_evpn (struct bgp_path_info *pi)
{
  ...
  if (pi->sub_type != BGP_ROUTE_IMPORTED || ! pi->extra || ! pi->extra ||
      ! pi->extra->parent)
    return true;
 ...
 */
int
bgp_evpn_route_target_cmp (struct ecommunity *ecom1, struct ecommunity *ecom2)
{
  if (ecom1 && ! ecom2)
    return -1;
 ...
 */
static int
evpn_vrf_route_target_cmp (struct vrf_route_target *rt1,
...
void
bgp_evpn_xxport_delete_ecomm (void *val)
{
 ...
 */
static struct vrf_route_target *
evpn_vrf_rt_new (struct ecommunity *ecom)
{
 ...
 */
static void
vrf_rt2ecom_val (struct ecommunity_val *to_eval,
 ...
 */
static void
map_vrf_to_rt (struct bgp *bgp_vrf, struct vrf_route_target *l3rt)
{
  uint32_t i = 0;
 ...
 */
static void
unmap_vrf_from_rt (struct bgp *bgp_vrf, struct vrf_route_target *l3rt)
{
  uint32_t i;
 ...
 */
static void
unmap_vni_from_rt (struct bgp *bgp, struct bgpevpn *vpn, struct irt_node *irt)
{
...
static void
bgp_evpn_get_rmac_nexthop (struct bgpevpn *vpn, const struct prefix_evpn *p,
 ...
 */
static void
form_auto_rt (struct bgp *bgp, vni_t vni, struct list *rtl, bool is_l3)
{
  struct ecommunity_val eval;
 ...
 */
static void
evpn_convert_nexthop_to_ipv6 (struct attr *attr)
{
                          ...
                          const struct bgp_path_info *local_pi)
{
  ...
  else if (evp->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE && local_pi)
    {
      /*
      ...
      if (is_evpn_prefix_ipaddr_none (evp))
      ...
      else
        {
          /* VNI IP -> Global */
          evpn_type2_prefix_global_copy (
              &global_p, evp, evpn_type2_path_info_get_mac (local_pi),
              NULL /* ip */);
        }
 ...
 */
struct bgp_dest *
bgp_evpn_global_node_lookup (struct bgp_table *table, safi_t safi,
  ...
  if (evp->prefix.route_type == BGP_EVPN_AD_ROUTE && parent_pi)
    {
       ...
       */
      evpn_type1_prefix_vni_ip_copy (&vni_p, evp, parent_pi->attr->nexthop);
      evp = &vni_p;
    }
 ...
 */
struct bgp_dest *
bgp_evpn_vni_node_lookup (const struct bgpevpn *vpn,
                          const struct prefix_evpn *p,
                          const struct bgp_path_info *parent_pi)
{
  if ((p->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE) &&
      (is_evpn_prefix_ipaddr_none (p) == true))
    return bgp_evpn_vni_mac_node_lookup (vpn->mac_table, p, parent_pi);
                             ...
                             uint8_t flags, uint32_t seq, esi_t *esi)
{
  ...
  zclient_create_header (
      s, add ? ZEBRA_REMOTE_MACIP_ADD : ZEBRA_REMOTE_MACIP_DEL, bgp->vrf_id);
  ...
  if (is_evpn_prefix_ipaddr_none (p))
    ...
    {
      ipa_len =
          is_evpn_prefix_ipaddr_v4 (p) ? IPV4_MAX_BYTELEN : IPV6_MAX_BYTELEN;
  ...
  frrtrace (5, frr_bgp, evpn_mac_ip_zsend, add, vpn, p, remote_vtep_ip, esi);
                            ...
                            int add)
{
  ...
  zclient_create_header (
      s, add ? ZEBRA_REMOTE_VTEP_ADD : ZEBRA_REMOTE_VTEP_DEL, bgp->vrf_id);
  stream_putl (s, vpn->vni);
 ...
 */
static void
build_evpn_type5_route_extcomm (struct bgp *bgp_vrf, struct attr *attr)
{
  ...
  for (ALL_LIST_ELEMENTS (vrf_export_rtl, node, nnode, l3rt))
    bgp_attr_set_ecommunity (
        attr, ecommunity_merge (bgp_attr_get_ecommunity (attr), l3rt->ecom));

  /* add the router mac extended community */
  if (! is_zero_mac (&attr->rmac))
    {
      encode_rmac_extcomm (&eval_rmac, &attr->rmac);
      ecommunity_add_val (bgp_attr_get_ecommunity (attr), &eval_rmac, true,
                          true);
    }
}
                          ...
                          int add_l3_ecomm)
{
  ...
  for (ALL_LIST_ELEMENTS (vpn->export_rtl, node, nnode, ecom))
    bgp_attr_set_ecommunity (
        attr, ecommunity_merge (bgp_attr_get_ecommunity (attr), ecom));
      ...
      if (vrf_export_rtl && ! list_isempty (vrf_export_rtl))
        {
          for (ALL_LIST_ELEMENTS (vrf_export_rtl, node, nnode, l3rt))
            bgp_attr_set_ecommunity (
                attr,
                ecommunity_merge (bgp_attr_get_ecommunity (attr), l3rt->ecom));
        }
    }
  ...
  if (attr->sticky)
    {
      ...
      bgp_attr_set_ecommunity (
          attr,
          ecommunity_merge (bgp_attr_get_ecommunity (attr), &ecom_sticky));
    }
  ...
  if (attr->default_gw)
    {
      ...
      bgp_attr_set_ecommunity (
          attr,
          ecommunity_merge (bgp_attr_get_ecommunity (attr), &ecom_default_gw));
    }
  ...
  if (attr->router_flag || proxy)
    {
      ...
      bgp_attr_set_ecommunity (
          attr, ecommunity_merge (bgp_attr_get_ecommunity (attr), &ecom_na));
    }
}
 ...
 */
static void
add_mac_mobility_to_attr (uint32_t seq_num, struct attr *attr)
{
      ...
      for (i = 0; i < ecomm->size; i++)
        {
          ...
          if (type == ECOMMUNITY_ENCODE_EVPN &&
            ...
            {
              ecom_val_ptr = (ecomm->val + (i * ecomm->unit_size));
              break;
            }
        }
    }
  ...
  /* Add MM to existing */
  else
    {
      ...
      if (ecomm)
        bgp_attr_set_ecommunity (attr, ecommunity_merge (ecomm, &ecom_tmp));
      else
        bgp_attr_set_ecommunity (attr, ecommunity_dup (&ecom_tmp));
    }
}
  ...
  if (p->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE)
    {
      ...
      if (pi->sub_type == BGP_ROUTE_IMPORTED)
        {
          ...
          if (is_evpn_prefix_ipaddr_v6 (p) && pi->attr->router_flag)
      ...
      ret = bgp_zebra_send_remote_macip (
          ...
          (is_evpn_prefix_ipaddr_none (p)
               ? NULL /* MAC update */
               : evpn_type2_path_info_get_mac (pi) /* MAC-IP update */),
          pi->attr->nexthop, 1, flags, seq, bgp_evpn_attr_get_esi (pi->attr));
    }
  ...
  if (p->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE)
    ret = bgp_zebra_send_remote_macip (
        ...
        (is_evpn_prefix_ipaddr_none (p)
             ? NULL /* MAC update */
             : evpn_type2_path_info_get_mac (pi) /* MAC-IP update */),
        (is_sync ? zero_vtep_ip : pi->attr->nexthop), 0, 0, 0, NULL);
  else if (p->prefix.route_type == BGP_EVPN_AD_ROUTE)
    ret = bgp_evpn_remote_es_evi_del (bgp, vpn, p);
  else
    ret = bgp_zebra_send_remote_vtep (bgp, vpn, p, VXLAN_FLOOD_DISABLED, 0);
                             ...
                             struct bgp_path_info *new_select)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
    {
      ...
      zlog_debug (
          "local path deleted %pFX es %s; new-path-es %s", evp,
          esi_to_str (&old_local->attr->esi, esi_buf, sizeof (esi_buf)),
          new_select ? esi_to_str (&new_select->attr->esi, esi_buf2,
                                   sizeof (esi_buf2))
                     : "");
    }
  ...
  global_dest = bgp_evpn_global_node_lookup (
      ...
      (const struct prefix_evpn *) bgp_dest_get_prefix (dest), &vpn->prd,
      old_local);
  if (global_dest)
    {
...
/*
 * Calculate the best path for an EVPN route. Install/update best path in
 * zebra, if appropriate. Note: vpn is NULL for local EAD-ES routes.
 */
int
                           ...
                           struct bgp_dest *dest)
{
      ...
      ! bgp_addpath_is_addpath_used (&bgp->tx_addpath, afi, safi))
    {
      if (bgp_zebra_has_route_changed (old_select))
        ret = evpn_zebra_install (
            bgp, vpn, (const struct prefix_evpn *) bgp_dest_get_prefix (dest),
            old_select);
  ...
  if (new_select && new_select->type == ZEBRA_ROUTE_BGP &&
    ...
    {
      ret = evpn_zebra_install (
          bgp, vpn, (struct prefix_evpn *) bgp_dest_get_prefix (dest),
          new_select);
       ...
       * peers.
       */
      if (new_select->sub_type == BGP_ROUTE_IMPORTED && old_select &&
          old_select->peer == bgp->peer_self &&
          old_select->type == ZEBRA_ROUTE_BGP &&
          old_select->sub_type == BGP_ROUTE_STATIC && vpn)
        evpn_delete_old_local_route (bgp, vpn, dest, old_select, new_select);
    }
  else
    {
...
bgp_evpn_route_get_local_path (struct bgp *bgp, struct bgp_dest *dest)
{
...
static int
update_evpn_type5_route_entry (struct bgp *bgp_evpn, struct bgp *bgp_vrf,
      ...
      if (! attrhash_cmp (tmp_pi->attr, attr))
        {
          ...
          *route_changed = 1;
          ...
          /* Add (or update) attribute to hash. */
          attr_new = bgp_attr_intern (attr);
          bgp_path_info_set_flag (dest, tmp_pi, BGP_PATH_ATTR_CHANGED);
...
static int
update_evpn_type5_route (struct bgp *bgp_vrf, struct prefix_evpn *evp,
                         struct attr *src_attr, afi_t src_afi, safi_t src_safi)
{
  ...
  if (bgp_debug_zebra (NULL))
    zlog_debug ("VRF %s type-5 route evp %pFX RMAC %pEA nexthop %pI4",
                vrf_id_to_name (bgp_vrf->vrf_id), evp, &attr.rmac,
                &attr.nexthop);

  frrtrace (4, frr_bgp, evpn_advertise_type5, bgp_vrf->vrf_id, evp, &attr.rmac,
            attr.nexthop);
  ...
  if (src_afi == AFI_IP6 &&
    ...
    {
      if (src_attr && ! IN6_IS_ADDR_UNSPECIFIED (&src_attr->mp_nexthop_global))
        {
          ...
          memcpy (&attr.evpn_overlay.gw_ip.ipaddr_v6,
                  &src_attr->mp_nexthop_global, sizeof (struct in6_addr));
        }
    }
      ...
      if (src_attr && src_attr->nexthop.s_addr != 0)
        {
          ...
          memcpy (&attr.evpn_overlay.gw_ip.ipaddr_v4, &src_attr->nexthop,
  ...
  dest = bgp_evpn_global_node_get (bgp_evpn->rib[afi][safi], afi, safi, evp,
...
static void
bgp_evpn_get_sync_info (struct bgp *bgp, esi_t *esi, struct bgp_dest *dest,
                        uint32_t loc_seq, uint32_t *max_sync_seq,
                        bool *active_on_peer, bool *peer_router,
                        bool *proxy_from_peer, const struct ethaddr *mac)
{
  struct bgp_path_info *tmp_pi;
  ...
  for (tmp_pi = bgp_dest_get_bgp_path_info (dest); tmp_pi;
       tmp_pi = tmp_pi->next)
    {
      ...
      if (bgp_evpn_path_info_cmp (bgp, tmp_pi, second_best_path, &paths_eq))
  ...
  if (! memcmp (esi, tmp_esi, sizeof (esi_t)))
    {
      ...
      if (second_best_path->attr->es_flags & ATTR_ES_PROXY_ADVERT)
        {
 ...
 */
static void
update_evpn_route_entry_sync_info (struct bgp *bgp, struct bgp_dest *dest,
                                   struct attr *attr, uint32_t loc_seq,
                                   bool setup_sync, const struct ethaddr *mac)
{
  esi_t *esi;
      ...
      if (setup_sync)
        {
          ...
          bgp_evpn_get_sync_info (bgp, esi, dest, loc_seq, &max_sync_seq,
                                  &active_on_peer, &peer_router,
                                  &proxy_from_peer, mac);
          ...
          if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
            {
              ...
              zlog_debug (
                  "setup sync info for %pFX es %s max_seq %d %s%s%s", evp,
                  esi_to_str (esi, esi_buf, sizeof (esi_buf)), max_sync_seq,
                  (attr->es_flags & ATTR_ES_PEER_ACTIVE) ? "peer-active " : "",
                  (attr->es_flags & ATTR_ES_PEER_PROXY) ? "peer-proxy " : "",
                  (attr->es_flags & ATTR_ES_PEER_ROUTER) ? "peer-router "
                                                         : "");
            }
        }
    }
...
update_evpn_route_entry (struct bgp *bgp, struct bgpevpn *vpn, afi_t afi,
                         ...
                         int add, struct bgp_path_info **pi, uint8_t flags,
  ...
  if (! local_pi)
    {
      ...
      tmp_pi = info_make (ZEBRA_ROUTE_BGP, BGP_ROUTE_STATIC, 0, bgp->peer_self,
                          attr_new, dest);
...
static void
evpn_zebra_reinstall_best_route (struct bgp *bgp, struct bgpevpn *vpn,
                                 struct bgp_dest *dest)
{
                                   ...
                                   struct bgp_path_info *local_pi)
{
  ...
  if (bgp_debug_zebra (NULL))
    zlog_debug ("evicting local evpn prefix %pBD as remote won", dest);
...
static inline bool
bgp_evpn_route_add_l3_ecomm_ok (struct bgpevpn *vpn,
                                const struct prefix_evpn *p, esi_t *esi)
{
 ...
 */
static int
update_evpn_route (struct bgp *bgp, struct bgpevpn *vpn, struct prefix_evpn *p,
                   uint8_t flags, uint32_t seq, esi_t *esi)
{
  struct bgp_dest *dest;
  struct attr attr;
  ...
  attr.router_flag = CHECK_FLAG (flags, ZEBRA_MACIP_TYPE_ROUTER_FLAG) ? 1 : 0;
  ...
  if (bgp_debug_zebra (NULL))
    {
      ...
      zlog_debug (
          ...
          vpn->bgp_vrf ? vrf_id_to_name (vpn->bgp_vrf->vrf_id) : "None",
          vpn->vni, p->prefix.route_type, p, &attr.rmac,
          &attr.mp_nexthop_global_in, esi_to_str (esi, buf3, sizeof (buf3)));
    }
  ...
  route_change = update_evpn_route_entry (
      ...
      (mac_only ? NULL : &p->prefix.macip_addr.mac), NULL /* ip */, 1, &pi,
      flags, seq, true /* setup_sync */, &old_is_sync);
  assert (pi);
  attr_new = pi->attr;
  ...
  if (route_change)
    {
      ...
      dest = bgp_evpn_global_node_get (bgp->rib[afi][safi], afi, safi, p,
                                       &vpn->prd, NULL);
      update_evpn_route_entry (bgp, vpn, afi, safi, dest, attr_new,
                               NULL /* mac */, NULL /* ip */, 1, &global_pi,
                               flags, seq, false /* setup_sync */,
                               NULL /* old_is_sync */);
 ...
 */
void
delete_evpn_route_entry (struct bgp *bgp, afi_t afi, safi_t safi,
                         struct bgp_dest *dest, struct bgp_path_info **pi)
{
  ...
  *pi = NULL;
  ...
  for (tmp_pi = bgp_dest_get_bgp_path_info (dest); tmp_pi;
       tmp_pi = tmp_pi->next)
    if (tmp_pi->peer == bgp->peer_self && tmp_pi->type == ZEBRA_ROUTE_BGP &&
...
static int
delete_evpn_type5_route (struct bgp *bgp_vrf, struct prefix_evpn *evp)
{
  afi_t afi = AFI_L2VPN;
  safi_t safi = SAFI_EVPN;
  struct bgp_dest *dest = NULL;
 ...
 */
static int
delete_evpn_route (struct bgp *bgp, struct bgpevpn *vpn, struct prefix_evpn *p)
{
...
void
bgp_evpn_update_type2_route_entry (struct bgp *bgp, struct bgpevpn *vpn,
                                   struct bgp_dest *dest,
                                   struct bgp_path_info *local_pi,
                                   const char *caller)
{
   ...
   */
  add_l3_ecomm = bgp_evpn_route_add_l3_ecomm_ok (
      vpn, &evp, (attr.es_flags & ATTR_ES_IS_LOCAL) ? &attr.esi : NULL);
  ...
  if (bgp_debug_zebra (NULL))
    {
      ...
      zlog_debug (
          ...
          vpn->bgp_vrf ? vrf_id_to_name (vpn->bgp_vrf->vrf_id) : " ", vpn->vni,
          &evp, &attr.rmac, &attr.mp_nexthop_global_in,
          esi_to_str (&attr.esi, buf3, sizeof (buf3)), attr.es_flags, caller);
    }

  /* Update the route entry. */
  route_change = update_evpn_route_entry (
      bgp, vpn, afi, safi, dest, &attr, NULL /* mac */, NULL /* ip */, 0, &pi,
      0, seq, true /* setup_sync */, &old_is_sync);
  ...
  if (route_change)
    {
      ...
      global_dest = bgp_evpn_global_node_get (bgp->rib[afi][safi], afi, safi,
                                              &evp, &vpn->prd, NULL);
      assert (global_dest);
      update_evpn_route_entry (bgp, vpn, afi, safi, global_dest, attr_new,
...
static void
update_type2_route (struct bgp *bgp, struct bgpevpn *vpn,
                    struct bgp_dest *dest)
{
  struct bgp_path_info *tmp_pi;
...
delete_global_type2_routes (struct bgp *bgp, struct bgpevpn *vpn)
{
  ...
  rddest = bgp_node_lookup (bgp->rib[afi][safi], (struct prefix *) &vpn->prd);
  if (rddest)
    {
...
static void
delete_vni_type2_route (struct bgp *bgp, struct bgp_dest *dest)
{
  struct bgp_path_info *pi;
  afi_t afi = AFI_L2VPN;
  safi_t safi = SAFI_EVPN;
...
static void
delete_vni_type2_routes (struct bgp *bgp, struct bgpevpn *vpn)
{
  struct bgp_dest *dest;
...
static int
bgp_evpn_vni_flood_mode_get (struct bgp *bgp, struct bgpevpn *vpn)
{
 ...
 */
static int
bgp_evpn_mcast_grp_change (struct bgp *bgp, struct bgpevpn *vpn,
 ...
 */
static int
install_evpn_route_entry_in_vrf (struct bgp *bgp_vrf,
                                 const struct prefix_evpn *evp,
                                 struct bgp_path_info *parent_pi)
{
  struct bgp_dest *dest;
  ...
  if (bgp_debug_zebra (NULL))
    zlog_debug ("vrf %s: import evpn prefix %pFX parent %p flags 0x%x",
  ...
  if (attr.evpn_overlay.type != OVERLAY_INDEX_GATEWAY_IP)
  ...
  else
    {
      ...
      if (bgp_debug_zebra (NULL))
        {
          zlog_debug (
              ...
              inet_ntop (pp->family, &attr.evpn_overlay.gw_ip, buf1,
                         sizeof (buf1)),
      ...
      if (afi == AFI_IP6)
        {
          memcpy (&attr.mp_nexthop_global, &attr.evpn_overlay.gw_ip.ipaddr_v6,
  ...
  for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
    if (pi->extra && (struct bgp_path_info *) pi->extra->parent == parent_pi)
      break;
  ...
  if (! pi)
  ...
  else
    {
      ...
      if ((afi == AFI_IP &&
          ...
          (afi == AFI_IP6 && ! IPV6_ADDR_SAME (&pi->attr->mp_nexthop_global,
  ...
  if (attr.evpn_overlay.type == OVERLAY_INDEX_GATEWAY_IP)
    {
      if (bgp_find_or_add_nexthop (bgp_vrf, bgp_vrf, afi, safi, pi, NULL, 0,
                                   NULL))
          ...
          if (BGP_DEBUG (nht, NHT))
            {
              inet_ntop (pp->family, &attr.evpn_overlay.gw_ip, buf1,
                         sizeof (buf1));
              zlog_debug ("%s: gateway IP NH unresolved", buf1);
            }
  ...
  bgp_aggregate_increment (bgp_vrf, bgp_dest_get_prefix (dest), pi, afi, safi);
  ...
  if (bgp_debug_zebra (NULL))
    zlog_debug ("... %s pi dest %p (l %d) pi %p (l %d, f 0x%x)",
                ...
                bgp_dest_get_lock_count (dest), pi, pi->lock, pi->flags);
 ...
 */
static int
install_evpn_route_entry_in_vni_common (struct bgp *bgp, struct bgpevpn *vpn,
                                        const struct prefix_evpn *p,
                                        struct bgp_dest *dest,
                                        struct bgp_path_info *parent_pi)
{
  ...
  if (! pi)
    {
      /* Create an info */
      pi = bgp_create_evpn_bgp_path_info (parent_pi, dest, parent_pi->attr);

      if (p->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE)
        {
          if (is_evpn_type2_dest_ipaddr_none (dest))
            evpn_type2_path_info_set_ip (pi, p->prefix.macip_addr.ip);
          else
            evpn_type2_path_info_set_mac (pi, p->prefix.macip_addr.mac);
        }
  ...
  else
    {
      ...
      if ((old_local_es != new_local_es) ||
          memcmp (&pi->attr->esi, &attr_new->esi, sizeof (attr_new->esi)))
        {

          if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
            zlog_debug ("VNI %d path %pFX chg to %s es", vpn->vni, &pi->net->p,
  ...
  if (local_pi && (old_local_es || new_local_es))
    bgp_evpn_update_type2_route_entry (bgp, vpn, dest, local_pi, __func__);
 ...
 */
static int
uninstall_evpn_route_entry_in_vni_common (struct bgp *bgp, struct bgpevpn *vpn,
                                          const struct prefix_evpn *p,
                                          struct bgp_dest *dest,
                                          struct bgp_path_info *parent_pi)
{
 ...
 */
static int
install_evpn_route_entry_in_vni_ip (struct bgp *bgp, struct bgpevpn *vpn,
                                    const struct prefix_evpn *p,
                                    struct bgp_path_info *parent_pi)
{
  int ret;
  ...
  ret = install_evpn_route_entry_in_vni_common (bgp, vpn, p, dest, parent_pi);
 ...
 */
static int
install_evpn_route_entry_in_vni_mac (struct bgp *bgp, struct bgpevpn *vpn,
                                     const struct prefix_evpn *p,
                                     struct bgp_path_info *parent_pi)
{
  int ret;
 ...
 */
static int
uninstall_evpn_route_entry_in_vni_ip (struct bgp *bgp, struct bgpevpn *vpn,
                                      const struct prefix_evpn *p,
                                      struct bgp_path_info *parent_pi)
{
  int ret;
  ...
  ret =
      uninstall_evpn_route_entry_in_vni_common (bgp, vpn, p, dest, parent_pi);
 ...
 */
static int
uninstall_evpn_route_entry_in_vrf (struct bgp *bgp_vrf,
                                   const struct prefix_evpn *evp,
                                   struct bgp_path_info *parent_pi)
{
  struct bgp_dest *dest;
  ...
  if (bgp_debug_zebra (NULL))
    zlog_debug ("vrf %s: unimport evpn prefix %pFX parent %p flags 0x%x",
                vrf_id_to_name (bgp_vrf->vrf_id), evp, parent_pi,
                parent_pi->flags);
  ...
  if (bgp_debug_zebra (NULL))
    zlog_debug ("... delete dest %p (l %d) pi %p (l %d, f 0x%x)", dest,
                bgp_dest_get_lock_count (dest), pi, pi->lock, pi->flags);

  /* Process for route leaking. */
  vpn_leak_from_vrf_withdraw (bgp_get_default (), bgp_vrf, pi);

  bgp_aggregate_decrement (bgp_vrf, bgp_dest_get_prefix (dest), pi, afi, safi);
                          ...
                          struct bgp_path_info *parent_pi)
{
  ...
  if (bgp_debug_update (parent_pi->peer, NULL, NULL, 1))
    zlog_debug ("%s (%u): Installing EVPN %pFX route in VNI %u IP/MAC table",
 ...
 */
static int
is_route_matching_for_vrf (struct bgp *bgp_vrf, struct bgp_path_info *pi)
{
 ...
 */
static int
bgp_evpn_route_rmac_self_check (struct bgp *bgp_vrf,
                                const struct prefix_evpn *evp,
                                struct bgp_path_info *pi)
{
          ...
          if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
            {
              ...
              zlog_debug ("vrf %s of evpn prefix %pFX skipped, local es %s",
 ...
 */
int
bgp_evpn_route_entry_install_if_vrf_match (struct bgp *bgp_vrf,
                                           struct bgp_path_info *pi,
                                           int install)
{
  int ret = 0;
  ...
  if (is_route_matching_for_vrf (bgp_vrf, pi))
    {
      ...
      /* don't import hosts that are locally attached */
      if (install &&
          bgp_evpn_skip_vrf_import_of_local_es (bgp_vrf, evp, pi, install))
        return 0;

      if (install)
        ret = install_evpn_route_entry_in_vrf (bgp_vrf, evp, pi);
      else
        ret = uninstall_evpn_route_entry_in_vrf (bgp_vrf, evp, pi);

      if (ret)
        flog_err (EC_BGP_EVPN_FAIL, "Failed to %s EVPN %pFX route in VRF %s",
 ...
 */
static int
install_uninstall_routes_for_vrf (struct bgp *bgp_vrf, int install)
{
  afi_t afi;
  safi_t safi;
                  ...
                  if (ret)
                    {
                      flog_err (EC_BGP_EVPN_FAIL,
                                ...
                                rtype == BGP_EVPN_MAC_IP_ROUTE ? "MACIP"
                                                               : "IMET",
                                vpn->vni);
...
/* Install any existing remote routes applicable for this VRF into VRF RIB.
 * This is invoked upon l3vni-add or l3vni import rt change
 */
static int
install_routes_for_vrf (struct bgp *bgp_vrf)
{
...
install_routes_for_vni (struct bgp *bgp, struct bgpevpn *vpn)
{
  ...
  ret = install_uninstall_routes_for_vni (bgp, vpn, BGP_EVPN_AD_ROUTE, 1);
  if (ret)
    return ret;
...
static int
uninstall_routes_for_vrf (struct bgp *bgp_vrf)
{
...
uninstall_routes_for_vni (struct bgp *bgp, struct bgpevpn *vpn)
{
  ...
  ret = install_uninstall_routes_for_vni (bgp, vpn, BGP_EVPN_AD_ROUTE, 0);
  if (ret)
    return ret;
 ...
 */
static int
install_uninstall_route_in_vrfs (struct bgp *bgp_def, afi_t afi, safi_t safi,
                                 struct prefix_evpn *evp,
                                 struct bgp_path_info *pi, struct list *vrfs,
                                 int install)
{
  struct bgp *bgp_vrf;
  struct listnode *node, *nnode;
  ...
  for (ALL_LIST_ELEMENTS (vrfs, node, nnode, bgp_vrf))
    {
      ...
      if (ret)
        {
          flog_err (EC_BGP_EVPN_FAIL, "%u: Failed to %s prefix %pFX in VRF %s",
                    bgp_def->vrf_id, install ? "install" : "uninstall", evp,
                    vrf_id_to_name (bgp_vrf->vrf_id));
          return ret;
        }
    }
...
install_uninstall_route_in_vnis (struct bgp *bgp, afi_t afi, safi_t safi,
                                 struct prefix_evpn *evp,
                                 struct bgp_path_info *pi, struct list *vnis,
                                 int install)
{
  struct bgpevpn *vpn;
  struct listnode *node, *nnode;
  ...
  for (ALL_LIST_ELEMENTS (vnis, node, nnode, vpn))
    {
      ...
      if (ret)
        {
          flog_err (EC_BGP_EVPN_FAIL,
                    "%u: Failed to %s EVPN %s route in VNI %u", bgp->vrf_id,
                    install ? "install" : "uninstall",
                    evp->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE ? "MACIP"
                                                                    : "IMET",
                    vpn->vni);
          return ret;
        }
 ...
 */
static int
bgp_evpn_install_uninstall_table (struct bgp *bgp, afi_t afi, safi_t safi,
                                  ...
                                  bool in_vni_rt, bool in_vrf_rt)
{
   ...
   */
  if (evp->prefix.route_type == BGP_EVPN_AD_ROUTE)
    evp = evpn_type1_prefix_vni_ip_copy (&ad_evp, evp, attr->nexthop);
...
install_uninstall_evpn_route (struct bgp *bgp, afi_t afi, safi_t safi,
                              ...
                              int import)
{
  return bgp_evpn_install_uninstall_table (bgp, afi, safi, p, pi, import, true,
                                           true);
}

void
bgp_evpn_import_type2_route (struct bgp_path_info *pi, int import)
{
  struct bgp *bgp_evpn;
  ...
  install_uninstall_evpn_route (bgp_evpn, AFI_L2VPN, SAFI_EVPN, &pi->net->p,
                                pi, import);
}
 ...
 */
static void
delete_withdraw_vrf_routes (struct bgp *bgp_vrf)
{
  /* Delete ipv4 default route and withdraw from peers */
  if (evpn_default_originate_set (bgp_vrf, AFI_IP, SAFI_UNICAST))
    bgp_evpn_install_uninstall_default_route (bgp_vrf, AFI_IP, SAFI_UNICAST,
                                              false);
  ...
  if (evpn_default_originate_set (bgp_vrf, AFI_IP6, SAFI_UNICAST))
    bgp_evpn_install_uninstall_default_route (bgp_vrf, AFI_IP6, SAFI_UNICAST,
                                              false);
 ...
 */
void
update_advertise_vrf_routes (struct bgp *bgp_vrf)
{
  struct bgp *bgp_evpn = NULL; /* EVPN bgp instance */
 ...
 */
static void
update_router_id_vrf (struct bgp *bgp_vrf)
{
 ...
 */
static void
withdraw_router_id_vrf (struct bgp *bgp_vrf)
{
  /* skip if the RD is configured */
  if (is_vrf_rd_configured (bgp_vrf))
    return;
...
static void
update_advertise_vni_route (struct bgp *bgp, struct bgpevpn *vpn,
                            struct bgp_dest *dest)
{
  struct bgp_dest *global_dest;
  ...
  if (evp->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE)
    {
      if (is_evpn_prefix_ipaddr_none (evp))
        {
          /* VNI MAC -> Global */
          evpn_type2_prefix_global_copy (&tmp_evp, evp, NULL /* mac */,
      ...
      else
        {
          /* VNI IP -> Global */
          evpn_type2_prefix_global_copy (
              &tmp_evp, evp, evpn_type2_path_info_get_mac (pi), NULL /* ip */);
        }
    }
  else
    {
  ...
  if (evp->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE)
    {
      ...
      update_evpn_route_entry (bgp, vpn, afi, safi, global_dest, attr,
                               NULL /* mac */, NULL /* ip */, 1, &global_pi, 0,
  ...
  else
    {
      ...
      bgp_evpn_mh_route_update (bgp, es, vpn, afi, safi, global_dest, attr,
 ...
 */
static void
update_router_id_vni (struct hash_bucket *bucket, struct bgp *bgp)
{
 ...
 */
static void
withdraw_router_id_vni (struct hash_bucket *bucket, struct bgp *bgp)
{
  struct bgpevpn *vpn = (struct bgpevpn *) bucket->data;
 ...
 */
static void
create_advertise_type3 (struct hash_bucket *bucket, void *data)
{
 ...
 */
static void
delete_withdraw_type3 (struct hash_bucket *bucket, void *data)
{
  struct bgpevpn *vpn = bucket->data;
  struct bgp *bgp = data;
  struct prefix_evpn p;
                     ...
                     uint32_t addpath_id)
{
  ...
  if (ipaddr_len)
    {
      ...
      p.prefix.macip_addr.ip.ipa_type =
          (ipaddr_len == IPV4_MAX_BYTELEN) ? IPADDR_V4 : IPADDR_V6;
  ...
  /* Process the route. */
  if (attr)
    bgp_update (peer, (struct prefix *) &p, addpath_id, attr, afi, safi,
                ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, &label[0], num_labels,
                0, &evpn);
  else
                     ...
                     uint32_t addpath_id)
{
   ...
   */
  if (attr && (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_PMSI_TUNNEL)))
    {
                     ...
                     uint32_t addpath_id)
{
  ...
  if (psize == 34)
  ...
  else
    {
      ...
      memcpy (&p.prefix.prefix_addr.ip.ipaddr_v6, pfx, IPV6_MAX_BYTELEN);
   ...
   */
  if (bgp_evpn_is_esi_valid (&evpn.eth_s_id) && ! ipaddr_is_zero (&evpn.gw_ip))
    {
      flog_err (EC_BGP_EVPN_ROUTE_INVALID,
  ...
  if (attr && is_valid_update)
    bgp_update (peer, (struct prefix *) &p, addpath_id, attr, afi, safi,
                ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, &label, 1, 0, &evpn);
  else
    {
      if (! is_valid_update)
        {
          ...
          zlog_warn (
              ...
              peer->hostname, peer->bgp->vrf_id, &p, attr_str);
        }
      bgp_withdraw (peer, (struct prefix *) &p, addpath_id, afi, safi,
                    ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, &label, 1, &evpn);
    }
...
static void
evpn_mpattr_encode_type5 (struct stream *s, const struct prefix *p,
  ...
  if (attr && attr->evpn_overlay.type == OVERLAY_INDEX_GATEWAY_IP)
    {
      ...
      if (IS_IPADDR_V4 (&p_evpn_p->prefix_addr.ip))
        stream_put_ipv4 (s, evpn_overlay->gw_ip.ipaddr_v4.s_addr);
      else
 ...
 */
static void
cleanup_vni_on_disable (struct hash_bucket *bucket, struct bgp *bgp)
{
  struct bgpevpn *vpn = (struct bgpevpn *) bucket->data;
 ...
 */
static void
free_vni_entry (struct hash_bucket *bucket, struct bgp *bgp)
{
  struct bgpevpn *vpn = (struct bgpevpn *) bucket->data;
 ...
 */
static void
evpn_auto_rt_import_add_for_vrf (struct bgp *bgp_vrf)
{
  struct bgp *bgp_evpn = NULL;
 ...
 */
static void
evpn_auto_rt_import_delete_for_vrf (struct bgp *bgp_vrf)
{
  evpn_rt_delete_auto (bgp_vrf, bgp_vrf->l3vni, bgp_vrf->vrf_import_rtl, true);
}
 ...
 */
static void
evpn_auto_rt_export_add_for_vrf (struct bgp *bgp_vrf)
{
 ...
 */
static void
evpn_auto_rt_export_delete_for_vrf (struct bgp *bgp_vrf)
{
  evpn_rt_delete_auto (bgp_vrf, bgp_vrf->l3vni, bgp_vrf->vrf_export_rtl, true);
}

static void
bgp_evpn_handle_export_rt_change_for_vrf (struct bgp *bgp_vrf)
{
  struct bgp *bgp_evpn = NULL;
 ...
 */
static void
update_autort_vni (struct hash_bucket *bucket, struct bgp *bgp)
{
  struct bgpevpn *vpn = bucket->data;
 ...
 */
static void
update_autort_l3vni (struct bgp *bgp)
{
...
void
bgp_evpn_withdraw_type5_route (struct bgp *bgp_vrf, const struct prefix *p,
...
void
bgp_evpn_withdraw_type5_routes (struct bgp *bgp_vrf, afi_t afi, safi_t safi)
{
          ...
          if (CHECK_FLAG (pi->flags, BGP_PATH_SELECTED) &&
              is_route_injectable_into_evpn (pi))
            {
              bgp_evpn_withdraw_type5_route (
                  bgp_vrf, bgp_dest_get_prefix (dest), afi, safi);
              break;
            }
        }
    }
 ...
 */
void
bgp_evpn_install_uninstall_default_route (struct bgp *bgp_vrf, afi_t afi,
                                          safi_t safi, bool add)
{
  ...
  if (add)
    {
      bgp_evpn_advertise_type5_route (bgp_vrf, &ip_prefix, NULL, afi, safi);
    }
  else
    {
      bgp_evpn_withdraw_type5_route (bgp_vrf, &ip_prefix, afi, safi);
    }
}
 ...
 */
void
bgp_evpn_advertise_type5_route (struct bgp *bgp_vrf, const struct prefix *p,
 ...
 */
void
bgp_evpn_advertise_type5_routes (struct bgp *bgp_vrf, afi_t afi, safi_t safi)
{
  struct bgp_table *table = NULL;
  struct bgp_dest *dest = NULL;
  struct bgp_path_info *pi;
              ...
              if (bgp_vrf->adv_cmd_rmap[afi][safi].map)
                {
                  ...
                  ret = route_map_apply (bgp_vrf->adv_cmd_rmap[afi][safi].map,
                  ...
                  bgp_evpn_advertise_type5_route (bgp_vrf,
                                                  bgp_dest_get_prefix (dest),
                                                  &tmp_attr, afi, safi);
                }
              else
                bgp_evpn_advertise_type5_route (
                    bgp_vrf, bgp_dest_get_prefix (dest), pi->attr, afi, safi);
              break;
            }
        }
    }
...
static void
rt_list_remove_node (struct list *rt_list, struct ecommunity *ecomdel,
                     bool is_l3)
{
...
void
evpn_rt_delete_auto (struct bgp *bgp, vni_t vni, struct list *rtl, bool is_l3)
{
...
static void
evpn_vrf_rt_routes_map (struct bgp *bgp_vrf)
{
...
static void
evpn_vrf_rt_routes_unmap (struct bgp *bgp_vrf)
{
...
static bool
rt_list_has_cfgd_rt (struct list *rt_list)
{
  struct listnode *node = NULL, *nnode = NULL;
  struct vrf_route_target *l3rt = NULL;
...
static void
unconfigure_import_rt_for_vrf_fini (struct bgp *bgp_vrf)
{
...
static void
unconfigure_export_rt_for_vrf_fini (struct bgp *bgp_vrf)
{
...
void
bgp_evpn_configure_import_rt_for_vrf (struct bgp *bgp_vrf,
                                      struct ecommunity *ecomadd,
                                      bool is_wildcard)
{
  struct vrf_route_target *newrt;
...
void
bgp_evpn_configure_import_auto_rt_for_vrf (struct bgp *bgp_vrf)
{
  if (CHECK_FLAG (bgp_vrf->vrf_flags, BGP_VRF_IMPORT_AUTO_RT_CFGD))
...
void
bgp_evpn_unconfigure_import_rt_for_vrf (struct bgp *bgp_vrf,
...
void
bgp_evpn_unconfigure_import_auto_rt_for_vrf (struct bgp *bgp_vrf)
{
  if (! CHECK_FLAG (bgp_vrf->vrf_flags, BGP_VRF_IMPORT_AUTO_RT_CFGD))
    return; /* Already un-configured */
...
void
bgp_evpn_configure_export_rt_for_vrf (struct bgp *bgp_vrf,
                                      struct ecommunity *ecomadd)
{
  struct vrf_route_target *newrt;
...
void
bgp_evpn_configure_export_auto_rt_for_vrf (struct bgp *bgp_vrf)
{
  if (CHECK_FLAG (bgp_vrf->vrf_flags, BGP_VRF_EXPORT_AUTO_RT_CFGD))
    return; /* Already configured */
...
void
bgp_evpn_unconfigure_export_rt_for_vrf (struct bgp *bgp_vrf,
                                        struct ecommunity *ecomdel)
{
...
void
bgp_evpn_unconfigure_export_auto_rt_for_vrf (struct bgp *bgp_vrf)
{
  if (! CHECK_FLAG (bgp_vrf->vrf_flags, BGP_VRF_EXPORT_AUTO_RT_CFGD))
    return; /* Already un-configured */
  ...
  if (withdraw)
    {
      ...
      hash_iterate (
          bgp->vnihash,
          (void (*) (struct hash_bucket *, void *)) withdraw_router_id_vni,
          bgp);
  ...
  else
    {
          ...
          for (ALL_LIST_ELEMENTS_RO (bm->bgp, node, bgp_vrf))
            {
              if (bgp_vrf->evpn_info->advertise_pip &&
                  (bgp_vrf->evpn_info->pip_ip_static.s_addr == INADDR_ANY))
                {
                  bgp_vrf->evpn_info->pip_ip = bgp->router_id;
      ...
      hash_iterate (
          bgp->vnihash,
          (void (*) (struct hash_bucket *, void *)) update_router_id_vni, bgp);
    }
}
 ...
 */
void
bgp_evpn_handle_autort_change (struct bgp *bgp)
{
  hash_iterate (bgp->vnihash,
                (void (*) (struct hash_bucket *, void *)) update_autort_vni,
                bgp);
  if (bgp->l3vni)
...
void
bgp_evpn_handle_vrf_rd_change (struct bgp *bgp_vrf, int withdraw)
{
  if (withdraw)
    delete_withdraw_vrf_routes (bgp_vrf);
  else
 ...
 */
char *
bgp_evpn_label2str (mpls_label_t *label, uint32_t num_labels, char *buf,
                    int len)
{
  vni_t vni1, vni2;
 ...
 */
void
bgp_evpn_route2json (const struct prefix_evpn *p, json_object *json)
{
  ...
  switch (p->prefix.route_type)
    ...
    case BGP_EVPN_MAC_IP_ROUTE:
      json_object_int_add (json, "ethTag", p->prefix.macip_addr.eth_tag);
      json_object_int_add (json, "macLen", 8 * ETH_ALEN);
      json_object_string_add (
          json, "mac",
          prefix_mac2str (&p->prefix.macip_addr.mac, buf1, sizeof (buf1)));

      if (! is_evpn_prefix_ipaddr_none (p))
        {
          family = is_evpn_prefix_ipaddr_v4 (p) ? AF_INET : AF_INET6;
          prefixlen = (family == AF_INET) ? IPV4_MAX_BITLEN : IPV6_MAX_BITLEN;
          inet_ntop (family, &p->prefix.macip_addr.ip.ip.addr, buf2,
                     PREFIX2STR_BUFFER);
    ...
    case BGP_EVPN_IMET_ROUTE:
      json_object_int_add (json, "ethTag", p->prefix.imet_addr.eth_tag);
      family = is_evpn_prefix_ipaddr_v4 (p) ? AF_INET : AF_INET6;
      prefixlen = (family == AF_INET) ? IPV4_MAX_BITLEN : IPV6_MAX_BITLEN;
    ...
    case BGP_EVPN_IP_PREFIX_ROUTE:
      json_object_int_add (json, "ethTag", p->prefix.prefix_addr.eth_tag);
      family = is_evpn_prefix_ipaddr_v4 (p) ? AF_INET : AF_INET6;
      inet_ntop (family, &p->prefix.prefix_addr.ip.ip.addr, buf2,
                 sizeof (buf2));
 ...
 */
void
bgp_evpn_encode_prefix (struct stream *s, const struct prefix *p,
                        const struct prefix_rd *prd, mpls_label_t *label,
                        uint32_t num_labels, struct attr *attr,
                        bool addpath_capable, uint32_t addpath_tx_id)
{
  ...
  switch (evp->prefix.route_type)
    ...
    case BGP_EVPN_MAC_IP_ROUTE:
      ...
      if (ipa_len)                  /* IP */
        stream_put (s, &evp->prefix.macip_addr.ip.ip.addr, ipa_len);
  ...
  for (; pnt < lim; pnt += psize)
    {
      ...
      switch (rtype)
        {
        case BGP_EVPN_MAC_IP_ROUTE:
          if (process_type2_route (peer, afi, safi, withdraw ? NULL : attr,
        ...
        case BGP_EVPN_IMET_ROUTE:
          if (process_type3_route (peer, afi, safi, withdraw ? NULL : attr,
                                   pnt, psize, addpath_id))
            {
        ...
        case BGP_EVPN_IP_PREFIX_ROUTE:
          if (process_type5_route (peer, afi, safi, withdraw ? NULL : attr,
                                   pnt, psize, addpath_id))
            {
              flog_err (EC_BGP_PKT_PROCESS,
 ...
 */
void
bgp_evpn_map_vrf_to_its_rts (struct bgp *bgp_vrf)
{
  struct listnode *node, *nnode;
  struct vrf_route_target *l3rt;
 ...
 */
void
bgp_evpn_unmap_vrf_from_its_rts (struct bgp *bgp_vrf)
{
  struct listnode *node, *nnode;
  struct vrf_route_target *l3rt;
 ...
 */
void
bgp_evpn_derive_auto_rd_for_vrf (struct bgp *bgp)
{
 ...
 */
bool
bgp_evpn_lookup_l3vni_l2vni_table (vni_t vni)
{
              ...
              ifindex_t svi_ifindex)
{
  ...
  vpn->import_rtl->cmp = (int (*) (void *, void *)) bgp_evpn_route_target_cmp;
  vpn->import_rtl->del = bgp_evpn_xxport_delete_ecomm;
  vpn->export_rtl = list_new ();
  vpn->export_rtl->cmp = (int (*) (void *, void *)) bgp_evpn_route_target_cmp;
...
static void
hash_evpn_free (struct bgpevpn *vpn)
{
  XFREE (MTYPE_BGP_EVPN, vpn);
}
...
int
bgp_filter_evpn_routes_upon_martian_nh_change (struct bgp *bgp)
{
  afi_t afi;
  safi_t safi;
  struct bgp_dest *rd_dest, *dest;
          ...
          for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
            {
              ...
              if (! (pi->type == ZEBRA_ROUTE_BGP &&
                     pi->sub_type == BGP_ROUTE_NORMAL))
                continue;
              if (bgp_nexthop_self (bgp, afi, pi->type, pi->sub_type, pi->attr,
                                    dest))
                {
 ...
 */
int
bgp_evpn_local_macip_del (struct bgp *bgp, vni_t vni, struct ethaddr *mac,
  ...
  if (! vpn || ! is_vni_live (vpn))
    {
      flog_warn (EC_BGP_EVPN_VPN_VNI,
                 "%u: VNI hash entry for VNI %u %s at MACIP DEL", bgp->vrf_id,
                 vni, vpn ? "not live" : "not found");
      return -1;
    }
 ...
 */
int
bgp_evpn_local_macip_add (struct bgp *bgp, vni_t vni, struct ethaddr *mac,
                          ...
                          esi_t *esi)
{
  ...
  if (! vpn || ! is_vni_live (vpn))
    {
      flog_warn (EC_BGP_EVPN_VPN_VNI,
                 "%u: VNI hash entry for VNI %u %s at MACIP ADD", bgp->vrf_id,
                 vni, vpn ? "not live" : "not found");
      return -1;
    }
...
static void
link_l2vni_hash_to_l3vni (struct hash_bucket *bucket, struct bgp *bgp_vrf)
{
  struct bgpevpn *vpn = (struct bgpevpn *) bucket->data;
  struct bgp *bgp_evpn = NULL;
...
int
bgp_evpn_local_l3vni_add (vni_t l3vni, vrf_id_t vrf_id,
...
int
bgp_evpn_local_l3vni_del (vni_t l3vni, vrf_id_t vrf_id)
{
  struct bgp *bgp_vrf = NULL;  /* bgp vrf instance */
  struct bgp *bgp_evpn = NULL; /* EVPN bgp instance */
  struct listnode *node = NULL;
                        ...
                        struct in_addr mcast_grp, ifindex_t svi_ifindex)
{
  ...
  if (vpn)
  ...
  else
    {
      ...
      vpn = bgp_evpn_new (bgp, vni, originator_ip, tenant_vrf_id, mcast_grp,
                          svi_ifindex);
    }
  ...
  if (bgp_evpn_vni_flood_mode_get (bgp, vpn) == VXLAN_FLOOD_HEAD_END_REPL)
    {
      ...
      if (update_evpn_route (bgp, vpn, &p, 0, 0, NULL))
        {
          flog_err (EC_BGP_EVPN_ROUTE_CREATE,
                    "%u: Type3 route creation failure for VNI %u", bgp->vrf_id,
                    vni);
          return -1;
        }
    }
 ...
 */
void
bgp_evpn_flood_control_change (struct bgp *bgp)
{
...
bgp_evpn_cleanup_on_disable (struct bgp *bgp)
{
  hash_iterate (
      bgp->vnihash,
      (void (*) (struct hash_bucket *, void *)) cleanup_vni_on_disable, bgp);
}
...
{
  hash_iterate (bgp->vnihash,
                (void (*) (struct hash_bucket *, void *)) free_vni_entry, bgp);
...
bgp_evpn_init (struct bgp *bgp)
{
  ...
  bgp->import_rt_hash = hash_create (import_rt_hash_key_make,
                                     import_rt_hash_cmp, "BGP Import RT Hash");
...
void
bgp_evpn_vrf_delete (struct bgp *bgp_vrf)
{
  bgp_evpn_unmap_vrf_from_its_rts (bgp_vrf);
 ...
 */
int
bgp_evpn_get_type5_prefixlen (const struct prefix *pfx)
{
 ...
 */
bool
bgp_evpn_is_prefix_nht_supported (const struct prefix *pfx)
{
  struct prefix_evpn *evp = (struct prefix_evpn *) pfx;
...
static void *
bgp_evpn_remote_ip_hash_alloc (void *p)
{
...
static unsigned int
bgp_evpn_remote_ip_hash_key_make (const void *p)
{
...
static bool
bgp_evpn_remote_ip_hash_cmp (const void *p1, const void *p2)
{
...
static void
bgp_evpn_remote_ip_hash_init (struct bgpevpn *vpn)
{
  ...
  vpn->remote_ip_hash =
      hash_create (bgp_evpn_remote_ip_hash_key_make,
                   bgp_evpn_remote_ip_hash_cmp, "BGP EVPN remote IP hash");
}

static void
bgp_evpn_remote_ip_hash_free (struct hash_bucket *bucket, void *args)
{
...
static void
bgp_evpn_remote_ip_hash_destroy (struct bgpevpn *vpn)
{
...
static void
bgp_evpn_remote_ip_hash_add (struct bgpevpn *vpn, struct bgp_path_info *pi)
{
...
static void
bgp_evpn_remote_ip_hash_del (struct bgpevpn *vpn, struct bgp_path_info *pi)
{
  struct evpn_remote_ip tmp;
  struct evpn_remote_ip *ip;
  struct prefix_evpn *evp;
...
static void
show_remote_ip_entry (struct hash_bucket *bucket, void *args)
{
...
void
bgp_evpn_show_remote_ip_hash (struct hash_bucket *bucket, void *args)
{
  struct bgpevpn *vpn = (struct bgpevpn *) bucket->data;
  struct vty *vty = (struct vty *) args;
  ...
  vty_out (vty, "VNI: %u\n", vpn->vni);
  bgp_evpn_remote_ip_hash_iterate (
      vpn, (void (*) (struct hash_bucket *, void *)) show_remote_ip_entry,
      vty);
  vty_out (vty, "\n");
}

static void
bgp_evpn_remote_ip_hash_link_nexthop (struct hash_bucket *bucket, void *args)
{
  struct evpn_remote_ip *ip = (struct evpn_remote_ip *) bucket->data;
  struct bgpevpn *vpn = (struct bgpevpn *) args;
...
static void
bgp_evpn_remote_ip_hash_unlink_nexthop (struct hash_bucket *bucket, void *args)
{
  struct evpn_remote_ip *ip = (struct evpn_remote_ip *) bucket->data;
  struct bgpevpn *vpn = (struct bgpevpn *) args;
...
static unsigned int
vni_svi_hash_key_make (const void *p)
{
  const struct bgpevpn *vpn = p;
...
static bool
vni_svi_hash_cmp (const void *p1, const void *p2)
{
  const struct bgpevpn *vpn1 = p1;
  const struct bgpevpn *vpn2 = p2;
...
bgp_evpn_vni_svi_hash_lookup (struct bgp *bgp, ifindex_t svi)
{
  struct bgpevpn *vpn;
  struct bgpevpn tmp;
...
static void
bgp_evpn_link_to_vni_svi_hash (struct bgp *bgp, struct bgpevpn *vpn)
{
...
static void
bgp_evpn_unlink_from_vni_svi_hash (struct bgp *bgp, struct bgpevpn *vpn)
{
  if (vpn->svi_ifindex == 0)
    return;
...
void
bgp_evpn_show_vni_svi_hash (struct hash_bucket *bucket, void *args)
{
 ...
 */
bool
bgp_evpn_is_gateway_ip_resolved (struct bgp_nexthop_cache *bnc)
{
  struct bgp *bgp_evpn = NULL;
  struct bgpevpn *vpn = NULL;
  struct evpn_remote_ip tmp;
  ...
  if (p->family == AF_INET)
    {
      ...
      memcpy (&(tmp.addr.ipaddr_v4), &(p->u.prefix4), sizeof (struct in_addr));
    }
...
static void
bgp_evpn_remote_ip_process_nexthops (struct bgpevpn *vpn, struct ipaddr *addr,
                                     bool resolve)
{
  afi_t afi;
  struct prefix p;
  ...
  if (addr->ipa_type == IPADDR_V4)
    {
      afi = AFI_IP;
      p.family = AF_INET;
      memcpy (&(p.u.prefix4), &(addr->ipaddr_v4), sizeof (struct in_addr));
  ...
  if (BGP_DEBUG (nht, NHT))
    zlog_debug ("%s(%u): vni %u mac/ip %s for NH %pFX",
                vpn->bgp_vrf->name_pretty, vpn->tenant_vrf_id, vpn->vni,
                (resolve ? "add" : "delete"), &bnc->prefix);
 ...
 */
mpls_label_t *
bgp_evpn_path_info_labels_get_l3vni (mpls_label_t *labels, uint32_t num_labels)
{
  if (! labels)
    return NULL;
 ...
 */
vni_t
bgp_evpn_path_info_get_l3vni (const struct bgp_path_info *pi)
{
  if (! pi->extra)
    return 0;
 ...
 */
static bool
bgp_evpn_path_is_dvni (const struct bgp *bgp_vrf,
 ...
 */
bool
bgp_evpn_mpath_has_dvni (const struct bgp *bgp_vrf,
