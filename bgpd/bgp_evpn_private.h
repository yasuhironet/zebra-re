...
RB_PROTOTYPE (bgp_es_evi_rb_head, bgp_evpn_es_evi, rb_node, bgp_es_evi_rb_cmp);
/*
...
#define VNI_FLAG_USE_TWO_LABELS                                               \
  0x20 /* Attach both L2-VNI and L3-VNI if                                    \
          needed for this VPN */
...
static inline int
is_vrf_rd_configured (struct bgp *bgp_vrf)
{
  return (CHECK_FLAG (bgp_vrf->vrf_flags, BGP_VRF_RD_CFGD));
}

static inline int
bgp_evpn_vrf_rd_matches_existing (struct bgp *bgp_vrf, struct prefix_rd *prd)
{
  return (memcmp (&bgp_vrf->vrf_prd.val, prd->val, ECOMMUNITY_SIZE) == 0);
}

static inline vni_t
bgpevpn_get_l3vni (struct bgpevpn *vpn)
{
  return vpn->bgp_vrf ? vpn->bgp_vrf->l3vni : 0;
}

static inline void
bgpevpn_get_rmac (struct bgpevpn *vpn, struct ethaddr *rmac)
{
...
static inline void
bgpevpn_unlink_from_l3vni (struct bgpevpn *vpn)
{
...
static inline void
bgpevpn_link_to_l3vni (struct bgpevpn *vpn)
{
...
static inline int
is_l3vni_live (struct bgp *bgp_vrf)
{
...
static inline void
encode_es_rt_extcomm (struct ecommunity_val *eval, struct ethaddr *mac)
{
...
static inline void
encode_df_elect_extcomm (struct ecommunity_val *eval, uint16_t pref)
{
...
static inline void
encode_esi_label_extcomm (struct ecommunity_val *eval, bool single_active)
{
  memset (eval, 0, sizeof (struct ecommunity_val));
  eval->val[0] = ECOMMUNITY_ENCODE_EVPN;
...
static inline void
encode_rmac_extcomm (struct ecommunity_val *eval, struct ethaddr *rmac)
{
  memset (eval, 0, sizeof (*eval));
  eval->val[0] = ECOMMUNITY_ENCODE_EVPN;
...
static inline void
encode_default_gw_extcomm (struct ecommunity_val *eval)
{
  memset (eval, 0, sizeof (*eval));
...
static inline void
encode_mac_mobility_extcomm (int static_mac, uint32_t seq,
...
static inline void
encode_na_flag_extcomm (struct ecommunity_val *eval, uint8_t na_flag,
                        bool proxy)
{
  memset (eval, 0, sizeof (*eval));
  eval->val[0] = ECOMMUNITY_ENCODE_EVPN;
...
static inline void
ip_prefix_from_type5_prefix (const struct prefix_evpn *evp, struct prefix *ip)
{
...
static inline int
is_evpn_prefix_default (const struct prefix *evp)
{
  if (evp->family != AF_EVPN)
    return 0;

  return ((evp->u.prefix_evpn.prefix_addr.ip_prefix_length == 0) ? 1 : 0);
}

static inline void
ip_prefix_from_type2_prefix (const struct prefix_evpn *evp, struct prefix *ip)
{
  memset (ip, 0, sizeof (struct prefix));
  if (is_evpn_prefix_ipaddr_v4 (evp))
    {
...
static inline void
ip_prefix_from_evpn_prefix (const struct prefix_evpn *evp, struct prefix *ip)
{
  ...
  if (ip_prefix->family == AF_INET)
    {
      ...
      memcpy (&ip.ipaddr_v4, &ip_prefix->u.prefix4, sizeof (struct in_addr));
    }
  else
    {
      ip.ipa_type = IPADDR_V6;
      memcpy (&ip.ipaddr_v6, &ip_prefix->u.prefix6, sizeof (struct in6_addr));
    }
...
static inline void
build_evpn_type4_prefix (struct prefix_evpn *p, esi_t *esi,
                         struct in_addr originator_ip)
{
  memset (p, 0, sizeof (struct prefix_evpn));
  p->family = AF_EVPN;
...
static inline void
build_evpn_type1_prefix (struct prefix_evpn *p, uint32_t eth_tag, esi_t *esi,
                         struct in_addr originator_ip)
{
  memset (p, 0, sizeof (struct prefix_evpn));
  p->family = AF_EVPN;
...
static inline void
evpn_type1_prefix_global_copy (struct prefix_evpn *global_p,
...
static inline void
evpn_type2_path_info_set_mac (struct bgp_path_info *local_pi,
                              const struct ethaddr mac)
{
  assert (local_pi->extra);
...
static inline void
evpn_type2_path_info_set_ip (struct bgp_path_info *local_pi,
                             const struct ipaddr ip)
{
  assert (local_pi->extra);
...
static inline bool
is_evpn_type2_dest_ipaddr_none (const struct bgp_dest *dest)
{
...
static inline int
evpn_default_originate_set (struct bgp *bgp, afi_t afi, safi_t safi)
{
  if (afi == AFI_IP && CHECK_FLAG (bgp->af_flags[AFI_L2VPN][SAFI_EVPN],
...
static inline void
es_get_system_mac (esi_t *esi, struct ethaddr *mac)
{
  /*
...
static inline bool
bgp_evpn_is_svi_macip_enabled (struct bgpevpn *vpn)
{
...
static inline bool
bgp_evpn_is_path_local (struct bgp *bgp, struct bgp_path_info *pi)
{
