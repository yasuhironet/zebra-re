...
void
bgp_dump_listener_info (struct vty *vty)
{
 ...
 */
static int
bgp_md5_set_socket (int socket, union sockunion *su, uint16_t prefixlen,
                    const char *password)
{
  int ret = -1;
  int en = ENOSYS;
  ...
  if ((su2.sa.sa_family == AF_INET && prefixlen == IPV4_MAX_BITLEN) ||
    ...
    ret = sockopt_tcp_signature_ext (socket, &su2, prefixlen, password);
  en = errno;
#endif /* HAVE_TCP_MD5SIG */
...
static int
bgp_md5_set_connect (int socket, union sockunion *su, uint16_t prefixlen,
                     const char *password)
{
  int ret = -1;
        ...
        if (listener->su.sa.sa_family == peer->su.sa.sa_family)
          {
            ...
            ret = bgp_md5_set_socket (listener->fd, &peer->su, prefixlen,
                                      password);
            break;
          }
    }
...
int
bgp_md5_set_prefix (struct bgp *bgp, struct prefix *p, const char *password)
{
  int ret = 0;
  union sockunion su;
  struct listnode *node;
  ...
  frr_with_privs (&bgpd_privs)
    {
      for (ALL_LIST_ELEMENTS_RO (bm->listen_sockets, node, listener))
        if (listener->su.sa.sa_family == p->family &&
            ((bgp->vrf_id == VRF_DEFAULT) || (listener->bgp == bgp)))
          {
            prefix2sockunion (p, &su);
            ret =
                bgp_md5_set_socket (listener->fd, &su, p->prefixlen, password);
            break;
          }
    }
...
int
bgp_md5_unset_prefix (struct bgp *bgp, struct prefix *p)
{
...
static void
bgp_socket_set_buffer_size (const int fd)
{
...
static void
bgp_accept (struct event *thread)
{
  int bgp_sock;
  int accept_sock;
  union sockunion su;
  ...
  if (bgp_sock < 0)
    {
      if (save_errno == EINVAL)
        {
          struct vrf *vrf = bgp ? vrf_lookup_by_id (bgp->vrf_id) : NULL;
          ...
          flog_err_sys (
              ...
              safe_strerror (save_errno), accept_sock, VRF_LOGNAME (vrf));
      ...
      if (peer1)
        {
          ...
          EVENT_OFF (peer1->t_start); /* created in peer_create() */

          if (peer_active (peer1))
            {
              if (CHECK_FLAG (peer1->flags, PEER_FLAG_TIMER_DELAYOPEN))
                BGP_EVENT_ADD (peer1, TCP_connection_open_w_delay);
              else
  ...
  if (bgp_debug_neighbor_events (peer1))
    zlog_debug (
        ...
        inet_sutop (&su, buf), bgp_sock, peer1->status, peer1->fd);
  ...
  if (peer_established (peer1) &&
      CHECK_FLAG (peer1->sflags, PEER_STATUS_NSF_MODE))
    {
      ...
      if (CHECK_FLAG (peer1->flags, PEER_FLAG_GRACEFUL_RESTART) ||
          CHECK_FLAG (peer1->flags, PEER_FLAG_GRACEFUL_RESTART_HELPER))
...
/* BGP socket bind. */
static char *
bgp_get_bound_name (struct peer *peer)
{
  if (! peer)
    return NULL;
  ...
  if (peer->su.sa.sa_family != AF_INET && peer->su.sa.sa_family != AF_INET6)
...
bgp_connect (struct peer *peer)
{
  ...
  if (peer->fd < 0)
    {
      ...
      if (bgp_debug_neighbor_events (peer))
        zlog_debug (
            "%s: Failure to create socket for connection to %s, error received: %s(%d)",
            __func__, peer->host, safe_strerror (errno), errno);
      return -1;
    }
  ...
  if (peer->password)
    {
      uint16_t prefixlen =
          peer->su.sa.sa_family == AF_INET ? IPV4_MAX_BITLEN : IPV6_MAX_BITLEN;

      if (! BGP_PEER_SU_UNSPEC (peer))
        bgp_md5_set (peer);

      bgp_md5_set_connect (peer->fd, &peer->su, prefixlen, peer->password);
    }
  ...
  if (bgp_debug_neighbor_events (peer))
    zlog_debug ("%s [Event] Connect start to %s fd %d", peer->host, peer->host,
                peer->fd);
...
bgp_getsockname (struct peer *peer)
{
  ...
  if (! bgp_zebra_nexthop_set (peer->su_local, peer->su_remote, &peer->nexthop,
                               peer))
    {
...
static int
bgp_listener (int sock, struct sockaddr *sa, socklen_t salen, struct bgp *bgp)
{
  struct bgp_listener *listener;
  int ret, en;
  ...
  event_add_read (bm->master, bgp_accept, listener, sock, &listener->thread);
...
bgp_socket (struct bgp *bgp, unsigned short port, const char *address)
{
  ...
  frr_with_privs (&bgpd_privs)
    {
      ret =
          vrf_getaddrinfo (address, port_str, &req, &ainfo_save, bgp->vrf_id);
    }
  if (ret != 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "getaddrinfo: %s", gai_strerror (ret));
      return -1;
    }
  if (bgp_option_check (BGP_OPT_NO_ZEBRA) && bgp->vrf_id != VRF_DEFAULT)
    {
  ...
  for (ainfo = ainfo_save; ainfo; ainfo = ainfo->ai_next)
    {
      ...
      frr_with_privs (&bgpd_privs)
        {
          sock = vrf_socket (
              ...
              bgp->vrf_id,
              (bgp->inst_type == BGP_INSTANCE_TYPE_VRF ? bgp->name : NULL));
        }
      if (sock < 0)
        {
      ...
      ret = bgp_listener (sock, ainfo->ai_addr, ainfo->ai_addrlen, bgp);
      if (ret == 0)
        ++count;
      else
        close (sock);
  ...
  if (count == 0 && bgp->inst_type != BGP_INSTANCE_TYPE_VRF)
    {
      flog_err (
          ...
          __func__, port);
      flog_err_sys (EC_LIB_SOCKET, "%s: Program cannot continue", __func__);
      exit (-1);
    }
 ...
 */
void
bgp_close_vrf_socket (struct bgp *bgp)
{
