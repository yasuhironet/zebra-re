 ...
 */
bool
bgp_addpath_is_addpath_used (struct bgp_addpath_bgp_data *d, afi_t afi,
                             safi_t safi)
{
 ...
 */
void
bgp_addpath_init_bgp_data (struct bgp_addpath_bgp_data *d)
{
  safi_t safi;
  afi_t afi;
  int i;
 ...
 */
void
bgp_addpath_free_info_data (struct bgp_addpath_info_data *d,
  ...
  for (i = 0; i < BGP_ADDPATH_MAX; i++)
    {
      if (d->addpath_tx_id[i] != IDALLOC_INVALID)
        idalloc_free_to_pool (&nd->free_ids[i], d->addpath_tx_id[i]);
    }
}
 ...
 */
uint32_t
bgp_addpath_id_for_peer (struct peer *peer, afi_t afi, safi_t safi,
 ...
 */
bool
bgp_addpath_info_has_ids (struct bgp_addpath_info_data *d)
{
  int i;
 ...
 */
void
bgp_addpath_free_node_data (struct bgp_addpath_bgp_data *bd,
                            ...
                            safi_t safi)
{
  ...
  for (i = 0; i < BGP_ADDPATH_MAX; i++)
    {
      idalloc_drain_pool (bd->id_allocators[afi][safi][i], &(nd->free_ids[i]));
    }
}
 ...
 */
bool
bgp_addpath_dmed_required (int strategy)
{
 ...
 */
bool
bgp_addpath_tx_path (enum bgp_addpath_strat strat, struct bgp_path_info *pi)
{
  switch (strat)
    {
...
static void
bgp_addpath_flush_type_rn (struct bgp *bgp, afi_t afi, safi_t safi,
                           ...
                           struct bgp_dest *dest)
{
  ...
  for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
    {
      if (pi->tx_addpath.addpath_tx_id[addpath_type] != IDALLOC_INVALID)
        {
          idalloc_free (bgp->tx_addpath.id_allocators[afi][safi][addpath_type],
                        pi->tx_addpath.addpath_tx_id[addpath_type]);
          pi->tx_addpath.addpath_tx_id[addpath_type] = IDALLOC_INVALID;
        }
    }
}
 ...
 * strategy, and afi/safi combination. This lets us let go of all memory held
 * to track ID numbers associated with an addpath type not in use. Since
 * post-bestpath ID processing is skipped for types not used, this is the only
 * chance to free this data.
 */
static void
bgp_addpath_flush_type (struct bgp *bgp, afi_t afi, safi_t safi,
                        enum bgp_addpath_strat addpath_type)
{
      ...
      if (safi == SAFI_MPLS_VPN)
        {
          ...
          for (ndest = bgp_table_top (table); ndest;
            ...
            bgp_addpath_flush_type_rn (bgp, afi, safi, addpath_type, ndest);
        }
      else
        {
          bgp_addpath_flush_type_rn (bgp, afi, safi, addpath_type, dest);
        }
    }
 ...
 */
static void
bgp_addpath_populate_path (struct id_alloc *allocator,
 ...
 * and afi/safi combination. Since we won't waste the time computing addpath
 * IDs for unused strategies, the first time a peer is configured to use a
 * strategy, we have to backfill the data. In labeled-unicast, addpath
 * allocations SHOULD be done in unicast SAFI.
 */
static void
bgp_addpath_populate_type (struct bgp *bgp, afi_t afi, safi_t safi,
                           enum bgp_addpath_strat addpath_type)
{
  struct bgp_dest *dest, *ndest;
  char buf[200];
  ...
  bgp->tx_addpath.id_allocators[afi][safi][addpath_type] = idalloc_new (buf);
      ...
      if (safi == SAFI_MPLS_VPN)
        {
          ...
          for (ndest = bgp_table_top (table); ndest;
               ndest = bgp_route_next (ndest))
            for (bi = bgp_dest_get_bgp_path_info (ndest); bi; bi = bi->next)
              bgp_addpath_populate_path (allocator, bi, addpath_type);
        }
      else
        {
          for (bi = bgp_dest_get_bgp_path_info (dest); bi; bi = bi->next)
            bgp_addpath_populate_path (allocator, bi, addpath_type);
        }
    }
}
 ...
 */
void
bgp_addpath_type_changed (struct bgp *bgp)
{
  afi_t afi;
  safi_t safi;
 ...
 */
void
bgp_addpath_set_peer_type (struct peer *peer, afi_t afi, safi_t safi,
                           enum bgp_addpath_strat addpath_type)
{
      ...
      if (group != NULL)
        {
          for (ALL_LIST_ELEMENTS (group->peer, node, nnode, tmp_peer))
            {
              if (tmp_peer->addpath_type[afi][safi] == old_type)
                {
                  bgp_addpath_set_peer_type (tmp_peer, afi, safi,
                                             addpath_type);
                }
            }
        }
 ...
 * best-per-as updates from needing to do a separate withdraw and update just
 * to swap out which path is sent.
 */
void
bgp_addpath_update_ids (struct bgp *bgp, struct bgp_dest *bn, afi_t afi,
                        safi_t safi)
{
  int i;
  struct bgp_path_info *pi;
  ...
  for (i = 0; i < BGP_ADDPATH_MAX; i++)
    {
      struct id_alloc *alloc = bgp->tx_addpath.id_allocators[afi][safi][i];
      ...
      for (pi = bgp_dest_get_bgp_path_info (bn); pi; pi = pi->next)
        {
          if (pi->tx_addpath.addpath_tx_id[i] != IDALLOC_INVALID &&
            ...
            {
              idalloc_free_to_pool (pool_ptr, pi->tx_addpath.addpath_tx_id[i]);
              pi->tx_addpath.addpath_tx_id[i] = IDALLOC_INVALID;
            }
        }
          ...
          if (pi->tx_addpath.addpath_tx_id[i] == IDALLOC_INVALID &&
              bgp_addpath_tx_path (i, pi))
            {
              pi->tx_addpath.addpath_tx_id[i] =
                  idalloc_allocate_prefer_pool (alloc, pool_ptr);
            }
        }
