...
static inline int
bgp_is_valid_label (const mpls_label_t *label)
{
...
static inline void
bgp_register_for_label (struct bgp_dest *dest, struct bgp_path_info *pi)
{
  bgp_reg_dereg_for_label (dest, pi, true);
}

static inline void
bgp_unregister_for_label (struct bgp_dest *dest)
{
