      ...
      for (ALL_LIST_ELEMENTS_RO (bgp->peer, node, peer))
        {
          ...
          if (! next_peer || ntohl (sockunion2ip (&next_peer->su)) >
...
/* BGP write methods. */
static int
write_bgpPeerTable (int action, uint8_t *var_val, uint8_t var_val_type,
...
static uint8_t *
bgpIdentifier (struct variable *v, oid name[], size_t *length, int exact,
               size_t *var_len, WriteMethod **write_method)
{
  struct bgp *bgp;
          ...
          for (path = bgp_dest_get_bgp_path_info (dest); path;
               path = path->next)
            {
              if (path->peer->su.sin.sin_family == AF_INET &&
                  ntohl (paddr.s_addr) <
                      ntohl (path->peer->su.sin.sin_addr.s_addr))
                {
                  if (min)
                    {
                      if (ntohl (path->peer->su.sin.sin_addr.s_addr) <
...
static uint8_t *
bgp4PathAttrTable (struct variable *v, oid name[], size_t *length, int exact,
                   size_t *var_len, WriteMethod **write_method)
{
  struct bgp *bgp;
  struct bgp_path_info *path;
...
bgpTrapEstablished (struct peer *peer)
{
  ...
  smux_trap (bgp_variables, array_size (bgp_variables), bgp_trap_oid,
             ...
             sizeof (bgp_oid) / sizeof (oid), index, IN_ADDR_SIZE, bgpTrapList,
             array_size (bgpTrapList), BGPESTABLISHED);
  return 0;
}
...
bgpTrapBackwardTransition (struct peer *peer)
{
  ...
  smux_trap (bgp_variables, array_size (bgp_variables), bgp_trap_oid,
             array_size (bgp_trap_oid), bgp_oid,
             sizeof (bgp_oid) / sizeof (oid), index, IN_ADDR_SIZE, bgpTrapList,
             array_size (bgpTrapList), BGPBACKWARDTRANSITION);
  return 0;
}

int
bgp_snmp_bgp4_init (struct event_loop *tm)
{
