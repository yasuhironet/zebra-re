...
static int
route_value_match (struct rmap_value *rv, uint32_t value)
{
...
static uint32_t
route_value_adjust (struct rmap_value *rv, uint32_t current, struct peer *peer)
{
  uint32_t value;
...
/* Compares the peer specified in the 'match peer' clause with the peer
    received in bgp_path_info->peer. If it is the same, or if the peer
   structure received is a peer_group containing it, returns RMAP_MATCH. */
...
static const struct route_map_rule_cmd route_match_peer_cmd = {
  "peer", route_match_peer, route_match_peer_compile, route_match_peer_free
};
...
route_match_script (void *rule, const struct prefix *prefix, void *object)
{
  ...
  if (frrscript_load (fs, routematch_function, NULL))
    {
      zlog_err ("Issue loading script or function; defaulting to no match");
      return RMAP_NOMATCH;
    }
  ...
  int result = frrscript_call (
      fs, routematch_function, ("prefix", prefix), ("attributes", &newattr),
      ("peer", path->peer), ("RM_FAILURE", LUA_RM_FAILURE),
      ("RM_NOMATCH", LUA_RM_NOMATCH), ("RM_MATCH", LUA_RM_MATCH),
  ...
  long long *action =
      frrscript_get_result (fs, routematch_function, "action", lua_tointegerp);
...
static void *
route_match_script_compile (const char *arg)
{
  char *scriptname;
...
static void
route_match_script_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}

static const struct route_map_rule_cmd route_match_script_cmd = {
  "script", route_match_script, route_match_script_compile,
                                 ...
                                 const struct prefix *prefix, void *object)
{
  ...
  if (prefix->family == AF_FLOWSPEC)
    return route_match_prefix_list_flowspec (afi, plist, prefix);
...
static void *
route_match_ipv6_next_hop_prefix_list_compile (const char *arg)
{
  return XSTRDUP (MTYPE_ROUTE_MAP_COMPILED, arg);
}

static void
route_match_ipv6_next_hop_prefix_list_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static const struct route_map_rule_cmd
    route_match_ipv6_next_hop_prefix_list_cmd = {
      "ipv6 next-hop prefix-list", route_match_ipv6_next_hop_prefix_list,
  ...
  if (prefix->family == AF_INET)
    {
      ...
      if (path->attr->nexthop.s_addr == INADDR_ANY && ! path->attr->nh_ifindex)
        return RMAP_MATCH;
    }
  return RMAP_NOMATCH;
}
...
static void *
route_match_ip_next_hop_type_compile (const char *arg)
{
  return XSTRDUP (MTYPE_ROUTE_MAP_COMPILED, arg);
}

static void
route_match_ip_next_hop_type_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static const struct route_map_rule_cmd route_match_ip_next_hop_type_cmd = {
  ...
  route_match_ip_next_hop_type_compile, route_match_ip_next_hop_type_free
};
...
static const struct route_map_rule_cmd route_match_evpn_default_route_cmd = {
...
/* Route map `mac address' match statement.  `arg' should be
   access-list name. */
static void *
route_match_mac_address_compile (const char *arg)
{
  return XSTRDUP (MTYPE_ROUTE_MAP_COMPILED, arg);
}
...
/* Free route map's compiled `ip address' value. */
static void
route_match_mac_address_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
route_match_vni (void *rule, const struct prefix *prefix, void *object)
{
  ...
  for (; label_cnt < BGP_MAX_LABELS && label_cnt < path->extra->num_labels;
       label_cnt++)
    {
...
static void *
route_match_vni_compile (const char *arg)
{
  vni_t *vni = NULL;
  char *end = NULL;
...
static void
route_match_vni_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static const struct route_map_rule_cmd route_match_evpn_vni_cmd = {
  "evpn vni", route_match_vni, route_match_vni_compile, route_match_vni_free
};
...
static void *
route_match_evpn_route_type_compile (const char *arg)
{
...
static void
route_match_evpn_route_type_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static void *
route_match_rd_compile (const char *arg)
{
...
static const struct route_map_rule_cmd route_match_evpn_rd_cmd = {
  "evpn rd", route_match_rd, route_match_rd_compile, route_match_rd_free
};
                           ...
                           void *object)
{
  ...
  memcpy (&path->attr->evpn_overlay.gw_ip, &gw_ip->ip.addr, IPADDRSZ (gw_ip));
 ...
 */
static void *
route_set_evpn_gateway_ip_compile (const char *arg)
{
...
static void
route_set_evpn_gateway_ip_free (void *rule)
{
  struct ipaddr *gw_ip = rule;
...
static void *
route_match_vrl_source_vrf_compile (const char *arg)
{
...
/* Free route map's compiled `route-type' value. */
static void
route_match_vrl_source_vrf_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}

static const struct route_map_rule_cmd route_match_vrl_source_vrf_cmd = {
  "source-vrf", route_match_vrl_source_vrf, route_match_vrl_source_vrf_compile,
  ...
  if (bgp_attr_get_community (path->attr))
    {
      ...
      for (int i = 0; i < num; i++)
        {
          const char *com2alias = bgp_community2alias (communities[i]);
...
static void *
route_match_alias_compile (const char *arg)
{
...
static void
route_match_alias_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}

static const struct route_map_rule_cmd route_match_alias_cmd = {
  "alias", route_match_alias, route_match_alias_compile, route_match_alias_free
};
...
static const struct route_map_rule_cmd route_match_local_pref_cmd = {
  "local-preference", route_match_local_pref, route_match_local_pref_compile,
...
route_match_community (void *rule, const struct prefix *prefix, void *object)
{
  ...
  if (rcom->exact)
    {
      if (community_list_exact_match (bgp_attr_get_community (path->attr),
                                      list))
        return RMAP_MATCH;
    }
  else
    {
      if (community_list_match (bgp_attr_get_community (path->attr), list))
        return RMAP_MATCH;
    }
 ...
 */
static void *
route_match_get_community_key (void *rule)
{
  struct rmap_community *rcom;
...
route_match_lcommunity (void *rule, const struct prefix *prefix, void *object)
{
  ...
  if (rcom->exact)
    {
      if (lcommunity_list_exact_match (bgp_attr_get_lcommunity (path->attr),
                                       list))
        return RMAP_MATCH;
    }
  else
    {
      if (lcommunity_list_match (bgp_attr_get_lcommunity (path->attr), list))
        return RMAP_MATCH;
    }
...
static const struct route_map_rule_cmd route_match_lcommunity_cmd = {
  "large-community", route_match_lcommunity, route_match_lcommunity_compile,
  route_match_lcommunity_free, route_match_get_community_key
};
...
static void *
route_set_srte_color_compile (const char *arg)
{
  uint32_t *color;
...
static void
route_set_srte_color_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
route_set_ip_nexthop_compile (const char *arg)
{
  ...
  rins =
      XCALLOC (MTYPE_ROUTE_MAP_COMPILED, sizeof (struct rmap_ip_nexthop_set));
...
static void *
route_set_l3vpn_nexthop_encapsulation_compile (const char *arg)
{
...
/* Free route map's compiled `ip nexthop' value. */
static void
route_set_l3vpn_nexthop_encapsulation_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static const struct route_map_rule_cmd
    route_set_l3vpn_nexthop_encapsulation_cmd = {
      "l3vpn next-hop encapsulation", route_set_l3vpn_nexthop_encapsulation,
...
static const struct route_map_rule_cmd route_set_table_id_cmd = {
  "table", route_set_table_id, route_value_compile, route_value_free
};
...
static void *
route_aspath_replace_compile (const char *arg)
{
  return XSTRDUP (MTYPE_ROUTE_MAP_COMPILED, arg);
}

static void
route_aspath_replace_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
route_set_aspath_replace (void *rule, const struct prefix *dummy, void *object)
{
  ...
  if (path->peer->sort != BGP_PEER_EBGP)
    {
      zlog_warn ("`set as-path replace` is supported only for EBGP peers");
      return RMAP_NOOP;
    }
  ...
  if (strmatch (replace, "any"))
    {
      path->attr->aspath = aspath_replace_all_asn (aspath_new, own_asn);
    }
  else
    {
      ...
      path->attr->aspath =
          aspath_replace_specific_asn (aspath_new, replace_asn, own_asn);
    }
...
static void *
route_set_ecommunity_none_compile (const char *arg)
{
  struct rmap_ecom_set *rcs;
  bool none = false;
...
static void *
route_set_ecommunity_nt_compile (const char *arg)
{
  struct rmap_ecom_set *rcs;
  struct ecommunity *ecom;
...
route_set_ecommunity_lb (void *rule, const struct prefix *prefix, void *object)
{
  ...
  encode_lb_extcomm (
      as, bw_bytes, rels->non_trans, &lb_eval,
...
static void *
route_set_ecommunity_lb_compile (const char *arg)
{
  ...
  rels = XCALLOC (MTYPE_ROUTE_MAP_COMPILED, sizeof (struct rmap_ecomm_lb_set));
...
static void
route_set_ecommunity_lb_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static void *
route_set_aigp_metric_compile (const char *arg)
{
  return XSTRDUP (MTYPE_ROUTE_MAP_COMPILED, arg);
}

static void
route_set_aigp_metric_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static const struct route_map_rule_cmd route_match_ipv6_address_cmd = {
  "ipv6 address", route_match_ipv6_address, route_match_ipv6_address_compile,
...
static void *
route_match_ipv6_next_hop_address_compile (const char *arg)
{
...
static void
route_match_ipv6_next_hop_address_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}

static const struct route_map_rule_cmd
    route_match_ipv6_next_hop_address_cmd = {
      "ipv6 next-hop address", route_match_ipv6_next_hop_address,
...
static void *
route_match_ipv4_next_hop_compile (const char *arg)
{
  struct in_addr *address;
  int ret;
...
static void
route_match_ipv4_next_hop_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}

static const struct route_map_rule_cmd route_match_ipv4_next_hop_cmd = {
  "ip next-hop address", route_match_ipv4_next_hop,
  route_match_ipv4_next_hop_compile, route_match_ipv4_next_hop_free
};
...
static void *
route_match_ipv6_next_hop_type_compile (const char *arg)
{
  struct in6_addr *address;
  int ret;
...
static void
route_match_ipv6_next_hop_type_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}

static const struct route_map_rule_cmd route_match_ipv6_next_hop_type_cmd = {
  "ipv6 next-hop type", route_match_ipv6_next_hop_type,
  route_match_ipv6_next_hop_type_compile, route_match_ipv6_next_hop_type_free
};
...
/* Route map commands for ip nexthop set. */
static const struct route_map_rule_cmd route_set_ipv6_nexthop_global_cmd = {
  "ipv6 next-hop global", route_set_ipv6_nexthop_global,
  route_set_ipv6_nexthop_global_compile, route_set_ipv6_nexthop_global_free
};
...
/* Route map commands for ip nexthop set. */
static const struct route_map_rule_cmd route_set_ipv6_nexthop_local_cmd = {
                             ...
                             void *object)
{
  ...
  if ((CHECK_FLAG (peer->rmap_type, PEER_RMAP_TYPE_IN) ||
  ...
  else if (CHECK_FLAG (peer->rmap_type, PEER_RMAP_TYPE_OUT))
    {
      ...
      memset (&(path->attr->mp_nexthop_global), 0, sizeof (struct in6_addr));
      memset (&(path->attr->mp_nexthop_local), 0, sizeof (struct in6_addr));
    }
...
route_set_vpnv6_nexthop (void *rule, const struct prefix *prefix, void *object)
{
  ...
  /* Set next hop value. */
  memcpy (&path->attr->mp_nexthop_global, address, sizeof (struct in6_addr));
...
static void *
route_match_extcommunity_compile (const char *arg)
{
  int *rpki_status;
...
static const struct route_map_rule_cmd route_match_rpki_extcommunity_cmd = {
  "rpki-extcommunity", route_match_rpki_extcommunity,
  route_match_extcommunity_compile, route_value_free
};
                            ...
                            int route_update)
{
          ...
          if (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG))
            {
              if (bgp_debug_update (peer, NULL, NULL, 1))
                zlog_debug (
                    ...
                    rmap_name, afi2str (afi), safi2str (safi), peer->host);
  ...
  if (filter->advmap.aname && (strcmp (rmap_name, filter->advmap.aname) == 0))
    {
      filter->advmap.amap = map;
    }

  if (filter->advmap.cname && (strcmp (rmap_name, filter->advmap.cname) == 0))
    {
 ...
 * called for each route-map and within this function we walk the list of
 * peers, network statements, etc looking to see if they use this route-map.
 */
static void
bgp_route_map_process_update (struct bgp *bgp, const char *rmap_name,
                              bool route_update)
{
  int i;
  bool matched;
      ...
      FOREACH_AFI_SAFI (afi, safi)
        {
           ...
           * route-maps */
          bgp_route_map_process_peer (rmap_name, map, peer, afi, safi,
                                      route_update);
        }
    }
      ...
      if (bgp->table_map[afi][safi].name &&
          (strcmp (rmap_name, bgp->table_map[afi][safi].name) == 0))
        {
          ...
          if (BGP_DEBUG (zebra, ZEBRA))
            zlog_debug ("Processing route_map %s(%s:%s) update on table map",
          ...
          if (route_update && ! bgp_static->backdoor)
            {
              ...
              if (bgp_debug_zebra (bn_p))
                zlog_debug (
                    ...
                    rmap_name, afi2str (afi), safi2str (safi),
                    inet_ntop (bn_p->family, &bn_p->u.prefix, buf,
                               sizeof (buf)));
              bgp_static_update (bgp, bn_p, bgp_static, afi, safi);
            }
        }
      ...
      for (bn = bgp_table_top (bgp->aggregate[afi][safi]); bn;
           bn = bgp_route_next (bn))
        {
          ...
          if (aggregate->suppress_map_name &&
              strmatch (aggregate->suppress_map_name, rmap_name))
            {
              ...
              bgp_aggregate_toggle_suppressed (
                  aggregate, bgp, bgp_dest_get_prefix (bn), afi, safi, false);
          ...
          if (matched && route_update)
            {
              ...
              if (bgp_debug_zebra (bn_p))
                zlog_debug (
                    ...
                    inet_ntop (bn_p->family, &bn_p->u.prefix, buf,
                               sizeof (buf)));
              (void) bgp_aggregate_route (bgp, bn_p, afi, safi, aggregate);
            }
        }
    }
    ...
    for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
      {
        ...
        for (ALL_LIST_ELEMENTS_RO (red_list, node, red))
          {
            if (! red->rmap.name || (strcmp (rmap_name, red->rmap.name) != 0))
              continue;
...
void
bgp_route_map_update_timer (struct event *thread)
{
  ...
  if (bm->rmap_update_timer)
    {
       ...
       * started */
      for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
        update_group_policy_update (bgp, BGP_POLICY_ROUTE_MAP, rmap_name, true,
                                    1);
    }
  else
    {
...
static void
bgp_route_map_event (const char *rmap_name)
{
  if (route_map_mark_updated (rmap_name) == 0)
    bgp_route_map_mark_update (rmap_name);
 ...
 */
static const char *
parse_evpn_rt_type (const char *num_rt_type)
{
            ...
            "Gateway IP address in IPv4 format\n")
{
  ...
  if (su.sin.sin_addr.s_addr == 0 || ! ipv4_unicast_valid (&su.sin.sin_addr))
    {
      vty_out (vty,
...
{
  const char *xpath = "./match-condition[condition='frr-bgp-route-map:peer']";
  char xpath_value[XPATH_MAXLEN];
  ...
  if (addrv4_str)
  ...
  else if (addrv6_str)
    {
      snprintf (xpath_value, sizeof (xpath_value),
                ...
                xpath);
      nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, addrv6_str);
    }
  else
    {
      snprintf (xpath_value, sizeof (xpath_value),
            ...
            "The script name to run, without .lua; e.g. 'myroutemap' to run myroutemap.lua\n")
{
  ...
  if (no)
    {
      nb_cli_enqueue_change (vty, xpath, NB_OP_DESTROY, NULL);
      snprintf (xpath_value, sizeof (xpath_value),
                "%s/rmap-match-condition/frr-bgp-route-map:script", xpath);
      nb_cli_enqueue_change (vty, xpath_value, NB_OP_DESTROY, script);
  ...
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-match-condition/frr-bgp-route-map:script", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, script);
            ...
            "Percentage of routes\n")
{
  ...
  nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-match-condition/frr-bgp-route-map:probability", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY,
                         argv[idx_number]->arg);
            ...
            "IP Access-list name\n")
{
  ...
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-match-condition/frr-bgp-route-map:list-name", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, argv[idx_acl]->arg);
            ...
            "IP prefix-list name\n")
{
  ...
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-match-condition/frr-bgp-route-map:list-name", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, argv[idx_word]->arg);
            ...
            "Use peer address (for BGP only)\n")
{
  ...
  if (strmatch (argv[0]->text, "no"))
  ...
  else
    {
      nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
      snprintf (xpath_value, sizeof (xpath_value),
                "%s/rmap-set-action/frr-bgp-route-map:ipv4-nexthop", xpath);
      nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, "peer-address");
    }
  return nb_cli_apply_changes (vty, NULL);
}
            ...
            "Don't modify existing Next hop address\n")
{
  ...
  if (strmatch (argv[0]->text, "no"))
  ...
  else
    {
      ...
      snprintf (xpath_value, sizeof (xpath_value),
                "%s/rmap-set-action/frr-bgp-route-map:ipv4-nexthop", xpath);
      nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, "unchanged");
    }
  return nb_cli_apply_changes (vty, NULL);
}
...
DEFPY_YANG (set_l3vpn_nexthop_encapsulation,
            set_l3vpn_nexthop_encapsulation_cmd,
...
{
  int idx_number = 2;
  const char *xpath = "./set-action[action='frr-bgp-route-map:label-index']";
  char xpath_value[XPATH_MAXLEN];
            ...
            "Number of times to insert\n")
{
  ...
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, argv[idx_num]->arg);
  return nb_cli_apply_changes (vty, NULL);
}
...
ALIAS_YANG (no_set_aspath_exclude,
            ...
            "VPN extended community\n")
{
  ...
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-set-action/frr-bgp-route-map:extcommunity-soo", xpath);
  str = argv_concat (argv, argc, idx_asn_nn);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, str);
  ret = nb_cli_apply_changes (vty, NULL);
  XFREE (MTYPE_TMP, str);
...
DEFUN_YANG (set_ecommunity_none,
            ...
            "No extended community attribute\n")
{
  ...
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-set-action/frr-bgp-route-map:extcommunity-none", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, "true");
  return nb_cli_apply_changes (vty, NULL);
}

DEFUN_YANG (no_set_ecommunity_none,
            ...
            "Attribute is set as non-transitive\n")
{
  ...
  snprintf (xpath_bandwidth, sizeof (xpath_bandwidth),
            ...
            xpath);
  snprintf (
      xpath_non_transitive, sizeof (xpath_non_transitive),
  ...
  if (argv_find (argv, argc, "non-transitive", &idx_non_transitive))
    ...
    nb_cli_enqueue_change (vty, xpath_non_transitive, NB_OP_MODIFY, "false");
            ...
            "unknown heritage\n")
{
  int idx_origin = 2;
  const char *origin_type;
  const char *xpath = "./set-action[action='frr-bgp-route-map:set-origin']";
  char xpath_value[XPATH_MAXLEN];
            ...
            "BGP atomic aggregate attribute\n")
{
  ...
  nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-set-action/frr-bgp-route-map:atomic-aggregate", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, NULL);
...
{
  const char *xpath = "./set-action[action='frr-bgp-route-map:aigp-metric']";
  char xpath_value[XPATH_MAXLEN];
            ...
            "IP address of aggregator\n")
{
  ...
  const char *xpath = "./set-action[action='frr-bgp-route-map:aggregator']";
  as_t as_value;
  ...
  snprintf (xpath_asn, sizeof (xpath_asn),
            "%s/rmap-set-action/frr-bgp-route-map:aggregator/aggregator-asn",
            xpath);
  nb_cli_enqueue_change (vty, xpath_asn, NB_OP_MODIFY, argv[idx_number]->arg);

  snprintf (
      xpath_addr, sizeof (xpath_addr),
      "%s/rmap-set-action/frr-bgp-route-map:aggregator/aggregator-address",
      xpath);
  nb_cli_enqueue_change (vty, xpath_addr, NB_OP_MODIFY, argv[idx_ipv4]->arg);
            ...
            "IPv6 address of next hop\n")
{
  ...
  nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-match-condition/frr-bgp-route-map:ipv6-address", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, argv[argc - 1]->arg);
            ...
            "IP address of next-hop\n")
{
  ...
  nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-match-condition/frr-bgp-route-map:ipv4-address", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, argv[4]->arg);
...
{
  int idx_ipv4 = 2;
  const char *xpath = "./set-action[action='frr-bgp-route-map:originator-id']";
  char xpath_value[XPATH_MAXLEN];

  nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
  snprintf (xpath_value, sizeof (xpath_value),
            "%s/rmap-set-action/frr-bgp-route-map:originator-id", xpath);
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, argv[idx_ipv4]->arg);
