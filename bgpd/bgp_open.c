...
void
bgp_capability_vty_out (struct vty *vty, struct peer *peer, bool use_json,
      ...
      if (hdr->code == CAPABILITY_CODE_MP)
        {
          ...
          (void) bgp_map_afi_safi_iana2int (ntohs (mpc.afi), mpc.safi, &afi,
                                            &safi);
              ...
              switch (afi)
                ...
                case AFI_IP:
                  json_object_string_add (
                      json_cap, "capabilityErrorMultiProtocolAfi", "IPv4");
                  break;
                case AFI_IP6:
                  json_object_string_add (
                      json_cap, "capabilityErrorMultiProtocolAfi", "IPv6");
                  break;
                case AFI_L2VPN:
                  json_object_string_add (
                      json_cap, "capabilityErrorMultiProtocolAfi", "L2VPN");
                  break;
                case AFI_UNSPEC:
                case AFI_MAX:
                  json_object_int_add (
                      json_cap, "capabilityErrorMultiProtocolAfiUnknown",
                      ntohs (mpc.afi));
                  break;
                }
              switch (safi)
                ...
                case SAFI_UNICAST:
                  json_object_string_add (
                      json_cap, "capabilityErrorMultiProtocolSafi", "unicast");
                  break;
                case SAFI_MULTICAST:
                ...
                case SAFI_ENCAP:
                  json_object_string_add (
                      json_cap, "capabilityErrorMultiProtocolSafi", "encap");
                  break;
                case SAFI_EVPN:
                  json_object_string_add (
                      json_cap, "capabilityErrorMultiProtocolSafi", "EVPN");
                  break;
                case SAFI_FLOWSPEC:
                  json_object_string_add (json_cap,
                                          "capabilityErrorMultiProtocolSafi",
                ...
                case SAFI_MAX:
                  json_object_int_add (
                      json_cap, "capabilityErrorMultiProtocolSafiUnknown",
                      mpc.safi);
                  break;
                }
            }
      ...
      else
        {
          if (use_json)
            json_object_int_add (
                json_cap, "capabilityErrorUnknownCapabilityCode", hdr->code);
          else
...
bgp_capability_mp (struct peer *peer, struct capability_header *hdr)
{
  ...
  if (bgp_debug_neighbor_events (peer))
    zlog_debug ("%s OPEN has %s capability for afi/safi: %s/%s", peer->host,
...
static void
bgp_capability_orf_not_support (struct peer *peer, iana_afi_t afi,
                                iana_safi_t safi, uint8_t type, uint8_t mode)
{
  if (bgp_debug_neighbor_events (peer))
        ...
        case CAPABILITY_CODE_ORF:
          switch (type)
            ...
            case ORF_TYPE_PREFIX:
              break;
            default:
              bgp_capability_orf_not_support (peer, pkt_afi, pkt_safi, type,
                                              mode);
              continue;
            }
          break;
...
static int
bgp_capability_llgr (struct peer *peer, struct capability_header *caphdr)
{
/*
 * +--------------------------------------------------+
  ...
  if (hdr->length != CAPABILITY_CODE_AS4_LEN)
    {
      flog_err (EC_BGP_PKT_OPEN,
                "%s AS4 capability has incorrect data length %d", peer->host,
                hdr->length);
      return 0;
    }
...
static int
bgp_capability_ext_message (struct peer *peer, struct capability_header *hdr)
{
...
static int
bgp_capability_role (struct peer *peer, struct capability_header *hdr)
{
...
static int
bgp_capability_software_version (struct peer *peer,
                                 struct capability_header *hdr)
{
  struct stream *s = BGP_INPUT (peer);
  ...
  while (stream_get_getp (s) < end)
    {
      ...
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s OPEN has %s capability (%u), length %u", peer->host,
                    lookup_msg (capcode_str, caphdr.code, NULL), caphdr.code,
                    caphdr.length);
          ...
          if (caphdr.length < cap_minsizes[caphdr.code])
            {
              zlog_info (
                  ...
                  caphdr.length, (unsigned) cap_minsizes[caphdr.code]);
              bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR,
                               BGP_NOTIFY_OPEN_MALFORMED_ATTR);
              return -1;
            }
          if (caphdr.length && caphdr.length % cap_modsizes[caphdr.code] != 0)
            {
              zlog_info (
                  ...
                  peer->host, lookup_msg (capcode_str, caphdr.code, NULL),
                  caphdr.length, (unsigned) cap_modsizes[caphdr.code]);
              bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR,
                               BGP_NOTIFY_OPEN_MALFORMED_ATTR);
              return -1;
            }
          ...
          if (caphdr.code > 128)
            {
              ...
              flog_warn (EC_BGP_CAPABILITY_VENDOR,
                         "%s Vendor specific capability %d", peer->host,
                         caphdr.code);
            }
          else
            {
...
static bool
bgp_role_violation (struct peer *peer)
{
  ...
  if (remote_role == ROLE_UNDEFINED &&
    ...
    {
      const char *err_msg = "Strict mode. Please set the role on your side.";
 ...
 */
as_t
peek_for_as4_capability (struct peer *peer, uint16_t length)
{
  struct stream *s = BGP_INPUT (peer);
          ...
          while (stream_get_getp (s) < capd_end)
            {
              ...
              if (hdr.code == CAPABILITY_CODE_AS4)
                {
                  if (BGP_DEBUG (as4, AS4))
                    zlog_debug ("[AS4] found AS4 capability, about to parse");
 ...
 */
int
bgp_open_option_parse (struct peer *peer, uint16_t length, int *mp_capability)
{
  int ret = 0;
  uint8_t *error;
  ...
  while (stream_get_getp (s) < end)
    {
      ...
      if (STREAM_READABLE (s) < opt_length)
        {
          zlog_info ("%s Option length error (%d)", peer->host, opt_length);
          bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR,
                           BGP_NOTIFY_OPEN_MALFORMED_ATTR);
          return -1;
        }
...
static void
bgp_open_capability_orf (struct stream *s, struct peer *peer, afi_t afi,
                         safi_t safi, uint8_t code, bool ext_opt_params)
{
  uint16_t cap_len;
  uint8_t orf_len;
  unsigned long capp;
  ...
  if (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_ORF_PREFIX_SM) ||
    ...
    {
      stream_putc (s, (code == CAPABILITY_CODE_ORF ? ORF_TYPE_PREFIX
                                                   : ORF_TYPE_PREFIX_OLD));
...
static void
bgp_peer_send_gr_capability (struct stream *s, struct peer *peer,
                             bool ext_opt_params)
{
  int len;
  iana_afi_t pkt_afi = IANA_AFI_IPV4;
  ...
  if (peer->bgp->t_startup)
    {
      ...
      if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
        zlog_debug ("[BGP_GR] Sending R-Bit for peer: %s", peer->host);
    }
  ...
  if (CHECK_FLAG (peer->bgp->flags, BGP_FLAG_GRACEFUL_NOTIFICATION))
    {
      ...
      if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
        zlog_debug ("[BGP_GR] Sending N-Bit for peer: %s", peer->host);
    }
      ...
      FOREACH_AFI_SAFI (afi, safi)
        {
          ...
          if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
            zlog_debug (
                "[BGP_GR] Sending GR Capability for AFI :%d :, SAFI :%d:", afi,
                safi);
...
static void
bgp_peer_send_llgr_capability (struct stream *s, struct peer *peer,
                               bool ext_opt_params)
{
  int len;
  iana_afi_t pkt_afi = IANA_AFI_IPV4;
...
uint16_t
bgp_open_capability (struct stream *s, struct peer *peer, bool ext_opt_params)
{
  uint16_t len;
      ...
      if (peer->afc[afi][safi])
        {
          ...
          stream_putc (s, BGP_OPEN_OPT_CAP);
          ext_opt_params ? stream_putw (s, CAPABILITY_CODE_MP_LEN + 2)
          ...
          if (CHECK_FLAG (peer->flags, PEER_FLAG_CAPABILITY_ENHE) &&
               ...
               safi == SAFI_LABELED_UNICAST))
            {
              ...
              stream_putc (s, BGP_OPEN_OPT_CAP);
              ext_opt_params ? stream_putw (s, CAPABILITY_CODE_ENHE_LEN + 2)
      ...
      if (peer->afc[afi][safi])
        {
          ...
          if (safi == SAFI_LABELED_UNICAST)
            if (peer->addpath_type[afi][SAFI_UNICAST] != BGP_ADDPATH_NONE)
              adv_addpath_tx = true;
        }
    }
  ...
  stream_putc (s, BGP_OPEN_OPT_CAP);
  ext_opt_params
      ? stream_putw (s, (CAPABILITY_CODE_ADDPATH_LEN * afi_safi_count) + 2)
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      if (peer->afc[afi][safi])
        {
          bool adv_addpath_rx = ! CHECK_FLAG (peer->af_flags[afi][safi],
          ...
          if (adv_addpath_rx)
            ...
            {
              UNSET_FLAG (peer->af_cap[afi][safi], PEER_CAP_ADDPATH_AF_RX_ADV);
            }
  ...
  if (CHECK_FLAG (peer->flags, PEER_FLAG_DYNAMIC_CAPABILITY))
    {
      ...
      stream_putc (s, BGP_OPEN_OPT_CAP);
      ext_opt_params ? stream_putw (s, CAPABILITY_CODE_DYNAMIC_LEN + 2)
  ...
  if (cmd_hostname_get ())
    {
      ...
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s Sending hostname cap with hn = %s, dn = %s",
                    peer->host, cmd_hostname_get (), cmd_domainname_get ());
    }
  ...
  if (peergroup_flag_check (peer, PEER_FLAG_CAPABILITY_SOFT_VERSION) ||
      peer->sort == BGP_PEER_IBGP)
    {
      ...
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s Sending Software Version cap, value: %s", peer->host,
