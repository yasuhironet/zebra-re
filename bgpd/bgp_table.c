 ...
 */
void
bgp_dest_unlock_node (struct bgp_dest *dest)
{
 ...
 */
struct bgp_dest *
bgp_dest_lock_node (struct bgp_dest *dest)
{
 ...
 */
const char *
bgp_dest_get_prefix_str (struct bgp_dest *dest)
{
 ...
 */
static void
bgp_node_destroy (route_table_delegate_t *delegate, struct route_table *table,
                  struct route_node *node)
{
  ...
  if (rt->bgp)
    {
      bgp_addpath_free_node_data (&rt->bgp->tx_addpath, &bgp_node->tx_addpath,
                                  rt->afi, rt->safi);
    }
 ...
 */
struct bgp_table *
bgp_table_init (struct bgp *bgp, afi_t afi, safi_t safi)
{
  struct bgp_table *rt;
...
void
bgp_delete_listnode (struct bgp_node *node)
{
...
struct bgp_node *
bgp_table_subtree_lookup (const struct bgp_table *table,
...
static ssize_t
printfrr_bd (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
