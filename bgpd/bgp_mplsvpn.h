...
extern int bgp_nlri_parse_vpn (struct peer *, struct attr *,
                               struct bgp_nlri *);
...
extern int argv_find_and_parse_vpnvx (struct cmd_token **argv, int argc,
                                      int *index, afi_t *afi);
extern int bgp_show_mpls_vpn (struct vty *vty, afi_t afi,
...
extern void vpn_leak_from_vrf_withdraw (struct bgp *to_bgp,
                                        struct bgp *from_bgp,
                                        struct bgp_path_info *path_vrf);
...
extern void vpn_leak_to_vrf_update_all (struct bgp *to_bgp,
                                        struct bgp *from_bgp, afi_t afi);
...
extern void vpn_leak_zebra_vrf_sid_withdraw_per_af (struct bgp *bgp,
                                                    afi_t afi);
extern void vpn_leak_zebra_vrf_sid_withdraw_per_vrf (struct bgp *bgp);
extern int vpn_leak_label_callback (mpls_label_t label, void *lblid,
                                    bool alloc);
...
extern void transpose_sid (struct in6_addr *sid, uint32_t label,
...
static inline bool
is_bgp_vrf_mplsvpn (struct bgp *bgp)
{
  afi_t afi;
...
static inline int
vpn_leak_to_vpn_active (struct bgp *bgp_vrf, afi_t afi, const char **pmsg)
{
...
static inline int
vpn_leak_from_vpn_active (struct bgp *bgp_vrf, afi_t afi, const char **pmsg)
{
  if (bgp_vrf->inst_type != BGP_INSTANCE_TYPE_VRF &&
      bgp_vrf->inst_type != BGP_INSTANCE_TYPE_DEFAULT)
    {
...
static inline void
vpn_leak_prechange (enum vpn_policy_direction direction, afi_t afi,
...
static inline void
vpn_leak_postchange (enum vpn_policy_direction direction, afi_t afi,
                     struct bgp *bgp_vpn, struct bgp *bgp_vrf)
{
  /* Detect when default bgp instance is not (yet) defined by config */
  if (! bgp_vpn)
  ...
  if (direction == BGP_VPN_POLICY_DIR_TOVPN)
    {
      ...
      if ((! bgp_vrf->vpn_policy[afi].tovpn_sid &&
          ...
          (! bgp_vrf->tovpn_sid && bgp_vrf->tovpn_zebra_vrf_sid_last_sent))
 ...
 */
static inline bool
is_route_injectable_into_vpn (struct bgp_path_info *pi)
{
  struct bgp_path_info *parent_pi;
  struct bgp_table *table;
  struct bgp_dest *dest;

  if (pi->sub_type != BGP_ROUTE_IMPORTED || ! pi->extra || ! pi->extra->parent)
    return true;
...
static inline bool
is_pi_family_vpn (struct bgp_path_info *pi)
{
