...
extern int
argv_find_and_parse_vpnvx (struct cmd_token **argv, int argc, int *index,
                           afi_t *afi)
{
  int ret = 0;
...
void
encode_label (mpls_label_t label, mpls_label_t *label_pnt)
{
  uint8_t *pnt = (uint8_t *) label_pnt;
  if (pnt == NULL)
    return;
      ...
      if ((psize - VPN_PREFIXLEN_MIN_BYTES) > (ssize_t) sizeof (p.u))
        {
          flog_err (
              ...
              peer->host, prefixlen - VPN_PREFIXLEN_MIN_BYTES * 8,
              sizeof (p.u));
          ret = BGP_NLRI_PARSE_ERROR_PACKET_LENGTH;
          goto done;
        }
      ...
      if ((psize - VPN_PREFIXLEN_MIN_BYTES) > prefix_blen (&p))
        {
          flog_err (
              ...
              peer->host, prefixlen - VPN_PREFIXLEN_MIN_BYTES * 8, p.family,
              prefix_blen (&p));
          ret = BGP_NLRI_PARSE_ERROR_PACKET_LENGTH;
          goto done;
        }
      ...
      if (attr)
        {
          bgp_update (peer, &p, addpath_id, attr, packet->afi, SAFI_MPLS_VPN,
                      ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, &label, 1, 0,
                      NULL);
        }
      else
        {
          bgp_withdraw (peer, &p, addpath_id, packet->afi, SAFI_MPLS_VPN,
 ...
 */
void
vpn_leak_zebra_vrf_label_update (struct bgp *bgp, afi_t afi)
{
 ...
 */
void
vpn_leak_zebra_vrf_label_withdraw (struct bgp *bgp, afi_t afi)
{
  mpls_label_t label = MPLS_LABEL_NONE;
  int debug = BGP_DEBUG (vpn, VPN_LEAK_LABEL);
  ...
  if (bgp->vrf_id == VRF_UNKNOWN)
    {
      if (debug)
        {
          zlog_debug ("%s: vrf_id not set, can't delete zebra vrf label",
                      __func__);
        }
      return;
    }
 ...
 */
void
vpn_leak_zebra_vrf_sid_update_per_af (struct bgp *bgp, afi_t afi)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_LABEL);
 ...
 */
void
vpn_leak_zebra_vrf_sid_update_per_vrf (struct bgp *bgp)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_LABEL);
  enum seg6local_action_t act;
  struct seg6local_context ctx = {};
  ...
  if (! tovpn_sid)
    {
      if (debug)
        zlog_debug ("%s: vrf %s: sid not set", __func__, bgp->name_pretty);
      return;
    }

  if (debug)
    zlog_debug ("%s: vrf %s: setting sid %pI6 for vrf id %d", __func__,
 ...
 */
void
vpn_leak_zebra_vrf_sid_update (struct bgp *bgp, afi_t afi)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_LABEL);
  ...
  if (debug)
    zlog_debug ("%s: vrf %s: afi %s: sid not set", __func__, bgp->name_pretty,
                afi2str (afi));
}
 ...
 */
void
vpn_leak_zebra_vrf_sid_withdraw_per_af (struct bgp *bgp, afi_t afi)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_LABEL);
 ...
 */
void
vpn_leak_zebra_vrf_sid_withdraw_per_vrf (struct bgp *bgp)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_LABEL);
 ...
 */
void
vpn_leak_zebra_vrf_sid_withdraw (struct bgp *bgp, afi_t afi)
{
...
int
vpn_leak_label_callback (mpls_label_t label, void *labelid, bool allocated)
{
  struct vpn_policy *vp = (struct vpn_policy *) labelid;
  int debug = BGP_DEBUG (vpn, VPN_LEAK_LABEL);

  if (debug)
    zlog_debug ("%s: label=%u, allocated=%d", __func__, label, allocated);
  ...
  if (! allocated)
    {
      ...
      if (CHECK_FLAG (vp->flags, BGP_VPN_POLICY_TOVPN_LABEL_AUTO) &&
          (vp->tovpn_label != MPLS_LABEL_NONE))
        {
          ...
          vpn_leak_prechange (BGP_VPN_POLICY_DIR_TOVPN, vp->afi,
                              bgp_get_default (), vp->bgp);
          vp->tovpn_label = MPLS_LABEL_NONE;
          vpn_leak_postchange (BGP_VPN_POLICY_DIR_TOVPN, vp->afi,
                               bgp_get_default (), vp->bgp);
        }
      return 0;
    }
  ...
  if (vp->tovpn_label != MPLS_LABEL_NONE)
    {
      ...
      flog_err (EC_BGP_LABEL, "%s: %s had label %u but got new assignment %u",
                __func__, vp->bgp->name_pretty, vp->tovpn_label, label);
      /* use new one */
    }

  vpn_leak_prechange (BGP_VPN_POLICY_DIR_TOVPN, vp->afi, bgp_get_default (),
                      vp->bgp);
  vp->tovpn_label = label;
  vpn_leak_postchange (BGP_VPN_POLICY_DIR_TOVPN, vp->afi, bgp_get_default (),
                       vp->bgp);
...
static void
sid_register (struct bgp *bgp, const struct in6_addr *sid,
              const char *locator_name)
{
  struct bgp_srv6_function *func;
  func = XCALLOC (MTYPE_BGP_SRV6_FUNCTION, sizeof (struct bgp_srv6_function));
  func->sid = *sid;
  snprintf (func->locator_name, sizeof (func->locator_name), "%s",
            locator_name);
  listnode_add (bgp->srv6_functions, func);
}

void
srv6_function_free (struct bgp_srv6_function *func)
{
  XFREE (MTYPE_BGP_SRV6_FUNCTION, func);
}

void
sid_unregister (struct bgp *bgp, const struct in6_addr *sid)
{
...
static bool
sid_exist (struct bgp *bgp, const struct in6_addr *sid)
{
 ...
 */
static uint32_t
alloc_new_sid (struct bgp *bgp, uint32_t index,
...
void
ensure_vrf_tovpn_sid_per_af (struct bgp *bgp_vpn, struct bgp *bgp_vrf,
                             afi_t afi)
{
  ...
  if (debug)
    zlog_debug ("%s: try to allocate new SID for vrf %s: afi %s", __func__,
  ...
  tovpn_sid_transpose_label =
      alloc_new_sid (bgp_vpn, tovpn_sid_index, tovpn_sid_locator, tovpn_sid);

  if (tovpn_sid_transpose_label == 0)
    {
      if (debug)
        zlog_debug ("%s: not allocated new sid for vrf %s: afi %s", __func__,
                    bgp_vrf->name_pretty, afi2str (afi));
  ...
  if (debug)
    zlog_debug ("%s: new sid %pI6 allocated for vrf %s: afi %s", __func__,
...
void
ensure_vrf_tovpn_sid_per_vrf (struct bgp *bgp_vpn, struct bgp *bgp_vrf)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_FROM_VRF);
  struct srv6_locator_chunk *tovpn_sid_locator;
  struct in6_addr *tovpn_sid;
  ...
  if (debug)
    zlog_debug ("%s: new sid %pI6 allocated for vrf %s", __func__, tovpn_sid,
                bgp_vrf->name_pretty);
...
void
ensure_vrf_tovpn_sid (struct bgp *bgp_vpn, struct bgp *bgp_vrf, afi_t afi)
{
  /* per-af sid */
...
void
delete_vrf_tovpn_sid_per_af (struct bgp *bgp_vpn, struct bgp *bgp_vrf,
                             afi_t afi)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_FROM_VRF);
...
void
delete_vrf_tovpn_sid_per_vrf (struct bgp *bgp_vpn, struct bgp *bgp_vrf)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_FROM_VRF);
  uint32_t tovpn_sid_index = 0;
  bool tovpn_sid_auto = false;
...
void
delete_vrf_tovpn_sid (struct bgp *bgp_vpn, struct bgp *bgp_vrf, afi_t afi)
{
 ...
 */
void
transpose_sid (struct in6_addr *sid, uint32_t label, uint8_t offset,
               uint8_t len)
{
...
static bool
labels_same (struct bgp_path_info *bpi, mpls_label_t *label, uint32_t n)
{
  uint32_t i;
 ...
 */
static void
setlabels (struct bgp_path_info *bpi,
 ...
 */
static void
setsids (struct bgp_path_info *bpi, struct in6_addr *sid, uint32_t num_sids)
{
  uint32_t i;
...
static void
unsetsids (struct bgp_path_info *bpi)
{
  struct bgp_path_info_extra *extra;
...
static bool
leak_update_nexthop_valid (struct bgp *to_bgp, struct bgp_dest *bn,
                           struct attr *new_attr, afi_t afi, safi_t safi,
                           struct bgp_path_info *source_bpi,
                           struct bgp_path_info *bpi, struct bgp *bgp_orig,
                           const struct prefix *p, int debug)
{
  ...
  if (bpi_ultimate->sub_type == BGP_ROUTE_REDISTRIBUTE ||
     ...
     * 'connected' parameter?
     */
    nh_valid = bgp_find_or_add_nexthop (to_bgp, bgp_nexthop, afi, safi, bpi,
                                        NULL, 0, p);
   ...
   * will be invalid.
   */
  if (to_bgp->srv6_enabled && (! new_attr->srv6_l3vpn && ! new_attr->srv6_vpn))
    {
      nh_valid = false;
    }
             ...
             struct prefix *nexthop_orig, int nexthop_self_flag, int debug)
{
  ...
  if (debug)
    zlog_debug ("%s: entry: leak-to=%s, p=%pBD, type=%d, sub_type=%d",
  ...
  if (bpi)
    {
          ...
          if (debug)
            {
              zlog_debug (
                  ...
                  __func__, to_bgp->name_pretty, source_bpi->flags, bpi->flags,
                  p);
            }
          return NULL;
        }
          ...
          if (new_attr->srv6_l3vpn)
            {
              ...
              extra->sid[0].loc_node_len = new_attr->srv6_l3vpn->loc_node_len;
      ...
      if (leak_update_nexthop_valid (to_bgp, bn, new_attr, afi, safi,
                                     source_bpi, bpi, bgp_orig, p, debug))
      ...
      if (debug)
        zlog_debug ("%s: ->%s: %pBD Found route, changed attr", __func__,
                    to_bgp->name_pretty, bn);
      ...
      if (new_attr->srv6_l3vpn)
        {
          ...
          extra->sid[0].loc_block_len = new_attr->srv6_l3vpn->loc_block_len;
          extra->sid[0].loc_node_len = new_attr->srv6_l3vpn->loc_node_len;
          extra->sid[0].func_len = new_attr->srv6_l3vpn->func_len;
          extra->sid[0].arg_len = new_attr->srv6_l3vpn->arg_len;
          extra->sid[0].transposition_len =
  ...
  bgp_dest_lock_node ((struct bgp_dest *) parent->net);
  if (bgp_orig)
    new->extra->bgp_orig = bgp_lock (bgp_orig);
  if (nexthop_orig)
    new->extra->nexthop_orig = *nexthop_orig;

  if (leak_update_nexthop_valid (to_bgp, bn, new_attr, afi, safi, source_bpi,
                                 new, bgp_orig, p, debug))
...
void
bgp_mplsvpn_path_nh_label_unlink (struct bgp_path_info *pi)
{
 ...
 */
static int
bgp_mplsvpn_get_label_per_nexthop_cb (mpls_label_t label, void *context,
                                      bool allocated)
{
  ...
  if (debug)
    zlog_debug ("%s: label=%u, allocated=%d, nexthop=%pFX", __func__, label,
                allocated, &blnc->nexthop);
  if (allocated)
  ...
  if (blnc->label != MPLS_INVALID_LABEL)
    bgp_zebra_send_nexthop_label (ZEBRA_MPLS_LABELS_ADD, blnc->label,
                                  blnc->nh->ifindex, blnc->nh->vrf_id,
  ...
  if (! blnc)
    {
       ...
       */
      bgp_lp_get (LP_TYPE_NEXTHOP, blnc, bgp_mplsvpn_get_label_per_nexthop_cb);
    }
      ...
      if (blnc->label != MPLS_INVALID_LABEL)
        {
          bgp_zebra_send_nexthop_label (
              ZEBRA_MPLS_LABELS_REPLACE, blnc->label, bnc->nexthop->ifindex,
              bnc->nexthop->vrf_id, ZEBRA_LSP_BGP, &blnc->nexthop);
        }
    }
 ...
 */
static mpls_label_t
vpn_leak_from_vrf_get_per_nexthop_label (afi_t afi, safi_t safi,
                                         struct bgp_path_info *pi,
                                         struct bgp *from_bgp,
                                         struct bgp *to_bgp)
{
  ...
  nh_valid = bgp_find_or_add_nexthop (from_bgp, bgp_nexthop, nh_afi, safi, pi,
                                      NULL, 0, NULL);
  ...
  return _vpn_leak_from_vrf_get_per_nexthop_label (pi, to_bgp, from_bgp, afi,
                                                   safi);
}

/* cf vnc_import_bgp_add_route_mode_nvegroup() and add_vnc_route() */
void
vpn_leak_from_vrf_update (struct bgp *to_bgp,             /* to */
                          ...
                          struct bgp_path_info *path_vrf) /* route */
{
  ...
  if (debug && bgp_attr_get_ecommunity (path_vrf->attr))
    {
      char *s = ecommunity_ecom2str (bgp_attr_get_ecommunity (path_vrf->attr),
                                     ECOMMUNITY_FORMAT_ROUTE_MAP, 0);

      zlog_debug ("%s: %s path_vrf->type=%d, EC{%s}", __func__, from_bgp->name,
                  path_vrf->type, s);
  ...
  if (from_bgp->vpn_policy[afi].rmap[BGP_VPN_POLICY_DIR_TOVPN])
    {
      ...
      ret = route_map_apply (
          from_bgp->vpn_policy[afi].rmap[BGP_VPN_POLICY_DIR_TOVPN], p, &info);
  ...
  if (debug && bgp_attr_get_ecommunity (&static_attr))
    {
      ...
      zlog_debug ("%s: post route map static_attr.ecommunity{%s}", __func__,
                  s);
      XFREE (MTYPE_ECOMMUNITY_STR, s);
    }
  ...
  if (old_ecom)
    {
      ...
      new_ecom = ecommunity_merge (
          new_ecom,
          from_bgp->vpn_policy[afi].rtlist[BGP_VPN_POLICY_DIR_TOVPN]);
      if (! old_ecom->refcnt)
  ...
  if (CHECK_FLAG (from_bgp->vpn_policy[afi].flags,
    ...
    {
      struct prefix *nexthop = &from_bgp->vpn_policy[afi].tovpn_nexthop;
  ...
  else
    {
      if (! CHECK_FLAG (from_bgp->af_flags[afi][SAFI_UNICAST],
                        BGP_CONFIG_VRF_TO_VRF_EXPORT))
        {
          if (afi == AFI_IP && ! BGP_ATTR_NEXTHOP_AFI_IP6 (path_vrf->attr))
            {
              /*
  ...
  if (CHECK_FLAG (from_bgp->vpn_policy[afi].flags,
    ...
    label_val = vpn_leak_from_vrf_get_per_nexthop_label (afi, safi, path_vrf,
                                                         from_bgp, to_bgp);
  else
    /* per VRF label mode */
  ...
  if (label_val == MPLS_INVALID_LABEL &&
                  ...
                  BGP_VPN_POLICY_TOVPN_LABEL_PER_NEXTHOP))
    {
       ...
       */
      if (debug)
        zlog_debug ("%s: %s skipping: waiting for a valid per-label nexthop.",
                    __func__, from_bgp->name_pretty);
  ...
  if (from_bgp->vpn_policy[afi].tovpn_sid_locator)
    {
      ...
      static_attr.srv6_l3vpn =
          XCALLOC (MTYPE_BGP_SRV6_L3VPN, sizeof (struct bgp_attr_srv6_l3vpn));
    ...
    {
      struct srv6_locator_chunk *locator = from_bgp->tovpn_sid_locator;
  ...
  new_info = leak_update (to_bgp, bn, new_attr, afi, safi, path_vrf, &label, 1,
...
void
vpn_leak_from_vrf_withdraw (struct bgp *to_bgp,             /* to */
                            struct bgp *from_bgp,           /* from */
                            struct bgp_path_info *path_vrf) /* route */
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_FROM_VRF);
  ...
  if (debug)
    {
      zlog_debug ("%s: entry: leak-from=%s, p=%pBD, type=%d, sub_type=%d",
...
void
vpn_leak_from_vrf_withdraw_all (struct bgp *to_bgp, struct bgp *from_bgp,
                                afi_t afi)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_FROM_VRF);
      ...
      for (bn = bgp_table_top (table); bn; bn = bgp_route_next (bn))
        {
          ...
          for (; bpi; bpi = bpi->next)
            {
              if (debug)
                zlog_debug ("%s: type %d, sub_type %d", __func__, bpi->type,
                            bpi->sub_type);
              if (bpi->sub_type != BGP_ROUTE_IMPORTED)
                continue;
              if (! bpi->extra)
                continue;
              if ((struct bgp *) bpi->extra->bgp_orig == from_bgp)
                {
                  /* delete route */
                  if (debug)
                  ...
                  vpn_leak_to_vrf_withdraw (bpi);
                  bgp_aggregate_decrement (to_bgp, bgp_dest_get_prefix (bn),
                                           bpi, afi, safi);
                  bgp_path_info_delete (bn, bpi);
                  bgp_process (to_bgp, bn, afi, safi);
                  bgp_mplsvpn_path_nh_label_unlink (bpi->extra->parent);
                }
            }
        }
    }
...
void
vpn_leak_from_vrf_update_all (struct bgp *to_bgp, struct bgp *from_bgp,
                              afi_t afi)
{
  struct bgp_dest *bn;
  struct bgp_path_info *bpi;
...
static struct bgp *
bgp_lookup_by_rd (struct bgp_path_info *bpi, struct prefix_rd *rd, afi_t afi)
{
  struct listnode *node, *nnode;
  struct bgp *bgp;
                               ...
                               struct prefix_rd *prd)
{
  ...
  if (! vpn_leak_from_vpn_active (to_bgp, afi, &debugmsg))
    {
      if (debug)
        zlog_debug ("%s: from vpn (%s) to vrf (%s), skipping: %s", __func__,
                    from_bgp->name_pretty, to_bgp->name_pretty, debugmsg);
      return false;
    }
  ...
  if (debug)
    zlog_debug ("%s: updating RD %s, %pFX to %s", __func__, rd_buf, p,
                to_bgp->name_pretty);
  ...
  old_ecom = bgp_attr_get_ecommunity (&static_attr);
  if (old_ecom &&
      CHECK_FLAG (to_bgp->af_flags[afi][safi], BGP_CONFIG_VRF_TO_VRF_IMPORT))
    {
      new_ecom = ecommunity_dup (old_ecom);
      ecommunity_strip_rts (new_ecom);
      bgp_attr_set_ecommunity (&static_attr, new_ecom);
  ...
  if (to_bgp->vpn_policy[afi].rmap[BGP_VPN_POLICY_DIR_FROMVPN])
    {
      ...
      ret = route_map_apply (
          to_bgp->vpn_policy[afi].rmap[BGP_VPN_POLICY_DIR_FROMVPN], p, &info);
      if (RMAP_DENYMATCH == ret)
        {
          bgp_attr_flush (&static_attr); /* free any added parts */
          if (debug)
  ...
  if (debug)
    zlog_debug ("%s: pfx %pBD: num_labels %d", __func__, path_vpn->net,
                num_labels);
...
bool
vpn_leak_to_vrf_update (struct bgp *from_bgp, struct bgp_path_info *path_vpn,
                        struct prefix_rd *prd)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
    {
      ...
      if (! path_vpn->extra || path_vpn->extra->bgp_orig != bgp)
        { /* no loop */
          leak_success |=
              vpn_leak_to_vrf_update_onevrf (bgp, from_bgp, path_vpn, prd);
        }
    }
  return leak_success;
}
...
void
vpn_leak_to_vrf_withdraw (struct bgp_path_info *path_vpn)
{
...
#endif
      if (debug)
        zlog_debug ("%s: path_vpn->net unexpectedly NULL, no prefix, bailing",
                    __func__);
      return;
    }
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
    {
      if (! vpn_leak_from_vpn_active (bgp, afi, &debugmsg))
        {
          if (debug)
            zlog_debug ("%s: from %s, skipping: %s", __func__,
                        bgp->name_pretty, debugmsg);
          continue;
        }
      ...
      if (debug)
        zlog_debug ("%s: withdrawing from vrf %s", __func__, bgp->name_pretty);
      ...
      for (bpi = bgp_dest_get_bgp_path_info (bn); bpi; bpi = bpi->next)
        {
          if (bpi->extra &&
              (struct bgp_path_info *) bpi->extra->parent == path_vpn)
            {
              break;
            }
        }
      ...
      if (bpi)
        {
          if (debug)
            zlog_debug ("%s: deleting bpi %p", __func__, bpi);
...
void
vpn_leak_to_vrf_withdraw_all (struct bgp *to_bgp, afi_t afi)
{
  struct bgp_dest *bn;
  struct bgp_path_info *bpi;
  safi_t safi = SAFI_UNICAST;
      ...
      for (bpi = bgp_dest_get_bgp_path_info (bn); bpi; bpi = bpi->next)
        {
          if (bpi->extra && bpi->extra->bgp_orig != to_bgp &&
              bpi->extra->parent && is_pi_family_vpn (bpi->extra->parent))
            {

              /* delete route */
              bgp_aggregate_decrement (to_bgp, bgp_dest_get_prefix (bn), bpi,
                                       afi, safi);
              bgp_path_info_delete (bn, bpi);
              bgp_process (to_bgp, bn, afi, safi);
            }
...
void
vpn_leak_to_vrf_update_all (struct bgp *to_bgp, struct bgp *vpn_from,
                            afi_t afi)
{
  struct bgp_dest *pdest;
  safi_t safi = SAFI_MPLS_VPN;
 ...
 */
static void
vpn_policy_routemap_update (struct bgp *bgp, const char *rmap_name)
{
                    ...
                    bgp->vpn_policy[afi].rmap_name[BGP_VPN_POLICY_DIR_TOVPN]))
        {
          ...
          if (debug)
            zlog_debug (
                ...
                __func__, rmap_name, bgp->as, afi2str (afi));

          vpn_leak_prechange (BGP_VPN_POLICY_DIR_TOVPN, afi,
                              bgp_get_default (), bgp);
          if (debug)
            zlog_debug ("%s: after vpn_leak_prechange", __func__);

          /* in case of definition/deletion */
          bgp->vpn_policy[afi].rmap[BGP_VPN_POLICY_DIR_TOVPN] = rmap;

          vpn_leak_postchange (BGP_VPN_POLICY_DIR_TOVPN, afi,
                               bgp_get_default (), bgp);

          if (debug)
            zlog_debug ("%s: after vpn_leak_postchange", __func__);
        }
      ...
      if (bgp->vpn_policy[afi].rmap_name[BGP_VPN_POLICY_DIR_FROMVPN] &&
              ...
              bgp->vpn_policy[afi].rmap_name[BGP_VPN_POLICY_DIR_FROMVPN]))
        {
          ...
          if (debug)
            {
              zlog_debug (
                  "%s: rmap \"%s\" matches vrf-policy fromvpn for as %d afi %s",
                  __func__, rmap_name, bgp->as, afi2str (afi));
            }
          ...
          /* in case of definition/deletion */
          bgp->vpn_policy[afi].rmap[BGP_VPN_POLICY_DIR_FROMVPN] = rmap;
 ...
 */
void
vpn_handle_router_id_update (struct bgp *bgp, bool withdraw, bool is_config)
{
  afi_t afi;
  int debug =
  ...
  if (! is_config)
    {
      if (debug)
        zlog_debug ("%s: skipping non explicit router-id change", __func__);
      return;
    }
      ...
      if (withdraw)
        {
          vpn_leak_prechange (BGP_VPN_POLICY_DIR_TOVPN, afi,
                              bgp_get_default (), bgp);
          if (debug)
            zlog_debug ("%s: %s after to_vpn vpn_leak_prechange", __func__,
                        export_name);
          ...
          for (ALL_LIST_ELEMENTS_RO (bgp->vpn_policy[afi].export_vrf, node,
                                     vname))
            {
              ...
              ecommunity_del_val (bgp_import->vpn_policy[afi].rtlist[idir],
      ...
      else
        {
           ...
           * to VPN
           */
          form_auto_rd (bgp->router_id, bgp->vrf_rd_id, &bgp->vrf_prd_auto);
          ...
          for (ALL_LIST_ELEMENTS_RO (bgp->vpn_policy[afi].export_vrf, node,
                                     vname))
            {
              ...
              if (bgp_import->vpn_policy[afi].rtlist[idir])
                bgp_import->vpn_policy[afi].rtlist[idir] = ecommunity_merge (
                    bgp_import->vpn_policy[afi].rtlist[idir], ecom);
              else
          ...
          vpn_leak_postchange (BGP_VPN_POLICY_DIR_TOVPN, afi,
                               bgp_get_default (), bgp);
          if (debug)
            zlog_debug ("%s: %s after to_vpn vpn_leak_postchange", __func__,
                        export_name);
        }
    }
}
...
void
vpn_policy_routemap_event (const char *rmap_name)
{
  int debug = BGP_DEBUG (vpn, VPN_LEAK_RMAP_EVENT);
  struct listnode *mnode, *mnnode;
  struct bgp *bgp;
...
void
vrf_import_from_vrf (struct bgp *to_bgp, struct bgp *from_bgp, afi_t afi,
                     safi_t safi)
{
  const char *export_name;
  enum vpn_policy_direction idir, edir;
  ...
  if (! listcount (from_bgp->vpn_policy[afi].export_vrf))
    ...
    {
      for (ALL_LIST_ELEMENTS_RO (from_bgp->vpn_policy[afi].export_vrf, node,
                                 tmp_name))
        {
          if (strcmp (vname, tmp_name) == 0)
            {
  ...
  if (first_export)
    {
      ...
      SET_FLAG (from_bgp->vpn_policy[afi].flags, BGP_VPN_POLICY_TOVPN_RD_SET);
      prefix_rd2str (&from_bgp->vpn_policy[afi].tovpn_rd, buf, sizeof (buf),
                     from_bgp->asnotation);
      from_bgp->vpn_policy[afi].rtlist[edir] =
          ecommunity_str2com (buf, ECOMMUNITY_ROUTE_TARGET, 0);
      SET_FLAG (from_bgp->af_flags[afi][safi], BGP_CONFIG_VRF_TO_VRF_EXPORT);
      from_bgp->vpn_policy[afi].tovpn_label = BGP_PREVENT_VRF_2_VRF_LEAK;
    }
  ecom = from_bgp->vpn_policy[afi].rtlist[edir];
  if (to_bgp->vpn_policy[afi].rtlist[idir])
    to_bgp->vpn_policy[afi].rtlist[idir] =
        ecommunity_merge (to_bgp->vpn_policy[afi].rtlist[idir], ecom);
  else
  ...
  if (debug)
    {
      ...
      from_name = from_bgp->name ? from_bgp->name : VRF_DEFAULT_NAME;

      ecom1 = ecommunity_ecom2str (to_bgp->vpn_policy[afi].rtlist[idir],
                                   ECOMMUNITY_FORMAT_ROUTE_MAP, 0);

      ecom2 = ecommunity_ecom2str (to_bgp->vpn_policy[afi].rtlist[edir],
                                   ECOMMUNITY_FORMAT_ROUTE_MAP, 0);
...
void
vrf_unimport_from_vrf (struct bgp *to_bgp, struct bgp *from_bgp, afi_t afi,
                       safi_t safi)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (to_bgp->vpn_policy[afi].import_vrf, node, vname))
    {
      if (strcmp (vname, tmp_name) == 0)
        break;
    }
  ...
  if (to_bgp->vpn_policy[afi].import_vrf->count == 0)
    {
      if (! to_bgp->vpn_policy[afi].rmap[idir])
        UNSET_FLAG (to_bgp->af_flags[afi][safi], BGP_CONFIG_VRF_TO_VRF_IMPORT);
      if (to_bgp->vpn_policy[afi].rtlist[idir])
       ...
       "Label value\n")
{
  ...
  return bgp_static_set_safi (
      ...
      argv[idx_ext_community]->arg, argv[idx_label]->arg, NULL, 0, NULL, NULL,
      NULL, NULL);
}
       ...
       "Label value\n")
{
  ...
  return bgp_static_unset_safi (
      AFI_IP, SAFI_MPLS_VPN, vty, argv[idx_ipv4_prefixlen]->arg,
      argv[idx_ext_community]->arg, argv[idx_label]->arg, 0, NULL, NULL, NULL);
}
       ...
       "route map name\n")
{
  ...
  if (argc == 8)
    ...
    return bgp_static_set_safi (
        AFI_IP6, SAFI_MPLS_VPN, vty, argv[idx_ipv6_prefix]->arg,
        argv[idx_ext_community]->arg, argv[idx_label]->arg, NULL, 0, NULL,
        NULL, NULL, NULL);
}
  ...
  if (argv_find_and_parse_afi (argv, argc, &idx, &afi))
    {
      ...
      if (argv_find (argv, argc, "rd", &idx) &&
          strcmp (argv[idx + 1]->arg, "all"))
        {
          ...
          return bgp_show_mpls_vpn (vty, afi, &prd, bgp_show_type_normal, NULL,
                                    0, use_json (argc, argv));
        }
      else
        {
          return bgp_show_mpls_vpn (vty, afi, NULL, bgp_show_type_normal, NULL,
                                    0, use_json (argc, argv));
        }
    }
  return CMD_SUCCESS;
  ...
  if (argv_find_and_parse_vpnvx (argv, argc, &idx, &afi))
    {
      ...
      return bgp_show_mpls_vpn (vty, afi, &prd, bgp_show_type_normal, NULL, 0,
                                0);
    }
  return CMD_SUCCESS;
}
       ...
       BGP_VPNVX_HELP_STR)
{
  ...
  if (argv_find_and_parse_vpnvx (argv, argc, &idx, &afi))
    return bgp_show_mpls_vpn (vty, afi, NULL, bgp_show_type_normal, NULL, 0,
                              0);
  return CMD_SUCCESS;
}
       ...
       "Display BGP tags for prefixes\n")
{
  ...
  if (argv_find_and_parse_vpnvx (argv, argc, &idx, &afi))
    return bgp_show_mpls_vpn (vty, afi, NULL, bgp_show_type_normal, NULL, 1,
                              0);
  return CMD_SUCCESS;
}
  ...
  if (argv_find_and_parse_vpnvx (argv, argc, &idx, &afi))
    {
      ...
      return bgp_show_mpls_vpn (vty, afi, &prd, bgp_show_type_normal, NULL, 1,
                                0);
    }
  return CMD_SUCCESS;
}
  ...
  if (argv_find_and_parse_vpnvx (argv, argc, &idx, &afi))
    {
      ...
      return bgp_show_mpls_vpn (vty, afi, NULL, bgp_show_type_neighbor, &su, 0,
                                uj);
    }
  return CMD_SUCCESS;
}
  ...
  if (argv_find_and_parse_vpnvx (argv, argc, &idx, &afi))
    {
      ...
      if (prefix_rd_all)
        return bgp_show_mpls_vpn (vty, afi, NULL, bgp_show_type_neighbor, &su,
                                  0, uj);
      else
        return bgp_show_mpls_vpn (vty, afi, &prd, bgp_show_type_neighbor, &su,
                                  0, uj);
    }
  return CMD_SUCCESS;
}
...
vrf_id_t
get_first_vrf_for_redirect_with_rt (struct ecommunity *eckey)
{
  struct listnode *mnode, *mnnode;
  struct bgp *bgp;
  afi_t afi = AFI_IP;
  ...
  for (ALL_LIST_ELEMENTS_RO (bm->bgp, next, bgp))
    {
      ...
      vpn_leak_postchange (BGP_VPN_POLICY_DIR_TOVPN, AFI_IP, bgp_default, bgp);

      vpn_leak_postchange (BGP_VPN_POLICY_DIR_TOVPN, AFI_IP6, bgp_default,
                           bgp);
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (bm->bgp, next, bgp))
    {
      ...
      vpn_leak_postchange (BGP_VPN_POLICY_DIR_FROMVPN, AFI_IP, bgp_default,
                           bgp);

      vpn_leak_postchange (BGP_VPN_POLICY_DIR_FROMVPN, AFI_IP6, bgp_default,
                           bgp);
    }
}
 ...
 */
void
bgp_vpn_leak_unimport (struct bgp *from_bgp)
{
  struct bgp *to_bgp;
  const char *tmp_name;
  char *vname;
      ...
      for (ALL_LIST_ELEMENTS_RO (bm->bgp, next, to_bgp))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (to_vpolicy->import_vrf, node, vname))
            {
              if (strcmp (vname, tmp_name) == 0)
                {
          ...
          if (debug)
            zlog_debug (
                "%s: unimport routes from %s to_bgp %s afi %s import vrfs count %u",
                __func__, from_bgp->name_pretty, to_bgp->name_pretty,
                afi2str (afi), to_vpolicy->import_vrf->count);
           ...
           */
          for (ALL_LIST_ELEMENTS_RO (to_bgp->vpn_policy[afi].export_vrf, node,
                                     vname))
            {
              if (strcmp (vname, tmp_name) == 0)
                {
                  vrf_unimport_from_vrf (from_bgp, to_bgp, afi, safi);
                  break;
                }
            }
        }
 ...
 */
void
bgp_vpn_leak_export (struct bgp *from_bgp)
{
  afi_t afi;
  const char *export_name;
  char *vname;
          ...
          for (ALL_LIST_ELEMENTS_RO (to_vpolicy->import_vrf, node, vname))
            {
              ...
              if (debug)
                zlog_debug (
                    "%s: found from_bgp %s in to_bgp %s import list, import routes.",
                    __func__, export_name, to_bgp->name_pretty);
               ...
               */
              if (ecom)
                ecommunity_del_val (to_vpolicy->rtlist[idir],
                                    (struct ecommunity_val *) ecom->val);
              vrf_import_from_vrf (to_bgp, from_bgp, afi, safi);
              break;
            }
        }
    }
