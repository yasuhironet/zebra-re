 ...
 */
int
bgp_maximum_paths_set (struct bgp *bgp, afi_t afi, safi_t safi, int peertype,
 ...
 */
int
bgp_maximum_paths_unset (struct bgp *bgp, afi_t afi, safi_t safi, int peertype)
{
  if (! bgp || (afi >= AFI_MAX) || (safi >= SAFI_MAX))
    return -1;
 ...
 */
static int
bgp_interface_same (struct interface *ifp1, struct interface *ifp2)
{
  if (! ifp1 && ! ifp2)
    return 1;
 ...
 */
int
bgp_path_info_nexthop_cmp (struct bgp_path_info *bpi1,
  ...
  if (! compare)
    {
      if (bpi1->attr->mp_nexthop_len == bpi2->attr->mp_nexthop_len)
        {
          switch (bpi1->attr->mp_nexthop_len)
            ...
            case BGP_ATTR_NHLEN_VPNV4:
              compare = IPV4_ADDR_CMP (&bpi1->attr->mp_nexthop_global_in,
            ...
            case BGP_ATTR_NHLEN_IPV6_GLOBAL_AND_LL:
              ...
              if (! bpi1->attr->mp_nexthop_prefer_global &&
                ...
                compare =
                    ! bgp_interface_same (bpi1->peer->ifp, bpi2->peer->ifp);
       ...
       */
      else if (bpi1->attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV6_GLOBAL ||
               bpi1->attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV6_GLOBAL_AND_LL)
        {
          compare = IPV6_ADDR_CMP (&bpi1->attr->mp_nexthop_global,
                                   &bpi2->attr->mp_nexthop_global);
          if (! compare)
  ...
  if (! compare && bpi1->attr->nh_type != NEXTHOP_TYPE_BLACKHOLE)
    {
      if (bpi1->extra && bpi1->extra->bgp_orig && bpi2->extra &&
          bpi2->extra->bgp_orig)
        {
          if (bpi1->extra->bgp_orig->vrf_id != bpi2->extra->bgp_orig->vrf_id)
            {
              compare = 1;
            }
        }
 ...
 */
static int
bgp_path_info_mpath_cmp (void *val1, void *val2)
{
  ...
  if (! compare)
    {
      if (! bpi1->peer->su_remote && ! bpi2->peer->su_remote)
        ...
        compare = sockunion_cmp (bpi1->peer->su_remote, bpi2->peer->su_remote);
    }
 ...
 */
void
bgp_mp_list_add (struct list *mp_list, struct bgp_path_info *mpinfo)
{
...
bgp_path_info_mpath_new (void)
{
  ...
  new_mpath =
      XCALLOC (MTYPE_BGP_MPATH_INFO, sizeof (struct bgp_path_info_mpath));
 ...
 */
void
bgp_path_info_mpath_free (struct bgp_path_info_mpath **mpath)
{
  if (mpath && *mpath)
    {
      if ((*mpath)->mp_attr)
 ...
 */
static void
bgp_path_info_mpath_enqueue (struct bgp_path_info *prev_info,
 ...
 */
void
bgp_path_info_mpath_dequeue (struct bgp_path_info *path)
{
 ...
 */
uint32_t
bgp_path_info_mpath_count (struct bgp_path_info *path)
{
  if (! path->mpath)
    return 0;
 ...
 */
static void
bgp_path_info_mpath_count_set (struct bgp_path_info *path, uint16_t count)
{
  struct bgp_path_info_mpath *mpath;
  if (! count && ! path->mpath)
    return;
 ...
 */
static void
bgp_path_info_mpath_lb_update (struct bgp_path_info *path, bool set,
 ...
 */
struct attr *
bgp_path_info_mpath_attr (struct bgp_path_info *path)
{
  if (! path->mpath)
    return NULL;
 ...
 */
bool
bgp_path_info_mpath_chkwtd (struct bgp *bgp, struct bgp_path_info *path)
{
 ...
 */
uint64_t
bgp_path_info_mpath_cumbw (struct bgp_path_info *path)
{
  if (! path->mpath)
    return 0;
 ...
 */
static void
bgp_path_info_mpath_attr_set (struct bgp_path_info *path, struct attr *attr)
{
  struct bgp_path_info_mpath *mpath;
  if (! attr && ! path->mpath)
    return;
                            ...
                            struct bgp_maxpaths_cfg *mpath_cfg)
{
  ...
  if (debug)
    zlog_debug (
        ...
        mp_list ? listcount (mp_list) : 0, old_mpath_count, old_cum_bw);
  ...
  while (mp_node || cur_mpath)
    {
      ...
      next_mpath = cur_mpath ? bgp_path_info_mpath_next (cur_mpath) : NULL;
      tmp_info = mp_node ? listgetdata (mp_node) : NULL;

      if (debug)
        zlog_debug ("%pRN(%s): comparing candidate %s with existing mpath %s",
                    bgp_dest_to_rnode (dest), bgp->name_pretty,
      ...
      if (mp_node && (listgetdata (mp_node) == cur_mpath))
        {
          ...
          if ((mpath_count < maxpaths) && prev_mpath &&
              bgp_path_info_nexthop_cmp (prev_mpath, cur_mpath))
            {
              bgp_path_info_mpath_enqueue (prev_mpath, cur_mpath);
              prev_mpath = cur_mpath;
              mpath_count++;
              if (ecommunity_linkbw_present (
                      bgp_attr_get_ecommunity (cur_mpath->attr), &bwval))
                cum_bw += bwval;
              else
                all_paths_lb = false;
              if (debug)
                {
                  bgp_path_info_path_with_addpath_rx_str (cur_mpath, path_buf,
                                                          sizeof (path_buf));
                  zlog_debug ("%pRN: %s is still multipath, cur count %d",
                              bgp_dest_to_rnode (dest), path_buf, mpath_count);
                }
            }
          else
            {
      ...
      if (cur_mpath &&
          (! mp_node ||
           (bgp_path_info_mpath_cmp (cur_mpath, listgetdata (mp_node)) < 0)))
        {
          /*
          ...
          if (debug)
            {
              bgp_path_info_path_with_addpath_rx_str (cur_mpath, path_buf,
                                                      sizeof (path_buf));
              zlog_debug ("%pRN: remove mpath %s nexthop %pI4, cur count %d",
                          bgp_dest_to_rnode (dest), path_buf,
                          &cur_mpath->attr->nexthop, mpath_count);
            }
          cur_mpath = next_mpath;
      ...
      else
        {
          ...
          if ((mpath_count < maxpaths) && (new_mpath != new_best) &&
              bgp_path_info_nexthop_cmp (prev_mpath, new_mpath))
            {
              bgp_path_info_mpath_dequeue (new_mpath);

              bgp_path_info_mpath_enqueue (prev_mpath, new_mpath);
              prev_mpath = new_mpath;
              mpath_changed = 1;
              mpath_count++;
              if (ecommunity_linkbw_present (
                      bgp_attr_get_ecommunity (new_mpath->attr), &bwval))
                cum_bw += bwval;
              else
                all_paths_lb = false;
              if (debug)
                {
                  bgp_path_info_path_with_addpath_rx_str (new_mpath, path_buf,
                                                          sizeof (path_buf));
                  zlog_debug ("%pRN: add mpath %s nexthop %pI4, cur count %d",
                              bgp_dest_to_rnode (dest), path_buf,
                              &new_mpath->attr->nexthop, mpath_count);
                }
            }
          mp_node = mp_next_node;
        }
  ...
  if (new_best)
    {
      ...
      if (debug)
        zlog_debug (
            ...
            bgp_dest_to_rnode (dest), bgp->name_pretty, mpath_count,
            mpath_changed ? "YES" : "NO", all_paths_lb, cum_bw);

      if (mpath_changed ||
          (bgp_path_info_mpath_count (new_best) != old_mpath_count))
        SET_FLAG (new_best->flags, BGP_PATH_MULTIPATH_CHG);
      if ((mpath_count - 1) != old_mpath_count || old_cum_bw != cum_bw)
 ...
 */
void
bgp_mp_dmed_deselect (struct bgp_path_info *dmed_best)
{
  ...
  for (mpinfo = bgp_path_info_mpath_first (dmed_best); mpinfo; mpinfo = mpnext)
    {
                                      ...
                                      struct bgp_path_info *old_best)
{
  ...
  if (new_best->peer &&
      CHECK_FLAG (new_best->peer->bgp->flags, BGP_FLAG_MULTIPATH_RELAX_AS_SET))
    {

      /* aggregate attribute from multipath constituents */
      aspath = aspath_dup (attr.aspath);
      origin = attr.origin;
      community = bgp_attr_get_community (&attr)
              ...
              if (community)
                {
                  commerge = community_merge (
                      community, bgp_attr_get_community (mpinfo->attr));
                  community = community_uniq_sort (commerge);
                  community_free (&commerge);
                }
              else
                community =
                    community_dup (bgp_attr_get_community (mpinfo->attr));
            }
              ...
              if (ecomm)
                {
                  ecommerge = ecommunity_merge (
                      ecomm, bgp_attr_get_ecommunity (mpinfo->attr));
                  ecomm = ecommunity_uniq_sort (ecommerge);
                  ecommunity_free (&ecommerge);
                }
              else
                ecomm =
                    ecommunity_dup (bgp_attr_get_ecommunity (mpinfo->attr));
            }
              ...
              if (lcomm)
                {
                  lcommerge = lcommunity_merge (
                      lcomm, bgp_attr_get_lcommunity (mpinfo->attr));
                  lcomm = lcommunity_uniq_sort (lcommerge);
                  lcommunity_free (&lcommerge);
                }
              else
                lcomm =
                    lcommunity_dup (bgp_attr_get_lcommunity (mpinfo->attr));
            }
        }
