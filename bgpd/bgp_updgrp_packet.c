  ...
  if (nhafi == AFI_IP)
    {
      ...
      route_map_sets_nh =
          (CHECK_FLAG (vec->flags, BPKT_ATTRVEC_FLAGS_RMAP_IPV4_NH_CHANGED) ||
           CHECK_FLAG (vec->flags, BPKT_ATTRVEC_FLAGS_RMAP_VPNV4_NH_CHANGED) ||
      ...
      switch (nhlen)
        ...
        default:
          ...
          flog_warn (EC_BGP_INVALID_NEXTHOP_LENGTH,
      ...
      if (route_map_sets_nh &&
          ((peer->sort != BGP_PEER_EBGP) || ROUTE_MAP_OUT (filter)))
        {
          if (CHECK_FLAG (vec->flags, BPKT_ATTRVEC_FLAGS_RMAP_NH_PEER_ADDRESS))
            {
      ...
      if (bgp_debug_update (peer, NULL, NULL, 0))
        {
          if (nhlen == BGP_ATTR_NHLEN_IPV6_GLOBAL_AND_LL ||
            ...
            zlog_debug ("u%" PRIu64 ":s%" PRIu64
                        ...
                        PAF_SUBGRP (paf)->id, peer->host, mod_v6nhg, mod_v6nhl,
                        (nhlen == BGP_ATTR_NHLEN_VPNV6_GLOBAL_AND_LL
                             ? " and RD"
                             : ""));
          else
            zlog_debug (
                ...
                PAF_SUBGRP (paf)->update_group->id, PAF_SUBGRP (paf)->id,
                peer->host, mod_v6nhg,
                (nhlen == BGP_ATTR_NHLEN_VPNV6_GLOBAL ? " and RD" : ""));
        }
    }
  ...
  while (adv)
    {
      ...
      if (stream_empty (s))
        {
          ...
          total_attr_len = bgp_packet_attribute (
              NULL, peer, s, adv->baa->attr, &vecarr, NULL, afi, safi, from,
              NULL, NULL, 0, 0, 0, path);
          ...
          if (space_remaining < space_needed)
            {
              flog_err (EC_BGP_UPDGRP_ATTR_LEN,
                        "u%" PRIu64 ":s%" PRIu64
      ...
      if ((afi == AFI_IP && safi == SAFI_UNICAST) &&
      ...
      else
        {
          ...
          if (dest->pdest)
            prd = (struct prefix_rd *) bgp_dest_get_prefix (dest->pdest);
      ...
      if (bgp_debug_update (NULL, dest_p, subgrp->update_group, 0))
        {
          ...
          bgp_debug_rdpfxpath2str (afi, safi, prd, dest_p, label_pnt,
                                   num_labels, addpath_capable, addpath_tx_id,
                                   &adv->baa->attr->evpn_overlay, pfx_buf,
                                   sizeof (pfx_buf));
  ...
  while ((adv = bgp_adv_fifo_first (&subgrp->sync->withdraw)) != NULL)
    {
      ...
      space_remaining = STREAM_WRITEABLE (s) - BGP_MAX_PACKET_SIZE_OVERFLOW;
      space_needed = BGP_NLRI_LENGTH + addpath_overhead + BGP_TOTAL_ATTR_LEN +
                     bgp_packet_mpattr_prefix_size (afi, safi, dest_p);
      ...
      if (afi == AFI_IP && safi == SAFI_UNICAST &&
      ...
      else
        {
          ...
          bgp_packet_mpunreach_prefix (s, dest_p, afi, safi, prd, NULL, 0,
                                       addpath_capable, addpath_tx_id, NULL);
        }
                                ...
                                struct attr *attr, struct peer *from)
{
  ...
  stream_putw (s, 0);
  total_attr_len =
      bgp_packet_attribute (NULL, peer, s, attr, &vecarr, &p, afi, safi, from,
  ...
  if (p.family == AF_INET && safi == SAFI_UNICAST &&
      ! peer_cap_enhe (peer, afi, safi))
    stream_put_prefix_addpath (s, &p, addpath_capable,
                               BGP_ADDPATH_TX_ID_FOR_DEFAULT_ORIGINATE);
...
subgroup_default_withdraw_packet (struct update_subgroup *subgrp)
{
  ...
  if (CHECK_FLAG (subgrp->sflags, SUBGRP_STATUS_PEER_DEFAULT_ORIGINATED))
    {
      subgrp->scount--;
      UNSET_FLAG (subgrp->sflags, SUBGRP_STATUS_PEER_DEFAULT_ORIGINATED);
    }
}
...
bpacket_vec_arr_inherit_attr_flags (struct bpacket_attr_vec_arr *vecarr,
                                    ...
                                    struct attr *attr)
{
  if (CHECK_FLAG (attr->rmap_change_flags, BATTR_RMAP_NEXTHOP_PEER_ADDRESS))
  ...
  if (CHECK_FLAG (attr->rmap_change_flags, BATTR_RMAP_IPV6_LL_NHOP_CHANGED))
    SET_FLAG (vecarr->entries[BGP_ATTR_VEC_NH].flags,
