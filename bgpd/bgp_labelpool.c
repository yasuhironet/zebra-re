...
lp_cbq_docallback (struct work_queue *wq, void *data)
{
  ...
  if (lcbq->label == MPLS_LABEL_NONE)
    {
      /* shouldn't happen */
      flog_err (EC_BGP_LABEL, "%s: error: label==MPLS_LABEL_NONE", __func__);
      return WQ_SUCCESS;
    }
  ...
  if (lcbq->allocated && rc)
    {
      ...
      if (debug)
        zlog_debug (
            "%s: callback rejected allocation, releasing labelid=%p label=%u",
...
static void
lp_cbq_item_free (struct work_queue *wq, void *data)
{
  XFREE (MTYPE_BGP_LABEL_CBQ, data);
}

static void
lp_lcb_free (void *goner)
{
  XFREE (MTYPE_BGP_LABEL_CB, goner);
}

static void
lp_chunk_free (void *goner)
{
...
void
bgp_lp_init (struct event_loop *master, struct labelpool *pool)
{
...
static void
check_bgp_lu_cb_unlock (struct lp_lcb *lcb)
{
...
static void
check_bgp_lu_cb_lock (struct lp_lcb *lcb)
{
  if (lcb->type == LP_TYPE_BGP_LU)
  ...
  for (ALL_LIST_ELEMENTS_RO (lp->chunks, node, chunk))
    {
      ...
      if (debug)
        zlog_debug ("%s: chunk first=%u last=%u", __func__, chunk->first,
                    chunk->last);
      ...
      if (skiplist_insert (lp->inuse, (void *) lbl, labelid))
        {
          ...
          zlog_err ("%s: unable to insert inuse label %u (id %p)", __func__,
 ...
 */
static struct lp_lcb *
lcb_alloc (int type, void *labelid,
 ...
 */
void
bgp_lp_get (int type, void *labelid,
            int (*cbfunc) (mpls_label_t label, void *labelid, bool allocated))
{
  struct lp_lcb *lcb;
  int requested = 0;
  ...
  if (! skiplist_search (lp->ledger, labelid, (void **) &lcb))
  ...
  else
    {
      ...
      if (debug)
        zlog_debug ("%s: inserting lcb=%p label=%u", __func__, lcb,
                    lcb->label);
      ...
      if (rc)
        {
          /* shouldn't happen */
          flog_err (EC_BGP_LABEL, "%s: can't insert new LCB into ledger list",
                    __func__);
  ...
  if (debug)
    zlog_debug ("%s: slow path. lcb=%p label=%u", __func__, lcb, lcb->label);
  ...
  struct lp_fifo *lf = XCALLOC (MTYPE_BGP_LABEL_FIFO, sizeof (struct lp_fifo));
...
void
bgp_lp_release (int type, void *labelid, mpls_label_t label)
{
  struct lp_lcb *lcb;
          ...
          for (ALL_LIST_ELEMENTS_RO (lp->chunks, node, chunk))
            {
              ...
              assert (bf_test_index (chunk->allocated_map, index));
 ...
 */
void
bgp_lp_event_chunk (uint8_t keep, uint32_t first, uint32_t last)
{
  struct lp_chunk *chunk;
  int debug = BGP_DEBUG (labelpool, LABELPOOL);
  struct lp_fifo *lf;
  ...
  if (last < first)
    {
      flog_err (EC_BGP_LABEL,
                "%s: zebra label chunk invalid: first=%u, last=%u", __func__,
                first, last);
      return;
    }
  ...
  while (labelcount && (lf = lp_fifo_first (&lp->requests)))
    {
      ...
      if (lcb->label != MPLS_LABEL_NONE)
        {
          ...
          if (debug)
            {
              zlog_debug (
                  "%s: labelid %p: request already has a label: %u=0x%x, lcb=%p",
                  __func__, labelid, lcb->label, lcb->label, lcb);
            }
          /* if this was a BGP_LU request, unlock node
           */
          check_bgp_lu_cb_unlock (lcb);
      ...
      if (lcb->label == MPLS_LABEL_NONE)
        {
          ...
          if (debug)
            {
              zlog_debug ("%s: out of labels, await more", __func__);
            }
          break;
        }
       ...
       */
      struct lp_cbq_item *q =
          XCALLOC (MTYPE_BGP_LABEL_CBQ, sizeof (struct lp_cbq_item));
      ...
      if (debug)
        zlog_debug ("%s: assigning label %u to labelid %p", __func__, q->label,
                    q->labelid);
...
DEFUN (show_bgp_labelpool_summary,
  ...
  if (uj)
    {
      ...
      json_object_int_add (json, "requests", lp_fifo_count (&lp->requests));
...
DEFUN (show_bgp_labelpool_ledger,
      ...
      switch (lcb->type)
        ...
        case LP_TYPE_BGP_LU:
          if (! CHECK_FLAG (dest->flags, BGP_NODE_LABEL_REQUESTED))
            if (uj)
              {
                json_object_string_add (json_elem, "prefix", "INVALID");
                json_object_int_add (json_elem, "label", lcb->label);
              }
            else
              vty_out (vty, "%-18s         %u\n", "INVALID", lcb->label);
          else
            {
              ...
              if (uj)
                {
                  json_object_string_addf (json_elem, "prefix", "%pFX", p);
                  json_object_int_add (json_elem, "label", lcb->label);
                }
              else
                vty_out (vty, "%-18pFX    %u\n", p, lcb->label);
        ...
        case LP_TYPE_VRF:
          if (uj)
            {
              json_object_string_add (json_elem, "prefix", "VRF");
              json_object_int_add (json_elem, "label", lcb->label);
            }
          else
            vty_out (vty, "%-18s         %u\n", "VRF", lcb->label);
        ...
        case LP_TYPE_NEXTHOP:
          if (uj)
            {
              json_object_string_add (json_elem, "prefix", "nexthop");
              json_object_int_add (json_elem, "label", lcb->label);
            }
          else
            vty_out (vty, "%-18s         %u\n", "nexthop", lcb->label);
...
DEFUN (show_bgp_labelpool_inuse,
       ...
       JSON_STR)
{
  ...
  for (rc = skiplist_next (lp->inuse, (void **) &label, (void **) &dest,
                           &cursor);
       ! rc; rc = skiplist_next (lp->ledger, (void **) &label, (void **) &dest,
                                 &cursor))
    {
...
DEFUN (show_bgp_labelpool_requests,
...
DEFUN (show_bgp_labelpool_chunks,
      ...
      if (uj)
        {
          ...
          json_object_int_add (json_elem, "numberFree", chunk->nfree);
        }
      else
        vty_out (vty, "%10u %10u %10u %10u\n", chunk->first, chunk->last, size,
                 chunk->nfree);
    }
  if (uj)
    vty_json (vty, json);
...
static void
show_bgp_nexthop_label_afi (struct vty *vty, afi_t afi, struct bgp *bgp,
                            bool detail)
{
  ...
  frr_each (bgp_label_per_nexthop_cache, tree, iter)
    {
      ...
      vty_out (vty, " %s, label %s #paths %u\n",
               ...
               mpls_label2str (1, &iter->label, labelstr, sizeof (labelstr), 0,
                               true),
               iter->path_count);
      if (iter->nh)
        vty_out (vty, "  if %s\n",
                 ifindex2ifname (iter->nh->ifindex, iter->nh->vrf_id));
...
static int
test_cb (mpls_label_t label, void *labelid, bool allocated)
{
      ...
      if (! (tcb->counter[LPT_STAT_ALLOCATED] % LPT_TS_INTERVAL))
        {
          ...
          skiplist_insert (
              tcb->timestamps_alloc,
              (void *) (uintptr_t) tcb->counter[LPT_STAT_ALLOCATED],
              (void *) time_ms);
        }
      if (skiplist_insert (tcb->labels, labelid, (void *) (uintptr_t) label))
        {
...
static void
labelpool_test_event_handler (struct event *thread)
{
  struct lp_test *tcb;
...
static void
lptest_print_stats (struct vty *vty, struct lp_test *tcb)
{
  unsigned int i;
  ...
  for (i = 0; i < LPT_STAT_MAX; ++i)
    {
      vty_out (vty, "%20s: %10u\n", lpt_counter_names[i], tcb->counter[i]);
    }
  vty_out (vty, "\n");
  ...
  if (tcb->timestamps_alloc)
    {
      ...
      cursor = NULL;
      while (! skiplist_next (tcb->timestamps_alloc, &Key, &Value, &cursor))
        {
...
DEFPY (show_labelpool_perf_test,
...
DEFPY (stop_labelpool_perf_test,
...
DEFPY (clear_labelpool_perf_test,
                                    ...
                                    "label fraction denominator\n")
{
  ...
  if (lpt_inprogress && (testnum == lpt_generation))
    {
      vty_out (vty, "Error: Test %lu is still in progress (stop first)\n",
               testnum);
      return CMD_SUCCESS;
    }

  if (skiplist_search (lp_tests, (void *) (uintptr_t) testnum, (void **) &tcb))
    {
...
void
bgp_label_per_nexthop_free (struct bgp_label_per_nexthop_cache *blnc)
{
  if (blnc->label != MPLS_INVALID_LABEL)
    {
      bgp_zebra_send_nexthop_label (ZEBRA_MPLS_LABELS_DELETE, blnc->label,
...
void
bgp_label_per_nexthop_init (void)
{
