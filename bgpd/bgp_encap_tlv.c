    ...
    struct bgp_tea_subtlv_encap_l2tpv3_over_ip *st)
{
  ...
  new =
      XCALLOC (MTYPE_ENCAP_TLV, sizeof (struct bgp_attr_encap_subtlv) + total);
  assert (new);
    ...
    struct attr *attr)
{
  ...
  ENC_SUBTLV (BGP_TEA_SUBTLV_REMOTE_ENDPOINT, subtlv_encode_remote_endpoint,
              st_endpoint);
}
    ...
    struct attr *attr)
{
  ...
  tlv = XCALLOC (MTYPE_ENCAP_TLV, sizeof (struct bgp_attr_encap_subtlv) + 12);
...
/* rfc5512 4.1 */
static int
subtlv_decode_encap_gre (struct bgp_attr_encap_subtlv *subtlv,
...
static int
subtlv_decode_encap_pbb (struct bgp_attr_encap_subtlv *subtlv,
...
/* rfc5512 4.2 */
static int
subtlv_decode_proto_type (struct bgp_attr_encap_subtlv *subtlv,
...
/* rfc5512 4.3 */
static int
subtlv_decode_color (struct bgp_attr_encap_subtlv *subtlv,
...
/* rfc 5566 4. */
static int
subtlv_decode_ipsec_ta (struct bgp_attr_encap_subtlv *subtlv,
                               ...
                               struct bgp_tea_subtlv_remote_endpoint *st)
{
  ...
  if (subtlv->length != 8 && subtlv->length != 20)
    {
      zlog_debug ("%s, subtlv length %d does not equal 8 or 20", __func__,
                  subtlv->length);
      return -1;
    }
  if (subtlv->length == 8)
    {
      st->family = AF_INET;
      memcpy (&st->ip_address.v4.s_addr, subtlv->value, IPV4_MAX_BYTELEN);
    }
  else
    {
      st->family = AF_INET6;
      memcpy (&(st->ip_address.v6.s6_addr), subtlv->value, IPV6_MAX_BYTELEN);
    }
  i = subtlv->length - 4;
...
int
tlv_to_bgp_encap_type_vxlan (struct bgp_attr_encap_subtlv *stlv,
...
int
tlv_to_bgp_encap_type_nvgre (struct bgp_attr_encap_subtlv *stlv,
...
int
tlv_to_bgp_encap_type_mpls (struct bgp_attr_encap_subtlv *stlv,
