  ...
  frr_with_mutex (&peer->io_mtx)
    {
      ...
      if (CHECK_FLAG (peer->flags, PEER_FLAG_TIMER))
        holdtime =
            atomic_load_explicit (&peer->holdtime, memory_order_relaxed);
      else
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      if (peer->afc_nego[afi][safi] &&
        ...
        {
          if (bgp_debug_neighbor_events (peer))
            zlog_debug ("   afi %d safi %d didn't receive EOR", afi, safi);
          return;
        }
    }
 ...
 */
static void
bgp_process_pending_refresh (struct peer *peer, afi_t afi, safi_t safi)
{
  if (CHECK_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_REFRESH_PENDING))
    {
      UNSET_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_REFRESH_PENDING);
 ...
 */
void
bgp_generate_updgrp_packets (struct event *thread)
{
  ...
  wpq = atomic_load_explicit (&peer->bgp->wpkt_quanta, memory_order_relaxed);
                      ...
                      if (BGP_SEND_EOR (peer->bgp, afi, safi))
                        {
                           ...
                           * send time
                           */
                          peer->eor_stime[afi][safi] = monotime (NULL);

                          BGP_UPDATE_EOR_PKT (peer, afi, safi, s);
                          bgp_process_pending_refresh (peer, afi, safi);
                        }
                    }
                }
              continue;
  ...
  while (s && (++generated < wpq) && (peer->obuf->count <= bm->outq_limit));
...
bgp_open_send (struct peer *peer)
{
  ...
  stream_putw (s,
               (local_as <= BGP_AS_MAX) ? (uint16_t) local_as : BGP_AS_TRANS);
  ...
  if (CHECK_FLAG (peer->flags, PEER_FLAG_EXTENDED_OPT_PARAMS))
  ...
  else
    {
      ...
      stream_copy (tmp, s);
      if (bgp_open_capability (tmp, peer, false) > BGP_OPEN_NON_EXT_OPT_LEN)
        {
          stream_free (tmp);
          (void) bgp_open_capability (s, peer, true);
        }
 ...
 */
static uint8_t *
bgp_notify_encapsulate_hard_reset (uint8_t code, uint8_t subcode,
...
bool
bgp_has_graceful_restart_notification (struct peer *peer)
{
 ...
 */
bool
bgp_notify_send_hard_reset (struct peer *peer, uint8_t code, uint8_t subcode)
{
  ...
  if (code == BGP_NOTIFY_CEASE)
    {
      switch (subcode)
        ...
        case BGP_NOTIFY_CEASE_ADMIN_RESET:
           ...
           */
          if (CHECK_FLAG (peer->bgp->flags, BGP_FLAG_HARD_ADMIN_RESET))
            return true;
          else
            return false;
        default:
 ...
 */
bool
bgp_notify_received_hard_reset (struct peer *peer, uint8_t code,
                                uint8_t subcode)
{
  /* When the "N" bit has been exchanged, a Hard Reset message is used to
   * indicate to the peer that the session is to be fully terminated.
 ...
 */
static void
bgp_notify_send_internal (struct peer *peer, uint8_t code, uint8_t sub_code,
                          uint8_t *data, size_t datalen, bool use_curr)
{
  struct stream *s;
  ...
  if (hard_reset)
    {
      uint8_t *hard_reset_message =
          bgp_notify_encapsulate_hard_reset (code, sub_code, data, datalen);
  ...
  /* For debug */
  {
    ...
    if (bgp_notify.length && data)
      {
        bgp_notify.data =
            XMALLOC (MTYPE_BGP_NOTIFICATION, bgp_notify.length * 3);
        for (i = 0; i < bgp_notify.length; i++)
          if (first)
            {
              snprintf (c, sizeof (c), " %02x", data[i]);

              strlcat (bgp_notify.data, c, bgp_notify.length);
            }
          else
            {
              first = 1;
              ...
              strlcpy (bgp_notify.data, c, bgp_notify.length);
            }
      }
        ...
        if (data)
          {
            ...
            peer->notify.data = XCALLOC (MTYPE_BGP_NOTIFICATION, datalen);
 ...
 */
void
bgp_notify_send (struct peer *peer, uint8_t code, uint8_t sub_code)
{
  bgp_notify_send_internal (peer, code, sub_code, NULL, 0, true);
}

/*
 * Enqueue notification; called from the main pthread, peer object access is
 * ok.
 */
void
bgp_notify_send_with_data (struct peer *peer, uint8_t code, uint8_t sub_code,
                           uint8_t *data, size_t datalen)
{
 ...
 */
void
bgp_notify_io_invalid (struct peer *peer, uint8_t code, uint8_t sub_code,
                       uint8_t *data, size_t datalen)
{
                        ...
                        uint8_t subtype)
{
  ...
  if (bgp_debug_neighbor_events (peer))
    {
      if (! orf_refresh)
        zlog_debug ("%pBP sending REFRESH_REQ for afi/safi: %s/%s", peer,
                    iana_afi2str (pkt_afi), iana_safi2str (pkt_safi));
    }
  ...
  if (capability_code == CAPABILITY_CODE_MP)
    {
      ...
      if (bgp_debug_neighbor_events (peer))
        zlog_debug (
            ...
            peer, action == CAPABILITY_ACTION_SET ? "Advertising" : "Removing",
            iana_afi2str (pkt_afi), iana_safi2str (pkt_safi));
    }
  ...
  else
    {
      if (ntohl (peer->local_id.s_addr) == ntohl (remote_id.s_addr) &&
        ...
        flog_err (EC_BGP_ROUTER_ID_SAME,
                  "Peer's router-id %pI4 is the same as ours", &remote_id);
...
bgp_open_receive (struct peer *peer, bgp_size_t size)
{
      ...
      if (opttype == BGP_OPEN_NON_EXT_OPT_TYPE_EXTENDED_LENGTH)
        {
          ...
          SET_FLAG (peer->sflags, PEER_STATUS_EXT_OPT_PARAMS_LENGTH);
        }
    }
  ...
  if (optlen != 0)
    {
      ...
      if (STREAM_READABLE (peer->curr) < optlen)
        {
          flog_err (EC_BGP_PKT_OPEN, "%s: stream has not enough bytes (%u)",
                    peer->host, optlen);
          bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR,
                           BGP_NOTIFY_OPEN_MALFORMED_ATTR);
          return BGP_Stop;
  ...
  if (remote_as == BGP_AS_ZERO)
    {
      flog_err (EC_BGP_PKT_OPEN, "%s bad OPEN, got AS set to 0", peer->host);
  ...
  if (remote_id.s_addr == INADDR_ANY ||
    ...
    {
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s bad OPEN, wrong router identifier %pI4", peer->host,
                    &remote_id);
      bgp_notify_send_with_data (peer, BGP_NOTIFY_OPEN_ERR,
  ...
  if (peer->as_type == AS_UNSPECIFIED)
    {
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s bad OPEN, remote AS is unspecified currently",
                    peer->host);
      bgp_notify_send_with_data (peer, BGP_NOTIFY_OPEN_ERR,
                                 BGP_NOTIFY_OPEN_BAD_PEER_AS,
    ...
    {
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s bad OPEN, remote AS is %u, expected %u", peer->host,
                    remote_as, peer->as);
      bgp_notify_send_with_data (peer, BGP_NOTIFY_OPEN_ERR,
                                 BGP_NOTIFY_OPEN_BAD_PEER_AS,
                                 notify_data_remote_as, 2);
  ...
  if (bgp_getsockname (peer) < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "%s: bgp_getsockname() failed for peer: %s",
                    __func__, peer->host);
      return BGP_Stop;
    }
  ...
  if (! mp_capability ||
      CHECK_FLAG (peer->flags, PEER_FLAG_OVERRIDE_CAPABILITY))
    {
      ...
      peer->afc_nego[AFI_IP][SAFI_FLOWSPEC] = peer->afc[AFI_IP][SAFI_FLOWSPEC];
      ...
      if (peer->rtt_keepalive_rcv > peer->rtt_keepalive_conf)
        {
          ...
          snprintfrr (
              rtt_shutdown_reason, sizeof (rtt_shutdown_reason),
              "shutdown due to high round-trip-time (%dms > %dms, hit %u times)",
              peer->rtt, peer->rtt_expected, peer->rtt_keepalive_rcv);
...
static void
bgp_refresh_stalepath_timer_expire (struct event *thread)
{
  struct peer_af *paf;
  ...
  if (bgp_debug_neighbor_events (peer))
    zlog_debug ("%pBP route-refresh (BoRR) timer expired for afi/safi: %d/%d",
                peer, afi, safi);
  ...
  if (! peer_established (peer))
    {
      flog_err (EC_BGP_INVALID_STATUS,
                "%s [FSM] Update packet received under status %s", peer->host,
...
#define NLRI_ATTR_ARG                                                         \
  ...
  if (attribute_len)
    {
      attr_parse_ret =
          bgp_attr_parse (peer, &attr, attribute_len, &nlris[NLRI_MP_UPDATE],
  ...
  if (attr_parse_ret == BGP_ATTR_PARSE_WITHDRAW ||
      BGP_DEBUG (update, UPDATE_IN) || BGP_DEBUG (update, UPDATE_PREFIX))
    {
  ...
  for (int i = NLRI_UPDATE; i < NLRI_TYPE_MAX; i++)
    {
      ...
      switch (i)
        ...
        case NLRI_MP_WITHDRAW:
          nlri_ret = bgp_nlri_parse (peer, NLRI_ATTR_ARG, &nlris[i], 1);
          break;
        default:
      ...
      if (nlri_ret < BGP_NLRI_PARSE_OK &&
        ...
        {
          flog_err (EC_BGP_UPDATE_RCV, "%s [Error] Error parsing NLRI",
                    peer->host);
          if (peer_established (peer))
            bgp_notify_send (peer, BGP_NOTIFY_UPDATE_ERR,
                             i <= NLRI_WITHDRAW
          ...
          if (! CHECK_FLAG (peer->af_sflags[afi][safi],
                            PEER_STATUS_EOR_RECEIVED))
            {
              ...
              if (gr_info->eor_required == gr_info->eor_received)
                {
                  if (bgp_debug_neighbor_events (peer))
                    zlog_debug ("%s %d, %s %d", "EOR REQ",
                                gr_info->eor_required, "EOR RCV",
                                gr_info->eor_received);
...
bgp_route_refresh_receive (struct peer *peer, bgp_size_t size)
{
  ...
  if (! CHECK_FLAG (peer->cap, PEER_CAP_REFRESH_ADV))
    {
      flog_err (EC_BGP_NO_CAP, "%s [Error] BGP route refresh is not enabled",
                peer->host);
  ...
  if (bgp_map_afi_safi_iana2int (pkt_afi, pkt_safi, &afi, &safi))
    {
      zlog_info ("%s REFRESH_REQ for unrecognized afi/safi: %s/%s - ignored",
      ...
      if (subtype)
        {
          ...
          if (msg_length != 4)
            {
              zlog_err ("%s Enhanced Route Refresh message length error",
                        peer->host);
              bgp_notify_send (peer, BGP_NOTIFY_ROUTE_REFRESH_ERR,
           ...
           */
          if (subtype > 2)
            zlog_err ("%s Enhanced Route Refresh invalid subtype", peer->host);
        }
          ...
          if (orf_type == ORF_TYPE_PREFIX || orf_type == ORF_TYPE_PREFIX_OLD)
            {
              ...
              if (bgp_debug_neighbor_events (peer))
                {
                  zlog_debug ("%pBP rcvd Prefixlist ORF(%d) length %d", peer,
                              orf_type, orf_len);
                }

              /* ORF prefix-list name */
              snprintf (name, sizeof (name), "%s.%d.%d", peer->host, afi,
                        safi);
              ...
              if (p_pnt < p_end && *p_pnt & ORF_COMMON_PART_REMOVE_ALL)
                {
                  if (bgp_debug_neighbor_events (peer))
                    zlog_debug ("%pBP rcvd Remove-All pfxlist ORF request",
                                peer);
              ...
              while (p_pnt < p_end)
                {
                  ...
                  common = *p_pnt++;
                  /* after ++: p_pnt <= p_end */
                  ok = ((uint32_t) (p_end - p_pnt) >= sizeof (uint32_t));
                  if (ok)
                    {
                      memcpy (&seq, p_pnt, sizeof (uint32_t));
                  ...
                  if (bgp_debug_neighbor_events (peer))
                    {
                      ...
                      zlog_debug (
                          "%pBP rcvd %s %s seq %u %s/%d ge %d le %d%s", peer,
                          (common & ORF_COMMON_PART_REMOVE ? "Remove" : "Add"),
                          (common & ORF_COMMON_PART_DENY ? "deny" : "permit"),
                          orfp.seq,
                          inet_ntop (orfp.p.family, &orfp.p.u.prefix, buf,
                                     INET6_BUFSIZ),
  ...
  if (subtype == BGP_ROUTE_REFRESH_BORR)
    {
      ...
      if (CHECK_FLAG (peer->cap, PEER_CAP_RESTART_RCV) &&
        ...
        {
          if (bgp_debug_neighbor_events (peer))
            zlog_debug ("%pBP rcvd route-refresh (BoRR) for %s/%s before EoR",
                        peer, afi2str (afi), safi2str (safi));
          return BGP_PACKET_NOOP;
        }
      ...
      UNSET_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_EORR_RECEIVED);
      ...
      if (peer_established (peer))
        event_add_timer (bm->master, bgp_refresh_stalepath_timer_expire, paf,
  ...
  else if (subtype == BGP_ROUTE_REFRESH_EORR)
    {
      ...
      UNSET_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_BORR_RECEIVED);
    ...
    {
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%pBP rcvd route-refresh (REQUEST) for %s/%s", peer,
      ...
      if (CHECK_FLAG (peer->cap, PEER_CAP_ENHANCED_RR_RCV))
        {
          ...
          if (bgp_debug_neighbor_events (peer))
            zlog_debug ("%pBP sending route-refresh (BoRR) for %s/%s", peer,
                        afi2str (afi), safi2str (safi));
 ...
 */
static int
bgp_capability_msg_parse (struct peer *peer, uint8_t *pnt, bgp_size_t length)
{
  uint8_t *end;
  ...
  while (pnt < end)
    {
      ...
      if (action != CAPABILITY_ACTION_SET && action != CAPABILITY_ACTION_UNSET)
        {
          zlog_info ("%s Capability Action Value error %d", peer->host,
                     action);
          bgp_notify_send (peer, BGP_NOTIFY_CEASE,
                           BGP_NOTIFY_SUBCODE_UNSPECIFIC);
          return BGP_Stop;
      ...
      if (hdr->length < sizeof (struct capability_mp_data))
        {
          zlog_info (
              ...
              peer, sizeof (struct capability_mp_data), hdr->length);
          return BGP_Stop;
        }
      ...
      if (hdr->code == CAPABILITY_CODE_MP)
        {
          ...
          if (bgp_debug_neighbor_events (peer))
            zlog_debug ("%s CAPABILITY has %s MP_EXT CAP for afi/safi: %s/%s",
                        peer->host,
 ...
 */
void
bgp_process_packet (struct event *thread)
{
  ...
  rpkt_quanta_old =
      atomic_load_explicit (&peer->bgp->rpkt_quanta, memory_order_relaxed);
  fsm_update_result = 0;
  ...
  while (processed < rpkt_quanta_old)
    {
      ...
      switch (type)
        ...
        case BGP_MSG_OPEN:
          ...
          if (mprc == BGP_Stop)
            flog_err (EC_BGP_PKT_OPEN,
                      "%s: BGP OPEN receipt failed for peer: %s", __func__,
                      peer->host);
          break;
        case BGP_MSG_UPDATE:
          ...
          if (mprc == BGP_Stop)
            flog_err (EC_BGP_UPDATE_RCV,
                      "%s: BGP UPDATE receipt failed for peer: %s", __func__,
                      peer->host);
          break;
        case BGP_MSG_NOTIFY:
          ...
          if (mprc == BGP_Stop)
            flog_err (EC_BGP_NOTIFY_RCV,
                      "%s: BGP NOTIFY receipt failed for peer: %s", __func__,
                      peer->host);
          break;
...
void
bgp_send_delayed_eor (struct bgp *bgp)
{
  struct peer *peer;
  struct listnode *node, *nnode;
 ...
 */
void
bgp_packet_process_error (struct event *thread)
{
  struct peer *peer;
  int code;
