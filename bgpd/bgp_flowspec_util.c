...
static int
hexstr2num (uint8_t *hexstr, int len)
{
  int i = 0;
  int num = 0;
 ...
 */
static int
bgp_flowspec_call_non_opaque_decode (uint8_t *nlri_content, int len,
                                     ...
                                     uint8_t *match_num, int *error)
{
  ...
  if (*error < 0)
    flog_err (EC_BGP_FLOWSPEC_PACKET, "%s: flowspec_op_decode error %d",
              __func__, *error);
  else
    *match_num = *error;
  return ret;
...
bool
bgp_flowspec_contains_prefix (const struct prefix *pfs, struct prefix *input,
                              int prefix_check)
{
  uint32_t offset = 0;
  int type;
      ...
      switch (type)
        ...
        case FLOWSPEC_SRC_PREFIX:
          ...
          ret = bgp_flowspec_ip_address (BGP_FLOWSPEC_CONVERT_TO_NON_OPAQUE,
                                         nlri_content + offset, len - offset,
                                         &compare, &error, afi, NULL);
          if (ret <= 0)
            break;
          if (prefix_check && compare.prefixlen != input->prefixlen)
            break;
          if (compare.family != input->family)
            break;
          if ((input->family == AF_INET) &&
              IPV4_ADDR_SAME (&input->u.prefix4, &compare.u.prefix4))
            return true;
 ...
 */
int
bgp_flowspec_ip_address (enum bgp_flowspec_util_nlri_t type, uint8_t *nlri_ptr,
                         uint32_t max_len, void *result, int *error, afi_t afi,
                         uint8_t *ipv6_offset)
{
      ...
      if (prefix_local.family == AF_INET6)
        {
          ...
          ret = snprintfrr (display, BGP_FLOWSPEC_STRING_DISPLAY_MAX,
        ...
        prefix2str (&prefix_local, display, BGP_FLOWSPEC_STRING_DISPLAY_MAX);
      break;
 ...
 */
int
bgp_flowspec_op_decode (enum bgp_flowspec_util_nlri_t type, uint8_t *nlri_ptr,
 ...
 */
int
bgp_flowspec_bitmask_decode (enum bgp_flowspec_util_nlri_t type,
...
int
bgp_flowspec_match_rules_fill (uint8_t *nlri_content, int len,
  ...
  while (offset < len - 1 && error >= 0)
    {
      ...
      switch (type)
        ...
        case FLOWSPEC_SRC_PREFIX:
          ...
          ret = bgp_flowspec_ip_address (BGP_FLOWSPEC_CONVERT_TO_NON_OPAQUE,
                                         nlri_content + offset, len - offset,
                                         prefix, &error, afi, prefix_offset);
          if (error < 0)
            flog_err (EC_BGP_FLOWSPEC_PACKET,
                      "%s: flowspec_ip_address error %d", __func__, error);
          else
            {
              ...
              if (prefix->family == AF_INET &&
                ...
                bpem->match_bitmask_iprule |= bitmask;
                       ...
                       ! memcmp (&prefix->u.prefix6, &in6addr_any,
        ...
        case FLOWSPEC_FLOW_LABEL:
          ...
          offset += bgp_flowspec_call_non_opaque_decode (
              nlri_content + offset, len - offset, mval, match_num, &error);
          break;
        case FLOWSPEC_IP_PROTOCOL:
        ...
        case FLOWSPEC_TCP_FLAGS:
          ret = bgp_flowspec_bitmask_decode (
              BGP_FLOWSPEC_CONVERT_TO_NON_OPAQUE, nlri_content + offset,
              len - offset, &bpem->tcpflags, &error);
          if (error < 0)
            flog_err (EC_BGP_FLOWSPEC_PACKET,
                      "%s: flowspec_tcpflags_decode error %d", __func__,
                      error);
          else
        ...
        case FLOWSPEC_FRAGMENT:
          ret = bgp_flowspec_bitmask_decode (
              BGP_FLOWSPEC_CONVERT_TO_NON_OPAQUE, nlri_content + offset,
              len - offset, &bpem->fragment, &error);
          if (error < 0)
            flog_err (EC_BGP_FLOWSPEC_PACKET,
                      "%s: flowspec_fragment_type_decode error %d", __func__,
                      error);
          else
          ...
          break;
        default:
          flog_err (EC_LIB_DEVELOPMENT, "%s: unknown type %d", __func__, type);
        }
    }
...
bool
bgp_flowspec_get_first_nh (struct bgp *bgp, struct bgp_path_info *pi,
                           struct prefix *p, afi_t afi)
{
  ...
  if (bgp_pbr_build_and_validate_entry (bgp_dest_get_prefix (dest), pi, &api) <
      0)
    return true;
