...
uint16_t
decode_rd_type (const uint8_t *pnt)
{
  uint16_t v;
...
void
encode_rd_type (uint16_t v, uint8_t *pnt)
{
...
/* type == RD_TYPE_AS */
void
decode_rd_as (const uint8_t *pnt, struct rd_as *rd_as)
{
...
/* type == RD_TYPE_AS4 */
void
decode_rd_as4 (const uint8_t *pnt, struct rd_as *rd_as)
{
...
/* type == RD_TYPE_IP */
void
decode_rd_ip (const uint8_t *pnt, struct rd_ip *rd_ip)
{
...
void
decode_rd_vnc_eth (const uint8_t *pnt, struct rd_vnc_eth *rd_vnc_eth)
{
  ...
  if (p2)
    {
      ...
      half[p - str] = '\0';
      type = atoi (half);
      if (type != RD_TYPE_AS && type != RD_TYPE_IP && type != RD_TYPE_AS4)
        goto out;
...
char *
prefix_rd2str (const struct prefix_rd *prd, char *buf, size_t size,
...
void
form_auto_rd (struct in_addr router_id, uint16_t rd_id, struct prefix_rd *prd)
{
  char buf[100];
...
static ssize_t
printfrr_prd_asnotation (struct fbuf *buf, struct printfrr_eargs *ea,
...
static ssize_t
printfrr_prd (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
...
static ssize_t
printfrr_prd_dot (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
...
static ssize_t
printfrr_prd_dotplus (struct fbuf *buf, struct printfrr_eargs *ea,
                      const void *ptr)
{
