...
static int
bgp_reuse_index (int penalty, struct bgp_damp_config *bdc)
{
  unsigned int i;
  int index;
...
static void
bgp_reuse_list_add (struct bgp_damp_info *bdi, struct bgp_damp_config *bdc)
{
  int index;
...
static void
bgp_reuse_list_delete (struct bgp_damp_info *bdi, struct bgp_damp_config *bdc)
{
  if (bdi->next)
...
int
bgp_damp_decay (time_t tdiff, int penalty, struct bgp_damp_config *bdc)
{
  unsigned int i;
...
static void
bgp_reuse_timer (struct event *t)
{
  ...
  for (; bdi; bdi = next)
    {
      ...
      if (bdi->penalty < bdc->reuse_limit)
        {
          /* Reuse the route.  */
          bgp_path_info_unset_flag (bdi->dest, bdi->path, BGP_PATH_DAMPED);
          bdi->suppress_time = 0;
          ...
          if (bdi->lastrecord == BGP_RECORD_UPDATE)
            {
              bgp_path_info_unset_flag (bdi->dest, bdi->path,
                                        BGP_PATH_HISTORY);
              bgp_aggregate_increment (bgp, bgp_dest_get_prefix (bdi->dest),
...
int
bgp_damp_withdraw (struct bgp_path_info *path, struct bgp_dest *dest,
...
int
bgp_damp_update (struct bgp_path_info *path, struct bgp_dest *dest, afi_t afi,
                 safi_t safi)
{
  time_t t_now;
  struct bgp_damp_info *bdi;
...
void
bgp_damp_info_free (struct bgp_damp_info *bdi, int withdraw, afi_t afi,
                    safi_t safi)
{
...
static void
bgp_damp_parameter_set (time_t hlife, unsigned int reuse, unsigned int sup,
                        time_t maxsup, struct bgp_damp_config *bdc)
{
  ...
  bdc->reuse_index =
      XCALLOC (MTYPE_BGP_DAMP_ARRAY, sizeof (int) * bdc->reuse_index_size);
  ...
  bdc->scale_factor = (double) bdc->reuse_index_size / (reuse_max_ratio - 1);

  for (i = 0; i < bdc->reuse_index_size; i++)
    {
      bdc->reuse_index[i] =
          (int) (((double) bdc->half_life / DELTA_REUSE) *
                 log10 (1.0 / (bdc->reuse_limit *
...
static void
bgp_damp_config_clean (struct bgp_damp_config *bdc)
{
  /* Free decay array */
...
void
bgp_damp_info_clean (afi_t afi, safi_t safi)
{
  unsigned int i;
...
void
bgp_config_write_damp (struct vty *vty, afi_t afi, safi_t safi)
{
  if (damp[afi][safi].half_life == DEFAULT_HALF_LIFE * 60 &&
    ...
    vty_out (vty, "  bgp dampening %lld %d %d %lld\n",
             damp[afi][safi].half_life / 60LL, damp[afi][safi].reuse_limit,
             damp[afi][safi].suppress_value,
             damp[afi][safi].max_suppress_time / 60LL);
}

static const char *
bgp_get_reuse_time (unsigned int penalty, char *buf, size_t len, afi_t afi,
                    safi_t safi, bool use_json, json_object *json)
{
  ...
  if (penalty > damp[afi][safi].reuse_limit)
    {
      reuse_time =
          (int) (DELTA_T *
                 ((log ((double) damp[afi][safi].reuse_limit / penalty)) /
  ...
  if (reuse_time == 0)
  ...
  else if (reuse_time < ONE_DAY_SECOND)
    {
      if (use_json)
        {
          time_store = (3600000 * tm.tm_hour) + (60000 * tm.tm_min) +
                       (1000 * tm.tm_sec);
  ...
  else if (reuse_time < ONE_WEEK_SECOND)
    {
      if (use_json)
        {
          time_store = (86400000 * tm.tm_yday) + (3600000 * tm.tm_hour) +
      ...
      if (use_json)
        {
          time_store = (604800000 * tm.tm_yday / 7) +
                       (86400000 * (tm.tm_yday - ((tm.tm_yday / 7) * 7))) +
...
void
bgp_damp_info_vty (struct vty *vty, struct bgp_path_info *path, afi_t afi,
  ...
  if (json_path)
    {
      ...
      if (CHECK_FLAG (path->flags, BGP_PATH_DAMPED) &&
        ...
        bgp_get_reuse_time (penalty, timebuf, BGP_UPTIME_LEN, afi, safi, 1,
                            json_path);
    }
  else
    {
      ...
      if (CHECK_FLAG (path->flags, BGP_PATH_DAMPED) &&
        ...
        vty_out (vty, ", reuse in %s",
                 bgp_get_reuse_time (penalty, timebuf, BGP_UPTIME_LEN, afi,
                                     safi, 0, json_path));
...
const char *
bgp_damp_reuse_time_vty (struct vty *vty, struct bgp_path_info *path,
                         char *timebuf, size_t len, afi_t afi, safi_t safi,
                         bool use_json, json_object *json)
{
  ...
  return bgp_get_reuse_time (penalty, timebuf, len, afi, safi, use_json, json);
}

static int
bgp_print_dampening_parameters (struct bgp *bgp, struct vty *vty, afi_t afi,
      ...
      if (use_json)
        {
          ...
          json_object_int_add (json, "halfLifeSecs", bdc->half_life);
          json_object_int_add (json, "reusePenalty", bdc->reuse_limit);
          json_object_int_add (json, "suppressPenalty", bdc->suppress_value);
          json_object_int_add (json, "maxSuppressTimeSecs",
                               bdc->max_suppress_time);
          json_object_int_add (json, "maxSuppressPenalty", bdc->ceiling);
                               ...
                               uint16_t show_flags)
{
  ...
  if (! CHECK_FLAG (show_flags, BGP_SHOW_OPT_AFI_ALL))
    return bgp_print_dampening_parameters (bgp, vty, afi, safi, use_json);
      ...
      FOREACH_SAFI (safi)
        {
          ...
          bgp_print_dampening_parameters (bgp, vty, afi, safi, use_json);
        }
    }
  else
    {
