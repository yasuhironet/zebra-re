...
static struct peer *
peer_lookup_all_vrf (struct ipaddr *addr)
{
  struct bgp *bgp;
  struct peer *peer;
      ...
      for (ALL_LIST_ELEMENTS_RO (bgp->peer, node, peer))
        {
          switch (sockunion_family (&peer->su))
            {
            case AF_INET:
              if (IPV4_ADDR_SAME (&peer->su.sin.sin_addr, &addr->ip._v4_addr))
                return peer;
              break;
            case AF_INET6:
...
static struct peer *
peer_lookup_all_vrf_next (struct ipaddr *addr, oid *offset, sa_family_t family)
{
  struct bgp *bgp;
  struct peer *peer;
      ...
      for (ALL_LIST_ELEMENTS_RO (bgp->peer, node, peer))
        {
          ...
          switch (sockunion_family (&peer->su))
            ...
            case AF_INET:
              ...
              if (IPV4_ADDR_CMP (&peer->su.sin.sin_addr, &addr->ip._v4_addr) <
                      0 ||
                  IPV4_ADDR_SAME (&peer->su.sin.sin_addr, &addr->ip._v4_addr))
                continue;

              if (! next_peer || IPV4_ADDR_CMP (&next_peer->su.sin.sin_addr,
            ...
            case AF_INET6:
              ...
              if (! next_peer || IPV6_ADDR_CMP (&next_peer->su.sin6.sin6_addr,
...
static struct peer *
bgpv2PeerTable_lookup (struct variable *v, oid name[], size_t *length,
                       int exact, struct ipaddr *addr)
{
...
static uint8_t *
bgpv2PeerTable (struct variable *v, oid name[], size_t *length, int exact,
                size_t *var_len, WriteMethod **write_method)
{
  ...
  switch (v->magic)
    ...
    case BGP4V2_PEER_LOCAL_ADDR_TYPE:
      if (peer->su_local)
        return SNMP_INTEGER (
            peer->su_local->sa.sa_family == AF_INET ? AFI_IP : AFI_IP6);
      else
    ...
    case BGP4V2_PEER_LOCAL_ADDR:
      if (peer->su_local)
        if (peer->su_local->sa.sa_family == AF_INET)
          ...
          return SNMP_IP6ADDRESS (peer->su_local->sin6.sin6_addr);
      else
        return SNMP_IPADDRESS (bgp_empty_addr);
    case BGP4V2_PEER_REMOTE_ADDR_TYPE:
      if (peer->su_remote)
        return SNMP_INTEGER (
            peer->su_remote->sa.sa_family == AF_INET ? AFI_IP : AFI_IP6);
      else
        return SNMP_INTEGER (0);
    case BGP4V2_PEER_REMOTE_ADDR:
      if (peer->su_remote)
        if (peer->su_remote->sa.sa_family == AF_INET)
          ...
          return SNMP_IP6ADDRESS (peer->su_remote->sin6.sin6_addr);
      else
        return SNMP_IPADDRESS (bgp_empty_addr);
    case BGP4V2_PEER_LOCAL_PORT:
      if (peer->su_local)
        if (peer->su_local->sa.sa_family == AF_INET)
          ...
          return SNMP_INTEGER (ntohs (peer->su_local->sin6.sin6_port));
      else
        return SNMP_INTEGER (0);
    ...
    case BGP4V2_PEER_REMOTE_PORT:
      if (peer->su_remote)
        if (peer->su_remote->sa.sa_family == AF_INET)
          ...
          return SNMP_INTEGER (ntohs (peer->su_remote->sin6.sin6_port));
      else
        return SNMP_INTEGER (0);
...
static uint8_t *
bgpv2PeerErrorsTable (struct variable *v, oid name[], size_t *length,
...
static uint8_t *
bgpv2PeerEventTimesTable (struct variable *v, oid name[], size_t *length,
  ...
  do
    {
      ...
      for (path = bgp_dest_get_bgp_path_info (dest); path; path = path->next)
        {
          sa_family_t path_family = sockunion_family (&path->peer->su);
          ...
          if (path_family == AF_INET &&
            ...
            {
              if (! min ||
                  (min && IPV4_ADDR_CMP (&path->peer->su.sin.sin_addr,
                                         &min->peer->su.sin.sin_addr) < 0))
                min = path;
            }
          else if (path_family == AF_INET6 &&
                   IPV6_ADDR_CMP (&paddr.ip._v6_addr,
                                  &path->peer->su.sin6.sin6_addr) < 0)
            {
              if (! min ||
                  (min && IPV6_ADDR_CMP (&path->peer->su.sin6.sin6_addr,
                                         &min->peer->su.sin6.sin6_addr) < 0))
                min = path;
            }
        }
      ...
      if (min)
        {
          ...
          *offset = rn_p->prefixlen;
          ...
          if (family == AF_INET)
            {
              oid_copy_in_addr (offset, &min->peer->su.sin.sin_addr);
              addr->u.prefix4 = rn_p->u.prefix4;
            }
          else
            {
              oid_copy_in6_addr (offset, &min->peer->su.sin6.sin6_addr);
...
static uint8_t *
bgp4v2PathAttrTable (struct variable *v, oid name[], size_t *length, int exact,
                     size_t *var_len, WriteMethod **write_method)
{
  struct bgp *bgp;
...
int
bgp_snmp_bgp4v2_init (struct event_loop *tm)
{
