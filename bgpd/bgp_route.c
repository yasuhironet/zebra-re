...
bool
bgp_path_suppressed (struct bgp_path_info *pi)
{
  ...
  if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP) || (safi == SAFI_EVPN))
    {
      ...
      if (! bgp_dest_has_bgp_path_info_data (pdest))
        bgp_dest_set_bgp_table_info (pdest,
                                     bgp_table_init (table->bgp, afi, safi));
      else
...
struct bgp_dest *
bgp_safi_node_lookup (struct bgp_table *table, safi_t safi,
                      const struct prefix *p, struct prefix_rd *prd)
{
  struct bgp_dest *dest;
  struct bgp_dest *pdest = NULL;
...
static struct bgp_path_info_extra *
bgp_path_info_extra_new (void)
{
  struct bgp_path_info_extra *new;
  new = XCALLOC (MTYPE_BGP_ROUTE_EXTRA, sizeof (struct bgp_path_info_extra));
...
void
bgp_path_info_extra_free (struct bgp_path_info_extra **extra)
{
...
bgp_path_info_extra_get (struct bgp_path_info *pi)
{
  if (! pi->extra)
...
void
bgp_path_info_free_with_caller (const char *name, struct bgp_path_info *path)
{
  ...
  if (path->net)
    bgp_addpath_free_info_data (&path->tx_addpath, &path->net->tx_addpath);
...
bgp_path_info_lock (struct bgp_path_info *path)
{
  path->lock++;
  return path;
}
...
struct bgp_path_info *
bgp_path_info_unlock (struct bgp_path_info *path)
{
...
static int
bgp_dest_set_defer_flag (struct bgp_dest *dest, bool delete)
{
  struct peer *peer;
      ...
      if (bgp && (bgp->gr_info[afi][safi].t_select_deferral))
        {
          ...
          if (BGP_DEBUG (update, UPDATE_OUT))
            zlog_debug ("DEFER route %pBD(%s), dest %p", dest,
...
void
bgp_path_info_add_with_caller (const char *name, struct bgp_dest *dest,
...
void
bgp_path_info_reap (struct bgp_dest *dest, struct bgp_path_info *pi)
{
  if (pi->next)
...
void
bgp_path_info_delete (struct bgp_dest *dest, struct bgp_path_info *pi)
{
...
void
bgp_path_info_restore (struct bgp_dest *dest, struct bgp_path_info *pi)
{
...
static void
bgp_pcount_adjust (struct bgp_dest *dest, struct bgp_path_info *pi)
{
  ...
  if (! BGP_PATH_COUNTABLE (pi) && CHECK_FLAG (pi->flags, BGP_PATH_COUNTED))
    {
...
static int
bgp_label_index_differs (struct bgp_path_info *pi1, struct bgp_path_info *pi2)
{
 ...
 */
void
bgp_path_info_set_flag (struct bgp_dest *dest, struct bgp_path_info *pi,
                        uint32_t flag)
{
...
void
bgp_path_info_unset_flag (struct bgp_dest *dest, struct bgp_path_info *pi,
                          uint32_t flag)
{
...
void
bgp_path_info_path_with_addpath_rx_str (struct bgp_path_info *pi, char *buf,
                                        size_t buf_len)
{
  struct peer *peer;

  if (pi->sub_type == BGP_ROUTE_IMPORTED && bgp_get_imported_bpi_ultimate (pi))
...
bgp_get_imported_bpi_ultimate (struct bgp_path_info *info)
{
  ...
  for (bpi_ultimate = info; bpi_ultimate->extra && bpi_ultimate->extra->parent;
 ...
 */
static int
bgp_path_info_cmp (struct bgp *bgp, struct bgp_path_info *new,
                   ...
                   enum bgp_path_selection_reason *reason)
{
  ...
  *paths_eq = 0;
  ...
  if (exist == NULL)
    {
      *reason = bgp_path_selection_first;
      if (debug)
        zlog_debug ("%s(%s): %s is the initial bestpath", pfx_buf,
                    bgp->name_pretty, new_buf);
      return 1;
    }
  ...
  if (debug)
    {
      ...
      zlog_debug ("%s(%s): Comparing %s flags 0x%x with %s flags 0x%x",
                  pfx_buf, bgp->name_pretty, new_buf, new->flags, exist_buf,
                  exist->flags);
    }
  ...
  if (bgp_attr_get_community (newattr) &&
    ...
    {
      if (debug)
        zlog_debug ("%s: %s wins over %s due to LLGR_STALE community", pfx_buf,
                    new_buf, exist_buf);
      return 0;
    }
  ...
  if (bgp_attr_get_community (existattr) &&
    ...
    {
      if (debug)
        zlog_debug ("%s: %s loses to %s due to LLGR_STALE community", pfx_buf,
                    new_buf, exist_buf);
      return 1;
    }
      ...
      (new_p->u.prefix_evpn.route_type == BGP_EVPN_MAC_IP_ROUTE))
    {
          ...
          if (newattr->sticky && ! existattr->sticky)
            {
              *reason = bgp_path_selection_evpn_sticky_mac;
              if (debug)
                zlog_debug ("%s: %s wins over %s due to sticky MAC flag",
          ...
          if (! newattr->sticky && existattr->sticky)
            {
              *reason = bgp_path_selection_evpn_sticky_mac;
              if (debug)
                zlog_debug ("%s: %s loses to %s due to sticky MAC flag",
                            pfx_buf, new_buf, exist_buf);
              return 0;
            }
        }
          ...
          if (bgp_evpn_is_path_local (bgp, new))
            {
              *reason = bgp_path_selection_evpn_local_path;
              if (debug)
                zlog_debug ("%s: %s wins over %s as ES %s is same and local",
                            pfx_buf, new_buf, exist_buf,
                            esi_to_str (new_esi, esi_buf, sizeof (esi_buf)));
              return 1;
            }
          if (bgp_evpn_is_path_local (bgp, exist))
            {
              *reason = bgp_path_selection_evpn_local_path;
              if (debug)
                zlog_debug ("%s: %s loses to %s as ES %s is same and local",
                            pfx_buf, new_buf, exist_buf,
                            esi_to_str (new_esi, esi_buf, sizeof (esi_buf)));
              return 0;
            }
      ...
      if (new_mm_seq > exist_mm_seq)
        {
          *reason = bgp_path_selection_evpn_seq;
          if (debug)
            zlog_debug ("%s: %s wins over %s due to MM seq %u > %u", pfx_buf,
                        new_buf, exist_buf, new_mm_seq, exist_mm_seq);
          return 1;
        }
      ...
      if (new_mm_seq < exist_mm_seq)
        {
          *reason = bgp_path_selection_evpn_seq;
          if (debug)
            zlog_debug ("%s: %s loses to %s due to MM seq %u < %u", pfx_buf,
                        new_buf, exist_buf, new_mm_seq, exist_mm_seq);
          return 0;
        }
      ...
      if (same_esi && bgp_evpn_attr_is_local_es (newattr) &&
          old_proxy != new_proxy)
        {
          if (! new_proxy)
            {
              *reason = bgp_path_selection_evpn_non_proxy;
              if (debug)
                zlog_debug ("%s: %s wins over %s, same seq/es and non-proxy",
                            pfx_buf, new_buf, exist_buf);
              return 1;
            }
          ...
          *reason = bgp_path_selection_evpn_non_proxy;
          if (debug)
            zlog_debug ("%s: %s loses to %s, same seq/es and non-proxy",
                        pfx_buf, new_buf, exist_buf);
          return 0;
        }
          ...
          *reason = bgp_path_selection_evpn_lower_ip;
          if (debug)
            zlog_debug (
                ...
                pfx_buf, new_buf, exist_buf, new_mm_seq, &new->attr->nexthop);
          return 1;
        }
      if (nh_cmp > 0)
        {
  ...
  if (new_weight > exist_weight)
    {
      *reason = bgp_path_selection_weight;
      if (debug)
        zlog_debug ("%s: %s wins over %s due to weight %d > %d", pfx_buf,
                    new_buf, exist_buf, new_weight, exist_weight);
      return 1;
    }
  ...
  if (new_weight < exist_weight)
    {
      *reason = bgp_path_selection_weight;
      if (debug)
        zlog_debug ("%s: %s loses to %s due to weight %d < %d", pfx_buf,
                    new_buf, exist_buf, new_weight, exist_weight);
      return 0;
    }
  ...
  if (new_pref > exist_pref)
    {
      *reason = bgp_path_selection_local_pref;
      if (debug)
        zlog_debug ("%s: %s wins over %s due to localpref %d > %d", pfx_buf,
                    new_buf, exist_buf, new_pref, exist_pref);
      return 1;
    }
  ...
  if (new_pref < exist_pref)
    {
      *reason = bgp_path_selection_local_pref;
      if (debug)
        zlog_debug ("%s: %s loses to %s due to localpref %d < %d", pfx_buf,
                    new_buf, exist_buf, new_pref, exist_pref);
      return 0;
    }
  ...
  if (safi == SAFI_MPLS_VPN &&
      (CHECK_FLAG (new->peer->af_flags[afi][safi], PEER_FLAG_ACCEPT_OWN) ||
       CHECK_FLAG (exist->peer->af_flags[afi][safi], PEER_FLAG_ACCEPT_OWN)))
    {
      ...
      if (newattr->flag & ATTR_FLAG_BIT (BGP_ATTR_COMMUNITIES))
        new_accept_own =
            community_include (bgp_attr_get_community (newattr), accept_own);
      if (existattr->flag & ATTR_FLAG_BIT (BGP_ATTR_COMMUNITIES))
        exist_accept_own =
            community_include (bgp_attr_get_community (existattr), accept_own);

      if (new_accept_own && ! exist_accept_own)
        {
          *reason = bgp_path_selection_accept_own;
          if (debug)
            zlog_debug ("%s: %s wins over %s due to accept-own", pfx_buf,
                        new_buf, exist_buf);
          return 1;
        }
      ...
      if (! new_accept_own && exist_accept_own)
        {
          *reason = bgp_path_selection_accept_own;
          if (debug)
            zlog_debug ("%s: %s loses to %s due to accept-own", pfx_buf,
                        new_buf, exist_buf);
          return 0;
        }
    }
      ...
      if (new_aigp < exist_aigp)
        {
          *reason = bgp_path_selection_aigp;
          if (debug)
            zlog_debug ("%s: %s wins over %s due to AIGP %" PRIu64
                        " < %" PRIu64,
                        pfx_buf, new_buf, exist_buf, new_aigp, exist_aigp);
          return 1;
        }
      ...
      if (new_aigp > exist_aigp)
        {
          *reason = bgp_path_selection_aigp;
          if (debug)
            zlog_debug ("%s: %s loses to %s due to AIGP %" PRIu64
                        " > %" PRIu64,
                        pfx_buf, new_buf, exist_buf, new_aigp, exist_aigp);
          return 0;
        }
              ...
              *reason = bgp_path_selection_confed_as_path;
              if (debug)
                zlog_debug (
                    ...
                    pfx_buf, new_buf, exist_buf, aspath_hops,
  ...
  if (newattr->origin < existattr->origin)
    {
      *reason = bgp_path_selection_origin;
      if (debug)
        zlog_debug ("%s: %s wins over %s due to ORIGIN %s < %s", pfx_buf,
                    new_buf, exist_buf, bgp_origin_long_str[newattr->origin],
  ...
  if (newattr->origin > existattr->origin)
    {
      *reason = bgp_path_selection_origin;
      if (debug)
        zlog_debug ("%s: %s loses to %s due to ORIGIN %s > %s", pfx_buf,
                    new_buf, exist_buf, bgp_origin_long_str[newattr->origin],
                    bgp_origin_long_str[existattr->origin]);
      return 0;
    }
      ...
      if (new_med < exist_med)
        {
          *reason = bgp_path_selection_med;
          if (debug)
            zlog_debug ("%s: %s wins over %s due to MED %d < %d", pfx_buf,
                        new_buf, exist_buf, new_med, exist_med);
          return 1;
        }
      ...
      if (new_med > exist_med)
        {
          *reason = bgp_path_selection_med;
          if (debug)
            zlog_debug ("%s: %s loses to %s due to MED %d > %d", pfx_buf,
                        new_buf, exist_buf, new_med, exist_med);
          return 0;
        }
    }
  ...
  if (exist_sort == BGP_PEER_EBGP &&
      (new_sort == BGP_PEER_IBGP || new_sort == BGP_PEER_CONFED))
    {
      *reason = bgp_path_selection_peer;
      if (debug)
        zlog_debug ("%s: %s loses to %s due to iBGP peer < eBGP peer", pfx_buf,
                    new_buf, exist_buf);
      if (! CHECK_FLAG (bgp->flags, BGP_FLAG_PEERTYPE_MULTIPATH_RELAX))
        return 0;
      peer_sort_ret = 0;
  ...
  if (newm < existm)
    {
      if (debug && peer_sort_ret < 0)
        zlog_debug ("%s: %s wins over %s due to IGP metric %u < %u", pfx_buf,
                    new_buf, exist_buf, newm, existm);
      igp_metric_ret = 1;
    }
  ...
  if (newm > existm)
    {
      if (debug && peer_sort_ret < 0)
        zlog_debug ("%s: %s loses to %s due to IGP metric %u > %u", pfx_buf,
                    new_buf, exist_buf, newm, existm);
      igp_metric_ret = 0;
    }
  ...
  if (newm == existm)
    {
      ...
      if (((new->extra &&bgp_is_valid_label (&new->extra->label[0])) !=
           (exist->extra && bgp_is_valid_label (&exist->extra->label[0]))) ||
          (newl != existl))
        {
          if (debug)
            zlog_debug (
      ...
      else if (CHECK_FLAG (bgp->flags, BGP_FLAG_ASPATH_MULTIPATH_RELAX))
        {
          ...
          *paths_eq = 1;
          ...
          if (debug)
            zlog_debug ("%s: %s and %s are equal via multipath-relax", pfx_buf,
                        new_buf, exist_buf);
        }
      ...
      else if (peer_new->as == peer_exist->as)
        {
          *paths_eq = 1;
          ...
          if (debug)
            zlog_debug ("%s: %s and %s are equal via same remote-as", pfx_buf,
                        new_buf, exist_buf);
        }
    }
  else
      ...
      if (CHECK_FLAG (new->flags, BGP_PATH_SELECTED))
        {
          *reason = bgp_path_selection_older;
          if (debug)
            zlog_debug ("%s: %s wins over %s due to oldest external", pfx_buf,
                        new_buf, exist_buf);
          return 1;
        }
      ...
      if (CHECK_FLAG (exist->flags, BGP_PATH_SELECTED))
        {
          *reason = bgp_path_selection_older;
          if (debug)
            zlog_debug ("%s: %s loses to %s due to oldest external", pfx_buf,
                        new_buf, exist_buf);
          return 0;
        }
    }
  ...
  if (ntohl (new_id.s_addr) < ntohl (exist_id.s_addr))
    {
      *reason = bgp_path_selection_router_id;
      if (debug)
        zlog_debug ("%s: %s wins over %s due to Router-ID comparison", pfx_buf,
                    new_buf, exist_buf);
      return 1;
    }
  ...
  if (ntohl (new_id.s_addr) > ntohl (exist_id.s_addr))
    {
      *reason = bgp_path_selection_router_id;
      if (debug)
        zlog_debug ("%s: %s loses to %s due to Router-ID comparison", pfx_buf,
                    new_buf, exist_buf);
      return 0;
    }
  ...
  if (ret == 1)
    {
      *reason = bgp_path_selection_neighbor_ip;
      if (debug)
        zlog_debug ("%s: %s loses to %s due to Neighor IP comparison", pfx_buf,
                    new_buf, exist_buf);
      return 0;
    }
  ...
  *reason = bgp_path_selection_default;
  if (debug)
    zlog_debug ("%s: %s wins over %s due to nothing left to compare", pfx_buf,
                new_buf, exist_buf);
...
int
bgp_evpn_path_info_cmp (struct bgp *bgp, struct bgp_path_info *new,
                        struct bgp_path_info *exist, int *paths_eq)
{
  enum bgp_path_selection_reason reason;
...
int
bgp_path_info_cmp_compatible (struct bgp *bgp, struct bgp_path_info *new,
                              ...
                              enum bgp_path_selection_reason *reason)
{
  int paths_eq;
  int ret;
  ret = bgp_path_info_cmp (bgp, new, exist, &paths_eq, NULL, 0, pfx_buf, afi,
                           safi, reason);
...
bgp_input_filter (struct peer *peer, const struct prefix *p, struct attr *attr,
                  afi_t afi, safi_t safi)
{
...
static enum filter_type
bgp_output_filter (struct peer *peer, const struct prefix *p,
...
static bool
bgp_otc_filter (struct peer *peer, struct attr *attr)
{
  ...
  if (peer->local_role == ROLE_CUSTOMER || peer->local_role == ROLE_PEER ||
...
static bool
bgp_otc_egress (struct peer *peer, struct attr *attr)
{
  if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_OTC))
    {
      if (peer->local_role == ROLE_CUSTOMER ||
          peer->local_role == ROLE_RS_CLIENT || peer->local_role == ROLE_PEER)
        return true;
      return false;
    }
  if (peer->local_role == ROLE_PROVIDER || peer->local_role == ROLE_PEER ||
      peer->local_role == ROLE_RS_SERVER)
    {
      attr->flag |= ATTR_FLAG_BIT (BGP_ATTR_OTC);
...
static bool
bgp_check_role_applicability (afi_t afi, safi_t safi)
{
  return ((afi == AFI_IP || afi == AFI_IP6) && safi == SAFI_UNICAST);
}

static int
bgp_input_modifier (struct peer *peer, const struct prefix *p,
                    struct attr *attr, afi_t afi, safi_t safi,
...
static int
bgp_output_modifier (struct peer *peer, const struct prefix *p,
                     struct attr *attr, afi_t afi, safi_t safi,
      ...
      if (peer_af_flag_check (peer, afi, safi,
                              ...
                              PEER_FLAG_REMOVE_PRIVATE_AS_ALL))
        {
          if (peer_af_flag_check (peer, afi, safi,
                                  PEER_FLAG_REMOVE_PRIVATE_AS_ALL_REPLACE))
            attr->aspath =
                aspath_replace_private_asns (attr->aspath, bgp->as, peer->as);
           ...
           */
            ...
            attr->aspath = aspath_remove_private_asns (attr->aspath, peer->as);
        }
...
void
bgp_attr_add_llgr_community (struct attr *attr)
{
...
void
bgp_attr_add_gshut_community (struct attr *attr)
{
  struct community *old;
  struct community *new;
  struct community *merge;
...
void
bgp_notify_conditional_adv_scanner (struct update_subgroup *subgrp)
{
...
void
subgroup_announce_reset_nhop (uint8_t family, struct attr *attr)
{
  if (family == AF_INET)
    {
...
bool
subgroup_announce_check (struct bgp_dest *dest, struct bgp_path_info *pi,
                         ...
                         struct attr *post_attr)
{
  ...
  if (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_OUT) &&
    ...
    {
      if (BGP_DEBUG (update, UPDATE_OUT) || BGP_DEBUG (update, UPDATE_PREFIX))
        {
          zlog_debug ("%s reached maximum prefix to be send (%u)", peer->host,
                      peer->pmax_out[afi][safi]);
        }
      return false;
    }
  ...
  if (((afi == AFI_IP) || (afi == AFI_IP6)) && (safi == SAFI_MPLS_VPN) &&
       ...
       (pi->type == ZEBRA_ROUTE_BGP_DIRECT_EXT)))
    {
       ...
       * systems
       */
      zlog_debug ("%s: pfx %pFX bgp_direct->vpn route peer safe", __func__, p);
      samepeer_safe = 1;
    }
#endif
  ...
  if (((afi == AFI_IP) || (afi == AFI_IP6)) &&
      ...
      (pi->type == ZEBRA_ROUTE_BGP) && (pi->sub_type == BGP_ROUTE_IMPORTED))
    {
      ...
      if (! bgp_is_valid_label (&label))
        {
          if (bgp_debug_update (NULL, p, subgrp->update_group, 0))
            zlog_debug ("u%" PRIu64 ":s%" PRIu64
                        ...
                        subgrp->update_group->id, subgrp->id, p, &label);
          return false;
        }
    }
  ...
  if (! transparent && bgp_community_filter (peer, piattr))
    {
      if (bgp_debug_update (NULL, p, subgrp->update_group, 0))
        zlog_debug ("%s: community filter check fail for %pFX", __func__, p);
      return false;
    }
    ...
    if (peer->orf_plist[afi][safi])
      {
        if (prefix_list_apply (peer->orf_plist[afi][safi], p) == PREFIX_DENY)
          {
            if (bgp_debug_update (NULL, p, subgrp->update_group, 0))
              zlog_debug ("%pBP [Update:SEND] %pFX is filtered via ORF", peer,
                          p);
            return false;
          }
      }
  ...
  if (bgp_output_filter (peer, p, piattr, afi, safi) == FILTER_DENY)
    {
      if (bgp_debug_update (NULL, p, subgrp->update_group, 0))
        zlog_debug ("%pBP [Update:SEND] %pFX is filtered", peer, p);
      return false;
    }
...
#define NEXTHOP_IS_V6                                                         \
  ...
  if (bgp_check_role_applicability (afi, safi) && bgp_otc_egress (peer, attr))
    return false;
  ...
  if (filter->advmap.update_type == UPDATE_TYPE_WITHDRAW &&
      filter->advmap.aname && route_map_lookup_by_name (filter->advmap.aname))
    {
      ...
      /* Fill temp path_info */
      prep_for_rmap_apply (&rmap_path, &dummy_rmap_path_extra, dest, pi, peer,
                           &dummy_attr);

      struct route_map *amap = route_map_lookup_by_name (filter->advmap.aname);
  ...
  if (! post_attr && (ROUTE_MAP_OUT_NAME (filter) || bgp_path_suppressed (pi)))
    {
      struct bgp_path_info rmap_path = { 0 };
      struct bgp_path_info_extra dummy_rmap_path_extra = { 0 };
      ...
      if (bgp_path_suppressed (pi))
        ret = route_map_apply (UNSUPPRESS_MAP (filter), p, &rmap_path);
      else
      ...
      if (ret == RMAP_DENYMATCH)
        {
          if (bgp_debug_update (NULL, p, subgrp->update_group, 0))
            zlog_debug (
                "%pBP [Update:SEND] %pFX is filtered by route-map '%s'", peer,
                p,
  ...
  if (! bgp_rmap_nhop_changed (attr->rmap_change_flags,
      ...
      ! CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED))
    {
      ...
      if (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_SELF) ||
        ...
        {
          if (! reflect || CHECK_FLAG (peer->af_flags[afi][safi],
                                       PEER_FLAG_FORCE_NEXTHOP_SELF))
            {
              subgroup_announce_reset_nhop (
                  (peer_cap_enhe (peer, afi, safi) ? AF_INET6 : p->family),
                  attr);
              nh_reset = true;
            }
        }
      else if (peer->sort == BGP_PEER_EBGP)
        {
           ...
           * IPv4 only.
           */
          if ((p->family == AF_INET) && (! bgp_subgrp_multiaccess_check_v4 (
                                            piattr->nexthop, subgrp, from)))
            {
              subgroup_announce_reset_nhop (
                  (peer_cap_enhe (peer, afi, safi) ? AF_INET6 : p->family),
                  attr);
          ...
          if ((p->family == AF_INET6) &&
              (! bgp_subgrp_multiaccess_check_v6 (piattr->mp_nexthop_global,
                                                  subgrp, from)))
            {
              subgroup_announce_reset_nhop (
                  (peer_cap_enhe (peer, afi, safi) ? AF_INET6 : p->family),
      ...
      else if (CHECK_FLAG (pi->flags, BGP_PATH_ANNC_NH_SELF))
        {
          ...
          if (bgp_debug_update (NULL, p, subgrp->update_group, 0))
            zlog_debug ("%s: %pFX BGP_PATH_ANNC_NH_SELF, family=%s", __func__,
                        p, family2str (family));
      ...
      if (rpki_state != RPKI_NOT_BEING_USED)
        bgp_attr_set_ecommunity (
            attr, ecommunity_add_origin_validation_state (
                      rpki_state, bgp_attr_get_ecommunity (attr)));
    }
  ...
  if (nh_reset && bgp_path_info_mpath_chkwtd (bgp, pi) &&
    ...
    bgp_attr_set_ecommunity (
        attr, ecommunity_replace_linkbw (
...
static void
bgp_route_select_timer_expire (struct event *thread)
{
...
void
bgp_best_selection (struct bgp *bgp, struct bgp_dest *dest,
  ...
  if (CHECK_FLAG (bgp->flags, BGP_FLAG_DETERMINISTIC_MED))
    {
      ...
      for (pi1 = bgp_dest_get_bgp_path_info (dest); pi1; pi1 = pi1->next)
        bgp_path_info_unset_flag (dest, pi1, BGP_PATH_DMED_SELECTED);
      ...
      for (pi1 = bgp_dest_get_bgp_path_info (dest); pi1; pi1 = pi1->next)
        {
          ...
          if (pi1->peer != bgp->peer_self &&
              ! CHECK_FLAG (pi1->peer->sflags, PEER_STATUS_NSF_WAIT))
            {
              ...
              for (pi2 = pi1->next; pi2; pi2 = pi2->next)
                {
                  ...
                  if (pi2->peer != bgp->peer_self &&
                      ! CHECK_FLAG (pi2->peer->sflags, PEER_STATUS_NSF_WAIT))
                    if (pi2->peer->status != Established)
                      continue;
                  ...
                  if (bgp_path_info_cmp (bgp, pi2, new_select, &paths_eq,
                  ...
                  bgp_path_info_set_flag (dest, pi2, BGP_PATH_DMED_CHECK);
                }
            }
          bgp_path_info_set_flag (dest, new_select, BGP_PATH_DMED_CHECK);
          bgp_path_info_set_flag (dest, new_select, BGP_PATH_DMED_SELECTED);

          if (debug)
            {
              bgp_path_info_path_with_addpath_rx_str (new_select, path_buf,
                                                      sizeof (path_buf));
              zlog_debug ("%pBD(%s): %s is the bestpath from AS %u", dest,
                          bgp->name_pretty, path_buf,
                          aspath_get_first_as (new_select->attr->aspath));
            }
        }
    }
       ...
       (pi != NULL) && (nextpi = pi->next, 1); pi = nextpi)
    {
      ...
      if (BGP_PATH_HOLDDOWN (pi))
        {
           ...
           */
          if (debug)
            zlog_debug ("%s: %pBD(%s) pi from %s in holddown", __func__, dest,
                        bgp->name_pretty, pi->peer->host);

          if (CHECK_FLAG (pi->flags, BGP_PATH_REMOVED) && (pi != old_select))
        ...
        if (! peer_established (pi->peer))
          {
            ...
            if (debug)
              zlog_debug ("%s: %pBD(%s) non self peer %s not estab state",
                          __func__, dest, bgp->name_pretty, pi->peer->host);
      ...
      if (CHECK_FLAG (bgp->flags, BGP_FLAG_DETERMINISTIC_MED) &&
          (! CHECK_FLAG (pi->flags, BGP_PATH_DMED_SELECTED)))
        {
          ...
          if (debug)
            zlog_debug ("%s: %pBD(%s) pi %s dmed", __func__, dest,
                        bgp->name_pretty, pi->peer->host);
          continue;
        }
      ...
      reason = dest->reason;
      if (bgp_path_info_cmp (bgp, pi, new_select, &paths_eq, mpath_cfg, debug,
                             pfx_buf, afi, safi, &dest->reason))
        {
          if (new_select == NULL && reason != bgp_path_selection_none)
            dest->reason = reason;
          new_select = pi;
        }
    }
  ...
  if (debug)
    {
      ...
      zlog_debug (
          "%pBD(%s): After path selection, newbest is %s oldbest was %s", dest,
          bgp->name_pretty, path_buf,
                                    ...
                                    safi_t safi, uint32_t addpath_tx_id)
{
  ...
  bgp = SUBGRP_INST (subgrp);
  onlypeer =
      ((SUBGRP_PCOUNT (subgrp) == 1) ? (SUBGRP_PFIRST (subgrp))->peer : NULL);
  ...
  if (selected)
    {
      if (subgroup_announce_check (dest, selected, subgrp, p, &attr, NULL))
        {
          ...
          if (advertise)
            {
              if (! bgp_check_withdrawal (bgp, dest))
                {
                  struct attr *adv_attr = bgp_attr_intern (&attr);

                  bgp_adj_out_set_subgroup (dest, subgrp, adv_attr, selected);
                }
              else
 ...
 */
void
bgp_zebra_clear_route_change_flags (struct bgp_dest *dest)
{
  struct bgp_path_info *pi;
 ...
 */
bool
bgp_zebra_has_route_changed (struct bgp_path_info *selected)
{
...
static void
bgp_process_evpn_route_injection (struct bgp *bgp, afi_t afi, safi_t safi,
                                  struct bgp_dest *dest,
                                  struct bgp_path_info *new_select,
                                  struct bgp_path_info *old_select)
{
      ...
      if (bgp->adv_cmd_rmap[afi][safi].map)
        {
          ...
          prep_for_rmap_apply (&rmap_path, &rmap_path_extra, dest, new_select,
                               new_select->peer, &dummy_attr);

          RESET_FLAG (dummy_attr.rmap_change_flags);

          ret = route_map_apply (bgp->adv_cmd_rmap[afi][safi].map, p,
                                 &rmap_path);
          ...
          if (ret == RMAP_DENYMATCH)
            ...
            bgp_evpn_advertise_type5_route (bgp, p, &dummy_attr, afi, safi);
        }
      else
        {
          bgp_evpn_advertise_type5_route (bgp, p, new_select->attr, afi, safi);
        }
    }
 ...
 */
static bool
bgp_lu_need_null_label (struct bgp *bgp,
                        const struct bgp_path_info *new_select, afi_t afi,
                        mpls_label_t *label)
{
 ...
 */
static void
bgp_process_main_one (struct bgp *bgp, struct bgp_dest *dest, afi_t afi,
                      safi_t safi)
{
  struct bgp_path_info *new_select;
  struct bgp_path_info *old_select;
  ...
  if (CHECK_FLAG (bgp->flags, BGP_FLAG_DELETE_IN_PROGRESS))
    {
      if (dest)
        debug = bgp_debug_bestpath (dest);
      if (debug)
        zlog_debug ("%s: bgp delete in progress, ignoring event, p=%pBD(%s)",
  ...
  debug = bgp_debug_bestpath (dest);
  if (debug)
    zlog_debug ("%s: p=%pBD(%s) afi=%s, safi=%s start", __func__, dest,
  ...
  if (CHECK_FLAG (dest->flags, BGP_NODE_SELECT_DEFER))
    {
      if (BGP_DEBUG (update, UPDATE_OUT))
        zlog_debug ("SELECT_DEFER flag set for route %p(%s)", dest,
                    bgp->name_pretty);
      return;
    }

  /* Best path selection. */
  bgp_best_selection (bgp, dest, &bgp->maxpaths[afi][safi], &old_and_new, afi,
                      safi);
              ...
              ! bgp_is_valid_label (&dest->local_label))
            {
              ...
              if (bgp_lu_need_null_label (bgp, new_select, afi,
                                          &mpls_label_null))
                {
                  ...
                  dest->local_label =
                      mpls_lse_encode (mpls_label_null, 0, 0, 1);
                  bgp_set_valid_label (&dest->local_label);
                }
              else
                bgp_register_for_label (dest, new_select);
            }
        }
  ...
  if (debug)
    zlog_debug ("%s: p=%pBD(%s) afi=%s, safi=%s, old_select=%p, new_select=%p",
  ...
  if (old_select && old_select == new_select &&
      ...
      ! bgp_addpath_is_addpath_used (&bgp->tx_addpath, afi, safi))
    {
...
#endif
          if (bgp_fibupd_safi (safi) && ! bgp_option_check (BGP_OPT_NO_FIB))
            {
              ...
              if (new_select->type == ZEBRA_ROUTE_BGP &&
                ...
                bgp_zebra_announce (dest, p, old_select, bgp, afi, safi);
            }
        }
      ...
      if (CHECK_FLAG (old_select->flags, BGP_PATH_LINK_BW_CHG) ||
        ...
        bgp_process_evpn_route_injection (bgp, afi, safi, dest, old_select,
                                          old_select);
  ...
  if (new_select)
    {
      ...
      bgp_path_info_unset_flag (dest, new_select, BGP_PATH_ATTR_CHANGED);
   ...
   */
  if (safi == SAFI_UNICAST)
    group_announce_route (bgp, afi, SAFI_LABELED_UNICAST, dest, new_select);
...
void
bgp_best_path_select_defer (struct bgp *bgp, afi_t afi, safi_t safi)
{
  struct bgp_dest *dest;
  int cnt = 0;
...
bgp_process_wq (struct work_queue *wq, void *data)
{
...
void
bgp_process_queue_init (struct bgp *bgp)
{
...
static struct bgp_process_queue *
bgp_processq_alloc (struct bgp *bgp)
{
  struct bgp_process_queue *pqnode;

  pqnode =
      XCALLOC (MTYPE_BGP_PROCESS_QUEUE, sizeof (struct bgp_process_queue));
...
void
bgp_process (struct bgp *bgp, struct bgp_dest *dest, afi_t afi, safi_t safi)
{
#define ARBITRARY_PROCESS_QLEN 10000
  ...
  if (CHECK_FLAG (dest->flags, BGP_NODE_SELECT_DEFER))
    {
      if (BGP_DEBUG (update, UPDATE_OUT))
        zlog_debug ("BGP_NODE_SELECT_DEFER set for route %p", dest);
      return;
    }
  ...
  if (CHECK_FLAG (dest->flags, BGP_NODE_SOFT_RECONFIG))
    {
      if (BGP_DEBUG (update, UPDATE_OUT))
        zlog_debug ("Soft reconfigure table in progress for route %p", dest);
      return;
    }
...
static void
bgp_maximum_prefix_restart_timer (struct event *thread)
{
  struct peer *peer;
...
static uint32_t
bgp_filtered_routes_count (struct peer *peer, afi_t afi, safi_t safi)
{
  uint32_t count = 0;
  bool filtered = false;
  struct bgp_dest *dest;
                             ...
                             int always)
{
  ...
  uint32_t pcount =
      (CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_FORCE))
      ...
      if (peer->pmax_restart[afi][safi])
        {
          ...
          if (bgp_debug_neighbor_events (peer))
            zlog_debug (
                "%pBP Maximum-prefix restart timer started for %d secs", peer,
                peer->v_pmax_restart);
  ...
  if (pcount > (peer->pmax[afi][safi] * peer->pmax_threshold[afi][safi] / 100))
    {
 ...
 */
void
bgp_rib_remove (struct bgp_dest *dest, struct bgp_path_info *pi,
...
static void
bgp_rib_withdraw (struct bgp_dest *dest, struct bgp_path_info *pi,
                  struct peer *peer, afi_t afi, safi_t safi,
  ...
  if (safi == SAFI_MPLS_VPN)
    {
      ...
      pdest = bgp_node_get (peer->bgp->rib[afi][safi], (struct prefix *) prd);
      if (bgp_dest_has_bgp_path_info_data (pdest))
        {
          table = bgp_dest_get_bgp_table_info (pdest);

          vnc_import_bgp_del_vnc_host_route_mode_resolve_nve (peer->bgp, prd,
                                                              table, p, pi);
        }
      bgp_dest_unlock_node (pdest);
    }
...
bool
bgp_update_martian_nexthop (struct bgp *bgp, afi_t afi, safi_t safi,
                            uint8_t type, uint8_t stype, struct attr *attr,
                            struct bgp_dest *dest)
{
  bool ret = false;
  bool is_bgp_static_route =
      (type == ZEBRA_ROUTE_BGP && stype == BGP_ROUTE_STATIC) ? true : false;
...
static void
bgp_attr_add_no_export_community (struct attr *attr)
{
  struct community *old;
  struct community *new;
  struct community *merge;
...
static bool
bgp_accept_own (struct peer *peer, afi_t afi, safi_t safi, struct attr *attr,
                const struct prefix *prefix, int *sub_type)
{
...
void
bgp_update (struct peer *peer, const struct prefix *p, uint32_t addpath_id,
            struct attr *attr, afi_t afi, safi_t safi, int type, int sub_type,
            struct prefix_rd *prd, mpls_label_t *label, uint32_t num_labels,
            int soft_reconfig, struct bgp_route_evpn *evpn)
{
  ...
  if (frrtrace_enabled (frr_bgp, process_update))
    {
      ...
      prefix2str (p, pfxprint, sizeof (pfxprint));
      frrtrace (6, frr_bgp, process_update, peer, pfxprint, addpath_id, afi,
                safi, attr);
    }
  ...
  if (! soft_reconfig &&
      ...
      peer != bgp->peer_self)
    {
      ...
      if ((afi == AFI_L2VPN) && evpn)
        {
          memcpy (&attr->evpn_overlay, evpn, sizeof (struct bgp_route_evpn));
        }
  ...
  for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
    if (pi->peer == peer && pi->type == type && pi->sub_type == sub_type &&
  ...
  if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_ORIGINATOR_ID) &&
    ...
    {
      accept_own = bgp_accept_own (peer, afi, safi, attr, p, &sub_type);
      if (! accept_own)
        {
  ...
  if (peer->bgp->reject_as_sets)
    if (aspath_check_as_sets (attr->aspath))
      {
        reason = "as-path contains AS_SET or AS_CONFED_SET type;";
        goto filtered;
      }
  ...
  if (soft_reconfig && (afi == AFI_L2VPN) && evpn)
    memcpy (&new_attr.evpn_overlay, evpn, sizeof (struct bgp_route_evpn));
  ...
  if (! CHECK_FLAG (peer->flags, PEER_FLAG_IS_RFAPI_HD) &&
      bgp_update_martian_nexthop (bgp, afi, safi, type, sub_type, &new_attr,
                                  dest))
    {
  ...
  if (peer->soo[afi][safi])
    {
      struct ecommunity *old_ecomm = bgp_attr_get_ecommunity (&new_attr);
      struct ecommunity *ecomm_soo = peer->soo[afi][safi];
      struct ecommunity *new_ecomm;

      if (old_ecomm)
        {
          new_ecomm = ecommunity_merge (ecommunity_dup (old_ecomm), ecomm_soo);
  ...
  if (pi)
    {
      ...
      if (! CHECK_FLAG (pi->flags, BGP_PATH_REMOVED) && same_attr &&
          (! has_valid_label ||
           memcmp (&(bgp_path_info_extra_get (pi))->label, label,
                   num_labels * sizeof (mpls_label_t)) == 0))
        {
              ...
              if (bgp_debug_update (peer, p, NULL, 1))
                {
                  bgp_debug_rdpfxpath2str (
                      afi, safi, prd, p, label, num_labels, addpath_id ? 1 : 0,
                      addpath_id, evpn, pfx_buf, sizeof (pfx_buf));
                  zlog_debug ("%pBP rcvd %s", peer, pfx_buf);
                }

              if (bgp_damp_update (pi, dest, afi, safi) != BGP_DAMP_SUPPRESSED)
                {
              ...
              if (bgp_debug_update (peer, p, NULL, 1))
                {
                  ...
                  bgp_debug_rdpfxpath2str (
                      afi, safi, prd, p, label, num_labels, addpath_id ? 1 : 0,
                      addpath_id, evpn, pfx_buf, sizeof (pfx_buf));
                  zlog_debug ("%pBP rcvd %s...duplicate ignored", peer,
                              pfx_buf);
                }
          ...
          if (bgp_debug_update (peer, p, NULL, 1))
            {
              ...
              zlog_debug ("%pBP rcvd %s, flapped quicker than processing",
                          peer, pfx_buf);
            }
       ...
       * community.
       */
      if (((safi == SAFI_EVPN) || (safi == SAFI_MPLS_VPN)) && ! same_attr)
        {
          if ((pi->attr->flag & ATTR_FLAG_BIT (BGP_ATTR_EXT_COMMUNITIES)) &&
              (attr_new->flag & ATTR_FLAG_BIT (BGP_ATTR_EXT_COMMUNITIES)))
            {
              int cmp;

              cmp = ecommunity_cmp (bgp_attr_get_ecommunity (pi->attr),
      ...
      if (attr->srv6_l3vpn)
        {
          extra = bgp_path_info_extra_get (pi);
          if (sid_diff (&extra->sid[0].sid, &attr->srv6_l3vpn->sid))
            {
              ...
              if (attr->srv6_l3vpn->loc_block_len != 0)
                {
                  ...
                  extra->sid[0].loc_node_len = attr->srv6_l3vpn->loc_node_len;
        ...
        {
          extra = bgp_path_info_extra_get (pi);
          if (sid_diff (&extra->sid[0].sid, &attr->srv6_vpn->sid))
            {
      ...
      if (((afi == AFI_IP || afi == AFI_IP6) &&
          ...
          (safi == SAFI_EVPN && bgp_evpn_is_prefix_nht_supported (p)))
        {
          ...
          if (bgp_find_or_add_nexthop (bgp, bgp_nexthop, nh_afi, safi, pi,
                                       NULL, connected,
       ...
       */
      if (safi == SAFI_EVPN && (! same_attr || force_evpn_import) &&
      ...
      if (attr->srv6_l3vpn)
        {
          ...
          extra->sid[0].loc_block_len = attr->srv6_l3vpn->loc_block_len;
          extra->sid[0].loc_node_len = attr->srv6_l3vpn->loc_node_len;
          extra->sid[0].func_len = attr->srv6_l3vpn->func_len;
          extra->sid[0].arg_len = attr->srv6_l3vpn->arg_len;
          extra->sid[0].transposition_len =
      ...
      if (bgp_dest_has_bgp_path_info_data (pdest))
        {
          ...
          vnc_import_bgp_add_vnc_host_route_mode_resolve_nve (bgp, prd, table,
                                                              p, new);
        }
      bgp_dest_unlock_node (pdest);
    }
  ...
  if (bgp_debug_update (peer, p, NULL, 1))
    {
      ...
      bgp_debug_rdpfxpath2str (afi, safi, prd, p, label, num_labels,
                               addpath_id ? 1 : 0, addpath_id, evpn, pfx_buf,
                               sizeof (pfx_buf));
      zlog_debug ("%pBP rcvd UPDATE about %s -- DENIED due to: %s", peer,
                  pfx_buf, reason);
    }
...
void
bgp_withdraw (struct peer *peer, const struct prefix *p, uint32_t addpath_id,
              ...
              struct bgp_route_evpn *evpn)
{
        ...
        if (bgp_debug_update (peer, p, NULL, 1))
          {
            bgp_debug_rdpfxpath2str (afi, safi, prd, p, label, num_labels,
                                     ...
                                     pfx_buf, sizeof (pfx_buf));
            zlog_debug ("%s withdrawing route %s not in adj-in", peer->host,
                        pfx_buf);
          }
        bgp_dest_unlock_node (dest);
        return;
  ...
  if (bgp_debug_update (peer, p, NULL, 1))
    {
      bgp_debug_rdpfxpath2str (afi, safi, prd, p, label, num_labels,
                               addpath_id ? 1 : 0, addpath_id, NULL, pfx_buf,
                               sizeof (pfx_buf));
      zlog_debug ("%pBP rcvd UPDATE about %s -- withdrawn", peer, pfx_buf);
    }
 ...
 */
static void
bgp_announce_route_timer_expired (struct event *t)
{
  struct peer_af *paf;
  struct peer *peer;
 ...
 */
void
bgp_announce_route (struct peer *peer, afi_t afi, safi_t safi, bool force)
{
  struct peer_af *paf;
  struct update_subgroup *subgrp;
 ...
 */
static void
bgp_soft_reconfig_table_flag (struct bgp_table *table, bool flag)
{
  struct bgp_dest *dest;
  struct bgp_adj_in *ain;
...
static void
bgp_soft_reconfig_table_update (struct peer *peer, struct bgp_dest *dest,
                                struct bgp_adj_in *ain, afi_t afi, safi_t safi,
                                struct prefix_rd *prd)
{
  struct bgp_path_info *pi;
  ...
  if (pi)
    memcpy (&evpn, bgp_attr_get_evpn_overlay (pi->attr), sizeof (evpn));
  else
    memset (&evpn, 0, sizeof (evpn));

  bgp_update (peer, bgp_dest_get_prefix (dest), ain->addpath_rx_id, ain->attr,
              afi, safi, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, prd, label_pnt,
              num_labels, 1, &evpn);
}
    ...
    for (ain = dest->adj_in; ain; ain = ain->next)
      {
        ...
        bgp_soft_reconfig_table_update (peer, dest, ain, afi, safi, prd);
      }
}
 ...
 */
static void
bgp_soft_reconfig_table_task (struct event *thread)
{
      ...
      for (ain = dest->adj_in; ain; ain = ain->next)
        {
          for (ALL_LIST_ELEMENTS (table->soft_reconfig_peers, node, nnode,
                                  peer))
            {
              ...
              bgp_soft_reconfig_table_update (peer, dest, ain, table->afi,
                                              table->safi, prd);
              iter++;
            }
        }
  ...
  if (dest || table->soft_reconfig_init)
    {
      ...
      event_add_event (bm->master, bgp_soft_reconfig_table_task, table, 0,
 ...
 */
void
bgp_soft_reconfig_table_task_cancel (const struct bgp *bgp,
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      ...
      for (ALL_LIST_ELEMENTS (ntable->soft_reconfig_peers, node, nnode, npeer))
        {
          if (peer && peer != npeer)
            continue;
  ...
  if ((safi != SAFI_MPLS_VPN) && (safi != SAFI_ENCAP) && (safi != SAFI_EVPN))
    {
       ...
       * there
       */
      for (ALL_LIST_ELEMENTS (table->soft_reconfig_peers, node, nnode, npeer))
        {
          if (peer == npeer)
            break;
        }
      ...
      for (dest = bgp_table_top (peer->bgp->rib[afi][safi]); dest;
           dest = bgp_route_next (dest))
        {
          ...
          for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
            for (pi = bgp_dest_get_bgp_path_info (rm); pi; pi = pi->next)
              {
                if (pi->peer != peer)
                  continue;
                if (CHECK_FLAG (peer->af_sflags[afi][safi],
        ...
        for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
          {
            ...
            if (safi == SAFI_UNICAST &&
                (peer->bgp->inst_type == BGP_INSTANCE_TYPE_VRF ||
                 peer->bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT))
              vpn_leak_from_vrf_withdraw (bgp_get_default (), peer->bgp, pi);
...
void
bgp_set_stale_route (struct peer *peer, afi_t afi, safi_t safi)
{
      ...
      for (dest = bgp_table_top (peer->bgp->rib[afi][safi]); dest;
           dest = bgp_route_next (dest))
        {
          ...
          for (ndest = bgp_table_top (table); ndest;
            ...
            {
              for (pi = bgp_dest_get_bgp_path_info (ndest); pi; pi = pi->next)
                {
                  if (pi->peer != peer)
                    continue;
                      ...
                      ! CHECK_FLAG (pi->flags, BGP_PATH_UNUSEABLE))
                    {
                      if (bgp_debug_neighbor_events (peer))
                        zlog_debug (
                            ...
                            bgp_dest_get_prefix (ndest));

                      bgp_path_info_set_flag (ndest, pi, BGP_PATH_STALE);
                    }
                }
            }
        }
...
bool
bgp_outbound_policy_exists (struct peer *peer, struct bgp_filter *filter)
{
  if (peer->sort == BGP_PEER_IBGP)
    return true;
...
bool
bgp_inbound_policy_exists (struct peer *peer, struct bgp_filter *filter)
{
  if (peer->sort == BGP_PEER_IBGP)
    return true;
  ...
  if (peer->sort == BGP_PEER_EBGP &&
      (ROUTE_MAP_IN_NAME (filter) || PREFIX_LIST_IN_NAME (filter) ||
       FILTER_LIST_IN_NAME (filter) || DISTRIBUTE_IN_NAME (filter)))
    return true;
  return false;
}

static void
bgp_cleanup_table (struct bgp *bgp, struct bgp_table *table, safi_t safi)
{
  struct bgp_dest *dest;
  struct bgp_path_info *pi;
    ...
    for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = next)
      {
        ...
        if (safi == SAFI_EVPN)
          bgp_evpn_unimport_route (bgp, AFI_L2VPN, SAFI_EVPN, p, pi);
  ...
  for (afi = AFI_IP; afi < AFI_MAX; ++afi)
    {
      if (afi == AFI_L2VPN)
        continue;
      bgp_cleanup_table (bgp, bgp->rib[afi][SAFI_UNICAST], SAFI_UNICAST);
      /*
...
void
bgp_static_update (struct bgp *bgp, const struct prefix *p,
                   struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
      ...
      else
        {
          ...
          if (CHECK_FLAG (bgp->flags, BGP_FLAG_IMPORT_CHECK) &&
              (safi == SAFI_UNICAST || safi == SAFI_LABELED_UNICAST))
            {
              ...
              if (bgp_find_or_add_nexthop (bgp, bgp_nexthop, afi, safi, pi,
                                           NULL, 0, p))
                bgp_path_info_set_flag (dest, pi, BGP_PATH_VALID);
              else
                {
                  if (BGP_DEBUG (nht, NHT))
                    {
                      ...
                      inet_ntop (p->family, &p->u.prefix, buf1, sizeof (buf1));
                      zlog_debug (
  ...
  if (CHECK_FLAG (bgp->flags, BGP_FLAG_IMPORT_CHECK) &&
      (safi == SAFI_UNICAST || safi == SAFI_LABELED_UNICAST))
    {
      if (bgp_find_or_add_nexthop (bgp, bgp, afi, safi, new, NULL, 0, p))
        bgp_path_info_set_flag (dest, new, BGP_PATH_VALID);
      else
        {
          if (BGP_DEBUG (nht, NHT))
...
void
bgp_static_withdraw (struct bgp *bgp, const struct prefix *p, afi_t afi,
                     safi_t safi)
{
  struct bgp_dest *dest;
  struct bgp_path_info *pi;
 ...
 */
static void
bgp_static_withdraw_safi (struct bgp *bgp, const struct prefix *p, afi_t afi,
...
static void
bgp_static_update_safi (struct bgp *bgp, const struct prefix *p,
                        struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
  struct bgp_dest *dest;
  struct bgp_path_info *new;
  ...
  if (bgp_static->label != MPLS_INVALID_LABEL)
    num_labels = 1;
  dest =
      bgp_afi_node_get (bgp->rib[afi][safi], afi, safi, p, &bgp_static->prd);
      ...
      if (bgp_static->gatewayIp.family == AF_INET)
        {
          ...
          memcpy (&attr.evpn_overlay.gw_ip.ipaddr_v4,
                  &bgp_static->gatewayIp.u.prefix4, IPV4_MAX_BYTELEN);
        }
      else if (bgp_static->gatewayIp.family == AF_INET6)
        {
          ...
          memcpy (&attr.evpn_overlay.gw_ip.ipaddr_v6,
                  &bgp_static->gatewayIp.u.prefix6, IPV6_MAX_BYTELEN);
        }
      ...
      if (attrhash_cmp (pi->attr, attr_new) &&
      ...
      else
        {
          ...
          if (SAFI_MPLS_VPN == safi &&
              bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT)
            {
              vpn_leak_to_vrf_update (bgp, pi, &bgp_static->prd);
            }
#ifdef ENABLE_BGP_VNC
          rfapiProcessUpdate (pi->peer, NULL, p, &bgp_static->prd, pi->attr,
                              afi, safi, pi->type, pi->sub_type, &label);
#endif
          bgp_dest_unlock_node (dest);
          aspath_unintern (&attr.aspath);
          return;
...
static int
bgp_static_set (struct vty *vty, const char *negate, const char *ip_str,
  ...
  if (negate)
    {
      ...
      if ((rmap && bgp_static->rmap.name) &&
        ...
        {
          vty_out (vty, "%% route-map name doesn't match static route\n");
          bgp_dest_unlock_node (dest);
          return CMD_WARNING_CONFIG_FAILED;
        }
         ...
         dest = bgp_route_next (dest))
      {
        ...
        if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP) ||
            (safi == SAFI_EVPN))
          {
            ...
            for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
              {
                bgp_static = bgp_dest_get_bgp_static_info (rm);
                bgp_static_update_safi (bgp, bgp_dest_get_prefix (rm),
                                        bgp_static, afi, safi);
              }
          }
        else
    ...
    for (dest = bgp_table_top (bgp->route[afi][safi]); dest;
         dest = bgp_route_next (dest))
      {
        ...
        if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP) ||
            ...
            for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
              {
                ...
                bgp_static_withdraw_safi (
                    bgp, bgp_dest_get_prefix (rm), AFI_IP, safi,
                    (struct prefix_rd *) bgp_dest_get_prefix (dest));
                bgp_static_free (bgp_static);
                bgp_dest_set_bgp_static_info (rm, NULL);
          ...
          {
            bgp_static = bgp_dest_get_bgp_static_info (dest);
            bgp_static_withdraw (bgp, bgp_dest_get_prefix (dest), afi, safi);
            bgp_static_free (bgp_static);
            bgp_dest_set_bgp_static_info (dest, NULL);
            bgp_dest_unlock_node (dest);
          }
      ...
      for (dest = bgp_table_top (bgp->route[afi][safi]); dest;
           dest = bgp_route_next (dest))
        {
          ...
          if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP) ||
            ...
            {
              bgp_static = bgp_dest_get_bgp_static_info (dest);
              bgp_static_update (bgp, bgp_dest_get_prefix (dest), bgp_static,
                                 afi, safi);
            }
        }
    }
      ...
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
          if (pi->peer == bgp->peer_self &&
            ...
            {
              bgp_aggregate_decrement (bgp, bgp_dest_get_prefix (dest), pi,
                                       afi, safi);
              bgp_unlink_nexthop (pi);
              bgp_path_info_delete (dest, pi);
              bgp_process (bgp, dest, afi, safi);
 ...
 */
int
bgp_static_set_safi (afi_t afi, safi_t safi, struct vty *vty,
...
int
bgp_static_unset_safi (afi_t afi, safi_t safi, struct vty *vty,
                       const char *ip_str, const char *rd_str,
 ...
 */
static bool
aggr_suppress_map_test (struct bgp *bgp, struct bgp_aggregate *aggregate,
                        struct bgp_path_info *pi)
{
...
static bool
aggr_suppress_exists (struct bgp_aggregate *aggregate,
                      struct bgp_path_info *pi)
{
  if (pi->extra == NULL || pi->extra->aggr_suppressors == NULL)
    return false;
 ...
 */
static bool
aggr_suppress_path (struct bgp_aggregate *aggregate, struct bgp_path_info *pi)
{
 ...
 */
static bool
aggr_unsuppress_path (struct bgp_aggregate *aggregate,
                      struct bgp_path_info *pi)
{
...
static bool
bgp_aggregate_info_same (struct bgp_path_info *pi, uint8_t origin,
                         struct aspath *aspath, struct community *comm,
                         struct ecommunity *ecomm, struct lcommunity *lcomm)
{
...
static void
bgp_aggregate_install (struct bgp *bgp, afi_t afi, safi_t safi,
                       const struct prefix *p, uint8_t origin,
                       struct aspath *aspath, struct community *community,
                       struct ecommunity *ecommunity,
                       struct lcommunity *lcommunity, uint8_t atomic_aggregate,
  ...
  if (aggregate->count > 0)
    {
      ...
      attr = bgp_attr_aggregate_intern (bgp, origin, aspath, community,
                                        ecommunity, lcommunity, aggregate,
                                        atomic_aggregate, p);
 ...
 */
static bool
bgp_aggregate_med_match (struct bgp_aggregate *aggregate, struct bgp *bgp,
                         struct bgp_path_info *pi)
{
 ...
 */
static bool
bgp_aggregate_test_all_med (struct bgp_aggregate *aggregate, struct bgp *bgp,
 ...
 */
void
bgp_aggregate_toggle_suppressed (struct bgp_aggregate *aggregate,
 ...
 */
static void
bgp_aggregate_med_update (struct bgp_aggregate *aggregate, struct bgp *bgp,
                          const struct prefix *p, afi_t afi, safi_t safi,
                          struct bgp_path_info *pi)
{
  ...
  if (aggregate->med_mismatched &&
    ...
    bgp_aggregate_toggle_suppressed (aggregate, bgp, p, afi, safi, true);
  else
...
bool
bgp_aggregate_route (struct bgp *bgp, const struct prefix *p, afi_t afi,
                     safi_t safi, struct bgp_aggregate *aggregate)
{
      ...
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
          ...
          if (pi->attr->flag & ATTR_FLAG_BIT (BGP_ATTR_ATOMIC_AGGREGATE))
            atomic_aggregate = 1;
           ...
           */
          bgp_compute_aggregate_aspath_hash (aggregate, pi->attr->aspath);
  ...
  bgp_aggregate_install (bgp, afi, safi, p, origin, aspath, community,
                         ecommunity, lcommunity, atomic_aggregate, aggregate);

  return true;
}

void
bgp_aggregate_delete (struct bgp *bgp, const struct prefix *p, afi_t afi,
                      safi_t safi, struct bgp_aggregate *aggregate)
{
  struct bgp_table *table;
  struct bgp_dest *top;
          ...
          if (aggregate->as_set)
            {
               ...
               */
              bgp_remove_aspath_from_aggregate_hash (aggregate,
                                                     pi->attr->aspath);
...
static void
bgp_add_route_to_aggregate (struct bgp *bgp, const struct prefix *aggr_p,
                            ...
                            safi_t safi, struct bgp_aggregate *aggregate)
{
   ...
   */
  if (aggregate->match_med)
    bgp_aggregate_med_update (aggregate, bgp, aggr_p, afi, safi, pinew);
  ...
  if (aggregate->as_set)
    {
      /* Compute aggregate route's as-path.
       */
      bgp_compute_aggregate_aspath (aggregate, pinew->attr->aspath);

      /* Compute aggregate route's community.
       */
      if (bgp_attr_get_community (pinew->attr))
        bgp_compute_aggregate_community (aggregate,
                                         bgp_attr_get_community (pinew->attr));

      /* Compute aggregate route's extended community.
       */
      if (bgp_attr_get_ecommunity (pinew->attr))
        bgp_compute_aggregate_ecommunity (
            aggregate, bgp_attr_get_ecommunity (pinew->attr));

      /* Compute aggregate route's large community.
       */
      if (bgp_attr_get_lcommunity (pinew->attr))
        bgp_compute_aggregate_lcommunity (
            aggregate, bgp_attr_get_lcommunity (pinew->attr));
  ...
  bgp_aggregate_install (bgp, afi, safi, aggr_p, origin, aspath, community,
                         ecommunity, lcommunity, atomic_aggregate, aggregate);
}

static void
bgp_remove_route_from_aggregate (struct bgp *bgp, afi_t afi, safi_t safi,
                                 struct bgp_path_info *pi,
                                 struct bgp_aggregate *aggregate,
  ...
  if (aggregate->as_set)
    {
      /* Remove as-path from aggregate.
       */
      bgp_remove_aspath_from_aggregate (aggregate, pi->attr->aspath);
...
void
bgp_aggregate_increment (struct bgp *bgp, const struct prefix *p,
...
void
bgp_aggregate_decrement (struct bgp *bgp, const struct prefix *p,
  ...
  for (dest = child; dest; dest = bgp_dest_parent_nolock (dest))
    {
      ...
      if (aggregate != NULL && dest_p->prefixlen < p->prefixlen)
        {
          bgp_remove_route_from_aggregate (bgp, afi, safi, del, aggregate,
                                           dest_p);
        }
    }
  bgp_dest_unlock_node (child);
...
static const char *
bgp_rpki_validation2str (enum rpki_states v_state)
{
  switch (v_state)
    {
...
static int
bgp_aggregate_unset (struct vty *vty, const char *prefix_str, afi_t afi,
                     safi_t safi)
{
  VTY_DECLVAR_CONTEXT (bgp, bgp);
  int ret;
  ...
  aggregate = bgp_dest_get_bgp_aggregate_info (dest);
  bgp_aggregate_delete (bgp, &p, afi, safi, aggregate);
  bgp_aggregate_install (bgp, afi, safi, &p, 0, NULL, NULL, NULL, NULL, 0,
                         aggregate);
...
static int
bgp_aggregate_set (struct vty *vty, const char *prefix_str, afi_t afi,
...
void
bgp_free_aggregate_info (struct bgp_aggregate *aggregate)
{
  if (aggregate->community)
    community_free (&aggregate->community);

  hash_clean_and_free (&aggregate->community_hash, bgp_aggr_community_remove);
       ...
       "Route map with the route selectors\n")
{
  ...
  /* Handle configuration removal, otherwise installation. */
  if (no)
    return bgp_aggregate_unset (vty, prefix_str, AFI_IP6, SAFI_UNICAST);
...
bgp_redistribute_add (struct bgp *bgp, struct prefix *p,
                      ...
                      uint8_t type, unsigned short instance, route_tag_t tag)
{
  struct bgp_path_info *new;
      ...
      if (bpi)
        {
          ...
          if (attrhash_cmp (bpi->attr, new_attr) &&
          ...
          else
            {
              ...
              /* Process change. */
              bgp_aggregate_increment (bgp, p, bpi, afi, SAFI_UNICAST);
              ...
              if ((bgp->inst_type == BGP_INSTANCE_TYPE_VRF) ||
                  (bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT))
                {
                  ...
                  vpn_leak_from_vrf_update (bgp_get_default (), bgp, bpi);
                }
              return;
            }
        }
...
void
bgp_redistribute_delete (struct bgp *bgp, struct prefix *p, uint8_t type,
                         unsigned short instance)
{
  ...
  if (red)
    {
      dest = bgp_afi_node_get (bgp->rib[afi][SAFI_UNICAST], afi, SAFI_UNICAST,
                               p, NULL);
      ...
      if (pi)
        {
          ...
          bgp_aggregate_decrement (bgp, bgp_dest_get_prefix (dest), pi, afi,
                                   SAFI_UNICAST);
          bgp_path_info_delete (dest, pi);
          if (! CHECK_FLAG (bgp->flags, BGP_FLAG_DELETE_IN_PROGRESS))
            bgp_process (bgp, dest, afi, SAFI_UNICAST);
...
static void
route_vty_out_route (struct bgp_dest *dest, const struct prefix *p,
  ...
  if (p->family == AF_INET)
  ...
  else if (p->family == AF_FLOWSPEC)
    {
      route_vty_out_flowspec (vty, p, NULL,
                              json ? NLRI_STRING_FORMAT_JSON_SIMPLE
...
static void
route_vty_short_status_out (struct vty *vty, struct bgp_path_info *path,
                            const struct prefix *p, json_object *json_path)
{
  enum rpki_states rpki_state = RPKI_NOT_BEING_USED;
  ...
  if (json_path)
    {
      ...
      if (CHECK_FLAG (path->flags, BGP_PATH_SELECTED))
        {
          ...
          json_object_string_add (
              json_path, "selectionReason",
              bgp_path_selection_reason2str (path->net->reason));
        }
      ...
      /* Internal route. */
      if ((path->peer->as) && (path->peer->as == path->peer->local_as))
...
static char *
bgp_nexthop_hostname (struct peer *peer, struct bgp_nexthop_cache *bnc)
{
  if (peer->hostname &&
...
void
route_vty_out (struct vty *vty, const struct prefix *p,
               ...
               json_object *json_paths, bool wide)
{
  ...
  if (path->extra && path->extra->bgp_orig)
    {
      ...
      if (path->extra->bgp_orig->vrf_id == VRF_UNKNOWN)
        snprintf (vrf_id_str, sizeof (vrf_id_str), "@%s%s", VRFID_NONE_STR,
                  self);
      else
        snprintf (vrf_id_str, sizeof (vrf_id_str), "@%u%s",
                  path->extra->bgp_orig->vrf_id, self);

      if (path->extra->bgp_orig->inst_type != BGP_INSTANCE_TYPE_DEFAULT)
  ...
  if ((safi == SAFI_ENCAP) || (safi == SAFI_MPLS_VPN))
          ...
          if (json_paths)
          ...
          else
            {
              if (nexthop_hostname)
                ...
                len = vty_out (vty, "%pI4%s", &attr->nexthop, vrf_id_str);
  ...
  else if (p->family == AF_INET && ! BGP_ATTR_MP_NEXTHOP_LEN_IP6 (attr))
    {
      if (json_paths)
        ...
        {
          if (nexthop_hostname)
            len = vty_out (vty, "%pI4(%s)%s", &attr->nexthop, nexthop_hostname,
                           vrf_id_str);
          else
            len = vty_out (vty, "%pI4%s", &attr->nexthop, vrf_id_str);
          ...
          if (((attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV6_GLOBAL_AND_LL) &&
              ...
              (path->peer->conf_if))
            {
              if (path->peer->conf_if)
                ...
                {
                  if (nexthop_hostname)
                    len = vty_out (vty, "%pI6(%s)%s", &attr->mp_nexthop_local,
                                   nexthop_hostname, vrf_id_str);
                  else
                    len = vty_out (vty, "%pI6%s", &attr->mp_nexthop_local,
                                   vrf_id_str);
            ...
            {
              if (nexthop_hostname)
                len = vty_out (vty, "%pI6(%s)%s", &attr->mp_nexthop_global,
                               nexthop_hostname, vrf_id_str);
              else
                len = vty_out (vty, "%pI6%s", &attr->mp_nexthop_global,
                               vrf_id_str);
  ...
  if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_LOCAL_PREF))
    if (json_paths)
      json_object_int_add (json_path, "locPrf", attr->local_pref);
    else
  ...
  if (json_paths)
    json_object_string_addf (json_path, "peerId", "%pSU", &path->peer->su);

  /* Print aspath */
  if (attr->aspath)
    {
      if (json_paths)
        json_object_string_add (json_path, "path", attr->aspath->str);
      else
  ...
  if (json_paths)
    {
      ...
      if (nexthop_othervrf)
        {
          json_object_string_add (json_path, "nhVrfName", nexthop_vrfname);

          json_object_int_add (
              json_path, "nhVrfId",
              ((nexthop_vrfid == VRF_UNKNOWN) ? -1 : (int) nexthop_vrfid));
        }
    }
...
/* called from terminal list command */
void
route_vty_out_tmp (struct vty *vty, struct bgp_dest *dest,
      ...
      if (use_json)
        {
          if (p->family == AF_INET &&
            ...
            {
              json_object_string_addf (json_net, "nextHopGlobal", "%pI6",
                                       &attr->mp_nexthop_global);
            }
          else if (p->family == AF_EVPN && ! BGP_ATTR_NEXTHOP_AFI_IP6 (attr))
            {
              json_object_string_addf (json_net, "nextHop", "%pI4",
                                       &attr->mp_nexthop_global_in);
            }
          ...
          if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_LOCAL_PREF))
            json_object_int_add (json_net, "locPrf", attr->local_pref);

          json_object_int_add (json_net, "weight", attr->weight);

          /* Print aspath */
          if (attr->aspath)
            json_object_string_add (json_net, "path", attr->aspath->str);
          ...
          json_object_string_add (json_net, "origin",
                                  bgp_origin_long_str[attr->origin]);
        }
      else
        {
          if (p->family == AF_INET &&
            ...
            {
              len = vty_out (vty, "%pI6", &attr->mp_nexthop_global);
              len = wide ? (41 - len) : (16 - len);
              if (len < 1)
                vty_out (vty, "\n%*s", 36, " ");
              else
...
void
route_vty_out_tag (struct vty *vty, const struct prefix *p,
                   struct bgp_path_info *path, int display, safi_t safi,
                   json_object *json)
{
  ...
  if (((p->family == AF_INET) &&
      ...
      (! BGP_ATTR_MP_NEXTHOP_LEN_IP6 (attr)))
    {
      if (safi == SAFI_MPLS_VPN || safi == SAFI_ENCAP || safi == SAFI_EVPN)
        {
          if (json)
            json_object_string_addf (json_out, "mpNexthopGlobalIn", "%pI4",
                                     &attr->mp_nexthop_global_in);
          else
            vty_out (vty, "%-16pI4", &attr->mp_nexthop_global_in);
        }
        ...
        {
          if (json)
            json_object_string_addf (json_out, "nexthop", "%pI4",
                                     &attr->nexthop);
          else
            vty_out (vty, "%-16pI4", &attr->nexthop);
        }
           ...
           (BGP_ATTR_MP_NEXTHOP_LEN_IP6 (attr)))
    {
      ...
      if (attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV6_GLOBAL)
        {
          if (json)
            json_object_string_addf (json_out, "mpNexthopGlobalIn", "%pI6",
                                     &attr->mp_nexthop_global);
          else
            vty_out (vty, "%s",
                     inet_ntop (AF_INET6, &attr->mp_nexthop_global, buf_a,
                                sizeof (buf_a)));
        }
      else if (attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV6_GLOBAL_AND_LL)
        {
          ...
          if (json)
            json_object_string_add (json_out, "mpNexthopGlobalLocal", buf_a);
          else
            vty_out (vty, "%s", buf_a);
        }
    }
...
void
route_vty_out_overlay (struct vty *vty, const struct prefix *p,
    ...
    case AF_INET:
      if (! json_path)
      ...
      else
        {
          ...
          json_object_object_add (json_path, "nexthop", json_nexthop);
        }
      break;
    case AF_INET6:
      if (! json_path)
        ...
        {
          json_object_string_addf (json_nexthop, "ipv6Global", "%pI6",
                                   &attr->mp_nexthop_global);

          json_object_string_addf (json_nexthop, "ipv6LinkLocal", "%pI6",
                                   &attr->mp_nexthop_local);
      ...
      if (mac)
        {
          if (! json_path)
            ...
            {
              json_object_string_add (json_overlay, "rmac", mac);
            }
          XFREE (MTYPE_TMP, mac);
        }
    }
...
/* dampening route */
static void
damp_route_vty_out (struct vty *vty, const struct prefix *p,
                    struct bgp_path_info *path, int display, afi_t afi,
                    safi_t safi, bool use_json, json_object *json_paths)
{
  struct attr *attr = path->attr;
  int len;
  ...
  if (! use_json)
    ...
    {
      bgp_damp_reuse_time_vty (vty, path, timebuf, BGP_UPTIME_LEN, afi, safi,
                               use_json, json_path);
...
/* flap route */
static void
flap_route_vty_out (struct vty *vty, const struct prefix *p,
                    struct bgp_path_info *path, int display, afi_t afi,
                    safi_t safi, bool use_json, json_object *json_paths)
{
  struct attr *attr = path->attr;
  ...
  if (! use_json)
    {
      ...
      vty_out (
          vty, "%s ",
          peer_uptime (bdi->start_time, timebuf, BGP_UPTIME_LEN, 0, NULL));
  ...
  if (json_adv_to)
    {
      ...
      if (peer->conf_if)
        ...
        json_object_object_addf (json_adv_to, json_peer, "%pSU", &peer->su);
    }
  else
    {
      if (*first)
      ...
      if (peer->hostname &&
          CHECK_FLAG (peer->bgp->flags, BGP_FLAG_SHOW_HOSTNAME))
        {
          if (peer->conf_if)
            ...
            vty_out (vty, " %s(%pSU)", peer->hostname, &peer->su);
        }
      else
        {
          if (peer->conf_if)
...
static void
route_vty_out_tx_ids (struct vty *vty, struct bgp_addpath_info_data *d)
{
  int i;
  ...
  for (i = 0; i < BGP_ADDPATH_MAX; i++)
    {
      vty_out (vty, "TX-%s %u%s", bgp_addpath_names (i)->human_name,
               d->addpath_tx_id[i], i < BGP_ADDPATH_MAX - 1 ? " " : "\n");
    }
}

static void
route_vty_out_detail_es_info (struct vty *vty, struct bgp_path_info *pi,
                              struct attr *attr, json_object *json_path)
{
  char esi_buf[ESI_STR_LEN];
  bool es_local = ! ! CHECK_FLAG (attr->es_flags, ATTR_ES_IS_LOCAL);
  bool peer_router = ! ! CHECK_FLAG (attr->es_flags, ATTR_ES_PEER_ROUTER);
  bool peer_active = ! ! CHECK_FLAG (attr->es_flags, ATTR_ES_PEER_ACTIVE);
  bool peer_proxy = ! ! CHECK_FLAG (attr->es_flags, ATTR_ES_PEER_PROXY);
  esi_to_str (&attr->esi, esi_buf, sizeof (esi_buf));
  if (json_path)
    {
      json_object *json_es_info = NULL;

      json_object_string_add (json_path, "esi", esi_buf);
      if (es_local || bgp_evpn_attr_is_sync (attr))
        {
          json_es_info = json_object_new_object ();
          if (es_local)
            json_object_boolean_true_add (json_es_info, "localEs");
          if (peer_active)
            json_object_boolean_true_add (json_es_info, "peerActive");
          if (peer_proxy)
            json_object_boolean_true_add (json_es_info, "peerProxy");
          if (peer_router)
            json_object_boolean_true_add (json_es_info, "peerRouter");
          if (attr->mm_sync_seqnum)
            json_object_int_add (json_es_info, "peerSeq",
                                 attr->mm_sync_seqnum);
          json_object_object_add (json_path, "es_info", json_es_info);
        }
    }
  else
    {
      if (bgp_evpn_attr_is_sync (attr))
        vty_out (vty, "      ESI %s %s peer-info: (%s%s%sMM: %d)\n", esi_buf,
                 es_local ? "local-es" : "", peer_proxy ? "proxy " : "",
                 peer_active ? "active " : "", peer_router ? "router " : "",
                 attr->mm_sync_seqnum);
      else
        vty_out (vty, "      ESI %s %s\n", esi_buf,
                 es_local ? "local-es" : "");
...
void
route_vty_out_detail (struct vty *vty, struct bgp *bgp, struct bgp_dest *bn,
                      const struct prefix *p, struct bgp_path_info *path,
                      afi_t afi, safi_t safi, enum rpki_states rpki_curr_state,
                      json_object *json_paths)
{
  char buf[INET6_ADDRSTRLEN];
  char tag_buf[30];
  ...
  mpls_label_t label = MPLS_INVALID_LABEL;
  tag_buf[0] = '\0';
  struct bgp_path_info *bpi_ultimate = bgp_get_imported_bpi_ultimate (path);
  ...
  if (safi == SAFI_EVPN && attr->evpn_overlay.type == OVERLAY_INDEX_GATEWAY_IP)
    {
      char gwip_buf[INET6_ADDRSTRLEN];

      ipaddr2str (&attr->evpn_overlay.gw_ip, gwip_buf, sizeof (gwip_buf));
          ...
          if (is_pi_family_evpn (parent_ri))
            {
              ...
              vty_out (vty, BGP_RD_AS_FORMAT (bgp->asnotation),
                       (struct prefix_rd *) bgp_dest_get_prefix (pdest));
              vty_out (vty, ":%pFX, VNI %s",
                       (struct prefix_evpn *) bgp_dest_get_prefix (dest),
                       tag_buf);
              if (CHECK_FLAG (attr->es_flags, ATTR_ES_L3_NHG))
                vty_out (vty, ", L3NHG %s",
                         CHECK_FLAG (attr->es_flags, ATTR_ES_L3_NHG_ACTIVE)
                             ? "active"
                             : "inactive");
              vty_out (vty, "\n");
            }
      ...
      if (json_paths)
        {
          ...
          json_object_string_addf (json_path, "aggregatorId", "%pI4",
                                   &attr->aggregator_addr);
        }
      else
        {
          vty_out (vty, ", (aggregated by %u %pI4)", attr->aggregator_as,
                   &attr->aggregator_addr);
        }
    }

  if (CHECK_FLAG (path->peer->af_flags[afi][safi], PEER_FLAG_REFLECTOR_CLIENT))
    {
      if (json_paths)
  ...
  if (CHECK_FLAG (path->peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
    {
      if (json_paths)
          ...
          if (path->peer->conf_if)
            {
              if (path->peer->hostname &&
                ...
                vty_out (vty, " from %s(%s)", path->peer->hostname,
                         path->peer->conf_if);
              else
                vty_out (vty, " from %s", path->peer->conf_if);
            }
          else
            {
              if (path->peer->hostname &&
                ...
                vty_out (vty, " from %pSU", &path->peer->su);
            }
      ...
      if (json_paths)
        {
          ...
          if (nexthop_vrfid == VRF_UNKNOWN)
            ...
            {
              json_object_int_add (json_path, "nhVrfId", (int) nexthop_vrfid);
            }
        }
      else
        {
  ...
  if (path->peer != bgp->peer_self)
    {
      if (path->peer->as == path->peer->local_as)
        ...
        {
          if (bgp_confederation_peers_check (bgp, path->peer->as))
            {
              if (json_paths)
      ...
      if (json_paths)
        {
          ...
          json_object_string_add (json_bestpath, "selectionReason",
                                  bgp_path_selection_reason2str (bn->reason));
        }
      else
        {
          vty_out (vty, ", best");
          vty_out (vty, " (%s)", bgp_path_selection_reason2str (bn->reason));
        }
    }
  ...
  if (rpki_curr_state != RPKI_NOT_BEING_USED)
    {
      if (json_paths)
        json_object_string_add (json_path, "rpkiValidationState",
  ...
  if ((attr->flag & ATTR_FLAG_BIT (BGP_ATTR_ORIGINATOR_ID)) ||
      (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_CLUSTER_LIST)))
    {
      ...
      if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_ORIGINATOR_ID))
        {
          if (json_paths)
            json_object_string_addf (json_path, "originatorId", "%pI4",
                                     &attr->originator_id);
          else
            vty_out (vty, "      Originator: %pI4", &attr->originator_id);
        }
          ...
          if (json_paths)
            {
              ...
              for (i = 0; i < cluster->length / 4; i++)
                {
                  json_string = json_object_new_string (inet_ntop (
                      AF_INET, &cluster->list[i], buf, sizeof (buf)));
               ...
               * "string", cluster->str);
               */
              json_object_object_add (json_cluster_list, "list",
          ...
          else
            {
              ...
              for (i = 0; i < cluster->length / 4; i++)
                {
                  vty_out (vty, "%pI4 ", &cluster->list[i]);
                }
            }
        }
  ...
  if (path->extra && bgp_is_valid_label (&path->extra->label[0]) &&
    ...
    {
      mpls_lse_decode (path->extra->label[0], &label, &ttl, &exp, &bos);
  ...
  if (path->addpath_rx_id || bgp_addpath_info_has_ids (&path->tx_addpath))
    {
      if (json_paths)
        {
          json_object_int_add (json_path, "addpathRxId", path->addpath_rx_id);
          ...
          json_object_int_add (
              ...
              path->tx_addpath.addpath_tx_id[BGP_ADDPATH_ALL]);
        ...
        {
          vty_out (vty, "      AddPath ID: RX %u, ", path->addpath_rx_id);
          ...
          if ((addpath_capable && has_adj) ||
               ...
               CHECK_FLAG (path->flags, BGP_PATH_SELECTED)))
            {
              ...
              route_vty_out_advertised_to (
                  vty, peer, &first, "      Advertised to:", json_adv_to);
            }
        }
  ...
  if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_PMSI_TUNNEL))
    {
      const char *str =
          lookup_msg (bgp_pmsi_tnltype_str, bgp_attr_get_pmsi_tnl_type (attr),
      ...
      if (json_paths)
        {
          ...
          json_object_int_add (json_pmsi, "label", label2vni (&attr->label));
  ...
  if (path->peer->t_gr_restart && CHECK_FLAG (path->flags, BGP_PATH_STALE))
    {
  ...
  if (path->peer->t_llgr_stale[afi][safi] && bgp_attr_get_community (attr) &&
      community_include (bgp_attr_get_community (attr), COMMUNITY_LLGR_STALE))
    {
      unsigned long llgr_remaining =
          event_timer_remain_second (path->peer->t_llgr_stale[afi][safi]);
      ...
      if (json_paths)
        ...
        vty_out (vty, "      Time until Long-lived stale route deleted: %lu\n",
                 llgr_remaining);
    }
...
#define BGP_SHOW_HEADER_CSV                                                   \
  "Flags, Network, Next Hop, Metric, LocPrf, Weight, Path"
...
static int bgp_show_regexp (struct vty *vty, struct bgp *bgp,
                            const char *regstr, afi_t afi, safi_t safi,
...
static int
bgp_show_table (struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi,
      ...
      for (; pi; pi = pi->next)
        {
          ...
          if (type == bgp_show_type_lcommunity_list_exact)
            {
              ...
              if (! lcommunity_list_exact_match (
                      bgp_attr_get_lcommunity (pi->attr), list))
                continue;
            }
          ...
          if (! use_json && header)
            {
              ...
              if (type == bgp_show_type_dampend_paths ||
                ...
                vty_out (vty, (wide ? BGP_SHOW_HEADER_WIDE : BGP_SHOW_HEADER));
              header = false;
            }
          ...
          if (type == bgp_show_type_dampend_paths ||
              type == bgp_show_type_damp_neighbor)
            damp_route_vty_out (vty, dest_p, pi, display, afi, safi, use_json,
                                json_paths);
          else if (type == bgp_show_type_flap_statistics ||
                   type == bgp_show_type_flap_neighbor)
            flap_route_vty_out (vty, dest_p, pi, display, afi, safi, use_json,
                                json_paths);
          else
            {
          ...
          if (dest_p->family == AF_FLOWSPEC)
            {
              ...
              bgp_fs_nlri_get_string (
                  (unsigned char *) dest_p->u.prefix_flowspec.ptr,
                  dest_p->u.prefix_flowspec.prefixlen, retstr,
...
int
bgp_show_table_rd (struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi,
      ...
      if (itable != NULL)
        {
          ...
          bgp_show_table (vty, bgp, afi, safi, itable, type, output_arg, rd,
                          next == NULL, &output_cum, &total_cum,
          ...
          enum rpki_states rpki_target_state)
{
  ...
  if (safi == SAFI_FLOWSPEC && type == bgp_show_type_detail)
    {
      return bgp_show_table_flowspec (vty, bgp, afi, table, type, output_arg,
                                      use_json, 1, NULL, NULL);
    }
  ...
  if (safi == SAFI_EVPN)
    {
      if (! json)
        {
          vty_out (
              vty, "BGP routing table entry for %s%s%pFX\n",
              prd ? prefix_rd2str (prd, buf1, sizeof (buf1), bgp->asnotation)
                  : "",
      ...
      if (CHECK_FLAG (pi->flags, BGP_PATH_SELECTED))
        {
          ...
          no_advertise += community_include (picomm, COMMUNITY_NO_ADVERTISE);
          no_export += community_include (picomm, COMMUNITY_NO_EXPORT);
          local_as += community_include (picomm, COMMUNITY_LOCAL_AS);
          accept_own += community_include (picomm, COMMUNITY_ACCEPT_OWN);
          route_filter_translated_v4 +=
              community_include (picomm, COMMUNITY_ROUTE_FILTER_TRANSLATED_v4);
          route_filter_translated_v6 +=
              community_include (picomm, COMMUNITY_ROUTE_FILTER_TRANSLATED_v6);
          route_filter_v4 +=
              community_include (picomm, COMMUNITY_ROUTE_FILTER_v4);
          route_filter_v6 +=
              community_include (picomm, COMMUNITY_ROUTE_FILTER_v6);
          llgr_stale += community_include (picomm, COMMUNITY_LLGR_STALE);
          no_llgr += community_include (picomm, COMMUNITY_NO_LLGR);
          accept_own_nexthop +=
              community_include (picomm, COMMUNITY_ACCEPT_OWN_NEXTHOP);
          blackhole += community_include (picomm, COMMUNITY_BLACKHOLE);
  ...
  if (! json)
    {
          ...
          if (safi == SAFI_UNICAST)
            {
              if (bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT)
                vty_out (vty, ", table %s", VRF_DEFAULT_NAME);
              else
                vty_out (vty, ", vrf %s", bgp->name);
            }
        }
      else
      ...
      if (accept_own)
        ...
        vty_out (vty, ", mark translated RTs for VPNv4 route filtering");
      else if (route_filter_v4)
        vty_out (vty, ", attach RT as-is for VPNv4 route filtering");
      else if (route_filter_translated_v6)
        vty_out (vty, ", mark translated RTs for VPNv6 route filtering");
      else if (route_filter_v6)
        vty_out (vty, ", attach RT as-is for VPNv6 route filtering");
      else if (llgr_stale)
        vty_out (
            vty,
        ...
        vty_out (vty, ", accept local nexthop");
      else if (blackhole)
...
static void
bgp_show_path_info (const struct prefix_rd *pfx_rd, struct bgp_dest *bgp_node,
  ...
  for (pi = bgp_dest_get_bgp_path_info (bgp_node); pi; pi = pi->next)
    {
      ...
      if (p->family == AF_INET || p->family == AF_INET6)
        rpki_curr_state =
            hook_call (bgp_rpki_prefix_status, pi->peer, pi->attr, p);
      ...
      if (pathtype == BGP_PATH_SHOW_ALL ||
        ...
        route_vty_out_detail (vty, bgp, bgp_node,
                              bgp_dest_get_prefix (bgp_node), pi, afi, safi,
  ...
  if (json && json_paths)
    {
      ...
      if (pfx_rd)
        json_object_object_addf (json, json_header,
...
bgp_rd_from_dest (const struct bgp_dest *dest, safi_t safi)
{
  switch (safi)
    {
    case SAFI_MPLS_VPN:
...
bgp_show_route_in_table (struct vty *vty, struct bgp *bgp,
                         struct bgp_table *rib, const char *ip_str, afi_t afi,
                         safi_t safi, enum rpki_states rpki_target_state,
                         ...
                         enum bgp_path_type pathtype, bool use_json)
{
  ...
  if (safi == SAFI_MPLS_VPN || safi == SAFI_ENCAP)
      ...
      for (dest = bgp_table_top (rib); dest; dest = bgp_route_next (dest))
        {
          ...
          if (prefix_check && rm_p->prefixlen != match.prefixlen)
            {
              bgp_dest_unlock_node (rm);
              continue;
            }

          bgp_show_path_info ((struct prefix_rd *) dest_p, rm, vty, bgp, afi,
                              safi, json, pathtype, &display,
                              rpki_target_state);
          ...
          for (rm = bgp_table_top (table); rm; rm = bgp_route_next (rm))
            {
               ...
               * evpn route
               */
              if (evpn_type5_prefix_match (rm_p, &match) && rm->info)
                {
                  longest_pfx = rm;
                  int type5_pfxlen = bgp_evpn_get_type5_prefixlen (rm_p);
  ...
  else if (safi == SAFI_FLOWSPEC)
    {
      ...
      display = bgp_flowspec_display_match_per_ip (
          afi, rib, &match, prefix_check, vty, use_json, json_paths);
      if (use_json)
        {
          if (display)
      ...
      if (dest != NULL)
        {
          ...
          if (! prefix_check || dest_p->prefixlen == match.prefixlen)
            {
              bgp_show_path_info (NULL, dest, vty, bgp, afi, safi, json,
                                  pathtype, &display, rpki_target_state);
            }
                ...
                enum rpki_states rpki_target_state, bool use_json)
{
  ...
  return bgp_show_route_in_table (vty, bgp, bgp->rib[afi][safi], ip_str, afi,
                                  safi, rpki_target_state, prd, prefix_check,
                                  pathtype, use_json);
}
                     ...
                     safi_t safi, bool uj)
{
  ...
  ret = bgp_show (
      vty, bgp, afi, safi,
      (exact ? bgp_show_type_lcommunity_exact : bgp_show_type_lcommunity),
...
bgp_show_lcommunity_list (struct vty *vty, struct bgp *bgp, const char *lcom,
                          bool exact, afi_t afi, safi_t safi, bool uj)
{
  ...
  list =
      community_list_lookup (bgp_clist, lcom, 0, LARGE_COMMUNITY_LIST_MASTER);
  if (list == NULL)
    {
       ...
       JSON_STR)
{
  ...
  return bgp_show_lcommunity_list (vty, bgp, clist_number_or_name, exact_match,
                                   afi, safi, uj);
}
  ...
  if (argv_find (argv, argc, "AA:BB:CC", &idx))
    {
      ...
      return bgp_show_lcommunity (vty, bgp, argc, argv, exact_match, afi, safi,
                                  uj);
    }
  else
      ...
      if (uj)
        {
          ...
          json_object_object_add (json_all, get_afi_safi_str (afi, safi, true),
                                  json_afi_safi);
        }
      else
        {
...
DEFPY (show_ip_bgp_dampening_params,
  ...
  if (! all)
    {
      ...
      if (community)
        return bgp_show_community (vty, bgp, community, exact_match, afi, safi,
                                   show_flags);
      else
  ...
  else
    {
      ...
      if (CHECK_FLAG (show_flags, BGP_SHOW_OPT_AFI_IP) ||
        ...
        {
          afi =
              CHECK_FLAG (show_flags, BGP_SHOW_OPT_AFI_IP) ? AFI_IP : AFI_IP6;
              ...
              FOREACH_SAFI (safi)
                {
                  ...
                  if (community)
                    bgp_show_community (vty, abgp, community, exact_match, afi,
                                        safi, show_flags);
                  else
...
static int
bgp_show_regexp (struct vty *vty, struct bgp *bgp, const char *regstr,
...
static int
bgp_show_community (struct vty *vty, struct bgp *bgp, const char *comstr,
                    int exact, afi_t afi, safi_t safi, uint16_t show_flags)
{
...
static const char *table_stats_strs[][2] = {
  ...
  [BGP_STATS_ASPATH_MAXHOPS] = { "Longest AS-Path (hops)", "longestAsPath" },
  [BGP_STATS_ASPATH_MAXSIZE] = { "Largest AS-Path (bytes)", "largestAsPath" },
...
struct bgp_table_stats
{
  ...
  unsigned long long
      prefix_len_count[MAX (EVPN_ROUTE_PREFIXLEN, IPV6_MAX_BITLEN) + 1];

  double total_space;
};

static void
bgp_table_stats_rn (struct bgp_dest *dest, struct bgp_dest *top,
...
static void
bgp_table_stats_walker (struct event *t)
{
  struct bgp_dest *dest, *ndest;
  struct bgp_dest *top;
  ...
  for (dest = top; dest; dest = bgp_route_next (dest))
    {
      if (ts->table->safi == SAFI_MPLS_VPN || ts->table->safi == SAFI_ENCAP ||
...
static void
bgp_table_stats_all (struct vty *vty, afi_t afi, safi_t safi,
                     struct json_object *json_array)
{
  struct listnode *node, *nnode;
  struct bgp *bgp;
...
static int
bgp_table_stats_single (struct vty *vty, struct bgp *bgp, afi_t afi,
  ...
  if (! bgp->rib[afi][safi])
    {
      ...
      snprintf (warning_msg, sizeof (warning_msg),
                "%% No RIB exist's for the AFI(%d)/SAFI(%d)", afi, safi);
          ...
          if (! json)
            {
              snprintf (temp_buf, sizeof (temp_buf), "%12.2f",
                        ts.counts[i]
                            ...
                            : 0);
              vty_out (vty, "%-30s: %s",
                       table_stats_strs[i][TABLE_STATS_IDX_VTY], temp_buf);
            }
          else
            {
          ...
          if (! json)
          ...
          else
            {
              json_object_double_add (
                  json, table_stats_strs[i][TABLE_STATS_IDX_JSON],
                  ts.counts[i] ? (double) ts.counts[i] /
                                     (double) ts.counts[BGP_STATS_PREFIXES]
                               : 0);
            }
          break;
          ...
          if (afi == AFI_IP6)
            {
              if (! json)
              ...
              else
                {
                  json_object_double_add (
                      json, "/32equivalent",
                      (double) (ts.total_space * pow (2.0, -128 + 32)));
                }
              if (! json)
                {
                  snprintf (temp_buf, sizeof (temp_buf), "%12g",
              ...
              else
                {
                  json_object_double_add (
                      json, "/48equivalent",
                      (double) (ts.total_space * pow (2.0, -128 + 48)));
                }
            }
          else
            {
              if (! json)
              ...
              else
                {
                  json_object_double_add (
                      json, "%announced",
                      (double) (ts.total_space * 100. * pow (2.0, -32)));
                }
              if (! json)
                {
                  snprintf (temp_buf, sizeof (temp_buf), "%12.2f",
              ...
              else
                {
                  json_object_double_add (
                      json, "/8equivalent",
                      (double) (ts.total_space * pow (2.0, -32 + 8)));
                }
              if (! json)
                {
                  snprintf (temp_buf, sizeof (temp_buf), "%12.2f",
              ...
              else
                {
                  json_object_double_add (
                      json, "/24equivalent",
                      (double) (ts.total_space * pow (2.0, -32 + 24)));
                }
            }
          break;
        default:
      ...
      for (i = 0; i <= bitlen; i++)
        {
          ...
          json_object_int_add (ind_bit, temp_buf, ts.prefix_len_count[i]);
...
static void
bgp_peer_count_proc (struct bgp_dest *rn, struct peer_pcounts *pc)
{
...
static void
bgp_peer_count_walker (struct event *t)
{
       ...
       JSON_STR)
{
  ...
  return bgp_show_route (vty, NULL, network, AFI_L2VPN, SAFI_EVPN, NULL,
                         prefix_check, BGP_PATH_SHOW_ALL, RPKI_NOT_BEING_USED,
                         use_json (argc, argv));
}

static void
show_adj_route_header (struct vty *vty, struct peer *peer,
                       struct bgp_table *table, int *header1, int *header2,
                       json_object *json, json_object *json_scode,
                       json_object *json_ocode, bool wide, bool detail)
{
      ...
      if (json)
        {
          ...
          json_object_string_addf (json, "bgpLocalRouterId", "%pI4",
                                   &peer->bgp->router_id);
  ...
  if (match)
    {
      ...
      ret = bgp_output_modifier (peer, rn_p, &attr, afi, safi, rmap_name);

      if (ret != RMAP_DENY)
        {
          show_adj_route_header (vty, peer, table, header1, header2, json,
                                 json_scode, json_ocode, wide, detail);
          ...
          bgp_show_path_info (NULL /* prefix_rd */, dest, vty, bgp, afi, safi,
                              ...
                              RPKI_NOT_BEING_USED);
          if (use_json)
            json_object_object_addf (json_ar, json_net, "%pFX", rn_p);
          (*output_count)++;
        }
      else
        (*filtered_count)++;
      ...
      if (use_json)
        {
          ...
          json_object_string_addf (json, "bgpLocalRouterId", "%pI4",
                                   &bgp->router_id);
          json_object_int_add (json, "defaultLocPrf", bgp->default_local_pref);
          json_object_int_add (json, "localAS",
                               peer->change_local_as ? peer->change_local_as
                                                     : peer->local_as);
          json_object_object_add (json, "bgpStatusCodes", json_scode);
          json_object_object_add (json, "bgpOriginCodes", json_ocode);
          json_object_string_add (json, "bgpOriginatingDefaultNetwork",
      ...
      else
        {
          ...
          if (bgp->vrf_id == VRF_UNKNOWN)
            ...
            vty_out (vty, "%u", bgp->vrf_id);
          vty_out (vty, "\n");
          vty_out (vty, "Default local pref %u, ", bgp->default_local_pref);
          vty_out (vty, "local AS %u\n",
                   peer->change_local_as ? peer->change_local_as
                                         : peer->local_as);
          if (! detail)
          ...
          for (ain = dest->adj_in; ain; ain = ain->next)
            {
              ...
              if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP) ||
                ...
                {
                  if (use_json)
                    json_object_string_add (json_ar, "rd", rd_str);
              ...
              ret = bgp_input_modifier (peer, rn_p, &attr, afi, safi,
                                        rmap_name, NULL, 0, NULL);

              if (type == bgp_show_adj_route_filtered && ! route_filtered &&
                  ret != RMAP_DENY)
                {
                  bgp_attr_flush (&attr);
                  continue;
              ...
              if (detail)
                {
                  ...
                  if (route_filtered || ret == RMAP_DENY)
                    {
                      bpi.attr = &attr;
                      bpi.peer = peer;
                      buildit.info = &bpi;
                ...
                route_vty_out_tmp (vty, dest, rn_p, &attr, safi, use_json,
                                   json_ar, wide);
              bgp_attr_flush (&attr);
              (*output_count)++;
            }
            ...
            SUBGRP_FOREACH_PEER (adj->subgroup, paf)
              {
                ...
                show_adj_route_header (vty, peer, table, header1, header2,
                    ...
                    if (detail)
                      {
                        ...
                        bgp_show_path_info (NULL /* prefix_rd
                                                  */
                                            ,
                                            dest, vty, bgp, afi, safi,
                                            json_net, BGP_PATH_SHOW_ALL,
                                            &display, RPKI_NOT_BEING_USED);
                        if (use_json)
                          json_object_object_addf (json_ar, json_net, "%pFX",
                                                   rn_p);
                      }
                    else
          ...
          for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
            {
              ...
              if (detail)
                ...
                route_vty_out_tmp (vty, dest, rn_p, pi->attr, safi, use_json,
                                   json_ar, wide);
              (*output_count)++;
            }
        }
                 ...
                 const struct prefix *match, uint16_t show_flags)
{
  ...
  if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP) || (safi == SAFI_EVPN))
      ...
      for (dest = bgp_table_top (table); dest; dest = bgp_route_next (dest))
        {
          ...
          const struct prefix_rd *prd;
          prd = (const struct prefix_rd *) bgp_dest_get_prefix (dest);

          prefix_rd2str (prd, rd_str, sizeof (rd_str), bgp->asnotation);

          show_adj_route (vty, peer, table, afi, safi, type, rmap_name, json,
                          json_routes, json_scode, json_ocode, show_flags,
    ...
    show_adj_route (vty, peer, table, afi, safi, type, rmap_name, json,
                    json_ar, json_scode, json_ocode, show_flags, &header1,
                    &header2, rd_str, match, &output_count, &filtered_count);
              ...
              if (uj)
                {
                  if (first)
                    ...
                    vty_out (vty, ",\n");
                  vty_out (vty, "\"%s\":", get_afi_safi_str (afi, safi, true));
                }
              else
                vty_out (vty, "\nFor address family: %s\n",
                         get_afi_safi_str (afi, safi, false));
...
bgp_show_neighbor_route (struct vty *vty, struct peer *peer, afi_t afi,
                         safi_t safi, enum bgp_show_type type, bool use_json)
{
  uint16_t show_flags = 0;
...
DEFPY (show_ip_bgp_vrf_afi_safi_routes_detailed,
       ...
       "show [ip] bgp [<view|vrf> VIEWVRFNAME$vrf_name] ["
       BGP_AFI_CMD_STR
       " ["
       BGP_SAFI_WITH_LABEL_CMD_STR
       "]] detail [json$uj]",
       ...
       JSON_STR)
{
  ...
  if (! strcmp (argv[5]->arg, "all"))
    return bgp_show_route (vty, NULL, argv[6]->arg, afi, SAFI_MPLS_VPN, NULL,
...
uint8_t
bgp_distance_apply (const struct prefix *p, struct bgp_path_info *pinfo,
 ...
 */
static void
bgp_announce_routes_distance_update (struct bgp *bgp, afi_t update_afi,
                                     safi_t update_safi)
{
  afi_t afi;
  safi_t safi;
       ...
       "IP source prefix\n")
{
  ...
  bgp_distance_set (vty, argv[idx_number]->arg, argv[idx_ipv4_prefixlen]->arg,
                    NULL);
  return CMD_SUCCESS;
}
              ...
              while (pi)
                {
                  if (pi->extra && pi->extra->damp_info)
                    {
                      pi_temp = pi->next;
                      bgp_damp_info_free (pi->extra->damp_info, 1, afi, safi);
                      pi = pi_temp;
                    }
                  else
                    pi = pi->next;
...
static void
show_bgp_peerhash_entry (struct hash_bucket *bucket, void *arg)
{
  struct vty *vty = arg;
       ...
       "Display information about the BGP peerhash\n")
{
  ...
  for (ALL_LIST_ELEMENTS_RO (instances, node, bgp))
    {
      vty_out (vty, "BGP: %s\n", bgp->name);
      hash_iterate (bgp->peerhash, show_bgp_peerhash_entry, vty);
    }
...
static void
bgp_config_write_network_vpn (struct vty *vty, struct bgp *bgp, afi_t afi,
                              safi_t safi)
{
  struct bgp_dest *pdest;
  struct bgp_dest *dest;
      ...
      for (dest = bgp_table_top (table); dest; dest = bgp_route_next (dest))
        {
          ...
          vty_out (vty, "  network %pFX rd %s", p, bgp_static->prd_pretty);
          if (safi == SAFI_MPLS_VPN)
...
static void
bgp_config_write_network_evpn (struct vty *vty, struct bgp *bgp, afi_t afi,
                               safi_t safi)
{
  struct bgp_dest *pdest;
  struct bgp_dest *dest;
      ...
      for (dest = bgp_table_top (table); dest; dest = bgp_route_next (dest))
        {
          ...
          if (bgp_static->eth_s_id)
            esi_to_str (bgp_static->eth_s_id, esi_buf, sizeof (esi_buf));
          p = bgp_dest_get_prefix (dest);
          ...
          if (p->u.prefix_evpn.route_type == 5)
            {
              ...
              uint8_t family =
                  is_evpn_prefix_ipaddr_v4 ((struct prefix_evpn *) p)
                      ? AF_INET
                      : AF_INET6;
              inet_ntop (family, &p->u.prefix_evpn.prefix_addr.ip.ip.addr,
          ...
          if (bgp_static->gatewayIp.family == AF_INET ||
            ...
            inet_ntop (bgp_static->gatewayIp.family,
                       &bgp_static->gatewayIp.u.prefix, buf2, sizeof (buf2));
          vty_out (
              vty,
              "  network %s rd %s ethtag %u label %u esi %s gwip %s routermac %s\n",
              buf, bgp_static->prd_pretty,
              p->u.prefix_evpn.prefix_addr.eth_tag,
              decode_label (&bgp_static->label), esi_buf, buf2, macrouter);
  ...
  for (dest = bgp_table_top (bgp->aggregate[afi][safi]); dest;
       dest = bgp_route_next (dest))
    {
      ...
      if (bgp_aggregate->origin != BGP_ORIGIN_UNSPECIFIED)
        vty_out (vty, " origin %s", bgp_origin2str (bgp_aggregate->origin));

      if (bgp_aggregate->match_med)
        vty_out (vty, " matching-MED-only");

      if (bgp_aggregate->suppress_map_name)
        vty_out (vty, " suppress-map %s", bgp_aggregate->suppress_map_name);
  ...
  if (bgp->distance_ebgp[afi][safi] && bgp->distance_ibgp[afi][safi] &&
       ...
       bgp->distance_local[afi][safi] != ZEBRA_IBGP_DISTANCE_DEFAULT))
    {
      vty_out (vty, "  distance bgp %d %d %d\n", bgp->distance_ebgp[afi][safi],
               bgp->distance_ibgp[afi][safi], bgp->distance_local[afi][safi]);
    }
...
bgp_route_init (void)
{
  ...
  install_element (VIEW_NODE, &show_bgp_l2vpn_evpn_route_prefix_cmd);
  ...
  install_element (VIEW_NODE, &show_ip_bgp_vrf_afi_safi_routes_detailed_cmd);
