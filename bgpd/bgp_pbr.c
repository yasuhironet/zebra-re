...
#define IPV6_FRAGMENT_INVALID                                                 \
  "fragment not valid for IPv6 for this implementation"

RB_GENERATE (bgp_pbr_interface_head, bgp_pbr_interface, id_entry,
...
static int
bgp_pbr_rule_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct bgp_pbr_rule *bpr = (struct bgp_pbr_rule *) bucket->data;
  struct bgp_pbr_rule_unique *bpru = (struct bgp_pbr_rule_unique *) arg;
...
static int
bgp_pbr_action_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct bgp_pbr_action *bpa = (struct bgp_pbr_action *) bucket->data;
  struct bgp_pbr_action_unique *bpau = (struct bgp_pbr_action_unique *) arg;
...
static int
bgp_pbr_match_entry_walkcb (struct hash_bucket *bucket, void *arg)
{
...
static int
bgp_pbr_match_pername_walkcb (struct hash_bucket *bucket, void *arg)
{
  ...
  if (! strncmp (ipset_name, bpm->ipset_name, ZEBRA_IPSET_NAME_SIZE))
    {
      bpmi->bpm_found = bpm;
      return HASHWALK_ABORT;
    }
...
static int
bgp_pbr_match_iptable_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct bgp_pbr_match *bpm = (struct bgp_pbr_match *) bucket->data;
...
static int
bgp_pbr_match_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct bgp_pbr_match *bpm = (struct bgp_pbr_match *) bucket->data;
  struct bgp_pbr_match_unique *bpmu = (struct bgp_pbr_match_unique *) arg;
...
static int
snprintf_bgp_pbr_match_val (char *str, int len, struct bgp_pbr_match_val *mval,
                            const char *prepend)
{
  char *ptr = str;
  int delta;
...
#define INCREMENT_DISPLAY(_ptr, _cnt, _len)                                   \
  do                                                                          \
    {                                                                         \
      int sn_delta;                                                           \
                                                                              \
...
static bool
bgp_pbr_extract_enumerate_unary_opposite (uint8_t unary_operator,
                                          struct bgp_pbr_val_mask *and_valmask,
                                          struct list *or_valmask,
                                          uint32_t value, uint8_t type_entry)
{
  if (unary_operator == OPERATOR_UNARY_AND && and_valmask)
    {
      if (type_entry == FLOWSPEC_TCP_FLAGS)
        {
          and_valmask->mask |= TCP_HEADER_ALL_FLAGS & ~(value);
        }
    ...
    {
      and_valmask =
          XCALLOC (MTYPE_PBR_VALMASK, sizeof (struct bgp_pbr_val_mask));
      if (type_entry == FLOWSPEC_TCP_FLAGS)
        {
 ...
 */
static bool
bgp_pbr_extract_enumerate_unary (struct bgp_pbr_match_val list[], int num,
                                 uint8_t unary_operator, void *valmask,
                                 uint8_t type_entry)
{
  int i = 0;
  ...
  for (i = 0; i < num; i++)
    {
      if (i != 0 && list[i].unary_operator != unary_operator)
        return false;
      if (! (list[i].compare_operator & OPERATOR_COMPARE_EQUAL_TO) &&
          ! (list[i].compare_operator & OPERATOR_COMPARE_EXACT_MATCH))
        {
          if ((list[i].compare_operator & OPERATOR_COMPARE_LESS_THAN) &&
              (list[i].compare_operator & OPERATOR_COMPARE_GREATER_THAN))
            {
              ret = bgp_pbr_extract_enumerate_unary_opposite (
                  unary_operator, and_valmask, or_valmask, list[i].value,
                  type_entry);
              if (! ret)
                return ret;
              continue;
      ...
      if (unary_operator == OPERATOR_UNARY_AND && and_valmask)
        {
          if (type_entry == FLOWSPEC_TCP_FLAGS)
            and_valmask->mask |= TCP_HEADER_ALL_FLAGS & list[i].value;
        }
      else if (unary_operator == OPERATOR_UNARY_OR && or_valmask)
        {
          and_valmask =
 ...
 */
static bool
bgp_pbr_extract_enumerate (struct bgp_pbr_match_val list[], int num,
                           uint8_t unary_operator, void *valmask,
                           uint8_t type_entry)
{
  bool ret;
  ...
  ret = bgp_pbr_extract_enumerate_unary (list, num, unary_operator_val,
                                         valmask, type_entry);
  if (! ret && double_check)
    ret = bgp_pbr_extract_enumerate_unary (list, num, OPERATOR_UNARY_OR,
                                           valmask, type_entry);
  return ret;
}
 ...
 */
static uint8_t
bgp_pbr_match_val_get_operator (struct bgp_pbr_match_val list[], int num)
 ...
 */
static bool
bgp_pbr_extract (struct bgp_pbr_match_val list[], int num,
                 struct bgp_pbr_range_port *range)
{
  ...
  for (i = 0; i < num; i++)
    {
      if (i != 0 && (list[i].compare_operator == OPERATOR_COMPARE_EQUAL_TO))
        return false;
      if (i == 0 && (list[i].compare_operator == OPERATOR_COMPARE_EQUAL_TO))
        {
          if (range)
      ...
      if (list[i].compare_operator ==
          (OPERATOR_COMPARE_GREATER_THAN + OPERATOR_COMPARE_EQUAL_TO))
        {
          if (range)
            range->min_port = list[i].value;
        }
      else if (list[i].compare_operator ==
               (OPERATOR_COMPARE_LESS_THAN + OPERATOR_COMPARE_EQUAL_TO))
        {
          if (range)
            range->max_port = list[i].value;
        }
      else if (list[i].compare_operator == OPERATOR_COMPARE_LESS_THAN)
        {
          if (range)
            range->max_port = list[i].value - 1;
        }
      else if (list[i].compare_operator == OPERATOR_COMPARE_GREATER_THAN)
        {
          if (range)
...
static int
bgp_pbr_validate_policy_route (struct bgp_pbr_entry_main *api)
{
  ...
  if (api->match_protocol_num > 1)
    {
      if (BGP_DEBUG (pbr, PBR))
        zlog_debug (
            "BGP: match protocol operations:multiple protocols ( %d). ignoring.",
            api->match_protocol_num);
      return 0;
    }
  if (api->src_prefix_offset > 0 || api->dst_prefix_offset > 0)
    {
      if (BGP_DEBUG (pbr, PBR))
  ...
  if (api->match_protocol_num == 1 && api->protocol[0].value != PROTOCOL_UDP &&
      ...
      api->protocol[0].value != PROTOCOL_TCP)
    {
      if (BGP_DEBUG (pbr, PBR))
        zlog_debug (
            "BGP: match protocol operations:protocol (%d) not supported. ignoring",
            api->match_protocol_num);
      return 0;
    }
  ...
  if (! bgp_pbr_extract_enumerate (api->tcpflags, api->match_tcpflags_num,
                                   OPERATOR_UNARY_AND | OPERATOR_UNARY_OR,
                                   NULL, FLOWSPEC_TCP_FLAGS))
    {
      if (BGP_DEBUG (pbr, PBR))
  ...
  if (! bgp_pbr_extract (api->icmp_type, api->match_icmp_type_num, NULL))
    {
      if (! bgp_pbr_extract_enumerate (
              api->icmp_type, api->match_icmp_type_num, OPERATOR_UNARY_OR,
              NULL, FLOWSPEC_ICMP_TYPE))
        {
          if (BGP_DEBUG (pbr, PBR))
            zlog_debug (
                "BGP: match icmp type operations:too complex. ignoring.");
          return 0;
        }
      enumerate_icmp = true;
    }
  if (! bgp_pbr_extract (api->icmp_code, api->match_icmp_code_num, NULL))
    {
      if (! bgp_pbr_extract_enumerate (
              api->icmp_code, api->match_icmp_code_num, OPERATOR_UNARY_OR,
              NULL, FLOWSPEC_ICMP_CODE))
        {
          if (BGP_DEBUG (pbr, PBR))
            zlog_debug (
                "BGP: match icmp code operations:too complex. ignoring.");
          return 0;
        }
      else if (api->match_icmp_type_num > 1 && ! enumerate_icmp)
        {
          if (BGP_DEBUG (pbr, PBR))
            zlog_debug (
                "BGP: match icmp code is enumerate, and icmp type is not. too complex. ignoring.");
          return 0;
        }
    }
  ...
  if (api->match_packet_length_num)
    {
      ...
      ret = bgp_pbr_extract (api->packet_length, api->match_packet_length_num,
                             NULL);
      if (! ret)
        ret = bgp_pbr_extract_enumerate (
            api->packet_length, api->match_packet_length_num,
            OPERATOR_UNARY_OR | OPERATOR_UNARY_AND, NULL, FLOWSPEC_PKT_LEN);
      if (! ret)
        {
          if (BGP_DEBUG (pbr, PBR))
            zlog_debug (
                "BGP: match packet length operations:too complex. ignoring.");
          return 0;
        }
    }
  ...
  if (api->match_fragment_num)
    {
      ...
      success = bgp_pbr_extract_enumerate (
          api->fragment, api->match_fragment_num,
          OPERATOR_UNARY_OR | OPERATOR_UNARY_AND, NULL, FLOWSPEC_FRAGMENT);
      if (success)
        {
          int i;
          ...
          for (i = 0; i < api->match_fragment_num; i++)
            {
              if (api->fragment[i].value != 1 && api->fragment[i].value != 2 &&
                  api->fragment[i].value != 4 && api->fragment[i].value != 8)
                {
                  success = false;
              ...
              if (api->afi == AFI_IP6 && api->fragment[i].value == 1)
                {
                  success = false;
                  snprintf (fail_str, sizeof (fail_str),
          ...
          if (api->afi == AFI_IP6)
            {
              success = false;
              snprintf (fail_str, sizeof (fail_str), "%s",
                        IPV6_FRAGMENT_INVALID);
            }
        }
      else
        snprintf (fail_str, sizeof (fail_str), "too complex. ignoring");
      if (! success)
        {
          if (BGP_DEBUG (pbr, PBR))
            zlog_debug ("BGP: match fragment operation (%d) %s",
                        api->match_fragment_num, fail_str);
          return 0;
        }
    }
  ...
  if (api->match_src_port_num + api->match_dst_port_num + api->match_port_num >
      3)
    {
      if (BGP_DEBUG (pbr, PBR))
        zlog_debug (
            "BGP: match multiple port operations: too complex. ignoring.");
      return 0;
    }
  if ((api->match_src_port_num || api->match_dst_port_num ||
       api->match_port_num) &&
      (api->match_icmp_type_num || api->match_icmp_code_num))
    {
      if (BGP_DEBUG (pbr, PBR))
        zlog_debug (
            "BGP: match multiple port/imcp operations: too complex. ignoring.");
      return 0;
    }
              ...
              if (BGP_DEBUG (pbr, PBR))
                {
                  bgp_pbr_print_policy_route (api);
                  zlog_warn (
                      "PBR: iprule set DSCP/Flow Label %u not supported",
      ...
      if (BGP_DEBUG (pbr, PBR))
        {
          bgp_pbr_print_policy_route (api);
          zlog_debug (
              "BGP: match actions without src or dst address can not operate. ignoring.");
        }
      return 0;
    }
  return 1;
...
int
bgp_pbr_build_and_validate_entry (const struct prefix *p,
                                  struct bgp_path_info *path,
                                  struct bgp_pbr_entry_main *api)
{
  ...
  ret =
      bgp_flowspec_match_rules_fill ((uint8_t *) p->u.prefix_flowspec.ptr,
  ...
  if (path && bgp_attr_get_ecommunity (path->attr))
    {
      ...
      for (i = 0; i < ecom->size; i++)
        {
          ecom_eval =
              (struct ecommunity_val *) (ecom->val + (i * ECOMMUNITY_SIZE));
          action_count++;
          if (action_count > ACTIONS_MAX_NUM)
            {
              if (BGP_DEBUG (pbr, PBR_ERROR))
                flog_err (EC_BGP_FLOWSPEC_PACKET, "%s: %s (max %u)", __func__,
                          FSPEC_ACTION_EXCEED_LIMIT, action_count);
              break;
            }
          ...
          if ((ecom_eval->val[1] == (char) ECOMMUNITY_REDIRECT_VRF) &&
                   ...
                   (char) ECOMMUNITY_EXTENDED_COMMUNITY_PART_3))
            {
              ...
              ecommunity_add_val (eckey, &ecom_copy, false, false);

              api_action->action = ACTION_REDIRECT;
              api_action->u.redirect_vrf =
                  get_first_vrf_for_redirect_with_rt (eckey);
                   ...
                   (ecom_eval->val[1] == (char) ECOMMUNITY_REDIRECT_IP_NH))
            {
              ...
              if (api_action_redirect_ip &&
                ...
                {
                  if (memcmp (&api_action_redirect_ip->u.zr.redirect_ip_v6,
                  ...
                  if (path->attr->mp_nexthop_len == 0 ||
                    ...
                    continue;
                  memcpy (&api_action_redirect_ip->u.zr.redirect_ip_v6,
                ...
                {
                  api_action->action = ACTION_REDIRECT_IP;
                  memcpy (&api_action->u.zr.redirect_ip_v6,
                          &path->attr->mp_nexthop_global,
                          sizeof (struct in6_addr));
                  api_action->u.zr.duplicate = ecom_eval->val[7];
                  api_action_redirect_ip = api_action;
          ...
          else if ((ecom_eval->val[0] == (char) ECOMMUNITY_ENCODE_IP) &&
                   (ecom_eval->val[1] ==
          ...
          else
            {
              if (ecom_eval->val[0] != (char) ECOMMUNITY_ENCODE_TRANS_EXP)
                continue;
              ret = ecommunity_fill_pbr_action (ecom_eval, api_action, afi);
              if (ret != 0)
                continue;
  ...
  if (path && path->attr && bgp_attr_get_ipv6_ecommunity (path->attr))
    {
      ...
      for (i = 0; i < ecom->size; i++)
        {
          ipv6_ecom_eval =
              (struct ecommunity_val_ipv6 *) (ecom->val +
                                              (i * ecom->unit_size));
          action_count++;
          if (action_count > ACTIONS_MAX_NUM)
            {
          ...
          if ((ipv6_ecom_eval->val[1] ==
              ...
              (ipv6_ecom_eval->val[0] == (char) ECOMMUNITY_ENCODE_TRANS_EXP))
            {
              struct ecommunity *eckey = ecommunity_new ();
              ...
              ecom_copy.val[1] = ECOMMUNITY_ROUTE_TARGET;
              ecommunity_add_val_ipv6 (eckey, &ecom_copy, false, false);
              api_action->action = ACTION_REDIRECT;
              api_action->u.redirect_vrf =
                  get_first_vrf_for_redirect_with_rt (eckey);
              ecommunity_free (&eckey);
  ...
  if (api->action_num > 1 && discard_action_found)
    {
      api->action_num = 1;
      memset (&api->actions[0], 0, sizeof (struct bgp_pbr_entry_action));
          ...
          if (BGP_DEBUG (pbr, PBR))
            {
              bgp_pbr_print_policy_route (api);
              zlog_debug (
                  "%s: inconsistency: no match for afi src and dst (%u/%u)",
...
static void
bgp_pbr_match_entry_free (void *arg)
{
  struct bgp_pbr_match_entry *bpme;
      ...
      if (bpm->installed)
        {
          if (bpm->installed_in_iptable)
            {
              bgp_send_pbr_iptable (bpm->action, bpm, false);
...
static void *
bgp_pbr_match_alloc_intern (void *arg)
{
...
static void *
bgp_pbr_rule_alloc_intern (void *arg)
{
...
static void
bgp_pbr_bpa_remove (struct bgp_pbr_action *bpa)
{
  if ((bpa->refcnt == 0) && bpa->installed && bpa->table_id != 0)
    {
      bgp_send_pbr_rule_action (bpa, NULL, false);
      bgp_zebra_announce_default (bpa->bgp, &bpa->nh, bpa->afi, bpa->table_id,
                                  false);
      bpa->installed = false;
    }
}
...
static void
bgp_pbr_bpa_add (struct bgp_pbr_action *bpa)
{
...
static void *
bgp_pbr_action_alloc_intern (void *arg)
{
...
static void *
bgp_pbr_match_entry_alloc_intern (void *arg)
{
...
uint32_t
bgp_pbr_match_hash_key (const void *arg)
{
...
bool
bgp_pbr_match_hash_equal (const void *arg1, const void *arg2)
{
...
uint32_t
bgp_pbr_rule_hash_key (const void *arg)
{
...
bool
bgp_pbr_rule_hash_equal (const void *arg1, const void *arg2)
{
  ...
  if ((r1->flags & MATCH_IP_SRC_SET) && ! prefix_same (&r1->src, &r2->src))
    return false;

  if ((r1->flags & MATCH_IP_DST_SET) && ! prefix_same (&r1->dst, &r2->dst))
    return false;
...
uint32_t
bgp_pbr_match_entry_hash_key (const void *arg)
{
...
bool
bgp_pbr_match_entry_hash_equal (const void *arg1, const void *arg2)
{
...
uint32_t
bgp_pbr_action_hash_key (const void *arg)
{
...
bool
bgp_pbr_action_hash_equal (const void *arg1, const void *arg2)
{
...
struct bgp_pbr_rule *
bgp_pbr_rule_lookup (vrf_id_t vrf_id, uint32_t unique)
{
...
bgp_pbr_action_rule_lookup (vrf_id_t vrf_id, uint32_t unique)
{
  struct bgp *bgp = bgp_lookup_by_vrf_id (vrf_id);
...
bgp_pbr_match_ipset_lookup (vrf_id_t vrf_id, uint32_t unique)
{
  struct bgp *bgp = bgp_lookup_by_vrf_id (vrf_id);
...
bgp_pbr_match_ipset_entry_lookup (vrf_id_t vrf_id, char *ipset_name,
                                  uint32_t unique)
{
  struct bgp *bgp = bgp_lookup_by_vrf_id (vrf_id);
  struct bgp_pbr_match_entry_unique bpmeu;
...
struct bgp_pbr_match *
bgp_pbr_match_iptable_lookup (vrf_id_t vrf_id, uint32_t unique)
{
  struct bgp *bgp = bgp_lookup_by_vrf_id (vrf_id);
  struct bgp_pbr_match_iptable_unique bpmiu;
...
void
bgp_pbr_cleanup (struct bgp *bgp)
{
...
void
bgp_pbr_init (struct bgp *bgp)
{
  bgp->pbr_match_hash = hash_create_size (
      8, bgp_pbr_match_hash_key, bgp_pbr_match_hash_equal, "Match Hash");
  bgp->pbr_action_hash =
      hash_create_size (8, bgp_pbr_action_hash_key, bgp_pbr_action_hash_equal,
                        "Match Hash Entry");

  bgp->pbr_rule_hash = hash_create_size (
      8, bgp_pbr_rule_hash_key, bgp_pbr_rule_hash_equal, "Match Rule");
...
void
bgp_pbr_print_policy_route (struct bgp_pbr_entry_main *api)
{
  int i = 0;
  ...
  for (i = 0; i < api->action_num; i++)
    {
      switch (api->actions[i].action)
        ...
        case ACTION_TRAFFICRATE:
          INCREMENT_DISPLAY (ptr, nb_items, len);
          delta =
              snprintf (ptr, len, "@set rate %f", api->actions[i].u.r.rate);
          len -= delta;
          ptr += delta;
          break;
        case ACTION_TRAFFIC_ACTION:
          ...
          if (api->actions[i].u.za.filter & TRAFFIC_ACTION_TERMINATE)
            {
              delta = snprintf (ptr, len, " terminate (apply filter(s))");
              len -= delta;
              ptr += delta;
            }
        ...
        case ACTION_REDIRECT_IP:
          {
            ...
            if (inet_ntop (afi2family (api->afi), ptr_ip, local_buff,
              ...
              {
                delta = snprintf (ptr, len, "@redirect ip nh %s", local_buff);
                len -= delta;
                ptr += delta;
              }
            break;
        ...
        case ACTION_REDIRECT:
          {
            ...
            INCREMENT_DISPLAY (ptr, nb_items, len);
            delta =
                snprintf (ptr, len, "@redirect vrf %s(%u)", VRF_LOGNAME (vrf),
                          api->actions[i].u.redirect_vrf);
            len -= delta;
            ptr += delta;
            break;
...
static void
bgp_pbr_flush_iprule (struct bgp *bgp, struct bgp_pbr_action *bpa,
                      struct bgp_pbr_rule *bpr)
{
...
static void
bgp_pbr_flush_entry (struct bgp *bgp, struct bgp_pbr_action *bpa,
                     struct bgp_pbr_match *bpm,
                     struct bgp_pbr_match_entry *bpme)
{
...
static int
bgp_pbr_get_same_rule (struct hash_bucket *bucket, void *arg)
{
  struct bgp_pbr_rule *r1 = (struct bgp_pbr_rule *) bucket->data;
  struct bgp_pbr_rule_remain *ctxt = (struct bgp_pbr_rule_remain *) arg;
   ...
   */
  if (r2->action == NULL || r1->action != r2->action)
    {
      ctxt->bpr_found = r1;
      return HASHWALK_ABORT;
    }
...
static int
bgp_pbr_get_remaining_entry (struct hash_bucket *bucket, void *arg)
{
  struct bgp_pbr_match *bpm = (struct bgp_pbr_match *) bucket->data;
  ...
  if (! bpme->backpointer || bpm == bpme->backpointer ||
      bpme->backpointer->action == bpm->action)
    return HASHWALK_CONTINUE;
  /* ensure bpm other characteristics are equal */
  bpm_temp = bpme->backpointer;
  if (bpm_temp->vrf_id != bpm->vrf_id || bpm_temp->type != bpm->type ||
      bpm_temp->flags != bpm->flags || bpm_temp->tcp_flags != bpm->tcp_flags ||
      ...
      bpm_temp->family != bpm->family || bpm_temp->fragment != bpm->fragment)
    return HASHWALK_CONTINUE;
      ...
      if (bprr.bpr_found)
        {
          ...
          bgp_pbr_flush_iprule (bgp, local_bpa, local_bpr);
        }
      return;
    }
  ...
  if (bpmer.bpme_found)
    {
      ...
      bgp_pbr_flush_entry (bgp, local_bpa, local_bpm, bpmer.bpme_found);
    }
}

static uint8_t
bgp_pbr_next_type_entry (uint8_t type_entry)
{
  if (type_entry == FLOWSPEC_TCP_FLAGS)
...
static void
bgp_pbr_icmp_action (struct bgp *bgp, struct bgp_path_info *path,
                     struct bgp_pbr_filter *bpf,
    ...
    struct bgp_pbr_or_filter *bpof, uint8_t type_entry)
{
  ...
  if (type_entry == FLOWSPEC_TCP_FLAGS && bpof->tcpflags)
    ...
    {
      bgp_pbr_policyroute_remove_from_zebra_recursive (bgp, path, bpf, bpof,
                                                       next_type_entry);
      return;
    }
...
static void
bgp_pbr_policyroute_remove_from_zebra (struct bgp *bgp,
                                       struct bgp_path_info *path,
                                       struct bgp_pbr_filter *bpf,
...
static void
bgp_pbr_dump_entry (struct bgp_pbr_filter *bpf, bool add)
{
  struct bgp_pbr_range_port *src_port;
  struct bgp_pbr_range_port *dst_port;
  struct bgp_pbr_range_port *pkt_len;
  ...
  if (bpf->protocol)
    snprintf (protocol_str, sizeof (protocol_str), "proto %d", bpf->protocol);
  buffer[0] = '\0';
  if (bpf->protocol == IPPROTO_ICMP && src_port && dst_port)
    remaining_len += snprintf (buffer, sizeof (buffer), "type %d, code %d",
                               src_port->min_port, dst_port->min_port);
  else if (bpf->protocol == IPPROTO_UDP || bpf->protocol == IPPROTO_TCP)
    {

      if (src_port && src_port->min_port)
        remaining_len += snprintf (
            buffer, sizeof (buffer), "from [%u:%u]", src_port->min_port,
            src_port->max_port ? src_port->max_port : src_port->min_port);
      if (dst_port && dst_port->min_port)
        remaining_len += snprintf (
            buffer + remaining_len, sizeof (buffer) - remaining_len,
            "to [%u:%u]", dst_port->min_port,
            dst_port->max_port ? dst_port->max_port : dst_port->min_port);
    }
  if (pkt_len && (pkt_len->min_port || pkt_len->max_port))
    {
      remaining_len +=
          snprintf (buffer + remaining_len, sizeof (buffer) - remaining_len,
                    " len [%u:%u]", pkt_len->min_port,
                    pkt_len->max_port ? pkt_len->max_port : pkt_len->min_port);
    }
  else if (bpf->pkt_len_val)
    {
      remaining_len +=
          snprintf (buffer + remaining_len, sizeof (buffer) - remaining_len,
                    " %s len %u", bpf->pkt_len_val->mask ? "!" : "",
                    bpf->pkt_len_val->val);
    }
  if (bpf->tcp_flags)
    {
      remaining_len += snprintf (
          buffer + remaining_len, sizeof (buffer) - remaining_len,
          "tcpflags %x/%x", bpf->tcp_flags->val, bpf->tcp_flags->mask);
    }
  if (bpf->dscp)
    {
      snprintf (buffer + remaining_len, sizeof (buffer) - remaining_len,
                "%s dscp %d", bpf->dscp->mask ? "!" : "", bpf->dscp->val);
    }
  if (bpf->flow_label)
    {
      snprintf (buffer + remaining_len, sizeof (buffer) - remaining_len,
                "%s flow_label %d", bpf->flow_label->mask ? "!" : "",
                bpf->flow_label->val);
    }
  zlog_debug (
      "BGP: %s FS PBR from %s to %s, %s %s", add ? "adding" : "removing",
      bpf->src == NULL ? "<all>"
...
static void
bgp_pbr_policyroute_add_to_zebra_unit (struct bgp *bgp,
                                       struct bgp_path_info *path,
                                       struct bgp_pbr_filter *bpf,
                                       struct nexthop *nh, float *rate)
{
  ...
  bpa = hash_get (bgp->pbr_action_hash, &temp3, bgp_pbr_action_alloc_intern);
  ...
  if (bpf->type == BGP_PBR_IPRULE)
    {
      ...
      pbr_rule.action = bpa;
      bpr =
          hash_get (bgp->pbr_rule_hash, &pbr_rule, bgp_pbr_rule_alloc_intern);
      if (bpr->unique == 0)
        {
      ...
      if (bpr_found)
        {
          struct bgp_path_info_extra *extra = bgp_path_info_extra_get (path);

          if (extra && listnode_lookup_nocheck (extra->bgp_fs_iprule, bpr))
            {
              if (BGP_DEBUG (pbr, PBR_ERROR))
                zlog_err (
  ...
  temp.action = bpa;
  bpm = hash_get (bgp->pbr_match_hash, &temp, bgp_pbr_match_alloc_intern);
  ...
  if (bpm->unique == 0)
    {
      ...
      bpm->entry_hash = hash_create_size (8, bgp_pbr_match_entry_hash_key,
  ...
  if (bpme_found)
    {
      ...
      if (extra && listnode_lookup_nocheck (extra->bgp_fs_pbr, bpme))
        {
          if (BGP_DEBUG (pbr, PBR_ERROR))
            zlog_err ("%s: entry %p/%p already installed in bgp pbr", __func__,
                      path, bpme);
          return;
        }
    }
...
static void
bgp_pbr_policyroute_add_to_zebra_recursive (struct bgp *bgp,
                                            struct bgp_path_info *path,
                                            struct bgp_pbr_filter *bpf,
                                            struct bgp_pbr_or_filter *bpof,
                                            struct nexthop *nh, float *rate,
                                            uint8_t type_entry)
{
  ...
  if (type_entry == FLOWSPEC_TCP_FLAGS && bpof->tcpflags)
    ...
    {
      bgp_pbr_policyroute_add_to_zebra_recursive (bgp, path, bpf, bpof, nh,
                                                  rate, next_type_entry);
      return;
    }
  for (ALL_LIST_ELEMENTS (orig_list, node, nnode, valmask))
    {
...
static void
bgp_pbr_policyroute_add_to_zebra (struct bgp *bgp, struct bgp_path_info *path,
                                  struct bgp_pbr_filter *bpf,
                                  struct bgp_pbr_or_filter *bpof,
                                  struct nexthop *nh, float *rate)
{
  ...
  if (bpof->tcpflags)
    bgp_pbr_policyroute_add_to_zebra_recursive (bgp, path, bpf, bpof, nh, rate,
                                                FLOWSPEC_TCP_FLAGS);
  else if (bpof->dscp)
    bgp_pbr_policyroute_add_to_zebra_recursive (bgp, path, bpf, bpof, nh, rate,
                                                FLOWSPEC_DSCP);
...
static void
bgp_pbr_handle_entry (struct bgp *bgp, struct bgp_path_info *path,
                      struct bgp_pbr_entry_main *api, bool add)
{
  ...
  if (api->match_port_num >= 1)
    {
      bgp_pbr_extract (api->port, api->match_port_num, &range);
      srcp = dstp = &range;
    }
  else if (api->match_src_port_num >= 1)
    {
      bgp_pbr_extract (api->src_port, api->match_src_port_num, &range);
      srcp = &range;
      dstp = NULL;
    }
  else if (api->match_dst_port_num >= 1)
    {
      bgp_pbr_extract (api->dst_port, api->match_dst_port_num, &range);
      dstp = &range;
      srcp = NULL;
    }
  if (api->match_icmp_type_num >= 1)
    {
      proto = IPPROTO_ICMP;
      if (bgp_pbr_extract (api->icmp_type, api->match_icmp_type_num, &range))
        srcp = &range;
      else
        {
          bpof.icmp_type = list_new ();
          bgp_pbr_extract_enumerate (api->icmp_type, api->match_icmp_type_num,
                                     OPERATOR_UNARY_OR, bpof.icmp_type,
                                     FLOWSPEC_ICMP_TYPE);
        }
    }
  if (api->match_icmp_code_num >= 1)
    {
      proto = IPPROTO_ICMP;
      if (bgp_pbr_extract (api->icmp_code, api->match_icmp_code_num,
                           &range_icmp_code))
        dstp = &range_icmp_code;
      else
        {
          bpof.icmp_code = list_new ();
          bgp_pbr_extract_enumerate (api->icmp_code, api->match_icmp_code_num,
                                     OPERATOR_UNARY_OR, bpof.icmp_code,
                                     FLOWSPEC_ICMP_CODE);
        }
    }
      ...
      if (ret)
      ...
      else
        {
          ...
          bgp_pbr_extract_enumerate (
              api->packet_length, api->match_packet_length_num,
              OPERATOR_UNARY_OR, bpof.pkt_len, FLOWSPEC_PKT_LEN);
        }
    }
  if (api->match_dscp_num >= 1)
    {
      ...
      bgp_pbr_extract_enumerate (api->dscp, api->match_dscp_num,
                                 OPERATOR_UNARY_OR, bpof.dscp, FLOWSPEC_DSCP);
    }
  if (api->match_fragment_num)
    {
      bpof.fragment = list_new ();
      bgp_pbr_extract_enumerate (api->fragment, api->match_fragment_num,
                                 OPERATOR_UNARY_OR, bpof.fragment,
                                 FLOWSPEC_FRAGMENT);
    }
          ...
          if (api->actions[i].u.r.rate == 0)
            {
              ...
              bgp_pbr_policyroute_add_to_zebra (bgp, path, &bpf, &bpof, &nh,
                                                &rate);
            }
          else
            {
        ...
        case ACTION_REDIRECT_IP:
          ...
          if (api->afi == AFI_IP)
          ...
          else
            {
              ...
              memcpy (&nh.gate.ipv6, &api->actions[i].u.zr.redirect_ip_v6,
                      sizeof (struct in6_addr));
            }
          bgp_pbr_policyroute_add_to_zebra (bgp, path, &bpf, &bpof, &nh,
                                            &rate);
...
void
bgp_pbr_update_entry (struct bgp *bgp, const struct prefix *p,
                      ...
                      bool nlri_update)
{
  ...
  if (! bgp_zebra_tm_chunk_obtained ())
    {
      if (BGP_DEBUG (pbr, PBR_ERROR))
        flog_err (EC_BGP_TABLE_CHUNK, "%s: table chunk not obtained yet",
                  __func__);
      return;
    }
  ...
  if (bgp_pbr_build_and_validate_entry (p, info, &api) < 0)
    {
      if (BGP_DEBUG (pbr, PBR_ERROR))
        flog_err (EC_BGP_FLOWSPEC_INSTALLATION,
                  "%s: cancel updating entry %p in bgp pbr", __func__, info);
      return;
    }
...
int
bgp_pbr_interface_compare (const struct bgp_pbr_interface *a,
                          ...
                          struct bgp_pbr_interface_head *head)
{
  ...
  return (RB_FIND (bgp_pbr_interface_head, head, &pbr_if));
}
 ...
 */
void
bgp_pbr_reset (struct bgp *bgp, afi_t afi)
{
