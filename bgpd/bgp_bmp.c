...
static int
bmp_bgp_cmp (const struct bmp_bgp *a, const struct bmp_bgp *b)
{
  if (a->bgp < b->bgp)
    return -1;
  if (a->bgp > b->bgp)
...
static uint32_t
bmp_bgp_hash (const struct bmp_bgp *e)
{
...
static int
bmp_bgp_peer_cmp (const struct bmp_bgp_peer *a, const struct bmp_bgp_peer *b)
{
...
static uint32_t
bmp_bgp_peer_hash (const struct bmp_bgp_peer *e)
{
  return e->peerid;
}

DECLARE_HASH (bmp_peerh, struct bmp_bgp_peer, bpi, bmp_bgp_peer_cmp,
              bmp_bgp_peer_hash);
...
static int
bmp_listener_cmp (const struct bmp_listener *a, const struct bmp_listener *b)
{
  int c;
...
static void
bmp_listener_put (struct bmp_listener *bl)
{
...
static int
bmp_targets_cmp (const struct bmp_targets *a, const struct bmp_targets *b)
{
...
static int
bmp_qhash_cmp (const struct bmp_queue_entry *a,
               const struct bmp_queue_entry *b)
{
  ...
  if (a->afi == b->afi && a->safi == SAFI_MPLS_VPN && b->safi == SAFI_MPLS_VPN)
    {
      ret = prefix_cmp (&a->rd, &b->rd);
      if (ret)
        return ret;
...
static uint32_t
bmp_qhash_hkey (const struct bmp_queue_entry *e)
{
  uint32_t key;
...
DECLARE_HASH (bmp_qhash, struct bmp_queue_entry, bhi, bmp_qhash_cmp,
              bmp_qhash_hkey);

static int
bmp_active_cmp (const struct bmp_active *a, const struct bmp_active *b)
{
  int c;
...
static struct bmp *
bmp_new (struct bmp_targets *bt, int bmp_sock)
{
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      new->afistate[afi][safi] =
          bt->afimon[afi][safi] ? BMP_AFI_NEEDSYNC : BMP_AFI_INACTIVE;
    }
...
static void
bmp_common_hdr (struct stream *s, uint8_t ver, uint8_t type)
{
  stream_putc (s, ver);
...
static void
bmp_per_peer_hdr (struct stream *s, struct peer *peer, uint8_t flags,
...
static void
bmp_put_info_tlv (struct stream *s, uint16_t type, const char *string)
{
...
static int
bmp_send_initiation (struct bmp *bmp)
{
  int len;
  struct stream *s;
...
static void
bmp_notify_put (struct stream *s, struct bgp_notify *nfy)
{
  size_t len_pos;
  uint8_t marker[16] = {
  ...
  stream_putw_at (s, len_pos, stream_get_endp (s) - len_pos + sizeof (marker));
}

static struct stream *
bmp_peerstate (struct peer *peer, bool down)
{
  struct stream *s;
  size_t len;
  ...
  if (peer_established (peer) && ! down)
    {
      ...
      static const uint8_t dummy_open[] = {
        ...
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x13, 0x01,
      };
  ...
  else
    {
      ...
      bmp_common_hdr (s, BMP_VERSION_3, BMP_TYPE_PEER_DOWN_NOTIFICATION);
      bmp_per_peer_hdr (s, peer, 0, &uptime_real);
...
static int
bmp_send_peerup (struct bmp *bmp)
{
  struct peer *peer;
...
static void
bmp_send_all (struct bmp_bgp *bmpbgp, struct stream *s)
{
...
static struct bmp_mirrorq *
bmp_pull_mirror (struct bmp *bmp)
{
...
static void
bmp_mirror_cull (struct bmp_bgp *bmpbgp)
{
          ...
          frr_each (bmp_session, &bt->sessions, bmp)
            {
              ...
              while ((inner = bmp_pull_mirror (bmp)))
                {
                  if (! inner->refcount)
                    XFREE (MTYPE_BMP_MIRRORQ, inner);
                }

              zlog_warn (
                  "bmp[%s] lost mirror messages due to buffer size limit",
                  bmp->remote);
...
static int
bmp_mirror_packet (struct peer *peer, uint8_t type, bgp_size_t size,
  ...
  if (qitem->refcount == 0)
  ...
  else
    {
      ...
      bmpbgp->mirror_qsizemax =
          MAX (bmpbgp->mirror_qsizemax, bmpbgp->mirror_qsize);
    }
  return 0;
}

static void
bmp_wrmirror_lost (struct bmp *bmp, struct pullwr *pullwr)
{
  struct stream *s;
  struct timeval tv;
...
static bool
bmp_wrmirror (struct bmp *bmp, struct pullwr *pullwr)
{
  struct bmp_mirrorq *bmq;
  struct peer *peer;
  bool written = false;
...
static int
bmp_outgoing_packet (struct peer *peer, uint8_t type, bgp_size_t size,
                     struct stream *packet)
{
  if (type == BGP_MSG_OPEN)
    {
...
static int
bmp_peer_status_changed (struct peer *peer)
{
  struct bmp_bgp *bmpbgp = bmp_bgp_find (peer->bgp);
...
static int
bmp_peer_backward (struct peer *peer)
{
  struct bmp_bgp *bmpbgp = bmp_bgp_find (peer->bgp);
  struct bmp_bgp_peer *bbpeer;
...
static void
bmp_eor (struct bmp *bmp, afi_t afi, safi_t safi, uint8_t flags)
{
  struct peer *peer;
  struct listnode *node;
  struct stream *s, *s2;
  ...
  for (ALL_LIST_ELEMENTS_RO (bmp->targets->bgp->peer, node, peer))
    {
      ...
      bmp_common_hdr (s2, BMP_VERSION_3, BMP_TYPE_ROUTE_MONITORING);
...
static struct stream *
bmp_update (const struct prefix *p, struct prefix_rd *prd, struct peer *peer,
            struct attr *attr, afi_t afi, safi_t safi)
{
  ...
  total_attr_len =
      bgp_packet_attribute (NULL, peer, s, attr, &vecarr, NULL, afi, safi,
                            peer, NULL, NULL, 0, 0, 0, NULL);
  ...
  if (afi == AFI_IP && safi == SAFI_UNICAST)
  ...
  else
    {
      ...
      mpattrlen_pos =
          bgp_packet_mpattr_start (s, peer, afi, safi, &vecarr, attr);
...
static struct stream *
bmp_withdraw (const struct prefix *p, struct prefix_rd *prd, afi_t afi,
              safi_t safi)
{
  struct stream *s;
  ...
  if (afi == AFI_IP && safi == SAFI_UNICAST)
  ...
  else
    {
      ...
      bgp_packet_mpunreach_prefix (s, p, afi, safi, prd, NULL, 0, 0, 0, NULL);
...
static void
bmp_monitor (struct bmp *bmp, struct peer *peer, uint8_t flags,
...
static bool
bmp_wrsync (struct bmp *bmp, struct pullwr *pullwr)
{
  afi_t afi;
  safi_t safi;
      ...
      FOREACH_AFI_SAFI (afi, safi)
        {
          ...
          bmp->syncrdpos = NULL;
          zlog_info ("bmp[%s] %s %s sending table", bmp->remote,
...
afibreak:
  ...
  do
    {
          ...
          if (! bn)
            {
              if ((afi == AFI_L2VPN && safi == SAFI_EVPN) ||
                  (safi == SAFI_MPLS_VPN))
                {
                   ...
                   */
                  for (bmp->syncrdpos = bgp_route_next (bmp->syncrdpos);
                       bmp->syncrdpos;
                       bmp->syncrdpos = bgp_route_next (bmp->syncrdpos))
                    if (bgp_dest_get_bgp_table_info (bmp->syncrdpos))
                      return true;
                }
            eor:
              zlog_info ("bmp[%s] %s %s table completed (EoR)", bmp->remote,
          ...
          for (bpiter = bgp_dest_get_bgp_path_info (bn); bpiter;
               bpiter = bpiter->next)
            {
              if (! CHECK_FLAG (bpiter->flags, BGP_PATH_VALID))
                continue;
              if (bpiter->peer->qobj_node.nid <= bmp->syncpeerid)
                continue;
              if (bpi &&
                  bpiter->peer->qobj_node.nid > bpi->peer->qobj_node.nid)
                continue;
              bpi = bpiter;
            }
        }
      if (bmp->targets->afimon[afi][safi] & BMP_MON_PREPOLICY)
        {
          for (adjiter = bn->adj_in; adjiter; adjiter = adjiter->next)
            {
              if (adjiter->peer->qobj_node.nid <= bmp->syncpeerid)
                continue;
              if (adjin &&
                  adjiter->peer->qobj_node.nid > adjin->peer->qobj_node.nid)
                continue;
              adjin = adjiter;
            }
        }
  ...
  if (adjin && bpi && adjin->peer->qobj_node.nid < bpi->peer->qobj_node.nid)
    {
      bpi = NULL;
  ...
  if (bpi)
    bmp_monitor (bmp, bpi->peer, BMP_PEER_FLAG_L, bn_p, prd, bpi->attr, afi,
                 safi, bpi->uptime);
  if (adjin)
    bmp_monitor (bmp, adjin->peer, 0, bn_p, prd, adjin->attr, afi, safi,
                 adjin->uptime);
...
static bool
bmp_wrqueue (struct bmp *bmp, struct pullwr *pullwr)
{
  struct bmp_queue_entry *bqe;
  struct peer *peer;
  struct bgp_dest *bn = NULL;
  ...
  bn = bgp_safi_node_lookup (bmp->targets->bgp->rib[afi][safi], safi, &bqe->p,
                             prd);
  ...
  if (bmp->targets->afimon[afi][safi] & BMP_MON_PREPOLICY)
    {
      ...
      bmp_monitor (bmp, peer, 0, &bqe->p, prd, adjin ? adjin->attr : NULL, afi,
                   safi, adjin ? adjin->uptime : monotime (NULL));
      written = true;
    }
...
static void
bmp_wrfill (struct bmp *bmp, struct pullwr *pullwr)
{
  switch (bmp->state)
    {
    case BMP_PeerUp:
...
static void
bmp_wrerr (struct bmp *bmp, struct pullwr *pullwr, bool eof)
{
  if (eof)
    zlog_info ("bmp[%s] disconnected", bmp->remote);
  else
    flog_warn (EC_LIB_SYSTEM_CALL, "bmp[%s] connection error: %s", bmp->remote,
               strerror (errno));
...
static void
bmp_process_one (struct bmp_targets *bt, struct bgp *bgp, afi_t afi,
...
static int
bmp_process (struct bgp *bgp, afi_t afi, safi_t safi, struct bgp_dest *bn,
  ...
  if (frrtrace_enabled (frr_bgp, bmp_process))
    {
      ...
      frrtrace (5, frr_bgp, bmp_process, peer, pfxprint, afi, safi, withdraw);
    }
...
static void
bmp_stat_put_u32 (struct stream *s, size_t *cnt, uint16_t type, uint32_t value)
{
  stream_putw (s, type);
  stream_putw (s, 4);
  stream_putl (s, value);
...
static struct bmp *
bmp_open (struct bmp_targets *bt, int bmp_sock)
{
  ...
  snprintf (buf + strlen (buf), sizeof (buf) - strlen (buf), ":%u",
            su.sa.sa_family == AF_INET ? ntohs (su.sin.sin_port)
  ...
  bmp->pullwr = pullwr_new (bm->master, bmp_sock, bmp, bmp_wrfill, bmp_wrerr);
...
static void
bmp_accept (struct event *thread)
{
  union sockunion su;
...
static struct bmp_bgp *
bmp_bgp_find (struct bgp *bgp)
{
...
static void
bmp_bgp_put (struct bmp_bgp *bmpbgp)
{
  struct bmp_targets *bt;
...
static struct bmp_bgp_peer *
bmp_bgp_peer_get (struct peer *peer)
{
  struct bmp_bgp_peer *bbpeer;
...
static struct bmp_targets *
bmp_targets_find1 (struct bgp *bgp, const char *name)
{
  struct bmp_bgp *bmpbgp = bmp_bgp_find (bgp);
...
static struct bmp_targets *
bmp_targets_get (struct bgp *bgp, const char *name)
{
  struct bmp_targets *bt;
...
static void
bmp_targets_put (struct bmp_targets *bt)
{
  struct bmp *bmp;
...
static struct bmp_listener *
bmp_listener_find (struct bmp_targets *bt, const union sockunion *su, int port)
{
...
static struct bmp_listener *
bmp_listener_get (struct bmp_targets *bt, const union sockunion *su, int port)
{
...
static void
bmp_listener_start (struct bmp_listener *bl)
{
  int sock, ret;
...
static void
bmp_listener_stop (struct bmp_listener *bl)
{
...
static struct bmp_active *
bmp_active_find (struct bmp_targets *bt, const char *hostname, int port)
{
...
static struct bmp_active *
bmp_active_get (struct bmp_targets *bt, const char *hostname, int port)
{
  struct bmp_active *ba;
...
static void
bmp_active_put (struct bmp_active *ba)
{
...
static void
bmp_active_connect (struct bmp_active *ba)
{
  ...
  for (; ba->addrpos < ba->addrtotal; ba->addrpos++)
    {
      if (ba->ifsrc)
        {
          ...
          if (! ifp)
            {
              zlog_warn ("bmp[%s]: failed to find interface", ba->ifsrc);
              continue;
            }

          if (bgp_update_address (ifp, &ba->addrs[ba->addrpos], &ba->addrsrc))
            {
              zlog_warn ("bmp[%s]: failed to find matching address",
                         ba->ifsrc);
              continue;
            }
          zlog_info ("bmp[%s]: selected source address : %pSU", ba->ifsrc,
                     &ba->addrsrc);
        }
      ...
      if (ba->socket < 0)
        {
          zlog_warn ("bmp[%s]: failed to create socket", ba->hostname);
          continue;
        }
      ...
      if (! sockunion_is_null (&ba->addrsrc))
        {
          res_bind =
              sockunion_bind (ba->socket, &ba->addrsrc, 0, &ba->addrsrc);
          if (res_bind < 0)
            {
              zlog_warn (
      ...
      switch (res)
        {
        case connect_error:
          zlog_warn ("bmp[%s]: failed to connect to %pSU:%d", ba->hostname,
        ...
        case connect_success:
          zlog_info ("bmp[%s]: connected to  %pSU:%d", ba->hostname,
                     &ba->addrs[ba->addrpos], ba->port);
          break;
        case connect_in_progress:
          zlog_warn ("bmp[%s]: connect in progress  %pSU:%d", ba->hostname,
                     &ba->addrs[ba->addrpos], ba->port);
          bmp_active_setup (ba);
          return;
        }
...
static void
bmp_active_resolved (struct resolver_query *resq, const char *errstr,
                     int numaddrs, union sockunion *addr)
{
  ...
  if (numaddrs <= 0)
    {
      zlog_warn ("bmp[%s]: hostname resolution failed: %s", ba->hostname,
                 errstr);
      ba->last_err = errstr;
      ba->curretry += ba->curretry / 2;
      ba->addrpos = 0;
...
static void
bmp_active_thread (struct event *t)
{
  ...
  slen = sizeof (status);
  ret = getsockopt (ba->socket, SOL_SOCKET, SO_ERROR, (void *) &status, &slen);

  if (ret < 0 || status != 0)
    {
      ba->last_err = strerror (status);
      zlog_warn ("bmp[%s]: failed to connect to %pSU:%d: %s", ba->hostname,
                 &ba->addrs[ba->addrpos], ba->port, ba->last_err);
      goto out_next;
    }
...
static void
bmp_active_disconnected (struct bmp_active *ba)
{
  ba->bmp = NULL;
  bmp_active_setup (ba);
}
...
static void
bmp_active_setup (struct bmp_active *ba)
{
  EVENT_OFF (ba->t_timer);
  EVENT_OFF (ba->t_read);
  EVENT_OFF (ba->t_write);
  ...
  if (ba->socket == -1)
    event_add_timer_msec (bm->master, bmp_active_thread, ba, ba->curretry,
                          &ba->t_timer);
  else
    {
...
static void
bmp_targets_autocomplete (vector comps, struct cmd_token *token)
{
  struct bgp *bgp;
  ...
  for (ALL_LIST_ELEMENTS_RO (bm->bgp, node, bgp))
    {
      ...
      frr_each_safe (bmp_targets, &bmpbgp->targets, target)
        vector_set (comps, XSTRDUP (MTYPE_COMPLETION, target->name));
    }
}
  ...
  if (no)
    {
      ...
      if (ba->ifsrc || srcif)
        if ((! ba->ifsrc) || (! srcif) || ! strcmp (ba->ifsrc, srcif))
          {
            vty_out (vty, "%% No such active connection found\n");
            return CMD_WARNING;
          }
  ...
  frr_each (bmp_bgph, &bmp_bgph, bmpbgp)
    {
      ...
      vty_out (vty, "  Route Mirroring %9zu bytes (%zu messages) pending\n",
               bmpbgp->mirror_qsize, bmp_mirrorq_count (&bmpbgp->mirrorq));
          ...
          frr_each (bmp_actives, &bt->actives, ba)
            {
              ...
              if (ba->bmp)
                {
                  peer_uptime (ba->bmp->t_up.tv_sec, uptime, sizeof (uptime),
                               false, NULL);
                  ttable_add_row (tt, "%s:%d|Up|%s|%s|%pSU", ba->hostname,
                                  ba->port, ba->bmp->remote, uptime,
                                  &ba->addrsrc);
                  continue;
                }
              ...
              if (ba->t_timer)
                {
                  long trem = event_timer_remain_second (ba->t_timer);

                  peer_uptime (monotime (NULL) - trem, uptime, sizeof (uptime),
                               false, NULL);
                  state_str = "RetryWait";
                }
              else if (ba->t_read)
              ...
              ttable_add_row (
                  tt, "%s:%d|%s|%s|%s|%pSU", ba->hostname, ba->port, state_str,
                  ba->last_err ? ba->last_err : "", uptime, &ba->addrsrc);
              continue;
            }
          out = ttable_dump (tt, "\n");
          vty_out (vty, "%s", out);
          ...
          frr_each (bmp_session, &bt->sessions, bmp)
            {
              ...
              peer_uptime (bmp->t_up.tv_sec, uptime, sizeof (uptime), false,
                           NULL);

              ttable_add_row (tt, "%s|%s|%Lu|%Lu|%Lu|%Lu|%zu|%zu", bmp->remote,
                              uptime, bmp->cnt_update, bmp->cnt_mirror,
                              bmp->cnt_mirror_overruns, total, q, kq);
            }
          out = ttable_dump (tt, "\n");
          vty_out (vty, "%s", out);
          XFREE (MTYPE_TMP, out);
...
static int
bmp_config_write (struct bgp *bgp, struct vty *vty)
{
  struct bmp_bgp *bmpbgp = bmp_bgp_find (bgp);
  struct bmp_targets *bt;
  struct bmp_listener *bl;
...
static int
bgp_bmp_init (struct event_loop *tm)
{
