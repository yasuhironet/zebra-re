...
DEFINE_HOOK (bgp_inst_config_write, (struct bgp * bgp, struct vty *vty),
             (bgp, vty));
...
static const char *
get_afi_safi_vty_str (afi_t afi, safi_t safi)
{
  if (afi == AFI_IP)
    {
 ...
 */
static const char *
get_afi_safi_json_str (afi_t afi, safi_t safi)
{
  if (afi == AFI_IP)
    {
      if (safi == SAFI_UNICAST)
...
/* unset srv6 locator */
static int
bgp_srv6_locator_unset (struct bgp *bgp)
{
  int ret;
  ...
  ret = bgp_zebra_srv6_manager_release_locator_chunk (bgp->srv6_locator_name);
  if (ret < 0)
    return -1;
  ...
  for (ALL_LIST_ELEMENTS_RO (bm->bgp, node, bgp_vrf))
    {
      ...
      srv6_locator_chunk_free (&bgp_vrf->vpn_policy[AFI_IP].tovpn_sid_locator);
...
int
argv_find_and_parse_afi (struct cmd_token **argv, int argc, int *index,
                         afi_t *afi)
{
  int ret = 0;
...
int
argv_find_and_parse_safi (struct cmd_token **argv, int argc, int *index,
                          safi_t *safi)
{
  int ret = 0;
 ...
 */
static const char *
get_bgp_default_af_flag (afi_t afi, safi_t safi)
{
  switch (afi)
    {
    case AFI_IP:
...
int
bgp_get_vty (struct bgp **bgp, as_t *as, const char *name,
 ...
 * safi -> The parsed safi if it was included in the show command, returned
 * here bgp  -> Pointer to the bgp data structure we need to fill in. use_json
 * -> json is configured or not
 *
 ...
 */
int
bgp_vty_find_and_parse_afi_safi_bgp (struct vty *vty, struct cmd_token **argv,
      ...
      if (! *bgp)
        {
          if (use_json)
            ...
            vty_out (vty, "Default BGP instance not found\n");
          *idx = 0;
          return 0;
        }
    }
  ...
  if (ifp)
    {
      for (ALL_LIST_ELEMENTS_RO (bm->listen_sockets, node, listener))
        {
          if (sockunion_family (su) != sockunion_family (&listener->su))
            continue;
...
int
bgp_vty_return (struct vty *vty, enum bgp_create_error_code ret)
{
  const char *str = NULL;
                     ...
                     safi_t safi, int error)
{
  switch (error)
    ...
    case BGP_ERR_AF_UNCONFIGURED:
      if (vty)
        vty_out (vty, "%% BGP: Enable %s address family for the neighbor %s\n",
                 get_afi_safi_str (afi, safi, false), peer->host);
      else
        zlog_warn ("%% BGP: Enable %s address family for the neighbor %s",
                   get_afi_safi_str (afi, safi, false), peer->host);
      break;
    case BGP_ERR_SOFT_RECONFIG_UNCONFIGURED:
      if (vty)
...
static int
bgp_peer_clear (struct peer *peer, afi_t afi, safi_t safi,
                struct listnode **nnode, enum bgp_clear_type stype)
{
  ...
  if ((afi == AFI_UNSPEC) && (safi == SAFI_UNSPEC))
      ...
      for (index = BGP_AF_START; index < BGP_AF_MAX; index++)
        {
          ...
          if (stype == BGP_CLEAR_SOFT_NONE)
            ...
            ret = peer_clear_soft (peer, tmp_afi, tmp_safi, stype);
        }
  ...
  else if (safi == SAFI_UNSPEC)
    {
      ...
      for (tmp_safi = SAFI_UNICAST; tmp_safi < SAFI_MAX; tmp_safi++)
        {
          ...
          if (stype == BGP_CLEAR_SOFT_NONE)
            ...
            ret = peer_clear_soft (peer, afi, tmp_safi, stype);
        }
  ...
  if (sort == clear_external)
    {
      ...
      if (! found)
        vty_out (vty, "%% BGP: No external %s peer is configured\n",
  ...
  if (sort == clear_as)
    {
      ...
      if (! found)
        vty_out (vty, "%% BGP: No %s peer is configured with AS %s\n",
                 get_afi_safi_str (afi, safi, false), arg);
...
bgp_clear_star_soft_in (struct vty *vty, const char *name)
{
  ...
  FOREACH_AFI_SAFI (afi, safi)
    bgp_clear_vty (vty, name, afi, safi, clear_all, BGP_CLEAR_SOFT_IN, NULL);
}
...
bgp_clear_star_soft_out (struct vty *vty, const char *name)
{
  ...
  FOREACH_AFI_SAFI (afi, safi)
    bgp_clear_vty (vty, name, afi, safi, clear_all, BGP_CLEAR_SOFT_OUT, NULL);
}


void
bgp_clear_soft_in (struct bgp *bgp, afi_t afi, safi_t safi)
{
  bgp_clear (NULL, bgp, afi, safi, clear_all, BGP_CLEAR_SOFT_IN, NULL);
}

static int
peer_flag_modify_vty (struct vty *vty, const char *ip_str, uint64_t flag,
                      int set)
{
  int ret;
  struct peer *peer;
...
static int
peer_flag_set_vty (struct vty *vty, const char *ip_str, uint64_t flag)
{
  return peer_flag_modify_vty (vty, ip_str, flag, 1);
}

static int
peer_flag_unset_vty (struct vty *vty, const char *ip_str, uint64_t flag)
{
      ...
      if (argv_find (argv, argc, "as-notation", &idx_asnotation))
        {
          ...
          else if (strmatch (argv[idx_asnotation_kind]->text, "dot"))
            asnotation = ASNOTATION_DOT;
          else if (strmatch (argv[idx_asnotation_kind]->text, "plain"))
      ...
      if (lookup_alias)
        {
          ...
          if (bgp_ca_community_lookup (&ca))
            {
              vty_out (vty, "community (%s) already has this alias (%s)\n",
                       lookup_alias->community, lookup_alias->alias);
              return CMD_WARNING;
            }
          bgp_ca_alias_delete (&ca);
        }
      ...
      if (lookup_community)
        {
          ...
          if (bgp_ca_alias_lookup (&ca))
            {
              vty_out (vty, "alias (%s) already has this community (%s)\n",
                       lookup_community->alias, lookup_community->community);
              return CMD_WARNING;
            }
          bgp_ca_community_delete (&ca);
        }
       ...
       "Disable BGP route installation to RIB (Zebra)\n")
{
  if (bgp_option_check (BGP_OPT_NO_FIB))
    {
      vty_out (vty, "%% No-RIB option is already set, nothing to do here.\n");
      return CMD_SUCCESS;
    }
       ...
       "Disable BGP route installation to RIB (Zebra)\n")
{
  if (! bgp_option_check (BGP_OPT_NO_FIB))
    {
      vty_out (vty, "%% No-RIB option is not set, nothing to do here.\n");
      return CMD_SUCCESS;
    }
                         ...
                         uint16_t options, int set)
{
  ...
  if (set)
    {
      ...
      ret =
          bgp_maximum_paths_set (bgp, afi, safi, peer_type, maxpaths, options);
    }
  else
  ...
  if (ret < 0)
    {
      vty_out (
          vty, "%% Failed to %sset maximum-paths %s %u for afi %u, safi %u\n",
          (set == 1) ? "" : "un",
          (peer_type == BGP_PEER_EBGP) ? "ebgp" : "ibgp", maxpaths, afi, safi);
      return CMD_WARNING_CONFIG_FAILED;
    }
...
static int
bgp_global_update_delay_config_vty (struct vty *vty, uint16_t update_delay,
                                    uint16_t establish_wait)
{
      ...
      for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
        {
          if (bgp->v_update_delay != BGP_UPDATE_DELAY_DEF)
            {
              vty_out (vty, "%% update-delay configuration found in vrf %s\n",
  ...
  if (vrf_cfg)
    {
      vty_out (vty, "%%Failed: global update-delay config not permitted\n");
      return CMD_WARNING;
    }
...
static int
bgp_global_update_delay_deconfig_vty (struct vty *vty)
{
  struct listnode *node, *nnode;
  struct bgp *bgp;
...
static int
bgp_update_delay_config_vty (struct vty *vty, uint16_t update_delay,
                             uint16_t establish_wait)
{
  VTY_DECLVAR_CONTEXT (bgp, bgp);
...
static int
bgp_wpkt_quanta_config_vty (struct vty *vty, uint32_t quanta, bool set)
{
  VTY_DECLVAR_CONTEXT (bgp, bgp);
...
static int
bgp_rpkt_quanta_config_vty (struct vty *vty, uint32_t quanta, bool set)
{
  VTY_DECLVAR_CONTEXT (bgp, bgp);
...
void
bgp_config_write_rpkt_quanta (struct vty *vty, struct bgp *bgp)
{
  uint32_t quanta =
...
/* BGP TCP keepalive */
static void
bgp_config_tcp_keepalive (struct vty *vty, struct bgp *bgp)
{
  if (bgp->tcp_keepalive_idle)
    {
      vty_out (vty, " bgp tcp-keepalive %u %u %u\n", bgp->tcp_keepalive_idle,
               bgp->tcp_keepalive_intvl, bgp->tcp_keepalive_probes);
    }
}
...
{
  int idx_number = 1;
  return bgp_maxpaths_config_vty (vty, BGP_PEER_EBGP, argv[idx_number]->arg, 0,
                                  1);
}
...
{
  int idx_number = 2;
  return bgp_maxpaths_config_vty (vty, BGP_PEER_IBGP, argv[idx_number]->arg, 0,
                                  1);
}
...
DEFUN (bgp_maxpaths_ibgp_cluster,
       ...
       CMD_RANGE_STR
       (1,
       MULTIPATH_NUM)" equal-cluster-length",
       "Forward packets over multiple paths\n"
       "iBGP-multipath\n"
       "Number of paths\n"
...
DEFUN (bgp_ebgp_requires_policy,
...
DEFUN (no_bgp_ebgp_requires_policy,
...
DEFPY (bgp_lu_uses_explicit_null,
       bgp_lu_uses_explicit_null_cmd,
...
DEFUN (no_bgp_suppress_duplicates,
       ...
       GR_DISABLE)
{
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] bgp_graceful_restart_disable_cmd : START ");
       ...
       NO_GR_DISABLE)
{
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] no_bgp_graceful_restart_disable_cmd : START ");
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] no_bgp_graceful_restart_disable_cmd : END ");
  vty_out (
      vty,
      "Graceful restart configuration changed, reset all peers to take effect\n");
       ...
       GR_NEIGHBOR_CMD)
{
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] bgp_neighbor_graceful_restart_set_cmd : START ");
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] bgp_neighbor_graceful_restart_set_cmd : END ");
  vty_out (
      vty,
       ...
       NO_GR_NEIGHBOR_CMD)
{
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] no_bgp_neighbor_graceful_restart_set_cmd : START ");
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] no_bgp_neighbor_graceful_restart_set_cmd : END ");
  vty_out (
      vty,
      "Graceful restart configuration changed, reset this peer to take effect\n");
       ...
       "Graceful shutdown\n")
{
  ...
  if (no)
    ret = peer_flag_unset_vty (vty, neighbor, PEER_FLAG_GRACEFUL_SHUTDOWN);
  else
    ret = peer_flag_set_vty (vty, neighbor, PEER_FLAG_GRACEFUL_SHUTDOWN);
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      ...
      bgp_clear (vty, bgp, afi, safi, clear_peer, BGP_CLEAR_SOFT_IN, neighbor);
    }
...
DEFUN (no_bgp_llgr_stalepath_time,
...
static inline void
bgp_initiate_graceful_shut_unshut (struct vty *vty, struct bgp *bgp)
{
...
static int
bgp_global_graceful_shutdown_config_vty (struct vty *vty)
{
  struct listnode *node, *nnode;
  struct bgp *bgp;
  bool vrf_cfg = false;
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
      if (CHECK_FLAG (bgp->flags, BGP_FLAG_GRACEFUL_SHUTDOWN))
        {
          vty_out (vty, "%% graceful-shutdown configuration found in vrf %s\n",
                   bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT
                       ? VRF_DEFAULT_NAME
                       : bgp->name);
          vrf_cfg = true;
  ...
  if (vrf_cfg)
    {
      vty_out (vty, "%%Failed: global graceful-shutdown not permitted\n");
      return CMD_WARNING;
    }
...
static int
bgp_global_graceful_shutdown_deconfig_vty (struct vty *vty)
{
  struct listnode *node, *nnode;
  struct bgp *bgp;
...
listen_range_exists (struct bgp *bgp, struct prefix *range, int exact)
{
  ...
  for (ALL_LIST_ELEMENTS (bgp->group, node, nnode, group))
    {
      for (ALL_LIST_ELEMENTS (group->listen_range[afi], node1, nnode1, lr))
        {
          if (exact)
...
static int
peer_remote_as_vty (struct vty *vty, const char *peer_str, const char *as_str)
{
  VTY_DECLVAR_CONTEXT (bgp, bgp);
  int ret;
  as_t as;
...
static int
peer_conf_interface_get (struct vty *vty, const char *conf_if, int v6only,
                         const char *peer_group_name, const char *as_str)
{
  ...
  if (peer)
    ...
    {
      peer = peer_create (NULL, conf_if, bgp, bgp->as, as, as_type, NULL, true,
                          as_str);
       ...
       "Peer-group name\n")
{
  ...
  if (argc > idx_peer_group_word)
    ...
    return peer_conf_interface_get (vty, argv[idx_word]->arg, 0, NULL, NULL);
}
       ...
       "Peer-group name\n")
{
  ...
  if (argc > idx_peer_group_word)
    return peer_conf_interface_get (vty, argv[idx_word]->arg, 1,
                                    argv[idx_peer_group_word]->arg, NULL);
       ...
       "The number of keepalives with higher RTT to shutdown\n")
{
  ...
  if (argv_find (argv, argc, "count", &idx_count))
    peer->rtt_keepalive_conf = strtol (argv[idx_count + 1]->arg, NULL, 10);

  return peer_flag_set_vty (vty, argv[idx_peer]->arg, PEER_FLAG_RTT_SHUTDOWN);
}
...
peer_af_flag_modify_vty (struct vty *vty, const char *peer_str, afi_t afi,
                         safi_t safi, uint64_t flag, int set)
{
  int ret;
  struct peer *peer;
       ...
       "Allow inbound soft reconfiguration for this neighbor\n")
{
  int idx_peer = 1;
  return peer_af_flag_set_vty (vty, argv[idx_peer]->arg, bgp_node_afi (vty),
                               bgp_node_safi (vty), PEER_FLAG_SOFT_RECONFIG);
}
       ...
       "Configure a neighbor as Route Server client\n")
{
  ...
  if (! peer)
    return CMD_WARNING_CONFIG_FAILED;
  return peer_af_flag_set_vty (vty, argv[idx_peer]->arg, bgp_node_afi (vty),
                               bgp_node_safi (vty), PEER_FLAG_RSERVER_CLIENT);
}
       ...
       "Med attribute\n")
{
  ...
  if (! aspath && ! nexthop && ! med)
  ...
  else
    {
      ...
      if (! med)
        {
          if (peer_af_flag_check (peer, afi, safi, PEER_FLAG_MED_UNCHANGED))
            {
              ret |= peer_af_flag_unset_vty (vty, peer_str, afi, safi,
                                             PEER_FLAG_MED_UNCHANGED);
            }
...
static uint8_t
get_role_by_name (const char *role_str)
{
...
static int
peer_role_set_vty (struct vty *vty, const char *ip_str, const char *role_str,
                   bool strict_mode)
{
  struct peer *peer;
...
static int
peer_role_unset_vty (struct vty *vty, const char *ip_str)
{
  struct peer *peer;
  ...
  if (set)
    {
      ...
      ret = peer_default_originate_set (peer, afi, safi, rmap, route_map);
    }
  else
...
/* neighbor weight. */
static int
peer_weight_set_vty (struct vty *vty, const char *ip_str, afi_t afi,
...
static int
peer_weight_unset_vty (struct vty *vty, const char *ip_str, afi_t afi,
                       safi_t safi)
{
  int ret;
  struct peer *peer;
...
{
  int idx_peer = 2;
  return peer_weight_unset_vty (vty, argv[idx_peer]->arg, bgp_node_afi (vty),
                                bgp_node_safi (vty));
}
...
static int
peer_timers_set_vty (struct vty *vty, const char *ip_str, const char *keep_str,
                     const char *hold_str)
{
  int ret;
  struct peer *peer;
       ...
       "Holdtime\n")
{
  ...
  int idx_number = 3;
  int idx_number_2 = 4;
  return peer_timers_set_vty (vty, argv[idx_peer]->arg, argv[idx_number]->arg,
       ...
       "Filter outgoing updates\n")
{
  ...
  int idx_word = 3;
  int idx_in_out = 4;
  return peer_prefix_list_set_vty (vty, argv[idx_peer]->arg,
...
static int
peer_aslist_set_vty (struct vty *vty, const char *ip_str, afi_t afi,
                     safi_t safi, const char *name_str, const char *direct_str)
{
  int ret;
  struct peer *peer;
  int direct = FILTER_IN;
...
static int
peer_aslist_unset_vty (struct vty *vty, const char *ip_str, afi_t afi,
                       safi_t safi, const char *direct_str)
{
  int ret;
  struct peer *peer;
...
static int
peer_advertise_map_set_vty (struct vty *vty, const char *ip_str, afi_t afi,
                            safi_t safi, const char *advertise_str,
                            ...
                            bool set)
{
  ...
  if (set)
    ret =
        peer_advertise_map_set (peer, afi, safi, advertise_str, advertise_map,
...
ALIAS_HIDDEN (neighbor_advertise_map,
...
static int
peer_route_map_set_vty (struct vty *vty, const char *ip_str, afi_t afi,
                        safi_t safi, const char *name_str,
                        const char *direct_str)
{
  int ret;
       ...
       "Name of route map\n")
{
  int idx_peer = 1;
  int idx_word = 3;
  return peer_unsuppress_map_set_vty (vty, argv[idx_peer]->arg,
                                      bgp_node_afi (vty), bgp_node_safi (vty),
                                      argv[idx_word]->arg);
}
...
{
  int idx_peer = 2;
  return peer_unsuppress_map_unset_vty (
      vty, argv[idx_peer]->arg, bgp_node_afi (vty), bgp_node_safi (vty));
}
...
{
  int idx_peer = 2;
  return peer_maximum_prefix_unset_vty (
      vty, argv[idx_peer]->arg, bgp_node_afi (vty), bgp_node_safi (vty));
}
       ...
       "Use addpath to advertise all paths to a neighbor\n")
{
  ...
  if (peer->addpath_type[bgp_node_afi (vty)][bgp_node_safi (vty)] !=
      BGP_ADDPATH_ALL)
    {
      vty_out (vty, "%% Peer not currently configured to transmit all paths.");
      return CMD_WARNING_CONFIG_FAILED;
    }
...
DEFPY (neighbor_aspath_loop_detection,
...
DEFPY (no_neighbor_aspath_loop_detection,
...
static int
set_ecom_list (struct vty *vty, int argc, struct cmd_token **argv,
  ...
  for (; argc; --argc, ++argv)
    {
      if (is_rt6)
        ecomadd =
            ecommunity_str2com_ipv6 (argv[0]->arg, ECOMMUNITY_ROUTE_TARGET, 0);
      else
        ecomadd =
            ecommunity_str2com (argv[0]->arg, ECOMMUNITY_ROUTE_TARGET, 0);
      if (! ecomadd)
        {
 ...
 */
static afi_t
vpn_policy_getafi (struct vty *vty, struct bgp *bgp, bool v2vimport)
{
  afi_t afi;
       ...
       "Route Distinguisher (<as-number>:<number> | <ip-address>:<number>)\n")
{
  ...
  if (yes)
    {
      bgp->vpn_policy[afi].tovpn_rd_pretty = XSTRDUP (MTYPE_BGP, rd_str);
      ...
      if (bgp->vpn_policy[afi].tovpn_label != MPLS_LABEL_NONE)
        {
          ...
          bgp_lp_release (LP_TYPE_VRF, &bgp->vpn_policy[afi],
  ...
  if (label_auto)
    {
      ...
      bgp_lp_get (LP_TYPE_VRF, &bgp->vpn_policy[afi], vpn_leak_label_callback);
    }
       ...
       "Automatically assign a label\n")
{
  ...
  if (argv_find (argv, argc, "no", &idx))
    yes = false;
  debug =
      (BGP_DEBUG (vpn, VPN_LEAK_TO_VRF) | BGP_DEBUG (vpn, VPN_LEAK_FROM_VRF));
       ...
       "Automatically assign a label\n")
{
  ...
  if (sid_auto)
  ...
  else if (sid_idx != 0)
    {
      /* SID allocation index-mode */
      if (debug)
        zlog_debug ("%s: idx %ld per-vrf sid alloc.", __func__, sid_idx);
...
static int
vpn_policy_getdirs (struct vty *vty, const char *dstr, int *dodir)
{
...
DEFPY (af_route_map_vpn_imexport,
       ...
       /*
       future:
       "route-map <vpn|evpn|vrf NAME> <import|export> RMAP"
       */
  ...
  if (! bgp_default)
    {
      ...
      if (ret)
        {
          vty_out (vty, "VRF default is not configured as a bgp instance\n");
          return CMD_WARNING;
        }
    }
...
DEFPY (af_no_import_vrf_route_map,
       ...
       "The name of the VRF\n")
{
  ...
  if (remove)
  ...
  else
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (bgp->vpn_policy[afi].import_vrf, node, vname))
        {
  ...
  if (yes)
    {
      ...
      bgp->vpn_policy[afi].import_redirect_rtlist = ecommunity_dup (ecom);
    }
  else
    {
      if (bgp->vpn_policy[afi].import_redirect_rtlist)
       ...
       "Specify SRv6 locator\n")
{
  ...
  snprintf (bgp->srv6_locator_name, sizeof (bgp->srv6_locator_name), "%s",
            name);
          ...
          if (rm != NULL)
            {
              ...
              if (rm_p->prefixlen == match.prefixlen)
                {
                  SET_FLAG (rm->flags, BGP_NODE_USER_CLEAR);
  ...
  if (argv_find (argv, argc, "soft", &idx))
    {
      if (argv_find (argv, argc, "in", &idx) ||
        ...
        clr_type = strmatch (argv[idx]->text, "in") ? BGP_CLEAR_SOFT_IN
                                                    : BGP_CLEAR_SOFT_OUT;
      else
       ...
       "IPv6 prefix\n")
{
  ...
  return bgp_clear_prefix (vty, NULL, prefix, AFI_IP6, safi, NULL);
}

DEFUN (clear_bgp_instance_ipv6_safi_prefix,
       clear_bgp_instance_ipv6_safi_prefix_cmd,
       "clear [ip] bgp <view|vrf> VIEWVRFNAME ipv6 "
       BGP_SAFI_CMD_STR
       " prefix X:X::X:X/M",
       CLEAR_STR
       IP_STR
       ...
       "IPv6 prefix\n")
{
  ...
  return bgp_clear_prefix (vty, vrfview, prefix, AFI_IP6, safi, NULL);
}
...
static inline void
calc_peers_cfgd_estbd (struct bgp *bgp, int *peers_cfgd, int *peers_estbd)
{
  struct peer *peer;
  struct listnode *node;
...
static void
print_bgp_vrfs (struct bgp *bgp, struct vty *vty, json_object *json,
                const char *type)
{
  ...
  if (json)
    {
      int64_t vrf_id_ui =
          (bgp->vrf_id == VRF_UNKNOWN) ? -1 : (int64_t) bgp->vrf_id;
...
static int
show_bgp_vrfs_detail_common (struct vty *vty, struct bgp *bgp,
  ...
  if (use_vrf)
    {
      if (json)
      ...
      else
        {
          ...
          if (bgp->l3vni)
            {
              vty_out (vty, "L3VNI %u, L3VNI-SVI %s, Router MAC %pEA\n",
                       bgp->l3vni,
                       ifindex2ifname (bgp->l3vni_svi_ifindex, bgp->vrf_id),
                       &bgp->rmac);
            }
  ...
  if (vrf_name)
    {
      ...
      show_bgp_vrfs_detail_common (vty, bgp, json_vrf, name, type, true);
  ...
  for (ALL_LIST_ELEMENTS_RO (inst, node, bgp))
    {
      ...
      if (! uj && count == 1)
        {
          vty_out (vty, "%4s  %-5s  %-16s  %9s  %10s  %-37s\n", "Type", "Id",
                   "routerId", "#PeersCfg", "#PeersEstb", "Name");
      ...
      show_bgp_vrfs_detail_common (vty, bgp, json_vrf, name, type, false);
...
static void
show_tip_entry (struct hash_bucket *bucket, void *args)
{
...
static void
bgp_show_martian_nexthops (struct vty *vty, struct bgp *bgp)
{
  ...
  hash_iterate (bgp->tip_hash,
                (void (*) (struct hash_bucket *, void *)) show_tip_entry, vty);
}
       ...
       "Global BGP memory statistics\n")
{
  ...
  if ((count = mtype_stats_alloc (MTYPE_ECOMMUNITY)))
    vty_out (vty, "%ld BGP ext-community entries, using %s of memory\n", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
...
static void
bgp_show_bestpath_json (struct bgp *bgp, json_object *json)
{
  ...
  if (CHECK_FLAG (bgp->flags, BGP_FLAG_ASPATH_MULTIPATH_RELAX))
    {
      if (CHECK_FLAG (bgp->flags, BGP_FLAG_MULTIPATH_RELAX_AS_SET))
        json_object_string_add (bestpath, "multiPathRelax", "as-set");
      else
        json_object_string_add (bestpath, "multiPathRelax", "true");
    }
  else
  ...
  if (CHECK_FLAG (bgp->flags, BGP_FLAG_MED_CONFED) ||
      CHECK_FLAG (bgp->flags, BGP_FLAG_MED_MISSING_AS_WORST))
    {
      ...
      if (CHECK_FLAG (bgp->flags, BGP_FLAG_MED_MISSING_AS_WORST))
        json_object_string_add (bestpath, "med", "missing-as-worst");
      else
...
static void
bgp_show_peer_reset (struct vty *vty, struct peer *peer,
  ...
  if (use_json)
    {
      if (peer->last_reset == PEER_DOWN_NOTIFY_SEND ||
          peer->last_reset == PEER_DOWN_NOTIFY_RECEIVED)
        {
          ...
          subcode_str =
              bgp_notify_subcode_str (peer->notify.code, peer->notify.subcode);

          snprintf (errorcodesubcode_hexstr, sizeof (errorcodesubcode_hexstr),
                    "%02X%02X", peer->notify.code, peer->notify.subcode);
          json_object_string_add (json_peer, "lastErrorCodeSubcode",
                                  errorcodesubcode_hexstr);
          snprintf (errorcodesubcode_str, 255, "%s%s", code_str, subcode_str);
          json_object_string_add (json_peer, "lastNotificationReason",
                                  errorcodesubcode_str);
          json_object_boolean_add (json_peer, "lastNotificationHardReset",
                                   peer->notify.hard_reset);
          if (peer->last_reset == PEER_DOWN_NOTIFY_RECEIVED &&
              peer->notify.code == BGP_NOTIFY_CEASE &&
              (peer->notify.subcode == BGP_NOTIFY_CEASE_ADMIN_SHUTDOWN ||
               peer->notify.subcode == BGP_NOTIFY_CEASE_ADMIN_RESET) &&
              peer->notify.length)
            {
              char msgbuf[1024];
              const char *msg_str;
              ...
              msg_str = bgp_notify_admin_message (
                  msgbuf, sizeof (msgbuf), (uint8_t *) peer->notify.data,
                  peer->notify.length);
              json_object_string_add (json_peer, "lastShutdownDescription",
                                      msg_str);
            }
        }
      json_object_string_add (json_peer, "lastResetDueTo",
                              peer_down_str[(int) peer->last_reset]);
      json_object_int_add (json_peer, "lastResetCode", peer->last_reset);
      ...
      if (peer->last_reset == PEER_DOWN_NOTIFY_SEND ||
          peer->last_reset == PEER_DOWN_NOTIFY_RECEIVED)
        {
          ...
          vty_out (vty, " Notification %s (%s%s%s)\n",
                   peer->last_reset == PEER_DOWN_NOTIFY_SEND ? "sent"
                                                             : "received",
                   code_str, subcode_str,
                   peer->notify.hard_reset
                       ? bgp_notify_subcode_str (BGP_NOTIFY_CEASE,
...
static inline bool
bgp_has_peer_failed (struct peer *peer, afi_t afi, safi_t safi)
{
  return ((! peer_established (peer)) || ! peer->afc_recv[afi][safi]);
}

static void
bgp_show_failed_summary (struct vty *vty, struct bgp *bgp, struct peer *peer,
                         json_object *json_peer, int max_neighbor_width,
                         bool use_json)
{
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json_peer, "connectionsEstablished",
                           peer->established);
      json_object_int_add (json_peer, "connectionsDropped", peer->dropped);
  ...
  else
    {
      ...
      if (peer->hostname && CHECK_FLAG (bgp->flags, BGP_FLAG_SHOW_HOSTNAME))
        len = vty_out (vty, "%s%s(%s)", dn_flag, peer->hostname, peer->host);
      else
      ...
      vty_out (vty, "%7d %7d %9s", peer->established, peer->dropped,
...
static char *
bgp_peer_description_stripped (char *desc, uint32_t size)
{
...
static bool
bgp_show_summary_is_peer_filtered (struct peer *peer, struct peer *fpeer,
                                   int as_type, as_t as)
{
                  ...
                  uint16_t show_flags)
{
  ...
  bool show_established = CHECK_FLAG (show_flags, BGP_SHOW_OPT_ESTABLISHED);
  ...
  if (use_json)
    {
      ...
      for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
        {
          if (bgp_show_summary_is_peer_filtered (peer, fpeer, as_type, as))
            {
              filtered_count++;
              count++;
              continue;
          ...
          if (peer->afc[afi][safi])
            {
              ...
              if (peer->hostname &&
                ...
                snprintf (neighbor_buf, sizeof (neighbor_buf), "%s%s(%s) ",
                          dn_flag, peer->hostname, peer->host);
              else
                snprintf (neighbor_buf, sizeof (neighbor_buf), "%s%s ",
                          dn_flag, peer->host);
              ...
              if ((ents = listcount (bgp->group)))
                {
                  ...
                  json_object_int_add (json, "peerGroupMemory",
              ...
              if (show_failed)
                {
                  ...
                  vty_out (vty, BGP_SHOW_SUMMARY_HEADER_FAILED);
                }
            }
        }
      ...
      if (use_json)
        {
          ...
          if (show_failed && bgp_has_peer_failed (peer, afi, safi))
            {
              json_peer = json_object_new_object ();
              bgp_show_failed_summary (vty, bgp, peer, json_peer, 0, use_json);
            }
          else if (! show_failed)
            {
              if (show_established && bgp_has_peer_failed (peer, afi, safi))
                {
                  filtered_count++;
                  continue;
                }
              ...
              asn_asn2json (json_peer, "remoteAs", peer->as, bgp->asnotation);
              asn_asn2json (json_peer, "localAs",
                            peer->change_local_as ? peer->change_local_as
                                                  : peer->local_as,
                            bgp->asnotation);
              json_object_int_add (json_peer, "version", 4);
              json_object_int_add (json_peer, "msgRcvd", PEER_TOTAL_RX (peer));
              json_object_int_add (json_peer, "msgSent", PEER_TOTAL_TX (peer));

              atomic_size_t outq_count, inq_count;
              outq_count = atomic_load_explicit (&peer->obuf->count,
              ...
              json_object_int_add (
                  ...
                  (paf && PAF_SUBGRP (paf)) ? paf->subgroup->version : 0);
              json_object_int_add (json_peer, "outq", outq_count);
              json_object_int_add (json_peer, "inq", inq_count);
              peer_uptime (peer->uptime, timebuf, BGP_UPTIME_LEN, use_json,
                           json_peer);
              ...
              if (paf && PAF_SUBGRP (paf))
                ...
                json_object_int_add (json_peer, "pfxSnt", 0);
              ...
              if (CHECK_FLAG (peer->flags, PEER_FLAG_SHUTDOWN) ||
                  CHECK_FLAG (peer->bgp->flags, BGP_FLAG_SHUTDOWN))
                json_object_string_add (json_peer, "peerState", "Admin");
              else if (CHECK_FLAG (peer->sflags, PEER_STATUS_PREFIX_OVERFLOW))
                json_object_string_add (json_peer, "peerState", "PfxCt");
              else if (CHECK_FLAG (peer->flags, PEER_FLAG_PASSIVE))
                json_object_string_add (json_peer, "peerState", "Passive");
              else if (CHECK_FLAG (peer->sflags, PEER_STATUS_NSF_WAIT))
                json_object_string_add (json_peer, "peerState", "NSF passive");
              else if (CHECK_FLAG (peer->bgp->flags,
                                   BGP_FLAG_EBGP_REQUIRES_POLICY) &&
                       (! bgp_inbound_policy_exists (peer, filter) ||
                        ! bgp_outbound_policy_exists (peer, filter)))
                json_object_string_add (json_peer, "peerState", "Policy");
              else
                json_object_string_add (json_peer, "peerState", "OK");
              ...
              json_object_int_add (json_peer, "connectionsDropped",
                                   peer->dropped);
              if (peer->desc)
                json_object_string_add (json_peer, "desc", peer->desc);
            }
      ...
      else
        {
          ...
          if (show_failed && bgp_has_peer_failed (peer, afi, safi))
          ...
          else if (! show_failed)
            {
              ...
              if ((count - filtered_count) == 1)
                {
                  ...
                  vty_out (vty, "Neighbor");
                  vty_out (vty, "%*s", max_neighbor_width - 8, " ");
                  vty_out (vty, show_wide ? BGP_SHOW_SUMMARY_HEADER_ALL_WIDE
              ...
              vty_out (vty, "4 ");
              vty_out (vty, ASN_FORMAT_SPACE (bgp->asnotation), &peer->as);
              if (show_wide)
                vty_out (vty, ASN_FORMAT_SPACE (bgp->asnotation),
                         peer->change_local_as ? &peer->change_local_as
                                               : &peer->local_as);
              vty_out (vty, " %9u %9u %8" PRIu64 " %4zu %4zu %8s",
                       PEER_TOTAL_RX (peer), PEER_TOTAL_TX (peer),
                       (paf && PAF_SUBGRP (paf)) ? paf->subgroup->version : 0,
                       inq_count, outq_count,
                       peer_uptime (peer->uptime, timebuf, BGP_UPTIME_LEN, 0,
                                    NULL));
                  ...
                  if (paf && PAF_SUBGRP (paf))
                    {
                      if (CHECK_FLAG (bgp->flags,
                                      BGP_FLAG_EBGP_REQUIRES_POLICY) &&
                          ! bgp_outbound_policy_exists (peer, filter))
                        vty_out (vty, " %8s", "(Policy)");
                      else
                        vty_out (vty, " %8u", (PAF_SUBGRP (paf))->scount);
                    }
                  else
                    {
                      vty_out (vty, "    NoNeg");
              ...
              if (peer->desc)
                vty_out (vty, " %s",
                         bgp_peer_description_stripped (peer->desc,
                                                        show_wide ? 64 : 20));
              else
                vty_out (vty, " N/A");
              vty_out (vty, "\n");
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json, "displayedPeers", count - filtered_count);
      json_object_int_add (json, "totalPeers", count);
      json_object_int_add (json, "dynamicPeers", dn_count);
      ...
      if (count)
        {
          if (filtered_count == count)
            ...
            {
              if (show_failed)
                vty_out (vty, "\nDisplayed neighbors %d", failed_count);
          ...
          if (bgp_afi_safi_peer_exists (bgp, afi, safi))
            {
                  ...
                  if (use_json)
                    {
                      ...
                      vty_out (vty,
                               "\"%s\":", get_afi_safi_str (afi, safi, true));
                    }
                  else
                    {
                      vty_out (vty, "\n%s Summary (%s):\n",
              ...
              bgp_show_summary (vty, bgp, afi, safi, fpeer, as_type, as,
                                show_flags);
            }
          safi++;
          if (! safi_wildcard)
...
bgp_show_all_instances_summary_vty (struct vty *vty, afi_t afi, safi_t safi,
                                    const char *neighbor, int as_type, as_t as,
                                    uint16_t show_flags)
{
  struct listnode *node, *nnode;
  struct bgp *bgp;
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
      ...
      if (neighbor)
        {
          fpeer = peer_lookup_in_view (vty, bgp, neighbor, use_json);
          if (! fpeer)
            continue;
        }
      bgp_show_summary_afi_safi (vty, bgp, afi, safi, fpeer, as_type, as,
                                 show_flags);
    }
...
int
bgp_show_summary_vty (struct vty *vty, const char *name, afi_t afi,
                      safi_t safi, const char *neighbor, int as_type, as_t as,
                      uint16_t show_flags)
{
  struct bgp *bgp;
  bool use_json = CHECK_FLAG (show_flags, BGP_SHOW_OPT_JSON);
  ...
  if (name)
    {
      if (strmatch (name, "all"))
        {
          bgp_show_all_instances_summary_vty (vty, afi, safi, neighbor,
  ...
  if (argv_find (argv, argc, "remote-as", &idx))
    {
      if (argv[idx + 1]->arg[0] == 'i')
        ...
        {
          vty_out (vty, "%% Invalid neighbor remote-as value: %s\n",
                   argv[idx + 1]->arg);
          return CMD_SUCCESS;
        }
    }
...
const char *
get_afi_safi_str (afi_t afi, safi_t safi, bool for_json)
{
  if (for_json)
...
static void
bgp_show_peer_afi_orf_cap (struct vty *vty, struct peer *p, afi_t afi,
                           safi_t safi, uint16_t adv_smcap, uint16_t adv_rmcap,
...
static void
bgp_show_neighnor_graceful_restart_flags (struct vty *vty, struct peer *p,
                                          json_object *json)
{
  bool rbit = false;
  bool nbit = false;
  ...
  if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_ADV) &&
      (CHECK_FLAG (p->cap, PEER_CAP_RESTART_RCV)) && (peer_established (p)))
    {
...
static void
bgp_show_neighbor_graceful_restart_remote_mode (struct vty *vty,
                                                struct peer *peer,
                                                json_object *json)
{
...
static void
bgp_show_neighbor_graceful_restart_local_mode (struct vty *vty, struct peer *p,
                                               json_object *json)
{
...
static void
bgp_show_neighbor_graceful_restart_capability_per_afi_safi (struct vty *vty,
                                                            struct peer *peer,
                                                            json_object *json)
{
  afi_t afi;
  ...
  FOREACH_AFI_SAFI_NSF (afi, safi)
    {
      ...
      if (peer->nsf[afi][safi] &&
                      ...
                      PEER_CAP_RESTART_AF_PRESERVE_RCV))
        {
          ...
          if (json)
            {
              json_object_boolean_true_add (json_afi_safi, "fBit");
            }
          else
            vty_out (vty, "True\n");
        }
        ...
        {
          if (json)
            json_object_boolean_false_add (json_afi_safi, "fBit");
          else
            vty_out (vty, "False\n");
        }
      ...
      if (CHECK_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_EOR_SEND))
        {
          if (json)
            {
            ...
            {
              vty_out (vty, "Yes\n");
              vty_out (vty, "      End-of-RIB sent after update: ");
      ...
      if (json)
        {
          ...
          if (peer->t_gr_stale != NULL)
            {
              json_object_int_add (
                  json_timer, "stalePathTimerRemaining",
                  event_timer_remain_second (peer->t_gr_stale));
            }
          ...
          if (peer->bgp->gr_info[afi][safi].t_select_deferral != NULL)
            {

              json_object_int_add (
                  json_timer, "selectionDeferralTimerRemaining",
      ...
      else
        {
          ...
          if (peer->t_gr_stale != NULL)
            vty_out (vty, "      Stale Path Remaining(sec): %ld\n",
                     event_timer_remain_second (peer->t_gr_stale));
          /* Display Configured Selection
           * Deferral only when when
           * Gr mode is enabled.
      ...
      if (json)
        {
          ...
          json_object_object_add (json_afi_safi, "timers", json_timer);
...
static void
bgp_show_neighbor_graceful_restart_time (struct vty *vty, struct peer *p,
                                         json_object *json)
{
  if (json)
    {
...
static void
bgp_show_peer_gr_status (struct vty *vty, struct peer *p, json_object *json)
{
  char dn_flag[2] = { 0 };
  ...
  if (p->conf_if)
    {
      if (json)
        json_object_string_addf (json, "neighborAddr", "%pSU", &p->su);
      else
  ...
  else
    {
      ...
      if (json)
        json_object_string_add (json, "neighborAddr", neighborAddr);
      else
  ...
  if (use_json)
    {
      ...
      if (CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_ADV) ||
          ...
          CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_RCV))
        {
          ...
          bgp_show_peer_afi_orf_cap (
              ...
              PEER_CAP_ORF_PREFIX_RM_ADV, PEER_CAP_ORF_PREFIX_SM_RCV,
      ...
      if (CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_ADV) ||
          ...
          CHECK_FLAG (p->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_OLD_RCV))
        {
          ...
          bgp_show_peer_afi_orf_cap (
              vty, p, afi, safi, PEER_CAP_ORF_PREFIX_SM_ADV,
              PEER_CAP_ORF_PREFIX_RM_ADV, PEER_CAP_ORF_PREFIX_SM_OLD_RCV,
      ...
      snprintf (orf_pfx_name, sizeof (orf_pfx_name), "%s.%d.%d", p->host, afi,
                safi);
      orf_pfx_count =
          prefix_bgp_show_prefix_list (NULL, afi, orf_pfx_name, use_json);
      ...
      if (CHECK_FLAG (p->af_sflags[afi][safi], PEER_STATUS_ORF_WAIT_REFRESH))
        json_object_string_add (json_addr, "orfFirstUpdate",
      ...
      if (CHECK_FLAG (p->af_flags[afi][safi],
        ...
        json_object_boolean_true_add (
            json_addr, "privateAsNumsAllReplacedInUpdatesToNbr");
      ...
      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_ALLOWAS_IN))
        {
          if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_ALLOWAS_IN_ORIGIN))
            json_object_boolean_true_add (json_addr, "allowAsInOrigin");
          else
      ...
      if (p->addpath_type[afi][safi] != BGP_ADDPATH_NONE)
        json_object_boolean_true_add (
            json_addr,
            bgp_addpath_names (p->addpath_type[afi][safi])->type_json_name);

      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_AS_OVERRIDE))
        json_object_string_add (json_addr, "overrideASNsInOutboundUpdates",
      ...
      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_DEFAULT_ORIGINATE))
        {
          if (p->default_rmap[afi][safi].name)
            json_object_string_add (json_addr, "defaultRouteMap",
      ...
      if (afi == AFI_L2VPN && safi == SAFI_EVPN)
        {
          if (is_evpn_enabled ())
            json_object_boolean_true_add (json_addr, "advertiseAllVnis");
        }

      if (filter->plist[FILTER_IN].name || filter->dlist[FILTER_IN].name ||
          filter->aslist[FILTER_IN].name || filter->map[RMAP_IN].name)
        json_object_boolean_true_add (json_addr, "inboundPathPolicyConfig");
      if (filter->plist[FILTER_OUT].name || filter->dlist[FILTER_OUT].name ||
          filter->aslist[FILTER_OUT].name || filter->map[RMAP_OUT].name ||
          filter->usmap.name)
        json_object_boolean_true_add (json_addr, "outboundPathPolicyConfig");

      /* prefix-list */
      if (filter->plist[FILTER_IN].name)
        json_object_string_add (json_addr, "incomingUpdatePrefixFilterList",
                                filter->plist[FILTER_IN].name);
      if (filter->plist[FILTER_OUT].name)
        json_object_string_add (json_addr, "outgoingUpdatePrefixFilterList",
                                filter->plist[FILTER_OUT].name);

      /* distribute-list */
      if (filter->dlist[FILTER_IN].name)
        json_object_string_add (json_addr, "incomingUpdateNetworkFilterList",
                                filter->dlist[FILTER_IN].name);
      if (filter->dlist[FILTER_OUT].name)
        json_object_string_add (json_addr, "outgoingUpdateNetworkFilterList",
                                filter->dlist[FILTER_OUT].name);

      /* filter-list. */
      if (filter->aslist[FILTER_IN].name)
        json_object_string_add (json_addr, "incomingUpdateAsPathFilterList",
                                filter->aslist[FILTER_IN].name);
      if (filter->aslist[FILTER_OUT].name)
        json_object_string_add (json_addr, "outgoingUpdateAsPathFilterList",
                                filter->aslist[FILTER_OUT].name);

      /* route-map. */
      if (filter->map[RMAP_IN].name)
        json_object_string_add (json_addr, "routeMapForIncomingAdvertisements",
                                filter->map[RMAP_IN].name);
      if (filter->map[RMAP_OUT].name)
        json_object_string_add (json_addr, "routeMapForOutgoingAdvertisements",
      ...
      if (filter->usmap.name)
        json_object_string_add (json_addr, "selectiveUnsuppressRouteMap",
                                filter->usmap.name);
      ...
      if (filter->advmap.aname)
        {
          ...
          json_object_string_add (json_advmap, "advertiseStatus",
                                  filter->advmap.update_type ==
                                      ...
                                      ? "Advertise"
                                      : "Withdraw");
          json_object_object_add (json_addr, "advertiseMap", json_advmap);
        }
      ...
      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX))
        {
          ...
          json_object_int_add (json_addr, "prefixAllowedWarningThresh",
  ...
  else
    {
      ...
      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_ALLOWAS_IN))
        {
          if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_ALLOWAS_IN_ORIGIN))
            vty_out (vty, "  Local AS allowed as path origin\n");
          else
      ...
      if (p->addpath_type[afi][safi] != BGP_ADDPATH_NONE)
        vty_out (
            vty, "  %s\n",
            bgp_addpath_names (p->addpath_type[afi][safi])->human_description);
      ...
      if (CHECK_FLAG (p->bgp->flags, BGP_FLAG_EBGP_REQUIRES_POLICY) &&
          ! bgp_inbound_policy_exists (p, filter))
        vty_out (vty, "  Inbound updates discarded due to missing policy\n");

      /* ebgp-requires-policy (outbound) */
      if (CHECK_FLAG (p->bgp->flags, BGP_FLAG_EBGP_REQUIRES_POLICY) &&
          ! bgp_outbound_policy_exists (p, filter))
        vty_out (vty, "  Outbound updates discarded due to missing policy\n");

      /* unsuppress-map */
      if (filter->usmap.name)
        vty_out (vty, "  Route map for selective unsuppress is %s%s\n",
                 filter->usmap.map ? "*" : "", filter->usmap.name);
      ...
      if (filter->advmap.aname && filter->advmap.cname)
        vty_out (
            ...
            filter->advmap.condition ? "EXIST" : "NON_EXIST",
            filter->advmap.cmap ? "*" : "", filter->advmap.cname,
            filter->advmap.amap ? "*" : "", filter->advmap.aname,
            filter->advmap.update_type == UPDATE_TYPE_ADVERTISE ? "Advertise"
                                                                : "Withdraw");
      ...
      /* maximum-prefix-out */
      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_OUT))
        vty_out (vty, "  Maximum allowed prefixes sent %u\n",
                 p->pmax_out[afi][safi]);
      ...
      if (CHECK_FLAG (p->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX))
        {
          vty_out (
              vty, "  Maximum prefixes allowed %u%s\n", p->pmax[afi][safi],
...
static void
bgp_show_peer (struct vty *vty, struct peer *p, bool use_json,
               json_object *json)
{
  struct bgp *bgp;
  ...
  if (! use_json)
    {
      if (p->conf_if) /* Configured interface name. */
        vty_out (vty, "BGP neighbor on %s: %pSU, ", p->conf_if, &p->su);
  ...
  if (use_json)
    {
      if (p->conf_if && BGP_PEER_SU_UNSPEC (p))
        ...
        json_object_string_addf (json_neigh, "bgpNeighborAddr", "%pSU",
                                 &p->su);
  ...
  else
    {
      ...
      vty_out (
          vty, "%s%s, ",
          CHECK_FLAG (p->flags, PEER_FLAG_LOCAL_AS_NO_PREPEND) ? " no-prepend"
                                                               : "",
  ...
  if ((p->as == p->local_as) || (p->as_type == AS_INTERNAL))
    ...
    {
      if (use_json)
        json_object_boolean_true_add (json_neigh, "nbrUnspecifiedLink");
      else
          ...
          if (dn_flag[0])
            {
              ...
              if (range)
                {
                  json_object_string_addf (json_neigh, "peerSubnetRangeGroup",
                                           "%pFX", range);
                }
            }
        }
          ...
          if (dn_flag[0])
            {
              ...
              if (range)
                {
                  vty_out (vty, " Belongs to the subnet range group: %pFX\n",
                           range);
                }
            }
        }
  ...
  if (use_json)
    {
      ...
      if (peer_established (p))
        {
          ...
          json_object_string_add (
              json_neigh, "bgpTimerUpString",
              peer_uptime (p->uptime, timebuf, BGP_UPTIME_LEN, 0, NULL));
          json_object_int_add (json_neigh, "bgpTimerUpEstablishedEpoch",
                               epoch_tbuf);
        }
      ...
      json_object_int_add (json_neigh, "bgpTimerConfiguredHoldTimeMsecs",
      ...
      json_object_int_add (json_neigh, "bgpTimerHoldTimeMsecs",
                           p->v_holdtime * 1000);
      json_object_int_add (json_neigh, "bgpTimerKeepAliveIntervalMsecs",
                           p->v_keepalive * 1000);
      if (CHECK_FLAG (p->flags, PEER_FLAG_TIMER_DELAYOPEN))
        {
          json_object_int_add (json_neigh, "bgpTimerDelayOpenTimeMsecs",
                               p->v_delayopen * 1000);
        }
      ...
      if (CHECK_FLAG (p->flags, PEER_FLAG_TCP_MSS))
        {
          ...
          json_object_int_add (json_neigh, "bgpTcpMssConfigured", p->tcp_mss);
      ...
      if (BGP_OPEN_EXT_OPT_PARAMS_CAPABLE (p))
        ...
        json_object_boolean_false_add (json_neigh,
                                       "extendedOptionalParametersLength");
      ...
      if (event_is_scheduled (bgp->t_condition_check))
        json_object_int_add (
            json_neigh, "bgpTimerUntilConditionalAdvertisementsSec",
            event_timer_remain_second (bgp->t_condition_check));
    }
  else
    {
      /* Administrative shutdown. */
      ...
      vty_out (vty, "  Last read %s",
               peer_uptime (p->readtime, timebuf, BGP_UPTIME_LEN, 0, NULL));
      ...
      vty_out (vty, "  Configured hold time is %d seconds",
               CHECK_FLAG (p->flags, PEER_FLAG_TIMER) ? p->holdtime
      ...
      if (CHECK_FLAG (p->flags, PEER_FLAG_TIMER_DELAYOPEN))
        vty_out (vty, "  Configured DelayOpenTime is %d seconds\n",
                 p->delayopen);
      ...
      /* Extended Optional Parameters Length for BGP OPEN Message */
      if (BGP_OPEN_EXT_OPT_PARAMS_CAPABLE (p))
        vty_out (vty, "  Extended Optional Parameters Length is enabled\n");
      ...
      if (use_json)
        {
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_AS4_RCV) ||
              CHECK_FLAG (p->cap, PEER_CAP_AS4_ADV))
            {
              if (CHECK_FLAG (p->cap, PEER_CAP_AS4_ADV) &&
                ...
                json_object_string_add (json_cap, "4byteAs",
                                        "advertisedAndReceived");
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_EXTENDED_MESSAGE_ADV) &&
            ...
            json_object_string_add (json_cap, "extendedMessage",
                                    "advertisedAndReceived");
          else if (CHECK_FLAG (p->cap, PEER_CAP_EXTENDED_MESSAGE_ADV))
            json_object_string_add (json_cap, "extendedMessage", "advertised");
          else if (CHECK_FLAG (p->cap, PEER_CAP_EXTENDED_MESSAGE_RCV))
            json_object_string_add (json_cap, "extendedMessage", "received");
              ...
              FOREACH_AFI_SAFI (afi, safi)
                {
                  ...
                  print_store = get_afi_safi_str (afi, safi, true);
                  ...
                  if (CHECK_FLAG (p->af_cap[afi][safi],
                                  ...
                                  PEER_CAP_ADDPATH_AF_TX_RCV))
                    {
                      ...
                      else if (CHECK_FLAG (p->af_cap[afi][safi],
                                           PEER_CAP_ADDPATH_AF_TX_ADV))
                        json_object_boolean_true_add (json_sub,
                                                      "txAdvertised");
                      else if (CHECK_FLAG (p->af_cap[afi][safi],
                                           PEER_CAP_ADDPATH_AF_TX_RCV))
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_DYNAMIC_RCV) ||
              CHECK_FLAG (p->cap, PEER_CAP_DYNAMIC_ADV))
            {
              if (CHECK_FLAG (p->cap, PEER_CAP_DYNAMIC_ADV) &&
                ...
                json_object_string_add (json_cap, "dynamic",
                                        "advertisedAndReceived");
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_ROLE_RCV) ||
              CHECK_FLAG (p->cap, PEER_CAP_ROLE_ADV))
            {
              if (CHECK_FLAG (p->cap, PEER_CAP_ROLE_ADV) &&
                ...
                json_object_string_add (json_cap, "role", "advertised");
              else if (CHECK_FLAG (p->cap, PEER_CAP_ROLE_RCV))
                json_object_string_add (json_cap, "role", "received");
            }
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_ENHE_RCV) ||
              CHECK_FLAG (p->cap, PEER_CAP_ENHE_ADV))
            {
              ...
              if (CHECK_FLAG (p->cap, PEER_CAP_ENHE_ADV) &&
                ...
                json_object_string_add (json_cap, "extendedNexthop",
                                        "advertisedAndReceived");
              ...
              if (CHECK_FLAG (p->cap, PEER_CAP_ENHE_RCV))
                {
                  ...
                  for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
                    {
                      if (CHECK_FLAG (p->af_cap[AFI_IP][safi],
                                      PEER_CAP_ENHE_AF_RCV))
                        {
                          print_store = get_afi_safi_str (AFI_IP, safi, true);
                          json_object_string_add (
                              json_nxt, print_store,
                              "recieved"); /* misspelled for compatibility */
                        }
                    }
                  json_object_object_add (
                      json_cap, "extendedNexthopFamililesByPeer", json_nxt);
                }
            }
                  ...
                  FOREACH_AFI_SAFI (afi, safi)
                    {
                      if (CHECK_FLAG (p->af_cap[afi][safi],
                                      PEER_CAP_ENHE_AF_RCV))
                        {
                          afi_safi_str = get_afi_safi_str (afi, safi, true);
                          json_object_string_add (json_llgr, afi_safi_str,
                                                  "received");
                        }
                    }
                  json_object_object_add (
                      json_cap, "longLivedGracefulRestartByPeer", json_llgr);
                }
            }
              ...
              if (CHECK_FLAG (p->cap, PEER_CAP_REFRESH_ADV) &&
                   ...
                   CHECK_FLAG (p->cap, PEER_CAP_REFRESH_OLD_RCV)))
                {
                  if (CHECK_FLAG (p->cap, PEER_CAP_REFRESH_OLD_RCV) &&
                      CHECK_FLAG (p->cap, PEER_CAP_REFRESH_NEW_RCV))
                    json_object_string_add (json_cap, "routeRefresh",
                ...
                json_object_string_add (json_cap, "routeRefresh",
                                        "advertised");
              else if (CHECK_FLAG (p->cap, PEER_CAP_REFRESH_NEW_RCV) ||
                       CHECK_FLAG (p->cap, PEER_CAP_REFRESH_OLD_RCV))
              ...
              CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_RCV))
            {
              if (CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_ADV) &&
                ...
                json_object_string_add (json_cap, "enhancedRouteRefresh",
                                        "advertisedAndReceived");
              else if (CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_ADV))
                json_object_string_add (json_cap, "enhancedRouteRefresh",
                                        "advertised");
              else if (CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_RCV))
                json_object_string_add (json_cap, "enhancedRouteRefresh",
                                        "received");
            }
              ...
              if (p->afc_adv[afi][safi] || p->afc_recv[afi][safi])
                {
                  ...
                  json_object_object_add (json_multi,
                                          get_afi_safi_str (afi, safi, true),
                                          json_exten);
                }
            }
          json_object_object_add (json_cap, "multiprotocolExtensions",
                                  json_multi);
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_HOSTNAME_ADV))
            {
              json_object_string_add (
                  ...
                  bgp->peer_self->hostname ? bgp->peer_self->hostname : "n/a");
              json_object_string_add (json_hname, "advDomainName",
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_HOSTNAME_RCV))
            {
              json_object_string_add (json_hname, "rcvHostName",
                                      p->hostname ? p->hostname : "n/a");
              json_object_string_add (json_hname, "rcvDomainName",
                                      p->domainname ? p->domainname : "n/a");
            }

          json_object_object_add (json_cap, "hostName", json_hname);
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_SOFT_VERSION_ADV))
            json_object_string_add (json_soft_version,
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_SOFT_VERSION_RCV))
            json_object_string_add (json_soft_version,
                                    ...
                                    p->soft_version ? p->soft_version : "n/a");
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_RCV) ||
              CHECK_FLAG (p->cap, PEER_CAP_RESTART_ADV))
            {
              if (CHECK_FLAG (p->cap, PEER_CAP_RESTART_ADV) &&
                ...
                json_object_string_add (json_cap, "gracefulRestartCapability",
                                        "advertised");
                  ...
                  FOREACH_AFI_SAFI (afi, safi)
                    {
                      if (CHECK_FLAG (p->af_cap[afi][safi],
                                      PEER_CAP_RESTART_AF_RCV))
                        {
                          ...
                          restart_af_count++;
                          json_object_object_add (
                              json_restart, get_afi_safi_str (afi, safi, true),
                              json_sub);
                        }
                    }
                  if (! restart_af_count)
      ...
      else
        {
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_ROLE_RCV) ||
              CHECK_FLAG (p->cap, PEER_CAP_ROLE_ADV))
            {
              ...
              if (CHECK_FLAG (p->cap, PEER_CAP_ROLE_RCV))
                vty_out (vty, " %sreceived",
                         CHECK_FLAG (p->cap, PEER_CAP_ROLE_ADV) ? "and " : "");
              vty_out (vty, "\n");
            }
              ...
              if (CHECK_FLAG (p->cap, PEER_CAP_ENHE_RCV))
                {
                  ...
                  for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
                    if (CHECK_FLAG (p->af_cap[AFI_IP][safi],
                                    PEER_CAP_ENHE_AF_RCV))
                      vty_out (vty, "           %s\n",
                               get_afi_safi_str (AFI_IP, safi, false));
                }
            }
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_LLGR_RCV) ||
              CHECK_FLAG (p->cap, PEER_CAP_LLGR_ADV))
            {
              vty_out (vty, "    Long-lived Graceful Restart:");
              if (CHECK_FLAG (p->cap, PEER_CAP_LLGR_ADV))
                vty_out (vty, " advertised");
              if (CHECK_FLAG (p->cap, PEER_CAP_LLGR_RCV))
                vty_out (vty, " %sreceived",
                         CHECK_FLAG (p->cap, PEER_CAP_LLGR_ADV) ? "and " : "");
              vty_out (vty, "\n");
          ...
          if (CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_ADV) ||
              CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_RCV))
            {
              ...
              if (CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_ADV))
                vty_out (vty, " advertised");
              if (CHECK_FLAG (p->cap, PEER_CAP_ENHANCED_RR_RCV))
                vty_out (vty, " %sreceived",
                         CHECK_FLAG (p->cap, PEER_CAP_REFRESH_ADV) ? "and "
                                                                   : "");
              vty_out (vty, "\n");
  ...
  if (use_json)
    {
          ...
          FOREACH_AFI_SAFI (afi, safi)
            {
              if (CHECK_FLAG (p->af_sflags[afi][safi], PEER_STATUS_EOR_SEND))
                {
                  json_object_boolean_true_add (
                      json_grace_send, get_afi_safi_str (afi, safi, true));
                  eor_send_af_count++;
                }
            }
          FOREACH_AFI_SAFI (afi, safi)
            {
              if (CHECK_FLAG (p->af_sflags[afi][safi],
                ...
                {
                  json_object_boolean_true_add (
                      json_grace_recv, get_afi_safi_str (afi, safi, true));
      ...
      if (p->t_gr_restart)
        json_object_int_add (json_grace, "gracefulRestartTimerMsecs",
                             event_timer_remain_second (p->t_gr_restart) *
                                 1000);

      if (p->t_gr_stale)
        json_object_int_add (json_grace, "gracefulStalepathTimerMsecs",
                             event_timer_remain_second (p->t_gr_stale) * 1000);
      /* more gr info in new format */
  ...
  if (use_json)
    {
      ...
      atomic_size_t outq_count, inq_count;
      outq_count =
          atomic_load_explicit (&p->obuf->count, memory_order_relaxed);
      inq_count = atomic_load_explicit (&p->ibuf->count, memory_order_relaxed);

      json_object_int_add (json_stat, "depthInq", (unsigned long) inq_count);
      json_object_int_add (json_stat, "depthOutq", (unsigned long) outq_count);
      json_object_int_add (
          json_stat, "opensSent",
          atomic_load_explicit (&p->open_out, memory_order_relaxed));
      json_object_int_add (
          json_stat, "opensRecv",
          atomic_load_explicit (&p->open_in, memory_order_relaxed));
      json_object_int_add (
          json_stat, "notificationsSent",
          atomic_load_explicit (&p->notify_out, memory_order_relaxed));
      json_object_int_add (
          json_stat, "notificationsRecv",
          atomic_load_explicit (&p->notify_in, memory_order_relaxed));
      json_object_int_add (
          json_stat, "updatesSent",
          atomic_load_explicit (&p->update_out, memory_order_relaxed));
      json_object_int_add (
          json_stat, "updatesRecv",
          atomic_load_explicit (&p->update_in, memory_order_relaxed));
      json_object_int_add (
          json_stat, "keepalivesSent",
          atomic_load_explicit (&p->keepalive_out, memory_order_relaxed));
      json_object_int_add (
          json_stat, "keepalivesRecv",
          atomic_load_explicit (&p->keepalive_in, memory_order_relaxed));
      json_object_int_add (
          json_stat, "routeRefreshSent",
          atomic_load_explicit (&p->refresh_out, memory_order_relaxed));
      json_object_int_add (
          json_stat, "routeRefreshRecv",
          atomic_load_explicit (&p->refresh_in, memory_order_relaxed));
      json_object_int_add (
          json_stat, "capabilitySent",
          atomic_load_explicit (&p->dynamic_cap_out, memory_order_relaxed));
      json_object_int_add (
          json_stat, "capabilityRecv",
          atomic_load_explicit (&p->dynamic_cap_in, memory_order_relaxed));
    ...
    {
      atomic_size_t outq_count, inq_count, open_out, open_in, notify_out,
          notify_in, update_out, update_in, keepalive_out, keepalive_in,
          refresh_out, refresh_in, dynamic_cap_out, dynamic_cap_in;
      outq_count =
          atomic_load_explicit (&p->obuf->count, memory_order_relaxed);
      inq_count = atomic_load_explicit (&p->ibuf->count, memory_order_relaxed);
      open_out = atomic_load_explicit (&p->open_out, memory_order_relaxed);
      open_in = atomic_load_explicit (&p->open_in, memory_order_relaxed);
      notify_out = atomic_load_explicit (&p->notify_out, memory_order_relaxed);
      notify_in = atomic_load_explicit (&p->notify_in, memory_order_relaxed);
      update_out = atomic_load_explicit (&p->update_out, memory_order_relaxed);
      update_in = atomic_load_explicit (&p->update_in, memory_order_relaxed);
      keepalive_out =
          atomic_load_explicit (&p->keepalive_out, memory_order_relaxed);
      keepalive_in =
          atomic_load_explicit (&p->keepalive_in, memory_order_relaxed);
      refresh_out =
          atomic_load_explicit (&p->refresh_out, memory_order_relaxed);
      refresh_in = atomic_load_explicit (&p->refresh_in, memory_order_relaxed);
      dynamic_cap_out =
          atomic_load_explicit (&p->dynamic_cap_out, memory_order_relaxed);
      dynamic_cap_in =
          atomic_load_explicit (&p->dynamic_cap_in, memory_order_relaxed);
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json_neigh, "minBtwnAdvertisementRunsTimerMsecs",
                           p->v_routeadv * 1000);
      ...
      if (p->update_if || p->update_source)
        {
          if (p->update_if)
            ...
            json_object_string_addf (json_neigh, "updateSource", "%pSU",
                                     p->update_source);
        }
    }
  else
          ...
          if (p->last_reset_cause)
            {
              ...
              for (i = 1; i <= p->last_reset_cause->size; i++)
                {
                  vty_out (vty, "%02X", *msg++);
          ...
          if (use_json)
            {
              ...
              json_object_int_add (
                  json_neigh, "restartInTimerMsec",
                  event_timer_remain_second (p->t_pmax_restart) * 1000);
            }
          else
            vty_out (
                vty,
                "  Reduce the no. of prefix from %s, will restart in %ld seconds\n",
                p->host, event_timer_remain_second (p->t_pmax_restart));
        }
      else
        {
          if (use_json)
  ...
  if (p->sort != BGP_PEER_IBGP)
      ...
      if (use_json)
        {
          if (p->gtsm_hops > BGP_GTSM_HOPS_DISABLED)
            json_object_int_add (json_neigh, "externalBgpNbrMaxHopsAway",
                                 p->gtsm_hops);
          else
            json_object_int_add (json_neigh, "externalBgpNbrMaxHopsAway",
                                 p->ttl);
  ...
  else
    {
      if (use_json)
        {
          if (p->gtsm_hops > BGP_GTSM_HOPS_DISABLED)
            json_object_int_add (json_neigh, "internalBgpNbrMaxHopsAway",
                                 p->gtsm_hops);
          else
            json_object_int_add (json_neigh, "internalBgpNbrMaxHopsAway",
                                 p->ttl);
  ...
  if (p->su_local)
  ...
  else
    {
      if (use_json)
        {
          json_object_string_add (json_neigh, "hostLocal", "Unknown");
  ...
  if (p->su_remote)
  ...
  else
    {
      if (use_json)
        {
          json_object_string_add (json_neigh, "hostForeign", "Unknown");
  ...
  if (p->su_local)
      ...
      if (use_json)
        {
          json_object_string_addf (json_neigh, "nexthop", "%pI4",
                                   &p->nexthop.v4);
          json_object_string_addf (json_neigh, "nexthopGlobal", "%pI6",
                                   &p->nexthop.v6_global);
          json_object_string_addf (json_neigh, "nexthopLocal", "%pI6",
                                   &p->nexthop.v6_local);
          if (p->shared_network)
  ...
  else
    {
      if (use_json)
        {
          json_object_string_add (json_neigh, "nexthop", "Unknown");
          json_object_string_add (json_neigh, "nexthopGlobal", "Unknown");
          json_object_string_add (json_neigh, "nexthopLocal", "Unknown");
          json_object_string_add (json_neigh, "bgpConnection", "Unknown");
        }
    }
      ...
      if (peer_established (p))
        {
          ...
          if (CHECK_FLAG (p->flags, PEER_FLAG_RTT_SHUTDOWN))
            {
              json_object_int_add (json_neigh, "shutdownRttInMsecs",
                                   p->rtt_expected);
              json_object_int_add (json_neigh, "shutdownRttAfterCount",
                                   p->rtt_keepalive_rcv);
            }
        }
      if (p->t_start)
        json_object_int_add (json_neigh, "nextStartTimerDueInMsecs",
                             event_timer_remain_second (p->t_start) * 1000);
      if (p->t_connect)
        json_object_int_add (json_neigh, "nextConnectTimerDueInMsecs",
  ...
  else
    {
      ...
      if (peer_established (p))
        {
          ...
          if (CHECK_FLAG (p->flags, PEER_FLAG_RTT_SHUTDOWN))
            vty_out (vty, "Shutdown when RTT > %dms, count > %u\n",
      ...
      if (p->t_routeadv)
        vty_out (vty, "MRAI (interval %u) timer expires in %ld seconds\n",
                 p->v_routeadv, event_timer_remain_second (p->t_routeadv));
      if (p->password)
        vty_out (vty, "Peer Authentication Enabled\n");

      vty_out (vty, "Read thread: %s  Write thread: %s  FD used: %d\n",
               p->t_read ? "on" : "off",
               CHECK_FLAG (p->thread_flags, PEER_THREAD_WRITES_ON) ? "on"
                                                                   : "off",
               p->fd);
    }
...
static int
bgp_show_neighbor_graceful_restart (struct vty *vty, struct bgp *bgp,
      ...
      if (type == show_all)
        {
          ...
          if (json)
            json_object_object_add (json, peer->host, json_neighbor);
        }
...
bgp_show_neighbor (struct vty *vty, struct bgp *bgp, enum show_type type,
                   union sockunion *su, const char *conf_if, bool use_json,
                   json_object *json)
{
  struct listnode *node, *nnode;
  struct peer *peer;
  ...
  if (type != show_peer && type != show_ipv4_peer && type != show_ipv6_peer &&
      ! nbr_output && ! use_json)
...
static void
bgp_show_neighbor_graceful_restart_vty (struct vty *vty, enum show_type type,
                                        const char *ip_str, afi_t afi,
                                        json_object *json)
{
  ...
  if (ip_str)
    {
      ret = str2sockunion (ip_str, &su);
      if (ret < 0)
        bgp_show_neighbor_graceful_restart (vty, bgp, type, NULL, ip_str, afi,
                                            json);
      else
        bgp_show_neighbor_graceful_restart (vty, bgp, type, &su, NULL, afi,
                                            json);
    }
  else
    bgp_show_neighbor_graceful_restart (vty, bgp, type, NULL, NULL, afi, json);
}

static void
bgp_show_all_instances_neighbors_vty (struct vty *vty, enum show_type type,
      ...
      if (use_json)
        {
          if (! (json = json_object_new_object ()))
            {
              flog_err (EC_BGP_JSON_MEM_ERROR,
...
static int
bgp_show_neighbor_vty (struct vty *vty, const char *name, enum show_type type,
                       const char *ip_str, bool use_json)
{
  int ret;
  struct bgp *bgp;
  ...
  if (name)
    {
      if (strmatch (name, "all"))
        {
          bgp_show_all_instances_neighbors_vty (vty, type, ip_str, use_json);
          return CMD_SUCCESS;
        }
      else
        {
       ...
       JSON_STR)
{
  ...
  return bgp_show_neighbor_graceful_restart_afi_all (vty, sh_type, sh_arg, afi,
                                                     uj);
}
...
static void
community_show_all_iterator (struct hash_bucket *bucket, struct vty *vty)
{
       ...
       "List all bgp community information\n")
{
  ...
  hash_iterate (
      community_hash (),
      (void (*) (struct hash_bucket *, void *)) community_show_all_iterator,
      vty);
...
static void
lcommunity_show_all_iterator (struct hash_bucket *bucket, struct vty *vty)
{
       ...
       "List all bgp large-community information\n")
{
  ...
  hash_iterate (
      lcommunity_hash (),
      (void (*) (struct hash_bucket *, void *)) lcommunity_show_all_iterator,
      vty);
...
static void
bgp_show_global_graceful_restart_mode_vty (struct vty *vty, struct bgp *bgp)
{
...
static int
bgp_show_neighbor_graceful_restart_afi_all (struct vty *vty,
                                            enum show_type type,
                                            const char *ip_str, afi_t afi,
                                            bool use_json)
{
  ...
  if ((afi == AFI_MAX) && (ip_str == NULL))
    ...
    {
      bgp_show_neighbor_graceful_restart_vty (vty, type, ip_str, afi, json);
    }
  else
    {
      if (json)
...
static int
bgp_show_route_leak_vty (struct vty *vty, const char *name, afi_t afi,
  ...
  if (json)
    {
      ...
      if (! CHECK_FLAG (bgp->af_flags[afi][safi],
      ...
      else
        {
          ...
          json_object_object_add (json, "importFromVrfs", json_import_vrfs);
          dir = BGP_VPN_POLICY_DIR_FROMVPN;
          if (bgp->vpn_policy[afi].rtlist[dir])
            {
              ecom_str = ecommunity_ecom2str (bgp->vpn_policy[afi].rtlist[dir],
      ...
      if (! CHECK_FLAG (bgp->af_flags[afi][safi],
                        BGP_CONFIG_VRF_TO_VRF_EXPORT))
        {
          ...
          json_object_string_add (json, "routeDistinguisher", "none");
      ...
      else
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (bgp->vpn_policy[afi].export_vrf, node,
            ...
            json_object_array_add (json_export_vrfs,
                                   json_object_new_string (vname));
          json_object_object_add (json, "exportToVrfs", json_export_vrfs);
          json_object_string_addf (json, "routeDistinguisher", "%s",
...
static int
bgp_show_all_instance_route_leak_vty (struct vty *vty, afi_t afi, safi_t safi,
                                      bool use_json)
{
  struct listnode *node, *nnode;
  struct bgp *bgp;
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
      ...
      if (use_json)
        {
          if (bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT)
            json_object_object_add (json_vrfs, VRF_DEFAULT_NAME, json_vrf);
          else
            json_object_object_add (json_vrfs, vrf_name, json_vrf);
        }
    }
...
static int
bgp_show_update_groups (struct vty *vty, const char *name, int afi, int safi,
...
static void
show_bgp_updgrps_adj_info_aux (struct vty *vty, const char *name, afi_t afi,
                               safi_t safi, const char *what,
                               uint64_t subgrp_id)
{
  struct bgp *bgp;
  ...
  if (ip && afi)
    {
      afiz = bgp_vty_afi_from_str (afi);
      if (afiz != AFI_IP)
        vty_out (vty, "%% Cannot specify both 'ip' and 'ipv6'\n");
      return CMD_WARNING;
    }
                         ...
                         json_object *json)
{
  ...
  if (conf->as_type == AS_SPECIFIED || conf->as_type == AS_EXTERNAL)
    {
      if (json)
        ...
        {
          vty_out (vty, "\nBGP peer-group %s, remote AS ", group->name);
  ...
  if ((group->bgp->as == conf->as) || (conf->as_type == AS_INTERNAL))
    {
      if (json)
        json_object_string_add (json_peer_group, "type", "internal");
      else
    ...
    {
      if (json)
        json_object_string_add (json_peer_group, "type", "external");
      else
      ...
      if (conf->afc[afi][safi])
        {
          ...
          if (json)
            json_object_array_add (
                json_peer_group_afc,
                json_object_new_string (get_afi_safi_str (afi, safi, false)));
          else
  ...
  if (json)
    {
      json_object_object_add (json_peer_group, "addressFamiliesConfigured",
                              json_peer_group_afc);
    }
  else
    {
      ...
      if (lr_count)
        {
          if (json)
            {
              if (! json_peer_group_dynamic)
                json_peer_group_dynamic = json_object_new_object ();

              json_peer_group_dynamic_af = json_object_new_object ();
              json_peer_group_ranges = json_object_new_array ();
              json_object_int_add (json_peer_group_dynamic_af, "count",
                                   lr_count);
            }
          else
            {
              ...
              if (json)
                {
                  ...
                  snprintfrr (buf, sizeof (buf), "%pFX", range);

                  json_object_array_add (json_peer_group_ranges,
          ...
          if (json)
            {
              json_object_object_add (json_peer_group_dynamic_af, "ranges",
                                      json_peer_group_ranges);

              json_object_object_add (json_peer_group_dynamic, afi2str (afi),
      ...
      for (ALL_LIST_ELEMENTS (group->peer, node, nnode, peer))
        {
          ...
          if (json)
            {
              json_object *json_peer_group_member = json_object_new_object ();

              json_object_string_add (json_peer_group_member, "status",
                                      peer_status);

              if (dynamic)
                json_object_boolean_true_add (json_peer_group_member,
                                              "dynamic");

              json_object_object_add (json_peer_group_members, peer->host,
       ...
       "Default metric\n")
{
  ...
  red = bgp_redist_add (bgp, AFI_IP, protocol, instance);
  changed = bgp_redistribute_metric_set (bgp, red, AFI_IP, protocol, metric);
  return bgp_redistribute_set (bgp, AFI_IP, protocol, instance, changed);
}
...
static int
peer_tcp_mss_vty (struct vty *vty, const char *peer_str,
       ...
       "TCP MSS value\n")
{
  ...
  return peer_tcp_mss_vty (vty, argv[peer_index]->arg, argv[mss_index]->arg);
}

DEFUN (no_neighbor_tcp_mss,
  ...
  if (check != ! no)
    {
      ...
      bgp_clear (vty, bgp, bgp_node_afi (vty), bgp_node_safi (vty), clear_all,
...
bool
peergroup_flag_check (struct peer *peer, uint64_t flag)
{
...
static bool
peergroup_af_flag_check (struct peer *peer, afi_t afi, safi_t safi,
                         uint64_t flag)
{
  if (! peer_group_active (peer))
    {
...
static bool
peergroup_filter_check (struct peer *peer, afi_t afi, safi_t safi,
                        uint8_t type, int direct)
{
  ...
  switch (type)
    ...
    case PEER_FT_ADVERTISE_MAP:
      return ! ! (
          filter->advmap.aname &&
          ((filter->advmap.condition == direct) && filter->advmap.cname));
    default:
      return false;
    }
}
 ...
 */
static bool
peergroup_af_addpath_check (struct peer *peer, afi_t afi, safi_t safi)
{
...
static void
bgp_vpn_policy_config_write_afi (struct vty *vty, struct bgp *bgp, afi_t afi)
{
  int indent = 2;
  ...
  if (CHECK_FLAG (bgp->vpn_policy[afi].flags,
                  BGP_VPN_POLICY_TOVPN_LABEL_PER_NEXTHOP))
    vty_out (vty, "%*slabel vpn export allocation-mode per-nexthop\n", indent,
             "");
  ...
  if (bgp->vpn_policy[afi].rtlist[BGP_VPN_POLICY_DIR_FROMVPN] &&
      ...
      ecommunity_cmp (bgp->vpn_policy[afi].rtlist[BGP_VPN_POLICY_DIR_FROMVPN],
                      bgp->vpn_policy[afi].rtlist[BGP_VPN_POLICY_DIR_TOVPN]))
    {
...
static void
bgp_config_write_filter (struct vty *vty, struct peer *peer, afi_t afi,
                         safi_t safi)
{
  struct bgp_filter *filter;
  char *addr;
                              ...
                              struct peer *peer)
{
  ...
  if (peer->sort != BGP_PEER_IBGP && peer->ttl != BGP_DEFAULT_TTL &&
      ! (peer->gtsm_hops != BGP_GTSM_HOPS_DISABLED && peer->ttl == MAXTTL))
    {
  ...
  bool discard_attrs = bgp_path_attribute_discard (peer, discard_attrs_str,
...
/* BGP peer configuration display function. */
static void
bgp_config_write_peer_af (struct vty *vty, struct bgp *bgp, struct peer *peer,
                          afi_t afi, safi_t safi)
{
  ...
  flag_scomm =
      peergroup_af_flag_check (peer, afi, safi, PEER_FLAG_SEND_COMMUNITY);
  flag_secomm =
  ...
  if (peergroup_af_flag_check (peer, afi, safi, PEER_FLAG_MAX_PREFIX))
    {
      ...
      if (peer_af_flag_check (peer, afi, safi, PEER_FLAG_MAX_PREFIX_WARNING))
        vty_out (vty, " warning-only");
      if (peer->pmax_restart[afi][safi])
        vty_out (vty, " restart %u", peer->pmax_restart[afi][safi]);
      if (peer_af_flag_check (peer, afi, safi, PEER_FLAG_MAX_PREFIX_FORCE))
        vty_out (vty, " force");
  ...
  if (peergroup_af_flag_check (peer, afi, safi, PEER_FLAG_SOO))
    {
      char *soo_str = ecommunity_ecom2str (peer->soo[afi][safi],
                                           ECOMMUNITY_FORMAT_ROUTE_MAP, 0);
      ...
      peer_af_flag_check (peer, afi, safi, PEER_FLAG_MED_UNCHANGED))
    {
      ...
      if (! peer_group_active (peer) ||
          ...
          peergroup_af_flag_check (peer, afi, safi, PEER_FLAG_MED_UNCHANGED))
        {
          ...
          vty_out (
              vty, "  neighbor %s attribute-unchanged%s%s%s\n", addr,
...
static void
bgp_vpn_config_write (struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi)
{
  if (! CHECK_FLAG (bgp->af_flags[afi][safi],
                    BGP_VPNVX_RETAIN_ROUTE_TARGET_ALL))
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
    {
      ...
      if (bgp->name)
        vty_out (vty, " %s %s",
                 (bgp->inst_type == BGP_INSTANCE_TYPE_VIEW) ? "view" : "vrf",
                 bgp->name);
      if (CHECK_FLAG (bgp->config, BGP_CONFIG_ASNOTATION))
        vty_out (vty, " as-notation %s", asn_mode2str (bgp->asnotation));
      ...
      if (! ! CHECK_FLAG (bgp->flags, BGP_FLAG_LOG_NEIGHBOR_CHANGES) !=
        ...
        vty_out (vty, " %sbgp log-neighbor-changes\n",
                 CHECK_FLAG (bgp->flags, BGP_FLAG_LOG_NEIGHBOR_CHANGES)
                     ? ""
                     : "no ");
      ...
      else if (! ! CHECK_FLAG (bgp->flags, BGP_FLAG_LU_IPV4_EXPLICIT_NULL))
      ...
      if (! ! CHECK_FLAG (bgp->flags, BGP_FLAG_SUPPRESS_DUPLICATES) !=
        ...
        vty_out (vty, " %sbgp suppress-duplicates\n",
                 CHECK_FLAG (bgp->flags, BGP_FLAG_SUPPRESS_DUPLICATES)
                     ? ""
                     : "no ");
      ...
      if (! ! CHECK_FLAG (bgp->flags, BGP_FLAG_HARD_ADMIN_RESET) !=
        ...
        vty_out (vty, " %sbgp hard-administrative-reset\n",
                 CHECK_FLAG (bgp->flags, BGP_FLAG_HARD_ADMIN_RESET) ? ""
                                                                    : "no ");
      ...
      if (bgp->confed_peers_cnt > 0)
        {
          ...
          for (i = 0; i < bgp->confed_peers_cnt; i++)
            vty_out (vty, " %s", bgp->confed_peers[i].as_pretty);
      ...
      if (! ! CHECK_FLAG (bgp->flags, BGP_FLAG_GRACEFUL_NOTIFICATION) !=
        ...
        vty_out (vty, " %sbgp graceful-restart notification\n",
                 CHECK_FLAG (bgp->flags, BGP_FLAG_GRACEFUL_NOTIFICATION)
                     ? ""
                     : "no ");
      ...
      if (! ! CHECK_FLAG (bgp->flags, BGP_FLAG_IMPORT_CHECK) !=
        ...
        vty_out (vty, " %sbgp network import-check\n",
                 CHECK_FLAG (bgp->flags, BGP_FLAG_IMPORT_CHECK) ? "" : "no ");
      ...
      bgp_config_write_family (vty, bgp, AFI_IP6, SAFI_LABELED_UNICAST);
...
static void
bgp_ac_peergroup (vector comps, struct cmd_token *token)
{
  struct bgp *bgp;
  struct peer_group *group;
...
static void
bgp_ac_peer (vector comps, struct cmd_token *token)
{
  struct bgp *bgp;
  struct peer *peer;
  struct listnode *lnbgp, *lnpeer;
...
bool
bgp_config_inprocess (void)
{
  return event_is_scheduled (t_bgp_cfg);
}

static void
bgp_config_finish (struct event *t)
{
  struct listnode *node;
  struct bgp *bgp;
...
#define BGP_POST_CONFIG_DELAY_SECONDS 1
   ...
   */
  event_add_timer (bm->master, bgp_config_finish, NULL, bgp_post_config_delay,
                   &t_bgp_cfg);
}

static int
config_write_interface_one (struct vty *vty, struct vrf *vrf)
{
  int write = 0;
       ...
       "Enable MPLS forwarding for eBGP directly connected peers\n")
{
  ...
  if (check != ! no)
    {
      if (no)
        UNSET_FLAG (iifp->flags, BGP_INTERFACE_MPLS_BGP_FORWARDING);
      else
        SET_FLAG (iifp->flags, BGP_INTERFACE_MPLS_BGP_FORWARDING);
...
bgp_vty_init (void)
{
  ...
  install_element (BGP_NODE, &bgp_neighbor_graceful_restart_disable_set_cmd);
  install_element (BGP_NODE,
                   &no_bgp_neighbor_graceful_restart_disable_set_cmd);

  /* "neighbor a:b:c:d graceful-restart-helper" command */
  install_element (BGP_NODE, &bgp_neighbor_graceful_restart_helper_set_cmd);
  ...
  install_element (BGP_NODE, &no_bgp_bestpath_peer_type_multipath_relax_cmd);
  ...
  install_element (BGP_FLOWSPECV4_NODE, &neighbor_soft_reconfiguration_cmd);
  install_element (BGP_FLOWSPECV4_NODE, &no_neighbor_soft_reconfiguration_cmd);
  install_element (BGP_FLOWSPECV6_NODE, &neighbor_soft_reconfiguration_cmd);
  install_element (BGP_FLOWSPECV6_NODE, &no_neighbor_soft_reconfiguration_cmd);
  ...
  install_element (BGP_VPNV6_NODE, &no_neighbor_nexthop_self_all_hidden_cmd);
  ...
  install_element (BGP_FLOWSPECV4_NODE, &neighbor_route_reflector_client_cmd);
  install_element (BGP_FLOWSPECV4_NODE,
                   &no_neighbor_route_reflector_client_cmd);
  install_element (BGP_FLOWSPECV6_NODE, &neighbor_route_reflector_client_cmd);
  install_element (BGP_FLOWSPECV6_NODE,
                   &no_neighbor_route_reflector_client_cmd);
  ...
  install_element (BGP_FLOWSPECV4_NODE, &no_neighbor_route_server_client_cmd);
  install_element (BGP_FLOWSPECV6_NODE, &neighbor_route_server_client_cmd);
  install_element (BGP_FLOWSPECV6_NODE, &no_neighbor_route_server_client_cmd);
  ...
  install_element (BGP_NODE, &neighbor_path_attribute_treat_as_withdraw_cmd);
  install_element (BGP_NODE,
                   &no_neighbor_path_attribute_treat_as_withdraw_cmd);
  ...
  install_element (BGP_NODE, &no_neighbor_disable_link_bw_encoding_ieee_cmd);

  /* "neighbor extended-optional-parameters" commands.  */
  install_element (BGP_NODE, &neighbor_extended_optional_parameters_cmd);
  install_element (BGP_NODE, &no_neighbor_extended_optional_parameters_cmd);
  ...
  install_element (BGP_IPV4_NODE, &af_label_vpn_export_allocation_mode_cmd);
  install_element (BGP_IPV6_NODE, &af_label_vpn_export_allocation_mode_cmd);
       ...
       COMMUNITY_VAL_STR)
{
  ...
  assert (str);
  int ret = community_list_set (bgp_clist, cl_name_or_number, str, seq, direct,
                                style);
       ...
       EXTCOMMUNITY_VAL_STR)
{
  ...
  int ret = extcommunity_list_unset (bgp_clist, cl_number_or_name, str, seq,
                                     direct, style);
      ...
      if (entry == list->head)
        {
          if (all_digit (list->name))
            ...
            vty_out (vty, "Named extended community %s list %s\n",
                     entry->style == EXTCOMMUNITY_LIST_STANDARD ? "standard"
                                                                : "expanded",
                     list->name);
        }
      vty_out (vty, "    %s %s\n", community_direct_str (entry->direct),
...
community_list_config_write (struct vty *vty)
{
  ...
  for (list = cm->num.head; list; list = list->next)
    for (entry = list->head; entry; entry = entry->next)
      {
        vty_out (vty, "bgp community-list %s seq %" PRId64 " %s %s\n",
  ...
  for (list = cm->num.head; list; list = list->next)
    for (entry = list->head; entry; entry = entry->next)
      {
        vty_out (vty, "bgp extcommunity-list %s seq %" PRId64 " %s %s\n",
                 list->name, entry->seq, community_direct_str (entry->direct),
                 community_list_config_str (entry));
        write++;
      }
  ...
  for (list = cm->num.head; list; list = list->next)
    for (entry = list->head; entry; entry = entry->next)
      {
        vty_out (vty, "bgp large-community-list %s seq %" PRId64 " %s %s\n",
                 list->name, entry->seq, community_direct_str (entry->direct),
                 community_list_config_str (entry));
        write++;
      }
    ...
    for (entry = list->head; entry; entry = entry->next)
      {
        vty_out (vty, "bgp large-community-list %s %s seq %" PRId64 " %s %s\n",
                 ...
                 entry->style == LARGE_COMMUNITY_LIST_STANDARD ? "standard"
                                                               : "expanded",
                 list->name, entry->seq, community_direct_str (entry->direct),
                 community_list_config_str (entry));
        write++;
...
community_list_vty (void)
{
  ...
  install_element (CONFIG_NODE, &no_bgp_lcommunity_list_name_standard_all_cmd);
  install_element (CONFIG_NODE, &no_bgp_lcommunity_list_name_expanded_all_cmd);
