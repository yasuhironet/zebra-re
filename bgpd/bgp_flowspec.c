...
static int
bgp_fs_nlri_validate (uint8_t *nlri_content, uint32_t len, afi_t afi)
{
  uint32_t offset = 0;
  int type;
  int ret = 0, error = 0;
...
int
bgp_nlri_parse_flowspec (struct peer *peer, struct attr *attr,
      ...
      if (BGP_DEBUG (flowspec, FLOWSPEC))
        {
          ...
          bgp_fs_nlri_get_string ((unsigned char *) p.u.prefix_flowspec.ptr,
                                  ...
                                  NLRI_STRING_FORMAT_MIN, NULL, afi);
          snprintf (ec_string, sizeof (ec_string), "EC{none}");
          if (attr && bgp_attr_get_ecommunity (attr))
            {
              s = ecommunity_ecom2str (bgp_attr_get_ecommunity (attr),
          ...
          snprintf (local_string, sizeof (local_string), "FS Rx %s %s %s %s",
