...
static void
sync_init (struct update_subgroup *subgrp, struct update_group *updgrp)
{
  ...
  subgrp->hash =
      hash_create (bgp_advertise_attr_hash_key, bgp_advertise_attr_hash_cmp,
                   "BGP SubGroup Hash");
   ...
   */
  subgrp->work =
      stream_new (peer->max_packet_size + BGP_MAX_PACKET_SIZE_OVERFLOW);
...
conf_copy (struct peer *dst, struct peer *src, afi_t afi, safi_t safi)
{
  ...
  if (DISTRIBUTE_OUT_NAME (srcfilter))
    {
      DISTRIBUTE_OUT_NAME (dstfilter) =
          XSTRDUP (MTYPE_BGP_FILTER_NAME, DISTRIBUTE_OUT_NAME (srcfilter));
      DISTRIBUTE_OUT (dstfilter) = DISTRIBUTE_OUT (srcfilter);
    }

  if (PREFIX_LIST_OUT_NAME (srcfilter))
    {
      PREFIX_LIST_OUT_NAME (dstfilter) =
          XSTRDUP (MTYPE_BGP_FILTER_NAME, PREFIX_LIST_OUT_NAME (srcfilter));
      PREFIX_LIST_OUT (dstfilter) = PREFIX_LIST_OUT (srcfilter);
    }

  if (FILTER_LIST_OUT_NAME (srcfilter))
    {
      FILTER_LIST_OUT_NAME (dstfilter) =
          XSTRDUP (MTYPE_BGP_FILTER_NAME, FILTER_LIST_OUT_NAME (srcfilter));
      FILTER_LIST_OUT (dstfilter) = FILTER_LIST_OUT (srcfilter);
    }

  if (ROUTE_MAP_OUT_NAME (srcfilter))
    {
      ROUTE_MAP_OUT_NAME (dstfilter) =
          XSTRDUP (MTYPE_BGP_FILTER_NAME, ROUTE_MAP_OUT_NAME (srcfilter));
      ROUTE_MAP_OUT (dstfilter) = ROUTE_MAP_OUT (srcfilter);
    }

  if (UNSUPPRESS_MAP_NAME (srcfilter))
    {
      UNSUPPRESS_MAP_NAME (dstfilter) =
          XSTRDUP (MTYPE_BGP_FILTER_NAME, UNSUPPRESS_MAP_NAME (srcfilter));
      UNSUPPRESS_MAP (dstfilter) = UNSUPPRESS_MAP (srcfilter);
    }

  if (ADVERTISE_MAP_NAME (srcfilter))
    {
      ADVERTISE_MAP_NAME (dstfilter) =
          XSTRDUP (MTYPE_BGP_FILTER_NAME, ADVERTISE_MAP_NAME (srcfilter));
  ...
  if (CONDITION_MAP_NAME (srcfilter))
    {
      CONDITION_MAP_NAME (dstfilter) =
          XSTRDUP (MTYPE_BGP_FILTER_NAME, CONDITION_MAP_NAME (srcfilter));
 ...
 */
static unsigned int
updgrp_hash_key_make (const void *p)
{
...
#define SEED2 2147483647
  ...
  key =
      jhash_1word ((peer->af_cap[afi][safi] & PEER_UPDGRP_AF_CAP_FLAGS), key);
  ...
  if (peer->group)
    key = jhash_1word (
        jhash (peer->group->name, strlen (peer->group->name), SEED1), key);
  ...
  if (filter->aslist[FILTER_OUT].name)
    key = jhash_1word (jhash (filter->aslist[FILTER_OUT].name,
                              strlen (filter->aslist[FILTER_OUT].name), SEED1),
                       key);

  if (filter->usmap.name)
    key = jhash_1word (
        jhash (filter->usmap.name, strlen (filter->usmap.name), SEED1), key);

  if (filter->advmap.aname)
    key = jhash_1word (
        jhash (filter->advmap.aname, strlen (filter->advmap.aname), SEED1),
        key);
   ...
   */
  key = jhash_1word (
      (peer->shared_network && peer_afi_active_nego (peer, AFI_IP6)), key);
  /*
  ...
  if (bgp_debug_neighbor_events (peer))
    {
      ...
      zlog_debug (
          ...
          CHECK_FLAG (peer->af_cap[afi][safi], PEER_UPDGRP_AF_CAP_FLAGS),
      ...
      zlog_debug (
          ...
          DISTRIBUTE_OUT_NAME (filter) ? DISTRIBUTE_OUT_NAME (filter)
                                       : "(NONE)",
          PREFIX_LIST_OUT_NAME (filter) ? PREFIX_LIST_OUT_NAME (filter)
                                        : "(NONE)",
          FILTER_LIST_OUT_NAME (filter) ? FILTER_LIST_OUT_NAME (filter)
                                        : "(NONE)",
          UNSUPPRESS_MAP_NAME (filter) ? UNSUPPRESS_MAP_NAME (filter)
                                       : "(NONE)",
          ADVERTISE_MAP_NAME (filter) ? ADVERTISE_MAP_NAME (filter) : "(NONE)",
      ...
      zlog_debug ("%pBP Update Group Hash: local role: %u AIGP: %d SOO: %s",
                  peer, peer->local_role,
                  ! ! CHECK_FLAG (peer->flags, PEER_FLAG_AIGP),
                  peer->soo[afi][safi] ? ecommunity_str (peer->soo[afi][safi])
                                       : "(NONE)");
...
updgrp_hash_cmp (const void *p1, const void *p2)
{
  ...
  /* check peer flags */
  if ((pe1->flags & PEER_UPDGRP_FLAGS) != (pe2->flags & PEER_UPDGRP_FLAGS))
    return false;
  ...
  if ((pe1->cap & PEER_UPDGRP_CAP_FLAGS) != (pe2->cap & PEER_UPDGRP_CAP_FLAGS))
    return false;
  ...
  if (ctx->uj)
    {
      ...
      json_object_string_add (json_time, "epochString", ctime (&epoch_tbuf));
      json_object_object_add (json_updgrp, "groupCreateTime", json_time);
      json_object_string_add (json_updgrp, "afi", afi2str (updgrp->afi));
      json_object_string_add (json_updgrp, "safi", safi2str (updgrp->safi));
    }
  else
    {
      ...
      if (ctx->uj)
        {
          ...
          json_object_boolean_add (
              ...
              CHECK_FLAG (updgrp->conf->flags, PEER_FLAG_LOCAL_AS_NO_PREPEND));
          json_object_boolean_add (
              json_updgrp, "replaceLocalAs",
              CHECK_FLAG (updgrp->conf->flags, PEER_FLAG_LOCAL_AS_REPLACE_AS));
        }
      else
        {
          vty_out (
      ...
      if (ctx->uj)
        {
          ...
          json_object_int_add (json_subgrp, "subGroupId", subgrp->id);
          tbuf = monotime (NULL);
          tbuf -= subgrp->uptime;
          epoch_tbuf = time (NULL) - tbuf;
          json_subgrp_time = json_object_new_object ();
          json_object_int_add (json_subgrp_time, "epoch", epoch_tbuf);
      ...
      if (subgrp->split_from.update_group_id || subgrp->split_from.subgroup_id)
        {
          if (ctx->uj)
            {
              json_object_int_add (json_subgrp, "splitGroupId",
                                   subgrp->split_from.update_group_id);
              json_object_int_add (json_subgrp, "splitSubGroupId",
      ...
      if (ctx->uj)
        {
          ...
          json_object_int_add (json_subgrp_event, "peerRefreshEvents",
                               subgrp->peer_refreshes_combined);
          json_object_int_add (json_subgrp_event, "mergeCheckEvents",
                               subgrp->merge_checks_triggered);
          json_object_object_add (json_subgrp, "statistics",
                                  json_subgrp_event);
          json_object_int_add (
              json_subgrp, "coalesceTime",
              (UPDGRP_INST (subgrp->update_group))->coalesce_time);
          json_object_int_add (json_subgrp, "version", subgrp->version);
          json_pkt_info = json_object_new_object ();
          json_object_int_add (json_pkt_info, "qeueueLen",
                               bpacket_queue_length (SUBGRP_PKTQ (subgrp)));
          json_object_int_add (json_pkt_info, "queuedTotal",
                               subgroup_total_packets_enqueued (subgrp));
          json_object_int_add (
              json_pkt_info, "queueHwmLen",
              bpacket_queue_hwm_length (SUBGRP_PKTQ (subgrp)));
          json_object_int_add (json_pkt_info, "totalEnqueued",
                               subgroup_total_packets_enqueued (subgrp));
          json_object_object_add (json_subgrp, "packetQueueInfo",
                                  json_pkt_info);
          json_object_int_add (json_subgrp, "adjListCount", subgrp->adj_count);
          json_object_boolean_add (
          ...
          if (ctx->uj)
            {
              ...
              SUBGRP_FOREACH_PEER (subgrp, paf)
                {
                  json_object *peer = json_object_new_string (paf->peer->host);
                  json_object_array_add (json_peers, peer);
                }
              json_object_object_add (json_subgrp, "peers", json_peers);
            }
          else
            {
              vty_out (vty, "    Peers:\n");
  ...
  if (ctx->uj)
    {
      ...
      json_object_object_addf (ctx->json_updategrps, json_updgrp, "%" PRIu64,
                               updgrp->id);
    }
                                ...
                                struct vty *vty, uint64_t id)
{
  ...
  ctx.subgrp_id = id;
  ctx.flags = 0;
  update_group_af_walk (bgp, afi, safi, updgrp_show_packet_queue_walkcb, &ctx);
}
                          ...
                          int send_enqueued_pkts)
{
  ...
  if (BGP_DEBUG (update_groups, UPDATE_GROUPS))
    zlog_debug ("peer %s added to subgroup s%" PRIu64, paf->peer->host,
                subgrp->id);
}
                                      ...
                                      struct peer_af *paf)
{
  ...
  if (BGP_DEBUG (update_groups, UPDATE_GROUPS))
    zlog_debug ("peer %s deleted from subgroup s%" PRIu64 " peer cnt %d",
 ...
 */
static void
update_subgroup_merge_check_thread_cb (struct event *thread)
{
  struct update_subgroup *subgrp;
 ...
 */
bool
update_subgroup_trigger_merge_check (struct update_subgroup *subgrp, int force)
{
  ...
  SUBGRP_FOREACH_ADJ (source, aout)
    {
       ...
       * Copy the adj out.
       */
      aout_copy = bgp_adj_out_alloc (dest, aout->dest, aout->addpath_tx_id);
      aout_copy->attr = aout->attr ? bgp_attr_intern (aout->attr) : NULL;
    }
...
updgrp_policy_update_walkcb (struct update_group *updgrp, void *arg)
{
  ...
  switch (ctx->policy_type)
    ...
    case BGP_POLICY_ROUTE_MAP:
      changed =
          updgrp_route_map_update (updgrp, ctx->policy_name, &def_changed);
      break;
...
static int
update_group_walkcb (struct hash_bucket *bucket, void *arg)
{
 ...
 */
void
update_group_policy_update (struct bgp *bgp, enum bgp_policy_type ptype,
...
update_subgroup_split_peer (struct peer_af *paf, struct update_group *updgrp)
{
  ...
  if (old_subgrp->peer_count == 1)
    {
      ...
      if (BGP_DEBUG (update_groups, UPDATE_GROUPS))
        zlog_debug ("u%" PRIu64 ":s%" PRIu64 " peer %s moved to u%" PRIu64
                    ":s%" PRIu64,
                    old_id, subgrp->id, paf->peer->host, updgrp->id,
                    subgrp->id);
  ...
  if (BGP_DEBUG (update_groups, UPDATE_GROUPS))
    zlog_debug ("u%" PRIu64 ":s%" PRIu64
                " peer %s split and moved into u%" PRIu64 ":s%" PRIu64,
...
update_bgp_group_init (struct bgp *bgp)
{
  ...
  AF_FOREACH (afid)
    bgp->update_groups[afid] = hash_create (
        updgrp_hash_key_make, updgrp_hash_cmp, "BGP Update Group Hash");
}
  ...
  if (uj)
    {
      ...
      if (bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT)
        ...
        vname = bgp->name;
      json_object_object_add (json_vrf_obj, vname, ctx.json_updategrps);
...
update_group_periodic_merge (struct bgp *bgp)
{
  ...
  update_group_walk (bgp, update_group_periodic_merge_walkcb, (void *) reason);
}
...
subgroup_trigger_write (struct update_subgroup *subgrp)
{
  ...
  SUBGRP_FOREACH_PEER (subgrp, paf)
    if (peer_established (paf->peer))
      event_add_timer_msec (bm->master, bgp_generate_updgrp_packets, paf->peer,
                            0, &paf->peer->t_generate_updgrp_packets);
}
...
bool
bgp_addpath_capable (struct bgp_path_info *bpi, struct peer *peer, afi_t afi,
                     safi_t safi)
{
  return (bgp_addpath_tx_path (peer->addpath_type[afi][safi], bpi) ||
          (safi == SAFI_LABELED_UNICAST &&
           bgp_addpath_tx_path (peer->addpath_type[afi][SAFI_UNICAST], bpi)));
}

bool
bgp_check_selected (struct bgp_path_info *bpi, struct peer *peer,
