...
static struct bgp_evpn_es_vtep *bgp_evpn_es_vtep_add (struct bgp *bgp,
                                                      ...
                                                      uint16_t df_pref);
static void bgp_evpn_es_vtep_del (struct bgp *bgp, struct bgp_evpn_es *es,
                                  struct in_addr vtep_ip, bool esr);
 ...
 */
static int
bgp_evpn_es_route_select_install (struct bgp *bgp, struct bgp_evpn_es *es,
                                  struct bgp_dest *dest)
{
      ...
      if (bgp_zebra_has_route_changed (old_select))
        {
          bgp_evpn_es_vtep_add (bgp, es, old_select->attr->nexthop,
                                true /*esr*/, old_select->attr->df_alg,
  ...
  if (new_select && new_select->type == ZEBRA_ROUTE_BGP &&
    ...
    {
      bgp_evpn_es_vtep_add (bgp, es, new_select->attr->nexthop, true /*esr */,
  ...
  else
    {
      if (old_select && old_select->type == ZEBRA_ROUTE_BGP &&
          old_select->sub_type == BGP_ROUTE_IMPORTED)
        bgp_evpn_es_vtep_del (bgp, es, old_select->attr->nexthop,
                              true /*esr*/);
    }
...
static int
bgp_evpn_es_route_install (struct bgp *bgp, struct bgp_evpn_es *es,
...
static int
bgp_evpn_es_route_uninstall (struct bgp *bgp, struct bgp_evpn_es *es,
                             struct prefix_evpn *p,
                             struct bgp_path_info *parent_pi)
{
  int ret;
...
int
bgp_evpn_es_route_install_uninstall (struct bgp *bgp, struct bgp_evpn_es *es,
                                     ...
                                     struct bgp_path_info *pi, int install)
{
  ...
  if (ret)
    {
      flog_err (EC_BGP_EVPN_FAIL, "%u: Failed to %s EVPN %s route in ESI %s",
                bgp->vrf_id, install ? "install" : "uninstall", "ES",
                es->esi_str);
      return ret;
    }
  return 0;
...
/* Delete (and withdraw) local routes for specified ES from global and ES
 * table. Also remove all remote routes from the per ES table. Invoked when ES
 * is deleted.
 */
static void
bgp_evpn_es_route_del_all (struct bgp *bgp, struct bgp_evpn_es *es)
{
  struct bgp_dest *dest;
 ...
 */
int
bgp_evpn_mh_route_update (struct bgp *bgp, struct bgp_evpn_es *es,
  ...
  if (*route_changed)
    {
      if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
        zlog_debug ("local ES %s vni %u route-type %s nexthop %pI4 updated",
 ...
 */
static int
bgp_evpn_mh_route_delete (struct bgp *bgp, struct bgp_evpn_es *es,
                          struct bgpevpn *vpn,
                          ...
                          struct prefix_evpn *p)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
    zlog_debug ("local ES %s vni %u route-type %s nexthop %pI4 delete",
   ...
   */
  global_dest =
      bgp_evpn_global_node_lookup (bgp->rib[afi][safi], safi, p, prd, NULL);
  if (global_dest)
    {
 ...
 */
int
delete_global_ead_evi_routes (struct bgp *bgp, struct bgpevpn *vpn)
{
  afi_t afi;
  safi_t safi;
...
static void
bgp_evpn_type4_route_extcomm_build (struct bgp_evpn_es *es, struct attr *attr)
{
  ...
  bgp_attr_set_ecommunity (
      attr, ecommunity_merge (bgp_attr_get_ecommunity (attr), &ecom_es_rt));
  ...
  bgp_attr_set_ecommunity (
      attr, ecommunity_merge (bgp_attr_get_ecommunity (attr), &ecom_df));
}

/* Create or update local type-4 route */
static int
bgp_evpn_type4_route_update (struct bgp *bgp, struct bgp_evpn_es *es,
                             struct prefix_evpn *p)
{
  int ret = 0;
  int route_changed = 0;
  ...
  ret = bgp_evpn_mh_route_update (bgp, es, NULL, afi, safi, dest, &attr, &pi,
                                  &route_changed);
  if (ret != 0)
  ...
  if (route_changed)
    {
      ...
      bgp_evpn_mh_route_update (bgp, es, NULL, afi, safi, dest, attr_new,
...
static int
bgp_evpn_type4_route_delete (struct bgp *bgp, struct bgp_evpn_es *es,
                             struct prefix_evpn *p)
{
  if (! es->es_base_frag)
    return -1;

  return bgp_evpn_mh_route_delete (bgp, es, NULL /* l2vni */, es->es_base_frag,
                                   p);
}

/* Process remote/received EVPN type-4 route (advertise or withdraw)  */
int
bgp_evpn_type4_route_process (struct peer *peer, afi_t afi, safi_t safi,
                              ...
                              uint32_t addpath_id)
{
   ...
   */
  if (psize != BGP_EVPN_TYPE4_V4_PSIZE && psize != BGP_EVPN_TYPE4_V6_PSIZE)
    {
...
static bool
bgp_evpn_type4_prefix_match (struct prefix_evpn *p, struct bgp_evpn_es *es)
{
...
static int
bgp_evpn_type4_remote_routes_import (struct bgp *bgp, struct bgp_evpn_es *es,
                                     bool install)
{
  int ret;
  afi_t afi;
                                       ...
                                       struct attr *attr)
{
  ...
  if (listcount (bgp_mh_info->ead_es_export_rtl))
    {
      for (ALL_LIST_ELEMENTS_RO (bgp_mh_info->ead_es_export_rtl, rt_node,
                                 ecom))
      ...
      for (ALL_LIST_ELEMENTS_RO (es_frag->es_evi_frag_list, evi_node, es_evi))
        {
          if (! CHECK_FLAG (es_evi->flags, BGP_EVPNES_EVI_LOCAL))
            continue;
          for (ALL_LIST_ELEMENTS_RO (es_evi->vpn->export_rtl, rt_node, ecom))
            bgp_attr_set_ecommunity (
...
static void
bgp_evpn_type1_evi_route_extcomm_build (struct bgp_evpn_es *es,
 ...
 */
static int
bgp_evpn_type1_route_update (struct bgp *bgp, struct bgp_evpn_es *es,
                             struct bgpevpn *vpn,
                             struct bgp_evpn_es_frag *es_frag,
                             struct prefix_evpn *p)
{
  ...
  if (vpn)
    {
      ...
      /* Create or update route entry. */
      ret = bgp_evpn_mh_route_update (bgp, es, vpn, afi, safi, dest, &attr,
                                      &pi, &route_changed);
      if (ret != 0)
        flog_err (EC_BGP_ES_INVALID,
  ...
  if (route_changed)
    {
      ...
      dest = bgp_evpn_global_node_get (bgp->rib[afi][safi], afi, safi, p,
                                       global_rd, NULL);
      bgp_evpn_mh_route_update (bgp, es, vpn, afi, safi, dest, attr_new,
                                &global_pi, &route_changed);
 ...
 * Update all type-1 local routes for this VNI from VNI/ES tables and the
 * global table and advertise these routes to peers.
 */

static void
bgp_evpn_ead_es_route_update (struct bgp *bgp, struct bgp_evpn_es *es)
{
...
static void
bgp_evpn_ead_evi_route_update (struct bgp *bgp, struct bgp_evpn_es *es,
                               struct bgpevpn *vpn, struct prefix_evpn *p)
{
  if (bgp_evpn_type1_route_update (bgp, es, vpn, NULL, p))
    flog_err (EC_BGP_EVPN_ROUTE_CREATE,
              "EAD-EVI route creation failure for ESI %s VNI %u", es->esi_str,
              vpn->vni);
}

void
update_type1_routes_for_evi (struct bgp *bgp, struct bgpevpn *vpn)
{
  struct prefix_evpn p;
  struct bgp_evpn_es *es;
  struct bgp_evpn_es_evi *es_evi;
...
static void
bgp_evpn_ead_es_route_delete (struct bgp *bgp, struct bgp_evpn_es *es)
{
  struct listnode *node;
  struct bgp_evpn_es_frag *es_frag;
  struct prefix_evpn p;
...
static int
bgp_evpn_ead_evi_route_delete (struct bgp *bgp, struct bgp_evpn_es *es,
                               struct bgpevpn *vpn, struct prefix_evpn *p)
{
...
static void
bgp_evpn_local_type1_evi_route_add (struct bgp *bgp, struct bgp_evpn_es *es)
{
 ...
 */
static void
bgp_evpn_local_type1_evi_route_del (struct bgp *bgp, struct bgp_evpn_es *es)
{
  struct listnode *evi_node;
  struct prefix_evpn p;
  struct bgp_evpn_es_evi *es_evi;
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_evi_list, evi_node, es_evi))
    {
      ...
      if (bgp_evpn_mh_route_delete (bgp, es, es_evi->vpn, NULL, &p))
        flog_err (EC_BGP_EVPN_ROUTE_CREATE,
                  "%u: Type4 route creation failure for ESI %s", bgp->vrf_id,
                  es->esi_str);
    }
}
 ...
 */
int
bgp_evpn_type1_route_process (struct peer *peer, afi_t afi, safi_t safi,
                              struct attr *attr, uint8_t *pfx, int psize,
                              uint32_t addpath_id)
{
  struct prefix_rd prd;
...
void
bgp_evpn_mh_config_ead_export_rt (struct bgp *bgp, struct ecommunity *ecomcfg,
                                  bool del)
{
  ...
  if (del)
    {
      if (ecomcfg == NULL)
        {
          ...
          for (ALL_LIST_ELEMENTS (bgp_mh_info->ead_es_export_rtl, node, nnode,
                                  ecom))
            {
              ecommunity_free (&ecom);
              list_delete_node (bgp_mh_info->ead_es_export_rtl, node);
            }
        }
      ...
      /* Delete a specific export RT */
      else
        {
          ...
          assert (node_to_del);
          list_delete_node (bgp_mh_info->ead_es_export_rtl, node_to_del);
        }
    }
  else
    {
  ...
  RB_FOREACH (es, bgp_es_rb_head, &bgp_mh_info->es_rb_tree)
    {
      if (! bgp_evpn_is_es_local (es) || ! bgp_evpn_local_es_is_active (es))
        continue;

      if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
        zlog_debug ("local ES %s del/re-add EAD route on export RT change",
                    es->esi_str);
 ...
 */
static int
bgp_evpn_es_vtep_cmp (void *p1, void *p2)
{
...
bgp_evpn_es_vtep_new (struct bgp_evpn_es *es, struct in_addr vtep_ip)
{
...
static void
bgp_evpn_es_vtep_free (struct bgp_evpn_es_vtep *es_vtep)
{
  struct bgp_evpn_es *es = es_vtep->es;

  if (CHECK_FLAG (es_vtep->flags, BGP_EVPNES_VTEP_ESR) || es_vtep->evi_cnt)
...
static struct bgp_evpn_es_vtep *
bgp_evpn_es_vtep_find (struct bgp_evpn_es *es, struct in_addr vtep_ip)
{
...
static int
bgp_zebra_send_remote_es_vtep (struct bgp *bgp,
                               struct bgp_evpn_es_vtep *es_vtep, bool add)
{
  ...
  zclient_create_header (
      s, add ? ZEBRA_REMOTE_ES_VTEP_ADD : ZEBRA_REMOTE_ES_VTEP_DEL,
      bgp->vrf_id);
...
static void
bgp_evpn_es_vtep_re_eval_active (struct bgp *bgp,
                                 struct bgp_evpn_es_vtep *es_vtep,
                                 bool param_change)
{
  bool old_active;
  ...
  if ((old_active != new_active) || (new_active && param_change))
    {
      ...
      if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
        zlog_debug ("es %s vtep %pI4 %s df %u/%u", es_vtep->es->esi_str,
                      ...
                      uint16_t df_pref)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("es %s vtep %pI4 add %s df %u/%u", es_vtep->es->esi_str,
                &es_vtep->vtep_ip, esr ? "esr" : "ead", df_alg, df_pref);

  if (esr)
    {
      SET_FLAG (es_vtep->flags, BGP_EVPNES_VTEP_ESR);
      if ((es_vtep->df_pref != df_pref) || (es_vtep->df_alg != df_alg))
        {
          param_change = true;
...
static void
bgp_evpn_es_vtep_del (struct bgp *bgp, struct bgp_evpn_es *es,
                      struct in_addr vtep_ip, bool esr)
{
  struct bgp_evpn_es_vtep *es_vtep;
 ...
 ****************************************************************************/
static void
bgp_evpn_path_es_info_free (struct bgp_path_es_info *es_info)
{
...
void
bgp_evpn_path_mh_info_free (struct bgp_path_mh_info *mh_info)
{
  if (mh_info->es_info)
...
bgp_evpn_path_es_info_new (struct bgp_path_info *pi, vni_t vni)
{
  ...
  if (! es_info)
    {
      mh_info->es_info = es_info = XCALLOC (MTYPE_BGP_EVPN_PATH_ES_INFO,
...
static void
bgp_evpn_path_es_unlink (struct bgp_path_es_info *es_info)
{
  ...
  if (es_info->vni)
    ...
    list_delete_node (es->macip_global_path_list, &es_info->es_listnode);
...
void
bgp_evpn_path_es_link (struct bgp_path_info *pi, vni_t vni, esi_t *esi)
{
  struct bgp_path_es_info *es_info;
  struct bgp_evpn_es *es;
  struct bgp *bgp_evpn;
  ...
  es_info =
      (pi->extra && pi->extra->mh_info) ? pi->extra->mh_info->es_info : NULL;
...
static bool
bgp_evpn_is_macip_path (struct bgp_path_info *pi)
{
  struct prefix_evpn *evp;
                                       ...
                                       const char *reason)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
    zlog_debug ("update paths linked to es %s on es-vrf %s %s", es->esi_str,
                es_vrf->bgp_vrf->name_pretty, reason);
  ...
  for (ALL_LIST_ELEMENTS_RO (es->macip_global_path_list, node, es_info))
    {
      ...
      if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
        zlog_debug ("update path %pFX linked to es %s on vrf chg", &pi->net->p,
                    es->esi_str);
      bgp_evpn_route_entry_install_if_vrf_match (es_vrf->bgp_vrf, pi, 1);
    }
}

static void
bgp_evpn_es_frag_free (struct bgp_evpn_es_frag *es_frag)
{
...
static void
bgp_evpn_es_frag_free_unused (struct bgp_evpn_es_frag *es_frag)
{
...
static void
bgp_evpn_es_frag_free_all (struct bgp_evpn_es *es)
{
  struct listnode *node;
...
bgp_evpn_es_frag_new (struct bgp_evpn_es *es)
{
  struct bgp_evpn_es_frag *es_frag;
  ...
  bgp = bgp_get_evpn ();
  snprintfrr (buf, sizeof (buf), "%pI4:%hu", &bgp->router_id, es_frag->rd_id);
...
bgp_evpn_es_find_frag_with_space (struct bgp_evpn_es *es)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_frag_list, node, es_frag))
    {
      if (listcount (es_frag->es_evi_frag_list) < bgp_mh_info->evi_per_es_frag)
        return es_frag;
    }
...
static void
bgp_evpn_es_frag_evi_add (struct bgp_evpn_es_evi *es_evi)
{
  struct bgp_evpn_es_frag *es_frag;
  struct bgp_evpn_es *es = es_evi->es;

  if (es_evi->es_frag || ! (CHECK_FLAG (es_evi->flags, BGP_EVPNES_EVI_LOCAL)))
    return;
...
static void
bgp_evpn_es_frag_evi_del (struct bgp_evpn_es_evi *es_evi,
  ...
  if (send_ead_del_if_empty && ! listcount (es_frag->es_evi_frag_list))
    {
      ...
      if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
        zlog_debug ("es %s frag %u ead-es route delete", es->esi_str,
                    es_frag->rd_id);
      build_evpn_type1_prefix (&p, BGP_EVPN_AD_ES_ETH_TAG, &es->esi,
                               es->originator_ip);
      p.prefix.ead_addr.frag_id = es_frag->rd_id;
...
static void
bgp_evpn_es_frag_evi_update_all (struct bgp_evpn_es *es, bool add)
{
  struct listnode *node;
  struct bgp_evpn_es_evi *es_evi;
...
static int
bgp_es_rb_cmp (const struct bgp_evpn_es *es1, const struct bgp_evpn_es *es2)
{
...
struct bgp_evpn_es *
bgp_evpn_es_find (const esi_t *esi)
{
...
static struct bgp_evpn_es *
bgp_evpn_es_new (struct bgp *bgp, const esi_t *esi)
{
  struct bgp_evpn_es *es;
 ...
 */
static void
bgp_evpn_es_free (struct bgp_evpn_es *es, const char *caller)
{
...
static inline bool
bgp_evpn_is_es_local_and_non_bypass (struct bgp_evpn_es *es)
{
  return (es->flags & BGP_EVPNES_LOCAL) && ! (es->flags & BGP_EVPNES_BYPASS);
}

/* init local info associated with the ES */
static void
bgp_evpn_es_local_info_set (struct bgp *bgp, struct bgp_evpn_es *es)
{
  bool old_is_local;
  bool is_local;
...
static void
bgp_evpn_es_local_info_clear (struct bgp_evpn_es *es, bool finish)
{
  bool old_is_local;
  bool is_local;
...
static void
bgp_evpn_es_remote_info_re_eval (struct bgp_evpn_es *es)
{
 ...
 */
bool
bgp_evpn_es_add_l3_ecomm_ok (esi_t *esi)
{
  struct bgp_evpn_es *es;
...
static bool
bgp_evpn_is_valid_local_path (struct bgp_path_info *pi)
{
  return (CHECK_FLAG (pi->flags, BGP_PATH_VALID) &&
          pi->type == ZEBRA_ROUTE_BGP && pi->sub_type == BGP_ROUTE_STATIC);
}
 ...
 */
static void
bgp_evpn_mac_update_on_es_oper_chg (struct bgp_evpn_es *es)
{
  struct listnode *node;
  struct bgp_path_es_info *es_info;
  struct bgp_path_info *pi;
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("update paths linked to es %s on oper chg", es->esi_str);
  ...
  for (ALL_LIST_ELEMENTS_RO (es->macip_evi_path_list, node, es_info))
    {
      ...
      if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
        zlog_debug ("update path %d %pFX linked to es %s on oper chg",
                    es_info->vni, &pi->net->p, es->esi_str);

      bgp_evpn_update_type2_route_entry (bgp, vpn, pi->net, pi, __func__);
    }
}

static bool
bgp_evpn_is_valid_bgp_path (struct bgp_path_info *pi)
{
  return (CHECK_FLAG (pi->flags, BGP_PATH_VALID) &&
 ...
 */
static void
bgp_evpn_mac_update_on_es_local_chg (struct bgp_evpn_es *es, bool is_local)
{
  struct listnode *node;
  struct bgp_path_es_info *es_info;
  struct bgp_path_info *pi;
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("update paths linked to es %s on chg to %s", es->esi_str,
  ...
  for (ALL_LIST_ELEMENTS_RO (es->macip_global_path_list, node, es_info))
    {
      ...
      if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
        zlog_debug ("update path %pFX linked to es %s on chg to %s",
                    &pi->net->p, es->esi_str,
                    is_local ? "local" : "non-local");
...
static void
bgp_evpn_local_es_deactivate (struct bgp *bgp, struct bgp_evpn_es *es)
{
  struct prefix_evpn p;
  int ret;
  ...
  if (ret)
    {
      flog_err (EC_BGP_EVPN_ROUTE_DELETE,
                "%u failed to delete type-4 route for ESI %s", bgp->vrf_id,
                es->esi_str);
    }
...
static void
bgp_evpn_local_es_down (struct bgp *bgp, struct bgp_evpn_es *es)
{
  bool old_active;
...
static void
bgp_evpn_local_es_activate (struct bgp *bgp, struct bgp_evpn_es *es,
...
static void
bgp_evpn_local_es_up (struct bgp *bgp, struct bgp_evpn_es *es, bool regen_esr)
{
 ...
 */
static void
bgp_evpn_local_es_bypass_update (struct bgp *bgp, struct bgp_evpn_es *es,
                                 bool bypass)
{
...
static void
bgp_evpn_local_es_do_del (struct bgp *bgp, struct bgp_evpn_es *es)
{
  struct bgp_evpn_es_evi *es_evi;
  ...
  for (ALL_LIST_ELEMENTS (es->es_evi_list, evi_node, evi_next_node, es_evi))
    {
...
bool
bgp_evpn_is_esi_local_and_non_bypass (esi_t *esi)
{
...
int
bgp_evpn_local_es_del (struct bgp *bgp, esi_t *esi)
{
  struct bgp_evpn_es *es = NULL;
 ...
 */
int
bgp_evpn_local_es_add (struct bgp *bgp, esi_t *esi,
                       ...
                       uint16_t df_pref, bool bypass)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("add local es %s orig-ip %pI4 df_pref %u %s", es->esi_str,
  ...
  if (new_es)
    bgp_evpn_type4_remote_routes_import (bgp, es, true /* install */);
...
static void
bgp_evpn_es_json_frag_fill (json_object *json_frags, struct bgp_evpn_es *es)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_frag_list, node, es_frag))
    {
      ...
      json_object_string_addf (json_frag, "rd", "%pRDP", &es_frag->prd);
...
static void
bgp_evpn_es_frag_show_detail (struct vty *vty, struct bgp_evpn_es *es)
{
  struct listnode *node;
  struct bgp_evpn_es_frag *es_frag;
...
static char *
bgp_evpn_es_vteps_str (char *vtep_str, struct bgp_evpn_es *es,
...
static void
bgp_evpn_es_json_vtep_fill (json_object *json_vteps,
                            struct bgp_evpn_es_vtep *es_vtep)
{
  ...
  json_object_string_addf (json_vtep_entry, "vtep_ip", "%pI4",
                           &es_vtep->vtep_ip);
  if (es_vtep->flags & (BGP_EVPNES_VTEP_ESR | BGP_EVPNES_VTEP_ACTIVE))
    {
      ...
      if (es_vtep->flags & BGP_EVPNES_VTEP_ESR)
        {
          ...
          json_object_string_add (
              ...
              evpn_es_df_alg2str (es_vtep->df_alg, alg_buf, sizeof (alg_buf)));
        }
    }
...
static void
bgp_evpn_es_vteps_show_detail (struct vty *vty, struct bgp_evpn_es *es)
{
  char vtep_flag_str[BGP_EVPN_FLAG_STR_SZ];
  struct listnode *node;
  struct bgp_evpn_es_vtep *es_vtep;
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_vtep_list, node, es_vtep))
    {
      ...
      vty_out (vty, "  %pI4 flags: %s", &es_vtep->vtep_ip, vtep_flag_str);

      if (es_vtep->flags & BGP_EVPNES_VTEP_ESR)
        vty_out (
            vty, " df_alg: %s df_pref: %u\n",
            evpn_es_df_alg2str (es_vtep->df_alg, alg_buf, sizeof (alg_buf)),
            es_vtep->df_pref);
      else
        vty_out (vty, "\n");
...
static void
bgp_evpn_es_show_entry (struct vty *vty, struct bgp_evpn_es *es,
                        json_object *json)
{
  struct listnode *node;
  struct bgp_evpn_es_vtep *es_vtep;
  ...
  if (json)
    {
      ...
      if (es->es_base_frag)
        json_object_string_addf (json, "rd", "%pRDP", &es->es_base_frag->prd);
      ...
      json_object_int_add (json, "vniCount", listcount (es->es_evi_list));
    }
  else
    {
      char type_str[4];
...
static void
bgp_evpn_es_show_entry_detail (struct vty *vty, struct bgp_evpn_es *es,
                               json_object *json)
{
  if (json)
    {
      ...
      if (es->flags &
          (BGP_EVPNES_OPER_UP | BGP_EVPNES_ADV_EVI | BGP_EVPNES_BYPASS))
        {
          json_flags = json_object_new_array ();
          if (es->flags & BGP_EVPNES_OPER_UP)
            json_array_string_add (json_flags, "up");
          if (es->flags & BGP_EVPNES_ADV_EVI)
            json_array_string_add (json_flags, "advertiseEVI");
          if (es->flags & BGP_EVPNES_BYPASS)
      ...
      json_object_string_addf (json, "originator_ip", "%pI4",
                               &es->originator_ip);
      json_object_int_add (json, "remoteVniCount", es->remote_es_evi_cnt);
      json_object_int_add (json, "vrfCount", listcount (es->es_vrf_list));
      ...
      if (es->flags & BGP_EVPNES_LOCAL)
        json_object_int_add (json, "localEsDfPreference", es->df_pref);
      if (listcount (es->es_vtep_list))
        {
          json_vteps = json_object_new_array ();
          for (ALL_LIST_ELEMENTS_RO (es->es_vtep_list, node, es_vtep))
      ...
      if (es->inconsistencies)
        {
          ...
          if (es->inconsistencies & BGP_EVPNES_INCONS_VTEP_LIST)
            json_array_string_add (json_incons, "vni-vtep-mismatch");
          json_object_object_add (json, "inconsistencies", json_incons);
        }
    }
  else
    {
      ...
      if (es->inconsistencies)
        {
          incons_str[0] = '\0';
          if (es->inconsistencies & BGP_EVPNES_INCONS_VTEP_LIST)
            strlcat (incons_str, "vni-vtep-mismatch", sizeof (incons_str));
        }
      else
        {
...
/* Display all ESs */
void
bgp_evpn_es_show (struct vty *vty, bool uj, bool detail)
{
  struct bgp_evpn_es *es;
...
/* Display specific ES */
void
bgp_evpn_es_show_esi (struct vty *vty, esi_t *esi, bool uj)
{
  struct bgp_evpn_es *es;
  json_object *json = NULL;
...
/******************************** L3 NHG management *************************/
static void
bgp_evpn_l3nhg_zebra_add_v4_or_v6 (struct bgp_evpn_es_vrf *es_vrf, bool v4_nhg)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("es %s vrf %u %s nhg %u to zebra", es->esi_str,
                es_vrf->bgp_vrf->vrf_id, v4_nhg ? "v4_nhg" : "v6_nhg", nhg_id);
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_vtep_list, node, es_vtep))
    {
      ...
      if (v4_nhg)
        ...
        ipv4_to_ipv4_mapped_ipv6 (&nh.gate.ipv6, es_vtep->vtep_ip);
...
static bool
bgp_evpn_l3nhg_zebra_ok (struct bgp_evpn_es_vrf *es_vrf)
{
  if (! bgp_mh_info->host_routes_use_l3nhg)
    return false;
...
static void
bgp_evpn_l3nhg_zebra_add (struct bgp_evpn_es_vrf *es_vrf)
{
...
static void
bgp_evpn_l3nhg_zebra_del_v4_or_v6 (struct bgp_evpn_es_vrf *es_vrf, bool v4_nhg)
{
  struct zapi_nhg api_nhg = {};
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("es %s vrf %u %s nhg %u to zebra", es_vrf->es->esi_str,
                es_vrf->bgp_vrf->vrf_id, v4_nhg ? "v4_nhg" : "v6_nhg",
                api_nhg.id);


  frrtrace (4, frr_bgp, evpn_mh_nhg_zsend, false, v4_nhg, api_nhg.id, es_vrf);

  zclient_nhg_send (zclient, ZEBRA_NHG_DEL, &api_nhg);
}

static void
bgp_evpn_l3nhg_zebra_del (struct bgp_evpn_es_vrf *es_vrf)
{
  if (! bgp_evpn_l3nhg_zebra_ok (es_vrf))
    return;
...
static void
bgp_evpn_l3nhg_deactivate (struct bgp_evpn_es_vrf *es_vrf)
{
  if (! (es_vrf->flags & BGP_EVPNES_VRF_NHG_ACTIVE))
    return;

  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("es %s vrf %u nhg %u de-activate", es_vrf->es->esi_str,
...
static void
bgp_evpn_l3nhg_activate (struct bgp_evpn_es_vrf *es_vrf, bool update)
{
  ...
  if (es_vrf->flags & BGP_EVPNES_VRF_NHG_ACTIVE)
    ...
    {
      if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
        zlog_debug ("es %s vrf %u nhg %u activate", es_vrf->es->esi_str,
                    es_vrf->bgp_vrf->vrf_id, es_vrf->nhg_id);
      es_vrf->flags |= BGP_EVPNES_VRF_NHG_ACTIVE;
      /* MAC-IPs can now be installed via the L3NHG */
 ...
 */
static void
bgp_evpn_l3nhg_update_on_vtep_chg (struct bgp_evpn_es *es)
{
...
static int
bgp_es_vrf_rb_cmp (const struct bgp_evpn_es_vrf *es_vrf1,
...
void
bgp_evpn_vrf_es_init (struct bgp *bgp_vrf)
{
...
static struct bgp_evpn_es_vrf *
bgp_evpn_es_vrf_find (struct bgp_evpn_es *es, struct bgp *bgp_vrf)
{
  struct bgp_evpn_es_vrf es_vrf;
...
static struct bgp_evpn_es_vrf *
bgp_evpn_es_vrf_create (struct bgp_evpn_es *es, struct bgp *bgp_vrf)
{
  struct bgp_evpn_es_vrf *es_vrf;
...
static void
bgp_evpn_es_vrf_delete (struct bgp_evpn_es_vrf *es_vrf)
{
  struct bgp_evpn_es *es = es_vrf->es;
  struct bgp *bgp_vrf = es_vrf->bgp_vrf;

  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("es %s vrf %u nhg %u delete", es->esi_str, bgp_vrf->vrf_id,
                es_vrf->nhg_id);
...
void
bgp_evpn_es_vrf_deref (struct bgp_evpn_es_evi *es_evi)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("es-evi %s vni %u vrf %u de-ref", es_evi->es->esi_str,
                es_evi->vpn->vni, es_vrf->bgp_vrf->vrf_id);
...
void
bgp_evpn_es_vrf_ref (struct bgp_evpn_es_evi *es_evi, struct bgp *bgp_vrf)
{
  struct bgp_evpn_es *es = es_evi->es;
  struct bgp_evpn_es_vrf *es_vrf = es_evi->es_vrf;
 ...
 */
void
bgp_evpn_es_evi_vrf_deref (struct bgpevpn *vpn)
{
  struct bgp_evpn_es_evi *es_evi;
...
void
bgp_evpn_es_evi_vrf_ref (struct bgpevpn *vpn)
{
  struct bgp_evpn_es_evi *es_evi;
 ...
 */
void
bgp_evpn_es_vrf_use_nhg (struct bgp *bgp_vrf, esi_t *esi, bool *use_l3nhg,
...
bool
bgp_evpn_path_es_use_nhg (struct bgp *bgp_vrf, struct bgp_path_info *pi,
                          uint32_t *nhg_p)
{
  esi_t *esi;
  ...
  *nhg_p = 0;
  ...
  for (mpinfo = bgp_path_info_mpath_next (pi); mpinfo;
       mpinfo = bgp_path_info_mpath_next (mpinfo))
    {
       ...
       * multipath
       */
      if (memcmp (esi, bgp_evpn_attr_get_esi (mpinfo->attr), sizeof (*esi)))
        return false;
    }
...
static void
bgp_evpn_es_vrf_show_entry (struct vty *vty, struct bgp_evpn_es_vrf *es_vrf,
                            json_object *json)
{
  struct bgp_evpn_es *es = es_vrf->es;
  struct bgp *bgp_vrf = es_vrf->bgp_vrf;
...
static void
bgp_evpn_es_vrf_show_es (struct vty *vty, json_object *json_array,
                         struct bgp_evpn_es *es)
{
  json_object *json = NULL;
  struct listnode *es_vrf_node;
...
/* Display all ES VRFs */
void
bgp_evpn_es_vrf_show (struct vty *vty, bool uj, struct bgp_evpn_es *es)
{
  json_object *json_array = NULL;
...
void
bgp_evpn_es_vrf_show_esi (struct vty *vty, esi_t *esi, bool uj)
{
  struct bgp_evpn_es *es;
 ...
 */
static int
bgp_evpn_es_evi_vtep_cmp (void *p1, void *p2)
{
...
static struct bgp_evpn_es_evi_vtep *
bgp_evpn_es_evi_vtep_new (struct bgp_evpn_es_evi *es_evi,
                          struct in_addr vtep_ip)
{
...
static void
bgp_evpn_es_evi_vtep_free (struct bgp_evpn_es_evi_vtep *evi_vtep)
{
...
/* check if VTEP is already part of the list */
static struct bgp_evpn_es_evi_vtep *
bgp_evpn_es_evi_vtep_find (struct bgp_evpn_es_evi *es_evi,
                           struct in_addr vtep_ip)
{
  struct listnode *node = NULL;
  struct bgp_evpn_es_evi_vtep *evi_vtep;
                                     ...
                                     struct bgp_evpn_es_evi_vtep *evi_vtep)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("es %s evi %u vtep %pI4 %s", evi_vtep->es_evi->es->esi_str,
  ...
  if (new_active)
    evi_vtep->es_vtep = bgp_evpn_es_vtep_add (
        bgp, evi_vtep->es_evi->es, evi_vtep->vtep_ip, false /*esr*/, 0, 0);
  else
    {
      if (evi_vtep->es_vtep)
        {
          bgp_evpn_es_vtep_do_del (bgp, evi_vtep->es_vtep, false /*esr*/);
                          ...
                          struct in_addr vtep_ip, bool ead_es)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("add es %s evi %u vtep %pI4 %s", evi_vtep->es_evi->es->esi_str,
                evi_vtep->es_evi->vpn->vni, &evi_vtep->vtep_ip,
                          ...
                          struct in_addr vtep_ip, bool ead_es)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("del es %s evi %u vtep %pI4 %s", evi_vtep->es_evi->es->esi_str,
                evi_vtep->es_evi->vpn->vni, &evi_vtep->vtep_ip,
                ead_es ? "ead_es" : "ead_evi");
...
static int
bgp_es_evi_rb_cmp (const struct bgp_evpn_es_evi *es_evi1,
...
static struct bgp_evpn_es_evi *
bgp_evpn_es_evi_find (struct bgp_evpn_es *es, struct bgpevpn *vpn)
{
  struct bgp_evpn_es_evi es_evi;
 ...
 */
static struct bgp_evpn_es_evi *
bgp_evpn_es_evi_new (struct bgp_evpn_es *es, struct bgpevpn *vpn)
{
  struct bgp_evpn_es_evi *es_evi;
...
static void
bgp_evpn_es_evi_local_info_set (struct bgp_evpn_es_evi *es_evi)
{
  struct bgpevpn *vpn = es_evi->vpn;
...
/* eval remote info associated with the ES */
static void
bgp_evpn_es_evi_remote_info_re_eval (struct bgp_evpn_es_evi *es_evi)
{
  struct bgp_evpn_es *es = es_evi->es;
...
bgp_evpn_local_es_evi_do_del (struct bgp_evpn_es_evi *es_evi)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("del local es %s evi %u", es_evi->es->esi_str,
                es_evi->vpn->vni);
      ...
      if (CHECK_FLAG (es->flags, BGP_EVPNES_ADV_EVI))
        {
          build_evpn_type1_prefix (&p, BGP_EVPN_AD_EVI_ETH_TAG, &es->esi,
                                   es->originator_ip);
          if (bgp_evpn_ead_evi_route_delete (bgp, es, es_evi->vpn, &p))
            flog_err (EC_BGP_EVPN_ROUTE_DELETE,
                      "%u: EAD-EVI route deletion failure for ESI %s VNI %u",
                      bgp->vrf_id, es->esi_str, es_evi->vpn->vni);
        }
    }
...
int
bgp_evpn_local_es_evi_del (struct bgp *bgp, esi_t *esi, vni_t vni)
{
  struct bgpevpn *vpn;
  struct bgp_evpn_es *es;
  struct bgp_evpn_es_evi *es_evi;
  ...
  if (! es)
    {
      flog_err (EC_BGP_ES_CREATE,
                ...
                bgp->vrf_id, vni, esi_to_str (esi, buf, sizeof (buf)));
      return -1;
    }
...
int
bgp_evpn_local_es_evi_add (struct bgp *bgp, esi_t *esi, vni_t vni)
{
  struct bgpevpn *vpn;
  struct prefix_evpn p;
  struct bgp_evpn_es *es;
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("add local es %s evi %u", es->esi_str, vni);
                            ...
                            const struct prefix_evpn *p)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("del remote %s es %s evi %u vtep %pI4",
                p->prefix.ead_addr.eth_tag ? "ead-es" : "ead-evi",
  ...
  if (! es_evi)
    {
      if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
        zlog_debug ("del remote %s es %s evi %u vtep %pI4, NO es-evi",
                    p->prefix.ead_addr.eth_tag ? "ead-es" : "ead-evi",
                    esi_to_str (&p->prefix.ead_addr.esi, buf, sizeof (buf)),
                    vpn->vni, &p->prefix.ead_addr.ip.ipaddr_v4);
      return 0;
...
static void
bgp_evpn_remote_es_evi_flush (struct bgp_evpn_es_evi *es_evi)
{
  struct listnode *node = NULL;
  ...
  /* delete all VTEPs */
  for (ALL_LIST_ELEMENTS (es_evi->es_evi_vtep_list, node, nnode, evi_vtep))
    {
      evi_vtep->flags &=
...
void
bgp_evpn_vni_es_init (struct bgpevpn *vpn)
{
...
void
bgp_evpn_vni_es_cleanup (struct bgpevpn *vpn)
{
  struct bgp_evpn_es_evi *es_evi;
...
static char *
bgp_evpn_es_evi_vteps_str (char *vtep_str, struct bgp_evpn_es_evi *es_evi,
                           uint8_t vtep_str_size)
{
  char vtep_flag_str[BGP_EVPN_FLAG_STR_SZ];
  struct listnode *node;
...
static void
bgp_evpn_es_evi_json_vtep_fill (json_object *json_vteps,
                                struct bgp_evpn_es_evi_vtep *evi_vtep)
{
  json_object *json_vtep_entry;
  json_object *json_flags;
...
static void
bgp_evpn_es_evi_show_entry (struct vty *vty, struct bgp_evpn_es_evi *es_evi,
                            json_object *json)
{
  struct listnode *node;
  struct bgp_evpn_es_evi_vtep *evi_vtep;
  ...
  if (json)
      ...
      if (listcount (es_evi->es_evi_vtep_list))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (es_evi->es_evi_vtep_list, node, evi_vtep))
            {
              bgp_evpn_es_evi_json_vtep_fill (json_vteps, evi_vtep);
            }
          json_object_object_add (json, "vteps", json_vteps);
        }
    }
  else
    {
      ...
      vty_out (vty, "%-8d %-30s %-5s %s\n", es_evi->vpn->vni,
               es_evi->es->esi_str, type_str, vtep_str);
    }
}

static void
bgp_evpn_es_evi_show_entry_detail (struct vty *vty,
                                   struct bgp_evpn_es_evi *es_evi,
                                   json_object *json)
{
  ...
  if (json)
    {
      ...
      if (es_evi->es_frag)
        json_object_string_addf (json, "esFragmentRd", BGP_RD_AS_FORMAT (mode),
                                 &es_evi->es_frag->prd);
      if (es_evi->flags & BGP_EVPNES_EVI_INCONS_VTEP_LIST)
        {
          json_flags = json_object_new_array ();
      ...
      if (es_evi->es_frag)
        {
          ...
          vty_out (vty, BGP_RD_AS_FORMAT (mode), &es_evi->es_frag->prd);
          vty_out (vty, "\n");
        }
      vty_out (vty, " Inconsistencies: %s\n",
...
static void
bgp_evpn_es_evi_show_one_vni (struct bgpevpn *vpn, struct vty *vty,
...
static void
bgp_evpn_es_evi_show_one_vni_hash_cb (struct hash_bucket *bucket, void *ctxt)
{
...
/* Display all ES EVIs */
void
bgp_evpn_es_evi_show (struct vty *vty, bool uj, bool detail)
{
  json_object *json_array = NULL;
...
void
bgp_evpn_es_evi_show_vni (struct vty *vty, vni_t vni, bool uj, bool detail)
{
 ...
 */
static void
bgp_evpn_es_cons_checks_timer_start (void)
{
  ...
  event_add_timer (bm->master, bgp_evpn_run_consistency_checks, NULL,
                   BGP_EVPN_CONS_CHECK_INTERVAL, &bgp_mh_info->t_cons_check);
}

/* queue up the es for background consistency checks */
static void
bgp_evpn_es_cons_checks_pend_add (struct bgp_evpn_es *es)
{
...
static void
bgp_evpn_es_cons_checks_pend_del (struct bgp_evpn_es *es)
{
  if (! CHECK_FLAG (es->flags, BGP_EVPNES_CONS_CHECK_PEND))
    return;

  UNSET_FLAG (es->flags, BGP_EVPNES_CONS_CHECK_PEND);
  list_delete_node (bgp_mh_info->pend_es_list, &es->pend_es_listnode);
}

/* Number of active VTEPs associated with the ES-per-EVI */
static uint32_t
bgp_evpn_es_evi_get_active_vtep_cnt (struct bgp_evpn_es_evi *es_evi)
{
  struct bgp_evpn_es_evi_vtep *evi_vtep;
  struct listnode *node;
  uint32_t vtep_cnt = 0;
...
static uint32_t
bgp_evpn_es_get_active_vtep_cnt (struct bgp_evpn_es *es)
{
  struct listnode *node;
  uint32_t vtep_cnt = 0;
  struct bgp_evpn_es_vtep *es_vtep;
...
static struct bgp_evpn_es_vtep *
bgp_evpn_es_get_next_active_vtep (struct bgp_evpn_es *es,
                                  struct bgp_evpn_es_vtep *es_vtep)
{
  struct listnode *node;
...
static struct bgp_evpn_es_evi_vtep *
bgp_evpn_es_evi_get_next_active_vtep (struct bgp_evpn_es_evi *es_evi,
                                      struct bgp_evpn_es_evi_vtep *evi_vtep)
{
  struct listnode *node;
...
static void
bgp_evpn_es_evi_set_inconsistent (struct bgp_evpn_es_evi *es_evi)
{
  if (! CHECK_FLAG (es_evi->flags, BGP_EVPNES_EVI_INCONS_VTEP_LIST))
    {
      ...
      if (! es_evi->es->incons_evi_vtep_cnt && BGP_DEBUG (evpn_mh, EVPN_MH_ES))
        zlog_debug ("inconsistency detected - es %s vtep list mismatch",
                    es_evi->es->esi_str);
      ++es_evi->es->incons_evi_vtep_cnt;
      SET_FLAG (es_evi->es->inconsistencies, BGP_EVPNES_INCONS_VTEP_LIST);
    }
}

static uint32_t
bgp_evpn_es_run_consistency_checks (struct bgp_evpn_es *es)
{
  int proc_cnt = 0;
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_evi_list, evi_node, es_evi))
    {
      ...
      evi_active_vtep_cnt = bgp_evpn_es_evi_get_active_vtep_cnt (es_evi);
      ...
      es_vtep = NULL;
      evi_vtep = NULL;
      while ((es_vtep = bgp_evpn_es_get_next_active_vtep (es, es_vtep)))
        {
          evi_vtep = bgp_evpn_es_evi_get_next_active_vtep (es_evi, evi_vtep);
          if (! evi_vtep)
            {
              bgp_evpn_es_evi_set_inconsistent (es_evi);
              break;
...
static void
bgp_evpn_run_consistency_checks (struct event *t)
{
  int proc_cnt = 0;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS (bgp_mh_info->pend_es_list, node, nextnode, es))
    {
      ++proc_cnt;
 ...
 ****************************************************************************/
static void
bgp_evpn_nh_zebra_update_send (struct bgp_evpn_nh *nh, bool add)
{
  struct stream *s;
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    {
      if (add)
        zlog_debug ("evpn vrf %s nh %s rmac %pEA add to zebra",
                    nh->bgp_vrf->name_pretty, nh->nh_str, &nh->rmac);
      else if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
        zlog_debug ("evpn vrf %s nh %s del to zebra", nh->bgp_vrf->name_pretty,
                    nh->nh_str);
    }
...
static void
bgp_evpn_nh_zebra_update (struct bgp_evpn_nh *nh, bool add)
{
...
static struct bgp_evpn_nh *
bgp_evpn_nh_find (struct bgp *bgp_vrf, struct ipaddr *ip)
{
...
static struct bgp_evpn_nh *
bgp_evpn_nh_add (struct bgp *bgp_vrf, struct ipaddr *ip,
                 struct bgp_path_info *pi)
{
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("evpn vrf %s nh %s rmac %pEA add", n->bgp_vrf->name_pretty,
                n->nh_str, &n->rmac);
...
static void
bgp_evpn_nh_del (struct bgp_evpn_nh *n)
{
  struct bgp_evpn_nh *tmp_n;
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("evpn vrf %s nh %s del to zebra", bgp_vrf->name_pretty,
                n->nh_str);
...
static void
hash_evpn_nh_free (struct bgp_evpn_nh *ben)
{
  XFREE (MTYPE_BGP_EVPN_NH, ben);
}

static unsigned int
bgp_evpn_nh_hash_keymake (const void *p)
{
  ...
  return jhash2 (ip->ipaddr_v6.s6_addr32, array_size (ip->ipaddr_v6.s6_addr32),
                 0);
}

static bool
bgp_evpn_nh_cmp (const void *p1, const void *p2)
{
...
void
bgp_evpn_nh_init (struct bgp *bgp_vrf)
{
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("evpn vrf %s nh init", bgp_vrf->name_pretty);
  bgp_vrf->evpn_nh_table = hash_create (bgp_evpn_nh_hash_keymake,
                                        bgp_evpn_nh_cmp, "BGP EVPN NH table");
}

static void
bgp_evpn_nh_flush_entry (struct bgp_evpn_nh *nh)
{
  struct listnode *node;
  struct listnode *nnode;
...
static void
bgp_evpn_nh_flush_cb (struct hash_bucket *bucket, void *ctxt)
{
...
void
bgp_evpn_nh_finish (struct bgp *bgp_vrf)
{
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
...
static void
bgp_evpn_nh_update_ref_pi (struct bgp_evpn_nh *nh)
{
  struct listnode *node;
  struct bgp_path_info *pi;
  struct bgp_path_evpn_nh_info *nh_info;
...
static void
bgp_evpn_nh_clear_ref_pi (struct bgp_evpn_nh *nh, struct bgp_path_info *pi)
{
  if (nh->ref_pi != pi)
    return;

  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    zlog_debug ("evpn vrf %s nh %s ref_pi clear", nh->bgp_vrf->name_pretty,
                nh->nh_str);
  nh->ref_pi = NULL;
...
static void
bgp_evpn_path_nh_info_free (struct bgp_path_evpn_nh_info *nh_info)
{
...
bgp_evpn_path_nh_info_new (struct bgp_path_info *pi)
{
  ...
  if (! nh_info)
    {
      mh_info->nh_info = nh_info = XCALLOC (
          MTYPE_BGP_EVPN_PATH_NH_INFO, sizeof (struct bgp_path_evpn_nh_info));
      nh_info->pi = pi;
    }
...
static void
bgp_evpn_path_nh_unlink (struct bgp_path_evpn_nh_info *nh_info)
{
...
static void
bgp_evpn_path_nh_link (struct bgp *bgp_vrf, struct bgp_path_info *pi)
{
  struct bgp_path_evpn_nh_info *nh_info;
  ...
  if (! bgp_vrf->evpn_nh_table)
    {
      if (BGP_DEBUG (evpn_mh, EVPN_MH_RT))
        zlog_debug ("path %pFX linked to vrf %s failed", &pi->net->p,
                    bgp_vrf->name_pretty);
      return;
    }

  nh_info =
      (pi->extra && pi->extra->mh_info) ? pi->extra->mh_info->nh_info : NULL;
  ...
  if (BGP_DEBUG (evpn_mh, EVPN_MH_ES))
    {
      if (! nh->ref_pi)
        zlog_debug ("path %pFX linked to nh %s %s with no valid pi",
                    &pi->net->p, nh->bgp_vrf->name_pretty, nh->nh_str);
    }
}

void
bgp_evpn_path_nh_del (struct bgp *bgp_vrf, struct bgp_path_info *pi)
{
  struct bgp_path_evpn_nh_info *nh_info;
...
void
bgp_evpn_path_nh_add (struct bgp *bgp_vrf, struct bgp_path_info *pi)
{
  bgp_evpn_path_nh_link (bgp_vrf, pi);
}

static void
bgp_evpn_nh_show_entry (struct bgp_evpn_nh *nh, struct vty *vty,
                        json_object *json_array)
{
  ...
  if (json)
    ...
    {
      vty_out (vty, "%-15s %-15s %-17s %-10d %s\n", nh->bgp_vrf->name_pretty,
               nh->nh_str, mac_buf, listcount (nh->pi_list), prefix_buf);
    }
...
static void
bgp_evpn_nh_show_hash_cb (struct hash_bucket *bucket, void *ctxt)
{
  struct bgp_evpn_nh *nh = (struct bgp_evpn_nh *) bucket->data;
...
void
bgp_evpn_nh_show (struct vty *vty, bool uj)
{
  json_object *json_array = NULL;
  struct bgp *bgp_vrf;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (bm->bgp, node, bgp_vrf))
    {
      hash_iterate (
          ...
          (void (*) (struct hash_bucket *, void *)) bgp_evpn_nh_show_hash_cb,
          &wctx);
    }
...
void
bgp_evpn_switch_ead_evi_rx (void)
{
  struct bgp *bgp;
  struct bgp_evpn_es *es;
  struct bgp_evpn_es_evi *es_evi;
