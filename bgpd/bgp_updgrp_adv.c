 ...
 ********************/
static int
bgp_adj_out_compare (const struct bgp_adj_out *o1,
...
adj_lookup (struct bgp_dest *dest, struct update_subgroup *subgrp,
...
static void
subgrp_withdraw_stale_addpath (struct updwalk_context *ctx,
  ...
  RB_FOREACH_SAFE (adj, bgp_adj_out_rb, &ctx->dest->adj_out, adj_next)
    {
      ...
      for (pi = bgp_dest_get_bgp_path_info (ctx->dest); pi; pi = pi->next)
        {
          id = bgp_addpath_id_for_peer (peer, afi, safi, &pi->tx_addpath);
      ...
      if (! pi)
        {
          subgroup_process_announce_selected (subgrp, NULL, ctx->dest, afi,
                                              safi, adj->addpath_tx_id);
        }
    }
}
...
group_announce_route_walkcb (struct update_group *updgrp, void *arg)
{
  ...
  if (BGP_DEBUG (update, UPDATE_OUT))
    zlog_debug ("%s: afi=%s, safi=%s, p=%pRN", __func__, afi2str (afi),
      ...
      RB_FOREACH (adj, bgp_adj_out_rb, &dest->adj_out)
        {
          ...
          if ((flags & UPDWALK_FLAGS_ADVQUEUE) && adj->adv && adj->adv->baa)
            {
              route_vty_out_tmp (vty, dest, dest_p, adj->adv->baa->attr,
...
static void
updgrp_show_adj (struct bgp *bgp, afi_t afi, safi_t safi, struct vty *vty,
...
static void
subgroup_coalesce_timer (struct event *thread)
{
  struct update_subgroup *subgrp;
  struct bgp *bgp;
 ...
 */
struct bgp_adj_out *
bgp_adj_out_alloc (struct update_subgroup *subgrp, struct bgp_dest *dest,
                   uint32_t addpath_tx_id)
{
  struct bgp_adj_out *adj;
...
void
bgp_adj_out_set_subgroup (struct bgp_dest *dest,
      ...
      if (BGP_DEBUG (update, UPDATE_OUT))
        {
          ...
          zlog_debug ("%s suppress UPDATE w/ attr: %s", peer->host, attr_str);
        }
 ...
 */
void
bgp_adj_out_unset_subgroup (struct bgp_dest *dest,
      ...
      if (adj->attr && withdraw)
        {
          ...
          trigger_write = ! bgp_adv_fifo_count (&subgrp->sync->withdraw);
...
void
bgp_adj_out_remove_subgroup (struct bgp_dest *dest, struct bgp_adj_out *adj,
                             struct update_subgroup *subgrp)
{
  if (adj->attr)
      ...
      for (ri = bgp_dest_get_bgp_path_info (dest); ri; ri = ri->next)
        {
          ...
          subgroup_process_announce_selected (
              ...
              bgp_addpath_id_for_peer (peer, afi, safi_rib, &ri->tx_addpath));
        }
    }
...
subgroup_announce_route (struct update_subgroup *subgrp)
{
  ...
  if (onlypeer &&
      CHECK_FLAG (
          onlypeer->af_sflags[SUBGRP_AFI (subgrp)][SUBGRP_SAFI (subgrp)],
          PEER_STATUS_ORF_WAIT_REFRESH))
    return;

  if (SUBGRP_SAFI (subgrp) != SAFI_MPLS_VPN &&
      SUBGRP_SAFI (subgrp) != SAFI_ENCAP && SUBGRP_SAFI (subgrp) != SAFI_EVPN)
          ...
          for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
            {
              ...
              new_ret = route_map_apply_ext (peer->default_rmap[afi][safi].map,
                  ...
                  if (new_pref < pref)
                    {
                      ...
                      new_attr = bgp_attr_intern (tmp_pi.attr);
                      bgp_attr_flush (tmp_pi.attr);
                    }
                  subgroup_announce_reset_nhop (
                      (peer_cap_enhe (peer, afi, safi) ? AF_INET6 : AF_INET),
                      new_attr);
                  ret = new_ret;
                }
              else
          ...
          for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
            {
              if (CHECK_FLAG (pi->flags, BGP_PATH_SELECTED))
                if (subgroup_announce_check (dest, pi, subgrp,
                                             ...
                                             NULL))
                  {
                    struct attr *default_attr = bgp_attr_intern (&attr);

                    bgp_adj_out_set_subgroup (dest, subgrp, default_attr, pi);
                  }
            }
...
subgroup_announce_all (struct update_subgroup *subgrp)
{
  ...
  if (! subgrp->t_coalesce)
    {
      event_add_timer_msec (bm->master, subgroup_coalesce_timer, subgrp,
