...
static int64_t
bgp_alist_new_seq_get (struct as_list *list)
{
  int64_t maxseq;
  int64_t newseq;
...
bgp_aslist_seq_check (struct as_list *list, int64_t seq)
{
  struct as_filter *entry;
...
static void
as_filter_entry_replace (struct as_list *list, struct as_filter *replace,
                         struct as_filter *entry)
{
  if (replace->next)
    {
...
bool
config_bgp_aspath_validate (const char *regstr)
{
       ...
       "A regular-expression (1234567890_^|[,{}() ]$*+.?-\\) to match the BGP AS paths\n")
{
  ...
  type =
      argv_find (argv, argc, "deny", &idx) ? AS_FILTER_DENY : AS_FILTER_PERMIT;
...
static void
as_list_show (struct vty *vty, struct as_list *aslist, json_object *json)
{
  struct as_filter *asfilter;
      ...
      if (json)
        {
          ...
          json_object_int_add (json_asfilter, "sequenceNumber", asfilter->seq);
          json_object_string_add (json_asfilter, "type",
                                  filter_type_str (asfilter->type));
          json_object_string_add (json_asfilter, "regExp", asfilter->reg_str);
...
static void
as_list_show_all (struct vty *vty, json_object *json)
{
  struct as_list *aslist;
...
config_write_as_list (struct vty *vty)
{
  ...
  for (aslist = as_list_master.str.head; aslist; aslist = aslist->next)
    for (asfilter = aslist->head; asfilter; asfilter = asfilter->next)
      {
        vty_out (vty, "bgp as-path access-list %s seq %" PRId64 " %s %s\n",
                 aslist->name, asfilter->seq, filter_type_str (asfilter->type),
                 asfilter->reg_str);
        write++;
      }
  return write;
...
static void
bgp_aspath_filter_cmd_completion (vector comps, struct cmd_token *token)
{
  struct as_list *aslist;
