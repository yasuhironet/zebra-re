  ...
  if (interval > 0)
    {
      ...
      event_add_timer (bm->master, bgp_dump_interval_func, bgp_dump, interval,
                       &bgp_dump->t_interval);
    }
  else
    {
                            ...
                            struct bgp_path_info *path, unsigned int seq)
{
  ...
  if (afi == AFI_IP && addpath_capable)
    bgp_dump_header (obuf, MSG_TABLE_DUMP_V2,
                     TABLE_DUMP_V2_RIB_IPV4_UNICAST_ADDPATH, BGP_DUMP_ROUTES);
  else if (afi == AFI_IP)
    bgp_dump_header (obuf, MSG_TABLE_DUMP_V2, TABLE_DUMP_V2_RIB_IPV4_UNICAST,
                     BGP_DUMP_ROUTES);
  else if (afi == AFI_IP6 && addpath_capable)
    bgp_dump_header (obuf, MSG_TABLE_DUMP_V2,
                     TABLE_DUMP_V2_RIB_IPV6_UNICAST_ADDPATH, BGP_DUMP_ROUTES);
  else if (afi == AFI_IP6)
  ...
  if (afi == AFI_IP)
    {
       ...
       * align on 8 bits */
      stream_write (obuf, (uint8_t *) &p->u.prefix4, (p->prefixlen + 7) / 8);
    }
  else if (afi == AFI_IP6)
    {
      /* We'll dump only the useful bits (those not 0), but have to
       * align on 8 bits */
      stream_write (obuf, (uint8_t *) &p->u.prefix6, (p->prefixlen + 7) / 8);
    }
...
static void
bgp_dump_interval_func (struct event *t)
{
...
int
bgp_dump_state (struct peer *peer)
{
  struct stream *obuf;
                      ...
                      struct stream *packet)
{
  ...
  if (peer->su.sa.sa_family == AF_INET)
    {
      addpath_capable = bgp_addpath_encode_rx (peer, AFI_IP, SAFI_UNICAST);
    }
  else if (peer->su.sa.sa_family == AF_INET6)
    {
      addpath_capable = bgp_addpath_encode_rx (peer, AFI_IP6, SAFI_UNICAST);
    }
  ...
  if (CHECK_FLAG (peer->cap, PEER_CAP_AS4_RCV) && addpath_capable)
    {
      bgp_dump_header (obuf, MSG_PROTOCOL_BGP4MP, BGP4MP_MESSAGE_AS4_ADDPATH,
                       bgp_dump->type);
    }
    ...
    {
      bgp_dump_header (obuf, MSG_PROTOCOL_BGP4MP, BGP4MP_MESSAGE_ADDPATH,
                       bgp_dump->type);
    }
  else
    {
...
static int
bgp_dump_packet (struct peer *peer, uint8_t type, bgp_size_t size,
                 struct stream *packet)
{
  /* bgp_dump_all. */
  ...
  if (bgp_dump_all.filename)
    {
      ...
      if (bgp_dump_all.interval_str)
        vty_out (vty, "dump bgp %s %s %s\n", type_str, bgp_dump_all.filename,
  ...
  if (bgp_dump_updates.filename)
    {
      ...
      if (bgp_dump_updates.interval_str)
        ...
        vty_out (vty, "dump bgp %s %s\n", type_str, bgp_dump_updates.filename);
    }
