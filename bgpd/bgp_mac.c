...
static unsigned int
bgp_mac_hash_key_make (const void *data)
{
...
static bool
bgp_mac_hash_cmp (const void *d1, const void *d2)
{
...
{
  bm->self_mac_hash =
      hash_create (bgp_mac_hash_key_make, bgp_mac_hash_cmp, "BGP MAC Hash");
}

static void
bgp_mac_hash_free (void *data)
{
...
static void
bgp_mac_hash_interface_string_del (void *val)
{
  char *data = val;
...
static void
bgp_mac_find_ifp_internal (struct hash_bucket *bucket, void *arg)
{
...
static void
bgp_process_mac_rescan_table (struct bgp *bgp, struct peer *peer,
                              struct bgp_table *table, struct ethaddr *macaddr)
{
  ...
  for (pdest = bgp_table_top (table); pdest; pdest = bgp_route_next (pdest))
    {
      ...
      for (dest = bgp_table_top (sub); dest; dest = bgp_route_next (dest))
        {
          bool dest_affected;
          ...
          if (pevpn->family == AF_EVPN &&
              ...
              memcmp (&p->u.prefix_evpn.macip_addr.mac, macaddr, ETH_ALEN) ==
                  0)
            dest_affected = true;
          else
            dest_affected = false;
              ...
              if (bgp_debug_update (peer, p, NULL, 1))
                {
                  ...
                  bgp_debug_rdpfxpath2str (
                      AFI_L2VPN, SAFI_EVPN, &prd, p, label_pnt, num_labels,
                      pi->addpath_rx_id ? 1 : 0, pi->addpath_rx_id, NULL,
          ...
          bgp_update (peer, p, pi->addpath_rx_id, pi->attr, AFI_L2VPN,
                      SAFI_EVPN, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd,
...
static void
bgp_mac_rescan_evpn_table (struct bgp *bgp, struct ethaddr *macaddr)
{
  struct listnode *node;
  struct peer *peer;
  safi_t safi;
  ...
  for (ALL_LIST_ELEMENTS_RO (bgp->peer, node, peer))
    {
      ...
      if (bgp_debug_update (peer, NULL, NULL, 1))
        zlog_debug (
            "Processing EVPN MAC interface change on peer %s %s", peer->host,
...
static void
bgp_mac_rescan_all_evpn_tables (struct ethaddr *macaddr)
{
  struct listnode *node;
  struct bgp *bgp;
...
static void
bgp_mac_remove_ifp_internal (struct bgp_self_mac *bsm, char *ifname,
                             struct ethaddr *macaddr)
{
...
void
bgp_mac_add_mac_entry (struct interface *ifp)
{
  ...
  if (bsm->ifp_list->count == 0)
    {
      ...
      if (old_bsm)
        bgp_mac_remove_ifp_internal (old_bsm, ifname, &old_bsm->macaddr);
    }
  else
    {
      /*
      ...
      if (old_bsm)
        bgp_mac_remove_ifp_internal (old_bsm, ifp->name, &old_bsm->macaddr);
...
void
bgp_mac_del_mac_entry (struct interface *ifp)
{
  struct bgp_self_mac lookup;
  struct bgp_self_mac *bsm;
 ...
 */
bool
bgp_mac_exist (const struct ethaddr *mac)
{
  struct bgp_self_mac lookup;
  struct bgp_self_mac *bsm;
 ...
 */
bool
bgp_mac_entry_exists (const struct prefix *p)
{
...
static void
bgp_mac_show_mac_entry (struct hash_bucket *bucket, void *arg)
{
  struct vty *vty = arg;
  struct bgp_self_mac *bsm = bucket->data;
  struct listnode *node;
...
void
bgp_mac_dump_table (struct vty *vty)
{
