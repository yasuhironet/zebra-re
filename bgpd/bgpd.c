...
static int
bgp_check_main_socket (bool create, struct bgp *bgp)
{
...
void
bgp_option_norib_set_runtime (void)
{
  struct bgp *bgp;
  struct listnode *node;
  afi_t afi;
...
void
bgp_option_norib_unset_runtime (void)
{
  struct bgp *bgp;
  struct listnode *node;
  afi_t afi;
 ...
 */
static int
bgp_router_id_set (struct bgp *bgp, const struct in_addr *id, bool is_config)
{
  struct peer *peer;
              ...
              if (bgp->established_peers == 0)
                {
                  if (BGP_DEBUG (zebra, ZEBRA))
                    zlog_debug ("RID change : vrf %s(%u), RTR ID %pI4",
                                bgp->name_pretty, bgp->vrf_id, addr);
                  /*
                  ...
                  bgp_router_id_set (
                      bgp, addr,
                      (bgp->router_id.s_addr == INADDR_ANY) ? true : false);
                }
            }
        }
    }
...
bm_wait_for_fib_set (bool set)
{
  ...
  if (send_msg && zclient)
    zebra_route_notify_send (ZEBRA_ROUTE_NOTIFY_REQUEST, zclient, set);
}

/* Set the suppress fib pending for the bgp configuration */
void
bgp_suppress_fib_pending_set (struct bgp *bgp, bool set)
{
  bool send_msg = false;
...
void
bgp_timers_set (struct bgp *bgp, uint32_t keepalive, uint32_t holdtime,
...
void
bgp_tcp_keepalive_set (struct bgp *bgp, uint16_t keepalive_idle,
...
void
bgp_tcp_keepalive_unset (struct bgp *bgp)
{
...
void
bgp_confederation_id_set (struct bgp *bgp, as_t as, const char *as_str)
{
  struct peer *peer;
  struct listnode *node, *nnode;
  int already_confed;
...
void
bgp_confederation_peers_add (struct bgp *bgp, as_t as, const char *as_str)
{
  struct peer *peer;
  struct listnode *node, *nnode;
  ...
  bgp->confed_peers =
      XREALLOC (MTYPE_BGP_CONFED_LIST, bgp->confed_peers,
                (bgp->confed_peers_cnt + 1) * sizeof (struct as_confed));
    ...
    if (bgp->confed_peers[i].as == as)
      {
        ...
        for (j = i + 1; j < bgp->confed_peers_cnt; j++)
          {
            bgp->confed_peers[j - 1].as = bgp->confed_peers[j].as;
...
void
bgp_default_local_preference_set (struct bgp *bgp, uint32_t local_pref)
{
  if (! bgp)
    return;
...
/* Local preference configuration.  */
void
bgp_default_subgroup_pkt_queue_max_set (struct bgp *bgp, uint32_t queue_size)
{
  if (! bgp)
    return;
...
int
bgp_map_afi_safi_iana2int (iana_afi_t pkt_afi, iana_safi_t pkt_safi,
...
int
bgp_map_afi_safi_int2iana (afi_t afi, safi_t safi, iana_afi_t *pkt_afi,
  ...
  if (PAF_SUBGRP (af))
    {
      if (BGP_DEBUG (update_groups, UPDATE_GROUPS))
        zlog_debug ("u%" PRIu64 ":s%" PRIu64 " remove peer %s",
                    af->subgroup->update_group->id, af->subgroup->id,
                    peer->host);
    }
...
static unsigned int
peer_hash_key_make (const void *p)
{
...
static bool
peer_hash_same (const void *p1, const void *p2)
{
...
void
peer_flag_inherit (struct peer *peer, uint64_t flag)
{
  bool group_val;
...
bool
peer_af_flag_check (struct peer *peer, afi_t afi, safi_t safi, uint64_t flag)
{
  return ! ! CHECK_FLAG (peer->af_flags[afi][safi], flag);
}

void
peer_af_flag_inherit (struct peer *peer, afi_t afi, safi_t safi, uint64_t flag)
{
  bool group_val;
...
static inline enum bgp_peer_sort
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      if (peer->as_type == AS_INTERNAL)
        ...
        {
          assert (bgp);
          return (local_as == peer->as ? BGP_PEER_IBGP : BGP_PEER_EBGP);
        }
      ...
      if (peer->as_type == AS_UNSPECIFIED)
        {
          ...
          if (peer->group && (peer->group->conf->as_type != AS_UNSPECIFIED))
            {
              if (peer->group->conf->as_type == AS_SPECIFIED)
                {
              ...
              else if (peer->group->conf->as_type == AS_INTERNAL)
                return BGP_PEER_IBGP;
              else
                return BGP_PEER_EBGP;
            }
...
enum bgp_peer_sort
peer_sort_lookup (struct peer *peer)
{
  return peer->sort;
}
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      if (peer->filter[afi][safi].advmap.aname)
        XFREE (MTYPE_BGP_FILTER_NAME, peer->filter[afi][safi].advmap.aname);
      if (peer->filter[afi][safi].advmap.cname)
        XFREE (MTYPE_BGP_FILTER_NAME, peer->filter[afi][safi].advmap.cname);
    }
...
int
bgp_global_gr_init (struct bgp *bgp)
{
...
int
bgp_peer_gr_init (struct peer *peer)
{
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("%s called ..", __func__);
...
static void
bgp_srv6_cleanup (struct bgp *bgp)
{
...
peer_new (struct bgp *bgp)
{
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      ...
      SET_FLAG (peer->af_flags_invert[afi][safi], PEER_FLAG_SEND_COMMUNITY);
  ...
  SET_FLAG (peer->af_flags[AFI_L2VPN][SAFI_EVPN], PEER_FLAG_NEXTHOP_UNCHANGED);
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      ...
      peer_dst->addpath_type[afi][safi] = peer_src->addpath_type[afi][safi];
    }
...
bgp_peer_conf_if_to_su_update_v6 (struct peer *peer, struct interface *ifp)
{
  ...
  if (ifp->nbr_connected && (ifc_nbr = listnode_head (ifp->nbr_connected)))
    {
...
void
bgp_recalculate_afi_safi_bestpaths (struct bgp *bgp, afi_t afi, safi_t safi)
{
...
peer_create (union sockunion *su, const char *conf_if, struct bgp *bgp,
             as_t local_as, as_t remote_as, int as_type,
 ...
 */
bool
bgp_afi_safi_peer_exists (struct bgp *bgp, afi_t afi, safi_t safi)
{
  struct listnode *node;
  struct peer *peer;
                ...
                const char *as_str)
{
  ...
  if (bgp_config_check (peer->bgp, BGP_CONFIG_CONFEDERATION) &&
      ! bgp_confederation_peers_check (peer->bgp, as) && peer->bgp->as != as)
                ...
                as_t *as, int as_type, const char *as_str)
{
  ...
  if (peer)
      ...
      if (peer->group)
        {
          ...
          if (peer->group->conf->as || peer->group->conf->as_type)
            {
          ...
          enum bgp_peer_sort peer_sort_type = peer_sort (peer->group->conf);

          /* Explicit AS numbers used, compare AS numbers */
          if (as_type == AS_SPECIFIED)
            {
              if (((peer_sort_type == BGP_PEER_IBGP) && (bgp->as != *as)) ||
                  ((peer_sort_type == BGP_PEER_EBGP) && (bgp->as == *as)))
                {
                  *as = peer->as;
  ...
  else
    {
      ...
      if (bgp_config_check (bgp, BGP_CONFIG_CONFEDERATION) &&
          ! bgp_confederation_peers_check (bgp, *as) && *as && bgp->as != *as)
        local_as = bgp->confed_id;
      else
        local_as = bgp->as;

      peer_create (su, conf_if, bgp, local_as, *as, as_type, NULL, true,
                   as_str);
    }
...
const char *
bgp_get_name_by_role (uint8_t role)
{
  switch (role)
    {
    case ROLE_PROVIDER:
...
bgp_get_asnotation (struct bgp *bgp)
{
  if (! bgp)
                                ...
                                afi_t afi, safi_t safi)
{
  ...
  flags_tmp ^=
      conf->af_flags_invert[afi][safi] ^ peer->af_flags_invert[afi][safi];
  ...
  if (! CHECK_FLAG (pfilter_ovrd[in], PEER_FT_DISTRIBUTE_LIST))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].dlist[in].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].dlist[in].alist);
    }

  if (! CHECK_FLAG (pfilter_ovrd[in], PEER_FT_PREFIX_LIST))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].plist[in].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].plist[in].plist);
    }

  if (! CHECK_FLAG (pfilter_ovrd[in], PEER_FT_FILTER_LIST))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].aslist[in].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].aslist[in].aslist);
    }

  if (! CHECK_FLAG (pfilter_ovrd[RMAP_IN], PEER_FT_ROUTE_MAP))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].map[in].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].map[RMAP_IN].map);
    }

  /* outbound filter apply */
  if (! CHECK_FLAG (pfilter_ovrd[out], PEER_FT_DISTRIBUTE_LIST))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].dlist[out].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].dlist[out].alist);
    }

  if (! CHECK_FLAG (pfilter_ovrd[out], PEER_FT_PREFIX_LIST))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].plist[out].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].plist[out].plist);
    }

  if (! CHECK_FLAG (pfilter_ovrd[out], PEER_FT_FILTER_LIST))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].aslist[out].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].aslist[out].aslist);
    }

  if (! CHECK_FLAG (pfilter_ovrd[RMAP_OUT], PEER_FT_ROUTE_MAP))
    {
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].map[RMAP_OUT].name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, group, filter[afi][safi].map[RMAP_OUT].map);
    }
  ...
  if (! CHECK_FLAG (pfilter_ovrd[RMAP_OUT], PEER_FT_ADVERTISE_MAP))
    {
      ...
      PEER_STR_ATTR_INHERIT (peer, group, filter[afi][safi].advmap.cname,
                             MTYPE_BGP_FILTER_NAME);
...
static int
peer_activate_af (struct peer *peer, afi_t afi, safi_t safi)
{
  int active;
  struct peer *other;
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      flog_err (EC_BGP_PEER_GROUP, "%s was called for peer-group %s", __func__,
                peer->host);
      return 1;
    }
  ...
  if (! active && peer_active (peer))
  ...
  else
    {
      ...
      if (other && (other->status == OpenSent || other->status == OpenConfirm))
        {
  ...
  if (peer_af_delete (peer, afi, safi) != 0)
    {
      flog_err (EC_BGP_PEER_DELETE,
                "couldn't delete af structure for peer %s(%s, %s)", peer->host,
  ...
  if (peer->t_gr_stale)
    {
      ...
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%pBP graceful restart stalepath timer stopped", peer);
    }
...
peer_group2peer_config_copy (struct peer_group *group, struct peer *peer)
{
  ...
  if (! CHECK_FLAG (peer->flags_override, PEER_FLAG_CAPABILITY_SOFT_VERSION))
  ...
  if (! CHECK_FLAG (peer->flags_override, PEER_FLAG_PASSWORD))
    PEER_STR_ATTR_INHERIT (peer, group, password, MTYPE_PEER_PASSWORD);
...
void
peer_notify_unconfig (struct peer *peer)
{
  if (BGP_IS_VALID_STATE_FOR_NOTIF (peer->status))
...
static void
peer_notify_shutdown (struct peer *peer)
{
...
void
peer_group_notify_unconfig (struct peer_group *group)
{
      ...
      if (first_member)
        {
          gtype = peer_sort (group->conf);
          /* Advertisement-interval reset */
          if (! CHECK_FLAG (group->conf->flags, PEER_FLAG_ROUTEADV))
            {
              group->conf->v_routeadv = (gtype == BGP_PEER_IBGP)
                                            ? BGP_DEFAULT_IBGP_ROUTEADV
                                            : BGP_DEFAULT_EBGP_ROUTEADV;
            }
...
static void
bgp_startup_timer_expire (struct event *thread)
{
  struct bgp *bgp;
 ...
 */
static void
bgp_vrf_string_name_delete (void *data)
{
  char *vname = data;
            ...
            const char *as_pretty, enum asnotation_mode asnotation)
{
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    {
      if (inst_type == BGP_INSTANCE_TYPE_DEFAULT)
        zlog_debug ("Creating Default VRF, AS %s", bgp->as_pretty);
      else
  ...
  bgp->vrf_id =
      (inst_type == BGP_INSTANCE_TYPE_DEFAULT) ? VRF_DEFAULT : VRF_UNKNOWN;
  ...
  bgp->peerhash =
      hash_create (peer_hash_key_make, peer_hash_same, "BGP Peer Hash");
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      ...
      bgp->vpn_policy[afi].tovpn_zebra_vrf_label_last_sent = MPLS_LABEL_NONE;

      bgp->vpn_policy[afi].import_vrf = list_new ();
      bgp->vpn_policy[afi].import_vrf->del = bgp_vrf_string_name_delete;
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    bgp_label_per_nexthop_cache_init (&bgp->mpls_labels_per_nexthop[afi]);
  ...
  if (inst_type == BGP_INSTANCE_TYPE_DEFAULT)
  ...
  else
    {
      ...
      snprintf (bgp->name_pretty, len, "%s %s",
                (bgp->inst_type == BGP_INSTANCE_TYPE_VRF) ? "VRF" : "VIEW", n);
    }
 ...
 */
int
bgp_handle_socket (struct bgp *bgp, struct vrf *vrf, vrf_id_t old_vrf_id,
                   bool create)
{
  struct listnode *node;
  char *address;
...
int
bgp_lookup_by_as_name_type (struct bgp **bgp_val, as_t *as, const char *name,
         ...
         enum asnotation_mode asnotation)
{
  ...
  *bgp_val = bgp;
  ...
  if (IS_BGP_INST_KNOWN_TO_ZEBRA (bgp))
    {
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: Registering BGP instance %s to zebra", __func__,
                    bgp->name_pretty);
...
static void
bgp_zclient_set_redist (afi_t afi, int type, unsigned short instance,
...
static void
bgp_set_redist_vrf_bitmaps (struct bgp *bgp, bool set)
{
  afi_t afi;
  int i;
      ...
      for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (red_list, node, red))
            bgp_zclient_set_redist (afi, i, red->instance, bgp->vrf_id, set);
        }
    }
}
...
bgp_delete (struct bgp *bgp)
{
  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      ...
      for (struct bgp_dest *dest = bgp_table_top (bgp->aggregate[afi][safi]);
  ...
  for (afi = 0; afi < AFI_MAX; ++afi)
    {
      for (enum vpn_policy_direction dir = 0; dir < BGP_VPN_POLICY_DIR_MAX;
           ++dir)
        {
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      ...
      if (bgp->vpn_policy[afi].tovpn_sid_locator != NULL)
        srv6_locator_chunk_free (&bgp->vpn_policy[afi].tovpn_sid_locator);
      ...
      if (bgp->vpn_policy[afi].tovpn_sid != NULL)
        {
          ...
          XFREE (MTYPE_BGP_SRV6_SID, bgp->vpn_policy[afi].tovpn_sid);
        }
    }
...
peer_lookup_dynamic_neighbor (struct bgp *bgp, union sockunion *su)
{
  ...
  if (bgp_debug_neighbor_events (NULL))
    zlog_debug ("Dynamic Neighbor %pFX matches group %s listen range %pFX",
  ...
  if (dncount >= gbgp->dynamic_neighbors_limit)
    {
      if (bgp_debug_neighbor_events (NULL))
        zlog_debug ("Dynamic Neighbor %pFX rejected - at limit %d", &prefix,
  ...
  if (CHECK_FLAG (group->conf->flags, PEER_FLAG_SHUTDOWN))
    {
      if (bgp_debug_neighbor_events (NULL))
        zlog_debug ("Dynamic Neighbor %pFX rejected - group %s disabled",
                    &prefix, group->name);
      return NULL;
    }
  ...
  if (bgp_debug_neighbor_events (peer))
    zlog_debug ("%s Dynamic Neighbor added, group %s count %d", peer->host,
                group->name, dncount);
...
static void
peer_drop_dynamic_neighbor (struct peer *peer)
{
  int dncount = -1;
  if (peer->group->bgp)
    {
...
bool
bgp_path_attribute_discard (struct peer *peer, char *buf, size_t size)
{
  if (! buf)
    return false;
...
bool
bgp_path_attribute_treat_as_withdraw (struct peer *peer, char *buf,
                                      size_t size)
{
  if (! buf)
    return false;
...
peer_active (struct peer *peer)
{
  ...
  if (peer->afc[AFI_IP][SAFI_UNICAST] || peer->afc[AFI_IP][SAFI_MULTICAST] ||
      ...
      peer->afc[AFI_IP][SAFI_FLOWSPEC] || peer->afc[AFI_IP6][SAFI_UNICAST] ||
...
bool
peer_afc_received (struct peer *peer)
{
  afi_t afi;
  safi_t safi;
...
bool
peer_afc_advertised (struct peer *peer)
{
  afi_t afi;
  safi_t safi;
...
peer_flag_action_set (const struct peer_flag_action *action_list, int size,
                      struct peer_flag_action *action, uint64_t flag)
{
  int i;
  int found = 0;
  int reset_in = 0;
...
static void
peer_flag_modify_action (struct peer *peer, uint64_t flag)
{
          ...
          if (peer->t_pmax_restart)
            {
              ...
              if (bgp_debug_neighbor_events (peer))
                zlog_debug ("%pBP Maximum-prefix restart timer canceled",
                            peer);
            }
...
void
bgp_shutdown_enable (struct bgp *bgp, const char *msg)
{
  struct peer *peer;
  struct listnode *node;
  /* length(1) + message(N) */
  ...
  zlog_info ("Enabled administrative shutdown on BGP instance AS %u", bgp->as);
...
void
bgp_shutdown_disable (struct bgp *bgp)
{
...
static int
peer_flag_modify (struct peer *peer, uint64_t flag, int set)
{
  int found;
  int size;
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      member_invert = CHECK_FLAG (member->flags_invert, flag) && ! invert;
...
int
peer_flag_set (struct peer *peer, uint64_t flag)
{
  return peer_flag_modify (peer, flag, 1);
}

int
peer_flag_unset (struct peer *peer, uint64_t flag)
{
  return peer_flag_modify (peer, flag, 0);
}

static int
peer_af_flag_modify (struct peer *peer, afi_t afi, safi_t safi, uint64_t flag,
                     bool set)
{
  int found;
  int size;
  ...
  if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      if (set && CHECK_FLAG (peer->af_flags[afi][safi], flag))
        {
          COND_FLAG (peer->af_flags_override[afi][safi], flag, ! invert);
          return 0;
        }
  ...
  if (flag & PEER_FLAG_RSERVER_CLIENT)
    {
      if (set)
        ...
        UNSET_FLAG (peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED);
    }
  ...
  if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      COND_FLAG (peer->af_flags_override[afi][safi], flag, set != invert);
    }
  else
    {
      /*
      ...
      for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
        {
          /* Skip peers with overridden configuration. */
          if (CHECK_FLAG (member->af_flags_override[afi][safi], flag))
            continue;

          /* Check if only member without group is inverted. */
          member_invert =
              CHECK_FLAG (member->af_flags_invert[afi][safi], flag) &&
              ! invert;
          ...
          /* Update flag on peer-group member. */
          COND_FLAG (member->af_flags[afi][safi], flag, set != member_invert);
          ...
          if (peer_established (member))
            {
              if (! set && flag == PEER_FLAG_SOFT_RECONFIG)
                ...
                {
                  if (flag == PEER_FLAG_REFLECTOR_CLIENT)
                    member->last_reset = PEER_DOWN_RR_CLIENT_CHANGE;
                  else if (flag == PEER_FLAG_RSERVER_CLIENT)
                    member->last_reset = PEER_DOWN_RS_CLIENT_CHANGE;
                  else if (flag == PEER_FLAG_ORF_PREFIX_SM)
                    member->last_reset = PEER_DOWN_CAPABILITY_CHANGE;
                  else if (flag == PEER_FLAG_ORF_PREFIX_RM)
                    member->last_reset = PEER_DOWN_CAPABILITY_CHANGE;

                  peer_change_action (member, afi, safi, action.type);
                }
            }
        }
    }
...
int
peer_af_flag_set (struct peer *peer, afi_t afi, safi_t safi, uint64_t flag)
{
  return peer_af_flag_modify (peer, afi, safi, flag, 1);
}

int
peer_af_flag_unset (struct peer *peer, afi_t afi, safi_t safi, uint64_t flag)
{
...
void
peer_tx_shutdown_message_set (struct peer *peer, const char *msg)
{
  XFREE (MTYPE_PEER_TX_SHUTDOWN_MSG, peer->tx_shutdown_message);
...
void
peer_tx_shutdown_message_unset (struct peer *peer)
{
  XFREE (MTYPE_PEER_TX_SHUTDOWN_MSG, peer->tx_shutdown_message);
}
...
int
peer_role_set (struct peer *peer, uint8_t role, bool strict_mode)
{
  struct peer *member;
  struct listnode *node, *nnode;
      ...
      if (peer->local_role == role)
        {
          if (CHECK_FLAG (peer->flags, PEER_FLAG_ROLE_STRICT_MODE) &&
             ...
             * down?
             */
            UNSET_FLAG (peer->flags, PEER_FLAG_ROLE_STRICT_MODE);
          if (! CHECK_FLAG (peer->flags, PEER_FLAG_ROLE_STRICT_MODE) &&
              strict_mode)
            {
              SET_FLAG (peer->flags, PEER_FLAG_ROLE_STRICT_MODE);
      ...
      if (member->local_role == role)
        {
          if (CHECK_FLAG (member->flags, PEER_FLAG_ROLE_STRICT_MODE) &&
             ...
             * down?
             */
            UNSET_FLAG (member->flags, PEER_FLAG_ROLE_STRICT_MODE);
          if (! CHECK_FLAG (member->flags, PEER_FLAG_ROLE_STRICT_MODE) &&
              strict_mode)
            {
              SET_FLAG (peer->flags, PEER_FLAG_ROLE_STRICT_MODE);
              SET_FLAG (member->flags, PEER_FLAG_ROLE_STRICT_MODE);
              /* Restart session to throw Role Mismatch
               * Notification
               */
...
int
peer_role_unset (struct peer *peer)
{
  struct peer *member;
  struct listnode *node, *nnode;
                            ...
                            const char *rmap, struct route_map *route_map)
{
  ...
  if (rmap)
  ...
  else if (! rmap)
    {
      ...
      if (peer->default_rmap[afi][safi].name)
        {
          map = route_map_lookup_by_name (peer->default_rmap[afi][safi].name);
          XFREE (MTYPE_ROUTE_MAP_NAME, peer->default_rmap[afi][safi].name);
        }
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      /* Set flag and configuration on peer-group member. */
      SET_FLAG (member->af_flags[afi][safi], PEER_FLAG_DEFAULT_ORIGINATE);
      if (rmap)
        {
          struct route_map *map = NULL;
...
peer_default_originate_unset (struct peer *peer, afi_t afi, safi_t safi)
{
  ...
  if (peer_group_active (peer))
    {
      peer_af_flag_inherit (peer, afi, safi, PEER_FLAG_DEFAULT_ORIGINATE);
      PEER_STR_ATTR_INHERIT (peer, peer->group, default_rmap[afi][safi].name,
                             MTYPE_ROUTE_MAP_NAME);
      PEER_ATTR_INHERIT (peer, peer->group, default_rmap[afi][safi].map);
    }
  else
    {
      struct route_map *map = NULL;
      ...
      /* Otherwise remove flag and configuration from peer. */
      peer_af_flag_unset (peer, afi, safi, PEER_FLAG_DEFAULT_ORIGINATE);
      if (peer->default_rmap[afi][safi].name)
        {
          map = route_map_lookup_by_name (peer->default_rmap[afi][safi].name);
          XFREE (MTYPE_ROUTE_MAP_NAME, peer->default_rmap[afi][safi].name);
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      /* Remove flag and configuration on peer-group member. */
      UNSET_FLAG (member->af_flags[afi][safi], PEER_FLAG_DEFAULT_ORIGINATE);
      if (member->default_rmap[afi][safi].name)
        {
          map =
              route_map_lookup_by_name (member->default_rmap[afi][safi].name);
          XFREE (MTYPE_ROUTE_MAP_NAME, member->default_rmap[afi][safi].name);
        }
      route_map_counter_decrement (map);
...
void
peer_port_set (struct peer *peer, uint16_t port)
{
  peer->port = port;
 ...
 */
void
peer_tcp_mss_set (struct peer *peer, uint32_t tcp_mss)
{
 ...
 */
void
peer_tcp_mss_unset (struct peer *peer)
{
...
/* neighbor weight. */
int
peer_weight_set (struct peer *peer, afi_t afi, safi_t safi, uint16_t weight)
{
  struct peer *member;
  struct listnode *node, *nnode;
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      /* Skip peers with overridden configuration. */
      if (CHECK_FLAG (member->af_flags_override[afi][safi], PEER_FLAG_WEIGHT))
        continue;
...
int
peer_timers_set (struct peer *peer, uint32_t keepalive, uint32_t holdtime)
{
  struct peer *member;
  struct listnode *node, *nnode;
...
int
peer_timers_connect_set (struct peer *peer, uint32_t connect)
{
  struct peer *member;
  struct listnode *node, *nnode;
...
int
peer_advertise_interval_set (struct peer *peer, uint32_t routeadv)
{
  struct peer *member;
  struct listnode *node, *nnode;
 ...
 */
int
peer_timers_delayopen_set (struct peer *peer, uint32_t delayopen)
{
  struct peer *member;
  struct listnode *node;
 ...
 */
int
peer_timers_delayopen_unset (struct peer *peer)
{
  struct peer *member;
  struct listnode *node;
...
int
peer_local_as_set (struct peer *peer, as_t as, bool no_prepend,
                   bool replace_as, const char *as_str)
{
  ...
  old_no_prepend = ! ! CHECK_FLAG (peer->flags, PEER_FLAG_LOCAL_AS_NO_PREPEND);
  old_replace_as = ! ! CHECK_FLAG (peer->flags, PEER_FLAG_LOCAL_AS_REPLACE_AS);
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      /* Skip peers with the same configuration. */
      old_no_prepend =
          CHECK_FLAG (member->flags, PEER_FLAG_LOCAL_AS_NO_PREPEND);
      old_replace_as =
          CHECK_FLAG (member->flags, PEER_FLAG_LOCAL_AS_REPLACE_AS);
      if (member->change_local_as == as &&
          CHECK_FLAG (member->flags, PEER_FLAG_LOCAL_AS) &&
          old_no_prepend == no_prepend && old_replace_as == replace_as)
        continue;
      ...
      if (as_str)
        member->change_local_as_pretty = XSTRDUP (MTYPE_BGP, as_str);
    }
  ...
  if (peer_group_active (peer))
    {
      ...
      PEER_STR_ATTR_INHERIT (peer, peer->group, password, MTYPE_PEER_PASSWORD);
    }
  else
    {
      /* Otherwise remove flag and configuration from peer. */
...
peer_distribute_update (struct access_list *access)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
    {
      if (access->name)
        update_group_policy_update (bgp, BGP_POLICY_DISTRIBUTE_LIST,
                                    access->name, true, 0);
      for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
        {
          FOREACH_AFI_SAFI (afi, safi)
                      ...
                      const char *name)
{
  ...
  if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      /* Set override-flag and process peer route updates. */
      SET_FLAG (peer->filter_override[afi][safi][direct], PEER_FT_PREFIX_LIST);
      peer_on_policy_change (peer, afi, safi, (direct == FILTER_OUT) ? 1 : 0);
...
peer_prefix_list_unset (struct peer *peer, afi_t afi, safi_t safi, int direct)
{
  ...
  /* Unset override-flag unconditionally. */
  UNSET_FLAG (peer->filter_override[afi][safi][direct], PEER_FT_PREFIX_LIST);
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
    {
      ...
      update_group_policy_update (bgp, BGP_POLICY_PREFIX_LIST,
                                  plist ? prefix_list_name (plist) : NULL,
                                  true, 0);
                 ...
                 const char *name)
{
  ...
  if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      /* Set override-flag and process peer route updates. */
      SET_FLAG (peer->filter_override[afi][safi][direct], PEER_FT_FILTER_LIST);
      peer_on_policy_change (peer, afi, safi, (direct == FILTER_OUT) ? 1 : 0);
...
peer_aslist_unset (struct peer *peer, afi_t afi, safi_t safi, int direct)
{
  ...
  /* Unset override-flag unconditionally. */
  UNSET_FLAG (peer->filter_override[afi][safi][direct], PEER_FT_FILTER_LIST);
                    ...
                    const char *name, struct route_map *route_map)
{
  ...
  if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      /* Set override-flag and process peer route updates. */
      SET_FLAG (peer->filter_override[afi][safi][direct], PEER_FT_ROUTE_MAP);
      peer_on_policy_change (peer, afi, safi, (direct == RMAP_OUT) ? 1 : 0);
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      /* Process peer route updates. */
      peer_on_policy_change (member, afi, safi, (direct == RMAP_OUT) ? 1 : 0);
    }
  return 0;
}
  ...
  if (peer_group_active (peer))
    {
      PEER_STR_ATTR_INHERIT (peer, peer->group,
                             ...
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, peer->group, filter[afi][safi].map[direct].map);
    }
  else
    {
      struct route_map *map = NULL;
                         ...
                         const char *name, struct route_map *route_map)
{
  ...
  if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
    {
      /* Set override-flag and process peer route updates. */
      SET_FLAG (peer->filter_override[afi][safi][0], PEER_FT_UNSUPPRESS_MAP);
...
peer_unsuppress_map_unset (struct peer *peer, afi_t afi, safi_t safi)
{
  ...
  if (peer_group_active (peer))
    {
      PEER_STR_ATTR_INHERIT (peer, peer->group, filter[afi][safi].usmap.name,
                             MTYPE_BGP_FILTER_NAME);
      PEER_ATTR_INHERIT (peer, peer->group, filter[afi][safi].usmap.map);
    }
  else
    {
      struct route_map *map = NULL;
...
static bool
peer_maximum_prefix_clear_overflow (struct peer *peer)
{
  ...
  if (peer->t_pmax_restart)
    {
      EVENT_OFF (peer->t_pmax_restart);
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%pBP Maximum-prefix restart timer cancelled", peer);
    }
  BGP_EVENT_ADD (peer, BGP_Start);
  return true;
}
  ...
  for (ALL_LIST_ELEMENTS (peer->group->peer, node, nnode, member))
    {
      ...
      if (force)
        SET_FLAG (member->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_FORCE);
      else
        UNSET_FLAG (member->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_FORCE);

      if (warning)
        SET_FLAG (member->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING);
      else
        UNSET_FLAG (member->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING);
  ...
  if (peer_group_active (peer))
    {
      ...
      peer_af_flag_inherit (peer, afi, safi, PEER_FLAG_MAX_PREFIX_FORCE);
      peer_af_flag_inherit (peer, afi, safi, PEER_FLAG_MAX_PREFIX_WARNING);
...
void
peer_maximum_prefix_out_refresh_routes (struct peer *peer, afi_t afi,
                                        safi_t safi)
{
  update_group_adjust_peer (peer_af_find (peer, afi, safi));
...
int
peer_maximum_prefix_out_set (struct peer *peer, afi_t afi, safi_t safi,
                             uint32_t max)
{
  struct peer *member;
  struct listnode *node, *nnode;
...
int
peer_maximum_prefix_out_unset (struct peer *peer, afi_t afi, safi_t safi)
{
  struct peer *member;
  struct listnode *node;
  /* Inherit configuration from peer-group if peer is member. */
  ...
  for (ALL_LIST_ELEMENTS_RO (peer->group->peer, node, member))
    {
      ...
      /* Remove flag and configuration on peer-group member.
       */
      UNSET_FLAG (member->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_OUT);
...
int
is_ebgp_multihop_configured (struct peer *peer)
{
  struct peer_group *group;
  struct listnode *node, *nnode;
  struct peer *peer1;
      ...
      (peer->sort != BGP_PEER_IBGP))
    {
      ...
      if (! CHECK_FLAG (peer->sflags, PEER_STATUS_GROUP))
      ...
      else
        {
          ...
          for (ALL_LIST_ELEMENTS (group->peer, node, nnode, gpeer))
            {
          ...
          for (ALL_LIST_ELEMENTS (group->peer, node, nnode, gpeer))
            {
               ...
               */
              if (gpeer->fd >= 0 && gpeer->gtsm_hops != BGP_GTSM_HOPS_DISABLED)
                sockopt_minttl (gpeer->su.sa.sa_family, gpeer->fd,
                                MAXTTL + 1 - gpeer->gtsm_hops);
              if ((gpeer->status < Established) && gpeer->doppelganger &&
...
static void
peer_reset_message_stats (struct peer *peer)
{
  if (peer)
    {
      atomic_store_explicit (&peer->open_in, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->open_out, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->update_in, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->update_out, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->keepalive_in, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->keepalive_out, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->notify_in, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->notify_out, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->refresh_in, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->refresh_out, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->dynamic_cap_in, 0, memory_order_relaxed);
      atomic_store_explicit (&peer->dynamic_cap_out, 0, memory_order_relaxed);
    }
}
...
/* Display peer uptime.*/
char *
peer_uptime (time_t uptime2, char *buf, size_t len, bool use_json,
             json_object *json)
{
  ...
  if (uptime1 < ONE_DAY_SECOND)
    ...
    snprintf (buf, len, "%02dy%02dw%dd", tm.tm_year - 70, tm.tm_yday / 7,
...
void
bgp_master_init (struct event_loop *master, const int buffer_size,
...
static void
bgp_instasn_autocomplete (vector comps, struct cmd_token *token)
{
...
void
bgp_pthreads_finish (void)
{
  frr_pthread_stop_all ();
}

static int
peer_unshut_after_cfg (struct bgp *bgp)
{
  struct listnode *node;
  struct peer *peer;
  ...
  for (ALL_LIST_ELEMENTS_RO (bgp->peer, node, peer))
    {
      ...
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s: released from config-pending hold", peer->host);
...
struct peer *
peer_lookup_in_view (struct vty *vty, struct bgp *bgp, const char *ip_str,
                     bool use_json)
{
  int ret;
  struct peer *peer;
              ...
              if (use_json)
                {
                  ...
                  json_object_string_add (json_no, "malformedAddressOrName",
                                          ip_str);
...
void
bgp_gr_apply_running_config (void)
{
  struct peer *peer = NULL;
  struct bgp *bgp = NULL;
  struct listnode *node, *nnode;
...
static ssize_t
printfrr_bp (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
