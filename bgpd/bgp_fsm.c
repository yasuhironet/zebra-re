...
int
bgp_peer_reg_with_nht (struct peer *peer)
{
  int connected = 0;
  ...
  frr_with_mutex (&peer->io_mtx, &from_peer->io_mtx)
    {
      ...
      while (from_peer->ibuf->head)
        stream_fifo_push (peer->ibuf, stream_fifo_pop (from_peer->ibuf));
      ...
      if (bgp_getsockname (from_peer) < 0)
        {
          flog_err (
              ...
              (CHECK_FLAG (from_peer->sflags, PEER_STATUS_ACCEPT_PEER)
                   ? "accept"
                   : ""),
...
/* BGP start timer.  This function set BGP_Start event to thread value
   and process event. */
static void
bgp_start_timer (struct event *thread)
{
  struct peer *peer;
...
static void
bgp_connect_timer (struct event *thread)
{
  struct peer *peer;
...
/* BGP holdtime timer. */
static void
bgp_holdtime_timer (struct event *thread)
{
...
void
bgp_routeadv_timer (struct event *thread)
{
  struct peer *peer;
...
void
bgp_delayopen_timer (struct event *thread)
{
  struct peer *peer;
  ...
  if (bgp_debug_neighbor_events (peer))
    zlog_debug ("%s [FSM] Timer (DelayOpentimer expire)", peer->host);
...
static void
bgp_graceful_restart_timer_off (struct peer *peer)
{
  afi_t afi;
  safi_t safi;

  FOREACH_AFI_SAFI (afi, safi)
    if (CHECK_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_LLGR_WAIT))
      return;
  ...
  if (peer_dynamic_neighbor (peer) &&
    ...
    {
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s (dynamic neighbor) deleted (%s)", peer->host,
                    __func__);
      peer_delete (peer);
    }
...
static void
bgp_llgr_stale_timer_expire (struct event *thread)
{
  struct peer_af *paf;
  struct peer *peer;
  afi_t afi;
...
static void
bgp_set_llgr_stale (struct peer *peer, afi_t afi, safi_t safi)
{
            ...
            for (pi = bgp_dest_get_bgp_path_info (rm); pi; pi = pi->next)
              {
                ...
                bgp_recalculate_afi_safi_bestpaths (peer->bgp, afi, safi);
...
static void
bgp_graceful_restart_timer_expire (struct event *thread)
{
      ...
      if (peer->llgr[afi][safi].stale_time)
        {
          ...
          if (bgp_debug_neighbor_events (peer))
            zlog_debug ("%pBP Long-lived stale timer (%s) started for %d sec",
                        peer, get_afi_safi_str (afi, safi, false),
                        peer->llgr[afi][safi].stale_time);

          SET_FLAG (peer->af_sflags[afi][safi], PEER_STATUS_LLGR_WAIT);

          bgp_set_llgr_stale (peer, afi, safi);
          bgp_clear_stale_route (peer, afi, safi);

          event_add_timer (bm->master, bgp_llgr_stale_timer_expire, paf,
                           peer->llgr[afi][safi].stale_time,
...
static void
bgp_graceful_stale_timer_expire (struct event *thread)
{
  struct peer *peer;
  afi_t afi;
  safi_t safi;
  ...
  if (bgp_debug_neighbor_events (peer))
    zlog_debug ("%pBP graceful restart stalepath timer expired", peer);
...
static void
bgp_graceful_deferral_timer_expire (struct event *thread)
{
...
bgp_maxmed_update (struct bgp *bgp)
{
  ...
  if (bgp->maxmed_active != maxmed_active || bgp->maxmed_value != maxmed_value)
    {
...
int
bgp_fsm_error_subcode (int status)
{
...
static void
bgp_maxmed_onstartup_timer (struct event *thread)
{
  struct bgp *bgp;
...
static void
bgp_update_delay_timer (struct event *thread)
{
  struct bgp *bgp;
...
static void
bgp_establish_wait_timer (struct event *thread)
{
  struct bgp *bgp;
...
void
bgp_fsm_change_status (struct peer *peer, enum bgp_fsm_status status)
{
  struct bgp *bgp;
  uint32_t peer_count;
      ...
      if (CHECK_FLAG (peer->bgp->flags, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
        {
          ...
          zlog_info ("%%ADJCHANGE: neighbor %pBP in vrf %s Down %s", peer,
      ...
      if (CHECK_FLAG (peer->sflags, PEER_STATUS_NSF_WAIT))
        {
          if (bgp_debug_neighbor_events (peer))
            {
              zlog_debug ("%pBP graceful restart timer started for %d sec",
      ...
      if (peer->t_refresh_stalepath)
        {
          ...
          if (bgp_debug_neighbor_events (peer))
            zlog_debug ("%pBP route-refresh restart stalepath timer stopped",
                        peer);
        }
          ...
          FOREACH_AFI_SAFI (afi, safi)
            {
              ...
              if (BGP_DEBUG (update, UPDATE_OUT))
                zlog_debug ("peer %s, EOR_required %d", peer->host,
                            gr_info->eor_required);
      ...
      if ((peer->status == OpenConfirm) || (peer_established (peer)))
        {
          ...
          snprintf (orf_name, sizeof (orf_name), "%s.%d.%d", peer->host, afi,
                    safi);
 ...
 */
static void
bgp_connect_check (struct event *thread)
{
  int status;
  socklen_t slen;
  int ret;
  ...
  if (ret < 0)
    {
      zlog_err ("can't get sockopt for nonblocking connect: %d(%s)", errno,
                safe_strerror (errno));
  ...
  if (status == 0)
    ...
    {
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s [Event] Connect failed %d(%s)", peer->host, status,
...
bgp_connect_success (struct peer *peer)
{
  if (peer->fd < 0)
    {
      flog_err (EC_BGP_CONNECT, "%s peer's fd is negative value %d", __func__,
                peer->fd);
      return bgp_stop (peer);
    }
...
bgp_connect_success_w_delayopen (struct peer *peer)
{
  ...
  if (! peer->t_delayopen)
    BGP_TIMER_ON (peer->t_delayopen, bgp_delayopen_timer, peer->v_delayopen);

  if (bgp_debug_neighbor_events (peer))
    zlog_debug ("%s [FSM] BGP OPEN message delayed for %d seconds", peer->host,
                peer->delayopen);
...
bgp_start (struct peer *peer)
{
  ...
  if (peer->bgp->vrf_id == VRF_UNKNOWN)
    {
      if (bgp_debug_neighbor_events (peer))
        flog_err (EC_BGP_FSM, "%s [FSM] In a VRF that is not initialised yet",
                  peer->host);
  ...
  switch (status)
    ...
    case connect_success:
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("%s [FSM] Connect immediately success, fd %d", peer->host,
                    peer->fd);
...
bgp_fsm_holdtime_expire (struct peer *peer)
{
   ...
   */
  if (peer_established (peer) && bgp_has_graceful_restart_notification (peer))
...
static int
bgp_start_deferral_timer (struct bgp *bgp, afi_t afi, safi_t safi,
                          struct graceful_restart_info *gr_info)
{
  ...
  if (BGP_DEBUG (update, UPDATE_OUT))
    zlog_debug ("Started the deferral timer for %s eor_required %d",
                get_afi_safi_str (afi, safi, false), gr_info->eor_required);
  return 0;
}

/* Update the graceful restart information for the specified AFI, SAFI */
static int
bgp_update_gr_info (struct peer *peer, afi_t afi, safi_t safi)
{
  struct graceful_restart_info *gr_info;
  struct bgp *bgp = peer->bgp;
  int ret = 0;
...
bgp_establish (struct peer *peer)
{
  ...
  if (CHECK_FLAG (peer->bgp->flags, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
    {
      struct vrf *vrf = vrf_lookup_by_id (peer->bgp->vrf_id);
      zlog_info (
          "%%ADJCHANGE: neighbor %pBP in vrf %s Up", peer,
          vrf ? ((vrf->vrf_id != VRF_DEFAULT) ? vrf->name : VRF_DEFAULT_NAME)
              : "");
    }
          ...
          if (! BGP_SELECT_DEFER_DISABLE (peer->bgp))
          ...
          else
            {
              if (BGP_PEER_GRACEFUL_RESTART_CAPABLE (peer) &&
                ...
                peer->bgp->gr_info[afi][safi].eor_required++;
            }
        }
    }
           ...
           (bgp_global_gr_mode_get (peer->bgp) == GLOBAL_GR)))
        {
          FOREACH_AFI_SAFI (afi, safi)
            /* Send route processing complete
               message to RIB */
            bgp_zebra_update (peer->bgp, afi, safi,
  ...
  if (peer->doppelganger && (peer->doppelganger->status != Deleted))
    {
      if (bgp_debug_neighbor_events (peer))
        zlog_debug ("[Event] Deleting stub connection for peer %s",
                    peer->host);
...
void
bgp_fsm_nht_update (struct peer *peer, bool has_valid_nexthops)
{
  if (! peer)
    return;
...
int
bgp_event_update (struct peer *peer, enum bgp_fsm_events event)
{
  ...
  if (ret >= BGP_FSM_SUCCESS)
    {
      if (ret == BGP_FSM_SUCCESS_STATE_TRANSFER && next == Established)
        {
...
int
bgp_gr_lookup_n_update_all_peer (struct bgp *bgp,
  ...
  for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
      ...
      if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
        zlog_debug ("%s [BGP_GR] Peer: (%s) :", __func__, peer->host);
      ...
      if (peer_old_state == PEER_GLOBAL_INHERIT)
        {
          ...
          switch (global_new_state)
            ...
            case GLOBAL_DISABLE:
              ...
              break;
            case GLOBAL_INVALID:
              zlog_debug ("%s [BGP_GR] GLOBAL_INVALID", __func__);
...
int
bgp_gr_update_all (struct bgp *bgp, int global_gr_cmd)
{
  ...
  if (global_old_state != GLOBAL_INVALID)
    ...
    {
      zlog_err ("%s [BGP_GR] global_old_state == GLOBAL_INVALID", __func__);
      return BGP_ERR_GR_OPERATION_FAILED;
    }

  if (global_new_state == GLOBAL_INVALID)
    {
      zlog_err ("%s [BGP_GR] global_new_state == GLOBAL_INVALID", __func__);
  ...
  if (global_new_state == global_old_state)
    {
      /* Trace msg */
      if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
        zlog_debug ("%s [BGP_GR] global_new_state == global_old_state :%s",
...
const char *
print_peer_gr_mode (enum peer_mode pr_mode)
{
...
const char *
print_peer_gr_cmd (enum peer_gr_command pr_gr_cmd)
{
...
const char *
print_global_gr_mode (enum global_mode gl_mode)
{
...
const char *
print_global_gr_cmd (enum global_gr_command gl_gr_cmd)
{
...
enum global_mode
bgp_global_gr_mode_get (struct bgp *bgp)
{
  return bgp->global_gr_present_state;
}

enum peer_mode
bgp_peer_gr_mode_get (struct peer *peer)
{
  return peer->peer_gr_present_state;
}

int
bgp_neighbor_graceful_restart (struct peer *peer, int peer_gr_cmd)
{
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("%s [BGP_GR] START:Peer: (%s) : peer_gr_cmd :%s:", __func__,
  ...
  if (peer_new_state == PEER_INVALID)
    {
      zlog_debug ("[BGP_GR] Invalid bgp graceful restart command used !");
      return BGP_ERR_GR_INVALID_CMD;
    }

  if (peer_new_state != peer_old_state)
    {
      result = peer_state.action_fun (peer, peer_old_state, peer_new_state);
    }
  else
    {
      if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
        zlog_debug ("[BGP_GR] peer_old_state == peer_new_state !");
      return BGP_GR_NO_OPERATION;
    }
...
unsigned int
bgp_peer_gr_action (struct peer *peer, int old_peer_state, int new_peer_state)
{
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug (
  ...
  if ((old_peer_state == PEER_INVALID) || (new_peer_state == PEER_INVALID))
    {
...
inline void
bgp_peer_move_to_gr_mode (struct peer *peer, int new_state)

{
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] Peer state changed  --to-->  : %d : !", new_state);
}

void
bgp_peer_gr_flags_update (struct peer *peer)
{
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
  ...
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug ("[BGP_GR] Peer %s Flag PEER_FLAG_GRACEFUL_RESTART : %s : !",
                peer->host,
  ...
  if (CHECK_FLAG (peer->peer_gr_new_status_flag,
    ...
    SET_FLAG (peer->flags, PEER_FLAG_GRACEFUL_RESTART_GLOBAL_INHERIT);
  else
    UNSET_FLAG (peer->flags, PEER_FLAG_GRACEFUL_RESTART_GLOBAL_INHERIT);
  if (BGP_DEBUG (graceful_restart, GRACEFUL_RESTART))
    zlog_debug (
        "[BGP_GR] Peer %s Flag PEER_FLAG_GRACEFUL_RESTART_GLOBAL_INHERIT : %s : !",
        peer->host,
        (CHECK_FLAG (peer->flags, PEER_FLAG_GRACEFUL_RESTART_GLOBAL_INHERIT)
             ? "Set"
             : "UnSet"));
  ...
  if (! CHECK_FLAG (peer->flags, PEER_FLAG_GRACEFUL_RESTART) &&
      ! CHECK_FLAG (peer->flags, PEER_FLAG_GRACEFUL_RESTART_HELPER))
    {
      zlog_debug ("[BGP_GR] Peer %s UNSET PEER_STATUS_NSF_MODE!", peer->host);
      ...
      if (CHECK_FLAG (peer->sflags, PEER_STATUS_NSF_WAIT))
        {
          ...
          peer_nsf_stop (peer);
          zlog_debug ("[BGP_GR] Peer %s UNSET PEER_STATUS_NSF_WAIT!",
                      peer->host);
        }
    }
}
