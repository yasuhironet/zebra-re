...
const char *const aspath_segment_type_str[] = {
  "as-invalid", "as-set", "as-sequence", "as-confed-sequence", "as-confed-set"
};
...
assegment_new (uint8_t type, unsigned short length)
{
...
#define AS_SEG_END   1
static char
aspath_delimiter_char (uint8_t type, uint8_t which)
{
  int i;
  struct
  {
...
bool
aspath_check_as_sets (struct aspath *aspath)
{
  struct assegment *seg = aspath->segments;
...
bool
aspath_check_as_zero (struct aspath *aspath)
{
  struct assegment *seg = aspath->segments;
  unsigned int i;
...
static void
aspath_make_str_count (struct aspath *as, bool make_json)
{
      ...
      for (i = 0; i < seg->length; i++)
        {
          ...
          len += snprintfrr (str_buf + len, str_size - len,
                             ASN_FORMAT (as->asnotation), &seg->as[i]);

          if (i < (seg->length - 1))
            len += snprintf (str_buf + len, str_size - len, "%c", separator);
        }
...
void
aspath_str_update (struct aspath *as, bool make_json)
{
...
static int
assegments_parse (struct stream *s, size_t length, struct assegment **result,
                  int use32bit)
{
  ...
  if ((STREAM_READABLE (s) < length) ||
      (STREAM_READABLE (s) < AS_HEADER_SIZE) || (length % AS16_VALUE_SIZE))
    return -1;
...
static size_t
assegment_header_put (struct stream *s, uint8_t type, int length)
{
  size_t lenp;
 ...
 */
int
aspath_loop_check_confed (struct aspath *aspath, as_t asno)
{
  struct assegment *seg;
  int count = 0;
...
struct aspath *
aspath_replace_all_asn (struct aspath *aspath, as_t our_asn)
{
  struct aspath *new;
  struct assegment *seg;
...
struct aspath *
aspath_replace_private_asns (struct aspath *aspath, as_t asn, as_t peer_asn)
{
  struct aspath *new;
  struct assegment *seg;
      ...
      for (i = 0; i < seg->length; i++)
        {
          ...
          if (BGP_AS_IS_PRIVATE (seg->as[i]) && (seg->as[i] != peer_asn))
            seg->as[i] = asn;
        }
      seg = seg->next;
    }
...
struct aspath *
aspath_remove_private_asns (struct aspath *aspath, as_t peer_asn)
{
  struct aspath *new;
  struct assegment *seg;
  ...
  if (as1segtail->type == AS_SEQUENCE)
    {
      ...
      assegment_append_asns (as2segtail, as2seghead->as, as2seghead->length);
  ...
  for (srcseg = source->segments; srcseg; srcseg = srcseg->next)
    {
      ...
      for (i = 0; i < srcseg->length; i++)
        {
          skip_as = 0;
          for (exclseg = exclude_list->segments; exclseg && ! skip_as;
               exclseg = exclseg->next)
...
static struct aspath *
aspath_add_asns (struct aspath *aspath, as_t asno, uint8_t type, unsigned num)
{
...
/* Compare leftmost AS value for MED check.  If as1's leftmost AS and
   ...
   only).  */
bool
aspath_cmp_left_confed (const struct aspath *aspath1,
...
static const char *
aspath_gettoken (const char *buf, enum as_token *token, unsigned long *asno)
{
  const char *p = buf;
  as_t asval;
  bool found = false;
...
struct aspath *
aspath_str2aspath (const char *str, enum asnotation_mode asnotation)
{
...
{
  ashash =
      hash_create_size (32768, aspath_key_make, aspath_cmp, "BGP AS Path");
}
...
static void
aspath_show_all_iterator (struct hash_bucket *bucket, struct vty *vty)
{
  struct aspath *as;
...
aspath_print_all_vty (struct vty *vty)
{
  hash_iterate (
      ashash,
      (void (*) (struct hash_bucket *, void *)) aspath_show_all_iterator, vty);
}

static struct aspath *
bgp_aggr_aspath_lookup (struct bgp_aggregate *aggregate, struct aspath *aspath)
{
  return hash_lookup (aggregate->aspath_hash, aspath);
}

static void *
bgp_aggr_aspath_hash_alloc (void *p)
{
...
static void
bgp_aggr_aspath_prepare (struct hash_bucket *hb, void *arg)
{
...
void
bgp_aggr_aspath_remove (void *arg)
{
...
void
bgp_compute_aggregate_aspath (struct bgp_aggregate *aggregate,
                              struct aspath *aspath)
{
                                   ...
                                   struct aspath *aspath)
{
  ...
  if (aggregate->aspath_hash == NULL)
    aggregate->aspath_hash = hash_create (aspath_key_make, aspath_cmp,
...
void
bgp_compute_aggregate_aspath_val (struct bgp_aggregate *aggregate)
{
  if (aggregate == NULL)
    return;
  ...
  if (aggregate->aspath_hash && aggregate->aspath_hash->count)
    {
      hash_iterate (aggregate->aspath_hash, bgp_aggr_aspath_prepare,
                    &aggregate->aspath);
    }
}

void
bgp_remove_aspath_from_aggregate (struct bgp_aggregate *aggregate,
                                  struct aspath *aspath)
{
  struct aspath *aggr_aspath = NULL;
  struct aspath *ret_aspath = NULL;

  if ((! aggregate) || (! aggregate->aspath_hash) || (! aspath))
    return;
  ...
  if (aggr_aspath)
    {
      ...
      if (aggr_aspath->refcnt == 0)
        {
          ret_aspath = hash_release (aggregate->aspath_hash, aggr_aspath);
...
void
bgp_remove_aspath_from_aggregate_hash (struct bgp_aggregate *aggregate,
                                       struct aspath *aspath)
{
  struct aspath *aggr_aspath = NULL;
  struct aspath *ret_aspath = NULL;
