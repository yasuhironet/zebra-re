...
static int
bgp_router_id_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx Router Id update VRF %u Id %pFX", vrf_id, &router_id);
...
static int
bgp_read_nexthop_update (ZAPI_CALLBACK_ARGS)
{
...
static int
bgp_read_fec_update (ZAPI_CALLBACK_ARGS)
{
...
static int
bgp_ifp_destroy (struct interface *ifp)
{
  struct bgp *bgp;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx Intf del VRF %u IF %s", ifp->vrf->vrf_id, ifp->name);
...
static int
bgp_ifp_up (struct interface *ifp)
{
  struct connected *c;
...
static int
bgp_ifp_down (struct interface *ifp)
{
  struct connected *c;
  struct nbr_connected *nc;
  struct listnode *node, *nnode;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx Intf down VRF %u IF %s", ifp->vrf->vrf_id, ifp->name);
...
static int
bgp_interface_address_add (ZAPI_CALLBACK_ARGS)
{
          ...
          for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
            {
              ...
              if ((peer->conf_if &&
                   ...
                   IN6_IS_ADDR_LINKLOCAL (&peer->nexthop.v6_global)))
                {
                  ...
                  memcpy (&peer->nexthop.v6_global, &addr->u.prefix6,
                          IPV6_MAX_BYTELEN);
...
static int
bgp_interface_address_delete (ZAPI_CALLBACK_ARGS)
{
  struct listnode *node, *nnode;
  struct connected *ifc;
  struct peer *peer;
      ...
      for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
        {
          ...
          if (! IN6_IS_ADDR_LINKLOCAL (&addr->u.prefix6) &&
              memcmp (&peer->nexthop.v6_global, &addr->u.prefix6, 16) == 0)
            {
...
static int
bgp_interface_nbr_address_add (ZAPI_CALLBACK_ARGS)
{
  ...
  if (bgp_debug_zebra (ifc->address))
    zlog_debug ("Rx Intf neighbor add VRF %u IF %s addr %pFX", vrf_id,
                ifc->ifp->name, ifc->address);
...
static int
bgp_interface_nbr_address_delete (ZAPI_CALLBACK_ARGS)
{
  struct nbr_connected *ifc = NULL;
  struct bgp *bgp;
  ...
  if (bgp_debug_zebra (ifc->address))
    zlog_debug ("Rx Intf neighbor del VRF %u IF %s addr %pFX", vrf_id,
                ifc->ifp->name, ifc->address);
...
static int
bgp_interface_vrf_update (ZAPI_CALLBACK_ARGS)
{
  struct interface *ifp;
  vrf_id_t new_vrf_id;
  struct connected *c;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx Intf VRF change VRF %u IF %s NewVRF %u", vrf_id, ifp->name,
                new_vrf_id);
...
static int
zebra_read_route (ZAPI_CALLBACK_ARGS)
{
  ...
  if (add)
    {
      ...
      for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
        {
          if (i != api.type)
            bgp_redistribute_delete (bgp, &api.prefix, i, api.instance);
        }

      /* Now perform the add/update. */
      bgp_redistribute_add (bgp, &api.prefix, &nexthop, ifindex, nhtype,
                            bhtype, api.distance, api.metric, api.type,
                            api.instance, api.tag);
    }
  else
    {
      bgp_redistribute_delete (bgp, &api.prefix, api.type, api.instance);
    }
  ...
  if (bgp_debug_zebra (&api.prefix))
    {
      ...
      if (add)
        {
          inet_ntop (api.prefix.family, &nexthop, buf, sizeof (buf));
          zlog_debug (
              "Rx route ADD VRF %u %s[%d] %pFX nexthop %s (type %d if %u) metric %u distance %u tag %" ROUTE_TAG_PRI,
              vrf_id, zebra_route_string (api.type), api.instance, &api.prefix,
              buf, nhtype, ifindex, api.metric, api.distance, api.tag);
        }
      else
        {
...
struct interface *
if_lookup_by_ipv6_exact (struct in6_addr *addr, ifindex_t ifindex,
                         vrf_id_t vrf_id)
{
  struct vrf *vrf;
  struct listnode *cnode;
...
bool
bgp_zebra_nexthop_set (union sockunion *local, union sockunion *remote,
  ...
  if (local->sa.sa_family == AF_INET)
    {
      ...
      if (peer->update_if)
        ...
        ifp =
            if_lookup_by_ipv4_exact (&local->sin.sin_addr, peer->bgp->vrf_id);
    }
      ...
      if (IN6_IS_ADDR_LINKLOCAL (&local->sin6.sin6_addr))
        {
          if (peer->conf_if || peer->ifname)
            ifp = if_lookup_by_name (peer->conf_if ? peer->conf_if
                                                   : peer->ifname,
                                     peer->bgp->vrf_id);
      ...
      if (! ret)
        {
           ...
           */
          v6_ll_avail = if_get_ipv6_local (ifp, &nexthop->v6_global);
          memcpy (&nexthop->v6_local, &nexthop->v6_global, IPV6_MAX_BYTELEN);
        }
      else
        v6_ll_avail = if_get_ipv6_local (ifp, &nexthop->v6_local);
      ...
      if (path->attr->mp_nexthop_prefer_global)
      ...
      else
        {
          ...
          if (IN6_IS_ADDR_UNSPECIFIED (&path->attr->mp_nexthop_global) &&
              path->peer->su_remote &&
              path->peer->su_remote->sa.sa_family == AF_INET6)
            {
              nexthop = &path->peer->su_remote->sin6.sin6_addr;
              if (IN6_IS_ADDR_LINKLOCAL (nexthop))
                *ifindex = path->peer->nexthop.ifp->ifindex;
            }
          else
            {
...
static bool
bgp_table_map_apply (struct route_map *map, const struct prefix *p,
  ...
  if (bgp_debug_zebra (p))
    {
      if (p->family == AF_INET)
        {
          zlog_debug ("Zebra rmap deny: IPv4 route %pFX nexthop %pI4", p,
                      &path->attr->nexthop);
        }
      if (p->family == AF_INET6)
        {
          ...
          zlog_debug ("Zebra rmap deny: IPv6 route %pFX nexthop %pI6", p,
                      nexthop);
        }
    }
  return false;
...
static void
bgp_zebra_tm_connect (struct event *t)
{
  ...
  if (ret < 0)
  ...
  else
    {
      ...
      if (! bgp_tm_chunk_obtained)
        {
          if (bgp_zebra_get_table_range (bgp_tm_chunk_size, &bgp_tm_min,
                                         &bgp_tm_max) >= 0)
            {
...
bool
bgp_zebra_tm_chunk_obtained (void)
{
  return bgp_tm_chunk_obtained;
}

uint32_t
bgp_zebra_tm_get_id (void)
{
  static int table_id;
...
void
bgp_zebra_init_tm_connect (struct bgp *bgp)
{
  int delay = 1;
...
int
bgp_zebra_get_table_range (uint32_t chunk_size, uint32_t *start, uint32_t *end)
{
  int ret;
  ...
  if (ret < 0)
    {
      flog_err (EC_BGP_TABLE_CHUNK, "BGP: Error getting table chunk %u",
                chunk_size);
      return -1;
    }
...
static bool
update_ipv4nh_for_route_install (int nh_othervrf, struct bgp *nh_bgp,
          ...
          if (! ifindex)
            {
              if (pi->peer->conf_if)
                ...
                ifindex =
                    ifname2ifindex (pi->peer->ifname, pi->peer->bgp->vrf_id);
...
static bool
bgp_zebra_use_nhop_weighted (struct bgp *bgp, struct attr *attr,
...
void
bgp_zebra_announce (struct bgp_dest *dest, const struct prefix *p,
                    ...
                    safi_t safi)
{
  ...
  if (safi == SAFI_FLOWSPEC)
    {
      bgp_pbr_update_entry (bgp, bgp_dest_get_prefix (dest), info, afi, safi,
                            true);
      return;
    }
  ...
  if (info->type == ZEBRA_ROUTE_BGP && info->sub_type == BGP_ROUTE_IMPORTED)
    {

      /* Obtain peer from parent */
      if (info->extra && info->extra->parent)
        peer = ((struct bgp_path_info *) (info->extra->parent))->peer;
    }
  ...
  for (; mpinfo; mpinfo = bgp_path_info_mpath_next (mpinfo))
    {
      ...
      *mpinfo_cp = *mpinfo;
      ...
      if (do_wt_ecmp)
        {
          if (! bgp_zebra_use_nhop_weighted (bgp, mpinfo->attr, cum_bw,
                                             &nh_weight))
            continue;
        }
      ...
      if (bgp_debug_zebra (&api.prefix))
        {
          if (mpinfo->extra)
            {
              zlog_debug ("%s: p=%pFX, bgp_is_valid_label: %d", __func__, p,
                          bgp_is_valid_label (&mpinfo->extra->label[0]));
            }
          else
            {
              zlog_debug ("%s: p=%pFX, extra is NULL, no label", __func__, p);
            }
        }
      ...
      if (bgp->table_map[afi][safi].name)
        {
          ...
          if (! bgp_table_map_apply (bgp->table_map[afi][safi].map, p,
                                     mpinfo_cp))
            continue;
      ...
      if (nh_family == AF_INET)
        {
          nh_updated = update_ipv4nh_for_route_install (
              nh_othervrf, bgp_orig, &mpinfo_cp->attr->nexthop,
              mpinfo_cp->attr, is_parent_evpn, api_nh);
        }
      else
        {
          ...
          nexthop = bgp_path_info_to_ipv6_nexthop (mpinfo_cp, &ifindex);
          ...
          if (! nexthop)
            ...
            nh_updated = update_ipv6nh_for_route_install (
                nh_othervrf, bgp_orig, nexthop, ifindex, mpinfo, info,
      ...
      if (labels && (num_labels > 0) &&
          (is_evpn || bgp_is_valid_label (&labels[0])))
        {
          ...
          if (is_evpn)
            {
              nh_label =
                  *bgp_evpn_path_info_labels_get_l3vni (labels, num_labels);
      ...
      if (is_evpn &&
        ...
        memcpy (&api_nh->rmac, &(mpinfo->attr->rmac), sizeof (struct ethaddr));

      api_nh->weight = nh_weight;

      if (mpinfo->extra && ! is_evpn && bgp_is_valid_label (&labels[0]) &&
  ...
  if (is_add && CHECK_FLAG (bm->flags, BM_FLAG_SEND_EXTRA_DATA_TO_ZEBRA))
    {
      ...
      const char *reason = bgp_path_selection_reason2str (dest->reason);

      strlcpy (bzo.aspath, info->attr->aspath->str, sizeof (bzo.aspath));

      if (info->attr->flag & ATTR_FLAG_BIT (BGP_ATTR_COMMUNITIES))
        strlcpy (bzo.community, bgp_attr_get_community (info->attr)->str,
                 sizeof (bzo.community));

      if (info->attr->flag & ATTR_FLAG_BIT (BGP_ATTR_LARGE_COMMUNITIES))
        strlcpy (bzo.lcommunity, bgp_attr_get_lcommunity (info->attr)->str,
                 sizeof (bzo.lcommunity));

      strlcpy (bzo.selection_reason, reason, sizeof (bzo.selection_reason));

      SET_FLAG (api.message, ZAPI_MESSAGE_OPAQUE);
      api.opaque.length =
          MIN (sizeof (struct bgp_zebra_opaque), ZAPI_MESSAGE_OPAQUE_LENGTH);
  ...
  if (bgp_debug_zebra (p))
    {
      ...
      zlog_debug ("%s: %pFX: announcing to zebra (recursion %sset)", __func__,
                  p, (recursion_flag ? "" : "NOT "));
    }
  zclient_route_send (is_add ? ZEBRA_ROUTE_ADD : ZEBRA_ROUTE_DELETE, zclient,
                      &api);
}
  ...
  for (dest = bgp_table_top (table); dest; dest = bgp_route_next (dest))
    for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
      if (CHECK_FLAG (pi->flags, BGP_PATH_SELECTED) &&
        ...
        bgp_zebra_announce (dest, bgp_dest_get_prefix (dest), pi, bgp, afi,
                            safi);
}

/* Announce routes of any bgp subtype of a table to zebra */
void
bgp_zebra_announce_table_all_subtypes (struct bgp *bgp, afi_t afi, safi_t safi)
{
  struct bgp_dest *dest;
  struct bgp_table *table;
  struct bgp_path_info *pi;
...
void
bgp_zebra_withdraw (const struct prefix *p, struct bgp_path_info *info,
                    struct bgp *bgp, safi_t safi)
{
  ...
  if (bgp_debug_zebra (p))
    zlog_debug ("Tx route delete VRF %u %pFX", bgp->vrf_id, &api.prefix);
...
void
bgp_zebra_withdraw_table_all_subtypes (struct bgp *bgp, afi_t afi, safi_t safi)
{
  struct bgp_dest *dest;
  struct bgp_table *table;
  struct bgp_path_info *pi;
      ...
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
          if (CHECK_FLAG (pi->flags, BGP_PATH_SELECTED) &&
              (pi->type == ZEBRA_ROUTE_BGP))
            bgp_zebra_withdraw (bgp_dest_get_prefix (dest), pi, bgp, safi);
        }
    }
}

struct bgp_redist *
bgp_redist_lookup (struct bgp *bgp, afi_t afi, uint8_t type,
...
struct bgp_redist *
bgp_redist_add (struct bgp *bgp, afi_t afi, uint8_t type,
                unsigned short instance)
{
  struct list *red_list;
  struct bgp_redist *red;
...
static void
bgp_redist_del (struct bgp *bgp, afi_t afi, uint8_t type,
                unsigned short instance)
{
  struct bgp_redist *red;
                      ...
                      unsigned short instance, bool changed)
{
  ...
  if (instance)
  ...
  else
    {
      ...
      if (EVPN_ENABLED (bgp) && type == ZEBRA_ROUTE_VNC_DIRECT)
        {
          vnc_export_bgp_enable (bgp,
                                 afi); /* only enables if mode bits cfg'd */
        }
#endif
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Tx redistribute add VRF %u afi %d %s %d", bgp->vrf_id, afi,
                         ...
                         unsigned short instance)
{
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Tx redistribute del/add VRF %u afi %d %s %d", bgp->vrf_id,
...
bool
bgp_redistribute_metric_set (struct bgp *bgp, struct bgp_redist *red,
                             afi_t afi, int type, uint32_t metric)
{
  ...
  for (dest = bgp_table_top (bgp->rib[afi][SAFI_UNICAST]); dest;
       dest = bgp_route_next (dest))
    {
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
          if (pi->sub_type == BGP_ROUTE_REDISTRIBUTE && pi->type == type &&
...
int
bgp_redistribute_unreg (struct bgp *bgp, afi_t afi, int type,
                        unsigned short instance)
{
  struct bgp_redist *red;
  ...
  if (bgp_install_info_to_zebra (bgp))
    {
      ...
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("Tx redistribute del VRF %u afi %d %s %d", bgp->vrf_id,
                    afi, zebra_route_string (type), instance);
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE, zclient, afi, type,
                               instance, bgp->vrf_id);
    }
...
void
bgp_redistribute_redo (struct bgp *bgp)
{
  afi_t afi;
  int i;
  struct list *red_list;
...
bgp_zebra_initiate_radv (struct bgp *bgp, struct peer *peer)
{
   ...
   */
  peer->ifp ? zclient_send_interface_radv_req (zclient, bgp->vrf_id, peer->ifp,
                                               1, ra_interval)
...
int
bgp_zebra_advertise_subnet (struct bgp *bgp, int advertise, vni_t vni)
{
...
int
bgp_zebra_advertise_svi_macip (struct bgp *bgp, int advertise, vni_t vni)
{
  struct stream *s = NULL;
...
int
bgp_zebra_advertise_gw_macip (struct bgp *bgp, int advertise, vni_t vni)
{
  struct stream *s = NULL;
...
int
bgp_zebra_vxlan_flood_control (struct bgp *bgp,
...
int
bgp_zebra_dup_addr_detection (struct bgp *bgp)
{
  struct stream *s;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug (
        "dup addr detect %s max_moves %u time %u freeze %s freeze_time %u",
        bgp->evpn_info->dup_addr_detect ? "enable" : "disable",
        bgp->evpn_info->dad_max_moves, bgp->evpn_info->dad_time,
        bgp->evpn_info->dad_freeze ? "enable" : "disable",
        bgp->evpn_info->dad_freeze_time);

  s = zclient->obuf;
  stream_reset (s);
  zclient_create_header (s, ZEBRA_DUPLICATE_ADDR_DETECTION, bgp->vrf_id);
...
static int
rule_notify_owner (ZAPI_CALLBACK_ARGS)
{
  ...
  if (! bgp_pbra)
    {
      ...
      if (! bgp_pbr && note != ZAPI_RULE_REMOVED)
        {
          if (BGP_DEBUG (zebra, ZEBRA))
            zlog_debug ("%s: Fail to look BGP rule (%u)", __func__, unique);
          return 0;
        }
    }
    ...
    case ZAPI_RULE_INSTALLED:
      if (bgp_pbra)
      ...
      else
        {
          ...
          listnode_add_force (&extra->bgp_fs_iprule, bgp_pbr);
        }
      if (BGP_DEBUG (zebra, ZEBRA))
...
static int
ipset_notify_owner (ZAPI_CALLBACK_ARGS)
{
  uint32_t unique;
  enum zapi_ipset_notify_owner note;
  struct bgp_pbr_match *bgp_pbim;

  if (! zapi_ipset_notify_decode (zclient->ibuf, &unique, &note))
    return -1;
  ...
  if (! bgp_pbim)
    {
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: Fail to look BGP match ( %u, ID %u)", __func__, note,
                    unique);
      return 0;
    }
...
static int
ipset_entry_notify_owner (ZAPI_CALLBACK_ARGS)
{
  uint32_t unique;
  ...
  if (! zapi_ipset_entry_notify_decode (zclient->ibuf, &unique, ipset_name,
                                        &note))
    return -1;
  ...
  if (! bgp_pbime)
    {
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: Fail to look BGP match entry (%u, ID %u)", __func__,
                    note, unique);
      return 0;
    }
  ...
  switch (note)
    ...
    case ZAPI_IPSET_ENTRY_FAIL_INSTALL:
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: Received IPSET_ENTRY_FAIL_INSTALL", __func__);
    ...
    case ZAPI_IPSET_ENTRY_INSTALLED:
      {
        ...
        bgp_pbime->install_in_progress = false;
        if (BGP_DEBUG (zebra, ZEBRA))
          zlog_debug ("%s: Received IPSET_ENTRY_INSTALLED", __func__);
    ...
    case ZAPI_IPSET_ENTRY_REMOVED:
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: Received IPSET_ENTRY_REMOVED", __func__);
      break;
    }
  return 0;
}
...
static int
iptable_notify_owner (ZAPI_CALLBACK_ARGS)
{
  uint32_t unique;
  enum zapi_iptable_notify_owner note;
  struct bgp_pbr_match *bgpm;

  if (! zapi_iptable_notify_decode (zclient->ibuf, &unique, &note))
    return -1;
  bgpm = bgp_pbr_match_iptable_lookup (vrf_id, unique);
  if (! bgpm)
    {
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: Fail to look BGP iptable (%u %u)", __func__, note,
                    unique);
      return 0;
    }
  switch (note)
    ...
    case ZAPI_IPTABLE_FAIL_INSTALL:
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: Received IPTABLE_FAIL_INSTALL", __func__);
...
static int
bgp_zebra_route_notify_owner (int command, struct zclient *zclient,
                              zebra_size_t length, vrf_id_t vrf_id)
{
  ...
  if (! zapi_route_notify_decode (zclient->ibuf, &p, &table_id, &note, &afi,
                                  &safi))
    {
  ...
  if (! bgp)
    {
      flog_err (EC_BGP_INVALID_BGP_INSTANCE,
                "%s : bgp instance not found vrf %d", __func__, vrf_id);
      return -1;
    }

  /* Find the bgp route node */
  dest = bgp_safi_node_lookup (bgp->rib[afi][safi], safi, &p, &bgp->vrf_prd);
  if (! dest)
    return -1;
  ...
  switch (note)
    ...
    case ZAPI_ROUTE_INSTALLED:
      ...
      if (new_select)
        ...
        {
          flog_err (EC_BGP_INVALID_ROUTE, "selected route %pRN not found",
                    dest);
    ...
    case ZAPI_ROUTE_FAIL_INSTALL:
      new_select = NULL;
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("route: %pRN Failed to Install into Fib", dest);
      UNSET_FLAG (dest->flags, BGP_NODE_FIB_INSTALL_PENDING);
      UNSET_FLAG (dest->flags, BGP_NODE_FIB_INSTALLED);
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
    ...
    case ZAPI_ROUTE_BETTER_ADMIN_WON:
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("route: %pRN removed due to better admin won", dest);
      new_select = NULL;
      UNSET_FLAG (dest->flags, BGP_NODE_FIB_INSTALL_PENDING);
      UNSET_FLAG (dest->flags, BGP_NODE_FIB_INSTALLED);
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
    ...
    case ZAPI_ROUTE_REMOVE_FAIL:
      zlog_warn ("%s: Route %pRN failure to remove", __func__, dest);
      break;
    }
 ...
 */
static void
bgp_encode_pbr_rule_action (struct stream *s, struct bgp_pbr_action *pbra,
...
static void
bgp_encode_pbr_ipset_match (struct stream *s, struct bgp_pbr_match *pbim)
{
  stream_putl (s, pbim->unique);
  stream_putl (s, pbim->type);
  stream_putc (s, pbim->family);
  stream_put (s, pbim->ipset_name, ZEBRA_IPSET_NAME_SIZE);
}

static void
bgp_encode_pbr_ipset_entry_match (struct stream *s,
                                  struct bgp_pbr_match_entry *pbime)
{
  stream_putl (s, pbime->unique);
  /* check that back pointer is not null */
  stream_put (s, pbime->backpointer->ipset_name, ZEBRA_IPSET_NAME_SIZE);
...
static void
bgp_encode_pbr_iptable_match (struct stream *s, struct bgp_pbr_action *bpa,
                              struct bgp_pbr_match *pbm)
{
  ...
  stream_put (s, pbm->ipset_name, ZEBRA_IPSET_NAME_SIZE);
...
static int
bgp_zebra_process_local_es_add (ZAPI_CALLBACK_ARGS)
{
  esi_t esi;
  struct bgp *bgp = NULL;
  struct stream *s = NULL;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx add ESI %s originator-ip %pI4 active %u df_pref %u %s",
                esi_to_str (&esi, buf, sizeof (buf)), &originator_ip, active,
...
static int
bgp_zebra_process_local_es_del (ZAPI_CALLBACK_ARGS)
{
  esi_t esi;
  struct bgp *bgp = NULL;
  struct stream *s = NULL;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx del ESI %s", esi_to_str (&esi, buf, sizeof (buf)));
...
static int
bgp_zebra_process_local_es_evi (ZAPI_CALLBACK_ARGS)
{
  esi_t esi;
  vni_t vni;
  struct bgp *bgp;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx %s ESI %s VNI %u", (cmd == ZEBRA_VNI_ADD) ? "add" : "del",
...
static int
bgp_zebra_process_local_l3vni (ZAPI_CALLBACK_ARGS)
{
  int filter = 0;
  vni_t l3vni = 0;
  ...
  if (cmd == ZEBRA_L3VNI_ADD)
    {
      ...
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug (
            ...
            vrf_id_to_name (vrf_id), l3vni, &svi_rmac, &vrr_rmac,
            filter ? "prefix-routes-only" : "none", svi_ifindex);

      frrtrace (8, frr_bgp, evpn_local_l3vni_add_zrecv, l3vni, vrf_id,
                &svi_rmac, &vrr_rmac, filter, originator_ip, svi_ifindex,
                is_anycast_mac);
    ...
    {
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("Rx L3-VNI DEL VRF %s VNI %u", vrf_id_to_name (vrf_id),
                    l3vni);
...
static int
bgp_zebra_process_local_vni (ZAPI_CALLBACK_ARGS)
{
  struct stream *s;
  vni_t vni;
  struct bgp *bgp;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx VNI %s VRF %s VNI %u tenant-vrf %s SVI ifindex %u",
                (cmd == ZEBRA_VNI_ADD) ? "add" : "del",
                vrf_id_to_name (vrf_id), vni, vrf_id_to_name (tenant_vrf_id),
                svi_ifindex);
  ...
  if (cmd == ZEBRA_VNI_ADD)
    {
      ...
      return bgp_evpn_local_vni_add (
          bgp, vni, vtep_ip.s_addr != INADDR_ANY ? vtep_ip : bgp->router_id,
...
static int
bgp_zebra_process_local_macip (ZAPI_CALLBACK_ARGS)
{
  struct stream *s;
  vni_t vni;
  struct bgp *bgp;
  ...
  if (ipa_len != 0 && ipa_len != IPV4_MAX_BYTELEN &&
      ipa_len != IPV6_MAX_BYTELEN)
    {
      flog_err (EC_BGP_MACIP_LEN,
                "%u:Recv MACIP %s with invalid IP addr length %d", vrf_id,
                (cmd == ZEBRA_MACIP_ADD) ? "Add" : "Del", ipa_len);
      return -1;
    }
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug (
        ...
        vrf_id, (cmd == ZEBRA_MACIP_ADD) ? "Add" : "Del", flags, &mac, &ip,
        vni, seqnum, state, esi_to_str (&esi, buf2, sizeof (buf2)));

  if (cmd == ZEBRA_MACIP_ADD)
    {
      frrtrace (6, frr_bgp, evpn_local_macip_add_zrecv, vni, &mac, &ip, flags,
                seqnum, &esi);

      return bgp_evpn_local_macip_add (bgp, vni, &mac, &ip, flags, seqnum,
                                       &esi);
    }
  else
    {
      frrtrace (4, frr_bgp, evpn_local_macip_del_zrecv, vni, &mac, &ip, state);
...
static int
bgp_zebra_process_local_ip_prefix (ZAPI_CALLBACK_ARGS)
{
  struct stream *s = NULL;
  ...
  if (cmd == ZEBRA_IP_PREFIX_ROUTE_ADD)
    {
      ...
      if (p.family == AF_INET)
        bgp_evpn_advertise_type5_route (bgp_vrf, &p, NULL, AFI_IP,
                                        SAFI_UNICAST);
      else
        bgp_evpn_advertise_type5_route (bgp_vrf, &p, NULL, AFI_IP6,
                                        SAFI_UNICAST);
    }
  else
    {
      if (p.family == AF_INET)
        ...
        bgp_evpn_withdraw_type5_route (bgp_vrf, &p, AFI_IP6, SAFI_UNICAST);
    }
  return 0;
}

static int
bgp_zebra_process_label_chunk (ZAPI_CALLBACK_ARGS)
{
  struct stream *s = NULL;
  ...
  if (zclient->redist_default != proto)
    {
      flog_err (EC_BGP_LM_ERROR, "Got LM msg with wrong proto %u", proto);
      return 0;
    }
  if (zclient->instance != instance)
    {
      flog_err (EC_BGP_LM_ERROR, "Got LM msg with wrong instance %u", proto);
      return 0;
    }

  if (first > last || first < MPLS_LABEL_UNRESERVED_MIN ||
      last > MPLS_LABEL_UNRESERVED_MAX)
    {

      flog_err (EC_BGP_LM_ERROR, "%s: Invalid Label chunk: %u - %u", __func__,
                first, last);
      return 0;
    }
  if (BGP_DEBUG (zebra, ZEBRA))
    {
      zlog_debug ("Label Chunk assign: %u - %u (%u) ", first, last,
                  response_keep);
    }
...
static int
bgp_ifp_create (struct interface *ifp)
{
  struct bgp *bgp;

  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("Rx Intf add VRF %u IF %s", ifp->vrf->vrf_id, ifp->name);
...
static int
bgp_zebra_process_srv6_locator_chunk (ZAPI_CALLBACK_ARGS)
{
  struct stream *s = NULL;
...
static int
bgp_zebra_process_srv6_locator_add (ZAPI_CALLBACK_ARGS)
{
...
static int
bgp_zebra_process_srv6_locator_delete (ZAPI_CALLBACK_ARGS)
{
  struct srv6_locator loc = {};
  struct bgp *bgp = bgp_get_default ();
  struct listnode *node, *nnode;
...
static int
bgp_if_new_hook (struct interface *ifp)
{
...
static int
bgp_if_delete_hook (struct interface *ifp)
{
...
void
bgp_zebra_init (struct event_loop *master, unsigned short instance)
{
...
void
bgp_send_pbr_rule_action (struct bgp_pbr_action *pbra,
                          struct bgp_pbr_rule *pbr, bool install)
{
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    {
      if (pbr)
        zlog_debug ("%s: table %d (ip rule) %d", __func__, pbra->table_id,
                    install);
      else
        zlog_debug ("%s: table %d fwmark %d %d", __func__, pbra->table_id,
                    pbra->fwmark, install);
    }
  s = zclient->obuf;
  stream_reset (s);

  zclient_create_header (s, install ? ZEBRA_RULE_ADD : ZEBRA_RULE_DELETE,
                         VRF_DEFAULT);
...
void
bgp_send_pbr_ipset_match (struct bgp_pbr_match *pbrim, bool install)
{
  struct stream *s;
  ...
  if (pbrim->install_in_progress)
    return;
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("%s: name %s type %d %d, ID %u", __func__, pbrim->ipset_name,
                pbrim->type, install, pbrim->unique);
  s = zclient->obuf;
  stream_reset (s);

  zclient_create_header (s, install ? ZEBRA_IPSET_CREATE : ZEBRA_IPSET_DESTROY,
                         VRF_DEFAULT);
...
void
bgp_send_pbr_ipset_entry_match (struct bgp_pbr_match_entry *pbrime,
                                bool install)
{
  struct stream *s;
  ...
  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("%s: name %s %d %d, ID %u", __func__,
                pbrime->backpointer->ipset_name, pbrime->unique, install,
                pbrime->unique);
  s = zclient->obuf;
  stream_reset (s);

  zclient_create_header (
      s, install ? ZEBRA_IPSET_ENTRY_ADD : ZEBRA_IPSET_ENTRY_DELETE,
      VRF_DEFAULT);
...
static void
bgp_encode_pbr_interface_list (struct bgp *bgp, struct stream *s,
                               uint8_t family)
{
...
static int
bgp_pbr_get_ifnumber (struct bgp *bgp, uint8_t family)
{
  struct bgp_pbr_config *bgp_pbr_cfg = bgp->bgp_pbr_cfg;
  struct bgp_pbr_interface_head *head;
  struct bgp_pbr_interface *pbr_if;
...
void
bgp_send_pbr_iptable (struct bgp_pbr_action *pba, struct bgp_pbr_match *pbm,
                      bool install)
{
  struct stream *s;
  int ret = 0;
  ...
  zclient_create_header (s, install ? ZEBRA_IPTABLE_ADD : ZEBRA_IPTABLE_DELETE,
                         VRF_DEFAULT);
 ...
 */
void
bgp_zebra_announce_default (struct bgp *bgp, struct nexthop *nh, afi_t afi,
                            uint32_t table_id, bool announce)
{
  struct zapi_nexthop *api_nh;
  struct zapi_route api;
  struct prefix p;
  ...
  if (! nh ||
      (nh->type != NEXTHOP_TYPE_IPV4 && nh->type != NEXTHOP_TYPE_IPV6) ||
  ...
  if (afi == AFI_IP && nh->gate.ipv4.s_addr != INADDR_ANY)
    {
      ...
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("BGP: %s default route to %pI4 table %d (redirect IP)",
                    announce ? "adding" : "withdrawing", &nh->gate.ipv4,
                    table_id);
      ...
      zclient_route_send (announce ? ZEBRA_ROUTE_ADD : ZEBRA_ROUTE_DELETE,
           ...
           memcmp (&nh->gate.ipv6, &in6addr_any, sizeof (struct in6_addr)))
    {
      api_nh->vrf_id = nh->vrf_id;
      memcpy (&api_nh->gate.ipv6, &nh->gate.ipv6, sizeof (struct in6_addr));
      api_nh->type = NEXTHOP_TYPE_IPV6;

      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("BGP: %s default route to %pI6 table %d (redirect IP)",
                    announce ? "adding" : "withdrawing", &nh->gate.ipv6,
                    table_id);
  ...
  else if (nh->vrf_id != bgp->vrf_id)
    {
      ...
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_info ("BGP: %s default route to %s table %d (redirect VRF)",
                   announce ? "adding" : "withdrawing", vrf->name, table_id);
      zclient_route_send (announce ? ZEBRA_ROUTE_ADD : ZEBRA_ROUTE_DELETE,
                          zclient, &api);
      return;
    }
...
int
bgp_zebra_send_capabilities (struct bgp *bgp, bool disable)
{
  struct zapi_cap api;
  int ret = BGP_GR_SUCCESS;

  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("%s: Sending %sable for %s", __func__, disable ? "dis" : "en",
                bgp->name_pretty);

  if (zclient == NULL)
    {
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: %s zclient invalid", __func__, bgp->name_pretty);
  ...
  if ((zclient->sock < 0) || (zclient->t_connect))
    {
      if (BGP_DEBUG (zebra, ZEBRA))
        zlog_debug ("%s: %s client not connected", __func__, bgp->name_pretty);
      return BGP_GR_FAILURE;
    }
  ...
  if (zclient_capabilities_send (ZEBRA_CLIENT_CAPABILITIES, zclient, &api) ==
    ...
    {
      zlog_err ("%s: %s error sending capability", __func__, bgp->name_pretty);
      ret = BGP_GR_FAILURE;
    }
  else
    {
 ...
 */
int
bgp_zebra_update (struct bgp *bgp, afi_t afi, safi_t safi,
...
int
bgp_zebra_stale_timer_update (struct bgp *bgp)
{
  struct zapi_cap api;

  if (BGP_DEBUG (zebra, ZEBRA))
    zlog_debug ("%s: %s Timer Update to %u", __func__, bgp->name_pretty,
                bgp->rib_stale_time);
...
int
bgp_zebra_srv6_manager_get_locator_chunk (const char *name)
{
  return srv6_manager_get_locator_chunk (zclient, name);
}

int
bgp_zebra_srv6_manager_release_locator_chunk (const char *name)
{
  return srv6_manager_release_locator_chunk (zclient, name);
}

void
bgp_zebra_send_nexthop_label (int cmd, mpls_label_t label, ifindex_t ifindex,
                              vrf_id_t vrf_id, enum lsp_types_t ltype,
                              struct prefix *p)
{
