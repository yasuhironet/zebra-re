...
int
argv_find_and_parse_oly_idx (struct cmd_token **argv, int argc, int *oly_idx,
...
static void
display_vrf_import_rt (struct vty *vty, struct vrf_irt_node *irt,
                       json_object *json)
{
  const uint8_t *pnt;
  ...
  for (ALL_LIST_ELEMENTS (irt->vrfs, node, nnode, tmp_bgp_vrf))
    {
      if (json)
        json_object_array_add (
            json_vrfs,
            json_object_new_string (vrf_id_to_name (tmp_bgp_vrf->vrf_id)));
      else
...
static void
show_vrf_import_rt_entry (struct hash_bucket *bucket, void *args[])
{
...
static void
display_import_rt (struct vty *vty, struct irt_node *irt, json_object *json)
{
  const uint8_t *pnt;
  uint8_t type, sub_type;
  struct ecommunity_as eas;
...
static void
show_import_rt_entry (struct hash_bucket *bucket, void *args[])
{
  json_object *json = NULL;
  struct vty *vty = NULL;
...
static void
bgp_evpn_show_route_rd_header (struct vty *vty, struct bgp_dest *rd_dest,
                               json_object *json, char *rd_str, int len)
{
  uint16_t type;
  struct rd_as rd_as;
  struct rd_ip rd_ip;
...
static void
bgp_evpn_show_route_header (struct vty *vty, struct bgp *bgp, uint64_t tbl_ver,
                            json_object *json)
{
  char ri_header[] =
  ...
  vty_out (vty, "BGP table version is %" PRIu64 ", local router ID is %pI4\n",
...
static void
display_l3vni (struct vty *vty, struct bgp *bgp_vrf, json_object *json)
{
  char *ecom_str;
  struct listnode *node, *nnode;
  ...
  if (json)
    {
      ...
      json_object_string_add (json, "advertisePip",
                              bgp_vrf->evpn_info->advertise_pip ? "Enabled"
                                                                : "Disabled");
      json_object_string_addf (json, "sysIP", "%pI4",
                               &bgp_vrf->evpn_info->pip_ip);
      json_object_string_add (
          json, "sysMac",
          prefix_mac2str (&bgp_vrf->evpn_info->pip_rmac, buf2, sizeof (buf2)));
      json_object_string_add (
          json, "rmac", prefix_mac2str (&bgp_vrf->rmac, buf2, sizeof (buf2)));
    }
  else
    {
      ...
      vty_out (vty, "  Router-MAC: %s\n",
               prefix_mac2str (&bgp_vrf->rmac, buf2, sizeof (buf2)));
    }
  ...
  for (ALL_LIST_ELEMENTS (bgp_vrf->vrf_import_rtl, node, nnode, l3rt))
    {
      ecom_str =
          ecommunity_ecom2str (l3rt->ecom, ECOMMUNITY_FORMAT_ROUTE_MAP, 0);
...
static void
display_vni (struct vty *vty, struct bgpevpn *vpn, json_object *json)
{
  char *ecom_str;
  struct listnode *node, *nnode;
  ...
  if (json)
    {
      ...
      json_object_string_addf (json, "rd", BGP_RD_AS_FORMAT (asnotation),
                               &vpn->prd);
      json_object_string_addf (json, "originatorIp", "%pI4",
                               &vpn->originator_ip);
      json_object_string_addf (json, "mcastGroup", "%pI4", &vpn->mcast_grp);
      ...
      if (! vpn->advertise_gw_macip && bgp_evpn &&
        ...
        json_object_string_add (json, "advertiseGatewayMacip", "Active");
      else if (vpn->advertise_gw_macip)
        json_object_string_add (json, "advertiseGatewayMacip", "Enabled");
      else
        json_object_string_add (json, "advertiseGatewayMacip", "Disabled");
      if (! vpn->advertise_svi_macip && bgp_evpn &&
          bgp_evpn->evpn_info->advertise_svi_macip)
        json_object_string_add (json, "advertiseSviMacIp", "Active");
      else if (vpn->advertise_svi_macip)
        json_object_string_add (json, "advertiseSviMacIp", "Enabled");
      else
        json_object_string_add (json, "advertiseSviMacIp", "Disabled");
      json_object_string_add (
          json, "sviInterface",
...
static void
show_esi_routes (struct bgp *bgp, struct bgp_evpn_es *es, struct vty *vty,
                 json_object *json)
{
  int header = 1;
      ...
      if (pi)
        {
          ...
          if (header)
            {
              bgp_evpn_show_route_header (vty, bgp, tbl_ver, json);
              header = 0;
            }
          ...
          if (add_prefix_to_json)
            {
              ...
              json_object_object_addf (json, json_prefix, "%pFX", p);
            }
          else
            {
...
static void
bgp_evpn_show_routes_mac_ip_es (struct vty *vty, esi_t *esi, json_object *json,
                                int detail, bool global_table)
{
  struct bgp_node *rn;
  struct bgp_path_info *pi;
      ...
      for (ALL_LIST_ELEMENTS_RO (es_list, node, es_info))
        {
          ...
          if (detail)
            ...
            route_vty_out (vty, &rn->p, pi, 0, SAFI_EVPN, json_path, false);
...
static void
bgp_evpn_show_routes_mac_ip_evi_es (struct vty *vty, esi_t *esi,
...
static void
bgp_evpn_show_routes_mac_ip_global_es (struct vty *vty, esi_t *esi,
                                       json_object *json, int detail)
{
...
static void
show_vni_routes (struct bgp *bgp, struct bgpevpn *vpn, struct vty *vty,
                 int type, bool mac_table, struct in_addr vtep_ip,
                 json_object *json, int detail)
{
  struct bgp_dest *dest;
  struct bgp_path_info *pi;
      ...
      for (; pi; pi = pi->next)
        {
          ...
          if (vtep_ip.s_addr != INADDR_ANY &&
              ! IPV4_ADDR_SAME (&(vtep_ip), &(pi->attr->nexthop)))
            continue;
              ...
              if (is_evpn_prefix_ipaddr_none (evp))
                {
                  ...
                  evpn_type2_prefix_global_copy (
                      (struct prefix_evpn *) &tmp_p, evp, NULL /* mac */,
              ...
              else
                {
                  ...
                  evpn_type2_prefix_global_copy (
                      ...
                      evpn_type2_path_info_get_mac (pi), NULL /* ip */);
                }
            }
          else
          ...
          if (detail)
            ...
            route_vty_out (vty, &tmp_p, pi, 0, SAFI_EVPN, json_path, false);
  ...
  else
    {
      if (prefix_cnt == 0)
        ...
        vty_out (vty, "\nDisplayed %u prefixes (%u paths)%s\n", prefix_cnt,
                 path_cnt, type ? " (of requested type)" : "");
      vty_out (vty, "\n");
    }
}

static void
show_vni_routes_hash (struct hash_bucket *bucket, void *arg)
{
...
static void
show_vni_routes_all_hash (struct hash_bucket *bucket, void *arg)
{
  struct bgpevpn *vpn = (struct bgpevpn *) bucket->data;
  struct vni_walk_ctx *wctx = arg;
  struct vty *vty = wctx->vty;
...
static void
show_l3vni_entry (struct vty *vty, struct bgp *bgp, json_object *json)
{
  json_object *json_vni = NULL;
  json_object *json_import_rtl = NULL;
  json_object *json_export_rtl = NULL;
  ...
  if (json)
    {
      ...
      json_object_string_addf (
          json_vni, "rd", BGP_RD_AS_FORMAT (bgp->asnotation), &bgp->vrf_prd);
      json_object_string_add (json_vni, "advertiseGatewayMacip", "n/a");
      json_object_string_add (json_vni, "advertiseSviMacIp", "n/a");
      json_object_string_add (json_vni, "advertisePip",
                              bgp->evpn_info->advertise_pip ? "Enabled"
                                                            : "Disabled");
      json_object_string_addf (json_vni, "sysIP", "%pI4",
                               &bgp->evpn_info->pip_ip);
      json_object_string_add (
          json_vni, "sysMAC",
          prefix_mac2str (&bgp->evpn_info->pip_rmac, buf2, sizeof (buf2)));
      json_object_string_add (
          json_vni, "rmac", prefix_mac2str (&bgp->rmac, buf2, sizeof (buf2)));
    }
  else
    {
...
static void
show_vni_entry (struct hash_bucket *bucket, void *args[])
{
  struct vty *vty;
  json_object *json;
  json_object *json_vni = NULL;
  ...
  if (json)
    {
      ...
      json_object_string_add (json_vni, "inKernel",
                              is_vni_live (vpn) ? "True" : "False");
      json_object_string_addf (json_vni, "rd", BGP_RD_AS_FORMAT (asnotation),
                               &vpn->prd);
      json_object_string_addf (json_vni, "originatorIp", "%pI4",
                               &vpn->originator_ip);
      ...
      if (! vpn->advertise_gw_macip && bgp_evpn &&
          bgp_evpn->advertise_gw_macip)
        json_object_string_add (json_vni, "advertiseGatewayMacip", "Active");
      else if (vpn->advertise_gw_macip)
        json_object_string_add (json_vni, "advertiseGatewayMacip", "Enabled");
      else
        json_object_string_add (json_vni, "advertiseGatewayMacip", "Disabled");
      if (! vpn->advertise_svi_macip && bgp_evpn &&
          bgp_evpn->evpn_info->advertise_svi_macip)
        json_object_string_add (json_vni, "advertiseSviMacIp", "Active");
      else if (vpn->advertise_svi_macip)
        json_object_string_add (json_vni, "advertiseSviMacIp", "Enabled");
      else
        json_object_string_add (json_vni, "advertiseSviMacIp", "Disabled");
    }
  else
    {
          ...
          for (; pi; pi = pi->next)
            {
              ...
              if (header)
                {
                  if (use_json)
                    {
                      json_object_int_add (json, "bgpTableVersion", tbl_ver);
                      json_object_string_addf (json, "bgpLocalRouterId",
                                               "%pI4", &bgp->router_id);
                      json_object_int_add (json, "defaultLocPrf",
                                           bgp->default_local_pref);
                      asn_asn2json (json, "localAS", bgp->as, bgp->asnotation);
                    }
                  else
                    {
              ...
              if (rd_header)
                {
                  ...
                  bgp_evpn_show_route_rd_header (vty, dest, json_nroute,
              ...
              if (option == SHOW_DISPLAY_TAGS)
                ...
                route_vty_out_overlay (vty, bgp_dest_get_prefix (rm), pi,
                                       no_display, json_array);
              else
                route_vty_out (vty, bgp_dest_get_prefix (rm), pi, no_display,
                               SAFI_EVPN, json_array, false);
              no_display = 1;
            }
          ...
          if (use_json && json_array)
            {
              ...
              json_object_string_addf (json_prefix_info, "prefix", "%pFX", p);

              json_object_int_add (json_prefix_info, "prefixLen",
                                   p->prefixlen);

              json_object_object_add (json_prefix_info, "paths", json_array);
              json_object_object_addf (json_nroute, json_prefix_info, "%pFX",
                                       p);
              json_array = NULL;
            }
        }
       ...
       JSON_STR)
{
  return bgp_show_ethernet_vpn (vty, NULL, bgp_show_type_normal, NULL,
                                SHOW_DISPLAY_STANDARD, use_json (argc, argv));
}
  ...
  if (argv_find (argv, argc, "large-community", &idx))
    {
      ...
      ret =
          bgp_show_ethernet_vpn (vty, NULL, show_type, lcom,
                                 SHOW_DISPLAY_STANDARD, use_json (argc, argv));
  ...
  else if (argv_find (argv, argc, "community", &idx))
    {
      ...
      ret =
          bgp_show_ethernet_vpn (vty, NULL, show_type, com,
                                 SHOW_DISPLAY_STANDARD, use_json (argc, argv));
      community_free (&com);
    }
       ...
       "Name of the route map\n")
{
  ...
  return bgp_static_set_safi (
      ...
      BGP_EVPN_IP_PREFIX_ROUTE, argv[idx_esi]->arg, argv[idx_gwip]->arg,
 ...
 */
static void
evpn_configure_vrf_rd (struct bgp *bgp_vrf, struct prefix_rd *rd,
 ...
 */
static void
evpn_unconfigure_vrf_rd (struct bgp *bgp_vrf)
{
  /* If we have already advertise type-5 routes with a diffrent RD, we
   * have to delete and withdraw them firs
   */
 ...
 */
static void
evpn_configure_rd (struct bgp *bgp, struct bgpevpn *vpn, struct prefix_rd *rd,
                   const char *rd_pretty)
{
 ...
 */
static void
evpn_show_vrf_import_rts (struct vty *vty, struct bgp *bgp_evpn,
                          json_object *json)
{
  void *args[2];
  ...
  hash_iterate (
      bgp_evpn->vrf_import_rt_hash,
      (void (*) (struct hash_bucket *, void *)) show_vrf_import_rt_entry,
      args);
}
 ...
 */
static void
evpn_show_import_rts (struct vty *vty, struct bgp *bgp, json_object *json)
{
  void *args[2];
 ...
 */
static void
evpn_show_routes_vni_all (struct vty *vty, struct bgp *bgp, int type,
                          ...
                          json_object *json, int detail)
{
  ...
  hash_iterate (bgp->vnihash,
                (void (*) (struct hash_bucket *, void *)) show_vni_routes_hash,
                &wctx);
}
 ...
 */
static void
evpn_show_routes_vni_all_type_all (struct vty *vty, struct bgp *bgp,
                                   ...
                                   int detail)
{
  ...
  hash_iterate (
      bgp->vnihash,
      (void (*) (struct hash_bucket *, void *)) show_vni_routes_all_hash,
      &wctx);
}
 ...
 */
static void
evpn_show_route_vni_multicast (struct vty *vty, struct bgp *bgp, vni_t vni,
                               struct in_addr orig_ip, json_object *json)
{
  struct bgpevpn *vpn;
 ...
 */
static void
evpn_show_route_vni_macip (struct vty *vty, struct bgp *bgp, vni_t vni,
                           ...
                           json_object *json)
{
  ...
  build_evpn_type2_prefix (&p, mac ? mac : &empty_mac, ip ? ip : &empty_ip);
  ...
  if (ip && mac)
    {
      for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
        {
          if (memcmp (mac, evpn_type2_path_info_get_mac (pi), sizeof (*mac)) ==
              0)
            break;
        }
  ...
  /* Prefix and num paths displayed once per prefix. */
  route_vty_out_detail_header (vty, bgp, dest, (struct prefix *) &p, NULL, afi,
                               safi, json, false);
  ...
  for (pi = bgp_dest_get_bgp_path_info (dest); pi; pi = pi->next)
    {
      ...
      if (ip && mac &&
          memcmp (mac, evpn_type2_path_info_get_mac (pi), sizeof (*mac)) != 0)
        continue;
      ...
      if (is_evpn_prefix_ipaddr_none (evp))
        {
          /* VNI MAC -> Global */
          evpn_type2_prefix_global_copy ((struct prefix_evpn *) &tmp_p, evp,
                                         NULL /* mac */,
                                         evpn_type2_path_info_get_ip (pi));
        }
      else
      ...
      route_vty_out_detail (vty, bgp, dest, (struct prefix *) &tmp_p, pi, afi,
                            safi, RPKI_NOT_BEING_USED, json_path);
...
static void
evpn_show_routes_esi (struct vty *vty, struct bgp *bgp, esi_t *esi,
                      json_object *json)
{
 ...
 */
static void
evpn_show_routes_vni (struct vty *vty, struct bgp *bgp, vni_t vni, int type,
                      bool mac_table, struct in_addr vtep_ip,
                      json_object *json)
{
  struct bgpevpn *vpn;
                          ...
                          struct ipaddr *ip, json_object *json)
{
  ...
  dest = bgp_safi_node_lookup (bgp->rib[afi][safi], safi, (struct prefix *) &p,
                               prd);
  if (! dest || ! bgp_dest_has_bgp_path_info_data (dest))
    {
      if (! json)
 ...
 */
static void
evpn_show_route_rd (struct vty *vty, struct bgp *bgp, struct prefix_rd *prd,
                    int type, json_object *json)
{
  ...
  if (json)
    {
      if (add_rd_to_json)
        json_object_object_addf (json, json_rd,
                                 BGP_RD_AS_FORMAT (bgp->asnotation), prd);
      else
        {
 ...
 */
static void
evpn_show_route_rd_all_macip (struct vty *vty, struct bgp *bgp,
                              struct ethaddr *mac, struct ipaddr *ip,
                              json_object *json)
{
  struct bgp_dest *rd_dest;
  ...
  for (rd_dest = bgp_table_top (bgp->rib[afi][safi]); rd_dest;
       rd_dest = bgp_route_next (rd_dest))
    {
      ...
      if (pi)
        {
          /* RD header - per RD. */
          bgp_evpn_show_route_rd_header (vty, rd_dest, json_rd, rd_str,
                                         RD_ADDRSTRLEN);
          prefix_cnt++;
        }
      ...
      if (json)
        {
          json_object_object_add (json_prefix, "paths", json_paths);
          json_object_object_addf (json_rd, json_prefix, "%pFX", p);
          if (add_rd_to_json)
                      ...
                      json_object *json, int detail, bool self_orig)
{
  ...
  for (rd_dest = bgp_table_top (bgp->rib[afi][safi]); rd_dest;
       rd_dest = bgp_route_next (rd_dest))
    {
      ...
      for (dest = bgp_table_top (table); dest; dest = bgp_route_next (dest))
        {
          json_object *json_prefix = NULL; /* contains prefix under a RD */
          json_object *json_paths = NULL;  /* array of paths under a prefix*/
          const struct prefix_evpn *evp =
              (const struct prefix_evpn *) bgp_dest_get_prefix (dest);
          int add_prefix_to_json = 0;
          const struct prefix *p = bgp_dest_get_prefix (dest);
          ...
          if (detail)
            route_vty_out_detail_header (
                vty, bgp, dest, bgp_dest_get_prefix (dest),
                (struct prefix_rd *) rd_destp, AFI_L2VPN, SAFI_EVPN,
                json_prefix, false);
              ...
              if (detail)
                {
                  route_vty_out_detail (
                      ...
                      AFI_L2VPN, SAFI_EVPN, RPKI_NOT_BEING_USED, json_path);
                }
              else
                route_vty_out (vty, p, pi, 0, SAFI_EVPN, json_path, false);
...
int
bgp_evpn_show_all_routes (struct vty *vty, struct bgp *bgp, int type,
 ...
 */
static void
evpn_show_vni (struct vty *vty, struct bgp *bgp, vni_t vni, json_object *json)
{
  uint8_t found = 0;
  struct bgpevpn *vpn;
 ...
 */
static void
evpn_show_all_vnis (struct vty *vty, struct bgp *bgp, json_object *json)
{
  void *args[2];
  ...
  if (! json)
    {
      ...
      vty_out (vty, "  %-10s %-4s %-21s %-25s %-25s %-37s\n", "VNI", "Type",
               "RD", "Import RT", "Export RT", "Tenant VRF");
    }
 ...
 */
static void
evpn_set_advertise_svi_macip (struct bgp *bgp, struct bgpevpn *vpn,
                              uint32_t set)
{
  if (! vpn)
    {
      ...
      bgp_zebra_advertise_svi_macip (bgp, bgp->evpn_info->advertise_svi_macip,
                                     0);
    }
  else
    {
 ...
 */
static void
evpn_set_advertise_default_gw (struct bgp *bgp, struct bgpevpn *vpn)
{
  if (! vpn)
    {
 ...
 */
static void
evpn_unset_advertise_default_gw (struct bgp *bgp, struct bgpevpn *vpn)
{
  if (! vpn)
    {
      if (! bgp->advertise_gw_macip)
 ...
 */
static void
evpn_process_default_originate_cmd (struct bgp *bgp_vrf, afi_t afi, bool add)
{
 ...
 */
static void
evpn_set_advertise_subnet (struct bgp *bgp, struct bgpevpn *vpn)
{
 ...
 */
static void
evpn_unset_advertise_subnet (struct bgp *bgp, struct bgpevpn *vpn)
{
  if (! vpn->advertise_subnet)
    return;
...
static void
bgp_evpn_set_unset_resolve_overlay_index (struct bgp *bgp, bool set)
{
 ...
 */
static void
evpn_set_advertise_autort_rfc8365 (struct bgp *bgp)
{
 ...
 */
static void
evpn_unset_advertise_autort_rfc8365 (struct bgp *bgp)
{
...
static void
write_vni_config (struct vty *vty, struct bgpevpn *vpn)
{
  char *ecom_str;
  struct listnode *node, *nnode;
  struct ecommunity *ecom;
      ...
      if (is_import_rt_configured (vpn))
        {
          for (ALL_LIST_ELEMENTS (vpn->import_rtl, node, nnode, ecom))
            {
              ecom_str =
                  ecommunity_ecom2str (ecom, ECOMMUNITY_FORMAT_ROUTE_MAP, 0);
       ...
       "Duplicate address detection freeze time (30-3600)\n")
{
  ...
  if (argc == 2)
  ...
  else
    {
      if (max_moves)
        {
          if (bgp_vrf->evpn_info->dad_max_moves != max_moves)
            {
              vty_out (vty, "%% Value does not match with config\n");
              return CMD_SUCCESS;
            }
          bgp_vrf->evpn_info->dad_max_moves = EVPN_DAD_DEFAULT_MAX_MOVES;
        }
      ...
      if (freeze_time)
        {
          if (bgp_vrf->evpn_info->dad_freeze_time != freeze_time)
            {
              vty_out (vty, "%% Value does not match with config\n");
              return CMD_SUCCESS;
            }
       ...
       "Name of the route map\n")
{
  ...
  if ((oly != OVERLAY_INDEX_TYPE_NONE) && (oly != OVERLAY_INDEX_GATEWAY_IP))
    {
       ...
       MAC_STR)
{
  ...
  if (! bgp_vrf || EVPN_ENABLED (bgp_vrf))
    {
      vty_out (vty, "This command is supported under L3VNI BGP EVPN VRF\n");
      return CMD_WARNING_CONFIG_FAILED;
    }
  bgp_evpn = bgp_get_evpn ();
      ...
      if (ip.s_addr != INADDR_ANY)
        {
          ...
          if (IPV4_ADDR_SAME (&ip, &bgp_vrf->evpn_info->pip_ip_static))
            return CMD_SUCCESS;
      ...
      if (! is_zero_mac (&mac->eth_addr))
        {
          ...
          if (memcmp (&bgp_vrf->evpn_info->pip_rmac_static, &mac->eth_addr,
                      ETH_ALEN) == 0)
            return CMD_SUCCESS;

          memcpy (&bgp_vrf->evpn_info->pip_rmac_static, &mac->eth_addr,
                  ETH_ALEN);
          memcpy (&bgp_vrf->evpn_info->pip_rmac,
                  &bgp_vrf->evpn_info->pip_rmac_static, ETH_ALEN);
        }
      else
        {
          /* Copy zebra sys mac */
          if (! is_zero_mac (&bgp_vrf->evpn_info->pip_rmac_zebra))
            memcpy (&bgp_vrf->evpn_info->pip_rmac,
                    &bgp_vrf->evpn_info->pip_rmac_zebra, ETH_ALEN);
        }
    }
  else
    {
      if (argc == 2)
        {
          ...
          /* copy anycast mac */
          memcpy (&bgp_vrf->evpn_info->pip_rmac, &bgp_vrf->rmac, ETH_ALEN);
        }
      else
        {
          /* remove MAC-IP option retain PIP knob. */
          if ((ip.s_addr != INADDR_ANY) &&
              ! IPV4_ADDR_SAME (&ip, &bgp_vrf->evpn_info->pip_ip_static))
            {
          ...
          if (! is_zero_mac (&mac->eth_addr) &&
              memcmp (&bgp_vrf->evpn_info->pip_rmac_static, &mac->eth_addr,
                      ETH_ALEN) != 0)
            {
              vty_out (vty, "%% BGP EVPN PIP MAC does not match\n");
              return CMD_WARNING_CONFIG_FAILED;
            }
          ...
          if (memcmp (&bgp_vrf->evpn_info->pip_rmac,
                      &bgp_vrf->evpn_info->pip_rmac_static, ETH_ALEN) == 0)
            {
              /* Copy zebra sys mac */
              if (! is_zero_mac (&bgp_vrf->evpn_info->pip_rmac_zebra))
                memcpy (&bgp_vrf->evpn_info->pip_rmac,
                ...
                {
                  /* copy anycast mac */
                  memcpy (&bgp_vrf->evpn_info->pip_rmac, &bgp_vrf->rmac,
                          ETH_ALEN);
                }
            }
        }
      ...
      if (uj)
        {
          ...
          json_object_string_add (json, "flooding",
                                  bgp_evpn->vxlan_flood_ctrl ==
                                          VXLAN_FLOOD_HEAD_END_REPL
                                      ? "Head-end replication"
                                      : "Disabled");
          json_object_string_add (json, "vxlanFlooding",
                                  bgp_evpn->vxlan_flood_ctrl ==
                                          VXLAN_FLOOD_HEAD_END_REPL
                                      ? "Enabled"
                                      : "Disabled");
      ...
      else
        {
          vty_out (vty, "Advertise Gateway Macip: %s\n",
                   bgp_evpn->advertise_gw_macip ? "Enabled" : "Disabled");
          vty_out (vty, "Advertise SVI Macip: %s\n",
              ...
              "Remote IP hash\n")
{
  ...
  hash_iterate (
      bgp_evpn->vnihash,
      (void (*) (struct hash_bucket *, void *)) bgp_evpn_show_remote_ip_hash,
      vty);
              ...
              "Show vni-svi-hash\n")
{
  ...
  hash_iterate (
      bgp_evpn->vni_svi_hash,
      (void (*) (struct hash_bucket *, void *)) bgp_evpn_show_vni_svi_hash,
      vty);
...
static int
bgp_evpn_cli_parse_type_cmp (int *type, const char *type_str)
{
...
int
bgp_evpn_cli_parse_type (int *type, struct cmd_token **argv, int argc)
{
  int type_idx = 0;
  ...
  if (argv_find (argv, argc, "type", &type_idx))
    {
      ...
      if (bgp_evpn_cli_parse_type_cmp (type, argv[type_idx + 1]->arg) != 0)
        return -1;
    }
...
DEFUN (show_bgp_l2vpn_evpn_route_rd,
       ...
       "show bgp l2vpn evpn route rd <ASN:NN_OR_IP-ADDRESS:NN|all> [type "
       EVPN_TYPE_ALL_LIST
       "] [json]",
       SHOW_STR
       BGP_STR
       ...
       JSON_STR)
{
  ...
  if ((! uj && ((argc == (idx + 1 + 7)) && argv[idx + 7]->arg != NULL)) ||
      (uj && ((argc == (idx + 1 + 8)) && argv[idx + 7]->arg != NULL)))
    {
       ...
       JSON_STR)
{
  ...
  if ((! uj && (argc == (idx + 1 + 5 + da) && argv[idx + 5 + da]->arg)) ||
      (uj && (argc == (idx + 1 + 6 + da) && argv[idx + 5 + da]->arg)))
    {
       ...
       JSON_STR)
{
  ...
  evpn_show_routes_vni_all (vty, bgp, BGP_EVPN_MAC_IP_ROUTE, true, addr, json,
                            ! ! detail);
       ...
       JSON_STR)
{
  ...
  evpn_show_routes_vni_all (vty, bgp, BGP_EVPN_MAC_IP_ROUTE, false, addr, json,
                            ! ! detail);
       ...
       JSON_STR)
{
  ...
  evpn_show_routes_vni_all (vty, bgp, BGP_EVPN_IMET_ROUTE, false, addr, json,
                            ! ! detail);
       ...
       JSON_STR)
{
  ...
  evpn_show_routes_vni (vty, bgp, vni, BGP_EVPN_AD_ROUTE, false, addr, json);
       ...
       JSON_STR)
{
  ...
  evpn_show_routes_vni (vty, bgp, vni, BGP_EVPN_IMET_ROUTE, false, addr, json);
...
DEFPY_HIDDEN (show_bgp_l2vpn_evpn_route_mac_ip_evi_es,
...
DEFPY_HIDDEN (show_bgp_l2vpn_evpn_route_mac_ip_global_es,
...
ALIAS_HIDDEN (show_bgp_l2vpn_evpn_summary,
...
ALIAS_HIDDEN (show_bgp_l2vpn_evpn_route,
...
ALIAS_HIDDEN (show_bgp_l2vpn_evpn_route_rd,
...
ALIAS_HIDDEN (show_bgp_l2vpn_evpn_route_rd_macip,
...
ALIAS_HIDDEN (show_bgp_l2vpn_evpn_route_vni,
              show_bgp_evpn_route_vni_cmd,
              "show bgp evpn route vni "
              CMD_VNI_RANGE
...
ALIAS_HIDDEN (show_bgp_l2vpn_evpn_route_vni_all,
...
ALIAS_HIDDEN (show_bgp_l2vpn_evpn_import_rt,
 ...
 */
static bool
bgp_evpn_rt_matches_existing (struct list *rtl, struct ecommunity *ecomtarget)
{
  struct listnode *node;
  struct ecommunity *ecom;
 ...
 */
static bool
bgp_evpn_vrf_rt_matches_existing (struct list *rtl,
       ...
       JSON_STR)
{
  ...
  if (! json)
  ...
  else
    {
      ...
      json_object_string_addf (json, "local-ip", "%pI4", &bgp->originator_ip);
      ...
      for (ALL_LIST_ELEMENTS_RO (bgp->vrf_export_rtl, node, l3rt))
        json_object_array_add (
            json_export_rts,
            json_object_new_string (ecommunity_str (l3rt->ecom)));
...
static int
add_rt (struct bgp *bgp, struct ecommunity *ecom, bool is_import,
        bool is_wildcard)
{
  /* Do nothing if we already have this route-target */
  if (is_import)
    {
      if (! bgp_evpn_vrf_rt_matches_existing (bgp->vrf_import_rtl, ecom))
    ...
    {
      if (! bgp_evpn_vrf_rt_matches_existing (bgp->vrf_export_rtl, ecom))
...
static int
del_rt (struct bgp *bgp, struct ecommunity *ecom, bool is_import)
{
...
static int
parse_rtlist (struct bgp *bgp, struct vty *vty, int argc,
              struct cmd_token **argv, int rt_idx, bool is_add, bool is_import)
{
  int ret = CMD_SUCCESS;
  ...
  for (int i = rt_idx; i < argc; i++)
    {
      ...
      ecom = ecommunity_str2com (argv[i]->arg, ECOMMUNITY_ROUTE_TARGET, 0);
  ...
  if (rt_type == RT_TYPE_IMPORT)
    {
      if (! CHECK_FLAG (bgp->vrf_flags, BGP_VRF_IMPORT_RT_CFGD))
        {
          vty_out (vty, "%% Import RT is not configured for this VRF\n");
          return CMD_WARNING_CONFIG_FAILED;
        }
    }
  else if (rt_type == RT_TYPE_EXPORT)
    {
      if (! CHECK_FLAG (bgp->vrf_flags, BGP_VRF_EXPORT_RT_CFGD))
        {
          vty_out (vty, "%% Export RT is not configured for this VRF\n");
          return CMD_WARNING_CONFIG_FAILED;
        }
    }
...
DEFPY (bgp_evpn_ead_ess_frag_evi_limit,
       ...
       "limit\n")
{
  bgp_mh_info->evi_per_es_frag = no ? BGP_EVPN_MAX_EVI_PER_ES_FRAG : limit;

  return CMD_SUCCESS;
}

DEFUN (bgp_evpn_ead_es_rt,
       ...
       "Route target (A.B.C.D:MN|EF:OPQR|GHJK:MN)\n")
{
  ...
  if (! bgp_evpn_rt_matches_existing (bgp_mh_info->ead_es_export_rtl, ecomadd))
       ...
       EVPN_ASN_IP_HELP_STR)
{
  ...
  if (! bgp_evpn_rt_matches_existing (bgp_mh_info->ead_es_export_rtl, ecomdel))
    {
...
static int
vni_cmp (const void **a, const void **b)
{
...
bgp_ethernetvpn_init (void)
{
  ...
  install_element (VIEW_NODE, &show_ip_bgp_l2vpn_evpn_neighbor_routes_cmd);
  ...
  install_element (BGP_EVPN_NODE, &bgp_evpn_enable_resolve_overlay_index_cmd);
  ...
  install_element (VIEW_NODE, &show_bgp_l2vpn_evpn_route_vni_multicast_cmd);
  ...
  install_element (VIEW_NODE, &show_bgp_l2vpn_evpn_route_mac_ip_evi_es_cmd);
  install_element (VIEW_NODE, &show_bgp_l2vpn_evpn_route_mac_ip_global_es_cmd);
  ...
  install_element (BGP_EVPN_VNI_NODE, &bgp_evpn_advertise_default_gw_vni_cmd);
  install_element (BGP_EVPN_VNI_NODE,
                   &no_bgp_evpn_advertise_default_gw_vni_cmd);
  install_element (BGP_EVPN_VNI_NODE, &bgp_evpn_advertise_vni_subnet_cmd);
  install_element (BGP_EVPN_VNI_NODE, &no_bgp_evpn_advertise_vni_subnet_cmd);
}
