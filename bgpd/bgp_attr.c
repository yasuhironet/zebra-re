...
{
  cluster_hash =
      hash_create (cluster_hash_key_make, cluster_hash_cmp, "BGP Cluster");
}
 ...
 */
static bool
encap_same (const struct bgp_attr_encap_subtlv *h1,
...
static unsigned int
encap_hash_key_make (const void *p)
{
...
{
  encap_hash =
      hash_create (encap_hash_key_make, encap_hash_cmp, "BGP Encap Hash");
#ifdef ENABLE_BGP_VNC
...
static bool
bgp_attr_aigp_get_tlv_metric (uint8_t *pnt, int length, uint64_t *aigp)
{
  uint8_t *data = pnt;
  uint8_t tlv_type;
  uint16_t tlv_length;
...
static uint64_t
bgp_aigp_metric_total (struct bgp_path_info *bpi)
{
...
static void
stream_put_bgp_aigp_tlv_metric (struct stream *s, struct bgp_path_info *bpi)
{
...
static bool
bgp_attr_aigp_valid (uint8_t *pnt, int length)
{
  uint8_t *data = pnt;
  uint8_t tlv_type;
  uint16_t tlv_length;
  ...
  while (length)
    {
      ...
      if (length < tlv_length)
        {
          zlog_err ("Bad AIGP attribute length: %u, but TLV length: %u",
                    length, tlv_length);
          return false;
        }
      ...
      if (tlv_type == BGP_AIGP_TLV_METRIC &&
        ...
        {
          zlog_err ("Bad AIGP TLV (%s) length: %u", BGP_AIGP_TLV_METRIC_DESC,
                    tlv_length);
          return false;
        }
...
static void *
srv6_l3vpn_hash_alloc (void *p)
{
  return p;
}

static void
srv6_l3vpn_free (struct bgp_attr_srv6_l3vpn *l3vpn)
{
...
static void
srv6_l3vpn_unintern (struct bgp_attr_srv6_l3vpn **l3vpnp)
{
...
static void *
srv6_vpn_hash_alloc (void *p)
{
  return p;
}

static void
srv6_vpn_free (struct bgp_attr_srv6_vpn *vpn)
{
  XFREE (MTYPE_BGP_SRV6_VPN, vpn);
}

static struct bgp_attr_srv6_vpn *
srv6_vpn_intern (struct bgp_attr_srv6_vpn *vpn)
{
...
static void
srv6_vpn_unintern (struct bgp_attr_srv6_vpn **vpnp)
{
...
static uint32_t
srv6_l3vpn_hash_key_make (const void *p)
{
...
static bool
srv6_l3vpn_hash_cmp (const void *p1, const void *p2)
{
...
static bool
srv6_l3vpn_same (const struct bgp_attr_srv6_l3vpn *h1,
...
static unsigned int
srv6_vpn_hash_key_make (const void *p)
{
...
static bool
srv6_vpn_hash_cmp (const void *p1, const void *p2)
{
...
static bool
srv6_vpn_same (const struct bgp_attr_srv6_vpn *h1,
...
srv6_init (void)
{
  srv6_l3vpn_hash = hash_create (srv6_l3vpn_hash_key_make, srv6_l3vpn_hash_cmp,
...
static void
attr_show_all_iterator (struct hash_bucket *bucket, struct vty *vty)
{
...
attr_show_all (struct vty *vty)
{
  hash_iterate (
      attrhash,
      (void (*) (struct hash_bucket *, void *)) attr_show_all_iterator, vty);
}
...
struct attr *
bgp_attr_default_set (struct attr *attr, struct bgp *bgp, uint8_t origin)
{
...
struct attr *
bgp_attr_aggregate_intern (struct bgp *bgp, uint8_t origin,
...
)
{
  ...
  for (i = 0; i <= 2; i++) /* O,T,P, but not E */
    if (CHECK_FLAG (desired_flags, attr_flag_str[i].key) !=
      ...
      {
        flog_err (EC_BGP_ATTR_FLAG, "%s attribute must%s be flagged as \"%s\"",
  ...
  if (! seen)
    {
      zlog_debug (
          ...
          __func__, lookup_msg (attr_str, attr_code, NULL), real_flags,
          desired_flags);
    }
}
...
bgp_attr_origin (struct bgp_attr_parser_args *args)
{
  ...
  if (length != 1)
    {
      flog_err (EC_BGP_ATTR_LEN, "Origin attribute length is not one %d",
                length);
  ...
  if ((attr->origin != BGP_ORIGIN_IGP) && (attr->origin != BGP_ORIGIN_EGP) &&
    ...
    {
      flog_err (EC_BGP_ATTR_ORIGIN, "Origin attribute value is invalid %d",
                attr->origin);
...
bgp_attr_aspath (struct bgp_attr_parser_args *args)
{
  ...
  if (! attr->aspath)
    {
      flog_err (EC_BGP_ATTR_MAL_AS_PATH,
                "Malformed AS path from %s, length is %d", peer->host, length);
  ...
  if (peer->bgp && peer->bgp->reject_as_sets &&
    ...
    {
      flog_err (EC_BGP_ATTR_MAL_AS_PATH,
                "AS_SET and AS_CONFED_SET are deprecated from %pBP", peer);
      return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_MAL_AS_PATH, 0);
    }
  ...
  if (CHECK_FLAG (peer->flags, PEER_FLAG_ENFORCE_FIRST_AS))
    {
      if (peer->sort == BGP_PEER_EBGP &&
        ...
        {
          flog_err (EC_BGP_ATTR_FIRST_AS, "%s incorrect first AS (must be %u)",
                    peer->host, peer->as);
          return BGP_ATTR_PARSE_WITHDRAW;
        }
    }
...
bgp_attr_nexthop (struct bgp_attr_parser_args *args)
{
  ...
  if (length != 4)
    {
      flog_err (EC_BGP_ATTR_LEN, "Nexthop attribute length isn't four [%d]",
                length);
...
bgp_attr_med (struct bgp_attr_parser_args *args)
{
  ...
  if (length != 4)
    {
      flog_err (EC_BGP_ATTR_LEN, "MED attribute length isn't four [%d]",
                length);
...
bgp_attr_local_pref (struct bgp_attr_parser_args *args)
{
  ...
  if (peer->sort == BGP_PEER_IBGP && length != 4)
    {
      flog_err (EC_BGP_ATTR_LEN, "LOCAL_PREF attribute length isn't 4 [%u]",
                length);
      return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
                                 args->total);
    }
...
bgp_attr_aggregator (struct bgp_attr_parser_args *args)
{
  ...
  if (length != wantedlen)
    {
      flog_err (EC_BGP_ATTR_LEN, "AGGREGATOR attribute length isn't %u [%u]",
                wantedlen, length);
      return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
                                 args->total);
    }
  ...
  if (aggregator_as == BGP_AS_ZERO)
    {
      flog_err (EC_BGP_ATTR_LEN,
                "%s: AGGREGATOR AS number is 0 for aspath: %s", peer->host,
                         ...
                         struct in_addr *as4_aggregator_addr)
{
  ...
  *as4_aggregator_as = aggregator_as;
  ...
  if (aggregator_as == BGP_AS_ZERO)
    {
      flog_err (EC_BGP_ATTR_LEN,
                "%s: AS4_AGGREGATOR AS number is 0 for aspath: %s", peer->host,
                aspath_print (attr->aspath));
      ...
      if (BGP_DEBUG (as4, AS4))
        {
          ...
          if (attr->flag & (ATTR_FLAG_BIT (BGP_ATTR_AS4_AGGREGATOR)))
            zlog_debug ("[AS4] %s %s AS4_AGGREGATOR", peer->host,
                        "AS4 capable peer, yet it sent");
        }
...
bgp_attr_community (struct bgp_attr_parser_args *args)
{
  ...
  bgp_attr_set_community (
      attr, community_parse ((uint32_t *) stream_pnt (peer->curr), length));
...
#define LEN_LEFT              (length - (stream_get_getp (s) - start))
  if ((length > STREAM_READABLE (s)) || (length < BGP_MP_REACH_MIN_SIZE))
    {
      zlog_info ("%s: %s sent invalid length, %lu, of MP_REACH_NLRI", __func__,
  ...
  switch (attr->mp_nexthop_len)
    ...
    case BGP_ATTR_NHLEN_IPV4:
      ...
      if (attr->nexthop.s_addr == INADDR_ANY)
        memcpy (&attr->nexthop.s_addr, &attr->mp_nexthop_global_in,
                IPV4_MAX_BYTELEN);
      break;
      ...
      if (IN6_IS_ADDR_LINKLOCAL (&attr->mp_nexthop_global))
        {
          if (! peer->nexthop.ifp)
            {
              zlog_warn (
                  "%s sent a v6 global attribute but address is a V6 LL and there's no peer interface information. Hence, withdrawing",
                  peer->host);
              return BGP_ATTR_PARSE_WITHDRAW;
            }
    ...
    case BGP_ATTR_NHLEN_VPNV6_GLOBAL_AND_LL:
      ...
      if (IN6_IS_ADDR_LINKLOCAL (&attr->mp_nexthop_global))
        {
          if (! peer->nexthop.ifp)
            {
              zlog_warn (
                  "%s sent a v6 global and LL attribute but global address is a V6 LL and there's no peer interface information. Hence, withdrawing",
                  peer->host);
              return BGP_ATTR_PARSE_WITHDRAW;
            }
          attr->nh_ifindex = peer->nexthop.ifp->ifindex;
      ...
      if (! IN6_IS_ADDR_LINKLOCAL (&attr->mp_nexthop_local))
        {
          if (bgp_debug_update (peer, NULL, NULL, 1))
            zlog_debug (
                ...
                peer->host, &attr->mp_nexthop_global, &attr->mp_nexthop_local);
      ...
      if (! peer->nexthop.ifp)
        {
          zlog_warn (
              "%s sent a v6 LL next-hop and there's no peer interface information. Hence, withdrawing",
              peer->host);
          return BGP_ATTR_PARSE_WITHDRAW;
        }
  ...
  if (! LEN_LEFT)
    {
      zlog_info ("%s: %s sent SNPA which couldn't be read", __func__,
                 peer->host);
      return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
    }
  ...
  if (nlri_len > STREAM_READABLE (s))
    {
      zlog_info ("%s: %s sent MP_REACH_NLRI which couldn't be read", __func__,
                 peer->host);
      return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
    }
  ...
  if (! nlri_len)
    {
      zlog_info (
          "%s: %s sent a zero-length NLRI. Hence, treating as a EOR marker",
          __func__, peer->host);
...
bgp_attr_large_community (struct bgp_attr_parser_args *args)
{
  ...
  bgp_attr_set_lcommunity (attr,
                           lcommunity_parse (stream_pnt (peer->curr), length));
  ...
  if (BGP_ATTR_ENCAP == type)
    {
      ...
      if (tlv_length != length)
        {
          zlog_info ("%s: tlv_length(%d) != length(%d)", __func__, tlv_length,
                     length);
        }
    }
  ...
  while (length >= 4)
    {
      ...
      if (BGP_ATTR_ENCAP == type)
      ...
      else
        {
          ...
          for (stlv_last = vnc_subtlvs; stlv_last && stlv_last->next;
               stlv_last = stlv_last->next)
            ;
          if (stlv_last)
            stlv_last->next = tlv;
  ...
  if (length < BGP_PREFIX_SID_SRV6_L3_SERVICE_SID_STRUCTURE_LENGTH)
    {
      flog_err (
          ...
          BGP_PREFIX_SID_SRV6_L3_SERVICE_SID_STRUCTURE_LENGTH, length);
      return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
                                 args->total);
    }
      ...
      if (attr->srv6_l3vpn)
        {
          ...
          return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_MAL_ATTR,
                                     args->total);
        }
      attr->srv6_l3vpn =
                   ...
                   struct bgp_attr_parser_args *args)
{
  ...
  if (type == BGP_PREFIX_SID_LABEL_INDEX)
  ...
  else if (type == BGP_PREFIX_SID_IPV6)
    {
      if (length != BGP_PREFIX_SID_IPV6_LENGTH)
        {
          flog_err (EC_BGP_ATTR_LEN,
                    "Prefix SID IPv6 length is %hu instead of %u", length,
      ...
      if (length < (2 + BGP_PREFIX_SID_ORIGINATOR_SRGB_LENGTH))
        {
          flog_err (
              ...
              length, 2 + BGP_PREFIX_SID_ORIGINATOR_SRGB_LENGTH);
          return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
                                     args->total);
        }
  ...
  else if (type == BGP_PREFIX_SID_VPN_SID)
    {
      if (length != BGP_PREFIX_SID_VPN_SID_LENGTH)
        {
          flog_err (EC_BGP_ATTR_LEN,
                    "Prefix SID VPN SID length is %hu instead of %u", length,
      ...
      stream_get (&ipv6_sid, peer->curr, sizeof (ipv6_sid)); /* sid_value */
      ...
      attr->srv6_vpn =
          XCALLOC (MTYPE_BGP_SRV6_VPN, sizeof (struct bgp_attr_srv6_vpn));
  ...
  /* Placeholder code for Unsupported TLV */
    ...
    {
      if (bgp_debug_update (peer, NULL, NULL, 1))
        zlog_debug ("%s attr Prefix-SID sub-type=%u is not supported, skipped",
                    peer->host, type);
...
bgp_attr_pmsi_tunnel (struct bgp_attr_parser_args *args)
{
  ...
  if (tnl_type > PMSI_TNLTYPE_MAX)
    {
      flog_err (EC_BGP_ATTR_PMSI_TYPE, "Invalid PMSI tunnel attribute type %d",
                tnl_type);
      return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_OPT_ATTR_ERR,
                                 args->total);
    }
      ...
      if (length != 9)
        {
          flog_err (EC_BGP_ATTR_PMSI_LEN,
                    "Bad PMSI tunnel attribute length %d for IR", length);
          return bgp_attr_malformed (args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
                                     args->total);
        }
    }
...
static enum bgp_attr_parse_ret
bgp_attr_aigp (struct bgp_attr_parser_args *args)
{
  struct peer *const peer = args->peer;
  struct attr *const attr = args->attr;
  const bgp_size_t length = args->length;
...
/* OTC attribute. */
static enum bgp_attr_parse_ret
bgp_attr_otc (struct bgp_attr_parser_args *args)
{
  struct peer *const peer = args->peer;
  struct attr *const attr = args->attr;
  const bgp_size_t length = args->length;
...
bgp_attr_unknown (struct bgp_attr_parser_args *args)
{
  ...
  transit->val =
      XREALLOC (MTYPE_TRANSIT_VAL, transit->val, transit->length + total);
...
static int
bgp_attr_check (struct peer *peer, struct attr *attr, bgp_size_t length)
{
  uint8_t type = 0;
   ...
   */
  if (CHECK_FLAG (peer->cap, PEER_CAP_RESTART_RCV) && ! attr->flag && ! length)
    return BGP_ATTR_PARSE_WITHDRAW;
  ...
  if (! CHECK_FLAG (attr->flag, ATTR_FLAG_BIT (BGP_ATTR_MP_REACH_NLRI)) &&
    ...
    return type ? BGP_ATTR_PARSE_MISSING_MANDATORY : BGP_ATTR_PARSE_PROCEED;
  ...
  while (BGP_INPUT_PNT (peer) < endp)
    {
      ...
      if (bgp_attr_flag_invalid (&attr_args))
        {
          ret = bgp_attr_malformed (
              &attr_args, BGP_NOTIFY_UPDATE_ATTR_FLAG_ERR, attr_args.total);
          if (ret == BGP_ATTR_PARSE_PROCEED)
            continue;
      ...
      switch (type)
        ...
        case BGP_ATTR_AGGREGATOR:
          ...
          break;
        case BGP_ATTR_AS4_AGGREGATOR:
          ret = bgp_attr_as4_aggregator (&attr_args, &as4_aggregator,
                                         &as4_aggregator_addr);
          break;
        case BGP_ATTR_COMMUNITIES:
      ...
      if (BGP_INPUT_PNT (peer) != attr_endp)
        {
          flog_warn (EC_BGP_ATTRIBUTE_FETCH_ERROR,
                     "%s: BGP attribute %s, fetch error", peer->host,
                     lookup_msg (attr_str, type, NULL));
          bgp_notify_send (peer, BGP_NOTIFY_UPDATE_ERR,
                           BGP_NOTIFY_UPDATE_ATTR_LENG_ERR);
          ret = BGP_ATTR_PARSE_ERROR;
 ...
 */
void
bgp_attr_extcom_tunnel_type (struct attr *attr, bgp_encap_types *tunnel_type)
{
...
size_t
bgp_packet_mpattr_start (struct stream *s, struct peer *peer, afi_t afi,
                         ...
                         struct attr *attr)
{
  ...
  switch (nh_afi)
    ...
    case AFI_L2VPN:
      if (safi != SAFI_FLOWSPEC)
        flog_err (EC_BGP_ATTR_NH_SEND_LEN,
...
size_t
bgp_packet_mpattr_prefix_size (afi_t afi, safi_t safi, const struct prefix *p)
{
  ...
  switch (safi)
    ...
    case SAFI_MAX:
      assert (
          ! "Attempting to figure size for a SAFI_UNSPEC/SAFI_MAX this is a DEV ESCAPE");
      break;
    case SAFI_UNICAST:
    case SAFI_MULTICAST:
      break;
    ...
    case SAFI_EVPN:
      ...
      if (afi == AFI_L2VPN)
        ...
        assert (
            ! "Attempting to figure size for SAFI_EVPN and !AFI_L2VPN and FRR will not have the proper values");
      break;
    case SAFI_FLOWSPEC:
 ...
 */
static void
bgp_packet_mpattr_tea (struct bgp *bgp, struct peer *peer, struct stream *s,
                       struct attr *attr, uint8_t attrtype)
{
...
static bool
bgp_append_local_as (struct peer *peer, afi_t afi, safi_t safi)
{
...
bgp_size_t
bgp_packet_attribute (struct bgp *bgp, struct peer *peer, struct stream *s,
...
void
bgp_packet_mpunreach_prefix (struct stream *s, const struct prefix *p,
...
/* Make attribute packet. */
void
bgp_dump_routes_attr (struct stream *s, struct bgp_path_info *bpi,
...
void
bgp_path_attribute_discard_vty (struct vty *vty, struct peer *peer,
...
void
bgp_path_attribute_withdraw_vty (struct vty *vty, struct peer *peer,
...
enum bgp_attr_parse_ret
bgp_attr_ignore (struct peer *peer, uint8_t type)
{
