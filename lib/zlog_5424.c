...
zlog_5424_bufsz (struct zlt_5424 *zte, size_t nmsgs, size_t *low_space)
{
  size_t ret = 128;
...
static size_t
zlog_5424_one (struct zlt_5424 *zte, struct zlog_msg *msg, struct state *state)
{
  size_t textlen;
  ...
  need +=
      bprintfrr (fbuf, " %s %s %jd %.*s ", cmd_hostname_get () ?: "-",
                 zlog_progname, pid, (int) (zlog_prefixsz - 2), zlog_prefix);
  ...
  if (xrefdata)
    {
      ...
      if (zte->kw_location)
        need += bprintfrr (fbuf, " file=\"%s\" line=\"%d\" func=\"%s\"",
...
static size_t
zlog_3164_one (struct zlt_5424 *zte, struct zlog_msg *msg, struct state *state)
{
  size_t textlen;
  struct fbuf *fbuf = state->fbuf;
  char *orig_pos = fbuf->pos;
...
static size_t
zlog_journald_one (struct zlt_5424 *zte, struct zlog_msg *msg,
                   struct state *state)
{
  size_t textlen;
  struct fbuf *fbuf = state->fbuf;
...
static size_t
zlog_one (struct zlt_5424 *zte, struct zlog_msg *msg, struct state *state)
{
  switch (zte->fmt)
    {
    case ZLOG_FMT_5424:
...
static void
zlog_5424_err (struct zlt_5424 *zte, size_t count)
{
  if (! count)
    {
      zte->current_err = 0;
...
static void
zlog_5424 (struct zlog_target *zt, struct zlog_msg *msgs[], size_t nmsgs)
{
  size_t i;
      ...
      if (prio <= zte->zt.prio_min)
        {
          ...
          if (zte->packets)
            {
              mpos->msg_hdr.msg_iovlen = state.iov - mpos->msg_hdr.msg_iov;
              mpos++;
            }
          count++;
        }
          ...
          if (zte->packets)
            {
              ...
              for (sendpos = mmsg; sendpos < mpos;)
                {
                  ret = sendmmsg (fd, sendpos, mpos - sendpos, 0);
                  if (ret <= 0)
                    break;
                  sendpos += ret;
                }
 ...
 */
static void
gmtime_assafe (time_t ts, struct tm *res)
{
  res->tm_sec = ts % 60;
  ts /= 60;
  res->tm_min = ts % 60;
 ...
 */
static void
zlog_5424_sigsafe (struct zlog_target *zt, const char *text, size_t len)
{
  ...
  switch (zte->fmt)
    ...
    case ZLOG_FMT_5424:
      ...
      bprintfrr (&fbuf, "<%d>1 %04u-%02u-%02uT%02u:%02u:%02uZ - %s %jd %.*s  ",
                 zte->facility | LOG_CRIT, tm.tm_year + 1900, tm.tm_mon + 1,
                 tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, zlog_progname,
                 pid, (int) (zlog_prefixsz - 2), zlog_prefix);
      break;
    ...
    case ZLOG_FMT_LOCAL:
      ...
      bprintfrr (&fbuf, "<%d>%3s %2u %02u:%02u:%02u %s%s[%jd]: ",
                 zte->facility | LOG_CRIT, months_3164[tm.tm_mon], tm.tm_mday,
                 tm.tm_hour, tm.tm_min, tm.tm_sec,
                 (zte->fmt == ZLOG_FMT_LOCAL) ? "" : "- ", zlog_progname, pid);
      break;
...
void
zlog_5424_init (struct zlog_cfg_5424 *zcf)
{
  pthread_mutex_init (&zcf->cfg_mtx, NULL);
}

static void
zlog_5424_target_free (struct zlt_5424 *zlt)
{
  if (! zlt)
    return;
...
void
zlog_5424_fini (struct zlog_cfg_5424 *zcf, bool keepopen)
{
  if (keepopen)
    zcf->active = NULL;
...
static void
zlog_5424_cycle (struct zlog_cfg_5424 *zcf, int fd)
{
  ...
  if (fd >= 0)
    {
       ...
       */
      zt = zlog_target_clone (MTYPE_LOG_5424, &zcf->active->zt, sizeof (*zlt));
      ...
      zlt->packets =
          (zcf->sock_type == SOCK_DGRAM) || (zcf->sock_type == SOCK_SEQPACKET);
      zlt->sa = zcf->sa;
...
static void
zlog_5424_reconnect (struct event *t)
{
  ...
  if (zcf->active)
    {
      ...
      if (ret == 0)
        zlog_warn ("logging socket %pSE closed by peer", zcf->filename);
      else
...
static int
zlog_5424_unix (struct sockaddr_un *suna, int sock_type)
{
  int fd;
...
static int
zlog_5424_open (struct zlog_cfg_5424 *zcf, int sock_type)
{
  int fd = -1;
  int flags = 0;
  int err;
  ...
  switch (zcf->dst)
    ...
    case ZLOG_5424_DST_FILE:
      ...
      frr_with_privs (lib_privs)
        {
          fd = open (zcf->filename,
                     flags | O_WRONLY | O_APPEND | O_CLOEXEC | O_NOCTTY);
        }
      if (fd >= 0)
        break;
      if (zcf->file_nocreate || flags)
        {
          flog_err_sys (EC_LIB_SYSTEM_CALL, "could not open log file %pSE: %m",
                        zcf->filename);
          break;
        }
      ...
      frr_with_privs (lib_privs)
        {
          ...
          prevmask = umask (0777 ^ zcf->file_mode);
          fd = open (zcf->filename,
                     O_WRONLY | O_APPEND | O_CLOEXEC | O_NOCTTY | O_CREAT |
                         O_EXCL,
                     zcf->file_mode);
          umask (prevmask);
        }
      ...
      frr_with_privs (lib_privs)
        {
          fd =
              open (zcf->filename, O_WRONLY | O_APPEND | O_CLOEXEC | O_NOCTTY);
        }
      if (fd >= 0)
        break;
  ...
  if (need_reconnect)
    {
      ...
      if (fd != -1)
        {
          event_add_read (zcf->master, zlog_5424_reconnect, zcf, fd,
                          &zcf->t_reconnect);
          zcf->reconn_backoff_cur = zcf->reconn_backoff;
        }
      else
        {
          event_add_timer_msec (zcf->master, zlog_5424_reconnect, zcf,
                                zcf->reconn_backoff_cur, &zcf->t_reconnect);
...
bool
zlog_5424_apply_dst (struct zlog_cfg_5424 *zcf)
{
  int fd = -1;
...
bool
zlog_5424_apply_meta (struct zlog_cfg_5424 *zcf)
{
  frr_with_mutex (&zcf->cfg_mtx)
    {
      if (zcf->active)
...
void
zlog_5424_state (struct zlog_cfg_5424 *zcf, size_t *lost_msgs, int *last_errno,
                 bool *stale_errno, struct timeval *err_ts)
{
  if (lost_msgs)
    *lost_msgs = zcf->active ? atomic_load_explicit (&zcf->active->lost_msgs,
...
bool
zlog_5424_rotate (struct zlog_cfg_5424 *zcf)
{
