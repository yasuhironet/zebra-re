...
static void
opt_extend (const struct optspec *os)
{
...
static const struct option lo_zclient[] = { { "socket", required_argument,
                                              NULL, 'z' },
                                            { NULL } };
...
bool
frr_zclient_addr (struct sockaddr_storage *sa, socklen_t *sa_len,
                  const char *path)
{
...
void
frr_init_vtydir (void)
{
  snprintf (frr_vtydir, sizeof (frr_vtydir), DAEMON_VTY_DIR, "", "");
}

void
frr_preinit (struct frr_daemon_info *daemon, int argc, char **argv)
{
  di = daemon;
  ...
  snprintf (pidfile_default, sizeof (pidfile_default), "%s/%s.pid", frr_vtydir,
            di->name);
  snprintf (frr_zclientpath, sizeof (frr_zclientpath), ZEBRA_SERV_PATH, "",
            "");
#ifdef HAVE_SQLITE3
  snprintf (dbfile_default, sizeof (dbfile_default), "%s/%s.db", frr_dbdir,
            di->name);
#endif
...
bool
frr_is_startup_fd (int fd)
{
  return ! ! (startup_fds & (UINT64_C (0x1) << (uint64_t) fd));
}

void
frr_opt_add (const char *optstr, const struct option *longopts,
             const char *helpstr)
{
...
static void
frr_mkdir (const char *path, bool strip)
{
  char buf[256];
  mode_t prev;
  int ret;
...
static void
_err_print (const void *cookie, const char *errstr)
{
...
frr_init (void)
{
...
#ifdef HAVE_SQLITE3
  snprintf (dbfile_default, sizeof (dbfile_default), "%s/%s%s%s.db", frr_dbdir,
            p_pathspace, di->name, p_instance);
#endif
  ...
  if (! frr_zclient_addr (&zclient_addr, &zclient_addr_len, frr_zclientpath))
    {
  ...
  if (nb_db_init () != NB_OK)
    flog_warn (EC_LIB_NB_DATABASE,
               "%s: failed to initialize northbound database", __func__);
  ...
  do
    {
      ...
      switch (rcvd_signal)
        ...
        case SIGTSTP:
          ...
          while (nrecv == -1 && (errno == EINTR || errno == EAGAIN));
 ...
 */
static void
frr_config_read_in (struct event *t)
{
  ...
  if (frr_get_cli_mode () == FRR_CLI_TRANSACTIONAL)
    {
      ...
      ret = nb_candidate_commit (context, vty_shared_candidate_config, true,
  ...
  if (! (di->flags & FRR_NO_SPLIT_CONFIG))
    {
      ...
      event_add_event (master, frr_config_read_in, NULL, 0, &di->read_in);
    }
  ...
  if (! di->vty_path)
    {
      ...
      if (di->instance)
        snprintf (vtypath_default, sizeof (vtypath_default), "%s/%s-%d.vty",
                  dir, di->name, di->instance);
      else
        snprintf (vtypath_default, sizeof (vtypath_default), "%s/%s.vty", dir,
                  di->name);
...
frr_terminal_close (int isexit)
{
  ...
  if (nullfd == -1)
  ...
  else
    {
       ...
       */
      for (fd = 2; fd >= 0; fd--)
        if (isatty (fd) && (fd != STDOUT_FILENO || ! logging_to_stdout))
          dup2 (nullfd, fd);
      close (nullfd);
    }
}
...
void
frr_run (struct event_loop *master)
{
...
#ifdef INTERP
static const char interp[] __attribute__ ((section (".interp"), used)) =
    INTERP;
#endif
/*
