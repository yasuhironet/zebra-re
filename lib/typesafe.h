...
#define frr_each(prefix, head, item)                                          \
  for (item = prefix##_first (head); item; item = prefix##_next (head, item))
...
#define frr_each_safe(prefix, head, item)                                     \
  for (typeof (prefix##_next_safe (head, NULL)) prefix##_safe =               \
           prefix##_next_safe (head, (item = prefix##_first (head)));         \
       item; item = prefix##_safe, prefix##_safe = prefix##_next_safe (       \
                                       head, prefix##_safe))
#define frr_each_from(prefix, head, item, from)                               \
  for (item = from, from = prefix##_next_safe (head, item); item;             \
...
#define frr_rev_each(prefix, head, item)                                      \
  for (item = prefix##_last (head); item; item = prefix##_prev (head, item))
#define frr_rev_each_safe(prefix, head, item)                                 \
  for (typeof (prefix##_prev_safe (head, NULL)) prefix##_safe =               \
           prefix##_prev_safe (head, (item = prefix##_last (head)));          \
       item; item = prefix##_safe, prefix##_safe = prefix##_prev_safe (       \
                                       head, prefix##_safe))
#define frr_rev_each_from(prefix, head, item, from)                           \
  for (item = from, from = prefix##_prev_safe (head, item); item;             \
...
#define _SLIST_LAST &typesafe_slist_sentinel
  ...
  static inline void
  typesafe_list_add (struct slist_head *head, struct slist_item **pos,
...
#define DECLARE_LIST(prefix, type, field)                                     \
  ...
  macro_inline void prefix##_add_after (struct prefix##_head *h, type *after, \
                                        type *item)                           \
  {                                                                           \
  ...
  static inline void
  typesafe_dlist_add (struct dlist_head *head, struct dlist_item *prev,
  ...
  static inline void
  typesafe_dlist_swap_all (struct dlist_head *a, struct dlist_head *b)
  {
...
#define HEAP_RESIZE_TRESH_UP(h) (h->hh.count + 1 >= h->hh.arraysz)
#define HEAP_RESIZE_TRESH_DN(h)                                               \
  (h->hh.count == 0 || h->hh.arraysz - h->hh.count > (h->hh.count + 1024) / 2)
...
#define DECLARE_HEAP(prefix, type, field, cmpfn)                              \
  ...
  extern void typesafe_heap_pushdown (
      struct heap_head *head, uint32_t index, struct heap_item *item,
      int (*cmpfn) (const struct heap_item *a, const struct heap_item *b));
...
#define _PREDECL_SORTLIST(prefix)                                             \
  struct prefix##_head                                                        \
  {                                                                           \
    struct ssort_head sh;                                                     \
  };                                                                          \
  struct prefix##_item                                                        \
  {                                                                           \
    struct ssort_item si;                                                     \
  };                                                                          \
  MACRO_REQUIRE_SEMICOLON () /* end */
...
#define PREDECL_SORTLIST_UNIQ(prefix)    _PREDECL_SORTLIST (prefix)
#define PREDECL_SORTLIST_NONUNIQ(prefix) _PREDECL_SORTLIST (prefix)
...
#define _DECLARE_SORTLIST(prefix, type, field, cmpfn_nuq, cmpfn_uq)           \
  ...
  {                                                                           \
    ...
    int c = 1;                                                                \
    while (*np &&                                                             \
           (c = cmpfn_uq (container_of (*np, type, field.si), item)) < 0)     \
      np = &(*np)->next;                                                      \
    if (c == 0)                                                               \
...
#define DECLARE_SORTLIST_UNIQ(prefix, type, field, cmpfn)                     \
  _DECLARE_SORTLIST (prefix, type, field, cmpfn, cmpfn);                      \
                                                                              \
  macro_inline const type *prefix##_const_find (                              \
      const struct prefix##_head *h, const type *item)                        \
  {                                                                           \
    const struct ssort_item *sitem = h->sh.first;                             \
    int cmpval = 0;                                                           \
    while (sitem &&                                                           \
           (cmpval = cmpfn (container_of (sitem, type, field.si), item)) < 0) \
      sitem = sitem->next;                                                    \
    if (! sitem || cmpval > 0)                                                \
      return NULL;                                                            \
    return container_of (sitem, type, field.si);                              \
...
#define _HASH_SIZE(tabshift) ((1U << (tabshift)) >> 1)
#define HASH_SIZE(head)      _HASH_SIZE ((head).tabshift)
#define _HASH_KEY(tabshift, val)                                              \
  ({                                                                          \
    assume ((tabshift) >= 2 && (tabshift) <= 33);                             \
    (val) >> (33 - (tabshift));                                               \
  })
#define HASH_KEY(head, val)       _HASH_KEY ((head).tabshift, val)
#define HASH_GROW_THRESHOLD(head) ((head).count >= HASH_SIZE (head))
...
#define _DECLARE_SKIPLIST(prefix, type, field, cmpfn_nuq, cmpfn_uq)           \
  ...
  {                                                                           \
    memset (h, 0, sizeof (*h));                                               \
    h->sh.hitem.next[SKIPLIST_OVERFLOW] =                                     \
        (struct sskip_item *) ((uintptr_t) h->sh.overflow | 1);               \
  }                                                                           \
  macro_inline void prefix##_fini (struct prefix##_head *h)                   \
  {                                                                           \
    memset (h, 0, sizeof (*h));                                               \
  ...
  macro_inline const type *prefix##_const_find_gteq (                         \
      const struct prefix##_head *h, const type *item)                        \
  {                                                                           \
    const struct sskip_item *sitem =                                          \
        typesafe_skiplist_find_gteq (&h->sh, &item->field.si, cmpfn_nuq);     \
    return container_of_null (sitem, type, field.si);                         \
  }                                                                           \
  macro_inline const type *prefix##_const_find_lt (                           \
      const struct prefix##_head *h, const type *item)                        \
  {                                                                           \
    const struct sskip_item *sitem =                                          \
        typesafe_skiplist_find_lt (&h->sh, &item->field.si, cmpfn_nuq);       \
    return container_of_null (sitem, type, field.si);                         \
  }                                                                           \
  TYPESAFE_FIND_CMP (prefix, type)                                            \
  macro_inline type *prefix##_del (struct prefix##_head *h, type *item)       \
  {                                                                           \
    struct sskip_item *sitem =                                                \
  ...
  {                                                                           \
    ...
    *a = *b;                                                                  \
    *b = tmp;                                                                 \
    a->sh.hitem.next[SKIPLIST_OVERFLOW] =                                     \
        (struct sskip_item *) ((uintptr_t) a->sh.overflow | 1);               \
    b->sh.hitem.next[SKIPLIST_OVERFLOW] =                                     \
        (struct sskip_item *) ((uintptr_t) b->sh.overflow | 1);               \
  }                                                                           \
  macro_pure const type *prefix##_const_first (const struct prefix##_head *h) \
  {                                                                           \
...
#define PREDECL_SKIPLIST_UNIQ(prefix) _PREDECL_SKIPLIST (prefix)
#define DECLARE_SKIPLIST_UNIQ(prefix, type, field, cmpfn)                     \
  ...
  {                                                                           \
    const struct sskip_item *sitem =                                          \
        typesafe_skiplist_find (&h->sh, &item->field.si, &prefix##__cmp);     \
    return container_of_null (sitem, type, field.si);                         \
  }                                                                           \
  TYPESAFE_FIND (prefix, type)                                                \
  TYPESAFE_MEMBER_VIA_FIND (prefix, type)                                     \
                                                                              \
  _DECLARE_SKIPLIST (prefix, type, field, prefix##__cmp, prefix##__cmp);      \
  MACRO_REQUIRE_SEMICOLON () /* end */

#define PREDECL_SKIPLIST_NONUNIQ(prefix) _PREDECL_SKIPLIST (prefix)
#define DECLARE_SKIPLIST_NONUNIQ(prefix, type, field, cmpfn)                  \
  ...
  {                                                                           \
    ...
    return 0;                                                                 \
                                                                              ...
                                                                              \
  _DECLARE_SKIPLIST (prefix, type, field, prefix##__cmp, prefix##__cmp_uq);   \
  TYPESAFE_MEMBER_VIA_FIND_GTEQ (prefix, type, cmpfn)                         \
  MACRO_REQUIRE_SEMICOLON () /* end */


  extern struct sskip_item *typesafe_skiplist_add (
