...
#define TELNET_NAWS_SB_LEN 5
  ...
  static inline void
  vty_push_context (struct vty *vty, int node, uint64_t id)
  {
    vty->node = node;
    vty->qobj_index = id;
  }
...
#define VTY_PUSH_XPATH(nodeval, value)                                        \
  do                                                                          \
    {                                                                         \
      ...
      vty->node = nodeval;                                                    \
      strlcpy (vty->xpath[vty->xpath_index], value, sizeof (vty->xpath[0]));  \
      vty->xpath_index++;                                                     \
    }                                                                         \
  while (0)
  ...
  do                                                                          \
    {                                                                         \
      if (vty->type != VTY_FILE && ! vty->private_config &&                   \
          vty->xpath_index > 0 &&                                             \
          ! yang_dnode_exists (vty->candidate_config->dnode, VTY_CURR_XPATH)) \
        {                                                                     \
          vty_out (vty, "Current configuration object was deleted "           \
                        "by another process.\n\n");                           \
          return CMD_WARNING;                                                 \
...
#define IS_DIRECTORY_SEP(c) ((c) == DIRECTORY_SEP)
  ...
  extern FILE *vty_open_config (const char *config_file,
                                char *config_default_dir);
  extern bool vty_read_config (struct nb_config *config,
  ...
  extern int vty_mgmt_send_lockds_req (struct vty *vty,
                                       Mgmtd__DatastoreId ds_id, bool lock,
                                       bool scok);
  extern void vty_mgmt_resume_response (struct vty *vty, bool success);

  static inline bool
  vty_needs_implicit_commit (struct vty *vty)
  {
