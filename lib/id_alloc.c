 ...
 */
static struct id_alloc_page *
find_or_create_page (struct id_alloc *alloc, uint32_t id, int create)
{
  ...
  if (subdir == NULL)
    {
      if (create)
        {
          subdir = XCALLOC (MTYPE_IDALLOC_SUBDIRECTORY, sizeof (*subdir));
  ...
  if (page == NULL && create)
  ...
  else if (page != NULL && create)
    {
      flog_err (EC_LIB_ID_CONSISTENCY,
                "ID Allocator %s attempt to re-create page at %u", alloc->name,
                id);
    }
 ...
 */
void
idalloc_free (struct id_alloc *alloc, uint32_t id)
{
  struct id_alloc_page *page = NULL;
 ...
 */
static struct id_alloc_page *
create_next_page (struct id_alloc *alloc)
{
 ...
 * is being reserved by number, this may end up scanning the whole single
 * linked list of pages in order to remove it.
 */
static void
reserve_bit (struct id_alloc *alloc, struct id_alloc_page *page, int word,
             int offset)
{
              ...
              while (itr)
                {
                  if (itr->next_has_free == page)
                    {
                      itr->next_has_free = page->next_has_free;
                      return;
                    }
 ...
 */
uint32_t
idalloc_allocate (struct id_alloc *alloc)
{
  ...
  if (alloc->has_free == NULL)
    {
      flog_err (EC_LIB_ID_EXHAUST, "ID Allocator %s has run out of IDs.",
                alloc->name);
 ...
 * when the ID being "reserved" has allready been assigned/reserved. This
 * should only be done with low numbered IDs, as the allocator needs to reserve
 * bit-map pages in order
 */
uint32_t
idalloc_reserve (struct id_alloc *alloc, uint32_t id)
{
  struct id_alloc_page *page;
  int word, offset;
 ...
 */
static void
idalloc_destroy_subdir (struct id_alloc_subdir *subdir)
{
  int i;
 ...
 */
static void
idalloc_destroy_dir (struct id_alloc_dir *dir)
{
  int i;
 ...
 */
void
idalloc_destroy (struct id_alloc *alloc)
{
  int i;
 ...
 */
void
idalloc_free_to_pool (struct id_alloc_pool **pool_ptr, uint32_t id)
{
 ...
 */
void
idalloc_drain_pool (struct id_alloc *alloc, struct id_alloc_pool **pool_ptr)
{
 ...
 */
uint32_t
idalloc_allocate_prefer_pool (struct id_alloc *alloc,
