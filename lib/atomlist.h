...
extern "C"
{
   ...
   *
   * ATOMPTR_USER is currently unused (and available for atomic hash or
...
#define ATOMPTR_NULL (0)
  ...
  static inline void *
  atomptr_p (atomptr_t val)
  {
    return (void *) (val & ATOMPTR_MASK);
  }
  static inline bool
  atomptr_l (atomptr_t val)
  {
    return (bool) (val & ATOMPTR_LOCK);
  }
  static inline bool
  atomptr_u (atomptr_t val)
  {
...
#define atomic_find_warn                                                      \
   ...
   * updates.  an inactive / blocked / deadlocked other thread cannot cause
   * such delays, and to cause such delays a thread must be heavily hitting the
   * list - it's a rather theoretical concern.
   */
...
#define DECLARE_ATOMLIST(prefix, type, field)                                 \
  ...
  {                                                                           \
    char *p = atomptr_p (                                                     \
        atomic_load_explicit (&h->ah.first, memory_order_acquire));           \
    return p ? (type *) (p - offsetof (type, field)) : NULL;                  \
  }                                                                           \
  macro_inline type *prefix##_next (struct prefix##_head *h, type *item)      \
  {                                                                           \
    char *p = atomptr_p (                                                     \
        atomic_load_explicit (&item->field.ai.next, memory_order_acquire));   \
    return p ? (type *) (p - offsetof (type, field)) : NULL;                  \
  }                                                                           \
...
#define _PREDECL_ATOMSORT(prefix)                                             \
  struct prefix##_head                                                        \
  {                                                                           \
    struct atomsort_head ah;                                                  \
  };                                                                          \
  struct prefix##_item                                                        \
  {                                                                           \
    struct atomsort_item ai;                                                  \
  };                                                                          \
  MACRO_REQUIRE_SEMICOLON () /* end */
...
#define PREDECL_ATOMSORT_UNIQ(prefix) _PREDECL_ATOMSORT (prefix)
  ...
  {                                                                           \
    return cmpfn (container_of (a, type, field.ai),                           \
                  container_of (b, type, field.ai));                          \
                                                                              ...
                                                                              \
  _DECLARE_ATOMSORT (prefix, type, field, prefix##__cmp, prefix##__cmp);      \
                                                                              \
  atomic_find_warn macro_inline type *prefix##_find (struct prefix##_head *h, \
                                                     const type *item)        \
  {                                                                           \
    type *p = prefix##_first (h);                                             \
    int cmpval = 0;                                                           \
...
#define PREDECL_ATOMSORT_NONUNIQ(prefix) _PREDECL_ATOMSORT (prefix)
#define DECLARE_ATOMSORT_NONUNIQ(prefix, type, field, cmpfn)                  \
  ...
  {                                                                           \
    ...
    return 0;                                                                 \
                                                                              ...
                                                                              \
  _DECLARE_ATOMSORT (prefix, type, field, prefix##__cmp, prefix##__cmp_uq);   \
  MACRO_REQUIRE_SEMICOLON () /* end */

  struct atomsort_item *
  atomsort_add (struct atomsort_head *h, struct atomsort_item *item,
