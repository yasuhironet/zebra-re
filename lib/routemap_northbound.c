 ...
 */
int
lib_route_map_entry_match_destroy (struct nb_cb_destroy_args *args)
{
  ...
  rv = rhc->rhc_mhook (rhc->rhc_rmi, rhc->rhc_rule, NULL, rhc->rhc_event,
...
int
lib_route_map_entry_set_destroy (struct nb_cb_destroy_args *args)
{
  struct routemap_hook_context *rhc;
  int rv;
  ...
  rv = rhc->rhc_shook (rhc->rhc_rmi, rhc->rhc_rule, NULL, args->errmsg,
                       args->errmsg_len);
  if (rv != CMD_SUCCESS)
    return NB_ERR_INCONSISTENCY;
 ...
 */
static int
lib_route_map_create (struct nb_cb_create_args *args)
{
  struct route_map *rm;
  const char *rm_name;
...
static int
lib_route_map_destroy (struct nb_cb_destroy_args *args)
{
  struct route_map *rm;
 ...
 */
static int
lib_route_map_entry_create (struct nb_cb_create_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      action = yang_dnode_get_enum (args->dnode, "./action") == 0 ? RMAP_PERMIT
                                                                  : RMAP_DENY;
      rm = nb_running_get_entry (args->dnode, NULL, true);
...
static int
lib_route_map_entry_destroy (struct nb_cb_destroy_args *args)
{
  struct route_map_index *rmi;
 ...
 */
static int
lib_route_map_entry_action_modify (struct nb_cb_modify_args *args)
{
  struct route_map_index *rmi;
 ...
 */
static int
lib_route_map_entry_call_modify (struct nb_cb_modify_args *args)
{
  struct route_map_index *rmi;
...
static int
lib_route_map_entry_call_destroy (struct nb_cb_destroy_args *args)
{
  struct route_map_index *rmi;
    ...
    case NB_EV_VALIDATE:
      ...
      switch (policy)
        ...
        case 1: /* next */
                /* FALLTHROUGH */
        case 2: /* goto */
          rm_action = yang_dnode_get_enum (args->dnode, "../action");
          if (rm_action == 1 /* deny */)
            {
              /*
 ...
 */
static int
lib_route_map_entry_goto_value_modify (struct nb_cb_modify_args *args)
{
  struct route_map_index *rmi;
  uint16_t rmi_index;
  uint16_t rmi_next;
    ...
    struct nb_cb_modify_args *args)
{
  ...
  rv = rmap_match_set_hook.match_interface (rhc->rhc_rmi, "interface", ifname,
  ...
  if (IS_MATCH_IPv4_ADDRESS_LIST (condition))
    {
      ...
      rv = rmap_match_set_hook.match_ip_address (
          rhc->rhc_rmi, "ip address", acl, RMAP_EVENT_FILTER_ADDED,
          args->errmsg, args->errmsg_len);
    }
  else if (IS_MATCH_IPv4_PREFIX_LIST (condition))
    {
      if (rmap_match_set_hook.match_ip_address_prefix_list == NULL)
        return NB_OK;
      rhc->rhc_mhook = rmap_match_set_hook.no_match_ip_address_prefix_list;
      ...
      rv = rmap_match_set_hook.match_ip_address_prefix_list (
          rhc->rhc_rmi, "ip address prefix-list", acl, RMAP_EVENT_PLIST_ADDED,
          args->errmsg, args->errmsg_len);
    }
  else if (IS_MATCH_IPv4_NEXTHOP_LIST (condition))
    {
      ...
      rv = rmap_match_set_hook.match_ip_next_hop (
          rhc->rhc_rmi, "ip next-hop", acl, RMAP_EVENT_FILTER_ADDED,
          args->errmsg, args->errmsg_len);
    }
  else if (IS_MATCH_IPv6_NEXTHOP_LIST (condition))
    {
      ...
      rv = rmap_match_set_hook.match_ipv6_next_hop (
          rhc->rhc_rmi, "ipv6 next-hop", acl, RMAP_EVENT_FILTER_ADDED,
          args->errmsg, args->errmsg_len);
    }
  else if (IS_MATCH_IPv4_NEXTHOP_PREFIX_LIST (condition))
    {
      if (rmap_match_set_hook.match_ip_next_hop_prefix_list == NULL)
        return NB_OK;
      rhc->rhc_mhook = rmap_match_set_hook.no_match_ip_next_hop_prefix_list;
      rhc->rhc_rule = "ip next-hop prefix-list";
      rhc->rhc_event = RMAP_EVENT_PLIST_DELETED;
      rv = rmap_match_set_hook.match_ip_next_hop_prefix_list (
          rhc->rhc_rmi, "ip next-hop prefix-list", acl, RMAP_EVENT_PLIST_ADDED,
          args->errmsg, args->errmsg_len);
    }
  else if (IS_MATCH_IPv6_NEXTHOP_PREFIX_LIST (condition))
    {
      if (rmap_match_set_hook.match_ipv6_next_hop_prefix_list == NULL)
        return NB_OK;
      rhc->rhc_mhook = rmap_match_set_hook.no_match_ipv6_next_hop_prefix_list;
  ...
  else if (IS_MATCH_IPv6_ADDRESS_LIST (condition))
    {
      ...
      rv = rmap_match_set_hook.match_ipv6_address (
          rhc->rhc_rmi, "ipv6 address", acl, RMAP_EVENT_FILTER_ADDED,
          args->errmsg, args->errmsg_len);
    }
  else if (IS_MATCH_IPv6_PREFIX_LIST (condition))
    {
      if (rmap_match_set_hook.match_ipv6_address_prefix_list == NULL)
        return NB_OK;
      rhc->rhc_mhook = rmap_match_set_hook.no_match_ipv6_address_prefix_list;
    ...
    struct nb_cb_modify_args *args)
{
  ...
  rv = rmap_match_set_hook.match_metric (rhc->rhc_rmi, "metric", type,
 ...
 */
static int
lib_route_map_entry_set_action_create (struct nb_cb_create_args *args)
{
    ...
    struct nb_cb_modify_args *args)
{
  ...
  rv = rmap_match_set_hook.set_ip_nexthop (
      rhc->rhc_rmi, "ip next-hop", address, args->errmsg, args->errmsg_len);
  if (rv != CMD_SUCCESS)
    {
      rhc->rhc_shook = NULL;
      return NB_ERR_INCONSISTENCY;
    ...
    struct nb_cb_modify_args *args)
{
  ...
  rv = rmap_match_set_hook.set_ipv6_nexthop_local (
      rhc->rhc_rmi, "ipv6 next-hop local", address, args->errmsg,
      args->errmsg_len);
  if (rv != CMD_SUCCESS)
    {
      rhc->rhc_shook = NULL;
 ...
 */
static int
set_action_modify (enum nb_event event, const struct lyd_node *dnode,
                   ...
                   char *errmsg, size_t errmsg_len)
{
  ...
  rv = rmap_match_set_hook.set_metric (rhc->rhc_rmi, "metric", value, errmsg,
                                       errmsg_len);
  if (rv != CMD_SUCCESS)
    {
      rhc->rhc_shook = NULL;
...
lib_route_map_entry_set_action_value_modify (struct nb_cb_modify_args *args)
{
  ...
  return set_action_modify (args->event, args->dnode, args->resource, metric,
                            args->errmsg, args->errmsg_len);
}
 ...
 */
static int
set_action_min_metric_modify (enum nb_event event,
                              ...
                              char *errmsg, size_t errmsg_len)
{
  ...
  rv = rmap_match_set_hook.set_min_metric (rhc->rhc_rmi, "min-metric", value,
                                           errmsg, errmsg_len);
  if (rv != CMD_SUCCESS)
    {
      rhc->rhc_shook = NULL;
 ...
 */
static int
set_action_max_metric_modify (enum nb_event event,
                              const struct lyd_node *dnode,
                              union nb_resource *resource, const char *value,
                              char *errmsg, size_t errmsg_len)
{
  ...
  rv = rmap_match_set_hook.set_max_metric (rhc->rhc_rmi, "max-metric", value,
                                           errmsg, errmsg_len);
  if (rv != CMD_SUCCESS)
    {
      rhc->rhc_shook = NULL;
    ...
    struct nb_cb_modify_args *args)
{
  ...
  if (args->event == NB_EV_VALIDATE &&
    ...
    {
      snprintf (args->errmsg, args->errmsg_len, "Can't add zero to metric");
      return NB_ERR_VALIDATION;
    }
  ...
  return set_action_modify (args->event, args->dnode, args->resource,
                            metric_str, args->errmsg, args->errmsg_len);
}
...
lib_route_map_entry_set_action_use_round_trip_time_modify (
    struct nb_cb_modify_args *args)
{
  return set_action_modify (args->event, args->dnode, args->resource, "rtt",
                            args->errmsg, args->errmsg_len);
}
...
lib_route_map_entry_set_action_tag_modify (struct nb_cb_modify_args *args)
{
  ...
  rv = rmap_match_set_hook.set_tag (rhc->rhc_rmi, "tag", tag, args->errmsg,
                                    args->errmsg_len);
  if (rv != CMD_SUCCESS)
    {
      rhc->rhc_shook = NULL;
