...
#define LOGMSG_FLAG_PERSISTENT (1 << 1)
   ...
   * on context, it may still be the right thing to use fprintf though -- try
   * to determine whether something is a log message or something else.
   */

  extern void vzlogx (const struct xref_logmsg *xref, int prio,
                      const char *fmt, va_list ap) PRINTFRR (3, 0);
#define vzlog(prio, ...) vzlogx (NULL, prio, __VA_ARGS__)

  PRINTFRR (2, 3)
  static inline void
  zlog (int prio, const char *fmt, ...)
  {
    va_list ap;
  ...
  PRINTFRR (2, 3)
  static inline void
  zlog_ref (const struct xref_logmsg *xref, const char *fmt, ...)
  {
    va_list ap;
  ...
  do                                                                          \
    {                                                                         \
      ...
      static const struct xref_logmsg _xref __attribute__ ((used)) = {        \
        .xref = XREF_INIT (XREFT_LOGMSG, &_xrefdata.xrefdata, __func__),      \
        .fmtstring = (msg),                                                   \
        .priority = (prio),                                                   \
        .ec = (ec_),                                                          \
        .args = (#__VA_ARGS__),                                               \
...
#define ZLOG_DISABLED (LOG_EMERG - 1)
  ...
  /* zlog_msg encapsulates a particular logging call from somewhere in the
   * code. The same struct is passed around to all zlog_targets.
   *
...
#define ZLOG_TS_UTC (1 << 10)
   ...
   * thread's messages
   */
  extern void zlog_msg_pid (struct zlog_msg *msg, intmax_t *pid,
                            intmax_t *tid);
