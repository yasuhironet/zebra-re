...
extern "C"
{
   ...
   *
   * output
   *    List of 'yang_data' structures used to retrieve the RPC output
   * parameters. It can be set to NULL when it's known that the given YANG RPC
   * or Action doesn't have any output parameters.
   *
   * Returns:
   *    CMD_SUCCESS on success, CMD_WARNING otherwise.
   */
  extern int nb_cli_rpc (struct vty *vty, const char *xpath,
                         struct list *input, struct list *output);
