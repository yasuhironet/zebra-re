...
#define YANG_TRANSLATE_MAX         2
  ...
  RB_PROTOTYPE (yang_translators, yang_translator, entry,
   ...
   *
   * Returns:
   *    Pointer to newly created YANG module translator, or NULL in the case of
   * an error.
   */
   ...
   *
   * xpath
   *    Pointer to previously allocated buffer containing the xpath expression
   * to be translated.
   *
   * xpath_len
  ...
  /*
   * Initialize the YANG module translator subsystem. Should be called only
   * once during the daemon initialization process.
   */
  extern void yang_translator_init (void);

  /*
   * Finish the YANG module translator subsystem gracefully. Should be called
   * only when the daemon is exiting.
   */
