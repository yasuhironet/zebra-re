 ...
 */
int
frrscript_names_add_function_name (const char *function_name)
{
  ...
  if (frrscript_names_add (&frrscript_names_hash, insert))
    {
      zlog_warn ("Failed to add hook call function name to script_names");
      return 1;
    }
  return 0;
}
 ...
 */
int
frrscript_names_set_script_name (const char *function_name,
                                 const char *script_name)
{
  struct frrscript_names_entry lookup;

  strlcpy (lookup.function_name, function_name, sizeof (lookup.function_name));
 ...
 */
char *
frrscript_names_get_script_name (const char *function_name)
{
  struct frrscript_names_entry lookup;
...
uint32_t
frrscript_names_hash_key (const struct frrscript_names_entry *snhe)
{
  return string_hash_make (snhe->function_name);
}

int
frrscript_names_hash_cmp (const struct frrscript_names_entry *snhe1,
...
static unsigned int
codec_hash_key (const void *data)
{
...
static bool
codec_hash_cmp (const void *d1, const void *d2)
{
...
unsigned int
lua_function_hash_key (const void *data)
{
...
bool
lua_function_hash_cmp (const void *d1, const void *d2)
{
...
int
_frrscript_call_lua (struct lua_function_state *lfs, int nargs)
{
  ...
  switch (ret)
    ...
    case LUA_ERRMEM:
      ...
      break;
    case LUA_ERRERR:
      zlog_err ("Lua hook call '%s' : error handler error: %s", lfs->name,
                lua_tostring (lfs->L, -1));
      break;
    case LUA_ERRGCMM:
      zlog_err ("Lua hook call '%s' : garbage collector error: %s", lfs->name,
                lua_tostring (lfs->L, -1));
      break;
    default:
...
void *
frrscript_get_result (struct frrscript *fs, const char *function_name,
                      const char *name,
...
void
frrscript_register_type_codec (struct frrscript_codec *codec)
{
...
void
frrscript_register_type_codecs (struct frrscript_codec *codecs)
{
...
frrscript_new (const char *name)
{
  ...
  fs->lua_function_hash = hash_create (
      lua_function_hash_key, lua_function_hash_cmp, "Lua function state hash");
  return fs;
}

int
frrscript_load (struct frrscript *fs, const char *function_name,
                int (*load_cb) (struct frrscript *))
{
  ...
  if (snprintf (script_name, sizeof (script_name), "%s/%s.lua", scriptdir,
    ...
    {
      zlog_err ("frrscript: path to script %s/%s.lua is too long", scriptdir,
                fs->name);
      goto fail;
    }
  ...
  if (lua_isfunction (L, lua_gettop (L)) == 0)
    {
      zlog_err ("frrscript: loaded script '%s' but %s not found", script_name,
                function_name);
      goto fail;
    }
...
void
frrscript_delete (struct frrscript *fs)
{
...
void
frrscript_init (const char *sd)
{
  codec_hash =
      hash_create (codec_hash_key, codec_hash_cmp, "Lua type encoders");
