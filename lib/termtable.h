...
#define TTSTYLE_BLANK 1
   ...
   * delimited by '|'. For example, to make the first column of the table
   * above, the call is:
   *
   ...
   *  - If there are N columns, '|' must appear n-1 times or the row will not
   * be created
   *
   ...
   *
   * @return pointer to the first cell in the created row, or NULL if not
   * enough columns were specified
   */
   ...
   *
   * Available padding options are LEFT and RIGHT (the alignment enum is
   * reused). Both options may be set. Padding is treated as though it is stuck
   * to the walls of the cell. Suppose a cell has:
   *
   * lpad = 4
   * rpad = 2
   * align = RIGHT
  ...
  void ttable_pad (struct ttable *tt, unsigned int srow, unsigned int scol,
                   ...
                   enum ttable_align align, short pad);
