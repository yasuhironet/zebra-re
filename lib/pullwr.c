...
struct pullwr *
_pullwr_new (struct event_loop *tm, int fd, void *arg,
             void (*fill) (void *, struct pullwr *),
             void (*err) (void *, struct pullwr *, bool))
{
...
void
pullwr_del (struct pullwr *pullwr)
{
...
void
pullwr_cfg (struct pullwr *pullwr, int64_t max_spin_usec,
...
void
pullwr_bump (struct pullwr *pullwr)
{
  if (pullwr->writer)
    return;
...
static size_t
pullwr_iov (struct pullwr *pullwr, struct iovec *iov)
{
  size_t len1;
...
static void
pullwr_resize (struct pullwr *pullwr, size_t need)
{
  struct iovec iov[2];
  size_t niov, newsize;
  char *newbuf;
  ...
  if (niov >= 1)
    {
      ...
      if (niov >= 2)
        memcpy (newbuf + iov[0].iov_len, iov[1].iov_base, iov[1].iov_len);
    }
...
void
pullwr_write (struct pullwr *pullwr, const void *data, size_t len)
{
  ...
  do
    {
      ...
      while (pullwr->valid < pullwr->thresh && pullwr->valid != lastvalid &&
             ! maxspun)
        {
...
void
pullwr_stats (struct pullwr *pullwr, uint64_t *total_written, size_t *pending,
