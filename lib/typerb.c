...
static inline void
rbe_rotate_left (struct rbt_tree *rbt, struct rb_entry *rbe)
{
...
static inline void
rbe_rotate_right (struct rbt_tree *rbt, struct rb_entry *rbe)
{
  struct rb_entry *parent;
  struct rb_entry *tmp;
...
static inline void
rbe_insert_color (struct rbt_tree *rbt, struct rb_entry *rbe)
{
  struct rb_entry *parent, *gparent, *tmp;

  rbt->count++;

  while ((parent = RBE_PARENT (rbe)) != NULL && RBE_COLOR (parent) == RB_RED)
    {
...
static inline void
rbe_remove_color (struct rbt_tree *rbt, struct rb_entry *parent,
                  struct rb_entry *rbe)
{
  struct rb_entry *tmp;
...
typed_rb_remove (struct rbt_tree *rbt, struct rb_entry *rbe)
{
  return rbe_remove (rbt, rbe);
}

struct typed_rb_entry *
typed_rb_insert (struct rbt_tree *rbt, struct rb_entry *rbe,
...
typed_rb_find (const struct rbt_tree *rbt, const struct rb_entry *key,
               int (*cmpfn) (const struct typed_rb_entry *a,
                             const struct typed_rb_entry *b))
{
...
const struct rb_entry *
typed_rb_find_gteq (const struct rbt_tree *rbt, const struct rb_entry *key,
                    int (*cmpfn) (const struct typed_rb_entry *a,
                                  const struct typed_rb_entry *b))
{
...
const struct rb_entry *
typed_rb_find_lt (const struct rbt_tree *rbt, const struct rb_entry *key,
                  int (*cmpfn) (const struct typed_rb_entry *a,
                                const struct typed_rb_entry *b))
{
  const struct rb_entry *tmp = RBH_ROOT (rbt), *best = NULL;
...
struct rb_entry *
typed_rb_next (const struct rb_entry *rbe_const)
{
  ...
  if (RBE_RIGHT (rbe) != NULL)
  ...
  else
    {
      if (RBE_PARENT (rbe) && (rbe == RBE_LEFT (RBE_PARENT (rbe))))
        ...
        {
          while (RBE_PARENT (rbe) && (rbe == RBE_RIGHT (RBE_PARENT (rbe))))
            rbe = RBE_PARENT (rbe);
          rbe = RBE_PARENT (rbe);
        }
    }
...
struct rb_entry *
typed_rb_prev (const struct rb_entry *rbe_const)
{
  struct rb_entry *rbe = (struct rb_entry *) rbe_const;
  ...
  if (RBE_LEFT (rbe))
  ...
  else
    {
      if (RBE_PARENT (rbe) && (rbe == RBE_RIGHT (RBE_PARENT (rbe))))
        ...
        {
          while (RBE_PARENT (rbe) && (rbe == RBE_LEFT (RBE_PARENT (rbe))))
            rbe = RBE_PARENT (rbe);
          rbe = RBE_PARENT (rbe);
        }
    }
...
struct rb_entry *
typed_rb_min (const struct rbt_tree *rbt)
{
...
struct rb_entry *
typed_rb_max (const struct rbt_tree *rbt)
{
  struct rb_entry *rbe = RBH_ROOT (rbt);
  struct rb_entry *parent = NULL;
...
bool
typed_rb_member (const struct typed_rb_root *rbt,
