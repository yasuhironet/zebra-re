...
extern "C"
{
  ...
  /* This is a "pull-driven" write event handler.  Instead of having some
   * buffer or being driven by the availability of data, it triggers on the
   * space being available on the socket for data to be written on and then
   * calls fill() to get data to be sent.
   *
   ...
   * - if there's nothing more to be sent, fill() returns without doing
   * anything and pullwr goes into idle state after flushing all buffered data
   * out.
   ...
   *   doing things.
   *
   * When the err() callback is called, the pullwr should be considered
   * unusable and released with pullwr_del().  This can be done from inside the
   * callback, the pullwr code holds no more references on it when calling
   * err().
   */
...
#define pullwr_new(tm, fd, arg, fill, err)                                    \
   ...
   *   lot of data to a local endpoint that reads quickly (i.e. no
   * backpressure)
   *
   *   default: 2500 (2.5 ms)
   *
  ...
  extern void pullwr_write (struct pullwr *pullwr, const void *data,
                            size_t len);

  static inline void
  pullwr_write_stream (struct pullwr *pullwr, struct stream *s)
  {
