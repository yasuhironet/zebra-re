...
static void
vty_show_nb_errors (struct vty *vty, int error, const char *errmsg)
{
...
static int
nb_cli_classic_commit (struct vty *vty)
{
  ...
  context.user = vty;
  ret = nb_candidate_commit (context, vty->candidate_config, true, NULL, NULL,
...
static void
nb_cli_pending_commit_clear (struct vty *vty)
{
...
int
nb_cli_pending_commit_check (struct vty *vty)
{
  int ret = CMD_SUCCESS;
...
nb_cli_schedule_command (struct vty *vty)
{
  ...
  vty->pending_cmds_bufpos =
      strlcat (vty->pending_cmds_buf, vty->buf, vty->pending_cmds_buflen);
...
void
nb_cli_enqueue_change (struct vty *vty, const char *xpath,
...
static int
nb_cli_apply_changes_internal (struct vty *vty, const char *xpath_base,
                               bool clear_pending)
{
  bool error = false;
  char buf[BUFSIZ];
  ...
  nb_candidate_edit_config_changes (
      ...
      xpath_base, VTY_CURR_XPATH, vty->xpath_index, buf, sizeof (buf), &error);
  if (error)
    {
      /*
...
int
nb_cli_apply_changes (struct vty *vty, const char *xpath_base_fmt, ...)
{
...
int
nb_cli_apply_changes_clear_pending (struct vty *vty,
...
int
nb_cli_rpc (struct vty *vty, const char *xpath, struct list *input,
            struct list *output)
{
  ...
  ret =
      nb_callback_rpc (nb_node, xpath, input, output, errmsg, sizeof (errmsg));
  switch (ret)
    {
    case NB_OK:
      return CMD_SUCCESS;
...
void
nb_cli_confirmed_commit_clean (struct vty *vty)
{
...
int
nb_cli_confirmed_commit_rollback (struct vty *vty)
{
  struct nb_context context = {};
  ...
  if (ret == NB_OK)
    {
      vty_out (vty, "Rollback performed successfully (Transaction ID #%u).\n",
               transaction_id);
...
static void
nb_cli_confirmed_commit_timeout (struct event *thread)
{
...
static int
nb_cli_commit (struct vty *vty, bool force, unsigned int confirmed_timeout,
               char *comment)
{
  struct nb_context context = {};
...
static int
nb_cli_candidate_load_file (struct vty *vty, enum nb_cfg_format format,
                            ...
                            const char *path, bool replace)
{
  ...
  switch (format)
    ...
    case NB_CFG_FMT_XML:
      ...
      err =
          lyd_parse_data_path (ly_ctx, path, ly_format,
                               LYD_PARSE_ONLY | LYD_PARSE_NO_STATE, 0, &dnode);
      if (err || ! dnode)
        {
          flog_warn (EC_LIB_LIBYANG, "%s: lyd_parse_path() failed", __func__);
          vty_out (vty, "%% Failed to load configuration:\n\n");
          vty_out (vty, "%s",
...
static int
nb_cli_candidate_load_transaction (struct vty *vty, uint32_t transaction_id,
                                   bool replace)
{
...
void
nb_cli_show_config_prepare (struct nb_config *config, bool with_defaults)
{
...
static int
lyd_node_cmp (const struct lyd_node **dnode1, const struct lyd_node **dnode2)
{
...
static void
show_dnode_children_cmds (struct vty *vty, const struct lyd_node *root,
                          bool with_defaults)
{
...
void
nb_cli_show_dnode_cmds (struct vty *vty, const struct lyd_node *root,
                        bool with_defaults)
{
  struct nb_node *nb_node;
...
static void
nb_cli_show_config_cmds (struct vty *vty, struct nb_config *config,
                         bool with_defaults)
{
...
static int
nb_cli_show_config_libyang (struct vty *vty, LYD_FORMAT format,
                            ...
                            bool with_defaults)
{
  ...
  if (translator &&
      yang_translate_dnode (translator, YANG_TRANSLATE_FROM_NATIVE, &dnode) !=
...
static int
nb_cli_show_config (struct vty *vty, struct nb_config *config,
                    enum nb_cfg_format format,
                    struct yang_translator *translator, bool with_defaults)
{
  ...
  switch (format)
    ...
    case NB_CFG_FMT_CMDS:
      ...
      break;
    case NB_CFG_FMT_JSON:
      return nb_cli_show_config_libyang (vty, LYD_JSON, config, translator,
                                         with_defaults);
    case NB_CFG_FMT_XML:
      return nb_cli_show_config_libyang (vty, LYD_XML, config, translator,
                                         with_defaults);
    }
...
static int
nb_write_config (struct nb_config *config, enum nb_cfg_format format,
                 ...
                 size_t pathlen)
{
  ...
  if (fd < 0)
    {
      flog_warn (EC_LIB_SYSTEM_CALL, "%s: mkstemp() failed: %s", __func__,
                 safe_strerror (errno));
      return -1;
    }
  if (fchmod (fd, CONFIGFILE_MASK) != 0)
    {
      flog_warn (EC_LIB_SYSTEM_CALL, "%s: fchmod() failed: %s(%d):", __func__,
  ...
  if (config)
    ret = nb_cli_show_config (file_vty, config, format, translator, false);
  vty_close (file_vty);
...
static int
nb_cli_show_config_compare (struct vty *vty, struct nb_config *config1,
       ...
       "Update candidate configuration\n")
{
  ...
  if (nb_candidate_update (vty->candidate_config) != NB_OK)
    {
      vty_out (vty, "%% Failed to update the candidate configuration.\n\n");
      vty_out (vty, "Please check the logs for more details.\n");
      return CMD_WARNING;
    }
  ...
  if (filename)
    {
      ...
      return nb_cli_candidate_load_file (vty, format, translator, filename,
                                         ! ! replace);
    }
       ...
       SHOW_STR)
{
  ...
  if (vty->xpath_index == 0)
    return nb_cli_show_config (vty, vty->candidate_config, NB_CFG_FMT_CMDS,
                               NULL, false);
...
#ifdef HAVE_CONFIG_ROLLBACKS
static void
nb_cli_show_transactions_cb (void *arg, int transaction_id,
...
static int
nb_cli_show_transactions (struct vty *vty)
{
  struct ttable *tt;
  ...
  if (transaction_id)
    {
      ...
      if (changes)
        {
          ...
          ret = nb_cli_show_config_compare (vty, prev_config, config, format,
                                            translator);
          if (prev_config)
      ...
      nb_cli_show_config (vty, config, format, translator, ! ! with_defaults);
      nb_config_free (config);
...
static int
nb_cli_oper_data_cb (const struct lysc_node *snode,
                     struct yang_translator *translator,
                     struct yang_data *data, void *arg)
{
  ...
  if (err)
    {
      flog_warn (EC_LIB_LIBYANG, "%s: lyd_new_path(%s) failed: %s", __func__,
                 data->xpath, ly_errmsg (ly_native_ctx));
      goto error;
    }
       ...
       "Merge configuration data\n")
{
  ...
  ret =
      nb_oper_data_iterate (xpath, translator, 0, nb_cli_oper_data_cb, dnode);
  if (ret != NB_OK)
    {
      if (format == LYD_JSON)
        vty_out (vty, "{}\n");
  ...
  while ((module = ly_ctx_get_module_iter (ly_ctx, &idx)))
    {
      ...
      snprintf (flags, sizeof (flags), "%c%c", module->implemented ? 'I' : ' ',
      ...
      for (ALL_LIST_ELEMENTS_RO (translator->modules, ln, tmodule))
        {
          ttable_add_row (tt, "%s|%s|%s|%.2f", translator->family,
                          tmodule->module->name, tmodule->deviations->name,
                          tmodule->coverage);
        }
    }
...
#ifdef HAVE_CONFIG_ROLLBACKS
static int
nb_cli_rollback_configuration (struct vty *vty, uint32_t transaction_id)
{
  struct nb_context context = {};
  ...
  context.client = NB_CLIENT_CLI;
  context.user = vty;
  ret = nb_candidate_commit (context, candidate, true, comment, NULL, errmsg,
                             sizeof (errmsg));
  ...
  switch (ret)
    ...
    case NB_OK:
      ...
      return CMD_SUCCESS;
    case NB_ERR_NO_CHANGES:
      vty_out (vty, "%% Aborting - no configuration changes detected.\n\n");
      return CMD_WARNING;
    default:
...
DEFINE_HOOK (nb_client_debug_set_all, (uint32_t flags, bool set),
             (flags, set));

static void
nb_debug_set_all (uint32_t flags, bool set)
{
...
static int
nb_debug_config_write (struct vty *vty)
{
  for (unsigned int i = 0; i < array_size (nb_debugs); i++)
...
static struct debug_callbacks nb_dbg_cbs = { .debug_set_all =
                                                 nb_debug_set_all };
...
void
nb_cli_install_default (int node)
{
...
static void
yang_module_autocomplete (vector comps, struct cmd_token *token)
{
  const struct lys_module *module;
...
static void
yang_translator_autocomplete (vector comps, struct cmd_token *token)
{
  struct yang_translator *module_tr;
...
void
nb_cli_init (struct event_loop *tm)
{
  master = tm;
