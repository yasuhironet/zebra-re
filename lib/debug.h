...
#define DEBUG_OPT_NONE  0x00000000
   ...
   *
   *    The low 3 bytes of this field may be used to store arbitrary
   * information. Usually they are used to store flags that tune how detailed
   * the logging for a particular debug record is. Use the DEBUG_OPT* macros to
   * manipulate those bytes.
   *
...
#define DEBUG_FLAGS_OFF(name, fl)                                             \
  DEBUG_FLAGS_SET (&(name)->flags, (type), false)
