...
static inline void
mt_count_alloc (struct memtype *mt, size_t size, void *ptr)
{
  size_t current;
  size_t oldsize;

  current =
      1 + atomic_fetch_add_explicit (&mt->n_alloc, 1, memory_order_relaxed);

  oldsize = atomic_load_explicit (&mt->n_max, memory_order_relaxed);
  if (current > oldsize)
    /* note that this may fail, but approximation is sufficient */
    atomic_compare_exchange_weak_explicit (&mt->n_max, &oldsize, current,
  ...
  if (current > oldsize)
    /* note that this may fail, but approximation is sufficient */
    atomic_compare_exchange_weak_explicit (&mt->max_size, &oldsize, current,
                                           memory_order_relaxed,
                                           memory_order_relaxed);
#endif
}
...
static inline void
mt_count_free (struct memtype *mt, void *ptr)
{
...
void
qcountfree (struct memtype *mt, void *ptr)
{
  if (ptr)
    mt_count_free (mt, ptr);
}
...
int
log_memstats (FILE *fp, const char *prefix)
{
