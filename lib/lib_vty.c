...
static int
qmem_walker (void *arg, struct memgroup *mg, struct memtype *mt)
{
  struct vty *vty = arg;
  if (! mt)
    {
            ...
            "The End of Configuration\n")
{
  ...
  frrtime_to_interval (readin_time, readin_time_str, sizeof (readin_time_str));
...
void
cmd_init_config_callbacks (void (*start_config_cb) (void),
...
static void
defaults_autocomplete (vector comps, struct cmd_token *token)
{
  const char **p;
