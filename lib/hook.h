...
extern "C"
{
   ...
   * -1999 ... -1000          - modules registering calls that should run
   * before the daemon's bits 1000 ... 1999 - modules calls that should run
   * after daemon's
   *
...
#define _hook_reg_svar(hook, funcptr, arg, has_arg, module, funcname, prio)   \
  do                                                                          \
    {                                                                         \
      ...
      _hook_register (hook, &stack_hookent, funcptr, arg, has_arg, module,    \
                      funcname, prio);                                        \
    }                                                                         \
  while (0)

#define hook_register(hookname, func)                                         \
  _hook_reg_svar (&_hook_##hookname, _hook_typecheck_##hookname (func), NULL, \
                  false, THIS_MODULE, #func, HOOK_DEFAULT_PRIORITY)
...
#define hook_unregister_arg(hookname, func, arg)                              \
  _hook_unregister (&_hook_##hookname, _hook_typecheck_arg_##hookname (func), \
                    arg, true)
...
#define DECLARE_HOOK(hookname, arglist, passlist)                             \
  ...
  __attribute__ ((unused)) static inline void                                 \
      *_hook_typecheck_arg_##hookname (int (*funcptr) HOOK_ADDDEF arglist)    \
  {                                                                           \
    return (void *) funcptr;                                                  \
  }                                                                           \
