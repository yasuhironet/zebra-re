...
static int
event_timer_cmp (const struct event *a, const struct event *b)
{
...
static unsigned int
cpu_record_hash_key (const struct cpu_event_history *a)
{
...
static bool
cpu_record_hash_cmp (const struct cpu_event_history *a,
...
static void *
cpu_record_hash_alloc (struct cpu_event_history *a)
{
...
static void
vty_out_cpu_event_history (struct vty *vty, struct cpu_event_history *a)
{
  vty_out (vty, "%5zu %10zu.%03zu %9zu %8zu %9zu %8zu %9zu %9zu %9zu %10zu",
           ...
           (a->real.total / a->total_calls), a->real.max, a->total_cpu_warn,
           a->total_wall_warn, a->total_starv_warn);
...
static void
cpu_record_hash_print (struct hash_bucket *bucket, void *args[])
{
  ...
  copy.total_starv_warn =
      atomic_load_explicit (&a->total_starv_warn, memory_order_seq_cst);
  copy.cpu.total = atomic_load_explicit (&a->cpu.total, memory_order_seq_cst);
...
static void
cpu_record_print (struct vty *vty, uint8_t filter)
{
...
static void
cpu_record_hash_clear (struct hash_bucket *bucket, void *args[])
{
...
static void
cpu_record_clear (uint8_t filter)
{
  uint8_t *tmp = &filter;
  struct event_loop *m;
  struct listnode *ln;
...
static void
show_thread_poll_helper (struct vty *vty, struct event_loop *m)
{
  const char *name = m->name ? m->name : "main";
  char underline[strlen (name) + 1];
  struct event *thread;
  ...
  vty_out (vty, "----------------------%s\n", underline);
  vty_out (vty, "Count: %u/%d\n", (uint32_t) m->handler.pfdcount, m->fd_limit);
...
static void
show_thread_timers_helper (struct vty *vty, struct event_loop *m)
{
  const char *name = m->name ? m->name : "main";
  char underline[strlen (name) + 1];
  struct event *thread;
...
event_master_create (const char *name)
{
  ...
  rv->read =
      XCALLOC (MTYPE_EVENT_POLL, sizeof (struct event *) * rv->fd_limit);
  ...
  rv->cpu_record = hash_create_size (
      ...
      (bool (*) (const void *, const void *)) cpu_record_hash_cmp, tmhashname);
...
void
event_master_set_name (struct event_loop *master, const char *name)
{
...
static void
thread_add_unuse (struct event_loop *m, struct event *thread)
{
...
static void
thread_list_free (struct event_loop *m, struct event_list_head *list)
{
  struct event *t;
...
static void
thread_array_free (struct event_loop *m, struct event **thread_array)
{
  struct event *t;
  int index;
 ...
 */
void
event_master_free_unused (struct event_loop *m)
{
  frr_with_mutex (&m->mtx)
    {
      struct event *t;
...
void
event_master_free (struct event_loop *m)
{
  struct event *t;
...
unsigned long
event_timer_remain_msec (struct event *thread)
{
  int64_t remain;
...
unsigned long
event_timer_remain_second (struct event *thread)
{
  return event_timer_remain_msec (thread) / 1000LL;
}

struct timeval
event_timer_remain (struct event *thread)
{
...
static int
time_hhmmss (char *buf, int buf_size, long sec)
{
  long hh;
  long mm;
  int wr;
...
char *
event_timer_to_hhmmss (char *buf, int buf_size, struct event *t_timer)
{
  if (t_timer)
...
/* Get new thread.  */
static struct event *
thread_get (struct event_loop *m, uint8_t type, void (*func) (struct event *),
            void *arg, const struct xref_eventsched *xref)
{
...
static void
thread_free (struct event_loop *master, struct event *thread)
{
  /* Update statistics. */
...
static int
fd_poll (struct event_loop *m, const struct timeval *timer_wait, bool *eintr_p)
{
  sigset_t origsigs;
...
/* Add new read thread. */
void
_event_add_read_write (const struct xref_eventsched *xref,
                       ...
                       void *arg, int fd, struct event **t_ptr)
{
  ...
  if (dir == EVENT_READ)
    frrtrace (9, frr_libfrr, schedule_read, m, xref->funcname, xref->xref.file,
              xref->xref.line, t_ptr, fd, 0, arg, 0);
  else
  ...
  if (fd >= m->fd_limit)
    assert (
        ! "Number of FD's open is greater than FRR currently configured to handle, aborting");
...
static void
_event_add_timer_timeval (const struct xref_eventsched *xref,
                          struct event_loop *m, void (*func) (struct event *),
                          ...
                          struct event **t_ptr)
{
  ...
  frrtrace (9, frr_libfrr, schedule_timer, m, xref->funcname, xref->xref.file,
            xref->xref.line, t_ptr, 0, 0, arg, (long) time_relative->tv_sec);
...
void
_event_add_timer (const struct xref_eventsched *xref, struct event_loop *m,
...
void
_event_add_timer_msec (const struct xref_eventsched *xref,
                       struct event_loop *m, void (*func) (struct event *),
...
void
_event_add_timer_tv (const struct xref_eventsched *xref, struct event_loop *m,
                     void (*func) (struct event *), void *arg,
                     struct timeval *tv, struct event **t_ptr)
{
...
void
_event_add_event (const struct xref_eventsched *xref, struct event_loop *m,
                  void (*func) (struct event *), void *arg, int val,
                  struct event **t_ptr)
{
  struct event *thread = NULL;

  frrtrace (9, frr_libfrr, schedule_event, m, xref->funcname, xref->xref.file,
            xref->xref.line, t_ptr, 0, val, arg, 0);
 ...
 */
static void
event_cancel_rw (struct event_loop *master, int fd, short state, int idx_hint)
{
  bool found = false;
 ...
 */
static void
cancel_arg_helper (struct event_loop *master, const struct cancel_req *cr)
{
  struct event *t;
  nfds_t i;
  int fd;
 ...
 */
static void
do_event_cancel (struct event_loop *master)
{
 ...
 */
static void
cancel_event_helper (struct event_loop *m, void *arg, int flags)
{
  struct cancel_req *cr;
 ...
 */
void
event_cancel_event (struct event_loop *master, void *arg)
{
 ...
 */
void
event_cancel_event_ready (struct event_loop *m, void *arg)
{
 ...
 */
void
event_cancel (struct event **thread)
{
 ...
 */
void
event_cancel_async (struct event_loop *master, struct event **thread,
                    void *eventobj)
{
  ...
  if (thread && *thread)
    ...
    frrtrace (9, frr_libfrr, event_cancel_async, master, NULL, NULL, 0, NULL,
              0, 0, eventobj, 0);
...
thread_timer_wait (struct event_timer_list_head *timers,
...
static struct event *
thread_run (struct event_loop *m, struct event *thread, struct event *fetch)
{
  *fetch = *thread;
...
static int
thread_process_io_helper (struct event_loop *m, struct event *thread,
 ...
 */
static void
thread_process_io (struct event_loop *m, unsigned int num)
{
...
static unsigned int
thread_process_timers (struct event_loop *m, struct timeval *timenow)
{
...
static unsigned int
thread_process (struct event_list_head *list)
{
  struct event *thread;
  unsigned int ready = 0;
...
struct event *
event_fetch (struct event_loop *m, struct event *fetch)
{
  struct event *thread = NULL;
  struct timeval now;
...
unsigned long
event_consumed_time (RUSAGE_T *now, RUSAGE_T *start, unsigned long *cputime)
{
 ...
 */
int
event_should_yield (struct event *thread)
{
  int result;
...
void
event_set_yield_time (struct event *thread, unsigned long yield_time)
{
  frr_with_mutex (&thread->mtx)
    {
 ...
 */
void
event_call (struct event *thread)
{
  ...
  frrtrace (9, frr_libfrr, event_call, thread->master, thread->xref->funcname,
            thread->xref->xref.file, thread->xref->xref.line, NULL,
            thread->u.fd, thread->u.val, thread->arg, thread->u.sands.tv_sec);
  ...
  atomic_fetch_add_explicit (&thread->hist->real.total, walltime,
                             memory_order_seq_cst);
  exp = atomic_load_explicit (&thread->hist->real.max, memory_order_seq_cst);
  while (exp < walltime && ! atomic_compare_exchange_weak_explicit (
  ...
  if (cputime_enabled_here && cputime_enabled)
    {
      ...
      atomic_fetch_add_explicit (&thread->hist->cpu.total, cputime,
                                 memory_order_seq_cst);
      exp =
          atomic_load_explicit (&thread->hist->cpu.max, memory_order_seq_cst);
      while (exp < cputime && ! atomic_compare_exchange_weak_explicit (
...
/* Execute thread */
void
_event_execute (const struct xref_eventsched *xref, struct event_loop *m,
                void (*func) (struct event *), void *arg, int val)
{
  struct event *thread;
...
void
debug_signals (const sigset_t *sigs)
{
  int i, found;
  sigset_t tmpsigs;
  char buf[300];
...
static ssize_t
printfrr_thread_dbg (struct fbuf *buf, struct printfrr_eargs *ea,
...
static ssize_t
printfrr_thread (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
  ...
  if (! thread)
    {
       ...
       */
      printfrr_time (buf, ea, &remain, TIMEFMT_TIMER_DEADLINE | TIMEFMT_SKIP);
      return bputch (buf, '-');
    }
