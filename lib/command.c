...
const char *
cmd_release_get (void)
{
  return host.release;
}

const char *
cmd_version_get (void)
{
  return host.version;
}

bool
cmd_allow_reserved_ranges_get (void)
{
  return host.allow_reserved_ranges;
}

const char *
cmd_software_version_get (void)
{
...
void
_install_element (enum node_type ntype, const struct cmd_element *cmd)
{
  struct cmd_node *cnode;
...
static void
cmd_finalize_iter (struct hash_bucket *hb, void *arg)
{
...
void
cmd_finalize_node (struct cmd_node *cnode)
{
...
void
uninstall_element (enum node_type ntype, const struct cmd_element *cmd)
{
  struct cmd_node *cnode;
  ...
  if (full_cli)
    {
      ...
      if (host.motdfile)
        vty_out (vty, "banner motd file %s\n", host.motdfile);
               ...
               strncmp (host.motd, FRR_DEFAULT_MOTD, strlen (host.motd)))
 ...
 * completions (as is the case in vtysh). For instance, if the passed vline
 * ends with '4.3.2', the strings 'A.B.C.D' and 'A.B.C.D/M' will _not_ be
 * returned.
 *
      ...
      for (unsigned int i = 0; i < vector_active (initial_comps); i++)
        {
          ...
          if (token->type == WORD_TKN)
            ...
            {
              const char *ref =
                  vector_lookup (vline, vector_active (vline) - 1);
...
static int
cmd_execute_command_real (vector vline, enum cmd_filter_type filter,
                          struct vty *vty, const struct cmd_element **cmd,
                          unsigned int up_level)
{
  ...
  struct cmd_token **argv =
      XMALLOC (MTYPE_TMP, argv_list->count * sizeof (struct cmd_token *));
  struct listnode *ln;
      ...
      if (vty->config)
        {
          ...
          if (frr_get_cli_mode () == FRR_CLI_CLASSIC &&
              running_config->version > vty->candidate_config->version)
      ...
      while (vty->node > CONFIG_NODE)
        {
          ...
          ret = cmd_execute_command_real (vline, FILTER_RELAXED, vty, cmd, 0);
 ...
 */
static int
handle_pipe_action (struct vty *vty, const char *cmd_in, char **cmd_out)
{
  /* look for `|` */
...
static int
handle_pipe_action_done (struct vty *vty, const char *cmd_exec)
{
  if (vty->filter)
...
int
cmd_execute (struct vty *vty, const char *cmd,
 ...
 */
int
command_config_read_one_line (struct vty *vty, const struct cmd_element **cmd,
                              uint32_t line_num, int use_daemon)
{
  ...
  while (! (use_daemon && ret == CMD_SUCCESS_DAEMON) &&
         ...
         ret != CMD_NO_LEVEL_UP)
    ret =
        cmd_execute_command_real (vline, FILTER_STRICT, vty, cmd, ++up_level);

  if (ret == CMD_NO_LEVEL_UP)
    ret = CMD_ERR_NO_MATCH;

  if (ret != CMD_SUCCESS && ret != CMD_WARNING && ret != CMD_SUCCESS_DAEMON)
    {
      ...
      if (vty_log_commands)
        {
          ...
          /* now log the command */
          zlog_notice ("config-from-file# %.*s", len ? len - 1 : 0, vty->buf);
        }
...
static void
print_cmd (struct vty *vty, const char *cmd)
{
...
static int
file_write_config (struct vty *vty)
{
  int fd, dirfd;
...
int
cmd_domainname_set (const char *domainname)
{
...
void
cmd_banner_motd_line (const char *line)
{
...
DEFUN (no_allow_reserved_ranges,
...
int
cmd_find_cmds (struct vty *vty, struct cmd_token **argv, int argc)
{
  ...
  if (cr != 0)
    {
      switch (cr)
        ...
        case REG_ECTYPE:
          ...
          break;
        case REG_EESCAPE:
          vty_out (vty, "%% Regex ended with escape character (\\)\n");
          break;
        case REG_ESUBREG:
          vty_out (vty, "%% Invalid number in \\digit construction\n");
          break;
        case REG_EBRACK:
        ...
        case REG_EBRACE:
          ...
          break;
        case REG_ERANGE:
          vty_out (vty, "%% Invalid endpoint in range expression\n");
          break;
        case REG_ESPACE:
       ...
       "Function name (in the script)\n")
{
  ...
  if (frrscript_load (fs, argv[2]->arg, NULL))
    {
      vty_out (vty, "/etc/frr/scripts/%s.lua or function '%s' not found\n",
...
void
cmd_show_lib_debugs (struct vty *vty)
{
