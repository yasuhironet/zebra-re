...
static inline int
yang_translator_compare (const struct yang_translator *a,
...
static bool
yang_mapping_hash_cmp (const void *value1, const void *value2)
{
...
static unsigned int
yang_mapping_hash_key (const void *value)
{
  return string_hash_make (value);
}

static void *
yang_mapping_hash_alloc (void *p)
{
...
static void
yang_mapping_hash_free (void *arg)
{
...
static void
yang_mapping_add (struct yang_translator *translator, int dir,
                  const struct lysc_node *snode, const char *xpath_from_fmt,
                  const char *xpath_to_fmt)
{
  ...
  mapping = hash_get (translator->mappings[dir], &s, yang_mapping_hash_alloc);
  ...
  for (unsigned int i = 0; i < array_size (keys); i++)
    {
      ...
      strlcpy (mapping->xpath_to_fmt, xpfmt, sizeof (mapping->xpath_to_fmt));
      XFREE (MTYPE_TMP, xpfmt);
    }
}

static void
yang_tmodule_delete (struct yang_tmodule *tmodule)
{
...
yang_translator_load (const char *path)
{
  ...
  err =
      lyd_parse_data_path (ly_translator_ctx, path, LYD_JSON,
                           LYD_PARSE_NO_STATE, LYD_VALIDATE_NO_STATE, &dnode);
  if (err)
    {
  ...
  dnode =
      yang_dnode_get (dnode, "/frr-module-translator:frr-module-translator");
  /*
  ...
  if (translator != NULL)
    {
      flog_warn (EC_LIB_YANG_TRANSLATOR_LOAD,
                 "%s: module translator \"%s\" is loaded already", __func__,
                 family);
  ...
  for (size_t i = 0; i < YANG_TRANSLATE_MAX; i++)
    translator->mappings[i] =
        hash_create (yang_mapping_hash_key, yang_mapping_hash_cmp,
  ...
  for (size_t i = 0; i < set->count; i++)
    {
      ...
      tmodule = XCALLOC (MTYPE_YANG_TRANSLATOR_MODULE, sizeof (*tmodule));

      module_name = yang_dnode_get_string (set->dnodes[i], "./name");
      tmodule->module =
          ly_ctx_load_module (translator->ly_ctx, module_name, NULL, NULL);
      if (! tmodule->module)
        {
          flog_warn (EC_LIB_YANG_TRANSLATOR_LOAD,
                     "%s: failed to load module: %s", __func__, module_name);
  ...
  for (size_t i = 0; i < set->count; i++)
    {
      ...
      xpath_custom = yang_dnode_get_string (set->dnodes[i], "./custom");

      snode_custom = yang_find_snode (translator->ly_ctx, xpath_custom, 0);
      if (! snode_custom)
        {
          flog_warn (EC_LIB_YANG_TRANSLATOR_LOAD, "%s: unknown data path: %s",
                     __func__, xpath_custom);
          ly_set_free (set, NULL);
          goto error;
        }

      xpath_native = yang_dnode_get_string (set->dnodes[i], "./native");
      snode_native = yang_find_snode (ly_native_ctx, xpath_native, 0);
      if (! snode_native)
        {
          flog_warn (EC_LIB_YANG_TRANSLATOR_LOAD, "%s: unknown data path: %s",
                     __func__, xpath_native);
          ly_set_free (set, NULL);
          goto error;
        }

      yang_mapping_add (translator, YANG_TRANSLATE_TO_NATIVE, snode_custom,
                        xpath_custom, xpath_native);
      yang_mapping_add (translator, YANG_TRANSLATE_FROM_NATIVE, snode_native,
                        xpath_native, xpath_custom);
    }
  ly_set_free (set, NULL);
...
void
yang_translator_unload (struct yang_translator *translator)
{
  for (size_t i = 0; i < YANG_TRANSLATE_MAX; i++)
                      ...
                      char *xpath, size_t xpath_len)
{
  ...
  if (! snode)
    {
      flog_warn (EC_LIB_YANG_TRANSLATION_ERROR, "%s: unknown data path: %s",
                 __func__, xpath);
...
#pragma GCC diagnostic pop
  if (n < 0)
    {
      flog_warn (EC_LIB_YANG_TRANSLATION_ERROR, "%s: sscanf() failed: %s",
...
#pragma GCC diagnostic ignored "-Wformat-nonliteral"
  /* processing format strings from mapping node... */
  snprintf (xpath, xpath_len, mapping->xpath_to_fmt, keys[0], keys[1], keys[2],
            keys[3]);
#pragma GCC diagnostic pop
...
int
yang_translate_dnode (const struct yang_translator *translator, int dir,
                      struct lyd_node **dnode)
{
  struct ly_ctx *ly_ctx;
  struct lyd_node *new;
      ...
      LYD_TREE_DFS_BEGIN (root, dnode_iter)
        {
          ...
          ret = yang_translate_xpath (translator, dir, xpath, sizeof (xpath));
          switch (ret)
            {
          ...
          if (lyd_new_path (new, ly_ctx, xpath,
            ...
            {
              flog_err (EC_LIB_LIBYANG, "%s: lyd_new_path() failed", __func__);
              goto error;
            }
...
static int
yang_translator_validate_cb (const struct lysc_node *snode_custom, void *arg)
{
  ...
  mapping =
      yang_mapping_lookup (args->translator, YANG_TRANSLATE_TO_NATIVE, xpath);
  if (! mapping)
    {
      flog_warn (EC_LIB_YANG_TRANSLATOR_LOAD, "%s: missing mapping for \"%s\"",
                 __func__, xpath);
      args->errors += 1;
      return YANG_ITER_CONTINUE;
    }

  snode_native = lys_find_path (ly_native_ctx, NULL, mapping->xpath_to_fmt, 0);
  assert (snode_native);
...
yang_translator_validate (struct yang_translator *translator)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (translator->modules, ln, tmodule))
    {
      yang_snodes_iterate (tmodule->module, yang_translator_validate_cb,
...
static int
yang_module_nodes_count_cb (const struct lysc_node *snode, void *arg)
{
...
static unsigned int
yang_module_nodes_count (const struct lys_module *module)
{
...
yang_translator_init (void)
{
  ...
  if (! ly_ctx_load_module (ly_translator_ctx, "frr-module-translator", NULL,
                            NULL))
    {
...
void
yang_translator_terminate (void)
{
