...
csv_t *
csv_init (csv_t *csv, char *buf, int buflen)
{
  if (csv == NULL)
    {
...
char *
csv_field_iter (csv_record_t *rec, csv_field_t **fld)
{
  if (! rec)
    return NULL;
...
static csv_field_t *
csv_add_field_to_record (csv_t *csv, csv_record_t *rec, char *col)
{
  csv_field_t *fld;
  char *str = rec->record;
...
csv_record_t *
csv_encode (csv_t *csv, int count, ...)
{
  int tempc;
  va_list list;
  char *buf = csv->buf;
...
csv_record_t *
csv_encode_record (csv_t *csv, csv_record_t *rec, int count, ...)
{
  int tempc;
  va_list list;
  char *str;
...
csv_record_t *
csv_concat_record (csv_t *csv, csv_record_t *rec1, csv_record_t *rec2)
{
  char *curr;
  char *ret;
  csv_record_t *rec;
