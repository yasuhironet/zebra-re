...
#define ZEBRA_SR_POLICY_NAME_MAX_LENGTH 100
  ...
  zebra_error_type2str (enum zebra_error_types type)
  {
...
#define ZAPI_MESSAGE_OPAQUE_LENGTH 1024
  ...
  extern char *zclient_dump_route_flags (uint32_t flags, char *buf,
                                         size_t len);
...
#define ZEBRA_NEIGH_STATE_NONE       (0x00)
  ...
  int zclient_neigh_ip_encode (struct stream *s, uint16_t cmd,
                               union sockunion *in, union sockunion *out,
                               struct interface *ifp, int ndm_state);
...
#define ZEBRA_NHG_PROTO_LOWER                                                 \
  ...
  extern void zclient_redistribute (int command, struct zclient *, afi_t,
   ...
   * This function assumes the stream getp points at the first byte of the
   * header. If the function is successful then the stream getp will point to
   * the byte immediately after the last byte of the header.
   *
   * zmsg
  ...
  extern enum zclient_send_status
  zclient_interface_set_master (struct zclient *client,
  ...
  zebra_interface_vrf_update_read (struct stream *s, vrf_id_t vrf_id,
                                   vrf_id_t *new_vrf_id);
  extern int zebra_router_id_update_read (struct stream *s,
                                          struct prefix *rid);
  ...
  extern int zapi_sr_policy_encode (struct stream *s, int cmd,
                                    struct zapi_sr_policy *zp);
  extern int zapi_sr_policy_decode (struct stream *s,
                                    struct zapi_sr_policy *zp);
  ...
  extern enum zclient_send_status
  zclient_route_send (uint8_t, struct zclient *, struct zapi_route *);
  extern enum zclient_send_status
  zclient_send_rnh (struct zclient *zclient, int command,
                    const struct prefix *p, safi_t safi, bool connected,
                    bool resolve_via_default, vrf_id_t vrf_id);
  int zapi_nexthop_encode (struct stream *s, const struct zapi_nexthop *api_nh,
                           uint32_t api_flags, uint32_t api_message);
  extern int zapi_route_encode (uint8_t, struct stream *, struct zapi_route *);
  extern int zapi_route_decode (struct stream *s, struct zapi_route *api);
  extern int zapi_nexthop_decode (struct stream *s,
  ...
  extern enum zclient_send_status zclient_nhg_send (struct zclient *zclient,
                                                    int cmd,
...
#define ZEBRA_IPSET_NAME_SIZE 32
  ...
  extern bool zapi_error_decode (struct stream *s,
  ...
  zclient_capabilities_send (uint32_t cmd, struct zclient *zclient,
                             struct zapi_cap *api);
  extern int32_t zapi_capabilities_decode (struct stream *s,
                                           struct zapi_cap *api);

  static inline void
  zapi_route_set_blackhole (struct zapi_route *api,
                            enum blackhole_type bh_type)
  {
    api->nexthop_num = 1;
  ...
  enum zclient_send_status zclient_send_opaque_unicast (
      struct zclient *zclient, uint32_t type, uint8_t proto, uint16_t instance,
      uint32_t session_id, const uint8_t *data, size_t datasize);
