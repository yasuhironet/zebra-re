...
hash_expand (struct hash *hash)
{
  ...
  new_index =
      XCALLOC (MTYPE_HASH_INDEX, sizeof (struct hash_bucket *) * new_size);
...
unsigned int
string_hash_make (const char *str)
{
  unsigned int hash = 0;
...
void
hash_iterate (struct hash *hash, void (*func) (struct hash_bucket *, void *),
              void *arg)
{
  unsigned int i;
...
void
hash_walk (struct hash *hash, int (*func) (struct hash_bucket *, void *),
           void *arg)
{
  unsigned int i;
  struct hash_bucket *hb;
...
void
hash_clean_and_free (struct hash **hash, void (*free_func) (void *))
{
  if (! *hash)
    return;
...
static void
hash_to_list_iter (struct hash_bucket *hb, void *arg)
{
