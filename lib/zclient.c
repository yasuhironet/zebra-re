...
struct zclient *
zclient_new (struct event_loop *master, struct zclient_options *opt,
...
unsigned short *
redist_check_instance (struct redist_proto *red, unsigned short instance)
{
...
void
redist_add_instance (struct redist_proto *red, unsigned short instance)
{
  unsigned short *in;
...
void
redist_del_instance (struct redist_proto *red, unsigned short instance)
{
  unsigned short *id;
...
void
redist_del_all_instances (struct redist_proto *red)
{
...
zclient_socket_connect (struct zclient *zclient)
{
  ...
  if (ret < 0)
    {
      if (zclient_debug)
        zlog_debug ("%s connect failure: %d(%s)", __func__, errno,
                    safe_strerror (errno));
      close (sock);
      return -1;
    }
...
static void
zclient_flush_data (struct event *thread)
{
...
zclient_send_message (struct zclient *zclient)
{
  ...
  switch (buffer_write (zclient->wb, zclient->sock,
    ...
    case BUFFER_ERROR:
      flog_err (EC_LIB_ZAPI_SOCKET,
                "%s: buffer_write failed to zclient fd %d, closing", __func__,
                zclient->sock);
...
int
zclient_read_header (struct stream *s, int sock, uint16_t *size,
...
bool
zapi_parse_header (struct stream *zmsg, struct zmsghdr *hdr)
{
...
enum zclient_send_status
zclient_send_hello (struct zclient *zclient)
{
  struct stream *s;
...
zclient_send_reg_requests (struct zclient *zclient, vrf_id_t vrf_id)
{
  ...
  zclient_send_router_id_update (zclient, ZEBRA_ROUTER_ID_ADD, AFI_IP, vrf_id);
          ...
          for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
            {
              ...
              for (ALL_LIST_ELEMENTS_RO (zclient->mi_redist[afi][i].instances,
                                         node, id))
                if (! (i == zclient->redist_default &&
                       *id == zclient->instance))
                  zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient,
                                           afi, i, *id, VRF_DEFAULT);
            }
        }
    }
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
        if (i != zclient->redist_default &&
          ...
          zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient, afi, i, 0,
                                   vrf_id);

      /* If default information is needed. */
      if (vrf_bitmap_check (zclient->default_information[afi], vrf_id))
        zebra_redistribute_default_send (ZEBRA_REDISTRIBUTE_DEFAULT_ADD,
                                         zclient, afi, vrf_id);
    }
}
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      ...
      /* If default information is needed. */
      if (vrf_bitmap_check (zclient->default_information[afi], vrf_id))
        zebra_redistribute_default_send (ZEBRA_REDISTRIBUTE_DEFAULT_DELETE,
                                         zclient, afi, vrf_id);
    }
}
...
zclient_start (struct zclient *zclient)
{
  ...
  if (set_nonblocking (zclient->sock) < 0)
    flog_err (EC_LIB_ZAPI_SOCKET, "%s: set_nonblocking(%d) failed", __func__,
              zclient->sock);
...
static void
zclient_connect (struct event *t)
{
  struct zclient *zclient;
...
zclient_send_rnh (struct zclient *zclient, int command, const struct prefix *p,
                  safi_t safi, bool connected, bool resolve_via_def,
                  vrf_id_t vrf_id)
{
  struct stream *s;
 ...
 */
enum zclient_send_status
zclient_route_send (uint8_t cmd, struct zclient *zclient,
                    struct zapi_route *api)
{
...
static int
zapi_nexthop_labels_cmp (const struct zapi_nexthop *next1,
...
static int
zapi_nexthop_srv6_cmp (const struct zapi_nexthop *next1,
                       const struct zapi_nexthop *next2)
{
  int ret = 0;

  ret =
      memcmp (&next1->seg6_segs, &next2->seg6_segs, sizeof (struct in6_addr));
  if (ret != 0)
    return ret;
...
static int
zapi_nexthop_cmp_no_labels (const struct zapi_nexthop *next1,
                            const struct zapi_nexthop *next2)
{
  int ret = 0;
  ...
  switch (next1->type)
    ...
    case NEXTHOP_TYPE_IPV6:
      ret = nexthop_g_addr_cmp (next1->type, &next1->gate, &next2->gate);
      if (ret != 0)
        return ret;
      break;
      ...
      if (next1->backup_num > 0 || next2->backup_num > 0)
        {
          ...
          ret =
              memcmp (next1->backup_idx, next2->backup_idx, next1->backup_num);
          if (ret != 0)
            return ret;
        }
    }
...
static int
zapi_nexthop_cmp (const void *item1, const void *item2)
{
  int ret = 0;
...
static void
zapi_nexthop_group_sort (struct zapi_nexthop *nh_grp, uint16_t nexthop_num)
{
  qsort (nh_grp, nexthop_num, sizeof (struct zapi_nexthop), &zapi_nexthop_cmp);
}
 ...
 */
int
zapi_nexthop_encode (struct stream *s, const struct zapi_nexthop *api_nh,
                     uint32_t api_flags, uint32_t api_message)
{
  ...
  if (CHECK_FLAG (nh_flags, ZAPI_NEXTHOP_FLAG_SEG6))
    stream_write (s, &api_nh->seg6_segs, sizeof (struct in6_addr));
...
int
zapi_srv6_locator_chunk_encode (struct stream *s,
...
int
zapi_srv6_locator_chunk_decode (struct stream *s, struct srv6_locator_chunk *c)
{
  uint16_t len = 0;
...
int
zapi_srv6_locator_encode (struct stream *s, const struct srv6_locator *l)
{
...
int
zapi_srv6_locator_decode (struct stream *s, struct srv6_locator *l)
{
  uint16_t len = 0;
...
static int
zapi_nhg_encode (struct stream *s, int cmd, struct zapi_nhg *api_nhg)
{
  int i;
  ...
  if (cmd != ZEBRA_NHG_DEL && cmd != ZEBRA_NHG_ADD)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                "%s: Specified zapi NHG command (%d) doesn't exist", __func__,
                cmd);
      return -1;
    }
  ...
  if (api_nhg->nexthop_num >= MULTIPATH_NUM ||
    ...
    {
      flog_err (EC_LIB_ZAPI_ENCODE, "%s: zapi NHG encode with invalid input",
                __func__);
      return -1;
    }
  ...
  if (cmd == ZEBRA_NHG_ADD)
    {
      /* Nexthops */
      zapi_nexthop_group_sort (api_nhg->nexthops, api_nhg->nexthop_num);
      ...
      for (i = 0; i < api_nhg->backup_nexthop_num; i++)
        zapi_nexthop_encode (s, &api_nhg->backup_nexthops[i], 0, 0);
    }
...
int
zapi_route_encode (uint8_t cmd, struct stream *s, struct zapi_route *api)
{
  ...
  if (api->type >= ZEBRA_ROUTE_MAX)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                "%s: Specified route type (%u) is not a legal value", __func__,
                api->type);
      return -1;
    }
  ...
  if (api->safi < SAFI_UNICAST || api->safi >= SAFI_MAX)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                "%s: Specified route SAFI (%u) is not a legal value", __func__,
                api->safi);
      return -1;
    }
      ...
      if (api->nexthop_num > MULTIPATH_NUM)
        {
          flog_err (
              ...
              __func__, &api->prefix, api->nexthop_num, MULTIPATH_NUM);
          return -1;
        }
          ...
          if (api_nh->label_num > MPLS_MAX_LABELS)
            {
              flog_err (
                  ...
                  __func__, &api->prefix, api_nh->label_num, MPLS_MAX_LABELS);
              return -1;
            }
      ...
      if (api->backup_nexthop_num > MULTIPATH_NUM)
        {
          flog_err (
              ...
              __func__, &api->prefix, api->backup_nexthop_num, MULTIPATH_NUM);
          return -1;
        }
 ...
 */
int
zapi_nexthop_decode (struct stream *s, struct zapi_nexthop *api_nh,
                     uint32_t api_flags, uint32_t api_message)
{
  int i, ret = -1;
  ...
  switch (api_nh->type)
    ...
    case NEXTHOP_TYPE_IPV4:
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      STREAM_GET (&api_nh->gate.ipv4.s_addr, s, IPV4_MAX_BYTELEN);
  ...
  if (CHECK_FLAG (api_nh->flags, ZAPI_NEXTHOP_FLAG_LABEL))
    {
      ...
      if (api_nh->label_num > MPLS_MAX_LABELS)
        {
          flog_err (EC_LIB_ZAPI_ENCODE,
                    "%s: invalid number of MPLS labels (%u)", __func__,
                    api_nh->label_num);
          return -1;
        }
  ...
  if (CHECK_FLAG (api_nh->flags, ZAPI_NEXTHOP_FLAG_SEG6))
    STREAM_GET (&api_nh->seg6_segs, s, sizeof (struct in6_addr));
...
int
zapi_route_decode (struct stream *s, struct zapi_route *api)
{
  struct zapi_nexthop *api_nh;
  int i;
  ...
  if (api->type >= ZEBRA_ROUTE_MAX)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                "%s: Specified route type: %d is not a legal value", __func__,
                api->type);
      return -1;
    }
  ...
  switch (api->prefix.family)
    ...
    case AF_INET6:
      ...
      break;
    default:
      flog_err (EC_LIB_ZAPI_ENCODE, "%s: Specified family %d is not v4 or v6",
  ...
  if (CHECK_FLAG (api->message, ZAPI_MESSAGE_SRCPFX))
    {
      ...
      if (api->prefix.family != AF_INET6 || api->src_prefix.prefixlen == 0)
        {
          flog_err (
              EC_LIB_ZAPI_ENCODE,
  ...
  if (CHECK_FLAG (api->message, ZAPI_MESSAGE_NEXTHOP))
    {
      ...
      if (api->nexthop_num > MULTIPATH_NUM)
        {
          flog_err (EC_LIB_ZAPI_ENCODE, "%s: invalid number of nexthops (%u)",
  ...
  if (CHECK_FLAG (api->message, ZAPI_MESSAGE_BACKUP_NEXTHOPS))
    {
      ...
      if (api->backup_nexthop_num > MULTIPATH_NUM)
        {
          flog_err (EC_LIB_ZAPI_ENCODE,
                    "%s: invalid number of backup nexthops (%u)", __func__,
...
static void
zapi_encode_prefix (struct stream *s, struct prefix *p, uint8_t family)
{
  struct prefix any;
...
int
zapi_pbr_rule_encode (uint8_t cmd, struct stream *s, struct pbr_rule *zrule)
{
  stream_reset (s);
  ...
  zapi_encode_prefix (s, &(zrule->filter.src_ip), zrule->filter.src_ip.family);
  stream_putw (s, zrule->filter.src_port); /* src port */
  zapi_encode_prefix (s, &(zrule->filter.dst_ip), zrule->filter.src_ip.family);
...
int
zapi_tc_qdisc_encode (uint8_t cmd, struct stream *s, struct tc_qdisc *qdisc)
{
  stream_reset (s);
  zclient_create_header (s, cmd, VRF_DEFAULT);
...
int
zapi_tc_class_encode (uint8_t cmd, struct stream *s, struct tc_class *class)
{
  stream_reset (s);
  zclient_create_header (s, cmd, VRF_DEFAULT);
...
int
zapi_tc_filter_encode (uint8_t cmd, struct stream *s, struct tc_filter *filter)
{
  stream_reset (s);
  zclient_create_header (s, cmd, VRF_DEFAULT);
...
bool
zapi_nhg_notify_decode (struct stream *s, uint32_t *id,
...
bool
zapi_route_notify_decode (struct stream *s, struct prefix *p,
                          uint32_t *tableid,
...
bool
zapi_rule_notify_decode (struct stream *s, uint32_t *seqno, uint32_t *priority,
                         uint32_t *unique, char *ifname,
...
bool
zapi_ipset_notify_decode (struct stream *s, uint32_t *unique,
...
bool
zapi_ipset_entry_notify_decode (struct stream *s, uint32_t *unique,
                                char *ipset_name,
...
bool
zapi_iptable_notify_decode (struct stream *s, uint32_t *unique,
...
nexthop_from_zapi_nexthop (const struct zapi_nexthop *znh)
{
  ...
  if (znh->label_num)
    {
      nexthop_add_labels (n, ZEBRA_LSP_NONE, znh->label_num, znh->labels);
    }
  ...
  if (znh->seg6local_action != ZEBRA_SEG6_LOCAL_ACTION_UNSPEC)
    nexthop_add_srv6_seg6local (n, znh->seg6local_action, &znh->seg6local_ctx);
 ...
 */
int
zapi_nexthop_from_nexthop (struct zapi_nexthop *znh, const struct nexthop *nh)
{
  int i;
 ...
 */
int
zapi_backup_nexthop_from_nexthop (struct zapi_nexthop *znh,
 ...
 */
const char *
zapi_nexthop2str (const struct zapi_nexthop *znh, char *buf, int bufsize)
{
...
bool
zapi_error_decode (struct stream *s, enum zebra_error_types *error)
{
  ...
  if (zclient_debug)
    zlog_debug ("%s: type: %s", __func__, zebra_error_type2str (*error));
...
enum zclient_send_status
zebra_redistribute_default_send (int command, struct zclient *zclient,
...
int
zebra_route_notify_send (int command, struct zclient *zclient, bool set)
{
  struct stream *s;
...
static int
zclient_vrf_add (ZAPI_CALLBACK_ARGS)
{
  struct vrf *vrf;
...
static int
zclient_vrf_delete (ZAPI_CALLBACK_ARGS)
{
  struct vrf *vrf;
...
static int
zclient_interface_add (ZAPI_CALLBACK_ARGS)
{
  struct interface *ifp;
  ...
  if (! vrf)
    {
      zlog_debug ("Rx'd interface add from Zebra, but VRF %u does not exist",
                  vrf_id);
      return -1;
    }
...
zebra_interface_state_read (struct stream *s, vrf_id_t vrf_id)
{
  ...
  if (ifp == NULL)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                "INTERFACE_STATE: Cannot find IF %s in VRF %d", ifname_tmp,
                vrf_id);
      return NULL;
    }
...
static int
zclient_interface_delete (ZAPI_CALLBACK_ARGS)
{
  struct interface *ifp;
  struct stream *s = zclient->ibuf;
...
static int
zclient_interface_up (ZAPI_CALLBACK_ARGS)
{
  struct interface *ifp;
  struct stream *s = zclient->ibuf;
...
static int
zclient_interface_down (ZAPI_CALLBACK_ARGS)
{
  struct interface *ifp;
  struct stream *s = zclient->ibuf;
...
static int
zclient_handle_error (ZAPI_CALLBACK_ARGS)
{
...
static int
link_params_set_value (struct stream *s, struct interface *ifp)
{
...
struct interface *
zebra_interface_link_params_read (struct stream *s, vrf_id_t vrf_id,
                                  bool *changed)
{
  struct if_link_params *iflp;
  ...
  if (ifp == NULL)
    {
      flog_err (EC_LIB_ZAPI_ENCODE, "%s: unknown ifindex %u, shouldn't happen",
                __func__, ifindex);
      return NULL;
    }
  ...
  if (ifp == NULL)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                ...
                (type == ZEBRA_INTERFACE_ADDRESS_ADD) ? "ADD" : "DEL", ifindex,
                vrf_id);
      return NULL;
    }
                                 ...
                                 vrf_id_t *new_vrf_id)
{
  ...
  if (ifp == NULL)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                "INTERFACE_VRF_UPDATE: Cannot find IF %s in VRF %d", ifname,
                vrf_id);
      return NULL;
    }
...
static int
zclient_read_sync_response (struct zclient *zclient, uint16_t expected_cmd)
{
  struct stream *s;
  uint16_t size = -1;
  uint8_t marker;
 ...
 */
int
lm_label_manager_connect (struct zclient *zclient, int async)
{
  int ret;
  struct stream *s;
  uint8_t result;
  uint16_t cmd =
      async ? ZEBRA_LABEL_MANAGER_CONNECT_ASYNC : ZEBRA_LABEL_MANAGER_CONNECT;
  ...
  if (proto != zclient->redist_default)
    flog_err (EC_LIB_ZAPI_ENCODE,
              "Wrong proto (%u) in LM connect response. Should be %u", proto,
 ...
 */
int
srv6_manager_get_locator_chunk (struct zclient *zclient,
 ...
 */
int
srv6_manager_release_locator_chunk (struct zclient *zclient,
                                    const char *locator_name)
{
  struct stream *s;
  const size_t len = strlen (locator_name);
 ...
 */
int
lm_get_label_chunk (struct zclient *zclient, uint8_t keep, uint32_t base,
                    uint32_t chunk_size, uint32_t *start, uint32_t *end)
{
  ...
  if (proto != zclient->redist_default)
    flog_err (EC_LIB_ZAPI_ENCODE,
              "Wrong proto (%u) in get chunk response. Should be %u", proto,
              zclient->redist_default);
  if (instance != zclient->instance)
    flog_err (EC_LIB_ZAPI_ENCODE,
              "Wrong instId (%u) in get chunk response Should be %u", instance,
              zclient->instance);
  ...
  if (*start > *end || *start < MPLS_LABEL_UNRESERVED_MIN ||
    ...
    {
      flog_err (EC_LIB_ZAPI_ENCODE, "Invalid Label chunk: %u - %u", *start,
                *end);
      return -1;
    }
 ...
 */
int
tm_table_manager_connect (struct zclient *zclient)
{
  int ret;
  struct stream *s;
  uint8_t result;
  ...
  /* read response */
  if (zclient_read_sync_response (zclient, ZEBRA_TABLE_MANAGER_CONNECT) != 0)
    return -1;
  ...
  STREAM_GETC (s, result);
  if (zclient_debug)
    zlog_debug ("%s: Table Manager connect response received, result %u",
                __func__, result);
 ...
 */
int
tm_get_table_chunk (struct zclient *zclient, uint32_t chunk_size,
                    uint32_t *start, uint32_t *end)
{
  ...
  if (ret == 0)
    {
      flog_err (EC_LIB_ZAPI_SOCKET, "%s: zclient->sock connection closed",
                __func__);
      close (zclient->sock);
      zclient->sock = -1;
      return -1;
  ...
  if (zclient_debug)
    zlog_debug ("%s: Table chunk request (%d bytes) sent", __func__, ret);
 ...
 */
int
tm_release_table_chunk (struct zclient *zclient, uint32_t start, uint32_t end)
{
  struct stream *s;
...
enum zclient_send_status
zebra_send_sr_policy (struct zclient *zclient, int cmd,
...
int
zapi_sr_policy_encode (struct stream *s, int cmd, struct zapi_sr_policy *zp)
{
  ...
  if (zt->label_num > MPLS_MAX_LABELS)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                ...
                __func__, zt->local_label, zt->label_num, MPLS_MAX_LABELS);
      return -1;
    }
...
int
zapi_sr_policy_decode (struct stream *s, struct zapi_sr_policy *zp)
{
...
int
zapi_sr_policy_notify_status_decode (struct stream *s,
                                     struct zapi_sr_policy *zp)
{
  memset (zp, 0, sizeof (*zp));
...
enum zclient_send_status
zebra_send_mpls_labels (struct zclient *zclient, int cmd,
...
int
zapi_labels_encode (struct stream *s, int cmd, struct zapi_labels *zl)
{
  struct zapi_nexthop *znh;
  ...
  if (zl->nexthop_num > MULTIPATH_NUM)
    {
      flog_err (EC_LIB_ZAPI_ENCODE,
                ...
                __func__, zl->local_label, zl->nexthop_num, MULTIPATH_NUM);
      return -1;
    }
...
int
zapi_labels_decode (struct stream *s, struct zapi_labels *zl)
{
  struct zapi_nexthop *znh;
  ...
  if (CHECK_FLAG (zl->message, ZAPI_LABELS_FTN))
    {
      ...
      switch (zl->route.prefix.family)
        ...
        case AF_INET:
          ...
          STREAM_GET (&zl->route.prefix.u.prefix4.s_addr, s, psize);
          break;
        case AF_INET6:
        ...
        default:
          flog_err (EC_LIB_ZAPI_ENCODE,
                    "%s: Specified family %u is not v4 or v6", __func__,
                    zl->route.prefix.family);
          return -1;
        }
  ...
  if (zl->nexthop_num > MULTIPATH_NUM)
    {
      flog_warn (
          ...
          __func__, &zl->route.prefix, zl->nexthop_num, MULTIPATH_NUM);
    }
      ...
      if (zl->backup_nexthop_num > MULTIPATH_NUM)
        {
          flog_warn (
              ...
              __func__, &zl->route.prefix, zl->backup_nexthop_num,
              MULTIPATH_NUM);
        }

      zl->backup_nexthop_num = MIN (MULTIPATH_NUM, zl->backup_nexthop_num);
 ...
 */
int
zebra_read_pw_status_update (ZAPI_CALLBACK_ARGS, struct zapi_pw_status *pw)
{
  struct stream *s;
...
static int
zclient_capability_decode (ZAPI_CALLBACK_ARGS)
{
  ...
  if (vrf_backend < 0 || vrf_configure_backend (vrf_backend))
    {
      flog_err (EC_LIB_ZAPI_ENCODE, "%s: Garbage VRF backend type: %d",
                __func__, vrf_backend);
      goto stream_failure;
    }
...
enum zclient_send_status
zclient_send_mlag_data (struct zclient *client, struct stream *client_s)
{
  struct stream *s;
 ...
 */
enum zclient_send_status
zclient_send_opaque (struct zclient *zclient, uint32_t type,
 ...
 */
int
zclient_opaque_decode (struct stream *s, struct zapi_opaque_msg *info)
{
 ...
 */
enum zclient_send_status
zclient_register_opaque (struct zclient *zclient, uint32_t type)
{
  struct stream *s;
 ...
 */
enum zclient_send_status
zclient_unregister_opaque (struct zclient *zclient, uint32_t type)
{
  struct stream *s;
...
int
zapi_opaque_reg_decode (struct stream *s, struct zapi_opaque_reg_info *info)
{
  STREAM_GETL (s, info->type);
  STREAM_GETC (s, info->proto);
  STREAM_GETW (s, info->instance);
...
int
zapi_client_close_notify_decode (struct stream *s,
...
static void
zclient_read (struct event *thread)
{
  size_t already;
  ...
  if (length < ZEBRA_HEADER_SIZE)
    {
      flog_err (EC_LIB_ZAPI_MISSMATCH,
                "%s: socket %d message length %u is less than %d ", __func__,
                zclient->sock, length, ZEBRA_HEADER_SIZE);
      zclient_failed (zclient);
      return;
    }
  ...
  if (length > STREAM_SIZE (zclient->ibuf))
    {
      ...
      flog_err (EC_LIB_ZAPI_ENCODE,
                ...
                __func__, length, (unsigned long) STREAM_SIZE (zclient->ibuf));
...
void
zclient_redistribute (int command, struct zclient *zclient, afi_t afi,
                      int type, unsigned short instance, vrf_id_t vrf_id)
{
...
void
zclient_redistribute_default (int command, struct zclient *zclient, afi_t afi,
                              vrf_id_t vrf_id)
{
...
static void
zclient_event (enum zclient_event event, struct zclient *zclient)
{
  switch (event)
    {
    ...
    case ZCLIENT_READ:
      zclient->t_read = NULL;
      event_add_read (zclient->master, zclient_read, zclient, zclient->sock,
                      &zclient->t_read);
      break;
    }
}
                              ...
                              struct interface *slave)
{
  ...
  zclient_create_header (s, ZEBRA_INTERFACE_SET_MASTER, master->vrf->vrf_id);
 ...
 */
int32_t
zapi_capabilities_decode (struct stream *s, struct zapi_cap *api)
{
 ...
 */
uint32_t
zclient_get_nhg_start (uint32_t proto)
{
...
char *
zclient_dump_route_flags (uint32_t flags, char *buf, size_t len)
{
  if (flags == 0)
    {
...
char *
zclient_evpn_dump_macip_flags (uint8_t flags, char *buf, size_t len)
{
  if (flags == 0)
    {
      snprintfrr (buf, len, "None ");
...
static int
zclient_neigh_ip_read_entry (struct stream *s, struct ipaddr *add)
{
  uint8_t family;
...
int
zclient_neigh_ip_encode (struct stream *s, uint16_t cmd, union sockunion *in,
  ...
  if (out && sockunion_family (out) != AF_UNSPEC)
    {
      ...
      stream_write (s, sockunion_get_addr (out), sockunion_get_addrlen (out));
    }
  else
...
int
zclient_neigh_ip_decode (struct stream *s, struct zapi_neigh_ip *api)
{
  int ret;
...
int
zclient_send_zebra_gre_request (struct zclient *client, struct interface *ifp)
{
  struct stream *s;
