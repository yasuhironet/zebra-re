...
static PyObject *
refuse_new (PyTypeObject *type, PyObject *args, PyObject *kwds)
{
...
static PyObject *elffile_secbyidx (struct elffile *w, Elf_Scn *scn,
                                   size_t idx);
...
static int
elfreloc_cmp (const struct elfreloc *a, const struct elfreloc *b)
{
...
static uint32_t
elfreloc_hash (const struct elfreloc *reloc)
{
...
elfrelocs_get (struct elfrelocs_head *head, GElf_Addr offset)
{
...
static PyObject *
elfreloc_getsection (PyObject *self, PyObject *args)
{
...
static PyObject *
elfreloc_getaddend (PyObject *obj, void *closure)
{
...
static PyObject *
elfreloc_repr (PyObject *arg)
{
  struct elfreloc *w = (struct elfreloc *) arg;

  return PyUnicode_FromFormat (
      "<ELFReloc @%lu %s+%lu>", (unsigned long) w->rela->r_offset,
      (w->symname && w->symname[0]) ? w->symname : "[0]",
      (unsigned long) w->rela->r_addend);
}

static void
elfreloc_free (void *arg)
{
...
static PyObject *
elfsect_getaddr (PyObject *self, void *closure)
{
...
static PyObject *
elfsect_getreloc (PyObject *self, PyObject *args)
{
  struct elfsect *w = (struct elfsect *) self;
...
static PyObject *
elfsect_subscript (PyObject *self, PyObject *key)
{
  ...
  if (! PySlice_Check (key))
    {
      PyErr_SetString (PyExc_IndexError, "ELFSection subscript must be slice");
      return NULL;
    }
  slice = (PySliceObject *) key;
  if (PyLong_Check (slice->stop))
    {
      if (PySlice_GetIndicesEx (key, w->shdr->sh_size, &start, &stop, &step,
                                &sllen))
        return NULL;
  ...
  if (offs > w->ef->len)
    {
      PyErr_Format (ELFAccessError, "access (%lu) beyond end of file (%lu)",
                    offs, w->ef->len);
      return NULL;
    }
  if (len == ~0UL)
...
static void
elfsect_add_relocations (struct elfsect *w, Elf_Scn *rel, GElf_Shdr *relhdr)
{
  size_t i, entries;
  ...
  for (i = 0; i < entries; i++)
    {
      ...
      relw =
          (struct elfreloc *) typeobj_elfreloc.tp_alloc (&typeobj_elfreloc, 0);
      relw->es = w;
      ...
      if (sym)
        {
          relw->symname =
              elf_strptr (w->ef->elf, symhdr->sh_link, sym->st_name);
...
static PyObject *
elfsect_wrap (struct elffile *ef, Elf_Scn *scn, size_t idx, const char *name)
{
  struct elfsect *w;
  size_t i;
...
static Elf_Scn *
elf_find_section (struct elffile *ef, const char *name, size_t *idx)
{
  size_t i;
  const char *secname;
...
static Elf_Scn *
elf_find_addr (struct elffile *ef, uint64_t addr, size_t *idx)
{
  size_t i;
...
static PyObject *
elffile_secbyidx (struct elffile *w, Elf_Scn *scn, size_t idx)
{
  const char *name;
  PyObject *ret;
...
static PyObject *
elffile_get_section (PyObject *self, PyObject *args)
{
  const char *name;
...
static PyObject *
elffile_get_section_addr (PyObject *self, PyObject *args)
{
...
static PyObject *
elffile_get_section_idx (PyObject *self, PyObject *args)
{
...
static PyObject *
elffile_get_symbol (PyObject *self, PyObject *args)
{
  ...
  for (i = 0; i < w->nsym; i++)
    {
      ...
      return Py_BuildValue ("sKN", symname, (unsigned long long) sym->st_value,
                            pysect);
    }
  Py_RETURN_NONE;
}
...
static PyObject *
elffile_getreloc (PyObject *self, PyObject *args)
{
  struct elffile *w = (struct elffile *) self;
  struct elfreloc *relw;
  unsigned long offs;
...
static PyObject *
elffile_find_note (PyObject *self, PyObject *args)
{
  ...
  for (i = 0; i < w->ehdr->e_phnum; i++)
    {
      ...
      notedata = elf_getdata_rawchunk (w->elf, phdr->p_offset, phdr->p_filesz,
                                       ELF_T_NHDR);
      ...
      offset = 0;
      while ((offset =
                  gelf_getnote (notedata, offset, nhdr, &nameoffs, &dataoffs)))
        {
          ...
          e = PyLong_FromUnsignedLongLong (phdr->p_vaddr + dataoffs +
                                           nhdr->n_descsz);
...
static bool
elffile_virt2file (struct elffile *w, GElf_Addr virt, GElf_Addr *offs)
{
  *offs = 0;
...
static PyObject *
elffile_subscript (PyObject *self, PyObject *key)
{
  ...
  if (! PySlice_Check (key))
    {
      PyErr_SetString (PyExc_IndexError, "ELFFile subscript must be slice");
      return NULL;
    }
  slice = (PySliceObject *) key;
  stop = -1;
  ...
  for (size_t i = 0; i < w->ehdr->e_phnum; i++)
    {
      ...
      if (xstart < phdr->p_vaddr || xstart >= phdr->p_vaddr + phdr->p_memsz)
        continue;
      if (! str &&
          (xstop < phdr->p_vaddr || xstop > phdr->p_vaddr + phdr->p_memsz))
        {
          PyErr_Format (ELFAccessError,
...
#ifdef HAVE_ELF_GETDATA_RAWCHUNK
static char *
elfdata_strptr (Elf_Data *data, size_t offset)
{
  char *p;
...
static void
elffile_add_dynreloc (struct elffile *w, Elf_Data *reldata, size_t entries,
                      Elf_Data *symdata, Elf_Data *strdata, Elf_Type typ)
{
  size_t i;
          ...
          if (elffile_virt2file (w, rel->r_offset, &offs))
            {
              ...
              ptr = elf_getdata_rawchunk (w->elf, offs, w->elfclass / 8,
                                          ELF_T_ADDR);
              ...
              if (ptr)
                {
                  ...
                  if (BYTE_ORDER == BIG_ENDIAN &&
                    ...
                    dst += sizeof (rel_offs) - ptr->d_size;
...
static PyObject *
elffile_load (PyTypeObject *type, PyObject *args, PyObject *kwds)
{
  const char *filename;
  ...
  for (size_t i = 0; i < w->ehdr->e_phnum; i++)
    {
      ...
      if (elffile_virt2file (w, symtab, &offset))
        symdata =
            elf_getdata_rawchunk (w->elf, offset, w->len - offset, ELF_T_SYM);
      if (elffile_virt2file (w, strtab, &offset))
        strdata = elf_getdata_rawchunk (w->elf, offset, strsz, ELF_T_BYTE);
      ...
      if (dynrela && dynrelasz && dynrelaent &&
          elffile_virt2file (w, dynrela, &offset))
        {
          ...
          reladata =
              elf_getdata_rawchunk (w->elf, offset, dynrelasz, ELF_T_RELA);

          c = dynrelasz / dynrelaent;
          elffile_add_dynreloc (w, reladata, c, symdata, strdata, ELF_T_RELA);
        }
      ...
      if (dynrel && dynrelsz && dynrelent &&
          elffile_virt2file (w, dynrel, &offset))
        {
          ...
          reldata = elf_getdata_rawchunk (w->elf, offset, dynrelsz, ELF_T_REL);

          c = dynrelsz / dynrelent;
          elffile_add_dynreloc (w, reldata, c, symdata, strdata, ELF_T_REL);
        }
    }
#endif
...
static PyObject *
elfpy_debug (PyObject *self, PyObject *args)
{
  int arg;
...
elf_py_init (PyObject *pymod)
{
  ...
  ELFFormatError =
      PyErr_NewException ("_clippy.ELFFormatError", PyExc_ValueError, NULL);
  PyModule_AddObject (pymod, "ELFFormatError", ELFFormatError);
  ELFAccessError =
      PyErr_NewException ("_clippy.ELFAccessError", PyExc_IndexError, NULL);
