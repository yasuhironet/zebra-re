...
extern "C"
{
   ...
   *   gotten from the stream (though, the user may reposition getp to earlier
   * in the stream, if they wish).
   *
...
#undef stream_write
   ...
   *
   * Each function has a safe variant, which ensures that the operation
   * performed is atomic with respect to the operations performed by all other
   * safe variants. In other words, the safe variants lock the stream_fifo's
   * mutex before performing their action. These are provided for convenience
   * when using stream_fifo in a multithreaded context, to alleviate the need
   * for the caller to implement their own synchronization around the
   * stream_fifo.
   *
  ...
  extern void stream_fifo_push_safe (struct stream_fifo *fifo,
                                     struct stream *s);
  ...
  ptr_get_be64 (const uint8_t *ptr, uint64_t *out)
  {
    uint32_t tmp1, tmp2;
  ...
  static inline const uint8_t *
  ptr_get_be32 (const uint8_t *ptr, uint32_t *out)
  {
    uint32_t tmp;
  ...
  ptr_get_be16 (uint8_t *ptr, uint16_t *out)
  {
    uint16_t tmp;
