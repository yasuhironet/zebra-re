...
#define MATCHER_ERROR(matcher_rv)                                             \
  ((matcher_rv) == MATCHER_INCOMPLETE || (matcher_rv) == MATCHER_NO_MATCH ||  \
   ...
   * variety of forms depending on the passed vline. If the last element in
   * vline is NULL, all previous elements are considered to be complete words
   * (the case when a space is the last token of the line) and completions are
   * generated based on what could follow that input. If the last element in
   * vline is not NULL and each sequential element matches the corresponding
   * tokens of one or more commands exactly (e.g. 'encapv4' and not 'en') the
   * same result is generated. If the last element is not NULL and the best
   * possible match is a partial match, then the result generated will be all
   * possible continuations of that element (e.g. 'encapv4', 'encapv6', etc for
   * input 'en').
   * @return matcher status
   */
