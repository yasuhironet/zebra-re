 ...
 */
int
ls_node_id_same (struct ls_node_id i1, struct ls_node_id i2)
{
  ...
  if (i1.origin == ISIS_L1 || i1.origin == ISIS_L2)
    {
      if (memcmp (i1.id.iso.sys_id, i2.id.iso.sys_id, ISO_SYS_ID_LEN) != 0 ||
...
struct ls_node *
ls_node_new (struct ls_node_id adv, struct in_addr rid, struct in6_addr rid6)
{
  struct ls_node *new;
  ...
  if (! IPV4_NET0 (rid.s_addr))
    ...
    {
      if (adv.origin == OSPFv2 || adv.origin == STATIC || adv.origin == DIRECT)
        {
...
void
ls_node_del (struct ls_node *node)
{
  if (! node)
    return;
...
int
ls_node_same (struct ls_node *n1, struct ls_node *n2)
{
  /* First, check pointer */
  if ((n1 && ! n2) || (! n1 && n2))
    return 0;
  ...
  if (CHECK_FLAG (n1->flags, LS_NODE_ROUTER_ID6) &&
    ...
    return 0;
  if (CHECK_FLAG (n1->flags, LS_NODE_FLAG) && (n1->node_flag != n2->node_flag))
    return 0;
  ...
  if (CHECK_FLAG (n1->flags, LS_NODE_SR))
    {
      if (n1->srgb.flag != n2->srgb.flag ||
        ...
        return 0;
      if ((n1->algo[0] != n2->algo[0]) || (n1->algo[1] != n2->algo[1]))
        return 0;
...
ls_attributes_new (struct ls_node_id adv, struct in_addr local,
                   struct in6_addr local6, uint32_t local_id)
{
...
void
ls_attributes_srlg_del (struct ls_attributes *attr)
{
  if (! attr)
    return;
...
void
ls_attributes_del (struct ls_attributes *attr)
{
  if (! attr)
    return;
...
int
ls_attributes_same (struct ls_attributes *l1, struct ls_attributes *l2)
{
  /* First, check pointer */
  if ((l1 && ! l2) || (! l1 && l2))
    return 0;
  ...
  if (CHECK_FLAG (l1->flags, LS_ATTR_REMOTE_ADDR) &&
      ! IPV4_ADDR_SAME (&l1->standard.remote_addr, &l2->standard.remote_addr))
    return 0;
  ...
  for (int i = 0; i < LS_ADJ_MAX; i++)
    {
      ...
      if (((l1->adv.origin == ISIS_L1) || (l1->adv.origin == ISIS_L2)) &&
          (memcmp (&l1->adj_sid[i].neighbor.sysid,
                   &l2->adj_sid[i].neighbor.sysid, ISO_SYS_ID_LEN) != 0))
        return 0;
  ...
  if (CHECK_FLAG (l1->flags, LS_ATTR_SRLG) &&
      ((l1->srlg_len != l2->srlg_len) ||
       memcmp (l1->srlgs, l2->srlgs, l1->srlg_len * sizeof (uint32_t)) != 0))
    return 0;
 ...
 */
struct ls_prefix *
ls_prefix_new (struct ls_node_id adv, struct prefix *p)
{
...
void
ls_prefix_del (struct ls_prefix *pref)
{
  if (! pref)
    return;
...
int
ls_prefix_same (struct ls_prefix *p1, struct ls_prefix *p2)
{
  /* First, check pointer */
  if ((p1 && ! p2) || (! p1 && p2))
    return 0;
 ...
 */
uint64_t
sysid_to_key (const uint8_t sysid[ISO_SYS_ID_LEN])
{
  uint64_t key = 0;
...
struct ls_vertex *
ls_vertex_add (struct ls_ted *ted, struct ls_node *node)
{
  ...
  switch (node->adv.origin)
    ...
    case STATIC:
    case DIRECT:
      key = ((uint64_t) ntohl (node->adv.id.ip.addr.s_addr)) & 0xffffffff;
      break;
    case ISIS_L1:
    case ISIS_L2:
...
void
ls_vertex_del (struct ls_ted *ted, struct ls_vertex *vertex)
{
...
void
ls_vertex_del_all (struct ls_ted *ted, struct ls_vertex *vertex)
{
  if (! ted || ! vertex)
    return;
...
struct ls_vertex *
ls_vertex_update (struct ls_ted *ted, struct ls_node *node)
{
...
struct ls_vertex *
ls_find_vertex_by_key (struct ls_ted *ted, const uint64_t key)
{
...
struct ls_vertex *
ls_find_vertex_by_id (struct ls_ted *ted, struct ls_node_id nid)
{
  struct ls_vertex vertex = {};
  ...
  switch (nid.origin)
    ...
    case STATIC:
    case DIRECT:
      vertex.key = ((uint64_t) ntohl (nid.id.ip.addr.s_addr)) & 0xffffffff;
      break;
    case ISIS_L1:
    case ISIS_L2:
...
int
ls_vertex_same (struct ls_vertex *v1, struct ls_vertex *v2)
{
  if ((v1 && ! v2) || (! v1 && v2))
    return 0;
...
void
ls_vertex_clean (struct ls_ted *ted, struct ls_vertex *vertex,
 ...
 */
static void
ls_edge_connect_to (struct ls_ted *ted, struct ls_edge *edge)
{
  ...
  if (vertex == NULL)
    {
      ...
      node = ls_node_new (edge->attributes->adv, inaddr_any, in6addr_any);
...
static struct ls_edge_key
get_edge_key (struct ls_attributes *attr, bool dst)
{
  ...
  if (dst)
    {
      if (CHECK_FLAG (attr->flags, LS_ATTR_NEIGH_ADDR))
      ...
      else if (CHECK_FLAG (attr->flags, LS_ATTR_NEIGH_ID))
        {
          ...
          key.family = AF_LOCAL;
          key.k.link_id = (((uint64_t) std->remote_id) & 0xffffffff) |
...
struct ls_edge *
ls_edge_add (struct ls_ted *ted, struct ls_attributes *attributes)
{
  struct ls_edge *new;
...
struct ls_edge *
ls_find_edge_by_key (struct ls_ted *ted, const struct ls_edge_key key)
{
  struct ls_edge edge = {};
...
struct ls_edge *
ls_edge_update (struct ls_ted *ted, struct ls_attributes *attributes)
{
  struct ls_edge *old;
...
int
ls_edge_same (struct ls_edge *e1, struct ls_edge *e2)
{
  if ((e1 && ! e2) || (! e1 && e2))
    return 0;
...
void
ls_edge_del (struct ls_ted *ted, struct ls_edge *edge)
{
  if (! ted || ! edge)
    return;
...
void
ls_edge_del_all (struct ls_ted *ted, struct ls_edge *edge)
{
  if (! ted || ! edge)
    return;
 ...
 */
struct ls_subnet *
ls_subnet_add (struct ls_ted *ted, struct ls_prefix *ls_pref)
{
...
struct ls_subnet *
ls_subnet_update (struct ls_ted *ted, struct ls_prefix *pref)
{
...
int
ls_subnet_same (struct ls_subnet *s1, struct ls_subnet *s2)
{
  if ((s1 && ! s2) || (! s1 && s2))
    return 0;
...
void
ls_subnet_del (struct ls_ted *ted, struct ls_subnet *subnet)
{
  if (! ted || ! subnet)
    return;
...
void
ls_subnet_del_all (struct ls_ted *ted, struct ls_subnet *subnet)
{
  if (! ted || ! subnet)
    return;
...
struct ls_subnet *
ls_find_subnet (struct ls_ted *ted, const struct prefix *prefix)
{
  struct ls_subnet subnet = {};
 ...
 */
struct ls_ted *
ls_ted_new (const uint32_t key, const char *name, uint32_t as_number)
{
  struct ls_ted *new;
...
void
ls_ted_del_all (struct ls_ted **ted)
{
  struct ls_vertex *vertex;
  struct ls_edge *edge;
  struct ls_subnet *subnet;
...
void
ls_ted_clean (struct ls_ted *ted)
{
  struct ls_vertex *vertex;
  struct ls_edge *edge;
  struct ls_subnet *subnet;
...
void
ls_connect (struct ls_vertex *vertex, struct ls_edge *edge, bool source)
{
...
void
ls_disconnect (struct ls_vertex *vertex, struct ls_edge *edge, bool source)
{
...
void
ls_connect_vertices (struct ls_vertex *src, struct ls_vertex *dst,
                     struct ls_edge *edge)
{
  if (edge == NULL)
    return;
...
void
ls_disconnect_edge (struct ls_edge *edge)
{
  if (edge == NULL)
    return;
...
int
ls_register (struct zclient *zclient, bool server)
{
  int rc;
...
int
ls_unregister (struct zclient *zclient, bool server)
{
  int rc;
...
int
ls_request_sync (struct zclient *zclient)
{
  struct stream *s;
  uint16_t flags = 0;
...
static struct ls_node *
ls_parse_node (struct stream *s)
{
  struct ls_node *node;
  size_t len;
...
static struct ls_attributes *
ls_parse_attributes (struct stream *s)
{
      ...
      for (size_t i = 0; i < nb_ext_adm_grp; i++)
        {
          ...
          admin_group_bulk_set (&attr->ext_admin_group, bitmap_data, i);
        }
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_LOCAL_ADDR))
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_ADJ_SID))
    {
      ...
      attr->adj_sid[ADJ_PRI_IPV4].neighbor.addr.s_addr = stream_get_ipv4 (s);
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_BCK_ADJ_SID))
    {
      ...
      attr->adj_sid[ADJ_BCK_IPV4].neighbor.addr.s_addr = stream_get_ipv4 (s);
    }
...
stream_failure:
  zlog_err ("LS(%s): Could not parse Link State Attributes. Abort!", __func__);
...
static struct ls_prefix *
ls_parse_prefix (struct stream *s)
{
...
static int
ls_format_node (struct stream *s, struct ls_node *node)
{
  size_t len;
...
static int
ls_format_attributes (struct stream *s, struct ls_attributes *attr)
{
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_EXT_ADM_GRP))
    {
      ...
      for (size_t i = 0; i < nb_ext_adm_grp; i++)
        stream_putl (s, admin_group_get_offset (&attr->ext_admin_group, i));
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_LOCAL_ADDR))
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_ADJ_SID))
    {
      ...
      stream_put_ipv4 (s, attr->adj_sid[ADJ_PRI_IPV4].neighbor.addr.s_addr);
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_BCK_ADJ_SID))
    {
      ...
      stream_put_ipv4 (s, attr->adj_sid[ADJ_BCK_IPV4].neighbor.addr.s_addr);
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_ADJ_SID6))
    {
...
static int
ls_format_prefix (struct stream *s, struct ls_prefix *ls_pref)
{
  size_t len;
...
static int
ls_format_msg (struct stream *s, struct ls_message *msg)
{
...
int
ls_send_msg (struct zclient *zclient, struct ls_message *msg,
...
struct ls_message *
ls_vertex2msg (struct ls_message *msg, struct ls_vertex *vertex)
{
...
struct ls_message *
ls_edge2msg (struct ls_message *msg, struct ls_edge *edge)
{
  /* Allocate space if needed */
  if (msg == NULL)
    msg = XCALLOC (MTYPE_LS_DB, sizeof (struct ls_message));
...
struct ls_message *
ls_subnet2msg (struct ls_message *msg, struct ls_subnet *subnet)
{
  /* Allocate space if needed */
  if (msg == NULL)
    msg = XCALLOC (MTYPE_LS_DB, sizeof (struct ls_message));
...
struct ls_vertex *
ls_msg2vertex (struct ls_ted *ted, struct ls_message *msg, bool delete)
{
...
struct ls_edge *
ls_msg2edge (struct ls_ted *ted, struct ls_message *msg, bool delete)
{
...
struct ls_subnet *
ls_msg2subnet (struct ls_ted *ted, struct ls_message *msg, bool delete)
{
...
struct ls_element *
ls_msg2ted (struct ls_ted *ted, struct ls_message *msg, bool delete)
{
...
struct ls_element *
ls_stream2ted (struct ls_ted *ted, struct stream *s, bool delete)
{
  struct ls_message *msg;
  struct ls_element *lse = NULL;
...
void
ls_delete_msg (struct ls_message *msg)
{
  if (msg == NULL)
    return;
...
int
ls_sync_ted (struct ls_ted *ted, struct zclient *zclient,
             struct zapi_opaque_reg_info *dst)
{
  struct ls_vertex *vertex;
  struct ls_edge *edge;
...
static const char *const type2txt[] = { "Unknown", "Standard",    "ABR",
                                        "ASBR",    "Remote ASBR", "Pseudo" };

static const char *const status2txt[] = { "Unknown", "New",  "Update",
                                          "Delete",  "Sync", "Orphan" };

static const char *
ls_node_id_to_text (struct ls_node_id lnid, char *str, size_t size)
{
...
static void
ls_show_vertex_vty (struct ls_vertex *vertex, struct vty *vty, bool verbose)
{
  ...
  if (! verbose)
    {
      sbuf_push (
          &sbuf, 0, "\t%d Outgoing Edges, %d Incoming Edges, %d Subnets\n",
  ...
  if (CHECK_FLAG (lsn->flags, LS_NODE_SR))
    {
      ...
      sbuf_push (&sbuf, 8, "SRGB: [%d/%d]", lsn->srgb.lower_bound, upper);
      if (CHECK_FLAG (lsn->flags, LS_NODE_SRLB))
        {
          upper = lsn->srlb.lower_bound + lsn->srlb.range_size - 1;
          sbuf_push (&sbuf, 0, "\tSRLB: [%d/%d]", lsn->srlb.lower_bound,
                     upper);
        }
      ...
      for (int i = 0; i < 2; i++)
        {
          ...
          sbuf_push (&sbuf, 0, lsn->algo[i] == 0 ? "SPF " : "S-SPF ");
        }
  ...
  for (ALL_LIST_ELEMENTS_RO (vertex->outgoing_edges, node, edge))
    {
      if (edge->destination)
        {
          ...
          sbuf_push (&sbuf, 6, "To:\t%s(%pI4)", lsn->name, &lsn->router_id);
        }
      else
        {
      ...
      if ((CHECK_FLAG (attr->flags, LS_ATTR_LOCAL_ADDR)))
        sbuf_push (&sbuf, 0, "\tLocal:  %pI4\tRemote: %pI4\n",
                   &attr->standard.local, &attr->standard.remote);
      else if ((CHECK_FLAG (attr->flags, LS_ATTR_LOCAL_ADDR6)))
        sbuf_push (&sbuf, 0, "\tLocal:  %pI6\tRemote: %pI6\n",
                   &attr->standard.local6, &attr->standard.remote6);
    }
      ...
      if (edge->source)
        {
          ...
          sbuf_push (&sbuf, 6, "From:\t%s(%pI4)", lsn->name, &lsn->router_id);
        }
      else
        {
...
static void
ls_show_vertex_json (struct ls_vertex *vertex, struct json_object *json)
{
  struct ls_node *lsn;
  ...
  if (CHECK_FLAG (lsn->flags, LS_NODE_TYPE))
    json_object_string_add (json, "vertex-type", type2txt[lsn->type]);
  if (CHECK_FLAG (lsn->flags, LS_NODE_AS_NUMBER))
  ...
  if (CHECK_FLAG (lsn->flags, LS_NODE_SR))
    {
      ...
      if (CHECK_FLAG (lsn->flags, LS_NODE_SRLB))
        {
          json_object_int_add (jsr, "srlb-size", lsn->srlb.range_size);
          json_object_int_add (jsr, "srlb-lower", lsn->srlb.lower_bound);
        }
      if (CHECK_FLAG (lsn->flags, LS_NODE_MSD))
...
void
ls_show_vertex (struct ls_vertex *vertex, struct vty *vty,
...
void
ls_show_vertices (struct ls_ted *ted, struct vty *vty,
                  struct json_object *json, bool verbose)
{
  struct ls_vertex *vertex;
...
static const char *
edge_key_to_text (struct ls_edge_key key)
{
...
static void
ls_show_edge_vty (struct ls_edge *edge, struct vty *vty, bool verbose)
{
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_EXT_ADM_GRP) &&
      admin_group_nb_words (&attr->ext_admin_group) != 0)
    {
      ...
      if (admin_group_buf[0] != '\0' && (sbuf.pos + strlen (admin_group_buf) +
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_MAX_RSV_BW))
    sbuf_push (&sbuf, 4, "Maximum Reservable Bandwidth: %g (Bytes/s)\n",
               attr->standard.max_rsv_bw);
  if (CHECK_FLAG (attr->flags, LS_ATTR_UNRSV_BW))
    {
      sbuf_push (&sbuf, 4, "Unreserved Bandwidth per Class Type\n");
      for (int i = 0; i < MAX_CLASS_TYPE; i += 2)
        sbuf_push (&sbuf, 8, "[%d]: %g (Bytes/sec)\t[%d]: %g (Bytes/s)\n", i,
                   attr->standard.unrsv_bw[i], i + 1,
...
static void
ls_show_edge_json (struct ls_edge *edge, struct json_object *json)
{
  struct ls_attributes *attr;
  struct json_object *jte, *jbw, *jobj,
      *jsr = NULL, *jsrlg, *js_ext_ag, *js_ext_ag_arr_word, *js_ext_ag_arr_bit;
  char buf[INET6_BUFSIZ];
  ...
  if (edge->destination)
    json_object_int_add (json, "remote-vertex-id", edge->destination->key);
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_ADM_GRP))
    json_object_int_add (jte, "admin-group", attr->standard.admin_group);
  if (CHECK_FLAG (attr->flags, LS_ATTR_EXT_ADM_GRP))
    {
      ...
      json_object_object_add (js_ext_ag, "bitPositions", js_ext_ag_arr_bit);
      for (i = 0; i < admin_group_nb_words (&attr->ext_admin_group); i++)
        {
          bitmap = admin_group_get_offset (&attr->ext_admin_group, i);
          snprintf (buf_ag, sizeof (buf_ag), "0x%08x", bitmap);
          json_object_array_add (js_ext_ag_arr_word,
                                 json_object_new_string (buf_ag));
        }
      for (i = 0; i < (admin_group_size (&attr->ext_admin_group) * WORD_SIZE);
           i++)
        {
          if (admin_group_get (&attr->ext_admin_group, i))
            json_object_array_add (js_ext_ag_arr_bit, json_object_new_int (i));
        }
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_LOCAL_ADDR))
    {
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_LOCAL_ID))
    json_object_int_add (jte, "local-identifier", attr->standard.local_id);
  if (CHECK_FLAG (attr->flags, LS_ATTR_NEIGH_ID))
    json_object_int_add (jte, "remote-identifier", attr->standard.remote_id);
  if (CHECK_FLAG (attr->flags, LS_ATTR_MAX_BW))
    json_object_double_add (jte, "max-link-bandwidth", attr->standard.max_bw);
  if (CHECK_FLAG (attr->flags, LS_ATTR_MAX_RSV_BW))
      ...
      for (int i = 0; i < MAX_CLASS_TYPE; i++)
        {
          ...
          json_object_double_add (jobj, buf, attr->standard.unrsv_bw[i]);
          json_object_array_add (jbw, jobj);
        }
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_REMOTE_AS))
    json_object_int_add (jte, "remote-asn", attr->standard.remote_as);
  if (CHECK_FLAG (attr->flags, LS_ATTR_REMOTE_ADDR))
    {
      snprintfrr (buf, INET6_BUFSIZ, "%pI4", &attr->standard.remote_addr);
      json_object_string_add (jte, "remote-as-address", buf);
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_REMOTE_ADDR6))
    {
      snprintfrr (buf, INET6_BUFSIZ, "%pI6", &attr->standard.remote_addr6);
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_PACKET_LOSS))
    json_object_double_add (jte, "loss",
                            attr->extended.pkt_loss * LOSS_PRECISION);
  if (CHECK_FLAG (attr->flags, LS_ATTR_AVA_BW))
    json_object_double_add (jte, "available-bandwidth", attr->extended.ava_bw);
  if (CHECK_FLAG (attr->flags, LS_ATTR_RSV_BW))
    json_object_double_add (jte, "residual-bandwidth", attr->extended.rsv_bw);
  if (CHECK_FLAG (attr->flags, LS_ATTR_USE_BW))
    json_object_double_add (jte, "utilized-bandwidth", attr->extended.used_bw);
  if (CHECK_FLAG (attr->flags, LS_ATTR_SRLG))
    {
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_ADJ_SID))
    {
      ...
      json_object_object_add (json, "segment-routing", jsr);
      jobj = json_object_new_object ();
      json_object_int_add (jobj, "adj-sid", attr->adj_sid[ADJ_PRI_IPV4].sid);
      snprintfrr (buf, 6, "0x%x", attr->adj_sid[ADJ_PRI_IPV4].flags);
      json_object_string_add (jobj, "flags", buf);
      json_object_int_add (jobj, "weight", attr->adj_sid[ADJ_PRI_IPV4].weight);
  ...
  if (CHECK_FLAG (attr->flags, LS_ATTR_BCK_ADJ_SID))
    {
      ...
      jobj = json_object_new_object ();
      json_object_int_add (jobj, "adj-sid", attr->adj_sid[ADJ_BCK_IPV4].sid);
      snprintfrr (buf, 6, "0x%x", attr->adj_sid[ADJ_BCK_IPV4].flags);
      json_object_string_add (jobj, "flags", buf);
      json_object_int_add (jobj, "weight", attr->adj_sid[ADJ_BCK_IPV4].weight);
      json_object_array_add (jsr, jobj);
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_ADJ_SID6))
    {
      ...
      json_object_object_add (json, "segment-routing", jsr);
      jobj = json_object_new_object ();
      json_object_int_add (jobj, "adj-sid", attr->adj_sid[ADJ_PRI_IPV6].sid);
      snprintfrr (buf, 6, "0x%x", attr->adj_sid[ADJ_PRI_IPV6].flags);
      json_object_string_add (jobj, "flags", buf);
      json_object_int_add (jobj, "weight", attr->adj_sid[ADJ_PRI_IPV6].weight);
      json_object_array_add (jsr, jobj);
    }
  if (CHECK_FLAG (attr->flags, LS_ATTR_BCK_ADJ_SID6))
    {
      ...
      jobj = json_object_new_object ();
      json_object_int_add (jobj, "adj-sid", attr->adj_sid[ADJ_BCK_IPV6].sid);
      snprintfrr (buf, 6, "0x%x", attr->adj_sid[ADJ_BCK_IPV6].flags);
      json_object_string_add (jobj, "flags", buf);
      json_object_int_add (jobj, "weight", attr->adj_sid[ADJ_BCK_IPV6].weight);
      json_object_array_add (jsr, jobj);
    }
}

void
ls_show_edge (struct ls_edge *edge, struct vty *vty, struct json_object *json,
              bool verbose)
{
  /* Sanity Check */
  if (! edge)
...
void
ls_show_edges (struct ls_ted *ted, struct vty *vty, struct json_object *json,
               bool verbose)
{
  struct ls_edge *edge;
...
static void
ls_show_subnet_vty (struct ls_subnet *subnet, struct vty *vty, bool verbose)
{
...
static void
ls_show_subnet_json (struct ls_subnet *subnet, struct json_object *json)
{
  struct ls_prefix *pref;
  json_object *jsr;
  char buf[INET6_BUFSIZ];
...
void
ls_show_subnet (struct ls_subnet *subnet, struct vty *vty,
                struct json_object *json, bool verbose)
{
  /* Sanity Check */
  if (! subnet)
...
void
ls_show_subnets (struct ls_ted *ted, struct vty *vty, struct json_object *json,
                 bool verbose)
{
  struct ls_subnet *subnet;
...
void
ls_show_ted (struct ls_ted *ted, struct vty *vty, struct json_object *json,
             bool verbose)
{
  json_object *jted;
  ...
  if (json)
    {
      ...
      json_object_int_add (jted, "edgesCount", edges_count (&ted->edges));
  ...
  if (vty)
    {
      ...
      vty_out (vty, "\n\tTotal: %zu Vertices, %zu Edges, %zu Subnets\n\n",
               vertices_count (&ted->vertices), edges_count (&ted->edges),
               subnets_count (&ted->subnets));
    }
}

void
ls_dump_ted (struct ls_ted *ted)
{
  struct ls_vertex *vertex;
  struct ls_edge *edge;
  struct ls_subnet *subnet;
  ...
  frr_each (vertices, &ted->vertices, vertex)
    {
      zlog_debug ("    Ted node (%s %pI4 %s)",
                  vertex->node->name[0] ? vertex->node->name : "no name node",
  ...
  frr_each (edges, &ted->edges, edge)
    {
      zlog_debug ("    Ted edge key:%s src:%pI4 dst:%pI4",
                  ...
                  edge->source ? &edge->source->node->router_id : &inaddr_any,
