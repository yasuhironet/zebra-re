...
static void
work_queue_item_remove (struct work_queue *wq, struct work_queue_item *item)
{
...
struct work_queue *
work_queue_new (struct event_loop *m, const char *queue_name)
{
...
void
work_queue_free_and_null (struct work_queue **wqp)
{
...
work_queue_schedule (struct work_queue *wq, unsigned int delay)
{
  ...
  if (CHECK_FLAG (wq->flags, WQ_UNPLUGGED) &&
      ! event_is_scheduled (wq->thread) && ! work_queue_empty (wq))
    {
      ...
      if (delay > 0)
        {
          event_add_timer_msec (wq->master, work_queue_run, wq, delay,
                                &wq->thread);
          event_ignore_late_timer (wq->thread);
        }
      else
        event_add_event (wq->master, work_queue_run, wq, 0, &wq->thread);
...
static void
work_queue_item_requeue (struct work_queue *wq, struct work_queue_item *item)
{
  work_queue_item_dequeue (wq, item);
       ...
       "Work Queue information\n")
{
  ...
  vty_out (vty, "%c %8s %5s %8s %8s %7s %6s %8s %6s %s\n", 'P', "Items",
           "Hold", "Total", "Total", "Best", "Gran.", "Total", "Avg.", "Name");

  for (ALL_LIST_ELEMENTS_RO (work_queues, node, wq))
    {
      vty_out (vty, "%c %8d %5d %8ld %8ld %7d %6d %8ld %6u %s\n",
               (CHECK_FLAG (wq->flags, WQ_UNPLUGGED) ? ' ' : 'P'),
               work_queue_item_count (wq), wq->spec.hold, wq->runs, wq->yields,
 ...
 */
void
work_queue_run (struct event *thread)
{
  struct work_queue *wq;
  ...
  STAILQ_FOREACH_SAFE (item, &wq->items, wq, titem)
    {
      ...
      if (! (cycles % wq->cycles.granularity) && event_should_yield (thread))
        {
          yielded = 1;
          goto stats;
        }
