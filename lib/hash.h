...
#define hashcount(X) ((X)->count)
   ...
   * Create a hash table.
   *
   * The created hash table uses chaining and a user-provided comparator
   * function to resolve collisions. For best performance use a perfect hash
   * function. Worst case lookup time is O(N) when using a constant hash
   * function. Best case lookup time is O(1) when using a perfect hash
   * function.
   *
   * The initial size of the created hash table is HASH_INITIAL_SIZE.
   *
   * hash_key
   *    hash function to use; should return a unique unsigned integer when
   * called with a data item. Collisions are acceptable.
   *
   * hash_cmp
   ...
   *
   * This function is somewhat counterintuitive in its usage. In order to look
   * up an element from its key, you must provide the data item itself, with
   * the portions used in the hash function set to the same values as the data
   * item to retrieve. To insert a data element, either provide the key as just
   * described and provide alloc_func as described below to allocate the full
   * data element, or provide the full data element and pass
   * 'hash_alloc_intern' to alloc_func.
   *
   * hash
   ...
   *
   * This function is equivalent to calling hash_get with alloc_func set to
   * NULL.
   *
   * hash
   *    hash table to operate on
   ...
   *
   * arg
   *    arbitrary argument passed as the second parameter in each call to
   * 'func'
   */
  ...
  extern void hash_walk (struct hash *hash,
                         int (*func) (struct hash_bucket *, void *),
                         void *arg);
   ...
   *    function to call with each removed item, intended to free the data
   */
  extern void hash_clean_and_free (struct hash **hash,
                                   void (*free_func) (void *));
