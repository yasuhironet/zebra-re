...
int
readn (int fd, uint8_t *ptr, int nbytes)
{
  int nleft;
  int nread;
...
int
writen (int fd, const uint8_t *ptr, int nbytes)
{
  int nleft;
  int nwritten;
...
set_nonblocking (int fd)
{
  ...
  if (flags < 0)
    {
      flog_err (EC_LIB_SYSTEM_CALL, "fcntl(F_GETFL) failed for fd %d: %s", fd,
                safe_strerror (errno));
      return -1;
    }
