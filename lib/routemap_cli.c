                 ...
                 ROUTE_MAP_SEQUENCE_CMD_STR)
{
  ...
  snprintf (xpath, sizeof (xpath), "/frr-route-map:lib/route-map[name='%s']",
            name);
...
int
route_map_instance_cmp (const struct lyd_node *dnode1,
...
void
route_map_instance_show (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
...
void
route_map_instance_show_end (struct vty *vty, const struct lyd_node *dnode)
{
  vty_out (vty, "exit\n");
  vty_out (vty, "!\n");
}

DEFPY_YANG (match_interface,
            match_interface_cmd,
...
DEFPY_YANG (no_match_ip_address,
...
DEFPY_YANG (match_ip_address_prefix_list,
...
DEFPY_YANG (no_match_ip_address_prefix_list,
...
DEFPY_YANG (no_match_ip_next_hop,
...
DEFPY_YANG (match_ip_next_hop_prefix_list,
...
DEFPY_YANG (match_ip_next_hop_type,
...
DEFPY_YANG (no_match_ip_next_hop_type,
...
DEFPY_YANG (no_match_ipv6_address,
...
DEFPY_YANG (match_ipv6_address_prefix_list,
...
DEFPY_YANG (match_ipv6_next_hop_type,
...
DEFPY_YANG (no_match_ipv6_next_hop_type,
            ...
            "Tag value\n")
{
  ...
  nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
  snprintf (xpath_value, sizeof (xpath_value), "%s/rmap-match-condition/tag",
            xpath);
...
void
route_map_condition_show (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
  ...
  if (IS_MATCH_INTERFACE (condition))
  ...
  else if (IS_MATCH_IPv4_NEXTHOP_TYPE (condition))
    {
      vty_out (vty, " match ip next-hop type %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-match-condition/ipv4-next-hop-type"));
    }
  else if (IS_MATCH_IPv6_NEXTHOP_TYPE (condition))
    {
      vty_out (vty, " match ipv6 next-hop type %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-match-condition/ipv6-next-hop-type"));
    }
    ...
    {
      vty_out (
          vty, " match ip address prefix-len %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " match ipv6 address prefix-len %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " match ip next-hop prefix-len %s\n",
          yang_dnode_get_string (
              dnode,
              "./rmap-match-condition/frr-zebra-route-map:ipv4-prefix-length"));
    }
  ...
  else if (IS_MATCH_ALIAS (condition))
    {
      vty_out (vty, " match alias %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-match-condition/frr-bgp-route-map:alias"));
    }
  else if (IS_MATCH_SCRIPT (condition))
    {
      vty_out (vty, " match script %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-match-condition/frr-bgp-route-map:script"));
    }
  else if (IS_MATCH_ORIGIN (condition))
    {
      vty_out (vty, " match origin %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-match-condition/frr-bgp-route-map:origin"));
    }
  else if (IS_MATCH_RPKI (condition))
    {
      vty_out (vty, " match rpki %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-match-condition/frr-bgp-route-map:rpki"));
    }
  else if (IS_MATCH_RPKI_EXTCOMMUNITY (condition))
    {
      vty_out (
          vty, " match rpki-extcommunity %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " match probability %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-match-condition/frr-bgp-route-map:probability"));
    }
  else if (IS_MATCH_SRC_VRF (condition))
    {
      vty_out (
          vty, " match source-vrf %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-match-condition/frr-bgp-route-map:source-vrf"));
    }
  ...
  else if (IS_MATCH_AS_LIST (condition))
    {
      vty_out (
          vty, " match as-path %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-match-condition/frr-bgp-route-map:list-name"));
    }
  else if (IS_MATCH_EVPN_ROUTE_TYPE (condition))
    {
      vty_out (
          vty, " match evpn route-type %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " match evpn vni %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-match-condition/frr-bgp-route-map:evpn-vni"));
    }
  else if (IS_MATCH_EVPN_DEFAULT_ROUTE (condition))
    {
      vty_out (
          vty, " match evpn default-route %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " match ip route-source prefix-list %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-match-condition/frr-bgp-route-map:list-name"));
    }
    ...
    {
      vty_out (
          vty, " match ip next-hop address %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-match-condition/frr-bgp-route-map:ipv4-address"));
    }
  else if (IS_MATCH_IPV6_NH (condition))
    {
      vty_out (
          vty, " match ipv6 next-hop address %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-match-condition/frr-bgp-route-map:ipv6-address"));
    }
}

DEFPY_YANG (set_ip_nexthop,
            set_ip_nexthop_cmd,
...
DEFPY_YANG (set_ipv6_nexthop_local,
...
DEFPY_YANG (no_set_ipv6_nexthop_local,
            ...
            "Subtract round trip time\n")
{
  ...
  if (rtt)
    ...
    {
      snprintf (xpath_value, sizeof (xpath_value), "%s/rmap-set-action/value",
                xpath);
      snprintf (value, sizeof (value), "%s", metric_str);
    }
            ...
            "Minimum metric value\n")
{
  ...
  snprintf (xpath_value, sizeof (xpath_value), "%s/rmap-set-action/min-metric",
            xpath);
  snprintf (value, sizeof (value), "%s", metric_str);
...
DEFPY_YANG (no_set_min_metric,
            no_set_min_metric_cmd,
            ...
            "Miximum metric value\n")
{
  ...
  snprintf (xpath_value, sizeof (xpath_value), "%s/rmap-set-action/max-metric",
            xpath);
  snprintf (value, sizeof (value), "%s", metric_str);
...
DEFPY_YANG (no_set_max_metric,
            no_set_max_metric_cmd,
            ...
            "Color of the SR-TE Policies to match with\n")
{
  ...
  char *arg =
      argv_find (argv, argc, "(1-4294967295)", &idx) ? argv[idx]->arg : NULL;
...
void
route_map_action_show (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
  const char *action = yang_dnode_get_string (dnode, "./action");
  const struct lyd_node *ln;
  ...
  if (IS_SET_IPv4_NH (action))
    {
      vty_out (
          vty, " set ip next-hop %s\n",
          yang_dnode_get_string (dnode, "./rmap-set-action/ipv4-address"));
    }
  else if (IS_SET_IPv6_NH (action))
    {
      vty_out (
          vty, " set ipv6 next-hop local %s\n",
          yang_dnode_get_string (dnode, "./rmap-set-action/ipv6-address"));
    }
  else if (IS_SET_METRIC (action))
    {
      if (yang_dnode_get (dnode, "./rmap-set-action/use-round-trip-time"))
      ...
      else if (yang_dnode_get (dnode, "./rmap-set-action/add-metric"))
        {
          vty_out (
              vty, " set metric +%s\n",
              yang_dnode_get_string (dnode, "./rmap-set-action/add-metric"));
        }
      else if (yang_dnode_get (dnode, "./rmap-set-action/subtract-metric"))
        {
          vty_out (vty, " set metric -%s\n",
                   yang_dnode_get_string (
                       dnode, "./rmap-set-action/subtract-metric"));
        }
      else
        {
          vty_out (vty, " set metric %s\n",
                   yang_dnode_get_string (dnode, "./rmap-set-action/value"));
        }
    }
  else if (IS_SET_MIN_METRIC (action))
    {
      vty_out (vty, " set min-metric %s\n",
               yang_dnode_get_string (dnode, "./rmap-set-action/min-metric"));
    }
  else if (IS_SET_MAX_METRIC (action))
    {
      vty_out (vty, " set max-metric %s\n",
               yang_dnode_get_string (dnode, "./rmap-set-action/max-metric"));
    }
  ...
  else if (IS_SET_SRC (action))
    {
      if (yang_dnode_exists (
              dnode, "./rmap-set-action/frr-zebra-route-map:ipv4-src-address"))
        vty_out (
            vty, " set src %s\n",
            yang_dnode_get_string (
                dnode,
  ...
  else if (IS_SET_METRIC_TYPE (action))
    {
      vty_out (vty, " set metric-type %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-ospf-route-map:metric-type"));
    }
  else if (IS_SET_FORWARDING_ADDR (action))
    {
      vty_out (
          vty, " set forwarding-address %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-set-action/frr-ospf6-route-map:ipv6-address"));
    }
  else if (IS_SET_WEIGHT (action))
    {
      vty_out (vty, " set weight %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:weight"));
    }
  else if (IS_SET_TABLE (action))
    {
      vty_out (vty, " set table %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:table"));
    }
  else if (IS_SET_LOCAL_PREF (action))
    {
      vty_out (vty, " set local-preference %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:local-pref"));
    }
  else if (IS_SET_LABEL_INDEX (action))
    {
      vty_out (vty, " set label-index %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:label-index"));
    }
  else if (IS_SET_DISTANCE (action))
    {
      vty_out (vty, " set distance %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:distance"));
    }
  else if (IS_SET_ORIGIN (action))
    {
      vty_out (vty, " set origin %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:origin"));
    }
  ...
  else if (IS_SET_AIGP_METRIC (action))
    {
      vty_out (vty, " set aigp-metric %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:aigp-metric"));
    }
  else if (IS_SET_ORIGINATOR_ID (action))
    {
      vty_out (
          vty, " set originator-id %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-set-action/frr-bgp-route-map:originator-id"));
    }
  else if (IS_SET_COMM_LIST_DEL (action))
    {
      acl = NULL;
      if ((ln = yang_dnode_get (
               dnode, "./rmap-set-action/frr-bgp-route-map:comm-list-name")) !=
          NULL)
        acl = yang_dnode_get_string (ln, NULL);
    ...
    {
      if (yang_dnode_exists (
              dnode, "./rmap-set-action/frr-bgp-route-map:community-string"))
      ...
      else
        {
          if (true ==
              yang_dnode_get_bool (
                  dnode, "./rmap-set-action/frr-bgp-route-map:community-none"))
    ...
    {
      vty_out (
          vty, " set extcommunity rt %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-set-action/frr-bgp-route-map:extcommunity-rt"));
    }
  else if (IS_SET_EXTCOMMUNITY_NT (action))
    {
      vty_out (
          vty, " set extcommunity nt %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-set-action/frr-bgp-route-map:extcommunity-nt"));
    }
  else if (IS_SET_EXTCOMMUNITY_SOO (action))
    {
      vty_out (
          vty, " set extcommunity soo %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-set-action/frr-bgp-route-map:extcommunity-soo"));
    }
    ...
    {
      if (yang_dnode_get_bool (
              dnode, "./rmap-set-action/frr-bgp-route-map:extcommunity-none"))
        vty_out (vty, " set extcommunity none\n");
    }
  else if (IS_SET_AGGREGATOR (action))
    {
      vty_out (
          vty, " set aggregator as %s %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " set as-path exclude %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-set-action/frr-bgp-route-map:exclude-as-path"));
    }
  else if (IS_SET_AS_REPLACE (action))
    {
      vty_out (
          vty, " set as-path replace %s\n",
          yang_dnode_get_string (
              dnode, "./rmap-set-action/frr-bgp-route-map:replace-as-path"));
    }
  else if (IS_SET_AS_PREPEND (action))
    {
      if (yang_dnode_exists (
              dnode, "./rmap-set-action/frr-bgp-route-map:prepend-as-path"))
        vty_out (
            vty, " set as-path prepend %s\n",
            yang_dnode_get_string (
                dnode, "./rmap-set-action/frr-bgp-route-map:prepend-as-path"));
      else
        {
          vty_out (vty, " set as-path prepend last-as %u\n",
                   yang_dnode_get_uint8 (
                       dnode, "./rmap-set-action/frr-bgp-route-map:last-as"));
        }
    }
  else if (IS_SET_IPV6_NH_GLOBAL (action))
    {
      vty_out (vty, " set ipv6 next-hop global %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:ipv6-address"));
    }
    ...
    {
      if (true == yang_dnode_get_bool (
                      dnode, "./rmap-set-action/frr-bgp-route-map:preference"))
  ...
  else if (IS_SET_IPV4_VPN_NH (action))
    {
      vty_out (vty, " set ipv4 vpn next-hop %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:ipv4-address"));
    }
  else if (IS_SET_BGP_IPV4_NH (action))
    {
      vty_out (vty, " set ip next-hop %s\n",
               yang_dnode_get_string (
                   dnode, "./rmap-set-action/frr-bgp-route-map:ipv4-nexthop"));
    }
  else if (IS_SET_BGP_EVPN_GATEWAY_IP_IPV4 (action))
    {
      vty_out (
          vty, " set evpn gateway-ip ipv4 %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " set evpn gateway-ip ipv6 %s\n",
          yang_dnode_get_string (
              dnode,
    ...
    {
      vty_out (
          vty, " set l3vpn next-hop encapsulation %s\n",
          yang_dnode_get_string (
              dnode,
...
DEFPY_YANG (no_rmap_onmatch_next,
...
DEFPY_YANG (no_rmap_onmatch_goto,
...
ALIAS_YANG (rmap_onmatch_goto,
            rmap_continue_cmd,
            "continue (1-65535)$rm_num",
            "Continue on a different entry within the route-map\n"
            "Route-map entry sequence number\n")

ALIAS_YANG (no_rmap_onmatch_goto,
...
void
route_map_exit_policy_show (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
void
route_map_call_show (struct vty *vty, const struct lyd_node *dnode,
                     bool show_defaults)
{
  vty_out (vty, " call %s\n", yang_dnode_get_string (dnode, NULL));
}

DEFPY_YANG (rmap_description,
...
void
route_map_description_show (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
  vty_out (vty, " description %s\n", yang_dnode_get_string (dnode, NULL));
}

DEFPY_YANG (route_map_optimization,
...
void
route_map_optimization_disabled_show (struct vty *vty,
...
static void
rmap_autocomplete (vector comps, struct cmd_token *token)
{
