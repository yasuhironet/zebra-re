...
static void
systemd_send_watchdog (struct event *t)
{
...
void
systemd_send_started (struct event_loop *m)
{
  assert (m != NULL);
...
void
systemd_send_status (const char *status)
{
  char buffer[1024];
...
static intmax_t
getenv_int (const char *varname, intmax_t dflt)
{
  char *val, *err;
  intmax_t intval;
