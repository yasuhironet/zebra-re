...
#define listnode_init(X, val)   ((X)->data = (val))
  ...
  listnode_add_before (struct list *list, struct listnode *pp, void *data);
   ...
   */
  extern struct listnode *listnode_lookup (struct list *list,
                                           const void *data);
   ...
   *
   * The sorting algorithm used is quicksort. Runtimes are equivalent to those
   * of quicksort plus N. The sort is not stable.
   *
   * For portability reasons, the comparison function takes a pointer to
   * pointer to void. This pointer should be dereferenced to get the actual
   * data pointer. It is always safe to do this.
   *
   * list
   *    list to sort
   *
   ...
   *
   * plist
   *    pointer to list pointer; this will be set to NULL after the list has
   * been deleted
   */
...
#define ALL_LIST_ELEMENTS_RO(list, node, data)                                \
  ...
  extern struct listnode *listnode_lookup_nocheck (struct list *list,
                                                   void *data);
