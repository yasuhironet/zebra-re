...
#define LS_PREF_SR           0x10
   ...
   */
  extern struct ls_node *ls_node_new (struct ls_node_id adv,
                                      struct in_addr rid,
  ...
  /**
   * Check if two Link State Nodes are equal. Note that this routine has the
   * same return value sense as '==' (which is different from a comparison).
   *
  ...
  /**
   * Check if two Link State Attributes are equal. Note that this routine has
   * the same return value sense as '==' (which is different from a
   * comparison).
   *
   ...
   */
  extern struct ls_prefix *ls_prefix_new (struct ls_node_id adv,
                                          struct prefix *p);
   ...
   * Vertex with its direct neighbors and the list of incoming Edges (E) that
   * connect the direct neighbors to this Vertex. Indeed, the Edge (E) is
   * unidirectional, thus, it is necessary to add 2 Edges to model a
   * bidirectional relation between 2 Vertices.
   *
  ...
  macro_inline int
  vertex_cmp (const struct ls_vertex *node1, const struct ls_vertex *node2)
  {
  ...
  macro_inline int
  edge_cmp (const struct ls_edge *edge1, const struct ls_edge *edge2)
  {
   ...
   */
  macro_inline int
  subnet_cmp (const struct ls_subnet *a, const struct ls_subnet *b)
  {
  ...
  /**
   * Delete Link State Vertex. This function clean internal Vertex lists
   * (incoming and outgoing Link State Edge and Link State Subnet). Vertex Data
   * structure is freed but not the Link State Node. Link State DB is not
   * modified if Vertex is NULL or not found in the Data Base. Note that
   * referenced to Link State Edges & SubNets are not removed as they could be
   ...
   * from this vertex. Link State Update message is sent if zclient is not
   * NULL.
   *
   * @param ted		Link State Data Base
  ...
  /**
   * This function convert the ISIS ISO system ID into a 64 bits unsigned
   * integer following the architecture dependent byte order.
   *
  ...
  /**
   * Check if two Vertices are equal. Note that this routine has the same
  ...
  /**
   * Update the Link State Attributes information of an existing Edge. If there
   * is no corresponding Edge in the Link State Data Base, a new Edge is
   * created.
   *
   * @param ted		Link State Data Base
   * @param attributes	Link State Attributes
  ...
  /**
   * Remove Edge from the Link State DB. Edge data structure is freed but not
   * the Link State Attributes data structure. Link State DB is not modified if
   * Edge is NULL or not found in the Data Base.
   *
   * @param ted	Link State Data Base
  ...
  ls_find_edge_by_source (struct ls_ted *ted,
                          struct ls_attributes *attributes);

  /**
   * Find Edge in the Link State Data Base by the destination (remote IPv4 or
   * IPv6 address of remote ID) information of the Link State Attributes
   *
   * @param ted		Link State Data Base
   * @param attributes	Link State Attributes
   *
  ...
  /**
   * Update the Link State Prefix information of an existing Subnet. If there
   * is no corresponding Subnet in the Link State Data Base, a new Subnet is
   * created.
   *
   * @param ted	Link State Data Base
   * @param pref	Link State Prefix
  ...
  /**
   * Connect Source and Destination Vertices by given Edge. Only non NULL
   * source and destination vertices are connected.
   *
   * @param src	Link State Source Vertex
   * @param dst	Link State Destination Vertex
   * @param edge	Link State Edge. Must not be NULL
   */
  extern void ls_connect_vertices (struct ls_vertex *src,
                                   struct ls_vertex *dst,
                                   struct ls_edge *edge);

  /**
   * Connect Link State Edge to the Link State Vertex which could be a Source
   * or a Destination Vertex.
   *
...
#define LS_MSG_TYPE_PREFIX     3
   ...
   *
   * @param zclient  Zebra client structure
   * @param server   Unregister daemon as a server (true) or as a client
   * (false)
   *
   * @return	   0 if success, -1 otherwise
   */
   ...
   *                      False otherwise. If true, return value is NULL in
   * case of deletion.
   *
   ...
   */
  extern struct ls_edge *ls_msg2edge (struct ls_ted *ted,
                                      struct ls_message *msg, bool delete);
  ...
  /**
   * Convert Link State Message into Link State element (Vertex, Edge or
   * Subnet) and update TED accordingly to the message event: SYNC, ADD, UPDATE
   * or DELETE.
   *
   * @param ted		Link State Database
   * @param msg		Link State Message
   * @param delete	True to delete the Link State Element from the
   * Database, False otherwise. If true, return value is NULL in case of
   * deletion.
   *
   * @return	Element if success, NULL otherwise or if Element is removed
   */
  extern struct ls_element *ls_msg2ted (struct ls_ted *ted,
                                        struct ls_message *msg, bool delete);
   ...
   * @return	Element if success, NULL otherwise or if Element is removed
   */
  extern struct ls_element *ls_stream2ted (struct ls_ted *ted,
                                           struct stream *s, bool delete);
  ...
  /**
   * Show all Link State Vertices information. If both vty and json are
   * specified, Json format output supersedes standard vty output.
   *
   * @param ted		Link State Data Base. Must not be NULL
   * @param vty		Pointer to vty output, could be NULL
   * @param json		Pointer to json output, could be NULL
  ...
  /**
   * Show all Link State Subnet information. If both vty and json are
   * specified, Json format output supersedes standard vty output.
   *
   * @param ted		Link State Data Base. Must not be NULL
   * @param vty		Pointer to vty output, could be NULL
