...
const char *
prefix_list_name (struct prefix_list *plist)
{
  return plist->name;
}

afi_t
prefix_list_afi (struct prefix_list *plist)
{
...
static int
prefix_list_compare_func (const struct prefix_list *a,
                          const struct prefix_list *b)
{
...
static struct prefix_list_entry *
prefix_seq_check (struct prefix_list *plist, int64_t seq)
{
  struct prefix_list_entry *pentry;
...
static void
trie_uninstall_fn (struct prefix_list_entry *object,
                   struct prefix_list_entry **updptr)
{
 ...
 */
static bool
prefix_list_entry_is_duplicated (struct prefix_list *list,
...
static void
trie_install_fn (struct prefix_list_entry *object,
                 struct prefix_list_entry **updptr)
{
  while (*updptr)
    {
 ...
 */
void
prefix_list_entry_update_start (struct prefix_list_entry *ple)
{
 ...
 */
void
prefix_list_entry_update_finish (struct prefix_list_entry *ple)
{
  struct prefix_list *pl = ple->pl;
  struct prefix_list_entry *point;
  ...
  route_map_notify_pentry_dependencies (pl->name, ple, RMAP_EVENT_PLIST_ADDED);
 ...
 */
void
prefix_list_entry_delete2 (struct prefix_list_entry *ple)
{
...
static int
prefix_list_entry_match (struct prefix_list_entry *pentry,
...
static void
vty_show_prefix_entry (struct vty *vty, json_object *json, afi_t afi,
                       struct prefix_list *plist, struct prefix_master *master,
  ...
  if (dtype == normal_display)
    {
      if (json)
        {
          json_object_string_add (json_pl, "addressFamily", afi2str (afi));
          json_object_int_add (json_pl, "entries", plist->count);
          if (plist->desc)
            json_object_string_add (json_pl, "description", plist->desc);
        }
      else
        {
      ...
      if (json)
        {
          ...
          json_object_int_add (json_pl, "rangeEntries", plist->rangecount);
      ...
      if (json)
        {
          ...
          json_object_object_add (json_pl, "entries", json_entries);
        }
          ...
          if (json)
            {
              ...
              json_object_int_add (json_entry, "sequenceNumber", pentry->seq);
              json_object_string_add (json_entry, "type",
                                      prefix_list_type_str (pentry));
              json_object_string_addf (json_entry, "prefix", "%pFX",
                                       &pentry->prefix);

              if (pentry->ge)
                json_object_int_add (json_entry, "minimumPrefixLength",
                                     pentry->ge);
              if (pentry->le)
                json_object_int_add (json_entry, "maximumPrefixLength",
                                     pentry->le);

              if (dtype == detail_display || dtype == sequential_display)
                {
                  json_object_int_add (json_entry, "hitCount", pentry->hitcnt);
                  json_object_int_add (json_entry, "referenceCount",
                                       pentry->refcnt);
                }
            }
          else
...
vty_show_prefix_list (struct vty *vty, afi_t afi, const char *name,
                      const char *seq, enum display_type dtype, bool uj)
{
  struct prefix_list *plist;
  struct prefix_master *master;
  int64_t seqnum = 0;
  ...
  if (name)
    {
      ...
      vty_show_prefix_entry (vty, json_proto, afi, plist, master, dtype,
                             seqnum);
    }
  else
    {
      ...
      if (match)
        {
          ...
          if (type == normal_display || type == first_match_display)
            vty_out (vty, " (hit count: %ld, refcount: %ld)", pentry->hitcnt,
                     pentry->refcnt);
       ...
       "First matched prefix\n")
{
  ...
  return vty_show_prefix_list_prefix (vty, AFI_IP6, prefix_list, prefix_str,
                                      dtype);
}
  ...
  if (! plist)
    {
      vty_out (vty, "%% no prefix list named %s for AFI %s\n", prefix_list,
               afi2str (family2afi (match->family)));
      return CMD_WARNING;
    }
...
int
prefix_bgp_orf_set (char *name, afi_t afi, struct orf_prefix *orfp, int permit,
                    int set)
{
  struct prefix_list *plist;
  struct prefix_list_entry *pentry;
  ...
  if (set)
  ...
  else
    {
      pentry = prefix_list_entry_lookup (
          plist, &orfp->p, (permit ? PREFIX_PERMIT : PREFIX_DENY), orfp->seq,
          orfp->le, orfp->ge);
...
static const struct cmd_variable_handler plist_var_handlers[] = {
  ...
  { .tokenname = "PREFIXLIST_NAME", .completions = plist_autocomplete },
  { .completions = NULL }
};
