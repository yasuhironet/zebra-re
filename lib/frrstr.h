...
extern "C"
{
   ...
   *
   * If 'replace' is longer than 'find', this function performs N+1
   * allocations, where N is the number of times 'find' occurs in 'str'. If
   * 'replace' is equal in length or shorter than 'find', only 1 allocation is
   * performed.
   *
   * str
   ...
   *
   * Returns:
   *    A new string, allocated with MTYPE_TMP, that is the result of
   * performing the replacement on 'str'. This must be freed by the caller.
   */
  char *frrstr_replace (const char *str, const char *find,
                        const char *replace);
