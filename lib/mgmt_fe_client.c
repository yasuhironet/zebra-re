...
struct debug mgmt_dbg_fe_client = { 0,
                                    "Management frontend client operations" };


static inline const char *
dsid2name (Mgmtd__DatastoreId id)
{
  switch ((int) id)
    {
    case MGMTD_DS_NONE:
  ...
  FOREACH_SESSION_IN_LIST (client, session)
    {
      if (session->session_id == session_id)
        {
          MGMTD_FE_CLIENT_DBG ("Found session of client-id %" PRIu64
...
static int
mgmt_fe_client_send_msg (struct mgmt_fe_client *client,
...
static int
mgmt_fe_send_register_req (struct mgmt_fe_client *client)
{
...
static int
mgmt_fe_send_session_req (struct mgmt_fe_client *client,
                          struct mgmt_fe_client_session *session, bool create)
{
  ...
  MGMTD_FE_CLIENT_DBG ("Sending SESSION_REQ %s message for client-id %" PRIu64,
...
int
mgmt_fe_send_lockds_req (struct mgmt_fe_client *client, uint64_t session_id,
...
int
mgmt_fe_send_setcfg_req (struct mgmt_fe_client *client, uint64_t session_id,
...
int
mgmt_fe_send_commitcfg_req (struct mgmt_fe_client *client, uint64_t session_id,
...
int
mgmt_fe_send_get_req (struct mgmt_fe_client *client, uint64_t session_id,
                      ...
                      Mgmtd__YangGetDataReq *data_req[], int num_data_reqs)
{
  ...
  MGMTD_FE_CLIENT_DBG (
      "Sending GET_REQ (iscfg %d) message for DS:%s session-id %" PRIu64
      " (#xpaths:%d)",
...
int
mgmt_fe_send_regnotify_req (struct mgmt_fe_client *client, uint64_t session_id,
                            uint64_t req_id, Mgmtd__DatastoreId ds_id,
                            bool register_req,
...
static int
mgmt_fe_client_handle_msg (struct mgmt_fe_client *client,
                           Mgmtd__FeMessage *fe_msg)
{
  ...
  switch ((int) fe_msg->message_case)
    ...
    case MGMTD__FE_MESSAGE__MESSAGE_SESSION_REPLY:
      if (fe_msg->session_reply->create &&
          fe_msg->session_reply->has_client_conn_id)
        {
          ...
          if (session && fe_msg->session_reply->success)
            {
              MGMTD_FE_CLIENT_DBG ("Session Created for client-id %" PRIu64,
                                   fe_msg->session_reply->client_conn_id);
              session->session_id = fe_msg->session_reply->session_id;
            }
          else
            {
              MGMTD_FE_CLIENT_ERR (
      ...
      if (session && session->client &&
        ...
        (*session->client->cbs.client_session_notify) (
            ...
            fe_msg->session_reply->create, fe_msg->session_reply->success,
            fe_msg->session_reply->session_id, session->user_ctx);
      break;
    case MGMTD__FE_MESSAGE__MESSAGE_LOCKDS_REPLY:
      ...
      if (session && session->client && session->client->cbs.lock_ds_notify)
        (*session->client->cbs.lock_ds_notify) (
            client, client->user_data, session->client_id,
            fe_msg->lockds_reply->session_id, session->user_ctx,
            fe_msg->lockds_reply->req_id, fe_msg->lockds_reply->lock,
            fe_msg->lockds_reply->success, fe_msg->lockds_reply->ds_id,
    ...
    case MGMTD__FE_MESSAGE__MESSAGE_SETCFG_REPLY:
      ...
      if (session && session->client && session->client->cbs.set_config_notify)
        (*session->client->cbs.set_config_notify) (
            client, client->user_data, session->client_id,
            fe_msg->setcfg_reply->session_id, session->user_ctx,
            fe_msg->setcfg_reply->req_id, fe_msg->setcfg_reply->success,
            fe_msg->setcfg_reply->ds_id, fe_msg->setcfg_reply->implicit_commit,
    ...
    case MGMTD__FE_MESSAGE__MESSAGE_COMMCFG_REPLY:
      ...
      if (session && session->client &&
        ...
        (*session->client->cbs.commit_config_notify) (
            client, client->user_data, session->client_id,
            fe_msg->commcfg_reply->session_id, session->user_ctx,
            fe_msg->commcfg_reply->req_id, fe_msg->commcfg_reply->success,
            fe_msg->commcfg_reply->src_ds_id, fe_msg->commcfg_reply->dst_ds_id,
    ...
    case MGMTD__FE_MESSAGE__MESSAGE_GET_REPLY:
      ...
      session = mgmt_fe_find_session_by_session_id (
          client, fe_msg->get_reply->session_id);

      if (session && session->client && session->client->cbs.get_data_notify)
        (*session->client->cbs.get_data_notify) (
            client, client->user_data, session->client_id,
            fe_msg->get_reply->session_id, session->user_ctx,
            fe_msg->get_reply->req_id, fe_msg->get_reply->success,
            fe_msg->get_reply->ds_id,
            fe_msg->get_reply->data ? fe_msg->get_reply->data->data : NULL,
            fe_msg->get_reply->data ? fe_msg->get_reply->data->n_data : 0,
            fe_msg->get_reply->data ? fe_msg->get_reply->data->next_indx : 0,
...
static void
mgmt_fe_client_process_msg (uint8_t version, uint8_t *data, size_t len,
                            struct msg_conn *conn)
{
  ...
  if (! fe_msg)
    {
      MGMTD_FE_CLIENT_DBG ("Failed to decode %zu bytes from server.", len);
      return;
    }
  MGMTD_FE_CLIENT_DBG ("Decoded %zu bytes of message(msg: %u/%u) from server",
...
static int
_notify_connect_disconnect (struct msg_client *msg_client, bool connected)
{
  struct mgmt_fe_client *client =
      ...
      FOREACH_SESSION_IN_LIST (client, session)
        {
          ...
          if (session->client->cbs.client_session_notify)
            {
              (*session->client->cbs.client_session_notify) (
                  client, client->user_data, session->client_id, false, true,
...
static int
mgmt_fe_client_notify_connect (struct msg_client *client)
{
  return _notify_connect_disconnect (client, true);
}

static int
mgmt_fe_client_notify_disconnect (struct msg_conn *conn)
{
...
static void
mgmt_debug_client_fe_set_all (uint32_t flags, bool set)
{
  DEBUG_FLAGS_SET (&mgmt_dbg_fe_client, flags, set);
}

static int
mgmt_debug_fe_client_config_write (struct vty *vty)
{
...
void
mgmt_debug_fe_client_show_debug (struct vty *vty)
{
...
void
mgmt_fe_client_lib_vty_init (void)
{
...
uint
mgmt_fe_client_session_count (struct mgmt_fe_client *client)
{
 ...
 */
enum mgmt_result
mgmt_fe_create_client_session (struct mgmt_fe_client *client,
                               uint64_t client_id, uintptr_t user_ctx)
{
  struct mgmt_fe_client_session *session;

  session =
      XCALLOC (MTYPE_MGMTD_FE_SESSION, sizeof (struct mgmt_fe_client_session));
  assert (session);
 ...
 */
void
mgmt_fe_client_destroy (struct mgmt_fe_client *client)
{
  struct mgmt_fe_client_session *session;

  MGMTD_FE_CLIENT_DBG ("Destroying MGMTD Frontend Client '%s'", client->name);
