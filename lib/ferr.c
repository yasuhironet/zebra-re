...
static bool
ferr_hash_cmp (const void *a, const void *b)
{
...
void
log_ref_add (struct log_ref *ref)
{
  uint32_t i = 0;
...
void
log_ref_display (struct vty *vty, uint32_t code, bool json)
{
  struct log_ref *ref;
      ...
      if (json)
        {
          ...
          json_object_string_add (obj, "description", ref->description);
...
log_ref_init (void)
{
  frr_with_mutex (&refs_mtx)
    {
      refs =
          hash_create (ferr_hash_key, ferr_hash_cmp, "Error Reference Texts");
    }
}
...
PRINTFRR (7, 0)
static ferr_r
ferr_set_va (const char *file, int line, const char *func, enum ferr_kind kind,
             const char *pathname, int errno_val, const char *text, va_list va)
{
...
ferr_r
ferr_set_internal (const char *file, int line, const char *func,
...
ferr_r
ferr_set_internal_ext (const char *file, int line, const char *func,
...
#define REPLACE "$ERR"
void
vty_print_error (struct vty *vty, ferr_r err, const char *msg, ...)
{
  ...
  if (! replacepos)
  ...
  else
    {
      ...
      vty_out (vty, "%s%s%s\n", tmpmsg,
               last_error ? last_error->message : "(no error?)", replacepos);
    }
}
