...
#define YANG_ITER_STOP     -1
  ...
  /*
   * Create a new YANG module and load it using libyang. If the YANG module is
   * not found in the YANG_MODELS_PATH directory, the program will exit with an
   * error. Once loaded, a YANG module can't be unloaded anymore.
   *
   * module_name
  ...
  /*
   * Check if the libyang schema node represents typeless data (e.g.
   * containers, leafs of type empty, etc).
   *
   * snode
   *    libyang schema node to operate on.
   *
  ...
  /*
   * Get the type structure of a leaf of leaf-list. If the type is a leafref,
   * the final (if there is a chain of leafrefs) target's type is found.
   *
   * snode
   *    libyang schema node to operate on.
   *
   * Returns:
   *    The found type if the schema node represents a leaf or a leaf-list,
   * NULL otherwise.
   */
...
#define LY_FOR_KEYS(snode, skey)                                              \
   ...
   *
   * xpath
   *    Optional limited XPath (absolute or relative) format string. See Path
   * in libyang documentation for restrictions.
   *
   * ...
   *    any parameters for xpath_fmt.
   *
   ...
   */
  extern void yang_dnode_change_leaf (struct lyd_node *dnode,
                                      const char *value);
   ...
   *
   * config
   *    Specify whether the data node will contain only configuration data
   * (true) or both configuration data and state data (false).
   *
   * Returns:
   ...
   */
  extern struct yang_data *yang_data_new (const char *xpath,
                                          const char *value);
  ...
  /*
   * Create a new linked list of yang_data structures. The list 'del' callback
   * is initialized appropriately so that the entire list can be deleted safely
   * with list_delete_and_null().
   *
   * Returns:
   ...
   *
   * embedded_modules
   *    Specify whether libyang should attempt to look for embedded YANG
   * modules.
   *
   * explicit_compile
   ...
   *
   * enable
   *    When set to true, enable libyang verbose debugging, otherwise disable
   * it.
   */
   ...
   * embedded_modules
   *    Specify whether libyang should attempt to look for embedded YANG
   * modules. defer_compile Hold off on compiling modules until
   * yang_init_loading_complete is called.
   */
  extern void yang_init (bool embedded_modules, bool defer_compile);

  /*
   * Should be called after yang_init and all yang_module_load()s have been
   * done, compiles all modules loaded into the yang context.
   */
  extern void yang_init_loading_complete (void);

  /*
   * Finish the YANG subsystem gracefully. Should be called only when the
   * daemon is exiting.
   */
