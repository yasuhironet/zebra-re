...
static void
frrzmq_read_msg (struct event *t)
{
...
static void
frrzmq_write_msg (struct event *t)
{
  struct frrzmq_cb **cbp = EVENT_ARG (t);
  struct frrzmq_cb *cb;
  ...
  while (1)
    {
      zmq_pollitem_t polli = { .socket = cb->zmqsock, .events = ZMQ_POLLOUT };
      ret = zmq_poll (&polli, 1, 0);
...
int
_frrzmq_event_add_write (const struct xref_eventsched *xref,
                         struct event_loop *master,
                         void (*msgfunc) (void *arg, void *zmqsock),
                         void (*errfunc) (void *arg, void *zmqsock), void *arg,
                         void *zmqsock, struct frrzmq_cb **cbp)
{
  ...
  if (events & ZMQ_POLLOUT)
    ...
    event_add_write (master, frrzmq_write_msg, cbp, fd, &cb->write.thread);
  return 0;
}

void
frrzmq_thread_cancel (struct frrzmq_cb **cb, struct cb_core *core)
{
  if (! cb || ! *cb)
    return;
  core->cancelled = true;
...
void
frrzmq_check_events (struct frrzmq_cb **cbp, struct cb_core *core, int event)
{
  struct frrzmq_cb *cb;
  int events;
  size_t len;
  ...
  if ((events & event) && core->thread && ! core->cancelled)
    {
      ...
      if (event == ZMQ_POLLIN)
        event_add_event (tm, frrzmq_read_msg, cbp, cb->fd, &core->thread);
      else
        event_add_event (tm, frrzmq_write_msg, cbp, cb->fd, &core->thread);
    }
}
