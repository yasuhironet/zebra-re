 ...
 */
int
if_cmp_name_func (const char *p1, const char *p2)
{
  unsigned int l1, l2;
  long int x1, x2;
  int res;
...
static int
if_cmp_func (const struct interface *ifp1, const struct interface *ifp2)
{
  return if_cmp_name_func (ifp1->name, ifp2->name);
}

static int
if_cmp_index_func (const struct interface *ifp1, const struct interface *ifp2)
{
...
void
if_new_via_zapi (struct interface *ifp)
{
...
void
if_destroy_via_zapi (struct interface *ifp)
{
...
void
if_up_via_zapi (struct interface *ifp)
{
...
void
if_down_via_zapi (struct interface *ifp)
{
...
static struct interface *
if_create_name (const char *name, struct vrf *vrf)
{
  struct interface *ifp;
...
static struct interface *
if_lookup_by_ifindex (ifindex_t ifindex, vrf_id_t vrf_id)
{
  struct vrf *vrf;
...
/* Interface existence check by index. */
struct interface *
if_vrf_lookup_by_index_next (ifindex_t ifindex, vrf_id_t vrf_id)
{
...
ifindex2ifname (ifindex_t ifindex, vrf_id_t vrf_id)
{
  ...
  return ((ifp = if_lookup_by_index (ifindex, vrf_id)) != NULL) ? ifp->name
                                                                : "unknown";
}
...
if_lookup_by_name (const char *name, vrf_id_t vrf_id)
{
  ...
  if (! vrf || ! name || strnlen (name, INTERFACE_NAMSIZ) == INTERFACE_NAMSIZ)
    return NULL;
...
struct interface *
if_lookup_by_name_vrf (const char *name, struct vrf *vrf)
{
  struct interface if_tmp;
 ...
 */
struct interface *
if_lookup_address_local (const void *src, int family, vrf_id_t vrf_id)
{
  struct vrf *vrf = vrf_lookup_by_id (vrf_id);
...
/* Lookup interface by IP address. */
struct connected *
if_lookup_address (const void *matchaddr, int family, vrf_id_t vrf_id)
{
  struct vrf *vrf = vrf_lookup_by_id (vrf_id);
  struct prefix addr;
  int bestlen = 0;
...
struct interface *
if_lookup_prefix (const struct prefix *prefix, vrf_id_t vrf_id)
{
  struct vrf *vrf = vrf_lookup_by_id (vrf_id);
  struct listnode *cnode;
  struct interface *ifp;
...
size_t
if_lookup_by_hwaddr (const uint8_t *hw_addr, size_t addrsz,
                     struct interface ***result, vrf_id_t vrf_id)
{
  ...
  if (rs->count)
    {
      *result = XCALLOC (MTYPE_TMP, sizeof (struct interface *) * rs->count);
...
/* Get interface by name if given name interface doesn't exist create
   one. */
struct interface *
if_get_by_name (const char *name, vrf_id_t vrf_id, const char *vrf_name)
{
  struct interface *ifp = NULL;
  struct vrf *vrf;
...
int
if_set_index (struct interface *ifp, ifindex_t ifindex)
{
...
static void
if_set_name (struct interface *ifp, const char *name)
{
...
int
if_is_no_ptm_operative (const struct interface *ifp)
{
  return ((ifp->flags & IFF_UP) &&
          ((ifp->flags & IFF_RUNNING) ||
           ! CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_LINKDETECTION)));
...
int
if_is_loopback_exact (const struct interface *ifp)
{
...
int
if_is_vrf (const struct interface *ifp)
{
...
if_dump (const struct interface *ifp)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, c))
    zlog_info ("Interface %s vrf %s(%u) index %d metric %d mtu %d mtu6 %d %s",
...
static int
connected_same_prefix (const struct prefix *p1, const struct prefix *p2)
{
...
unsigned int
connected_count_by_family (struct interface *ifp, int family)
{
  struct listnode *cnode;
...
struct connected *
connected_lookup_prefix_exact (struct interface *ifp, const struct prefix *p)
{
  struct listnode *node;
...
/* Find the address on our side that will be used when packets
   are sent to dst. */
struct connected *
connected_lookup_prefix (struct interface *ifp, const struct prefix *addr)
{
  struct listnode *cnode;
  struct connected *c;
  struct connected *match;
...
struct connected *
connected_get_linklocal (struct interface *ifp)
{
  struct listnode *n;
...
bool
if_link_params_cmp (struct if_link_params *iflp1, struct if_link_params *iflp2)
{
...
void
if_link_params_copy (struct if_link_params *dst, struct if_link_params *src)
{
...
struct if_link_params *
if_link_params_enable (struct interface *ifp)
{
...
static int
vrfname_by_ifname (const char *ifname, const char **vrfname)
{
  struct vrf *vrf;
  struct interface *ifp;
  int count = 0;
...
static void
netns_ifname_split (const char *xpath, char *ifname, char *vrfname)
{
  char *delim;
  int len;
...
static void
cli_show_interface (struct vty *vty, const struct lyd_node *dnode,
                    bool show_defaults)
{
  vty_out (vty, "!\n");
  ...
  if (vrf_is_backend_netns ())
    {
      ...
      netns_ifname_split (yang_dnode_get_string (dnode, "./name"), ifname,
                          vrfname);
...
static void
cli_show_interface_end (struct vty *vty, const struct lyd_node *dnode)
{
  vty_out (vty, "exit\n");
}

void
if_vty_config_start (struct vty *vty, struct interface *ifp)
{
  vty_frame (vty, "!\n");
...
void
if_vty_config_end (struct vty *vty)
{
...
static void
cli_show_interface_desc (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
...
static void
if_autocomplete (vector comps, struct cmd_token *token)
{
  struct interface *ifp;
  struct vrf *vrf;
...
static int
if_config_write_single (const struct lyd_node *dnode, void *arg)
{
...
static int
if_nb_config_write (struct vty *vty)
{
...
void
if_cmd_init_default (void)
{
  if_cmd_init (if_nb_config_write);
}

void
if_zapi_callbacks (int (*create) (struct interface *ifp),
 ...
 */
static int
lib_interface_create (struct nb_cb_create_args *args)
{
  const char *ifname;
  struct interface *ifp;
      ...
      if (vrf_is_backend_netns ())
        {
          ...
          ifp = if_get_by_name (ifname_ns, VRF_UNKNOWN, vrfname_ns);
        }
      else
        {
...
static int
lib_interface_destroy (struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct vrf *vrf;
 ...
 */
static const void *
lib_interface_get_next (struct nb_cb_get_next_args *args)
{
  struct vrf *vrf;
...
static int
lib_interface_get_keys (struct nb_cb_get_keys_args *args)
{
  ...
  if (vrf_is_backend_netns ())
    snprintf (args->keys->key[0], sizeof (args->keys->key[0]), "%s:%s",
 ...
 */
static int
lib_interface_description_modify (struct nb_cb_modify_args *args)
{
  struct interface *ifp;
...
static int
lib_interface_description_destroy (struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
