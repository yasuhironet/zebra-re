...
struct debug mgmt_dbg_be_client = { 0,
                                    "Management backend client operations" };
...
static int
mgmt_be_client_send_msg (struct mgmt_be_client *client_ctx,
...
mgmt_be_batch_create (struct mgmt_be_txn_ctx *txn, uint64_t batch_id)
{
  ...
  if (! batch)
    {
      batch =
          XCALLOC (MTYPE_MGMTD_BE_BATCH, sizeof (struct mgmt_be_batch_ctx));
      assert (batch);

      batch->batch_id = batch_id;
      mgmt_be_batches_add_tail (&txn->cfg_batches, batch);

      MGMTD_BE_CLIENT_DBG ("Added new batch-id: %" PRIu64 " to transaction",
                           batch_id);
    }
...
static void
mgmt_be_batch_delete (struct mgmt_be_txn_ctx *txn,
  ...
  if ((*batch)->txn_req.event == MGMTD_BE_TXN_PROC_SETCFG)
    {
      for (indx = 0; indx < MGMTD_MAX_CFG_CHANGES_IN_BATCH; indx++)
        {
          if ((*batch)->txn_req.req.set_cfg.cfg_changes[indx].value)
            {
              free ((char *) (*batch)
                        ->txn_req.req.set_cfg.cfg_changes[indx]
                        .value);
            }
        }
    }
...
static void
mgmt_be_cleanup_all_batches (struct mgmt_be_txn_ctx *txn)
{
  struct mgmt_be_batch_ctx *batch = NULL;
...
mgmt_be_txn_create (struct mgmt_be_client *client_ctx, uint64_t txn_id)
{
  ...
  if (txn)
    {
      MGMTD_BE_CLIENT_ERR ("Can't create existing txn-id: %" PRIu64, txn_id);
      return NULL;
    }
...
static void
mgmt_be_txn_delete (struct mgmt_be_client *client_ctx,
                    struct mgmt_be_txn_ctx **txn)
{
  ...
  if (client_ctx->cbs.txn_notify)
    (void) (*client_ctx->cbs.txn_notify) (client_ctx, client_ctx->user_data,
...
static void
mgmt_be_cleanup_all_txns (struct mgmt_be_client *client_ctx)
{
  struct mgmt_be_txn_ctx *txn = NULL;
...
static int
mgmt_be_send_txn_reply (struct mgmt_be_client *client_ctx, uint64_t txn_id,
                        bool create)
{
  Mgmtd__BeMessage be_msg;
...
static int
mgmt_be_process_txn_req (struct mgmt_be_client *client_ctx, uint64_t txn_id,
                         bool create)
{
  struct mgmt_be_txn_ctx *txn;
  ...
  if (create)
    {
      ...
      if (client_ctx->cbs.txn_notify)
        (*client_ctx->cbs.txn_notify) (client_ctx, client_ctx->user_data,
...
static int
mgmt_be_send_cfgdata_create_reply (struct mgmt_be_client *client_ctx,
...
static void
mgmt_be_txn_cfg_abort (struct mgmt_be_txn_ctx *txn)
{
...
static int
mgmt_be_txn_cfg_prepare (struct mgmt_be_txn_ctx *txn)
{
      ...
      if (! txn->nb_txn)
        {
          ...
          nb_candidate_edit_config_changes (
              client_ctx->candidate_config, txn_req->req.set_cfg.cfg_changes,
          ...
          if (error)
            {
              ...
              MGMTD_BE_CLIENT_ERR (
                  ...
                  " batch-id: %" PRIu64 " to candidate, err: '%s'",
                  txn->txn_id, batch->batch_id, err_buf);
              return -1;
            }
          gettimeofday (&edit_nb_cfg_end, NULL);
          edit_nb_cfg_tm =
              timeval_elapsed (edit_nb_cfg_end, edit_nb_cfg_start);
  ...
  client_ctx->avg_prep_nb_cfg_tm =
      ((client_ctx->avg_prep_nb_cfg_tm * client_ctx->num_prep_nb_cfg) +
       prep_nb_cfg_tm) /
  ...
  FOREACH_BE_TXN_BATCH_IN_LIST (txn, batch)
    {
      mgmt_be_send_cfgdata_create_reply (client_ctx, txn->txn_id,
                                         batch->batch_id, error ? false : true,
                                         error ? err_buf : NULL);
      if (! error)
        {
          SET_FLAG (batch->flags, MGMTD_BE_BATCH_FLAGS_CFG_PREPARED);
          mgmt_be_batches_del (&txn->cfg_batches, batch);
 ...
 */
static int
mgmt_be_update_setcfg_in_batch (struct mgmt_be_client *client_ctx,
                                struct mgmt_be_txn_ctx *txn, uint64_t batch_id,
  ...
  for (index = 0; index < num_req; index++)
    {
      ...
      if (cfg_req[index]->req_type == MGMTD__CFG_DATA_REQ_TYPE__DELETE_DATA)
...
static int
mgmt_be_process_cfgdata_req (struct mgmt_be_client *client_ctx,
                             uint64_t txn_id, uint64_t batch_id,
                             Mgmtd__YangCfgDataReq *cfg_req[], int num_req,
                             bool end_of_data)
{
  ...
  mgmt_be_update_setcfg_in_batch (client_ctx, txn, batch_id, cfg_req, num_req);
...
static int
mgmt_be_send_apply_reply (struct mgmt_be_client *client_ctx, uint64_t txn_id,
                          ...
                          bool success, const char *error_if_any)
{
  ...
  MGMTD_BE_CLIENT_DBG (
      "Sending CFG_APPLY_REPLY txn-id %" PRIu64 " %zu batch ids %" PRIu64
      " - %" PRIu64,
      txn_id, num_batch_ids, success && num_batch_ids ? batch_ids[0] : 0,
...
static int
mgmt_be_txn_proc_cfgapply (struct mgmt_be_txn_ctx *txn)
{
  struct mgmt_be_client *client_ctx;
  ...
  mgmt_be_send_apply_reply (client_ctx, txn->txn_id, batch_ids, num_processed,
                            true, NULL);
...
static int
mgmt_be_process_cfg_apply (struct mgmt_be_client *client_ctx, uint64_t txn_id)
{
  struct mgmt_be_txn_ctx *txn;
...
static int
mgmt_be_client_handle_msg (struct mgmt_be_client *client_ctx,
                           Mgmtd__BeMessage *be_msg)
{
  /*
  ...
  switch ((int) be_msg->message_case)
    ...
    case MGMTD__BE_MESSAGE__MESSAGE_CFG_DATA_REQ:
      ...
      mgmt_be_process_cfgdata_req (
          ...
          be_msg->cfg_data_req->batch_id, be_msg->cfg_data_req->data_req,
          be_msg->cfg_data_req->n_data_req, be_msg->cfg_data_req->end_of_data);
      break;
...
static void
mgmt_be_client_process_msg (uint8_t version, uint8_t *data, size_t len,
                            struct msg_conn *conn)
{
  ...
  if (! be_msg)
    {
      MGMTD_BE_CLIENT_DBG ("Failed to decode %zu bytes from server", len);
      return;
    }
...
int
mgmt_be_send_subscr_req (struct mgmt_be_client *client_ctx, bool subscr_xpaths,
                         int num_xpaths, char **reg_xpaths)
{
  Mgmtd__BeMessage be_msg;
...
static int
_notify_conenct_disconnect (struct msg_client *msg_client, bool connected)
{
  struct mgmt_be_client *client =
  ...
  if (client->cbs.client_connect_notify)
    (void) (*client->cbs.client_connect_notify) (client, client->user_data,
                                                 connected);
...
static int
mgmt_be_client_notify_conenct (struct msg_client *client)
{
  return _notify_conenct_disconnect (client, true);
}

static int
mgmt_be_client_notify_disconenct (struct msg_conn *conn)
{
...
static void
mgmt_debug_client_be_set_all (uint32_t flags, bool set)
{
  DEBUG_FLAGS_SET (&mgmt_dbg_be_client, flags, set);
}

static int
mgmt_debug_be_client_config_write (struct vty *vty)
{
...
void
mgmt_debug_be_client_show_debug (struct vty *vty)
{
...
struct mgmt_be_client *
mgmt_be_client_create (const char *client_name, struct mgmt_be_client_cbs *cbs,
...
void
mgmt_be_client_lib_vty_init (void)
{
...
void
mgmt_be_client_destroy (struct mgmt_be_client *client)
{
  MGMTD_BE_CLIENT_DBG ("Destroying MGMTD Backend Client '%s'", client->name);
