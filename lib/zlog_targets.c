...
void
zlog_fd (struct zlog_target *zt, struct zlog_msg *msgs[], size_t nmsgs)
{
...
static void
zlog_fd_sigsafe (struct zlog_target *zt, const char *text, size_t len)
{
  struct zlt_fd *zte = container_of (zt, struct zlt_fd, zt);
  struct iovec iov[4];
  int fd;
...
void
zlog_file_init (struct zlog_cfg_file *zcf)
{
...
static void
zlog_file_target_free (struct zlt_fd *zlt)
{
  if (! zlt)
    return;
...
void
zlog_file_fini (struct zlog_cfg_file *zcf)
{
  if (zcf->active)
    {
      struct zlt_fd *ztf;
...
static bool
zlog_file_cycle (struct zlog_cfg_file *zcf)
{
  ...
  do
    {
      ...
      zt = zlog_target_clone (MTYPE_LOG_FD, &zcf->active->zt, sizeof (*zlt));
...
void
zlog_file_set_other (struct zlog_cfg_file *zcf)
{
...
bool
zlog_file_set_filename (struct zlog_cfg_file *zcf, const char *filename)
{
  frr_with_mutex (&zcf->cfg_mtx)
    {
      XFREE (MTYPE_LOG_FD_NAME, zcf->filename);
...
bool
zlog_file_set_fd (struct zlog_cfg_file *zcf, int fd)
{
  frr_with_mutex (&zcf->cfg_mtx)
    {
      if (zcf->fd == fd)
...
bool
zlog_file_rotate (struct zlog_cfg_file *zcf)
{
...
static void
zlog_crashlog_sigsafe (struct zlog_target *zt, const char *text, size_t len)
{
...
static void
zlog_crashlog_plain (struct zlog_target *zt, struct zlog_msg *msgs[],
                     size_t nmsgs)
{
  size_t i, len;
  const char *text;
...
static int
zlt_aux_init (const char *prefix, int prio_min)
{
  zlog_is_aux = true;
...
static int
zlt_init (const char *progname, const char *protoname, unsigned short instance,
          uid_t uid, gid_t gid)
{
...
__attribute__ ((_CONSTRUCTOR (450))) static void
zlog_startup_init (void)
{
...
static void
zlog_syslog (struct zlog_target *zt, struct zlog_msg *msgs[], size_t nmsgs)
{
  size_t i;
...
static void
zlog_syslog_sigsafe (struct zlog_target *zt, const char *text, size_t len)
{
...
void
zlog_syslog_set_facility (int facility)
{
...
void
zlog_syslog_set_prio_min (int prio_min)
{
  struct zlog_target *newztc;
