...
#define OS_THREAD_NAMELEN 16
   ...
   * including ID, start and stop functions and the desired name.
   * Alternatively, if attr is NULL, the default attributes will be used. The
   * pthread will be set up to run a basic threadmaster loop and the name will
   * be "Anonymous". Scheduling tasks onto the threadmaster in the 'master'
   * field of the returned frr_pthread will cause them to run on that pthread.
   *
   ...
   * exposed by the pthread. It waits until the 'running' condition is
   * satisfied (see struct definition of frr_pthread).
   *
  ...
  /*
   * Notifies other pthreads that the calling thread has finished setting up
   * and is ready to begin work.
   *
   * This will allow any other pthreads waiting in 'frr_pthread_wait_running'
   * to proceed.
   *
   ...
   * @param result - where to store the thread's result, if any. May be NULL if
   * a result is not needed.
   */
...
#define frr_with_mutex(...)                                                   \
  for (pthread_mutex_t MACRO_REPEAT (_frr_with_mutex, ##__VA_ARGS__) *_once = \
           NULL;                                                              \
...
#define frr_mutex_lock_autounlock(mutex)                                      \
  ...
  _frr_mtx_lock (pthread_mutex_t *mutex)
  {
  ...
  static inline void
  _frr_mtx_unlock (pthread_mutex_t **mutex)
  {
    if (! *mutex)
      return;
