...
db_init (const char *path_fmt, ...)
{
  ...
  if (sqlite3_open_v2 (path, &dbp,
                       ...
                       NULL) != SQLITE_OK)
    {
      if (dbp == NULL)
        {
          zlog_warn ("%s: failed to open database '%s'", __func__, path);
          return -1;
        }

      zlog_warn ("%s: failed to open database '%s': %s", __func__, path,
                 sqlite3_errmsg (dbp));
  ...
  while (*sptr)
    {
      ...
      switch (*sptr)
        ...
        case 's':
          ...
          vlen = va_arg (vl, int);
          if (sqlite3_bind_text (ss, column++, str, vlen, SQLITE_STATIC) !=
              SQLITE_OK)
            return -1;
          break;
        case 'b':
          blob = va_arg (vl, void *);
          vlen = va_arg (vl, int);
          if (sqlite3_bind_blob (ss, column++, blob, vlen, SQLITE_STATIC) !=
              SQLITE_OK)
            return -1;
          break;
        case 'n':
...
int
db_execute (const char *stmt_fmt, ...)
{
  char stmt[BUFSIZ];
  va_list ap;
