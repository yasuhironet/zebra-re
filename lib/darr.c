...
static uint
darr_next_count (uint count, size_t esize)
{
  uint ncount;
...
static size_t
darr_size (uint count, size_t esize)
{
  return count * esize + sizeof (struct darr_metadata);
}

void *
__darr_resize (void *a, uint count, size_t esize)
{
...
void *
__darr_insert_n (void *a, uint at, uint count, size_t esize, bool zero)
{
