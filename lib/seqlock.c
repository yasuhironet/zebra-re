...
#define wait_time(sqlo, val, time, reltime)                                   \
  sys_futex ((int *) &sqlo->pos, FUTEX_WAIT_BITSET, (int) val, time, NULL, ~0U)
...
static int
wait_time (struct seqlock *sqlo, uint32_t val, const struct timespec *abstime,
...
#define wait_time(sqlo, val, time, reltime)                                   \
  pthread_cond_timedwait (&sqlo->wake, &sqlo->lock, time);
...
void
seqlock_wait (struct seqlock *sqlo, seqlock_val_t val)
{
  ...
  while (cur & SEQLOCK_HELD)
    {
      ...
      if ((cur & SEQLOCK_WAITERS) ||
          atomic_compare_exchange_weak_explicit (
              &sqlo->pos, &cur, cur | SEQLOCK_WAITERS, memory_order_relaxed,
...
bool
seqlock_timedwait (struct seqlock *sqlo, seqlock_val_t val,
...
bool
seqlock_check (struct seqlock *sqlo, seqlock_val_t val)
{
  seqlock_val_t cur;
...
void
seqlock_acquire_val (struct seqlock *sqlo, seqlock_val_t val)
{
  seqlock_val_t prev;
...
void
seqlock_release (struct seqlock *sqlo)
{
  seqlock_val_t prev;
...
void
seqlock_init (struct seqlock *sqlo)
{
  sqlo->pos = 0;
  wait_init (sqlo);
}
...
seqlock_val_t
seqlock_cur (struct seqlock *sqlo)
{
  return SEQLOCK_VAL (atomic_load_explicit (&sqlo->pos, memory_order_relaxed));
}

seqlock_val_t
seqlock_bump (struct seqlock *sqlo)
{
