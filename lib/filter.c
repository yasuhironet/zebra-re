...
static int
filter_match_cisco (struct filter *mfilter, const struct prefix *p)
{
  ...
  if (filter->extended)
    {
      ...
      if (memcmp (&check_addr, &filter->addr.s_addr, IPV4_MAX_BYTELEN) == 0 &&
          memcmp (&check_mask, &filter->mask.s_addr, IPV4_MAX_BYTELEN) == 0)
        return 1;
    }
  else if (memcmp (&check_addr, &filter->addr.s_addr, IPV4_MAX_BYTELEN) == 0)
    return 1;
...
/* If filter match to the prefix then return 1. */
static int
filter_match_zebra (struct filter *mfilter, const struct prefix *p)
{
...
enum filter_type
access_list_apply (struct access_list *access, const void *object)
{
  struct filter *filter;
...
int64_t
filter_new_seq_get (struct access_list *access)
{
  int64_t maxseq;
  int64_t newseq;
  struct filter *filter;
...
filter_seq_check (struct access_list *access, int64_t seq)
{
  struct filter *filter;
...
static const char *
filter_type2str (struct filter *filter)
{
  if (filter->cisco)
    {
...
static int
filter_show (struct vty *vty, const char *name, afi_t afi, bool use_json)
{
  struct access_list *access;
  struct access_master *master;
      ...
      for (mfilter = access->head; mfilter; mfilter = mfilter->next)
        {
              ...
              if (json)
                {
                  ...
                  json_object_object_add (json_proto, access->name, json_acl);

                  json_object_string_add (json_acl, "type", type);
                  json_object_string_add (json_acl, "addressFamily",
                                          afi2str (afi));
                  json_rules = json_object_new_array ();
                  json_object_object_add (json_acl, "rules", json_rules);
                }
              else
                {
          ...
          if (json)
            {
              ...
              json_object_int_add (json_rule, "sequenceNumber", mfilter->seq);
          ...
          if (! mfilter->cisco)
            config_write_access_zebra (vty, mfilter, json_rule);
          else if (filter->extended)
            config_write_access_cisco (vty, mfilter, json_rule);
          else
            {
              if (json)
                {
                  json_object_string_addf (json_rule, "address", "%pI4",
                                           &filter->addr);
                  json_object_string_addf (json_rule, "mask", "%pI4",
                                           &filter->addr_mask);
                }
              else
                {
  ...
  if (json)
  ...
  else
    {
      if (p->prefixlen == 0 && ! filter->exact)
        ...
        vty_out (vty, " %pFX%s", p, filter->exact ? " exact-match" : "");
...
static void
access_list_autocomplete_afi (afi_t afi, vector comps, struct cmd_token *token)
{
  struct access_list *access;
  struct access_list *next;
  struct access_master *master;
...
static void
access_list_autocomplete (vector comps, struct cmd_token *token)
{
...
static void
access_list4_autocomplete (vector comps, struct cmd_token *token)
{
  access_list_autocomplete_afi (AFI_IP, comps, token);
}

static void
access_list6_autocomplete (vector comps, struct cmd_token *token)
{
  access_list_autocomplete_afi (AFI_IP6, comps, token);
}

static void
access_list_mac_autocomplete (vector comps, struct cmd_token *token)
{
  access_list_autocomplete_afi (AFI_L2VPN, comps, token);
}

static const struct cmd_variable_handler access_list_handlers[] = {
  { .tokenname = "ACCESSLIST_NAME", .completions = access_list_autocomplete },
