...
const char *
zlog_priority_str (int priority)
{
...
void
log_show_syslog (struct vty *vty)
{
  ...
  if (level == ZLOG_DISABLED)
    ...
    vty_out (vty, "level %s, facility %s, ident %s\n", zlog_priority[level],
             facility_name (zlog_syslog_get_facility ()), zlog_progname);
}
            ...
            "Show current logging configuration\n")
{
  ...
  if (zt_file.prio_min == ZLOG_DISABLED || ! zt_file.filename)
    ...
    vty_out (vty, "level %s, filename %s", zlog_priority[zt_file.prio_min],
             zt_file.filename);
  vty_out (vty, "\n");
  ...
  if (log_cmdline_syslog_lvl != ZLOG_DISABLED)
    vty_out (vty, "From command line: \"--log syslog --log-level %s\"\n",
             zlog_priority[log_cmdline_syslog_lvl]);
  if (log_cmdline_stdout_lvl != ZLOG_DISABLED)
    vty_out (vty, "From command line: \"--log stdout --log-level %s\"\n",
             zlog_priority[log_cmdline_stdout_lvl]);
  if (zt_file_cmdline.prio_min != ZLOG_DISABLED)
    vty_out (vty, "From command line: \"--log file:%s --log-level %s\"\n",
...
static int
set_log_file (struct zlog_cfg_file *target, struct vty *vty, const char *fname,
              int loglevel)
{
...
void
command_setup_early_logging (const char *dest, const char *level)
{
  int nlevel;
  char *sep;
  int len;
...
void
log_config_write (struct vty *vty)
{
  ...
  if (zt_filterfile.parent.prio_min != ZLOG_DISABLED &&
      zt_filterfile.parent.filename)
    {
      vty_out (vty, "log filtered-file %s", zt_filterfile.parent.filename);

      if (zt_filterfile.parent.prio_min != log_default_lvl)
        vty_out (vty, " %s", zlog_priority[zt_filterfile.parent.prio_min]);
      vty_out (vty, "\n");
    }
  ...
  if (log_config_stdout_lvl != ZLOG_DISABLED)
    {
      ...
      if (log_config_stdout_lvl != log_default_lvl)
        vty_out (vty, " %s", zlog_priority[log_config_stdout_lvl]);
      vty_out (vty, "\n");
    }
  ...
  if (log_config_syslog_lvl != ZLOG_DISABLED)
    {
      ...
      if (log_config_syslog_lvl != log_default_lvl)
        vty_out (vty, " %s", zlog_priority[log_config_syslog_lvl]);
      vty_out (vty, "\n");
    }
  ...
  if (zt_file_cmdline.prio_min != ZLOG_DISABLED)
    {
      vty_out (
          vty, "! \"log file %s %s\" enabled by \"--log\" startup option\n",
          zt_file_cmdline.filename, zlog_priority[zt_file_cmdline.prio_min]);
      show_cmdline_hint = true;
    }
...
static int
log_vty_init (const char *progname, const char *protoname,
