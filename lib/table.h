...
#define l_right link[1]
  ...
  static inline void *
  route_table_get_info (struct route_table *table)
  {
    return table->info;
  }

  static inline void
  route_table_set_info (struct route_table *table, void *d)
  {
    table->info = d;
  }
  ...
  route_node_lookup_maynull (struct route_table *table,
                             union prefixconstptr pu);
  ...
  extern struct route_node *
  route_node_match_ipv6 (struct route_table *table,
  ...
  route_node_get_lock_count (struct route_node *node)
  {
    return node->lock;
  }
