...
struct if_rmap *
if_rmap_lookup (struct if_rmap_ctx *ctx, const char *ifname)
{
...
void
if_rmap_hook_add (struct if_rmap_ctx *ctx,
...
void
if_rmap_hook_delete (struct if_rmap_ctx *ctx,
                     void (*func) (struct if_rmap_ctx *ctx, struct if_rmap *))
{
...
static struct if_rmap *
if_rmap_get (struct if_rmap_ctx *ctx, const char *ifname)
{
  struct if_rmap key = { .ifname = ifname };
  struct if_rmap *ret;
...
static void
if_rmap_set (struct if_rmap_ctx *ctx, const char *ifname,
...
static void
if_rmap_unset (struct if_rmap_ctx *ctx, const char *ifname,
...
static int
if_route_map_handler (struct vty *vty, bool no, const char *dir,
  ...
  else
    {
      ...
      if (yang_dnode_existsf (
      ...
      else
        {
          ...
          snprintf (xpath, sizeof (xpath),
                    "./if-route-maps/if-route-map[interface='%s']", ifname);
        }
    }
...
DEFPY_YANG (no_if_ipv4_route_map,
...
DEFPY_YANG (no_if_ipv6_route_map,
...
void
cli_show_if_route_map (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
...
void
if_rmap_yang_modify_cb (struct if_rmap_ctx *ctx, const struct lyd_node *dnode,
...
void
if_rmap_yang_destroy_cb (struct if_rmap_ctx *ctx, const struct lyd_node *dnode)
{
  const char *ifname = yang_dnode_get_string (dnode, "interface");
...
void
if_rmap_ctx_delete (struct if_rmap_ctx *ctx)
{
