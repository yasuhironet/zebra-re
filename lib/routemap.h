...
/* route map maximum length. Not strictly the maximum xpath length but cannot
 * be greater
 */
...
#define IS_MATCH_INTERFACE(C) (strmatch (C, "frr-route-map:interface"))
...
#define IS_MATCH_METRIC(C) (strmatch (C, "frr-route-map:match-metric"))
...
#define IS_MATCH_ORIGIN(C) (strmatch (C, "frr-bgp-route-map:match-origin"))
...
#define IS_MATCH_PROBABILITY(C) (strmatch (C, "frr-bgp-route-map:probability"))
#define IS_MATCH_SRC_VRF(C)     (strmatch (C, "frr-bgp-route-map:source-vrf"))
#define IS_MATCH_PEER(C)        (strmatch (C, "frr-bgp-route-map:peer"))
#define IS_MATCH_AS_LIST(C)     (strmatch (C, "frr-bgp-route-map:as-path-list"))
...
#define IS_MATCH_EVPN_VNI(C) (strmatch (C, "frr-bgp-route-map:evpn-vni"))
#define IS_MATCH_EVPN_DEFAULT_ROUTE(C)                                        \
  (strmatch (C, "frr-bgp-route-map:evpn-default-route"))
#define IS_MATCH_EVPN_RD(C) (strmatch (C, "frr-bgp-route-map:evpn-rd"))
...
#define IS_MATCH_IPV4_NH(C) (strmatch (C, "frr-bgp-route-map:ipv4-nexthop"))
#define IS_MATCH_IPV6_NH(C) (strmatch (C, "frr-bgp-route-map:ipv6-nexthop"))

/* Route-map set actions */
#define IS_SET_IPv4_NH(A)     (strmatch (A, "frr-route-map:ipv4-next-hop"))
#define IS_SET_IPv6_NH(A)     (strmatch (A, "frr-route-map:ipv6-next-hop"))
#define IS_SET_METRIC(A)      (strmatch (A, "frr-route-map:set-metric"))
...
#define IS_SET_SR_TE_COLOR(A) (strmatch (A, "frr-route-map:set-sr-te-color"))
/* Zebra route-map set actions */
#define IS_SET_SRC(A) (strmatch (A, "frr-zebra-route-map:src-address"))
/* OSPF route-map set actions */
#define IS_SET_METRIC_TYPE(A) (strmatch (A, "frr-ospf-route-map:metric-type"))
#define IS_SET_FORWARDING_ADDR(A)                                             \
  (strmatch (A, "frr-ospf6-route-map:forwarding-address"))
/* BGP route-map_set actions */
#define IS_SET_WEIGHT(A) (strmatch (A, "frr-bgp-route-map:weight"))
...
#define IS_SET_LABEL_INDEX(A) (strmatch (A, "frr-bgp-route-map:label-index"))
#define IS_SET_DISTANCE(A)    (strmatch (A, "frr-bgp-route-map:distance"))
#define IS_SET_ORIGIN(A)      (strmatch (A, "frr-bgp-route-map:set-origin"))
...
#define IS_SET_COMMUNITY(A) (strmatch (A, "frr-bgp-route-map:set-community"))
...
#define IS_SET_AGGREGATOR(A) (strmatch (A, "frr-bgp-route-map:aggregator"))
...
#define route_map_install_set(c)                                              \
  ...
  struct route_map *route_map_lookup_warn_noexist (struct vty *vty,
                                                   const char *name);
...
#define route_map_apply(map, prefix, object)                                  \
  ...
  extern void route_map_upd8_dependency (route_map_event_t type,
                                         const char *arg,
  ...
  extern void route_map_no_set_ipv6_nexthop_local_hook (
      int (*func) (struct route_map_index *index, const char *command,
                   const char *arg, char *errmsg, size_t errmsg_len));
  /* set metric */
  extern void route_map_set_metric_hook (
      int (*func) (struct route_map_index *index, const char *command,
                   const char *arg, char *errmsg, size_t errmsg_len));
  /* no set metric */
  ...
  extern void route_map_no_set_min_metric_hook (
      int (*func) (struct route_map_index *index, const char *command,
                   const char *arg, char *errmsg, size_t errmsg_len));
  /* set tag */
  extern void route_map_set_tag_hook (
      int (*func) (struct route_map_index *index, const char *command,
                   const char *arg, char *errmsg, size_t errmsg_len));
  /* no set tag */
  extern void route_map_no_set_tag_hook (
      int (*func) (struct route_map_index *index, const char *command,
                   const char *arg, char *errmsg, size_t errmsg_len));
  ...
  {
    /* match interface */
    int (*match_interface) (struct route_map_index *index, const char *command,
    ...
    /* no match ipv6 next-hop prefix-list */
    int (*no_match_ipv6_next_hop_prefix_list) (
        struct route_map_index *index, const char *command, const char *arg,
        route_map_event_t type, char *errmsg, size_t errmsg_len);
    ...
    /* match metric */
    int (*match_metric) (struct route_map_index *index, const char *command,
                         const char *arg, route_map_event_t type, char *errmsg,
                         size_t errmsg_len);

    /* no match metric */
    int (*no_match_metric) (struct route_map_index *index, const char *command,
                            const char *arg, route_map_event_t type,
                            char *errmsg, size_t errmsg_len);

    /* match tag */
    int (*match_tag) (struct route_map_index *index, const char *command,
                      const char *arg, route_map_event_t type, char *errmsg,
                      size_t errmsg_len);

    /* no match tag */
    int (*no_match_tag) (struct route_map_index *index, const char *command,
                         const char *arg, route_map_event_t type, char *errmsg,
                         size_t errmsg_len);

    /* set sr-te color */
    int (*set_srte_color) (struct route_map_index *index, const char *command,
                           const char *arg, char *errmsg, size_t errmsg_len);
    ...
    /* set ip nexthop */
    int (*set_ip_nexthop) (struct route_map_index *index, const char *command,
                           const char *arg, char *errmsg, size_t errmsg_len);
    ...
    /* set metric */
    int (*set_metric) (struct route_map_index *index, const char *command,
                       const char *arg, char *errmsg, size_t errmsg_len);

    /* no set metric */
    int (*no_set_metric) (struct route_map_index *index, const char *command,
                          const char *arg, char *errmsg, size_t errmsg_len);
    /* set min-metric */
    int (*set_min_metric) (struct route_map_index *index, const char *command,
                           const char *arg, char *errmsg, size_t errmsg_len);
    ...
    /* set max-metric */
    int (*set_max_metric) (struct route_map_index *index, const char *command,
                           const char *arg, char *errmsg, size_t errmsg_len);
    ...
    /* set tag */
    int (*set_tag) (struct route_map_index *index, const char *command,
                    const char *arg, char *errmsg, size_t errmsg_len);

    /* no set tag */
    int (*no_set_tag) (struct route_map_index *index, const char *command,
                       const char *arg, char *errmsg, size_t errmsg_len);
  };
  ...
  extern struct route_map_index *route_map_index_get (struct route_map *map,
                                                      enum route_map_type type,
                                                      int pref);
  extern void route_map_index_delete (struct route_map_index *index,
                                      int notify);
  ...
  extern void route_map_condition_show (struct vty *vty,
                                        const struct lyd_node *dnode,
                                        bool show_defaults);
  extern void route_map_action_show (struct vty *vty,
                                     const struct lyd_node *dnode,
                                     bool show_defaults);
  extern void route_map_exit_policy_show (struct vty *vty,
                                          const struct lyd_node *dnode,
                                          bool show_defaults);
  extern void route_map_call_show (struct vty *vty,
                                   const struct lyd_node *dnode,
                                   bool show_defaults);
  extern void route_map_description_show (struct vty *vty,
                                          const struct lyd_node *dnode,
                                          bool show_defaults);
  extern void route_map_optimization_disabled_show (
