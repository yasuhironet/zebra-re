  ...
  if (zprivs_state.syscaps_i && zprivs_state.syscaps_i->num)
    {
      cap_set_flag (zprivs_state.caps, CAP_INHERITABLE,
                    zprivs_state.syscaps_i->num, zprivs_state.syscaps_i->caps,
                    CAP_SET);
    }
 ...
 */
static struct zebra_privs_refs_t *
get_privs_refs (struct zebra_privs_t *privs)
{
  ...
  if (privs_per_process)
  ...
  else
    {
      ...
      if (refs == NULL)
        {
          refs = XCALLOC (MTYPE_PRIVS, sizeof (struct zebra_privs_refs_t));
          refs->tid = tid;
...
_zprivs_raise (struct zebra_privs_t *privs, const char *funcname)
{
      ...
      if (++(refs->refcount) == 1)
        {
          ...
          if (privs->change (ZPRIVS_RAISE))
            {
              zlog_err ("%s: Failed to raise privileges (%s)", funcname,
                        safe_strerror (errno));
            }
          errno = save_errno;
...
void
_zprivs_lower (struct zebra_privs_t **privs)
{
  int save_errno = errno;
  struct zebra_privs_refs_t *refs;
          ...
          if ((*privs)->change (ZPRIVS_LOWER))
            {
              zlog_err ("%s: Failed to lower privileges (%s)",
                        refs->raised_in_funcname, safe_strerror (errno));
            }
          errno = save_errno;
...
void
zprivs_preinit (struct zebra_privs_t *zprivs)
{
