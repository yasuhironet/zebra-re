...
#define IS_MPLS_UNRESERVED_LABEL(label)                                       \
  (label >= MPLS_LABEL_UNRESERVED_MIN && label <= MPLS_LABEL_UNRESERVED_MAX)
...
#define MPLS_INVALID_LABEL 0xFFFDFFFF
  ...
  static inline vni_t
  label2vni (const mpls_label_t *label)
  {
    uint8_t *tag = (uint8_t *) label;
    vni_t vni;
  ...
  mpls_lse_encode (mpls_label_t label, uint32_t ttl, uint32_t exp,
                   uint32_t bos)
  {
    mpls_lse_t lse;
   ...
   */
  static inline void
  mpls_lse_decode (mpls_lse_t lse, mpls_label_t *label, uint32_t *ttl,
...
#define MPLS_INVALID_LABEL_INDEX 0xFFFFFFFF
  ...
  static inline char *
  label2str (mpls_label_t label, enum lsp_types_t type, char *buf, size_t len)
  {
...
#define MPLS_LABEL_STRLEN 1024
   ...
   */
  char *mpls_label2str (uint8_t num_labels, const mpls_label_t *labels,
                        char *buf, int len, enum lsp_types_t type, int pretty);
