...
setsockopt_so_recvbuf (int sock, int size)
{
  ...
  if (size != orig_req)
    flog_err (EC_LIB_SOCKET, "%s: fd %d: SO_RCVBUF set to %d (requested %d)",
...
setsockopt_so_sendbuf (const int sock, int size)
{
  ...
  if (size != orig_req)
    flog_err (EC_LIB_SOCKET, "%s: fd %d: SO_SNDBUF set to %d (requested %d)",
              __func__, sock, size, orig_req);
}
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "fd %d: can't getsockopt SO_SNDBUF: %d (%s)", sock, errno,
                    safe_strerror (errno));
      return ret;
    }
  return optval;
...
int
getsockopt_so_recvbuf (const int sock)
{
  uint32_t optval;
  socklen_t optlen = sizeof (optval);
  int ret =
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "fd %d: can't getsockopt SO_RCVBUF: %d (%s)", sock, errno,
                    safe_strerror (errno));
      return ret;
    }
  return optval;
...
setsockopt_ipv6_pktinfo (int sock, int val)
{
  ...
  if (ret < 0)
    flog_err (EC_LIB_SOCKET, "can't setsockopt IPV6_RECVPKTINFO : %s",
              safe_strerror (errno));
#else  /*RFC2292*/
...
setsockopt_ipv6_tclass (int sock, int tclass)
{
  ...
  if (ret < 0)
    flog_err (EC_LIB_SOCKET,
              "Can't set IPV6_TCLASS option for fd %d to %#x: %s", sock,
              tclass, safe_strerror (errno));
#endif
  return ret;
}
 ...
 */
int
setsockopt_ipv4_multicast (int sock, int optname, struct in_addr if_addr,
                           unsigned int mcast_addr, ifindex_t ifindex)
{
  ...
  ret =
      setsockopt (sock, IPPROTO_IP, optname, (void *) &mreqn, sizeof (mreqn));
  if ((ret < 0) && (optname == IP_ADD_MEMBERSHIP) && (errno == EADDRINUSE))
    {
      ...
      setsockopt (sock, IPPROTO_IP, IP_DROP_MEMBERSHIP, (void *) &mreqn,
                  sizeof (mreqn));
      ret = setsockopt (sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *) &mreqn,
                        sizeof (mreqn));
    }
  return ret;
...
int
setsockopt_ipv4_multicast_loop (int sock, uint8_t val)
{
  int ret;
...
setsockopt_ipv4_ifindex (int sock, ifindex_t val)
{
  ...
  if (ret < 0)
    flog_err (EC_LIB_SOCKET, "Can't set IP_PKTINFO option for fd %d to %d: %s",
              sock, val, safe_strerror (errno));
#elif defined(IP_RECVIF)
  ret = setsockopt (sock, IPPROTO_IP, IP_RECVIF, &val, sizeof (val));
  if (ret < 0)
    flog_err (EC_LIB_SOCKET, "Can't set IP_RECVIF option for fd %d to %d: %s",
              sock, val, safe_strerror (errno));
#else
...
setsockopt_ipv4_tos (int sock, int tos)
{
  ...
  if (ret < 0)
    flog_err (EC_LIB_SOCKET, "Can't set IP_TOS option for fd %d to %#x: %s",
...
int
sockopt_tcp_signature_ext (int sock, union sockunion *su, uint16_t prefixlen,
                           const char *password)
{
  ...
  if (ret < 0)
    {
      if (ENOENT == errno)
        ...
        flog_err_sys (EC_LIB_SYSTEM_CALL,
                      "sockopt_tcp_signature: setsockopt(%d): %s", sock,
                      safe_strerror (errno));
    }
  return ret;
...
int
sockopt_tcp_mss_set (int sock, int tcp_maxseg)
{
  int ret = 0;
  socklen_t tcp_maxseg_len = sizeof (tcp_maxseg);

  ret =
      setsockopt (sock, IPPROTO_TCP, TCP_MAXSEG, &tcp_maxseg, tcp_maxseg_len);
  if (ret != 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL, "%s failed: setsockopt(%d): %s",
...
sockopt_tcp_mss_get (int sock)
{
  ...
  ret =
      getsockopt (sock, IPPROTO_TCP, TCP_MAXSEG, &tcp_maxseg, &tcp_maxseg_len);
  if (ret != 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL, "%s failed: getsockopt(%d): %s",
                    __func__, sock, safe_strerror (errno));
      return 0;
    }
...
int
setsockopt_tcp_keepalive (int sock, uint16_t keepalive_idle,
                          uint16_t keepalive_intvl, uint16_t keepalive_probes)
{
  ...
  if (setsockopt (sock, SOL_SOCKET, SO_KEEPALIVE, &val, sizeof (val)) < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "%s failed: setsockopt SO_KEEPALIVE (%d): %s", __func__,
  ...
  if (setsockopt (sock, IPPROTO_TCP, TCP_KEEPIDLE, &val, sizeof (val)) < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "%s failed: setsockopt TCP_KEEPIDLE (%d): %s", __func__,
                    sock, safe_strerror (errno));
      return -1;
    }
  ...
  if (setsockopt (sock, IPPROTO_TCP, TCP_KEEPINTVL, &val, sizeof (val)) < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "%s failed: setsockopt TCP_KEEPINTVL (%d): %s", __func__,
                    sock, safe_strerror (errno));
      return -1;
    }
  ...
  if (setsockopt (sock, IPPROTO_TCP, TCP_KEEPCNT, &val, sizeof (val)) < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "%s failed: setsockopt TCP_KEEPCNT (%d): %s", __func__,
                    sock, safe_strerror (errno));
      return -1;
    }
