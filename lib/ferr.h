...
extern "C"
{
   ...
   *
   * guaranteed to have boolean evaluation to "false" when OK, "true" when
   * error (i.e. can be changed to pointer in the future if necessary)
   *
...
/* example: uses bogus #define to keep indent.py happy */
  ...
  ferr_r
  foo_bar_set (struct object *obj, int bar)
  {
    if (bar < 1 || bar >= 100)
