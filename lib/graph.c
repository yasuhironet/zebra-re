...
static void
graph_vector_remove (vector v, unsigned int ix)
{
  if (ix >= v->active)
    return;
...
struct graph_node *
graph_find_node (struct graph *graph, void *data)
{
  struct graph_node *g;
...
bool
graph_has_edge (struct graph_node *from, struct graph_node *to)
{
  for (unsigned int i = vector_active (from->to); i--; /**/)
    if (vector_slot (from->to, i) == to)
      return true;
...
static void
_graph_dfs (struct graph *graph, struct graph_node *start, vector visited,
...
void
graph_dfs (struct graph *graph, struct graph_node *start,
           void (*dfs_cb) (struct graph_node *, void *), void *arg)
{
...
void
graph_dump_dot_default_print_cb (struct graph_node *gn, struct buffer *buf)
{
  char nbuf[64];
...
char *
graph_dump_dot (struct graph *graph, struct graph_node *start,
