...
#define bf_init(v, N)                                                         \
  do                                                                          \
    {                                                                         \
      ...
      (v).m = ((N) / WORD_SIZE + 1);                                          \
      (v).data =                                                              \
          (word_t *) XCALLOC (MTYPE_BITFIELD, ((v).m * sizeof (word_t)));     \
    }                                                                         \
  while (0)
...
#define bf_cmp(v1, v2)                                                        \
  (memcmp ((v1).data, (v2).data, ((v1).m * sizeof (word_t))))
...
#define bf_find_bit(v, b)                                                     \
  ...
  bf_find_next_clear_bit_wrap (bitfield_t *v, word_t start_index,
                               word_t max_index)
  {
    int start_bit;
  ...
  static inline unsigned int
  bf_find_next_set_bit (bitfield_t v, word_t start_index)
  {
    int start_bit;
