...
static int
rmap_cmd_name_cmp (const struct route_map_rule_cmd_proxy *a,
...
static uint32_t
rmap_cmd_name_hash (const struct route_map_rule_cmd_proxy *item)
{
...
/* match interface */
void
route_map_match_interface_hook (int (*func) (
...
/* no match interface */
void
route_map_no_match_interface_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
/* match ip address */
void
route_map_match_ip_address_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
/* no match ip address */
void
route_map_no_match_ip_address_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_match_ip_address_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ip_address_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
/* match ip next hop */
void
route_map_match_ip_next_hop_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ip_next_hop_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
/* match ipv6 next-hop */
void
route_map_match_ipv6_next_hop_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ipv6_next_hop_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_match_ip_next_hop_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ip_next_hop_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_match_ip_next_hop_type_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ip_next_hop_type_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
/* match ipv6 address */
void
route_map_match_ipv6_address_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ipv6_address_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_match_ipv6_address_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ipv6_address_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_match_ipv6_next_hop_type_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ipv6_next_hop_type_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_match_ipv6_next_hop_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
void
route_map_no_match_ipv6_next_hop_prefix_list_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
/* match metric */
void
route_map_match_metric_hook (int (*func) (struct route_map_index *index,
...
/* no match metric */
void
route_map_no_match_metric_hook (int (*func) (
    struct route_map_index *index, const char *command, const char *arg,
    route_map_event_t type, char *errmsg, size_t errmsg_len))
{
...
/* match tag */
void
route_map_match_tag_hook (int (*func) (struct route_map_index *index,
                                       const char *command, const char *arg,
                                       route_map_event_t type, char *errmsg,
                                       size_t errmsg_len))
{
...
/* no match tag */
void
route_map_no_match_tag_hook (int (*func) (struct route_map_index *index,
                                          const char *command, const char *arg,
                                          route_map_event_t type, char *errmsg,
                                          size_t errmsg_len))
{
...
/* set sr-te color */
void
route_map_set_srte_color_hook (int (*func) (struct route_map_index *index,
                                            const char *command,
...
/* no set sr-te color */
void
route_map_no_set_srte_color_hook (int (*func) (struct route_map_index *index,
                                               const char *command,
                                               const char *arg, char *errmsg,
                                               size_t errmsg_len))
{
...
/* set ip nexthop */
void
route_map_set_ip_nexthop_hook (int (*func) (struct route_map_index *index,
                                            const char *command,
                                            const char *arg, char *errmsg,
                                            size_t errmsg_len))
{
...
/* no set ip nexthop */
void
route_map_no_set_ip_nexthop_hook (int (*func) (struct route_map_index *index,
                                               const char *command,
                                               const char *arg, char *errmsg,
                                               size_t errmsg_len))
{
...
void
route_map_set_ipv6_nexthop_local_hook (
...
void
route_map_no_set_ipv6_nexthop_local_hook (
    int (*func) (struct route_map_index *index, const char *command,
                 const char *arg, char *errmsg, size_t errmsg_len))
{
...
/* set metric */
void
route_map_set_metric_hook (int (*func) (struct route_map_index *index,
                                        const char *command, const char *arg,
...
/* no set metric */
void
route_map_no_set_metric_hook (int (*func) (struct route_map_index *index,
                                           const char *command,
                                           const char *arg, char *errmsg,
                                           size_t errmsg_len))
{
...
/* set min-metric */
void
route_map_set_min_metric_hook (int (*func) (struct route_map_index *index,
                                            const char *command,
                                            const char *arg, char *errmsg,
                                            size_t errmsg_len))
{
...
/* no set min-metric */
void
route_map_no_set_min_metric_hook (int (*func) (struct route_map_index *index,
                                               const char *command,
                                               const char *arg, char *errmsg,
                                               size_t errmsg_len))
{
...
/* set max-metric */
void
route_map_set_max_metric_hook (int (*func) (struct route_map_index *index,
                                            const char *command,
                                            const char *arg, char *errmsg,
                                            size_t errmsg_len))
{
...
/* no set max-metric */
void
route_map_no_set_max_metric_hook (int (*func) (struct route_map_index *index,
                                               const char *command,
                                               const char *arg, char *errmsg,
                                               size_t errmsg_len))
{
...
/* set tag */
void
route_map_set_tag_hook (int (*func) (struct route_map_index *index,
                                     const char *command, const char *arg,
                                     char *errmsg, size_t errmsg_len))
{
...
/* no set tag */
void
route_map_no_set_tag_hook (int (*func) (struct route_map_index *index,
                                        const char *command, const char *arg,
                                        char *errmsg, size_t errmsg_len))
{
...
int
generic_match_add (struct route_map_index *index, const char *command,
                   const char *arg, route_map_event_t type, char *errmsg,
                   size_t errmsg_len)
{
...
int
generic_match_delete (struct route_map_index *index, const char *command,
                      const char *arg, route_map_event_t type, char *errmsg,
                      size_t errmsg_len)
{
  enum rmap_compile_rets ret;
...
int
generic_set_add (struct route_map_index *index, const char *command,
                 const char *arg, char *errmsg, size_t errmsg_len)
{
  enum rmap_compile_rets ret;
...
int
generic_set_delete (struct route_map_index *index, const char *command,
                    const char *arg, char *errmsg, size_t errmsg_len)
{
  enum rmap_compile_rets ret;
...
static unsigned int
route_map_hash_key_make (const void *p)
{
...
struct route_map *
route_map_lookup_warn_noexist (struct vty *vty, const char *name)
{
...
int
route_map_mark_updated (const char *name)
{
  struct route_map *map;
  int ret = -1;
  struct route_map tmp_map;
...
void
route_map_walk_update_list (void (*route_map_update_fn) (char *name))
{
...
static const char *
route_map_cmd_result_str (enum route_map_cmd_result_t res)
{
  switch (res)
    {
    case RMAP_MATCH:
...
static const char *
route_map_result_str (route_map_result_t res)
{
  switch (res)
    {
    case RMAP_DENYMATCH:
      ...
      if (json)
        {
          ...
          json_object_int_add (json_rule, "sequenceNumber", index->pref);
          ...
          json_object_object_add (json_rule, "matchClauses", json_matches);
          for (rule = index->match_list.head; rule; rule = rule->next)
            {
              char buf[BUFSIZ];

              snprintf (buf, sizeof (buf), "%s %s", rule->cmd->str,
                        rule->rule_str);
          ...
          json_object_object_add (json_rule, "setClauses", json_sets);
          ...
          /* Call clause */
          if (index->nextrm)
            json_object_string_add (json_rule, "callClause", index->nextrm);

          /* Exit Policy */
          if (index->exitpolicy == RMAP_GOTO)
            snprintf (action, sizeof (action), "Goto %d", index->nextpref);
          else if (index->exitpolicy == RMAP_NEXT)
            snprintf (action, sizeof (action), "Continue to next entry");
          else if (index->exitpolicy == RMAP_EXIT)
            snprintf (action, sizeof (action), "Exit routemap");
          if (action[0] != '\0')
            json_object_string_add (json_rule, "action", action);
        }
      else
        {
...
static int
sort_route_map (const void **map1, const void **map2)
{
...
static int
vty_show_route_map (struct vty *vty, const char *name, bool use_json)
{
  struct route_map *map;
...
static int
vty_show_unused_route_map (struct vty *vty)
{
  struct list *maplist = list_new ();
  struct listnode *ln;
  struct route_map *map;
...
route_map_index_delete (struct route_map_index *index, int notify)
{
  ...
  if (unlikely (CHECK_FLAG (rmap_debug, DEBUG_ROUTEMAP)))
    zlog_debug ("Deleting route-map %s sequence %d", index->map->name,
                index->pref);
  ...
  while ((rule = index->match_list.head) != NULL)
    {
      if (IS_RULE_IPv4_PREFIX_LIST (rule->cmd->str))
        route_map_pfx_tbl_update (RMAP_EVENT_PLIST_DELETED, index, AFI_IP,
                                  rule->rule_str);
      else if (IS_RULE_IPv6_PREFIX_LIST (rule->cmd->str))
        route_map_pfx_tbl_update (RMAP_EVENT_PLIST_DELETED, index, AFI_IP6,
                                  rule->rule_str);
...
route_map_index_add (struct route_map *map, enum route_map_type type, int pref)
{
  ...
  if (unlikely (CHECK_FLAG (rmap_debug, DEBUG_ROUTEMAP)))
    zlog_debug ("Route-map %s add sequence %d, type: %s", map->name, pref,
...
void
_route_map_install_match (struct route_map_rule_cmd_proxy *proxy)
{
...
void
_route_map_install_set (struct route_map_rule_cmd_proxy *proxy)
{
                     ...
                     const char *match_arg, route_map_event_t type)
{
  ...
  if (compile && cmd->func_get_rmap_rule_key)
    rule_key = (*cmd->func_get_rmap_rule_key) (compile);
  else
    rule_key = match_arg;
      ...
      if (rule->cmd == cmd)
        {
          ...
          if (type >= RMAP_EVENT_CALL_ADDED)
            {
              delete_rmap_event_type = get_route_map_delete_event (type);
              route_map_upd8_dependency (delete_rmap_event_type,
                                         rule->rule_str, index->map->name);
            }
    ...
    if (rule->cmd == cmd &&
        (rulecmp (rule->rule_str, match_arg) == 0 || match_arg == NULL))
      {
        ...
        if (cmd->func_get_rmap_rule_key)
          rule_key = (*cmd->func_get_rmap_rule_key) (rule->value);
        else
          rule_key = match_arg;
                     ...
                     void *object, enum route_map_cmd_result_t *match_ret)
{
  ...
  do
    {
      candidate_rmap_list = route_map_get_index_list (&rn, prefix, table);
      if (! rn)
        break;
      ...
      if (best_index && head_index && (best_index->pref < head_index->pref))
        {
      ...
      for (ALL_LIST_ELEMENTS (candidate_rmap_list, ln, nn, index))
        {
          ...
          ret = route_map_apply_match (&index->match_list, prefix, object);
 ...
 */
static bool
route_map_is_ip_pfx_list_rule_present (struct route_map_index *index)
{
 ...
 */
static void
route_map_add_plist_entries (afi_t afi, struct route_map_index *index,
                             const char *plist_name,
                             struct prefix_list_entry *entry)
{
  struct route_map_rule_list *match_list = NULL;
  ...
  if (entry)
    {
      if (afi == AFI_IP)
        {
          route_map_pfx_table_add (index->map->ipv4_prefix_table, index,
                                   entry);
        }
      else
        {
          route_map_pfx_table_add (index->map->ipv6_prefix_table, index,
                                   entry);
        }
    }
  else
 ...
 */
static void
route_map_del_plist_entries (afi_t afi, struct route_map_index *index,
                             const char *plist_name,
                             struct prefix_list_entry *entry)
{
  struct route_map_rule_list *match_list = NULL;
  ...
  if (entry)
    {
      if (afi == AFI_IP)
        {
          route_map_pfx_table_del (index->map->ipv4_prefix_table, index,
                                   entry);
        }
      else
        {
          route_map_pfx_table_del (index->map->ipv6_prefix_table, index,
                                   entry);
        }
    }
  else
 ...
 */
static void
route_map_trie_update (afi_t afi, route_map_event_t event,
                       struct route_map_index *index, const char *plist_name)
{
  if (event == RMAP_EVENT_PLIST_ADDED)
          ...
          if (! route_map_is_ipv6_pfx_list_rule_present (index))
            {
              ...
              route_map_add_plist_entries (afi, index, plist_name, NULL);
            }
          else
            {
  ...
  else if (event == RMAP_EVENT_PLIST_DELETED)
    {
      if (afi == AFI_IP)
        {
          route_map_del_plist_entries (afi, index, plist_name, NULL);
  ...
  if (event == RMAP_EVENT_PLIST_ADDED)
    {
      if (afi == AFI_IP)
        {
          if (! route_map_is_ipv6_pfx_list_rule_present (index))
            route_map_add_plist_entries (afi, index, plist_name, pentry);
        }
      else
        {
          if (! route_map_is_ip_pfx_list_rule_present (index))
...
static void
route_map_pentry_process_dependency (struct hash_bucket *bucket, void *data)
{
  char *rmap_name = NULL;
  ...
  for (index = rmap->head; index; index = index->next)
    {
      ...
      for (match = match_list->head; match; match = match->next)
        {
          if (strcmp (match->rule_str, pentry_dep->plist_name) == 0)
            {
              if (IS_RULE_IPv4_PREFIX_LIST (match->cmd->str) &&
                  family == AF_INET)
                {
                  route_map_pentry_update (pentry_dep->event,
                                           pentry_dep->plist_name, index,
                                           pentry_dep->pentry);
                }
              else if (IS_RULE_IPv6_PREFIX_LIST (match->cmd->str) &&
                       family == AF_INET6)
                {
                  route_map_pentry_update (pentry_dep->event,
                                           pentry_dep->plist_name, index,
  ...
  if (dep)
    {
      ...
      hash_iterate (dep->dep_rmap_hash, route_map_pentry_process_dependency,
                    (void *) &pentry_dep);
    }
}
...
/* Apply route map's each index to the object.
   ...
   case we
     ...
     on-match next    - If this clause is matched, then the set statements
                        are executed and then we drop through to the next
   clause on-match goto n  - If this clause is matched, then the set statements
...
*/
route_map_result_t
route_map_apply_ext (struct route_map *map, const struct prefix *prefix,
                     void *match_object, void *set_object, int *pref)
{
  ...
  if (recursion > RMAP_RECURSION_LIMIT)
    {
      ...
      flog_warn (EC_LIB_RMAP_RECURSION_LIMIT,
  ...
  else
    {
      if (unlikely (CHECK_FLAG (rmap_debug, DEBUG_ROUTEMAP)))
        zlog_debug (
            ...
            prefix, map->name, route_map_cmd_result_str (match_ret));
      /*
      ...
      if (! skip_match_clause)
        {
          ...
          /* Apply this index. */
          match_ret =
              route_map_apply_match (&index->match_list, prefix, match_object);
          if (unlikely (CHECK_FLAG (rmap_debug, DEBUG_ROUTEMAP)))
            {
              zlog_debug (
          ...
          if (index->type == RMAP_PERMIT)
            /* 'action' */
            {
              ...
              for (set = index->set_list.head; set; set = set->next)
                 ...
                 * return code.
                 */
                (void) (*set->cmd->func_apply) (set->value, prefix,
                                                set_object);
              ...
              if (index->nextrm)
                {
                  ...
                  if (nextrm) /* Target route-map found,
                    ...
                    {
                      recursion++;
                      ret = route_map_apply_ext (nextrm, prefix, match_object,
                                                 set_object, NULL);
                      recursion--;
                    }
...
void
route_map_event_hook (void (*func) (const char *name))
{
...
route_map_rmap_hash_cmp (const void *p1, const void *p2)
{
  return strcmp (((const struct route_map_dep_data *) p1)->rname,
                 ((const struct route_map_dep_data *) p2)->rname) == 0;
}
...
static void
route_map_clear_reference (struct hash_bucket *bucket, void *arg)
{
  ...
  if (dep_data)
    {
      if (unlikely (CHECK_FLAG (rmap_debug, DEBUG_ROUTEMAP)))
        zlog_debug ("Clearing reference for %s to %s count: %d", dep->dep_name,
                    tmp_dep_data.rname, dep_data->refcnt);
...
static unsigned int
route_map_dep_data_hash_make_key (const void *p)
{
...
route_map_name_hash_alloc (void *p)
{
  ...
  dep_data =
      XCALLOC (MTYPE_ROUTE_MAP_DEP_DATA, sizeof (struct route_map_dep_data));
  tmp_dep_data = p;
...
static unsigned int
route_map_dep_hash_make_key (const void *p)
{
  return (string_hash_make ((char *) p));
}

static void
route_map_print_dependency (struct hash_bucket *bucket, void *data)
{
                      ...
                      const char *rmap_name, route_map_event_t type)
{
  ...
  switch (type)
    ...
    case RMAP_EVENT_FILTER_DELETED:
      ...
      if (! dep_data->refcnt)
        {
          ret_dep_data = hash_release (dep->dep_rmap_hash, &tmp_dep_data);
          if (ret_dep_data)
            {
              XFREE (MTYPE_ROUTE_MAP_NAME, ret_dep_data->rname);
...
static void
route_map_process_dependency (struct hash_bucket *bucket, void *data)
{
  struct route_map_dep_data *dep_data = NULL;
  char *rmap_name = NULL;
...
void
route_map_notify_dependencies (const char *affected_name,
                               route_map_event_t event)
{
...
static void
clear_route_map_helper (struct route_map *map)
{
  struct route_map_index *index;
...
void
route_map_show_debug (struct vty *vty)
{
  if (CHECK_FLAG (rmap_debug, DEBUG_ROUTEMAP))
...
static int
rmap_config_write_debug (struct vty *vty)
{
  int write = 0;
...
void
route_map_counter_increment (struct route_map *map)
{
  if (map)
    map->use_count++;
}
...
void
route_map_counter_decrement (struct route_map *map)
{
  if (map)
    {
      if (map->use_count <= 0)
      ...
      if (rm_pfx_tbl4)
        {
          ...
          for (rn = route_top (rm_pfx_tbl4); rn; rn = route_next (rn))
            {
              vty_out (vty, "    %pRN (%d)\n", rn,
              ...
              if (! rmap_index_list || ! listcount (rmap_index_list))
                vty_out (vty, "%*s%s\n", len, "", "-");
              else
                for (ALL_LIST_ELEMENTS (rmap_index_list, ln, nln, index))
                  {
      ...
      if (rm_pfx_tbl6)
        {
          ...
          vty_out (vty, "%s%39s%s\n", "_______________", "",
                   "____________________");
          for (rn = route_top (rm_pfx_tbl6); rn; rn = route_next (rn))
            {
              vty_out (vty, "    %pRN (%d)\n", rn,
                       route_node_get_lock_count (rn));
...
route_map_init (void)
{
  ...
  route_map_master_hash = hash_create_size (
      8, route_map_hash_key_make, route_map_hash_cmp, "Route Map Master Hash");

  for (i = 1; i < ROUTE_MAP_DEP_MAX; i++)
    route_map_dep_hash[i] =
        hash_create_size (8, route_map_dep_hash_make_key,
                          route_map_dep_hash_cmp, "Route Map Dep Hash");
