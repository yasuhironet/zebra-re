 ...
 * listening on for vty connection (either TCP or Unix socket based). The
 * daemon is responsible for calling frr_vty_serv() itself.
 */
...
#define FRR_DAEMON_INFO(execname, constname, ...)                             \
  static struct frr_daemon_info execname##_di = { .name = #execname,          \
                                                  ...
                                                  .module = THIS_MODULE,      \
                                                  __VA_ARGS__ };              \
  FRR_COREMOD_SETUP (.name = #execname, .description = #execname " daemon",   \
                     .version = FRR_VERSION, );                               \
  MACRO_REQUIRE_SEMICOLON () /* end */

  extern void frr_init_vtydir (void);
  extern void frr_preinit (struct frr_daemon_info *daemon, int argc,
                           char **argv);
