...
static void
zlog_live (struct zlog_target *zt, struct zlog_msg *msgs[], size_t nmsgs)
{
  ...
  for (i = 0; i < nmsgs; i++)
    {
      ...
      hdr->tid = tid;
      hdr->lost_msgs =
          atomic_load_explicit (&zte->lost_msgs, memory_order_relaxed);
      hdr->prio = prio;
      hdr->flags = 0;
      hdr->textlen = textlen;
      ...
      if (xref)
        {
          memcpy (hdr->uid, xref->xref.xrefdata->uid, sizeof (hdr->uid));
          hdr->ec = xref->ec;
        }
      else
        {
  ...
  for (size_t msgpos = 0; msgpos < msgtotal; msgpos += sent)
    {
      ...
      if (sent <= 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
        {
          atomic_fetch_add_explicit (&zte->lost_msgs, msgtotal - msgpos,
...
static void
zlog_live_sigsafe (struct zlog_target *zt, const char *text, size_t len)
{
  struct zlt_live *zte = container_of (zt, struct zlt_live, zt);
...
void
zlog_live_open (struct zlog_live_cfg *cfg, int prio_min, int *other_fd)
{
  int sockets[2];
...
void
zlog_live_open_fd (struct zlog_live_cfg *cfg, int prio_min, int fd)
{
  struct zlt_live *zte;
  struct zlog_target *zt;
...
void
zlog_live_close (struct zlog_live_cfg *cfg)
{
  struct zlt_live *zte;
  int fd;
...
void
zlog_live_disown (struct zlog_live_cfg *cfg)
{
  struct zlt_live *zte;
  uint_fast32_t state;
