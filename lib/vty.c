...
static int
vty_mgmt_lock_candidate_inline (struct vty *vty)
{
...
static int
vty_mgmt_unlock_candidate_inline (struct vty *vty)
{
  assert (vty->mgmt_locked_candidate_ds);
...
static int
vty_mgmt_lock_running_inline (struct vty *vty)
{
...
static int
vty_mgmt_unlock_running_inline (struct vty *vty)
{
  assert (vty->mgmt_locked_running_ds);
...
void
vty_mgmt_resume_response (struct vty *vty, bool success)
{
...
void
vty_frame (struct vty *vty, const char *format, ...)
{
  va_list args;
...
void
vty_endframe (struct vty *vty, const char *endtext)
{
...
bool
vty_set_include (struct vty *vty, const char *regexp)
{
  int errcode;
  bool ret = true;
  char errbuf[256];
  ...
  errcode =
      regcomp (&vty->include, regexp, REG_EXTENDED | REG_NEWLINE | REG_NOSUB);
  if (errcode)
    {
      ret = false;
...
vty_out (struct vty *vty, const char *format, ...)
{
  ...
  switch (vty->type)
    ...
    case VTY_TERM:
      ...
      buffer_put_crlf (vty->obuf, (uint8_t *) filtered, strlen (filtered));
      break;
    case VTY_SHELL:
      if (vty->of)
        {
...
static int
vty_json_helper (struct vty *vty, struct json_object *json, uint32_t options)
{
  const char *text;
...
int
vty_json (struct vty *vty, struct json_object *json)
{
  return vty_json_helper (
      vty, json, JSON_C_TO_STRING_PRETTY | JSON_C_TO_STRING_NOSLASHESCAPE);
}

int
vty_json_no_pretty (struct vty *vty, struct json_object *json)
{
  return vty_json_helper (vty, json, JSON_C_TO_STRING_NOSLASHESCAPE);
}

void
vty_json_empty (struct vty *vty)
{
          ...
          while (fgets (buf, sizeof (buf), f))
            {
              ...
              for (s = buf + strlen (buf);
                   (s > buf) && isspace ((unsigned char) s[-1]); s--)
                ;
              *s = '\0';
              vty_out (vty, "%s\n", buf);
            }
...
static void
vty_describe_fold (struct vty *vty, int cmd_width, unsigned int desc_width,
...
vty_flush (struct event *thread)
{
  ...
  if ((vty->lines == 0) || (vty->width == 0) || (vty->height == 0))
    ...
    flushrc =
        buffer_flush_window (vty->obuf, vty->wfd, vty->width, 1, erase, 0);
  else
  ...
  if (mgmt_fe_client)
    {
      ...
      mgmt_fe_create_client_session (mgmt_fe_client, new->mgmt_client_id,
                                     (uintptr_t) new);
...
static void
vty_stdio_atexit (void)
{
  vty_stdio_reset (1);
}

void
vty_stdio_suspend (void)
{
  if (! stdio_vty)
    return;
...
struct vty *
vty_stdio (void (*atclose) (int isexit))
{
  struct vty *vty;
...
static void
vty_accept (struct event *thread)
{
  ...
  if (! sockunion2hostprefix (&su, &p))
    {
      close (vty_sock);
      zlog_info ("Vty unable to convert prefix from sockunion %pSU", &su);
      return;
    }
  ...
  ret = setsockopt (vty_sock, IPPROTO_TCP, TCP_NODELAY, (char *) &on,
                    sizeof (on));
  if (ret < 0)
    zlog_info ("can't set sockopt to vty_sock : %s", safe_strerror (errno));
...
vty_serv_un (const char *path)
{
  ...
  if (sock < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "Cannot create unix stream socket: %s",
                    safe_strerror (errno));
      return;
    }
...
static void
vtysh_accept (struct event *thread)
{
  struct vty_serv *vtyserv = EVENT_ARG (thread);
  int accept_sock = vtyserv->sock;
  int sock;
...
static int
vtysh_do_pass_fd (struct vty *vty)
{
  struct iovec iov[1] = {
    {
...
vtysh_flush (struct vty *vty)
{
  ...
  switch (ret)
    ...
    case BUFFER_PENDING:
      ...
      break;
    case BUFFER_ERROR:
      flog_err (EC_LIB_SOCKET, "%s: write error to fd %d, closing", __func__,
                vty->fd);
      buffer_reset (vty->lbuf);
      buffer_reset (vty->obuf);
      vty_close (vty);
...
void
vty_pass_fd (struct vty *vty, int fd)
{
  if (vty->pass_fd != -1)
    close (vty->pass_fd);
...
static void
vtysh_read (struct event *thread)
{
  int ret;
  int sock;
  int nbytes;
...
static void
vtysh_write (struct event *thread)
{
  struct vty *vty = EVENT_ARG (thread);
...
void
vty_serv_start (const char *addr, unsigned short port, const char *path)
{
...
vty_close (struct vty *vty)
{
  ...
  if (vty->mgmt_num_pending_setcfg)
    MGMTD_FE_CLIENT_ERR ("vty closed, uncommitted config will be lost.");
...
static void
vty_timeout (struct event *thread)
{
  struct vty *vty;
...
void
vty_read_file (struct nb_config *config, FILE *confp)
{
  struct vty *vty;
...
void
vty_read_file_finish (struct vty *vty, struct nb_config *config)
{
  struct vty_error *ve;
  ...
  for (ALL_LIST_ELEMENTS_RO (vty->error, node, ve))
    {
      ...
      if (nl)
        *nl = '\0';
      flog_err (EC_LIB_VTY, "%s on config line %u: %s", message, ve->line_num,
                ve->error_buf);
    }
  ...
  if (config == NULL)
    {
      ...
      ret = nb_candidate_commit (context, vty->candidate_config, true,
                                 "Read configuration file", NULL, errmsg,
                                 sizeof (errmsg));
      if (ret != NB_OK && ret != NB_ERR_NO_CHANGES)
        zlog_err ("%s: failed to read configuration file: %s (%s)", __func__,
                  nb_err_name (ret), errmsg);
    }
...
FILE *
vty_open_config (const char *config_file, char *config_default_dir)
{
  char cwd[MAXPATHLEN];
  FILE *confp = NULL;
  const char *fullpath;
      ...
      if (confp == NULL)
        {
          ...
          if (confp)
            ...
            {
              flog_err (EC_LIB_VTY, "%s: can't open configuration file [%s]",
      ...
      if (confp == NULL)
        {
          ...
          if (confp)
            ...
            {
              flog_err (EC_LIB_VTY, "can't open configuration file [%s]",
                        config_default_dir);
              goto tmp_free_and_out;
            }
        }
...
bool
vty_read_config (struct nb_config *config, const char *config_file,
...
int
vty_config_enter (struct vty *vty, bool private_config, bool exclusive,
                  bool file_lock)
{
...
void
vty_config_exit (struct vty *vty)
{
...
int
vty_config_node_exit (struct vty *vty)
{
  vty->xpath_index = 0;
...
static void
vty_event_serv (enum vty_event event, struct vty_serv *vty_serv)
{
  switch (event)
    {
    case VTY_SERV:
...
#ifdef VTYSH
    case VTYSH_SERV:
      event_add_read (vty_master, vtysh_accept, vty_serv, vty_serv->sock,
                      &vty_serv->t_accept);
      break;
#endif /* VTYSH */
    case VTY_READ:
...
static void
vty_event (enum vty_event event, struct vty *vty)
{
  switch (event)
    {
#ifdef VTYSH
    case VTYSH_READ:
      event_add_read (vty_master, vtysh_read, vty, vty->fd, &vty->t_read);
      break;
    case VTYSH_WRITE:
      event_add_write (vty_master, vtysh_write, vty, vty->wfd, &vty->t_write);
      break;
#endif /* VTYSH */
    case VTY_READ:
      event_add_read (vty_master, vty_read, vty, vty->fd, &vty->t_read);

      /* Time out treatment. */
      if (vty->v_timeout)
        {
          EVENT_OFF (vty->t_timeout);
          event_add_timer (vty_master, vty_timeout, vty, vty->v_timeout,
                           &vty->t_timeout);
        }
      break;
    case VTY_WRITE:
      event_add_write (vty_master, vty_flush, vty, vty->wfd, &vty->t_write);
      break;
    case VTY_TIMEOUT_RESET:
      EVENT_OFF (vty->t_timeout);
      if (vty->v_timeout)
  ...
  if (! c)
    {
      ...
      if (chdir (SYSCONFDIR))
        {
          flog_err_sys (EC_LIB_SYSTEM_CALL,
                        "Failure to chdir to %s, errno: %d", SYSCONFDIR,
                        errno);
          exit (-1);
        }
      if (getcwd (vty_cwd, sizeof (vty_cwd)) == NULL)
        {
          flog_err_sys (EC_LIB_SYSTEM_CALL, "Failure to getcwd, errno: %d",
                        errno);
          exit (-1);
        }
    }
...
static void
vty_mgmt_server_connected (struct mgmt_fe_client *client, uintptr_t usr_data,
                           bool connected)
{
 ...
 */
static void
vty_mgmt_session_notify (struct mgmt_fe_client *client, uintptr_t usr_data,
...
static void
vty_mgmt_ds_lock_notified (struct mgmt_fe_client *client, uintptr_t usr_data,
                           ...
                           Mgmtd__DatastoreId ds_id, char *errmsg_if_any)
{
  ...
  if (! success)
    ...
    {
      MGMTD_FE_CLIENT_DBG ("%socked DS %u successfully", lock_ds ? "L" : "Unl",
                           ds_id);
...
static void
vty_mgmt_set_config_result_notified (struct mgmt_fe_client *client,
                                     uintptr_t usr_data, uint64_t client_id,
                                     uintptr_t session_id,
                                     uintptr_t session_ctx, uint64_t req_id,
                                     bool success, Mgmtd__DatastoreId ds_id,
                                     bool implicit_commit, char *errmsg_if_any)
{
  struct vty *vty;
...
vty_mgmt_commit_config_result_notified (
    ...
    uintptr_t session_id, uintptr_t session_ctx, uint64_t req_id, bool success,
    ...
    bool validate_only, char *errmsg_if_any)
{
  ...
  if (! success)
    ...
    {
      MGMTD_FE_CLIENT_DBG ("COMMIT_CONFIG request for client 0x%" PRIx64
                           " req-id %" PRIu64 " was successfull",
                           client_id, req_id);
      if (errmsg_if_any)
        vty_out (vty, "MGMTD: %s\n", errmsg_if_any);
...
vty_mgmt_get_data_result_notified (
    struct mgmt_fe_client *client, uintptr_t usr_data, uint64_t client_id,
    uintptr_t session_id, uintptr_t session_ctx, uint64_t req_id, bool success,
    Mgmtd__DatastoreId ds_id, Mgmtd__YangData **yang_data, size_t num_data,
...
bool
vty_mgmt_fe_enabled (void)
{
  return mgmt_fe_client && mgmt_fe_connected;
}

bool
vty_mgmt_should_process_cli_apply_changes (struct vty *vty)
{
  return vty->type != VTY_FILE && vty_mgmt_fe_enabled ();
}

int
vty_mgmt_send_lockds_req (struct vty *vty, Mgmtd__DatastoreId ds_id, bool lock,
                          bool scok)
{
  assert (mgmt_fe_client);
...
int
vty_mgmt_send_config_data (struct vty *vty, bool implicit_commit)
{
  ...
  if (implicit_commit)
    {
      if (vty_mgmt_lock_candidate_inline (vty))
        {
          vty_out (vty, "%% command failed, could not lock candidate DS\n");
          return -1;
        }
      else if (vty_mgmt_lock_running_inline (vty))
        {
          vty_out (vty, "%% command failed, could not lock running DS\n");
          vty_mgmt_unlock_candidate_inline (vty);
          return -1;
        }
    }
  ...
  for (indx = 0; indx < vty->num_cfg_changes; indx++)
    {
      ...
      if (vty->cfg_changes[indx].value)
        {
          ...
          value[indx].encoded_str_val = (char *) vty->cfg_changes[indx].value;
      ...
      switch (vty->cfg_changes[indx].operation)
        {
        case NB_OP_DESTROY:
          cfg_req[indx].req_type = MGMTD__CFG_DATA_REQ_TYPE__DELETE_DATA;
          break;
        ...
        case NB_OP_APPLY_FINISH:
          cfg_req[indx].req_type = MGMTD__CFG_DATA_REQ_TYPE__SET_DATA;
          break;
        case NB_OP_GET_ELEM:
        case NB_OP_GET_NEXT:
        case NB_OP_GET_KEYS:
        ...
        case NB_OP_RPC:
        default:
          assertf (false, "Invalid operation type for send config: %d",
  ...
  if (mgmt_fe_send_setcfg_req (mgmt_fe_client, vty->mgmt_session_id,
                               ...
                               indx, implicit_commit, MGMTD_DS_RUNNING))
    {
...
int
vty_mgmt_send_commit_config (struct vty *vty, bool validate_only, bool abort)
{
  if (mgmt_fe_client && vty->mgmt_session_id)
    {
      vty->mgmt_req_id++;
...
int
vty_mgmt_send_get_req (struct vty *vty, bool is_config,
...
void
vty_init (struct event_loop *master_thread, bool do_command_logging)
{
