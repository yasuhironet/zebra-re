 ...
 */
bool
frr_sigevent_check (sigset_t *setp)
{
  sigset_t blocked;
  int i;
  bool ret;
...
void
frr_signal_timer (struct event *t)
{
...
void
signal_init (struct event_loop *m, int sigc, struct frr_signal_t signals[])
{
...
#ifdef SIGEVENT_SCHEDULE_THREAD
  sigmaster.t = NULL;
  event_add_timer (m, frr_signal_timer, &sigmaster, FRR_SIGNAL_TIMER_INTERVAL,
                   &sigmaster.t);
#endif /* SIGEVENT_SCHEDULE_THREAD */
}
