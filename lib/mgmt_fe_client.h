...
#define MGMTD_DS_MAX_ID      MGMTD_DS_OPERATIONAL + 1
  ...
  struct mgmt_fe_client_cbs
  {
    ...
    void (*commit_config_notify) (
        ...
        uintptr_t session_id, uintptr_t user_session_client, uint64_t req_id,
    ...
    int (*data_notify) (uint64_t client_id, uint64_t session_id,
                        ...
                        Mgmtd__DatastoreId ds_id, Mgmtd__YangData **yang_data,
                        size_t num_data);
  };
...
#define MGMTD_DBG_FE_CLIENT_CHECK()                                           \
  ...
  mgmt_fe_create_client_session (struct mgmt_fe_client *client,
                                 uint64_t client_id, uintptr_t user_client);
