...
void
ringbuf_del (struct ringbuf *buf)
{
...
size_t
ringbuf_remain (struct ringbuf *buf)
{
...
size_t
ringbuf_space (struct ringbuf *buf)
{
  return buf->size - ringbuf_remain (buf);
}

size_t
ringbuf_put (struct ringbuf *buf, const void *data, size_t size)
{
...
size_t
ringbuf_get (struct ringbuf *buf, void *data, size_t size)
{
  uint8_t *dp = data;
...
size_t
ringbuf_peek (struct ringbuf *buf, size_t offset, void *data, size_t size)
{
  uint8_t *dp = data;
  size_t remain = ringbuf_remain (buf);
  if (offset >= remain)
...
size_t
ringbuf_copy (struct ringbuf *to, struct ringbuf *from, size_t size)
{
...
void
ringbuf_reset (struct ringbuf *buf)
{
  buf->start = buf->end = 0;
  buf->empty = true;
}
...
void
ringbuf_wipe (struct ringbuf *buf)
{
