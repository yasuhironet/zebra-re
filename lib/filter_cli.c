 ...
 */
static int
acl_get_seq_cb (const struct lyd_node *dnode, void *arg)
{
  int64_t *seq = arg;
 ...
 */
static int64_t
acl_get_seq (struct vty *vty, const char *xpath, bool is_remove)
{
  int64_t seq = 0;
...
static int
acl_remove_if_empty (struct vty *vty, const char *iptype, const char *name)
{
  char xpath[XPATH_MAXLEN];

  snprintf (xpath, sizeof (xpath),
            "/frr-filter:lib/access-list[type='%s'][name='%s']/remark", iptype,
            name);
  ...
  snprintf (xpath, sizeof (xpath),
            "/frr-filter:lib/access-list[type='%s'][name='%s']", iptype, name);
  /*
...
static int
acl_remove (struct vty *vty, const char *iptype, const char *name,
            int64_t sseq)
{
  char xpath[XPATH_MAXLEN];
  int rv;
            ...
            "Wildcard bits\n")
{
  ...
  if (seq_str == NULL)
    ...
    snprintfrr (xpath_entry, sizeof (xpath_entry), "%s/entry[sequence='%s']",
                xpath, seq_str);
  ...
  if (host_str != NULL && mask_str == NULL)
    ...
    {
      nb_cli_enqueue_change (vty, "./network/address", NB_OP_MODIFY, host_str);
      nb_cli_enqueue_change (vty, "./network/mask", NB_OP_MODIFY, mask_str);
    }
  else
    {
            ...
            "Any destination host\n")
{
  ...
  if (src_str != NULL && src_mask_str == NULL)
    ...
    {
      nb_cli_enqueue_change (vty, "./network/address", NB_OP_MODIFY, src_str);
  ...
  if (dst_str != NULL && dst_mask_str == NULL)
    {
      nb_cli_enqueue_change (vty, "./destination-host", NB_OP_MODIFY, dst_str);
    }
  else if (dst_str != NULL && dst_mask_str != NULL)
    {
      nb_cli_enqueue_change (vty, "./destination-network/address",
                             NB_OP_MODIFY, dst_str);
      nb_cli_enqueue_change (vty, "./destination-network/mask", NB_OP_MODIFY,
                             dst_mask_str);
    }
  else
    {
      nb_cli_enqueue_change (vty, "./destination-any", NB_OP_CREATE, NULL);
    }

  return nb_cli_apply_changes (vty, "%s", xpath_entry);
}

DEFPY_YANG (no_access_list_ext,
            ...
            "Match any IPv4\n")
{
  ...
  if (prefix_str != NULL)
    {
      nb_cli_enqueue_change (vty, "./ipv4-prefix", NB_OP_MODIFY, prefix_str);
...
DEFPY_YANG (no_access_list_remark,
            ...
            "Match any IPv6\n")
{
  ...
  if (prefix_str != NULL)
    {
      nb_cli_enqueue_change (vty, "./ipv6-prefix", NB_OP_MODIFY, prefix_str);
...
DEFPY_YANG (no_ipv6_access_list,
...
DEFPY_YANG (no_ipv6_access_list_all,
...
DEFPY_YANG (ipv6_access_list_remark,
...
DEFPY_YANG (no_ipv6_access_list_remark,
...
DEFPY_YANG (no_mac_access_list_all,
...
DEFPY_YANG (mac_access_list_remark,
...
DEFPY_YANG (no_mac_access_list_remark,
            ...
            ACCESS_LIST_REMARK_STR)
{
  ...
  snprintf (xpath, sizeof (xpath),
            "/frr-filter:lib/access-list[type='mac'][name='%s']/remark", name);
  nb_cli_enqueue_change (vty, xpath, NB_OP_DESTROY, NULL);
...
ALIAS (no_mac_access_list_remark,
...
int
access_list_cmp (const struct lyd_node *dnode1, const struct lyd_node *dnode2)
{
...
void
access_list_show (struct vty *vty, const struct lyd_node *dnode,
                  bool show_defaults)
{
      ...
      if (yang_dnode_exists (dnode, "./host") ||
          ...
          yang_dnode_exists (dnode, "./source-any"))
        {
          ...
          if (yang_dnode_exists (dnode, "./destination-host") ||
              yang_dnode_exists (dnode, "./destination-network/address") ||
  ...
  if (cisco_style)
    {
      ...
      if (yang_dnode_exists (dnode, "./destination-network"))
        {
          yang_dnode_get_ipv4 (&addr, dnode, "./destination-network/address");
          yang_dnode_get_ipv4 (&mask, dnode, "./destination-network/mask");
          vty_out (vty, " %pI4 %pI4", &addr, &mask);
        }
      else if (yang_dnode_exists (dnode, "./destination-host"))
        vty_out (vty, " host %s",
                 yang_dnode_get_string (dnode, "./destination-host"));
...
void
access_list_remark_show (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
  int type = yang_dnode_get_enum (dnode, "../type");
 ...
 */
static int
plist_remove_if_empty (struct vty *vty, const char *iptype, const char *name)
{
  char xpath[XPATH_MAXLEN];

  snprintf (xpath, sizeof (xpath),
            "/frr-filter:lib/prefix-list[type='%s'][name='%s']/remark", iptype,
            name);
  /* List is not empty if there is a remark, check that: */
  if (yang_dnode_exists (vty->candidate_config->dnode, xpath))
    return CMD_SUCCESS;
  ...
  /* Check if we have any entries: */
  snprintf (xpath, sizeof (xpath),
            "/frr-filter:lib/prefix-list[type='%s'][name='%s']", iptype, name);
  /*
   * NOTE: if the list is empty it will return the first sequence
   * number: 5.
   */
...
static int
plist_remove (struct vty *vty, const char *iptype, const char *name,
              const char *seq, const char *action, union prefixconstptr prefix,
              int ge, int le)
{
  int64_t sseq;
  ...
  if (prefix_str != NULL)
    {
      ...
      if (ge_str)
        {
          nb_cli_enqueue_change (vty, "./ipv4-prefix-length-greater-or-equal",
                                 NB_OP_MODIFY, ge_str);
        }
      else
        {
      ...
      if (le_str)
        {
          nb_cli_enqueue_change (vty, "./ipv4-prefix-length-lesser-or-equal",
                                 NB_OP_MODIFY, le_str);
        }
      else
        {
...
DEFPY_YANG (no_ip_prefix_list_seq,
...
DEFPY_YANG (no_ip_prefix_list_all,
...
DEFPY_YANG (ip_prefix_list_remark,
...
DEFPY_YANG (no_ip_prefix_list_remark,
...
ALIAS (no_ip_prefix_list_remark,
  ...
  if (prefix_str != NULL)
    {
      ...
      if (ge_str)
        {
          nb_cli_enqueue_change (vty, "./ipv6-prefix-length-greater-or-equal",
                                 NB_OP_MODIFY, ge_str);
        }
      else
        {
      ...
      if (le_str)
        {
          nb_cli_enqueue_change (vty, "./ipv6-prefix-length-lesser-or-equal",
                                 NB_OP_MODIFY, le_str);
        }
      else
        {
...
DEFPY_YANG (no_ipv6_prefix_list,
...
DEFPY_YANG (no_ipv6_prefix_list_seq,
...
DEFPY_YANG (no_ipv6_prefix_list_all,
...
DEFPY_YANG (ipv6_prefix_list_remark,
...
DEFPY_YANG (no_ipv6_prefix_list_remark,
...
ALIAS (no_ipv6_prefix_list_remark,
...
int
prefix_list_cmp (const struct lyd_node *dnode1, const struct lyd_node *dnode2)
{
  uint32_t seq1 = yang_dnode_get_uint32 (dnode1, "./sequence");
  uint32_t seq2 = yang_dnode_get_uint32 (dnode2, "./sequence");
...
void
prefix_list_show (struct vty *vty, const struct lyd_node *dnode,
                  bool show_defaults)
{
  int type = yang_dnode_get_enum (dnode, "../type");
...
void
prefix_list_remark_show (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
  int type = yang_dnode_get_enum (dnode, "../type");
