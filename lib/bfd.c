      ...
      if (ifp == NULL)
        {
          if (bsglobal.debugging)
            zlog_debug ("%s: Can't find interface by ifindex: %d ", __func__,
                        ifindex);
          return NULL;
        }
    }
...
bfd_last_update (time_t last_update, char *buf, size_t len)
{
  ...
  snprintf (buf, len, "%d:%02d:%02d:%02d", tm.tm_yday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
}
 ...
 */
void
bfd_client_sendmsg (struct zclient *zclient, int command, vrf_id_t vrf_id)
{
  struct stream *s;
  ...
  if (ret == ZCLIENT_SEND_FAILURE)
    {
      if (bsglobal.debugging)
        zlog_debug ("%s:  %ld: zclient_send_message() failed", __func__,
                    (long) getpid ());
      return;
    }
...
int
zclient_bfd_command (struct zclient *zc, struct bfd_session_arg *args)
{
  struct stream *s;
  size_t addrlen;
  ...
  if (bsglobal.shutting_down)
    {
      if (bsglobal.debugging)
        zlog_debug ("%s: Suppressing BFD peer reg/dereg messages", __func__);
      return -1;
    }
...
struct bfd_session_params *
bfd_sess_new (bsp_status_update updatecb, void *arg)
{
...
static bool
_bfd_sess_valid (const struct bfd_session_params *bsp)
{
      ...
      if (bsglobal.debugging)
        {
          if (bsp->args.family == AF_INET)
            ...
            zlog_debug ("%s: invalid address: %pI6", __func__, &bsp->args.dst);
        }
      return false;
    }
  ...
  if (bsp->args.mhop &&
    ...
    {
      if (bsglobal.debugging)
        zlog_debug ("%s: multi hop but no local address provided", __func__);
      return false;
    }
...
_bfd_sess_send (struct event *t)
{
  ...
  if (bsp->lastev == BSE_INSTALL)
    {
      bsp->args.command =
          bsp->installed ? ZEBRA_BFD_DEST_UPDATE : ZEBRA_BFD_DEST_REGISTER;
    }
  else
  ...
  if (rv == 0)
  ...
  else
    {
      ...
      zlog_err (
          ...
          __func__, &src, &dst, bsp->args.ifnamelen ? bsp->args.ifname : "*",
...
static void
_bfd_sess_remove (struct bfd_session_params *bsp)
{
...
void
bfd_sess_free (struct bfd_session_params **bsp)
{
  if (*bsp == NULL)
    return;
...
static bool
bfd_sess_address_changed (const struct bfd_session_params *bsp,
...
void
bfd_sess_set_ipv4_addrs (struct bfd_session_params *bsp,
...
void
bfd_sess_set_ipv6_addrs (struct bfd_session_params *bsp,
...
void
bfd_sess_set_interface (struct bfd_session_params *bsp, const char *ifname)
{
...
void
bfd_sess_set_profile (struct bfd_session_params *bsp, const char *profile)
{
  if (profile == NULL)
    {
...
void
bfd_sess_set_vrf (struct bfd_session_params *bsp, vrf_id_t vrf_id)
{
...
void
bfd_sess_set_hop_count (struct bfd_session_params *bsp, uint8_t hops)
{
...
void
bfd_sess_set_cbit (struct bfd_session_params *bsp, bool enable)
{
  bsp->args.cbit = enable;
}

void
bfd_sess_set_timers (struct bfd_session_params *bsp,
...
void
bfd_sess_set_auto_source (struct bfd_session_params *bsp, bool enable)
{
...
void
bfd_sess_install (struct bfd_session_params *bsp)
{
...
void
bfd_sess_uninstall (struct bfd_session_params *bsp)
{
  bsp->lastev = BSE_UNINSTALL;
  event_add_event (bsglobal.tm, _bfd_sess_send, bsp, 0, &bsp->installev);
}
...
bfd_sess_status (const struct bfd_session_params *bsp)
{
  return bsp->bss.state;
}

uint8_t
bfd_sess_hop_count (const struct bfd_session_params *bsp)
{
  return bsp->args.hops;
}

const char *
bfd_sess_profile (const struct bfd_session_params *bsp)
{
  return bsp->args.profilelen ? bsp->args.profile : NULL;
}

void
bfd_sess_addresses (const struct bfd_session_params *bsp, int *family,
...
const char *
bfd_sess_interface (const struct bfd_session_params *bsp)
{
...
const char *
bfd_sess_vrf (const struct bfd_session_params *bsp)
{
  return vrf_id_to_name (bsp->args.vrf_id);
}

vrf_id_t
bfd_sess_vrf_id (const struct bfd_session_params *bsp)
{
  return bsp->args.vrf_id;
}

bool
bfd_sess_cbit (const struct bfd_session_params *bsp)
{
  return bsp->args.cbit;
}

void
bfd_sess_timers (const struct bfd_session_params *bsp,
                 uint8_t *detection_multiplier, uint32_t *min_rx,
                 uint32_t *min_tx)
{
...
bool
bfd_sess_auto_source (const struct bfd_session_params *bsp)
{
  return bsp->auto_source;
}

void
bfd_sess_show (struct vty *vty, struct json_object *json,
               struct bfd_session_params *bsp)
{
  ...
  if (json)
  ...
  else
    {
      vty_out (
          ...
          bsp->args.detection_multiplier, bsp->args.min_rx, bsp->args.min_tx);
    }
 ...
 */
int
zclient_bfd_session_replay (ZAPI_CALLBACK_ARGS)
{
  struct bfd_session_params *bsp;
...
int
zclient_bfd_session_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (bsglobal.debugging)
    {
      ifstr[0] = 0;
      if (ifp)
        snprintf (ifstr, sizeof (ifstr), " (interface %s)", ifp->name);

      snprintf (cbitstr, sizeof (cbitstr), " (CPI bit %s)",
                remote_cbit ? "yes" : "no");

      zlog_debug ("%s: %pFX -> %pFX%s VRF %s(%u)%s: %s", __func__, &sp, &dp,
                  ifstr, vrf_id_to_name (vrf_id), vrf_id, cbitstr,
 ...
 */
static int
bfd_protocol_integration_finish (void)
{
  if (bsglobal.zc == NULL)
    return 0;
...
void
bfd_protocol_integration_init (struct zclient *zc, struct event_loop *tm)
{
...
void
bfd_protocol_integration_set_debug (bool enable)
{
  bsglobal.debugging = enable;
}

void
bfd_protocol_integration_set_shutdown (bool enable)
{
  bsglobal.shutting_down = enable;
}

bool
bfd_protocol_integration_debug (void)
{
  return bsglobal.debugging;
}

bool
bfd_protocol_integration_shutting_down (void)
{
...
static void
bfd_source_cache_get (struct bfd_session_params *session)
{
  struct bfd_source_cache *source;
  ...
  switch (session->args.family)
    ...
    case AF_INET:
      ...
      memcpy (&target.u.prefix4, &session->args.dst, sizeof (struct in_addr));
      break;
    case AF_INET6:
      target.family = AF_INET6;
      target.prefixlen = IPV6_MAX_BITLEN;
      memcpy (&target.u.prefix6, &session->args.dst, sizeof (struct in6_addr));
      break;
    default:
      return;
    }
...
static void
bfd_source_cache_put (struct bfd_session_params *session)
{
  ...
  SLIST_REMOVE (&bsglobal.source_list, session->source_cache, bfd_source_cache,
                entry);
 ...
 */
static bool
bfd_source_cache_update (struct bfd_source_cache *source,
  ...
  for (nh_index = 0; nh_index < route->nexthop_num; nh_index++)
    {
      ...
      if (interface == NULL)
        {
          zlog_err ("next hop interface not found (index %d)", nh->ifindex);
          continue;
        }
...
int
bfd_nht_update (const struct prefix *match, const struct zapi_route *route)
{
  struct bfd_source_cache *source;
