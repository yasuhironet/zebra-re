...
struct cmd_token *
cmd_token_new (enum cmd_token_type type, uint8_t attr, const char *text,
               const char *desc)
{
...
static void
cmd_token_varname_do (struct cmd_token *token, const char *varname,
                      uint8_t varname_src)
{
...
void
cmd_token_varname_set (struct cmd_token *token, const char *varname)
{
  if (varname)
    {
...
static void
cmd_token_varname_fork (struct graph_node *node, struct cmd_token *prevtoken)
{
...
void
cmd_token_varname_join (struct graph_node *join, const char *varname)
{
  if (! varname)
    return;
...
void
cmd_token_varname_seqappend (struct graph_node *node)
{
...
cmd_merge_nodes (struct graph *oldgraph, struct graph *newgraph,
                 struct graph_node *old, struct graph_node *new, int direction)
{
...
void
cmd_graph_node_print_cb (struct graph_node *gn, struct buffer *buf)
{
  static bool wasend;
   ...
   */
  snprintf (nbuf, sizeof (nbuf), ">, style = filled, fillcolor = \"%s\" ];\n",
            color);
  buffer_putstr (buf, nbuf);
...
char *
cmd_graph_dump_dot (struct graph *cmdgraph)
{
