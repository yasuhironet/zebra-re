 ...
 */
static struct c_path *
cpath_copy (struct c_path *dest, const struct c_path *src)
{
 ...
 */
static void
cpath_replace (struct c_path *next_path, struct c_path *cur_path)
{
 ...
 */
static struct v_node *
vnode_new (struct ls_vertex *vertex)
{
  struct v_node *vnode;
...
get_vertex_by_ipv4 (struct ls_ted *ted, struct in_addr ipv4)
{
 ...
 */
static struct ls_vertex *
get_vertex_by_ipv6 (struct ls_ted *ted, struct in6_addr ipv6)
{
  struct ls_subnet *subnet;
  struct prefix p;
...
struct cspf *
cspf_init (struct cspf *algo, const struct ls_vertex *src,
...
struct cspf *
cspf_init_v4 (struct cspf *algo, struct ls_ted *ted, const struct in_addr src,
              const struct in_addr dst, struct constraints *csts)
{
...
struct cspf *
cspf_init_v6 (struct cspf *algo, struct ls_ted *ted, const struct in6_addr src,
              const struct in6_addr dst, struct constraints *csts)
{
  struct ls_vertex *vsrc;
  struct ls_vertex *vdst;
  struct cspf *new_algo;
 ...
 */
static bool
prune_edge (const struct c_path *path, const struct ls_edge *edge,
 ...
 * the next vertex on the computed path. If Vertex has not already been
 * visited, list of edges of the current path is augmented with this edge if
 * the new cost is lower than prior path up to this vertex. Current path is
 * re-inserted in the Priority Queue with its new cost i.e. current cost + edge
 * cost.
 *
 ...
 */
static bool
relax_constraints (struct cspf *algo, struct ls_edge *edge)
{
  ...
  switch (algo->csts.ctype)
    ...
    case CSPF_METRIC:
      ...
      break;
    case CSPF_TE_METRIC:
      total_cost = edge->attributes->standard.te_metric + algo->path->weight;
      break;
    case CSPF_DELAY:
...
struct c_path *
compute_p2p_path (struct cspf *algo, struct ls_ted *ted)
{
      ...
      for (ALL_LIST_ELEMENTS_RO (vertex->outgoing_edges, node, edge))
        {
           ...
           * candidate path
           */
          if (relax_constraints (algo, edge) && algo->pdst->weight < cur_cost)
            {
