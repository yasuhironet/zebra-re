...
static int
route_table_hash_cmp (const struct route_node *a, const struct route_node *b)
{
...
static void
route_common (const struct prefix *n, const struct prefix *p,
              struct prefix *new)
{
  int i;
  uint8_t diff;
...
/* Find matched prefix. */
struct route_node *
route_node_match (struct route_table *table, union prefixconstptr pu)
{
...
struct route_node *
route_node_match_ipv4 (struct route_table *table, const struct in_addr *addr)
{
...
struct route_node *
route_node_match_ipv6 (struct route_table *table, const struct in6_addr *addr)
{
...
struct route_node *
route_node_lookup (struct route_table *table, union prefixconstptr pu)
{
...
/* Lookup same prefix node.  Return NULL when we can't find route. */
struct route_node *
route_node_lookup_maynull (struct route_table *table, union prefixconstptr pu)
{
  struct route_node rn, *node;
  prefix_copy (&rn.p, pu.p);
  apply_mask (&rn.p);
...
struct route_node *
route_node_get (struct route_table *table, union prefixconstptr pu)
{
...
unsigned long
route_table_count (struct route_table *table)
{
  return table->count;
}
 ...
 */
struct route_node *
route_node_create (route_table_delegate_t *delegate, struct route_table *table)
{
  struct route_node *node;
 ...
 * Default delegate.
 */
static route_table_delegate_t default_delegate = { .create_node =
                                                       route_node_create,
                                                   .destroy_node =
                                                       route_node_destroy };
 ...
 */
struct route_node *
route_table_get_next (struct route_table *table, union prefixconstptr pu)
{
  const struct prefix *p = pu.p;
  struct route_node *node;
