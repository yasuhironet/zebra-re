...
nexthop_group_tail (const struct nexthop_group *nhg)
{
...
uint8_t
nexthop_group_nexthop_num (const struct nexthop_group *nhg)
{
  struct nexthop *nhop;
  uint8_t num = 0;
...
bool
nexthop_group_has_label (const struct nexthop_group *nhg)
{
  struct nexthop *nhop;
...
struct nexthop *
nexthop_exists (const struct nexthop_group *nhg, const struct nexthop *nh)
{
 ...
 */
static struct nexthop *
nhg_nh_find (const struct nexthop_group *nhg, const struct nexthop *nh)
{
  struct nexthop *nexthop;
  int ret;
  ...
  for (nexthop = nhg->nexthop; nexthop; nexthop = nexthop->next)
    {
      ...
      switch (nexthop->type)
        ...
        case NEXTHOP_TYPE_IPV6:
          ret = nexthop_g_addr_cmp (nexthop->type, &nexthop->gate, &nh->gate);
          if (ret != 0)
            continue;
          break;
...
bool
nexthop_group_equal_no_recurse (const struct nexthop_group *nhg1,
                                const struct nexthop_group *nhg2)
{
...
/* This assumes ordered */
bool
nexthop_group_equal (const struct nexthop_group *nhg1,
                     const struct nexthop_group *nhg2)
{
  struct nexthop *nh1 = NULL;
  struct nexthop *nh2 = NULL;
...
void
nexthop_group_copy (struct nexthop_group *to, const struct nexthop_group *from)
{
  to->nhgr = from->nhgr;
...
void
nexthop_group_delete (struct nexthop_group **nhg)
{
...
static void
_nexthop_add_sorted (struct nexthop **head, struct nexthop *nexthop)
{
...
void
nexthop_group_add_sorted (struct nexthop_group *nhg, struct nexthop *nexthop)
{
  struct nexthop *tail;
...
void
_nexthop_del (struct nexthop_group *nhg, struct nexthop *nh)
{
  struct nexthop *nexthop;
...
static void
nexthop_unlink (struct nexthop_group *nhg, struct nexthop *nexthop)
{
 ...
 */
void
nexthop_group_copy_nh_sorted (struct nexthop_group *nhg,
...
void
copy_nexthops (struct nexthop **tnh, const struct nexthop *nh,
...
uint32_t
nexthop_group_hash_no_recurse (const struct nexthop_group *nhg)
{
  struct nexthop *nh;
  uint32_t key = 0;
...
uint32_t
nexthop_group_hash (const struct nexthop_group *nhg)
{
  struct nexthop *nh;
  uint32_t key = 0;
...
void
nexthop_group_mark_duplicates (struct nexthop_group *nhg)
{
...
static void
nhgc_delete_nexthops (struct nexthop_group_cmd *nhgc)
{
  struct nexthop *nexthop;
...
static int
nhgc_cmp_helper (const char *a, const char *b)
{
  if (! a && ! b)
    return 0;
...
static int
nhgc_addr_cmp_helper (const union sockunion *a, const union sockunion *b)
{
  if (! a && ! b)
    return 0;
...
static int
nhgl_cmp (struct nexthop_hold *nh1, struct nexthop_hold *nh2)
{
  int ret;
...
static void
nhgl_delete (struct nexthop_hold *nh)
{
...
static struct nexthop_group_cmd *
...
static void
nhgc_delete (struct nexthop_group_cmd *nhgc)
{
...
static void
nexthop_group_save_nhop (struct nexthop_group_cmd *nhgc,
                         const char *nhvrf_name, const union sockunion *addr,
                         const char *intf, bool onlink, const char *labels,
                         const uint32_t weight, const char *backup_str)
{
 ...
 */
static void
nexthop_group_unsave_nhop (struct nexthop_group_cmd *nhgc,
                           const char *nhvrf_name, const union sockunion *addr,
                           const char *intf)
{
  struct nexthop_hold *nh;
 ...
 */
static bool
nexthop_group_parse_nexthop (struct nexthop *nhop, const union sockunion *addr,
  ...
  if (labels)
    {
      ...
      if (ret < 0)
        ...
        nexthop_add_labels (nhop, ZEBRA_LSP_NONE, labelnum, larray);
    }
  else if (vni)
    {
  ...
  if (backup_str)
    {
      ...
      ret = nexthop_str2backups (backup_str, &num, nhop->backup_idx);
      if (ret == 0)
        {
 ...
 */
static bool
nexthop_group_parse_nhh (struct nexthop *nhop, const struct nexthop_hold *nhh)
{
  return (nexthop_group_parse_nexthop (nhop, nhh->addr, nhh->intf, nhh->onlink,
                                       nhh->nhvrf_name, nhh->labels, nhh->vni,
                                       NULL, nhh->weight, nhh->backup_str));
}
       ...
       "One or more indexes in the range (0-254) separated by ','\n")
{
  ...
  legal =
      nexthop_group_parse_nexthop (&nhop, addr, intf, ! ! onlink, vrf_name,
                                   label, vni, &lbl_ret, weight, backup_idx);

  if (nhop.type == NEXTHOP_TYPE_IPV6 &&
      IN6_IS_ADDR_LINKLOCAL (&nhop.gate.ipv6))
    {
      vty_out (vty, "Specified a v6 LL with no interface, rejecting\n");
      return CMD_WARNING_CONFIG_FAILED;
    }
      ...
      switch (lbl_ret)
        ...
        case -2:
          vty_out (vty, "%% Cannot use reserved label(s) (%d-%d)\n",
                   MPLS_LABEL_RESERVED_MIN, MPLS_LABEL_RESERVED_MAX);
          break;
        case -3:
  ...
  if (yes)
    {
      ...
      /* Save config always */
      nexthop_group_save_nhop (nhgc, vrf_name, addr, intf, ! ! onlink, label,
                               weight, backup_idx);
...
void
nexthop_group_write_nexthop_simple (struct vty *vty, const struct nexthop *nh,
                                    char *altifname)
{
  char *ifname;
...
void
nexthop_group_write_nexthop (struct vty *vty, const struct nexthop *nh)
{
  struct vrf *vrf;
  int i;
  ...
  if (nh->nh_label && nh->nh_label->num_labels > 0)
    {
      ...
      mpls_label2str (nh->nh_label->num_labels, nh->nh_label->label, buf,
                      sizeof (buf), nh->nh_label_type, 0);
...
void
nexthop_group_json_nexthop (json_object *j, const struct nexthop *nh)
{
  struct vrf *vrf;
...
static void
nexthop_group_write_nexthop_internal (struct vty *vty,
...
static int
nexthop_group_write (struct vty *vty)
{
  struct nexthop_group_cmd *nhgc;
  struct nexthop_hold *nh;
  ...
  RB_FOREACH (nhgc, nhgc_entry_head, &nhgc_entries)
    {
      ...
      if (nhgc->nhg.nhgr.buckets)
        vty_out (vty,
                 ...
                 nhgc->nhg.nhgr.buckets, nhgc->nhg.nhgr.idle_timer,
...
void
nexthop_group_enable_vrf (struct vrf *vrf)
{
  struct nexthop_group_cmd *nhgc;
...
void
nexthop_group_disable_vrf (struct vrf *vrf)
{
  struct nexthop_group_cmd *nhgc;
  struct nexthop_hold *nhh;
...
void
nexthop_group_interface_state_change (struct interface *ifp,
...
static void
nhg_name_autocomplete (vector comps, struct cmd_token *token)
{
  struct nexthop_group_cmd *nhgc;
...
void
nexthop_group_init (void (*new) (const char *name),
                    void (*modify) (const struct nexthop_group_cmd *nhgc),
                    void (*add_nexthop) (const struct nexthop_group_cmd *nhg,
                                         const struct nexthop *nhop),
                    void (*del_nexthop) (const struct nexthop_group_cmd *nhg,
