...
#define WQ_UNPLUGGED (1 << 0) /* available for draining */
  ...
  static inline int
  work_queue_item_count (struct work_queue *wq)
  {
    return wq->item_count;
  }

  static inline bool
  work_queue_empty (struct work_queue *wq)
  {
  ...
  static inline void
  work_queue_item_enqueue (struct work_queue *wq, struct work_queue_item *item)
  {
  ...
  static inline void
  work_queue_item_dequeue (struct work_queue *wq, struct work_queue_item *item)
  {
