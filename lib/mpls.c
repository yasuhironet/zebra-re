 ...
 */
int
mpls_str2label (const char *label_str, uint8_t *num_labels,
                mpls_label_t *labels)
{
 ...
 */
char *
mpls_label2str (uint8_t num_labels, const mpls_label_t *labels, char *buf,
  ...
  for (i = 0; i < num_labels; i++)
    {
      ...
      if (pretty)
        ...
        snprintf (
            label_buf, sizeof (label_buf), "%u",
            ((type == ZEBRA_LSP_EVPN) ? label2vni (&labels[i]) : labels[i]));
