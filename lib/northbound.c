...
static int
nb_node_check_config_only (const struct lysc_node *snode, void *arg)
{
...
static int
nb_node_new_cb (const struct lysc_node *snode, void *arg)
{
  ...
  if (CHECK_FLAG (snode->nodetype, LYS_CONTAINER | LYS_LIST))
    {
      ...
      (void) yang_snodes_iterate_subtree (
          snode, NULL, nb_node_check_config_only, 0, &config_only);
      if (config_only)
...
static int
nb_node_del_cb (const struct lysc_node *snode, void *arg)
{
  struct nb_node *nb_node;
...
void
nb_node_set_dependency_cbs (const char *dependency_xpath,
...
bool
nb_node_has_dependency (struct nb_node *node)
{
  return node->dep_cbs.get_dependency_xpath != NULL;
}

static int
nb_node_validate_cb (const struct nb_node *nb_node,
                     enum nb_operation operation, int callback_implemented,
                     bool optional)
{
  bool valid;
 ...
 */
static unsigned int
nb_node_validate_cbs (const struct nb_node *nb_node)

{
  ...
  error += nb_node_validate_cb (nb_node, NB_OP_CREATE, ! ! nb_node->cbs.create,
                                false);
  error += nb_node_validate_cb (nb_node, NB_OP_MODIFY, ! ! nb_node->cbs.modify,
                                false);
  error += nb_node_validate_cb (nb_node, NB_OP_DESTROY,
                                ! ! nb_node->cbs.destroy, false);
  error +=
      nb_node_validate_cb (nb_node, NB_OP_MOVE, ! ! nb_node->cbs.move, false);
  ...
  error +=
      nb_node_validate_cb (nb_node, NB_OP_RPC, ! ! nb_node->cbs.rpc, false);

  return error;
}

static unsigned int
nb_node_validate_priority (const struct nb_node *nb_node)
{
...
static int
nb_node_validate (const struct lysc_node *snode, void *arg)
{
...
struct nb_config *
nb_config_new (struct lyd_node *dnode)
{
...
void
nb_config_free (struct nb_config *config)
{
  if (config->dnode)
...
struct nb_config *
nb_config_dup (const struct nb_config *config)
{
...
int
nb_config_merge (struct nb_config *config_dst, struct nb_config *config_src,
...
void
nb_config_replace (struct nb_config *config_dst, struct nb_config *config_src,
                   bool preserve_source)
{
  /* Update version. */
...
static inline int
nb_config_cb_compare (const struct nb_config_cb *a,
...
static void
nb_config_diff_add_change (struct nb_config_cbs *changes,
                           enum nb_operation operation, uint32_t *seq,
                           const struct lyd_node *dnode)
{
  struct nb_config_change *change;
...
void
nb_config_diff_del_changes (struct nb_config_cbs *changes)
{
  while (! RB_EMPTY (nb_config_cbs, changes))
    {
      struct nb_config_change *change;

      change = (struct nb_config_change *) RB_ROOT (nb_config_cbs, changes);
 ...
 */
void
nb_config_diff_created (const struct lyd_node *dnode, uint32_t *seq,
                        struct nb_config_cbs *changes)
{
  ...
  switch (dnode->schema->nodetype)
    ...
    case LYS_LIST:
      if (nb_operation_is_valid (NB_OP_CREATE, dnode->schema))
        nb_config_diff_add_change (changes, NB_OP_CREATE, seq, dnode);
...
static void
nb_config_diff_deleted (const struct lyd_node *dnode, uint32_t *seq,
                        struct nb_config_cbs *changes)
{
  /* Ignore unimplemented nodes. */
  if (! dnode->schema->priv)
...
static int
nb_lyd_diff_get_op (const struct lyd_node *dnode)
{
 ...
 */
void
nb_config_diff (const struct nb_config *config1,
                const struct nb_config *config2, struct nb_config_cbs *changes)
{
  ...
  err = lyd_diff_siblings (config1->dnode, config2->dnode, LYD_DIFF_DEFAULTS,
                           &diff);
  assert (! err);
      ...
      LYD_TREE_DFS_BEGIN (root, dnode)
        {
          ...
          switch (op)
            ...
            case 'r': /* replace */
              ...
              target = yang_dnode_get (config2->dnode, path);
              assert (target);
              nb_config_diff_add_change (changes, NB_OP_MODIFY, &seq, target);
              break;
            case 'n': /* none */
            default:
              break;
...
int
nb_candidate_edit (struct nb_config *candidate, const struct nb_node *nb_node,
  ...
  switch (operation)
    ...
    case NB_OP_MODIFY:
      err = lyd_new_path (candidate->dnode, ly_native_ctx, xpath_edit,
                          (void *) data->value, LYD_NEW_PATH_UPDATE, &dnode);
      if (err)
        {
          flog_warn (EC_LIB_LIBYANG, "%s: lyd_new_path(%s) failed: %d",
                     __func__, xpath_edit, err);
          return NB_ERR;
        }
      else if (dnode)
        {
          ...
          LY_ERR err =
              lyd_new_implicit_tree (dnode, LYD_IMPLICIT_NO_STATE, NULL);
          if (err)
            {
              flog_warn (EC_LIB_LIBYANG, "%s: lyd_new_implicit_all failed: %d",
                         __func__, err);
            }
          /*
           * create dependency
          ...
          if (nb_node->dep_cbs.get_dependency_xpath)
            {
              ...
              err = lyd_new_path (candidate->dnode, ly_native_ctx, dep_xpath,
                                  NULL, LYD_NEW_PATH_UPDATE, &dep_dnode);
              /* Create default nodes */
              if (! err && dep_dnode)
                err = lyd_new_implicit_tree (dep_dnode, LYD_IMPLICIT_NO_STATE,
                                             NULL);
              if (err)
                {
    ...
    case NB_OP_RPC:
      flog_warn (EC_LIB_DEVELOPMENT, "%s: unknown operation (%u) [xpath %s]",
                 __func__, operation, xpath_edit);
      return NB_ERR;
    }
...
static void
nb_update_candidate_changes (struct nb_config *candidate,
  ...
  LYD_TREE_DFS_BEGIN (root, dnode)
    {
      ...
      switch (op)
        ...
        case 'd': /* delete */
          ...
          break;
        case 'r': /* replace */
          nb_config_diff_add_change (cfg_chgs, NB_OP_MODIFY, seq, dnode);
          break;
        case 'n': /* none */
        default:
          break;
...
static bool
nb_is_operation_allowed (struct nb_node *nb_node, struct nb_cfg_change *change)
{
  enum nb_operation oper = change->operation;
...
void
nb_candidate_edit_config_changes (struct nb_config *candidate_config,
                                  struct nb_cfg_change cfg_changes[],
                                  size_t num_cfg_changes,
                                  const char *xpath_base,
                                  const char *curr_xpath, int xpath_index,
                                  char *err_buf, int err_bufsize, bool *error)
{
  uint32_t seq = 0;
  ...
  for (size_t i = 0; i < num_cfg_changes; i++)
    {
      ...
      if (xpath_index > 0 && (xpath_base[0] == '.' || change->xpath[0] == '.'))
       ...
       * configuration.
       */
      ret = nb_candidate_edit (candidate_config, nb_node, change->operation,
                               xpath, NULL, data);
      yang_data_free (data);
      if (ret != NB_OK && ret != NB_ERR_NOT_FOUND)
        {
          flog_warn (
              ...
              __func__, nb_operation_name (change->operation), xpath);
          if (error)
            *error = true;
          continue;
        }
...
bool
nb_candidate_needs_update (const struct nb_config *candidate)
{
...
int
nb_candidate_update (struct nb_config *candidate)
{
 ...
 */
int
nb_candidate_validate_yang (struct nb_config *candidate, bool no_state,
                            char *errmsg, size_t errmsg_len)
{
  if (lyd_validate_all (
          &candidate->dnode, ly_native_ctx,
          no_state ? LYD_VALIDATE_NO_STATE : LYD_VALIDATE_PRESENT, NULL) != 0)
    {
...
int
nb_candidate_validate_code (struct nb_context *context,
      ...
      LYD_TREE_DFS_BEGIN (root, child)
        {
          ...
          ret = nb_callback_pre_validate (context, nb_node, child, errmsg,
                                          errmsg_len);
          if (ret != NB_OK)
            return NB_ERR_VALIDATION;
  ...
  RB_FOREACH (cb, nb_config_cbs, changes)
    {
      ...
      ret = nb_callback_configuration (context, NB_EV_VALIDATE, change, errmsg,
                                       errmsg_len);
      if (ret != NB_OK)
        return NB_ERR_VALIDATION;
    }
...
int
nb_candidate_diff_and_validate_yang (struct nb_context *context,
                                     struct nb_config *candidate,
                                     struct nb_config_cbs *changes,
                                     char *errmsg, size_t errmsg_len)
{
...
int
nb_candidate_validate (struct nb_context *context, struct nb_config *candidate,
                       char *errmsg, size_t errmsg_len)
{
  struct nb_config_cbs changes;
  int ret;
...
int
nb_candidate_commit_prepare (struct nb_context context,
                             struct nb_config *candidate, const char *comment,
                             struct nb_transaction **transaction,
                             ...
                             char *errmsg, size_t errmsg_len)
{
  ...
  if (! skip_validate && nb_candidate_validate_yang (candidate, true, errmsg,
                                                     errmsg_len) != NB_OK)
    {
      flog_warn (EC_LIB_NB_CANDIDATE_INVALID,
                 "%s: failed to validate candidate configuration", __func__);
      return NB_ERR_VALIDATION;
    }
  ...
  if (*transaction == NULL)
    {
      flog_warn (EC_LIB_NB_TRANSACTION_CREATION_FAILED,
                 "%s: failed to create transaction: %s", __func__, errmsg);
      nb_config_diff_del_changes (&changes);
...
void
nb_candidate_commit_abort (struct nb_transaction *transaction, char *errmsg,
                           size_t errmsg_len)
{
  (void) nb_transaction_process (NB_EV_ABORT, transaction, errmsg, errmsg_len);
  nb_transaction_free (transaction);
}

void
nb_candidate_commit_apply (struct nb_transaction *transaction,
                           bool save_transaction, uint32_t *transaction_id,
                           char *errmsg, size_t errmsg_len)
{
  (void) nb_transaction_process (NB_EV_APPLY, transaction, errmsg, errmsg_len);
...
int
nb_candidate_commit (struct nb_context context, struct nb_config *candidate,
                     bool save_transaction, const char *comment,
                     uint32_t *transaction_id, char *errmsg, size_t errmsg_len)
{
  struct nb_transaction *transaction = NULL;
  int ret;

  ret = nb_candidate_commit_prepare (context, candidate, comment, &transaction,
                                     false, false, errmsg, errmsg_len);
  /*
   * Apply the changes if the preparation phase succeeded. Otherwise abort
   * the transaction.
   */
  if (ret == NB_OK)
    nb_candidate_commit_apply (transaction, save_transaction, transaction_id,
                               errmsg, errmsg_len);
...
int
nb_running_lock (enum nb_client client, const void *user)
{
  int ret = -1;
...
int
nb_running_unlock (enum nb_client client, const void *user)
{
  int ret = -1;
...
int
nb_running_lock_check (enum nb_client client, const void *user)
{
  int ret = -1;
...
static void
nb_log_config_callback (const enum nb_event event, enum nb_operation operation,
                        const struct lyd_node *dnode)
{
  const char *value;
  char xpath[XPATH_MAXLEN];
  ...
  zlog_debug ("northbound callback: event [%s] op [%s] xpath [%s] value [%s]",
              nb_event_name (event), nb_operation_name (operation), xpath,
              value);
}

static int
nb_callback_create (struct nb_context *context, const struct nb_node *nb_node,
...
static int
nb_callback_modify (struct nb_context *context, const struct nb_node *nb_node,
                    enum nb_event event, const struct lyd_node *dnode,
                    union nb_resource *resource, char *errmsg,
                    size_t errmsg_len)
{
...
static int
nb_callback_destroy (struct nb_context *context, const struct nb_node *nb_node,
                     enum nb_event event, const struct lyd_node *dnode,
                     char *errmsg, size_t errmsg_len)
{
...
static int
nb_callback_move (struct nb_context *context, const struct nb_node *nb_node,
                  enum nb_event event, const struct lyd_node *dnode,
                  char *errmsg, size_t errmsg_len)
{
...
static int
nb_callback_pre_validate (struct nb_context *context,
                          const struct nb_node *nb_node,
...
static void
nb_callback_apply_finish (struct nb_context *context,
                          const struct nb_node *nb_node,
                          const struct lyd_node *dnode, char *errmsg,
                          size_t errmsg_len)
{
...
struct yang_data *
nb_callback_get_elem (const struct nb_node *nb_node, const char *xpath,
                      const void *list_entry)
{
  ...
  DEBUGD (&nb_dbg_cbs_state,
          "northbound callback (get_elem): xpath [%s] list_entry [%p]", xpath,
          list_entry);
...
const void *
nb_callback_get_next (const struct nb_node *nb_node,
...
int
nb_callback_get_keys (const struct nb_node *nb_node, const void *list_entry,
...
const void *
nb_callback_lookup_entry (const struct nb_node *nb_node,
...
int
nb_callback_rpc (const struct nb_node *nb_node, const char *xpath,
 ...
 */
static int
nb_callback_configuration (struct nb_context *context,
                           const enum nb_event event,
                           struct nb_config_change *change, char *errmsg,
                           size_t errmsg_len)
{
  ...
  switch (operation)
    {
    case NB_OP_CREATE:
      ret = nb_callback_create (context, nb_node, event, dnode, resource,
                                errmsg, errmsg_len);
      break;
    case NB_OP_MODIFY:
      ret = nb_callback_modify (context, nb_node, event, dnode, resource,
                                errmsg, errmsg_len);
      break;
    case NB_OP_DESTROY:
      ret = nb_callback_destroy (context, nb_node, event, dnode, errmsg,
                                 errmsg_len);
      break;
    case NB_OP_MOVE:
    ...
    case NB_OP_RPC:
      yang_dnode_get_path (dnode, xpath, sizeof (xpath));
      flog_err (EC_LIB_DEVELOPMENT, "%s: unknown operation (%u) [xpath %s]",
                __func__, operation, xpath);
      exit (1);
    }
  ...
  if (ret != NB_OK)
    {
      ...
      switch (event)
        ...
        case NB_EV_APPLY:
          ...
          break;
        default:
          flog_err (EC_LIB_DEVELOPMENT, "%s: unknown event (%u) [xpath %s]",
                    ...
                    char *errmsg, size_t errmsg_len)
{
  ...
  if (nb_running_lock_check (context.client, context.user))
    {
      strlcpy (errmsg, "running configuration is locked by another client",
               errmsg_len);
      return NULL;
    }
  ...
  if (transaction_in_progress)
    {
      strlcpy (errmsg, "there's already another transaction in progress",
               errmsg_len);
      return NULL;
    }
  ...
  if (comment)
    strlcpy (transaction->comment, comment, sizeof (transaction->comment));
...
static void
nb_transaction_free (struct nb_transaction *transaction)
{
...
static int
nb_transaction_process (enum nb_event event,
  ...
  RB_FOREACH (cb, nb_config_cbs, &transaction->changes)
    {
      ...
      ret = nb_callback_configuration (&transaction->context, event, change,
                                       errmsg, errmsg_len);
      switch (event)
        {
        case NB_EV_PREPARE:
...
nb_apply_finish_cb_new (struct nb_config_cbs *cbs,
                        const struct nb_node *nb_node,
                        const struct lyd_node *dnode)
{
  struct nb_config_cb *cb;
...
static void
nb_transaction_apply_finish (struct nb_transaction *transaction, char *errmsg,
                             size_t errmsg_len)
{
      ...
      if (change->cb.operation == NB_OP_DESTROY)
        {
           ...
           */
          yang_dnode_get_path (dnode, xpath, sizeof (xpath));
          dnode = yang_dnode_get (transaction->config->dnode, xpath);
        }
      while (dnode)
        {
          struct nb_node *nb_node;
  ...
  RB_FOREACH (cb, nb_config_cbs, &cbs)
    nb_callback_apply_finish (&transaction->context, cb->nb_node, cb->dnode,
                              errmsg, errmsg_len);
...
static int
nb_oper_data_iter_children (const struct lysc_node *snode, const char *xpath,
                            const void *list_entry,
                            const struct yang_list_keys *list_keys,
  ...
  LY_LIST_FOR (lysc_node_child (snode), child)
    {
      ...
      ret = nb_oper_data_iter_node (child, xpath, list_entry, list_keys,
...
static int
nb_oper_data_iter_leaf (const struct nb_node *nb_node, const char *xpath,
                        const void *list_entry,
                        const struct yang_list_keys *list_keys,
...
static int
nb_oper_data_iter_container (const struct nb_node *nb_node, const char *xpath,
                             const void *list_entry,
                             const struct yang_list_keys *list_keys,
                             struct yang_translator *translator,
                             uint32_t flags, nb_oper_data_cb cb, void *arg)
  ...
  do
    {
      ...
      list_entry =
          nb_callback_get_next (nb_node, parent_list_entry, list_entry);
      if (! list_entry)
        /* End of the list. */
        break;
...
static int
nb_oper_data_iter_list (const struct nb_node *nb_node, const char *xpath_list,
                        const void *parent_list_entry,
                        const struct yang_list_keys *parent_list_keys,
                        struct yang_translator *translator, uint32_t flags,
                        nb_oper_data_cb cb, void *arg)
...
static int
nb_oper_data_iter_node (const struct lysc_node *snode,
                        ...
                        uint32_t flags, nb_oper_data_cb cb, void *arg)
{
  ...
  if (! first && snode->nodetype != LYS_USES)
    {
      ...
      if (parent && parent->module != snode->module)
        snprintf (xpath + strlen (xpath), sizeof (xpath) - strlen (xpath),
                  "/%s:%s", snode->module->name, snode->name);
      else
        snprintf (xpath + strlen (xpath), sizeof (xpath) - strlen (xpath),
                  "/%s", snode->name);
    }
  ...
  switch (snode->nodetype)
    {
    case LYS_CONTAINER:
      ret = nb_oper_data_iter_container (nb_node, xpath, list_entry, list_keys,
    ...
    case LYS_LEAF:
      ...
      break;
    case LYS_LEAFLIST:
      ret = nb_oper_data_iter_leaflist (nb_node, xpath, list_entry, list_keys,
                                        translator, flags, cb, arg);
      break;
    case LYS_LIST:
      ret = nb_oper_data_iter_list (nb_node, xpath, list_entry, list_keys,
                                    translator, flags, cb, arg);
      break;
    case LYS_USES:
...
int
nb_oper_data_iterate (const char *xpath, struct yang_translator *translator,
                      uint32_t flags, nb_oper_data_cb cb, void *arg)
{
  struct nb_node *nb_node;
  const void *list_entry = NULL;
  ...
  if (! nb_node)
    {
      flog_warn (EC_LIB_YANG_UNKNOWN_DATA_PATH, "%s: unknown data path: %s",
                 __func__, xpath);
      return NB_ERR;
    }
  ...
  if (err || ! dnode)
    {
      ...
      flog_warn (EC_LIB_LIBYANG, "%s: lyd_new_path() failed %s", __func__,
                 errmsg);
      return NB_ERR;
    }
  ...
  if (dnode->schema->nodetype == LYS_LIST && lyd_child (dnode))
    ret = nb_oper_data_iter_children (nb_node->snode, xpath, list_entry,
                                      &list_keys, translator, true, flags, cb,
                                      arg);
  else
    ret =
        nb_oper_data_iter_node (nb_node->snode, xpath, list_entry, &list_keys,
...
bool
nb_operation_is_valid (enum nb_operation operation,
                       const struct lysc_node *snode)
{
  struct nb_node *nb_node = snode->priv;
  struct lysc_node_container *scontainer;
    ...
    case NB_OP_DESTROY:
      ...
      switch (snode->nodetype)
        ...
        case LYS_LEAF:
          ...
          if (sleaf->when)
            return true;
          if (CHECK_FLAG (sleaf->flags, LYS_MAND_TRUE) || sleaf->dflt)
            return false;
          break;
        case LYS_CONTAINER:
          scontainer = (struct lysc_node_container *) snode;
...
int
nb_notification_send (const char *xpath, struct list *arguments)
{
  int ret;
...
static bool
running_config_entry_cmp (const void *value1, const void *value2)
{
...
static unsigned int
running_config_entry_key_make (const void *value)
{
  return string_hash_make (value);
}

static void *
running_config_entry_alloc (void *p)
{
...
static void
running_config_entry_free (void *arg)
{
  XFREE (MTYPE_NB_CONFIG_ENTRY, arg);
}

void
nb_running_set_entry (const struct lyd_node *dnode, void *entry)
{
  struct nb_config_entry *config, s;

  yang_dnode_get_path (dnode, s.xpath, sizeof (s.xpath));
  config = hash_get (running_config_entries, &s, running_config_entry_alloc);
  config->entry = entry;
}

void
nb_running_move_tree (const char *xpath_from, const char *xpath_to)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (entries, ln, entry))
    {
      ...
      char *newpath = frrstr_replace (entry->xpath, xpath_from, xpath_to);
      strlcpy (entry->xpath, newpath, sizeof (entry->xpath));
      XFREE (MTYPE_TMP, newpath);

      (void) hash_get (running_config_entries, entry, hash_alloc_intern);
    }
...
static void *
nb_running_unset_entry_helper (const struct lyd_node *dnode)
{
  struct nb_config_entry *config, s;
  struct lyd_node *child;
  void *entry = NULL;
...
void *
nb_running_unset_entry (const struct lyd_node *dnode)
{
  void *entry;
...
static void *
nb_running_get_entry_worker (const struct lyd_node *dnode, const char *xpath,
                             bool abort_if_not_found, bool rec_search)
{
  ...
  flog_err (EC_LIB_YANG_DNODE_NOT_FOUND, "%s: failed to find entry [xpath %s]",
            __func__, xpath_buf);
...
void *
nb_running_get_entry (const struct lyd_node *dnode, const char *xpath,
                      bool abort_if_not_found)
{
  return nb_running_get_entry_worker (dnode, xpath, abort_if_not_found, true);
}

void *
nb_running_get_entry_non_rec (const struct lyd_node *dnode, const char *xpath,
                              bool abort_if_not_found)
{
  return nb_running_get_entry_worker (dnode, xpath, abort_if_not_found, false);
}

/* Logging functions. */
const char *
nb_event_name (enum nb_event event)
{
  switch (event)
    {
    case NB_EV_VALIDATE:
...
const char *
nb_operation_name (enum nb_operation operation)
{
  switch (operation)
    {
    case NB_OP_CREATE:
...
const char *
nb_err_name (enum nb_error error)
{
  switch (error)
    {
    case NB_OK:
...
const char *
nb_client_name (enum nb_client client)
{
  switch (client)
    {
    case NB_CLIENT_CLI:
...
static void
nb_load_callbacks (const struct frr_yang_module_info *module)
{
...
nb_validate_callbacks (void)
{
  ...
  if (errors > 0)
    {
      flog_err (EC_LIB_NB_CBS_VALIDATION,
...
void
nb_init (struct event_loop *tm,
         const struct frr_yang_module_info *const modules[], size_t nmodules,
         bool db_enabled)
{
  ...
  for (size_t i = 0; i < nmodules; i++)
    {
      DEBUGD (&nb_dbg_events, "northbound: loading %s.yang", modules[i]->name);
  ...
  running_config_entries =
      hash_create (running_config_entry_key_make, running_config_entry_cmp,
