...
extern "C"
{
   ...
   *believes it can continue to operate correctly. LOG_ERR	- Error
   *situations indicating malfunctions.  Probably require attention.
   *
...
#define FRR_TIMESTAMP_LEN 40
  extern size_t
  frr_timestamp (int timestamp_precision /* # subsecond digits */, char *buf,
                 size_t buflen);
...
#define OSPF_LOG_DEBUG(cond, fmt, ...)                                        \
  OSPF_LOG (debug, cond, fmt, ##__VA_ARGS__)
