...
static void
distribute_free_if_empty (struct distribute_ctx *ctx, struct distribute *dist)
{
  int i;
...
struct distribute *
distribute_lookup (struct distribute_ctx *ctx, const char *ifname)
{
...
void
distribute_list_add_hook (struct distribute_ctx *ctx,
...
void
distribute_list_delete_hook (struct distribute_ctx *ctx,
                             void (*func) (struct distribute_ctx *ctx,
                                           struct distribute *))
{
...
static struct distribute *
distribute_get (struct distribute_ctx *ctx, const char *ifname)
{
  struct distribute key;
...
static void
distribute_list_set (struct distribute_ctx *ctx, const char *ifname,
...
/* Unset distribute-list.  If matched distribute-list exist then
   return 1. */
static int
distribute_list_unset (struct distribute_ctx *ctx, const char *ifname,
                       enum distribute_type type, const char *alist_name)
{
  struct distribute *dist;
...
/* Set access-list name to the distribute list. */
static void
distribute_list_prefix_set (struct distribute_ctx *ctx, const char *ifname,
                            enum distribute_type type, const char *plist_name)
{
  struct distribute *dist;
...
/* Unset distribute-list.  If matched distribute-list exist then
   return 1. */
static int
distribute_list_prefix_unset (struct distribute_ctx *ctx, const char *ifname,
...
static enum distribute_type
distribute_direction (const char *dir, bool v4)
{
  if (dir[0] == 'i')
    {
      if (v4)
...
int
distribute_list_parser (bool prefix, bool v4, const char *dir,
                        const char *list, const char *ifname)
{
  ...
  void (*distfn) (struct distribute_ctx *, const char *, enum distribute_type,
                  const char *) =
...
int
distribute_list_no_parser (struct vty *vty, bool prefix, bool v4,
                           ...
                           const char *ifname)
{
  ...
  int (*distfn) (struct distribute_ctx *, const char *, enum distribute_type,
                 const char *) =
...
int
config_show_distribute (struct vty *vty, struct distribute_ctx *dist_ctxt)
{
  unsigned int i;
  int has_print = 0;
  ...
  if (dist)
    {
      has_print =
          distribute_print (vty, dist->list, 0, DISTRIBUTE_V4_OUT, has_print);
      has_print = distribute_print (vty, dist->prefix, 1, DISTRIBUTE_V4_OUT,
                                    has_print);
      has_print =
          distribute_print (vty, dist->list, 0, DISTRIBUTE_V6_OUT, has_print);
      has_print = distribute_print (vty, dist->prefix, 1, DISTRIBUTE_V6_OUT,
                                    has_print);
    }
  if (has_print)
    vty_out (vty, "\n");
  ...
  if (dist)
    {
      has_print =
          distribute_print (vty, dist->list, 0, DISTRIBUTE_V4_IN, has_print);
      has_print =
          distribute_print (vty, dist->prefix, 1, DISTRIBUTE_V4_IN, has_print);
      has_print =
          distribute_print (vty, dist->list, 0, DISTRIBUTE_V6_IN, has_print);
      has_print =
          distribute_print (vty, dist->prefix, 1, DISTRIBUTE_V6_IN, has_print);
    }
  if (has_print)
    vty_out (vty, "\n");
  else
        ...
        if (dist->ifname)
          {
            vty_out (vty, "    %s filtered by", dist->ifname);
            has_print = 0;
            has_print = distribute_print (vty, dist->list, 0, DISTRIBUTE_V4_IN,
                                          has_print);
            has_print = distribute_print (vty, dist->prefix, 1,
                                          DISTRIBUTE_V4_IN, has_print);
            has_print = distribute_print (vty, dist->list, 0, DISTRIBUTE_V6_IN,
                                          has_print);
            has_print = distribute_print (vty, dist->prefix, 1,
...
int
config_write_distribute (struct vty *vty, struct distribute_ctx *dist_ctxt)
{
  unsigned int i;
  int j;
  int output, v6;
...
void
distribute_list_delete (struct distribute_ctx **ctx)
{
...
distribute_list_ctx_create (struct vrf *vrf)
{
