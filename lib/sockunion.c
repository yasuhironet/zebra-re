...
sockopt_reuseport (int sock)
{
  ...
  if (ret < 0)
    {
      flog_err (EC_LIB_SOCKET, "can't set sockopt SO_REUSEPORT to socket %d",
                sock);
      return -1;
    }
  return 0;
  ...
  if (family == AF_INET6)
    {
      ...
      if (ret < 0)
        {
          flog_err (EC_LIB_SOCKET,
                    "can't set sockopt IPV6_UNICAST_HOPS %d to socket %d", ttl,
                    sock);
          return -1;
        }
      return 0;
...
sockopt_minttl (int family, int sock, int minttl)
{
  ...
  if (family == AF_INET)
    {
      int ret =
          setsockopt (sock, IPPROTO_IP, IP_MINTTL, &minttl, sizeof (minttl));
      if (ret < 0)
        flog_err (EC_LIB_SOCKET,
                  "can't set sockopt IP_MINTTL to %d on socket %d: %s", minttl,
  ...
  if (family == AF_INET6)
    {
      ...
      if (ret < 0)
        {
          flog_err (EC_LIB_SOCKET,
                    "can't set sockopt IPV6_V6ONLY to socket %d", sock);
          return -1;
        }
      return 0;
    }
...
void
sockunion_set (union sockunion *su, int family, const uint8_t *addr,
               size_t bytes)
{
...
static ssize_t
printfrr_psu (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
  ...
  switch (sockunion_family (su))
    ...
    case AF_INET6:
      ...
      if (include_scope && su->sin6.sin6_scope_id)
        ret += bprintfrr (buf, "%%%u", (unsigned int) su->sin6.sin6_scope_id);
      if (include_port)
    ...
    case AF_UNIX:
      {
        ...
        if (su->sun.sun_path[0] == '\0' && su->sun.sun_path[1])
          {
            len =
                strnlen (su->sun.sun_path + 1, sizeof (su->sun.sun_path) - 1);
...
int
sockunion_is_null (const union sockunion *su)
{
...
static ssize_t
printfrr_pf (struct fbuf *buf, struct printfrr_eargs *ea, uintmax_t val)
{
  switch (val)
    {
    case AF_INET:
...
static ssize_t
printfrr_so (struct fbuf *buf, struct printfrr_eargs *ea, uintmax_t val)
{
  switch (val)
    {
    case SOCK_STREAM:
