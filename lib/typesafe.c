...
bool
typesafe_list_member (const struct slist_head *head,
...
bool
typesafe_dlist_member (const struct dlist_head *head,
...
void
typesafe_hash_grow (struct thash_head *head)
{
...
void
typesafe_hash_shrink (struct thash_head *head)
{
  uint32_t newsize = head->count, i, j;
  uint8_t newshift, delta;
...
static inline struct sskip_item *
sl_level_get (const struct sskip_item *item, size_t level)
{
...
static inline void
sl_level_set (struct sskip_item *item, size_t level, struct sskip_item *value)
{
  if (level < SKIPLIST_OVERFLOW)
...
struct sskip_item *
typesafe_skiplist_add (struct sskip_head *head, struct sskip_item *item,
...
typesafe_skiplist_find (const struct sskip_head *head,
                        const struct sskip_item *item,
                        int (*cmpfn) (const struct sskip_item *a,
                                      const struct sskip_item *b))
{
...
const struct sskip_item *
typesafe_skiplist_find_gteq (const struct sskip_head *head,
                             const struct sskip_item *item,
                             int (*cmpfn) (const struct sskip_item *a,
                                           const struct sskip_item *b))
{
...
const struct sskip_item *
typesafe_skiplist_find_lt (const struct sskip_head *head,
                           const struct sskip_item *item,
                           int (*cmpfn) (const struct sskip_item *a,
                                         const struct sskip_item *b))
{
...
struct sskip_item *
typesafe_skiplist_del (struct sskip_head *head, struct sskip_item *item,
                       int (*cmpfn) (const struct sskip_item *a,
                                     const struct sskip_item *b))
{
  size_t level = SKIPLIST_MAXDEPTH;
  ...
  while (level)
    {
      ...
      if (next == item)
        {
          sl_level_set (prev, level - 1, sl_level_get (item, level - 1));
          level--;
          found = true;
          continue;
        }
...
struct sskip_item *
typesafe_skiplist_pop (struct sskip_head *head)
{
  size_t level = SKIPLIST_MAXDEPTH;
...
void
typesafe_heap_resize (struct heap_head *head, bool grow)
{
  uint32_t newsize;
...
void
typesafe_heap_pushdown (struct heap_head *head, uint32_t pos,
...
#define nary_last(x) (((x) & (HEAP_NARY - 1)) == HEAP_NARY - 1)
      if (downpos >= head->count || cmpfn (head->array[downpos], item) >= 0)
        {
      ...
      else if (nary_last (pos) ||
               cmpfn (head->array[rghtpos], head->array[downpos]) >= 0)
        moveto = downpos;
      else
        moveto = rghtpos;
#undef nary_last
...
void
typesafe_heap_pullup (struct heap_head *head, uint32_t pos,
                      struct heap_item *item,
                      int (*cmpfn) (const struct heap_item *a,
                                    const struct heap_item *b))
{
