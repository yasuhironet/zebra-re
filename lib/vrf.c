...
int
vrf_switch_to_netns (vrf_id_t vrf_id)
{
  char *name;
...
int
vrf_switchback_to_initial (void)
{
 ...
 */
struct vrf *
vrf_update (vrf_id_t new_vrf_id, const char *name)
{
  struct vrf *vrf = NULL;
  ...
  if (vrf && new_vrf_id != VRF_UNKNOWN && vrf->vrf_id != VRF_UNKNOWN &&
      vrf->vrf_id != new_vrf_id)
    {
      if (debug_vrf)
        {
          zlog_debug ("Vrf Update event: %s old id: %u, new id: %u", name,
                      vrf->vrf_id, new_vrf_id);
        }
...
const char *
vrf_id_to_name (vrf_id_t vrf_id)
{
  struct vrf *vrf;
...
static unsigned int
vrf_hash_bitmap_key (const void *data)
{
...
static bool
vrf_hash_bitmap_cmp (const void *a, const void *b)
{
...
static void *
vrf_hash_bitmap_alloc (void *data)
{
...
static void
vrf_hash_bitmap_free (void *data)
{
...
void
vrf_init (int (*create) (struct vrf *), int (*enable) (struct vrf *),
  ...
  if (vrf_is_backend_netns ())
    {
      ...
      strlcpy (default_vrf->data.l.netns_name, VRF_DEFAULT_NAME, NS_NAMSIZ);
...
static void
vrf_terminate_single (struct vrf *vrf)
{
...
int
vrf_is_backend_netns (void)
{
  return (vrf_backend == VRF_BACKEND_NETNS);
}

int
vrf_get_backend (void)
{
...
int
vrf_configure_backend (enum vrf_backend_type backend)
{
...
void
vrf_set_default_name (const char *default_name)
{
  snprintf (vrf_default_name, VRF_NAMSIZ, "%s", default_name);
}

const char *
vrf_get_default_name (void)
{
  return vrf_default_name;
}

int
vrf_bind (vrf_id_t vrf_id, int fd, const char *ifname)
{
  int ret = 0;
...
int
vrf_getaddrinfo (const char *node, const char *service,
...
int
vrf_ioctl (vrf_id_t vrf_id, int d, unsigned long request, char *params)
{
...
int
vrf_sockunion_socket (const union sockunion *su, vrf_id_t vrf_id,
                      const char *interfacename)
{
  int ret, save_errno, ret2;
 ...
 */
static int
lib_vrf_create (struct nb_cb_create_args *args)
{
  const char *vrfname;
  struct vrf *vrfp;
...
static int
lib_vrf_destroy (struct nb_cb_destroy_args *args)
{
  struct vrf *vrfp;
...
static const void *
lib_vrf_get_next (struct nb_cb_get_next_args *args)
{
...
static int
lib_vrf_get_keys (struct nb_cb_get_keys_args *args)
{
  struct vrf *vrfp = (struct vrf *) args->list_entry;
...
static const void *
lib_vrf_lookup_entry (struct nb_cb_lookup_entry_args *args)
{
