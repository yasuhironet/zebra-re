...
char *
mlag_role2str (enum mlag_role role, char *buf, size_t size)
{
  switch (role)
    {
    case MLAG_ROLE_NONE:
...
char *
mlag_lib_msgid_to_str (enum mlag_msg_type msg_type, char *buf, size_t size)
{
  switch (msg_type)
    {
    case MLAG_REGISTER:
...
int
mlag_lib_decode_mlag_hdr (struct stream *s, struct mlag_msg *msg,
                          size_t *length)
{
...
int
mlag_lib_decode_mroute_add (struct stream *s, struct mlag_mroute_add *msg,
                            size_t *length)
{
...
int
mlag_lib_decode_mroute_del (struct stream *s, struct mlag_mroute_del *msg,
                            size_t *length)
{
...
int
mlag_lib_decode_mlag_status (struct stream *s, struct mlag_status *msg)
{
  if (s == NULL || msg == NULL)
    return -1;
...
int
mlag_lib_decode_vxlan_update (struct stream *s, struct mlag_vxlan *msg)
{
  if (s == NULL || msg == NULL)
    return -1;
...
int
mlag_lib_decode_frr_status (struct stream *s, struct mlag_frr_status *msg)
{
  if (s == NULL || msg == NULL)
    return -1;
