...
static ssize_t
printfrr_hexdump (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
  ssize_t ret = 0;
...
static ssize_t
printfrr_hexdstr (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
  ssize_t ret = 0;
  ssize_t input_len = printfrr_ext_len (ea);
  const uint8_t *pos, *end;
...
static ssize_t
bquote (struct fbuf *buf, const uint8_t *pos, size_t len, unsigned int flags)
{
  ssize_t ret = 0;
...
static ssize_t
printfrr_escape (struct fbuf *buf, struct printfrr_eargs *ea, const void *vptr)
{
...
static ssize_t
printfrr_quote (struct fbuf *buf, struct printfrr_eargs *ea, const void *vptr)
{
  ssize_t len = printfrr_ext_len (ea);
  const uint8_t *ptr = vptr;
  ssize_t ret = 0;
...
ssize_t
printfrr_time (struct fbuf *buf, struct printfrr_eargs *ea,
...
static ssize_t
do_subsec (struct fbuf *buf, const struct timespec *ts, int precision,
           unsigned int flags)
{
...
static ssize_t
printfrr_abstime (struct fbuf *buf, struct printfrr_eargs *ea,
                  const struct timespec *ts, unsigned int flags)
{
...
static ssize_t
printfrr_reltime (struct fbuf *buf, struct printfrr_eargs *ea,
                  const struct timespec *ts, unsigned int flags)
{
  struct timespec real_ts[1];
  ssize_t ret = 0;
  ...
  if (flags & TIMEFMT_BASIC)
    {
      ...
      if (week_day.quot)
        ret += bprintfrr (buf, "%lldw%s%lldd%s%02lldh", week_day.quot, space,
                          week_day.rem, space, day_hour.rem);
      else if (day_hour.quot)
        ret += bprintfrr (buf, "%lldd%s%02lldh%s%02lldm", day_hour.quot, space,
                          day_hour.rem, space, hour_min.rem);
      else
        ret += bprintfrr (buf, "%02lld:%02lld:%02lld", hour_min.quot,
                          hour_min.rem, min_sec.rem);
      /* no sub-seconds here */
  ...
  ret += bprintfrr (buf, "%02lld:%02lld:%02lld", day_hour.rem, hour_min.rem,
                    min_sec.rem);
...
static ssize_t
printfrr_ts (struct fbuf *buf, struct printfrr_eargs *ea, const void *vptr)
{
...
static ssize_t
printfrr_tv (struct fbuf *buf, struct printfrr_eargs *ea, const void *vptr)
{
...
static ssize_t
printfrr_tt (struct fbuf *buf, struct printfrr_eargs *ea, const void *vptr)
{
