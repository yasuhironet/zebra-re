...
static struct key *
key_lookup (const struct keychain *keychain, uint32_t index)
{
  struct listnode *node;
  struct key *key;
...
struct key *
key_lookup_for_accept (const struct keychain *keychain, uint32_t index)
{
  struct listnode *node;
  struct key *key;
  time_t now;
...
static struct key *
key_get (const struct keychain *keychain, uint32_t index)
{
  struct key *key;
...
const struct keychain_algo_info algo_info[] = {
  ...
  { KEYCHAIN_ALGO_HMAC_SHA256, "hmac-sha-256", KEYCHAIN_HMAC_SHA256_HASH_SIZE,
    KEYCHAIN_ALGO_SHA256_INTERNAL_BLK_SIZE, "HMAC-SHA-256" },
  { KEYCHAIN_ALGO_HMAC_SHA384, "hmac-sha-384", KEYCHAIN_HMAC_SHA384_HASH_SIZE,
    KEYCHAIN_ALGO_SHA384_INTERNAL_BLK_SIZE, "HMAC-SHA-384" },
  { KEYCHAIN_ALGO_HMAC_SHA512, "hmac-sha-512", KEYCHAIN_HMAC_SHA512_HASH_SIZE,
    KEYCHAIN_ALGO_SHA512_INTERNAL_BLK_SIZE, "HMAC-SHA-512" },
  { KEYCHAIN_ALGO_MAX, "max", KEYCHAIN_MAX_HASH_SIZE,
    KEYCHAIN_ALGO_MAX_INTERNAL_BLK_SIZE, "Not defined" }
};

uint16_t
keychain_get_block_size (enum keychain_hash_algo key)
{
  return algo_info[key].block;
}

uint16_t
keychain_get_hash_len (enum keychain_hash_algo key)
{
  return algo_info[key].length;
}

const char *
keychain_get_description (enum keychain_hash_algo key)
{
...
const char *
keychain_get_algo_name_by_id (enum keychain_hash_algo key)
{
  return algo_info[key].name;
}

DEFUN (cryptographic_algorithm,
...
DEFUN (no_cryptographic_algorithm,
       ...
       "Year to expire\n")
{
  ...
  return key_lifetime_set (vty, &key->accept, argv[idx_hhmmss]->arg,
       ...
       "Year to expire\n")
{
  ...
  return key_lifetime_set (vty, &key->send, argv[idx_hhmmss]->arg,
                           argv[idx_number]->arg, argv[idx_month]->arg,
                           argv[idx_number_2]->arg, argv[idx_hhmmss_2]->arg,
                           argv[idx_number_3]->arg, argv[idx_month_2]->arg,
                           argv[idx_number_4]->arg);
      ...
      for (ALL_LIST_ELEMENTS_RO (keychain->key, knode, key))
        {
          ...
          if (key->hash_algo != KEYCHAIN_ALGO_NULL)
            vty_out (vty, "  cryptographic-algorithm %s\n",
                     keychain_get_algo_name_by_id (key->hash_algo));
          ...
          if (key->send.start)
            {
              ...
              if (key->send.end == -1)
                ...
                vty_out (vty, " duration %ld",
                         (long) (key->send.end - key->send.start));
              else
                {
...
static void
keychain_active_config (vector comps, struct cmd_token *token)
{
  struct keychain *keychain;
  struct listnode *node;
