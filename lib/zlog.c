...
static void
zlog_tls_key_fini (void)
{
  pthread_key_delete (zlog_tls_key);
}

static inline struct zlog_tls *
zlog_tls_get (void)
{
...
static inline void
zlog_tls_set (struct zlog_tls *val)
{
...
zlog_tls_buffer_init (void)
{
  ...
  mmfd = openat (zlog_tmpdirfd, mmpath, O_RDWR | O_CREAT | O_EXCL | O_CLOEXEC,
                 0600);
  if (mmfd < 0)
    {
  ...
  /* note next statement is under above if() */
    ...
    if (ftruncate (mmfd, TLS_LOG_BUF_SIZE) < 0)
      {
        zlog_err ("failed to allocate thread log buffer \"%s\": %s", mmpath,
                  strerror (errno));
...
void
zlog_msg_pid (struct zlog_msg *msg, intmax_t *pid, intmax_t *tid)
{
#ifndef __OpenBSD__
...
static void
vzlog_notls (const struct xref_logmsg *xref, int prio, const char *fmt,
             va_list ap)
{
  struct zlog_target *zt;
...
static void
vzlog_tls (struct zlog_tls *zlog_tls, const struct xref_logmsg *xref, int prio,
...
static void
zlog_backtrace_msg (const struct xref_logmsg *xref, int prio)
{
  ...
  while (unw_step (&cursor) > 0)
    {
      ...
      if (dladdr ((void *) ip, &dlinfo))
        zlog (prio, "| (%s) %-36s %16lx+%08lx %16lx %s", uid, name,
              (long) dlinfo.dli_fbase, (long) ip - (long) dlinfo.dli_fbase,
              (long) sp, dlinfo.dli_fname);
      else
        zlog (prio, "| (%s) %-36s %16lx %16lx", uid, name, (long) ip,
              (long) sp);

      if (is_thread)
        zlog (prio, "| (%s) ^- scheduled from %s(), %s:%u", uid,
              tc->xref->xref.func, tc->xref->xref.file, tc->xref->xref.line);
    }
...
#endif
  if (! found_thread && tc)
    zlog (prio, "| (%s) scheduled from %s(), %s:%u", uid, tc->xref->xref.func,
          tc->xref->xref.file, tc->xref->xref.line);
}

void
vzlogx (const struct xref_logmsg *xref, int prio, const char *fmt, va_list ap)
{
  struct zlog_tls *zlog_tls = zlog_tls_get ();
  ...
  if (xref)
    {
      ...
      xrdl =
          container_of (xref->xref.xrefdata, struct xrefdata_logmsg, xrefdata);
      if (xrdl->fl_print_bt)
...
void
zlog_sigsafe (const char *text, size_t len)
{
  struct zlog_target *zt;
...
void
_zlog_assert_failed (const struct xref_assert *xref, const char *extra, ...)
{
  va_list ap;
  ...
  if (extra)
    {
      ...
      zlog (LOG_CRIT, "%s:%d: %s(): assertion (%s) failed, extra info: %pVA",
            xref->xref.file, xref->xref.line, xref->xref.func, xref->expr,
            &vaf);

      va_end (ap);
    }
  else
    zlog (LOG_CRIT, "%s:%d: %s(): assertion (%s) failed", xref->xref.file,
          xref->xref.line, xref->xref.func, xref->expr);
...
int
zlog_msg_prio (struct zlog_msg *msg)
{
  return msg->prio;
}

const struct xref_logmsg *
zlog_msg_xref (struct zlog_msg *msg)
{
  return msg->xref;
}

const char *
zlog_msg_text (struct zlog_msg *msg, size_t *textlen)
{
  if (! msg->text)
    {
      va_list args;
...
void
zlog_msg_args (struct zlog_msg *msg, size_t *hdrlen, size_t *n_argpos,
...
size_t
zlog_msg_ts (struct zlog_msg *msg, struct fbuf *out, uint32_t flags)
{
  ...
  if (! (msg->ts_flags & ZLOG_TS_FORMAT) ||
      ((msg->ts_flags ^ flags) & ZLOG_TS_UTC))
    {
      ...
      strftime (msg->ts_str, sizeof (msg->ts_str), "%Y-%m-%dT%H:%M:%S", &tm);
      ...
      if (flags & ZLOG_TS_UTC)
        ...
        snprintfrr (msg->ts_zonetail, sizeof (msg->ts_zonetail), "%+03d:%02d",
                    (int) (tm.tm_gmtoff / 3600),
                    (int) (labs (tm.tm_gmtoff) / 60) % 60);

      msg->ts_dot = msg->ts_str + strlen (msg->ts_str);
      snprintfrr (msg->ts_dot,
                  msg->ts_str + sizeof (msg->ts_str) - msg->ts_dot, ".%09lu",
                  (unsigned long) msg->ts.tv_nsec);
...
size_t
zlog_msg_ts_3164 (struct zlog_msg *msg, struct fbuf *out, uint32_t flags)
{
  flags &= ZLOG_TS_UTC;
  ...
  if (! msg->ts_3164_str[0] || flags != msg->ts_3164_flags)
    {
      ...
      snprintfrr (msg->ts_3164_str, sizeof (msg->ts_3164_str),
                  "%3s %2d %02d:%02d:%02d", months[tm.tm_mon], tm.tm_mday,
...
struct zlog_target *
zlog_target_replace (struct zlog_target *oldzt, struct zlog_target *newzt)
{
  if (newzt)
...
void
zlog_aux_init (const char *prefix, int prio_min)
{
  if (prefix)
...
void
zlog_init (const char *progname, const char *protoname,
           unsigned short instance, uid_t uid, gid_t gid)
{
  zlog_uid = uid;
  zlog_gid = gid;
  ...
  if (instance)
    ...
    {
      snprintfrr (zlog_tmpdir, sizeof (zlog_tmpdir), "%s/%s.%ld", TMPBASEDIR,
                  progname, (long) getpid ());

      zlog_prefixsz =
          snprintfrr (zlog_prefix, sizeof (zlog_prefix), "%s: ", protoname);
    }
  ...
  if (mkdir (zlog_tmpdir, 0700) != 0)
    {
      zlog_err ("failed to mkdir \"%s\": %s", zlog_tmpdir, strerror (errno));
      goto out_warn;
    }
  ...
  if (zlog_tmpdirfd < 0)
    {
      zlog_err ("failed to open \"%s\": %s", zlog_tmpdir, strerror (errno));
      goto out_warn;
    }
  ...
  if (zlog_tmpdirfd >= 0)
    {
      ...
      if (rmdir (zlog_tmpdir))
        zlog_err ("failed to rmdir \"%s\": %s", zlog_tmpdir, strerror (errno));
    }
}
