...
#define NB_YANG_XPATH_KEY(__xpath, __indx1, __indx2)                          \
  ...
  /*
   * Set of configuration callbacks that can be associated to a northbound
   * node.
   */
  struct nb_callbacks
  {
  ...
  /*
   * Northbound-specific data that is allocated for each schema node of the
   * native YANG modules.
   */
  struct nb_node
  {
...
#define NB_OPER_DATA_ITER_NORECURSE 0x0001
  ...
  /* Hooks. */
  DECLARE_HOOK (nb_notification_send,
                (const char *xpath, struct list *arguments),
                (xpath, arguments));
  DECLARE_HOOK (nb_client_debug_config_write, (struct vty * vty), (vty));
  DECLARE_HOOK (nb_client_debug_set_all, (uint32_t flags, bool set),
                (flags, set));
   ...
   *
   * dnode
   *    Pointer to a libyang data node containing the configuration data. If
   * NULL is given, an empty configuration will be created.
   *
   * Returns:
   ...
   */
  extern void nb_config_diff_created (const struct lyd_node *dnode,
                                      uint32_t *seq,
                                      struct nb_config_cbs *changes);
   ...
   */
  extern int nb_candidate_diff_and_validate_yang (
      struct nb_context *context, struct nb_config *candidate,
      struct nb_config_cbs *changes, char *errmsg, size_t errmsg_len);
  ...
  /*
   * Abort a previously created configuration transaction, releasing all
   * resources allocated during the preparation phase.
   *
   * transaction
   ...
   *
   * save_transaction
   *    Specify whether the transaction should be recorded in the transactions
   * log or not.
   *
   * transaction_id
   *    Optional output parameter providing the ID of the committed
   * transaction.
   *
   * errmsg
   *    Buffer to store human-readable error message in case of error.
   ...
   *
   * candidate
   *    Candidate configuration to commit. It's preserved regardless if the
   * commit operation fails or not.
   *
   * save_transaction
   *    Specify whether the transaction should be recorded in the transactions
   * log or not.
   ...
   *
   * Returns:
   *    0 if the running configuration is unlocked or if the client/user owns
   * the lock, -1 otherwise.
   */
  ...
  /*
   * Send a YANG notification. This is a no-op unless the
   * 'nb_notification_send' hook was registered by a northbound plugin.
   *
   * xpath
   ...
   *
   * Suppose we have xpath A/B/C/D, with user pointers associated to C and D.
   * We need to move B to be under Z, so the new xpath is Z/B/C/D. Because user
   ...
   */
  extern void nb_running_move_tree (const char *xpath_from,
                                    const char *xpath_to);
   ...
   *
   * If an user point is not found, this function follows the parent nodes in
   * the running configuration until an user pointer is found or until the root
   * node is reached.
   *
   * dnode
   ...
   *
   *    As a rule of thumb, this parameter should be set to true in the
   * following scenarios:
   *    - Calling this function from any northbound configuration callback
   * during the NB_EV_APPLY phase.
   *    - Calling this function from a 'delete' northbound configuration
   * callback during any phase.
   *
   ...
   *
   *    In all other cases, this parameter should be set to false and the
   * caller should check if the function returned NULL or not.
   *
   * Returns:
   ...
   * need to be validated and not only the callbacks from the newly loaded
   * module. This is because augmentations can change the properties of the
   * augmented module, making mandatory the implementation of additional
   * callbacks.
   */
  ...
  /*
   * Finish the northbound layer gracefully. Should be called only when the
   * daemon is exiting.
   */
