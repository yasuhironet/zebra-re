...
static int
_nexthop_labels_cmp (const struct nexthop *nh1, const struct nexthop *nh2)
{
...
static int
_nexthop_srv6_cmp (const struct nexthop *nh1, const struct nexthop *nh2)
{
  int ret = 0;
  ...
  ret = memcmp (&nh1->nh_srv6->seg6local_ctx, &nh2->nh_srv6->seg6local_ctx,
                sizeof (struct seg6local_context));
  if (ret != 0)
    return ret;

  ret = memcmp (&nh1->nh_srv6->seg6_segs, &nh2->nh_srv6->seg6_segs,
...
int
nexthop_g_addr_cmp (enum nexthop_types_t type, const union g_addr *addr1,
...
static int
_nexthop_gateway_cmp (const struct nexthop *nh1, const struct nexthop *nh2)
{
  return nexthop_g_addr_cmp (nh1->type, &nh1->gate, &nh2->gate);
}

static int
_nexthop_source_cmp (const struct nexthop *nh1, const struct nexthop *nh2)
{
  return nexthop_g_addr_cmp (nh1->type, &nh1->src, &nh2->src);
}

static int
_nexthop_cmp_no_labels (const struct nexthop *next1,
                        const struct nexthop *next2)
{
  ...
  ret = memcmp (next1->backup_idx, next2->backup_idx, next1->backup_num);
...
int
nexthop_cmp (const struct nexthop *next1, const struct nexthop *next2)
{
  int ret = 0;
 ...
 */
int
nexthop_cmp_basic (const struct nexthop *nh1, const struct nexthop *nh2)
{
  int ret = 0;
  const struct mpls_label_stack *nhl1 = NULL;
  const struct mpls_label_stack *nhl2 = NULL;
 ...
 */
bool
nexthop_labels_match (const struct nexthop *nh1, const struct nexthop *nh2)
{
...
bool
nexthop_same (const struct nexthop *nh1, const struct nexthop *nh2)
{
  if (nh1 && ! nh2)
    return false;
...
bool
nexthop_same_no_labels (const struct nexthop *nh1, const struct nexthop *nh2)
{
  if (nh1 && ! nh2)
    return false;
 ...
 */
struct nexthop *
nexthop_from_ifindex (ifindex_t ifindex, vrf_id_t vrf_id)
{
...
struct nexthop *
nexthop_from_ipv4 (const struct in_addr *ipv4, const struct in_addr *src,
                   vrf_id_t vrf_id)
{
  struct nexthop *nexthop;
...
struct nexthop *
nexthop_from_ipv4_ifindex (const struct in_addr *ipv4,
                           const struct in_addr *src, ifindex_t ifindex,
                           vrf_id_t vrf_id)
{
  struct nexthop *nexthop;
...
struct nexthop *
nexthop_from_ipv6 (const struct in6_addr *ipv6, vrf_id_t vrf_id)
{
  struct nexthop *nexthop;
...
struct nexthop *
nexthop_from_ipv6_ifindex (const struct in6_addr *ipv6, ifindex_t ifindex,
                           vrf_id_t vrf_id)
{
  struct nexthop *nexthop;
...
struct nexthop *
nexthop_from_blackhole (enum blackhole_type bh_type, vrf_id_t nh_vrf_id)
{
  struct nexthop *nexthop;
...
void
nexthop_add_labels (struct nexthop *nexthop, enum lsp_types_t ltype,
                    uint8_t num_labels, const mpls_label_t *labels)
{
  ...
  nh_label = XCALLOC (MTYPE_NH_LABEL, sizeof (struct mpls_label_stack) +
...
void
nexthop_add_srv6_seg6local (struct nexthop *nexthop, uint32_t action,
                            const struct seg6local_context *ctx)
{
  ...
  if (! nexthop->nh_srv6)
    nexthop->nh_srv6 = XCALLOC (MTYPE_NH_SRV6, sizeof (struct nexthop_srv6));
...
void
nexthop_del_srv6_seg6local (struct nexthop *nexthop)
{
  if (! nexthop->nh_srv6)
    return;
...
void
nexthop_add_srv6_seg6 (struct nexthop *nexthop, const struct in6_addr *segs)
{
  if (! segs)
    return;
...
void
nexthop_del_srv6_seg6 (struct nexthop *nexthop)
{
  if (! nexthop->nh_srv6)
    return;
...
const char *
nexthop2str (const struct nexthop *nexthop, char *str, int size)
{
  switch (nexthop->type)
    {
    case NEXTHOP_TYPE_IFINDEX:
 ...
 */
struct nexthop *
nexthop_next (const struct nexthop *nexthop)
{
...
unsigned int
nexthop_level (const struct nexthop *nexthop)
{
  unsigned int rv = 0;

  for (const struct nexthop *par = nexthop->rparent; par; par = par->rparent)
    rv++;
...
uint32_t
nexthop_hash_quick (const struct nexthop *nexthop)
{
  uint32_t key = 0x45afe398;
  int i;

  key = jhash_3words (nexthop->type, nexthop->vrf_id, nexthop->nh_label_type,
                      key);
      ...
      while (labels >= 3)
        {
          key = jhash_3words (nexthop->nh_label->label[i],
                              ...
                              nexthop->nh_label->label[i + 2], key);
          labels -= 3;
          i += 3;
        }
      ...
      if (labels >= 2)
        {
          key = jhash_2words (nexthop->nh_label->label[i],
                              nexthop->nh_label->label[i + 1], key);
          labels -= 2;
          i += 2;
        }
  ...
  key = jhash_2words (nexthop->ifindex,
                      CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_ONLINK), key);
  ...
  if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_HAS_BACKUP))
    {
      ...
      while (backups >= 3)
        {
          key =
              jhash_3words (nexthop->backup_idx[i], nexthop->backup_idx[i + 1],
...
uint32_t
nexthop_hash (const struct nexthop *nexthop)
{
...
void
nexthop_copy_no_recurse (struct nexthop *copy, const struct nexthop *nexthop,
  ...
  if (nexthop->nh_srv6)
    {
      ...
      if (! sid_zero (&nexthop->nh_srv6->seg6_segs))
        nexthop_add_srv6_seg6 (copy, &nexthop->nh_srv6->seg6_segs);
    }
}

void
nexthop_copy (struct nexthop *copy, const struct nexthop *nexthop,
              struct nexthop *rparent)
{
...
struct nexthop *
nexthop_dup_no_recurse (const struct nexthop *nexthop, struct nexthop *rparent)
{
...
struct nexthop *
nexthop_dup (const struct nexthop *nexthop, struct nexthop *rparent)
{
  struct nexthop *new = nexthop_new ();
 ...
 */
int
nexthop_str2backups (const char *str, int *num_backups, uint8_t *backups)
{
...
ssize_t
printfrr_nhs (struct fbuf *buf, const struct nexthop *nexthop)
{
  ssize_t ret = 0;
  ...
  switch (nexthop->type)
    ...
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      ret +=
          bprintfrr (buf, "%pI4 if %u", &nexthop->gate.ipv4, nexthop->ifindex);
      break;
    case NEXTHOP_TYPE_IPV6:
    case NEXTHOP_TYPE_IPV6_IFINDEX:
      ret +=
          bprintfrr (buf, "%pI6 if %u", &nexthop->gate.ipv6, nexthop->ifindex);
      break;
    case NEXTHOP_TYPE_BLACKHOLE:
...
static ssize_t
printfrr_nh (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
  ...
  switch (*ea->fmt)
    ...
    case 'v':
      ...
      switch (nexthop->type)
        ...
        case NEXTHOP_TYPE_IPV4:
        case NEXTHOP_TYPE_IPV4_IFINDEX:
          ret += bprintfrr (buf, "%s%pI4", v_via, &nexthop->gate.ipv4);
          do_ifi = true;
          break;
        case NEXTHOP_TYPE_IPV6:
        case NEXTHOP_TYPE_IPV6_IFINDEX:
          ret += bprintfrr (buf, "%s%pI6", v_via, &nexthop->gate.ipv6);
          do_ifi = true;
          break;
        case NEXTHOP_TYPE_IFINDEX:
          ret +=
              bprintfrr (buf, "%sdirectly connected, %s", v_is,
      ...
      if (*ea->fmt == 'g')
        {
          ...
          switch (nexthop->type)
            ...
            case NEXTHOP_TYPE_IPV4:
            case NEXTHOP_TYPE_IPV4_IFINDEX:
              ret += bprintfrr (buf, "%pI4", &nexthop->gate.ipv4);
              break;
            case NEXTHOP_TYPE_IPV6:
            case NEXTHOP_TYPE_IPV6_IFINDEX:
              ret += bprintfrr (buf, "%pI6", &nexthop->gate.ipv6);
              break;
            case NEXTHOP_TYPE_IFINDEX:
            case NEXTHOP_TYPE_BLACKHOLE:
              break;
...
bool
nexthop_is_ifindex_type (const struct nexthop *nh)
{
