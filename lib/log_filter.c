 ...
 */
static int
zlog_filter_lookup (const char *lookup)
{
  for (int i = 0; i < zlog_filter_count; i++)
    {
      if (strncmp (lookup, zlog_filters[i], sizeof (zlog_filters[0])) == 0)
        return i;
    }
  return -1;
}
...
int
zlog_filter_add (const char *filter)
{
  frr_with_mutex (&logfilterlock)
    {
...
int
zlog_filter_del (const char *filter)
{
  frr_with_mutex (&logfilterlock)
    {
...
int
zlog_filter_dump (char *buf, size_t max_size)
{
  int len = 0;
...
static int
search_buf (const char *buf, size_t len)
{
  char *found = NULL;
  ...
  frr_with_mutex (&logfilterlock)
    {
      for (int i = 0; i < zlog_filter_count; i++)
        {
          found = memmem (buf, len, zlog_filters[i], strlen (zlog_filters[i]));
          if (found != NULL)
            return 0;
        }
    }
...
static void
zlog_filterfile_fd (struct zlog_target *zt, struct zlog_msg *msgs[],
                    size_t nmsgs)
{
...
void
zlog_filterfile_init (struct zlog_cfg_filterfile *zcf)
{
...
void
zlog_filterfile_fini (struct zlog_cfg_filterfile *zcf)
{
