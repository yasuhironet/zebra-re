...
/**
 * Read data from a socket into streams containing 1 or more full msgs headed
 *by mgmt_msg_hdr which contain API messages (currently protobuf).
 *
 * Args:
...
mgmt_msg_read (struct mgmt_msg_state *ms, int fd, bool debug)
{
 ...
 */
bool
mgmt_msg_procbufs (struct mgmt_msg_state *ms,
...
mgmt_msg_write (struct mgmt_msg_state *ms, int fd, bool debug)
{
  const char *dbgtag = debug ? ms->idtag : NULL;
  struct stream *s;
  size_t nproc = 0;
  ...
  if (ms->outs)
    {
      MGMT_MSG_DBG (dbgtag, "found unqueued stream with %zu bytes, queueing",
  ...
  for (s = stream_fifo_head (&ms->outq); s && nproc < ms->max_write_buf;
       s = stream_fifo_head (&ms->outq))
    {
      ...
      if (n <= 0)
        {
          if (n == 0)
            MGMT_MSG_ERR (ms, "connection closed while writing");
            ...
            MGMT_MSG_ERR (ms, "error while writing %zd bytes: %s (%d)", left,
      ...
      if (n != left)
        {
          MGMT_MSG_DBG (dbgtag, "short stream write %zd of %zd", n, left);
 ...
 */
int
mgmt_msg_send_msg (struct mgmt_msg_state *ms, uint8_t version, void *msg,
 ...
 */
int
mgmt_msg_connect (const char *path, size_t sendbuf, size_t recvbuf,
                  const char *dbgtag)
{
  int ret, sock, len;
 ...
 */
size_t
mgmt_msg_reset_writes (struct mgmt_msg_state *ms)
{
  struct stream *s;
  size_t nproc = 0;
...
void
mgmt_msg_init (struct mgmt_msg_state *ms, size_t max_read_buf,
...
void
mgmt_msg_destroy (struct mgmt_msg_state *ms)
{
...
static void
msg_conn_write (struct event *thread)
{
...
static void
msg_conn_read (struct event *thread)
{
  struct msg_conn *conn = EVENT_ARG (thread);
...
static void
msg_conn_proc_msgs (struct event *thread)
{
  struct msg_conn *conn = EVENT_ARG (thread);

  if (mgmt_msg_procbufs (
          &conn->mstate,
...
static void
msg_conn_sched_read (struct msg_conn *conn)
{
  event_add_read (conn->loop, msg_conn_read, conn, conn->fd, &conn->read_ev);
}

static void
msg_conn_sched_write (struct msg_conn *conn)
{
...
static void
msg_conn_sched_proc_msgs (struct msg_conn *conn)
{
...
void
msg_conn_disconnect (struct msg_conn *conn, bool reconnect)
{
  ...
  if (reconnect)
    {
      ...
      msg_client_sched_connect (container_of (conn, struct msg_client, conn),
                                MSG_CONN_DEFAULT_CONN_RETRY_MSEC);
    }
}

int
msg_conn_send_msg (struct msg_conn *conn, uint8_t version, void *msg,
                   ...
                   bool short_circuit_ok)
{
  ...
  if (conn->fd == -1)
    {
      MGMT_MSG_ERR (&conn->mstate, "can't send message on closed connection");
      return -1;
    }
  ...
  if (conn->remote_conn && short_circuit_ok)
    {
      ...
      conn->remote_conn->handle_msg (version, buf, n, conn->remote_conn);
...
void
msg_conn_cleanup (struct msg_conn *conn)
{
...
static void
msg_client_connect_timer (struct event *thread)
{
  msg_client_connect (EVENT_ARG (thread));
}

static void
msg_client_sched_connect (struct msg_client *client, unsigned long msec)
{
  ...
  if (msec)
    event_add_timer_msec (conn->loop, msg_client_connect_timer, client, msec,
...
static bool
msg_client_connect_short_circuit (struct msg_client *client)
{
  struct msg_conn *server_conn;
  struct msg_server *server;
  const char *dbgtag = client->conn.debug ? client->conn.mstate.idtag : NULL;
  ...
  if (! server)
    {
      MGMT_MSG_DBG (dbgtag, "no short-circuit connection available for %s",
                    client->sopath);
...
static void
msg_client_connect (struct msg_client *client)
{
  struct msg_conn *conn = &client->conn;
  const char *dbgtag = conn->debug ? conn->mstate.idtag : NULL;
  ...
  if (conn->fd == -1)
    ...
    msg_client_sched_connect (client, MSG_CONN_DEFAULT_CONN_RETRY_MSEC);
...
void
msg_client_cleanup (struct msg_client *client)
{
...
static void
msg_server_accept (struct event *event)
{
  ...
  if (fd < 0)
    {
      zlog_err ("Failed to accept %s client connection: %s", server->idtag,
                safe_strerror (errno));
      return;
    }
  set_nonblocking (fd);
...
int
msg_server_init (struct msg_server *server, const char *sopath,
                 ...
                 const char *idtag, struct debug *debug)
{
  ...
  if (ret < 0)
    {
      zlog_err ("Failed to bind %s server socket to '%s': %s", server->idtag,
  ...
  if (ret < 0)
    {
      zlog_err ("Failed to listen on %s server socket: %s", server->idtag,
                safe_strerror (errno));
      goto fail;
    }
...
void
msg_server_cleanup (struct msg_server *server)
{
                        ...
                        void *user, const char *idtag)
{
  ...
  msg_conn_accept_init (conn, tm, fd, notify_disconnect, handle_msg, max_read,
                        max_write, max_size, idtag);
  conn->user = user;
  return conn;
}

void
msg_server_conn_delete (struct msg_conn *conn)
{
  if (! conn)
    return;
