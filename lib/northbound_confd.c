...
static void
frr_confd_get_xpath (const confd_hkeypath_t *kp, char *xpath, size_t len)
{
  char *p;
...
static int
frr_confd_val2str (const char *xpath, const confd_value_t *value, char *string,
                   size_t string_size)
{
  ...
  if (confd_val2str (csp->info.type, value, string, string_size) == CONFD_ERR)
    {
...
static int
frr_confd_hkeypath_get_list_entry (const confd_hkeypath_t *kp,
  ...
  for (int i = kp->len; i >= 0; i--)
    {
      ...
      for (int j = 0; kp->v[i][j].type != C_NOEXISTS; j++)
        {
          strlcpy (keys.key[keys.num], (char *) kp->v[i][j].val.buf.ptr,
      ...
      if (! CHECK_FLAG (nb_node_list->flags, F_NB_NODE_KEYLESS_LIST))
        {
          *list_entry = nb_callback_lookup_entry (nb_node, *list_entry, &keys);
          if (*list_entry == NULL)
            return -1;
        }
      else
...
static void
getdatetime (struct confd_datetime *datetime)
{
  struct tm tm;
  struct timeval tv;
                         ...
                         confd_value_t *oldv, confd_value_t *newv, void *args)
{
  ...
  switch (cdb_op)
    ...
    case MOP_MODIFIED:
      ...
      return ITER_RECURSE;
    default:
      flog_err (EC_LIB_DEVELOPMENT, "%s: unexpected operation %u [xpath %s]",
                __func__, cdb_op, xpath);
      iter_args->error = true;
      return ITER_STOP;
    }
  ...
  if (nb_node->snode->nodetype != LYS_LEAFLIST && newv &&
                         ...
                         sizeof (value_str)) != 0)
    {
      flog_err (EC_LIB_CONFD_DATA_CONVERT,
                "%s: failed to convert ConfD value to a string", __func__);
      iter_args->error = true;
      return ITER_STOP;
    }
  ...
  ret = nb_candidate_edit (iter_args->candidate, nb_node, nb_op, xpath, NULL,
                           data);
  yang_data_free (data);
  if (ret != NB_OK)
    {
...
static void
frr_confd_cdb_read_cb_prepare (int fd, int *subp, int reslen)
{
  ...
  if (iter_args.error)
    {
      ...
      if (cdb_sub_abort_trans (
              cdb_sub_sock, CONFD_ERRCODE_APPLICATION_INTERNAL, 0, 0,
              "Couldn't apply configuration changes") != CONFD_OK)
        {
  ...
  if (ret != NB_OK && ret != NB_ERR_NO_CHANGES)
    {
      ...
      if (cdb_sub_abort_trans (cdb_sub_sock, errcode, 0, 0, "%s", errmsg) !=
          CONFD_OK)
        {
          flog_err_confd ("cdb_sub_abort_trans");
          return;
...
static void
frr_confd_cdb_read_cb_commit (int fd, int *subp, int reslen)
{
  /*
...
static int
frr_confd_cdb_read_cb_abort (int fd, int *subp, int reslen)
{
  /*
   * No need to process the configuration changes again as we're already
   * keeping track of them in the "transaction" variable.
...
static void
frr_confd_cdb_read_cb (struct event *thread)
{
...
static void *
thread_cdb_trigger_subscriptions (void *data)
{
  int sock;
  ...
  if (sock < 0)
    {
      flog_err (EC_LIB_SOCKET, "%s: failed to create socket: %s", __func__,
                safe_strerror (errno));
      return NULL;
    }
...
static int
frr_confd_subscribe (const struct lysc_node *snode, void *arg)
{
...
frr_confd_init_cdb (void)
{
  ...
  RB_FOREACH (module, yang_modules, &yang_modules)
    {
      ...
      if (module->confd_hash == 0)
        {
          flog_err (EC_LIB_LIBCONFD,
                    "%s: failed to find hash value for namespace %s", __func__,
                    module->info->ns);
          goto error;
        }
       ...
       */
      yang_snodes_iterate (module->info, frr_confd_subscribe, 0, module);
    }
  ...
  if (pthread_create (&cdb_trigger_thread, NULL,
    ...
    {
      flog_err (EC_LIB_SYSTEM_CALL, "%s: error creating pthread: %s", __func__,
                safe_strerror (errno));
      goto error;
    }
...
static int
frr_confd_transaction_init (struct confd_trans_ctx *tctx)
{
...
static int
frr_confd_data_get_elem (struct confd_trans_ctx *tctx, confd_hkeypath_t *kp)
{
  struct nb_node *nb_node;
  char xpath[XPATH_MAXLEN];
  struct yang_data *data;
...
static int
frr_confd_data_get_next (struct confd_trans_ctx *tctx, confd_hkeypath_t *kp,
                         long next)
{
  struct nb_node *nb_node;
  char xpath[XPATH_MAXLEN];
  ...
  if (frr_confd_hkeypath_get_list_entry (kp, nb_node, &parent_list_entry) != 0)
    {
      ...
      if (! CHECK_FLAG (nb_node->flags, F_NB_NODE_KEYLESS_LIST))
        {
          ...
          memset (&keys, 0, sizeof (keys));
          if (nb_callback_get_keys (nb_node, nb_next, &keys) != NB_OK)
            {
 ...
 */
static int
frr_confd_data_get_object (struct confd_trans_ctx *tctx, confd_hkeypath_t *kp)
{
  struct nb_node *nb_node;
  ...
  LY_LIST_FOR (lysc_node_child (nb_node->snode), child)
    {
      ...
      snprintf (xpath_child, sizeof (xpath_child), "%s/%s", xpath,
                child->name);
      data = nb_callback_get_elem (nb_node_child, xpath_child, list_entry);
      if (data)
        {
          if (data->value)
            CONFD_SET_STR (v, data->value);
          else
            {
              ...
              CONFD_SET_XMLTAG (
                  ...
                  confd_str2hash (nb_node_child->snode->module->ns));
            }
 ...
 */
static int
frr_confd_data_get_next_object (struct confd_trans_ctx *tctx,
  ...
  for (int j = 0; j < CONFD_OBJECTS_PER_TIME; j++)
    {
      ...
      nb_next = nb_callback_get_next (nb_node, parent_list_entry, nb_next);
      if (! nb_next)
        /* End of the list. */
        break;
      ...
      LY_LIST_FOR (lysc_node_child (nb_node->snode), child)
        {
          ...
          snprintf (xpath_child, sizeof (xpath_child), "%s/%s", xpath,
                    child->name);
          data = nb_callback_get_elem (nb_node_child, xpath_child, nb_next);
          if (data)
            {
              if (data->value)
                CONFD_SET_STR (v, data->value);
...
static int
frr_confd_notification_send (const char *xpath, struct list *arguments)
{
  struct nb_node *nb_node;
  struct yang_module *module;
      ...
      if (! nb_node_arg)
        {
          flog_warn (EC_LIB_YANG_UNKNOWN_DATA_PATH,
                     "%s: unknown data path: %s", __func__, data->xpath);
          XFREE (MTYPE_CONFD, values);
          return NB_ERR;
        }

      CONFD_SET_TAG_STR (&values[i++], nb_node_arg->confd_hash, data->value);
    }
  CONFD_SET_TAG_XMLEND (&values[i++], nb_node->confd_hash, module->confd_hash);
...
static int
frr_confd_action_init (struct confd_user_info *uinfo)
{
...
static int
frr_confd_action_execute (struct confd_user_info *uinfo, struct xml_tag *name,
                          confd_hkeypath_t *kp, confd_tag_value_t *params,
                          int nparams)
{
  char xpath[XPATH_MAXLEN];
  struct nb_node *nb_node;
  ...
  if (kp)
  ...
  else
    {
      ...
      snprintf (xpath, sizeof (xpath), "/%s:%s", confd_ns2prefix (name->ns),
                confd_hash2str (name->tag));
    }
  ...
  if (nb_callback_rpc (nb_node, xpath, input, output, errmsg,
                       sizeof (errmsg)) != NB_OK)
    {
      flog_warn (EC_LIB_NB_CB_RPC, "%s: rpc callback failed: %s", __func__,
                 xpath);
      ret = CONFD_ERR;
      goto exit;
    }
...
static int
frr_confd_dp_read (struct confd_daemon_ctx *dctx, int fd)
{
  int ret;
...
static void
frr_confd_dp_ctl_read (struct event *thread)
{
...
static void
frr_confd_dp_worker_read (struct event *thread)
{
  struct confd_daemon_ctx *dctx = EVENT_ARG (thread);
  int fd = EVENT_FD (thread);
...
static int
frr_confd_subscribe_state (const struct lysc_node *snode, void *arg)
{
...
static int
frr_confd_init_dp (const char *program_name)
{
  ...
  if (confd_register_notification_stream (dctx, &ncbs, &live_ctx) != CONFD_OK)
    {
  ...
  event_add_read (master, frr_confd_dp_ctl_read, dctx, dp_ctl_sock, &t_dp_ctl);
...
static int
frr_confd_debug_config_write (struct vty *vty)
{
...
static int
frr_confd_debug_set_all (uint32_t flags, bool set)
{
...
static void
frr_confd_cli_init (void)
{
  hook_register (nb_client_debug_config_write, frr_confd_debug_config_write);
...
static int
frr_confd_calculate_snode_hash (const struct lysc_node *snode, void *arg)
{
  struct nb_node *nb_node = snode->priv;
...
static int
frr_confd_init (const char *program_name)
{
...
static int
frr_confd_module_late_init (struct event_loop *tm)
{
  master = tm;

  if (frr_confd_init (frr_get_progname ()) < 0)
    {
      flog_err (EC_LIB_CONFD_INIT, "failed to initialize the ConfD module");
      return -1;
    }
