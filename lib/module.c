...
void
frrmod_init (struct frrmod_runtime *modinfo)
{
...
frrmod_load (const char *spec, const char *dir,
  ...
  do                                                                          \
    {                                                                         \
      if (pFerrlog && (iErr < FRRMOD_LOAD_N_ERRSTR))                          \
        {                                                                     \
          aErr[iErr++] = asprintfrr (MTYPE_MODULE_LOAD_ERR, __VA_ARGS__);     \
        }                                                                     \
    }                                                                         \
  while (0)
  ...
  if (! strchr (name, '/'))
    {
      if (execname)
        {
          snprintf (fullpath, sizeof (fullpath), "%s/%s_%s.so", dir, execname,
                    name);
          handle = dlopen (fullpath, RTLD_NOW | RTLD_GLOBAL);
          if (! handle)
            ERR_RECORD ("loader error: dlopen(%s): %s", fullpath, dlerror ());
        }
      if (! handle)
        {
