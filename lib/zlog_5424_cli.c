...
static int
target_cmp (const struct zlog_cfg_5424_user *a,
...
static struct zlog_cfg_5424_user *
...
static void
log_5424_free (struct zlog_cfg_5424_user *cfg, bool keepopen)
{
...
static void
clear_dst (struct zlog_cfg_5424_user *cfg)
{
  XFREE (MTYPE_LOG_5424_DATA, cfg->filename);
...
static int
reconf_dst (struct zlog_cfg_5424_user *cfg, struct vty *vty)
{
...
static int
reconf_meta (struct zlog_cfg_5424_user *cfg, struct vty *vty)
{
  if (! cfg->reconf_dst && ! cfg->reconf_meta && vty->type != VTY_FILE)
    vty_out (vty,
             "%% Changes will be applied when exiting this config block\n");
...
static int
reconf_clear_dst (struct zlog_cfg_5424_user *cfg, struct vty *vty)
{
...
static enum zlog_5424_format
log_5424_fmt (const char *fmt, enum zlog_5424_format dflt)
{
  if (! fmt)
    return dflt;
       ...
       FORMAT_HELP)
{
  ...
  if (perms)
    {
      ...
      if (*errp || errp == perms || perm_val == 0 || (perm_val & ~0666))
        {
  ...
  dst = (strcmp (type, "fifo") == 0) ? ZLOG_5424_DST_FIFO : ZLOG_5424_DST_FILE;

  if (cfg->filename && ! strcmp (path, cfg->filename) && dst == cfg->cfg.dst &&
      ...
      if (user)
        {
          ...
          if (! pwent)
            vty_out (
                ...
                user,
                errno ? safe_strerror (errno) : "No entry by this user name");
          else
      ...
      if (group)
        {
          ...
          if (! grent)
            vty_out (
                ...
                group,
                errno ? safe_strerror (errno) : "No entry by this group name");
          else
...
static int
dst_unix (struct vty *vty, const char *no, const char *path,
          enum zlog_5424_format fmt, enum unix_special special)
{
  ...
  if (cfg->cfg.dst == ZLOG_5424_DST_UNIX && cfg->filename &&
      ! strcmp (path, cfg->filename) && cfg->cfg.active && cfg->cfg.fmt == fmt)
    return CMD_SUCCESS;
...
{
  return dst_unix (vty, no, "/run/systemd/journal/socket", ZLOG_FMT_JOURNALD,
                   SPECIAL_JOURNALD);
}
       ...
       FORMAT_HELP)
{
  ...
  if (envvar_problem)
    vty_out (vty, "%% environment variable \"%s\" not present or invalid.\n",
             envvar);
...
static int
log_5424_node_exit (struct vty *vty)
{
  VTY_DECLVAR_CONTEXT (zlog_cfg_5424_user, cfg);
...
static int
log_5424_config_write (struct vty *vty)
{
  struct zlog_cfg_5424_user *cfg;
      ...
      switch (cfg->cfg.dst)
        ...
        case ZLOG_5424_DST_FIFO:
          ...
          vty_out (vty, " destination %s %s",
                   (cfg->cfg.dst == ZLOG_5424_DST_FIFO) ? "fifo" : "file",
                   cfg->filename);
          ...
          if (cfg->cfg.file_nocreate)
                   ...
                   cfg->cfg.file_mode != (LOGFILE_MASK & 0666))
            {
              ...
              if (cfg->file_group)
                vty_out (vty, " group %s", cfg->file_group);
              if (cfg->cfg.file_mode != (LOGFILE_MASK & 0666))
                vty_out (vty, " mode %04o", cfg->cfg.file_mode);
            }
          vty_out (vty, "%s\n", fmt_str);
          break;
          ...
          switch (cfg->unix_special)
            ...
            case SPECIAL_SYSLOG:
              if (cfg->cfg.fmt == ZLOG_FMT_DEV_LOG)
                ...
                vty_out (vty, " destination syslog supports-rfc5424\n");
              break;
  ...
  frr_each (targets, &targets, cfg)
    {
      ...
      switch (cfg->cfg.dst)
        ...
        case ZLOG_5424_DST_UNIX:
          vty_out (vty, "  logging to %s: %pSE\n",
                   ...
                   : (cfg->cfg.dst == ZLOG_5424_DST_UNIX) ? "unix socket"
                                                          : "file",
                   cfg->filename);
          break;
        }
          ...
          for (struct log_option *opt = log_opts; opt->name; opt++)
            {
              ...
              if (*ptr)
                {
                  vty_out (vty, "%s%s", first ? "" : ", ", opt->name);
                  first = false;
                }
            }
          break;
      ...
      zlog_5424_state (&cfg->cfg, &lost_msgs, &last_errno, &stale_errno,
                       &err_ts);
...
static void
log_5424_autocomplete (vector comps, struct cmd_token *token)
{
  struct zlog_cfg_5424_user *cfg;
...
__attribute__ ((_CONSTRUCTOR (475))) static void
zlog_5424_startup_init (void)
{
...
static int
log_5424_early_init (struct event_loop *master)
{
...
static int
log_5424_rotate (void)
{
  struct zlog_cfg_5424_user *cfg;

  frr_each (targets, &targets, cfg)
    if (! zlog_5424_rotate (&cfg->cfg))
      zlog_err ("log rotation on extended log target %s failed", cfg->name);
