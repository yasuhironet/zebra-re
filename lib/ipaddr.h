...
#define IPADDR_STRING_SIZE 46
  ...
  static inline int
  ipaddr_family (const struct ipaddr *ip)
  {
    switch (ip->ipa_type)
      {
      case IPADDR_V4:
  ...
  static inline char *
  ipaddr2str (const struct ipaddr *ip, char *buf, int size)
  {
    buf[0] = '\0';
    if (ip)
...
#define IS_MAPPED_IPV6(A)                                                     \
   ...
   */
  static inline void
  ipv4_to_ipv4_mapped_ipv6 (struct in6_addr *in6, struct in_addr in)
  {
   ...
   */
  static inline void
  ipv4_mapped_ipv6_to_ipv4 (const struct in6_addr *in6, struct in_addr *in)
  {
   ...
   */
  static inline int
  ipaddr_cmp (const struct ipaddr *a, const struct ipaddr *b)
  {
    uint32_t va, vb;
    va = a->ipa_type;
    vb = b->ipa_type;
  ...
  static inline bool
  ipaddr_is_zero (const struct ipaddr *ip)
  {
    switch (ip->ipa_type)
      {
      case IPADDR_NONE:
