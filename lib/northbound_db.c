...
nb_db_init (void)
{
  ...
  if (db_execute (
          ...
          "    END;\n"
          "COMMIT;",
          NB_DLFT_MAX_CONFIG_ROLLBACKS, NB_DLFT_MAX_CONFIG_ROLLBACKS) != 0)
    return NB_ERR;
...
int
nb_db_transaction_save (const struct nb_transaction *transaction,
                        uint32_t *transaction_id)
{
  ...
  ss = db_prepare ("INSERT INTO transactions\n"
...
nb_db_transaction_load (uint32_t transaction_id)
{
  ...
  if (err || ! dnode)
    flog_warn (EC_LIB_LIBYANG, "%s: lyd_parse_data_mem() failed", __func__);
  else
...
int
nb_db_clear_transactions (unsigned int n_oldest)
{
#ifdef HAVE_CONFIG_ROLLBACKS
...
int
nb_db_set_max_transactions (unsigned int max)
{
#ifdef HAVE_CONFIG_ROLLBACKS
  /*
...
int
nb_db_transactions_iterate (void (*func) (void *arg, int transaction_id,
                                          const char *client_name,
                                          const char *date,
                                          const char *comment),
                            void *arg)
  ...
  while (db_run (ss) == SQLITE_ROW)
    {
      ...
      ret = db_loadf (ss, "%i%s%s%s", &transaction_id, &client_name, &date,
                      &comment);
      if (ret != 0)
        continue;
