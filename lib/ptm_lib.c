...
static csv_record_t *
_ptm_lib_encode_header (csv_t *csv, csv_record_t *rec, int msglen, int version,
                        int type, int cmd_id, char *client_name)
{
  ...
  if (rec)
    {
      rec1 = csv_encode_record (csv, rec, 5, msglen_buf, vers_buf, type_buf,
                                cmdid_buf, client_buf);
    }
  else
    {
      rec1 = csv_encode (csv, 5, msglen_buf, vers_buf, type_buf, cmdid_buf,
                         client_buf);
    }
  return (rec1);
}
...
static int
_ptm_lib_decode_header (csv_t *csv, int *msglen, int *version, int *type,
...
int
ptm_lib_append_msg (ptm_lib_handle_t *hdl, void *ctxt, const char *key,
                    const char *val)
{
...
int
ptm_lib_init_msg (ptm_lib_handle_t *hdl, int cmd_id, int type, void *in_ctxt,
                  void **out_ctxt)
{
  ...
  rec = _ptm_lib_encode_header (csv, NULL, 0, PTMLIB_MSG_VERSION, type, cmd_id,
                                hdl->client_name);
...
int
ptm_lib_cleanup_msg (ptm_lib_handle_t *hdl, void *ctxt)
{
  ptm_lib_msg_ctxt_t *p_ctxt = ctxt;
  csv_t *csv;
...
int
ptm_lib_complete_msg (ptm_lib_handle_t *hdl, void *ctxt, char *buf, int *len)
{
  ptm_lib_msg_ctxt_t *p_ctxt = ctxt;
  csv_t *csv;
  csv_record_t *rec;
...
ptm_lib_find_key_in_msg (void *ctxt, const char *key, char *val)
{
  ...
  for (hstr = csv_field_iter (hrec, &hfld),
      dstr = csv_field_iter (drec, &dfld);
       (hstr && dstr);
       hstr = csv_field_iter_next (&hfld), dstr = csv_field_iter_next (&dfld))
    {
...
int
ptm_lib_process_msg (ptm_lib_handle_t *hdl, int fd, char *inbuf, int inlen,
                     void *arg)
{
  int rc, len;
  char client_name[32];
  ...
  rc =
      _ptm_lib_decode_header (csv, &msglen, &ver, &type, &cmd_id, client_name);
...
ptm_lib_handle_t *
ptm_lib_register (char *client_name, ptm_cmd_cb cmd_cb,
                  ptm_notify_cb notify_cb, ptm_response_cb response_cb)
{
