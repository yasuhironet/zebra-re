...
void
sbuf_init (struct sbuf *dest, char *buf, size_t size)
{
  dest->fixed = (size > 0);
  if (dest->fixed)
    {
...
void
sbuf_push (struct sbuf *buf, int indent, const char *format, ...)
{
  va_list args;
  int written;
  ...
  written =
      snprintf (buf->buf + buf->pos, buf->size - buf->pos, "%*s", indent, "");
  ...
  va_start (args, format);
  written =
      vsnprintfrr (buf->buf + buf->pos, buf->size - buf->pos, format, args);
  va_end (args);
