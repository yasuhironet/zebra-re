...
struct spf_backoff *
spf_backoff_new (struct event_loop *m, const char *name, long init_delay,
                 long short_delay, long long_delay, long holddown,
                 long timetolearn)
{
...
static void
spf_backoff_timetolearn_elapsed (struct event *thread)
{
...
static void
spf_backoff_holddown_elapsed (struct event *thread)
{
  struct spf_backoff *backoff = EVENT_ARG (thread);
...
long
spf_backoff_schedule (struct spf_backoff *backoff)
{
  long rv = 0;
  struct timeval now;
  ...
  backoff_debug ("SPF Back-off(%s) schedule called in state %s", backoff->name,
  ...
  switch (backoff->state)
    ...
    case SPF_BACKOFF_QUIET:
      ...
      event_add_timer_msec (backoff->m, spf_backoff_holddown_elapsed, backoff,
...
timeval_format (struct timeval *tv)
{
  ...
  if (! tm ||
      strftime (timebuf, sizeof (timebuf), "%Z %a %Y-%m-%d %H:%M:%S", tm) == 0)
    {
      return "???";
    }
