...
static uint32_t
qobj_hash (const struct qobj_node *node)
{
  return (uint32_t) node->nid;
}

static int
qobj_cmp (const struct qobj_node *na, const struct qobj_node *nb)
{
  if (na->nid < nb->nid)
    return -1;
  if (na->nid > nb->nid)
...
void
qobj_reg (struct qobj_node *node, const struct qobj_nodetype *type)
{
  node->type = type;
...
void
qobj_unreg (struct qobj_node *node)
{
  pthread_rwlock_wrlock (&nodes_lock);
...
void *
qobj_get_typed (uint64_t id, const struct qobj_nodetype *type)
{
