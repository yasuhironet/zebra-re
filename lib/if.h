...
#define IFF_VIRTUAL 0x0
  ...
  if_lookup_address_local (const void *matchaddr, int family, vrf_id_t vrf_id);
  extern struct connected *if_lookup_address (const void *matchaddr,
  ...
  static inline bool
  if_address_is_local (const void *matchaddr, int family, vrf_id_t vrf_id)
  {
    return if_lookup_address_local (matchaddr, family, vrf_id) != NULL;
  }

  struct vrf;
  extern struct interface *if_lookup_by_name_vrf (const char *name,
                                                  struct vrf *vrf);
  ...
  connected_lookup_prefix_exact (struct interface *, const struct prefix *);
  extern unsigned int connected_count_by_family (struct interface *,
                                                 int family);
  extern struct nbr_connected *nbr_connected_new (void);
  extern void nbr_connected_free (struct nbr_connected *);
  struct nbr_connected *nbr_connected_check (struct interface *,
                                             struct prefix *);
