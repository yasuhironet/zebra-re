...
#define SRV6_LOCATOR_USID (1 << 0) /* The SRv6 Locator is a uSID Locator */
  ...
  static inline bool
  sid_same (const struct in6_addr *a, const struct in6_addr *b)
  {
    if (! a && ! b)
      return true;
    else if (! (a && b))
  ...
  static inline bool
  sid_diff (const struct in6_addr *a, const struct in6_addr *b)
  {
    return ! sid_same (a, b);
  }

  static inline bool
  sid_zero (const struct in6_addr *a)
  {
  ...
  static inline void *
  sid_copy (struct in6_addr *dst, const struct in6_addr *src)
  {
  ...
  int snprintf_seg6_segs (char *str, size_t size,
                          const struct seg6_segs *segs);
