...
static inline int
ns_map_compare (const struct ns_map_nsid *a, const struct ns_map_nsid *b)
{
...
static struct ns *
ns_get_created_internal (struct ns *ns, char *name, ns_id_t ns_id)
{
  int created = 0;
  /*
 ...
 */
static int
ns_enable_internal (struct ns *ns, void (*func) (ns_id_t, void *))
{
  if (! ns_is_enabled (ns))
    {
      if (have_netns ())
      ...
      if (! ns_is_enabled (ns))
        {
          flog_err_sys (EC_LIB_SYSTEM_CALL, "Can not enable NS %u: %s!",
      ...
      if (ns_debug)
        {
          if (have_netns ())
            zlog_info ("NS %u is associated with NETNS %s.", ns->ns_id,
                       ns->name);
 ...
 */
static void
ns_disable_internal (struct ns *ns)
{
  if (ns_is_enabled (ns))
    {
      if (ns_debug)
...
ns_id_t
ns_map_nsid_with_external (ns_id_t ns_id, bool map)
{
  struct ns_map_nsid *ns_map;
...
struct ns *
ns_get_created (struct ns *ns, char *name, ns_id_t ns_id)
{
...
int
ns_enable (struct ns *ns, void (*func) (ns_id_t, void *))
{
...
void
ns_walk_func (int (*func) (struct ns *, void *param_in, void **param_out),
...
void
ns_add_hook (int type, int (*func) (struct ns *))
{
  switch (type)
    {
    case NS_NEW_HOOK:
  ...
  if (! result)
    {
      if (vty)
        ...
        flog_warn (EC_LIB_LINUX_NS, "Invalid pathname for %s: %s", pathname,
                   safe_strerror (errno));
      return NULL;
    }
  ...
  if (check_base != NULL && strlen (check_base) + 1 > NS_NAMSIZ)
    {
      if (vty)
        ...
        flog_warn (EC_LIB_LINUX_NS, "NS name (%s) invalid: too long (>%d)",
...
void
ns_init_management (ns_id_t default_ns_id, ns_id_t internal_ns)
{
  int fd;
  ...
  if (! default_ns)
    {
      flog_err (EC_LIB_NS, "%s: failed to create the default NS!", __func__);
      exit (1);
    }
  if (have_netns ())
    {
  ...
  if (! ns_enable (default_ns, NULL))
    {
      flog_err (EC_LIB_NS, "%s: failed to enable the default NS!", __func__);
      exit (1);
    }
}
...
int
ns_switch_to_netns (const char *name)
{
  int ret;
  int fd;
 ...
 */
ns_id_t
ns_id_get_absolute (ns_id_t ns_id_reference, ns_id_t link_nsid)
{
  struct ns *ns;
