...
extern "C"
{
   ...
   * Deletes a node.
   *
   * Before deletion, this function removes all edges to and from this node
   * from any neighbor nodes.
   *
  ...
  /**
   * Deletes a graph.
   * Calls graph_delete_node on each node before freeing the graph struct
   * itself.
   *
   ...
   * libfrr since clippy itself is required to build libfrr. Instead it
   * directly includes the sources it needs. One of these is the command graph
   * implementation, which wraps this graph implementation. Since we need to
   * use the buffer.[ch] sources here, which indirectly rely on most of libfrr,
   * we have to ignore them when compiling clippy to avoid build dependency
   * issues.
   *
   ...
   *
   * The generated output is produced from a depth-first traversal of the
   * graph.
   *
   ...
   *        will use the default printer. See graph_dump_dot_default_print_cb
   * for an example.
