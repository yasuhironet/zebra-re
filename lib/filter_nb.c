...
/* Helper function. */
static void
acl_notify_route_map (struct access_list *acl, int route_map_event)
{
...
prefix_list_length_validate (struct nb_cb_modify_args *args)
{
 ...
 */
static void
prefix_list_entry_set_empty (struct prefix_list_entry *ple)
{
  ple->any = false;
                                    ...
                                    char *errmsg, size_t errmsg_len)
{
  ...
  if (af_type != YPLT_IPV4)
    {
      snprintf (errmsg, errmsg_len, "prefix-list type %u is mismatched.",
                af_type);
      return NB_ERR_VALIDATION;
    }
...
static int
lib_prefix_list_entry_prefix_length_greater_or_equal_modify (
...
static int
lib_prefix_list_entry_prefix_length_lesser_or_equal_modify (
    struct nb_cb_modify_args *args)
{
  struct prefix_list_entry *ple;
...
static int
lib_prefix_list_entry_prefix_length_greater_or_equal_destroy (
...
static int
lib_prefix_list_entry_prefix_length_lesser_or_equal_destroy (
    struct nb_cb_destroy_args *args)
{
  struct prefix_list_entry *ple;
 ...
 */
static void
cisco_unset_addr_mask (struct in_addr *addr, struct in_addr *mask)
{
...
static int
_acl_is_dup (const struct lyd_node *dnode, void *arg)
{
  struct acl_dup_args *ada = arg;
  int idx;

  /* This entry is the caller, so skip it. */
  if (ada->ada_entry_dnode && ada->ada_entry_dnode == dnode)
  ...
  for (idx = 0; idx < ADA_MAX_VALUES; idx++)
    {
      ...
      if (strcmp (yang_dnode_get_string (dnode, "%s", ada->ada_xpath[idx]),
                  ada->ada_value[idx]))
        return YANG_ITER_CONTINUE;
    }
...
bool
acl_is_dup (const struct lyd_node *dnode, struct acl_dup_args *ada)
{
  ada->ada_found = false;

  yang_dnode_iterate (
      _acl_is_dup, ada, dnode,
      "/frr-filter:lib/access-list[type='%s'][name='%s']/entry", ada->ada_type,
      ada->ada_name);
...
static bool
acl_cisco_is_dup (const struct lyd_node *dnode)
{
  const struct lyd_node *entry_dnode = yang_dnode_get_parent (dnode, "entry");
  struct acl_dup_args ada = {};
  int idx = 0, arg_idx = 0;
  static const char *cisco_entries[] = { "./host",
                                         "./network/address",
                                         "./network/mask",
                                         "./source-any",
                                         "./destination-host",
  ...
  while (cisco_entries[idx] != NULL)
    {
      ...
      ada.ada_value[arg_idx] =
          yang_dnode_get_string (entry_dnode, "%s", cisco_entries[idx]);
      arg_idx++;
      idx++;
    }
...
static bool
acl_zebra_is_dup (const struct lyd_node *dnode,
                  enum yang_access_list_type type)
{
  const struct lyd_node *entry_dnode = yang_dnode_get_parent (dnode, "entry");
  struct acl_dup_args ada = {};
  int idx = 0, arg_idx = 0;
  static const char *zebra_entries[] = { "./ipv4-prefix",
                                         "./ipv4-exact-match",
                                         "./ipv6-prefix",
                                         "./ipv6-exact-match",
                                         "./mac",
  ...
  while (zebra_entries[idx] != NULL)
    {
      ...
      ada.ada_value[arg_idx] =
          yang_dnode_get_string (entry_dnode, "%s", zebra_entries[idx]);
      arg_idx++;
      idx++;
    }
...
static void
plist_dnode_to_prefix (const struct lyd_node *dnode, bool *any,
  ...
  *le = 0;
  ...
  switch (yang_dnode_get_enum (dnode, "../type"))
    ...
    case YPLT_IPV4:
      ...
      if (yang_dnode_exists (dnode, "./ipv4-prefix-length-greater-or-equal"))
        *ge = yang_dnode_get_uint8 (dnode,
                                    "./ipv4-prefix-length-greater-or-equal");
      if (yang_dnode_exists (dnode, "./ipv4-prefix-length-lesser-or-equal"))
        *le = yang_dnode_get_uint8 (dnode,
                                    "./ipv4-prefix-length-lesser-or-equal");
      break;
    case YPLT_IPV6:
      yang_dnode_get_prefix (p, dnode, "./ipv6-prefix");
      if (yang_dnode_exists (dnode, "./ipv6-prefix-length-greater-or-equal"))
        *ge = yang_dnode_get_uint8 (dnode,
                                    "./ipv6-prefix-length-greater-or-equal");
      if (yang_dnode_exists (dnode, "./ipv6-prefix-length-lesser-or-equal"))
        *le = yang_dnode_get_uint8 (dnode,
...
static int
_plist_is_dup (const struct lyd_node *dnode, void *arg)
{
  ...
  /* This entry is the caller, so skip it. */
  if (pda->pda_entry_dnode && pda->pda_entry_dnode == dnode)
    return YANG_ITER_CONTINUE;
  ...
  if (pda->any)
    ...
    {
      if (! prefix_same (&pda->prefix, &p) || pda->ge != ge || pda->le != le)
        return YANG_ITER_CONTINUE;
    }
...
bool
plist_is_dup (const struct lyd_node *dnode, struct plist_dup_args *pda)
{
  pda->pda_found = false;

  yang_dnode_iterate (
      _plist_is_dup, pda, dnode,
      "/frr-filter:lib/prefix-list[type='%s'][name='%s']/entry", pda->pda_type,
      pda->pda_name);
 ...
 */
static int
lib_access_list_create (struct nb_cb_create_args *args)
{
...
static int
lib_access_list_destroy (struct nb_cb_destroy_args *args)
{
 ...
 */
static int
lib_access_list_remark_modify (struct nb_cb_modify_args *args)
{
  struct access_list *acl;
  const char *remark;
 ...
 */
static int
lib_access_list_entry_create (struct nb_cb_create_args *args)
{
  struct access_list *acl;
  struct filter *f;
...
static int
lib_access_list_entry_destroy (struct nb_cb_destroy_args *args)
{
  struct access_list *acl;
  struct filter *f;
...
lib_access_list_entry_ipv4_prefix_modify (struct nb_cb_modify_args *args)
{
  ...
  if (args->event == NB_EV_VALIDATE)
    {
      if (acl_zebra_is_dup (args->dnode,
 ...
 */
static int
lib_access_list_entry_destination_host_modify (struct nb_cb_modify_args *args)
{
  struct filter_cisco *fc;
  struct filter *f;
 ...
 */
static int
lib_access_list_entry_destination_any_create (struct nb_cb_create_args *args)
{
  struct filter_cisco *fc;
  struct filter *f;
...
static int
lib_access_list_entry_destination_any_destroy (struct nb_cb_destroy_args *args)
{
  struct filter_cisco *fc;
  struct filter *f;
 ...
 */
static int
lib_access_list_entry_any_create (struct nb_cb_create_args *args)
{
  struct filter_zebra *fz;
  struct filter *f;
  int type;
...
static int
lib_access_list_entry_any_destroy (struct nb_cb_destroy_args *args)
{
  struct filter_zebra *fz;
  struct filter *f;
 ...
 */
static int
lib_prefix_list_create (struct nb_cb_create_args *args)
{
...
static int
lib_prefix_list_destroy (struct nb_cb_destroy_args *args)
{
 ...
 */
static int
lib_prefix_list_remark_modify (struct nb_cb_modify_args *args)
{
  struct prefix_list *pl;
  const char *remark;
...
static int
lib_prefix_list_remark_destroy (struct nb_cb_destroy_args *args)
{
  struct prefix_list *pl;
 ...
 */
static int
lib_prefix_list_entry_create (struct nb_cb_create_args *args)
{
  struct prefix_list_entry *ple;
  struct prefix_list *pl;
...
static int
lib_prefix_list_entry_destroy (struct nb_cb_destroy_args *args)
{
  struct prefix_list_entry *ple;
 ...
 */
static int
lib_prefix_list_entry_action_modify (struct nb_cb_modify_args *args)
{
  struct prefix_list_entry *ple;
  int action_type;
...
static int
lib_prefix_list_entry_prefix_modify (struct nb_cb_modify_args *args)
{
  struct prefix_list_entry *ple;
  struct prefix p;
  ...
  if (! prefix_same (&ple->prefix, &p))
    {
      zlog_info ("%s: bad network %pFX correcting it to %pFX", __func__,
                 &ple->prefix, &p);
...
static int
lib_prefix_list_entry_prefix_destroy (struct nb_cb_destroy_args *args)
{
  struct prefix_list_entry *ple;
  ...
  if (args->event == NB_EV_VALIDATE)
    {
      ...
      return prefix_list_nb_validate_v4_af_type (plist_dnode, args->errmsg,
                                                 args->errmsg_len);
    }
  ...
  if (args->event == NB_EV_VALIDATE)
    {
      ...
      return prefix_list_nb_validate_v6_af_type (plist_dnode, args->errmsg,
                                                 args->errmsg_len);
    }
    ...
    struct nb_cb_modify_args *args)
{
  ...
  return lib_prefix_list_entry_prefix_length_greater_or_equal_modify (args);
}
    ...
    struct nb_cb_destroy_args *args)
{
  ...
  return lib_prefix_list_entry_prefix_length_greater_or_equal_destroy (args);
}
    ...
    struct nb_cb_destroy_args *args)
{
  ...
  return lib_prefix_list_entry_prefix_length_lesser_or_equal_destroy (args);
}
 ...
 */
static int
lib_prefix_list_entry_any_create (struct nb_cb_create_args *args)
{
  struct prefix_list_entry *ple;
  int type;
...
static int
lib_prefix_list_entry_any_destroy (struct nb_cb_destroy_args *args)
{
  struct prefix_list_entry *ple;
