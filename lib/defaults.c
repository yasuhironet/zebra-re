...
int
frr_version_cmp (const char *aa, const char *bb)
{
...
void
frr_default_add (struct frr_default *dflt)
{
  dflt->next = NULL;
  *dflt_next = dflt;
  dflt_next = &dflt->next;
...
static bool
frr_match_version (const char *name, const char *vspec, const char *version,
                   bool check)
{
  int cmp;
  ...
  if (! s->str)
    {
      if (check)
        fprintf (stderr, "invalid version specifier for %s: %s", name, vspec);
...
static void
frr_default_apply_one (struct frr_default *dflt, bool check)
{
  ...
  for (; entry->match_version || entry->match_profile; entry++)
    {
      if (entry->match_profile && strcmp (entry->match_profile, df_profile))
        continue;

      if (! dfltentry && frr_match_version (dflt->name, entry->match_version,
                                            df_version, check))
        dfltentry = entry;
      if (! saveentry && frr_match_version (dflt->name, entry->match_version,
                                            FRR_VER_SHORT, check))
        saveentry = entry;
...
bool
frr_defaults_profile_valid (const char *profile)
{
  const char **p;
...
const char *
frr_defaults_version (void)
{
  return df_version;
}

const char *
frr_defaults_profile (void)
{
  return df_profile;
}

void
frr_defaults_version_set (const char *version)
{
...
void
frr_defaults_profile_set (const char *profile)
{
