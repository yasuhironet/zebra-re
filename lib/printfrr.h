...
#define atm(a, b) atn (a, b) __attribute__ ((malloc))
  ...
  ssize_t vsnprintfrr (char *out, size_t sz, const char *fmt, va_list)
      at (3, 0);
  ...
  ssize_t vcsnprintfrr (char *out, size_t sz, const char *fmt, va_list)
      at (3, 0);
...
#define printfrr(fmt, ...)                                                    \
  do                                                                          \
    {                                                                         \
      char buf[256], *out;                                                    \
      out = asnprintfrr (MTYPE_TMP, buf, sizeof (buf), fmt, ##__VA_ARGS__);   \
      fputs (out, stdout);                                                    \
      if (out != buf)                                                         \
        XFREE (MTYPE_TMP, out);                                               \
    }                                                                         \
...
#define printfrr_ext_char(ch) ((ch) >= 'A' && (ch) <= 'Z')
  ...
  printfrr_ext_len (struct printfrr_eargs *ea)
  {
    ssize_t rv;
...
#define printfrr_ext_autoreg_i(matchs, print_fn)                              \
  ...
  static inline ssize_t
  bputs (struct fbuf *buf, const char *str)
  {
  ...
  static inline ssize_t
  bputch (struct fbuf *buf, char ch)
  {
  ...
  static inline ssize_t
  bputhex (struct fbuf *buf, uint8_t val)
  {
