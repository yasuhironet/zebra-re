...
#define AF_FLOWSPEC (AF_MAX + 2)
  ...
  static inline int
  is_evpn_prefix_ipaddr_none (const struct prefix_evpn *evp)
  {
  ...
  static inline int
  is_evpn_prefix_ipaddr_v4 (const struct prefix_evpn *evp)
  {
    if (evp->prefix.route_type == BGP_EVPN_AD_ROUTE)
  ...
  static inline int
  is_evpn_prefix_ipaddr_v6 (const struct prefix_evpn *evp)
  {
    if (evp->prefix.route_type == BGP_EVPN_AD_ROUTE)
...
#define IPV4_ADDR_CMP(D, S) memcmp ((D), (S), IPV4_MAX_BYTELEN)
  ...
  static inline bool
  ipv4_addr_same (const struct in_addr *a, const struct in_addr *b)
  {
...
#define IPV4_ADDR_SAME(A, B) ipv4_addr_same ((A), (B))
  ...
  static inline void
  ipv4_addr_copy (struct in_addr *dst, const struct in_addr *src)
  {
...
#define s6_addr32 __u6_addr.__u6_addr32
   ...
   */
  extern unsigned int prefix_bit (const uint8_t *prefix,
...
#define PREFIX2STR_BUFFER PREFIX_STRLEN
  ...
  extern int prefix_match (union prefixconstptr unet,
  ...
  extern int prefix_common_bits (union prefixconstptr ua,
...
#define prefix_copy(a, b)                                                     \
  ...
  static inline int
  ipv6_martian (const struct in6_addr *addr)
  {
  ...
  static inline bool
  ipv4_martian (const struct in_addr *addr)
  {
  ...
  static inline bool
  is_default_prefix4 (const struct prefix_ipv4 *p)
  {
  ...
  static inline bool
  is_default_prefix6 (const struct prefix_ipv6 *p)
  {
  ...
  static inline int
  is_host_route (const struct prefix *p)
  {
    if (p->family == AF_INET)
  ...
  static inline int
  is_default_host_route (const struct prefix *p)
  {
    if (p->family == AF_INET)
      {
  ...
  static inline bool
  is_ipv6_global_unicast (const struct in6_addr *p)
  {
...
enum {
  ...
  ipv6_mcast_scope (const struct in6_addr *addr)
  {
    return addr->s6_addr[1] & 0xf;
  }

  static inline bool
  ipv6_mcast_nofwd (const struct in6_addr *addr)
  {
    return (addr->s6_addr[1] & 0xf) <= MCAST_SCOPE_LINK;
  }

  static inline bool
  ipv6_mcast_ssm (const struct in6_addr *addr)
  {
  ...
  static inline bool
  ipv6_mcast_reserved (const struct in6_addr *addr)
  {
    uint32_t bits = ntohl (addr->s6_addr32[0]);
  ...
  static inline uint8_t
  ipv4_mcast_scope (const struct in_addr *addr)
  {
  ...
  static inline bool
  ipv4_mcast_nofwd (const struct in_addr *addr)
  {
    uint32_t bits = ntohl (addr->s_addr);
  ...
  static inline bool
  ipv4_mcast_ssm (const struct in_addr *addr)
  {
    uint32_t bits = ntohl (addr->s_addr);
