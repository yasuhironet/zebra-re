...
#define BFD_PROFILE_NAME_LEN 64
   ...
   */
  struct bfd_session_params *bfd_sess_new (bsp_status_update updatecb,
                                           void *args);
   ...
   *
   * NOTE:
   * If the interface changed the session is removed and must be installed
   * again with `bfd_sess_install`.
   *
   * \param bsp BFD session parameters.
   * \param ifname interface name (or `NULL` to remove it).
   */
  void bfd_sess_set_interface (struct bfd_session_params *bsp,
                               const char *ifname);
   ...
   */
  void bfd_sess_set_profile (struct bfd_session_params *bsp,
                             const char *profile);
   ...
   *
   * NOTE:
   * This function uninstalls the session immediately (if installed) and
   * cancels any previous `bfd_sess_install` calls.
   *
   * \param bsp session parameters.
   */
   ...
   * \see bfd_session_arg.
   */
  extern int zclient_bfd_command (struct zclient *zc,
