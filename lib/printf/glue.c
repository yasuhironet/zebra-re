...
ssize_t
bprintfrr (struct fbuf *out, const char *fmt, ...)
{
  ssize_t ret;
  va_list ap;
...
ssize_t
vsnprintfrr (char *out, size_t outsz, const char *fmt, va_list ap)
{
  struct fbuf fbb = {
    .buf = out,
    .pos = out,
...
ssize_t
snprintfrr (char *out, size_t outsz, const char *fmt, ...)
{
  struct fbuf fbb = {
    .buf = out,
    .pos = out,
...
ssize_t
vcsnprintfrr (char *out, size_t outsz, const char *fmt, va_list ap)
{
  if (! out || ! outsz)
...
ssize_t
csnprintfrr (char *out, size_t outsz, const char *fmt, ...)
{
  ssize_t ret;
  va_list ap;
...
char *
vasnprintfrr (struct memtype *mt, char *out, size_t outsz, const char *fmt,
              va_list ap)
{
  struct fbuf fb = {
    .buf = out,
...
char *
asnprintfrr (struct memtype *mt, char *out, size_t outsz, const char *fmt, ...)
{
  va_list ap;
  char *ret;
...
char *
vasprintfrr (struct memtype *mt, const char *fmt, va_list ap)
{
  char buf[256];
  char *ret;
...
char *
asprintfrr (struct memtype *mt, const char *fmt, ...)
{
  char buf[256];
  va_list ap;
  char *ret;
...
void
printfrr_ext_reg (const struct printfrr_ext *ext)
{
  uint8_t o;
  ptrdiff_t i;
  ...
  o = ext->match[0] - 'A';
  for (i = ext_offsets[o]; i < MAXEXT && entries[i].fmt[0] &&
  ...
  memmove (exts + i + 1, exts + i, (MAXEXT - i - 1) * sizeof (exts[0]));
...
ssize_t
printfrr_extp (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
...
ssize_t
printfrr_exti (struct fbuf *buf, struct printfrr_eargs *ea, uintmax_t num)
{
  const char *fmt = ea->fmt;
  const struct printfrr_ext *ext;
  size_t i;
...
static ssize_t
printfrr_fb (struct fbuf *out, struct printfrr_eargs *ea, const void *ptr)
{
  ...
  if (out)
    {
      copy_len = MIN (in->pos - in->buf, out->buf + out->len - out->pos);
      if (copy_len > 0)
        {
...
static ssize_t
printfrr_va (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
