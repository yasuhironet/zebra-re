...
static int
resolver_fd_cmp (const struct resolver_fd *a, const struct resolver_fd *b)
{
  return numcmp (a->fd, b->fd);
}

static uint32_t
resolver_fd_hash (const struct resolver_fd *item)
{
...
static struct resolver_fd *
resolver_fd_get (int fd, struct resolver_state *newstate)
{
...
static void
resolver_fd_drop_maybe (struct resolver_fd *resfd)
{
...
static void
resolver_cb_timeout (struct event *t)
{
...
static void
resolver_cb_socket_readable (struct event *t)
{
...
static void
resolver_cb_socket_writable (struct event *t)
{
  struct resolver_fd *resfd = EVENT_ARG (t);
  struct resolver_state *r = resfd->state;

  event_add_write (r->master, resolver_cb_socket_writable, resfd, resfd->fd,
                   &resfd->t_write);
  /* ^ ordering important:
...
static void
resolver_update_timeouts (struct resolver_state *r)
{
  ...
  if (tv)
    {
      ...
      event_add_timer_msec (r->master, resolver_cb_timeout, r, timeoutms,
                            &r->timeout);
    }
}

static void
ares_socket_cb (void *data, ares_socket_t fd, int readable, int writable)
{
  ...
  if (! readable)
    ...
    event_add_read (r->master, resolver_cb_socket_readable, resfd, fd,
                    &resfd->t_read);

  if (! writable)
    EVENT_OFF (resfd->t_write);
  else if (! resfd->t_write)
    event_add_write (r->master, resolver_cb_socket_writable, resfd, fd,
                     &resfd->t_write);
...
static void
ares_address_cb (void *arg, int status, int timeouts, struct hostent *he)
{
  ...
  for (i = 0; i < array_size (addr) && he->h_addr_list[i] != NULL; i++)
    {
      ...
      switch (he->h_addrtype)
        {
        case AF_INET:
          memcpy (&addr[i].sin.sin_addr, (uint8_t *) he->h_addr_list[i],
                  he->h_length);
          break;
        case AF_INET6:
          memcpy (&addr[i].sin6.sin6_addr, (uint8_t *) he->h_addr_list[i],
                  he->h_length);
          break;
        }
    }
...
static void
resolver_cb_literal (struct event *t)
{
...
void
resolver_resolve (struct resolver_query *query, int af, vrf_id_t vrf_id,
                  const char *hostname,
                  void (*callback) (struct resolver_query *, const char *, int,
                                    union sockunion *))
{
  ...
  if (ret == 0)
    {
      if (resolver_debug)
        zlog_debug ("[%p] Resolving '%s' (IP literal)", query, hostname);

      /* for consistency with proper name lookup, don't call the
       * callback immediately; defer to thread loop
       */
      event_add_timer_msec (state.master, resolver_cb_literal, query, 0,
                            &query->literal_cb);
      return;
    }
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "%s: Can't switch to VRF %u (%s)", __func__,
...
static int
resolver_config_write_debug (struct vty *vty)
{
  if (resolver_debug)
...
void
resolver_init (struct event_loop *tm)
{
