...
#define frrzmq_event_add_write_msg(m, f, e, a, z, d)                          \
  _xref_zmq_a (WRITE, f, d, _frrzmq_event_add_write (&_xref, m, f, e, a, z, d))
   ...
   * ZMQ_EVENTS after each invocation of zmq_send or zmq_recv.To be more
   * explicit: after calling zmq_send the socket may become readable (and vice
   * versa) without triggering a read event on the file descriptor.
   */
  extern void frrzmq_check_events (struct frrzmq_cb **cbp,
                                   struct cb_core *core, int event);
