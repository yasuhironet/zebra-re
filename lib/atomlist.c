...
void
atomlist_add_head (struct atomlist_head *h, struct atomlist_item *item)
{
  atomptr_t prevval;
  ...
  while (! atomic_compare_exchange_weak_explicit (
      &h->first, &prevval, i, memory_order_release, memory_order_relaxed))
    atomic_store_explicit (&item->next, prevval, memory_order_relaxed);
}

void
atomlist_add_tail (struct atomlist_head *h, struct atomlist_item *item)
{
  ...
  while (1)
    {
      ...
      if (! atomic_compare_exchange_strong_explicit (
              prev, &prevval, i, memory_order_consume, memory_order_consume))
        {
          hint = prevval;
          continue;
        }
...
static void
atomlist_del_core (struct atomlist_head *h, struct atomlist_item *item,
  ...
  while (1)
    {
      ...
      if (! atomic_compare_exchange_strong_explicit (
              upd, &updval, next, memory_order_consume, memory_order_consume))
        {
...
void
atomlist_del_hint (struct atomlist_head *h, struct atomlist_item *item,
...
atomlist_pop (struct atomlist_head *h)
{
  ...
  do
    {
      ...
      item->next = prevval;
      if (atomic_compare_exchange_strong_explicit (
              prev, &prevval, i, memory_order_release, memory_order_relaxed))
        break;
    }
  while (1);
...
static void
atomsort_del_core (struct atomsort_head *h, struct atomsort_item *item,
                   _Atomic atomptr_t *hint, atomptr_t next)
{
  _Atomic atomptr_t *prev = hint ? hint : &h->first, *upd;
  atomptr_t prevval, updval;
  ...
  while (1)
    {
      ...
      if (! atomic_compare_exchange_strong_explicit (
              upd, &updval, next, memory_order_relaxed, memory_order_relaxed))
        {
          /* prev doesn't point to item anymore, something
           * was inserted.  continue at same position forward.
           */
...
void
atomsort_del_hint (struct atomsort_head *h, struct atomsort_item *item,
                   _Atomic atomptr_t *hint)
{
  atomptr_t next;
...
struct atomsort_item *
atomsort_pop (struct atomsort_head *h)
{
