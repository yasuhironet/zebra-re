...
#define event_add_read(m, f, a, v, t)                                         \
  _xref_t_a (read_write, READ, m, f, a, v, t)
...
#define event_execute(m, f, a, v)                                             \
  ...
  static inline bool
  event_is_scheduled (struct event *thread)
  {
    if (thread)
      return true;
  ...
  static inline void
  event_ignore_late_timer (struct event *event)
  {
