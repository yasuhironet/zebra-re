...
void
yang_module_embed (struct yang_module_embed *embed)
{
  embed->next = NULL;
  *embedupd = embed;
  embedupd = &embed->next;
...
static LY_ERR
yang_module_imp_clb (const char *mod_name, const char *mod_rev,
...
static inline int
yang_module_compare (const struct yang_module *a, const struct yang_module *b)
{
...
yang_module_load (const char *module_name)
{
  ...
  if (! module_info)
    {
      flog_err (EC_LIB_YANG_MODULE_LOAD, "%s: failed to load data model: %s",
...
int
yang_snodes_iterate_subtree (const struct lysc_node *snode,
  ...
  switch (snode->nodetype)
    ...
    default:
      assert (snode->nodetype != LYS_AUGMENT &&
              snode->nodetype != LYS_GROUPING && snode->nodetype != LYS_USES);
      break;
    }
...
int
yang_snodes_iterate (const struct lys_module *module, yang_iterate_cb cb,
  ...
  while ((module_iter = ly_ctx_get_module_iter (ly_native_ctx, &idx)))
    {
      ...
      LY_LIST_FOR (module_iter->compiled->data, snode)
        {
          ret = yang_snodes_iterate_subtree (snode, module, cb, flags, arg);
          if (ret == YANG_ITER_STOP)
            return ret;
        }
...
void
yang_snode_get_path (const struct lysc_node *snode, enum yang_path_type type,
                     char *xpath, size_t xpath_len)
{
  switch (type)
    ...
    case YANG_PATH_DATA:
      ...
      break;
    default:
      flog_err (EC_LIB_DEVELOPMENT, "%s: unknown yang path type: %u", __func__,
                type);
      exit (1);
    }
}
...
struct lysc_node *
yang_find_snode (struct ly_ctx *ly_ctx, const char *xpath, uint32_t options)
{
  struct lysc_node *snode;
  struct ly_set *set;
  LY_ERR err;
...
struct lysc_node *
yang_snode_real_parent (const struct lysc_node *snode)
{
...
struct lysc_node *
yang_snode_parent_list (const struct lysc_node *snode)
{
  struct lysc_node *parent = snode->parent;
...
bool
yang_snode_is_typeless_data (const struct lysc_node *snode)
{
...
const char *
yang_snode_get_default (const struct lysc_node *snode)
{
  const struct lysc_node_leaf *sleaf;
  ...
  switch (snode->nodetype)
    ...
    case LYS_LEAF:
      ...
      return sleaf->dflt
                 ? lyd_value_get_canonical (sleaf->module->ctx, sleaf->dflt)
                 : NULL;
    case LYS_LEAFLIST:
...
unsigned int
yang_snode_num_keys (const struct lysc_node *snode)
{
...
void
yang_dnode_get_path (const struct lyd_node *dnode, char *xpath,
                     size_t xpath_len)
{
...
const char *
yang_dnode_get_schema_name (const struct lyd_node *dnode,
...
struct lyd_node *
yang_dnode_get (const struct lyd_node *dnode, const char *xpath)
{
...
struct lyd_node *
yang_dnode_getf (const struct lyd_node *dnode, const char *xpath_fmt, ...)
{
  va_list ap;
  char xpath[XPATH_MAXLEN];
...
bool
yang_dnode_exists (const struct lyd_node *dnode, const char *xpath)
{
  struct ly_set *set = NULL;
  bool exists = false;
...
bool
yang_dnode_existsf (const struct lyd_node *dnode, const char *xpath_fmt, ...)
{
  va_list ap;
  char xpath[XPATH_MAXLEN];
...
void
yang_dnode_iterate (yang_dnode_iter_cb cb, void *arg,
...
bool
yang_dnode_is_default (const struct lyd_node *dnode, const char *xpath)
{
...
bool
yang_dnode_is_defaultf (const struct lyd_node *dnode, const char *xpath_fmt,
                        ...)
{
  if (! xpath_fmt)
...
bool
yang_dnode_is_default_recursive (const struct lyd_node *dnode)
{
...
void
yang_dnode_change_leaf (struct lyd_node *dnode, const char *value)
{
...
struct lyd_node *
yang_dnode_new (struct ly_ctx *ly_ctx, bool config_only)
{
...
struct lyd_node *
yang_dnode_dup (const struct lyd_node *dnode)
{
...
void
yang_dnode_free (struct lyd_node *dnode)
{
  while (dnode->parent)
...
struct yang_data *
yang_data_new (const char *xpath, const char *value)
{
...
void
yang_data_free (struct yang_data *data)
{
  if (data->value)
    free (data->value);
...
struct yang_data *
yang_data_list_find (const struct list *list, const char *xpath_fmt, ...)
{
  char xpath[XPATH_MAXLEN];
  struct yang_data *data;
...
static void
ly_log_cb (LY_LOG_LEVEL level, const char *msg, const char *path)
{
...
const char *
yang_print_errors (struct ly_ctx *ly_ctx, char *buf, size_t buf_len)
{
...
void
yang_debugging_set (bool enable)
{
  if (enable)
    {
...
struct ly_ctx *
yang_ctx_new_setup (bool embedded_modules, bool explicit_compile)
{
...
void
yang_init (bool embedded_modules, bool defer_compile)
{
...
void
yang_init_loading_complete (void)
{
  /* Compile everything */
...
bool
yang_is_last_list_dnode (const struct lyd_node *dnode)
{
...
bool
yang_is_last_level_dnode (const struct lyd_node *dnode)
{
      ...
      switch (node->schema->nodetype)
        ...
        case LYS_CONTAINER:
          if (! CHECK_FLAG (node->schema->flags, LYS_PRESENCE))
            {
              if (node->parent &&
                  (node->parent->schema->module == dnode->schema->module))
...
uint32_t
yang_get_list_pos (const struct lyd_node *node)
{
  return lyd_list_pos (node);
}

uint32_t
yang_get_list_elements_count (const struct lyd_node *node)
{
  unsigned int count;
