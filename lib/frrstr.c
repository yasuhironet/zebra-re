...
void
frrstr_split (const char *string, const char *delimiter, char ***result,
              int *argc)
{
  if (! string)
    return;
  ...
  while (copy)
    {
      ...
      if (++idx == sz)
        *result = XREALLOC (MTYPE_TMP, *result, (sz *= 2) * sizeof (char *));
      (*argc)++;
    }
...
vector
frrstr_split_vec (const char *string, const char *delimiter)
{
  char **result;
  int argc;
...
char *
frrstr_join (const char **parts, int argc, const char *join)
{
  int i;
  char *str;
  char *p;
...
char *
frrstr_join_vec (vector v, const char *join)
{
  char **argv;
  int argc;
...
void
frrstr_filter_vec (vector v, regex_t *filter)
{
...
char *
frrstr_replace (const char *str, const char *find, const char *replace)
{
  char *ch;
...
bool
frrstr_startswith (const char *str, const char *prefix)
{
  if (! str || ! prefix)
    return false;
...
bool
frrstr_endswith (const char *str, const char *suffix)
{
  if (! str || ! suffix)
    return false;
...
char *
frrstr_hex (char *buff, size_t bufsiz, const uint8_t *str, size_t num)
{
  if (bufsiz == 0)
    return buff;
