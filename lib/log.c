...
size_t
frr_timestamp (int timestamp_precision, char *buf, size_t buflen)
{
  static struct
  {
    time_t last;
  ...
  if (cache.last != clock.tv_sec)
    {
      ...
      cache.len =
          strftime (cache.buf, sizeof (cache.buf), "%Y/%m/%d %H:%M:%S", &tm);
    }
...
void
zlog_signal (int signo, const char *action, void *siginfo_v,
             void *program_counter)
{
  ...
  char buf
      [sizeof (
           "DEFAULT: Received signal S at T (si_addr 0xP, PC 0xP); aborting...") +
       100];
  ...
  if (program_counter)
    bprintfrr (&fb, " (si_addr 0x%tx, PC 0x%tx)", (ptrdiff_t) siginfo->si_addr,
  ...
  if (! tc)
    ...
    bprintfrr (&fb, "in thread %s scheduled from %s:%d %s()\n",
               tc->xref->funcname, tc->xref->xref.file, tc->xref->xref.line,
               tc->xref->xref.func);
  ...
  while (unw_step (&cursor) > 0)
    {
      ...
      if (dladdr ((void *) ip, &dlinfo))
        zlog (priority, "%-30s %16lx %16lx %s (mapped at %p)", name, (long) ip,
              (long) sp, dlinfo.dli_fname, dlinfo.dli_fbase);
      else
        zlog (priority, "%-30s %16lx %16lx", name, (long) ip, (long) sp);
    }
#elif defined(HAVE_GLIBC_BACKTRACE)
  void *array[20];
  int size, i;
...
void
zlog_thread_info (int log_level)
{
  struct event *tc;
  tc = pthread_getspecific (thread_current);
...
zroute_lookup (unsigned int zroute)
{
  ...
  if (zroute >= array_size (route_types))
    {
      flog_err (EC_LIB_DEVELOPMENT, "unknown zebra route type: %u", zroute);
      return &unknown;
    }
...
const char *
zebra_route_string (unsigned int zroute)
{
  return zroute_lookup (zroute)->string;
}

char
zebra_route_char (unsigned int zroute)
{
...
zserv_command_string (unsigned int command)
{
  if (command >= array_size (command_types))
    {
      flog_err (EC_LIB_DEVELOPMENT, "unknown zserv command type: %u", command);
...
const char *
zserv_gr_client_cap_string (uint32_t zcc)
{
...
void
zlog_hexdump (const void *mem, size_t len)
{
  char line[64];
