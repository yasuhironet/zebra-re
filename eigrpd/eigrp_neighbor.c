...
void
holddown_timer_expired (struct event *thread)
{
...
void
eigrp_nbr_state_set (struct eigrp_neighbor *nbr, uint8_t state)
{
  nbr->state = state;
    ...
    case EIGRP_NEIGHBOR_PENDING:
      {
        ...
        EVENT_OFF (nbr->t_holddown);
        event_add_timer (master, holddown_timer_expired, nbr, nbr->v_holddown,
                         &nbr->t_holddown);
        break;
      }
    case EIGRP_NEIGHBOR_UP:
...
int
eigrp_nbr_count_get (struct eigrp *eigrp)
{
 ...
 */
void
eigrp_nbr_hard_restart (struct eigrp_neighbor *nbr, struct vty *vty)
{
  struct eigrp *eigrp = nbr->ei->eigrp;
...
int
eigrp_nbr_split_horizon_check (struct eigrp_route_descriptor *ne,
                               struct eigrp_interface *ei)
{
