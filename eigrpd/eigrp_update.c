...
bool
eigrp_update_prefix_apply (struct eigrp *eigrp, struct eigrp_interface *ei,
 ...
 */
static void
eigrp_update_receive_GR_ask (struct eigrp *eigrp, struct eigrp_neighbor *nbr,
 ...
 */
void
eigrp_update_receive (struct eigrp *eigrp, struct ip *iph,
                      ...
                      struct eigrp_interface *ei, int size)
{
  ...
  if (IS_DEBUG_EIGRP_PACKET (0, RECV))
    zlog_debug (
        ...
        size, ifindex2ifname (nbr->ei->ifp->ifindex, eigrp->vrf_id), &nbr->src,
      ...
      if (nbr->state == EIGRP_NEIGHBOR_UP)
        {
          ...
          nbr->recv_sequence_number = ntohl (eigrph->sequence);
          zlog_info ("Neighbor %pI4 (%s) is down: peer restarted", &nbr->src,
                     ifindex2ifname (nbr->ei->ifp->ifindex, eigrp->vrf_id));
          eigrp_nbr_state_set (nbr, EIGRP_NEIGHBOR_PENDING);
          zlog_info ("Neighbor %pI4 (%s) is pending: new adjacency", &nbr->src,
                     ifindex2ifname (nbr->ei->ifp->ifindex, eigrp->vrf_id));
          eigrp_update_send_init (nbr);
        }
    }
          ...
          if (dest != NULL)
          ...
          else
            {
              ...
              ne->reported_distance =
                  eigrp_calculate_metrics (eigrp, tlv->metric);
              /*
               * Filtering
               */
...
static void
eigrp_update_place_on_nbr_queue (struct eigrp_neighbor *nbr,
                                 struct eigrp_packet *ep, uint32_t seq_no,
                                 int length)
{
  if ((nbr->ei->params.auth_type == EIGRP_AUTH_TYPE_MD5) &&
      (nbr->ei->params.auth_keychain != NULL))
...
static void
eigrp_update_send_to_all_nbrs (struct eigrp_interface *ei,
                               struct eigrp_packet *ep)
{
...
eigrp_update_send_all (struct eigrp *eigrp, struct eigrp_interface *exception)
{
  ...
  for (ALL_LIST_ELEMENTS (eigrp->topology_changes_internalIPV4, node2, nnode2,
                          pe))
    {
      if (pe->req_action & EIGRP_FSM_NEED_UPDATE)
        {
 ...
 */
void
eigrp_update_send_GR_thread (struct event *thread)
{
  struct eigrp_neighbor *nbr;
  ...
  if (nbr->retrans_queue->count > 0)
    {
      event_add_timer_msec (master, eigrp_update_send_GR_thread, nbr, 10,
                            &nbr->t_nbr_send_gr);
      return;
    }
 ...
 */
void
eigrp_update_send_GR (struct eigrp_neighbor *nbr, enum GR_type gr_type,
                      struct vty *vty)
{
  ...
  if (gr_type == EIGRP_GR_FILTER)
    {
      ...
      zlog_info ("Neighbor %pI4 (%s) is resync: route configuration changed",
                 &nbr->src, ifindex2ifname (ei->ifp->ifindex, eigrp->vrf_id));
    }
  else if (gr_type == EIGRP_GR_MANUAL)
    {
      /* Graceful restart was called manually */
      zlog_info ("Neighbor %pI4 (%s) is resync: manually cleared", &nbr->src,
                 ifindex2ifname (ei->ifp->ifindex, eigrp->vrf_id));

      if (vty != NULL)
        {
          vty_time_print (vty, 0);
          vty_out (vty, "Neighbor %pI4 (%s) is resync: manually cleared\n",
                   &nbr->src,
                   ifindex2ifname (ei->ifp->ifindex, eigrp->vrf_id));
        }
    }
 ...
 */
void
eigrp_update_send_interface_GR (struct eigrp_interface *ei,
 ...
 */
void
eigrp_update_send_process_GR (struct eigrp *eigrp, enum GR_type gr_type,
                              struct vty *vty)
{
  struct listnode *node;
