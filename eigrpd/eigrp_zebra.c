...
static int
eigrp_router_id_update_zebra (ZAPI_CALLBACK_ARGS)
{
  struct eigrp *eigrp;
...
static int
eigrp_zebra_route_notify_owner (ZAPI_CALLBACK_ARGS)
{
  struct prefix p;
...
eigrp_zebra_init (void)
{
  ...
  zclient =
      zclient_new (master, &opt, eigrp_handlers, array_size (eigrp_handlers));
...
static int
eigrp_zebra_read_route (ZAPI_CALLBACK_ARGS)
{
...
static int
eigrp_interface_address_add (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
...
static int
eigrp_interface_address_delete (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
...
void
eigrp_zebra_route_add (struct eigrp *eigrp, struct prefix *p,
...
void
eigrp_zebra_route_delete (struct eigrp *eigrp, struct prefix *p)
{
  struct zapi_route api;
...
static int
eigrp_is_type_redistributed (int type, vrf_id_t vrf_id)
{
...
int
eigrp_redistribute_set (struct eigrp *eigrp, int type,
