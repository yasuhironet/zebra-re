...
void
eigrp_cli_show_header (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
...
void
eigrp_cli_show_end_header (struct vty *vty, const struct lyd_node *dnode)
{
  vty_out (vty, "exit\n");
  vty_out (vty, "!\n");
}
...
void
eigrp_cli_show_router_id (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
 ...
 */
DEFPY_YANG (eigrp_passive_interface,
...
{
  if (no)
    nb_cli_enqueue_change (vty, "./passive-interface", NB_OP_DESTROY, ifname);
  else
    nb_cli_enqueue_change (vty, "./passive-interface", NB_OP_CREATE, ifname);

  return nb_cli_apply_changes (vty, NULL);
}

void
eigrp_cli_show_passive_interface (struct vty *vty,
 ...
 */
DEFPY_YANG (eigrp_timers_active,
            ...
            "Disable time limit for active state\n")
{
  if (disabled)
    ...
    nb_cli_enqueue_change (vty, "./active-time", NB_OP_MODIFY, timer_str);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_eigrp_timers_active,
...
void
eigrp_cli_show_active_time (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
void
eigrp_cli_show_variance (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
 ...
 */
DEFPY_YANG (eigrp_maximum_paths,
...
DEFPY_YANG (no_eigrp_maximum_paths,
...
void
eigrp_cli_show_maximum_paths (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
 ...
 */
DEFPY_YANG (eigrp_metric_weights,
            ...
            "K6\n")
{
  ...
  if (k6)
    nb_cli_enqueue_change (vty, "./metric-weights/K6", NB_OP_MODIFY, k6_str);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_eigrp_metric_weights,
...
void
eigrp_cli_show_metrics (struct vty *vty, const struct lyd_node *dnode,
                        bool show_defaults)
{
...
{
  if (no)
    nb_cli_enqueue_change (vty, "./network", NB_OP_DESTROY, prefix_str);
  else
    nb_cli_enqueue_change (vty, "./network", NB_OP_CREATE, prefix_str);

  return nb_cli_apply_changes (vty, NULL);
}

void
eigrp_cli_show_network (struct vty *vty, const struct lyd_node *dnode,
                        bool show_defaults)
{
...
{
  if (no)
    nb_cli_enqueue_change (vty, "./neighbor", NB_OP_DESTROY, addr_str);
  else
    nb_cli_enqueue_change (vty, "./neighbor", NB_OP_CREATE, addr_str);

  return nb_cli_apply_changes (vty, NULL);
}

void
eigrp_cli_show_neighbor (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
  const char *prefix = yang_dnode_get_string (dnode, NULL);
 ...
 */
DEFPY_YANG (eigrp_redistribute_source_metric,
            eigrp_redistribute_source_metric_cmd,
            "[no] redistribute "
            FRR_REDIST_STR_EIGRPD
...
void
eigrp_cli_show_redistribute (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
  ...
  if (bw || rlbt || delay || load || mtu)
    vty_out (vty, " metric %s %s %s %s %s", bw, delay, rlbt, load, mtu);
  vty_out (vty, "\n");
}
...
DEFPY_YANG (no_eigrp_if_delay,
            ...
            "Throughput delay (tens of microseconds)\n")
{
  nb_cli_enqueue_change (vty, "./frr-eigrpd:eigrp/delay", NB_OP_DESTROY, NULL);
  return nb_cli_apply_changes (vty, NULL);
}

void
eigrp_cli_show_delay (struct vty *vty, const struct lyd_node *dnode,
                      bool show_defaults)
{
...
DEFPY_YANG (no_eigrp_if_bandwidth,
            ...
            "Bandwidth in kilobits\n")
{
  nb_cli_enqueue_change (vty, "./frr-eigrpd:eigrp/bandwidth", NB_OP_DESTROY,
                         NULL);
  return nb_cli_apply_changes (vty, NULL);
}

void
eigrp_cli_show_bandwidth (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
 ...
 */
DEFPY_YANG (eigrp_if_ip_hellointerval,
...
DEFPY_YANG (no_eigrp_if_ip_hellointerval,
...
void
eigrp_cli_show_hello_interval (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
 ...
 */
DEFPY_YANG (eigrp_if_ip_holdinterval,
...
DEFPY_YANG (no_eigrp_if_ip_holdinterval,
            ...
            "Seconds before neighbor is considered down\n")
{
  nb_cli_enqueue_change (vty, "./frr-eigrpd:eigrp/hold-time", NB_OP_DESTROY,
                         NULL);
  return nb_cli_apply_changes (vty, NULL);
}

void
eigrp_cli_show_hold_time (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
 ...
 */
DEFPY_YANG (eigrp_ip_summary_address,
...
DEFPY_YANG (no_eigrp_ip_summary_address,
...
void
eigrp_cli_show_summarize_address (struct vty *vty,
                                  const struct lyd_node *dnode,
                                  bool show_defaults)
{
 ...
 */
DEFPY_YANG (eigrp_authentication_mode,
...
DEFPY_YANG (no_eigrp_authentication_mode,
...
void
eigrp_cli_show_authentication (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
  const struct lyd_node *instance = yang_dnode_get_parent (dnode, "instance");
  uint16_t asn = yang_dnode_get_uint16 (instance, "./asn");
 ...
 */
DEFPY_YANG (eigrp_authentication_keychain,
...
DEFPY_YANG (no_eigrp_authentication_keychain,
...
void
eigrp_cli_show_keychain (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
  const struct lyd_node *instance = yang_dnode_get_parent (dnode, "instance");
  uint16_t asn = yang_dnode_get_uint16 (instance, "./asn");
