...
static int
eigrp_ifp_create (struct interface *ifp)
{
  struct eigrp_interface *ei = ifp->info;
...
static int
eigrp_ifp_up (struct interface *ifp)
{
  struct eigrp_interface *ei = ifp->info;
...
static int
eigrp_ifp_down (struct interface *ifp)
{
  if (IS_DEBUG_EIGRP (zebra, ZEBRA_INTERFACE))
...
static int
eigrp_ifp_destroy (struct interface *ifp)
{
  if (if_is_up (ifp))
 ...
 */
static void
eigrp_mtu_convert (struct eigrp_metrics *metric, uint32_t host_mtu)
{
...
bool
eigrp_if_is_passive (struct eigrp_interface *ei)
{
