...
static void
eigrp_vty_display_prefix_entry (struct vty *vty, struct eigrp *eigrp,
                                struct eigrp_prefix_descriptor *pe, bool all)
{
  bool first = true;
  ...
  for (ALL_LIST_ELEMENTS_RO (pe->entries, node, te))
    {
      if (all || (((te->flags & EIGRP_ROUTE_DESCRIPTOR_SUCCESSOR_FLAG) ==
...
static struct eigrp *
eigrp_vty_get_eigrp (struct vty *vty, const char *vrf_name)
{
  struct vrf *vrf;
...
static void
eigrp_topology_helper (struct vty *vty, struct eigrp *eigrp, const char *all)
{
...
static void
eigrp_interface_helper (struct vty *vty, struct eigrp *eigrp,
...
static void
eigrp_neighbors_helper (struct vty *vty, struct eigrp *eigrp,
                        const char *ifname, const char *detail)
{
  struct eigrp_interface *ei;
      ...
      for (ALL_LIST_ELEMENTS (ei->nbrs, node2, nnode2, nbr))
        {
          if (nbr->state != EIGRP_NEIGHBOR_DOWN)
            {
              zlog_debug (
                  "Neighbor %pI4 (%s) is down: manually cleared", &nbr->src,
