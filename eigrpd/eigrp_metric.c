  ...
  if (scaled != EIGRP_CLASSIC_MAX)
    {
      ...
      bandwidth =
          (bandwidth < EIGRP_METRIC_MAX) ? bandwidth : EIGRP_BANDWIDTH_MAX;
    }
...
eigrp_calculate_metrics (struct eigrp *eigrp, struct eigrp_metrics metric)
{
  ...
  if (eigrp->k_values[2])
    composite +=
        ((eigrp_metric_t) eigrp->k_values[2] * (eigrp_metric_t) metric.delay);
                               ...
                               struct eigrp_route_descriptor *entry)
{
  ...
  entry->total_metric.bandwidth =
      entry->total_metric.bandwidth > bw ? bw : entry->total_metric.bandwidth;
                       ...
                       struct eigrp_metrics metric2)
{
  if ((metric1.bandwidth == metric2.bandwidth) &&
      ...
      (metric1.mtu[1] == metric2.mtu[1]) && (metric1.mtu[2] == metric2.mtu[2]))
    {
      return true;
    }
