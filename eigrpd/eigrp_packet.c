...
static int
eigrp_retrans_count_exceeded (struct eigrp_packet *ep,
...
int
eigrp_make_sha256_digest (struct eigrp_interface *ei, struct stream *s,
                          uint8_t flags)
{
  struct key *key = NULL;
  struct keychain *keychain;
...
void
eigrp_write (struct event *thread)
{
...
#define EIGRP_WRITE_IPHL_SHIFT 2
  ...
  if (ret < 0)
    zlog_warn (
        ...
        &iph.ip_dst, iph.ip_id, iph.ip_off, iph.ip_len, ei->ifp->name,
...
out:
  ...
  if (! list_isempty (eigrp->oi_write_q))
    {
      event_add_write (master, eigrp_write, eigrp, eigrp->fd, &eigrp->t_write);
    }
}

/* Starting point of packet process function. */
void
eigrp_read (struct event *thread)
{
  int ret;
  struct stream *ibuf;
  struct eigrp *eigrp;
  ...
  */
  ...
  if (ntohs (eigrph->ASNumber) != eigrp->AS)
    {
      if (IS_DEBUG_EIGRP_TRANSMIT (0, RECV))
        zlog_debug (
            ...
            ntohs (eigrph->vrid), &iph->ip_dst, ntohs (eigrph->ASNumber));
      return;
    }
  ...
  if (eigrp_if_is_passive (ei))
    {
      if (IS_DEBUG_EIGRP_TRANSMIT (0, RECV))
        zlog_debug (
            ...
            ntohs (eigrph->vrid), &iph->ip_dst, &ei->address.u.prefix4);
  ...
  if (IS_DEBUG_EIGRP_TRANSMIT (0, RECV))
    zlog_debug (
        ...
        ntohl (eigrph->sequence), ntohl (eigrph->ack), length, IF_NAME (ei),
      ...
      if ((ep) && (ntohl (eigrph->ack) == ep->sequence_number))
        {
          ...
          if ((nbr->state == EIGRP_NEIGHBOR_PENDING) &&
              (ntohl (eigrph->ack) == nbr->init_sequence_number))
            {
              ...
              zlog_info ("Neighbor(%pI4) adjacency became full", &nbr->src);
...
eigrp_recv_packet (struct eigrp *eigrp, int fd, struct interface **ifp,
                   struct stream *ibuf)
{
  int ret;
  struct ip *iph;
...
struct eigrp_packet *
eigrp_packet_new (size_t size, struct eigrp_neighbor *nbr)
{
  ...
  if (ep)
    {
      ...
      event_add_write (master, eigrp_write, nbr->ei->eigrp, nbr->ei->eigrp->fd,
...
/* Make EIGRP header. */
void
eigrp_packet_header_init (int type, struct eigrp *eigrp, struct stream *s,
...
void
eigrp_fifo_push (struct eigrp_fifo *fifo, struct eigrp_packet *ep)
{
  ep->next = fifo->head;
  ep->previous = NULL;
...
struct eigrp_packet *
eigrp_fifo_next (struct eigrp_fifo *fifo)
{
  return fifo->tail;
}
...
void
eigrp_unack_packet_retrans (struct event *thread)
{
  struct eigrp_neighbor *nbr;
...
void
eigrp_unack_multicast_packet_retrans (struct event *thread)
{
  struct eigrp_neighbor *nbr;
  nbr = (struct eigrp_neighbor *) EVENT_ARG (thread);
  ...
  if (ep)
    {
      ...
      /*Start retransmission timer*/
      event_add_timer (master, eigrp_unack_multicast_packet_retrans, nbr,
                       EIGRP_PACKET_RETRANS_TIME, &ep->t_retrans_timer);
...
static struct TLV_IPv4_Internal_type *
...
uint16_t
eigrp_add_authTLV_MD5_to_stream (struct stream *s, struct eigrp_interface *ei)
{
  struct key *key;
  struct keychain *keychain;
  struct TLV_MD5_Authentication_Type *authTLV;
