 ...
 */
void
eigrp_hello_timer (struct event *thread)
{
      ...
      if (eigrp_nbr_state_get (nbr) != EIGRP_NEIGHBOR_DOWN)
        {
          if ((param->K1 & param->K2 & param->K3 & param->K4 & param->K5) ==
            ...
            {
              zlog_info (
                  "Neighbor %pI4 (%s) going down: Kvalue mismatch", &nbr->src,
                  ifindex2ifname (nbr->ei->ifp->ifindex, eigrp->vrf_id));
 ...
 */
void
eigrp_hello_receive (struct eigrp *eigrp, struct ip *iph,
      ...
      if ((length > 0) && (length <= size))
        {
          ...
          switch (type)
            ...
            case EIGRP_TLV_AUTH:
              {
                if (eigrp_hello_authentication_decode (s, tlv_header, nbr) ==
                    0)
                  return;
                else
                  break;
...
void
eigrp_sw_version_initialize (void)
{
 ...
 */
static uint16_t
eigrp_sequence_encode (struct eigrp *eigrp, struct stream *s)
{
 ...
 */
static uint16_t
eigrp_next_sequence_encode (struct eigrp *eigrp, struct stream *s)
{
...
eigrp_hello_encode (struct eigrp_interface *ei, in_addr_t addr, uint32_t ack,
 ...
 */
void
eigrp_hello_send (struct eigrp_interface *ei, uint8_t flags,
      ...
      if (ei->eigrp->t_write == NULL)
        {
          if (flags & EIGRP_HELLO_GRACEFUL_SHUTDOWN)
            {
              event_execute (master, eigrp_write, ei->eigrp, ei->eigrp->fd);
            }
          else
            {
              event_add_write (master, eigrp_write, ei->eigrp, ei->eigrp->fd,
                               &ei->eigrp->t_write);
            }
        }
    }
