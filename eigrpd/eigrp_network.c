...
int
eigrp_sock_init (struct vrf *vrf)
{
  int eigrp_sock = -1;
  int ret;
#ifdef IP_HDRINCL
...
int
eigrp_if_ipmulticast (struct eigrp *top, struct prefix *p,
                      unsigned int ifindex)
{
  ...
  ret =
      setsockopt (top->fd, IPPROTO_IP, IP_MULTICAST_LOOP, (void *) &val, len);
  if (ret < 0)
...
int
eigrp_network_set (struct eigrp *eigrp, struct prefix *p)
{
 ...
 */
static int
eigrp_network_match_iface (const struct prefix *co_prefix,
...
int
eigrp_network_unset (struct eigrp *eigrp, struct prefix *p)
{
  struct route_node *rn;
  struct listnode *node, *nnode;
  struct eigrp_interface *ei;
