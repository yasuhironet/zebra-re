...
static const char *
packet_type2str (uint8_t packet_type)
{
...
static const char *
prefix_state2str (enum eigrp_fsm_states state)
{
  switch (state)
    {
...
static const char *
fsm_state2str (enum eigrp_fsm_events event)
{
  switch (event)
    {
...
static const char *
change2str (enum metric_change change)
{
  switch (change)
    {
 ...
 */
int
eigrp_fsm_event (struct eigrp_fsm_action_message *msg)
{
...
int
eigrp_fsm_event_keep_state (struct eigrp_fsm_action_message *msg)
{
  struct eigrp *eigrp = msg->eigrp;
  struct eigrp_prefix_descriptor *prefix = msg->prefix;
  ...
  if (prefix->state == EIGRP_FSM_STATE_PASSIVE)
    {
      if (! eigrp_metrics_is_same (prefix->reported_metric, ne->total_metric))
        {
...
int
eigrp_fsm_event_dinc (struct eigrp_fsm_action_message *msg)
{
...
eigrp_fsm_event_lr_fcs (struct eigrp_fsm_action_message *msg)
{
  ...
  prefix->reported_metric = ne->total_metric;
  prefix->fdistance = prefix->fdistance > prefix->distance ? prefix->distance
                                                           : prefix->fdistance;
...
int
eigrp_fsm_event_qact (struct eigrp_fsm_action_message *msg)
{
  struct list *successors = eigrp_topology_get_successor (msg->prefix);
  struct eigrp_route_descriptor *ne;
