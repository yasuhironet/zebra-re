...
void
watchfrr_set_ignore_daemon (struct vty *vty, const char *dname, bool ignore)
{
  struct daemon *dmn;
  ...
  if (dmn)
    ...
    vty_out (vty, "%s is not configured for running at the moment", dname);
}

static void
printhelp (FILE *target)
{
...
-h, --help	Display this help and exit\n",
      ...
      DEFAULT_MIN_RESTART, DEFAULT_MAX_RESTART, DEFAULT_OPERATIONAL_TIMEOUT,
      DEFAULT_PERIOD, DEFAULT_TIMEOUT, DEFAULT_RESTART_TIMEOUT,
      DEFAULT_RESTART_CMD, DEFAULT_START_CMD, DEFAULT_STOP_CMD, frr_vtydir);
}
...
run_background (char *shell_cmd)
{
  ...
  switch (child = fork ())
    ...
    case 0:
       ...
       * easily. */
      if (setpgid (0, 0) < 0)
        zlog_warn ("setpgid(0,0) failed: %s", safe_strerror (errno));
      {
        char shell[] = "sh";
        char dashc[] = "-c";
        char *const argv[4] = { shell, dashc, shell_cmd, NULL };
        execv ("/bin/sh", argv);
        flog_err_sys (EC_LIB_SYSTEM_CALL, "execv(/bin/sh -c '%s') failed: %s",
...
static void
restart_kill (struct event *t_kill)
{
...
sigchild (void)
{
  ...
  if (WIFSTOPPED (status))
    zlog_warn ("%s %s process %d is stopped", what, name, (int) child);
  else if (WIFSIGNALED (status))
    zlog_warn ("%s %s process %d terminated due to signal %d", what, name,
               (int) child, WTERMSIG (status));
  else if (WIFEXITED (status))
    {
      if (WEXITSTATUS (status) != 0)
        zlog_warn ("%s %s process %d exited with non-zero status %d", what,
                   name, (int) child, WEXITSTATUS (status));
      else
        {
          zlog_debug ("%s %s process %d exited normally", what, name,
                      (int) child);

          if (restart && restart != &gs.restart)
            {
              dmn = container_of (restart, struct daemon, restart);
              restart_done (dmn);
            }
          else if (restart)
            for (dmn = gs.daemons; dmn; dmn = dmn->next)
    ...
    flog_err_sys (EC_LIB_SYSTEM_CALL,
                  "cannot interpret %s %s process %d wait status 0x%x", what,
                  name, (int) child, status);
  phase_check ();
}

static int
run_job (struct restart_info *restart, const char *cmdtype,
         const char *command, int force, int update_interval)
{
  ...
  if (restart->pid)
    {
      if (gs.loglevel > LOG_DEBUG + 1)
        zlog_debug ("cannot %s %s, previous pid %d still running", cmdtype,
                    restart->name, (int) restart->pid);
      return -1;
    }
  ...
  if ((time_elapsed (&delay, &restart->time)->tv_sec < restart->interval) &&
      ! force)
    {
      ...
      if (gs.loglevel > LOG_DEBUG + 1)
        zlog_debug ("postponing %s %s: elapsed time %ld < retry interval %ld",
  ...
  if (update_interval)
    {
      ...
      if (gs.loglevel > LOG_DEBUG + 1)
        zlog_debug ("restart %s interval is now %ld", restart->name,
                    restart->interval);
    }
  return restart->pid;
}
...
#define SET_READ_HANDLER(DMN)                                                 \
  do                                                                          \
    {                                                                         \
      (DMN)->t_read = NULL;                                                   \
      event_add_read (master, handle_read, (DMN), (DMN)->fd, &(DMN)->t_read); \
    }                                                                         \
  while (0);
...
#define SET_WAKEUP_DOWN(DMN)                                                  \
  do                                                                          \
    {                                                                         \
      (DMN)->t_wakeup = NULL;                                                 \
      event_add_timer_msec (master, wakeup_down, (DMN), FUZZY (gs.period),    \
                            &(DMN)->t_wakeup);                                \
    }                                                                         \
  while (0);
...
static void
wakeup_down (struct event *t_wakeup)
{
...
static void
wakeup_init (struct event *t_wakeup)
{
  struct daemon *dmn = EVENT_ARG (t_wakeup);
  ...
  if (try_connect (dmn) < 0)
    {
      zlog_info ("%s state -> down : initial connection attempt failed",
                 dmn->name);
...
static void
restart_done (struct daemon *dmn)
{
...
static void
daemon_restarting_operational (struct event *thread)
{
...
daemon_down (struct daemon *dmn, const char *why)
{
  if (IS_UP (dmn) || (dmn->state == DAEMON_INIT))
    flog_err (EC_WATCHFRR_CONNECTION, "%s state -> down : %s", dmn->name, why);
...
static void
handle_read (struct event *t_read)
{
 ...
 */
static void
daemon_send_ready (int exitcode)
{
  FILE *fp;
  static int sent = 0;
  char started[1024];
...
static void
check_connect (struct event *t_write)
{
...
static void
wakeup_connect_hanging (struct event *t_wakeup)
{
  struct daemon *dmn = EVENT_ARG (t_wakeup);
  char why[100];
...
try_connect (struct daemon *dmn)
{
  ...
  if (access (addr.sun_path, W_OK) < 0)
    {
      if (errno != ENOENT)
        flog_err_sys (EC_LIB_SYSTEM_CALL, "%s: access to socket %s denied: %s",
  ...
  if ((sock = socket (AF_UNIX, SOCK_STREAM, 0)) < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "%s(%s): cannot make socket: %s", __func__,
  ...
  if (set_nonblocking (sock) < 0 || set_cloexec (sock) < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "%s(%s): set_nonblocking/cloexec(%d) failed", __func__,
                    addr.sun_path, sock);
      close (sock);
      return -1;
    }
  ...
  if (connect (sock, (struct sockaddr *) &addr, len) < 0)
    {
      if ((errno != EINPROGRESS) && (errno != EWOULDBLOCK))
        {
          if (gs.loglevel > LOG_DEBUG)
            zlog_debug ("%s(%s): connect failed: %s", __func__, addr.sun_path,
                        safe_strerror (errno));
          close (sock);
          return -1;
        }
      ...
      dmn->fd = sock;
      event_add_write (master, check_connect, dmn, dmn->fd, &dmn->t_write);
...
static void
phase_hanging (struct event *t_hanging)
{
...
static void
set_phase (enum restart_phase new_phase)
{
  gs.phase = new_phase;
...
static void
wakeup_unresponsive (struct event *t_wakeup)
{
  struct daemon *dmn = EVENT_ARG (t_wakeup);
...
static void
wakeup_no_answer (struct event *t_wakeup)
{
  struct daemon *dmn = EVENT_ARG (t_wakeup);
...
static void
wakeup_send_echo (struct event *t_wakeup)
{
  ...
  if (((rc = write (dmn->fd, echocmd, sizeof (echocmd))) < 0) ||
      ((size_t) rc != sizeof (echocmd)))
    {
      ...
      snprintf (why, sizeof (why), "write '%s' returned %d instead of %u",
                echocmd, (int) rc, (unsigned int) sizeof (echocmd));
      daemon_down (dmn, why);
    }
  else
    {
...
void
watchfrr_status (struct vty *vty)
{
  struct daemon *dmn;
  struct timeval delay;
  ...
  for (dmn = gs.daemons; dmn; dmn = dmn->next)
    {
      ...
      if (dmn->restart.pid)
        ...
        vty_out (vty,
                 ...
                 (intmax_t) dmn->restart.interval - (intmax_t) delay.tv_sec,
...
static void
startup_timeout (struct event *t_wakeup)
{
  daemon_send_ready (1);
}
...
static void
netns_create (int dirfd, const char *nsname)
{
...
static void
watchfrr_start_config (void)
{
  gs.reading_configuration = true;
}

static void
watchfrr_end_config (void)
{
  gs.reading_configuration = false;
}

static void
watchfrr_init (int argc, char **argv)
{
  ...
  for (i = optind; i < argc; i++)
    {
      ...
      gs.numdown++;
      dmn->fd = -1;
      event_add_timer_msec (master, wakeup_init, dmn, 0, &dmn->t_wakeup);
  ...
  if (! gs.daemons)
    {
      fprintf (stderr, "Must specify one or more daemons to monitor.\n\n");
      frr_help_exit (1);
    }
  ...
  while ((opt = frr_getopt (argc, argv, NULL)) != EOF)
    {
      ...
      switch (opt)
        ...
        case OPTION_MAXOPERATIONAL:
          {
            ...
            if ((sscanf (optarg, "%ld%1s", &gs.operational_timeout, garbage) !=
                 1) ||
                (gs.operational_timeout < 0))
              {
                fprintf (stderr, "Invalid Operational_timeout argument: %s\n",
                         optarg);
                frr_help_exit (1);
              }
          }
        ...
        case 'T':
          {
            char garbage[3];
            if ((sscanf (optarg, "%ld%1s", &gs.restart_timeout, garbage) !=
                 1) ||
