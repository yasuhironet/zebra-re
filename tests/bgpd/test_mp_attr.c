...
#define SHOULD_ERR   -1
  ...
  },
  {
      ...
      .data = { /* AFI / SAFI */ 0x0, AFI_IP, IANA_SAFI_FLOWSPEC,
                0x00,       /* no NH */
                0x00, 0x06, /* FS Length */
 ...
 */
static void
handle_result (struct peer *peer, struct test_segment *t, int parse_ret,
               int nlri_ret)
{
  ...
  if (! parse_ret)
    {
      ...
      printf ("MP: %u(%u)/%u(%u): recv %u, nego %u\n", nlri.afi, pkt_afi,
              nlri.safi, pkt_safi, peer->afc_recv[nlri.afi][nlri.safi],
...
main (void)
{
  ...
  while (mp_prefix_sid[i].name)
    parse_test (peer, &mp_prefix_sid[i++], BGP_ATTR_PREFIX_SID);
  printf ("failures: %d\n", failed);
  return failed;
}
