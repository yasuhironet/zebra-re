 ...
 */
static void
add_node (struct bgp_table *table, const char *prefix_str)
{
...
static bool
prefix_in_array (const struct prefix *p, struct prefix *prefix_array,
...
static void
check_lookup_result (struct bgp_dest *match, va_list arglist)
{
  char *prefix_str;
...
static void
do_test (struct bgp_table *table, const char *prefix, ...)
{
  va_list arglist;
  struct prefix p;
...
test_range_lookup (void)
{
  ...
  const char *prefixes[] = { "1.16.0.0/16",  "1.16.128.0/18", "1.16.192.0/18",
  ...
  do_test (table, "1.16.0.0/16", "1.16.0.0/16", "1.16.128.0/18",
           "1.16.192.0/18", "1.16.64.0/19", "1.16.160.0/19", "1.16.32.0/20",
           "1.16.32.0/21", NULL);
  ...
  do_test (table, "0.0.0.0/2", "1.16.0.0/16", "1.16.128.0/18", "1.16.192.0/18",
           "1.16.64.0/19", "1.16.160.0/19", "1.16.32.0/20", "1.16.32.0/21",
           "16.0.0.0/16", NULL);
}
