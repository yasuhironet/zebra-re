  ...
  FOREACH_AFI_SAFI (afi, safi)
    {
      ...
      api_result =
          bgp_maximum_paths_set (bgp, afi, safi, BGP_PEER_EBGP, 10, 0);
      EXPECT_TRUE (api_result == 0, test_result);
      api_result =
          bgp_maximum_paths_set (bgp, afi, safi, BGP_PEER_IBGP, 10, 0);
      EXPECT_TRUE (api_result == 0, test_result);
      ...
      EXPECT_TRUE (api_result == 0, test_result);
      EXPECT_TRUE ((bgp->maxpaths[afi][safi].maxpaths_ebgp == MULTIPATH_NUM),
                   test_result);
      EXPECT_TRUE ((bgp->maxpaths[afi][safi].maxpaths_ibgp == MULTIPATH_NUM),
                   test_result);
    }
...
static int
setup_bgp_path_info_mpath_update (testcase_t *t)
{
  int i;
  struct bgp *bgp;
  struct bgp_table *rt;
...
static int
run_bgp_path_info_mpath_update (testcase_t *t)
{
...
static int
cleanup_bgp_path_info_mpath_update (testcase_t *t)
{
  int i;
...
static void
run_tests (testcase_t *tests[], int num_tests, int *pass_count,
           int *fail_count)
{
