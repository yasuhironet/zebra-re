...
lsp_add (struct lspdb_head *lspdb, struct isis_area *area, int level,
...
static void
lsp_add_ip_reach (struct isis_lsp *lsp, const struct isis_test_node *tnode,
                  const char *prefix_str, uint32_t *next_sid_index)
{
  ...
  if (prefix.family == AF_INET)
    isis_tlvs_add_extended_ip_reach (lsp->tlvs, (struct prefix_ipv4 *) &prefix,
                                     10, false, pcfg_p);
  else
    isis_tlvs_add_ipv6_reach (lsp->tlvs, ISIS_MT_IPV6_UNICAST,
                              (struct prefix_ipv6 *) &prefix, 10, false,
                              pcfg_p);
}

static void
lsp_add_reach (struct isis_lsp *lsp, const struct isis_test_node *tnode,
               const uint8_t *ne_id, uint8_t pseudonode_id, uint32_t metric,
               int family, mpls_label_t *next_label)
{
...
static void
lsp_add_router_capability (struct isis_lsp *lsp,
...
static void
lsp_add_mt_router_info (struct isis_lsp *lsp,
                        const struct isis_test_node *tnode)
{
...
static void
lsp_add_protocols_supported (struct isis_lsp *lsp,
                             const struct isis_test_node *tnode)
{
...
static int
topology_load_node_level (const struct isis_topology *topology,
                          const struct isis_test_node *tnode,
  ...
  for (size_t i = 0; tnode->networks[i]; i++)
    {
      if (i > MAX_NETWORKS)
        {
          zlog_debug ("%s: node has too many networks (maximum is %u)",
  ...
  for (size_t i = 0; tnode->adjacencies[i].hostname[0]; i++)
    {
      ...
      if (i > MAX_ADJACENCIES)
        {
          zlog_debug ("%s: node has too many adjacencies (maximum is %u)",
...
static int
topology_load_node (const struct isis_topology *topology,
...
int
test_topology_load (const struct isis_topology *topology,
                    struct isis_area *area, struct lspdb_head lspdb[])
{
  for (int level = IS_LEVEL_1; level <= IS_LEVEL_2; level++)
  ...
  for (size_t i = 0; topology->nodes[i].hostname[0]; i++)
    {
      ...
      if (i > MAX_NODES)
        {
          zlog_debug ("%s: topology has too many nodes (maximum is %u)",
                      __func__, MAX_NODES);
          return -1;
        }
