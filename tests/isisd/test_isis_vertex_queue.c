...
setup_test_vertices (void)
{
  ...
  node_id[6] = 1;
  vertices[vertex_count] = isis_vertex_new (&t, node_id, VTYPE_PSEUDO_TE_IS);
  ...
  vertices[vertex_count] =
      isis_vertex_new (&t, node_id, VTYPE_NONPSEUDO_TE_IS);
  vertices[vertex_count]->d_N = 15;
  vertex_count++;
...
test_ordered (void)
{
  ...
  for (size_t i = 0; i < vertex_count; i++)
    {
      assert (isis_find_vertex (&q, &vertices[i]->N, vertices[i]->type) ==
              vertices[i]);
    }
