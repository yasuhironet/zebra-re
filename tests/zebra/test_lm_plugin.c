...
static int
test_client_connect (struct zserv *client, vrf_id_t vrf_id)
{
  return 0;
}

static int
test_client_disconnect (struct zserv *client)
{
  return 0;
}
...
static int
lm_get_chunk_pi (struct label_manager_chunk **lmc, struct zserv *client,
...
static int
lm_release_chunk_pi (struct zserv *client, uint32_t start, uint32_t end)
{
...
test_lp_plugin (void)
{
  ...
  fprintf (stdout,
           ...
           lmc->start, lmc->end, lmc->proto, lmc->instance, lmc->session_id,
           lmc->keep ? "yes" : "no");
