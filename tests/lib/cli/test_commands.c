...
static int
test_callback (const struct cmd_element *cmd, struct vty *vty, int argc,
...
static void
test_run (struct prng *prng, struct vty *vty, const char *cmd,
          unsigned int edit_dist, unsigned int node_index, int verbose)
{
  const char *test_str;
  vector vline;
  int ret;
    ...
    if ((cnode = vector_slot (cmdvec, i)) != NULL)
      {
        ...
        if (isspace ((unsigned char) test_str[strlen (test_str) - 1]))
          {
        ...
        if (descriptions != NULL)
          {
            for (j = 0; j < vector_active (descriptions); j++)
              {
                struct cmd_token *ct = vector_slot (descriptions, j);
                printf ("  '%s' '%s'\n", ct->text, ct->desc);
              }
      ...
      for (edit_distance = 0; edit_distance <= max_edit_distance;
           edit_distance++)
        {
          ...
          for (iteration = 0; iteration < num_iterations; iteration++)
            test_run (prng, vty, vector_slot (test_cmds, test_cmd),
