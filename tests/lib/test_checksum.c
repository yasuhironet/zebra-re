...
static uint16_t
ospfd_checksum (uint8_t *buffer, testsz_t len, testoff_t off)
{
  uint8_t *sp, *ep, *p, *q;
  int c0 = 0, c1 = 0;
  int x, y;
...
static uint16_t
iso_csum_create (uint8_t *buffer, testsz_t len, testoff_t off)
{
...
static int
verify (uint8_t *buffer, testsz_t len)
{
  uint8_t *p;
  uint32_t c0;
  uint32_t c1;
  ...
  while (1)
    {
      ...
      if (exercise >= 6)
        {
          ...
          if (in_csum_iov != in_csum)
            printf (
                ...
                iov[0].iov_len, iov[1].iov_len, iov[2].iov_len, in_csum_iov,
                in_csum);
      ...
      ospfd = ospfd_checksum (buffer, exercise + sizeof (uint16_t), exercise);
      if (verify (buffer, exercise + sizeof (uint16_t)))
        printf ("\nverify: ospfd failed\n");
      isisd = iso_csum_create (buffer, exercise + sizeof (uint16_t), exercise);
      if (verify (buffer, exercise + sizeof (uint16_t)))
        printf ("\nverify: isisd failed\n");
      lib = fletcher_checksum (buffer, exercise + sizeof (uint16_t), exercise);
      if (verify (buffer, exercise + sizeof (uint16_t)))
        printf ("\nverify: lib failed\n");
      ...
      if (ospfd != lib)
        {
          printf ("\nMismatch in values at size %d\n"
                  ...
                  "lib: 0x%04x\n",
                  exercise, ospfd, ospfd_vals.a.c0, ospfd_vals.a.c1,
                  ospfd_vals.x, ospfd_vals.y, isisd, isisd_vals.a.c0,
                  isisd_vals.a.c1, isisd_vals.x, isisd_vals.y, lib);
          ...
          if (ospfd_vals.a.c0 == isisd_vals.a.c0 &&
              ospfd_vals.a.c1 == isisd_vals.a.c1)
            {
              ...
              for (i = 0; reducts[i].name != NULL; i++)
                {
                  ospfd = reducts[i].f (
                      &ospfd_vals, exercise + sizeof (uint16_t), exercise);
                  printf ("%20s: x: %02x, y %02x, checksum 0x%04x\n",
                          reducts[i].name, ospfd_vals.x & 0xff,
