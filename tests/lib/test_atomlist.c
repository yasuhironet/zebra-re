...
static int
icmp (const struct item *a, const struct item *b)
{
  if (a->val1 > b->val1)
    return 1;
  if (a->val1 < b->val1)
...
#define deftestrun(name, _desc, _prefill, _sorted)                            \
  ...
  struct testrun tr_##name = { .desc = _desc,                                 \
                               .lineno = __LINE__,                            \
                               .prefill = _prefill,                           \
                               .func = &trfunc_##name,                        \
                               .sorted = _sorted };                           \
...
deftestrun (add, "add vs. add", 0, false) for (; i < NITEM / NTHREADS; i++)
    alist_add_head (&ahead, &itm[i * NTHREADS + offset]);
endtestrun

    deftestrun (del, "del vs. del", NOCLEAR,
                false) for (; i < NITEM / NTHREADS / 10; i++)
        alist_del (&ahead, &itm[i * NTHREADS + offset]);
endtestrun

    deftestrun (addtail, "add_tail vs. add_tail", 0,
                false) for (; i < NITEM / NTHREADS; i++)
        alist_add_tail (&ahead, &itm[i * NTHREADS + offset]);
endtestrun

    deftestrun (pop, "pop vs. pop", NOCLEAR,
                false) for (; i < NITEM / NTHREADS;) if (alist_pop (&ahead)) i
    ++;
else n++;
endtestrun
...
deftestrun (headN_vs_popN, "add_head(N) vs. pop(N)", NTHREADS / 2,
            false) if (offset < NTHREADS / 2)
{
  struct item *dr = NULL;
...
deftestrun (tailN_vs_pop1, "add_tail(N) vs. pop(1)", 1, false) if (offset == 0)
{
  struct item *dr = NULL;
...
deftestrun (tail1_vs_popN, "add_tail(1) vs. pop(N)", 0,
            false) if (offset < NTHREADS - 1)
{
  struct item *dr = NULL;
...
endtestrun
    ...
    deftestrun (sort_add, "add_sort vs. add_sort", 0,
                true) for (; i < NITEM / NTHREADS / 10; i++)
        asort_add (&shead, &itm[i * NTHREADS + offset]);
endtestrun

    deftestrun (sort_del, "del_sort vs. del_sort", NOCLEAR,
                true) for (; i < NITEM / NTHREADS / 10; i++)
        asort_del (&shead, &itm[i * NTHREADS + offset]);
endtestrun

deftestrun (sort_add_del, "add_sort vs. del_sort", NTHREADS / 2,
            true) if (offset < NTHREADS / 2)
{
