...
static int
printchk (const char *ref, const char *fmt, ...)
{
  va_list ap;
  char bufrr[256];
  ...
  for (size_t i = 0; i < fb.outpos_i; i++)
    printf ("\t[%zu: %u..%u] = \"%.*s\"\n", i, outpos[i].off_start,
            outpos[i].off_end, (int) (outpos[i].off_end - outpos[i].off_start),
...
static void
test_va (const char *ref, const char *fmt, ...)
{
...
main (int argc, char **argv)
{
  size_t i;
  float flts[] = {
    123.456789, 23.456789e-30, 3.456789e+30, INFINITY, NAN,
  };
  ...
  printchk ("12 34 00 ca fe 00 aa 55", "%*pHX", (int) sizeof (randhex),
            randhex);
