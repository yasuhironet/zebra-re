                ...
                const struct prefix_ipv6 *src_p)
{
  ...
  ec = asprintf (&rv, "%s%s%s", dst_str, (src_str[0] != '\0') ? " from " : "",
                 src_str);
...
static void
test_state_add_route (struct test_state *test, struct prefix_ipv6 *dst_p,
...
static void
test_state_del_route (struct test_state *test, struct prefix_ipv6 *dst_p,
                      struct prefix_ipv6 *src_p)
{
  struct route_node *rn =
      srcdest_rnode_lookup (test->table, (struct prefix *) dst_p, src_p);
  struct prefix hash_entry[2];
...
static void
verify_log (struct hash_bucket *bucket, void *arg)
{
...
static void
dump_log (struct hash_bucket *bucket, void *arg)
{
  struct prefix *hash_entry = bucket->data;
  struct prefix_ipv6 *dst_p = (struct prefix_ipv6 *) &hash_entry[0];
  struct prefix_ipv6 *src_p = (struct prefix_ipv6 *) &hash_entry[1];
...
test_failed (struct test_state *test, const char *message,
             const struct prefix_ipv6 *dst_p, const struct prefix_ipv6 *src_p)
{
  char *route_id = format_srcdest (dst_p, src_p);
          ...
          if (route_node_get_lock_count (rn) != expected_lock)
            {
              ...
              test_failed (
                  test, "Src rnode lock count doesn't match expected count!",
                  dst_p, src_p);
            }
        }
...
static void
test_state_add_rand_route (struct test_state *test, struct prng *prng)
{
...
static void
test_state_del_rand_route (struct test_state *test, struct prng *prng)
{
  struct prefix_ipv6 dst_p, src_p;
...
static void
test_state_del_one_route (struct test_state *test, struct prng *prng)
{
