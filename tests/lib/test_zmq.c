...
static void
msg_buf_free (void *data, void *hint)
{
  XFREE (MTYPE_TESTBUF, data);
}

static int
recv_delim (void *zmqsock)
{
  /* receive delim */
  zmq_msg_t zdelim;
  int more;
...
run_client (int syncfd)
{
  ...
  for (i = 0; i < 8; i++)
    {
      snprintf (buf, sizeof (buf), "msg #%d %c%c%c", i, 'a' + i, 'b' + i,
                'c' + i);
      ...
      for (j = 1; j <= i; j++)
        {
          ...
          zmq_msg_init_data (&part, dyn, strlen (dyn) + 1, msg_buf_free, NULL);
...
static void
recv_id_and_delim (void *zmqsock, zmq_msg_t *msg_id)
{
  /* receive id */
  zmq_msg_init (msg_id);
...
static void
send_id_and_delim (void *zmqsock, zmq_msg_t *msg_id)
{
  /* Send Id */
...
static void
serverwritefn (void *arg, void *zmqsock)
{
...
static void
serverpartfn (void *arg, void *zmqsock, zmq_msg_t *msg, unsigned partnum)
{
  static int num = 0;
...
static void
serverfn (void *arg, void *zmqsock)
{
  static int num = 0;
  ...
  frrzmq_event_add_read_part (master, serverpartfn, NULL, NULL, zmqsock, &cb);
}
