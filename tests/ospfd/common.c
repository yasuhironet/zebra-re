...
int
sort_paths (const void **path1, const void **path2)
{
...
void
print_route_table (struct vty *vty, struct route_table *rt)
{
      ...
      for (ALL_LIST_ELEMENTS_RO (or->paths, pnode, path))
        {
          ...
          vty_out (vty, "  -> %pI4 with adv router %pI4", &path->nexthop,
                   &path->adv_router);

          if (path->srni.backup_label_stack)
            {
              label_stack = path->srni.backup_label_stack;
              mpls_label2str (label_stack->num_labels, label_stack->label, buf,
                              MPLS_LABEL_STRLEN, ZEBRA_LSP_NONE, true);
...
test_find_node (struct ospf_topology *topology, const char *hostname)
{
...
static void
inject_router_lsa (struct vty *vty, struct ospf *ospf,
                   struct ospf_topology *topology, struct ospf_test_node *root,
                   struct ospf_test_node *tnode)
{
  ...
  for (link_count = 0; tnode->adjacencies[link_count].hostname[0];
       link_count++)
    {
      ...
      str2prefix_ipv4 (tnode->adjacencies[link_count].network, &prefix);

      inet_aton (tfound_adj_node->router_id, &adj_router_id);
      data.s_addr = prefix.prefix.s_addr;
      link_info_set (&s, adj_router_id, data, LSA_LINK_TYPE_POINTOPOINT, 0,
                     tadj->metric);
...
static void
inject_sr_db_entry (struct vty *vty, struct ospf_test_node *tnode,
...
int
topology_load (struct vty *vty, struct ospf_topology *topology,
