...
static inline int
qpb__address_family__set (Qpb__AddressFamily *pb_family, uint8_t family)
{
  switch (family)
    {
    case AF_INET:
...
static inline int
qpb__address_family__get (Qpb__AddressFamily pb_family, uint8_t *family)
{
...
static inline int
qpb__l3_prefix__get (const Qpb__L3Prefix *pb_prefix, uint8_t family,
                     struct prefix *prefix)
{
  ...
  switch (family)
    ...
    case AF_INET:
      memset ((struct prefix_ipv4 *) prefix, 0, sizeof (struct prefix_ipv4));
      break;

    case AF_INET6:
      memset ((struct prefix_ipv6 *) prefix, 0, sizeof (struct prefix_ipv6));
      break;
...
static inline int
qpb__l3_address__get (const Qpb__L3Address *l3_addr, uint8_t *family,
                      union g_addr *addr)
{
  if (l3_addr->v4)
    {
...
static inline int
qpb__if_identifier__get (Qpb__IfIdentifier *if_id, uint *if_index, char **name)
{
  char *str;
  uint ix;
