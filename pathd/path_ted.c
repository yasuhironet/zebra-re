...
void
path_ted_init (struct event_loop *master)
{
 ...
 */
uint32_t
path_ted_start_importing_igp (const char *daemon_str)
{
  uint32_t status = 0;
  ...
  if (ls_register (zclient, false /*client*/) != 0)
  ...
  else
    {
      if (path_ted_link_state_sync () != -1)
        {
          PATH_TED_DEBUG ("%s: PATHD-TED: Importing %s data ON", __func__,
                          PATH_TED_IGP_PRINT (ted_state_g.import));
        }
      else
        {
          PATH_TED_WARN ("%s: PATHD-TED: Importing %s data OFF", __func__,
                         PATH_TED_IGP_PRINT (ted_state_g.import));
          ted_state_g.import = IMPORT_UNKNOWN;
        }
    }
 ...
 */
uint32_t
path_ted_stop_importing_igp (void)
{
  uint32_t status = 0;
  ...
  if (ted_state_g.import != IMPORT_UNKNOWN)
    {
      if (ls_unregister (zclient, false /*client*/) != 0)
        {
          PATH_TED_ERROR ("%s: PATHD-TED: Unable to unregister Link State",
                          __func__);
          status = 1;
        }
      else
        {
          ted_state_g.import = IMPORT_UNKNOWN;
          PATH_TED_DEBUG ("%s: PATHD-TED: Importing igp data OFF", __func__);
        }
      path_ted_timer_sync_cancel ();
    }
  return status;
...
path_ted_create_ted (void)
{
  ...
  if (ted == NULL)
    {
      PATH_TED_ERROR (
          "%s Unable to initialize TED Key [%d] ASN [%d] Name [%s]", __func__,
          TED_KEY, TED_ASN, TED_NAME);
    }
  else
    {
      PATH_TED_INFO ("%s Initialize TED Key [%d] ASN [%d] Name [%s]", __func__,
                     TED_KEY, TED_ASN, TED_NAME);
    }
...
uint32_t
path_ted_rcvd_message (struct ls_message *msg)
{
  ...
  if (msg == NULL)
    {
      PATH_TED_ERROR ("%s: [rcv ted] TED received NULL message ", __func__);
      return 1;
    }
  ...
  switch (msg->type)
    ...
    default:
      PATH_TED_DEBUG ("%s: [rcv ted] TED received unknown message type [%d]",
                      __func__, msg->type);
      break;
    }
  return 0;
...
uint32_t
path_ted_query_type_f (struct ipaddr *local, struct ipaddr *remote)
{
      ...
      if (edge)
        {
          if (edge->attributes->standard.remote.s_addr ==
              ...
              CHECK_FLAG (edge->attributes->flags, LS_ATTR_ADJ_SID))
            {
      ...
      if (edge)
        {
          if ((0 == memcmp (&edge->attributes->standard.remote6,
               ...
               CHECK_FLAG (edge->attributes->flags, LS_ATTR_ADJ_SID6)))
            {
...
uint32_t
path_ted_query_type_c (struct prefix *prefix, uint8_t algo)
{
  uint32_t sid = MPLS_LABEL_NONE;
...
uint32_t
path_ted_query_type_e (struct prefix *prefix, uint32_t iface_id)
{
  uint32_t sid = MPLS_LABEL_NONE;
  struct ls_subnet *subnet;
  ...
  switch (prefix->family)
    ...
    case AF_INET6:
      subnet = ls_find_subnet (ted_state_g.ted, prefix);
      if (subnet && subnet->vertex && subnet->vertex->outgoing_edges)
        {
          /* from the vertex linked in subnet */
          /* loop over outgoing edges */
          for (ALL_LIST_ELEMENTS_RO (subnet->vertex->outgoing_edges, lst_node,
                                     edge))
            {
              /* and look for ifaceid */
              /* so get sid of attribute */
              if (CHECK_FLAG (edge->attributes->flags, LS_ATTR_LOCAL_ID) &&
                  edge->attributes->standard.local_id == iface_id)
                {
                  sid = subnet->ls_pref->sr.sid;
                  break;
                }
...
/* clang-format off */
DEFPY(path_ted_import,                         path_ted_import_cmd,
  ...
  if (ted_state_g.import)
    {
      if (path_ted_stop_importing_igp ())
        ...
        {
          PATH_TED_DEBUG ("%s: PATHD-TED: Importing igp data already OFF",
                          __func__);
        }
    }
  return CMD_SUCCESS;
...
int
path_ted_cli_debug_config_write (struct vty *vty)
{
...
void
path_ted_show_debugging (struct vty *vty)
{
  if (DEBUG_FLAGS_CHECK (&ted_state_g.dbg, PATH_TED_DEBUG_BASIC))
...
int
path_ted_cli_debug_set_all (uint32_t flags, bool set)
{
 ...
 */
uint32_t
path_ted_config_write (struct vty *vty)
{
...
path_ted_link_state_sync (void)
{
  ...
  if (status == -1)
    {
      PATH_TED_ERROR ("%s: PATHD-TED: Opaque error asking for TED sync ",
                      __func__);
      return status;
    }
  else
  ...
  event_add_timer (ted_state_g.main, path_ted_timer_handler_sync, &ted_state_g,
                   ted_state_g.link_state_delay_interval,
 ...
 */
void
path_ted_timer_handler_sync (struct event *thread)
{
  /* data unpacking */
 ...
 */
int
path_ted_segment_list_refresh (void)
{
  int status = 0;
 ...
 */
void
path_ted_timer_handler_refresh (struct event *thread)
{
  if (! path_ted_is_initialized ())
    return;
 ...
 */
void
path_ted_timer_sync_cancel (void)
{
 ...
 */
void
path_ted_timer_refresh_cancel (void)
{
 ...
 */
uint32_t
path_ted_get_current_igp (uint32_t igp)
{
  switch (igp)
    {
    case ISIS_L1:
      ...
      if (ted_state_g.import != IMPORT_ISIS)
        {
          PATH_TED_ERROR (
              ...
              __func__, PATH_TED_IGP_PRINT (ted_state_g.import),
              LS_IGP_PRINT (igp));
          return 1;
        }
      break;
