...
static void pcep_lib_parse_srp (struct path *path,
                                struct pcep_object_srp *srp);
static void pcep_lib_parse_lsp (struct path *path,
...
int
pcep_lib_initialize (struct frr_pthread *fpt)
{
...
pcep_session *
pcep_lib_connect (struct ipaddr *src_addr, int src_port,
                  struct ipaddr *dst_addr, int dst_port, short msd,
  ...
  if (pcep_options->tcp_md5_auth[0] != '\0')
    {
      ...
      strlcpy (config->tcp_authentication_str, pcep_options->tcp_md5_auth,
...
void
pcep_lib_disconnect (pcep_session *sess)
{
  assert (sess != NULL);
  disconnect_pce (sess);
}
...
int
pcep_lib_pceplib_socket_write_cb (void *fpt, void **thread, int fd,
                                  void *payload)
{
...
int
pcep_lib_pceplib_socket_read_cb (void *fpt, void **thread, int fd,
                                 void *payload)
{
...
/* Callbacks called by path_pcep_controller when a socket is ready to
 * read/write
 */

void
pcep_lib_socket_write_ready (struct event *thread)
{
...
void
pcep_lib_socket_read_ready (struct event *thread)
{
  struct pcep_ctrl_socket_data *data = EVENT_ARG (thread);
  assert (data != NULL);
...
void
pcep_lib_pceplib_event_cb (void *fpt, pcep_event *event)
{
...
void *
pcep_lib_pthread_start_passthrough (void *data)
{
...
int
pcep_lib_pthread_create_cb (pthread_t *thread_id, const pthread_attr_t *attr,
                            ...
                            const char *thread_name)
{
  ...
  struct pcep_lib_pthread_passthrough_data *passthrough_data =
      XMALLOC (MTYPE_PCEP, sizeof (struct pcep_lib_pthread_passthrough_data));
  passthrough_data->data = data;
  passthrough_data->start_routine = start_routine;

  struct frr_pthread_attr fpt_attr = { .start =
...
int
pcep_lib_pthread_stop_cb (struct frr_pthread *fpt, void **res)
{
  pcep_lib_finalize ();
...
struct pcep_message *
pcep_lib_format_report (struct pcep_caps *caps, struct path *path)
{
  double_linked_list *objs = pcep_lib_format_path (caps, path);
  return pcep_msg_create_report (objs);
}

static struct pcep_object_rp *
...
struct pcep_message *
pcep_lib_format_request (struct pcep_caps *caps, struct path *path)
{
...
struct pcep_message *
pcep_lib_format_error (int error_type, int error_value, struct path *path)
{
  ...
  dll_append (objs, srp);
  return pcep_msg_create_error_with_objects (error_type, error_value, objs);
}
...
pcep_lib_format_request_cancelled (uint32_t reqid)
{
  ...
  notify = pcep_obj_create_notify (PCEP_NOTIFY_TYPE_PENDING_REQUEST_CANCELLED,
...
struct path *
pcep_lib_parse_path (struct pcep_message *msg)
{
  struct path *path;
      ...
      switch (CLASS_TYPE (obj->object_class, obj->object_type))
        ...
        default:
          flog_warn (
              ...
              pcep_object_class_name (obj->object_class), obj->object_class,
...
void
pcep_lib_parse_capabilities (struct pcep_message *msg, struct pcep_caps *caps)
{
  double_linked_list *objs = msg->obj_list;
  double_linked_list_node *node;
...
pcep_lib_copy_counters (pcep_session *sess)
{
...
void
pcep_lib_free_counters (struct counters_group *counters)
{
  free_counter_group (counters);
}

pcep_session *
pcep_lib_copy_pcep_session (pcep_session *sess)
{
  if (! sess)
    {
      return NULL;
...
int
pceplib_logging_cb (int priority, const char *fmt, va_list args)
{
  char buffer[1024];
...
double_linked_list *
pcep_lib_format_path (struct pcep_caps *caps, struct path *path)
{
  ...
  if (path->plsp_id == 0 || IS_IPADDR_NONE (&path->nbkey.endpoint) ||
    ...
    {
      assert (path->pcc_addr.ipa_type == path->nbkey.endpoint.ipa_type);
      if (IS_IPADDR_V4 (&path->pcc_addr))
        {
          tlv = (struct pcep_object_tlv_header *)
              pcep_tlv_create_ipv4_lsp_identifiers (
                  &path->pcc_addr.ipaddr_v4, &path->nbkey.endpoint.ipaddr_v4,
                  0, 0, &path->pcc_addr.ipaddr_v4);
        }
      else
        {
          tlv = (struct pcep_object_tlv_header *)
              pcep_tlv_create_ipv6_lsp_identifiers (
                  &path->pcc_addr.ipaddr_v6, &path->nbkey.endpoint.ipaddr_v6,
                  0, 0, &path->pcc_addr.ipaddr_v6);
        }
    }
  assert (tlv != NULL);
  ...
  if ((path->plsp_id != 0) && (path->binding_sid != MPLS_LABEL_NONE))
    {
      ...
      tlv = (struct pcep_object_tlv_header *) pcep_tlv_create_tlv_arbitrary (
          binding_sid_lsp_tlv_data, sizeof (binding_sid_lsp_tlv_data),
  ...
  for (struct path_hop *hop = path->first_hop; hop != NULL; hop = hop->next)
    {
      uint32_t sid;
      ...
      if (hop->has_attribs)
        {
          sid = ENCODE_SR_ERO_SID (hop->sid.mpls.label,
                                   ...
                                   hop->sid.mpls.is_bottom, hop->sid.mpls.ttl);
        }
      else
        {
            ...
            case PCEP_SR_SUBOBJ_NAI_IPV4_NODE:
              ero_obj = (struct pcep_object_ro_subobj *)
                  pcep_obj_create_ro_subobj_sr_ipv4_node (
                      ...
                      hop->is_mpls,     /* M Flag */
                      sid, &hop->nai.local_addr.ipaddr_v4);
              break;
            case PCEP_SR_SUBOBJ_NAI_IPV6_NODE:
              ero_obj = (struct pcep_object_ro_subobj *)
                  pcep_obj_create_ro_subobj_sr_ipv6_node (
                      ...
                      hop->has_attribs, /* C Flag */
                      hop->is_mpls,     /* M Flag */
                      sid, &hop->nai.local_addr.ipaddr_v6);
              break;
            ...
            case PCEP_SR_SUBOBJ_NAI_UNNUMBERED_IPV4_ADJACENCY:
              ero_obj = (struct pcep_object_ro_subobj *)
                  pcep_obj_create_ro_subobj_sr_unnumbered_ipv4_adj (
                      ...
                      hop->has_attribs, /* C Flag */
                      hop->is_mpls,     /* M Flag */
                      sid, hop->nai.local_addr.ipaddr_v4.s_addr,
                      hop->nai.local_iface,
                      hop->nai.remote_addr.ipaddr_v4.s_addr,
      ...
      if (ero_obj == NULL)
        {
          ero_obj = (struct pcep_object_ro_subobj *)
              pcep_obj_create_ro_subobj_sr_nonai (
                  hop->is_loose, sid, hop->has_attribs, /* C Flag */
                  hop->is_mpls);                        /* M Flag */
        }
...
void
pcep_lib_format_constraints (struct path *path, double_linked_list *objs)
{
  ...
  /* Metric Objects */
  for (struct path_metric *m = path->first_metric; m != NULL; m = m->next)
    {
      metric = pcep_obj_create_metric (m->type, m->is_bound, m->is_computed,
                                       m->value);
      assert (metric != NULL);
...
void
pcep_lib_parse_open (struct pcep_caps *caps, struct pcep_object_open *open)
{
  ...
  for (node = tlv->of_list->head; node != NULL; node = node->next_node)
    {
      ...
      if (of_code >= 32)
        {
          zlog_warn ("Ignoring unexpected objective function with code %u",
                     of_code);
          continue;
        }
...
void
pcep_lib_parse_rp (struct path *path, struct pcep_object_rp *rp)
{
...
void
pcep_lib_parse_srp (struct path *path, struct pcep_object_srp *srp)
{
...
void
pcep_lib_parse_lsp (struct path *path, struct pcep_object_lsp *lsp)
{
...
void
pcep_lib_parse_lsp_symbolic_name (
    struct path *path, struct pcep_object_tlv_symbolic_path_name *tlv)
{
...
void
pcep_lib_parse_lspa (struct path *path, struct pcep_object_lspa *lspa)
{
...
void
pcep_lib_parse_metric (struct path *path, struct pcep_object_metric *obj)
{
...
void
pcep_lib_parse_vendor_info (struct path *path,
                            struct pcep_object_vendor_info *obj)
{
...
void
pcep_lib_parse_ero (struct path *path, struct pcep_object_ro *ero)
{
  ...
  for (node = objs->tail; node != NULL; node = node->prev_node)
    {
      ...
      switch (obj->ro_subobj_type)
        ...
        case RO_SUBOBJ_TYPE_SR:
          hop = pcep_lib_parse_ero_sr (hop, (struct pcep_ro_subobj_sr *) obj);
          break;
...
struct path_hop *
pcep_lib_parse_ero_sr (struct path_hop *next, struct pcep_ro_subobj_sr *sr)
{
  struct path_hop *hop = NULL;
  union sid sid;
  ...
  if (sr->flag_m)
    {
      sid.mpls =
          (struct sid_mpls){ .label = GET_SR_ERO_SID_LABEL (sr->sid),
      ...
      switch (sr->nai_type)
        ...
        case PCEP_SR_SUBOBJ_NAI_UNKNOWN:
          ...
          flog_warn (EC_PATH_PCEP_UNEXPECTED_SR_NAI,
                     ...
                     pcep_nai_type_name (sr->nai_type), sr->nai_type);
          break;
        }
    }
...
struct counters_group *
copy_counter_group (struct counters_group *from)
{
  int size, i;
  ...
  for (i = 0; i <= from->max_subgroups; i++)
    result->subgroups[i] = copy_counter_subgroup (from->subgroups[i]);
  return result;
}
...
struct counter *
copy_counter (struct counter *from)
{
...
void
free_counter_group (struct counters_group *group)
{
  int i;
  if (group == NULL)
    return;
...
void
free_counter_subgroup (struct counters_subgroup *subgroup)
{
  int i;
  if (subgroup == NULL)
    return;
...
void
free_counter (struct counter *counter)
{
  if (counter == NULL)
    return;
