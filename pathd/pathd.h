...
RB_PROTOTYPE (srte_candidate_head, srte_candidate, entry,
...
struct srte_segment_entry *
srte_segment_entry_add (struct srte_segment_list *segment_list,
                        uint32_t index);
...
int srte_segment_entry_set_nai (struct srte_segment_entry *segment,
                                ...
                                uint32_t remote_iface, uint8_t algo,
                                uint8_t pref_len);
...
void srte_candidate_unset_metric (struct srte_candidate *candidate,
                                  enum srte_candidate_metric_type type);
void srte_candidate_set_objfun (struct srte_candidate *candidate,
                                bool required, enum objfun_type type);
...
srte_segment_entry_find (struct srte_segment_list *segment_list,
                         uint32_t index);
void srte_candidate_status_update (struct srte_candidate *candidate,
                                   int status);
