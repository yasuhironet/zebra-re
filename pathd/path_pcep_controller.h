...
int pcep_ctrl_sync_path (struct frr_pthread *fpt, int pcc_id,
                         struct path *path);
 ...
 * sent by all PCCs.  The parameter is_stable is used to hint whether the
 * status will soon change, this is used to ensure all report updates are sent
 * even when missing status update events */
