...
static int schedule_thread_socket (struct ctrl_state *ctrl_state, int pcc_id,
                                   ...
                                   bool is_read, void *payload, int fd,
                                   struct event **thread,
...
int
pcep_ctrl_initialize (struct event_loop *main_thread, struct frr_pthread **fpt,
                      pcep_main_event_handler_t event_handler)
{
  ...
  *fpt = frr_pthread_new (&attr, "PCEP thread", "pcep_controller");
  if (*fpt == NULL)
    {
      flog_err (EC_PATH_SYSTEM_CALL, "failed to initialize PCEP thread");
      return 1;
    }
...
int
pcep_ctrl_finalize (struct frr_pthread **fpt)
{
  assert (fpt != NULL);
...
int
pcep_ctrl_update_pcc_options (struct frr_pthread *fpt, struct pcc_opts *opts)
{
...
int
pcep_ctrl_update_pce_options (struct frr_pthread *fpt, struct pce_opts *opts)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
...
int
pcep_ctrl_remove_pcc (struct frr_pthread *fpt, struct pce_opts *pce_opts)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
...
int
pcep_ctrl_reset_pcc_session (struct frr_pthread *fpt, char *pce_name)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
...
int
pcep_ctrl_pathd_event (struct frr_pthread *fpt,
...
int
pcep_ctrl_sync_path (struct frr_pthread *fpt, int pcc_id, struct path *path)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
...
int
pcep_ctrl_sync_done (struct frr_pthread *fpt, int pcc_id)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
...
pcep_ctrl_get_counters (struct frr_pthread *fpt, int pcc_id)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
...
pcep_session *
pcep_ctrl_get_pcep_session (struct frr_pthread *fpt, int pcc_id)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
  struct pcc_state *pcc_state;
...
pcep_ctrl_get_pcc_info (struct frr_pthread *fpt, const char *pce_name)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
...
int
pcep_ctrl_send_report (struct frr_pthread *fpt, int pcc_id, struct path *path,
                       bool is_stable)
{
  struct ctrl_state *ctrl_state = get_ctrl_state (fpt);
  return send_to_thread (ctrl_state, pcc_id, EV_SEND_REPORT, is_stable, path);
}


int
pcep_ctrl_send_error (struct frr_pthread *fpt, int pcc_id,
...
int
pcep_ctrl_halt_cb (struct frr_pthread *fpt, void **res)
{
  event_add_event (fpt->master, pcep_thread_finish_event_handler, (void *) fpt,
                   0, NULL);
...
void
pcep_refine_path_event_cb (struct event *thread)
{
...
void
pcep_thread_start_sync (struct ctrl_state *ctrl_state, int pcc_id)
{
  send_to_main (ctrl_state, pcc_id, PCEP_MAIN_EVENT_START_SYNC, NULL);
}

void
pcep_thread_update_path (struct ctrl_state *ctrl_state, int pcc_id,
                         struct path *path)
{
  send_to_main (ctrl_state, pcc_id, PCEP_MAIN_EVENT_UPDATE_CANDIDATE, path);
}

void
pcep_thread_initiate_path (struct ctrl_state *ctrl_state, int pcc_id,
                           struct path *path)
{
  send_to_main (ctrl_state, pcc_id, PCEP_MAIN_EVENT_INITIATE_CANDIDATE, path);
}

void
pcep_thread_remove_candidate_path_segments (struct ctrl_state *ctrl_state,
                                            struct pcc_state *pcc_state)
{
  if (! pcc_state)
    return;
...
void
pcep_thread_schedule_sync_best_pce (struct ctrl_state *ctrl_state, int pcc_id,
                                    int delay, struct event **thread)
{
...
void
pcep_thread_cancel_timer (struct event **thread)
{
                                ...
                                int retry_count, struct event **thread)
{
  ...
  PCEP_DEBUG ("Schedule RECONNECT_PCC for %us (retry %u)", delay, retry_count);
  schedule_thread_timer (ctrl_state, pcc_id, TM_RECONNECT_PCC, TO_UNDEFINED,
                         delay, NULL, thread);
}
                              ...
                              struct event **thread)
{
  ...
  PCEP_DEBUG ("Schedule timeout %s for %us", timeout_type_name (timeout_type),
              delay);
  schedule_thread_timer (ctrl_state, pcc_id, TM_TIMEOUT, timeout_type, delay,
                         param, thread);
}

void
pcep_thread_schedule_pceplib_timer (struct ctrl_state *ctrl_state, int delay,
                                    void *payload, struct event **thread,
                                    pcep_ctrl_thread_callback timer_cb)
{
  PCEP_DEBUG ("Schedule PCEPLIB_TIMER for %us", delay);
  schedule_thread_timer_with_cb (ctrl_state, 0, TM_PCEPLIB_TIMER, TO_UNDEFINED,
                                 delay, payload, thread, timer_cb);
}

void
pcep_thread_schedule_session_timeout (struct ctrl_state *ctrl_state,
...
int
pcep_thread_pcc_count (struct ctrl_state *ctrl_state)
{
  if (ctrl_state == NULL)
    {
      return 0;
...
int
pcep_thread_refine_path (struct ctrl_state *ctrl_state, int pcc_id,
                         ...
                         void *payload)
{
  ...
  event_add_event (ctrl_state->main, pcep_refine_path_event_cb, (void *) data,
                   0, NULL);
  return 0;
}
                                ...
                                struct pcep_refine_path_event_data *data)
{
  ...
  int pcc_id = data->pcc_id;
  pcep_refine_callback_t continue_lsp_update_handler =
      data->continue_lsp_update_handler;
...
void
pcep_thread_finish_event_handler (struct event *thread)
{
  int i;
...
int
schedule_thread_timer_with_cb (struct ctrl_state *ctrl_state, int pcc_id,
                               enum pcep_ctrl_timer_type timer_type,
                               enum pcep_ctrl_timeout_type timeout_type,
                               uint32_t delay, void *payload,
                               struct event **thread,
                               pcep_ctrl_thread_callback timer_cb)
{
  ...
  event_add_timer (ctrl_state->self, timer_cb, (void *) data, delay, thread);

  return 0;
}

int
schedule_thread_timer (struct ctrl_state *ctrl_state, int pcc_id,
                       enum pcep_ctrl_timer_type timer_type,
                       enum pcep_ctrl_timeout_type timeout_type,
                       uint32_t delay, void *payload, struct event **thread)
{
  return schedule_thread_timer_with_cb (ctrl_state, pcc_id, timer_type,
                                        timeout_type, delay, payload, thread,
                                        pcep_thread_timer_handler);
}

void
pcep_thread_timer_handler (struct event *thread)
{
  /* data unpacking */
  struct pcep_ctrl_timer_data *data = EVENT_ARG (thread);
  assert (data != NULL);
  ...
  switch (timer_type)
    ...
    case TM_TIMEOUT:
      ...
      if (! pcc_state)
        return;
      pcep_pcc_timeout_handler (ctrl_state, pcc_state, timeout_type, param);
      break;
...
void
pcep_thread_pcep_event (struct event *thread)
{
...
int
schedule_thread_socket (struct ctrl_state *ctrl_state, int pcc_id,
                        ...
                        pcep_ctrl_thread_callback socket_cb)
{
  ...
  if (is_read)
    {
      event_add_read (ctrl_state->self, socket_cb, (void *) data, fd, thread);
    }
  else
    {
      event_add_write (ctrl_state->self, socket_cb, (void *) data, fd, thread);
    }
...
int
pcep_thread_socket_write (void *fpt, void **thread, int fd, void *payload,
                          pcep_ctrl_thread_callback socket_cb)
{
  struct ctrl_state *ctrl_state = ((struct frr_pthread *) fpt)->data;

  return schedule_thread_socket (ctrl_state, 0, SOCK_PCEPLIB, false, payload,
                                 fd, (struct event **) thread, socket_cb);
}

int
pcep_thread_socket_read (void *fpt, void **thread, int fd, void *payload,
                         pcep_ctrl_thread_callback socket_cb)
{
  struct ctrl_state *ctrl_state = ((struct frr_pthread *) fpt)->data;

  return schedule_thread_socket (ctrl_state, 0, SOCK_PCEPLIB, true, payload,
                                 fd, (struct event **) thread, socket_cb);
}

int
pcep_thread_send_ctrl_event (void *fpt, void *payload,
                             pcep_ctrl_thread_callback cb)
{
  struct ctrl_state *ctrl_state = ((struct frr_pthread *) fpt)->data;

  return send_to_thread_with_cb (ctrl_state, 0, EV_PCEPLIB_EVENT, 0, payload,
                                 cb);
}
...
int
send_to_thread (struct ctrl_state *ctrl_state, int pcc_id,
                enum pcep_ctrl_event_type type, uint32_t sub_type,
                void *payload)
{
  return send_to_thread_with_cb (ctrl_state, pcc_id, type, sub_type, payload,
                                 pcep_thread_event_handler);
}

int
send_to_thread_with_cb (struct ctrl_state *ctrl_state, int pcc_id,
                        enum pcep_ctrl_event_type type, uint32_t sub_type,
...
void
pcep_thread_event_handler (struct event *thread)
{
  /* data unpacking */
  struct pcep_ctrl_event_data *data = EVENT_ARG (thread);
  assert (data != NULL);
  ...
  switch (type)
    ...
    case EV_UPDATE_PCE_OPTS:
      ...
      pcep_thread_event_update_pce_options (ctrl_state, pcc_id, pce_opts);
      break;
    case EV_REMOVE_PCC:
      pce_opts = (struct pce_opts *) payload;
      if (pcep_thread_event_remove_pcc (ctrl_state, pce_opts) == 0)
        pcep_pcc_multi_pce_remove_pcc (ctrl_state, ctrl_state->pcc);
      break;
    case EV_PATHD_EVENT:
      assert (payload != NULL);
      path_event_type = (enum pcep_pathd_event_type) sub_type;
      path = (struct path *) payload;
      pcep_thread_event_pathd_event (ctrl_state, path_event_type, path);
      break;
    case EV_SYNC_PATH:
      assert (payload != NULL);
      path = (struct path *) payload;
      pcep_pcc_multi_pce_sync_path (ctrl_state, pcc_id, ctrl_state->pcc);
    ...
    case EV_RESET_PCC_SESSION:
      pcc_state =
          pcep_pcc_get_pcc_by_name (ctrl_state->pcc, (const char *) payload);
      if (pcc_state)
        {
        ...
        {
          flog_warn (EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,
                     "Cannot reset state for PCE: %s", (const char *) payload);
        }
      break;
    case EV_SEND_REPORT:
      assert (payload != NULL);
      ...
      if (pcc_id == 0)
        ...
        {
          pcc_state = pcep_pcc_get_pcc_by_id (ctrl_state->pcc, pcc_id);
          pcep_pcc_send_report (ctrl_state, pcc_state, path, (bool) sub_type);
        }
      break;
    ...
    case EV_SEND_ERROR:
      ...
      pcc_state = pcep_pcc_get_pcc_by_id (ctrl_state->pcc, pcc_id);
      pcep_pcc_send_error (ctrl_state, pcc_state, error, (bool) sub_type);
      break;
    case EV_PCEPLIB_EVENT:
      flog_warn (EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,
                 "Unexpected event received in controller thread: %u", type);
      break;
    }
}

int
pcep_thread_event_update_pcc_options (struct ctrl_state *ctrl_state,
                                      struct pcc_opts *opts)
{
  assert (opts != NULL);
...
int
pcep_thread_event_update_pce_options (struct ctrl_state *ctrl_state,
                                      int pcc_id, struct pce_opts *pce_opts)
{
  ...
  if (current_pcc_id)
    {
      pcc_state = pcep_pcc_get_pcc_by_id (ctrl_state->pcc, current_pcc_id);
    }
  else
    {
      pcc_state = pcep_pcc_initialize (ctrl_state, get_next_id (ctrl_state));
...
int
pcep_thread_event_remove_pcc_by_id (struct ctrl_state *ctrl_state, int pcc_id)
{
  if (pcc_id)
    {
      struct pcc_state *pcc_state =
...
int
pcep_thread_event_remove_pcc_all (struct ctrl_state *ctrl_state)
{
  assert (ctrl_state != NULL);
  ...
  for (int i = 0; i < MAX_PCC; i++)
    {
      pcep_thread_event_remove_pcc_by_id (
          ctrl_state, pcep_pcc_get_pcc_id_by_idx (ctrl_state->pcc, i));
    }
  return 0;
}

int
pcep_thread_event_remove_pcc (struct ctrl_state *ctrl_state,
                              struct pce_opts *pce_opts)
{
  assert (ctrl_state != NULL);
  ...
  if (pce_opts)
    {
      int pcc_id = pcep_pcc_get_pcc_id_by_ip_port (ctrl_state->pcc, pce_opts);
      if (pcc_id)
        {
...
int
pcep_thread_event_sync_path (struct ctrl_state *ctrl_state, int pcc_id,
                             struct path *path)
{
  struct pcc_state *pcc_state =
      pcep_pcc_get_pcc_by_id (ctrl_state->pcc, pcc_id);
...
int
pcep_thread_event_sync_done (struct ctrl_state *ctrl_state, int pcc_id)
{
  struct pcc_state *pcc_state =
      pcep_pcc_get_pcc_by_id (ctrl_state->pcc, pcc_id);
...
int
pcep_thread_event_pathd_event (struct ctrl_state *ctrl_state,
                               enum pcep_pathd_event_type type,
                               struct path *path)
{
  int i;
...
int
send_to_main (struct ctrl_state *ctrl_state, int pcc_id,
              enum pcep_main_event_type type, void *payload)
{
  ...
  event_add_event (ctrl_state->main, pcep_main_event_handler, (void *) data, 0,
                   NULL);
  return 0;
}

void
pcep_main_event_handler (struct event *thread)
{
  /* data unpacking */
...
void
set_ctrl_state (struct frr_pthread *fpt, struct ctrl_state *ctrl_state)
{
  assert (fpt != NULL);
  fpt->data = ctrl_state;
}
...
struct ctrl_state *
get_ctrl_state (struct frr_pthread *fpt)
{
  assert (fpt != NULL);
...
int
get_next_id (struct ctrl_state *ctrl_state)
{
  return ++ctrl_state->pcc_last_id;
}

int
set_pcc_state (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
  assert (ctrl_state != NULL);
...
void
remove_pcc_state (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
  assert (ctrl_state != NULL);
  assert (pcep_pcc_get_pcc_id (pcc_state) != 0);
  ...
  if (idx != -1)
    {
      ...
      ctrl_state->pcc_count--;
      PCEP_DEBUG ("removed pce pcc_id (%d)", pcep_pcc_get_pcc_id (pcc_state));
    }
}

uint32_t
backoff_delay (uint32_t max, uint32_t base, uint32_t retry_count)
{
...
const char *
timer_type_name (enum pcep_ctrl_timer_type type)
{
  switch (type)
    {
    case TM_UNDEFINED:
...
const char *
timeout_type_name (enum pcep_ctrl_timeout_type type)
{
  switch (type)
    {
    case TO_UNDEFINED:
