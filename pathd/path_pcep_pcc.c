...
static bool has_pending_req_for (struct pcc_state *pcc_state,
                                 struct path *path);
...
static inline int
req_entry_compare (const struct req_entry *a, const struct req_entry *b)
{
...
struct pcc_state *
pcep_pcc_initialize (struct ctrl_state *ctrl_state, int index)
{
...
void
pcep_pcc_finalize (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
...
int
compare_pcc_opts (struct pcc_opts *lhs, struct pcc_opts *rhs)
{
  int retval;
...
int
compare_pce_opts (struct pce_opts *lhs, struct pce_opts *rhs)
{
  if (lhs == NULL)
    {
      return 1;
...
int
pcep_pcc_update (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state,
...
void
pcep_pcc_reconnect (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
...
int
pcep_pcc_enable (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
...
int
pcep_pcc_disable (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
  switch (pcc_state->status)
    ...
    case PCEP_PCC_OPERATING:
      ...
      if (get_pce_count_connected (ctrl_state->pcc) == 0)
        {
          pcep_thread_remove_candidate_path_segments (ctrl_state, pcc_state);
        }
      pcc_state->sess = NULL;
      pcc_state->status = PCEP_PCC_DISCONNECTED;
      return 0;
...
void
pcep_pcc_sync_path (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state,
                    struct path *path)
{
  ...
  if ((path->type == SRTE_CANDIDATE_TYPE_DYNAMIC) &&
      (path->first_hop == NULL) && ! has_pending_req_for (pcc_state, path))
    {
  ...
  if (pcc_state->caps.is_stateful)
    {
      if (filter_path (pcc_state, path))
        ...
        {
          PCEP_DEBUG ("%s Skipping %s candidate path %s synchronization",
...
void
pcep_pcc_sync_done (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
  struct req_entry *req;
...
void
pcep_pcc_send_report (struct ctrl_state *ctrl_state,
                      ...
                      bool is_stable)
{
  ...
  if (is_stable && (real_status != PCEP_LSP_OPERATIONAL_DOWN))
    {
      PCEP_DEBUG ("(%s)%s Send report for candidate path (!DOWN) %s", __func__,
                  pcc_state->tag, path->name);
...
void
pcep_pcc_send_error (struct ctrl_state *ctrl_state,
...
void
pcep_pcc_timeout_handler (struct ctrl_state *ctrl_state,
                          struct pcc_state *pcc_state,
...
void
pcep_pcc_pathd_event_handler (struct ctrl_state *ctrl_state,
                              struct pcc_state *pcc_state,
                              ...
                              struct path *path)
{
  ...
  if (! filter_path (pcc_state, path))
    {
      PCEP_DEBUG ("%s Skipping %s candidate path %s event", pcc_state->tag,
  ...
  switch (type)
    ...
    case PCEP_PATH_CREATED:
      ...
      PCEP_DEBUG ("%s Candidate path %s created", pcc_state->tag, path->name);
    ...
    case PCEP_PATH_UPDATED:
      PCEP_DEBUG ("%s Candidate path %s updated", pcc_state->tag, path->name);
      if (pcc_state->caps.is_stateful)
        send_report (pcc_state, path);
      return;
    case PCEP_PATH_REMOVED:
      PCEP_DEBUG ("%s Candidate path %s removed", pcc_state->tag, path->name);
...
void
pcep_pcc_pcep_event_handler (struct ctrl_state *ctrl_state,
  ...
  switch (event->event_type)
    ...
    case PCC_SENT_INVALID_OPEN:
      ...
      PCEP_DEBUG (
          ...
          pcc_state->tag,
          pcc_state->sess->pcc_config.keep_alive_pce_negotiated_timer_seconds,
          pcc_state->sess->pcc_config.dead_timer_pce_negotiated_seconds);
 ...
 */
bool
update_best_pce (struct pcc_state **pcc, int best)
{
  PCEP_DEBUG (" recalculating pce precedence ");
  if (best)
    {
      struct pcc_state *best_pcc_state = pcep_pcc_get_pcc_by_id (pcc, best);
      if (best_pcc_state->previous_best != best_pcc_state->is_best)
        ...
        {
          PCEP_DEBUG (" %s No Resynch best (%i) previous best (%i)",
                      best_pcc_state->tag, best_pcc_state->id,
                      best_pcc_state->previous_best);
        }
    }
...
int
get_best_pce (struct pcc_state **pcc)
{
...
int
get_pce_count_connected (struct pcc_state **pcc)
{
  int count = 0;
  for (int i = 0; i < MAX_PCC; i++)
    {
...
int
get_previous_best_pce (struct pcc_state **pcc)
{
...
int
pcep_pcc_multi_pce_remove_pcc (struct ctrl_state *ctrl_state,
...
int
pcep_pcc_multi_pce_sync_path (struct ctrl_state *ctrl_state, int pcc_id,
                              struct pcc_state **pcc)
{
      ...
      if (previous_best_pcc_id != 0)
        {
          ...
          if (update_best_pce (ctrl_state->pcc, previous_best_pcc_id) == true)
            {
              cancel_comp_requests (
                  ctrl_state,
                  pcep_pcc_get_pcc_by_id (pcc, previous_best_pcc_id));
              pcep_thread_start_sync (ctrl_state, previous_best_pcc_id);
            }
        }
    }
 ...
 * timer expires */
int
pcep_pcc_timer_update_best_pce (struct ctrl_state *ctrl_state, int pcc_id)
{
  int ret = 0;
...
int
pcep_pcc_calculate_best_pce (struct pcc_state **pcc)
{
      ...
      if (pcc[i] && pcc[i]->pce_opts &&
          pcc[i]->status != PCEP_PCC_DISCONNECTED)
        {
          ...
          if (pcc[i]->pce_opts->precedence <= best_precedence)
            {
              if (best_pce != -1 && pcc[best_pce]->pce_opts->precedence ==
                                        pcc[i]->pce_opts->precedence)
                {
                  if (ipaddr_cmp (&pcc[i]->pce_opts->addr,
                                  &pcc[best_pce]->pce_opts->addr) > 0)
              ...
              else
                {
                  if (! pcc[i]->previous_best)
                    {
                      best_precedence = pcc[i]->pce_opts->precedence;
                      best_pce = i;
                    }
                }
            }
  ...
  zlog_debug ("multi-pce: calculate data : sb (%i) sp (%i) oc (%i) b (%i)  ",
  ...
  if (step_0_best != best_pce)
    {
      ...
      if (previous_best_pce != -1 &&
          pcc[previous_best_pce]->status == PCEP_PCC_DISCONNECTED)
        {
          ...
          if (one_connected_pce != -1)
            {
              ...
              zlog_debug ("multi-pce: one connected best pce (default) (%i) ",
                          one_connected_pce + 1);
            }
          else
            {
                  ...
                  if (pcc[i] && pcc[i]->pce_opts)
                    {
                      best_pce = i;
                      pcc[i]->is_best = true;
                      zlog_debug ("(disconnected) best pce (default) (%i) ",
                                  i + 1);
                      break;
                    }
                }
...
int
pcep_pcc_get_pcc_id_by_ip_port (struct pcc_state **pcc,
  ...
  for (int idx = 0; idx < MAX_PCC; idx++)
    {
      if (pcc[idx])
        {
          if ((ipaddr_cmp ((const struct ipaddr *) &pcc[idx]->pce_opts->addr,
                           (const struct ipaddr *) &pce_opts->addr) == 0) &&
              pcc[idx]->pce_opts->port == pce_opts->port)
            {
              zlog_debug ("found pcc_id (%d) idx (%d)", pcc[idx]->id, idx);
              return pcc[idx]->id;
            }
        }
    }
...
int
pcep_pcc_get_pcc_id_by_idx (struct pcc_state **pcc, int idx)
{
  if (pcc == NULL || idx < 0)
    {
      return 0;
...
struct pcc_state *
pcep_pcc_get_pcc_by_id (struct pcc_state **pcc, int id)
{
  if (pcc == NULL || id < 0)
    {
      return NULL;
      ...
      if (pcc[i])
        {
          if (pcc[i]->id == id)
            {
              zlog_debug ("found id (%d) pcc_idx (%d)", pcc[i]->id, i);
              return pcc[i];
            }
        }
    }
...
struct pcc_state *
pcep_pcc_get_pcc_by_name (struct pcc_state **pcc, const char *pce_name)
{
...
int
pcep_pcc_get_pcc_idx_by_id (struct pcc_state **pcc, int id)
{
  if (pcc == NULL)
    {
      return -1;
      ...
      if (pcc[idx])
        {
          if (pcc[idx]->id == id)
            {
              zlog_debug ("found pcc_id (%d) array_idx (%d)", pcc[idx]->id,
                          idx);
              return idx;
            }
        }
...
int
pcep_pcc_get_free_pcc_idx (struct pcc_state **pcc)
{
  assert (pcc != NULL);
...
int
pcep_pcc_get_pcc_id (struct pcc_state *pcc)
{
  return ((pcc == NULL) ? 0 : pcc->id);
}

void
pcep_pcc_copy_pcc_info (struct pcc_state **pcc, struct pcep_pcc_info *pcc_info)
{
  struct pcc_state *pcc_state =
...
void
handle_pcep_open (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state,
                  struct pcep_message *msg)
{
...
void
handle_pcep_message (struct ctrl_state *ctrl_state,
...
void
handle_pcep_lsp_update (struct ctrl_state *ctrl_state,
                        struct pcc_state *pcc_state, struct pcep_message *msg)
{
  struct path *path;
...
void
continue_pcep_lsp_update (struct ctrl_state *ctrl_state,
                          struct pcc_state *pcc_state, struct path *path,
                          void *payload)
{
...
void
handle_pcep_lsp_initiate (struct ctrl_state *ctrl_state,
                          struct pcc_state *pcc_state,
                          struct pcep_message *msg)
{
  ...
  if (path->do_remove)
    {
      ...
      frr_each (nbkey_map, &pcc_state->nbkey_map, key)
        {
          ipaddr2str (&key->nbkey.endpoint, endpoint, sizeof (endpoint));
          flog_warn (
      ...
      if (path->name == NULL)
        {
          ...
          flog_warn (EC_PATH_PCEP_PROTOCOL_ERROR,
                     ...
                     format_pcep_message (msg));
          send_pcep_error (pcc_state, PCEP_ERRT_RECEPTION_OF_INV_OBJECT,
...
void
handle_pcep_comp_reply (struct ctrl_state *ctrl_state,
                        struct pcc_state *pcc_state, struct pcep_message *msg)
{
  char err[MAX_ERROR_MSG_SIZE] = "";
  struct req_entry *req;
  ...
  if (req == NULL)
    {
       ...
       */
      PCEP_DEBUG ("%s Received computation reply for unknown request %d",
  ...
  PCEP_DEBUG ("%s Received computation reply %d (no-path: %s)", pcc_state->tag,
              path->req_id, path->no_path ? "true" : "false");
  PCEP_DEBUG_PATH ("%s", format_path (path));
...
const char *
ipaddr_type_name (struct ipaddr *addr)
{
...
bool
filter_path (struct pcc_state *pcc_state, struct path *path)
{
...
void
select_pcc_addresses (struct pcc_state *pcc_state)
{
...
void
select_transport_address (struct pcc_state *pcc_state)
{
...
void
update_tag (struct pcc_state *pcc_state)
{
  if (pcc_state->pce_opts != NULL)
    {
      ...
      if (IS_IPADDR_V6 (&pcc_state->pce_opts->addr))
        {
          snprintfrr (pcc_state->tag, sizeof (pcc_state->tag), "%pI6:%i (%u)",
                      &pcc_state->pce_opts->addr.ipaddr_v6,
                      pcc_state->pce_opts->port, pcc_state->id);
        }
      else
        {
          snprintfrr (pcc_state->tag, sizeof (pcc_state->tag), "%pI4:%i (%u)",
...
void
update_originator (struct pcc_state *pcc_state)
{
  char *originator;
  if (pcc_state->originator != NULL)
    {
...
void
schedule_reconnect (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state)
{
...
void
schedule_session_timeout (struct ctrl_state *ctrl_state,
                          struct pcc_state *pcc_state)
{
...
void
cancel_session_timeout (struct ctrl_state *ctrl_state,
                        struct pcc_state *pcc_state)
{
  /* No need to schedule timeout if multiple PCEs are connected */
  if (pcc_state->t_session_timeout == NULL)
...
void
send_pcep_message (struct pcc_state *pcc_state, struct pcep_message *msg)
{
  if (pcc_state->sess != NULL)
    {
...
void
send_pcep_error (struct pcc_state *pcc_state, enum pcep_error_type error_type,
                 enum pcep_error_value error_value, struct path *trigger_path)
{
  struct pcep_message *msg;
...
void
send_report (struct pcc_state *pcc_state, struct path *path)
{
...
void
specialize_outgoing_path (struct pcc_state *pcc_state, struct path *path)
{
...
void
specialize_incoming_path (struct pcc_state *pcc_state, struct path *path)
{
...
bool
validate_incoming_path (struct pcc_state *pcc_state, struct path *path,
...
void
send_comp_request (struct ctrl_state *ctrl_state, struct pcc_state *pcc_state,
                   struct req_entry *req)
{
  assert (req != NULL);
  ...
  pcep_thread_schedule_timeout (ctrl_state, pcc_state->id,
                                TO_COMPUTATION_REQUEST, timeout, (void *) req,
                                &req->t_retry);
}

void
cancel_comp_requests (struct ctrl_state *ctrl_state,
                      struct pcc_state *pcc_state)
{
...
void
cancel_comp_request (struct ctrl_state *ctrl_state,
...
void
set_pcc_address (struct pcc_state *pcc_state, struct lsp_nb_key *nbkey,
                 struct ipaddr *addr)
{
  select_pcc_addresses (pcc_state);
...
void
lookup_plspid (struct pcc_state *pcc_state, struct path *path)
{
  ...
  if (path->nbkey.color != 0)
    {
      ...
      if (plspid_mapping == NULL)
        {
          plspid_mapping = XCALLOC (MTYPE_PCEP, sizeof (*plspid_mapping));
          ...
          nbkey_mapping = XCALLOC (MTYPE_PCEP, sizeof (*nbkey_mapping));
...
void
lookup_nbkey (struct pcc_state *pcc_state, struct path *path)
{
...
void
free_req_entry (struct req_entry *req)
{
...
struct req_entry *
push_new_req (struct pcc_state *pcc_state, struct path *path)
{
  struct req_entry *req;
...
void
repush_req (struct pcc_state *pcc_state, struct req_entry *req)
{
...
struct req_entry *
pop_req (struct pcc_state *pcc_state, uint32_t reqid)
{
...
struct req_entry *
pop_req_no_reqid (struct pcc_state *pcc_state, uint32_t reqid)
{
  struct path path = { .req_id = reqid };
  struct req_entry key = { .path = &path };
  struct req_entry *req;
...
bool
add_reqid_mapping (struct pcc_state *pcc_state, struct path *path)
{
...
void
remove_reqid_mapping (struct pcc_state *pcc_state, struct path *path)
{
...
uint32_t
lookup_reqid (struct pcc_state *pcc_state, struct path *path)
{
  struct req_map_data key, *mapping;
  key.nbkey = path->nbkey;
  mapping = req_map_find (&pcc_state->req_map, &key);
...
bool
has_pending_req_for (struct pcc_state *pcc_state, struct path *path)
{
  struct req_entry key = { .path = path };
  struct req_entry *req;
      ...
      if (! req)
        {
          ...
          PCEP_DEBUG_PATH ("%s Purge pending reqid: no_path(%s)", __func__,
                           path->no_path ? "TRUE" : "FALSE");
          if (lookup_reqid (pcc_state, path) != 0)
            {
              PCEP_DEBUG_PATH ("%s Purge pending reqid: DONE ", __func__);
...
static uint32_t
hash_nbkey (const struct lsp_nb_key *nbkey)
{
  uint32_t hash;
...
static int
cmp_nbkey (const struct lsp_nb_key *a, const struct lsp_nb_key *b)
{
...
int
plspid_map_cmp (const struct plspid_map_data *a,
                const struct plspid_map_data *b)
{
...
uint32_t
plspid_map_hash (const struct plspid_map_data *e)
{
  return hash_nbkey (&e->nbkey);
}

int
nbkey_map_cmp (const struct nbkey_map_data *a, const struct nbkey_map_data *b)
{
...
uint32_t
nbkey_map_hash (const struct nbkey_map_data *e)
{
  return e->plspid;
}

int
req_map_cmp (const struct req_map_data *a, const struct req_map_data *b)
{
  return cmp_nbkey (&a->nbkey, &b->nbkey);
}

uint32_t
req_map_hash (const struct req_map_data *e)
{
  return hash_nbkey (&e->nbkey);
}
