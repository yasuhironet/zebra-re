...
static void _format_pcep_object_details (int ps,
...
static void
_format_pcep_object_tlv (int ps, struct pcep_object_tlv_header *tlv_header);
static void
...
const char *
pcc_status_name (enum pcc_status status)
{
  switch (status)
    {
...
const char *
pcep_event_type_name (pcep_event_type event_type)
{
  switch (event_type)
    {
...
const char *
pcep_error_type_name (enum pcep_error_type error_type)
{
  switch (error_type)
    {
...
const char *
pcep_error_value_name (enum pcep_error_type error_type,
...
const char *
pcep_message_type_name (enum pcep_message_types pcep_message_type)
{
...
const char *
pcep_object_class_name (enum pcep_object_classes obj_class)
{
  switch (obj_class)
    {
...
const char *
pcep_object_type_name (enum pcep_object_classes obj_class,
...
const char *
pcep_lsp_status_name (enum pcep_lsp_operational_status status)
{
  switch (status)
    {
...
const char *
pcep_tlv_type_name (enum pcep_object_tlv_types tlv_type)
{
  switch (tlv_type)
    {
...
const char *
pcep_ro_type_name (enum pcep_ro_subobj_types ro_type)
{
  switch (ro_type)
    {
...
const char *
pcep_nai_type_name (enum pcep_sr_subobj_nai nai_type)
{
  switch (nai_type)
    {
...
const char *
pcep_metric_type_name (enum pcep_metric_types type)
{
  switch (type)
    {
...
const char *
pcep_nopath_tlv_err_code_name (enum pcep_nopath_tlv_err_codes type)
{
  switch (type)
    {
...
const char *
format_objfun_set (uint32_t flags)
{
  int i, c;
  PATHD_FORMAT_INIT ();
...
const char *
format_pcc_opts (struct pcc_opts *opts)
{
  PATHD_FORMAT_INIT ();
...
const char *
format_pcc_state (struct pcc_state *state)
{
  PATHD_FORMAT_INIT ();
...
const char *
format_ctrl_state (struct ctrl_state *state)
{
  PATHD_FORMAT_INIT ();
...
const char *
format_pcep_event (pcep_event *event)
{
  PATHD_FORMAT_INIT ();
...
const char *
format_pcep_message (struct pcep_message *msg)
{
  PATHD_FORMAT_INIT ();
...
void
_format_pcc_opts (int ps, struct pcc_opts *opts)
{
  if (opts == NULL)
    {
      PATHD_FORMAT ("NULL\n");
...
void
_format_pce_opts (int ps, struct pce_opts *opts)
{
  if (opts == NULL)
    {
      PATHD_FORMAT ("NULL\n");
...
void
_format_pcc_caps (int ps, struct pcep_caps *caps)
{
  int ps2 = ps + DEBUG_IDENT_SIZE;
  PATHD_FORMAT ("\n");
...
void
_format_pcc_state (int ps, struct pcc_state *state)
{
  if (state == NULL)
    {
      PATHD_FORMAT ("NULL\n");
...
void
_format_ctrl_state (int ps, struct ctrl_state *state)
{
  if (state == NULL)
    {
      PATHD_FORMAT ("NULL\n");
...
void
_format_path (int ps, struct path *path)
{
  if (path == NULL)
    {
      PATHD_FORMAT ("NULL\n");
  ...
  else
    {
      ...
      if (path->first_metric == NULL)
        ...
        {
          PATHD_FORMAT ("%*smetrics: \n", ps2, "");
          for (struct path_metric *metric = path->first_metric; NULL != metric;
               metric = metric->next)
            {
              PATHD_FORMAT ("%*s- ", ps3 - 2, "");
...
void
_format_path_metric (int ps, struct path_metric *metric)
{
...
void
_format_path_hop (int ps, struct path_hop *hop)
{
      ...
      switch (hop->nai.type)
        ...
        case PCEP_SR_SUBOBJ_NAI_UNNUMBERED_IPV4_ADJACENCY:
          PATHD_FORMAT ("%*sNAI: %pI6(%u)/%pI6(%u)\n", ps, "",
                        &hop->nai.local_addr.ipaddr_v6, hop->nai.local_iface,
                        &hop->nai.remote_addr.ipaddr_v6,
...
void
_format_pcep_event (int ps, pcep_event *event)
{
  if (event == NULL)
    {
      PATHD_FORMAT ("NULL\n");
  ...
  else
    {
      ...
      PATHD_FORMAT ("%*sevent_time: %s", ps2, "", ctime (&event->event_time));
...
void
_format_pcep_message (int ps, struct pcep_message *msg)
{
  if (msg == NULL)
    {
      PATHD_FORMAT ("NULL\n");
...
void
_format_pcep_objects (int ps, double_linked_list *objs)
{
  if (objs == NULL)
    {
      PATHD_FORMAT ("NULL\n");
  ...
  else
    {
      ...
      PATHD_FORMAT ("\n");
      for (node = objs->head, i = 0; node != NULL; node = node->next_node, i++)
        {
          struct pcep_object_header *obj =
...
void
_format_pcep_object (int ps, struct pcep_object_header *obj)
{
  if (obj == NULL)
    {
      PATHD_FORMAT ("NULL\n");
...
void
_format_pcep_object_details (int ps, struct pcep_object_header *obj)
{
  switch (TUP (obj->object_class, obj->object_type))
    ...
    case TUP (PCEP_OBJ_CLASS_ERO, PCEP_OBJ_TYPE_ERO):
      ...
      break;
    case TUP (PCEP_OBJ_CLASS_METRIC, PCEP_OBJ_TYPE_METRIC):
      _format_pcep_object_metric (ps, (struct pcep_object_metric *) obj);
      break;
    case TUP (PCEP_OBJ_CLASS_BANDWIDTH, PCEP_OBJ_TYPE_BANDWIDTH_REQ):
    case TUP (PCEP_OBJ_CLASS_BANDWIDTH, PCEP_OBJ_TYPE_BANDWIDTH_CISCO):
      _format_pcep_object_bandwidth (ps, (struct pcep_object_bandwidth *) obj);
      break;
    case TUP (PCEP_OBJ_CLASS_NOPATH, PCEP_OBJ_TYPE_NOPATH):
      _format_pcep_object_nopath (ps, (struct pcep_object_nopath *) obj);
      break;
    case TUP (PCEP_OBJ_CLASS_OF, PCEP_OBJ_TYPE_OF):
...
void
_format_pcep_object_error (int ps, struct pcep_object_error *obj)
{
...
void
_format_pcep_object_open (int ps, struct pcep_object_open *obj)
{
...
void
_format_pcep_object_rp (int ps, struct pcep_object_rp *obj)
{
...
void
_format_pcep_object_srp (int ps, struct pcep_object_srp *obj)
{
...
void
_format_pcep_object_lsp (int ps, struct pcep_object_lsp *obj)
{
  PATHD_FORMAT ("%*splsp_id: %u\n", ps, "", obj->plsp_id);
  PATHD_FORMAT ("%*sstatus: %s\n", ps, "",
...
void
_format_pcep_object_lspa (int ps, struct pcep_object_lspa *obj)
{
...
void
_format_pcep_object_metric (int ps, struct pcep_object_metric *obj)
{
  PATHD_FORMAT ("%*stype: %s (%u)\n", ps, "",
...
void
_format_pcep_object_bandwidth (int ps, struct pcep_object_bandwidth *obj)
{
  PATHD_FORMAT ("%*sbandwidth: %f\n", ps, "", obj->bandwidth);
}

void
_format_pcep_object_nopath (int ps, struct pcep_object_nopath *obj)
{
  PATHD_FORMAT ("%*sni: %u\n", ps, "", obj->ni);
  PATHD_FORMAT ("%*sflag_c: %u\n", ps, "", obj->flag_c);
  PATHD_FORMAT ("%*serr_code: %s (%u)\n", ps, "",
                pcep_nopath_tlv_err_code_name (obj->err_code), obj->err_code);
}
...
void
_format_pcep_object_ro (int ps, struct pcep_object_ro *obj)
{
  ...
  for (node = obj_list->head, i = 0; node != NULL; node = node->next_node, i++)
    {
...
void
_format_pcep_object_ro_details (int ps, struct pcep_object_ro_subobj *ro)
{
  switch (ro->ro_subobj_type)
    {
    case RO_SUBOBJ_TYPE_IPV4:
      _format_pcep_object_ro_ipv4 (ps, (struct pcep_ro_subobj_ipv4 *) ro);
      break;
    case RO_SUBOBJ_TYPE_SR:
...
void
_format_pcep_object_ro_ipv4 (int ps, struct pcep_ro_subobj_ipv4 *obj)
{
...
void
_format_pcep_object_ro_sr (int ps, struct pcep_ro_subobj_sr *obj)
{
  PATHD_FORMAT ("%*snai_type = %s (%u)\n", ps, "",
...
void
_format_pcep_object_tlvs (int ps, struct pcep_object_header *obj)
{
  ...
  for (node = tlv_list->head, i = 0; node != NULL; node = node->next_node, i++)
    {
  ...
  switch (tlv_header->type)
    ...
    case PCEP_OBJ_TLV_TYPE_SYMBOLIC_PATH_NAME:
      _format_pcep_object_tlv_symbolic_path_name (
          ps, (struct pcep_object_tlv_symbolic_path_name *) tlv_header);
      break;
    case PCEP_OBJ_TLV_TYPE_STATEFUL_PCE_CAPABILITY:
      _format_pcep_object_tlv_stateful_pce_capability (
          ps, (struct pcep_object_tlv_stateful_pce_capability *) tlv_header);
      break;
    case PCEP_OBJ_TLV_TYPE_SR_PCE_CAPABILITY:
      _format_pcep_object_tlv_sr_pce_capability (
          ps, (struct pcep_object_tlv_sr_pce_capability *) tlv_header);
      break;
    case PCEP_OBJ_TLV_TYPE_PATH_SETUP_TYPE:
      _format_pcep_object_tlv_path_setup_type (
          ps, (struct pcep_object_tlv_path_setup_type *) tlv_header);
      break;
    case PCEP_OBJ_TLV_TYPE_NO_PATH_VECTOR:
    case PCEP_OBJ_TLV_TYPE_OBJECTIVE_FUNCTION_LIST:
    case PCEP_OBJ_TLV_TYPE_VENDOR_INFO:
