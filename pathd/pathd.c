...
static inline int
srte_segment_entry_compare (const struct srte_segment_entry *a,
...
static inline int
srte_segment_list_compare (const struct srte_segment_list *a,
...
static inline int
srte_candidate_compare (const struct srte_candidate *a,
                        const struct srte_candidate *b)
{
  return a->preference - b->preference;
}
RB_GENERATE (srte_candidate_head, srte_candidate, entry,
             srte_candidate_compare)

/* Generate rb-tree of SR Policy instances. */
static inline int
srte_policy_compare (const struct srte_policy *a, const struct srte_policy *b)
{
...
static void
srte_policy_status_log (struct srte_policy *policy)
{
 ...
 *
 * The given segment list structure will be freed and should not be used
 * anymore after calling this function.
 *
 * @param segment_list the segment list to remove from pathd.
 */
void
srte_segment_list_del (struct srte_segment_list *segment_list)
{
  struct srte_segment_entry *segment, *safe_seg;
  RB_FOREACH_SAFE (segment, srte_segment_entry_head, &segment_list->segments,
                   safe_seg)
    {
 ...
 */
void
srte_segment_entry_del (struct srte_segment_entry *segment)
{
 ...
 */
int
srte_segment_entry_set_nai (struct srte_segment_entry *segment,
 ...
 */
void
srte_segment_set_local_modification (struct srte_segment_list *s_list,
 ...
 */
struct srte_policy *
srte_policy_add (uint32_t color, struct ipaddr *endpoint,
                 enum srte_protocol_origin origin, const char *originator)
{
  ...
  if (originator != NULL)
    strlcpy (policy->originator, originator, sizeof (policy->originator));
 ...
 */
void
srte_policy_del (struct srte_policy *policy)
{
  ...
  while (! RB_EMPTY (srte_candidate_head, &policy->candidate_paths))
    {
      candidate = RB_ROOT (srte_candidate_head, &policy->candidate_paths);
 ...
 */
struct srte_policy *
srte_policy_find (uint32_t color, struct ipaddr *endpoint)
{
 ...
 */
int
srte_policy_update_ted_sid (void)
{
      ...
      RB_FOREACH (s_entry, srte_segment_entry_head, &s_list->segments)
        {
          PATH_TED_DEBUG (
              ...
              __func__, s_list->name, s_entry->index, s_entry->sid_value,
              s_entry->nai_local_prefix_len, s_entry->nai_local_iface,
          ...
          switch (s_entry->nai_type)
            ...
            case SRTE_SEGMENT_NAI_TYPE_IPV4_ADJACENCY:
              number_of_sid_clashed +=
                  srte_ted_do_query_type_f (s_entry, &s_entry->nai_local_addr,
                                            &s_entry->nai_remote_addr);
              break;
            case SRTE_SEGMENT_NAI_TYPE_IPV6_LOCAL_IFACE:
              prefix_cli.family = AF_INET6;
              prefix_cli.prefixlen = s_entry->nai_local_prefix_len;
              prefix_cli.u.prefix6 = s_entry->nai_local_addr.ip._v6_addr;
              number_of_sid_clashed += srte_ted_do_query_type_e (
                  s_entry, &prefix_cli, s_entry->nai_local_iface);
              break;
            case SRTE_SEGMENT_NAI_TYPE_IPV4_LOCAL_IFACE:
              prefix_cli.family = AF_INET;
              prefix_cli.prefixlen = s_entry->nai_local_prefix_len;
              prefix_cli.u.prefix4 = s_entry->nai_local_addr.ip._v4_addr;
              number_of_sid_clashed += srte_ted_do_query_type_e (
                  s_entry, &prefix_cli, s_entry->nai_local_iface);
              break;
            case SRTE_SEGMENT_NAI_TYPE_IPV6_ALGORITHM:
              ...
              prefix_cli.prefixlen = s_entry->nai_local_prefix_len;
              prefix_cli.u.prefix6 = s_entry->nai_local_addr.ip._v6_addr;
              number_of_sid_clashed += srte_ted_do_query_type_c (
                  s_entry, &prefix_cli, s_entry->nai_algorithm);
              break;
            case SRTE_SEGMENT_NAI_TYPE_IPV4_ALGORITHM:
              prefix_cli.family = AF_INET;
              prefix_cli.prefixlen = s_entry->nai_local_prefix_len;
 ...
 */
void
srte_policy_update_binding_sid (struct srte_policy *policy,
...
srte_policy_best_candidate (const struct srte_policy *policy)
{
  ...
  RB_FOREACH_REVERSE (candidate, srte_candidate_head, &policy->candidate_paths)
    {
...
srte_apply_changes (void)
{
  ...
  RB_FOREACH_SAFE (segment_list, srte_segment_list_head, &srte_segment_lists,
                   safe_sl)
    {
 ...
 */
void
srte_policy_apply_changes (struct srte_policy *policy)
{
  ...
  if (new_best_candidate != old_best_candidate)
    {
      PATH_POLICY_DEBUG (
          "SR-TE(%s, %u): best candidate changed from %s to %s", endpoint,
          policy->color,
  ...
  else if (new_best_candidate)
    {
      ...
      bool candidate_changed =
          CHECK_FLAG (new_best_candidate->flags, F_CANDIDATE_MODIFIED);
      ...
      if (candidate_changed || segment_list_changed)
        {
          PATH_POLICY_DEBUG ("SR-TE(%s, %u): best candidate %s changed",
  ...
  RB_FOREACH_SAFE (candidate, srte_candidate_head, &policy->candidate_paths,
                   safe)
    {
      if (CHECK_FLAG (candidate->flags, F_CANDIDATE_DELETED))
        {
...
srte_candidate_add (struct srte_policy *policy, uint32_t preference,
                    enum srte_protocol_origin origin, const char *originator)
{
  struct srte_candidate *candidate;
  struct srte_lsp *lsp;
 ...
 */
void
srte_candidate_del (struct srte_candidate *candidate)
{
  struct srte_policy *srte_policy = candidate->policy;

  RB_REMOVE (srte_candidate_head, &srte_policy->candidate_paths, candidate);
 ...
 */
void
srte_candidate_set_bandwidth (struct srte_candidate *candidate,
                              float bandwidth, bool required)
{
  ...
  ipaddr2str (&policy->endpoint, endpoint, sizeof (endpoint));
  PATH_POLICY_DEBUG (
      "SR-TE(%s, %u): candidate %s %sconfig bandwidth set to %f B/s", endpoint,
      policy->color, candidate->name, required ? "required " : "", bandwidth);
 ...
 */
void
srte_lsp_set_bandwidth (struct srte_lsp *lsp, float bandwidth, bool required)
{
  ...
  ipaddr2str (&policy->endpoint, endpoint, sizeof (endpoint));
  PATH_POLICY_DEBUG (
      "SR-TE(%s, %u): candidate %s %slsp bandwidth set to %f B/s", endpoint,
      policy->color, candidate->name, required ? "required" : "", bandwidth);
 ...
 */
void
srte_candidate_unset_bandwidth (struct srte_candidate *candidate)
{
  struct srte_policy *policy = candidate->policy;
  char endpoint[ENDPOINT_STR_LENGTH];
 ...
 */
void
srte_lsp_unset_bandwidth (struct srte_lsp *lsp)
{
  struct srte_candidate *candidate = lsp->candidate;
  struct srte_policy *policy = candidate->policy;
  char endpoint[ENDPOINT_STR_LENGTH];
 ...
 */
void
srte_candidate_set_metric (struct srte_candidate *candidate,
                           ...
                           bool required, bool is_bound, bool is_computed)
{
  ...
  PATH_POLICY_DEBUG (
      ...
      endpoint, policy->color, candidate->name, required ? "required " : "",
      srte_candidate_metric_name (type), type, value,
      is_bound ? "true" : "false", is_computed ? "true" : "false");
  assert ((type > 0) && (type <= MAX_METRIC_TYPE));
  srte_set_metric (&candidate->metrics[type - 1], value, required, is_bound,
                   is_computed);
 ...
 */
void
srte_lsp_set_metric (struct srte_lsp *lsp,
                     enum srte_candidate_metric_type type, float value,
                     bool required, bool is_bound, bool is_computed)
{
  struct srte_candidate *candidate = lsp->candidate;
...
void
srte_set_metric (struct srte_metric *metric, float value, bool required,
 ...
 */
void
srte_candidate_unset_metric (struct srte_candidate *candidate,
                             enum srte_candidate_metric_type type)
{
  ...
  ipaddr2str (&policy->endpoint, endpoint, sizeof (endpoint));
  PATH_POLICY_DEBUG ("SR-TE(%s, %u): candidate %s config metric %s (%u) unset",
                     endpoint, policy->color, candidate->name,
 ...
 */
void
srte_lsp_unset_metric (struct srte_lsp *lsp,
                       enum srte_candidate_metric_type type)
{
  struct srte_candidate *candidate = lsp->candidate;
  struct srte_policy *policy = candidate->policy;
  ...
  ipaddr2str (&policy->endpoint, endpoint, sizeof (endpoint));
  PATH_POLICY_DEBUG ("SR-TE(%s, %u): candidate %s lsp metric %s (%u) unset",
                     endpoint, policy->color, candidate->name,
                     srte_candidate_metric_name (type), type);
  assert ((type > 0) && (type <= MAX_METRIC_TYPE));
...
void
srte_unset_metric (struct srte_metric *metric)
{
 ...
 */
void
srte_candidate_set_objfun (struct srte_candidate *candidate, bool required,
                           enum objfun_type type)
{
  ...
  SET_FLAG (candidate->flags, F_CANDIDATE_MODIFIED);
  PATH_POLICY_DEBUG (
      "SR-TE(%s, %u): candidate %s %sobjective function set to %s", endpoint,
      policy->color, candidate->name, required ? "required " : "",
      objfun_type_name (type));
}
 ...
 */
void
srte_candidate_unset_objfun (struct srte_candidate *candidate)
{
  struct srte_policy *policy = candidate->policy;
  char endpoint[ENDPOINT_STR_LENGTH];
...
static uint32_t
filter_type_to_flag (enum affinity_filter_type type)
{
  switch (type)
    {
...
static const char *
filter_type_name (enum affinity_filter_type type)
{
  switch (type)
    {
    case AFFINITY_FILTER_EXCLUDE_ANY:
 ...
 *
 * @param candidate The candidate path of which the constraint should be
 * changed
 * @param type The affinity constraint type to set
 * @param filter The bitmask filter of the constraint
 */
void
srte_candidate_set_affinity_filter (struct srte_candidate *candidate,
                                    ...
                                    uint32_t filter)
{
  ...
  PATH_POLICY_DEBUG (
      "SR-TE(%s, %u): candidate %s affinity filter %s set to 0x%08x", endpoint,
      policy->color, candidate->name, filter_type_name (type), filter);
}
 ...
 */
void
srte_candidate_unset_affinity_filter (struct srte_candidate *candidate,
                                      enum affinity_filter_type type)
{
  struct srte_policy *policy = candidate->policy;
  char endpoint[ENDPOINT_STR_LENGTH];
  ...
  PATH_POLICY_DEBUG ("SR-TE(%s, %u): candidate %s affinity filter %s unset",
                     endpoint, policy->color, candidate->name,
 ...
 */
struct srte_candidate *
srte_candidate_find (struct srte_policy *policy, uint32_t preference)
{
 ...
 */
struct srte_segment_entry *
srte_segment_entry_find (struct srte_segment_list *segment_list,
                         uint32_t index)
{
  struct srte_segment_entry search;

  search.index = index;
  return RB_FIND (srte_segment_entry_head, &segment_list->segments, &search);
}
 ...
 */
void
srte_candidate_status_update (struct srte_candidate *candidate, int status)
{
  struct srte_policy *policy = candidate->policy;
  char endpoint[ENDPOINT_STR_LENGTH];
 ...
 */
void
srte_candidate_unset_segment_list (const char *originator, bool force)
{
  if (originator == NULL)
    {
  ...
  RB_FOREACH (policy, srte_policy_head, &srte_policies)
    {
      ...
      RB_FOREACH (candidate, srte_candidate_head, &policy->candidate_paths)
        {
          PATH_POLICY_DEBUG ("Unset segment lists checking candidate %s",
                             candidate->name);
          ...
          if (strncmp (segment_list->originator, originator,
              ...
              force)
            {
              PATH_POLICY_DEBUG ("Unset segment list %s", segment_list->name);
 ...
 */
const char *
srte_origin2str (enum srte_protocol_origin origin)
{
  switch (origin)
    {
...
void
path_policy_show_debugging (struct vty *vty)
{
  if (DEBUG_FLAGS_CHECK (&path_policy_debug, PATH_POLICY_DEBUG_BASIC))
...
void
trigger_pathd_candidate_created (struct srte_candidate *candidate)
{
...
void
trigger_pathd_candidate_created_timer (struct event *thread)
{
...
void
trigger_pathd_candidate_updated (struct srte_candidate *candidate)
{
  /* The hook is called asynchronously to let the PCEP module
  time to send a response to the PCE before receiving any updates from
  pathd. In addition, a minimum amount of time need to pass before
...
void
trigger_pathd_candidate_updated_timer (struct event *thread)
{
  struct srte_candidate *candidate = EVENT_ARG (thread);
  candidate->hook_timer = NULL;
...
void
trigger_pathd_candidate_removed (struct srte_candidate *candidate)
{
...
int32_t
srte_ted_do_query_type_c (struct srte_segment_entry *entry,
                          struct prefix *prefix_cli, uint32_t algo)
{
  ...
  if (ted_sid == MPLS_LABEL_NONE)
    {
      zlog_warn (" %s: PATHD-TED: SL: ERROR query C : ted-sid (%d)", __func__,
                 ted_sid);
    }
  else
    {
      PATH_TED_DEBUG ("%s: PATHD-TED: SL: Success query C : ted-sid (%d)",
                      __func__, ted_sid);
    }
  if (CHECK_SID (entry->segment_list->protocol_origin, ted_sid,
    ...
    srte_segment_set_local_modification (entry->segment_list, entry, ted_sid);
  return status;
}

int32_t
srte_ted_do_query_type_e (struct srte_segment_entry *entry,
                          struct prefix *prefix_cli, uint32_t local_iface)
{
  int32_t status = 0;
  uint32_t ted_sid = MPLS_LABEL_NONE;
  ...
  if (ted_sid == MPLS_LABEL_NONE)
    {
      zlog_warn (" %s: PATHD-TED: SL: ERROR query E : ted-sid (%d)", __func__,
                 ted_sid);
    }
  else
    {
      PATH_TED_DEBUG ("%s: PATHD-TED: SL: Success query E : ted-sid (%d)",
                      __func__, ted_sid);
    }
  if (CHECK_SID (entry->segment_list->protocol_origin, ted_sid,
                 entry->sid_value))
...
int32_t
srte_ted_do_query_type_f (struct srte_segment_entry *entry,
                          struct ipaddr *local, struct ipaddr *remote)
{
  ...
  if (ted_sid == MPLS_LABEL_NONE)
    {
      zlog_warn ("%s:SL:  ERROR query F : ted-sid (%d)", __func__, ted_sid);
    }
  else
    {
      PATH_TED_DEBUG ("%s:SL: Success query F : ted-sid (%d)", __func__,
                      ted_sid);
    }
  if (CHECK_SID (entry->segment_list->protocol_origin, ted_sid,
                 entry->sid_value))
