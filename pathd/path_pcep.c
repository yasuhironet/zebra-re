 ...
 * Globals.
 */
static struct pcep_glob pcep_glob_space = { .dbg = { 0,
                                                     "pathd module: pcep" } };
...
struct path_metric *
pcep_copy_metrics (struct path_metric *metric)
{
  if (metric == NULL)
    return NULL;
...
struct path_hop *
pcep_copy_hops (struct path_hop *hop)
{
  if (hop == NULL)
    return NULL;
...
struct path *
pcep_copy_path (struct path *path)
{
...
void
pcep_free_path (struct path *path)
{
  struct path_hop *hop;
  struct path_metric *metric;
  char *tmp;
...
void
notify_status (struct path *path, bool not_changed)
{
  ...
  if ((resp = path_pcep_config_get_path (&path->nbkey)))
    {
      ...
      flog_warn (EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,
                 ...
                 path->name);
      pcep_ctrl_send_report (pcep_g->fpt, path->pcc_id, resp, not_changed);
    }
}
...
int
pcep_main_event_handler (enum pcep_main_event_type type, int pcc_id,
                         void *payload)
{
  int ret = 0;
  ...
  switch (type)
    ...
    case PCEP_MAIN_EVENT_INITIATE_CANDIDATE:
      assert (payload != NULL);
      ret = pcep_main_event_initiate_candidate ((struct path *) payload);
      break;
    ...
    case PCEP_MAIN_EVENT_UNDEFINED:
      flog_warn (EC_PATH_PCEP_RECOVERABLE_INTERNAL_ERROR,
                 "Unexpected event received in the main thread: %u", type);
      break;
    }
...
int
pcep_main_event_start_sync (int pcc_id)
{
...
int
pcep_main_event_start_sync_cb (struct path *path, void *arg)
{
  int *pcc_id = (int *) arg;
...
int
pcep_main_event_initiate_candidate (struct path *path)
{
  int ret = 0;
  ...
  if (path->do_remove)
    {
      ...
      switch (ret)
        {
        case ERROR_19_1:
          error->error_value = PCEP_ERRV_LSP_UPDATE_FOR_NON_DELEGATED_LSP;
          break;
        case ERROR_19_3:
          error->error_value = PCEP_ERRV_LSP_UPDATE_UNKNOWN_PLSP_ID;
          break;
        case ERROR_19_9:
          error->error_value = PCEP_ERRV_LSP_NOT_PCE_INITIATED;
          break;
        default:
          zlog_warn ("(%s)PCE tried to REMOVE unknown error!", __func__);
...
int
pcep_main_event_update_candidate (struct path *path)
{
  int ret = 0;
...
int
pcep_main_event_remove_candidate_segments (const char *originator, bool force)
{
...
int
pathd_candidate_created_handler (struct srte_candidate *candidate)
{
...
int
pathd_candidate_updated_handler (struct srte_candidate *candidate)
{
  struct path *path = candidate_to_path (candidate);
...
int
pathd_candidate_removed_handler (struct srte_candidate *candidate)
{
  struct path *path = candidate_to_path (candidate);
 ...
 */
static int
pcep_module_config_pre (struct event_loop *tm)
{
...
static int
pcep_module_late_init (struct event_loop *tm)
{
