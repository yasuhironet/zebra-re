  ...
  RB_FOREACH (policy, srte_policy_head, &srte_policies)
    {
      ...
      if (policy->binding_sid != MPLS_LABEL_NONE)
        snprintf (binding_sid, sizeof (binding_sid), "%u",
                  policy->binding_sid);
      vty_out (
          vty, "Endpoint: %s  Color: %u  Name: %s  BSID: %s  Status: %s\n",
      ...
      RB_FOREACH (candidate, srte_candidate_head, &policy->candidate_paths)
        {
          ...
          else if (segment_list->protocol_origin == SRTE_ORIGIN_PCEP)
            segment_list_info = created_by_pce_info;
          else
            segment_list_info = candidate->lsp->segment_list->name;
          ...
          vty_out (
              ...
              candidate->type == SRTE_CANDIDATE_TYPE_EXPLICIT ? "explicit"
                                                              : "dynamic",
              segment_list_info,
              srte_origin2str (candidate->lsp->protocol_origin));
        }
...
DEFPY_NOSH (segment_routing_list,
            segment_routing_cmd,
            "segment-routing",
...
DEFPY_NOSH (sr_traffic_eng_list,
            sr_traffic_eng_cmd,
            "traffic-eng",
...
void
cli_show_srte_segment_list (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
void
cli_show_srte_segment_list_end (struct vty *vty, const struct lyd_node *dnode)
{
  vty_out (vty, "  exit\n");
}

static int
segment_list_has_src_dst (
    struct vty *vty, char *xpath, long index, const char *index_str,
    struct in_addr adj_src_ipv4, struct in_addr adj_dst_ipv4,
    struct in6_addr adj_src_ipv6, struct in6_addr adj_dst_ipv6,
    const char *adj_src_ipv4_str, const char *adj_dst_ipv4_str,
    const char *adj_src_ipv6_str, const char *adj_dst_ipv6_str)
{
  ...
  if (adj_src_ipv4_str != NULL)
    {
      nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY, "ipv4_adjacency");
      node_src_id = adj_src_ipv4_str;
    }
  else if (adj_src_ipv6_str != NULL)
    {
      nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY, "ipv6_adjacency");
  ...
  snprintf (xpath, XPATH_MAXLEN, "./segment[index='%s']/nai/remote-address",
            index_str);
    ...
    const char *has_iface_id, long iface_id, const char *iface_id_str)
{
  ...
  if (has_iface_id != NULL)
    {
      if (prefix_ipv4_str != NULL)
        {
          nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY, "ipv4_local_iface");
        }
      else if (prefix_ipv6_str != NULL)
        {
          nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY, "ipv6_local_iface");
        }
      else
        {
          return CMD_ERR_NO_MATCH;
  ...
  else
    {
      if (prefix_ipv4_str != NULL)
        {
          nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY, "ipv4_algo");
        }
      else if (prefix_ipv6_str != NULL)
        {
          nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY, "ipv6_algo");
        }
      else
        {
          return CMD_ERR_NO_MATCH;
  ...
  snprintf (xpath, XPATH_MAXLEN, "./segment[index='%s']/nai/local-prefix-len",
            index_str);
  nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY,
                         prefix_ipv4_str ? strchr (prefix_ipv4_str, '/') + 1
                                         : strchr (prefix_ipv6_str, '/') + 1);
  /* Alg / Iface */
  if (has_algo != NULL)
    {
      snprintf (xpath, XPATH_MAXLEN, "./segment[index='%s']/nai/algorithm",
                index_str);
      ...
      if (has_iface_id != NULL)
        {
          snprintf (xpath, XPATH_MAXLEN,
                    "./segment[index='%s']/nai/local-interface", index_str);
          nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY, iface_id_str);
        }
    }
  if (has_algo != NULL)
    {
      ...
      if (ted_sid == MPLS_LABEL_NONE)
        {
          zlog_err (
              ...
              __func__, &pre_ipaddr, prefix_cli.prefixlen, algo, ted_sid);
        }
    }
  if (has_iface_id != NULL)
    {
      ...
      if (ted_sid == MPLS_LABEL_NONE)
        {
          zlog_err (
              ...
              __func__, &pre_ipaddr, prefix_cli.prefixlen, iface_id, ted_sid);
        }
    }
  return CMD_SUCCESS;
}
...
/* clang-format on */
{
  ...
  if (has_adj != NULL)
    {
      status = segment_list_has_src_dst (
          vty, xpath, index, index_str, adj_src_ipv4, adj_dst_ipv4,
          adj_src_ipv6, adj_dst_ipv6, adj_src_ipv4_str, adj_dst_ipv4_str,
  ...
  else
    {
      status = segment_list_has_prefix (
          vty, xpath, index, index_str, prefix_ipv4, prefix_ipv4_str,
          prefix_ipv6, prefix_ipv6_str, has_algo, algo, algo_str, has_iface_id,
...
void
cli_show_srte_segment_list_segment (struct vty *vty,
  ...
  if (yang_dnode_exists (dnode, "./nai"))
    {
      ...
      if (yang_dnode_exists (dnode, "./nai/local-prefix-len"))
        {
          vty_out (vty, "/%s",
                   yang_dnode_get_string (dnode, "./nai/local-prefix-len"));
        }
      if (yang_dnode_exists (dnode, "./nai/local-interface"))
        {
          vty_out (vty, " iface %s",
                   yang_dnode_get_string (dnode, "./nai/local-interface"));
        }
      if (yang_dnode_exists (dnode, "./nai/algorithm"))
        {
          vty_out (vty, " algorithm %s",
                   yang_dnode_get_string (dnode, "./nai/algorithm"));
        }
    }
  vty_out (vty, "\n");
}
            ...
            "SR Policy endpoint IPv6 address\n")
{
  ...
  snprintf (xpath, sizeof (xpath),
            "/frr-pathd:pathd/srte/policy[color='%s'][endpoint='%s']", num_str,
            endpoint_str);
  nb_cli_enqueue_change (vty, xpath, NB_OP_CREATE, NULL);
...
void
cli_show_srte_policy (struct vty *vty, const struct lyd_node *dnode,
                      bool show_defaults)
{
...
void
cli_show_srte_policy_end (struct vty *vty, const struct lyd_node *dnode)
{
  vty_out (vty, "  exit\n");
}
...
void
cli_show_srte_policy_name (struct vty *vty, const struct lyd_node *dnode,
                           bool show_defaults)
{
...
void
cli_show_srte_policy_binding_sid (struct vty *vty,
                                  const struct lyd_node *dnode,
                                  bool show_defaults)
{
...
DEFPY_NOSH (srte_policy_candidate_dyn,
...
DEFPY (srte_candidate_bandwidth,
       ...
       "Required constraint\n")
{
  nb_cli_enqueue_change (vty, "./constraints/bandwidth/required", NB_OP_MODIFY,
                         required ? "true" : "false");
  nb_cli_enqueue_change (vty, "./constraints/bandwidth/value", NB_OP_MODIFY,
                         value);
  return nb_cli_apply_changes (vty, NULL);
}
...
DEFPY (srte_candidate_no_bandwidth,
       ...
       "Required constraint\n")
{
  nb_cli_enqueue_change (vty, "./constraints/bandwidth", NB_OP_DESTROY, NULL);
  return nb_cli_apply_changes (vty, NULL);
}
       ...
       "Required constraint\n")
{
  ...
  snprintf (xpath, sizeof (xpath), "./constraints/metrics[type='%s']/is-bound",
            type);
  nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY,
                         (bound != NULL) ? "true" : "false");
  snprintf (xpath, sizeof (xpath), "./constraints/metrics[type='%s']/required",
            type);
  nb_cli_enqueue_change (vty, xpath, NB_OP_MODIFY,
                         required ? "true" : "false");
  return nb_cli_apply_changes (vty, NULL);
...
{
  char xpath[XPATH_CANDIDATE_MAXLEN];
  snprintf (xpath, sizeof (xpath), "./constraints/metrics[type='%s']", type);
  nb_cli_enqueue_change (vty, xpath, NB_OP_DESTROY, NULL);
  return nb_cli_apply_changes (vty, NULL);
}
...
DEFPY_NOSH (show_debugging_pathd,
...
static const char *
metric_type_name (enum srte_candidate_metric_type type)
{
  switch (type)
    {
...
static void
config_write_float (struct vty *vty, float value)
{
...
static void
config_write_metric (struct vty *vty, enum srte_candidate_metric_type type,
...
static int
config_write_metric_cb (const struct lyd_node *dnode, void *arg)
{
  struct vty *vty = arg;
...
void
cli_show_srte_policy_candidate_path (struct vty *vty,
                                     const struct lyd_node *dnode,
                                     bool show_defaults)
{
  float bandwidth;
  ...
  if (strmatch (type, "dynamic"))
    {
      if (yang_dnode_exists (dnode, "./constraints/bandwidth"))
        {
          ...
          required =
              yang_dnode_get_bool (dnode, "./constraints/bandwidth/required");
      ...
      yang_dnode_iterate (config_write_metric_cb, vty, dnode,
                          "./constraints/metrics");
      if (yang_dnode_exists (dnode, "./constraints/objective-function"))
        {
          objfun_type = yang_dnode_get_enum (
              dnode, "./constraints/objective-function/type");
          required = yang_dnode_get_bool (
              dnode, "./constraints/objective-function/required");
...
void
cli_show_srte_policy_candidate_path_end (struct vty *vty,
                                         const struct lyd_node *dnode)
{
  const char *type = yang_dnode_get_string (dnode, "./type");
...
static int
config_write_dnode (const struct lyd_node *dnode, void *arg)
{
  struct vty *vty = arg;
...
int
config_write_segment_routing (struct vty *vty)
{
...
static int
path_policy_cli_debug_config_write (struct vty *vty)
{
...
static int
path_policy_cli_debug_set_all (uint32_t flags, bool set)
{
...
path_cli_init (void)
{
  ...
  install_element (SR_SEGMENT_LIST_NODE, &srte_segment_list_no_segment_cmd);
  ...
  install_element (SR_CANDIDATE_DYN_NODE, &srte_candidate_bandwidth_cmd);
  install_element (SR_CANDIDATE_DYN_NODE, &srte_candidate_no_bandwidth_cmd);
  install_element (SR_CANDIDATE_DYN_NODE, &srte_candidate_affinity_filter_cmd);
  install_element (SR_CANDIDATE_DYN_NODE,
                   &srte_candidate_no_affinity_filter_cmd);
  install_element (SR_CANDIDATE_DYN_NODE, &srte_candidate_metric_cmd);
  install_element (SR_CANDIDATE_DYN_NODE, &srte_candidate_no_metric_cmd);
  install_element (SR_CANDIDATE_DYN_NODE, &srte_candidate_objfun_cmd);
