 ...
 */
const void *
pathd_srte_segment_list_get_next (struct nb_cb_get_next_args *args)
{
  struct srte_segment_list *segment_list =
      (struct srte_segment_list *) args->list_entry;

  if (args->list_entry == NULL)
    segment_list = RB_MIN (srte_segment_list_head, &srte_segment_lists);
  else
...
int
pathd_srte_segment_list_get_keys (struct nb_cb_get_keys_args *args)
{
 ...
 */
const void *
pathd_srte_policy_get_next (struct nb_cb_get_next_args *args)
{
...
int
pathd_srte_policy_get_keys (struct nb_cb_get_keys_args *args)
{
  const struct srte_policy *policy = (struct srte_policy *) args->list_entry;
...
{
  struct srte_policy *policy = (struct srte_policy *) args->parent_list_entry;
  struct srte_candidate *candidate =
      (struct srte_candidate *) args->list_entry;

  if (args->list_entry == NULL)
    candidate = RB_MIN (srte_candidate_head, &policy->candidate_paths);
  else
...
int
pathd_srte_policy_candidate_path_get_keys (struct nb_cb_get_keys_args *args)
{
