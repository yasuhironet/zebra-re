...
void
path_pcep_refine_path (struct path *path)
{
...
struct path *
path_pcep_config_get_path (struct lsp_nb_key *key)
{
...
void
path_pcep_config_list_path (path_list_cb_t cb, void *arg)
{
  struct path *path;
...
struct path *
candidate_to_path (struct srte_candidate *candidate)
{
  char *name;
  struct path *path;
      ...
      if (CHECK_FLAG (srte_metric->flags, F_METRIC_IS_DEFINED))
        {
          ...
          metric->enforce =
              CHECK_FLAG (srte_metric->flags, F_METRIC_IS_REQUIRED);
          metric->is_bound =
              CHECK_FLAG (srte_metric->flags, F_METRIC_IS_BOUND);
          metric->is_computed =
              CHECK_FLAG (srte_metric->flags, F_METRIC_IS_COMPUTED);
        }
    }
  *path = (struct path){
...
void
copy_candidate_objfun_info (struct srte_candidate *candidate,
                            struct path *path)
{
  ...
  if (CHECK_FLAG (candidate->flags, F_CANDIDATE_HAS_OBJFUN))
    {
      ...
      path->enforce_pcc_objfun =
          CHECK_FLAG (candidate->flags, F_CANDIDATE_REQUIRED_OBJFUN);
    }
  else
    {
...
void
copy_candidate_affinity_filters (struct srte_candidate *candidate,
                                 struct path *path)
{
  ...
  path->affinity_filters[AFFINITY_FILTER_EXCLUDE_ANY - 1] =
      eany ? candidate->affinity_filters[AFFINITY_FILTER_EXCLUDE_ANY - 1] : 0;
  path->affinity_filters[AFFINITY_FILTER_INCLUDE_ANY - 1] =
      iany ? candidate->affinity_filters[AFFINITY_FILTER_INCLUDE_ANY - 1] : 0;
  path->affinity_filters[AFFINITY_FILTER_INCLUDE_ALL - 1] =
      iall ? candidate->affinity_filters[AFFINITY_FILTER_INCLUDE_ALL - 1] : 0;
}
  ...
  RB_FOREACH_REVERSE (segment, srte_segment_entry_head,
                      &segment_list->segments)
    {
      ...
      *hop = (struct path_hop){
        ...
        .has_nai = segment->nai_type != SRTE_SEGMENT_NAI_TYPE_NONE,
...
int
path_pcep_config_initiate_path (struct path *path)
{
  struct srte_policy *policy;
  struct srte_candidate *candidate;
  ...
  if (path->do_remove)
      ...
      if (candidate)
        {
          if (! path->is_delegated)
            {
              zlog_warn ("(%s)PCE tried to REMOVE but it's not Delegated!",
                         __func__);
              return ERROR_19_1;
            }
          if (candidate->type != SRTE_CANDIDATE_TYPE_DYNAMIC)
            {
              zlog_warn ("(%s)PCE tried to REMOVE but it's not PCE origin!",
                         __func__);
              return ERROR_19_9;
            }
          zlog_warn ("(%s)PCE tried to REMOVE found candidate!, let's remove",
                     __func__);
        ...
        {
          zlog_warn ("(%s)PCE tried to REMOVE not existing LSP!", __func__);
          return ERROR_19_3;
        }
      srte_apply_changes ();
    }
  else
    {
      ...
      if (! candidate)
        {
          policy = srte_policy_add (path->nbkey.color, &path->nbkey.endpoint,
          ...
          candidate = srte_candidate_add (policy, path->nbkey.preference,
                                          SRTE_ORIGIN_PCEP, path->originator);
          candidate->policy->srp_id = path->srp_id;
          strlcpy (candidate->name, path->name, sizeof (candidate->name));
      ...
      else
        {
          ...
          if ((path->originator != candidate->originator) ||
              (path->originator != policy->originator))
            {
              ...
              zlog_warn (
                  ...
                  path->originator, candidate->originator);
              return 1;
            }
          if ((policy->protocol_origin != SRTE_ORIGIN_PCEP) ||
              (candidate->protocol_origin != SRTE_ORIGIN_PCEP))
            {
               ...
               * another PCE, show a warning and regect the
               * initiated path */
              zlog_warn ("PCE %s tried to initiate a path created localy",
                         path->originator);
              return 1;
            }
        }
...
int
path_pcep_config_update_path (struct path *path)
{
  assert (path != NULL);
  ...
  snprintf (segment_list_name_buff, sizeof (segment_list_name_buff), "%s-%u",
            path->name, path->plsp_id);
  ...
  for (hop = path->first_hop, index = 10; hop != NULL;
       hop = hop->next, index += 10)
    {
      ...
      if (srte_segment_entry_set_nai (
              segment, srte_nai_type (hop->nai.type), &hop->nai.local_addr,
              hop->nai.local_iface, &hop->nai.remote_addr,
              hop->nai.remote_iface, 0, 0) == PATH_SID_ERROR)
  ...
  for (metric = path->first_metric; metric != NULL; metric = metric->next)
    srte_lsp_set_metric (
        candidate->lsp, (enum srte_candidate_metric_type) metric->type,
        metric->value, metric->enforce, metric->is_bound, metric->is_computed);
...
lookup_candidate (struct lsp_nb_key *key)
{
...
char *
candidate_name (struct srte_candidate *candidate)
{
