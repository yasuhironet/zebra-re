...
static int
pcep_cli_print_pce_config (struct pcep_config_group_opts *group_opts,
...
static const char PCEP_VTYSH_ARG_SESSION_TIMEOUT[] =
...
/* Used by PCEP_PCE_CONFIG_NODE sub-commands to operate on the current pce
 * group
 */
...
static struct cmd_node pcep_node = { .name = "srte pcep",
                                     .node = PCEP_NODE,
                                     .parent_node = SR_TRAFFIC_ENG_NODE,
                                     .prompt = "%s(config-sr-te-pcep)# " };

static struct cmd_node pcep_pcc_node = { .name = "srte pcep pcc",
                                         .node = PCEP_PCC_NODE,
                                         .parent_node = PCEP_NODE,
                                         .prompt =
                                             "%s(config-sr-te-pcep-pcc)# " };

static struct cmd_node pcep_pce_node = { .name = "srte pcep pce",
                                         .node = PCEP_PCE_NODE,
                                         .parent_node = PCEP_NODE,
                                         .prompt =
  ...
  if (arg_str != NULL)                                                        \
    {                                                                         \
      if (arg_val <= min_value || arg_val >= max_value)                       \
        {                                                                     \
          vty_out (vty, "%% Invalid value %ld in range [%d - %d]", arg_val,   \
                   min_value, max_value);                                     \
          return CMD_WARNING;                                                 \
        }                                                                     \
      arg_store = arg_val;                                                    \
...
#define MERGE_COMPARE_CONFIG_GROUP_VALUE(config_param, not_set_value)         \
  ...
  if (pce_opts_cli->pce_config_group_opts.config_param == not_set_value)      \
    {                                                                         \
      pce_opts_cli->pce_opts.config_opts.config_param =                       \
          ((pce_config != NULL && pce_config->config_param != not_set_value)  \
               ? pce_config->config_param                                     \
               : default_pcep_config_group_opts_g.config_param);              \
    }
  ...
  for (int i = 0; i < MAX_PCE; i++)
    {
      ...
      if (pce_rhs_cli != NULL)
        {
          if (strcmp (pce_name, pce_rhs_cli->pce_opts.pce_name) == 0)
            {
              return pce_rhs_cli;
            }
        }
 ...
 */
static bool
pcep_cli_add_pce (struct pce_opts_cli *pce_opts_cli)
{
  for (int i = 0; i < MAX_PCE; i++)
    {
...
static void
pcep_cli_delete_pce (const char *pce_name)
{
  for (int i = 0; i < MAX_PCE; i++)
    {
      if (pcep_g->pce_opts_cli[i] != NULL)
        {
          if (strcmp (pcep_g->pce_opts_cli[i]->pce_opts.pce_name, pce_name) ==
              0)
            {
  ...
  if (pce_opts_cli->pce_config_group_opts.tcp_md5_auth[0] == '\0')
    {
      if (pce_config != NULL && pce_config->tcp_md5_auth[0] != '\0')
        ...
        {
          tcp_md5_auth_str = default_pcep_config_group_opts_g.tcp_md5_auth;
        }
    }
  strlcpy (pce_opts_cli->pce_opts.config_opts.tcp_md5_auth, tcp_md5_auth_str,
           sizeof (pce_opts_cli->pce_opts.config_opts.tcp_md5_auth));

  struct ipaddr *source_ip = &pce_opts_cli->pce_config_group_opts.source_ip;
  if (pce_opts_cli->pce_config_group_opts.source_ip.ipa_type == IPADDR_NONE)
    {
      if (pce_config != NULL && pce_config->source_ip.ipa_type != IPADDR_NONE)
        {
...
/* Check if a pcep_config_group_opts already exists based on its name and
  ...
  for (int i = 0; i < MAX_PCE; i++)
    {
      ...
      if (pcep_pce_config_rhs != NULL)
        {
          if (strcmp (group_name, pcep_pce_config_rhs->name) == 0)
            {
 ...
 * false otherwise. */
static bool
pcep_cli_is_pcep_pce_config_used (const char *group_name)
{
  for (int i = 0; i < MAX_PCE; i++)
    {
      if (pcep_g->pce_opts_cli[i] != NULL)
...
static void
pcep_cli_delete_pcep_pce_config (const char *group_name)
{
  for (int i = 0; i < MAX_PCE; i++)
    {
      if (pcep_g->config_group_opts[i] != NULL)
...
static bool
pcep_cli_pcc_has_pce (const char *pce_name)
{
...
static void
pcep_cli_add_pce_connection (struct pce_opts *pce_opts)
{
  for (int i = 0; i < MAX_PCC; i++)
    {
...
static void
pcep_cli_remove_pce_connection (struct pce_opts *pce_opts)
{
  for (int i = 0; i < MAX_PCC; i++)
    {
...
static int
path_pcep_cli_debug (struct vty *vty, const char *no_str,
...
static int
path_pcep_cli_show_srte_pcep_counters (struct vty *vty)
{
  int i, j, row;
  time_t diff_time;
  struct tm tm_info;
          ...
          for (j = 0; j <= subgroup->num_counters; j++)
            {
              ...
              if (counter != NULL)
                {
                  ttable_add_row (tt, "%s|%s|%u", group_name,
...
static int
path_pcep_cli_pcep_pce_config (struct vty *vty, const char *pcep_pce_config)
{
  struct pcep_config_group_opts *pce_config =
...
static int
path_pcep_cli_pcep_pce_config_delete (struct vty *vty,
...
static int
path_pcep_cli_show_srte_pcep_pce_config (struct vty *vty,
                                         const char *pcep_pce_config)
{
  char buf[1024] = "";
  ...
  if (pcep_pce_config != NULL)
    {
      if (strcmp (pcep_pce_config, "default") == 0)
        ...
        {
          group_opts = pcep_cli_find_pcep_pce_config (pcep_pce_config);
        }
      if (group_opts == NULL)
        {
...
static int
path_pcep_cli_pce (struct vty *vty, const char *pce_peer_name)
{
...
static int
path_pcep_cli_pce_delete (struct vty *vty, const char *pce_peer_name)
{
  struct pce_opts_cli *pce_opts_cli = pcep_cli_find_pce (pce_peer_name);
  if (pce_opts_cli == NULL)
    {
...
static void
show_pce_peer (struct vty *vty, struct pce_opts_cli *pce_opts_cli)
{
...
static int
path_pcep_cli_show_srte_pcep_pce (struct vty *vty, const char *pce_peer)
{
  /* Only show 1 PCE */
  struct pce_opts_cli *pce_opts_cli;
  if (pce_peer != NULL)
...
static int
path_pcep_cli_peer_sr_draft07 (struct vty *vty)
{
...
static int
path_pcep_cli_peer_pce_initiated (struct vty *vty)
{
  struct pcep_config_group_opts *pce_config = NULL;
...
static int
path_pcep_cli_peer_tcp_md5_auth (struct vty *vty, const char *tcp_md5_auth)
{
  struct pcep_config_group_opts *pce_config = NULL;
...
static int
path_pcep_cli_peer_address (struct vty *vty, const char *ip_str,
  ...
  if (ipv6_str != NULL)
    {
      ...
      memcpy (&pce_opts->addr.ipaddr_v6, ipv6, sizeof (struct in6_addr));
    }
  else if (ip_str != NULL)
    {
...
static int
path_pcep_cli_peer_source_address (struct vty *vty, const char *ip_str,
                                   struct in_addr *ip, const char *ipv6_str,
                                   struct in6_addr *ipv6, const char *port_str,
                                   long port)
{
  ...
  if (ipv6_str != NULL)
  ...
  else if (ip_str != NULL)
    {
      ...
      memcpy (&pce_config->source_ip.ipaddr_v4, ip, sizeof (struct in_addr));
    }

  /* Handle the optional port */
  PCEP_VTYSH_INT_ARG_CHECK (port_str, port, pce_config->source_port, 0, 65535);

  return CMD_SUCCESS;
}

static int
path_pcep_cli_peer_pcep_pce_config_ref (struct vty *vty,
...
static int
path_pcep_cli_pcc (struct vty *vty)
{
...
static int
path_pcep_cli_pcc_delete (struct vty *vty)
{
...
static int
path_pcep_cli_pcc_pcc_msd (struct vty *vty, const char *msd_str, long msd,
                           bool reset)
{
  if (reset)
    pcc_msd_configured_g = false;
...
static int
path_pcep_cli_pcc_pcc_peer (struct vty *vty, const char *peer_name,
                            const char *precedence_str, long precedence)
{
  ...
  PCEP_VTYSH_INT_ARG_CHECK (precedence_str, precedence, pce_opts->precedence,
                            0, 256);
  ...
  memcpy (&pcc_opts_copy->addr, &pce_opts_cli->pce_opts.config_opts.source_ip,
...
static int
path_pcep_cli_pcc_pcc_peer_delete (struct vty *vty, const char *peer_name,
                                   const char *precedence_str, long precedence)
{
  /* Check if the pcc-peer is connected to the PCC */
  if (! pcep_cli_pcc_has_pce (peer_name))
    {
      vty_out (vty, "%% WARN: The peer [%s] is not connected to the PCC.\n",
               peer_name);
      return CMD_WARNING;
    }
...
static int
path_pcep_cli_show_srte_pcep_pcc (struct vty *vty)
{
...
static void
print_pcep_capabilities (char *buf, size_t buf_len, pcep_configuration *config)
{
...
static void
print_pcep_session (struct vty *vty, struct pce_opts *pce_opts,
                    struct pcep_pcc_info *pcc_info)
{
  char buf[1024];
  buf[0] = '\0';
      ...
      if (pcc_info->status == PCEP_PCC_SYNCHRONIZING ||
          pcc_info->status == PCEP_PCC_OPERATING)
        {
          ...
          vty_out (
              ...
              (uint32_t) (current_time - session->time_connected),
...
static int
path_pcep_cli_show_srte_pcep_session (struct vty *vty, const char *pcc_peer)
{
  struct pce_opts_cli *pce_opts_cli;
  struct pcep_pcc_info *pcc_info;
  ...
  if (pcc_peer != NULL)
    {
      ...
      if (pcc_info == NULL)
        {
          vty_out (vty, "%% Cannot retrieve PCEP session info for PCE [%s]\n",
                   pcc_peer);
          return CMD_WARNING;
        }
  ...
  for (int i = 0; i < MAX_PCC; i++)
    {
      ...
      num_pcep_sessions_conn += pcc_info->status == PCEP_PCC_OPERATING ? 1 : 0;
...
static int
path_pcep_cli_clear_srte_pcep_session (struct vty *vty, const char *pcc_peer)
{
  struct pce_opts_cli *pce_opts_cli;
...
int
pcep_cli_debug_config_write (struct vty *vty)
{
  char buff[128] = "";
...
int
pcep_cli_debug_set_all (uint32_t flags, bool set)
{
...
int
pcep_cli_pcep_config_write (struct vty *vty)
{
  vty_out (vty, "  pcep\n");
...
int
pcep_cli_pcc_config_write (struct vty *vty)
{
  struct pce_opts *pce_opts;
  char buf[128] = "";
  int lines = 0;
  ...
  for (int i = 0; i < MAX_PCC; i++)
    {
      ...
      if (pce_opts->precedence > 0 &&
          pce_opts->precedence != DEFAULT_PCE_PRECEDENCE)
        {
          csnprintfrr (buf, sizeof (buf), " %s %d", PCEP_VTYSH_ARG_PRECEDENCE,
                       pce_opts->precedence);
        }
      vty_out (vty, "%s\n", buf);
      lines++;
                           ...
                           char *buf, size_t buf_len)
{
  ...
  if ((group_opts->keep_alive_seconds > 0) ||
      ...
      (group_opts->max_keep_alive_seconds > 0))
    {
      ...
      if (group_opts->keep_alive_seconds > 0)
        {
          csnprintfrr (buf, buf_len, " %s %d", PCEP_VTYSH_ARG_KEEP_ALIVE,
                       group_opts->keep_alive_seconds);
        }
      if (group_opts->min_keep_alive_seconds > 0)
        {
          csnprintfrr (buf, buf_len, " %s %d", PCEP_VTYSH_ARG_KEEP_ALIVE_MIN,
                       group_opts->min_keep_alive_seconds);
        }
      if (group_opts->max_keep_alive_seconds > 0)
        {
          csnprintfrr (buf, buf_len, " %s %d", PCEP_VTYSH_ARG_KEEP_ALIVE_MAX,
  ...
  if ((group_opts->dead_timer_seconds > 0) ||
      ...
      (group_opts->max_dead_timer_seconds > 0))
    {
      ...
      if (group_opts->dead_timer_seconds > 0)
        {
          csnprintfrr (buf, buf_len, " %s %d", PCEP_VTYSH_ARG_DEAD_TIMER,
                       group_opts->dead_timer_seconds);
        }
      if (group_opts->min_dead_timer_seconds > 0)
        {
          csnprintfrr (buf, buf_len, " %s %d", PCEP_VTYSH_ARG_DEAD_TIMER_MIN,
                       group_opts->min_dead_timer_seconds);
        }
      if (group_opts->max_dead_timer_seconds > 0)
        {
          csnprintfrr (buf, buf_len, " %s %d", PCEP_VTYSH_ARG_DEAD_TIMER_MAX,
...
int
pcep_cli_pce_config_write (struct vty *vty)
{
  int lines = 0;
  char buf[1024] = "";
  ...
  for (int i = 0; i < MAX_PCE; i++)
    {
      ...
      if (pce_opts->port != PCEP_DEFAULT_PORT)
        {
          vty_out (vty, "    %s %d", PCEP_VTYSH_ARG_PORT, pce_opts->port);
        }
      vty_out (vty, "%s\n", buf);
      lines += 2;
      ...
      lines += pcep_cli_print_pce_config (&pce_opts_cli->pce_config_group_opts,
                                          buf, sizeof (buf));
...
int
pcep_cli_pcep_pce_config_write (struct vty *vty)
{
  int lines = 0;
  char buf[1024] = "";
  ...
  for (int i = 0; i < MAX_PCE; i++)
    {
      ...
      lines += pcep_cli_print_pce_config (group_opts, buf, sizeof (buf));
      vty_out (vty, "%s", buf);
      buf[0] = 0;
...
DEFPY_NOSH (pcep_cli_pcep_pce_config,
...
{
  return path_pcep_cli_peer_source_address (vty, ip_str, &ip, ipv6_str, &ipv6,
                                            port_str, port);
}
       ...
       "Multi-PCE delegation timeout value in seconds\n")
{
  return path_pcep_cli_peer_timers (
      ...
      min_peer_keep_alive, max_peer_keep_alive_str, max_peer_keep_alive,
      dead_timer_str, dead_timer, min_peer_dead_timer_str, min_peer_dead_timer,
      max_peer_dead_timer_str, max_peer_dead_timer, pcep_request_str,
      pcep_request, session_timeout_interval_str, session_timeout_interval,
      delegation_timeout_str, delegation_timeout);
}

DEFPY_NOSH (pcep_cli_pcc,
            pcep_cli_pcc_cmd,
            "pcc",
            "PCC configuration\n")
{
       ...
       "PCE precedence\n")
{
  if (no != NULL)
    {
      return path_pcep_cli_pcc_pcc_peer_delete (vty, peer, precedence_str,
                                                precedence);
    }
  else
    {
...
pcep_cli_init (void)
{
  ...
  hook_register (nb_client_debug_config_write, pcep_cli_debug_config_write);
  ...
  install_element (PCEP_PCE_CONFIG_NODE, &pcep_cli_peer_source_address_cmd);
