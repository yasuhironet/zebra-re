 ...
 */
bool
get_ipv4_router_id (struct in_addr *router_id)
{
  bool retval = false;
 ...
 */
bool
get_ipv6_router_id (struct in6_addr *router_id)
{
  bool retval = false;
  assert (router_id != NULL);
...
static void
path_zebra_connected (struct zclient *zclient)
{
...
static int
path_zebra_sr_policy_notify_status (ZAPI_CALLBACK_ARGS)
{
  ...
  policy = srte_policy_find (zapi_sr_policy.color, &zapi_sr_policy.endpoint);
  if (! policy)
    return -1;
...
static int
path_zebra_router_id_update (ZAPI_CALLBACK_ARGS)
{
  struct prefix pref;
  const char *family;
  ...
  if (pref.family == AF_INET)
    {
      pthread_mutex_lock (&g_router_id_v4_mtx);
      memcpy (&g_router_id_v4, &pref.u.prefix4, sizeof (g_router_id_v4));
    ...
    {
      pthread_mutex_lock (&g_router_id_v6_mtx);
      memcpy (&g_router_id_v6, &pref.u.prefix6, sizeof (g_router_id_v6));
    ...
    {
      zlog_warn ("Unexpected router ID address family for vrf %u: %u", vrf_id,
                 pref.family);
      return 0;
    }
 ...
 */
void
path_zebra_add_sr_policy (struct srte_policy *policy,
                          struct srte_segment_list *segment_list)
{
 ...
 */
void
path_zebra_delete_sr_policy (struct srte_policy *policy)
{
  struct zapi_sr_policy zp = {};
 ...
 */
int
path_zebra_request_label (mpls_label_t label)
{
  int ret;
  uint32_t start, end;
 ...
 */
void
path_zebra_release_label (mpls_label_t label)
{
  int ret;
...
static void
path_zebra_label_manager_connect (void)
{
  /* Connect to label manager. */
  while (zclient_socket_connect (zclient_sync) < 0)
    {
      zlog_warn ("%s: error connecting synchronous zclient!", __func__);
      sleep (1);
    }
  ...
  while (zclient_send_hello (zclient_sync) < 0)
    {
      zlog_warn ("%s: Error sending hello for synchronous zclient!", __func__);
      sleep (1);
    }
...
static int
path_zebra_opaque_msg_handler (ZAPI_CALLBACK_ARGS)
{
  int ret = 0;
  struct stream *s;
  ...
  switch (info.type)
    ...
    case LINK_STATE_SYNC:
      ...
      if (msg)
        {
          zlog_debug ("%s: [rcv ted] ls (%s) msg (%s)-(%s) !", __func__,
                      info.type == LINK_STATE_UPDATE ? "LINK_STATE_UPDATE"
                                                     : "LINK_STATE_SYNC",
        ...
        {
          zlog_err ("%s: [rcv ted] Could not parse LinkState stream message.",
                    __func__);
          return -1;
        }
      ...
      break;
    default:
      zlog_debug ("%s: [rcv ted] unknown opaque event (%d) !", __func__,
                  info.type);
      break;
    }
 ...
 */
void
path_zebra_init (struct event_loop *master)
{
