...
static int
vty_close_pager (struct vty *vty)
{
  if (! vty->is_paged)
    return 0;
...
static void
vtysh_pager_envdef (bool fallback)
{
  char *pager_defined;
...
static int
vtysh_client_lookup (const char *name)
{
  int idx = -1;
...
static ssize_t
vtysh_client_receive (struct vtysh_client *vclient, char *buf, size_t bufsz,
                      int *pass_fd)
{
  struct iovec iov[1] = {
    {
 ...
 */
static int
vtysh_client_run (struct vtysh_client *vclient, const char *line,
      ...
      if (! callback)
        {
           ...
           */
          assert (
              ((buf == bufvalid) || (bufvalid - buf <= 4 && buf[0] == 0x00)));
        }
...
/* Execute by name */
static int
vtysh_client_execute_name (const char *name, const char *line)
{
  int ret = CMD_SUCCESS;
  int idx_client = -1;
...
vtysh_client_config (struct vtysh_client *head_client, char *line)
{
  ...
  vty->of_saved = vty->of;
  vty->of = NULL;
  vtysh_client_run_all (head_client, line, 1, vtysh_config_parse_line, NULL);
  vty->of = vty->of_saved;
}
            ...
            if (cmd->daemon & vtysh_client[i].flag)
              {
                if (vtysh_client[i].fd < 0 &&
                  ...
                  {
                    for (vc = &vtysh_client[i]; vc; vc = vc->next)
        ...
        case CMD_SUCCESS_DAEMON:
          {
            ...
            if (strmatch (vty_buf_trimmed, "exit-vrf"))
              vty_out (vty, "end\n");
            cmd_stat = vtysh_client_execute (&vtysh_client[0], vty->buf);
            if (cmd_stat != CMD_SUCCESS)
              break;
  ...
  while (fgets (vty->buf, VTY_BUFSIZ, fp))
    {
      ...
      if (strmatch (vty_buf_trimmed, "exit") && vty->node == CONFIG_NODE)
        {
          fprintf (stderr, "line %d: Warning[%d]...: %s\n", lineno, vty->node,
                   "early exit from config file");
          break;
        }
      ...
      switch (ret)
        ...
        case CMD_ERR_AMBIGUOUS:
          ...
          break;
        case CMD_ERR_NO_MATCH:
          fprintf (stderr, "line %d: %% Unknown command[%d]: %s", lineno,
                   vty->node, vty->buf);
            ...
            for (i = 0; i < array_size (vtysh_client); i++)
              {
                if (cmd->daemon & vtysh_client[i].flag)
                  {
                    cmd_stat =
                        vtysh_client_execute (&vtysh_client[i], vty->buf);
                    /*
 ...
 */
static int
vtysh_process_questionmark (const char *input, int input_len)
{
  int ret, width = 0;
  unsigned int i;
...
/*
 * Entry point for user commands terminated with '?' character and typed
 * through the usual vtysh's stdin interface. This is the function being
 * registered with readline() api's.
 */
static int
vtysh_rl_describe (int a, int b)
{
  int ret;
 ...
 */
int
vtysh_execute_command_questionmark (char *input)
{
  ...
  if (! state)
    {
      ...
      if (rl_end && isspace ((unsigned char) rl_line_buffer[rl_end - 1]))
        vector_set (vline, NULL);
...
DEFUNSH (VTYSH_REALLYALL,
         vtysh_config_terminal_file_lock,
...
DEFUNSH (VTYSH_ISISD,
         vtysh_exit_isis_flex_algo,
         vtysh_exit_isis_flex_algo_cmd,
         "exit",
         "Exit current mode and down to previous mode\n")
{
  return vtysh_exit (vty);
...
DEFUNSH (VTYSH_ISISD,
         vtysh_quit_isis_flex_algo,
         vtysh_quit_isis_flex_algo_cmd,
         "quit",
         "Exit current mode and down to previous mode\n")
{
  return vtysh_exit_isisd (self, vty, argc, argv);
...
DEFSH (VTYSH_NH_GROUP, vtysh_no_nexthop_group_cmd, "no nexthop-group NHGNAME",
       NO_STR "Nexthop Group Configuration\n"
              "Name of the Nexthop Group\n")
...
static char *
do_prepend (struct vty *vty, struct cmd_token **argv, int argc)
{
...
static int
show_per_daemon (struct vty *vty, struct cmd_token **argv, int argc,
                 const char *headline)
{
  unsigned int i;
  int ret = CMD_SUCCESS;
...
static int
show_one_daemon (struct vty *vty, struct cmd_token **argv, int argc,
                 const char *name)
{
  int ret;
  char *line = do_prepend (vty, argv, argc);
...
{
  return show_per_daemon (vty, argv, argc, "Work queue statistics for %s:\n");
}
...
DEFUN (show_yang_module,
       show_yang_module_cmd,
       "show yang module [module-translator WORD] "
       DAEMONS_LIST,
       SHOW_STR
       "YANG information\n"
       "Show loaded modules\n"
...
DEFUN (show_yang_module_detail,
       show_yang_module_detail_cmd,
       "show yang module\
                 [module-translator WORD]\
                 WORD <compiled|summary|tree|yang|yin> "
       DAEMONS_LIST,
       SHOW_STR
       "YANG information\n"
       "Show loaded modules\n"
       ...
       "Add backtrace to log when message is printed\n")
{
  ...
  if (! strcmp (argv[0]->text, "no"))
    {
      uid = argv[3]->arg;
      snprintfrr (line, sizeof (line), "no debug unique-id %s backtrace", uid);
    }
  else
    {
      uid = argv[2]->arg;
      snprintfrr (line, sizeof (line), "debug unique-id %s backtrace", uid);
    }
...
static void
show_route_map_send (const char *route_map, bool json)
{
  unsigned int i;
  bool first = true;
...
vtysh_write_config_integrated (void)
{
  ...
  if (fp == NULL)
    {
      vty_out (vty, "%% Error: failed to open configuration file %s: %s\n",
  ...
  if (want_config_integrated ())
    {
      ...
      if (i < array_size (vtysh_client) && vtysh_client[i].fd != -1)
        {
          used_watchfrr = true;
          ret = vtysh_client_execute (&vtysh_client[i], "do write integrated");
        }
...
static void
vtysh_log_print (struct vtysh_client *vclient, struct zlog_live_hdr *hdr,
                 const char *text)
{
  ...
  if (! stderr_tty)
    {
      ...
      fprintf (stderr, "%s.%03u [%s] %s: %.*s\n", ts_buf,
               hdr->ts_nsec / 1000000U, label, vclient->name, (int) textlen,
               text);
      return;
    }
  ...
  fprintf (
      ...
      vis->c256_background, ts_buf, hdr->ts_nsec / 1000000U, vis->label,
      vclient->name);
  ...
  for (size_t fmtpos = 0; fmtpos < hdr->n_argpos; fmtpos++)
    {
      ...
      while (fmt->off_end > fmt->off_start && text[fmt->off_end - 1] == ' ')
        fmt->off_end--;

      fprintf (stderr, "%.*s\e[38;5;%dm%.*s\e[38;5;251m",
               (int) (fmt->off_start - textpos), text + textpos,
               vis->c256_formatarg, (int) (fmt->off_end - fmt->off_start),
               text + fmt->off_start);
      textpos = fmt->off_end;
    }
...
static void
vtysh_log_read (struct event *thread)
{
      ...
      for (; vclient; vclient = vclient->next)
        {
          ...
          if (ret != CMD_SUCCESS)
            {
              vty_out (vty, "%% failed to disable logs on %s\n",
                       vclient->name);
              ret_all = CMD_WARNING;
            }
          else
...
vtysh_connect (struct vtysh_client *vclient)
{
  ...
  if (! vclient->path[0])
    snprintf (vclient->path, sizeof (vclient->path), "%s/%s.vty", vtydir,
              vclient->name);
  path = vclient->path;
...
static int
vtysh_reconnect (struct vtysh_client *vclient)
{
  int ret;
...
static void
vtysh_update_all_instances (struct vtysh_client *head_client)
{
  struct vtysh_client *client;
  DIR *dir;
  struct dirent *file;
          ...
          if (frrstr_startswith (file->d_name, "ospfd-") &&
              ends_with (file->d_name, ".vty"))
            {
              if (n == MAXIMUM_INSTANCES)
                {
                  fprintf (stderr, "Parsing %s, client limit(%d) reached!\n",
                           vtydir, n);
                  break;
                }
              ...
              snprintf (client->path, sizeof (client->path), "%s/%s", vtydir,
                        file->d_name);
              client->next = NULL;
...
static void
vtysh_ac_line (void *arg, const char *line)
{
  vector comps = arg;
  size_t i;
...
static void
vtysh_autocomplete (vector comps, struct cmd_token *token)
{
  char accmd[256];
  size_t i;
  ...
  vty->of = NULL;
  for (i = 0; i < array_size (vtysh_client); i++)
    vtysh_client_run_all (&vtysh_client[i], accmd, 1, vtysh_ac_line, comps);
  vty->of = vty->of_saved;
}
