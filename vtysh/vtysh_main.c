...
static void
vtysh_rl_callback (char *line_read)
{
  HIST_ENTRY *last;
...
static void
vtysh_rl_read (struct event *thread)
{
...
main (int argc, char **argv, char **env)
{
  ...
  while (1)
    {
      opt = getopt_long (argc, argv, "be:c:d:nf:H:mEhCwN:ut", longopts, 0);
  ...
  snprintf (frr_config, sizeof (frr_config), "%s%s%s", sysconfdir, pathspace,
            FRR_CONFIG_NAME);
