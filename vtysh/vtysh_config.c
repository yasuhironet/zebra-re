...
static int
config_cmp (const struct config *c1, const struct config *c2)
{
  return strcmp (c1->name, c2->name);
}

static uint32_t
config_hash (const struct config *c)
{
...
static struct config *
config_get_vec (vector vec, int index, const char *line)
{
  ...
  if (! configuration)
    {
      configuration =
          XMALLOC (MTYPE_VTYSH_CONFIG, sizeof (struct configuration));
...
static struct config *
config_get_nested (struct config *parent, int index, const char *line)
{
  struct config *config;
 ...
 */
static void
config_add_line_uniq_end (struct list *config, const char *line)
{
...
static void
config_add_line_head (struct list *config, const char *line)
{
...
vtysh_config_parse_line (void *arg, const char *line)
{
  ...
  switch (c)
      ...
      if (config)
        {
          if (config->index == KEYCHAIN_NODE &&
            ...
            {
              config = config_get_nested (config, KEYCHAIN_KEY_NODE, line);
            }
          ...
          else if (strncmp (line, " link-params", strlen (" link-params")) ==
                   0)
            {
                   ...
                   strncmp (line, " exit-link-params", strlen (" exit")) == 0)
            {
              ...
              config->index = INTERFACE_NODE;
                   ...
                   ! strncmp (line, " no vrrp", strlen (" no vrrp")))
            {
              config_add_line (config->line, line);
            }
            ...
            {
              if (strncmp (line, " resilient", strlen (" resilient")) == 0)
    ...
    default:
      if (strncmp (line, "exit", strlen ("exit")) == 0)
          ...
          if (config)
            {
              if (config->exit)
                XFREE (MTYPE_VTYSH_CONFIG_LINE, config->exit);
              config->exit = XSTRDUP (MTYPE_VTYSH_CONFIG_LINE, line);
            }
        }
      ...
      else if (strncmp (line, "nexthop-group", strlen ("nexthop-group")) == 0)
      ...
      else if (strncmp (line, "affinity-map", strlen ("affinity-map")) == 0)
        config = config_get (AFFMAP_NODE, line);
      else if (strncmp (line, "route-map", strlen ("route-map")) == 0)
        config = config_get (RMAP_NODE, line);
      else if (strncmp (line, "no route-map", strlen ("no route-map")) == 0)
        config = config_get (RMAP_NODE, line);
      ...
      else if (strncmp (line, "mac access-list", strlen ("mac access-list")) ==
               0)
        config = config_get (ACCESS_MAC_NODE, line);
      else if (strncmp (line, "ip prefix-list", strlen ("ip prefix-list")) ==
               0)
        ...
        config = config_get (VTY_NODE, line);
               ...
               (strncmp (line, "ip forwarding", strlen ("ip forwarding")) ==
                0))
      ...
      else if (strncmp (line, "debug route-map", strlen ("debug route-map")) ==
               0)
        config = config_get (RMAP_DEBUG_NODE, line);
      else if (strncmp (line, "debug resolver", strlen ("debug resolver")) ==
               0)
        config = config_get (RESOLVER_DEBUG_NODE, line);
      else if (strncmp (line, "debug", strlen ("debug")) == 0)
        config = config_get (DEBUG_NODE, line);
      else if (strncmp (line, "password", strlen ("password")) == 0 ||
               strncmp (line, "enable password", strlen ("enable password")) ==
                   0)
      ...
      else if (strncmp (line, "segment-routing", strlen ("segment-routing")) ==
               0)
      ...
      else
        {
          if (strncmp (line, "log", strlen ("log")) == 0 ||
              ...
              strncmp (line, "no service ", strlen ("no service ")) == 0)
...
static void
configvec_dump (vector vec, bool nested)
{
  ...
  for (i = 0; i < vector_active (vec); i++)
    if ((configuration = vector_slot (vec, i)) != NULL)
      {
        while ((config = config_master_pop (&configuration->master)))
          {
            config_master_hash_del (&configuration->hash_master, config);
...
static int
vtysh_read_file (FILE *confp, bool dry_run)
{
  struct vty *vty;
  int ret;
 ...
 */
static int
vtysh_read_config (const char *config_file_path, bool dry_run)
{
  FILE *confp = NULL;
  bool save;
  int ret;
...
int
vtysh_apply_config (const char *config_file_path, bool dry_run, bool do_fork)
{
  /*
      ...
      for (unsigned int i = 0; i < array_size (vtysh_client); i++)
        {
          ...
          strlcpy (my_client, vtysh_client[i].name, sizeof (my_client));
      ...
      for (unsigned int i = 0; i < array_size (vtysh_client); i++)
        {
          ...
          else if (vtysh_client[i].fd == -1 && vtysh_client[i].next == NULL)
            {
              /*
