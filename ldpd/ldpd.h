...
#define NBR_STA_SESSION                                                       \
  (NBR_STA_INITIAL | NBR_STA_OPENREC | NBR_STA_OPENSENT | NBR_STA_OPER)
...
static const char *const log_procnames[] = { "parent", "ldpe", "lde" };
...
void ldp_applymask (int, union ldpd_addr *, const union ldpd_addr *, int);
int ldp_addrcmp (int, const union ldpd_addr *, const union ldpd_addr *);
int ldp_addrisset (int, const union ldpd_addr *);
int ldp_prefixcmp (int, const union ldpd_addr *, const union ldpd_addr *,
                   uint8_t);
...
void addr2sa (int af, const union ldpd_addr *, uint16_t, union sockunion *su);
...
int imsg_compose_event (struct imsgev *, uint16_t, uint32_t, pid_t, int,
                        void *, uint16_t);
...
int ldp_acl_request (struct imsgev *, char *, int, union ldpd_addr *, uint8_t);
...
struct iface *iface_new_api (struct ldpd_conf *conf, const char *name);
void iface_del_api (struct ldpd_conf *conf, struct iface *iface);
...
struct nbr_params *nbrp_new_api (struct ldpd_conf *conf,
                                 struct in_addr lsr_id);
void nbrp_del_api (struct ldpd_conf *conf, struct nbr_params *nbrp);
struct l2vpn *l2vpn_new_api (struct ldpd_conf *conf, const char *name);
void l2vpn_del_api (struct ldpd_conf *conf, struct l2vpn *l2vpn);
struct l2vpn_if *l2vpn_if_new_api (struct ldpd_conf *conf, struct l2vpn *l2vpn,
                                   const char *ifname);
void l2vpn_if_del_api (struct l2vpn *l2vpn, struct l2vpn_if *lif);
struct l2vpn_pw *l2vpn_pw_new_api (struct ldpd_conf *conf, struct l2vpn *l2vpn,
                                   const char *ifname);
void l2vpn_pw_del_api (struct l2vpn *l2vpn, struct l2vpn_pw *pw);
...
const char *log_in6addr_scope (const struct in6_addr *addr, ifindex_t ifidx);
