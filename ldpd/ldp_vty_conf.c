  ...
  RB_FOREACH (iface, iface_head, &ldpd_conf->iface_tree)
    {
      ...
      if (ia->hello_holdtime != LINK_DFLT_HOLDTIME && ia->hello_holdtime != 0)
  ...
  if (af_conf->flags & F_LDPD_AF_THELLO_ACCEPT)
    {
      ...
      if (af_conf->acl_thello_accept_from[0] != '\0')
        vty_out (vty, " from %s", af_conf->acl_thello_accept_from);
      vty_out (vty, "\n");
    }
  ...
  if ((af_conf->flags & F_LDPD_AF_ALLOCHOSTONLY) ||
      af_conf->acl_label_allocate_for[0] != '\0')
    {
      ...
      if (af_conf->flags & F_LDPD_AF_ALLOCHOSTONLY)
        ...
        vty_out (vty, " for %s", af_conf->acl_label_allocate_for);
      vty_out (vty, "\n");
    }
  ...
  if (af_conf->acl_label_advertise_for[0] != '\0' ||
      af_conf->acl_label_advertise_to[0] != '\0')
    {
      ...
      if (af_conf->acl_label_advertise_to[0] != '\0')
        vty_out (vty, " to %s", af_conf->acl_label_advertise_to);
      if (af_conf->acl_label_advertise_for[0] != '\0')
        vty_out (vty, " for %s", af_conf->acl_label_advertise_for);
      vty_out (vty, "\n");
    }
  ...
  if (af_conf->flags & F_LDPD_AF_EXPNULL)
    {
      ...
      if (af_conf->acl_label_expnull_for[0] != '\0')
        vty_out (vty, " for %s", af_conf->acl_label_expnull_for);
      vty_out (vty, "\n");
    }
  ...
  if (af_conf->acl_label_accept_for[0] != '\0' ||
      af_conf->acl_label_accept_from[0] != '\0')
    {
      ...
      if (af_conf->acl_label_accept_from[0] != '\0')
        vty_out (vty, " from %s", af_conf->acl_label_accept_from);
      if (af_conf->acl_label_accept_for[0] != '\0')
        vty_out (vty, " for %s", af_conf->acl_label_accept_for);
      vty_out (vty, "\n");
    }
...
ldp_config_write (struct vty *vty)
{
  ...
  if (ldpd_conf->trans_pref == DUAL_STACK_LDPOV4)
    vty_out (vty, " dual-stack transport-connection prefer ipv4\n");
...
int
ldp_vty_address_family (struct vty *vty, const char *negate,
                        const char *af_str)
{
  struct ldpd_af_conf *af_conf;
  int af;
      ...
      if (negate)
        {
          switch (hello_type)
            ...
            case HELLO_LINK:
              ...
              break;
            case HELLO_TARGETED:
              vty_conf->thello_holdtime = TARGETED_DFLT_HOLDTIME;
              break;
            }
        }
      else
      ...
      if (negate)
        {
          switch (hello_type)
            {
            case HELLO_LINK:
              vty_conf->lhello_interval = DEFAULT_HELLO_INTERVAL;
              break;
            case HELLO_TARGETED:
              vty_conf->thello_interval = DEFAULT_HELLO_INTERVAL;
              break;
            }
        }
      else
...
int
ldp_vty_neighbor_targeted (struct vty *vty, const char *negate,
                           const char *addr_str)
{
  int af;
...
int
ldp_vty_label_advertise (struct vty *vty, const char *negate,
                         const char *acl_to_str, const char *acl_for_str)
{
  struct ldpd_af_conf *af_conf;
  int af;
...
int
ldp_vty_label_allocate (struct vty *vty, const char *negate,
                        const char *host_routes, const char *acl_for_str)
{
  struct ldpd_af_conf *af_conf;
  int af;
...
int
ldp_vty_label_expnull (struct vty *vty, const char *negate,
...
int
ldp_vty_label_accept (struct vty *vty, const char *negate,
                      const char *acl_from_str, const char *acl_for_str)
{
  struct ldpd_af_conf *af_conf;
  int af;
...
int
ldp_vty_wait_for_sync_interval (struct vty *vty, const char *negate, long secs)
{
  switch (vty->node)
    {
    case LDP_NODE:
...
int
ldp_vty_neighbor_password (struct vty *vty, const char *negate,
  ...
  if (negate)
  ...
  else
    {
      ...
      if (password_len >= sizeof (nbrp->auth.md5key))
        vty_out (vty, "%% password has been truncated to %zu characters.",
...
int
ldp_vty_l2vpn_pwtype (struct vty *vty, const char *negate,
                      const char *type_str)
{
  VTY_DECLVAR_CONTEXT (l2vpn, l2vpn);
  int pw_type;
...
int
ldp_vty_l2vpn_interface (struct vty *vty, const char *negate,
                         const char *ifname)
{
  VTY_DECLVAR_CONTEXT (l2vpn, l2vpn);
  struct l2vpn_if *lif;
...
int
ldp_vty_l2vpn_pseudowire (struct vty *vty, const char *negate,
                          const char *ifname)
{
  VTY_DECLVAR_CONTEXT (l2vpn, l2vpn);
  struct l2vpn_pw *pw;
...
int
ldp_vty_l2vpn_pw_cword (struct vty *vty, const char *negate,
...
int
ldp_vty_l2vpn_pw_nbr_addr (struct vty *vty, const char *negate,
                           const char *addr_str)
{
  VTY_DECLVAR_CONTEXT_SUB (l2vpn_pw, pw);
  int af;
  ...
  if (ldp_get_address (addr_str, &af, &addr) == -1 || bad_addr (af, &addr))
    {
      vty_out (vty, "%% Malformed address\n");
      return (CMD_WARNING_CONFIG_FAILED);
    }
...
int
ldp_vty_l2vpn_pw_nbr_id (struct vty *vty, const char *negate,
