...
if_update_af (struct iface_af *ia)
{
  ...
  if (ia->state == IF_STA_DOWN)
    ...
    {
      if (ia->enabled && ia->iface->operative && addr_ok && socket_ok &&
          rtr_id_ok)
        return;
...
/* ARGSUSED */
static void
if_hello_timer (struct event *thread)
{
...
if_to_ctl (struct iface_af *ia)
{
  ...
  gettimeofday (&now, NULL);
  if (ia->state != IF_STA_DOWN && ia->uptime != 0)
    {
...
if_join_ipv4_group (struct iface *iface, struct in_addr *addr)
{
  ...
  if (setsockopt_ipv4_multicast (global.ipv4.ldp_disc_socket,
                                 IP_ADD_MEMBERSHIP, if_addr, addr->s_addr,
                                 iface->ifindex) < 0)
    {
...
if_leave_ipv4_group (struct iface *iface, struct in_addr *addr)
{
  ...
  if (setsockopt_ipv4_multicast (global.ipv4.ldp_disc_socket,
                                 IP_DROP_MEMBERSHIP, if_addr, addr->s_addr,
                                 iface->ifindex) < 0)
    {
...
if_join_ipv6_group (struct iface *iface, struct in6_addr *addr)
{
  ...
  if (setsockopt (global.ipv6.ldp_disc_socket, IPPROTO_IPV6, IPV6_JOIN_GROUP,
                  &mreq, sizeof (mreq)) < 0)
    {
      log_warn ("%s: error IPV6_JOIN_GROUP, interface %s address %s", __func__,
...
if_leave_ipv6_group (struct iface *iface, struct in6_addr *addr)
{
  ...
  if (setsockopt (global.ipv6.ldp_disc_socket, IPPROTO_IPV6, IPV6_LEAVE_GROUP,
...
const struct
{
  ...
  { LDP_SYNC_STA_NOT_ACH, LDP_SYNC_EVT_LDP_SYNC_START,
    LDP_SYNC_ACT_LDP_START_SYNC, 0 },
  { LDP_SYNC_STA_NOT_ACH, LDP_SYNC_EVT_LDP_SYNC_COMPLETE,
    LDP_SYNC_ACT_LDP_COMPLETE_SYNC, LDP_SYNC_STA_ACH },
  { LDP_SYNC_STA_NOT_ACH, LDP_SYNC_EVT_CONFIG_LDP_OFF,
    LDP_SYNC_ACT_CONFIG_LDP_OFF, 0 },
  { LDP_SYNC_STA_NOT_ACH, LDP_SYNC_EVT_IFACE_SHUTDOWN,
    LDP_SYNC_ACT_IFACE_SHUTDOWN, 0 },
  { LDP_SYNC_STA_NOT_ACH, LDP_SYNC_EVT_SESSION_CLOSE, LDP_SYNC_ACT_NOTHING,
    0 },
  ...
  { LDP_SYNC_STA_ACH, LDP_SYNC_EVT_CONFIG_LDP_OFF, LDP_SYNC_ACT_CONFIG_LDP_OFF,
    LDP_SYNC_STA_NOT_ACH },
  { LDP_SYNC_STA_ACH, LDP_SYNC_EVT_LDP_SYNC_COMPLETE, LDP_SYNC_ACT_NOTHING,
    0 },
  { LDP_SYNC_STA_ACH, LDP_SYNC_EVT_LDP_SYNC_START, LDP_SYNC_ACT_NOTHING, 0 },
  { LDP_SYNC_STA_ACH, LDP_SYNC_EVT_IFACE_SHUTDOWN, LDP_SYNC_ACT_IFACE_SHUTDOWN,
    LDP_SYNC_STA_NOT_ACH },
  { LDP_SYNC_STA_ACH, LDP_SYNC_EVT_SESSION_CLOSE,
    LDP_SYNC_ACT_IFACE_START_SYNC, LDP_SYNC_STA_NOT_ACH },
  { LDP_SYNC_STA_ACH, LDP_SYNC_EVT_ADJ_DEL, LDP_SYNC_ACT_IFACE_START_SYNC,
    LDP_SYNC_STA_NOT_ACH },
...
const char *const ldp_sync_action_names[] = { "NOTHING",
                                              "IFACE SYNC START",
                                              "LDP START SYNC",
                                              "LDP COMPLETE SYNC",
                                              "CONFIG LDP OFF",
...
{
  debug_evt_ldp_sync ("%s: interface %s (%d), sync_start=%d", __func__, name,
                      ifindex, sync_start);
...
static void
iface_wait_for_ldp_sync_timer (struct event *thread)
{
...
static void
start_wait_for_ldp_sync_timer (struct iface *iface)
{
...
static void
stop_wait_for_ldp_sync_timer (struct iface *iface)
{
  EVENT_OFF (iface->ldp_sync.wait_for_sync_timer);
}
...
ldp_sync_fsm_adj_event (struct adj *adj, enum ldp_sync_event event)
{
  ...
  debug_evt_ldp_sync ("%s: event %s, "
                      ...
                      adj->source.link.ia->iface->ifindex, &adj->lsr_id,
  ...
  RB_FOREACH (adj, nbr_adj_head, &nbr->adj_tree)
    {
      ...
      debug_evt_ldp_sync ("%s: event %s, iface %s, lsr-id %pI4", __func__,
                          ldp_sync_event_names[event], iface->name, &nbr->id);
...
{
  debug_evt_ldp_sync ("%s: interface %s (%d) proto %s", __func__,
                      state_req->name, state_req->ifindex,
  ...
  return send_ldp_sync_state_update (
      state_req->name, state_req->ifindex,
  ...
  if (ldp_sync_fsm_tbl[i].state == -1)
    {
      ...
      log_warnx ("%s: interface %s, event %s not expected in "
                 "state %s ",
                 __func__, iface->name, ldp_sync_event_names[event],
                 ldp_sync_state_name (old_state));
      return (0);
    }
