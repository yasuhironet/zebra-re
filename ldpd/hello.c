...
send_hello (enum hello_type type, struct iface_af *ia, struct tnbr *tnbr)
{
  ...
  switch (type)
    ...
    case HELLO_TARGETED:
      ...
      if (CHECK_FLAG (tnbr->flags, F_TNBR_CONFIGURED) || tnbr->pw_count ||
          tnbr->rlfa_count)
...
recv_hello (struct in_addr lsr_id, struct ldp_msg *msg, int af,
            union ldpd_addr *src, struct iface *iface, int multicast,
            char *buf, uint16_t len)
{
  ...
  if (holdtime != 0 && holdtime < MIN_HOLDTIME)
    {
      log_debug ("%s: lsr-id %pI4: invalid hello holdtime (%u)", __func__,
                 &lsr_id, holdtime);
      return;
    }
  ...
  if (r == -1)
    {
      log_debug ("%s: lsr-id %pI4: failed to decode optional params", __func__,
                 &lsr_id);
      return;
    }
  if (r != len)
    {
      log_debug ("%s: lsr-id %pI4: unexpected data in message", __func__,
                 &lsr_id);
      return;
    }
  ...
  if (bad_addr (af, &trans_addr))
    {
      log_debug ("%s: lsr-id %pI4: invalid transport address %s", __func__,
  ...
  if (af == AF_INET6 && IN6_IS_SCOPE_EMBED (&trans_addr.v6))
    {
      ...
      if (CHECK_FLAG (flags, F_HELLO_TARGETED))
        {
          log_debug (
              "%s: lsr-id %pI4: invalid targeted hello transport address %s",
  ...
  if (CHECK_FLAG (flags, F_HELLO_TARGETED))
    {
      ...
      if (af == AF_INET6 && IN6_IS_SCOPE_EMBED (&src->v6))
        {
          log_debug (
              "%s: lsr-id %pI4: targeted hello with link-local source address",
              __func__, &lsr_id);
          return;
        }
      ...
      if (tnbr && CHECK_FLAG (tnbr->flags, F_TNBR_DYNAMIC) &&
      ...
      if (! tnbr)
        {
          ...
          if (! CHECK_FLAG (af_conf->flags, F_LDPD_AF_THELLO_ACCEPT))
            return;
          if (ldpe_acl_check (af_conf->acl_thello_accept_from, af, src,
                              (af == AF_INET) ? 32 : 128) != FILTER_PERMIT)
            return;
  ...
  debug_hello_recv ("%s lsr-id %pI4 transport-address %s holdtime %u%s",
                    log_hello_src (&source), &lsr_id,
                    log_addr (af, &trans_addr), holdtime,
                    (ds_tlv) ? " (dual stack TLV present)" : "");
  ...
  if (ds_tlv && trans_pref != leconf->trans_pref)
    {
       ...
       */
      log_debug (
          "%s: lsr-id %pI4: remote transport preference does not match the local preference",
          __func__, &lsr_id);
      if (nbr)
  ...
  if (nbr && nbr->af == af &&
    ...
    {
      log_warnx (
          "%s: lsr-id %pI4: hello packet advertising a different transport address",
          __func__, &lsr_id);
      if (adj)
        adj_del (adj, S_SHUTDOWN);
      return;
  ...
  if (nbr == NULL)
    {
      ...
      if (nbrt)
        {
          log_debug (
              "%s: transport address %s is already being used by lsr-id %pI4",
              __func__, log_addr (af, &trans_addr), &nbrt->id);
          if (adj)
            adj_del (adj, S_SHUTDOWN);
          return;
        }
   ...
   */
  if (nbr == NULL &&
      (! ds_tlv || ((trans_pref == DUAL_STACK_LDPOV4 && af == AF_INET) ||
...
tlv_decode_opt_hello_prms (char *buf, uint16_t len, int *tlvs_rcvd, int af,
                           union ldpd_addr *addr, uint32_t *conf_number,
                           uint16_t *trans_pref)
{
  struct tlv tlv;
  uint16_t tlv_len;
