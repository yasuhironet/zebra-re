...
void
ldpe_imsg_compose_parent_sync (int type, pid_t pid, void *data,
                               uint16_t datalen)
{
...
/* ARGSUSED */
static void
ldpe_dispatch_main (struct event *thread)
{
      ...
      switch (imsg.hdr.type)
        ...
        case IMSG_SOCKET_IPC:
          ...
          if ((fd = imsg.fd) == -1)
            {
              log_warnx (
                  "%s: expected to receive imsg fd to lde but didn't receive any",
                  __func__);
              break;
            }
        ...
        case IMSG_CLOSE_SOCKETS:
          ...
          edisc_socket = -1;
          session_socket = -1;
          if (CHECK_FLAG ((ldp_af_conf_get (leconf, af))->flags,
                          F_LDPD_AF_ENABLED))
        ...
        case IMSG_SETUP_SOCKETS:
          af = imsg.hdr.peerid;
          if (disc_socket == -1 || edisc_socket == -1 || session_socket == -1)
            {
              if (disc_socket != -1)
                close (disc_socket);
              if (edisc_socket != -1)
        ...
        case IMSG_LDP_SYNC_IF_STATE_REQUEST:
          if (imsg.hdr.len !=
              IMSG_HEADER_SIZE + sizeof (struct ldp_igp_sync_if_state_req))
            {
              log_warnx ("%s: wrong imsg len", __func__);
              break;
            }
...
/* ARGSUSED */
static void
ldpe_dispatch_lde (struct event *thread)
{
  struct imsgev *iev = EVENT_ARG (thread);
  struct imsgbuf *ibuf = &iev->ibuf;
  struct imsg imsg;
...
/* ARGSUSED */
static void
ldpe_dispatch_pfkey (struct event *thread)
{
...
int
ldpe_acl_check (char *acl_name, int af, union ldpd_addr *addr,
                uint8_t prefixlen)
{
      ...
      if (idx == 0 || idx == iface->ifindex)
        {
          ...
          ictl = if_to_ctl (ia);
          imsg_compose_event (&c->iev, IMSG_CTL_SHOW_INTERFACE, 0, 0, -1, ictl,
...
ldpe_adj_ctl (struct ctl_conn *c)
{
  ...
  RB_FOREACH (adj, global_adj_head, &global.adj_tree)
    {
      actl = adj_to_ctl (adj);
      imsg_compose_event (&c->iev, IMSG_CTL_SHOW_DISCOVERY, 0, 0, -1, actl,
  ...
  RB_FOREACH (iface, iface_head, &leconf->iface_tree)
    {
      ...
      if (RB_EMPTY (ia_adj_head, &iface->ipv4.adj_tree) &&
        ...
        ictl.no_adj = 1;
      imsg_compose_event (&c->iev, IMSG_CTL_SHOW_DISC_IFACE, 0, 0, -1, &ictl,
                          sizeof (ictl));

      RB_FOREACH (adj, ia_adj_head, &iface->ipv4.adj_tree)
        {
          actl = adj_to_ctl (adj);
          imsg_compose_event (&c->iev, IMSG_CTL_SHOW_DISC_ADJ, 0, 0, -1, actl,
                              sizeof (struct ctl_adj));
        }
  ...
  RB_FOREACH (tnbr, tnbr_head, &leconf->tnbr_tree)
    {
      ...
      imsg_compose_event (&c->iev, IMSG_CTL_SHOW_DISC_TNBR, 0, 0, -1, &tctl,
                          sizeof (tctl));
  ...
  RB_FOREACH (nbr, nbr_addr_head, &nbrs_by_addr)
    {
      ...
      RB_FOREACH (adj, nbr_adj_head, &nbr->adj_tree)
        {
          actl = adj_to_ctl (adj);
          imsg_compose_event (&c->iev, IMSG_CTL_SHOW_NBR_DISC, 0, 0, -1, actl,
                              sizeof (struct ctl_adj));
        }

      imsg_compose_event (&c->iev, IMSG_CTL_SHOW_NBR_END, 0, 0, -1, NULL, 0);
    }
  imsg_compose_event (&c->iev, IMSG_CTL_END, 0, 0, -1, NULL, 0);
}
  ...
  RB_FOREACH (iface, iface_head, &leconf->iface_tree)
    {
      ...
      imsg_compose_event (&c->iev, IMSG_CTL_SHOW_LDP_SYNC, 0, 0, -1, ictl,
