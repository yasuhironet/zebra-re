...
int tlv_decode_fec_elm (struct nbr *, struct ldp_msg *, char *, uint16_t,
                        struct map *);
...
int ldpe_imsg_compose_lde (int, uint32_t, pid_t, void *, uint16_t);
...
/* adjacency.c */
struct adj *adj_new (struct in_addr, struct hello_source *, union ldpd_addr *);
