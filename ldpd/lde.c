...
static void lde_nbr_addr_update (struct lde_nbr *, struct lde_addr *, int);
...
static void lde_check_filter_af (int, struct ldpd_af_conf *, const char *);
...
lde (void)
{
  ...
  event_add_read (master, iev_main->handler_read, iev_main, iev_main->ibuf.fd,
                  &iev_main->ev_read);
...
lde_init (struct ldpd_init *init)
{
  ...
  frr_zclient_addr (&zclient_addr, &zclient_addr_len, init->zclient_serv_path);
  zclient_sync_init ();
}
...
void
lde_imsg_compose_parent_sync (int type, pid_t pid, void *data,
                              uint16_t datalen)
{
                       ...
                       uint16_t datalen)
{
  if (iev_ldpe->ibuf.fd == -1)
    return (0);
  return (imsg_compose_event (iev_ldpe, type, peerid, pid, -1, data, datalen));
}

/* ARGSUSED */
static void
lde_dispatch_imsg (struct event *thread)
{
      ...
      switch (imsg.hdr.type)
        ...
        case IMSG_ADDRESS_ADD:
          ...
          if (lde_address_add (ln, lde_addr) < 0)
            {
              log_debug ("%s: cannot add address %s, it already exists",
                         __func__, log_addr (lde_addr->af, &lde_addr->addr));
            }
          break;
        case IMSG_ADDRESS_DEL:
          ...
          if (lde_address_del (ln, lde_addr) < 0)
            {
              log_debug ("%s: cannot delete address %s, it does not exist",
                         __func__, log_addr (lde_addr->af, &lde_addr->addr));
            }
          break;
...
/* ARGSUSED */
static void
lde_dispatch_parent (struct event *thread)
{
  ...
  for (;;)
    {
      ...
      switch (imsg.hdr.type)
        ...
        case IMSG_PW_UPDATE:
          if (imsg.hdr.len !=
              IMSG_HEADER_SIZE + sizeof (struct zapi_pw_status))
        ...
        case IMSG_NETWORK_UPDATE:
          ...
          switch (imsg.hdr.type)
            {
            case IMSG_NETWORK_ADD:
              lde_kernel_insert (&fec, kr->af, &kr->nexthop, kr->ifindex,
                                 kr->route_type, kr->route_instance,
        ...
        case IMSG_SOCKET_IPC:
          ...
          if ((fd = imsg.fd) == -1)
            {
              log_warnx (
                  "%s: expected to receive imsg fd to ldpe but didn't receive any",
                  __func__);
              break;
            }
        ...
        case IMSG_DEBUG_UPDATE:
          if (imsg.hdr.len != IMSG_HEADER_SIZE + sizeof (ldp_debug))
            {
              log_warnx ("%s: wrong imsg len", __func__);
              break;
            }
        ...
        case IMSG_FILTER_UPDATE:
          ...
          laccess = imsg.data;
          lde_check_filter_af (AF_INET, &ldeconf->ipv4, laccess->name);
          lde_check_filter_af (AF_INET6, &ldeconf->ipv6, laccess->name);
          break;
        case IMSG_RLFA_REG:
          if (imsg.hdr.len !=
              IMSG_HEADER_SIZE + sizeof (struct zapi_rlfa_request))
            {
              log_warnx ("%s: wrong imsg len", __func__);
              break;
            }
          rlfa_req = imsg.data;
          rnode =
              rlfa_node_find (&rlfa_req->destination, rlfa_req->pq_address);
          if (! rnode)
            rnode =
                rlfa_node_new (&rlfa_req->destination, rlfa_req->pq_address);
        ...
        case IMSG_RLFA_UNREG_ALL:
          if (imsg.hdr.len != IMSG_HEADER_SIZE + sizeof (struct zapi_rlfa_igp))
            {
              log_warnx ("%s: wrong imsg len", __func__);
              break;
            }
          ...
          RB_FOREACH_SAFE (rnode, ldp_rlfa_node_head, &rlfa_node_tree, rntmp)
            {
...
int
lde_acl_check (char *acl_name, int af, union ldpd_addr *addr,
               uint8_t prefixlen)
{
...
static bool
lde_fec_connected (const struct fec_node *fn)
{
  struct fec_nh *fnh;
...
static bool
lde_fec_outside_mpls_network (const struct fec_node *fn)
{
  struct fec_nh *fnh;
...
lde_update_label (struct fec_node *fn)
{
  ...
  switch (fn->fec.type)
    ...
    case FEC_TYPE_IPV6:
      ...
      if (lde_acl_check (ldeconf->ipv6.acl_label_allocate_for, AF_INET6,
                         (union ldpd_addr *) &fn->fec.u.ipv6.prefix,
  ...
  if (fn->local_label != NO_LABEL && fn->local_label > MPLS_LABEL_RESERVED_MAX)
...
lde_send_change_klabel (struct fec_node *fn, struct fec_nh *fnh)
{
  ...
  switch (fn->fec.type)
    ...
    case FEC_TYPE_PWID:
      ...
      if (! pw || fn->local_label == NO_LABEL || fnh->remote_label == NO_LABEL)
        return;
...
lde_send_labelmapping (struct lde_nbr *ln, struct fec_node *fn, int single)
{
  ...
  if (lw)
    {
      if (! fec_find (&ln->sent_map_pending, &fn->fec))
        {
          debug_evt (
              "%s: FEC %s: scheduling to send label mapping later (waiting for pending label release)",
  ...
  switch (fn->fec.type)
    ...
    case FEC_TYPE_IPV4:
      ...
      if (lde_acl_check (ldeconf->ipv4.acl_label_advertise_to, AF_INET,
                         (union ldpd_addr *) &ln->id, 32) != FILTER_PERMIT)
        return;
  ...
  lde_imsg_compose_ldpe (IMSG_MAPPING_ADD, ln->peerid, 0, &map, sizeof (map));
  if (single)
                        ...
                        struct map *wcard, struct status_tlv *st)
{
  ...
  lde_imsg_compose_ldpe (IMSG_WITHDRAW_ADD, ln->peerid, 0, &map, sizeof (map));
                       ...
                       struct map *wcard, uint32_t label)
{
  ...
  lde_imsg_compose_ldpe (IMSG_RELEASE_ADD, ln->peerid, 0, &map, sizeof (map));
      ...
      if (lre == NULL)
        {
          ...
          lde_imsg_compose_ldpe (IMSG_REQUEST_ADD, ln->peerid, 0, &map,
                                 sizeof (map));
          if (single)
            lde_imsg_compose_ldpe (IMSG_REQUEST_ADD_END, ln->peerid, 0, NULL,
                                   0);
...
void
lde_send_notification (struct lde_nbr *ln, uint32_t status_code,
                       uint32_t msg_id, uint16_t msg_type)
{
  struct notify_msg nm;
...
static void
lde_nbr_addr_update (struct lde_nbr *ln, struct lde_addr *lde_addr,
                     int removed)
{
  struct fec *fec;
  struct fec_node *fn;
  ...
  if (lre != NULL)
    {
      ...
      if (fec_insert (t, &lre->fec))
        {
          log_warnx ("failed to add %s to %s req", log_fec (&lre->fec),
                     sent ? "sent" : "recv");
          free (lre);
          return (NULL);
        }
          ...
          RB_FOREACH (f, fec_tree, &ft)
            {
              ...
              if (lde_acl_check (acl_for_filter, af, prefix, plen) !=
                  FILTER_PERMIT)
                {
                  me = (struct lde_map *) fec_find (&ln->sent_map, &fn->fec);
                  if (me)
...
lde_change_accept_filter (int af)
{
  ...
  RB_FOREACH (ln, nbr_tree, &lde_nbrs)
    {
      if (lde_acl_check (acl_from_filter, AF_INET, (union ldpd_addr *) &ln->id,
                         IPV4_MAX_BITLEN) != FILTER_PERMIT)
        {
...
lde_address_find (struct lde_nbr *ln, int af, union ldpd_addr *addr)
{
  ...
  TAILQ_FOREACH (lde_addr, &ln->addr_list, entry)
    if (lde_addr->af == af && ldp_addrcmp (af, &lde_addr->addr, addr) == 0)
      return (lde_addr);
 ...
 */
static void
zclient_sync_retry (struct event *thread)
{
  zclient_sync_init ();
}
...
lde_get_label_chunk (void)
{
  ...
  ret = lm_get_label_chunk (zclient_sync, 0, MPLS_LABEL_BASE_ANY, CHUNK_SIZE,
                            &start, &end);
  if (ret < 0)
    {
      ...
      if (label <= label_chunk->end && label >= label_chunk->start)
        {
          ...
          /* if nobody is using this chunk and it's not current_label_chunk,
           * then free it */
          if (! label_chunk->used_mask && (current_label_chunk != node))
            {
              if (lde_release_label_chunk (label_chunk->start,
                                           label_chunk->end) != 0)
...
lde_get_next_label (void)
{
  ...
  if (! current_label_chunk ||
      current_label_chunk == listtail (label_chunk_list) || label == NO_LABEL)
    {
...
void
lde_route_update (struct iface *iface, int af)
{
  struct fec *f;
  struct fec_node *fn;
  struct fec_nh *fnh;
...
void
lde_route_update_release (struct iface *iface, int af)
{
  struct lde_nbr *ln;
  struct fec *f;
  struct fec_node *fn;
...
void
lde_route_update_release_all (int af)
{
  struct lde_nbr *ln;
  struct fec *f;
  struct fec_node *fn;
