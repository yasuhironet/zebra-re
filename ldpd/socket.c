...
sock_set_md5sig (int fd, int af, union ldpd_addr *addr, const char *password)
{
  ...
  if (ret < 0)
    log_warnx ("%s: can't set TCP_MD5SIG option on fd %d: %s", __func__, fd,
...
{
  if (setsockopt (fd, IPPROTO_IP, IP_RECVDSTADDR, &enable, sizeof (enable)) <
      0)
    {
...
sock_set_ipv4_mcast (struct iface *iface)
{
  ...
  if (setsockopt_ipv4_multicast_if (global.ipv4.ldp_disc_socket, if_addr,
                                    iface->ifindex) < 0)
    {
      log_warn ("%s: error setting IP_MULTICAST_IF, interface %s", __func__,
                iface->name);
      return (-1);
    }
...
{
  if (setsockopt (fd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &hoplimit,
                  sizeof (hoplimit)) < 0)
    {
...
{
  if (setsockopt (fd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &hoplimit,
                  sizeof (hoplimit)) < 0)
    {
...
{
  if (setsockopt (global.ipv6.ldp_disc_socket, IPPROTO_IPV6, IPV6_MULTICAST_IF,
                  &iface->ifindex, sizeof (iface->ifindex)) < 0)
    {
      log_warn ("%s: error setting IPV6_MULTICAST_IF, interface %s", __func__,
                iface->name);
      return (-1);
    }
