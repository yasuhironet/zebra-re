...
const struct
{
  ...
  { NBR_STA_INITIAL, NBR_EVT_INIT_RCVD, NBR_ACT_PASSIVE_INIT,
    NBR_STA_OPENREC },
  { NBR_STA_OPENREC, NBR_EVT_KEEPALIVE_RCVD, NBR_ACT_SESSION_EST,
    NBR_STA_OPER },
  /* Active Role */
  { NBR_STA_PRESENT, NBR_EVT_CONNECT_UP, NBR_ACT_CONNECT_SETUP,
    NBR_STA_INITIAL },
  { NBR_STA_INITIAL, NBR_EVT_INIT_SENT, NBR_ACT_NOTHING, NBR_STA_OPENSENT },
  { NBR_STA_OPENSENT, NBR_EVT_INIT_RCVD, NBR_ACT_KEEPALIVE_SEND,
    NBR_STA_OPENREC },
  /* Session Maintenance */
  ...
  { NBR_STA_SESSION, NBR_EVT_CLOSE_SESSION, NBR_ACT_CLOSE_SESSION,
    NBR_STA_PRESENT },
  { -1, NBR_EVT_NOTHING, NBR_ACT_NOTHING, 0 },
};

const char *const nbr_event_names[] = { "NOTHING",       "ADJACENCY MATCHED",
                                        "CONNECTION UP", "SESSION CLOSE",
                                        "INIT RECEIVED", "KEEPALIVE RECEIVED",
                                        "PDU RECEIVED",  "PDU SENT",
                                        "INIT SENT" };

const char *const nbr_action_names[] = { "NOTHING",
...
nbr_fsm (struct nbr *nbr, enum nbr_event event)
{
  ...
  if (nbr_fsm_tbl[i].state == -1)
    {
      ...
      log_warnx ("%s: lsr-id %pI4, event %s not expected in state %s",
                 __func__, &nbr->id, nbr_event_names[event],
  ...
  if (old_state != nbr->state)
    {
      log_debug (
          "%s: event %s resulted in action %s and changing state for lsr-id %pI4 from %s to %s",
         ...
         uint32_t scope_id)
{
  ...
  log_debug ("%s: lsr-id %pI4 transport-address %s", __func__, &id,
             log_addr (af, addr));
...
#else
      sock_set_md5sig (
          (ldp_af_global_get (&global, nbr->af))->ldp_session_socket, nbr->af,
...
nbr_del (struct nbr *nbr)
{
  ...
  pfkey_remove (nbr);
#else
  sock_set_md5sig ((ldp_af_global_get (&global, nbr->af))->ldp_session_socket,
...
static void
nbr_ktimer (struct event *thread)
{
...
static void
nbr_ktimeout (struct event *thread)
{
  struct nbr *nbr = EVENT_ARG (thread);
...
static void
nbr_itimeout (struct event *thread)
{
  struct nbr *nbr = EVENT_ARG (thread);
...
static void
nbr_idtimer (struct event *thread)
{
  struct nbr *nbr = EVENT_ARG (thread);
...
static void
nbr_connect_cb (struct event *thread)
{
  struct nbr *nbr = EVENT_ARG (thread);
  int error;
  socklen_t len;
...
nbr_establish_connection (struct nbr *nbr)
{
  ...
  if (nbrp && nbrp->auth.method == AUTH_MD5SIG)
    {
      ...
      if (setsockopt (nbr->fd, IPPROTO_TCP, TCP_MD5SIG, &opt, sizeof (opt)) ==
          -1)
        {
  ...
  if (nbr->af == AF_INET)
    {
      if (sock_set_ipv4_tos (nbr->fd, IPTOS_PREC_INTERNETCONTROL) == -1)
        log_warn ("%s: lsr-id %pI4, sock_set_ipv4_tos error", __func__,
                  &nbr->id);
    }
  else if (nbr->af == AF_INET6)
    {
      if (sock_set_ipv6_dscp (nbr->fd, IPTOS_PREC_INTERNETCONTROL) == -1)
        log_warn ("%s: lsr-id %pI4, sock_set_ipv6_dscp error", __func__,
                  &nbr->id);
    }
   ...
   */
  RB_FOREACH (adj, nbr_adj_head, &nbr->adj_tree)
    send_hello (adj->source.type, adj->source.link.ia, adj->source.target);
...
nbr_gtsm_enabled (struct nbr *nbr, struct nbr_params *nbrp)
{
  ...
  if (CHECK_FLAG ((ldp_af_conf_get (leconf, nbr->af))->flags,
                  F_LDPD_AF_NO_GTSM))
    return (0);
...
nbr_act_session_operational (struct nbr *nbr)
{
  ...
  return (ldpe_imsg_compose_lde (IMSG_NEIGHBOR_UP, nbr->peerid, 0, &lde_nbr,
                                 sizeof (lde_nbr)));
}
