...
static int show_interface_msg (struct vty *, struct imsg *,
                               struct show_params *);
static int show_interface_msg_json (struct imsg *, struct show_params *,
                                    json_object *);
static int show_discovery_msg (struct vty *, struct imsg *,
                               struct show_params *);
static void show_discovery_detail_adj (struct vty *, char *, struct ctl_adj *);
static int show_discovery_detail_msg (struct vty *, struct imsg *,
                                      struct show_params *);
static int show_discovery_msg_json (struct imsg *, struct show_params *,
                                    json_object *);
static void show_discovery_detail_adj_json (json_object *, struct ctl_adj *);
static int show_discovery_detail_msg_json (struct imsg *, struct show_params *,
                                           json_object *);
static int show_ldp_sync_msg (struct vty *, struct imsg *,
                              struct show_params *);
static int show_ldp_sync_msg_json (struct imsg *, struct show_params *,
                                   json_object *);

static int show_nbr_msg (struct vty *, struct imsg *, struct show_params *);
static int show_nbr_msg_json (struct imsg *, struct show_params *,
                              json_object *);
static void show_nbr_detail_adj (struct vty *, char *, struct ctl_adj *);
static int show_nbr_detail_msg (struct vty *, struct imsg *,
                                struct show_params *);
static void show_nbr_detail_adj_json (struct ctl_adj *, json_object *);
static int show_nbr_detail_msg_json (struct imsg *, struct show_params *,
                                     json_object *);
static void show_nbr_capabilities (struct vty *, struct ctl_nbr *);
static int show_nbr_capabilities_msg (struct vty *, struct imsg *,
                                      struct show_params *);
static void show_nbr_capabilities_json (struct ctl_nbr *, json_object *);
static int show_nbr_capabilities_msg_json (struct imsg *, struct show_params *,
                                           json_object *);
static int show_lib_msg (struct vty *, struct imsg *, struct show_params *);
static int show_lib_detail_msg (struct vty *, struct imsg *,
                                struct show_params *);
static int show_lib_msg_json (struct imsg *, struct show_params *,
                              json_object *);
static int show_lib_detail_msg_json (struct imsg *, struct show_params *,
                                     json_object *);
static int show_l2vpn_binding_msg (struct vty *, struct imsg *,
                                   struct show_params *);
static int show_l2vpn_binding_msg_json (struct imsg *, struct show_params *,
                                        json_object *);
static int show_l2vpn_pw_msg (struct vty *, struct imsg *,
                              struct show_params *);
static int show_l2vpn_pw_msg_json (struct imsg *, struct show_params *,
                                   json_object *);
static int ldp_vty_dispatch_msg (struct vty *, struct imsg *,
                                 enum show_command, struct show_params *,
                                 json_object *);
static int ldp_vty_dispatch (struct vty *, struct imsgbuf *, enum show_command,
                             struct show_params *);
                    ...
                    struct show_params *params)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_INTERFACE:
      ...
      snprintf (timers, sizeof (timers), "%u/%u", iface->hello_interval,
                iface->hello_holdtime);

      vty_out (vty, "%-4s %-11s %-6s %-8s %-12s %3u\n", af_name (iface->af),
               iface->name, if_state_name (iface->state),
               iface->uptime == 0 ? "00:00:00" : log_time (iface->uptime),
                         ...
                         json_object *json)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_INTERFACE:
      ...
      json_object_int_add (json_iface, "helloHoldtime", iface->hello_holdtime);
                        ...
                        json_object *json)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_LDP_SYNC:
      ...
      json_iface = json_object_new_object ();
      json_object_string_add (json_iface, "state",
                              iface->in_sync ? "labelExchangeComplete"
                                             : "labelExchangeNotComplete");
      json_object_int_add (json_iface, "waitTime", iface->wait_time);
...
show_discovery_detail_adj (struct vty *vty, char *buffer, struct ctl_adj *adj)
{
  ...
  snprintfrr (buffer + buflen, LDPBUFSIZ - buflen, "      LSR Id: %pI4:0\n",
              &adj->id);
                           ...
                           struct show_params *params)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_DISC_IFACE:
      ...
      snprintf (ifaces_buffer + buflen, LDPBUFSIZ - buflen, "    %s: %s\n",
                iface->name, (iface->no_adj) ? "(no adjacencies)" : "");
      break;
                         ...
                         json_object *json)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_DISCOVERY:
      ...
      json_object_string_add (json_adj, "addressFamily", af_name (adj->af));
      json_object_string_addf (json_adj, "neighborId", "%pI4", &adj->id);
      switch (adj->type)
        {
        case HELLO_LINK:
          json_object_string_add (json_adj, "type", "link");
          json_object_string_add (json_adj, "interface", adj->ifname);
          break;
        case HELLO_TARGETED:
...
show_discovery_detail_adj_json (json_object *json, struct ctl_adj *adj)
{
  ...
  json_object_string_add (json_adj, "sourceAddress",
                          log_addr (adj->af, &adj->src_addr));
  json_object_string_add (json_adj, "transportAddress",
                          log_addr (adj->af, &adj->trans_addr));
  json_object_int_add (json_adj, "helloHoldtime", adj->holdtime);
  json_object_int_add (json_adj, "helloHoldtimeRemaining",
                       adj->holdtime_remaining);
  json_object_int_add (json_adj, "dualStackCapabilityTlv", adj->ds_tlv);
  json_object_array_add (json_array, json_adj);
}
                                ...
                                json_object *json)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_DISCOVERY:
      ...
      if (CHECK_FLAG (ldpd_conf->ipv4.flags, F_LDPD_AF_ENABLED))
        json_object_string_add (
            json, "transportAddressIPv4",
            log_addr (AF_INET, &ldpd_conf->ipv4.trans_addr));
      if (CHECK_FLAG (ldpd_conf->ipv6.flags, F_LDPD_AF_ENABLED))
        json_object_string_add (
            json, "transportAddressIPv6",
            log_addr (AF_INET6, &ldpd_conf->ipv6.trans_addr));
    ...
    case IMSG_CTL_SHOW_DISC_TNBR:
      ...
      json_object_string_add (json_target, "sourceAddress",
                              log_addr (tnbr->af, trans_addr));
      json_object_object_add (json_targets, log_addr (tnbr->af, &tnbr->addr),
                              json_target);
...
show_nbr_msg (struct vty *vty, struct imsg *imsg, struct show_params *params)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_NBR:
      ...
      vty_out (vty, "%-4s %-15pI4 %-11s %-15s", af_name (nbr->af), &nbr->id,
...
show_nbr_detail_adj (struct vty *vty, char *buffer, struct ctl_adj *adj)
{
  ...
  switch (adj->type)
    {
    case HELLO_LINK:
      snprintf (buffer + buflen, LDPBUFSIZ - buflen, "      Interface: %s\n",
                adj->ifname);
      break;
    case HELLO_TARGETED:
      snprintf (buffer + buflen, LDPBUFSIZ - buflen,
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_NBR:
      ...
      vty_out (vty,
               ...
               nbr->holdtime, nbr->holdtime / KEEPALIVE_PER_PERIOD);
                   ...
                   json_object *json)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_NBR:
      ...
      json_object_string_add (json_nbr, "addressFamily", af_name (nbr->af));
      json_object_string_addf (json_nbr, "neighborId", "%pI4", &nbr->id);
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_NBR:
      ...
      json_object_object_add (
          json, inet_ntop (AF_INET, &nbr->id, buf, sizeof (buf)), json_nbr);
      ...
      json_object_int_add (json_counter, "keepalive", stats->kalive_sent);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "address", stats->addr_sent);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      ...
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "notification", stats->notif_sent);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "capability", stats->capability_sent);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelMapping", stats->labelmap_sent);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelRequest", stats->labelreq_sent);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      ...
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelRelease", stats->labelrel_sent);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      ...
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "keepalive", stats->kalive_rcvd);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "address", stats->addr_rcvd);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "addressWithdraw",
      ...
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "notification", stats->notif_rcvd);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "capability", stats->capability_rcvd);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelMapping", stats->labelmap_rcvd);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelRequest", stats->labelreq_rcvd);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelWithdraw",
      ...
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelRelease", stats->labelrel_rcvd);
      json_object_array_add (json_array, json_counter);
      json_counter = json_object_new_object ();
      json_object_int_add (json_counter, "labelAbortRequest",
      ...
      json_object_object_add (json_nbr, "discoverySources", json_nbr_sources);
      json_v4adjs = NULL;
      json_v6adjs = NULL;
      break;
    case IMSG_CTL_SHOW_NBR_DISC:
          ...
          if (! json_v4adjs)
            {
              ...
              json_object_object_add (json_nbr_sources, "ipv4", json_v4adjs);
            }
          ...
          if (! json_v6adjs)
            {
              ...
              json_object_object_add (json_nbr_sources, "ipv6", json_v6adjs);
            }
...
static int
show_nbr_capabilities_msg (struct vty *vty, struct imsg *imsg,
                           struct show_params *params)
{
  struct ctl_nbr *nbr;
...
show_lib_msg (struct vty *vty, struct imsg *imsg, struct show_params *params)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_LIB_RCVD:
      ...
      if (imsg->hdr.type == IMSG_CTL_SHOW_LIB_BEGIN && ! rt->no_downstream)
        break;
...
static int
show_lib_detail_msg (struct vty *vty, struct imsg *imsg,
                     struct show_params *params)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_LIB_SENT:
      ...
      snprintfrr (sent_buffer + buflen, LDPBUFSIZ - buflen, "%12s%pI4:0\n", "",
                  &rt->nexthop);
      break;
    case IMSG_CTL_SHOW_LIB_RCVD:
      rt = imsg->data;
      ...
      snprintfrr (rcvd_buffer + buflen, LDPBUFSIZ - buflen,
                  "%12s%pI4:0, label %s%s\n", "", &rt->nexthop,
                  log_label (rt->remote_label), rt->in_use ? " (in use)" : "");
      break;
    case IMSG_CTL_SHOW_LIB_END:
      rt = imsg->data;

      if (params->lib.remote_label != NO_LABEL && ! downstream)
        break;
      vty_out (vty, "%s\n", dstnet);
                          ...
                          json_object *json)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_LIB_BEGIN:
      ...
      json_object_object_add (json_lib_entry, "advertisedTo", json_adv_labels);
      ...
      if (pw->local_label != NO_LABEL)
        {
          ...
          vty_out (vty, "%-8sCbit: %u,    VC Type: %s,    GroupID: %u\n", "",
                   pw->local_cword, pw_type_name (pw->type), pw->local_gid);
          vty_out (vty, "%-8sMTU: %u\n", "", pw->local_ifmtu);
      ...
      if (pw->remote_label != NO_LABEL)
        {
          ...
          vty_out (vty, "%-8sCbit: %u,    VC Type: %s,    GroupID: %u\n", "",
                   pw->remote_cword, pw_type_name (pw->type), pw->remote_gid);
          vty_out (vty, "%-8sMTU: %u\n", "", pw->remote_ifmtu);
        }
      else
                             ...
                             json_object *json)
{
  ...
  switch (imsg->hdr.type)
    ...
    case IMSG_CTL_SHOW_L2VPN_BINDING:
      ...
      json_object_string_addf (json_pw, "destination", "%pI4", &pw->lsr_id);
      ...
      if (pw->local_label != NO_LABEL)
        {
          ...
          json_object_int_add (json_pw, "localControlWord", pw->local_cword);
          json_object_string_add (json_pw, "localVcType",
                                  pw_type_name (pw->type));
          json_object_int_add (json_pw, "localGroupID", pw->local_gid);
          json_object_int_add (json_pw, "localIfMtu", pw->local_ifmtu);
          json_object_string_add (json_pw, "lastFailureReason",
                                  pw_error_code (pw->reason));
        }
      else
        json_object_string_add (json_pw, "localLabel", "unassigned");

      /* remote binding */
      if (pw->remote_label != NO_LABEL)
        {
          json_object_int_add (json_pw, "remoteLabel", pw->remote_label);
          json_object_int_add (json_pw, "remoteControlWord", pw->remote_cword);
          json_object_string_add (json_pw, "remoteVcType",
                                  pw_type_name (pw->type));
          json_object_int_add (json_pw, "remoteGroupID", pw->remote_gid);
          json_object_int_add (json_pw, "remoteIfMtu", pw->remote_ifmtu);
        }
      else
        json_object_string_add (json_pw, "remoteLabel", "unassigned");

      snprintfrr (key_name, sizeof (key_name), "%pI4: %u", &pw->lsr_id,
                  pw->pwid);
                       ...
                       struct show_params *params, json_object *json)
{
  ...
  if (params->detail)
    {
      if (params->json)
        ret = show_discovery_detail_msg_json (imsg, params, json);
      else
                      ...
                      struct show_params *params, json_object *json)
{
  ...
  if (params->neighbor.capabilities)
    {
      if (params->json)
        ret = show_nbr_capabilities_msg_json (imsg, params, json);
      else
...
static int
ldp_vty_dispatch_msg (struct vty *vty, struct imsg *imsg,
                      enum show_command cmd, struct show_params *params,
                      json_object *json)
{
  switch (cmd)
    {
    ...
    case SHOW_L2VPN_BINDING:
      return (ldp_vty_dispatch_l2vpn_binding (vty, imsg, params, json));
    case SHOW_LDP_SYNC:
      ...
      while (! done)
        {
          ...
          if (n == 0)
            break;
          done = ldp_vty_dispatch_msg (vty, &imsg, cmd, params, json);
          imsg_free (&imsg);
        }
    }
...
ldp_vty_show_binding (struct vty *vty, const char *af_str, const char *prefix,
                      int longer_prefixes, const char *neighbor,
                      unsigned long local_label, unsigned long remote_label,
                      const char *detail, const char *json)
{
  ...
  if (neighbor && (inet_pton (AF_INET, neighbor, &params.lib.neighbor) != 1 ||
...
int
ldp_vty_show_discovery (struct vty *vty, const char *af_str,
                        const char *detail, const char *json)
{
  struct imsgbuf ibuf;
  struct show_params params;
  ...
  if (params.detail)
    imsg_compose (&ibuf, IMSG_CTL_SHOW_DISCOVERY_DTL, 0, 0, -1, NULL, 0);
  else
                       ...
                       const char *detail, const char *json)
{
  ...
  if (lsr_id && (inet_pton (AF_INET, lsr_id, &params.neighbor.lsr_id) != 1 ||
                           ...
                           unsigned long remote_label, const char *json)
{
  ...
  memset (&params, 0, sizeof (params));
  params.json = (json) ? 1 : 0;
  if (peer && (inet_pton (AF_INET, peer, &params.l2vpn.peer) != 1 ||
                      ...
                      const char *vcid, const char *json)
{
  ...
  if (ifname)
    strlcpy (params.l2vpn.ifname, ifname, sizeof (params.l2vpn.ifname));
  if (vcid)
...
ldp_vty_clear_nbr (struct vty *vty, const char *addr_str)
{
  ...
  if (addr_str && (ldp_get_address (addr_str, &nbr.af, &nbr.raddr) == -1 ||
