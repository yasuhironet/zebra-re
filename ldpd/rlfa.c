...
static int
ldp_rlfa_client_compare (const struct ldp_rlfa_client *a,
...
static int
ldp_rlfa_node_compare (const struct ldp_rlfa_node *a,
...
rlfa_client_new (struct ldp_rlfa_node *rnode, struct zapi_rlfa_igp *igp)
{
...
void
rlfa_client_del (struct ldp_rlfa_client *rclient)
{
...
struct ldp_rlfa_client *
rlfa_client_find (struct ldp_rlfa_node *rnode, struct zapi_rlfa_igp *igp)
{
  struct ldp_rlfa_client rclient;
...
rlfa_node_new (const struct prefix *destination, struct in_addr pq_address)
{
...
void
rlfa_node_del (struct ldp_rlfa_node *rnode)
{
  /* Delete RLFA clients. */
...
struct ldp_rlfa_node *
rlfa_node_find (const struct prefix *destination, struct in_addr pq_address)
{
  struct ldp_rlfa_node rnode = {};
...
void
lde_rlfa_client_send (struct ldp_rlfa_client *rclient)
{
  struct ldp_rlfa_node *rnode = rclient->node;
...
void
lde_rlfa_label_update (const struct fec *fec)
{
  struct ldp_rlfa_node *rnode;

  if (fec->type != FEC_TYPE_IPV4 || fec->u.ipv4.prefixlen != IPV4_MAX_BITLEN)
    return;
...
void
lde_rlfa_check (struct ldp_rlfa_client *rclient)
{
  struct lde_nbr *ln;
  struct lde_map *me;
  struct fec fec;
 ...
 */
void
lde_rlfa_update_clients (struct fec *fec, struct lde_nbr *ln, uint32_t label)
{
...
void
ldpe_rlfa_init (struct ldp_rlfa_client *rclient)
{
  struct tnbr *tnbr;
  union ldpd_addr pq_address = {};
...
void
ldpe_rlfa_exit (struct ldp_rlfa_client *rclient)
{
  struct tnbr *tnbr;
  union ldpd_addr pq_address = {};
