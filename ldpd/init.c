...
send_init (struct nbr *nbr)
{
  ...
  size = LDP_HDR_SIZE + LDP_MSG_SIZE + SESS_PRMS_SIZE + CAP_TLV_DYNAMIC_SIZE +
         CAP_TLV_TWCARD_SIZE + CAP_TLV_UNOTIF_SIZE;
...
recv_init (struct nbr *nbr, char *buf, uint16_t len)
{
  ...
  if (sess.lsr_id != ldp_rtr_id_get (leconf) || ntohs (sess.lspace_id) != 0)
    {
  ...
  while (len > 0)
    {
      ...
      switch (tlv_type)
        ...
        case TLV_TYPE_DYNAMIC_CAP:
          ...
          log_debug (
              "%s: lsr-id %pI4 announced the Dynamic Capability Announcement capability",
              __func__, &nbr->id);
          break;
        case TLV_TYPE_TWCARD_CAP:
          ...
          log_debug (
              "%s: lsr-id %pI4 announced the Typed Wildcard FEC capability",
              __func__, &nbr->id);
          break;
        case TLV_TYPE_UNOTIF_CAP:
          ...
          log_debug (
              "%s: lsr-id %pI4 announced the Unrecognized Notification capability",
              __func__, &nbr->id);
          break;
        default:
          if (! CHECK_FLAG (ntohs (tlv.type), UNKNOWN_FLAG))
            send_notification_rtlvs (nbr, S_UNSSUPORTDCAP, msg.id, msg.type,
                                     tlv_type, tlv_len, buf);
          /* ignore unknown tlv */
          break;
        }
      buf += tlv_len;
  ...
  nbr->keepalive =
      MIN (nbr_get_keepalive (nbr->af, nbr->id), ntohs (sess.keepalive_time));
  ...
  while (len > 0)
    {
      ...
      switch (tlv_type)
        ...
        case TLV_TYPE_TWCARD_CAP:
          ...
          log_debug ("%s: lsr-id %pI4 %s the Typed Wildcard FEC capability",
                     __func__, &nbr->id, (enable) ? "announced" : "withdrew");
          break;
        case TLV_TYPE_UNOTIF_CAP:
          if (tlv_len != CAP_TLV_UNOTIF_LEN)
            {
          ...
          log_debug (
              "%s: lsr-id %pI4 %s the Unrecognized Notification capability",
              __func__, &nbr->id, (enable) ? "announced" : "withdrew");
          break;
        case TLV_TYPE_DYNAMIC_CAP:
          /*
