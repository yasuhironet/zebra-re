...
ldp_sync_zebra_send_state_update (struct ldp_igp_sync_if_state *state)
{
  if (zclient_send_opaque (zclient, LDP_IGP_SYNC_IF_STATE_UPDATE,
                           (const uint8_t *) state,
                           sizeof (*state)) == ZCLIENT_SEND_FAILURE)
    return -1;
  else
    return 0;
}
...
int
ldp_zebra_send_rlfa_labels (struct zapi_rlfa_response *rlfa_labels)
{
  int ret;
...
kmpw_add (struct zapi_pw *zpw)
{
  debug_zebra_out ("pseudowire %s nexthop %s (add)", zpw->ifname,
                   log_addr (zpw->af, (union ldpd_addr *) &zpw->nexthop));
...
ldp_ifp_create (struct interface *ifp)
{
  ...
  debug_zebra_in ("interface add %s index %d mtu %d", ifp->name, ifp->ifindex,
                  ifp->mtu);
...
static int
ldp_ifp_up (struct interface *ifp)
{
  return ldp_interface_status_change (ifp);
}

static int
ldp_ifp_down (struct interface *ifp)
{
  return ldp_interface_status_change (ifp);
}
...
ldp_interface_address_add (ZAPI_CALLBACK_ARGS)
{
  ...
  debug_zebra_in ("address add %s/%u interface %s", log_addr (ka.af, &ka.addr),
                  ka.prefixlen, ifp->name);
...
void
ldp_zebra_regdereg_zebra_info (bool want_register)
{
...
void
ldp_zebra_init (struct event_loop *master)
{
