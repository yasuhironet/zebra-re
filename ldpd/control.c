...
/* ARGSUSED */
static void
control_accept (struct event *thread)
{
  int connfd;
  socklen_t len;
  struct sockaddr_un s_un;
  ...
  len = sizeof (s_un);
  if ((connfd = accept (EVENT_FD (thread), (struct sockaddr *) &s_un, &len)) ==
      -1)
    {
      /*
...
/* ARGSUSED */
static void
control_dispatch_imsg (struct event *thread)
{
  ...
  for (;;)
    {
      ...
      switch (imsg.hdr.type)
        ...
        case IMSG_CTL_SHOW_L2VPN_BINDING:
          ...
          ldpe_imsg_compose_lde (imsg.hdr.type, 0, imsg.hdr.pid, imsg.data,
...
control_imsg_relay (struct imsg *imsg)
{
  ...
  return (imsg_compose_event (&c->iev, imsg->hdr.type, 0, imsg->hdr.pid, -1,
