...
static __inline int l2vpn_if_compare (const struct l2vpn_if *,
                                      const struct l2vpn_if *);
static __inline int l2vpn_pw_compare (const struct l2vpn_pw *,
...
l2vpn_pw_init (struct l2vpn_pw *pw)
{
  ...
  l2vpn_pw_fec (pw, &fec);
  lde_kernel_insert (&fec, AF_INET, (union ldpd_addr *) &pw->lsr_id, 0, 0, 0,
                     0, (void *) pw);
    ...
    RB_FOREACH (pw, l2vpn_pw_head, &l2vpn->pw_tree)
      {
        ...
        strlcpy (pwctl.l2vpn_name, pw->l2vpn->name, sizeof (pwctl.l2vpn_name));
        strlcpy (pwctl.ifname, pw->ifname, sizeof (pwctl.ifname));
        pwctl.pwid = pw->pwid;
        pwctl.lsr_id = pw->lsr_id;
        pwctl.status = PW_NOT_FORWARDING;
        if (pw->enabled && pw->local_status == PW_FORWARDING &&
            pw->remote_status == PW_FORWARDING)
          pwctl.status = PW_FORWARDING;

        lde_imsg_compose_ldpe (IMSG_CTL_SHOW_L2VPN_PW, 0, pid, &pwctl,
                               sizeof (pwctl));
      }
}
      ...
      if (me)
        {
          ...
          lde_imsg_compose_ldpe (IMSG_CTL_SHOW_L2VPN_BINDING, 0, pid, &pwctl,
                                 sizeof (pwctl));
        }
      else if (pw)
        {
