...
static uint8_t *
ldpLsrId (struct variable *v, oid name[], size_t *length, int exact,
...
static uint8_t *
ldpLoopDetectCap (struct variable *v, oid name[], size_t *length, int exact,
                  size_t *var_len, WriteMethod **write_method)
{
  if (smux_header_generic (v, name, length, exact, var_len, write_method) ==
      MATCH_FAILED)
...
static uint8_t *
ldpEntityLastChange (struct variable *v, oid name[], size_t *length, int exact,
                     size_t *var_len, WriteMethod **write_method)
{
  if (smux_header_generic (v, name, length, exact, var_len, write_method) ==
      MATCH_FAILED)
...
static uint8_t *
ldpEntityIndexNext (struct variable *v, oid name[], size_t *length, int exact,
                    size_t *var_len, WriteMethod **write_method)
{
  if (smux_header_generic (v, name, length, exact, var_len, write_method) ==
      MATCH_FAILED)
...
static struct ldpd_af_conf *
ldpEntityTable_lookup (struct variable *v, oid *name, size_t *length,
                       int exact, uint32_t *index)
{
  int len;
...
static uint8_t *
ldpEntityTable (struct variable *v, oid name[], size_t *length, int exact,
                size_t *var_len, WriteMethod **write_method)
{
...
static uint8_t *
ldpEntityStatsTable (struct variable *v, oid name[], size_t *length, int exact,
                     size_t *var_len, WriteMethod **write_method)
{
  struct in_addr entityLdpId = { .s_addr = 0 };
  int len;
...
static void
ldpHelloAdjacencyTable_oid_to_index (struct variable *v, oid name[],
                                     size_t *length,
...
ldpHelloAdjacencyTable_lookup_helper (struct in_addr *entityLdpId,
                                      uint32_t *entityIndex,
                                      struct in_addr *peerLdpId,
                                      uint32_t *adjacencyIndex)
{
...
static struct ctl_adj *
ldpHelloAdjacencyTable_next_helper (int first, struct in_addr *entityLdpId,
                                    uint32_t *entityIndex,
                                    struct in_addr *peerLdpId,
                                    uint32_t *adjacencyIndex)
{
...
static struct ctl_adj *
ldpHelloAdjacencyTable_lookup (struct variable *v, oid name[], size_t *length,
                               int exact, struct in_addr *entityLdpId,
                               uint32_t *entityIndex,
                               struct in_addr *peerLdpId,
                               uint32_t *adjacencyIndex)
{
  ...
  if (exact)
    {
      ...
      ldpHelloAdjacencyTable_oid_to_index (v, name, length, entityLdpId,
  ...
  else
    {
      ...
      hello_adj = ldpHelloAdjacencyTable_next_helper (
          first, entityLdpId, entityIndex, peerLdpId, adjacencyIndex);
    }
  return hello_adj;
}

static uint8_t *
ldpHelloAdjacencyTable (struct variable *v, oid name[], size_t *length,
                        int exact, size_t *var_len, WriteMethod **write_method)
{
  ...
  struct ctl_adj *ctl_adj = ldpHelloAdjacencyTable_lookup (
      v, name, length, exact, &entityLdpId, &entityIndex, &peerLdpId,
      &adjacencyIndex);
...
static void
ldpPeerTable_oid_to_index (struct variable *v, oid name[], size_t *length,
                           struct in_addr *entityLdpId, uint32_t *entityIndex,
                           struct in_addr *peerLdpId)
{
  oid *offset = name + v->namelen;
  int offsetlen = *length - v->namelen;
...
ldpPeerTable_lookup_next (int first, struct in_addr peerLdpId)
{
  struct nbr *nbr = NULL;
...
static struct ctl_nbr *
ldpPeerTable_lookup (struct variable *v, oid name[], size_t *length, int exact,
                     struct in_addr *entityLdpId, uint32_t *entityIndex,
                     struct in_addr *peerLdpId)
{
  struct ctl_nbr *ctl_nbr = NULL;
  ...
  if (exact)
    {
      if (*length <
          (long unsigned int) v->namelen + LDP_PEER_ENTRY_MAX_IDX_LEN)
        return NULL;

      ldpPeerTable_oid_to_index (v, name, length, entityLdpId, entityIndex,
                                 peerLdpId);
...
static uint8_t *
ldpPeerTable (struct variable *v, oid name[], size_t *length, int exact,
              size_t *var_len, WriteMethod **write_method)
{
  struct in_addr entityLdpId = { .s_addr = 0 };
  uint32_t entityIndex = 0;
...
static uint8_t *
ldpSessionTable (struct variable *v, oid name[], size_t *length, int exact,
                 size_t *var_len, WriteMethod **write_method)
{
  struct in_addr entityLdpId = { .s_addr = 0 };
  uint32_t entityIndex = 0;
  ...
  switch (v->magic)
    ...
    case MPLSLDPSESSIONROLE:
      if (ldp_addrcmp (ctl_nbr->af, &ctl_nbr->laddr, &ctl_nbr->raddr) > 0)
...
static uint8_t *
ldpSessionStatsTable (struct variable *v, oid name[], size_t *length,
                      int exact, size_t *var_len, WriteMethod **write_method)
{
  struct in_addr entityLdpId = { .s_addr = 0 };
  uint32_t entityIndex = 0;
  ...
  struct ctl_nbr *ctl_nbr = ldpPeerTable_lookup (
      v, name, length, exact, &entityLdpId, &entityIndex, &peerLdpId);
...
static struct variable ldpe_variables[] = {
  ...
  { MPLS_LDP_LSR_LOOP_DETECTION_CAPABLE,
    INTEGER,
    RONLY,
    ldpLoopDetectCap,
    3,
    { 1, 1, 2 } },
  { MPLS_LDP_ENTITY_LAST_CHANGE,
    TIMESTAMP,
    RONLY,
    ldpEntityLastChange,
    3,
    { 1, 2, 1 } },
  { MPLS_LDP_ENTITY_INDEX_NEXT,
    UNSIGNED32,
    RONLY,
    ldpEntityIndexNext,
    3,
  ...
  /* MPLS LDP mplsLdpEntityTable. */
  { MPLSLDPENTITYLDPID, STRING, RONLY, ldpEntityTable, 5, { 1, 2, 3, 1, 1 } },
  { MPLSLDPENTITYINDEX,
    UNSIGNED32,
    RONLY,
    ldpEntityTable,
  ...
  { MPLSLDPENTITYSTATSSESSIONATTEMPTS,
    ...
    5,
    { 1, 2, 4, 1, 1 } },
  { MPLSLDPENTITYSTATSSESSIONREJHELLO,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 2 } },
  { MPLSLDPENTITYSTATSSESSIONREJAD,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 3 } },
  { MPLSLDPENTITYSTATSSESSIONREJMAXPDU,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 4 } },
  { MPLSLDPENTITYSTATSSESSIONREJLR,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 5 } },
  { MPLSLDPENTITYSTATSBADLDPID,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 6 } },
  { MPLSLDPENTITYSTATSBADPDULENGTH,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 7 } },
  { MPLSLDPENTITYSTATSBADMSGLENGTH,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 8 } },
  { MPLSLDPENTITYSTATSBADTLVLENGTH,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 9 } },
  { MPLSLDPENTITYSTATSMALFORMEDTLV,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 10 } },
  { MPLSLDPENTITYSTATSKEEPALIVEEXP,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 11 } },
  { MPLSLDPENTITYSTATSSHUTDOWNRCVNOTIFY,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
    { 1, 2, 4, 1, 12 } },
  { MPLSLDPENTITYSTATSSHUTDOWNSENTNOTIFY,
    COUNTER32,
    RONLY,
    ldpEntityStatsTable,
    5,
...
ldpTrapSession (struct nbr *nbr, unsigned int sptrap)
{
  ...
  smux_trap (ldpe_variables, array_size (ldpe_variables), ldp_trap_oid,
             ...
             LDP_PEER_ENTRY_MAX_IDX_LEN + 1, ldpSessionTrapList,
             array_size (ldpSessionTrapList), sptrap);
}
...
static int
ldp_snmp_nbr_state_change (struct nbr *nbr, int old_state)
{
...
static int
ldp_snmp_init (struct event_loop *tm)
{
...
static int
ldp_snmp_register_mib (struct event_loop *tm)
{
