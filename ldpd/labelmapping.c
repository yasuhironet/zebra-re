  ...
  while ((me = TAILQ_FIRST (mh)) != NULL)
    {
      ...
      if (CHECK_FLAG (me->map.flags, F_MAP_STATUS))
        SET_FLAG (err,
                  gen_status_tlv (buf, me->map.st.status_code,
  ...
  do
    {
      ...
      if ((tlen = tlv_decode_fec_elm (nbr, &msg, buf, feclen, &map)) == -1)
        goto err;
      if (map.type == MAP_TYPE_PWID && ! CHECK_FLAG (map.flags, F_MAP_PW_ID) &&
          type != MSG_TYPE_LABELWITHDRAW && type != MSG_TYPE_LABELRELEASE)
        {
          send_notification (nbr->tcp, S_MISS_MSG, msg.id, msg.type);
          goto err;
        }
...
len_fec_tlv (struct map *map)
{
  ...
  switch (map->type)
    ...
    case MAP_TYPE_WILDCARD:
      ...
      break;
    case MAP_TYPE_PREFIX:
      len += FEC_ELM_PREFIX_MIN_LEN + PREFIX_SIZE (map->fec.prefix.prefixlen);
      break;
    case MAP_TYPE_PWID:
      ...
      while (pw_len > 0)
        {
          ...
          switch (stlv.type)
            ...
            case SUBTLV_IFMTU:
              ...
              memcpy (&map->fec.pwid.ifmtu, buf + off + SUBTLV_HDR_SIZE,
                      sizeof (uint16_t));
