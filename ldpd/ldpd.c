...
static void merge_af (int, struct ldpd_af_conf *, struct ldpd_af_conf *);
...
static void merge_l2vpn (struct ldpd_conf *, struct l2vpn *, struct l2vpn *);
...
static void
ldp_load_module (const char *name)
{
  const char *dir;
...
/* ldpd privileges */
static zebra_capabilities_t _caps_p[] = { ZCAP_BIND, ZCAP_NET_ADMIN };
...
FRR_DAEMON_INFO (ldpd, LDP, .vty_port = LDP_VTY_PORT,

                 .proghelp = "Implementation of the LDP protocol.",

                 .signals = ldp_signals, .n_signals = array_size (ldp_signals),
...
static void
ldp_config_fork_apply (struct event *t)
{
  /*
...
main (int argc, char *argv[])
{
  ...
  while (1)
    {
      ...
      switch (opt)
        ...
        case OPTION_CTLSOCK:
          ...
          strlcat (ctl_sock_path, ctl_sock_name, sizeof (ctl_sock_path));
          break;
        case 'n':
  ...
  if (lflag || eflag)
    {
      ...
      zlog_init (ldpd_di.progname, "LDP", 0, ids.uid_normal, ids.gid_normal);
    }
  if (lflag)
    lde ();
  else if (eflag)
  ...
  if (socketpair (AF_UNIX, SOCK_STREAM, PF_UNSPEC, pipe_parent2ldpe_sync) ==
      -1)
    fatal ("socketpair");
  ...
  if (socketpair (AF_UNIX, SOCK_STREAM, PF_UNSPEC, pipe_parent2lde_sync) == -1)
    fatal ("socketpair");
  ...
  /* start children */
  lde_pid = start_child (PROC_LDE_ENGINE, saved_argv0, pipe_parent2lde[1],
                         pipe_parent2lde_sync[1]);
  ldpe_pid = start_child (PROC_LDP_ENGINE, saved_argv0, pipe_parent2ldpe[1],
  ...
  event_add_read (master, iev_ldpe->handler_read, iev_ldpe, iev_ldpe->ibuf.fd,
                  &iev_ldpe->ev_read);
  ...
  while (true)
    {
      ...
      if (WIFSIGNALED (status))
        log_warnx ("%s terminated; signal %d",
                   (pid == lde_pid ? "label decision engine" : "ldp engine"),
                   WTERMSIG (status));
...
start_child (enum ldpd_process p, char *argv0, int fd_async, int fd_sync)
{
  ...
  if (nullfd == -1)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL, "%s: failed to open /dev/null: %s",
...
/* ARGSUSED */
static void
main_dispatch_ldpe (struct event *thread)
{
  ...
  for (;;)
    {
      ...
      switch (imsg.hdr.type)
        ...
        case IMSG_REQUEST_SOCKETS:
          ...
          break;
        case IMSG_ACL_CHECK:
          if (imsg.hdr.len != IMSG_HEADER_SIZE + sizeof (struct acl_check))
            fatalx ("IMSG_ACL_CHECK imsg with wrong len");
          ldp_acl_reply (iev, (struct acl_check *) imsg.data);
          break;
        case IMSG_LDP_SYNC_IF_STATE_UPDATE:
          if (imsg.hdr.len !=
              IMSG_HEADER_SIZE + sizeof (struct ldp_igp_sync_if_state))
            fatalx ("IMSG_LDP_SYNC_IF_STATE_UPDATE imsg with wrong len");

          ldp_sync_zebra_send_state_update (
              (struct ldp_igp_sync_if_state *) imsg.data);
          break;
        default:
...
/* ARGSUSED */
static void
main_dispatch_lde (struct event *thread)
{
  struct imsgev *iev = EVENT_ARG (thread);
  struct imsgbuf *ibuf = &iev->ibuf;
  struct imsg imsg;
  ...
  for (;;)
    {
      ...
      switch (imsg.hdr.type)
        ...
        case IMSG_KPW_SET:
        case IMSG_KPW_UNSET:
          if (imsg.hdr.len - IMSG_HEADER_SIZE != sizeof (struct zapi_pw))
        ...
        case IMSG_RLFA_LABELS:
          if (imsg.hdr.len !=
              IMSG_HEADER_SIZE + sizeof (struct zapi_rlfa_response))
            {
...
/* ARGSUSED */
void
ldp_write_handler (struct event *thread)
{
  struct imsgev *iev = EVENT_ARG (thread);
  struct imsgbuf *ibuf = &iev->ibuf;
  ssize_t n;
...
{
  if (eb->wbuf.queued)
    event_add_write (master, eb->handler, eb->arg, eb->wbuf.fd, &eb->ev);
}

void
evbuf_init (struct evbuf *eb, int fd, void (*handler) (struct event *),
            void *arg)
{
  msgbuf_init (&eb->wbuf);
  eb->wbuf.fd = fd;
...
main_imsg_send_ipc_sockets (struct imsgbuf *ldpe_buf, struct imsgbuf *lde_buf)
{
  ...
  if (imsg_compose (ldpe_buf, IMSG_SOCKET_IPC, 0, 0, pipe_ldpe2lde[0], NULL,
                    0) == -1)
    return (-1);

  if (imsg_compose (lde_buf, IMSG_SOCKET_IPC, 0, 0, pipe_ldpe2lde[1], NULL,
                    0) == -1)
    return (-1);
...
main_imsg_send_net_socket (int af, enum socket_type type)
{
  ...
  if (fd == -1)
    {
      log_warnx ("%s: failed to create %s socket for address-family %s",
                 __func__, socket_name (type), af_name (af));
      return;
    }
...
ldp_acl_reply (struct imsgev *iev, struct acl_check *acl_check)
{
  ...
  imsg_compose_event (iev, IMSG_ACL_CHECK, 0, 0, -1, &result, sizeof (result));
}
...
main_imsg_send_config (struct ldpd_conf *xconf)
{
  ...
  if (main_imsg_compose_both (IMSG_RECONF_CONF, xconf, sizeof (*xconf)) == -1)
    return (-1);

  RB_FOREACH (iface, iface_head, &xconf->iface_tree)
    {
      if (main_imsg_compose_both (IMSG_RECONF_IFACE, iface, sizeof (*iface)) ==
          -1)
        return (-1);
    }
  ...
  RB_FOREACH (tnbr, tnbr_head, &xconf->tnbr_tree)
    {
      if (main_imsg_compose_both (IMSG_RECONF_TNBR, tnbr, sizeof (*tnbr)) ==
          -1)
        return (-1);
    }
  ...
  RB_FOREACH (nbrp, nbrp_head, &xconf->nbrp_tree)
    {
      if (main_imsg_compose_both (IMSG_RECONF_NBRP, nbrp, sizeof (*nbrp)) ==
          -1)
        return (-1);
    }
  ...
  RB_FOREACH (l2vpn, l2vpn_head, &xconf->l2vpn_tree)
    {
      if (main_imsg_compose_both (IMSG_RECONF_L2VPN, l2vpn, sizeof (*l2vpn)) ==
          -1)
        return (-1);
  ...
  switch (ldpd_process)
    ...
    case PROC_MAIN:
      if (update_sockets && iev_ldpe)
        imsg_compose_event (iev_ldpe, IMSG_CLOSE_SOCKETS, af, 0, -1, NULL, 0);
      break;
    }
}
  ...
  RB_FOREACH_SAFE (xn, nbrp_head, &xconf->nbrp_tree, ntmp)
    {
...
#else
                  sock_set_md5sig ((ldp_af_global_get (&global, nbr->af))
                                       ->ldp_session_socket,
      ...
      if (nbrp->flags != xn->flags || nbrp->keepalive != xn->keepalive ||
      ...
      nbrp->gtsm_hops = xn->gtsm_hops;
      nbrp->auth.method = xn->auth.method;
      strlcpy (nbrp->auth.md5key, xn->auth.md5key, sizeof (nbrp->auth.md5key));
  ...
  RB_FOREACH_SAFE (xp, l2vpn_pw_head, &xl->pw_tree, ptmp)
    {
      ...
      if (pw->af != xp->af || ldp_addrcmp (pw->af, &pw->addr, &xp->addr))
        reinstall_tnbr = 1;
      else
        reinstall_tnbr = 0;
