      ...
      if (ia && IN_MULTICAST (ntohl (dst->v4.s_addr)))
        {
          ...
          if (sock_set_ipv4_mcast (ia->iface) == -1)
            {
              log_debug ("%s: error setting multicast interface, %s", __func__,
                         ia->iface->name);
              return (-1);
            }
        }
...
/* Discovery functions */
void
disc_recv_packet (struct event *thread)
{
...
#error "Unsupported socket API"
  ...
  if ((pdu_len < (LDP_HDR_PDU_LEN + LDP_MSG_SIZE)) ||
    ...
    {
      log_debug ("%s: invalid LDP packet length %u, source %s", __func__,
  ...
  if (msg_len < LDP_MSG_LEN || ((msg_len + LDP_MSG_DEAD_LEN) > pdu_len))
    {
      log_debug ("%s: invalid LDP message length %u, source %s", __func__,
...
void
session_accept (struct event *thread)
{
  int fd = EVENT_FD (thread);
...
static void
session_read (struct event *thread)
{
  int fd = EVENT_FD (thread);
      ...
      while (len >= LDP_MSG_SIZE)
        {
          ...
          switch (type)
            ...
            case MSG_TYPE_LABELABORTREQ:
              ...
              break;
            default:
              log_debug ("%s: unknown LDP message from nbr %pI4", __func__,
                         &nbr->id);
              if (! (ntohs (msg->type) & UNKNOWN_FLAG))
                {
                  nbr->stats.unknown_msg++;
                  send_notification (nbr->tcp, S_UNKNOWN_MSG, msg->id,
                                     msg->type);
                }
              /* ignore the message */
              ret = 0;
...
static void
session_write (struct event *thread)
{
...
pending_conn_new (int fd, int af, union ldpd_addr *addr)
{
  ...
  event_add_timer (master, pending_conn_timeout, pconn, PENDING_CONN_TIMEOUT,
                   &pconn->ev_timeout);
...
static void
pending_conn_timeout (struct event *thread)
{
