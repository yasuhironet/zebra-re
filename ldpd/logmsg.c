...
log_addr (int af, const union ldpd_addr *addr)
{
  ...
  switch (af)
    ...
    case AF_INET:
      round = (round + 1) % NUM_LOGS;
      if (inet_ntop (AF_INET, &addr->v4, buf[round], sizeof (buf[round])) ==
          NULL)
        return ("???");
      return (buf[round]);
    case AF_INET6:
...
log_time (time_t t)
{
  ...
  if (week > 0)
    snprintfrr (buf, TF_LEN, "%02" PRIu64 "w%01" PRIu64 "d%02" PRIu64 "h",
                week, day, hrs);
  else if (day > 0)
    snprintfrr (buf, TF_LEN, "%01" PRIu64 "d%02" PRIu64 "h%02" PRIu64 "m", day,
                hrs, min);
  else
    snprintfrr (buf, TF_LEN, "%02" PRIu64 ":%02" PRIu64 ":%02" PRIu64, hrs,
                min, sec);
  ...
  switch (map->type)
    ...
    case MAP_TYPE_TYPED_WCARD:
      ...
      switch (map->fec.twcard.type)
        {
        case MAP_TYPE_PREFIX:
          if (snprintf (buf + strlen (buf), sizeof (buf) - strlen (buf),
...
log_fec (const struct fec *fec)
{
  ...
  switch (fec->type)
    ...
    case FEC_TYPE_IPV4:
      ...
      if (snprintf (buf, sizeof (buf), "ipv4 %s/%u", log_addr (AF_INET, &addr),
                    fec->u.ipv4.prefixlen) == -1)
        return ("???");
      break;
    case FEC_TYPE_IPV6:
      ...
      break;
    case FEC_TYPE_PWID:
      if (snprintfrr (buf, sizeof (buf), "pwid %u (%s) - %pI4",
