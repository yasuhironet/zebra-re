  ...
  switch (a->source.type)
    ...
    case HELLO_LINK:
      ...
      if (if_cmp_name_func (a->source.link.ia->iface->name,
                            b->source.link.ia->iface->name) > 0)
        return (1);
      return (ldp_addrcmp (a->source.link.ia->af, &a->source.link.src_addr,
                           &b->source.link.src_addr));
    case HELLO_TARGETED:
      return (ldp_addrcmp (a->source.target->af, &a->source.target->addr,
         ...
         union ldpd_addr *addr)
{
  ...
  log_debug ("%s: lsr-id %pI4, %s", __func__, &lsr_id, log_hello_src (source));
...
/* ARGSUSED */
static void
adj_itimer (struct event *thread)
{
...
/* ARGSUSED */
static void
tnbr_hello_timer (struct event *thread)
{
...
adj_to_ctl (struct adj *adj)
{
  ...
  actl.holdtime_remaining = event_timer_remain_second (adj->inactivity_timer);
