...
void
send_notification_rtlvs (struct nbr *nbr, uint32_t status_code,
                         uint32_t msg_id, uint16_t msg_type, uint16_t tlv_type,
                         uint16_t tlv_len, char *tlv_data)
{
  struct notify_msg nm;
...
recv_notification (struct nbr *nbr, char *buf, uint16_t len)
{
  ...
  while (len > 0)
    {
      ...
      switch (tlv_type)
        ...
        case TLV_TYPE_PW_STATUS:
          ...
          break;
        case TLV_TYPE_FEC:
          if ((tlen = tlv_decode_fec_elm (nbr, &msg, buf, tlv_len, &nm.fec)) ==
              -1)
            return (-1);
          ...
          if (! (ntohs (tlv.type) & UNKNOWN_FLAG))
            {
              ...
              send_notification_rtlvs (nbr, S_UNKNOWN_TLV, msg.id, msg.type,
  ...
  switch (nm.status_code)
    ...
    case S_PW_STATUS:
    case S_ENDOFLIB:
      ldpe_imsg_compose_lde (IMSG_NOTIFICATION, nbr->peerid, 0, &nm,
                             sizeof (nm));
      break;
    case S_NO_HELLO:
      leconf->stats.session_rejects_hello++;
