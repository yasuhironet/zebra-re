      ...
      RB_FOREACH (me, lde_map_head, &fn->upstream)
        {
          ...
          lde_imsg_compose_ldpe (IMSG_CTL_SHOW_LIB_SENT, 0, pid, &rtctl,
                                 sizeof (rtctl));
        }
      ...
      RB_FOREACH (me, lde_map_head, &fn->downstream)
        {
          ...
          lde_imsg_compose_ldpe (IMSG_CTL_SHOW_LIB_RCVD, 0, pid, &rtctl,
                                 sizeof (rtctl));
        }
             ...
             unsigned short route_instance)
{
  ...
  LIST_FOREACH (fnh, &fn->nexthops, entry)
    if (fnh->af == af && ldp_addrcmp (af, &fnh->nexthop, nexthop) == 0 &&
        fnh->ifindex == ifindex && fnh->route_type == route_type &&
...
lde_kernel_insert (struct fec *fec, int af, union ldpd_addr *nexthop,
                   ifindex_t ifindex, uint8_t route_type,
                   unsigned short route_instance, int connected, void *data)
{
  struct fec_node *fn;
  struct fec_nh *fnh;
  struct iface *iface;
  ...
  if (fnh == NULL)
    {
      fnh = fec_nh_add (fn, af, nexthop, ifindex, route_type, route_instance);
      /*
      ...
      if (CHECK_FLAG (ldeconf->flags, F_LDPD_ORDERED_CONTROL) && ! connected &&
          iface != NULL && fec->type != FEC_TYPE_PWID)
 ...
 * withdrawing the old one. So, after processing a
 * ZEBRA_REDISTRIBUTE_IPV[46]_ADD message, we need to check for nexthops that
 * were removed and, for each of them (if any), withdraw the associated labels
 * from zebra.
 */
void
          ...
          if (CHECK_FLAG (ldeconf->flags, F_LDPD_ENABLED))
            {
              ...
              if (CHECK_FLAG (fnh->flags, F_FEC_NH_CONNECTED) || iface ||
  ...
  switch (fec.type)
    ...
    case FEC_TYPE_IPV4:
      ...
      if (lde_acl_check (ldeconf->ipv4.acl_label_accept_for, AF_INET,
                         (union ldpd_addr *) &fec.u.ipv4.prefix,
    ...
    case FEC_TYPE_IPV6:
      ...
      if (lde_acl_check (ldeconf->ipv6.acl_label_accept_for, AF_INET6,
                         (union ldpd_addr *) &fec.u.ipv6.prefix,
...
lde_check_release (struct map *map, struct lde_nbr *ln)
{
  ...
  if (map->type == MAP_TYPE_WILDCARD || map->type == MAP_TYPE_TYPED_WCARD ||
  ...
  switch (wcard->type)
    ...
    case MAP_TYPE_PWID:
      ...
      if (fec->u.pwid.type != wcard->fec.pwid.type)
        return (0);
      if (me == NULL ||
          (me->map.fec.pwid.group_id != wcard->fec.pwid.group_id))
        return (0);
      return (1);
    default:
      fatalx ("lde_wildcard_apply: unexpected fec type");
...
/* ARGSUSED */
void
lde_gc_timer (struct event *thread)
{
  struct fec *fec, *safe;
  struct fec_node *fn;
  int count = 0;
