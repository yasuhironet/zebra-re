...
static inline tree
build_const_char_string (int len, const char *str)
{
...
static inline bool
gimple_clobber_p (gimple s __unused)
{
  return false;
}
...
static inline bool
cgraph_for_node_and_aliases (cgraph_node_ptr node,
...
static inline void
gimple_init_singleton (gimple g __unused)
{
}
#endif
...
static inline const gcond *
as_a_const_gcond (const_gimple stmt)
{
  return stmt;
}

static inline gdebug *
as_a_gdebug (gimple stmt)
{
  return stmt;
}

static inline const gdebug *
...
static inline const gphi *
as_a_const_gphi (const_gimple stmt)
{
  return stmt;
}

static inline greturn *
as_a_greturn (gimple stmt)
{
  return stmt;
}

static inline const greturn *
...
static inline const char *
get_decl_section_name (const_tree decl)
{
...
#define cgraph_create_edge_including_clones(caller, callee, old_call_stmt,    \
                                            call_stmt, count, freq, reason)   \
  (caller)->create_edge_including_clones ((callee), (old_call_stmt),          \
                                          (call_stmt), (count), (reason))
#else
#define cgraph_create_edge(caller, callee, call_stmt, count, freq)            \
...
#define cgraph_create_edge_including_clones(caller, callee, old_call_stmt,    \
  ...
  (caller)->create_edge_including_clones (                                    \
      (callee), (old_call_stmt), (call_stmt), (count), (freq), (reason))
#endif
...
static inline cgraph_node_ptr
cgraph_function_node (cgraph_node_ptr node, enum availability *availability)
{
...
static inline bool
cgraph_only_called_directly_p (cgraph_node_ptr node)
{
...
static inline void
cgraph_call_node_duplication_hooks (cgraph_node_ptr node,
...
static inline void
cgraph_call_edge_duplication_hooks (cgraph_edge *cs1, cgraph_edge *cs2)
{
...
template <>
inline bool
is_a_helper<const ggoto *>::test (const_gimple gs)
{
...
template <>
inline bool
is_a_helper<const greturn *>::test (const_gimple gs)
{
...
#ifdef __cplusplus
static inline void
debug_tree (const_tree t)
{
  debug_tree (CONST_CAST_TREE (t));
}

static inline void
debug_gimple_stmt (const_gimple s)
{
