...
#define ALL_LIST_ELEMENTS(list, node, nextnode, data)                         \
  (node) = listhead (list), ((data) = NULL);                                  \
  (node) != NULL && ((data) = listgetdata (node), (nextnode) = node->next);   \
...
#define frr_with_privs(p) for (int x = 1; x; x--)
#define frr_with_mutex(m) for (int x = 1; x; x--)
...
#define ALL_LSDB(lsdb, lsa)                                                   \
  const struct route_node *iterend = ospf6_lsdb_head (lsdb, 0, 0, 0, &lsa);   \
  lsa;                                                                        \
  lsa = ospf6_lsdb_next (iterend, lsa)
