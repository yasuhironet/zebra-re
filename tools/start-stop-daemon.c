      ...
      while (schedule_str != NULL)
        {
          ...
          str_len =
              slash ? slash - schedule_str : (ptrdiff_t) strlen (schedule_str);
...
parse_options (int argc, char *const *argv)
{
  ...
  for (;;)
    {
      c = getopt_long (
          argc, argv, "HKSVa:n:op:qr:d:s:tu:vx:c:N:bmR:", longopts, (int *) 0);
      if (c == -1)
        break;
      switch (c)
        {
          ...
          for (;;)
            {
              ...
              TVCALC (interval,
                      ratio * (TVELEM (&after) - TVELEM (&before) + TVADJUST));
              TVCALC (maxinterval,
                      TVELEM (&stopat) - TVELEM (&after) + TVADJUST);
