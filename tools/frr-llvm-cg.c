...
static void
dbgloc_add (struct json_object *jsobj, LLVMValueRef obj)
{
  unsigned file_len = 0;
...
js_get_or_make (struct json_object *parent, const char *key,
...
static bool
try_struct_fptr (struct json_object *js_call, LLVMValueRef gep,
                 const char *prefix)
{
...
static void
walk_const_fptrs (struct json_object *js_call, LLVMValueRef value,
  ...
  if (LLVMIsAFunction (value))
    {
      ...
      js_fptrs = js_get_or_make (js_call, "funcptrs", json_object_new_array);
      ...
      json_object_array_add (js_fptrs,
                             json_object_new_string_len (fn_name, fn_len));
      return;
    }
  ...
  switch (kind)
    ...
    case LLVMConstantStructValueKind:
      ...
      for (unsigned i = 0; i < len; i++)
        walk_const_fptrs (js_call, LLVMGetOperand (value, i), prefix,
                          hdr_written);
      break;
    ...
    case LLVMConstantExprValueKind:
      switch (LLVMGetConstOpcode (value))
        ...
        default:
          ...
          if (! *hdr_written)
            {
              fprintf (stderr, "%s: calls function pointer from constexpr\n",
                       prefix);
              *hdr_written = true;
            }
...
#ifdef FRR_SPECIFIC
static bool
is_thread_sched (const char *name, size_t len)
{
#define thread_prefix "_"
  static const char *const names[] = {
    thread_prefix "event_add_read_write", thread_prefix "event_add_timer",
    thread_prefix "event_add_timer_msec", thread_prefix "event_add_timer_tv",
    thread_prefix "event_add_event",      thread_prefix "event_execute",
  };
  size_t i;
...
static bool
_check_val (bool cond, const char *text, LLVMValueRef dumpval)
{
  if (cond)
    return true;
...
static void
handle_yang_module (struct json_object *js_special, LLVMValueRef yang_mod)
{
  ...
  for (unsigned i = 0; i < len - 1; i++)
    {
      ...
      check_val (LLVMGetValueKind (cbs) == LLVMConstantStructValueKind, value);
      ...
      for (unsigned i = 0; i < cblen; i++)
        {
          ...
          if (dbginfo_struct_member (dbginfo, cbs_type, i, &sname, &mname))
            {
              (void) 0;
            }
          ...
          if (LLVMIsAFunction (cb))
            {
              ...
              json_object_object_add (
                  js_cbs, mname, json_object_new_string_len (fn_name, fn_len));
            }
...
static void
handle_daemoninfo (struct json_object *js_special, LLVMValueRef daemoninfo)
{
...
static void
process_call (struct json_object *js_calls, struct json_object *js_special,
              LLVMValueRef instr, LLVMValueRef function)
{
  ...
  if (! called_name || called_len == 0)
    {
      ...
      char prefix[256];
      snprintf (prefix, sizeof (prefix), "%.*s:%d:%.*s()", (int) file_len,
                file, line, (int) name_len, name_c);

      if (LLVMIsALoadInst (called) &&
          LLVMIsAGetElementPtrInst (LLVMGetOperand (called, 0)) &&
          try_struct_fptr (js_call, LLVMGetOperand (called, 0), prefix))
      ...
      if (LLVMIsAAllocaInst (last))
        {
           ...
           * (yet?)
           */
          fprintf (stderr, "%s: call to a function pointer variable\n",
                   prefix);
      ...
      else if (LLVMIsACallInst (last))
        {
          ...
          js_indirect =
              js_get_or_make (js_call, "return_of", json_object_new_array);
                      ...
                      strlen ("_hook_typecheck_")))
    {
      ...
      js_hook = js_get_or_make (js_special, "hooks", json_object_new_object);
      js_hook = js_get_or_make (js_hook, hook_name, json_object_new_array);

      js_this = json_object_new_object ();
      json_object_array_add (js_hook, js_this);

      dbgloc_add (js_this, instr);
      json_object_object_add (js_this, "target",
                              json_object_new_string_len (target, target_len));
      return;
  ...
  for (unsigned argno = 0; argno < n_args; argno++)
    {
      ...
      if (LLVMIsAFunction (param))
        {
          js_fptrs =
              js_get_or_make (js_call, "funcptrs", json_object_new_array);

          target_name = LLVMGetValueName2 (param, &target_len);

          json_object_array_add (
              js_fptrs, json_object_new_string_len (target_name, target_len));
        }
    }
}

static void
process_fn (struct json_object *funcs, struct json_object *js_special,
      ...
      for (LLVMValueRef instr = LLVMGetFirstInstruction (basicBlock); instr;
           instr = LLVMGetNextInstruction (instr))
        {
          ...
          if (LLVMIsACallInst (instr) || LLVMIsAInvokeInst (instr))
            process_call (js_calls, js_special, instr, function);
        }
    }
}
...
main (int argc, char **argv)
{
  ...
  if (! strcmp (inp, "-"))
    ...
    {
      ret = LLVMCreateMemoryBufferWithContentsOfFile (inp, &memoryBuffer,
                                                      &message);
    }
