...
DEFPY (watch_redistribute,
       watch_redistribute_cmd,
       "sharp watch [vrf NAME$vrf_name] redistribute "
       FRR_REDIST_STR_SHARPD,
       ...
       "Should the route be connected\n")
{
  ...
  sharp_zebra_nexthop_watch (&p, vrf->vrf_id, type_import, true,
                             ! ! connected);

  return CMD_SUCCESS;
}

DEFPY (watch_nexthop_v4,
       ...
       "The opaque data\n")
{
  ...
  if (nexthop_group)
    {
      ...
      if (nhgc->backup_list_name[0])
        {
          struct nexthop_group_cmd *bnhgc = nhgc_find (nhgc->backup_list_name);

          if (! bnhgc)
            {
              vty_out (vty, "%% Backup group %s not found for group %s\n",
                       nhgc->backup_list_name, nhgc->name);
              return CMD_WARNING;
            }
  ...
  sharp_install_routes_helper (&prefix, sg.r.vrf_id, sg.r.inst, nhgid,
                               &sg.r.nhop_group, &sg.r.backup_nhop_group, rts,
                               0, sg.r.opaque);
       ...
       "Value of instance\n")
{
  ...
  sg.r.vrf_id = vrf->vrf_id;
  rts = routes;
  sharp_remove_routes_helper (&prefix, sg.r.vrf_id, sg.r.inst, rts);
       ...
       "Instance\n")
{
  ...
  if (sharp_install_lsps_helper (true /*install*/, update_p,
                                 pfx->family > 0 ? &p : NULL, type, instance,
                                 inlabel, &(nhgc->nhg), backup_nhg) == 0)
    return CMD_SUCCESS;
  else
    {
...
DEFPY (sharp_remove_lsp_prefix_v4,
...
static void
sharp_srv6_locator_chunk_free (struct prefix_ipv6 *chunk)
{
       ...
       "SRv6 Locator name\n")
{
  ...
  if (! loc_found)
    {
      loc = XCALLOC (MTYPE_SRV6_LOCATOR, sizeof (struct sharp_srv6_locator));
      loc->chunks = list_new ();
      loc->chunks->del = (void (*) (void *)) sharp_srv6_locator_chunk_free;
       ...
       "Bytes/second (IEEE floating point format)\n")
{
  ...
  for (ALL_LIST_ELEMENTS_RO (path->edges, node, edge))
    {
      if (src4.s_addr != INADDR_ANY)
        vty_out (vty, " %pI4", &edge->attributes->standard.remote);
      else
        vty_out (vty, " %pI6", &edge->attributes->standard.remote6);
    }
  vty_out (vty, "\n");
  cpath_del (path);
  return CMD_SUCCESS;
