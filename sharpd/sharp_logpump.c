...
static void
logpump_done (struct event *thread)
{
  double x;
  ...
  do
    {
      for (size_t inburst = 0; inburst < lp_burst; inburst++)
        zlog_debug ("log pump: %zu (burst %zu)", lp_ctr++, inburst);
...
static int
logpump_halt (struct frr_pthread *fpt, void **res)
{
  return 0;
}
...
void
sharp_logpump_run (struct vty *vty, unsigned duration, unsigned frequency,
                   unsigned burst)
{
  if (lpt != NULL)
    {
