...
static void
add_zclient (struct zclient *client)
{
...
static int
sharp_ifp_create (struct interface *ifp)
{
  return 0;
}

static int
sharp_ifp_destroy (struct interface *ifp)
{
  return 0;
}

static int
interface_address_add (ZAPI_CALLBACK_ARGS)
{
...
static int
interface_address_delete (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
...
static int
sharp_ifp_up (struct interface *ifp)
{
  return 0;
}

static int
sharp_ifp_down (struct interface *ifp)
{
  return 0;
}

int
sharp_install_lsps_helper (bool install_p, bool update_p,
                           const struct prefix *p, uint8_t type, int instance,
                           uint32_t in_label, const struct nexthop_group *nhg,
 ...
 */
static bool
route_add (const struct prefix *p, vrf_id_t vrf_id, uint8_t instance,
      ...
      for (ALL_NEXTHOPS_PTR (nhg, nh))
        {
          ...
          if (nh->nh_label && (nh->nh_label_type == ZEBRA_LSP_EVPN))
 ...
 */
static bool
route_delete (struct prefix *p, vrf_id_t vrf_id, uint8_t instance)
{
  struct zapi_route api;
...
static void
sharp_install_routes_restart (struct prefix *p, uint32_t count,
                              ...
                              uint32_t routes, uint32_t flags, char *opaque)
{
  ...
  for (i = count; i < routes; i++)
    {
      bool buffered = route_add (p, vrf_id, (uint8_t) instance, nhgid, nhg,
                                 backup_nhg, flags, opaque);
      if (v4)
...
void
sharp_install_routes_helper (struct prefix *p, vrf_id_t vrf_id,
                             ...
                             uint32_t routes, uint32_t flags, char *opaque)
{
  ...
  sharp_install_routes_restart (p, 0, vrf_id, instance, nhgid, nhg, backup_nhg,
                                routes, flags, opaque);
}

static void
sharp_remove_routes_restart (struct prefix *p, uint32_t count, vrf_id_t vrf_id,
...
void
sharp_remove_routes_helper (struct prefix *p, vrf_id_t vrf_id,
                            uint8_t instance, uint32_t routes)
{
...
static void
handle_repeated (bool installed)
{
...
static void
sharp_zclient_buffer_ready (void)
{
  switch (wb.restart)
    {
    case SHARP_INSTALL_ROUTES_RESTART:
      sharp_install_routes_restart (&wb.p, wb.count, wb.vrf_id, wb.instance,
                                    wb.nhgid, wb.nhg, wb.backup_nhg, wb.routes,
                                    wb.flags, wb.opaque);
      return;
    case SHARP_DELETE_ROUTES_RESTART:
      sharp_remove_routes_restart (&wb.p, wb.count, wb.vrf_id, wb.instance,
                                   wb.routes);
      return;
    }
}
...
static int
route_notify_owner (ZAPI_CALLBACK_ARGS)
{
  struct timeval r;
  struct prefix p;
      ...
      if (sg.r.total_routes == sg.r.installed_routes)
        {
          ...
          zlog_debug ("Installed All Items %jd.%ld", (intmax_t) r.tv_sec,
                      (long) r.tv_usec);
      ...
      if (sg.r.total_routes == sg.r.removed_routes)
        {
          monotime (&sg.r.t_end);
          timersub (&sg.r.t_end, &sg.r.t_start, &r);
          zlog_debug ("Removed all Items %jd.%ld", (intmax_t) r.tv_sec,
                      (long) r.tv_usec);
...
static void
zebra_connected (struct zclient *zclient)
{
...
void
vrf_label_add (vrf_id_t vrf_id, afi_t afi, mpls_label_t label)
{
  zclient_send_vrf_label (zclient, vrf_id, afi, label, ZEBRA_LSP_SHARP);
}

void
nhg_add (uint32_t id, const struct nexthop_group *nhg,
         const struct nexthop_group *backup_nhg)
{
  ...
  if (api_nhg.nexthop_num == 0)
    {
      zlog_debug ("%s: nhg %u not sent: no valid nexthops", __func__, id);
      is_valid = false;
      goto done;
    }
      ...
      for (ALL_NEXTHOPS_PTR (backup_nhg, nh))
        {
          ...
          if (nh->ifindex == 0)
            {
              zlog_debug ("%s: nhg %u: invalid backup nexthop", __func__, id);
              is_valid = false;
              break;
            }

          api_nh = &api_nhg.backup_nexthops[api_nhg.backup_nexthop_num];
...
void
sharp_zebra_nexthop_watch (struct prefix *p, vrf_id_t vrf_id, bool import,
                           bool watch, bool connected)
{
  ...
  if (zclient_send_rnh (zclient, command, p, SAFI_UNICAST, connected, false,
                        vrf_id) == ZCLIENT_SEND_FAILURE)
    zlog_warn ("%s: Failure to send nexthop to zebra", __func__);
}

static int
sharp_debug_nexthops (struct zapi_route *api)
{
  int i;
      ...
      switch (znh->type)
        ...
        case NEXTHOP_TYPE_IPV4:
          zlog_debug (
              ...
              &znh->gate.ipv4.s_addr, znh->type, znh->ifindex, znh->vrf_id,
              znh->label_num);
          break;
        ...
        case NEXTHOP_TYPE_IPV6:
          zlog_debug (
              ...
              &znh->gate.ipv6, znh->type, znh->ifindex, znh->vrf_id,
              znh->label_num);
          break;
        case NEXTHOP_TYPE_IFINDEX:
          zlog_debug ("        Nexthop IFINDEX: %d, ifindex: %d", znh->type,
                      znh->ifindex);
          break;
...
static int
sharp_nexthop_update (ZAPI_CALLBACK_ARGS)
{
...
static int
sharp_redistribute_route (ZAPI_CALLBACK_ARGS)
{
  struct zapi_route api;
...
void
sharp_redistribute_vrf (struct vrf *vrf, int type)
{
  zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient, AFI_IP, type, 0,
                           vrf->vrf_id);
}
...
int
sharp_zclient_create (uint32_t session_id)
{
  struct zclient *client;
  struct sharp_zclient *node;
  ...
  client =
      zclient_new (master, &zclient_options_default, sharp_opaque_handlers,
...
int
sharp_zclient_delete (uint32_t session_id)
{
  struct sharp_zclient *node;
...
static const char *const type2txt[] = { "Generic", "Vertex", "Edge",
                                        "Subnet" };
static const char *const status2txt[] = { "Unknown", "New",  "Update",
                                          "Delete",  "Sync", "Orphan" };
/* Handler for opaque messages */
static int
sharp_opaque_handler (ZAPI_CALLBACK_ARGS)
{
  struct stream *s;
      ...
      if (lse)
        {
          zlog_debug (" |- Got %s %s from Link State Database",
                      status2txt[lse->status], type2txt[lse->type]);
          lse->status = SYNC;
        }
      else
        zlog_debug ("%s: Error to convert Stream into Link State", __func__);
    }
 ...
 */
void
sharp_opaque_send (uint32_t type, uint32_t proto, uint32_t instance,
  ...
  for (i = 0; i < count; i++)
    {
      if (proto == 0)
        ...
        ret = zclient_send_opaque_unicast (zclient, type, proto, instance,
                                           session_id, buf, sizeof (buf));
      if (ret == ZCLIENT_SEND_FAILURE)
        {
          zlog_debug ("%s: send_opaque() failed => %d", __func__, ret);
          break;
        }
    }
}
 ...
 */
void
sharp_opaque_reg_send (bool is_reg, uint32_t proto, uint32_t instance,
...
void
sharp_zebra_send_arp (const struct interface *ifp, const struct prefix *p)
{
  zclient_send_neigh_discovery_req (zclient, ifp, p);
}

static int
nhg_notify_owner (ZAPI_CALLBACK_ARGS)
{
...
int
sharp_zebra_srv6_manager_get_locator_chunk (const char *locator_name)
{
...
static int
sharp_zebra_process_srv6_locator_chunk (ZAPI_CALLBACK_ARGS)
{
  ...
  for (ALL_LIST_ELEMENTS (sg.srv6_locators, node, nnode, loc))
    {
      ...
      if (strcmp (loc->name, s6c.locator_name) != 0)
        {
          zlog_err ("%s: Locator name unmatch %s:%s", __func__, loc->name,
                    s6c.locator_name);
          continue;
        }
...
int
sharp_zebra_send_interface_protodown (struct interface *ifp, bool down)
{
...
int
sharp_zebra_send_tc_filter_rate (struct interface *ifp,
...
sharp_zebra_init (void)
{
  ...
  zclient =
      zclient_new (master, &opt, sharp_handlers, array_size (sharp_handlers));
