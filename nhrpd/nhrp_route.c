...
nhrp_route_update_get (const struct prefix *p, int create)
{
...
static void
nhrp_route_update_put (struct route_node *rn)
{
  struct route_info *ri = rn->info;

  if (! ri->ifp && ! ri->nhrp_ifp && sockunion_is_null (&ri->via))
    {
...
static void
nhrp_route_update_zebra (const struct prefix *p, union sockunion *nexthop,
                         struct interface *ifp)
{
  struct route_node *rn;
...
static void
nhrp_zebra_register_neigh (vrf_id_t vrf_id, afi_t afi, bool reg)
{
  struct stream *s;
  ...
  zclient_create_header (
      s, reg ? ZEBRA_NHRP_NEIGH_REGISTER : ZEBRA_NHRP_NEIGH_UNREGISTER,
      vrf_id);
  stream_putw (s, afi);
...
void
nhrp_route_update_nhrp (const struct prefix *p, struct interface *ifp)
{
  struct route_node *rn;
  struct route_info *ri;
...
void
nhrp_route_announce (int add, enum nhrp_cache_type type,
                     ...
                     const union sockunion *nexthop, uint32_t mtu)
{
  ...
  switch (api.prefix.family)
    {
    case AF_INET:
      if (api.prefix.prefixlen == IPV4_MAX_BITLEN && nexthop_ref &&
      ...
      break;
    case AF_INET6:
      if (api.prefix.prefixlen == IPV6_MAX_BITLEN && nexthop_ref &&
  ...
  if (unlikely (debug_flags & NHRP_DEBUG_ROUTE))
    {
      ...
      zlog_debug (
          ...
          nexthop_ref
              ? inet_ntop (api.prefix.family, &api_nh->gate, buf, sizeof (buf))
              : "<onlink>",
...
int
nhrp_route_read (ZAPI_CALLBACK_ARGS)
{
  struct zapi_route api;
  struct zapi_nexthop *api_nh;
...
int
nhrp_route_get_nexthop (const union sockunion *addr, struct prefix *p,
                        union sockunion *via, struct interface **ifp)
{
  ...
  if (ri->nhrp_ifp)
    ...
    {
      debugf (NHRP_DEBUG_ROUTE, "lookup %pFX: zebra route dev %s", &lookup,
...
nhrp_route_address (struct interface *in_ifp, union sockunion *addr,
                    struct prefix *p, struct nhrp_peer **peer)
{
...
static void
nhrp_table_node_cleanup (struct route_table *table, struct route_node *node)
{
  if (! node->info)
    return;
...
void
nhrp_send_zebra_configure_arp (struct interface *ifp, int family)
{
  struct stream *s;

  if (! zclient || zclient->sock < 0)
    {
      debugf (NHRP_DEBUG_COMMON, "%s() : zclient not ready", __func__);
      return;
    }
  s = zclient->obuf;
  stream_reset (s);
...
void
nhrp_send_zebra_gre_source_set (struct interface *ifp, unsigned int link_idx,
...
void
nhrp_send_zebra_nbr (union sockunion *in, union sockunion *out,
                     struct interface *ifp)
{
  struct stream *s;
  ...
  s = zclient->obuf;
  stream_reset (s);
  zclient_neigh_ip_encode (
      s, out ? ZEBRA_NEIGH_IP_ADD : ZEBRA_NEIGH_IP_DEL, in, out, ifp,
      out ? ZEBRA_NEIGH_STATE_REACHABLE : ZEBRA_NEIGH_STATE_FAILED);
  stream_putw_at (s, 0, stream_get_endp (s));
  zclient_send_message (zclient);
}

int
nhrp_send_zebra_gre_request (struct interface *ifp)
{
  return zclient_send_zebra_gre_request (zclient, ifp);
}

void
nhrp_zebra_terminate (void)
{
...
int
nhrp_gre_update (ZAPI_CALLBACK_ARGS)
{
  struct stream *s;
  ...
  if (val)
    {
      if (gre_info.vtep_ip.s_addr != val->vtep_ip.s_addr ||
          ...
          gre_info.ikey != val->ikey || gre_info.okey != val->okey)
        {
          /* update */
