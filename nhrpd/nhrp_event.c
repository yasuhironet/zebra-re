...
static void
evmgr_connection_error (struct event_manager *evmgr)
{
...
static void
evmgr_recv_message (struct event_manager *evmgr, struct zbuf *zb)
{
  struct zbuf zl;
  uint32_t eventid = 0;
  size_t len;
  ...
  debugf (NHRP_DEBUG_EVENT, "evmgr: received: eventid=%d result=%s", eventid,
          result);
  if (eventid && result[0])
    {
...
evmgr_write (struct event *t)
{
  ...
  if (r > 0)
    {
      event_add_write (master, evmgr_write, evmgr, evmgr->fd, &evmgr->t_write);
    }
  else if (r < 0)
    {
      evmgr_connection_error (evmgr);
...
static void
evmgr_hexdump (struct zbuf *zb, const uint8_t *val, size_t vallen)
{
...
static void
evmgr_put (struct zbuf *zb, const char *fmt, ...)
{
  ...
  for (pos = fmt; (nxt = strchr (pos, '%')) != NULL; pos = nxt + 2)
    {
      ...
      switch (nxt[1])
        ...
        case '%':
          ...
          break;
        case 'u':
          zb->tail += snprintf ((char *) zb->tail, zbuf_tailroom (zb), "%u",
                                va_arg (va, uint32_t));
          break;
        case 's':
...
static void
evmgr_submit (struct event_manager *evmgr, struct zbuf *obuf)
{
  if (obuf->error)
    {
      zbuf_free (obuf);
...
static void
evmgr_reconnect (struct event *t)
{
  struct event_manager *evmgr = EVENT_ARG (t);
  int fd;
  ...
  if (fd < 0)
    {
      zlog_warn ("%s: failure connecting nhrp-event socket: %s", __func__,
                 strerror (errno));
      zbufq_reset (&evmgr->obuf);
...
void
evmgr_set_socket (const char *socket)
{
  if (nhrp_event_socket_path)
    {
...
void
evmgr_terminate (void)
{
}

void
evmgr_notify (const char *name, struct nhrp_cache *c,
              void (*cb) (struct nhrp_reqid *, void *))
{
  ...
  zb = zbuf_alloc (1024 +
                   (vc ? (vc->local.certlen + vc->remote.certlen) * 2 : 0));
  ...
  if (vc)
    {
      evmgr_put (zb,
                 ...
                 "remote_nbma=%U\n"
                 "remote_cert=%H\n",
                 c->new.peer->requested ? "yes" : "no", &vc->local.nbma,
                 vc->local.cert, vc->local.certlen, &c->remote_addr,
                 &vc->remote.nbma, vc->remote.cert, vc->remote.certlen);
    }
