...
void *
znl_push (struct zbuf *zb, size_t n)
{
  return zbuf_pushn (zb, ZNL_ALIGN (n));
}

void *
znl_pull (struct zbuf *zb, size_t n)
{
  return zbuf_pulln (zb, ZNL_ALIGN (n));
}

struct nlmsghdr *
znl_nlmsg_push (struct zbuf *zb, uint16_t type, uint16_t flags)
{
  struct nlmsghdr *n;
...
void
znl_nlmsg_complete (struct zbuf *zb, struct nlmsghdr *n)
{
  n->nlmsg_len = zb->tail - (uint8_t *) n;
}

struct nlmsghdr *
znl_nlmsg_pull (struct zbuf *zb, struct zbuf *payload)
{
  struct nlmsghdr *n;
  size_t plen;
...
struct rtattr *
znl_rta_push (struct zbuf *zb, uint16_t type, const void *val, size_t len)
{
  struct rtattr *rta;
  uint8_t *dst;
...
struct rtattr *
znl_rta_push_u32 (struct zbuf *zb, uint16_t type, uint32_t val)
{
  return znl_rta_push (zb, type, &val, sizeof (val));
}

struct rtattr *
znl_rta_nested_push (struct zbuf *zb, uint16_t type)
{
  struct rtattr *rta;
...
void
znl_rta_nested_complete (struct zbuf *zb, struct rtattr *rta)
{
...
struct rtattr *
znl_rta_pull (struct zbuf *zb, struct zbuf *payload)
{
  struct rtattr *rta;
  size_t plen;
...
int
znl_open (int protocol, int groups)
{
