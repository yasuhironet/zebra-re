...
nhrp_packet_push (struct zbuf *zb, uint8_t type,
...
struct nhrp_packet_header *
nhrp_packet_pull (struct zbuf *zb, union sockunion *src_nbma,
                  union sockunion *src_proto, union sockunion *dst_proto)
{
  struct nhrp_packet_header *hdr;
  ...
  sockunion_set (src_nbma, afi2family (htons (hdr->afnum)),
                 zbuf_pulln (zb, hdr->src_nbma_address_len +
...
uint16_t
nhrp_packet_calculate_checksum (const uint8_t *pdu, uint16_t len)
{
...
void
nhrp_packet_complete (struct zbuf *zb, struct nhrp_packet_header *hdr)
{
  unsigned short size;
...
nhrp_cie_push (struct zbuf *zb, uint8_t code, const union sockunion *nbma,
...
struct nhrp_cie_header *
nhrp_cie_pull (struct zbuf *zb, struct nhrp_packet_header *hdr,
               union sockunion *nbma, union sockunion *proto)
{
  struct nhrp_cie_header *cie;
...
nhrp_ext_push (struct zbuf *zb, struct nhrp_packet_header *hdr, uint16_t type)
{
  ...
  if (! hdr->extension_offset)
    hdr->extension_offset = htons (zb->tail - (uint8_t *) hdr -
...
void
nhrp_ext_complete (struct zbuf *zb, struct nhrp_extension_header *ext)
{
...
struct nhrp_extension_header *
nhrp_ext_pull (struct zbuf *zb, struct zbuf *payload)
{
  struct nhrp_extension_header *ext;
  uint16_t plen;
...
void
nhrp_ext_request (struct zbuf *zb, struct nhrp_packet_header *hdr,
...
int
nhrp_ext_reply (struct zbuf *zb, struct nhrp_packet_header *hdr,
...
static void
nhrp_packet_recvraw (struct event *t)
{
