...
static unsigned int
nhrp_gre_info_key (const void *data)
{
...
static bool
nhrp_gre_info_cmp (const void *data, const void *key)
{
...
static void *
nhrp_interface_gre_alloc (void *data)
{
...
nhrp_gre_info_alloc (struct nhrp_gre_info *p)
{
  struct nhrp_gre_info *a;
...
static int
nhrp_if_new_hook (struct interface *ifp)
{
...
static int
nhrp_if_delete_hook (struct interface *ifp)
{
...
void
nhrp_interface_init (void)
{
  hook_register_prio (if_add, 0, nhrp_if_new_hook);
  hook_register_prio (if_del, 0, nhrp_if_delete_hook);

  nhrp_gre_list =
      hash_create (nhrp_gre_info_key, nhrp_gre_info_cmp, "NHRP GRE list Hash");
}

void
nhrp_interface_update_mtu (struct interface *ifp, afi_t afi)
{
  struct nhrp_interface *nifp = ifp->info;
...
static void
nhrp_interface_update_source (struct interface *ifp)
{
  struct nhrp_interface *nifp = ifp->info;
...
static void
nhrp_interface_interface_notifier (struct notifier_block *n, unsigned long cmd)
{
  struct nhrp_interface *nifp =
  ...
  switch (cmd)
    ...
    case NOTIFY_INTERFACE_ADDRESS_CHANGED:
      ...
      debugf (NHRP_DEBUG_IF, "%s: NBMA change: address %pSU", nifp->ifp->name,
              &nifp->nbma);
      break;
    }
}
...
void
nhrp_interface_update_nbma (struct interface *ifp,
                            struct nhrp_gre_info *gre_info)
{
  ...
  if (ifp->ll_type != ZEBRA_LLT_IPGRE)
    debugf (NHRP_DEBUG_IF, "%s: Ignoring non GRE interface type %u", __func__,
            ifp->ll_type);
  else
    {
      if (! gre_info)
      ...
      if (nifp->nbmaifp)
        {
          ...
          notifier_del (&nifp->nbmanifp_notifier, &prev_nifp->notifier_list);
        }
      nifp->nbmaifp = nbmaifp;
      if (nbmaifp)
        {
          notifier_add (&nifp->nbmanifp_notifier, &nbmanifp->notifier_list,
...
static void
nhrp_interface_update_address (struct interface *ifp, afi_t afi, int force)
{
  ...
  if (best && if_ad->configured &&
    ...
    {
      zlog_notice ("%s: %pFX is not a host prefix", ifp->name, best->address);
      best = NULL;
    }
...
void
nhrp_interface_update (struct interface *ifp)
{
  struct nhrp_interface *nifp = ifp->info;
  ...
  if (enabled != nifp->enabled)
    {
      ...
      notifier_call (&nifp->notifier_list,
                     enabled ? NOTIFY_INTERFACE_UP : NOTIFY_INTERFACE_DOWN);
    }
}

int
nhrp_ifp_create (struct interface *ifp)
{
  debugf (NHRP_DEBUG_IF, "if-add: %s, ifindex: %u, hw_type: %d %s", ifp->name,
          ifp->ifindex, ifp->ll_type, if_link_type_str (ifp->ll_type));
...
int
nhrp_ifp_destroy (struct interface *ifp)
{
...
static void
interface_config_update_nhrp_map (struct nhrp_cache_config *cc, void *data)
{
  ...
  if (cc->type == NHRP_CACHE_LOCAL)
    nhrp_cache_update_binding (c, NHRP_CACHE_LOCAL, 0, NULL, 0, NULL, NULL);
  else
    {
      nhrp_cache_update_binding (c, NHRP_CACHE_STATIC, 0,
                                 nhrp_peer_get (ifp, &cc->nbma), 0, NULL,
                                 NULL);
    }
}

static void
nhrp_interface_update_cache_config (struct interface *ifp, bool available,
                                    uint8_t family)
{
  struct map_ctx mapctx;

  mapctx = (struct map_ctx){ .family = family, .enabled = available };
  nhrp_cache_config_foreach (ifp, interface_config_update_nhrp_map, &mapctx);
}

int
nhrp_ifp_up (struct interface *ifp)
{
...
int
nhrp_ifp_down (struct interface *ifp)
{
...
int
nhrp_interface_address_add (ZAPI_CALLBACK_ARGS)
{
  ...
  nhrp_interface_update_address (ifc->ifp,
                                 family2afi (PREFIX_FAMILY (ifc->address)), 0);
  nhrp_interface_update_cache_config (ifc->ifp, true,
                                      PREFIX_FAMILY (ifc->address));
  return 0;
}

int
nhrp_interface_address_delete (ZAPI_CALLBACK_ARGS)
{
  struct connected *ifc;
...
void
nhrp_interface_notify_add (struct interface *ifp, struct notifier_block *n,
                           notifier_fn_t fn)
{
  struct nhrp_interface *nifp = ifp->info;
...
void
nhrp_interface_notify_del (struct interface *ifp, struct notifier_block *n)
{
  struct nhrp_interface *nifp = ifp->info;
...
void
nhrp_interface_set_protection (struct interface *ifp, const char *profile,
...
void
nhrp_interface_set_source (struct interface *ifp, const char *ifname)
{
  struct nhrp_interface *nifp = ifp->info;
