...
static void
nhrp_reg_reply (struct nhrp_reqid *reqid, void *arg)
{
  ...
  while ((cie = nhrp_cie_pull (&p->payload, p->hdr, &cie_nbma, &cie_proto)) !=
         NULL)
    {
      proto = sockunion_family (&cie_proto) != AF_UNSPEC ? &cie_proto
                                                         : &p->src_proto;
      debugf (NHRP_DEBUG_COMMON, "NHS: CIE registration: %pSU: %d", proto,
              cie->code);
          ...
          if (nhrp_cie_pull (&extpl, p->hdr, &cie_nbma, &cie_proto) &&
              nhrp_cie_pull (&extpl, p->hdr, &cie_nbma, &cie_proto))
            {
              ...
              debugf (NHRP_DEBUG_IF,
                      "%s: NAT detected, real NBMA address: %pSU", ifp->name,
                      &nifp->nbma);
            }
          break;
  ...
  event_add_timer (master, nhrp_reg_send_req, r, holdtime / 3, &r->t_register);
...
static void
nhrp_reg_timeout (struct event *t)
{
  ...
  if (r->timeout >= 16 && sockunion_family (&r->proto_addr) != AF_UNSPEC)
    {
      ...
      if (c)
        nhrp_cache_update_binding (c, NHRP_CACHE_NHS, -1, NULL, 0, NULL, NULL);
...
static void
nhrp_reg_peer_notify (struct notifier_block *n, unsigned long cmd)
{
  struct nhrp_registration *r =
...
static void
nhrp_reg_send_req (struct event *t)
{
  struct nhrp_registration *r = EVENT_ARG (t);
  struct nhrp_nhs *nhs = r->nhs;
  struct interface *ifp = nhs->ifp;
  ...
  if (! nhrp_peer_check (r->peer, 2))
    {
      debugf (NHRP_DEBUG_COMMON, "NHS: Waiting link for %pSU",
              &r->peer->vc->remote.nbma);
      event_add_timer (master, nhrp_reg_send_req, r, 120, &r->t_register);
      return;
    }

  event_add_timer (master, nhrp_reg_timeout, r, r->timeout, &r->t_register);
  ...
  zb = zbuf_alloc (1400);
  hdr = nhrp_packet_push (zb, NHRP_PACKET_REGISTRATION_REQUEST, &nifp->nbma,
                          &if_ad->addr, dst_proto);
  hdr->hop_count = 1;
  if (! (if_ad->flags & NHRP_IFF_REG_NO_UNIQUE))
    hdr->flags |= htons (NHRP_FLAG_REGISTRATION_UNIQUE);

  hdr->u.request_id =
      htonl (nhrp_reqid_alloc (&nhrp_packet_reqid, &r->reqid, nhrp_reg_reply));
...
static void
nhrp_reg_delete (struct nhrp_registration *r)
{
...
static void
nhrp_nhs_resolve_cb (struct resolver_query *q, const char *errstr, int n,
  ...
  if (n < 0)
    {
      ...
      event_add_timer (master, nhrp_nhs_resolve, nhs, 5, &nhs->t_resolve);
      return;
    }
...
static void
nhrp_nhs_resolve (struct event *t)
{
...
int
nhrp_nhs_add (struct interface *ifp, afi_t afi, union sockunion *proto_addr,
              const char *nbma_fqdn)
{
  ...
  *nhs = (struct nhrp_nhs){
  ...
  event_add_timer_msec (master, nhrp_nhs_resolve, nhs, 1000, &nhs->t_resolve);

  return NHRP_OK;
}

int
nhrp_nhs_del (struct interface *ifp, afi_t afi, union sockunion *proto_addr,
              const char *nbma_fqdn)
{
  struct nhrp_interface *nifp = ifp->info;
  struct nhrp_nhs *nhs;
...
int
nhrp_nhs_free (struct nhrp_interface *nifp, afi_t afi, struct nhrp_nhs *nhs)
{
  struct nhrp_registration *r;
...
void
nhrp_nhs_interface_del (struct interface *ifp)
{
  struct nhrp_interface *nifp = ifp->info;
  struct nhrp_nhs *nhs;
  afi_t afi;
...
void
nhrp_nhs_foreach (struct interface *ifp, afi_t afi,
...
int
nhrp_nhs_match_ip (union sockunion *in_ip, struct nhrp_interface *nifp)
{
  int i;
  struct nhrp_nhs *nhs;
  struct nhrp_registration *reg;
