...
zbuf_size (struct zbuf *zb)
{
  return zb->end - zb->buf;
}

static inline size_t
zbuf_used (struct zbuf *zb)
{
  return zb->tail - zb->head;
}

static inline size_t
zbuf_tailroom (struct zbuf *zb)
{
  return zb->end - zb->tail;
}

static inline size_t
zbuf_headroom (struct zbuf *zb)
{
...
static inline void
zbuf_set_rerror (struct zbuf *zb)
{
  zb->error = 1;
  zb->head = zb->tail;
}
...
static inline void
zbuf_set_werror (struct zbuf *zb)
{
  zb->error = 1;
  zb->tail = zb->end;
}
...
static inline void *
__zbuf_pull (struct zbuf *zb, size_t size, int error)
{
  void *head = zb->head;
...
static inline void
zbuf_get (struct zbuf *zb, void *dst, size_t len)
{
...
static inline uint32_t
zbuf_get_be32 (struct zbuf *zb)
{
  return be32toh (zbuf_get32 (zb));
}

static inline void *
__zbuf_push (struct zbuf *zb, size_t size, int error)
{
  void *tail = zb->tail;
...
#define zbuf_may_push(zb, type)                                               \
  ((type *) __zbuf_may_push (zb, sizeof (type), 0))
#define zbuf_may_pushn(zb, sz) (__zbuf_push (zb, sz, 0))

static inline void
zbuf_put (struct zbuf *zb, const void *src, size_t len)
{
...
static inline void
zbuf_put8 (struct zbuf *zb, uint8_t val)
{
...
static inline void
zbuf_put_be16 (struct zbuf *zb, uint16_t val)
{
  struct unaligned16
  {
    uint16_t value;
...
static inline void
zbuf_put_be32 (struct zbuf *zb, uint32_t val)
{
  struct unaligned32
  {
    uint32_t value;
