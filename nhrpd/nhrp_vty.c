...
static int
nhrp_vty_return (struct vty *vty, int ret)
{
...
static int
toggle_flag (struct vty *vty, const struct message *flag_desc,
...
DEFUN_NOSH (show_debugging_nhrp,
...
static int
nhrp_config_write (struct vty *vty)
{
#ifndef NO_DEBUG
...
static afi_t
cmd_to_afi (const struct cmd_token *tok)
{
  return strcmp (tok->text, "ipv6") == 0 ? AFI_IP6 : AFI_IP;
}

static const char *
afi_to_cmd (afi_t afi)
{
  if (afi == AFI_IP6)
    return "ipv6";
  return "ip";
...
DEFUN (nhrp_multicast_nflog_group,
...
DEFUN (no_nhrp_multicast_nflog_group,
       ...
       "Advertise bound interface MTU similar to OpenNHRP\n")
{
  ...
  if (argv[3]->arg[0] == 'o')
    ...
    {
      nifp->afi[AFI_IP].configured_mtu = strtoul (argv[3]->arg, NULL, 10);
    }
...
DEFUN (if_no_nhrp_map_multicast,
...
static void
show_ip_nhrp_cache (struct nhrp_cache *c, void *pctx)
{
  ...
  if (c->cur.type == NHRP_CACHE_LOCAL)
  ...
  else
    {
      ...
      if (c->cur.peer &&
        ...
        sockunion2str (&c->cur.remote_nbma_claimed, buf[2], sizeof (buf[2]));
      else
        snprintf (buf[2], sizeof (buf[2]), "-");
    }
  ...
  if (ctx->json)
    {
      ...
      json_object_string_add (json, "type", nhrp_cache_type_str[c->cur.type]);
      ...
      if (c->cur.peer)
        json_object_string_add (json, "identity", c->cur.peer->vc->remote.id);
      else
  ...
  vty_out (ctx->vty, "%-8s %-8s %-24s %-24s %-24s %c%c%c    %s\n",
           c->ifp->name, nhrp_cache_type_str[c->cur.type], buf[0], buf[1],
           buf[2], c->used ? 'U' : ' ', c->t_timeout ? 'T' : ' ',
           c->t_auth ? 'A' : ' ',
           c->cur.peer ? c->cur.peer->vc->remote.id : "-");
}

static void
show_ip_nhrp_nhs (struct nhrp_nhs *n, struct nhrp_registration *reg,
                  void *pctx)
{
  struct info_ctx *ctx = pctx;
  struct vty *vty = ctx->vty;
  ...
  if (reg && reg->peer)
    sockunion2str (&reg->peer->vc->remote.nbma, buf[0], sizeof (buf[0]));
  else
...
static void
show_ip_nhrp_shortcut (struct nhrp_shortcut *s, void *pctx)
{
  struct info_ctx *ctx = pctx;
  struct nhrp_cache *c;
  struct vty *vty = ctx->vty;
  ...
  if (ctx->json)
    {
      json = json_object_new_object ();
      json_object_string_add (json, "type", nhrp_cache_type_str[s->type]);
  ...
  vty_out (ctx->vty, "%-8s %-24s %-24s %s\n", nhrp_cache_type_str[s->type],
           buf1, buf2, (c && c->cur.peer) ? c->cur.peer->vc->remote.id : "");
}

static void
show_ip_opennhrp_cache (struct nhrp_cache *c, void *pctx)
{
  struct info_ctx *ctx = pctx;
  char buf[3][SU_ADDRSTRLEN];
  struct json_object *json = NULL;
  ...
  if (sockunion_family (&c->cur.remote_nbma_natoa) != AF_UNSPEC)
    sockunion2str (&c->cur.remote_nbma_natoa, buf[2], sizeof (buf[2]));
  if (ctx->json)
    {
      json = json_object_new_object ();
      json_object_string_add (json, "type", nhrp_cache_type_str[c->cur.type]);
      ...
      if (sockunion_family (&c->cur.remote_nbma_natoa) != AF_UNSPEC)
        json_object_string_add (json, "nbmaNatOaAddress", buf[2]);
...
static void
show_dmvpn_entry (struct nhrp_vc *vc, void *ctx)
{
...
static void
clear_nhrp_cache (struct nhrp_cache *c, void *data)
{
...
static void
clear_nhrp_shortcut (struct nhrp_shortcut *s, void *data)
{
  struct info_ctx *ctx = data;
...
static void
interface_config_write_nhrp_map (struct nhrp_cache_config *c, void *data)
{
