...
static inline void
notifier_init (struct notifier_list *l)
{
  notifier_list_init (&l->head);
}

static inline void
notifier_add (struct notifier_block *n, struct notifier_list *l,
              notifier_fn_t action)
{
  n->action = action;
...
static inline void
notifier_del (struct notifier_block *n, struct notifier_list *l)
{
  notifier_list_del (&l->head, n);
}

static inline void
notifier_call (struct notifier_list *l, int cmd)
{
...
static inline int
notifier_active (struct notifier_list *l)
{
...
void nhrp_send_zebra_nbr (union sockunion *in, union sockunion *out,
...
void nhrp_interface_notify_add (struct interface *ifp,
                                struct notifier_block *n, notifier_fn_t fn);
void nhrp_interface_notify_del (struct interface *ifp,
                                struct notifier_block *n);
...
int nhrp_nhs_add (struct interface *ifp, afi_t afi,
                  union sockunion *proto_addr, const char *nbma_fqdn);
int nhrp_nhs_del (struct interface *ifp, afi_t afi,
                  union sockunion *proto_addr, const char *nbma_fqdn);
int nhrp_nhs_free (struct nhrp_interface *nifp, afi_t afi,
                   struct nhrp_nhs *nhs);
void nhrp_nhs_terminate (void);
void nhrp_nhs_foreach (struct interface *ifp, afi_t afi,
                       void (*cb) (struct nhrp_nhs *,
                                   struct nhrp_registration *, void *),
                       void *ctx);
...
struct nhrp_reqid *nhrp_reqid_lookup (struct nhrp_reqid_pool *,
                                      uint32_t reqid);
...
void nhrp_peer_send_indication (struct interface *ifp, uint16_t,
                                struct zbuf *);
