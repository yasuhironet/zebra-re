...
static unsigned int
nhrp_vc_key (const void *peer_data)
{
...
static bool
nhrp_vc_cmp (const void *cache_data, const void *key_data)
{
...
static void
nhrp_vc_free (void *data)
{
  XFREE (MTYPE_NHRP_VC, data);
}

struct nhrp_vc *
nhrp_vc_get (const union sockunion *src, const union sockunion *dst,
             int create)
{
  struct nhrp_vc key;
  key.local.nbma = *src;
...
static void
nhrp_vc_check_delete (struct nhrp_vc *vc)
{
...
static void
nhrp_vc_update (struct nhrp_vc *vc, long cmd)
{
  vc->updating = 1;
...
static void
nhrp_vc_ipsec_reset (struct nhrp_vc *vc)
{
  vc->local.id[0] = 0;
  vc->local.certlen = 0;
  vc->remote.id[0] = 0;
...
int
nhrp_vc_ipsec_updown (uint32_t child_id, struct nhrp_vc *vc)
{
...
void
nhrp_vc_notify_add (struct nhrp_vc *vc, struct notifier_block *n,
...
void
nhrp_vc_notify_del (struct nhrp_vc *vc, struct notifier_block *n)
{
...
static void
nhrp_vc_iterator (struct hash_bucket *b, void *ctx)
{
...
void
nhrp_vc_foreach (void (*cb) (struct nhrp_vc *, void *), void *ctx)
{
