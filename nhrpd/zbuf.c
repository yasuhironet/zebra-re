...
void
zbuf_init (struct zbuf *zb, void *buf, size_t len, size_t datalen)
{
  *zb = (struct zbuf){
    .buf = buf,
    .end = (uint8_t *) buf + len,
...
void
zbuf_reset_head (struct zbuf *zb, void *ptr)
{
...
static void
zbuf_remove_headroom (struct zbuf *zb)
{
...
ssize_t
zbuf_read (struct zbuf *zb, int fd, size_t maxlen)
{
  ssize_t r;
...
ssize_t
zbuf_write (struct zbuf *zb, int fd)
{
  ssize_t r;
...
ssize_t
zbuf_recv (struct zbuf *zb, int fd)
{
  ssize_t r;
...
ssize_t
zbuf_send (struct zbuf *zb, int fd)
{
  ssize_t r;
...
void *
zbuf_may_pull_until (struct zbuf *zb, const char *sep, struct zbuf *msg)
{
...
void
zbufq_init (struct zbuf_queue *zbq)
{
...
void
zbufq_reset (struct zbuf_queue *zbq)
{
  struct zbuf *buf;
...
void
zbufq_queue (struct zbuf_queue *zbq, struct zbuf *zb)
{
  zbuf_queue_add_tail (&zbq->queue_head, zb);
}

int
zbufq_write (struct zbuf_queue *zbq, int fd)
{
  struct iovec iov[16];
  struct zbuf *zb;
  ssize_t r;
...
void
zbuf_copy (struct zbuf *zdst, struct zbuf *zsrc, size_t len)
{
  const void *src;
  void *dst;
...
void
zbuf_copy_peek (struct zbuf *zdst, struct zbuf *zsrc, size_t len)
{
  const void *src;
  void *dst;
