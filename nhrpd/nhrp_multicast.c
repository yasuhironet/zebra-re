...
static void
nhrp_multicast_send (struct nhrp_peer *p, struct zbuf *zb)
{
  size_t addrlen;
  int ret;
...
static void
nhrp_multicast_forward_nbma (union sockunion *nbma_addr, struct interface *ifp,
                             struct zbuf *pkt)
{
...
static void
nhrp_multicast_forward_cache (struct nhrp_cache *c, void *pctx)
{
  struct mcast_ctx *ctx = (struct mcast_ctx *) pctx;

  if (c->cur.type == NHRP_CACHE_DYNAMIC && c->cur.peer)
    nhrp_multicast_forward_nbma (&c->cur.peer->vc->remote.nbma, ctx->ifp,
                                 ctx->pkt);
}

static void
nhrp_multicast_forward (struct nhrp_multicast *mcast, void *pctx)
{
  struct mcast_ctx *ctx = (struct mcast_ctx *) pctx;
...
static void
netlink_mcast_log_handler (struct nlmsghdr *msg, struct zbuf *zb)
{
  struct nfgenmsg *nf;
  struct rtattr *rta;
  struct zbuf rtapl;
...
static void
netlink_mcast_log_recv (struct event *t)
{
...
static void
netlink_mcast_log_register (int fd, int group)
{
  struct nlmsghdr *n;
  struct nfgenmsg *nf;
...
void
netlink_mcast_set_nflog_group (int nlgroup)
{
...
static int
nhrp_multicast_free (struct interface *ifp, struct nhrp_multicast *mcast)
{
...
int
nhrp_multicast_add (struct interface *ifp, afi_t afi,
...
int
nhrp_multicast_del (struct interface *ifp, afi_t afi,
                    union sockunion *nbma_addr)
{
  struct nhrp_interface *nifp = ifp->info;
  struct nhrp_multicast *mcast;
  ...
  frr_each_safe (nhrp_mcastlist, &nifp->afi[afi].mcastlist_head, mcast)
    {
      ...
      debugf (NHRP_DEBUG_COMMON, "Deleting multicast entry (%pSU)", nbma_addr);
...
void
nhrp_multicast_interface_del (struct interface *ifp)
{
  struct nhrp_interface *nifp = ifp->info;
  struct nhrp_multicast *mcast;
  afi_t afi;
...
void
nhrp_multicast_foreach (struct interface *ifp, afi_t afi,
