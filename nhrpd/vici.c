...
static int
blob_equal (const struct blob *b, const char *str)
{
...
static int
blob2buf (const struct blob *b, char *buf, size_t n)
{
  if (! b || b->len >= (int) n)
    return 0;
...
static void
vici_zbuf_puts (struct zbuf *obuf, const char *str)
{
...
static void
vici_connection_error (struct vici_conn *vici)
{
  nhrp_vc_reset ();
...
static void
vici_parse_message (struct vici_conn *vici, struct zbuf *msg,
  ...
  while ((type = zbuf_may_pull (msg, uint8_t)) != NULL)
    {
      switch (*type)
        ...
        case VICI_SECTION_START:
          ...
          debugf (NHRP_DEBUG_VICI, "VICI: Section start '%.*s'", key.len,
                  key.ptr);
        ...
        case VICI_KEY_VALUE:
          ...
          debugf (NHRP_DEBUG_VICI, "VICI: Key '%.*s'='%.*s'", key.len, key.ptr,
                  val.len, val.ptr);
        ...
        case VICI_LIST_START:
          key.len = zbuf_get8 (msg);
          key.ptr = zbuf_pulln (msg, key.len);
          debugf (NHRP_DEBUG_VICI, "VICI: List start '%.*s'", key.len,
                  key.ptr);
          break;
        case VICI_LIST_ITEM:
          val.len = zbuf_get_be16 (msg);
          val.ptr = zbuf_pulln (msg, val.len);
          debugf (NHRP_DEBUG_VICI, "VICI: List item: '%.*s'", val.len,
                  val.ptr);
          parser (ctx, *type, &key, &val);
          break;
        case VICI_LIST_END:
...
static void
parse_sa_message (struct vici_message_ctx *ctx, enum vici_type_t msgtype,
              ...
              if (vc)
                {
                  blob2buf (&sactx->local.id, vc->local.id,
                            sizeof (vc->local.id));
                  if (blob2buf (&sactx->local.cert, (char *) vc->local.cert,
                                sizeof (vc->local.cert)))
                    vc->local.certlen = sactx->local.cert.len;
                  blob2buf (&sactx->remote.id, vc->remote.id,
                            sizeof (vc->remote.id));
                  if (blob2buf (&sactx->remote.cert, (char *) vc->remote.cert,
                                sizeof (vc->remote.cert)))
                    vc->remote.certlen = sactx->remote.cert.len;
                  sactx->kill_ikesa |=
                      nhrp_vc_ipsec_updown (sactx->child_uniqueid, vc);
      ...
      switch (key->ptr[0])
              ...
              if (blob2buf (val, buf, sizeof (buf)))
                if (str2sockunion (buf, &sactx->local.host) < 0)
                  flog_err (EC_NHRP_SWAN,
                            "VICI: bad strongSwan local-host: %s", buf);
            }
          ...
          if (blob_equal (key, "remote-host") && ctx->nsections == 1)
            {
              if (blob2buf (val, buf, sizeof (buf)))
                if (str2sockunion (buf, &sactx->remote.host) < 0)
                  flog_err (EC_NHRP_SWAN,
                            "VICI: bad strongSwan remote-host: %s", buf);
            }
        ...
        case 's':
          if (blob_equal (key, "state") && ctx->nsections == 3)
            {
              sactx->child_ok =
                  (sactx->event == 0 && (blob_equal (val, "INSTALLED") ||
...
static void
parse_cmd_response (struct vici_message_ctx *ctx, enum vici_type_t msgtype,
                    const struct blob *key, const struct blob *val)
{
  char buf[512];
...
static void
vici_recv_sa (struct vici_conn *vici, struct zbuf *msg, int event)
{
  char buf[32];
  struct handle_sa_ctx ctx = { .event = event, .msgctx.nsections = 0 };
...
static void
vici_recv_message (struct vici_conn *vici, struct zbuf *msg)
{
  uint32_t msglen;
  uint8_t msgtype;
  struct blob name;
...
static void
vici_submit (struct vici_conn *vici, struct zbuf *obuf)
{
  if (vici->fd < 0)
    {
      zbuf_free (obuf);
...
static void
vici_submit_request (struct vici_conn *vici, const char *name, ...)
{
  struct zbuf *obuf;
  uint32_t *hdrlen;
  va_list va;
...
static void
vici_register_event (struct vici_conn *vici, const char *name)
{
  struct zbuf *obuf;
  uint32_t *hdrlen;
  uint8_t namelen;
  ...
  if (! fp)
    {
      if (! vici_charon_not_found)
        {
          flog_err (EC_NHRP_SWAN, "VICI: Failed to retrieve charon file path");
...
vici_init (void)
{
  ...
  event_add_timer_msec (master, vici_reconnect, vici, 10, &vici->t_reconnect);
}
...
void
vici_terminate_vc_by_profile_name (char *profile_name)
{
  struct vici_conn *vici = &vici_connection;
...
void
vici_terminate_vc_by_ike_id (unsigned int ike_id)
{
  struct vici_conn *vici = &vici_connection;
  char ike_id_str[10];
...
void
vici_request_vc (const char *profile, union sockunion *src,
                 union sockunion *dst, int prio)
{
  ...
  vici_submit_request (
      vici, "initiate", VICI_KEY_VALUE, "child", strlen (profile), profile,
      VICI_KEY_VALUE, "timeout", (size_t) 2, "-1", VICI_KEY_VALUE, "async",
      (size_t) 1, "1", VICI_KEY_VALUE, "init-limits", (size_t) 1,
      prio ? "0" : "1", VICI_KEY_VALUE, "my-host", strlen (buf[0]), buf[0],
      VICI_KEY_VALUE, "other-host", strlen (buf[1]), buf[1], VICI_END);
}

int
sock_open_unix (const char *path)
{
  int ret, fd;
