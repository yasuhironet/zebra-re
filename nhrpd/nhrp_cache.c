...
const char *const nhrp_cache_type_str[] = {
  [NHRP_CACHE_INVALID] = "invalid",   [NHRP_CACHE_INCOMPLETE] = "incomplete",
  [NHRP_CACHE_NEGATIVE] = "negative", [NHRP_CACHE_CACHED] = "cached",
  [NHRP_CACHE_DYNAMIC] = "dynamic",   [NHRP_CACHE_NHS] = "nhs",
  [NHRP_CACHE_STATIC] = "static",     [NHRP_CACHE_LOCAL] = "local",
};

static unsigned int
nhrp_cache_protocol_key (const void *peer_data)
{
...
static bool
nhrp_cache_protocol_cmp (const void *cache_data, const void *key_data)
{
...
static void
nhrp_cache_free (struct nhrp_cache *c)
{
...
static unsigned int
nhrp_cache_config_protocol_key (const void *peer_data)
{
...
static bool
nhrp_cache_config_protocol_cmp (const void *cache_data, const void *key_data)
{
...
static void *
nhrp_cache_config_alloc (void *data)
{
...
void
nhrp_cache_config_free (struct nhrp_cache_config *c)
{
  struct nhrp_interface *nifp = c->ifp->info;
...
nhrp_cache_config_get (struct interface *ifp, union sockunion *remote_addr,
                       int create)
{
  ...
  if (! nifp->cache_config_hash)
    {
      nifp->cache_config_hash =
          hash_create (nhrp_cache_config_protocol_key,
                       nhrp_cache_config_protocol_cmp, "NHRP Config Cache");
      if (! nifp->cache_config_hash)
        return NULL;
    }
...
static void
do_nhrp_cache_free (struct hash_bucket *hb,
...
static void
do_nhrp_cache_config_free (struct hash_bucket *hb,
                           void *arg __attribute__ ((__unused__)))
{
...
void
nhrp_cache_interface_del (struct interface *ifp)
{
  struct nhrp_interface *nifp = ifp->info;
  ...
  if (nifp->cache_config_hash)
    {
      hash_iterate (nifp->cache_config_hash, do_nhrp_cache_config_free, NULL);
      hash_free (nifp->cache_config_hash);
    }
}

struct nhrp_cache *
nhrp_cache_get (struct interface *ifp, union sockunion *remote_addr,
                int create)
{
  struct nhrp_interface *nifp = ifp->info;
...
static void
nhrp_cache_do_free (struct event *t)
{
...
static void
nhrp_cache_do_timeout (struct event *t)
{
  struct nhrp_cache *c = EVENT_ARG (t);
...
static void
nhrp_cache_update_route (struct nhrp_cache *c)
{
  struct prefix pfx;
      ...
      if (p)
        {
          ...
          debugf (NHRP_DEBUG_COMMON,
                  ...
                  p->online, p->requested, nifp->ipsec_profile ? 1 : 0);
        }
      else
        debugf (NHRP_DEBUG_COMMON, "cache (peer check failed: no p)");
...
static void
nhrp_cache_peer_notifier (struct notifier_block *n, unsigned long cmd)
{
...
static void
nhrp_cache_reset_new (struct nhrp_cache *c)
{
  EVENT_OFF (c->t_auth);
...
static void
nhrp_cache_update_timers (struct nhrp_cache *c)
{
  EVENT_OFF (c->t_timeout);
    ...
    case NHRP_CACHE_NUM_TYPES:
      if (c->cur.expires)
        event_add_timer (master, nhrp_cache_do_timeout, c,
                         c->cur.expires - monotime (NULL), &c->t_timeout);
      break;
    }
}

static void
nhrp_cache_authorize_binding (struct nhrp_reqid *r, void *arg)
{
      ...
      if (sockunion_family (&c->cur.remote_nbma_natoa) != AF_UNSPEC)
        {
          ...
          if (c->cur.peer)
            netlink_update_binding (c->cur.peer->ifp, &c->remote_addr,
                                    &c->cur.remote_nbma_natoa);
        }
...
static void
nhrp_cache_do_auth_timeout (struct event *t)
{
  struct nhrp_cache *c = EVENT_ARG (t);
  c->t_auth = NULL;
...
static void
nhrp_cache_newpeer_notifier (struct notifier_block *n, unsigned long cmd)
{
      ...
      if (nhrp_peer_check (c->new.peer, 1))
        {
          ...
          event_add_timer (master, nhrp_cache_do_auth_timeout, c, 10,
                           &c->t_auth);
        }
      break;
    case NOTIFY_PEER_DOWN:
...
int
nhrp_cache_update_binding (struct nhrp_cache *c, enum nhrp_cache_type type,
                           int holding_time, struct nhrp_peer *p, uint32_t mtu,
                           union sockunion *nbma_oa,
  ...
  if (c->cur.type == type && c->cur.peer == p && c->cur.mtu == mtu)
  ...
  else
    {
      debugf (
          ...
          c->cur.type, type, (c->cur.peer == p) ? "same" : "diff", c->cur.mtu,
          mtu, buf[0], nbma_oa ? buf[1] : "(NULL)", c->map);
      c->new.type = type;
      c->new.peer = p;
      c->new.mtu = mtu;
      ...
      if (c->new.type == NHRP_CACHE_INVALID ||
      ...
      else
        {
          ...
          event_add_timer (master, nhrp_cache_do_auth_timeout, c, 60,
                           &c->t_auth);
        }
    }
  nhrp_cache_update_timers (c);
...
void
nhrp_cache_set_used (struct nhrp_cache *c, int used)
{
  c->used = used;
  if (c->used)
...
static void
nhrp_cache_iterator (struct hash_bucket *b, void *ctx)
{
...
static void
nhrp_cache_config_iterator (struct hash_bucket *b, void *ctx)
{
...
void
nhrp_cache_foreach (struct interface *ifp,
...
void
nhrp_cache_config_foreach (struct interface *ifp,
                           void (*cb) (struct nhrp_cache_config *, void *),
                           void *ctx)
{
  struct nhrp_interface *nifp = ifp->info;
...
void
nhrp_cache_notify_add (struct nhrp_cache *c, struct notifier_block *n,
                       notifier_fn_t fn)
{
...
void
nhrp_cache_notify_del (struct nhrp_cache *c, struct notifier_block *n)
{
