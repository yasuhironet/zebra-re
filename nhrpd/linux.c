...
int
os_sendmsg (const uint8_t *buf, size_t len, int ifindex, const uint8_t *addr,
...
static int
linux_configure_arp (const char *iface, int on)
{
  struct ifreq ifr;
...
static int
linux_icmp_redirect_off (const char *iface)
{
  char fname[PATH_MAX];
  int fd, ret = -1;

  snprintf (fname, sizeof (fname), "/proc/sys/net/ipv4/conf/%s/send_redirects",
            iface);
...
int
os_configure_dmvpn (unsigned int ifindex, const char *ifname, int af)
{
  int ret = 0;
