...
static void
nhrp_shortcut_check_use (struct nhrp_shortcut *s)
{
  if (s->expiring && s->cache && s->cache->used)
    {
      debugf (NHRP_DEBUG_ROUTE, "Shortcut %pFX used and expiring", s->p);
      nhrp_shortcut_send_resolution_req (s);
    }
}

static void
nhrp_shortcut_do_expire (struct event *t)
{
...
static void
nhrp_shortcut_cache_notify (struct notifier_block *n, unsigned long cmd)
{
      ...
      if (! s->route_installed)
        {
          debugf (NHRP_DEBUG_ROUTE,
                  ...
                  &c->remote_addr, c && c->ifp ? c->ifp->name : "<unk>");
...
static void
nhrp_shortcut_update_binding (struct nhrp_shortcut *s,
                              enum nhrp_cache_type type, struct nhrp_cache *c,
                              int holding_time)
{
  s->type = type;
  if (c != s->cache)
          ...
          if (s->cache->route_installed)
            {
              ...
              debugf (
                  ...
                  &s->cache->cur.remote_nbma_natoa, s->cache->ifp->name);
      ...
      if (! s->cache || ! s->cache->route_installed)
        {
          debugf (NHRP_DEBUG_ROUTE,
                  ...
                  s->cache ? "yes" : "no",
                  s->cache ? (s->cache->route_installed ? "yes" : "no")
                           : "n/a");
...
static void
nhrp_shortcut_delete (struct nhrp_shortcut *s)
{
...
static void
nhrp_shortcut_do_purge (struct event *t)
{
  struct nhrp_shortcut *s = EVENT_ARG (t);
  s->t_timer = NULL;
  nhrp_shortcut_delete (s);
...
static struct nhrp_shortcut *
nhrp_shortcut_get (struct prefix *p)
{
  struct nhrp_shortcut *s;
  struct route_node *rn;
  ...
  if (! rn->info)
    {
      s = rn->info =
          XCALLOC (MTYPE_NHRP_SHORTCUT, sizeof (struct nhrp_shortcut));
...
static void
nhrp_shortcut_recv_resolution_rep (struct nhrp_reqid *reqid, void *arg)
{
  ...
  if (pp->hdr->type != NHRP_PACKET_RESOLUTION_REPLY)
    {
      if (pp->hdr->type == NHRP_PACKET_ERROR_INDICATION &&
          pp->hdr->u.error.code == NHRP_ERROR_PROTOCOL_ADDRESS_UNREACHABLE)
        {
          ...
          nhrp_shortcut_update_binding (s, NHRP_CACHE_NEGATIVE, NULL,
                                        holding_time);
        }
      else
        {
  ...
  proto =
      sockunion_family (&cie_proto) != AF_UNSPEC ? &cie_proto : &pp->dst_proto;
  if (cie->holding_time)
            ...
            do
              {
                ...
                cie_nat = nhrp_cie_pull (&extpl, pp->hdr, &cie_nat_nbma,
                                         &cie_nat_proto);
  ...
  if (sockunion_family (nbma))
    {
      ...
      if (c)
        {
          debugf (NHRP_DEBUG_COMMON, "Shortcut: cache found, update binding");
          nhrp_cache_update_binding (c, NHRP_CACHE_DYNAMIC, holding_time,
                                     nhrp_peer_get (pp->ifp, nbma),
                                     htons (cie->mtu), nbma, &cie_nbma);
        }
      else
        {
          debugf (NHRP_DEBUG_COMMON, "Shortcut: no cache for proto %pSU",
                  proto);
        }
          ...
          if (c_dst)
            {
              ...
              nhrp_cache_update_binding (c_dst, NHRP_CACHE_DYNAMIC,
                                         holding_time,
                                         nhrp_peer_get (pp->ifp, nbma),
                                         htons (cie->mtu), nbma, &cie_nbma);
            }
  ...
  if (c)
    {
      ...
      if (ps)
        {
          ps->addr = s->addr;
          debugf (NHRP_DEBUG_COMMON, "Shortcut: calling update_binding");
          nhrp_shortcut_update_binding (ps, NHRP_CACHE_DYNAMIC, c,
                                        holding_time);
        }
      else
        {
          debugf (NHRP_DEBUG_COMMON, "Shortcut: proto diff but no ps");
        }
    }
  else
    {
      debugf (NHRP_DEBUG_COMMON,
              ...
              c ? "no" : "yes",
              proto && pp && sockunion_same (proto, &pp->dst_proto) ? "yes"
                                                                    : "no");
    }
...
static void
nhrp_shortcut_send_resolution_req (struct nhrp_shortcut *s)
{
  struct zbuf *zb;
  ...
  hdr = nhrp_packet_push (
      ...
      &nifp->afi[family2afi (sockunion_family (&s->addr))].addr, &s->addr);
  ...
  if (sockunion_family (&nifp->nat_nbma) != AF_UNSPEC)
    {
      cie =
          nhrp_cie_push (zb, NHRP_CODE_SUCCESS, &nifp->nat_nbma, &if_ad->addr);
...
void
nhrp_shortcut_initiate (union sockunion *addr)
{
  struct prefix p;
  struct nhrp_shortcut *s;
  ...
  if (s && s->type != NHRP_CACHE_INCOMPLETE)
    {
      s->addr = *addr;
      EVENT_OFF (s->t_timer);
      event_add_timer (master, nhrp_shortcut_do_purge, s, 30, &s->t_timer);
      nhrp_shortcut_send_resolution_req (s);
    }
}

void
nhrp_shortcut_init (void)
{
...
void
nhrp_shortcut_foreach (afi_t afi, void (*cb) (struct nhrp_shortcut *, void *),
                       void *ctx)
{
...
void
nhrp_shortcut_purge (struct nhrp_shortcut *s, int force)
{
  EVENT_OFF (s->t_timer);
  nhrp_reqid_free (&nhrp_packet_reqid, &s->reqid);
  ...
  if (force)
    {
      ...
      event_add_timer_msec (master, nhrp_shortcut_do_purge, s, 5, &s->t_timer);
    }
  else
    {
...
static void
nhrp_shortcut_purge_prefix (struct nhrp_shortcut *s, void *ctx)
{
...
void
nhrp_shortcut_prefix_change (const struct prefix *p, int deleted)
{
