...
static void
nhrp_peer_check_delete (struct nhrp_peer *p)
{
...
static void
nhrp_peer_notify_up (struct event *t)
{
...
static void
__nhrp_peer_check (struct nhrp_peer *p)
{
  struct nhrp_vc *vc = p->vc;
  struct interface *ifp = p->ifp;
  struct nhrp_interface *nifp = ifp->info;
...
static void
nhrp_peer_vc_notify (struct notifier_block *n, unsigned long cmd)
{
...
static void
nhrp_peer_ifp_notify (struct notifier_block *n, unsigned long cmd)
{
...
static unsigned int
nhrp_peer_key (const void *peer_data)
{
...
static bool
nhrp_peer_cmp (const void *cache_data, const void *key_data)
{
...
static void
do_peer_hash_free (void *hb_data)
{
...
void
nhrp_peer_interface_del (struct interface *ifp)
{
  struct nhrp_interface *nifp = ifp->info;
...
struct nhrp_peer *
nhrp_peer_get (struct interface *ifp, const union sockunion *remote_nbma)
{
  struct nhrp_interface *nifp = ifp->info;
  ...
  if (! nifp->peer_hash)
    {
      nifp->peer_hash =
          hash_create (nhrp_peer_key, nhrp_peer_cmp, "NHRP Peer Hash");
      if (! nifp->peer_hash)
        return NULL;
    }
...
struct nhrp_peer *
nhrp_peer_ref (struct nhrp_peer *p)
{
  if (p)
    p->ref++;
  return p;
...
void
nhrp_peer_unref (struct nhrp_peer *p)
{
  if (p)
    {
      p->ref--;
...
static void
nhrp_peer_request_timeout (struct event *t)
{
  struct nhrp_peer *p = EVENT_ARG (t);
  struct nhrp_vc *vc = p->vc;
  struct interface *ifp = p->ifp;
...
static void
nhrp_peer_defer_vici_request (struct event *t)
{
  struct nhrp_peer *p = EVENT_ARG (t);
  struct nhrp_vc *vc = p->vc;
  struct interface *ifp = p->ifp;
...
int
nhrp_peer_check (struct nhrp_peer *p, int establish)
{
  struct nhrp_vc *vc = p->vc;
  struct interface *ifp = p->ifp;
  struct nhrp_interface *nifp = ifp->info;
  ...
  if (p->prio)
  ...
  else
    {
      ...
      event_add_timer_msec (master, nhrp_peer_defer_vici_request, p, r_time_ms,
                            &p->t_timer);
    }
...
void
nhrp_peer_notify_add (struct nhrp_peer *p, struct notifier_block *n,
                      notifier_fn_t fn)
{
...
void
nhrp_peer_notify_del (struct nhrp_peer *p, struct notifier_block *n)
{
...
void
nhrp_peer_send (struct nhrp_peer *p, struct zbuf *zb)
{
  ...
  debugf (NHRP_DEBUG_KERNEL, "PACKET: Send %pSU -> %pSU", &p->vc->local.nbma,
          &p->vc->remote.nbma);
...
static void
nhrp_process_nat_extension (struct nhrp_packet_parser *pp,
                            union sockunion *proto, union sockunion *cie_nbma)
{
              ...
              while (nhrp_cie_pull (&payload, pp->hdr, cie_nbma, &cie_proto))
                {
                  ...
                  if (! sockunion_cmp (proto, &cie_proto))
                    {
                      debugf (NHRP_DEBUG_COMMON,
                              "cie_nbma for proto %pSU is %pSU", proto,
                              cie_nbma);
                      break;
                    }
                }
...
static void
nhrp_handle_resolution_req (struct nhrp_packet_parser *pp)
{
  ...
  while ((cie = nhrp_cie_pull (&pp->payload, pp->hdr, &cie_nbma,
                               &cie_proto)) != NULL)
    {
      ...
      debugf (NHRP_DEBUG_COMMON,
              "shortcut res_rep: parsing CIE with prefixlen=%u", prefix_len);
      if (prefix_len == 0 || prefix_len >= hostprefix_len)
        prefix_len = hostprefix_len;

      if (prefix_len != hostprefix_len &&
          ! (pp->hdr->flags & htons (NHRP_FLAG_REGISTRATION_UNIQUE)))
        {
      ...
      if (sockunion_family (&cie_nbma_nat) == AF_UNSPEC)
        {
          ...
          if (! sockunion_same (&pp->src_nbma, &pp->peer->vc->remote.nbma) &&
      ...
      debugf (NHRP_DEBUG_COMMON,
              "shortcut res_rep: holdtime is %u (if 0, using %u)", holdtime,
              pp->if_ad->holdtime);
      if (! holdtime)
      ...
      if (! c)
        {
          debugf (NHRP_DEBUG_COMMON, "shortcut res_rep: no cache found");
  ...
  hdr->flags = pp->hdr->flags & htons (NHRP_FLAG_RESOLUTION_SOURCE_IS_ROUTER |
  ...
  cie = nhrp_cie_push (zb, NHRP_CODE_SUCCESS, &nifp->nbma, &pp->if_ad->addr);
  cie->holding_time = htons (pp->if_ad->holdtime);
  cie->mtu = htons (pp->if_ad->mtu);
  if (pp->if_ad->network_id && pp->route_type == NHRP_ROUTE_OFF_NBMA)
    cie->prefix_length = pp->route_prefix.prefixlen;
  else
    cie->prefix_length = 8 * sockunion_get_addrlen (&pp->if_ad->addr);
...
static void
nhrp_handle_registration_request (struct nhrp_packet_parser *p)
{
  struct interface *ifp = p->ifp;
  struct zbuf *zb, payload;
  struct nhrp_packet_header *hdr;
  ...
  while ((cie = nhrp_cie_pull (&payload, hdr, &cie_nbma, &cie_proto)) != NULL)
    {
      ...
      if (prefix_len != hostprefix_len &&
          ! (p->hdr->flags & htons (NHRP_FLAG_REGISTRATION_UNIQUE)))
        {
          cie->code = NHRP_CODE_BINDING_NON_UNIQUE;
          continue;
        }
      ...
      proto_addr = (sockunion_family (&cie_proto) == AF_UNSPEC) ? &p->src_proto
                                                                : &cie_proto;
      nbma_addr = (sockunion_family (&cie_nbma) == AF_UNSPEC) ? &p->src_nbma
                                                              : &cie_nbma;
      nbma_natoa = NULL;
      if (natted)
        {
          nbma_natoa =
              (sockunion_family (&p->peer->vc->remote.nbma) == AF_UNSPEC)
                  ? nbma_addr
      ...
      if (! nhrp_cache_update_binding (
              c, NHRP_CACHE_DYNAMIC, holdtime, nhrp_peer_ref (p->peer),
          ...
          if (natted)
            {
              cie = nhrp_cie_push (zb, NHRP_CODE_SUCCESS,
                                   &p->peer->vc->remote.nbma, &p->src_proto);
...
static int
parse_ether_packet (struct zbuf *zb, uint16_t protocol_type,
    ...
    case ETH_P_IP:
      {
        ...
        if (iph)
          {
            if (src)
              sockunion_set (src, AF_INET, (uint8_t *) &iph->saddr,
                             sizeof (iph->saddr));
            if (dst)
              sockunion_set (dst, AF_INET, (uint8_t *) &iph->daddr,
                             sizeof (iph->daddr));
          }
      }
      break;
    case ETH_P_IPV6:
      {
        ...
        if (iph)
          {
            if (src)
              sockunion_set (src, AF_INET6, (uint8_t *) &iph->saddr,
                             sizeof (iph->saddr));
            if (dst)
              sockunion_set (dst, AF_INET6, (uint8_t *) &iph->daddr,
                             sizeof (iph->daddr));
          }
      }
      break;
...
void
nhrp_peer_send_indication (struct interface *ifp, uint16_t protocol_type,
                           struct zbuf *pkt)
{
  union sockunion dst;
  struct zbuf *zb, payload;
...
static void
nhrp_handle_error_ind (struct nhrp_packet_parser *pp)
{
...
static void
nhrp_handle_traffic_ind (struct nhrp_packet_parser *p)
{
  union sockunion dst;
...
static void
nhrp_peer_forward (struct nhrp_peer *p, struct nhrp_packet_parser *pp)
{
  ...
  while ((ext = nhrp_ext_pull (&pp->extensions, &extpl)) != NULL)
    {
      ...
      switch (type)
        ...
        case NHRP_EXTENSION_REVERSE_TRANSIT_NHS:
          ...
          if ((type == NHRP_EXTENSION_REVERSE_TRANSIT_NHS) ==
              (packet_types[hdr->type].type == PACKET_REPLY))
            {
              ...
              while (nhrp_cie_pull (&extpl, pp->hdr, &cie_nbma,
                                    &cie_protocol) != NULL)
                {
        ...
        case NHRP_EXTENSION_NAT_ADDRESS:
          ...
          if (proto)
            {
              debugf (NHRP_DEBUG_COMMON, "Proto is %pSU", proto);
              c = nhrp_cache_get (nifp->ifp, proto, 0);
            }

          if (c)
            {
              debugf (NHRP_DEBUG_COMMON, "c->cur.remote_nbma_natoa is %pSU",
                      &c->cur.remote_nbma_natoa);
              if (sockunion_family (&c->cur.remote_nbma_natoa) != AF_UNSPEC)
                {
                  cie = nhrp_cie_push (zb, NHRP_CODE_SUCCESS,
                                       &c->cur.remote_nbma_natoa, proto);
                  if (! cie)
                    goto err;
                }
            }
            ...
            {
              if (proto)
                debugf (NHRP_DEBUG_COMMON, "No cache entry for proto %pSU",
                        proto);
...
static void
nhrp_packet_debug (struct zbuf *zb, const char *dir)
{
  union sockunion src_nbma, src_proto, dst_proto;
  struct nhrp_packet_header *hdr;
  struct zbuf zhdr;
...
void
nhrp_peer_recv (struct nhrp_peer *p, struct zbuf *zb)
{
  struct nhrp_packet_header *hdr;
  struct nhrp_vc *vc = p->vc;
  struct interface *ifp = p->ifp;
  ...
  target_addr = (packet_types[hdr->type].type == PACKET_REPLY) ? &pp.src_proto
                                                               : &pp.dst_proto;
  ...
  if (sockunion_same (&pp.src_proto, &pp.dst_proto))
    ...
    pp.route_type =
        nhrp_route_address (pp.ifp, target_addr, &pp.route_prefix, &peer);
