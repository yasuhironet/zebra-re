...
static int
static_nht_data_cmp (const struct static_nht_data *nhtd1,
...
static unsigned int
static_nht_data_hash (const struct static_nht_data *nhtd)
{
  unsigned int key = 0;
...
DECLARE_HASH (static_nht_hash, struct static_nht_data, itm,
              static_nht_data_cmp, static_nht_data_hash);
...
static int
static_ifp_create (struct interface *ifp)
{
...
static int
static_ifp_destroy (struct interface *ifp)
{
...
static int
static_ifp_up (struct interface *ifp)
{
...
static int
static_ifp_down (struct interface *ifp)
{
  static_ifindex_update (ifp, false);
...
route_notify_owner (ZAPI_CALLBACK_ARGS)
{
  ...
  switch (note)
    ...
    case ZAPI_ROUTE_FAIL_INSTALL:
      ...
      zlog_warn ("%s: Route %pFX failed to install for table: %u", __func__,
                 &p, table_id);
      break;
    case ZAPI_ROUTE_BETTER_ADMIN_WON:
      static_nht_mark_state (&p, safi, vrf_id, STATIC_NOT_INSTALLED);
      zlog_warn ("%s: Route %pFX over-ridden by better route for table: %u",
                 __func__, &p, table_id);
      break;
    ...
    case ZAPI_ROUTE_REMOVE_FAIL:
      static_nht_mark_state (&p, safi, vrf_id, STATIC_INSTALLED);
      zlog_warn ("%s: Route %pFX failure to remove for table: %u", __func__,
                 &p, table_id);
      break;
    }
...
static int
static_zebra_nexthop_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (nhr.type == ZEBRA_ROUTE_CONNECT)
    {
      if (static_nexthop_is_local (vrf_id, &matched, nhr.prefix.family))
        nhr.nexthop_num = 0;
    }
  ...
  if (nhtd)
    {
      ...
      static_nht_update (NULL, &matched, nhr.nexthop_num, afi, nhr.safi,
                         nhtd->nh_vrf_id);
    }
  else
    zlog_err ("No nhtd?");
...
static void
static_zebra_capabilities (struct zclient_capabilities *cap)
{
...
static bool
static_nht_hash_decref (struct static_nht_data **nhtd_p)
{
...
static bool
static_zebra_nht_get_prefix (const struct static_nexthop *nh, struct prefix *p)
{
  switch (nh->type)
    {
    case STATIC_IFNAME:
...
void
static_zebra_nht_register (struct static_nexthop *nh, bool reg)
{
  ...
  else if (reg)
    {
      ...
      if (nhtd->refcount > 1)
        DEBUGD (&static_dbg_route,
                "Reusing registered nexthop(%pFX) for %pRN %d", &lookup.nh, rn,
                nhtd->nh_num);
    }
  else
    {
      /* !reg && !nh->nh_registered */
      zlog_warn ("trying to unregister nexthop %pFX twice", &lookup.nh);
      return;
    }
  ...
  if (reg)
      ...
      if (nhtd->nh_num)
        {
          ...
          static_nht_update (&rn->p, &nhtd->nh, nhtd->nh_num, afi, si->safi,
                             nh->nh_vrf_id);
          return;
        }
  ...
  else
    {
      ...
      DEBUGD (&static_dbg_route, "Unregistering nexthop(%pFX) for %pRN",
              &lookup.nh, rn);
    }

  if (zclient_send_rnh (zclient, cmd, &lookup.nh, si->safi, false, false,
                        nh->nh_vrf_id) == ZCLIENT_SEND_FAILURE)
    zlog_warn ("%s: Failure to send nexthop %pFX for %pRN to zebra", __func__,
               &lookup.nh, rn);
  else if (reg)
...
extern void
static_zebra_route_add (struct static_path *pn, bool install)
{
  struct route_node *rn = pn->rn;
  ...
  zclient_route_send (install ? ZEBRA_ROUTE_ADD : ZEBRA_ROUTE_DELETE, zclient,
                      &api);
}
...
void
static_zebra_vrf_register (struct vrf *vrf)
{
...
void
static_zebra_vrf_unregister (struct vrf *vrf)
{
  if (vrf->vrf_id == VRF_DEFAULT)
    return;
