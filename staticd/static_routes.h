...
#include "staticd/static_vrf.h"
  ...
  static inline void
  static_get_nh_type (enum static_nh_type stype, char *type, size_t size)
  {
    switch (stype)
      {
      case STATIC_IFNAME:
...
#define NEXTHOP_STR (INET6_ADDRSTRLEN + INTERFACE_NAMSIZ + 25)
  ...
  extern void static_next_hop_bfd_profile (struct static_nexthop *sn,
                                           const char *name);
  extern void static_next_hop_bfd_multi_hop (struct static_nexthop *sn,
                                             bool mhop);

  /** Call this function after zebra client initialization. */
  extern void static_bfd_initialize (struct zclient *zc,
