      ...
      if (src_table)
        {
          ...
          for (src_node = route_top (src_table); src_node;
               src_node = route_next (src_node))
            {
              ...
              frr_each_safe (static_path_list, &src_si->path_list, src_pn)
                {
                  frr_each_safe (static_nexthop_list, &src_pn->nexthop_list,
                                 nh)
                    {
                      static_nexthop_list_del (&src_pn->nexthop_list, nh);
                      XFREE (MTYPE_STATIC_NEXTHOP, nh);
                    }
                  static_path_list_del (&src_si->path_list, src_pn);
...
void
static_install_path (struct static_path *pn)
{
  struct static_nexthop *nh;
...
static void
static_uninstall_path (struct static_path *pn)
{
  if (static_nexthop_list_count (&pn->nexthop_list))
    static_zebra_route_add (pn, true);
  else
...
struct route_node *
static_add_route (afi_t afi, safi_t safi, struct prefix *p,
                  struct prefix_ipv6 *src_p, struct static_vrf *svrf)
{
...
static void
static_del_src_route (struct route_node *rn)
{
  struct static_path *pn;
  struct static_route_info *si;
...
void
static_del_route (struct route_node *rn)
{
  struct static_path *pn;
  struct static_route_info *si;
  struct route_table *src_table;
...
bool
static_add_nexthop_validate (const char *nh_vrf_name, enum static_nh_type type,
                             struct ipaddr *ipaddr)
{
  ...
  switch (type)
    ...
    case STATIC_IPV4_GATEWAY_IFNAME:
      if (if_address_is_local (&ipaddr->ipaddr_v4, AF_INET, vrf->vrf_id))
        return false;
      break;
    case STATIC_IPV6_GATEWAY:
    case STATIC_IPV6_GATEWAY_IFNAME:
      if (if_address_is_local (&ipaddr->ipaddr_v6, AF_INET6, vrf->vrf_id))
        return false;
      break;
    case STATIC_IFNAME:
...
struct static_path *
static_add_path (struct route_node *rn, uint32_t table_id, uint8_t distance)
{
  struct static_path *pn;
  struct static_route_info *si;
...
void
static_del_path (struct static_path *pn)
{
                    ...
                    const char *nh_vrf, uint32_t color)
{
  ...
  frr_each (static_nexthop_list, &pn->nexthop_list, cp)
    {
      if (nh->type == STATIC_IPV4_GATEWAY && cp->type == STATIC_IPV4_GATEWAY)
        {
          if (ntohl (nh->addr.ipv4.s_addr) < ntohl (cp->addr.ipv4.s_addr))
            break;
          if (ntohl (nh->addr.ipv4.s_addr) > ntohl (cp->addr.ipv4.s_addr))
            continue;
        }
...
void
static_install_nexthop (struct static_nexthop *nh)
{
...
void
static_delete_nexthop (struct static_nexthop *nh)
{
  struct static_path *pn = nh->pn;
  struct route_node *rn = pn->rn;
...
static void
static_ifindex_update_nh (struct interface *ifp, bool up,
                          struct route_node *rn, struct static_path *pn,
                          struct static_nexthop *nh, struct static_vrf *svrf,
                          safi_t safi)
{
  if (! nh->ifname[0])
    return;
...
static void
static_ifindex_update_af (struct interface *ifp, bool up, afi_t afi,
                          safi_t safi)
{
          ...
          frr_each (static_path_list, &si->path_list, pn)
            {
              frr_each (static_nexthop_list, &pn->nexthop_list, nh)
                {
                  static_ifindex_update_nh (ifp, up, rn, pn, nh, svrf, safi);
                }
            }
        }
    }
 ...
 */
static void
static_fixup_vrf (struct static_vrf *svrf, struct route_table *stable,
 ...
 */
static void
static_enable_vrf (struct static_vrf *svrf, struct route_table *stable,
                   afi_t afi, safi_t safi)
{
  struct route_node *rn;
  struct static_nexthop *nh;
 ...
 */
void
static_fixup_vrf_ids (struct static_vrf *enable_svrf)
{
  struct route_table *stable;
  struct vrf *vrf;
  afi_t afi;
 ...
 */
static void
static_cleanup_vrf (struct static_vrf *svrf, struct route_table *stable,
                    afi_t afi, safi_t safi)
{
  struct route_node *rn;
  struct static_nexthop *nh;
 ...
 */
static void
static_disable_vrf (struct route_table *stable, afi_t afi, safi_t safi)
{
  struct route_node *rn;
  struct static_nexthop *nh;
  struct static_path *pn;
 ...
 */
void
static_cleanup_vrf_ids (struct static_vrf *disable_svrf)
{
  struct vrf *vrf;
  afi_t afi;
  safi_t safi;
 ...
 */
static void
static_fixup_intf_nh (struct route_table *stable, struct interface *ifp,
                      afi_t afi, safi_t safi)
{
  struct route_node *rn;
  struct static_nexthop *nh;
 ...
 */
void
static_install_intf_nh (struct interface *ifp)
{
  struct route_table *stable;
  struct vrf *vrf;
  afi_t afi;
...
struct stable_info *
static_get_stable_info (struct route_node *rn)
{
...
void
static_get_nh_str (struct static_nexthop *nh, char *nexthop, size_t size)
{
  switch (nh->type)
    {
    case STATIC_IFNAME:
    ...
    case STATIC_IPV4_GATEWAY:
      ...
      break;
    case STATIC_IPV4_GATEWAY_IFNAME:
      snprintfrr (nexthop, size, "ip4-ifindex : %pI4 : %s", &nh->addr.ipv4,
                  nh->ifname);
      break;
    case STATIC_BLACKHOLE:
    ...
    case STATIC_IPV6_GATEWAY:
      ...
      break;
    case STATIC_IPV6_GATEWAY_IFNAME:
      snprintfrr (nexthop, size, "ip6-ifindex : %pI6 : %s", &nh->addr.ipv6,
                  nh->ifname);
      break;
    };
}
