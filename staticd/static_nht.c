...
static void
static_nht_update_path (struct static_path *pn, struct prefix *nhp,
                        uint32_t nh_num, vrf_id_t nh_vrf_id, struct vrf *vrf)
{
  ...
  frr_each (static_nexthop_list, &pn->nexthop_list, nh)
    {
      ...
      if (nhp->family == AF_INET6 &&
          memcmp (&nhp->u.prefix6, &nh->addr.ipv6, IPV6_MAX_BYTELEN) == 0)
        nh->nh_valid = ! ! nh_num;
...
static void
static_nht_update_safi (struct prefix *sp, struct prefix *nhp, uint32_t nh_num,
...
void
static_nht_update (struct prefix *sp, struct prefix *nhp, uint32_t nh_num,
                   afi_t afi, safi_t safi, vrf_id_t nh_vrf_id)
{
  ...
  RB_FOREACH (vrf, vrf_name_head, &vrfs_by_name)
    static_nht_update_safi (sp, nhp, nh_num, afi, safi, vrf, nh_vrf_id);
}

static void
static_nht_reset_start_safi (struct prefix *nhp, afi_t afi, safi_t safi,
...
void
static_nht_reset_start (struct prefix *nhp, afi_t afi, safi_t safi,
                        vrf_id_t nh_vrf_id)
{
  struct vrf *vrf;
...
static void
static_nht_mark_state_safi (struct prefix *sp, afi_t afi, safi_t safi,
                            struct vrf *vrf, enum static_install_states state)
{
  struct static_vrf *svrf;
  struct route_table *stable;
  struct route_node *rn;
...
void
static_nht_mark_state (struct prefix *sp, safi_t safi, vrf_id_t vrf_id,
