...
static int
static_route_nb_run (struct vty *vty, struct static_route_args *args)
{
  int ret;
  struct prefix p, src;
  struct in_addr mask;
  ...
  switch (args->afi)
    ...
    case AFI_IP:
      ...
      if (args->prefix_mask)
        {
          assert (inet_pton (AF_INET, args->prefix_mask, &mask) == 1);
  ...
  if (! args->delete)
    {
      if (args->source)
        ...
        snprintf (ab_xpath, sizeof (ab_xpath),
                  ...
                  "frr-staticd:staticd", "staticd", args->vrf, buf_prefix,
                  yang_afi_safi_value2identity (args->afi, args->safi),
                  table_id, buf_nh_type, args->nexthop_vrf, buf_gate_str,
                  args->interface_name);
      ...
      if (args->source)
        snprintf (xpath_prefix, sizeof (xpath_prefix),
                  FRR_S_ROUTE_SRC_INFO_KEY_XPATH, "frr-staticd:staticd",
                  "staticd", args->vrf, buf_prefix,
                  yang_afi_safi_value2identity (args->afi, args->safi),
                  buf_src_prefix, table_id, distance);
      else
        snprintf (xpath_prefix, sizeof (xpath_prefix),
                  FRR_STATIC_ROUTE_INFO_KEY_XPATH, "frr-staticd:staticd",
                  "staticd", args->vrf, buf_prefix,
                  yang_afi_safi_value2identity (args->afi, args->safi),
                  table_id, distance);
      ...
      snprintf (ab_xpath, sizeof (ab_xpath), FRR_STATIC_ROUTE_NH_KEY_XPATH,
      ...
      if (type == STATIC_BLACKHOLE)
        {
          ...
          strlcat (ab_xpath, FRR_STATIC_ROUTE_NH_BH_XPATH, sizeof (ab_xpath));
          ...
          if (args->flag)
            {
              ...
              nb_cli_enqueue_change (vty, ab_xpath, NB_OP_MODIFY, bh_type);
            }
          else
            {
              nb_cli_enqueue_change (vty, ab_xpath, NB_OP_MODIFY, "null");
            }
        }
      if (type == STATIC_IPV4_GATEWAY_IFNAME ||
          type == STATIC_IPV6_GATEWAY_IFNAME)
        {
          ...
          if (args->onlink)
            ...
            nb_cli_enqueue_change (vty, ab_xpath, NB_OP_MODIFY, "false");
        }
      if (type == STATIC_IPV4_GATEWAY || type == STATIC_IPV6_GATEWAY ||
          ...
          type == STATIC_IPV6_GATEWAY_IFNAME)
        {
          ...
          strlcat (ab_xpath, FRR_STATIC_ROUTE_NH_COLOR_XPATH,
                   sizeof (ab_xpath));
          if (args->color)
            nb_cli_enqueue_change (vty, ab_xpath, NB_OP_MODIFY, args->color);
        }
      if (args->label)
        {
          ...
          nb_cli_enqueue_change (vty, xpath_mpls, NB_OP_DESTROY, NULL);

          ostr = XSTRDUP (MTYPE_TMP, args->label);
          while ((nump = strsep (&ostr, "/")) != NULL)
            {
              snprintf (ab_xpath, sizeof (ab_xpath),
                        FRR_STATIC_ROUTE_NHLB_KEY_XPATH, label_stack_id);
              strlcpy (xpath_label, xpath_mpls, sizeof (xpath_label));
              strlcat (xpath_label, ab_xpath, sizeof (xpath_label));
              nb_cli_enqueue_change (vty, xpath_label, NB_OP_MODIFY, nump);
              label_stack_id++;
            }
      ...
      if (args->source)
        {
          if (args->distance)
            snprintf (ab_xpath, sizeof (ab_xpath),
                      FRR_DEL_S_ROUTE_SRC_NH_KEY_XPATH, "frr-staticd:staticd",
                      "staticd", args->vrf, buf_prefix,
                      yang_afi_safi_value2identity (args->afi, args->safi),
                      buf_src_prefix, table_id, distance, buf_nh_type,
      ...
      else
        {
          if (args->distance)
            snprintf (ab_xpath, sizeof (ab_xpath),
                      FRR_DEL_S_ROUTE_NH_KEY_XPATH, "frr-staticd:staticd",
                      "staticd", args->vrf, buf_prefix,
                      yang_afi_safi_value2identity (args->afi, args->safi),
                      table_id, distance, buf_nh_type, args->nexthop_vrf,
                      buf_gate_str, args->interface_name);
          else
            snprintf (ab_xpath, sizeof (ab_xpath),
                      FRR_DEL_S_ROUTE_NH_KEY_NO_DISTANCE_XPATH,
...
void
static_cli_show (struct vty *vty, const struct lyd_node *dnode,
                 bool show_defaults)
{
  const char *vrf;
...
static int
mpls_label_iter_cb (const struct lyd_node *dnode, void *arg)
{
  ...
  if (yang_dnode_exists (dnode, "./label"))
    {
      if (iter->first)
        ...
        vty_out (iter->vty, "/%s", yang_dnode_get_string (dnode, "./label"));
      iter->first = false;
    }
...
static void
nexthop_cli_show (struct vty *vty, const struct lyd_node *route,
                  ...
                  const struct lyd_node *nexthop, bool show_defaults)
{
  ...
  if (afi == AFI_IP)
    vty_out (vty, "%sip", strmatch (vrf, VRF_DEFAULT_NAME) ? "" : " ");
  else
    vty_out (vty, "%sipv6", strmatch (vrf, VRF_DEFAULT_NAME) ? "" : " ");
  ...
  if (src)
    vty_out (vty, " from %s", yang_dnode_get_string (src, "./src-prefix"));

  nh_type = yang_dnode_get_enum (nexthop, "./nh-type");
  switch (nh_type)
    {
    case STATIC_IFNAME:
      vty_out (vty, " %s", yang_dnode_get_string (nexthop, "./interface"));
      break;
    case STATIC_IPV4_GATEWAY:
    case STATIC_IPV6_GATEWAY:
      vty_out (vty, " %s", yang_dnode_get_string (nexthop, "./gateway"));
      break;
      ...
      if (yang_dnode_get_bool (bfd_dnode, "./multi-hop"))
        {
          ...
          if (yang_dnode_exists (bfd_dnode, "./source"))
            vty_out (vty, " source %s",
                     yang_dnode_get_string (bfd_dnode, "./source"));
        }
      else
        vty_out (vty, " bfd");
...
void
static_nexthop_cli_show (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
...
int
static_nexthop_cli_cmp (const struct lyd_node *dnode1,
                        const struct lyd_node *dnode2)
{
  ...
  switch (nh_type1)
    {
    case STATIC_IFNAME:
      ret = if_cmp_name_func (yang_dnode_get_string (dnode1, "./interface"),
...
int
static_route_list_cli_cmp (const struct lyd_node *dnode1,
                           const struct lyd_node *dnode2)
{
...
int
static_src_list_cli_cmp (const struct lyd_node *dnode1,
                         const struct lyd_node *dnode2)
{
  struct prefix prefix1, prefix2;
...
int
static_path_list_cli_cmp (const struct lyd_node *dnode1,
                          const struct lyd_node *dnode2)
{
