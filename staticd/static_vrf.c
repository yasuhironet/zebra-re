      ...
      for (safi = SAFI_UNICAST; safi <= SAFI_MULTICAST; safi++)
        {
          ...
          info =
              XCALLOC (MTYPE_STATIC_RTABLE_INFO, sizeof (struct stable_info));
          info->svrf = svrf;
          info->afi = afi;
          info->safi = safi;
...
static int
static_vrf_enable (struct vrf *vrf)
{
...
static int
static_vrf_disable (struct vrf *vrf)
{
...
static int
static_vrf_delete (struct vrf *vrf)
{
  struct route_table *table;
  struct static_vrf *svrf;
  safi_t safi;
...
struct route_table *
static_vrf_static_table (afi_t afi, safi_t safi, struct static_vrf *svrf)
{
  if (! svrf)
    return NULL;
...
static int
static_vrf_config_write (struct vty *vty)
{
