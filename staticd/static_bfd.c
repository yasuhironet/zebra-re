 ...
 */
static void
static_next_hop_bfd_change (struct static_nexthop *sn,
                            const struct bfd_session_status *bss)
{
  switch (bss->state)
    ...
    case BSS_DOWN:
      ...
      DEBUGD (&static_dbg_bfd, "%s: next hop is down, remove it from RIB",
              __func__);
      sn->path_down = true;
...
void
static_next_hop_bfd_monitor_enable (struct static_nexthop *sn,
                                    const struct lyd_node *dnode)
{
  ...
  /* Configure the session.*/
  if (family == AF_INET)
    bfd_sess_set_ipv4_addrs (sn->bsp, use_source ? &source.ip._v4_addr : NULL,
                             &sn->addr.ipv4);
  else if (family == AF_INET6)
    bfd_sess_set_ipv6_addrs (sn->bsp, use_source ? &source.ip._v6_addr : NULL,
                             &sn->addr.ipv6);

  bfd_sess_set_interface (sn->bsp, use_interface ? sn->ifname : NULL);

  bfd_sess_set_profile (
      sn->bsp,
      use_profile ? yang_dnode_get_string (dnode, "./profile") : NULL);
...
void
static_next_hop_bfd_monitor_disable (struct static_nexthop *sn)
{
...
void
static_next_hop_bfd_source (struct static_nexthop *sn,
                            const struct ipaddr *source)
{
  ...
  bfd_sess_set_auto_source (sn->bsp, false);
  if (family == AF_INET)
    bfd_sess_set_ipv4_addrs (sn->bsp, &source->ip._v4_addr, &sn->addr.ipv4);
  else if (family == AF_INET6)
    bfd_sess_set_ipv6_addrs (sn->bsp, &source->ip._v6_addr, &sn->addr.ipv6);

  bfd_sess_install (sn->bsp);
}

void
static_next_hop_bfd_auto_source (struct static_nexthop *sn)
{
  if (sn->bsp == NULL)
    return;
...
void
static_next_hop_bfd_multi_hop (struct static_nexthop *sn, bool mhop)
{
  if (sn->bsp == NULL)
    return;
...
void
static_next_hop_bfd_profile (struct static_nexthop *sn, const char *name)
{
  if (sn->bsp == NULL)
    return;
...
void
static_bfd_initialize (struct zclient *zc, struct event_loop *tm)
{
 ...
 */
static void
static_bfd_show_nexthop_json (struct vty *vty, struct json_object *jo,
...
static void
static_bfd_show_path_json (struct vty *vty, struct json_object *jo,
...
static void
static_bfd_show_json (struct vty *vty)
{
...
static void
static_bfd_show_nexthop (struct vty *vty, const struct static_nexthop *sn)
{
...
static void
static_bfd_show_path (struct vty *vty, struct route_table *rt)
{
  struct route_node *rn;
...
void
static_bfd_show (struct vty *vty, bool json)
{
  struct vrf *vrf;
