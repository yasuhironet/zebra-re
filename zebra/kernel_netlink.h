...
#define NL_PKT_BUF_SIZE     8192
   ...
   */
  extern struct rtnexthop *nl_attr_rtnh (struct nlmsghdr *n,
                                         unsigned int maxlen);
  ...
  extern int netlink_parse_info (
      int (*filter) (struct nlmsghdr *, ns_id_t, int), struct nlsock *nl,
      const struct zebra_dplane_info *dp_info, int count, bool startup);
  extern int netlink_talk_filter (struct nlmsghdr *h, ns_id_t ns, int startup);
  extern int netlink_talk (int (*filter) (struct nlmsghdr *, ns_id_t,
                                          int startup),
  ...
  /*
   * Configure size of the batch buffer and sending threshold. If 'unset',
