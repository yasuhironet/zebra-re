...
static int
make_advertisement_packet (struct interface *ifp, struct prefix *p,
                           struct stream *s)
{
...
irdp_send (struct interface *ifp, struct prefix *p, struct stream *s)
{
  ...
  if (irdp->flags & IF_DEBUG_MESSAGES)
    zlog_debug ("IRDP: TX Advert on %s %pFX Holdtime=%d Preference=%d",
                ifp->name, p, irdp->flags & IF_SHUTDOWN ? 0 : irdp->Lifetime,
                get_pref (irdp, p));
...
static void
irdp_advertisement (struct interface *ifp, struct prefix *p)
{
  struct stream *s;
  s = stream_new (128);
...
void
irdp_send_thread (struct event *t_advert)
{
  uint32_t timer, tmp;
...
void
irdp_advert_off (struct interface *ifp)
{
  struct zebra_if *zi = ifp->info;
  struct irdp_interface *irdp = zi->irdp;
  struct listnode *node, *nnode;
...
void
process_solicit (struct interface *ifp)
{
  struct zebra_if *zi = ifp->info;
  struct irdp_interface *irdp = zi->irdp;
  uint32_t timer;
...
static int
irdp_init (struct event_loop *master)
{
  irdp_if_init ();
