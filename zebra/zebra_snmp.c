...
static uint8_t *ipFwTable (struct variable *, oid[], size_t *, int, size_t *,
                           WriteMethod **);
static uint8_t *ipCidrNumber (struct variable *, oid[], size_t *, int,
                              size_t *, WriteMethod **);
...
static uint8_t *
ipFwNumber (struct variable *v, oid objid[], size_t *objid_len, int exact,
...
static uint8_t *
ipCidrNumber (struct variable *v, oid objid[], size_t *objid_len, int exact,
              size_t *val_len, WriteMethod **write_method)
{
  static int result;
  struct route_table *table;
...
static int
in_addr_cmp (uint8_t *p1, uint8_t *p2)
{
  int i;
               ...
               struct route_node **np, struct route_entry **re)
{
  ...
  if (in_addr_cmp ((uint8_t *) &(*re)->nhe->nhg.nexthop->gate.ipv4,
                   (uint8_t *) &re2->nhe->nhg.nexthop->gate.ipv4) <= 0)
    return;
...
get_fwtable_route_node (struct variable *v, oid objid[], size_t *objid_len,
                        int exact, struct route_node **np,
                        struct route_entry **re)
{
  struct in_addr dest;
  struct route_table *table;
  ...
  if (exact)
    {
      ...
      for (*np = route_top (table); *np; *np = route_next (*np))
        {
          if (! in_addr_cmp (&(*np)->p.u.prefix, (uint8_t *) &dest))
            {
              RNODE_FOREACH_RE (*np, *re)
                {
                  if (! in_addr_cmp (
                          (uint8_t *) &(*re)->nhe->nhg.nexthop->gate.ipv4,
                          (uint8_t *) &nexthop))
          ...
          RNODE_FOREACH_RE (np2, re2)
            {
              ...
              if ((policy < policy2) ||
                  ...
                  ((policy == policy2) && (proto == proto2) &&
                   (in_addr_cmp ((uint8_t *) &re2->nhe->nhg.nexthop->gate.ipv4,
                                 (uint8_t *) &nexthop) >= 0)))
                check_replace (np2, re2, np, re);
            }
        }
...
static uint8_t *
ipFwTable (struct variable *v, oid objid[], size_t *objid_len, int exact,
           size_t *val_len, WriteMethod **write_method)
{
...
static uint8_t *
ipCidrTable (struct variable *v, oid objid[], size_t *objid_len, int exact,
             size_t *val_len, WriteMethod **write_method)
{
  if (smux_header_table (v, objid, objid_len, exact, val_len, write_method) ==
      MATCH_FAILED)
...
static int
zebra_snmp_init (struct event_loop *tm)
{
  smux_init (tm);
