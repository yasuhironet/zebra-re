...
static int
zebra_neigh_rb_cmp (const struct zebra_neigh_ent *n1,
...
RB_GENERATE (zebra_neigh_rb_head, zebra_neigh_ent, rb_node,
             zebra_neigh_rb_cmp);

static struct zebra_neigh_ent *
zebra_neigh_find (ifindex_t ifindex, struct ipaddr *ip)
{
...
zebra_neigh_new (ifindex_t ifindex, struct ipaddr *ip, struct ethaddr *mac)
{
  ...
  if (IS_ZEBRA_DEBUG_NEIGH)
    zlog_debug ("zebra neigh new if %d %pIA %pEA", n->ifindex, &n->ip,
                &n->mac);
...
static void
zebra_neigh_pbr_rules_update (struct zebra_neigh_ent *n)
{
...
static void
zebra_neigh_free (struct zebra_neigh_ent *n)
{
  ...
  if (IS_ZEBRA_DEBUG_NEIGH)
    zlog_debug ("zebra neigh free if %d %pIA %pEA", n->ifindex, &n->ip,
                &n->mac);
...
/* kernel neigh del */
void
zebra_neigh_del (struct interface *ifp, struct ipaddr *ip)
{
  struct zebra_neigh_ent *n;

  if (IS_ZEBRA_DEBUG_NEIGH)
    zlog_debug ("zebra neigh del if %s/%d %pIA", ifp->name, ifp->ifindex, ip);
...
/* kernel neigh add */
void
zebra_neigh_add (struct interface *ifp, struct ipaddr *ip, struct ethaddr *mac)
{
  struct zebra_neigh_ent *n;

  if (IS_ZEBRA_DEBUG_NEIGH)
    zlog_debug ("zebra neigh add if %s/%d %pIA %pEA", ifp->name, ifp->ifindex,
                ip, mac);
...
void
zebra_neigh_deref (struct zebra_pbr_rule *rule)
{
  struct zebra_neigh_ent *n = rule->action.neigh;

  if (IS_ZEBRA_DEBUG_NEIGH)
    zlog_debug ("zebra neigh deref if %d %pIA by pbr rule %u", n->ifindex,
                &n->ip, rule->rule.seq);
...
static void
zebra_neigh_read_on_first_ref (void)
{
...
void
zebra_neigh_ref (int ifindex, struct ipaddr *ip, struct zebra_pbr_rule *rule)
{
  struct zebra_neigh_ent *n;

  if (IS_ZEBRA_DEBUG_NEIGH)
    zlog_debug ("zebra neigh ref if %d %pIA by pbr rule %u", ifindex, ip,
                rule->rule.seq);
...
static void
zebra_neigh_show_one (struct vty *vty, struct zebra_neigh_ent *n)
{
...
void
zebra_neigh_show (struct vty *vty)
{
  struct zebra_neigh_ent *n;
