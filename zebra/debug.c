  ...
  if (no)
    {
      ...
      UNSET_FLAG (zebra_debug_dplane_dpdk, ZEBRA_DEBUG_DPLANE_DPDK_DETAIL);
    }
  else
    {
       ...
       "Nexthop Debugging\n")
{
  ...
  if (neigh)
    {
      if (no)
        UNSET_FLAG (zebra_debug_evpn_mh, ZEBRA_DEBUG_EVPN_MH_NEIGH);
      else
