...
static unsigned int
zebra_vxlan_vni_hash_keymake (const void *p)
{
...
static bool
zebra_vxlan_vni_hash_cmp (const void *p1, const void *p2)
{
...
static int
zebra_vxlan_if_vni_walk_callback (struct hash_bucket *bucket, void *ctxt)
{
  int ret;
...
static void
zebra_vxlan_if_vni_iterate_callback (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_vxlan_vni *vni;
  struct zebra_vxlan_if_ctx *ctx;
...
static int
zebra_vxlan_if_del_vni (struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
  vni_t vni;
  struct zebra_if *zif;
  ...
  if (zl3vni)
  ...
  else
    {
      ...
      if (! zevpn)
        {
          zlog_debug ("Failed to locate VNI hash at del, IF %s(%u) VNI %u",
      ...
      if (zebra_evpn_vxlan_del (zevpn))
        {
          flog_err (EC_ZEBRA_VNI_DEL_FAILED,
                    "Failed to del EVPN hash %p, IF %s(%u) VNI %u", zevpn,
...
static int
zebra_vxlan_if_update_vni (struct interface *ifp, struct zebra_vxlan_vni *vnip,
  ...
  if (zl3vni)
    {
      ...
      zl3vni->vid =
          (zl3vni->vid != vnip->access_vlan) ? vnip->access_vlan : zl3vni->vid;
      br_if = zif->brslave_info.br_if;
  ...
  else
    {
      ...
      if (! zevpn)
        {
          zlog_debug ("Failed to find EVPN hash on update, IF %s(%u) VNI %u",
                      ifp->name, ifp->ifindex, vni);
          return -1;
        }
      ...
      zevpn->vid =
          (zevpn->vid != vnip->access_vlan) ? vnip->access_vlan : zevpn->vid;
      br_if = zif->brslave_info.br_if;
...
static int
zebra_vxlan_if_add_vni (struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
  vni_t vni;
  struct zebra_if *zif;
  struct zebra_l2info_vxlan *vxl;
  ...
  if (zl3vni)
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug (
            "Add L3-VNI %u intf %s(%u) VLAN %u local IP %pI4 master %u", vni,
...
static void
zebra_vxlan_if_vni_entry_del (struct zebra_if *zif,
                              struct zebra_vxlan_vni *vni)
{
  if (vni)
    {
...
static int
zebra_vxlan_if_vni_entry_add (struct zebra_if *zif,
                              struct zebra_vxlan_vni *vni)
{
...
static int
zebra_vxlan_if_add_update_vni (struct zebra_if *zif,
                               struct zebra_vxlan_vni *vni, void *ctxt)
{
  ...
  if ((hashcount (ctx->old_vni_table) == 0) ||
    ...
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("vxlan %s adding vni(%d, %d)", zif->ifp->name, vni->vni,
                    vni->access_vlan);
  ...
  if (old_vni->access_vlan != vni->access_vlan)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("vxlan %s updating vni(%d, %d) -> vni(%d, %d)",
                    zif->ifp->name, old_vni->vni, old_vni->access_vlan,
                    vni->vni, vni->access_vlan);
...
static int
zebra_vxlan_if_vni_entry_update_callback (struct zebra_if *zif,
                                          struct zebra_vxlan_vni *vni,
                                          void *ctxt)
{
  struct zebra_vxlan_if_update_ctx *ctx;
...
static int
zebra_vxlan_if_vni_entry_del_callback (struct zebra_if *zif,
                                       struct zebra_vxlan_vni *vni, void *ctxt)
{
...
static int
zebra_vxlan_if_vni_entry_down_callback (struct zebra_if *zif,
                                        struct zebra_vxlan_vni *vni,
                                        void *ctxt)
{
...
static int
zebra_vxlan_if_vni_entry_up_callback (struct zebra_if *zif,
                                      struct zebra_vxlan_vni *vni, void *ctxt)
{
...
static void
zebra_vxlan_if_vni_clean (struct hash_bucket *bucket, void *arg)
{
  struct zebra_if *zif;
  struct zebra_vxlan_vni *vni;
...
{
  return hash_create (zebra_vxlan_vni_hash_keymake, zebra_vxlan_vni_hash_cmp,
                      "Zebra Vxlan VNI Table");
}

void
zebra_vxlan_vni_table_destroy (struct hash *vni_table)
{
  hash_clean_and_free (&vni_table, zebra_vxlan_vni_free);
}

int
zebra_vxlan_if_vni_table_destroy (struct zebra_if *zif)
{
  ...
  if (vni_info->vni_table)
    {
      zebra_vxlan_if_vni_iterate (zif, zebra_vxlan_if_vni_entry_del_callback,
                                  NULL);
...
int
zebra_vxlan_if_vni_table_create (struct zebra_if *zif)
{
  struct zebra_vxlan_vni_info *vni_info;
...
static int
zif_vlanid_vni_walker (struct zebra_if *zif, struct zebra_vxlan_vni *vnip,
                       void *arg)
{
...
struct zebra_vxlan_vni *
zebra_vxlan_if_vlanid_vni_find (struct zebra_if *zif, vlanid_t vid)
{
  struct zebra_vxlan_if_vlan_ctx ctx = {};
...
void
zebra_vxlan_if_vni_iterate (struct zebra_if *zif,
...
void
zebra_vxlan_if_vni_walk (struct zebra_if *zif,
                         int (*func) (struct zebra_if *zif,
                                      struct zebra_vxlan_vni *, void *),
                         void *arg)
{
...
vni_t
zebra_vxlan_if_access_vlan_vni_find (struct zebra_if *zif,
                                     struct interface *br_if)
{
  struct zebra_vxlan_vni *vni = NULL;
...
int
zebra_vxlan_if_vni_table_add_update (struct interface *ifp,
...
int
zebra_vxlan_if_vni_mcast_group_del (struct interface *ifp, vni_t vni_id,
                                    struct in_addr *mcast_group)
{
...
int
zebra_vxlan_if_vni_down (struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
  vni_t vni;
  struct zebra_if *zif;
  struct zebra_l3vni *zl3vni;
  ...
  if (zl3vni)
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("Intf %s(%u) L3-VNI %u is DOWN", ifp->name, ifp->ifindex,
                    vni);
  ...
  else
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("Intf %s(%u) L2-VNI %u is DOWN", ifp->name, ifp->ifindex,
                    vni);
      ...
      if (! zevpn)
        {
          zlog_debug ("Failed to locate VNI hash at DOWN, IF %s(%u) VNI %u",
                      ifp->name, ifp->ifindex, vni);
          return -1;
        }
...
int
zebra_vxlan_if_vni_up (struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
  vni_t vni;
  struct zebra_if *zif;
  struct zebra_evpn *zevpn;
  ...
  if (zl3vni)
  ...
  else
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("Intf %s(%u) L2-VNI %u is UP", ifp->name, ifp->ifindex,
                    vni);
      ...
      if (! zevpn)
        {
          zlog_debug ("Failed to locate EVPN hash at UP, IF %s(%u) VNI %u",
                      ifp->name, ifp->ifindex, vni);
          return -1;
        }
...
int
zebra_vxlan_if_vni_del (struct interface *ifp, vni_t vni)
{
  struct zebra_if *zif;
...
zebra_vxlan_if_del (struct interface *ifp)
{
  ...
  if (IS_ZEBRA_VXLAN_IF_VNI (zif))
    {
      vni_info = VNI_INFO_FROM_ZEBRA_IF (zif);
      zebra_evpn_vl_vxl_deref (vni_info->vni.access_vlan, vni_info->vni.vni,
                               zif);
 ...
 */
int
zebra_vxlan_if_update (struct interface *ifp,
                       struct zebra_vxlan_if_update_ctx *ctx)
{
  struct zebra_if *zif;
  struct zebra_vxlan_vni_info *vni_info;
  ...
  zebra_vxlan_if_vni_iterate (zif, zebra_vxlan_if_vni_entry_update_callback,
                              ctx);
...
int
zebra_vxlan_if_vni_add (struct interface *ifp, struct zebra_vxlan_vni *vni)
{
  struct zebra_if *zif;
  struct zebra_vxlan_vni_info *vni_info;
...
zebra_vxlan_if_add (struct interface *ifp)
{
  ...
  if (IS_ZEBRA_VXLAN_IF_VNI (zif))
    {
      vni_info = VNI_INFO_FROM_ZEBRA_IF (zif);
      zebra_evpn_vl_vxl_ref (vni_info->vni.access_vlan, vni_info->vni.vni,
                             zif);
