 ...
 */
bool
zebra_opaque_handles_msgid (uint16_t id)
{
  bool ret = false;
 ...
 */
uint32_t
zebra_opaque_enqueue_batch (struct stream_fifo *batch)
{
  uint32_t counter = 0;
  struct stream *msg;
 ...
 */
static void
process_messages (struct event *event)
{
 ...
 */
static int
dispatch_opq_messages (struct stream_fifo *msg_fifo)
{
      ...
      for (client = reg->clients; client; client = client->next)
        {
          ...
          if (CHECK_FLAG (info.flags, ZAPI_OPAQUE_FLAG_UNICAST))
            {
              ...
              if (IS_ZEBRA_DEBUG_RECV && IS_ZEBRA_DEBUG_DETAIL)
                zlog_debug ("%s: found matching unicast client %s", __func__,
           ...
           */
          zclient = zserv_acquire_client (client->proto, client->instance,
                                          client->session_id);
          if (zclient)
            {
              if (IS_ZEBRA_DEBUG_SEND && IS_ZEBRA_DEBUG_DETAIL)
                zlog_debug ("%s: sending %s to client %s", __func__,
                            (dup ? "dup" : "msg"),
                            opq_client2str (buf, sizeof (buf), client));
            ...
            {
              if (IS_ZEBRA_DEBUG_RECV && IS_ZEBRA_DEBUG_DETAIL)
                zlog_debug ("%s: type %u: no zclient for %s", __func__,
                            info.type,
                            opq_client2str (buf, sizeof (buf), client));
              /* Registered but gone? */
              if (dup)
 ...
 */
static int
handle_opq_registration (const struct zmsghdr *hdr, struct stream *msg)
{
  int ret = 0;
  ...
  if (reg)
      ...
      if (client)
        {
          ...
          if (IS_ZEBRA_DEBUG_RECV)
            zlog_debug ("%s: duplicate opq reg for client %s", __func__,
                        opq_client2str (buf, sizeof (buf), client));
          goto done;
        }
  ...
  else
    {
      ...
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: client %s registers for new reg %u", __func__,
                    opq_client2str (buf, sizeof (buf), client), info.type);
 ...
 */
static int
handle_opq_unregistration (const struct zmsghdr *hdr, struct stream *msg)
{
  int ret = 0;
  struct zapi_opaque_reg_info info;
  struct opq_client_reg *client;
  ...
  if (reg == NULL)
    {
      ...
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug (
            "%s: unknown client %s/%u/%u unregisters for unknown type %u",
            __func__, zebra_route_string (info.proto), info.instance,
  ...
  if (client == NULL)
    {
      ...
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: unknown client %s/%u/%u unregisters for %u", __func__,
...
static bool
opq_client_match (const struct opq_client_reg *client,
                  const struct zapi_opaque_reg_info *info)
{
...
static void
opq_reg_free (struct opq_msg_reg **reg)
{
  XFREE (MTYPE_OPQ, (*reg));
}
...
static void
opq_client_free (struct opq_client_reg **client)
{
  XFREE (MTYPE_OPQ, (*client));
}

static const char *
opq_client2str (char *buf, size_t buflen, const struct opq_client_reg *client)
{
  char sbuf[20];
...
static uint32_t
registration_hash (const struct opq_msg_reg *reg)
{
  return reg->type;
}
...
static int
registration_compare (const struct opq_msg_reg *reg1,
                      const struct opq_msg_reg *reg2)
{
