...
#ifdef __OpenBSD__
static int
kernel_rtm_add_labels (struct mpls_label_stack *nh_label,
                       struct sockaddr_mpls *smpls)
{
  if (nh_label->num_labels > 1)
    {
      flog_warn (EC_ZEBRA_MAX_LABELS_PUSH,
                 "%s: can't push %u labels at once (maximum is 1)", __func__,
                 nh_label->num_labels);
      return -1;
    }
...
static int
kernel_rtm (int cmd, const struct prefix *p, const struct nexthop_group *ng,
            uint32_t metric)
...
#define SET_IN6_LINKLOCAL_IFINDEX(a, i)                                       \
  ...
  while (0)
      ...
      if (nexthop->nh_label)
        {
          if (kernel_rtm_add_labels (nexthop->nh_label, &smpls) != 0)
            continue;
          smplsp = (union sockunion *) &smpls;
        }
#endif
      error = rtm_write (cmd, &sin_dest, &sin_mask, gate ? &sin_gate : NULL,
          ...
          if (! gate)
          ...
          else
            {
              switch (p->family)
                {
                case AF_INET:
                  inet_ntop (AF_INET, &sin_gate.sin.sin_addr, gate_buf,
                             sizeof (gate_buf));
                  break;

                case AF_INET6:
                  inet_ntop (AF_INET6, &sin_gate.sin6.sin6_addr, gate_buf,
                             sizeof (gate_buf));
                  break;

                default:
                  snprintf (gate_buf, sizeof (gate_buf), "(invalid-af)");
                  break;
                }
            }
        }
      switch (error)
        ...
        case ZEBRA_ERR_NOERROR:
          nexthop_num++;
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("%s: %pFX: successfully did NH %s", __func__, p,
                        gate_buf);
          if (cmd == RTM_ADD)
        ...
        case ZEBRA_ERR_RTEXIST:
          if (cmd != RTM_ADD)
            flog_err (EC_LIB_SYSTEM_CALL,
                      "%s: rtm_write() returned %d for command %d", __func__,
                      error, cmd);
          continue;
        ...
        default:
          flog_err (
              ...
              __func__, p, error, lookup_msg (rtm_type_str, cmd, NULL));
          break;
        }
    } /* for (ALL_NEXTHOPS(...))*/
...
kernel_route_update (struct zebra_dplane_ctx *ctx)
{
...
int
kernel_neigh_register (vrf_id_t vrf_id, struct zserv *client, bool reg)
{
  /* TODO */
  return 0;
}
...
enum zebra_dplane_result
kernel_neigh_update_ctx (struct zebra_dplane_ctx *ctx)
{
  return ZEBRA_DPLANE_REQUEST_SUCCESS;
}
 ...
 */
enum zebra_dplane_result
kernel_mac_update_ctx (struct zebra_dplane_ctx *ctx)
{
  return ZEBRA_DPLANE_REQUEST_SUCCESS;
}
