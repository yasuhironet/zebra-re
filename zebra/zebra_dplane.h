...
#define DPLANE_NETCONF_IFINDEX_DEFAULT -2
  ...
  /*
   * The dataplane context struct is used to exchange info between the main
   * zebra context and the dataplane module(s). If these are two independent
   * pthreads, they cannot share existing global data structures safely.
   */
   ...
   * using a callback function. The caller can supply an optional void* arg
   * also. If the function returns 'true', the context will be dequeued and
   * freed without being processed.
   */
  int dplane_clean_ctx_queue (bool (*context_cb) (struct zebra_dplane_ctx *ctx,
                                                  void *arg),
                              void *val);

  /* Return a dataplane results context block after use; the caller's pointer
   * will be cleared.
   */
  ...
  void dplane_ctx_set_ifname (struct zebra_dplane_ctx *ctx,
                              const char *ifname);
  ifindex_t dplane_ctx_get_ifindex (const struct zebra_dplane_ctx *ctx);
  void dplane_ctx_set_ifindex (struct zebra_dplane_ctx *ctx,
                               ifindex_t ifindex);
  ...
  void dplane_ctx_set_src (struct zebra_dplane_ctx *ctx,
  ...
  void dplane_ctx_set_instance (struct zebra_dplane_ctx *ctx,
                                uint16_t instance);
  ...
  void dplane_ctx_set_distance (struct zebra_dplane_ctx *ctx,
                                uint8_t distance);
  ...
  void dplane_ctx_set_nexthops (struct zebra_dplane_ctx *ctx,
                                struct nexthop *nh);
  ...
  void dplane_ctx_set_lsp_flags (struct zebra_dplane_ctx *ctx, uint32_t flags);
  ...
  struct zebra_nhlfe *dplane_ctx_add_nhlfe (
  ...
  void dplane_ctx_set_intf_metric (struct zebra_dplane_ctx *ctx,
                                   uint32_t metric);
  uint32_t
  dplane_ctx_get_intf_pd_reason_val (const struct zebra_dplane_ctx *ctx);
  void dplane_ctx_set_intf_pd_reason_val (struct zebra_dplane_ctx *ctx,
                                          bool val);
  ...
  unsigned int dplane_ctx_gre_get_mtu (const struct zebra_dplane_ctx *ctx);
   ...
   */
  enum zebra_dplane_result dplane_rem_mac_add (
      const struct interface *ifp, const struct interface *bridge_ifp,
      vlanid_t vid, const struct ethaddr *mac, vni_t vni,
      struct in_addr vtep_ip, bool sticky, uint32_t nhg_id, bool was_static);
  ...
  enum zebra_dplane_result
  dplane_rem_neigh_delete (const struct interface *ifp,
                           const struct ipaddr *ip);
  ...
  /* Configure limit on the number of pending, queued updates. If 'unset',
...
#define DPLANE_PROV_FLAG_THREADED 0x1
  ...
  /* Providers can offer an entry-point for shutdown and cleanup. This is
   * called with 'early' during shutdown, to indicate that the dataplane
   * subsystem is allowing work to move through the providers and finish. When
   * called without 'early', the provider should release all resources (if it
   * has any allocated).
   */
  int dplane_provider_register (
      const char *name, enum dplane_provider_prio prio, int flags,
      int (*start_fp) (struct zebra_dplane_provider *),
      int (*fp) (struct zebra_dplane_provider *),
      int (*fini_fp) (struct zebra_dplane_provider *, bool early), void *data,
  ...
  /*
   * Initialize the dataplane modules at zebra startup. This is currently
   * called by the rib module. Zebra registers a results callback with the
   * dataplane. The callback is called in the dataplane pthread context, so the
   * expectation is that the contexts are queued for the zebra main pthread.
   */
