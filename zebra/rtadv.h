...
#define ND_OPT_DNSSL 31
  ...
  struct nd_opt_rdnss
  { /* Recursive DNS server option [RFC8106 5.1] */
  ...
  struct nd_opt_dnssl
  { /* DNS search list option [RFC8106 5.2] */
  ...
  /*
   * ipv6 nd prefixes can be manually defined, derived from the kernel
   * interface configs or both.  If both, manual flag/timer settings are used.
   */
  ...
  extern void rtadv_add_prefix (struct zebra_if *zif,
                                const struct prefix_ipv6 *p);
  extern void rtadv_delete_prefix (struct zebra_if *zif,
