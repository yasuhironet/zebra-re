...
static bool
is_route_v4_over_v6 (unsigned char rtm_family,
                     enum nexthop_types_t nexthop_type)
{
  if (rtm_family == AF_INET && (nexthop_type == NEXTHOP_TYPE_IPV6 ||
...
static bool
kernel_nexthops_supported (void)
{
...
static bool
is_proto_nhg (uint32_t id, int type)
{
...
/* Is vni mcast group */
static bool
is_mac_vni_mcast_group (struct ethaddr *mac, vni_t vni,
 ...
 */
static uint8_t
neigh_flags_to_netlink (uint8_t dplane_flags)
{
  uint8_t flags = 0;
 ...
 */
static uint16_t
neigh_state_to_netlink (uint16_t dplane_state)
{
  uint16_t state = 0;
...
static inline int
proto2zebra (int proto, int family, bool is_nexthop)
{
  switch (proto)
    {
    case RTPROT_BABEL:
 ...
 */
vrf_id_t
vrf_lookup_by_table (uint32_t table_id, ns_id_t ns_id)
{
  struct vrf *vrf;
 ...
 */
static int
parse_encap_mpls (struct rtattr *tb, mpls_label_t *labels)
{
...
parse_encap_seg6local (struct rtattr *tb, struct seg6local_context *ctx)
{
  ...
  if (tb_encap[SEG6_LOCAL_NH4])
    ctx->nh4 = *(struct in_addr *) RTA_DATA (tb_encap[SEG6_LOCAL_NH4]);

  if (tb_encap[SEG6_LOCAL_NH6])
    ctx->nh6 = *(struct in6_addr *) RTA_DATA (tb_encap[SEG6_LOCAL_NH6]);
...
static int
parse_encap_seg6 (struct rtattr *tb, struct in6_addr *segs)
{
  ...
  if (tb_encap[SEG6_IPTUNNEL_SRH])
    {
      ipt = (struct seg6_iptunnel_encap *) RTA_DATA (
          tb_encap[SEG6_IPTUNNEL_SRH]);
                       ...
                       void *gate, afi_t afi, vrf_id_t vrf_id)
{
  ...
  if (tb[RTA_ENCAP] && tb[RTA_ENCAP_TYPE] &&
      *(uint16_t *) RTA_DATA (tb[RTA_ENCAP_TYPE]) == LWTUNNEL_ENCAP_MPLS)
    {
      num_labels = parse_encap_mpls (tb[RTA_ENCAP], labels);
    }
  if (tb[RTA_ENCAP] && tb[RTA_ENCAP_TYPE] &&
      *(uint16_t *) RTA_DATA (tb[RTA_ENCAP_TYPE]) == LWTUNNEL_ENCAP_SEG6_LOCAL)
    {
      seg6l_act = parse_encap_seg6local (tb[RTA_ENCAP], &seg6l_ctx);
    }
  if (tb[RTA_ENCAP] && tb[RTA_ENCAP_TYPE] &&
      *(uint16_t *) RTA_DATA (tb[RTA_ENCAP_TYPE]) == LWTUNNEL_ENCAP_SEG6)
    {
...
static uint8_t
parse_multipath_nexthops_unicast (ns_id_t ns_id, struct nexthop_group *ng,
  ...
  for (;;)
    {
      ...
      if (rtnh->rtnh_len > sizeof (*rtnh))
        {
          ...
          if (rtnh_tb[RTA_ENCAP] && rtnh_tb[RTA_ENCAP_TYPE] &&
              *(uint16_t *) RTA_DATA (rtnh_tb[RTA_ENCAP_TYPE]) ==
                  LWTUNNEL_ENCAP_MPLS)
            {
              num_labels = parse_encap_mpls (rtnh_tb[RTA_ENCAP], labels);
            }
          if (rtnh_tb[RTA_ENCAP] && rtnh_tb[RTA_ENCAP_TYPE] &&
              *(uint16_t *) RTA_DATA (rtnh_tb[RTA_ENCAP_TYPE]) ==
            ...
            {
              seg6l_act =
                  parse_encap_seg6local (rtnh_tb[RTA_ENCAP], &seg6l_ctx);
            }
          if (rtnh_tb[RTA_ENCAP] && rtnh_tb[RTA_ENCAP_TYPE] &&
              *(uint16_t *) RTA_DATA (rtnh_tb[RTA_ENCAP_TYPE]) ==
                  LWTUNNEL_ENCAP_SEG6)
            {
              num_segs = parse_encap_seg6 (rtnh_tb[RTA_ENCAP], &seg6_segs);
            }
        }
      ...
      if (gate && rtm->rtm_family == AF_INET)
        {
          if (index)
            nh = nexthop_from_ipv4_ifindex (gate, prefsrc, index, nh_vrf_id);
          else
            nh = nexthop_from_ipv4 (gate, prefsrc, nh_vrf_id);
        }
      else if (gate && rtm->rtm_family == AF_INET6)
        {
          if (index)
            nh = nexthop_from_ipv6_ifindex (gate, index, nh_vrf_id);
          else
                                            ...
                                            struct zebra_dplane_ctx *ctx)
{
  ...
  if (! startup && selfroute && h->nlmsg_type == RTM_NEWROUTE &&
      ! zrouter.asic_offloaded && ! ctx)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug (
            "Route type: %d Received that we think we have originated, ignoring",
            rtm->rtm_protocol);
      return 0;
    }
  ...
  if (rtm->rtm_family == AF_INET)
    {
      ...
      if (rtm->rtm_src_len != 0)
        {
          flog_warn (EC_ZEBRA_UNSUPPORTED_V4_SRCDEST,
  ...
  if (h->nlmsg_type == RTM_NEWROUTE)
    {
      ...
      if (! tb[RTA_MULTIPATH])
        {
          ...
          if (! nhe_id)
            {
              nh = parse_nexthop_unicast (ns_id, rtm, tb, bh_type, index,
                                          prefsrc, gate, afi, vrf_id);
          ...
          if (! nhe_id)
            {
               ...
               */
              nhop_num = parse_multipath_nexthops_unicast (
                  ns_id, ng, rtm, rtnh, tb, prefsrc, vrf_id);
      ...
      if (nhe_id || ng)
        {
          dplane_rib_add_multipath (afi, SAFI_UNICAST, &p, &src_p, re, ng,
                                    startup, ctx);
          if (ng)
            nexthop_group_delete (&ng);
        }
  ...
  else
    {
      ...
      if (nhe_id)
        {
          rib_delete (afi, SAFI_UNICAST, vrf_id, proto, 0, flags, &p, &src_p,
                      NULL, nhe_id, table, metric, distance, true);
        }
      else
        {
          if (! tb[RTA_MULTIPATH])
            {
              ...
              rib_delete (afi, SAFI_UNICAST, vrf_id, proto, 0, flags, &p,
                          &src_p, &nh, 0, table, metric, distance, true);
            }
          else
            {
              ...
              rib_delete (afi, SAFI_UNICAST, vrf_id, proto, 0, flags, &p,
                          &src_p, NULL, 0, table, metric, distance, true);
            }
        }
    }
...
static int
netlink_route_change_read_unicast (struct nlmsghdr *h, ns_id_t ns_id,
                                   int startup)
{
  return netlink_route_change_read_unicast_internal (h, ns_id, startup, NULL);
}

static struct mcast_route_data *mroute = NULL;

static int
netlink_route_change_read_multicast (struct nlmsghdr *h, ns_id_t ns_id,
                                     int startup)
{
  int len;
  struct rtmsg *rtm;
  ...
  if (tb[RTA_SRC])
    {
      if (rtm->rtm_family == RTNL_FAMILY_IPMR)
        m->src.ipaddr_v4 = *(struct in_addr *) RTA_DATA (tb[RTA_SRC]);
      else
        m->src.ipaddr_v6 = *(struct in6_addr *) RTA_DATA (tb[RTA_SRC]);
    }

  if (tb[RTA_DST])
    {
      if (rtm->rtm_family == RTNL_FAMILY_IPMR)
        m->grp.ipaddr_v4 = *(struct in_addr *) RTA_DATA (tb[RTA_DST]);
      else
        m->grp.ipaddr_v6 = *(struct in6_addr *) RTA_DATA (tb[RTA_DST]);
    }
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    {
      ...
      zlog_debug (
          ...
          zvrf_name (zvrf), vrf, nl_msg_type_to_str (h->nlmsg_type), &m->src,
          &m->grp, ifp ? ifp->name : "Unknown", iif, oif_list, m->lastused);
    }
  return 0;
}

int
netlink_route_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len;
  struct rtmsg *rtm;
  ...
  /* Connected route. */
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s %s %s proto %s NS %u", nl_msg_type_to_str (h->nlmsg_type),
 ...
 */
static bool
_netlink_route_add_gateway_info (uint8_t route_family, uint8_t gw_family,
...
static int
build_label_stack (struct mpls_label_stack *nh_label,
                   enum lsp_types_t nh_label_type, mpls_lse_t *out_lse,
  ...
  for (int i = 0; nh_label && i < nh_label->num_labels; i++)
    {
      ...
      if (nh_label_type == ZEBRA_LSP_EVPN)
        ...
        out_lse[num_labels] = mpls_lse_encode (nh_label->label[i], 0, 0, 0);
      num_labels++;
    }
...
static bool
_netlink_nexthop_encode_dvni_label (const struct nexthop *nexthop,
                                    struct nlmsghdr *nlmsg,
                                    mpls_lse_t *out_lse, size_t buflen,
                                    char *label_buf)
{
  struct in_addr ipv4;
      ...
      if (IS_MAPPED_IPV6 (&nexthop->gate.ipv6))
        {
          ...
          if (! nl_attr_put (nlmsg, buflen, LWTUNNEL_IP_DST, &ipv4, 4))
            return false;
        }
      else
        {
...
static bool
_netlink_route_encode_label_info (const struct nexthop *nexthop,
                                  ...
                                  size_t label_buf_size)
{
  ...
  num_labels = build_label_stack (nh_label, nh_label_type, out_lse, label_buf,
                                  label_buf_size);

  if (num_labels && nh_label_type == ZEBRA_LSP_EVPN)
    {
      if (! nl_attr_put16 (nlmsg, buflen, RTA_ENCAP_TYPE, LWTUNNEL_ENCAP_IP))
        return false;
      ...
      if (_netlink_nexthop_encode_dvni_label (nexthop, nlmsg, out_lse, buflen,
                                              label_buf) == false)
        return false;
  ...
  else if (num_labels)
    {
      ...
      if (rtmsg->rtm_family == AF_MPLS)
      ...
      else
        {
          ...
          if (! nl_attr_put (nlmsg, buflen, MPLS_IPTUNNEL_DST, &out_lse,
                             num_labels * sizeof (mpls_lse_t)))
            return false;
          nl_attr_nest_end (nlmsg, nest);
        }
...
static bool
_netlink_route_encode_nexthop_src (const struct nexthop *nexthop, int family,
                                   struct nlmsghdr *nlmsg, size_t buflen,
                                   int bytelen)
{
  if (family == AF_INET)
    {
      if (nexthop->rmap_src.ipv4.s_addr != INADDR_ANY)
        ...
        {
          if (! nl_attr_put (nlmsg, buflen, RTA_PREFSRC, &nexthop->src.ipv4,
                             bytelen))
            return false;
        }
    }
  else if (family == AF_INET6)
    {
      if (! IN6_IS_ADDR_UNSPECIFIED (&nexthop->rmap_src.ipv6))
        ...
        {
          if (! nl_attr_put (nlmsg, buflen, RTA_PREFSRC, &nexthop->src.ipv6,
                             bytelen))
            return false;
        }
    }
...
static ssize_t
fill_seg6ipt_encap (char *buffer, size_t buflen, const struct in6_addr *seg)
{
   ...
   */
  if (buflen <
      (sizeof (struct seg6_iptunnel_encap) + sizeof (struct ipv6_sr_hdr) + 16))
    return -1;
 ...
 */
static bool
_netlink_route_build_singlepath (const struct prefix *p, const char *routedesc,
                                 int bytelen, const struct nexthop *nexthop,
                                 ...
                                 size_t req_size, int cmd)
{
          ...
          switch (nexthop->nh_srv6->seg6local_action)
            ...
            case ZEBRA_SEG6_LOCAL_ACTION_END_X:
              if (! nl_attr_put32 (nlmsg, req_size, SEG6_LOCAL_ACTION,
                ...
                return false;
              if (! nl_attr_put (nlmsg, req_size, SEG6_LOCAL_NH6, &ctx->nh6,
            ...
            case ZEBRA_SEG6_LOCAL_ACTION_END_DX4:
              if (! nl_attr_put32 (nlmsg, req_size, SEG6_LOCAL_ACTION,
                ...
                return false;
              if (! nl_attr_put (nlmsg, req_size, SEG6_LOCAL_NH4, &ctx->nh4,
            ...
            case ZEBRA_SEG6_LOCAL_ACTION_END_DT4:
              if (! nl_attr_put32 (nlmsg, req_size, SEG6_LOCAL_ACTION,
                ...
                return false;
              if (! nl_attr_put32 (nlmsg, req_size, SEG6_LOCAL_VRFTABLE,
                                   ctx->table))
                return false;
              break;
            ...
            case ZEBRA_SEG6_LOCAL_ACTION_UNSPEC:
              zlog_err ("%s: unsupport seg6local behaviour action=%u",
                        __func__, nexthop->nh_srv6->seg6local_action);
              return false;
            }
          nl_attr_nest_end (nlmsg, nest);
        }
  ...
  if (is_route_v4_over_v6 (rtmsg->rtm_family, nexthop->type))
    {
      ...
      if (cmd == RTM_NEWROUTE)
        {
          if (! _netlink_route_encode_nexthop_src (nexthop, AF_INET, nlmsg,
                                                   req_size, bytelen))
            return false;
        }
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: 5549 (%s): %pFX nexthop via %s %s if %u vrf %s(%u)",
                    ...
                    nexthop->ifindex, VRF_LOGNAME (vrf), nexthop->vrf_id);
      return true;
    }
  ...
  if (nexthop->type == NEXTHOP_TYPE_IPV4 ||
      nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX)
    {
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        {
          inet_ntop (AF_INET, &nexthop->gate.ipv4, addrstr, sizeof (addrstr));
  ...
  if (nexthop->type == NEXTHOP_TYPE_IPV6 ||
      nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX)
    {
      if (! _netlink_route_add_gateway_info (
              rtmsg->rtm_family, AF_INET6, nlmsg, req_size, bytelen, nexthop))
        return false;

      if (cmd == RTM_NEWROUTE)
        {
          if (! _netlink_route_encode_nexthop_src (nexthop, AF_INET6, nlmsg,
                                                   req_size, bytelen))
            return false;
        }
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        {
          inet_ntop (AF_INET6, &nexthop->gate.ipv6, addrstr, sizeof (addrstr));
          zlog_debug ("%s: (%s): %pFX nexthop via %s %s if %u vrf %s(%u)",
                      __func__, routedesc, p, addrstr, label_buf,
                      nexthop->ifindex, VRF_LOGNAME (vrf), nexthop->vrf_id);
        }
  ...
  if (nexthop->type == NEXTHOP_TYPE_IFINDEX)
    {
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: (%s): %pFX nexthop via if %u vrf %s(%u)", __func__,
                    routedesc, p, nexthop->ifindex, VRF_LOGNAME (vrf),
                    nexthop->vrf_id);
    }
 ...
 */
static inline bool
_netlink_set_tag (struct nlmsghdr *n, unsigned int maxlen, route_tag_t tag)
{
  if (tag > 0 && tag <= 255)
    {
                                ...
                                route_tag_t tag)
{
  ...
  if (nexthop->type == NEXTHOP_TYPE_IPV4 ||
      nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX)
    {
      if (! _netlink_route_add_gateway_info (rtmsg->rtm_family, AF_INET, nlmsg,
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: (%s): %pFX nexthop via %pI4 %s if %u vrf %s(%u)",
                    __func__, routedesc, p, &nexthop->gate.ipv4, label_buf,
                    nexthop->ifindex, VRF_LOGNAME (vrf), nexthop->vrf_id);
    }
  if (nexthop->type == NEXTHOP_TYPE_IPV6 ||
      nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX)
    {
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: (%s): %pFX nexthop via %pI6 %s if %u vrf %s(%u)",
                    __func__, routedesc, p, &nexthop->gate.ipv6, label_buf,
                    nexthop->ifindex, VRF_LOGNAME (vrf), nexthop->vrf_id);
    }
                                ...
                                size_t req_size, int cmd)
{
  ...
  return _netlink_route_build_singlepath (
      p, routedesc, bytelen, nhlfe->nexthop, nlmsg, rtmsg, req_size, cmd);
}
                               ...
                               struct rtmsg *rtmsg, const union g_addr **src)
{
  ...
  family = NHLFE_FAMILY (nhlfe);
  bytelen = (family == AF_INET ? 4 : 16);
  return _netlink_route_build_multipath (p, routedesc, bytelen, nhlfe->nexthop,
                                         nlmsg, req_size, rtmsg, src, 0);
}

static void
_netlink_mpls_debug (int cmd, uint32_t label, const char *routedesc)
{
  if (IS_ZEBRA_DEBUG_KERNEL)
...
static int
netlink_neigh_update (int cmd, int ifindex, void *addr, char *lla, int llalen,
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    {
      ...
      struct interface *ifp =
          if_lookup_by_index_per_ns (zebra_ns_lookup (ns_id), ifindex);
      if (ifp)
        {
          if (family == AF_INET6)
          ...
          zlog_debug (
              ...
              __func__, nl_msg_type_to_str (cmd), ifp->name, ifindex, ip_str,
              (struct ethaddr *) lla, vrf_id_to_name (ifp->vrf->vrf_id),
              ifp->vrf->vrf_id);
        }
    }
...
static bool
nexthop_set_src (const struct nexthop *nexthop, int family, union g_addr *src)
{
  if (family == AF_INET)
    {
      if (nexthop->rmap_src.ipv4.s_addr != INADDR_ANY)
 ...
 */
static int
netlink_route_nexthop_encap (struct nlmsghdr *n, size_t nlen,
                             struct nexthop *nh)
{
  struct rtattr *nest;
 ...
 */
ssize_t
netlink_route_multipath_msg_encode (int cmd, struct zebra_dplane_ctx *ctx,
                                    ...
                                    bool force_nhg)
{
  ...
  if ((cmd == RTM_NEWROUTE) && ((p->family == AF_INET) || v6_rr_semantics))
  ...
  if (src_p)
    {
      if (! nl_attr_put (&req->n, datalen, RTA_SRC, &src_p->u.prefix, bytelen))
        return 0;
    }
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s: %s %pFX vrf %u(%u)", __func__, nl_msg_type_to_str (cmd),
                p, dplane_ctx_get_vrf (ctx), table_id);
      ...
      if (setsrc)
        {
          if (p->family == AF_INET)
            {
              if (! nl_attr_put (&req->n, datalen, RTA_PREFSRC, &src.ipv4,
                                 bytelen))
                return 0;
            }
          else if (p->family == AF_INET6)
            {
              if (! nl_attr_put (&req->n, datalen, RTA_PREFSRC, &src.ipv6,
                                 bytelen))
                return 0;
            }
        }
      ...
      for (ALL_NEXTHOPS_PTR (dplane_ctx_get_ng (ctx), nexthop))
        {
          ...
          if (NEXTHOP_IS_ACTIVE (nexthop->flags))
            {
              routedesc =
                  nexthop->rparent ? "recursive, single-path" : "single-path";

              if (! _netlink_set_tag (&req->n, datalen, tag))
                return 0;

              if (! _netlink_route_build_singlepath (p, routedesc, bytelen,
                                                     nexthop, &req->n, &req->r,
                                                     datalen, cmd))
                return 0;
      ...
      for (ALL_NEXTHOPS_PTR (dplane_ctx_get_ng (ctx), nexthop))
        {
          ...
          if (NEXTHOP_IS_ACTIVE (nexthop->flags))
            {
              routedesc =
                  nexthop->rparent ? "recursive, multipath" : "multipath";
              nexthop_num++;

              if (! _netlink_route_build_multipath (p, routedesc, bytelen,
                                                    nexthop, &req->n, datalen,
                                                    &req->r, &src1, tag))
                return 0;
...
int
kernel_get_ipmr_sg_stats (struct zebra_vrf *zvrf, void *in)
{
  ...
  if (mroute->family == AF_INET)
    {
      ...
      nl_attr_put (&req.n, sizeof (req), RTA_SRC, &mroute->src.ipaddr_v4,
                   sizeof (mroute->src.ipaddr_v4));
      nl_attr_put (&req.n, sizeof (req), RTA_DST, &mroute->grp.ipaddr_v4,
  ...
  else
    {
      ...
      nl_attr_put (&req.n, sizeof (req), RTA_SRC, &mroute->src.ipaddr_v6,
                   sizeof (mroute->src.ipaddr_v6));
      nl_attr_put (&req.n, sizeof (req), RTA_DST, &mroute->grp.ipaddr_v6,
  ...
  if (mroute->family == AF_INET)
    actual_table =
        (zvrf->table_id == RT_TABLE_MAIN) ? RT_TABLE_DEFAULT : zvrf->table_id;
  else
...
static bool
_netlink_nexthop_build_group (struct nlmsghdr *n, size_t req_size, uint32_t id,
                              const struct nh_grp *z_grp, const uint8_t count,
                              bool resilient,
      ...
      for (int i = 0; i < count; i++)
        {
          ...
          if (IS_ZEBRA_DEBUG_KERNEL)
            {
              if (i == 0)
                snprintf (buf, sizeof (buf1), "group %u", grp[i].id);
              else
                {
                  snprintf (buf1, sizeof (buf1), "/%u", grp[i].id);
                  strlcat (buf, buf1, sizeof (buf));
                }
            }
        }
      if (! nl_attr_put (n, req_size, NHA_GROUP, grp, count * sizeof (*grp)))
        return false;
      ...
      if (resilient)
        {
          ...
          nl_attr_put16 (n, req_size, NHA_RES_GROUP_BUCKETS, nhgr->buckets);
          nl_attr_put32 (n, req_size, NHA_RES_GROUP_IDLE_TIMER,
                         nhgr->idle_timer * 1000);
          nl_attr_put32 (n, req_size, NHA_RES_GROUP_UNBALANCED_TIMER,
                         nhgr->unbalanced_timer * 1000);
          nl_attr_nest_end (n, nest);

          nl_attr_put16 (n, req_size, NHA_GROUP_TYPE, NEXTHOP_GRP_TYPE_RES);
        }
    }
 ...
 */
ssize_t
netlink_nexthop_msg_encode (uint16_t cmd, const struct zebra_dplane_ctx *ctx,
                            void *buf, size_t buflen, bool fpm)
{
  ...
  if (proto_nexthops_only () && ! is_proto_nhg (id, type))
    {
      if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_NHG)
        zlog_debug ("%s: nhg_id %u (%s): proto-based nexthops only, ignoring",
                    __func__, id, zebra_route_string (type));
      return 0;
    }
      ...
      if (dplane_ctx_get_nhe_nh_grp_count (ctx))
        {
          ...
          if (! _netlink_nexthop_build_group (
                  &req->n, buflen, id, dplane_ctx_get_nhe_nh_grp (ctx),
                  dplane_ctx_get_nhe_nh_grp_count (ctx), ! ! nhgr->buckets,
                  nhgr))
            return 0;
        }
      else
        {
          ...
          switch (nh->type)
            ...
            case NEXTHOP_TYPE_IPV4_IFINDEX:
              if (! nl_attr_put (&req->n, buflen, NHA_GATEWAY, &nh->gate.ipv4,
                                 IPV4_MAX_BYTELEN))
                return 0;
              break;
            case NEXTHOP_TYPE_IPV6:
            case NEXTHOP_TYPE_IPV6_IFINDEX:
              if (! nl_attr_put (&req->n, buflen, NHA_GATEWAY, &nh->gate.ipv6,
                                 IPV6_MAX_BYTELEN))
                return 0;
              break;
            case NEXTHOP_TYPE_BLACKHOLE:
              if (! nl_attr_put (&req->n, buflen, NHA_BLACKHOLE, NULL, 0))
                return 0;
          ...
          if (! nl_attr_put32 (&req->n, buflen, NHA_OIF, nh->ifindex))
            return 0;
          ...
          if (num_labels && nh->nh_label_type == ZEBRA_LSP_EVPN)
            {
              if (! nl_attr_put16 (&req->n, buflen, NHA_ENCAP_TYPE,
                                   LWTUNNEL_ENCAP_IP))
                return 0;
              ...
              if (_netlink_nexthop_encode_dvni_label (
                      nh, &req->n, out_lse, buflen, label_buf) == false)
                return 0;
          ...
          else if (num_labels)
            {
              ...
              if (! nl_attr_put16 (&req->n, buflen, NHA_ENCAP_TYPE, encap))
                return 0;
              nest = nl_attr_nest (&req->n, buflen, NHA_ENCAP);
              if (! nest)
                return 0;
              if (! nl_attr_put (&req->n, buflen, MPLS_IPTUNNEL_DST, &out_lse,
                                 num_labels * sizeof (mpls_lse_t)))
                return 0;
          ...
          if (nh->nh_srv6)
            {
              if (nh->nh_srv6->seg6local_action !=
                  ZEBRA_SEG6_LOCAL_ACTION_UNSPEC)
                {
                  ...
                  if (! nl_attr_put (&req->n, buflen, NHA_ENCAP_TYPE, &encap,
                                     sizeof (uint16_t)))
                    return 0;

                  nest =
                      nl_attr_nest (&req->n, buflen, NHA_ENCAP | NLA_F_NESTED);
                  if (! nest)
                    return 0;
                  ...
                  switch (action)
                    ...
                    case SEG6_LOCAL_ACTION_END_T:
                      if (! nl_attr_put32 (&req->n, buflen, SEG6_LOCAL_ACTION,
                                           SEG6_LOCAL_ACTION_END_T))
                        return 0;
                      if (! nl_attr_put32 (&req->n, buflen, SEG6_LOCAL_TABLE,
                                           ctx->table))
                        return 0;
                      break;
                    case SEG6_LOCAL_ACTION_END_DX4:
                      if (! nl_attr_put32 (&req->n, buflen, SEG6_LOCAL_ACTION,
                                           SEG6_LOCAL_ACTION_END_DX4))
                        return 0;
                      if (! nl_attr_put (&req->n, buflen, SEG6_LOCAL_NH4,
              ...
              if (! sid_zero (&nh->nh_srv6->seg6_segs))
                {
                  ...
                  if (tun_len < 0)
                    return 0;
                  if (! nl_attr_put (&req->n, buflen, SEG6_IPTUNNEL_SRH,
                                     tun_buf, tun_len))
                    return 0;
                  nl_attr_nest_end (&req->n, nest);
                }
        ...
        nexthop_done:
          ...
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("%s: ID (%u): %pNHv(%d) vrf %s(%u) %s ", __func__, id,
                        nh, nh->ifindex, vrf_id_to_name (nh->vrf_id),
                        nh->vrf_id, label_buf);
        }
...
static ssize_t
netlink_nexthop_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                             size_t buflen)
{
  enum dplane_op_e op;
  int cmd = 0;
                                ...
                                struct zebra_dplane_ctx *ctx)
{
  ...
  return netlink_batch_add_msg (bth, ctx, netlink_nexthop_msg_encoder, false);
}

static ssize_t
netlink_newroute_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                              size_t buflen)
{
  return netlink_route_multipath_msg_encode (RTM_NEWROUTE, ctx, buf, buflen,
                                             false, false);
}

static ssize_t
netlink_delroute_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                              size_t buflen)
{
  return netlink_route_multipath_msg_encode (RTM_DELROUTE, ctx, buf, buflen,
                                             false, false);
}
      ...
      if (p->family == AF_INET || v6_rr_semantics)
        {
          ...
          if (RSYSTEM_ROUTE (dplane_ctx_get_type (ctx)) &&
            ...
            return netlink_batch_add_msg (bth, ctx,
                                          netlink_delroute_msg_encoder, true);
        }
      else
        {
          /*
          ...
          if (! RSYSTEM_ROUTE (dplane_ctx_get_old_type (ctx)))
            netlink_batch_add_msg (bth, ctx, netlink_delroute_msg_encoder,
                                   true);
        }
 ...
 */
static struct nexthop
netlink_nexthop_process_nh (struct rtattr **tb, unsigned char family,
                            struct interface **ifp, ns_id_t ns_id)
{
  ...
  ifp_lookup = if_lookup_by_index_per_ns (zebra_ns_lookup (ns_id), nh.ifindex);
...
static int
netlink_nexthop_process_group (struct rtattr **tb, struct nh_grp *z_grp,
                               int z_grp_size, struct nhg_resilience *nhgr)
{
  uint8_t count = 0;
 ...
 */
int
netlink_nexthop_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len;
  /* nexthop group id */
  uint32_t id;
  ...
  netlink_parse_rtattr_flags (tb, NHA_MAX, RTM_NHA (nhm), len, NLA_F_NESTED);
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s ID (%u) %s NS %u", nl_msg_type_to_str (h->nlmsg_type), id,
  ...
  if (h->nlmsg_type == RTM_NEWNEXTHOP)
    {
      ...
      if (tb[NHA_GROUP])
        {
           ...
           */
          grp_count =
              netlink_nexthop_process_group (tb, grp, array_size (grp), &nhgr);
        }
      else
        {
          if (tb[NHA_BLACKHOLE])
             ...
             */
            nh = netlink_nexthop_process_nh (tb, family, &ifp, ns_id);
          else
            {
      ...
      if (zebra_nhg_kernel_find (id, &nh, grp, grp_count, vrf_id, afi, type,
                                 startup, &nhgr))
        return -1;
    }
 ...
 */
static int
netlink_request_nexthop (struct zebra_ns *zns, int family, int type)
{
  struct
  {
    struct nlmsghdr n;
 ...
 */
int
netlink_nexthop_read (struct zebra_ns *zns)
{
  int ret;
  struct zebra_dplane_info dp_info;
...
int
kernel_neigh_update (int add, int ifindex, void *addr, char *lla, int llalen,
                     ns_id_t ns_id, uint8_t family, bool permanent)
{
 ...
 */
static ssize_t
netlink_neigh_update_msg_encode (const struct zebra_dplane_ctx *ctx, int cmd,
                                 const void *lla, int llalen,
                                 const struct ipaddr *ip, bool replace_obj,
                                 uint8_t family, uint8_t type, uint8_t flags,
                                 uint16_t state, uint32_t nhg_id, bool nfy,
                                 uint8_t nfy_flags, bool ext,
                                 uint32_t ext_flags, void *data,
                                 size_t datalen, uint8_t protocol)
{
  ...
  if (cmd == RTM_NEWNEIGH)
    req->n.nlmsg_flags |=
        NLM_F_CREATE | (replace_obj ? NLM_F_REPLACE : NLM_F_APPEND);
  req->n.nlmsg_type = cmd;
  req->ndm.ndm_family = family;
  ...
  if (nfy)
    {
      ...
      nest = nl_attr_nest (&req->n, datalen, NDA_FDB_EXT_ATTRS | NLA_F_NESTED);
      if (! nest)
        return 0;

      if (! nl_attr_put (&req->n, datalen, NFEA_ACTIVITY_NOTIFY, &nfy_flags,
                         sizeof (nfy_flags)))
        return 0;
                                ...
                                void *buf, size_t buflen)
{
  ...
  return netlink_neigh_update_msg_encode (
      ...
      (NUD_NOARP | NUD_PERMANENT), 0 /*nhg*/, false /*nfy*/, 0 /*nfy_flags*/,
      false /*ext*/, 0 /*ext_flags*/, buf, buflen, proto);
}
...
static int
netlink_macfdb_change (struct nlmsghdr *h, int len, ns_id_t ns_id)
{
  struct ndmsg *ndm;
  struct interface *ifp;
  struct zebra_if *zif;
  ...
  if (RTA_PAYLOAD (tb[NDA_LLADDR]) != ETH_ALEN)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s AF_BRIDGE IF %u - LLADDR is not MAC, len %lu",
                    nl_msg_type_to_str (h->nlmsg_type), ndm->ndm_ifindex,
  ...
  if (tb[NDA_DST])
    {
      ...
      dst_present = 1;
      memcpy (&vtep_ip.s_addr, RTA_DATA (tb[NDA_DST]), IPV4_MAX_BYTELEN);
      snprintfrr (dst_buf, sizeof (dst_buf), " dst %pI4", &vtep_ip);
    }
  else
      ...
      if (nfea_tb[NFEA_ACTIVITY_NOTIFY])
        {
          ...
          nfy_flags = *(uint8_t *) RTA_DATA (nfea_tb[NFEA_ACTIVITY_NOTIFY]);
  ...
  if (filter_vlan && vid != filter_vlan)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("        Filtered due to filter vlan: %d", filter_vlan);
      return 0;
    }
  ...
  if (dst_present)
    {
      if (vni_mcast_grp)
        return zebra_vxlan_if_vni_mcast_group_del (ifp, vni, &vtep_ip);
...
static int
netlink_macfdb_table (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len;
  struct ndmsg *ndm;
...
static int
netlink_request_macs (struct nlsock *netlink_cmd, int family, int type,
...
netlink_macfdb_read (struct zebra_ns *zns)
{
  ...
  ret = netlink_request_macs (&zns->netlink_cmd, AF_BRIDGE, RTM_GETNEIGH, 0);
  if (ret < 0)
    return ret;
  /* We are reading entire table. */
  filter_vlan = 0;
  ret = netlink_parse_info (netlink_macfdb_table, &zns->netlink_cmd, &dp_info,
                            0, true);
...
static int
netlink_request_specific_mac (struct zebra_ns *zns, int family, int type,
                              struct interface *ifp, const struct ethaddr *mac,
                              vlanid_t vid, vni_t vni, uint8_t flags)
{
  struct
  {
    struct nlmsghdr n;
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("Tx %s %s IF %s(%u) MAC %pEA vid %u vni %u",
                ...
                nl_family_to_str (req.ndm.ndm_family), ifp->name, ifp->ifindex,
                mac, vid, vni);
...
int
netlink_macfdb_read_specific_mac (struct zebra_ns *zns,
                                  struct interface *br_if,
                                  const struct ethaddr *mac, vlanid_t vid)
{
  ...
  /* Get bridge FDB table for specific bridge - we do the VLAN filtering.
   */
  ret = netlink_request_specific_mac (zns, AF_BRIDGE, RTM_GETNEIGH, br_if, mac,
                                      vid, 0, 0);
  if (ret < 0)
    return ret;
...
int
netlink_macfdb_read_mcast_for_vni (struct zebra_ns *zns, struct interface *ifp,
                                   vni_t vni)
{
  struct zebra_if *zif;
  ...
  ret = netlink_request_specific_mac (zns, AF_BRIDGE, RTM_GETNEIGH, ifp, &mac,
                                      0, vni, NTF_SELF);
  if (ret < 0)
    return ret;
 ...
 */
ssize_t
netlink_macfdb_update_ctx (struct zebra_dplane_ctx *ctx, void *data,
                           size_t datalen)
{
  ...
  cmd = dplane_ctx_get_op (ctx) == DPLANE_OP_MAC_INSTALL ? RTM_NEWNEIGH
                                                         : RTM_DELNEIGH;
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    {
      ...
      zlog_debug (
          ...
          dplane_ctx_mac_is_sticky (ctx) ? "sticky " : "", mac, &vtep_ip,
          nhg_id, (update_flags & DPLANE_MAC_REMOTE) ? " rem" : "",
  ...
  total = netlink_neigh_update_msg_encode (
      ...
      &vtep_ip, true, AF_BRIDGE, 0, flags, state, nhg_id, nfy, nfy_flags,
      false /*ext*/, 0 /*ext_flags*/, data, datalen, proto);
 ...
 */
static void
netlink_handle_5549 (struct ndmsg *ndm, struct zebra_if *zif,
...
static int
netlink_nbr_entry_state_to_zclient (int nbr_state)
{
...
static int
netlink_ipneigh_change (struct nlmsghdr *h, int len, ns_id_t ns_id)
{
  struct ndmsg *ndm;
  struct interface *ifp;
  struct zebra_if *zif;
  ...
  if (l2_len == IPV4_MAX_BYTELEN || l2_len == 0)
    {
      ...
      zsend_nhrp_neighbor_notify (
          cmd, ifp, &ip, netlink_nbr_entry_state_to_zclient (ndm->ndm_state),
          &link_layer_ipv4);
    }
      ...
      if (ndm->ndm_state & NUD_VALID)
        {
          ...
          if (link_if)
            zebra_vxlan_handle_kernel_neigh_update (
                ifp, link_if, &ip, &mac, ndm->ndm_state, is_ext, is_router,
                local_inactive, dp_static);
          return 0;
        }
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("Rx %s family %s IF %s(%u) vrf %s(%u) IP %pIA",
                nl_msg_type_to_str (h->nlmsg_type),
                nl_family_to_str (ndm->ndm_family), ifp->name,
                ndm->ndm_ifindex, ifp->vrf->name, ifp->vrf->vrf_id, &ip);
...
static int
netlink_neigh_table (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len;
  struct ndmsg *ndm;
...
static int
netlink_request_neigh (struct nlsock *netlink_cmd, int family, int type,
                       ifindex_t ifindex)
{
  struct
  {
...
netlink_neigh_read (struct zebra_ns *zns)
{
  ...
  ret = netlink_request_neigh (&zns->netlink_cmd, AF_UNSPEC, RTM_GETNEIGH, 0);
  if (ret < 0)
    return ret;
  ret = netlink_parse_info (netlink_neigh_table, &zns->netlink_cmd, &dp_info,
                            0, true);
 ...
 */
static int
netlink_request_specific_neigh_in_vlan (struct zebra_ns *zns, int type,
                                        ...
                                        ifindex_t ifindex)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s: Tx %s family %s IF %u IP %pIA flags 0x%x", __func__,
                nl_msg_type_to_str (type),
...
int
netlink_neigh_read_specific_ip (const struct ipaddr *ip,
                                struct interface *vlan_if)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s: neigh request IF %s(%u) IP %pIA vrf %s(%u)", __func__,
                vlan_if->name, vlan_if->ifindex, ip, vlan_if->vrf->name,
                vlan_if->vrf->vrf_id);
...
int
netlink_neigh_change (struct nlmsghdr *h, ns_id_t ns_id)
{
  int len;
  struct ndmsg *ndm;
 ...
 */
static ssize_t
netlink_neigh_update_ctx (const struct zebra_dplane_ctx *ctx, int cmd,
                          void *buf, size_t buflen)
{
  const struct ipaddr *ip;
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug (
        ...
        dplane_ctx_get_ifname (ctx), dplane_ctx_get_ifindex (ctx), ip,
        link_ip ? "Link" : "MAC", buf2, flags, state, ext ? "ext " : "",
        ext_flags);
...
static int
netlink_neigh_table_update_ctx (const struct zebra_dplane_ctx *ctx, void *data,
                                size_t datalen)
{
  struct
  {
  ...
  if (! nl_attr_put (&req->n, datalen, NDTPA_MCAST_PROBES, &val, sizeof (val)))
    return 0;
  val = dplane_ctx_neightable_get_ucast_probes (ctx);
  if (! nl_attr_put (&req->n, datalen, NDTPA_UCAST_PROBES, &val, sizeof (val)))
    return 0;
  nl_attr_nest_end (&req->n, nest);
...
static ssize_t
netlink_neigh_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                           size_t buflen)
{
  ssize_t ret = 0;
  ...
  switch (dplane_ctx_get_op (ctx))
    ...
    case DPLANE_OP_VTEP_ADD:
      ret = netlink_vxlan_flood_update_ctx (ctx, RTM_NEWNEIGH, buf, buflen);
      break;
    case DPLANE_OP_VTEP_DELETE:
      ret = netlink_vxlan_flood_update_ctx (ctx, RTM_DELNEIGH, buf, buflen);
      break;
...
{
  return netlink_batch_add_msg (bth, ctx, netlink_macfdb_update_ctx, false);
}
...
netlink_put_neigh_update_msg (struct nl_batch *bth,
                              struct zebra_dplane_ctx *ctx)
{
  return netlink_batch_add_msg (bth, ctx, netlink_neigh_msg_encoder, false);
}
 ...
 */
ssize_t
netlink_mpls_multipath_msg_encode (int cmd, struct zebra_dplane_ctx *ctx,
                                   void *buf, size_t buflen)
{
  mpls_lse_t lse;
  ...
  if (cmd == RTM_NEWROUTE)
    {
      ...
      route_type =
          re_type_from_lsp_type (dplane_ctx_get_best_nhlfe (ctx)->type);
  ...
  if (nexthop_num == 1)
    {
      ...
      _netlink_mpls_debug (cmd, dplane_ctx_get_in_label (ctx), routedesc);
          ...
          if ((cmd == RTM_NEWROUTE &&
                ...
                CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))))
            {
              ...
              if (! _netlink_mpls_build_singlepath (
                      &p, routedesc, nhlfe, &req->n, &req->r, buflen, cmd))
                return false;
          ...
          if ((cmd == RTM_NEWROUTE &&
                ...
                CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))))
            {
              ...
              if (! _netlink_mpls_build_multipath (
                      &p, routedesc, nhlfe, &req->n, buflen, &req->r, &src1))
                return 0;
            }
        }
 ...
 */
static int
netlink_fdb_nh_update (uint32_t nh_id, struct in_addr vtep_ip)
{
  struct
  {
    struct nlmsghdr n;
  ...
  if (! nl_attr_put (&req.n, sizeof (req), NHA_FDB, NULL, 0))
    return -1;
  if (! nl_attr_put (&req.n, sizeof (req), NHA_GATEWAY, &vtep_ip,
                     IPV4_MAX_BYTELEN))
    return -1;

  if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_EVPN_MH_NH)
    {
      zlog_debug ("Tx %s fdb-nh 0x%x %pI4", nl_msg_type_to_str (cmd), nh_id,
                  &vtep_ip);
    }
...
static int
netlink_fdb_nh_del (uint32_t nh_id)
{
  struct
  {
    struct nlmsghdr n;
...
static int
netlink_fdb_nhg_update (uint32_t nhg_id, uint32_t nh_cnt,
  ...
  if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_EVPN_MH_NH)
    {
      ...
      for (i = 0; i < nh_cnt; ++i)
        {
          snprintf (nh_buf, sizeof (nh_buf), "%u ", grp[i].id);
          strlcat (vtep_str, nh_buf, sizeof (vtep_str));
        }

      zlog_debug ("Tx %s fdb-nhg 0x%x %s", nl_msg_type_to_str (cmd), nhg_id,
                  vtep_str);
    }
...
static int
netlink_fdb_nhg_del (uint32_t nhg_id)
{
  return netlink_fdb_nh_del (nhg_id);
}

int
kernel_upd_mac_nh (uint32_t nh_id, struct in_addr vtep_ip)
{
  return netlink_fdb_nh_update (nh_id, vtep_ip);
}

int
kernel_del_mac_nh (uint32_t nh_id)
{
  return netlink_fdb_nh_del (nh_id);
}

int
kernel_upd_mac_nhg (uint32_t nhg_id, uint32_t nh_cnt, struct nh_grp *nh_ids)
{
  return netlink_fdb_nhg_update (nhg_id, nh_cnt, nh_ids);
}

int
kernel_del_mac_nhg (uint32_t nhg_id)
{
