...
int
lib_vrf_zebra_ribs_rib_get_keys (struct nb_cb_get_keys_args *args)
{
 ...
 */
struct yang_data *
lib_vrf_zebra_ribs_rib_afi_safi_name_get_elem (
    struct nb_cb_get_elem_args *args)
{
  const struct zebra_router_table *zrt = args->list_entry;

  return yang_data_new_string (
      args->xpath, yang_afi_safi_value2identity (zrt->afi, zrt->safi));
}
...
int
lib_vrf_zebra_ribs_rib_route_get_keys (struct nb_cb_get_keys_args *args)
{
    ...
    struct nb_cb_get_keys_args *args)
{
  ...
  switch (nexthop->type)
    ...
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      snprintfrr (args->keys->key[2], sizeof (args->keys->key[2]), "%pI4",
                  &nexthop->gate.ipv4);
      if (nexthop->ifindex)
    ...
    case NEXTHOP_TYPE_IPV6_IFINDEX:
      snprintfrr (args->keys->key[2], sizeof (args->keys->key[2]), "%pI6",
                  &nexthop->gate.ipv6);
    ...
    struct nb_cb_get_elem_args *args)
{
  ...
  switch (nexthop->type)
    ...
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      ...
      memcpy (&addr.ipaddr_v4, &(nexthop->gate.ipv4), sizeof (struct in_addr));
      break;
    case NEXTHOP_TYPE_IPV6:
    case NEXTHOP_TYPE_IPV6_IFINDEX:
    ...
    struct nb_cb_get_elem_args *args)
{
  ...
  if (nexthop->ifindex)
    return yang_data_new_string (
        args->xpath, ifindex2ifname (nexthop->ifindex, nexthop->vrf_id));
