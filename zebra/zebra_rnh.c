...
get_rnh_table (vrf_id_t vrfid, afi_t afi, safi_t safi)
{
...
static void
zebra_rnh_remove_from_routing_table (struct rnh *rnh)
{
  ...
  if (IS_ZEBRA_DEBUG_NHT_DETAILED)
    zlog_debug ("%s: %s(%u):%pRN removed from tracking on %pRN", __func__,
...
static void
zebra_rnh_store_in_routing_table (struct rnh *rnh)
{
  struct zebra_vrf *zvrf = zebra_vrf_lookup_by_id (rnh->vrf_id);
  struct route_table *table = zvrf->table[rnh->afi][rnh->safi];
  struct route_node *rn;
  ...
  if (IS_ZEBRA_DEBUG_NHT_DETAILED)
    zlog_debug ("%s: %s(%u):%pRN added for tracking on %pRN", __func__,
                VRF_LOGNAME (zvrf->vrf), rnh->vrf_id, rnh->node, rn);
...
struct rnh *
zebra_add_rnh (struct prefix *p, vrf_id_t vrfid, safi_t safi, bool *exists)
{
  ...
  if (IS_ZEBRA_DEBUG_NHT)
    {
      ...
      zlog_debug ("%s(%u): Add RNH %pFX for safi: %u", VRF_LOGNAME (vrf),
                  vrfid, p, safi);
    }
  ...
  if (! table)
    {
      ...
      flog_warn (EC_ZEBRA_RNH_NO_TABLE,
                 "%s(%u): Add RNH %pFX - table not found", VRF_LOGNAME (vrf),
                 vrfid, p);
      *exists = false;
      return NULL;
    }
...
struct rnh *
zebra_lookup_rnh (struct prefix *p, vrf_id_t vrfid, safi_t safi)
{
  struct route_table *table;
  struct route_node *rn;
  ...
  if (IS_ZEBRA_DEBUG_NHT)
    {
      ...
      zlog_debug ("%s(%u): Del RNH %pRN", VRF_LOGNAME (vrf), rnh->vrf_id,
                  rnh->node);
    }
  ...
  if (! listnode_lookup (rnh->zebra_pseudowire_list, pw))
    {
      ...
      pw->rnh = rnh;
      zebra_evaluate_rnh (zvrf, family2afi (pw->af), 1, &nh, SAFI_UNICAST);
    }
  else
    *nht_exists = true;
}
 ...
 */
static void
zebra_rnh_clear_nexthop_rnh_filters (struct route_entry *re)
{
 ...
 */
static int
zebra_rnh_apply_nht_rmap (afi_t afi, struct zebra_vrf *zvrf,
  ...
  if (prn && re)
    {
      for (nexthop = re->nhe->nhg.nexthop; nexthop; nexthop = nexthop->next)
        {
          ret = zebra_nht_route_map_check (afi, proto, &prn->p, zvrf, re,
                                           nexthop);
 ...
 */
static void
zebra_rnh_notify_protocol_clients (struct zebra_vrf *zvrf, afi_t afi,
      ...
      if (prn && re)
        {
          ...
          num_resolving_nh =
              zebra_rnh_apply_nht_rmap (afi, zvrf, prn, re, client->proto);
          if (num_resolving_nh)
...
static const int RNH_INVALID_NH_FLAGS =
    (NEXTHOP_FLAG_RECURSIVE | NEXTHOP_FLAG_DUPLICATE |
     NEXTHOP_FLAG_RNH_FILTERED);

bool
rnh_nexthop_valid (const struct route_entry *re, const struct nexthop *nh)
{
 ...
 */
static bool
rnh_check_re_nexthops (const struct route_entry *re, const struct rnh *rnh)
{
  bool ret = false;
                                 ...
                                 struct route_node **prn)
{
  ...
  *prn = NULL;
  ...
  while (rn)
    {
      if (IS_ZEBRA_DEBUG_NHT_DETAILED)
        zlog_debug ("%s: %s(%u):%pRN Possible Match to %pRN", __func__,
                    VRF_LOGNAME (zvrf->vrf), rnh->vrf_id, rnh->node, rn);
 ...
 */
static void
zebra_rnh_eval_nexthop_entry (struct zebra_vrf *zvrf, afi_t afi, int force,
                              struct route_node *nrn, struct rnh *rnh,
                              struct route_node *prn, struct route_entry *re)
{
  int state_changed = 0;
  ...
  if (state_changed || force)
    {
      ...
      zebra_rnh_notify_protocol_clients (zvrf, afi, nrn, rnh, prn, rnh->state);
...
static void
zebra_rnh_evaluate_entry (struct zebra_vrf *zvrf, afi_t afi, int force,
  ...
  if (IS_ZEBRA_DEBUG_NHT)
    {
      zlog_debug ("%s(%u):%pRN: Evaluate RNH, %s", VRF_LOGNAME (zvrf->vrf),
                  zvrf->vrf->vrf_id, nrn, force ? "(force)" : "");
    }
 ...
 */
static void
zebra_rnh_clear_nhc_flag (struct zebra_vrf *zvrf, afi_t afi,
                          struct route_node *nrn)
{
  struct rnh *rnh;
  struct route_entry *re;
 ...
 */
void
zebra_evaluate_rnh (struct zebra_vrf *zvrf, afi_t afi, int force,
...
void
zebra_print_rnh_table (vrf_id_t vrfid, afi_t afi, safi_t safi, struct vty *vty,
                       const struct prefix *p, json_object *json)
{
  struct route_table *table;
  struct route_node *rn;
...
static void
copy_state (struct rnh *rnh, const struct route_entry *re,
            struct route_node *rn)
{
...
next_valid_primary_nh (struct route_entry *re, struct nexthop *nh)
{
  ...
  while (nh)
    {
      ...
      if (IS_ZEBRA_DEBUG_NHT_DETAILED)
        zlog_debug ("%s: checking primary NH %pNHv", __func__, nh);
      ...
      for (idx = 0; bnh != NULL; idx++)
        {
           ...
           */
          if (IS_ZEBRA_DEBUG_NHT_DETAILED)
            zlog_debug ("%s: checking backup %pNHv [%d]", __func__, bnh, idx);
          ...
          for (i = 0; i < nh->backup_num; i++)
            {
              ...
              if (nh->backup_idx[i] == idx)
                {
                  if (IS_ZEBRA_DEBUG_NHT_DETAILED)
                    zlog_debug ("%s: backup %d activated", __func__, i);
 ...
 */
static bool
compare_valid_nexthops (struct route_entry *r1, struct route_entry *r2)
{
      ...
      if (nh1 && nh2)
        {
          ...
          if (nexthop_cmp (nh1, nh2) != 0)
            {
              if (IS_ZEBRA_DEBUG_NHT_DETAILED)
                zlog_debug ("%s: nh1: %pNHv, nh2: %pNHv differ", __func__, nh1,
                            nh2);
              goto done;
            }
        }
      else if (nh1 || nh2)
        {
          ...
          if (IS_ZEBRA_DEBUG_NHT_DETAILED)
            zlog_debug ("%s: nh1 %s, nh2 %s", __func__,
                        nh1 ? "non-NULL" : "NULL", nh2 ? "non-NULL" : "NULL");
          goto done;
        }
      else
  ...
  if (rnh_hide_backups)
    {
      ...
      if (r1->nhe->backup_info)
        hash1 = nexthop_group_hash (&r1->nhe->backup_info->nhe->nhg);

      if (r2->nhe->backup_info)
        hash2 = nexthop_group_hash (&r2->nhe->backup_info->nhe->nhg);

      if (IS_ZEBRA_DEBUG_NHT_DETAILED)
        zlog_debug ("%s: backup hash1 %#x, hash2 %#x", __func__, hash1, hash2);
  ...
  while (1)
    {
      ...
      if (nh1 && nh2)
      ...
      else if (nh1 || nh2)
        {
          /* One list has more valid nexthops than the other */
          if (IS_ZEBRA_DEBUG_NHT_DETAILED)
            zlog_debug ("%s: backup nh1 %s, nh2 %s", __func__,
                        nh1 ? "non-NULL" : "NULL", nh2 ? "non-NULL" : "NULL");
          goto done;
        }
      else
...
int
zebra_send_rnh_update (struct rnh *rnh, struct zserv *client, vrf_id_t vrf_id,
                       uint32_t srte_color)
{
  ...
  switch (rn->p.family)
    ...
    default:
      flog_err (EC_ZEBRA_RNH_UNKNOWN_FAMILY,
                "%s: Unknown family (%d) notification attempted", __func__,
                rn->p.family);
      goto failure;
    }
 ...
 */
void
show_nexthop_json_helper (json_object *json_nexthop,
                          ...
                          const struct route_entry *re)
{
  ...
  if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_HAS_BACKUP))
    {
      ...
      json_object_object_add (json_nexthop, "backupIndex", json_backups);
    }
  ...
  if (nexthop->nh_label && nexthop->nh_label->num_labels)
    {
      ...
      for (int label_index = 0; label_index < nexthop->nh_label->num_labels;
           label_index++)
        json_object_array_add (
            json_labels,
            json_object_new_int (
                ((nexthop->nh_label_type == ZEBRA_LSP_EVPN)
                     ? label2vni (&nexthop->nh_label->label[label_index])
  ...
  if (nexthop->srte_color)
    json_object_int_add (json_nexthop, "srteColor", nexthop->srte_color);
  ...
  if (nexthop->nh_srv6)
    {
      ...
      json_object_object_add (json_nexthop, "seg6local", json_seg6local);
 ...
 */
void
show_route_nexthop_helper (struct vty *vty, const struct route_entry *re,
                           const struct nexthop *nexthop)
{
...
static void
print_rnh (struct route_node *rn, struct vty *vty, json_object *json)
{
  struct rnh *rnh;
  struct nexthop *nexthop;
  struct listnode *node;
  ...
  if (json)
    {
      ...
      json_object_object_add (
          json, inet_ntop (rn->p.family, &rn->p.u.prefix, buf, BUFSIZ),
          json_nht);
      json_object_boolean_add (json_nht, "nhtConnected",
                               CHECK_FLAG (rnh->flags, ZEBRA_NHT_CONNECTED));
      json_object_object_add (json_nht, "clientList", json_client_array);
      ...
      if (json)
        {
          ...
          json_object_string_add (
              json_client, "protocolFiltered",
              (rnh->filtered[client->proto] ? "(filtered)" : "none"));
        }
      else
        {
  ...
  if (! list_isempty (rnh->zebra_pseudowire_list))
    {
      if (json)
        json_object_boolean_true_add (json_nht, "zebraPseudowires");
      else
...
static int
zebra_cleanup_rnh_client (vrf_id_t vrf_id, afi_t afi, safi_t safi,
                          struct zserv *client)
{
...
static int
zebra_client_cleanup_rnh (struct zserv *client)
{
  struct vrf *vrf;
  struct zebra_vrf *zvrf;
  ...
  RB_FOREACH (vrf, vrf_id_head, &vrfs_by_id)
    {
      ...
      if (zvrf)
        {
          zebra_cleanup_rnh_client (zvrf_id (zvrf), AFI_IP, SAFI_UNICAST,
                                    client);
          zebra_cleanup_rnh_client (zvrf_id (zvrf), AFI_IP, SAFI_MULTICAST,
                                    client);
          zebra_cleanup_rnh_client (zvrf_id (zvrf), AFI_IP6, SAFI_UNICAST,
                                    client);
          zebra_cleanup_rnh_client (zvrf_id (zvrf), AFI_IP6, SAFI_MULTICAST,
                                    client);
        }
    }
...
int
rnh_resolve_via_default (struct zebra_vrf *zvrf, int family)
{
 ...
 */
void
rnh_set_hide_backups (bool hide_p)
{
  rnh_hide_backups = hide_p;
}

bool
rnh_get_hide_backups (void)
{
