...
int
is_zebra_import_table_enabled (afi_t afi, vrf_id_t vrf_id, uint32_t table_id)
{
  /*
...
/* Redistribute routes. */
static void
zebra_redistribute (struct zserv *client, int type, unsigned short instance,
    ...
    RNODE_FOREACH_RE (rn, newre)
      {
        if (IS_ZEBRA_DEBUG_RIB)
          zlog_debug (
              ...
              zebra_route_string (newre->type), newre->instance,
              newre->distance, newre->metric, zebra_check_addr (&rn->p));
 ...
 */
static bool
zebra_redistribute_check (const struct route_node *rn,
                          const struct route_entry *re, struct zserv *client)
{
...
void
redistribute_update (const struct route_node *rn, const struct route_entry *re,
          ...
          if (IS_ZEBRA_DEBUG_RIB)
            {
              zlog_debug (
                  ...
                  __func__, zebra_route_string (client->proto), rn, re->vrf_id,
                  re->table, re->type, re->distance, re->metric);
            }
          zsend_redistribute_route (ZEBRA_REDISTRIBUTE_ROUTE_ADD, client, rn,
                                    re);
        }
 ...
 */
void
redistribute_delete (const struct route_node *rn,
  ...
  if (IS_ZEBRA_DEBUG_RIB)
    {
      ...
      zlog_debug ("%u:%u%pRN: Redist del: re %p (%u:%s), new re %p (%u:%s)",
                  ...
                  old_re ? zebra_route_string (old_re->type) : "None", new_re,
                  new_inst,
...
void
zebra_redistribute_add (ZAPI_HANDLER_ARGS)
{
  afi_t afi = 0;
  int type = 0;
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug (
        ...
        zebra_route_string (type), VRF_LOGNAME (zvrf->vrf), zvrf_id (zvrf),
        instance);
  ...
  if (type == 0 || type >= ZEBRA_ROUTE_MAX)
    {
      zlog_debug ("%s: Specified Route Type %d does not exist", __func__,
                  type);
      return;
    }
  ...
  if (instance)
  ...
  else
    {
      if (! vrf_bitmap_check (client->redist[afi][type], zvrf_id (zvrf)))
        {
          if (IS_ZEBRA_DEBUG_EVENT)
            zlog_debug ("%s: setting vrf %s(%u) redist bitmap", __func__,
                        VRF_LOGNAME (zvrf->vrf), zvrf_id (zvrf));
...
void
zebra_redistribute_delete (ZAPI_HANDLER_ARGS)
{
  afi_t afi = 0;
  int type = 0;
  unsigned short instance;
...
void
zebra_redistribute_default_add (ZAPI_HANDLER_ARGS)
{
  afi_t afi = 0;
...
void
zebra_redistribute_default_delete (ZAPI_HANDLER_ARGS)
{
  afi_t afi = 0;
...
zebra_interface_up_update (struct interface *ifp)
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug ("MESSAGE: ZEBRA_INTERFACE_UP %s vrf %s(%u)", ifp->name,
...
zebra_interface_down_update (struct interface *ifp)
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug ("MESSAGE: ZEBRA_INTERFACE_DOWN %s vrf %s(%u)", ifp->name,
                ifp->vrf->name, ifp->vrf->vrf_id);
...
zebra_interface_add_update (struct interface *ifp)
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug ("MESSAGE: ZEBRA_INTERFACE_ADD %s vrf %s(%u)", ifp->name,
                ifp->vrf->name, ifp->vrf->vrf_id);
...
zebra_interface_delete_update (struct interface *ifp)
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug ("MESSAGE: ZEBRA_INTERFACE_DELETE %s vrf %s(%u)", ifp->name,
                ifp->vrf->name, ifp->vrf->vrf_id);
                                    ...
                                    struct connected *ifc)
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug ("MESSAGE: ZEBRA_INTERFACE_ADDRESS_ADD %pFX on %s vrf %s(%u)",
                ifc->address, ifp->name, ifp->vrf->name, ifp->vrf->vrf_id);
      ...
      if (CHECK_FLAG (ifc->conf, ZEBRA_IFC_REAL))
        {
          ...
          zsend_interface_address (ZEBRA_INTERFACE_ADDRESS_ADD, client, ifp,
                                   ifc);
        }
    }
...
/* Interface VRF change. This function is invoked *post* VRF change and sends
 * an add to clients who are interested in the new VRF but not in the old VRF.
 */
void
...
int
zebra_add_import_table_entry (struct zebra_vrf *zvrf, struct route_node *rn,
                              struct route_entry *re, const char *rmap_name)
{
  ...
  RNODE_FOREACH_RE (rn, same)
    {
      ...
      if (same->type == re->type && same->instance == re->instance &&
          same->table == re->table && same->type != ZEBRA_ROUTE_CONNECT)
        break;
    }
  ...
  newre = zebra_rib_route_entry_new (
      0, ZEBRA_ROUTE_TABLE, re->table, re->flags, re->nhe_id, zvrf->table_id,
      re->metric, re->mtu, zebra_import_table_distance[afi][re->table],
      re->tag);
...
int
zebra_del_import_table_entry (struct zebra_vrf *zvrf, struct route_node *rn,
                              struct route_entry *re)
{
  ...
  rib_delete (afi, SAFI_UNICAST, zvrf->vrf->vrf_id, ZEBRA_ROUTE_TABLE,
              re->table, re->flags, &p, NULL, re->nhe->nhg.nexthop, re->nhe_id,
...
int
zebra_import_table (afi_t afi, vrf_id_t vrf_id, uint32_t table_id,
                    uint32_t distance, const char *rmap_name, int add)
{
  ...
  if (! is_zebra_valid_kernel_table (table_id) || (table_id == RT_TABLE_MAIN))
    return -1;
  ...
  table =
      zebra_vrf_get_table_with_table_id (afi, SAFI_UNICAST, vrf_id, table_id);
  if (table == NULL)
    {
      return 0;
    }
    ...
    {
      zlog_debug ("%s routes from table %d", add ? "Importing" : "Unimporting",
                  table_id);
    }
...
int
zebra_import_table_config (struct vty *vty, vrf_id_t vrf_id)
{
  int i;
  afi_t afi;
  int write = 0;
      ...
      for (i = 1; i < ZEBRA_KERNEL_TABLE_MAX; i++)
        {
          ...
          if (zebra_import_table_distance[afi][i] !=
              ZEBRA_TABLE_DISTANCE_DEFAULT)
            {
              vty_out (vty, "%s import-table %d distance %d", afi_str[afi], i,
...
static void
zebra_import_table_rm_update_vrf_afi (struct zebra_vrf *zvrf, afi_t afi,
  ...
  if (! table)
    {
      if (IS_ZEBRA_DEBUG_RIB_DETAILED)
        zlog_debug ("%s: Table id=%d not found", __func__, table_id);
      return;
    }
...
static void
zebra_import_table_rm_update_vrf (struct zebra_vrf *zvrf, const char *rmap)
{
  afi_t afi;
  int i;
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    {
      for (i = 1; i < ZEBRA_KERNEL_TABLE_MAX; i++)
        {
          if (! is_zebra_import_table_enabled (afi, zvrf->vrf->vrf_id, i))
            continue;
...
void
zebra_import_table_rm_update (const char *rmap)
{
  struct vrf *vrf;
  struct zebra_vrf *zvrf;
