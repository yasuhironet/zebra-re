 ...
 */
static const char *
fpm_nh_encap_type_to_str (enum fpm_nh_encap_type_t encap_type)
{
  switch (encap_type)
    {
 ...
 */
static int
netlink_route_info_add_nh (struct netlink_route_info *ri,
                           struct nexthop *nexthop, struct route_entry *re)
{
  ...
  if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_EVPN))
    {
      ...
      if (zvrf)
        {
          ifp = if_lookup_by_index_per_ns (zvrf->zns, nexthop->ifindex);
          if (ifp)
            {
 ...
 */
static int
netlink_route_info_fill (struct netlink_route_info *ri, int cmd,
 ...
 */
static int
netlink_route_info_encode (struct netlink_route_info *ri, char *in_buf,
                           size_t in_buf_len)
{
  size_t bytelen;
          ...
          if (nhi->type == NEXTHOP_TYPE_IPV4_IFINDEX && ri->af == AF_INET6)
            {
              ...
              nl_attr_put (&req->n, in_buf_len, RTA_GATEWAY, &ipv6, bytelen);
            }
          else
  ...
  for (nexthop_num = 0; nexthop_num < ri->num_nhs; nexthop_num++)
    {
      ...
      if (nhi->gateway)
        nl_attr_put (&req->n, in_buf_len, RTA_GATEWAY, nhi->gateway, bytelen);
 ...
 */
static void
zfpm_log_route_info (struct netlink_route_info *ri, const char *label)
{
  struct netlink_nh_info *nhi;
  unsigned int i;
  ...
  for (i = 0; i < ri->num_nhs; i++)
    {
      ...
      zfpm_debug (
          "  Intf: %u, Gateway: %s, Recursive: %s, Type: %s, Encap type: %s",
 ...
 */
int
zfpm_netlink_encode_mac (struct fpm_mac_info_t *mac, char *in_buf,
                         size_t in_buf_len)
{
  size_t buf_offset;
