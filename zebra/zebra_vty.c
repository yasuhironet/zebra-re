...
static void show_ip_route_dump_vty (struct vty *vty,
                                    struct route_table *table);
       ...
       JSON_STR)
{
  ...
  return do_show_ip_route (vty, VRF_DEFAULT_NAME, ip ? AFI_IP : AFI_IP6,
                           SAFI_MULTICAST, false, uj, 0, NULL, false, 0, 0, 0,
                           false, &ctx);
}
...
static char
re_status_output_char (const struct route_entry *re,
                       const struct nexthop *nhop, bool is_fib)
{
  if (CHECK_FLAG (re->status, ROUTE_ENTRY_INSTALLED))
    {
      bool star_p = false;

      if (nhop && ! CHECK_FLAG (nhop->flags, NEXTHOP_FLAG_DUPLICATE) &&
          ! CHECK_FLAG (nhop->flags, NEXTHOP_FLAG_RECURSIVE))
        {
          /* More-specific test for 'fib' output */
          if (is_fib)
            {
              star_p = ! ! CHECK_FLAG (nhop->flags, NEXTHOP_FLAG_FIB);
            }
          else
            star_p = true;
        }
      ...
      if (zrouter.asic_offloaded && CHECK_FLAG (re->flags, ZEBRA_FLAG_TRAPPED))
        return 't';
 ...
 */
static void
show_nh_backup_helper (struct vty *vty, const struct route_entry *re,
                       const struct nexthop *nexthop)
{
  ...
  for (i = 0; i < nexthop->backup_num; i++)
    {
      ...
      while (backup)
        {
          vty_out (vty, "  ");
          show_nexthop_detail_helper (vty, re, backup, true /*backup*/);
          vty_out (vty, "\n");
 ...
 */
static void
show_nexthop_detail_helper (struct vty *vty, const struct route_entry *re,
                            const struct nexthop *nexthop, bool is_backup)
{
  char addrstr[32];
  ...
  if (is_backup)
    vty_out (vty, "    b%s", nexthop->rparent ? "  " : "");
  else
    vty_out (vty, "  %c%s", re_status_output_char (re, nexthop, false),
             nexthop->rparent ? "  " : "");
  ...
  switch (nexthop->type)
    ...
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      vty_out (vty, " %pI4", &nexthop->gate.ipv4);
      if (nexthop->ifindex)
        vty_out (vty, ", via %s",
  ...
  if (nexthop->nh_label && nexthop->nh_label->num_labels)
    {
      vty_out (vty, ", label %s",
               mpls_label2str (nexthop->nh_label->num_labels,
                               ...
                               nexthop->nh_label_type, 1 /*pretty*/));
    }
...
static void
zebra_show_ip_route_opaque (struct vty *vty, struct route_entry *re,
                            struct json_object *json)
{
  ...
  switch (re->type)
    ...
    case ZEBRA_ROUTE_SHARP:
      if (json)
        json_object_string_add (json, "opaque", (char *) re->opaque->data);
      else
        vty_out (vty, "    Opaque Data: %s", (char *) re->opaque->data);
      break;
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "communities", bzo.community);
          json_object_string_add (json, "largeCommunities", bzo.lcommunity);
    ...
    case ZEBRA_ROUTE_OSPF6:
      ...
      if (json)
        {
          json_object_string_add (json, "ospfPathType", ozo.path_type);
          if (ozo.area_id[0] != '\0')
            json_object_string_add (json, "ospfAreaId", ozo.area_id);
          if (ozo.tag[0] != '\0')
            json_object_string_add (json, "ospfTag", ozo.tag);
        }
      else
        {
...
static void
uptime2str (time_t uptime, char *buf, size_t bufsize)
{
  time_t cur;
      ...
      if (mcast)
        {
          ...
          mcast_info = (info->safi == SAFI_MULTICAST) ? " using Multicast RIB"
                                                      : " using Unicast RIB";
        }
      ...
      for (ALL_NEXTHOPS (re->nhe->nhg, nexthop))
        {
          ...
          show_nexthop_detail_helper (vty, re, nexthop, false /*not backup*/);
          vty_out (vty, "\n");
...
vty_show_ip_route (struct vty *vty, struct route_node *rn,
                   struct route_entry *re, json_object *json, bool is_fib,
                   bool show_ng)
{
  const struct nexthop *nexthop;
  int len = 0;
  ...
  if (((re->type == ZEBRA_ROUTE_CONNECT) && (re->distance || re->metric)) ||
      (re->type != ZEBRA_ROUTE_CONNECT))
    len += vty_out (vty, " [%u/%u]", re->distance, re->metric);
  ...
  for (ALL_NEXTHOPS_PTR (nhg, nexthop))
    {
      if (first_p)
        ...
        {
          vty_out (vty, "  %c%*c", re_status_output_char (re, nexthop, is_fib),
                   len - 3 + (2 * nexthop_level (nexthop)), ' ');
        }
...
static void
vty_show_ip_route_detail_json (struct vty *vty, struct route_node *rn,
                               bool use_fib)
{
      ...
      RNODE_FOREACH_RE (rn, re)
        {
          ...
          if (ospf_instance_id && (re->type != ZEBRA_ROUTE_OSPF ||
...
static void
do_show_ip_route_all (struct vty *vty, struct zebra_vrf *zvrf, afi_t afi,
  ...
  RB_FOREACH (zrt, zebra_router_table_head, &zrouter.tables)
    {
      ...
      do_show_ip_route (vty, zvrf_name (zvrf), afi, SAFI_UNICAST, use_fib,
...
static int
do_show_ip_route (struct vty *vty, const char *vrf_name, afi_t afi,
                  safi_t safi, bool use_fib, bool use_json, route_tag_t tag,
                  const struct prefix *longer_prefix_p, bool supernets_only,
                  int type, unsigned short ospf_instance_id, uint32_t tableid,
                  bool show_ng, struct route_show_ctx *ctx)
{
  struct route_table *table;
  ...
  do_show_route_helper (vty, zvrf, table, afi, use_fib, tag, longer_prefix_p,
                        supernets_only, type, ospf_instance_id, use_json,
                        tableid, show_ng, ctx);
              ...
              if (uj)
                {
                  ...
                  json_nexthop = json_object_new_object ();
                  json_object_object_add (json, zvrf_name (zvrf), json_vrf);
  ...
  if (uj)
    {
      ...
      json_object_object_add (json_vrf, (afi == AFI_IP) ? "ipv4" : "ipv6",
                              json_nexthop);
    }
...
static void
show_nexthop_group_out (struct vty *vty, struct nhg_hash_entry *nhe,
                        json_object *json_nhe_hdr)
{
  ...
  if (json)
    {
      ...
      if (event_is_scheduled (nhe->timer))
        json_object_string_add (
            json, "timeToDeletion",
            event_timer_to_hhmmss (time_left, sizeof (time_left), nhe->timer));
  ...
  if (nhe->ifp)
    {
      if (json)
        json_object_int_add (json, "interfaceIndex", nhe->ifp->ifindex);
      else
  ...
  if (! zebra_nhg_depends_is_empty (nhe))
    {
      ...
      frr_each (nhg_connected_tree, &nhe->nhg_depends, rb_node_dep)
        {
          if (json_depends)
            json_object_array_add (json_depends,
          ...
          if (json_backup_nexthop_array)
            {
              ...
              show_nexthop_json_helper (json_backup_nexthops, nexthop, NULL);
  ...
  if (! zebra_nhg_dependents_is_empty (nhe))
    {
      ...
      if (json)
        json_object_object_add (json, "dependents", json_dependants);
      else
        vty_out (vty, "\n");
    }
  ...
  if (nhe->nhg.nhgr.buckets)
    {
      if (json)
        {
          json_object_int_add (json, "buckets", nhe->nhg.nhgr.buckets);
          json_object_int_add (json, "idleTimer", nhe->nhg.nhgr.idle_timer);
      ...
      else
        {
          vty_out (
              ...
              nhe->nhg.nhgr.unbalanced_timer, nhe->nhg.nhgr.unbalanced_time);
        }
    }
...
static int
show_nexthop_group_id_cmd_helper (struct vty *vty, uint32_t id,
                                  json_object *json)
{
...
static int
nhe_show_walker (struct hash_bucket *bucket, void *arg)
{
...
static void
show_nexthop_group_cmd_helper (struct vty *vty, struct zebra_vrf *zvrf,
                               afi_t afi, int type, json_object *json)
{
...
static void
if_nexthop_group_dump_vty (struct vty *vty, struct interface *ifp)
{
  ...
  if (vrf_all)
  ...
  else
    {
      ...
      if (table_all)
        ...
        do_show_ip_route (vty, vrf->name, afi, SAFI_UNICAST, ! ! fib, ! ! json,
          ...
          if (json)
          ...
          else
            {
              if (use_fib)
                ...
                vty_out (vty, "%% Network not in RIB\n");
            }
          return CMD_WARNING;
        }
    }
      ...
      RB_FOREACH (vrf, vrf_name_head, &vrfs_by_name)
        {
          ...
          if (table_id == 0)
            table = zebra_vrf_table (afi, SAFI_UNICAST, zvrf->vrf->vrf_id);
          else
  ...
  if (! vrf_name)
    {
      ...
      RB_FOREACH (vrf, vrf_name_head, &vrfs_by_name)
        {
          zvrf = vrf->info;
          if ((zvrf == NULL) || (zvrf->table[afi][SAFI_UNICAST] == NULL))
            continue;
                        ...
                        struct route_entry *re, unsigned int num)
{
  ...
  switch (nexthop->type)
    ...
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      ...
      if (nexthop->src.ipv4.s_addr &&
          (inet_ntop (AF_INET, &nexthop->src.ipv4, buf, sizeof (buf))))
    ...
    case NEXTHOP_TYPE_IPV6_IFINDEX:
      ...
      break;
    case NEXTHOP_TYPE_IFINDEX:
      vty_out (vty, "      Nexthop is an interface (directly connected).\n");
      vty_out (vty, "      interface index: %d\n", nexthop->ifindex);
      vty_out (vty, "      interface name: %s\n",
               ifindex2ifname (nexthop->ifindex, nexthop->vrf_id));
      break;
    case NEXTHOP_TYPE_BLACKHOLE:
      ...
      switch (nexthop->bh_type)
        ...
        case BLACKHOLE_REJECT:
          ...
          break;
        case BLACKHOLE_ADMINPROHIB:
          vty_out (vty, "      Blackhole type: admin-prohibited\n");
          break;
        case BLACKHOLE_NULL:
      ...
      RNODE_FOREACH_RE (rn, re)
        {
          ...
          if (uptime < ONE_DAY_SECOND)
            snprintf (time, sizeof (time), "%02d:%02d:%02d", tm.tm_hour,
                      tm.tm_min, tm.tm_sec);
          else if (uptime < ONE_WEEK_SECOND)
            snprintf (time, sizeof (time), "%dd%02dh%02dm", tm.tm_yday,
                      tm.tm_hour, tm.tm_min);
          else
            snprintf (time, sizeof (time), "%02dw%dd%02dh", tm.tm_yday / 7,
          ...
          for (ALL_NEXTHOPS_PTR (&(re->nhe->nhg), nexthop))
            {
              nexthop_num++;
              show_ip_route_nht_dump (vty, nexthop, re, nexthop_num);
            }
  ...
  if (use_json)
    {
      ...
      json_object_object_add (json_route_summary, "routes", json_route_routes);
    }
          ...
          if (i == ZEBRA_ROUTE_BGP)
            {
              if (use_json)
                {
                  json_object *json_route_ebgp = json_object_new_object ();

                  json_object_int_add (json_route_ebgp, "fib",
                                       fib_cnt[ZEBRA_ROUTE_BGP]);
                  json_object_int_add (json_route_ebgp, "rib",
                                       rib_cnt[ZEBRA_ROUTE_BGP]);
                  json_object_int_add (json_route_ebgp, "fibOffLoaded",
                                       offload_cnt[ZEBRA_ROUTE_BGP]);
                  json_object_int_add (json_route_ebgp, "fibTrapped",
                                       trap_cnt[ZEBRA_ROUTE_BGP]);

                  json_object_string_add (json_route_ebgp, "type", "ebgp");
                  json_object_array_add (json_route_routes, json_route_ebgp);

                  json_object *json_route_ibgp = json_object_new_object ();

                  json_object_int_add (json_route_ibgp, "fib",
                                       fib_cnt[ZEBRA_ROUTE_IBGP]);
                  json_object_int_add (json_route_ibgp, "rib",
                                       rib_cnt[ZEBRA_ROUTE_IBGP]);
                  json_object_int_add (json_route_ibgp, "fibOffLoaded",
                                       offload_cnt[ZEBRA_ROUTE_IBGP]);
                  json_object_int_add (json_route_ibgp, "fibTrapped",
                                       trap_cnt[ZEBRA_ROUTE_IBGP]);
                  json_object_string_add (json_route_ibgp, "type", "ibgp");
                  json_object_array_add (json_route_routes, json_route_ibgp);
                }
              else
                {
          ...
          else
            {
              if (use_json)
                {
                  json_object *json_route_type = json_object_new_object ();

                  json_object_int_add (json_route_type, "fib", fib_cnt[i]);
                  json_object_int_add (json_route_type, "rib", rib_cnt[i]);

                  json_object_int_add (json_route_type, "fibOffLoaded",
                                       offload_cnt[i]);
                  json_object_int_add (json_route_type, "fibTrapped",
                                       trap_cnt[i]);
                  json_object_string_add (json_route_type, "type",
                                          zebra_route_string (i));
                  json_object_array_add (json_route_routes, json_route_type);
                }
              else
                vty_out (vty, "%-20s %-20d %-20d \n", zebra_route_string (i),
                         rib_cnt[i], fib_cnt[i]);
  ...
  RB_FOREACH (vrf, vrf_name_head, &vrfs_by_name)
    {
      ...
      if (zvrf_id (zvrf) == VRF_UNKNOWN || ! zvrf_is_active (zvrf))
        ...
        vty_out (vty, "id %u netns %s", zvrf_id (zvrf), zvrf_ns_name (zvrf));
      else
...
DEFPY (evpn_mh_mac_holdtime,
       ...
       "Duration in seconds\n")
{
  return zebra_evpn_mh_mac_holdtime_update (vty, duration, no ? true : false);
}
       ...
       "duration in seconds\n")
{
  ...
  return zebra_evpn_mh_startup_delay_update (vty, duration, no ? true : false);
}
       ...
       "Prefix routes only \n")
{
  ...
  snprintf (xpath, sizeof (xpath),
            FRR_VRF_KEY_XPATH "/frr-zebra:zebra/l3vni-id", VRF_DEFAULT_NAME);
       ...
       "prefix-routes-only\n")
{
  ...
  if (filter)
    nb_cli_enqueue_change (vty, "./frr-zebra:zebra/prefix-only", NB_OP_MODIFY,
                           "true");
       ...
       "prefix-routes-only\n")
{
  ...
  if (filter)
    nb_cli_enqueue_change (vty, "./frr-zebra:zebra/prefix-only", NB_OP_DESTROY,
                           "true");
...
DEFUN (show_evpn_mac_vni_all_detail,
...
DEFUN (show_evpn_neigh_vni_all_detail,
...
DEFPY (clear_evpn_dup_addr,
       ...
       "clear evpn dup-addr vni <all$vni_all |"
       CMD_VNI_RANGE
       ...
       "IPv6 address\n")
{
  ...
  int ret = CMD_SUCCESS;
  struct list *input;
  struct yang_data *yang_dup = NULL, *yang_dup_ip = NULL, *yang_dup_mac = NULL;
  ...
  if (! vni_str)
    {
      yang_dup = yang_data_new (
          "/frr-zebra:clear-evpn-dup-addr/input/clear-dup-choice", "all-case");
    }
  else
    {
  ...
  if (yang_dup)
    {
      ...
      ret = nb_cli_rpc (vty, "/frr-zebra:clear-evpn-dup-addr", input, NULL);
    }
       ...
       "route-map name\n")
{
  ...
  ret = zebra_import_table (AFI_IP, VRF_DEFAULT, table_id, distance, rmap, 1);
  if (rmap)
    XFREE (MTYPE_ROUTE_MAP_NAME, rmap);
              ...
              "Number of packets to process before relinquishing thread\n")
{
  atomic_store_explicit (&zrouter.packets_to_process,
                         ZEBRA_ZAPI_PACKETS_TO_PROCESS, memory_order_relaxed);
...
DEFUN (zebra_on_rib_process_script,
