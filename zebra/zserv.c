 ...
 */
void
zserv_client_delete (struct zserv *client)
{
 ...
 */
void
zserv_log_message (const char *errmsg, struct stream *msg, struct zmsghdr *hdr)
{
 ...
 */
static void
zserv_client_fail (struct zserv *client)
{
 ...
 */
static void
zserv_write (struct event *thread)
{
 ...
 * previous invocation of this task and needs to be resumed to finish a
 * message. Otherwise, the socket data stream is assumed to be at the beginning
 * of a new ZAPI message (specifically at the header). The header is read and
 * validated. If the header passed validation then the length field found in
 * the header is used to compute the total length of the message. That much
 * data is read (but not inspected), appended to the header, placed into a
 * stream and pushed onto the client's input queue. A task is then scheduled on
 * the main thread to process the client's input queue. Finally, if all of this
 * was successful, this task reschedules itself.
 *
 * Any failure in any of these actions is handled by terminating the client.
 */
static void
zserv_read (struct event *thread)
{
  struct zserv *client = EVENT_ARG (thread);
  int sock;
  size_t already;
  ...
  p2p_orig =
      atomic_load_explicit (&zrouter.packets_to_process, memory_order_relaxed);
  cache = stream_fifo_new ();
  p2p = p2p_orig;
  ...
  while (p2p)
    {
      ...
      if (! hdrvalid)
        {
          snprintf (errmsg, sizeof (errmsg), "%s: Message has corrupt header",
                    __func__);
      ...
      if (already < hdr.length)
        {
          nb = stream_read_try (client->ibuf_work, sock, hdr.length - already);
          if ((nb == 0 || nb == -1))
            {
              if (IS_ZEBRA_DEBUG_EVENT)
      ...
      if (IS_ZEBRA_DEBUG_PACKET)
        zlog_debug ("zebra message[%s:%u:%u] comes from socket [%d]",
                    zserv_command_string (hdr.command), hdr.vrf_id, hdr.length,
                    sock);
  ...
  if (p2p < p2p_orig)
    {
      ...
      frr_with_mutex (&client->ibuf_mtx)
        {
          while (cache->head)
            stream_fifo_push (client->ibuf_fifo, stream_fifo_pop (cache));
        }
...
static void
zserv_client_event (struct zserv *client, enum zserv_client_event event)
{
  switch (event)
    {
 ...
 *
 * Each message is popped off the client's input queue and the action
 * associated with the message is executed. This proceeds until there are no
 * more messages, an error occurs, or the processing limit is reached.
 *
 ...
 */
static void
zserv_process_messages (struct event *thread)
{
  struct zserv *client = EVENT_ARG (thread);
  struct stream *msg;
...
int
zserv_send_message (struct zserv *client, struct stream *msg)
{
  frr_with_mutex (&client->obuf_mtx)
    {
 ...
 */
int
zserv_send_batch (struct zserv *client, struct stream_fifo *fifo)
{
  struct stream *msg;
 ...
 */
static void
zserv_client_free (struct zserv *client)
{
  if (client == NULL)
    return;
      ...
      if (DYNAMIC_CLIENT_GR_DISABLED (client))
        {
          ...
          zlog_notice (
              ...
              client->sock, nnhgs, zebra_route_string (client->proto));
        }
      client->sock = -1;
    }
...
void
zserv_close_client (struct zserv *client)
{
  bool free_p = true;
  ...
  if (client->pthread)
    {
      ...
      if (IS_ZEBRA_DEBUG_EVENT)
        zlog_debug ("Closing client '%s'", zebra_route_string (client->proto));
 ...
 */
static void
zserv_handle_client_fail (struct event *thread)
{
  struct zserv *client = EVENT_ARG (thread);
 ...
 */
struct zserv *
zserv_acquire_client (uint8_t proto, unsigned short instance,
                      uint32_t session_id)
{
 ...
 */
void
zserv_release_client (struct zserv *client)
{
  /*
      ...
      if (client->busy_count <= 0)
        {
           ...
           */
          if (client->is_closed)
            event_add_event (zrouter.master, zserv_handle_client_fail, client,
                             0, &client->t_cleanup);
        }
    }
 ...
 */
static void
zserv_accept (struct event *thread)
{
  int accept_sock;
  int client_sock;
...
zserv_start (char *path)
{
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "Can't bind zserv socket on %s: %s", path,
                    safe_strerror (errno));
      close (zsock);
      zsock = -1;
      return;
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "Can't listen to zserv socket %s: %s", path,
                    safe_strerror (errno));
      close (zsock);
      zsock = -1;
      return;
...
void
zserv_event (struct zserv *client, enum zserv_event event)
{
  switch (event)
    {
    case ZSERV_ACCEPT:
    ...
    case ZSERV_PROCESS_MESSAGES:
      event_add_event (zrouter.master, zserv_process_messages, client, 0,
                       &client->t_process);
      break;
    case ZSERV_HANDLE_CLIENT_FAIL:
      event_add_event (zrouter.master, zserv_handle_client_fail, client, 0,
                       &client->t_cleanup);
    }
}
...
zebra_show_client_detail (struct vty *vty, struct zserv *client)
{
  ...
  if (client->nh_reg_time)
    {
      ...
      if (client->nh_last_upd_time)
        vty_out (
            vty, "Nexthop Last Update Time: %s \n",
  ...
  vty_out (vty, "NHT v4      %-12u%-12u%-12u\n", client->v4_nh_watch_add_cnt,
           0, client->v4_nh_watch_rem_cnt);
  vty_out (vty, "NHT v6      %-12u%-12u%-12u\n", client->v6_nh_watch_add_cnt,
           0, client->v6_nh_watch_rem_cnt);
...
static void
zebra_show_stale_client_detail (struct vty *vty, struct zserv *client)
{
          ...
          if (info->stale_client_ptr)
            {
              ...
              if (info->t_stale_removal)
                {
                  vty_out (vty, "Stale delete timer: %ld sec\n",
                           event_timer_remain_second (info->t_stale_removal));
                }
            }
        }
    }
 ...
 */
static struct zserv *
find_client_internal (uint8_t proto, unsigned short instance,
                      uint32_t session_id)
{
 ...
 */
struct zserv *
zserv_find_client (uint8_t proto, unsigned short instance)
{
  struct zserv *client;
 ...
 */
struct zserv *
zserv_find_client_session (uint8_t proto, unsigned short instance,
                           uint32_t session_id)
{
  struct zserv *client;
...
static int
zserv_client_close_cb (struct zserv *closed_client)
{
  struct listnode *node, *nnode;
  struct zserv *client = NULL;
