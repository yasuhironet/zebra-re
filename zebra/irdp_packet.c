...
static void
parse_irdp_packet (char *p, int len, struct interface *ifp)
{
  ...
  if (! ((ntohl (ip->ip_dst.s_addr) == INADDR_BROADCAST) &&
       ...
       ! (irdp->flags & IF_BROADCAST)))
    {
      flog_warn (
          ...
          ifp->name, irdp->flags & IF_BROADCAST ? "broadcast" : "multicast");
      return;
    }
  ...
  switch (icmp->type)
    ...
    case ICMP_ROUTERSOLICIT:
      ...
      if (irdp->flags & IF_DEBUG_MESSAGES)
        zlog_debug ("IRDP: RX Solicit on %s from %pI4", ifp->name, &src);
...
static int
irdp_recvmsg (int sock, uint8_t *buf, int size, int *ifindex)
{
  struct msghdr msg;
  struct iovec iov;
  ...
  if (msg.msg_flags & MSG_CTRUNC)
    {
      flog_warn (EC_LIB_SOCKET, "IRDP: recvmsg: truncated control message");
      return ret;
    }
...
irdp_read_raw (struct event *r)
{
  ...
  event_add_read (zrouter.master, irdp_read_raw, NULL, irdp_sock, &t_irdp_raw);
  ...
  if (! (irdp->flags & IF_ACTIVE))
    {
      ...
      if (irdp->flags & IF_DEBUG_MISC)
        zlog_debug ("IRDP: RX ICMP for disabled interface %s", ifp->name);
      return;
    }
...
void
send_packet (struct interface *ifp, struct stream *s, uint32_t dst,
             struct prefix *p, uint32_t ttl)
{
  ...
  if (setsockopt (irdp_sock, IPPROTO_IP, IP_HDRINCL, (char *) &on,
                  sizeof (on)) < 0)
    flog_err (EC_LIB_SOCKET, "IRDP: Cannot set IP_HDRINCLU %s(%d) on %s",
  ...
  if (dst == INADDR_BROADCAST)
    {
      ...
      if (setsockopt (irdp_sock, SOL_SOCKET, SO_BROADCAST, &bon,
                      sizeof (bon)) < 0)
        flog_err (EC_LIB_SOCKET, "IRDP: Cannot set SO_BROADCAST %s(%d) on %s",
                  safe_strerror (errno), errno, ifp->name);
    }
  ...
  if (sendmsg (irdp_sock, msg, 0) < 0)
    flog_err (EC_LIB_SOCKET, "IRDP: sendmsg send failure %s(%d) on %s",
              safe_strerror (errno), errno, ifp->name);
}
