...
static int zl3vni_nh_install (struct zebra_l3vni *zl3vni,
...
static int svd_nh_uninstall (struct zebra_l3vni *zl3vni,
                             struct zebra_neigh *n);
...
static struct zebra_mac *zl3vni_rmac_add (struct zebra_l3vni *zl3vni,
                                          const struct ethaddr *rmac);
static int zl3vni_rmac_del (struct zebra_l3vni *zl3vni,
...
bool
zebra_evpn_do_dup_addr_detect (struct zebra_vrf *zvrf)
{
...
/* Private functions */
static int
host_rb_entry_compare (const struct host_rb_entry *hle1,
                       const struct host_rb_entry *hle2)
{
  ...
  if (hle1->p.family == AF_INET)
    ...
    {
      return memcmp (&hle1->p.u.prefix6, &hle2->p.u.prefix6, IPV6_MAX_BYTELEN);
    }
  else if (hle1->p.family == AF_EVPN)
    {
       ...
       */
      family1 =
          is_evpn_prefix_ipaddr_v4 ((const struct prefix_evpn *) &hle1->p)
              ? AF_INET
              : AF_INET6;
      family2 =
          is_evpn_prefix_ipaddr_v4 ((const struct prefix_evpn *) &hle2->p)
              ? AF_INET
              : AF_INET6;
    ...
    {
      zlog_debug ("%s: Unexpected family type: %d", __func__, hle1->p.family);
      return 0;
    }
}
RB_GENERATE (host_rb_tree_entry, host_rb_entry, hl_entry,
             host_rb_entry_compare);

static uint32_t
rb_host_count (struct host_rb_tree_entry *hrbe)
{
...
static int
l3vni_rmac_nh_list_cmp (void *p1, void *p2)
{
...
static void
l3vni_rmac_nh_free (struct ipaddr *vtep_ip)
{
  XFREE (MTYPE_EVPN_VTEP, vtep_ip);
}

static void
l3vni_rmac_nh_list_nh_delete (struct zebra_l3vni *zl3vni,
                              struct zebra_mac *zrmac, struct ipaddr *vtep_ip)
{
 ...
 */
static void
zevpn_print_neigh_hash_all_evpn (struct hash_bucket *bucket, void **args)
{
  struct vty *vty;
  ...
  wctx.addr_width = 15;
  wctx.json = json_evpn;
  hash_iterate (zevpn->neigh_table, zebra_evpn_find_neigh_addr_width, &wctx);

  if (json == NULL)
    zebra_evpn_print_neigh_hdr (vty, &wctx);

  if (print_dup)
    hash_iterate (zevpn->neigh_table, zebra_evpn_print_dad_neigh_hash, &wctx);
  else
 ...
 */
static void
zevpn_print_neigh_hash_all_evpn_detail (struct hash_bucket *bucket,
                                        void **args)
{
  struct vty *vty;
  json_object *json = NULL, *json_evpn = NULL;
  ...
  if (print_dup)
    hash_iterate (zevpn->neigh_table, zebra_evpn_print_dad_neigh_hash_detail,
                  &wctx);
  else
    hash_iterate (zevpn->neigh_table, zebra_evpn_print_neigh_hash_detail,
                  &wctx);
...
static void
zl3vni_print_nh (struct zebra_neigh *n, struct vty *vty, json_object *json)
{
  ...
  if (! json)
  ...
  else
    {
      ...
      if (n->refcnt)
        ...
        {
          json_object_int_add (json, "refCount", rb_host_count (&n->host_rb));
          RB_FOREACH (hle, host_rb_tree_entry, &n->host_rb)
            json_object_array_add (
                json_hosts, json_object_new_string (
...
static void
zl3vni_print_rmac (struct zebra_mac *zrmac, struct vty *vty, json_object *json)
{
 ...
 */
static void
zevpn_print_mac_hash_all_evpn (struct hash_bucket *bucket, void *ctxt)
{
  struct vty *vty;
  json_object *json = NULL, *json_evpn = NULL;
 ...
 */
static void
zevpn_print_mac_hash_all_evpn_detail (struct hash_bucket *bucket, void *ctxt)
{
  struct vty *vty;
  json_object *json = NULL, *json_evpn = NULL;
  json_object *json_mac = NULL;
  ...
  wctx->json = json_mac;
  if (wctx->print_dup)
    hash_iterate (zevpn->mac_table, zebra_evpn_print_dad_mac_hash_detail,
                  wctx);
  else
    hash_iterate (zevpn->mac_table, zebra_evpn_print_mac_hash_detail, wctx);
  wctx->json = json;
  if (json)
    {
      if (wctx->count)
...
static void
zl3vni_print_nh_hash (struct hash_bucket *bucket, void *ctx)
{
  ...
  if (! json_evpn)
  ...
  else
    {
      ...
      json_object_object_add (
          json_evpn, ipaddr2str (&(n->ip), buf2, sizeof (buf2)), json_nh);
    }
}

static void
zl3vni_print_nh_all_table (struct hash *nh_table, vni_t vni, struct vty *vty,
                           json_object *json)
{
  uint32_t num_nh = 0;
...
static void
zl3vni_print_nh_hash_all_vni (struct hash_bucket *bucket, void **args)
{
  struct vty *vty = NULL;
...
static void
zl3vni_print_rmac_hash_all_vni (struct hash_bucket *bucket, void **args)
{
  struct vty *vty = NULL;
  json_object *json = NULL;
  json_object *json_evpn = NULL;
...
static void
zl3vni_print_rmac_hash (struct hash_bucket *bucket, void *ctx)
{
...
static void
zl3vni_print (struct zebra_l3vni *zl3vni, void **ctx)
{
  char buf[PREFIX_STRLEN];
  struct vty *vty = NULL;
  json_object *json = NULL;
  ...
  if (! json)
  ...
  else
    {
      ...
      json_object_string_add (json, "tenantVrf", zl3vni_vrf_name (zl3vni));
      json_object_string_addf (json, "localVtepIp", "%pI4",
                               &zl3vni->local_vtep_ip);
      json_object_string_add (json, "vxlanIntf",
                              zl3vni_vxlan_if_name (zl3vni));
      json_object_string_add (json, "sviIntf", zl3vni_svi_if_name (zl3vni));
...
static void
zl3vni_print_hash (struct hash_bucket *bucket, void *ctx[])
{
  struct vty *vty = NULL;
  json_object *json = NULL;
  json_object *json_evpn = NULL;
  ...
  if (! json)
    {
      vty_out (vty, "%-10u %-4s %-21s %-8lu %-8lu %-15s %-37s\n", zl3vni->vni,
               ...
               hashcount (zl3vni->rmac_table), hashcount (zl3vni->nh_table),
               "n/a", zl3vni_vrf_name (zl3vni));
    }
  else
    {
      char vni_str[VNI_STR_LEN];
...
static void
zl3vni_print_hash_detail (struct hash_bucket *bucket, void *data)
{
  struct vty *vty = NULL;
  struct zebra_l3vni *zl3vni = NULL;
  ...
  zebra_vxlan_print_vni (vty, zes->zvrf, zl3vni->vni, use_json, json_array);
...
static int
zvni_map_to_svi_ns (struct ns *ns, void *_in_param, void **_p_ifp)
{
 ...
 */
struct interface *
zvni_map_to_svi (vlanid_t vid, struct interface *br_if)
{
  struct interface *tmp_if = NULL;
  struct zebra_if *zif;
  struct zebra_from_svi_param in_param;
  ...
  ns_walk_func (zvni_map_to_svi_ns, (void *) &in_param, (void **) p_ifp);
  return tmp_if;
}
...
static int
zevpn_build_vni_hash_table (struct zebra_if *zif, struct zebra_vxlan_vni *vnip,
                            void *arg)
{
  vni_t vni;
  struct zebra_evpn *zevpn;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("Build vni table for vni %u for Intf %s", vni, ifp->name);
  ...
  if (zl3vni)
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("create L3-VNI hash for Intf %s(%u) L3-VNI %u", ifp->name,
                    ifp->ifindex, vni);
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("create l3vni %u svi_if %s mac_vlan_if %s", vni,
...
static int
zevpn_build_hash_table_zns (struct ns *ns,
  ...
  for (rn = route_top (zns->if_table); rn; rn = route_next (rn))
    {
      ...
      if (zvrf->zns->ns_id != vxl->link_nsid)
        {
          if (IS_ZEBRA_DEBUG_VXLAN)
            zlog_debug ("Intf %s(%u) link not in same "
 ...
 */
static void
zebra_evpn_vxlan_cleanup_all (struct hash_bucket *bucket, void *arg)
{
  struct zebra_evpn *zevpn = NULL;
  struct zebra_l3vni *zl3vni = NULL;
...
/* cleanup L3VNI */
static void
zl3vni_cleanup_all (struct hash_bucket *bucket, void *args)
{
  struct zebra_l3vni *zl3vni = NULL;
...
static void
rb_find_or_add_host (struct host_rb_tree_entry *hrbe,
...
static void
rb_delete_host (struct host_rb_tree_entry *hrbe, struct prefix *host)
{
  struct host_rb_entry lookup;
  struct host_rb_entry *hle;
...
zl3vni_rmac_lookup (struct zebra_l3vni *zl3vni, const struct ethaddr *rmac)
{
 ...
 */
static struct zebra_mac *
zl3vni_rmac_add (struct zebra_l3vni *zl3vni, const struct ethaddr *rmac)
{
 ...
 */
static int
zl3vni_rmac_del (struct zebra_l3vni *zl3vni, struct zebra_mac *zrmac)
{
  struct zebra_mac *tmp_rmac;
 ...
 */
static int
zl3vni_rmac_install (struct zebra_l3vni *zl3vni, struct zebra_mac *zrmac)
{
 ...
 */
static int
zl3vni_rmac_uninstall (struct zebra_l3vni *zl3vni, struct zebra_mac *zrmac)
{
...
/* handle rmac add */
static int
zl3vni_remote_rmac_add (struct zebra_l3vni *zl3vni, const struct ethaddr *rmac,
                        const struct ipaddr *vtep_ip)
{
  struct zebra_mac *zrmac = NULL;
  struct ipaddr *vtep = NULL;
  ...
  if (! zrmac)
    {
      ...
      hook_call (zebra_rmac_update, zrmac, zl3vni, false, "new RMAC added");

      /* install rmac in kernel */
      zl3vni_rmac_install (zl3vni, zrmac);
    }
  else if (! IPV4_ADDR_SAME (&zrmac->fwd_info.r_vtep_ip, &vtep_ip->ipaddr_v4))
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
...
/* handle rmac delete */
static void
zl3vni_remote_rmac_del (struct zebra_l3vni *zl3vni, struct zebra_mac *zrmac,
          ...
          listcount (zrmac->nh_list))
        {
          ...
          if (IS_ZEBRA_DEBUG_VXLAN)
            zlog_debug (
                ...
                zl3vni->vni, &ipv4_vtep, &zrmac->fwd_info.r_vtep_ip,
                &zrmac->macaddr);
      ...
      if (! listcount (zrmac->nh_list))
        {
          ...
          if (IS_ZEBRA_DEBUG_VXLAN)
            zlog_debug ("L3VNI %u RMAC %pEA vtep_ip %pIA delete", zl3vni->vni,
                        &zrmac->macaddr, vtep_ip);
 ...
 */
static struct zebra_neigh *
_nh_lookup (struct zebra_l3vni *zl3vni, const struct ipaddr *ip)
{
 ...
 */
static struct zebra_neigh *
zl3vni_nh_lookup (struct zebra_l3vni *zl3vni, const struct ipaddr *ip)
{
 ...
 */
static struct zebra_neigh *
svd_nh_lookup (const struct ipaddr *ip)
{
 ...
 */
static struct zebra_neigh *
_nh_add (struct zebra_l3vni *zl3vni, const struct ipaddr *ip,
 ...
 */
static struct zebra_neigh *
zl3vni_nh_add (struct zebra_l3vni *zl3vni, const struct ipaddr *ip,
               const struct ethaddr *mac)
{
 ...
 */
static int
zl3vni_nh_del (struct zebra_l3vni *zl3vni, struct zebra_neigh *n)
{
  struct zebra_neigh *tmp_n;
  struct host_rb_entry *hle;
 ...
 */
static struct zebra_neigh *
svd_nh_add (const struct ipaddr *ip, const struct ethaddr *mac)
{
 ...
 */
static int
svd_nh_del (struct zebra_neigh *n)
{
  if (n->refcnt > 0)
    return -1;
 ...
 */
static int
_nh_install (struct zebra_l3vni *zl3vni, struct interface *ifp,
             struct zebra_neigh *n)
{
  uint8_t flags;
  int ret = 0;
  ...
  dplane_rem_neigh_add (ifp, &n->ip, &n->emac, flags, false /*was_static*/);
 ...
 */
static int
_nh_uninstall (struct interface *ifp, struct zebra_neigh *n)
{
  if (! (n->flags & ZEBRA_NEIGH_REMOTE) ||
      ! (n->flags & ZEBRA_NEIGH_REMOTE_NH))
    return 0;
 ...
 */
static int
zl3vni_nh_install (struct zebra_l3vni *zl3vni, struct zebra_neigh *n)
{
 ...
 */
static int
zl3vni_nh_uninstall (struct zebra_l3vni *zl3vni, struct zebra_neigh *n)
{
 ...
 */
static int
svd_nh_install (struct zebra_l3vni *zl3vni, struct zebra_neigh *n)
{
 ...
 */
static int
svd_nh_uninstall (struct zebra_l3vni *zl3vni, struct zebra_neigh *n)
{
...
static int
zl3vni_remote_nh_add (struct zebra_l3vni *zl3vni, const struct ipaddr *vtep_ip,
                      const struct ethaddr *rmac,
  ...
  else if (memcmp (&nh->emac, rmac, ETH_ALEN) != 0)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug (
            ...
            zl3vni->vni, &nh->emac, rmac, vtep_ip, host_prefix);
...
static void
zl3vni_remote_nh_del (struct zebra_l3vni *zl3vni, struct zebra_neigh *nh,
...
static int
svd_remote_nh_add (struct zebra_l3vni *zl3vni, const struct ipaddr *vtep_ip,
                   const struct ethaddr *rmac,
                   const struct prefix *host_prefix)
{
  struct zebra_neigh *nh = NULL;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("SVD NH ADD refcnt (%u) for nexthop %pIA", nh->refcnt,
                vtep_ip);
...
static int
svd_remote_nh_del (struct zebra_l3vni *zl3vni, const struct ipaddr *vtep_ip)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("SVD NH Del refcnt (%u) for nexthop %pIA", nh->refcnt,
                vtep_ip);
 ...
 */
static int
zl3vni_local_nh_add_update (struct zebra_l3vni *zl3vni, struct ipaddr *ip,
                            uint16_t state)
{
#ifdef GNU_LINUX
  struct zebra_neigh *n = NULL;
...
static int
zl3vni_local_nh_del (struct zebra_l3vni *zl3vni, struct ipaddr *ip)
{
  struct zebra_neigh *n = NULL;
 ...
 */
static unsigned int
l3vni_hash_keymake (const void *p)
{
 ...
 */
static bool
l3vni_hash_cmp (const void *p1, const void *p2)
{
 ...
 */
static struct zebra_l3vni *
zl3vni_add (vni_t vni, vrf_id_t vrf_id)
{
 ...
 */
static int
zl3vni_del (struct zebra_l3vni *zl3vni)
{
  struct zebra_l3vni *tmp_zl3vni;
...
static int
zl3vni_map_to_vxlan_if_ns (struct ns *ns, void *_zl3vni, void **_pifp)
{
  struct zebra_ns *zns = ns->info;
  ...
  for (rn = route_top (zns->if_table); rn; rn = route_next (rn))
    {
      ...
      if (zvrf->zns->ns_id != vxl->link_nsid)
        {
          if (IS_ZEBRA_DEBUG_VXLAN)
            zlog_debug ("Intf %s(%u) VNI %u, link not in same "
                        "namespace than BGP EVPN core instance ",
...
struct interface *
zl3vni_map_to_vxlan_if (struct zebra_l3vni *zl3vni)
{
  struct interface **p_ifp;
  struct interface *ifp = NULL;
  ...
  ns_walk_func (zl3vni_map_to_vxlan_if_ns, (void *) zl3vni, (void **) p_ifp);
  return ifp;
}

struct interface *
zl3vni_map_to_svi_if (struct zebra_l3vni *zl3vni)
{
...
struct interface *
zl3vni_map_to_mac_vlan_if (struct zebra_l3vni *zl3vni)
{
  struct zebra_if *zif = NULL; /* zebra_if for vxlan_if */
  ...
  return zebra_evpn_map_to_macvlan (zif->brslave_info.br_if, zl3vni->svi_if);
}
...
static int
zl3vni_from_svi_ns (struct ns *ns, void *_in_param, void **_p_zl3vni)
{
  int found = 0;
  vni_t vni_id = 0;
  struct zebra_ns *zns = ns->info;
 ...
 */
static struct zebra_l3vni *
zl3vni_from_svi (struct interface *ifp, struct interface *br_if)
{
  struct zebra_l3vni *zl3vni = NULL;
  struct zebra_if *zif = NULL;
  struct zebra_from_svi_param in_param = {};
...
vni_t
vni_id_from_svi (struct interface *ifp, struct interface *br_if)
{
  vni_t vni = 0;
  struct zebra_evpn *zevpn = NULL;
  struct zebra_l3vni *zl3vni = NULL;
...
static inline void
zl3vni_get_vrr_rmac (struct zebra_l3vni *zl3vni, struct ethaddr *rmac)
{
  if (! zl3vni)
    return;
 ...
 */
static int
zl3vni_send_add_to_client (struct zebra_l3vni *zl3vni)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug (
        ...
        zl3vni->vni, vrf_id_to_name (zl3vni_vrf_id (zl3vni)), &svi_rmac,
 ...
 */
static int
zl3vni_send_del_to_client (struct zebra_l3vni *zl3vni)
{
  struct stream *s = NULL;
  struct zserv *client = NULL;
...
void
zebra_vxlan_process_l3vni_oper_up (struct zebra_l3vni *zl3vni)
{
  if (! zl3vni)
    return;
...
void
zebra_vxlan_process_l3vni_oper_down (struct zebra_l3vni *zl3vni)
{
  if (! zl3vni)
    return;
...
static void
zevpn_add_to_l3vni_list (struct hash_bucket *bucket, void *ctxt)
{
 ...
 */
static int
zebra_vxlan_handle_vni_transition (struct zebra_vrf *zvrf, vni_t vni, int add)
{
  struct zebra_evpn *zevpn = NULL;
  struct zebra_l3vni *zl3vni = NULL;
  ...
  if (add)
      ...
      if (zebra_evpn_vxlan_del (zevpn))
        {
          flog_err (EC_ZEBRA_VNI_DEL_FAILED,
                    "Failed to del EVPN hash %p, VNI %u", zevpn, zevpn->vni);
          return -1;
        }
    }
  else
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("Adding L2-VNI %u - transition from L3-VNI", vni);
...
static void
zl3vni_del_rmac_hash_entry (struct hash_bucket *bucket, void *ctx)
{
  struct zebra_mac *zrmac = NULL;
  struct zebra_l3vni *zl3vni = NULL;
...
static void
zl3vni_del_nh_hash_entry (struct hash_bucket *bucket, void *ctx)
{
  struct zebra_neigh *n = NULL;
  struct zebra_l3vni *zl3vni = NULL;
...
static int
zebra_vxlan_readd_remote_rmac (struct zebra_l3vni *zl3vni,
                               struct ethaddr *rmac)
{
  struct zebra_mac *zrmac = NULL;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("Del remote RMAC %pEA L3VNI %u - readd", rmac, zl3vni->vni);
...
int
is_l3vni_for_prefix_routes_only (vni_t vni)
{
  struct zebra_l3vni *zl3vni = NULL;
...
void
zebra_vxlan_evpn_vrf_route_add (vrf_id_t vrf_id, const struct ethaddr *rmac,
                                const struct ipaddr *vtep_ip,
                                const struct prefix *host_prefix)
{
  struct zebra_l3vni *zl3vni = NULL;
...
void
zebra_vxlan_evpn_vrf_route_del (vrf_id_t vrf_id, struct ipaddr *vtep_ip,
                                struct prefix *host_prefix)
{
  struct zebra_l3vni *zl3vni = NULL;
  struct zebra_neigh *nh = NULL;
...
void
zebra_vxlan_print_specific_rmac_l3vni (struct vty *vty, vni_t l3vni,
  ...
  if (! zrmac)
    {
      if (use_json)
        ...
        vty_out (vty, "%% Requested RMAC doesn't exist in L3-VNI %u\n", l3vni);
      return;
    }
...
void
zebra_vxlan_print_rmacs_l3vni (struct vty *vty, vni_t l3vni, bool use_json)
{
  struct zebra_l3vni *zl3vni;
  uint32_t num_rmacs;
  struct rmac_walk_ctx wctx;
...
void
zebra_vxlan_print_rmacs_all_l3vni (struct vty *vty, bool use_json)
{
  json_object *json = NULL;
  void *args[2];
  ...
  hash_iterate (
      zrouter.l3vni_table,
      (void (*) (struct hash_bucket *, void *)) zl3vni_print_rmac_hash_all_vni,
      args);
...
void
zebra_vxlan_print_specific_nh_l3vni (struct vty *vty, vni_t l3vni,
...
static void
l3vni_print_nh_table (struct hash *nh_table, struct vty *vty, bool use_json)
{
  uint32_t num_nh;
  struct nh_walk_ctx wctx;
  json_object *json = NULL;
...
void
zebra_vxlan_print_nh_l3vni (struct vty *vty, vni_t l3vni, bool use_json)
{
  struct zebra_l3vni *zl3vni = NULL;
...
void
zebra_vxlan_print_nh_svd (struct vty *vty, bool use_json)
{
  if (! is_evpn_enabled ())
    {
      if (use_json)
...
void
zebra_vxlan_print_nh_all_l3vni (struct vty *vty, bool use_json)
{
  json_object *json = NULL;
  void *args[2];
  ...
  hash_iterate (
      zrouter.l3vni_table,
      (void (*) (struct hash_bucket *, void *)) zl3vni_print_nh_hash_all_vni,
      args);
 ...
 */
void
zebra_vxlan_print_l3vni (struct vty *vty, vni_t vni, bool use_json)
{
  void *args[2];
  json_object *json = NULL;
  struct zebra_l3vni *zl3vni = NULL;
...
void
zebra_vxlan_print_vrf_vni (struct vty *vty, struct zebra_vrf *zvrf,
  ...
  if (! json_vrfs)
    {
      vty_out (vty, "%-37s %-10u %-20s %-20s %-5s %-18s\n", zvrf_name (zvrf),
               zl3vni->vni, zl3vni_vxlan_if_name (zl3vni),
  ...
  else
    {
      ...
      json_object_string_add (json_vrf, "sviIntf",
                              zl3vni_svi_if_name (zl3vni));
      json_object_string_add (json_vrf, "state", zl3vni_state2str (zl3vni));
 ...
 */
void
zebra_vxlan_print_neigh_all_vni_detail (struct vty *vty,
                                        struct zebra_vrf *zvrf, bool print_dup,
                                        bool use_json)
{
  json_object *json = NULL;
  void *args[3];
 ...
 */
void
zebra_vxlan_print_neigh_vni_dad (struct vty *vty, struct zebra_vrf *zvrf,
                                 vni_t vni, bool use_json)
{
  struct zebra_evpn *zevpn;
  uint32_t num_neigh;
 ...
 */
void
zebra_vxlan_print_macs_all_vni_detail (struct vty *vty, struct zebra_vrf *zvrf,
                                       bool print_dup, bool use_json)
{
  struct mac_walk_ctx wctx;
  json_object *json = NULL;
  ...
  wctx.json = json;
  wctx.print_dup = print_dup;
  hash_iterate (zvrf->evpn_table, zevpn_print_mac_hash_all_evpn_detail, &wctx);
...
void
zebra_vxlan_print_macs_vni_dad (struct vty *vty, struct zebra_vrf *zvrf,
                                vni_t vni, bool use_json)
{
  struct zebra_evpn *zevpn;
  struct mac_walk_ctx wctx;
...
int
zebra_vxlan_clear_dup_detect_vni_mac (struct zebra_vrf *zvrf, vni_t vni,
                                      ...
                                      size_t errmsg_len)
{
  ...
  if (! mac)
    {
      snprintf (errmsg, errmsg_len, "Requested MAC does not exist in VNI %u\n",
                vni);
      return -1;
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (mac->neigh_list, node, nbr))
    {
       ...
       * installed locally.
       */
      if (zvrf->dad_freeze && CHECK_FLAG (nbr->flags, ZEBRA_NEIGH_DUPLICATE))
        {
  ...
  if (CHECK_FLAG (mac->flags, ZEBRA_MAC_LOCAL))
    {
      /* Inform to BGP */
      if (zebra_evpn_mac_send_add_to_client (
              zevpn->vni, &mac->macaddr, mac->flags, mac->loc_seq, mac->es))
        return 0;
...
int
zebra_vxlan_clear_dup_detect_vni_ip (struct zebra_vrf *zvrf, vni_t vni,
                                     ...
                                     size_t errmsg_len)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("%s: clear neigh %s in dup state, flags 0x%x seq %u", __func__,
...
static void
zevpn_clear_dup_mac_hash (struct hash_bucket *bucket, void *ctxt)
{
  struct mac_walk_ctx *wctx = ctxt;
  struct zebra_mac *mac;
  struct zebra_evpn *zevpn;
  ...
  for (ALL_LIST_ELEMENTS_RO (mac->neigh_list, node, nbr))
    {
      if (CHECK_FLAG (nbr->flags, ZEBRA_NEIGH_LOCAL) && nbr->dad_count)
        ZEBRA_NEIGH_SET_INACTIVE (nbr);
...
static void
zevpn_clear_dup_detect_hash_vni_all (struct hash_bucket *bucket, void **args)
{
  struct zebra_evpn *zevpn;
  struct zebra_vrf *zvrf;
  ...
  if (hashcount (zevpn->neigh_table))
    {
      ...
      n_wctx.zevpn = zevpn;
      n_wctx.zvrf = zvrf;
      hash_iterate (zevpn->neigh_table, zebra_evpn_clear_dup_neigh_hash,
                    &n_wctx);
    }
...
int
zebra_vxlan_clear_dup_detect_vni_all (struct zebra_vrf *zvrf)
{
  void *args[1];
...
int
zebra_vxlan_clear_dup_detect_vni (struct zebra_vrf *zvrf, vni_t vni)
{
  struct zebra_evpn *zevpn;
  struct mac_walk_ctx m_wctx;
  struct neigh_walk_ctx n_wctx;
...
void
zebra_vxlan_print_evpn (struct vty *vty, bool uj)
{
  int num_l2vnis = 0;
  int num_l3vnis = 0;
  int num_vnis = 0;
  ...
  if (uj)
    {
      ...
      if (zebra_evpn_do_dup_addr_detect (zvrf))
        json_object_boolean_true_add (json, "isDuplicateAddrDetection");
      else
        json_object_boolean_false_add (json, "isDuplicateAddrDetection");
      json_object_int_add (json, "maxMoves", zvrf->dad_max_moves);
      json_object_int_add (json, "detectionTime", zvrf->dad_time);
      json_object_int_add (json, "detectionFreezeTime", zvrf->dad_freeze_time);
      json_object_boolean_add (json, "isDetectionFreeze", zvrf->dad_freeze);
  ...
  else
    {
      ...
      vty_out (vty, "Duplicate address detection: %s\n",
               zebra_evpn_do_dup_addr_detect (zvrf) ? "Enable" : "Disable");
 ...
 */
void
zebra_vxlan_print_vnis (struct vty *vty, struct zebra_vrf *zvrf, bool use_json)
{
  json_object *json = NULL;
  void *args[2];
  ...
  if (! use_json)
    vty_out (vty, "%-10s %-4s %-21s %-8s %-8s %-15s %-37s\n", "VNI", "Type",
             "VxLAN IF", "# MACs", "# ARPs", "# Remote VTEPs", "Tenant VRF");
  ...
  hash_iterate (
      zvrf->evpn_table,
      (void (*) (struct hash_bucket *, void *)) zebra_evpn_print_hash, args);
...
void
zebra_vxlan_dup_addr_detection (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  int time = 0;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug (
        ...
        dup_addr_detect ? "enable" : "disable", zvrf->dad_max_moves,
        zvrf->dad_time, zvrf->dad_freeze ? "enable" : "disable",
        zvrf->dad_freeze_time);
 ...
 */
void
zebra_vxlan_print_vnis_detail (struct vty *vty, struct zebra_vrf *zvrf,
                               bool use_json)
{
  json_object *json_array = NULL;
  ...
  hash_iterate (
      zvrf->evpn_table,
      (void (*) (struct hash_bucket *, void *)) zebra_evpn_print_hash_detail,
      &zes);

  /* Display all L3-VNIs */
  hash_iterate (
      zrouter.l3vni_table,
      (void (*) (struct hash_bucket *, void *)) zl3vni_print_hash_detail,
      &zes);
 ...
 */
int
zebra_vxlan_handle_kernel_neigh_del (struct interface *ifp,
                                     ...
                                     struct ipaddr *ip)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("Del neighbor %pIA intf %s(%u) -> L2-VNI %u", ip, ifp->name,
    ...
    bool local_inactive, bool dp_static)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
    zlog_debug (
        ...
        is_ext ? "ext-learned " : "", is_router ? "router " : "",
        local_inactive ? "local_inactive " : "", dp_static ? "peer_sync " : "",
        zevpn->vni);

  /* Is this about a local neighbor or a remote one? */
  if (! is_ext)
    return zebra_evpn_local_neigh_update (zevpn, ifp, ip, macaddr, is_router,
 ...
 */
void
zebra_vxlan_remote_macip_del (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  vni_t vni;
  ...
  while (l < hdr->length)
    {
      int res_length = zebra_vxlan_remote_macip_helper (
          false, s, &vni, &macaddr, &ipa_len, &ip, &vtep_ip, NULL, NULL, NULL);
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug (
            "Recv MACIP DEL VNI %u MAC %pEA%s%s Remote VTEP %pI4 from %s", vni,
            &macaddr, ipa_len ? " IP " : "",
 ...
 */
void
zebra_vxlan_remote_macip_add (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  vni_t vni;
  struct ethaddr macaddr;
  ...
  while (l < hdr->length)
    {
      ...
      int res_length =
          zebra_vxlan_remote_macip_helper (true, s, &vni, &macaddr, &ipa_len,
      ...
      /* Enqueue to workqueue for processing */
      zebra_rib_queue_evpn_rem_macip_add (vni, &macaddr, &ip, flags, seq,
                                          vtep_ip, &esi);
    }
 ...
 */
int
zebra_vxlan_check_readd_vtep (struct interface *ifp, vni_t vni,
                              struct in_addr vtep_ip)
{
  struct zebra_if *zif;
  struct zebra_vrf *zvrf = NULL;
 ...
 */
int
zebra_vxlan_dp_network_mac_add (struct interface *ifp, struct interface *br_if,
                                struct ethaddr *macaddr, vlanid_t vid,
                                ...
                                bool dp_static)
{
  ...
  if (vid)
    {
      if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("dpAdd MAC %pEA VID %u - ignore as no nhid", macaddr, vid);
      return 0;
    }

  /* Get vxlan's vid for netlink message has no it. */
  vid = ((struct zebra_if *) ifp->info)->l2info.vxl.vni_info.vni.access_vlan;
  ...
  if (! nhg_id || ! zebra_evpn_nhg_is_local_es (nhg_id, &es) ||
    ...
    {
      if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("dpAdd remote MAC %pEA VID %u", macaddr, vid);
      return zebra_vxlan_check_del_local_mac (ifp, br_if, macaddr, vid, vni);
    }
  ...
  acc_ifp = es->zif->ifp;
  return zebra_vxlan_local_mac_add_update (acc_ifp, br_if, macaddr, vid,
 ...
 */
int
zebra_vxlan_dp_network_mac_del (struct interface *ifp, struct interface *br_if,
                                struct ethaddr *macaddr, vlanid_t vid,
                                vni_t vni)
{
  struct zebra_if *zif = NULL;
  ...
  if (CHECK_FLAG (mac->flags, ZEBRA_MAC_REMOTE))
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("dpDel remote MAC %pEA intf %s(%u) VNI %u - readd",
           ...
           zebra_evpn_es_local_mac_via_network_port (mac->es))
    {
       ...
       */
      if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("dpDel local-nw-MAC %pEA VNI %u", macaddr, vni);
 ...
 */
int
zebra_vxlan_local_mac_add_update (struct interface *ifp,
                                  struct interface *br_if,
                                  struct ethaddr *macaddr, vlanid_t vid,
                                  ...
                                  bool dp_static)
{
  ...
  if (! zevpn)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug (
            ...
            sticky ? "sticky " : "", macaddr, ifp->name, ifp->ifindex, vid);
      return 0;
    }
  ...
  zvrf = zebra_vrf_get_evpn ();
  return zebra_evpn_add_update_local_mac (
      zvrf, zevpn, ifp, macaddr, vid, sticky, local_inactive, dp_static, NULL);
}
 ...
 */
void
zebra_vxlan_remote_vtep_del_zapi (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  unsigned short l = 0;
  vni_t vni;
 ...
 */
void
zebra_vxlan_remote_vtep_del (vrf_id_t vrf_id, vni_t vni,
                             struct in_addr vtep_ip)
{
  struct zebra_evpn *zevpn;
  ...
  if (! zevpn)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("Failed to locate VNI hash for remote VTEP DEL, VNI %u",
                    vni);
      return;
    }
 ...
 */
void
zebra_vxlan_remote_vtep_add (vrf_id_t vrf_id, vni_t vni,
  ...
  if (zvtep)
  ...
  else
    {
      ...
      if (zvtep)
        ...
        flog_err (EC_ZEBRA_VTEP_ADD_FAILED,
                  "Failed to add remote VTEP, VNI %u zevpn %p", vni, zevpn);
    }
}
 ...
 */
void
zebra_vxlan_remote_vtep_add_zapi (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  unsigned short l = 0;
  vni_t vni;
  ...
  while (l < hdr->length)
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("Recv VTEP ADD %pI4 VNI %u flood %d from %s", &vtep_ip,
                    vni, flood_control, zebra_route_string (client->proto));
 ...
 */
int
zebra_vxlan_add_del_gw_macip (struct interface *ifp, const struct prefix *p,
                              int add)
{
  struct ipaddr ip;
  struct ethaddr macaddr;
          ...
          if (svi_if_zif)
            {
              ...
              zevpn = zebra_evpn_from_svi (svi_if, svi_if_link);
            }
        }
  ...
  if (p->family == AF_INET)
    {
      ip.ipa_type = IPADDR_V4;
      memcpy (&(ip.ipaddr_v4), &(p->u.prefix4), sizeof (struct in_addr));
    }
  else if (p->family == AF_INET6)
    {
      ip.ipa_type = IPADDR_V6;
      memcpy (&(ip.ipaddr_v6), &(p->u.prefix6), sizeof (struct in6_addr));
    }
  ...
  else
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("SVI %s(%u) VNI %u VRF %s is UP, installing neighbors",
                    ifp->name, ifp->ifindex, zevpn->vni, ifp->vrf->name);
 ...
 */
void
zebra_vxlan_macvlan_down (struct interface *ifp)
{
  struct zebra_l3vni *zl3vni = NULL;
  ...
  if (! link_ifp)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("macvlan parent link is not found. Parent index %d ifp %s",
                    zif->link_ifindex,
                    ifindex2ifname (zif->link_ifindex, ifp->vrf->vrf_id));
      return;
    }
 ...
 */
void
zebra_vxlan_macvlan_up (struct interface *ifp)
{
  struct zebra_l3vni *zl3vni = NULL;
  struct zebra_if *zif, *link_zif;
  struct interface *link_ifp, *link_if;
...
int
zebra_vxlan_process_vrf_vni_cmd (struct zebra_vrf *zvrf, vni_t vni, char *err,
  ...
  if (add)
    {
      ...
      if (zl3vni)
        {
          snprintf (err, err_str_sz, "VNI is already configured as L3-VNI");
          return -1;
        }
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("%s: l3vni %u svi_if %s mac_vlan_if %s", __func__, vni,
                    zl3vni->svi_if ? zl3vni->svi_if->name : "NIL",
                    zl3vni->mac_vlan_if ? zl3vni->mac_vlan_if->name : "NIL");

      /* formulate l2vni list */
      hash_iterate (zvrf_evpn->evpn_table, zevpn_add_to_l3vni_list, zl3vni);
  ...
  else
    {
      ...
      if (zvrf->l3vni != vni)
        {
          snprintf (err, err_str_sz, "VNI %d doesn't exist in VRF: %s", vni,
                    zvrf->vrf->name);
          return -1;
        }
...
int
zebra_vxlan_vrf_enable (struct zebra_vrf *zvrf)
{
  struct zebra_l3vni *zl3vni = NULL;
...
int
zebra_vxlan_vrf_disable (struct zebra_vrf *zvrf)
{
  struct zebra_l3vni *zl3vni = NULL;
...
int
zebra_vxlan_vrf_delete (struct zebra_vrf *zvrf)
{
  struct zebra_l3vni *zl3vni = NULL;
  vni_t vni;
 ...
 */
void
zebra_vxlan_flood_control (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  ...
  if (! EVPN_ENABLED (zvrf))
    {
      zlog_err ("EVPN flood control for non-EVPN VRF %u", zvrf_id (zvrf));
      return;
    }
 ...
 */
void
zebra_vxlan_advertise_svi_macip (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  int advertise;
  vni_t vni = 0;
  ...
  if (! vni)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("EVPN SVI-MACIP Adv %s, currently %s",
                    ...
                    advertise_svi_macip_enabled (NULL) ? "enabled"
                                                       : "disabled");
      ...
      if (advertise)
        ...
        {
          hash_iterate (zvrf->evpn_table,
                        zebra_evpn_svi_macip_del_for_evpn_hash, NULL);
          zvrf->advertise_svi_macip = advertise;
        }
    }
  else
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("EVPN SVI macip Adv %s on VNI %d, currently %s",
                    advertise ? "enabled" : "disabled", vni,
                    advertise_svi_macip_enabled (zevpn) ? "enabled"
                                                        : "disabled");
      ...
      vlan_if =
          zvni_map_to_svi (zl2_info_vni->access_vlan, zif->brslave_info.br_if);
      if (! vlan_if)
        return;
 ...
 */
void
zebra_vxlan_advertise_subnet (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  int advertise;
  vni_t vni = 0;
 ...
 */
void
zebra_vxlan_advertise_gw_macip (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  int advertise;
  vni_t vni = 0;
  ...
  if (! vni)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("EVPN gateway macip Adv %s, currently %s",
                    advertise ? "enabled" : "disabled",
                    advertise_gw_macip_enabled (NULL) ? "enabled"
                                                      : "disabled");
      ...
      if (advertise_gw_macip_enabled (zevpn))
        hash_iterate (zvrf->evpn_table, zebra_evpn_gw_macip_add_for_evpn_hash,
                      NULL);
      else
        hash_iterate (zvrf->evpn_table, zebra_evpn_gw_macip_del_for_evpn_hash,
                      NULL);
    }
  else
    {
...
static int
macfdb_read_ns (struct ns *ns, void *_in_param __attribute__ ((unused)),
...
static int
neigh_read_ns (struct ns *ns, void *_in_param __attribute__ ((unused)),
               void **out_param __attribute__ ((unused)))
{
  struct zebra_ns *zns = ns->info;
 ...
 */
void
zebra_vxlan_advertise_all_vni (ZAPI_HANDLER_ARGS)
{
  struct stream *s = NULL;
  int advertise = 0;
  enum vxlan_flood_control flood_ctrl;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("EVPN VRF %s(%u) VNI Adv %s, currently %s, flood control %u",
                ...
                advertise ? "enabled" : "disabled",
                is_evpn_enabled () ? "enabled" : "disabled", flood_ctrl);
...
void
zebra_vxlan_cleanup_tables (struct zebra_vrf *zvrf)
{
...
bool
is_vrf_l3vni_svd_backed (vrf_id_t vrf_id)
{
  struct zebra_l3vni *zl3vni = NULL;
...
static int
zebra_vxlan_sg_send (struct zebra_vrf *zvrf, struct prefix_sg *sg,
...
static unsigned int
zebra_vxlan_sg_hash_key_make (const void *p)
{
  const struct zebra_vxlan_sg *vxlan_sg = p;

  return (jhash_2words (vxlan_sg->sg.src.s_addr, vxlan_sg->sg.grp.s_addr, 0));
}

static bool
zebra_vxlan_sg_hash_eq (const void *p1, const void *p2)
{
...
zebra_vxlan_sg_new (struct zebra_vrf *zvrf, struct prefix_sg *sg)
{
...
static struct zebra_vxlan_sg *
zebra_vxlan_sg_find (struct zebra_vrf *zvrf, struct prefix_sg *sg)
{
...
static struct zebra_vxlan_sg *
zebra_vxlan_sg_add (struct zebra_vrf *zvrf, struct prefix_sg *sg)
{
  struct zebra_vxlan_sg *vxlan_sg;
  ...
  zebra_vxlan_sg_send (zvrf, sg, vxlan_sg->sg_str, ZEBRA_VXLAN_SG_ADD);

  return vxlan_sg;
}

static void
zebra_vxlan_sg_del (struct zebra_vxlan_sg *vxlan_sg)
{
  struct in_addr sip;
  struct zebra_vrf *zvrf;
...
static void
zebra_vxlan_sg_do_deref (struct zebra_vrf *zvrf, struct in_addr sip,
                         struct in_addr mcast_grp)
{
  struct zebra_vxlan_sg *vxlan_sg;
  struct prefix_sg sg;
...
static struct zebra_vxlan_sg *
zebra_vxlan_sg_do_ref (struct zebra_vrf *zvrf, struct in_addr sip,
                       struct in_addr mcast_grp)
{
  struct zebra_vxlan_sg *vxlan_sg;
  struct prefix_sg sg;
...
void
zebra_vxlan_sg_deref (struct in_addr local_vtep_ip, struct in_addr mcast_grp)
{
  struct zebra_vrf *zvrf;
...
void
zebra_vxlan_sg_ref (struct in_addr local_vtep_ip, struct in_addr mcast_grp)
{
  struct zebra_vrf *zvrf;
...
static void
zebra_vxlan_xg_pre_cleanup (struct hash_bucket *bucket, void *arg)
{
...
static void
zebra_vxlan_xg_post_cleanup (struct hash_bucket *bucket, void *arg)
{
  struct zebra_vxlan_sg *vxlan_sg = (struct zebra_vxlan_sg *) bucket->data;
...
static void
zebra_vxlan_sg_cleanup (struct hash_bucket *bucket, void *arg)
{
  struct zebra_vxlan_sg *vxlan_sg = (struct zebra_vxlan_sg *) bucket->data;
...
static void
zebra_vxlan_cleanup_sg_table (struct zebra_vrf *zvrf)
{
  /* increment the ref count against (*,G) to prevent them from being
   * deleted
   */
...
static void
zebra_vxlan_sg_replay_send (struct hash_bucket *bucket, void *arg)
{
  struct zebra_vxlan_sg *vxlan_sg = (struct zebra_vxlan_sg *) bucket->data;

  zebra_vxlan_sg_send (vxlan_sg->zvrf, &vxlan_sg->sg, vxlan_sg->sg_str,
                       ZEBRA_VXLAN_SG_ADD);
}

/* Handle message from client to replay vxlan SG entries */
void
zebra_vxlan_sg_replay (ZAPI_HANDLER_ARGS)
{
  if (IS_ZEBRA_DEBUG_VXLAN)
...
static void
zebra_evpn_vrf_cfg_cleanup (struct zebra_vrf *zvrf)
{
  struct zebra_l3vni *zl3vni = NULL;
...
static int
zebra_evpn_bgp_cfg_clean_up (struct zserv *client)
{
  struct vrf *vrf;
  struct zebra_vrf *zvrf;
...
static int
zebra_evpn_pim_cfg_clean_up (struct zserv *client)
{
...
static int
zebra_evpn_cfg_clean_up (struct zserv *client)
{
 ...
 */
extern void
zebra_vxlan_handle_result (struct zebra_dplane_ctx *ctx)
{
  return;
}
...
void
zebra_vxlan_set_accept_bgp_seq (bool set)
{
  accept_bgp_seq = set;
}

bool
zebra_vxlan_get_accept_bgp_seq (void)
{
