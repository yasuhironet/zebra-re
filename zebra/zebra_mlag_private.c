 ...
 */
static int
zebra_mlag_private_write_data (uint8_t *data, uint32_t len)
{
  int rc = 0;
...
static void
zebra_mlag_read (struct event *thread)
{
  ...
  if (curr_len < ZEBRA_MLAG_LEN_SIZE)
    {
      ...
      if (data_len == 0 || data_len == -1)
        {
          if (IS_ZEBRA_DEBUG_MLAG)
            zlog_debug ("MLAG connection closed socket : %d", mlag_socket);
          close (mlag_socket);
  ...
  if (curr_len < tot_len)
    {
      ...
      data_len =
          read (mlag_socket, mlag_rd_buffer + curr_len, tot_len - curr_len);
      if (data_len == 0 || data_len == -1)
        {
          if (IS_ZEBRA_DEBUG_MLAG)
            zlog_debug ("MLAG connection closed socket : %d", mlag_socket);
...
static void
zebra_mlag_connect (struct event *thread)
{
 ...
 */
static int
zebra_mlag_private_monitor_state (void)
{
...
static int
zebra_mlag_private_open_channel (void)
{
  ...
  if (zrouter.mlag_info.connected == true)
    {
      if (IS_ZEBRA_DEBUG_MLAG)
        zlog_debug ("%s: Zebra already connected to MLAGD", __func__);
      return 0;
    }
  ...
  if (zrouter.mlag_info.timer_running == true)
    {
      if (IS_ZEBRA_DEBUG_MLAG)
        zlog_debug ("%s: Connection retry is in progress for MLAGD", __func__);
      return 0;
    }
...
static int
zebra_mlag_private_close_channel (void)
{
  ...
  if (zrouter.mlag_info.clients_interested_cnt)
    {
      if (IS_ZEBRA_DEBUG_MLAG)
        zlog_debug ("%s: still %d clients are connected, skip", __func__,
...
static int
zebra_mlag_private_cleanup_data (void)
{
  zmlag_master = NULL;
