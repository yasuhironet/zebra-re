...
static void
zebra_ptm_flush_messages (struct event *thread)
{
  ptm_cb.t_write = NULL;
...
void
zebra_ptm_connect (struct event *t)
{
  int init = 0;
      ...
      if (! if_is_operative (ifp) && send_linkdown)
        {
          if (IS_ZEBRA_DEBUG_EVENT)
            zlog_debug ("%s: Bringing down interface %s", __func__, ifp->name);
          if_down (ifp);
        }
    }
  ...
  if ((ifp->ifindex != IFINDEX_INTERNAL) && (ifp->ptm_enable))
    {
      ...
      if (if_is_no_ptm_operative (ifp) && send_linkup)
        {
          if (IS_ZEBRA_DEBUG_EVENT)
            zlog_debug ("%s: Bringing up interface %s", __func__, ifp->name);
          if_up (ifp, true);
        }
    }
...
zebra_ptm_socket_init (void)
{
  ...
  if (set_nonblocking (sock) < 0)
    {
      if (IS_ZEBRA_DEBUG_EVENT)
        zlog_debug ("%s: Unable to set socket non blocking[%s]", __func__,
                    safe_strerror (errno));
      close (sock);
      return -1;
    }
  ...
  if (ret < 0)
    {
      if (IS_ZEBRA_DEBUG_EVENT)
        zlog_debug ("%s: Unable to connect to socket %s [%s]", __func__,
      ...
      if (ifp)
        {
          zlog_debug (
              ...
              dp->prefixlen, ifp->name, bfd_get_status_str (status));
        }
      else
        {
...
zebra_ptm_handle_bfd_msg (void *arg, void *in_ctxt, struct interface *ifp)
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug (
        ...
        __func__, ifp ? ifp->name : "N/A", bfdst_str, vrf_str, dest_str,
        src_str);

  if (str2prefix (dest_str, &dest_prefix) == 0)
    {
      flog_err (EC_ZEBRA_PREFIX_PARSE_ERROR, "%s: Peer addr %s not found",
                __func__, dest_str);
      return -1;
    }
  ...
  if (strcmp (ZEBRA_PTM_INVALID_SRC_IP, src_str))
    {
      if (str2prefix (src_str, &src_prefix) == 0)
        {
          flog_err (EC_ZEBRA_PREFIX_PARSE_ERROR, "%s: Local addr %s not found",
                    __func__, src_str);
          return -1;
        }
    }
                          ...
                          char *cbl_str)
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    zlog_debug ("%s: Recv Port [%s] cbl status [%s]", __func__, ifp->name,
                cbl_str);
      ...
      if (! ifp)
        {
          flog_warn (EC_ZEBRA_UNKNOWN_INTERFACE,
                     "%s: %s not found in interface list", __func__, port_str);
          return -1;
        }
      if (count > 1)
        {
          flog_warn (EC_ZEBRA_UNKNOWN_INTERFACE,
                     "%s: multiple interface with name %s", __func__,
                     port_str);
          return -1;
        }
    }
...
void
zebra_ptm_sock_read (struct event *thread)
{
  int sock;
  int rc;
  ...
  if (((rc == 0) && ! errno) ||
    ...
    {
      flog_err_sys (EC_LIB_SOCKET, "%s routing socket error: %s(%d) bytes %d",
  ...
  ptm_cb.t_read = NULL;
  event_add_read (zrouter.master, zebra_ptm_sock_read, NULL, ptm_cb.ptm_sock,
                  &ptm_cb.t_read);
}
      ...
      if (src_p.family == AF_INET)
        {
          ...
          ptm_lib_append_msg (ptm_hdl, out_ctxt, ZEBRA_PTM_BFD_SRC_IP_FIELD,
                              buf);
        }
      else
        {
...
void
zebra_ptm_bfd_dst_deregister (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct prefix src_p;
  struct prefix dst_p;
  ...
  if (multi_hop)
    {
      ...
      if (src_p.family == AF_INET)
        ...
        inet_ntop (AF_INET6, &src_p.u.prefix6, buf, sizeof (buf));
      ptm_lib_append_msg (ptm_hdl, out_ctxt, ZEBRA_PTM_BFD_SRC_IP_FIELD, buf);
...
/* BFD client register */
void
zebra_ptm_bfd_client_register (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  unsigned int pid;
  void *out_ctxt = NULL;
...
void
zebra_ptm_show_status (struct vty *vty, json_object *json,
                       struct interface *ifp)
{
  const char *status;
 ...
 */
static struct ptm_process *
pp_new (pid_t pid, struct zserv *zs)
{
  ...
  TAILQ_FOREACH (pp, &ppqueue, pp_entry)
    {
      if (pp->pp_pid == pid && pp->pp_zs != zs)
        zlog_err ("%s:%d pid and client pointer doesn't match", __FILE__,
                  __LINE__);
    }
#endif /* PTM_DEBUG */
...
static struct ptm_process *
pp_lookup_byzs (struct zserv *zs)
{
  struct ptm_process *pp;
 ...
 */
static void
zebra_ptm_send_bfdd (struct stream *msg)
{
...
static void
zebra_ptm_send_clients (struct stream *msg)
{
  struct listnode *node;
  struct zserv *client;
  struct stream *msgc;
...
static int
_zebra_ptm_bfd_client_deregister (struct zserv *zs)
{
  struct stream *msg;
  struct ptm_process *pp;
  ...
  if (pp == NULL)
    {
      zlog_err ("%s:%d failed to find process pid registration", __FILE__,
                __LINE__);
      return -1;
    }
 ...
 */
static void
_zebra_ptm_reroute (struct zserv *zs, struct zebra_vrf *zvrf,
                    struct stream *msg, uint32_t command)
{
  struct stream *msgc;
  ...
  if (STREAM_READABLE (msg) > STREAM_WRITEABLE (msgc))
    {
      zlog_warn (
          "Cannot fit extended BFD header plus original message contents into ZAPI packet; dropping message");
      goto stream_failure;
    }
...
void
zebra_ptm_bfd_dst_replay (ZAPI_HANDLER_ARGS)
{
  struct stream *msgc;
 ...
 */
void
zebra_ptm_if_init (struct zebra_if *zifp __attribute__ ((__unused__)))
{
  /* NOTHING */
}
...
void
zebra_ptm_show_status (struct vty *vty __attribute__ ((__unused__)),
...
void
zebra_ptm_write (struct vty *vty __attribute__ ((__unused__)))
{
  /* NOTHING */
}

void
zebra_ptm_if_write (struct vty *vty __attribute__ ((__unused__)),
                    struct zebra_if *zifp __attribute__ ((__unused__)))
{
  /* NOTHING */
}
void
zebra_ptm_if_set_ptm_state (struct interface *i __attribute__ ((__unused__)),
