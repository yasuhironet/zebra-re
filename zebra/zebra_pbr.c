...
/* definitions */
static const struct message tcp_value_str[] = { { TCP_HEADER_FIN, "FIN" },
                                                { TCP_HEADER_SYN, "SYN" },
                                                { TCP_HEADER_RST, "RST" },
                                                { TCP_HEADER_PSH, "PSH" },
                                                { TCP_HEADER_ACK, "ACK" },
...
static const struct message fragment_value_str[] = { { 1, "dont-fragment" },
                                                     { 2, "is-fragment" },
                                                     { 4, "first-fragment" },
                                                     { 8, "last-fragment" },
                                                     { 0 } };
...
DEFINE_HOOK (zebra_pbr_ipset_update, (int cmd, struct zebra_pbr_ipset *ipset),
             (cmd, ipset));
...
uint32_t
zebra_pbr_rules_hash_key (const void *arg)
{
  ...
  return jhash_3words (rule->rule.filter.src_port, rule->rule.filter.dst_port,
                       prefix_hash_key (&rule->rule.filter.dst_ip),
                       jhash_1word (rule->rule.unique, key));
}

bool
zebra_pbr_rules_hash_equal (const void *arg1, const void *arg2)
{
...
static int
pbr_rule_lookup_unique_walker (struct hash_bucket *b, void *data)
{
...
uint32_t
zebra_pbr_ipset_hash_key (const void *arg)
{
...
bool
zebra_pbr_ipset_hash_equal (const void *arg1, const void *arg2)
{
  ...
  if (strncmp (r1->ipset_name, r2->ipset_name, ZEBRA_IPSET_NAME_SIZE))
    return false;
  return true;
}

void
zebra_pbr_ipset_entry_free (void *arg)
{
...
uint32_t
zebra_pbr_ipset_entry_hash_key (const void *arg)
{
...
bool
zebra_pbr_ipset_entry_hash_equal (const void *arg1, const void *arg2)
{
 ...
 */
static void
_zebra_pbr_iptable_free_all (void *arg, bool all)
{
  ...
  if (iptable->interface_name_list)
    {
      for (ALL_LIST_ELEMENTS (iptable->interface_name_list, node, nnode, name))
        {
...
void
zebra_pbr_iptable_free (void *arg)
{
  _zebra_pbr_iptable_free_all (arg, false);
}

uint32_t
zebra_pbr_iptable_hash_key (const void *arg)
{
  const struct zebra_pbr_iptable *iptable = arg;
  uint32_t *pnt = (uint32_t *) &(iptable->ipset_name);
  uint32_t key;

  key = jhash2 (pnt, ZEBRA_IPSET_NAME_HASH_SIZE, 0x63ab42de);
  ...
  return jhash_3words (iptable->filter_bm, iptable->type, iptable->unique,
                       key);
}

bool
zebra_pbr_iptable_hash_equal (const void *arg1, const void *arg2)
{
...
static void *
pbr_rule_alloc_intern (void *arg)
{
...
static struct zebra_pbr_rule *
pbr_rule_free (struct zebra_pbr_rule *hash_data, bool free_data)
{
...
static struct zebra_pbr_rule *
pbr_rule_release (struct zebra_pbr_rule *rule, bool free_data)
{
...
void
zebra_pbr_show_rule_unit (struct zebra_pbr_rule *rule, struct vty *vty)
{
...
static int
zebra_pbr_show_rules_walkcb (struct hash_bucket *bucket, void *arg)
{
...
void
zebra_pbr_show_rule (struct vty *vty)
{
...
void
zebra_pbr_config_write (struct vty *vty)
{
...
void
zebra_pbr_expand_action_update (bool enable)
{
  zebra_pbr_expand_action = enable;
}

static void
zebra_pbr_expand_rule (struct zebra_pbr_rule *rule)
{
  struct prefix p;
  ...
  table = zebra_vrf_get_table_with_table_id (AFI_IP, SAFI_UNICAST, VRF_DEFAULT,
                                             rule->rule.action.table);
  if (! table)
    return;
...
void
zebra_pbr_add_rule (struct zebra_pbr_rule *rule)
{
  ...
  if (found)
    {
      if (IS_ZEBRA_DEBUG_PBR)
        zlog_debug ("%s: seq: %d, prior: %d, unique: %d, ifname: %s -- update",
    ...
    {
      if (IS_ZEBRA_DEBUG_PBR)
        zlog_debug ("%s: seq: %d, prior: %d, unique: %d, ifname: %s -- new",
                    __func__, rule->rule.seq, rule->rule.priority,
                    rule->rule.unique, rule->rule.ifname);
...
void
zebra_pbr_del_rule (struct zebra_pbr_rule *rule)
{
  if (IS_ZEBRA_DEBUG_PBR)
    zlog_debug ("%s: seq: %d, prior: %d, unique: %d, ifname: %s", __func__,
                rule->rule.seq, rule->rule.priority, rule->rule.unique,
                rule->rule.ifname);

  (void) dplane_pbr_rule_delete (rule);

  if (pbr_rule_release (rule, true))
    zlog_debug ("%s: Rule being deleted we know nothing about", __func__);
}

void
zebra_pbr_process_iptable (struct zebra_dplane_ctx *ctx)
{
  int mode, ret = 0;
...
void
zebra_pbr_process_ipset (struct zebra_dplane_ctx *ctx)
{
  int mode, ret = 0;
  struct zebra_pbr_ipset ipset;
...
void
zebra_pbr_process_ipset_entry (struct zebra_dplane_ctx *ctx)
{
  int mode, ret = 0;
...
static void
zebra_pbr_cleanup_rules (struct hash_bucket *b, void *data)
{
  struct zebra_pbr_rule *rule = b->data;
  int *sock = data;
...
static void
zebra_pbr_cleanup_ipset (struct hash_bucket *b, void *data)
{
...
static void
zebra_pbr_cleanup_ipset_entry (struct hash_bucket *b, void *data)
{
...
static void
zebra_pbr_cleanup_iptable (struct hash_bucket *b, void *data)
{
...
static int
zebra_pbr_client_close_cleanup (struct zserv *client)
{
...
static void *
pbr_ipset_alloc_intern (void *arg)
{
...
void
zebra_pbr_create_ipset (struct zebra_pbr_ipset *ipset)
{
...
void
zebra_pbr_destroy_ipset (struct zebra_pbr_ipset *ipset)
{
  ...
  if (lookup)
    ...
    zlog_debug ("%s: IPSet Entry being deleted we know nothing about",
                __func__);
}
...
const char *
zebra_pbr_ipset_type2str (uint32_t type)
{
  return lookup_msg (ipset_type_msg, type, "Unrecognized IPset Type");
}

static int
zebra_pbr_ipset_pername_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct pbr_ipset_name_lookup *pinl = (struct pbr_ipset_name_lookup *) arg;
  struct zebra_pbr_ipset *zpi = (struct zebra_pbr_ipset *) bucket->data;

  if (! strncmp (pinl->ipset_name, zpi->ipset_name, ZEBRA_IPSET_NAME_SIZE))
    {
      pinl->ipset = zpi;
      return HASHWALK_ABORT;
    }
...
static void *
pbr_ipset_entry_alloc_intern (void *arg)
{
...
void
zebra_pbr_add_ipset_entry (struct zebra_pbr_ipset_entry *ipset)
{
...
void
zebra_pbr_del_ipset_entry (struct zebra_pbr_ipset_entry *ipset)
{
  ...
  if (lookup)
    ...
    zlog_debug ("%s: IPSet being deleted we know nothing about", __func__);
}

static void *
pbr_iptable_alloc_intern (void *arg)
{
...
void
zebra_pbr_add_iptable (struct zebra_pbr_iptable *iptable)
{
  struct zebra_pbr_iptable *ipt_hash;

  ipt_hash =
      hash_get (zrouter.iptable_hash, iptable, pbr_iptable_alloc_intern);
  (void) dplane_pbr_iptable_add (ipt_hash);
}

void
zebra_pbr_del_iptable (struct zebra_pbr_iptable *iptable)
{
  ...
  if (lookup)
    ...
    zlog_debug ("%s: IPTable being deleted we know nothing about", __func__);
}
 ...
 */
void
zebra_pbr_dplane_result (struct zebra_dplane_ctx *ctx)
{
  ...
  if (op == DPLANE_OP_RULE_ADD || op == DPLANE_OP_RULE_UPDATE)
    ...
    zsend_iptable_notify_owner (ctx, res == ZEBRA_DPLANE_REQUEST_SUCCESS
    ...
    zsend_ipset_entry_notify_owner (ctx, res == ZEBRA_DPLANE_REQUEST_SUCCESS
 ...
 */
int
kernel_pbr_rule_del (struct zebra_pbr_rule *rule)
{
  return 0;
}
...
static const char *
zebra_pbr_prefix2str (union prefixconstptr pu, char *str, int size)
{
...
static void
zebra_pbr_display_icmp (struct vty *vty, struct zebra_pbr_ipset_entry *zpie)
{
  char decoded_str[20];
  uint16_t port;
  struct zebra_pbr_ipset *zpi;
  ...
  if (zpie->src_port_max || zpie->dst_port_max)
    {
      vty_out (vty, ":icmp:[type <%u:%u>;code <%u:%u>", zpie->src_port_min,
               zpie->src_port_max, zpie->dst_port_min, zpie->dst_port_max);
    }
  else
    {
      port =
          ((zpie->src_port_min << 8) & 0xff00) + (zpie->dst_port_min & 0xff);
      memset (decoded_str, 0, sizeof (decoded_str));
      snprintf (decoded_str, sizeof (decoded_str), "%u/%u", zpie->src_port_min,
                zpie->dst_port_min);
      vty_out (vty, ":%s:%s", zpi->family == AF_INET6 ? "ipv6-icmp" : "icmp",
               lookup_msg (zpi->family == AF_INET6 ? icmpv6_typecode_str
                                                   : icmp_typecode_str,
                           port, decoded_str));
    }
}
...
static void
zebra_pbr_display_port (struct vty *vty, uint32_t filter_bm, uint16_t port_min,
                        uint16_t port_max, uint8_t proto)
{
  if (! (filter_bm & PBR_FILTER_PROTO))
    {
      if (port_max)
        vty_out (vty, ":udp/tcp:%d-%d", port_min, port_max);
      else
        vty_out (vty, ":udp/tcp:%d", port_min);
    }
  else
    {
      if (port_max)
        vty_out (vty, ":proto %d:%d-%d", proto, port_min, port_max);
      else
        vty_out (vty, ":proto %d:%d", proto, port_min);
    }
}

static int
zebra_pbr_show_ipset_entry_walkcb (struct hash_bucket *bucket, void *arg)
{
  ...
  if ((zpi->type == IPSET_NET_NET) || (zpi->type == IPSET_NET_PORT_NET))
    {
      char buf[PREFIX_STRLEN];

      zebra_pbr_prefix2str (&(zpie->src), buf, sizeof (buf));
      vty_out (vty, "\tfrom %s", buf);
      if (zpie->filter_bm & PBR_FILTER_SRC_PORT && zpie->proto != IPPROTO_ICMP)
        zebra_pbr_display_port (vty, zpie->filter_bm, zpie->src_port_min,
                                zpie->src_port_max, zpie->proto);
      vty_out (vty, " to ");
      zebra_pbr_prefix2str (&(zpie->dst), buf, sizeof (buf));
      vty_out (vty, "%s", buf);
      if (zpie->filter_bm & PBR_FILTER_DST_PORT && zpie->proto != IPPROTO_ICMP)
        zebra_pbr_display_port (vty, zpie->filter_bm, zpie->dst_port_min,
                                zpie->dst_port_max, zpie->proto);
      if (zpie->proto == IPPROTO_ICMP)
        zebra_pbr_display_icmp (vty, zpie);
    }
  else if ((zpi->type == IPSET_NET) || (zpi->type == IPSET_NET_PORT))
    {
      char buf[PREFIX_STRLEN];
  ...
  ret = hook_call (zebra_pbr_ipset_entry_get_stat, zpie, &pkts, &bytes);
  if (ret && pkts > 0)
...
static int
zebra_pbr_show_ipset_walkcb (struct hash_bucket *bucket, void *arg)
{
  ...
  vty_out (vty, "IPset %s type %s family %s\n", zpi->ipset_name,
           zebra_pbr_ipset_type2str (zpi->type), family2str (zpi->family));
  unique.vty = vty;
  unique.zpi = zpi;
  unique.zns = zns;
...
size_t
zebra_pbr_tcpflags_snprintf (char *buffer, size_t len, uint16_t tcp_val)
{
  ...
  for (pnt = tcp_value_str; memcmp (pnt, &nt, sizeof (struct message)); pnt++)
    if (pnt->key & tcp_val)
      {
        len_written += snprintf (buffer + len_written, len - len_written,
                                 "%s%s", incr ? "," : "", pnt->str);
        incr++;
      }
  return len_written;
 ...
 */
void
zebra_pbr_show_ipset_list (struct vty *vty, char *ipsetname)
{
  struct zebra_pbr_ipset *zpi;
  ...
  hash_walk (zrouter.ipset_hash, zebra_pbr_show_ipset_walkcb, &uniqueipset);
}
...
static int
zebra_pbr_rule_lookup_fwmark_walkcb (struct hash_bucket *bucket, void *arg)
{
...
static void
zebra_pbr_show_iptable_unit (struct zebra_pbr_iptable *iptable,
                             struct vty *vty, struct zebra_ns *zns)
{
  int ret;
  uint64_t pkts = 0, bytes = 0;
  ...
  vty_out (vty, "IPtable %s family %s action %s (%u)\n", iptable->ipset_name,
           ...
           iptable->unique);
  if (iptable->type == IPSET_NET_PORT || iptable->type == IPSET_NET_PORT_NET)
    {
  ...
  if (iptable->tcp_flags || iptable->tcp_mask_flags)
    {
      ...
      zebra_pbr_tcpflags_snprintf (tcp_flag_str, sizeof (tcp_flag_str),
                                   iptable->tcp_flags);
  ...
  if (iptable->filter_bm & (MATCH_DSCP_SET | MATCH_DSCP_INVERSE_SET))
    {
      vty_out (vty, "\t dscp %s %d\n",
               iptable->filter_bm & MATCH_DSCP_INVERSE_SET ? "not" : "",
               iptable->dscp_value);
    }
  if (iptable->filter_bm &
      (MATCH_FLOW_LABEL_SET | MATCH_FLOW_LABEL_INVERSE_SET))
    {
      vty_out (vty, "\t flowlabel %s %d\n",
               iptable->filter_bm & MATCH_FLOW_LABEL_INVERSE_SET ? "not" : "",
               iptable->flow_label);
    }
  if (iptable->fragment)
    {
      ...
      vty_out (vty, "\t fragment%s %s\n",
               iptable->filter_bm & MATCH_FRAGMENT_INVERSE_SET ? " not" : "",
               lookup_msg (fragment_value_str, iptable->fragment, val_str));
    }
  if (iptable->protocol)
    {
      vty_out (vty, "\t protocol %d\n", iptable->protocol);
    }
  ret = hook_call (zebra_pbr_iptable_get_stat, iptable, &pkts, &bytes);
  if (ret && pkts > 0)
    vty_out (vty, "\t pkts %" PRIu64 ", bytes %" PRIu64 "\n", pkts, bytes);
  if (iptable->action != ZEBRA_IPTABLES_DROP)
    {
      ...
      prfl.ptr = NULL;
      hash_walk (zrouter.rules_hash, &zebra_pbr_rule_lookup_fwmark_walkcb,
                 &prfl);
      if (prfl.ptr)
        {
...
static int
zebra_pbr_show_iptable_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct zebra_pbr_iptable *iptable =
  ...
  else if (! strncmp (iptable_name, iptable->ipset_name,
...
void
zebra_pbr_show_iptable (struct vty *vty, char *iptable_name)
{
  struct zebra_ns *zns = zebra_ns_lookup (NS_DEFAULT);
  struct zebra_pbr_env_display env;
...
void
zebra_pbr_iptable_update_interfacelist (struct stream *s,
                                        struct zebra_pbr_iptable *zpi)
{
  uint32_t i = 0, index;
