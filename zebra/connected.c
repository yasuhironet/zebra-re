...
struct connected *
connected_check (struct interface *ifp, union prefixconstptr pu)
{
...
struct connected *
connected_check_ptp (struct interface *ifp, union prefixconstptr pu,
...
/* Called from if_up(). */
void
connected_up (struct interface *ifp, struct connected *ifc)
{
  afi_t afi;
  struct prefix p;
  struct nexthop nh = {
  ...
  switch (afi)
    ...
    case AFI_L2VPN:
    case AFI_MAX:
      flog_warn (EC_ZEBRA_CONNECTED_AFI_UNKNOWN, "Received unknown AFI: %s",
                 afi2str (afi));
      return;
      break;
    }

  metric = (ifc->metric < (uint32_t) METRIC_MAX) ? ifc->metric : ifp->metric;
  ...
  rib_add (afi, SAFI_UNICAST, zvrf->vrf->vrf_id, ZEBRA_ROUTE_CONNECT, 0, flags,
           &p, NULL, &nh, 0, zvrf->table_id, metric, 0, 0, 0, false);

  rib_add (afi, SAFI_MULTICAST, zvrf->vrf->vrf_id, ZEBRA_ROUTE_CONNECT, 0,
           flags, &p, NULL, &nh, 0, zvrf->table_id, metric, 0, 0, 0, false);
...
void
connected_add_ipv4 (struct interface *ifp, int flags,
                    ...
                    uint32_t metric)
{
  ...
  if (! dest && (prefixlen == IPV4_MAX_BITLEN) && if_is_pointopoint (ifp))
    zlog_debug ("PtP interface %s with addr %pI4/%d needs a peer address",
...
void
connected_down (struct interface *ifp, struct connected *ifc)
{
  afi_t afi;
  struct prefix p;
  struct nexthop nh = {
  ...
  if (CHECK_FLAG (ifc->conf, ZEBRA_IFC_DOWN))
    {
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s: ifc %p, %pFX already DOWN", __func__, ifc,
                    ifc->address);
      return;
    }
   ...
   * head.
   */
  rib_delete (afi, SAFI_UNICAST, zvrf->vrf->vrf_id, ZEBRA_ROUTE_CONNECT, 0, 0,
              &p, NULL, &nh, 0, zvrf->table_id, 0, 0, false);

  rib_delete (afi, SAFI_MULTICAST, zvrf->vrf->vrf_id, ZEBRA_ROUTE_CONNECT, 0,
...
static void
connected_delete_helper (struct connected *ifc, struct prefix *p)
{
...
void
connected_delete_ipv4 (struct interface *ifp, int flags,
                       const struct in_addr *addr, uint16_t prefixlen,
...
void
connected_add_ipv6 (struct interface *ifp, int flags,
                    const struct in6_addr *addr, const struct in6_addr *dest,
                    uint16_t prefixlen, const char *label, uint32_t metric)
{
...
void
connected_delete_ipv6 (struct interface *ifp, const struct in6_addr *address,
