...
static int zapi_nhg_decode (struct stream *s, int cmd,
...
zserv_encode_vrf (struct stream *s, struct zebra_vrf *zvrf)
{
  ...
  if (netns_name)
    strlcpy (data.l.netns_name, basename ((char *) netns_name), NS_NAMSIZ);
  else
...
static int
zserv_encode_nexthop (struct stream *s, struct nexthop *nexthop)
{
 ...
 */
static void
zserv_encode_error (struct stream *s, enum zebra_error_types error)
{
 ...
 */
int
zsend_interface_address (int cmd, struct zserv *client, struct interface *ifp,
  ...
  for (ALL_LIST_ELEMENTS (zrouter.client_list, node, nnode, client))
    {
      ...
      zsend_interface_nbr_address (ZEBRA_INTERFACE_NBR_ADDRESS_ADD, client,
                                   ifp, ifc);
    }
}
  ...
  for (ALL_LIST_ELEMENTS (zrouter.client_list, node, nnode, client))
    {
      ...
      zsend_interface_nbr_address (ZEBRA_INTERFACE_NBR_ADDRESS_DELETE, client,
                                   ifp, ifc);
    }
}
  ...
  for (ALL_LIST_ELEMENTS (ifp->connected, cnode, cnnode, c))
    {
      ...
      if (zsend_interface_address (ZEBRA_INTERFACE_ADDRESS_ADD, client, ifp,
                                   c) < 0)
        return -1;
    }
...
int
zsend_redistribute_route (int cmd, struct zserv *client,
                          ...
                          const struct route_entry *re)
{
  ...
  if (IS_ZEBRA_DEBUG_SEND)
    zlog_debug ("%s: %s to client %s: type %s, vrf_id %d, p %pFX", __func__,
                zserv_command_string (cmd), zebra_route_string (client->proto),
                zebra_route_string (api.type), api.vrf_id, &api.prefix);
  return zserv_send_message (client, s);
}
 ...
 */
static int
zsend_nexthop_lookup_mrib (struct zserv *client, struct ipaddr *addr,
                           struct route_entry *re, struct zebra_vrf *zvrf)
{
  struct stream *s;
  unsigned long nump;
  uint8_t num;
...
int
zsend_nhg_notify (uint16_t type, uint16_t instance, uint32_t session_id,
                  uint32_t id, enum zapi_nhg_notify_owner note)
{
  ...
  if (! client)
    {
      if (IS_ZEBRA_DEBUG_PACKET)
        {
          zlog_debug ("Not Notifying Owner: %u(%u) about %u(%d)", type,
                      instance, id, note);
        }
      return 0;
    }
  ...
  if (IS_ZEBRA_DEBUG_SEND)
    zlog_debug ("%s: type %d, id %d, note %s", __func__, type, id,
 ...
 */
static int
route_notify_internal (const struct route_node *rn, int type,
                       uint16_t instance, vrf_id_t vrf_id, uint32_t table_id,
                       ...
                       safi_t safi)
{
  ...
  if (! client || ! client->notify_owner)
    {
      if (IS_ZEBRA_DEBUG_PACKET)
        zlog_debug ("Not Notifying Owner: %s about prefix %pRN(%u) %d vrf: %u",
                    zebra_route_string (type), rn, table_id, note, vrf_id);
      return 0;
    }

  if (IS_ZEBRA_DEBUG_PACKET)
    zlog_debug ("Notifying Owner: %s about prefix %pRN(%u) %d vrf: %u",
                zebra_route_string (type), rn, table_id, note, vrf_id);
...
int
zsend_route_notify_owner (const struct route_node *rn, struct route_entry *re,
                          enum zapi_route_notify_owner note, afi_t afi,
                          safi_t safi)
{
 ...
 */
int
zsend_route_notify_owner_ctx (const struct zebra_dplane_ctx *ctx,
...
static void
zread_route_notify_request (ZAPI_HANDLER_ARGS)
{
  uint8_t notify;
...
void
zsend_rule_notify_owner (const struct zebra_dplane_ctx *ctx,
...
void
zsend_iptable_notify_owner (const struct zebra_dplane_ctx *ctx,
  ...
  if (ipt_hash)
    {
      if (op == DPLANE_OP_IPTABLE_ADD &&
          CHECK_FLAG (ipt_hash->internal_flags, IPTABLE_INSTALL_QUEUED))
        UNSET_FLAG (ipt_hash->internal_flags, IPTABLE_INSTALL_QUEUED);
      else if (op == DPLANE_OP_IPTABLE_DELETE &&
               CHECK_FLAG (ipt_hash->internal_flags, IPTABLE_UNINSTALL_QUEUED))
        UNSET_FLAG (ipt_hash->internal_flags, IPTABLE_UNINSTALL_QUEUED);
    }
  if (IS_ZEBRA_DEBUG_PACKET)
...
void
zsend_ipset_notify_owner (const struct zebra_dplane_ctx *ctx,
...
void
zsend_nhrp_neighbor_notify (int cmd, struct interface *ifp,
  ...
  for (ALL_LIST_ELEMENTS (zrouter.client_list, node, nnode, client))
    {
      ...
      s = stream_new (ZEBRA_MAX_PACKET_SIZ);
      zclient_neigh_ip_encode (s, cmd, &ip, link_layer_ipv4, ifp, ndm_state);
      stream_putw_at (s, 0, stream_get_endp (s));
      zserv_send_message (client, s);
    }
}
...
int
zsend_router_id_update (struct zserv *client, afi_t afi, struct prefix *p,
                        vrf_id_t vrf_id)
{
  int blen;
  struct stream *s;
...
int
zsend_label_manager_connect_response (struct zserv *client, vrf_id_t vrf_id,
...
static int
zsend_assign_table_chunk_response (struct zserv *client, vrf_id_t vrf_id,
...
static int
zsend_table_manager_connect_response (struct zserv *client, vrf_id_t vrf_id,
                                      uint16_t result)
{
  struct stream *s = stream_new (ZEBRA_MAX_PACKET_SIZ);
...
int
zsend_zebra_srv6_locator_add (struct zserv *client, struct srv6_locator *loc)
{
  struct stream *s = stream_new (ZEBRA_MAX_PACKET_SIZ);
...
int
zsend_zebra_srv6_locator_delete (struct zserv *client,
...
/* Nexthop register */
static void
zread_rnh_register (ZAPI_HANDLER_ARGS)
{
  struct rnh *rnh;
  struct stream *s;
  struct prefix p;
  ...
  if (IS_ZEBRA_DEBUG_NHT)
    zlog_debug ("rnh_register msg from client %s: hdr->length=%d vrf=%u",
  ...
  while (l < hdr->length)
    {
      ...
      if (p.family == AF_INET)
        ...
        {
          flog_err (EC_ZEBRA_UNKNOWN_FAMILY,
                    "rnh_register: Received unknown family type %d", p.family);
          return;
        }
      ...
      else if (! connected && CHECK_FLAG (rnh->flags, ZEBRA_NHT_CONNECTED))
      ...
      if (! exist || flag_changed)
        zebra_evaluate_rnh (zvrf, family2afi (p.family), 1, &p, safi);
...
/* Nexthop register */
static void
zread_rnh_unregister (ZAPI_HANDLER_ARGS)
{
  struct rnh *rnh;
  struct stream *s;
  struct prefix p;
  ...
  if (IS_ZEBRA_DEBUG_NHT)
    zlog_debug ("rnh_unregister msg from client %s: hdr->length=%d vrf: %u",
                zebra_route_string (client->proto), hdr->length,
                zvrf->vrf->vrf_id);
...
/* FEC register */
static void
zread_fec_register (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  unsigned short l = 0;
  struct prefix p;
  ...
  while (l < hdr->length)
    {
      ...
      if (p.family != AF_INET && p.family != AF_INET6)
        {
          flog_err (EC_ZEBRA_UNKNOWN_FAMILY,
                    "fec_register: Received unknown family type %d", p.family);
          return;
        }
      STREAM_GETC (s, p.prefixlen);
      if ((p.family == AF_INET && p.prefixlen > IPV4_MAX_BITLEN) ||
        ...
        {
          zlog_debug ("%s: Specified prefix hdr->length: %d is to long for %d",
...
/* FEC unregister */
static void
zread_fec_unregister (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  unsigned short l = 0;
  struct prefix p;
 ...
 */
static void
zread_interface_add (ZAPI_HANDLER_ARGS)
{
  struct vrf *vrf;
...
static void
zread_interface_delete (ZAPI_HANDLER_ARGS)
{
}
 ...
 */
static void
zread_interface_set_protodown (ZAPI_HANDLER_ARGS)
{
  ifindex_t ifindex;
  struct interface *ifp;
  char down;
  ...
  if (! ifp)
    {
      zlog_warn ("Cannot set protodown %s for interface %u; does not exist",
                 down ? "on" : "off", ifindex);
...
bool
zserv_nexthop_num_warn (const char *caller, const struct prefix *p,
  ...
  if (nexthop_num > zrouter.multipath_num)
    {
      ...
      flog_warn (
          ...
          caller, (p ? buff : "(NULL)"), nexthop_num, zrouter.multipath_num);
      return true;
    }
...
nexthop_from_zapi (const struct zapi_nexthop *api_nh, uint32_t flags,
                   struct prefix *p, uint16_t backup_nexthop_num)
{
  ...
  switch (api_nh->type)
    ...
    case NEXTHOP_TYPE_IPV4:
      if (IS_ZEBRA_DEBUG_RECV)
        {
          inet_ntop (AF_INET, &api_nh->gate.ipv4, nhbuf, sizeof (nhbuf));
          zlog_debug ("%s: nh=%s, vrf_id=%d", __func__, nhbuf, api_nh->vrf_id);
        }
      nexthop = nexthop_from_ipv4 (&api_nh->gate.ipv4, NULL, api_nh->vrf_id);
      break;
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      if (IS_ZEBRA_DEBUG_RECV)
        {
          inet_ntop (AF_INET, &api_nh->gate.ipv4, nhbuf, sizeof (nhbuf));
          zlog_debug ("%s: nh=%s, vrf_id=%d, ifindex=%d", __func__, nhbuf,
                      api_nh->vrf_id, api_nh->ifindex);
        }

      nexthop = nexthop_from_ipv4_ifindex (&api_nh->gate.ipv4, NULL,
                                           api_nh->ifindex, api_nh->vrf_id);
    ...
    case NEXTHOP_TYPE_IPV6:
      if (IS_ZEBRA_DEBUG_RECV)
        {
          inet_ntop (AF_INET6, &api_nh->gate.ipv6, nhbuf, sizeof (nhbuf));
          zlog_debug ("%s: nh=%s, vrf_id=%d", __func__, nhbuf, api_nh->vrf_id);
        }
    ...
    case NEXTHOP_TYPE_IPV6_IFINDEX:
      ...
      nexthop = nexthop_from_ipv6_ifindex (&api_nh->gate.ipv6, api_nh->ifindex,
                                           api_nh->vrf_id);
    ...
    case NEXTHOP_TYPE_BLACKHOLE:
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: nh blackhole %d", __func__, api_nh->bh_type);

      nexthop = nexthop_from_blackhole (api_nh->bh_type, api_nh->vrf_id);
      break;
    }
      ...
      if (api_nh->backup_num > NEXTHOP_MAX_BACKUPS)
        {
          if (IS_ZEBRA_DEBUG_RECV || IS_ZEBRA_DEBUG_EVENT)
            zlog_debug ("%s: invalid backup nh count %d", __func__,
                        api_nh->backup_num);
      ...
      for (i = 0; i < api_nh->backup_num; i++)
        {
          ...
          if (api_nh->backup_idx[i] < backup_nexthop_num)
            ...
            {
              if (IS_ZEBRA_DEBUG_RECV || IS_ZEBRA_DEBUG_EVENT)
                zlog_debug ("%s: invalid backup nh idx %d", __func__,
                            api_nh->backup_idx[i]);
              nexthop_free (nexthop);
              nexthop = NULL;
              goto done;
...
static bool
zapi_read_nexthops (struct zserv *client, struct prefix *p,
                    struct zapi_nexthop *nhops, uint32_t flags,
                    uint32_t message, uint16_t nexthop_num,
                    uint16_t backup_nh_num, struct nexthop_group **png,
                    struct nhg_backup_info **pbnhg)
{
  ...
  if (pbnhg && backup_nh_num > 0)
    {
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: adding %d backup nexthops", __func__, backup_nh_num);
  ...
  for (i = 0; i < nexthop_num; i++)
    {
          ...
          if (IS_ZEBRA_DEBUG_RECV)
            {
              ...
              zlog_debug ("%s: backup nh %s with BACKUP flag!", __func__,
                          nhbuf);
            }
      ...
      if (CHECK_FLAG (api_nh->flags, ZAPI_NEXTHOP_FLAG_LABEL) &&
          ...
          api_nh->type != NEXTHOP_TYPE_BLACKHOLE && api_nh->label_num > 0)
        {
          ...
          nexthop_add_labels (nexthop, label_type, api_nh->label_num,
                              &api_nh->labels[0]);
        }
          ...
          api_nh->type != NEXTHOP_TYPE_BLACKHOLE)
        {
          if (IS_ZEBRA_DEBUG_RECV)
            zlog_debug ("%s: adding seg6local action %s", __func__,
                        seg6local_action2str (api_nh->seg6local_action));
      ...
      if (IS_ZEBRA_DEBUG_RECV)
        {
          ...
          if (nexthop->nh_label && nexthop->nh_label->num_labels > 0)
            {
              mpls_label2str (
                  nexthop->nh_label->num_labels, nexthop->nh_label->label,
                  labelbuf, sizeof (labelbuf), nexthop->nh_label_type, false);
            }

          zlog_debug ("%s: nh=%s, vrf_id=%d %s", __func__, nhbuf,
                      api_nh->vrf_id, labelbuf);
        }
...
static int
zapi_nhg_decode (struct stream *s, int cmd, struct zapi_nhg *api_nhg)
{
  uint16_t i;
  ...
  if (api_nhg->nexthop_num <= 0)
    {
      flog_warn (EC_ZEBRA_NEXTHOP_CREATION_FAILED, "%s: No nexthops sent",
                 __func__);
      return -1;
    }
  ...
  for (i = 0; i < api_nhg->backup_nexthop_num; i++)
    {
      ...
      if (zapi_nexthop_decode (s, znh, 0, 0) != 0)
        {
          flog_warn (EC_ZEBRA_NEXTHOP_CREATION_FAILED,
                     "%s: Backup Nexthop creation failed", __func__);
          return -1;
        }
    }
...
static void
zread_nhg_del (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_nhg api_nhg = {};
  ...
  if (zapi_nhg_decode (s, hdr->command, &api_nhg) < 0)
    {
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: Unable to decode zapi_nhg sent", __func__);
      return;
    }
  ...
  if (nhe)
    {
      ...
      zsend_nhg_notify (api_nhg.proto, client->instance, client->session_id,
                        api_nhg.id, ZAPI_NHG_REMOVED);
    }
  else
    zsend_nhg_notify (api_nhg.proto, client->instance, client->session_id,
                      api_nhg.id, ZAPI_NHG_REMOVE_FAIL);
}

static void
zread_nhg_add (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_nhg api_nhg = {};
  ...
  if ((! zapi_read_nexthops (client, NULL, api_nhg.nexthops, 0, 0,
                             api_nhg.nexthop_num, api_nhg.backup_nexthop_num,
                             &nhg, NULL)) ||
...
static void
zread_route_add (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_route api;
  afi_t afi;
  ...
  if (zapi_route_decode (s, &api) < 0)
    {
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: Unable to decode zapi_route sent", __func__);
      return;
    }
  ...
  if (IS_ZEBRA_DEBUG_RECV)
    zlog_debug ("%s: p=(%u:%u)%pFX, msg flags=0x%x, flags=0x%x", __func__,
                vrf_id, api.tableid, &api.prefix, (int) api.message,
                api.flags);

  /* Allocate new route. */
  re = zebra_rib_route_entry_new (vrf_id, api.type, api.instance, api.flags,
                                  api.nhgid,
                                  api.tableid ? api.tableid : zvrf->table_id,
                                  api.metric, api.mtu, api.distance, api.tag);
  ...
  if (! re->nhe_id &&
      (! zapi_read_nexthops (client, &api.prefix, api.nexthops, api.flags,
       ...
       ! zapi_read_nexthops (client, &api.prefix, api.backup_nexthops,
                             api.flags, api.message, api.backup_nexthop_num,
  ...
  if (CHECK_FLAG (api.message, ZAPI_MESSAGE_OPAQUE))
    {
      re->opaque = XMALLOC (MTYPE_RE_OPAQUE,
  ...
  if (afi != AFI_IP6 && CHECK_FLAG (api.message, ZAPI_MESSAGE_SRCPFX))
    {
      flog_warn (EC_ZEBRA_RX_SRCDEST_WRONG_AFI,
                 "%s: Received SRC Prefix but afi is not v6", __func__);
      nexthop_group_delete (&ng);
      zebra_nhg_backup_free (&bnhg);
  ...
  ret =
      rib_add_multipath_nhe (afi, api.safi, &api.prefix, src_p, re, n, false);
...
void
zapi_re_opaque_free (struct re_opaque *opaque)
{
  XFREE (MTYPE_RE_OPAQUE, opaque);
}

static void
zread_route_del (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_route api;
  afi_t afi;
  ...
  if (afi != AFI_IP6 && CHECK_FLAG (api.message, ZAPI_MESSAGE_SRCPFX))
    {
      flog_warn (EC_ZEBRA_RX_SRCDEST_WRONG_AFI,
                 "%s: Received a src prefix while afi is not v6", __func__);
      return;
    }
  if (CHECK_FLAG (api.message, ZAPI_MESSAGE_SRCPFX))
    src_p = &api.src_prefix;
  ...
  if (IS_ZEBRA_DEBUG_RECV)
    zlog_debug ("%s: p=(%u:%u)%pFX, msg flags=0x%x, flags=0x%x", __func__,
                zvrf_id (zvrf), table_id, &api.prefix, (int) api.message,
                api.flags);

  rib_delete (afi, api.safi, zvrf_id (zvrf), api.type, api.instance, api.flags,
              &api.prefix, src_p, NULL, 0, table_id, api.metric, api.distance,
              false);
...
static void
zread_nexthop_lookup_mrib (ZAPI_HANDLER_ARGS)
{
  struct ipaddr addr;
...
static void
zread_router_id_add (ZAPI_HANDLER_ARGS)
{
  afi_t afi;
  struct prefix p;
  struct prefix zero;
  ...
  if (afi <= AFI_UNSPEC || afi >= AFI_MAX)
    {
      zlog_warn (
          "Invalid AFI %u while registering for router ID notifications", afi);
      goto stream_failure;
    }
  ...
  if ((p.family == AF_INET && p.u.prefix4.s_addr == INADDR_ANY) ||
      (p.family == AF_INET6 &&
       memcmp (&p.u.prefix6, &zero.u.prefix6, sizeof (struct in6_addr)) == 0))
    return;
...
static void
zread_router_id_delete (ZAPI_HANDLER_ARGS)
{
  afi_t afi;
...
static void
zsend_capabilities (struct zserv *client, struct zebra_vrf *zvrf)
{
  struct stream *s = stream_new (ZEBRA_MAX_PACKET_SIZ);
...
void
zsend_capabilities_all_clients (void)
{
  struct listnode *node, *nnode;
  ...
  if ((proto < ZEBRA_ROUTE_MAX) && (proto > ZEBRA_ROUTE_CONNECT))
    {
      zlog_notice (
          ...
          client->sock, zebra_route_string (proto), zvrf->vrf->vrf_id);
      if (instance)
...
static void
zread_vrf_unregister (ZAPI_HANDLER_ARGS)
{
  int i;
  afi_t afi;
 ...
 */
static int
zapi_labels_validate (const struct zapi_labels *zl)
{
  int ret = -1;
  int i, j, idx;
  uint32_t bits[8];
          ...
          if (ival & bits[idx])
            {
              ...
              if (IS_ZEBRA_DEBUG_RECV)
                zlog_debug (
                    "%s: invalid zapi mpls message: duplicate backup nexthop index %d",
                    __func__, znh->backup_idx[i]);
              goto done;
            }
 ...
 */
static void
zread_mpls_labels_add (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  ...
  if (zapi_labels_decode (s, &zl) < 0)
    {
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: Unable to decode zapi_labels sent", __func__);
      return;
    }
 ...
 */
static void
zread_mpls_labels_delete (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_labels zl;
  ...
  if (zl.nexthop_num > 0)
  ...
  else
    {
      ...
      if (CHECK_FLAG (zl.message, ZAPI_LABELS_FTN))
        mpls_ftn_uninstall (zvrf, zl.type, &zl.route.prefix, zl.route.type,
                            zl.route.instance);
    }
}
 ...
 */
static void
zread_mpls_labels_replace (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_labels zl;
...
static void
zread_sr_policy_set (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  ...
  if (zapi_sr_policy_decode (s, &zp) < 0)
    {
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: Unable to decode zapi_sr_policy sent", __func__);
      return;
    }
  zt = &zp.segment_list;
  if (zt->label_num < 1)
    {
      if (IS_ZEBRA_DEBUG_RECV)
        zlog_debug ("%s: SR-TE tunnel must contain at least one label",
                    __func__);
      return;
    }
...
static void
zread_sr_policy_delete (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_sr_policy zp;
  struct zebra_sr_policy *policy;
...
int
zsend_sr_policy_notify_status (uint32_t color, struct ipaddr *endpoint,
                               char *name, int status)
{
  ...
  if (! client)
    {
      if (IS_ZEBRA_DEBUG_PACKET)
        zlog_debug ("Not notifying pathd about policy %s"
                    " status change to %d",
                    name, status);
      return 0;
    }
  ...
  if (IS_ZEBRA_DEBUG_PACKET)
    zlog_debug ("Notifying pathd about policy %s status change"
                " to %d",
                name, status);
...
int
zsend_client_close_notify (struct zserv *client, struct zserv *closed_client)
{
  struct stream *s = stream_new (ZEBRA_MAX_PACKET_SIZ);
...
int
zsend_srv6_manager_get_locator_chunk_response (struct zserv *client,
                                               vrf_id_t vrf_id,
                                               struct srv6_locator *loc)
{
...
static void
zread_table_manager_connect (struct zserv *client, struct stream *msg,
                             vrf_id_t vrf_id)
{
  struct stream *s;
  uint8_t proto;
  ...
  if ((proto >= ZEBRA_ROUTE_MAX) || (proto <= ZEBRA_ROUTE_STATIC))
    {
      flog_err (EC_ZEBRA_TM_WRONG_PROTO, "client %d has wrong protocol %s",
...
static void
zread_label_manager_connect (struct zserv *client, struct stream *msg,
                             vrf_id_t vrf_id)
{
  struct stream *s;
  /* type of protocol (lib/zebra.h) */
...
static void
zread_get_label_chunk (struct zserv *client, struct stream *msg,
                       vrf_id_t vrf_id)
{
  struct stream *s;
  uint8_t keep;
...
static void
zread_release_label_chunk (struct zserv *client, struct stream *msg)
{
  struct stream *s;
  uint32_t start, end;
  uint8_t proto;
...
static void
zread_label_manager_request (ZAPI_HANDLER_ARGS)
{
...
static void
zread_get_table_chunk (struct zserv *client, struct stream *msg,
                       struct zebra_vrf *zvrf)
{
  struct stream *s;
  uint32_t size;
  ...
  if (! tmc)
    flog_err (EC_ZEBRA_TM_CANNOT_ASSIGN_CHUNK,
              "%s: Unable to assign Table Chunk of size %u", __func__, size);
  else
    zlog_debug ("Assigned Table Chunk %u - %u", tmc->start, tmc->end);
  /* send response back */
...
static void
zread_release_table_chunk (struct zserv *client, struct stream *msg,
                           struct zebra_vrf *zvrf)
{
  struct stream *s;
  uint32_t start, end;
...
static void
zread_table_manager_request (ZAPI_HANDLER_ARGS)
{
...
static void
zread_srv6_manager_get_locator_chunk (struct zserv *client, struct stream *msg,
                                      vrf_id_t vrf_id)
{
  struct stream *s = msg;
  uint16_t len;
...
static void
zread_srv6_manager_release_locator_chunk (struct zserv *client,
...
static void
zread_srv6_manager_request (ZAPI_HANDLER_ARGS)
{
  switch (hdr->command)
    {
...
static void
zread_pseudowire (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
      ...
      if (pw)
        {
          flog_warn (EC_ZEBRA_PSEUDOWIRE_EXISTS,
                     "%s: pseudowire %s already exists [%s]", __func__, ifname,
      ...
      if (! pw)
        {
          flog_warn (EC_ZEBRA_PSEUDOWIRE_NONEXISTENT,
                     "%s: pseudowire %s not found [%s]", __func__, ifname,
                     zserv_command_string (hdr->command));
          return;
        }
...
static void
zread_interface_set_master (ZAPI_HANDLER_ARGS)
{
...
static void
zread_vrf_label (ZAPI_HANDLER_ARGS)
{
  struct interface *ifp;
  mpls_label_t nlabel;
  afi_t afi;
  ...
  for (i = 0; i < total; i++)
    {
      ...
      if (! (zpr.rule.filter.dst_ip.family == AF_INET ||
        ...
        {
          zlog_warn ("Unsupported PBR destination IP family: %s (%hhu)",
...
static inline void
zread_tc_qdisc (ZAPI_HANDLER_ARGS)
{
...
static inline void
zread_tc_class (ZAPI_HANDLER_ARGS)
{
...
static inline void
zread_tc_filter (ZAPI_HANDLER_ARGS)
{
        ...
        case TC_FILTER_FLOWER:
          {
            ...
            if (filter_bm & TC_FLOWER_SRC_IP)
              {
                STREAM_GETC (s, filter.filter.u.flower.src_ip.family);
                STREAM_GETC (s, filter.filter.u.flower.src_ip.prefixlen);
                STREAM_GET (&filter.filter.u.flower.src_ip.u.prefix, s,
                            prefix_blen (&filter.filter.u.flower.src_ip));

                if (! (filter.filter.u.flower.src_ip.family == AF_INET ||
                       filter.filter.u.flower.src_ip.family == AF_INET6))
                  {
                    zlog_warn (
                        "Unsupported TC source IP family: %s (%hhu)",
                        family2str (filter.filter.u.flower.src_ip.family),
            ...
            if (filter_bm & TC_FLOWER_SRC_PORT)
              {
                STREAM_GETW (s, filter.filter.u.flower.src_port_min);
                STREAM_GETW (s, filter.filter.u.flower.src_port_max);
              }
            if (filter_bm & TC_FLOWER_DST_IP)
              {
                STREAM_GETC (s, filter.filter.u.flower.dst_ip.family);
                STREAM_GETC (s, filter.filter.u.flower.dst_ip.prefixlen);
                STREAM_GET (&filter.filter.u.flower.dst_ip.u.prefix, s,
                            prefix_blen (&filter.filter.u.flower.dst_ip));
                if (! (filter.filter.u.flower.dst_ip.family == AF_INET ||
                       filter.filter.u.flower.dst_ip.family == AF_INET6))
                  {
                    zlog_warn (
                        "Unsupported TC destination IP family: %s (%hhu)",
                        family2str (filter.filter.u.flower.dst_ip.family),
            ...
            if (filter_bm & TC_FLOWER_DST_PORT)
              {
                STREAM_GETW (s, filter.filter.u.flower.dst_port_min);
                STREAM_GETW (s, filter.filter.u.flower.dst_port_max);
              }
            if (filter_bm & TC_FLOWER_DSFIELD)
              {
                STREAM_GETC (s, filter.filter.u.flower.dsfield);
                STREAM_GETC (s, filter.filter.u.flower.dsfield_mask);
              }
...
static inline void
zread_ipset_entry (ZAPI_HANDLER_ARGS)
{
  ...
  for (i = 0; i < total; i++)
    {
      ...
      if (! (zpi.dst.family == AF_INET || zpi.dst.family == AF_INET6))
        {
          zlog_warn ("Unsupported PBR destination IP family: %s (%hhu)",
                     family2str (zpi.dst.family), zpi.dst.family);
          goto stream_failure;
        }
      if (! (zpi.src.family == AF_INET || zpi.src.family == AF_INET6))
        {
          zlog_warn ("Unsupported PBR source IP family: %s (%hhu)",
      ...
      zpi.backpointer = zebra_pbr_lookup_ipset_pername (ipset.ipset_name);
...
static inline void
zebra_neigh_register (ZAPI_HANDLER_ARGS)
{
  afi_t afi;
  ...
  if (afi <= AFI_UNSPEC || afi >= AFI_MAX)
    {
      zlog_warn (
          "Invalid AFI %u while registering for neighbors notifications", afi);
      goto stream_failure;
    }
...
static inline void
zebra_neigh_unregister (ZAPI_HANDLER_ARGS)
{
  afi_t afi;
...
static inline void
zebra_gre_get (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  ifindex_t idx;
  struct interface *ifp;
  ...
  if (ifp && IS_ZEBRA_IF_GRE (ifp) && zebra_if)
    {
      ...
      ifp_link = if_lookup_by_index_per_ns (
          zebra_ns_lookup (gre_info->link_nsid), gre_info->ifindex_link);
      if (ifp_link)
...
static inline void
zebra_configure_arp (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  uint8_t fam;
  ifindex_t idx;
...
static inline void
zebra_neigh_ip_add (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
...
static inline void
zebra_neigh_ip_del (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  struct zapi_neigh_ip api = {};
  int ret;
...
static inline void
zread_iptable (ZAPI_HANDLER_ARGS)
{
...
static inline void
zread_neigh_discover (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  ifindex_t ifindex;
  struct interface *ifp;
...
static inline void
zebra_gre_source_set (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  ...
  if (! ifp_link || ! ifp)
    {
      zlog_warn (
          "GRE (index %u, VRF %u) or GRE link interface (index %u, VRF %u) not found, when setting GRE params",
...
static void
zsend_error_msg (struct zserv *client, enum zebra_error_types error,
...
static void
zserv_error_no_vrf (ZAPI_HANDLER_ARGS)
{
  if (IS_ZEBRA_DEBUG_PACKET && IS_ZEBRA_DEBUG_RECV)
    zlog_debug ("ZAPI message specifies unknown VRF: %d", hdr->vrf_id);

  zsend_error_msg (client, ZEBRA_NO_VRF, hdr);
}

static void
zserv_error_invalid_msg_type (ZAPI_HANDLER_ARGS)
{
 ...
 */
void
zserv_handle_commands (struct zserv *client, struct stream_fifo *fifo)
{
  struct zmsghdr hdr;
  struct zebra_vrf *zvrf;
  struct stream *msg;
