...
#define NO_PROTO 0
   ...
   * Otherwise, if the keep field is not set (value 0) for the chunk, it will
   * be automatically released when the client disconnects or when it
   * reconnects (in case it died unexpectedly, we can know it's the same
   * because it will have the same proto+instance+session values)
   */
  ...
  void lm_get_chunk_call (struct label_manager_chunk **lmc,
  ...
  int lm_client_connect_response (uint8_t proto, uint16_t instance,
                                  ...
                                  uint8_t result);
  int lm_get_chunk_response (struct label_manager_chunk *lmc,
