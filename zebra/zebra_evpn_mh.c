...
static int
zebra_es_evi_rb_cmp (const struct zebra_evpn_es_evi *es_evi1,
                     const struct zebra_evpn_es_evi *es_evi2)
{
  return memcmp (&es_evi1->es->esi, &es_evi2->es->esi, ESI_BYTES);
}
RB_GENERATE (zebra_es_evi_rb_head, zebra_evpn_es_evi, rb_node,
             zebra_es_evi_rb_cmp);
...
zebra_evpn_es_evi_new (struct zebra_evpn_es *es, struct zebra_evpn *zevpn)
{
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("es %s evi %d new", es_evi->es->esi_str, es_evi->zevpn->vni);
 ...
 */
static void
zebra_evpn_es_evi_re_eval_send_to_client (struct zebra_evpn_es_evi *es_evi)
{
  bool old_ready;
  bool new_ready;
 ...
 */
static void
zebra_evpn_es_evi_free (struct zebra_evpn_es_evi *es_evi)
{
  struct zebra_evpn_es *es = es_evi->es;
  struct zebra_evpn *zevpn = es_evi->zevpn;

  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("es %s evi %d free", es_evi->es->esi_str, es_evi->zevpn->vni);
...
static void
zebra_evpn_local_es_evi_do_del (struct zebra_evpn_es_evi *es_evi)
{
  if (! (es_evi->flags & ZEBRA_EVPNES_EVI_LOCAL))
    return;
  ...
  list_delete_node (es_evi->zevpn->local_es_evi_list, &es_evi->l2vni_listnode);

  es_evi->flags &= ~ZEBRA_EVPNES_EVI_LOCAL;
  zebra_evpn_es_evi_free (es_evi);
}
static void
zebra_evpn_local_es_evi_del (struct zebra_evpn_es *es,
                             struct zebra_evpn *zevpn)
{
  struct zebra_evpn_es_evi *es_evi;
 ...
 */
static void
zebra_evpn_es_evi_mac_install (struct zebra_evpn_es_evi *es_evi)
{
...
static void
zebra_evpn_local_es_evi_add (struct zebra_evpn_es *es,
                             struct zebra_evpn *zevpn)
{
  struct zebra_evpn_es_evi *es_evi;
  ...
  if (! es_evi)
    {
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("local es %s evi %d add", es_evi->es->esi_str,
                    es_evi->zevpn->vni);
      es_evi->flags |= ZEBRA_EVPNES_EVI_LOCAL;
...
static void
zebra_evpn_es_evi_show_entry (struct vty *vty,
                              struct zebra_evpn_es_evi *es_evi,
  ...
  if (json_array)
    {
      ...
      json_object_int_add (json, "vni", es_evi->zevpn->vni);
      if (es_evi->flags &
          (ZEBRA_EVPNES_EVI_LOCAL | ZEBRA_EVPNES_EVI_READY_FOR_BGP))
        {
          json_flags = json_object_new_array ();
          if (es_evi->flags & ZEBRA_EVPNES_EVI_LOCAL)
            json_array_string_add (json_flags, "local");
          if (es_evi->flags & ZEBRA_EVPNES_EVI_READY_FOR_BGP)
            json_array_string_add (json_flags, "readyForBgp");
  ...
  else
    {
      ...
      vty_out (vty, " Ready for BGP: %s\n",
               (es_evi->flags & ZEBRA_EVPNES_EVI_READY_FOR_BGP) ? "yes"
                                                                : "no");
      vty_out (vty, "\n");
    }
}
...
static void
zebra_evpn_es_evi_show_one_evpn (struct zebra_evpn *zevpn, struct vty *vty,
                                 json_object *json_array, int detail)
{
  ...
  RB_FOREACH (es_evi, zebra_es_evi_rb_head, &zevpn->es_evi_rb_tree)
    {
      if (detail)
        zebra_evpn_es_evi_show_entry_detail (vty, es_evi, json_array);
      else
...
static void
zebra_evpn_es_evi_show_one_evpn_hash_cb (struct hash_bucket *bucket,
                                         void *ctxt)
{
  struct zebra_evpn *zevpn = (struct zebra_evpn *) bucket->data;
  struct evpn_mh_show_ctx *wctx = (struct evpn_mh_show_ctx *) ctxt;

  zebra_evpn_es_evi_show_one_evpn (zevpn, wctx->vty, wctx->json, wctx->detail);
}

void
zebra_evpn_es_evi_show (struct vty *vty, bool uj, int detail)
{
...
void
zebra_evpn_es_evi_show_vni (struct vty *vty, bool uj, vni_t vni, int detail)
{
  json_object *json_array = NULL;
  struct zebra_evpn *zevpn;
...
void
zebra_evpn_es_evi_init (struct zebra_evpn *zevpn)
{
...
void
zebra_evpn_es_evi_cleanup (struct zebra_evpn *zevpn)
{
  struct zebra_evpn_es_evi *es_evi;
 ...
 */
void
zebra_evpn_update_all_es (struct zebra_evpn *zevpn)
{
  struct zebra_evpn_es_evi *es_evi;
  struct listnode *node;
  ...
  if (vxlan_if)
    {
      ...
      if (if_is_operative (vxlan_if) && vxlan_zif->brslave_info.br_if)
        {
          vni = zebra_vxlan_if_vni_find (vxlan_zif, zevpn->vni);
          /* VLAN-VNI mappings may not exist */
          if (vni)
            {
              vlan_if = zvni_map_to_svi (vni->access_vlan,
...
static unsigned int
zebra_evpn_acc_vl_hash_keymake (const void *p)
{
...
static bool
zebra_evpn_acc_vl_cmp (const void *p1, const void *p2)
{
...
zebra_evpn_acc_vl_new (vlanid_t vid, struct interface *br_if)
{
  ...
  if (vlan_if)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("vlan %d bridge %s SVI %s set", vid, br_if->name,
                    vlan_if->name);
 ...
 */
static void
zebra_evpn_acc_vl_free (struct zebra_evpn_access_bd *acc_bd)
{
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
...
static void
zebra_evpn_acc_vl_cleanup_all (struct hash_bucket *bucket, void *arg)
{
 ...
 */
static void
zebra_evpn_acc_bd_free_on_deref (struct zebra_evpn_access_bd *acc_bd)
{
...
void
zebra_evpn_acc_bd_svi_set (struct zebra_if *vlan_zif, struct zebra_if *br_zif,
                           bool is_up)
{
  struct zebra_evpn_access_bd *acc_bd;
  uint16_t vid;
  ...
  if (is_up)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("vlan %d bridge %s SVI %s set", vid, tmp_br_zif->ifp->name,
                    vlan_zif->ifp->name);
    ...
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("vlan %d bridge %s SVI clear", vid, tmp_br_zif->ifp->name);
 ...
 */
void
zebra_evpn_acc_bd_svi_mac_add (struct interface *vlan_if)
{
  zebra_evpn_acc_bd_svi_set (vlan_if->info, NULL, if_is_operative (vlan_if));
}

/* called when a EVPN-L2VNI is set or cleared against a BD */
static void
zebra_evpn_acc_bd_evpn_set (struct zebra_evpn_access_bd *acc_bd,
                            struct zebra_evpn *zevpn,
                            struct zebra_evpn *old_zevpn)
{
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("access vlan %d bridge %s l2-vni %u set", acc_bd->vid,
                acc_bd->bridge_zif->ifp->name, zevpn ? zevpn->vni : 0);
  ...
  if (acc_bd->vlan_zif)
    {
      if (zevpn)
        ...
        zebra_evpn_mac_svi_del (acc_bd->vlan_zif->ifp, old_zevpn);
    }
}

/* handle VLAN->VxLAN_IF association */
void
zebra_evpn_vl_vxl_ref (uint16_t vid, vni_t vni_id, struct zebra_if *vxlan_zif)
{
  vni_t old_vni;
  struct zebra_evpn_access_bd *acc_bd;
  struct zebra_evpn *old_zevpn;
...
void
zebra_evpn_vl_vxl_deref (uint16_t vid, vni_t vni_id,
                         struct zebra_if *vxlan_zif)
{
  struct interface *br_if;
  struct zebra_evpn_access_bd *acc_bd;
...
/* handle EVPN add/del */
void
zebra_evpn_vxl_evpn_set (struct zebra_if *zif, struct zebra_evpn *zevpn,
                         bool set)
{
  struct zebra_vxlan_vni *vni;
  struct zebra_evpn_access_bd *acc_bd;
...
void
zebra_evpn_vl_mbr_ref (uint16_t vid, struct zebra_if *zif)
{
  struct interface *br_if;
  struct zebra_evpn_access_bd *acc_bd;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("access vlan %d bridge %s mbr %s ref", vid, br_if->name,
                zif->ifp->name);
...
void
zebra_evpn_vl_mbr_deref (uint16_t vid, struct zebra_if *zif)
{
  struct interface *br_if;
  struct zebra_evpn_access_bd *acc_bd;
  struct listnode *node;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("access vlan %d bridge %s mbr %s deref", vid, br_if->name,
                zif->ifp->name);
...
static void
zebra_evpn_acc_vl_adv_svi_mac_cb (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_evpn_access_bd *acc_bd = bucket->data;
...
static void
zebra_evpn_acc_vl_adv_svi_mac_all (void)
{
  hash_iterate (zmh_info->evpn_vlan_table, zebra_evpn_acc_vl_adv_svi_mac_cb,
                NULL);
}

static void
zebra_evpn_acc_vl_json_fill (struct zebra_evpn_access_bd *acc_bd,
                             json_object *json, bool detail)
{
  json_object_int_add (json, "vlan", acc_bd->vid);
  if (acc_bd->vxlan_zif)
    json_object_string_add (json, "vxlanIf", acc_bd->vxlan_zif->ifp->name);
  if (acc_bd->zevpn)
    json_object_int_add (json, "vni", acc_bd->zevpn->vni);
  if (acc_bd->mbr_zifs)
    json_object_int_add (json, "memberIfCount", listcount (acc_bd->mbr_zifs));
      ...
      for (ALL_LIST_ELEMENTS_RO (acc_bd->mbr_zifs, node, zif))
        {
          ...
          json_object_string_add (json_mbr, "ifName", zif->ifp->name);
...
static void
zebra_evpn_acc_vl_show_entry_detail (struct vty *vty,
                                     struct zebra_evpn_access_bd *acc_bd,
                                     json_object *json)
{
  struct zebra_if *zif;
...
static void
zebra_evpn_acc_vl_show_entry (struct vty *vty,
                              struct zebra_evpn_access_bd *acc_bd,
                              json_object *json)
{
  if (json)
...
static void
zebra_evpn_acc_vl_show_hash (struct hash_bucket *bucket, void *ctxt)
{
...
void
zebra_evpn_acc_vl_show (struct vty *vty, bool uj)
{
  struct evpn_mh_show_ctx wctx;
  json_object *json_array = NULL;
  ...
  hash_iterate (zmh_info->evpn_vlan_table, zebra_evpn_acc_vl_show_hash, &wctx);
...
void
zebra_evpn_acc_vl_show_detail (struct vty *vty, bool uj)
{
  struct evpn_mh_show_ctx wctx;
  json_object *json_array = NULL;
...
void
zebra_evpn_acc_vl_show_vid (struct vty *vty, bool uj, vlanid_t vid,
                            struct interface *br_if)
{
  json_object *json = NULL;
  struct zebra_evpn_access_bd *acc_bd;
 ...
 */
void
zebra_evpn_if_init (struct zebra_if *zif)
{
 ...
 */
void
zebra_evpn_if_cleanup (struct zebra_if *zif)
{
  vlanid_t vid;
 ...
 */
static uint32_t
zebra_evpn_nhid_alloc (struct zebra_evpn_es *es)
{
  uint32_t id;
  uint32_t nh_id;
...
static void
zebra_evpn_nhid_free (uint32_t nh_id, struct zebra_evpn_es *es)
{
...
static unsigned int
zebra_evpn_nh_ip_hash_keymake (const void *p)
{
...
static bool
zebra_evpn_nh_ip_cmp (const void *p1, const void *p2)
{
...
static unsigned int
zebra_evpn_nhg_hash_keymake (const void *p)
{
...
static bool
zebra_evpn_nhg_cmp (const void *p1, const void *p2)
{
...
bool
zebra_evpn_nhg_is_local_es (uint32_t nhg_id, struct zebra_evpn_es **local_es)
{
  struct zebra_evpn_es *es;
...
static void
zebra_evpn_nhg_mac_update (struct zebra_evpn_es *es)
{
  struct zebra_mac *mac;
  struct listnode *node;
  bool local_via_nw;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    zlog_debug ("mac update on es %s nhg %s", es->esi_str,
                (es->flags & ZEBRA_EVPNES_NHG_ACTIVE) ? "activate"
                                                      : "de-activate");
          ...
          if (es->flags & ZEBRA_EVPNES_NHG_ACTIVE)
            {
              if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
                zlog_debug ("%smac %pEA install via es %s nhg 0x%x",
                            (mac->flags & ZEBRA_MAC_REMOTE) ? "rem"
                                                            : "local-nw",
                            &mac->macaddr, es->esi_str, es->nhg_id);
            ...
            {
              if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
                zlog_debug ("%smac %pEA un-install es %s",
                            (mac->flags & ZEBRA_MAC_REMOTE) ? "rem"
                                                            : "local-nw",
...
static void
zebra_evpn_nhg_update (struct zebra_evpn_es *es)
{
  uint32_t nh_cnt = 0;
  ...
  if (nh_cnt)
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
        {
          ...
          zlog_debug ("es %s nhg %u add %s", es->esi_str, es->nhg_id, nh_str);
        }
  ...
  else
    {
      if (es->flags & ZEBRA_EVPNES_NHG_ACTIVE)
        {
          if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
            zlog_debug ("es %s nhg %u del", es->esi_str, es->nhg_id);
          es->flags &= ~ZEBRA_EVPNES_NHG_ACTIVE;
...
static void
zebra_evpn_es_l2_nh_show_entry (struct zebra_evpn_l2_nh *nh, struct vty *vty,
                                json_object *json_array)
{
  if (json_array)
    {
...
static void
zebra_evpn_l2_nh_show_cb (struct hash_bucket *bucket, void *ctxt)
{
...
void
zebra_evpn_l2_nh_show (struct vty *vty, bool uj)
{
  struct evpn_mh_show_ctx wctx;
  json_object *json_array = NULL;
...
static struct zebra_evpn_l2_nh *
...
static void
zebra_evpn_l2_nh_free (struct zebra_evpn_l2_nh *nh)
{
...
static void
zebra_evpn_l2_nh_es_vtep_ref (struct zebra_evpn_es_vtep *es_vtep)
{
  if (es_vtep->nh)
    return;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
    zlog_debug ("es %s vtep %pI4 nh %u ref %u", es_vtep->es->esi_str,
                &es_vtep->vtep_ip, es_vtep->nh->nh_id, es_vtep->nh->ref_cnt);
...
static void
zebra_evpn_l2_nh_es_vtep_deref (struct zebra_evpn_es_vtep *es_vtep)
{
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
    zlog_debug ("es %s vtep %pI4 nh %u deref %u", es_vtep->es->esi_str,
 ...
 */
static int
zebra_evpn_es_vtep_cmp (void *p1, void *p2)
{
...
static struct zebra_evpn_es_vtep *
zebra_evpn_es_vtep_new (struct zebra_evpn_es *es, struct in_addr vtep_ip)
{
  struct zebra_evpn_es_vtep *es_vtep;
...
static void
zebra_evpn_es_vtep_free (struct zebra_evpn_es_vtep *es_vtep)
{
...
static struct zebra_evpn_es_vtep *
zebra_evpn_es_vtep_find (struct zebra_evpn_es *es, struct in_addr vtep_ip)
{
...
static bool
zebra_evpn_es_br_port_dplane_clear (struct zebra_evpn_es *es)
{
...
zebra_evpn_es_br_port_dplane_update_needed (struct zebra_evpn_es *es)
{
  return (es->flags & ZEBRA_EVPNES_NON_DF) ||
         (es->flags & ZEBRA_EVPNES_NHG_ACTIVE) || listcount (es->es_vtep_list);
}

/* returns TRUE if dplane entry was updated */
static bool
zebra_evpn_es_br_port_dplane_update (struct zebra_evpn_es *es,
                                     const char *caller)
{
  ...
  if (es->flags & ZEBRA_EVPNES_BYPASS)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("es %s SPH filter disabled as it is in bypass",
                    es->esi_str);
    }
  else
    {
      if (listcount (es->es_vtep_list) > ES_VTEP_MAX_CNT)
        {
          zlog_warn ("es %s vtep count %d exceeds filter cnt %d", es->esi_str,
...
/* returns TRUE if dplane entry was updated */
static bool
zebra_evpn_es_df_change (struct zebra_evpn_es *es, bool new_non_df,
                         const char *caller, const char *reason)
{
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("df-change es %s %s to %s; %s: %s", es->esi_str,
                old_non_df ? "non-df" : "df", new_non_df ? "non-df" : "df",
                caller, reason);
...
/* returns TRUE if dplane entry was updated */
static bool
zebra_evpn_es_run_df_election (struct zebra_evpn_es *es, const char *caller)
{
  struct listnode *node = NULL;
  struct zebra_evpn_es_vtep *es_vtep;
   ...
   */
  if (! (es->flags & ZEBRA_EVPNES_LOCAL) ||
      (es->flags & ZEBRA_EVPNES_BYPASS) || ! zmh_info->es_originator_ip.s_addr)
    return zebra_evpn_es_df_change (es, new_non_df, caller, "not-ready");
  ...
  if (! (es->flags & ZEBRA_EVPNES_OPER_UP))
    {
      new_non_df = true;
      return zebra_evpn_es_df_change (es, new_non_df, caller, "oper-down");
    }
  ...
  if (es->df_delay_timer)
    {
      new_non_df = true;
      return zebra_evpn_es_df_change (es, new_non_df, caller, "df-delay");
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_vtep_list, node, es_vtep))
    {
      ...
      if ((es_vtep->df_pref > es->df_pref) ||
          ((es_vtep->df_pref == es->df_pref) &&
           (es_vtep->vtep_ip.s_addr < zmh_info->es_originator_ip.s_addr)))
        {
          new_non_df = true;
          break;
        }
...
static void
zebra_evpn_es_vtep_add (struct zebra_evpn_es *es, struct in_addr vtep_ip,
                        bool esr_rxed, uint8_t df_alg, uint16_t df_pref)
{
  ...
  if (! es_vtep)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("es %s vtep %pI4 add", es->esi_str, &vtep_ip);
...
static void
zebra_evpn_es_vtep_del (struct zebra_evpn_es *es, struct in_addr vtep_ip)
{
  struct zebra_evpn_es_vtep *es_vtep;
  bool dplane_updated = false;
  ...
  if (es_vtep)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("es %s vtep %pI4 del", es->esi_str, &vtep_ip);
...
static int
zebra_es_rb_cmp (const struct zebra_evpn_es *es1,
...
zebra_evpn_es_find (const esi_t *esi)
{
  struct zebra_evpn_es tmp;
 ...
 */
static struct zebra_evpn_es *
zebra_evpn_es_new (const esi_t *esi)
{
  struct zebra_evpn_es *es;
 ...
 */
static void
zebra_evpn_es_free (struct zebra_evpn_es **esp)
{
...
static int
zebra_evpn_es_send_add_to_client (struct zebra_evpn_es *es)
{
  struct zserv *client;
  struct stream *s;
  uint8_t oper_up;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("send add local es %s %pI4 active %u df_pref %u%s to %s",
                es->esi_str, &zmh_info->es_originator_ip, oper_up, es->df_pref,
...
static int
zebra_evpn_es_send_del_to_client (struct zebra_evpn_es *es)
{
  struct zserv *client;
  struct stream *s;
...
static void
zebra_evpn_es_re_eval_send_to_client (struct zebra_evpn_es *es,
                                      bool es_evi_re_reval)
{
  ...
  if ((es->flags & ZEBRA_EVPNES_LOCAL) && zmh_info->es_originator_ip.s_addr)
...
void
zebra_evpn_es_send_all_to_client (bool add)
{
          ...
          for (ALL_LIST_ELEMENTS_RO (es->es_evi_list, evi_node, es_evi))
            {
              ...
              if (add)
                zebra_evpn_es_evi_send_to_client (es, es_evi->zevpn,
                                                  true /* add */);
              else
                zebra_evpn_es_evi_send_to_client (es, es_evi->zevpn,
                                                  false /* add */);
 ...
 */
static void
zebra_evpn_es_setup_evis (struct zebra_evpn_es *es)
{
...
static void
zebra_evpn_flush_local_mac (struct zebra_mac *mac, struct interface *ifp)
{
  vlanid_t vid;
  struct zebra_if *zif;
...
static void
zebra_evpn_es_flush_local_macs (struct zebra_evpn_es *es,
  ...
  for (ALL_LIST_ELEMENTS (es->mac_list, node, nnode, mac))
    {
       ...
       * over */
      if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("VNI %u mac %pEA update; local ES %s %s", mac->zevpn->vni,
                    &mac->macaddr, es->esi_str, add ? "add" : "del");
      zebra_evpn_flush_local_mac (mac, ifp);
    }
}

void
zebra_evpn_es_local_br_port_update (struct zebra_if *zif)
{
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("es %s br_port change old %u new %u", es->esi_str, old_br_port,
                new_br_port);
...
static void
zebra_evpn_mh_dup_addr_detect_off (void)
{
  struct zebra_vrf *zvrf;
  bool old_detect;
  bool new_detect;
  ...
  if (old_detect && ! new_detect)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("evpn-mh config caused DAD addr detect chg from %s to %s",
                    old_detect ? "on" : "off", new_detect ? "on" : "off");
 ...
 */
static void
zebra_evpn_mh_advertise_reach_neigh_only (void)
{
...
static void
zebra_evpn_mh_advertise_svi_mac (void)
{
...
static void
zebra_evpn_es_df_delay_exp_cb (struct event *t)
{
  struct zebra_evpn_es *es;
 ...
 */
static void
zebra_evpn_mh_on_first_local_es (void)
{
...
static void
zebra_evpn_es_local_info_set (struct zebra_evpn_es *es, struct zebra_if *zif)
{
  if (es->flags & ZEBRA_EVPNES_LOCAL)
    return;

  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("local es %s add; nhg %u if %s", es->esi_str, es->nhg_id,
                zif->ifp->name);
  ...
  zif->es_info.es = es;
  es->df_pref =
      zif->es_info.df_pref ? zif->es_info.df_pref : EVPN_MH_DF_PREF_DEFAULT;
  ...
  if (! zmh_info->es_base_evpn)
    ...
    zebra_evpn_es_re_eval_send_to_client (es, false /* es_evi_re_reval */);

  /* Start the DF delay timer on the local ES */
  if (! es->df_delay_timer)
    event_add_timer (zrouter.master, zebra_evpn_es_df_delay_exp_cb, es,
                     ZEBRA_EVPN_MH_DF_DELAY_TIME, &es->df_delay_timer);
...
static void
zebra_evpn_es_local_info_clear (struct zebra_evpn_es **esp)
{
  struct zebra_if *zif;
  struct zebra_evpn_es *es = *esp;
  bool dplane_updated = false;
  ...
  es->flags &=
      ~(ZEBRA_EVPNES_OPER_UP | ZEBRA_EVPNES_BR_PORT | ZEBRA_EVPNES_BYPASS);
...
static void
zebra_evpn_local_es_del (struct zebra_evpn_es **esp)
{
  struct zebra_evpn_es_evi *es_evi;
  struct listnode *node = NULL;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    {
      zif = es->zif;
      zlog_debug ("local es %s del; nhg %u if %s", es->esi_str, es->nhg_id,
                  zif ? zif->ifp->name : "-");
    }
...
static void
zebra_evpn_es_remote_info_re_eval (struct zebra_evpn_es **esp)
{
  struct zebra_evpn_es *es = *esp;
      ...
      if (! (es->flags & ZEBRA_EVPNES_REMOTE))
        {
          ...
          if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
            zlog_debug ("remote es %s add; nhg %u", es->esi_str, es->nhg_id);
        }
    }
  else
    {
      if (es->flags & ZEBRA_EVPNES_REMOTE)
        {
          es->flags &= ~ZEBRA_EVPNES_REMOTE;
          if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
            zlog_debug ("remote es %s del; nhg %u", es->esi_str, es->nhg_id);
          zebra_evpn_es_free (esp);
        }
    }
}
 ...
 */
static int
zebra_evpn_local_es_update (struct zebra_if *zif, esi_t *esi)
{
...
static int
zebra_evpn_type3_esi_update (struct zebra_if *zif, uint32_t lid,
...
int
zebra_evpn_remote_es_del (const esi_t *esi, struct in_addr vtep_ip)
{
...
static void
zebra_evpn_remote_es_flush (struct zebra_evpn_es **esp)
{
  struct zebra_evpn_es_vtep *es_vtep;
  struct listnode *node;
  struct listnode *nnode;
  ...
  for (ALL_LIST_ELEMENTS (es->es_vtep_list, node, nnode, es_vtep))
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("es %s vtep %pI4 flush", es->esi_str, &es_vtep->vtep_ip);
      zebra_evpn_es_vtep_free (es_vtep);
    }
...
int
zebra_evpn_remote_es_add (const esi_t *esi, struct in_addr vtep_ip,
                          bool esr_rxed, uint8_t df_alg, uint16_t df_pref)
{
  char buf[ESI_STR_LEN];
  struct zebra_evpn_es *es;
  ...
  if (df_alg != EVPN_MH_DF_ALG_PREF)
    zlog_warn ("remote es %s vtep %pI4 add %s with unsupported df_alg %d",
               esi_to_str (esi, buf, sizeof (buf)), &vtep_ip,
               esr_rxed ? "esr" : "", df_alg);
...
void
zebra_evpn_proc_remote_es (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  ...
  if (! is_evpn_enabled ())
    {
      zlog_debug ("%s: EVPN not enabled yet we received a es_add zapi call",
                  __func__);
      return;
    }
  ...
  if (hdr->command == ZEBRA_REMOTE_ES_VTEP_ADD)
    {
      ...
      esr_rxed = (zapi_flags & ZAPI_ES_VTEP_FLAG_ESR_RXED) ? true : false;
...
void
zebra_evpn_es_mac_deref_entry (struct zebra_mac *mac)
{
 ...
 */
bool
zebra_evpn_es_mac_ref_entry (struct zebra_mac *mac, struct zebra_evpn_es *es)
{
  if (mac->es == es)
    return false;
...
bool
zebra_evpn_es_mac_ref (struct zebra_mac *mac, const esi_t *esi)
{
  struct zebra_evpn_es *es;
...
static int
zebra_evpn_es_evi_send_to_client (struct zebra_evpn_es *es,
                                  struct zebra_evpn *zevpn, bool add)
{
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("send %s local es %s evi %u to %s", add ? "add" : "del",
                es->esi_str, zevpn->vni, zebra_route_string (client->proto));
...
static int
zebra_evpn_es_sys_mac_update (struct zebra_if *zif, struct ethaddr *sysmac)
{
  int rv;
...
static int
zebra_evpn_es_lid_update (struct zebra_if *zif, uint32_t lid)
{
  int rv;
...
static int
zebra_evpn_es_type0_esi_update (struct zebra_if *zif, esi_t *esi)
{
  int rv;
...
void
zebra_evpn_es_cleanup (void)
{
  struct zebra_evpn_es *es;
  struct zebra_evpn_es *es_next;

  RB_FOREACH_SAFE (es, zebra_es_rb_head, &zmh_info->es_rb_tree, es_next)
    {
      zebra_evpn_local_es_del (&es);
      if (es)
...
static void
zebra_evpn_es_df_pref_update (struct zebra_if *zif, uint16_t df_pref)
{
  struct zebra_evpn_es *es;
  uint16_t tmp_pref;
 ...
 */
static void
zebra_evpn_es_bypass_update_macs (struct zebra_evpn_es *es,
  ...
  for (ALL_LIST_ELEMENTS (es->mac_list, node, nnode, mac))
    {
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("VNI %u mac %pEA %s update es %s", mac->zevpn->vni,
                    &mac->macaddr, bypass ? "bypass" : "non-bypass",
                    es->esi_str);
      zebra_evpn_flush_local_mac (mac, ifp);
    }
  ...
  for (ALL_LIST_ELEMENTS (zif->mac_list, node, nnode, mac))
    {
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("VNI %u mac %pEA %s update ifp %s", mac->zevpn->vni,
                    &mac->macaddr, bypass ? "bypass" : "non-bypass",
                    ifp->name);
      zebra_evpn_flush_local_mac (mac, ifp);
    }
...
void
zebra_evpn_es_bypass_update (struct zebra_evpn_es *es, struct interface *ifp,
                             bool bypass)
{
  bool old_bypass;
  bool dplane_updated;
...
static void
zebra_evpn_es_bypass_cfg_update (struct zebra_if *zif, bool bypass)
{
...
bool
zebra_evpn_is_if_es_capable (struct zebra_if *zif)
{
...
void
zebra_evpn_if_es_print (struct vty *vty, json_object *json,
                        struct zebra_if *zif)
{
  ...
  if (json)
    {
      ...
      if (zif->es_info.lid || ! is_zero_mac (&zif->es_info.sysmac))
        {
          json_object_int_add (json_evpn, "esId", zif->es_info.lid);
      ...
      if (zif->flags & ZIF_FLAG_EVPN_MH_UPLINK)
        json_object_string_add (
            json_evpn, "uplink",
            CHECK_FLAG (zif->flags, ZIF_FLAG_EVPN_MH_UPLINK_OPER_UP) ? "up"
                                                                     : "down");
    }
  else
    {
...
static void
zebra_evpn_local_mac_oper_state_change (struct zebra_evpn_es *es)
{
  struct zebra_mac *mac;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (es->mac_list, node, mac))
    {
      if (! (mac->flags & ZEBRA_MAC_LOCAL) || ! zebra_evpn_mac_is_static (mac))
        continue;

      if (es->flags & ZEBRA_EVPNES_OPER_UP)
        {
          if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
            zlog_debug ("VNI %u mac %pEA move to acc %s es %s %s ",
                        mac->zevpn->vni, &mac->macaddr, es->zif->ifp->name,
                        es->esi_str,
                        (es->flags & ZEBRA_EVPNES_OPER_UP) ? "up" : "down");
          /* switch the local macs to access port */
          if (zebra_evpn_sync_mac_dp_install (mac, false /*set_inactive*/,
      ...
      else
        {
          ...
          if (! (es->flags & ZEBRA_EVPNES_NHG_ACTIVE))
            continue;
          if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
            zlog_debug ("VNI %u mac %pEA move to nhg %u es %s %s ",
                        mac->zevpn->vni, &mac->macaddr, es->nhg_id,
                        es->esi_str,
                        (es->flags & ZEBRA_EVPNES_OPER_UP) ? "up" : "down");
          zebra_evpn_rem_mac_install (mac->zevpn, mac, true /*was_static*/);
        }
    }
}

void
zebra_evpn_es_if_oper_state_change (struct zebra_if *zif, bool up)
{
  struct zebra_evpn_es *es = zif->es_info.es;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("es %s state changed to %s ", es->esi_str, up ? "up" : "down");
  if (up)
    es->flags |= ZEBRA_EVPNES_OPER_UP;
  else
    es->flags &= ~ZEBRA_EVPNES_OPER_UP;
...
static char *
zebra_evpn_es_vtep_str (char *vtep_str, struct zebra_evpn_es *es,
      ...
      if (first)
        {
          ...
          strlcat (
              vtep_str,
              inet_ntop (AF_INET, &zvtep->vtep_ip, ip_buf, sizeof (ip_buf)),
              vtep_str_size);
        }
      else
        {
...
static void
zebra_evpn_es_json_vtep_fill (struct zebra_evpn_es *es,
  ...
  for (ALL_LIST_ELEMENTS_RO (es->es_vtep_list, node, es_vtep))
    {
      ...
      if (es_vtep->nh)
        json_object_int_add (json_vtep_entry, "nexthopId", es_vtep->nh->nh_id);
      json_object_array_add (json_vteps, json_vtep_entry);
    }
}

static void
zebra_evpn_es_show_entry (struct vty *vty, struct zebra_evpn_es *es,
                          json_object *json_array)
{
  char type_str[5];
  ...
  if (json_array)
    {
      ...
      if (es->flags &
          (ZEBRA_EVPNES_LOCAL | ZEBRA_EVPNES_REMOTE | ZEBRA_EVPNES_NON_DF))
        {
          json_flags = json_object_new_array ();
          if (es->flags & ZEBRA_EVPNES_LOCAL)
            json_array_string_add (json_flags, "local");
      ...
      if (es->zif)
        json_object_string_add (json, "accessPort", es->zif->ifp->name);
  ...
  else
    {
      ...
      vty_out (vty, "%-30s %-4s %-21s %s\n", es->esi_str, type_str,
               es->zif ? es->zif->ifp->name : "-", vtep_str);
    }
}

static void
zebra_evpn_es_show_entry_detail (struct vty *vty, struct zebra_evpn_es *es,
                                 json_object *json)
{
  char type_str[80];
  char alg_buf[EVPN_DF_ALG_STR_LEN];
  ...
  if (json)
    {
      ...
      if (es->flags)
        {
          ...
          if (es->flags & ZEBRA_EVPNES_NHG_ACTIVE)
            json_array_string_add (json_flags, "nexthopGroupActive");
          json_object_object_add (json, "flags", json_flags);
        }
      ...
      if (es->df_delay_timer)
        json_object_string_add (json, "dfDelayTimer",
  ...
  else
    {
      ...
      if (es->flags & ZEBRA_EVPNES_LOCAL)
        {
          ...
          vty_out (vty, " Bridge port: %s\n",
                   (es->flags & ZEBRA_EVPNES_BR_PORT) ? "yes" : "no");
        }
      vty_out (vty, " Ready for BGP: %s\n",
               (es->flags & ZEBRA_EVPNES_READY_FOR_BGP) ? "yes" : "no");
      if (es->flags & ZEBRA_EVPNES_BYPASS)
      ...
      for (ALL_LIST_ELEMENTS_RO (es->es_vtep_list, node, es_vtep))
        {
          vty_out (vty, "     %pI4", &es_vtep->vtep_ip);
          if (es_vtep->flags & ZEBRA_EVPNES_VTEP_RXED_ESR)
...
void
zebra_evpn_es_show (struct vty *vty, bool uj)
{
  struct zebra_evpn_es *es;
  json_object *json_array = NULL;
...
void
zebra_evpn_es_show_detail (struct vty *vty, bool uj)
{
  struct zebra_evpn_es *es;
  json_object *json_array = NULL;
...
void
zebra_evpn_es_show_esi (struct vty *vty, bool uj, esi_t *esi)
{
  struct zebra_evpn_es *es;
...
int
zebra_evpn_mh_if_write (struct vty *vty, struct interface *ifp)
{
  ...
  if (! type_3_esi && memcmp (&zif->es_info.esi, zero_esi, sizeof (*zero_esi)))
...
DEFPY_HIDDEN (zebra_evpn_es_bypass,
...
void
zebra_evpn_mh_json (json_object *json)
{
  json_object *json_array;
  char thread_buf[EVENT_TIMER_STRLEN];
  ...
  json_object_int_add (json, "uplinkConfigCount", zmh_info->uplink_cfg_cnt);
...
void
zebra_evpn_mh_print (struct vty *vty)
{
 ...
 */
void
zebra_evpn_es_set_base_evpn (struct zebra_evpn *zevpn)
{
  struct listnode *node;
  struct zebra_evpn_es *es;
 ...
 */
void
zebra_evpn_es_clear_base_evpn (struct zebra_evpn *zevpn)
{
  struct listnode *node;
  struct zebra_evpn_es *es;
...
static int
zebra_evpn_es_get_one_base_evpn_cb (struct hash_bucket *b, void *data)
{
 ...
 */
static void
zebra_evpn_es_get_one_base_evpn (void)
{
  struct zebra_vrf *zvrf;
 ...
 */
bool
zebra_evpn_is_es_bond (struct interface *ifp)
{
  struct zebra_if *zif = ifp->info;
...
bool
zebra_evpn_is_es_bond_member (struct interface *ifp)
{
  struct zebra_if *zif = ifp->info;
...
void
zebra_evpn_mh_update_protodown_bond_mbr (struct zebra_if *zif, bool clear,
                                         const char *caller)
{
  bool new_protodown;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES && (new_protodown_rc != old_protodown_rc))
    zlog_debug ("%s bond mbr %s protodown_rc changed; old 0x%x new 0x%x",
                caller, zif->ifp->name, old_protodown_rc, new_protodown_rc);
...
static void
zebra_evpn_mh_update_protodown_bond (struct zebra_if *bond_zif)
{
  struct zebra_if *zif;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (bond_zif->bond_info.mbr_zifs, node, zif))
    {
      zebra_evpn_mh_update_protodown_bond_mbr (zif, false /*clear*/, __func__);
    }
}

/* The global EVPN MH protodown rc is applied to all local ESs */
static void
zebra_evpn_mh_update_protodown_es (struct zebra_evpn_es *es,
                                   bool resync_dplane)
{
  struct zebra_if *zif;
  ...
  old_protodown_rc = zif->protodown_rc;
  zif->protodown_rc &= ~ZEBRA_PROTODOWN_EVPN_ALL;
  zif->protodown_rc |= (zmh_info->protodown_rc & ZEBRA_PROTODOWN_EVPN_ALL);

  if (IS_ZEBRA_DEBUG_EVPN_MH_ES && (old_protodown_rc != zif->protodown_rc))
...
static void
zebra_evpn_mh_clear_protodown_es (struct zebra_evpn_es *es)
{
  struct zebra_if *zif;
  uint32_t old_protodown_rc;
...
static void
zebra_evpn_mh_update_protodown_es_all (void)
{
  struct listnode *node;
  struct zebra_evpn_es *es;
...
static void
zebra_evpn_mh_update_protodown (uint32_t protodown_rc, bool set)
{
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
    zlog_debug ("mh protodown_rc changed; old 0x%x new 0x%x", old_protodown_rc,
                zmh_info->protodown_rc);
  zebra_evpn_mh_update_protodown_es_all ();
}

static inline bool
zebra_evpn_mh_is_all_uplinks_down (void)
{
  return zmh_info->uplink_cfg_cnt && ! zmh_info->uplink_oper_up_cnt;
}

static void
zebra_evpn_mh_uplink_oper_flags_update (struct zebra_if *zif, bool set)
{
...
static void
zebra_evpn_mh_uplink_cfg_update (struct zebra_if *zif, bool set)
{
...
void
zebra_evpn_mh_uplink_oper_update (struct zebra_if *zif)
{
  bool old_protodown = zebra_evpn_mh_is_all_uplinks_down ();
  bool new_protodown;
...
static void
zebra_evpn_mh_startup_delay_exp_cb (struct event *t)
{
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
...
static void
zebra_evpn_mh_startup_delay_timer_start (const char *rc)
{
  ...
  if (zmh_info->startup_delay_time)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("startup-delay timer started for %d sec on %s",
                    zmh_info->startup_delay_time, rc);
      event_add_timer (zrouter.master, zebra_evpn_mh_startup_delay_exp_cb,
 ...
 ****************************************************************************/
void
zebra_evpn_proc_remote_nh (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  vrf_id_t vrf_id;
  struct ipaddr nh;
  ...
  if (hdr->command == ZEBRA_EVPN_REMOTE_NH_ADD)
    {
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("evpn remote nh %d %pIA rmac %pEA add pfx %pFX", vrf_id,
                    &nh, &rmac, &dummy_prefix);
    ...
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
        zlog_debug ("evpn remote nh %d %pIA del pfx %pFX", vrf_id, &nh,
                    &dummy_prefix);
...
/*****************************************************************************/
void
zebra_evpn_mh_config_write (struct vty *vty)
{
...
int
zebra_evpn_mh_neigh_holdtime_update (struct vty *vty, uint32_t duration,
                                     bool set_default)
{
  if (set_default)
...
int
zebra_evpn_mh_mac_holdtime_update (struct vty *vty, uint32_t duration,
                                   bool set_default)
{
  if (set_default)
...
int
zebra_evpn_mh_startup_delay_update (struct vty *vty, uint32_t duration,
                                    bool set_default)
{
  if (set_default)
...
int
zebra_evpn_mh_redirect_off (struct vty *vty, bool redirect_off)
{
...
void
zebra_evpn_interface_init (void)
{
...
zebra_evpn_mh_init (void)
{
  ...
  zmh_info->evpn_vlan_table =
      hash_create (zebra_evpn_acc_vl_hash_keymake, zebra_evpn_acc_vl_cmp,
...
void
zebra_evpn_mh_terminate (void)
{
  list_delete (&zmh_info->local_es_list);

  hash_iterate (zmh_info->evpn_vlan_table, zebra_evpn_acc_vl_cleanup_all,
                NULL);
