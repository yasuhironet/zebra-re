...
static struct irdp_interface *
irdp_if_get (struct interface *ifp)
{
...
static int
irdp_if_delete (struct interface *ifp)
{
  struct zebra_if *zi = ifp->info;
  if (! zi)
    return 0;
...
static const char *
inet_2a (uint32_t a, char *b, size_t b_len)
{
...
static int
if_group (struct interface *ifp, int sock, uint32_t group, int add_leave)
{
  struct ip_mreq m;
  struct prefix *p;
  int ret;
  ...
  if (! p)
    {
      flog_warn (EC_ZEBRA_NO_IFACE_ADDR, "IRDP: can't get address for %s",
                 ifp->name);
      return 1;
    }
...
static void
if_set_defaults (struct irdp_interface *irdp)
{
...
irdp_if_start (struct interface *ifp, int multicast, int set_defaults)
{
  ...
  if (! (ifp->flags & IFF_UP))
    {
      flog_warn (EC_ZEBRA_IRDP_IFACE_DOWN, "IRDP: Interface is down %s",
                 ifp->name);
    }
      ...
      if (! (ifp->flags & (IFF_MULTICAST | IFF_ALLMULTI)))
        {
          flog_warn (EC_ZEBRA_IRDP_IFACE_MCAST_DISABLED,
                     "IRDP: Interface not multicast enabled %s", ifp->name);
        }
    }
...
int
irdp_config_write (struct vty *vty, struct interface *ifp)
{
  struct zebra_if *zi = ifp->info;
  struct irdp_interface *irdp = zi->irdp;
  struct Adv *adv;
  ...
  if (irdp->flags & IF_ACTIVE || irdp->flags & IF_SHUTDOWN)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (irdp->AdvPrefList, node, adv))
        vty_out (vty, " ip irdp address %s preference %d\n",
                 inet_2a (adv->ip.s_addr, b1, sizeof (b1)), adv->pref);
