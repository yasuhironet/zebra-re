...
#define ZEBRA_BRIDGE_MASTER_UP         (1 << 2)
  ...
  struct zebra_vxlan_if_ctx
  {
    /* input */
    struct zebra_if *zif;
    int (*func) (struct zebra_if *zif, struct zebra_vxlan_vni *vni, void *arg);
...
#define IS_ZEBRA_IF_BRIDGE_VLAN_AWARE(zif)                                    \
  ...
  extern void zebra_l2_bridge_add_update (
      struct interface *ifp, struct zebra_l2info_bridge *bridge_info, int add);
  ...
  extern void zebra_l2if_update_bridge (struct interface *ifp,
                                        uint8_t chgflags);
