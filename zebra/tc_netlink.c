...
static void
tc_calc_rate_table (struct tc_ratespec *ratespec, uint32_t *table,
                    uint32_t mtu)
{
  if (mtu == 0)
    mtu = 2047;
...
static int
tc_flower_get_inet_prefix (const struct prefix *prefix,
...
static int
tc_flower_get_inet_mask (const struct prefix *prefix, struct inet_prefix *addr)
{
  addr->family = prefix->family;
 ...
 */
static ssize_t
netlink_qdisc_msg_encode (int cmd, struct zebra_dplane_ctx *ctx, void *data,
                          size_t datalen)
{
  struct nlsock *nl;
 ...
 */
static ssize_t
netlink_tclass_msg_encode (int cmd, struct zebra_dplane_ctx *ctx, void *data,
                           size_t datalen)
{
  ...
  req->t.tcm_handle =
      TC_H_MAKE (TC_QDISC_MAJOR_ZEBRA, dplane_ctx_tc_class_get_handle (ctx));
        ...
        case TC_QDISC_HTB:
          {
            ...
            if (rate >> 32 != 0)
              {
                nl_attr_put (&req->n, datalen, TCA_HTB_RATE64, &rate,
                             sizeof (rate));
              }
...
static int
netlink_tfilter_flower_port_type (uint8_t ip_proto, bool src)
{
...
static void
netlink_tfilter_flower_put_options (struct nlmsghdr *n, size_t datalen,
                                    struct zebra_dplane_ctx *ctx)
{
  ...
  if (filter_bm & TC_FLOWER_SRC_PORT)
    {
      ...
      if (max > min)
        {
          nl_attr_put16 (n, datalen, TCA_FLOWER_KEY_PORT_SRC_MIN, htons (min));

          nl_attr_put16 (n, datalen, TCA_FLOWER_KEY_PORT_SRC_MAX, htons (max));
        }
      else
        {
  ...
  if (filter_bm & TC_FLOWER_DST_PORT)
    {
      ...
      if (max > min)
        {
          nl_attr_put16 (n, datalen, TCA_FLOWER_KEY_PORT_DST_MIN, htons (min));

          nl_attr_put16 (n, datalen, TCA_FLOWER_KEY_PORT_DST_MAX, htons (max));
        }
      else
        {
          int type = netlink_tfilter_flower_port_type (
  ...
  classid =
      TC_H_MAKE (TC_QDISC_MAJOR_ZEBRA, dplane_ctx_tc_filter_get_classid (ctx));
 ...
 */
static ssize_t
netlink_tfilter_msg_encode (int cmd, struct zebra_dplane_ctx *ctx, void *data,
                            size_t datalen)
{
  enum dplane_op_e op = dplane_ctx_get_op (ctx);
  ...
  if (op == DPLANE_OP_TC_FILTER_ADD || op == DPLANE_OP_TC_FILTER_UPDATE)
    {
      ...
      zlog_debug (
          ...
          op == DPLANE_OP_TC_FILTER_UPDATE ? "update" : "add", priority,
          protocol, kind_str, dplane_ctx_tc_filter_get_handle (ctx),
...
static ssize_t
netlink_newqdisc_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                              size_t buflen)
{
...
static ssize_t
netlink_delqdisc_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                              size_t buflen)
{
...
static ssize_t
netlink_newtclass_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                               size_t buflen)
{
...
static ssize_t
netlink_deltclass_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                               size_t buflen)
{
...
static ssize_t
netlink_newtfilter_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                                size_t buflen)
{
...
static ssize_t
netlink_deltfilter_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                                size_t buflen)
{
                                 ...
                                 struct zebra_dplane_ctx *ctx)
{
  ...
  if (op == DPLANE_OP_TC_QDISC_INSTALL)
    {
      ret = netlink_batch_add_msg (bth, ctx, netlink_newqdisc_msg_encoder,
                                   false);
    }
  else if (op == DPLANE_OP_TC_QDISC_UNINSTALL)
    {
      ret = netlink_batch_add_msg (bth, ctx, netlink_delqdisc_msg_encoder,
                                   false);
    }
  else
    {
                                 ...
                                 struct zebra_dplane_ctx *ctx)
{
  ...
  if (op == DPLANE_OP_TC_CLASS_ADD || op == DPLANE_OP_TC_CLASS_UPDATE)
    {
      ret = netlink_batch_add_msg (bth, ctx, netlink_newtclass_msg_encoder,
                                   false);
    }
  else if (op == DPLANE_OP_TC_CLASS_DELETE)
    {
      ret = netlink_batch_add_msg (bth, ctx, netlink_deltclass_msg_encoder,
                                   false);
    }
  else
    {
                                  ...
                                  struct zebra_dplane_ctx *ctx)
{
  ...
  if (op == DPLANE_OP_TC_FILTER_ADD)
    {
      ret = netlink_batch_add_msg (bth, ctx, netlink_newtfilter_msg_encoder,
                                   false);
    }
  else if (op == DPLANE_OP_TC_FILTER_UPDATE)
    {
      ...
      (void) netlink_batch_add_msg (bth, ctx, netlink_deltfilter_msg_encoder,
                                    false);
      ret = netlink_batch_add_msg (bth, ctx, netlink_newtfilter_msg_encoder,
                                   false);
    }
    ...
    {
      ret = netlink_batch_add_msg (bth, ctx, netlink_deltfilter_msg_encoder,
                                   false);
    }
  else
    {
 ...
 */
static int
netlink_request_filters (struct zebra_ns *zns, int family, int type,
                         ifindex_t ifindex)
{
  struct
  {
 ...
 */
static int
netlink_request_qdiscs (struct zebra_ns *zns, int family, int type)
{
  struct
  {
    struct nlmsghdr n;
...
int
netlink_qdisc_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  struct tcmsg *tcm;
      ...
      if (startup && TC_H_MAJ (tcm->tcm_handle) == TC_QDISC_MAJOR_ZEBRA)
        {
          ...
          zlog_debug ("%s: %s leftover qdisc: ifindex %d kind %s", __func__,
...
int
netlink_tclass_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  struct tcmsg *tcm;
...
int
netlink_tfilter_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  struct tcmsg *tcm;
...
int
netlink_qdisc_read (struct zebra_ns *zns)
{
  int ret;
  ...
  ret = netlink_parse_info (netlink_qdisc_change, &zns->netlink_cmd, &dp_info,
                            0, true);
  if (ret < 0)
    return ret;
...
int
netlink_tfilter_read_for_interface (struct zebra_ns *zns, ifindex_t ifindex)
{
  int ret;
  struct zebra_dplane_info dp_info;
