...
#define DYNAMIC_CLIENT_GR_DISABLED(_client)                                   \
   ...
   */
  extern struct zserv *zserv_find_client (uint8_t proto,
   ...
   *    The Zebra API client.
   */
  struct zserv *zserv_find_client_session (uint8_t proto,
                                           unsigned short instance,
                                           uint32_t session_id);
   ...
   * release_client() api, to ensure that the in-use marker is cleared
   * properly.
   *
   * Returns:
   *    The Zebra API client.
   ...
   * Close a client.
   *
   * Kills a client's thread, removes the client from the client list and
  ...
  extern void zread_client_capabilities (struct zserv *client,
                                         struct zmsghdr *hdr,
                                         struct stream *msg,
                                         struct zebra_vrf *zvrf);
