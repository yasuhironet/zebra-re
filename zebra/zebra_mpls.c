...
static int fec_change_update_lsp (struct zebra_vrf *zvrf,
                                  struct zebra_fec *fec,
...
static struct zebra_fec *fec_find (struct route_table *table,
                                   struct prefix *p);
...
static int mpls_lsp_uninstall_all (struct hash *lsp_table,
...
static void mpls_lsp_uninstall_all_type (struct hash_bucket *bucket,
                                         void *ctxt);
...
static int lsp_backup_znh_install (struct zebra_lsp *lsp,
                                   enum lsp_types_t type,
                                   const struct zapi_nexthop *znh);
 ...
 */
static void
clear_nhlfe_installed (struct zebra_lsp *lsp)
{
 ...
 */
static int
lsp_install (struct zebra_vrf *zvrf, mpls_label_t label, struct route_node *rn,
  ...
  for (nexthop = re->nhe->nhg.nexthop; nexthop; nexthop = nexthop->next)
    {
      ...
      if (nhlfe)
      ...
      else
        {
          ...
          nhlfe = nhlfe_add (lsp, lsp_type, nexthop->type, &nexthop->gate,
      ...
      for (rn = route_top (zvrf->fec_table[af]); rn; rn = route_next (rn))
        {
          ...
          if (IS_ZEBRA_DEBUG_MPLS)
            zlog_debug ("Update fec %pRN new label %u upon label block", rn,
                        new_label);
 ...
 */
static uint32_t
fec_derive_label_from_index (struct zebra_vrf *zvrf, struct zebra_fec *fec)
{
  uint32_t label;
 ...
 */
static int
fec_change_update_lsp (struct zebra_vrf *zvrf, struct zebra_fec *fec,
                       mpls_label_t old_label)
{
 ...
 */
static int
fec_send (struct zebra_fec *fec, struct zserv *client)
{
  struct stream *s;
  struct route_node *rn;
 ...
 */
static void
fec_update_clients (struct zebra_fec *fec)
{
 ...
 */
static void
fec_print (struct zebra_fec *fec, struct vty *vty)
{
  struct route_node *rn;
  struct listnode *node;
  struct zserv *client;
...
fec_add (struct route_table *table, struct prefix *p, mpls_label_t label,
         uint32_t flags, uint32_t label_index)
{
  struct route_node *rn;
  struct zebra_fec *fec;
 ...
 */
static int
nhlfe_nexthop_active_ipv4 (struct zebra_nhlfe *nhlfe, struct nexthop *nexthop)
{
  struct route_table *table;
 ...
 */
static int
nhlfe_nexthop_active_ipv6 (struct zebra_nhlfe *nhlfe, struct nexthop *nexthop)
{
  struct route_table *table;
 ...
 */
static int
nhlfe_nexthop_active (struct zebra_nhlfe *nhlfe)
{
  struct nexthop *nexthop;
 ...
 */
static void
lsp_select_best_nhlfe (struct zebra_lsp *lsp)
{
  struct zebra_nhlfe *nhlfe;
  ...
  frr_each_safe (nhlfe_list, &lsp->nhlfe_list, nhlfe)
    {
      ...
      UNSET_FLAG (nhlfe->flags, (NHLFE_FLAG_SELECTED | NHLFE_FLAG_MULTIPATH));
      ...
      if (CHECK_FLAG (lsp->flags, LSP_FLAG_INSTALLED) && ! changed)
        {
          ...
          if ((nh_sel && ! nh_inst) || (nh_sel && nh_inst && nh_chg) ||
              (nh_inst && ! nh_sel))
            changed = 1;
        }
 ...
 */
static void
lsp_uninstall_from_kernel (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_lsp *lsp;
 ...
 */
static void
lsp_schedule (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_lsp *lsp;
  ...
  if (IS_ZEBRA_DEBUG_MPLS)
    {
      ...
      zlog_debug (
          ...
          lsp->ile.in_label, oldbest ? buf : "NULL", newbest ? buf2 : "NULL",
          lsp->flags, lsp->num_ecmp);
    }
      ...
      if (! newbest)
      ...
      else if (CHECK_FLAG (lsp->flags, LSP_FLAG_CHANGED))
        {
          ...
          switch (dplane_lsp_update (lsp))
            ...
            case ZEBRA_DPLANE_REQUEST_FAILURE:
              flog_warn (EC_ZEBRA_LSP_INSTALL_FAILURE,
                         "LSP Update Failure: %u", lsp->ile.in_label);
              break;
            case ZEBRA_DPLANE_REQUEST_SUCCESS:
              zvrf->lsp_installs++;
              break;
 ...
 */
static int
lsp_processq_add (struct zebra_lsp *lsp)
{
 ...
 */
static void
lsp_check_free (struct hash *lsp_table, struct zebra_lsp **plsp)
{
  struct zebra_lsp *lsp;
...
static void
lsp_free_nhlfe (struct zebra_lsp *lsp)
{
  struct zebra_nhlfe *nhlfe;
 ...
 */
static void
lsp_free (struct hash *lsp_table, struct zebra_lsp **plsp)
{
  struct zebra_lsp *lsp;
 ...
 */
static char *
nhlfe2str (const struct zebra_nhlfe *nhlfe, char *buf, int size)
{
 ...
 */
static int
nhlfe_nhop_match (struct zebra_nhlfe *nhlfe, enum nexthop_types_t gtype,
                  const union g_addr *gate, ifindex_t ifindex)
{
  struct nexthop *nhop;
  int cmp = 1;
  ...
  switch (nhop->type)
    ...
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      cmp =
          memcmp (&(nhop->gate.ipv4), &(gate->ipv4), sizeof (struct in_addr));
    ...
    case NEXTHOP_TYPE_IPV6_IFINDEX:
      cmp =
          memcmp (&(nhop->gate.ipv6), &(gate->ipv6), sizeof (struct in6_addr));
 ...
 */
static struct zebra_nhlfe *
nhlfe_find (struct nhlfe_list_head *list, enum lsp_types_t lsp_type,
            enum nexthop_types_t gtype, const union g_addr *gate,
            ifindex_t ifindex)
{
  struct zebra_nhlfe *nhlfe;
           ...
           const mpls_label_t *labels, bool is_backup)
{
  ...
  /* Allocate new object */
  nhlfe = nhlfe_alloc (lsp, lsp_type, gtype, gate, ifindex, vrf_id, num_labels,
                       labels);
 ...
 */
static void
nhlfe_free (struct zebra_nhlfe *nhlfe)
{
  if (! nhlfe)
    return;
 ...
 */
static int
nhlfe_del (struct zebra_nhlfe *nhlfe)
{
  struct zebra_lsp *lsp;
 ...
 */
static void
nhlfe_out_label_update (struct zebra_nhlfe *nhlfe,
                        struct mpls_label_stack *nh_label)
{
...
static int
mpls_lsp_uninstall_all (struct hash *lsp_table, struct zebra_lsp *lsp,
                        enum lsp_types_t type)
{
  struct zebra_nhlfe *nhlfe;
  int schedule_lsp = 0;
  ...
  if (schedule_lsp)
    {
      if (IS_ZEBRA_DEBUG_MPLS)
        {
          zlog_debug ("Schedule LSP in-label %u flags 0x%x", lsp->ile.in_label,
                      lsp->flags);
        }
      if (lsp_processq_add (lsp))
        return -1;
...
static json_object *
nhlfe_json (struct zebra_nhlfe *nhlfe)
{
 ...
 */
static void
nhlfe_print (struct zebra_nhlfe *nhlfe, struct vty *vty, const char *indent)
{
  struct nexthop *nexthop;
 ...
 */
static void
lsp_print (struct vty *vty, struct zebra_lsp *lsp)
{
 ...
 */
static int
lsp_cmp (const struct zebra_lsp *lsp1, const struct zebra_lsp *lsp2)
{
 ...
 */
void
zebra_mpls_lsp_dplane_result (struct zebra_dplane_ctx *ctx)
{
  struct zebra_vrf *zvrf;
  mpls_label_t label;
  struct zebra_ile tmp_ile;
  ...
  if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
    zlog_debug ("LSP dplane ctx %p, op %s, in-label %u, result %s", ctx,
                dplane_op2str (op), dplane_ctx_get_in_label (ctx),
      ...
      if (lsp == NULL)
        {
          if (IS_ZEBRA_DEBUG_DPLANE)
            zlog_debug ("LSP ctx %p: in-label %u not found", ctx,
      ...
      if (status != ZEBRA_DPLANE_REQUEST_SUCCESS)
        {
          ...
          clear_nhlfe_installed (lsp);
          flog_warn (EC_ZEBRA_LSP_INSTALL_FAILURE,
                     "LSP Install Failure: in-label %u", lsp->ile.in_label);
          break;
        }
 ...
 */
static bool
compare_notif_nhlfes (const struct nhlfe_list_head *ctx_head,
                      struct nhlfe_list_head *nhlfe_head, int *start_counter,
                      int *end_counter)
{
  struct zebra_nhlfe *nhlfe;
  ...
  frr_each_safe (nhlfe_list, nhlfe_head, nhlfe)
    {
      ...
      if (ctx_nhlfe && ctx_nexthop)
          ...
          if (is_debug)
            {
              ...
              zlog_debug ("LSP dplane notif: matched nh %s (%sinstalled)", buf,
                          tstr);
            }
      ...
      else
        {
          ...
          if (is_debug)
            zlog_debug ("LSP dplane notif: no match, nh %s", buf);
        }
    }
 ...
 */
static int
update_nhlfes_from_ctx (struct nhlfe_list_head *nhlfe_head,
      ...
      if (ctx_nhlfe && ctx_nexthop)
        {
          ...
          if (CHECK_FLAG (ctx_nhlfe->flags, NHLFE_FLAG_INSTALLED))
            {
              if (is_debug)
                zlog_debug ("%s: matched lsp nhlfe %s (installed)", __func__,
                            buf);
          ...
          if (CHECK_FLAG (ctx_nhlfe->nexthop->flags, NEXTHOP_FLAG_FIB))
            {
              SET_FLAG (nhlfe->nexthop->flags, NEXTHOP_FLAG_ACTIVE);
              SET_FLAG (nhlfe->nexthop->flags, NEXTHOP_FLAG_FIB);
            }
          else
            {
              UNSET_FLAG (nhlfe->nexthop->flags, NEXTHOP_FLAG_ACTIVE);
      ...
      else
        {
          /* Not mentioned in lfib set -> uninstalled */
          if (is_debug)
            zlog_debug ("%s: no match for lsp nhlfe %s", __func__, buf);
          UNSET_FLAG (nhlfe->flags, NHLFE_FLAG_INSTALLED);
          UNSET_FLAG (nhlfe->flags, NHLFE_FLAG_SELECTED);
          UNSET_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB);
          UNSET_FLAG (nexthop->flags, NEXTHOP_FLAG_ACTIVE);
 ...
 */
void
zebra_mpls_process_dplane_notify (struct zebra_dplane_ctx *ctx)
{
  struct zebra_vrf *zvrf;
  struct zebra_ile tmp_ile;
  struct hash *lsp_table;
  ...
  changed_p = compare_notif_nhlfes (ctx_list, &lsp->nhlfe_list, &start_count,
                                    &end_count);
  ...
  if (compare_notif_nhlfes (ctx_list, &lsp->backup_nhlfe_list, &start_count,
                            &end_count))
  ...
  if (is_debug)
    zlog_debug (
        "LSP dplane notif: lfib backups, start_count %d, end_count %d%s",
        start_count, end_count, changed_p ? ", changed" : "");
 ...
 */
int
zebra_mpls_lsp_install (struct zebra_vrf *zvrf, struct route_node *rn,
                        struct route_entry *re)
{
  struct route_table *table;
  struct zebra_fec *fec;
 ...
 */
int
zebra_mpls_lsp_uninstall (struct zebra_vrf *zvrf, struct route_node *rn,
                          struct route_entry *re)
{
  struct route_table *table;
  struct zebra_fec *fec;
...
/*
 * Add an NHLFE to an LSP, return the newly-added object. This path only
 * changes the LSP object - nothing is scheduled for processing, for example.
 */
struct zebra_nhlfe *
 ...
 */
struct zebra_nhlfe *
zebra_mpls_lsp_add_backup_nhlfe (struct zebra_lsp *lsp,
                                 enum lsp_types_t lsp_type,
                                 enum nexthop_types_t gtype,
                                 const union g_addr *gate, ifindex_t ifindex,
                                 uint8_t num_labels,
 ...
 */
struct zebra_nhlfe *
zebra_mpls_lsp_add_nh (struct zebra_lsp *lsp, enum lsp_types_t lsp_type,
                       const struct nexthop *nh)
{
  struct zebra_nhlfe *nhlfe;

  nhlfe =
      nhlfe_add (lsp, lsp_type, nh->type, &nh->gate, nh->ifindex, nh->vrf_id,
 ...
 */
struct zebra_nhlfe *
zebra_mpls_lsp_add_backup_nh (struct zebra_lsp *lsp, enum lsp_types_t lsp_type,
                              const struct nexthop *nh)
{
  struct zebra_nhlfe *nhlfe;
 ...
 */
void
zebra_mpls_nhlfe_free (struct zebra_nhlfe *nhlfe)
{
      ...
      if (! fec)
        {
          flog_err (EC_ZEBRA_FEC_ADD_FAILED,
                    "Failed to add FEC %pFX upon register, client %s", p,
  ...
  if (! fec)
    {
      flog_err (EC_ZEBRA_FEC_RM_FAILED,
                "Failed to find FEC %pFX upon unregister, client %s", p,
                zebra_route_string (client->proto));
      return -1;
    }
 ...
 */
static int
zebra_mpls_cleanup_fecs_for_client (struct zserv *client)
{
  struct zebra_vrf *zvrf = zebra_vrf_lookup_by_id (VRF_DEFAULT);
  struct route_node *rn;
  struct zebra_fec *fec;
 ...
 */
static int
zebra_mpls_cleanup_zclient_labels (struct zserv *client)
{
  struct vrf *vrf;
  struct zebra_vrf *zvrf;
...
zebra_mpls_static_fec_del (struct zebra_vrf *zvrf, struct prefix *p)
{
  ...
  if (! fec)
    {
      flog_err (EC_ZEBRA_FEC_RM_FAILED, "Failed to find FEC %pFX upon delete",
                p);
      return -1;
    }
  ...
  if (IS_ZEBRA_DEBUG_MPLS)
    {
      zlog_debug ("Delete fec %pFX label %u label index %u", p, fec->label,
                  fec->label_index);
    }
 ...
 */
void
zebra_mpls_print_fec (struct vty *vty, struct zebra_vrf *zvrf,
                      struct prefix *p)
{
  struct route_table *table;
  struct route_node *rn;
...
static void
mpls_zebra_nhe_update (struct route_entry *re, afi_t afi,
...
static bool
ftn_update_nexthop (bool add_p, struct nexthop *nexthop, enum lsp_types_t type,
                    const struct zapi_nexthop *znh)
{
...
void
zebra_mpls_ftn_uninstall (struct zebra_vrf *zvrf, enum lsp_types_t type,
 ...
 */
static bool
ftn_update_znh (bool add_p, enum lsp_types_t type, struct nexthop *head,
                const struct zapi_nexthop *znh)
{
 ...
 */
void
zebra_mpls_zapi_labels_process (bool add_p, struct zebra_vrf *zvrf,
  ...
  for (i = 0; i < zl->nexthop_num; i++)
    {
      ...
      if (add_p)
        ...
        ret = mpls_lsp_uninstall (zvrf, zl->type, zl->local_label, znh->type,
                                  &znh->gate, znh->ifindex, false);
      if (ret < 0)
        {
          if (IS_ZEBRA_DEBUG_RECV || IS_ZEBRA_DEBUG_MPLS)
            {
              ...
              zlog_debug ("%s: Unable to %sinstall LSP: label %u, znh %s",
                          __func__, (add_p ? "" : "un"), zl->local_label, buf);
            }
          continue;
        }
  ...
  for (i = 0; i < zl->backup_nexthop_num; i++)
    {
          ...
          if (IS_ZEBRA_DEBUG_RECV || IS_ZEBRA_DEBUG_MPLS)
            {
              zapi_nexthop2str (znh, buf, sizeof (buf));
              zlog_debug (
                  "%s: Unable to %sinstall backup LSP: label %u, znh %s",
                  __func__, (add_p ? "" : "un"), zl->local_label, buf);
            }
          continue;
        }
      ...
      found = ftn_update_znh (add_p, zl->type,
                              new_nhe->backup_info->nhe->nhg.nexthop, znh);
      if (found)
        {
          counter++;
        }
      ...
      if (IS_ZEBRA_DEBUG_MPLS)
        {
          ...
          nhlfe2str (nhlfe, buf, sizeof (buf));
          mpls_label2str (num_out_labels, out_labels, buf2, sizeof (buf2), 0,
                          0);
          mpls_label2str (nh->nh_label->num_labels, nh->nh_label->label, buf3,
                          sizeof (buf3), nh->nh_label_type, 0);

          zlog_debug (
              "LSP in-label %u type %d %snexthop %s out-label(s) changed to %s (old %s)",
              lsp->ile.in_label, type, backup_str, buf, buf2, buf3);
        }
      ...
      if (IS_ZEBRA_DEBUG_MPLS)
        {
          ...
          zlog_debug (
              "Add LSP in-label %u type %d %snexthop %s out-label(s) %s",
                  ...
                  const union g_addr *gate, ifindex_t ifindex)
{
  ...
  nhlfe = lsp_add_nhlfe (lsp, type, num_out_labels, out_labels, gtype, gate,
                         ifindex, VRF_DEFAULT, false /*backup*/);
  if (nhlfe == NULL)
    return -1;
 ...
 */
static int
lsp_znh_install (struct zebra_lsp *lsp, enum lsp_types_t type,
                 const struct zapi_nexthop *znh)
{
  struct zebra_nhlfe *nhlfe;

  nhlfe =
      lsp_add_nhlfe (lsp, type, znh->label_num, znh->labels, znh->type,
                     &znh->gate, znh->ifindex, znh->vrf_id, false /*backup*/);
  if (nhlfe == NULL)
    return -1;
  ...
  if (CHECK_FLAG (znh->flags, ZAPI_NEXTHOP_FLAG_HAS_BACKUP))
    {
      ...
      memcpy (nhlfe->nexthop->backup_idx, znh->backup_idx, znh->backup_num);
 ...
 */
static int
lsp_backup_znh_install (struct zebra_lsp *lsp, enum lsp_types_t type,
                        const struct zapi_nexthop *znh)
{
  struct zebra_nhlfe *nhlfe;
  ...
  nhlfe =
      lsp_add_nhlfe (lsp, type, znh->label_num, znh->labels, znh->type,
                     &znh->gate, znh->ifindex, znh->vrf_id, true /*backup*/);
  if (nhlfe == NULL)
    {
      if (IS_ZEBRA_DEBUG_MPLS)
        zlog_debug ("%s: unable to add backup nhlfe, label: %u", __func__,
                    lsp->ile.in_label);
      return -1;
    }
...
struct zebra_lsp *
mpls_lsp_find (struct zebra_vrf *zvrf, mpls_label_t in_label)
{
  struct hash *lsp_table;
  struct zebra_ile tmp_ile;
...
int
mpls_lsp_uninstall_all_vrf (struct zebra_vrf *zvrf, enum lsp_types_t type,
                            mpls_label_t in_label)
{
  struct hash *lsp_table;
  struct zebra_ile tmp_ile;
 ...
 */
static void
mpls_lsp_uninstall_all_type (struct hash_bucket *bucket, void *ctxt)
{
 ...
 */
static void
mpls_ftn_uninstall_all (struct zebra_vrf *zvrf, int afi,
                        enum lsp_types_t lsp_type)
{
  struct route_table *table;
  struct route_node *rn;
  ...
  if (nhlfe)
    {
      ...
      if (nh->nh_label->num_labels == 1 && nh->nh_label->label[0] == out_label)
        /* No change */
        return 0;
      ...
      if (IS_ZEBRA_DEBUG_MPLS)
        {
          ...
          zlog_debug (
              ...
              in_label, buf, out_label, nh->nh_label->label[0]);
        }
      ...
      if (IS_ZEBRA_DEBUG_MPLS)
        {
          char buf[BUFSIZ];
          nhlfe2str (nhlfe, buf, sizeof (buf));
          zlog_debug ("Del static LSP in-label %u nexthop %s", in_label, buf);
        }
 ...
 */
void
zebra_mpls_print_lsp (struct vty *vty, struct zebra_vrf *zvrf,
  ...
  if (use_json)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (lsp_list, node, lsp))
        json_object_object_add (
            json, label2str (lsp->ile.in_label, 0, buf, sizeof (buf)),
            lsp_json (lsp));
          ...
          frr_each_safe (nhlfe_list, &lsp->nhlfe_list, nhlfe)
            {
              ...
              switch (nexthop->type)
                ...
                case NEXTHOP_TYPE_IPV4:
                case NEXTHOP_TYPE_IPV4_IFINDEX:
                  inet_ntop (AF_INET, &nexthop->gate.ipv4, nh_buf,
                             sizeof (nh_buf));
                  break;
                case NEXTHOP_TYPE_IPV6:
                case NEXTHOP_TYPE_IPV6_IFINDEX:
                  inet_ntop (AF_INET6, &nexthop->gate.ipv6, nh_buf,
                             sizeof (nh_buf));
                  break;
                case NEXTHOP_TYPE_BLACKHOLE:
                  break;
              ...
              if (nexthop->type != NEXTHOP_TYPE_IFINDEX && nexthop->nh_label)
                out_label_str =
                    mpls_label2str (nexthop->nh_label->num_labels,
                                    &nexthop->nh_label->label[0], buf,
                                    sizeof (buf), nexthop->nh_label_type, 1);
              else
                out_label_str = "-";

              ttable_add_row (tt, "%u|%s|%s|%s", lsp->ile.in_label,
                              nhlfe_type2str (nhlfe->type), nh_buf,
                              out_label_str);
            }
        }
 ...
 */
static char *
nhlfe_config_str (const struct zebra_nhlfe *nhlfe, char *buf, int size)
{
  const struct nexthop *nh;
  ...
  switch (nh->type)
    ...
    case NEXTHOP_TYPE_IPV4_IFINDEX:
      ...
      if (nh->ifindex)
        strlcat (buf, ifindex2ifname (nh->ifindex, VRF_DEFAULT), size);
      break;
    case NEXTHOP_TYPE_IPV6:
    case NEXTHOP_TYPE_IPV6_IFINDEX:
      ...
      frr_each (nhlfe_list, &lsp->nhlfe_list, nhlfe)
        {
          ...
          switch (nh->nh_label->label[0])
            ...
            case MPLS_LABEL_IMPLICIT_NULL:
              ...
              break;
            default:
              snprintf (lstr, sizeof (lstr), "%u", nh->nh_label->label[0]);
              break;
            }
 ...
 */
int
zebra_mpls_label_block_add (struct zebra_vrf *zvrf, uint32_t start_label,
                            uint32_t end_label)
{
 ...
 */
void
zebra_mpls_cleanup_tables (struct zebra_vrf *zvrf)
{
 ...
 */
void
zebra_mpls_client_cleanup_vrf_label (uint8_t proto)
{
  struct vrf *vrf;
