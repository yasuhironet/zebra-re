...
DEFINE_HOOK (srv6_manager_client_disconnect, (struct zserv * client),
             (client));
...
DEFINE_HOOK (srv6_manager_release_chunk,
             (struct zserv * client, const char *locator_name,
              vrf_id_t vrf_id),
...
void
srv6_manager_client_connect_call (struct zserv *client, vrf_id_t vrf_id)
{
  hook_call (srv6_manager_client_connect, client, vrf_id);
}

void
srv6_manager_get_locator_chunk_call (struct srv6_locator **loc,
                                     struct zserv *client,
                                     const char *locator_name, vrf_id_t vrf_id)
{
...
void
srv6_manager_release_locator_chunk_call (struct zserv *client,
...
int
srv6_manager_client_disconnect_cb (struct zserv *client)
{
...
static int
zebra_srv6_cleanup (struct zserv *client)
{
  return 0;
}

void
zebra_srv6_locator_add (struct srv6_locator *locator)
{
...
void
zebra_srv6_locator_delete (struct srv6_locator *locator)
{
  struct listnode *n;
  ...
  for (ALL_LIST_ELEMENTS_RO ((struct list *) locator->chunks, n, c))
    {
      ...
      if (! client)
        {
          zlog_warn ("%s: Not found zclient(proto=%u, instance=%u).", __func__,
                     c->proto, c->instance);
          continue;
        }
...
void
zebra_notify_srv6_locator_add (struct srv6_locator *locator)
{
  struct listnode *node;
  struct zserv *client;
...
void
zebra_notify_srv6_locator_delete (struct srv6_locator *locator)
{
  struct listnode *n;
  struct srv6_locator_chunk *c;
  struct zserv *client;
  ...
  for (ALL_LIST_ELEMENTS_RO ((struct list *) locator->chunks, n, c))
    {
      ...
      if (! client)
        {
          zlog_warn ("Not found zclient(proto=%u, instance=%u).", c->proto,
                     c->instance);
          continue;
        }
      zsend_zebra_srv6_locator_delete (client, locator);
...
assign_srv6_locator_chunk (uint8_t proto, uint16_t instance,
                           uint32_t session_id, const char *locator_name)
{
  ...
  if (! loc)
    {
      zlog_info ("%s: locator %s was not found", __func__, locator_name);
      return NULL;
    }
                                      ...
                                      vrf_id_t vrf_id)
{
  ...
  *loc = assign_srv6_locator_chunk (client->proto, client->instance,
  ...
  if (! *loc)
    ...
    zlog_info ("Assigned locator chunk %s to %s instance %u", (*loc)->name,
               zebra_route_string (client->proto), client->instance);

  if (*loc && (*loc)->status_up)
    ret = zsend_srv6_manager_get_locator_chunk_response (client, vrf_id, *loc);
  return ret;
}
 ...
 */
static int
release_srv6_locator_chunk (uint8_t proto, uint16_t instance,
                            uint32_t session_id, const char *locator_name)
{
  int ret = -1;
  struct listnode *node;
  ...
  if (IS_ZEBRA_DEBUG_PACKET)
    zlog_debug ("%s: Releasing srv6-locator on %s", __func__, locator_name);
...
static int
zebra_srv6_manager_release_locator_chunk (struct zserv *client,
                                          const char *locator_name,
                                          vrf_id_t vrf_id)
{
 ...
 */
int
release_daemon_srv6_locator_chunks (struct zserv *client)
{
  int ret;
  int count = 0;
  struct zebra_srv6 *srv6 = zebra_srv6_get_default ();
      ...
      for (ALL_LIST_ELEMENTS_RO (loc->chunks, chunk_node, chunk))
        {
          if (chunk->proto == client->proto &&
              ...
              chunk->session_id == client->session_id && chunk->keep == 0)
            {
              ret = release_srv6_locator_chunk (chunk->proto, chunk->instance,
  ...
  if (IS_ZEBRA_DEBUG_PACKET)
    zlog_debug ("%s: Released %d srv6-locator chunks", __func__, count);

  return count;
}

void
zebra_srv6_init (void)
{
