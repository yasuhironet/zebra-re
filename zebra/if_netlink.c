...
static inline void
zebra_if_set_ziftype (struct interface *ifp, enum zebra_iftype zif_type,
...
static void
netlink_determine_zebra_iftype (const char *kind, enum zebra_iftype *zif_type)
{
...
static void
netlink_vrf_change (struct nlmsghdr *h, struct rtattr *tb, uint32_t ns_id,
                    const char *name)
{
  ...
  if (! linkinfo[IFLA_INFO_DATA])
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: IFLA_INFO_DATA missing from VRF message: %s",
                    __func__, name);
      return;
    }

  netlink_parse_rtattr_nested (attr, IFLA_VRF_MAX, linkinfo[IFLA_INFO_DATA]);
  if (! attr[IFLA_VRF_TABLE])
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: IFLA_VRF_TABLE missing from VRF message: %s",
                    __func__, name);
      return;
    }
  ...
  if (h->nlmsg_type == RTM_NEWLINK)
    {
      ...
      if (! vrf)
        {
          flog_err (EC_LIB_INTERFACE, "VRF %s id %u not created", name,
                    ifi->ifi_index);
          return;
        }
      ...
      if (! vrf_enable (vrf))
        {
          flog_err (EC_LIB_INTERFACE, "Failed to enable VRF %s id %u", name,
                    ifi->ifi_index);
          return;
        }
    }
  else // h->nlmsg_type == RTM_DELLINK
    {
      ...
      if (! vrf)
        {
          flog_warn (EC_ZEBRA_VRF_NOT_FOUND, "%s: vrf not found", __func__);
          return;
        }
...
static uint32_t
get_iflink_speed (struct interface *interface, int *error)
{
  struct ifreq ifdata;
  ...
  frr_with_privs (&zserv_privs)
    {
      sd = vrf_socket (PF_INET, SOCK_DGRAM, IPPROTO_IP, interface->vrf->vrf_id,
                       NULL);
      if (sd < 0)
        {
          if (IS_ZEBRA_DEBUG_KERNEL)
  ...
  if (rc < 0)
    {
      if (errno != EOPNOTSUPP && IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("IOCTL failure to read interface %s speed: %d %s", ifname,
                    errno, safe_strerror (errno));
...
uint32_t
kernel_get_speed (struct interface *ifp, int *error)
{
                             ...
                             size_t buflen)
{
  ...
  if (gre_info->ikey &&
    ...
    return 0;
  if (gre_info->okey &&
      ! nl_attr_put32 (&req->n, buflen, IFLA_GRE_IKEY, gre_info->okey))
    return 0;
...
static int
netlink_extract_gre_info (struct rtattr *link_data,
                          struct zebra_l2info_gre *gre_info)
{
  ...
  if (! attr[IFLA_GRE_LOCAL])
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("IFLA_GRE_LOCAL missing from GRE IF message");
    }
  else
    gre_info->vtep_ip = *(struct in_addr *) RTA_DATA (attr[IFLA_GRE_LOCAL]);
  if (! attr[IFLA_GRE_REMOTE])
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("IFLA_GRE_REMOTE missing from GRE IF message");
    }
  else
  ...
  if (! attr[IFLA_GRE_LINK])
    ...
    {
      gre_info->ifindex_link = *(ifindex_t *) RTA_DATA (attr[IFLA_GRE_LINK]);
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("IFLA_GRE_LINK obtained is %u", gre_info->ifindex_link);
    }
                            ...
                            struct zebra_l2info_vxlan *vxl_info)
{
  ...
  if (attr[IFLA_VXLAN_COLLECT_METADATA])
    {
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("IFLA_VXLAN_COLLECT_METADATA=%u in VXLAN IF message", svd);
    }
  ...
  if (! attr[IFLA_VXLAN_LINK])
    ...
    {
      ifindex_link = *(ifindex_t *) RTA_DATA (attr[IFLA_VXLAN_LINK]);
                                 ...
                                 ns_id_t link_nsid)
{
  ...
  if (IS_ZEBRA_IF_BRIDGE (ifp))
  ...
  else if (IS_ZEBRA_IF_VLAN (ifp))
    {
      ...
      zebra_evpn_acc_bd_svi_set (ifp->info, NULL, ! ! if_is_operative (ifp));
    }
  else if (IS_ZEBRA_IF_VXLAN (ifp))
    {
      ...
      if (link_nsid != NS_UNKNOWN && vxlan_info.ifindex_link)
        zebra_if_update_link (ifp, vxlan_info.ifindex_link, link_nsid);
    }
  else if (IS_ZEBRA_IF_GRE (ifp))
    {
      ...
      if (link_nsid != NS_UNKNOWN && gre_info.ifindex_link)
        zebra_if_update_link (ifp, gre_info.ifindex_link, link_nsid);
    }
}

static int
netlink_bridge_vxlan_vlan_vni_map_update (struct interface *ifp,
                                          struct rtattr *af_spec)
{
  ...
  for (i = RTA_DATA (af_spec), rem = RTA_PAYLOAD (af_spec); RTA_OK (i, rem);
       i = RTA_NEXT (i, rem))
    {
      ...
      netlink_parse_rtattr_nested (aftb, IFLA_BRIDGE_VLAN_TUNNEL_MAX, i);
      ...
      vni.access_vlan =
          *(vlanid_t *) RTA_DATA (aftb[IFLA_BRIDGE_VLAN_TUNNEL_VID]);

      if (aftb[IFLA_BRIDGE_VLAN_TUNNEL_FLAGS])
        flags = *(uint16_t *) RTA_DATA (aftb[IFLA_BRIDGE_VLAN_TUNNEL_FLAGS]);
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("Vlan-Vni(%d:%d-%d:%d) update for VxLAN IF %s(%u)",
                    vni_start.access_vlan, vni_end.access_vlan, vni_start.vni,
                    vni_end.vni, ifp->name, ifp->ifindex);
...
static int
netlink_bridge_vxlan_update (struct interface *ifp, struct rtattr *af_spec)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("Access VLAN %u for VxLAN IF %s(%u)", access_vlan, ifp->name,
                ifp->ifindex);
...
static void
netlink_bridge_vlan_update (struct interface *ifp, struct rtattr *af_spec)
{
  struct rtattr *i;
  int rem;
...
static int
netlink_bridge_interface (struct nlmsghdr *h, int len, ns_id_t ns_id,
                          int startup)
{
  char *name = NULL;
  struct ifinfomsg *ifi;
  ...
  if (! ifp)
    {
      zlog_debug ("Cannot find bridge IF %s(%u)", name, ifi->ifi_index);
      return 0;
    }
...
static bool
is_if_protodown_reason_only_frr (uint32_t rc_bitfield)
{
 ...
 */
static void
netlink_proc_dplane_if_protodown (struct zebra_if *zif, struct rtattr **tb)
{
  bool protodown;
  bool old_protodown;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("interface %s dplane change, protdown %s", zif->ifp->name,
                protodown ? "on" : "off");
  ...
  if (zebra_evpn_is_es_bond_member (zif->ifp))
    {
      ...
      if (! protodown && CHECK_FLAG (zif->flags, ZIF_FLAG_UNSET_PROTODOWN))
        {
          if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug (
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("bond mbr %s reinstate protodown %s in the dplane",
...
static uint8_t
netlink_parse_lacp_bypass (struct rtattr **linkinfo)
{
  uint8_t bypass = 0;
  ...
  if (mbrinfo[IFLA_BOND_SLAVE_AD_RX_BYPASS])
    bypass = *(uint8_t *) RTA_DATA (mbrinfo[IFLA_BOND_SLAVE_AD_RX_BYPASS]);
 ...
 */
static void
if_sweep_protodown (struct zebra_if *zif)
{
  bool protodown;
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("interface %s sweeping protodown %s reason 0x%x",
                zif->ifp->name, protodown ? "on" : "off", zif->protodown_rc);
 ...
 */
static int
netlink_interface (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len;
  struct ifinfomsg *ifi;
  struct rtattr *tb[IFLA_MAX + 1];
  ...
  netlink_parse_rtattr_flags (tb, IFLA_MAX, IFLA_RTA (ifi), len, NLA_F_NESTED);
  ...
  if (tb[IFLA_LINKINFO])
    {
      netlink_parse_rtattr_nested (linkinfo, IFLA_INFO_MAX, tb[IFLA_LINKINFO]);
...
static int
netlink_request_intf_addr (struct nlsock *netlink_cmd, int family, int type,
                           uint32_t filter_mask)
{
  ...
  frrtrace (4, frr_zebra, netlink_request_intf_addr, netlink_cmd, family, type,
            filter_mask);
...
interface_lookup_netlink (struct zebra_ns *zns)
{
  ...
  if (ret < 0)
    return ret;
  ret = netlink_parse_info (netlink_interface, netlink_cmd, &dp_info, 0, true);
  if (ret < 0)
    return ret;
 ...
 */
static int
interface_addr_lookup_netlink (struct zebra_ns *zns)
{
  int ret;
  struct zebra_dplane_info dp_info;
  struct nlsock *netlink_cmd = &zns->netlink_cmd;
  ...
  if (ret < 0)
    return ret;
  ret = netlink_parse_info (netlink_interface_addr, netlink_cmd, &dp_info, 0,
                            true);
  if (ret < 0)
    return ret;
...
int
kernel_interface_set_master (struct interface *master, struct interface *slave)
{
...
static ssize_t
netlink_address_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                             size_t buflen)
{
  int bytelen;
      ...
      if (dplane_ctx_intf_is_connected (ctx))
        {
          ...
          if (! nl_attr_put (&req->n, buflen, IFA_ADDRESS, &p->u.prefix,
                             bytelen))
            return 0;
        }
      else if (cmd == RTM_NEWADDR)
        {
          struct in_addr broad = { .s_addr = ipv4_broadcast_addr (
                                       p->u.prefix4.s_addr, p->prefixlen) };
          if (! nl_attr_put (&req->n, buflen, IFA_BROADCAST, &broad, bytelen))
            return 0;
        }
    }
...
{
  return netlink_batch_add_msg (bth, ctx, netlink_address_msg_encoder, false);
}

static ssize_t
netlink_intf_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                          size_t buflen)
{
  enum dplane_op_e op;
  int cmd = 0;
...
enum netlink_msg_status
netlink_put_intf_update_msg (struct nl_batch *bth,
                             struct zebra_dplane_ctx *ctx)
{
...
int
netlink_interface_addr (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len;
  struct ifaddrmsg *ifa;
      ...
      if (startup)
        {
           ...
           */
          flog_warn (EC_LIB_INTERFACE, "%s: can't find interface by index %d",
                     __func__, ifa->ifa_index);
          return 0;
        }
      else
        {
          flog_err (EC_LIB_INTERFACE, "%s: can't find interface by index %d",
                    __func__, ifa->ifa_index);
          return -1;
        }
    }
  ...
  if (IS_ZEBRA_DEBUG_KERNEL) /* remove this line to see initial ifcfg */
    {
      ...
      zlog_debug ("%s %s %s flags 0x%x:", __func__,
                  nl_msg_type_to_str (h->nlmsg_type), ifp->name, kernel_flags);
      if (tb[IFA_LOCAL])
        zlog_debug (
            "  IFA_LOCAL     %s/%d",
            inet_ntop (ifa->ifa_family, RTA_DATA (tb[IFA_LOCAL]), buf, BUFSIZ),
            ifa->ifa_prefixlen);
      if (tb[IFA_ADDRESS])
      ...
      if (tb[IFA_CACHEINFO])
        {
          ...
          zlog_debug ("  IFA_CACHEINFO pref %d, valid %d", ci->ifa_prefered,
                      ci->ifa_valid);
        }
    }
  ...
  if (addr == NULL)
    {
      zlog_debug ("%s: Local Interface Address is NULL for %s", __func__,
                  ifp->name);
      return -1;
    }
      ...
      if (ifa->ifa_prefixlen > IPV4_MAX_BITLEN)
        {
          zlog_err (
              ...
              ifa->ifa_prefixlen, nl_msg_type_to_str (h->nlmsg_type));
          return -1;
        }
      ...
      if (h->nlmsg_type == RTM_NEWADDR)
        {
          ...
          if (! (kernel_flags & (IFA_F_DADFAILED | IFA_F_TENTATIVE)))
            connected_add_ipv6 (ifp, flags, (struct in6_addr *) addr,
                                (struct in6_addr *) broad, ifa->ifa_prefixlen,
                                label, metric);
        }
      else
        connected_delete_ipv6 (ifp, (struct in6_addr *) addr, NULL,
                               ifa->ifa_prefixlen);
    }
 ...
 */
int
netlink_interface_addr_dplane (struct nlmsghdr *h, ns_id_t ns_id,
                               int startup /*ignored*/)
{
  int len;
  struct ifaddrmsg *ifa;
  ...
  if (ifa->ifa_family != AF_INET && ifa->ifa_family != AF_INET6)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: %s: Invalid address family: %u", __func__,
                    nl_msg_type_to_str (h->nlmsg_type), ifa->ifa_family);
      return 0;
    }
  ...
  if (len < 0)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: %s: netlink msg bad size: %d %zu", __func__,
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    { /* remove this line to see initial ifcfg */
      ...
      zlog_debug ("%s: %s nsid %u ifindex %u flags 0x%x:", __func__,
                  nl_msg_type_to_str (h->nlmsg_type), ns_id, ifa->ifa_index,
                  kernel_flags);
      if (tb[IFA_LOCAL])
  ...
  if (ifa->ifa_family == AF_INET && ifa->ifa_prefixlen > IPV4_MAX_BITLEN)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: %s: Invalid prefix length: %u", __func__,
  ...
  if (ifa->ifa_family == AF_INET6)
    {
      ...
      if (h->nlmsg_type == RTM_NEWADDR &&
        ...
        {
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("%s: %s: Invalid/tentative addr", __func__,
                        nl_msg_type_to_str (h->nlmsg_type));
          return 0;
        }
    }
  ...
  if (addr == NULL)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: %s: No local interface address", __func__,
                    nl_msg_type_to_str (h->nlmsg_type));
      return -1;
    }
...
int
netlink_link_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len;
  struct ifinfomsg *ifi;
  struct rtattr *tb[IFLA_MAX + 1];
  ...
  if (h->nlmsg_type == RTM_NEWLINK)
      ...
      if (ifp == NULL || ! CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_ACTIVE))
        {
          ...
          if (! tb[IFLA_MTU])
            {
              zlog_debug ("RTM_NEWLINK for interface %s(%u) without MTU set",
                          name, ifi->ifi_index);
              return 0;
            }
          ...
          if (IS_ZEBRA_IF_BRIDGE (ifp))
            {
              zif = ifp->info;
              if (IS_ZEBRA_DEBUG_KERNEL)
                zlog_debug ("RTM_NEWLINK ADD for %s(%u), vlan-aware %d", name,
                            ifp->ifindex, IS_ZEBRA_IF_BRIDGE_VLAN_AWARE (zif));
            }
        }
      else if (ifp->vrf->vrf_id != vrf_id)
        {
          ...
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug (
                ...
                name, ifp->ifindex, ifp->vrf->vrf_id, vrf_id, ifi->ifi_flags);
      ...
      else
        {
          ...
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug (
                ...
                name, ifp->ifindex, zif_slave_type, master_infindex,
                ifi->ifi_flags);
          ...
          if (if_is_no_ptm_operative (ifp))
            {
              ...
              ifp->flags = ifi->ifi_flags & 0x0000fffff;
              if (! if_is_no_ptm_operative (ifp) ||
                  CHECK_FLAG (zif->flags, ZIF_FLAG_PROTODOWN))
                {
                  if (IS_ZEBRA_DEBUG_KERNEL)
                    zlog_debug ("Intf %s(%u) has gone DOWN", name,
                                ifp->ifindex);
                  if_down (ifp);
                  rib_update (RIB_UPDATE_KERNEL);
                }
              else if (if_is_operative (ifp))
                {
                   ...
                   * interface status. */
                  if (IS_ZEBRA_DEBUG_KERNEL)
                    zlog_debug ("Intf %s(%u) PTM up, notifying clients", name,
                                ifp->ifindex);
                  if_up (ifp, ! is_up);
                  ...
                  if (IS_ZEBRA_IF_VLAN (ifp) && mac_updated)
                    {
                      ...
                      link_if = if_lookup_by_index_per_ns (
                          zebra_ns_lookup (NS_DEFAULT), link_ifindex);
                      if (link_if)
                        zebra_vxlan_svi_up (ifp, link_if);
                    }
                  else if (mac_updated && IS_ZEBRA_IF_BRIDGE (ifp))
                    {
                      zlog_debug ("Intf %s(%u) bridge changed MAC address",
                                  name, ifp->ifindex);
                      chgflags = ZEBRA_BRIDGE_MASTER_MAC_CHANGE;
                    }
                }
            }
          else
            {
              ...
              if (if_is_operative (ifp) &&
                  ! CHECK_FLAG (zif->flags, ZIF_FLAG_PROTODOWN))
                {
                  ...
                  if_up (ifp, true);
                  if (IS_ZEBRA_IF_BRIDGE (ifp))
                    chgflags = ZEBRA_BRIDGE_MASTER_UP;
                }
              else
                {
                  if (IS_ZEBRA_DEBUG_KERNEL)
          ...
          if (IS_ZEBRA_IF_BRIDGE (ifp))
            {
              if (IS_ZEBRA_DEBUG_KERNEL)
                zlog_debug ("RTM_NEWLINK update for %s(%u), vlan-aware %d",
                            name, ifp->ifindex,
  ...
  else
    {
      ...
      if (ifp == NULL)
        {
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("RTM_DELLINK for unknown interface %s(%u)", name,
                        ifi->ifi_index);
          return 0;
        }
...
ssize_t
netlink_intf_msg_encode (uint16_t cmd, const struct zebra_dplane_ctx *ctx,
                         void *buf, size_t buflen)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s: %s, protodown=%d reason_val=%d ifindex=%u", __func__,
                nl_msg_type_to_str (cmd), down, pd_reason_val, ifindex);
...
void
if_netlink_set_frr_protodown_r_bit (uint8_t bit)
{
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug (
...
void
if_netlink_unset_frr_protodown_r_bit (void)
{
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug (
        "Protodown reason bit index changed: bit-index %u -> bit-index %u",
...
bool
if_netlink_frr_protodown_r_bit_is_set (void)
{
  return (frr_protodown_r_bit != FRR_PROTODOWN_REASON_DEFAULT_BIT);
}

uint8_t
if_netlink_get_frr_protodown_r_bit (void)
{
 ...
 */
static int
netlink_request_tunneldump (struct zebra_ns *zns, int family, int ifindex)
{
  struct
  {
    struct nlmsghdr n;
 ...
 */
int
netlink_tunneldump_read (struct zebra_ns *zns)
{
  int ret = 0;
  struct zebra_dplane_info dp_info;
  ...
  for (rn = route_top (zns->if_table); rn; rn = route_next (rn))
    {
      ...
      ret = netlink_request_tunneldump (zns, PF_BRIDGE, tmp_if->ifindex);
      if (ret < 0)
        return ret;
...
static void
vxlan_vni_state_change (struct zebra_if *zif, uint16_t id, uint8_t state)
{
  struct zebra_vxlan_vni *vnip;
...
static void
vlan_id_range_state_change (struct interface *ifp, uint16_t id_start,
 ...
 */
int
netlink_vlan_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  int len, rem;
  struct interface *ifp;
  ...
  if (! IS_ZEBRA_IF_VXLAN (ifp))
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("Ignoring non-vxlan IF (%s) for vlan update", ifp->name);

      return 0;
    }

  if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("%s %s IF %s NS %u", nl_msg_type_to_str (h->nlmsg_type),
  ...
  for (attr = BRVLAN_RTA (bvm); RTA_OK (attr, rem);
       attr = RTA_NEXT (attr, rem))
    {
      ...
      vinfo =
          (struct bridge_vlan_info *) RTA_DATA (vtb[BRIDGE_VLANDB_ENTRY_INFO]);
      ...
      if (vtb[BRIDGE_VLANDB_ENTRY_RANGE])
        vrange = *(uint32_t *) RTA_DATA (vtb[BRIDGE_VLANDB_ENTRY_RANGE]);

      if (IS_ZEBRA_DEBUG_KERNEL || IS_ZEBRA_DEBUG_VXLAN)
        {
          if (vrange)
            zlog_debug ("VLANDB_ENTRY: VID (%u-%u) state=%s", vinfo->vid,
                        vrange, port_state2str (state));
          else
            zlog_debug ("VLANDB_ENTRY: VID (%u) state=%s", vinfo->vid,
                        port_state2str (state));
        }

      vlan_id_range_state_change (ifp, vinfo->vid,
                                  (vrange ? vrange : vinfo->vid), state);
    }
 ...
 */
static int
netlink_request_vlan (struct zebra_ns *zns, int family, int type)
{
  struct
  {
    struct nlmsghdr n;
 ...
 */
int
netlink_vlan_read (struct zebra_ns *zns)
{
  int ret;
  struct zebra_dplane_info dp_info;
  ...
  ret = netlink_parse_info (netlink_vlan_change, &zns->netlink_cmd, &dp_info,
                            0, 1);
