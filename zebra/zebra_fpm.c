 ...
 */
static inline int
zfpm_thread_should_yield (struct event *t)
{
 ...
 */
static const char *
zfpm_state_to_str (enum zfpm_state state)
{
  switch (state)
    {
 ...
 */
static inline void
zfpm_rnodes_iter_init (struct zfpm_rnodes_iter *iter)
{
 ...
 */
static inline void
zfpm_rnodes_iter_pause (struct zfpm_rnodes_iter *iter)
{
 ...
 */
static inline void
zfpm_rnodes_iter_cleanup (struct zfpm_rnodes_iter *iter)
{
  route_table_iter_cleanup (&iter->iter);
 ...
 */
static inline void
zfpm_stats_init (struct zfpm_stats *stats)
{
 ...
 */
static inline void
zfpm_stats_reset (struct zfpm_stats *stats)
{
 ...
 */
static inline void
zfpm_stats_copy (const struct zfpm_stats *src, struct zfpm_stats *dest)
{
 ...
 */
static void
zfpm_stats_compose (const struct zfpm_stats *s1, const struct zfpm_stats *s2,
...
static inline void
zfpm_connect_off (void)
{
  EVENT_OFF (zfpm_g->t_connect);
}

static inline void
zfpm_conn_down_off (void)
{
 ...
 */
static void
zfpm_conn_up_thread_cb (struct event *thread)
{
...
zfpm_connect_check (void)
{
  ...
  slen = sizeof (status);
  ret =
      getsockopt (zfpm_g->sock, SOL_SOCKET, SO_ERROR, (void *) &status, &slen);
 ...
 */
static void
zfpm_conn_down_thread_cb (struct event *thread)
{
  struct route_node *rnode;
  struct zfpm_rnodes_iter *iter;
  rib_dest_t *dest;
 ...
 */
static void
zfpm_read_cb (struct event *thread)
{
  size_t already;
  struct stream *ibuf;
  uint16_t msg_len;
 ...
 */
static inline int
zfpm_encode_mac (struct fpm_mac_info_t *mac, char *in_buf, size_t in_buf_len,
                 fpm_msg_type_e *msg_type)
{
  size_t len = 0;
  ...
  do
    {
      ...
      data = fpm_msg_data (hdr);
      data_len =
          zfpm_encode_mac (mac, (char *) data, buf_end - data, &msg_type);
      assert (data_len);
 ...
 */
static void
zfpm_write_cb (struct event *thread)
{
  struct stream *s;
  int num_writes;
 ...
 */
static void
zfpm_connect_cb (struct event *t)
{
  int sock, ret;
  ...
  if (sock < 0)
    {
      zlog_err ("Failed to create socket for connect(): %s", strerror (errno));
 ...
 */
static void
zfpm_set_state (enum zfpm_state state, const char *reason)
{
...
static int
zfpm_trigger_remove (struct route_node *rn)
{
  rib_dest_t *dest;
 ...
 */
static unsigned int
zfpm_mac_info_hash_keymake (const void *p)
{
 ...
 */
static bool
zfpm_mac_info_cmp (const void *p1, const void *p2)
{
  const struct fpm_mac_info_t *fpm_mac1 = p1;
  const struct fpm_mac_info_t *fpm_mac2 = p2;

  if (memcmp (fpm_mac1->macaddr.octet, fpm_mac2->macaddr.octet, ETH_ALEN) != 0)
    return false;
 ...
 */
static struct fpm_mac_info_t *
zfpm_mac_info_lookup (struct fpm_mac_info_t *key)
{
 ...
 */
static void
zfpm_mac_info_del (struct fpm_mac_info_t *fpm_mac)
{
 ...
 */
static int
zfpm_trigger_rmac_update (struct zebra_mac *rmac, struct zebra_l3vni *zl3vni,
                          bool delete, const char *reason)
{
  ...
  if (reason)
    {
      zfpm_debug ("triggering update to FPM - Reason: %s - %pEA", reason,
                  &rmac->macaddr);
    }
 ...
 */
static void
zfpm_trigger_rmac_update_wrapper (struct hash_bucket *bucket, void *args)
{
 ...
 */
static void
zfpm_iterate_rmac_table (struct hash_bucket *bucket, void *args)
{
 ...
 */
static void
zfpm_stats_timer_cb (struct event *t)
{
  zfpm_g->t_stats = NULL;
...
zfpm_start_stats_timer (void)
{
  ...
  event_add_timer (zfpm_g->master, zfpm_stats_timer_cb, 0, ZFPM_STATS_IVL_SECS,
                   &zfpm_g->t_stats);
}
...
zfpm_init_message_format (const char *format)
{
  ...
  flog_warn (EC_ZEBRA_FPM_FORMAT_UNKNOWN, "Unknown fpm format '%s'", format);
}
...
static int
fpm_remote_srv_write (struct vty *vty)
{
  struct in_addr in;
 ...
 */
static int
zfpm_init (struct event_loop *master)
{
  int enable = 1;
  uint16_t port = 0;
  ...
  zfpm_g->fpm_mac_info_table =
      hash_create (zfpm_mac_info_hash_keymake, zfpm_mac_info_cmp,
