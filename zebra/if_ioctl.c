  ...
  for (n = 0; n < ifconf.ifc_len;)
    {
      ...
      ifp = if_get_by_name (ifreq->ifr_name, VRF_DEFAULT, VRF_DEFAULT_NAME);
      if_add_update (ifp);
      ...
      if (ifap->ifa_addr->sa_family == AF_INET)
        {
          ...
          if (if_is_pointopoint (ifp) && ifap->ifa_dstaddr &&
              ! IPV4_ADDR_SAME (
                  &addr->sin_addr,
                  &((struct sockaddr_in *) ifap->ifa_dstaddr)->sin_addr))
            {
          ...
          connected_add_ipv4 (ifp, flags, &addr->sin_addr, prefixlen, dest_pnt,
                              NULL, METRIC_MAX);
        }
      if (ifap->ifa_addr->sa_family == AF_INET6)
        {
          ...
          if (IN6_IS_ADDR_LINKLOCAL (&addr->sin6_addr))
            {
              addr->sin6_scope_id =
                  ntohs (*(uint16_t *) &addr->sin6_addr.s6_addr[2]);
              addr->sin6_addr.s6_addr[2] = addr->sin6_addr.s6_addr[3] = 0;
            }
#endif

          connected_add_ipv6 (ifp, flags, &addr->sin6_addr, NULL, prefixlen,
                              NULL, METRIC_MAX);
        }
    }
