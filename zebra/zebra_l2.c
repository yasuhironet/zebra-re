...
/* Private functions */
static void
map_slaves_to_bridge (struct interface *br_if, int link, bool update_slave,
                      uint8_t chgflags)
{
  struct vrf *vrf;
                  ...
                  if (update_slave)
                    {
                      zebra_l2if_update_bridge_slave (
                          ifp, br_slave->bridge_ifindex, br_slave->ns_id,
                          chgflags);
                    }
                }
            }
 ...
 */
static void
zebra_l2_bond_lacp_bypass_eval (struct zebra_if *bond_zif)
{
  ...
  if (bond_zif->bond_info.mbr_zifs)
    {
      for (ALL_LIST_ELEMENTS_RO (bond_zif->bond_info.mbr_zifs, node, bond_mbr))
        {
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
    zlog_debug ("bond %s lacp bypass changed to %s", bond_zif->ifp->name,
                new_bypass ? "on" : "off");
...
void
zebra_l2_map_slave_to_bond (struct zebra_if *zif, vrf_id_t vrf_id)
{
  ...
  if (bond_if && (bond_zif = bond_if->info))
    {
      if (bond_zif->bond_info.mbr_zifs)
        {
          if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
            zlog_debug ("bond mbr %s linked to %s", zif->ifp->name,
                        bond_if->name);
...
void
zebra_l2_unmap_slave_from_bond (struct zebra_if *zif)
{
  struct zebra_l2info_bondslave *bond_slave = &zif->bondslave_info;
  struct zebra_if *bond_zif;
  ...
  if (zebra_evpn_is_es_bond (bond_zif->ifp))
    zebra_evpn_mh_update_protodown_bond_mbr (zif, true /*clear*/, __func__);
...
void
zebra_l2if_update_bond (struct interface *ifp, bool add)
{
  struct zebra_if *zif;
...
void
zebra_l2if_update_bridge (struct interface *ifp, uint8_t chgflags)
{
  if (! chgflags)
    return;
 ...
 */
void
zebra_l2_greif_add_update (struct interface *ifp,
                                ...
                                uint8_t chgflags)
{
  ...
  if (zif->zif_type == ZEBRA_IF_VXLAN && chgflags != ZEBRA_BRIDGE_NO_ACTION)
    {
...
void
zebra_l2if_update_bond_slave (struct interface *ifp, ifindex_t bond_ifindex,
                              bool new_bypass)
{
  struct zebra_if *zif;
  ...
  if (old_bypass != new_bypass)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_ES || IS_ZEBRA_DEBUG_EVENT)
        zlog_debug ("bond-mbr %s lacp bypass changed to %s", zif->ifp->name,
                    new_bypass ? "on" : "off");
...
void
zebra_vlan_bitmap_compute (struct interface *ifp, uint32_t vid_start,
                           uint16_t vid_end)
{
  uint32_t vid;
  struct zebra_if *zif;
...
void
zebra_vlan_mbr_re_eval (struct interface *ifp, bitfield_t old_vlan_bitmap)
{
  uint32_t vid;
  struct zebra_if *zif;
