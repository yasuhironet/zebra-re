...
void
zebra_sr_policy_del (struct zebra_sr_policy *policy)
{
...
struct zebra_sr_policy *
zebra_sr_policy_find (uint32_t color, struct ipaddr *endpoint)
{
  struct zebra_sr_policy policy = {};

  policy.color = color;
  policy.endpoint = *endpoint;
  return RB_FIND (zebra_sr_policy_instance_head, &zebra_sr_policy_instances,
                  &policy);
}
...
static int
zebra_sr_policy_notify_update_client (struct zebra_sr_policy *policy,
                                      struct zserv *client)
{
  ...
  switch (policy->endpoint.ipa_type)
    ...
    case IPADDR_NONE:
      flog_warn (EC_LIB_DEVELOPMENT,
                 "%s: unknown policy endpoint address family: %u", __func__,
                 policy->endpoint.ipa_type);
      exit (1);
    }
...
static void
zebra_sr_policy_notify_update (struct zebra_sr_policy *policy)
{
  struct rnh *rnh;
  struct prefix p = {};
  ...
  for (ALL_LIST_ELEMENTS_RO (rnh->client_list, node, client))
    {
      if (policy->status == ZEBRA_SR_POLICY_UP)
        ...
        zebra_send_rnh_update (rnh, client, zvrf_id (zvrf), policy->color);
    }
}

static void
zebra_sr_policy_activate (struct zebra_sr_policy *policy,
...
static void
zebra_sr_policy_update (struct zebra_sr_policy *policy, struct zebra_lsp *lsp,
                        struct zapi_srte_tunnel *old_tunnel)
{
  ...
  bsid_changed = policy->segment_list.local_label != old_tunnel->local_label;
...
static void
zebra_sr_policy_deactivate (struct zebra_sr_policy *policy)
{
  policy->status = ZEBRA_SR_POLICY_DOWN;
  policy->lsp = NULL;
...
int
zebra_sr_policy_validate (struct zebra_sr_policy *policy,
...
int
zebra_sr_policy_bsid_install (struct zebra_sr_policy *policy)
{
...
void
zebra_sr_policy_bsid_uninstall (struct zebra_sr_policy *policy,
...
int
zebra_sr_policy_label_update (mpls_label_t label,
...
static int
zebra_srte_client_close_cleanup (struct zserv *client)
{
