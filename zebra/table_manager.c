 ...
 */
void
table_manager_enable (struct zebra_vrf *zvrf)
{
...
assign_table_chunk (uint8_t proto, uint16_t instance, uint32_t size,
 ...
 */
int
release_table_chunk (uint8_t proto, uint16_t instance, uint32_t start,
                     uint32_t end, struct zebra_vrf *zvrf)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (tbl_mgr->lc_list, node, tmc))
    {
      ...
      if (tmc->proto != proto || tmc->instance != instance)
        {
          flog_err (EC_ZEBRA_TM_DAEMON_MISMATCH, "%s: Daemon mismatch!!",
                    __func__);
          continue;
        }
      tmc->proto = NO_PROTO;
  ...
  if (ret != 0)
    flog_err (EC_ZEBRA_TM_UNRELEASED_CHUNK, "%s: Table chunk not released!!",
              __func__);
 ...
 */
int
release_daemon_table_chunks (struct zserv *client)
{
      ...
      for (ALL_LIST_ELEMENTS_RO (zvrf->tbl_mgr->lc_list, node, tmc))
        {
          if (tmc->proto == proto && tmc->instance == instance)
            {
              ret = release_table_chunk (tmc->proto, tmc->instance, tmc->start,
                                         tmc->end, zvrf);
              if (ret == 0)
                count++;
            }
...
static void
table_range_add (struct zebra_vrf *zvrf, uint32_t start, uint32_t end)
{
  if (! zvrf->tbl_mgr)
    return;
...
void
table_manager_disable (struct zebra_vrf *zvrf)
{
  if (! zvrf->tbl_mgr)
    return;
  if (! vrf_is_backend_netns () && strcmp (zvrf_name (zvrf), VRF_DEFAULT_NAME))
...
int
table_manager_range (struct vty *vty, bool add, struct zebra_vrf *zvrf,
