...
void
macfdb_read_for_bridge (struct zebra_ns *zns, struct interface *ifp,
...
void
macfdb_read_mcast_entry_for_vni (struct zebra_ns *zns, struct interface *ifp,
                                 vni_t vni)
{
...
void
macfdb_read_specific_mac (struct zebra_ns *zns, struct interface *br_if,
...
void
neigh_read_for_vlan (struct zebra_ns *zns, struct interface *vlan_if)
{
  netlink_neigh_read_for_vlan (zns, vlan_if);
}

void
neigh_read_specific_ip (const struct ipaddr *ip, struct interface *vlan_if)
{
  netlink_neigh_read_specific_ip (ip, vlan_if);
}

void
kernel_read_pbr_rules (struct zebra_ns *zns)
{
  netlink_rules_read (zns);
}

void
kernel_read_tc_qdisc (struct zebra_ns *zns)
{
  netlink_qdisc_read (zns);
}

void
vlan_read (struct zebra_ns *zns)
{
