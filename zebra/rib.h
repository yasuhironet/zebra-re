...
#define ZEBRA_KERNEL_TABLE_MAX                                                \
  252 /* support for no more than this rt tables */
...
#define RNODE_FIRST_RE(rn, re)                                                \
  RE_DEST_FIRST_ROUTE (rib_dest_from_rnode (rn), re)
...
#define route_entry_dump(prefix, src, re)                                     \
  ...
  zebra_rib_route_entry_new (vrf_id_t vrf_id, int type, uint8_t instance,
                             uint32_t flags, uint32_t nhe_id,
...
#define ZEBRA_RIB_NOTFOUND        3
  ...
  extern void rib_install_kernel (struct route_node *rn,
                                  struct route_entry *re,
                                  struct route_entry *old);
  extern void rib_uninstall_kernel (struct route_node *rn,
                                    struct route_entry *re);
  ...
  extern struct route_entry *rib_match (afi_t afi, safi_t safi,
                                        vrf_id_t vrf_id,
  ...
  extern unsigned long rib_score_proto (uint8_t proto,
  ...
  int zebra_rib_queue_evpn_route_add (vrf_id_t vrf_id,
  ...
  int zebra_rib_queue_evpn_rem_macip_del (vni_t vni,
                                          const struct ethaddr *macaddr,
                                          const struct ipaddr *ip,
                                          struct in_addr vtep_ip);
  int zebra_rib_queue_evpn_rem_macip_add (
      vni_t vni, const struct ethaddr *macaddr, const struct ipaddr *ipaddr,
      uint8_t flags, uint32_t seq, struct in_addr vtep_ip, const esi_t *esi);
  ...
  extern void zebra_rib_evaluate_rn_nexthops (struct route_node *rn,
                                              uint32_t seq, bool rt_delete);
   ...
   * rib_table_info
   */
  static inline struct rib_table_info *
   ...
   * rib nexthops.
   */
  static inline struct nexthop_group *
  rib_get_fib_nhg (struct route_entry *re)
  {
   ...
   */
  static inline struct nexthop_group *
  rib_get_fib_backup_nhg (struct route_entry *re)
  {
    return &(re->fib_backup_ng);
  }

  extern void zebra_gr_process_client (afi_t afi, vrf_id_t vrf_id,
