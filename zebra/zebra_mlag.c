...
DEFINE_HOOK (zebra_mlag_private_write_data, (uint8_t * data, uint32_t len),
             (data, len));
...
zebra_mlag_send_register (void)
{
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug ("%s: Enqueued MLAG Register to MLAG Thread ", __func__);
}
 ...
 */
void
zebra_mlag_send_deregister (void)
{
  struct stream *s = NULL;
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug ("%s: Enqueued MLAG De-Register to MLAG Thread ", __func__);
}
 ...
 */
void
zebra_mlag_process_mlag_data (uint8_t *data, uint32_t len)
{
  struct stream *s = NULL;
  int msg_type = 0;
  ...
  if (msg_type <= 0)
    {
      ...
      stream_free (s);
      zlog_err ("%s: failed to process mlag data-%d, %u", __func__, msg_type,
                len);
      return;
    }
  ...
  event_add_event (zrouter.master, zebra_mlag_post_data_from_main_thread, s, 0,
                   NULL);
}
 ...
 */
static void
zebra_mlag_client_msg_handler (struct event *event)
{
  struct stream *s;
  uint32_t wr_count = 0;
  uint32_t msg_type = 0;
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug (":%s: Processing MLAG write, %u messages in queue", __func__,
                wr_count);
  ...
  for (wr_count = 0; wr_count < max_count; wr_count++)
    {
      ...
      if (! s)
        {
          zlog_debug (":%s: Got a NULL Messages, some thing wrong", __func__);
          break;
        }
      ...
      if (len > 0)
        {
          hook_call (zebra_mlag_private_write_data, mlag_wr_buffer, len);
          ...
          if (msg_type == MLAG_DEREGISTER)
            {
              event_add_event (zrouter.master, zebra_mlag_terminate_pthread,
                               NULL, 0, NULL);
            }
        }
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug (":%s: Posted  %d messages to MLAGD", __func__, wr_count);
  /*
 ...
 */
void
zebra_mlag_handle_process_state (enum zebra_mlag_state state)
{
  if (state == MLAG_UP)
    {
 ...
 */
static int
zebra_mlag_signal_write_thread (void)
{
  if (IS_ZEBRA_DEBUG_MLAG)
   ...
   */
  event_add_event (zrouter.mlag_info.th_master, zebra_mlag_client_msg_handler,
                   NULL, 0, &zrouter.mlag_info.t_write);
  return 0;
}
 ...
 */
static void
zebra_mlag_publish_process_state (struct zserv *client,
                                  zebra_message_types_t msg_type)
{
  struct stream *s;

  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug ("%s: Publishing MLAG process state:%s to %s Client", __func__,
 ...
 */
static void
zebra_mlag_post_data_from_main_thread (struct event *thread)
{
      ...
      if (client->mlag_updates_interested == true)
        {
          if (msg_type != ZEBRA_MLAG_MSG_BCAST &&
              ! CHECK_FLAG (client->mlag_reg_mask1, (1 << msg_type)))
            {
              continue;
            }

          if (IS_ZEBRA_DEBUG_MLAG)
            zlog_debug ("%s: Posting MLAG data of length-%d to client:%d ",
 ...
 */
static void
zebra_mlag_terminate_pthread (struct event *event)
{
  if (IS_ZEBRA_DEBUG_MLAG)
  ...
  if (zrouter.mlag_info.clients_interested_cnt)
    {
      if (IS_ZEBRA_DEBUG_MLAG)
        zlog_debug ("Zebra MLAG: still some clients are interested");
      return;
    }
 ...
 */
void
zebra_mlag_client_register (ZAPI_HANDLER_ARGS)
{
  struct stream *s;
  uint32_t reg_mask = 0;
  int rc = 0;
  ...
  if (zrouter.mlag_info.clients_interested_cnt == 1)
    {
       ...
       */
      if (IS_ZEBRA_DEBUG_MLAG)
        zlog_debug ("First client, opening the channel with MLAG");
      ...
      if (rc < 0)
        {
          ...
          if (IS_ZEBRA_DEBUG_MLAG)
            zlog_debug (
                "Fail to open channel with MLAG,rc:%d, post Proto-down", rc);
          zebra_mlag_publish_process_state (client, ZEBRA_MLAG_PROCESS_DOWN);
        }
    }
 ...
 */
void
zebra_mlag_client_unregister (ZAPI_HANDLER_ARGS)
{
  if (IS_ZEBRA_DEBUG_MLAG)
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug ("Client De-Registered successfully for MLAG Updates");
}
 ...
 */
void
zebra_mlag_forward_client_msg (ZAPI_HANDLER_ARGS)
{
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug ("%s: Enqueued Client:%d data to MLAG Thread ", __func__,
                client->proto);
}
...
int32_t
zebra_mlag_test_mlag_internal (const char *none, const char *primary,
...
int
zebra_mlag_protobuf_encode_client_data (struct stream *s, uint32_t *msg_type)
{
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug ("%s: Mlag ProtoBuf encoding of message:%s, len:%d", __func__,
  ...
  *msg_type = mlag_msg.msg_type;
  switch (mlag_msg.msg_type)
        ...
        if (msg.owner_id == MLAG_OWNER_INTERFACE)
          {
            ...
            pay_load.intf_name = XMALLOC (MTYPE_MLAG_PBUF, vrf_name_len);
            strlcpy (pay_load.intf_name, msg.intf_name, vrf_name_len);
          }
    ...
    case MLAG_MROUTE_ADD_BULK:
      {
        struct mlag_mroute_add msg;
        ZebraMlagMrouteAddBulk Bulk_msg = ZEBRA_MLAG_MROUTE_ADD_BULK__INIT;
        ...
        pay_load = XMALLOC (MTYPE_MLAG_PBUF,
                            sizeof (ZebraMlagMrouteAdd *) * mlag_msg.msg_cnt);
        ...
        for (i = 0, actual = 0; i < mlag_msg.msg_cnt; i++, actual++)
          {
            ...
            pay_load[i] =
                XMALLOC (MTYPE_MLAG_PBUF, sizeof (ZebraMlagMrouteAdd));
            zebra_mlag_mroute_add__init (pay_load[i]);

            vrf_name_len = strlen (msg.vrf_name) + 1;
            pay_load[i]->vrf_name = XMALLOC (MTYPE_MLAG_PBUF, vrf_name_len);
            strlcpy (pay_load[i]->vrf_name, msg.vrf_name, vrf_name_len);
            ...
            if (msg.owner_id == MLAG_OWNER_INTERFACE)
              {
                ...
                strlcpy (pay_load[i]->intf_name, msg.intf_name, vrf_name_len);
              }
          }
        if (! cleanup)
          {
    ...
    case MLAG_MROUTE_DEL_BULK:
      {
        struct mlag_mroute_del msg;
        ZebraMlagMrouteDelBulk Bulk_msg = ZEBRA_MLAG_MROUTE_DEL_BULK__INIT;
        ...
        pay_load = XMALLOC (MTYPE_MLAG_PBUF,
                            sizeof (ZebraMlagMrouteDel *) * mlag_msg.msg_cnt);
        ...
        for (i = 0, actual = 0; i < mlag_msg.msg_cnt; i++, actual++)
          {
            ...
            pay_load[i] =
                XMALLOC (MTYPE_MLAG_PBUF, sizeof (ZebraMlagMrouteDel));
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug ("%s: length of Mlag ProtoBuf encoded message:%s, %d", __func__,
                mlag_lib_msgid_to_str (mlag_msg.msg_type, buf, sizeof (buf)),
                len);
   ...
   */
  len =
      zebra_mlag__header__pack (&hdr, (mlag_wr_buffer + ZEBRA_MLAG_LEN_SIZE));
  n_len = htonl (len);
  ...
  if (IS_ZEBRA_DEBUG_MLAG)
    zlog_debug (
        "%s: length of Mlag ProtoBuf message:%s with Header  %d", __func__,
        mlag_lib_msgid_to_str (mlag_msg.msg_type, buf, sizeof (buf)), len);
...
static void
zebra_fill_protobuf_msg (struct stream *s, char *name, int len)
{
...
int
zebra_mlag_protobuf_decode_message (struct stream *s, uint8_t *data,
                                    uint32_t len)
{
  uint32_t msg_type;
  ZebraMlagHeader *hdr;
        ...
        case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_STATUS_UPDATE:
          {
            ...
            msg = zebra_mlag_status_update__unpack (NULL, hdr->data.len,
                                                    hdr->data.data);
            if (msg == NULL)
              {
            ...
            stream_putw (s, MLAG_MSG_NO_BATCH);
            /* Actual Data */
            zebra_fill_protobuf_msg (s, msg->peerlink, INTERFACE_NAMSIZ);
        ...
        case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_VXLAN_UPDATE:
          {
            ...
            msg = zebra_mlag_vxlan_update__unpack (NULL, hdr->data.len,
                                                   hdr->data.data);
            if (msg == NULL)
              {
                zebra_mlag__header__free_unpacked (hdr, NULL);
        ...
        case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_MROUTE_ADD_BULK:
          {
            ...
            Bulk_msg = zebra_mlag_mroute_add_bulk__unpack (NULL, hdr->data.len,
                                                           hdr->data.data);
            if (Bulk_msg == NULL)
              {
                zebra_mlag__header__free_unpacked (hdr, NULL);
            ...
            /* Payload len */
            stream_putw (
                s, (Bulk_msg->n_mroute_add * sizeof (struct mlag_mroute_add)));
            /* No. of msgs in Batch */
            ...
            for (i = 0; i < Bulk_msg->n_mroute_add; i++)
              {
                if (STREAM_SIZE (s) < VRF_NAMSIZ + 22 + INTERFACE_NAMSIZ)
                  {
                    zlog_warn (
            ...
            zebra_mlag_mroute_add_bulk__free_unpacked (Bulk_msg, NULL);
          }
          break;
        case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_MROUTE_DEL_BULK:
          {
            ...
            Bulk_msg = zebra_mlag_mroute_del_bulk__unpack (NULL, hdr->data.len,
                                                           hdr->data.data);
            if (Bulk_msg == NULL)
              {
                zebra_mlag__header__free_unpacked (hdr, NULL);
            ...
            /* Payload len */
            stream_putw (
                s, (Bulk_msg->n_mroute_del * sizeof (struct mlag_mroute_del)));
            /* No. of msgs in Batch */
            ...
            for (i = 0; i < Bulk_msg->n_mroute_del; i++)
              {
                if (STREAM_SIZE (s) < VRF_NAMSIZ + 16 + INTERFACE_NAMSIZ)
                  {
                    zlog_warn (
            ...
            zebra_mlag_mroute_del_bulk__free_unpacked (Bulk_msg, NULL);
          }
          break;
        case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_ZEBRA_STATUS_UPDATE:
          {
            ...
            msg = zebra_mlag_zebra_status_update__unpack (NULL, hdr->data.len,
                                                          hdr->data.data);
            if (msg == NULL)
              {
                zebra_mlag__header__free_unpacked (hdr, NULL);
            ...
            zebra_mlag_zebra_status_update__free_unpacked (msg, NULL);
          }
          break;
        default:
          break;
