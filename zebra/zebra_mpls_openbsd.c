...
static int
kernel_send_rtmsg_v4 (int action, mpls_label_t in_label,
                      const struct zebra_nhlfe *nhlfe)
{
  ...
  if (ret == -1)
    flog_err_sys (EC_LIB_SOCKET, "%s: %s", __func__, safe_strerror (errno));
...
static int
kernel_send_rtmsg_v6 (int action, mpls_label_t in_label,
                      const struct zebra_nhlfe *nhlfe)
{
  struct iovec iov[5];
  struct rt_msghdr hdr;
...
static int
kernel_lsp_cmd (struct zebra_dplane_ctx *ctx)
{
      ...
      if (((action == RTM_ADD || action == RTM_CHANGE) &&
            ...
            CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))))
        {
          if (nhlfe->nexthop->nh_label->num_labels > 1)
            {
              flog_warn (EC_ZEBRA_MAX_LABELS_PUSH,
                         ...
                         __func__, nhlfe->nexthop->nh_label->num_labels);
              continue;
            }
          ...
          switch (NHLFE_FAMILY (nhlfe))
            {
            case AF_INET:
              kernel_send_rtmsg_v4 (action, dplane_ctx_get_in_label (ctx),
                                    nhlfe);
              break;
            case AF_INET6:
              kernel_send_rtmsg_v6 (action, dplane_ctx_get_in_label (ctx),
                                    nhlfe);
              break;
            default:
              break;
...
kernel_lsp_update (struct zebra_dplane_ctx *ctx)
{
  int ret;
...
kmpw_install (struct zebra_dplane_ctx *ctx)
{
  struct ifreq ifr;
  struct ifmpwreq imr;
  ...
  switch (dplane_ctx_get_pw_af (ctx))
    ...
    case AF_INET6:
      ...
      break;
    default:
      zlog_debug ("%s: unhandled pseudowire address-family (%u)", __func__,
                  dplane_ctx_get_pw_af (ctx));
      return ZEBRA_DPLANE_REQUEST_FAILURE;
    }
  memcpy (&imr.imr_nexthop, (struct sockaddr *) &ss, sizeof (imr.imr_nexthop));
...
static enum zebra_dplane_result
kmpw_uninstall (struct zebra_dplane_ctx *ctx)
{
  struct ifreq ifr;
  struct ifmpwreq imr;
 ...
 */
enum zebra_dplane_result
kernel_pw_update (struct zebra_dplane_ctx *ctx)
{
...
mpls_kernel_init (void)
{
  ...
  if (getsockopt (kr_state.fd, SOL_SOCKET, SO_RCVBUF, &default_rcvbuf,
                  &optlen) == -1)
    flog_err_sys (EC_LIB_SOCKET, "kr_init getsockopt SOL_SOCKET SO_RCVBUF");
  else
