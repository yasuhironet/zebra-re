 ...
 */
uint32_t
num_valid_macs (struct zebra_evpn *zevpn)
{
  unsigned int i;
  uint32_t num_macs = 0;
  struct hash *hash;
...
uint32_t
num_dup_detected_macs (struct zebra_evpn *zevpn)
{
  unsigned int i;
  uint32_t num_macs = 0;
  struct hash *hash;
 ...
 */
static void
zebra_evpn_mac_ifp_new (struct zebra_if *zif)
{
...
static void
zebra_evpn_mac_ifp_unlink (struct zebra_mac *zmac)
{
  struct zebra_if *zif;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    zlog_debug ("VNI %d MAC %pEA unlinked from ifp %s (%u)", zmac->zevpn->vni,
                &zmac->macaddr, ifp->name, ifp->ifindex);
...
void
zebra_evpn_mac_ifp_del (struct interface *ifp)
{
 ...
 */
static void
zebra_evpn_mac_ifp_link (struct zebra_mac *zmac, struct interface *ifp)
{
  struct zebra_if *zif;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    zlog_debug ("VNI %d MAC %pEA linked to ifp %s (%u)", zmac->zevpn->vni,
                &zmac->macaddr, ifp->name, ifp->ifindex);
...
void
zebra_evpn_mac_clear_fwd_info (struct zebra_mac *zmac)
{
  zebra_evpn_mac_ifp_unlink (zmac);
 ...
 */
int
zebra_evpn_rem_mac_install (struct zebra_evpn *zevpn, struct zebra_mac *mac,
                            bool was_static)
{
 ...
 */
int
zebra_evpn_rem_mac_uninstall (struct zebra_evpn *zevpn, struct zebra_mac *mac,
                              bool force)
{
  const struct zebra_if *zif, *br_zif;
  ...
  res =
      dplane_rem_mac_del (ifp, br_ifp, vid, &mac->macaddr, vni->vni, vtep_ip);
  if (res != ZEBRA_DPLANE_REQUEST_FAILURE)
    return 0;
  else
    return -1;
 ...
 */
void
zebra_evpn_deref_ip2mac (struct zebra_evpn *zevpn, struct zebra_mac *mac)
{
...
static void
zebra_evpn_mac_get_access_info (struct zebra_mac *mac,
                                struct interface **p_ifp, vlanid_t *vid)
{
  struct zebra_vxlan_vni *vni;
...
static char *
zebra_evpn_zebra_mac_flag_dump (struct zebra_mac *mac, char *buf, size_t len)
{
  if (mac->flags == 0)
    {
...
static void
zebra_evpn_dad_mac_auto_recovery_exp (struct event *t)
{
...
static void
zebra_evpn_dup_addr_detect_for_mac (struct zebra_vrf *zvrf,
                                    struct zebra_mac *mac,
      ...
      if (zvrf->dad_freeze && zvrf->dad_freeze_time)
        {
          ...
          event_add_timer (
              zrouter.master, zebra_evpn_dad_mac_auto_recovery_exp, mac,
 ...
 */
void
zebra_evpn_print_mac (struct zebra_mac *mac, void *ctxt, json_object *json)
{
  struct vty *vty;
              ...
              if (IS_ZEBRA_NEIGH_ACTIVE (n))
                json_object_array_add (json_active_nbrs,
                                       json_object_new_string (ipaddr2str (
                                           &n->ip, buf2, sizeof (buf2))));
              else
                json_object_array_add (json_inactive_nbrs,
                                       json_object_new_string (ipaddr2str (
                                           &n->ip, buf2, sizeof (buf2))));
            }
...
static char *
zebra_evpn_print_mac_flags (struct zebra_mac *mac, char *flags_buf,
                            size_t flags_buf_sz)
{
 ...
 */
void
zebra_evpn_print_mac_hash (struct hash_bucket *bucket, void *ctxt)
{
  struct vty *vty;
  ...
  if (CHECK_FLAG (mac->flags, ZEBRA_MAC_LOCAL))
  ...
  else if (CHECK_FLAG (mac->flags, ZEBRA_MAC_REMOTE))
    {
      ...
      if (json_mac_hdr == NULL)
        {
          if ((wctx->flags & SHOW_REMOTE_MAC_FROM_VTEP) && (wctx->count == 0))
            {
              vty_out (vty, "\nVNI %u\n\n", wctx->zevpn->vni);
          ...
          if (mac->es == NULL)
            inet_ntop (AF_INET, &mac->fwd_info.r_vtep_ip, addr_buf,
                       sizeof (addr_buf));

          vty_out (
              vty, "%-17s %-6s %-5s %-30s %-5s %u/%u\n", buf1, "remote",
              zebra_evpn_print_mac_flags (mac, flags_buf, sizeof (flags_buf)),
 ...
 */
void
zebra_evpn_print_mac_hash_detail (struct hash_bucket *bucket, void *ctxt)
{
  struct vty *vty;
 ...
 */
int
zebra_evpn_macip_send_msg_to_client (vni_t vni, const struct ethaddr *macaddr,
 ...
 */
struct zebra_mac *
zebra_evpn_mac_add (struct zebra_evpn *zevpn, const struct ethaddr *macaddr)
{
  ...
  if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    {
      ...
      zlog_debug (
          "%s: MAC %pEA flags %s", __func__, &mac->macaddr,
          zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
    }
  return mac;
}
 ...
 */
int
zebra_evpn_mac_del (struct zebra_evpn *zevpn, struct zebra_mac *mac)
{
  struct zebra_mac *tmp_mac;
...
static bool
zebra_evpn_check_mac_del_from_db (struct mac_walk_ctx *wctx,
                                  struct zebra_mac *mac)
{
 ...
 */
static void
zebra_evpn_mac_del_hash_entry (struct hash_bucket *bucket, void *arg)
{
  ...
  if (zebra_evpn_check_mac_del_from_db (wctx, mac))
    {
      if (wctx->upd_client && (mac->flags & ZEBRA_MAC_LOCAL))
        {
          zebra_evpn_mac_send_del_to_client (wctx->zevpn->vni, &mac->macaddr,
                                             mac->flags, false);
        }
      if (wctx->uninstall)
        {
 ...
 */
void
zebra_evpn_mac_del_all (struct zebra_evpn *zevpn, int uninstall,
 ...
 */
struct zebra_mac *
zebra_evpn_mac_lookup (struct zebra_evpn *zevpn, const struct ethaddr *mac)
{
 ...
 */
int
zebra_evpn_mac_send_add_to_client (vni_t vni, const struct ethaddr *macaddr,
                                   ...
                                   struct zebra_evpn_es *es)
{
  ...
  return zebra_evpn_macip_send_msg_to_client (
      vni, macaddr, NULL, flags, seq, ZEBRA_NEIGH_ACTIVE, es, ZEBRA_MACIP_ADD);
}
 ...
 */
int
zebra_evpn_mac_send_del_to_client (vni_t vni, const struct ethaddr *macaddr,
...
int
zebra_evpn_sync_mac_dp_install (struct zebra_mac *mac, bool set_inactive,
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        {
          ...
          zlog_debug (
              ...
              set_static ? "install" : "uninstall", zevpn->vni, &mac->macaddr,
              mac->es ? mac->es->esi_str : "-",
              zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)),
              set_inactive ? "inactive " : "");
        }
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    {
      ...
      zlog_debug (
          "dp-install sync-mac vni %u mac %pEA es %s %s%s%s", zevpn->vni,
...
void
zebra_evpn_mac_send_add_del_to_client (struct zebra_mac *mac,
                                       bool old_bgp_ready, bool new_bgp_ready)
{
  if (new_bgp_ready)
    zebra_evpn_mac_send_add_to_client (mac->zevpn->vni, &mac->macaddr,
 ...
 */
static void
zebra_evpn_mac_hold_exp_cb (struct event *t)
{
  struct zebra_mac *mac;
  bool old_bgp_ready;
  bool new_bgp_ready;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    {
      ...
      zlog_debug (
          "sync-mac vni %u mac %pEA es %s %shold expired", mac->zevpn->vni,
          &mac->macaddr, mac->es ? mac->es->esi_str : "-",
          zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
    }
...
static inline void
zebra_evpn_mac_start_hold_timer (struct zebra_mac *mac)
{
  if (mac->hold_timer)
    return;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    {
      ...
      zlog_debug (
          "sync-mac vni %u mac %pEA es %s %shold started", mac->zevpn->vni,
          &mac->macaddr, mac->es ? mac->es->esi_str : "-",
          zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
    }
...
void
zebra_evpn_mac_stop_hold_timer (struct zebra_mac *mac)
{
  if (! mac->hold_timer)
    return;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    {
      ...
      zlog_debug (
          "sync-mac vni %u mac %pEA es %s %shold stopped", mac->zevpn->vni,
          &mac->macaddr, mac->es ? mac->es->esi_str : "-",
          zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
    }
...
void
zebra_evpn_sync_mac_del (struct zebra_mac *mac)
{
  bool old_static;
  bool new_static;
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    {
      ...
      zlog_debug (
          "sync-mac del vni %u mac %pEA es %s seq %d f %s", mac->zevpn->vni,
          &mac->macaddr, mac->es ? mac->es->esi_str : "-", mac->loc_seq,
          zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
    }
...
static inline bool
zebra_evpn_mac_is_bgp_seq_ok (struct zebra_evpn *zevpn, struct zebra_mac *mac,
      ...
      if (is_local && ! zebra_evpn_mac_is_ready_for_bgp (mac->flags))
        {
          if (IS_ZEBRA_DEBUG_EVPN_MH_MAC || IS_ZEBRA_DEBUG_VXLAN)
            zlog_debug (
                ...
                sync ? "sync" : "rem", zevpn->vni, n_type, &mac->macaddr,
                tmp_seq, mac->flags);
          return true;
        }
                                 ...
                                 uint8_t flags, uint32_t seq, const esi_t *esi)
{
      ...
      if (sticky || remote_gw)
        {
          if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
            zlog_debug (
                "Ignore sync-macip vni %u mac %pEA%s%s%s%s", zevpn->vni,
                macaddr, ipa_len ? " IP " : "",
                ipa_len ? ipaddr2str (ipaddr, ipbuf, sizeof (ipbuf)) : "",
                sticky ? " sticky" : "", remote_gw ? " remote_gw" : "");
          return NULL;
        }
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_MAC && (old_flags != new_flags))
        {
          ...
          zlog_debug (
              "sync-mac vni %u mac %pEA old_f %snew_f %s", zevpn->vni, macaddr,
              zebra_evpn_zebra_mac_flag_dump (&omac, omac_buf,
                                              sizeof (omac_buf)),
              zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
        }
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
    {
      ...
      zlog_debug (
          "sync-mac %s vni %u mac %pEA es %s seq %d f %s%s%s",
  ...
  if (seq_change || es_change || ! old_local)
    zebra_evpn_process_neigh_on_local_mac_change (zevpn, mac, seq_change,
                                                  es_change);
 ...
 */
static bool
zebra_evpn_local_mac_update_fwd_info (struct zebra_mac *mac,
...
static void
zebra_evpn_send_mac_hash_entry_to_client (struct hash_bucket *bucket,
                                          void *arg)
{
  struct mac_walk_ctx *wctx = arg;
  ...
  if (CHECK_FLAG (zmac->flags, ZEBRA_MAC_LOCAL))
    zebra_evpn_mac_send_add_to_client (wctx->zevpn->vni, &zmac->macaddr,
                                       zmac->flags, zmac->loc_seq, zmac->es);
}

/* Iterator to Notify Local MACs of a EVPN */
void
zebra_evpn_send_mac_list_to_client (struct zebra_evpn *zevpn)
{
  struct mac_walk_ctx wctx;
...
void
zebra_evpn_rem_mac_del (struct zebra_evpn *zevpn, struct zebra_mac *mac)
{
...
/* Print Duplicate MAC */
void
zebra_evpn_print_dad_mac_hash (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_mac *mac;
...
void
zebra_evpn_print_dad_mac_hash_detail (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_mac *mac;
  ...
  if (update_mac)
    {
      ...
      zebra_evpn_dup_addr_detect_for_mac (zvrf, mac, mac->fwd_info.r_vtep_ip,
                                          do_dad, &is_dup_detect, false);
...
int
zebra_evpn_add_update_local_mac (struct zebra_vrf *zvrf,
  ...
  if (! mac)
    {
      if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        zlog_debug ("ADD %sMAC %pEA intf %s(%u) VID %u -> VNI %u%s",
                    sticky ? "sticky " : "", macaddr, ifp->name, ifp->ifindex,
                    vid, zevpn->vni, local_inactive ? " local-inactive" : "");
      ...
      if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        {
          ...
          zlog_debug (
              ...
              zevpn->vni, local_inactive ? "local-inactive " : "",
              zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
        }
              ...
              dp_static == old_static && ! es_change)
            {
              if (IS_ZEBRA_DEBUG_VXLAN)
                zlog_debug (
                    ...
                    sticky ? "sticky " : "", macaddr, ifp->name, ifp->ifindex,
                    vid, zevpn->vni, local_inactive ? " local_inactive" : "");
              return 0;
            }
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        {
          ...
          zlog_debug (
              "local mac vni %u mac %pEA es %s seq %d f %s%s", zevpn->vni,
              macaddr, mac->es ? mac->es->esi_str : "", mac->loc_seq,
              zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)),
...
int
zebra_evpn_del_local_mac (struct zebra_evpn *zevpn, struct zebra_mac *mac,
                          bool clear_static)
{
  bool old_bgp_ready;
  bool new_bgp_ready;
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
        {
          ...
          zlog_debug (
              "re-add sync-mac vni %u mac %pEA es %s seq %d f %s", zevpn->vni,
              &mac->macaddr, mac->es ? mac->es->esi_str : "-", mac->loc_seq,
              zebra_evpn_zebra_mac_flag_dump (mac, mac_buf, sizeof (mac_buf)));
        }
...
void
zebra_evpn_mac_gw_macip_add (struct interface *ifp, struct zebra_evpn *zevpn,
                             const struct ipaddr *ip, struct zebra_mac **macp,
...
void
zebra_evpn_mac_svi_del (struct interface *ifp, struct zebra_evpn *zevpn)
{
  struct zebra_mac *mac;
...
void
zebra_evpn_mac_svi_add (struct interface *ifp, struct zebra_evpn *zevpn)
{
  struct zebra_mac *mac = NULL;
  struct ethaddr macaddr;
  struct zebra_if *zif = ifp->info;
