...
static const struct
{
  ...
  [ZEBRA_ROUTE_NHG] = { ZEBRA_ROUTE_NHG,
  ...
  [ZEBRA_ROUTE_VNC_DIRECT] = { ZEBRA_ROUTE_VNC_DIRECT,
                               ZEBRA_EBGP_DISTANCE_DEFAULT, META_QUEUE_BGP },
...
static const char *
subqueue2str (enum meta_queue_indexes index)
{
  switch (index)
    {
    case META_QUEUE_NHG:
...
static ssize_t
printfrr_zebra_node (struct fbuf *buf, struct printfrr_eargs *ea,
                     const void *ptr)
{
...
static char *
_dump_re_status (const struct route_entry *re, char *buf, size_t len)
{
  if (re->status == 0)
    {
...
int
is_zebra_valid_kernel_table (uint32_t table_id)
{
#ifdef linux
...
int
is_zebra_main_routing_table (uint32_t table_id)
{
...
int
zebra_check_addr (const struct prefix *p)
{
  if (p->family == AF_INET)
    {
      uint32_t addr;
...
static void
route_entry_attach_ref (struct route_entry *re, struct nhg_hash_entry *new)
{
  re->nhe = new;
  re->nhe_id = new->id;
...
int
route_entry_update_nhe (struct route_entry *re,
...
int
rib_handle_nhg_replace (struct nhg_hash_entry *old_entry,
                        struct nhg_hash_entry *new_entry)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED || IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: replacing routes nhe (%u) OLD %p NEW %p", __func__,
                new_entry->id, new_entry, old_entry);

  /* We have to do them ALL */
  RB_FOREACH (zrt, zebra_router_table_head, &zrouter.tables)
    {
      for (rn = route_top (zrt->table); rn; rn = srcdest_route_next (rn))
        {
          RNODE_FOREACH_RE_SAFE (rn, re, next)
            {
              if (re->nhe && re->nhe == old_entry)
                ret += route_entry_update_nhe (re, new_entry);
            }
        }
    }
...
struct route_entry *
rib_match (afi_t afi, safi_t safi, vrf_id_t vrf_id, const union g_addr *addr,
...
struct route_entry *
rib_match_multicast (afi_t afi, vrf_id_t vrf_id, union g_addr *gaddr,
                     struct route_node **rn_out)
{
  ...
  switch (zrouter.ipv4_multicast_mode)
    ...
    case MCAST_MIX_MRIB_FIRST:
      ...
      if (! mre)
        re = ure = rib_match (afi, SAFI_UNICAST, vrf_id, gaddr, &u_rn);
      break;
  ...
  if (IS_ZEBRA_DEBUG_RIB)
    {
      char buf[BUFSIZ];
      inet_ntop (afi == AFI_IP ? AF_INET : AF_INET6, gaddr, buf, BUFSIZ);

      zlog_debug ("%s: %s: %pRN vrf: %s(%u) found %s, using %s", __func__, buf,
                  (re == mre) ? m_rn : u_rn, vrf_id_to_name (vrf_id), vrf_id,
 ...
 */
void
rib_install_kernel (struct route_node *rn, struct route_entry *re,
                    struct route_entry *old)
{
  ...
  if (old && (old != re) && (old->type != re->type))
    zsend_route_notify_owner (rn, old, ZAPI_ROUTE_BETTER_ADMIN_WON, info->afi,
                              info->safi);
...
rib_uninstall_kernel (struct route_node *rn, struct route_entry *re)
{
  ...
  switch (dplane_route_delete (rn, re))
    ...
    case ZEBRA_DPLANE_REQUEST_FAILURE:
      flog_err (EC_ZEBRA_DP_INSTALL_FAIL,
                "%u:%pRN: Failed to enqueue dataplane uninstall", re->vrf_id,
                rn);
      break;
    case ZEBRA_DPLANE_REQUEST_SUCCESS:
      if (zvrf)
...
void
zebra_rib_evaluate_rn_nexthops (struct route_node *rn, uint32_t seq,
                                bool rt_delete)
{
  rib_dest_t *dest = rib_dest_from_rnode (rn);
  struct rnh *rnh;
  ...
  while (rn)
    {
      if (IS_ZEBRA_DEBUG_NHT_DETAILED)
        zlog_debug ("%s: %pRN Being examined for Nexthop Tracking Count: %zd",
                    __func__, rn, dest ? rnh_list_count (&dest->nht) : 0);

      if (rt_delete && (! dest || ! rnh_list_count (&dest->nht)))
        {
          if (IS_ZEBRA_DEBUG_NHT_DETAILED)
            zlog_debug ("%pRN has no tracking NHTs. Bailing", rn);
          break;
        }
      if (! dest)
        {
      ...
      frr_each_safe (rnh_list, &dest->nht, rnh)
        {
          ...
          if (IS_ZEBRA_DEBUG_NHT_DETAILED)
            zlog_debug (
                ...
                zvrf_name (zvrf), zvrf_id (zvrf), rn, rnh->node, seq,
                rnh->seqno);
          ...
          if (rnh->seqno == seq)
            {
              if (IS_ZEBRA_DEBUG_NHT_DETAILED)
                zlog_debug ("    Node processed and moved already");
              continue;
            }

          rnh->seqno = seq;
          zebra_evaluate_rnh (zvrf, family2afi (p->family), 0, p, rnh->safi);
        }
...
rib_gc_dest (struct route_node *rn)
{
  ...
  zebra_rib_evaluate_rn_nexthops (rn, zebra_router_get_next_sequence (), true);
                     ...
                     struct route_entry *new)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB)
    zlog_debug ("%s(%u:%u):%pRN: Adding route rn %p, re %p (%s)",
                zvrf_name (zvrf), zvrf_id (zvrf), new->table, rn, rn, new,
                     ...
                     struct route_entry *old)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB)
    zlog_debug ("%s(%u:%u):%pRN: Deleting route rn %p, re %p (%s)",
                zvrf_name (zvrf), zvrf_id (zvrf), old->table, rn, rn, old,
          ...
          if (IS_ZEBRA_DEBUG_RIB)
            {
              if (new != old)
                zlog_debug (
                    ...
                    zvrf_name (zvrf), zvrf_id (zvrf), new->table, rn, rn, new,
                    zebra_route_string (new->type), old,
                    zebra_route_string (old->type));
              else
                zlog_debug ("%s(%u:%u):%pRN: Updating route rn %p, re %p (%s)",
                            zvrf_name (zvrf), zvrf_id (zvrf), new->table, rn,
                            rn, new, zebra_route_string (new->type));
            }
  ...
  if (alternate->type == ZEBRA_ROUTE_CONNECT)
    {
      ...
      for (ALL_NEXTHOPS (alternate->nhe->nhg, nexthop))
        {
          struct interface *ifp =
              if_lookup_by_index (nexthop->ifindex, alternate->vrf_id);
      ...
      for (ALL_NEXTHOPS (current->nhe->nhg, nexthop))
        {
          struct interface *ifp =
              if_lookup_by_index (nexthop->ifindex, current->vrf_id);
...
rib_process (struct route_node *rn)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    {
      ...
      zlog_debug ("%s(%u:%u):%pRN: Processing rn %p", VRF_LOGNAME (vrf),
                  vrf_id, re->table, rn, rn);
    }
  ...
  RNODE_FOREACH_RE_SAFE (rn, re, next)
    {
          ...
          if (! nexthop_active_update (rn, re))
            {
                  ...
                  if (re != old_selected)
                    {
                      if (IS_ZEBRA_DEBUG_RIB)
                        zlog_debug (
                            ...
                            __func__, VRF_LOGNAME (vrf), vrf_id, rn);
                      rib_unlink (rn, re);
                      continue;
                    }
                  else
              ...
              zsend_route_notify_owner (rn, re, ZAPI_ROUTE_FAIL_INSTALL,
                                        info->afi, info->safi);
              continue;
            }
        }
      ...
      /* Infinite distance. */
      if (re->distance == DISTANCE_INFINITY && re->type != ZEBRA_ROUTE_KERNEL)
        {
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    {
      ...
      entry = old_selected   ? old_selected
  ...
  bool selected_changed =
      new_selected && CHECK_FLAG (new_selected->status, ROUTE_ENTRY_CHANGED);
      ...
      if (old_selected)
        {
           ...
           */
          if (! new_selected ||
              CHECK_FLAG (old_selected->status, ROUTE_ENTRY_REMOVED))
            redistribute_delete (rn, old_selected, new_selected);
      ...
      if (CHECK_FLAG (re->status, ROUTE_ENTRY_REMOVED))
        {
          if (IS_ZEBRA_DEBUG_RIB)
            {
              rnode_debug (rn, vrf_id, "rn %p, removing re %p", (void *) rn,
                           (void *) re);
            }
          rib_unlink (rn, re);
        }
...
static void
zebra_rib_evaluate_mpls (struct route_node *rn)
{
  rib_dest_t *dest = rib_dest_from_rnode (rn);
  ...
  if (CHECK_FLAG (dest->flags, RIB_DEST_UPDATE_LSPS))
    {
      if (IS_ZEBRA_DEBUG_MPLS)
        zlog_debug ("%s(%u): Scheduling all LSPs upon RIB completion",
 ...
 */
static bool
rib_route_match_ctx (const struct route_entry *re,
                     const struct zebra_dplane_ctx *ctx, bool is_update)
{
  bool result = false;
...
static void
zebra_rib_fixup_system (struct route_node *rn)
{
  struct route_entry *re;
...
static bool
rib_compare_routes (const struct route_entry *re1,
 ...
 */
static bool
rib_update_nhg_from_ctx (struct nexthop_group *re_nhg,
                         const struct nexthop_group *ctx_nhg, bool *changed_p)
{
  bool matched_p = true;
      ...
      if (! nexthop_same (ctx_nexthop, nexthop))
        {
          ...
          if (IS_ZEBRA_DEBUG_RIB_DETAILED || IS_ZEBRA_DEBUG_NHG_DETAIL)
            {
              zlog_debug (
                  "%s: no ctx match for rib nh %pNHv %s", __func__, nexthop,
      ...
      if (CHECK_FLAG (ctx_nexthop->flags, NEXTHOP_FLAG_FIB))
          ...
          if (! CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))
            {
              if (IS_ZEBRA_DEBUG_NHG_DETAIL)
                zlog_debug ("%s: rib nh %pNHv -> installed", __func__,
                            nexthop);
      ...
      else
        {
          if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))
            {
              if (IS_ZEBRA_DEBUG_NHG_DETAIL)
                zlog_debug ("%s: rib nh %pNHv -> uninstalled", __func__,
                            nexthop);
 ...
 */
static bool
rib_update_re_from_ctx (struct route_entry *re, struct route_node *rn,
                        struct zebra_dplane_ctx *ctx)
{
  ...
  if (matched)
  ...
  else if (CHECK_FLAG (re->status, ROUTE_ENTRY_USE_FIB_NHG))
    {
       ...
       */
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s(%u:%u):%pRN update_from_ctx(): replacing fib nhg",
                    VRF_LOGNAME (vrf), re->vrf_id, re->table, rn);
    ...
    {
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s(%u:%u):%pRN update_from_ctx(): no fib nhg",
                    VRF_LOGNAME (vrf), re->vrf_id, re->table, rn);
    }
...
check_backups:
  ...
  if (matched)
  ...
  else if (re->fib_backup_ng.nexthop)
    {
       ...
       */
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s(%u):%pRN update_from_ctx(): replacing fib backup nhg",
                    VRF_LOGNAME (vrf), re->vrf_id, rn);
    ...
    {
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s(%u):%pRN update_from_ctx(): no fib backup nhg",
                    VRF_LOGNAME (vrf), re->vrf_id, rn);
    }
  ...
  if (ctxnhg && ctxnhg->nexthop)
    {
      ...
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug (
            ...
            VRF_LOGNAME (vrf), re->vrf_id, rn, (changed_p ? "true" : "false"));

      copy_nexthops (&(re->fib_backup_ng.nexthop), ctxnhg->nexthop, NULL);
    }
 ...
 */
static void
rib_process_result (struct zebra_dplane_ctx *ctx)
{
  struct zebra_vrf *zvrf = NULL;
  struct vrf *vrf;
  struct route_node *rn = NULL;
  ...
  if (re)
    {
      if (re->dplane_sequence != seq)
        {
          if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
            zlog_debug ("%s(%u):%pRN Stale dplane result for re %p",
                        VRF_LOGNAME (vrf), dplane_ctx_get_vrf (ctx), rn, re);
        }
      else
        {
  ...
  if (old_re)
    {
      if (old_re->dplane_sequence != dplane_ctx_get_old_seq (ctx))
        {
          if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
            zlog_debug ("%s(%u:%u):%pRN Stale dplane result for old_re %p",
                        VRF_LOGNAME (vrf), dplane_ctx_get_vrf (ctx),
                        old_re->table, rn, old_re);
        }
      else
    ...
    case DPLANE_OP_ROUTE_UPDATE:
      if (status == ZEBRA_DPLANE_REQUEST_SUCCESS)
          ...
          if (old_re && old_re != re)
            {
              ...
              UNSET_FLAG (old_re->status, ROUTE_ENTRY_INSTALLED);
            }
          ...
          if (re)
            {
              ...
              if (! fib_changed)
                {
                  if (IS_ZEBRA_DEBUG_DPLANE_DETAIL)
                    zlog_debug ("%s(%u:%u):%pRN no fib change for re",
                                VRF_LOGNAME (vrf), dplane_ctx_get_vrf (ctx),
      ...
      else
        {
          ...
          zlog_warn ("%s(%u:%u):%pRN: Route install failed", VRF_LOGNAME (vrf),
                     dplane_ctx_get_vrf (ctx), dplane_ctx_get_table (ctx), rn);
        }
      break;
 ...
 */
static int
rib_count_installed_nh (struct route_entry *re)
{
  int count = 0;
  struct nexthop *nexthop;
 ...
 */
static void
rib_process_dplane_notify (struct zebra_dplane_ctx *ctx)
{
  struct route_node *rn = NULL;
  struct route_entry *re = NULL;
  struct vrf *vrf;
 ...
 */
static void
process_subq_evpn (struct listnode *lnode)
{
  ...
  if (w->type == WQ_EVPN_WRAPPER_TYPE_VRFROUTE)
    {
      if (w->add_p)
        zebra_vxlan_evpn_vrf_route_add (w->vrf_id, &w->macaddr, &w->ip,
                                        &w->prefix);
      else
        zebra_vxlan_evpn_vrf_route_del (w->vrf_id, &w->ip, &w->prefix);
    }
  else if (w->type == WQ_EVPN_WRAPPER_TYPE_REM_ES)
    {
      if (w->add_p)
        zebra_evpn_remote_es_add (&w->esi, w->ip.ipaddr_v4, w->esr_rxed,
                                  w->df_alg, w->df_pref);
      else
  ...
  else if (w->type == WQ_EVPN_WRAPPER_TYPE_REM_MACIP)
    {
      ...
      if (w->add_p)
        zebra_evpn_rem_macip_add (w->vni, &w->macaddr, ipa_len, &w->ip,
                                  w->flags, w->seq, w->vtep_ip, &w->esi);
      else
        zebra_evpn_rem_macip_del (w->vni, &w->macaddr, ipa_len, &w->ip,
                                  w->vtep_ip);
    }
 ...
 */
static void
process_subq_nhg (struct listnode *lnode)
{
  struct nhg_ctx *ctx;
  ...
  if (w->type == WQ_NHG_WRAPPER_TYPE_CTX)
    {
      ...
      if (IS_ZEBRA_DEBUG_RIB_DETAILED)
        zlog_debug ("NHG Context id=%u dequeued from sub-queue %s", ctx->id,
                    subqueue2str (qindex));
  ...
  else if (w->type == WQ_NHG_WRAPPER_TYPE_NHG)
    {
      ...
      newnhe = zebra_nhg_proto_add (nhe->id, nhe->type, nhe->zapi_instance,
                                    nhe->zapi_session, &nhe->nhg, 0);

      /* Report error to daemon via ZAPI */
      if (newnhe == NULL)
        zsend_nhg_notify (nhe->type, nhe->zapi_instance, nhe->zapi_session,
                          nhe->id, ZAPI_NHG_FAIL_INSTALL);
...
static void
process_subq_early_label (struct listnode *lnode)
{
...
static void
process_subq_route (struct listnode *lnode, uint8_t qindex)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    {
      ...
      zlog_debug ("%s(%u:%u):%pRN rn %p dequeued from sub-queue %s",
                  zvrf_name (zvrf), zvrf_id (zvrf), re ? re->table : 0, rnode,
                  rnode, subqueue2str (qindex));
    }
...
static void
rib_re_nhg_free (struct route_entry *re)
{
  if (re->nhe && re->nhe_id)
    {
...
static void
early_route_memory_free (struct zebra_early_route *ere)
{
  if (ere->re_nhe)
...
static void
process_subq_early_route_add (struct zebra_early_route *ere)
{
  ...
  /* Lookup table.  */
  table = zebra_vrf_get_table_with_table_id (ere->afi, ere->safi, re->vrf_id,
                                             re->table);
  if (! table)
    {
      ...
      if (! nhe)
        {
          ...
          flog_err (
              ...
              ere->src_p_provided
                  ? prefix2str (&ere->src_p, buf2, sizeof (buf2))
                  : "");
  ...
  if (IS_ZEBRA_DEBUG_RIB)
    {
      rnode_debug (
          rn, re->vrf_id,
          "Inserting route rn %p, re %p (%s) existing %p, same_count %d", rn,
          re, zebra_route_string (re->type), same, same_count);

      if (IS_ZEBRA_DEBUG_RIB_DETAILED)
        route_entry_dump (&ere->p, ere->src_p_provided ? &ere->src_p : NULL,
                          re);
    }
      ...
      if (CHECK_FLAG (re->status, ROUTE_ENTRY_REMOVED))
        {
          ...
          if (IS_ZEBRA_DEBUG_RIB)
            rnode_debug (rn, re->vrf_id, "rn %p, removing unneeded re %p", rn,
                         re);
...
static void
process_subq_early_route_delete (struct zebra_early_route *ere)
{
  struct route_table *table;
  struct route_node *rn;
  struct route_entry *re;
      ...
      if (IS_ZEBRA_DEBUG_RIB)
        {
          ...
          if (ere->src_p_provided && ere->src_p.prefixlen)
            prefix2str (&ere->src_p, src_buf, sizeof (src_buf));
          else
            src_buf[0] = '\0';

          zlog_debug ("%s[%d]:%pRN%s%s doesn't exist in rib", vrf->name,
                      ere->re->table, rn, (src_buf[0] != '\0') ? " from " : "",
                      src_buf);
        }
      early_route_memory_free (ere);
      return;
  ...
  RNODE_FOREACH_RE (rn, re)
    {
      ...
      if (re->type == ZEBRA_ROUTE_KERNEL && re->metric != ere->re->metric)
        continue;
      if (re->type == ZEBRA_ROUTE_CONNECT && (rtnh = re->nhe->nhg.nexthop) &&
          ...
          if (IS_ZEBRA_DEBUG_RIB)
            {
              if (nh)
                rnode_debug (rn, ere->re->vrf_id,
                             ...
                             inet_ntop (afi2family (ere->afi), &nh->gate, buf2,
                                        sizeof (buf2)),
                             nh->ifindex, ere->re->type);
              else
                rnode_debug (rn, ere->re->vrf_id,
                             "type %d doesn't exist in rib", ere->re->type);
            }
          route_unlock_node (rn);
          early_route_memory_free (ere);
          return;
 ...
 */
static void
process_subq_early_route (struct listnode *lnode)
{
...
static void
process_subq_gr_run (struct listnode *lnode)
{
 ...
 */
static unsigned int
process_subq (struct list *subq, enum meta_queue_indexes qindex)
{
 ...
 */
static int
rib_meta_queue_add (struct meta_queue *mq, void *data)
{
  struct route_node *rn = NULL;
  ...
  if (CHECK_FLAG (rib_dest_from_rnode (rn)->flags, RIB_ROUTE_QUEUED (qindex)))
    {
      if (IS_ZEBRA_DEBUG_RIB_DETAILED)
        rnode_debug (rn, re->vrf_id, "rn %p is already queued in sub-queue %s",
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    rnode_debug (rn, re->vrf_id, "queued rn %p into sub-queue %s", (void *) rn,
                 subqueue2str (qindex));
...
static int
early_label_meta_queue_add (struct meta_queue *mq, void *data)
{
...
static int
rib_meta_queue_nhg_ctx_add (struct meta_queue *mq, void *data)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    zlog_debug ("NHG Context id=%u queued into sub-queue %s", ctx->id,
                subqueue2str (qindex));
...
static int
rib_meta_queue_nhg_add (struct meta_queue *mq, void *data)
{
  struct nhg_hash_entry *nhe = NULL;
  uint8_t qindex = META_QUEUE_NHG;
  struct wq_nhg_wrapper *w;
...
static int
rib_meta_queue_evpn_add (struct meta_queue *mq, void *data)
{
...
static int
mq_add_handler (void *data,
                int (*mq_add_func) (struct meta_queue *mq, void *data))
{
  if (zrouter.ribq == NULL)
    {
      flog_err (EC_ZEBRA_WQ_NONEXISTENT, "%s: work_queue does not exist!",
                __func__);
      return -1;
    }
...
void
mpls_ftn_uninstall (struct zebra_vrf *zvrf, enum lsp_types_t type,
...
void
mpls_zapi_labels_process (bool add_p, struct zebra_vrf *zvrf,
...
rib_queue_add (struct route_node *rn)
{
  ...
  if (! rnode_to_ribs (rn))
    {
      zlog_debug ("%s: called for route_node (%p, %u) with no ribs", __func__,
 ...
 */
int
rib_queue_nhg_ctx_add (struct nhg_ctx *ctx)
{
  assert (ctx);
 ...
 */
int
rib_queue_nhe_add (struct nhg_hash_entry *nhe)
{
  if (nhe == NULL)
    return -1;
 ...
 */
int
zebra_rib_queue_evpn_route_add (vrf_id_t vrf_id, const struct ethaddr *rmac,
                                ...
                                const struct prefix *host_prefix)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    zlog_debug ("%s: (%u)%pIA, host prefix %pFX enqueued", __func__, vrf_id,
                vtep_ip, host_prefix);
...
int
zebra_rib_queue_evpn_route_del (vrf_id_t vrf_id, const struct ipaddr *vtep_ip,
                                const struct prefix *host_prefix)
{
  struct wq_evpn_wrapper *w;
...
int
zebra_rib_queue_evpn_rem_es_add (const esi_t *esi,
...
int
zebra_rib_queue_evpn_rem_es_del (const esi_t *esi,
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    {
      ...
      zlog_debug ("%s: vtep %pI4, esi %s enqueued", __func__, vtep_ip, buf);
    }
 ...
 */
int
zebra_rib_queue_evpn_rem_macip_add (vni_t vni, const struct ethaddr *macaddr,
                                    const struct ipaddr *ipaddr, uint8_t flags,
                                    uint32_t seq, struct in_addr vtep_ip,
                                    const esi_t *esi)
{
  struct wq_evpn_wrapper *w;
  char buf[ESI_STR_LEN];
...
int
zebra_rib_queue_evpn_rem_macip_del (vni_t vni, const struct ethaddr *macaddr,
                                    ...
                                    struct in_addr vtep_ip)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    zlog_debug ("%s: mac %pEA, vtep %pI4 enqueued", __func__, macaddr,
                &vtep_ip);
 ...
 */
int
zebra_rib_queue_evpn_rem_vtep_add (vrf_id_t vrf_id, vni_t vni,
                                   struct in_addr vtep_ip, int flood_control)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    zlog_debug ("%s: vrf %u, vtep %pI4 enqueued", __func__, vrf_id, &vtep_ip);

  return mq_add_handler (w, rib_meta_queue_evpn_add);
}

int
zebra_rib_queue_evpn_rem_vtep_del (vrf_id_t vrf_id, vni_t vni,
                                   struct in_addr vtep_ip)
{
  struct wq_evpn_wrapper *w;
...
static void
evpn_meta_queue_free (struct meta_queue *mq, struct list *l,
                      struct zebra_vrf *zvrf)
{
...
static void
nhg_meta_queue_free (struct meta_queue *mq, struct list *l,
                     struct zebra_vrf *zvrf)
{
  struct wq_nhg_wrapper *w;
  struct listnode *node, *nnode;
      ...
      if (zvrf)
        {
          ...
          if (w->type == WQ_NHG_WRAPPER_TYPE_CTX && w->u.ctx->vrf_id != vrf_id)
            continue;
          else if (w->type == WQ_NHG_WRAPPER_TYPE_NHG &&
...
static void
early_label_meta_queue_free (struct meta_queue *mq, struct list *l,
                             struct zebra_vrf *zvrf)
{
  struct wq_label_wrapper *w;
  struct listnode *node, *nnode;
...
static void
rib_meta_queue_free (struct meta_queue *mq, struct list *l,
                     struct zebra_vrf *zvrf)
{
...
static void
early_route_meta_queue_free (struct meta_queue *mq, struct list *l,
                             struct zebra_vrf *zvrf)
{
...
static void
rib_meta_queue_gr_run_free (struct meta_queue *mq, struct list *l,
                            struct zebra_vrf *zvrf)
{
...
void
meta_queue_free (struct meta_queue *mq, struct zebra_vrf *zvrf)
{
...
{
  if (! (zrouter.ribq =
             work_queue_new (zrouter.master, "route_node processing")))
    {
...
rib_dest_t *
zebra_rib_create_dest (struct route_node *rn)
{
  rib_dest_t *dest;
...
rib_addnode (struct route_node *rn, struct route_entry *re, int process)
{
  ...
  if (CHECK_FLAG (re->status, ROUTE_ENTRY_REMOVED))
    {
      if (IS_ZEBRA_DEBUG_RIB)
        rnode_debug (rn, re->vrf_id, "rn %p, un-removed re %p", (void *) rn,
                     (void *) re);
...
rib_delnode (struct route_node *rn, struct route_entry *re)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB)
    rnode_debug (rn, re->vrf_id, "rn %p, re %p, removing", (void *) rn,
                 (void *) re);
  SET_FLAG (re->status, ROUTE_ENTRY_REMOVED);
  ...
  if (is_zebra_import_table_enabled (afi, re->vrf_id, re->table))
    {
      ...
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s(%u):%pRN: Freeing route rn %p, re %p (%s)",
                    vrf_id_to_name (re->vrf_id), re->vrf_id, rn, rn, re,
                    zebra_route_string (re->type));
    }
 ...
 */
static void
_route_entry_dump_nh (const struct route_entry *re, const char *straddr,
                      const struct nexthop *nexthop)
{
  ...
  switch (nexthop->type)
    ...
    case NEXTHOP_TYPE_IFINDEX:
      ...
      snprintf (nhname, sizeof (nhname), "%s", ifp ? ifp->name : "Unknown");
      break;
  ...
  if (nexthop->nh_label && nexthop->nh_label->num_labels > 0)
    {
      mpls_label2str (nexthop->nh_label->num_labels, nexthop->nh_label->label,
                      label_str, sizeof (label_str), nexthop->nh_label_type,
                      0 /*pretty*/);
  ...
  zlog_debug (
      "%s: %s %s[%u] %svrf %s(%u) %s%s with flags %s%s%s%s%s%s%s%s%s", straddr,
      (nexthop->rparent ? "  NH" : "NH"), nhname, nexthop->ifindex, label_str,
 ...
 */
void
_route_entry_dump (const char *func, union prefixconstptr pp,
                   union prefixconstptr src_pp, const struct route_entry *re)
{
...
static int
rib_meta_queue_gr_run_add (struct meta_queue *mq, void *data)
{
...
static int
rib_meta_queue_early_route_add (struct meta_queue *mq, void *data)
{
  ...
  if (IS_ZEBRA_DEBUG_RIB_DETAILED)
    zlog_debug ("Route %pFX(%u) queued for processing into sub-queue %s",
                &ere->p, ere->re->vrf_id,
...
int
rib_add_gr_run (afi_t afi, vrf_id_t vrf_id, uint8_t proto, uint8_t instance)
{
  struct meta_q_gr_run *gr_run;
 ...
 */
int
rib_add_multipath_nhe (afi_t afi, safi_t safi, struct prefix *p,
...
void
rib_delete (afi_t afi, safi_t safi, vrf_id_t vrf_id, int type,
...
rib_add (afi_t afi, safi_t safi, vrf_id_t vrf_id, int type,
         unsigned short instance, uint32_t flags, struct prefix *p,
         struct prefix_ipv6 *src_p, const struct nexthop *nh, uint32_t nhe_id,
...
static const char *
rib_update_event2str (enum rib_update_event event)
{
...
static void
rib_update_route_node (struct route_node *rn, int type)
{
  struct route_entry *re, *next;
...
void
rib_update_table (struct route_table *table, enum rib_update_event event,
                  int rtype)
{
  struct route_node *rn;
  ...
  if (IS_ZEBRA_DEBUG_EVENT)
    {
      ...
      zvrf =
          table->info ? ((struct rib_table_info *) table->info)->zvrf : NULL;
      vrf = zvrf ? zvrf->vrf : NULL;

      zlog_debug ("%s: %s VRF %s Table %u event %s Route type: %s", __func__,
                  table->info
                      ? afi2str (((struct rib_table_info *) table->info)->afi)
                      : "Unknown",
...
static void
rib_update_handle_vrf_all (enum rib_update_event event, int rtype)
{
  struct zebra_router_table *zrt;
...
rib_update_ctx_init (vrf_id_t vrf_id, enum rib_update_event event)
{
...
static void
rib_update_ctx_fini (struct rib_update_ctx **ctx)
{
  XFREE (MTYPE_RIB_UPDATE_CTX, *ctx);
}

static void
rib_update_handler (struct event *thread)
{
  struct rib_update_ctx *ctx;
...
void
rib_update (enum rib_update_event event)
{
  struct rib_update_ctx *ctx;
...
void
rib_sweep_route (struct event *t)
{
  struct vrf *vrf;
  struct zebra_vrf *zvrf;
...
unsigned long
rib_score_proto_table (uint8_t proto, unsigned short instance,
                       struct route_table *table)
{
  struct route_node *rn;
  struct route_entry *re;
...
unsigned long
rib_score_proto (uint8_t proto, unsigned short instance)
{
  struct vrf *vrf;
  struct zebra_vrf *zvrf;
  ...
  RB_FOREACH (vrf, vrf_id_head, &vrfs_by_id)
    {
      ...
      cnt += rib_score_proto_table (proto, instance,
             ...
             rib_score_proto_table (proto, instance,
                                    zvrf->table[AFI_IP6][SAFI_UNICAST]);

      frr_each (otable, &zvrf->other_tables, ort)
        cnt += rib_score_proto_table (proto, instance, ort->table);
    }
 ...
 */
static void
handle_pw_result (struct zebra_dplane_ctx *ctx)
{
  struct zebra_pw *pw;
 ...
 */
static void
rib_process_dplane_results (struct event *thread)
{
  struct zebra_dplane_ctx *ctx;
  ...
  if (script_name)
    {
      ...
      if (fs)
        ret = frrscript_load (fs, ZEBRA_ON_RIB_PROCESS_HOOK_CALL, NULL);
    }
  ...
  do
    {
       ...
       */
      shut_p =
          atomic_load_explicit (&zrouter.in_shutdown, memory_order_relaxed);
      if (shut_p)
        {
          while (ctx)
            {
...
#ifdef HAVE_SCRIPTING
          if (ret == 0)
            frrscript_call (fs, ZEBRA_ON_RIB_PROCESS_HOOK_CALL, ("ctx", ctx));
#endif /* HAVE_SCRIPTING */
 ...
 */
static int
rib_dplane_results (struct dplane_ctx_list_head *ctxlist)
{
  /* Take lock controlling queue of results */
  frr_with_mutex (&dplane_mutex)
    {
