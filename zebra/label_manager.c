 ...
 */
void
lm_client_connect_call (struct zserv *client, vrf_id_t vrf_id)
{
  hook_call (lm_client_connect, client, vrf_id);
}
void
lm_get_chunk_call (struct label_manager_chunk **lmc, struct zserv *client,
                   uint8_t keep, uint32_t size, uint32_t base, vrf_id_t vrf_id)
{
  hook_call (lm_get_chunk, lmc, client, keep, size, base, vrf_id);
}
void
lm_release_chunk_call (struct zserv *client, uint32_t start, uint32_t end)
{
 ...
 */
int
release_daemon_label_chunks (struct zserv *client)
{
  ...
  if (IS_ZEBRA_DEBUG_PACKET)
    zlog_debug (
        ...
        __func__, zebra_route_string (client->proto), client->instance,
        client->session_id);

  for (ALL_LIST_ELEMENTS_RO (lbl_mgr.lc_list, node, lmc))
    {
      if (lmc->proto == client->proto && lmc->instance == client->instance &&
          lmc->session_id == client->session_id && lmc->keep == 0)
        {
          ret = release_label_chunk (lmc->proto, lmc->instance,
                                     lmc->session_id, lmc->start, lmc->end);
          if (ret == 0)
            count++;
        }
    }
...
int
lm_client_disconnect_cb (struct zserv *client)
{
...
create_label_chunk (uint8_t proto, unsigned short instance,
                    uint32_t session_id, uint8_t keep, uint32_t start,
                    uint32_t end)
{
                             ...
                             uint32_t base)
{
  ...
  /* sanities */
  if ((base < MPLS_LABEL_UNRESERVED_MIN) || (end > MPLS_LABEL_UNRESERVED_MAX))
    {
      zlog_err ("Invalid LM request arguments: base: %u, size: %u", base,
                size);
      return NULL;
    }
  ...
  if (insert_node)
    {
      lmc = create_label_chunk (proto, instance, session_id, keep, base, end);
  ...
  if (first_node)
    {
      ...
      for (node = first_node; node && (node != last_node); node = next)
        {
 ...
 * does not apply
 * @return Pointer to the assigned label chunk, or NULL if the request could
 * not be satisfied
 */
struct label_manager_chunk *
assign_label_chunk (uint8_t proto, unsigned short instance,
                    uint32_t session_id, uint8_t keep, uint32_t size,
                    uint32_t base)
{
  struct label_manager_chunk *lmc;
  ...
  if (base != MPLS_LABEL_BASE_ANY)
    return assign_specific_label_chunk (proto, instance, session_id, keep,
                                        size, base);
  ...
  if (start_free > MPLS_LABEL_UNRESERVED_MAX - size + 1)
    {
      flog_err (EC_ZEBRA_LM_EXHAUSTED_LABELS,
                "Reached max labels. Start: %u, size: %u", start_free, size);
      return NULL;
    }
 ...
 */
static int
label_manager_release_label_chunk (struct zserv *client, uint32_t start,
                                   uint32_t end)
{
 ...
 */
int
release_label_chunk (uint8_t proto, unsigned short instance,
                     uint32_t session_id, uint32_t start, uint32_t end)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (lbl_mgr.lc_list, node, lmc))
    {
      ...
      if (lmc->proto != proto || lmc->instance != instance ||
          lmc->session_id != session_id)
        {
          flog_err (EC_ZEBRA_LM_DAEMON_MISMATCH, "%s: Daemon mismatch!!",
                    __func__);
          continue;
        }
      lmc->proto = NO_PROTO;
  ...
  if (ret != 0)
    flog_err (EC_ZEBRA_LM_UNRELEASED_CHUNK, "%s: Label chunk not released!!",
              __func__);
...
static int
label_manager_connect (struct zserv *client, vrf_id_t vrf_id)
{
  /*
...
static int
label_manager_disconnect (struct zserv *client)
{
  release_daemon_label_chunks (client);
  return 0;
}
static int
label_manager_get_chunk (struct label_manager_chunk **lmc,
...
int
lm_client_connect_response (uint8_t proto, uint16_t instance,
                            ...
                            uint8_t result)
{
  ...
  if (! client)
    {
      zlog_err (
          "%s: could not find client for daemon %s instance %u session %u",
          __func__, zebra_route_string (proto), instance, session_id);
      return 1;
    }
...
int
lm_get_chunk_response (struct label_manager_chunk *lmc, struct zserv *client,
                       vrf_id_t vrf_id)
{
  if (! lmc)
    ...
    zlog_debug ("Assigned Label Chunk %u - %u to %s instance %u", lmc->start,
                lmc->end, zebra_route_string (client->proto),
                client->instance);
