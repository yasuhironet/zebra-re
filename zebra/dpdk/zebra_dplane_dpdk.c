...
void
zd_dpdk_stat_show (struct vty *vty)
{
  uint32_t tmp_cnt;
...
#define ZD_DPDK_SHOW_COUNTER(label, counter)                                  \
  do                                                                          \
    {                                                                         \
      tmp_cnt = atomic_load_explicit (&counter, memory_order_relaxed);        \
      vty_out (vty, "%28s: %u\n", (label), (tmp_cnt));                        \
    }                                                                         \
  while (0)
...
static void
zd_dpdk_flow_stat_show (struct vty *vty, int in_ifindex, intptr_t dp_flow_ptr)
{
  ...
  if (rc)
    {
      vty_out (vty, "PBR dpdk flow query failed; in_ifindex %d rc %d\n",
...
static int
zd_dpdk_pbr_show_rules_walkcb (struct hash_bucket *bucket, void *arg)
{
...
void
zd_dpdk_pbr_flows_show (struct vty *vty)
{
...
static void
zd_dpdk_rule_add (struct zebra_dplane_ctx *ctx)
{
  ...
  frr_with_privs (&zserv_privs)
    {
      flow =
          rte_flow_create (in_dport->port_id, &attrs, items, actions, &error);
    }
  ...
  if (flow)
    {
      ...
      if (IS_ZEBRA_DEBUG_DPLANE_DPDK_DETAIL)
        zlog_debug ("PBR dpdk flow 0x%" PRIxPTR
                    " created ifname %s seq %d pri %u unique %d\n",
                    (intptr_t) flow, dplane_ctx_rule_get_ifname (ctx), seq,
                    pri, unique);
    }
  else
    {
...
static void
zd_dpdk_rule_del (struct zebra_dplane_ctx *ctx, const char *ifname,
...
static void
zd_dpdk_rule_update (struct zebra_dplane_ctx *ctx)
{
  enum dplane_op_e op;
  int in_ifindex;
  ...
  switch (op)
    ...
    case DPLANE_OP_RULE_UPDATE:
      ...
      zd_dpdk_rule_del (ctx, dplane_ctx_rule_get_ifname (ctx), in_ifindex,
                        dp_flow_ptr);
      zd_dpdk_rule_add (ctx);
      break;
 ...
 */
static void
zd_dpdk_process_update (struct zebra_dplane_ctx *ctx)
{
...
static int
zd_dpdk_process (struct zebra_dplane_provider *prov)
{
...
static void
zd_dpdk_port_show_entry (struct zd_dpdk_port *dport, struct vty *vty,
                         int detail)
{
  ...
  if (detail)
    {
      ...
      vty_out (vty, " Switch: %s Domain: %u Port: %u\n",
               dev_info->switch_info.name, dev_info->switch_info.domain_id,
  ...
  else
    {
      vty_out (vty, "%-4u %-16s %-16s %-16d %s,%u,%u\n", dport->port_id,
               ...
               dev_info->switch_info.domain_id, dev_info->switch_info.port_id);
    }
}


static struct zd_dpdk_port *
...
void
zd_dpdk_port_show (struct vty *vty, uint16_t port_id, bool uj, int detail)
{
  int count;
  struct zd_dpdk_port *dport;
...
zd_dpdk_port_init (void)
{
  ...
  dpdk_ctx->dpdk_ports = XCALLOC (
      MTYPE_DPDK_PORTS, sizeof (struct zd_dpdk_port) * RTE_MAX_ETHPORTS);
  ...
  RTE_ETH_FOREACH_DEV (port_id)
  {
    ...
    if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
      zlog_debug (
          ...
          port_id, dev_info->device ? dev_info->device->name : "-",
          dev_info->if_index, dev_info->switch_info.name,
          dev_info->switch_info.domain_id, dev_info->switch_info.port_id);
    if (rte_flow_isolate (port_id, 1, &error))
      {
        if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
          zlog_debug ("Flow isolate on port %u failed %d\n", port_id,
                      error.type);
      }
    else
      {
...
static int
zd_dpdk_start (struct zebra_dplane_provider *prov)
{
  if (IS_ZEBRA_DEBUG_DPLANE_DPDK)
...
static int
zd_dpdk_finish (struct zebra_dplane_provider *prov, bool early)
{
  int rc;
...
static int
zd_dpdk_plugin_init (struct event_loop *tm)
{
  int ret;
