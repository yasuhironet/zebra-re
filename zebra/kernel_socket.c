...
size_t
_rta_get (caddr_t sap, void *destp, size_t destlen, bool checkaf)
{
  ...
  if (copylen > 0 && dest != NULL)
    {
      ...
      if (copylen > destlen)
        {
          zlog_warn ("%s: destination buffer too small (%zu vs %zu)", __func__,
                     copylen, destlen);
...
size_t
rta_get (caddr_t sap, void *destp, size_t destlen)
{
  return _rta_get (sap, destp, destlen, true);
}

size_t
rta_getattr (caddr_t sap, void *destp, size_t destlen)
{
  return _rta_get (sap, destp, destlen, false);
}

size_t
rta_getsdlname (caddr_t sap, void *destp, short *destlen)
{
  ...
  if (copylen > 0 && dest != NULL && sdl->sdl_family == AF_LINK)
    {
      if (copylen > IFNAMSIZ)
        {
          zlog_warn ("%s: destination buffer too small (%zu vs %d)", __func__,
                     copylen, IFNAMSIZ);
...
const char *
rtatostr (unsigned int flags, char *buf, size_t buflen)
{
...
ifan_read (struct if_announcemsghdr *ifan)
{
  ...
  if ((ifp == NULL) || ((ifp->ifindex == IFINDEX_INTERNAL) &&
                        (ifan->ifan_what == IFAN_ARRIVAL)))
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: creating interface for ifindex %d, name %s", __func__,
                    ifan->ifan_index, ifan->ifan_name);

      /* Create Interface */
      ifp = if_get_by_name (ifan->ifan_name, VRF_DEFAULT, VRF_DEFAULT_NAME);
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s: interface %s index %d", __func__, ifan->ifan_name,
                ifan->ifan_index);
  ...
  if (ifm->ifm_msglen < sizeof (struct if_msghdr))
    {
      flog_err (EC_ZEBRA_NETLINK_LENGTH_ERROR,
                "%s: ifm->ifm_msglen %d too short", __func__, ifm->ifm_msglen);
      return -1;
    }
  ...
  if ((ifp = if_lookup_by_index (ifm->ifm_index, VRF_DEFAULT)) != NULL)
    {
      ...
      if (ifnlen && (strncmp (ifp->name, ifname, IFNAMSIZ) != 0))
        {
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("%s: ifp name %s doesn't match sdl name %s", __func__,
                        ifp->name, ifname);
          ifp = NULL;
        }
    }
      ...
      if (ifp == NULL)
        {
          ...
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("%s: creating ifp for ifindex %d", __func__,
                        ifm->ifm_index);
        }

      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: updated/created ifp, ifname %s, ifindex %d", __func__,
...
static void
ifam_read_mesg (struct ifa_msghdr *ifm, union sockunion *addr,
                union sockunion *mask, union sockunion *brd, char *ifname,
                short *ifnlen)
{
  caddr_t pnt, end;
  union sockunion dst;
  ...
  for (maskbit = 1; maskbit; maskbit <<= 1)
    {
      ...
      if (pnt > end)
        {
          zlog_warn ("%s: overflow detected (pnt:%p end:%p)", __func__, pnt,
                     end);
          break;
        }
    }
        ...
        case AF_INET6:
          {
            ...
            zlog_debug (
                ...
                __func__, ifm->ifam_index, (ifnlen ? ifname : "(nil)"),
                rtatostr (ifm->ifam_addrs, fbuf, sizeof (fbuf)),
                ifm->ifam_flags, addr, masklen, brd, &dst, &gateway);
          }
          break;
        default:
          zlog_debug ("%s: ifindex %d, ifname %s, ifam_addrs {%s}", __func__,
...
ifam_read (struct ifa_msghdr *ifam)
{
  ...
  if ((ifp = if_lookup_by_index (ifam->ifam_index, VRF_DEFAULT)) == NULL)
    {
      flog_warn (EC_ZEBRA_UNKNOWN_INTERFACE,
                 "%s: no interface for ifname %s, index %d", __func__, ifname,
                 ifam->ifam_index);
      return -1;
    }
  ...
  switch (sockunion_family (&addr))
    ...
    case AF_INET6:
      ...
      if (ifam->ifam_type == RTM_NEWADDR)
        connected_add_ipv6 (ifp, flags, &addr.sin6.sin6_addr, NULL,
                            ip6_masklen (mask.sin6.sin6_addr),
                            (isalias ? ifname : NULL), METRIC_MAX);
      else
...
static int
rtm_read_mesg (struct rt_msghdr *rtm, union sockunion *dest,
               union sockunion *mask, union sockunion *gate, char *ifname,
               short *ifnlen)
{
  caddr_t pnt, end;
  int maskbit;
...
rtm_read (struct rt_msghdr *rtm)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s: got rtm of type %d (%s) addrs {%s}", __func__,
                rtm->rtm_type, lookup_msg (rtm_type_str, rtm->rtm_type, NULL),
  ...
  if (dest.sa.sa_family == AF_INET)
  ...
  else if (dest.sa.sa_family == AF_INET6)
    {
      ...
      if (! nh.type)
        {
          nh.type = ifindex ? NEXTHOP_TYPE_IPV6_IFINDEX : NEXTHOP_TYPE_IPV6;
  ...
  if (rtm->rtm_type == RTM_GET || rtm->rtm_type == RTM_ADD ||
    ...
    rib_add (afi, SAFI_UNICAST, VRF_DEFAULT, proto, 0, zebra_flags, &p, NULL,
             &nh, 0, RT_TABLE_MAIN, 0, 0, distance, 0, false);
  else
    rib_delete (afi, SAFI_UNICAST, VRF_DEFAULT, proto, 0, zebra_flags, &p,
                NULL, &nh, 0, RT_TABLE_MAIN, 0, distance, true);
}
 ...
 */
int
rtm_write (int message, union sockunion *dest, union sockunion *mask,
      ...
      if (! ifp)
        {
          ...
          if (dest)
            inet_ntop (AF_INET, &dest->sin.sin_addr, dest_buf,
                       INET_ADDRSTRLEN);
          if (mask)
            inet_ntop (AF_INET, &mask->sin.sin_addr, mask_buf,
                       INET_ADDRSTRLEN);
          flog_warn (
...
kernel_read (struct event *thread)
{
      ...
      if (errno == ENOBUFS)
        {
          ...
          return;
#else
          flog_err (EC_ZEBRA_RECVMSG_OVERRUN, "routing socket overrun: %s",
                    safe_strerror (errno));
          /*
  ...
  if (rtm->rtm_msglen != nbytes)
    {
      zlog_debug ("%s: rtm->rtm_msglen %d, nbytes %d, type %d", __func__,
...
routing_socket (struct zebra_ns *zns)
{
  ...
  if (setsockopt (routing_sock, SOL_SOCKET, SO_RERROR, &n, sizeof (n)) < 0)
    flog_err_sys (EC_LIB_SOCKET, "Can't set SO_RERROR on routing socket");
#endif
  ...
  if (getsockopt (routing_sock, SOL_SOCKET, SO_RCVBUF, &default_rcvbuf,
                  &optlen) == -1)
    flog_err_sys (EC_LIB_SOCKET, "routing_sock sockopt SOL_SOCKET SO_RCVBUF");
  else
    {
      for (; rcvbufsize > default_rcvbuf &&
             setsockopt (routing_sock, SOL_SOCKET, SO_RCVBUF, &rcvbufsize,
                         sizeof (rcvbufsize)) == -1 &&
             errno == ENOBUFS;
           rcvbufsize /= 2)
        ;
