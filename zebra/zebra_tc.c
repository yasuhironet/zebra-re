...
static uint32_t
lookup_key (const struct message *mz, const char *msg, uint32_t nf)
{
...
const char *
tc_qdisc_kind2str (uint32_t type)
{
...
uint32_t
zebra_tc_qdisc_hash_key (const void *arg)
{
...
bool
zebra_tc_qdisc_hash_equal (const void *arg1, const void *arg2)
{
...
static int
tc_qdisc_lookup_ifindex_walker (struct hash_bucket *b, void *data)
{
...
static void *
tc_qdisc_alloc_intern (void *arg)
{
...
static struct zebra_tc_qdisc *
tc_qdisc_free (struct zebra_tc_qdisc *hash_data, bool free_data)
{
...
static struct zebra_tc_qdisc *
tc_qdisc_release (struct zebra_tc_qdisc *qdisc, bool free_data)
{
...
void
zebra_tc_qdisc_install (struct zebra_tc_qdisc *qdisc)
{
  if (IS_ZEBRA_DEBUG_TC)
    zlog_debug ("%s: install tc qdisc ifindex %d kind %s", __func__,
                qdisc->qdisc.ifindex, tc_qdisc_kind2str (qdisc->qdisc.kind));
...
void
zebra_tc_qdisc_uninstall (struct zebra_tc_qdisc *qdisc)
{
  if (IS_ZEBRA_DEBUG_TC)
    zlog_debug ("%s: uninstall tc qdisc ifindex %d kind %s", __func__,
                qdisc->qdisc.ifindex, tc_qdisc_kind2str (qdisc->qdisc.kind));

  (void) dplane_tc_qdisc_uninstall (qdisc);

  if (tc_qdisc_release (qdisc, true))
    zlog_debug ("%s: tc qdisc being deleted we know nothing about", __func__);
}

uint32_t
zebra_tc_class_hash_key (const void *arg)
{
...
bool
zebra_tc_class_hash_equal (const void *arg1, const void *arg2)
{
...
static void *
tc_class_alloc_intern (void *arg)
{
...
static struct zebra_tc_class *
tc_class_free (struct zebra_tc_class *hash_data, bool free_data)
{
...
static struct zebra_tc_class *
tc_class_release (struct zebra_tc_class *class, bool free_data)
{
...
void
zebra_tc_class_add (struct zebra_tc_class *class)
{
  if (IS_ZEBRA_DEBUG_TC)
...
void
zebra_tc_class_delete (struct zebra_tc_class *class)
{
  if (IS_ZEBRA_DEBUG_TC)
  ...
  if (tc_class_release (class, true))
    zlog_debug ("%s: tc class being deleted we know nothing about", __func__);
}

const char *
tc_filter_kind2str (uint32_t type)
{
...
uint32_t
zebra_tc_filter_hash_key (const void *arg)
{
...
bool
zebra_tc_filter_hash_equal (const void *arg1, const void *arg2)
{
...
static struct zebra_tc_filter *
tc_filter_free (struct zebra_tc_filter *hash_data, bool free_data)
{
...
static struct zebra_tc_filter *
tc_filter_release (struct zebra_tc_filter *filter, bool free_data)
{
...
static void *
tc_filter_alloc_intern (void *arg)
{
...
void
zebra_tc_filter_add (struct zebra_tc_filter *filter)
{
  if (IS_ZEBRA_DEBUG_TC)
    zlog_debug ("%s: add tc filter ifindex %d priority %u handle %08x kind %s",
                __func__, filter->filter.ifindex, filter->filter.priority,
...
void
zebra_tc_filter_delete (struct zebra_tc_filter *filter)
{
  if (IS_ZEBRA_DEBUG_PBR)
    zlog_debug (
        "%s: delete tc filter ifindex %d priority %u handle %08x kind %s",
        __func__, filter->filter.ifindex, filter->filter.priority,
        filter->filter.handle, tc_filter_kind2str (filter->filter.kind));

  (void) dplane_tc_filter_delete (filter);

  if (tc_filter_release (filter, true))
    zlog_debug ("%s: tc filter being deleted we know nothing about", __func__);
}
