...
int
if_ioctl (unsigned long request, caddr_t buffer)
{
  int sock;
  int ret;
  int err = 0;
...
/* call ioctl system call */
int
vrf_if_ioctl (unsigned long request, caddr_t buffer, vrf_id_t vrf_id)
{
  int sock;
  int ret;
  int err = 0;
...
#ifndef HAVE_NETLINK
static int
if_ioctl_ipv6 (unsigned long request, caddr_t buffer)
{
  int sock;
  int ret;
  int err = 0;
...
if_get_mtu (struct interface *ifp)
{
  ...
  if (vrf_if_ioctl (SIOCGIFMTU, (caddr_t) &ifreq, ifp->vrf->vrf_id) < 0)
    {
      zlog_info ("Can't lookup mtu by ioctl(SIOCGIFMTU) for %s(%u)", ifp->name,
                 ifp->vrf->vrf_id);
      ifp->mtu6 = ifp->mtu = -1;
      return;
    }
...
kernel_address_update_ctx (struct zebra_dplane_ctx *ctx)
{
  int ret = -1;
 ...
 */
static int
if_set_prefix_ctx (const struct zebra_dplane_ctx *ctx)
{
  int ret;
 ...
 */
static int
if_unset_prefix_ctx (const struct zebra_dplane_ctx *ctx)
{
  int ret;
  struct ifaliasreq addreq;
  struct sockaddr_in addr, mask, peer;
...
if_get_flags (struct interface *ifp)
{
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL,
                    "vrf_if_ioctl(SIOCGIFFLAGS %s) failed: %s", ifp->name,
                    safe_strerror (errno));
      return;
    }
  ...
  if (ret == -1)
    ...
    flog_err_sys (EC_LIB_SYSTEM_CALL, "if_ioctl(SIOCGIFDATA %s) failed: %s",
  ...
  if (if_ioctl (SIOCGIFMEDIA, (caddr_t) &ifmr) == -1)
    {
      if (errno != EINVAL)
        flog_err_sys (EC_LIB_SYSTEM_CALL,
                      "if_ioctl(SIOCGIFMEDIA %s) failed: %s", ifp->name,
                      safe_strerror (errno));
    }
...
if_set_flags (struct interface *ifp, uint64_t flags)
{
  ...
  if (ret < 0)
    {
      zlog_info ("can't set interface %s(%u) flags %" PRIu64, ifp->name,
                 ifp->vrf->vrf_id, flags);
      return ret;
    }
  return 0;
}
...
if_unset_flags (struct interface *ifp, uint64_t flags)
{
  ...
  if (ret < 0)
    {
      zlog_warn ("can't unset interface %s(%u) flags %" PRIu64, ifp->name,
                 ifp->vrf->vrf_id, flags);
      return ret;
    }
  return 0;
 ...
 */
static int
if_set_prefix6_ctx (const struct zebra_dplane_ctx *ctx)
{
  int ret;
 ...
 */
static int
if_unset_prefix6_ctx (const struct zebra_dplane_ctx *ctx)
{
  int ret;
  struct in6_aliasreq addreq;
  struct sockaddr_in6 addr;
