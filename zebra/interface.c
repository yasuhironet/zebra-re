...
static void
if_zebra_speed_update (struct event *thread)
{
  ...
  if (new_speed != ifp->speed)
    {
      zlog_info ("%s: %s old speed: %u new speed: %u", __func__, ifp->name,
                 ifp->speed, new_speed);
      ifp->speed = new_speed;
      if_add_update (ifp);
      changed = true;
...
static void
zebra_if_nhg_dependents_free (struct zebra_if *zebra_if)
{
  nhg_connected_tree_free (&zebra_if->nhg_dependents);
}

static void
zebra_if_nhg_dependents_init (struct zebra_if *zebra_if)
{
  nhg_connected_tree_init (&zebra_if->nhg_dependents);
}


route_table_delegate_t zebra_if_table_delegate = { .create_node =
                                                       route_node_create,
                                                   .destroy_node =
...
static void
if_nhg_dependents_check_valid (struct nhg_hash_entry *nhe)
{
  zebra_nhg_check_valid (nhe);
}

static void
if_down_nhg_dependents (const struct interface *ifp)
{
...
static void
if_nhg_dependents_release (const struct interface *ifp)
{
  struct nhg_connected *rb_node_dep = NULL;
  struct zebra_if *zif = (struct zebra_if *) ifp->info;
  ...
  if (ifp->info)
    {
      ...
      if (ZEBRA_IF_IS_PROTODOWN (zebra_if) && zebra_if->protodown_rc &&
        ...
        zebra_if_update_protodown_rc (
            ifp, true, (zebra_if->protodown_rc & ~ZEBRA_PROTODOWN_ALL));
...
/* Build the table key */
static void
if_build_key (uint32_t ifindex, struct prefix *p)
{
  p->family = AF_INET;
...
struct interface *
if_lookup_by_index_per_ns (struct zebra_ns *ns, uint32_t ifindex)
{
  struct prefix p;
  struct route_node *rn;
...
struct interface *
if_lookup_by_index_per_nsid (ns_id_t ns_id, uint32_t ifindex)
{
  struct zebra_ns *zns;
          ...
          if (p->family == AF_INET)
            {
              ...
              if (dplane_res == ZEBRA_DPLANE_REQUEST_FAILURE)
                {
                  flog_err_sys (EC_ZEBRA_IFACE_ADDR_ADD_FAILED,
  ...
  if (! CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_ACTIVE))
    {
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("interface %s vrf %s(%u) index %d becomes active.",
                    ifp->name, ifp->vrf->name, ifp->vrf->vrf_id, ifp->ifindex);
    }
  else
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("interface %s vrf %s(%u) index %d is added.", ifp->name,
                    ifp->vrf->name, ifp->vrf->vrf_id, ifp->ifindex);
    }
}
...
void
if_delete_update (struct interface **pifp)
{
  struct zebra_if *zif;
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("interface %s vrf %s(%u) index %d is now inactive.", ifp->name,
                ifp->vrf->name, ifp->vrf->vrf_id, ifp->ifindex);
...
static void
ipv6_ll_address_to_mac (struct in6_addr *address, uint8_t *mac)
{
...
void
if_nbr_mac_to_ipv4ll_neigh_update (struct interface *ifp, char mac[6],
                                   struct in6_addr *address, int add)
{
   ...
   */
  if (add)
    kernel_neigh_update (add, ifp->ifindex, (void *) &ipv4_ll.s_addr, mac, 6,
                         ns_id, AF_INET, true);
...
void
if_nhg_dependents_add (struct interface *ifp, struct nhg_hash_entry *nhe)
{
  if (ifp->info)
    {
      struct zebra_if *zif = (struct zebra_if *) ifp->info;
...
void
if_nhg_dependents_del (struct interface *ifp, struct nhg_hash_entry *nhe)
{
  if (ifp->info)
    {
      struct zebra_if *zif = (struct zebra_if *) ifp->info;
...
unsigned int
if_nhg_dependents_count (const struct interface *ifp)
{
  if (ifp->info)
    {
      struct zebra_if *zif = (struct zebra_if *) ifp->info;
...
bool
if_nhg_dependents_is_empty (const struct interface *ifp)
{
  if (ifp->info)
    {
      struct zebra_if *zif = (struct zebra_if *) ifp->info;
...
/* Interface is up. */
void
if_up (struct interface *ifp, bool install_connected)
{
  struct zebra_if *zif;
  ...
  if (ifp->ptm_enable && (ifp->ptm_status == ZEBRA_PTM_STATUS_DOWN))
    {
      flog_warn (EC_ZEBRA_PTM_NOT_READY,
                 "%s: interface %s hasn't passed ptm check", __func__,
                 ifp->name);
      return;
    }
                      ...
                      ns_id_t ns_id)
{
  ...
  zif->link =
      if_lookup_by_index_per_ns (zebra_ns_lookup (ns_id), link_ifindex);
}
 ...
 */
void
zebra_if_update_all_links (struct zebra_ns *zns)
{
  struct route_node *rn;
  struct interface *ifp;
  struct zebra_if *zif;
  ...
  for (rn = route_top (zns->if_table); rn; rn = route_next (rn))
    {
      ...
      if ((zif->link_ifindex != IFINDEX_INTERNAL) && ! zif->link)
        {
          zif->link =
              if_lookup_by_index_per_nsid (zif->link_nsid, zif->link_ifindex);
          if (IS_ZEBRA_DEBUG_KERNEL)
...
static bool
if_ignore_set_protodown (const struct interface *ifp, bool new_down,
      ...
      if (new_down == old_down)
        {
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug (
                ...
                new_down ? "on" : "off", zif->protodown_rc, new_protodown_rc);
...
int
zebra_if_update_protodown_rc (struct interface *ifp, bool new_down,
                              uint32_t new_protodown_rc)
{
  struct zebra_if *zif;
  ...
  zlog_info (
      ...
      new_down ? "on" : "off", ifp->name, ifp->ifindex, zif->protodown_rc,
      new_protodown_rc);
...
int
zebra_if_set_protodown (struct interface *ifp, bool new_down,
 ...
 */
static void
zebra_if_addr_update_ctx (struct zebra_dplane_ctx *ctx, struct interface *ifp)
{
  uint8_t flags = 0;
  ...
  if (addr->family == AF_INET)
    {
      if (op == DPLANE_OP_INTF_ADDR_ADD)
        connected_add_ipv4 (ifp, flags, &addr->u.prefix4, addr->prefixlen,
                            dest ? &dest->u.prefix4 : NULL, label, metric);
      else if (CHECK_FLAG (flags, ZEBRA_IFA_PEER))
        {
          /* Delete with a peer address */
          connected_delete_ipv4 (ifp, flags, &addr->u.prefix4, addr->prefixlen,
                                 &dest->u.prefix4);
        }
      else
        connected_delete_ipv4 (ifp, flags, &addr->u.prefix4, addr->prefixlen,
      ...
      if (op == DPLANE_OP_INTF_ADDR_ADD)
        {
          connected_add_ipv6 (ifp, flags, &addr->u.prefix6,
                              dest ? &dest->u.prefix6 : NULL, addr->prefixlen,
                              label, metric);
        }
      else
        connected_delete_ipv6 (ifp, &addr->u.prefix6, NULL, addr->prefixlen);
    }
...
static void
zebra_if_update_ctx (struct zebra_dplane_ctx *ctx, struct interface *ifp)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("%s: %s: if %s(%u) ctx-protodown %s ctx-reason %d", __func__,
                dplane_op2str (dplane_ctx_get_op (ctx)), ifp->name,
  ...
  if (! zif)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: if %s(%u) zebra info pointer is NULL", __func__,
  ...
  if (dp_res != ZEBRA_DPLANE_REQUEST_SUCCESS)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: if %s(%u) dplane update failed", __func__, ifp->name,
                    ifp->ifindex);
      goto done;
    }
 ...
 */
static void
zebra_if_netconf_update_ctx (struct zebra_dplane_ctx *ctx,
                             struct interface *ifp, ifindex_t ifindex)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug (
        ...
        (*mcast_set ? "ON" : "OFF"), (*linkdown_set ? "ON" : "OFF"));
}

void
zebra_if_dplane_result (struct zebra_dplane_ctx *ctx)
{
  struct zebra_ns *zns;
  struct interface *ifp;
  ns_id_t ns_id;
  ...
  if (IS_ZEBRA_DEBUG_DPLANE_DETAIL || IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("Intf dplane ctx %p, op %s, ifindex (%u), result %s", ctx,
                dplane_op2str (op), ifindex, dplane_res2str (dp_res));
  ...
  if (ifp == NULL)
    {
      if (op != DPLANE_OP_INTF_NETCONFIG || (ifindex != -1 && ifindex != -2))
        {
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("%s: can't find ifp at nsid %u index %d", __func__,
                        ns_id, ifindex);
...
static void
connected_dump_vty (struct vty *vty, json_object *json,
                    struct connected *connected)
{
  ...
  if (connected->label)
    {
      if (json)
        json_object_string_add (json_addr, "label", connected->label);
      else
...
static void
nbr_connected_dump_vty (struct vty *vty, json_object *json,
...
static const char *
zebra_ziftype_2str (enum zebra_iftype zif_type)
{
  switch (zif_type)
    {
    case ZEBRA_IF_OTHER:
...
static void
ifs_dump_brief_vty (struct vty *vty, struct vrf *vrf)
{
  struct connected *connected;
  struct listnode *node;
  struct route_node *rn;
...
static void
ifs_dump_brief_vty_json (json_object *json, struct vrf *vrf)
{
  struct connected *connected;
  struct listnode *node;
  struct interface *ifp;
...
const char *
zebra_protodown_rc_str (uint32_t protodown_rc, char *pd_buf,
                        uint32_t pd_buf_len)
{
  pd_buf[0] = '\0';
  size_t len;
...
static inline bool
if_is_protodown_applicable (struct interface *ifp)
{
...
static void
zebra_vxlan_if_vni_dump_vty (struct vty *vty, struct zebra_vxlan_vni *vni)
{
...
static void
zebra_vxlan_if_vni_hash_dump_vty (struct hash_bucket *bucket, void *ctxt)
{
  struct vty *vty;
...
static void
zebra_vxlan_if_dump_vty (struct vty *vty, struct zebra_if *zebra_if)
{
  ...
  if (vxlan_info->ifindex_link && (vxlan_info->link_nsid != NS_UNKNOWN))
    {
      ...
      ifp = if_lookup_by_index_per_ns (zebra_ns_lookup (vxlan_info->link_nsid),
                                       vxlan_info->ifindex_link);
      vty_out (vty, " Link Interface %s", ifp == NULL ? "Unknown" : ifp->name);
    }
  ...
  if (IS_ZEBRA_VXLAN_IF_VNI (zebra_if))
    ...
    {
      hash_iterate (vni_info->vni_table, zebra_vxlan_if_vni_hash_dump_vty,
                    vty);
    }
...
if_dump_vty (struct vty *vty, struct interface *ifp)
{
      ...
      if (gre_info->vtep_ip.s_addr != INADDR_ANY)
        {
          ...
          if (gre_info->vtep_ip_remote.s_addr != INADDR_ANY)
            vty_out (vty, " , remote %pI4", &gre_info->vtep_ip_remote);
          vty_out (vty, "\n");
        }
      ...
      if (IS_PARAM_SET (iflp, LP_UNRSV_BW))
        {
          ...
          for (i = 0; i < MAX_CLASS_TYPE; i += 2)
            vty_out (vty,
                     ...
                     iflp->unrsv_bw[i], i + 1, iflp->unrsv_bw[i + 1]);
        }
  ...
  vty_out (vty,
           ...
           ifp->stats.rx_packets, ifp->stats.rx_multicast, ifp->stats.rx_bytes,
  ...
  vty_out (vty, "    %lu output packets, %lu bytes, %lu dropped\n",
           ifp->stats.tx_packets, ifp->stats.tx_bytes, ifp->stats.tx_dropped);
...
static void
zebra_vxlan_if_vni_dump_vty_json (json_object *json_if,
                                  struct zebra_vxlan_vni *vni)
{
  ...
  if (vni->mcast_grp.s_addr != INADDR_ANY)
    json_object_string_addf (json_if, "mcastGroup", "%pI4", &vni->mcast_grp);
}

static void
zebra_vxlan_if_vni_hash_dump_vty_json (struct hash_bucket *bucket, void *ctxt)
{
  json_object *json_if;
  struct zebra_vxlan_vni *vni;
...
static void
zebra_vxlan_if_dump_vty_json (json_object *json_if, struct zebra_if *zebra_if)
{
  struct zebra_l2info_vxlan *vxlan_info;
  struct zebra_vxlan_vni_info *vni_info;
  ...
  if (vxlan_info->vtep_ip.s_addr != INADDR_ANY)
    json_object_string_addf (json_if, "vtepIp", "%pI4", &vxlan_info->vtep_ip);
  ...
  if (IS_ZEBRA_VXLAN_IF_VNI (zebra_if))
    ...
    {
      hash_iterate (vni_info->vni_table, zebra_vxlan_if_vni_hash_dump_vty_json,
                    json_if);
    }
}

static void
if_dump_vty_json (struct vty *vty, struct interface *ifp, json_object *json)
{
  struct connected *connected;
  struct nbr_connected *nbr_connected;
  struct listnode *node;
      ...
      if (CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_LINKDETECTION))
        {
          json_object_string_add (json_if, "operationalStatus",
                                  if_is_running (ifp) ? "up" : "down");
          json_object_boolean_add (json_if, "linkDetection", true);
        }
      else
        {
          json_object_boolean_add (json_if, "linkDetection", false);
        }
    }
  else
    {
  ...
  if (zebra_if->up_last[0])
    json_object_string_add (json_if, "lastLinkUp", zebra_if->up_last);
  if (zebra_if->down_last[0])
    json_object_string_add (json_if, "lastLinkDown", zebra_if->down_last);
  ...
  if (zebra_if->desc)
    json_object_string_add (json_if, "OsDescription", zebra_if->desc);
  ...
  json_object_string_add (json_if, "interfaceType",
                          zebra_ziftype_2str (zebra_if->zif_type));
  json_object_string_add (json_if, "interfaceSlaveType",
                          zebra_zifslavetype_2str (zebra_if->zif_slave_type));
  ...
  if (IS_ZEBRA_IF_BRIDGE_SLAVE (ifp))
    {
      ...
      if (br_slave->bridge_ifindex != IFINDEX_INTERNAL)
        {
          if (br_slave->br_if)
            json_object_string_add (json_if, "masterInterface",
                                    br_slave->br_if->name);
          else
  ...
  json_object_boolean_add (json_if, "lacpBypass",
  ...
  if (if_is_protodown_applicable (ifp))
    {
      json_object_string_add (json_if, "protodown",
                              (ZEBRA_IF_IS_PROTODOWN (zebra_if)) ? "on"
                                                                 : "off");
      if (zebra_if->protodown_rc)
        json_object_string_add (json_if, "protodownReason",
  ...
  if (zebra_if->link_ifindex != IFINDEX_INTERNAL)
    {
      if (zebra_if->link)
        ...
        json_object_int_add (json_if, "parentIfindex", zebra_if->link_ifindex);
    }
  ...
  if (HAS_LINK_PARAMS (ifp))
    {
      ...
      json_object_object_add (json_if, "trafficEngineeringLinkParameters",
                              json_te);

      if (IS_PARAM_SET (iflp, LP_TE_METRIC))
        json_object_int_add (json_te, "teMetric", iflp->te_metric);
      if (IS_PARAM_SET (iflp, LP_MAX_BW))
        json_object_double_add (json_te, "maximumBandwidth", iflp->max_bw);
      if (IS_PARAM_SET (iflp, LP_MAX_RSV_BW))
        json_object_double_add (json_te, "maximumReservableBandwidth",
                                iflp->max_rsv_bw);
      if (IS_PARAM_SET (iflp, LP_UNRSV_BW))
        {
          ...
          json_object_object_add (json_te, "unreservedBandwidth", json_bws);
          for (unsigned int i = 0; i < MAX_CLASS_TYPE; ++i)
            {
              char buf_ct[64];

              snprintf (buf_ct, sizeof (buf_ct), "classType%u", i);
              json_object_double_add (json_bws, buf_ct, iflp->unrsv_bw[i]);
            }
        }

      if (IS_PARAM_SET (iflp, LP_ADM_GRP))
        json_object_int_add (json_te, "administrativeGroup", iflp->admin_grp);
      if (IS_PARAM_SET (iflp, LP_DELAY))
        {
          json_object_int_add (json_te, "linkDelayAverage", iflp->av_delay);
          if (IS_PARAM_SET (iflp, LP_MM_DELAY))
            {
      ...
      if (IS_PARAM_SET (iflp, LP_DELAY_VAR))
        json_object_int_add (json_te, "linkDelayVariation", iflp->delay_var);
      if (IS_PARAM_SET (iflp, LP_PKT_LOSS))
        json_object_double_add (json_te, "linkPacketLoss", iflp->pkt_loss);
      if (IS_PARAM_SET (iflp, LP_AVA_BW))
        json_object_double_add (json_te, "availableBandwidth", iflp->ava_bw);
      if (IS_PARAM_SET (iflp, LP_RES_BW))
        json_object_double_add (json_te, "residualBandwidth", iflp->res_bw);
      if (IS_PARAM_SET (iflp, LP_USE_BW))
        json_object_double_add (json_te, "utilizedBandwidth", iflp->use_bw);
      if (IS_PARAM_SET (iflp, LP_RMT_AS))
        json_object_string_addf (json_te, "neighborAsbrIp", "%pI4",
                                 &iflp->rmt_ip);
  ...
  if (listhead (ifp->nbr_connected))
    {
      ...
      json_object_object_add (json_if, "neighborIpAddresses", json_nbr_addrs);

      for (ALL_LIST_ELEMENTS_RO (ifp->nbr_connected, node, nbr_connected))
        nbr_connected_dump_vty (vty, json_nbr_addrs, nbr_connected);
    }
  ...
  json_object_int_add (json_if, "inputCrcErrors", ifp->stats.rx_crc_errors);
  json_object_int_add (json_if, "inputFrameErrors",
                       ifp->stats.rx_frame_errors);
  json_object_int_add (json_if, "inputFifoErrors", ifp->stats.rx_fifo_errors);
  ...
  json_object_int_add (json_if, "outputDroppedPackets", ifp->stats.tx_dropped);
  ...
  json_object_int_add (json_if, "outputFifoErrors", ifp->stats.tx_fifo_errors);
  ...
  if (count > 1)
    {
      if (uj)
        ...
        {
          vty_out (vty, "%% There are multiple interfaces with name %s\n",
                   ifname);
...
static void
if_show_description (struct vty *vty, struct vrf *vrf)
{
  struct interface *ifp;
...
int
if_multicast_set (struct interface *ifp)
{
  struct zebra_if *if_data;
  ...
  if (CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_ACTIVE))
    {
      if (if_set_flags (ifp, IFF_MULTICAST) < 0)
        {
          zlog_debug ("Can't set multicast flag on interface %s", ifp->name);
          return -1;
        }
      if_refresh (ifp);
    }
...
int
if_multicast_unset (struct interface *ifp)
{
  struct zebra_if *if_data;
  ...
  if (CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_ACTIVE))
    {
      if (if_unset_flags (ifp, IFF_MULTICAST) < 0)
        {
          zlog_debug ("Can't unset multicast flag on interface %s", ifp->name);
          return -1;
        }
      if_refresh (ifp);
    }
...
int
if_linkdetect (struct interface *ifp, bool detect)
{
...
int
if_shutdown (struct interface *ifp)
{
  struct zebra_if *if_data;
...
int
if_no_shutdown (struct interface *ifp)
{
  struct zebra_if *if_data;
...
static void
link_param_cmd_set_float (struct interface *ifp, float *field, uint32_t type,
                          float value)
{
       ...
       "Activate link parameters on this interface\n")
{
  ...
  if (IS_ZEBRA_DEBUG_EVENT || IS_ZEBRA_DEBUG_MPLS)
    zlog_debug ("Link-params: enable TE link parameters on interface %s",
                ifp->name);
            ...
            "32-bit Hexadecimal value (e.g. 0xa1)\n")
{
  ...
  if (yang_dnode_exists (running_config->dnode, xpath))
    {
      vty_out (vty, "cannot use the admin-grp command when affinity is set\n");
      return CMD_WARNING_CONFIG_FAILED;
    }
...
DEFPY_YANG (no_link_params_admin_grp,
  ...
  if (argc == 2)
  ...
  else
    {
      ...
      if (IS_PARAM_UNSET (iflp, LP_DELAY) ||
          IS_PARAM_UNSET (iflp, LP_MM_DELAY) || iflp->av_delay != delay ||
          iflp->min_delay != low || iflp->max_delay != high)
        {
          iflp->av_delay = delay;
          SET_PARAM (iflp, LP_DELAY);
          iflp->min_delay = low;
            ...
            "Affinity names\n")
{
  ...
  if (yang_dnode_exists (running_config->dnode, xpath))
    {
      vty_out (vty, "cannot use the affinity command when admin-grp is set\n");
      return CMD_WARNING_CONFIG_FAILED;
    }

  return ag_change (vty, argc, argv,
                    "./frr-zebra:zebra/link-params/affinities/affinity", no,
                    no ? 2 : 1);
}
...
DEFPY_YANG (no_link_params_affinity_mode,
...
void
cli_show_legacy_admin_group (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
...
void
cli_show_affinity_mode (struct vty *vty, const struct lyd_node *dnode,
                        bool show_defaults)
{
...
void
cli_show_affinity (struct vty *vty, const struct lyd_node *dnode,
                   bool show_defaults)
{
...
int
if_ip_address_install (struct interface *ifp, struct prefix *prefix,
                       const char *label, struct prefix *pp)
{
  ...
  if (! CHECK_FLAG (ifc->conf, ZEBRA_IFC_QUEUED) &&
      ...
      ! (if_data && if_data->shutdown == IF_ZEBRA_DATA_ON))
    {
      ...
      if (dplane_res == ZEBRA_DPLANE_REQUEST_FAILURE)
        {
          zlog_debug ("dplane can't set interface IP address: %s.",
                      dplane_res2str (dplane_res));
          return NB_ERR;
        }
...
int
if_ip_address_uinstall (struct interface *ifp, struct prefix *prefix)
{
       ...
       "IP Address (e.g. 10.0.0.1/8)\n")
{
  ...
  VTY_DECLVAR_CONTEXT (interface, ifp);
  return ip_address_uninstall (vty, ifp, argv[idx_ipv4_prefixlen]->arg, NULL,
                               NULL);
}
...
int
if_ipv6_address_install (struct interface *ifp, struct prefix *prefix,
                         const char *label)
{
  struct zebra_if *if_data;
       ...
       "IPv6 address (e.g. 3ffe:506::1/48)\n")
{
  ...
  VTY_DECLVAR_CONTEXT (interface, ifp);
  return ipv6_address_install (vty, ifp, argv[idx_ipv6_prefixlen]->arg, NULL,
                               NULL);
}
       ...
       "IPv6 address (e.g. 3ffe:506::1/48)\n")
{
  ...
  VTY_DECLVAR_CONTEXT (interface, ifp);
  return ipv6_address_uninstall (vty, ifp, argv[idx_ipv6_prefixlen]->arg, NULL,
                                 NULL);
}
            ...
            if (CHECK_FLAG (ifc->conf, ZEBRA_IFC_CONFIGURED))
              {
                ...
                p = ifc->address;
                vty_out (
                    vty, " ip%s address %s", p->family == AF_INET ? "" : "v6",
        ...
        if (if_data)
          {
            if (if_data->multicast != IF_ZEBRA_DATA_UNSPEC)
              vty_out (vty, " %smulticast\n",
                       if_data->multicast == IF_ZEBRA_DATA_ON ? "" : "no ");
