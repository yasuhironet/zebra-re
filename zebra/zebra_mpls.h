...
#define FEC_FLAG_CONFIGURED (1 << 0)
   ...
   */
  int zebra_mpls_write_label_block_config (struct vty *vty,
  ...
  struct zebra_nhlfe *zebra_mpls_lsp_add_nhlfe (
  ...
  /*
   * Add NHLFE or backup NHLFE to an LSP based on a nexthop. These just
   * maintain the LSP and NHLFE objects; nothing is scheduled for processing.
   ...
   */
  int zebra_mpls_label_already_bound (struct zebra_vrf *zvrf,
                                      mpls_label_t label);
  ...
  /*
   * Display MPLS FEC to label binding for a specific FEC (VTY command
   * handler).
   */
   ...
   */
  struct zebra_lsp *mpls_lsp_find (struct zebra_vrf *zvrf,
                                   mpls_label_t in_label);
   ...
   */
  int mpls_lsp_uninstall_all_vrf (struct zebra_vrf *zvrf,
                                  enum lsp_types_t type,
                                  mpls_label_t in_label);
   ...
   */
  static inline int
  re_type_from_lsp_type (enum lsp_types_t lsp_type)
  {
    switch (lsp_type)
      {
      case ZEBRA_LSP_STATIC:
  ...
  static inline void
  mpls_mark_lsps_for_processing (struct zebra_vrf *zvrf, struct prefix *p)
  {
  ...
  static inline void
  mpls_unmark_lsps_for_processing (struct route_node *rn)
  {
  ...
  static inline int
  mpls_should_lsps_be_processed (struct route_node *rn)
  {
    rib_dest_t *dest = rib_dest_from_rnode (rn);
  ...
  extern bool
      mpls_pw_reach_strict; /* Strict pseudowire reachability checking */
