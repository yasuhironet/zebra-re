...
static void
show_vrf_proto_rm (struct vty *vty, struct zebra_vrf *zvrf, int af_type)
{
  int i;
  ...
  if (PROTO_RM_NAME (zvrf, af_type, i))
    vty_out (vty, "%-24s  : %-10s\n", "any", PROTO_RM_NAME (zvrf, af_type, i));
  else
    vty_out (vty, "%-24s  : none\n", "any");
}

static void
show_vrf_nht_rm (struct vty *vty, struct zebra_vrf *zvrf, int af_type,
                 json_object *json)
{
  int i;
  ...
  if (json)
    {
      if (NHT_RM_NAME (zvrf, af_type, i))
        json_object_string_add (json, "any", NHT_RM_NAME (zvrf, af_type, i));
      else
...
static int
show_proto_rm (struct vty *vty, int af_type, const char *vrf_all,
               const char *vrf_name)
{
...
static int
show_nht_rm (struct vty *vty, int af_type, const char *vrf_all,
...
static int
ip_protocol_rm_add (struct zebra_vrf *zvrf, const char *rmap, int rtype,
  ...
  if (PROTO_RM_MAP (zvrf, afi, rtype))
    {
      ...
      if (table)
        rib_update_table (table, RIB_UPDATE_RMAP_CHANGE, rtype);
    }
...
static int
ip_protocol_rm_del (struct zebra_vrf *zvrf, const char *rmap, int rtype,
                    afi_t afi, safi_t safi)
{
  struct route_table *table;
...
static int
ip_nht_rm_add (struct zebra_vrf *zvrf, const char *rmap, int rtype, int afi)
{
...
static int
ip_nht_rm_del (struct zebra_vrf *zvrf, const char *rmap, int rtype, int afi)
{
...
DEFPY_YANG (match_ip_address_prefix_len,
...
DEFPY_YANG (no_match_ip_address_prefix_len,
...
DEFPY_YANG (match_ipv6_address_prefix_len,
...
DEFPY_YANG (match_ip_nexthop_prefix_len,
...
DEFPY_YANG (no_match_ip_nexthop_prefix_len,
...
DEFPY_YANG (match_source_instance,
...
DEFPY_YANG (no_match_source_instance,
...
DEFPY_YANG (set_src,
            ...
            "IPv6 src address\n")
{
  const char *xpath = "./set-action[action='frr-zebra-route-map:src-address']";
  char xpath_value[XPATH_MAXLEN];
...
DEFPY_YANG (ipv6_protocol,
            ipv6_protocol_cmd,
            "ipv6 protocol "
            FRR_IP6_PROTOCOL_MAP_STR_ZEBRA
            " $proto route-map ROUTE-MAP$rmap",
            IP6_STR
...
/* Route map `ip address' match statement.  `arg' should be
   access-list name. */
static void *
route_match_address_compile (const char *arg)
{
  return XSTRDUP (MTYPE_ROUTE_MAP_COMPILED, arg);
}
...
static void
route_match_address_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static void *
route_match_address_prefix_list_compile (const char *arg)
{
  return XSTRDUP (MTYPE_ROUTE_MAP_COMPILED, arg);
}

static void
route_match_address_prefix_list_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static void *
route_match_address_prefix_len_compile (const char *arg)
{
  uint32_t *prefix_len;
  char *endptr = NULL;
...
static void
route_match_address_prefix_len_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
route_match_source_protocol (void *rule, const struct prefix *p, void *object)
{
  ...
  return ((nh_data->source_protocol == *rib_type) ? RMAP_MATCH : RMAP_NOMATCH);
}
...
static void *
route_match_source_instance_compile (const char *arg)
{
  uint8_t *instance;
  int i;
...
static void
route_match_source_instance_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
 ...
 */
static void
zebra_rib_table_rm_update (const char *rmap)
{
  int i = 0;
  struct route_table *table;
  struct vrf *vrf = NULL;
      ...
      for (i = 0; i <= ZEBRA_ROUTE_MAX; i++)
        {
          ...
          if (rmap_name && (strcmp (rmap_name, rmap) == 0))
            {
              if (IS_ZEBRA_DEBUG_EVENT)
                zlog_debug ("%s : AFI_IP rmap %s, route type %s", __func__,
               ...
               */
              if (! old)
                route_map_counter_increment (PROTO_RM_MAP (zvrf, AFI_IP, i));
              /* There is single rib table for all protocols
               */
              table = zvrf->table[AFI_IP][SAFI_UNICAST];
              if (table)
                {
                  rib_update_table (table, RIB_UPDATE_RMAP_CHANGE, i);
                }
            }
          ...
          if (rmap_name && (strcmp (rmap_name, rmap) == 0))
            {
              if (IS_ZEBRA_DEBUG_EVENT)
                zlog_debug ("%s : AFI_IP6 rmap %s, route type %s", __func__,
                            rmap, zebra_route_string (i));
              ...
              PROTO_RM_MAP (zvrf, AFI_IP6, i) =
                  route_map_lookup_by_name (rmap_name);
              if (! old)
                route_map_counter_increment (PROTO_RM_MAP (zvrf, AFI_IP6, i));
              /* There is single rib table for all protocols
               */
 ...
 */
static void
zebra_nht_rm_update (const char *rmap)
{
  int i = 0;
  struct route_table *table;
  struct vrf *vrf = NULL;
          ...
          if (rmap_name && (strcmp (rmap_name, rmap) == 0))
            {
              ...
              NHT_RM_MAP (zvrf, AFI_IP, i) =
                  route_map_lookup_by_name (rmap_name);
              if (! old)
                route_map_counter_increment (NHT_RM_MAP (zvrf, AFI_IP, i));
              /* There is single rib table for all protocols
               */
              if (afi_ip == 0)
                {
          ...
          if (rmap_name && (strcmp (rmap_name, rmap) == 0))
            {
              ...
              NHT_RM_MAP (zvrf, AFI_IP6, i) =
                  route_map_lookup_by_name (rmap_name);
              if (! old)
                route_map_counter_increment (NHT_RM_MAP (zvrf, AFI_IP6, i));
              /* There is single rib table for all protocols
               */
              if (afi_ipv6 == 0)
                {
                  ...
                  if (table)
                    {
                      ...
                      zebra_evaluate_rnh (zvrf, AFI_IP6, 1, NULL,
                                          SAFI_UNICAST);
                    }
                }
            }
...
static void
zebra_route_map_process_update_cb (char *rmap_name)
{
  if (IS_ZEBRA_DEBUG_EVENT)
...
static void
zebra_route_map_update_timer (struct event *thread)
{
  if (IS_ZEBRA_DEBUG_EVENT)
...
static void
zebra_route_map_set_delay_timer (uint32_t value)
{
...
void
zebra_add_import_table_route_map (afi_t afi, const char *rmap_name,
                                  uint32_t table)
{
...
zebra_nht_route_map_check (afi_t afi, int client_proto, const struct prefix *p,
                           struct zebra_vrf *zvrf, struct route_entry *re,
                           struct nexthop *nexthop)
{
  struct route_map *rmap = NULL;
  route_map_result_t ret = RMAP_PERMITMATCH;
...
static void
zebra_route_map_event (const char *rmap_name)
{
  if (route_map_mark_updated (rmap_name) == 0)
    zebra_route_map_mark_update (rmap_name);
...
void
zebra_routemap_vrf_delete (struct zebra_vrf *zvrf)
{
  afi_t afi;
  uint8_t type;
      ...
      for (type = 0; type <= ZEBRA_ROUTE_MAX; type++)
        {
          if (PROTO_RM_NAME (zvrf, afi, type))
            XFREE (MTYPE_ROUTE_MAP_NAME, PROTO_RM_NAME (zvrf, afi, type));
          if (NHT_RM_NAME (zvrf, afi, type))
            XFREE (MTYPE_ROUTE_MAP_NAME, NHT_RM_NAME (zvrf, afi, type));
        }
    }
}
...
void
zebra_routemap_config_write_protocol (struct vty *vty, struct zebra_vrf *zvrf)
{
  int i;
  char space[2];
