 ...
 */
static unsigned int
neigh_hash_keymake (const void *p)
{
...
int
neigh_list_cmp (void *p1, void *p2)
{
  const struct zebra_neigh *n1 = p1;
  const struct zebra_neigh *n2 = p2;
...
uint32_t
num_dup_detected_neighs (struct zebra_evpn *zevpn)
{
  unsigned int i;
 ...
 */
void
zebra_evpn_find_neigh_addr_width (struct hash_bucket *bucket, void *ctxt)
{
 ...
 */
int
remote_neigh_count (struct zebra_mac *zmac)
{
 ...
 */
int
zebra_evpn_rem_neigh_install (struct zebra_evpn *zevpn, struct zebra_neigh *n,
                              bool was_static)
{
 ...
 */
void
zebra_evpn_install_neigh_hash (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_neigh *n;
  struct neigh_walk_ctx *wctx = ctxt;
...
static void
zebra_evpn_local_neigh_ref_mac (struct zebra_neigh *n,
                                const struct ethaddr *macaddr,
                                struct zebra_mac *mac, bool send_mac_update)
{
  bool old_static;
  bool new_static;
  ...
  if (n->flags & ZEBRA_NEIGH_ALL_PEER_FLAGS)
    {
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
        zlog_debug ("sync-neigh ref mac vni %u ip %pIA mac %pEA ref %d",
...
static void
zebra_evpn_sync_neigh_dp_install (struct zebra_neigh *n, bool set_inactive,
                                  bool force_clear_static, const char *caller)
{
  ...
  if (! ifp)
    {
      if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
        zlog_debug (
            ...
            caller, n->zevpn->vni, &n->ip, &n->emac, n->ifindex, n->flags);
      return;
    }
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
    zlog_debug (
        ...
        caller, n->zevpn->vni, &n->ip, &n->emac, ifp->name, n->ifindex,
        n->flags, set_router ? " router" : "", set_static ? " static" : "",
 ...
 */
int
zebra_evpn_neigh_send_add_to_client (vni_t vni, const struct ipaddr *ip,
                                     const struct ethaddr *macaddr,
 ...
 */
int
zebra_evpn_neigh_send_del_to_client (vni_t vni, struct ipaddr *ip,
                                     ...
                                     int state, bool force)
{
  ...
  return zebra_evpn_macip_send_msg_to_client (vni, macaddr, ip, flags, 0,
                                              state, NULL, ZEBRA_MACIP_DEL);
}

static void
zebra_evpn_neigh_send_add_del_to_client (struct zebra_neigh *n,
                                         bool old_bgp_ready,
                                         bool new_bgp_ready)
{
  if (new_bgp_ready)
    zebra_evpn_neigh_send_add_to_client (n->zevpn->vni, &n->ip, &n->emac,
                                         n->mac, n->flags, n->loc_seq);
  else if (old_bgp_ready)
    zebra_evpn_neigh_send_del_to_client (n->zevpn->vni, &n->ip, &n->emac,
                                         n->flags, n->state, true /*force*/);
}
 ...
 */
void
zebra_evpn_sync_neigh_static_chg (struct zebra_neigh *n, bool old_n_static,
                                  ...
                                  bool defer_mac_dp, const char *caller)
{
  ...
  if (! defer_n_dp)
    zebra_evpn_sync_neigh_dp_install (
        n, false /* set_inactive */, false /* force_clear_static */, __func__);
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
    zlog_debug (
        ...
        n->zevpn->vni, &n->ip, &n->emac, n->flags, mac->sync_neigh_cnt,
 ...
 */
static void
zebra_evpn_neigh_hold_exp_cb (struct event *t)
{
  struct zebra_neigh *n;
  bool old_bgp_ready;
  bool new_bgp_ready;
   ...
   */
  if (old_n_static != new_n_static)
    zebra_evpn_sync_neigh_static_chg (n, old_n_static, new_n_static,
                                      false /*defer_n_dp*/,
...
static inline void
zebra_evpn_neigh_start_hold_timer (struct zebra_neigh *n)
{
  if (n->hold_timer)
    return;
...
static void
zebra_evpn_local_neigh_deref_mac (struct zebra_neigh *n, bool send_mac_update)
{
  struct zebra_mac *mac = n->mac;
  ...
  if ((n->flags & ZEBRA_NEIGH_ALL_PEER_FLAGS) && mac->sync_neigh_cnt)
    {
      ...
      new_static = zebra_evpn_mac_is_static (mac);
      if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
        zlog_debug ("sync-neigh deref mac vni %u ip %pIA mac %pEA ref %d",
                    n->zevpn->vni, &n->ip, &n->emac, mac->sync_neigh_cnt);
      if ((old_static != new_static) && send_mac_update)
        /* program the local mac in the kernel */
        zebra_evpn_sync_mac_dp_install (mac, false /* set_inactive */,
      ...
      if (is_local && ! zebra_evpn_neigh_is_ready_for_bgp (n))
        {
          if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH || IS_ZEBRA_DEBUG_VXLAN)
            zlog_debug (
                ...
                sync ? "sync" : "remote", zevpn->vni, n_type, macaddr, &n->ip,
                tmp_seq, n->flags);
          return true;
        }
...
zebra_evpn_neigh_add (struct zebra_evpn *zevpn, const struct ipaddr *ip,
                      const struct ethaddr *mac, struct zebra_mac *zmac,
                      uint32_t n_flags)
{
  ...
  zebra_evpn_local_neigh_ref_mac (n, mac, zmac, false /* send_mac_update */);
 ...
 */
int
zebra_evpn_neigh_del (struct zebra_evpn *zevpn, struct zebra_neigh *n)
{
  struct zebra_neigh *tmp_n;
...
void
zebra_evpn_sync_neigh_del (struct zebra_neigh *n)
{
  bool old_n_static;
  bool new_n_static;

  if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
    zlog_debug ("sync-neigh del vni %u ip %pIA mac %pEA f 0x%x", n->zevpn->vni,
                &n->ip, &n->emac, n->flags);
...
struct zebra_neigh *
zebra_evpn_proc_sync_neigh_update (struct zebra_evpn *zevpn,
                                   struct zebra_neigh *n, uint16_t ipa_len,
                                   ...
                                   struct zebra_mac *mac)
{
  ...
  if (! n)
    {
      ...
      n = zebra_evpn_neigh_add (zevpn, ipaddr, &mac->macaddr, mac, n_flags);
      n->ifindex = ifindex;
      ZEBRA_NEIGH_SET_ACTIVE (n);
  ...
  else
    {
      ...
      if (mac_change)
        {
          if (old_bgp_ready)
            {
              zebra_evpn_neigh_send_del_to_client (zevpn->vni, &n->ip,
                                                   &n->emac, n->flags,
                                                   n->state, false /*force*/);
              old_bgp_ready = false;
            }
      ...
      if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH && (old_flags != n->flags))
        zlog_debug ("sync-neigh vni %u ip %pIA mac %pEA old_f 0x%x new_f 0x%x",
                    n->zevpn->vni, &n->ip, &n->emac, old_flags, n->flags);
  ...
  if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
    zlog_debug (
        ...
        created ? "created" : "updated", n->zevpn->vni, &n->ip, &n->emac,
        ifp ? ifp->name : "", ifindex, n->loc_seq, n->flags,
        inform_bgp ? " inform_bgp" : "", inform_dataplane ? " inform_dp" : "");

  if (inform_dataplane)
    zebra_evpn_sync_neigh_dp_install (
        n, set_dp_inactive, false /* force_clear_static */, __func__);
 ...
 */
static int
zebra_evpn_neigh_uninstall (struct zebra_evpn *zevpn, struct zebra_neigh *n)
{
  struct interface *vlan_if;
 ...
 */
static void
zebra_evpn_neigh_del_hash_entry (struct hash_bucket *bucket, void *arg)
{
  ...
  if (((wctx->flags & DEL_LOCAL_NEIGH) && (n->flags & ZEBRA_NEIGH_LOCAL)) ||
       ...
       IPV4_ADDR_SAME (&n->r_vtep_ip, &wctx->r_vtep_ip)))
    {
      if (wctx->upd_client && (n->flags & ZEBRA_NEIGH_LOCAL))
        zebra_evpn_neigh_send_del_to_client (wctx->zevpn->vni, &n->ip,
 ...
 */
void
zebra_evpn_neigh_del_all (struct zebra_evpn *zevpn, int uninstall,
                          int upd_client, uint32_t flags)
{
  ...
  hash_iterate (zevpn->neigh_table, zebra_evpn_neigh_del_hash_entry, &wctx);
}
 ...
 */
struct zebra_neigh *
zebra_evpn_neigh_lookup (struct zebra_evpn *zevpn, const struct ipaddr *ip)
{
 ...
 */
void
zebra_evpn_process_neigh_on_local_mac_change (struct zebra_evpn *zevpn,
                                              struct zebra_mac *zmac,
                                              bool seq_change, bool es_change)
{
  struct zebra_neigh *n = NULL;
  struct listnode *node = NULL;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug ("Processing neighbors on local MAC %pEA %s, VNI %u",
                &zmac->macaddr, seq_change ? "CHANGE" : "ADD", zevpn->vni);
  ...
  for (ALL_LIST_ELEMENTS_RO (zmac->neigh_list, node, n))
    {
      if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
        {
          if (IS_ZEBRA_NEIGH_INACTIVE (n) || seq_change || es_change)
            {
              ZEBRA_NEIGH_SET_ACTIVE (n);
              ...
              if (! (zebra_evpn_do_dup_addr_detect (zvrf) &&
                ...
                zebra_evpn_neigh_send_add_to_client (zevpn->vni, &n->ip,
                                                     &n->emac, n->mac,
                                                     n->flags, n->loc_seq);
            }
        }
 ...
 */
void
zebra_evpn_process_neigh_on_local_mac_del (struct zebra_evpn *zevpn,
                                           struct zebra_mac *zmac)
{
  struct zebra_neigh *n = NULL;
  struct listnode *node = NULL;
          ...
          if (IS_ZEBRA_NEIGH_ACTIVE (n))
            {
              ...
              zebra_evpn_neigh_send_del_to_client (
                  zevpn->vni, &n->ip, &n->emac, n->flags, ZEBRA_NEIGH_ACTIVE,
                  false /*force*/);
            }
        }
    }
 ...
 */
void
zebra_evpn_process_neigh_on_remote_mac_add (struct zebra_evpn *zevpn,
                                            struct zebra_mac *zmac)
{
  struct zebra_neigh *n = NULL;
  struct listnode *node = NULL;
 ...
 */
void
zebra_evpn_process_neigh_on_remote_mac_del (struct zebra_evpn *zevpn,
                                            struct zebra_mac *zmac)
{
...
static inline void
zebra_evpn_local_neigh_update_log (const char *pfx, struct zebra_neigh *n,
                                   bool is_router, bool local_inactive,
                                   bool old_bgp_ready, bool new_bgp_ready,
                                   bool inform_dataplane, bool inform_bgp,
                                   const char *sfx)
{
  if (! IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
    return;
 ...
 */
static int
zebra_evpn_ip_inherit_dad_from_mac (struct zebra_vrf *zvrf,
                                    ...
                                    struct zebra_neigh *nbr)
{
  ...
  if (new_zmac)
    is_new_mac_dup = CHECK_FLAG (new_zmac->flags, ZEBRA_MAC_DUPLICATE);
...
static void
zebra_evpn_dad_ip_auto_recovery_exp (struct event *t)
{
  struct zebra_vrf *zvrf = NULL;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    zlog_debug (
        ...
        __func__, &nbr->emac, &nbr->ip, nbr->flags, nbr->dad_count,
        zevpn->vni);
  ...
  if (CHECK_FLAG (nbr->flags, ZEBRA_NEIGH_LOCAL))
    {
      zebra_evpn_neigh_send_add_to_client (zevpn->vni, &nbr->ip, &nbr->emac,
...
static void
zebra_evpn_dup_addr_detect_for_neigh (struct zebra_vrf *zvrf,
                                      struct zebra_neigh *nbr,
      ...
      if (zvrf->dad_freeze && zvrf->dad_freeze_time)
        {
          ...
          event_add_timer (zrouter.master, zebra_evpn_dad_ip_auto_recovery_exp,
...
int
zebra_evpn_local_neigh_update (struct zebra_evpn *zevpn, struct interface *ifp,
                               ...
                               bool local_inactive, bool dp_static)
{
  ...
  if (! zmac)
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("AUTO MAC %pEA created for neigh %pIA on VNI %u", macaddr,
                    ip, zevpn->vni);

      zmac = zebra_evpn_mac_add_auto (zevpn, macaddr);
      if (! zmac)
        {
          zlog_debug ("Failed to add MAC %pEA VNI %u", macaddr, zevpn->vni);
          return -1;
        }
    }
  else
      ...
      if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
        {
          ...
          cur_is_router = ! ! CHECK_FLAG (n->flags, ZEBRA_NEIGH_ROUTER_FLAG);
          new_static = zebra_evpn_neigh_is_static (n);
          if (! mac_different && is_router == cur_is_router &&
              old_local_inactive == local_inactive && dp_static != new_static)
            {
              if (IS_ZEBRA_DEBUG_VXLAN)
                zlog_debug (
          ...
          if (! mac_different)
            {
               ...
               */
              is_neigh_freezed =
                  (zebra_evpn_do_dup_addr_detect (zvrf) && zvrf->dad_freeze &&
                   CHECK_FLAG (n->flags, ZEBRA_NEIGH_DUPLICATE));

              zebra_evpn_local_neigh_update_log (
                  "local", n, is_router, local_inactive, old_bgp_ready,
                  new_bgp_ready, false, false, "flag-update");
              ...
              if (! is_neigh_freezed)
                ...
                {
                  if (IS_ZEBRA_DEBUG_VXLAN && IS_ZEBRA_NEIGH_ACTIVE (n))
                    zlog_debug ("        Neighbor active and frozen");
                }
              return 0;
            }
          ...
          if (old_zmac)
            {
              old_mac_seq = CHECK_FLAG (old_zmac->flags, ZEBRA_MAC_REMOTE)
                                ? old_zmac->rem_seq
                                : old_zmac->loc_seq;
              neigh_mac_change = upd_mac_seq = true;
              zebra_evpn_local_neigh_deref_mac (n, true /* send_mac_update */);
            }
          ...
          /* Link to new MAC */
          zebra_evpn_local_neigh_ref_mac (n, macaddr, zmac,
      ...
      else if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_REMOTE))
        {
           ...
           */
          if (memcmp (n->emac.octet, macaddr->octet, ETH_ALEN) != 0)
            {
              old_zmac = n->mac;
              if (old_zmac)
                {
                  old_mac_seq = CHECK_FLAG (old_zmac->flags, ZEBRA_MAC_REMOTE)
                                    ...
                                    : old_zmac->loc_seq;
                  neigh_mac_change = upd_mac_seq = true;
                  zebra_evpn_local_neigh_deref_mac (n, true /* send_update */);
                }
  ...
  if (upd_mac_seq)
    {
      ...
      seq1 = CHECK_FLAG (zmac->flags, ZEBRA_MAC_REMOTE) ? zmac->rem_seq + 1
                                                        : zmac->loc_seq;
      seq2 = neigh_mac_change ? old_mac_seq + 1 : 0;
      mac_new_seq =
          zmac->loc_seq < MAX (seq1, seq2) ? MAX (seq1, seq2) : zmac->loc_seq;
    }
  ...
  if (upd_mac_seq && zmac->loc_seq != mac_new_seq)
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug ("Seq changed for MAC %pEA VNI %u - old %u new %u", macaddr,
                    zevpn->vni, zmac->loc_seq, mac_new_seq);
      zmac->loc_seq = mac_new_seq;
      if (zebra_evpn_mac_send_add_to_client (zevpn->vni, macaddr, zmac->flags,
...
int
zebra_evpn_remote_neigh_update (struct zebra_evpn *zevpn,
                                struct interface *ifp, const struct ipaddr *ip,
                                const struct ethaddr *macaddr, uint16_t state)
{
  struct zebra_neigh *n = NULL;
                                            ...
                                            void *arg)
{
  ...
  if (CHECK_FLAG (zn->flags, ZEBRA_NEIGH_LOCAL) && IS_ZEBRA_NEIGH_ACTIVE (zn))
    {
...
void
zebra_evpn_send_neigh_to_client (struct zebra_evpn *zevpn)
{
  struct neigh_walk_ctx wctx;
  ...
  hash_iterate (zevpn->neigh_table, zebra_evpn_send_neigh_hash_entry_to_client,
                &wctx);
}

void
zebra_evpn_clear_dup_neigh_hash (struct hash_bucket *bucket, void *ctxt)
{
  struct neigh_walk_ctx *wctx = ctxt;
  struct zebra_neigh *nbr;
  ...
  if (IS_ZEBRA_DEBUG_VXLAN)
    {
      ...
      zlog_debug ("%s: clear neigh %s dup state, flags 0x%x seq %u", __func__,
 ...
 */
void
zebra_evpn_print_neigh (struct zebra_neigh *n, void *ctxt, json_object *json)
{
  struct vty *vty;
  ...
  if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_REMOTE))
    {
      if (n->mac->es)
        {
          if (json)
            json_object_string_add (json, "remoteEs", n->mac->es->esi_str);
          else
        ...
        {
          if (json)
            json_object_string_addf (json, "remoteVtep", "%pI4",
                                     &n->r_vtep_ip);
          else
  ...
  if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_ROUTER_FLAG))
    {
      if (! json)
        {
          vty_out (vty, flags_present ? " ,Router" : " Flags: Router");
          flags_present = true;
        }
    }
  if (json == NULL)
      ...
      if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_DUPLICATE))
        {
          vty_out (vty, " Duplicate, detected at %s",
                   time_to_string (n->dad_dup_detect_time, timebuf));
        }
      else if (n->dad_count)
        {
          monotime_since (&n->detect_start_time, &detect_start_time);
          if (detect_start_time.tv_sec <= zvrf->dad_time)
            {
              time_to_string (n->detect_start_time.tv_sec, timebuf);
              vty_out (
                  vty,
...
void
zebra_evpn_print_neigh_hdr (struct vty *vty, struct neigh_walk_ctx *wctx)
{
...
static char *
zebra_evpn_print_neigh_flags (struct zebra_neigh *n, char *flags_buf,
 ...
 */
void
zebra_evpn_print_neigh_hash (struct hash_bucket *bucket, void *ctxt)
{
  struct vty *vty;
  ...
  if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
    {
      ...
      if (json_evpn == NULL)
      ...
      else
        {
          ...
          if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_DEF_GW))
            json_object_boolean_true_add (json_row, "defaultGateway");
          json_object_int_add (json_row, "localSequence", n->loc_seq);
          json_object_int_add (json_row, "remoteSequence", n->rem_seq);
          json_object_int_add (json_row, "detectionCount", n->dad_count);
          if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_DUPLICATE))
            json_object_boolean_true_add (json_row, "isDuplicate");
          else
            json_object_boolean_false_add (json_row, "isDuplicate");
        }
      wctx->count++;
    }
  else if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_REMOTE))
      ...
      if (json_evpn == NULL)
        {
          ...
          if (n->mac->es == NULL)
            inet_ntop (AF_INET, &n->r_vtep_ip, addr_buf, sizeof (addr_buf));

          vty_out (
              vty, "%*s %-6s %-5s %-8s %-17s %-30s %u/%u\n", -wctx->addr_width,
              buf2, "remote",
              zebra_evpn_print_neigh_flags (n, flags_buf, sizeof (flags_buf)),
              state_str, buf1, n->mac->es ? n->mac->es->esi_str : addr_buf,
              n->loc_seq, n->rem_seq);
        }
      else
        {
          ...
          json_object_string_add (json_row, "mac", buf1);
          if (n->mac->es)
            json_object_string_add (json_row, "remoteEs", n->mac->es->esi_str);
          else
            json_object_string_addf (json_row, "remoteVtep", "%pI4",
                                     &n->r_vtep_ip);
          if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_DEF_GW))
            json_object_boolean_true_add (json_row, "defaultGateway");
          json_object_int_add (json_row, "localSequence", n->loc_seq);
 ...
 */
void
zebra_evpn_print_neigh_hash_detail (struct hash_bucket *bucket, void *ctxt)
{
  struct vty *vty;
  json_object *json_evpn = NULL, *json_row = NULL;
  struct zebra_neigh *n;
...
void
zebra_evpn_print_dad_neigh_hash (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_neigh *nbr;
...
void
zebra_evpn_print_dad_neigh_hash_detail (struct hash_bucket *bucket, void *ctxt)
{
  struct zebra_neigh *nbr;
...
void
zebra_evpn_neigh_remote_macip_add (struct zebra_evpn *zevpn,
                                   struct zebra_vrf *zvrf,
  ...
  if (update_neigh)
    {
      if (! n)
        {
          n = zebra_evpn_neigh_add (zevpn, ipaddr, &mac->macaddr, mac, 0);
        }
      else
        {
          ...
          if (! zebra_evpn_neigh_is_bgp_seq_ok (zevpn, n, &mac->macaddr, seq,
                                                false))
            return;
          if (CHECK_FLAG (n->flags, ZEBRA_NEIGH_LOCAL))
            {
              ...
              if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
                zlog_debug (
                    ...
                    n->zevpn->vni, &n->ip, &n->emac, seq, n->flags);
              if (IS_ZEBRA_NEIGH_ACTIVE (n))
                zebra_evpn_neigh_send_del_to_client (
                    zevpn->vni, &n->ip, &n->emac, n->flags, n->state,
                    false /*force*/);
              zebra_evpn_neigh_clear_sync_info (n);
            }
          if (memcmp (&n->emac, &mac->macaddr, sizeof (struct ethaddr)) != 0)
            {
              /* update neigh list for macs */
              old_mac = zebra_evpn_mac_lookup (zevpn, &n->emac);
              if (old_mac)
                {
      ...
      zebra_evpn_dup_addr_detect_for_neigh (zvrf, n, n->r_vtep_ip, do_dad,
                                            &is_dup_detect, false);
      /* Install the entry. */
      if (! is_dup_detect)
...
int
zebra_evpn_neigh_gw_macip_add (struct interface *ifp, struct zebra_evpn *zevpn,
                               struct ipaddr *ip, struct zebra_mac *mac)
{
  struct zebra_neigh *n;
  ...
  if (advertise_gw_macip_enabled (zevpn))
    {
      ...
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug (
            ...
            ifp->name, ifp->ifindex, zevpn->vni, &mac->macaddr, ip, n->flags);

      zebra_evpn_neigh_send_add_to_client (zevpn->vni, ip, &n->emac, n->mac,
                                           n->flags, n->loc_seq);
    }
...
void
zebra_evpn_neigh_remote_uninstall (struct zebra_evpn *zevpn,
                                   struct zebra_vrf *zvrf,
                                   struct zebra_neigh *n,
                                   struct zebra_mac *mac,
                                   const struct ipaddr *ipaddr)
  ...
  else
    {
      if (IS_ZEBRA_DEBUG_VXLAN)
        zlog_debug (
            ...
            __func__, ipaddr, &n->emac, n->flags, &mac->macaddr);
    }
}

int
zebra_evpn_neigh_del_ip (struct zebra_evpn *zevpn, const struct ipaddr *ip)
{
  struct zebra_neigh *n;
  struct zebra_mac *zmac;
  bool old_bgp_ready;
  ...
  if (! zvrf)
    {
      zlog_debug ("%s: VNI %u vrf lookup failed.", __func__, zevpn->vni);
      return -1;
    }
