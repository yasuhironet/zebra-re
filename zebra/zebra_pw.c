...
static inline int
zebra_pw_compare (const struct zebra_pw *a, const struct zebra_pw *b)
{
...
struct zebra_pw *
zebra_pw_add (struct zebra_vrf *zvrf, const char *ifname, uint8_t protocol,
              struct zserv *client)
{
  struct zebra_pw *pw;

  if (IS_ZEBRA_DEBUG_PW)
    zlog_debug ("%u: adding pseudowire %s protocol %s", zvrf_id (zvrf), ifname,
...
void
zebra_pw_del (struct zebra_vrf *zvrf, struct zebra_pw *pw)
{
  if (IS_ZEBRA_DEBUG_PW)
...
void
zebra_pw_change (struct zebra_pw *pw, ifindex_t ifindex, int type, int af,
...
struct zebra_pw *
zebra_pw_find (struct zebra_vrf *zvrf, const char *ifname)
{
  struct zebra_pw pw;
  strlcpy (pw.ifname, ifname, sizeof (pw.ifname));
...
static int
zebra_pw_enabled (struct zebra_pw *pw)
{
  if (pw->protocol == ZEBRA_ROUTE_STATIC)
    {
...
void
zebra_pw_update (struct zebra_pw *pw)
{
...
static void
zebra_pw_install (struct zebra_pw *pw)
{
  if (IS_ZEBRA_DEBUG_PW)
    zlog_debug ("%u: installing pseudowire %s protocol %s", pw->vrf_id,
                pw->ifname, zebra_route_string (pw->protocol));
...
static void
zebra_pw_uninstall (struct zebra_pw *pw)
{
  if (pw->status != PW_FORWARDING)
    return;

  if (IS_ZEBRA_DEBUG_PW)
    zlog_debug ("%u: uninstalling pseudowire %s protocol %s", pw->vrf_id,
                pw->ifname, zebra_route_string (pw->protocol));
 ...
 */
void
zebra_pw_install_failure (struct zebra_pw *pw, int pwstatus)
{
  if (IS_ZEBRA_DEBUG_PW)
    zlog_debug (
...
static void
zebra_pw_install_retry (struct event *thread)
{
...
static void
zebra_pw_update_status (struct zebra_pw *pw, int status)
{
  pw->status = status;
  if (pw->client)
...
static int
zebra_pw_check_reachability_strict (const struct zebra_pw *pw,
...
static int
zebra_pw_check_reachability (const struct zebra_pw *pw)
{
  struct route_entry *re;
  const struct nexthop *nexthop;
  const struct nexthop_group *nhg;
  ...
  re = rib_match (family2afi (pw->af), SAFI_UNICAST, pw->vrf_id, &pw->nexthop,
                  NULL);
  if (! re)
    {
      if (IS_ZEBRA_DEBUG_PW)
        zlog_debug ("%s: no route found for %s", __func__, pw->ifname);
      return -1;
    }
...
void
zebra_pw_init (struct zebra_vrf *zvrf)
{
...
void
zebra_pw_exit (struct zebra_vrf *zvrf)
{
  struct zebra_pw *pw;
       ...
       "IPv6 address\n")
{
  ...
  zebra_pw_change (pw, pw->ifindex, pw->type, af, &nexthop, pw->local_label,
                   pw->remote_label, pw->flags, &pw->data);
  ...
  RB_FOREACH (pw, zebra_pw_head, &zvrf->pseudowires)
    {
      ...
      if (pw->local_label != MPLS_NO_LABEL &&
          pw->remote_label != MPLS_NO_LABEL)
        snprintf (buf_labels, sizeof (buf_labels), "%u/%u", pw->local_label,
                  pw->remote_label);
      else
...
static void
vty_show_mpls_pseudowire_detail (struct vty *vty)
{
  struct zebra_vrf *zvrf;
  struct zebra_pw *pw;
  struct route_entry *re;
...
static void
vty_show_mpls_pseudowire (struct zebra_pw *pw, json_object *json_pws)
{
  struct route_entry *re;
  struct nexthop *nexthop;
  struct nexthop_group *nhg;
  ...
  json_object_string_add (
      json_pw, "Status",
      (zebra_pw_enabled (pw) && pw->status == PW_FORWARDING) ? "Up" : "Down");
  re = rib_match (family2afi (pw->af), SAFI_UNICAST, pw->vrf_id, &pw->nexthop,
                  NULL);
  if (re == NULL)
    goto done;
...
static void
vty_show_mpls_pseudowire_detail_json (struct vty *vty)
{
...
static int
zebra_pw_config (struct vty *vty)
{
  int write = 0;
  struct zebra_vrf *zvrf;
  struct zebra_pw *pw;
  ...
  RB_FOREACH (pw, zebra_static_pw_head, &zvrf->static_pseudowires)
    {
      ...
      if (pw->af != AF_UNSPEC)
        ...
        vty_out (vty, " ! Incomplete config, specify a neighbor address\n");
