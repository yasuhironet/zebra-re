...
int
zebra_ns_disabled (struct ns *ns)
{
  struct zebra_ns *zns = ns->info;
 ...
 */
static int
zebra_ns_disable_internal (struct zebra_ns *zns, bool complete)
{
  if (zns->if_table)
 ...
 */
int
zebra_ns_early_shutdown (struct ns *ns,
 ...
 */
int
zebra_ns_final_shutdown (struct ns *ns,
                         void *param_in __attribute__ ((unused)),
                         void **param_out __attribute__ ((unused)))
{
  struct zebra_ns *zns = ns->info;
...
zebra_ns_init (void)
{
  ...
  if (! default_ns)
    {
      flog_err (EC_ZEBRA_NS_NO_DEFAULT, "%s: failed to find default ns",
                __func__);
...
int
zebra_ns_config_write (struct vty *vty, struct ns *ns)
{
  if (ns && ns->name != NULL)
    vty_out (vty, " netns %s\n", ns->name);
  return 0;
