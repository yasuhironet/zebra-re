...
struct route_table *
zebra_vrf_get_table_with_table_id (afi_t afi, safi_t safi, vrf_id_t vrf_id,
                                   uint32_t table_id)
{
  struct zebra_vrf *zvrf = vrf_info_lookup (vrf_id);
  struct other_route_table *otable;
...
static void
zebra_rnhtable_node_cleanup (struct route_table *table,
                             struct route_node *node)
{
  if (node->info)
...
struct zebra_vrf *
zebra_vrf_alloc (struct vrf *vrf)
{
  struct zebra_vrf *zvrf;
      ...
      if (zvrf_id (zvrf) == VRF_DEFAULT)
        {
          ...
          if (zvrf->tbl_mgr && (zvrf->tbl_mgr->start || zvrf->tbl_mgr->end))
 ...
 */
static void
vrf_update_vrf_id (ns_id_t ns_id, void *opaqueptr)
{
...
int
zebra_vrf_netns_handler_create (struct vty *vty, struct vrf *vrf,
  ...
  if (vrf->vrf_id != VRF_UNKNOWN && vrf->ns_ctxt == NULL)
    {
      if (vty)
        ...
        zlog_info ("VRF %u is already configured with VRF %s", vrf->vrf_id,
                   vrf->name);
      return CMD_WARNING_CONFIG_FAILED;
    }
  if (vrf->ns_ctxt != NULL)
    {
      ...
      if (! strcmp (ns->name, pathname))
        {
          if (vty)
            vty_out (vty, "VRF %u already configured with NETNS %s\n",
                     vrf->vrf_id, ns->name);
          else
            zlog_info ("VRF %u already configured with NETNS %s", vrf->vrf_id,
                       ns->name);
          return CMD_WARNING;
        }
    }
  ...
  if (ns && ns->vrf_ctxt)
    {
      ...
      if (vrf2 == vrf)
        return CMD_SUCCESS;
      if (vty)
        vty_out (vty, "NS %s is already configured with VRF %u(%s)\n",
                 ns->name, vrf2->vrf_id, vrf2->name);
      else
        zlog_info ("NS %s is already configured with VRF %u(%s)", ns->name,
                   vrf2->vrf_id, vrf2->name);
      return CMD_WARNING_CONFIG_FAILED;
    }
  ...
  if (! ns_enable (ns, vrf_update_vrf_id))
    {
      if (vty)
        ...
        zlog_info ("Can not associate NS %u with NETNS %s", ns->ns_id,
                   ns->name);
      return CMD_WARNING_CONFIG_FAILED;
    }
