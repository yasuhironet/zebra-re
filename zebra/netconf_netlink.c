...
netconf_rta (struct netconfmsg *ncm)
{
 ...
 */
int
netlink_netconf_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  ...
  if (tb[NETCONFA_IGNORE_ROUTES_WITH_LINKDOWN])
    {
      ival = *(uint32_t *) RTA_DATA (tb[NETCONFA_IGNORE_ROUTES_WITH_LINKDOWN]);
      if (ival != 0)
 ...
 */
int
netlink_request_netconf (int sockfd)
{
  struct nlsock *nls;
  struct
  {
...
netlink_put_intf_netconfig (struct nl_batch *bth, struct zebra_dplane_ctx *ctx)
{
  ...
  snprintf (mpls_proc, sizeof (mpls_proc), "/proc/sys/net/mpls/conf/%s/input",
            ifname);
      ...
      if (fd < 0)
        {
          flog_err_sys (EC_LIB_SOCKET,
                        "%s: Unable to open %s for writing: %s(%d)", __func__,
                        mpls_proc, safe_strerror (errno), errno);
          return ret;
        }
      if (write (fd, set, 2) == 2)
        ret = FRR_NETLINK_SUCCESS;
      else
        flog_err_sys (EC_LIB_SOCKET, "%s: Unsuccessful write to %s: %s(%d)",
                      __func__, mpls_proc, safe_strerror (errno), errno);
      close (fd);
    }
  return ret;
}
