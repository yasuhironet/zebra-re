...
depends_find (const struct nexthop *nh, afi_t afi, int type, bool from_dplane);
...
static void
nhg_connected_free (struct nhg_connected *dep)
{
  XFREE (MTYPE_NHG_CONNECTED, dep);
}

static struct nhg_connected *
nhg_connected_new (struct nhg_hash_entry *nhe)
{
...
void
nhg_connected_tree_free (struct nhg_connected_tree_head *head)
{
...
struct nhg_hash_entry *
zebra_nhg_resolve (struct nhg_hash_entry *nhe)
{
...
unsigned int
zebra_nhg_depends_count (const struct nhg_hash_entry *nhe)
{
  return nhg_connected_tree_count (&nhe->nhg_depends);
}

bool
zebra_nhg_depends_is_empty (const struct nhg_hash_entry *nhe)
{
  return nhg_connected_tree_is_empty (&nhe->nhg_depends);
}

static void
zebra_nhg_depends_del (struct nhg_hash_entry *from,
                       struct nhg_hash_entry *depend)
{
...
static void
zebra_nhg_depends_init (struct nhg_hash_entry *nhe)
{
...
bool
zebra_nhg_dependents_is_empty (const struct nhg_hash_entry *nhe)
{
  return nhg_connected_tree_is_empty (&nhe->nhg_dependents);
}

static void
zebra_nhg_dependents_del (struct nhg_hash_entry *from,
...
static void
zebra_nhg_dependents_add (struct nhg_hash_entry *to,
                          struct nhg_hash_entry *dependent)
{
...
static void
zebra_nhg_dependents_init (struct nhg_hash_entry *nhe)
{
...
static void
zebra_nhg_dependents_release (struct nhg_hash_entry *nhe)
{
  struct nhg_connected *rb_node_dep = NULL;
...
static void
zebra_nhg_depends_release (struct nhg_hash_entry *nhe)
{
...
static int
zebra_nhg_insert_id (struct nhg_hash_entry *nhe)
{
...
static void
zebra_nhg_set_if (struct nhg_hash_entry *nhe, struct interface *ifp)
{
  nhe->ifp = ifp;
  ...
  if (! zebra_nhg_depends_is_empty (nhe))
    {
      frr_each (nhg_connected_tree, &nhe->nhg_depends, rb_node_dep)
        {
          if (IS_ZEBRA_DEBUG_NHG_DETAIL)
            zlog_debug ("%s: nhe %p (%pNG), dep %p (%pNG)", __func__, nhe, nhe,
...
void
zebra_nhe_init (struct nhg_hash_entry *nhe, afi_t afi,
 ...
 */
struct nhg_hash_entry *
zebra_nhe_copy (const struct nhg_hash_entry *orig, uint32_t id)
{
  struct nhg_hash_entry *nhe;
...
uint32_t
zebra_nhg_hash_key (const void *arg)
{
...
uint32_t
zebra_nhg_id_key (const void *arg)
{
  const struct nhg_hash_entry *nhe = arg;
...
static bool
nhg_compare_nexthops (const struct nexthop *nh1, const struct nexthop *nh2)
{
...
bool
zebra_nhg_hash_equal (const void *arg1, const void *arg2)
{
...
bool
zebra_nhg_hash_id_equal (const void *arg1, const void *arg2)
{
  const struct nhg_hash_entry *nhe1 = arg1;
  const struct nhg_hash_entry *nhe2 = arg2;
...
static int
zebra_nhg_process_grp (struct nexthop_group *nhg,
...
static void
handle_recursive_depend (struct nhg_connected_tree_head *nhg_depends,
                         struct nexthop *nh, afi_t afi, int type)
{
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: head %p, nh %pNHv", __func__, nhg_depends, nh);

  depend = zebra_nhg_rib_find (0, &resolved_ng, afi, type);

  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: nh %pNHv => %p (%u)", __func__, nh, depend,
                depend ? depend->id : 0);
 ...
 */
static bool
zebra_nhe_find (struct nhg_hash_entry **nhe, /* return value */
                struct nhg_hash_entry *lookup,
                struct nhg_connected_tree_head *nhg_depends, afi_t afi,
                bool from_dplane)
{
  bool created = false;
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: id %u, lookup %p, vrf %d, type %d, depends %p%s",
                __func__, lookup->id, lookup, lookup->vrf_id, lookup->type,
  ...
  if (! from_dplane && lookup->id < ZEBRA_NHG_PROTO_LOWER)
  ...
  else
    {
       ...
       * It goes in ID table.
       */
      newnhe = hash_get (zrouter.nhgs_id, lookup, zebra_nhg_hash_alloc);
    }
  ...
  *nhe = newnhe;
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: => created %p (%pNG)", __func__, newnhe, newnhe);
  ...
  if (nh->next == NULL && newnhe->id < ZEBRA_NHG_PROTO_LOWER)
  ...
  else
    {
      ...
      for (nh = newnhe->nhg.nexthop; nh; nh = nh->next)
        {
          if (IS_ZEBRA_DEBUG_NHG_DETAIL)
            zlog_debug ("%s: depends NH %pNHv %s", __func__, nh,
                        CHECK_FLAG (nh->flags, NEXTHOP_FLAG_RECURSIVE) ? "(R)"
                                                                       : "");

          depends_find_add (&newnhe->nhg_depends, nh, afi, newnhe->type,
                            from_dplane);
        }
    }
  ...
  if (nh->next == NULL && CHECK_FLAG (nh->flags, NEXTHOP_FLAG_RECURSIVE))
    {
      if (IS_ZEBRA_DEBUG_NHG_DETAIL)
        zlog_debug ("%s: backup depend NH %pNHv (R)", __func__, nh);
  ...
  else
    {
      ...
      for (; nh; nh = nh->next)
        {
          if (IS_ZEBRA_DEBUG_NHG_DETAIL)
            zlog_debug ("%s: backup depend NH %pNHv %s", __func__, nh,
                        CHECK_FLAG (nh->flags, NEXTHOP_FLAG_RECURSIVE) ? "(R)"
                                                                       : "");
 ...
 */
static bool
zebra_nhg_find (struct nhg_hash_entry **nhe, uint32_t id,
                struct nexthop_group *nhg,
                struct nhg_connected_tree_head *nhg_depends, vrf_id_t vrf_id,
                afi_t afi, int type, bool from_dplane)
{
  struct nhg_hash_entry lookup = {};
  bool created = false;
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: id %u, nhg %p, vrf %d, type %d, depends %p", __func__, id,
...
static uint32_t
nhg_ctx_get_id (const struct nhg_ctx *ctx)
{
  return ctx->id;
}

static void
nhg_ctx_set_status (struct nhg_ctx *ctx, enum nhg_ctx_status status)
{
  ctx->status = status;
}

static enum nhg_ctx_status
nhg_ctx_get_status (const struct nhg_ctx *ctx)
{
  return ctx->status;
}

static void
nhg_ctx_set_op (struct nhg_ctx *ctx, enum nhg_ctx_op_e op)
{
  ctx->op = op;
}

static enum nhg_ctx_op_e
nhg_ctx_get_op (const struct nhg_ctx *ctx)
{
  return ctx->op;
}

static vrf_id_t
nhg_ctx_get_vrf_id (const struct nhg_ctx *ctx)
{
  return ctx->vrf_id;
}

static int
nhg_ctx_get_type (const struct nhg_ctx *ctx)
{
  return ctx->type;
}

static int
nhg_ctx_get_afi (const struct nhg_ctx *ctx)
{
  return ctx->afi;
}

static struct nexthop *
nhg_ctx_get_nh (struct nhg_ctx *ctx)
{
  return &ctx->u.nh;
}

static uint8_t
nhg_ctx_get_count (const struct nhg_ctx *ctx)
{
  return ctx->count;
}

static struct nh_grp *
nhg_ctx_get_grp (struct nhg_ctx *ctx)
{
  return ctx->u.grp;
}

static struct nhg_resilience *
nhg_ctx_get_resilience (struct nhg_ctx *ctx)
{
  return &ctx->resilience;
}

static struct nhg_ctx *
nhg_ctx_new (void)
{
  struct nhg_ctx *new;
...
void
nhg_ctx_free (struct nhg_ctx **ctx)
{
  struct nexthop *nh;
...
static struct nhg_ctx *
nhg_ctx_init (uint32_t id, struct nexthop *nh, struct nh_grp *grp,
...
static void
zebra_nhg_set_valid (struct nhg_hash_entry *nhe)
{
...
static void
zebra_nhg_set_invalid (struct nhg_hash_entry *nhe)
{
  struct nhg_connected *rb_node_dep;
...
void
zebra_nhg_check_valid (struct nhg_hash_entry *nhe)
{
  struct nhg_connected *rb_node_dep = NULL;
  bool valid = false;
...
static void
zebra_nhg_release_all_deps (struct nhg_hash_entry *nhe)
{
...
static void
zebra_nhg_release (struct nhg_hash_entry *nhe)
{
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
...
static void
zebra_nhg_handle_uninstall (struct nhg_hash_entry *nhe)
{
...
static void
zebra_nhg_handle_install (struct nhg_hash_entry *nhe, bool install)
{
      ...
      if (install)
        {
          if (IS_ZEBRA_DEBUG_NHG_DETAIL)
            zlog_debug (
                ...
                __func__, nhe->id, nhe->flags, rb_node_dep->nhe);
 ...
 */
static void
zebra_nhg_handle_kernel_state_change (struct nhg_hash_entry *nhe,
                                      bool is_delete)
{
  if (nhe->refcnt)
    {
...
static int
nhg_ctx_process_new (struct nhg_ctx *ctx)
{
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: id %u, count %d, lookup => %p", __func__, id, count,
                lookup);
  ...
  if (nhg_ctx_get_count (ctx))
    {
      ...
      if (zebra_nhg_process_grp (nhg, &nhg_depends, nhg_ctx_get_grp (ctx),
      ...
      if (! zebra_nhg_find (&nhe, id, nhg, &nhg_depends, vrf_id, afi, type,
                            true))
        depends_decrement_free (&nhg_depends);
    ...
    nhe = zebra_nhg_find_nexthop (id, nhg_ctx_get_nh (ctx), afi, type, true);
...
static int
nhg_ctx_process_del (struct nhg_ctx *ctx)
{
  struct nhg_hash_entry *nhe = NULL;
  uint32_t id = nhg_ctx_get_id (ctx);
...
static void
nhg_ctx_fini (struct nhg_ctx **ctx)
{
  /*
...
int
nhg_ctx_process (struct nhg_ctx *ctx)
{
  int ret = 0;
                       ...
                       int startup, struct nhg_resilience *nhgr)
{
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: nh %pNHv, id %u, count %d", __func__, nh, id,
                (int) count);
...
int
zebra_nhg_kernel_del (uint32_t id, vrf_id_t vrf_id)
{
  struct nhg_ctx *ctx = NULL;
...
static struct nhg_hash_entry *
depends_find_recursive (const struct nexthop *nh, afi_t afi, int type)
{
  struct nhg_hash_entry *nhe;
...
static struct nhg_hash_entry *
depends_find_singleton (const struct nexthop *nh, afi_t afi, int type,
                        bool from_dplane)
{
  struct nhg_hash_entry *nhe;
...
static void
depends_add (struct nhg_connected_tree_head *head,
             struct nhg_hash_entry *depend)
{
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: head %p nh %pNHv", __func__, head, depend->nhg.nexthop);
                  ...
                  afi_t afi, int type, bool from_dplane)
{
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: nh %pNHv => %p", __func__, nh, depend);
...
static void
depends_decrement_free (struct nhg_connected_tree_head *head)
{
...
zebra_nhg_rib_find_nhe (struct nhg_hash_entry *rt_nhe, afi_t rt_afi)
{
  ...
  if (! (rt_nhe && rt_nhe->nhg.nexthop))
    {
      flog_err (EC_ZEBRA_TABLE_LOOKUP_FAILED, "No nexthop passed to %s",
                __func__);
      return NULL;
    }
 ...
 */
void
zebra_nhg_backup_free (struct nhg_backup_info **p)
{
  if (p && *p)
    {
      if ((*p)->nhe)
...
zebra_nhg_get_backup_nhg (struct nhg_hash_entry *nhe)
{
...
static void
zebra_nhg_free_members (struct nhg_hash_entry *nhe)
{
...
void
zebra_nhg_free (struct nhg_hash_entry *nhe)
{
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    {
      /* Group or singleton? */
      if (nhe->nhg.nexthop && nhe->nhg.nexthop->next)
        zlog_debug ("%s: nhe %p (%pNG), refcnt %d", __func__, nhe, nhe,
                    nhe->refcnt);
      else
        zlog_debug ("%s: nhe %p (%pNG), refcnt %d, NH %pNHv", __func__, nhe,
                    nhe, nhe->refcnt, nhe->nhg.nexthop);
    }
 ...
 */
void
zebra_nhg_hash_free_zero_id (struct hash_bucket *b, void *arg)
{
  ...
  if (nhe->backup_info && nhe->backup_info->nhe->id == 0)
    {
      while (
          (dep = nhg_connected_tree_pop (&nhe->backup_info->nhe->nhg_depends)))
        nhg_connected_free (dep);
...
static void
zebra_nhg_timer (struct event *thread)
{
...
void
zebra_nhg_decrement_ref (struct nhg_hash_entry *nhe)
{
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: nhe %p (%pNG) %d => %d", __func__, nhe, nhe, nhe->refcnt,
                nhe->refcnt - 1);
  ...
  if (! zebra_router_in_shutdown () && nhe->refcnt <= 0 &&
      ...
      ! CHECK_FLAG (nhe->flags, NEXTHOP_GROUP_KEEP_AROUND))
    {
      ...
      event_add_timer (zrouter.master, zebra_nhg_timer, nhe, zrouter.nhg_keep,
                       &nhe->timer);
      return;
    }
...
void
zebra_nhg_increment_ref (struct nhg_hash_entry *nhe)
{
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: nhe %p (%pNG) %d => %d", __func__, nhe, nhe, nhe->refcnt,
                nhe->refcnt + 1);
...
static struct nexthop *
nexthop_set_resolved (afi_t afi, const struct nexthop *newhop,
                      struct nexthop *nexthop, struct zebra_sr_policy *policy)
{
          ...
          if (num_labels >= MPLS_MAX_LABELS)
            {
              if (IS_ZEBRA_DEBUG_NHG || IS_ZEBRA_DEBUG_RIB)
                zlog_debug ("%s: too many labels in newhop %pNHv", __func__,
                            newhop);
              break;
            }
      ...
      for (i = 0; i < nexthop->nh_label->num_labels; i++)
        {
          ...
          if (num_labels >= MPLS_MAX_LABELS)
            {
              if (IS_ZEBRA_DEBUG_NHG || IS_ZEBRA_DEBUG_RIB)
                zlog_debug ("%s: too many labels in nexthop %pNHv", __func__,
                            nexthop);
              break;
            }
  ...
  if (num_labels)
    nexthop_add_labels (resolved_hop, label_type, num_labels, labels);
  ...
  if (nexthop->nh_srv6)
    {
      ...
      nexthop_add_srv6_seg6 (resolved_hop, &nexthop->nh_srv6->seg6_segs);
    }
...
static bool
nexthop_valid_resolve (const struct nexthop *nexthop,
 ...
 */
static bool
nexthop_set_evpn_dvni_svd (vrf_id_t re_vrf_id, struct nexthop *nexthop)
{
 ...
 */
static int
resolve_backup_nexthops (const struct nexthop *nexthop,
  ...
  for (i = 0; i < nexthop->backup_num; i++)
    {
      ...
      if (j < map->map_count)
        {
          resolved->backup_idx[resolved->backup_num] = map->map[j].new_idx;
          resolved->backup_num++;

          SET_FLAG (resolved->flags, NEXTHOP_FLAG_HAS_BACKUP);

          if (IS_ZEBRA_DEBUG_RIB_DETAILED)
            zlog_debug ("%s: found map idx orig %d, new %d", __func__,
                        map->map[j].orig_idx, map->map[j].new_idx);
       ...
       */
      if (newnh->nh_label && resolved->nh_label && nexthop->nh_label)
        {
          if (resolved->nh_label->num_labels > nexthop->nh_label->num_labels)
            {
              /* Prepare new label stack */
              num_labels = 0;
              for (j = 0; j < newnh->nh_label->num_labels; j++)
                {
              ...
              for (j = nexthop->nh_label->num_labels;
                   j < resolved->nh_label->num_labels; j++)
                {
                  labels[num_labels] = resolved->nh_label->label[j];
                  num_labels++;
                }

              /* Replace existing label stack in the backup */
              nexthop_del_labels (newnh);
              nexthop_add_labels (newnh, bnh->nh_label_type, num_labels,
                                  labels);
            }
        }
      ...
      if (IS_ZEBRA_DEBUG_RIB_DETAILED)
        zlog_debug ("%s: added idx orig %d, new %d", __func__, idx, j);
 ...
 */
static int
nexthop_active (struct nexthop *nexthop, struct nhg_hash_entry *nhe,
      ...
      if (! ifp)
        {
          if (IS_ZEBRA_DEBUG_RIB_DETAILED)
            zlog_debug (
                "nexthop %pNHv marked onlink but nhif %u doesn't exist",
      ...
      if (! if_is_operative (ifp))
        {
          if (IS_ZEBRA_DEBUG_RIB_DETAILED)
            zlog_debug (
                "nexthop %pNHv marked onlink but nhif %s is not operational",
                nexthop, ifp->name);
          return 0;
        }
      return 1;
  ...
  while (rn)
    {
      ...
      if (prefix_same (&rn->p, top))
        if (((afi == AFI_IP) && (rn->p.prefixlen != IPV4_MAX_BITLEN)) ||
            ((afi == AFI_IP6) && (rn->p.prefixlen != IPV6_MAX_BITLEN)))
          {
            if (IS_ZEBRA_DEBUG_RIB_DETAILED)
              zlog_debug (
      ...
      if ((match->type == ZEBRA_ROUTE_CONNECT) ||
        ...
        {
          match = zebra_nhg_connected_ifindex (rn, match, nexthop->ifindex);
          ...
          if (IS_ZEBRA_DEBUG_NHG_DETAIL)
            zlog_debug ("%s: CONNECT match %p (%pNG), newhop %pNHv", __func__,
                        match, match->nhe, newhop);
      ...
      else if (CHECK_FLAG (flags, ZEBRA_FLAG_ALLOW_RECURSION))
        {
          ...
          if (! CHECK_FLAG (match->status, ROUTE_ENTRY_INSTALLED) &&
              ! CHECK_FLAG (match->status, ROUTE_ENTRY_ROUTE_REPLACING))
            {
              if (IS_ZEBRA_DEBUG_RIB_DETAILED)
                zlog_debug (
          ...
          for (ALL_NEXTHOPS_PTR (nhg, newhop))
            {
              ...
              if (resolver && newhop->backup_num > 0)
                {
                  resolve_backup_nexthops (newhop, match->nhe, resolver, nhe,
                                           &map);
                }
            }
          ...
          if (! use_recursive_backups || nhg == NULL || nhg->nexthop == NULL)
            goto done_with_match;
          ...
          for (ALL_NEXTHOPS_PTR (nhg, newhop))
            {
              ...
              SET_FLAG (nexthop->flags, NEXTHOP_FLAG_RECURSIVE);
              nexthop_set_resolved (afi, newhop, nexthop, NULL);
              resolved = 1;
            }
        ...
        done_with_match:
          ...
          if (resolved)
            ...
            zlog_debug ("        %s: Recursion failed to find", __func__);
...
nexthop_active_check (struct route_node *rn, struct route_entry *re,
                      struct nexthop *nexthop, struct nhg_hash_entry *nhe)
{
  ...
  switch (nexthop->type)
    {
    case NEXTHOP_TYPE_IFINDEX:
      if (nexthop_active (nexthop, nhe, &rn->p, re->type, re->flags, &mtu,
                          vrf_id))
        SET_FLAG (nexthop->flags, NEXTHOP_FLAG_ACTIVE);
      else
...
skip_check:
  ...
  if (! CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_ACTIVE))
    {
      if (IS_ZEBRA_DEBUG_RIB_DETAILED)
        zlog_debug ("        %s: Unable to find active nexthop", __func__);
      return 0;
    }
  ...
  if (ret == RMAP_DENYMATCH)
    {
      if (IS_ZEBRA_DEBUG_RIB)
        {
          zlog_debug ("%u:%pRN: Filtering out with NH %pNHv due to route map",
 ...
 */
static bool
zebra_nhg_set_valid_if_active (struct nhg_hash_entry *nhe)
{
  struct nhg_connected *rb_node_dep = NULL;
  bool valid = false;
 ...
 */
static bool
nexthop_list_set_evpn_dvni (struct route_entry *re, struct nexthop_group *nhg)
{
 ...
 */
static uint32_t
nexthop_list_active_update (struct route_node *rn, struct route_entry *re,
                            struct nhg_hash_entry *nhe, bool is_backup)
{
  ...
  for (; nexthop; nexthop = nexthop->next)
    {
       ...
       */
      new_active =
          nexthop_active_check (rn, re, nexthop, (is_backup ? NULL : nhe));
...
static uint32_t
proto_nhg_nexthop_active_update (struct nexthop_group *nhg)
{
  struct nexthop *nh;
 ...
 */
int
nexthop_active_update (struct route_node *rn, struct route_entry *re)
{
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: re %p nhe %p (%pNG), curr_nhe %p", __func__, re, re->nhe,
                re->nhe, curr_nhe);
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: re %p curr_active %u", __func__, re, curr_active);
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: re %p backup_active %u", __func__, re, backup_active);
 ...
 */
static uint8_t
zebra_nhg_nhe2grp_internal (struct nh_grp *grp, uint8_t curr_index,
...
uint8_t
zebra_nhg_nhe2grp (struct nh_grp *grp, struct nhg_hash_entry *nhe, int max_num)
{
...
void
zebra_nhg_install_kernel (struct nhg_hash_entry *nhe)
{
  struct nhg_connected *rb_node_dep = NULL;
  ...
  if (zebra_nhg_set_valid_if_active (nhe))
    {
      if (IS_ZEBRA_DEBUG_NHG_DETAIL)
        zlog_debug ("%s: valid flag set for nh %pNG", __func__, nhe);
    }
...
void
zebra_nhg_uninstall_kernel (struct nhg_hash_entry *nhe)
{
...
void
zebra_nhg_dplane_result (struct zebra_dplane_ctx *ctx)
{
  enum dplane_op_e op;
  ...
  if (IS_ZEBRA_DEBUG_DPLANE_DETAIL || IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("Nexthop dplane ctx %p, op %s, nexthop ID (%u), result %s",
  ...
  switch (op)
    ...
    case DPLANE_OP_NH_DELETE:
      if (status != ZEBRA_DPLANE_REQUEST_SUCCESS)
        flog_err (EC_ZEBRA_DP_DELETE_FAIL,
                  "Failed to uninstall Nexthop ID (%u) from the kernel", id);
    ...
    case DPLANE_OP_NH_UPDATE:
      ...
      if (status == ZEBRA_DPLANE_REQUEST_SUCCESS)
      ...
      else
        {
          ...
          if (! (zebra_nhg_proto_nexthops_only () && ! PROTO_OWNED (nhe)))
            flog_err (EC_ZEBRA_DP_INSTALL_FAIL,
                      "Failed to install Nexthop (%pNG) into the kernel", nhe);
        }
      break;
...
static int
zebra_nhg_sweep_entry (struct hash_bucket *bucket, void *arg)
{
  struct nhg_hash_entry *nhe = NULL;
...
void
zebra_nhg_sweep_table (struct hash *hash)
{
  uint32_t count;
...
static void
zebra_nhg_mark_keep_entry (struct hash_bucket *bucket, void *arg)
{
 ...
 */
void
zebra_nhg_enable_kernel_nexthops (bool set)
{
  g_nexthops_enabled = set;
}

bool
zebra_nhg_kernel_nexthops_enabled (void)
{
...
void
zebra_nhg_set_recursive_use_backups (bool set)
{
  use_recursive_backups = set;
}

bool
zebra_nhg_recursive_use_backups (void)
{
 ...
 */
void
zebra_nhg_set_proto_nexthops_only (bool set)
{
  proto_nexthops_only = set;
}

bool
zebra_nhg_proto_nexthops_only (void)
{
...
struct nhg_hash_entry *
zebra_nhg_proto_add (uint32_t id, int type, uint16_t instance,
                     uint32_t session, struct nexthop_group *nhg, afi_t afi)
{
  struct nhg_hash_entry lookup;
  ...
  for (ALL_NEXTHOPS_PTR (nhg, newhop))
    {
      if (CHECK_FLAG (newhop->flags, NEXTHOP_FLAG_HAS_BACKUP))
        {
          if (IS_ZEBRA_DEBUG_NHG)
            zlog_debug ("%s: id %u, backup nexthops not supported", __func__,
                        id);
          return NULL;
        }
      ...
      if (newhop->type == NEXTHOP_TYPE_BLACKHOLE)
        {
          if (IS_ZEBRA_DEBUG_NHG)
            zlog_debug ("%s: id %u, blackhole nexthop not supported", __func__,
                        id);
          return NULL;
        }
      ...
      if (newhop->type == NEXTHOP_TYPE_IFINDEX)
        {
          if (IS_ZEBRA_DEBUG_NHG)
            zlog_debug ("%s: id %u, nexthop without gateway not supported",
                        __func__, id);
          return NULL;
        }
      ...
      if (! newhop->ifindex)
        {
          if (IS_ZEBRA_DEBUG_NHG)
            zlog_debug ("%s: id %u, nexthop without ifindex is not supported",
                        __func__, id);
          return NULL;
        }
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: %s nhe %p (%u), vrf %d, type %s", __func__,
                (replace ? "replaced" : "added"), new, new->id, new->vrf_id,
...
struct nhg_hash_entry *
zebra_nhg_proto_del (uint32_t id, int type)
{
  struct nhg_hash_entry *nhe;
  ...
  if (type != nhe->type)
    {
      if (IS_ZEBRA_DEBUG_NHG)
        zlog_debug ("%s: id %u, type %s mismatch, sent by %s, ignoring",
  ...
  if (nhe->refcnt > 1)
    {
      if (IS_ZEBRA_DEBUG_NHG)
        zlog_debug ("%s: %pNG, still being used by routes refcnt %u", __func__,
                    nhe, nhe->refcnt);
      return nhe;
    }

  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: deleted nhe %p (%pNG), vrf %d, type %s", __func__, nhe,
                nhe, nhe->vrf_id, zebra_route_string (nhe->type));
...
static void
zebra_nhg_score_proto_entry (struct hash_bucket *bucket, void *arg)
{
  struct nhg_hash_entry *nhe;
  ...
  if (nhe->type == iter->type && PROTO_OWNED (nhe))
    {
      if (IS_ZEBRA_DEBUG_NHG_DETAIL)
        zlog_debug (
            ...
            __func__, nhe, nhe, nhe->vrf_id, zebra_route_string (nhe->type));
...
unsigned long
zebra_nhg_score_proto (int type)
{
  struct nhg_hash_entry *nhe;
  struct nhg_score_proto_iter iter = {};
  struct listnode *ln;
...
static ssize_t
printfrr_nhghe (struct fbuf *buf, struct printfrr_eargs *ea, const void *ptr)
{
 ...
 */
void
zebra_interface_nhg_reinstall (struct interface *ifp)
{
  struct nhg_connected *rb_node_dep = NULL;
  ...
  if (IS_ZEBRA_DEBUG_NHG_DETAIL)
    zlog_debug ("%s: Installing interface %s associated NHGs into kernel",
                __func__, ifp->name);
          ...
          zebra_nhg_depends_is_empty (rb_node_dep->nhe))
        {
          ...
          if (IS_ZEBRA_DEBUG_NHG)
            zlog_debug ("%s install nhe %pNG nh type %u flags 0x%x", __func__,
                        rb_node_dep->nhe, nh->type, rb_node_dep->nhe->flags);
          zebra_nhg_install_kernel (rb_node_dep->nhe);
          ...
          frr_each_safe (nhg_connected_tree, &rb_node_dep->nhe->nhg_dependents,
            ...
            {
              if (IS_ZEBRA_DEBUG_NHG)
                zlog_debug ("%s dependent nhe %pNG unset installed flag",
