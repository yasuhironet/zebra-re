...
static void
dump_on_off (uint32_t ival, const char *prefix)
{
  zlog_debug ("%s%s", prefix, (ival != 0) ? "on" : "off");
}

static inline void
flag_write (int flags, int flag, const char *flagstr, char *buf, size_t buflen)
{
...
const char *
nlmsg_flags2str (uint16_t flags, char *buf, size_t buflen)
{
  const char *bufp = buf;
...
const char *
if_flags2str (uint32_t flags, char *buf, size_t buflen)
{
  const char *bufp = buf;
...
const char *
rtm_flags2str (uint32_t flags, char *buf, size_t buflen)
{
  const char *bufp = buf;
...
const char *
neigh_state2str (uint32_t flags, char *buf, size_t buflen)
{
  const char *bufp = buf;
...
const char *
neigh_flags2str (uint32_t flags, char *buf, size_t buflen)
{
  const char *bufp = buf;
...
const char *
ifa_flags2str (uint32_t flags, char *buf, size_t buflen)
{
  const char *bufp = buf;
...
const char *
nh_flags2str (uint32_t flags, char *buf, size_t buflen)
{
  const char *bufp = buf;
 ...
 */
static void
nllink_pdr_dump (struct rtattr *rta, size_t msglen)
{
  size_t plen;
  uint32_t u32v;
...
static void
nllink_linkinfo_dump (struct rtattr *rta, size_t msglen)
{
  size_t plen;
  char dbuf[128];
...
static void
nllink_dump (struct ifinfomsg *ifi, size_t msglen)
{
  uint8_t *datap;
  struct rtattr *rta;
  size_t plen, it;
...
static void
nlroute_dump (struct rtmsg *rtm, size_t msglen)
{
...
static void
nlneigh_dump (struct ndmsg *ndm, size_t msglen)
{
  struct rtattr *rta;
  uint8_t *datap;
  size_t plen, it;
...
static void
nlifa_dump (struct ifaddrmsg *ifa, size_t msglen)
{
  struct rtattr *rta;
  size_t plen;
  uint32_t u32v;
...
static void
nltnl_dump (struct tunnel_msg *tnlm, size_t msglen)
{
  struct rtattr *attr;
...
static void
nlnh_dump (struct nhmsg *nhm, size_t msglen)
{
  struct rtattr *rta;
  int ifindex;
  size_t plen;
...
next_rta:
  ...
  switch (rta_type)
    ...
    case NHA_GROUP:
      ...
      for (i = 0; i < count; i++)
        zlog_debug ("      id %d weight %d", nhgrp[i].id, nhgrp[i].weight);
      break;
    case NHA_ENCAP_TYPE:
...
static void
nlrule_dump (struct fib_rule_hdr *frh, size_t msglen)
{
  struct rtattr *rta;
  size_t plen;
  uint8_t u8v;
...
static void
nlncm_dump (const struct netconfmsg *ncm, size_t msglen)
{
  ...
  rta =
      (void *) ((const char *) ncm + NLMSG_ALIGN (sizeof (struct netconfmsg)));
...
void
nl_dump (void *msg, size_t msglen)
{
  ...
  switch (nlmsg->nlmsg_type)
    ...
    case RTM_GETROUTE:
      ...
      zlog_debug (
          ...
          rtm->rtm_src_len, rtm->rtm_tos, rtm->rtm_table, rtm->rtm_protocol,
    ...
    case RTM_DELNEIGH:
      ...
      zlog_debug (
          ...
          ndm->ndm_flags,
          neigh_flags2str (ndm->ndm_flags, fbuf, sizeof (fbuf)), ndm->ndm_type,
          rtm_type2str (ndm->ndm_type));
    ...
    case RTM_DELRULE:
      ...
      zlog_debug (
          ...
          frh->family, af_type2str (frh->family), frh->dst_len, frh->src_len,
          frh->tos, frh->table, frh->res1, frh->res2, frh->action,
          frh_action2str (frh->action), frh->flags);
    ...
    case RTM_DELADDR:
      ...
      zlog_debug (
          ...
          ifa->ifa_family, af_type2str (ifa->ifa_family), ifa->ifa_prefixlen,
          ifa->ifa_flags, if_flags2str (ifa->ifa_flags, fbuf, sizeof (fbuf)),
    ...
    case RTM_DELNETCONF:
      ...
      zlog_debug (" ncm [family=%s (%d)]", af_type2str (ncm->ncm_family),
                  ncm->ncm_family);
