...
#define WALK_FINISH(fnc, ev)                                                  \
  event_add_event ((fnc)->fthread->master, fpm_process_event, (fnc), (ev),    \
                   NULL)
...
static int fpm_nl_enqueue (struct fpm_nl_ctx *fnc,
  ...
  if (inet_pton (AF_INET, argv[2]->arg, naddr) == 1)
    {
      ...
      sin->sin_port = port ? htons (port) : htons (SOUTHBOUND_DEFAULT_PORT);
...
DEFUN (no_fpm_set_address,
       ...
       "Remote FPM server port\n")
{
  event_add_event (gfnc->fthread->master, fpm_process_event, gfnc, FNE_DISABLE,
                   &gfnc->t_event);
  return CMD_SUCCESS;
}
...
#define SHOW_COUNTER(label, counter)                                          \
  ...
  SHOW_COUNTER ("Data plane items processed", gfnc->counters.dplane_contexts);
...
static int
fpm_write_config (struct vty *vty)
{
  struct sockaddr_in *sin;
  struct sockaddr_in6 *sin6;
  int written = 0;
...
static void
fpm_reconnect (struct fpm_nl_ctx *fnc)
{
  ...
  event_add_timer (fnc->fthread->master, fpm_connect, fnc, 3, &fnc->t_connect);
}

static void
fpm_read (struct event *t)
{
  ...
  rv = stream_read_try (fnc->ibuf, fnc->socket, STREAM_WRITEABLE (fnc->ibuf));
  if (rv == 0)
    {
  ...
  while (available_bytes)
    {
      ...
      if (hdr->nlmsg_len < sizeof (*hdr))
        {
          zlog_warn ("%s: [seq=%u] invalid message length %u (< %zu)",
                     __func__, hdr->nlmsg_seq, hdr->nlmsg_len, sizeof (*hdr));
          continue;
        }
      ...
      switch (hdr->nlmsg_type)
        ...
        case RTM_NEWROUTE:
          ...
          if (netlink_route_change_read_unicast_internal (hdr, 0, false,
                                                          ctx) != 1)
            {
              dplane_ctx_fini (&ctx);
              stream_pulldown (fnc->ibuf);
  ...
  if (fnc->connecting == true)
    {
      ...
      rv = getsockopt (fnc->socket, SOL_SOCKET, SO_ERROR, &status, &statuslen);
      if (rv == -1 || status != 0)
        {
          if (rv != -1)
            ...
            zlog_warn ("%s: SO_ERROR failed: %s", __func__, strerror (status));
...
fpm_connect (struct event *t)
{
  ...
  if (rv == -1 && errno != EINPROGRESS)
    {
      atomic_fetch_add_explicit (&fnc->counters.connection_errors, 1,
                                 memory_order_relaxed);
      close (sock);
      zlog_warn ("%s: fpm connection failed: %s", __func__, strerror (errno));
      event_add_timer (fnc->fthread->master, fpm_connect, fnc, 3,
                       &fnc->t_connect);
      return;
    }
  ...
  fnc->socket = sock;
  if (! fnc->connecting)
    event_add_read (fnc->fthread->master, fpm_read, fnc, sock, &fnc->t_read);
  event_add_write (fnc->fthread->master, fpm_write, fnc, sock, &fnc->t_write);
   ...
   */
  if (! fnc->connecting)
    event_add_timer (zrouter.master, fpm_lsp_reset, fnc, 0, &fnc->t_lspreset);
}
 ...
 */
static int
fpm_nl_enqueue (struct fpm_nl_ctx *fnc, struct zebra_dplane_ctx *ctx)
{
  ...
  switch (op)
    ...
    case DPLANE_OP_ROUTE_DELETE:
      rv = netlink_route_multipath_msg_encode (
          RTM_DELROUTE, ctx, nl_buf, sizeof (nl_buf), true, fnc->use_nhg);
      if (rv <= 0)
        {
          zlog_err ("%s: netlink_route_multipath_msg_encode failed", __func__);
          return 0;
        }
    ...
    case DPLANE_OP_ROUTE_INSTALL:
      rv = netlink_route_multipath_msg_encode (
          RTM_NEWROUTE, ctx, &nl_buf[nl_buf_len], sizeof (nl_buf) - nl_buf_len,
          true, fnc->use_nhg);
      if (rv <= 0)
        {
          zlog_err ("%s: netlink_route_multipath_msg_encode failed", __func__);
    ...
    case DPLANE_OP_MAC_DELETE:
      ...
      if (rv <= 0)
        {
          zlog_err ("%s: netlink_macfdb_update_ctx failed", __func__);
          return 0;
        }
    ...
    case DPLANE_OP_NH_DELETE:
      ...
      if (rv <= 0)
        {
          zlog_err ("%s: netlink_nexthop_msg_encode failed", __func__);
          return 0;
        }
  ...
  atomic_fetch_add_explicit (&fnc->counters.obuf_bytes,
                             ...
                             memory_order_relaxed);
  obytes =
      atomic_load_explicit (&fnc->counters.obuf_bytes, memory_order_relaxed);
  obytes_peak =
      atomic_load_explicit (&fnc->counters.obuf_peak, memory_order_relaxed);
...
static int
fpm_lsp_send_cb (struct hash_bucket *bucket, void *arg)
{
...
fpm_lsp_send (struct event *t)
{
  ...
  if (fla.complete)
  ...
  else
    {
      ...
      event_add_timer (zrouter.master, fpm_lsp_send, fnc, 0, &fnc->t_lspwalk);
    }
}
...
static int
fpm_nhg_send_cb (struct hash_bucket *bucket, void *arg)
{
...
fpm_nhg_send (struct event *t)
{
  ...
  if (fna.complete)
    ...
    event_add_timer (zrouter.master, fpm_nhg_send, fnc, 0, &fnc->t_nhgwalk);
}
...
fpm_rib_send (struct event *t)
{
          ...
          if (fpm_nl_enqueue (fnc, ctx) == -1)
            {
              ...
              event_add_timer (zrouter.master, fpm_rib_send, fnc, 1,
                               &fnc->t_ribwalk);
              return;
            }
  ...
  event_add_event (zrouter.master, fpm_rmac_reset, fnc, 0, &fnc->t_rmacreset);
}
...
static void
fpm_enqueue_rmac_table (struct hash_bucket *bucket, void *arg)
{
  ...
  dplane_mac_init (fra->ctx, fra->zl3vni->vxlan_if, zif->brslave_info.br_if,
                   vid, &zrmac->macaddr, vni->vni, zrmac->fwd_info.r_vtep_ip,
                   sticky, 0 /*nhg*/, 0 /*update_flags*/);
...
static void
fpm_enqueue_l3vni_table (struct hash_bucket *bucket, void *arg)
{
  struct fpm_rmac_arg *fra = arg;
 ...
 */
static void
fpm_nhg_reset_cb (struct hash_bucket *bucket, void *arg)
{
  struct nhg_hash_entry *nhe = bucket->data;
 ...
 */
static void
fpm_lsp_reset_cb (struct hash_bucket *bucket, void *arg)
{
  struct zebra_lsp *lsp = bucket->data;
 ...
 */
static void
fpm_unset_rmac_table (struct hash_bucket *bucket, void *arg)
{
  struct zebra_mac *zrmac = bucket->data;
...
static void
fpm_unset_l3vni_table (struct hash_bucket *bucket, void *arg)
{
  struct zebra_l3vni *zl3vni = bucket->data;
...
static void
fpm_process_queue (struct event *t)
{
  struct fpm_nl_ctx *fnc = EVENT_ARG (t);
  struct zebra_dplane_ctx *ctx;
  bool no_bufs = false;
 ...
 */
static void
fpm_process_event (struct event *t)
{
  struct fpm_nl_ctx *fnc = EVENT_ARG (t);
  ...
  switch (event)
    ...
    case FNE_NHG_FINISHED:
      if (IS_ZEBRA_DEBUG_FPM)
        zlog_debug ("%s: next hop groups walk finished", __func__);
      break;
 ...
 */
static int
fpm_nl_start (struct zebra_dplane_provider *prov)
{
  struct fpm_nl_ctx *fnc;
...
static int
fpm_nl_finish_early (struct fpm_nl_ctx *fnc)
{
...
static int
fpm_nl_finish_late (struct fpm_nl_ctx *fnc)
{
...
static int
fpm_nl_finish (struct zebra_dplane_provider *prov, bool early)
{
  struct fpm_nl_ctx *fnc;
...
static int
fpm_nl_process (struct zebra_dplane_provider *prov)
{
  struct zebra_dplane_ctx *ctx;
  struct fpm_nl_ctx *fnc;
  int counter, limit;
      ...
      if (fnc->socket != -1 && fnc->connecting == false)
        {
           ...
           */
          atomic_fetch_add_explicit (&fnc->counters.ctxqueue_len, 1,
                                     memory_order_relaxed);
  ...
  stored_peak_queue = atomic_load_explicit (&fnc->counters.ctxqueue_len_peak,
                                            memory_order_relaxed);
  if (stored_peak_queue < peak_queue)
    atomic_store_explicit (&fnc->counters.ctxqueue_len_peak, peak_queue,
                           memory_order_relaxed);
...
static int
fpm_nl_new (struct event_loop *tm)
{
  ...
  rv = dplane_provider_register (prov_name, DPLANE_PRIO_POSTPROCESS,
                                 ...
                                 fpm_nl_process, fpm_nl_finish, gfnc, &prov);
