       ...
       "Specify SRv6 locator function length in bits\n")
{
  ...
  if (func_bit_len > 20)
    {
      vty_out (vty, "%% currently func_bit_len > 20 is not supported\n");
      return CMD_WARNING_CONFIG_FAILED;
    }
          ...
          if (memcmp (&chunk->prefix.prefix, zero, 16) == 0)
            {
              ...
              chunk->prefix = *prefix;
              for (ALL_LIST_ELEMENTS_RO (zrouter.client_list, client_node,
                                         client))
                {
...
static int
zebra_sr_config (struct vty *vty)
{
  struct zebra_srv6 *srv6 = zebra_srv6_get_default ();
  struct listnode *node;
  struct srv6_locator *locator;
      ...
      for (ALL_LIST_ELEMENTS_RO (srv6->locators, node, locator))
        {
          ...
          vty_out (vty, "    prefix %s/%u", str, locator->prefix.prefixlen);
