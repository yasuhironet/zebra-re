...
static struct vrf *
vrf_handler_create (struct vty *vty, const char *vrfname)
{
  if (strlen (vrfname) > VRF_NAMSIZ)
    {
      flog_warn (EC_LIB_VRF_LENGTH,
                 "%% VRF name %s invalid: length exceeds %d bytes", vrfname,
                 VRF_NAMSIZ);
      return NULL;
    }
...
static void
zebra_ns_notify_create_context_from_entry_name (const char *name)
{
  ...
  frr_with_privs (&zserv_privs)
    {
      ret = zebra_vrf_netns_handler_create (
          NULL, vrf, netnspath, ns_id_external, ns_id, ns_id_relative);
    }
  if (ret != CMD_SUCCESS)
    {
      flog_warn (EC_ZEBRA_NS_VRF_CREATION_FAILED,
...
static int
zebra_ns_continue_read (struct zebra_netns_info *zns_info, int stop_retry)
{
...
static int
zebra_ns_notify_self_identify (struct stat *netst)
{
...
static bool
zebra_ns_notify_is_default_netns (const char *name)
{
...
static void
zebra_ns_ready_read (struct event *t)
{
  ...
  if (strmatch (VRF_DEFAULT_NAME, basename (netnspath)))
    {
      zlog_warn (
          "NS notify : NS %s is already default VRF.Cancel VRF Creation",
          basename (netnspath));
...
static void
zebra_ns_notify_read (struct event *t)
{
  ...
  for (event = (struct inotify_event *) buf; (char *) event < &buf[len];
                                         ...
                                         event->len))
    {
      ...
      if (offsetof (struct inotify_event, name) + event->len >= sizeof (buf))
        {
      ...
      netnsinfo = XCALLOC (MTYPE_NETNS_MISC, sizeof (struct zebra_netns_info));
...
zebra_ns_notify_parse (void)
{
  ...
  if (srcdir == NULL)
    {
      flog_err_sys (EC_LIB_SYSTEM_CALL, "NS parsing init: failed to parse %s",
                    NS_RUN_DIR);
      return;
    }
