 ...
 */
void
zebra_gr_stale_client_cleanup (struct list *client_list)
{
  ...
  for (ALL_LIST_ELEMENTS (client_list, node, nnode, s_client))
    {
      ...
      TAILQ_FOREACH_SAFE (info, &s_client->gr_info_queue, gr_info, ninfo)
        {
...
zebra_gr_client_info_create (struct zserv *client)
{
  struct client_gr_info *info;
 ...
 */
static void
zebra_gr_client_info_delete (struct zserv *client, struct client_gr_info *info)
{
 ...
 */
int32_t
zebra_gr_client_disconnect (struct zserv *client)
{
  ...
  if (stale_client)
    {
      LOG_GR ("%s: Stale client %s exist, we should not be here!", __func__,
      ...
      if (ZEBRA_CLIENT_GR_ENABLED (info->capabilities) &&
          (info->t_stale_removal == NULL))
        {
          ...
          event_add_timer (zrouter.master,
                           zebra_gr_route_stale_delete_timer_expiry, info,
                           info->stale_removal_time, &info->t_stale_removal);
          info->stale_client_ptr = client;
 ...
 */
static struct zserv *
zebra_gr_find_stale_client (struct zserv *client)
{
  struct listnode *node, *nnode;
  struct zserv *stale_client;
 ...
 */
void
zebra_gr_client_reconnect (struct zserv *client)
{
  struct listnode *node, *nnode;
  ...
  /* Find the stale client */
  for (ALL_LIST_ELEMENTS (zrouter.stale_client_list, node, nnode, old_client))
    {
 ...
 */
void
zread_client_capabilities (ZAPI_HANDLER_ARGS)
{
  struct zapi_cap api;
  struct client_gr_info *info = NULL;
  struct stream *s;
  ...
  if (zapi_capabilities_decode (s, &api))
    {
      LOG_GR ("%s: Error in reading capabilities for client %s", __func__,
              zebra_route_string (client->proto));
      return;
    }
  ...
  if (client->proto <= ZEBRA_ROUTE_CONNECT)
    {
      LOG_GR ("%s: GR capabilities for client %s not supported", __func__,
              zebra_route_string (client->proto));
      return;
    }
  ...
  switch (api.cap)
      ...
      if (! info->gr_enable)
        {
          ...
          LOG_GR ("%s: Cient %s vrf %s(%u) GR enabled count %d", __func__,
    ...
    case ZEBRA_CLIENT_ROUTE_UPDATE_COMPLETE:
      if (! info)
        {
          LOG_GR (
              "%s: Client %s route update complete for AFI %d, SAFI %d, no Graceful Restart communication, returning",
              __func__, zebra_route_string (client->proto), api.afi, api.safi);
          return;
        }

      LOG_GR (
          "%s: Client %s vrf %s(%u) route update complete for AFI %d, SAFI %d",
          __func__, zebra_route_string (client->proto), VRF_LOGNAME (vrf),
    ...
    case ZEBRA_CLIENT_ROUTE_UPDATE_PENDING:
      if (! info)
        ...
        {
          LOG_GR (
              "%s: Client %s vrf %s(%u) route update pending for AFI %d, SAFI %d",
              __func__, zebra_route_string (client->proto), VRF_LOGNAME (vrf),
              info->vrf_id, api.afi, api.safi);
 ...
 */
static void
zebra_gr_route_stale_delete_timer_expiry (struct event *thread)
{
  ...
  if (cnt > 0)
    {
      LOG_GR (
          "%s: Client %s vrf %s(%u) processed %d routes. Start timer again",
          __func__, zebra_route_string (client->proto), VRF_LOGNAME (vrf),
          info->vrf_id, cnt);

      event_add_timer (
          zrouter.master, zebra_gr_route_stale_delete_timer_expiry, info,
          ZEBRA_DEFAULT_STALE_UPDATE_DELAY, &info->t_stale_removal);
    }
  else
    {
      /* No routes to delete for the VRF */
      LOG_GR ("%s: Client %s vrf %s(%u) all stale routes processed", __func__,
              zebra_route_string (client->proto), VRF_LOGNAME (vrf),
              info->vrf_id);
 ...
 */
static bool
zebra_gr_process_route_entry (struct zserv *client, struct route_node *rn,
  ...
  if (re->uptime < client->restart_time)
    {
      if (IS_ZEBRA_DEBUG_RIB)
        zlog_debug ("%s: Client %s stale route %pFX is deleted", __func__,
...
static void
zebra_gr_delete_stale_route_table_afi (struct event *event)
{
      ...
      RNODE_FOREACH_RE_SAFE (rn, re, next)
        {
          ...
          if (re->type == gac->proto && re->instance == gac->instance &&
              zebra_gr_process_route_entry (gac->info->stale_client_ptr, rn,
                                            re))
            n++;
          ...
          if ((n >= ZEBRA_MAX_STALE_ROUTE_COUNT) &&
            ...
            {
              event_add_timer (zrouter.master,
                               zebra_gr_delete_stale_route_table_afi, gac,
                               ZEBRA_DEFAULT_STALE_UPDATE_DELAY, &gac->t_gac);
            }
        }
    }
 ...
 */
static int32_t
zebra_gr_delete_stale_route (struct client_gr_info *info,
                             struct zebra_vrf *zvrf)
{
  ...
  if (s_client == NULL)
    {
      LOG_GR ("%s: Stale client %s(%u) not present", __func__, zvrf->vrf->name,
              zvrf->vrf->vrf_id);
      return -1;
    }
 ...
 */
static int32_t
zebra_gr_delete_stale_routes (struct client_gr_info *info)
{
  struct zebra_vrf *zvrf;
  uint64_t cnt = 0;
 ...
 */
static void
zebra_gr_process_client_stale_routes (struct zserv *client,
                                      struct client_gr_info *info)
{
  afi_t afi;
      ...
      if (info->af_enabled[afi] && ! info->route_sync[afi])
        {
          ...
          LOG_GR (
              "%s: Client %s vrf: %s(%u) route update not completed for AFI %d",
              __func__, zebra_route_string (client->proto), VRF_LOGNAME (vrf),
              info->vrf_id, afi);
          return;
        }
  ...
  if (info->t_stale_removal)
    {
      ...
      LOG_GR ("%s: Client %s canceled stale delete timer vrf %s(%d)", __func__,
              zebra_route_string (client->proto), VRF_LOGNAME (vrf),
              info->vrf_id);
      EVENT_OFF (info->t_stale_removal);
    }
...
void
zebra_gr_process_client (afi_t afi, vrf_id_t vrf_id, uint8_t proto,
                         uint8_t instance)
{
  ...
  event_add_event (zrouter.master, zebra_gr_delete_stale_route_table_afi, gac,
                   0, &gac->t_gac);
}
