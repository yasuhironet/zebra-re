...
#define SHOW_REMOTE_NEIGH_FROM_VTEP 0x8
  ...
  /**************************** SYNC neigh handling **************************/
  static inline bool
  zebra_evpn_neigh_is_static (struct zebra_neigh *neigh)
  {
    return ! ! (neigh->flags & ZEBRA_NEIGH_ALL_PEER_FLAGS);
  }

  static inline bool
  zebra_evpn_neigh_is_ready_for_bgp (struct zebra_neigh *n)
  {
    bool mac_ready;
    bool neigh_ready;
  ...
  static inline void
  zebra_evpn_neigh_stop_hold_timer (struct zebra_neigh *n)
  {
    if (! n->hold_timer)
      return;
  ...
  void zebra_evpn_sync_neigh_static_chg (struct zebra_neigh *n,
                                         bool old_n_static, bool new_n_static,
                                         bool defer_n_dp, bool defer_mac_dp,
                                         const char *caller);

  static inline bool
  zebra_evpn_neigh_clear_sync_info (struct zebra_neigh *n)
  {
  ...
  void zebra_evpn_find_neigh_addr_width (struct hash_bucket *bucket,
                                         void *ctxt);
  int remote_neigh_count (struct zebra_mac *zmac);
  ...
  void zebra_evpn_clear_dup_neigh_hash (struct hash_bucket *bucket,
                                        void *ctxt);
  ...
  void zebra_evpn_print_neigh_hdr (struct vty *vty,
                                   struct neigh_walk_ctx *wctx);
  void zebra_evpn_print_neigh_hash_detail (struct hash_bucket *bucket,
                                           void *ctxt);
  void zebra_evpn_print_dad_neigh_hash (struct hash_bucket *bucket,
                                        void *ctxt);
  ...
  void zebra_evpn_neigh_remote_uninstall (struct zebra_evpn *zevpn,
                                          ...
                                          struct zebra_mac *mac,
                                          const struct ipaddr *ipaddr);
  int zebra_evpn_neigh_del_ip (struct zebra_evpn *zevpn,
                               const struct ipaddr *ip);
