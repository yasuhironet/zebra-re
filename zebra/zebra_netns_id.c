 ...
 * those routines are implemented locally to avoid having external
 * dependencies. Copyright (C) 2018 6WIND
 */
...
initiate_nlh (char *buf, unsigned int *seq, int type)
{
  struct nlmsghdr *nlh;
...
static int
send_receive (int sock, struct nlmsghdr *nlh, unsigned int seq, char *buf)
{
  int ret;
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "netlink( %u) sendmsg() error: %s", sock,
                    safe_strerror (errno));
      return -1;
    }
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "netlink recvmsg: error %d (errno %u)", ret,
                    errno);
      return -1;
    }
  ...
  if (nlh->nlmsg_seq != seq)
    {
      flog_err (EC_ZEBRA_NETLINK_BAD_SEQUENCE,
                "netlink recvmsg: bad sequence number %x (expected %x)", seq,
                nlh->nlmsg_seq);
      return -1;
    }
  return ret;
 ...
 */
static ns_id_t
extract_nsid (struct nlmsghdr *nlh, char *buf)
{
 ...
 */
ns_id_t
zebra_ns_id_get (const char *netnspath, int fd_param)
{
  int ns_id = -1;
  ...
  if (sock < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "netlink( %u) socket() error: %s", sock,
                    safe_strerror (errno));
      if (netnspath)
        close (fd);
      return NS_UNKNOWN;
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "netlink( %u) socket() bind error: %s",
...
#else
ns_id_t
zebra_ns_id_get (const char *netnspath, int fd __attribute__ ((unused)))
{
  return zebra_ns_id_get_fallback (netnspath);
}
...
#ifdef HAVE_NETNS
static void
zebra_ns_create_netns_directory (void)
{
