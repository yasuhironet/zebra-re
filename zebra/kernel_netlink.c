 ...
 * underlying buffer since the last message that wouldn't fit the batch would
 * go over the upper boundary and then it would have to be encoded again into a
 * new buffer. If the difference between the limit and the length of the buffer
 * is big enough (bigger than the biggest Netlink message) then this situation
 * won't occur.
 */
...
static const struct message rtproto_str[] = {
  ...
  { RTPROT_BIRD, "BIRD" },
#endif /* RTPROT_BIRD */
  { RTPROT_MROUTED, "mroute" },    { RTPROT_BGP, "BGP" },
  { RTPROT_OSPF, "OSPF" },         { RTPROT_ISIS, "IS-IS" },
  { RTPROT_RIP, "RIP" },           { RTPROT_RIPNG, "RIPNG" },
  { RTPROT_ZSTATIC, "static" },    { 0 }
};
...
int
netlink_config_write_helper (struct vty *vty)
{
  uint32_t size =
      atomic_load_explicit (&nl_batch_bufsize, memory_order_relaxed);
  uint32_t threshold =
      atomic_load_explicit (&nl_batch_send_threshold, memory_order_relaxed);
...
void
netlink_set_batch_buffer_size (uint32_t size, uint32_t threshold, bool set)
{
  if (! set)
    {
...
int
netlink_talk_filter (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  /*
...
netlink_recvbuf (struct nlsock *nl, uint32_t newsize)
{
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "Can't get %s receive buffer size: %s",
  ...
  frr_with_privs (&zserv_privs)
    {
      ret = setsockopt (nl->sock, SOL_SOCKET, SO_RCVBUFFORCE, &rcvbufsize,
                        sizeof (rcvbufsize));
    }
  if (ret < 0)
  ...
  if (ret < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "Can't set %s receive buffer size: %s",
                    nl->name, safe_strerror (errno));
      return -1;
    }
...
static int
netlink_socket (struct nlsock *nl, unsigned long groups, uint32_t ext_groups[],
                uint8_t ext_group_size, ns_id_t ns_id)
{
  ...
  if (ret < 0 || namelen != sizeof (snl))
    {
      flog_err_sys (EC_LIB_SOCKET, "Can't get %s socket name: %s", nl->name,
                    safe_strerror (errno));
      close (sock);
      return -1;
    }
 ...
 */
static int
netlink_information_fetch (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  /*
  ...
  switch (h->nlmsg_type)
    ...
    default:
      ...
      flog_err (EC_ZEBRA_UNKNOWN_NLMSG,
                ...
                nl_msg_type_to_str (h->nlmsg_type), h->nlmsg_type, ns_id);
      break;
    }
  return 0;
}
 ...
 */
static int
dplane_netlink_information_fetch (struct nlmsghdr *h, ns_id_t ns_id,
                                  int startup)
{
  /*
...
static void
kernel_read (struct event *thread)
{
  ...
  netlink_parse_info (netlink_information_fetch, &zns->netlink, &dp_info, 5,
                      false);
 ...
 */
int
kernel_dplane_read (struct zebra_dplane_info *info)
{
  struct nlsock *nl = kernel_netlink_nlsock_lookup (info->sock);

  netlink_parse_info (dplane_netlink_information_fetch, nl, info, 5, false);
 ...
 */
static void
netlink_install_filter (int sock, uint32_t pid, uint32_t dplane_pid)
{
  /*
  ...
  struct sock_filter filter[] = {
     ...
     */
    BPF_JUMP (BPF_JMP | BPF_JEQ | BPF_K, htons (RTM_NEWNETCONF), 2, 0),
    /*
     * 7: Compare to RTM_DELNETCONF
     */
    BPF_JUMP (BPF_JMP | BPF_JEQ | BPF_K, htons (RTM_DELNETCONF), 1, 0),
    /*
...
void
netlink_parse_rtattr_flags (struct rtattr **tb, int max, struct rtattr *rta,
 ...
 */
void
netlink_parse_rtattr_nested (struct rtattr **tb, int max, struct rtattr *rta)
{
  netlink_parse_rtattr (tb, max, RTA_DATA (rta), RTA_PAYLOAD (rta));
}

bool
nl_addraw_l (struct nlmsghdr *n, unsigned int maxlen, const void *data,
             unsigned int len)
{
...
bool
nl_attr_put (struct nlmsghdr *n, unsigned int maxlen, int type,
...
bool
nl_attr_put8 (struct nlmsghdr *n, unsigned int maxlen, int type, uint8_t data)
{
  return nl_attr_put (n, maxlen, type, &data, sizeof (uint8_t));
}

bool
nl_attr_put16 (struct nlmsghdr *n, unsigned int maxlen, int type,
               uint16_t data)
{
...
bool
nl_attr_put32 (struct nlmsghdr *n, unsigned int maxlen, int type,
               uint32_t data)
{
...
bool
nl_attr_put64 (struct nlmsghdr *n, unsigned int maxlen, int type,
               uint64_t data)
{
...
struct rtattr *
nl_attr_nest (struct nlmsghdr *n, unsigned int maxlen, int type)
{
...
int
nl_attr_nest_end (struct nlmsghdr *n, struct rtattr *nest)
{
...
struct rtnexthop *
nl_attr_rtnh (struct nlmsghdr *n, unsigned int maxlen)
{
...
void
nl_attr_rtnh_end (struct nlmsghdr *n, struct rtnexthop *rtnh)
{
  rtnh->rtnh_len = (uint8_t *) NLMSG_TAIL (n) - (uint8_t *) rtnh;
}

bool
nl_rta_put (struct rtattr *rta, unsigned int maxlen, int type,
...
bool
nl_rta_put16 (struct rtattr *rta, unsigned int maxlen, int type, uint16_t data)
{
  return nl_rta_put (rta, maxlen, type, &data, sizeof (uint16_t));
}

bool
nl_rta_put64 (struct rtattr *rta, unsigned int maxlen, int type, uint64_t data)
{
  return nl_rta_put (rta, maxlen, type, &data, sizeof (uint64_t));
}

struct rtattr *
nl_rta_nest (struct rtattr *rta, unsigned int maxlen, int type)
{
...
int
nl_rta_nest_end (struct rtattr *rta, struct rtattr *nest)
{
...
const char *
nl_rtproto_to_str (uint8_t rtproto)
{
  return lookup_msg (rtproto_str, rtproto, "");
}

const char *
nl_family_to_str (uint8_t family)
{
  return lookup_msg (family_str, family, "");
}

const char *
nl_rttype_to_str (uint8_t rttype)
{
...
static void
netlink_parse_nlattr (struct nlattr **tb, int max, struct nlattr *nla, int len)
{
...
static void
netlink_parse_extended_ack (struct nlmsghdr *h)
{
  ...
  netlink_parse_nlattr (tb, NLMSGERR_ATTR_MAX, ERR_NLA (err, inner_len), len);
  ...
  if (msg && *msg != '\0')
    {
      ...
      if (is_err)
        ...
        flog_warn (EC_ZEBRA_NETLINK_EXTENDED_WARNING, "Extended Warning: %s",
                   msg);
    }
}
 ...
 */
static ssize_t
netlink_send_msg (const struct nlsock *nl, void *buf, size_t buflen)
{
  struct sockaddr_nl snl = {};
  struct iovec iov = {};
  struct msghdr msg = {};
 ...
 */
static int
netlink_recv_msg (struct nlsock *nl, struct msghdr *msg)
{
  struct iovec iov;
  int status;
  ...
  if (status == -1)
    {
      if (errno == EWOULDBLOCK || errno == EAGAIN)
        return 0;
      flog_err (EC_ZEBRA_RECVMSG_OVERRUN, "%s recvmsg overrun: %s", nl->name,
                safe_strerror (errno));
      /*
 ...
 */
static int
netlink_parse_error (const struct nlsock *nl, struct nlmsghdr *h, bool is_cmd,
                     bool startup)
{
  ...
  if (h->nlmsg_len < NLMSG_LENGTH (sizeof (struct nlmsgerr)))
    {
      flog_err (EC_ZEBRA_NETLINK_LENGTH_ERROR, "%s error: message truncated",
                nl->name);
      return -1;
    }
  ...
  if (err->error == 0)
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        {
          zlog_debug ("%s: %s ACK: type=%s(%u), seq=%u, pid=%u", __func__,
        ...
        (-errnum == EOPNOTSUPP))))
    {
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: error: %s type=%s(%u), seq=%u, pid=%u", nl->name,
                    safe_strerror (-errnum), nl_msg_type_to_str (msg_type),
  ...
  if (msg_type == RTM_DELNEIGH ||
    ...
    {
      if ((msg_type != RTM_GETNEXTHOP && msg_type != RTM_GETVLAN) || ! startup)
  ...
  while (1)
    {
      ...
      for (h = (struct nlmsghdr *) nl->buf;
           ...
           h = NLMSG_NEXT (h, status))
        {
          ...
          if (h->nlmsg_type == NLMSG_ERROR)
            {
              int err = netlink_parse_error (nl, h, zns->is_cmd, startup);
          ...
          if (error < 0)
            {
              zlog_debug ("%s filter function error", nl->name);
              ret = error;
            }
        }
      ...
      if (status)
        {
          flog_err (EC_ZEBRA_NETLINK_LENGTH_ERROR,
                    "%s error: data remnant size %d", nl->name, status);
          return -1;
        }
    }
  return ret;
                   ...
                   bool startup)
{
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug ("netlink_talk: %s type %s(%u), len=%d seq=%u flags 0x%x",
                nl->name, nl_msg_type_to_str (n->nlmsg_type), n->nlmsg_type,
 ...
 */
int
netlink_talk (int (*filter) (struct nlmsghdr *, ns_id_t, int startup),
 ...
 */
int
netlink_request (struct nlsock *nl, void *req)
{
  ...
  if (nl->sock < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "%s socket isn't active.", nl->name);
      return -1;
    }
...
static int
nl_batch_read_resp (struct nl_batch *bth)
{
  struct nlmsghdr *h;
  struct sockaddr_nl snl;
  struct msghdr msg = {};
  ...
  while (true)
    {
      ...
      if (status == -1 || status == 0)
        {
          while ((ctx = dplane_ctx_dequeue (&(bth->ctx_list))) != NULL)
            {
              if (status == -1)
      ...
      while (true)
        {
          ...
          if (dplane_ctx_get_ns (ctx)->seq > seq)
            zlog_warn (
                ...
                __func__, seq, dplane_ctx_get_ns (ctx)->seq);
        }
      ...
      if (ignore_msg)
        {
           ...
           * what is going on
           */
          int err = netlink_parse_error (nl, h, bth->zns->is_cmd, false);

          zlog_debug ("%s: netlink error message seq=%d %d", __func__,
                      h->nlmsg_seq, err);
          continue;
        }
      ...
      if (h->nlmsg_type == NLMSG_ERROR)
        {
          ...
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug ("%s: netlink error message seq=%d ", __func__,
                        h->nlmsg_seq);
          continue;
        }
      ...
      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: ignoring message type 0x%04x(%s) NS %u", __func__,
                    h->nlmsg_type, nl_msg_type_to_str (h->nlmsg_type),
                    bth->zns->ns_id);
    }
...
static void
nl_batch_reset (struct nl_batch *bth)
{
...
static void
nl_batch_init (struct nl_batch *bth, struct dplane_ctx_list_head *ctx_out_q)
{
  /*
...
static void
nl_batch_send (struct nl_batch *bth)
{
  struct zebra_dplane_ctx *ctx;
  bool err = false;
  ...
  if (bth->curlen != 0 && bth->zns != NULL)
    {
      struct nlsock *nl = kernel_netlink_nlsock_lookup (bth->zns->sock);

      if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug ("%s: %s, batch size=%zu, msg cnt=%zu", __func__, nl->name,
...
netlink_batch_add_msg (struct nl_batch *bth, struct zebra_dplane_ctx *ctx,
                       ssize_t (*msg_encoder) (struct zebra_dplane_ctx *,
                                               void *, size_t),
                       bool ignore_res)
{
  int seq;
...
static enum netlink_msg_status
nl_put_msg (struct nl_batch *bth, struct zebra_dplane_ctx *ctx)
{
...
void
kernel_update_multi (struct dplane_ctx_list_head *ctx_list)
{
...
static void
kernel_netlink_nlsock_insert (struct nlsock *nls)
{
  NLSOCK_LOCK ();
...
static void
kernel_netlink_nlsock_remove (struct nlsock *nls)
{
  NLSOCK_LOCK ();
...
static uint32_t
kernel_netlink_nlsock_key (const void *arg)
{
...
static bool
kernel_netlink_nlsock_hash_equal (const void *arg1, const void *arg2)
{
...
kernel_init (struct zebra_ns *zns)
{
  ...
  zns->netlink.sock = -1;
  if (netlink_socket (&zns->netlink, groups, &ext_groups, 1, zns->ns_id) < 0)
    {
      zlog_err ("Failure to create %s socket", zns->netlink.name);
      exit (-1);
    }
  ...
  if (netlink_socket (&zns->netlink_cmd, 0, 0, 0, zns->ns_id) < 0)
    {
      zlog_err ("Failure to create %s socket", zns->netlink_cmd.name);
      exit (-1);
    }
  ...
  if (netlink_socket (&zns->netlink_dplane_out, 0, 0, 0, zns->ns_id) < 0)
    {
      zlog_err ("Failure to create %s socket", zns->netlink_dplane_out.name);
      exit (-1);
    }
  ...
  snprintf (zns->netlink_dplane_in.name, sizeof (zns->netlink_dplane_in.name),
            "netlink-dp-in (NS %u)", zns->ns_id);
  zns->netlink_dplane_in.sock = -1;
  if (netlink_socket (&zns->netlink_dplane_in, dplane_groups, 0, 0,
                      zns->ns_id) < 0)
    {
      zlog_err ("Failure to create %s socket", zns->netlink_dplane_in.name);
      exit (-1);
    }
  ...
  if (ret < 0)
    zlog_notice ("Registration for RTNLGRP_BRVLAN Membership failed : %d %s",
   ...
   */
  one = 1;
  ret = setsockopt (zns->netlink_cmd.sock, SOL_NETLINK, NETLINK_EXT_ACK, &one,
                    sizeof (one));

  if (ret < 0)
    zlog_notice ("Registration for extended cmd ACK failed : %d %s", errno,
                 safe_strerror (errno));

  one = 1;
  ret = setsockopt (zns->netlink_dplane_out.sock, SOL_NETLINK, NETLINK_EXT_ACK,
                    &one, sizeof (one));

  if (ret < 0)
    zlog_notice ("Registration for extended dp ACK failed : %d %s", errno,
                 safe_strerror (errno));
   ...
   */
  one = 1;
  ret = setsockopt (zns->netlink_dplane_out.sock, SOL_NETLINK, NETLINK_CAP_ACK,
                    &one, sizeof (one));
  if (ret < 0)
    zlog_notice (
  ...
  if (fcntl (zns->netlink_cmd.sock, F_SETFL, O_NONBLOCK) < 0)
    zlog_err ("Can't set %s socket error: %s(%d)", zns->netlink_cmd.name,
              safe_strerror (errno), errno);
  ...
  if (fcntl (zns->netlink_dplane_in.sock, F_SETFL, O_NONBLOCK) < 0)
    zlog_err ("Can't set %s socket error: %s(%d)", zns->netlink_dplane_in.name,
              safe_strerror (errno), errno);
...
static void
kernel_nlsock_fini (struct nlsock *nls)
{
  if (nls && nls->sock >= 0)
    {
...
void
kernel_terminate (struct zebra_ns *zns, bool complete)
{
