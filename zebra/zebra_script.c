...
void
zebra_script_init (void)
{
  frrscript_names_add_function_name (ZEBRA_ON_RIB_PROCESS_HOOK_CALL);
}

void
zebra_script_destroy (void)
{
  frrscript_names_destroy ();
}

void
lua_pushnh_grp (lua_State *L, const struct nh_grp *nh_grp)
{
  lua_newtable (L);
...
void
lua_pushzebra_dplane_ctx (lua_State *L, const struct zebra_dplane_ctx *ctx)
{
    ...
    case DPLANE_OP_NH_DELETE:
      ...
      lua_newtable (L);
      {
        ...
        lua_newtable (L);
        {
          ...
          lua_pushnexthop_group (L, dplane_ctx_get_nhe_ng (ctx));
          lua_setfield (L, -2, "ng");
          lua_pushnh_grp (L, dplane_ctx_get_nhe_nh_grp (ctx));
