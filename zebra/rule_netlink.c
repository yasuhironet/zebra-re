 ...
 *
 * Encodes netlink RTM_ADDRULE/RTM_DELRULE message to buffer buf of size
 * buflen.
 *
...
netlink_rule_msg_encode (int cmd, const struct zebra_dplane_ctx *ctx,
                         ...
                         uint8_t dsfield, uint8_t ip_protocol, void *buf,
                         size_t buflen)
{
  ...
  if (! nl_attr_put (&req->n, buflen, FRA_IFNAME, ifname, strlen (ifname) + 1))
    return 0;
  ...
  if (filter_bm & PBR_FILTER_SRC_IP)
    {
      ...
      if (! nl_attr_put (&req->n, buflen, FRA_SRC, &src_ip->u.prefix, bytelen))
        return 0;
    }
  ...
  if (filter_bm & PBR_FILTER_DST_IP)
    {
      ...
      if (! nl_attr_put (&req->n, buflen, FRA_DST, &dst_ip->u.prefix, bytelen))
        return 0;
    }
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug (
        ...
        nl_msg_type_to_str (cmd), nl_family_to_str (family), ifname, priority,
        fwmark, src_ip, dst_ip, table);

  return NLMSG_ALIGN (req->n.nlmsg_len);
}

static ssize_t
netlink_rule_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                          size_t buflen)
{
  int cmd = RTM_NEWRULE;
  ...
  return netlink_rule_msg_encode (
      ...
      dplane_ctx_rule_get_priority (ctx), dplane_ctx_rule_get_table (ctx),
      dplane_ctx_rule_get_src_ip (ctx), dplane_ctx_rule_get_dst_ip (ctx),
      dplane_ctx_rule_get_fwmark (ctx), dplane_ctx_rule_get_dsfield (ctx),
      dplane_ctx_rule_get_ipproto (ctx), buf, buflen);
}

static ssize_t
netlink_oldrule_msg_encoder (struct zebra_dplane_ctx *ctx, void *buf,
                             size_t buflen)
{
  return netlink_rule_msg_encode (
...
netlink_put_rule_update_msg (struct nl_batch *bth,
                             struct zebra_dplane_ctx *ctx)
{
  ...
  if (op == DPLANE_OP_RULE_UPDATE)
    netlink_batch_add_msg (bth, ctx, netlink_oldrule_msg_encoder, true);
 ...
 */
int
netlink_rule_change (struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
  struct zebra_ns *zns;
  struct fib_rule_hdr *frh;
  ...
  if (frh->family != AF_INET && frh->family != AF_INET6)
    {
      if (frh->family == RTNL_FAMILY_IPMR || frh->family == RTNL_FAMILY_IP6MR)
        {
          if (IS_ZEBRA_DEBUG_KERNEL)
            zlog_debug (
  ...
  if (tb[FRA_SRC])
    {
      if (frh->family == AF_INET)
        memcpy (&rule.rule.filter.src_ip.u.prefix4, RTA_DATA (tb[FRA_SRC]), 4);
      else
        memcpy (&rule.rule.filter.src_ip.u.prefix6, RTA_DATA (tb[FRA_SRC]),
                16);
  ...
  if (tb[FRA_DST])
    {
      if (frh->family == AF_INET)
        memcpy (&rule.rule.filter.dst_ip.u.prefix4, RTA_DATA (tb[FRA_DST]), 4);
      else
        memcpy (&rule.rule.filter.dst_ip.u.prefix6, RTA_DATA (tb[FRA_DST]),
                16);
  ...
  if (IS_ZEBRA_DEBUG_KERNEL)
    zlog_debug (
        ...
        rule.ifname, rule.rule.priority, &rule.rule.filter.src_ip,
 ...
 */
static int
netlink_request_rules (struct zebra_ns *zns, int family, int type)
{
  struct
  {
    struct nlmsghdr n;
 ...
 */
int
netlink_rules_read (struct zebra_ns *zns)
{
  int ret;
  ...
  ret = netlink_parse_info (netlink_rule_change, &zns->netlink_cmd, &dp_info,
                            0, true);
  if (ret < 0)
    return ret;
