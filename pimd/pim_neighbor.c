...
static void
dr_election_by_addr (struct interface *ifp)
{
...
static void
dr_election_by_pri (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  struct listnode *node;
  struct pim_neighbor *neigh;
  ...
  for (ALL_LIST_ELEMENTS_RO (pim_ifp->pim_neighbor_list, node, neigh))
    {
      if (PIM_DEBUG_PIM_TRACE)
        {
          zlog_info ("%s: neigh pri %u addr %pPA if dr addr %pPA", __func__,
                     neigh->dr_priority, &neigh->source_addr,
                     &pim_ifp->pim_dr_addr);
        }
      if ((neigh->dr_priority > dr_pri) ||
          ((neigh->dr_priority == dr_pri) &&
           (pim_addr_cmp (neigh->source_addr, pim_ifp->pim_dr_addr) > 0)))
        {
          pim_ifp->pim_dr_addr = neigh->source_addr;
 ...
 */
int
pim_if_dr_election (struct interface *ifp)
{
  ...
  if (pim_addr_cmp (old_dr_addr, pim_ifp->pim_dr_addr))
    {
      ...
      if (PIM_DEBUG_PIM_EVENTS)
        zlog_debug ("%s: DR was %pPA now is %pPA on interface %s", __func__,
...
static void
update_dr_priority (struct pim_neighbor *neigh,
                    pim_hello_options hello_options, uint32_t dr_priority)
{
  ...
  bit_flip = (will_set_pri != PIM_OPTION_IS_SET (neigh->hello_options,
...
static void
on_neighbor_timer (struct event *t)
{
  struct pim_neighbor *neigh;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("Expired %d sec holdtime for neighbor %pPA on interface %s",
...
void
pim_neighbor_timer_reset (struct pim_neighbor *neigh, uint16_t holdtime)
{
  ...
  if (PIM_DEBUG_PIM_TRACE_DETAIL)
    zlog_debug ("%s: starting %u sec timer for neighbor %pPA on %s", __func__,
                neigh->holdtime, &neigh->source_addr, neigh->interface->name);

  event_add_timer (router->master, on_neighbor_timer, neigh, neigh->holdtime,
                   &neigh->t_expire_timer);
}

static void
on_neighbor_jp_timer (struct event *t)
{
  struct pim_neighbor *neigh = EVENT_ARG (t);
  struct pim_rpf rpf;

  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s:Sending JP Agg to %pPA on %s with %d groups", __func__,
                  ...
                  struct list *addr_list)
{
  ...
  if (PIM_DEBUG_PIM_EVENTS)
    zlog_debug ("%s: creating PIM neighbor %pPA on interface %s", __func__,
                &source_addr, ifp->name);

  zlog_notice ("PIM NEIGHBOR UP: neighbor %pPA on interface %s", &source_addr,
               ifp->name);
...
static void
delete_prefix_list (struct pim_neighbor *neigh)
{
      ...
      for (ALL_LIST_ELEMENTS_RO (neigh->prefix_list, p_node, p))
        {
          zlog_debug (
              ...
              __func__, (unsigned) neigh, (unsigned) neigh->prefix_list,
...
void
pim_neighbor_free (struct pim_neighbor *neigh)
{
...
struct pim_neighbor *
pim_neighbor_find_by_secondary (struct interface *ifp, struct prefix *src)
{
  struct pim_interface *pim_ifp;
...
struct pim_neighbor *
pim_neighbor_find (struct interface *ifp, pim_addr source_addr, bool secondary)
{
  struct pim_interface *pim_ifp;
  struct listnode *node;
  struct pim_neighbor *neigh;
                  ...
                  struct list *addr_list, int send_hello_now)
{
  ...
  neigh = pim_neighbor_new (ifp, source_addr, hello_options, holdtime,
                            propagation_delay, override_interval, dr_priority,
                            generation_id, addr_list);
  if (! neigh)
    {
      return 0;
    }
...
void
pim_neighbor_delete (struct interface *ifp, struct pim_neighbor *neigh,
                     const char *delete_message)
{
  ...
  if (PIM_DEBUG_PIM_TRACE)
    {
      zlog_debug ("%s: deleting PIM neighbor %pPA on interface %s", __func__,
...
void
pim_neighbor_delete_all (struct interface *ifp, const char *delete_message)
{
  struct pim_interface *pim_ifp;
  struct listnode *neigh_node;
...
struct prefix *
pim_neighbor_find_secondary (struct pim_neighbor *neigh, struct prefix *addr)
{
  struct listnode *node;
  struct prefix *p;
...
*/
static void
delete_from_neigh_addr (struct interface *ifp, struct list *addr_list,
                        pim_addr neigh_addr)
{
            ...
            if (p)
              {
                zlog_info (
                    ...
                    addr, &neigh_addr, &neigh->source_addr, ifp->name);
...
void
pim_neighbor_update (struct pim_neighbor *neigh,
                     pim_hello_options hello_options, uint16_t holdtime,
                     uint32_t dr_priority, struct list *addr_list)
{
  ...
  zlog_debug (
      ...
      neigh->prefix_list ? (int) listcount (neigh->prefix_list) : -1,
      (unsigned) addr_list, addr_list ? (int) listcount (addr_list) : -1);
#endif
