                       ...
                       struct pim_nexthop_cache *pnc, int command)
{
  ...
  if (PIM_DEBUG_PIM_NHT)
    zlog_debug ("%s: NHT %sregistered addr %pFX(%s) with Zebra ret:%d ",
                __func__, (command == ZEBRA_NEXTHOP_REGISTER) ? " " : "de", &p,
                pim->vrf->name, ret);
...
static struct pim_nexthop_cache *
...
static struct pim_nexthop_cache *
pim_nht_get (struct pim_instance *pim, pim_addr addr)
{
  struct pim_nexthop_cache *pnc = NULL;
  struct pim_rpf rpf;
  ...
  if (! pnc)
    {
      ...
      if (PIM_DEBUG_PIM_NHT_DETAIL)
        zlog_debug ("%s: NHT cache and zebra notification added for %pPA(%s)",
 ...
 */
int
pim_find_or_track_nexthop (struct pim_instance *pim, pim_addr addr,
...
void
pim_nht_bsr_add (struct pim_instance *pim, pim_addr addr)
{
  struct pim_nexthop_cache *pnc;
...
static void
pim_nht_drop_maybe (struct pim_instance *pim, struct pim_nexthop_cache *pnc)
{
  if (PIM_DEBUG_PIM_NHT)
    zlog_debug (
        "%s: NHT %pPA(%s) rp_list count:%d upstream count:%ld BSR count:%u",
        __func__, &pnc->rpf.rpf_addr, pim->vrf->name, pnc->rp_list->count,
...
void
pim_delete_tracked_nexthop (struct pim_instance *pim, pim_addr addr,
                            struct pim_upstream *up, struct rp_info *rp)
{
  struct pim_nexthop_cache *pnc = NULL;
  struct pim_nexthop_cache lookup;
  ...
  if (! pnc)
    {
      zlog_warn ("attempting to delete nonexistent NHT entry %pPA", &addr);
      return;
    }
...
void
pim_nht_bsr_del (struct pim_instance *pim, pim_addr addr)
{
  struct pim_nexthop_cache *pnc = NULL;
  struct pim_nexthop_cache lookup;
  ...
  if (! pnc)
    {
      zlog_warn ("attempting to delete nonexistent NHT BSR entry %pPA", &addr);
      return;
    }
...
bool
pim_nht_bsr_rpf_check (struct pim_instance *pim, pim_addr bsr_addr,
  ...
  if (! pnc || ! CHECK_FLAG (pnc->flags, PIM_NEXTHOP_ANSWER_RECEIVED))
    {
      ...
      num_ifindex =
          zclient_lookup_nexthop (pim, nexthop_tab, router->multipath,
                                  bsr_addr, PIM_NEXTHOP_LOOKUP_MAX);
      ...
      for (i = 0; i < num_ifindex; i++)
        {
          ...
          ifp = if_lookup_by_index (znh->ifindex, pim->vrf->vrf_id);
...
void
pim_rp_nexthop_del (struct rp_info *rp_info)
{
...
static void
pim_update_rp_nh (struct pim_instance *pim, struct pim_nexthop_cache *pnc)
{
...
static int
pim_update_upstream_nh_helper (struct hash_bucket *bucket, void *arg)
{
  ...
  if (PIM_DEBUG_PIM_NHT)
    {
      zlog_debug (
          "%s: NHT upstream %s(%s) old ifp %s new ifp %s", __func__,
          up->sg_str, pim->vrf->name,
          old.source_nexthop.interface ? old.source_nexthop.interface->name
                                       : "Unknown",
...
static int
pim_update_upstream_nh (struct pim_instance *pim,
                        struct pim_nexthop_cache *pnc)
{
...
static int
pim_upstream_nh_if_update_helper (struct hash_bucket *bucket, void *arg)
{
...
void
pim_upstream_nh_if_update (struct pim_instance *pim, struct interface *ifp)
{
...
static int
pim_ecmp_nexthop_search (struct pim_instance *pim,
                         struct pim_nexthop_cache *pnc,
                         ...
                         struct prefix *grp, int neighbor_needed)
{
      ...
      if (pim->ecmp_rebalance_enable == 0)
        {
          ...
          for (nh_node = pnc->nexthop; nh_node; nh_node = nh_node->next)
            {
              curr_route_valid =
                  (nexthop->interface->ifindex == nh_node->ifindex);
              if (curr_route_valid)
                break;
            }

          if (curr_route_valid &&
              ! pim_if_connected_to_source (nexthop->interface, src))
            {
              ...
              else
                {
                  ...
                  if (PIM_DEBUG_PIM_NHT)
                    zlog_debug (
                        ...
                        __func__, &src, &grp_addr, pim->vrf->name,
   ...
   */
  for (nh_node = pnc->nexthop, i = 0; nh_node; nh_node = nh_node->next, i++)
    {
      ifps[i] = if_lookup_by_index (nh_node->ifindex, pim->vrf->vrf_id);
      if (ifps[i])
        {
#if PIM_IPV == 4
      ...
      if (! ifp)
        {
          if (PIM_DEBUG_PIM_NHT)
            zlog_debug (
                ...
                __FILE__, __func__, first_ifindex, &src, pim->vrf->name);
          if (nh_iter == mod_val)
      ...
      if (! pim_ifp || ! pim_ifp->pim_enable)
        {
          if (PIM_DEBUG_PIM_NHT)
            zlog_debug (
                ...
                __func__, ifp->name, pim->vrf->name, first_ifindex, &src);
          if (nh_iter == mod_val)
            mod_val++; // Select nexthpath
          nh_iter++;
          continue;
      ...
      if (neighbor_needed && ! pim_if_connected_to_source (ifp, src))
        {
          ...
          if (! nbr && ! if_is_loopback (ifp))
            {
              if (PIM_DEBUG_PIM_NHT)
                zlog_debug ("%s: pim nbr not found on input interface %s(%s)",
      ...
      if (nh_iter == mod_val)
        {
          ...
          if (PIM_DEBUG_PIM_NHT)
            zlog_debug (
                ...
                __func__, &src, &grp_addr, pim->vrf->name, ifp->name, &nh_addr,
...
/* This API is used to parse Registered address nexthop update coming from
 * Zebra
 */
int
pim_parse_nexthop_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (! zapi_nexthop_update_decode (zclient->ibuf, &match, &nhr))
    {
      zlog_err ("%s: Decode of nexthop update from zebra failed", __func__);
      return 0;
    }
            ...
            case NEXTHOP_TYPE_IPV6_IFINDEX:
              {
                ...
                ifp1 = if_lookup_by_index (nexthop->ifindex, pim->vrf->vrf_id);
  ...
  if (PIM_DEBUG_PIM_NHT)
    zlog_debug (
        ...
        __func__, &match, pim->vrf->name, nhr.nexthop_num, pnc->nexthop_num,
        vrf_id, pnc->upstream_hash->count, listcount (pnc->rp_list));
...
int
pim_ecmp_nexthop_lookup (struct pim_instance *pim, struct pim_nexthop *nexthop,
                         pim_addr src, struct prefix *grp, int neighbor_needed)
{
  struct pim_nexthop_cache *pnc;
  struct pim_zlookup_nexthop nexthop_tab[router->multipath];
  struct pim_neighbor *nbrs[router->multipath], *nbr = NULL;
  ...
  if (PIM_DEBUG_PIM_NHT_DETAIL)
    zlog_debug ("%s: Looking up: %pPA(%s), last lookup time: %lld", __func__,
                &src, pim->vrf->name, nexthop->last_lookup_time);
  ...
  num_ifindex = zclient_lookup_nexthop (pim, nexthop_tab, router->multipath,
                                        src, PIM_NEXTHOP_LOOKUP_MAX);
  if (num_ifindex < 1)
    {
      if (PIM_DEBUG_PIM_NHT)
  ...
  for (i = 0; i < num_ifindex; i++)
    {
      ifps[i] = if_lookup_by_index (nexthop_tab[i].ifindex, pim->vrf->vrf_id);
      if (ifps[i])
        {
          nbrs[i] =
              pim_neighbor_find (ifps[i], nexthop_tab[i].nexthop_addr, true);
          ...
          if (! nbr && ! if_is_loopback (ifp))
            {
              ...
              if (PIM_DEBUG_PIM_NHT)
                zlog_debug (
                    ...
                    __func__, &nexthop_tab[i].nexthop_addr, ifp->name,
                    pim->vrf->name, &src);
              i++;
              continue;
            }
...
int
pim_ecmp_fib_lookup_if_vif_index (struct pim_instance *pim, pim_addr src,
                                  struct prefix *grp)
{
