...
#if MAXVIFS > 256
CPP_NOTICE (
    "Work needs to be done to make this work properly via the pim mroute socket\n");
#endif /* MAXVIFS > 256 */
...
pim_router_init (void)
{
  ...
  */
  ...
  router->infinite_assert_metric.route_metric = PIM_ASSERT_ROUTE_METRIC_MAX;
...
pim_init (void)
{
  if (! inet_pton (PIM_AF, PIM_ALL_PIM_ROUTERS, &qpim_all_pim_routers_addr))
    {
