 ...
 */
static void
pim_upstream_remove_children (struct pim_instance *pim,
      ...
      if (child)
        {
          ...
          if (PIM_UPSTREAM_FLAG_TEST_USE_RPT (child->flags))
            pim_upstream_mroute_iif_update (child->channel_oil, __func__);
        }
    }
 ...
 */
static void
pim_upstream_find_new_children (struct pim_instance *pim,
                                struct pim_upstream *up)
{
  struct pim_upstream *child;
...
pim_upstream_find_parent (struct pim_instance *pim, struct pim_upstream *child)
{
  pim_sgaddr any = child->sg;
  struct pim_upstream *up = NULL;

  // (S,G)
  if (! pim_addr_is_any (child->sg.src) && ! pim_addr_is_any (child->sg.grp))
    {
      any.src = PIMADDR_ANY;
...
static void
upstream_channel_oil_detach (struct pim_upstream *up)
{
...
static void
pim_upstream_timers_stop (struct pim_upstream *up)
{
...
struct pim_upstream *
pim_upstream_del (struct pim_instance *pim, struct pim_upstream *up,
                  const char *name)
{
  ...
  if (PIM_DEBUG_TRACE)
    zlog_debug ("pim_upstream free vrf:%s %s flags 0x%x", pim->vrf->name,
                up->sg_str, up->flags);
  ...
  if (! pim_addr_is_any (up->upstream_addr))
    {
      ...
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: Deregister upstream %s addr %pPA with Zebra NHT",
...
pim_upstream_send_join (struct pim_upstream *up)
{
  ...
  if (PIM_DEBUG_PIM_TRACE)
    {
      zlog_debug ("%s: RPF'%s=%pPA(%s) for Interface %s", __func__, up->sg_str,
                  &up->rpf.rpf_addr, pim_upstream_state2str (up->join_state),
                  up->rpf.source_nexthop.interface->name);
      if (pim_rpf_addr_is_inaddr_any (&up->rpf))
        {
          zlog_debug ("%s: can't send join upstream: RPF'%s=%pPA", __func__,
...
static void
join_timer_stop (struct pim_upstream *up)
{
...
join_timer_start (struct pim_upstream *up)
{
  ...
  if (nbr)
    ...
    {
      EVENT_OFF (up->t_join_timer);
      event_add_timer (router->master, on_join_timer, up, router->t_periodic,
                       &up->t_join_timer);
    }
 ...
 */
void
pim_upstream_join_timer_restart (struct pim_upstream *up, struct pim_rpf *old)
{
  // EVENT_OFF(up->t_join_timer);
  join_timer_start (up);
}
...
static void
pim_upstream_join_timer_restart_msec (struct pim_upstream *up,
                                      int interval_msec)
{
  if (PIM_DEBUG_PIM_EVENTS)
    {
...
void
pim_update_suppress_timers (uint32_t suppress_time)
{
...
void
pim_upstream_join_suppress (struct pim_upstream *up, pim_addr rpf,
                            int holdtime)
{
  ...
  if (up->t_join_timer)
  ...
  else
    {
      ...
      if (nbr)
        {
          join_timer_remain_msec = pim_time_timer_remain_msec (nbr->jp_timer);
        }
    }
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug (
        ...
        __FILE__, __func__, up->sg_str, &rpf, join_timer_remain_msec,
        t_joinsuppress_msec);
  ...
  if (join_timer_remain_msec < t_joinsuppress_msec)
    {
      if (PIM_DEBUG_PIM_TRACE)
        {
          zlog_debug ("%s %s: suppressing Join(S,G)=%s for %ld msec", __FILE__,
                      __func__, up->sg_str, t_joinsuppress_msec);
        }
...
void
pim_upstream_join_timer_decrease_to_t_override (const char *debug_label,
                                                struct pim_upstream *up)
{
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: to RPF'%s=%pPA: join_timer=%ld msec t_override=%d msec",
...
static void
forward_on (struct pim_upstream *up)
{
...
static void
forward_off (struct pim_upstream *up)
{
  struct listnode *chnode;
  struct listnode *chnextnode;
  struct pim_ifchannel *ch;
...
void
pim_upstream_register_reevaluate (struct pim_instance *pim)
{
  struct pim_upstream *up;
 ...
 */
void
pim_upstream_update_use_rpt (struct pim_upstream *up, bool update_mroute)
{
  bool old_use_rpt;
  bool new_use_rpt;
  ...
  if (old_use_rpt != new_use_rpt)
    {
      if (PIM_DEBUG_PIM_EVENTS)
        zlog_debug ("%s switched from %s to %s", up->sg_str,
                    old_use_rpt ? "RPT" : "SPT", new_use_rpt ? "RPT" : "SPT");
      if (update_mroute)
 ...
 */
void
pim_upstream_reeval_use_rpt (struct pim_instance *pim)
{
  struct pim_upstream *up;
...
void
pim_upstream_switch (struct pim_instance *pim, struct pim_upstream *up,
                     enum pim_upstream_state new_state)
{
  ...
  if (pim_addr_is_any (up->upstream_addr))
    {
      if (PIM_DEBUG_PIM_EVENTS)
        zlog_debug ("%s: RPF not configured for %s", __func__, up->sg_str);
      return;
    }
  ...
  if (! up->rpf.source_nexthop.interface)
    {
      if (PIM_DEBUG_PIM_EVENTS)
        zlog_debug ("%s: RP not reachable for %s", __func__, up->sg_str);
      return;
    }

  if (PIM_DEBUG_PIM_EVENTS)
    {
      zlog_debug ("%s: PIM_UPSTREAM_%s: (S,G) old: %s new: %s", __func__,
                  up->sg_str, pim_upstream_state2str (up->join_state),
      ...
      if (old_state != new_state)
        {
          ...
          pim_upstream_update_use_rpt (up, true /*update_mroute*/);
          new_use_rpt = ! ! PIM_UPSTREAM_FLAG_TEST_USE_RPT (up->flags);
          if (new_use_rpt && (new_use_rpt != old_use_rpt) && up->parent)
      ...
      if (send_xg_jp)
        {
          if (PIM_DEBUG_PIM_TRACE_DETAIL)
            zlog_debug (
                ...
                up->parent->rpf.source_nexthop.interface ? up->parent->rpf
                                                               .source_nexthop
                                                               .interface->name
                                                         : "Unknown",
                up->rpf.source_nexthop.interface ? up->rpf.source_nexthop
                                                       .interface->name
                                                 : "Unknown");
          pim_jp_agg_single_upstream_send (&up->parent->rpf, up->parent,
                                           1 /* (W,G) Join */);
        }
      join_timer_stop (up);
    }
...
int
pim_upstream_compare (const struct pim_upstream *up1,
...
void
pim_upstream_fill_static_iif (struct pim_upstream *up,
...
static struct pim_upstream *
pim_upstream_new (struct pim_instance *pim, pim_sgaddr *sg,
                  ...
                  struct pim_ifchannel *ch)
{
  ...
  if (! pim_rp_set_upstream_addr (pim, &up->upstream_addr, sg->src, sg->grp))
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: Received a (*,G) with no RP configured", __func__);
    }
  ...
  if (pim_addr_is_any (up->sg.src))
    {
      up->sources = list_new ();
      up->sources->cmp = (int (*) (void *, void *)) pim_upstream_compare;
    }
  else
    up->sources = NULL;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    {
      zlog_debug (
          ...
          __func__, up->sg_str, &up->upstream_addr, up->ref_count);
    }
...
uint32_t
pim_up_mlag_local_cost (struct pim_upstream *up)
{
  if (! (pim_up_mlag_is_local (up)) &&
      ! (up->flags & PIM_UPSTREAM_FLAG_MASK_MLAG_INTERFACE))
    return router->infinite_assert_metric.route_metric;

  if ((up->rpf.source_nexthop.interface == up->pim->vxlan.peerlink_rif) &&
      (up->rpf.source_nexthop.mrib_route_metric <
       (router->infinite_assert_metric.route_metric -
...
uint32_t
pim_up_mlag_peer_cost (struct pim_upstream *up)
{
...
struct pim_upstream *
pim_upstream_find (struct pim_instance *pim, pim_sgaddr *sg)
{
...
struct pim_upstream *
pim_upstream_find_or_add (pim_sgaddr *sg, struct interface *incoming,
                          int flags, const char *name)
{
  struct pim_interface *pim_ifp = incoming->info;

  return (pim_upstream_add (pim_ifp->pim, sg, incoming, flags, name, NULL));
}
...
pim_upstream_ref (struct pim_upstream *up, int flags, const char *name)
{
  ...
  ++up->ref_count;
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s(%s): upstream %s ref count %d increment", __func__, name,
                up->sg_str, up->ref_count);
}

struct pim_upstream *
pim_upstream_add (struct pim_instance *pim, pim_sgaddr *sg,
                  struct interface *incoming, int flags, const char *name,
                  struct pim_ifchannel *ch)
{
  struct pim_upstream *up = NULL;
  int found = 0;
 ...
 */
int
pim_upstream_eval_inherit_if (struct pim_upstream *up,
                              struct pim_ifchannel *ch,
 ...
 */
static bool
pim_upstream_empty_immediate_olist (struct pim_instance *pim,
                                    struct pim_upstream *up)
{
...
static inline bool
pim_upstream_is_msdp_peer_sa (struct pim_upstream *up)
{
 ...
 */
bool
pim_upstream_evaluate_join_desired (struct pim_instance *pim,
                                    struct pim_upstream *up)
{
  bool empty_imm_oil;
  bool empty_inh_oil;
...
*/
void
pim_upstream_update_join_desired (struct pim_instance *pim,
                                  struct pim_upstream *up)
{
...
*/
void
pim_upstream_rpf_genid_changed (struct pim_instance *pim, pim_addr neigh_addr)
{
  struct pim_upstream *up;
  ...
  frr_each (rb_pim_upstream, &pim->upstream_head, up)
    {
      ...
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug (
            ...
            __func__, &neigh_addr, up->sg_str, pim->vrf->name,
            up->join_state == PIM_UPSTREAM_JOINED, &rpf_addr);
...
void
pim_upstream_rpf_interface_changed (struct pim_upstream *up,
...
void
pim_upstream_update_could_assert (struct pim_upstream *up)
{
  struct listnode *chnode;
  struct listnode *chnextnode;
  struct pim_ifchannel *ch;
...
void
pim_upstream_update_my_assert_metric (struct pim_upstream *up)
{
  struct listnode *chnode;
  struct listnode *chnextnode;
  struct pim_ifchannel *ch;
 ...
 * from the OIL */
static void
pim_upstream_fhr_kat_expiry (struct pim_instance *pim, struct pim_upstream *up)
{
  if (! PIM_UPSTREAM_FLAG_TEST_FHR (up->flags))
    return;
  ...
  EVENT_OFF (up->t_rs_timer);
  /* remove regiface from the OIL if it is there*/
  pim_channel_del_oif (up->channel_oil, pim->regiface, PIM_OIF_FLAG_PROTO_PIM,
                       __func__);
 ...
 * to the OIL */
static void
pim_upstream_fhr_kat_start (struct pim_upstream *up)
{
  if (pim_upstream_could_register (up))
    {
      if (PIM_DEBUG_PIM_TRACE)
 ...
 */
struct pim_upstream *
pim_upstream_keep_alive_timer_proc (struct pim_upstream *up)
{
  struct pim_instance *pim;
  ...
  if (PIM_UPSTREAM_FLAG_TEST_SRC_LHR (up->flags))
    {
      ...
      if (parent)
        {
          pim_jp_agg_single_upstream_send (&parent->rpf, parent, true);
        }
    }
...
static void
pim_upstream_keep_alive_timer (struct event *t)
{
  struct pim_upstream *up;
...
void
pim_upstream_keep_alive_timer_start (struct pim_upstream *up, uint32_t time)
{
  if (! PIM_UPSTREAM_FLAG_TEST_SRC_STREAM (up->flags))
    {
      if (PIM_DEBUG_PIM_TRACE)
...
static void
pim_upstream_msdp_reg_timer (struct event *t)
{
 ...
 */
int
pim_upstream_switch_to_spt_desired_on_rp (struct pim_instance *pim,
                                          pim_sgaddr *sg)
{
...
pim_upstream_set_sptbit (struct pim_upstream *up, struct interface *incoming)
{
  ...
  if (up->rpf.source_nexthop.interface != incoming)
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug (
            ...
            __func__, incoming->name, up->rpf.source_nexthop.interface->name);
      return;
    }
  ...
  if (! pim_upstream_evaluate_join_desired (up->channel_oil->pim, up))
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: %s Join is not Desired", __func__, up->sg_str);
      return;
    }
  ...
  if (pim_if_connected_to_source (up->rpf.source_nexthop.interface,
    ...
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: %s is directly connected to the source", __func__,
                    up->sg_str);
  ...
  // OR RPF_interface(S) != RPF_interface(RP(G))
  if (! starup ||
      up->rpf.source_nexthop.interface != starup->rpf.source_nexthop.interface)
    {
      struct pim_upstream *starup = up->parent;
  ...
  if (pim_upstream_is_sg_rpt (up) && pim_upstream_empty_inherited_olist (up))
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: %s OR inherited_olist(S,G,rpt) == NULL", __func__,
                    up->sg_str);
      up->sptbit = PIM_UPSTREAM_SPTBIT_TRUE;
      return;
    }
  ...
  if (up->parent && pim_rpf_is_same (&up->rpf, &up->parent->rpf))
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: %s RPF'(S,G) is the same as RPF'(*,G)", __func__,
                    up->sg_str);
      up->sptbit = PIM_UPSTREAM_SPTBIT_TRUE;
      return;
    }
...
static void
pim_upstream_register_stop_timer (struct event *t)
{
  struct pim_interface *pim_ifp;
  struct pim_instance *pim;
  struct pim_upstream *up;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    {
      ...
      zlog_debug (
          "%s: (S,G)=%s[%s] upstream register stop timer %s", __func__,
          up->sg_str, pim->vrf->name,
      ...
      if (! pim_ifp)
        {
          if (PIM_DEBUG_PIM_TRACE)
            zlog_debug ("%s: Interface: %s is not configured for pim",
                        __func__, up->rpf.source_nexthop.interface->name);
          return;
        }
...
void
pim_upstream_start_register_stop_timer (struct pim_upstream *up,
                                        int null_register)
{
  uint32_t time;
  ...
  event_add_timer (router->master, pim_upstream_register_stop_timer, up, time,
                   &up->t_rs_timer);
}

int
pim_upstream_inherited_olist_decide (struct pim_instance *pim,
                                     struct pim_upstream *up)
{
  struct interface *ifp;
      ...
      if (pim_upstream_evaluate_join_desired_interface (up, ch, starch))
        {
          ...
          pim_channel_add_oif (up->channel_oil, ifp, flag, __func__);
          output_intf++;
        }
    }
 ...
 */
int
pim_upstream_inherited_olist (struct pim_instance *pim,
                              struct pim_upstream *up)
{
 ...
 */
void
pim_upstream_find_new_rpf (struct pim_instance *pim)
{
  struct pim_upstream *up;
  struct pim_rpf old;
  enum pim_rpf_result rpf_result;
  ...
  frr_each (rb_pim_upstream, &pim->upstream_head, up)
    {
      if (pim_addr_is_any (up->upstream_addr))
        {
          if (PIM_DEBUG_PIM_TRACE)
            zlog_debug ("%s: RP not configured for Upstream %s", __func__,
                        up->sg_str);
          continue;
        }
...
unsigned int
pim_upstream_hash_key (const void *arg)
{
...
void
pim_upstream_terminate (struct pim_instance *pim)
{
  struct pim_upstream *up;
 ...
 */
static bool
pim_upstream_kat_start_ok (struct pim_upstream *up)
{
...
static bool
pim_upstream_sg_running_proc (struct pim_upstream *up)
{
  bool rv = false;
  ...
  if (pim_upstream_kat_start_ok (up))
    {
      ...
      if (! PIM_UPSTREAM_FLAG_TEST_SRC_STREAM (up->flags))
        {
          if (PIM_DEBUG_PIM_TRACE)
            zlog_debug ("source reference created on kat restart %s[%s]",
                        up->sg_str, pim->vrf->name);
...
pim_upstream_sg_running (void *arg)
{
  ...
  if (! up->channel_oil->installed)
    {
      if (PIM_DEBUG_TRACE)
        zlog_debug ("%s: %s%s is not installed in mroute", __func__,
                    up->sg_str, pim->vrf->name);
      return;
    }
  ...
  if (up->channel_oil->oil_inherited_rescan)
    {
      if (PIM_DEBUG_TRACE)
        zlog_debug ("%s: Handling unscanned inherited_olist for %s[%s]",
                    __func__, up->sg_str, pim->vrf->name);
      pim_upstream_inherited_olist_decide (pim, up);
...
void
pim_upstream_add_lhr_star_pimreg (struct pim_instance *pim)
{
  struct pim_upstream *up;
...
void
pim_upstream_spt_prefix_list_update (struct pim_instance *pim,
 ...
 */
void
pim_upstream_remove_lhr_star_pimreg (struct pim_instance *pim,
                                     const char *nlist)
{
  struct pim_upstream *up;
...
void
pim_upstream_init (struct pim_instance *pim)
{
  char name[64];
