...
int
rtnl_send_check (struct rtnl_handle *rth, const char *buf, int len)
{
  struct nlmsghdr *h;
  int status;
  char resp[1024];
          ...
          while (NLMSG_OK (h, (uint32_t) msglen))
            {
              ...
              if (h->nlmsg_type == NLMSG_ERROR)
                {
                  struct nlmsgerr *merr = (struct nlmsgerr *) NLMSG_DATA (h);
                  if (h->nlmsg_len < NLMSG_LENGTH (sizeof (struct nlmsgerr)))
                    {
                      fprintf (stderr, "ERROR truncated\n");
                    }
...
int
rtnl_dump_filter (struct rtnl_handle *rth, rtnl_filter_t filter, void *arg1,
  ...
  while (1)
    {
      ...
      for (h = (struct nlmsghdr *) iov.iov_base; status >= (int) sizeof (*h);)
        {
          int err;
          int len = h->nlmsg_len;
          int l = len - sizeof (*h);
...
struct rtattr *
addattr_nest_compat (struct nlmsghdr *n, int maxlen, int type,
...
int
addattr_nest_compat_end (struct nlmsghdr *n, struct rtattr *start)
{
...
int
rta_addattr_l (struct rtattr *rta, int maxlen, int type, const void *data,
               int alen)
{
  struct rtattr *subrta;
  int len = RTA_LENGTH (alen);
...
int
__parse_rtattr_nested_compat (struct rtattr *tb[], int max, struct rtattr *rta,
                              int len)
{
