...
void
pim_msg_build_header (pim_addr src, pim_addr dst, uint8_t *pim_msg,
                      size_t pim_msg_size, uint8_t pim_msg_type, bool no_fwd)
{
...
uint8_t *
pim_msg_addr_encode_ipv4_ucast (uint8_t *buf, struct in_addr addr)
{
...
uint8_t *
pim_msg_addr_encode_ipv4_group (uint8_t *buf, struct in_addr addr)
{
  buf[0] = PIM_MSG_ADDRESS_FAMILY_IPV4; /* addr family */
  buf[1] = '\0';                        /* native encoding */
  buf[2] = '\0';                        /* reserved */
...
uint8_t *
pim_msg_addr_encode_ipv4_source (uint8_t *buf, struct in_addr addr,
                                 uint8_t bits)
{
  buf[0] = PIM_MSG_ADDRESS_FAMILY_IPV4; /* addr family */
  buf[1] = '\0';                        /* native encoding */
...
uint8_t *
pim_msg_addr_encode_ipv6_source (uint8_t *buf, struct in6_addr addr,
                                 uint8_t bits)
{
...
uint8_t *
pim_msg_addr_encode_ipv6_ucast (uint8_t *buf, struct in6_addr addr)
{
  buf[0] = PIM_MSG_ADDRESS_FAMILY_IPV6; /* addr family */
  buf[1] = '\0';                        /* native encoding */
  buf += 2;
...
uint8_t *
pim_msg_addr_encode_ipv6_group (uint8_t *buf, struct in6_addr addr)
{
  buf[0] = PIM_MSG_ADDRESS_FAMILY_IPV6; /* addr family */
  buf[1] = '\0';                        /* native encoding */
  buf[2] = '\0';                        /* reserved */
...
uint8_t *
pim_msg_addr_encode_ucast (uint8_t *buf, pim_addr addr)
{
  return pim_msg_addr_encode (ucast) (buf, addr);
}

uint8_t *
pim_msg_addr_encode_group (uint8_t *buf, pim_addr addr)
{
  return pim_msg_addr_encode (group) (buf, addr);
}

uint8_t *
pim_msg_addr_encode_source (uint8_t *buf, pim_addr addr, uint8_t bits)
{
...
size_t
pim_msg_build_jp_groups (struct pim_jp_groups *grp,
      ...
      if (pim_addr_is_any (source->up->sg.src))
        {
          ...
          bits =
              PIM_ENCODE_SPARSE_BIT | PIM_ENCODE_WC_BIT | PIM_ENCODE_RPT_BIT;
          ...
          if (PIM_UPSTREAM_FLAG_TEST_SEND_SG_RPT_PRUNE (child->flags))
            {
              pim_msg_addr_encode_source (
                  (uint8_t *) &grp->s[tgroups], child->sg.src,
