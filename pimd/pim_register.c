...
void
pim_register_stop_send (struct interface *ifp, pim_sgaddr *sg, pim_addr src,
                        pim_addr originator)
{
...
static void
pim_reg_stop_upstream (struct pim_instance *pim, struct pim_upstream *up)
{
  switch (up->reg_state)
    {
    case PIM_REG_NOINFO:
  ...
  if (up)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (up->sources, up_node, child))
        {
          if (PIM_DEBUG_PIM_REG)
            zlog_debug ("Executing Reg stop for %s", child->sg_str);
...
#if PIM_IPV == 6
struct in6_addr
pim_register_get_unicast_v6_addr (struct pim_interface *p_ifp)
{
  ...
  for (ALL_LIST_ELEMENTS (p_ifp->sec_addr_list, node, nextnode, sec_addr))
    {
...
void
pim_register_send (const uint8_t *buf, int buf_size, pim_addr src,
                   ...
                   struct pim_upstream *up)
{
  ...
  if (! ifp)
    {
      if (PIM_DEBUG_PIM_REG)
        zlog_debug ("%s: No interface to transmit register on", __func__);
      return;
    }
  pinfo = (struct pim_interface *) ifp->info;
  if (! pinfo)
    {
      if (PIM_DEBUG_PIM_REG)
        zlog_debug ("%s: Interface: %s not configured for pim to transmit on!",
                    __func__, ifp->name);
      return;
    }
  ...
  if (PIM_DEBUG_PIM_REG)
    {
      zlog_debug ("%s: Sending %s %sRegister Packet to %pPA on %s", __func__,
                  up->sg_str, null_register ? "NULL " : "", &rpg->rpf_addr,
                  ifp->name);
    }
  ...
  *b1 |= null_register << 6;
  ...
  pim_msg_build_header (src, rpg->rpf_addr, buffer,
                        buf_size + PIM_MSG_REGISTER_LEN, PIM_MSG_TYPE_REGISTER,
                        false);
...
#if PIM_IPV == 4
void
pim_null_register_send (struct pim_upstream *up)
{
  struct ip ip_hdr;
  struct pim_interface *pim_ifp;
  struct pim_rpf *rpg;
  ...
  if (! pim_ifp)
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: Cannot send null-register for %s no valid iif",
                    __func__, up->sg_str);
      return;
    }
  ...
  if (! rpg)
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: Cannot send null-register for %s no RPF to the RP",
                    __func__, up->sg_str);
      return;
    }
  ...
  pim_register_send ((uint8_t *) &ip_hdr, sizeof (struct ip), src, rpg, 1, up);
}
#else
void
pim_null_register_send (struct pim_upstream *up)
{
  ...
  if (! pim_ifp)
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("Cannot send null-register for %s no valid iif",
                    up->sg_str);
      return;
    }
  ...
  if (! rpg)
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("Cannot send null-register for %s no RPF to the RP",
                    up->sg_str);
      return;
    }
  ...
  src = pim_ifp->primary_address;
  pim_register_send ((uint8_t *) buffer, sizeof (ip6_hdr) + PIM_MSG_HEADER_LEN,
                     src, rpg, 1, up);
}
#endif
 ...
 */
int
pim_register_recv (struct interface *ifp, pim_addr dest_addr,
...
#define PIM_MSG_REGISTER_BIT_RESERVED_LEN 4
  ...
  if ((*bits & PIM_REGISTER_NR_BIT) &&
       ...
       (int) sizeof (struct ip6_hdr)))
    {
      ...
      header = (struct pim_msg_header *) (tlv_buf +
      ...
      header->checksum = 0;
      computed_checksum =
          in_cksum_with_ph6 (&ph, header, htonl (PIM_MSG_HEADER_LEN));
  ...
  if (PIM_DEBUG_PIM_REG)
    zlog_debug ("Received Register message%pSG from %pPA on %s, rp: %d", &sg,
                &src_addr, ifp->name, i_am_rp);
  ...
  if (! rpg)
    {
      zlog_warn (
          "%s: Received Register Message %pSG from %pPA on %s where the RP could not be looked up",
  ...
  if (i_am_rp && (! pim_addr_cmp (dest_addr, rp_addr)))
    {
      ...
      if (pim->register_plist)
        {
          ...
          plist = prefix_list_lookup (PIM_AFI, pim->register_plist);

          pim_addr_to_prefix (&src, sg.src);

          if (prefix_list_apply_ext (plist, NULL, &src, true) == PREFIX_DENY)
            {
      ...
      if (! upstream)
        {
          upstream = pim_upstream_add (pim, &sg, ifp,
      ...
      else
        {
          ...
          if (upstream->sptbit != PIM_UPSTREAM_SPTBIT_TRUE)
            {
              pim_mroute_update_counters (upstream->channel_oil);
              /*
              ...
              if (upstream->channel_oil->cc.oldpktcnt <
                ...
                pim_upstream_set_sptbit (
                    upstream, upstream->rpf.source_nexthop.interface);
            }
        }
      ...
      if ((upstream->sptbit == PIM_UPSTREAM_SPTBIT_TRUE) ||
          (SwitchToSptDesiredOnRp (pim, &sg)))
        {
          if (sentRegisterStop)
            {
              pim_upstream_keep_alive_timer_start (upstream,
      ...
      if (PIM_DEBUG_PIM_REG)
        {
          if (! i_am_rp)
            zlog_debug (
                "Received Register packet for %pSG, Rejecting packet because I am not the RP configured for group",
                &sg);
          else
            zlog_debug (
                "Received Register packet for %pSG, Rejecting packet because the dst ip address is not the actual RP",
                &sg);
        }
      pim_register_stop_send (ifp, &sg, dest_addr, src_addr);
    }
 ...
 */
void
pim_reg_del_on_couldreg_fail (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;
