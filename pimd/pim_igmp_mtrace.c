...
mtrace_primary_address (struct interface *ifp)
{
...
static bool
mtrace_fwd_info_weak (struct pim_instance *pim, struct igmp_mtrace *mtracep,
                      struct igmp_mtrace_rsp *rspp, struct interface **ifpp)
{
...
static bool
mtrace_fwd_info (struct pim_instance *pim, struct igmp_mtrace *mtracep,
                 struct igmp_mtrace_rsp *rspp, struct interface **ifpp)
{
  pim_sgaddr sg;
...
static void
mtrace_rsp_set_fwd_code (struct igmp_mtrace_rsp *mtrace_rspp,
...
static void
mtrace_rsp_init (struct igmp_mtrace_rsp *mtrace_rspp)
{
...
static void
mtrace_rsp_debug (uint32_t qry_id, int rsp, struct igmp_mtrace_rsp *mrspp)
{
  ...
  zlog_debug (
      ...
      rsp, ntohl (qry_id), mrspp->arrival, &incoming, &outgoing, &prev_hop,
      mrspp->rtg_proto, mrspp->fwd_code);
}

static void
mtrace_debug (struct pim_interface *pim_ifp, struct igmp_mtrace *mtracep,
              int mtrace_len)
{
  ...
  zlog_debug (
      ...
      &pim_ifp->primary_address, mtracep->hops, mtracep->type, mtrace_len, &ga,
      &sa, &da, &ra, mtracep->rsp_ttl, ntohl (mtracep->qry_id));
...
query_arrival_time (void)
{
  ...
  if (gettimeofday (&tv, NULL) < 0)
    {
      if (PIM_DEBUG_MTRACE)
        zlog_debug ("Query arrival time lookup failed: errno=%d: %s", errno,
                    safe_strerror (errno));
      return 0;
    }
...
static int
mtrace_send_packet (struct interface *ifp, struct igmp_mtrace *mtracep,
...
static int
mtrace_un_forward_packet (struct pim_instance *pim, struct ip *ip_hdr,
                          struct interface *interface)
{
  ...
  if (interface == NULL)
    {
      ...
      if (! pim_nexthop_lookup (pim, &nexthop, ip_hdr->ip_dst, 0))
        {
          if (PIM_DEBUG_MTRACE)
            zlog_debug ("Dropping mtrace packet, no route to destination");
          return -1;
        }
  ...
  if (sent < 0)
    {
      if (PIM_DEBUG_MTRACE)
        zlog_debug ("Failed to forward mtrace packet: sendto errno=%d, %s",
  ...
  if (PIM_DEBUG_MTRACE)
    {
      zlog_debug ("Fwd mtrace packet len=%u to %pI4 ttl=%u",
                  ntohs (ip_hdr->ip_len), &ip_hdr->ip_dst, ip_hdr->ip_ttl);
    }
...
static int
mtrace_mc_forward_packet (struct pim_instance *pim, struct ip *ip_hdr)
{
  pim_sgaddr sg;
  ...
  if (c_oil == NULL)
    {
      if (PIM_DEBUG_MTRACE)
        {
          zlog_debug ("Dropping mtrace multicast packet len=%u to %pI4 ttl=%u",
                      ntohs (ip_hdr->ip_len), &ip_hdr->ip_dst, ip_hdr->ip_ttl);
        }
      return -1;
    }
...
static int
mtrace_forward_packet (struct pim_instance *pim, struct ip *ip_hdr)
{
...
static int
mtrace_send_mc_response (struct pim_instance *pim, struct igmp_mtrace *mtracep,
                         size_t mtrace_len)
{
  pim_sgaddr sg;
  struct channel_oil *c_oil;
...
static int
mtrace_send_response (struct pim_instance *pim, struct igmp_mtrace *mtracep,
                      size_t mtrace_len)
{
  struct pim_nexthop nexthop;
...
int
igmp_mtrace_recv_qry_req (struct gm_sock *igmp, struct ip *ip_hdr,
                          ...
                          char *igmp_msg, int igmp_msg_len)
{
   ...
   */
  if (! IPV4_CLASS_DE (ntohl (ip_hdr->ip_dst.s_addr)))
    if (! if_address_is_local (&ip_hdr->ip_dst, AF_INET, pim->vrf->vrf_id))
  ...
  if (igmp_msg_len < (int) sizeof (struct igmp_mtrace))
    {
      if (PIM_DEBUG_MTRACE)
        zlog_debug (
            ...
            from_str, ifp->name, igmp_msg_len, sizeof (struct igmp_mtrace));
      return -1;
    }
  ...
  /* if response fields length is equal to a whole number of responses */
    ...
    {
      if (PIM_DEBUG_MTRACE)
        zlog_debug ("Recv mtrace packet from %s on %s: invalid length %d",
      ...
      if (! out_ifp->info)
        {
          ...
          mtrace_rsp_set_fwd_code (rspp, MTRACE_FWD_CODE_NO_MULTICAST);
          return mtrace_send_response (pim, mtracep, mtrace_len);
        }
...
int
igmp_mtrace_recv_response (struct gm_sock *igmp, struct ip *ip_hdr,
                           struct in_addr from, const char *from_str,
                           char *igmp_msg, int igmp_msg_len)
{
