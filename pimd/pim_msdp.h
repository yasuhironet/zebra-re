...
#define PIM_MSDP_PEER_READ_ON(mp)                                             \
  event_add_read (mp->pim->msdp.master, pim_msdp_read, mp, mp->fd, &mp->t_read)
...
void pim_msdp_peer_reset_tcp_conn (struct pim_msdp_peer *mp,
                                   const char *rc_str);
 ...
 * `mesh_group_name` and starts state machine. If no group name is provided
 * then the peer will work standalone.
 *
