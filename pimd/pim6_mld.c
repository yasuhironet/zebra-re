...
#define log_sg(sg, msg)                                                       \
  "[MLD %s:%s %pSG] " msg, sg->iface->ifp->vrf->name, sg->iface->ifp->name,   \
      &sg->sgaddr
...
in6_multicast_scope (const pim_addr *addr)
{
  return addr->s6_addr[1] & 0xf;
}

bool
in6_multicast_nofwd (const pim_addr *addr)
{
...
static int
gm_packet_sg_cmp (const struct gm_packet_sg *a, const struct gm_packet_sg *b)
{
...
static struct gm_packet_sg *
gm_packet_sg_find (struct gm_sg *sg, enum gm_sub_sense sense,
...
static int
gm_grp_pending_cmp (const struct gm_grp_pending *a,
...
static int
gm_gsq_pending_cmp (const struct gm_gsq_pending *a,
...
static uint32_t
gm_gsq_pending_hash (const struct gm_gsq_pending *a)
{
...
int
gm_sg_cmp (const struct gm_sg *a, const struct gm_sg *b)
{
  return pim_sgaddr_cmp (a->sgaddr, b->sgaddr);
}

static struct gm_sg *
gm_sg_find (struct gm_if *gm_ifp, pim_addr grp, pim_addr src)
{
  struct gm_sg ref = {};
...
static struct gm_sg *
gm_sg_make (struct gm_if *gm_ifp, pim_addr grp, pim_addr src)
{
...
static int
gm_subscriber_cmp (const struct gm_subscriber *a,
...
static uint32_t
gm_subscriber_hash (const struct gm_subscriber *a)
{
...
static struct gm_subscriber *
gm_subscriber_findref (struct gm_if *gm_ifp, pim_addr addr)
{
...
static struct gm_subscriber *
gm_subscriber_get (struct gm_if *gm_ifp, pim_addr addr)
{
  struct gm_subscriber ref = {}, *ret;
...
static void
gm_subscriber_drop (struct gm_subscriber **subp)
{
...
static void
gm_expiry_calc (struct gm_query_timers *timers)
{
  unsigned int expire =
...
static void
gm_sg_update (struct gm_sg *sg, bool has_expired)
{
  ...
  else if ((sg->state == GM_SG_JOIN || sg->state == GM_SG_JOIN_EXPIRING) &&
           ! has_expired)
  ...
  if (desired != sg->state && ! gm_ifp->stopping)
    {
      ...
      if (desired == GM_SG_JOIN_EXPIRING || desired == GM_SG_NOPRUNE_EXPIRING)
        {
  ...
  if (new_join && ! sg->tib_joined)
    {
      ...
      sg->tib_joined =
          tib_sg_gm_join (gm_ifp->pim, sg->sgaddr, gm_ifp->ifp, &sg->oil);
      if (! sg->tib_joined)
        zlog_warn ("MLD join for %pSG%%%s not propagated into TIB",
  ...
  if (desired == GM_SG_NOINFO)
    {
      ...
      assertf ((! sg->t_sg_expire &&
               ...
               &sg->sgaddr, gm_ifp->ifp->name, has_expired, sg->t_sg_expire,
               gm_states[prev], gm_states[desired],
               gm_packet_sg_subs_count (sg->subs_positive),
...
static void
gm_packet_free (struct gm_packet_state *pkt)
{
...
static struct gm_packet_sg *
gm_packet_sg_setup (struct gm_packet_state *pkt, struct gm_sg *sg,
                    bool is_excl, bool is_src)
{
  struct gm_packet_sg *item;
...
static bool
gm_packet_sg_drop (struct gm_packet_sg *item)
{
...
static void
gm_packet_drop (struct gm_packet_state *pkt, bool trace)
{
...
static void
gm_packet_sg_remove_sources (struct gm_if *gm_ifp,
                             struct gm_subscriber *subscriber, pim_addr grp,
                             pim_addr *srcs, size_t n_src,
...
static void
gm_sg_expiry_cancel (struct gm_sg *sg)
{
 ...
 */
static void
gm_handle_v2_pass1 (struct gm_packet_state *pkt, struct mld_v2_rec_hdr *rechdr,
                    size_t n_src)
{
      ...
      if (old_grp)
        {
          ...
          gm_packet_sg_remove_sources (pkt->iface, subscriber, rechdr->grp,
                                       rechdr->srcs, n_src, GM_SUB_NEG);
          return;
        }
  ...
  for (j = 0; j < n_src; j++)
    {
      ...
      if (! sg)
        sg = gm_sg_make (pkt->iface, rechdr->grp, rechdr->srcs[j]);
...
static void
gm_handle_v2_pass2_incl (struct gm_packet_state *pkt, size_t i)
{
...
static void
gm_handle_v2_pass2_excl (struct gm_packet_state *pkt, size_t offs)
{
      ...
      for (i = 0; i < item->n_exclude; i++)
        {
          ...
          old_src =
              gm_packet_sg_find (item_src->sg, GM_SUB_NEG, pkt->subscriber);
          if (old_src)
            gm_packet_sg_drop (old_src);
           ...
           */
          old_src =
              gm_packet_sg_find (item_src->sg, GM_SUB_POS, pkt->subscriber);
          if (old_src)
            gm_packet_sg_drop (old_src);
        }
  ...
  for (i = 0; i < item->n_exclude; i++)
    {
      ...
      item_src = &pkt->items[offs + 1 + i];
      item_dup = gm_packet_sg_subs_add (item_src->sg->subs_negative, item_src);
...
static void
gm_handle_v2_report (struct gm_if *gm_ifp, const struct sockaddr_in6 *pkt_src,
  ...
  if (len < sizeof (*hdr))
    {
      if (PIM_DEBUG_GM_PACKETS)
        zlog_debug (log_pkt_src ("malformed MLDv2 report (truncated header)"));
  ...
  if (n_records > len / sizeof (struct mld_v2_rec_hdr))
    {
       ...
       */
      zlog_warn (
          log_pkt_src ("malformed MLDv2 report (infeasible record count)"));
      gm_ifp->stats.rx_drop_malformed++;
      return;
    }
  ...
  for (i = 0; i < n_records; i++)
    {
      ...
      if (len < record_size)
        {
          zlog_warn (
              log_pkt_src ("malformed MLDv2 report (truncated source list)"));
          gm_ifp->stats.rx_trunc_report++;
          break;
        }
      if (! IN6_IS_ADDR_MULTICAST (&rechdr->grp))
        {
          zlog_warn (
              log_pkt_src ("malformed MLDv2 report (invalid group %pI6)"),
              &rechdr->grp);
          gm_ifp->stats.rx_trunc_report++;
          break;
        }
...
static void
gm_handle_v1_report (struct gm_if *gm_ifp, const struct sockaddr_in6 *pkt_src,
                     char *data, size_t len)
{
...
static void
gm_handle_v1_leave (struct gm_if *gm_ifp, const struct sockaddr_in6 *pkt_src,
                    char *data, size_t len)
{
  struct mld_v1_pkt *hdr;
  ...
  while (gm_ifp->n_pending)
    {
      ...
      if (remain_ms > 0)
        {
          if (PIM_DEBUG_GM_EVENTS)
            zlog_debug (log_ifp ("next general expiry in %" PRId64 "ms"),
                        remain_ms / 1000);

          event_add_timer_tv (router->master, gm_t_expire, gm_ifp, &remain,
                              &gm_ifp->t_expire);
          return;
        }
 ...
 */
static void
gm_handle_q_general (struct gm_if *gm_ifp, struct gm_query_timers *timers)
{
...
static bool
gm_sg_check_recent (struct gm_if *gm_ifp, struct gm_sg *sg, struct timeval ref)
{
  struct gm_packet_state *pkt;
      ...
      frr_each (gm_packet_sg_subs, sg->subs_positive, item)
        {
          ...
          if (! best_pkt || timercmp (&pkt->received, &best_pkt->received, >))
            {
              best_pkt = pkt;
              sg->most_recent = item;
            }
...
static void
gm_sg_timer_start (struct gm_if *gm_ifp, struct gm_sg *sg,
                   struct timeval expire_wait)
{
  struct timeval now;
...
static void
gm_handle_q_groupsrc (struct gm_if *gm_ifp, struct gm_query_timers *timers,
                      pim_addr grp, const pim_addr *srcs, size_t n_src)
{
  struct gm_sg *sg;
  size_t i;
...
static void
gm_t_grp_expire (struct event *t)
{
...
static void
gm_handle_q_group (struct gm_if *gm_ifp, struct gm_query_timers *timers,
                   pim_addr grp)
{
  ...
  if (pim_addr_is_any (sg->sgaddr.src))
    {
      ...
      if (PIM_DEBUG_GM_TRACE)
        zlog_debug (log_ifp ("*,%pPAs expiry timer starting"), &grp);
      GM_UPDATE_SG_STATE (sg);
      gm_sg_timer_start (gm_ifp, sg, timers->expire_wait);
...
static void
gm_bump_querier (struct gm_if *gm_ifp)
{
...
static void
gm_t_other_querier (struct event *t)
{
  struct gm_if *gm_ifp = EVENT_ARG (t);
  struct pim_interface *pim_ifp = gm_ifp->ifp->info;
...
static void
gm_handle_query (struct gm_if *gm_ifp, const struct sockaddr_in6 *pkt_src,
                 pim_addr *pkt_dst, char *data, size_t len)
{
  ...
  if (! general_query && ! IN6_IS_ADDR_MULTICAST (&hdr->grp))
    {
      zlog_warn (log_pkt_src ("malformed MLDv2 query (invalid group %pI6)"),
                 &hdr->grp);
      gm_ifp->stats.rx_drop_malformed++;
      return;
    }
  ...
  if (len >= sizeof (struct mld_v2_query_hdr))
    {
      ...
      if (len < sizeof (struct mld_v2_query_hdr) + src_space)
        {
          zlog_warn (
              log_pkt_src ("malformed MLDv2 query (truncated source list)"));
          gm_ifp->stats.rx_drop_malformed++;
          return;
        }
  ...
  if (! IPV6_ADDR_SAME (pkt_dst, &gm_all_hosts))
    {
      if (pim_addr_is_any (hdr->grp))
        {
          zlog_warn (log_pkt_src ("wrong destination %pPA for general query"),
                     pkt_dst);
      ...
      if (! IPV6_ADDR_SAME (&hdr->grp, pkt_dst))
        {
          gm_ifp->stats.rx_drop_dstaddr++;
          zlog_warn (
              log_pkt_src ("wrong destination %pPA for group specific query"),
              pkt_dst);
          return;
        }
    }
  ...
  if (PIM_DEBUG_GM_TRACE_DETAIL)
    zlog_debug (
        log_ifp (
            "query timers: QRV=%u max_resp=%ums qqic=%ums expire_wait=%pTVI"),
        timers.qrv, timers.max_resp_ms, timers.qqic_ms, &timers.expire_wait);
  ...
  if (general_query)
  ...
  else
    {
      ...
      assume (ntohs (hdr->n_src) <=
              (len - sizeof (struct mld_v2_query_hdr)) / sizeof (pim_addr));
...
static void
gm_rx_process (struct gm_if *gm_ifp, const struct sockaddr_in6 *pkt_src,
...
static bool
ip6_check_hopopts_ra (uint8_t *hopopts, size_t hopopt_len, uint16_t alert_type)
{
  uint8_t *hopopt_end;
...
gm_t_recv (struct event *t)
{
  ...
  if (! ip6_check_hopopts_ra (hopopts, hopopt_len, IP6_ALERT_MLD))
    {
      zlog_err (log_pkt_src ("packet without IPv6 Router Alert MLD option"));
  ...
  gm_rx_process (gm_ifp, pkt_src, &pktinfo->ipi6_addr, iov->iov_base, pktlen);
...
static void
gm_send_query (struct gm_if *gm_ifp, pim_addr grp, const pim_addr *srcs,
               size_t n_srcs, bool s_bit)
{
  struct pim_interface *pim_ifp = gm_ifp->ifp->info;
  ...
  query.v2_query.max_resp_code = mld_max_resp_encode (gm_ifp->cur_max_resp);
  query.v2_query.flags = (gm_ifp->cur_qrv < 8) ? gm_ifp->cur_qrv : 0;
  if (s_bit)
    query.v2_query.flags |= 0x08;
  query.v2_query.qqic = igmp_msg_encode16to8 (gm_ifp->cur_query_intv / 1000);
 ...
 */
static void
gm_send_specific (struct gm_gsq_pending *pend_gsq)
{
...
static void
gm_trigger_specific (struct gm_sg *sg)
{
  struct gm_if *gm_ifp = sg->iface;
  struct pim_interface *pim_ifp = gm_ifp->ifp->info;
  ...
  if (sg->n_query)
    event_add_timer_msec (router->master, gm_t_sg_query, sg,
                          gm_ifp->cur_query_intv_trig, &sg->t_sg_query);
...
static void
gm_vrf_socket_incref (struct pim_instance *pim)
{
  ...
  frr_with_privs (&pimd_privs)
    {
      ...
      if (pim->gm_socket < 0)
        {
          zlog_err ("(VRF %s) could not create MLD socket: %m", vrf->name);
          return;
        }

      ret = setsockopt (pim->gm_socket, SOL_ICMPV6, ICMP6_FILTER, filter,
                        sizeof (filter));
      if (ret)
        zlog_err ("(VRF %s) failed to set ICMP6_FILTER: %m", vrf->name);

      intval = 1;
      ret = setsockopt (pim->gm_socket, SOL_IPV6, IPV6_RECVPKTINFO, &intval,
                        sizeof (intval));
      if (ret)
        zlog_err ("(VRF %s) failed to set IPV6_RECVPKTINFO: %m", vrf->name);

      intval = 1;
      ret = setsockopt (pim->gm_socket, SOL_IPV6, IPV6_RECVHOPOPTS, &intval,
                        sizeof (intval));
      if (ret)
        zlog_err ("(VRF %s) failed to set IPV6_HOPOPTS: %m", vrf->name);

      intval = 1;
      ret = setsockopt (pim->gm_socket, SOL_IPV6, IPV6_RECVHOPLIMIT, &intval,
                        sizeof (intval));
      if (ret)
        zlog_err ("(VRF %s) failed to set IPV6_HOPLIMIT: %m", vrf->name);

      intval = 1;
      ret = setsockopt (pim->gm_socket, SOL_IPV6, IPV6_MULTICAST_LOOP, &intval,
                        sizeof (intval));
      if (ret)
        zlog_err ("(VRF %s) failed to disable IPV6_MULTICAST_LOOP: %m",
                  vrf->name);

      intval = 1;
      ret = setsockopt (pim->gm_socket, SOL_IPV6, IPV6_MULTICAST_HOPS, &intval,
                        sizeof (intval));
      if (ret)
        zlog_err ("(VRF %s) failed to set IPV6_MULTICAST_HOPS: %m", vrf->name);
       ...
       */
      intval = 1;
      ret = setsockopt (pim->gm_socket, SOL_IPV6, IPV6_MULTICAST_ALL, &intval,
                        sizeof (intval));
      if (ret)
        zlog_info (
...
static void
gm_vrf_socket_decref (struct pim_instance *pim)
{
  ...
  frr_with_privs (&pimd_privs)
    {
      ...
      ret = setsockopt (gm_ifp->pim->gm_socket, SOL_IPV6, IPV6_JOIN_GROUP,
                        &mreq, sizeof (mreq));
      if (ret)
        zlog_err ("(%s) failed to join ff02::16 (all-MLDv2): %m", ifp->name);
    }
}

void
gm_group_delete (struct gm_if *gm_ifp)
{
  struct gm_sg *sg;
  struct gm_packet_state *pkt;
...
void
gm_ifp_teardown (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;
  struct gm_if *gm_ifp;
  ...
  frr_with_privs (&pimd_privs)
    {
      ...
      mreq.ipv6mr_multiaddr = gm_all_routers;
      mreq.ipv6mr_interface = ifp->ifindex;
      ret = setsockopt (gm_ifp->pim->gm_socket, SOL_IPV6, IPV6_LEAVE_GROUP,
                        &mreq, sizeof (mreq));
      if (ret)
        zlog_err ("(%s) failed to leave ff02::16 (all-MLDv2): %m", ifp->name);
    }
...
static void
gm_update_ll (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;
  struct gm_if *gm_ifp = pim_ifp->mld;
  bool was_querier;

  was_querier = ! IPV6_ADDR_CMP (&gm_ifp->cur_ll_lowest, &gm_ifp->querier) &&
  ...
  if (pim_addr_is_any (gm_ifp->cur_ll_lowest))
    {
      if (was_querier)
        zlog_info (log_ifp ("lost link-local address, stopping querier"));
      return;
    }
...
void
gm_ifp_update (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;
  struct gm_if *gm_ifp;
  bool changed = false;
...
static struct vrf *
gm_cmd_vrf_lookup (struct vty *vty, const char *vrf_str, int *err)
{
  struct vrf *ret;
...
static void
gm_show_if_one_detail (struct vty *vty, struct interface *ifp)
{
...
static void
gm_show_if_one (struct vty *vty, struct interface *ifp, json_object *js_if,
                struct ttable *tt)
{
  struct pim_interface *pim_ifp = (struct pim_interface *) ifp->info;
  struct gm_if *gm_ifp = pim_ifp->mld;
  ...
  if (js_if)
    {
      ...
      json_object_string_addf (js_if, "version", "%d", gm_ifp->cur_version);
      json_object_string_addf (js_if, "upTime", "%pTVMs", &gm_ifp->started);
      json_object_boolean_add (js_if, "querier", querier);
      json_object_string_addf (js_if, "querierIp", "%pPA", &gm_ifp->querier);
      if (querier)
        json_object_string_addf (js_if, "queryTimer", "%pTH", gm_ifp->t_query);
      else
        json_object_string_addf (js_if, "otherQuerierTimer", "%pTH",
                                 gm_ifp->t_other_querier);
      json_object_int_add (js_if, "timerRobustnessValue", gm_ifp->cur_qrv);
      json_object_int_add (js_if, "lastMemberQueryCount", gm_ifp->cur_lmqc);
    ...
    {
      ttable_add_row (tt, "%s|%s|%pPAs|%d|%s|%pPAs|%pTH|%pTVMs", ifp->name,
                      "up", &pim_ifp->primary_address, gm_ifp->cur_version,
                      querier ? "local" : "other", &gm_ifp->querier,
                      gm_ifp->t_query, &gm_ifp->started);
    }
}

static void
gm_show_if_vrf (struct vty *vty, struct vrf *vrf, const char *ifname,
...
static void
gm_show_if (struct vty *vty, struct vrf *vrf, const char *ifname, bool detail,
            json_object *js)
{
  if (vrf)
...
static void
gm_show_stats_one (struct vty *vty, struct gm_if *gm_ifp, json_object *js_if)
{
...
static void
gm_show_stats_vrf (struct vty *vty, struct vrf *vrf, const char *ifname,
                   json_object *js)
{
  struct interface *ifp;
  json_object *js_vrf;
...
static void
gm_show_joins_one (struct vty *vty, struct gm_if *gm_ifp,
      ...
      if (! js_if)
        {
          ...
          vty_out (
              ...
              &sg->sgaddr.grp, &sg->sgaddr.src, gm_states[sg->state], recent,
              untracked, &sg->created);
          ...
          frr_each (gm_packet_sg_subs, sg->subs_positive, item)
            {
              ...
              vty_out (vty, "    %-58pPA  %-16s  %10.0pTVMs\n",
                       &packet->subscriber->addr, "(JOIN)", &packet->received);
            }
      ...
      if (! js_group || pim_addr_cmp (js_grpaddr, sg->sgaddr.grp))
        {
          ...
          json_object_object_addf (js_if, js_group, "%pPA", &sg->sgaddr.grp);
          js_grpaddr = sg->sgaddr.grp;
        }

      js_src = json_object_new_object ();
      json_object_object_addf (js_group, js_src, "%pPAs", &sg->sgaddr.src);

      json_object_string_add (js_src, "state", gm_states[sg->state]);
      json_object_string_addf (js_src, "created", "%pTVMs", &sg->created);
      json_object_string_addf (js_src, "lastSeen", "%pTVMs", recent);

      if (untracked)
        json_object_string_addf (js_src, "untrackedLastSeen", "%pTVMs",
                                 untracked);
      if (! detail)
        continue;
...
static void
gm_show_joins_vrf (struct vty *vty, struct vrf *vrf, const char *ifname,
                   const struct prefix_ipv6 *groups,
                   const struct prefix_ipv6 *sources, bool detail,
                   json_object *js)
{
       ...
       JSON_STR)
{
  ...
  if (json)
    ...
    vty_out (vty, "%-30s  %-30s  %-16s  %10s  %10s  %10s\n", "Group", "Source",
             "State", "LastSeen", "NonTrkSeen", "Created");

  if (vrf)
    gm_show_joins_vrf (vty, vrf, ifname, groups, sources, ! ! detail, js);
  else
    RB_FOREACH (vrf, vrf_name_head, &vrfs_by_name)
      gm_show_joins_vrf (vty, vrf, ifname, groups, sources, ! ! detail, js);
  return vty_json (vty, js);
...
static void
gm_show_groups (struct vty *vty, struct vrf *vrf, bool uj)
{
  struct interface *ifp;
  struct ttable *tt = NULL;
  char *table;
          ...
          if (uj)
            {
              ...
              json_object_string_addf (json_group, "group", "%pPAs",
                                       &sg->sgaddr.grp);

              json_object_int_add (json_group, "version",
                                   pim_ifp->mld_version);
              json_object_string_addf (json_group, "uptime", "%pTVMs",
                                       &sg->created);
              json_object_array_add (json_groups, json_group);
            }
          else
            {
              ttable_add_row (tt, "%s|%pPAs|%d|%pTVMs", ifp->name,
                              &sg->sgaddr.grp, pim_ifp->mld_version,
                              &sg->created);
            }
        } /* scan gm groups */
    }     /* scan interfaces */
  ...
  for (size_t i = 0; i < gm_ifp->n_pending; i++)
    {
      ...
      vty_out (vty,
               "[%zu]: query %" PRId64 "ms ago, expiry in %" PRId64 "ms\n", i,
               query / 1000, expiry / 1000);
    }
      ...
      frr_each (gm_packet_sg_subs, sg->subs_positive, item)
        {
          ...
          vty_out (vty, "\t\t+%s%s [%pPAs %p] %p+%u\n",
                   item->is_src ? "S" : "", item->is_excl ? "E" : "",
                   &pkt->subscriber->addr, pkt->subscriber, pkt, item->offset);
