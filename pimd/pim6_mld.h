...
/* If the timer gm_t_sg_expire is started without a leave message being
 * received, the sg->state should be moved to expiring states. When the timer
 * expires, we do not expect the state to be in join state. If a JOIN message
 * is received while the timer is running, the state will be moved to JOIN and
 * this timer will be switched off. Hence the below state transition is done.
 */
...
static inline bool
gm_sg_state_want_join (enum gm_sg_state state)
{
