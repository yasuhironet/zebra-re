 ...
 */
static inline bool
pim_vxlan_is_orig_mroute (struct pim_vxlan_sg *vxlan_sg)
{
  return ! pim_addr_is_any (vxlan_sg->sg.src);
}

static inline bool
pim_vxlan_is_local_sip (struct pim_upstream *up)
{
  return ! pim_addr_is_any (up->sg.src) && up->rpf.source_nexthop.interface &&
         if_is_loopback (up->rpf.source_nexthop.interface);
}

static inline bool
pim_vxlan_is_term_dev_cfg (struct pim_instance *pim, struct interface *ifp)
{
