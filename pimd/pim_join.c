...
static void
recv_join (struct interface *ifp, struct pim_neighbor *neigh,
           ...
           uint8_t source_flags)
{
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug (
        ...
        ! ! (source_flags & PIM_WILDCARD_BIT_MASK), &upstream, holdtime,
  ...
  if (CHECK_FLAG (source_flags, PIM_WILDCARD_BIT_MASK))
    {
      ...
      if (! rp)
        {
          zlog_warn ("%s: Lookup of RP failed for %pSG", __func__, sg);
          return;
        }
      /*
  ...
  pim_ifchannel_join_add (ifp, neigh->source_addr, upstream, sg, source_flags,
                          holdtime);
}

static void
recv_prune (struct interface *ifp, struct pim_neighbor *neigh,
            uint16_t holdtime, pim_addr upstream, pim_sgaddr *sg,
            uint8_t source_flags)
{
  struct pim_interface *pim_ifp = NULL;
  ...
  if (CHECK_FLAG (source_flags, PIM_WILDCARD_BIT_MASK))
    {
       ...
       */
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: Prune received with RP(%pPAs) for %pSG", __func__,
                    &sg->src, sg);
...
int
pim_joinprune_recv (struct interface *ifp, struct pim_neighbor *neigh,
                    pim_addr src_addr, uint8_t *tlv_buf, int tlv_buf_size)
{
  ...
  */
  addr_offset =
      pim_parse_addr_ucast (&msg_upstream_addr, buf, pastend - buf, &wrong_af);
  if (addr_offset < 1)
    {
          ...
          if (pim_addr_is_any (sg.src))
            {
              ...
              if (starg_ch)
                pim_ifchannel_set_star_g_join_state (starg_ch, 0, 1);
            }
        }
      ...
      for (source = 0; source < msg_num_pruned_sources; ++source)
        {
          ...
          recv_prune (ifp, neigh, msg_holdtime, msg_upstream_addr, &sg,
                      msg_source_flags);
          /*
          ...
          /* (*,G) prune received */
          for (ALL_LIST_ELEMENTS (sg_ch->sources, ch_node, nch_node, child))
            {
              ...
              if (up)
                {
                  ...
                  pim_channel_del_inherited_oif (
                      up->channel_oil, starg_ch->interface, __func__);
                }
            }
        }
 ...
 */
int
pim_joinprune_send (struct pim_rpf *rpf, struct list *groups)
{
      ...
      if (group_size > packet_left)
        {
          pim_msg_build_header (pim_ifp->primary_address,
                                ...
                                packet_size, PIM_MSG_TYPE_JOIN_PRUNE, false);
