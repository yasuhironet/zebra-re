...
on_trace (const char *label, struct interface *ifp, struct in_addr from,
          struct in_addr group_addr, int num_sources, struct in_addr *sources)
{
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      zlog_debug ("%s: from %s on %s: group=%s sources=%d", label, from_str,
                  ifp->name, group_str, num_sources);
    }
}

static inline long
igmp_gmi_msec (struct gm_group *group)
{
...
void
igmp_group_reset_gmi (struct gm_group *group)
{
...
static void
igmp_source_timer (struct event *t)
{
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      zlog_debug ("%s: Source timer expired for group %s source %s on %s",
                  __func__, group_str, source_str, group->interface->name);
    }
...
static void
source_timer_off (struct gm_group *group, struct gm_source *source)
{
...
static void
igmp_source_timer_on (struct gm_group *group, struct gm_source *source,
                      long interval_msec)
{
  ...
  if (PIM_DEBUG_GM_EVENTS)
    {
      ...
      zlog_debug (
          ...
          interval_msec / 1000, interval_msec % 1000, group_str, source_str,
...
void
igmp_source_reset_gmi (struct gm_group *group, struct gm_source *source)
{
  long group_membership_interval_msec;
  struct interface *ifp;
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      zlog_debug (
          ...
          group_membership_interval_msec % 1000, group_str, ifp->name);
    }
...
static void
source_mark_delete_flag (struct gm_group *group)
{
...
static void
source_mark_send_flag (struct gm_group *group)
{
  struct listnode *src_node;
  struct gm_source *src;
...
static int
source_mark_send_flag_by_timer (struct gm_group *group)
{
  struct listnode *src_node;
  struct gm_source *src;
...
static void
source_clear_send_flag (struct list *source_list)
{
  struct listnode *src_node;
  struct gm_source *src;
...
*/
static void
group_exclude_fwd_anysrc_ifempty (struct gm_group *group)
{
  struct pim_interface *pim_ifp = group->interface->info;
...
void
igmp_source_free (struct gm_source *source)
{
...
static void
source_channel_oil_detach (struct gm_source *source)
{
...
*/
void
igmp_source_delete (struct gm_source *source)
{
  struct gm_group *group;
  struct in_addr src;
  ...
  if (IGMP_SOURCE_TEST_FORWARDING (source->source_flags))
    {
      ...
      zlog_warn (
          ...
          __func__, source_str, group_str, group->interface->name);
      /* warning only */
    }
  ...
  */
  ...
  if (group->group_filtermode_isexcl &&
      (listcount (group->group_source_list) != 0) && src.s_addr != INADDR_ANY)
    {
...
static void
source_delete_by_flag (struct list *source_list)
{
  struct listnode *src_node;
...
void
igmp_source_delete_expired (struct list *source_list)
{
  struct listnode *src_node;
  struct listnode *src_nextnode;
  struct gm_source *src;
...
struct gm_source *
igmp_find_source_by_addr (struct gm_group *group, struct in_addr src_addr)
{
  struct listnode *src_node;
  struct gm_source *src;
...
struct gm_source *
igmp_get_source_by_addr (struct gm_group *group, struct in_addr src_addr,
                         bool *new)
{
  struct gm_source *src;
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      zlog_debug ("Creating new IGMP source %s for group %s on interface %s",
                  source_str, group_str, group->interface->name);
    }
...
static void
allow (struct gm_sock *igmp, struct in_addr from, struct in_addr group_addr,
...
void
igmpv3_report_isin (struct gm_sock *igmp, struct in_addr from,
                    struct in_addr group_addr, int num_sources,
                    struct in_addr *sources)
{
  on_trace (__func__, igmp->interface, from, group_addr, num_sources, sources);

  allow (igmp, from, group_addr, num_sources, sources);
}

static void
isex_excl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
  struct gm_source *source;
  int i;
...
static void
isex_incl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
  int i;
...
void
igmpv3_report_isex (struct gm_sock *igmp, struct in_addr from,
                    struct in_addr group_addr, int num_sources,
...
static void
toin_incl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
...
static void
toin_excl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
  int num_sources_tosend;
  int i;
...
void
igmpv3_report_toin (struct gm_sock *igmp, struct in_addr from,
                    struct in_addr group_addr, int num_sources,
                    struct in_addr *sources)
{
  struct interface *ifp = igmp->interface;
...
static void
toex_incl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
...
static void
toex_excl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
  int num_sources_tosend = 0;
  int i;
...
void
igmpv3_report_toex (struct gm_sock *igmp, struct in_addr from,
                    struct in_addr group_addr, int num_sources,
                    struct in_addr *sources)
{
  struct interface *ifp = igmp->interface;
...
void
igmpv3_report_allow (struct gm_sock *igmp, struct in_addr from,
                     struct in_addr group_addr, int num_sources,
                     struct in_addr *sources)
{
  on_trace (__func__, igmp->interface, from, group_addr, num_sources, sources);
...
static void
igmp_send_query_group (struct gm_group *group, char *query_buf,
                       size_t query_buf_size, int num_sources, int s_flag)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (pim_ifp->gm_socket_list, sock_node, igmp))
    {
      igmp_send_query (pim_ifp->igmp_version, group, query_buf, query_buf_size,
...
*/
static void
group_retransmit_group (struct gm_group *group)
{
 ...
 */
static int
group_retransmit_sources (struct gm_group *group, int send_with_sflag_set)
{
  struct pim_interface *pim_ifp;
  long lmqc;                               /* Last Member Query Count */
  long lmqi_msec;                          /* Last Member Query Interval */
      ...
      if (send_with_sflag_set)
        {
          ...
          query_buf1_max_sources =
              (sizeof (query_buf1) - IGMP_V3_SOURCES_OFFSET) >> 2;
          if (num_sources_tosend1 > query_buf1_max_sources)
          ...
          else
            {
              ...
              */
              ...
              igmp_send_query_group (group, query_buf1, sizeof (query_buf1),
      ...
      if (num_sources_tosend2 > query_buf2_max_sources)
        {
          ...
          zlog_warn (
              ...
              __func__, group_str, group->interface->name, num_sources_tosend2,
      ...
      else
        {
          ...
          */
          ...
          igmp_send_query_group (group, query_buf2, sizeof (query_buf2),
...
static void
igmp_group_retransmit (struct event *t)
{
  struct gm_group *group;
...
*/
static void
group_retransmit_timer_on (struct gm_group *group)
{
  struct pim_interface *pim_ifp;
  long lmqi_msec; /* Last Member Query Interval */
...
static long
igmp_group_timer_remain_msec (struct gm_group *group)
{
  return pim_time_timer_remain_msec (group->t_group_timer);
}

static long
igmp_source_timer_remain_msec (struct gm_source *source)
{
...
*/
static void
group_query_send (struct gm_group *group)
{
  struct pim_interface *pim_ifp;
  long lmqc; /* Last Member Query Count */
...
*/
static void
source_query_send_by_flag (struct gm_group *group, int num_sources_tosend)
{
  struct pim_interface *pim_ifp;
  struct listnode *src_node;
  struct gm_source *src;
...
static void
block_excl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
  int num_sources_tosend = 0;
  int i;
...
static void
block_incl (struct gm_group *group, int num_sources, struct in_addr *sources)
{
  int num_sources_tosend = 0;
  int i;
...
void
igmpv3_report_block (struct gm_sock *igmp, struct in_addr from,
                     struct in_addr group_addr, int num_sources,
                     struct in_addr *sources)
{
  struct interface *ifp = igmp->interface;
...
void
igmp_group_timer_lower_to_lmqt (struct gm_group *group)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  char *ifname;
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      zlog_debug ("%s: group %s on %s: LMQC=%d LMQI=%d dsec LMQT=%d msec",
                  __func__, group_str, ifname, lmqc, lmqi_dsec, lmqt_msec);
    }
...
void
igmp_source_timer_lower_to_lmqt (struct gm_source *source)
{
  struct gm_group *group;
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      zlog_debug (
          ...
          __func__, group_str, source_str, ifname, lmqc, lmqi_dsec, lmqt_msec);
    }
...
void
igmp_v3_send_query (struct gm_group *group, int fd, const char *ifname,
  ...
  if (PIM_DEBUG_GM_PACKETS)
    {
      ...
      zlog_debug (
          ...
          dst_str, ifname, group_str, num_sources, msg_size, s_flag,
      ...
      if (group_addr.s_addr == INADDR_ANY)
        {
          ...
          zlog_warn (
              ...
              __func__, dst_str, ifname, group_str, num_sources);
        }
    }
}

void
igmp_v3_recv_query (struct gm_sock *igmp, const char *from_str, char *igmp_msg)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
      ...
      if (PIM_DEBUG_GM_TRACE)
        {
          ...
          zlog_debug (
              ...
              ifaddr_str, qqi ? "recv-non-default" : "default",
          ...
          if (group)
            {
              int recv_num_sources =
                  ntohs (*(uint16_t *) (igmp_msg + IGMP_V3_NUMSOURCES_OFFSET));
              ...
              if (recv_num_sources < 1)
              ...
              else
                {
                   ...
                   * timers to LMQT */
                  struct in_addr *sources =
                      (struct in_addr *) (igmp_msg + IGMP_V3_SOURCES_OFFSET);
...
static bool
igmp_pkt_grp_addr_ok (struct interface *ifp, const char *from_str,
      ...
      if (PIM_DEBUG_GM_PACKETS)
        {
          zlog_debug (
              ...
              &grp.s_addr, from_str, ifp->name, pim_ifp->boundary_oil_plist);
        }
      return false;
    }
...
int
igmp_v3_recv_report (struct gm_sock *igmp, struct in_addr from,
                     const char *from_str, char *igmp_msg, int igmp_msg_len)
{
  ...
  if (igmp_validate_checksum (igmp_msg, igmp_msg_len) == -1)
    {
      zlog_warn ("Recv IGMPv3 report from %s on %s with invalid checksum",
                 from_str, ifp->name);
      return -1;
    }
  ...
  if (PIM_DEBUG_GM_PACKETS)
    {
      zlog_debug ("Recv IGMP report v3 from %s on %s: size=%d groups=%d",
  ...
  for (i = 0; i < num_groups; ++i)
    {
      ...
      rec_num_sources = ntohs (*(
          uint16_t *) (group_record + IGMP_V3_GROUP_RECORD_NUMSOURCES_OFFSET));

      memcpy (&rec_group, group_record + IGMP_V3_GROUP_RECORD_GROUP_OFFSET,
      ...
      if (PIM_DEBUG_GM_PACKETS)
        {
          zlog_debug (
              ...
              from_str, ifp->name, i, rec_type, rec_auxdatalen,
          ...
          if (PIM_DEBUG_GM_PACKETS)
            {
              ...
              zlog_debug (
                  ...
                  from_str, ifp->name, i, &rec_group, src_str);
            }
        } /* for (sources) */
      ...
      if (igmp_pkt_grp_addr_ok (ifp, from_str, rec_group, rec_type))
        switch (rec_type)
          ...
          case IGMP_GRP_REC_TYPE_MODE_IS_INCLUDE:
            igmpv3_report_isin (igmp, from, rec_group, rec_num_sources,
                                (struct in_addr *) sources);
            break;
          case IGMP_GRP_REC_TYPE_MODE_IS_EXCLUDE:
            igmpv3_report_isex (igmp, from, rec_group, rec_num_sources,
                                (struct in_addr *) sources, 0);
            break;
          case IGMP_GRP_REC_TYPE_CHANGE_TO_INCLUDE_MODE:
            igmpv3_report_toin (igmp, from, rec_group, rec_num_sources,
                                (struct in_addr *) sources);
            break;
          case IGMP_GRP_REC_TYPE_CHANGE_TO_EXCLUDE_MODE:
            igmpv3_report_toex (igmp, from, rec_group, rec_num_sources,
                                (struct in_addr *) sources);
            break;
          case IGMP_GRP_REC_TYPE_ALLOW_NEW_SOURCES:
            igmpv3_report_allow (igmp, from, rec_group, rec_num_sources,
                                 (struct in_addr *) sources);
            break;
          case IGMP_GRP_REC_TYPE_BLOCK_OLD_SOURCES:
            igmpv3_report_block (igmp, from, rec_group, rec_num_sources,
                                 (struct in_addr *) sources);
            break;
          default:
            zlog_warn (
