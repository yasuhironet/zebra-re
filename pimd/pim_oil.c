...
pim_channel_oil_dump (struct channel_oil *c_oil, char *buf, size_t size)
{
  ...
  snprintfrr (buf, size, "%pSG IIF: %s, OIFS: ", &sg, ifp ? ifp->name : "(?)");
...
int
pim_channel_oil_compare (const struct channel_oil *cc1,
...
void
pim_oil_init (struct pim_instance *pim)
{
  rb_pim_oil_init (&pim->channel_oil_head);
}

void
pim_oil_terminate (struct pim_instance *pim)
{
...
void
pim_channel_oil_free (struct channel_oil *c_oil)
{
  XFREE (MTYPE_PIM_CHANNEL_OIL, c_oil);
}

struct channel_oil *
pim_find_channel_oil (struct pim_instance *pim, pim_sgaddr *sg)
{
...
struct channel_oil *
pim_channel_oil_add (struct pim_instance *pim, pim_sgaddr *sg,
                     const char *name)
{
  struct channel_oil *c_oil;
 ...
 */
void
pim_clear_nocache_state (struct pim_interface *pim_ifp)
{
  struct channel_oil *c_oil;
...
struct channel_oil *
pim_channel_oil_del (struct channel_oil *c_oil, const char *name)
{
  if (PIM_DEBUG_MROUTE)
    {
      ...
      zlog_debug ("%s(%s): Del oil for %pSG, Ref Count: %d (Predecrement)",
                  __func__, name, &sg, c_oil->oil_ref_count);
    }
  --c_oil->oil_ref_count;
...
void
pim_channel_oil_upstream_deref (struct channel_oil *c_oil)
{
...
int
pim_channel_del_oif (struct channel_oil *channel_oil, struct interface *oif,
      ...
      if (PIM_DEBUG_MROUTE)
        {
          zlog_debug (
              ...
              channel_oil->oif_flags[pim_ifp->mroute_vif_index], oif->name,
              pim_ifp->mroute_vif_index,
  ...
  if (PIM_DEBUG_MROUTE)
    {
      zlog_debug (
          ...
          oil_mcastgrp (channel_oil), proto_mask, *oil_parent (channel_oil),
...
void
pim_channel_del_inherited_oif (struct channel_oil *c_oil,
                               struct interface *oif, const char *caller)
{
  struct pim_upstream *up = c_oil->up;

  pim_channel_del_oif (c_oil, oif, PIM_OIF_FLAG_PROTO_STAR, caller);
...
static bool
pim_channel_eval_oif_mute (struct channel_oil *c_oil,
                           struct pim_interface *pim_ifp)
{
...
void
pim_channel_update_oif_mute (struct channel_oil *c_oil,
                             struct pim_interface *pim_ifp)
{
  bool old_mute;
  bool new_mute;
  ...
  old_mute =
      ! ! (c_oil->oif_flags[pim_ifp->mroute_vif_index] & PIM_OIF_FLAG_MUTE);
 ...
 */
static void
pim_channel_update_mute (struct channel_oil *c_oil)
{
  struct pim_interface *pim_reg_ifp;
  struct pim_interface *vxlan_ifp;
...
int
pim_channel_add_oif (struct channel_oil *channel_oil, struct interface *oif,
                     uint32_t proto_mask, const char *caller)
{
  struct pim_interface *pim_ifp;
  int old_ttl;
      ...
      if (pim_upstream_mroute_add (channel_oil, __func__))
        {
          ...
          oil_if_set (channel_oil, pim_ifp->mroute_vif_index, old_ttl);
          return -5;
        }
    }
