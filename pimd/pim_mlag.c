...
#define PIM_MLAG_ADD_OIF_TO_OIL(ch, ch_oil)                                   \
  do                                                                          \
    {                                                                         \
      if (PIM_DEBUG_MLAG)                                                     \
        zlog_debug ("%s: add Dual-active Interface to %s "                    \
                    "to oil:%s",                                              \
...
#define PIM_MLAG_DEL_OIF_TO_OIL(ch, ch_oil)                                   \
  do                                                                          \
    {                                                                         \
      if (PIM_DEBUG_MLAG)                                                     \
        zlog_debug ("%s: del Dual-active Interface to %s "                    \
                    "to oil:%s",                                              \
                    __func__, ch->interface->name, ch->sg_str);               \
      pim_channel_update_oif_mute (ch_oil, ch->interface->info);              \
    }                                                                         \
...
static void
pim_mlag_calculate_df_for_ifchannels (struct pim_upstream *up, bool is_df)
{
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Calculating DF for Dual active if-channel%s", __func__,
                up->sg_str);
...
static void
pim_mlag_inherit_mlag_flags (struct pim_upstream *up, bool is_df)
{
  ...
  for (ALL_LIST_ELEMENTS (up->ifchannels, chnode, chnextnode, ch))
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (up->sources, listnode, child))
        {
          if (PIM_DEBUG_MLAG)
            zlog_debug ("%s: Updating DF for child:%s", __func__,
                        child->sg_str);
...
bool
pim_mlag_up_df_role_update (struct pim_instance *pim, struct pim_upstream *up,
                            bool is_df, const char *reason)
{
  ...
  if (is_df == old_is_df)
    {
      if (PIM_DEBUG_MLAG)
        zlog_debug ("%s: Ignoring Role update for %s, since no change",
                    __func__, up->sg_str);
      return false;
    }
...
static bool
pim_mlag_up_df_role_elect (struct pim_instance *pim, struct pim_upstream *up)
{
  bool is_df;
  uint32_t peer_cost;
  uint32_t local_cost;
  ...
  if (! (router->mlag_flags & PIM_MLAGF_STATUS_RXED))
    return pim_mlag_up_df_role_update (pim, up, true /*is_df*/, "mlagd-down");
  ...
  if (! (router->mlag_flags & PIM_MLAGF_PEER_CONN_UP))
    {
      ...
      return pim_mlag_up_df_role_update (pim, up, is_df, "peer-down");
    }
  ...
  if (! (router->mlag_flags & PIM_MLAGF_PEER_ZEBRA_UP))
    return pim_mlag_up_df_role_update (pim, up, true /*is_df*/, "zebra-down");
  ...
  if (! PIM_UPSTREAM_FLAG_TEST_MLAG_PEER (up->flags))
    return pim_mlag_up_df_role_update (pim, up, false /*is_df*/,
                                       "no-peer-mroute");
 ...
 */
static void
pim_mlag_up_peer_add (struct mlag_mroute_add *msg)
{
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("peer MLAG mroute add %s:%pSG cost %d", msg->vrf_name, &sg,
                msg->cost_to_rp);
  ...
  if (! vrf)
    {
      if (PIM_DEBUG_MLAG)
        zlog_debug ("peer MLAG mroute add failed %s:%pSG; no vrf",
                    msg->vrf_name, &sg);
      return;
    }
  pim = vrf->info;
  ...
  if (up)
    {
       ...
       */
      if (! PIM_UPSTREAM_FLAG_TEST_MLAG_PEER (up->flags))
        pim_upstream_ref (up, PIM_UPSTREAM_FLAG_MASK_MLAG_PEER, __func__);
    }
  else
    {
      ...
      if (! up)
        {
          if (PIM_DEBUG_MLAG)
            zlog_debug ("peer MLAG mroute add failed %s:%pSG", vrf->name, &sg);
          return;
        }
    }
...
pim_mlag_up_peer_deref (struct pim_instance *pim, struct pim_upstream *up)
{
  if (! PIM_UPSTREAM_FLAG_TEST_MLAG_PEER (up->flags))
    return up;
...
static void
pim_mlag_up_peer_del (struct mlag_mroute_del *msg)
{
  struct pim_upstream *up;
  struct pim_instance *pim;
  pim_sgaddr sg;
  ...
  if (! vrf)
    {
      if (PIM_DEBUG_MLAG)
        zlog_debug ("peer MLAG mroute del skipped %s:%pSG; no vrf",
                    msg->vrf_name, &sg);
      return;
    }
  pim = vrf->info;
  ...
  if (! up)
    {
      if (PIM_DEBUG_MLAG)
        zlog_debug ("peer MLAG mroute del skipped %s:%pSG; no up", vrf->name,
                    &sg);
      return;
    }
 ...
 */
static void
pim_mlag_up_local_add_send (struct pim_instance *pim, struct pim_upstream *up)
{
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("local MLAG mroute add %s:%s", vrf->name, up->sg_str);
...
static void
pim_mlag_up_local_del_send (struct pim_instance *pim, struct pim_upstream *up)
{
  struct stream *s = NULL;
  struct vrf *vrf = pim->vrf;
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("local MLAG mroute del %s:%s", vrf->name, up->sg_str);
...
void
pim_mlag_up_local_add (struct pim_instance *pim, struct pim_upstream *up)
{
  pim_mlag_up_df_role_elect (pim, up);
...
void
pim_mlag_up_local_del (struct pim_instance *pim, struct pim_upstream *up)
{
  pim_mlag_up_df_role_elect (pim, up);
 ...
 */
static void
pim_mlag_up_local_reeval (bool mlagd_send, const char *reason_code)
{
  struct pim_upstream *up;
  struct vrf *vrf;
  struct pim_instance *pim;
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s re-run DF election because of %s", __func__, reason_code);
  RB_FOREACH (vrf, vrf_name_head, &vrfs_by_name)
    {
      pim = vrf->info;
      frr_each (rb_pim_upstream, &pim->upstream_head, up)
        {
          ...
          if (pim_mlag_up_df_role_elect (pim, up) && mlagd_send)
            pim_mlag_up_local_add_send (pim, up);
        }
    }
}
...
static inline void
pim_mlag_vxlan_state_update (void)
{
...
static void
pim_mlag_process_mlagd_state_change (struct mlag_status msg)
{
  bool role_chg = false;
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: msg dump: my_role: %s, peer_state: %s", __func__,
                ...
                (msg.peer_state == MLAG_STATE_RUNNING ? "RUNNING" : "DOWN"));

  if (! (router->mlag_flags & PIM_MLAGF_LOCAL_CONN_UP))
    {
      if (PIM_DEBUG_MLAG)
        zlog_debug ("%s: msg ignored mlagd process state down", __func__);
      return;
    }
  ...
  if (state_chg)
    {
      if (! (router->mlag_flags & PIM_MLAGF_PEER_CONN_UP))
         ...
         */
        pim_mlag_up_local_reeval (true /*mlagd_send*/, "peer_down");
      else
         ...
         */
        pim_mlag_up_local_reeval (true /*mlagd_send*/, "peer_up");
    }
  else if (role_chg)
    {
...
static void
pim_mlag_process_peer_frr_state_change (struct mlag_frr_status msg)
{
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: msg dump: peer_frr_state: %s", __func__,
      ...
      if (! (router->mlag_flags & PIM_MLAGF_PEER_ZEBRA_UP))
        {
           ...
           */
          pim_mlag_up_local_reeval (true /*mlagd_send*/, "zebra_up");
        }
    }
  else
    {
      if (router->mlag_flags & PIM_MLAGF_PEER_ZEBRA_UP)
        {
          ...
          pim_mlag_up_local_reeval (true /*mlagd_send*/, "zebra_down");
        }
    }
}

static void
pim_mlag_process_vxlan_update (struct mlag_vxlan *msg)
{
  ...
  if (PIM_DEBUG_MLAG)
    {
      inet_ntop (AF_INET, &router->local_vtep_ip, addr_buf1, INET_ADDRSTRLEN);
      inet_ntop (AF_INET, &router->anycast_vtep_ip, addr_buf2,
                 INET_ADDRSTRLEN);

      zlog_debug ("%s: msg dump: local-ip:%s, anycast-ip:%s", __func__,
                  addr_buf1, addr_buf2);
    }
}

static void
pim_mlag_process_mroute_add (struct mlag_mroute_add msg)
{
  if (PIM_DEBUG_MLAG)
    {
      pim_sgaddr sg;
      ...
      zlog_debug (
          ...
          __func__, msg.vrf_name, msg.source_ip, msg.group_ip, &sg,
          msg.cost_to_rp);
      zlog_debug (
          "(%pSG)owner_id: %d, DR: %d, Dual active: %d, vrf_id: 0x%x intf_name: %s",
          &sg, msg.owner_id, msg.am_i_dr, msg.am_i_dual_active, msg.vrf_id,
          msg.intf_name);
    }
...
static void
pim_mlag_process_mroute_del (struct mlag_mroute_del msg)
{
  if (PIM_DEBUG_MLAG)
    {
      pim_sgaddr sg;
      ...
      sg.grp.s_addr = ntohl (msg.group_ip);
      sg.src.s_addr = ntohl (msg.source_ip);
      zlog_debug ("%s: msg dump: vrf_name: %s, s.ip: 0x%x, g.ip: 0x%x(%pSG)",
                  __func__, msg.vrf_name, msg.source_ip, msg.group_ip, &sg);
      zlog_debug ("(%pSG)owner_id: %d, vrf_id: 0x%x intf_name: %s", &sg,
...
int
pim_zebra_mlag_handle_msg (int cmd, struct zclient *zclient,
                           uint16_t zapi_length, vrf_id_t vrf_id)
{
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Received msg type: %s length: %d, bulk_cnt: %d", __func__,
                mlag_lib_msgid_to_str (mlag_msg.msg_type, buf, sizeof (buf)),
...
int
pim_zebra_mlag_process_up (ZAPI_CALLBACK_ARGS)
{
  if (PIM_DEBUG_MLAG)
...
static void
pim_mlag_param_reset (void)
{
  /* reset the cached params and stats */
  router->mlag_flags &= ~(PIM_MLAGF_STATUS_RXED | PIM_MLAGF_LOCAL_CONN_UP |
...
int
pim_zebra_mlag_process_down (ZAPI_CALLBACK_ARGS)
{
  if (PIM_DEBUG_MLAG)
...
static void
pim_mlag_register_handler (struct event *thread)
{
  uint32_t bit_mask = 0;
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Posting Client Register to MLAG mask: 0x%x", __func__,
                bit_mask);
...
pim_mlag_register (void)
{
  ...
  event_add_event (router->master, pim_mlag_register_handler, NULL, 0, NULL);
}

static void
pim_mlag_deregister_handler (struct event *thread)
{
  if (! zclient)
    return;

  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Posting Client De-Register to MLAG from PIM", __func__);
  router->connected_to_mlag = false;
  zclient_send_mlag_deregister (zclient);
}

void
pim_mlag_deregister (void)
{
  ...
  event_add_event (router->master, pim_mlag_deregister_handler, NULL, 0, NULL);
}

void
pim_if_configure_mlag_dualactive (struct pim_interface *pim_ifp)
{
  if (! pim_ifp || ! pim_ifp->pim || pim_ifp->activeactive == true)
    return;

  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Configuring active-active on Interface: %s", __func__,
                "NULL");
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Total MLAG configured Interfaces on router: %d, Inst: %d",
...
void
pim_if_unconfigure_mlag_dualactive (struct pim_interface *pim_ifp)
{
  if (! pim_ifp || ! pim_ifp->pim || pim_ifp->activeactive == false)
    return;

  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: UnConfiguring active-active on Interface: %s", __func__,
                "NULL");
...
void
pim_instance_mlag_init (struct pim_instance *pim)
{
  if (! pim)
    return;
...
void
pim_instance_mlag_terminate (struct pim_instance *pim)
{
