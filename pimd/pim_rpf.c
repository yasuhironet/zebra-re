...
void
pim_rpf_set_refresh_time (struct pim_instance *pim)
{
  pim->last_route_change_time = pim_time_monotonic_usec ();
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: vrf(%s) New last route change time: %" PRId64, __func__,
                pim->vrf->name, pim->last_route_change_time);
}

bool
pim_nexthop_lookup (struct pim_instance *pim, struct pim_nexthop *nexthop,
                    pim_addr addr, int neighbor_needed)
{
  ...
  if ((! pim_addr_cmp (nexthop->last_lookup, addr)) &&
      (nexthop->last_lookup_time > pim->last_route_change_time))
    {
      if (PIM_DEBUG_PIM_NHT)
        zlog_debug ("%s: Using last lookup for %pPAs at %lld, %" PRId64
                    " addr %pPAs",
    ...
    {
      if (PIM_DEBUG_PIM_NHT)
        zlog_debug ("%s: Looking up: %pPAs, last lookup time: %lld, %" PRId64,
                    __func__, &addr, nexthop->last_lookup_time,
  ...
  if (num_ifindex < 1)
    {
      if (PIM_DEBUG_PIM_NHT)
        zlog_debug ("%s %s: could not find nexthop ifindex for address %pPAs",
      ...
      if (! ifp)
        {
          if (PIM_DEBUG_ZEBRA)
            zlog_debug (
                ...
                __FILE__, __func__, first_ifindex, &addr);
          i++;
          continue;
        }
  ...
  if (found)
    {
      if (PIM_DEBUG_ZEBRA)
        zlog_debug (
            ...
            __FILE__, __func__, &nexthop_tab[i].nexthop_addr, &addr, ifp->name,
...
static int
nexthop_mismatch (const struct pim_nexthop *nh1, const struct pim_nexthop *nh2)
{
...
static void
pim_rpf_cost_change (struct pim_instance *pim, struct pim_upstream *up,
                     uint32_t old_cost)
{
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Cost_to_rp of upstream-%s changed to:%u, from:%u",
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s: Cost_to_rp of upstream-%s changed to:%u", __func__,
...
enum pim_rpf_result
pim_rpf_update (struct pim_instance *pim, struct pim_upstream *up,
                struct pim_rpf *old, const char *caller)
{
  ...
  if (pim_addr_is_any (up->upstream_addr))
    {
      zlog_debug ("%s(%s): RP is not configured yet for %s", __func__, caller,
                  up->sg_str);
      return PIM_RPF_OK;
    }
  ...
  if (nexthop_mismatch (&rpf->source_nexthop, &saved.source_nexthop))
    {
      ...
      if (PIM_DEBUG_ZEBRA)
        zlog_debug (
            "%s(%s): (S,G)=%s source nexthop now is: interface=%s address=%pPAs pref=%d metric=%d",
            __func__, caller, up->sg_str,
  ...
  if (saved.source_nexthop.interface != rpf->source_nexthop.interface)
    {
      ...
      if (PIM_DEBUG_ZEBRA)
        {
          zlog_debug (
              "%s(%s): (S,G)=%s RPF_interface(S) changed from %s to %s",
              __func__, caller, up->sg_str,
              saved.source_nexthop.interface ? saved.source_nexthop
                                                   .interface->name
                                             : "<oldif?>",
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug ("%s(%s): Cost_to_rp of upstream-%s changed to:%u", __func__,
                caller, up->sg_str, rpf->source_nexthop.mrib_route_metric);
 ...
 */
void
pim_upstream_rpf_clear (struct pim_instance *pim, struct pim_upstream *up)
{
...
*/
static pim_addr
pim_rpf_find_rpf_addr (struct pim_upstream *up)
{
  ...
  if (! up->rpf.source_nexthop.interface)
    {
      zlog_warn ("%s: missing RPF interface for upstream (S,G)=%s", __func__,
                 up->sg_str);
...
unsigned int
pim_rpf_hash_key (const void *arg)
{
...
{
  const struct pim_nexthop_cache *r1 = (const struct pim_nexthop_cache *) arg1;
  const struct pim_nexthop_cache *r2 = (const struct pim_nexthop_cache *) arg2;
