...
uint8_t *
pim_tlv_append_uint16 (uint8_t *buf, const uint8_t *buf_pastend,
...
uint8_t *
pim_tlv_append_2uint16 (uint8_t *buf, const uint8_t *buf_pastend,
...
uint8_t *
pim_tlv_append_uint32 (uint8_t *buf, const uint8_t *buf_pastend,
 ...
 */
int
pim_encode_addr_ucast (uint8_t *buf, pim_addr addr)
{
  uint8_t *start = buf;
...
int
pim_encode_addr_ucast_prefix (uint8_t *buf, struct prefix *p)
{
  switch (p->family)
    {
    case AF_INET:
...
uint8_t *
pim_tlv_append_addrlist_ucast (uint8_t *buf, const uint8_t *buf_pastend,
...
static int
check_tlv_length (const char *label, const char *tlv_name, const char *ifname,
                  pim_addr src_addr, int correct_len, int option_len)
{
  if (option_len != correct_len)
    {
      zlog_warn (
          "%s: PIM hello %s TLV with incorrect value size=%d correct=%d from %pPAs on interface %s",
          label, tlv_name, option_len, correct_len, &src_addr, ifname);
      return -1;
    }
...
static void
check_tlv_redefinition_uint16 (const char *label, const char *tlv_name,
                               const char *ifname, pim_addr src_addr,
                               pim_hello_options options,
                               pim_hello_options opt_mask, uint16_t new,
                               uint16_t old)
{
...
static void
check_tlv_redefinition_uint32 (const char *label, const char *tlv_name,
                               const char *ifname, pim_addr src_addr,
                               pim_hello_options options,
                               pim_hello_options opt_mask, uint32_t new,
                               uint32_t old)
{
  if (PIM_OPTION_IS_SET (options, opt_mask))
    zlog_warn (
...
static void
check_tlv_redefinition_uint32_hex (const char *label, const char *tlv_name,
                                   const char *ifname, pim_addr src_addr,
                                   pim_hello_options options,
                                   pim_hello_options opt_mask, uint32_t new,
                                   uint32_t old)
...
int
pim_tlv_parse_holdtime (const char *ifname, pim_addr src_addr,
                        ...
                        const uint8_t *tlv_curr)
{
  ...
  if (check_tlv_length (__func__, label, ifname, src_addr, sizeof (uint16_t),
                        option_len))
    {
      return -1;
    }
...
int
pim_tlv_parse_lan_prune_delay (const char *ifname, pim_addr src_addr,
                               pim_hello_options *hello_options,
...
int
pim_tlv_parse_dr_priority (const char *ifname, pim_addr src_addr,
                           pim_hello_options *hello_options,
                           ...
                           uint16_t option_len, const uint8_t *tlv_curr)
{
  ...
  if (check_tlv_length (__func__, label, ifname, src_addr, sizeof (uint32_t),
                        option_len))
    {
      return -1;
    }
...
int
pim_tlv_parse_generation_id (const char *ifname, pim_addr src_addr,
                             pim_hello_options *hello_options,
                             ...
                             uint16_t option_len, const uint8_t *tlv_curr)
{
  ...
  check_tlv_redefinition_uint32_hex (
      ...
      PIM_OPTION_MASK_GENERATION_ID, PIM_TLV_GET_GENERATION_ID (tlv_curr),
...
int
pim_parse_addr_ucast_prefix (struct prefix *p, const uint8_t *buf,
                             int buf_size)
{
  ...
  if (type)
    {
      zlog_warn ("%s: unknown unicast address encoding type=%d", __func__,
                 type);
      return -2;
    }
  ...
  switch (family)
    ...
    case PIM_MSG_ADDRESS_FAMILY_IPV4:
      if ((addr + sizeof (struct in_addr)) > pastend)
        {
          zlog_warn ("%s: IPv4 unicast address overflow: left=%td needed=%zu",
                     __func__, pastend - addr, sizeof (struct in_addr));
          return -3;
        }
    ...
    case PIM_MSG_ADDRESS_FAMILY_IPV6:
      if ((addr + sizeof (struct in6_addr)) > pastend)
        {
          zlog_warn ("%s: IPv6 unicast address overflow: left=%td needed %zu",
                     __func__, pastend - addr, sizeof (struct in6_addr));
          return -3;
        }
...
int
pim_parse_addr_ucast (pim_addr *out, const uint8_t *buf, int buf_size,
                      bool *wrong_af)
{
  struct prefix p;
  int ret;
...
int
pim_parse_addr_group (pim_sgaddr *sg, const uint8_t *buf, int buf_size)
{
  ...
  if (type)
    {
      zlog_warn ("%s: unknown group address encoding type=%d from", __func__,
                 type);
      return -2;
    }
  ...
  if ((addr + sizeof (sg->grp)) > pastend)
    {
      zlog_warn ("%s: group address overflow: left=%td needed=%zu from",
...
int
pim_parse_addr_source (pim_sgaddr *sg, uint8_t *flags, const uint8_t *buf,
                       int buf_size)
{
  ...
  switch (family)
    ...
    case PIM_MSG_ADDRESS_FAMILY:
      if ((addr + sizeof (sg->src)) > pastend)
        {
          zlog_warn ("%s: IP source address overflow: left=%td needed=%zu",
      ...
      */
      if (mask_len != PIM_MAX_BITLEN)
        {
          zlog_warn ("%s: IP bad source address mask: %d", __func__, mask_len);
          return -4;
        }
...
int
pim_tlv_parse_addr_list (const char *ifname, pim_addr src_addr,
                         pim_hello_options *hello_options,
  ...
  while (addr < pastend)
    {
      ...
      /*
        Parse ucast addr
       */
      addr_offset = pim_parse_addr_ucast_prefix (&tmp, addr, pastend - addr);
      if (addr_offset < 1)
        {
            ...
            case AF_INET:
              {
                ...
                pim_inet4_dump ("<addr?>", tmp.u.prefix4, addr_str,
                                sizeof (addr_str));
                zlog_debug (
