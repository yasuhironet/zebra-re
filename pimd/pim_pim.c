...
static const char *
pim_pim_msgtype2str (enum pim_msg_type type)
{
  switch (type)
    {
...
static void
sock_close (struct interface *ifp)
{
  ...
  if (pim_ifp->pim_sock_fd > 0 && close (pim_ifp->pim_sock_fd))
    {
      zlog_warn (
          ...
          pim_ifp->pim_sock_fd, ifp->name, errno, safe_strerror (errno));
    }
...
void
pim_sock_delete (struct interface *ifp, const char *delete_message)
{
  ...
  if (! ifp->info)
    {
      flog_err (EC_PIM_CONFIG,
                "%s: %s: but PIM not enabled on interface %s (!)", __func__,
                delete_message, ifp->name);
      return;
    }
...
static bool
pim_pkt_dst_addr_ok (enum pim_msg_type type, pim_addr addr)
{
...
int
pim_pim_packet (struct interface *ifp, uint8_t *buf, size_t len, pim_sgaddr sg)
{
  ...
  if (len <= sizeof (*ip_hdr))
    {
      if (PIM_DEBUG_PIM_PACKETS)
        zlog_debug ("PIM packet size=%zu shorter than minimum=%zu", len,
                    sizeof (*ip_hdr));
      return -1;
    }
  ...
  if (pim_msg_len < PIM_PIM_MIN_LEN)
    {
      if (PIM_DEBUG_PIM_PACKETS)
        zlog_debug ("PIM message size=%d shorter than minimum=%d", pim_msg_len,
                    PIM_PIM_MIN_LEN);
      return -1;
    }
      ...
      if (pim_msg_len < PIM_MSG_REGISTER_LEN)
        {
          if (PIM_DEBUG_PIM_PACKETS)
            zlog_debug (
                "PIM Register Message size=%d shorther than min length %d",
  ...
  if (PIM_DEBUG_PIM_PACKETS)
    {
      zlog_debug (
          ...
          pim_pim_msgtype2str (header->type), &sg.src, &sg.grp, ifp->name,
          header->ver, pim_msg_len, checksum);
  ...
  if (! pim_pkt_dst_addr_ok (header->type, sg.grp))
    {
      zlog_warn (
          ...
          __func__, &sg.grp, pim_pim_msgtype2str (header->type), &sg.src);
      return -1;
    }
  ...
  switch (header->type)
    ...
    case PIM_MSG_TYPE_ASSERT:
      ...
      pim_neighbor_timer_reset (neigh, neigh->holdtime);
      return pim_assert_recv (ifp, neigh, sg.src, pim_msg + PIM_MSG_HEADER_LEN,
                              pim_msg_len - PIM_MSG_HEADER_LEN);
      break;
    case PIM_MSG_TYPE_BOOTSTRAP:
  ...
  while (cont)
    {
      ...
      len = pim_socket_recvfromto (fd, buf, sizeof (buf), &from, &fromlen, &to,
                                   &tolen, &ifindex);
      if (len < 0)
        {
          if (errno == EINTR)
      ...
      if (! ifp || ! ifp->info)
        {
          if (PIM_DEBUG_PIM_PACKETS)
            zlog_debug (
                ...
                __func__, ifp ? ifp->name : "Unknown", ifindex);
          goto done;
        }
#if PIM_IPV == 4
      ...
      if (fail)
        {
          if (PIM_DEBUG_PIM_PACKETS)
            zlog_debug ("%s: pim_pim_packet() return=%d", __func__, fail);
          goto done;
        }
...
static void
pim_sock_read_on (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
static int
pim_sock_open (struct interface *ifp)
{
  int fd;
  struct pim_interface *pim_ifp = ifp->info;
  ...
  if (pim_socket_join (fd, qpim_all_pim_routers_addr, pim_ifp->primary_address,
                       ifp->ifindex, pim_ifp))
    {
      close (fd);
      return -2;
...
void
pim_ifstat_reset (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
void
pim_sock_reset (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
#if PIM_IPV == 4
static int
pim_msg_send_frame (int fd, char *buf, size_t len, struct sockaddr *dst,
                    size_t salen, const char *ifname)
{
  ...
  zlog_warn ("%s: sendto() failure to %pSU: iface=%s fd=%d msg_size=%zd: %m",
...
#else
static int
pim_msg_send_frame (pim_addr src, pim_addr dst, ifindex_t ifindex,
...
int
pim_msg_send (int fd, pim_addr src, pim_addr dst, uint8_t *pim_msg,
...
#define IPDEFTTL 64
  ...
  if (PIM_DEBUG_PIM_PACKETS)
    zlog_debug ("%s: to %pPA on %s: msg_size=%d checksum=%x", __func__, &dst,
...
static int
hello_send (struct interface *ifp, uint16_t holdtime)
{
  ...
  if (PIM_DEBUG_PIM_HELLO)
    zlog_debug (
        ...
        __func__, &qpim_all_pim_routers_addr, ifp->name, holdtime,
        pim_ifp->pim_propagation_delay_msec,
        pim_ifp->pim_override_interval_msec,
        pim_ifp->pim_can_disable_join_suppression, pim_ifp->pim_dr_priority,
        pim_ifp->pim_generation_id, listcount (ifp->connected));

  pim_tlv_size = pim_hello_build_tlv (
      ifp, pim_msg + PIM_PIM_MIN_LEN, sizeof (pim_msg) - PIM_PIM_MIN_LEN,
      holdtime, pim_ifp->pim_dr_priority, pim_ifp->pim_generation_id,
  ...
  pim_msg_build_header (pim_ifp->primary_address, qpim_all_pim_routers_addr,
                        pim_msg, pim_msg_size, PIM_MSG_TYPE_HELLO, false);
...
int
pim_hello_send (struct interface *ifp, uint16_t holdtime)
{
  struct pim_interface *pim_ifp = ifp->info;
...
static void
hello_resched (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
 ...
 */
static void
on_pim_hello_send (struct event *t)
{
  struct pim_interface *pim_ifp;
 ...
 */
void
pim_hello_restart_now (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
 ...
 */
void
pim_hello_restart_triggered (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  ...
  event_add_timer_msec (router->master, on_pim_hello_send, ifp, random_msec,
                        &pim_ifp->t_pim_hello_timer);
}

int
pim_sock_add (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  uint32_t old_genid;
