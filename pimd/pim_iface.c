...
static void
pim_sec_addr_free (struct pim_secondary_addr *sec_addr)
{
  XFREE (MTYPE_PIM_SEC_ADDR, sec_addr);
}

__attribute__ ((unused)) static int
pim_sec_addr_comp (const void *p1, const void *p2)
{
...
pim_if_new (struct interface *ifp, bool gm, bool pim, bool ispimreg,
            bool is_vxlan_term)
{
  struct pim_interface *pim_ifp;
  ...
  pim_ifp->gm_query_max_response_time_dsec = GM_QUERY_MAX_RESPONSE_TIME_DSEC;
...
void
pim_if_delete (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
void
pim_if_update_could_assert (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
static void
pim_if_update_my_assert_metric (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
...
static void
pim_addr_change (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
static int
detect_primary_address_change (struct interface *ifp, int force_prim_as_any,
                               const char *caller)
{
  struct pim_interface *pim_ifp = ifp->info;
  ...
  if (PIM_DEBUG_ZEBRA)
    zlog_debug ("%s: old=%pPA new=%pPA on interface %s: %s", __func__,
...
static void
pim_sec_addr_del (struct pim_interface *pim_ifp,
                  struct pim_secondary_addr *sec_addr)
{
...
static int
pim_sec_addr_add (struct pim_interface *pim_ifp, struct prefix *addr)
{
  int changed = 0;
  struct pim_secondary_addr *sec_addr;
...
static int
pim_sec_addr_del_all (struct pim_interface *pim_ifp)
{
  int changed = 0;
...
static int
pim_sec_addr_update (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;
...
static int
detect_secondary_address_change (struct interface *ifp, int force_prim_as_any,
                                 const char *caller)
{
  struct pim_interface *pim_ifp = ifp->info;
  int changed = 0;
...
static void
detect_address_change (struct interface *ifp, int force_prim_as_any,
                       const char *caller)
{
  int changed = 0;
  struct pim_interface *pim_ifp;
...
int
pim_update_source_set (struct interface *ifp, pim_addr source)
{
  struct pim_interface *pim_ifp = ifp->info;
...
void
pim_if_addr_add (struct connected *ifc)
{
  struct pim_interface *pim_ifp;
  struct interface *ifp;
  bool vxlan_term;
  ...
  if (PIM_DEBUG_ZEBRA)
    zlog_debug ("%s: %s ifindex=%d connected IP address %pFX %s", __func__,
  ...
  if (IN6_IS_ADDR_LINKLOCAL (&ifc->address->u.prefix6) ||
      IN6_IS_ADDR_LOOPBACK (&ifc->address->u.prefix6))
    {
      ...
      else if (IPV6_ADDR_CMP (&ifc->address->u.prefix6, &pim_ifp->ll_lowest) <
               0)
        pim_ifp->ll_lowest = ifc->address->u.prefix6;

      if (IPV6_ADDR_CMP (&ifc->address->u.prefix6, &pim_ifp->ll_highest) > 0)
        pim_ifp->ll_highest = ifc->address->u.prefix6;

      if (PIM_DEBUG_ZEBRA)
        zlog_debug ("%s: new link-local %pI6, lowest now %pI6, highest %pI6",
          ...
          for (ALL_LIST_ELEMENTS (pim_ifp->gm_join_list, node, nextnode, ij))
            {
               ...
               */
              close (ij->sock_fd);
              join_fd = gm_join_sock (ifp->name, ifp->ifindex, ij->group_addr,
      ...
      if (ifc->address->family == AF_INET)
        {
          if (igmp)
            igmp_sock_delete (igmp);
          /* if addr new, add IGMP socket */
          pim_igmp_sock_add (pim_ifp->gm_socket_list, ifaddr, ifp, true);
        }
    } /* igmp mtrace only */
#endif
      ...
      if (! pim_addr_is_any (pim_ifp->primary_address))
        {
          ...
          if (pnc)
            pim_sendmsg_zebra_rnh (pim_ifp->pim, zclient, pnc,
...
static void
pim_if_addr_del_igmp (struct connected *ifc)
{
#if PIM_IPV == 4
...
static void
pim_if_addr_del_pim (struct connected *ifc)
{
  struct pim_interface *pim_ifp = ifc->ifp->info;
...
void
pim_if_addr_del (struct connected *ifc, int force_prim_as_any)
{
  struct interface *ifp;
  ...
  if (PIM_DEBUG_ZEBRA)
    zlog_debug ("%s: %s ifindex=%d disconnected IP address %pFX %s", __func__,
                ifp->name, ifp->ifindex, ifc->address,
                CHECK_FLAG (ifc->flags, ZEBRA_IFA_SECONDARY) ? "secondary"
                                                             : "primary");
      ...
      for (ALL_LIST_ELEMENTS_RO (ifc->ifp->connected, cnode, cc))
        {
          ...
          if (IPV6_ADDR_CMP (&cc->address->u.prefix6, &pim_ifp->ll_lowest) < 0)
            pim_ifp->ll_lowest = cc->address->u.prefix6;
          if (IPV6_ADDR_CMP (&cc->address->u.prefix6, &pim_ifp->ll_highest) >
              0)
            pim_ifp->ll_highest = cc->address->u.prefix6;
        }

      if (pim_ifp->ll_lowest.s6_addr[0] == 0xff)
        memset (&pim_ifp->ll_lowest, 0, sizeof (pim_ifp->ll_lowest));

      if (PIM_DEBUG_ZEBRA)
        zlog_debug (
            "%s: removed link-local %pI6, lowest now %pI6, highest %pI6",
            ifc->ifp->name, &ifc->address->u.prefix6, &pim_ifp->ll_lowest,
            &pim_ifp->ll_highest);
...
void
pim_if_addr_add_all (struct interface *ifp)
{
  struct connected *ifc;
  struct listnode *node;
  struct listnode *nextnode;
...
void
pim_if_addr_del_all (struct interface *ifp)
{
  struct connected *ifc;
  struct listnode *node;
  struct listnode *nextnode;
...
void
pim_if_addr_del_all_igmp (struct interface *ifp)
{
  struct connected *ifc;
  struct listnode *node;
  struct listnode *nextnode;
  ...
  if (! v4_addrs && v6_addrs)
    {
      ...
      if (ifp->vrf->vrf_id == VRF_DEFAULT)
        ...
        lo_ifp = if_lookup_by_name (ifp->vrf->name, ifp->vrf->vrf_id);
 ...
 */
int
pim_if_add_vif (struct interface *ifp, bool ispimreg, bool is_vxlan_term)
{
  struct pim_interface *pim_ifp = ifp->info;
  pim_addr ifaddr;
  ...
  if (pim_ifp->mroute_vif_index > 0)
    {
      zlog_warn ("%s: vif_index=%d > 0 on interface %s ifindex=%d", __func__,
  ...
  if (ifp->ifindex < 0)
    {
      zlog_warn ("%s: ifindex=%d < 0 on interface %s", __func__, ifp->ifindex,
                 ifp->name);
      return -2;
    }
    ...
    {
      zlog_warn ("%s: ifindex=%d on interface %s", __func__, ifp->ifindex,
                 ifp->name);
      return -2;
    }
...
int
pim_if_del_vif (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;

  if (pim_ifp->mroute_vif_index < 1)
    {
      zlog_warn ("%s: vif_index=%d < 1 on interface %s ifindex=%d", __func__,
                 pim_ifp->mroute_vif_index, ifp->name, ifp->ifindex);
      return -1;
    }
...
// DBS - VRF Revist
struct interface *
pim_if_find_by_vif_index (struct pim_instance *pim, ifindex_t vif_index)
{
  struct interface *ifp;
 ...
 */
int
pim_if_find_vifindex_by_ifindex (struct pim_instance *pim, ifindex_t ifindex)
{
  struct pim_interface *pim_ifp;
  struct interface *ifp;
...
int
pim_if_lan_delay_enabled (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
uint16_t
pim_if_effective_propagation_delay_msec (struct interface *ifp)
{
...
uint16_t
pim_if_effective_override_interval_msec (struct interface *ifp)
{
  if (pim_if_lan_delay_enabled (ifp))
    {
      struct pim_interface *pim_ifp;
...
int
pim_if_t_override_msec (struct interface *ifp)
{
...
uint16_t
pim_if_jp_override_interval_msec (struct interface *ifp)
{
...
*/
struct pim_neighbor *
pim_if_find_neighbor (struct interface *ifp, pim_addr addr)
{
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: neighbor not found for address %pPA on interface %s",
...
long
pim_if_t_suppressed_msec (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
static void
gm_join_free (struct gm_join *ij)
{
  XFREE (MTYPE_PIM_IGMP_JOIN, ij);
}

static struct gm_join *
gm_join_find (struct list *join_list, pim_addr group_addr,
...
static int
gm_join_sock (const char *ifname, ifindex_t ifindex, pim_addr group_addr,
              pim_addr source_addr, struct pim_interface *pim_ifp)
{
  int join_fd;
  ...
  if (pim_gm_join_source (join_fd, ifindex, group_addr, source_addr))
    {
      zlog_warn (
          ...
          __func__, join_fd, &group_addr, &source_addr, ifindex, ifname, errno,
          safe_strerror (errno));
...
static struct gm_join *
gm_join_new (struct interface *ifp, pim_addr group_addr, pim_addr source_addr)
{
  struct pim_interface *pim_ifp;
  struct gm_join *ij;
  int join_fd;
  ...
  join_fd =
      gm_join_sock (ifp->name, ifp->ifindex, group_addr, source_addr, pim_ifp);
  if (join_fd < 0)
    {
...
ferr_r
pim_if_gm_join_add (struct interface *ifp, pim_addr group_addr,
                    pim_addr source_addr)
{
  struct pim_interface *pim_ifp;
  struct gm_join *ij;
...
int
pim_if_gm_join_del (struct interface *ifp, pim_addr group_addr,
                    pim_addr source_addr)
{
  struct pim_interface *pim_ifp;
  struct gm_join *ij;
  ...
  if (! pim_ifp->gm_join_list)
    {
      zlog_warn ("%s: no " GM " join on interface %s", __func__, ifp->name);
      return -2;
    }
  ...
  if (close (ij->sock_fd))
    {
      zlog_warn ("%s: failure closing sock_fd=%d for " GM
                 " group %pPAs source %pPAs on interface %s: errno=%d: %s",
                 __func__, ij->sock_fd, &group_addr, &source_addr, ifp->name,
...
pim_if_gm_join_del_all (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  struct listnode *node;
  struct listnode *nextnode;
 ...
 */
void
pim_if_assert_on_neighbor_down (struct interface *ifp, pim_addr neigh_addr)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
...
void
pim_if_update_join_desired (struct pim_interface *pim_ifp)
{
  struct pim_ifchannel *ch;
...
void
pim_if_update_assert_tracking_desired (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
 ...
 */
void
pim_if_create_pimreg (struct pim_instance *pim)
{
  ...
  if (! pim->regiface)
    {
      ...
      pim->regiface =
          if_get_by_name (pimreg_name, pim->vrf->vrf_id, pim->vrf->name);
      ...
      pim_if_new (pim->regiface, false, false, true, false /*vxlan_term*/);
...
struct prefix *
pim_if_connected_to_source (struct interface *ifp, pim_addr src)
{
  struct listnode *cnode;
  struct connected *c;
  struct prefix p;
...
bool
pim_if_is_vrf_device (struct interface *ifp)
{
  if (if_is_vrf (ifp))
    return true;
...
int
pim_if_ifchannel_count (struct pim_interface *pim_ifp)
{
  struct pim_ifchannel *ch;
  int count = 0;
...
static int
pim_ifp_create (struct interface *ifp)
{
  struct pim_instance *pim;
  ...
  if (PIM_DEBUG_ZEBRA)
    {
      zlog_debug (
          ...
          __func__, ifp->name, ifp->ifindex, ifp->vrf->name, ifp->vrf->vrf_id,
  ...
  if (pim_if_is_vrf_device (ifp))
    {
      ...
      if (! ifp->info)
        {
          pim_ifp =
              pim_if_new (ifp, false, false, false, false /*vxlan_term*/);
          ifp->info = pim_ifp;
        }
...
static int
pim_ifp_up (struct interface *ifp)
{
  uint32_t table_id;
  struct pim_interface *pim_ifp;
  struct pim_instance *pim;
...
static int
pim_ifp_down (struct interface *ifp)
{
  if (PIM_DEBUG_ZEBRA)
    {
      zlog_debug (
...
static int
pim_ifp_destroy (struct interface *ifp)
{
  if (PIM_DEBUG_ZEBRA)
    {
      zlog_debug (
...
static int
pim_if_new_hook (struct interface *ifp)
{
  return 0;
}

static int
pim_if_delete_hook (struct interface *ifp)
{
  if (ifp->info)
    pim_if_delete (ifp);
