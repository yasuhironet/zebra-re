...
RB_GENERATE (pim_ifchannel_rb, pim_ifchannel, pim_ifp_rb,
             pim_ifchannel_compare);

int
pim_ifchannel_compare (const struct pim_ifchannel *ch1,
...
void
pim_ifchannel_delete (struct pim_ifchannel *ch)
{
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: ifchannel entry %s(%s) del start", __func__, ch->sg_str,
                ch->interface->name);
      ...
      if (ch->upstream->dualactive_ifchannel_count == 1)
        {
          ...
          PIM_UPSTREAM_FLAG_UNSET_MLAG_INTERFACE (ch->upstream->flags);
        }
  ...
  if (ch->upstream->channel_oil)
    {
      ...
      if (! pim_addr_is_any (ch->sg.src) &&
          pim_upstream_evaluate_join_desired_interface (ch->upstream, ch,
                                                        ch->parent))
        pim_channel_add_oif (ch->upstream->channel_oil, ch->interface,
      ...
      if (ch->upstream->sources)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (ch->upstream->sources, up_node, child))
            pim_channel_del_inherited_oif (child->channel_oil, ch->interface,
                                           __func__);
        }
    }
  ...
  */
  ...
  else
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug (
            ...
            __func__, ch->upstream->ref_count, ch->interface->name,
            ch->sg_str);
    }
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: ifchannel entry %s(%s) is deleted ", __func__, ch->sg_str,
                ch->interface->name);
...
void
delete_on_noinfo (struct pim_ifchannel *ch)
{
...
void
pim_ifchannel_ifjoin_switch (const char *caller, struct pim_ifchannel *ch,
  ...
  if (old_state == new_state)
    {
      if (PIM_DEBUG_PIM_EVENTS)
        {
          zlog_debug ("%s called by %s: non-transition on state %d (%s)",
              ...
              for (ALL_LIST_ELEMENTS_RO (up->sources, up_node, child))
                {
                  ...
                  if (PIM_DEBUG_PIM_TRACE)
                    zlog_debug ("%s %s: Prune(S,G)=%s from %s", __FILE__,
                  ...
                  if (oil_if_has (c_oil, pim_ifp->mroute_vif_index))
                    pim_channel_del_inherited_oif (c_oil, ch->interface,
                                                   __func__);
                }
            }
          if (ch->ifjoin_state == PIM_IFJOIN_JOIN)
            {
              for (ALL_LIST_ELEMENTS_RO (up->sources, up_node, child))
                {
                  if (PIM_DEBUG_PIM_TRACE)
                    zlog_debug ("%s %s: Join(S,G)=%s from %s", __FILE__,
                                __func__, child->sg_str, up->sg_str);

                  /* check if the channel can be
                   * inherited into the SG's OIL
                   */
                  child_ch = pim_ifchannel_find (ch->interface, &child->sg);
                  if (pim_upstream_eval_inherit_if (child, child_ch, ch))
                    {
                      pim_channel_add_oif (child->channel_oil, ch->interface,
                                           PIM_OIF_FLAG_PROTO_STAR, __func__);
                      pim_upstream_update_join_desired (pim_ifp->pim, child);
                    }
                }
            }
        }
...
const char *
pim_ifchannel_ifjoin_name (enum pim_ifjoin_state ifjoin_state, int flags)
{
  switch (ifjoin_state)
    {
...
const char *
pim_ifchannel_ifassert_name (enum pim_ifassert_state ifassert_state)
{
...
*/
void
reset_ifassert_state (struct pim_ifchannel *ch)
{
  EVENT_OFF (ch->t_ifassert_timer);
...
pim_ifchannel_find (struct interface *ifp, pim_sgaddr *sg)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
...
static void
ifmembership_set (struct pim_ifchannel *ch, enum pim_ifmembership membership)
{
  struct pim_interface *pim_ifp = ch->interface->info;
...
void
pim_ifchannel_membership_clear (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
...
void
pim_ifchannel_delete_on_noinfo (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
pim_ifchannel_find_parent (struct pim_ifchannel *ch)
{
  ...
  // (S,G)
  if (! pim_addr_is_any (parent_sg.src) && ! pim_addr_is_any (parent_sg.grp))
    {
...
struct pim_ifchannel *
pim_ifchannel_add (struct interface *ifp, pim_sgaddr *sg, uint8_t source_flags,
                   int up_flags)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
  ...
  if (PIM_I_am_DualActive (pim_ifp))
    {
      ...
      if (PIM_DEBUG_MLAG)
        zlog_debug (
            ...
            __func__, up->sg_str, up->dualactive_ifchannel_count, up->flags);
    }
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: ifchannel %s(%s) is created ", __func__, ch->sg_str,
                ch->interface->name);
...
static void
ifjoin_to_noinfo (struct pim_ifchannel *ch)
{
  pim_ifchannel_ifjoin_switch (__func__, ch, PIM_IFJOIN_NOINFO);
  pim_forward_stop (ch);
...
static void
on_ifjoin_expiry_timer (struct event *t)
{
  struct pim_ifchannel *ch;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: ifchannel %s expiry timer", __func__, ch->sg_str);
...
static void
on_ifjoin_prune_pending_timer (struct event *t)
{
  struct pim_ifchannel *ch;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: IFCHANNEL%pSG %s Prune Pending Timer Popped", __func__,
                &ch->sg,
                pim_ifchannel_ifjoin_name (ch->ifjoin_state, ch->flags));
  ...
  if (ch->ifjoin_state == PIM_IFJOIN_PRUNE_PENDING)
    {
      ...
      if (! PIM_IF_FLAG_TEST_S_G_RPT (ch->flags))
      ...
      else
        {
          ...
          struct pim_upstream *parent = ch->upstream->parent;
          ...
          if (! ch->upstream->channel_oil->installed)
            pim_upstream_mroute_add (ch->upstream->channel_oil, __func__);
        }
...
static void
check_recv_upstream (int is_join, struct interface *recv_ifp,
                     ...
                     int holdtime)
{
  ...
  if (pim_rpf_addr_is_inaddr_any (&up->rpf))
    {
      /* RPF'(S,G) not found */
      zlog_warn ("%s %s: RPF'%s not found", __FILE__, __func__, up->sg_str);
      return;
    }
...
static int
nonlocal_upstream (int is_join, struct interface *recv_ifp, pim_addr upstream,
                   pim_sgaddr *sg, uint8_t source_flags, uint16_t holdtime)
{
  ...
  if (PIM_DEBUG_PIM_TRACE_DETAIL)
    zlog_warn ("%s: recv %s (S,G)=%pSG to non-local upstream=%pPAs on %s",
...
static void
pim_ifchannel_ifjoin_handler (struct pim_ifchannel *ch,
                              struct pim_interface *pim_ifp)
{
...
void
pim_ifchannel_join_add (struct interface *ifp, pim_addr neigh_addr,
                        ...
                        uint8_t source_flags, uint16_t holdtime)
{
  ...
  if (ch->ifassert_state == PIM_IFASSERT_I_AM_LOSER)
    {
      zlog_warn ("%s: Assert Loser recv Join%s from %pPA on %s", __func__,
                 ch->sg_str, &neigh_addr, ifp->name);
  ...
  switch (ch->ifjoin_state)
      ...
      if (ch->t_ifjoin_expiry_timer)
        {
          unsigned long remain =
              event_timer_remain_second (ch->t_ifjoin_expiry_timer);
          if (remain > holdtime)
            {
              /*
    ...
    case PIM_IFJOIN_PRUNE_PENDING:
      ...
      if ((source_flags & PIM_ENCODE_RPT_BIT) && ! pim_addr_is_any (sg->src))
        {
          /*
  ...
  if (holdtime != 0xFFFF)
    {
      event_add_timer (router->master, on_ifjoin_expiry_timer, ch, holdtime,
                       &ch->t_ifjoin_expiry_timer);
    }
}

void
pim_ifchannel_prune (struct interface *ifp, pim_addr upstream, pim_sgaddr *sg,
                     uint8_t source_flags, uint16_t holdtime)
{
  struct pim_ifchannel *ch;
  struct pim_interface *pim_ifp;
  ...
  if (! ch && ! (source_flags & PIM_ENCODE_RPT_BIT))
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug (
            "%s: Received prune with no relevant ifchannel %s%pSG state: %d",
            __func__, ifp->name, sg, source_flags);
      return;
    }
  ...
  switch (ch->ifjoin_state)
      ...
      if (source_flags & PIM_ENCODE_RPT_BIT)
        {
          ...
          EVENT_OFF (ch->t_ifjoin_prune_pending_timer);
          EVENT_OFF (ch->t_ifjoin_expiry_timer);
          event_add_timer_msec (router->master, on_ifjoin_prune_pending_timer,
    ...
    case PIM_IFJOIN_JOIN:
      ...
      pim_ifchannel_ifjoin_switch (__func__, ch, PIM_IFJOIN_PRUNE_PENDING);
      ...
      /* If we called ifjoin_prune() directly instead, care should
         ...
         deleted. */
      EVENT_OFF (ch->t_ifjoin_prune_pending_timer);
      event_add_timer_msec (router->master, on_ifjoin_prune_pending_timer, ch,
...
int
pim_ifchannel_local_membership_add (struct interface *ifp, pim_sgaddr *sg,
                                    bool is_vxlan)
{
  ...
  if (! pim_ifp)
    {
      if (PIM_DEBUG_EVENTS)
        zlog_debug ("%s:%pSG Expected pim interface setup for %s", __func__,
                    sg, ifp->name);
      return 0;
    }
      ...
      if (pim_is_grp_ssm (pim, sg->grp))
        {
          if (PIM_DEBUG_PIM_EVENTS)
            zlog_debug (
                "%s: local membership (S,G)=%pSG ignored as group is SSM",
                __func__, sg);
          return 1;
        }
    }
  ...
  if (pim_addr_is_any (sg->src))
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (up->sources, up_node, child))
        {
          if (PIM_DEBUG_EVENTS)
            zlog_debug ("%s %s: IGMP (S,G)=%s(%s) from %s", __FILE__, __func__,
          ...
          if (! child->rpf.source_nexthop.interface)
            {
              ...
              if (PIM_DEBUG_EVENTS)
                zlog_debug ("Skipped (S,G)=%s(%s) from %s: no iif",
                            child->sg_str, ifp->name, up->sg_str);
              continue;
            }
          ...
          if (pim->spt.plist)
            {
              struct prefix_list *plist =
                  prefix_list_lookup (AFI_IP, pim->spt.plist);
              struct prefix g;

              pim_addr_to_prefix (&g, up->sg.grp);
              if (prefix_list_apply_ext (plist, NULL, &g, true) == PREFIX_DENY)
                {
...
void
pim_ifchannel_local_membership_del (struct interface *ifp, pim_sgaddr *sg)
{
      ...
      for (ALL_LIST_ELEMENTS (up->sources, up_node, up_nnode, child))
        {
          ...
          if (PIM_DEBUG_EVENTS)
            zlog_debug ("%s %s: Prune(S,G)=%s(%s) from %s", __FILE__, __func__,
          ...
          if (! pim_upstream_evaluate_join_desired_interface (child, ch,
            ...
            {
              pim_channel_del_inherited_oif (c_oil, ifp, __func__);
            }
...
void
pim_ifchannel_update_could_assert (struct pim_ifchannel *ch)
{
  int old_couldassert =
 ...
 */
void
pim_ifchannel_update_my_assert_metric (struct pim_ifchannel *ch)
{
  ...
  if (PIM_DEBUG_PIM_EVENTS)
    zlog_debug (
        ...
        &ch->ifassert_my_metric.ip_address, my_metric_new.rpt_bit_flag,
        my_metric_new.metric_preference, my_metric_new.route_metric,
        &my_metric_new.ip_address);
...
void
pim_ifchannel_update_assert_tracking_desired (struct pim_ifchannel *ch)
{
  int old_atd =
      PIM_FORCE_BOOLEAN (PIM_IF_FLAG_TEST_ASSERT_TRACKING_DESIRED (ch->flags));
  int new_atd =
  ...
  if (PIM_DEBUG_PIM_EVENTS)
    zlog_debug (
        ...
        __func__, &ch->sg.src, &ch->sg.grp, ch->interface->name, old_atd,
        new_atd);
      ...
      switch (child->ifjoin_state)
        ...
        case PIM_IFJOIN_PRUNE_PENDING_TMP:
          ...
          if ((I_am_RP (pim, child->sg.grp)) &&
              (! pim_upstream_empty_inherited_olist (child->upstream)))
            {
              pim_channel_add_oif (child->upstream->channel_oil, ch->interface,
                                   PIM_OIF_FLAG_PROTO_STAR, __func__);
              pim_upstream_update_join_desired (pim, child->upstream);
            }
