...
static void
pim_bsm_rpinfo_free (struct bsm_rpinfo *bsrp_info)
{
...
static void
pim_bsm_rpinfos_free (struct bsm_rpinfos_head *head)
{
...
static void
pim_free_bsgrp_data (struct bsgrp_node *bsgrp_node)
{
...
static void
pim_free_bsgrp_node (struct route_table *rt, struct prefix *grp)
{
...
static void
pim_bsm_frag_free (struct bsm_frag *bsfrag)
{
  XFREE (MTYPE_PIM_BSM_FRAG, bsfrag);
}

static void
pim_bsm_frags_free (struct bsm_scope *scope)
{
...
int
pim_bsm_rpinfo_cmp (const struct bsm_rpinfo *node1,
...
static struct bsgrp_node *
pim_bsm_new_bsgrp_node (struct route_table *rt, struct prefix *grp)
{
  struct route_node *rn;
...
static void
pim_on_bs_timer (struct event *t)
{
  struct route_node *rn;
  ...
  if (PIM_DEBUG_BSM)
    zlog_debug ("%s: Bootstrap Timer expired for scope: %d", __func__,
                scope->sz_id);
...
static void
pim_bs_timer_stop (struct bsm_scope *scope)
{
  if (PIM_DEBUG_BSM)
...
static void
pim_bs_timer_start (struct bsm_scope *scope, int bs_timeout)
{
  if (! scope)
    {
      if (PIM_DEBUG_BSM)
...
static inline void
pim_bs_timer_restart (struct bsm_scope *scope, int bs_timeout)
{
  pim_bs_timer_start (scope, bs_timeout);
}

void
pim_bsm_proc_init (struct pim_instance *pim)
{
...
void
pim_bsm_proc_free (struct pim_instance *pim)
{
  struct route_node *rn;
  struct bsgrp_node *bsgrp;
...
static bool
is_hold_time_elapsed (void *data)
{
  struct bsm_rpinfo *bsrp;
...
static void
pim_on_g2rp_timer (struct event *t)
{
  struct bsm_rpinfo *bsrp;
  ...
  if (rp_info->rp_src != RP_SRC_STATIC)
    {
      ...
      if (bsrp)
        {
          pim_g2rp_timer_start (bsrp, (bsrp->rp_holdtime - bsrp->elapse_time));
          pim_rp_change (pim, bsrp->rp_address, bsgrp_node->group, RP_SRC_BSR);
        }
      else
        {
          pim_rp_del (pim, bsrp_addr, bsgrp_node->group, NULL, RP_SRC_BSR);
        }
    }
  ...
  if (! bsm_rpinfos_count (bsgrp_node->bsrp_list) &&
    ...
    {
      pim_free_bsgrp_node (pim->global_scope.bsrp_table, &bsgrp_node->group);
      pim_free_bsgrp_data (bsgrp_node);
    }
}

static void
pim_g2rp_timer_start (struct bsm_rpinfo *bsrp, int hold_time)
{
  if (! bsrp)
    {
      if (PIM_DEBUG_BSM)
  ...
  if (PIM_DEBUG_BSM)
    zlog_debug (
        ...
        __func__, &bsrp->bsgrp_node->group, &bsrp->rp_address, hold_time,
        bsrp->rp_holdtime);
...
static inline void
pim_g2rp_timer_restart (struct bsm_rpinfo *bsrp, int hold_time)
{
  pim_g2rp_timer_start (bsrp, hold_time);
}

static void
pim_g2rp_timer_stop (struct bsm_rpinfo *bsrp)
{
  if (! bsrp)
    return;

  if (PIM_DEBUG_BSM)
    zlog_debug ("%s : stopping g2rp timer for grp: %pFX - rp: %pPAs", __func__,
...
static void
pim_instate_pend_list (struct bsgrp_node *bsgrp_node)
{
  ...
  if (! rn || (prefix_same (&rp_all->group, &bsgrp_node->group) &&
               pim_rpf_addr_is_inaddr_any (&rp_all->rp)))
    {
      ...
      if (pend)
        pim_rp_new (pim, pend->rp_address, bsgrp_node->group, NULL,
                    RP_SRC_BSR);
      had_rp_node = false;
    }
  else
  ...
  if ((! had_rp_node) && (! pend))
    {
      pim_free_bsgrp_node (bsgrp_node->scope->bsrp_table, &bsgrp_node->group);
      pim_free_bsgrp_data (bsgrp_node);
      return;
    }
  ...
  if ((had_rp_node) && (rp_info->rp_src != RP_SRC_STATIC))
    {
      ...
      if (active && pend)
        {
          if (pim_addr_cmp (active->rp_address, pend->rp_address))
            pim_rp_change (pim, pend->rp_address, bsgrp_node->group,
                           RP_SRC_BSR);
        }
      ...
      if ((! active) && (! pend))
        {
          if (PIM_DEBUG_BSM)
            {
              zlog_debug ("%s: Both bsrp and partial list are empty",
                          __func__);
            }
      ...
      if ((active) && (! pend))
        {
          pim_rp_del (pim, active->rp_address, bsgrp_node->group, NULL,
                      RP_SRC_BSR);
          pim_free_bsgrp_node (bsgrp_node->scope->bsrp_table,
                               &bsgrp_node->group);
          if (PIM_DEBUG_BSM)
            {
              zlog_debug ("%s:Pend List is null,del grp node", __func__);
            }
          pim_free_bsgrp_data (bsgrp_node);
          return;
        }
  ...
  if ((had_rp_node) && (rp_info->rp_src == RP_SRC_STATIC))
    {
      ...
      if (! pend)
        {
          if (PIM_DEBUG_BSM)
            zlog_debug ("%s: Partial list is empty, static rp exists",
                        __func__);
          pim_free_bsgrp_node (bsgrp_node->scope->bsrp_table,
                               &bsgrp_node->group);
          pim_free_bsgrp_data (bsgrp_node);
   ...
   */
  bsm_rpinfos_swap_all (bsgrp_node->bsrp_list, bsgrp_node->partial_bsrp_list);
...
static bool
is_preferred_bsr (struct pim_instance *pim, pim_addr bsr, uint32_t bsr_prio)
{
...
static void
pim_bsm_update (struct pim_instance *pim, pim_addr bsr, uint32_t bsr_prio)
{
  if (pim_addr_cmp (bsr, pim->global_scope.current_bsr))
    {
...
void
pim_bsm_clear (struct pim_instance *pim)
{
  struct route_node *rn;
...
static bool
pim_bsm_send_intf (uint8_t *buf, int len, struct interface *ifp,
                   pim_addr dst_addr)
{
  ...
  if (! pim_ifp)
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: Pim interface not available for %s", __func__,
                    ifp->name);
      return false;
    }
  ...
  if (pim_ifp->pim_sock_fd == -1)
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: Pim sock not available for %s", __func__, ifp->name);
      return false;
    }

  if (pim_msg_send (pim_ifp->pim_sock_fd, pim_ifp->primary_address, dst_addr,
                    buf, len, ifp))
    {
      zlog_warn ("%s: Could not send BSM message on interface: %s", __func__,
                 ifp->name);
      return false;
    }
...
static bool
pim_bsm_frag_send (uint8_t *buf, uint32_t len, struct interface *ifp,
                   uint32_t pim_mtu, pim_addr dst_addr, bool no_fwd)
{
  ...
  if (pim_mtu < (PIM_MIN_BSM_LEN))
    {
      zlog_warn ("%s: mtu(pim mtu: %d) size less than minimum bootstrap len",
                 __func__, pim_mtu);
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: mtu (pim mtu:%d) less than minimum bootstrap len",
                    __func__, pim_mtu);
      return false;
    }
      ...
      while (this_rp_cnt < total_rp_cnt)
        {
          ...
          if ((this_rp_cnt < total_rp_cnt) ||
              (this_pkt_rem < (PIM_BSM_GRP_LEN + PIM_BSM_RP_LEN)))
            {
               ...
               * fill grp hdr
               */
              memcpy (pkt, (uint8_t *) grpinfo, PIM_BSM_GRP_LEN);
              curgrp = (struct bsmmsg_grpinfo *) pkt;
              pkt += PIM_BSM_GRP_LEN;
              this_pkt_rem -= PIM_BSM_GRP_LEN;
              pak_pending = false;
  ...
  if (pak_pending)
    {
      this_pkt_len = pim_mtu - this_pkt_rem;
      pim_msg_build_header (pim_ifp->primary_address, dst_addr, pak_start,
                            this_pkt_len, PIM_MSG_TYPE_BOOTSTRAP, no_fwd);
      pim_bsm_send_intf (pak_start, (pim_mtu - this_pkt_rem), ifp, dst_addr);
    }
...
static void
pim_bsm_fwd_whole_sz (struct pim_instance *pim, uint8_t *buf, uint32_t len,
                      int sz)
{
  ...
  FOR_ALL_INTERFACES (pim->vrf, ifp)
    {
      ...
      if (pim_mtu < len)
        {
          ret = pim_bsm_frag_send (buf, len, ifp, pim_mtu, dst_addr, no_fwd);
          if (PIM_DEBUG_BSM)
            zlog_debug ("%s: pim_bsm_frag_send returned %s", __func__,
                        ret ? "TRUE" : "FALSE");
        }
      else
        {
          pim_msg_build_header (pim_ifp->primary_address, dst_addr, buf, len,
                                PIM_MSG_TYPE_BOOTSTRAP, no_fwd);
          if (! pim_bsm_send_intf (buf, len, ifp, dst_addr))
            {
              if (PIM_DEBUG_BSM)
                zlog_debug ("%s: pim_bsm_send_intf returned false", __func__);
            }
        }
    }
}
...
bool
pim_bsm_new_nbr_fwd (struct pim_neighbor *neigh, struct interface *ifp)
{
  pim_addr dst_addr;
  struct pim_interface *pim_ifp;
  struct bsm_scope *scope;
  ...
  if (! pim_addr_cmp (pim_ifp->pim_dr_addr, pim_ifp->primary_address))
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: It is not DR, so don't forward BSM packet", __func__);
    }

  if (! pim_ifp->bsm_enable)
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: BSM proc not enabled on %s", __func__, ifp->name);
      return ret;
    }
  ...
  if (! bsm_frags_count (scope->bsm_frags))
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: BSM list for the scope is empty", __func__);
      return ret;
    }
  ...
  frr_each (bsm_frags, scope->bsm_frags, bsfrag)
    {
      if (pim_mtu < bsfrag->size)
        {
          ret = pim_bsm_frag_send (bsfrag->data, bsfrag->size, ifp, pim_mtu,
                                   dst_addr, no_fwd);
          if (! ret)
            {
              if (PIM_DEBUG_BSM)
                zlog_debug ("%s: pim_bsm_frag_send failed", __func__);
            }
        }
      else
        {
          ...
          pim_msg_build_header (pim_ifp->primary_address, dst_addr,
                                ...
                                PIM_MSG_TYPE_BOOTSTRAP, no_fwd);
          ret = pim_bsm_send_intf (bsfrag->data, bsfrag->size, ifp, dst_addr);
          if (! ret)
            {
              if (PIM_DEBUG_BSM)
                zlog_debug ("%s: pim_bsm_frag_send failed", __func__);
...
struct bsgrp_node *
pim_bsm_get_bsgrp_node (struct bsm_scope *scope, struct prefix *grp)
{
  struct route_node *rn;
  struct bsgrp_node *bsgrp;
  ...
  if (! rn)
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: Route node doesn't exist for the group", __func__);
      return NULL;
    }
  bsgrp = rn->info;
  route_unlock_node (rn);
...
static uint32_t
hash_calc_on_grp_rp (struct prefix group, pim_addr rp, uint8_t hashmasklen)
{
  uint64_t temp;
  uint32_t hash;
  uint32_t grpaddr;
...
#else
  rp_add =
      rp.s6_addr32[0] ^ rp.s6_addr32[1] ^ rp.s6_addr32[2] ^ rp.s6_addr32[3];
#endif
...
static bool
pim_install_bsm_grp_rp (struct pim_instance *pim, struct bsgrp_node *grpnode,
                        struct bsmmsg_rpinfo *rp)
{
  ...
  bsm_rpinfo->hash =
      hash_calc_on_grp_rp (grpnode->group, rp->rpaddr.addr, hashMask_len);
  if (bsm_rpinfos_add (grpnode->partial_bsrp_list, bsm_rpinfo) == NULL)
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s, bs_rpinfo node added to the partial bs_rplist.",
                    __func__);
      return true;
    }
...
static void
pim_update_pending_rp_cnt (struct bsm_scope *sz, struct bsgrp_node *bsgrp,
                           uint16_t bsm_frag_tag, uint32_t total_rp_count)
{
...
static bool
pim_bsm_parse_install_g2rp (struct bsm_scope *scope, uint8_t *buf, int buflen,
  ...
  while (buflen > offset)
    {
      ...
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s, Group %pPAs  Rpcount:%d Fragment-Rp-count:%d",
      ...
      if (grpinfo.group.mask > PIM_MAX_BITLEN)
        {
          if (PIM_DEBUG_BSM)
            zlog_debug ("%s, prefix length specified: %d is too long",
      ...
      if (grpinfo.rp_count == 0)
        {
          ...
          if (PIM_DEBUG_BSM)
            zlog_debug ("%s, Rp count is zero for group: %pPAs", __func__,
                        &grp_addr);

          old_rpinfo = bsm_rpinfos_first (bsgrp->bsrp_list);
          if (old_rpinfo)
            pim_rp_del (scope->pim, old_rpinfo->rp_address, group, NULL,
                        RP_SRC_BSR);
      ...
      if (! bsgrp)
        {
          if (PIM_DEBUG_BSM)
            zlog_debug ("%s, Create new  BSM Group node.", __func__);

          /* create a new node to be added to the tree. */
          bsgrp = pim_bsm_new_bsgrp_node (scope->bsrp_table, &group);

          if (! bsgrp)
            {
              zlog_debug ("%s, Failed to get the BSM group node.", __func__);
              continue;
            }
      ...
      pim_update_pending_rp_cnt (scope, bsgrp, bsm_frag_tag, grpinfo.rp_count);
      frag_rp_cnt = grpinfo.frag_rp_count;
      ins_count = 0;

      while (frag_rp_cnt--)
        {
          if (offset + (int) sizeof (struct bsmmsg_rpinfo) > buflen)
            {
              if (PIM_DEBUG_BSM)
                zlog_debug (
          ...
          if (PIM_DEBUG_BSM)
            {
              ...
              zlog_debug ("%s, Rp address - %pPAs; pri:%d hold:%d", __func__,
                          &rp_addr, rpinfo.rp_pri, rpinfo.rp_holdtime);
            }
...
int
pim_bsm_process (struct interface *ifp, pim_sgaddr *sg, uint8_t *buf,
                 uint32_t buf_size, bool no_fwd)
{
  ...
  if (! pim_ifp)
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s: multicast not enabled on interface %s", __func__,
                    ifp->name);
      return -1;
    }
  ...
  if (pim_ifp->pim_passive_enable)
    {
      if (PIM_DEBUG_PIM_PACKETS)
        zlog_debug ("skip receiving PIM message on passive interface %s",
                    ifp->name);
      return 0;
    }
  ...
  if (! pim_ifp->bsm_enable)
    {
      zlog_warn ("%s: BSM not enabled on interface %s", __func__, ifp->name);
  ...
  if (! empty_bsm)
    {
      ...
      if (msg_grp->group.sz)
        {
          if (PIM_DEBUG_BSM)
            zlog_debug ("%s : Administratively scoped range BSM received",
                        __func__);
  ...
  if (! is_preferred_bsr (pim, bsr_addr, bshdr->bsr_prio))
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s : Received a non-preferred BSM", __func__);
      pim->bsm_dropped++;
      return -1;
    }
  ...
  if (! pim_addr_cmp (sg->grp, qpim_all_pim_routers_addr))
    {
       ...
       * no-forward set
       */
      if (! no_fwd && ! pim_nht_bsr_rpf_check (pim, bsr_addr, ifp, sg->src))
        {
          if (PIM_DEBUG_BSM)
            zlog_debug ("BSM check: RPF to BSR %pPAs is not %pPA%%%s",
  ...
  else if (if_address_is_local (&sg->grp, PIM_AF, pim->vrf->vrf_id))
    {
      ...
      if (! pim_ifp->ucast_bsm_accept)
        {
          if (PIM_DEBUG_BSM)
            zlog_debug ("%s : Unicast BSM not enabled on interface %s",
                        __func__, ifp->name);
          pim_ifp->pim_ifstat_ucast_bsm_cfg_miss++;
          pim->bsm_dropped++;
          return -1;
    ...
    {
      if (PIM_DEBUG_BSM)
        zlog_debug ("%s : Invalid destination address", __func__);
      pim->bsm_dropped++;
      return -1;
    }
  ...
  if (! pim_bsm_parse_install_g2rp (
          ...
          (buf_size - PIM_BSM_HDR_LEN - PIM_MSG_HEADER_LEN), frag_tag))
    {
      if (PIM_DEBUG_BSM)
        {
  ...
  if (pim_ifp->pim->global_scope.bsm_frag_tag != frag_tag)
    {
      if (PIM_DEBUG_BSM)
        {
          zlog_debug ("%s: Current frag tag: %d Frag teg rcvd: %d", __func__,
  ...
  if (! no_fwd)
    {
      ...
      bsfrag =
          XCALLOC (MTYPE_PIM_BSM_FRAG, sizeof (struct bsm_frag) + buf_size);

      bsfrag->size = buf_size;
      memcpy (bsfrag->data, buf, buf_size);
      bsm_frags_add_tail (pim_ifp->pim->global_scope.bsm_frags, bsfrag);
    }
