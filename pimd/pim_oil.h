...
/*
  ...
  Case where (S, G) entry not installed in the kernel:
    ...
    the outgoing interface where join is received. Keep this entry in the
  stack, but don't install in the kernel(channel_oil.installed = 0).
  ...
  Case where (S, G) entry installed in the kernel:
    ...
    and install the entry in the kernel, if channel_oil not present, then
  create a new_channel oil(channel_oil.installed = 1).
...
oil_origin (struct channel_oil *c_oil)
{
  return &c_oil->oil.mfcc_origin;
}

static inline pim_addr *
oil_mcastgrp (struct channel_oil *c_oil)
{
  return &c_oil->oil.mfcc_mcastgrp;
}

static inline vifi_t *
oil_parent (struct channel_oil *c_oil)
{
  return &c_oil->oil.mfcc_parent;
}

static inline bool
oil_if_has (struct channel_oil *c_oil, vifi_t ifi)
{
  return ! ! c_oil->oil.mfcc_ttls[ifi];
}

static inline void
oil_if_set (struct channel_oil *c_oil, vifi_t ifi, uint8_t set)
{
  c_oil->oil.mfcc_ttls[ifi] = set;
}

static inline int
oil_if_cmp (struct mfcctl *oil1, struct mfcctl *oil2)
{
...
static inline bool
oil_if_has (struct channel_oil *c_oil, mifi_t ifi)
{
  return ! ! IF_ISSET (ifi, &c_oil->oil.mf6cc_ifset);
}

static inline void
oil_if_set (struct channel_oil *c_oil, mifi_t ifi, uint8_t set)
{
  if (set)
...
static inline int
oil_if_cmp (struct mf6cctl *oil1, struct mf6cctl *oil2)
{
