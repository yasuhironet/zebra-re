...
static void
tlv_trace_bool (const char *label, const char *tlv_name, const char *ifname,
                pim_addr src_addr, int isset, int value)
{
  if (isset)
    zlog_debug ("%s: PIM hello option from %pPAs on interface %s: %s=%d",
                label, &src_addr, ifname, tlv_name, value);
}

static void
tlv_trace_uint16 (const char *label, const char *tlv_name, const char *ifname,
                  pim_addr src_addr, int isset, uint16_t value)
{
  if (isset)
    zlog_debug ("%s: PIM hello option from %pPAs on interface %s: %s=%u",
                label, &src_addr, ifname, tlv_name, value);
}

static void
tlv_trace_uint32 (const char *label, const char *tlv_name, const char *ifname,
                  pim_addr src_addr, int isset, uint32_t value)
{
  if (isset)
    zlog_debug ("%s: PIM hello option from %pPAs on interface %s: %s=%u",
                label, &src_addr, ifname, tlv_name, value);
...
static void
tlv_trace_uint32_hex (const char *label, const char *tlv_name,
                      const char *ifname, pim_addr src_addr, int isset,
                      uint32_t value)
{
  if (isset)
    zlog_debug ("%s: PIM hello option from %pPAs on interface %s: %s=%08x",
                label, &src_addr, ifname, tlv_name, value);
}

static void
tlv_trace_list (const char *label, const char *tlv_name, const char *ifname,
                pim_addr src_addr, int isset, struct list *addr_list)
{
  if (isset)
    zlog_debug (
        "%s: PIM hello option from %pPAs on interface %s: %s size=%d list=%p",
        label, &src_addr, ifname, tlv_name,
        addr_list ? ((int) listcount (addr_list)) : -1, (void *) addr_list);
}
...
int
pim_hello_recv (struct interface *ifp, pim_addr src_addr, uint8_t *tlv_buf,
                int tlv_buf_size)
{
  ...
  while (tlv_curr < tlv_pastend)
    {
      ...
      if (remain < PIM_TLV_MIN_SIZE)
        {
          if (PIM_DEBUG_PIM_HELLO)
            zlog_debug (
                ...
                __func__, remain, PIM_TLV_MIN_SIZE, &src_addr, ifp->name);
      ...
      if ((tlv_curr + option_len) > tlv_pastend)
        {
          if (PIM_DEBUG_PIM_HELLO)
            zlog_debug (
                ...
                __func__, option_type, option_len, tlv_pastend - tlv_curr,
                &src_addr, ifp->name);
      ...
      if (PIM_DEBUG_PIM_HELLO)
        zlog_debug (
            ...
            __func__, remain, option_type, option_len, &src_addr, ifp->name);

      switch (option_type)
        {
        case PIM_MSG_OPTION_TYPE_HOLDTIME:
          if (pim_tlv_parse_holdtime (ifp->name, src_addr, &hello_options,
                                      &hello_option_holdtime, option_len,
                                      tlv_curr))
            {
        ...
        case PIM_MSG_OPTION_TYPE_LAN_PRUNE_DELAY:
          if (pim_tlv_parse_lan_prune_delay (
                  ...
                  &hello_option_override_interval, option_len, tlv_curr))
            {
              FREE_ADDR_LIST_THEN_RETURN (-4);
            }
          break;
        case PIM_MSG_OPTION_TYPE_DR_PRIORITY:
          if (pim_tlv_parse_dr_priority (ifp->name, src_addr, &hello_options,
                                         &hello_option_dr_priority, option_len,
                                         tlv_curr))
            {
        ...
        case PIM_MSG_OPTION_TYPE_GENERATION_ID:
          if (pim_tlv_parse_generation_id (ifp->name, src_addr, &hello_options,
        ...
        case PIM_MSG_OPTION_TYPE_ADDRESS_LIST:
          if (pim_tlv_parse_addr_list (ifp->name, src_addr, &hello_options,
                                       &hello_option_addr_list, option_len,
                                       tlv_curr))
            {
              return -7;
            }
        ...
        case PIM_MSG_OPTION_TYPE_DM_STATE_REFRESH:
          if (PIM_DEBUG_PIM_HELLO)
            zlog_debug (
                ...
                __func__, option_type, option_len, &src_addr, ifp->name);
          break;
        default:
          if (PIM_DEBUG_PIM_HELLO)
            zlog_debug (
  ...
  */
  ...
  if (PIM_DEBUG_PIM_HELLO)
    {
      tlv_trace_uint16 (
          __func__, "holdtime", ifp->name, src_addr,
      ...
      tlv_trace_bool (
          __func__, "can_disable_join_suppression", ifp->name, src_addr,
          PIM_OPTION_IS_SET (hello_options, PIM_OPTION_MASK_LAN_PRUNE_DELAY),
          PIM_OPTION_IS_SET (hello_options,
                             PIM_OPTION_MASK_CAN_DISABLE_JOIN_SUPPRESSION));
      tlv_trace_uint32 (
          __func__, "dr_priority", ifp->name, src_addr,
      ...
      tlv_trace_uint32_hex (
          ...
          hello_option_generation_id);
      tlv_trace_list (
          __func__, "address_list", ifp->name, src_addr,
  ...
  if (! neigh)
    {
      ...
      neigh = pim_neighbor_add (
          ...
          hello_option_propagation_delay, hello_option_override_interval,
      ...
      if (! pim_bsm_new_nbr_fwd (neigh, ifp))
        {
          if (PIM_DEBUG_PIM_HELLO)
            zlog_debug ("%s: forwarding bsm to new nbr failed", __func__);
        }
          ...
          (hello_option_generation_id != neigh->generation_id))
        {
          ...
          if (PIM_DEBUG_PIM_HELLO)
            zlog_debug (
                ...
                __func__, hello_option_generation_id, neigh->generation_id,
                &src_addr, ifp->name);

          pim_upstream_rpf_genid_changed (pim_ifp->pim, neigh->source_addr);
...
int
pim_hello_build_tlv (struct interface *ifp, uint8_t *tlv_buf, int tlv_buf_size,
                     ...
                     int can_disable_join_suppression)
{
  ...
  /* Holdtime */
  curr = pim_tlv_append_uint16 (curr, pastend, PIM_MSG_OPTION_TYPE_HOLDTIME,
                                holdtime);
  if (! curr)
    {
      if (PIM_DEBUG_PIM_HELLO)
  ...
  /* DR Priority */
  curr = pim_tlv_append_uint32 (curr, pastend, PIM_MSG_OPTION_TYPE_DR_PRIORITY,
                                dr_priority);
  if (! curr)
    {
      if (PIM_DEBUG_PIM_HELLO)
  ...
  curr = pim_tlv_append_uint32 (
      curr, pastend, PIM_MSG_OPTION_TYPE_GENERATION_ID, generation_id);
  if (! curr)
    {
      if (PIM_DEBUG_PIM_HELLO)
        {
  ...
  if (ifp->connected->count)
    {
      curr = pim_tlv_append_addrlist_ucast (curr, pastend, ifp, PIM_AF);
      if (! curr)
        {
          if (PIM_DEBUG_PIM_HELLO)
            {
      ...
      if (pim->send_v6_secondary)
        {
          curr = pim_tlv_append_addrlist_ucast (curr, pastend, ifp, AF_INET6);
          if (! curr)
            {
              if (PIM_DEBUG_PIM_HELLO)
                {
...
*/
void
pim_hello_require (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
