...
static void
zclient_lookup_connect (struct event *t)
{
...
static void
zclient_lookup_sched (struct zclient *zlookup, int delay)
{
...
static void
zclient_lookup_sched_now (struct zclient *zlookup)
{
  event_add_event (router->master, zclient_lookup_connect, zlookup, 0,
                   &zlookup->t_connect);

  zlog_notice ("%s: zclient lookup immediate connection scheduled", __func__);
}

/* Schedule reconnection, if needed. */
static void
zclient_lookup_reconnect (struct zclient *zlookup)
{
...
static void
zclient_lookup_failed (struct zclient *zlookup)
{
  if (zlookup->sock >= 0)
    {
      if (close (zlookup->sock))
...
zclient_lookup_new (void)
{
  ...
  if (! zlookup)
    {
      flog_err (EC_LIB_ZAPI_SOCKET, "%s: zclient_new() failure", __func__);
      return;
    }
...
static int
zclient_read_nexthop (struct pim_instance *pim, struct zclient *zlookup,
                      ...
                      const int tab_size, pim_addr addr)
{
  ...
  if (PIM_DEBUG_PIM_NHT_DETAIL)
    zlog_debug ("%s: addr=%pPAs(%s)", __func__, &addr, pim->vrf->name);
  ...
  if (raddr.ipa_type != PIM_IPADDR || pim_addr_cmp (raddr.ipaddr_pim, addr))
    {
      zlog_warn ("%s: address mismatch: addr=%pPA(%s) raddr=%pIA", __func__,
                 &addr, pim->vrf->name, &raddr);
      /* warning only */
    }
  ...
  for (i = 0; i < nexthop_num; ++i)
    {
      ...
      if (num_ifindex >= tab_size)
        {
          zlog_warn (
              ...
              __func__, (num_ifindex + 1), tab_size, &addr, pim->vrf->name);
          return num_ifindex;
        }
      ...
      switch (nexthop_type)
        ...
        case NEXTHOP_TYPE_IPV4:
          ...
          ++num_ifindex;
#else
          zlog_warn ("cannot use IPv4 nexthop %pI4(%d) for IPv6 %pPA", &nh_ip4,
                     nh_ifi, &addr);
#endif
          break;
        ...
        case NEXTHOP_TYPE_IPV6_IFINDEX:
           ...
           * secondary
           */
          struct interface *ifp = if_lookup_by_index (nh_ifi, nexthop_vrf_id);
          ...
          nexthop_tab[num_ifindex].nexthop_addr = nbr->source_addr;
          nexthop_tab[num_ifindex].ifindex = nh_ifi;
          ++num_ifindex;
#endif
          break;
...
static int
zclient_lookup_nexthop_once (struct pim_instance *pim,
                             struct pim_zlookup_nexthop nexthop_tab[],
                             const int tab_size, pim_addr addr)
{
  struct stream *s;
  ...
  if (zlookup->sock < 0)
    {
      flog_err (EC_LIB_ZAPI_SOCKET,
                "%s: zclient lookup socket is not connected", __func__);
      zclient_lookup_failed (zlookup);
      return -1;
    }
  ...
  if (pim->vrf->vrf_id == VRF_UNKNOWN)
    {
      zlog_notice ("%s: VRF: %s does not fully exist yet, delaying lookup",
...
void
zclient_lookup_read_pipe (struct event *thread)
{
...
int
zclient_lookup_nexthop (struct pim_instance *pim,
                        struct pim_zlookup_nexthop nexthop_tab[],
                        const int tab_size, pim_addr addr, int max_lookup)
{
  int lookup;
  ...
  for (lookup = 0; lookup < max_lookup; ++lookup)
    {
      ...
      num_ifindex =
          zclient_lookup_nexthop_once (pim, nexthop_tab, tab_size, addr);
      if (num_ifindex < 1)
        {
          if (PIM_DEBUG_PIM_NHT_DETAIL)
            zlog_debug (
          ...
          if (lookup > 0)
            {
              ...
              if (PIM_DEBUG_PIM_NHT)
                zlog_debug (
                    ...
                    __func__, lookup, max_lookup, first_ifindex, &addr,
                    pim->vrf->name, nexthop_tab[0].protocol_distance,
      ...
      if (PIM_DEBUG_PIM_NHT)
        zlog_debug (
            ...
            __func__, lookup, max_lookup, &nexthop_addr, &addr, pim->vrf->name,
            nexthop_tab[0].protocol_distance, nexthop_tab[0].route_metric);
...
pim_zlookup_sg_statistics (struct channel_oil *c_oil)
{
  ...
  stream_reset (s);
  zclient_create_header (s, ZEBRA_IPMR_ROUTE_STATS, c_oil->pim->vrf->vrf_id);
