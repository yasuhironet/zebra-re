...
int
pim_mroute_set (struct pim_instance *pim, int enable)
{
  int err;
  int opt, data;
      ...
      frr_with_privs (&pimd_privs)
        {
          ...
          err = setsockopt (pim->mroute_socket, PIM_IPPROTO, MRT_TABLE, &data,
                            data_len);
          if (err)
            {
              zlog_warn (
                  "%s %s: failure: setsockopt(fd=%d,PIM_IPPROTO, MRT_TABLE=%d): errno=%d: %s",
                  __FILE__, __func__, pim->mroute_socket, data, errno,
                  safe_strerror (errno));
              return -1;
            }
        }
  ...
  frr_with_privs (&pimd_privs)
    {
       ...
       */
      data = 1;
      err = setsockopt (pim->mroute_socket, PIM_IPPROTO, opt, &data, data_len);
      if (err)
        {
          zlog_warn (
  ...
  if (enable)
    {
      ...
      data = 1;
      if (setsockopt (pim->mroute_socket, PIM_IPPROTO, IP_PKTINFO, &data,
                      data_len))
        {
  ...
  if (enable)
    {
      ...
      data = 1;
      if (setsockopt (pim->mroute_socket, PIM_IPPROTO, IPV6_RECVPKTINFO, &data,
                      data_len))
        {
          zlog_warn (
  ...
  if (set_nonblocking (pim->mroute_socket) < 0)
    {
      zlog_warn ("Could not set non blocking on socket fd=%d: errno=%d: %s",
                 pim->mroute_socket, errno, safe_strerror (errno));
      return -1;
    }
...
int
pim_mroute_msg_nocache (int fd, struct interface *ifp, const kernmsg *msg)
{
  ...
  if (! pim_ifp || ! pim_ifp->pim_enable)
    {
      if (PIM_DEBUG_MROUTE)
        zlog_debug ("%s: %s on interface, dropping packet to %pSG", ifp->name,
                    ! pim_ifp ? "Multicast not enabled" : "PIM not enabled",
                    &sg);
      return 0;
    }
  ...
  if (! pim_is_grp_ssm (pim_ifp->pim, sg.grp))
    {
      ...
      if (pim_rpf_addr_is_inaddr_any (rpg))
        {
          if (PIM_DEBUG_MROUTE)
            zlog_debug ("%s: null RPF for packet to %pSG", ifp->name, &sg);
          return 0;
        }
    }
  ...
  if (! pim_if_connected_to_source (ifp, msg->msg_im_src))
    {
      if (PIM_DEBUG_MROUTE)
        zlog_debug ("%s: incoming packet to %pSG from non-connected source",
                    ifp->name, &sg);
      return 0;
    }
  ...
  if (! (PIM_I_am_DR (pim_ifp)))
    {
      ...
      if (PIM_DEBUG_MROUTE_DETAIL)
        zlog_debug ("%s: not DR on interface, not forwarding traffic for %pSG",
                    ifp->name, &sg);
  ...
  if (PIM_DEBUG_MROUTE)
    {
      zlog_debug ("%s: Adding a Route %s for WHOLEPKT consumption", __func__,
                  up->sg_str);
    }
  ...
  if (! up->t_rs_timer)
    {
      if (pim_is_grp_ssm (pim_ifp->pim, sg.grp))
        {
          if (PIM_DEBUG_PIM_REG)
            zlog_debug ("%pSG register forward skipped as group is SSM", &sg);
          return 0;
        }
      ...
      if (! PIM_UPSTREAM_FLAG_TEST_FHR (up->flags))
        {
          if (PIM_DEBUG_PIM_REG)
            zlog_debug ("%s register forward skipped, not FHR", up->sg_str);
          return 0;
        }

      pim_register_send ((uint8_t *) buf + sizeof (ipv_hdr),
                         len - sizeof (ipv_hdr), pim_ifp->primary_address, rpg,
                         0, up);
    }
  return 0;
}
...
int
pim_mroute_msg_wrongvif (int fd, struct interface *ifp, const kernmsg *msg)
{
      ...
      if (! PIM_UPSTREAM_FLAG_TEST_FHR (up->flags))
        {
          ...
          if (! (pim_addr_is_any (up->upstream_register)) &&
              pim_nexthop_lookup (pim_ifp->pim, &source, up->upstream_register,
                                  0))
            {
      ...
      if (! up)
        {
          if (PIM_DEBUG_MROUTE)
            zlog_debug (
                "%pSG: WRONGVIF%s unable to create upstream on interface", &sg,
                ifp->name);
          return -2;
        }
      PIM_UPSTREAM_FLAG_SET_SRC_STREAM (up->flags);
...
#if PIM_IPV == 4
static int
process_igmp_packet (struct pim_instance *pim, const char *buf,
                     size_t buf_size, ifindex_t ifindex)
{
  ...
  pim_ifp = ifp->info;
  ifaddr = connected_src ? connected_src->u.prefix4 : pim_ifp->primary_address;
  igmp = pim_igmp_sock_lookup_ifaddr (pim_ifp->gm_socket_list, ifaddr);

  if (PIM_DEBUG_GM_PACKETS)
    {
      zlog_debug ("%s(%s): igmp kernel upcall on %s(%p) for %pI4 -> %pI4",
                  __func__, pim->vrf->name, ifp->name, igmp, &ip_hdr->ip_src,
                  &ip_hdr->ip_dst);
    }
  if (igmp)
    pim_igmp_packet (igmp, (char *) buf, buf_size);
  else if (PIM_DEBUG_GM_PACKETS)
    zlog_debug ("No IGMP socket on interface: %s with connected source: %pI4",
                ifp->name, &ifaddr);
...
int
pim_mroute_msg (struct pim_instance *pim, const char *buf, size_t buf_size,
                ifindex_t ifindex)
{
  struct interface *ifp;
  const ipv_hdr *ip_hdr;
  ...
  if ((ip_hdr->ip6_vfc & 0xf) == 0)
    {
      ...
      switch (msg->msg_im_msgtype)
        {
        case GMMSG_WRONGVIF:
          return pim_mroute_msg_wrongvif (pim->mroute_socket, ifp, msg);
        case GMMSG_NOCACHE:
          return pim_mroute_msg_nocache (pim->mroute_socket, ifp, msg);
        case GMMSG_WHOLEPKT:
          return pim_mroute_msg_wholepkt (pim->mroute_socket, ifp,
                                          (const char *) msg, buf_size);
        case GMMSG_WRVIFWHOLE:
          return pim_mroute_msg_wrvifwhole (pim->mroute_socket, ifp,
                                            (const char *) msg, buf_size);
        default:
          break;
        }
...
static void
mroute_read_off (struct pim_instance *pim)
{
  EVENT_OFF (pim->thread);
}

int
pim_mroute_socket_enable (struct pim_instance *pim)
{
  int fd;
  ...
  frr_with_privs (&pimd_privs)
    {
      ...
      ret = setsockopt (fd, SOL_ICMPV6, ICMP6_FILTER, filter, sizeof (filter));
      if (ret)
        zlog_err ("(VRF %s) failed to set mroute control filter: %m",
                  pim->vrf->name);
#endif
  ...
  if (pim_mroute_set (pim, 1))
    {
      zlog_warn ("Could not enable mroute on socket fd=%d: errno=%d: %s", fd,
                 errno, safe_strerror (errno));
      close (fd);
      pim->mroute_socket = -1;
      return -3;
...
int
pim_mroute_socket_disable (struct pim_instance *pim)
{
 ...
 */
int
pim_mroute_add_vif (struct interface *ifp, pim_addr ifaddr,
                    unsigned char flags)
{
  struct pim_interface *pim_ifp = ifp->info;
  pim_vifctl vc;
  ...
  if (vc.vc_flags & VIFF_TUNNEL)
    {
      memcpy (&vc.vc_rmt_addr, &vif_remote_addr, sizeof (vc.vc_rmt_addr));
    }
#endif
#endif
  ...
  if (err)
    {
      zlog_warn (
          ...
          __func__, pim_ifp->pim->mroute_socket, ifp->ifindex, &ifaddr, flags,
          errno, safe_strerror (errno));
      return -2;
    }
...
int
pim_mroute_del_vif (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;
  pim_vifctl vc;
  int err;
 ...
 */
bool
pim_mroute_allow_iif_in_oil (struct channel_oil *c_oil, int oif_index)
{
...
static inline void
pim_mroute_copy (struct channel_oil *out, struct channel_oil *in)
{
  int i;
  ...
  *oil_parent (out) = *oil_parent (in);
  ...
  for (i = 0; i < MAXVIFS; ++i)
    {
      if (*oil_parent (out) == i && ! pim_mroute_allow_iif_in_oil (in, i))
        {
          oil_if_set (out, i, 0);
          continue;
        }
 ...
 */
static int
pim_mroute_add (struct channel_oil *c_oil, const char *name)
{
...
static int
pim_upstream_get_mroute_iif (struct channel_oil *c_oil, const char *name)
{
  vifi_t iif = MAXVIFS;
...
static int
pim_upstream_mroute_update (struct channel_oil *c_oil, const char *name)
{
  char buf[1000];
  ...
  if (*oil_parent (c_oil) >= MAXVIFS)
    {
      ...
      if (PIM_DEBUG_MROUTE)
        zlog_debug ("%s(%s) %s mroute not ready to be installed; %s", __func__,
 ...
 */
static void
pim_upstream_all_sources_iif_update (struct pim_upstream *up)
{
 ...
 */
int
pim_upstream_mroute_add (struct channel_oil *c_oil, const char *name)
{
  vifi_t iif;
  ...
  if (*oil_parent (c_oil) != iif)
    {
      *oil_parent (c_oil) = iif;
      if (pim_addr_is_any (*oil_origin (c_oil)) && c_oil->up)
 ...
 */
int
pim_upstream_mroute_iif_update (struct channel_oil *c_oil, const char *name)
{
  vifi_t iif;
  char buf[1000];
  ...
  *oil_parent (c_oil) = iif;
  ...
  if (PIM_DEBUG_MROUTE_DETAIL)
    zlog_debug ("%s(%s) %s mroute iif update %d", __func__, name,
...
int
pim_static_mroute_add (struct channel_oil *c_oil, const char *name)
{
  return pim_mroute_add (c_oil, name);
}

void
pim_static_mroute_iif_update (struct channel_oil *c_oil, int input_vif_index,
                              const char *name)
{
...
int
pim_mroute_del (struct channel_oil *c_oil, const char *name)
{
  struct pim_instance *pim = c_oil->pim;
  int err;
      ...
      if (PIM_DEBUG_MROUTE)
        {
          ...
          zlog_debug (
              "Channel%pSG is not installed no need to collect data from kernel",
              &sg);
        }
      return;
    }
  ...
  if (ioctl (pim->mroute_socket, PIM_SIOCGETSGCNT, &sgreq))
    {
      ...
      zlog_warn (
          ...
          (unsigned long) PIM_SIOCGETSGCNT, &sg, errno, safe_strerror (errno));
      return;
    }
