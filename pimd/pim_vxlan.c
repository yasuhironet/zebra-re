...
pim_vxlan_do_reg_work (void)
{
  ...
  if (sec_count > PIM_VXLAN_NULL_REG_INTERVAL)
    {
      sec_count = 0;
      listnode = vxlan_info.next_work ? vxlan_info.next_work
      ...
      if (work_cnt > vxlan_info.max_work_cnt)
        {
          ...
          if (PIM_DEBUG_VXLAN)
            zlog_debug ("vxlan SG %d work items proc and pause", work_cnt);
          return;
        }
    }
...
static void
pim_vxlan_add_work (struct pim_vxlan_sg *vxlan_sg)
{
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s work list add", vxlan_sg->sg_str);
  vxlan_sg->work_node = listnode_add (vxlan_info.work_list, vxlan_sg);
  /* XXX: adjust max_work_cnt if needed */
}

static void
pim_vxlan_del_work (struct pim_vxlan_sg *vxlan_sg)
{
  if (! vxlan_sg->work_node)
    return;

  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s work list del", vxlan_sg->sg_str);
...
void
pim_vxlan_update_sg_reg_state (struct pim_instance *pim,
...
static void
pim_vxlan_work_timer_cb (struct event *t)
{
...
static void
pim_vxlan_work_timer_setup (bool start)
{
 ...
 ***************************************************************************/
static void
pim_vxlan_orig_mr_up_del (struct pim_vxlan_sg *vxlan_sg)
{
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s orig mroute-up del", vxlan_sg->sg_str);
      ...
      if (up)
        {
          ...
          if (r == PIM_RPF_FAILURE)
            {
              if (PIM_DEBUG_VXLAN)
                zlog_debug ("vxlan SG %s rpf_update failure",
                            vxlan_sg->sg_str);
            }
        }
    }
...
static void
pim_vxlan_orig_mr_up_iif_update (struct pim_vxlan_sg *vxlan_sg)
{
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s orig mroute-up updated with iif %s",
                vxlan_sg->sg_str, vxlan_sg->iif ? vxlan_sg->iif->name : "-");
}
 ...
 */
static void
pim_vxlan_orig_mr_up_add (struct pim_vxlan_sg *vxlan_sg)
{
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s orig mroute-up add with iif %s", vxlan_sg->sg_str,
  ...
  if (up)
    {
      ...
      if (! PIM_UPSTREAM_FLAG_TEST_STATIC_IIF (up->flags))
        {
          pim_delete_tracked_nexthop (vxlan_sg->pim, up->upstream_addr, up,
                                      NULL);
        }
      ...
      if (term_ifp)
        pim_channel_update_oif_mute (up->channel_oil, term_ifp);
      pim_vxlan_orig_mr_up_iif_update (vxlan_sg);
      /* mute pimreg on origination mroutes */
      if (pim->regiface)
        pim_channel_update_oif_mute (up->channel_oil, pim->regiface->info);
    }
  else
    {
      up = pim_upstream_add (vxlan_sg->pim, &vxlan_sg->sg, vxlan_sg->iif,
                             flags, __func__, NULL);
      vxlan_sg->up = up;
    }
  ...
  if (! up)
    {
      if (PIM_DEBUG_VXLAN)
        zlog_debug ("vxlan SG %s orig mroute-up add failed", vxlan_sg->sg_str);
      return;
    }
...
static void
pim_vxlan_orig_mr_oif_add (struct pim_vxlan_sg *vxlan_sg)
{
  if (! vxlan_sg->up || ! vxlan_sg->orig_oif)
    return;

  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s oif %s add", vxlan_sg->sg_str,
                vxlan_sg->orig_oif->name);

  vxlan_sg->flags |= PIM_VXLAN_SGF_OIF_INSTALLED;
  pim_channel_add_oif (vxlan_sg->up->channel_oil, vxlan_sg->orig_oif,
                       PIM_OIF_FLAG_PROTO_VXLAN, __func__);
}

static void
pim_vxlan_orig_mr_oif_del (struct pim_vxlan_sg *vxlan_sg)
{
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s oif %s del", vxlan_sg->sg_str, orig_oif->name);

  vxlan_sg->flags &= ~PIM_VXLAN_SGF_OIF_INSTALLED;
  pim_channel_del_oif (vxlan_sg->up->channel_oil, orig_oif,
                       PIM_OIF_FLAG_PROTO_VXLAN, __func__);
}

static inline struct interface *
pim_vxlan_orig_mr_oif_get (struct pim_instance *pim)
{
  return (vxlan_mlag.flags & PIM_VXLAN_MLAGF_ENABLED) ? pim->vxlan.peerlink_rif
                                                      : NULL;
}
 ...
 */
static inline struct interface *
pim_vxlan_orig_mr_iif_get (struct pim_instance *pim)
{
  return ((vxlan_mlag.flags & PIM_VXLAN_MLAGF_ENABLED) &&
          pim->vxlan.peerlink_rif)
             ? pim->vxlan.peerlink_rif
             : pim->vxlan.default_iif;
}

static bool
pim_vxlan_orig_mr_add_is_ok (struct pim_vxlan_sg *vxlan_sg)
{
...
static void
pim_vxlan_orig_mr_install (struct pim_vxlan_sg *vxlan_sg)
{
...
static void
pim_vxlan_orig_mr_add (struct pim_vxlan_sg *vxlan_sg)
{
...
static void
pim_vxlan_orig_mr_del (struct pim_vxlan_sg *vxlan_sg)
{
  if (PIM_DEBUG_VXLAN)
...
static void
pim_vxlan_orig_mr_iif_update (struct hash_bucket *bucket, void *arg)
{
  struct interface *ifp;
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s iif changed from %s to %s", vxlan_sg->sg_str,
                old_iif ? old_iif->name : "-", ifp ? ifp->name : "-");
...
pim_vxlan_get_term_ifp (struct pim_instance *pim)
{
  return pim->vxlan.term_if ? (struct pim_interface *) pim->vxlan.term_if->info
                            : NULL;
}

static void
pim_vxlan_term_mr_oif_add (struct pim_vxlan_sg *vxlan_sg)
{
  if (vxlan_sg->flags & PIM_VXLAN_SGF_OIF_INSTALLED)
    return;

  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s term-oif %s add", vxlan_sg->sg_str,
                vxlan_sg->term_oif->name);

  if (pim_ifchannel_local_membership_add (vxlan_sg->term_oif, &vxlan_sg->sg,
                                          true /*is_vxlan */))
    {
      vxlan_sg->flags |= PIM_VXLAN_SGF_OIF_INSTALLED;
      /* update the inherited OIL */
    ...
    {
      zlog_warn ("vxlan SG %s term-oif %s add failed", vxlan_sg->sg_str,
                 vxlan_sg->term_oif->name);
    }
}

static void
pim_vxlan_term_mr_oif_del (struct pim_vxlan_sg *vxlan_sg)
{
  if (! (vxlan_sg->flags & PIM_VXLAN_SGF_OIF_INSTALLED))
    return;

  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s oif %s del", vxlan_sg->sg_str,
                vxlan_sg->term_oif->name);
...
static void
pim_vxlan_update_sg_entry_mlag (struct pim_instance *pim,
 ...
 */
void
pim_vxlan_inherit_mlag_flags (struct pim_instance *pim,
                              struct pim_upstream *up, bool inherit)
{
  struct listnode *listnode;
  ...
  for (ALL_LIST_ELEMENTS_RO (up->sources, listnode, child))
    {
      pim_vxlan_update_sg_entry_mlag (pim, child, true /* inherit */);
    }
}

static void
pim_vxlan_term_mr_up_add (struct pim_vxlan_sg *vxlan_sg)
{
  struct pim_upstream *up;
  int flags = 0;
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s term mroute-up add", vxlan_sg->sg_str);
  ...
  up = pim_upstream_add (vxlan_sg->pim, &vxlan_sg->sg, NULL /* iif */, flags,
                         __func__, NULL);
  vxlan_sg->up = up;

  if (! up)
    {
      zlog_warn ("vxlan SG %s term mroute-up add failed", vxlan_sg->sg_str);
      return;
    }
...
static void
pim_vxlan_term_mr_up_del (struct pim_vxlan_sg *vxlan_sg)
{
  struct pim_upstream *up = vxlan_sg->up;
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s term mroute-up del", vxlan_sg->sg_str);
  vxlan_sg->up = NULL;
  if (up->flags & PIM_UPSTREAM_FLAG_MASK_SRC_VXLAN_TERM)
    {
      /* update SG entries that are inheriting from this XG entry */
      pim_vxlan_inherit_mlag_flags (vxlan_sg->pim, up, false /*enable*/);
...
static void
pim_vxlan_term_mr_add (struct pim_vxlan_sg *vxlan_sg)
{
  if (PIM_DEBUG_VXLAN)
...
static void
pim_vxlan_term_mr_del (struct pim_vxlan_sg *vxlan_sg)
{
  if (PIM_DEBUG_VXLAN)
...
static unsigned int
pim_vxlan_sg_hash_key_make (const void *p)
{
...
static bool
pim_vxlan_sg_hash_eq (const void *p1, const void *p2)
{
...
static struct pim_vxlan_sg *
pim_vxlan_sg_new (struct pim_instance *pim, pim_sgaddr *sg)
{
  struct pim_vxlan_sg *vxlan_sg;
...
struct pim_vxlan_sg *
pim_vxlan_sg_find (struct pim_instance *pim, pim_sgaddr *sg)
{
...
struct pim_vxlan_sg *
pim_vxlan_sg_add (struct pim_instance *pim, pim_sgaddr *sg)
{
  struct pim_vxlan_sg *vxlan_sg;
...
static void
pim_vxlan_sg_del_item (struct pim_vxlan_sg *vxlan_sg)
{
...
void
pim_vxlan_sg_del (struct pim_instance *pim, pim_sgaddr *sg)
{
  struct pim_vxlan_sg *vxlan_sg;
 ...
 */
static void
pim_vxlan_sg_peerlink_oif_update (struct hash_bucket *bucket, void *arg)
{
 ...
 */
bool
pim_vxlan_get_register_src (struct pim_instance *pim, struct pim_upstream *up,
                            struct in_addr *src_p)
{
...
void
pim_vxlan_mlag_update (bool enable, bool peer_state, uint32_t role,
                       ...
                       struct in_addr *reg_addr)
{
  ...
  if (PIM_DEBUG_VXLAN)
    {
      inet_ntop (AF_INET, reg_addr, addr_buf, INET_ADDRSTRLEN);
      zlog_debug ("vxlan MLAG update %s state %s role %d rif %s addr %s",
                  enable ? "enable" : "disable", peer_state ? "up" : "down",
                  role, peerlink_rif ? peerlink_rif->name : "-", addr_buf);
    }
...
/****************************** misc callbacks *******************************/
static void
pim_vxlan_set_default_iif (struct pim_instance *pim, struct interface *ifp)
{
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("%s: vxlan default iif changed from %s to %s", __func__,
                old_iif ? old_iif->name : "-", ifp ? ifp->name : "-");
  ...
  if (pim->vxlan.sg_hash)
    hash_iterate (pim->vxlan.sg_hash, pim_vxlan_orig_mr_iif_update, NULL);
}

static void
pim_vxlan_up_cost_update (struct pim_instance *pim, struct pim_upstream *up,
                          struct interface *old_peerlink_rif)
{
  ...
  if (up->rpf.source_nexthop.interface &&
      ((up->rpf.source_nexthop.interface == pim->vxlan.peerlink_rif) ||
       (up->rpf.source_nexthop.interface == old_peerlink_rif)))
    {
      if (PIM_DEBUG_VXLAN)
        zlog_debug (
            "RPF cost adjust for %s on peerlink-rif (old: %s, new: %s) change",
            up->sg_str, old_peerlink_rif ? old_peerlink_rif->name : "-",
            pim->vxlan.peerlink_rif ? pim->vxlan.peerlink_rif->name : "-");
      pim_mlag_up_local_add (pim, up);
    }
}

static void
pim_vxlan_term_mr_cost_update (struct hash_bucket *bucket, void *arg)
{
  ...
  pim_vxlan_up_cost_update (vxlan_sg->pim, up, old_peerlink_rif);

  for (ALL_LIST_ELEMENTS_RO (up->sources, listnode, child))
    pim_vxlan_up_cost_update (vxlan_sg->pim, child, old_peerlink_rif);
}

static void
pim_vxlan_sg_peerlink_rif_update (struct hash_bucket *bucket, void *arg)
{
...
static void
pim_vxlan_set_peerlink_rif (struct pim_instance *pim, struct interface *ifp)
{
  struct interface *old_iif;
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("%s: vxlan peerlink_rif changed from %s to %s", __func__,
                old_iif ? old_iif->name : "-", ifp ? ifp->name : "-");
  ...
  if (old_iif != new_iif)
    {
       ...
       */
      if (pim->vxlan.sg_hash)
        hash_iterate (pim->vxlan.sg_hash, pim_vxlan_sg_peerlink_rif_update,
                      old_iif);
    }
  ...
  if (old_oif != new_oif)
    {
      if (PIM_DEBUG_VXLAN)
        zlog_debug ("%s: vxlan orig oif changed from %s to %s", __func__,
                    old_oif ? old_oif->name : "-",
                    new_oif ? new_oif->name : "-");
      if (pim->vxlan.sg_hash)
        hash_iterate (pim->vxlan.sg_hash, pim_vxlan_sg_peerlink_oif_update,
                      new_oif);
    }
}

static void
pim_vxlan_term_mr_oif_update (struct hash_bucket *bucket, void *arg)
{
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan SG %s term oif changed from %s to %s", vxlan_sg->sg_str,
...
static void
pim_vxlan_term_oif_update (struct pim_instance *pim, struct interface *ifp)
{
  ...
  if (pim->vxlan.sg_hash)
    hash_iterate (pim->vxlan.sg_hash, pim_vxlan_term_mr_oif_update, ifp);
}

void
pim_vxlan_add_vif (struct interface *ifp)
{
...
void
pim_vxlan_del_vif (struct interface *ifp)
{
  struct pim_interface *pim_ifp = ifp->info;
  struct pim_instance *pim = pim_ifp->pim;
...
void
pim_vxlan_add_term_dev (struct pim_instance *pim, struct interface *ifp)
{
  struct pim_interface *pim_ifp;
  ...
  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan term oif cfg changed from %s to %s",
                pim->vxlan.term_if_cfg ? pim->vxlan.term_if_cfg->name : "-",
                ifp->name);
  ...
  if (pim_ifp)
  ...
  else
    {
      ...
      (void) pim_if_new (ifp, false /*igmp*/, true /*pim*/, false /*pimreg*/,
                         true /*vxlan_term*/);
    }
}
...
void
pim_vxlan_del_term_dev (struct pim_instance *pim)
{
  struct interface *ifp = pim->vxlan.term_if_cfg;
  struct pim_interface *pim_ifp;

  if (PIM_DEBUG_VXLAN)
    zlog_debug ("vxlan term oif cfg changed from %s to -", ifp->name);
...
void
pim_vxlan_init (struct pim_instance *pim)
{
  char hash_name[64];

  snprintf (hash_name, sizeof (hash_name), "PIM %s vxlan SG hash",
            pim->vrf->name);
...
void
pim_vxlan_exit (struct pim_instance *pim)
{
