  ...
  if (nbr)
    {
      if (PIM_DEBUG_TRACE)
        zlog_debug ("up %s remove from nbr %s/%pPAs jp-agg-list", up->sg_str,
                    nbr->interface->name, &nbr->source_addr);
    }
...
void
pim_jp_agg_add_group (struct list *group, struct pim_upstream *up,
  ...
  if (nbr)
    {
      if (PIM_DEBUG_TRACE)
        zlog_debug ("up %s add to nbr %s/%pPAs jp-agg-list", up->sg_str,
                    up->rpf.source_nexthop.interface->name, &nbr->source_addr);
    }
...
void
pim_jp_agg_switch_interface (struct pim_rpf *orpf, struct pim_rpf *nrpf,
...
void
pim_jp_agg_single_upstream_send (struct pim_rpf *rpf, struct pim_upstream *up,
                                 bool is_join)
{
