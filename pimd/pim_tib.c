...
tib_sg_oil_setup (struct pim_instance *pim, pim_sgaddr sg,
                  struct interface *oif)
{
  ...
  if (up)
    {
      ...
      (void) pim_ecmp_nexthop_lookup (pim, &nexthop, vif_source, &grp, 0);
      if (nexthop.interface)
        input_iface_vif_index =
            pim_if_find_vifindex_by_ifindex (pim, nexthop.interface->ifindex);
    }
  else
    input_iface_vif_index =
  ...
  if (PIM_DEBUG_ZEBRA)
    zlog_debug ("%s: NHT %pSG vif_source %pPAs vif_index:%d", __func__, &sg,
                &vif_source, input_iface_vif_index);

  if (input_iface_vif_index < 1)
    {
      if (PIM_DEBUG_GM_TRACE)
        zlog_debug ("%s %s: could not find input interface for %pSG", __FILE__,
                    __func__, &sg);
      ...
      ! (PIM_I_am_DR (pim_oif)))
    {
      ...
      if (PIM_DEBUG_GM_TRACE)
        zlog_debug (
            ...
            __func__, &sg, oif->name, input_iface_vif_index);
...
bool
tib_sg_gm_join (struct pim_instance *pim, pim_sgaddr sg, struct interface *oif,
                struct channel_oil **oilp)
{
  ...
  if (! pim_oif)
    {
      if (PIM_DEBUG_GM_TRACE)
        zlog_debug ("%s: multicast not enabled on oif=%s?", __func__,
                    oif->name);
      return false;
    }
  ...
  if (PIM_I_am_DR (pim_oif) || PIM_I_am_DualActive (pim_oif))
    {
      ...
      result =
          pim_channel_add_oif (*oilp, oif, PIM_OIF_FLAG_PROTO_GM, __func__);
      if (result)
        {
          if (PIM_DEBUG_MROUTE)
            zlog_warn ("%s: add_oif() failed with return=%d", __func__,
                       result);
          return false;
        }
    }
  ...
  if (! pim_ifchannel_local_membership_add (oif, &sg, false /*is_vxlan*/))
    {
      if (PIM_DEBUG_MROUTE)
        zlog_warn ("%s: Failure to add local membership for %pSG", __func__,
                   &sg);
...
void
tib_sg_gm_prune (struct pim_instance *pim, pim_sgaddr sg,
