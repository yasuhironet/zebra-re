...
void
pim_msdp_write (struct event *thread)
{
          ...
          if (ERRNO_IO_RETRY (errno))
            {
              if (PIM_DEBUG_MSDP_INTERNAL)
                {
                  zlog_debug ("MSDP peer %s pim_msdp_write io retry",
                              mp->key_str);
                }
              break;
            }
  ...
  if (PIM_DEBUG_MSDP_INTERNAL)
    {
      zlog_debug ("MSDP peer %s pim_msdp_write wrote %d packets", mp->key_str,
                  work_cnt);
    }
}
...
static void
pim_msdp_pkt_sa_push_to_one_peer (struct pim_instance *pim,
                                  struct pim_msdp_peer *mp)
{
  struct stream *s;
...
static void
pim_msdp_pkt_sa_push (struct pim_instance *pim, struct pim_msdp_peer *mp)
{
...
static int
pim_msdp_pkt_sa_fill_hdr (struct pim_instance *pim, int local_cnt,
                          struct in_addr rp)
{
  int curr_tlv_ecnt;
...
static void
pim_msdp_pkt_sa_gen (struct pim_instance *pim, struct pim_msdp_peer *mp)
{
  ...
  local_cnt =
      pim_msdp_pkt_sa_fill_hdr (pim, local_cnt, pim->msdp.originator_id);
...
static void
pim_msdp_pkt_sa_tx_done (struct pim_instance *pim)
{
  struct listnode *mpnode;
  struct pim_msdp_peer *mp;
...
void
pim_msdp_pkt_sa_tx (struct pim_instance *pim)
{
...
void
pim_msdp_pkt_sa_tx_one_to_one_peer (struct pim_msdp_peer *mp,
  ...
  if (prefix_len != IPV4_MAX_BITLEN)
    {
      ...
      flog_err (EC_PIM_MSDP_PACKET,
                "rxed sa update with invalid prefix length %d", prefix_len);
      return;
    }
  if (PIM_DEBUG_MSDP_PACKETS)
    {
      ...
      if (PIM_DEBUG_MSDP_INTERNAL)
        {
          zlog_debug (
              ...
              mp->key_str, mp->packet_size, old_endp, new_endp);
        }
      return -1;
    }
...
void
pim_msdp_read (struct event *thread)
{
  struct pim_msdp_peer *mp;
  int rc;
  uint32_t len;
