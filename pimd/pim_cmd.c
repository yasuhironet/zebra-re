...
static struct vrf *
pim_cmd_lookup_vrf (struct vty *vty, struct cmd_token *argv[], const int argc,
                    int *idx, bool uj)
{
  struct vrf *vrf;
...
static void
pim_show_assert_helper (struct vty *vty, struct pim_interface *pim_ifp,
...
static void
pim_show_assert_internal_helper (struct vty *vty,
                                 struct pim_interface *pim_ifp,
                                 struct pim_ifchannel *ch)
{
  struct in_addr ifaddr;
...
static void
pim_show_assert_metric_helper (struct vty *vty, struct pim_interface *pim_ifp,
                               struct pim_ifchannel *ch)
{
  ...
  vty_out (vty, "%-16s %-15s %-15pPAs %-15pPAs %-3s %4u %6u %-15s\n",
           ch->interface->name,
           inet_ntop (AF_INET, &ifaddr, buf, sizeof (buf)), &ch->sg.src,
           &ch->sg.grp, am.rpt_bit_flag ? "yes" : "no", am.metric_preference,
...
static void
pim_show_assert_winner_metric_helper (struct vty *vty,
                                      struct pim_interface *pim_ifp,
                                      struct pim_ifchannel *ch)
{
  char addr_str[INET_ADDRSTRLEN];
  ...
  vty_out (vty, "%-16s %-15s %-15pPAs %-15pPAs %-3s %-4s %-6s %-15s\n",
           ch->interface->name,
           inet_ntop (AF_INET, &ifaddr, buf, sizeof (buf)), &ch->sg.src,
           &ch->sg.grp, am->rpt_bit_flag ? "yes" : "no", pref_str, metr_str,
           addr_str);
}
...
static void
igmp_show_interfaces (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct interface *ifp;
  time_t now;
  char buf[PREFIX_STRLEN];
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->gm_socket_list, sock_node, igmp))
        {
          ...
          pim_time_timer_to_hhmmss (query_hhmmss, sizeof (query_hhmmss),
          ...
          if (uj)
            {
              ...
              if (igmp->t_igmp_query_timer)
                {
                  ...
                  json_object_string_add (json_row, "queryTimer",
                                          query_hhmmss);
                }
              json_object_string_addf (json_row, "querierIp", "%pI4",
                                       &igmp->querier_addr);
              ...
              if (igmp->mtrace_only)
                {
                  json_object_boolean_true_add (json_row, "mtraceOnly");
                }
            }
          else
            {
              vty_out (vty, "%-16s  %5s  %15s  %d  %7s  %17pI4  %11s  %8s\n",
                       ifp->name,
...
static void
igmp_show_interfaces_single (struct pim_instance *pim, struct vty *vty,
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->gm_socket_list, sock_node, igmp))
        {
          ...
          pim_time_timer_to_hhmmss (query_hhmmss, sizeof (query_hhmmss),
                                    igmp->t_igmp_query_timer);
          pim_time_timer_to_hhmmss (other_hhmmss, sizeof (other_hhmmss),
          ...
          qri_msec = pim_ifp->gm_query_max_response_time_dsec * 100;
          ...
          if (uj)
            {
              ...
              json_object_string_add (json_row, "queryQueryTimer",
                                      query_hhmmss);
              json_object_string_add (json_row, "queryOtherTimer",
                                      other_hhmmss);
              json_object_int_add (json_row, "version", pim_ifp->igmp_version);
              json_object_int_add (
                  json_row, "timerGroupMembershipIntervalMsec", gmi_msec);
              json_object_int_add (json_row, "lastMemberQueryCount", lmqc);
              json_object_int_add (json_row, "timerLastMemberQueryMsec",
                                   lmqt_msec);
              json_object_int_add (
                  json_row, "timerOlderHostPresentIntervalMsec", ohpi_msec);
              json_object_int_add (
                  json_row, "timerOtherQuerierPresentIntervalMsec", oqpi_msec);
              json_object_int_add (json_row, "timerQueryInterval",
                                   igmp->querier_query_interval);
              json_object_int_add (json_row, "timerQueryResponseIntervalMsec",
                                   qri_msec);
              json_object_int_add (json_row, "timerRobustnessVariable",
          ...
          else
            {
              ...
              vty_out (vty, "State     : %s\n",
                       if_is_up (ifp) ? (igmp->mtrace_only ? "mtrace" : "up")
                                      : "down");
              ...
              vty_out (vty, "Querier     : %s\n",
                       igmp->t_igmp_query_timer ? "local" : "other");
              vty_out (vty, "QuerierIp   : %pI4", &igmp->querier_addr);
              if (pim_ifp->primary_address.s_addr == igmp->querier_addr.s_addr)
              ...
              vty_out (vty, "Group Membership Interval      : %lis\n",
                       gmi_msec / 1000);
              vty_out (vty, "Last Member Query Count        : %d\n", lmqc);
...
static void
igmp_show_interface_join (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct interface *ifp;
  time_t now;
  json_object *json = NULL;
  ...
  if (uj)
    {
      json = json_object_new_object ();
      json_object_string_add (json, "vrf", vrf_id_to_name (pim->vrf->vrf_id));
    }
  else
    {
      vty_out (
              ...
              if (! json_iface)
                {
                  ...
                  json_object_object_add (json_iface, "groups", json_grp_arr);
                }

              json_grp = json_object_new_object ();
              json_object_string_add (json_grp, "source", source_str);
              json_object_string_add (json_grp, "group", group_str);
              json_object_string_add (json_grp, "primaryAddr", pri_addr_str);
              json_object_int_add (json_grp, "sockFd", ij->sock_fd);
              json_object_string_add (json_grp, "upTime", uptime);
              json_object_array_add (json_grp_arr, json_grp);
            }
          else
            {
              vty_out (vty, "%-16s %-15s %-15s %-15s %6d %8s\n", ifp->name,
                       pri_addr_str, source_str, group_str, ij->sock_fd,
                       uptime);
            }
...
static void
igmp_show_statistics (struct pim_instance *pim, struct vty *vty,
                      const char *ifname, bool uj)
{
  struct interface *ifp;
  ...
  FOR_ALL_INTERFACES (pim->vrf, ifp)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->gm_group_list, group_node, group))
        {
          for (ALL_LIST_ELEMENTS_RO (group->group_source_list, source_node,
                                     src))
            {
  ...
  if (uj)
    {
      ...
      json_object_string_add (json_row, "name", ifname ? ifname : "global");
      ...
      json_object_int_add (json_row, "mtraceResponse", igmp_stats.mtrace_rsp);
      json_object_int_add (json_row, "mtraceRequest", igmp_stats.mtrace_req);
      json_object_int_add (json_row, "unsupported", igmp_stats.unsupported);
      json_object_int_add (json_row, "totalReceivedMessages",
                           igmp_stats.total_recv_messages);
      json_object_int_add (json_row, "peakGroups", igmp_stats.peak_groups);
      json_object_int_add (json_row, "totalGroups", igmp_stats.total_groups);
      json_object_int_add (json_row, "totalSourceGroups",
                           igmp_stats.total_source_groups);
      json_object_int_add (json_row, "joinsFailed", igmp_stats.joins_failed);
      json_object_int_add (json_row, "joinsSent", igmp_stats.joins_sent);
      ...
      json_object_object_add (json, ifname ? ifname : "global", json_row);
      vty_json (vty, json);
    }
  else
    {
...
static void
igmp_source_json_helper (struct gm_source *src, json_object *json_sources,
...
static void
igmp_group_print (struct interface *ifp, struct vty *vty, bool uj,
                  json_object *json, struct gm_group *grp, time_t now,
                  bool detail)
{
  json_object *json_iface = NULL;
      ...
      if (json_groups)
        {
          ...
          if (grp->igmp_version == IGMP_DEFAULT_VERSION)
            json_object_string_add (json_group, "mode",
                                    grp->group_filtermode_isexcl ? "EXCLUDE"
                                                                 : "INCLUDE");
          ...
          json_object_int_add (json_group, "version", grp->igmp_version);
          ...
          if (detail)
            {
              ...
              json_object_object_add (json_group, "sources", json_sources);
              ...
              for (ALL_LIST_ELEMENTS_RO (grp->group_source_list, srcnode, src))
                {
                  ...
                  pim_time_timer_to_mmss (mmss, sizeof (mmss),
                                          src->t_source_timer);
                  pim_time_uptime (src_uptime, sizeof (src_uptime),
...
static void
igmp_show_groups_interface_single (struct pim_instance *pim, struct vty *vty,
                                   bool uj, const char *ifname,
  ...
  if (uj)
    {
      ...
      json_object_int_add (json, "watermarkLimit", pim->gm_watermark_limit);
    }
  else
    {
...
static void
igmp_show_groups (struct pim_instance *pim, struct vty *vty, bool uj,
                  const char *grp_str, bool detail)
{
  struct interface *ifp;
  time_t now;
...
igmp_show_group_retransmission (struct pim_instance *pim, struct vty *vty)
{
  ...
  vty_out (vty, "Interface        Group           RetTimer Counter RetSrcs\n");
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->gm_group_list, grpnode, grp))
        {
          ...
          pim_inet4_dump ("<group?>", grp->group_addr, group_str,
                          sizeof (group_str));
          pim_time_timer_to_mmss (grp_retr_mmss, sizeof (grp_retr_mmss),
...
static void
igmp_sources_print (struct interface *ifp, char *group_str,
                    struct gm_source *src, time_t now, json_object *json,
      ...
      if (! json_group)
        {
          ...
          if (! json_group)
            return;
          json_object_string_add (json_group, "group", group_str);
          json_object_object_add (json_iface, group_str, json_group);
          json_sources = json_object_new_array ();
          if (! json_sources)
            return;
          json_object_object_add (json_group, "sources", json_sources);
...
static void
igmp_show_sources_interface_single (struct pim_instance *pim, struct vty *vty,
                                    bool uj, const char *ifname,
                                    const char *grp_str)
{
  struct interface *ifp;
  ...
  if (grp_str)
    {
      ...
      /* scan group sources */
      for (ALL_LIST_ELEMENTS_RO (grp->group_source_list, srcnode, src))
        igmp_sources_print (ifp, group_str, src, now, json, vty, uj);
    }
  else
    {
      struct listnode *grpnode;
...
static void
igmp_show_sources (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct interface *ifp;
  time_t now;
  json_object *json = NULL;
...
igmp_show_source_retransmission (struct pim_instance *pim, struct vty *vty)
{
  ...
  vty_out (vty, "Interface        Group           Source          Counter\n");
          ...
          for (ALL_LIST_ELEMENTS_RO (grp->group_source_list, srcnode, src))
            {
              ...
              vty_out (vty, "%-16s %-15s %-15s %7d\n", ifp->name, group_str,
                       source_str, src->source_query_retransmit_count);
...
static void
clear_igmp_interfaces (struct pim_instance *pim)
{
  struct interface *ifp;
...
static void
clear_interfaces (struct pim_instance *pim)
{
...
#define PIM_GET_PIM_INTERFACE(pim_ifp, ifp)                                   \
  ...
  if (! pim_ifp)                                                              \
    {                                                                         \
      vty_out (vty, "%% Enable PIM and/or IGMP on this interface first\n");   \
 ...
 */
static void
pim_cli_legacy_mesh_group_behavior (struct vty *vty, const char *gname)
{
  const char *vrfname;
  ...
  snprintf (xpath_value, sizeof (xpath_value),
            FRR_PIM_VRF_XPATH "/msdp-mesh-groups[name='%s']", "frr-pim:pimd",
            "pim", vrfname, "frr-routing:ipv4", gname);
  ...
  if (yang_dnode_exists (vty->candidate_config->dnode, xpath_member_value))
    {
      member_dnode =
          yang_dnode_get (vty->candidate_config->dnode, xpath_member_value);
       ...
       JSON_STR)
{
  ...
  if (ifname)
    igmp_show_groups_interface_single (vrf->info, vty, ! ! json, ifname,
                                       grp_str, ! ! detail);
  else
       ...
       JSON_STR)
{
  ...
  if (ifname)
    igmp_show_sources_interface_single (vrf->info, vty, ! ! json, ifname,
                                        grp_str);
  else
       ...
       JSON_STR)
{
  ...
  if (uj)
    {
      ...
      inet_ntop (AF_INET, &router->local_vtep_ip, addr_buf, INET_ADDRSTRLEN);
      json_object_string_add (json, "localVtepIp", addr_buf);
      inet_ntop (AF_INET, &router->anycast_vtep_ip, addr_buf, INET_ADDRSTRLEN);
      json_object_string_add (json, "anycastVtepIp", addr_buf);
      json_object_string_add (json, "peerlinkRif", router->peerlink_rif);
  ...
  vty_out (vty, "MLAG daemon connection: %s\n",
           (router->mlag_flags & PIM_MLAGF_LOCAL_CONN_UP) ? "up" : "down");
  vty_out (vty, "MLAG peer state: %s\n",
           (router->mlag_flags & PIM_MLAGF_PEER_CONN_UP) ? "up" : "down");
  vty_out (vty, "Zebra peer state: %s\n",
           (router->mlag_flags & PIM_MLAGF_PEER_ZEBRA_UP) ? "up" : "down");
...
static void
pim_show_mlag_up_entry_detail (struct vrf *vrf, struct vty *vty,
                               struct pim_upstream *up, char *src_str,
                               char *grp_str, json_object *json)
{
  if (json)
    {
      json_object *json_row = NULL;
      ...
      if (pim_up_mlag_is_local (up))
        json_object_array_add (own_list, json_object_new_string ("local"));
      if (up->flags & (PIM_UPSTREAM_FLAG_MASK_MLAG_PEER))
        json_object_array_add (own_list, json_object_new_string ("peer"));
      if (up->flags & (PIM_UPSTREAM_FLAG_MASK_MLAG_INTERFACE))
        json_object_array_add (own_list, json_object_new_string ("Interface"));
      json_object_object_add (json_row, "owners", own_list);

      json_object_int_add (json_row, "localCost", pim_up_mlag_local_cost (up));
      json_object_int_add (json_row, "peerCost", pim_up_mlag_peer_cost (up));
  ...
  else
    {
      ...
      vty_out (vty, "%-15s %-15s %-6s %-11u %-10d %2s\n", src_str, grp_str,
               own_str, pim_up_mlag_local_cost (up),
               pim_up_mlag_peer_cost (up),
               PIM_UPSTREAM_FLAG_TEST_MLAG_NON_DF (up->flags) ? "n" : "y");
    }
}

static void
pim_show_mlag_up_detail (struct vrf *vrf, struct vty *vty,
  ...
  frr_each (rb_pim_upstream, &pim->upstream_head, up)
    {
      ...
      if (group)
        {
          if (strcmp (src_str, src_or_group) || strcmp (grp_str, group))
            continue;
        }
      else
        {
          if (strcmp (src_str, src_or_group) && strcmp (grp_str, src_or_group))
            continue;
        }
      pim_show_mlag_up_entry_detail (vrf, vty, up, src_str, grp_str, json);
    }
...
static void
pim_show_mlag_up_vrf (struct vrf *vrf, struct vty *vty, bool uj)
{
  json_object *json = NULL;
  ...
  frr_each (rb_pim_upstream, &pim->upstream_head, up)
    {
      ...
      if (uj)
      ...
      else
        {
          ...
          if (up->flags & (PIM_UPSTREAM_FLAG_MASK_MLAG_INTERFACE))
            strlcat (own_str, "I", sizeof (own_str));
          vty_out (vty, "%-15pPAs %-15pPAs %-6s %-11u %-10u %2s\n",
                   &up->sg.src, &up->sg.grp, own_str,
                   pim_up_mlag_local_cost (up), pim_up_mlag_peer_cost (up),
                   PIM_UPSTREAM_FLAG_TEST_MLAG_NON_DF (up->flags) ? "n" : "y");
        }
    }
  if (uj)
...
static void
pim_show_mlag_help_string (struct vty *vty, bool uj)
{
  if (! uj)
    {
...
DEFUN (show_ip_pim_mlag_up_vrf_all,
...
DEFPY (show_ip_pim_rp_vrf_all,
       ...
       JSON_STR)
{
  return pim_show_rp_vrf_all_helper (vty, group_str, (struct prefix *) group,
                                     ! ! json);
}
  ...
  for (ALL_LIST_ELEMENTS_RO (pim->ssmpingd_list, node, ss))
    {
      ...
      if (pim_socket_getsockname (ss->sock_fd, (struct sockaddr *) &bind_addr,
                                  &len))
        {
          vty_out (
              vty,
      ...
      vty_out (vty, "%-15s %6d %-15s %5d %8s %8lld\n", source_str, ss->sock_fd,
               bind_addr_str, ntohs (bind_addr.sin_port), ss_uptime,
       ...
       "Prefix-List name\n")
{
  ...
  snprintf (reg_alist_xpath, sizeof (reg_alist_xpath), FRR_PIM_VRF_XPATH,
            "frr-pim:pimd", "pim", vrfname, "frr-routing:ipv4");
  strlcat (reg_alist_xpath, "/register-accept-list", sizeof (reg_alist_xpath));
       ...
       "Send v6 secondary addresses\n")
{
  ...
  nb_cli_enqueue_change (vty, send_v6_secondary_xpath, NB_OP_MODIFY, "false");
       ...
       "Name of a prefix-list\n")
{
  ...
  strlcat (ssm_plist_xpath, "/ssm-prefix-list", sizeof (ssm_plist_xpath));
  ssm_plist_dnode =
      yang_dnode_get (vty->candidate_config->dnode, ssm_plist_xpath);
...
static void
ip_pim_ssm_show_group_type (struct pim_instance *pim, struct vty *vty, bool uj,
                            const char *group)
{
  struct in_addr group_addr;
  const char *type_str;
  ...
  if (result <= 0)
  ...
  else
    {
      if (pim_is_group_224_4 (group_addr))
        type_str = pim_is_grp_ssm (pim, group_addr) ? "SSM" : "ASM";
      else
       ...
       "Enable PIM ECMP \n")
{
  ...
  snprintf (ecmp_xpath, sizeof (ecmp_xpath), FRR_PIM_VRF_XPATH, "frr-pim:pimd",
       ...
       IFACE_IGMP_STR)
{
  ...
  int printed = snprintf (pim_if_xpath, sizeof (pim_if_xpath),
       ...
       "Source address\n")
{
  ...
  snprintf (xpath, sizeof (xpath), FRR_GMP_JOIN_XPATH, "frr-routing:ipv4",
...
{
  nb_cli_enqueue_change (vty, "./enable", NB_OP_MODIFY, "true");
  nb_cli_enqueue_change (vty, "./igmp-version", NB_OP_MODIFY, argv[3]->arg);
       ...
       IFACE_PIM_HELLO_HOLD_STR)
{
  ...
  if (argc == idx_hold + 1)
    ...
    return pim_process_ip_pim_hello_cmd (vty, argv[idx_time]->arg, NULL);
}
       ...
       DEBUG_PIM_PIM_REG_PACKETS_STR)
{
  if (! no)
    ...
    return pim_no_debug_pim_packets_cmd (hello, joins, registers, vty);
}
       ...
       "peer ip address\n")
{
  ...
  snprintf (msdp_peer_xpath, sizeof (msdp_peer_xpath), FRR_PIM_VRF_XPATH,
            "frr-pim:pimd", "pim", vrfname, "frr-routing:ipv4");
  snprintf (temp_xpath, sizeof (temp_xpath), "/msdp-peer[peer-ip='%s']",
            argv[4]->arg);
...
static void
ip_msdp_show_mesh_group (struct vty *vty, struct pim_msdp_mg *mg,
...
static void
ip_msdp_show_peers (struct pim_instance *pim, struct vty *vty, bool uj)
{
...
static void
ip_msdp_show_peers_detail (struct pim_instance *pim, struct vty *vty,
...
static void
ip_msdp_show_sa (struct pim_instance *pim, struct vty *vty, bool uj)
{
      ...
      if (uj)
      ...
      else
        {
          vty_out (vty, "%-15pPAs  %15pPAs  %15s  %5c  %3c  %8s\n",
                   &sa->sg.src, &sa->sg.grp, rp_str, local_str[0], spt_str[0],
                   timebuf);
        }
    }
...
ip_msdp_show_sa_entry_detail (struct pim_msdp_sa *sa, const char *src_str,
                              const char *grp_str, struct vty *vty, bool uj,
                              json_object *json)
{
  char rp_str[INET_ADDRSTRLEN];
  char peer_str[INET_ADDRSTRLEN];
...
static void
ip_msdp_show_sa_detail (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct listnode *sanode;
  struct pim_msdp_sa *sa;
  json_object *json = NULL;
...
static void
ip_msdp_show_sa_addr (struct pim_instance *pim, struct vty *vty,
...
static void
ip_msdp_show_sa_sg (struct pim_instance *pim, struct vty *vty, const char *src,
                    const char *grp, bool uj)
{
  struct listnode *sanode;
  struct pim_msdp_sa *sa;
  json_object *json = NULL;
...
static void
pim_show_vxlan_sg_entry (struct pim_vxlan_sg *vxlan_sg,
  ...
  if (json)
    {
      ...
      snprintfrr (grp_str, sizeof (grp_str), "%pPAs", &vxlan_sg->sg.grp);
      snprintfrr (src_str, sizeof (src_str), "%pPAs", &vxlan_sg->sg.src);
      ...
      if (! cwd->json_group)
        {
          ...
          json_object_object_add (json, grp_str, cwd->json_group);
        }
  ...
  else
    {
      vty_out (vty, "%-15pPAs %-15pPAs %-15s %-15s %-5s\n", &vxlan_sg->sg.src,
               &vxlan_sg->sg.grp, iif_name, oif_name, installed ? "I" : "");
    }
}

static void
pim_show_vxlan_sg_hash_entry (struct hash_bucket *bucket, void *arg)
{
...
static void
pim_show_vxlan_sg (struct pim_instance *pim, struct vty *vty, bool uj)
{
  json_object *json = NULL;
  struct pim_sg_cache_walk_data cwd;
...
static void
pim_show_vxlan_sg_match_addr (struct pim_instance *pim, struct vty *vty,
                              char *addr_str, bool uj)
{
  json_object *json = NULL;
  struct pim_sg_cache_walk_data cwd;
  int result = 0;
...
static void
pim_show_vxlan_sg_one (struct pim_instance *pim, struct vty *vty,
      ...
      if (uj)
        {
          ...
          if (installed)
            ...
            json_object_boolean_false_add (json, "installed");
        }
      else
        {
          vty_out (vty, "SG : %s\n", vxlan_sg->sg_str);
...
static void
pim_show_vxlan_sg_work (struct pim_instance *pim, struct vty *vty, bool uj)
{
  json_object *json = NULL;
  struct pim_sg_cache_walk_data cwd;
  struct listnode *node;
              ...
              "MLAG\n")
{
  ...
  snprintf (mlag_xpath, sizeof (mlag_xpath), FRR_PIM_VRF_XPATH, "frr-pim:pimd",
            "pim", "default", "frr-routing:ipv4");
              ...
              "unique ip address\n")
{
  ...
  snprintf (mlag_peerlink_rif_xpath, sizeof (mlag_peerlink_rif_xpath),
            FRR_PIM_VRF_XPATH, "frr-pim:pimd", "pim", "default",
            "frr-routing:ipv4");
  ...
  snprintf (mlag_my_role_xpath, sizeof (mlag_my_role_xpath), FRR_PIM_VRF_XPATH,
            "frr-pim:pimd", "pim", "default", "frr-routing:ipv4");
  strlcat (mlag_my_role_xpath, "/mlag/my-role", sizeof (mlag_my_role_xpath));
  ...
  if (! strcmp (argv[idx]->arg, "up"))
    {
      nb_cli_enqueue_change (vty, mlag_peer_state_xpath, NB_OP_MODIFY, "true");
    }
