  ...
  frr_with_privs (&pimd_privs)
    {
      ...
      if (setsockopt (fd, IPPROTO_IP, IP_HDRINCL, &on, sizeof (on)))
        zlog_err ("%s: Could not turn on IP_HDRINCL option: %m", __func__);
#endif
    }
}
...
#if PIM_IPV == 4
static inline int
pim_setsockopt (int protocol, int fd, struct interface *ifp)
{
  int one = 1;
  int ttl = 1;
  ...
  if (setsockopt (fd, IPPROTO_IP, IP_RECVDSTADDR, &one, sizeof (one)))
    zlog_warn ("Could not set IP_RECVDSTADDR on socket fd=%d: %m", fd);
#else
  flog_err (
      EC_LIB_DEVELOPMENT,
  ...
  if (protocol == IPPROTO_IGMP)
    {
      ...
      if (setsockopt (fd, IPPROTO_IP, IP_OPTIONS, ra, 4))
        {
          zlog_warn ("Could not set Router Alert Option on socket fd=%d: %m",
                     fd);
          close (fd);
  ...
  if (setsockopt (fd, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof (ttl)))
    {
      zlog_warn ("Could not set multicast TTL=%d on socket fd=%d: %m", ttl,
                 fd);
      close (fd);
  ...
  if (setsockopt_ipv4_multicast_if (fd, PIMADDR_ANY, ifp->ifindex))
    {
      zlog_warn ("Could not set Outgoing Interface Option on socket fd=%d: %m",
                 fd);
      close (fd);
...
pim_setsockopt (int protocol, int fd, struct interface *ifp)
{
  ...
  if (setsockopt (fd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &mreq, sizeof (mreq)))
    {
      zlog_warn ("Could not set Outgoing Interface Option on socket fd=%d: %m",
                 fd);
      close (fd);
...
int
pim_socket_mcast (int protocol, pim_addr ifaddr, struct interface *ifp,
                  uint8_t loop)
{
  int fd;
  int ret;
  ...
  if (fd < 0)
    {
      zlog_warn ("Could not create multicast socket: errno=%d: %s", errno,
                 safe_strerror (errno));
      return PIM_SOCK_ERR_SOCKET;
    }
  ...
  if (ret)
    {
      close (fd);
      zlog_warn ("Could not set fd: %d for interface: %s to device", fd,
                 ifp->name);
  ...
  if (setsockopt_multicast_loop (fd, loop))
    {
      zlog_warn ("Could not %s Multicast Loopback Option on socket fd=%d: %m",
...
int
pim_socket_join (int fd, pim_addr group, pim_addr ifaddr, ifindex_t ifindex,
                 struct pim_interface *pim_ifp)
{
  int ret;

#if PIM_IPV == 4
  ret = setsockopt_ipv4_multicast (fd, IP_ADD_MEMBERSHIP, ifaddr, group.s_addr,
                                   ifindex);
#else
  ...
  if (PIM_DEBUG_TRACE)
    zlog_debug ("Socket fd=%d joined group %pPAs on interface address %pPAs",
                fd, &group, &ifaddr);
  return ret;
}
...
#if PIM_IPV == 4
static void
cmsg_getdstaddr (struct msghdr *mh, struct sockaddr_storage *dst,
                 ifindex_t *ifindex)
{
  struct cmsghdr *cmsg;
...
int
pim_socket_recvfromto (int fd, uint8_t *buf, size_t len,
...
int
pim_socket_getsockname (int fd, struct sockaddr *name, socklen_t *namelen)
{
