...
pim_msdp_update_sock_send_buffer_size (int fd)
{
  ...
  if (getsockopt (fd, SOL_SOCKET, SO_SNDBUF, &optval, &optlen) < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "getsockopt of SO_SNDBUF failed %s",
                    safe_strerror (errno));
      return;
    }
  ...
  if (optval < size)
    {
      if (setsockopt (fd, SOL_SOCKET, SO_SNDBUF, &size, sizeof (size)) < 0)
        {
          flog_err_sys (EC_LIB_SOCKET, "Couldn't increase send buffer: %s",
                        safe_strerror (errno));
        }
    }
}
...
static void
pim_msdp_sock_accept (struct event *thread)
{
  union sockunion su;
  ...
  if (mp->fd >= 0)
    {
      if (PIM_DEBUG_MSDP_EVENTS)
        {
          zlog_notice (
              "msdp peer new connection from %pSU stop old connection", &su);
        }
...
int
pim_msdp_sock_listen (struct pim_instance *pim)
{
  int sock;
  int socklen;
  ...
  if (pim->vrf->vrf_id != VRF_DEFAULT)
    {
      ...
      if (! ifp)
        {
          flog_err (EC_LIB_INTERFACE, "%s: Unable to lookup vrf interface: %s",
      ...
      if (pim_socket_bind (sock, ifp))
        {
          flog_err_sys (EC_LIB_SOCKET, "%s: Unable to bind to socket: %s",
  ...
  if (rc < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "pim_msdp_socket bind to port %d: %s",
  ...
  if (setsockopt_ipv4_tos (sock, IPTOS_PREC_INTERNETCONTROL))
    {
      zlog_warn ("can't set sockopt IP_TOS to MSDP socket %d: %s", sock,
                 safe_strerror (errno));
    }
...
pim_msdp_sock_connect (struct pim_msdp_peer *mp)
{
  ...
  if (mp->fd >= 0)
    {
      if (PIM_DEBUG_MSDP_EVENTS)
        {
          zlog_notice ("msdp duplicate connect to %s nuke old connection",
                       mp->key_str);
        }
  ...
  if (mp->fd < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "pim_msdp_socket socket failure: %s",
                    safe_strerror (errno));
      return -1;
    }
  ...
  if (mp->pim->vrf->vrf_id != VRF_DEFAULT)
    {
      struct interface *ifp =
          if_lookup_by_name (mp->pim->vrf->name, mp->pim->vrf->vrf_id);
      if (! ifp)
        {
          flog_err (EC_LIB_INTERFACE, "%s: Unable to lookup vrf interface: %s",
  ...
  if (rc < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "pim_msdp_socket connect bind failure: %s",
                    safe_strerror (errno));
      close (mp->fd);
      mp->fd = -1;
      return rc;
  ...
  if (setsockopt_ipv4_tos (mp->fd, IPTOS_PREC_INTERNETCONTROL))
    {
      zlog_warn ("can't set sockopt IP_TOS to MSDP socket %d: %s", mp->fd,
                 safe_strerror (errno));
    }

  /* Connect to the remote mp. */
  return (
      sockunion_connect (mp->fd, &mp->su_peer, htons (PIM_MSDP_TCP_PORT), 0));
}
