...
pim_global_config_write_worker (struct pim_instance *pim, struct vty *vty)
{
  ...
  if (pim->msdp.hold_time != PIM_MSDP_PEER_HOLD_TIME ||
    ...
    {
      vty_out (vty, "%sip msdp timers %u %u", spaces, pim->msdp.hold_time,
               pim->msdp.keep_alive);
...
#if PIM_IPV == 4
static int
gm_config_write (struct vty *vty, int writes, struct pim_interface *pim_ifp)
{
  /* IF ip igmp */
  ...
  if (pim_ifp->gm_last_member_query_count != GM_DEFAULT_ROBUSTNESS_VARIABLE)
    {
...
int
pim_config_write (struct vty *vty, int writes, struct interface *ifp,
                  struct pim_instance *pim)
{
  ...
  if (pim_ifp->pim_hello_period != PIM_DEFAULT_HELLO_PERIOD)
    {
      vty_out (vty, " " PIM_AF_NAME " pim hello %d",
...
int
pim_interface_config_write (struct vty *vty)
{
  struct pim_instance *pim;
