 ...
 */
const char *
pim_cli_get_vrf_name (struct vty *vty)
{
...
int
pim_process_join_prune_cmd (struct vty *vty, const char *jpi_str)
{
  char xpath[XPATH_MAXLEN];

  snprintf (xpath, sizeof (xpath), FRR_PIM_ROUTER_XPATH, FRR_PIM_AF_XPATH_VAL);
...
int
pim_process_no_join_prune_cmd (struct vty *vty)
{
  char xpath[XPATH_MAXLEN];
...
int
pim_process_spt_switchover_infinity_cmd (struct vty *vty)
{
  const char *vrfname;
  ...
  snprintf (spt_plist_xpath, sizeof (spt_plist_xpath), FRR_PIM_VRF_XPATH,
  ...
  snprintf (spt_action_xpath, sizeof (spt_action_xpath), FRR_PIM_VRF_XPATH,
            "frr-pim:pimd", "pim", vrfname, FRR_PIM_AF_XPATH_VAL);
...
int
pim_process_spt_switchover_prefixlist_cmd (struct vty *vty, const char *plist)
{
  const char *vrfname;
  char spt_plist_xpath[XPATH_MAXLEN];
  char spt_action_xpath[XPATH_MAXLEN];
...
int
pim_process_no_spt_switchover_cmd (struct vty *vty)
{
  const char *vrfname;
  char spt_plist_xpath[XPATH_MAXLEN];
  char spt_action_xpath[XPATH_MAXLEN];
...
int
pim_process_pim_packet_cmd (struct vty *vty, const char *packet)
{
  char xpath[XPATH_MAXLEN];
...
int
pim_process_no_pim_packet_cmd (struct vty *vty)
{
  char xpath[XPATH_MAXLEN];
...
int
pim_process_keepalivetimer_cmd (struct vty *vty, const char *kat)
{
  const char *vrfname;
...
int
pim_process_no_keepalivetimer_cmd (struct vty *vty)
{
  const char *vrfname;
  char ka_timer_xpath[XPATH_MAXLEN];
...
int
pim_process_rp_kat_cmd (struct vty *vty, const char *rpkat)
{
  const char *vrfname;
  ...
  snprintf (rp_ka_timer_xpath, sizeof (rp_ka_timer_xpath), FRR_PIM_VRF_XPATH,
            "frr-pim:pimd", "pim", vrfname, FRR_PIM_AF_XPATH_VAL);
...
int
pim_process_no_rp_kat_cmd (struct vty *vty)
{
  const char *vrfname;
  char rp_ka_timer[6];
  char rp_ka_timer_xpath[XPATH_MAXLEN];
  ...
  strlcat (rs_timer_xpath, "/register-suppress-time", sizeof (rs_timer_xpath));
...
int
pim_process_register_suppress_cmd (struct vty *vty, const char *rst)
{
  char xpath[XPATH_MAXLEN];
...
int
pim_process_no_register_suppress_cmd (struct vty *vty)
{
  char xpath[XPATH_MAXLEN];
...
int
pim_process_ip_pim_cmd (struct vty *vty)
{
...
int
pim_process_ip_pim_passive_cmd (struct vty *vty, bool enable)
{
  if (enable)
    nb_cli_enqueue_change (vty, "./pim-passive-enable", NB_OP_MODIFY, "true");
  else
    nb_cli_enqueue_change (vty, "./pim-passive-enable", NB_OP_MODIFY, "false");
...
int
pim_process_no_ip_pim_cmd (struct vty *vty)
{
  const struct lyd_node *mld_enable_dnode;
  char mld_if_xpath[XPATH_MAXLEN];

  int printed = snprintf (mld_if_xpath, sizeof (mld_if_xpath),
                          "%s/frr-gmp:gmp/address-family[address-family='%s']",
                          VTY_CURR_XPATH, FRR_PIM_AF_XPATH_VAL);

  if (printed >= (int) (sizeof (mld_if_xpath)))
    {
      vty_out (vty, "Xpath too long (%d > %u)", printed + 1, XPATH_MAXLEN);
      return CMD_WARNING_CONFIG_FAILED;
    }

  mld_enable_dnode =
      yang_dnode_getf (vty->candidate_config->dnode, FRR_GMP_ENABLE_XPATH,
                       VTY_CURR_XPATH, FRR_PIM_AF_XPATH_VAL);
  ...
  if (! mld_enable_dnode)
  ...
  else
    {
      if (! yang_dnode_get_bool (mld_enable_dnode, "."))
        ...
        nb_cli_enqueue_change (vty, "./pim-enable", NB_OP_MODIFY, "false");
    }
...
int
pim_process_ip_pim_drprio_cmd (struct vty *vty, const char *drpriority_str)
{
  nb_cli_enqueue_change (vty, "./dr-priority", NB_OP_MODIFY, drpriority_str);
...
int
pim_process_no_ip_pim_drprio_cmd (struct vty *vty)
{
...
int
pim_process_ip_pim_hello_cmd (struct vty *vty, const char *hello_str,
                              const char *hold_str)
{
  const struct lyd_node *mld_enable_dnode;
  ...
  if (hold_str)
    nb_cli_enqueue_change (vty, "./hello-holdtime", NB_OP_MODIFY, hold_str);
...
int
pim_process_no_ip_pim_hello_cmd (struct vty *vty)
{
...
int
pim_process_ip_pim_activeactive_cmd (struct vty *vty, const char *no)
{
  if (no)
    nb_cli_enqueue_change (vty, "./active-active", NB_OP_MODIFY, "false");
  else
    {
      nb_cli_enqueue_change (vty, "./pim-enable", NB_OP_MODIFY, "true");
...
int
pim_process_ip_pim_boundary_oil_cmd (struct vty *vty, const char *oil)
{
  nb_cli_enqueue_change (vty, "./multicast-boundary-oil", NB_OP_MODIFY, oil);
...
int
pim_process_no_ip_pim_boundary_oil_cmd (struct vty *vty)
{
...
int
pim_process_ip_mroute_cmd (struct vty *vty, const char *interface,
                           const char *group_str, const char *source_str)
{
  ...
  return nb_cli_apply_changes (vty, FRR_PIM_MROUTE_XPATH, FRR_PIM_AF_XPATH_VAL,
                               source_str, group_str);
}

int
pim_process_no_ip_mroute_cmd (struct vty *vty, const char *interface,
                              const char *group_str, const char *source_str)
{
  nb_cli_enqueue_change (vty, ".", NB_OP_DESTROY, NULL);
...
int
pim_process_rp_cmd (struct vty *vty, const char *rp_str, const char *group_str)
{
  const char *vrfname;
  ...
  snprintf (rp_group_xpath, sizeof (rp_group_xpath), FRR_PIM_STATIC_RP_XPATH,
...
int
pim_process_no_rp_cmd (struct vty *vty, const char *rp_str,
...
int
pim_process_rp_plist_cmd (struct vty *vty, const char *rp_str,
                          const char *prefix_list)
{
  ...
  snprintf (rp_plist_xpath, sizeof (rp_plist_xpath), FRR_PIM_STATIC_RP_XPATH,
            "frr-pim:pimd", "pim", vrfname, FRR_PIM_AF_XPATH_VAL, rp_str);
...
int
pim_process_no_rp_plist_cmd (struct vty *vty, const char *rp_str,
                             const char *prefix_list)
{
  char rp_xpath[XPATH_MAXLEN];
...
bool
pim_sgaddr_match (pim_sgaddr item, pim_sgaddr match)
{
  return (pim_addr_is_any (match.grp) ||
          ! pim_addr_cmp (match.grp, item.grp)) &&
         (pim_addr_is_any (match.src) || ! pim_addr_cmp (match.src, item.src));
}

void
json_object_pim_ifp_add (struct json_object *json, struct interface *ifp)
{
  ...
  json_object_string_addf (json, "address", "%pPA", &pim_ifp->primary_address);
...
void
pim_print_ifp_flags (struct vty *vty, struct interface *ifp)
{
  vty_out (vty, "Flags\n");
  vty_out (vty, "-----\n");
...
json_object_pim_upstream_add (json_object *json, struct pim_upstream *up)
{
  ...
  json_object_boolean_add (
      ...
      CHECK_FLAG (up->flags, PIM_UPSTREAM_FLAG_MASK_DR_JOIN_DESIRED_UPDATED));
  json_object_boolean_add (json, "firstHopRouter",
  ...
  json_object_boolean_add (
      json, "sendSGRptPrune",
      CHECK_FLAG (up->flags, PIM_UPSTREAM_FLAG_MASK_SEND_SG_RPT_PRUNE));
  json_object_boolean_add (
      json, "lastHopRouter",
      CHECK_FLAG (up->flags, PIM_UPSTREAM_FLAG_MASK_SRC_LHR));
  json_object_boolean_add (
      json, "disableKATExpiry",
      CHECK_FLAG (up->flags, PIM_UPSTREAM_FLAG_MASK_DISABLE_KAT_EXPIRY));
  json_object_boolean_add (
...
void
pim_show_rpf_refresh_stats (struct vty *vty, struct pim_instance *pim,
  ...
  if (json)
    {
      ...
      json_object_int_add (json, "nexthopLookups", pim->nexthop_lookups);
  ...
  else
    {
      vty_out (vty,
               ...
               (long long) pim->rpf_cache_refresh_events, refresh_uptime,
               (long long) pim->nexthop_lookups,
               (long long) pim->nexthop_lookups_avoided);
    }
}

void
pim_show_rpf (struct pim_instance *pim, struct vty *vty, json_object *json)
{
  ...
  if (! json)
    {
      ...
      ttable_add_row (
          tt, "Source|Group|RpfIface|RpfAddress|RibNextHop|Metric|Pref");
      ...
      if (json)
        {
          ...
          json_object_string_addf (json_row, "rpfAddress", "%pPA",
                                   &rpf->rpf_addr);
          json_object_string_addf (json_row, "ribNexthop", "%pPAs",
                                   &rpf->source_nexthop.mrib_nexthop_addr);
          json_object_int_add (json_row, "routeMetric",
                               rpf->source_nexthop.mrib_route_metric);
          json_object_int_add (json_row, "routePreference",
                               rpf->source_nexthop.mrib_metric_preference);
          json_object_object_add (json_group, src_str, json_row);
        }
      else
        {
          ttable_add_row (tt, "%pPAs|%pPAs|%s|%pPA|%pPAs|%d|%d", &up->sg.src,
                          &up->sg.grp, rpf_ifname, &rpf->rpf_addr,
                          &rpf->source_nexthop.mrib_nexthop_addr,
                          rpf->source_nexthop.mrib_route_metric,
                          rpf->source_nexthop.mrib_metric_preference);
        }
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->pim_neighbor_list, neighnode, neigh))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (neigh->prefix_list, prefix_node, p))
            ttable_add_row (tt, "%s|%pPAs|%pPAs|%pFX", ifp->name, &ifaddr,
                            &neigh->source_addr, p);
        }
    }
  /* Dump the generated table. */
...
void
pim_show_state (struct pim_instance *pim, struct vty *vty,
                const char *src_or_group, const char *group, json_object *json)
{
  ...
  frr_each (rb_pim_oil, &pim->channel_oil_head, c_oil)
    {
      ...
      snprintfrr (grp_str, sizeof (grp_str), "%pPAs", oil_mcastgrp (c_oil));
      snprintfrr (src_str, sizeof (src_str), "%pPAs", oil_origin (c_oil));
      ...
      if (json)
          ...
          if (! json_ifp_in)
            {
              ...
              json_object_boolean_add (json_source, "isRpt", isRpt);
        ...
        vty_out (vty, "%-6d %-15pPAs  %-15pPAs  %-3s  %-16s  ",
                 c_oil->installed, oil_origin (c_oil), oil_mcastgrp (c_oil),
                 isRpt ? "y" : "n", in_ifname);
#endif
      ...
      for (oif_vif_index = 0; oif_vif_index < MAXVIFS; ++oif_vif_index)
        {
          ...
          if (json)
          ...
          else
            {
              flag[0] = '\0';
              snprintf (
                  flag, sizeof (flag), "(%c%c%c%c%c)",
                  (c_oil->oif_flags[oif_vif_index] & PIM_OIF_FLAG_PROTO_GM)
                      ? 'I'
                      : ' ',
                  (c_oil->oif_flags[oif_vif_index] & PIM_OIF_FLAG_PROTO_PIM)
                      ? 'J'
                      : ' ',
                  (c_oil->oif_flags[oif_vif_index] & PIM_OIF_FLAG_PROTO_VXLAN)
                      ? 'V'
                      : ' ',
                  (c_oil->oif_flags[oif_vif_index] & PIM_OIF_FLAG_PROTO_STAR)
                      ? '*'
                      : ' ',
                  (c_oil->oif_flags[oif_vif_index] & PIM_OIF_FLAG_MUTE) ? 'M'
                                                                        : ' ');
              ...
              if (first_oif)
              ...
              else
                {
                  ...
                  vty_out (vty, ", %s%s", out_ifname, flag);
#else
                  ttable_add_row (tt, "%c|%c|%c|%c|%c|%s%s", ' ', ' ', ' ',
                                  ' ', ' ', out_ifname, flag);
#endif
                }
            }
 ...
 */
void
pim_show_statistics (struct pim_instance *pim, struct vty *vty,
      ...
      if (! uj)
      ...
      else
        {
          ...
          json_object_int_add (json_row, "bsmDroppedConfig",
                               pim_ifp->pim_ifstat_bsm_cfg_miss);
          json_object_int_add (json_row, "bsmDroppedUnicast",
                               pim_ifp->pim_ifstat_ucast_bsm_cfg_miss);
          json_object_int_add (json_row, "bsmDroppedInvalidScopeZone",
                               pim_ifp->pim_ifstat_bsm_invalid_sz);
...
void
pim_show_upstream (struct pim_instance *pim, struct vty *vty, pim_sgaddr *sg,
                   json_object *json)
{
  struct pim_upstream *up;
  struct ttable *tt = NULL;
      ...
      if (json)
        {
          ...
          if (up->flags & PIM_UPSTREAM_FLAG_MASK_FHR ||
            ...
            {
              json_object_string_add (json_row, "rpfAddress", src_str);
            }

          json_object_string_add (json_row, "source", src_str);
          json_object_string_add (json_row, "group", grp_str);
          json_object_string_add (json_row, "state", state_str);
          json_object_string_add (json_row, "joinState",
                                  pim_upstream_state2str (up->join_state));
          json_object_string_add (json_row, "regState",
        ...
        {
          ttable_add_row (
              tt, "%s|%pPAs|%pPAs|%s|%s|%s|%s|%s|%d",
              up->rpf.source_nexthop.interface ? up->rpf.source_nexthop
                                                     .interface->name
                                               : "Unknown",
              &up->sg.src, &up->sg.grp, state_str, uptime, join_timer,
...
static void
pim_show_join_desired_helper (struct pim_instance *pim, struct vty *vty,
                              struct pim_upstream *up, json_object *json,
...
void
pim_show_join_desired (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct pim_upstream *up;
  struct ttable *tt = NULL;
  char *table = NULL;
...
void
pim_show_upstream_rpf (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct pim_upstream *up;
  struct ttable *tt = NULL;
  char *table = NULL;
  ...
  if (uj)
  ...
  else
    {
      /* Prepare table. */
      tt = ttable_new (&ttable_styles[TTSTYLE_BLANK]);
      ttable_add_row (tt, "Source|Group|RpfIface|RibNextHop|RpfAddress");
      tt->style.cell.rpad = 2;
      tt->style.corner = '+';
      ttable_restyle (tt);
    }
  ...
  frr_each (rb_pim_upstream, &pim->upstream_head, up)
    {
      ...
      if (uj)
        ...
        {
          ttable_add_row (tt, "%pPAs|%pPAs|%s|%pPA|%pPA", &up->sg.src,
                          &up->sg.grp, rpf_ifname,
                          &rpf->source_nexthop.mrib_nexthop_addr,
                          &rpf->rpf_addr);
        }
    }
...
static void
pim_show_join_helper (struct pim_interface *pim_ifp, struct pim_ifchannel *ch,
                      json_object *json, time_t now, struct ttable *tt)
{
  ...
  if (json)
      ...
      if (! json_grp)
        {
          ...
          json_object_object_addf (json_grp, json_row, "%pPAs", &ch->sg.src);
          json_object_object_addf (json_iface, json_grp, "%pPAs", &ch->sg.grp);
        }
      else
        json_object_object_addf (json_grp, json_row, "%pPAs", &ch->sg.src);
    }
    ...
    {
      ttable_add_row (tt, "%s|%pPAs|%pPAs|%pPAs|%s|%s|%s|%s",
...
int
pim_show_join_cmd_helper (const char *vrf, struct vty *vty, pim_addr s_or_g,
...
int
pim_show_join_vrf_all_cmd_helper (struct vty *vty, const char *json)
{
  pim_sgaddr sg = { 0 };
  ...
  RB_FOREACH (vrf_struct, vrf_name_head, &vrfs_by_name)
    {
      ...
      if (json)
        json_object_object_add (json_parent, vrf_struct->name, json_vrf);
    }
  if (json)
    vty_json (vty, json_parent);
...
void
pim_show_join (struct pim_instance *pim, struct vty *vty, pim_sgaddr *sg,
               json_object *json)
{
  struct pim_interface *pim_ifp;
  ...
  if (! json)
    {
      ...
      tt = ttable_new (&ttable_styles[TTSTYLE_BLANK]);
      ttable_add_row (
          tt, "Interface|Address|Source|Group|State|Uptime|Expire|Prune");
      tt->style.cell.rpad = 2;
      tt->style.corner = '+';
      ttable_restyle (tt);
    }
...
static void
pim_show_jp_agg_helper (struct interface *ifp, struct pim_neighbor *neigh,
...
int
pim_show_jp_agg_list_cmd_helper (const char *vrf, struct vty *vty)
{
  struct vrf *v;
  struct pim_instance *pim;
...
void
pim_show_jp_agg_list (struct pim_instance *pim, struct vty *vty)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  FOR_ALL_INTERFACES (pim->vrf, ifp)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->pim_neighbor_list, n_node, neigh))
        {
          for (ALL_LIST_ELEMENTS_RO (neigh->upstream_jp_agg, jag_node, jag))
            {
              for (ALL_LIST_ELEMENTS_RO (jag->sources, js_node, js))
                {
                  pim_show_jp_agg_helper (ifp, neigh, js->up, js->is_join, tt);
                }
            }
        }
    }
...
int
pim_show_membership_cmd_helper (const char *vrf, struct vty *vty, bool uj)
{
  struct vrf *v;
...
static void
pim_show_membership_helper (struct vty *vty, struct pim_interface *pim_ifp,
                            struct pim_ifchannel *ch, struct json_object *json)
{
  json_object *json_iface = NULL;
  json_object *json_row = NULL;
...
void
pim_show_membership (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
  struct interface *ifp;
      ...
      json_object_object_foreach (json, key, val)
      {
        ...
        json_object_object_foreach (val, if_field_key, if_field_val)
        {
          ...
          if (type == json_type_object)
            {
              const char *address, *source, *localMembership;
              ...
              source = json_object_get_string (json_tmp);

              json_object_object_get_ex (if_field_val, "localMembership",
                                         &json_tmp);
              localMembership = json_object_get_string (json_tmp);

              ttable_add_row (tt, "%s|%s|%s|%s|%s", key, address, source,
                              if_field_key, localMembership);
            }
        }
      }
...
static void
pim_show_channel_helper (struct pim_instance *pim,
                         struct pim_interface *pim_ifp,
                         struct pim_ifchannel *ch, json_object *json, bool uj,
                         struct ttable *tt)
{
  ...
  if (uj)
    {
      ...
      json_object_string_addf (json_row, "source", "%pPAs", &up->sg.src);
      json_object_string_addf (json_row, "group", "%pPAs", &up->sg.grp);
      ...
      json_object_object_addf (json_group, json_row, "%pPAs", &up->sg.src);
    }
  else
    {
      ttable_add_row (
          tt, "%s|%pPAs|%pPAs|%s|%s|%s|%s|%s", ch->interface->name,
          &up->sg.src, &up->sg.grp,
...
void
pim_show_channel (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct pim_interface *pim_ifp;
  struct pim_ifchannel *ch;
  struct interface *ifp;
...
int
pim_show_channel_cmd_helper (const char *vrf, struct vty *vty, bool uj)
{
  struct vrf *v;
...
int
pim_show_interface_cmd_helper (const char *vrf, struct vty *vty, bool uj,
                               bool mlag, const char *interface)
{
  ...
  if (interface)
    pim_show_interfaces_single (v->info, vty, interface, mlag, json_parent);
  else
...
int
pim_show_interface_vrf_all_cmd_helper (struct vty *vty, bool uj, bool mlag,
  ...
  RB_FOREACH (v, vrf_name_head, &vrfs_by_name)
    {
      ...
      if (interface)
        pim_show_interfaces_single (v->info, vty, interface, mlag, json_vrf);
      else
...
void
pim_show_interfaces (struct pim_instance *pim, struct vty *vty, bool mlag,
                     json_object *json)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  if (! uj)
    {
      ...
      tt = ttable_new (&ttable_styles[TTSTYLE_BLANK]);
      ttable_add_row (
          tt, "Interface|State|Address|PIM Nbrs|PIM DR|FHR|IfChannels");
      tt->style.cell.rpad = 2;
      tt->style.corner = '+';
      ttable_restyle (tt);
      ...
      json_object_object_foreach (json, key, val)
      {
        ...
        if (json_object_object_get_ex (val, "pimDesignatedRouterLocal",
                                       &json_tmp))
          {
            pimdr = "local";
          }
        ...
        ttable_add_row (tt, "%s|%s|%s|%d|%s|%d|%d", key, state, address,
                        neighbors, pimdr, firsthpr, pimifchnl);
      }
      json_object_free (json);
...
void
pim_show_interfaces_single (struct pim_instance *pim, struct vty *vty,
                            const char *ifname, bool mlag, json_object *json)
{
  pim_addr ifaddr;
  struct interface *ifp;
  struct listnode *neighnode;
  ...
  FOR_ALL_INTERFACES (pim->vrf, ifp)
    {
      ...
      found_ifname = 1;
      ifaddr = pim_ifp->primary_address;
      snprintfrr (dr_str, sizeof (dr_str), "%pPAs", &pim_ifp->pim_dr_addr);
      ...
      if (json)
        {
          ...
          if (! pim_addr_is_any (pim_ifp->update_source))
            {
              json_object_string_addf (json_row, "useSource", "%pPAs",
          ...
          if (pim_ifp->sec_addr_list)
            {
              ...
              for (ALL_LIST_ELEMENTS_RO (pim_ifp->sec_addr_list, sec_node,
                                         sec_addr))
                {
                  json_object_array_add (
                      sec_list,
                      json_object_new_stringf ("%pFXh", &sec_addr->addr));
                }
              json_object_object_add (json_row, "secondaryAddressList",
                                      sec_list);
            }

          if (pim_ifp->pim_passive_enable)
            json_object_boolean_true_add (json_row, "passive");
          ...
          if (pim_ifp->pim_neighbor_list->count)
            {
              ...
              for (ALL_LIST_ELEMENTS_RO (pim_ifp->pim_neighbor_list, neighnode,
                                         neigh))
                {
                  ...
                  snprintfrr (neigh_src_str, sizeof (neigh_src_str), "%pPAs",
                              &neigh->source_addr);
                  ...
                  json_object_string_add (json_pim_neighbor, "address",
                                          neigh_src_str);
                  json_object_string_add (json_pim_neighbor, "upTime", uptime);
                  json_object_string_add (json_pim_neighbor, "holdtime",
                                          expire);

                  json_object_object_add (json_pim_neighbors, neigh_src_str,
                                          json_pim_neighbor);
                }
          ...
          frr_each (rb_pim_upstream, &pim->upstream_head, up)
            {
               ...
               * If not create it.
               */
              json_object_object_get_ex (json_fhr_sources, grp_str,
                                         &json_group);

              if (! json_group)
                {
                  json_group = json_object_new_object ();
                  json_object_object_add (json_fhr_sources, grp_str,
                                          json_group);
                }
          ...
          if (json_fhr_sources)
            {
              json_object_object_add (json_row, "firstHopRouter",
                                      json_fhr_sources);
            }
          ...
          json_object_int_add (json_row, "effectivePropagationDelay",
                               pim_if_effective_propagation_delay_msec (ifp));
          json_object_int_add (json_row, "effectiveOverrideInterval",
                               pim_if_effective_override_interval_msec (ifp));
          json_object_int_add (json_row, "joinPruneOverrideInterval",
                               pim_if_jp_override_interval_msec (ifp));

          json_object_int_add (json_row, "propagationDelay",
                               pim_ifp->pim_propagation_delay_msec);
          json_object_int_add (
              json_row, "propagationDelayHighest",
              pim_ifp->pim_neighbors_highest_propagation_delay_msec);
          json_object_int_add (json_row, "overrideInterval",
                               pim_ifp->pim_override_interval_msec);
          json_object_int_add (
              json_row, "overrideIntervalHighest",
              pim_ifp->pim_neighbors_highest_override_interval_msec);
          if (pim_ifp->bsm_enable)
            json_object_boolean_true_add (json_row, "bsmEnabled");
          if (pim_ifp->ucast_bsm_accept)
            json_object_boolean_true_add (json_row, "ucastBsmEnabled");
          json_object_object_add (json, ifp->name, json_row);
        }
      else
        {
          ...
          if (pim_ifp->pim_passive_enable)
            vty_out (vty, "Passive    : %s\n",
                     (pim_ifp->pim_passive_enable) ? "yes" : "no");
          ...
          frr_each (rb_pim_upstream, &pim->upstream_head, up)
            {
              ...
              if (strcmp (ifp->name, up->rpf.source_nexthop.interface->name) !=
                  0)
                continue;
              ...
              pim_time_uptime (uptime, sizeof (uptime),
                               now - up->state_transition);
              vty_out (vty, "%pPAs : %pPAs is a source, uptime is %s\n",
...
void
ip_pim_ssm_show_group_range (struct pim_instance *pim, struct vty *vty,
                             bool uj)
{
...
static int
pim_print_vty_pnc_cache_walkcb (struct hash_bucket *bucket, void *arg)
{
...
static int
pim_print_json_pnc_cache_walkcb (struct hash_bucket *backet, void *arg)
{
  ...
  for (nh_node = pnc->nexthop; nh_node; nh_node = nh_node->next)
    {
      first_ifindex = nh_node->ifindex;
      ifp = if_lookup_by_index (first_ifindex, pim->vrf->vrf_id);
      snprintfrr (addr_str, sizeof (addr_str), "%pPA", &pnc->rpf.rpf_addr);
...
int
pim_show_nexthop_lookup_cmd_helper (const char *vrf, struct vty *vty,
...
int
pim_show_nexthop_cmd_helper (const char *vrf, struct vty *vty, bool uj)
{
  struct vrf *v;
...
void
pim_show_nexthop (struct pim_instance *pim, struct vty *vty, bool uj)
{
  ...
  if (uj)
    {
      hash_walk (pim->rpf_hash, pim_print_json_pnc_cache_walkcb, &jcwd);
...
int
pim_show_neighbors_cmd_helper (const char *vrf, struct vty *vty,
...
int
pim_show_neighbors_vrf_all_cmd_helper (struct vty *vty, const char *json,
                                       const char *interface)
{
  struct vrf *v;
  json_object *json_parent = NULL;
  ...
  RB_FOREACH (v, vrf_name_head, &vrfs_by_name)
    {
      ...
      if (interface)
        pim_show_neighbors_single (v->info, vty, interface, json_vrf);
      else
...
void
pim_show_neighbors_single (struct pim_instance *pim, struct vty *vty,
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->pim_neighbor_list, neighnode, neigh))
        {
           ...
           */
          if (strcmp (neighbor, "detail") && strcmp (neighbor, ifp->name) &&
          ...
          if (json)
          ...
          else
            {
              ...
              vty_out (vty, "    Hello Option - T-bit           : %s\n",
                       option_t_bit ? "yes" : "no");
              bfd_sess_show (vty, json_ifp, neigh->bfd_session);
              vty_out (vty, "\n");
            }
        }
    }
...
void
pim_show_neighbors (struct pim_instance *pim, struct vty *vty,
                    json_object *json)
{
  struct listnode *neighnode;
  struct interface *ifp;
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->pim_neighbor_list, neighnode, neigh))
        {
          ...
          if (json)
            ...
            {
              ttable_add_row (tt, "%s|%pPAs|%s|%s|%d", ifp->name,
                              &neigh->source_addr, uptime, expire,
                              neigh->dr_priority);
            }
        }
...
int
gm_process_query_max_response_time_cmd (struct vty *vty, const char *qmrt_str)
{
...
int
gm_process_no_query_max_response_time_cmd (struct vty *vty)
{
...
int
gm_process_last_member_query_count_cmd (struct vty *vty, const char *lmqc_str)
{
  const struct lyd_node *pim_enable_dnode;
  ...
  nb_cli_enqueue_change (vty, "./robustness-variable", NB_OP_MODIFY, lmqc_str);
  return nb_cli_apply_changes (vty, FRR_GMP_INTERFACE_XPATH,
                               FRR_PIM_AF_XPATH_VAL);
}

int
gm_process_no_last_member_query_count_cmd (struct vty *vty)
{
...
int
gm_process_last_member_query_interval_cmd (struct vty *vty,
                                           const char *lmqi_str)
{
  const struct lyd_node *pim_enable_dnode;
...
int
gm_process_no_last_member_query_interval_cmd (struct vty *vty)
{
  nb_cli_enqueue_change (vty, "./last-member-query-interval", NB_OP_DESTROY,
                         NULL);
  return nb_cli_apply_changes (vty, FRR_GMP_INTERFACE_XPATH,
                               FRR_PIM_AF_XPATH_VAL);
}
...
int
pim_process_ssmpingd_cmd (struct vty *vty, enum nb_operation operation,
                          const char *src_str)
{
  const char *vrfname;
  ...
  snprintf (ssmpingd_ip_xpath, sizeof (ssmpingd_ip_xpath), FRR_PIM_VRF_XPATH,
            "frr-pim:pimd", "pim", vrfname, FRR_PIM_AF_XPATH_VAL);
...
int
pim_process_bsm_cmd (struct vty *vty)
{
...
int
pim_process_no_bsm_cmd (struct vty *vty)
{
...
int
pim_process_unicast_bsm_cmd (struct vty *vty)
{
  const struct lyd_node *gm_enable_dnode;
...
int
pim_process_no_unicast_bsm_cmd (struct vty *vty)
{
  ...
  if (! json)
    {
      ...
      tt = ttable_new (&ttable_styles[TTSTYLE_BLANK]);
      ttable_add_row (
          tt, "Interface|Address|ifi|Vif|PktsIn|PktsOut|BytesIn|BytesOut");
      tt->style.cell.rpad = 2;
      tt->style.corner = '+';
      ttable_restyle (tt);
    }
  ...
  FOR_ALL_INTERFACES (pim->vrf, ifp)
    {
      ...
      if (json)
        ...
        {
          ttable_add_row (
              tt, "%s|%pPAs|%d|%d|%lu|%lu|%lu|%lu", ifp->name,
...
void
pim_cmd_show_ip_multicast_helper (struct pim_instance *pim, struct vty *vty)
{
  ...
  pim_time_uptime (uptime, sizeof (uptime), now - pim->mroute_socket_creation);
...
void
show_mroute (struct pim_instance *pim, struct vty *vty, pim_sgaddr *sg,
  ...
  if (! json)
    {
      ...
      /* Prepare table. */
      tt = ttable_new (&ttable_styles[TTSTYLE_BLANK]);
      ttable_add_row (tt, "Source|Group|Flags|Proto|Input|Output|TTL|Uptime");
      tt->style.cell.rpad = 2;
      tt->style.corner = '+';
      ttable_restyle (tt);
    }
          ...
          if (json)
            {
              ...
              if (c_oil->oif_flags[oif_vif_index] & PIM_OIF_FLAG_PROTO_GM)
#if PIM_IPV == 4
                json_object_boolean_true_add (json_ifp_out, "protocolIgmp");
#else
                json_object_boolean_true_add (json_ifp_out, "protocolMld");
#endif

              if (c_oil->oif_flags[oif_vif_index] & PIM_OIF_FLAG_PROTO_VXLAN)
              ...
              json_object_string_add (json_ifp_out, "inboundInterface",
                                      in_ifname);
              json_object_int_add (json_ifp_out, "iVifI", *oil_parent (c_oil));
              json_object_string_add (json_ifp_out, "outboundInterface",
                                      out_ifname);
              json_object_int_add (json_ifp_out, "oVifI", oif_vif_index);
              json_object_int_add (json_ifp_out, "ttl", ttl);
              json_object_string_add (json_ifp_out, "upTime", mroute_uptime);
          ...
          else
            {
              ...
              ttable_add_row (tt, "%s|%s|%s|%s|%s|%s|%d|%s", src_str, grp_str,
                              state_str, proto, in_ifname, out_ifname, ttl,
                              mroute_uptime);
      ...
      if (! json && ! found_oif)
        {
          ttable_add_row (tt, "%pPAs|%pPAs|%s|%s|%s|%s|%d|%s",
                          oil_origin (c_oil), oil_mcastgrp (c_oil), state_str,
                          "none", in_ifname, "none", 0, "--:--:--");
        }
    }
      ...
      for (oif_vif_index = 0; oif_vif_index < MAXVIFS; ++oif_vif_index)
        {
          ...
          ifp_out = pim_if_find_by_vif_index (pim, oif_vif_index);
          pim_time_uptime (oif_uptime, sizeof (oif_uptime),
                           now - s_route->c_oil.oif_creation[oif_vif_index]);
          found_oif = 1;
          ...
          if (json)
          ...
          else
            {
              ttable_add_row (tt, "%pPAs|%pPAs|%s|%s|%s|%s|%d|%s",
                              ...
                              in_ifname, out_ifname, ttl, oif_uptime);
              if (first && ! fill)
                {
                  src_str[0] = '\0';
                  grp_str[0] = '\0';
...
static void
show_mroute_count_per_channel_oil (struct channel_oil *c_oil,
                                   json_object *json, struct ttable *tt)
{
  json_object *json_group = NULL;
  json_object *json_source = NULL;
  ...
  if (json)
    {
      ...
      json_object_int_add (json_source, "lastUsed", c_oil->cc.lastused / 100);
    ...
    {
      ttable_add_row (tt, "%pPAs|%pPAs|%llu|%ld|%ld|%ld", oil_origin (c_oil),
                      oil_mcastgrp (c_oil), c_oil->cc.lastused / 100,
  ...
  if (! json)
    {
      ...
      /* Prepare table. */
      tt = ttable_new (&ttable_styles[TTSTYLE_BLANK]);
      ttable_add_row (tt, "Source|Group|LastUsed|Packets|Bytes|WrongIf");
      tt->style.cell.rpad = 2;
      tt->style.corner = '+';
      ttable_restyle (tt);
    }
...
void
show_mroute_summary (struct pim_instance *pim, struct vty *vty,
                     json_object *json)
{
  struct listnode *node;
  struct channel_oil *c_oil;
  ...
  if (! json)
    {
      ...
      vty_out (vty, "%-20s %u/%u\n", "Total",
               ...
               (starg_sw_mroute_cnt + starg_hw_mroute_cnt + sg_sw_mroute_cnt +
                sg_hw_mroute_cnt));
    }
  else
    {
      ...
      json_object_int_add (json_starg, "installed", starg_hw_mroute_cnt);
...
int
clear_ip_mroute_count_command (struct vty *vty, const char *name)
{
  struct listnode *node;
  struct channel_oil *c_oil;
  struct static_route *sr;
...
struct vrf *
pim_cmd_lookup (struct vty *vty, const char *name)
{
  struct vrf *vrf;
...
void
clear_mroute (struct pim_instance *pim)
{
  struct pim_upstream *up;
  struct interface *ifp;
...
void
clear_pim_statistics (struct pim_instance *pim)
{
  struct interface *ifp;
...
int
clear_pim_interface_traffic (const char *vrf, struct vty *vty)
{
  struct interface *ifp = NULL;
...
int
pim_debug_pim_packets_cmd (const char *hello, const char *joins,
...
int
pim_no_debug_pim_packets_cmd (const char *hello, const char *joins,
                              const char *registers, struct vty *vty)
{
  if (hello)
    {
...
int
pim_show_rpf_helper (const char *vrf, struct vty *vty, bool json)
{
  struct pim_instance *pim;
  struct vrf *v;
  json_object *json_parent = NULL;
...
int
pim_show_rpf_vrf_all_helper (struct vty *vty, bool json)
{
  struct vrf *vrf;
  json_object *json_parent = NULL;
  json_object *json_vrf = NULL;
...
int
pim_show_rp_helper (const char *vrf, struct vty *vty, const char *group_str,
...
int
pim_show_rp_vrf_all_helper (struct vty *vty, const char *group_str,
                            const struct prefix *group, bool json)
{
  struct vrf *vrf;
  json_object *json_parent = NULL;
...
int
pim_show_secondary_helper (const char *vrf, struct vty *vty)
{
  struct pim_instance *pim;
  struct vrf *v;
...
int
pim_show_statistics_helper (const char *vrf, struct vty *vty, const char *word,
                            bool uj)
{
  struct pim_instance *pim;
  struct vrf *v;
...
int
pim_show_upstream_helper (const char *vrf, struct vty *vty, pim_addr s_or_g,
...
int
pim_show_upstream_vrf_all_helper (struct vty *vty, bool json)
{
  pim_sgaddr sg = { 0 };
  struct vrf *vrf;
  json_object *json_parent = NULL;
...
int
pim_show_upstream_join_desired_helper (const char *vrf, struct vty *vty,
                                       bool uj)
{
  struct pim_instance *pim;
  struct vrf *v;
...
int
pim_show_upstream_rpf_helper (const char *vrf, struct vty *vty, bool uj)
{
  struct pim_instance *pim;
  struct vrf *v;
...
int
pim_show_state_helper (const char *vrf, struct vty *vty,
...
int
pim_show_state_vrf_all_helper (struct vty *vty, const char *s_or_g_str,
...
int
pim_show_multicast_helper (const char *vrf, struct vty *vty)
{
  struct vrf *v;
  struct pim_instance *pim;
...
int
pim_show_multicast_vrf_all_helper (struct vty *vty)
{
  struct vrf *vrf;
...
int
pim_show_multicast_count_helper (const char *vrf, struct vty *vty, bool json)
{
  struct pim_instance *pim;
  struct vrf *v;
  json_object *json_parent = NULL;
...
int
pim_show_multicast_count_vrf_all_helper (struct vty *vty, bool json)
{
  struct vrf *vrf;
  json_object *json_parent = NULL;
  json_object *json_vrf = NULL;
...
int
pim_show_mroute_helper (const char *vrf, struct vty *vty, pim_addr s_or_g,
...
int
pim_show_mroute_vrf_all_helper (struct vty *vty, bool fill, bool json)
{
  pim_sgaddr sg = { 0 };
  struct vrf *vrf;
  json_object *json_parent = NULL;
...
int
pim_show_mroute_count_helper (const char *vrf, struct vty *vty, bool json)
{
  struct pim_instance *pim;
  struct vrf *v;
  json_object *json_parent = NULL;
...
int
pim_show_mroute_count_vrf_all_helper (struct vty *vty, bool json)
{
  struct vrf *vrf;
  json_object *json_parent = NULL;
  json_object *json_vrf = NULL;
...
int
pim_show_mroute_summary_helper (const char *vrf, struct vty *vty, bool json)
{
  struct pim_instance *pim;
  struct vrf *v;
  json_object *json_parent = NULL;
...
int
pim_show_mroute_summary_vrf_all_helper (struct vty *vty, bool json)
{
  struct vrf *vrf;
  json_object *json_parent = NULL;
  json_object *json_vrf = NULL;
...
void
pim_show_interface_traffic (struct pim_instance *pim, struct vty *vty, bool uj)
{
  struct interface *ifp = NULL;
  struct pim_interface *pim_ifp = NULL;
  json_object *json = NULL;
      ...
      if (uj)
        {
          ...
          json_object_int_add (json_row, "bsmRx", pim_ifp->pim_ifstat_bsm_rx);
          json_object_int_add (json_row, "bsmTx", pim_ifp->pim_ifstat_bsm_tx);
          json_object_object_add (json, ifp->name, json_row);
        }
      else
        {
          vty_out (
              ...
              pim_ifp->pim_ifstat_reg_send, pim_ifp->pim_ifstat_reg_stop_recv,
              pim_ifp->pim_ifstat_reg_stop_send,
              pim_ifp->pim_ifstat_assert_recv, pim_ifp->pim_ifstat_assert_send,
              pim_ifp->pim_ifstat_bsm_rx, pim_ifp->pim_ifstat_bsm_tx);
        }
    }
  if (uj)
    vty_json (vty, json);
...
void
pim_show_interface_traffic_single (struct pim_instance *pim, struct vty *vty,
                                   const char *ifname, bool uj)
{
  struct interface *ifp = NULL;
  struct pim_interface *pim_ifp = NULL;
...
int
pim_show_interface_traffic_helper (const char *vrf, const char *if_name,
...
void
clear_pim_interfaces (struct pim_instance *pim)
{
  struct interface *ifp;
...
void
pim_show_bsr (struct pim_instance *pim, struct vty *vty, bool uj)
{
  char uptime[10];
...
int
pim_show_bsr_helper (const char *vrf, struct vty *vty, bool uj)
{
  struct pim_instance *pim;
  struct vrf *v;
...
static void
pim_show_group_rp_mappings_info (struct pim_instance *pim, struct vty *vty,
                                 bool uj)
{
  ...
  for (rn = route_top (pim->global_scope.bsrp_table); rn; rn = route_next (rn))
    {
      ...
      frr_each (bsm_rpinfos, bsgrp->bsrp_list, bsm_rp)
        {
          if (uj)
            {
              json_row = json_object_new_object ();
              json_object_string_addf (json_row, "Rp Address", "%pPA",
                                       &bsm_rp->rp_address);
              json_object_int_add (json_row, "Rp HoldTime",
                                   bsm_rp->rp_holdtime);
              json_object_int_add (json_row, "Rp Priority", bsm_rp->rp_prio);
              json_object_int_add (json_row, "Hash Val", bsm_rp->hash);
              json_object_object_addf (json_group, json_row, "%pPA",
                                       &bsm_rp->rp_address);
            }
          else
            {
              ttable_add_row (tt, "%pPA|%u|%u|%u", &bsm_rp->rp_address,
                              bsm_rp->rp_prio, bsm_rp->rp_holdtime,
                              bsm_rp->hash);
            }
        }
      /* Dump the generated table. */
...
int
pim_show_group_rp_mappings_info_helper (const char *vrf, struct vty *vty,
                                        bool uj)
{
  struct pim_instance *pim;
  struct vrf *v;
...
static void
pim_show_bsm_db (struct pim_instance *pim, struct vty *vty, bool uj)
{
  int count = 0;
  int fragment = 1;
  ...
  frr_each (bsm_frags, pim->global_scope.bsm_frags, bsfrag)
    {
      ...
      if (uj)
        {
          json_object_string_addf (json, "BSR address", "%pPA", &bsr_addr);
          json_object_int_add (json, "BSR priority", hdr->bsr_prio);
          json_object_int_add (json, "Hashmask Length", hdr->hm_len);
          json_object_int_add (json, "Fragment Tag", ntohs (hdr->frag_tag));
        }
      else
        {
          ...
          vty_out (vty, "%-15pPA %-15d %-15d %-15d\n", &bsr_addr,
                   hdr->bsr_prio, hdr->hm_len, ntohs (hdr->frag_tag));
        }
      ...
      while (offset < len)
        {
          ...
          if (group->group.family == PIM_MSG_ADDRESS_FAMILY_IPV4)
            grp.family = AF_INET;
          else if (group->group.family == PIM_MSG_ADDRESS_FAMILY_IPV6)
            grp.family = AF_INET6;
          ...
          if (uj)
            {
              ...
              if (! json_group)
                {
                  json_group = json_object_new_object ();
                  json_object_int_add (json_group, "Rp Count",
                                       group->rp_count);
                  json_object_int_add (json_group, "Fragment Rp count",
                                       group->frag_rp_count);
                  json_object_object_add (json, grp_str, json_group);
                }
            }
          ...
          if (! uj)
            vty_out (vty, "RpAddress     HoldTime     Priority\n");
          ...
          while (frag_rp_cnt--)
            {
              ...
              /* unaligned, again */
              memcpy (&rp_addr, &bsm_rpinfo->rpaddr.addr, sizeof (rp_addr));
...
int
pim_show_bsm_db_helper (const char *vrf, struct vty *vty, bool uj)
{
  struct pim_instance *pim;
  struct vrf *v;
