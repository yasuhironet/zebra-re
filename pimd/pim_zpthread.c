...
static void
pim_mlag_zebra_fill_header (enum mlag_msg_type msg_type)
{
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug (":%s: msg_type: %d/%d len %d", __func__, msg_type,
                fill_msg_type, data_len);
}

static void
pim_mlag_zebra_flush_buffer (void)
{
  uint32_t msg_type;
  ...
  if (mlag_bulk_cnt > 1)
    {
      ...
      if (msg_type == MLAG_MROUTE_ADD_BULK)
        {
          stream_putw_at (router->mlag_stream, 4,
      ...
      else
        {
          flog_err (EC_LIB_ZAPI_ENCODE,
                    "unknown bulk message type %d bulk_count %d", msg_type,
                    mlag_bulk_cnt);
          stream_reset (router->mlag_stream);
          mlag_bulk_cnt = 0;
          return;
...
static void
pim_mlag_zebra_check_for_buffer_flush (uint32_t curr_msg_type,
                                       uint32_t prev_msg_type)
{
  ...
  if (curr_msg_type == prev_msg_type &&
      (curr_msg_type == MLAG_MROUTE_ADD || curr_msg_type == MLAG_MROUTE_DEL))
    {
      mlag_bulk_cnt++;
      return;
    }
 ...
 */
static void
pim_mlag_zthread_handler (struct event *event)
{
  ...
  if (PIM_DEBUG_MLAG)
    zlog_debug (":%s: Processing MLAG write, %d messages in queue", __func__,
                wr_count);
  ...
  for (wr_count = 0; wr_count < PIM_MLAG_POST_LIMIT; wr_count++)
    {
      ...
      pim_mlag_zebra_check_for_buffer_flush (curr_msg_type, prev_msg_type);
...
int
pim_mlag_signal_zpthread (void)
{
  if (router->master)
    {
      if (PIM_DEBUG_MLAG)
        zlog_debug (":%s: Scheduling PIM MLAG write Thread", __func__);
      event_add_event (router->master, pim_mlag_zthread_handler, NULL, 0,
                       &router->zpthread_mlag_write);
    }
  return (0);
}
