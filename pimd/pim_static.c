...
void
pim_static_route_free (struct static_route *s_route)
{
...
static struct static_route *
static_route_new (ifindex_t iif, ifindex_t oif, pim_addr group,
                  pim_addr source)
{
      ...
      if (sroute->iif == pim_ifp->mroute_vif_index)
        {
          ...
          for (i = 0; i < MAXVIFS; i++)
            if (sroute->oif_ttls[i])
              {
                struct interface *oifp = pim_if_find_by_vif_index (pim, i);
                if (pim_addr_is_any (sroute->source))
                  ...
                  vty_out (vty, " " PIM_AF_NAME " mroute %s %pPA %pPA\n",
                           oifp->name, &sroute->group, &sroute->source);
                count++;
              }
        }
    }
