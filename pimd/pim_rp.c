...
void
pim_rp_init (struct pim_instance *pim)
{
  ...
  if (! pim_get_all_mcast_group (&rp_info->group))
    {
      flog_err (EC_LIB_DEVELOPMENT, "Unable to convert all-multicast prefix");
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("Allocated: %p for rp_info: %p(%pFX) Lock: %d", rn, rp_info,
                &rp_info->group, route_node_get_lock_count (rn));
}

void
pim_rp_free (struct pim_instance *pim)
{
  if (pim->rp_table)
    route_table_finish (pim->rp_table);
  pim->rp_table = NULL;
...
pim_rp_find_prefix_list (struct pim_instance *pim, pim_addr rp,
                         const char *plist)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (pim->rp_list, node, rp_info))
    {
      if ((! pim_addr_cmp (rp, rp_info->rp.rpf_addr)) && rp_info->plist &&
          strcmp (rp_info->plist, plist) == 0)
        {
          return rp_info;
        }
    }
 ...
 */
static int
pim_rp_prefix_list_used (struct pim_instance *pim, const char *plist)
{
  struct listnode *node;
  struct rp_info *rp_info;
 ...
 */
static struct rp_info *
pim_rp_find_exact (struct pim_instance *pim, pim_addr rp,
 ...
 */
struct rp_info *
pim_rp_find_match_group (struct pim_instance *pim, const struct prefix *group)
{
  struct listnode *node;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    {
      if (best)
        zlog_debug ("Lookedup(%pFX): prefix_list match %s, rn %p found: %pFX",
...
void
pim_rp_prefix_list_update (struct pim_instance *pim, struct prefix_list *plist)
{
  struct listnode *node;
  struct rp_info *rp_info;
...
static void
pim_rp_check_interfaces (struct pim_instance *pim, struct rp_info *rp_info)
{
...
void
pim_upstream_update (struct pim_instance *pim, struct pim_upstream *up)
{
  ...
  pim_rp_set_upstream_addr (pim, &new_upstream_addr, up->sg.src, up->sg.grp);

  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: pim upstream update for old upstream %pPA", __func__,
                &old_upstream_addr);
  ...
  if (rpf_result == PIM_RPF_CHANGED ||
      (rpf_result == PIM_RPF_FAILURE && old_rpf.source_nexthop.interface))
    pim_zebra_upstream_rpf_changed (pim, up, &old_rpf);
}

int
pim_rp_new (struct pim_instance *pim, pim_addr rp_addr, struct prefix group,
            const char *plist, enum rp_source rp_src_flag)
{
  ...
  if (plist)
    {
       ...
       */
      for (ALL_LIST_ELEMENTS (pim->rp_list, node, nnode, tmp_rp_info))
        {
          if (! pim_addr_cmp (rp_info->rp.rpf_addr, tmp_rp_info->rp.rpf_addr))
            {
              if (tmp_rp_info->plist)
                pim_rp_del_config (pim, rp_addr, NULL, tmp_rp_info->plist);
              else
                pim_rp_del_config (
                    pim, rp_addr,
                    prefix2str (&tmp_rp_info->group, buffer, BUFSIZ), NULL);
            }
        }
  ...
  else
    {
      ...
      if (prefix_same (&rp_all->group, &rp_info->group) &&
          pim_rpf_addr_is_inaddr_any (&rp_all->rp))
        {
          ...
          pim_rp_refresh_group_to_rp_mapping (pim);
          pim_find_or_track_nexthop (pim, nht_p, NULL, rp_all, NULL);

          if (! pim_ecmp_nexthop_lookup (pim, &rp_all->rp.source_nexthop,
      ...
      if (tmp_rp_info)
        {
          if (tmp_rp_info->plist)
          ...
          else
            {
               ...
               */
              if (prefix_same (&rp_info->group, &tmp_rp_info->group))
                {
                  if ((rp_src_flag == RP_SRC_STATIC) &&
                      (tmp_rp_info->rp_src == RP_SRC_STATIC))
                    {
                      XFREE (MTYPE_PIM_RP, rp_info);
                      return PIM_GROUP_OVERLAP;
                    }

                  result = pim_rp_change (pim, rp_addr, tmp_rp_info->group,
                                          rp_src_flag);
                  XFREE (MTYPE_PIM_RP, rp_info);
                  return result;
                }
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("Allocated: %p for rp_info: %p(%pFX) Lock: %d", rn, rp_info,
                &rp_info->group, rn ? route_node_get_lock_count (rn) : 0);
...
void
pim_rp_del_config (struct pim_instance *pim, pim_addr rp_addr,
                   const char *group_range, const char *plist)
{
  ...
  if (! result)
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: String to prefix failed for %pPAs group", __func__,
                    &rp_addr);
      return;
    }
...
int
pim_rp_del (struct pim_instance *pim, pim_addr rp_addr, struct prefix group,
            const char *plist, enum rp_source rp_src_flag)
{
  struct prefix g_all;
  struct rp_info *rp_info;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    zlog_debug ("%s: Delete RP %pPA for the group %pFX", __func__, &rp_addr,
                &group);
      ...
      if (bsgrp)
        {
          ...
          if (bsrp)
            {
              if (PIM_DEBUG_PIM_TRACE)
                zlog_debug ("%s: BSM RP %pPA found for the group %pFX",
                            __func__, &bsrp->rp_address, &group);
              return pim_rp_change (pim, bsrp->rp_address, group, RP_SRC_BSR);
            }
        }
      else
        {
          if (PIM_DEBUG_PIM_TRACE)
            zlog_debug ("%s: BSM RP not found for the group %pFX", __func__,
                        &group);
        }
    }
...
int
pim_rp_change (struct pim_instance *pim, pim_addr new_rp_addr,
...
void
pim_rp_setup (struct pim_instance *pim)
{
  struct listnode *node;
  struct rp_info *rp_info;
  pim_addr nht_p;
      ...
      if (pim_rp_check_interface_addrs (rp_info, pim_ifp))
        {
          ...
          rp_info->i_am_rp = 1;
          if (PIM_DEBUG_PIM_NHT_RP)
            zlog_debug ("%s: %pPA: i am rp", __func__, &rp_info->rp.rpf_addr);
        }
    }
...
void
pim_i_am_rp_re_evaluate (struct pim_instance *pim)
{
  struct listnode *node;
  struct rp_info *rp_info;
  bool i_am_rp_changed = false;
          ...
          if (PIM_DEBUG_PIM_NHT_RP)
            {
              if (rp_info->i_am_rp)
                ...
                zlog_debug ("%s: %pPA: i am no longer rp", __func__,
                            &rp_info->rp.rpf_addr);
            }
        }
    }
 ...
 */
int
pim_rp_i_am_rp (struct pim_instance *pim, pim_addr group)
{
  struct prefix g;
  struct rp_info *rp_info;
 ...
 */
struct pim_rpf *
pim_rp_g (struct pim_instance *pim, pim_addr group)
{
  struct prefix g;
  struct rp_info *rp_info;
  ...
  if (rp_info)
    {
      ...
      (void) pim_ecmp_nexthop_lookup (pim, &rp_info->rp.source_nexthop, nht_p,
                                      &rp_info->group, 1);
      return (&rp_info->rp);
    }
 ...
 */
int
pim_rp_set_upstream_addr (struct pim_instance *pim, pim_addr *up,
  ...
  for (ALL_LIST_ELEMENTS_RO (pim->rp_list, node, rp_info))
    {
      ...
      if (rp_info->plist)
        vty_out (vty, "%s" PIM_AF_NAME " pim rp %pPA prefix-list %s\n", spaces,
                 &rp_addr, rp_info->plist);
      else
...
void
pim_rp_show_information (struct pim_instance *pim, struct prefix *range,
  ...
  if (! json)
    {
      ...
      ttable_add_row (
          tt, "RP address|group/prefix-list|OIF|I am RP|Source|Group-Type");
...
#endif
      const char *group_type = pim_is_grp_ssm (pim, group) ? "SSM" : "ASM";
      ...
      if (json)
        {
           ...
           */
          if (prev_rp_info &&
              (pim_addr_cmp (prev_rp_info->rp.rpf_addr, rp_info->rp.rpf_addr)))
            {
          ...
          if (rp_info->plist)
            ...
            json_object_string_addf (json_row, "group", "%pFX",
                                     &rp_info->group);
          json_object_string_add (json_row, "source", source);
          json_object_string_add (json_row, "groupType", group_type);
      ...
      else
        {
          ...
          ttable_add_row (
              tt, "%pPA|%s|%s|%s|%s|%s", &rp_info->rp.rpf_addr,
              rp_info->plist ? rp_info->plist : grp,
              rp_info->rp.source_nexthop.interface ? rp_info->rp.source_nexthop
                                                         .interface->name
                                                   : "Unknown",
              rp_info->i_am_rp ? "yes" : "no", source, group_type);
        }
...
void
pim_resolve_rp_nh (struct pim_instance *pim, struct pim_neighbor *nbr)
{
...
#endif
          if (PIM_DEBUG_PIM_NHT_RP)
            zlog_debug ("%s: addr %pPA new nexthop addr %pPAs interface %s",
                        __func__, &nht_p, &nbr->source_addr, ifp1->name);
        }
    }
}
