...
static const char *
rtg_proto_str (enum mtrace_rtg_proto proto)
{
  static char buf[80];
...
static void
print_rtg_proto (uint32_t rtg_proto)
{
  printf ("%s", rtg_proto_str (rtg_proto));
}

static void
print_fwd_ttl (uint32_t fwd_ttl)
{
  printf ("thresh^ %d", fwd_ttl);
}

static const char *
fwd_code_str (enum mtrace_fwd_code code)
{
  static char buf[80];
...
static void
print_fwd_code (uint32_t fwd_code)
{
  printf ("%s", fwd_code_str (fwd_code));
}

static void
print_rsp (struct igmp_mtrace_rsp *rsp)
{
...
static void
print_dest (struct igmp_mtrace *mtrace)
{
  print_line_no (0);
...
static void
print_summary (struct igmp_mtrace *mtrace, int hops, long msec)
{
  int i;
  int t = 0;
...
static void
print_responses (struct igmp_mtrace *mtrace, int hops, long msec)
{
  int i;
...
static int
send_query (int fd, struct in_addr to_addr, struct igmp_mtrace *mtrace)
{
...
static void
print_query (struct igmp_mtrace *mtrace)
{
...
static int
recv_response (int fd, int *hops, struct igmp_mtrace *mtracer)
{
  int recvd;
...
static int
wait_for_response (int fd, int *hops, struct igmp_mtrace *mtrace,
                   long *ret_msec)
{
  fd_set readfds;
...
static bool
check_end (struct igmp_mtrace *mtrace, int hops)
{
