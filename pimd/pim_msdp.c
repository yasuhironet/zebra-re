...
static void pim_msdp_peer_cr_timer_setup (struct pim_msdp_peer *mp,
                                          bool start);
static void pim_msdp_peer_ka_timer_setup (struct pim_msdp_peer *mp,
                                          bool start);
...
static void
pim_msdp_sa_adv_timer_cb (struct event *t)
{
...
static void
pim_msdp_sa_adv_timer_setup (struct pim_instance *pim, bool start)
{
  EVENT_OFF (pim->msdp.sa_adv_timer);
  if (start)
    {
      event_add_timer (pim->msdp.master, pim_msdp_sa_adv_timer_cb, pim,
                       PIM_MSDP_SA_ADVERTISMENT_TIME, &pim->msdp.sa_adv_timer);
    }
}

/* RFC-3618:Sec-5.3 - SA cache state timer */
static void
pim_msdp_sa_state_timer_cb (struct event *t)
{
...
pim_msdp_sa_state_timer_setup (struct pim_msdp_sa *sa, bool start)
{
  ...
  if (start)
    {
      event_add_timer (sa->pim->msdp.master, pim_msdp_sa_state_timer_cb, sa,
...
static bool
pim_msdp_sa_upstream_add_ok (struct pim_msdp_sa *sa,
...
pim_msdp_sa_new (struct pim_instance *pim, pim_sgaddr *sg, struct in_addr rp)
{
  struct pim_msdp_sa *sa;
...
static struct pim_msdp_sa *
pim_msdp_sa_find (struct pim_instance *pim, pim_sgaddr *sg)
{
...
static struct pim_msdp_sa *
pim_msdp_sa_add (struct pim_instance *pim, pim_sgaddr *sg, struct in_addr rp)
{
  struct pim_msdp_sa *sa;
...
void
pim_msdp_sa_ref (struct pim_instance *pim, struct pim_msdp_peer *mp,
...
static void
pim_msdp_sa_local_add (struct pim_instance *pim, pim_sgaddr *sg)
{
  struct in_addr rp;
  rp.s_addr = INADDR_ANY;
...
void
pim_msdp_sa_local_del (struct pim_instance *pim, pim_sgaddr *sg)
{
  struct pim_msdp_sa *sa;
...
static void
pim_msdp_sa_local_del_on_up_del (struct pim_instance *pim, pim_sgaddr *sg)
{
  struct pim_msdp_sa *sa;
...
static void
pim_msdp_sa_local_setup (struct pim_instance *pim)
{
  struct pim_upstream *up;
...
void
pim_msdp_i_am_rp_changed (struct pim_instance *pim)
{
 ...
 * (*, G) */
void
pim_msdp_up_join_state_changed (struct pim_instance *pim,
                                struct pim_upstream *xg_up)
{
  struct listnode *sanode;
  struct pim_msdp_sa *sa;
...
static void
pim_msdp_up_xg_del (struct pim_instance *pim, pim_sgaddr *sg)
{
  struct listnode *sanode;
  struct pim_msdp_sa *sa;
...
void
pim_msdp_up_del (struct pim_instance *pim, pim_sgaddr *sg)
{
  if (PIM_DEBUG_MSDP_INTERNAL)
    {
...
static unsigned int
pim_msdp_sa_hash_key_make (const void *p)
{
...
static void
pim_msdp_peer_timer_expiry_log (struct pim_msdp_peer *mp,
...
static void
pim_msdp_peer_hold_timer_cb (struct event *t)
{
...
pim_msdp_peer_hold_timer_setup (struct pim_msdp_peer *mp, bool start)
{
  ...
  if (start)
    {
      event_add_timer (pim->msdp.master, pim_msdp_peer_hold_timer_cb, mp,
                       pim->msdp.hold_time, &mp->hold_timer);
    }
}
...
static void
pim_msdp_peer_ka_timer_cb (struct event *t)
{
  struct pim_msdp_peer *mp;
...
pim_msdp_peer_ka_timer_setup (struct pim_msdp_peer *mp, bool start)
{
  ...
  if (start)
    {
      event_add_timer (mp->pim->msdp.master, pim_msdp_peer_ka_timer_cb, mp,
...
pim_msdp_peer_active_connect (struct pim_msdp_peer *mp)
{
  ...
  if (PIM_DEBUG_MSDP_INTERNAL)
    {
      zlog_debug ("MSDP peer %s pim_msdp_peer_active_connect: %d", mp->key_str,
                  rc);
    }
...
static void
pim_msdp_peer_cr_timer_cb (struct event *t)
{
  struct pim_msdp_peer *mp;
...
pim_msdp_peer_cr_timer_setup (struct pim_msdp_peer *mp, bool start)
{
  ...
  if (start)
    {
      event_add_timer (mp->pim->msdp.master, pim_msdp_peer_cr_timer_cb, mp,
...
static void
pim_msdp_addr2su (union sockunion *su, struct in_addr addr)
{
  sockunion_init (su);
...
pim_msdp_peer_add (struct pim_instance *pim, const struct in_addr *peer,
                   const struct in_addr *local, const char *mesh_group_name)
{
  struct pim_msdp_peer *mp;
...
struct pim_msdp_peer *
pim_msdp_peer_find (struct pim_instance *pim, struct in_addr peer_addr)
{
...
void
pim_msdp_peer_del (struct pim_msdp_peer **mp)
{
  if (*mp == NULL)
    return;
...
void
pim_msdp_peer_change_source (struct pim_msdp_peer *mp,
...
static unsigned int
pim_msdp_peer_hash_key_make (const void *p)
{
...
void
pim_msdp_mg_free (struct pim_instance *pim, struct pim_msdp_mg **mgp)
{
  ...
  if (PIM_DEBUG_MSDP_EVENTS)
    {
      zlog_debug ("MSDP mesh-group %s deleted", (*mgp)->mesh_group_name);
    }
...
struct pim_msdp_mg *
pim_msdp_mg_new (struct pim_instance *pim, const char *mesh_group_name)
{
...
void
pim_msdp_mg_mbr_del (struct pim_msdp_mg *mg, struct pim_msdp_mg_mbr *mbr)
{
  ...
  if (PIM_DEBUG_MSDP_EVENTS)
    {
      ...
      zlog_debug ("MSDP mesh-group %s mbr %s deleted", mg->mesh_group_name,
                  ip_str);
    }
...
static void
pim_msdp_src_del (struct pim_msdp_mg *mg)
{
  struct pim_msdp_mg_mbr *mbr;
...
/*********************** MSDP feature APIs *********************************/
int
pim_msdp_config_write (struct pim_instance *pim, struct vty *vty,
                       const char *spaces)
{
  struct pim_msdp_mg *mg;
...
bool
pim_msdp_peer_config_write (struct vty *vty, struct pim_instance *pim,
                            const char *spaces)
{
  struct pim_msdp_peer *mp;
...
static void
pim_msdp_enable (struct pim_instance *pim)
{
  if (pim->msdp.flags & PIM_MSDPF_ENABLE)
    {
...
/* MSDP init */
void
pim_msdp_init (struct pim_instance *pim, struct event_loop *master)
{
...
void
pim_msdp_exit (struct pim_instance *pim)
{
  struct pim_msdp_mg *mg;
...
void
pim_msdp_mg_src_add (struct pim_instance *pim, struct pim_msdp_mg *mg,
                     struct in_addr *ai)
{
  struct pim_msdp_mg_mbr *mbr;
  struct listnode *mbr_node;
  ...
  if (ai->s_addr == INADDR_ANY)
    {
      if (PIM_DEBUG_MSDP_EVENTS)
        zlog_debug ("MSDP mesh-group %s src unset", mg->mesh_group_name);
      return;
    }
  ...
  if (PIM_DEBUG_MSDP_EVENTS)
    zlog_debug ("MSDP mesh-group %s src %pI4 set", mg->mesh_group_name,
                &mg->src_ip);
}

struct pim_msdp_mg_mbr *
pim_msdp_mg_mbr_add (struct pim_instance *pim, struct pim_msdp_mg *mg,
                     struct in_addr *ia)
{
  struct pim_msdp_mg_mbr *mbr;
  ...
  if (PIM_DEBUG_MSDP_EVENTS)
    zlog_debug ("MSDP mesh-group %s mbr %pI4 created", mg->mesh_group_name,
                &mbr->mbr_ip);
