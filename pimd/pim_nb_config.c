...
#else /* PIM_IPV != 6 */
#define pim6_msdp_err(funcname, argtype) MACRO_REQUIRE_SEMICOLON ()

#define yang_dnode_get_pimaddr yang_dnode_get_ipv4
#endif /* PIM_IPV != 6 */

static void
pim_if_membership_clear (struct interface *ifp)
{
 ...
 */
static void
pim_if_membership_refresh (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
#if PIM_IPV == 4
  ...
  frr_each_from (gm_sgs, gm_ifp->sgs, sg, sg_start)
    {
      if (! in6_multicast_nofwd (&sg->sgaddr.grp))
        {
          pim_ifchannel_local_membership_add (ifp, &sg->sgaddr,
                                              false /*is_vxlan*/);
        }
    }
#endif
...
static int
interface_pim_use_src_cmd_worker (struct interface *ifp, pim_addr source_addr,
                                  char *errmsg, size_t errmsg_len)
{
  ...
  switch (result)
    ...
    case PIM_IFACE_NOT_FOUND:
      ret = NB_ERR;
      snprintf (errmsg, errmsg_len, "Pim not enabled on this interface %s",
                ifp->name);
      break;
...
static int
pim_cmd_spt_switchover (struct pim_instance *pim, enum pim_spt_switchover spt,
                        const char *plist)
{
...
static int
pim_ssm_cmd_worker (struct pim_instance *pim, const char *plist, char *errmsg,
                    size_t errmsg_len)
{
  ...
  switch (result)
    ...
    case PIM_SSM_ERR_NO_VRF:
      snprintf (errmsg, errmsg_len, "VRF doesn't exist");
      break;
    case PIM_SSM_ERR_DUP:
      snprintf (errmsg, errmsg_len, "duplicate config");
      break;
    default:
      snprintf (errmsg, errmsg_len, "ssm range config failed");
    }
...
static int
pim_rp_cmd_worker (struct pim_instance *pim, pim_addr rp_addr,
                   struct prefix group, const char *plist, char *errmsg,
                   size_t errmsg_len)
{
  int result;
  ...
  if (result == PIM_RP_NO_PATH)
    {
      snprintfrr (errmsg, errmsg_len, "No Path to RP address specified: %pPA",
                  &rp_addr);
...
static int
pim_no_rp_cmd_worker (struct pim_instance *pim, pim_addr rp_addr,
                      struct prefix group, const char *plist, char *errmsg,
                      size_t errmsg_len)
{
  ...
  if (result == PIM_GROUP_BAD_ADDRESS)
    {
      snprintf (errmsg, errmsg_len, "Bad group address specified: %s",
                group_str);
      return NB_ERR_INCONSISTENCY;
    }
  ...
  if (result == PIM_RP_NOT_FOUND)
    {
      snprintf (errmsg, errmsg_len, "Unable to find specified RP");
      return NB_ERR_INCONSISTENCY;
    }
...
static bool
is_pim_interface (const struct lyd_node *dnode)
{
  ...
  pim_enable_dnode = yang_dnode_getf (
      dnode, "%s/frr-pim:pim/address-family[address-family='%s']/pim-enable",
      if_xpath, FRR_PIM_AF_XPATH_VAL);
  igmp_enable_dnode = yang_dnode_getf (
      dnode, "%s/frr-gmp:gmp/address-family[address-family='%s']/enable",
      if_xpath, FRR_PIM_AF_XPATH_VAL);

  if (((pim_enable_dnode) && (yang_dnode_get_bool (pim_enable_dnode, "."))) ||
      ((igmp_enable_dnode) && (yang_dnode_get_bool (igmp_enable_dnode, "."))))
    return true;
...
static int
pim_cmd_gm_start (struct interface *ifp)
{
  struct pim_interface *pim_ifp;
...
#if PIM_IPV == 4
static void
igmp_sock_query_interval_reconfig (struct gm_sock *igmp)
{
  ...
  if (PIM_DEBUG_GM_TRACE)
    zlog_debug ("%s: Querier %pPAs on %s reconfig query_interval=%d", __func__,
                &igmp->ifaddr, ifp->name, pim_ifp->gm_default_query_interval);
...
static void
igmp_sock_query_reschedule (struct gm_sock *igmp)
{
  if (igmp->mtrace_only)
    return;
...
#if PIM_IPV == 4
static void
change_query_interval (struct pim_interface *pim_ifp, int query_interval)
{
...
static void
change_query_max_response_time (struct interface *ifp,
                                int query_max_response_time_dsec)
{
  ...
  if (pim_ifp->gm_query_max_response_time_dsec == query_max_response_time_dsec)
    return;
...
int
routing_control_plane_protocols_name_validate (struct nb_cb_create_args *args)
{
  const char *name;
 ...
 */
int
pim_address_family_create (struct nb_cb_create_args *args)
{
  switch (args->event)
    {
    case NB_EV_VALIDATE:
...
int
pim_address_family_destroy (struct nb_cb_destroy_args *args)
{
  switch (args->event)
    {
    case NB_EV_VALIDATE:
 ...
 */
int
pim_address_family_packets_modify (struct nb_cb_modify_args *args)
{
  switch (args->event)
    {
    case NB_EV_VALIDATE:
    ...
    case NB_EV_ABORT:
      break;
    case NB_EV_APPLY:
      router->packet_process = yang_dnode_get_uint8 (args->dnode, NULL);
      break;
    }
 ...
 */
int
pim_address_family_join_prune_interval_modify (struct nb_cb_modify_args *args)
{
  switch (args->event)
    {
    case NB_EV_VALIDATE:
 ...
 */
int
pim_address_family_register_suppress_time_modify (
    struct nb_cb_modify_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_ABORT:
      break;
    case NB_EV_APPLY:
      pim_update_suppress_timers (yang_dnode_get_uint16 (args->dnode, NULL));
      break;
    }
...
/*
 * XPath:
 * /frr-routing:routing/control-plane-protocols/control-plane-protocol/frr-pim:pim/address-family/ecmp
 */
int
routing_control_plane_protocols_control_plane_protocol_pim_address_family_ecmp_modify (
    struct nb_cb_modify_args *args)
{
  struct vrf *vrf;
...
/*
 * XPath:
 * /frr-routing:routing/control-plane-protocols/control-plane-protocol/frr-pim:pim/address-family/ecmp-rebalance
 */
int
routing_control_plane_protocols_control_plane_protocol_pim_address_family_ecmp_rebalance_modify (
    struct nb_cb_modify_args *args)
{
  struct vrf *vrf;
  struct pim_instance *pim;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      vrf = nb_running_get_entry (args->dnode, NULL, true);
      pim = vrf->info;
      pim->ecmp_rebalance_enable = yang_dnode_get_bool (args->dnode, NULL);
    }
...
/*
 * XPath:
 * /frr-routing:routing/control-plane-protocols/control-plane-protocol/frr-pim:pim/address-family/keep-alive-timer
 */
int
routing_control_plane_protocols_control_plane_protocol_pim_address_family_keep_alive_timer_modify (
    struct nb_cb_modify_args *args)
{
  struct vrf *vrf;
  struct pim_instance *pim;
...
/*
 * XPath:
 * /frr-routing:routing/control-plane-protocols/control-plane-protocol/frr-pim:pim/address-family/rp-keep-alive-timer
 */
int
routing_control_plane_protocols_control_plane_protocol_pim_address_family_rp_keep_alive_timer_modify (
    struct nb_cb_modify_args *args)
{
  struct vrf *vrf;
  struct pim_instance *pim;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      vrf = nb_running_get_entry (args->dnode, NULL, true);
      pim = vrf->info;
      pim->rp_keep_alive_time = yang_dnode_get_uint16 (args->dnode, NULL);
      break;
    }
    ...
    struct nb_cb_apply_finish_args *args)
{
  ...
  switch (spt_switch_action)
    {
    case PIM_SPT_INFINITY:
      if (yang_dnode_exists (args->dnode, "./spt-infinity-prefix-list"))
        prefix_list =
            yang_dnode_get_string (args->dnode, "./spt-infinity-prefix-list");
    ...
    struct nb_cb_modify_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      result =
          pim_ssm_cmd_worker (pim, plist_name, args->errmsg, args->errmsg_len);
    ...
    struct nb_cb_destroy_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      vrf = nb_running_get_entry (args->dnode, NULL, true);
      pim = vrf->info;
      result = pim_ssm_cmd_worker (pim, NULL, args->errmsg, args->errmsg_len);
 ...
 */
int
pim_msdp_hold_time_modify (struct nb_cb_modify_args *args)
{
  struct pim_instance *pim;
  struct vrf *vrf;
 ...
 */
int
pim_msdp_keep_alive_modify (struct nb_cb_modify_args *args)
{
  struct pim_instance *pim;
  struct vrf *vrf;
 ...
 */
int
pim_msdp_connection_retry_modify (struct nb_cb_modify_args *args)
{
  struct pim_instance *pim;
  struct vrf *vrf;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      vrf = nb_running_get_entry (args->dnode, NULL, true);
      pim = vrf->info;
      pim->msdp.connection_retry = yang_dnode_get_uint16 (args->dnode, NULL);
      break;
    }
 ...
 */
int
pim_msdp_mesh_group_create (struct nb_cb_create_args *args)
{
...
int
pim_msdp_mesh_group_destroy (struct nb_cb_destroy_args *args)
{
  struct pim_msdp_mg *mg;
  struct vrf *vrf;
 ...
 */
int
pim_msdp_mesh_group_source_modify (struct nb_cb_modify_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      vrf_dnode = yang_dnode_get_parent (args->dnode, "address-family");
...
int
pim_msdp_mesh_group_source_destroy (struct nb_cb_destroy_args *args)
{
  const struct lyd_node *vrf_dnode;
  struct pim_msdp_mg *mg;
  struct vrf *vrf;
 ...
 */
int
pim_msdp_mesh_group_members_create (struct nb_cb_create_args *args)
{
  const struct lyd_node *vrf_dnode;
...
int
pim_msdp_mesh_group_members_destroy (struct nb_cb_destroy_args *args)
{
  struct pim_msdp_mg_mbr *mbr;
  struct pim_msdp_mg *mg;
    ...
    struct nb_cb_create_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      mp = pim_msdp_peer_add (pim, &peer_ip.ipaddr_v4, &source_ip.ipaddr_v4,
                              NULL);
 ...
 */
int
lib_interface_pim_address_family_create (struct nb_cb_create_args *args)
{
  switch (args->event)
    {
    case NB_EV_VALIDATE:
...
int
lib_interface_pim_address_family_destroy (struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      if (! pim_cmd_interface_delete (ifp))
        {
          snprintf (args->errmsg, args->errmsg_len,
                    "Unable to delete interface information %s", ifp->name);
          return NB_ERR_INCONSISTENCY;
        }
    }
 ...
 */
int
lib_interface_pim_address_family_pim_enable_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  switch (args->event)
    ...
    case NB_EV_VALIDATE:
      ...
      mcast_if_count = yang_get_list_elements_count (if_dnode);

      /* Limiting mcast interfaces to number of VIFs */
      if (mcast_if_count == MAXVIFS)
        {
          snprintf (args->errmsg, args->errmsg_len,
                    "Max multicast interfaces(%d) reached.", MAXVIFS);
          return NB_ERR_VALIDATION;
        }
      break;
    case NB_EV_PREPARE:
      ...
      if (yang_dnode_get_bool (args->dnode, NULL))
        {
          if (! pim_cmd_interface_add (ifp))
            {
              snprintf (args->errmsg, args->errmsg_len,
                        "Could not enable PIM SM on interface %s", ifp->name);
              return NB_ERR_INCONSISTENCY;
            }
        }
      else
 ...
 */
int
lib_interface_pim_address_family_pim_passive_enable_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-pim:pim/address-family/hello-interval
 */
int
lib_interface_pim_address_family_hello_interval_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ifp = nb_running_get_entry (args->dnode, NULL, true);
      pim_ifp = ifp->info;
      pim_ifp->pim_hello_period = yang_dnode_get_uint8 (args->dnode, NULL);
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-pim:pim/address-family/hello-holdtime
 */
int
lib_interface_pim_address_family_hello_holdtime_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
int
lib_interface_pim_address_family_hello_holdtime_destroy (
    struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
 ...
 */
int
lib_interface_pim_address_family_bfd_create (struct nb_cb_create_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
int
lib_interface_pim_address_family_bfd_destroy (struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  const struct lyd_node *if_dnode;
 ...
 */
void
lib_interface_pim_address_family_bfd_apply_finish (
    struct nb_cb_apply_finish_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-pim:pim/address-family/bfd/min-rx-interval
 */
int
lib_interface_pim_address_family_bfd_min_rx_interval_modify (
    struct nb_cb_modify_args *args)
{
  switch (args->event)
    {
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-pim:pim/address-family/bfd/min-tx-interval
 */
int
lib_interface_pim_address_family_bfd_min_tx_interval_modify (
    struct nb_cb_modify_args *args)
{
  switch (args->event)
    {
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-pim:pim/address-family/bfd/detect_mult
 */
int
lib_interface_pim_address_family_bfd_detect_mult_modify (
    struct nb_cb_modify_args *args)
{
  switch (args->event)
    {
 ...
 */
int
lib_interface_pim_address_family_bfd_profile_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
int
lib_interface_pim_address_family_bfd_profile_destroy (
    struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
 ...
 */
int
lib_interface_pim_address_family_bsm_modify (struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
 ...
 */
int
lib_interface_pim_address_family_unicast_bsm_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ifp = nb_running_get_entry (args->dnode, NULL, true);
      pim_ifp = ifp->info;
      pim_ifp->ucast_bsm_accept = yang_dnode_get_bool (args->dnode, NULL);
 ...
 */
int
lib_interface_pim_address_family_active_active_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
    ...
    struct nb_cb_modify_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      pim_ifp->pim_dr_priority = yang_dnode_get_uint32 (args->dnode, NULL);
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      result = interface_pim_use_src_cmd_worker (
          ifp, source_addr, args->errmsg, args->errmsg_len);
    ...
    struct nb_cb_destroy_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      result = interface_pim_use_src_cmd_worker (
          ifp, PIMADDR_ANY, args->errmsg, args->errmsg_len);
    ...
    struct nb_cb_modify_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      pim_ifp->boundary_oil_plist = XSTRDUP (MTYPE_PIM_INTERFACE, plist);
 ...
 */
int
lib_interface_pim_address_family_mroute_create (struct nb_cb_create_args *args)
{
  switch (args->event)
    {
    case NB_EV_VALIDATE:
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      if (! oif)
        {
          snprintf (args->errmsg, args->errmsg_len,
                    "No such interface name %s", oifname);
          return NB_ERR_INCONSISTENCY;
        }
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      if (yang_dnode_get (args->dnode, "./group-list"))
        {
          yang_dnode_get_prefix (&group, args->dnode, "./group-list");
          apply_mask (&group);
        ...
        {
          plist = yang_dnode_get_string (args->dnode, "./prefix-list");
          ...
          result = pim_no_rp_cmd_worker (pim, rp_addr, group, plist,
                                         args->errmsg, args->errmsg_len);
        }
    ...
    struct nb_cb_create_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      apply_mask (&group);
      return pim_rp_cmd_worker (pim, rp_addr, group, NULL, args->errmsg,
                                args->errmsg_len);
    }
    ...
    struct nb_cb_destroy_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      return pim_no_rp_cmd_worker (pim, rp_addr, group, NULL, args->errmsg,
                                   args->errmsg_len);
    }
    ...
    struct nb_cb_modify_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      return pim_rp_cmd_worker (pim, rp_addr, group, plist, args->errmsg,
                                args->errmsg_len);
    }
    ...
    struct nb_cb_destroy_args *args)
{
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ...
      return pim_no_rp_cmd_worker (pim, rp_addr, group, plist, args->errmsg,
                                   args->errmsg_len);
      break;
    }
 ...
 */
int
lib_interface_gmp_address_family_create (struct nb_cb_create_args *args)
{
  switch (args->event)
    {
    case NB_EV_VALIDATE:
...
int
lib_interface_gmp_address_family_destroy (struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
 ...
 */
int
lib_interface_gmp_address_family_enable_modify (struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  bool gm_enable;
  struct pim_interface *pim_ifp;
 ...
 */
int
lib_interface_gmp_address_family_igmp_version_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
int
lib_interface_gmp_address_family_igmp_version_destroy (
    struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
 ...
 */
int
lib_interface_gmp_address_family_mld_version_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
int
lib_interface_gmp_address_family_mld_version_destroy (
    struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-gmp:gmp/address-family/query-interval
 */
int
lib_interface_gmp_address_family_query_interval_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  int query_interval;
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-gmp:gmp/address-family/query-max-response-time
 */
int
lib_interface_gmp_address_family_query_max_response_time_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  int query_max_response_time_dsec;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ifp = nb_running_get_entry (args->dnode, NULL, true);
      query_max_response_time_dsec = yang_dnode_get_uint16 (args->dnode, NULL);
      change_query_max_response_time (ifp, query_max_response_time_dsec);
    }
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-gmp:gmp/address-family/last-member-query-interval
 */
int
lib_interface_gmp_address_family_last_member_query_interval_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ifp = nb_running_get_entry (args->dnode, NULL, true);
      pim_ifp = ifp->info;
      last_member_query_interval = yang_dnode_get_uint16 (args->dnode, NULL);
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-gmp:gmp/address-family/robustness-variable
 */
int
lib_interface_gmp_address_family_robustness_variable_modify (
    struct nb_cb_modify_args *args)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  switch (args->event)
    ...
    case NB_EV_APPLY:
      ifp = nb_running_get_entry (args->dnode, NULL, true);
      pim_ifp = ifp->info;
      last_member_query_count = yang_dnode_get_uint8 (args->dnode, NULL);
 ...
 */
int
lib_interface_gmp_address_family_static_group_create (
    struct nb_cb_create_args *args)
{
  struct interface *ifp;
  pim_addr source_addr;
...
int
lib_interface_gmp_address_family_static_group_destroy (
    struct nb_cb_destroy_args *args)
{
  struct interface *ifp;
  pim_addr source_addr;
