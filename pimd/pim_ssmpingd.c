...
void
pim_ssmpingd_init (struct pim_instance *pim)
{
  int result;
  ...
  result =
      inet_pton (PIM_AF, PIM_SSMPINGD_REPLY_GROUP, &pim->ssmpingd_group_addr);

  assert (result > 0);
}

void
pim_ssmpingd_destroy (struct pim_instance *pim)
{
  if (pim->ssmpingd_list)
    list_delete (&pim->ssmpingd_list);
}

static struct ssmpingd_sock *
ssmpingd_find (struct pim_instance *pim, pim_addr source_addr)
{
...
static void
ssmpingd_free (struct ssmpingd_sock *ss)
{
...
#if PIM_IPV == 4
static inline int
ssmpingd_setsockopt (int fd, pim_addr addr, int mttl)
{
...
static int
ssmpingd_socket (pim_addr addr, int port, int mttl)
{
  ...
  if (fd < 0)
    {
      flog_err_sys (EC_LIB_SOCKET, "%s: could not create socket: errno=%d: %s",
...
static void
ssmpingd_delete (struct ssmpingd_sock *ss)
{
  assert (ss);
...
static void
ssmpingd_sendto (struct ssmpingd_sock *ss, const uint8_t *buf, int len,
                 struct sockaddr_storage to)
{
  socklen_t tolen = sizeof (to);
  int sent;

  sent = sendto (ss->sock_fd, buf, len, MSG_DONTWAIT, (struct sockaddr *) &to,
                 tolen);
  if (sent != len)
    {
      if (sent < 0)
        {
          zlog_warn (
              ...
              __func__, &to, ss->sock_fd, len, errno, safe_strerror (errno));
        }
      else
        {
          zlog_warn ("%s: sendto() partial to %pSUp, fd=%d len=%d: sent=%d",
...
static int
ssmpingd_read_msg (struct ssmpingd_sock *ss)
{
  ...
  len = pim_socket_recvfromto (ss->sock_fd, buf, sizeof (buf), &from, &fromlen,
                               &to, &tolen, &ifindex);
  ...
  if (buf[0] != PIM_SSMPINGD_REQUEST)
    {
      zlog_warn (
          ...
          __func__, buf[0], &from, &to, ifp ? ifp->name : "<iface?>", ifindex,
  ...
  if (PIM_DEBUG_SSMPINGD)
    {
      zlog_debug (
          ...
          __func__, &from, &to, ifp ? ifp->name : "<iface?>", ifindex,
          ss->sock_fd, &ss->source_addr);
    }
...
static void
ssmpingd_sock_read (struct event *t)
{
  struct ssmpingd_sock *ss;
...
static void
ssmpingd_read_on (struct ssmpingd_sock *ss)
{
...
static struct ssmpingd_sock *
ssmpingd_new (struct pim_instance *pim, pim_addr source_addr)
{
  struct ssmpingd_sock *ss;
  int sock_fd;
  ...
  if (sock_fd < 0)
    {
      zlog_warn ("%s: ssmpingd_socket() failure for source %pPA", __func__,
                 &source_addr);
      return 0;
    }
...
int
pim_ssmpingd_start (struct pim_instance *pim, pim_addr source_addr)
{
  struct ssmpingd_sock *ss;
  ...
  zlog_info ("%s: starting ssmpingd for source %pPAs", __func__, &source_addr);

  ss = ssmpingd_new (pim, source_addr);
  if (! ss)
    {
      zlog_warn ("%s: ssmpingd_new() failure for source %pPAs", __func__,
                 &source_addr);
      return -1;
    }
...
int
pim_ssmpingd_stop (struct pim_instance *pim, pim_addr source_addr)
{
  struct ssmpingd_sock *ss;
  ...
  if (! ss)
    {
      zlog_warn ("%s: could not find ssmpingd for source %pPAs", __func__,
                 &source_addr);
      return -1;
    }

  zlog_info ("%s: stopping ssmpingd for source %pPAs", __func__, &source_addr);
