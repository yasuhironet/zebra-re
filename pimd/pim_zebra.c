...
static int
pim_router_id_update_zebra (ZAPI_CALLBACK_ARGS)
{
...
static int
pim_zebra_interface_vrf_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (PIM_DEBUG_ZEBRA)
    zlog_debug ("%s: %s updating from %u to %u", __func__, ifp->name, vrf_id,
                new_vrf_id);
...
static void
dump_if_address (struct interface *ifp)
{
  struct connected *ifc;
  struct listnode *node;

  zlog_debug ("%s %s: interface %s addresses:", __FILE__, __func__, ifp->name);
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, ifc))
    {
      ...
      zlog_debug ("%s %s: interface %s address %pI4 %s", __FILE__, __func__,
                  ifp->name, &p->u.prefix4,
...
static int
pim_zebra_if_address_add (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
  struct prefix *p;
  struct pim_interface *pim_ifp;
  ...
  */
  ...
  if (PIM_DEBUG_ZEBRA)
    {
      zlog_debug ("%s: %s(%u) connected IP address %pFX flags %u %s", __func__,
                  c->ifp->name, vrf_id, p, c->flags,
                  CHECK_FLAG (c->flags, ZEBRA_IFA_SECONDARY) ? "secondary"
                                                             : "primary");
  ...
  if (p->family != PIM_AF)
  ...
  else if (! CHECK_FLAG (c->flags, ZEBRA_IFA_SECONDARY))
    {
      ...
      if (pim_addr_cmp (primary_addr, addr))
        {
          if (PIM_DEBUG_ZEBRA)
            zlog_warn ("%s: %s : forcing secondary flag on %pFX", __func__,
                       c->ifp->name, p);
          SET_FLAG (c->flags, ZEBRA_IFA_SECONDARY);
        }
    }
...
static int
pim_zebra_if_address_del (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
  struct prefix *p;
  struct vrf *vrf = vrf_lookup_by_id (vrf_id);
...
void
pim_zebra_update_all_interfaces (struct pim_instance *pim)
{
  struct interface *ifp;
...
void
pim_zebra_upstream_rpf_changed (struct pim_instance *pim,
...
pim_zebra_vxlan_sg_proc (ZAPI_CALLBACK_ARGS)
{
  struct stream *s;
  struct pim_instance *pim;
  pim_sgaddr sg;
...
void
pim_scan_oil (struct pim_instance *pim)
{
...
static void
on_rpf_cache_refresh (struct event *t)
{
...
void
sched_rpf_cache_refresh (struct pim_instance *pim)
{
...
static void
pim_zebra_capabilities (struct zclient_capabilities *cap)
{
...
void
pim_forward_start (struct pim_ifchannel *ch)
{
  ...
  pim_channel_add_oif (up->channel_oil, ch->interface, mask, __func__);
}

void
pim_forward_stop (struct pim_ifchannel *ch)
{
  struct pim_upstream *up = ch->upstream;

  if (PIM_DEBUG_PIM_TRACE)
    {
      zlog_debug ("%s: (S,G)=%s oif=%s installed: %d", __func__, ch->sg_str,
...
void
pim_zebra_interface_set_master (struct interface *vrf, struct interface *ifp)
{
