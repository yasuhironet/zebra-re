...
void
igmp_anysource_forward_start (struct pim_instance *pim, struct gm_group *group)
{
...
void
igmp_anysource_forward_stop (struct gm_group *group)
{
  struct gm_source *source;
...
static void
igmp_source_forward_reevaluate_one (struct pim_instance *pim,
                                    struct gm_source *source, int is_grp_ssm)
{
  pim_sgaddr sg;
  ...
  if (IGMP_SOURCE_TEST_FORWARDING (source->source_flags))
      ...
      if (pim_addr_is_any (source->source_addr))
      ...
      else
        {
          if (! is_grp_ssm)
            {
              if (PIM_DEBUG_PIM_EVENTS)
                zlog_debug ("local membership del for %pSG as G is now ASM",
                            &sg);
              igmp_source_forward_stop (source);
            }
        }
  ...
  else
    {
      if (! pim_addr_is_any (source->source_addr) && (is_grp_ssm))
        {
          if (PIM_DEBUG_PIM_EVENTS)
            zlog_debug ("local membership add for %pSG as G is now SSM", &sg);
          igmp_source_forward_start (pim, source);
        }
    }
}
...
void
igmp_source_forward_reevaluate_all (struct pim_instance *pim)
{
  ...
  FOR_ALL_INTERFACES (pim->vrf, ifp)
    {
      ...
      /* scan igmp groups */
      for (ALL_LIST_ELEMENTS (pim_ifp->gm_group_list, grpnode, grp_nextnode,
                              grp))
        {
          ...
          if (is_grp_ssm && grp->group_filtermode_isexcl)
          ...
          else
            {
              ...
              for (ALL_LIST_ELEMENTS_RO (grp->group_source_list, srcnode, src))
                {
                  igmp_source_forward_reevaluate_one (pim, src, is_grp_ssm);
                } /* scan group sources */
            }
        } /* scan igmp groups */

      RB_FOREACH_SAFE (ch, pim_ifchannel_rb, &pim_ifp->ifchannel_rb, ch_temp)
        {
...
void
igmp_source_forward_start (struct pim_instance *pim, struct gm_source *source)
{
  ...
  if (tib_sg_gm_join (pim, sg, group->interface, &source->source_channel_oil))
 ...
 */
void
igmp_source_forward_stop (struct gm_source *source)
{
 ...
 */
static int
igmp_sock_open (struct in_addr ifaddr, struct interface *ifp)
{
  int fd;
  int join = 0;
  ...
  */
  ...
  if (inet_aton (PIM_ALL_IGMP_ROUTERS, &group))
  ...
  else
    {
      zlog_warn (
          "%s %s: IGMP socket fd=%d interface %pI4: could not solve %s to group address: errno=%d: %s",
          __FILE__, __func__, fd, &ifaddr, PIM_ALL_IGMP_ROUTERS, errno,
          safe_strerror (errno));
    }
...
static void
igmp_sock_dump (array_t *igmp_sock_array)
{
  ...
  for (int i = 0; i < size; ++i)
    {
      ...
      zlog_debug ("%s %s: [%d/%d] igmp_addr=%pI4 fd=%d", __FILE__, __func__, i,
                  size, &igmp->ifaddr, igmp->fd);
    }
}
#endif

struct gm_sock *
pim_igmp_sock_lookup_ifaddr (struct list *igmp_sock_list,
...
static void
pim_igmp_other_querier_expire (struct event *t)
{
  struct gm_sock *igmp;
...
void
pim_igmp_other_querier_timer_on (struct gm_sock *igmp)
{
  ...
  */
  ...
  event_add_timer_msec (router->master, pim_igmp_other_querier_expire, igmp,
                        other_querier_present_interval_msec,
                        &igmp->t_other_querier_timer);
}

void
pim_igmp_other_querier_timer_off (struct gm_sock *igmp)
{
  assert (igmp);
...
int
igmp_validate_checksum (char *igmp_msg, int igmp_msg_len)
{
  ...
  if (ntohs (checksum) != recv_checksum)
    {
      zlog_warn ("Invalid checksum received %x, calculated %x", recv_checksum,
                 ntohs (checksum));
      return -1;
    }
...
static int
igmp_recv_query (struct gm_sock *igmp, int query_version, int max_resp_code,
                 struct in_addr from, const char *from_str, char *igmp_msg,
                 int igmp_msg_len)
{
  struct interface *ifp;
  struct pim_interface *pim_ifp;
  ...
  if (igmp_validate_checksum (igmp_msg, igmp_msg_len) == -1)
    {
      zlog_warn ("Recv IGMP query v%d from %s on %s with invalid checksum",
  ...
  if (! pim_if_connected_to_source (ifp, from))
    {
      if (PIM_DEBUG_GM_PACKETS)
        zlog_debug (
            "Recv IGMP query on interface: %s from a non-connected source: %s",
            ifp->name, from_str);
      return 0;
    }
  ...
  if (ntohl (from.s_addr) < ntohl (igmp->ifaddr.s_addr))
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (pim_ifp->gm_group_list, grpnode, group))
        {
      ...
      if (PIM_DEBUG_GM_TRACE)
        {
          ...
          zlog_debug (
              ...
              ifp->name, ifaddr_str, ntohl (igmp->ifaddr.s_addr), from_str,
              ntohl (from.s_addr));
        }
...
static void
on_trace (const char *label, struct interface *ifp, struct in_addr from)
{
  if (PIM_DEBUG_GM_TRACE)
    {
...
static int
igmp_v1_recv_report (struct gm_sock *igmp, struct in_addr from,
                     const char *from_str, char *igmp_msg, int igmp_msg_len)
{
  ...
  if (igmp_validate_checksum (igmp_msg, igmp_msg_len) == -1)
    {
      zlog_warn ("Recv IGMP report v1 from %s on %s with invalid checksum",
                 from_str, ifp->name);
      return -1;
    }
...
bool
pim_igmp_verify_header (struct ip *ip_hdr, size_t len, size_t *hlen)
{
  char *igmp_msg;
  int igmp_msg_len;
  int msg_type;
  ...
  *hlen = ip_hlen;
  ...
  if (igmp_msg_len < PIM_IGMP_MIN_LEN)
    {
      zlog_warn ("IGMP message size=%d shorter than minimum=%d", igmp_msg_len,
                 PIM_IGMP_MIN_LEN);
      return false;
    }
...
int
pim_igmp_packet (struct gm_sock *igmp, char *buf, size_t len)
{
  ...
  switch (msg_type)
    ...
    case PIM_IGMP_V3_MEMBERSHIP_REPORT:
      return igmp_v3_recv_report (igmp, ip_hdr->ip_src, from_str, igmp_msg,
                                  igmp_msg_len);

    case PIM_IGMP_V2_MEMBERSHIP_REPORT:
      return igmp_v2_recv_report (igmp, ip_hdr->ip_src, from_str, igmp_msg,
                                  igmp_msg_len);

    case PIM_IGMP_V1_MEMBERSHIP_REPORT:
      return igmp_v1_recv_report (igmp, ip_hdr->ip_src, from_str, igmp_msg,
                                  igmp_msg_len);
...
void
pim_igmp_general_query_on (struct gm_sock *igmp)
{
  struct pim_interface *pim_ifp;
  int startup_mode;
  int query_interval;
  ...
  if (startup_mode)
    {
      ...
      if (igmp->startup_query_count == igmp->querier_robustness_variable)
        ...
        query_interval = PIM_IGMP_SQI (pim_ifp->gm_default_query_interval);
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      zlog_debug (
          ...
          ifaddr_str, query_interval, startup_mode ? "startup" : "non-startup",
          igmp->fd);
    }
...
void
pim_igmp_general_query_off (struct gm_sock *igmp)
{
  assert (igmp);
...
static void
pim_igmp_general_query (struct event *t)
{
  struct gm_sock *igmp;
...
static void
sock_close (struct gm_sock *igmp)
{
  ...
  if (close (igmp->fd))
    {
      flog_err (
          ...
          &igmp->ifaddr, igmp->fd, igmp->interface->name, errno,
          safe_strerror (errno));
    }
...
void
igmp_startup_mode_on (struct gm_sock *igmp)
{
  struct pim_interface *pim_ifp;
...
static void
igmp_group_free (struct gm_group *group)
{
...
static void
igmp_group_count_incr (struct pim_interface *pim_ifp)
{
  ...
  if (pim_ifp->pim->gm_group_count == pim_ifp->pim->gm_watermark_limit)
    {
      zlog_warn ("IGMP group count reached watermark limit: %u(vrf: %s)",
                 pim_ifp->pim->gm_group_count,
...
static void
igmp_group_count_decr (struct pim_interface *pim_ifp)
{
...
void
igmp_group_delete (struct gm_group *group)
{
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      pim_inet4_dump ("<group?>", group->group_addr, group_str,
                      sizeof (group_str));
      zlog_debug ("Deleting IGMP group %s from interface %s", group_str,
...
void
igmp_group_delete_empty_include (struct gm_group *group)
{
  assert (! group->group_filtermode_isexcl);
...
void
igmp_sock_free (struct gm_sock *igmp)
{
...
void
igmp_sock_delete (struct gm_sock *igmp)
{
  struct pim_interface *pim_ifp;
...
igmp_sock_delete_all (struct interface *ifp)
{
  ...
  for (ALL_LIST_ELEMENTS (pim_ifp->gm_socket_list, igmp_node, igmp_nextnode,
                          igmp))
    {
...
static unsigned int
igmp_group_hash_key (const void *arg)
{
...
void
pim_igmp_if_init (struct pim_interface *pim_ifp, struct interface *ifp)
{
  char hash_name[64];
  ...
  pim_ifp->gm_group_hash =
      hash_create (igmp_group_hash_key, igmp_group_hash_equal, hash_name);
}

void
pim_igmp_if_reset (struct pim_interface *pim_ifp)
{
  struct listnode *grp_node, *grp_nextnode;
  struct gm_group *grp;

  for (ALL_LIST_ELEMENTS (pim_ifp->gm_group_list, grp_node, grp_nextnode, grp))
    {
      igmp_group_delete (grp);
    }
}
...
void
pim_igmp_if_fini (struct pim_interface *pim_ifp)
{
  pim_igmp_if_reset (pim_ifp);
...
static struct gm_sock *
igmp_sock_new (int fd, struct in_addr ifaddr, struct interface *ifp,
               int mtrace_only)
{
  struct pim_interface *pim_ifp;
  struct gm_sock *igmp;
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      zlog_debug (
          "Creating IGMP socket fd=%d for address %pI4 on interface %s", fd,
          &ifaddr, ifp->name);
    }
...
static void
igmp_read_on (struct gm_sock *igmp)
{
...
struct gm_sock *
pim_igmp_sock_add (struct list *igmp_sock_list, struct in_addr ifaddr,
                   struct interface *ifp, bool mtrace_only)
{
  ...
  if (fd < 0)
    {
      zlog_warn ("Could not open IGMP socket for %pI4 on %s", &ifaddr,
                 ifp->name);
      return NULL;
    }
  ...
  if (bind (fd, (struct sockaddr *) &sin, sizeof (sin)) != 0)
    {
      zlog_warn ("Could not bind IGMP socket for %pI4 on %s: %s(%d)", &ifaddr,
                 ifp->name, strerror (errno), errno);
      close (fd);
 ...
 */
static void
igmp_group_timer (struct event *t)
{
  struct gm_group *group;
...
static void
group_timer_off (struct gm_group *group)
{
  ...
  if (PIM_DEBUG_GM_TRACE)
    {
      ...
      pim_inet4_dump ("<group?>", group->group_addr, group_str,
                      sizeof (group_str));
      zlog_debug ("Cancelling TIMER event for group %s on %s", group_str,
                  group->interface->name);
    }
...
void
igmp_group_timer_on (struct gm_group *group, long interval_msec,
                     const char *ifname)
{
  group_timer_off (group);
  ...
  */
  ...
  event_add_timer_msec (router->master, igmp_group_timer, group, interval_msec,
                        &group->t_group_timer);
}

struct gm_group *
find_group_by_addr (struct gm_sock *igmp, struct in_addr group_addr)
{
...
struct gm_group *
igmp_add_group_by_addr (struct gm_sock *igmp, struct in_addr group_addr)
{
  struct gm_group *group;
  struct pim_interface *pim_ifp = igmp->interface->info;
  ...
  if (! pim_is_group_224_4 (group_addr))
    {
      zlog_warn ("%s: Group Specified is not part of 224.0.0.0/4", __func__);
      return NULL;
    }
...
void
igmp_send_query (int igmp_version, struct gm_group *group, char *query_buf,
                 int query_buf_size, int num_sources, struct in_addr dst_addr,
                 struct in_addr group_addr, int query_max_response_time_dsec,
                 uint8_t s_flag, struct gm_sock *igmp)
{
  ...
  if (igmp_version == 3)
    {
      igmp_v3_send_query (group, igmp->fd, igmp->interface->name, query_buf,
    ...
    {
      igmp_v2_send_query (group, igmp->fd, igmp->interface->name, query_buf,
                          dst_addr, group_addr, query_max_response_time_dsec);
    }
}

void
igmp_send_query_on_intf (struct interface *ifp, int igmp_ver)
{
  struct pim_interface *pim_ifp = ifp->info;
  ...
  for (ALL_LIST_ELEMENTS_RO (pim_ifp->gm_socket_list, sock_node, igmp))
    {
      ...
      igmp_send_query (igmp_ver, 0 /* igmp_group */, query_buf,
                       sizeof (query_buf), 0 /* num_sources */, dst_addr,
                       group_addr, pim_ifp->gm_query_max_response_time_dsec,
                       1 /* s_flag: always set for general queries */, igmp);
    }
