...
void
igmp_v2_send_query (struct gm_group *group, int fd, const char *ifname,
  ...
  if (PIM_DEBUG_GM_PACKETS)
    {
      ...
      zlog_debug ("Send IGMPv2 QUERY to %s on %s for group %s", dst_str,
                  ifname, group_str);
    }
...
int
igmp_v2_recv_report (struct gm_sock *igmp, struct in_addr from,
...
int
igmp_v2_recv_leave (struct gm_sock *igmp, struct ip *ip_hdr,
                    const char *from_str, char *igmp_msg, int igmp_msg_len)
{
  struct interface *ifp = igmp->interface;
  struct in_addr group_addr;
  ...
  if (igmp_validate_checksum (igmp_msg, igmp_msg_len) == -1)
    {
      zlog_warn ("Recv IGMPv2 LEAVE from %s on %s with invalid checksum",
                 from_str, ifp->name);
      return -1;
    }
