...
void
pim_ifassert_winner_set (struct pim_ifchannel *ch,
                         ...
                         struct pim_assert_metric winner_metric)
{
  ...
  int metric_changed =
      ! pim_assert_metric_match (&ch->ifassert_winner_metric, &winner_metric);
      ...
      if (ch->ifassert_state != new_state)
        {
          zlog_debug (
              ...
              pim_ifchannel_ifassert_name (new_state), ch->interface->name);
        }

      if (winner_changed)
        zlog_debug (
            "%s: (S,G)=%s assert winner changed from %pPAs to %pPAs on interface %s",
            __func__, ch->sg_str, &ch->ifassert_winner, &winner,
            ch->interface->name);
      ...
      if (winner_changed)
        {
          ...
          rpf_result =
              pim_rpf_update (pim_ifp->pim, ch->upstream, &old_rpf, __func__);
          if (rpf_result == PIM_RPF_CHANGED ||
              (rpf_result == PIM_RPF_FAILURE &&
               old_rpf.source_nexthop.interface))
            pim_zebra_upstream_rpf_changed (pim_ifp->pim, ch->upstream,
                                            &old_rpf);
          /* update kernel multicast forwarding cache (MFC) */
          if (ch->upstream->rpf.source_nexthop.interface &&
              ch->upstream->channel_oil)
            pim_upstream_mroute_iif_update (ch->upstream->channel_oil,
                                            __func__);
        }
...
static void
on_trace (const char *label, struct interface *ifp, pim_addr src)
{
...
static int
preferred_assert (const struct pim_ifchannel *ch,
...
static int
acceptable_assert (const struct pim_assert_metric *my_metric,
                   const struct pim_assert_metric *recv_metric)
{
...
static int
inferior_assert (const struct pim_assert_metric *my_metric,
                 const struct pim_assert_metric *recv_metric)
{
...
static int
cancel_assert (const struct pim_assert_metric *recv_metric)
{
...
static void
if_could_assert_do_a1 (const char *caller, struct pim_ifchannel *ch)
{
      ...
      if (assert_action_a1 (ch))
        {
          zlog_warn (
              ...
              __func__, caller, ch->sg_str, ch->interface->name);
          /* log warning only */
        }
    }
}
...
static int
dispatch_assert (struct interface *ifp, pim_addr source_addr,
                 pim_addr group_addr, struct pim_assert_metric recv_metric)
{
  ...
  switch (ch->ifassert_state)
    ...
    case PIM_IFASSERT_I_AM_LOSER:
      if (! pim_addr_cmp (recv_metric.ip_address, ch->ifassert_winner))
        {
...
int
pim_assert_recv (struct interface *ifp, struct pim_neighbor *neigh,
                 pim_addr src_addr, uint8_t *buf, int buf_size)
{
  ...
  if (offset < 1)
    {
      zlog_warn ("%s: pim_parse_addr_group() failure: from %pPAs on %s",
  ...
  */
  ...
  if (offset < 1 || wrong_af)
    {
      zlog_warn ("%s: pim_parse_addr_ucast() failure: from %pPAs on %s",
                 __func__, &src_addr, ifp->name);
      return -2;
    }
  curr += offset;
  ...
  */
  ...
  msg_metric.rpt_bit_flag =
      msg_metric.metric_preference & 0x80000000; /* save highest bit */
...
*/
int
pim_assert_metric_better (const struct pim_assert_metric *m1,
...
int
pim_assert_metric_match (const struct pim_assert_metric *m1,
                         const struct pim_assert_metric *m2)
{
  if (m1->rpt_bit_flag != m2->rpt_bit_flag)
    return 0;
...
int
pim_assert_build_msg (uint8_t *pim_msg, int buf_size, struct interface *ifp,
                      ...
                      uint32_t rpt_bit_flag)
{
  ...
  if (! pim_msg_curr)
    {
      zlog_warn ("%s: failure encoding group address %pPA: space left=%d",
  ...
  if (! pim_msg_curr)
    {
      zlog_warn ("%s: failure encoding source address %pPA: space left=%d",
                 __func__, &source_addr, remain);
      return -2;
    }

  /* Metric preference */
  pim_write_uint32 (pim_msg_curr, rpt_bit_flag ? metric_preference | 0x80000000
                                               : metric_preference);
  pim_msg_curr += 4;
  ...
  */
  ...
  pim_msg_build_header (pim_ifp->primary_address, qpim_all_pim_routers_addr,
                        pim_msg, pim_msg_size, PIM_MSG_TYPE_ASSERT, false);

  return pim_msg_size;
}

static int
pim_assert_do (struct pim_ifchannel *ch, struct pim_assert_metric metric)
{
  ...
  if (! ifp)
    {
      if (PIM_DEBUG_PIM_TRACE)
        zlog_debug ("%s: channel%s has no associated interface!", __func__,
                    ch->sg_str);
      return -1;
    }
  pim_ifp = ifp->info;
  ...
  pim_msg_size = pim_assert_build_msg (
      pim_msg, sizeof (pim_msg), ifp, ch->sg.grp, ch->sg.src,
      metric.metric_preference, metric.route_metric, metric.rpt_bit_flag);
  if (pim_msg_size < 1)
    {
  ...
  */
  ...
  if (PIM_DEBUG_PIM_TRACE)
    {
      zlog_debug ("%s: to %s: (S,G)=%s pref=%u metric=%u rpt_bit=%u", __func__,
  ...
  if (pim_msg_send (pim_ifp->pim_sock_fd, pim_ifp->primary_address,
                    qpim_all_pim_routers_addr, pim_msg, pim_msg_size, ifp))
    {
...
int
pim_assert_send (struct pim_ifchannel *ch)
{
 ...
 */
static int
pim_assert_cancel (struct pim_ifchannel *ch)
{
...
static void
on_assert_timer (struct event *t)
{
  struct pim_ifchannel *ch;
  struct interface *ifp;
  ...
  if (PIM_DEBUG_PIM_TRACE)
    {
      zlog_debug ("%s: (S,G)=%s timer expired on interface %s", __func__,
                  ch->sg_str, ifp->name);
    }
...
static void
assert_timer_off (struct pim_ifchannel *ch)
{
  if (PIM_DEBUG_PIM_TRACE)
    {
...
static void
pim_assert_timer_set (struct pim_ifchannel *ch, int interval)
{
  assert_timer_off (ch);
...
static void
pim_assert_timer_reset (struct pim_ifchannel *ch)
{
...
*/
int
assert_action_a1 (struct pim_ifchannel *ch)
{
...
*/
static void
assert_action_a2 (struct pim_ifchannel *ch,
                  struct pim_assert_metric winner_metric)
{
...
*/
static int
assert_action_a3 (struct pim_ifchannel *ch)
{
  if (ch->ifassert_state != PIM_IFASSERT_I_AM_WINNER)
    {
      if (PIM_DEBUG_PIM_EVENTS)
...
*/
void
assert_action_a4 (struct pim_ifchannel *ch)
{
...
*/
void
assert_action_a5 (struct pim_ifchannel *ch)
{
...
*/
static void
assert_action_a6 (struct pim_ifchannel *ch,
                  struct pim_assert_metric winner_metric)
{
