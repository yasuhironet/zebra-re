...
ospf_apiclient_connect (char *host, int syncport)
{
  ...
  ret = setsockopt (async_server_sock, SOL_SOCKET, SO_REUSEADDR, (void *) &on,
                    sizeof (on));
  if (ret < 0)
    {
...
#ifdef SO_REUSEPORT
  ret = setsockopt (async_server_sock, SOL_SOCKET, SO_REUSEPORT, (void *) &on,
                    sizeof (on));
  if (ret < 0)
    {
 ...
 */
int
ospf_apiclient_register_opaque_type (struct ospf_apiclient *cl, uint8_t ltype,
                                     uint8_t otype)
{
  struct msg *msg;
  int rc;
  ...
  msg =
      new_msg_register_opaque_type (ospf_apiclient_get_seqnr (), ltype, otype);
  if (! msg)
    {
...
int
ospf_apiclient_lsa_delete (struct ospf_apiclient *oclient, struct in_addr addr,
                           uint8_t lsa_type, uint8_t opaque_type,
