...
lsa_inject (struct event *t)
{
  ...
  rc =
      ospf_apiclient_lsa_originate (cl, ifaddr, area_id, lsa_type, opaque_type,
                                    opaque_id, opaquedata, opaquelen);
...
lsa_update_callback (struct in_addr ifaddr, struct in_addr area_id,
                     uint8_t is_self_originated, struct lsa_header *lsa)
{
...
static void
ready_callback (uint8_t lsa_type, uint8_t opaque_type, struct in_addr addr)
{
...
{
  printf (
      "Usage: ospfclient <ospfd> <lsatype> <opaquetype> <opaqueid> <ifaddr> <areaid>\n");
  printf (
      "where ospfd     : router where API-enabled OSPF daemon is running\n");
  printf (
      "      lsatype   : either 9, 10, or 11 depending on flooding scope\n");
  printf (
      "      opaquetype: 0-255 (e.g., experimental applications use > 128)\n");
  printf ("      opaqueid  : arbitrary application instance (24 bits)\n");
  printf (
      "      ifaddr    : interface IP address (for type 9) otherwise ignored\n");
  printf (
      "      areaid    : area in IP address format (for type 10) otherwise ignored\n");
...
main (int argc, char *argv[])
{
  ...
  ospf_apiclient_register_callback (oclient, ready_callback, new_if_callback,
                                    del_if_callback, ism_change_callback,
                                    nsm_change_callback, lsa_update_callback,
                                    lsa_delete_callback);
