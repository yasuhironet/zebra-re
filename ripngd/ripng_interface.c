...
static int
ripng_multicast_join (struct interface *ifp, int sock)
{
  int ret;
  ...
  if (if_is_multicast (ifp))
    {
      ...
      if (ret < 0 && save_errno == EADDRINUSE)
        {
           ...
           */
          zlog_warn ("ripng join on %s EADDRINUSE (ignoring)", ifp->name);
      ...
      if (IS_RIPNG_DEBUG_EVENT)
        zlog_debug ("RIPng %s join to all-rip-routers multicast group",
                    ifp->name);
...
static int
ripng_multicast_leave (struct interface *ifp, int sock)
{
  int ret;
  struct ipv6_mreq mreq;
...
static int
ripng_ifp_up (struct interface *ifp)
{
...
static int
ripng_ifp_down (struct interface *ifp)
{
  ripng_interface_sync (ifp);
  ripng_if_down (ifp);
...
static int
ripng_ifp_create (struct interface *ifp)
{
  ripng_interface_sync (ifp);
...
static int
ripng_ifp_destroy (struct interface *ifp)
{
  ripng_interface_sync (ifp);
  if (if_is_up (ifp))
    {
...
int
ripng_interface_vrf_update (ZAPI_CALLBACK_ARGS)
{
...
void
ripng_interface_clean (struct ripng *ripng)
{
  struct interface *ifp;
  struct ripng_interface *ri;
...
ripng_apply_address_add (struct connected *ifc)
{
  ...
  if ((ripng_enable_if_lookup (ripng, ifc->ifp->name) >= 0) ||
    ...
    ripng_redistribute_add (ripng, ZEBRA_ROUTE_CONNECT, RIPNG_ROUTE_INTERFACE,
                            &address, ifc->ifp->ifindex, NULL, 0);
}

int
ripng_interface_address_add (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
  struct prefix *p;
...
ripng_apply_address_del (struct connected *ifc)
{
  ...
  ripng_redistribute_delete (ripng, ZEBRA_ROUTE_CONNECT, RIPNG_ROUTE_INTERFACE,
                             &address, ifc->ifp->ifindex);
}

int
ripng_interface_address_delete (ZAPI_CALLBACK_ARGS)
{
  ...
  if (ifc)
    {
      ...
      if (p->family == AF_INET6)
        {
          if (IS_RIPNG_DEBUG_ZEBRA)
            zlog_debug ("RIPng connected address %pFX delete", p);
  ...
  if (p->family == AF_INET6)
    {
      ...
      node =
          agg_node_match (ripng->enable_network, (struct prefix *) &address);
...
int
ripng_enable_network_add (struct ripng *ripng, struct prefix *p)
{
  struct agg_node *node;
...
int
ripng_enable_network_delete (struct ripng *ripng, struct prefix *p)
{
  struct agg_node *node;
...
/* Lookup function. */
static int
ripng_enable_if_lookup (struct ripng *ripng, const char *ifname)
{
  unsigned int i;
  char *str;
...
int
ripng_enable_if_add (struct ripng *ripng, const char *ifname)
{
  int ret;
...
int
ripng_enable_if_delete (struct ripng *ripng, const char *ifname)
{
  int index;
  char *str;
...
/* Wake up interface. */
static void
ripng_interface_wakeup (struct event *t)
{
  struct interface *ifp;
  struct ripng_interface *ri;
      ...
      if (set)
        {
          ...
          if ((ripng_enable_if_lookup (ripng, connected->ifp->name) >= 0) ||
      ...
      else
        {
          ...
          if (ripng_redistribute_check (ripng, ZEBRA_ROUTE_CONNECT))
            ripng_redistribute_add (ripng, ZEBRA_ROUTE_CONNECT,
  ...
  if (ri->enable_network || ri->enable_interface)
    {
      ...
      event_add_timer (master, ripng_interface_wakeup, ifp, 1, &ri->t_wakeup);
...
static void
ripng_enable_apply_all (struct ripng *ripng)
{
  struct interface *ifp;
...
void
ripng_clean_network (struct ripng *ripng)
{
  unsigned int i;
  char *str;
  struct agg_node *rn;
...
static int
ripng_passive_interface_lookup (struct ripng *ripng, const char *ifname)
{
  unsigned int i;
  char *str;
...
static void
ripng_passive_interface_apply_all (struct ripng *ripng)
{
  struct interface *ifp;
...
/* Passive interface. */
int
ripng_passive_interface_set (struct ripng *ripng, const char *ifname)
{
...
int
ripng_passive_interface_unset (struct ripng *ripng, const char *ifname)
{
  int i;
  char *str;
...
void
ripng_passive_interface_clean (struct ripng *ripng)
{
  unsigned int i;
  char *str;
...
int
ripng_network_write (struct vty *vty, struct ripng *ripng)
{
  unsigned int i;
  const char *ifname;
  struct agg_node *node;
...
void
ripng_interface_sync (struct interface *ifp)
{
  struct ripng_interface *ri;
