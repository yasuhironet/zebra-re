...
static void
ripng_zebra_ipv6_send (struct ripng *ripng, struct agg_node *rp, uint8_t cmd)
{
...
void
ripng_zebra_ipv6_add (struct ripng *ripng, struct agg_node *rp)
{
...
void
ripng_zebra_ipv6_delete (struct ripng *ripng, struct agg_node *rp)
{
...
static int
ripng_zebra_read_route (ZAPI_CALLBACK_ARGS)
{
  struct ripng *ripng;
  struct zapi_route api;
...
void
ripng_redistribute_conf_update (struct ripng *ripng, int type)
{
...
void
ripng_redistribute_conf_delete (struct ripng *ripng, int type)
{
  if (zclient->sock > 0)
...
int
ripng_redistribute_check (struct ripng *ripng, int type)
{
  return ripng->redist[type].enabled;
}

void
ripng_redistribute_enable (struct ripng *ripng)
{
  for (int i = 0; i < ZEBRA_ROUTE_MAX; i++)
    {
      ...
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient, AFI_IP6, i, 0,
                               ripng->vrf->vrf_id);
    }
}

void
ripng_redistribute_disable (struct ripng *ripng)
{
  for (int i = 0; i < ZEBRA_ROUTE_MAX; i++)
    {
      if (! ripng_redistribute_check (ripng, i))
      ...
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE, zclient, AFI_IP6, i,
                               0, ripng->vrf->vrf_id);
    }
}

void
ripng_redistribute_write (struct vty *vty, struct ripng *ripng)
{
  int i;
...
void
ripng_zebra_vrf_register (struct vrf *vrf)
{
...
void
ripng_zebra_vrf_deregister (struct vrf *vrf)
{
  if (vrf->vrf_id == VRF_DEFAULT)
    return;
...
static void
ripng_zebra_capabilities (struct zclient_capabilities *cap)
{
...
void
zebra_init (struct event_loop *master)
{
