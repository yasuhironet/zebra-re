...
static inline int
ripng_instance_compare (const struct ripng *a, const struct ripng *b)
{
...
struct ripng *
ripng_info_get_instance (const struct ripng_info *rinfo)
{
...
/* Create ripng socket. */
int
ripng_make_socket (struct vrf *vrf)
{
  int ret;
  int sock;
  ...
  frr_with_privs (&ripngd_privs)
    {
      sock =
          vrf_socket (AF_INET6, SOCK_DGRAM, IPPROTO_UDP, vrf->vrf_id, vrf_dev);
      if (sock < 0)
        {
                   ...
                   struct interface *ifp)
{
  ...
  if (ret < 0)
    {
      if (to)
        flog_err_sys (EC_LIB_SOCKET, "RIPng send fail on %s to %pI6: %s",
                      ifp->name, &to->sin6_addr, safe_strerror (errno));
      else
        flog_err_sys (EC_LIB_SOCKET, "RIPng send fail on %s: %s", ifp->name,
                      safe_strerror (errno));
    }
...
static int
ripng_recv_packet (int sock, uint8_t *buf, int bufsize,
  ...
  for (lim = (caddr_t) packet + size; (caddr_t) rte < lim; rte++)
    {
      if (rte->metric == RIPNG_METRIC_NEXTHOP)
        zlog_debug ("  nexthop %pI6/%d", &rte->addr, rte->prefixlen);
      else
        zlog_debug ("  %pI6/%d metric %d tag %" ROUTE_TAG_PRI, &rte->addr,
                    rte->prefixlen, rte->metric,
...
static void
ripng_nexthop_rte (struct rte *rte, struct sockaddr_in6 *from,
                   struct ripng_nexthop *nexthop)
{
  ...
  if (IS_RIPNG_DEBUG_RECV)
    zlog_debug ("RIPng nexthop RTE address %pI6 tag %" ROUTE_TAG_PRI
                " prefixlen %d",
                &rte->addr, (route_tag_t) ntohs (rte->tag), rte->prefixlen);
  ...
  if (ntohs (rte->tag) != 0)
    zlog_warn ("RIPng nexthop RTE with non zero tag value %" ROUTE_TAG_PRI
               " from %pI6",
               (route_tag_t) ntohs (rte->tag), &from->sin6_addr);

  if (rte->prefixlen != 0)
    zlog_warn ("RIPng nexthop RTE with non zero prefixlen value %d from %pI6",
...
static void
ripng_garbage_collect (struct event *t)
{
...
static void ripng_timeout_update (struct ripng *ripng,
                                  struct ripng_info *rinfo);
 ...
 */
struct ripng_info *
ripng_ecmp_add (struct ripng *ripng, struct ripng_info *rinfo_new)
{
  ...
  if (listcount (list) && listcount (list) >= ripng->ecmp)
    {
      ...
      for (ALL_LIST_ELEMENTS (list, node, nnode, rinfo_exist))
        if (! from_highest ||
            (from_highest &&
             IPV6_ADDR_CMP (&rinfo_exist->from, &from_highest->from) > 0))
          {
...
void
ripng_ecmp_change (struct ripng *ripng)
{
  struct agg_node *rp;
  struct ripng_info *rinfo;
  struct list *list;
              ...
              for (ALL_LIST_ELEMENTS (list, node, nextnode, rinfo))
                {
                  if (! from_highest ||
                      (from_highest &&
                       IPV6_ADDR_CMP (&rinfo->from, &from_highest->from) > 0))
                    from_highest = rinfo;
                }
 ...
 */
struct ripng_info *
ripng_ecmp_replace (struct ripng *ripng, struct ripng_info *rinfo_new)
{
  struct agg_node *rp = rinfo_new->rp;
 ...
 */
struct ripng_info *
ripng_ecmp_delete (struct ripng *ripng, struct ripng_info *rinfo)
{
...
static void
ripng_timeout_update (struct ripng *ripng, struct ripng_info *rinfo)
{
  if (rinfo->metric != RIPNG_METRIC_INFINITY)
    {
      EVENT_OFF (rinfo->t_timeout);
      event_add_timer (master, ripng_timeout, rinfo, ripng->timeout_time,
                       &rinfo->t_timeout);
    }
}
              ...
              struct ripng_interface *ri)
{
  ...
  int distribute = ripng_distribute == RIPNG_FILTER_OUT ? DISTRIBUTE_V6_OUT
                                                        : DISTRIBUTE_V6_IN;
  ...
  if (ri->routemap[RIPNG_FILTER_IN])
    {
      ...
      if (ret == RMAP_DENYMATCH)
        {
          if (IS_RIPNG_DEBUG_PACKET)
            zlog_debug ("RIPng %pFX is filtered by route-map in", &p);
          return;
        }
...
void
ripng_redistribute_add (struct ripng *ripng, int type, int sub_type,
                        ...
                        struct in6_addr *nexthop, route_tag_t tag)
{
  ...
  if (IS_RIPNG_DEBUG_EVENT)
    {
      if (! nexthop)
        zlog_debug ("Redistribute new prefix %pFX on the interface %s", p,
                    ifindex2ifname (ifindex, ripng->vrf->vrf_id));
      else
        zlog_debug (
            "Redistribute new prefix %pFX with nexthop %pI6 on the interface %s",
            p, nexthop, ifindex2ifname (ifindex, ripng->vrf->vrf_id));
    }
...
void
ripng_redistribute_delete (struct ripng *ripng, int type, int sub_type,
                           struct prefix_ipv6 *p, ifindex_t ifindex)
{
  struct agg_node *rp;
  struct ripng_info *rinfo;
      ...
      if (list != NULL && listcount (list) != 0)
        {
          ...
          if (rinfo != NULL && rinfo->type == type &&
              rinfo->sub_type == sub_type && rinfo->ifindex == ifindex)
            {
...
void
ripng_redistribute_withdraw (struct ripng *ripng, int type)
{
  struct agg_node *rp;
  struct ripng_info *rinfo = NULL;
  struct list *list = NULL;
        ...
        if ((rinfo->type == type) &&
            (rinfo->sub_type != RIPNG_ROUTE_INTERFACE))
          {
            ...
            if (IS_RIPNG_DEBUG_EVENT)
              {
                struct prefix_ipv6 *p =
                    (struct prefix_ipv6 *) agg_node_get_prefix (rp);

                zlog_debug (
                    "Poisone %pFX on the interface %s [withdraw]", p,
                        ...
                        int hoplimit)
{
  ...
  if (ripng_lladdr_check (ifp, &from->sin6_addr))
    {
      zlog_warn ("RIPng packet comes from my own link local address %pI6",
                 &from->sin6_addr);
      ripng_peer_bad_packet (ripng, from);
      return;
    }
  ...
  if (hoplimit >= 0 && hoplimit != 255)
    {
      zlog_warn ("RIPng packet comes with non 255 hop count %d from %pI6",
  ...
  for (lim = ((caddr_t) packet) + size; (caddr_t) rte < lim; rte++)
    {
      ...
      if (IN6_IS_ADDR_MULTICAST (&rte->addr))
        {
          zlog_warn ("Destination prefix is a multicast address %pI6/%d [%d]",
      ...
      if (IN6_IS_ADDR_LINKLOCAL (&rte->addr))
        {
          zlog_warn ("Destination prefix is a link-local address %pI6/%d [%d]",
                     &rte->addr, rte->prefixlen, rte->metric);
          ripng_peer_bad_route (ripng, from);
          continue;
        }
      if (IN6_IS_ADDR_LOOPBACK (&rte->addr))
        {
          zlog_warn ("Destination prefix is a loopback address %pI6/%d [%d]",
                     &rte->addr, rte->prefixlen, rte->metric);
          ripng_peer_bad_route (ripng, from);
          continue;
        }
      ...
      if (rte->prefixlen > IPV6_MAX_BITLEN)
        {
          zlog_warn ("Invalid prefix length %pI6/%d from %pI6%%%s", &rte->addr,
                     rte->prefixlen, &from->sin6_addr, ifp->name);
          ripng_peer_bad_route (ripng, from);
          continue;
        }
      ...
      if (! (rte->metric >= 1 && rte->metric <= 16))
        {
          zlog_warn ("Invalid metric %d from %pI6%%%s", rte->metric,
...
static void
ripng_read (struct event *thread)
{
  ...
  if (len < 0)
    {
      zlog_warn ("RIPng recvfrom failed (VRF %s): %s.", ripng->vrf_name,
                 safe_strerror (errno));
      return;
    }
  ...
  if (((len - 4) % 20) != 0)
    {
      zlog_warn ("RIPng invalid packet size %d from %pI6 (VRF %s)", len,
                 &from.sin6_addr, ripng->vrf_name);
      ripng_peer_bad_packet (ripng, &from);
      return;
    }
  ...
  if (IS_RIPNG_DEBUG_EVENT)
    zlog_debug ("RIPng packet received from %pI6 port %d on %s (VRF %s)",
  ...
  if (ifp == NULL)
    {
      zlog_warn ("RIPng packet comes from unknown interface %d (VRF %s)",
...
static void
ripng_clear_changed_flag (struct ripng *ripng)
{
  struct agg_node *rp;
  struct ripng_info *rinfo = NULL;
  struct list *list = NULL;
...
static void
ripng_triggered_interval (struct event *t)
{
  struct ripng *ripng = EVENT_ARG (t);
...
void
ripng_triggered_update (struct event *t)
{
  struct ripng *ripng = EVENT_ARG (t);
  struct interface *ifp;
  struct ripng_interface *ri;
                      ...
                      int route_type)
{
  ...
  if (IS_RIPNG_DEBUG_EVENT)
    {
      if (to)
        zlog_debug ("RIPng update routes to neighbor %pI6", &to->sin6_addr);
      else
      ...
      if ((list = rp->info) != NULL &&
          ...
          rinfo->suppress == 0)
        {
          ...
          if (ri->routemap[RIPNG_FILTER_OUT])
            {
              ...
              if (ret == RMAP_DENYMATCH)
                {
                  if (IS_RIPNG_DEBUG_PACKET)
                    zlog_debug ("RIPng %pFX is filtered by route-map out", p);
                  continue;
                }
            }
          ...
          if (ripng->redist[rinfo->type].route_map.name)
            {
              ret = route_map_apply (ripng->redist[rinfo->type].route_map.map,
                                     (struct prefix *) p, rinfo);

              if (ret == RMAP_DENYMATCH)
                {
                  if (IS_RIPNG_DEBUG_PACKET)
                    zlog_debug ("RIPng %pFX is filtered by route-map", p);
                  continue;
                }
            }
          ...
          if (! rinfo->metric_set)
            {
              ...
              if (ripng->redist[rinfo->type].metric_config &&
                  rinfo->metric != RIPNG_METRIC_INFINITY)
                {
                  rinfo->metric_out = ripng->redist[rinfo->type].metric;
                }
              else
                {
          ...
          if (rinfo->metric_out != RIPNG_METRIC_INFINITY)
            ripng_offset_list_apply_out (ripng, p, ifp, &rinfo->metric_out);
...
struct ripng *
ripng_create (const char *vrf_name, struct vrf *vrf, int socket)
{
  struct ripng *ripng;
  ...
  ripng->update_time =
      yang_get_default_uint32 ("%s/timers/update-interval", RIPNG_INSTANCE);
  ripng->timeout_time =
      yang_get_default_uint32 ("%s/timers/holddown-interval", RIPNG_INSTANCE);
  ripng->garbage_time =
      yang_get_default_uint32 ("%s/timers/flush-interval", RIPNG_INSTANCE);
  ...
  ripng->offset_list_master->cmp = (int (*) (void *, void *)) offset_list_cmp;
  ripng->offset_list_master->del = (void (*) (void *)) ripng_offset_list_free;
  ripng->distribute_ctx = distribute_list_ctx_create (vrf);
  distribute_list_add_hook (ripng->distribute_ctx, ripng_distribute_update);
  distribute_list_delete_hook (ripng->distribute_ctx, ripng_distribute_update);
...
void
ripng_event (struct ripng *ripng, enum ripng_event event, int sock)
{
  int jitter = 0;
  ...
  switch (event)
    ...
    case RIPNG_TRIGGERED_UPDATE:
      if (ripng->t_triggered_interval)
        ...
        event_add_event (master, ripng_triggered_update, ripng, 0,
  ...
  for (rp = agg_route_top (ripng->table); rp; rp = agg_route_next (rp))
    {
      ...
      if ((list = rp->info) != NULL)
        for (ALL_LIST_ELEMENTS_RO (list, listnode, rinfo))
          {
#ifdef DEBUG
            vty_out (vty, "%c(%s) 0/%d %pRN ", zebra_route_char (rinfo->type),
                     ripng_route_subtype_print (rinfo), rinfo->suppress, rp);
#else
            vty_out (vty, "%c(%s) %pRN ", zebra_route_char (rinfo->type),
            ...
            vty_out (vty, " %2d  %3" ROUTE_TAG_PRI "  ", rinfo->metric,
...
void
ripng_ecmp_disable (struct ripng *ripng)
{
  struct agg_node *rp;
...
static void
ripng_distribute_update (struct distribute_ctx *ctx, struct distribute *dist)
{
  struct interface *ifp;
  struct ripng_interface *ri;
  struct access_list *alist;
...
void
ripng_clean (struct ripng *ripng)
{
...
static void
ripng_if_rmap_update (struct if_rmap_ctx *ctx, struct if_rmap *if_rmap)
{
...
static void
ripng_routemap_update_redistribute (struct ripng *ripng)
{
  for (int i = 0; i < ZEBRA_ROUTE_MAX; i++)
    {
      if (ripng->redist[i].route_map.name)
        {
          ripng->redist[i].route_map.map =
              route_map_lookup_by_name (ripng->redist[i].route_map.name);
          route_map_counter_increment (ripng->redist[i].route_map.map);
        }
    }
}
...
static void
ripng_vrf_link (struct ripng *ripng, struct vrf *vrf)
{
  struct interface *ifp;
...
static void
ripng_vrf_unlink (struct ripng *ripng, struct vrf *vrf)
{
  struct interface *ifp;
...
static void
ripng_instance_enable (struct ripng *ripng, struct vrf *vrf, int sock)
{
  ripng->sock = sock;
...
static void
ripng_instance_disable (struct ripng *ripng)
{
...
static int
ripng_vrf_delete (struct vrf *vrf)
{
  struct ripng *ripng;
...
static int
ripng_vrf_enable (struct vrf *vrf)
{
  struct ripng *ripng;
  int socket;
...
static int
ripng_vrf_disable (struct vrf *vrf)
{
  struct ripng *ripng;
