 ...
 */
static void
clear_ripng_route (struct ripng *ripng)
{
  struct agg_node *rp;

  if (IS_RIPNG_DEBUG_EVENT)
    zlog_debug ("Clearing all RIPng routes (VRF %s)", ripng->vrf_name);
...
int
clear_ripng_route_rpc (struct nb_cb_rpc_args *args)
{
  struct ripng *ripng;
  struct yang_data *yang_vrf;

  yang_vrf =
      yang_data_list_find (args->input, "%s/%s", args->xpath, "input/vrf");
  if (yang_vrf)
    {
