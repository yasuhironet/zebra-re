...
struct ripng_peer *
ripng_peer_lookup (struct ripng *ripng, struct in6_addr *addr)
{
...
struct ripng_peer *
ripng_peer_lookup_next (struct ripng *ripng, struct in6_addr *addr)
{
  struct ripng_peer *peer;
  struct listnode *node, *nnode;
 ...
 **/
static void
ripng_peer_timeout (struct event *t)
{
  struct ripng_peer *peer;
...
static struct ripng_peer *
ripng_peer_get (struct ripng *ripng, struct in6_addr *addr)
{
  struct ripng_peer *peer;
  ...
  event_add_timer (master, ripng_peer_timeout, peer, RIPNG_PEER_TIMER_DEFAULT,
                   &peer->t_timeout);
...
void
ripng_peer_update (struct ripng *ripng, struct sockaddr_in6 *from,
                   uint8_t version)
{
  struct ripng_peer *peer;
...
void
ripng_peer_bad_route (struct ripng *ripng, struct sockaddr_in6 *from)
{
  struct ripng_peer *peer;
  peer = ripng_peer_get (ripng, &from->sin6_addr);
  peer->recv_badroutes++;
...
void
ripng_peer_bad_packet (struct ripng *ripng, struct sockaddr_in6 *from)
{
  struct ripng_peer *peer;
  peer = ripng_peer_get (ripng, &from->sin6_addr);
...
void
ripng_peer_display (struct vty *vty, struct ripng *ripng)
{
  struct ripng_peer *peer;
  struct listnode *node, *nnode;
