  ...
  if (! no)
    {
      ...
      pbrms->dsfield = (pbrms->dsfield & ~PBR_DSFIELD_DSCP) | (rawDscp << 2);
    }
  else
    {
...
static void
pbrms_clear_set_vrf_config (struct pbr_map_sequence *pbrms)
{
...
static void
pbrms_clear_set_nhg_config (struct pbr_map_sequence *pbrms)
{
  if (pbrms->nhgrp_name)
...
static void
pbrms_clear_set_nexthop_config (struct pbr_map_sequence *pbrms)
{
  if (pbrms->nhg)
...
static void
pbrms_clear_set_config (struct pbr_map_sequence *pbrms)
{
...
DEFPY (no_pbr_map_nexthop_group,
  ...
  if (intf)
    {
      ...
      if (count > 1)
        {
          vty_out (vty, "Specified Intf %s exists in multiple VRFs\n", intf);
       ...
       "Name of the pbr-map to apply\n")
{
  ...
  if (no)
  ...
  else
    {
      ...
      snprintf (pbr_ifp->mapname, sizeof (pbr_ifp->mapname), "%s", mapname);
...
static void
vty_show_pbrms (struct vty *vty, const struct pbr_map_sequence *pbrms,
                bool detail)
{
  char rbuf[64];
  ...
  if (detail)
    vty_out (vty, "        Installed: %" PRIu64 "(%u) Reason: %s\n",
             pbrms->installed, pbrms->unique, pbrms->reason ? rbuf : "Valid");
  else
    vty_out (vty, "        Installed: %s Reason: %s\n",
             pbrms->installed ? "yes" : "no", pbrms->reason ? rbuf : "Valid");
  ...
  if (pbrms->nhgrp_name)
  ...
  else if (pbrms->nhg)
    {
      ...
      if (detail)
        ...
        vty_out (vty, "          Installed: %s Tableid: %u\n",
                 pbr_nht_get_installed (pbrms->internal_nhg_name) ? "yes"
                                                                  : "no",
                 pbr_nht_get_table (pbrms->internal_nhg_name));
    }
...
static void
vty_json_pbrms (json_object *j, struct vty *vty,
                const struct pbr_map_sequence *pbrms)
{
  json_object *jpbrm, *nexthop_group;
  char *nhg_name =
      pbrms->nhgrp_name ? pbrms->nhgrp_name : pbrms->internal_nhg_name;
  char rbuf[64];
  ...
  if (pbrms->dsfield & PBR_DSFIELD_DSCP)
    json_object_int_add (jpbrm, "matchDscp",
                         (pbrms->dsfield & PBR_DSFIELD_DSCP) >> 2);
  if (pbrms->dsfield & PBR_DSFIELD_ECN)
    json_object_int_add (jpbrm, "matchEcn", pbrms->dsfield & PBR_DSFIELD_ECN);

  json_object_array_add (j, jpbrm);
}

static void
vty_show_pbr_map (struct vty *vty, const struct pbr_map *pbrm, bool detail)
{
  struct pbr_map_sequence *pbrms;
...
static void
vty_json_pbr_map (json_object *j, struct vty *vty, const struct pbr_map *pbrm)
{
  struct pbr_map_sequence *pbrms;
  struct listnode *node;
  json_object *jpbrms;
      ...
      FOR_ALL_INTERFACES (vrf, ifp)
        {
          ...
          if (this_iface)
            {
              json_object_string_add (this_iface, "name", ifp->name);
              json_object_int_add (this_iface, "index", ifp->ifindex);
              json_object_string_add (this_iface, "policy", pbr_ifp->mapname);
              json_object_boolean_add (this_iface, "valid", pbrm);

              json_object_array_add (j, this_iface);
              continue;
            }

          vty_out (vty, "  %s(%d) with pbr-policy %s", ifp->name, ifp->ifindex,
                   pbr_ifp->mapname);
          if (! pbrm)
...
static int
pbr_interface_config_write (struct vty *vty)
{
  struct interface *ifp;
  struct vrf *vrf;
...
static int
pbr_vty_map_config_write_sequence (struct vty *vty, struct pbr_map *pbrm,
                                   struct pbr_map_sequence *pbrms)
{
...
static int
pbr_vty_map_config_write (struct vty *vty)
{
  struct pbr_map *pbrm;
...
static void
pbr_map_completer (vector comps, struct cmd_token *token)
{
  struct pbr_map *pbrm;
