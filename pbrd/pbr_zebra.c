...
pbr_if_new (struct interface *ifp)
{
...
int
pbr_ifp_create (struct interface *ifp)
{
...
int
pbr_ifp_destroy (struct interface *ifp)
{
  DEBUGD (&pbr_dbg_zebra, "%s: %s", __func__, ifp->name);
...
int
pbr_ifp_up (struct interface *ifp)
{
...
int
pbr_ifp_down (struct interface *ifp)
{
...
static int
interface_vrf_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (! ifp)
    {
      DEBUGD (&pbr_dbg_zebra, "%s: VRF change interface not found", __func__);

      return 0;
    }

  DEBUGD (&pbr_dbg_zebra, "%s: %s VRF change %u -> %u", __func__, ifp->name,
          vrf_id, new_vrf_id);
...
route_notify_owner (ZAPI_CALLBACK_ARGS)
{
  ...
  switch (note)
    ...
    case ZAPI_ROUTE_FAIL_INSTALL:
      DEBUGD (&pbr_dbg_zebra, "%s: [%pFX] Route install failure for table: %u",
              __func__, &p, table_id);
      break;
    ...
    case ZAPI_ROUTE_INSTALLED:
      DEBUGD (&pbr_dbg_zebra,
              "%s: [%pFX] Route installed succeeded for table: %u", __func__,
              &p, table_id);
      pbr_nht_route_installed_for_table (table_id);
      break;
    case ZAPI_ROUTE_REMOVED:
      DEBUGD (&pbr_dbg_zebra,
              "%s: [%pFX] Route Removed succeeded for table: %u", __func__, &p,
              table_id);
      pbr_nht_route_removed_for_table (table_id);
      break;
    case ZAPI_ROUTE_REMOVE_FAIL:
      DEBUGD (&pbr_dbg_zebra, "%s: [%pFX] Route remove fail for table: %u",
              __func__, &p, table_id);
      break;
    }
...
rule_notify_owner (ZAPI_CALLBACK_ARGS)
{
  ...
  if (! pbrms)
    {
      DEBUGD (&pbr_dbg_zebra, "%s: Failure to lookup pbrms based upon %u",
              __func__, unique);
      return 0;
    }
...
static void
route_add_helper (struct zapi_route *api, struct nexthop_group nhg,
                  uint8_t install_afi)
{
  ...
  for (ALL_NEXTHOPS (nhg, nhop))
    {
      ...
      switch (nhop->type)
        ...
        case NEXTHOP_TYPE_IPV6:
          memcpy (&api_nh->gate.ipv6, &nhop->gate.ipv6, IPV6_MAX_BYTELEN);
          break;
 ...
 */
void
route_add (struct pbr_nexthop_group_cache *pnhgc, struct nexthop_group nhg,
           afi_t install_afi)
{
  ...
  switch (install_afi)
    ...
    case AFI_L2VPN:
      DEBUGD (&pbr_dbg_zebra,
              "%s: Asked to install unsupported route type: L2VPN", __func__);
      break;
    case AFI_UNSPEC:
      DEBUGD (&pbr_dbg_zebra, "%s: Asked to install unspecified route type",
              __func__);
      break;
    }
}
 ...
 */
void
route_delete (struct pbr_nexthop_group_cache *pnhgc, afi_t afi)
{
  struct zapi_route api;
  ...
  switch (afi)
    ...
    case AFI_L2VPN:
      DEBUGD (&pbr_dbg_zebra,
              "%s: Asked to delete unsupported route type: L2VPN", __func__);
      break;
    case AFI_UNSPEC:
      DEBUGD (&pbr_dbg_zebra, "%s: Asked to delete unspecified route type",
              __func__);
      break;
    }
}
...
static int
pbr_zebra_nexthop_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (DEBUG_MODE_CHECK (&pbr_dbg_zebra, DEBUG_MODE_ALL))
    {
      ...
      DEBUGD (&pbr_dbg_zebra, "%s: Received Nexthop update: %pFX against %pFX",
      ...
      for (i = 0; i < nhr.nexthop_num; i++)
        {
          DEBUGD (&pbr_dbg_zebra,
                  ...
                  __func__, nhr.nexthops[i].type, nhr.nexthops[i].vrf_id,
                  nhr.nexthops[i].ifindex, &nhr.nexthops[i].gate.ipv4);
        }
    }
...
pbr_zebra_init (void)
{
  ...
  zclient =
      zclient_new (master, &opt, pbr_handlers, array_size (pbr_handlers));
...
void
pbr_send_rnh (struct nexthop *nhop, bool reg)
{
  uint32_t command;
  struct prefix p;

  command = (reg) ? ZEBRA_NEXTHOP_REGISTER : ZEBRA_NEXTHOP_UNREGISTER;
  ...
  if (zclient_send_rnh (zclient, command, &p, SAFI_UNICAST, false, false,
                        nhop->vrf_id) == ZCLIENT_SEND_FAILURE)
    {
...
static void
pbr_encode_pbr_map_sequence_prefix (struct stream *s, struct prefix *p,
...
static bool
pbr_encode_pbr_map_sequence (struct stream *s, struct pbr_map_sequence *pbrms,
                             struct interface *ifp)
{
  unsigned char family;
...
bool
pbr_send_pbr_map (struct pbr_map_sequence *pbrms,
                  struct pbr_map_interface *pmi, bool install, bool changed)
{
  ...
  DEBUGD (&pbr_dbg_zebra, "%s: for %s %d(%" PRIu64 ")", __func__, pbrm->name,
  ...
  if (pbr_encode_pbr_map_sequence (s, pbrms, pmi->ifp))
    ...
    {
      DEBUGD (&pbr_dbg_zebra, "%s: %s seq %u encode failed, skipped", __func__,
