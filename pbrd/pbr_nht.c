...
static void
pbr_nht_install_nexthop_group (struct pbr_nexthop_group_cache *pnhgc,
...
static bool
pbr_nhrc_hash_equal (const void *arg1, const void *arg2)
{
...
static void
pbr_nh_delete (struct pbr_nexthop_cache **pnhc)
{
  struct nhrc *nhrc;
  ...
  if (! nhrc || nhrc->refcount == 0)
    {
      DEBUGD (&pbr_dbg_nht, "%s: Removing nexthop from Zebra", __func__);
...
static void
pbr_nh_delete_iterate (struct hash_bucket *b, void *p)
{
  pbr_nh_delete ((struct pbr_nexthop_cache **) &b->data);
}

static uint32_t
pbr_nh_hash_key (const void *arg)
{
  uint32_t key;
...
static bool
pbr_nh_hash_equal (const void *arg1, const void *arg2)
{
  ...
  switch (pbrnc1->nexthop.type)
    ...
    case NEXTHOP_TYPE_IPV6:
      return ! memcmp (&pbrnc1->nexthop.gate.ipv6, &pbrnc2->nexthop.gate.ipv6,
                       16);
...
static void
pbr_nhgc_delete (struct pbr_nexthop_group_cache *p)
{
...
static void *
pbr_nhgc_alloc (void *p)
{
  struct pbr_nexthop_group_cache *new;
  struct pbr_nexthop_group_cache *pnhgc = (struct pbr_nexthop_group_cache *) p;
...
void
pbr_nhgroup_add_cb (const char *name)
{
  ...
  if (! nhgc)
    {
      DEBUGD (&pbr_dbg_nht, "%s: Could not find nhgc with name: %s", __func__,
              name);
      return;
    }
...
void
pbr_nhgroup_modify_cb (const struct nexthop_group_cmd *nhgc)
{
}

void
pbr_nhgroup_add_nexthop_cb (const struct nexthop_group_cmd *nhgc,
  ...
  if (DEBUG_MODE_CHECK (&pbr_dbg_nht, DEBUG_MODE_ALL))
    {
      ...
      DEBUGD (&pbr_dbg_nht, "%s: Added %s to nexthop-group %s", __func__,
              debugstr, nhgc->name);
    }
...
void
pbr_nhgroup_del_nexthop_cb (const struct nexthop_group_cmd *nhgc,
                            const struct nexthop *nhop)
{
  char debugstr[256];
  struct pbr_nexthop_group_cache pnhgc_find = {};
  ...
  if (DEBUG_MODE_CHECK (&pbr_dbg_nht, DEBUG_MODE_ALL))
    {
      nexthop2str (nhop, debugstr, sizeof (debugstr));
      DEBUGD (&pbr_dbg_nht, "%s: Removed %s from nexthop-group %s", __func__,
              debugstr, nhgc->name);
    }
...
void
pbr_nhgroup_delete_cb (const char *name)
{
                                    ...
                                    uint32_t table_id, bool installed)
{
  if (pnhgc->table_id == table_id)
    {
      DEBUGD (&pbr_dbg_nht, "%s: %s: Table ID (%u) matches %s", __func__,
...
static void
pbr_nht_find_nhg_from_table_install (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_group_cache *pnhgc =
...
void
pbr_nht_route_installed_for_table (uint32_t table_id)
{
  hash_iterate (pbr_nhg_hash, pbr_nht_find_nhg_from_table_install, &table_id);
}

static void
pbr_nht_find_nhg_from_table_remove (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_group_cache *pnhgc =
      (struct pbr_nexthop_group_cache *) b->data;
  uint32_t table_id = *(uint32_t *) data;
...
void
pbr_nht_route_removed_for_table (uint32_t table_id)
{
  hash_iterate (pbr_nhg_hash, pbr_nht_find_nhg_from_table_remove, &table_id);
}
 ...
 */
static afi_t
pbr_nht_which_afi (struct nexthop_group nhg, enum nexthop_types_t nh_type)
{
  struct nexthop *nexthop;
  ...
  if (! bh && v6 && v4)
    DEBUGD (&pbr_dbg_nht, "%s: Saw both V6 and V4 nexthops...using %s",
            __func__, afi2str (install_afi));
  if (bh && (v6 || v4))
    DEBUGD (
        &pbr_dbg_nht,
        "%s: Saw blackhole nexthop(s) with %s%s%s nexthop(s), using AFI_MAX.",
        __func__, v4 ? "v4" : "", (v4 && v6) ? " and " : "", v6 ? "v6" : "");
...
void
pbr_nht_change_group (const char *name)
{
  struct nexthop_group_cmd *nhgc;
  struct pbr_nexthop_group_cache *pnhgc;
  ...
  if (! pnhgc)
    {
      DEBUGD (&pbr_dbg_nht,
              "%s: Could not find nexthop-group cache w/ name '%s'", __func__,
              name);
      return;
    }
...
char *
pbr_nht_nexthop_make_name (char *name, size_t l, uint32_t seqno, char *buffer)
{
...
void
pbr_nht_set_seq_nhg_data (struct pbr_map_sequence *pbrms,
...
void
pbr_nht_set_seq_nhg (struct pbr_map_sequence *pbrms, const char *name)
{
  struct nexthop_group_cmd *nhgc;
...
void
pbr_nht_add_individual_nexthop (struct pbr_map_sequence *pbrms,
                                const struct nexthop *nhop)
{
  struct pbr_nexthop_group_cache *pnhgc;
  struct pbr_nexthop_group_cache find;
  ...
  pbrms->internal_nhg_name = XSTRDUP (
      MTYPE_TMP, pbr_nht_nexthop_make_name (
                     pbrms->parent->name, PBR_NHC_NAMELEN, pbrms->seqno, buf));
  ...
  if (nhop->vrf_id != VRF_DEFAULT)
    {
      ...
      if (vrf)
        strlcpy (pnhc->vrf_name, vrf->name, sizeof (pnhc->vrf_name));
    }

  if (nhop->ifindex != 0)
    {
      struct interface *ifp = if_lookup_by_index (nhop->ifindex, nhop->vrf_id);

      if (ifp)
        strlcpy (pnhc->intf_name, ifp->name, sizeof (pnhc->intf_name));
    }
  pbr_nht_install_nexthop_group (pnhgc, *pbrms->nhg);
}

static void
pbr_nht_release_individual_nexthop (struct pbr_map_sequence *pbrms)
{
  struct pbr_nexthop_group_cache *pnhgc;
  struct pbr_nexthop_group_cache find;
  struct pbr_nexthop_cache *pnhc;
...
void
pbr_nht_delete_individual_nexthop (struct pbr_map_sequence *pbrms)
{
...
void
pbr_nht_delete_group (const char *name)
{
  ...
  RB_FOREACH (pbrm, pbr_map_entry_head, &pbr_maps)
    {
      for (ALL_LIST_ELEMENTS_RO (pbrm->seqnumbers, snode, pbrms))
        {
          if (pbrms->nhgrp_name && strmatch (pbrms->nhgrp_name, name))
            {
...
bool
pbr_nht_nexthop_valid (struct nexthop_group *nhg)
{
...
bool
pbr_nht_nexthop_group_valid (const char *name)
{
  struct pbr_nexthop_group_cache *pnhgc;
  struct pbr_nexthop_group_cache lookup;
                                      ...
                                      struct pbr_nht_individual *pnhi)
{
  ...
  switch (pnhi->nhr->prefix.family)
    ...
    case AF_INET:
      ...
      break;
    case AF_INET6:
      if (memcmp (&pnhc->nexthop.gate.ipv6, &pnhi->nhr->prefix.u.prefix6,
                  16) != 0)
        goto done; /* Unrelated change */
      break;
    }
                                             ...
                                             struct pbr_nht_individual *pnhi)
{
  ...
  if (pnhc->nexthop.ifindex != pnhi->ifp->ifindex) /* Un-related interface */
    goto done;
...
/* Given this update either from interface or nexthop tracking, re-validate
 * this nexthop.
 *
...
static void
pbr_nht_individual_nexthop_update_lookup (struct hash_bucket *b, void *data)
{
  ...
  DEBUGD (&pbr_dbg_nht, "    Found %pFX: old: %d new: %d", &pnhi->nhr->prefix,
          old_valid, pnhc->valid);
...
static void
pbr_nexthop_group_cache_iterate_to_group (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_cache *pnhc = b->data;
...
static void
pbr_nexthop_group_cache_to_nexthop_group (
    struct nexthop_group *nhg, struct pbr_nexthop_group_cache *pnhgc)
{
  hash_iterate (pnhgc->nhh, pbr_nexthop_group_cache_iterate_to_group, nhg);
}

static void
pbr_nht_nexthop_update_lookup (struct hash_bucket *b, void *data)
{
  ...
  hash_iterate (pnhgc->nhh, pbr_nht_individual_nexthop_update_lookup, &pnhi);
...
void
pbr_nht_nexthop_update (struct zapi_route *nhr)
{
...
static int
pbr_nht_nhrc_vrf_change (struct hash_bucket *b, void *data)
{
...
static int
pbr_nht_individual_nexthop_vrf_handle (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_cache *pnhc = b->data;
  struct pbr_nht_individual *pnhi = data;
      ...
      if (pnhc->nexthop.vrf_id != pbr_vrf_id (pnhi->pbr_vrf))
        {
          ...
          do
            {
              nhrcvi.nhrc = NULL;
              hash_walk (pbr_nhrc_hash, pbr_nht_nhrc_vrf_change, &nhrcvi);
              if (nhrcvi.nhrc)
                {
                  hash_release (pbr_nhrc_hash, nhrcvi.nhrc);
                  nhrcvi.nhrc->nexthop.vrf_id = pbr_vrf_id (pnhi->pbr_vrf);
                  (void) hash_get (pbr_nhrc_hash, nhrcvi.nhrc,
                                   hash_alloc_intern);
...
static void
pbr_nht_clear_looked_at (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_cache *pnhc = b->data;
...
static void
pbr_nht_nexthop_vrf_handle (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_group_cache *pnhgc = b->data;
  ...
  do
    {
      ...
      pnhi.pnhc = NULL;
      hash_walk (pnhgc->nhh, pbr_nht_individual_nexthop_vrf_handle, &pnhi);
...
void
pbr_nht_vrf_update (struct pbr_vrf *pbr_vrf)
{
  hash_iterate (pbr_nhg_hash, pbr_nht_nexthop_vrf_handle, pbr_vrf);
}

static void
pbr_nht_individual_nexthop_interface_handle (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_cache *pnhc = b->data;
  struct pbr_nht_individual *pnhi = data;
...
static void
pbr_nht_nexthop_interface_handle (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_group_cache *pnhgc = b->data;
  ...
  do
    {
      memset (&pnhi, 0, sizeof (pnhi));
      pnhi.ifp = ifp;
      hash_iterate (pnhgc->nhh, pbr_nht_individual_nexthop_interface_handle,
                    &pnhi);
      ...
      pbr_map_check_interface_nh_group_change (pnhgc->name, ifp, old_ifindex);
    }
  while (pnhi.pnhc);
}

void
pbr_nht_interface_update (struct interface *ifp)
{
...
static void
pbr_nht_nexthop_interface_update_lookup (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_group_cache *pnhgc = b->data;
  struct pbr_nht_individual pnhi = {};
  struct nexthop_group nhg = {};
  ...
  pnhi.ifp = data;
  pnhi.valid = false;
  hash_iterate (pnhgc->nhh, pbr_nht_individual_nexthop_interface_update_lookup,
                &pnhi);
...
void
pbr_nht_nexthop_interface_update (struct interface *ifp)
{
  hash_iterate (pbr_nhg_hash, pbr_nht_nexthop_interface_update_lookup, ifp);
}

static bool
pbr_nhg_allocated_id_hash_equal (const void *arg1, const void *arg2)
{
...
static uint32_t
pbr_nhg_allocated_id_hash_key (const void *arg)
{
...
static uint32_t
pbr_nhg_hash_key (const void *arg)
{
  const struct pbr_nexthop_group_cache *nhgc = arg;
...
static bool
pbr_nhg_hash_equal (const void *arg1, const void *arg2)
{
...
uint32_t
pbr_nht_find_next_unallocated_table_id (void)
{
   ...
   */
  for (iter.table_id = pbr_nhg_low_table; iter.table_id < pbr_nhg_high_table;
       ++iter.table_id)
...
bool
pbr_nht_has_unallocated_table (void)
{
  return ! ! pbr_next_unallocated_table_id;
}

void
pbr_nht_update_next_unallocated_table_id (void)
{
  pbr_next_unallocated_table_id = pbr_nht_find_next_unallocated_table_id ();
}
...
void
pbr_nht_set_tableid_range (uint32_t low, uint32_t high)
{
...
void
pbr_nht_write_table_range (struct vty *vty)
{
...
uint32_t
pbr_nht_get_next_rule (uint32_t seqno)
{
  return seqno + pbr_nhg_low_rule - 1;
}
void
pbr_nht_set_rule_range (uint32_t low, uint32_t high)
{
...
void
pbr_nht_write_rule_range (struct vty *vty)
{
...
uint32_t
pbr_nht_get_table (const char *name)
{
  struct pbr_nexthop_group_cache find;
  struct pbr_nexthop_group_cache *pnhgc;
...
bool
pbr_nht_get_installed (const char *name)
{
  struct pbr_nexthop_group_cache find;
  struct pbr_nexthop_group_cache *pnhgc;
...
static void
pbr_nht_show_nhg_nexthops (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_cache *pnhc = b->data;
  struct vty *vty = data;
...
static void
pbr_nht_json_nhg_nexthops (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_cache *pnhc = b->data;
...
static void
pbr_nht_show_nhg (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_group_cache *pnhgc = b->data;
...
static void
pbr_nht_json_nhg (struct hash_bucket *b, void *data)
{
  struct pbr_nexthop_group_cache *pnhgc = b->data;
  struct pbr_nht_show *pns = data;
...
void
pbr_nht_show_nexthop_group (struct vty *vty, const char *name)
{
...
void
pbr_nht_json_nexthop_group (json_object *j, const char *name)
{
  struct pbr_nht_show pns;
...
pbr_nht_init (void)
{
  pbr_nhg_hash = hash_create_size (16, pbr_nhg_hash_key, pbr_nhg_hash_equal,
                                   "PBR NHG Cache Hash");
  pbr_nhrc_hash =
  ...
  pbr_nhg_allocated_id_hash = hash_create_size (
      16, pbr_nhg_allocated_id_hash_key, pbr_nhg_allocated_id_hash_equal,
