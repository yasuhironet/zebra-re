...
static inline int
pbr_map_compare (const struct pbr_map *pbrmap1, const struct pbr_map *pbrmap2)
{
  return strcmp (pbrmap1->name, pbrmap2->name);
}

static int
pbr_map_sequence_compare (const struct pbr_map_sequence *pbrms1,
...
static void
pbr_map_sequence_delete (struct pbr_map_sequence *pbrms)
{
...
static int
pbr_map_interface_compare (const struct pbr_map_interface *pmi1,
...
static void
pbr_map_interface_list_delete (struct pbr_map_interface *pmi)
{
...
static bool
pbrms_is_installed (const struct pbr_map_sequence *pbrms,
...
static bool
pbr_map_interface_is_valid (const struct pbr_map_interface *pmi)
{
...
static void
pbr_map_pbrms_update_common (struct pbr_map_sequence *pbrms, bool install,
                             bool changed)
{
  struct pbr_map *pbrm;
...
static void
pbr_map_pbrms_install (struct pbr_map_sequence *pbrms, bool changed)
{
  pbr_map_pbrms_update_common (pbrms, true, changed);
}

static void
pbr_map_pbrms_uninstall (struct pbr_map_sequence *pbrms)
{
...
void
pbr_map_reason_string (unsigned int reason, char *buf, int size)
{
  unsigned int bit;
  int len = 0;
      ...
      if ((reason & (1 << bit)) && (len < size))
        {
          len += snprintf ((buf + len), (size - len), "%s%s",
                           (len > 0) ? ", " : "", pbr_map_reason_str[bit]);
        }
    }
}

void
pbr_map_final_interface_deletion (struct pbr_map *pbrm,
                                  struct pbr_map_interface *pmi)
{
...
void
pbr_map_interface_delete (struct pbr_map *pbrm, struct interface *ifp_del)
{
...
void
pbr_map_add_interface (struct pbr_map *pbrm, struct interface *ifp_add)
{
  struct listnode *node;
  struct pbr_map_interface *pmi;
                                        ...
                                        struct pbr_map **pbrm)
{
  if (! ifp->info)
    {
      DEBUGD (&pbr_dbg_map, "%s: %s has no pbr_interface info", __func__,
              ifp->name);
      return -1;
    }
...
void
pbr_map_policy_interface_update (const struct interface *ifp, bool state_up)
{
  struct pbr_interface *pbr_ifp;
  struct pbr_map_sequence *pbrms;
  struct pbr_map *pbrm;
  ...
  DEBUGD (&pbr_dbg_map, "%s: %s %s rules on interface %s", __func__,
          pbr_ifp->mapname, (state_up ? "installing" : "removing"), ifp->name);
...
static void
pbrms_vrf_update (struct pbr_map_sequence *pbrms,
                  const struct pbr_vrf *pbr_vrf)
{
  const char *vrf_name = pbr_vrf_name (pbr_vrf);

  if (pbrms->vrf_lookup &&
      (strncmp (vrf_name, pbrms->vrf_name, sizeof (pbrms->vrf_name)) == 0))
    {
...
void
pbr_map_vrf_update (const struct pbr_vrf *pbr_vrf)
{
  struct pbr_map *pbrm;
  struct pbr_map_sequence *pbrms;
  struct listnode *node;
...
void
pbr_map_write_interfaces (struct vty *vty, struct interface *ifp)
{
...
extern void
pbr_map_delete (struct pbr_map_sequence *pbrms)
{
  struct pbr_map *pbrm;
...
static void
pbr_map_delete_common (struct pbr_map_sequence *pbrms)
{
...
void
pbr_map_delete_nexthops (struct pbr_map_sequence *pbrms)
{
  pbr_map_delete_common (pbrms);
}

void
pbr_map_delete_vrf (struct pbr_map_sequence *pbrms)
{
  pbr_map_delete_common (pbrms);
}
      ...
      for (ALL_LIST_ELEMENTS_RO (pbrm->incoming, inode, pmi))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (pbrm->seqnumbers, snode, pbrms))
            {
              DEBUGD (&pbr_dbg_map, "%s: Comparing %u to %u", __func__,
                      pbrms->unique, unique);
...
static void
pbr_map_add_interfaces (struct pbr_map *pbrm)
{
...
uint8_t
pbr_map_decode_dscp_enum (const char *name)
{
...
struct pbr_map_sequence *
pbrms_get (const char *name, uint32_t seqno)
{
  struct pbr_map *pbrm;
  struct pbr_map_sequence *pbrms;
  struct listnode *node;
  ...
  if (! pbrm)
    {
      ...
      pbrm->seqnumbers->del = (void (*) (void *)) pbr_map_sequence_delete;
  ...
  if (! pbrms)
    {
      ...
      pbrms->reason = PBR_MAP_INVALID_EMPTY | PBR_MAP_INVALID_NO_NEXTHOPS;
      pbrms->vrf_name[0] = '\0';
...
pbr_map_sequence_check_nexthops_valid (struct pbr_map_sequence *pbrms)
{
  ...
  if (pbrms->vrf_lookup)
    {
      struct pbr_vrf *pbr_vrf = pbr_vrf_lookup_by_name (pbrms->vrf_name);
  ...
  if (pbrms->nhg && ! pbr_nht_nexthop_group_valid (pbrms->internal_nhg_name))
...
static void
pbr_map_sequence_check_not_empty (struct pbr_map_sequence *pbrms)
{
...
static void
pbr_map_sequence_check_vlan_actions (struct pbr_map_sequence *pbrms)
{
 ...
 */
static void
pbr_map_sequence_check_valid (struct pbr_map_sequence *pbrms)
{
...
static bool
pbr_map_check_valid_internal (struct pbr_map *pbrm)
{
  struct pbr_map_sequence *pbrms;
  struct listnode *node;
 ...
 */
bool
pbr_map_check_valid (const char *name)
{
  struct pbr_map *pbrm;
...
void
pbr_map_schedule_policy_from_nhg (const char *nh_group, bool installed)
{
  struct pbr_map_sequence *pbrms;
  struct pbr_map *pbrm;
  struct listnode *node;
      ...
      for (ALL_LIST_ELEMENTS_RO (pbrm->seqnumbers, node, pbrms))
        {
          ...
          if (pbrms->nhgrp_name && (strcmp (nh_group, pbrms->nhgrp_name) == 0))
            {
          ...
          if (pbrms->nhg && (strcmp (nh_group, pbrms->internal_nhg_name) == 0))
            {
              pbrms->nhs_installed = installed;
...
void
pbr_map_policy_install (const char *name)
{
  struct pbr_map_sequence *pbrms;
  struct pbr_map *pbrm;
  struct listnode *node, *inode;
  ...
  for (ALL_LIST_ELEMENTS_RO (pbrm->seqnumbers, node, pbrms))
    {
      DEBUGD (&pbr_dbg_map, "%s: Looking at what to install %s(%u) %d %d",
              __func__, name, pbrms->seqno, pbrm->valid, pbrms->nhs_installed);
      ...
      if (pbrm->valid && pbrms->nhs_installed && pbrm->incoming->count)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (pbrm->incoming, inode, pmi))
            if (pbr_map_interface_is_valid (pmi))
              pbr_send_pbr_map (pbrms, pmi, true, false);
        }
    }
}

void
pbr_map_policy_delete (struct pbr_map *pbrm, struct pbr_map_interface *pmi)
{
  struct listnode *node;
  struct pbr_map_sequence *pbrms;
  bool sent = false;
 ...
 */
void
pbr_map_check_nh_group_change (const char *nh_group)
{
  struct pbr_map_sequence *pbrms;
  struct pbr_map *pbrm;
  struct listnode *node, *inode;
      ...
      for (ALL_LIST_ELEMENTS_RO (pbrm->seqnumbers, node, pbrms))
        {
          found_name = false;
          if (pbrms->nhgrp_name)
            found_name = ! strcmp (nh_group, pbrms->nhgrp_name);
          else if (pbrms->nhg)
            found_name = ! strcmp (nh_group, pbrms->internal_nhg_name);
          ...
          if (found_name)
            {
              ...
              if (pbrms->nhgrp_name)
                pbr_nht_set_seq_nhg_data (pbrms,
...
void
pbr_map_check_vrf_nh_group_change (const char *nh_group,
...
void
pbr_map_check_interface_nh_group_change (const char *nh_group,
                                         struct interface *ifp,
...
void
pbr_map_check (struct pbr_map_sequence *pbrms, bool changed)
{
  struct pbr_map *pbrm;
  bool install;

  pbrm = pbrms->parent;
  DEBUGD (&pbr_dbg_map, "%s: for %s(%u)", __func__, pbrm->name, pbrms->seqno);
  if (pbr_map_check_valid (pbrm->name))
    DEBUGD (&pbr_dbg_map, "We are totally valid %s", pbrm->name);

  if (pbrms->reason == PBR_MAP_VALID_SEQUENCE_NUMBER)
    {
      install = true;
      DEBUGD (&pbr_dbg_map, "%s: Installing %s(%u) reason: %" PRIu64, __func__,
              pbrm->name, pbrms->seqno, pbrms->reason);
      DEBUGD (&pbr_dbg_map, "    Sending PBR_MAP_POLICY_INSTALL event");
    }
  else
    {
      install = false;
      DEBUGD (&pbr_dbg_map, "%s: Removing %s(%u) reason: %" PRIu64, __func__,
              pbrm->name, pbrms->seqno, pbrms->reason);
    }
...
void
pbr_map_install (struct pbr_map *pbrm)
{
  struct pbr_map_sequence *pbrms;
  struct listnode *node;
