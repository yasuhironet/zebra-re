...
_lsdb_count_assert (struct ospf6_lsdb *lsdb)
{
  ...
  zlog_debug ("PANIC !! lsdb[%p]->count = %d, real = %d", lsdb, lsdb->count,
              num);
  for (ALL_LSDB (lsdb, debug, debugnext))
...
static inline void
ospf6_lsdb_stats_update (struct ospf6_lsa *lsa, struct ospf6_lsdb *lsdb,
                         int count)
{
...
struct ospf6_lsa *
ospf6_lsdb_lookup (uint16_t type, uint32_t id, uint32_t adv_router,
                   struct ospf6_lsdb *lsdb)
{
  struct route_node *node;
  struct prefix_ipv6 key;
...
struct ospf6_lsa *
ospf6_find_external_lsa (struct ospf6 *ospf6, struct prefix *p)
{
...
struct ospf6_lsa *
ospf6_find_inter_prefix_lsa (struct ospf6 *ospf6, struct ospf6_area *area,
                             struct prefix *p)
{
  struct ospf6_lsa *lsa;
...
struct ospf6_lsa *
ospf6_lsdb_lookup_next (uint16_t type, uint32_t id, uint32_t adv_router,
                        struct ospf6_lsdb *lsdb)
{
  struct route_node *node;
  struct prefix_ipv6 key;
...
ospf6_lsdb_head (struct ospf6_lsdb *lsdb, int argmode, uint16_t type,
                 uint32_t adv_router, struct ospf6_lsa **lsa)
{
...
struct ospf6_lsa *
ospf6_lsdb_next (const struct route_node *iterend, struct ospf6_lsa *lsa)
{
  ...
  for (ALL_LSDB (lsdb, lsa, lsanext))
    {
      ...
      if (lsa->retrans_count != 0)
        {
          if (IS_OSPF6_DEBUG_LSA_TYPE (lsa->header->type))
            zlog_debug ("Remove MaxAge %s retrans_count %d", lsa->name,
                        lsa->retrans_count);
...
uint32_t
ospf6_new_ls_id (uint16_t type, uint32_t adv_router, struct ospf6_lsdb *lsdb)
{
  struct ospf6_lsa *lsa;
...
uint32_t
ospf6_new_ls_seqnum (uint16_t type, uint32_t id, uint32_t adv_router,
                     struct ospf6_lsdb *lsdb)
{
  struct ospf6_lsa *lsa;
