...
static int
ospf6_gr_lsa_originate (struct ospf6_interface *oi,
                        enum ospf6_gr_restart_reason reason)
{
  ...
  grace_lsa = (struct ospf6_grace_lsa *) ((caddr_t) lsa_header +
  ...
  if (reason == OSPF6_GR_UNKNOWN_RESTART)
    {
      ...
      *uv32 = htonl (1);
      ...
      if (n != length)
        flog_err (EC_LIB_DEVELOPMENT, "%s: could not send entire message",
                  __func__);
    }
  else
    {
...
static void
ospf6_gr_flush_grace_lsas (struct ospf6 *ospf6)
{
...
static void
ospf6_gr_restart_exit (struct ospf6 *ospf6, const char *reason)
{
  struct ospf6_area *area;
...
void
ospf6_gr_restart_enter (struct ospf6 *ospf6,
                        enum ospf6_gr_restart_reason reason, time_t timestamp)
{
...
static int
ospf6_router_lsa_contains_adj (struct ospf6_area *area, in_addr_t adv_router,
...
static bool
ospf6_gr_check_router_lsa_consistency (struct ospf6 *ospf6,
                                       struct ospf6_area *area,
                                       struct ospf6_lsa *lsa)
{
...
/*
 * Check for LSAs that are inconsistent with the pre-restart LSAs, and abort
 * the ongoing graceful restart when that's the case.
 */
void
ospf6_gr_check_lsdb_consistency (struct ospf6 *ospf6, struct ospf6_area *area)
{
  uint16_t type;
  struct ospf6_lsa *lsa;
...
static bool
ospf6_gr_check_adj_id (struct ospf6_area *area, in_addr_t neighbor_router_id)
{
...
static bool
ospf6_gr_check_adjs_lsa_transit (struct ospf6_area *area,
                                 in_addr_t neighbor_router_id,
  ...
  if (neighbor_router_id == ospf6->router_id)
    {
      ...
      network_lsa =
          (struct ospf6_network_lsa *) ((char *) lsa->header +
                                        sizeof (struct ospf6_lsa_header));
...
static bool
ospf6_gr_check_adjs_lsa (struct ospf6_area *area, struct ospf6_lsa *lsa)
{
  struct ospf6_router_lsa *router_lsa;
  char *start, *end, *current;
 ...
 */
static bool
ospf6_gr_check_adjs (struct ospf6 *ospf6)
{
  struct ospf6_area *area;
...
static void
ospf6_gr_grace_period_expired (struct event *thread)
{
...
void
ospf6_gr_iface_send_grace_lsa (struct event *thread)
{
 ...
 */
static void
ospf6_gr_nvm_update (struct ospf6 *ospf6, bool prepare)
{
 ...
 */
void
ospf6_gr_nvm_delete (struct ospf6 *ospf6)
{
  const char *inst_name;
  json_object *json;
  json_object *json_instances;
 ...
 */
void
ospf6_gr_nvm_read (struct ospf6 *ospf6)
{
  const char *inst_name;
  json_object *json;
  json_object *json_instances;
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, onode, ospf6))
    {
      ...
      if (IS_DEBUG_OSPF6_GR)
        zlog_debug (
            ...
            ospf6->gr_info.grace_period, ospf6_vrf_id_to_name (ospf6->vrf_id));
...
static int
ospf6_gr_neighbor_change (struct ospf6_neighbor *on, int next_state,
                          int prev_state)
{
...
int
config_write_ospf6_gr (struct vty *vty, struct ospf6 *ospf6)
{
  if (! ospf6->gr_info.restart_support)
    return 0;
...
DEFPY (ospf6_graceful_restart_prepare,
       ...
       "Maximum length of the 'grace period' in seconds\n")
{
  ...
  if (ospf6->gr_info.prepare_in_progress)
    {
      vty_out (vty, "%% Error: Graceful Restart preparation in progress\n");
      return CMD_WARNING;
    }
