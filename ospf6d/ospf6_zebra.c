...
void
ospf6_zebra_vrf_register (struct ospf6 *ospf6)
{
      ...
      if (IS_OSPF6_DEBUG_ZEBRA (RECV))
        {
          zlog_debug ("%s: Register VRF %s id %u", __func__,
                      ospf6_vrf_id_to_name (ospf6->vrf_id), ospf6->vrf_id);
        }
...
void
ospf6_zebra_vrf_deregister (struct ospf6 *ospf6)
{
  if (! zclient || zclient->sock < 0 || ! ospf6)
    return;
...
static int
ospf6_router_id_update_zebra (ZAPI_CALLBACK_ARGS)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Zebra router-id update %pI4 vrf %s id %u",
                &router_id.u.prefix4, ospf6_vrf_id_to_name (vrf_id), vrf_id);
...
void
ospf6_zebra_redistribute (int type, vrf_id_t vrf_id)
{
...
void
ospf6_zebra_no_redistribute (int type, vrf_id_t vrf_id)
{
  if (! vrf_bitmap_check (zclient->redist[AFI_IP6][type], vrf_id))
    return;
...
void
ospf6_zebra_import_default_route (struct ospf6 *ospf6, bool unreg)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (SEND))
    zlog_debug ("%s: sending cmd %s for %pFX (vrf %u)", __func__,
                zserv_command_string (command), &prefix, ospf6->vrf_id);

  if (zclient_send_rnh (zclient, command, &prefix, SAFI_UNICAST, false, true,
                        ospf6->vrf_id) == ZCLIENT_SEND_FAILURE)
    flog_err (EC_LIB_ZAPI_SOCKET, "%s: zclient_send_rnh() failed", __func__);
}

static int
ospf6_zebra_import_check_update (ZAPI_CALLBACK_ARGS)
{
  struct ospf6 *ospf6;
  ...
  if (! zapi_nexthop_update_decode (zclient->ibuf, &matched, &nhr))
    {
      zlog_err ("%s[%u]: Failure to decode route", __func__, ospf6->vrf_id);
      return -1;
    }
...
static int
ospf6_zebra_if_address_update_add (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Zebra Interface address add: %s %5s %pFX", c->ifp->name,
                prefix_family_str (c->address), c->address);
...
static int
ospf6_zebra_if_address_update_delete (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Zebra Interface address delete: %s %5s %pFX", c->ifp->name,
                prefix_family_str (c->address), c->address);
...
static int
ospf6_zebra_gr_update (struct ospf6 *ospf6, int command, uint32_t stale_time)
{
  struct zapi_cap api;
...
int
ospf6_zebra_gr_enable (struct ospf6 *ospf6, uint32_t stale_time)
{
...
int
ospf6_zebra_gr_disable (struct ospf6 *ospf6)
{
  if (IS_DEBUG_OSPF6_GR)
...
static int
ospf6_zebra_read_route (ZAPI_CALLBACK_ARGS)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug (
        ...
        zebra_route_string (api.type), &api.prefix, nexthop, ifindex, api.tag);
  ...
  if (cmd == ZEBRA_REDISTRIBUTE_ROUTE_ADD)
    ospf6_asbr_redistribute_add (api.type, ifindex, &api.prefix,
                                 api.nexthop_num, nexthop, api.tag, ospf6);
  else
      ...
      for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
        {
          if (vrf_bitmap_check (zclient->redist[AFI_IP6][i], VRF_DEFAULT))
            json_object_array_add (
                json_array, json_object_new_string (zebra_route_string (i)));
        }
...
static void
ospf6_zebra_append_opaque_attr (struct ospf6_route *request,
                                struct zapi_route *api)
{
  ...
  switch (request->path.type)
    ...
    case OSPF6_PATH_TYPE_INTER:
      /* OSPF area ID */
      (void) inet_ntop (AF_INET, &request->path.area_id, ospf_opaque.area_id,
                          ...
                          struct ospf6 *ospf6)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (SEND))
    zlog_debug ("Zebra Send %s route: %pFX", (type == REM ? "remove" : "add"),
                &request->prefix);
  ...
  api.distance =
      ospf6_distance_apply ((struct prefix_ipv6 *) dest, request, ospf6);
  ...
  if (ret == ZCLIENT_SEND_FAILURE)
    flog_err (EC_LIB_ZAPI_SOCKET, "zclient_route_send() %s failed: %s",
...
{
  if (ospf6->gr_info.restart_in_progress || ospf6->gr_info.prepare_in_progress)
    {
      if (IS_DEBUG_OSPF6_GR)
        zlog_debug (
...
void
ospf6_zebra_add_discard (struct ospf6_route *request, struct ospf6 *ospf6)
{
  struct zapi_route api;
  ...
  if (! CHECK_FLAG (request->flag, OSPF6_ROUTE_BLACKHOLE_ADDED))
    ...
    {
      if (IS_OSPF6_DEBUG_ZEBRA (SEND))
        zlog_debug ("Zebra: Blackhole route present already %pFX", dest);
    }
}

void
ospf6_zebra_delete_discard (struct ospf6_route *request, struct ospf6 *ospf6)
{
  struct zapi_route api;
  struct prefix *dest = &request->prefix;
  ...
  if (CHECK_FLAG (request->flag, OSPF6_ROUTE_BLACKHOLE_ADDED))
    ...
    {
      if (IS_OSPF6_DEBUG_ZEBRA (SEND))
        zlog_debug ("Zebra: Blackhole route already deleted %pFX", dest);
    }
}
...
int
ospf6_distance_set (struct vty *vty, struct ospf6 *o, const char *distance_str,
...
void
ospf6_zebra_init (struct event_loop *master)
{
