...
static void
ospf6_reset_mcastloop (int ospf6_sock)
{
  unsigned int off = 0;
...
static void
ospf6_set_pktinfo (int ospf6_sock)
{
  setsockopt_ipv6_pktinfo (ospf6_sock, 1);
}

static void
ospf6_set_transport_class (int ospf6_sock)
{
...
void
ospf6_serv_close (int *ospf6_sock)
{
  if (*ospf6_sock != -1)
    {
      close (*ospf6_sock);
...
int
ospf6_serv_sock (struct ospf6 *ospf6)
{
  int ospf6_sock;
...
int
ospf6_sso (ifindex_t ifindex, struct in6_addr *group, int option, int sockfd)
{
...
int
ospf6_sendmsg (struct in6_addr *src, struct in6_addr *dst, ifindex_t ifindex,
...
int
ospf6_recvmsg (struct in6_addr *src, struct in6_addr *dst, ifindex_t *ifindex,
               struct iovec *message, int ospf6_sock)
{
  int retval;
