...
static void
ospf6_add (struct ospf6 *ospf6)
{
  listnode_add (om6->ospf6, ospf6);
}

static void
ospf6_del (struct ospf6 *ospf6)
{
  listnode_delete (om6->ospf6, ospf6);
}

const char *
ospf6_vrf_id_to_name (vrf_id_t vrf_id)
{
...
void
ospf6_vrf_link (struct ospf6 *ospf6, struct vrf *vrf)
{
...
void
ospf6_vrf_unlink (struct ospf6 *ospf6, struct vrf *vrf)
{
  if (vrf->info == (void *) ospf6)
    vrf->info = NULL;
...
static int
ospf6_vrf_delete (struct vrf *vrf)
{
  return 0;
}

static void
ospf6_set_redist_vrf_bitmaps (struct ospf6 *ospf6, bool set)
{
  int type;
  ...
  for (type = 0; type < ZEBRA_ROUTE_MAX; type++)
    {
      ...
      if (IS_OSPF6_DEBUG_ZEBRA (RECV))
        zlog_debug ("%s: setting redist vrf %d bitmap for type %d", __func__,
                    ospf6->vrf_id, type);
      if (set)
        vrf_bitmap_set (zclient->redist[AFI_IP6][type], ospf6->vrf_id);
      else
        vrf_bitmap_unset (zclient->redist[AFI_IP6][type], ospf6->vrf_id);
    }
  ...
  if (red_list)
    {
      if (set)
        vrf_bitmap_set (zclient->default_information[AFI_IP6], ospf6->vrf_id);
      else
...
static int
ospf6_vrf_disable (struct vrf *vrf)
{
...
static int
ospf6_vrf_enable (struct vrf *vrf)
{
  struct ospf6 *ospf6 = NULL;
  vrf_id_t old_vrf_id;
  int ret = 0;
...
ospf6_top_route_hook_add (struct ospf6_route *route)
{
  ...
  if (route->table->scope_type == OSPF6_SCOPE_TYPE_GLOBAL)
    ...
    {
      if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL) || IS_OSPF6_DEBUG_BROUTER)
        zlog_debug (
  ...
  if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL) || IS_OSPF6_DEBUG_BROUTER)
    {
      ...
      zlog_debug ("%s: brouter %s add with adv router %x nh count %u",
                  __func__, brouter_name, route->path.origin.adv_router,
  ...
  if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL) || IS_OSPF6_DEBUG_BROUTER)
    {
      ...
      brouter_id = ADV_ROUTER_IN_PREFIX (&route->prefix);
      inet_ntop (AF_INET, &brouter_id, brouter_name, sizeof (brouter_name));
      zlog_debug ("%s: brouter %p %s del with adv router %x nh %u", __func__,
                  (void *) route, brouter_name, route->path.origin.adv_router,
                  listcount (route->nh_list));
    }
  route->flag |= OSPF6_ROUTE_REMOVE;
  ospf6_abr_examin_brouter (ADV_ROUTER_IN_PREFIX (&route->prefix), route,
...
void
ospf6_master_init (struct event_loop *master)
{
...
static void
ospf6_maxage_remover (struct event *thread)
{
...
ospf6_maxage_remove (struct ospf6 *o)
{
  if (o)
    event_add_timer (master, ospf6_maxage_remover, o,
                     OSPF_LSA_MAXAGE_REMOVE_DELAY_DEFAULT, &o->maxage_remover);
}

bool
ospf6_router_id_update (struct ospf6 *ospf6, bool init)
{
...
static void
ospf6_db_clear (struct ospf6 *ospf6)
{
  struct ospf6_interface *oi;
  struct interface *ifp;
...
static void
ospf6_process_reset (struct ospf6 *ospf6)
{
  struct interface *ifp;
  struct vrf *vrf = vrf_lookup_by_id (ospf6->vrf_id);
...
static void
ospf6_reinstall_routes (struct ospf6 *ospf6)
{
       ...
       "To zebra\n")
{
  ...
  if (no && CHECK_FLAG (ospf6->config_flags, OSPF6_SEND_EXTRA_DATA_TO_ZEBRA))
    {
      UNSET_FLAG (ospf6->config_flags, OSPF6_SEND_EXTRA_DATA_TO_ZEBRA);
      ospf6_reinstall_routes (ospf6);
    }
  else if (! CHECK_FLAG (ospf6->config_flags, OSPF6_SEND_EXTRA_DATA_TO_ZEBRA))
    {
...
void
ospf6_restart_spf (struct ospf6 *ospf6)
{
  ospf6_route_remove_all (ospf6->route_table);
  ospf6_route_remove_all (ospf6->brouter_table);
...
/* Set the max paths */
static void
ospf6_maxpath_set (struct ospf6 *ospf6, uint16_t paths)
{
...
static void
ospf6_show (struct vty *vty, struct ospf6 *o, json_object *json, bool use_json)
{
  struct listnode *n;
  struct ospf6_area *oa;
  ...
  if (use_json)
    {
      ...
      /* XXX */
      json_object_int_add (json, "lsaMinimumArrivalMsecs", o->lsa_minarrival);

      /* Show SPF parameters */
      json_object_int_add (json, "spfScheduleDelayMsecs", o->spf_delay);
      json_object_int_add (json, "holdTimeMinMsecs", o->spf_holdtime);
      json_object_int_add (json, "holdTimeMaxMsecs", o->spf_max_holdtime);
      json_object_int_add (json, "holdTimeMultiplier", o->spf_hold_multiplier);
      ...
      if (o->ts_spf.tv_sec || o->ts_spf.tv_usec)
        {
          ...
          json_object_string_add (json, "spfLastExecutedMsecs", buf);
          json_object_string_add (json, "spfLastExecutedReason", rbuf);

          json_object_int_add (json, "spfLastDurationSecs",
      ...
      if (event_is_scheduled (o->t_spf_calc))
        {
          ...
          time_store = monotime_until (&o->t_spf_calc->u.sands, NULL) / 1000LL;
      ...
      /* LSAs */
      json_object_int_add (json, "numberOfAsScopedLsa", o->lsdb->count);
      /* Areas */
      json_object_int_add (json, "numberOfAreaInRouter",
                           listcount (o->area_list));

      json_object_int_add (json, "AuthTrailerHigherSeqNo", o->seqnum_h);
  ...
  else
    {
      ...
      vty_out (vty, " SPF timer %s%s\n",
               (event_is_scheduled (o->t_spf_calc) ? "due in " : "is "), buf);
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      ...
      vrf_id_ui =
          (ospf6->vrf_id == VRF_UNKNOWN) ? -1 : (int64_t) ospf6->vrf_id;

      if (uj)
        {
          json_object_int_add (json_vrf, "vrfId", vrf_id_ui);
          json_object_string_addf (json_vrf, "routerId", "%pI4", &router_id);
...
bool
ospf6_is_valid_summary_addr (struct vty *vty, struct prefix *p)
{
       ...
       "Timer interval(in seconds)\n")
{
  ...
  ospf6_external_aggr_delay_timer_set (ospf6, OSPF6_EXTL_AGGR_DEFAULT_DELAY);
  return CMD_SUCCESS;
}
...
static void
ospf6_show_vrf_name (struct vty *vty, struct ospf6 *ospf6, json_object *json)
{
  if (json)
    {
      if (ospf6->vrf_id == VRF_DEFAULT)
        json_object_string_add (json, "vrfName", "default");
      else
        json_object_string_add (json, "vrfName", ospf6->name);
...
ospf6_show_summary_address (struct vty *vty, struct ospf6 *ospf6,
                            json_object *json, bool uj, const char *detail)
{
      ...
      if (uj)
        {
          ...
          if (OSPF6_EXTERNAL_RT_COUNT (aggr) && detail)
            {
              json_object_int_add (json_aggr, "ID", aggr->id);
              json_object_int_add (json_aggr, "Flags", aggr->aggrflags);
              hash_walk (aggr->match_extnl_hash,
                         ospf6_print_json_external_routes_walkcb, json_aggr);
            }
        }
      else
        {
          ...
          (aggr->mtype == DEFAULT_METRIC_TYPE) ? vty_out (vty, "%-16s", "E2")
                                               : vty_out (vty, "%-16s", "E1");
          vty_out (vty, "%-11d",
                   (aggr->metric != -1) ? aggr->metric
                                        : DEFAULT_DEFAULT_METRIC);
          ...
          if (OSPF6_EXTERNAL_RT_COUNT (aggr) && detail)
            {
              ...
              hash_walk (aggr->match_extnl_hash,
                         ospf6_print_vty_external_routes_walkcb, vty);
              vty_out (vty, "\n");
            }
  ...
  if (uj)
    json_object_object_add (json, ospf6->name, json_vrf);
...
static void
ospf6_stub_router_config_write (struct vty *vty, struct ospf6 *ospf6)
{
  if (CHECK_FLAG (ospf6->flag, OSPF6_STUB_ROUTER))
...
static int
ospf6_distance_config_write (struct vty *vty, struct ospf6 *ospf6)
{
  struct route_node *rn;
...
static int
ospf6_asbr_summary_config_write (struct vty *vty, struct ospf6 *ospf6)
{
  struct route_node *rn;
  struct ospf6_external_aggr_rt *aggr;
  char buf[PREFIX2STR_BUFFER];
  ...
  for (ALL_LIST_ELEMENTS (om6->ospf6, node, nnode, ospf6))
    {
      ...
      if (CHECK_FLAG (ospf6->config_flags, OSPF6_SEND_EXTRA_DATA_TO_ZEBRA))
      ...
      if (ospf6->write_oi_count != OSPF6_WRITE_INTERFACE_COUNT_DEFAULT)
...
void
install_element_ospf6_clear_process (void)
{
