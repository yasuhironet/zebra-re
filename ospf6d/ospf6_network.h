...
#define OSPF6_MESSAGE_WRITE_ON(oi)                                            \
  do                                                                          \
    {                                                                         \
      bool list_was_empty = list_isempty (oi->area->ospf6->oi_write_q);       \
      if ((oi)->on_write_q == 0)                                              \
        {                                                                     \
          listnode_add (oi->area->ospf6->oi_write_q, (oi));                   \
          (oi)->on_write_q = 1;                                               \
        }                                                                     \
      if (list_was_empty && ! list_isempty (oi->area->ospf6->oi_write_q))     \
        event_add_write (master, ospf6_write, oi->area->ospf6,                \
                         oi->area->ospf6->fd, &oi->area->ospf6->t_write);     \
    }                                                                         \
  while (0)
