...
int
str2area_id (const char *str, uint32_t *area_id, int *area_id_fmt)
{
  char *ep;
...
void
area_id2str (char *buf, int len, uint32_t area_id, int area_id_fmt)
{
...
static void
ospf6_nssa_default_originate_set (struct ospf6 *ospf6, struct ospf6_area *area,
...
static void
ospf6_nssa_default_originate_unset (struct ospf6 *ospf6,
 ...
 */
struct ospf6_area *
ospf6_area_create (uint32_t area_id, struct ospf6 *o, int df)
{
...
struct ospf6_area *
ospf6_area_lookup (uint32_t area_id, struct ospf6 *ospf6)
{
  struct ospf6_area *oa;
  struct listnode *n;
  ...
  if (use_json)
    {
      ...
      json_object_boolean_add (json_area, "areaIsStub", IS_AREA_STUB (oa));
      json_object_boolean_add (json_area, "areaIsNSSA", IS_AREA_NSSA (oa));
      if (IS_AREA_STUB (oa) || IS_AREA_NSSA (oa))
        {
          json_object_boolean_add (json_area, "areaNoSummary", oa->no_summary);
        }
      ...
      for (ALL_LIST_ELEMENTS_RO (oa->if_list, i, oi))
        json_object_array_add (array_interfaces,
      ...
      if (oa->ts_spf.tv_sec || oa->ts_spf.tv_usec)
        {
          ...
          if (result / TIMER_SECOND_MICRO > 0)
            {
              json_object_int_add (json_area, "spfLastExecutedSecs",
                                   result / TIMER_SECOND_MICRO);

              json_object_int_add (json_area, "spfLastExecutedMicroSecs",
                                   result % TIMER_SECOND_MICRO);
            }
          else
            {
              json_object_int_add (json_area, "spfLastExecutedSecs", 0);
              json_object_int_add (json_area, "spfLastExecutedMicroSecs",
                                   result);
            }
        }
...
void
ospf6_area_config_write (struct vty *vty, struct ospf6 *ospf6)
{
  struct listnode *node;
  struct ospf6_area *oa;
  struct ospf6_route *range;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
    {
      for (range = ospf6_route_head (oa->range_table); range;
        ...
        {
          vty_out (vty, " area %s range %pFX", oa->name, &range->prefix);
          ...
          if (oa->nssa_default_originate.enabled)
            {
              ...
              if (oa->nssa_default_originate.metric_value != -1)
                vty_out (vty, " metric %d",
                         oa->nssa_default_originate.metric_value);
      ...
      for (range = ospf6_route_head (oa->nssa_range_table); range;
           range = ospf6_route_next (range))
        {
          vty_out (vty, " area %s nssa range %pFX", oa->name, &range->prefix);
          ...
          if (CHECK_FLAG (range->flag, OSPF6_ROUTE_DO_NOT_ADVERTISE))
            ...
            {
              if (range->path.u.cost_config != OSPF_AREA_RANGE_COST_UNSPEC)
                vty_out (vty, " cost %u", range->path.u.cost_config);
            }
          vty_out (vty, "\n");
        }
  ...
  if (strmatch (inout, "in"))
    {
      ...
      PREFIX_NAME_IN (area) = XSTRDUP (MTYPE_OSPF6_PLISTNAME, plistname);
    }
  else
    {
      PREFIX_LIST_OUT (area) = plist;
      XFREE (MTYPE_OSPF6_PLISTNAME, PREFIX_NAME_OUT (area));
      PREFIX_NAME_OUT (area) = XSTRDUP (MTYPE_OSPF6_PLISTNAME, plistname);
    }
...
void
ospf6_filter_update (struct access_list *access)
{
  struct ospf6_area *oa;
  struct listnode *n, *node, *nnode;
  struct ospf6 *ospf6;
  ...
  for (ALL_LIST_ELEMENTS (om6->ospf6, node, nnode, ospf6))
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, n, oa))
        {
          if (IMPORT_NAME (oa) && strcmp (IMPORT_NAME (oa), access->name) == 0)
            {
              IMPORT_LIST (oa) =
          ...
          if (EXPORT_NAME (oa) && strcmp (EXPORT_NAME (oa), access->name) == 0)
            {
              EXPORT_LIST (oa) =
                  access_list_lookup (AFI_IP6, EXPORT_NAME (oa));
              update = true;
            }
        }
...
void
ospf6_plist_update (struct prefix_list *plist)
{
  struct listnode *node, *nnode;
  struct ospf6_area *oa;
  struct listnode *n;
      ...
      for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, n, oa))
        {
          ...
          if (PREFIX_NAME_OUT (oa) && ! strcmp (PREFIX_NAME_OUT (oa), name))
            {
              PREFIX_LIST_OUT (oa) =
                  prefix_list_lookup (AFI_IP6, PREFIX_NAME_OUT (oa));
              update = true;
            }
        }
...
static int
ipv6_ospf6_spf_tree_common (struct vty *vty, struct ospf6 *ospf6, bool uj)
{
  struct listnode *node;
  struct ospf6_area *oa;
  struct prefix prefix;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
    {
      ...
      if (route == NULL)
        {
          if (uj)
            {
              json_object_string_add (json, oa->name,
      ...
      if (uj)
        {
          json_object_object_add (json_area, root->name, json_head);
...
static int
show_ospf6_area_spf_tree_common (struct vty *vty, struct cmd_token **argv,
                                 struct ospf6 *ospf6, uint32_t area_id,
                                 int idx_ipv4)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          show_ospf6_area_spf_tree_common (vty, argv, ospf6, area_id,
                                           idx_ipv4);
          if (! all_vrf)
            break;
        }
...
void
ospf6_area_interface_delete (struct ospf6_interface *oi)
{
  struct ospf6_area *oa;
  struct listnode *node, *nnode;
  struct ospf6 *ospf6;
