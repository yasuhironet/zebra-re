...
static inline bool
ospf6_nexthop_is_set (const struct ospf6_nexthop *nh)
{
  return nh->type != 0;
}

static inline bool
ospf6_nexthop_is_same (const struct ospf6_nexthop *nha,
...
static inline void
ospf6_nexthop_clear (struct ospf6_nexthop *nh)
{
  memset (nh, 0, sizeof (*nh));
}

static inline void
ospf6_nexthop_copy (struct ospf6_nexthop *nha, const struct ospf6_nexthop *nhb)
{
...
#define OSPF6_PATH_COST_IS_CONFIGURED(path)                                   \
  (path.u.cost_config != OSPF_AREA_RANGE_COST_UNSPEC)
...
#define ospf6_route_is_identical(ra, rb)                                      \
  ((ra)->type == (rb)->type && prefix_same (&(ra)->prefix, &(rb)->prefix) &&  \
...
extern int ospf6_nexthop_cmp (struct ospf6_nexthop *a,
                              struct ospf6_nexthop *b);
...
#define ospf6_route_copy_nexthops(dst, src)                                   \
  ospf6_copy_nexthops (dst->nh_list, src->nh_list)
...
extern void ospf6_route_show_detail (struct vty *vty,
                                     struct ospf6_route *route,
                                     json_object *json, bool use_json);
