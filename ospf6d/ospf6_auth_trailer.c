...
void
ospf6_auth_hdr_dump_send (struct ospf6_header *ospfh, uint16_t length)
{
  ...
  if (at_len > 0)
    {
      ...
      zlog_debug ("  seqnum high 0x%08x", htonl (ospf6_at_hdr->seqnum_h));
      zlog_debug ("  seqnum high 0x%08x", htonl (ospf6_at_hdr->seqnum_l));
      zlog_debug ("  Data %s", temp);
    }
}

void
ospf6_auth_hdr_dump_recv (struct ospf6_header *ospfh, uint16_t length,
  ...
  if (at_len > 0)
    {
      ...
      zlog_debug ("  seqnum high 0x%08x", ntohl (ospf6_at_hdr->seqnum_h));
      zlog_debug ("  seqnum high 0x%08x", ntohl (ospf6_at_hdr->seqnum_l));
      zlog_debug ("  Data %s", temp);
    }
}

unsigned char *
ospf6_hash_message_xor (unsigned char *mes1, unsigned char *mes2, uint32_t len)
{
...
static void
md5_digest (unsigned char *mes, uint32_t len, unsigned char *digest)
{
...
static void
sha256_digest (unsigned char *mes, uint32_t len, unsigned char *digest)
{
#ifdef CRYPTO_OPENSSL
...
#ifdef CRYPTO_OPENSSL
static void
sha1_digest (unsigned char *mes, uint32_t len, unsigned char *digest)
{
  EVP_MD_CTX *ctx;
...
static void
sha384_digest (unsigned char *mes, uint32_t len, unsigned char *digest)
{
  EVP_MD_CTX *ctx;
...
static void
sha512_digest (unsigned char *mes, uint32_t len, unsigned char *digest)
{
  EVP_MD_CTX *ctx;
...
static void
ospf6_hash_hmac_sha_digest (enum keychain_hash_algo key, unsigned char *mes,
...
uint16_t
ospf6_auth_len_get (struct ospf6_interface *oi)
{
  uint16_t at_len = 0;
          ...
          if (keychain)
            {
              ...
              if (key && key->string && key->hash_algo != KEYCHAIN_ALGO_NULL)
                {
                  at_len = OSPF6_AUTH_HDR_MIN_SIZE +
                           keychain_get_hash_len (key->hash_algo);
                }
            }
        }
    }
  else if (CHECK_FLAG (oi->at_data.flags, OSPF6_AUTH_TRAILER_MANUAL_KEY))
    {
      at_len = OSPF6_AUTH_HDR_MIN_SIZE +
               keychain_get_hash_len (oi->at_data.hash_algo);
    }
...
int
ospf6_auth_validate_pkt (struct ospf6_interface *oi, unsigned int *pkt_len,
                         ...
                         unsigned int *lls_block_len)
{
  ...
  switch (oh->type)
    ...
    default:
      if (IS_OSPF6_DEBUG_AUTH_RX)
        zlog_err ("RECV[%s] : Wrong packet type %d", oi->interface->name,
                  oh->type);
      return OSPF6_AUTH_VALIDATE_FAILURE;
    }
          ...
          if (IS_OSPF6_DEBUG_AUTH_RX)
            {
              ...
              zlog_err (
                  ...
                  on->seqnum_l[oh->type], on->seqnum_h[oh->type], oh_seqnum_l,
                  oh_seqnum_h);
            }
...
int
ospf6_auth_check_digest (struct ospf6_header *oh, struct ospf6_interface *oi,
                         struct in6_addr *src, unsigned int lls_block_len)
{
  ...
  ospf6_auth =
      (struct ospf6_auth_hdr *) ((uint8_t *) oh + (oh_len + lls_block_len));
  if (CHECK_FLAG (oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN))
    {
      ...
      if (! keychain)
        {
          if (IS_OSPF6_DEBUG_AUTH_RX)
            zlog_err ("RECV[%s]: Keychain doesn't exist for %s",
                      oi->interface->name, ospf6_message_type (oh->type));
          return OSPF6_AUTH_VALIDATE_FAILURE;
        }
      ...
      if (! key)
        {
          if (IS_OSPF6_DEBUG_AUTH_RX)
            zlog_err ("RECV[%s]: Auth, Invalid SA for %s", oi->interface->name,
                      ospf6_message_type (oh->type));
          return OSPF6_AUTH_VALIDATE_FAILURE;
        }
      ...
      if (key && key->string && key->hash_algo != KEYCHAIN_ALGO_NULL)
        ...
        {
          if (IS_OSPF6_DEBUG_AUTH_RX)
            zlog_err ("RECV[%s]: Incomplete keychain config for %s",
                      oi->interface->name, ospf6_message_type (oh->type));
          return OSPF6_AUTH_VALIDATE_FAILURE;
        }
    }
  ...
  ospf6_auth_update_digest (oi, oh, ospf6_auth, auth_str,
                            (oh_len + auth_len + lls_block_len), hash_algo);
...
void
ospf6_auth_digest_send (struct in6_addr *src, struct ospf6_interface *oi,
                        ...
                        uint32_t pkt_len)
{
  ...
  if ((ospf6->seqnum_l == 0xFFFFFFFF) && (ospf6->seqnum_h == 0xFFFFFFFF))
    {
      ospf6->seqnum_l = 0;
      ospf6->seqnum_h = 0;
      zlog_err (
  ...
  ospf6_auth = (struct ospf6_auth_hdr *) ((uint8_t *) oh + ntohs (oh->length));
  ...
  ospf6_auth_update_digest (oi, oh, ospf6_auth, auth_str, pkt_len, hash_algo);
  ...
  if (CHECK_FLAG (oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN_VALID))
    {
      ...
      oi->at_data.key_id = 0;
      UNSET_FLAG (oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN_VALID);
    }
}

void
ospf6_auth_update_digest (struct ospf6_interface *oi, struct ospf6_header *oh,
...
int
config_write_ospf6_debug_auth (struct vty *vty)
{
  if (IS_OSPF6_DEBUG_AUTH_TX)
...
void
install_element_ospf6_debug_auth (void)
{
...
static void
ospf6_intf_auth_clear (struct vty *vty, struct interface *ifp)
{
  ...
  if (IS_OSPF6_DEBUG_INTERFACE)
    zlog_debug ("Interface %s: clear authentication rx/tx drop counters",
                ifp->name);
...
void
install_element_ospf6_clear_intf_auth (void)
{
 ...
 */
void
ospf6_auth_seqno_nvm_update (struct ospf6 *ospf6)
{
...
ospf6_auth_seqno_nvm_delete (struct ospf6 *ospf6)
{
  ...
  zlog_err ("Higher order sequence number delete for %s process", ospf6->name);
 ...
 */
void
ospf6_auth_seqno_nvm_read (struct ospf6 *ospf6)
{
  const char *inst_name;
  json_object *json;
  json_object *json_instances;
  ...
  json_object_object_get_ex (json_instance, "sequence_number", &json_seqnum);
