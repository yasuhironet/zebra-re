...
static uint8_t *ospfv3NbrEntry (struct variable *, oid *, size_t *, int,
                                size_t *, WriteMethod **);
static uint8_t *ospfv3IfEntry (struct variable *, oid *, size_t *, int,
                               size_t *, WriteMethod **);
...
static uint8_t *
ospfv3GeneralGroup (struct variable *v, oid *name, size_t *length, int exact,
                    size_t *var_len, WriteMethod **write_method)
{
  ...
  switch (v->magic)
    ...
    case OSPFv3AREABDRRTRSTATUS:
      if (ospf6)
        return SNMP_INTEGER (
            ospf6_check_and_set_router_abr (ospf6) ? SNMP_TRUE : SNMP_FALSE);
      return SNMP_INTEGER (SNMP_FALSE);
    case OSPFv3ASBDRRTRSTATUS:
      if (ospf6)
        return SNMP_INTEGER (ospf6_asbr_is_asbr (ospf6) ? SNMP_TRUE
                                                        : SNMP_FALSE);
      return SNMP_INTEGER (SNMP_FALSE);
    ...
    case OSPFv3EXTLSACOUNT:
      if (ospf6)
        {
          count = 0;
          for (ALL_LSDB_TYPED (ospf6->lsdb, htons (OSPF6_LSTYPE_AS_EXTERNAL),
                               lsa))
            count += 1;
...
static uint8_t *
ospfv3AreaEntry (struct variable *v, oid *name, size_t *length, int exact,
                 size_t *var_len, WriteMethod **write_method)
{
  ...
  zlog_debug ("SNMP access by area: %s, exact=%d len=%d length=%lu", a, exact,
  ...
  *length = v->namelen + 1;
  ...
  zlog_debug ("SNMP found area: %s, exact=%d len=%d length=%lu", a, exact, len,
...
static uint8_t *
ospfv3WwLsdbEntry (struct variable *v, oid *name, size_t *length, int exact,
                   size_t *var_len, WriteMethod **write_method)
{
  struct vrf *vrf;
  ...
  *offset = ntohl (lsa->header->id);
  ...
  switch (v->magic & OSPFv3WWCOLUMN)
    ...
    case OSPFv3WWLSDBTYPEKNOWN:
      return SNMP_INTEGER (
          OSPF6_LSA_IS_KNOWN (lsa->header->type) ? SNMP_TRUE : SNMP_FALSE);
    }
  return NULL;
}

static uint8_t *
ospfv3IfEntry (struct variable *v, oid *name, size_t *length, int exact,
               size_t *var_len, WriteMethod **write_method)
{
  struct vrf *vrf;
  ifindex_t ifindex = 0;
      ...
      for (ALL_LIST_ELEMENTS_RO (ifslist, i, iif))
        {
          ...
          if (! oi)
            continue;
          if (iif->ifindex > ifindex ||
              (iif->ifindex == ifindex && (oi->instance_id > instid)))
            break;
          oi = NULL;
        }
...
static uint8_t *
ospfv3NbrEntry (struct variable *v, oid *name, size_t *length, int exact,
                size_t *var_len, WriteMethod **write_method)
{
  struct vrf *vrf;
  ifindex_t ifindex = 0;
...
static int
ospf6TrapNbrStateChange (struct ospf6_neighbor *on, int next_state,
                         int prev_state)
{
  oid index[3];
  ...
  if ((next_state != OSPF6_NEIGHBOR_FULL) &&
      (next_state != OSPF6_NEIGHBOR_TWOWAY) && (next_state >= prev_state))
    return 0;
  ...
  smux_trap (ospfv3_variables, array_size (ospfv3_variables), ospfv3_trap_oid,
             array_size (ospfv3_trap_oid), ospfv3_oid,
...
static int
ospf6TrapIfStateChange (struct ospf6_interface *oi, int next_state,
                        int prev_state)
{
  oid index[2];
...
/* Register OSPFv3-MIB. */
static int
ospf6_snmp_init (struct event_loop *master)
{
  smux_init (master);
