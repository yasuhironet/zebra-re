...
void
ospf6_hello_print (struct ospf6_header *oh, int action)
{
  ...
  if ((IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV) &&
      ...
      (IS_OSPF6_DEBUG_MESSAGE (oh->type, SEND) && action == OSPF6_ACTION_SEND))
    {
...
void
ospf6_dbdesc_print (struct ospf6_header *oh, int action)
{
  ...
  dbdesc =
      (struct ospf6_dbdesc *) ((caddr_t) oh + sizeof (struct ospf6_header));

  ospf6_options_printbuf (dbdesc->options, options, sizeof (options));

  zlog_debug ("    MBZ: %#x Option: %s IfMTU: %hu", dbdesc->reserved1, options,
              ntohs (dbdesc->ifmtu));
...
void
ospf6_lsreq_print (struct ospf6_header *oh, int action)
{
  char *p;
      ...
      (IS_OSPF6_DEBUG_MESSAGE (oh->type, SEND) && action == OSPF6_ACTION_SEND))
    {
      ...
      for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header));
           ...
           p += sizeof (struct ospf6_lsreq_entry))
        {
          ...
          zlog_debug ("    [%s Id:%pI4 Adv:%pI4]", ospf6_lstype_name (e->type),
                      &e->id, &e->adv_router);
        }
...
void
ospf6_lsupdate_print (struct ospf6_header *oh, int action)
{
  ...
  lsupdate =
      (struct ospf6_lsupdate *) ((caddr_t) oh + sizeof (struct ospf6_header));
...
void
ospf6_lsack_print (struct ospf6_header *oh, int action)
{
  char *p;
...
static void
ospf6_packet_free (struct ospf6_packet *op)
{
  if (op->s)
    stream_free (op->s);
...
static void
ospf6_fifo_push (struct ospf6_fifo *fifo, struct ospf6_packet *op)
{
  if (fifo->tail)
    fifo->tail->next = op;
  else
...
static void
ospf6_fifo_push_head (struct ospf6_fifo *fifo, struct ospf6_packet *op)
{
  op->next = fifo->head;
...
static struct ospf6_packet *
ospf6_fifo_pop (struct ospf6_fifo *fifo)
{
...
static struct ospf6_packet *
ospf6_fifo_head (struct ospf6_fifo *fifo)
{
  return fifo->head;
}
...
void
ospf6_fifo_flush (struct ospf6_fifo *fifo)
{
  struct ospf6_packet *op;
...
void
ospf6_fifo_free (struct ospf6_fifo *fifo)
{
...
static void
ospf6_packet_add (struct ospf6_interface *oi, struct ospf6_packet *op)
{
...
static void
ospf6_packet_add_top (struct ospf6_interface *oi, struct ospf6_packet *op)
{
...
static void
ospf6_packet_delete (struct ospf6_interface *oi)
{
  struct ospf6_packet *op;
                  ...
                  struct ospf6_interface *oi, struct ospf6_header *oh)
{
  ...
  if (ntohs (hello->hello_interval) != oi->hello_interval)
    {
      zlog_warn (
          ...
          oi->interface->vrf->name, oi->interface->name, src, &oh->router_id,
  ...
  if (OSPF6_OPT_ISSET (hello->options, OSPF6_OPT_N) !=
    ...
    {
      zlog_warn ("VRF %s: IF %s N-bit mismatch", oi->interface->vrf->name,
                 oi->interface->name);
      return;
    }

  if (((OSPF6_OPT_ISSET_EXT (hello->options, OSPF6_OPT_AT) == OSPF6_OPT_AT) &&
       (oi->at_data.flags == 0)) ||
      ((OSPF6_OPT_ISSET_EXT (hello->options, OSPF6_OPT_AT) != OSPF6_OPT_AT) &&
       (oi->at_data.flags != 0)))
    {
      if (IS_OSPF6_DEBUG_AUTH_RX)
        zlog_warn ("VRF %s: IF %s AT-bit mismatch in hello packet",
  ...
  /* TwoWay check */
  for (p = (char *) ((caddr_t) hello + sizeof (struct ospf6_hello));
       p + sizeof (uint32_t) <= OSPF6_MESSAGE_END (oh); p += sizeof (uint32_t))
    {
...
static void
ospf6_dbdesc_recv_master (struct ospf6_header *oh, struct ospf6_neighbor *on)
{
  struct ospf6_dbdesc *dbdesc;
  char *p;
  ...
  switch (on->state)
    ...
    case OSPF6_NEIGHBOR_EXCHANGE:
      ...
      if (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MSBIT))
        {
          zlog_warn ("DbDesc recv: Master/Slave bit mismatch Nbr %s",
                     on->name);
      ...
      if (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_IBIT))
        {
          zlog_warn ("DbDesc recv: Initialize bit mismatch Nbr %s", on->name);
          event_add_event (master, seqnumber_mismatch, on, 0, NULL);
          return;
        }
      ...
      if (memcmp (on->options, dbdesc->options, sizeof (on->options)))
        {
          zlog_warn ("DbDesc recv: Option field mismatch Nbr %s", on->name);
          event_add_event (master, seqnumber_mismatch, on, 0, NULL);
          return;
        }
    ...
    case OSPF6_NEIGHBOR_FULL:
      ...
      zlog_warn ("DbDesc recv: Not duplicate dbdesc in state %s Nbr %s",
  ...
  if (on->request_list->count)
    event_add_event (master, ospf6_lsreq_send, on, 0, &on->thread_send_lsreq);
  ...
  if (! CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MBIT) &&
    ...
    event_add_event (master, exchange_done, on, 0, &on->thread_exchange_done);
  else
    {
...
static void
ospf6_dbdesc_recv_slave (struct ospf6_header *oh, struct ospf6_neighbor *on)
{
  struct ospf6_dbdesc *dbdesc;
  char *p;
  ...
  switch (on->state)
    ...
    case OSPF6_NEIGHBOR_EXSTART:
      ...
      if (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_IBIT) &&
          ...
          ntohs (oh->length) ==
              sizeof (struct ospf6_header) + sizeof (struct ospf6_dbdesc))
        {
    ...
    case OSPF6_NEIGHBOR_EXCHANGE:
      ...
      if (! CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MSBIT))
        {
          zlog_warn ("DbDesc slave recv: Master/Slave bit mismatch Nbr %s",
                     on->name);
          event_add_event (master, seqnumber_mismatch, on, 0, NULL);
          return;
        }
      ...
      if (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_IBIT))
        {
          zlog_warn ("DbDesc slave recv: Initialize bit mismatch Nbr %s",
                     on->name);
          event_add_event (master, seqnumber_mismatch, on, 0, NULL);
          return;
        }
      ...
      if (memcmp (on->options, dbdesc->options, sizeof (on->options)))
        {
          zlog_warn ("DbDesc slave recv: Option field mismatch Nbr %s",
                     on->name);
          event_add_event (master, seqnumber_mismatch, on, 0, NULL);
          return;
        }
    ...
    case OSPF6_NEIGHBOR_FULL:
      ...
      zlog_warn ("DbDesc slave recv: Not duplicate dbdesc in state %s Nbr %s",
                 ospf6_neighbor_state_str[on->state], on->name);
      event_add_event (master, seqnumber_mismatch, on, 0, NULL);
      return;
                   ...
                   struct ospf6_interface *oi, struct ospf6_header *oh)
{
  ...
  if (((OSPF6_OPT_ISSET_EXT (dbdesc->options, OSPF6_OPT_AT) == OSPF6_OPT_AT) &&
       (oi->at_data.flags == 0)) ||
      ((OSPF6_OPT_ISSET_EXT (dbdesc->options, OSPF6_OPT_AT) != OSPF6_OPT_AT) &&
       (oi->at_data.flags != 0)))
    {
      if (IS_OSPF6_DEBUG_AUTH_RX)
        zlog_warn ("VRF %s: IF %s AT-bit mismatch in dbdesc packet",
                   oi->interface->vrf->name, oi->interface->name);
      oi->at_data.rx_drop++;
      return;
    }
  ...
  if (! oi->mtu_ignore && ntohs (dbdesc->ifmtu) != oi->ifmtu)
    {
      zlog_warn ("VRF %s: I/F %s MTU mismatch (my %d rcvd %d)",
                 oi->interface->vrf->name, oi->interface->name, oi->ifmtu,
                 ntohs (dbdesc->ifmtu));
      return;
    }
                  ...
                  struct ospf6_interface *oi, struct ospf6_header *oh)
{
  ...
  if (on->state != OSPF6_NEIGHBOR_EXCHANGE &&
      on->state != OSPF6_NEIGHBOR_LOADING && on->state != OSPF6_NEIGHBOR_FULL)
    {
      if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV_HDR))
  ...
  while (length)
    {
      if (length < OSPF6_PREFIX_MIN_SIZE)
        {
          zlog_warn ("%s: undersized IPv6 prefix header", __func__);
          return MSG_NG;
        }
      /* safe to look deeper */
      if (current->prefix_length > IPV6_MAX_BITLEN)
        {
          zlog_warn ("%s: invalid PrefixLength (%u bits)", __func__,
                     current->prefix_length);
          return MSG_NG;
        }
      /* covers both fixed- and variable-sized fields */
      requested_pfx_bytes =
          OSPF6_PREFIX_MIN_SIZE + OSPF6_PREFIX_SPACE (current->prefix_length);
      ...
      current =
          (struct ospf6_prefix *) ((caddr_t) current + requested_pfx_bytes);
      real_num_pfxs++;
    }
  if (real_num_pfxs != req_num_pfxs)
    {
      zlog_warn ("%s: IPv6 prefix number mismatch (%u required, %u real)",
...
static unsigned
ospf6_lsa_examin (struct ospf6_lsa_header *lsah, const uint16_t lsalen,
  ...
  switch (lsatype)
    ...
    case OSPF6_LSTYPE_ROUTER:
      ...
      if ((lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_ROUTER_LSA_MIN_SIZE) %
        ...
        {
          zlog_warn ("%s: Router LSA interface description alignment error",
                     __func__);
          return MSG_NG;
        }
      break;
    case OSPF6_LSTYPE_NETWORK:
      ...
      if ((lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_NETWORK_LSA_MIN_SIZE) %
          OSPF6_NETWORK_LSDESC_FIX_SIZE)
        {
          zlog_warn ("%s: Network LSA router description alignment error",
                     __func__);
          return MSG_NG;
        }
      break;
    case OSPF6_LSTYPE_INTER_PREFIX:
      ...
      return ospf6_prefixes_examin (
          (struct ospf6_prefix *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE +
                                   OSPF6_INTER_PREFIX_LSA_MIN_SIZE),
          lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTER_PREFIX_LSA_MIN_SIZE, 1);
    case OSPF6_LSTYPE_INTER_ROUTER:
      ...
      if (lsalen > OSPF6_LSA_HEADER_SIZE + OSPF6_INTER_ROUTER_LSA_FIX_SIZE)
        {
          zlog_warn ("%s: Inter Router LSA oversized (%u B) LSA", __func__,
                     lsalen);
          return MSG_NG;
        }
      break;
    ...
    case OSPF6_LSTYPE_TYPE_7:
      ...
      if (headeronly)
        break;
      as_external_lsa =
          (struct ospf6_as_external_lsa *) ((caddr_t) lsah +
      ...
      if (exp_length + OSPF6_PREFIX_MIN_SIZE > lsalen)
        {
          zlog_warn ("%s: AS External undersized (%u B) LSA header", __func__,
                     lsalen);
          return MSG_NG;
        }
      /* forwarding address */
       ...
       */
      return ospf6_prefixes_examin (
          (struct ospf6_prefix *) ((caddr_t) as_external_lsa +
    ...
    case OSPF6_LSTYPE_LINK:
      ...
      if (headeronly)
        break;
      link_lsa =
          (struct ospf6_link_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
      return ospf6_prefixes_examin (
    ...
    case OSPF6_LSTYPE_INTRA_PREFIX:
      ...
      if (headeronly)
        break;
      intra_prefix_lsa =
          (struct ospf6_intra_prefix_lsa *) ((caddr_t) lsah +
                                             OSPF6_LSA_HEADER_SIZE);
      return ospf6_prefixes_examin (
    ...
    case OSPF6_LSTYPE_GRACE_LSA:
      if (lsalen < OSPF6_LSA_HEADER_SIZE + GRACE_PERIOD_TLV_SIZE +
        ...
        {
          if (IS_DEBUG_OSPF6_GR)
            zlog_debug ("%s: Undersized GraceLSA.", __func__);
          return MSG_NG;
        }
    }
  ...
  while (length)
    {
      ...
      if (length < OSPF6_LSA_HEADER_SIZE)
        {
          zlog_warn ("%s: undersized (%zu B) trailing (#%u) LSA header",
      ...
      if (lsalen < OSPF6_LSA_HEADER_SIZE)
        {
          zlog_warn ("%s: malformed LSA header #%u, declared length is %u B",
      ...
      if (headeronly)
        {
          ...
          if (MSG_OK != ospf6_lsa_examin (lsah, lsalen, 1))
            {
              zlog_warn ("%s: anomaly in header-only %s LSA #%u", __func__,
          ...
          if (lsalen > length)
            {
              zlog_warn (
                  ...
                  __func__, ospf6_lstype_name (lsah->type), counted_lsas,
                  lsalen, length);
              return MSG_NG;
            }
...
ospf6_packet_examin (struct ospf6_header *oh, const unsigned bytesonwire)
{
  ...
  if (bytesonwire < OSPF6_HEADER_SIZE)
    {
      zlog_warn ("%s: undersized (%u B) packet", __func__, bytesonwire);
      return MSG_NG;
    }
  ...
  if (bytesonwire != ntohs (oh->length))
    {
      zlog_warn ("%s: %s packet length error (%u real, %u declared)", __func__,
                 ospf6_message_type (oh->type), bytesonwire,
                 ntohs (oh->length));
      return MSG_NG;
    }
  ...
  if (oh->version != OSPFV3_VERSION)
    {
      zlog_warn ("%s: invalid (%u) protocol version", __func__, oh->version);
      return MSG_NG;
    }
  ...
  if (oh->type < OSPF6_MESSAGE_TYPE_ALL && ospf6_packet_minlen[oh->type] &&
    ...
    {
      zlog_warn ("%s: undersized (%u B) %s packet", __func__, bytesonwire,
  ...
  switch (oh->type)
    ...
    case OSPF6_MESSAGE_TYPE_DBDESC:
      ...
      test = ospf6_lsaseq_examin (
          (struct ospf6_lsa_header *) ((caddr_t) oh + OSPF6_HEADER_SIZE +
                                       OSPF6_DB_DESC_MIN_SIZE),
          bytesonwire - OSPF6_HEADER_SIZE - OSPF6_DB_DESC_MIN_SIZE, 1, 0);
      break;
    case OSPF6_MESSAGE_TYPE_LSREQ:
      /* RFC5340 A.3.4, packet header + N>=0 LS description blocks. */
      if (0 == (bytesonwire - OSPF6_HEADER_SIZE - OSPF6_LS_REQ_MIN_SIZE) %
    ...
    case OSPF6_MESSAGE_TYPE_LSUPDATE:
      ...
      test = ospf6_lsaseq_examin (
          (struct ospf6_lsa_header *) ((caddr_t) lsupd +
                                       OSPF6_LS_UPD_MIN_SIZE),
          bytesonwire - OSPF6_HEADER_SIZE - OSPF6_LS_UPD_MIN_SIZE, 0,
    ...
    case OSPF6_MESSAGE_TYPE_LSACK:
      ...
      test = ospf6_lsaseq_examin (
          (struct ospf6_lsa_header *) ((caddr_t) oh + OSPF6_HEADER_SIZE +
                                       OSPF6_LS_ACK_MIN_SIZE),
          bytesonwire - OSPF6_HEADER_SIZE - OSPF6_LS_ACK_MIN_SIZE, 1, 0);
      break;
    default:
...
static int
ospf6_rxpacket_examin (struct ospf6_interface *oi, struct ospf6_header *oh,
  ...
  if (oh->area_id != oi->area->area_id)
    {
      if (oh->area_id == OSPF_AREA_BACKBONE)
        ...
        zlog_warn (
            ...
            oi->interface->vrf->name, oi->interface->name,
            on ? on->name : "null", &oh->router_id, &oi->area->area_id,
            &oh->area_id);
      return MSG_NG;
    }
...
static int
ospf6_read_helper (int sockfd, struct ospf6 *ospf6)
{
  int len;
  ...
  if (oi == NULL || oi->area == NULL ||
    ...
    {
      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV_HDR))
  ...
  if (CHECK_FLAG (oi->flag, OSPF6_INTERFACE_PASSIVE))
    {
      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV_HDR))
        zlog_debug ("%s: Ignore message on passive interface %s", __func__,
                    oi->interface->name);
      return OSPF6_READ_CONTINUE;
    }
  ...
  ret = ospf6_auth_validate_pkt (oi, (uint32_t *) &len, oh, &at_len, &lls_len);
  if (ret == OSPF6_AUTH_VALIDATE_SUCCESS)
    {
      ...
      if (ret == OSPF6_AUTH_VALIDATE_FAILURE)
        {
          if (IS_OSPF6_DEBUG_AUTH_RX)
            zlog_err ("RECV[%s]: OSPF packet auth digest miss-match on %s",
  ...
  if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV_HDR))
    {
      ...
      if ((at_len != 0) && IS_OSPF6_DEBUG_AUTH_RX)
        ospf6_auth_hdr_dump_recv (oh, (len + at_len + lls_len), lls_len);
    }
...
void
ospf6_receive (struct event *thread)
{
  int sockfd;
  struct ospf6 *ospf6;
  int count = 0;
...
static void
ospf6_fill_hdr_checksum (struct ospf6_interface *oi, struct ospf6_packet *op)
{
  struct ipv6_ph ph = {};
  struct ospf6_header *oh;
  void *offset = NULL;
...
static void
ospf6_make_header (uint8_t type, struct ospf6_interface *oi, struct stream *s)
{
  struct ospf6_header *oh;
...
static void
ospf6_fill_header (struct ospf6_interface *oi, struct stream *s,
                   uint16_t length)
{
  struct ospf6_header *oh;
...
static void
ospf6_fill_lsupdate_header (struct stream *s, uint32_t lsa_num)
{
  struct ospf6_header *oh;
...
static void
ospf6_auth_trailer_copy_keychain_key (struct ospf6_interface *oi)
{
  ...
  if (keychain)
    {
      ...
      if (key && key->string && key->hash_algo != KEYCHAIN_ALGO_NULL)
        {
          ...
          oi->at_data.auth_key =
              XSTRDUP (MTYPE_OSPF6_AUTH_MANUAL_KEY, key->string);
          oi->at_data.key_id = key->index;
          SET_FLAG (oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN_VALID);
        }
    }
}
...
static uint16_t
ospf6_make_hello (struct ospf6_interface *oi, struct stream *s)
{
...
static void
ospf6_write (struct event *thread)
{
  ...
  while ((pkt_count < ospf6->write_oi_count) && oi)
    {
      ...
      if (oi->at_data.flags != 0)
        {
          ...
          if (at_len)
            {
              iovector[0].iov_len = ntohs (oh->length) + at_len;
              ospf6_auth_digest_send (oi->linklocal_addr, oi, oh, at_len,
                                      iovector[0].iov_len);
            }
          else
            {
      ...
      if (IS_OSPF6_DEBUG_MESSAGE (oh->type, SEND_HDR))
        {
          zlog_debug ("%s send on %s", ospf6_message_type (oh->type),
                      oi->interface->name);
      ...
      switch (oh->type)
        ...
        case OSPF6_MESSAGE_TYPE_HELLO:
          ...
          /* log if latency exceeds the hello period */
          if (latency > ((int64_t) oi->hello_interval * 1000000))
            zlog_warn ("%s hello TX high latency %" PRId64 "us.", __func__,
                       latency);
  ...
  if (! list_isempty (ospf6->oi_write_q))
    event_add_write (master, ospf6_write, ospf6, ospf6->fd, &ospf6->t_write);
}

void
ospf6_hello_send (struct event *thread)
{
  struct ospf6_interface *oi;
  struct ospf6_packet *op;
...
static uint16_t
ospf6_make_dbdesc (struct ospf6_neighbor *on, struct stream *s)
{
      ...
      for (ALL_LSDB (on->dbdesc_list, lsa, lsanext))
        {
          ...
          /* MTU check */
          if ((length + sizeof (struct ospf6_lsa_header) + OSPF6_HEADER_SIZE) >
          ...
          stream_put (s, lsa->header, sizeof (struct ospf6_lsa_header));
...
void
ospf6_dbdesc_send (struct event *thread)
{
  struct ospf6_neighbor *on;
  uint16_t length = OSPF6_HEADER_SIZE;
  struct ospf6_packet *op;
  ...
  if (on->state < OSPF6_NEIGHBOR_EXSTART)
    {
      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_DBDESC, SEND))
        zlog_debug ("Quit to send DbDesc to neighbor %s state %s", on->name,
  ...
  if (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MSBIT))
    event_add_timer (master, ospf6_dbdesc_send, on,
                     on->ospf6_if->rxmt_interval, &on->thread_send_dbdesc);
...
void
ospf6_dbdesc_send_newone (struct event *thread)
{
  struct ospf6_neighbor *on;
  struct ospf6_lsa *lsa, *lsanext;
  unsigned int size = 0;
...
static uint16_t
ospf6_make_lsreq (struct ospf6_neighbor *on, struct stream *s)
{
  uint16_t length = 0;
  ...
  for (ALL_LSDB (on->request_list, lsa, lsanext))
    {
      if ((length + OSPF6_HEADER_SIZE) > ospf6_packet_max (on->ospf6_if))
        {
          ospf6_lsa_unlock (lsa);
          if (lsanext)
            ospf6_lsa_unlock (lsanext);
...
static uint16_t
ospf6_make_lsack_neighbor (struct ospf6_neighbor *on, struct ospf6_packet **op)
{
  uint16_t length = 0;
  struct ospf6_lsa *lsa, *lsanext;
  int lsa_cnt = 0;
          ...
          if (lsa_cnt)
            {
              ...
              /* new packet */
              *op = ospf6_packet_new (on->ospf6_if->ifmtu);
              ospf6_make_header (OSPF6_MESSAGE_TYPE_LSACK, on->ospf6_if,
                                 (*op)->s);
              length = 0;
              lsa_cnt = 0;
            }
...
void
ospf6_lsreq_send (struct event *thread)
{
  struct ospf6_neighbor *on;
  struct ospf6_packet *op;
  uint16_t length = OSPF6_HEADER_SIZE;
  ...
  if (on->state != OSPF6_NEIGHBOR_EXCHANGE &&
      on->state != OSPF6_NEIGHBOR_LOADING)
    {
      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSREQ, SEND_HDR))
        zlog_debug ("Quit to send LSReq to neighbor %s state %s", on->name,
                    ospf6_neighbor_state_str[on->state]);
      return;
    }
  ...
  if (on->request_list->count == 0)
    {
      event_add_event (master, loading_done, on, 0, &on->event_loading_done);
      return;
    }
  ...
  if (on->request_list->count != 0)
    {
      event_add_timer (master, ospf6_lsreq_send, on,
                       on->ospf6_if->rxmt_interval, &on->thread_send_lsreq);
    }
}

static void
ospf6_send_lsupdate (struct ospf6_neighbor *on, struct ospf6_interface *oi,
                     struct ospf6_packet *op)
{
  if (on)
    {
...
static uint16_t
ospf6_make_lsupdate_list (struct ospf6_neighbor *on, struct ospf6_packet **op,
                          int *lsa_cnt)
{
  ...
  for (ALL_LSDB (on->lsupdate_list, lsa, lsanext))
    {
      if ((length + OSPF6_LSA_SIZE (lsa->header) + OSPF6_HEADER_SIZE) >
          ospf6_packet_max (on->ospf6_if))
        {
          ...
          *lsa_cnt = 0;
          ospf6_make_header (OSPF6_MESSAGE_TYPE_LSUPDATE, on->ospf6_if,
                             (*op)->s);
          stream_forward_endp ((*op)->s, OSPF6_LS_UPD_MIN_SIZE);
        }
      ospf6_lsa_age_update_to_send (lsa, on->ospf6_if->transdelay);
...
static uint16_t
ospf6_make_ls_retrans_list (struct ospf6_neighbor *on,
...
void
ospf6_lsupdate_send_neighbor (struct event *thread)
{
  struct ospf6_neighbor *on;
  struct ospf6_packet *op;
  uint16_t length = OSPF6_HEADER_SIZE;
  ...
  if (on->lsupdate_list->count != 0)
  ...
  else if (on->retrans_list->count != 0)
    {
      event_add_timer (master, ospf6_lsupdate_send_neighbor, on,
                       on->ospf6_if->rxmt_interval, &on->thread_send_lsupdate);
    }
}

int
ospf6_lsupdate_send_neighbor_now (struct ospf6_neighbor *on,
                                  struct ospf6_lsa *lsa)
{
  struct ospf6_packet *op;
  uint16_t length = OSPF6_HEADER_SIZE;
  ...
  stream_put (op->s, lsa->header, OSPF6_LSA_SIZE (lsa->header));
  length =
      OSPF6_HEADER_SIZE + OSPF6_LS_UPD_MIN_SIZE + OSPF6_LSA_SIZE (lsa->header);
  ospf6_fill_header (on->ospf6_if, op->s, length);
...
static uint16_t
ospf6_make_lsupdate_interface (struct ospf6_interface *oi,
                               struct ospf6_packet **op, int *lsa_cnt)
{
  uint16_t length = OSPF6_LS_UPD_MIN_SIZE;
  struct ospf6_lsa *lsa, *lsanext;
  ...
  for (ALL_LSDB (oi->lsupdate_list, lsa, lsanext))
    {
      if (length + OSPF6_LSA_SIZE (lsa->header) + OSPF6_HEADER_SIZE >
          ospf6_packet_max (oi))
        {
          ospf6_fill_header (oi, (*op)->s, length + OSPF6_HEADER_SIZE);
          (*op)->length = length + OSPF6_HEADER_SIZE;
          ospf6_fill_lsupdate_header ((*op)->s, *lsa_cnt);
...
void
ospf6_lsupdate_send_interface (struct event *thread)
{
  struct ospf6_interface *oi;
  struct ospf6_packet *op;
  uint16_t length = OSPF6_HEADER_SIZE;
...
void
ospf6_lsack_send_neighbor (struct event *thread)
{
  struct ospf6_neighbor *on;
  struct ospf6_packet *op;
  uint16_t length = OSPF6_HEADER_SIZE;
  ...
  if (on->state < OSPF6_NEIGHBOR_EXCHANGE)
    {
      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSACK, SEND_HDR))
        zlog_debug ("Quit to send LSAck to neighbor %s state %s", on->name,
                    ospf6_neighbor_state_str[on->state]);
      return;
    }
...
static uint16_t
ospf6_make_lsack_interface (struct ospf6_interface *oi,
                            struct ospf6_packet *op)
{
  uint16_t length = 0;
  struct ospf6_lsa *lsa, *lsanext;
...
void
ospf6_lsack_send_interface (struct event *thread)
{
  struct ospf6_interface *oi;
  struct ospf6_packet *op;
  uint16_t length = OSPF6_HEADER_SIZE;
  ...
  op->length = length;
  if ((oi->state == OSPF6_INTERFACE_POINTTOPOINT) ||
      (oi->state == OSPF6_INTERFACE_DR) || (oi->state == OSPF6_INTERFACE_BDR))
    op->dst = allspfrouters6;
  else
    op->dst = alldrouters6;
       ...
       "Debug only receiving message, header only\n")
{
  ...
  if (argc == 5)
    level = OSPF6_DEBUG_MESSAGE_SEND | OSPF6_DEBUG_MESSAGE_RECV |
            OSPF6_DEBUG_MESSAGE_SEND_HDR | OSPF6_DEBUG_MESSAGE_RECV_HDR;
  else if (! strncmp (argv[idx_send_recv]->arg, "send-h", 6))
    level = OSPF6_DEBUG_MESSAGE_SEND_HDR;
  else if (! strncmp (argv[idx_send_recv]->arg, "s", 1))
    level = OSPF6_DEBUG_MESSAGE_SEND;
