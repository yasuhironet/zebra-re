  ...
  if (lsa)
    {
      ...
      if ((char *) lsdesc + sizeof (struct ospf6_router_lsdesc) <= end)
        {
...
ospf6_router_is_stub_router (struct ospf6_lsa *lsa)
{
  ...
  if (lsa != NULL && OSPF6_LSA_IS_TYPE (ROUTER, lsa))
    {
      rtr_lsa = (struct ospf6_router_lsa *) ((caddr_t) lsa->header +
                                             sizeof (struct ospf6_lsa_header));
...
void
ospf6_router_lsa_originate (struct event *thread)
{
  ...
  router_lsa = (struct ospf6_router_lsa *) ((caddr_t) lsa_header +
                                            sizeof (struct ospf6_lsa_header));
      ...
      if ((oa->router_lsa_size_limit != 0) &&
           ...
           oa->router_lsa_size_limit))
        {
          if ((caddr_t) lsdesc ==
            ...
            {
              zlog_warn ("Size limit setting for Router-LSA too short");
              return;
            }
          ...
          /* describe links for each interfaces */
          lsdesc =
              (struct ospf6_router_lsdesc *) ((caddr_t) router_lsa +
                                              sizeof (
                        ...
                        json_object *json_obj, bool use_json)
{
  ...
  for (current = start; current + sizeof (struct ospf6_network_lsdesc) <= end;
       current += sizeof (struct ospf6_network_lsdesc))
    {
      ...
      if (use_json)
        json_object_array_add (json_arr, json_object_new_string (buf));
      else
...
void
ospf6_network_lsa_originate (struct event *thread)
{
  struct ospf6_interface *oi;
  ...
  lsdesc = (struct ospf6_network_lsdesc *) ((caddr_t) network_lsa +
...
void
ospf6_link_lsa_originate (struct event *thread)
{
  struct ospf6_interface *oi;
                                       ...
                                       int buflen, int pos)
{
  ...
  if (lsa)
    {
      intra_prefix_lsa =
          (struct ospf6_intra_prefix_lsa *) ((caddr_t) lsa->header +
                                             sizeof (struct ospf6_lsa_header));
      ...
      start =
          (char *) intra_prefix_lsa + sizeof (struct ospf6_intra_prefix_lsa);
      end = (char *) lsa->header + ntohs (lsa->header->length);
      current = start;
...
void
ospf6_intra_prefix_lsa_originate_stub (struct event *thread)
{
  struct ospf6_area *oa;
      ...
      if (old)
        {
          ospf6_lsa_purge (old);
          /* find previous LSA */
          old_next = ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_INTRA_PREFIX),
  ...
  lsa_header = (struct ospf6_lsa_header *) buffer;
  intra_prefix_lsa =
      (struct ospf6_intra_prefix_lsa *) ((caddr_t) lsa_header +
                                         sizeof (struct ospf6_lsa_header));
  ...
  for (ALL_LIST_ELEMENTS_RO (oa->if_list, i, oi))
    {
      ...
      if (oi->state != OSPF6_INTERFACE_LOOPBACK &&
          oi->state != OSPF6_INTERFACE_POINTTOPOINT && full_count != 0)
        {
          if (IS_OSPF6_DEBUG_ORIGINATE (INTRA_PREFIX))
...
void
ospf6_intra_prefix_lsa_originate_transit (struct event *thread)
{
  struct ospf6_interface *oi;
...
static void
ospf6_intra_prefix_update_route_origin (struct ospf6_route *oa_route,
                                        struct ospf6 *ospf6)
{
  ...
  for (ospf6_route_lock (g_route);
       g_route && ospf6_route_is_prefix (&oa_route->prefix, g_route);
       g_route = nroute)
    {
      ...
      if (ospf6_route_is_same_origin (g_route, oa_route))
        {
          h_path =
              (struct ospf6_path *) listgetdata (listhead (g_route->paths));
          g_route->path.origin.type = h_path->origin.type;
          g_route->path.origin.id = h_path->origin.id;
          g_route->path.origin.adv_router = h_path->origin.adv_router;
          if (nroute)
...
void
ospf6_intra_prefix_route_ecmp_path (struct ospf6_area *oa,
          ...
          if (listcount (old_route->paths))
              ...
              if (route_updated)
                {
                  ...
                  if (old_route->path.origin.id == route->path.origin.id &&
                    ...
                    {
                      ospf6_intra_prefix_update_route_origin (old_route,
                                                              oa->ospf6);
                    }
                  break;
                }
          ...
          else
            {
              ...
              if (oa->route_table->hook_remove)
                ospf6_route_remove (old_route, oa->route_table);
              else
                SET_FLAG (old_route->flag, OSPF6_ROUTE_REMOVE);
              break;
            }
        }
      if (route_updated)
      ...
      if (old_route->path.cost == route->path.cost)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (old_route->paths, anode, o_path))
            {
              ls_entry =
                  ospf6_route_lookup (&o_path->ls_prefix, oa->spf_table);
              if (ls_entry == NULL)
                {
                  if (IS_OSPF6_DEBUG_EXAMIN (INTRA_PREFIX))
              ...
              lsa = ospf6_lsdb_lookup (o_path->origin.type, o_path->origin.id,
                                       o_path->origin.adv_router, oa->lsdb);
              if (lsa == NULL)
                {
                  if (IS_OSPF6_DEBUG_EXAMIN (INTRA_PREFIX))
                    {
                      ...
                      ospf6_linkstate_prefix (o_path->origin.adv_router,
                                              o_path->origin.id, &adv_prefix);
                      zlog_debug ("%s: adv_router %pFX lsa not found",
                                  __func__, &adv_prefix);
                    }
                  continue;
                }
              ...
              if (intra_prefix_lsa->ref_adv_router == oa->ospf6->router_id)
                {
                  ifp =
                      if_lookup_prefix (&old_route->prefix, oa->ospf6->vrf_id);
                }

              if (ifp)
                {
                  /* Nexthop interface found */
                  ospf6_route_add_nexthop (old_route, ifp->ifindex, NULL);
                }
              else
                {
                   ...
                   * link state entry
                   */
                  ospf6_route_merge_nexthops (old_route, ls_entry);
                }
            }
...
ospf6_intra_prefix_lsa_add (struct ospf6_lsa *lsa)
{
  ...
  intra_prefix_lsa =
      (struct ospf6_intra_prefix_lsa *) OSPF6_LSA_HEADER_END (lsa->header);
  ...
  for (current = start; current < end; current += OSPF6_PREFIX_SIZE (op))
    {
          ...
          if (IS_OSPF6_DEBUG_EXAMIN (INTRA_PREFIX))
            {
              ospf6_linkstate_prefix2str (
                  (struct prefix *) OSPF6_PREFIX_BODY (op), buf, sizeof (buf));
              zlog_debug ("%s: Skipping Prefix %s has NU option set", __func__,
                          buf);
            }
          continue;
        }
      ...
      if (direct_connect)
        {
          ifp = if_lookup_prefix (&route->prefix, oa->ospf6->vrf_id);
        }
...
static void
ospf6_intra_prefix_lsa_remove_update_route (struct ospf6_lsa *lsa,
                                            struct ospf6_area *oa,
                                            struct ospf6_route *route)
{
  struct listnode *anode, *anext;
      ...
      for (ospf6_route_lock (route);
           route && ospf6_route_is_prefix (&prefix, route); route = nroute)
        {
          ...
          if (listcount (route->paths) > 1)
            {
              ospf6_intra_prefix_lsa_remove_update_route (lsa, oa, route);
            }
          else
            {
              ...
              if (IS_OSPF6_DEBUG_EXAMIN (INTRA_PREFIX))
                {
                  ...
                  zlog_debug (
                      ...
                      __func__, buf, route->path.type, route->path.cost,
                      listcount (route->paths), listcount (route->nh_list));
                }
...
ospf6_brouter_debug_print (struct ospf6_route *brouter)
{
  ...
  zlog_info ("  memory: prev: %p this: %p next: %p parent rnode: %p",
             ...
             (void *) brouter->rnode);
  zlog_info ("  type: %d prefix: %s installed: %s changed: %s", brouter->type,
             destination, installed, changed);
...
ospf6_intra_brouter_calculation (struct ospf6_area *oa)
{
  ...
  if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID (oa->area_id) ||
    ...
    zlog_debug ("%s: border-router calculation for area %s", __func__,
                oa->name);
  ...
  for (brouter = ospf6_route_head (oa->spf_table); brouter;
       brouter = ospf6_route_next (brouter))
    {
      ...
      if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID (brouter_id) ||
          IS_OSPF6_DEBUG_ROUTE (MEMORY))
        {
          zlog_debug ("%p: transfer: area %s brouter %s", (void *) brouter,
                      oa->name, brouter_name);
          ospf6_brouter_debug_print (brouter);
        }
    }
      ...
      if (brouter->path.type == OSPF6_PATH_TYPE_INTER)
        {
          ...
          ospf6_linkstate_prefix (brouter->path.origin.adv_router, htonl (0),
                                  &adv_prefix);
              ...
              if (IS_OSPF6_DEBUG_BROUTER)
                {
                  zlog_debug (
                      ...
                      __func__, brouter_name, brouter->path.origin.adv_router);
                  ospf6_brouter_debug_print (brouter);
                }
      ...
      if (CHECK_FLAG (brouter->flag, OSPF6_ROUTE_REMOVE))
               ...
               CHECK_FLAG (brouter->flag, OSPF6_ROUTE_CHANGE))
        {
          if (IS_OSPF6_DEBUG_BROUTER ||
              IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID (brouter_id) ||
              IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID (oa->area_id))
            zlog_info ("%s: brouter %s appears via area %s", __func__,
                       brouter_name, oa->name);
      ...
      else
        {
          if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID (brouter_id) ||
              IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID (oa->area_id))
            zlog_debug ("brouter %s still exists via area %s", brouter_name,
                        oa->name);
  ...
  if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID (oa->area_id) ||
      IS_OSPF6_DEBUG_ROUTE (MEMORY))
    zlog_debug ("%s: border-router calculation for area %s: done", __func__,
                oa->name);
}
...
config_write_ospf6_debug_brouter (struct vty *vty)
{
  ...
  if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER)
    {
      inet_ntop (AF_INET, &conf_debug_ospf6_brouter_specific_router_id, buf,
                 sizeof (buf));
      vty_out (vty, "debug ospf6 border-routers router-id %s\n", buf);
    }
  if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA)
    {
      inet_ntop (AF_INET, &conf_debug_ospf6_brouter_specific_area_id, buf,
                 sizeof (buf));
