...
static int
ospf6_abr_nssa_am_elected (struct ospf6_area *oa)
{
  ...
  for (ALL_LSDB_TYPED (oa->lsdb, type, lsa))
    {
      ...
      router_lsa =
          (struct ospf6_router_lsa *) ((caddr_t) lsa->header +
  ...
  if (best == NULL)
    {
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s: best electable ABR not found", __func__);
      return 0;
    }
  ...
  if (IPV4_ADDR_CMP (best, &oa->ospf6->router_id) <= 0)
    ...
    {
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s: not elected best %pI4, router ID %pI4", __func__,
...
static void
ospf6_flush_translated_lsa (struct ospf6_area *area)
{
  uint16_t type;
  ...
  for (ALL_LSDB_TYPED_ADVRTR (area->lsdb, type, ospf6->router_id, type7))
    {
      type5 = ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_AS_EXTERNAL),
                                 type7->external_lsa_id, ospf6->router_id,
                                 ospf6->lsdb);
...
void
ospf6_abr_nssa_check_status (struct ospf6 *ospf6)
{
  ...
  for (ALL_LIST_ELEMENTS (ospf6->area_list, lnode, nnode, area))
    {
      ...
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s: checking area %s flag %x", __func__, area->name,
                    area->flag);
      ...
      if (! CHECK_FLAG (area->ospf6->flag, OSPF6_FLAG_ABR))
      ...
      else
        {
          ...
          if (area->NSSATranslatorRole == OSPF6_NSSA_ROLE_ALWAYS)
            area->NSSATranslatorState = OSPF6_NSSA_TRANSLATE_ENABLED;
          else
            {
...
static void
ospf6_abr_unapprove_summaries (struct ospf6 *ospf6)
{
...
void
ospf6_asbr_prefix_readvertise (struct ospf6 *ospf6)
{
...
static void
ospf6_abr_announce_aggregates (struct ospf6 *ospf6)
{
  struct ospf6_area *area;
...
void
ospf6_abr_remove_unapproved_summaries (struct ospf6 *ospf6)
{
  struct listnode *node, *nnode;
  struct ospf6_area *area;
  struct ospf6_lsa *lsa;
 ...
 */
static void
ospf6_abr_task (struct ospf6 *ospf6)
{
  if (IS_OSPF6_DEBUG_ABR)
    zlog_debug ("%s : Start", __func__);
...
static void
ospf6_abr_unapprove_translates (struct ospf6 *ospf6)
{
  struct ospf6_lsa *lsa;
  uint16_t type;
...
ospf6_lsa_translated_nssa_new (struct ospf6_area *area,
                               struct ospf6_lsa *type7)
{
  ...
  if (area->NSSATranslatorState == OSPF6_NSSA_TRANSLATE_DISABLED)
    {
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s: Translation disabled for area %s", __func__,
                    area->name);
      return NULL;
    }
  ...
  if (range && ! prefix_same (&prefix, &range->prefix) &&
    ...
    {
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s: LSA %s suppressed by range %pFX of area %s", __func__,
  ...
  extnew = (struct ospf6_as_external_lsa *) ((caddr_t) lsa_header +
                                             sizeof (struct ospf6_lsa_header));
  ext = (struct ospf6_as_external_lsa *) ((caddr_t) (type7->header) +
                                          sizeof (struct ospf6_lsa_header));
  old_ptr = (caddr_t) ((caddr_t) ext + sizeof (struct ospf6_as_external_lsa));
  new_ptr =
      (caddr_t) ((caddr_t) extnew + sizeof (struct ospf6_as_external_lsa));
  ...
  tag_offset = sizeof (*ext) + OSPF6_PREFIX_SPACE (ext->prefix.prefix_length);

  /* Forwarding address */
  if (CHECK_FLAG (ext->bits_metric, OSPF6_ASBR_BIT_F))
    {
      memcpy (new_ptr, (caddr_t) ext + tag_offset, sizeof (struct in6_addr));
  ...
  if (IS_OSPF6_DEBUG_NSSA)
    zlog_debug ("%s: Originated type5 LSA id %pI4", __func__, &lsa_header->id);
  return lsa;
}

/* Delete LSA from retransmission list */
static void
ospf6_ls_retransmit_delete_nbr_as (struct ospf6 *ospf6, struct ospf6_lsa *lsa)
{
  struct listnode *node, *nnode;
  struct ospf6_area *area;
...
struct ospf6_lsa *
ospf6_translated_nssa_refresh (struct ospf6_area *area,
                               struct ospf6_lsa *type7,
                               struct ospf6_lsa *type5)
{
  ...
  if (type5 == NULL)
    {
      ...
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s: try to find translated Type-5 LSA for %s", __func__,
                    type7->name);
  ...
  if (type5)
    {
      if (CHECK_FLAG (type5->flag, OSPF6_LSA_LOCAL_XLT))
        ...
        {
          if (IS_OSPF6_DEBUG_NSSA)
            zlog_debug ("%s: Invalid translated LSA %s", __func__,
                        type5->name);
          return NULL;
        }
    }
  ...
  if (ospf6_lsa_age_current (type7) != OSPF_LSA_MAXAGE)
    {
      if ((new = ospf6_lsa_translated_nssa_new (area, type7)) == NULL)
        {
          if (IS_OSPF6_DEBUG_NSSA)
            zlog_debug ("%s: Could not translate Type-7 for %pI4", __func__,
                        &type7->header->id);
          return NULL;
        }
    }
...
static void
ospf6_abr_translate_nssa (struct ospf6_area *area, struct ospf6_lsa *lsa)
{
  ...
  if (IS_OSPF6_DEBUG_NSSA)
    zlog_debug ("%s : LSA Id %pI4 external ID %pI4, Translating type 7 to 5",
          ...
          if (! CHECK_FLAG (match->flag, OSPF6_ROUTE_REMOVE))
            {
              if (IS_OSPF6_DEBUG_NSSA)
                zlog_debug (
                    ...
                    __func__, &lsa->header->id, &match->prefix, area->name);
  ...
  if (old && ! OSPF6_LSA_IS_MAXAGE (old))
    {
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s : found old translated LSA Id %pI4, skip", __func__,
                    &old->header->id);
...
static void
ospf6_abr_process_nssa_translates (struct ospf6 *ospf6)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
    {
      ...
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("%s : looking at area %pI4", __func__, &oa->area_id);

      type = htons (OSPF6_LSTYPE_TYPE_7);
      for (ALL_LSDB_TYPED (oa->lsdb, type, lsa))
        {
          zlog_debug ("%s : lsa %s , id %pI4 , adv router %pI4", __func__,
                      lsa->name, &lsa->header->id, &lsa->header->adv_router);
...
static void
ospf6_abr_send_nssa_aggregates (struct ospf6 *ospf6)
{
  struct listnode *node;
  struct ospf6_area *area;
  struct ospf6_route *range;
...
static void
ospf6_abr_remove_unapproved_translates (struct ospf6 *ospf6)
{
  struct ospf6_lsa *lsa;
  uint16_t type;
...
static void
ospf6_abr_nssa_type_7_default_create (struct ospf6 *ospf6,
                                      struct ospf6_area *oa)
{
  ...
  if (IS_OSPF6_DEBUG_NSSA)
    zlog_debug ("Announcing Type-7 default route into NSSA area %s", oa->name);
...
static void
ospf6_abr_nssa_type_7_default_delete (struct ospf6 *ospf6,
                                      struct ospf6_area *oa)
{
  struct ospf6_lsa *lsa;
  ...
  if (lsa && ! OSPF6_LSA_IS_MAXAGE (lsa))
    {
      if (IS_OSPF6_DEBUG_NSSA)
        zlog_debug ("Withdrawing Type-7 default route from area %s", oa->name);
...
void
ospf6_abr_nssa_type_7_defaults (struct ospf6 *ospf6)
{
  struct listnode *node;
  struct ospf6_area *oa;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
    {
      if (IS_AREA_NSSA (oa) && oa->nssa_default_originate.enabled &&
          (IS_OSPF6_ABR (ospf6) ||
           (IS_OSPF6_ASBR (ospf6) && ospf6->nssa_default_import_check.status)))
...
static void
ospf6_abr_nssa_task (struct ospf6 *ospf6)
{
  if (IS_OSPF6_DEBUG_NSSA)
...
int
ospf6_redistribute_check (struct ospf6 *ospf6, struct ospf6_route *route,
                          int type)
{
...
static void
ospf6_abr_task_timer (struct event *thread)
{
...
void
ospf6_schedule_abr_task (struct ospf6 *ospf6)
{
...
static void
ospf6_nssa_flush_area (struct ospf6_area *area)
{
  uint16_t type;
...
static void
ospf6_check_and_originate_type7_lsa (struct ospf6_area *area)
{
   ...
   */
  for (rn = route_top (area->ospf6->rt_aggr_tbl); rn; rn = route_next (rn))
    {
      if (! rn->info)
        continue;
      ...
      if (CHECK_FLAG (aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_ORIGINATED))
        {
          if (IS_OSPF6_DEBUG_NSSA)
            zlog_debug ("Originating Type-7 LSAs for area %s", area->name);
...
static void
ospf6_ase_lsa_refresh (struct ospf6 *o)
{
  struct ospf6_lsa *old;

  for (struct ospf6_route *route = ospf6_route_head (o->external_table); route;
       route = ospf6_route_next (route))
    {
      old = ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_AS_EXTERNAL),
                               route->path.origin.id, o->router_id, o->lsdb);
      if (old)
        {
          EVENT_OFF (old->refresh);
          event_add_event (master, ospf6_lsa_refresh, old, 0, &old->refresh);
        }
      else
        {
...
void
ospf6_area_nssa_update (struct ospf6_area *area)
{
  if (IS_AREA_NSSA (area))
    {
  ...
  else
    {
      ...
      if (ospf6_check_and_set_router_abr (area->ospf6))
      ...
      else
        {
           ...
           */
          if (IS_OSPF6_DEBUG_NSSA)
            zlog_debug ("Refresh type-5 LSAs, area %s", area->name);

          type = htons (OSPF6_LSTYPE_AS_EXTERNAL);
          for (ALL_LSDB_TYPED_ADVRTR (area->ospf6->lsdb, type,
                                      area->ospf6->router_id, lsa))
            {
              if (IS_OSPF6_DEBUG_NSSA)
...
int
ospf6_area_nssa_set (struct ospf6 *ospf6, struct ospf6_area *area)
{
...
int
ospf6_area_nssa_unset (struct ospf6 *ospf6, struct ospf6_area *area)
{
  if (IS_AREA_NSSA (area))
    {
      ...
      area->nssa_range_table = OSPF6_ROUTE_TABLE_CREATE (AREA, PREFIX_RANGES);
...
ospf6_get_nssa_fwd_addr (struct ospf6_area *oa)
{
  struct listnode *node, *nnode;
...
void
ospf6_nssa_lsa_originate (struct ospf6_route *route, struct ospf6_area *area,
                          bool p_bit)
{
  char buffer[OSPF6_MAX_LSASIZE];
  struct ospf6_lsa_header *lsa_header;
  ...
  /* External Route Tag */
  if (info && CHECK_FLAG (as_external_lsa->bits_metric, OSPF6_ASBR_BIT_T))
    {
  ...
  lsa_header->seqnum = ospf6_new_ls_seqnum (
      lsa_header->type, lsa_header->id, lsa_header->adv_router, area->lsdb);
...
void
ospf6_abr_check_translate_nssa (struct ospf6_area *area, struct ospf6_lsa *lsa)
{
  ...
  type5 =
      ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_AS_EXTERNAL),
                         lsa->external_lsa_id, ospf6->router_id, ospf6->lsdb);
  if (! type5)
    {
      if (IS_OSPF6_DEBUG_NSSA)
       ...
       "Advertised metric for this range\n")
{
  ...
  range = ospf6_route_lookup ((struct prefix *) prefix, oa->nssa_range_table);
  if (range == NULL)
    {
...
void
config_write_ospf6_debug_nssa (struct vty *vty)
{
  if (IS_OSPF6_DEBUG_NSSA)
...
void
install_element_ospf6_debug_nssa (void)
{
