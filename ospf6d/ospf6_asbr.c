...
DEFINE_MTYPE_STATIC (OSPF6D, OSPF6_REDISTRIBUTE,
...
ospf6_originate_type5_type7_lsas (struct ospf6_route *route,
                                  struct ospf6 *ospf6)
{
                                 ...
                                 struct ospf6 *ospf6)
{
  ...
  if (IS_OSPF6_DEBUG_ASBR || IS_OSPF6_DEBUG_ORIGINATE (AS_EXTERNAL))
    zlog_debug ("Originate AS-External-LSA for %pFX", &route->prefix);
  ...
  lsa_header->seqnum = ospf6_new_ls_seqnum (
      lsa_header->type, lsa_header->id, lsa_header->adv_router, ospf6->lsdb);
...
void
ospf6_orig_as_external_lsa (struct event *thread)
{
  ...
  for (ALL_LSDB_TYPED_ADVRTR (oi->area->ospf6->lsdb, type, adv_router, lsa))
    {
      if (IS_OSPF6_DEBUG_ASBR)
        zlog_debug ("%s: Send update of AS-External LSA %s seq 0x%x", __func__,
...
ospf6_as_external_lsa_get_tag (struct ospf6_lsa *lsa)
{
  ...
  external =
      (struct ospf6_as_external_lsa *) OSPF6_LSA_HEADER_END (lsa->header);

  if (! CHECK_FLAG (external->bits_metric, OSPF6_ASBR_BIT_T))
    return 0;

  tag_offset =
      sizeof (*external) + OSPF6_PREFIX_SPACE (external->prefix.prefix_length);
...
void
ospf6_asbr_update_route_ecmp_path (struct ospf6_route *old,
      ...
      for (ALL_LIST_ELEMENTS (old_route->paths, anode, anext, o_path))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (o_path->nh_list, nnode, nh))
            {
              for (ALL_LIST_ELEMENTS (old_route->nh_list, rnode, rnext, rnh))
                {
          ...
          if (listcount (old_route->paths))
            {
              for (ALL_LIST_ELEMENTS (old_route->paths, anode, anext, o_path))
                {
                  ospf6_merge_nexthops (old_route->nh_list, o_path->nh_list);
                }
              ...
              if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
                {
                  zlog_debug (
                      ...
                      __func__, &old_route->prefix, old_route->path.cost,
                      route->path.cost);
                }
              if (old == old_route)
                old = next_route;
              ospf6_route_remove (old_route, ospf6->route_table);
            }
        }
      if (route_updated)
        break;
  ...
  for (old_route = old; old_route; old_route = old_route->next)
    {
      ...
      if ((old_route->path.cost == route->path.cost) &&
          (old_route->path.u.cost_e2 == route->path.u.cost_e2))
        {
          ...
          if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
            {
              zlog_debug ("%s: old route %pFX path  cost %u e2 %u", __func__,
                          &old_route->prefix, old_route->path.cost,
              ...
              if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
                {
                  zlog_debug (
                      ...
                      __func__, &route->prefix, listcount (ecmp_path->nh_list),
          ...
          for (ALL_LIST_ELEMENTS_RO (old_route->paths, anode, o_path))
            {
              ...
              asbr_entry = ospf6_route_lookup (&o_path->ls_prefix,
                                               ospf6->brouter_table);
              if (asbr_entry == NULL)
                {
                  if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
                    zlog_debug ("%s: ls_prfix %pFX asbr_entry not found.",
          ...
          if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
            zlog_debug ("%s: route %pFX with effective paths %u nh %u",
...
static int
ospf6_ase_forward_address_check (struct ospf6 *ospf6,
      ...
      for (ALL_LIST_ELEMENTS_RO (oa->if_list, node, oi))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (ifp->connected, cnode, c))
            {
              if (IPV6_ADDR_SAME (&c->address->u.prefix6, fwd_addr))
                return 0;
            }
        }
    }
...
ospf6_asbr_lsa_add (struct ospf6_lsa *lsa)
{
  ...
  if (asbr_entry == NULL)
  ...
  else
    {
      ...
      if (! CHECK_FLAG (asbr_entry->path.router_bits, OSPF6_ROUTER_BIT_E))
        {
          if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
            zlog_debug ("External bit reset ASBR route entry : %pFX",
                        &asbr_id);
          return;
        }
      ...
      if (ntohs (lsa->header->type) == OSPF6_LSTYPE_TYPE_7 &&
        ...
        {
          if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
            zlog_debug ("Intra-area route to NSSA ASBR not found: %pFX",
                        &asbr_id);
          return;
        }
    }
  ...
  if (ntohs (lsa->header->type) == OSPF6_LSTYPE_TYPE_7 &&
      ...
      (CHECK_FLAG (external->prefix.prefix_options, OSPF6_PREFIX_OPTION_P) ||
       oa->no_summary))
    {
      if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
      ...
      if (! IN6_IS_ADDR_UNSPECIFIED (&fwd_addr.u.prefix6))
        {
          if (! ospf6_ase_forward_address_check (ospf6, &fwd_addr.u.prefix6))
            {
              if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
                zlog_debug ("Fwd address %pFX is local address", &fwd_addr);
              return;
            }

          /* Find the forwarding entry */
          asbr_entry =
              ospf6_route_lookup_bestmatch (&fwd_addr, ospf6->route_table);
          if (asbr_entry == NULL)
            {
              if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
                zlog_debug ("Fwd address not found: %pFX", &fwd_addr);
              return;
            }
        }
    }
  ...
  if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
    zlog_debug ("%s: %s %u route add %pFX cost %u(%u) nh %u", __func__,
                (type == OSPF6_LSTYPE_AS_EXTERNAL) ? "AS-External" : "NSSA",
  ...
  if (! old)
  ...
  else
    {
       ...
       */
      if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
        zlog_debug ("%s : old route %pFX cost %u(%u) nh %u", __func__,
                    &route->prefix, route->path.cost, route->path.u.cost_e2,
                    listcount (route->nh_list));
...
void
ospf6_asbr_lsa_remove (struct ospf6_lsa *lsa, struct ospf6_route *asbr_entry)
{
  struct ospf6_as_external_lsa *external;
  ...
  if (type == OSPF6_LSTYPE_TYPE_7)
    {
      if (debug)
        zlog_debug ("%s: Withdraw  Type 7 route for %s", __func__, lsa->name);
      oa = lsa->lsdb->data;
    }
  else
    {
      if (debug)
        zlog_debug ("%s: Withdraw AS-External route for %s", __func__,
                    lsa->name);
      ...
      if (CHECK_FLAG (external->bits_metric, OSPF6_ASBR_BIT_E))
        {
          ...
          route_to_del->path.u.cost_e2 = OSPF6_ASBR_METRIC (external);
        }
      else
        {
  ...
  for (ospf6_route_lock (route);
       route && ospf6_route_is_prefix (&prefix, route); route = nroute)
    {
          ...
          for (ALL_LIST_ELEMENTS (route->paths, anode, anext, o_path))
            {
              ...
              if (asbr_entry &&
                  (o_path->cost != route_to_del->path.cost ||
                   o_path->u.cost_e2 != route_to_del->path.u.cost_e2))
                {
                  if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
                    {
                      zlog_debug (
              ...
              for (ALL_LIST_ELEMENTS_RO (o_path->nh_list, nnode, nh))
                {
                  for (ALL_LIST_ELEMENTS (route->nh_list, rnode, rnext, rnh))
                    {
                      if (! ospf6_nexthop_is_same (rnh, nh))
                        continue;
              ...
              if (listcount (route->paths))
                {
                  ...
                  if (oa->ospf6->route_table->hook_add)
                    (*oa->ospf6->route_table->hook_add) (route);
                  ...
                  if ((route->path.origin.id == lsa->header->id) &&
                       ...
                       lsa->header->adv_router))
                    {
                      ...
                      route->path.origin.type = h_path->origin.type;
                      route->path.origin.id = h_path->origin.id;
                      route->path.origin.adv_router =
                          h_path->origin.adv_router;
                    }
                }
      ...
      else
        {
          ...
          if (asbr_entry &&
              (! ospf6_route_is_same_origin (route, route_to_del) ||
               ...
               (route->path.u.cost_e2 != route_to_del->path.u.cost_e2)))
            {
              if (debug)
                {
                  zlog_debug (
      ...
      if (debug)
        {
          zlog_debug ("%s: AS-External %u route remove %pFX cost %u(%u) nh %u",
                      __func__,
                      route->path.type == OSPF6_PATH_TYPE_EXTERNAL1 ? 1 : 2,
                      &route->prefix, route->path.cost, route->path.u.cost_e2,
                      listcount (route->nh_list));
...
void
ospf6_asbr_lsentry_add (struct ospf6_route *asbr_entry, struct ospf6 *ospf6)
{
  struct ospf6_lsa *lsa;
  uint16_t type;
  uint32_t router;
  ...
  if (! CHECK_FLAG (asbr_entry->flag, OSPF6_ROUTE_BEST))
    {
      char buf[16];
      inet_ntop (AF_INET, &ADV_ROUTER_IN_PREFIX (&asbr_entry->prefix), buf,
                 sizeof (buf));
...
void
ospf6_asbr_lsentry_remove (struct ospf6_route *asbr_entry, struct ospf6 *ospf6)
{
  struct ospf6_lsa *lsa;
  uint16_t type;
  uint32_t router;
...
static void
ospf6_asbr_routemap_set (struct ospf6_redist *red, const char *mapname)
{
...
static void
ospf6_asbr_routemap_unset (struct ospf6_redist *red)
{
  if (ROUTEMAP_NAME (red))
    free (ROUTEMAP_NAME (red));
...
static void
ospf6_asbr_routemap_update_timer (struct event *thread)
{
  ...
  for (type = 0; type < ZEBRA_ROUTE_MAX; type++)
    {
      ...
      if (ROUTEMAP (red))
        {
          if (IS_OSPF6_DEBUG_ASBR)
            zlog_debug ("%s: route-map %s update, reset redist %s", __func__,
...
void
ospf6_asbr_distribute_list_update (struct ospf6 *ospf6,
                                   struct ospf6_redist *red)
{
      ...
      for (type = 0; type < ZEBRA_ROUTE_MAX; type++)
        {
          ...
          if (ROUTEMAP (red))
          ...
          else
            {
               ...
               */
              if (IS_OSPF6_DEBUG_ASBR)
                zlog_debug ("%s: route-map %s deleted, reset redist %s",
                            __func__, mapname, ZROUTE_NAME (type));
...
static void
ospf6_asbr_routemap_event (const char *name)
{
  int type;
  struct listnode *node, *nnode;
  struct ospf6 *ospf6;
...
struct ospf6_redist *
ospf6_redist_lookup (struct ospf6 *ospf6, int type, unsigned short instance)
{
...
static struct ospf6_redist *
ospf6_redist_add (struct ospf6 *ospf6, int type, uint8_t instance)
{
  struct ospf6_redist *red;
...
static void
ospf6_redist_del (struct ospf6 *ospf6, struct ospf6_redist *red, int type)
{
  if (red)
    {
...
void
ospf6_asbr_status_update (struct ospf6 *ospf6, int status)
{
  ...
  if (status)
    {
      if (IS_OSPF6_ASBR (ospf6))
        {
          zlog_info ("ASBR[%s:Status:%d]: Already ASBR", ospf6->name, status);
          return;
        }
  ...
  else
    {
      if (! IS_OSPF6_ASBR (ospf6))
        {
          zlog_info ("ASBR[%s:Status:%d]: Already non ASBR", ospf6->name,
                     status);
          return;
        }
...
static void
ospf6_asbr_redistribute_set (struct ospf6 *ospf6, int type)
{
  ospf6_zebra_redistribute (type, ospf6->vrf_id);
...
static void
ospf6_asbr_redistribute_unset (struct ospf6 *ospf6, struct ospf6_redist *red,
                               int type)
{
  struct ospf6_route *route;
  ...
  for (route = ospf6_route_head (ospf6->external_table); route;
       route = ospf6_route_next (route))
    {
      ...
      ospf6_asbr_redistribute_remove (info->type, 0, &route->prefix, ospf6);
    }
  ...
  for (ALL_LSDB (oa->ospf6->lsdb, lsa, lsanext))
    {
      if (ntohs (lsa->header->type) == OSPF6_LSTYPE_AS_EXTERNAL)
        {
          if (IS_OSPF6_DEBUG_ASBR)
            zlog_debug ("%s: Flooding AS-External LSA %s", __func__,
                        lsa->name);
...
void
ospf6_asbr_remove_externals_from_area (struct ospf6_area *oa)
{
  struct ospf6_lsa *lsa, *lsanext;
  struct listnode *node, *nnode;
...
static void
ospf6_external_lsa_fwd_addr_set (struct ospf6 *ospf6,
...
void
ospf6_asbr_redistribute_add (int type, ifindex_t ifindex,
                             ...
                             struct ospf6 *ospf6)
{
  ...
  if (IS_OSPF6_DEBUG_ASBR)
    zlog_debug ("Redistribute %pFX (%s)", prefix,
                type == DEFAULT_ROUTE ? "default-information-originate"
                                      : ZROUTE_NAME (type));
  ...
  if (ROUTEMAP (red))
    {
      ...
      if (ret == RMAP_DENYMATCH)
        {
          if (IS_OSPF6_DEBUG_ASBR)
            zlog_debug ("Denied by route-map \"%s\"", ROUTEMAP_NAME (red));
          ospf6_asbr_redistribute_remove (type, ifindex, prefix, ospf6);
          return;
        }
    }
...
static void
ospf6_asbr_external_lsa_remove_by_id (struct ospf6 *ospf6, uint32_t id)
{
  struct ospf6_lsa *lsa;

  lsa = ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_AS_EXTERNAL), htonl (id),
                                         ...
                                         struct ospf6_external_aggr_rt *aggr)
{
  ...
  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Flushing Aggregate route (%pFX)", __func__, &aggr->p);
  ...
  if (aggr->route)
    {
      if (IS_OSPF6_DEBUG_AGGR)
        zlog_debug ("%s: Remove the blackhole route", __func__);

      ospf6_zebra_route_update_remove (aggr->route, ospf6);
      if (aggr->route->route_option)
        XFREE (MTYPE_OSPF6_EXTERNAL_INFO, aggr->route->route_option);
                              ...
                              struct ospf6_route *rt)
{
  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug (
        "%s: Unlinking external route(%pFX) from aggregator(%pFX), external route count:%ld",
        __func__, &rt->prefix, &aggr->p, OSPF6_EXTERNAL_RT_COUNT (aggr));
                                ...
                                struct prefix *prefix, struct ospf6 *ospf6)
{
  ...
  if (IS_OSPF6_DEBUG_ASBR)
    zlog_debug ("Removing route from external table %pFX", prefix);
       ...
       "Route map name\n")
{
  ...
  if (! red)
  ...
  else
    {
      ...
      if (red->dmetric.value == metric && red->dmetric.type == metric_type &&
...
int
ospf6_redistribute_config_write (struct vty *vty, struct ospf6 *ospf6)
{
  int type;
  struct ospf6_redist *red;
...
static void
ospf6_redistribute_show_config (struct vty *vty, struct ospf6 *ospf6,
                                json_object *json_array, json_object *json,
                                bool use_json)
{
  int type;
      ...
      if (use_json)
        {
          ...
          json_object_string_add (json_route, "routeType", ZROUTE_NAME (type));
          json_object_int_add (json_route, "numberOfRoutes", nroute[type]);
          json_object_boolean_add (json_route, "routeMapNamePresent",
                                   ROUTEMAP_NAME (red));
        }
      ...
      if (ROUTEMAP_NAME (red))
        {
          if (use_json)
            {
              json_object_string_add (json_route, "routeMapName",
                                      ROUTEMAP_NAME (red));
              json_object_boolean_add (json_route, "routeMapFound",
                                       ROUTEMAP (red));
            }
          else
            vty_out (vty, "    %d: %s with route-map \"%s\"%s\n", nroute[type],
                     ZROUTE_NAME (type), ROUTEMAP_NAME (red),
                     (ROUTEMAP (red) ? "" : " (not found !)"));
        }
      else
        {
          if (! use_json)
...
static void
ospf6_redistribute_default_set (struct ospf6 *ospf6, int originate)
{
  ...
  switch (cur_originate)
    ...
    case DEFAULT_ORIGINATE_ZEBRA:
      zclient_redistribute_default (ZEBRA_REDISTRIBUTE_DEFAULT_DELETE, zclient,
                                    AFI_IP6, ospf6->vrf_id);
      ospf6_asbr_redistribute_remove (DEFAULT_ROUTE, 0, (struct prefix *) &p,
                                      ospf6);
  ...
  switch (originate)
    ...
    case DEFAULT_ORIGINATE_ZEBRA:
      ...
      break;
    case DEFAULT_ORIGINATE_ALWAYS:
      ospf6_asbr_redistribute_add (DEFAULT_ROUTE, 0, (struct prefix *) &p, 0,
                                   &nexthop, 0, ospf6);
      break;
    }
}
       ...
       "Pointer to route-map entries\n")
{
  ...
  if ((sameRtmap) && (red->dmetric.type == mtype) &&
      (red->dmetric.value == mval) && (cur_originate == default_originate))
    return CMD_SUCCESS;
...
static const struct route_map_rule_cmd ospf6_routemap_rule_match_tag_cmd = {
  "tag",
...
static const struct route_map_rule_cmd ospf6_routemap_rule_set_metric_cmd = {
  "metric",
...
/* add "set metric-type" */
DEFUN_YANG (ospf6_routemap_set_metric_type,
            ...
            "OSPF[6] external type 2 metric\n")
{
  ...
  const char *xpath = "./set-action[action='frr-ospf-route-map:metric-type']";
...
DEFUN_YANG (ospf6_routemap_no_set_metric_type,
...
DEFUN_YANG (ospf6_routemap_set_forwarding,
            ...
            "IPv6 Address\n")
{
  ...
  nb_cli_enqueue_change (vty, xpath_value, NB_OP_MODIFY, argv[idx_ipv6]->arg);
  return nb_cli_apply_changes (vty, NULL);
}

/* delete "set forwarding-address" */
DEFUN_YANG (ospf6_routemap_no_set_forwarding,
  ...
  if (lsa)
    {
      ...
      if (pos == 0)
        ...
        {
          in6 = *((struct in6_addr *) ((caddr_t) external +
                                       sizeof (struct ospf6_as_external_lsa) +
                                       OSPF6_PREFIX_SPACE (
                                           external->prefix.prefix_length)));
        }
      if (buf)
        {
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json_obj, "referenceLsType",
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json_route, "metricType", route->path.metric_type);
      json_object_int_add (json_route, "routeCost",
...
void
ospf6_asbr_redistribute_disable (struct ospf6 *ospf6)
{
  int type;
  struct ospf6_redist *red;
...
void
ospf6_asbr_redistribute_reset (struct ospf6 *ospf6)
{
  int type;
  struct ospf6_redist *red;
  ...
  for (type = 0; type <= ZEBRA_ROUTE_MAX; type++)
    {
      ...
      if (type == DEFAULT_ROUTE)
        {
          ospf6_redistribute_default_set (ospf6, ospf6->default_originate);
          continue;
        }
      if (ROUTEMAP_NAME (red))
...
static void
ospf6_default_originate_write (struct vty *vty, struct ospf6 *o)
{
  struct ospf6_redist *red;
...
int
ospf6_distribute_config_write (struct vty *vty, struct ospf6 *o)
{
  if (o == NULL)
    return 0;
...
void
install_element_ospf6_debug_asbr (void)
{
...
/* ASBR Summarisation */
void
ospf6_fill_aggr_route_details (struct ospf6 *ospf6,
                               struct ospf6_external_aggr_rt *aggr)
{
  ...
  rt_aggr->path.cost = ((aggr->metric == -1) ? DEFAULT_DEFAULT_METRIC
...
static void
ospf6_summary_add_aggr_route_and_blackhole (
    struct ospf6 *ospf6, struct ospf6_external_aggr_rt *aggr)
{
...
static void
ospf6_originate_new_aggr_lsa (struct ospf6 *ospf6,
                              struct ospf6_external_aggr_rt *aggr)
{
  ...
  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Originate new aggregate route(%pFX)", __func__, &aggr->p);

  aggr->id = ospf6->external_id++;

  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("Advertise AS-External Id:%pI4 prefix %pFX metric %u",
  ...
  if (lsa)
    {
      if (IS_OSPF6_DEBUG_AGGR)
        zlog_debug ("%s: Set the origination bit for aggregator", __func__);
                                    ...
                                    struct ospf6_external_aggr_rt *aggr)
{
  ...
  if (OSPF6_EXTERNAL_RT_COUNT (aggr) == 0)
    {
      if (IS_OSPF6_DEBUG_AGGR)
        zlog_debug ("%s: No routes present under this aggregation", __func__);
      return;
    }
  ...
  if (! CHECK_FLAG (aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_ORIGINATED))
    {
      if (IS_OSPF6_DEBUG_AGGR)
        zlog_debug ("%s: Now it is advertisable", __func__);
                             ...
                             struct ospf6_route *rt)
{
  ...
  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Prepare to originate Summary route(%pFX)", __func__,
                &aggr->p);
  ...
  aggr_lsa =
      ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_AS_EXTERNAL), htonl (aggr->id),
                         ospf6->router_id, ospf6->lsdb);

  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Aggr LSA ID: %d flags %x.", __func__, aggr->id,
                aggr->aggrflags);
  ...
  if (CHECK_FLAG (aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_NO_ADVERTISE))
    {
      ...
      if (lsa)
        {
          if (IS_OSPF6_DEBUG_AGGR)
            zlog_debug ("%s: Purge the external LSA %s.", __func__, lsa->name);
          ospf6_external_lsa_purge (ospf6, lsa);
          info->id = 0;
          rt->path.origin.id = 0;
        }
      ...
      if (aggr_lsa)
        {
          if (IS_OSPF6_DEBUG_AGGR)
            zlog_debug ("%s: Purge the aggr external LSA %s.", __func__,
                        lsa->name);
          ospf6_asbr_summary_remove_lsa_and_route (ospf6, aggr);
        }
  ...
  if (CHECK_FLAG (aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_ORIGINATED))
    {
      if (! aggr_lsa)
        {
          zlog_warn ("%s: Could not refresh/originate %pFX", __func__,
                     &aggr->p);
      ...
      mtype = CHECK_FLAG (external->bits_metric, OSPF6_ASBR_BIT_E) ? 2 : 1;
          ...
          (mtype != aggr->mtype))
        {
          ...
          if (IS_OSPF6_DEBUG_AGGR)
            zlog_debug (
                ...
                __func__, tag, aggr->tag, metric, aggr->metric, mtype,
                aggr->mtype, &aggr->p);

          aggr_lsa = ospf6_originate_type5_type7_lsas (aggr->route, ospf6);
          if (aggr_lsa)
            SET_FLAG (aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_ORIGINATED);
        }
...
static void
ospf6_aggr_handle_external_info (void *data)
{
  ...
  aggr = ospf6_external_aggr_match (ospf6, &rt->prefix);
  if (aggr)
    {
      ...
      if (lsa)
        {
          ...
          event_add_event (master, ospf6_lsa_refresh, lsa, 0, &lsa->refresh);
          return;
        }
    }
...
ospf6_asbr_summary_config_delete (struct ospf6 *ospf6, struct route_node *rn)
{
  ...
  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Deleting Aggregate route (%pFX)", __func__, &aggr->p);
                                   ...
                                   struct ospf6_external_aggr_rt *aggr)
{
  ...
  lsa = ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_AS_EXTERNAL), htonl (aggr->id),
                           ospf6->router_id, ospf6->lsdb);
  if (! lsa)
    {
      zlog_warn ("%s: Could not refresh/originate %pFX", __func__, &aggr->p);
  ...
  mtype = CHECK_FLAG (asel->bits_metric, OSPF6_ASBR_BIT_E) ? 2 : 1;
   ...
   */
  if ((tag != aggr->tag) || (metric != (unsigned int) rt_aggr->path.cost) ||
      (mtype != aggr->mtype))
    {
      if (IS_OSPF6_DEBUG_AGGR)
        zlog_debug (
            ...
            (unsigned int) rt_aggr->path.cost, mtype, aggr->mtype, &aggr->p);

      (void) ospf6_originate_type5_type7_lsas (aggr->route, ospf6);
    }
...
static void
ospf6_handle_external_aggr_update (struct ospf6 *ospf6)
{
      ...
      else if (aggr->action == OSPF6_ROUTE_AGGR_MODIFY)
        {
          ...
          if (CHECK_FLAG (aggr->aggrflags, OSPF6_EXTERNAL_AGGRT_ORIGINATED) &&
                            ...
                            OSPF6_EXTERNAL_AGGRT_NO_ADVERTISE))
            {
              ...
              ret = ospf6_handle_external_aggr_modify (ospf6, aggr);
...
static void
ospf6_aggr_unlink_external_info (void *data)
{
  struct ospf6_route *rt = (struct ospf6_route *) data;
...
void
ospf6_external_aggregator_free (struct ospf6_external_aggr_rt *aggr)
{
  hash_clean_and_free (&aggr->match_extnl_hash,
                       ospf6_aggr_unlink_external_info);

  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Release the aggregator Address(%pFX)", __func__,
                &aggr->p);
...
static void
ospf6_handle_exnl_rt_after_aggr_del (struct ospf6 *ospf6,
                                     struct ospf6_route *rt)
{
  struct ospf6_lsa *lsa;
  ...
  if (lsa)
    ...
    {
      if (IS_OSPF6_DEBUG_AGGR)
        zlog_debug ("%s: Originate external route(%pFX)", __func__,
                    &rt->prefix);
...
static void
ospf6_handle_aggregated_exnl_rt (struct ospf6 *ospf6,
                                 struct ospf6_external_aggr_rt *aggr,
                                 struct ospf6_route *rt)
{
  struct ospf6_lsa *lsa;
  ...
  if (prefix_same (&aggr->p, &rt->prefix))
    {
      if (IS_OSPF6_DEBUG_AGGR)
        zlog_debug (
            "%s: External Route prefix same as Aggregator(%pFX), so don't flush.",
            __func__, &rt->prefix);
  ...
  lsa = ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_AS_EXTERNAL), htonl (info->id),
                           ospf6->router_id, ospf6->lsdb);
  if (lsa)
    {
      ext_lsa =
          (struct ospf6_as_external_lsa *) ((char *) (lsa->header) +
                                            sizeof (struct ospf6_lsa_header));
...
ospf6_handle_external_aggr_add (struct ospf6 *ospf6)
{
  ...
  for (rt = ospf6_route_head (ospf6->external_table); rt;
       rt = ospf6_route_next (rt))
    {
      ...
      if (aggr)
        {
           ...
           * are handled here.
           */
          ospf6_handle_aggregated_exnl_rt (ospf6, aggr, rt);
          continue;
        }
       ...
       */
      ospf6_handle_exnl_rt_after_aggr_del (ospf6, rt);
    }
}

static void
ospf6_asbr_summary_process (struct event *thread)
{
  struct ospf6 *ospf6 = EVENT_ARG (thread);
  int operation = 0;
  ...
  if (event_is_scheduled (ospf6->t_external_aggr))
    {
      if (ospf6->aggr_action == OSPF6_ROUTE_AGGR_ADD)
        {
          ...
          if (IS_OSPF6_DEBUG_AGGR)
            zlog_debug (
                "%s: Not required to restart timer,set is already added.",
                __func__);
          return;
        }
      ...
      if (operation == OSPF6_ROUTE_AGGR_ADD)
        {
          if (IS_OSPF6_DEBUG_AGGR)
            zlog_debug ("%s, Restarting Aggregator delay timer.", __func__);
          EVENT_OFF (ospf6->t_external_aggr);
        }
    }

  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Start Aggregator delay timer %u(in seconds).", __func__,
...
int
ospf6_asbr_external_rt_advertise (struct ospf6 *ospf6, struct prefix *p)
{
...
int
ospf6_external_aggr_delay_timer_set (struct ospf6 *ospf6, uint16_t interval)
{
...
static unsigned int
ospf6_external_rt_hash_key (const void *data)
{
...
static bool
ospf6_external_rt_hash_cmp (const void *d1, const void *d2)
{
...
ospf6_external_aggr_new (struct prefix *p)
{
  ...
  aggr->match_extnl_hash =
      hash_create (ospf6_external_rt_hash_key, ospf6_external_rt_hash_cmp,
                   "Ospf6 external route hash");
  return aggr;
}

static void
ospf6_external_aggr_add (struct ospf6 *ospf6,
                         struct ospf6_external_aggr_rt *aggr)
{
  struct route_node *rn;

  if (IS_OSPF6_DEBUG_AGGR)
    zlog_debug ("%s: Adding Aggregate route to Aggr table (%pFX)", __func__,
                &aggr->p);
...
int
ospf6_asbr_external_rt_no_advertise (struct ospf6 *ospf6, struct prefix *p)
{
  struct ospf6_external_aggr_rt *aggr;
  route_tag_t tag = 0;
  ...
  if (aggr)
  ...
  else
    {
      ...
      ospf6_start_asbr_summary_delay_timer (ospf6, aggr, OSPF6_ROUTE_AGGR_ADD);
    }
...
int
ospf6_external_aggr_config_set (struct ospf6 *ospf6, struct prefix *p,
  ...
  if (aggregator)
    {
      if (CHECK_FLAG (aggregator->aggrflags,
                      OSPF6_EXTERNAL_AGGRT_NO_ADVERTISE))
        UNSET_FLAG (aggregator->aggrflags, OSPF6_EXTERNAL_AGGRT_NO_ADVERTISE);
      else if ((aggregator->tag == tag) && (aggregator->metric == metric) &&
...
int
ospf6_external_aggr_config_unset (struct ospf6 *ospf6, struct prefix *p)
{
  struct route_node *rn;
  struct ospf6_external_aggr_rt *aggr;
  ...
  ospf6_start_asbr_summary_delay_timer (ospf6, aggr, OSPF6_ROUTE_AGGR_DEL);

  return OSPF6_SUCCESS;
}

void
ospf6_handle_external_lsa_origination (struct ospf6 *ospf6,
                                       struct ospf6_route *rt,
                                       struct prefix *p)
{
  ...
  if (! is_default_prefix (p))
    {
      aggr = ospf6_external_aggr_match (ospf6, p);

      if (aggr)
        {

          if (IS_OSPF6_DEBUG_AGGR)
            zlog_debug ("%s: Send Aggregate LSA (%pFX)", __func__, &aggr->p);
...
void
ospf6_unset_all_aggr_flag (struct ospf6 *ospf6)
{
  struct route_node *rn = NULL;
  struct ospf6_external_aggr_rt *aggr;
