  ...
  if (old_state < OSPF6_NEIGHBOR_TWOWAY && state >= OSPF6_NEIGHBOR_TWOWAY)
    {
      ...
      if (memcmp (&on->linklocal_addr, &dst, sizeof (dst)))
        {
          bfd_sess_set_ipv6_addrs (on->bfd_session, &src, &on->linklocal_addr);
        }

      bfd_sess_install (on->bfd_session);
    }
  else if (old_state >= OSPF6_NEIGHBOR_TWOWAY && state < OSPF6_NEIGHBOR_TWOWAY)
 ...
 */
static void
ospf6_bfd_reg_dereg_all_nbr (struct ospf6_interface *oi, bool install)
{
...
static void
ospf6_bfd_callback (struct bfd_session_params *bsp,
                           ...
                           struct ospf6_neighbor *on)
{
  ...
  bfd_sess_set_timers (on->bfd_session, oi->bfd_config.detection_multiplier,
  ...
  if (argc > prof_idx)
    {
      ...
      oi->bfd_config.profile = XSTRDUP (MTYPE_TMP, argv[prof_idx]->arg);
    }
...
DEFUN (no_ipv6_ospf6_bfd_profile,
