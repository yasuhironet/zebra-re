  ...
  if (lsa->header->adv_router == INADDR_ANY)
    {
      if (IS_OSPF6_DEBUG_ORIGINATE_TYPE (lsa->header->type))
        zlog_debug ("Refusing to originate LSA (zero router ID): %s",
                    lsa->name);
...
void
ospf6_lsa_originate_process (struct ospf6_lsa *lsa, struct ospf6 *process)
{
...
void
ospf6_lsa_originate_area (struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
  lsa->lsdb = oa->lsdb;
...
void
ospf6_external_lsa_purge (struct ospf6 *ospf6, struct ospf6_lsa *lsa)
{
      ...
      if (lsa)
        {
          if (IS_OSPF6_DEBUG_NSSA)
            zlog_debug ("withdraw type 7 lsa, LS ID: %u", htonl (id));
 ...
 */
void
ospf6_lsa_purge_multi_ls_id (struct ospf6_area *oa, struct ospf6_lsa *lsa)
{
  int ls_id = 0;
  ...
  lsa_next = ospf6_lsdb_lookup (type, htonl (++ls_id), oa->ospf6->router_id,
                                oa->lsdb);
  while (lsa_next)
    {
...
ospf6_install_lsa (struct ospf6_lsa *lsa)
{
  ...
  if (old)
    {
      if (ntohs (lsa->header->type) == OSPF6_LSTYPE_TYPE_7)
        {
          if (IS_OSPF6_DEBUG_NSSA)
            zlog_debug ("%s : old LSA %s", __func__, lsa->name);
  ...
  if (IS_OSPF6_DEBUG_LSA_TYPE (lsa->header->type) ||
    ...
    zlog_debug ("%s Install LSA: %s age %d seqnum %x in LSDB.", __func__,
      ...
      if (old == NULL)
        {
          if (IS_OSPF6_DEBUG_LSA_TYPE (lsa->header->type) ||
              IS_OSPF6_DEBUG_EXAMIN_TYPE (lsa->header->type))
            zlog_debug ("%s: New router LSA %s", __func__, lsa->name);
...
void
ospf6_flood_interface (struct ospf6_neighbor *from, struct ospf6_lsa *lsa,
                       struct ospf6_interface *oi)
{
  ...
  for (ALL_LIST_ELEMENTS (oi->neighbor_list, node, nnode, on))
    {
      ...
      if ((oi->area->ospf6->inst_shutdown) ||
        ...
        {
          if (is_debug)
            zlog_debug ("%s: Send LSA %s (age %d) update now", __func__,
                        lsa->name, ntohs (lsa->header->age));
      ...
      else
        {
          ...
          if (is_debug)
            zlog_debug ("Add retrans-list of neighbor %s ", on->name);
          ...
          if (! old)
            {
              ...
              if (is_debug)
                zlog_debug ("Increment %s from retrans_list of %s", lsa->name,
                            on->name);
  ...
  if (retrans_added == 0)
    {
      if (is_debug)
        zlog_debug ("No retransmission scheduled, next interface %s",
                    oi->interface->name);
      return;
    }
...
void
ospf6_flood_area (struct ospf6_neighbor *from, struct ospf6_lsa *lsa,
                  struct ospf6_area *oa)
{
  struct listnode *node, *nnode;
  struct ospf6_interface *oi;
...
static void
ospf6_flood_process (struct ospf6_neighbor *from, struct ospf6_lsa *lsa,
      ...
      if (rem && ! ospf6_lsa_compare (rem, lsa))
        {
          if (IS_OSPF6_DEBUG_FLOODING ||
            ...
            zlog_debug ("Remove %s from retrans_list of %s", rem->name,
                        on->name);
...
static bool
ospf6_lsa_check_min_arrival (struct ospf6_lsa *lsa,
                             struct ospf6_neighbor *from)
{
                   ...
                   struct ospf6_lsa_header *lsa_header)
{
      ...
      if (IS_OSPF6_DEBUG_EXAMIN_TYPE (lsa_header->type))
        {
          zlog_debug (
              ...
              ospf6_lstype_name (lsa_header->type), &lsa_header->id,
              &lsa_header->adv_router, ntohl (lsa_header->seqnum));
        }
      return;
    }
  ...
  if (old == NULL || ismore_recent < 0)
    {
      ...
      self_originated =
          (new->header->adv_router == from->ospf6_if->area->ospf6->router_id);
          ...
          if (OSPF6_LSA_IS_MAXAGE (new))
            {
              ...
              if (IS_DEBUG_OSPF6_GR)
                zlog_debug ("%s, Received a maxage GraceLSA from router %pI4",
                            __func__, &new->header->adv_router);
              if (old)
                {
                  ospf6_process_maxage_grace_lsa (ospf6, new, from);
                }
              else
                {
                  if (IS_DEBUG_OSPF6_GR)
          ...
          else
            {
              ...
              if (IS_DEBUG_OSPF6_GR)
                zlog_debug ("%s, Received a GraceLSA from router %pI4",
                            __func__, &new->header->adv_router);
      ...
      if (self_originated)
        {
          if (from->ospf6_if->area->ospf6->gr_info.restart_in_progress)
            {
              if (IS_DEBUG_OSPF6_GR)
                zlog_debug (
          ...
          event_add_event (master, ospf6_lsa_refresh, new, 0, &new->refresh);
        }
  ...
  if (ospf6_lsdb_lookup (new->header->type, new->header->id,
                         new->header->adv_router, from->request_list))
    {
      ...
      zlog_warn ("Received is not newer, on the neighbor %s request-list",
                 from->name);
      zlog_warn ("BadLSReq, discard the received LSA lsa %s send badLSReq",
                 new->name);
  ...
  /* (8) previous database copy is more recent */
  {
    ...
    /* Otherwise, Send database copy of this LSA to this neighbor */
    {
      ...
      if (OSPF6_LSA_IS_MAXAGE (old) && ! OSPF6_LSA_IS_MAXAGE (new))
        {
      ...
      event_add_event (master, ospf6_lsupdate_send_neighbor, from, 0,
