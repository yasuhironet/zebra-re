...
struct ospf6 *
ospf6_get_by_lsdb (struct ospf6_lsa *lsa)
{
...
const char *
ospf6_lstype_name (uint16_t type)
{
  static char buf[8];
...
const char *
ospf6_lstype_short_name (uint16_t type)
{
  static char buf[8];
  const struct ospf6_lsa_handler *handler;
...
uint8_t
ospf6_lstype_debug (uint16_t type)
{
  const struct ospf6_lsa_handler *handler;
  handler = ospf6_get_lsa_handler (type);
...
int
metric_type (struct ospf6 *ospf6, int type, uint8_t instance)
{
...
int
metric_value (struct ospf6 *ospf6, int type, uint8_t instance)
{
  struct ospf6_redist *red;
...
int
ospf6_lsa_is_differ (struct ospf6_lsa *lsa1, struct ospf6_lsa *lsa2)
{
  int len;
...
int
ospf6_lsa_is_changed (struct ospf6_lsa *lsa1, struct ospf6_lsa *lsa2)
{
  int length;
...
void
ospf6_lsa_age_update_to_send (struct ospf6_lsa *lsa, uint32_t transdelay)
{
  unsigned short age;
                        ...
                        json_object *json_array, bool use_json)
{
  ...
  switch (type)
      ...
      if (use_json)
        {
          ...
          json_object_string_add (json_obj, "advRouter", adv_router);
          json_object_int_add (json_obj, "age", ospf6_lsa_age_current (lsa));
          json_object_int_add (json_obj, "seqNum",
        ...
        vty_out (vty, "%-4s %-15s%-15s%4hu %8lx %30s\n",
                 ospf6_lstype_short_name (lsa->header->type), id, adv_router,
                 ospf6_lsa_age_current (lsa),
                 (unsigned long) ntohl (lsa->header->seqnum),
                 handler->lh_get_prefix_str (lsa, buf, sizeof (buf), 0));
      break;
    ...
    case OSPF6_LSTYPE_INTRA_PREFIX:
      while (handler->lh_get_prefix_str (lsa, buf, sizeof (buf), cnt) != NULL)
        {
          if (use_json)
            {
              json_object_string_add (
              ...
              json_object_int_add (
                  json_obj, "seqNum",
                  (unsigned long) ntohl (lsa->header->seqnum));
              json_object_string_add (json_obj, "payload", buf);
              json_object_array_add (json_array, json_obj);
              json_obj = json_object_new_object ();
            }
          else
            vty_out (vty, "%-4s %-15s%-15s%4hu %8lx %30s\n",
                     ...
                     (unsigned long) ntohl (lsa->header->seqnum), buf);
          cnt++;
        }
      if (use_json)
  ...
  if (use_json)
    {
      ...
      json_object_string_add (json_obj, "advertisingRouter", adv_router);
      json_object_int_add (json_obj, "lsSequenceNumber",
                           (unsigned long) ntohl (lsa->header->seqnum));
      json_object_int_add (json_obj, "checksum",
                           ntohs (lsa->header->checksum));
      json_object_int_add (json_obj, "length", ntohs (lsa->header->length));
...
/* ospf6 lsa expiry */
void
ospf6_lsa_expire (struct event *thread)
{
  struct ospf6_lsa *lsa;
  struct ospf6 *ospf6;
...
void
ospf6_lsa_refresh (struct event *thread)
{
...
void
ospf6_flush_self_originated_lsas_now (struct ospf6 *ospf6)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, node, oa))
    {
      end = ospf6_lsdb_head (oa->lsdb_self, 0, 0, ospf6->router_id, &lsa);
      while (lsa)
        {
      ...
      for (ALL_LIST_ELEMENTS (oa->if_list, node, nnode, oi))
        {
          end = ospf6_lsdb_head (oi->lsdb_self, 0, 0, ospf6->router_id, &lsa);
          while (lsa)
            {
              /* RFC 2328 (14.1):  Set MAXAGE */
              lsa->header->age = htons (OSPF_LSA_MAXAGE);
...
void
ospf6_lsa_debug_set_all (bool val)
{
  unsigned int i;
  struct ospf6_lsa_handler *handler = NULL;
