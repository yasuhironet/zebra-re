...
int
ospf6_ls_origin_same (struct ospf6_path *o_path, struct ospf6_path *r_path)
{
...
bool
ospf6_check_and_set_router_abr (struct ospf6 *o)
{
...
static void
ospf6_abr_delete_route (struct ospf6_route *summary,
                                     ...
                                     struct ospf6_area *area)
{
  ...
  if (summary)
    {
      old = ospf6_lsdb_lookup (summary->path.origin.type,
                               summary->path.origin.id, area->ospf6->router_id,
                               area->lsdb);
  ...
  if (route->type == OSPF6_DEST_TYPE_NETWORK)
    {
      ...
      if (EXPORT_LIST (route_area) &&
          access_list_apply (EXPORT_LIST (route_area), &route->prefix) ==
              FILTER_DENY)
        {
          if (IS_OSPF6_DEBUG_ABR)
      ...
      if (PREFIX_LIST_OUT (route_area) &&
          prefix_list_apply (PREFIX_LIST_OUT (route_area), &route->prefix) !=
              PREFIX_PERMIT)
        {
          if (IS_OSPF6_DEBUG_ABR)
      ...
      if (PREFIX_LIST_IN (area) &&
        ...
        {
          if (IS_OSPF6_DEBUG_ABR)
            zlog_debug ("%s: prefix %pFX was denied by prefix-list in",
                        __func__, &route->prefix);
          filter = true;
        }
  ...
  if (route->type == OSPF6_DEST_TYPE_ROUTER)
  ...
  else
    {
      ...
      if (route->type == OSPF6_DEST_TYPE_NETWORK &&
          route->path.origin.type == htons (OSPF6_LSTYPE_INTER_PREFIX))
        {
      ...
      if (route->path.origin.type == htons (OSPF6_LSTYPE_INTRA_PREFIX))
        {
          if (! CHECK_FLAG (route->flag, OSPF6_ROUTE_BEST))
            {
              if (is_debug)
      ...
      if (is_debug)
        zlog_debug ("Originating summary in area %s for %pFX cost %u",
  ...
  if (summary == NULL)
    {
      ...
      if (route->type == OSPF6_DEST_TYPE_ROUTER)
      ...
      else
        {
          ...
          old =
              ospf6_find_inter_prefix_lsa (area->ospf6, area, &route->prefix);
          if (old)
  ...
  if (route->type == OSPF6_DEST_TYPE_ROUTER)
    {
      router_lsa =
          (struct ospf6_inter_router_lsa *) ((caddr_t) lsa_header +
    ...
    {
      prefix_lsa =
          (struct ospf6_inter_prefix_lsa *) ((caddr_t) lsa_header +
                                             sizeof (struct ospf6_lsa_header));
...
void
ospf6_abr_range_update (struct ospf6_route *range, struct ospf6 *ospf6)
{
  uint32_t cost = 0;
  struct listnode *node, *nnode;
  struct ospf6_area *oa;
      ...
      if (CHECK_FLAG (range->flag, OSPF6_ROUTE_ACTIVE_SUMMARY) &&
      ...
      else
        {
          ...
          lsa = ospf6_lsdb_lookup (range->path.origin.type,
                                   range->path.origin.id, ospf6->router_id,
                                   oa->lsdb);
          if (lsa)
...
void
ospf6_abr_originate_summary (struct ospf6_route *route, struct ospf6 *ospf6)
{
  struct listnode *node, *nnode;
  struct ospf6_area *oa;
...
void
ospf6_abr_old_path_update (struct ospf6_route *old_route,
  ...
  for (ALL_LIST_ELEMENTS (old_route->paths, anode, anext, o_path))
    {
      ...
      if (IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_EXAMIN (INTER_PREFIX))
        zlog_debug ("%s: paths %u nh %u", __func__,
                    ...
                    old_route->nh_list ? listcount (old_route->nh_list) : 0);

      if (table->hook_add)
        (*table->hook_add) (old_route);

      if (old_route->path.origin.id == route->path.origin.id &&
          old_route->path.origin.adv_router == route->path.origin.adv_router)
        {
          struct ospf6_path *h_path;

          h_path =
              (struct ospf6_path *) listgetdata (listhead (old_route->paths));
          old_route->path.origin.type = h_path->origin.type;
          old_route->path.origin.id = h_path->origin.id;
          old_route->path.origin.adv_router = h_path->origin.adv_router;
        }
    }
}

void
ospf6_abr_old_route_remove (struct ospf6_lsa *lsa, struct ospf6_route *old,
                            struct ospf6_route_table *table)
{
  if (IS_OSPF6_DEBUG_ABR)
      ...
      for (ALL_LIST_ELEMENTS (old->paths, anode, anext, o_path))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (o_path->nh_list, nnode, nh))
            {
              for (ALL_LIST_ELEMENTS (old->nh_list, rnode, rnext, rnh))
                {
                  if (! ospf6_nexthop_is_same (rnh, nh))
                    continue;
                  if (IS_OSPF6_DEBUG_ABR)
      ...
      if (nh_updated)
        {
          if (listcount (old->paths))
            {
              if (IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_EXAMIN (INTER_PREFIX))
                zlog_debug ("%s: old %pFX updated nh %u", __func__,
                            &old->prefix,
                            old->nh_list ? listcount (old->nh_list) : 0);

              if (table->hook_add)
                (*table->hook_add) (old);

              if ((old->path.origin.id == lsa->header->id) &&
                  (old->path.origin.adv_router == lsa->header->adv_router))
                {
                  struct ospf6_path *h_path;

                  h_path = (struct ospf6_path *) listgetdata (
                      listhead (old->paths));
                  old->path.origin.type = h_path->origin.type;
                  old->path.origin.id = h_path->origin.id;
                  old->path.origin.adv_router = h_path->origin.adv_router;
                }
            }
          else
...
ospf6_abr_examin_summary (struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
  ...
  if (lsa->header->type == htons (OSPF6_LSTYPE_INTER_PREFIX))
    {
      ...
      prefix_lsa =
          (struct ospf6_inter_prefix_lsa *) OSPF6_LSA_HEADER_END (lsa->header);
  ...
  else if (lsa->header->type == htons (OSPF6_LSTYPE_INTER_ROUTER))
    {
      ...
      router_lsa =
          (struct ospf6_inter_router_lsa *) OSPF6_LSA_HEADER_END (lsa->header);
      ...
      if (route->path.area_id == oa->area_id &&
          ...
          ! CHECK_FLAG (route->flag, OSPF6_ROUTE_WAS_REMOVED))
        {
          ...
          if (listcount (route->paths) > 1)
            {
              for (ALL_LIST_ELEMENTS_RO (route->paths, anode, o_path))
                {
                      ...
                      o_path->origin.adv_router == lsa->header->adv_router)
                    {
                      ...
                      if (is_debug)
                        zlog_debug (
                            ...
                            __func__, &o_path->origin.adv_router);
  ...
  if (abr_entry == NULL || abr_entry->path.area_id != oa->area_id ||
    ...
    {
      if (is_debug)
        zlog_debug ("%s: ABR router entry %pFX does not exist, ignore",
                    __func__, &abr_prefix);
      if (old)
        {
          if (old->type == OSPF6_DEST_TYPE_ROUTER && oa->intra_brouter_calc)
            {
              if (is_debug)
                zlog_debug (
  ...
  for (old_route = old; old_route; old_route = old_route->next)
    {
      ...
      if ((ospf6_route_cmp (route, old_route) != 0))
        {
          if (is_debug)
            zlog_debug (
                ...
                __func__, (void *) old_route, &prefix, old_route->path.cost,
                route->path.cost);
          ...
          if (listcount (old_route->paths) > 1)
            ospf6_abr_old_path_update (old_route, route, table);
          continue;
        }
      ...
      if (o_path == NULL)
      ...
      else
        {
          ...
          ospf6_copy_nexthops (tmp_route->nh_list, o_path->nh_list);
...
void
ospf6_abr_examin_brouter (uint32_t router_id, struct ospf6_route *route,
                          struct ospf6 *ospf6)
{
  struct ospf6_lsa *lsa;
  struct ospf6_area *oa;
