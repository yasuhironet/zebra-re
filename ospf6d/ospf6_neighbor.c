...
ospf6_neighbor_lookup (uint32_t router_id, struct ospf6_interface *oi)
{
  struct listnode *n;
...
struct ospf6_neighbor *
ospf6_area_neighbor_lookup (struct ospf6_area *area, uint32_t router_id)
{
...
static void
ospf6_neighbor_clear_ls_lists (struct ospf6_neighbor *on)
{
...
struct ospf6_neighbor *
ospf6_neighbor_create (uint32_t router_id, struct ospf6_interface *oi)
{
  struct ospf6_neighbor *on;
  char buf[16];
  int type;
...
static void
ospf6_neighbor_state_change (uint8_t next_state, struct ospf6_neighbor *on,
                             int event)
{
  uint8_t prev_state;
  ...
  if (IS_OSPF6_DEBUG_NEIGHBOR (STATE))
    {
      zlog_debug (
          ...
          on->name, &on->router_id, ospf6_neighbor_state_str[prev_state],
  ...
  if (CHECK_FLAG (on->ospf6_if->area->ospf6->config_flags,
    ...
    zlog_notice ("AdjChg: Nbr %pI4(%s) on %s: %s -> %s (%s)", &on->router_id,
...
void
hello_received (struct event *thread)
{
  struct ospf6_neighbor *on;
  ...
  EVENT_OFF (on->inactivity_timer);
  event_add_timer (master, inactivity_timer, on, on->ospf6_if->dead_interval,
                   &on->inactivity_timer);
...
void
twoway_received (struct event *thread)
{
  struct ospf6_neighbor *on;
...
void
negotiation_done (struct event *thread)
{
  struct ospf6_neighbor *on;
...
static void
ospf6_neighbor_last_dbdesc_release (struct event *thread)
{
...
void
exchange_done (struct event *thread)
{
  struct ospf6_neighbor *on;
...
void
loading_done (struct event *thread)
{
  struct ospf6_neighbor *on;
...
void
seqnumber_mismatch (struct event *thread)
{
  struct ospf6_neighbor *on;
...
void
oneway_received (struct event *thread)
{
  struct ospf6_neighbor *on;
...
void
inactivity_timer (struct event *thread)
{
  struct ospf6_neighbor *on;
  ...
  */
  ...
  if (use_json)
    ...
    vty_out (vty, "%-15s %8s/%-11s %-15s %-15s %s[%s]\n", router_id,
             ospf6_neighbor_state_str[on->state], duration, drouter, bdrouter,
             on->ospf6_if->interface->name,
             ospf6_interface_state_str[on->ospf6_if->state]);
}
  ...
  if (use_json)
    {
      ...
      json_object_string_add (json_neighbor, "area", on->ospf6_if->area->name);
      ...
      json_object_string_add (json_neighbor, "neighborStateDuration",
                              duration);
      json_object_string_add (json_neighbor, "neighborDRouter", drouter);
      json_object_string_add (json_neighbor, "neighborBdRouter", bdrouter);
      snprintf (
          db_desc_str, sizeof (db_desc_str), "%s%s%s",
          ...
          (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MSBIT) ? "Master"
                                                            : "Slave"));
      json_object_string_add (json_neighbor, "dbDescStatus", db_desc_str);
      ...
      json_object_object_add (json_neighbor, "summaryListLsa", json_array);
      ...
      for (ALL_LSDB (on->request_list, lsa, lsanext))
        json_object_array_add (json_array, json_object_new_string (lsa->name));
      json_object_object_add (json_neighbor, "requestListLsa", json_array);
      ...
      for (ALL_LSDB (on->retrans_list, lsa, lsanext))
        json_object_array_add (json_array, json_object_new_string (lsa->name));
      json_object_object_add (json_neighbor, "reTransListLsa", json_array);
      ...
      json_object_string_add (json_neighbor, "pendingLsaDbDescTime", duration);
      json_object_string_add (
          json_neighbor, "dbDescSendThread",
          (event_is_scheduled (on->thread_send_dbdesc) ? "on" : "off"));
      json_array = json_object_new_array ();
      for (ALL_LSDB (on->dbdesc_list, lsa, lsanext))
        json_object_array_add (json_array, json_object_new_string (lsa->name));
      json_object_object_add (json_neighbor, "pendingLsaDbDesc", json_array);
      ...
      json_object_int_add (json_neighbor, "pendingLsaLsReqCount",
                           on->request_list->count);
      json_object_string_add (json_neighbor, "pendingLsaLsReqTime", duration);
      json_object_string_add (
          json_neighbor, "lsReqSendThread",
          (event_is_scheduled (on->thread_send_lsreq) ? "on" : "off"));
      json_array = json_object_new_array ();
      for (ALL_LSDB (on->request_list, lsa, lsanext))
        json_object_array_add (json_array, json_object_new_string (lsa->name));
      json_object_object_add (json_neighbor, "pendingLsaLsReq", json_array);
      ...
      json_object_string_add (
          ...
          (event_is_scheduled (on->thread_send_lsupdate) ? "on" : "off"));
      json_array = json_object_new_array ();
      for (ALL_LSDB (on->lsupdate_list, lsa, lsanext))
        json_object_array_add (json_array, json_object_new_string (lsa->name));
      json_object_object_add (json_neighbor, "pendingLsaLsUpdate", json_array);
      ...
      json_object_int_add (json_neighbor, "pendingLsaLsAckCount",
                           on->lsack_list->count);
      json_object_string_add (json_neighbor, "pendingLsaLsAckTime", duration);
      json_object_string_add (
          json_neighbor, "lsAckSendThread",
          (event_is_scheduled (on->thread_send_lsack) ? "on" : "off"));
      json_array = json_object_new_array ();
      for (ALL_LSDB (on->lsack_list, lsa, lsanext))
        json_object_array_add (json_array, json_object_new_string (lsa->name));
      json_object_object_add (json_neighbor, "pendingLsaLsAck", json_array);

      bfd_sess_show (vty, json_neighbor, on->bfd_session);

      if (on->auth_present == true)
        {
          json_object_string_add (json_neighbor, "authStatus", "enabled");
          json_object_int_add (json_neighbor, "recvdHelloHigherSeqNo",
                               on->seqnum_h[OSPF6_MESSAGE_TYPE_HELLO]);
          json_object_int_add (json_neighbor, "recvdHelloLowerSeqNo",
                               on->seqnum_l[OSPF6_MESSAGE_TYPE_HELLO]);
          json_object_int_add (json_neighbor, "recvdDBDescHigherSeqNo",
                               on->seqnum_h[OSPF6_MESSAGE_TYPE_DBDESC]);
          json_object_int_add (json_neighbor, "recvdDBDescLowerSeqNo",
                               on->seqnum_l[OSPF6_MESSAGE_TYPE_DBDESC]);
          json_object_int_add (json_neighbor, "recvdLSReqHigherSeqNo",
                               on->seqnum_h[OSPF6_MESSAGE_TYPE_LSREQ]);
          json_object_int_add (json_neighbor, "recvdLSReqLowerSeqNo",
                               on->seqnum_l[OSPF6_MESSAGE_TYPE_LSREQ]);
          json_object_int_add (json_neighbor, "recvdLSUpdHigherSeqNo",
                               on->seqnum_h[OSPF6_MESSAGE_TYPE_LSUPDATE]);
          json_object_int_add (json_neighbor, "recvdLSUpdLowerSeqNo",
                               on->seqnum_l[OSPF6_MESSAGE_TYPE_LSUPDATE]);
          json_object_int_add (json_neighbor, "recvdLSAckHigherSeqNo",
                               on->seqnum_h[OSPF6_MESSAGE_TYPE_LSACK]);
          json_object_int_add (json_neighbor, "recvdLSAckLowerSeqNo",
                               on->seqnum_l[OSPF6_MESSAGE_TYPE_LSACK]);
        }
      else
        json_object_string_add (json_neighbor, "authStatus", "disabled");
  ...
  else
    {
      ...
      vty_out (
          vty, "    DbDesc status: %s%s%s SeqNum: %#lx\n",
          (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_IBIT) ? "Initial " : ""),
          (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MBIT) ? "More " : ""),
          (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MSBIT) ? "Master"
                                                            : "Slave"),
...
static void
ospf6_neighbor_show_detail_common (struct vty *vty, struct ospf6 *ospf6,
                                   bool uj, bool detail, bool drchoice)
{
  struct ospf6_neighbor *on;
  struct ospf6_interface *oi;
  ...
  void (*showfunc) (struct vty *, struct ospf6_neighbor *, json_object *json,
                    bool use_json);
  ...
  else
    {
      if (showfunc == ospf6_neighbor_show)
        vty_out (vty, "%-15s %3s %11s %8s/%-12s %11s %s[%s]\n", "Neighbor ID",
                 "Pri", "DeadTime", "State", "IfState", "Duration", "I/F",
                 "State");
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_neighbor_show_detail_common (vty, ospf6, uj, detail, drchoice);
          if (! all_vrf)
            break;
        }
    }
...
static int
ospf6_neighbor_show_common (struct vty *vty, int argc, struct cmd_token **argv,
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_neighbor_show_common (vty, argc, argv, ospf6, idx_ipv4, uj);
