...
#define ROUTER_LSDESC_IS_TYPE(t, x)                                           \
  ((((struct ospf6_router_lsdesc *) (x))->type == OSPF6_ROUTER_LSDESC_##t)    \
       ? 1                                                                    \
       : 0)
  ...
  do                                                                          \
    {                                                                         \
      if (CHECK_FLAG ((oa)->flag, OSPF6_AREA_ENABLE))                         \
        event_add_event (master, ospf6_router_lsa_originate, oa, 0,           \
  ...
  do                                                                          \
    {                                                                         \
      if (! CHECK_FLAG ((oi)->flag, OSPF6_INTERFACE_DISABLE))                 \
        event_add_event (master, ospf6_network_lsa_originate, oi, 0,          \
...
#define OSPF6_LINK_LSA_SCHEDULE(oi)                                           \
  do                                                                          \
    {                                                                         \
      if (! CHECK_FLAG ((oi)->flag, OSPF6_INTERFACE_DISABLE))                 \
        event_add_event (master, ospf6_link_lsa_originate, oi, 0,             \
                         &(oi)->thread_link_lsa);                             \
    }                                                                         \
  while (0)
#define OSPF6_INTRA_PREFIX_LSA_SCHEDULE_STUB(oa)                              \
  do                                                                          \
    {                                                                         \
      if (CHECK_FLAG ((oa)->flag, OSPF6_AREA_ENABLE))                         \
        event_add_event (master, ospf6_intra_prefix_lsa_originate_stub, oa,   \
                         0, &(oa)->thread_intra_prefix_lsa);                  \
    }                                                                         \
  while (0)
#define OSPF6_INTRA_PREFIX_LSA_SCHEDULE_TRANSIT(oi)                           \
  do                                                                          \
    {                                                                         \
      if (! CHECK_FLAG ((oi)->flag, OSPF6_INTERFACE_DISABLE))                 \
        event_add_event (master, ospf6_intra_prefix_lsa_originate_transit,    \
...
#define OSPF6_AS_EXTERN_LSA_SCHEDULE(oi)                                      \
  do                                                                          \
    {                                                                         \
      if (! CHECK_FLAG ((oi)->flag, OSPF6_INTERFACE_DISABLE))                 \
        event_add_event (master, ospf6_orig_as_external_lsa, oi, 0,           \
                         &(oi)->thread_as_extern_lsa);                        \
    }                                                                         \
  while (0)
...
#define OSPF6_ROUTER_LSA_EXECUTE(oa)                                          \
  do                                                                          \
    {                                                                         \
      if (CHECK_FLAG ((oa)->flag, OSPF6_AREA_ENABLE))                         \
        event_execute (master, ospf6_router_lsa_originate, oa, 0);            \
    }                                                                         \
  while (0)
...
#define OSPF6_LINK_LSA_EXECUTE(oi)                                            \
  do                                                                          \
    {                                                                         \
      if (! CHECK_FLAG ((oi)->flag, OSPF6_INTERFACE_DISABLE))                 \
        event_execute (master, ospf6_link_lsa_originate, oi, 0);              \
    }                                                                         \
  while (0)
...
#define OSPF6_INTRA_PREFIX_LSA_EXECUTE_TRANSIT(oi)                            \
  do                                                                          \
    {                                                                         \
      ...
      event_execute (master, ospf6_intra_prefix_lsa_originate_transit, oi,    \
                     0);                                                      \
    }                                                                         \
  while (0)
