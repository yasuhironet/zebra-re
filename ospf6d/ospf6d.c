...
ospf6_lsdb_show (struct vty *vty, enum ospf_lsdb_show_level level,
                 ...
                 struct ospf6_lsdb *lsdb, json_object *json_obj, bool use_json)
{
  struct ospf6_lsa *lsa;
  const struct route_node *end = NULL;
  void (*showfunc) (struct vty *, struct ospf6_lsa *, json_object *, bool) =
      NULL;
  ...
  if ((level == OSPF6_LSDB_SHOW_LEVEL_NORMAL) && ! use_json)
    ospf6_lsa_show_summary_header (vty);
                         ...
                         type ? *type : 0, adv_router ? *adv_router : 0, &lsa);
  while (lsa)
    {
...
static void
ospf6_lsdb_show_wrapper (struct vty *vty, enum ospf_lsdb_show_level level,
                         uint16_t *type, uint32_t *id, uint32_t *adv_router,
                         bool uj, struct ospf6 *ospf6)
{
  struct listnode *i, *j;
  ...
  for (ALL_LIST_ELEMENTS_RO (o->area_list, i, oa))
    {
      ...
      ospf6_lsdb_show (vty, level, type, id, adv_router, oa->lsdb, json_obj,
                       uj);
      if (uj)
        json_object_array_add (json_array, json_obj);
    }
  if (uj)
    json_object_object_add (json, "areaScopedLinkStateDb", json_array);
  ...
  if (uj)
    json_object_object_add (json, "interfaceScopedLinkStateDb", json_array);
  if (uj)
    {
      json_array = json_object_new_array ();
      json_obj = json_object_new_object ();
  ...
  ospf6_lsdb_show (vty, level, type, id, adv_router, o->lsdb, json_obj, uj);
...
static void
ospf6_lsdb_type_show_wrapper (struct vty *vty, enum ospf_lsdb_show_level level,
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_lsdb_type_show_wrapper (vty, level, &type, NULL, NULL, uj,
                                        ospf6);
          if (! all_vrf)
            break;
        }
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_lsdb_show_wrapper (vty, level, NULL, NULL, &adv_router, uj,
                                   ospf6);
          if (! all_vrf)
            break;
        }
...
static int
ipv6_ospf6_database_aggr_router_common (struct vty *vty, uint32_t adv_router,
                                        struct ospf6 *ospf6)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf6->area_list, i, oa))
    {
      if (adv_router == ospf6->router_id)
        ...
        lsdb = oa->lsdb;
      if (ospf6_create_single_router_lsa (oa, lsdb, adv_router) == NULL)
        {
          vty_out (vty, "Adv router is not found in LSDB.");
          return CMD_SUCCESS;
        }
      ospf6_lsdb_show (vty, level, &type, NULL, NULL, oa->temp_router_lsa_lsdb,
                       NULL, false);
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_lsdb_type_show_wrapper (vty, level, &type, &id, NULL, uj,
                                        ospf6);
          if (! all_vrf)
            break;
        }
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_lsdb_type_show_wrapper (vty, level, &type, NULL, &adv_router,
                                        uj, ospf6);
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_lsdb_show_wrapper (vty, level, NULL, &id, &adv_router, uj,
                                   ospf6);
          if (! all_vrf)
            break;
        }
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          ospf6_lsdb_type_show_wrapper (vty, level, &type, &id, &adv_router,
                                        uj, ospf6);
...
static int
show_ospf6_border_routers_common (struct vty *vty, int argc,
                                  struct cmd_token **argv, struct ospf6 *ospf6,
                                  int idx_ipv4, int idx_argc)
{
  uint32_t adv_router;
...
void
ospf6_init (struct event_loop *master)
{
  ospf6_top_init ();
  ospf6_area_init ();
