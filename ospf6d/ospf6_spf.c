...
static int
ospf6_vertex_cmp (const struct ospf6_vertex *va, const struct ospf6_vertex *vb)
{
  /* ascending order */
  if (va->cost != vb->cost)
...
ospf6_vertex_create (struct ospf6_lsa *lsa)
{
  ...
  if (IS_OSPF6_DEBUG_SPF (PROCESS))
    zlog_debug ("%s: Creating vertex %s of type %s (0x%04hx) lsa %s", __func__,
                v->name,
...
ospf6_lsdesc_lsa (caddr_t lsdesc, struct ospf6_vertex *v)
{
  ...
  if (type == htons (OSPF6_LSTYPE_NETWORK))
    ...
    lsa = ospf6_create_single_router_lsa (v->area, v->area->lsdb, adv_router);
  if (IS_OSPF6_DEBUG_SPF (PROCESS))
    {
...
static char *
ospf6_lsdesc_backlink (struct ospf6_lsa *lsa, caddr_t lsdesc,
                       struct ospf6_vertex *v)
{
  ...
  size =
      (OSPF6_LSA_IS_TYPE (ROUTER, lsa) ? sizeof (struct ospf6_router_lsdesc)
                                       : sizeof (struct ospf6_network_lsdesc));
  for (backlink = OSPF6_LSA_HEADER_END (lsa->header) + 4;
       backlink + size <= OSPF6_LSA_END (lsa->header); backlink += size)
    {
      assert (
          ! (OSPF6_LSA_IS_TYPE (NETWORK, lsa) && VERTEX_IS_TYPE (NETWORK, v)));
                    ...
                    caddr_t lsdesc, struct ospf6 *ospf6)
{
  ...
  adv_router =
      (VERTEX_IS_TYPE (NETWORK, v) ? NETWORK_LSDESC_GET_NBR_ROUTERID (lsdesc)
                                   : ROUTER_LSDESC_GET_NBR_ROUTERID (lsdesc));
          ...
          if (IS_OSPF6_DEBUG_SPF (PROCESS))
            {
              zlog_debug ("%s: V lsa %s id %u, route id %u are different",
                          __func__, v->lsa->name, ntohl (v->lsa->header->id),
...
void
ospf6_spf_reason_string (uint32_t reason, char *buf, int size)
{
  uint32_t bit;
  int len = 0;
...
void
ospf6_spf_calculation (uint32_t router_id,
                       struct ospf6_route_table *result_table,
  ...
  while ((v = vertex_pqueue_pop (&candidate_list)))
    {
      ...
      size =
          (VERTEX_IS_TYPE (ROUTER, v) ? sizeof (struct ospf6_router_lsdesc)
                                      : sizeof (struct ospf6_network_lsdesc));
...
static void
ospf6_spf_calculation_thread (struct event *t)
{
  struct ospf6_area *oa;
  struct ospf6 *ospf6;
  ...
  for (ALL_LIST_ELEMENTS (v->child_list, node, nnode, c))
    {
      ...
      ospf6_spf_display_subtree (vty, next_prefix, restnum, c, json_child,
                                 use_json);
  ...
  if (use_json)
    {
      ...
      if (listcount (v->child_list))
        json_object_object_add (json_obj, "children", json_childs);
      else
...
static int
ospf6_timers_spf_set (struct vty *vty, unsigned int delay, unsigned int hold,
                      unsigned int max)
{
...
void
ospf6_spf_config_write (struct vty *vty, struct ospf6 *ospf6)
{
  ...
  if (ospf6->spf_delay != OSPF_SPF_DELAY_DEFAULT ||
    ...
    vty_out (vty, " timers throttle spf %d %d %d\n", ospf6->spf_delay,
             ospf6->spf_holdtime, ospf6->spf_max_holdtime);
}
...
struct ospf6_lsa *
ospf6_create_single_router_lsa (struct ospf6_area *area,
                                struct ospf6_lsdb *lsdb, uint32_t adv_router)
{
  struct ospf6_lsa *lsa = NULL;
  ...
  if (IS_OSPF6_DEBUG_SPF (PROCESS))
    zlog_debug ("%s: adv_router %s num_lsa %u to convert.", __func__, ifbuf,
                num_lsa);
  if (num_lsa == 1)
    return lsa;
  ...
  if (num_lsa == 0)
    {
      if (IS_OSPF6_DEBUG_SPF (PROCESS))
        zlog_debug ("%s: adv_router %s not found in LSDB.", __func__, ifbuf);
      return NULL;
    }
      ...
      if (IS_OSPF6_DEBUG_SPF (PROCESS))
        {
          ...
          zlog_debug (
              ...
              __func__, rtr_lsa->name, ntohs (lsa_header->length), ifbuf);
        }
  ...
  if (IS_OSPF6_DEBUG_SPF (PROCESS))
    zlog_debug ("%s: LSA %s id %u type 0%x len %u num_lsa %u", __func__,
                lsa->name, ntohl (lsa->header->id), ntohs (lsa->header->type),
...
void
ospf6_remove_temp_router_lsa (struct ospf6_area *area)
{
  ...
  for (ALL_LSDB (area->temp_router_lsa_lsdb, lsa, lsanext))
    {
      if (IS_OSPF6_DEBUG_SPF (PROCESS))
        zlog_debug ("%s Remove LSA %s lsa->lock %u lsdb count %u", __func__,
                    lsa->name, lsa->lock, area->temp_router_lsa_lsdb->count);
      ospf6_lsdb_remove (lsa, area->temp_router_lsa_lsdb);
    }
}

int
ospf6_ase_calculate_route (struct ospf6 *ospf6, struct ospf6_lsa *lsa,
                           struct ospf6_area *area)
{
  ...
  if (CHECK_FLAG (lsa->flag, OSPF6_LSA_LOCAL_XLT))
    {
      if (IS_OSPF6_DEBUG_SPF (PROCESS))
        zlog_debug ("%s: Rejecting Local translated LSA", __func__);
      return 0;
    }
  ...
  if (ntohs (lsa->header->type) == OSPF6_LSTYPE_AS_EXTERNAL)
    {
      ...
      if (route == NULL)
        {
          if (IS_OSPF6_DEBUG_SPF (PROCESS))
            zlog_debug ("%s: no external route %pFX", __func__, &prefix);
          return 0;
        }
      ...
      if (CHECK_FLAG (route->flag, OSPF6_ROUTE_REMOVE))
               ...
               CHECK_FLAG (route->flag, OSPF6_ROUTE_CHANGE))
        {
          if (hook_add)
            {
              if (IS_OSPF6_DEBUG_SPF (PROCESS))
                zlog_debug ("%s: add external route %pFX", __func__, &prefix);
              (*hook_add) (route);
            }
        }
    }
  else if (ntohs (lsa->header->type) == OSPF6_LSTYPE_TYPE_7)
    {
      ...
      if (route == NULL)
        {
          if (IS_OSPF6_DEBUG_SPF (PROCESS))
            zlog_debug ("%s: no route %pFX, area %s", __func__, &prefix,
                        area->name);
          return 0;
        }
      ...
      if (CHECK_FLAG (route->flag, OSPF6_ROUTE_REMOVE))
        {
          if (IS_OSPF6_DEBUG_SPF (PROCESS))
            zlog_debug ("%s : remove route %pFX, area %s", __func__, &prefix,
                        area->name);
               ...
               CHECK_FLAG (route->flag, OSPF6_ROUTE_CHANGE))
        {
          if (hook_add)
            {
              if (IS_OSPF6_DEBUG_SPF (PROCESS))
                zlog_debug ("%s: add nssa route %pFX, area %s", __func__,
                            &prefix, area->name);
              (*hook_add) (route);
            }
...
static void
ospf6_ase_calculate_timer (struct event *t)
{
  struct ospf6 *ospf6;
  struct ospf6_lsa *lsa;
  struct listnode *node, *nnode;
...
void
ospf6_ase_calculate_timer_add (struct ospf6 *ospf6)
{
  if (ospf6 == NULL)
    return;
