...
int
ospf6_interface_neighbor_count (struct ospf6_interface *oi)
{
  int count = 0;
...
ospf6_interface_lookup_by_ifindex (ifindex_t ifindex, vrf_id_t vrf_id)
{
...
static void
ospf6_interface_force_recalculate_cost (struct ospf6_interface *oi)
{
          ...
          if (ret == PREFIX_DENY)
            {
              if (IS_OSPF6_DEBUG_INTERFACE)
                zlog_debug ("%pFX on %s filtered by prefix-list %s ",
                            c->address, oi->interface->name, oi->plist_name);
              continue;
            }
        }
...
static int
ospf6_interface_state_change (uint8_t next_state, struct ospf6_interface *oi)
{
  uint8_t prev_state;
  struct ospf6 *ospf6;
  ...
  if ((prev_state == OSPF6_INTERFACE_DR ||
      ...
      (next_state != OSPF6_INTERFACE_DR && next_state != OSPF6_INTERFACE_BDR))
      ...
      for (ALL_LIST_ELEMENTS_RO (oi->neighbor_list, node, on))
        {
          if (on->state < OSPF6_NEIGHBOR_TWOWAY)
            continue;
          /* Schedule AdjOK. */
          event_add_event (master, adj_ok, on, 0, &on->thread_adj_ok);
        }
    }
...
static bool
ifmaddr_check (ifindex_t ifindex, struct in6_addr *addr)
{
...
void
interface_up (struct event *thread)
{
  struct ospf6_interface *oi;
  struct ospf6 *ospf6;
  ...
  if (ifmaddr_check (oi->interface->ifindex, &allspfrouters6))
    {
      zlog_info (
          ...
          oi->interface->name);
      event_add_timer (master, interface_up, oi, OSPF6_INTERFACE_SSO_RETRY_INT,
                       &oi->thread_sso);
      return;
    }
#endif /* __FreeBSD__ */
...
void
wait_timer (struct event *thread)
{
  struct ospf6_interface *oi;
...
void
backup_seen (struct event *thread)
{
  struct ospf6_interface *oi;
...
void
neighbor_change (struct event *thread)
{
  struct ospf6_interface *oi;
...
void
interface_down (struct event *thread)
{
  struct ospf6_interface *oi;
  struct listnode *node, *nnode;
  struct ospf6_neighbor *on;
...
static const char *
ospf6_iftype_str (uint8_t iftype)
{
  switch (iftype)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (ifp->connected, i, c))
        {
          ...
          switch (p->family)
            {
            case AF_INET:
              json_object_string_add (json_addr, "type", "inet");
              json_object_string_add (json_addr, "address", strbuf);
              json_object_array_add (json_arr, json_addr);
              break;
            case AF_INET6:
              json_object_string_add (json_addr, "type", "inet6");
              json_object_string_add (json_addr, "address", strbuf);
              json_object_array_add (json_arr, json_addr);
              break;
            default:
              json_object_string_add (json_addr, "type", "unknown");
              json_object_string_add (json_addr, "address", strbuf);
              json_object_array_add (json_arr, json_addr);
              break;
            }
      ...
      if (oi->area)
        {
          ...
          json_object_int_add (json_obj, "instanceId", oi->instance_id);
        ...
        json_object_boolean_false_add (json_obj, "attachedToArea");
    }
  else
    {
      if (oi->area)
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json_obj, "transmitDelaySec", oi->transdelay);
  ...
  else
    {
      ...
      if (! CHECK_FLAG (oi->flag, OSPF6_INTERFACE_PASSIVE))
        vty_out (vty, "   Hello %d(%pTHd), Dead %d, Retransmit %d\n",
                 oi->hello_interval, oi->thread_send_hello, oi->dead_interval,
                 oi->rxmt_interval);
      else
  ...
  if (use_json)
    {
      ...
      json_object_string_add (json_obj, "pendingLsaLsUpdateTime", duration);
      json_object_string_add (
          json_obj, "lsUpdateSendThread",
          (event_is_scheduled (oi->thread_send_lsupdate) ? "on" : "off"));

      json_arr = json_object_new_array ();
      for (ALL_LSDB (oi->lsupdate_list, lsa, lsanext))
        json_object_array_add (json_arr, json_object_new_string (lsa->name));
      ...
      json_object_int_add (json_obj, "pendingLsaLsAckCount",
                           oi->lsack_list->count);
      json_object_string_add (json_obj, "pendingLsaLsAckTime", duration);
      json_object_string_add (
          json_obj, "lsAckSendThread",
          (event_is_scheduled (oi->thread_send_lsack) ? "on" : "off"));
      ...
      if (use_json)
      ...
      else
        {
          vty_out (
              ...
              oi->bfd_config.detection_multiplier, oi->bfd_config.min_rx,
              oi->bfd_config.min_tx);
        }
    }
      ...
      if (use_json)
        {
          if (CHECK_FLAG (oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN))
            {
              json_object_string_add (json_auth, "authType", "keychain");
              json_object_string_add (json_auth, "keychainName",
                                      oi->at_data.keychain);
            }
          else if (CHECK_FLAG (oi->at_data.flags,
                               OSPF6_AUTH_TRAILER_MANUAL_KEY))
            json_object_string_add (json_auth, "authType", "manualkey");
          json_object_int_add (json_auth, "txPktDrop", oi->at_data.tx_drop);
          json_object_int_add (json_auth, "rxPktDrop", oi->at_data.rx_drop);
        }
      else
        {
          if (CHECK_FLAG (oi->at_data.flags, OSPF6_AUTH_TRAILER_KEYCHAIN))
...
static int
show_ospf6_interface_common (struct vty *vty, vrf_id_t vrf_id, int argc,
                             struct cmd_token **argv, int idx_ifname,
                             int intf_idx, int json_idx, bool uj)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (om6->ospf6, node, ospf6))
    {
      if (all_vrf || strcmp (ospf6->name, vrf_name) == 0)
        {
          show_ospf6_interface_common (vty, ospf6->vrf_id, argc, argv,
                                       idx_ifname, intf_idx, json_idx, uj);
...
static int
ospf6_interface_show_traffic (struct vty *vty, struct interface *intf_ifp,
          ...
          if (use_json)
            {
              ...
              json_object_int_add (json_interface, "helloRx", oi->hello_in);
              json_object_int_add (json_interface, "helloTx", oi->hello_out);
              json_object_int_add (json_interface, "dbDescRx", oi->db_desc_in);
              json_object_int_add (json_interface, "dbDescTx",
                                   oi->db_desc_out);
              json_object_int_add (json_interface, "lsReqRx", oi->ls_req_in);
              json_object_int_add (json_interface, "lsReqTx", oi->ls_req_out);
              json_object_int_add (json_interface, "lsUpdateRx",
                                   oi->ls_upd_in);
              json_object_int_add (json_interface, "lsUpdateTx",
                                   oi->ls_upd_out);
              json_object_int_add (json_interface, "lsAckRx", oi->ls_ack_in);
              json_object_int_add (json_interface, "lsAckTx", oi->ls_ack_out);
            ...
            vty_out (
                vty, "%-10s %8u/%-8u %7u/%-7u %7u/%-7u %7u/%-7u %7u/%-7u\n",
                oi->interface->name, oi->hello_in, oi->hello_out,
                oi->db_desc_in, oi->db_desc_out, oi->ls_req_in, oi->ls_req_out,
                oi->ls_upd_in, oi->ls_upd_out, oi->ls_ack_in, oi->ls_ack_out);
        }
    }
  else
    {
      ...
      if (use_json)
        {
          ...
          json_object_int_add (json_interface, "helloTx", oi->hello_out);
          json_object_int_add (json_interface, "dbDescRx", oi->db_desc_in);
          json_object_int_add (json_interface, "dbDescTx", oi->db_desc_out);
          json_object_int_add (json_interface, "lsReqRx", oi->ls_req_in);
          json_object_int_add (json_interface, "lsReqTx", oi->ls_req_out);
          json_object_int_add (json_interface, "lsUpdateRx", oi->ls_upd_in);
          json_object_int_add (json_interface, "lsUpdateTx", oi->ls_upd_out);
          json_object_int_add (json_interface, "lsAckRx", oi->ls_ack_in);
          json_object_int_add (json_interface, "lsAckTx", oi->ls_ack_out);

          json_object_object_add (json, oi->interface->name, json_interface);
        }
      else
        vty_out (vty, "%-10s %8u/%-8u %7u/%-7u %7u/%-7u %7u/%-7u %7u/%-7u\n",
                 oi->interface->name, oi->hello_in, oi->hello_out,
                 oi->db_desc_in, oi->db_desc_out, oi->ls_req_in,
                 oi->ls_req_out, oi->ls_upd_in, oi->ls_upd_out, oi->ls_ack_in,
                 oi->ls_ack_out);
    }
...
static int
ospf6_interface_show_traffic_common (struct vty *vty, int argc,
                                     struct cmd_token **argv, vrf_id_t vrf_id,
                                     bool uj)
{
  int idx_ifname = 0;
  int display_once = 0;
      ...
      if (uj)
        {
          if (ifp == NULL)
            {
              json_object_string_add (json, "status", "No Such Interface");
              json_object_string_add (json, "interface", intf_name);
              vty_json (vty, json);
              return CMD_WARNING;
            }
          if (ifp->info == NULL)
...
void
ospf6_interface_start (struct ospf6_interface *oi)
{
  struct ospf6 *ospf6;
...
void
ospf6_interface_stop (struct ospf6_interface *oi)
{
  struct ospf6_area *oa;
...
static int
config_write_ospf6_interface (struct vty *vty, struct vrf *vrf)
{
  struct ospf6_interface *oi;
  struct interface *ifp;
  ...
  FOR_ALL_INTERFACES (vrf, ifp)
    {
      ...
      if (oi->area_id_format != OSPF6_AREA_FMT_UNSET)
        {
          area_id2str (buf, sizeof (buf), oi->area_id, oi->area_id_format);
      ...
      if (oi->gr.hello_delay.interval != OSPF_HELLO_DELAY_DEFAULT)
        vty_out (vty, " ipv6 ospf6 graceful-restart hello-delay %u\n",
                 oi->gr.hello_delay.interval);
...
static int
ospf6_ifp_create (struct interface *ifp)
{
...
static int
ospf6_ifp_up (struct interface *ifp)
{
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug (
...
static int
ospf6_ifp_down (struct interface *ifp)
{
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug (
        "Zebra Interface state change: %s index %d flags %llx metric %d mtu %d bandwidth %d",
...
static int
ospf6_ifp_destroy (struct interface *ifp)
{
  if (if_is_up (ifp))
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Zebra Interface delete: %s index %d mtu %d", ifp->name,
                ifp->ifindex, ifp->mtu6);
...
void
ospf6_interface_clear (struct interface *ifp)
{
  struct ospf6_interface *oi;
...
void
ospf6_auth_write_config (struct vty *vty, struct ospf6_auth_data *at_data)
{
  if (CHECK_FLAG (at_data->flags, OSPF6_AUTH_TRAILER_KEYCHAIN))
    ...
    vty_out (vty, " ipv6 ospf6 authentication key-id %d hash-algo %s key %s\n",
             at_data->key_id,
...
void
ospf6_interface_auth_trailer_cmd_init (void)
{
  /*Install OSPF6 auth trailer commands at interface level */
  install_element (INTERFACE_NODE, &ipv6_ospf6_intf_auth_trailer_keychain_cmd);
  install_element (INTERFACE_NODE,
                   &no_ipv6_ospf6_intf_auth_trailer_keychain_cmd);
  install_element (INTERFACE_NODE, &ipv6_ospf6_intf_auth_trailer_key_cmd);
  install_element (INTERFACE_NODE, &no_ipv6_ospf6_intf_auth_trailer_key_cmd);
}
