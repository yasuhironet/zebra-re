    ...
    case OSPF6_SCOPE_TYPE_AREA:
      {
        ...
        switch (table->table_type)
          ...
          case OSPF6_TABLE_TYPE_SPF_RESULTS:
            snprintf (name, sizeof (name), "area %s spf table", oa->name);
            break;
          case OSPF6_TABLE_TYPE_ROUTES:
            snprintf (name, sizeof (name), "area %s route table", oa->name);
            break;
          case OSPF6_TABLE_TYPE_PREFIX_RANGES:
            snprintf (name, sizeof (name), "area %s range table", oa->name);
            break;
          case OSPF6_TABLE_TYPE_SUMMARY_PREFIXES:
            snprintf (name, sizeof (name), "area %s summary prefix table",
                      oa->name);
            break;
          case OSPF6_TABLE_TYPE_SUMMARY_ROUTERS:
            snprintf (name, sizeof (name), "area %s summary router table",
                      oa->name);
            break;
          default:
            snprintf (name, sizeof (name), "area %s unknown table", oa->name);
            break;
          }
      }
      break;
...
void
ospf6_linkstate_prefix (uint32_t adv_router, uint32_t id,
...
void
ospf6_add_route_nexthop_blackhole (struct ospf6_route *route)
{
  struct ospf6_nexthop *nh;
  struct ospf6_nexthop nh_match = {};
...
ospf6_route_zebra_copy_nexthops (struct ospf6_route *route,
                                 struct zapi_nexthop nexthops[], int entries,
                                 vrf_id_t vrf_id)
{
  struct ospf6_nexthop *nh;
  struct listnode *node;
          ...
          if (IS_OSPF6_DEBUG_ZEBRA (SEND))
            {
              zlog_debug ("  nexthop: %s %pI6%%%.*s(%d)",
                          nexthop_type_to_str (nh->type), &nh->address,
                          IFNAMSIZ, ifindex2ifname (nh->ifindex, vrf_id),
                          nh->ifindex);
            }
...
int
ospf6_nexthop_cmp (struct ospf6_nexthop *a, struct ospf6_nexthop *b)
{
...
static int
ospf6_path_cmp (struct ospf6_path *a, struct ospf6_path *b)
{
...
void
ospf6_path_free (struct ospf6_path *op)
{
  if (op->nh_list)
...
struct ospf6_path *
ospf6_path_dup (struct ospf6_path *path)
{
...
void
ospf6_copy_paths (struct list *dst, struct list *src)
{
...
struct ospf6_route *
ospf6_route_create (struct ospf6 *ospf6)
{
...
struct ospf6_route *
ospf6_route_lookup (struct prefix *prefix, struct ospf6_route_table *table)
{
...
route_table_assert (struct ospf6_route_table *table)
{
  ...
  for (r = ospf6_route_head (table); r; r = ospf6_route_next (r))
    zlog_info ("%p<-[%p]->%p : %pFX", r->prev, r, r->next, &r->prefix);
  zlog_debug ("DUMP END");
...
ospf6_route_add (struct ospf6_route *route, struct ospf6_route_table *table)
{
  ...
  if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
    zlog_debug ("%s %p: route add %p: %s paths %u nh %u",
                ...
                buf, listcount (route->paths), listcount (route->nh_list));
  ...
  if (old)
    {
      ...
      if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
        zlog_debug (
            ...
            listcount (route->nh_list), (void *) old, old->path.cost,
            listcount (old->paths), listcount (old->nh_list));
      else if (IS_OSPF6_DEBUG_ROUTE (TABLE))
      ...
      if (node->info == old)
        {
          ...
          if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
            zlog_debug ("%s:  replace old route %s", __func__, buf);
        }
  ...
  if (prev || next)
    {
      if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
        zlog_debug (
            ...
            ospf6_route_table_name (table), (void *) table, (void *) route,
            route->path.cost, (void *) prev, (void *) next,
            route_node_get_lock_count (node));
      else if (IS_OSPF6_DEBUG_ROUTE (TABLE))
        zlog_debug ("%s: route add cost %u: another path found",
                    ospf6_route_table_name (table), route->path.cost);
      ...
      if (node->info == next)
        {
          ...
          if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
            zlog_debug (
                ...
                ospf6_route_table_name (table), (void *) table, (void *) route,
                route->path.cost, (void *) next, next->path.cost);
        }
...
ospf6_route_remove (struct ospf6_route *route, struct ospf6_route_table *table)
{
  ...
  if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
    zlog_debug ("%s %p: route remove %p: %s cost %u refcount %u",
                ospf6_route_table_name (table), (void *) table, (void *) route,
                buf, route->path.cost, route->lock);
  else if (IS_OSPF6_DEBUG_ROUTE (TABLE))
      ...
      if (route->next && route->next->rnode == node)
        {
          ...
          if (IS_OSPF6_DEBUG_ROUTE (MEMORY))
            zlog_debug ("%s: remove route %s", __func__, buf);
        }
      else
        {
          node->info = NULL;
...
struct ospf6_route *
ospf6_route_match_next (struct prefix *prefix, struct ospf6_route *route)
{
  struct ospf6_route *next;
                  ...
                  json_object *json_routes, bool use_json)
{
  ...
  if (route->type == OSPF6_DEST_TYPE_LINKSTATE)
    ...
    inet_ntop (route->prefix.family, &route->prefix.u.prefix, destination,
               sizeof (destination));
  else
      ...
      if (use_json)
        {
          ...
          json_object_string_add (json_next_hop, "nextHop", nexthop);
          json_object_string_add (json_next_hop, "interfaceName",
                                  ifindex2ifname (nh->ifindex, vrf_id));
          json_object_array_add (json_array_next_hops, json_next_hop);
        }
      else
        {
          if (! i)
            {
              vty_out (vty, "%c%1s %2s %-30s %-25s %6.*s %s\n",
                       (ospf6_route_is_best (route) ? '*' : ' '),
                       OSPF6_DEST_TYPE_SUBSTR (route->type),
                       OSPF6_PATH_TYPE_SUBSTR (route->path.type), destination,
                       nexthop, IFNAMSIZ, ifindex2ifname (nh->ifindex, vrf_id),
                       duration);
              i++;
            }
          else
  ...
  if (use_json)
    {
      json_object_object_add (json_route, "nextHops", json_array_next_hops);
                         ...
                         json_object *json_routes, bool use_json)
{
  ...
  timerstring (&res, duration, sizeof (duration));
  if (use_json)
    json_object_string_add (json_route, "installedTimeSince", duration);
  else
  ...
  timerstring (&res, duration, sizeof (duration));
  if (use_json)
    json_object_string_add (json_route, "changedTimeSince", duration);
  else
  ...
  if (use_json)
    {
      ...
      snprintf (flag, sizeof (flag), "%s%s%s%s",
  ...
  if (use_json)
    {
      json_object_string_add (json_route, "lsOriginRoutePathType",
                              ospf6_lstype_name (route->path.origin.type));
      json_object_string_add (json_route, "lsId", id);
      json_object_string_add (json_route, "lsAdvertisingRouter", adv_router);
    }
  else
    {
  ...
  ospf6_prefix_options_printbuf (route->prefix_options, pfx_options,
                                 sizeof (pfx_options));
  if (use_json)
    json_object_string_add (json_route, "prefixOptions", pfx_options);
  else
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json_route, "metricCostE2", route->path.u.cost_e2);

      json_object_int_add (json_route, "pathsCount", route->paths->count);
      json_object_int_add (json_route, "nextHopCount", route->nh_list->count);
    }
  else
    {
  ...
  if (use_json)
    {
      ...
      json_object_int_add (json, "numberOfAlternativeRoutes", alternative);
  ...
  for (i = OSPF6_PATH_TYPE_INTRA; i <= OSPF6_PATH_TYPE_EXTERNAL2; i++)
    {
      if (use_json)
        ...
        vty_out (vty, "Number of %s routes: %d\n", OSPF6_PATH_TYPE_NAME (i),
                 pathtype[i]);
    }
}

static void
ospf6_route_show_table_prefix (struct vty *vty, struct prefix *prefix,
                               struct ospf6_route_table *table,
                               json_object *json, bool use_json)
{
  struct ospf6_route *route;
...
static void
ospf6_route_show_table_address (struct vty *vty, struct prefix *prefix,
                                struct ospf6_route_table *table,
                                json_object *json, bool use_json)
{
  struct ospf6_route *route;
...
static void
ospf6_route_show_table_type (struct vty *vty, int detail, uint8_t type,
                             struct ospf6_route_table *table,
                             json_object *json, bool use_json)
{
  struct ospf6_route *route;
...
int
ospf6_route_table_show (struct vty *vty, int argc_start, int argc,
                        ...
                        struct ospf6_route_table *table, bool use_json)
{
  ...
  for (i = argc_start; i < arg_end; i++)
    {
      ...
      if (use_json)
        json_object_string_add (json, "malformedArgument", argv[i]->arg);
      else
  ...
  if (isprefix && ! match)
    {
      ...
      if (! slash)
        ospf6_route_show_table_address (vty, &prefix, table, json, use_json);
      else
        ospf6_route_show_table_prefix (vty, &prefix, table, json, use_json);
  ...
  if (match)
    ospf6_route_show_table_match (vty, detail, &prefix, table, json, use_json);
  else if (type)
    ospf6_route_show_table_type (vty, detail, type, table, json, use_json);
  else
...
ospf6_linkstate_show (struct vty *vty, struct ospf6_route *route)
{
  ...
  if (ntohl (id))
    vty_out (vty, "%-7s %-15s %-15s %-8s %-14s %lu\n", "Network", routername,
             idname, rbits, options, (unsigned long) route->path.cost);
  else
    vty_out (vty, "%-7s %-15s %-15s %-8s %-14s %lu\n", "Router", routername,
             idname, rbits, options, (unsigned long) route->path.cost);
}
...
static void
ospf6_linkstate_show_table_exact (struct vty *vty, struct prefix *prefix,
                                  struct ospf6_route_table *table)
{
  struct ospf6_route *route;
                            ...
                            struct ospf6_route_table *table)
{
  ...
  if (is_router)
    ospf6_linkstate_prefix (router.u.prefix4.s_addr, id.u.prefix4.s_addr,
                            &prefix);
