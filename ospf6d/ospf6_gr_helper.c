...
struct ospf6_lsa_handler grace_lsa_handler = { .lh_type =
...
static unsigned int
ospf6_enable_rtr_hash_key (const void *data)
{
...
static bool
ospf6_enable_rtr_hash_cmp (const void *d1, const void *d2)
{
...
static void *
ospf6_enable_rtr_hash_alloc (void *p)
{
  struct advRtr *rid;
...
static void
ospf6_disable_rtr_hash_free (void *rtr)
{
  XFREE (MTYPE_OSPF6_GR_HELPER, rtr);
}

static void
ospf6_enable_rtr_hash_destroy (struct ospf6 *ospf6)
{
 ...
 */
static int
ospf6_extract_grace_lsa_fields (struct ospf6_lsa *lsa, uint32_t *interval,
                                uint8_t *reason)
{
 ...
 */
static void
ospf6_handle_grace_timer_expiry (struct event *thread)
{
 ...
 */
static bool
ospf6_check_chg_in_rxmt_list (struct ospf6_neighbor *nbr)
{
 ...
 */
int
ospf6_process_grace_lsa (struct ospf6 *ospf6, struct ospf6_lsa *lsa,
 ...
 */
void
ospf6_gr_helper_exit (struct ospf6_neighbor *nbr,
 ...
 */
void
ospf6_process_maxage_grace_lsa (struct ospf6 *ospf6, struct ospf6_lsa *lsa,
                                struct ospf6_neighbor *restarter)
{
  uint8_t restart_reason = 0;
  uint32_t grace_interval = 0;
 ...
 */
void
ospf6_helper_handle_topo_chg (struct ospf6 *ospf6, struct ospf6_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF6_GR)
    zlog_debug ("%s, Topo change detected due to lsa details : %s", __func__,
                lsa->name);
 ...
 */
static void
ospf6_gr_helper_support_set (struct ospf6 *ospf6, bool support)
{
 ...
 */
static void
ospf6_gr_helper_lsacheck_set (struct ospf6 *ospf6, bool enabled)
{
 ...
 */
static void
ospf6_gr_helper_supported_gracetime_set (struct ospf6 *ospf6,
                                         uint32_t interval)
{
...
static int
ospf6_print_vty_helper_dis_rtr_walkcb (struct hash_bucket *bucket, void *arg)
{
...
/* API to walk and print  all the Helper supported router ids.*/
static int
ospf6_print_json_helper_dis_rtr_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct advRtr *rtr = bucket->data;
 ...
 */
static void
ospf6_gr_helper_support_set_per_routerid (struct ospf6 *ospf6,
...
static void
show_ospfv6_gr_helper_per_nbr (struct vty *vty, json_object *json, bool uj,
                               struct ospf6_neighbor *nbr)
{
  if (! uj)
    {
...
static void
show_ospf6_gr_helper_details (struct vty *vty, struct ospf6 *ospf6,
                              json_object *json, bool uj, bool detail)
{
  ...
  if (! uj)
  ...
  else
    {
      ...
      json_object_string_add (
          json, "strictLsaCheck",
          (ospf6->ospf6_helper_cfg.strict_lsa_check) ? "Enabled" : "Disabled");

#if CONFDATE > 20240401
      CPP_NOTICE ("Remove deprecated json key: restartSupoort")
#endif
      json_object_string_add (json, "restartSupoort",
        ...
        for (ALL_LIST_ELEMENTS_RO (oa->if_list, j, oi))
          {
            ...
            if (uj)
              {
                json_object_object_get_ex (json, "neighbors", &json_neighbors);
                if (! json_neighbors)
                  {
...
DEFPY (no_ospf6_gr_helper_planned_only,
 ...
 */
static int
ospf6_grace_lsa_show_info (struct vty *vty, struct ospf6_lsa *lsa,
                           json_object *json, bool use_json)
{
  struct ospf6_lsa_header *lsah = NULL;
  struct tlv_header *tlvh = NULL;
...
void
ospf6_gr_helper_config_init (void)
{
 ...
 */
void
ospf6_gr_helper_init (struct ospf6 *ospf6)
{
  if (IS_DEBUG_OSPF6_GR)
  ...
  ospf6->ospf6_helper_cfg.enable_rtr_list =
      hash_create (ospf6_enable_rtr_hash_key, ospf6_enable_rtr_hash_cmp,
 ...
 */
void
ospf6_gr_helper_deinit (struct ospf6 *ospf6)
{
...
static int
ospf6_cfg_write_helper_enable_rtr_walkcb (struct hash_bucket *backet,
                                          void *arg)
{
...
int
config_write_ospf6_gr_helper (struct vty *vty, struct ospf6 *ospf6)
{
  if (ospf6->ospf6_helper_cfg.is_helper_supported)
...
int
config_write_ospf6_debug_gr_helper (struct vty *vty)
{
  if (IS_DEBUG_OSPF6_GR)
