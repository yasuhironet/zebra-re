...
static ssize_t
vrrp_send_garp (struct interface *ifp, uint8_t *buf, ssize_t pack_len)
{
...
static ssize_t
vrrp_build_garp (uint8_t *buf, struct interface *ifp, struct in_addr *v4)
{
  uint8_t *arp_ptr;
...
void
vrrp_garp_send (struct vrrp_router *r, struct in_addr *v4)
{
...
void
vrrp_garp_send_all (struct vrrp_router *r)
{
...
vrrp_garp_init (void)
{
  ...
  frr_with_privs (&vrrp_privs)
    {
      garp_fd =
          socket (PF_PACKET, SOCK_RAW | SOCK_CLOEXEC, htons (ETH_P_RARP));
    }

  if (garp_fd > 0)
    {
      DEBUGD (&vrrp_dbg_sock, VRRP_LOGPFX "Initialized gratuitous ARP socket");
      DEBUGD (&vrrp_dbg_arp,
              VRRP_LOGPFX "Initialized gratuitous ARP subsystem");
    }
  else
    {
      zlog_err (VRRP_LOGPFX "Error initializing gratuitous ARP subsystem");
    }
}
...
vrrp_garp_fini (void)
{
  ...
  DEBUGD (&vrrp_dbg_arp, VRRP_LOGPFX "Deinitialized gratuitous ARP subsystem");
}

bool
vrrp_garp_is_init (void)
{
  return garp_fd > 0;
}
