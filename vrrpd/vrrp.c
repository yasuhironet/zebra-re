 ...
 */
static void
vrrp_mac_set (struct ethaddr *mac, bool v6, uint8_t vrid)
{
  /*
 ...
 */
static void
vrrp_recalculate_timers (struct vrrp_router *r)
{
 ...
 */
static bool
vrrp_is_owner (struct interface *ifp, struct ipaddr *addr)
{
  /*
 ...
 */
static bool
vrrp_ifp_has_vrrp_mac (struct interface *ifp)
{
...
vrrp_lookup_by_if_mvl (struct interface *mvl_ifp)
{
  struct interface *p;
      ...
      ! vrrp_ifp_has_vrrp_mac (mvl_ifp))
    {
      if (mvl_ifp && mvl_ifp->link_ifindex == 0)
        DEBUGD (&vrrp_dbg_zebra,
                VRRP_LOGPFX "Interface %s has no parent ifindex; disregarding",
                mvl_ifp->name);
 ...
 */
static struct list *
vrrp_lookup_by_if (struct interface *ifp)
{
 ...
 */
static struct list *
vrrp_lookup_by_if_any (struct interface *ifp)
{
  struct vrrp_vrouter *vr;
  struct list *vrs;
...
void
vrrp_check_start (struct vrrp_vrouter *vr)
{
  ...
  whynot = (! start && ! whynot) ? "No Virtual IP address configured" : whynot;
  if (start)
...
void
vrrp_set_priority (struct vrrp_vrouter *vr, uint8_t priority)
{
...
void
vrrp_set_advertisement_interval (struct vrrp_vrouter *vr,
...
static bool
vrrp_has_ip (struct vrrp_vrouter *vr, struct ipaddr *ip)
{
...
int
vrrp_add_ip (struct vrrp_vrouter *vr, struct ipaddr *ip)
{
...
int
vrrp_add_ipv4 (struct vrrp_vrouter *vr, struct in_addr v4)
{
  struct ipaddr ip;
...
int
vrrp_add_ipv6 (struct vrrp_vrouter *vr, struct in6_addr v6)
{
  assert (vr->version != 2);
...
int
vrrp_del_ip (struct vrrp_vrouter *vr, struct ipaddr *ip)
{
  struct listnode *ln, *nn;
  struct ipaddr *iter;
  int ret = 0;
...
int
vrrp_del_ipv6 (struct vrrp_vrouter *vr, struct in6_addr v6)
{
  struct ipaddr ip;
...
int
vrrp_del_ipv4 (struct vrrp_vrouter *vr, struct in_addr v4)
{
  struct ipaddr ip;
...
static void
vrrp_router_addr_list_del_cb (void *val)
{
 ...
 */
static bool
vrrp_attach_interface (struct vrrp_router *r)
{
  /* Search for existing interface with computed MAC address */
  struct interface **ifps;

  size_t ifps_cnt = if_lookup_by_hwaddr (r->vmac.octet, sizeof (r->vmac.octet),
                                         &ifps, r->vr->ifp->vrf->vrf_id);
  ...
  if (candidates == 0)
    ...
    zlog_info (VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM "Interface: %s",
               r->vr->vrid, family2str (r->family), selection->name);
...
static struct vrrp_router *
vrrp_router_create (struct vrrp_vrouter *vr, int family)
{
  struct vrrp_router *r =
...
static void
vrrp_router_destroy (struct vrrp_router *r)
{
  if (r->is_active)
...
struct vrrp_vrouter *
vrrp_vrouter_create (struct interface *ifp, uint8_t vrid, uint8_t version)
{
  ...
  vr->checksum_with_ipv4_pseudoheader = vd.checksum_with_ipv4_pseudoheader;
...
void
vrrp_vrouter_destroy (struct vrrp_vrouter *vr)
{
...
struct vrrp_vrouter *
vrrp_lookup (const struct interface *ifp, uint8_t vrid)
{
  if (! ifp)
    return NULL;
 ...
 */
static int
vrrp_bind_to_primary_connected (struct vrrp_router *r)
{
 ...
 */
static void
vrrp_send_advertisement (struct vrrp_router *r)
{
  struct vrrp_pkt *pkt;
  ssize_t pktsz;
  ...
  if (r->src.ipa_type == IPADDR_NONE && vrrp_bind_to_primary_connected (r) < 0)
    return;
  ...
  if (sent < 0)
    {
      zlog_warn (VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
                 ...
                 r->vr->vrid, family2str (r->family), safe_strerror (errno));
    }
  else
    {
 ...
 */
static int
vrrp_recv_advertisement (struct vrrp_router *r, struct ipaddr *src,
                         struct vrrp_pkt *pkt, size_t pktsize)
{
  ...
  bool adveq =
      (pkt->hdr.v2.adver_int == MAX (r->vr->advertisement_interval / 100, 1));
  if (r->vr->version == 2 && ! adveq)
    {
      DEBUGD (
          ...
          r->vr->vrid, family2str (r->family), pkt->hdr.v2.adver_int,
  ...
  switch (r->fsm.state)
      ...
      if (pkt->hdr.priority == 0)
               ...
               ((pkt->hdr.priority == r->priority) && addrcmp > 0))
        {
          zlog_info (
              ...
              r->vr->vrid, family2str (r->family), sipstr, pkt->hdr.priority);
          EVENT_OFF (r->t_adver_timer);
          if (r->vr->version == 3)
            {
              r->master_adver_interval = htons (pkt->hdr.v3.adver_int);
            }
    ...
    case VRRP_STATE_BACKUP:
      ...
      else if (! r->vr->preempt_mode || pkt->hdr.priority >= r->priority)
        {
          if (r->vr->version == 3)
            {
...
vrrp_read (struct event *thread)
{
  ...
  if (DEBUG_MODE_CHECK (&vrrp_dbg_pkt, DEBUG_MODE_ALL))
    {
      DEBUGD (&vrrp_dbg_pkt,
              VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM "Datagram rx: ",
              r->vr->vrid, family2str (r->family));
      zlog_hexdump (r->ibuf, nbytes);
    }

  pktsize = vrrp_pkt_parse_datagram (
      r->family, r->vr->version, r->vr->checksum_with_ipv4_pseudoheader, &m,
      nbytes, &src, &pkt, errbuf, sizeof (errbuf));
 ...
 */
static int
vrrp_socket (struct vrrp_router *r)
{
  int ret;
  bool failed = false;
  ...
  if (r->vr->ifp->vrf->vrf_id != VRF_DEFAULT)
    {
      frr_with_privs (&vrrp_privs)
        {
          ret = setsockopt (r->sock_tx, SOL_SOCKET, SO_BINDTODEVICE,
                            r->mvl_ifp->name, strlen (r->mvl_ifp->name));
        }
  ...
  if (r->family == AF_INET)
    {
      ...
      frr_with_privs (&vrrp_privs)
        {
          ret = setsockopt (r->sock_rx, SOL_SOCKET, SO_BINDTODEVICE,
                            r->vr->ifp->name, strlen (r->vr->ifp->name));
        }
      if (ret)
        {
          zlog_warn (VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
      ...
      ret = setsockopt_ipv4_multicast (r->sock_rx, IP_ADD_MEMBERSHIP, v4,
  ...
  else if (r->family == AF_INET6)
    {
      ...
      ret = setsockopt (r->sock_rx, IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq,
                        sizeof (mreq));
      if (ret < 0)
        {
          zlog_warn (VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM
...
DEFINE_HOOK (vrrp_change_state_hook, (struct vrrp_router * r, int to),
             (r, to));
 ...
 */
static void
vrrp_change_state_master (struct vrrp_router *r)
{
 ...
 */
static void
vrrp_change_state_backup (struct vrrp_router *r)
{
 ...
 */
static void
vrrp_change_state_initialize (struct vrrp_router *r)
{
 ...
 */
static void
vrrp_change_state (struct vrrp_router *r, int to)
{
  if (r->fsm.state == to)
    return;
 ...
 */
static void
vrrp_adver_timer_expire (struct event *thread)
{
  struct vrrp_router *r = EVENT_ARG (thread);

  DEBUGD (&vrrp_dbg_proto,
          VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM "Adver_Timer expired",
          r->vr->vrid, family2str (r->family));
 ...
 */
static void
vrrp_master_down_timer_expire (struct event *thread)
{
  struct vrrp_router *r = EVENT_ARG (thread);
 ...
 */
static int
vrrp_startup (struct vrrp_router *r)
{
  ...
  if (r->vr->priority == VRRP_PRIO_MASTER ||
      vrrp_is_owner (r->vr->ifp, primary))
    {
      ...
      zlog_info (
          ...
          r->vr->vrid, family2str (r->family), r->vr->ifp->name, ipbuf);
    }
 ...
 */
static int
vrrp_shutdown (struct vrrp_router *r)
{
  uint8_t saved_prio;
 ...
 */
int
vrrp_event (struct vrrp_router *r, int event)
{
 ...
 */
static void
vrrp_autoconfig_autoaddrupdate (struct vrrp_router *r)
{
  struct listnode *ln;
  struct connected *c = NULL;
  bool is_v6_ll;
  ...
  for (ALL_LIST_ELEMENTS_RO (r->mvl_ifp->connected, ln, c))
    {
      ...
      if (c->address->family == r->family && ! is_v6_ll)
        {
          DEBUGD (&vrrp_dbg_auto,
                  VRRP_LOGPFX VRRP_LOGPFX_VRID VRRP_LOGPFX_FAM "Adding %pFX",
                  r->vr->vrid, family2str (r->family), c->address);
          if (r->family == AF_INET)
 ...
 */
static int
vrrp_autoconfig_if_add (struct interface *ifp)
{
  bool created = false;
  struct vrrp_vrouter *vr;
 ...
 */
static int
vrrp_autoconfig_if_del (struct interface *ifp)
{
  if (! vrrp_autoconfig_is_on)
    return 0;
 ...
 */
static int
vrrp_autoconfig_if_up (struct interface *ifp)
{
  if (! vrrp_autoconfig_is_on)
    return 0;
 ...
 */
static int
vrrp_autoconfig_if_down (struct interface *ifp)
{
  if (! vrrp_autoconfig_is_on)
    return 0;
 ...
 */
static int
vrrp_autoconfig_if_address_add (struct interface *ifp)
{
  if (! vrrp_autoconfig_is_on)
    return 0;
 ...
 */
static int
vrrp_autoconfig_if_address_del (struct interface *ifp)
{
  if (! vrrp_autoconfig_is_on)
    return 0;
...
void
vrrp_autoconfig_on (int version)
{
 ...
 */
static void
vrrp_bind_pending (struct interface *mvl_ifp)
{
  struct vrrp_vrouter *vr;

  DEBUGD (&vrrp_dbg_zebra,
          VRRP_LOGPFX "Searching for instances that could use interface %s",
          mvl_ifp->name);
  ...
  for (ALL_LIST_ELEMENTS_RO (vrs, ln, vr))
    {
      ...
      if (vr->ifp == ifp || vr->v4->mvl_ifp == ifp || vr->v6->mvl_ifp == ifp)
        {
          DEBUGD (&vrrp_dbg_auto,
                  VRRP_LOGPFX VRRP_LOGPFX_VRID "Interface %s down", vr->vrid,
                  ifp->name);
        }
    }
...
void
vrrp_if_add (struct interface *ifp)
{
  vrrp_bind_pending (ifp);
...
void
vrrp_if_del (struct interface *ifp)
{
  struct listnode *ln;
  struct vrrp_vrouter *vr;
...
void
vrrp_if_address_add (struct interface *ifp)
{
  struct vrrp_vrouter *vr;
  struct listnode *ln;
  struct list *vrs;
...
void
vrrp_if_address_del (struct interface *ifp)
{
  /*
...
int
vrrp_config_write_global (struct vty *vty)
{
...
static bool
vrrp_hash_cmp (const void *arg1, const void *arg2)
{
...
vrrp_init (void)
{
  ...
  vd.advertisement_interval =
      yang_get_default_uint16 ("%s/advertisement-interval", VRRP_XPATH_FULL);
  vd.preempt_mode = yang_get_default_bool ("%s/preempt", VRRP_XPATH_FULL);
  vd.accept_mode = yang_get_default_bool ("%s/accept-mode", VRRP_XPATH_FULL);
  ...
  vrrp_vrouters_hash =
      hash_create (&vrrp_hash_key, vrrp_hash_cmp, "VRRP virtual router hash");
