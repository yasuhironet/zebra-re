 ...
 */
static int
vrrp_ndisc_una_build (struct interface *ifp, struct ipaddr *ip, uint8_t *buf,
                      size_t bufsiz)
{
...
int
vrrp_ndisc_una_send (struct vrrp_router *r, struct ipaddr *ip)
{
...
int
vrrp_ndisc_una_send_all (struct vrrp_router *r)
{
  assert (r->family == AF_INET6);
...
vrrp_ndisc_init (void)
{
  ...
  if (ndisc_fd > 0)
    ...
    {
      zlog_err (VRRP_LOGPFX "Error initializing Neighbor Discovery socket");
    }
}

void
vrrp_ndisc_fini (void)
{
  close (ndisc_fd);
  ndisc_fd = -1;
