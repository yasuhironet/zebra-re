 ...
 */
DEFPY_YANG(vrrp_vrid,                        vrrp_vrid_cmd,
 ...
 */
DEFPY_YANG(vrrp_shutdown,                            vrrp_shutdown_cmd,
 ...
 */
DEFPY_YANG(vrrp_priority,                            vrrp_priority_cmd,
 ...
 * XPath: /frr-interface:lib/interface/frr-vrrpd:vrrp/vrrp-group/priority
 */
DEFPY_YANG(no_vrrp_priority,                               no_vrrp_priority_cmd,
 ...
 */
DEFPY_YANG(vrrp_advertisement_interval,                                          vrrp_advertisement_interval_cmd,
 ...
 * /frr-interface:lib/interface/frr-vrrpd:vrrp/vrrp-group/advertisement-interval
 */
DEFPY_YANG(no_vrrp_advertisement_interval,                                             no_vrrp_advertisement_interval_cmd,
 ...
 */
DEFPY_YANG(vrrp_ip,                      vrrp_ip_cmd,
 ...
 */
DEFPY_YANG(vrrp_ip6,                       vrrp_ip6_cmd,
 ...
 */
DEFPY_YANG(vrrp_preempt,                           vrrp_preempt_cmd,
 ...
 *        ipv4-pseudoheader
 */
DEFPY_YANG(vrrp_checksum_with_ipv4_pseudoheader,                                                   vrrp_checksum_with_ipv4_pseudoheader_cmd,
...
/* XXX: yang conversion */
DEFPY_YANG(vrrp_autoconfigure,                                 vrrp_autoconfigure_cmd,
...
/* XXX: yang conversion */
DEFPY_YANG(vrrp_default,                           vrrp_default_cmd,
...
vrrp_build_json (struct vrrp_vrouter *vr)
{
  ...
  if (vr->v4->addrs->count)
    {
      for (ALL_LIST_ELEMENTS_RO (vr->v4->addrs, ln, ip))
        {
          inet_ntop (vr->v4->family, &ip->ipaddr_v4, ipstr, sizeof (ipstr));
          json_object_array_add (v4_addrs, json_object_new_string (ipstr));
        }
    }
  ...
  json_object_int_add (v6_stats, "neighborAdverTx", vr->v6->stats.una_tx_cnt);
  ...
  if (vr->v6->addrs->count)
    {
      for (ALL_LIST_ELEMENTS_RO (vr->v6->addrs, ln, ip))
        {
          inet_ntop (vr->v6->family, &ip->ipaddr_v6, ipstr, sizeof (ipstr));
          json_object_array_add (v6_addrs, json_object_new_string (ipstr));
        }
    }
 ...
 */
static void
vrrp_show (struct vty *vty, struct vrrp_vrouter *vr)
{
  char ethstr4[ETHER_ADDR_STRLEN];
  char ethstr6[ETHER_ADDR_STRLEN];
  char ipstr[INET6_ADDRSTRLEN];
  ...
  ttable_add_row (tt, "%s|%s", "Autoconfigured", vr->autoconf ? "Yes" : "No");
  ...
  ttable_add_row (tt, "%s|%hhu", "Effective Priority (v4)", vr->v4->priority);
  ttable_add_row (tt, "%s|%hhu", "Effective Priority (v6)", vr->v6->priority);
  ttable_add_row (tt, "%s|%s", "Preempt Mode",
                  vr->preempt_mode ? "Yes" : "No");
  ttable_add_row (tt, "%s|%s", "Accept Mode", vr->accept_mode ? "Yes" : "No");
  ...
  ttable_add_row (tt, "%s|%d ms", "Skew Time (v4)", vr->v4->skew_time * CS2MS);
  ttable_add_row (tt, "%s|%d ms", "Skew Time (v6)", vr->v6->skew_time * CS2MS);
 ...
 */
static int
vrrp_instance_display_sort_cmp (const void **d1, const void **d2)
{
...
DEFPY_YANG(vrrp_vrid_show,                             vrrp_vrid_show_cmd,
...
DEFPY_YANG(vrrp_vrid_show_summary,                                     vrrp_vrid_show_summary_cmd,
...
DEFPY_YANG(debug_vrrp,                         debug_vrrp_cmd,
...
vrrp_vty_init (void)
{
  ...
  install_element (INTERFACE_NODE, &vrrp_checksum_with_ipv4_pseudoheader_cmd);
}
