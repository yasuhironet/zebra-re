 ...
 */
static uint16_t
vrrp_pkt_checksum (struct vrrp_pkt *pkt, size_t pktsize, struct ipaddr *src,
                   bool ipv4_ph)
{
  uint16_t chksum;
...
ssize_t
vrrp_pkt_adver_build (struct vrrp_pkt **pkt, struct ipaddr *src,
...
void
vrrp_pkt_free (struct vrrp_pkt *pkt)
{
  XFREE (MTYPE_VRRP_PKT, pkt);
}

size_t
vrrp_pkt_adver_dump (char *buf, size_t buflen, struct vrrp_pkt *pkt)
{
  if (buflen < 1)
    return 0;
  ...
  snprintf (tmpbuf, sizeof (tmpbuf), "type %u (%s), ", (hdr->vertype & 0x0F),
...
ssize_t
vrrp_pkt_parse_datagram (int family, int version, bool ipv4_ph,
                         struct msghdr *m, size_t read, struct ipaddr *src,
...
#define VRRP_PKT_VCHECK(cond, _f, ...)                                        \
  do                                                                          \
    {                                                                         \
      if (! (cond))                                                           \
        {                                                                     \
          if (errmsg)                                                         \
            snprintf (errmsg, errmsg_len, (_f), ##__VA_ARGS__);               \
          return -1;                                                          \
        }                                                                     \
    }                                                                         \
  while (0)
  ...
  if (family == AF_INET)
    {
      VRRP_PKT_VCHECK (read >= sizeof (struct ip),
      ...
      /* TTL check */
      VRRP_PKT_VCHECK (ip->ip_ttl == 255, "IPv4 TTL is %hhu; should be 255",
                       ip->ip_ttl);
  ...
  else if (family == AF_INET6)
    {
      ...
      VRRP_PKT_VCHECK (*hoplimit == 255,
                       "IPv6 Hop Limit is %hhu; should be 255", *hoplimit);
      ...
      *pkt = (struct vrrp_pkt *) buf;
      ...
      memcpy (&src->ipaddr_v6, &sa->sin6_addr, sizeof (struct in6_addr));
    }
  else
    {
  ...
  /* Size check */
  size_t minsize =
      (family == AF_INET) ? VRRP_MIN_PKT_SIZE_V4 : VRRP_MIN_PKT_SIZE_V6;
  size_t maxsize =
      (family == AF_INET) ? VRRP_MAX_PKT_SIZE_V4 : VRRP_MAX_PKT_SIZE_V6;
  VRRP_PKT_VCHECK (pktsize >= minsize, "VRRP packet is undersized (%zu < %zu)",
                   pktsize, minsize);
  VRRP_PKT_VCHECK (pktsize <= maxsize, "VRRP packet is oversized (%zu > %zu)",
                   pktsize, maxsize);
  ...
  VRRP_PKT_VCHECK ((*pkt)->hdr.chksum == chksum,
                   "Bad VRRP checksum %hx; should be %hx", (*pkt)->hdr.chksum,
                   chksum);
  ...
  if (version == 2)
    VRRP_PKT_VCHECK ((*pkt)->hdr.v2.auth_type == 0,
                     "Bad authentication type %hhu", (*pkt)->hdr.v2.auth_type);
