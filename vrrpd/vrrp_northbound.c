 ...
 */
static int
lib_interface_vrrp_vrrp_group_create (struct nb_cb_create_args *args)
{
...
lib_interface_vrrp_vrrp_group_get_keys (struct nb_cb_get_keys_args *args)
{
  ...
  args->keys->num = 1;
  snprintf (args->keys->key[0], sizeof (args->keys->key[0]), "%u", vr->vrid);
 ...
 */
static void
vrrp_yang_add_del_virtual_address (const struct lyd_node *dnode, bool add)
{
  struct vrrp_vrouter *vr;
  struct ipaddr ip;
