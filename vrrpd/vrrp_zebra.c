...
static void
vrrp_zebra_debug_if_state (struct interface *ifp, const char *func)
{
  DEBUGD (
      &vrrp_dbg_zebra,
      ...
      ifp->link_ifindex, ifp->hw_addr[0], ifp->hw_addr[1], ifp->hw_addr[2],
      ifp->hw_addr[3], ifp->hw_addr[4], ifp->hw_addr[5], (long) ifp->flags,
      ifp->metric, ifp->mtu, if_is_operative (ifp));
}

static void
vrrp_zebra_debug_if_dump_address (struct interface *ifp, const char *func)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, ifc))
    {
      ...
      DEBUGD (&vrrp_dbg_zebra, "%s: interface %s address %pFX %s", func,
              ifp->name, p,
...
static void
vrrp_zebra_connected (struct zclient *zclient)
{
...
static int
vrrp_router_id_update_zebra (int command, struct zclient *zclient,
...
int
vrrp_ifp_create (struct interface *ifp)
{
...
int
vrrp_ifp_destroy (struct interface *ifp)
{
  vrrp_zebra_debug_if_state (ifp, __func__);
...
int
vrrp_ifp_up (struct interface *ifp)
{
  vrrp_zebra_debug_if_state (ifp, __func__);
...
int
vrrp_ifp_down (struct interface *ifp)
{
  vrrp_zebra_debug_if_state (ifp, __func__);
...
static int
vrrp_zebra_if_address_add (int command, struct zclient *zclient,
                           zebra_size_t length, vrf_id_t vrf_id)
{
  struct connected *c;
...
static int
vrrp_zebra_if_address_del (int command, struct zclient *client,
                           zebra_size_t length, vrf_id_t vrf_id)
{
  struct connected *c;
...
void
vrrp_zebra_radv_set (struct vrrp_router *r, bool enable)
{
...
void
vrrp_zclient_send_interface_protodown (struct interface *ifp, bool down)
{
  DEBUGD (&vrrp_dbg_zebra,
