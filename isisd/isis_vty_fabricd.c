       ...
       "Trigger CSNP for all LSPs, not only circuit-scoped\n")
{
  ...
  fabricd_configure_triggered_csnp (area, FABRICD_DEFAULT_CSNP_DELAY, false);
  return CMD_SUCCESS;
}

static void
lsp_print_flooding (struct vty *vty, struct isis_lsp *lsp, struct isis *isis)
{
  char lspid[255];
  ...
  vty_out (vty, "    Last received on: %s (",
           lsp->flooding_interface ? lsp->flooding_interface : "(null)");
  ...
  for (enum isis_tx_type type = TX_LSP_NORMAL; type <= TX_LSP_CIRCUIT_SCOPED;
       type++)
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (lsp->flooding_neighbors[type], node,
                                 neighbor_id))
        {
...
static int
isis_vty_password_set (struct vty *vty, int argc, struct cmd_token *argv[],
                       int level)
{
  VTY_DECLVAR_CONTEXT (isis_area, area);
  ...
  if (strmatch (argv[idx_algo]->text, "clear"))
    {
      return isis_area_passwd_cleartext_set (area, level, passwd, snp_auth);
    }
  else if (strmatch (argv[idx_algo]->text, "md5"))
    {
      return isis_area_passwd_hmac_md5_set (area, level, passwd, snp_auth);
    }
...
static int
isis_vty_lsp_mtu_set (struct vty *vty, unsigned int lsp_mtu)
{
  VTY_DECLVAR_CONTEXT (isis_area, area);
  struct listnode *node;
  struct isis_circuit *circuit;
