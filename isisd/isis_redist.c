...
DEFINE_MTYPE_STATIC (ISISD, ISIS_RMAP_NAME,
...
static void
isis_redist_install (struct isis_area *area, int level, const struct prefix *p,
 ...
 * Schedule an lsp regenerate if necessary. */
static void
isis_redist_uninstall (struct isis_area *area, int level,
...
void
isis_redist_add (struct isis *isis, int type, struct prefix *p,
    ...
    for (level = 1; level <= ISIS_LEVELS; level++)
      {
        ...
        isis_redist_update_ext_reach (area, level, redist, p, src_p, info);
      }
}

void
isis_redist_delete (struct isis *isis, int type, struct prefix *p,
  ...
  if (is_default_prefix (p) && (! src_p || ! src_p->prefixlen))
    {
       ...
       * DEFAULT_ROUTE. */
      isis_redist_add (isis, DEFAULT_ROUTE, p, NULL, 254, MAX_WIDE_PATH_METRIC,
                       0);
      return;
    }
...
void
isis_redist_free (struct isis *isis)
{
...
void
isis_redist_set (struct isis_area *area, int level, int family, int type,
...
void
isis_redist_unset (struct isis_area *area, int level, int family, int type)
{
  struct isis_redist *redist = get_redist_settings (area, family, type, level);
  struct route_table *er_table = get_ext_reach (area, family, level);
  struct route_node *rn;
    ...
    for (level = 0; level < ISIS_LEVELS; level++)
      {
        ...
        if (! area->ext_reach[protocol][level])
          continue;
        for (rn = route_top (area->ext_reach[protocol][level]); rn;
             rn = srcdest_route_next (rn))
          {
            if (rn->info)
              XFREE (MTYPE_ISIS_EXT_INFO, rn->info);
...
int
isis_redist_config_write (struct vty *vty, struct isis_area *area, int family)
{
  int type;
  int level;
  int write = 0;
  ...
  for (level = 1; level <= ISIS_LEVELS; level++)
    {
      ...
      if (! redist->redist)
        continue;
      vty_out (vty, " default-information originate %s", family_str);
      if (! fabricd)
        vty_out (vty, " level-%d", level);
