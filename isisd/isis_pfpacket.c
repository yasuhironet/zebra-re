...
isis_multicast_join (int fd, int registerto, int if_num)
{
  ...
  if (IS_DEBUG_EVENTS)
    zlog_debug (
        ...
        mreq.mr_address[1], mreq.mr_address[2], mreq.mr_address[3],
        mreq.mr_address[4], mreq.mr_address[5]);
#endif /* EXTREME_DEBUG */
  if (setsockopt (fd, SOL_PACKET, PACKET_ADD_MEMBERSHIP, &mreq,
                  sizeof (struct packet_mreq)))
    {
      zlog_warn ("%s: setsockopt(): %s", __func__, safe_strerror (errno));
      return ISIS_WARNING;
    }
...
open_packet_socket (struct isis_circuit *circuit)
{
  ...
  if (fd < 0)
    {
      zlog_warn ("%s: socket() failed %s", __func__, safe_strerror (errno));
      return ISIS_WARNING;
    }
  ...
  if (bind (fd, (struct sockaddr *) (&s_addr), sizeof (struct sockaddr_ll)) <
      0)
    {
      zlog_warn ("%s: bind() failed: %s", __func__, safe_strerror (errno));
      close (fd);
      return ISIS_WARNING;
    }
  ...
  if (if_is_broadcast (circuit->interface))
    {
      ...
      /* joining ALL_L1_ISS */
      retval |=
          isis_multicast_join (circuit->fd, 1, circuit->interface->ifindex);
      /* joining ALL_L2_ISS */
      retval |=
          isis_multicast_join (circuit->fd, 2, circuit->interface->ifindex);
...
int
isis_recv_pdu_bcast (struct isis_circuit *circuit, uint8_t *ssnpa)
{
  ...
  bytesread = recvfrom (circuit->fd, (void *) &llc, LLC_LEN, MSG_PEEK,
  ...
  if ((bytesread < 0) ||
      (s_addr.sll_ifindex != (int) circuit->interface->ifindex))
    {
      if (bytesread < 0)
        {
          zlog_warn ("%s: ifname %s, fd %d, bytesread %d, recvfrom(): %s",
  ...
  unsigned int max_size = circuit->interface->mtu > circuit->interface->mtu6
                              ? circuit->interface->mtu
                              : circuit->interface->mtu6;
  uint8_t temp_buff[max_size];
  bytesread = recvfrom (circuit->fd, temp_buff, max_size, MSG_DONTWAIT,
                        (struct sockaddr *) &s_addr, (socklen_t *) &addr_len);
  if (bytesread < 0)
    {
      zlog_warn ("%s: recvfrom() failed", __func__);
...
int
isis_recv_pdu_p2p (struct isis_circuit *circuit, uint8_t *ssnpa)
{
  int bytesread, addr_len;
  struct sockaddr_ll s_addr;
