...
static oid isis_snmp_trap_data_var_pdu_lsp_id[] = { ISIS_MIB,
                                                    ISIS_NOTIFICATION_ENTRY,
                                                    ISIS_PDU_LSPID, 0 };
static oid isis_snmp_trap_data_var_pdu_fragment[] = { ISIS_MIB,
                                                      ISIS_NOTIFICATION_ENTRY,
                                                      ISIS_PDU_FRAGMENT, 0 };
static oid isis_snmp_trap_data_var_pdu_field_len[] = { ISIS_MIB,
                                                       ISIS_NOTIFICATION_ENTRY,
                                                       ISIS_PDU_FIELDLEN, 0 };
static oid isis_snmp_trap_data_var_pdu_max_area_addr[] = {
  ISIS_MIB, ISIS_NOTIFICATION_ENTRY, ISIS_PDU_MAXAREAADDR, 0
};
static oid isis_snmp_trap_data_var_pdu_proto_ver[] = { ISIS_MIB,
                                                       ISIS_NOTIFICATION_ENTRY,
                                                       ISIS_PDU_PROTOVER, 0 };
static oid isis_snmp_trap_data_var_pdu_lsp_size[] = { ISIS_MIB,
                                                      ISIS_NOTIFICATION_ENTRY,
                                                      ISIS_PDU_LSPSIZE, 0 };
static oid isis_snmp_trap_data_var_adj_state[] = { ISIS_MIB,
                                                   ISIS_NOTIFICATION_ENTRY,
                                                   ISIS_ADJ_STATE, 0 };
static oid isis_snmp_trap_data_var_error_offset[] = { ISIS_MIB,
                                                      ISIS_NOTIFICATION_ENTRY,
                                                      ISIS_ERROR_OFFSET, 0 };
...
static uint8_t *isis_snmp_find_man_area (struct variable *, oid *, size_t *,
                                         int, size_t *, WriteMethod **);
...
static uint8_t *isis_snmp_find_circ_counter (struct variable *, oid *,
                                             size_t *, int, size_t *,
                                             WriteMethod **);
...
static uint8_t *isis_snmp_find_isadj_ipaddr (struct variable *, oid *,
                                             size_t *, int, size_t *,
                                             WriteMethod **);
...
static struct variable isis_var_arr[] = {
  ...
  { ISIS_SYS_POLLESHELLORATE, UNSIGNED32, RONLY, isis_snmp_find_sys_object },
  ...
  { ISIS_SYS_NOTIFICATIONENABLE, INTEGER, RONLY, isis_snmp_find_sys_object },
  ...
  { ISIS_SYSLEVEL_MINLSPGENINT, UNSIGNED32, RONLY, isis_snmp_find_sys_level },
  ...
  { ISIS_SYSSTAT_CORRLSPS, COUNTER32, RONLY, isis_snmp_find_system_counter },
  ...
  { ISIS_NEXTCIRC_INDEX, UNSIGNED32, RONLY, isis_snmp_find_next_circ_index },
  ...
  { ISIS_CIRCLEVEL_WIDEMETRIC, UNSIGNED32, RONLY, isis_snmp_find_circ_level },
  { ISIS_CIRCLEVEL_ISPRIORITY, UNSIGNED32, RONLY, isis_snmp_find_circ_level },
  ...
  { ISIS_CIRCLEVEL_HELLOTIMER, UNSIGNED32, RONLY, isis_snmp_find_circ_level },
  { ISIS_CIRCLEVEL_DRHELLOTIMER, UNSIGNED32, RONLY,
    isis_snmp_find_circ_level },
  { ISIS_CIRCLEVEL_LSPTHROTTLE, UNSIGNED32, RONLY, isis_snmp_find_circ_level },
  ...
  { ISIS_CIRC_MAXAREAADDRMISMATCHES, COUNTER32, RONLY,
    isis_snmp_find_circ_counter },
  { ISIS_CIRC_AUTHTYPEFAILS, COUNTER32, RONLY, isis_snmp_find_circ_counter },
  { ISIS_CIRC_AUTHFAILS, COUNTER32, RONLY, isis_snmp_find_circ_counter },
  { ISIS_CIRC_LANDESISCHANGES, COUNTER32, RONLY, isis_snmp_find_circ_counter },
...
isis_snmp_adj_state (enum isis_adj_state state)
{
  switch (state)
    {
...
static inline uint32_t
isis_snmp_adj_neightype (enum isis_system_type type)
{
  switch (type)
    {
...
static int
isis_circuit_snmp_id_gen (struct isis_circuit *circuit)
{
  uint32_t id;
  uint32_t i;
...
static int
isis_circuit_snmp_id_free (struct isis_circuit *circuit)
{
...
isis_snmp_circuit_next (struct isis_circuit *circuit)
{
  uint32_t start;
  uint32_t off;
 ...
 */
static int
isis_snmp_circuit_get_level_lo (struct isis_circuit *circuit)
{
...
/* Check level match */
static int
isis_snmp_get_level_match (int is_type, int level)
{
 ...
 */
static int
isis_snmp_conv_exact (uint8_t *buf, size_t max_len, size_t *out_len,
...
static int
isis_snmp_conv_next (uint8_t *buf, size_t max_len, size_t *out_len,
 ...
 */
static int
isis_snmp_area_addr_lookup_exact (oid *oid_idx, size_t oid_idx_len,
...
static int
isis_snmp_area_addr_lookup_next (oid *oid_idx, size_t oid_idx_len,
                                 struct isis_area **ret_area,
                                 struct iso_address **ret_addr)
{
  uint8_t cmp_buf[ISIS_SNMP_OSI_ADDR_LEN_MAX];
  ...
  for (ALL_LIST_ELEMENTS_RO (area->area_addrs, addr_node, addr))
    {
      ...
      if (found_addr == NULL || addr->addr_len < found_addr->addr_len ||
          (addr->addr_len == found_addr->addr_len &&
           memcmp (addr->area_addr, found_addr->area_addr, addr->addr_len) <
               0))
        {
          found_area = area;
          found_addr = addr;
 ...
 */
static int
isis_snmp_circuit_lookup_exact (oid *oid_idx, size_t oid_idx_len,
                                struct isis_circuit **ret_circuit)
{
  struct isis_circuit *circuit;

  if (oid_idx == NULL || oid_idx_len < 1 || oid_idx[0] > SNMP_CIRCUITS_MAX)
    return 0;
...
static int
isis_snmp_circuit_lookup_next (oid *oid_idx, size_t oid_idx_len,
                               struct isis_circuit **ret_circuit)
{
  oid off;
  oid start;
 ...
 */
static int
isis_snmp_circuit_level_lookup_exact (oid *oid_idx, size_t oid_idx_len,
                                      int check_match,
                                      struct isis_circuit **ret_circuit,
                                      int *ret_level)
{
...
static int
isis_snmp_circuit_level_lookup_next (oid *oid_idx, size_t oid_idx_len,
                                     int check_match,
                                     struct isis_circuit **ret_circuit,
                                     int *ret_level)
{
 ...
 */
static int
isis_snmp_adj_helper (struct isis_adjacency *adj, int data_id, oid data_off,
                      uint8_t **ret_data, size_t *ret_data_len)
{
  uint8_t *data = NULL;
  size_t data_len = 0;
  ...
  switch (data_id)
    ...
    case ISIS_SNMP_ADJ_DATA_IP_ADDR:
      ...
      if (data_off >= adj->ipv4_address_count)
        {
          data = (uint8_t *) &adj
                     ->ll_ipv6_addrs[data_off - adj->ipv4_address_count];
...
static int
isis_snmp_adj_lookup_exact (oid *oid_idx, size_t oid_idx_len, int data_id,
                            struct isis_adjacency **ret_adj, oid *ret_data_idx,
                            uint8_t **ret_data, size_t *ret_data_len)
{
  int res;
  ...
  for (ALL_LIST_ELEMENTS_RO (circuit->snmp_adj_list, node, tmp_adj))
    {
      ...
      if (tmp_adj->snmp_idx == adj_idx)
        {
          res = isis_snmp_adj_helper (tmp_adj, data_id, data_off, &data,
                                      &data_len);
          if (res)
            adj = tmp_adj;
...
static int
isis_snmp_adj_lookup_next (oid *oid_idx, size_t oid_idx_len, int data_id,
                           struct isis_adjacency **ret_adj, oid *ret_data_idx,
                           uint8_t **ret_data, size_t *ret_data_len)
{
  struct listnode *node;
      ...
      for (ALL_LIST_ELEMENTS_RO (circuit->snmp_adj_list, node, tmp_adj))
        {
          ...
          if (isis_snmp_adj_helper (tmp_adj, data_id, data_idx, &data,
                                    &data_len))
            {
              adj = tmp_adj;
              break;
...
static uint8_t *
isis_snmp_find_sys_object (struct variable *v, oid *name, size_t *length,
...
static uint8_t *
isis_snmp_find_man_area (struct variable *v, oid *name, size_t *length,
                         int exact, size_t *var_len,
                         WriteMethod **write_method)
{
  int res;
  ...
  *write_method = NULL;
  ...
  if (exact)
    {
      res = isis_snmp_area_addr_lookup_exact (oid_idx, oid_idx_len, NULL,
                                              &area_addr);
    ...
    {
      res = isis_snmp_area_addr_lookup_next (oid_idx, oid_idx_len, NULL,
                                             &area_addr);
...
static uint8_t *
isis_snmp_find_area_addr (struct variable *v, oid *name, size_t *length,
                          int exact, size_t *var_len,
                          WriteMethod **write_method)
{
  /*
...
static uint8_t *
isis_snmp_find_summ_addr (struct variable *v, oid *name, size_t *length,
                          int exact, size_t *var_len,
                          WriteMethod **write_method)
{
  /*
...
static uint8_t *
isis_snmp_find_redistribute_addr (struct variable *v, oid *name,
...
static uint8_t *
isis_snmp_find_router (struct variable *v, oid *name, size_t *length,
                       int exact, size_t *var_len, WriteMethod **write_method)
{
  ...
  *write_method = NULL;
  ...
  if (exact)
    {
      res = isis_snmp_conv_exact (cmp_buf, sizeof (cmp_buf), &cmp_len, oid_idx,
                                  oid_idx_len);

      if (! res || cmp_len != ISIS_SYS_ID_LEN || oid_idx_len != (cmp_len + 2))
        /*
  ...
  res = isis_snmp_conv_next (cmp_buf, sizeof (cmp_buf), &cmp_len, &try_exact,
                             oid_idx, oid_idx_len);
...
static uint8_t *
isis_snmp_find_sys_level (struct variable *v, oid *name, size_t *length,
                          int exact, size_t *var_len,
                          WriteMethod **write_method)
{
  oid *oid_idx;
      ...
      if (level_match)
        {
          if (area->overload_bit)
            return SNMP_INTEGER (ISIS_SNMP_LEVEL_STATE_OVERLOADED);
      ...
      if (level_match)
        {
          ...
          if (area->newmetric)
            return SNMP_INTEGER (ISIS_SNMP_METRIC_STYLE_WIDE);
...
static uint8_t *
isis_snmp_find_system_counter (struct variable *v, oid *name, size_t *length,
                               int exact, size_t *var_len,
                               WriteMethod **write_method)
{
  oid *oid_idx;
...
static uint8_t *
isis_snmp_find_next_circ_index (struct variable *v, oid *name, size_t *length,
                                int exact, size_t *var_len,
                                WriteMethod **write_method)
{
  /* Check whether the instance identifier is valid */
...
static uint8_t *
isis_snmp_find_circ (struct variable *v, oid *name, size_t *length, int exact,
                     size_t *var_len, WriteMethod **write_method)
{
  ...
  *write_method = NULL;
  ...
  if (exact)
    ...
    {
      res = isis_snmp_circuit_lookup_next (oid_idx, oid_idx_len, &circuit);
...
static uint8_t *
isis_snmp_find_circ_level (struct variable *v, oid *name, size_t *length,
                           int exact, size_t *var_len,
                           WriteMethod **write_method)
{
  ...
  *write_method = NULL;
  ...
  if (exact)
    {
      res = isis_snmp_circuit_level_lookup_exact (oid_idx, oid_idx_len, 1,
                                                  &circuit, &level);
    ...
    {
      res = isis_snmp_circuit_level_lookup_next (oid_idx, oid_idx_len, 1,
                                                 &circuit, &level);
  ...
  switch (v->magic)
    ...
    case ISIS_CIRCLEVEL_ID:
      ...
      /* Adjacency fully-up */
      memcpy (circuit_id_val, circuit->u.p2p.neighbor->sysid, ISIS_SYS_ID_LEN);
    ...
    case ISIS_CIRCLEVEL_LSPTHROTTLE:
      if (circuit->area)
        return SNMP_INTEGER (circuit->area->min_spf_interval[level - 1] *
                             1000);
      else
        return SNMP_INTEGER (0);
...
static uint8_t *
isis_snmp_find_circ_counter (struct variable *v, oid *name, size_t *length,
                             int exact, size_t *var_len,
                             WriteMethod **write_method)
{
...
static uint8_t *
isis_snmp_find_isadj (struct variable *v, oid *name, size_t *length, int exact,
                      size_t *var_len, WriteMethod **write_method)
{
  ...
  if (exact)
    {
      res = isis_snmp_adj_lookup_exact (oid_idx, oid_idx_len,
                                        ISIS_SNMP_ADJ_DATA_NONE, &adj, NULL,
                                        NULL, NULL);
      ...
      if (adj->last_upd != 0)
        {
          ...
          if (val < (adj->last_upd + adj->hold_time))
            return SNMP_INTEGER (adj->last_upd + adj->hold_time - val);
        }
...
static uint8_t *
isis_snmp_find_isadj_area (struct variable *v, oid *name, size_t *length,
                           int exact, size_t *var_len,
                           WriteMethod **write_method)
{
  ...
  *write_method = NULL;
  ...
  if (exact)
    {
      res = isis_snmp_adj_lookup_exact (oid_idx, oid_idx_len,
                                        ISIS_SNMP_ADJ_DATA_AREA_ADDR, &adj,
                                        NULL, &data, &data_len);
...
static uint8_t *
isis_snmp_find_isadj_ipaddr (struct variable *v, oid *name, size_t *length,
                             int exact, size_t *var_len,
                             WriteMethod **write_method)
{
  /* Index circuit-id 1-255 + adj-id 1-... */
  ...
  *write_method = NULL;
  ...
  if (exact)
    {
      res = isis_snmp_adj_lookup_exact (oid_idx, oid_idx_len,
                                        ISIS_SNMP_ADJ_DATA_IP_ADDR, &adj, NULL,
                                        &data, &data_len);
    ...
    {
      res = isis_snmp_adj_lookup_next (oid_idx, oid_idx_len,
                                       ISIS_SNMP_ADJ_DATA_IP_ADDR, &adj,
                                       &data_idx, &data, &data_len);
      if (! res)
        return NULL;
...
static uint8_t *
isis_snmp_find_isadj_prot_supp (struct variable *v, oid *name, size_t *length,
                                int exact, size_t *var_len,
                                WriteMethod **write_method)
{
  /* Index circuit-id 1-255 + adj-id 1-... */
  ...
  *write_method = NULL;
  ...
  if (exact)
    {
      res = isis_snmp_adj_lookup_exact (oid_idx, oid_idx_len,
                                        ISIS_SNMP_ADJ_DATA_PROTO, &adj, NULL,
                                        &data, &data_len);
...
/* Register ISIS-MIB. */
static int
isis_snmp_init (struct event_loop *tm)
{
      ...
      if (v->findVar != h2f->ihtp_func)
        {
          /* Next table */
          h2f++;
          assert (h2f < (isis_func_to_prefix_arr + isis_func_to_prefix_count));
          assert (v->findVar == h2f->ihtp_func);
        }

      v->namelen = h2f->ihtp_pref_len + 1;
      memcpy (v->name, h2f->ihtp_pref_oid, h2f->ihtp_pref_len * sizeof (oid));
 ...
 */
static int
isis_snmp_trap_throttle (oid trap_id)
{
  time_t time_now;
  struct isis *isis = isis_lookup_by_vrfid (VRF_DEFAULT);
...
static int
isis_snmp_db_overload_update (const struct isis_area *area)
{
...
static int
isis_snmp_lsp_exceed_max_update (const struct isis_area *area,
                                 const uint8_t *lsp_id)
{
  ...
  snmp_varlist_add_variable (&notification_vars,
                             ...
                             array_size (isis_snmp_trap_data_var_pdu_lsp_id),
                             STRING, lsp_id, ISIS_SYS_ID_LEN + 2);
 ...
 */
static void
isis_snmp_update_worker_a (const struct isis_circuit *circuit, oid trap_id,
                           const oid *oid_a, size_t oid_a_len, uint8_t type_a,
                           const void *data_a, size_t data_a_len,
                           const oid *oid_b, size_t oid_b_len, uint8_t type_b,
                           const void *data_b, size_t data_b_len)
{
  ...
  /* Put in trap value */
  memcpy (var_name, isis_snmp_notifications, sizeof (isis_snmp_notifications));
  ...
  if (oid_a_len != 0)
    {
      ...
      snmp_varlist_add_variable (&notification_vars, oid_a, oid_a_len, type_a,
                                 (uint8_t *) data_a, data_a_len);
    }
  ...
  if (oid_b_len != 0)
    {
      ...
      snmp_varlist_add_variable (&notification_vars, oid_b, oid_b_len, type_b,
                                 (uint8_t *) data_b, data_b_len);
    }
 ...
 */
static void
isis_snmp_update_worker_b (const struct isis_circuit *circuit, oid trap_id,
                           const oid *oid_a, size_t oid_a_len, uint8_t type_a,
                           const void *data_a, size_t data_a_len,
                           const oid *oid_b, size_t oid_b_len, uint8_t type_b,
                           const void *data_b, size_t data_b_len)
...
static int
isis_snmp_id_len_mismatch_update (const struct isis_circuit *circuit,
                                  ...
                                  size_t raw_pdu_len)
{
  ...
  isis_snmp_update_worker_a (
      ...
      array_size (isis_snmp_trap_data_var_pdu_field_len), UNSIGNED32, &val,
      sizeof (val), isis_snmp_trap_data_var_pdu_fragment,
      array_size (isis_snmp_trap_data_var_pdu_fragment), STRING, raw_pdu,
      raw_pdu_len);
  return 0;
}
                                         ...
                                         size_t raw_pdu_len)
{
  ...
  isis_snmp_update_worker_a (
      ...
      array_size (isis_snmp_trap_data_var_pdu_max_area_addr), UNSIGNED32, &val,
      sizeof (val), isis_snmp_trap_data_var_pdu_fragment,
      array_size (isis_snmp_trap_data_var_pdu_fragment), STRING, raw_pdu,
      raw_pdu_len);
  return 0;
...
static int
isis_snmp_own_lsp_purge_update (const struct isis_circuit *circuit,
                                const uint8_t *lsp_id)
{
  if (! isis_snmp_trap_throttle (ISIS_TRAP_OWN_LSP_PURGE))
    return 0;

  isis_snmp_update_worker_a (circuit, ISIS_TRAP_OWN_LSP_PURGE, NULL, 0, STRING,
                             NULL, 0, isis_snmp_trap_data_var_pdu_lsp_id,
                             array_size (isis_snmp_trap_data_var_pdu_lsp_id),
                             STRING, lsp_id, ISIS_SYS_ID_LEN + 2);
  return 0;
}
...
static int
isis_snmp_seqno_skipped_update (const struct isis_circuit *circuit,
                                const uint8_t *lsp_id)
{
  if (! isis_snmp_trap_throttle (ISIS_TRAP_SEQNO_SKIPPED))
    return 0;

  isis_snmp_update_worker_a (circuit, ISIS_TRAP_SEQNO_SKIPPED, NULL, 0, STRING,
                             NULL, 0, isis_snmp_trap_data_var_pdu_lsp_id,
                             array_size (isis_snmp_trap_data_var_pdu_lsp_id),
                             STRING, lsp_id, ISIS_SYS_ID_LEN + 2);
  return 0;
    ...
    size_t raw_pdu_len)
{
  ...
  isis_snmp_update_worker_a (circuit, ISIS_TRAP_AUTHEN_TYPE_FAILURE, NULL, 0,
                             STRING, NULL, 0,
                             isis_snmp_trap_data_var_pdu_fragment,
                             array_size (isis_snmp_trap_data_var_pdu_fragment),
                             STRING, raw_pdu, raw_pdu_len);
  return 0;
}
                                         ...
                                         size_t raw_pdu_len)
{
  ...
  isis_snmp_update_worker_a (circuit, ISIS_TRAP_AUTHEN_FAILURE, NULL, 0,
                             STRING, NULL, 0,
                             isis_snmp_trap_data_var_pdu_fragment,
                             array_size (isis_snmp_trap_data_var_pdu_fragment),
                             STRING, raw_pdu, raw_pdu_len);
...
static int
isis_snmp_version_skew_update (const struct isis_circuit *circuit,
                               ...
                               size_t raw_pdu_len)
{
  ...
  isis_snmp_update_worker_b (
      ...
      array_size (isis_snmp_trap_data_var_pdu_proto_ver), UNSIGNED32, &val,
      sizeof (val), isis_snmp_trap_data_var_pdu_fragment,
      array_size (isis_snmp_trap_data_var_pdu_fragment), STRING, raw_pdu,
      raw_pdu_len);
  return 0;
...
static int
isis_snmp_area_mismatch_update (const struct isis_circuit *circuit,
...
static int
isis_snmp_reject_adjacency_update (const struct isis_circuit *circuit,
                                   const char *raw_pdu, size_t raw_pdu_len)
{
  ...
  isis_snmp_update_worker_a (circuit, ISIS_TRAP_REJ_ADJACENCY, NULL, 0, STRING,
                             NULL, 0, isis_snmp_trap_data_var_pdu_fragment,
                             array_size (isis_snmp_trap_data_var_pdu_fragment),
                             STRING, raw_pdu, raw_pdu_len);
  return 0;
}
...
static int
isis_snmp_lsp_too_large_update (const struct isis_circuit *circuit,
                                uint32_t pdu_size, const uint8_t *lsp_id)
{
  ...
  isis_snmp_update_worker_b (
      circuit, ISIS_TRAP_LSP_TOO_LARGE, isis_snmp_trap_data_var_pdu_lsp_size,
      array_size (isis_snmp_trap_data_var_pdu_lsp_size), UNSIGNED32, &pdu_size,
      sizeof (pdu_size), isis_snmp_trap_data_var_pdu_lsp_id,
...
static int
isis_snmp_adj_state_change_update (const struct isis_adjacency *adj)
{
  ...
  isis_snmp_update_worker_b (adj->circuit, ISIS_TRAP_ADJ_STATE_CHANGE,
                             isis_snmp_trap_data_var_pdu_lsp_id,
                             array_size (isis_snmp_trap_data_var_pdu_lsp_id),
                             STRING, lsp_id, ISIS_SYS_ID_LEN + 2,
                             isis_snmp_trap_data_var_adj_state,
                             array_size (isis_snmp_trap_data_var_adj_state),
                             INTEGER, &val, sizeof (val));
  return 0;
}

static int
isis_snmp_lsp_error_update (const struct isis_circuit *circuit,
                            const uint8_t *lsp_id, char const *raw_pdu,
                            size_t raw_pdu_len)
{
  /*
   * This is a special case because
  ...
  snmp_varlist_add_variable (&notification_vars,
                             ...
                             array_size (isis_snmp_trap_data_var_error_offset),
                             UNSIGNED32, (uint8_t *) &val, sizeof (val));
...
static int
isis_snmp_module_init (void)
{
  hook_register (isis_hook_db_overload, isis_snmp_db_overload_update);
  hook_register (isis_hook_lsp_exceed_max, isis_snmp_lsp_exceed_max_update);
  hook_register (isis_hook_id_len_mismatch, isis_snmp_id_len_mismatch_update);
