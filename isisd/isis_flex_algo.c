...
void *
isis_flex_algo_data_alloc (void *voidarg)
{
      ...
      for (int level = ISIS_LEVEL1; level <= ISIS_LEVEL2; level++)
        {
          ...
          data->spftree[tree][level - 1] = isis_spftree_new (
              arg->area, &arg->area->lspdb[level - 1], arg->area->isis->sysid,
              level, tree, SPF_TYPE_FORWARD, 0, arg->algorithm);
        }
    }
...
void
isis_flex_algo_data_free (void *voiddata)
{
 ...
 *      election.
 *    - fad is not NULL and *fad is NULL: use no local router capability FAD
 * for the election.
 *    - fad and *fad are not NULL: uses the *fad local definition instead of
 * the local definition from LSDB for the election.
...
/**
 * @brief Check the Flex-Algo Definition is supported by the current FRR
 * version
 * @param flex-algo
 * @return true if supported else false
 */
bool
isis_flex_algo_supported (struct flex_algo *fad)
{
 ...
 * @param area pointer
 * @param local router capability Flex-Algo Definition (FAD) double pointer.
 * @return elected flex-algo-definition object if exist and supported, else
 * NULL
 */
static struct isis_router_cap_fad *
 ...
 */
bool
sr_algorithm_participated (const struct isis_lsp *lsp, uint8_t algorithm)
{
...
bool
isis_flex_algo_constraint_drop (struct isis_spftree *spftree,
                                struct isis_lsp *lsp,
  ...
  if (! admin_group_zero (&fad->fad.admin_group_exclude_any))
    {
      ret = admin_group_match_any (&fad->fad.admin_group_exclude_any,
                                   link_admin_group, link_ext_admin_group);
      if (ret)
        return true;
    }
