...
static int
pdu_type_to_counter_index (uint8_t pdu_type)
{
  switch (pdu_type)
    {
    case L1_LAN_HELLO:
...
static const char *
pdu_counter_index_to_name (enum pdu_counter_index index)
{
  switch (index)
    {
...
void
pdu_counter_count (pdu_counter_t counter, uint8_t pdu_type)
{
...
void
pdu_counter_print (struct vty *vty, const char *prefix, pdu_counter_t counter)
{
...
void
pdu_counter_count_drop (struct isis_area *area, uint8_t pdu_type)
{
  pdu_counter_count (area->pdu_drop_counters, pdu_type);

  if (area->log_pdu_drops)
    {
      isis_log_pdu_drops (area, pdu_counter_index_to_name (
                                    pdu_type_to_counter_index (pdu_type)));
    }
}

uint64_t
pdu_counter_get_count (pdu_counter_t counter, uint8_t pdu_type)
{
  int index = pdu_type_to_counter_index (pdu_type);
