...
struct isis_tx_queue
{
  ...
  void (*send_event) (struct isis_circuit *circuit, struct isis_lsp *,
                      enum isis_tx_type);
  struct hash *hash;
};
...
static unsigned
tx_queue_hash_key (const void *p)
{
  const struct isis_tx_queue_entry *e = p;

  uint32_t id_key =
      jhash (e->lsp->hdr.lsp_id, ISIS_SYS_ID_LEN + 2, 0x55aa5a5a);

  return jhash_1word (e->lsp->level, id_key);
}

static bool
tx_queue_hash_cmp (const void *a, const void *b)
{
  ...
  if (memcmp (ea->lsp->hdr.lsp_id, eb->lsp->hdr.lsp_id, ISIS_SYS_ID_LEN + 2))
    return false;
...
static void
tx_queue_element_free (void *element)
{
...
void
isis_tx_queue_free (struct isis_tx_queue *queue)
{
  hash_clean_and_free (&queue->hash, tx_queue_element_free);
  XFREE (MTYPE_TX_QUEUE, queue);
}

static struct isis_tx_queue_entry *
tx_queue_find (struct isis_tx_queue *queue, struct isis_lsp *lsp)
{
  struct isis_tx_queue_entry e = { .lsp = lsp };

  return hash_lookup (queue->hash, &e);
}

static void
tx_queue_send_event (struct event *thread)
{
...
void
_isis_tx_queue_add (struct isis_tx_queue *queue, struct isis_lsp *lsp,
                    enum isis_tx_type type, const char *func, const char *file,
                    int line)
{
  if (! queue)
    return;
  ...
  if (IS_DEBUG_TX_QUEUE)
    {
      zlog_debug ("Add LSP %pLS to %s queue as %s LSP. (From %s %s:%d)",
...
void
_isis_tx_queue_del (struct isis_tx_queue *queue, struct isis_lsp *lsp,
  ...
  if (IS_DEBUG_TX_QUEUE)
    {
      zlog_debug ("Remove LSP %pLS from %s queue. (From %s %s:%d)",
                  lsp->hdr.lsp_id, queue->circuit->interface->name, func, file,
                  line);
    }
...
unsigned long
isis_tx_queue_len (struct isis_tx_queue *queue)
{
  if (! queue)
    return 0;
...
void
isis_tx_queue_clean (struct isis_tx_queue *queue)
{
