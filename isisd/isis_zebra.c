...
static int
isis_router_id_update_zebra (ZAPI_CALLBACK_ARGS)
{
...
static int
isis_zebra_if_address_add (ZAPI_CALLBACK_ARGS)
{
...
static int
isis_zebra_if_address_del (ZAPI_CALLBACK_ARGS)
{
  struct isis_circuit *circuit;
  struct connected *c;
...
static int
isis_zebra_link_params (ZAPI_CALLBACK_ARGS)
{
...
static int
isis_zebra_add_nexthops (struct isis *isis, struct list *nexthops,
...
void
isis_zebra_route_add_route (struct isis *isis, struct prefix *prefix,
...
void
isis_zebra_route_del_route (struct isis *isis, struct prefix *prefix,
                            struct prefix_ipv6 *src_p,
                            struct isis_route_info *route_info)
{
  struct zapi_route api;
 ...
 */
void
isis_zebra_prefix_sid_install (struct isis_area *area, struct prefix *prefix,
 ...
 */
void
isis_zebra_prefix_sid_uninstall (struct isis_area *area, struct prefix *prefix,
 ...
 */
void
isis_zebra_send_adjacency_sid (int cmd, const struct sr_adjacency *sra)
{
  ...
  znh->type = (sra->nexthop.family == AF_INET) ? NEXTHOP_TYPE_IPV4_IFINDEX
...
static int
isis_zebra_read (ZAPI_CALLBACK_ARGS)
{
  struct zapi_route api;
  struct isis *isis = NULL;
   ...
   * without this check)
   */
  if (api.prefix.prefixlen == 0 && api.src_prefix.prefixlen == 0 &&
      api.type == PROTO_TYPE)
    {
...
void
isis_zebra_redistribute_set (afi_t afi, int type, vrf_id_t vrf_id)
{
  if (type == DEFAULT_ROUTE)
    zclient_redistribute_default (ZEBRA_REDISTRIBUTE_DEFAULT_ADD, zclient, afi,
                                  vrf_id);
  else
...
void
isis_zebra_redistribute_unset (afi_t afi, int type, vrf_id_t vrf_id)
{
  if (type == DEFAULT_ROUTE)
 ...
 */
int
isis_zebra_rlfa_register (struct isis_spftree *spftree, struct rlfa *rlfa)
{
  ...
  zlog_debug ("ISIS-LFA: registering RLFA %pFX@%pI4 with LDP", &rlfa->prefix,
              &rlfa->pq_address);

  ret =
      zclient_send_opaque_unicast (zclient, LDP_RLFA_REGISTER, ZEBRA_ROUTE_LDP,
                                   0, 0, (const uint8_t *) &zr, sizeof (zr));
 ...
 */
void
isis_zebra_rlfa_unregister_all (struct isis_spftree *spftree)
{
  struct isis_area *area = spftree->area;
 ...
 */
bool
isis_zebra_label_manager_ready (void)
{
 ...
 */
int
isis_zebra_request_label_range (uint32_t base, uint32_t chunk_size)
{
  int ret;
  uint32_t start, end;
 ...
 */
int
isis_zebra_release_label_range (uint32_t start, uint32_t end)
{
  int ret;
 ...
 */
int
isis_zebra_label_manager_connect (void)
{
...
void
isis_zebra_vrf_register (struct isis *isis)
{
  ...
  if (isis->vrf_id != VRF_UNKNOWN)
    {
      if (IS_DEBUG_EVENTS)
        zlog_debug ("%s: Register VRF %s id %u", __func__, isis->name,
                    isis->vrf_id);
...
void
isis_zebra_vrf_deregister (struct isis *isis)
{
  if (! zclient || zclient->sock < 0 || ! isis)
    return;
  ...
  if (isis->vrf_id != VRF_UNKNOWN)
    {
      if (IS_DEBUG_EVENTS)
        zlog_debug ("%s: Deregister VRF %s id %u", __func__, isis->name,
                    isis->vrf_id);
 ...
 */
int
isis_zebra_ls_register (bool up)
{
  int rc;
 ...
 */
static int
isis_opaque_msg_handler (ZAPI_CALLBACK_ARGS)
{
  struct stream *s;
...
static int
isis_zebra_client_close_notify (ZAPI_CALLBACK_ARGS)
{
  int ret = 0;
...
void
isis_zebra_init (struct event_loop *master, int instance)
{
