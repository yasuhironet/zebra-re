...
isis_nexthop_create (int family, const union g_addr *const ip,
                     ifindex_t ifindex)
{
...
static struct list *
isis_nexthop_list_dup (const struct list *orig)
{
  struct list *copy;
...
static struct isis_nexthop *
nexthoplookup (struct list *nexthops, int family, union g_addr *ip,
               ifindex_t ifindex)
{
  struct listnode *node;
  struct isis_nexthop *nh;
...
void
adjinfo2nexthop (int family, struct list *nexthops, struct isis_adjacency *adj,
...
static void
isis_route_add_dummy_nexthops (struct isis_route_info *rinfo,
                               const uint8_t *sysid,
                               struct isis_sr_psid_info *sr,
                               struct mpls_label_stack *label_stack)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (adjacencies, node, vadj))
    {
      ...
      if (CHECK_FLAG (im->options, F_ISIS_UNIT_TEST))
        {
          isis_route_add_dummy_nexthops (rinfo, sadj->id, sr, label_stack);
          if (! allow_ecmp)
            break;
          continue;
        }
      ...
      switch (prefix->family)
        ...
        default:
          flog_err (EC_LIB_DEVELOPMENT, "%s: unknown address family [%d]",
                    __func__, prefix->family);
          exit (1);
        }
      adjinfo2nexthop (prefix->family, rinfo->nexthops, adj, sr, label_stack);
      if (! allow_ecmp)
        break;
    }
...
void
isis_route_node_cleanup (struct route_table *table, struct route_node *node)
{
  if (node->info)
...
void
isis_route_table_info_free (void *info)
{
  XFREE (MTYPE_ISIS_ROUTE_TABLE_INFO, info);
}

uint8_t
isis_route_table_algorithm (const struct route_table *table)
{
...
static bool
isis_sr_psid_info_same (struct isis_sr_psid_info *new,
...
static bool
isis_label_stack_same (struct mpls_label_stack *new,
                      ...
                      char *buf, size_t buf_size)
{
  ...
  if (new->cost != old->cost)
    {
      if (buf)
        snprintf (buf, buf_size, "cost (old: %u, new: %u)", old->cost,
                  new->cost);
      return 0;
    }
  ...
  if (new->depth != old->depth)
    {
      if (buf)
        snprintf (buf, buf_size, "depth (old: %u, new: %u)", old->depth,
                  new->depth);
      return 0;
    }
      ...
      if (! isis_sr_psid_info_same (&new_sr_algo, &old_sr_algo))
        {
          if (buf)
            snprintf (buf, buf_size,
                      "SR input label algo-%u (old: %s, new: %s)", i,
                      old_sr_algo.present ? "yes" : "no",
  ...
  for (ALL_LIST_ELEMENTS_RO (new->nexthops, node, new_nh))
    {
      old_nh = nexthoplookup (old->nexthops, new_nh->family, &new_nh->ip,
                              new_nh->ifindex);
      if (! old_nh)
        {
          if (buf)
                   ...
                   struct isis_area *area, struct route_table *table)
{
  ...
  rinfo_new = isis_route_info_new (prefix, src_p, cost, depth, sr, adjacencies,
                                   allow_ecmp);
  ...
  if (! rinfo_old)
    {
      if (IS_DEBUG_RTE_EVENTS)
        zlog_debug ("ISIS-Rte (%s) route created: %pFX", area->area_tag,
                    prefix);
...
void
isis_route_delete (struct isis_area *area, struct route_node *rode,
...
static void
isis_route_remove_previous_sid (struct isis_area *area, struct prefix *prefix,
                                struct isis_route_info *route_info)
{
  /*
...
static void
set_merge_route_info_sr_algo (struct isis_route_info *mrinfo,
                              struct isis_route_info *rinfo)
{
  for (int i = 0; i < SR_ALGORITHM_COUNT; i++)
    {
...
static void
_isis_route_verify_table (struct isis_area *area, struct route_table *table,
      ...
      if (IS_DEBUG_RTE_EVENTS)
        {
          ...
          zlog_debug (
              "ISIS-Rte (%s): route validate: %s %s %s %s", area->area_tag,
...
void
isis_route_verify_table (struct isis_area *area, struct route_table *table,
...
void
isis_route_verify_merge (struct isis_area *area,
...
static void
_isis_route_verify_merge (struct isis_area *area, struct route_table **tables,
...
void
isis_route_switchover_nexthop (struct isis_area *area,
