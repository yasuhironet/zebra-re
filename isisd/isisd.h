...
struct isis
{
  ...
  uint32_t
      circuit_ids_used[8]; /* 256 bits to track circuit ids 1 through 255 */
...
#define DEFAULT_LSP_MTU 1497
  ...
  struct spf_prefix_priority_acl spf_prefix_priorities[SPF_PREFIX_PRIO_MAX];
...
DECLARE_HOOK (isis_area_overload_bit_update, (struct isis_area * area),
              (area));
...
void isis_area_advertise_high_metrics_set (struct isis_area *area,
                                           bool advertise_high_metrics);
void isis_area_attached_bit_send_set (struct isis_area *area,
                                      bool attached_bit);
