...
#define ISIS_CIRCUIT_IF_DOWN_FROM_Z    0x02
  ...
  uint32_t
      desig_changes[ISIS_LEVELS]; /* lanLxDesignatedIntermediateSystemChanges
                                   */
...
void isis_circuit_switchover_routes (struct isis_circuit *circuit, int family,
                                     ...
                                     ifindex_t ifindex);
void isis_circuit_stream (struct isis_circuit *circuit,
...
DECLARE_HOOK (isis_circuit_new_hook, (struct isis_circuit * circuit),
              (circuit));
DECLARE_HOOK (isis_circuit_del_hook, (struct isis_circuit * circuit),
              (circuit));
