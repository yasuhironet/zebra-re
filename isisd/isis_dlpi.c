...
static int
dlpisend (int fd, const void *cbuf, size_t cbuflen, const void *dbuf,
...
static int
dlpimcast (int fd, const uint8_t *mcaddr)
{
  struct
  {
...
open_dlpi_dev (struct isis_circuit *circuit)
{
  ...
  if (fd == -1)
    {
      ...
      *cp = '\0';
      ...
      if (dia->dl_provider_style != DL_STYLE2)
        {
          zlog_warn ("%s: interface %s: %s is not style 2", __func__,
  ...
  else
    {
      ...
      if (dia->dl_provider_style != DL_STYLE1)
        {
          zlog_warn ("%s: interface %s: %s is not style 1", __func__,
                     circuit->interface->name, devpath);
          close (fd);
          return ISIS_WARNING;
        }
  ...
  if (dlpiaddr (fd, circuit->u.bc.snpa) == -1)
    {
      zlog_warn ("%s: interface %s: unable to get MAC address", __func__,
                 circuit->interface->name);
      close (fd);
      return ISIS_WARNING;
    }
  ...
  if (ioctl (fd, I_PUSH, "pfmod") == 0)
    {
      ...
      if (ioctl (fd, I_STR, &sioc) == -1)
        zlog_warn ("%s: could not perform PF_IOCSETF on %s", __func__,
                   circuit->interface->name);
    }
...
isis_recv_pdu_bcast (struct isis_circuit *circuit, uint8_t *ssnpa)
{
  ...
  if (retv < 0)
    {
      zlog_warn ("%s: getmsg failed: %s", __func__, safe_strerror (errno));
      return ISIS_WARNING;
    }
  ...
  if ((size_t) buflen > sizeof (sock_buff))
    {
      zlog_warn (
          ...
          __func__, sizeof (sock_buff), buflen, circuit->interface->name);
      return ISIS_WARNING;
    }
