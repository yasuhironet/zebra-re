...
DEFINE_MTYPE_STATIC (ISISD, ISIS_LFA_EXCL_IFACE,
...
static inline int
isis_spf_node_compare (const struct isis_spf_node *a,
 ...
 */
void
isis_spf_node_list_init (struct isis_spf_nodes *nodes)
{
 ...
 */
void
isis_spf_node_list_clear (struct isis_spf_nodes *nodes)
{
...
isis_spf_node_new (struct isis_spf_nodes *nodes, const uint8_t *sysid)
{
 ...
 */
struct isis_spf_node *
isis_spf_node_find (const struct isis_spf_nodes *nodes, const uint8_t *sysid)
{
  struct isis_spf_node node = {};
 ...
 */
int
lfa_tiebreaker_cmp (const struct lfa_tiebreaker *a,
 ...
 */
void
isis_lfa_tiebreakers_init (struct isis_area *area, int level)
{
 ...
 */
void
isis_lfa_tiebreakers_clear (struct isis_area *area, int level)
{
...
isis_lfa_tiebreaker_add (struct isis_area *area, int level, uint8_t index,
 ...
 */
void
isis_lfa_tiebreaker_delete (struct isis_area *area, int level,
                            struct lfa_tiebreaker *tie_b)
{
...
static bool
lfa_excl_interface_hash_cmp (const void *value1, const void *value2)
{
  return strmatch (value1, value2);
}

static unsigned int
lfa_excl_interface_hash_make (const void *value)
{
  return string_hash_make (value);
}

static void *
lfa_excl_interface_hash_alloc (void *p)
{
  return XSTRDUP (MTYPE_ISIS_LFA_EXCL_IFACE, p);
}

static void
lfa_excl_interface_hash_free (void *arg)
{
 ...
 */
void
isis_lfa_excluded_ifaces_init (struct isis_circuit *circuit, int level)
{
  circuit->lfa_excluded_ifaces[level - 1] =
      hash_create (lfa_excl_interface_hash_make, lfa_excl_interface_hash_cmp,
 ...
 */
void
isis_lfa_excluded_ifaces_clear (struct isis_circuit *circuit, int level)
{
 ...
 */
void
isis_lfa_excluded_iface_add (struct isis_circuit *circuit, int level,
                             const char *ifname)
{
 ...
 */
void
isis_lfa_excluded_iface_delete (struct isis_circuit *circuit, int level,
                                const char *ifname)
{
  char *found;

  found =
      hash_lookup (circuit->lfa_excluded_ifaces[level - 1], (char *) ifname);
  if (found)
    {
 ...
 */
bool
isis_lfa_excluded_iface_check (struct isis_circuit *circuit, int level,
                               const char *ifname)
{
 ...
 */
bool
isis_lfa_excise_adj_check (const struct isis_spftree *spftree,
                           const uint8_t *id)
{
 ...
 */
bool
isis_lfa_excise_node_check (const struct isis_spftree *spftree,
                            const uint8_t *id)
{
  const struct lfa_protected_resource *resource;
...
static int
tilfa_find_pnode_prefix_sid_cb (const struct prefix *prefix, uint32_t metric,
...
static uint32_t
tilfa_find_pnode_prefix_sid (struct isis_spftree *spftree,
                             const uint8_t *sysid)
{
  struct isis_lsp *lsp;
...
static int
tilfa_find_qnode_adj_sid_cb (const uint8_t *id, uint32_t metric,
                             bool oldmetric, struct isis_ext_subtlvs *subtlvs,
                             void *arg)
{
...
static mpls_label_t
tilfa_find_qnode_adj_sid (struct isis_spftree *spftree,
                          ...
                          const uint8_t *qnode_sysid)
{
  ...
  isis_lsp_iterate_is_reach (lsp, spftree->mtid, tilfa_find_qnode_adj_sid_cb,
                             &args);
  ...
  for (ALL_LIST_ELEMENTS_RO (repair_list, node, sid))
    {
      ...
      switch (sid->type)
        ...
        case TILFA_SID_ADJ:
          ...
          break;
        default:
          flog_err (EC_LIB_DEVELOPMENT, "%s: unknown TI-LFA SID type [%u]",
                    __func__, sid->type);
          exit (1);
        }
...
static int
tilfa_repair_list_apply (struct isis_spftree *spftree,
  ...
  for (ALL_LIST_ELEMENTS_RO (vertex_dest->Adj_N, node, vadj))
    {
      ...
      label_stack =
          tilfa_compute_label_stack (spftree->lspdb, sadj, repair_list);
      if (! label_stack)
        {
 ...
 */
static bool
lfa_ext_p_space_check (const struct isis_spftree *spftree_pc,
...
static bool
lfa_q_space_check (const struct isis_spftree *spftree_pc,
                   const struct isis_vertex *vertex)
{
...
static int
tilfa_build_repair_list (struct isis_spftree *spftree_pc,
                         struct isis_vertex *vertex_dest,
                         const struct isis_vertex *vertex,
  ...
  if ((! is_qnode ||
      ...
      vertex_child)
    {
      ...
      if (vertex->depth == (vertex_dest->depth - 2) &&
          VTYPE_IP (vertex_dest->type) && vertex_dest->N.ip.sr.present &&
...
static const char *
lfa_protection_type2str (enum lfa_protection_type type)
{
  switch (type)
    {
 ...
 */
static bool
lfa_check_needs_protection (const struct isis_spftree *spftree_pc,
                            const struct isis_vertex *vertex)
{
 ...
 */
int
isis_tilfa_check (struct isis_spftree *spftree_pc, struct isis_vertex *vertex)
{
  struct isis_spf_nodes used_pnodes;
  char buf[VID2STR_BUFFER];
  struct list *repair_list;
  ...
  if (! lfa_check_needs_protection (spftree_pc, vertex))
    {
      if (IS_DEBUG_LFA)
        zlog_debug (
            "ISIS-LFA: %s %s unaffected by %s", vtype2string (vertex->type),
            vid2string (vertex, buf, sizeof (buf)),
  ...
  if (VTYPE_IS (vertex->type))
    {
      ...
      if (adj &&
          isis_sr_adj_sid_find (adj, spftree_pc->family, ISIS_SR_LAN_BACKUP))
        {
          if (IS_DEBUG_LFA)
...
static bool
vertex_is_affected (struct isis_spftree *spftree_root,
  ...
  for (ALL_LIST_ELEMENTS_RO (vertex->parents, node, pvertex))
    {
      ...
      vertex_child =
          isis_find_vertex (&spftree_parent->paths, &vertex->N, vertex->type);
      if (! vertex_child)
        goto parents;
      ...
      for (ALL_LIST_ELEMENTS_RO (vertex_child->Adj_N, vnode, vadj))
        if (spf_adj_check_is_affected (vadj->sadj, resource,
                                       spftree_root->sysid, reverse))
          return true;

    parents:
      if (vertex_is_affected (spftree_root, adj_nodes, p_space, pvertex,
                              resource))
        return true;
    }
...
static void
lfa_calc_reach_nodes (struct isis_spftree *spftree,
  ...
  for (ALL_QUEUE_ELEMENTS_RO (&spftree->paths, node, vertex))
    {
      ...
      if (! vertex_is_affected (spftree_root, adj_nodes, p_space, vertex,
                                resource))
        {
          if (IS_DEBUG_LFA)
 ...
 */
struct isis_spftree *
isis_spf_reverse_run (const struct isis_spftree *spftree)
{
  ...
  spftree_reverse = isis_spftree_new (
      ...
      F_SPFTREE_NO_ADJACENCIES | F_SPFTREE_NO_ROUTES, spftree->algorithm);
 ...
 */
static void
lfa_calc_pq_spaces (struct isis_spftree *spftree_pc,
                    const struct lfa_protected_resource *resource)
{
      ...
      else
        {
          if (IS_DEBUG_LFA)
            zlog_debug ("ISIS-LFA: computing P-space (%s)",
                        print_sys_hostname (adj_node->sysid));
          lfa_calc_reach_nodes (adj_node->lfa.spftree, spftree, adj_nodes,
                                true, resource, &adj_node->lfa.p_space);
        }
    }
}
                    ...
                    struct lfa_protected_resource *resource)
{
  ...
  spftree_pc = isis_spftree_new (
      area, spftree->lspdb, spftree->sysid, spftree->level, spftree->tree_id,
      SPF_TYPE_TI_LFA, spftree->flags, spftree->algorithm);
 ...
 */
int
isis_spf_run_neighbors (struct isis_spftree *spftree)
{
  struct isis_lsp *lsp;
  struct isis_spf_node *adj_node;
  ...
  RB_FOREACH (adj_node, isis_spf_nodes, &spftree->adj_nodes)
    {
      ...
      adj_node->lfa.spftree = isis_spftree_new (
          spftree->area, spftree->lspdb, adj_node->sysid, spftree->level,
          spftree->tree_id, SPF_TYPE_FORWARD,
          F_SPFTREE_NO_ADJACENCIES | F_SPFTREE_NO_ROUTES, spftree->algorithm);
...
rlfa_pq_node_rtr_id (struct isis_spftree *spftree,
...
int
rlfa_cmp (const struct rlfa *a, const struct rlfa *b)
{
  return prefix_cmp (&a->prefix, &b->prefix);
}

static struct rlfa *
rlfa_add (struct isis_spftree *spftree, struct isis_vertex *vertex,
...
static void
rlfa_delete (struct isis_spftree *spftree, struct rlfa *rlfa)
{
...
static struct rlfa *
rlfa_lookup (struct isis_spftree *spftree, union prefixconstptr pu)
{
  struct rlfa s = {};
...
static void
isis_area_verify_routes_cb (struct event *thread)
{
...
static mpls_label_t
rlfa_nexthop_label (struct isis_spftree *spftree, struct isis_vertex_adj *vadj,
...
int
isis_rlfa_activate (struct isis_spftree *spftree, struct rlfa *rlfa,
                    struct zapi_rlfa_response *response)
{
  ...
  isis_route_create (&vertex->N.ip.p.dest, &vertex->N.ip.p.src, vertex->d_N,
                     vertex->depth, &vertex->N.ip.sr, vertex->Adj_N, true,
...
void
isis_rlfa_deactivate (struct isis_spftree *spftree, struct rlfa *rlfa)
{
  struct isis_area *area = spftree->area;
  struct isis_vertex *vertex = rlfa->vertex;
...
void
isis_rlfa_list_init (struct isis_spftree *spftree)
{
  rlfa_tree_init (&spftree->lfa.remote.rlfas);
}

void
isis_rlfa_list_clear (struct isis_spftree *spftree)
{
...
void
isis_rlfa_process_ldp_response (struct zapi_rlfa_response *response)
{
  struct isis *isis;
  ...
  if (response->pq_label != MPLS_INVALID_LABEL)
    {
      if (IS_DEBUG_LFA)
        zlog_debug ("ISIS-LFA: activating/updating RLFA for %pFX",
                    &rlfa->prefix);
...
void
isis_ldp_rlfa_handle_client_close (struct zapi_client_close_info *info)
{
 ...
 */
void
isis_rlfa_check (struct isis_spftree *spftree_pc, struct isis_vertex *vertex)
{
  struct isis_spftree *spftree_old = spftree_pc->lfa.old.spftree;
  struct rlfa *rlfa;
  const struct in_addr *rtr_id_pq;
 ...
 */
struct isis_spftree *
isis_rlfa_compute (struct isis_area *area, struct isis_spftree *spftree,
                   ...
                   struct lfa_protected_resource *resource)
{
  ...
  spftree_pc = isis_spftree_new (
      area, spftree->lspdb, spftree->sysid, spftree->level, spftree->tree_id,
      SPF_TYPE_RLFA, spftree->flags, spftree->algorithm);
  spftree_pc->lfa.old.spftree = spftree;
  spftree_pc->lfa.old.spftree_reverse = spftree_reverse;
...
static int
lfa_calc_dist_destination (struct isis_spftree *spftree,
                           ...
                           uint32_t *distance)
{
  ...
  switch (spftree->family)
    {
    case AF_INET:
      for (int vtype = VTYPE_IPREACH_INTERNAL; vtype <= VTYPE_IPREACH_TE;
           vtype++)
        {
          vertex = isis_find_vertex (&spftree->paths, &vertex_N->N.ip.p.dest,
                                     vtype);
          if (! vertex)
            continue;
...
static int
lfa_calc_dist_node (struct isis_spftree *spftree, const uint8_t *sysid,
                    uint32_t *distance)
{
  struct isis_vertex *vertex, *vertex_best = NULL;
 ...
 */
static bool
clfa_loop_free_check (struct isis_spftree *spftree,
                      struct isis_vertex *vertex_S_D,
                      struct isis_spf_adj *sadj_primary,
                      struct isis_spf_adj *sadj_N, uint32_t *path_metric)
{
  ...
  if (lfa_calc_dist_destination (node_N->lfa.spftree, vertex_S_D, &dist_N_D) !=
      0)
    return false;
  ...
  if (IS_DEBUG_LFA)
    zlog_debug ("ISIS-LFA: loop-free check: %u < %u + %u", dist_N_D, dist_N_S,
                dist_S_D);
 ...
 */
static bool
clfa_downstream_check (struct isis_spftree *spftree,
                       struct isis_vertex *vertex_S_D,
 ...
 */
static bool
clfa_node_protecting_check (struct isis_spftree *spftree,
                            struct isis_vertex *vertex_S_D,
                            struct isis_spf_adj *sadj_N,
                            struct isis_spf_adj *sadj_E)
{
  ...
  if (lfa_calc_dist_destination (node_E->lfa.spftree, vertex_S_D, &dist_E_D) !=
      0)
    return false;

  if (IS_DEBUG_LFA)
    zlog_debug ("ISIS-LFA: node protecting check: %u < %u + %u", dist_N_D,
                dist_N_E, dist_E_D);
...
void
isis_lfa_compute (struct isis_area *area, struct isis_circuit *circuit,
                  struct isis_spftree *spftree,
                  struct lfa_protected_resource *resource)
{
  ...
  for (ALL_QUEUE_ELEMENTS_RO (&spftree->paths, vnode, vertex))
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (spftree->sadj_list, snode, sadj_N))
        {
          ...
          if (circuit &&
              isis_lfa_excluded_iface_check (
                  circuit, level, sadj_N->adj->circuit->interface->name))
            continue;
      ...
      /* Check tie-breakers. */
      filtered_lfa_list = isis_lfa_tiebreakers (
          area, spftree, resource, vertex, sadj_primary, lfa_list);
...
static void
isis_spf_run_tilfa (struct isis_area *area, struct isis_circuit *circuit,
                    struct isis_spftree *spftree,
                    struct isis_spftree *spftree_reverse,
                    struct lfa_protected_resource *resource)
{
 ...
 */
void
isis_spf_run_lfa (struct isis_area *area, struct isis_spftree *spftree)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (area->circuit_list, node, circuit))
    {
      ...
      if (circuit->lfa_protection[level - 1])
      ...
      else if (circuit->tilfa_protection[level - 1])
        {
          /* Run TI-LFA. */
          assert (spftree_reverse);
          isis_spf_run_tilfa (area, circuit, spftree, spftree_reverse,
                              &resource);
        }
    }
