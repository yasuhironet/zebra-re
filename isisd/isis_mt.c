...
bool
isis_area_ipv6_dstsrc_enabled (struct isis_area *area)
{
...
uint16_t
isis_area_ipv6_topology (struct isis_area *area)
{
  struct isis_area_mt_setting *area_mt_setting;
...
void
area_add_mt_setting (struct isis_area *area,
                     struct isis_area_mt_setting *setting)
{
...
bool
area_is_mt (struct isis_area *area)
{
...
/* ADJ specific MT API */
static void
adj_mt_set (struct isis_adjacency *adj, unsigned int index, uint16_t mtid)
{
                    ...
                    struct isis_adjacency *adj)
{
  ...
  if (! changed && old_mt_count &&
      memcmp (adj->mt_set, old_mt_set, old_mt_count * sizeof (*old_mt_set)))
    changed = true;
...
static void
mt_set_add (uint16_t **mt_set, unsigned int *size, unsigned int *index,
            uint16_t mtid)
{
  ...
  if (*index >= *size)
    {
      *mt_set =
          XREALLOC (MTYPE_TMP, *mt_set, sizeof (**mt_set) * ((*index) + 1));
      *size = (*index) + 1;
    }
...
tlvs_add_mt_set (struct isis_area *area, struct isis_tlvs *tlvs,
                 unsigned int mt_count, uint16_t *mt_set, uint8_t *id,
                 uint32_t metric, struct isis_ext_subtlvs *ext)
{
  /* Check if MT is enable for this area */
  if (! area_is_mt (area))
    {
      lsp_debug ("ISIS (%s): Adding %pPN as te-style neighbor (MT disable)",
                 area->area_tag, id);
      isis_tlvs_add_extended_reach (tlvs, ISIS_MT_DISABLE, id, metric, ext);
      return;
    }
  ...
  for (unsigned int i = 0; i < mt_count; i++)
    {
      ...
      if (mt_set[i] == ISIS_MT_IPV4_UNICAST)
        ...
        {
          lsp_debug ("ISIS (%s): Adding %pPN as mt-style neighbor for %s",
                 ...
                 uint8_t *id, uint32_t metric)
{
  ...
  tlvs_add_mt_set (circuit->area, tlvs, adj->mt_count, adj->mt_set, id, metric,
                   circuit->ext);
}
...
{
#ifdef FABRICD
  hook_register (isis_circuit_config_write, circuit_write_mt_settings);
#endif
}
