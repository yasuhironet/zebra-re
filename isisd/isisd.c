...
void
isis_vrf_link (struct isis *isis, struct vrf *vrf)
{
...
void
isis_vrf_unlink (struct isis *isis, struct vrf *vrf)
{
  if (vrf->info == (void *) isis)
    vrf->info = NULL;
...
void
isis_master_init (struct event_loop *master)
{
...
isis_new (const char *vrf_name)
{
  ...
  if (IS_DEBUG_EVENTS)
    zlog_debug ("%s: Create new isis instance with vrf_name %s vrf_id %u",
...
void
isis_area_add_circuit (struct isis_area *area, struct isis_circuit *circuit)
{
...
void
isis_area_del_circuit (struct isis_area *area, struct isis_circuit *circuit)
{
  area->ip_circuits -= circuit->ip_router;
  area->ipv6_circuits -= circuit->ipv6_router;
...
struct isis_area *
isis_area_create (const char *area_tag, const char *vrf_name)
{
  struct isis_area *area;
  ...
  if (fabricd)
    ...
    area->is_type = yang_get_default_enum ("/frr-isisd:isis/instance/is-type");
  ...
  /* Flex-Algo */
  area->flex_algos =
      flex_algos_alloc (isis_flex_algo_data_alloc, isis_flex_algo_data_free);
#endif /* ifndef FABRICD */
  ...
  area->min_spf_interval[1] = yang_get_default_uint16 (
      "/frr-isisd:isis/instance/spf/minimum-interval/level-1");
  area->dynhostname =
      yang_get_default_bool ("/frr-isisd:isis/instance/dynamic-hostname");
  default_style =
  ...
  area->lsp_mtu = yang_get_default_uint16 ("/frr-isisd:isis/instance/lsp/mtu");
  ...
  area->attached_bit_rcv_ignore =
      yang_get_default_bool ("/frr-isisd:isis/instance/attach-receive-ignore");
...
struct isis_area *
isis_area_lookup_by_vrf (const char *area_tag, const char *vrf_name)
{
  struct isis_area *area;
  struct listnode *node;
  struct isis *isis = NULL;
...
struct isis_area *
isis_area_lookup (const char *area_tag, vrf_id_t vrf_id)
{
  struct isis_area *area;
  struct listnode *node;
  struct isis *isis;
...
void
isis_area_destroy (struct isis_area *area)
{
  struct listnode *node, *nnode;
  struct isis_circuit *circuit;
  ...
  for (int i = SPF_PREFIX_PRIO_CRITICAL; i <= SPF_PREFIX_PRIO_MEDIUM; i++)
    {
...
static int
isis_vrf_delete (struct vrf *vrf)
{
  if (IS_DEBUG_EVENTS)
    zlog_debug ("%s: VRF Deletion: %s(%u)", __func__, vrf->name, vrf->vrf_id);

  return 0;
}

static void
isis_set_redist_vrf_bitmaps (struct isis *isis, bool set)
{
  struct listnode *node;
  struct isis_area *area;
  int type;
    ...
    for (protocol = 0; protocol < REDIST_PROTOCOL_COUNT; protocol++)
      for (type = 0; type < ZEBRA_ROUTE_MAX + 1; type++)
        for (level = 0; level < ISIS_LEVELS; level++)
          if (area->redist_settings[protocol][type][level].redist == 1)
        ...
        if (type == DEFAULT_ROUTE)
          {
            if (set)
              ...
              vrf_bitmap_unset (zclient->default_information[afi],
                                isis->vrf_id);
          }
        else
          {
            if (set)
              vrf_bitmap_set (zclient->redist[afi][type], isis->vrf_id);
            else
              vrf_bitmap_unset (zclient->redist[afi][type], isis->vrf_id);
          }
      }
}

static int
isis_vrf_enable (struct vrf *vrf)
{
  struct isis *isis;
  vrf_id_t old_vrf_id;

  if (IS_DEBUG_EVENTS)
    zlog_debug ("%s: VRF %s id %u enabled", __func__, vrf->name, vrf->vrf_id);
  ...
  if (isis && isis->vrf_id != vrf->vrf_id)
    {
      ...
      isis_vrf_link (isis, vrf);
      if (IS_DEBUG_EVENTS)
        zlog_debug ("%s: isis linked to vrf %s vrf_id %u (old id %u)",
...
static int
isis_vrf_disable (struct vrf *vrf)
{
  struct isis *isis;
  ...
  if (isis)
    {
      ...
      isis_vrf_unlink (isis, vrf);
      if (IS_DEBUG_EVENTS)
        zlog_debug ("%s: isis old_vrf_id %d unlinked", __func__, old_vrf_id);
    }
...
isis_vrf_init (void)
{
  vrf_init (isis_vrf_new, isis_vrf_enable, isis_vrf_disable, isis_vrf_delete);
...
void
isis_filter_update (struct access_list *access)
{
  struct isis *isis;
  struct isis_area *area;
      ...
      for (ALL_LIST_ELEMENTS_RO (isis->area_list, anode, area))
        {
          for (int i = SPF_PREFIX_PRIO_CRITICAL; i <= SPF_PREFIX_PRIO_MEDIUM;
               i++)
            {
              ...
              ppa = &area->spf_prefix_priorities[i];
              ppa->list_v4 = access_list_lookup (AFI_IP, ppa->name);
              ppa->list_v6 = access_list_lookup (AFI_IP6, ppa->name);
            }
...
void
isis_prefix_list_update (struct prefix_list *plist)
{
  struct isis *isis;
  struct isis_area *area;
  struct listnode *node, *anode;
      ...
      for (ALL_LIST_ELEMENTS_RO (isis->area_list, anode, area))
        {
          for (int level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++)
            {
              const char *plist_name = prefix_list_name (plist);

              if (! area->rlfa_plist_name[level - 1])
                continue;

              if (! strmatch (area->rlfa_plist_name[level - 1], plist_name))
                continue;
  ...
  if (area->isis->sysid_set == 0)
    {
      ...
      if (IS_DEBUG_EVENTS)
        zlog_debug ("Router has SystemID %pSY", area->isis->sysid);
    }
  else
    {
      /*
       ...
       */
      if (memcmp (area->isis->sysid, GETSYSID (addr), ISIS_SYS_ID_LEN))
        {
          vty_out (
              vty,
 ...
 */
int
show_isis_interface_common (struct vty *vty, struct json_object *json,
                            const char *ifname, char detail,
                            const char *vrf_name, bool all_vrf)
{
  if (json)
    {
      return show_isis_interface_common_json (json, ifname, detail, vrf_name,
                                              all_vrf);
    }
  else
    {
      return show_isis_interface_common_vty (vty, ifname, detail, vrf_name,
                                             all_vrf);
    }
}

int
show_isis_interface_common_json (struct json_object *json, const char *ifname,
  ...
  if (! im)
    {
      ...
      json_object_string_add (json, "is-is-routing-process-enabled", "no");
      return CMD_SUCCESS;
    }
  if (vrf_name)
    {
          ...
          for (ALL_LIST_ELEMENTS_RO (isis->area_list, anode, area))
            {
              ...
              json_object_string_add (
                  area_json, "area", area->area_tag ? area->area_tag : "null");
...
int
show_isis_interface_common_vty (struct vty *vty, const char *ifname,
                                char detail, const char *vrf_name,
                                bool all_vrf)
{
  struct listnode *anode, *cnode, *inode;
       ...
       "json output\n")
{
  ...
  char *ifname =
      argv_find (argv, argc, "WORD", &idx_word) ? argv[idx_word]->arg : NULL;
  res = show_isis_interface_common (vty, json, ifname, ISIS_UI_LEVEL_DETAIL,
                                    vrf_name, all_vrf);
  if (uj)
    vty_json (vty, json);
  return res;
...
static int
id_to_sysid (struct isis *isis, const char *id, uint8_t *sysid)
{
...
static void
isis_neighbor_common_json (struct json_object *json, const char *id,
                           char detail, struct isis *isis, uint8_t *sysid)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (isis->area_list, anode, area))
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (area->circuit_list, cnode, circuit))
        {
          circuit_json = json_object_new_object ();
          json_object_int_add (circuit_json, "circuit", circuit->circuit_id);
                  ...
                  if (adjdb && adjdb->count)
                    {
                      for (ALL_LIST_ELEMENTS_RO (adjdb, node, adj))
                        if (! id ||
                          ...
                          isis_adj_print_json (adj, circuit_json, detail);
                    }
                }
            }
...
static void
isis_neighbor_common_vty (struct vty *vty, const char *id, char detail,
...
static void
isis_neighbor_common (struct vty *vty, struct json_object *json,
...
int
show_isis_neighbor_common (struct vty *vty, struct json_object *json,
...
static void
isis_neighbor_common_clear (struct vty *vty, const char *id, uint8_t *sysid,
                            struct isis *isis)
{
...
print_debug (struct vty *vty, int flags, int onoff)
{
  ...
  if (flags & DEBUG_TE)
    vty_out (vty, "IS-IS Traffic Engineering events debugging is %s\n",
             onoffs);
  if (flags & DEBUG_LFA)
...
static void
isis_spf_ietf_common (struct vty *vty, struct isis *isis)
{
  struct listnode *node;
  struct isis_area *area;
  for (ALL_LIST_ELEMENTS_RO (isis->area_list, node, area))
          ...
          if (area->spf_timer[level - 1])
            {
              struct timeval remain =
                  event_timer_remain (area->spf_timer[level - 1]);
...
static const char *
pdu_counter_index_to_name_json (enum pdu_counter_index index)
{
  switch (index)
    {
...
static void
common_isis_summary_json (struct json_object *json, struct isis *isis)
{
  int level;
  ...
  if (isis->area_list)
    json_object_int_add (json, "number-areas", isis->area_list->count);
  areas_json = json_object_new_array ();
  json_object_object_add (json, "areas", areas_json);
  for (ALL_LIST_ELEMENTS_RO (isis->area_list, node, area))
    {
      ...
      if (fabricd)
        {
          ...
          json_object_string_add (area_json, "tier",
                                  tier == ISIS_TIER_UNDEFINED ? "undefined"
                                                              : stier);
        }
      ...
      if (listcount (area->area_addrs) > 0)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (area->area_addrs, node2, area_addr))
            json_object_string_addf (area_json, "net", "%pISl", area_addr);
        }
      ...
      for (int i = 0; i < PDU_COUNTER_SIZE; i++)
        {
          if (! area->pdu_tx_counters[i])
            continue;
          json_object_int_add (tx_pdu_json, pdu_counter_index_to_name_json (i),
                               area->pdu_tx_counters[i]);
        }
      json_object_int_add (tx_pdu_json, "lsp-rxmt", area->lsp_rxmt_count);
      ...
      for (int i = 0; i < PDU_COUNTER_SIZE; i++)
        {
          if (! area->pdu_rx_counters[i])
            continue;
          json_object_int_add (rx_pdu_json, pdu_counter_index_to_name_json (i),
      ...
      for (level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++)
        {
          ...
          if (area->spf_timer[level - 1])
            json_object_string_add (level_json, "spf", "pending");
          else
            json_object_string_add (level_json, "spf", "no pending");
          json_object_int_add (level_json, "minimum-interval",
                               area->min_spf_interval[level - 1]);
          if (area->spf_delay_ietf[level - 1])
            json_object_string_add (level_json, "ietf-spf-delay-activated",
                                    "not used");
          if (area->ip_circuits)
            {
              isis_spf_print_json (area->spftree[SPFTREE_IPV4][level - 1],
                                   level_json);
            }
          if (area->ipv6_circuits)
            {
              isis_spf_print_json (area->spftree[SPFTREE_IPV6][level - 1],
                                   level_json);
            }
...
static void
common_isis_summary_vty (struct vty *vty, struct isis *isis)
{
  struct listnode *node, *node2;
  struct isis_area *area;
  int level;
          ...
          if (area->ip_circuits)
            {
              ...
              isis_spf_print (area->spftree[SPFTREE_IPV4][level - 1], vty);
            }

          if (area->ipv6_circuits)
            {
              vty_out (vty, "    IPv6 route computation:\n");
              isis_spf_print (area->spftree[SPFTREE_IPV6][level - 1], vty);
            }

          if (area->ipv6_circuits && isis_area_ipv6_dstsrc_enabled (area))
            {
              vty_out (vty, "    IPv6 dst-src route computation:\n");
              isis_spf_print (area->spftree[SPFTREE_DSTSRC][level - 1], vty);
            }
        }
    }
}
...
static void
common_isis_summary (struct vty *vty, struct json_object *json,
                     struct isis *isis)
{
  if (json)
    {
...
struct isis_lsp *
lsp_for_sysid (struct lspdb_head *head, const char *sysid_str,
               struct isis *isis)
{
  char sysid[255] = { 0 };
  uint8_t number[3] = { 0 };
...
void
show_isis_database_lspdb_json (struct json_object *json,
                               struct isis_area *area, int level,
                               struct lspdb_head *lspdb, const char *sysid_str,
                               int ui_level)
{
  struct isis_lsp *lsp;
  int lsp_count;
      ...
      if (lsp)
        {
          if (ui_level == ISIS_UI_LEVEL_DETAIL)
            lsp_print_detail (lsp, NULL, json, area->dynhostname, area->isis);
          else
            lsp_print_json (lsp, json, area->dynhostname, area->isis);
        }
      else if (sysid_str == NULL)
        {
          lsp_count = lsp_print_all (NULL, json, lspdb, ui_level,
...
void
show_isis_database_lspdb_vty (struct vty *vty, struct isis_area *area,
  ...
  if (lspdb_count (lspdb) > 0)
    {
      ...
      if (lsp)
        {
          if (ui_level == ISIS_UI_LEVEL_DETAIL)
            lsp_print_detail (lsp, vty, NULL, area->dynhostname, area->isis);
          else
            lsp_print_vty (lsp, vty, area->dynhostname, area->isis);
        }
      else if (sysid_str == NULL)
        {
          lsp_count = lsp_print_all (vty, NULL, lspdb, ui_level,
                                     area->dynhostname, area->isis);
...
static void
show_isis_database_json (struct json_object *json, const char *sysid_str,
                         int ui_level, struct isis *isis)
{
  struct listnode *node;
  struct isis_area *area;
  int level;
  struct json_object *tag_area_json, *area_json, *lsp_json, *area_arr_json,
      *arr_json;
      ...
      for (level = 0; level < ISIS_LEVELS; level++)
        {
          ...
          show_isis_database_lspdb_json (
              lsp_json, area, level, &area->lspdb[level], sysid_str, ui_level);
...
static void
show_isis_database_vty (struct vty *vty, const char *sysid_str, int ui_level,
                        struct isis *isis)
{
  struct listnode *node;
  struct isis_area *area;
  ...
  for (ALL_LIST_ELEMENTS_RO (isis->area_list, node, area))
    {
      ...
      for (level = 0; level < ISIS_LEVELS; level++)
        show_isis_database_lspdb_vty (vty, area, level, &area->lspdb[level],
                                      sysid_str, ui_level);
    }
}

static void
show_isis_database_common (struct vty *vty, struct json_object *json,
                           const char *sysid_str, int ui_level,
                           struct isis *isis)
{
  if (json)
 ...
 */
static int
show_isis_database (struct vty *vty, struct json_object *json,
                    const char *sysid_str, int ui_level, const char *vrf_name,
                    bool all_vrf)
{
  struct listnode *node;
  struct isis *isis;
       ...
       "json output\n")
{
  ...
  const char *vrf_name = VRF_DEFAULT_NAME;
  bool all_vrf = false;
  int uilevel = argv_find (argv, argc, "detail", &idx) ? ISIS_UI_LEVEL_DETAIL
                                                       : ISIS_UI_LEVEL_BRIEF;
       ...
       "ISO Routing area tag\n")
{
  ...
  if (area == NULL)
    {
      zlog_warn ("%s: could not find area with area-tag %s", __func__,
                 area_tag);
      return CMD_ERR_NO_MATCH;
    }
...
void
isis_area_lsp_mtu_set (struct isis_area *area, unsigned int lsp_mtu)
{
...
static int
isis_area_passwd_set (struct isis_area *area, int level, uint8_t passwd_type,
  ...
  if (passwd_type != ISIS_PASSWD_TYPE_UNUSED)
    {
      ...
      modified.len = len;
      strlcpy ((char *) modified.passwd, passwd, sizeof (modified.passwd));
...
isis_area_passwd_cleartext_set (struct isis_area *area, int level,
                                const char *passwd, uint8_t snp_auth)
{
  return isis_area_passwd_set (area, level, ISIS_PASSWD_TYPE_CLEARTXT, passwd,
                               snp_auth);
}
...
isis_area_passwd_hmac_md5_set (struct isis_area *area, int level,
                               const char *passwd, uint8_t snp_auth)
{
  return isis_area_passwd_set (area, level, ISIS_PASSWD_TYPE_HMAC_MD5, passwd,
                               snp_auth);
}

void
isis_area_invalidate_routes (struct isis_area *area, int levels)
{
#ifndef FABRICD
  struct flex_algo *fa;
  struct listnode *node;
  ...
  for (int level = ISIS_LEVEL1; level <= ISIS_LEVEL2; level++)
    {
      ...
      for (int tree = SPFTREE_IPV4; tree < SPFTREE_COUNT; tree++)
        {
          isis_spf_invalidate_routes (area->spftree[tree][level - 1]);

#ifndef FABRICD
          for (ALL_LIST_ELEMENTS_RO (area->flex_algos->flex_algos, node, fa))
            {
              data = fa->data;
              isis_spf_invalidate_routes (data->spftree[tree][level - 1]);
            }
#endif /* ifndef FABRICD */
        }
    }
...
void
isis_area_verify_routes (struct isis_area *area)
{
  for (int tree = SPFTREE_IPV4; tree < SPFTREE_COUNT; tree++)
...
void
isis_area_switchover_routes (struct isis_area *area, int family,
                             ...
                             int level)
{
  ...
  isis_spf_switchover_routes (area, area->spftree[tree], family, nexthop_ip,
                              ifindex, level);
}
...
void
isis_area_metricstyle_set (struct isis_area *area, bool old_metric,
                           bool new_metric)
{
...
void
isis_area_overload_bit_set (struct isis_area *area, bool overload_bit)
{
...
void
isis_area_overload_on_startup_set (struct isis_area *area,
...
void
config_end_lsp_generate (struct isis_area *area)
{
  if (listcount (area->area_addrs) > 0)
    {
...
void
isis_area_advertise_high_metrics_set (struct isis_area *area,
      ...
      for (ALL_LIST_ELEMENTS_RO (area->circuit_list, node, circuit))
        {
          ...
          configured_metric_l1 =
              yang_dnode_get_uint32 (dnode, "./frr-isisd:isis/metric/level-1");
          configured_metric_l2 =
              yang_dnode_get_uint32 (dnode, "./frr-isisd:isis/metric/level-2");
 ...
 */
void
isis_restart_write_overload_time (struct isis_area *isis_area,
 ...
 */
uint32_t
isis_restart_read_overload_time (struct isis_area *isis_area)
{
  char *filepath;
  const char *area_name;
  json_object *json;
  ...
  json_object_object_get_ex (json_area, "overload_time", &json_overload_time);
...
void
isis_area_attached_bit_send_set (struct isis_area *area, bool attached_bit)
{
...
void
isis_area_attached_bit_receive_set (struct isis_area *area, bool attached_bit)
{
...
void
isis_area_dynhostname_set (struct isis_area *area, bool dynhostname)
{
