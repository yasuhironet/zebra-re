...
int
lsp_id_cmp (uint8_t *id1, uint8_t *id2)
{
  return memcmp (id1, id2, ISIS_SYS_ID_LEN + 2);
}

int
lspdb_compare (const struct isis_lsp *a, const struct isis_lsp *b)
{
  return memcmp (a->hdr.lsp_id, b->hdr.lsp_id, sizeof (a->hdr.lsp_id));
}

void
lsp_db_init (struct lspdb_head *head)
{
  lspdb_init (head);
}

void
lsp_db_fini (struct lspdb_head *head)
{
  struct isis_lsp *lsp;
...
struct isis_lsp *
lsp_search (struct lspdb_head *head, const uint8_t *id)
{
 ...
 */
static void
lsp_remove_frags (struct lspdb_head *head, struct list *frags)
{
...
void
lsp_search_and_destroy (struct lspdb_head *head, const uint8_t *id)
{
  struct isis_lsp *lsp;
             ...
             uint16_t checksum, uint16_t rem_lifetime)
{
      ...
      if (IS_DEBUG_SNP_PACKETS)
        {
          zlog_debug (
              ...
              areatag, lsp->hdr.lsp_id, lsp->hdr.seqno, lsp->hdr.checksum,
  ...
  if (seqno > lsp->hdr.seqno ||
      (seqno == lsp->hdr.seqno &&
       ((lsp->hdr.rem_lifetime != 0 && rem_lifetime == 0) ||
        (lsp->hdr.checksum != checksum && lsp->hdr.rem_lifetime))))
    {
      if (IS_DEBUG_SNP_PACKETS)
        {
          zlog_debug (
              "ISIS-Snp (%s): Compare LSP %pLS seq 0x%08x, cksum 0x%04hx, lifetime %hus",
              areatag, lsp->hdr.lsp_id, seqno, checksum, rem_lifetime);
          zlog_debug (
...
static void
put_lsp_hdr (struct isis_lsp *lsp, size_t *len_pointer, bool keep)
{
  uint8_t pdu_type =
...
static void
lsp_add_auth (struct isis_lsp *lsp)
{
...
static void
lsp_pack_pdu (struct isis_lsp *lsp)
{
  if (! lsp->tlvs)
  ...
  lsp->hdr.checksum = ntohs (fletcher_checksum (
      STREAM_DATA (lsp->pdu) + 12, stream_get_endp (lsp->pdu) - 12, 12));
}
...
static void
lsp_purge_add_poi (struct isis_lsp *lsp, const uint8_t *sender)
{
  if (lsp->area == NULL)
    return;
  ...
  if (! lsp->tlvs)
    lsp->tlvs = isis_alloc_tlvs ();
  isis_tlvs_set_purge_originator (lsp->tlvs, lsp->area->isis->sysid, sender);
  isis_tlvs_set_dynamic_hostname (lsp->tlvs, cmd_hostname_get ());
}

static void
lsp_purge (struct isis_lsp *lsp, int level, const uint8_t *sender)
{
  /* reset stream */
  lsp_clear_data (lsp);
...
bool
isis_level2_adj_up (struct isis_area *area)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (area->circuit_list, cnode, circuit))
    {
      if (circuit->circ_type == CIRCUIT_T_BROADCAST)
      ...
      else if (circuit->circ_type == CIRCUIT_T_P2P && circuit->u.p2p.neighbor)
        {
          ...
          if (adj->level != ISIS_ADJ_LEVEL1 && adj->adj_state == ISIS_ADJ_UP)
            return true;
        }
    }
 ...
 */
void
set_overload_on_start_timer (struct event *thread)
{
...
static void
isis_reset_attach_bit (struct isis_adjacency *adj)
{
  ...
  if (adj->adj_state == ISIS_ADJ_UP && ! (lsp->hdr.lsp_bits & LSPBIT_ATT))
    {
      ...
      lsp_regenerate_schedule (area, IS_LEVEL_1, 0);
           ...
           (lsp->hdr.lsp_bits & LSPBIT_ATT) && ! isis_level2_adj_up (area))
    {
                 ...
                 struct isis_area *area, int level)
{
  ...
  if (area->dynhostname && lsp->tlvs->hostname && lsp->hdr.rem_lifetime)
    {
      isis_dynhn_insert (area->isis, lsp->hdr.lsp_id, lsp->tlvs->hostname,
...
static void
lsp_link_fragment (struct isis_lsp *lsp, struct isis_lsp *lsp0)
{
  if (! LSP_FRAGMENT (lsp->hdr.lsp_id))
    {
...
static void
lsp_adjust_stream (struct isis_lsp *lsp)
{
  if (lsp->pdu)
    {
...
struct isis_lsp *
lsp_new (struct isis_area *area, uint8_t *lsp_id, uint16_t rem_lifetime,
         uint32_t seqno, uint8_t lsp_bits, uint16_t checksum,
...
void
lsp_insert (struct lspdb_head *head, struct isis_lsp *lsp)
{
  lspdb_add (head, lsp);
  if (lsp->hdr.seqno)
    {
...
/*
 * Build a list of LSPs with non-zero ht and seqno bounded by start and stop
 * ids
 */
void
lsp_build_list_nonzero_ht (struct lspdb_head *head, const uint8_t *start_id,
                           const uint8_t *stop_id, struct list *list)
{
  ...
  frr_each_from (lspdb, head, lsp, start)
    {
      if (memcmp (lsp->hdr.lsp_id, stop_id, ISIS_SYS_ID_LEN + 2) > 0)
        break;
...
void
lspid_print (uint8_t *lsp_id, char *dest, size_t dest_len, char dynhost,
             char frag, struct isis *isis)
{
  ...
  if (frag)
    snprintf (dest, dest_len, "%s.%02x-%02x", id, LSP_PSEUDO_ID (lsp_id),
              LSP_FRAGMENT (lsp_id));
  else
...
static const char *
lsp_bits2string (uint8_t lsp_bits, char *buf, size_t buf_size)
{
  char *pos = buf;
...
void
lsp_print_common (struct isis_lsp *lsp, struct vty *vty,
                  struct json_object *json, char dynhost, struct isis *isis)
{
  if (json)
    {
...
void
lsp_print_json (struct isis_lsp *lsp, struct json_object *json, char dynhost,
                struct isis *isis)
{
  char LSPid[255];
  char age_out[8];
  ...
  json_object_string_add (json, "att-p-ol",
                          lsp_bits2string (lsp->hdr.lsp_bits, b, sizeof (b)));
}

void
lsp_print_vty (struct isis_lsp *lsp, struct vty *vty, char dynhost,
               struct isis *isis)
{
  char LSPid[255];
  char age_out[8];
...
void
lsp_print_detail (struct isis_lsp *lsp, struct vty *vty,
                  struct json_object *json, char dynhost, struct isis *isis)
{
  if (json)
    {
  ...
  else
    {
      ...
      if (lsp->tlvs)
        vty_multiline (vty, "  ", "%s", isis_format_tlvs (lsp->tlvs, NULL));
      vty_out (vty, "\n");
    }
}
...
int
lsp_print_all (struct vty *vty, struct json_object *json,
...
static uint16_t
lsp_refresh_time (struct isis_lsp *lsp, uint16_t rem_lifetime)
{
...
static void
lsp_build_internal_reach_ipv4 (struct isis_lsp *lsp, struct isis_area *area,
                               struct prefix_ipv4 *ipv4, uint32_t metric)
{
  struct sr_prefix_cfg *pcfgs[SR_ALGORITHM_COUNT] = { NULL };

  if (area->oldmetric)
    {
      lsp_debug ("ISIS (%s): Adding old-style IP reachability for %pFX",
                 area->area_tag, ipv4);
        ...
        for (int i = 0; i < SR_ALGORITHM_COUNT; i++)
          {
            ...
            if (flex_algo_id_valid (i) &&
              ...
              continue;
#endif /* ifndef FABRICD */
            pcfgs[i] = isis_sr_cfg_prefix_find (area, ipv4, i);
          }

      isis_tlvs_add_extended_ip_reach (lsp->tlvs, ipv4, metric, false, pcfgs);
    }
}

static void
lsp_build_internal_reach_ipv6 (struct isis_lsp *lsp, struct isis_area *area,
                               struct prefix_ipv6 *ipv6, uint32_t metric)
{
  struct sr_prefix_cfg *pcfgs[SR_ALGORITHM_COUNT] = { NULL };

  lsp_debug ("ISIS (%s): Adding IPv6 reachability for %pFX", area->area_tag,
             ipv6);
      ...
      if (area->newmetric)
        {
          ...
          isis_tlvs_add_extended_ip_reach (lsp->tlvs, ipv4, metric, true,
                                           pcfgs);
        }
    }
}
  ...
  for (struct route_node *rn = route_top (er_table); rn;
       rn = srcdest_route_next (rn))
    {
      ...
      if (! src_p || ! src_p->prefixlen)
            ...
            for (int i = 0; i < SR_ALGORITHM_COUNT; i++)
              {
                ...
                if (flex_algo_id_valid (i) &&
                  ...
                  continue;
#endif /* ifndef FABRICD */
                pcfgs[i] = isis_sr_cfg_prefix_find (area, p, i);
              }
        ...
        {
          isis_tlvs_add_ipv6_dstsrc_reach (lsp->tlvs, ISIS_MT_IPV6_DSTSRC, p,
                                           src_p, metric);
        }
    }
}
...
lsp_next_frag (uint8_t frag_num, struct isis_lsp *lsp0, struct isis_area *area,
               int level)
{
  struct isis_lsp *lsp;
...
lsp_build (struct isis_lsp *lsp, struct isis_area *area)
{
  ...
  if (area->ip_circuits > 0 || area->ipv6_circuits > 0)
    {
      ...
      if (area->ip_circuits > 0)
        {
          lsp_debug ("ISIS (%s): Found IPv4 circuit, adding IPv4 to NLPIDs",
                     area->area_tag);
      ...
      if (area->ipv6_circuits > 0)
        {
          lsp_debug ("ISIS (%s): Found IPv6 circuit, adding IPv6 to NLPIDs",
                     area->area_tag);
      ...
      for (ALL_LIST_ELEMENTS_RO (area->flex_algos->flex_algos, node, fa))
        {
          if (! fa->advertise_definition)
            continue;
          lsp_debug ("ISIS (%s):   Flex-Algo Definition %u", area->area_tag,
                     fa->algorithm);
          isis_tlvs_set_router_capability_fad (lsp->tlvs, fa, fa->algorithm,
                                               area->isis->sysid);
        }
#endif /* ifndef FABRICD */
      ...
      if (area->srdb.enabled)
        {
          ...
          rcap->srgb.flags = ISIS_SUBTLV_SRGB_FLAG_I | ISIS_SUBTLV_SRGB_FLAG_V;
          ...
          for (ALL_LIST_ELEMENTS_RO (area->flex_algos->flex_algos, node, fa))
            {
              ...
              fa->state = true;
              lsp_debug ("ISIS (%s):   SR Algorithm %u", area->area_tag,
                         fa->algorithm);
      ...
      if (area->newmetric)
        {
          ...
          lsp_debug ("ISIS (%s): Adding router ID also as TE router ID tlv.",
                     area->area_tag);
  ...
  if (IS_MPLS_TE (area->mta) &&
    ...
    {
      lsp_debug ("ISIS (%s): Adding IPv6 TE Router ID tlv.", area->area_tag);
      isis_tlvs_set_te_router_id_ipv6 (lsp->tlvs, &area->mta->router_id_ipv6);
    }
  ...
  if (fabricd)
    {
      lsp_debug ("ISIS (%s): Adding tier %hhu spine-leaf-extension tlv.",
                 area->area_tag, fabricd_tier (area));
      isis_tlvs_add_spine_leaf (lsp->tlvs, fabricd_tier (area), true, false,
                                false, false);
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (area->circuit_list, node, circuit))
    {
      if (! circuit->interface)
        ...
        lsp_debug ("ISIS (%s): Processing %s circuit %s", area->area_tag,
      ...
      if (area->advertise_passive_only && ! circuit->is_passive)
        {
          lsp_debug ("ISIS (%s): Circuit is not passive, ignoring.",
                     area->area_tag);
          continue;
        }
      ...
      switch (circuit->circ_type)
        ...
        case CIRCUIT_T_BROADCAST:
          if (level & circuit->is_type)
            {
              uint8_t *ne_id = (level == IS_LEVEL_1)
              ...
              if (LSP_PSEUDO_ID (ne_id))
                {
                  ...
                  if (area->newmetric)
                    tlvs_add_mt_bcast (lsp->tlvs, circuit, level, ne_id,
                                       metric);
                }
            }
          else
        ...
        case CIRCUIT_T_P2P:
          {
            ...
            if (nei && nei->adj_state == ISIS_ADJ_UP &&
                (level & nei->circuit_t))
              {
                ...
                if (area->oldmetric)
                  {
                    lsp_debug ("ISIS (%s): Adding old-style is reach for %pSY",
                               area->area_tag, ne_id);
                    isis_tlvs_add_oldstyle_reach (lsp->tlvs, ne_id, metric);
                  }
                if (area->newmetric)
                  {
                    ...
                    tlvs_add_mt_p2p (lsp->tlvs, circuit, ne_id,
                                     neighbor_metric);
                  }
              }
            else
  ...
  if (! fragments)
    {
      ...
      log_multiline (LOG_WARNING, "    ", "%s", isis_format_tlvs (tlvs, NULL));
      isis_free_tlvs (tlvs);
      return;
    }
  isis_free_tlvs (tlvs);
      ...
      if (node != listhead (fragments))
        {
          ...
          frag = lsp_next_frag (LSP_FRAGMENT (frag->hdr.lsp_id) + 1, lsp, area,
                                level);
      ...
      if (overload_time > 0)
        {
          ...
          event_add_timer (master, set_overload_on_start_timer, area,
                           overload_time, &area->t_overload_on_startup_timer);
        }
  ...
  if (oldlsp)
    {
      ...
      lsp_search_and_destroy (&area->lspdb[level - 1], oldlsp->hdr.lsp_id);
    }
  ...
  if (IS_DEBUG_UPDATE_PACKETS)
    {
      zlog_debug (
          ...
          area->area_tag, level, newlsp->hdr.lsp_id, newlsp->hdr.pdu_len,
          newlsp->hdr.seqno, newlsp->hdr.checksum, newlsp->hdr.rem_lifetime,
          refresh_time);
    }
  sched_debug (
  ...
  for (ALL_LIST_ELEMENTS_RO (lsp->lspu.frags, node, frag))
    {
      ...
      frag->hdr.lsp_bits = lsp_bits_generate (level, area->overload_bit,
                                              area->attached_bit_send, area);
  ...
  if (IS_DEBUG_UPDATE_PACKETS)
    {
      zlog_debug (
          ...
          area->area_tag, level, lsp->hdr.lsp_id, lsp->hdr.pdu_len,
 ...
 */
static void
lsp_refresh (struct event *thread)
{
  ...
  if (monotime_since (&area->last_lsp_refresh_event[level - 1], NULL) <
      ...
      ! (area->bfd_force_spf_refresh))
    {
      sched_debug ("ISIS (%s): Still unstable, postpone LSP L%d refresh",
                   area->area_tag, level);
      _lsp_regenerate_schedule (area, level, 0, false, __func__, __FILE__,
                                __LINE__);
      return;
    }

  sched_debug ("ISIS (%s): LSP L%d refresh timer expired. Refreshing LSP...",
               area->area_tag, level);
  lsp_regenerate (area, level);
}

int
_lsp_regenerate_schedule (struct isis_area *area, int level, int all_pseudo,
                          bool postpone, const char *func, const char *file,
                          int line)
{
  struct isis_lsp *lsp;
  ...
  sched_debug (
      ...
      area->area_tag, circuit_t2string (level), all_pseudo ? "" : "not ", func,
      file, line);
      ...
      if (area->lsp_regenerate_pending[lvl - 1] &&
          ! (area->bfd_signalled_down))
        {
          ...
          struct timeval remain =
              event_timer_remain (area->t_lsp_refresh[lvl - 1]);
          sched_debug (
          ...
          if (area->bfd_signalled_down)
          ...
          else
            {
              int64_t time_since_last = monotime_since (
                  &area->last_lsp_refresh_event[lvl - 1], NULL);
                  ...
                  int level)
{
  ...
  lsp_debug (
      ...
      area->area_tag, lsp->hdr.lsp_id, circuit->interface->name, level);

  lsp->level = level;
  /* RFC3787  section 4 SHOULD not set overload bit in pseudo LSPs */
  lsp->hdr.lsp_bits =
      lsp_bits_generate (level, 0, circuit->area->attached_bit_send, area);
  ...
  for (ALL_LIST_ELEMENTS_RO (adj_list, node, adj))
    {
      ...
      if (! (level == IS_LEVEL_1 && adj->sys_type == ISIS_SYSTYPE_L1_IS) &&
        ...
        {
          lsp_debug ("ISIS (%s): Ignoring neighbor %pSY, level does not match",
                     area->area_tag, adj->sysid);
          continue;
        }
      ...
      if (circuit->area->oldmetric)
        {
          isis_tlvs_add_oldstyle_reach (lsp->tlvs, ne_id, 0);
          lsp_debug ("ISIS (%s): Adding %pPN as old-style neighbor (peer)",
                     area->area_tag, ne_id);
        }
      if (circuit->area->newmetric)
        {
          isis_tlvs_add_extended_reach (lsp->tlvs, ISIS_MT_IPV4_UNICAST, ne_id,
                                        0, NULL);
          lsp_debug ("ISIS (%s): Adding %pPN as te-style neighbor (peer)",
                     area->area_tag, ne_id);
        }
    }
...
lsp_generate_pseudo (struct isis_circuit *circuit, int level)
{
  ...
  if ((circuit->is_type & level) != level || (circuit->state != C_STATE_UP) ||
  ...
  if (level == IS_LEVEL_1)
    event_add_timer (master, lsp_l1_refresh_pseudo, circuit, refresh_time,
                     &circuit->u.bc.t_refresh_pseudo_lsp[level - 1]);
  else if (level == IS_LEVEL_2)
    event_add_timer (master, lsp_l2_refresh_pseudo, circuit, refresh_time,
                     &circuit->u.bc.t_refresh_pseudo_lsp[level - 1]);
  ...
  if (IS_DEBUG_UPDATE_PACKETS)
    {
      zlog_debug (
          ...
          circuit->area->area_tag, level, lsp->hdr.lsp_id, lsp->hdr.pdu_len,
          lsp->hdr.seqno, lsp->hdr.checksum, lsp->hdr.rem_lifetime,
          refresh_time);
    }
 ...
 */
static void
lsp_l1_refresh_pseudo (struct event *thread)
{
  struct isis_circuit *circuit;
  uint8_t id[ISIS_SYS_ID_LEN + 2];
...
static void
lsp_l2_refresh_pseudo (struct event *thread)
{
  struct isis_circuit *circuit;
  uint8_t id[ISIS_SYS_ID_LEN + 2];
  ...
  for (lvl = IS_LEVEL_1; lvl <= IS_LEVEL_2; lvl++)
    {
      ...
      if (circuit->lsp_regenerate_pending[lvl - 1])
        {
          struct timeval remain =
              event_timer_remain (circuit->u.bc.t_refresh_pseudo_lsp[lvl - 1]);
          sched_debug (
      ...
      if (lvl == IS_LEVEL_1)
        {
          event_add_timer_msec (master, lsp_l1_refresh_pseudo, circuit,
                                timeout,
                                &circuit->u.bc.t_refresh_pseudo_lsp[lvl - 1]);
        }
      else if (lvl == IS_LEVEL_2)
        {
          event_add_timer_msec (master, lsp_l2_refresh_pseudo, circuit,
                                timeout,
                                &circuit->u.bc.t_refresh_pseudo_lsp[lvl - 1]);
        }
    }
      ...
      frr_each_from (lspdb, &area->lspdb[level], lsp, next)
        {
          ...
          if (lsp->age_out == 0)
            {
              zlog_debug ("ISIS-Upd (%s): L%u LSP %pLS seq 0x%08x aged out",
                          area->area_tag, lsp->level, lsp->hdr.lsp_id,
                          lsp->hdr.seqno);
               ...
               */
              if (! LSP_FRAGMENT (lsp->hdr.lsp_id))
                while (next && ! memcmp (next->hdr.lsp_id, lsp->hdr.lsp_id,
                                         ISIS_SYS_ID_LEN + 1))
                  next = lspdb_next (&area->lspdb[level], next);
          ...
          if (fabricd_init_c && lsp)
            {
              fabricd_sync_incomplete |=
                  ISIS_CHECK_FLAG (lsp->SSNflags, fabricd_init_c);
            }
        }
    }
...
void
lsp_purge_pseudo (uint8_t *id, struct isis_circuit *circuit, int level)
{
  struct isis_lsp *lsp;
 ...
 */
void
lsp_purge_non_exist (int level, struct isis_lsp_hdr *hdr,
                     struct isis_area *area)
{
  struct isis_lsp *lsp;
...
void
lsp_set_all_srmflags (struct isis_lsp *lsp, bool set)
{
  struct listnode *node;
  struct isis_circuit *circuit;
...
void
_lsp_flood (struct isis_lsp *lsp, struct isis_circuit *circuit,
            const char *func, const char *file, int line)
{
  if (IS_DEBUG_FLOODING)
    {
      zlog_debug ("Flooding LSP %pLS%s%s (From %s %s:%d)", lsp->hdr.lsp_id,
                  circuit ? " except on " : "",
                  circuit ? circuit->interface->name : "", func, file, line);
    }
...
static int
lsp_handle_adj_state_change (struct isis_adjacency *adj)
{
 ...
 */
int
isis_lsp_iterate_ip_reach (struct isis_lsp *lsp, int family, uint16_t mtid,
      ...
      if (! fabricd && ! pseudo_lsp && family == AF_INET &&
          mtid == ISIS_MT_IPV4_UNICAST)
        {
          struct isis_item_list *reachs[] = {
            &lsp->tlvs->oldstyle_ip_reach, &lsp->tlvs->oldstyle_ip_reach_ext
          };
          ...
          for (unsigned int i = 0; i < array_size (reachs); i++)
            {
              ...
              for (r = (struct isis_oldstyle_ip_reach *) reachs[i]->head; r;
                   r = r->next)
                {
                  bool external = i ? true : false;

                  if ((*cb) ((struct prefix *) &r->prefix, r->metric, external,
                             NULL, arg) == LSP_ITER_STOP)
      ...
      if (! pseudo_lsp && family == AF_INET)
        {
          ...
          if (mtid == ISIS_MT_IPV4_UNICAST)
            ...
            ipv4_reachs = isis_lookup_mt_items (&lsp->tlvs->mt_ip_reach, mtid);

          struct isis_extended_ip_reach *r;
          for (r = ipv4_reachs
                       ? (struct isis_extended_ip_reach *) ipv4_reachs->head
                       : NULL;
               r; r = r->next)
            {
              if ((*cb) ((struct prefix *) &r->prefix, r->metric, false,
    ...
    for (ALL_LIST_ELEMENTS_RO (lsp->lspu.frags, node, frag))
      {
        ...
        if (isis_lsp_iterate_ip_reach (frag, family, mtid, cb, arg) ==
            LSP_ITER_STOP)
          return LSP_ITER_STOP;
      }
 ...
 */
int
isis_lsp_iterate_is_reach (struct isis_lsp *lsp, uint16_t mtid,
      ...
      if (pseudo_lsp || mtid == ISIS_MT_IPV4_UNICAST)
        {
          ...
          for (struct isis_oldstyle_reach *reach =
            ...
            {
              if ((*cb) (reach->id, reach->metric, true, NULL, arg) ==
                  LSP_ITER_STOP)
                return LSP_ITER_STOP;
            }
        }
      ...
      if (te_neighs)
        {
          ...
          for (struct isis_extended_reach *reach =
               ...
               reach; reach = reach->next)
            {
              if ((*cb) (reach->id, reach->metric, false, reach->subtlvs,
                         arg) == LSP_ITER_STOP)
                return LSP_ITER_STOP;
            }
        }
    ...
    for (ALL_LIST_ELEMENTS_RO (lsp->lspu.frags, node, frag))
      {
        ...
        if (isis_lsp_iterate_is_reach (frag, mtid, cb, arg) == LSP_ITER_STOP)
          return LSP_ITER_STOP;
      }
...
{
  device_startup = true;
  hook_register (isis_adj_state_change_hook, lsp_handle_adj_state_change);
}
