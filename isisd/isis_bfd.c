...
static void
adj_bfd_cb (struct bfd_session_params *bsp,
            const struct bfd_session_status *bss, void *arg)
{
  struct isis_adjacency *adj = arg;

  if (IS_DEBUG_BFD)
    zlog_debug ("ISIS-BFD: BFD changed status for adjacency %s old %s new %s",
                isis_adj_name (adj), bfd_get_status_str (bss->previous_state),
...
static void
bfd_handle_adj_down (struct isis_adjacency *adj)
{
  bfd_sess_free (&adj->bfd_session);
}

static void
bfd_handle_adj_up (struct isis_adjacency *adj)
{
  ...
  bfd_sess_set_timers (adj->bfd_session, BFD_DEF_DETECT_MULT, BFD_DEF_MIN_RX,
                       BFD_DEF_MIN_TX);
  if (family == AF_INET)
    bfd_sess_set_ipv4_addrs (adj->bfd_session, &src_ip.ipv4, &dst_ip.ipv4);
  else
    bfd_sess_set_ipv6_addrs (adj->bfd_session, &src_ip.ipv6, &dst_ip.ipv6);
...
static int
bfd_handle_adj_state_change (struct isis_adjacency *adj)
{
...
static void
bfd_adj_cmd (struct isis_adjacency *adj)
{
...
void
isis_bfd_circuit_cmd (struct isis_circuit *circuit)
{
...
static int
bfd_handle_adj_ip_enabled (struct isis_adjacency *adj, int family, bool global)
{
...
static int
bfd_handle_circuit_add_addr (struct isis_circuit *circuit)
{
  struct isis_adjacency *adj;
  struct listnode *node;
...
void
isis_bfd_init (struct event_loop *tm)
{
