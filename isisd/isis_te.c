 ...
 */
void
isis_mpls_te_create (struct isis_area *area)
{
 ...
 */
void
isis_mpls_te_disable (struct isis_area *area)
{
  struct listnode *node;
  struct isis_circuit *circuit;
...
void
isis_mpls_te_term (struct isis_area *area)
{
  struct listnode *node;
  struct isis_circuit *circuit;
...
static void
isis_link_params_update_asla (struct isis_circuit *circuit,
...
static int
_isis_mpls_te_adj_ip_enabled (struct isis_adjacency *adj, int family,
                              bool global)
{
  struct isis_circuit *circuit;
  struct isis_ext_subtlvs *ext;
...
static int
isis_mpls_te_adj_ip_enabled (struct isis_adjacency *adj, int family,
                             bool global)
{
  int ret;
...
static int
_isis_mpls_te_adj_ip_disabled (struct isis_adjacency *adj, int family,
                               bool global)
{
  struct isis_circuit *circuit;
  struct isis_ext_subtlvs *ext;
...
static int
isis_mpls_te_adj_ip_disabled (struct isis_adjacency *adj, int family,
                              bool global)
{
  int ret;
...
static void
isis_mpls_te_circuit_ip_update (struct isis_circuit *circuit)
{
 ...
 */
static int
isis_te_export (uint8_t type, void *link_state)
{
...
lsp_to_vertex (struct ls_ted *ted, struct isis_lsp *lsp)
{
...
get_edge (struct ls_ted *ted, struct ls_attributes *attr)
{
  struct ls_edge *edge;
  ...
  if (CHECK_FLAG (edge->attributes->flags, LS_ATTR_LOCAL_ADDR))
    ...
    te_debug ("    |- %s Edge (%" PRIu64 ")",
              edge->status == NEW ? "Create" : "Found", edge->key.k.link_id);
...
get_attributes (struct ls_node_id adv, struct isis_ext_subtlvs *tlvs)
{
 ...
 */
static int
lsp_to_edge_cb (const uint8_t *id, uint32_t metric, bool old_metric,
  ...
  if (dst)
  ...
  else
    {
      ...
      if (edge->destination == NULL)
        {
          vertex = ls_find_vertex_by_key (args->ted, sysid_to_key (id));
 ...
 */
static int
lsp_to_subnet_cb (const struct prefix *prefix, uint32_t metric, bool external,
                  struct isis_subtlvs *subtlvs, void *arg)
{
  ...
  if (! std)
  ...
  else
    {
      ...
      te_debug ("   |- Adjust prefix %pFX with local address to: %pFX", prefix,
                &p);
    }
 ...
 */
static void
isis_te_parse_lsp (struct mpls_te_area *mta, struct isis_lsp *lsp)
{
  struct ls_ted *ted;
  struct ls_vertex *vertex;
  struct ls_edge *edge;
 ...
 */
static void
isis_te_delete_lsp (struct mpls_te_area *mta, struct isis_lsp *lsp)
{
  struct ls_ted *ted;
  struct ls_vertex *vertex = NULL;
 ...
 */
void
isis_te_lsp_event (struct isis_lsp *lsp, enum lsp_event event)
{
 ...
 */
int
isis_te_sync_ted (struct zapi_opaque_reg_info dst)
{
 ...
 */
void
isis_te_init_ted (struct isis_area *area)
{
  struct isis_lsp *lsp;
...
static void
show_router_id (struct vty *vty, struct isis_area *area)
{
  bool no_match = true;
...
static void
show_ext_sub (struct vty *vty, char *name, struct isis_ext_subtlvs *ext)
{
  struct sbuf buf;
 ...
 */
static struct ls_vertex *
vertex_for_arg (struct ls_ted *ted, const char *id, struct isis *isis)
{
  char sysid[255] = { 0 };
  uint8_t number[3];
  const char *pos;
 ...
 */
static int
show_ted (struct vty *vty, struct cmd_token *argv[], int argc,
 ...
 */
static int
show_isis_ted (struct vty *vty, struct cmd_token *argv[], int argc,
               struct isis *isis)
{
  struct listnode *node;
  struct isis_area *area;
