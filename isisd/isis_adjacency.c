...
adj_alloc (struct isis_circuit *circuit, const uint8_t *id)
{
...
isis_new_adj (const uint8_t *id, const uint8_t *snpa, int level,
...
struct isis_adjacency *
isis_adj_lookup (const uint8_t *sysid, struct list *adjdb)
{
  struct isis_adjacency *adj;
...
struct isis_adjacency *
isis_adj_lookup_snpa (const uint8_t *ssnpa, struct list *adjdb)
{
  struct listnode *node;
  struct isis_adjacency *adj;
...
struct isis_adjacency *
isis_adj_find (const struct isis_area *area, int level, const uint8_t *sysid)
{
  struct isis_adjacency *adj;
  struct listnode *node;
...
static void
isis_adj_route_switchover (struct isis_adjacency *adj)
{
  union g_addr ip = {};
  ifindex_t ifindex;
  unsigned int i;
  ...
  for (i = 0; i < adj->ipv4_address_count; i++)
    {
      ...
      isis_circuit_switchover_routes (adj->circuit, AF_INET, &ip, ifindex);
    }

  for (i = 0; i < adj->ll_ipv6_count; i++)
    {
      ip.ipv6 = adj->ll_ipv6_addrs[i];
      isis_circuit_switchover_routes (adj->circuit, AF_INET6, &ip, ifindex);
    }
...
void
isis_adj_process_threeway (struct isis_adjacency *adj,
...
const char *
isis_adj_name (const struct isis_adjacency *adj)
{
...
void
isis_log_adj_change (struct isis_adjacency *adj, enum isis_adj_state old_state,
...
void
isis_adj_state_change (struct isis_adjacency **padj,
                       enum isis_adj_state new_state, const char *reason)
{
          ...
          if (new_state == ISIS_ADJ_UP)
            {
              ...
              if (level == IS_LEVEL_1)
                {
                  event_add_timer (master, send_l1_csnp, circuit, 0,
                                   &circuit->t_send_csnp[0]);
                }
              else
                {
                  event_add_timer (master, send_l2_csnp, circuit, 0,
      ...
      for (unsigned int i = 0; i < adj->ll_ipv6_count; i++)
        {
          ...
          inet_ntop (AF_INET6, &adj->ll_ipv6_addrs[i], buf, sizeof (buf));
          zlog_debug ("%s", buf);
        }
    }
...
const char *
isis_adj_yang_state (enum isis_adj_state state)
{
  switch (state)
    {
    case ISIS_ADJ_DOWN:
...
void
isis_adj_expire (struct event *thread)
{
  struct isis_adjacency *adj;
 ...
 */
void
isis_adj_print_json (struct isis_adjacency *adj, struct json_object *json,
                     char detail)
{
  json_object *iface_json, *ipv4_addr_json, *ipv6_link_json,
      *ipv6_non_link_json, *topo_json, *dis_flaps_json, *area_addr_json,
      *adj_sid_json;
  time_t now;
  struct isis_dynhn *dyn;
  int level;
  ...
  if (detail == ISIS_UI_LEVEL_BRIEF)
    {
      if (adj->circuit)
        ...
        json_object_string_add (json, "interface", "NULL circuit!");
      ...
      if (adj->last_upd)
        {
          if (adj->last_upd + adj->hold_time < now)
            json_object_string_add (json, "last-upd", "expiring");
          else
  ...
  if (detail == ISIS_UI_LEVEL_DETAIL)
    {
      ...
      if (adj->circuit)
        ...
        json_object_string_add (iface_json, "name", "null-circuit");
      json_object_int_add (json, "level", adj->level);
      ...
      if (adj->last_upd)
        {
          if (adj->last_upd + adj->hold_time < now)
            json_object_string_add (iface_json, "last-upd", "expiring");
          else
            json_object_string_add (
                json, "expires-in",
                time2string (adj->last_upd + adj->hold_time - now));
      ...
      if (adj->mt_count != 1 || adj->mt_set[0] != ISIS_MT_IPV4_UNICAST)
        {
          ...
          json_object_object_add (iface_json, "topologies", topo_json);
          for (unsigned int i = 0; i < adj->mt_count; i++)
            {
              snprintfrr (buf, sizeof (buf), "topo-%d", i);
              json_object_string_add (topo_json, buf,
      ...
      if (adj->circuit && (adj->circuit->circ_type == CIRCUIT_T_BROADCAST))
        {
          ...
          if (dyn)
            {
              snprintfrr (buf, sizeof (buf), "%s-%02x", dyn->hostname,
                          adj->lanid[ISIS_SYS_ID_LEN]);
              json_object_string_add (iface_json, "lan-id", buf);
            }
          else
            {
              json_object_string_addf (iface_json, "lan-id", "%pSY",
                                       adj->lanid);
            }
          ...
          json_object_object_add (iface_json, "dis-flaps", dis_flaps_json);
          json_object_string_add (
              dis_flaps_json, "dis-record",
              isis_disflag2string (
                  adj->dis_record[ISIS_LEVELS + level - 1].dis));
      ...
      if (adj->area_address_count)
        {
          ...
          json_object_object_add (iface_json, "area-address", area_addr_json);
          for (unsigned int i = 0; i < adj->area_address_count; i++)
            {
              json_object_string_addf (area_addr_json, "isonet", "%pIS",
      ...
      if (adj->ipv4_address_count)
        {
          ...
          json_object_object_add (iface_json, "ipv4-address", ipv4_addr_json);
          for (unsigned int i = 0; i < adj->ipv4_address_count; i++)
            {
              inet_ntop (AF_INET, &adj->ipv4_addresses[i], buf, sizeof (buf));
          ...
          for (unsigned int i = 0; i < adj->ll_ipv6_count; i++)
            {
              char buf[INET6_ADDRSTRLEN];
              inet_ntop (AF_INET6, &adj->ll_ipv6_addrs[i], buf, sizeof (buf));
              json_object_string_add (ipv6_link_json, "ipv6", buf);
            }
        }
      if (adj->global_ipv6_count)
        {
          ...
          for (unsigned int i = 0; i < adj->global_ipv6_count; i++)
            {
              char buf[INET6_ADDRSTRLEN];
              inet_ntop (AF_INET6, &adj->global_ipv6_addrs[i], buf,
                         sizeof (buf));
              json_object_string_add (ipv6_non_link_json, "ipv6", buf);
            }
        }
      ...
      for (ALL_LIST_ELEMENTS_RO (adj->adj_sids, anode, sra))
        {
          ...
          json_object_string_add (adj_sid_json, "nexthop",
                                  (sra->nexthop.family == AF_INET) ? "IPv4"
                                                                   : "IPv6");
          json_object_string_add (adj_sid_json, "adj-type", adj_type);
          json_object_string_add (adj_sid_json, "is-backup", backup);
      ...
      if (adj->last_upd)
        {
          if (adj->last_upd + adj->hold_time < now)
            ...
            vty_out (vty, " %-9llu",
                     (unsigned long long) adj->last_upd + adj->hold_time -
                         now);
        }
      else
        vty_out (vty, "-        ");
      ...
      if (adj->circuit && (adj->circuit->circ_type == CIRCUIT_T_BROADCAST))
        {
          ...
          vty_out (vty, ", %s, DIS flaps: %u, Last: %s ago",
                   isis_disflag2string (
                       adj->dis_record[ISIS_LEVELS + level - 1].dis),
                   adj->dischanges[level - 1],
                   time2string (now - (adj->dis_record[ISIS_LEVELS + level - 1]
                                           .last_dis_change)));
        }
      vty_out (vty, "\n");
      ...
      if (adj->circuit && adj->circuit->bfd_config.enabled)
        {
          vty_out (vty, "    BFD is %s%s\n",
                   adj->bfd_session ? "active, status " : "configured",
                   ! adj->bfd_session ? ""
      ...
      for (ALL_LIST_ELEMENTS_RO (adj->adj_sids, anode, sra))
        {
          ...
          vty_out (vty, "    %s %s%s: %u\n",
                   (sra->nexthop.family == AF_INET) ? "IPv4" : "IPv6",
                   adj_type, backup, sid);
        }
      vty_out (vty, "\n");
    }
