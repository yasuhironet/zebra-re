...
DEFINE_HOOK (isis_circuit_new_hook, (struct isis_circuit * circuit),
             (circuit));
DEFINE_HOOK (isis_circuit_del_hook, (struct isis_circuit * circuit),
             (circuit));

static void
isis_circuit_enable (struct isis_circuit *circuit)
{
...
static void
isis_circuit_disable (struct isis_circuit *circuit)
{
  struct isis_area *area = circuit->area;
  struct interface *ifp = circuit->interface;
...
struct isis_circuit *
isis_circuit_new (struct interface *ifp, const char *tag)
{
  ...
  if (connected->address->family == AF_INET)
    {
      ...
      if (circuit->ext && circuit->area && IS_MPLS_TE (circuit->area->mta))
        {
...
#ifdef EXTREME_DEBUG
      if (IS_DEBUG_EVENTS)
        zlog_debug ("Added IP address %pFX to circuit %s", connected->address,
  ...
  if (connected->address->family == AF_INET6)
    {
      ...
      if (IN6_IS_ADDR_LINKLOCAL (&ipv6->prefix))
      ...
      else
        {
          ...
          if (circuit->ext && circuit->area && IS_MPLS_TE (circuit->area->mta))
            {
              IPV6_ADDR_COPY (&circuit->ext->local_addr6, &ipv6->prefix);
...
static uint8_t
isis_circuit_id_gen (struct isis *isis, struct interface *ifp)
{
...
static bool
isis_circuit_lfa_enabled (struct isis_circuit *circuit, int level)
{
...
void
isis_circuit_switchover_routes (struct isis_circuit *circuit, int family,
                                union g_addr *nexthop_ip, ifindex_t ifindex)
{
  ...
  if ((is_type == IS_LEVEL_1 || is_type == IS_LEVEL_1_AND_2) &&
    ...
    isis_area_switchover_routes (circuit->area, family, nexthop_ip, ifindex,
                                 IS_LEVEL_1);
...
#ifdef EXTREME_DEGUG
      if (IS_DEBUG_EVENTS)
        zlog_debug ("%s: if_id %d, isomtu %d snpa %pSY", __func__,
      ...
      if (circuit->circ_type == CIRCUIT_T_P2P)
        {
          ...
          if (adj)
            isis_log_adj_change (adj, adj->adj_state, ISIS_ADJ_DOWN,
      ...
      else if (circuit->circ_type == CIRCUIT_T_BROADCAST)
        {
          ...
          if (circuit->u.bc.adjdb[0])
            {
              adj_list = list_new ();
              isis_adj_build_up_list (circuit->u.bc.adjdb[0], adj_list);
          ...
          if (circuit->u.bc.adjdb[1])
            {
              adj_list = list_new ();
              isis_adj_build_up_list (circuit->u.bc.adjdb[1], adj_list);
              for (ALL_LIST_ELEMENTS_RO (adj_list, node, adj))
                isis_log_adj_change (adj, adj->adj_state, ISIS_ADJ_DOWN,
                                     "circuit is being brought down");
              list_delete (&adj_list);
  ...
  if (circuit->circ_type == CIRCUIT_T_BROADCAST)
    {
      ...
      _ISIS_CLEAR_FLAG (circuit->isis->circuit_ids_used, circuit->circuit_id);
...
void
isis_circuit_print_json (struct isis_circuit *circuit,
  ...
  if (detail == ISIS_UI_LEVEL_BRIEF)
    {
      ...
      json_object_string_add (iface_json, "name", circuit->interface->name);
  ...
  if (detail == ISIS_UI_LEVEL_DETAIL)
    {
      ...
      json_object_string_add (iface_json, "state",
                              circuit_state2string (circuit->state));
      if (circuit->is_passive)
        json_object_string_add (iface_json, "is-passive", "passive");
      else
        json_object_string_add (iface_json, "is-passive", "active");
      json_object_string_add (iface_json, "circuit-id", buf);
      json_object_string_add (iface_json, "type",
                              circuit_type2string (circuit->circ_type));
      json_object_string_add (iface_json, "level",
      ...
      for (level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++)
        {
          ...
          if (circuit->area->newmetric)
            json_object_int_add (level_json, "metric", circuit->te_metric[0]);
          else
            json_object_int_add (level_json, "metric", circuit->metric[0]);
          if (! circuit->is_passive)
            {
              ...
              json_object_object_add (level_json, "holddown", hold_json);
              json_object_int_add (hold_json, "count",
                                   circuit->hello_multiplier[0]);
              json_object_string_add (
                  hold_json, "pad",
                  isis_hello_padding2string (circuit->pad_hellos));
              ...
              if (circuit->circ_type == CIRCUIT_T_BROADCAST)
                {
                  lan_prio_json = json_object_new_object ();
                  json_object_object_add (level_json, "lan", lan_prio_json);
                  json_object_int_add (lan_prio_json, "priority",
                                       circuit->priority[0]);
                  json_object_string_add (
                      lan_prio_json, "is-dis",
                      (circuit->u.bc.is_dr[0] ? "yes" : "no"));
                }
            }
      ...
      if (listcount (circuit->ip_addrs) > 0)
        {
          ...
          json_object_object_add (iface_json, "ip-prefix", ipv4_addr_json);
          for (ALL_LIST_ELEMENTS_RO (circuit->ip_addrs, node, ip_addr))
            {
              snprintfrr (buf_prx, INET6_BUFSIZ, "%pFX", ip_addr);
              json_object_string_add (ipv4_addr_json, "ip", buf_prx);
            }
        }
      if (listcount (circuit->ipv6_link) > 0)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (circuit->ipv6_link, node, ip_addr))
            {
              snprintfrr (buf_prx, INET6_BUFSIZ, "%pFX", ip_addr);
              json_object_string_add (ipv6_link_json, "ipv6", buf_prx);
            }
        }
      if (listcount (circuit->ipv6_non_link) > 0)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (circuit->ipv6_non_link, node, ip_addr))
            {
              snprintfrr (buf_prx, INET6_BUFSIZ, "%pFX", ip_addr);
              json_object_string_add (ipv6_non_link_json, "ipv6", buf_prx);
            }
        }
    }
  return;
          ...
          if (! circuit->is_passive)
            {
              ...
              vty_out (
                  ...
                  circuit->hello_interval[0], circuit->hello_multiplier[0],
                  isis_hello_padding2string (circuit->pad_hellos));
              vty_out (vty, "      CNSP interval: %u, PSNP interval: %u\n",
                       circuit->csnp_interval[0], circuit->psnp_interval[0]);
              if (circuit->circ_type == CIRCUIT_T_BROADCAST)
          ...
          if (! circuit->is_passive)
            {
              ...
              vty_out (
                  vty,
                  "      Hello interval: %u, Holddown count: %u, Padding: %s\n",
                  circuit->hello_interval[1], circuit->hello_multiplier[1],
                  isis_hello_padding2string (circuit->pad_hellos));
              vty_out (vty, "      CNSP interval: %u, PSNP interval: %u\n",
                       circuit->csnp_interval[1], circuit->psnp_interval[1]);
              if (circuit->circ_type == CIRCUIT_T_BROADCAST)
                vty_out (vty, "      LAN Priority: %u, %s\n",
                         circuit->priority[1],
          ...
          if (circuit->hello_interval[0] == circuit->hello_interval[1])
            {
              if (circuit->hello_interval[0] != DEFAULT_HELLO_INTERVAL)
                {
                  vty_out (vty, " " PROTO_NAME " hello-interval %d\n",
                           circuit->hello_interval[0]);
                  write++;
                }
            }
          ...
          if (circuit->hello_multiplier[0] == circuit->hello_multiplier[1])
            {
              if (circuit->hello_multiplier[0] != DEFAULT_HELLO_MULTIPLIER)
                {
                  vty_out (vty, " " PROTO_NAME " hello-multiplier %d\n",
                           circuit->hello_multiplier[0]);
                  write++;
                }
            }
              ...
              for (i = 0; i < 2; i++)
                {
                  if (circuit->priority[i] != DEFAULT_PRIORITY)
                    {
                      vty_out (vty, " " PROTO_NAME " priority %d level-%d\n",
              ...
              for (i = 0; i < 2; i++)
                {
                  if (circuit->te_metric[i] != DEFAULT_CIRCUIT_METRIC)
                    {
                      vty_out (vty, " " PROTO_NAME " metric %d level-%d\n",
...
void
isis_circuit_af_set (struct isis_circuit *circuit, bool ip_router,
                     bool ipv6_router)
{
  struct isis_area *area = circuit->area;
...
isis_circuit_metric_set (struct isis_circuit *circuit, int level, int metric)
{
  ...
  if (metric > MAX_WIDE_LINK_METRIC)
    return ferr_cfg_invalid ("metric %d too large for wide metric", metric);
  if (circuit->area && circuit->area->oldmetric &&
      metric > MAX_NARROW_LINK_METRIC)
    return ferr_cfg_invalid ("metric %d too large for narrow metric", metric);
...
ferr_r
isis_circuit_passwd_set (struct isis_circuit *circuit, uint8_t passwd_type,
                         const char *passwd)
{
  int len;
  ...
  len = strlen (passwd);
  if (len > 254)
    return ferr_code_bug ("circuit password too long (max 254 chars)");
...
ferr_r
isis_circuit_passwd_cleartext_set (struct isis_circuit *circuit,
                                   const char *passwd)
{
...
ferr_r
isis_circuit_passwd_hmac_md5_set (struct isis_circuit *circuit,
                                  const char *passwd)
{
...
static int
isis_ifp_create (struct interface *ifp)
{
...
static int
isis_ifp_up (struct interface *ifp)
{
  struct isis_circuit *circuit = ifp->info;
...
static int
isis_ifp_down (struct interface *ifp)
{
  afi_t afi;
  struct isis_circuit *circuit = ifp->info;

  if (circuit && ! CHECK_FLAG (circuit->flags, ISIS_CIRCUIT_IF_DOWN_FROM_Z))
    {
...
static int
isis_ifp_destroy (struct interface *ifp)
{
  struct isis_circuit *circuit = ifp->info;
