...
void
isis_run_dr (struct event *thread)
{
  ...
  if (circuit->circ_type != CIRCUIT_T_BROADCAST)
    {
      zlog_warn ("%s: scheduled for non broadcast circuit from %s:%d",
                 __func__, thread->xref->xref.file, thread->xref->xref.line);
      return;
    }

  if (circuit->u.bc.run_dr_elect[level - 1])
    zlog_warn ("%s: run_dr_elect already set for l%d", __func__, level);
      ...
      for (i = DIS_RECORDS - 1; i > 0; i--)
        {
          ...
          adj->dis_record[(i * ISIS_LEVELS) + level - 1].last_dis_change =
              adj->dis_record[((i - 1) * ISIS_LEVELS) + level - 1]
                  .last_dis_change;
        }
    }
  return ISIS_OK;
          ...
          if (adj_dr)
            {
              ...
              if (cmp_res == 0)
                zlog_warn ("%s: multiple adjacencies with same SNPA",
                           __func__);
            }
          else
            {
