 ...
 */
int
dotformat2buff (uint8_t *buff, const char *dotted)
{
  int dotlen, len = 0;
 ...
 */
int
sysid2buff (uint8_t *buff, const char *dotted)
{
  int len = 0;
  const char *pos = dotted;
  uint8_t number[3];
...
nlpid2string (struct nlpids *nlpids)
{
  ...
  for (i = 0; i < nlpids->count; i++)
    {
      snprintf (tbuf, sizeof (tbuf), "%s", nlpid2str (nlpids->nlpids[i]));
...
const char *
isis_hello_padding2string (int hello_padding_type)
{
...
struct in_addr
newprefix2inaddr (uint8_t *prefix_start, uint8_t prefix_masklen)
{
  memset (&new_prefix, 0, sizeof (new_prefix));
  memcpy (&new_prefix, prefix_start,
          (prefix_masklen & 0x3F) ? ((((prefix_masklen & 0x3F) - 1) >> 3) + 1)
                                  : 0);
  return new_prefix;
}
 ...
 */
const char *
print_sys_hostname (const uint8_t *sysid)
{
...
void
log_multiline (int priority, const char *prefix, const char *format, ...)
{
  char shortbuf[256];
  va_list ap;
  char *p;
...
char *
log_uptime (time_t uptime, char *buf, size_t nbuf)
{
  struct tm *tm;
...
void
vty_multiline (struct vty *vty, const char *prefix, const char *format, ...)
{
  char shortbuf[256];
  va_list ap;
  char *p;
