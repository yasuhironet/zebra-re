 ...
 */
int
isis_ldp_sync_state_update (struct ldp_igp_sync_if_state state)
{
...
int
isis_ldp_sync_announce_update (struct ldp_igp_sync_announce announce)
{
  struct isis_area *area;
...
void
isis_ldp_sync_state_req_msg (struct isis_circuit *circuit)
{
 ...
 */
void
isis_ldp_sync_if_start (struct isis_circuit *circuit, bool send_state_req)
{
...
void
isis_ldp_sync_if_complete (struct isis_circuit *circuit)
{
  struct ldp_sync_info *ldp_sync_info;
...
void
isis_ldp_sync_ldp_fail (struct isis_circuit *circuit)
{
  struct ldp_sync_info *ldp_sync_info;
...
static int
isis_ldp_sync_adj_state_change (struct isis_adjacency *adj)
{
  ...
  if (adj->adj_state == ISIS_ADJ_UP)
  ...
  else
    {
      ...
      ils_debug ("%s: down on if %s", __func__, circuit->interface->name);
...
bool
isis_ldp_sync_if_metric_config (struct isis_circuit *circuit, int level,
                                int metric)
{
  struct ldp_sync_info *ldp_sync_info = circuit->ldp_sync_info;
  struct isis_area *area = circuit->area;
...
void
isis_ldp_sync_set_if_metric (struct isis_circuit *circuit, bool run_regen)
{
  struct ldp_sync_info *ldp_sync_info;
      ...
      if (circuit->is_type & IS_LEVEL_1)
        {
          if (circuit->area->newmetric)
            {
              ldp_sync_info->metric[0] = circuit->te_metric[0];
              circuit->te_metric[0] = ISIS_WIDE_METRIC_INFINITY;
            }
          else
            {
              ldp_sync_info->metric[0] = circuit->metric[0];
              circuit->metric[0] = ISIS_NARROW_METRIC_INFINITY;
            }
        }
      if (circuit->is_type & IS_LEVEL_2)
        {
          if (circuit->area->newmetric)
            {
              ldp_sync_info->metric[1] = circuit->te_metric[1];
              circuit->te_metric[1] = ISIS_WIDE_METRIC_INFINITY;
            }
          else
            {
              ldp_sync_info->metric[1] = circuit->metric[1];
              circuit->metric[1] = ISIS_NARROW_METRIC_INFINITY;
            }
        }
    }
  else
 ...
 */
static void
isis_ldp_sync_holddown_timer (struct event *thread)
{
  struct isis_circuit *circuit;
  struct ldp_sync_info *ldp_sync_info;
  ...
  ils_debug ("%s: holddown timer expired for %s state:sync achieved", __func__,
...
void
isis_ldp_sync_holddown_timer_add (struct isis_circuit *circuit)
{
  struct ldp_sync_info *ldp_sync_info;
...
void
isis_area_ldp_sync_enable (struct isis_area *area)
{
  struct isis_circuit *circuit;
...
void
isis_area_ldp_sync_disable (struct isis_area *area)
{
  struct isis_circuit *circuit;
  struct listnode *node;
...
void
isis_area_ldp_sync_set_holddown (struct isis_area *area, uint16_t holddown)
{
  struct isis_circuit *circuit;
  struct listnode *node;
...
void
isis_if_ldp_sync_enable (struct isis_circuit *circuit)
{
  struct ldp_sync_info *ldp_sync_info = circuit->ldp_sync_info;
  struct isis_area *area = circuit->area;
...
void
isis_if_ldp_sync_disable (struct isis_circuit *circuit)
{
  struct ldp_sync_info *ldp_sync_info = circuit->ldp_sync_info;
  struct isis_area *area = circuit->area;
...
void
isis_if_set_ldp_sync_holddown (struct isis_circuit *circuit)
{
  struct ldp_sync_info *ldp_sync_info = circuit->ldp_sync_info;
  struct isis_area *area = circuit->area;
...
static void
isis_circuit_ldp_sync_print_vty (struct isis_circuit *circuit, struct vty *vty)
{
  struct ldp_sync_info *ldp_sync_info;
  const char *ldp_state;

  if (circuit->ldp_sync_info == NULL || if_is_loopback (circuit->interface))
    return;
      ...
      if (ldp_sync_info->t_holddown != NULL)
        {
          ...
          vty_out (vty, "  Holddown timer is running %lld.%03lld remaining\n",
...
isis_ldp_sync_init (void)
{
  ...
  hook_register (isis_adj_state_change_hook, isis_ldp_sync_adj_state_change);
}
