...
DEFINE_MTYPE_STATIC (ISISD, FABRICD_NEIGHBOR,
                     "ISIS OpenFabric Neighbor Entry");
DEFINE_MTYPE_STATIC (ISISD, FABRICD_FLOODING_INFO,
...
neighbor_entry_new (const uint8_t *id, struct isis_adjacency *adj)
{
...
static void
neighbor_entry_del (struct neighbor_entry *neighbor)
{
  XFREE (MTYPE_FABRICD_NEIGHBOR, neighbor);
}

static void
neighbor_entry_del_void (void *arg)
{
  neighbor_entry_del ((struct neighbor_entry *) arg);
}

static void
neighbor_lists_clear (struct fabricd *f)
{
...
static unsigned
neighbor_entry_hash_key (const void *np)
{
...
static bool
neighbor_entry_hash_cmp (const void *a, const void *b)
{
...
static int
neighbor_entry_list_cmp (const void *a, const void *b)
{
  const struct neighbor_entry *na = a, *nb = b;
...
static struct neighbor_entry *
neighbor_entry_lookup_list (struct skiplist *list, const uint8_t *id)
{
...
static struct neighbor_entry *
neighbor_entry_lookup_hash (struct hash *hash, const uint8_t *id)
{
  struct neighbor_entry n = { { 0 } };
...
static int
fabricd_handle_adj_state_change (struct isis_adjacency *arg)
{
...
static void
neighbors_neighbors_update (struct fabricd *f)
{
  hash_clean (f->neighbors_neighbors, neighbor_entry_del_void);
...
struct fabricd *
fabricd_new (struct isis_area *area)
{
  ...
  rv->spftree =
      isis_spftree_new (area, &area->lspdb[IS_LEVEL_2 - 1], area->isis->sysid,
                        ISIS_LEVEL2, SPFTREE_IPV4, SPF_TYPE_FORWARD,
                        F_SPFTREE_HOPCOUNT_METRIC, SR_ALGORITHM_SPF);
  rv->neighbors =
      skiplist_new (0, neighbor_entry_list_cmp, neighbor_entry_del_void);
  rv->neighbors_neighbors = hash_create (
      neighbor_entry_hash_key, neighbor_entry_hash_cmp, "Fabricd Neighbors");
...
void
fabricd_finish (struct fabricd *f)
{
...
static void
fabricd_initial_sync_timeout (struct event *thread)
{
  struct fabricd *f = EVENT_ARG (thread);

  if (IS_DEBUG_ADJ_PACKETS)
    zlog_debug ("OpenFabric: Initial synchronization on %s timed out!",
...
void
fabricd_initial_sync_hello (struct isis_circuit *circuit)
{
  ...
  if (IS_DEBUG_ADJ_PACKETS)
    zlog_debug ("OpenFabric: Started initial synchronization with %pSY on %s",
                circuit->u.p2p.neighbor->sysid, circuit->interface->name);
}

bool
fabricd_initial_sync_is_in_progress (struct isis_area *area)
{
...
bool
fabricd_initial_sync_is_complete (struct isis_area *area)
{
  struct fabricd *f = area->fabricd;
...
struct isis_circuit *
fabricd_initial_sync_circuit (struct isis_area *area)
{
  struct fabricd *f = area->fabricd;
  if (! f)
    return NULL;
...
void
fabricd_initial_sync_finish (struct isis_area *area)
{
  struct fabricd *f = area->fabricd;
...
static uint8_t
fabricd_calculate_fabric_tier (struct isis_area *area)
{
  struct isis_spftree *local_tree = fabricd_spftree (area);
  struct listnode *node;

  struct isis_vertex *furthest_t0 = NULL, *second_furthest_t0 = NULL;
  ...
  if (! furthest_from_remote)
    ...
    {
      zlog_info ("OpenFabric: Found %pLS as furthest from remote dist == %u",
...
static void
fabricd_tier_set_timer (struct event *thread)
{
  struct fabricd *f = EVENT_ARG (thread);
...
static void
fabricd_tier_calculation_cb (struct event *thread)
{
  struct fabricd *f = EVENT_ARG (thread);
  ...
  zlog_info ("OpenFabric: Got tier %hhu from algorithm. Arming timer.", tier);
  f->tier_pending = tier;
...
static void
fabricd_bump_tier_calculation_timer (struct fabricd *f)
{
...
static void
fabricd_set_tier (struct fabricd *f, uint8_t tier)
{
  if (f->tier == tier)
    return;
...
void
fabricd_run_spf (struct isis_area *area)
{
  struct fabricd *f = area->fabricd;
...
struct isis_spftree *
fabricd_spftree (struct isis_area *area)
{
  struct fabricd *f = area->fabricd;
...
void
fabricd_configure_tier (struct isis_area *area, uint8_t tier)
{
  struct fabricd *f = area->fabricd;
...
uint8_t
fabricd_tier (struct isis_area *area)
{
  struct fabricd *f = area->fabricd;
...
int
fabricd_write_settings (struct isis_area *area, struct vty *vty)
{
  struct fabricd *f = area->fabricd;
  int written = 0;
  ...
  if (f->csnp_delay != FABRICD_DEFAULT_CSNP_DELAY || f->always_send_csnp)
    {
...
static void
move_to_queue (struct isis_lsp *lsp, struct neighbor_entry *n,
               enum isis_tx_type type, struct isis_circuit *circuit)
{
  ...
  if (IS_DEBUG_FLOODING)
    {
      zlog_debug ("OpenFabric: Adding %s to %s", print_sys_hostname (n->id),
...
static void
mark_neighbor_as_present (struct hash_bucket *bucket, void *arg)
{
...
static void
handle_firsthops (struct hash_bucket *bucket, void *arg)
{
...
lsp_for_neighbor (struct fabricd *f, struct neighbor_entry *n)
{
...
static void
fabricd_free_lsp_flooding_info (void *val)
{
  XFREE (MTYPE_FABRICD_FLOODING_INFO, val);
}

static void
fabricd_lsp_reset_flooding_info (struct isis_lsp *lsp,
                                 struct isis_circuit *circuit)
{
  lsp->flooding_time = time (NULL);

  XFREE (MTYPE_FABRICD_FLOODING_INFO, lsp->flooding_interface);
  for (enum isis_tx_type type = TX_LSP_NORMAL; type <= TX_LSP_CIRCUIT_SCOPED;
       type++)
    {
  ...
  if (circuit)
    {
      lsp->flooding_interface =
          XSTRDUP (MTYPE_FABRICD_FLOODING_INFO, circuit->interface->name);
    }
...
void
fabricd_lsp_flood (struct isis_lsp *lsp, struct isis_circuit *circuit)
{
  struct fabricd *f = lsp->area->fabricd;
  assert (f);
  ...
  struct isis_vertex *originator = isis_find_vertex (
      &f->spftree->paths, lsp->hdr.lsp_id, VTYPE_NONPSEUDO_TE_IS);
  ...
  while (! skiplist_next (f->neighbors, NULL, (void **) &n, &cursor))
    {
      ...
      if (IS_DEBUG_FLOODING)
        {
          zlog_debug ("Considering %s from NL...", print_sys_hostname (n->id));
        }
      ...
      for (er = (struct isis_extended_reach *) nlsp->tlvs->extended_reach.head;
           er; er = er->next)
        {
          ...
          nn = neighbor_entry_lookup_hash (f->neighbors_neighbors, er->id);
          ...
          if (nn)
            {
              if (IS_DEBUG_FLOODING)
                {
                  zlog_debug (
                      "Found neighbor %s in NN, removing it from NN and setting reflood.",
      ...
      move_to_queue (lsp, n,
                     need_reflood ? TX_LSP_NORMAL : TX_LSP_CIRCUIT_SCOPED,
                     circuit);
    }
...
void
fabricd_trigger_csnp (struct isis_area *area, bool circuit_scoped)
{
  struct fabricd *f = area->fabricd;
...
struct list *
fabricd_ip_addrs (struct isis_circuit *circuit)
{
...
void
fabricd_lsp_free (struct isis_lsp *lsp)
{
  XFREE (MTYPE_FABRICD_FLOODING_INFO, lsp->flooding_interface);
  for (enum isis_tx_type type = TX_LSP_NORMAL; type <= TX_LSP_CIRCUIT_SCOPED;
       type++)
...
void
fabricd_update_lsp_no_flood (struct isis_lsp *lsp,
                             struct isis_circuit *circuit)
{
  if (! fabricd)
    return;
...
void
fabricd_configure_triggered_csnp (struct isis_area *area, int delay,
                                  bool always_send_csnp)
{
  struct fabricd *f = area->fabricd;
...
fabricd_init (void)
{
  hook_register (isis_adj_state_change_hook, fabricd_handle_adj_state_change);
}
