...
const char *
vid2string (const struct isis_vertex *vertex, char *buff, int size)
{
...
static bool
prefix_sid_cmp (const void *value1, const void *value2)
{
...
static unsigned int
prefix_sid_key_make (const void *value)
{
...
struct isis_vertex *
isis_spf_prefix_sid_lookup (struct isis_spftree *spftree,
...
isis_vertex_new (struct isis_spftree *spftree, void *id, enum vertextype vtype)
{
  ...
  if (spftree->area->srdb.enabled && psid)
    {
      if (vertex->N.ip.sr.present && vertex->N.ip.sr.sid.value != psid->value)
      ...
      else
        {
          vadj->sr.sid = *psid;
          vadj->sr.label =
              sr_prefix_out_label (spftree->lspdb, vertex->N.ip.p.dest.family,
...
bool
isis_vertex_adj_exists (const struct isis_spftree *spftree,
...
static void
_isis_spftree_init (struct isis_spftree *tree)
{
...
static void
_isis_spftree_del (struct isis_spftree *spftree)
{
...
#ifndef FABRICD
static void
isis_spftree_clear (struct isis_spftree *spftree)
{
  _isis_spftree_del (spftree);
      ...
      for (int level = ISIS_LEVEL1; level <= ISIS_LEVEL2; level++)
        {
          ...
          area->spftree[tree][level - 1] = isis_spftree_new (
              area, &area->lspdb[level - 1], area->isis->sysid, level, tree,
...
static int
spf_adj_state_change (struct isis_adjacency *adj)
{
 ...
 */
struct isis_lsp *
isis_root_system_lsp (struct lspdb_head *lspdb, const uint8_t *sysid)
{
  struct isis_lsp *lsp;
 ...
 */
static struct isis_vertex *
isis_spf_add_root (struct isis_spftree *spftree)
{
  struct isis_vertex *vertex;
#ifdef EXTREME_DEBUG
...
static void
vertex_add_parent_firsthop (struct hash_bucket *bucket, void *arg)
{
...
static void
vertex_update_firsthops (struct isis_vertex *vertex,
  ...
  if (VTYPE_IP (vtype) && spftree->area->srdb.enabled && psid)
    {
      ...
      if (vertex_psid &&
          ! prefix_same (&vertex_psid->N.ip.p.dest, &vertex->N.ip.p.dest))
        {
          flog_warn (
...
isis_spf_add_local (struct isis_spftree *spftree, enum vertextype vtype,
                    void *id, struct isis_spf_adj *sadj, uint32_t cost,
...
process_N (struct isis_spftree *spftree, enum vertextype vtype, void *id,
           uint32_t dist, uint16_t depth, struct isis_prefix_sid *psid,
           struct isis_vertex *parent)
{
  struct isis_vertex *vertex;
#ifdef EXTREME_DEBUG
...
isis_spf_process_lsp (struct isis_spftree *spftree, struct isis_lsp *lsp,
                      uint32_t cost, uint16_t depth, uint8_t *root_sysid,
                      struct isis_vertex *parent)
{
  ...
  if (spftree->mtid != ISIS_MT_IPV4_UNICAST)
    mt_router_info =
        isis_tlvs_lookup_mt_router_info (lsp->tlvs, spftree->mtid);
          ...
          for (r = (struct isis_oldstyle_reach *)
               ...
               r; r = r->next)
            {
              ...
              if (! LSP_PSEUDO_ID (r->id) &&
                ...
                continue;
              if (! pseudo_lsp &&
                  ! memcmp (r->id, null_sysid, ISIS_SYS_ID_LEN))
                continue;
              dist = cost + r->metric;
              process_N (spftree,
      ...
      if (spftree->area->newmetric)
        {
          ...
          if (pseudo_lsp || spftree->mtid == ISIS_MT_IPV4_UNICAST)
            ...
            te_neighs =
                isis_lookup_mt_items (&lsp->tlvs->mt_reach, spftree->mtid);

          struct isis_extended_reach *er;
          for (er = te_neighs ? (struct isis_extended_reach *) te_neighs->head
                              : NULL;
               er; er = er->next)
            {
              /* C.2.6 a) */
  ...
  if (! pseudo_lsp && spftree->family == AF_INET)
    {
      ...
      if (spftree->mtid == ISIS_MT_IPV4_UNICAST)
        ...
        ipv4_reachs =
            isis_lookup_mt_items (&lsp->tlvs->mt_ip_reach, spftree->mtid);
  ...
  if (! pseudo_lsp && spftree->family == AF_INET6)
    {
      ...
      if (spftree->mtid == ISIS_MT_IPV4_UNICAST)
        ...
        ipv6_reachs =
            isis_lookup_mt_items (&lsp->tlvs->mt_ipv6_reach, spftree->mtid);
      ...
      for (r = ipv6_reachs ? (struct isis_ipv6_reach *) ipv6_reachs->head
           ...
           r; r = r->next)
        {
          dist = cost + r->metric;
          vtype =
              r->external ? VTYPE_IP6REACH_EXTERNAL : VTYPE_IP6REACH_INTERNAL;
          memset (&ip_info, 0, sizeof (ip_info));
              ...
              if (spftree->tree_id != SPFTREE_DSTSRC)
                {
                  ...
                  zlog_warn (
                      ...
                      srcdest2str (&ip_info.dest, r->subtlvs->source_prefix,
                                   buff, sizeof (buff)));
                  continue;
                }
...
adj_find (struct list *adj_list, const uint8_t *id, int level, uint16_t mtid,
          int family)
{
...
static int
isis_spf_preload_tent_ip_reach_cb (const struct prefix *prefix,
...
static void
isis_spf_preload_tent (struct isis_spftree *spftree, uint8_t *root_sysid,
  ...
  for (ALL_LIST_ELEMENTS_RO (spftree->sadj_list, node, sadj))
    {
      ...
      if (! LSP_PSEUDO_ID (sadj->id))
        ...
        {
          isis_spf_process_lsp (spftree, sadj->lsp, metric, 0, spftree->sysid,
                                parent);
        }
    }
}
...
static int
spf_adj_find_reverse_metric_cb (const uint8_t *id, uint32_t metric,
                                bool oldmetric,
 ...
 */
static void
spf_adj_get_reverse_metrics (struct isis_spftree *spftree)
{
  struct isis_spf_adj *sadj;
  ...
  for (ALL_LIST_ELEMENTS (spftree->sadj_list, node, nnode, sadj))
    {
      ...
      isis_lsp_iterate_is_reach (lsp_adj, spftree->mtid,
                                 spf_adj_find_reverse_metric_cb, &args);
...
static void
spf_adj_list_parse_tlv (struct isis_spftree *spftree, struct list *adj_list,
...
static void
spf_adj_list_parse_lsp (struct isis_spftree *spftree, struct list *adj_list,
      ...
      if (pseudo_lsp || spftree->mtid == ISIS_MT_IPV4_UNICAST)
        {
          ...
          for (struct isis_oldstyle_reach *reach =
            ...
            {
              spf_adj_list_parse_tlv (spftree, adj_list, reach->id,
...
static void
isis_spf_build_adj_list (struct isis_spftree *spftree, struct isis_lsp *lsp)
{
...
static void
init_spt (struct isis_spftree *spftree, int mtid)
{
...
static void
spf_path_process (struct isis_spftree *spftree, struct isis_vertex *vertex)
{
  struct isis_area *area = spftree->area;
              ...
              if (vertex->depth > 1)
                {
                  ...
                  if (listcount (vertex->Adj_N) > 1)
                    spftree->lfa.protection_counters.ecmp[priority] += 1;
                }
              break;
            }
...
static void
isis_spf_loop (struct isis_spftree *spftree, uint8_t *root_sysid)
{
  struct isis_vertex *vertex;
  struct isis_lsp *lsp;
  struct listnode *node;
...
struct isis_spftree *
isis_run_hopcount_spf (struct isis_area *area, uint8_t *sysid,
                       struct isis_spftree *spftree)
{
  if (! spftree)
    spftree = isis_spftree_new (area, &area->lspdb[IS_LEVEL_2 - 1], sysid,
...
void
isis_run_spf (struct isis_spftree *spftree)
{
  struct isis_lsp *root_lsp;
  struct isis_vertex *root_vertex;
  ...
  switch (spftree->tree_id)
    ...
    case SPFTREE_DSTSRC:
      ...
      break;
    case SPFTREE_COUNT:
      zlog_err ("%s should never be called with SPFTREE_COUNT as argument!",
                __func__);
      exit (1);
    }
...
static void
isis_run_spf_with_protection (struct isis_area *area,
                              struct isis_spftree *spftree)
{
...
void
isis_spf_verify_routes (struct isis_area *area, struct isis_spftree **trees,
                        int tree)
{
  if (area->is_type == IS_LEVEL_1)
    ...
    {
      isis_route_verify_merge (
          area, trees[0]->route_table, trees[0]->route_table_backup,
          trees[1]->route_table, trees[1]->route_table_backup, tree);
    }
}

void
isis_spf_invalidate_routes (struct isis_spftree *tree)
{
...
void
isis_spf_switchover_routes (struct isis_area *area,
                            struct isis_spftree **trees, int family,
                            union g_addr *nexthop_ip, ifindex_t ifindex,
                            int level)
{
  isis_route_switchover_nexthop (area, trees[level - 1]->route_table, family,
                                 nexthop_ip, ifindex);
}

static void
isis_run_spf_cb (struct event *thread)
{
  ...
  if (IS_DEBUG_SPF_EVENTS)
    zlog_debug ("ISIS-SPF (%s) L%d SPF needed, periodic SPF", area->area_tag,
                level);

  if (area->ip_circuits)
    {
      isis_run_spf_with_protection (area,
...
void
isis_spf_timer_free (void *run)
{
  XFREE (MTYPE_ISIS_SPF_RUN, run);
}

int
_isis_spf_schedule (struct isis_area *area, int level, const char *func,
...
static void
isis_print_paths (struct vty *vty, struct isis_vertex_queue *queue,
                  uint8_t *root_sysid)
{
  struct listnode *node;
  struct isis_vertex *vertex;
          ...
          if (vadj)
            {
              ...
              vty_out (vty, "%-20s %-9s ", print_sys_hostname (sadj->id),
...
void
isis_print_spftree (struct vty *vty, struct isis_spftree *spftree)
{
  ...
  switch (spftree->tree_id)
    ...
    case SPFTREE_COUNT:
      assert (
          ! "isis_print_spftree shouldn't be called with SPFTREE_COUNT as type");
      return;
    }
...
static void
show_isis_topology_common (struct vty *vty, int levels, struct isis *isis,
                           uint8_t algo)
{
#ifndef FABRICD
...
#ifndef FABRICD
static void
show_isis_flex_algo_display_eag (struct vty *vty, char *buf, int indent,
                                 struct admin_group *admin_group)
{
  if (admin_group_zero (admin_group))
    ...
    {
      vty_out (vty, "%s\n",
               admin_group_string (buf, ADMIN_GROUP_PRINT_MAX_SIZE, indent,
                                   admin_group));
...
static void
show_isis_flex_algo_common (struct vty *vty, struct isis *isis,
                            uint8_t algorithm)
{
...
static void
isis_print_route (struct ttable *tt, const struct prefix *prefix,
                  ...
                  for (int i = 0; i < nexthop->label_stack->num_labels; i++)
                    {
                      ...
                      label2str (nexthop->label_stack->label[i], 0, buf_label,
                                 sizeof (buf_label));
                      if (i != 0)
                        strlcat (buf_labels, "/", sizeof (buf_labels));
...
void
isis_print_routes (struct vty *vty, struct isis_spftree *spftree,
                   struct json_object **json, bool prefix_sid, bool backup)
{
  ...
  switch (spftree->tree_id)
    ...
    case SPFTREE_COUNT:
      assert (
          ! "isis_print_routes shouldn't be called with SPFTREE_COUNT as type");
      return;
    }
...
static void
show_isis_route_common (struct vty *vty, int levels, struct isis *isis,
                        bool prefix_sid, bool backup, uint8_t algo,
                        json_object **json)
{
...
static void
isis_print_frr_summary_line (struct ttable *tt, const char *protection,
                             uint32_t counters[SPF_PREFIX_PRIO_MAX])
{
  ...
  ttable_add_row (tt, "%s|%u|%u|%u|%u|%u", protection, critical, high, medium,
                  low, total);
}

static void
isis_print_frr_summary_line_coverage (struct ttable *tt,
...
static void
isis_print_frr_summary (struct vty *vty, struct isis_spftree *spftree)
{
  struct ttable *tt;
  char *table;
  const char *tree_id_text = NULL;
  ...
  switch (spftree->tree_id)
    ...
    case SPFTREE_COUNT:
      assert (
          ! "isis_print_frr_summary shouldn't be called with SPFTREE_COUNT as type");
      return;
    }
  ...
  tt = ttable_new (&ttable_styles[TTSTYLE_BLANK]);
  ttable_add_row (
      tt, "Protection \\ Priority|Critical|High    |Medium  |Low     |Total");
  tt->style.cell.rpad = 2;
  tt->style.corner = '+';
  ttable_restyle (tt);
  ttable_rowseps (tt, 0, BOTTOM, true, '-');
  ...
  for (int priority = SPF_PREFIX_PRIO_CRITICAL; priority < SPF_PREFIX_PRIO_MAX;
       priority++)
    {
      ...
      protectd[priority] =
          lfa[priority] + rlfa[priority] + tilfa[priority] + ecmp[priority];
  ...
  isis_print_frr_summary_line_coverage (tt, "Protection coverage", coverage,
                                        coverage_total);
...
static void
show_isis_frr_summary_common (struct vty *vty, int levels, struct isis *isis)
{
  struct listnode *node;
  struct isis_area *area;
          ...
          if (isis_area_ipv6_dstsrc_enabled (area))
            {
              isis_print_frr_summary (
                  vty, area->spftree[SPFTREE_DSTSRC][level - 1]);
            }
        }
    }
}

DEFUN (show_isis_frr_summary,
...
void
isis_spf_print (struct isis_spftree *spftree, struct vty *vty)
{
...
void
isis_spf_print_json (struct isis_spftree *spftree, struct json_object *json)
{
