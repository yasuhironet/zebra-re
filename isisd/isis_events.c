...
{
  if (IS_DEBUG_EVENTS)
    zlog_debug ("ISIS-Evt (%s) circuit %u on iface %s commencing on L%d",
...
circuit_resign_level (struct isis_circuit *circuit, int level)
{
  ...
  if (IS_DEBUG_EVENTS)
    zlog_debug ("ISIS-Evt (%s) circuit %u on iface %s resigning on L%d",
                circuit->area->area_tag, circuit->circuit_id,
                circuit->interface->name, level);
...
isis_circuit_is_type_set (struct isis_circuit *circuit, int newtype)
{
  ...
  if (IS_DEBUG_EVENTS)
    zlog_debug ("ISIS-Evt (%s) circuit type change %s -> %s",
                circuit->area->area_tag, circuit_t2string (circuit->is_type),
...
void
isis_event_dis_status_change (struct event *thread)
{
...
void
isis_event_auth_failure (char *area_tag, const char *error_string,
                         uint8_t *sysid)
{
  if (IS_DEBUG_EVENTS)
    zlog_debug ("ISIS-Evt (%s) Authentication failure %s from %pSY", area_tag,
                error_string, sysid);
