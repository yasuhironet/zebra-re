...
static const char *
isis_mtid2str_fake (uint16_t mtid)
{
...
void
isis_del_ext_subtlvs (struct isis_ext_subtlvs *ext)
{
...
copy_item_ext_subtlvs (struct isis_ext_subtlvs *exts, uint16_t mtid)
{
  ...
  for (ALL_LIST_ELEMENTS (exts->aslas, node, nnode, asla))
    {
      new_asla =
          XCALLOC (MTYPE_ISIS_SUBTLV, sizeof (struct isis_asla_subtlvs));
...
static void
format_item_asla_subtlvs (struct isis_asla_subtlvs *asla, struct sbuf *buf,
                          int indent)
{
  ...
  if (IS_SUBTLV (asla, EXT_ADM_GRP))
    {
      ...
      sbuf_push (buf, indent + 4, "Bit positions: %s\n",
                 admin_group_standard_print (
                     admin_group_buf, indent + 2 + strlen ("Admin Group: "),
                     asla->admin_group));
    }
      ...
      for (int j = 0; j < MAX_CLASS_TYPE; j += 2)
        {
          sbuf_push (buf, indent + 2,
                     ...
                     asla->unrsv_bw[j], j + 1, asla->unrsv_bw[j + 1]);
        }
    }
...
static void
format_item_ext_subtlvs (struct isis_ext_subtlvs *exts, struct sbuf *buf,
      ...
      if (json)
      ...
      else
        {
          ...
          sbuf_push (buf, indent + 2, "Bit positions: %s\n",
                     admin_group_standard_print (
                         admin_group_buf, indent + strlen ("Admin Group: "),
                         exts->adm_group));
        }
    }
      ...
      if (! json)
        {
          ...
          if (admin_group_buf[0] != '\0' &&
              (buf->pos + strlen (admin_group_buf) + SBUF_DEFAULT_SIZE / 2) <
                  buf->size)
            sbuf_push (buf, indent + 2, "Bit positions: %s\n",
                       admin_group_buf);
        }
  ...
  if (IS_SUBTLV (exts, EXT_LLRI))
    {
      if (json)
        {
          json_object_int_add (json, "link-local-id", exts->local_llri);
          json_object_int_add (json, "link-remote-id", exts->remote_llri);
        }
      else
        {
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "remote-iface-ip", aux_buf);
        }
      else
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "local-iface-ipv6", aux_buf);
        }
      else
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "remote-iface-ipv6", aux_buf);
        }
      else
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "max-bandwith-bytes-sec", aux_buf);
        }
      else
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "max-res-bandwith-bytes-sec", aux_buf);
        }
      else
  ...
  if (IS_SUBTLV (exts, EXT_RMT_AS))
    {
      if (json)
        {
          json_object_int_add (json, "inter-as-te-remote-as", exts->remote_as);
        }
      else
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "inter-as-te-remote-asbr-ip", aux_buf);
        }
      else
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "unidir-residual-band-bytes-sec",
                                  aux_buf);
        }
      else
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "unidir-available-band-bytes-sec",
                                  aux_buf);
        }
      else
        sbuf_push (buf, indent,
      ...
      if (json)
        {
          ...
          json_object_string_add (json, "unidir-utilized-band-bytes-sec",
                                  aux_buf);
        }
      else
      ...
      if (json)
        {
          ...
          for (adj = (struct isis_adj_sid *) exts->adj_sid.head; adj;
               adj = adj->next)
            {
              ...
              json_object_int_add (flags_json, "weight", adj->weight);
      ...
      if (json)
        {
          struct json_object *arr_adj_json, *flags_json;
          arr_adj_json = json_object_new_array ();
          json_object_object_add (json, "lan-adj-sid", arr_adj_json);
          for (lan = (struct isis_lan_adj_sid *) exts->adj_sid.head; lan;
               lan = lan->next)
            {
              ...
              json_object_int_add (flags_json, "weight", lan->weight);
              json_object_string_add (
                  flags_json, "flag-f",
...
static void
free_item_ext_subtlvs (struct isis_ext_subtlvs *exts)
{
  isis_del_ext_subtlvs (exts);
}

static int
pack_item_ext_subtlv_asla (struct isis_asla_subtlvs *asla, struct stream *s,
                           size_t *min_len)
{
  size_t subtlv_len;
...
static int
pack_item_ext_subtlvs (struct isis_ext_subtlvs *exts, struct stream *s,
                       size_t *min_len)
{
  struct isis_asla_subtlvs *asla;
  struct listnode *node;
...
static int
unpack_item_ext_subtlv_asla (uint16_t mtid, uint8_t subtlv_len,
...
static int
unpack_item_ext_subtlvs (uint16_t mtid, uint8_t len, struct stream *s,
                         struct sbuf *log, void *dest, int indent)
{
  uint8_t sum = 0;
  uint8_t subtlv_type;
  ...
  while (len > sum + 2)
    {
      ...
      switch (subtlv_type)
        ...
        case ISIS_SUBTLV_ASLA:
          if (unpack_item_ext_subtlv_asla (mtid, subtlv_len, s, log, indent,
                                           exts) < 0)
            {
...
static struct isis_item *
copy_item_prefix_sid (struct isis_item *i)
{
...
static void
format_item_prefix_sid (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
  ...
  if (json)
    {
      ...
      json_object_string_add (
          sr_json, "node", ((sid->flags & ISIS_PREFIX_SID_NODE) ? "yes" : ""));
      json_object_string_add (
          sr_json, "php",
...
static void
free_item_prefix_sid (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_SUBTLV, i);
}

static int
pack_item_prefix_sid (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_prefix_sid *sid = (struct isis_prefix_sid *) i;
...
static int
unpack_item_prefix_sid (uint16_t mtid, uint8_t len, struct stream *s,
                        struct sbuf *log, void *dest, int indent)
{
  ...
  format_item_prefix_sid (mtid, (struct isis_item *) &sid, log, NULL,
                          indent + 2);
  append_item (&subtlvs->prefix_sids,
               copy_item_prefix_sid ((struct isis_item *) &sid));
  return 0;
}
...
copy_subtlv_ipv6_source_prefix (struct prefix_ipv6 *p)
{
  if (! p)
    return NULL;
...
static void
format_subtlv_ipv6_source_prefix (struct prefix_ipv6 *p, struct sbuf *buf,
                                  struct json_object *json, int indent)
{
  if (! p)
    return;
...
static int
pack_subtlv_ipv6_source_prefix (struct prefix_ipv6 *p, struct stream *s)
{
  if (! p)
    return 0;
...
static int
unpack_subtlv_ipv6_source_prefix (enum isis_tlv_context context,
                                  uint8_t tlv_type, uint8_t tlv_len,
...
static void copy_items (enum isis_tlv_context context, enum isis_tlv_type type,
                        struct isis_item_list *src,
                        struct isis_item_list *dest);
static void format_items_ (uint16_t mtid, enum isis_tlv_context context,
                           enum isis_tlv_type type,
                           struct isis_item_list *items, struct sbuf *buf,
                           struct json_object *json, int indent);
...
static struct isis_subtlvs *
copy_subtlvs (struct isis_subtlvs *subtlvs)
{
  if (! subtlvs)
    return NULL;
...
static void
format_subtlvs (struct isis_subtlvs *subtlvs, struct sbuf *buf,
                struct json_object *json, int indent)
{
...
static void
isis_free_subtlvs (struct isis_subtlvs *subtlvs)
{
  if (! subtlvs)
    return;
...
static int
pack_subtlvs (struct isis_subtlvs *subtlvs, struct stream *s)
{
  int rv;
...
static struct isis_item *
copy_item_area_address (struct isis_item *i)
{
...
static void
format_item_area_address (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
                          struct json_object *json, int indent)
{
  struct isis_area_address *addr = (struct isis_area_address *) i;
...
static void
free_item_area_address (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_area_address (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_area_address *addr = (struct isis_area_address *) i;
...
static int
unpack_item_area_address (uint16_t mtid, uint8_t len, struct stream *s,
                          struct sbuf *log, void *dest, int indent)
{
  ...
  format_item_area_address (ISIS_MT_IPV4_UNICAST, (struct isis_item *) rv, log,
                            NULL, indent + 2);
...
static struct isis_item *
copy_item_oldstyle_reach (struct isis_item *i)
{
...
static void
format_item_oldstyle_reach (uint16_t mtid, struct isis_item *i,
                            struct sbuf *buf, struct json_object *json,
                            int indent)
{
  struct isis_oldstyle_reach *r = (struct isis_oldstyle_reach *) i;
...
static void
free_item_oldstyle_reach (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_oldstyle_reach (struct isis_item *i, struct stream *s,
                          size_t *min_len)
{
  struct isis_oldstyle_reach *r = (struct isis_oldstyle_reach *) i;
...
static int
unpack_item_oldstyle_reach (uint16_t mtid, uint8_t len, struct stream *s,
                            struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static struct isis_item *
copy_item_lan_neighbor (struct isis_item *i)
{
...
static void
format_item_lan_neighbor (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
                          struct json_object *json, int indent)
{
  struct isis_lan_neighbor *n = (struct isis_lan_neighbor *) i;
  char sys_id[ISO_SYSID_STRLEN];
...
static void
free_item_lan_neighbor (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_lan_neighbor (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_lan_neighbor *n = (struct isis_lan_neighbor *) i;
...
static int
unpack_item_lan_neighbor (uint16_t mtid, uint8_t len, struct stream *s,
                          struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
  ...
  format_item_lan_neighbor (mtid, (struct isis_item *) rv, log, NULL,
                            indent + 2);
...
static struct isis_item *
copy_item_lsp_entry (struct isis_item *i)
{
...
static void
format_item_lsp_entry (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
                       struct json_object *json, int indent)
{
  struct isis_lsp_entry *e = (struct isis_lsp_entry *) i;
  char sys_id[ISO_SYSID_STRLEN];
...
static void
free_item_lsp_entry (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_lsp_entry (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_lsp_entry *e = (struct isis_lsp_entry *) i;
...
static int
unpack_item_lsp_entry (uint16_t mtid, uint8_t len, struct stream *s,
                       struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static struct isis_item *
copy_item_extended_reach (struct isis_item *i)
{
...
static void
format_item_extended_reach (uint16_t mtid, struct isis_item *i,
                            struct sbuf *buf, struct json_object *json,
                            int indent)
{
  struct isis_extended_reach *r = (struct isis_extended_reach *) i;
  ...
  if (json)
    {
      ...
      json_object_string_add (reach_json, "mt-id",
                              (mtid == ISIS_MT_IPV4_UNICAST) ? "Extended"
                                                             : "MT");
      ...
      if (r->subtlvs)
        format_item_ext_subtlvs (r->subtlvs, NULL, json, indent + 2, mtid);
    }
  else
    {
      sbuf_push (buf, indent, "%s Reachability: %s (Metric: %u)",
                 (mtid == ISIS_MT_IPV4_UNICAST) ? "Extended" : "MT", sys_id,
                 r->metric);
      if (mtid != ISIS_MT_IPV4_UNICAST)
...
static void
free_item_extended_reach (struct isis_item *i)
{
...
static int
pack_item_extended_reach (struct isis_item *i, struct stream *s,
                          size_t *min_len)
{
  struct isis_extended_reach *r = (struct isis_extended_reach *) i;
  size_t len;
...
static int
unpack_item_extended_reach (uint16_t mtid, uint8_t len, struct stream *s,
                            struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static struct isis_item *
copy_item_oldstyle_ip_reach (struct isis_item *i)
{
...
static void
format_item_oldstyle_ip_reach (uint16_t mtid, struct isis_item *i,
                               struct sbuf *buf, struct json_object *json,
                               int indent)
{
  struct isis_oldstyle_ip_reach *r = (struct isis_oldstyle_ip_reach *) i;
...
static void
free_item_oldstyle_ip_reach (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_oldstyle_ip_reach (struct isis_item *i, struct stream *s,
                             size_t *min_len)
{
  struct isis_oldstyle_ip_reach *r = (struct isis_oldstyle_ip_reach *) i;
...
static int
unpack_item_oldstyle_ip_reach (uint16_t mtid, uint8_t len, struct stream *s,
                               struct sbuf *log, void *dest, int indent)
{
...
static void
copy_tlv_protocols_supported (struct isis_protocols_supported *src,
...
static void
format_tlv_protocols_supported (struct isis_protocols_supported *p,
                                struct sbuf *buf, struct json_object *json,
                                int indent)
{
  ...
  if (json)
    {
      ...
      json_object_object_add (json, "protocols-supported", protocol_json);
...
static void
free_tlv_protocols_supported (struct isis_protocols_supported *p)
{
  XFREE (MTYPE_ISIS_TLV, p->protocols);
}

static int
pack_tlv_protocols_supported (struct isis_protocols_supported *p,
                              struct stream *s)
{
  if (! p || ! p->count || ! p->protocols)
    return 0;
...
static int
unpack_tlv_protocols_supported (enum isis_tlv_context context,
                                uint8_t tlv_type, uint8_t tlv_len,
                                struct stream *s, struct sbuf *log, void *dest,
                                int indent)
{
...
static struct isis_item *
copy_item_ipv4_address (struct isis_item *i)
{
...
static void
format_item_ipv4_address (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
                          struct json_object *json, int indent)
{
  struct isis_ipv4_address *a = (struct isis_ipv4_address *) i;
...
static void
free_item_ipv4_address (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_ipv4_address (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_ipv4_address *a = (struct isis_ipv4_address *) i;
...
static int
unpack_item_ipv4_address (uint16_t mtid, uint8_t len, struct stream *s,
                          struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
  ...
  format_item_ipv4_address (mtid, (struct isis_item *) rv, log, NULL,
                            indent + 2);
...
static struct isis_item *
copy_item_ipv6_address (struct isis_item *i)
{
...
static void
format_item_ipv6_address (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
                          struct json_object *json, int indent)
{
  struct isis_ipv6_address *a = (struct isis_ipv6_address *) i;
...
static void
free_item_ipv6_address (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_ipv6_address (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_ipv6_address *a = (struct isis_ipv6_address *) i;
...
static int
unpack_item_ipv6_address (uint16_t mtid, uint8_t len, struct stream *s,
                          struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
  ...
  format_item_ipv6_address (mtid, (struct isis_item *) rv, log, NULL,
                            indent + 2);
...
static struct isis_item *
copy_item_global_ipv6_address (struct isis_item *i)
{
  struct isis_ipv6_address *a = (struct isis_ipv6_address *) i;
  struct isis_ipv6_address *rv = XCALLOC (MTYPE_ISIS_TLV, sizeof (*rv));
...
static void
format_item_global_ipv6_address (uint16_t mtid, struct isis_item *i,
                                 struct sbuf *buf, struct json_object *json,
                                 int indent)
{
  struct isis_ipv6_address *a = (struct isis_ipv6_address *) i;
...
static void
free_item_global_ipv6_address (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_global_ipv6_address (struct isis_item *i, struct stream *s,
                               size_t *min_len)
{
  struct isis_ipv6_address *a = (struct isis_ipv6_address *) i;
...
static int
unpack_item_global_ipv6_address (uint16_t mtid, uint8_t len, struct stream *s,
                                 struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static struct isis_item *
copy_item_mt_router_info (struct isis_item *i)
{
...
static void
format_item_mt_router_info (uint16_t mtid, struct isis_item *i,
                            struct sbuf *buf, struct json_object *json,
                            int indent)
{
  struct isis_mt_router_info *info = (struct isis_mt_router_info *) i;
  ...
  if (json)
    {
      ...
      json_object_string_add (mt_json, "overload",
                              info->overload ? "true" : "false");
      json_object_string_add (mt_json, "attached",
...
static void
free_item_mt_router_info (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_mt_router_info (struct isis_item *i, struct stream *s,
                          size_t *min_len)
{
  struct isis_mt_router_info *info = (struct isis_mt_router_info *) i;
...
static int
unpack_item_mt_router_info (uint16_t mtid, uint8_t len, struct stream *s,
                            struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
copy_tlv_te_router_id (const struct in_addr *id)
{
  if (! id)
    return NULL;
...
static void
format_tlv_te_router_id (const struct in_addr *id, struct sbuf *buf,
                         struct json_object *json, int indent)
{
  if (! id)
    return;
...
static void
free_tlv_te_router_id (struct in_addr *id)
{
  XFREE (MTYPE_ISIS_TLV, id);
}

static int
pack_tlv_te_router_id (const struct in_addr *id, struct stream *s)
{
  if (! id)
    return 0;
...
static int
unpack_tlv_te_router_id (enum isis_tlv_context context, uint8_t tlv_type,
...
static struct isis_item *
copy_item_extended_ip_reach (struct isis_item *i)
{
...
static void
format_item_extended_ip_reach (uint16_t mtid, struct isis_item *i,
                               struct sbuf *buf, struct json_object *json,
                               int indent)
{
  struct isis_extended_ip_reach *r = (struct isis_extended_ip_reach *) i;
  ...
  if (json)
    {
      ...
      json_object_string_add (
          json, "mt-id", (mtid == ISIS_MT_IPV4_UNICAST) ? "Extended" : "MT");
      json_object_string_add (
          json, "ip-reach",
          prefix2str (&r->prefix, prefixbuf, sizeof (prefixbuf)));
      ...
      if (mtid != ISIS_MT_IPV4_UNICAST)
        json_object_string_add (json, "mt-name", isis_mtid2str (mtid));
      if (r->subtlvs)
        {
...
static void
free_item_extended_ip_reach (struct isis_item *i)
{
...
static int
pack_item_extended_ip_reach (struct isis_item *i, struct stream *s,
                             size_t *min_len)
{
  struct isis_extended_ip_reach *r = (struct isis_extended_ip_reach *) i;
  uint8_t control;
...
static int
unpack_item_extended_ip_reach (uint16_t mtid, uint8_t len, struct stream *s,
                               struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
  ...
  if (control & ISIS_EXTENDED_IP_REACH_SUBTLV)
    {
      ...
      if (unpack_tlvs (ISIS_CONTEXT_SUBTLV_IP_REACH, subtlv_len, s, log,
...
static char *
copy_tlv_dynamic_hostname (const char *hostname)
{
  if (! hostname)
    return NULL;
...
static void
format_tlv_dynamic_hostname (const char *hostname, struct sbuf *buf,
                             struct json_object *json, int indent)
{
  if (! hostname)
    return;
...
static void
free_tlv_dynamic_hostname (char *hostname)
{
  XFREE (MTYPE_ISIS_TLV, hostname);
}

static int
pack_tlv_dynamic_hostname (const char *hostname, struct stream *s)
{
  if (! hostname)
    return 0;
...
static int
unpack_tlv_dynamic_hostname (enum isis_tlv_context context, uint8_t tlv_type,
...
copy_tlv_te_router_id_ipv6 (const struct in6_addr *id)
{
  if (! id)
    return NULL;
...
static void
format_tlv_te_router_id_ipv6 (const struct in6_addr *id, struct sbuf *buf,
                              struct json_object *json, int indent)
{
  if (! id)
    return;
...
static void
free_tlv_te_router_id_ipv6 (struct in6_addr *id)
{
  XFREE (MTYPE_ISIS_TLV, id);
}

static int
pack_tlv_te_router_id_ipv6 (const struct in6_addr *id, struct stream *s)
{
  if (! id)
    return 0;
...
static int
unpack_tlv_te_router_id_ipv6 (enum isis_tlv_context context, uint8_t tlv_type,
                              uint8_t tlv_len, struct stream *s,
                              struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
  ...
  format_tlv_te_router_id_ipv6 (tlvs->te_router_id_ipv6, log, NULL,
                                indent + 2);
  return 0;
}
...
static struct isis_spine_leaf *
copy_tlv_spine_leaf (const struct isis_spine_leaf *spine_leaf)
{
  if (! spine_leaf)
    return NULL;
...
static void
format_tlv_spine_leaf (const struct isis_spine_leaf *spine_leaf,
  ...
  if (json)
    {
      ...
      json_object_object_add (json, "spine-leaf-extension", spine_json);
...
static void
free_tlv_spine_leaf (struct isis_spine_leaf *spine_leaf)
{
...
static int
pack_tlv_spine_leaf (const struct isis_spine_leaf *spine_leaf,
                     struct stream *s)
{
  if (! spine_leaf)
    return 0;
...
static int
unpack_tlv_spine_leaf (enum isis_tlv_context context, uint8_t tlv_type,
                       uint8_t tlv_len, struct stream *s, struct sbuf *log,
                       void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
const char *
isis_threeway_state_name (enum isis_threeway_state state)
{
  switch (state)
    {
...
static struct isis_threeway_adj *
copy_tlv_threeway_adj (const struct isis_threeway_adj *threeway_adj)
{
  if (! threeway_adj)
    return NULL;
  ...
  if (json)
    {
      ...
      json_object_string_add (three_json, "state-name",
...
static void
free_tlv_threeway_adj (struct isis_threeway_adj *threeway_adj)
{
  XFREE (MTYPE_ISIS_TLV, threeway_adj);
}

static int
pack_tlv_threeway_adj (const struct isis_threeway_adj *threeway_adj,
                       struct stream *s)
{
  if (! threeway_adj)
    return 0;
...
static int
unpack_tlv_threeway_adj (enum isis_tlv_context context, uint8_t tlv_type,
                         uint8_t tlv_len, struct stream *s, struct sbuf *log,
                         void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static struct isis_item *
copy_item_ipv6_reach (struct isis_item *i)
{
...
static void
format_item_ipv6_reach (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
                        struct json_object *json, int indent)
{
  struct isis_ipv6_reach *r = (struct isis_ipv6_reach *) i;
  char prefixbuf[PREFIX2STR_BUFFER];
  ...
  if (json)
    {
      ...
      json_object_string_add (
          ...
          prefix2str (&r->prefix, prefixbuf, sizeof (prefixbuf)));
      json_object_int_add (reach_json, "metric", r->metric);
      json_object_string_add (reach_json, "down", r->down ? "yes" : "");
...
static void
free_item_ipv6_reach (struct isis_item *i)
{
...
static int
pack_item_ipv6_reach (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_ipv6_reach *r = (struct isis_ipv6_reach *) i;
  uint8_t control;
...
static int
unpack_item_ipv6_reach (uint16_t mtid, uint8_t len, struct stream *s,
                        struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
  ...
  if (memcmp (&orig_prefix, &rv->prefix.prefix, sizeof (orig_prefix)))
    sbuf_push (log, indent + 2, "WARNING: Prefix had hostbits set.\n");
  format_item_ipv6_reach (mtid, (struct isis_item *) rv, log, NULL,
                          indent + 2);
  ...
  if (control & ISIS_IPV6_REACH_SUBTLV)
    {
      ...
      if (unpack_tlvs (ISIS_CONTEXT_SUBTLV_IPV6_REACH, subtlv_len, s, log,
                       rv->subtlvs, indent + 4, &unpacked_known_tlvs))
        {
          goto out;
        }
...
static struct isis_router_cap *
copy_tlv_router_cap (const struct isis_router_cap *router_cap)
{
...
static void
format_tlv_router_cap_json (const struct isis_router_cap *router_cap,
                            struct json_object *json)
{
  ...
  json_object_string_add (cap_json, "flag-d",
                          router_cap->flags & ISIS_ROUTER_CAP_FLAG_D ? "1"
                                                                     : "0");
  json_object_string_add (cap_json, "flag-s",
                          router_cap->flags & ISIS_ROUTER_CAP_FLAG_S ? "1"
                                                                     : "0");
  ...
  if (router_cap->algo[0] != SR_ALGORITHM_UNSET)
    {
      ...
      json_object_object_add (json, "segment-routing-algorithm", alg_json);
      for (int i = 0; i < SR_ALGORITHM_COUNT; i++)
...
static void
format_tlv_router_cap (const struct isis_router_cap *router_cap,
                       struct sbuf *buf, int indent)
{
  ...
  if (router_cap->srgb.range_size != 0)
    sbuf_push (
        ...
        IS_SR_IPV4 (&router_cap->srgb) ? "1" : "0",
        IS_SR_IPV6 (&router_cap->srgb) ? "1" : "0",
        router_cap->srgb.lower_bound, router_cap->srgb.range_size);
  ...
  for (int i = 0; i < SR_ALGORITHM_COUNT; i++)
    {
      ...
      sbuf_push (buf, 0, "%s\n",
                 admin_group_string (admin_group_buf,
                                     ADMIN_GROUP_PRINT_MAX_SIZE, indent2,
                                     admin_group));
...
static void
free_tlv_router_cap (struct isis_router_cap *router_cap)
{
  if (! router_cap)
    return;
...
static size_t
isis_router_cap_tlv_size (const struct isis_router_cap *router_cap)
{
...
static int
pack_tlv_router_cap (const struct isis_router_cap *router_cap,
                     struct stream *s)
{
...
static int
unpack_tlv_router_cap (enum isis_tlv_context context, uint8_t tlv_type,
                       uint8_t tlv_len, struct stream *s, struct sbuf *log,
                       void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static struct isis_item *
copy_item_auth (struct isis_item *i)
{
...
static void
format_item_auth (uint16_t mtid, struct isis_item *i, struct sbuf *buf,
                  struct json_object *json, int indent)
{
  struct isis_auth *auth = (struct isis_auth *) i;
  char obuf[768];
...
static void
free_item_auth (struct isis_item *i)
{
  XFREE (MTYPE_ISIS_TLV, i);
}

static int
pack_item_auth (struct isis_item *i, struct stream *s, size_t *min_len)
{
  struct isis_auth *auth = (struct isis_auth *) i;
...
static int
unpack_item_auth (uint16_t mtid, uint8_t len, struct stream *s,
                  struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static struct isis_purge_originator *
copy_tlv_purge_originator (struct isis_purge_originator *poi)
{
  if (! poi)
    return NULL;
...
static void
format_tlv_purge_originator (struct isis_purge_originator *poi,
                             struct sbuf *buf, struct json_object *json,
                             int indent)
{
...
static void
free_tlv_purge_originator (struct isis_purge_originator *poi)
{
  XFREE (MTYPE_ISIS_TLV, poi);
}

static int
pack_tlv_purge_originator (struct isis_purge_originator *poi, struct stream *s)
{
  if (! poi)
    return 0;
...
static int
unpack_tlv_purge_originator (enum isis_tlv_context context, uint8_t tlv_type,
                             uint8_t tlv_len, struct stream *s,
                             struct sbuf *log, void *dest, int indent)
{
  struct isis_tlvs *tlvs = dest;
...
static void
init_item_list (struct isis_item_list *items)
{
  items->head = NULL;
...
static struct isis_item *
copy_item (enum isis_tlv_context context, enum isis_tlv_type type,
           struct isis_item *item)
{
...
static void
copy_items (enum isis_tlv_context context, enum isis_tlv_type type,
...
static void
format_item (uint16_t mtid, enum isis_tlv_context context,
             enum isis_tlv_type type, struct isis_item *i, struct sbuf *buf,
             struct json_object *json, int indent)
{
  const struct tlv_ops *ops = tlv_table[context][type];
...
static void
format_items_ (uint16_t mtid, enum isis_tlv_context context,
               enum isis_tlv_type type, struct isis_item_list *items,
               struct sbuf *buf, struct json_object *json, int indent)
{
  struct isis_item *i;
...
static void
free_item (enum isis_tlv_context tlv_context, enum isis_tlv_type tlv_type,
           struct isis_item *item)
{
...
static void
free_items (enum isis_tlv_context context, enum isis_tlv_type type,
            struct isis_item_list *items)
{
  struct isis_item *item, *next_item;
...
static int
pack_item (enum isis_tlv_context context, enum isis_tlv_type type,
...
static void
add_item_to_fragment (struct isis_item *i, const struct pack_order_entry *pe,
...
static int
pack_items_ (uint16_t mtid, enum isis_tlv_context context,
             enum isis_tlv_type type, struct isis_item_list *items,
             struct stream *s, struct isis_tlvs **fragment_tlvs,
             const struct pack_order_entry *pe,
             struct isis_tlvs *(*new_fragment) (struct list *l),
...
top:
  ...
  for (item = item ? item : items->head; item; item = item->next)
    {
      rv = pack_item (context, type, item, s, &min_len, fragment_tlvs, pe,
                      mtid);
      if (rv)
        goto too_long;
...
static void
append_item (struct isis_item_list *dest, struct isis_item *item)
{
  *dest->tail = item;
...
static void
delete_item (struct isis_item_list *dest, struct isis_item *del)
{
...
static struct isis_item *
last_item (struct isis_item_list *list)
{
  return container_of (list->tail, struct isis_item, next);
}

static int
unpack_item (uint16_t mtid, enum isis_tlv_context context, uint8_t tlv_type,
             uint8_t len, struct stream *s, struct sbuf *log, void *dest,
             int indent)
{
...
static int
unpack_tlv_with_items (enum isis_tlv_context context, uint8_t tlv_type,
                       uint8_t tlv_len, struct stream *s, struct sbuf *log,
                       void *dest, int indent)
{
  size_t tlv_start;
  ...
  while (tlv_pos < (size_t) tlv_len)
    {
      rv = unpack_item (mtid, context, tlv_type, tlv_len - tlv_pos, s, log,
                        dest, indent + 2);
      if (rv)
        return rv;
...
static int
isis_mt_item_list_cmp (const struct isis_item_list *a,
...
RB_PROTOTYPE (isis_mt_item_list, isis_item_list, mt_tree,
              isis_mt_item_list_cmp);
RB_GENERATE (isis_mt_item_list, isis_item_list, mt_tree,
             isis_mt_item_list_cmp);

struct isis_item_list *
isis_get_mt_items (struct isis_mt_item_list *m, uint16_t mtid)
{
...
struct isis_item_list *
isis_lookup_mt_items (struct isis_mt_item_list *m, uint16_t mtid)
{
...
static void
free_mt_items (enum isis_tlv_context context, enum isis_tlv_type type,
               struct isis_mt_item_list *m)
{
...
static void
format_mt_items (enum isis_tlv_context context, enum isis_tlv_type type,
...
static int
pack_mt_items (enum isis_tlv_context context, enum isis_tlv_type type,
  ...
  RB_FOREACH (n, isis_mt_item_list, m)
    {
      ...
      rv = pack_items_ (n->mtid, context, type, n, s, fragment_tlvs, pe,
...
static void
copy_mt_items (enum isis_tlv_context context, enum isis_tlv_type type,
...
struct isis_tlvs *
isis_copy_tlvs (struct isis_tlvs *tlvs)
{
  ...
  copy_mt_items (ISIS_CONTEXT_LSP, ISIS_TLV_MT_IP_REACH, &tlvs->mt_ip_reach,
                 &rv->mt_ip_reach);
...
static void
format_tlvs (struct isis_tlvs *tlvs, struct sbuf *buf,
             struct json_object *json, int indent)
{
  format_tlv_protocols_supported (&tlvs->protocols_supported, buf, json,
                                  indent);

  format_items (ISIS_CONTEXT_LSP, ISIS_TLV_AUTH, &tlvs->isis_auth, buf, json,
                indent);
  ...
  format_items (ISIS_CONTEXT_LSP, ISIS_TLV_LAN_NEIGHBORS, &tlvs->lan_neighbor,
                buf, json, indent);

  format_items (ISIS_CONTEXT_LSP, ISIS_TLV_LSP_ENTRY, &tlvs->lsp_entries, buf,
                json, indent);
  ...
  format_mt_items (ISIS_CONTEXT_LSP, ISIS_TLV_MT_REACH, &tlvs->mt_reach, buf,
                   json, indent);
  ...
  format_items (ISIS_CONTEXT_LSP, ISIS_TLV_IPV4_ADDRESS, &tlvs->ipv4_address,
                buf, json, indent);

  format_items (ISIS_CONTEXT_LSP, ISIS_TLV_IPV6_ADDRESS, &tlvs->ipv6_address,
                buf, json, indent);
  ...
  format_mt_items (ISIS_CONTEXT_LSP, ISIS_TLV_MT_IP_REACH, &tlvs->mt_ip_reach,
                   buf, json, indent);

  format_items (ISIS_CONTEXT_LSP, ISIS_TLV_IPV6_REACH, &tlvs->ipv6_reach, buf,
                json, indent);
...
const char *
isis_format_tlvs (struct isis_tlvs *tlvs, struct json_object *json)
{
  if (json)
    {
...
void
isis_free_tlvs (struct isis_tlvs *tlvs)
{
  if (! tlvs)
    return;
...
static void
safe_auth_md5 (struct stream *s, uint16_t *checksum, uint16_t *rem_lifetime)
{
...
static void
restore_auth_md5 (struct stream *s, uint16_t checksum, uint16_t rem_lifetime)
{
...
static void
update_auth_hmac_md5 (struct isis_auth *auth, struct stream *s, bool is_lsp)
{
  uint8_t digest[16];
  ...
  uint8_t *result =
      (uint8_t *) HMAC (EVP_md5 (), auth->passwd, auth->plength,
                        STREAM_DATA (s), stream_get_endp (s), NULL, NULL);

  memcpy (digest, result, 16);
#elif CRYPTO_INTERNAL
  hmac_md5 (STREAM_DATA (s), stream_get_endp (s), auth->passwd, auth->plength,
            digest);
#endif
...
static void
update_auth (struct isis_tlvs *tlvs, struct stream *s, bool is_lsp)
{
...
static int
handle_pack_entry (const struct pack_order_entry *pe, struct isis_tlvs *tlvs,
  ...
  if (pe->how_to_pack == ISIS_ITEMS)
    {
      ...
      rv = pack_items (pe->context, pe->type, l, stream, fragment_tlvs, pe,
                       new_fragment, new_fragment_arg);
    }
  else
    {
...
static int
pack_tlvs (struct isis_tlvs *tlvs, struct stream *stream,
           struct isis_tlvs *fragment_tlvs,
           struct isis_tlvs *(*new_fragment) (struct list *l),
           struct list *new_fragment_arg)
{
  ...
  if (fragment_tlvs)
    {
      copy_items (ISIS_CONTEXT_LSP, ISIS_TLV_AREA_ADDRESSES,
                  &tlvs->area_addresses, &fragment_tlvs->area_addresses);
    }
  ...
  if (tlvs->mt_router_info_empty)
  ...
  else
    {
      rv = pack_items (ISIS_CONTEXT_LSP, ISIS_TLV_MT_ROUTER_INFO,
                       &tlvs->mt_router_info, stream, NULL, NULL, NULL, NULL);
      if (rv)
        return rv;
      if (fragment_tlvs)
        {
...
int
isis_pack_tlvs (struct isis_tlvs *tlvs, struct stream *stream,
...
static struct isis_tlvs *
new_fragment (struct list *l)
{
...
struct list *
isis_fragment_tlvs (struct isis_tlvs *tlvs, size_t size)
{
...
static int
unpack_tlv_unknown (enum isis_tlv_context context, uint8_t tlv_type,
                    uint8_t tlv_len, struct stream *s, struct sbuf *log,
                    int indent)
{
  stream_forward_getp (s, tlv_len);
...
static int
unpack_tlv (enum isis_tlv_context context, size_t avail_len,
            struct stream *stream, struct sbuf *log, void *dest, int indent,
  ...
  if (ops && ops->unpack)
    {
      ...
      return ops->unpack (context, tlv_type, tlv_len, stream, log, dest,
                          indent + 2);
    }
...
static int
unpack_tlvs (enum isis_tlv_context context, size_t avail_len,
             struct stream *stream, struct sbuf *log, void *dest, int indent,
             bool *unpacked_known_tlvs)
{
  int rv;
...
int
isis_unpack_tlvs (size_t avail_len, struct stream *stream,
...
void
isis_tlvs_add_auth (struct isis_tlvs *tlvs, struct isis_passwd *passwd)
{
  free_items (ISIS_CONTEXT_LSP, ISIS_TLV_AUTH, &tlvs->isis_auth);
...
void
isis_tlvs_add_area_addresses (struct isis_tlvs *tlvs, struct list *addresses)
{
  struct listnode *node;
...
void
isis_tlvs_add_lan_neighbors (struct isis_tlvs *tlvs, struct list *neighbors)
{
  struct listnode *node;
  uint8_t *snpa;
...
void
isis_tlvs_set_protocols_supported (struct isis_tlvs *tlvs,
...
void
isis_tlvs_add_mt_router_info (struct isis_tlvs *tlvs, uint16_t mtid,
...
void
isis_tlvs_add_ipv4_address (struct isis_tlvs *tlvs, struct in_addr *addr)
{
...
void
isis_tlvs_add_ipv4_addresses (struct isis_tlvs *tlvs, struct list *addresses)
{
  struct listnode *node;
...
void
isis_tlvs_add_ipv6_addresses (struct isis_tlvs *tlvs, struct list *addresses)
{
  struct listnode *node;
...
void
isis_tlvs_add_global_ipv6_addresses (struct isis_tlvs *tlvs,
...
static bool
auth_validator_cleartxt (struct isis_passwd *passwd, struct stream *stream,
                         struct isis_auth *auth, bool is_lsp)
{
...
static bool
auth_validator_hmac_md5 (struct isis_passwd *passwd, struct stream *stream,
                         struct isis_auth *auth, bool is_lsp)
{
  uint8_t digest[16];
  uint16_t checksum;
...
int
isis_tlvs_auth_is_valid (struct isis_tlvs *tlvs, struct isis_passwd *passwd,
...
bool
isis_tlvs_area_addresses_match (struct isis_tlvs *tlvs, struct list *addresses)
{
...
static void
tlvs_area_addresses_to_adj (struct isis_tlvs *tlvs, struct isis_adjacency *adj,
                            bool *changed)
{
...
static void
tlvs_protocols_supported_to_adj (struct isis_tlvs *tlvs,
...
static void
tlvs_ipv4_addresses_to_adj (struct isis_tlvs *tlvs, struct isis_adjacency *adj,
                            bool *changed)
{
...
static void
tlvs_ipv6_addresses_to_adj (struct isis_tlvs *tlvs, struct isis_adjacency *adj,
                            bool *changed)
{
  ...
  if (adj->ll_ipv6_count != tlvs->ipv6_address.count)
    {
      ...
      *changed = true;
      ...
      adj->ll_ipv6_addrs =
          XREALLOC (MTYPE_ISIS_ADJACENCY_INFO, adj->ll_ipv6_addrs,
...
static void
tlvs_global_ipv6_addresses_to_adj (struct isis_tlvs *tlvs,
                                   struct isis_adjacency *adj, bool *changed)
{
...
void
isis_tlvs_to_adj (struct isis_tlvs *tlvs, struct isis_adjacency *adj,
                  bool *changed)
{
  *changed = false;
...
bool
isis_tlvs_own_snpa_found (struct isis_tlvs *tlvs, uint8_t *snpa)
{
...
void
isis_tlvs_add_lsp_entry (struct isis_tlvs *tlvs, struct isis_lsp *lsp)
{
...
void
isis_tlvs_add_csnp_entries (struct isis_tlvs *tlvs, uint8_t *start_id,
...
void
isis_tlvs_set_dynamic_hostname (struct isis_tlvs *tlvs, const char *hostname)
{
...
#ifndef FABRICD
void
isis_tlvs_set_router_capability_fad (struct isis_tlvs *tlvs,
...
int
isis_tlvs_sr_algo_count (const struct isis_router_cap *cap)
{
  int count = 0;
...
void
isis_tlvs_set_te_router_id (struct isis_tlvs *tlvs, const struct in_addr *id)
{
...
void
isis_tlvs_set_te_router_id_ipv6 (struct isis_tlvs *tlvs,
...
void
isis_tlvs_add_oldstyle_ip_reach (struct isis_tlvs *tlvs,
...
void
isis_tlvs_add_adj_sid (struct isis_ext_subtlvs *exts, struct isis_adj_sid *adj)
{
  append_item (&exts->adj_sid, (struct isis_item *) adj);
  SET_SUBTLV (exts, EXT_ADJ_SID);
}
...
void
isis_tlvs_del_adj_sid (struct isis_ext_subtlvs *exts, struct isis_adj_sid *adj)
{
...
void
isis_tlvs_add_lan_adj_sid (struct isis_ext_subtlvs *exts,
                           struct isis_lan_adj_sid *lan)
{
  append_item (&exts->lan_sid, (struct isis_item *) lan);
  SET_SUBTLV (exts, EXT_LAN_ADJ_SID);
...
void
isis_tlvs_del_lan_adj_sid (struct isis_ext_subtlvs *exts,
                           struct isis_lan_adj_sid *lan)
{
...
void
isis_tlvs_del_asla_flex_algo (struct isis_ext_subtlvs *ext,
                              struct isis_asla_subtlvs *asla)
{
...
void
isis_tlvs_free_asla (struct isis_ext_subtlvs *ext, uint8_t standard_apps)
{
  struct isis_asla_subtlvs *asla;
  struct listnode *node;
...
void
isis_tlvs_add_extended_ip_reach (struct isis_tlvs *tlvs,
...
void
isis_tlvs_add_ipv6_reach (struct isis_tlvs *tlvs, uint16_t mtid,
...
void
isis_tlvs_add_ipv6_dstsrc_reach (struct isis_tlvs *tlvs, uint16_t mtid,
                                 struct prefix_ipv6 *dest,
                                 struct prefix_ipv6 *src, uint32_t metric)
{
...
void
isis_tlvs_add_oldstyle_reach (struct isis_tlvs *tlvs, uint8_t *id,
                              uint8_t metric)
{
...
void
isis_tlvs_add_extended_reach (struct isis_tlvs *tlvs, uint16_t mtid,
...
void
isis_tlvs_add_threeway_adj (struct isis_tlvs *tlvs,
...
void
isis_tlvs_add_spine_leaf (struct isis_tlvs *tlvs, uint8_t tier, bool has_tier,
                          bool is_leaf, bool is_spine, bool is_backup)
{
...
void
isis_tlvs_set_purge_originator (struct isis_tlvs *tlvs,
                                ...
                                const uint8_t *sender)
{
  ...
  tlvs->purge_originator =
      XCALLOC (MTYPE_ISIS_TLV, sizeof (*tlvs->purge_originator));
