  ...
  switch (sra->adj->circuit->circ_type)
    ...
    case CIRCUIT_T_P2P:
      return yang_data_new_bool (
          args->xpath, sra->u.adj_sid->flags & EXT_SUBTLV_LINK_ADJ_SID_BFLG);
    }
    ...
    struct nb_cb_get_elem_args *args)
{
  ...
  switch (sra->adj->circuit->circ_type)
    {
    case CIRCUIT_T_BROADCAST:
      return yang_data_new_uint8 (args->xpath, sra->u.ladj_sid->family);
    case CIRCUIT_T_P2P:
    ...
    struct nb_cb_get_elem_args *args)
{
  ...
  switch (sra->adj->circuit->circ_type)
    {
    case CIRCUIT_T_BROADCAST:
      return yang_data_new_uint8 (args->xpath, sra->u.ladj_sid->weight);
    case CIRCUIT_T_P2P:
      /* LAN adjacency SID is not published with circuit type P2P */
      return NULL;
    }
    ...
    struct nb_cb_get_elem_args *args)
{
  ...
  switch (sra->adj->circuit->circ_type)
    ...
    case CIRCUIT_T_BROADCAST:
      return yang_data_new_bool (
          args->xpath, sra->u.ladj_sid->flags & EXT_SUBTLV_LINK_ADJ_SID_BFLG);
    case CIRCUIT_T_P2P:
      /* LAN adjacency SID is not published with circuit type P2P */
      return NULL;
    }
    ...
    struct nb_cb_get_elem_args *args)
{
  ...
  switch (circuit->circ_type)
    ...
    case CIRCUIT_T_BROADCAST:
      for (int level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++)
        {
          for (ALL_LIST_ELEMENTS_RO (circuit->u.bc.adjdb[level - 1], node,
                                     adj))
            total++;
        }
      break;
    ...
    struct nb_cb_get_elem_args *args)
{
  ...
  return yang_data_new_uint32 (args->xpath, circuit->max_area_addr_mismatches);
}
