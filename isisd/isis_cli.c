            ...
            VRF_CMD_HELP_STR)
{
  ...
  if (! yang_dnode_existsf (
          ...
          "/frr-isisd:isis/instance[area-tag='%s'][vrf='%s']", tag, vrf_name))
    {
  ...
  return nb_cli_apply_changes_clear_pending (
      vty, "/frr-isisd:isis/instance[area-tag='%s'][vrf='%s']", tag, vrf_name);
}

void
cli_show_router_isis (struct vty *vty, const struct lyd_node *dnode,
                      bool show_defaults)
{
  const char *vrf = NULL;
...
void
cli_show_router_isis_end (struct vty *vty, const struct lyd_node *dnode)
{
  vty_out (vty, "exit\n");
}
...
ALIAS_HIDDEN (ip_router_isis,
              ip_router_isis_vrf_cmd,
...
ALIAS_HIDDEN (ip6_router_isis,
              ip6_router_isis_vrf_cmd,
            ...
            "Routing process tag\n")
{
  ...
  dnode = yang_dnode_getf (vty->candidate_config->dnode, "%s/frr-isisd:isis",
                           VTY_CURR_XPATH);
  if (! dnode)
    return CMD_SUCCESS;
...
ALIAS_HIDDEN (no_ip_router_isis,
              no_ip_router_isis_vrf_cmd,
...
void
cli_show_ip_isis_ipv4 (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
...
void
cli_show_ip_isis_ipv6 (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_ip_isis_bfd_monitoring (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
...
void
cli_show_isis_area_address (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
void
cli_show_isis_is_type (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
...
void
cli_show_isis_dynamic_hostname (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_isis_overload (struct vty *vty, const struct lyd_node *dnode,
                        bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
 ...
 */
DEFPY_YANG (set_overload_bit_on_startup,
            set_overload_bit_on_startup_cmd,
...
DEFPY_YANG (no_set_overload_bit_on_startup,
            no_set_overload_bit_on_startup_cmd,
...
void
cli_show_isis_overload_on_startup (struct vty *vty,
                                   const struct lyd_node *dnode,
                                   bool show_defaults)
{
 ...
 */
DEFPY_YANG (advertise_high_metrics,
            advertise_high_metrics_cmd,
...
void
cli_show_advertise_high_metrics (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
  if (yang_dnode_get_bool (dnode, NULL))
 ...
 */
DEFPY_YANG (attached_bit_send,
            attached_bit_send_cmd,
...
void
cli_show_isis_attached_send (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
 ...
 */
DEFPY_YANG (attached_bit_receive_ignore,
            attached_bit_receive_ignore_cmd,
...
void
cli_show_isis_attached_receive (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_isis_metric_style (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
            ...
            "Send and check PDUs on receiving\n")
{
  ...
  nb_cli_enqueue_change (vty, "./area-password/password-type", NB_OP_MODIFY,
                         pwd_type);
...
void
cli_show_isis_area_pwd (struct vty *vty, const struct lyd_node *dnode,
                        bool show_defaults)
{
  const char *snp;
            ...
            "Send and check PDUs on receiving\n")
{
  ...
  nb_cli_enqueue_change (vty, "./domain-password/password-type", NB_OP_MODIFY,
                         pwd_type);
...
void
cli_show_isis_domain_pwd (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
  const char *snp;
...
DEFPY_YANG (no_lsp_gen_interval,
 ...
 */
DEFPY_YANG (lsp_refresh_interval,
...
{
  if (! level || strmatch (level, "level-1"))
    nb_cli_enqueue_change (vty, "./lsp/timers/level-1/refresh-interval",
                           NB_OP_MODIFY, val_str);
  if (! level || strmatch (level, "level-2"))
    nb_cli_enqueue_change (vty, "./lsp/timers/level-2/refresh-interval",
                           NB_OP_MODIFY, val_str);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_lsp_refresh_interval,
...
{
  if (! level || strmatch (level, "level-1"))
    nb_cli_enqueue_change (vty, "./lsp/timers/level-1/maximum-lifetime",
                           NB_OP_MODIFY, val_str);
  if (! level || strmatch (level, "level-2"))
    nb_cli_enqueue_change (vty, "./lsp/timers/level-2/maximum-lifetime",
                           NB_OP_MODIFY, val_str);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_max_lsp_lifetime,
...
void
cli_show_isis_lsp_timers (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
...
void
cli_show_isis_lsp_mtu (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
 ...
 */
DEFPY_YANG (advertise_passive_only,
            advertise_passive_only_cmd,
...
void
cli_show_advertise_passive_only (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
{
  if (! level || strmatch (level, "level-1"))
    nb_cli_enqueue_change (vty, "./spf/minimum-interval/level-1", NB_OP_MODIFY,
                           val_str);
  if (! level || strmatch (level, "level-2"))
    nb_cli_enqueue_change (vty, "./spf/minimum-interval/level-2", NB_OP_MODIFY,
                           val_str);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_spf_interval,
            no_spf_interval_cmd,
...
void
cli_show_isis_spf_min_interval (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
...
void
cli_show_isis_spf_ietf_backoff (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  vty_out (
      vty,
 ...
 */
DEFPY_YANG (spf_prefix_priority,
            spf_prefix_priority_cmd,
            ...
            "Access-list name\n")
{
  ...
  snprintf (xpath, XPATH_MAXLEN, "./spf/prefix-priorities/%s/access-list-name",
            priority);
...
DEFPY_YANG (no_spf_prefix_priority,
            no_spf_prefix_priority_cmd,
...
void
cli_show_isis_spf_prefix_priority (struct vty *vty,
                                   const struct lyd_node *dnode,
                                   bool show_defaults)
{
 ...
 */
DEFPY_YANG (area_purge_originator,
...
void
cli_show_isis_purge_origin (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
 ...
 */
DEFPY_YANG (isis_admin_group_send_zero,
            isis_admin_group_send_zero_cmd,
...
void
cli_show_isis_admin_group_send_zero (struct vty *vty,
                                     const struct lyd_node *dnode,
                                     bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
 ...
 */
DEFPY_HIDDEN (isis_asla_legacy_flag,
              isis_asla_legacy_flag_cmd,
              "[no] asla-legacy-flag",
              NO_STR
...
void
cli_show_isis_asla_legacy_flag (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_isis_mpls_te (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
 ...
 */
DEFPY_YANG (isis_mpls_te_router_addr,
...
{
  nb_cli_enqueue_change (vty, "./mpls-te/router-address", NB_OP_MODIFY,
                         router_address_str);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_isis_mpls_te_router_addr,
...
void
cli_show_isis_mpls_te_router_addr (struct vty *vty,
                                   const struct lyd_node *dnode,
                                   bool show_defaults)
{
 ...
 */
DEFPY_YANG (isis_mpls_te_router_addr_v6,
            isis_mpls_te_router_addr_v6_cmd,
...
DEFPY_YANG (no_isis_mpls_te_router_addr_v6,
            no_isis_mpls_te_router_addr_v6_cmd,
...
void
cli_show_isis_mpls_te_router_addr_ipv6 (struct vty *vty,
                                        const struct lyd_node *dnode,
                                        bool show_defaults)
{
...
DEFPY_YANG (isis_mpls_te_inter_as,
 ...
 */
DEFPY_YANG (isis_mpls_te_export,
            isis_mpls_te_export_cmd,
            "mpls-te export",
            MPLS_TE_STR
...
DEFPY_YANG (no_isis_mpls_te_export,
            no_isis_mpls_te_export_cmd,
            "no mpls-te export",
            NO_STR
            MPLS_TE_STR
...
void
cli_show_isis_mpls_te_export (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
 ...
 */
DEFPY_YANG (isis_default_originate,
            ...
            "Pointer to route-map entries\n")
{
  ...
  return nb_cli_apply_changes (
      vty, "./default-information-originate/%s[level='%s']", ip, level);
}

static void
vty_print_def_origin (struct vty *vty, const struct lyd_node *dnode,
...
void
cli_show_isis_def_origin_ipv4 (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
...
void
cli_show_isis_def_origin_ipv6 (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
  const char *level = yang_dnode_get_string (dnode, "./level");
 ...
 */
DEFPY_YANG (isis_redistribute,
            isis_redistribute_cmd,
            "[no] redistribute <ipv4$ip "
            PROTO_IP_REDIST_STR
            "$proto|ipv6$ip "
            PROTO_IP6_REDIST_STR
            "$proto> <level-1|level-2>$level"
            ...
            "Pointer to route-map entries\n")
{
  ...
  return nb_cli_apply_changes (
      vty, "./redistribute/%s[protocol='%s'][level='%s']", ip, proto, level);
}

static void
vty_print_redistribute (struct vty *vty, const struct lyd_node *dnode,
...
void
cli_show_isis_redistribute_ipv4 (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
  vty_print_redistribute (vty, dnode, show_defaults, "ipv4");
}
void
cli_show_isis_redistribute_ipv6 (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
...
void
cli_show_isis_mt_ipv4_multicast (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
...
void
cli_show_isis_mt_ipv4_mgmt (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
void
cli_show_isis_mt_ipv6_unicast (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
...
void
cli_show_isis_mt_ipv6_multicast (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
...
void
cli_show_isis_mt_ipv6_mgmt (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
void
cli_show_isis_mt_ipv6_dstsrc (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
...
void
cli_show_isis_sr_enabled (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
            ...
            "The upper bound of the local block (block size may not exceed 65535)\n")
{
  ...
  nb_cli_enqueue_change (
      vty, "./segment-routing/label-blocks/srlb/lower-bound", NB_OP_MODIFY,
      lb_lower_bound ? lb_lower_bound_str : NULL);
  nb_cli_enqueue_change (
      vty, "./segment-routing/label-blocks/srlb/upper-bound", NB_OP_MODIFY,
...
void
cli_show_isis_label_blocks (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
{
  nb_cli_enqueue_change (vty, "./segment-routing/msd/node-msd", NB_OP_MODIFY,
                         msd_str);
...
DEFPY_YANG (no_isis_sr_node_msd,
            ...
            "Maximum number of label that can be stack (1-16)\n")
{
  nb_cli_enqueue_change (vty, "./segment-routing/msd/node-msd", NB_OP_DESTROY,
                         NULL);
...
void
cli_show_isis_node_msd (struct vty *vty, const struct lyd_node *dnode,
                        bool show_defaults)
{
...
void
cli_show_isis_prefix_sid (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
  const char *prefix;
...
void
cli_show_isis_prefix_sid_algorithm (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
  const char *prefix;
...
void
cli_show_isis_frr_lfa_priority_limit (struct vty *vty,
                                      const struct lyd_node *dnode,
                                      bool show_defaults)
{
...
void
cli_show_isis_frr_lfa_tiebreaker (struct vty *vty,
                                  const struct lyd_node *dnode,
                                  bool show_defaults)
{
  ...
  if (! level || strmatch (level, "level-1"))
    {
      if (no)
        {
          nb_cli_enqueue_change (vty,
                                 "./fast-reroute/level-1/lfa/load-sharing",
                                 NB_OP_MODIFY, "true");
        }
      else
        {
  ...
  if (! level || strmatch (level, "level-2"))
    {
      if (no)
        {
          nb_cli_enqueue_change (vty,
                                 "./fast-reroute/level-2/lfa/load-sharing",
                                 NB_OP_MODIFY, "true");
        }
      else
        {
...
void
cli_show_isis_frr_lfa_load_sharing (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
  if (yang_dnode_get_bool (dnode, NULL))
...
/*
 * XPath:
 * /frr-isisd:isis/instance/fast-reroute/level-{1,2}/remote-lfa/prefix-list
 */
            ...
            "Enable router ID filtering for level-2 only\n")
{
  if (! level || strmatch (level, "level-1"))
    nb_cli_enqueue_change (vty,
                           "./fast-reroute/level-1/remote-lfa/prefix-list",
                           NB_OP_MODIFY, plist);
  if (! level || strmatch (level, "level-2"))
    nb_cli_enqueue_change (vty,
                           "./fast-reroute/level-2/remote-lfa/prefix-list",
                           NB_OP_MODIFY, plist);
...
void
cli_show_isis_frr_remote_lfa_plist (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
...
void
cli_show_ip_isis_passive (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_ip_isis_password (struct vty *vty, const struct lyd_node *dnode,
                           bool show_defaults)
{
...
void
cli_show_ip_isis_metric (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
  const char *l1 = yang_dnode_get_string (dnode, "./level-1");
  const char *l2 = yang_dnode_get_string (dnode, "./level-2");
 ...
 */
DEFPY_YANG (isis_hello_interval,
...
DEFPY_YANG (no_isis_hello_interval,
...
void
cli_show_ip_isis_hello_interval (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
  const char *l1 = yang_dnode_get_string (dnode, "./level-1");
  const char *l2 = yang_dnode_get_string (dnode, "./level-2");
 ...
 */
DEFPY_YANG (isis_hello_multiplier,
...
DEFPY_YANG (no_isis_hello_multiplier,
...
void
cli_show_ip_isis_hello_multi (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
  const char *l1 = yang_dnode_get_string (dnode, "./level-1");
  const char *l2 = yang_dnode_get_string (dnode, "./level-2");
...
{
  nb_cli_enqueue_change (vty, "./frr-isisd:isis/disable-three-way-handshake",
...
void
cli_show_ip_isis_threeway_shake (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
  if (yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_ip_isis_hello_padding (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
...
{
  if (! level || strmatch (level, "level-1"))
    nb_cli_enqueue_change (vty, "./frr-isisd:isis/csnp-interval/level-1",
                           NB_OP_MODIFY, intv_str);
  if (! level || strmatch (level, "level-2"))
    nb_cli_enqueue_change (vty, "./frr-isisd:isis/csnp-interval/level-2",
                           NB_OP_MODIFY, intv_str);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_csnp_interval,
...
void
cli_show_ip_isis_csnp_interval (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  const char *l1 = yang_dnode_get_string (dnode, "./level-1");
  const char *l2 = yang_dnode_get_string (dnode, "./level-2");
...
{
  if (! level || strmatch (level, "level-1"))
    nb_cli_enqueue_change (vty, "./frr-isisd:isis/psnp-interval/level-1",
                           NB_OP_MODIFY, intv_str);
  if (! level || strmatch (level, "level-2"))
...
void
cli_show_ip_isis_psnp_interval (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  const char *l1 = yang_dnode_get_string (dnode, "./level-1");
  const char *l2 = yang_dnode_get_string (dnode, "./level-2");
            ...
            "IPv6 dst-src topology\n")
{
  ...
  if (strmatch (topology, "ipv4-unicast"))
    return nb_cli_apply_changes (vty,
                                 "./frr-isisd:isis/multi-topology/standard");
  else
    return nb_cli_apply_changes (vty, "./frr-isisd:isis/multi-topology/%s",
                                 topology);
}

void
cli_show_ip_isis_mt_standard (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_ip_isis_mt_ipv4_multicast (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
...
void
cli_show_ip_isis_mt_ipv4_mgmt (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_ip_isis_mt_ipv6_unicast (struct vty *vty,
                                  const struct lyd_node *dnode,
                                  bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
...
void
cli_show_ip_isis_mt_ipv6_multicast (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
...
void
cli_show_ip_isis_mt_ipv6_mgmt (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_ip_isis_mt_ipv6_dstsrc (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
DEFPY_YANG (no_isis_circuit_type,
...
void
cli_show_ip_isis_circ_type (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
...
static int
ag_change (struct vty *vty, int argc, struct cmd_token **argv,
...
static int
ag_iter_cb (const struct lyd_node *dnode, void *arg)
{
...
void
cli_show_ip_isis_network_type (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
...
void
cli_show_ip_isis_priority (struct vty *vty, const struct lyd_node *dnode,
                           bool show_defaults)
{
  const char *l1 = yang_dnode_get_string (dnode, "./level-1");
  const char *l2 = yang_dnode_get_string (dnode, "./level-2");
 ...
 */
void
cli_show_ip_isis_frr (struct vty *vty, const struct lyd_node *dnode,
                      bool show_defaults)
{
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-isisd:isis/fast-reroute/level-{1,2}/lfa/enable
 */
DEFPY (isis_lfa,
       isis_lfa_cmd,
  ...
  if (! level || strmatch (level, "level-1"))
    {
      if (no)
        {
          nb_cli_enqueue_change (
              vty, "./frr-isisd:isis/fast-reroute/level-1/lfa/enable",
              NB_OP_MODIFY, "false");
        }
      else
        {
  ...
  if (! level || strmatch (level, "level-2"))
    {
      if (no)
        {
          nb_cli_enqueue_change (
              vty, "./frr-isisd:isis/fast-reroute/level-2/lfa/enable",
              NB_OP_MODIFY, "false");
        }
      else
        {
...
DEFPY (isis_lfa_exclude_interface,
       isis_lfa_exclude_interface_cmd,
...
void
cli_show_frr_lfa_exclude_interface (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
  ...
  if (! level || strmatch (level, "level-1"))
    {
      if (no)
        {
          nb_cli_enqueue_change (
              vty, "./frr-isisd:isis/fast-reroute/level-1/remote-lfa/enable",
              NB_OP_MODIFY, "false");
        }
      else
        {
  ...
  if (! level || strmatch (level, "level-2"))
    {
      if (no)
        {
          nb_cli_enqueue_change (
              vty, "./frr-isisd:isis/fast-reroute/level-2/remote-lfa/enable",
              NB_OP_MODIFY, "false");
        }
      else
        {
 ...
 */
DEFPY (isis_remote_lfa_max_metric,
...
void
cli_show_frr_remote_lfa_max_metric (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
...
/*
 * XPath:
 * /frr-interface:lib/interface/frr-isisd:isis/fast-reroute/level-{1,2}/ti-lfa/enable
 */
DEFPY (isis_ti_lfa,
       isis_ti_lfa_cmd,
  ...
  if (! level || strmatch (level, "level-1"))
    {
      if (no)
        {
          nb_cli_enqueue_change (
              vty, "./frr-isisd:isis/fast-reroute/level-1/ti-lfa/enable",
              NB_OP_MODIFY, "false");
          nb_cli_enqueue_change (
              vty,
  ...
  if (! level || strmatch (level, "level-2"))
    {
      if (no)
        {
          nb_cli_enqueue_change (
              vty, "./frr-isisd:isis/fast-reroute/level-2/ti-lfa/enable",
              NB_OP_MODIFY, "false");
          nb_cli_enqueue_change (
              vty,
...
void
cli_show_isis_log_adjacency (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_isis_log_pdu_drops (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
 ...
 */
DEFPY (isis_mpls_ldp_sync,
       isis_mpls_ldp_sync_cmd,
       "mpls ldp-sync",
       MPLS_STR
       MPLS_LDP_SYNC_STR)
{
...
DEFPY (no_isis_mpls_ldp_sync,
       no_isis_mpls_ldp_sync_cmd,
       "no mpls ldp-sync",
       NO_STR
       MPLS_STR
...
void
cli_show_isis_mpls_ldp_sync (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
  vty_out (vty, " mpls ldp-sync\n");
}

DEFPY (isis_mpls_ldp_sync_holddown,
       isis_mpls_ldp_sync_holddown_cmd,
...
DEFPY (no_isis_mpls_ldp_sync_holddown,
...
void
cli_show_isis_mpls_ldp_sync_holddown (struct vty *vty,
                                      const struct lyd_node *dnode,
                                      bool show_defaults)
{
 ...
 */
DEFPY (isis_mpls_if_ldp_sync,
       isis_mpls_if_ldp_sync_cmd,
...
void
cli_show_isis_mpls_if_ldp_sync (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
DEFPY (isis_mpls_if_ldp_sync_holddown,
       isis_mpls_if_ldp_sync_holddown_cmd,
       ...
       "Time in seconds\n")
{
  ...
  nb_cli_enqueue_change (vty, "./frr-isisd:isis/mpls/holddown", NB_OP_MODIFY,
                         holddown_str);
       ...
       "Time in seconds\n")
{
  ...
  nb_cli_enqueue_change (vty, "./frr-isisd:isis/mpls/holddown", NB_OP_DESTROY,
                         NULL);
...
void
cli_show_isis_mpls_if_ldp_sync_holddown (struct vty *vty,
                                         const struct lyd_node *dnode,
                                         bool show_defaults)
{
                 ...
                 "Flexible Algorithm Number\n")
{
  ...
  snprintf (xpath, sizeof (xpath), "%s/flex-algos/flex-algo[flex-algo='%ld']",
            VTY_CURR_XPATH, algorithm);

  nb_cli_enqueue_change (vty, ".", NB_OP_CREATE, NULL);

  ret = nb_cli_apply_changes (vty, "./flex-algos/flex-algo[flex-algo='%ld']",
                              algorithm);
  if (ret == CMD_SUCCESS)
...
DEFPY_YANG (advertise_definition,
            advertise_definition_cmd,
...
DEFPY_YANG (affinity_include_any,
            affinity_include_any_cmd,
...
DEFPY_YANG (affinity_include_all,
            affinity_include_all_cmd,
...
DEFPY_YANG (affinity_exclude_any,
            affinity_exclude_any_cmd,
...
{
  nb_cli_enqueue_change (vty, "./dplane-ip", no ? NB_OP_DESTROY : NB_OP_CREATE,
                         NULL);
  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (metric_type,
            metric_type_cmd,
            ...
            "Use Traffic Engineering metric\n")
{
  ...
  nb_cli_enqueue_change (vty, "./metric-type",
                         no ? NB_OP_DESTROY : NB_OP_MODIFY, no ? NULL : type);
  return nb_cli_apply_changes (vty, NULL);
}
...
void
cli_show_isis_flex_algo (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
  uint32_t algorithm;
...
void
cli_show_isis_flex_algo_end (struct vty *vty, const struct lyd_node *dnode)
{
  vty_out (vty, " !\n");
}
