 ...
 */
static inline int
sr_prefix_sid_cfg_compare (const struct sr_prefix_cfg *a,
...
isis_sr_find_srgb (struct lspdb_head *lspdb, const uint8_t *sysid)
{
  struct isis_lsp *lsp;
 ...
 */
mpls_label_t
sr_prefix_in_label (struct isis_area *area, struct isis_prefix_sid *psid,
                    bool local)
{
  /*
  ...
  if (psid->value >= (area->srdb.config.srgb_upper_bound -
                      area->srdb.config.srgb_lower_bound + 1))
    {
      flog_warn (EC_ISIS_SID_OVERFLOW,
                 "%s: SID index %u falls outside local SRGB range", __func__,
                 psid->value);
      return MPLS_INVALID_LABEL;
    }
 ...
 */
mpls_label_t
sr_prefix_out_label (struct lspdb_head *lspdb, int family,
                     struct isis_prefix_sid *psid, const uint8_t *nh_sysid,
                     bool last_hop)
{
  ...
  if (psid->value >= nh_srgb->range_size)
    {
      flog_warn (EC_ISIS_SID_OVERFLOW,
                 "%s: SID index %u falls outside remote SRGB range", __func__,
                 psid->value);
      return MPLS_INVALID_LABEL;
    }
 ...
 */
static bool
sr_prefix_is_node_sid (const struct interface *ifp,
 ...
 */
int
isis_sr_cfg_srgb_update (struct isis_area *area, uint32_t lower_bound,
                         uint32_t upper_bound)
{
  struct isis_sr_db *srdb = &area->srdb;

  sr_debug ("ISIS-Sr (%s): Update SRGB with new range [%u/%u]", area->area_tag,
 ...
 */
int
isis_sr_cfg_srlb_update (struct isis_area *area, uint32_t lower_bound,
                         uint32_t upper_bound)
{
  struct isis_sr_db *srdb = &area->srdb;
  ...
  sr_debug ("ISIS-Sr (%s): Update SRLB with new range [%u/%u]", area->area_tag,
            lower_bound, upper_bound);
 ...
 */
void
isis_sr_cfg_prefix_del (struct sr_prefix_cfg *pcfg)
{
  struct isis_area *area = pcfg->area;

  sr_debug ("ISIS-Sr (%s): Delete local Prefix-SID %pFX %s %u", area->area_tag,
            &pcfg->prefix,
 ...
 */
void
isis_sr_prefix_cfg2subtlv (const struct sr_prefix_cfg *pcfg, bool external,
 ...
 */
void
isis_area_delete_backup_adj_sids (struct isis_area *area, int level)
{
  struct sr_adjacency *sra;
  struct listnode *node, *nnode;

  for (ALL_LIST_ELEMENTS (area->srdb.adj_sids, node, nnode, sra))
    if (sra->type == ISIS_SR_LAN_BACKUP && (sra->adj->level & level))
      sr_adj_sid_del (sra);
}
 ...
 */
static int
sr_local_block_init (struct isis_area *area)
{
  struct isis_sr_db *srdb = &area->srdb;
  ...
  if (((srlb->end - srlb->start + 1) % SRLB_BLOCK_SIZE) != 0)
    srlb->max_block++;
  srlb->used_mark =
      XCALLOC (MTYPE_ISIS_SR_INFO, srlb->max_block * SRLB_BLOCK_SIZE);
  srlb->active = true;
 ...
 */
static void
sr_local_block_delete (struct isis_area *area)
{
  struct isis_sr_db *srdb = &area->srdb;
  struct sr_local_block *srlb = &srdb->srlb;
  ...
  sr_debug ("ISIS-Sr (%s): Remove SRLB [%u/%u]", area->area_tag, srlb->start,
            srlb->end);
 ...
 */
static mpls_label_t
sr_local_block_request_label (struct sr_local_block *srlb)
{
  mpls_label_t label;
  uint32_t index;
  uint32_t pos;
 ...
 */
static int
sr_local_block_release_label (struct sr_local_block *srlb, mpls_label_t label)
{
  uint32_t index;
  uint32_t pos;
 ...
 */
void
sr_adj_sid_add_single (struct isis_adjacency *adj, int family, bool backup,
                       struct list *nexthops)
{
  ...
  switch (family)
    ...
    case AF_INET6:
      ...
      break;
    default:
      flog_err (EC_LIB_DEVELOPMENT, "%s: unexpected address-family: %u",
                __func__, family);
      exit (1);
    }
 ...
 */
static void
sr_adj_sid_add (struct isis_adjacency *adj, int family)
{
  sr_adj_sid_add_single (adj, family, false, NULL);
}

static void
sr_adj_sid_update (struct sr_adjacency *sra, struct sr_local_block *srlb)
{
 ...
 */
static void
sr_adj_sid_del (struct sr_adjacency *sra)
{
  struct isis_circuit *circuit = sra->adj->circuit;
  struct isis_area *area = circuit->area;
 ...
 */
struct sr_adjacency *
isis_sr_adj_sid_find (struct isis_adjacency *adj, int family,
 ...
 */
static int
sr_adj_state_change (struct isis_adjacency *adj)
{
  struct sr_adjacency *sra;
  struct listnode *node, *nnode;
 ...
 */
static int
sr_adj_ip_enabled (struct isis_adjacency *adj, int family, bool global)
{
 ...
 */
static int
sr_adj_ip_disabled (struct isis_adjacency *adj, int family, bool global)
{
  struct sr_adjacency *sra;
  struct listnode *node, *nnode;
 ...
 */
static int
sr_if_new_hook (struct interface *ifp)
{
 ...
 */
static void
show_node (struct vty *vty, struct isis_area *area, int level, uint8_t algo)
{
  struct isis_lsp *lsp;
  struct ttable *tt;
  char buf[128];
  ...
  frr_each (lspdb, &area->lspdb[level - 1], lsp)
    {
      ...
      ttable_add_row (tt, "%pSY|%u - %u|%u - %u|%s|%u", lsp->hdr.lsp_id,
                      ...
                      cap->srlb.lower_bound + cap->srlb.range_size - 1, buf,
                      cap->msd);
    }
 ...
 */
int
isis_sr_start (struct isis_area *area)
{
  struct isis_sr_db *srdb = &area->srdb;
 ...
 */
void
isis_sr_stop (struct isis_area *area)
{
  struct isis_sr_db *srdb = &area->srdb;
  struct sr_adjacency *sra;
  struct listnode *node, *nnode;
 ...
 */
void
isis_sr_area_init (struct isis_area *area)
{
  struct isis_sr_db *srdb = &area->srdb;
 ...
 */
void
isis_sr_area_term (struct isis_area *area)
{
  struct isis_sr_db *srdb = &area->srdb;
