...
static int
ack_lsp (struct isis_lsp_hdr *hdr, struct isis_circuit *circuit, int level)
{
  unsigned long lenp;
  int retval;
  uint16_t length;
  ...
  if (retval != ISIS_OK)
    flog_err (EC_ISIS_PACKET, "ISIS-Upd (%s): Send L%d LSP PSNP on %s failed",
...
process_p2p_hello (struct iih_info *iih)
{
      ...
      if (tw_adj->state > ISIS_THREEWAY_DOWN)
        {
          if (IS_DEBUG_ADJ_PACKETS)
            {
              zlog_debug (
                  "ISIS-Adj (%s): Rcvd P2P IIH from (%s) with invalid three-way state: %d",
      ...
      if (tw_adj->neighbor_set &&
           ...
           tw_adj->neighbor_circuit_id != (uint32_t) iih->circuit->idx))
        {
          ...
          if (IS_DEBUG_ADJ_PACKETS)
            {
              zlog_debug (
                  "ISIS-Adj (%s): Rcvd P2P IIH from (%s) which lists IS/Circuit different from us as neighbor.",
                  iih->circuit->area->area_tag, iih->circuit->interface->name);
            }
  ...
  changed |=
      tlvs_to_adj_mt_set (iih->tlvs, iih->v4_usable, iih->v6_usable, adj);
          ...
          switch (iih->circ_type)
            ...
            case IS_LEVEL_1_AND_2:
              if (adj->adj_state != ISIS_ADJ_UP ||
                ...
                {
                  isis_adj_process_threeway (adj, tw_adj, ISIS_ADJ_LEVEL1);
                }
              break;
            case IS_LEVEL_2:
              if (adj->adj_state != ISIS_ADJ_UP)
          ...
          switch (iih->circ_type)
            ...
            case IS_LEVEL_2:
              if (adj->adj_state != ISIS_ADJ_UP ||
                  adj->adj_usage == ISIS_ADJ_LEVEL2)
                {
                  isis_adj_process_threeway (adj, tw_adj, ISIS_ADJ_LEVEL2);
                }
              else if ((adj->adj_usage == ISIS_ADJ_LEVEL1) ||
            ...
            case IS_LEVEL_1_AND_2:
              if (adj->adj_state != ISIS_ADJ_UP ||
                  adj->adj_usage == ISIS_ADJ_LEVEL1AND2)
                {
                  isis_adj_process_threeway (adj, tw_adj, ISIS_ADJ_LEVEL1AND2);
                }
              else if ((adj->adj_usage == ISIS_ADJ_LEVEL1) ||
                       (adj->adj_usage == ISIS_ADJ_LEVEL2))
                {
  ...
  if (IS_DEBUG_ADJ_PACKETS)
    {
      zlog_debug (
          ...
          iih->circuit->area->area_tag, iih->circuit->interface->name,
          circuit_t2string (iih->circuit->is_type), iih->circuit->circuit_id,
          iih->pdu_len);
    }
...
process_lan_hello (struct iih_info *iih)
{
  ...
  adj =
      isis_adj_lookup (iih->sys_id, iih->circuit->u.bc.adjdb[iih->level - 1]);
      ...
      if (! adj)
        {
          /* Do as in 8.4.2.5 */
          adj =
              isis_new_adj (iih->sys_id, iih->ssnpa, iih->level, iih->circuit);
        }
      else
        {
          if (iih->ssnpa)
  ...
  if (adj->dis_record[iih->level - 1].dis == ISIS_IS_DIS)
    {
      uint8_t *dis = (iih->level == 1) ? iih->circuit->u.bc.l1_desig_is
                                       : iih->circuit->u.bc.l2_desig_is;

      if (memcmp (dis, iih->dis, ISIS_SYS_ID_LEN + 1))
        {
          event_add_event (master, isis_event_dis_status_change, iih->circuit,
                           0, NULL);
...
static void
update_rej_adj_count (struct isis_circuit *circuit)
{
...
static int
process_hello (uint8_t pdu_type, struct isis_circuit *circuit, uint8_t *ssnpa)
{
...
#ifndef FABRICD
          isis_notif_reject_adjacency (circuit, "p2p hello on non p2p circuit",
      ...
      if (! (circuit->is_type & level))
        {
          ...
          update_rej_adj_count (circuit);
#ifndef FABRICD
          isis_notif_reject_adjacency (circuit, "Interface level mismatch",
                                       raw_pdu, sizeof (raw_pdu));
#endif /* ifndef FABRICD */
          return ISIS_WARNING;
        }
  ...
  struct iih_info iih = { .circuit = circuit, .ssnpa = ssnpa, .level = level };
  ...
  if (pdu_len_validate (iih.pdu_len, circuit))
    {
      ...
      update_rej_adj_count (circuit);
#ifndef FABRICD
      isis_notif_reject_adjacency (circuit, "Invalid PDU length", raw_pdu,
                                   sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      return ISIS_WARNING;
    }
  ...
  if (! p2p_hello && ! (level & iih.circ_type))
    {
      flog_err (EC_ISIS_PACKET, "Level %d LAN Hello with Circuit Type %d",
                level, iih.circ_type);
      update_rej_adj_count (circuit);
#ifndef FABRICD
      isis_notif_reject_adjacency (circuit, "LAN Hello with wrong IS-level",
                                   raw_pdu, sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      return ISIS_ERROR;
    }
  ...
  if (isis_unpack_tlvs (STREAM_READABLE (circuit->rcv_stream),
                        circuit->rcv_stream, &iih.tlvs, &error_log))
    {
      ...
      update_rej_adj_count (circuit);
#ifndef FABRICD
      isis_notif_reject_adjacency (circuit, "Failed to unpack TLVs", raw_pdu,
                                   sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      goto out;
    }
  ...
  if (! iih.tlvs->protocols_supported.count)
    {
      ...
      update_rej_adj_count (circuit);
#ifndef FABRICD
      isis_notif_reject_adjacency (circuit, "No supported protocols TLV",
                                   raw_pdu, sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      goto out;
    }
  ...
  if (auth_code != ISIS_AUTH_OK)
    {
      isis_event_auth_failure (circuit->area->area_tag,
                               "IIH authentication failure", iih.sys_id);
#ifndef FABRICD
      /* send northbound notification */
      stream_get_from (raw_pdu, circuit->rcv_stream, pdu_start,
                       pdu_end - pdu_start);
      if (auth_code == ISIS_AUTH_FAILURE)
        ...
        { /* AUTH_TYPE_FAILURE or NO_VALIDATOR */
          update_rej_adj_count (circuit);
  ...
  if (! memcmp (iih.sys_id, circuit->isis->sysid, ISIS_SYS_ID_LEN))
    {
      zlog_warn ("ISIS-Adj (%s): Received IIH with own sysid on %s - discard",
                 circuit->area->area_tag, circuit->interface->name);
      update_rej_adj_count (circuit);
#ifndef FABRICD
      isis_notif_reject_adjacency (circuit, "Received IIH with our own sysid",
                                   raw_pdu, sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      goto out;
    }
  ...
  if (! p2p_hello &&
      (listcount (circuit->area->area_addrs) == 0 ||
       (level == ISIS_LEVEL1 && ! isis_tlvs_area_addresses_match (
...
static void
lsp_flood_or_update (struct isis_lsp *lsp, struct isis_circuit *circuit,
                     bool circuit_scoped)
{
  if (! circuit_scoped)
 ...
 */
static int
process_lsp (uint8_t pdu_type, struct isis_circuit *circuit,
             const uint8_t *ssnpa, uint8_t max_area_addrs)
{
  ...
  if (IS_DEBUG_UPDATE_PACKETS)
    {
      zlog_debug ("ISIS-Upd (%s): Rcvd %sL%d LSP on %s, cirType %s, cirID %u",
                  circuit->area->area_tag,
  ...
  isis_notif_lsp_received (circuit, hdr.lsp_id, hdr.seqno, time (NULL), buf);
#endif /* ifndef FABRICD */
  ...
  if (IS_DEBUG_UPDATE_PACKETS)
    {
      zlog_debug (
          ...
          circuit->area->area_tag, level, hdr.lsp_id, hdr.seqno, hdr.checksum,
          hdr.rem_lifetime, hdr.pdu_len, circuit->interface->name);
    }
  ...
  if (iso_csum_verify (STREAM_DATA (circuit->rcv_stream) + 12,
    ...
    {
      zlog_debug ("ISIS-Upd (%s): LSP %pLS invalid LSP checksum 0x%04hx",
  ...
  if (isis_unpack_tlvs (STREAM_READABLE (circuit->rcv_stream),
                        circuit->rcv_stream, &tlvs, &error_log))
    {
      ...
      isis_notif_lsp_error (circuit, hdr.lsp_id, raw_pdu, sizeof (raw_pdu), 0,
                            0);
#endif /* ifndef FABRICD */
      goto out;
    }
  ...
  int auth_code =
      isis_tlvs_auth_is_valid (tlvs, passwd, circuit->rcv_stream, true);
  if (auth_code != ISIS_AUTH_OK)
    {
      isis_event_auth_failure (circuit->area->area_tag,
  ...
  int comp = 0;
  if (lsp)
    comp = lsp_compare (circuit->area->area_tag, lsp, hdr.seqno, hdr.checksum,
                        hdr.rem_lifetime);
  if (lsp && (lsp->own_lsp))
    goto dontcheckadj;
      ...
      if (! isis_adj_lookup_snpa (ssnpa, circuit->u.bc.adjdb[level - 1]))
        {
          zlog_debug (
              ...
              circuit->area->area_tag, hdr.lsp_id, hdr.seqno, hdr.checksum,
              hdr.rem_lifetime, circuit->interface->name);
...
dontcheckadj:
  ...
  if (lsp && (lsp->hdr.seqno == hdr.seqno) &&
      (lsp->hdr.checksum != hdr.checksum) && hdr.rem_lifetime)
    {
      zlog_warn (
  ...
  if (hdr.rem_lifetime == 0)
    {
      if (! lsp)
        ...
        {
          if (memcmp (hdr.lsp_id, circuit->isis->sysid, ISIS_SYS_ID_LEN))
            {
              ...
              if (comp == LSP_NEWER)
                {
                  lsp_update (lsp, &hdr, tlvs, circuit->rcv_stream,
                              circuit->area, level, lsp_confusion);
                  if (lsp_confusion)
                    isis_free_tlvs (tlvs);
                  tlvs = NULL;
                  /* ii */
                  ...
                  /* v */
                  ISIS_FLAGS_CLEAR_ALL (lsp->SSNflags); /* FIXME:
                                                           OTHER
                                                           than c
                                                           */
          ...
          else if (lsp->hdr.rem_lifetime != 0)
            {
              ...
              if (IS_DEBUG_UPDATE_PACKETS)
                zlog_debug (
                    ...
                    circuit->area->area_tag, hdr.lsp_id, lsp->hdr.seqno);
            }
          else
            {
      ...
      if ((! lsp || comp == LSP_NEWER))
        {
          ...
          if (LSP_FRAGMENT (hdr.lsp_id) != 0)
            {
              ...
              lsp0 = lsp_search (&circuit->area->lspdb[level - 1], lspid);
              if (! lsp0)
                {
          ...
          if (! lsp)
            {
              lsp = lsp_new_from_recv (&hdr, tlvs, circuit->rcv_stream, lsp0,
                                       circuit->area, level);
              tlvs = NULL;
              lsp_insert (&circuit->area->lspdb[level - 1], lsp);
            }
          else /* exists, so we overwrite */
            {
              lsp_update (lsp, &hdr, tlvs, circuit->rcv_stream, circuit->area,
                          level, false);
              tlvs = NULL;
            }
             ...
             const uint8_t *ssnpa)
{
  ...
  bool is_csnp =
      (pdu_type == L1_COMPLETE_SEQ_NUM || pdu_type == L2_COMPLETE_SEQ_NUM);
  char typechar = is_csnp ? 'C' : 'P';
  int level =
      (pdu_type == L1_COMPLETE_SEQ_NUM || pdu_type == L1_PARTIAL_SEQ_NUM)
          ? ISIS_LEVEL1
          : ISIS_LEVEL2;
  ...
  if (CHECK_FLAG (passwd->snp_auth, SNP_AUTH_RECV))
    {
      ...
      if (auth_code != ISIS_AUTH_OK)
        {
          isis_event_auth_failure (circuit->area->area_tag,
                                   "SNP authentication failure", rem_sys_id);
#ifndef FABRICD
          /* send northbound notification */
          stream_get_from (raw_pdu, circuit->rcv_stream, pdu_start,
                           pdu_end - pdu_start);
      ...
      for (struct isis_lsp_entry *entry = entry_head; entry;
           entry = entry->next)
        {
          zlog_debug (
              ...
              circuit->area->area_tag, typechar, entry->id, entry->seqno,
  ...
  for (struct isis_lsp_entry *entry = entry_head; entry; entry = entry->next)
    {
      ...
      bool own_lsp =
          ! memcmp (entry->id, circuit->isis->sysid, ISIS_SYS_ID_LEN);
      if (lsp)
        {
      ...
      else
        {
          ...
          if (entry->rem_lifetime && entry->checksum && entry->seqno &&
              memcmp (entry->id, circuit->isis->sysid, ISIS_SYS_ID_LEN))
            {
              ...
              if (LSP_FRAGMENT (entry->id))
                {
                  ...
                  memcpy (lspid, entry->id, ISIS_SYS_ID_LEN + 1);
                  LSP_FRAGMENT (lspid) = 0;
                  lsp0 = lsp_search (&circuit->area->lspdb[level - 1], lspid);
                  if (! lsp0)
                    {
              ...
              lsp = lsp_new (circuit->area, entry->id, entry->rem_lifetime, 0,
...
int
isis_handle_pdu (struct isis_circuit *circuit, uint8_t *ssnpa)
{
  int retval = ISIS_OK;
  size_t pdu_start = stream_get_getp (circuit->rcv_stream);
  size_t pdu_end = stream_get_endp (circuit->rcv_stream);
  ...
  uint8_t pdu_type =
      stream_getc (circuit->rcv_stream) & 0x1f; /* bits 6-8 are reserved */
  ...
  if (idrp == ISO9542_ESIS)
    {
      flog_err (EC_LIB_DEVELOPMENT, "No support for ES-IS packet IDRP=%hhx",
                idrp);
  ...
  if (idrp != ISO10589_ISIS)
    {
      flog_err (EC_ISIS_PACKET, "Not an IS-IS packet IDRP=%hhx", idrp);
      pdu_counter_count_drop (circuit->area, pdu_type);
      return ISIS_ERROR;
    }
...
#ifndef FABRICD
      /* send northbound notification */
      isis_notif_version_skew (circuit, version1, raw_pdu, sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      pdu_counter_count_drop (circuit->area, pdu_type);
      return ISIS_WARNING;
    }
...
#ifndef FABRICD
      /* send northbound notification */
      isis_notif_id_len_mismatch (circuit, id_len, raw_pdu, sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      pdu_counter_count_drop (circuit->area, pdu_type);
      return ISIS_ERROR;
    }
...
#ifndef FABRICD
      /* send northbound notification */
      isis_notif_version_skew (circuit, version2, raw_pdu, sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      pdu_counter_count_drop (circuit->area, pdu_type);
      return ISIS_WARNING;
    }
...
#ifndef FABRICD
      /* send northbound notification */
      isis_notif_max_area_addr_mismatch (circuit, max_area_addrs, raw_pdu,
                                         sizeof (raw_pdu));
#endif /* ifndef FABRICD */
      pdu_counter_count_drop (circuit->area, pdu_type);
      return ISIS_ERROR;
  ...
  switch (pdu_type)
    ...
    case L2_LINK_STATE:
    case FS_LINK_STATE:
      if (fabricd && pdu_type != L2_LINK_STATE && pdu_type != FS_LINK_STATE)
        {
          pdu_counter_count_drop (circuit->area, pdu_type);
          return ISIS_ERROR;
        }
...
void
isis_receive (struct event *thread)
{
  struct isis_circuit *circuit;
...
static uint8_t
hello_pdu_type (struct isis_circuit *circuit, int level)
{
  if (circuit->circ_type == CIRCUIT_T_BROADCAST)
...
put_hello_hdr (struct isis_circuit *circuit, int level, size_t *len_pointer)
{
  ...
  *len_pointer = stream_get_endp (circuit->snd_stream);
  ...
  if (circuit->circ_type == CIRCUIT_T_BROADCAST)
    {
      uint8_t *desig_is = (level == IS_LEVEL_1) ? circuit->u.bc.l1_desig_is
...
send_hello (struct isis_circuit *circuit, int level)
{
      ...
      if (circuit->u.p2p.neighbor)
        {
          ...
          isis_tlvs_add_threeway_adj (tlvs, threeway_state, ext_circuit_id,
                                      circuit->u.p2p.neighbor->sysid,
                                      circuit->u.p2p.neighbor->ext_circuit_id);
        }
      else
        {
          isis_tlvs_add_threeway_adj (tlvs, ISIS_THREEWAY_DOWN, ext_circuit_id,
                                      NULL, 0);
        }
    }
  ...
  if (mt_count == 0 && area_is_mt (circuit->area))
           ...
           (mt_count > 1))
    {
      for (unsigned int i = 0; i < mt_count; i++)
        isis_tlvs_add_mt_router_info (tlvs, mt_settings[i]->mtid, false,
                                      false);
    }
  ...
  if (circuit->ipv6_router)
    isis_tlvs_add_global_ipv6_addresses (tlvs, circuit->ipv6_non_link);

  bool should_pad_hello =
      circuit->pad_hellos == ISIS_HELLO_PADDING_ALWAYS ||
      (circuit->pad_hellos == ISIS_HELLO_PADDING_DURING_ADJACENCY_FORMATION &&
       circuit->upadjcount[0] + circuit->upadjcount[1] == 0);

  if (isis_pack_tlvs (tlvs, circuit->snd_stream, len_pointer, should_pad_hello,
                      false))
    {
      isis_free_tlvs (tlvs);
  ...
  retval = circuit->tx (circuit, level);
  if (retval != ISIS_OK)
    flog_err (EC_ISIS_PACKET, "ISIS-Adj (%s): Send L%d IIH on %s failed",
              circuit->area->area_tag, level, circuit->interface->name);
...
static void
send_hello_cb (struct event *thread)
{
  ...
  if (circuit->circ_type != CIRCUIT_T_BROADCAST)
    {
      zlog_warn (
          "ISIS-Hello (%s): Trying to send hello on unknown circuit type %d",
...
static void
_send_hello_sched (struct isis_circuit *circuit, struct event **threadp,
                   int level, long delay)
{
  ...
  event_add_timer_msec (master, send_hello_cb, &circuit->level_arg[level - 1],
                        isis_jitter (delay, IIH_JITTER), threadp);
}

void
send_hello_sched (struct isis_circuit *circuit, int level, long delay)
{
  if (circuit->circ_type == CIRCUIT_T_P2P)
    {
  ...
  if (circuit->circ_type != CIRCUIT_T_BROADCAST)
    {
      zlog_warn ("%s: encountered unknown circuit type %d on %s", __func__,
                 circuit->circ_type, circuit->interface->name);
      return;
    }
...
send_csnp (struct isis_circuit *circuit, int level)
{
  ...
  uint8_t pdu_type =
      (level == ISIS_LEVEL1) ? L1_COMPLETE_SEQ_NUM : L2_COMPLETE_SEQ_NUM;
  ...
  while (loop)
    {
      ...
      isis_tlvs_add_csnp_entries (tlvs, start, stop, num_lsps,
                                  &circuit->area->lspdb[level - 1], &last_lsp);
      /*
...
void
send_l1_csnp (struct event *thread)
{
  struct isis_circuit *circuit;
...
void
send_l2_csnp (struct event *thread)
{
  struct isis_circuit *circuit;
...
send_psnp (int level, struct isis_circuit *circuit)
{
  ...
  uint8_t pdu_type =
      (level == ISIS_LEVEL1) ? L1_PARTIAL_SEQ_NUM : L2_PARTIAL_SEQ_NUM;
...
void
send_l1_psnp (struct event *thread)
{
 ...
 */
void
send_l2_psnp (struct event *thread)
{
  struct isis_circuit *circuit;
 ...
 */
void
send_lsp (struct isis_circuit *circuit, struct isis_lsp *lsp,
          enum isis_tx_type tx_type)
{
  ...
  if (stream_get_endp (lsp->pdu) > stream_get_size (circuit->snd_stream))
    {
      flog_err (
          ...
          circuit->area->area_tag, lsp->level, lsp->hdr.lsp_id, lsp->hdr.seqno,
          lsp->hdr.checksum, lsp->hdr.rem_lifetime, circuit->interface->name,
  ...
  if (IS_DEBUG_UPDATE_PACKETS)
    {
      zlog_debug (
          ...
          lsp->level, lsp->hdr.lsp_id, lsp->hdr.seqno, lsp->hdr.checksum,
  ...
  if (retval != ISIS_OK)
    {
      flog_err (EC_ISIS_PACKET, "ISIS-Upd (%s): Send L%d LSP on %s failed %s",
                circuit->area->area_tag, lsp->level, circuit->interface->name,
...
void
isis_log_pdu_drops (struct isis_area *area, const char *pdu_type)
{
  ...
  zlog_info (
      "PDU drop detected of type: %s. %" PRIu64 " Total Drops; %" PRIu64
      " L1 IIH drops;  %" PRIu64 " L2 IIH drops; %" PRIu64
      " P2P IIH drops; %" PRIu64 " L1 LSP drops; %" PRIu64
      " L2 LSP drops; %" PRIu64 " FS LSP drops; %" PRIu64
      " L1 CSNP drops; %" PRIu64 " L2 CSNP drops; %" PRIu64
      " L1 PSNP drops; %" PRIu64 " L2 PSNP drops.",
