...
void
bfd_recvtimer_update (struct bfd_session *bs)
{
  ...
  event_add_timer_tv (master, bfd_recvtimer_cb, bs, &tv, &bs->recvtimer_ev);
}

void
bfd_echo_recvtimer_update (struct bfd_session *bs)
{
...
void
bfd_xmttimer_update (struct bfd_session *bs, uint64_t jitter)
{
...
void
bfd_echo_xmttimer_update (struct bfd_session *bs, uint64_t jitter)
{
  struct timeval tv = { .tv_sec = 0, .tv_usec = jitter };
  ...
  event_add_timer_tv (master, bfd_echo_xmt_cb, bs, &tv, &bs->echo_xmttimer_ev);
}

void
bfd_recvtimer_delete (struct bfd_session *bs)
{
  EVENT_OFF (bs->recvtimer_ev);
}

void
bfd_echo_recvtimer_delete (struct bfd_session *bs)
{
  EVENT_OFF (bs->echo_recvtimer_ev);
}

void
bfd_xmttimer_delete (struct bfd_session *bs)
{
  EVENT_OFF (bs->xmttimer_ev);
}

void
bfd_echo_xmttimer_delete (struct bfd_session *bs)
{
