 ...
 */
int
_ptm_bfd_send (struct bfd_session *bs, uint16_t *port, const void *data,
               size_t datalen)
{
  struct sockaddr *sa;
 ...
 */
static uint16_t
bfd_pkt_checksum (struct udphdr *pkt, size_t pktsize, struct in6_addr *ip,
                  sa_family_t family)
{
  uint16_t chksum;
 ...
 */
void
ptm_bfd_echo_fp_snd (struct bfd_session *bfd)
{
  int sd;
  ...
  /* add bfd echo */
  beph =
      (struct bfd_echo_pkt *) (sendbuff + sizeof (struct udphdr) +
...
void
ptm_bfd_echo_snd (struct bfd_session *bfd)
{
  struct sockaddr *sa;
  socklen_t salen;
  int sd;
...
static int
ptm_bfd_process_echo_pkt (struct bfd_vrf_global *bvrf, int s)
{
  struct bfd_session *bfd;
  uint32_t my_discr = 0;
  uint64_t my_rtt = 0;
...
void
ptm_bfd_snd (struct bfd_session *bfd, int fbit)
{
  struct bfd_pkt cp = {};
  ...
  if (bfd->polling)
  ...
  else
    {
      ...
      cp.timers.required_min_rx = htonl (bfd->cur_timers.required_min_rx);
    }
 ...
 */
ssize_t
bfd_recv_ipv4_fp (int sd, uint8_t *msgbuf, size_t msgbuflen, uint8_t *ttl,
                  ifindex_t *ifindex, struct sockaddr_any *local,
                  struct sockaddr_any *peer)
{
  ssize_t mlen;
  ...
  if (mlen == -1)
    {
      if (errno != EAGAIN || errno != EWOULDBLOCK || errno != EINTR)
        zlog_err ("%s: recv failed: %s", __func__, strerror (errno));
...
ssize_t
bfd_recv_ipv4 (int sd, uint8_t *msgbuf, size_t msgbuflen, uint8_t *ttl,
               ifindex_t *ifindex, struct sockaddr_any *local,
               struct sockaddr_any *peer)
{
  struct cmsghdr *cm;
        ...
        case IP_TTL:
          {
            ...
            if (ttlval > 255)
              {
                if (bglobal.debug_network)
                  zlog_debug ("%s: invalid TTL: %u", __func__, ttlval);
                return -1;
              }
            *ttl = ttlval;
            break;
...
ssize_t
bfd_recv_ipv6 (int sd, uint8_t *msgbuf, size_t msgbuflen, uint8_t *ttl,
               ifindex_t *ifindex, struct sockaddr_any *local,
               struct sockaddr_any *peer)
{
  struct cmsghdr *cm;
...
static void
bfd_sd_reschedule (struct bfd_vrf_global *bvrf, int sd)
{
  if (sd == bvrf->bg_shop)
    {
...
PRINTFRR (6, 7)
static void
cp_debug (bool mhop, struct sockaddr_any *peer, struct sockaddr_any *local,
          ifindex_t ifindex, vrf_id_t vrfid, const char *fmt, ...)
{
  ...
  if (local->sa_sin.sin_family)
    snprintf (localstr, sizeof (localstr), " local:%s", satostr (local));
  else
    localstr[0] = 0;
  ...
  zlog_debug ("control-packet: %s [mhop:%s%s%s%s%s]", buf, mhop ? "yes" : "no",
              peerstr, localstr, portstr, vrfstr);
}

static bool
bfd_check_auth (const struct bfd_session *bfd, const struct bfd_pkt *cp)
{
...
bfd_recv_cb (struct event *t)
{
  ...
  if (BFD_GETVER (cp->diag) != BFD_VERSION)
    {
      cp_debug (is_mhop, &peer, &local, ifindex, vrfid, "bad version %d",
                BFD_GETVER (cp->diag));
      return;
    }
  ...
  if (bfd && bfd->vrf && bfd->vrf->vrf_id != vrfid)
    {
      cp_debug (is_mhop, &peer, &local, ifindex, vrfid, "wrong vrfid.");
      return;
    }
  ...
  bfd->remote_timers.required_min_echo = ntohl (cp->timers.required_min_echo);
  ...
  if (bfd->cur_timers.required_min_rx > bfd->remote_timers.desired_min_tx)
    bfd->detect_TO = bfd->remote_detect_mult * bfd->cur_timers.required_min_rx;
  else
    bfd->detect_TO =
        bfd->remote_detect_mult * bfd->remote_timers.desired_min_tx;
 ...
 */
int
bp_bfd_echo_in (struct bfd_vrf_global *bvrf, int sd, uint8_t *ttl,
                uint32_t *my_discr, uint64_t *my_rtt)
{
  struct bfd_echo_pkt *bep;
  ssize_t rlen;
  ...
  if (sd == bvrf->bg_echo)
    {
#ifdef BFD_LINUX
      rlen = bfd_recv_ipv4_fp (sd, msgbuf, sizeof (msgbuf), ttl, &ifindex,
                               &local, &peer);
      ...
      bfd_offset = sizeof (struct udphdr) + sizeof (struct iphdr) +
                   sizeof (struct ethhdr);
#else
      rlen = bfd_recv_ipv4 (sd, msgbuf, sizeof (msgbuf), ttl, &ifindex, &local,
                            &peer);
      bfd_offset = 0;
#endif
    }
    ...
    {
      rlen = bfd_recv_ipv6 (sd, msgbuf, sizeof (msgbuf), ttl, &ifindex, &local,
                            &peer);
      bfd_offset = 0;
    }
  ...
  if (rlen < (ssize_t) sizeof (*bep))
    {
      cp_debug (false, &peer, &local, ifindex, vrfid, "small echo packet");
      return -1;
    }
  ...
  if ((*ttl == BFD_TTL_VAL) && (sd == bvrf->bg_echov6))
    {
      bp_udp_send (sd, *ttl - 1, msgbuf, rlen, (struct sockaddr *) &peer,
 ...
 */
int
bp_udp_send_fp (int sd, uint8_t *data, size_t datalen, struct bfd_session *bfd)
{
  ssize_t wlen;
  struct msghdr msg = { 0 };
  struct iovec iov[1];
  ...
  if (wlen <= 0)
    {
      if (bglobal.debug_network)
        zlog_debug ("%s: loopback failure: (%d) %s", __func__, errno,
                    strerror (errno));
      return -1;
    }
    ...
    {
      if (bglobal.debug_network)
        zlog_debug ("%s: partial send: %zd expected %zu", __func__, wlen,
                    datalen);
      return -1;
    }
...
int
bp_udp_send (int sd, uint8_t ttl, uint8_t *data, size_t datalen,
             struct sockaddr *to, socklen_t tolen)
{
  struct cmsghdr *cmsg;
  ssize_t wlen;
 ...
 */
int
bp_set_ttl (int sd, uint8_t value)
{
  int ttl = value;
...
int
bp_set_tos (int sd, uint8_t value)
{
  int tos = value;
...
bp_set_ipopts (int sd)
{
  ...
  if (setsockopt (sd, IPPROTO_IP, IP_RECVTTL, &rcvttl, sizeof (rcvttl)) == -1)
  ...
  if (setsockopt (sd, IPPROTO_IP, IP_PKTINFO, &pktinfo, sizeof (pktinfo)) ==
      -1)
    zlog_fatal ("set-ipopts: setsockopt(IP_PKTINFO, %d): %s", pktinfo,
                strerror (errno));
#endif /* BFD_LINUX */
#ifdef BFD_BSD
  int yes = 1;
  ...
  if (setsockopt (sd, IPPROTO_IP, IP_RECVDSTADDR, &yes, sizeof (yes)) == -1)
    zlog_fatal ("set-ipopts: setsockopt(IP_RECVDSTADDR, %d): %s", yes,
                strerror (errno));
...
static void
bp_bind_ip (int sd, uint16_t port)
{
  struct sockaddr_in sin;
...
int
bp_udp_shop (const struct vrf *vrf)
{
  int sd;

  frr_with_privs (&bglobal.bfdd_privs)
    {
      sd = vrf_socket (AF_INET, SOCK_DGRAM, PF_UNSPEC, vrf->vrf_id, vrf->name);
    }
  if (sd == -1)
...
int
bp_udp_mhop (const struct vrf *vrf)
{
  int sd;
...
bp_peer_socket (const struct bfd_session *bs)
{
  ...
  if (bs->key.ifname[0])
    device_to_bind = (const char *) bs->key.ifname;
           ...
           ((CHECK_FLAG (bs->flags, BFD_SESS_FLAG_MH) && bs->key.vrfname[0])))
    device_to_bind = (const char *) bs->key.vrfname;

  frr_with_privs (&bglobal.bfdd_privs)
    {
      sd = vrf_socket (AF_INET, SOCK_DGRAM, PF_UNSPEC, bs->vrf->vrf_id,
                       device_to_bind);
    }
  if (sd == -1)
    {
      zlog_err ("ipv4-new: failed to create socket: %s", strerror (errno));
      return -1;
    }
      ...
      if ((++pcount) > (BFD_SRCPORTMAX - BFD_SRCPORTINIT))
        {
          ...
          zlog_err ("ipv4-new: failed to bind port: %s", strerror (errno));
          close (sd);
          return -1;
        }
...
int
bp_peer_socketv6 (const struct bfd_session *bs)
{
  int sd, pcount;
  struct sockaddr_in6 sin6;
  static int srcPort = BFD_SRCPORTINIT;
  ...
  frr_with_privs (&bglobal.bfdd_privs)
    {
      sd = vrf_socket (AF_INET6, SOCK_DGRAM, PF_UNSPEC, bs->vrf->vrf_id,
                       device_to_bind);
    }
  if (sd == -1)
    {
  ...
  do
    {
      if ((++pcount) > (BFD_SRCPORTMAX - BFD_SRCPORTINIT))
        {
          /* Searched all ports, none available */
          zlog_err ("ipv6-new: failed to bind port: %s", strerror (errno));
          close (sd);
          return -1;
        }
      if (srcPort >= BFD_SRCPORTMAX)
...
int
bp_set_ttlv6 (int sd, uint8_t value)
{
  int ttl = value;
  ...
  if (setsockopt (sd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &ttl, sizeof (ttl)) ==
      -1)
    {
      zlog_warn ("set-ttlv6: setsockopt(IPV6_UNICAST_HOPS, %d): %s", value,
                 strerror (errno));
      return -1;
    }
...
int
bp_set_tosv6 (int sd, uint8_t value)
{
  int tos = value;

  if (setsockopt (sd, IPPROTO_IPV6, IPV6_TCLASS, &tos, sizeof (tos)) == -1)
    {
...
bp_set_ipv6opts (int sd)
{
  ...
  if (bp_set_ttlv6 (sd, BFD_TTL_VAL) == -1)
    zlog_fatal ("set-ipv6opts: setsockopt(IPV6_UNICAST_HOPS, %d): %s",
  ...
  if (setsockopt_ipv6_pktinfo (sd, ipv6_pktinfo) == -1)
    zlog_fatal ("set-ipv6opts: setsockopt(IPV6_PKTINFO, %d): %s", ipv6_pktinfo,
                strerror (errno));

  if (setsockopt (sd, IPPROTO_IPV6, IPV6_V6ONLY, &ipv6_only,
                  sizeof (ipv6_only)) == -1)
    zlog_fatal ("set-ipv6opts: setsockopt(IPV6_V6ONLY, %d): %s", ipv6_only,
                strerror (errno));
}

static void
bp_bind_ipv6 (int sd, uint16_t port)
{
  struct sockaddr_in6 sin6;
...
int
bp_udp6_shop (const struct vrf *vrf)
{
  int sd;
  ...
  frr_with_privs (&bglobal.bfdd_privs)
    {
      sd =
          vrf_socket (AF_INET6, SOCK_DGRAM, PF_UNSPEC, vrf->vrf_id, vrf->name);
    }
  if (sd == -1)
    {
...
int
bp_udp6_mhop (const struct vrf *vrf)
{
  int sd;
...
int
bp_echo_socket (const struct vrf *vrf)
{
  int s;

  frr_with_privs (&bglobal.bfdd_privs)
    {
      s = vrf_socket (AF_PACKET, SOCK_RAW, ETH_P_IP, vrf->vrf_id, vrf->name);
    }
  ...
  if (bind (s, (struct sockaddr *) &sll, sizeof (sll)) < 0)
    {
      zlog_warn ("Failed to bind echo socket: %s", safe_strerror (errno));
      close (s);
      return -1;
    }
...
int
bp_echov6_socket (const struct vrf *vrf)
{
  int s;
 ...
 */
void
bfd_peer_mac_set (int sd, struct bfd_session *bfd, struct sockaddr_any *peer,
                  struct interface *ifp)
{
  ...
  if (peer->sa_sin.sin_family == AF_INET)
    {
      /* IPV4 */
      struct sockaddr_in *addr = (struct sockaddr_in *) &arpreq_.arp_pa;
      ...
      if (ioctl (sd, SIOCGARP, &arpreq_) < 0)
        {
          zlog_warn ("BFD: getting peer's mac on %s failed error %s",
