...
static struct bfd_session *bfd_find_disc (struct sockaddr_any *sa,
                                          uint32_t ldisc);
static int bfd_session_update (struct bfd_session *bs,
...
static void
bfd_profile_set_default (struct bfd_profile *bp)
{
...
void
bfd_profile_free (struct bfd_profile *bp)
{
...
void
bfd_profile_apply (const char *profname, struct bfd_session *bs)
{
  struct bfd_profile *bp;
...
void
bfd_session_apply (struct bfd_session *bs)
{
  struct bfd_profile *bp;
  ...
  if (! CHECK_FLAG (bs->flags, BFD_SESS_FLAG_MH))
    {
      ...
      if (bs->peer_profile.min_echo_rx == BFD_DEF_REQ_MIN_ECHO_RX)
        ...
        bs->timers.required_min_echo_rx = bs->peer_profile.min_echo_rx;

      if (bs->peer_profile.min_echo_tx == BFD_DEF_DES_MIN_ECHO_TX)
        bs->timers.desired_min_echo_tx = bp->min_echo_tx;
      else
        bs->timers.desired_min_echo_tx = bs->peer_profile.min_echo_tx;
  ...
  if (bs->ses_state == PTM_BFD_UP && (bs->timers.desired_min_tx != min_tx ||
...
void
bfd_profile_remove (struct bfd_session *bs)
{
...
void
gen_bfd_key (struct bfd_key *key, struct sockaddr_any *peer,
...
struct bfd_session *
bs_peer_find (struct bfd_peer_cfg *bpc)
{
 ...
 */
int
bfd_session_enable (struct bfd_session *bs)
{
  ...
  if (bs->key.vrfname[0])
    {
      ...
      if (vrf == NULL)
        {
          zlog_err ("session-enable: specified VRF %s doesn't exists.",
                    bs->key.vrfname);
          return 0;
        }
    }
 ...
 */
void
bfd_session_disable (struct bfd_session *bs)
{
  /* We are using data plane, we don't need software. */
  if (bs->bdc)
    return;
...
void
ptm_bfd_start_xmt_timer (struct bfd_session *bfd, bool is_echo)
{
...
static void
ptm_bfd_echo_xmt_TO (struct bfd_session *bfd)
{
...
void
ptm_bfd_xmt_TO (struct bfd_session *bfd, int fbit)
{
...
void
ptm_bfd_echo_stop (struct bfd_session *bfd)
{
  bfd->echo_xmt_TO = 0;
...
void
ptm_bfd_echo_start (struct bfd_session *bfd)
{
...
void
ptm_bfd_sess_up (struct bfd_session *bfd)
{
...
void
ptm_bfd_sess_dn (struct bfd_session *bfd, uint8_t diag)
{
  int old_state = bfd->ses_state;
...
bfd_find_disc (struct sockaddr_any *sa, uint32_t ldisc)
{
  struct bfd_session *bs;
...
struct bfd_session *
ptm_bfd_sess_find (struct bfd_pkt *cp, struct sockaddr_any *peer,
                   struct sockaddr_any *local, struct interface *ifp,
...
void
bfd_echo_xmt_cb (struct event *t)
{
  struct bfd_session *bs = EVENT_ARG (t);
...
void
bfd_recvtimer_cb (struct event *t)
{
  struct bfd_session *bs = EVENT_ARG (t);
...
void
bfd_echo_recvtimer_cb (struct event *t)
{
  struct bfd_session *bs = EVENT_ARG (t);
...
int
bfd_session_update_label (struct bfd_session *bs, const char *nlabel)
{
  /* New label treatment:
...
static void
_bfd_session_update (struct bfd_session *bs, struct bfd_peer_cfg *bpc)
{
...
static int
bfd_session_update (struct bfd_session *bs, struct bfd_peer_cfg *bpc)
{
...
struct bfd_session *
ptm_bfd_sess_new (struct bfd_peer_cfg *bpc)
{
  ...
  if (bpc->bpc_has_localif)
    strlcpy (bfd->key.ifname, bpc->bpc_localif, sizeof (bfd->key.ifname));

  if (bpc->bpc_has_vrfname)
    strlcpy (bfd->key.vrfname, bpc->bpc_vrfname, sizeof (bfd->key.vrfname));
  else
    strlcpy (bfd->key.vrfname, VRF_DEFAULT_NAME, sizeof (bfd->key.vrfname));
...
struct bfd_session *
bs_registrate (struct bfd_session *bfd)
{
...
int
ptm_bfd_sess_del (struct bfd_peer_cfg *bpc)
{
  struct bfd_session *bs;
...
void
bfd_set_polling (struct bfd_session *bs)
{
  /*
 ...
 */
static void
bs_admin_down_handler (struct bfd_session *bs __attribute__ ((__unused__)),
...
static void
bs_down_handler (struct bfd_session *bs, int nstate)
{
  switch (nstate)
    {
...
static void
bs_init_handler (struct bfd_session *bs, int nstate)
{
  switch (nstate)
    {
    case PTM_BFD_ADM_DOWN:
...
static void
bs_up_handler (struct bfd_session *bs, int nstate)
{
  switch (nstate)
    {
    case PTM_BFD_ADM_DOWN:
...
void
bs_state_handler (struct bfd_session *bs, int nstate)
{
  switch (bs->ses_state)
    {
    case PTM_BFD_ADM_DOWN:
 ...
 */
void
bs_echo_timer_handler (struct bfd_session *bs)
{
  uint32_t old_timer;
  ...
  if (CHECK_FLAG (bs->flags, BFD_SESS_FLAG_ECHO) == 0 ||
      CHECK_FLAG (bs->flags, BFD_SESS_FLAG_MH) || bs->ses_state != PTM_BFD_UP)
    return;
 ...
 */
void
bs_final_handler (struct bfd_session *bs)
{
...
void
bs_set_slow_timers (struct bfd_session *bs)
{
  /*
...
void
bfd_set_echo (struct bfd_session *bs, bool echo)
{
  if (echo)
    {
...
void
bfd_set_shutdown (struct bfd_session *bs, bool shutdown)
{
  bool is_shutdown;
...
void
bfd_set_passive_mode (struct bfd_session *bs, bool passive)
{
  if (passive)
    {
...
const char *
satostr (const struct sockaddr_any *sa)
{
#define INETSTR_BUFCOUNT 8
  ...
  switch (sin->sin_family)
    {
    case AF_INET:
      inet_ntop (AF_INET, &sin->sin_addr, buf[bufidx], sizeof (buf[bufidx]));
      break;
    case AF_INET6:
...
int
strtosa (const char *addr, struct sockaddr_any *sa)
{
...
void
integer2timestr (uint64_t time, char *buf, size_t buflen)
{
...
const char *
bs_to_string (const struct bfd_session *bs)
{
  static char buf[256];
  ...
  pos += snprintf (
      buf + pos, sizeof (buf) - pos, " peer:%s",
      inet_ntop (bs->key.family, &bs->key.peer, addr_buf, sizeof (addr_buf)));
  pos += snprintf (
      buf + pos, sizeof (buf) - pos, " local:%s",
      inet_ntop (bs->key.family, &bs->key.local, addr_buf, sizeof (addr_buf)));
  if (bs->key.vrfname[0])
    pos +=
        snprintf (buf + pos, sizeof (buf) - pos, " vrf:%s", bs->key.vrfname);
  if (bs->key.ifname[0])
    pos +=
        snprintf (buf + pos, sizeof (buf) - pos, " ifname:%s", bs->key.ifname);
...
int
bs_observer_add (struct bfd_session *bs)
{
  struct bfd_session_observer *bso;
  ...
  memcpy (&bso->bso_addr.u.prefix, &bs->key.local, sizeof (bs->key.local));
...
void
bs_observer_del (struct bfd_session_observer *bso)
{
...
void
bs_to_bpc (struct bfd_session *bs, struct bfd_peer_cfg *bpc)
{
  ...
  if (bs->key.ifname[0])
    {
      ...
      strlcpy (bpc->bpc_localif, bs->key.ifname, sizeof (bpc->bpc_localif));
    }

  if (bs->key.vrfname[0])
    {
      bpc->bpc_has_vrfname = true;
      strlcpy (bpc->bpc_vrfname, bs->key.vrfname, sizeof (bpc->bpc_vrfname));
    }
}
...
static bool
bfd_id_hash_cmp (const void *n1, const void *n2)
{
...
static bool
bfd_key_hash_cmp (const void *n1, const void *n2)
{
  const struct bfd_session *bs1 = n1, *bs2 = n2;
  ...
  if (memcmp (&bs1->key.peer, &bs2->key.peer, sizeof (bs1->key.peer)))
    return false;
  if (memcmp (bs1->key.vrfname, bs2->key.vrfname, sizeof (bs1->key.vrfname)))
    return false;
  ...
  if (bs1->key.ifname[0] && bs2->key.ifname[0] &&
      memcmp (bs1->key.ifname, bs2->key.ifname, sizeof (bs1->key.ifname)))
    return false;
...
/* Iteration functions. */
void
bfd_id_iterate (hash_iter_func hif, void *arg)
{
  hash_iterate (bfd_id_hash, hif, arg);
}

void
bfd_key_iterate (hash_iter_func hif, void *arg)
{
 ...
 */
bool
bfd_id_insert (struct bfd_session *bs)
{
  return (hash_get (bfd_id_hash, bs, hash_alloc_intern) == bs);
}

bool
bfd_key_insert (struct bfd_session *bs)
{
...
bfd_initialize (void)
{
  ...
  bfd_key_hash =
      hash_create (bfd_key_hash_do, bfd_key_hash_cmp, "BFD session hash");
  TAILQ_INIT (&bplist);
}

static void
_bfd_free (struct hash_bucket *hb, void *arg __attribute__ ((__unused__)))
{
...
static int
_bfd_session_next (struct hash_bucket *hb, void *arg)
{
 ...
 */
const struct bfd_session *
bfd_session_next (const struct bfd_session *bs, bool mhop)
{
...
static void
_bfd_session_remove_manual (struct hash_bucket *hb,
                            void *arg __attribute__ ((__unused__)))
{
  struct bfd_session *bs = hb->data;
 ...
 */
static void
_bfd_profile_update (struct hash_bucket *hb, void *arg)
{
...
void
bfd_profile_update (struct bfd_profile *bp)
{
  hash_iterate (bfd_key_hash, _bfd_profile_update, bp);
}

static void
_bfd_profile_detach (struct hash_bucket *hb, void *arg)
{
  struct bfd_profile *bp = arg;
  struct bfd_session *bs = hb->data;
...
static void
bfd_profile_detach (struct bfd_profile *bp)
{
...
bfd_vrf_enable (struct vrf *vrf)
{
  ...
  if (! bvrf->bg_ev[1] && bvrf->bg_mhop != -1)
    event_add_read (master, bfd_recv_cb, bvrf, bvrf->bg_mhop, &bvrf->bg_ev[1]);
  ...
  if (! bvrf->bg_ev[4] && bvrf->bg_echo != -1)
    event_add_read (master, bfd_recv_cb, bvrf, bvrf->bg_echo, &bvrf->bg_ev[4]);
...
static int
bfd_vrf_disable (struct vrf *vrf)
{
  struct bfd_vrf_global *bvrf;
...
void
bfd_vrf_terminate (void)
{
  vrf_terminate ();
}

struct bfd_vrf_global *
bfd_vrf_look_by_session (struct bfd_session *bfd)
{
  struct vrf *vrf;
...
unsigned long
bfd_get_session_count (void)
{
  return bfd_key_hash->count;
}

void
bfd_rtt_init (struct bfd_session *bfd)
{
  uint8_t i;
