...
static int parse_peer_config (struct json_object *jo,
...
static int json_object_add_peer (struct json_object *jo,
 ...
 */
static int
config_add (struct bfd_peer_cfg *bpc, void *arg __attribute__ ((unused)))
{
  return ptm_bfd_sess_new (bpc) == NULL;
}

static int
config_del (struct bfd_peer_cfg *bpc, void *arg __attribute__ ((unused)))
{
  return ptm_bfd_sess_del (bpc) != 0;
}

static int
parse_config_json (struct json_object *jo, bpc_handle h, void *arg)
{
  const char *key, *sval;
  ...
  JSON_FOREACH (jo, joi, join)
    {
      ...
      if (strcmp (key, "ipv4") == 0)
      ...
      else
        {
          ...
          zlog_warn ("%s:%d invalid configuration: %s", __func__, __LINE__,
                     sval);
          error++;
        }
    }
...
static int
parse_list (struct json_object *jo, enum peer_list_type plt, bpc_handle h,
            void *arg)
{
  struct json_object *jo_val;
  struct bfd_peer_cfg bpc;
  ...
  for (idx = 0; idx < allen; idx++)
    {
      ...
      switch (plt)
        ...
        default:
          error++;
          zlog_err ("%s:%d: unsupported peer type", __func__, __LINE__);
          break;
        }
...
static int
parse_peer_config (struct json_object *jo, struct bfd_peer_cfg *bpc)
{
  const char *key, *sval;
  struct json_object *jo_val;
  struct json_object_iterator joi, join;
      ...
      if (strcmp (key, "multihop") == 0)
      ...
      else if (strcmp (key, "peer-address") == 0)
        {
          ...
          if (strtosa (sval, &bpc->bpc_peer) != 0 ||
            ...
            {
              zlog_debug ("%s:%d failed to parse peer-address '%s'", __func__,
                          __LINE__, sval);
              error++;
            }
      ...
      else if (strcmp (key, "local-address") == 0)
        {
          ...
          if (strtosa (sval, &bpc->bpc_local) != 0 ||
            ...
            {
              zlog_debug ("%s:%d failed to parse local-address '%s'", __func__,
                          __LINE__, sval);
              error++;
            }
      ...
      else if (strcmp (key, "local-interface") == 0)
        {
          ...
          sval = json_object_get_string (jo_val);
          if (strlcpy (bpc->bpc_localif, sval, sizeof (bpc->bpc_localif)) >
              sizeof (bpc->bpc_localif))
            {
              zlog_debug ("        local-interface: %s (truncated)", sval);
              error++;
            }
          else
            {
      ...
      else if (strcmp (key, "vrf-name") == 0)
        {
          ...
          sval = json_object_get_string (jo_val);
          if (strlcpy (bpc->bpc_vrfname, sval, sizeof (bpc->bpc_vrfname)) >
              sizeof (bpc->bpc_vrfname))
            {
              zlog_debug ("        vrf-name: %s (truncated)", sval);
              error++;
            }
          else
            {
        ...
        {
          bpc->bpc_detectmultiplier = json_object_get_int64 (jo_val);
      ...
      else if (strcmp (key, "label") == 0)
        {
          ...
          sval = json_object_get_string (jo_val);
          if (strlcpy (bpc->bpc_label, sval, sizeof (bpc->bpc_label)) >
              sizeof (bpc->bpc_label))
            {
              zlog_debug ("        label: %s (truncated)", sval);
              error++;
            }
          else
            {
        ...
        {
          sval = json_object_get_string (jo_val);
          zlog_warn ("%s:%d invalid configuration: '%s: %s'", __func__,
                     __LINE__, key, sval);
          error++;
        }
    }
  ...
  if (bpc->bpc_peer.sa_sin.sin_family == 0)
    {
      zlog_debug ("%s:%d no peer address provided", __func__, __LINE__);
      error++;
    }
...
static int
parse_peer_label_config (struct json_object *jo, struct bfd_peer_cfg *bpc)
{
 ...
 */
int
config_request_add (const char *jsonstr)
{
  struct json_object *jo;
...
int
config_request_del (const char *jsonstr)
{
  struct json_object *jo;
...
char *
config_response (const char *status, const char *error)
{
  ...
  jsonstr =
      XSTRDUP (MTYPE_BFDD_NOTIFICATION,
...
char *
config_notify (struct bfd_session *bs)
{
  ...
  switch (bs->ses_state)
    ...
    case PTM_BFD_DOWN:
      ...
      now = monotime (NULL);
      json_object_int_add (resp, "downtime", now - bs->downtime.tv_sec);
      break;
    case PTM_BFD_INIT:
...
char *
config_notify_config (const char *op, struct bfd_session *bs)
{
  struct json_object *resp;
  char *jsonstr;
...
int
config_notify_request (struct bfd_control_socket *bcs, const char *jsonstr,
                       bpc_handle bh)
{
  struct json_object *jo;
...
static int
json_object_add_peer (struct json_object *jo, struct bfd_session *bs)
{
      ...
      if (memcmp (&bs->key.local, &zero_addr, sizeof (bs->key.local)))
        json_object_string_add (jo, "local-address",
                                inet_ntop (bs->key.family, &bs->key.local,
                                           addr_buf, sizeof (addr_buf)));
      if (bs->key.ifname[0])
        json_object_string_add (jo, "local-interface", bs->key.ifname);
    }
...
struct peer_label *
pl_new (const char *label, struct bfd_session *bs)
{
  struct peer_label *pl;
