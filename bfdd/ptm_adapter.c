...
static void _ptm_msg_read_address (struct stream *msg,
...
PRINTFRR (2, 3)
static void
debug_printbpc (const struct bfd_peer_cfg *bpc, const char *fmt, ...)
{
  char timers[3][128] = {};
  char minttl_str[32] = {};
  char addr[3][128] = {};
  ...
  if (bpc->bpc_has_localif)
    snprintf (addr[2], sizeof (addr[2]), " ifname:%s", bpc->bpc_localif);
  ...
  if (bpc->bpc_has_profile)
    snprintf (profile, sizeof (profile), " profile:%s", bpc->bpc_profile);
  ...
  zlog_debug ("%s [mhop:%s %s%s%s%s%s%s%s%s%s]", msgbuf,
              ...
              timers[0], timers[1], timers[2], cbit_str, minttl_str, profile);
}

static void
_ptm_bfd_session_del (struct bfd_session *bs, uint8_t diag)
{
...
static int
_ptm_msg_address (struct stream *msg, int family, const void *addr)
{
...
int
ptm_bfd_notify (struct bfd_session *bs, uint8_t notify_state)
{
  struct stream *msg;
...
static void
_ptm_msg_read_address (struct stream *msg, struct sockaddr_any *sa)
{
  uint16_t family;
  ...
  switch (family)
    ...
    case AF_INET:
      ...
      STREAM_GET (&sa->sa_sin.sin_addr, msg, sizeof (sa->sa_sin.sin_addr));
    ...
    case AF_INET6:
      ...
      STREAM_GET (&sa->sa_sin6.sin6_addr, msg, sizeof (sa->sa_sin6.sin6_addr));
#ifdef HAVE_STRUCT_SOCKADDR_SA_LEN
...
static int
_ptm_msg_read (struct stream *msg, int command, vrf_id_t vrf_id,
               struct bfd_peer_cfg *bpc, struct ptm_client **pc)
{
  uint32_t pid;
  size_t ifnamelen;
  ...
  *pc = pc_new (pid);
  ...
  bpc->bpc_has_txinterval = (bpc->bpc_txinterval != BPC_DEF_TRANSMITINTERVAL);
  ...
  if (vrf_id != VRF_DEFAULT)
    {
      ...
      if (vrf)
        ...
        {
          zlog_err ("ptm-read: vrf id %u could not be identified", vrf_id);
          return -1;
        }
    }
  else
  ...
  if (bpc->bpc_local.sa_sin.sin_family != AF_UNSPEC &&
      (bpc->bpc_local.sa_sin.sin_family != bpc->bpc_peer.sa_sin.sin_family))
    {
...
static void
bfdd_dest_register (struct stream *msg, vrf_id_t vrf_id)
{
      ...
      if (bs == NULL)
        {
          if (bglobal.debug_zebra)
            zlog_debug ("ptm-add-dest: failed to create BFD session");
          return;
        }
    }
  else
...
static void
bfdd_dest_deregister (struct stream *msg, vrf_id_t vrf_id)
{
  struct ptm_client *pc;
 ...
 */
static void
bfdd_client_register (struct stream *msg)
{
  uint32_t pid;
 ...
 */
static void
bfdd_client_deregister (struct stream *msg)
{
  struct ptm_client *pc;
  uint32_t pid;
...
static int
bfdd_replay (ZAPI_CALLBACK_ARGS)
{
...
static void
bfdd_zebra_connected (struct zclient *zc)
{
...
static void
bfdd_sessions_enable_interface (struct interface *ifp)
{
  ...
  TAILQ_FOREACH (bso, &bglobal.bg_obslist, bso_entry)
    {
      bs = bso->bso_bs;
      /* check vrf name */
      if (bs->key.vrfname[0] && strcmp (vrf->name, bs->key.vrfname))
        continue;
      ...
      if (vrf_is_backend_netns () || strcmp (ifp->name, vrf->name))
        {
          ...
          if (bs->key.ifname[0] && strcmp (ifp->name, bs->key.ifname))
            continue;
        }
...
static void
bfdd_sessions_disable_interface (struct interface *ifp)
{
  struct bfd_session_observer *bso;
  struct bfd_session *bs;
...
void
bfdd_sessions_enable_vrf (struct vrf *vrf)
{
  struct bfd_session_observer *bso;
  struct bfd_session *bs;
...
void
bfdd_sessions_disable_vrf (struct vrf *vrf)
{
  struct bfd_session_observer *bso;
  struct bfd_session *bs;
...
static int
bfd_ifp_destroy (struct interface *ifp)
{
  if (bglobal.debug_zebra)
    zlog_debug ("zclient: delete interface %s (VRF %s(%u))", ifp->name,
...
static int
bfdd_interface_vrf_update (ZAPI_CALLBACK_ARGS)
{
...
static void
bfdd_sessions_enable_address (struct connected *ifc)
{
  struct bfd_session_observer *bso;
  struct bfd_session *bs;
...
static int
bfdd_interface_address_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (bglobal.debug_zebra)
    zlog_debug ("zclient: %s local address %pFX (VRF %u)",
                cmd == ZEBRA_INTERFACE_ADDRESS_ADD ? "add" : "delete",
                ifc->address, vrf_id);
...
static int
bfd_ifp_create (struct interface *ifp)
{
  if (bglobal.debug_zebra)
...
void
bfdd_zclient_init (struct zebra_privs_t *bfdd_priv)
{
...
void
bfdd_zclient_register (vrf_id_t vrf_id)
{
...
void
bfdd_zclient_unregister (vrf_id_t vrf_id)
{
  if (! zclient || zclient->sock < 0)
    return;
...
static struct ptm_client_notification *
pcn_new (struct ptm_client *pc, struct bfd_session *bs)
{
  struct ptm_client_notification *pcn;
...
static struct ptm_client_notification *
pcn_lookup (struct ptm_client *pc, struct bfd_session *bs)
{
  struct ptm_client_notification *pcn;
...
static void
pcn_free (struct ptm_client_notification *pcn)
{
  struct ptm_client *pc;
  struct bfd_session *bs;
  ...
  if (bglobal.debug_zebra)
    zlog_debug ("ptm-del-session: [%s] refcount=%" PRIu64, bs_to_string (bs),
                bs->refcount);
