...
__display_peer_counters_json (struct bfd_session *bs);
static void _display_peer_counters_json (struct vty *vty,
                                         struct bfd_session *bs);
static void _display_peer_counter_iter (struct hash_bucket *hb, void *arg);
static void _display_peer_counter_json_iter (struct hash_bucket *hb,
                                             void *arg);
 ...
 */
static void
_display_peer_header (struct vty *vty, struct bfd_session *bs)
{
...
static void
_display_peer (struct vty *vty, struct bfd_session *bs)
{
  char buf[256];
  time_t now;
  uint32_t min = 0;
  ...
  vty_out (vty, "\t\tPeer Type: %s\n",
           CHECK_FLAG (bs->flags, BFD_SESS_FLAG_CONFIG) ? "configured"
                                                        : "dynamic");
...
static struct json_object *
_peer_json_header (struct bfd_session *bs)
{
...
static struct json_object *
__display_peer_json (struct bfd_session *bs)
{
  ...
  switch (bs->ses_state)
    ...
    case PTM_BFD_UP:
      ...
      json_object_int_add (jo, "uptime", monotime (NULL) - bs->uptime.tv_sec);
      break;
  ...
  json_object_string_add (jo, "remote-diagnostic", diag2str (bs->remote_diag));
  ...
  json_object_int_add (jo, "remote-echo-receive-interval",
                       bs->remote_timers.required_min_echo / 1000);
  json_object_int_add (jo, "remote-detect-multiplier", bs->remote_detect_mult);
...
static void
_display_peer_json (struct vty *vty, struct bfd_session *bs)
{
...
static void
_display_peer_iter (struct hash_bucket *hb, void *arg)
{
...
static void
_display_peer_json_iter (struct hash_bucket *hb, void *arg)
{
  struct bfd_vrf_tuple *bvt = (struct bfd_vrf_tuple *) arg;
...
static void
_display_all_peers (struct vty *vty, char *vrfname, bool use_json)
{
  struct json_object *jo;
...
static void
_display_peer_counter (struct vty *vty, struct bfd_session *bs)
{
  _display_peer_header (vty, bs);

  /* Ask data plane for updated counters. */
  if (bfd_dplane_update_session_counters (bs) == -1)
    zlog_debug ("%s: failed to update BFD session counters (%s)", __func__,
                bs_to_string (bs));
...
static void
_display_peer_counters_json (struct vty *vty, struct bfd_session *bs)
{
...
static void
_display_peer_counter_iter (struct hash_bucket *hb, void *arg)
{
...
static void
_display_peer_counter_json_iter (struct hash_bucket *hb, void *arg)
{
  struct json_object *jo, *jon = NULL;
  struct bfd_session *bs = hb->data;
  struct bfd_vrf_tuple *bvt = arg;
...
static void
_display_peers_counter (struct vty *vty, char *vrfname, bool use_json)
{
  struct json_object *jo;
  struct bfd_vrf_tuple bvt = { 0 };
...
static void
_clear_peer_counter (struct bfd_session *bs)
{
...
static void
_display_peer_brief (struct vty *vty, struct bfd_session *bs)
{
  char addr_buf[INET6_ADDRSTRLEN];
...
static void
_display_peer_brief_iter (struct hash_bucket *hb, void *arg)
{
  struct bfd_vrf_tuple *bvt = arg;
  struct vty *vty;
  struct bfd_session *bs = hb->data;
...
static void
_display_peers_brief (struct vty *vty, const char *vrfname, bool use_json)
{
  struct json_object *jo;
  struct bfd_vrf_tuple bvt = { 0 };
                     ...
                     const char *vrfname)
{
  ...
  if (bs == NULL)
    {
      vty_out (vty, "%% Unable to find 'peer %s", label ? label : peer_str);
      if (ifname)
...
void
_display_rtt (uint32_t *min, uint32_t *avg, uint32_t *max,
              struct bfd_session *bs)
{
#ifdef BFD_LINUX
  uint8_t i;
 ...
 */
static int
bfd_configure_peer (struct bfd_peer_cfg *bpc, bool mhop,
                    const struct sockaddr_any *peer,
  ...
  if (peer->sa_sin.sin_family == AF_INET)
    {
      if (local && local->sa_sin.sin_family != AF_INET)
        {
          snprintf (ebuf, ebuflen, "local is IPv6, but peer is IPv4");
          return -1;
        }
  ...
  else if (peer->sa_sin.sin_family == AF_INET6)
    {
      if (local && local->sa_sin.sin_family != AF_INET6)
        {
          snprintf (ebuf, ebuflen, "local is IPv4, but peer is IPv6");
          return -1;
        }
...
static int
bfdd_write_config (struct vty *vty)
{
