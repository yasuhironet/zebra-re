...
control_notifypeer_find (struct bfd_control_socket *bcs,
                         struct bfd_session *bs);
...
static void
control_free (struct bfd_control_socket *bcs)
{
  struct bfd_control_queue *bcq;
  struct bfd_notify_peer *bnp;
...
struct bfd_notify_peer *
control_notifypeer_new (struct bfd_control_socket *bcs, struct bfd_session *bs)
{
  struct bfd_notify_peer *bnp;
...
static void
control_notifypeer_free (struct bfd_control_socket *bcs,
                         struct bfd_notify_peer *bnp)
{
...
control_queue_new (struct bfd_control_socket *bcs)
{
  struct bfd_control_queue *bcq;
...
static void
control_queue_free (struct bfd_control_socket *bcs,
                    struct bfd_control_queue *bcq)
{
  control_reset_buf (&bcq->bcq_bcb);
...
static int
control_queue_dequeue (struct bfd_control_socket *bcs)
{
  struct bfd_control_queue *bcq;
  ...
  bcs->bcs_outev = NULL;
  event_add_write (master, control_write, bcs, bcs->bcs_sd, &bcs->bcs_outev);
...
static int
control_queue_enqueue (struct bfd_control_socket *bcs,
                       struct bfd_control_msg *bcm)
{
  struct bfd_control_queue *bcq;
...
static int
control_queue_enqueue_first (struct bfd_control_socket *bcs,
                             struct bfd_control_msg *bcm)
{
...
static void
control_reset_buf (struct bfd_control_buffer *bcb)
{
...
control_read (struct event *t)
{
  ...
  if (plen < 2)
    {
      zlog_debug ("%s: client closed due small message length: %d", __func__,
                  bcm.bcm_length);
      control_free (bcs);
      return;
    }
...
#define FRR_BFD_MAXLEN 10 * 1024
  ...
  if (plen > FRR_BFD_MAXLEN)
    {
      zlog_debug ("%s: client closed, invalid message length: %d", __func__,
                  bcm.bcm_length);
      control_free (bcs);
      return;
    }
  ...
  bcb->bcb_left = plen;
  bcb->bcb_buf =
      XMALLOC (MTYPE_BFDD_NOTIFICATION, sizeof (bcm) + bcb->bcb_left + 1);
  if (bcb->bcb_buf == NULL)
    {
      zlog_warn ("%s: not enough memory for message size: %zu", __func__,
                 bcb->bcb_left);
      control_free (bcs);
      return;
    }
 ...
 */
static void
control_handle_request_add (struct bfd_control_socket *bcs,
                            struct bfd_control_msg *bcm)
{
...
static void
control_handle_request_del (struct bfd_control_socket *bcs,
                            struct bfd_control_msg *bcm)
{
  const char *json = (const char *) bcm->bcm_data;
...
_notify_find_peer (struct bfd_peer_cfg *bpc)
{
...
static void
_control_handle_notify (struct hash_bucket *hb, void *arg)
{
...
static void
control_handle_notify (struct bfd_control_socket *bcs,
                       struct bfd_control_msg *bcm)
{
...
static int
notify_add_cb (struct bfd_peer_cfg *bpc, void *arg)
{
  struct bfd_control_socket *bcs = arg;
...
static int
notify_del_cb (struct bfd_peer_cfg *bpc, void *arg)
{
  struct bfd_control_socket *bcs = arg;
  struct bfd_session *bs = _notify_find_peer (bpc);
  struct bfd_notify_peer *bnp;
...
static void
control_handle_notify_add (struct bfd_control_socket *bcs,
                           struct bfd_control_msg *bcm)
{
  const char *json = (const char *) bcm->bcm_data;
...
static void
control_handle_notify_del (struct bfd_control_socket *bcs,
                           struct bfd_control_msg *bcm)
{
  const char *json = (const char *) bcm->bcm_data;
 ...
 */
static void
control_response (struct bfd_control_socket *bcs, uint16_t id,
                  const char *status, const char *error)
{
  struct bfd_control_msg *bcm;
  char *jsonstr;
  ...
  if (jsonstr == NULL)
    {
      zlog_warn ("%s: config_response: failed to get JSON str", __func__);
      return;
    }
...
static void
_control_notify (struct bfd_control_socket *bcs, struct bfd_session *bs)
{
  struct bfd_control_msg *bcm;
  char *jsonstr;
  size_t jsonstrlen;
  ...
  if (jsonstr == NULL)
    {
      zlog_warn ("%s: config_notify: failed to get JSON str", __func__);
      return;
    }
...
int
control_notify (struct bfd_session *bs, uint8_t notify_state)
{
  struct bfd_control_socket *bcs;
  struct bfd_notify_peer *bnp;
...
static void
_control_notify_config (struct bfd_control_socket *bcs, const char *op,
                        struct bfd_session *bs)
{
  struct bfd_control_msg *bcm;
  char *jsonstr;
  ...
  if (jsonstr == NULL)
    {
      zlog_warn ("%s: config_notify_config: failed to get JSON str", __func__);
      return;
    }
...
int
control_notify_config (const char *op, struct bfd_session *bs)
{
  struct bfd_control_socket *bcs;
  struct bfd_notify_peer *bnp;
