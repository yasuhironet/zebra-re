 ...
 */
static const char *
bfd_dplane_messagetype2str (enum bfddp_message_type bmt)
{
  switch (bmt)
    {
    case ECHO_REQUEST:
...
static void
bfd_dplane_debug_message (const struct bfddp_message *msg)
{
  ...
  switch (bmt)
    ...
    case DP_DELETE_SESSION:
      ...
      if (flags & SESSION_IPV6)
        snprintfrr (addrs, sizeof (addrs), "src=%pI6 dst=%pI6",
                    &msg->data.session.src, &msg->data.session.dst);
      else
      ...
      zlog_debug ("  [flags=0x%08x{%s} %s ttl=%d detect_mult=%d "
                  "ifindex=%d ifname=%s]",
                  flags, buf, addrs, msg->data.session.ttl,
                  msg->data.session.detect_mult,
                  ntohl (msg->data.session.ifindex), msg->data.session.ifname);
      break;
    ...
    case BFD_STATE_CHANGE:
      ...
      zlog_debug ("  [lid=%u rid=%u flags=0x%02x{%s} state=%s "
    ...
    case BFD_SESSION_COUNTERS:
      zlog_debug ("  [lid=%u "
                  "control{in %" PRIu64 " bytes (%" PRIu64 " packets), "
                  "out %" PRIu64 " bytes (%" PRIu64 " packets)} "
                  "echo{in %" PRIu64 " bytes (%" PRIu64 " packets), "
                  "out %" PRIu64 " bytes (%" PRIu64 " packets)}]",
 ...
 */
static uint16_t
bfd_dplane_next_id (struct bfd_dplane_ctx *bdc)
{
  bdc->last_id++;
...
static ssize_t
bfd_dplane_flush (struct bfd_dplane_ctx *bdc)
{
  ssize_t total = 0;
  int rv;
...
static void
bfd_dplane_write (struct event *t)
{
                                 ...
                                 const struct bfddp_state_change *state)
{
  ...
  if (bs == NULL)
    {
      if (bglobal.debug_dplane)
        zlog_debug ("%s: failed to find session to update", __func__);
      return;
    }
  ...
  switch (bs->ses_state)
    ...
    default:
      zlog_warn ("%s: unhandled new state %d", __func__, bs->ses_state);
      break;
    }

  if (bglobal.debug_peer_event)
    zlog_debug ("state-change: [data plane: %s] %s -> %s", bs_to_string (bs),
 ...
 */
static int
bfd_dplane_enqueue (struct bfd_dplane_ctx *bdc, const void *buf, size_t buflen)
{
  size_t rlen;
  ...
  if (bdc->outbufev == NULL)
    event_add_write (master, bfd_dplane_write, bdc, bdc->sock, &bdc->outbufev);

  return 0;
}

static void
bfd_dplane_echo_request_handle (struct bfd_dplane_ctx *bdc,
...
static void
bfd_dplane_handle_message (struct bfddp_message *msg, void *arg)
{
  enum bfddp_message_type bmt;
 ...
 */
static int
bfd_dplane_expect (struct bfd_dplane_ctx *bdc, uint16_t id,
...
read_again:
  ...
  rv = stream_read_try (bdc->inbuf, bdc->sock, STREAM_WRITEABLE (bdc->inbuf));
  if (rv == 0)
    {
      if (bglobal.debug_dplane)
  ...
  while (rlen > 0)
    {
      ...
      if (bh->version != BFD_DP_VERSION)
        {
          zlog_err ("%s: bad data plane client version: %d", __func__,
                    bh->version);
          return -1;
        }
      ...
      if (id && ntohs (bh->id) == id)
        ...
        bfd_dplane_handle_message ((struct bfddp_message *) bh, bdc);
...
static void
bfd_dplane_read (struct event *t)
{
  struct bfd_dplane_ctx *bdc = EVENT_ARG (t);
  int rv;
...
static void
_bfd_session_register_dplane (struct hash_bucket *hb, void *arg)
{
...
static void
_bfd_session_unregister_dplane (struct hash_bucket *hb, void *arg)
{
  struct bfd_session *bs = hb->data;
  struct bfd_dplane_ctx *bdc = arg;
...
static void
bfd_dplane_ctx_free (struct bfd_dplane_ctx *bdc)
{
  if (bglobal.debug_dplane)
    zlog_debug ("%s: terminating data plane client %d", __func__, bdc->sock);
...
static void
_bfd_dplane_session_fill (const struct bfd_session *bs,
                          struct bfddp_message *msg)
{
...
static int
_bfd_dplane_add_session (struct bfd_dplane_ctx *bdc, struct bfd_session *bs)
{
  int rv;
...
static void
_bfd_dplane_update_session_counters (struct bfddp_message *msg, void *arg)
{
 ...
 */
static uint16_t
bfd_dplane_request_counters (const struct bfd_session *bs)
{
  struct bfddp_message msg = {};
 ...
 */
static void
bfd_dplane_accept (struct event *t)
{
 ...
 */
static void
_bfd_dplane_client_bootstrap (struct bfd_dplane_ctx *bdc)
{
...
static bool
bfd_dplane_client_connecting (struct bfd_dplane_ctx *bdc)
{
  int rv;
  ...
  if (rv == 0)
    {
      if (bglobal.debug_dplane)
        zlog_debug ("%s: connected to server: %d", __func__, bdc->sock);
  ...
  switch (rv)
    ...
    default:
      zlog_warn ("%s: connection failed: %s", __func__, strerror (errno));
      bfd_dplane_ctx_free (bdc);
      return true;
    }
}
...
static void
bfd_dplane_client_connect (struct event *t)
{
  struct bfd_dplane_ctx *bdc = EVENT_ARG (t);
  int rv, sock;
  socklen_t rvlen = sizeof (rv);
  ...
  if (rv == -1)
    {
      if (bglobal.debug_dplane)
        zlog_debug ("%s: server connection in progress: %d", __func__, sock);
...
reschedule_connect:
  ...
  EVENT_OFF (bdc->outbufev);
  socket_close (&sock);
  event_add_timer (master, bfd_dplane_client_connect, bdc, 3, &bdc->connectev);
}

static void
bfd_dplane_client_init (const struct sockaddr *sa, socklen_t salen)
{
  struct bfd_dplane_ctx *bdc;
  ...
  if (salen <= sizeof (bdc->addr))
  ...
  else
    {
      ...
      bdc->addrlen = sizeof (bdc->addr);
      zlog_warn ("%s: server address truncated (from %d to %d)", __func__,
                 salen, bdc->addrlen);
    }

  bdc->client = true;

  event_add_timer (master, bfd_dplane_client_connect, bdc, 0, &bdc->connectev);
 ...
 */
void
bfd_dplane_init (const struct sockaddr *sa, socklen_t salen, bool client)
{
  int sock;
...
int
bfd_dplane_add_session (struct bfd_session *bs)
{
  struct bfd_dplane_ctx *bdc;
...
int
bfd_dplane_update_session (const struct bfd_session *bs)
{
  struct bfddp_message msg = {};
...
int
bfd_dplane_delete_session (struct bfd_session *bs)
{
  struct bfddp_message msg = {};
  int rv;
 ...
 */
void
bfd_dplane_show_counters (struct vty *vty)
{
  struct bfd_dplane_ctx *bdc;
  ...
  TAILQ_FOREACH (bdc, &bglobal.bg_dplaneq, entry)
    {
      ...
      SHOW_COUNTER ("Input current usage", STREAM_READABLE (bdc->inbuf), "zu");
      ...
      SHOW_COUNTER ("Output current usage", STREAM_READABLE (bdc->inbuf),
                    "zu");
      vty_out (vty, "\n");
    }
#undef SHOW_COUNTER
...
int
bfd_dplane_update_session_counters (struct bfd_session *bs)
{
  uint16_t id;
  int rv;
  ...
  do
    {
      rv = bfd_dplane_expect (bs->bdc, id, _bfd_dplane_update_session_counters,
                              bs);
    }
  while (rv == -2);
