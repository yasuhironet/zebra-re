...
void
bfd_cli_show_header (struct vty *vty,
...
void
bfd_cli_show_header_end (struct vty *vty, const struct lyd_node *dnode
...
DEFPY_YANG_NOSH (bfd_peer_enter,
                 bfd_peer_enter_cmd,
                 ...
                 VRF_NAME_STR)
{
  ...
  if (multihop)
    {
      ...
      if (ifname)
        {
          vty_out (vty, "%% interface is prohibited when using multihop\n");
          return CMD_WARNING_CONFIG_FAILED;
        }
  ...
  slen =
      snprintf (xpath, sizeof (xpath),
                ...
                multihop ? "multi-hop" : "single-hop", source_str, peer_str);
  if (ifname)
    slen += snprintf (xpath + slen, sizeof (xpath) - slen, "[interface='%s']",
                      ifname);
  else if (! multihop)
    slen += snprintf (xpath + slen, sizeof (xpath) - slen, "[interface='*']");
  if (vrf)
  ...
  if (multihop == NULL && local_address_str != NULL)
    {
      snprintf (xpath_srcaddr, sizeof (xpath_srcaddr), "%s/source-addr",
                xpath);
...
static void
_bfd_cli_show_peer (struct vty *vty, const struct lyd_node *dnode,
...
void
bfd_cli_show_single_hop_peer (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
...
void
bfd_cli_show_multi_hop_peer (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
...
void
bfd_cli_show_peer_end (struct vty *vty, const struct lyd_node *dnode
                       __attribute__ ((__unused__)))
{
  vty_out (vty, " exit\n");
  vty_out (vty, " !\n");
...
void
bfd_cli_show_shutdown (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
...
void
bfd_cli_show_passive (struct vty *vty, const struct lyd_node *dnode,
                      bool show_defaults)
{
...
DEFPY_YANG (bfd_peer_minimum_ttl,
            ...
            "Minimum TTL expected\n")
{
  ...
  if (no)
    ...
    nb_cli_enqueue_change (vty, "./minimum-ttl", NB_OP_MODIFY, ttl_str);
  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (no_bfd_peer_minimum_ttl,
...
void
bfd_cli_show_minimum_ttl (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
  vty_out (vty, "  minimum-ttl %s\n", yang_dnode_get_string (dnode, NULL));
}

DEFPY_YANG (bfd_peer_mult,
            bfd_peer_mult_cmd,
...
void
bfd_cli_show_mult (struct vty *vty, const struct lyd_node *dnode,
                   bool show_defaults)
{
...
void
bfd_cli_show_rx (struct vty *vty, const struct lyd_node *dnode,
                 bool show_defaults)
{
            ...
            "Configure peer transmit interval value in milliseconds\n")
{
  ...
  snprintf (value, sizeof (value), "%ld", interval * 1000);
  nb_cli_enqueue_change (vty, "./desired-transmission-interval", NB_OP_MODIFY,
                         value);
...
void
bfd_cli_show_tx (struct vty *vty, const struct lyd_node *dnode,
                 bool show_defaults)
{
  uint32_t value = yang_dnode_get_uint32 (dnode, NULL);
...
void
bfd_cli_show_echo (struct vty *vty, const struct lyd_node *dnode,
                   bool show_defaults)
{
...
DEFPY_YANG (bfd_peer_echo_interval,
            ...
            "Configure peer echo rx/tx intervals value in milliseconds\n")
{
  ...
  nb_cli_enqueue_change (vty, "./desired-echo-transmission-interval",
                         NB_OP_MODIFY, value);
  nb_cli_enqueue_change (vty, "./required-echo-receive-interval", NB_OP_MODIFY,
                         value);

  return nb_cli_apply_changes (vty, NULL);
}

DEFPY_YANG (bfd_peer_echo_transmit_interval,
...
void
bfd_cli_show_desired_echo_transmission_interval (struct vty *vty,
...
DEFPY_YANG (bfd_peer_echo_receive_interval,
...
void
bfd_cli_show_required_echo_receive_interval (struct vty *vty,
                                             const struct lyd_node *dnode,
                                             bool show_defaults)
{
  uint32_t value = yang_dnode_get_uint32 (dnode, NULL);
...
void
bfd_cli_show_profile (struct vty *vty, const struct lyd_node *dnode,
                      bool show_defaults)
{
  vty_out (vty, " profile %s\n", yang_dnode_get_string (dnode, "./name"));
}

ALIAS_YANG (bfd_peer_mult,
            bfd_profile_mult_cmd,
            "detect-multiplier (2-255)$multiplier",
            "Configure peer detection multiplier\n"
            "Configure peer detection multiplier value\n")

ALIAS_YANG (bfd_peer_tx,
            bfd_profile_tx_cmd,
            "transmit-interval (10-60000)$interval",
            "Configure peer transmit interval\n"
            "Configure peer transmit interval value in milliseconds\n")

ALIAS_YANG (bfd_peer_rx,
            bfd_profile_rx_cmd,
            "receive-interval (10-60000)$interval",
            "Configure peer receive interval\n"
            "Configure peer receive interval value in milliseconds\n")

ALIAS_YANG (bfd_peer_shutdown,
            bfd_profile_shutdown_cmd,
            "[no] shutdown",
            NO_STR
            "Disable BFD peer\n")

ALIAS_YANG (bfd_peer_passive,
            bfd_profile_passive_cmd,
            "[no] passive-mode",
            NO_STR
            "Don't attempt to start sessions\n")

ALIAS_YANG (bfd_peer_minimum_ttl,
...
ALIAS_YANG (no_bfd_peer_minimum_ttl,
...
ALIAS_YANG (bfd_peer_echo_interval,
            bfd_profile_echo_interval_cmd,
            "echo-interval (10-60000)$interval",
            "Configure peer echo interval\n"
            "Configure peer echo interval value in milliseconds\n")

ALIAS_YANG (bfd_peer_echo_transmit_interval,
...
ALIAS_YANG (bfd_peer_echo_receive_interval,
...
void
bfd_cli_peer_profile_show (struct vty *vty, const struct lyd_node *dnode,
                           bool show_defaults)
{
  vty_out (vty, "  profile %s\n", yang_dnode_get_string (dnode, NULL));
}
...
static void
bfd_profile_var (vector comps, struct cmd_token *token)
{
