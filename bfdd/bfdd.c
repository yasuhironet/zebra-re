...
/* BFDd privileges */
static zebra_capabilities_t _caps_p[] = { ZCAP_BIND, ZCAP_SYS_ADMIN,
                                          ZCAP_NET_RAW };
...
socket_close (int *s)
{
  ...
  if (close (*s) != 0)
    zlog_err ("%s: close(%d): (%d) %s", __func__, *s, errno, strerror (errno));
  ...
  if (strcmp (type, "unix") == 0 || strcmp (type, "unixc") == 0)
  ...
  else if (strcmp (type, "ipv4") == 0 || strcmp (type, "ipv4c") == 0)
    {
      ...
      if (inet_pton (AF_INET, addr, &sa.sin.sin_addr) != 1)
        errx (1, "%s: inet_pton: invalid address %s", __func__, addr);
    }
  else if (strcmp (type, "ipv6") == 0 || strcmp (type, "ipv6c") == 0)
    {
      ...
      sptr = &addr[0];
      if (*sptr != '[')
        errx (1, "%s: invalid IPv6 address format: %s", __func__, addr);
...
bg_init (void)
{
  ...
  memcpy (&bglobal.bfdd_privs, &bfdd_privs, sizeof (bfdd_privs));
}
