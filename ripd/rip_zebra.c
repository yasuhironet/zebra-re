...
static void
rip_zebra_ipv4_send (struct rip *rip, struct route_node *rp, uint8_t cmd)
{
...
void
rip_zebra_ipv4_add (struct rip *rip, struct route_node *rp)
{
...
void
rip_zebra_ipv4_delete (struct rip *rip, struct route_node *rp)
{
...
static int
rip_zebra_read_route (ZAPI_CALLBACK_ARGS)
{
  struct rip *rip;
  struct zapi_route api;
  struct nexthop nh;
  ...
  if (cmd == ZEBRA_REDISTRIBUTE_ROUTE_ADD)
    rip_redistribute_add (rip, api.type, RIP_ROUTE_REDISTRIBUTE,
                          (struct prefix_ipv4 *) &api.prefix, &nh, api.metric,
...
void
rip_redistribute_conf_update (struct rip *rip, int type)
{
...
void
rip_redistribute_conf_delete (struct rip *rip, int type)
{
  if (zclient->sock > 0)
    zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE, zclient, AFI_IP, type,
                             0, rip->vrf->vrf_id);
...
int
rip_redistribute_check (struct rip *rip, int type)
{
  return rip->redist[type].enabled;
}

void
rip_redistribute_enable (struct rip *rip)
{
  for (int i = 0; i < ZEBRA_ROUTE_MAX; i++)
    {
      ...
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient, AFI_IP, i, 0,
                               rip->vrf->vrf_id);
    }
}

void
rip_redistribute_disable (struct rip *rip)
{
  for (int i = 0; i < ZEBRA_ROUTE_MAX; i++)
    {
      if (! rip_redistribute_check (rip, i))
      ...
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE, zclient, AFI_IP, i,
                               0, rip->vrf->vrf_id);
    }
}

void
rip_show_redistribute_config (struct vty *vty, struct rip *rip)
{
  for (int i = 0; i < ZEBRA_ROUTE_MAX; i++)
    {
      if (i == zclient->redist_default || ! rip_redistribute_check (rip, i))
        continue;
...
void
rip_zebra_vrf_register (struct vrf *vrf)
{
  if (vrf->vrf_id == VRF_DEFAULT)
    return;

  if (IS_RIP_DEBUG_EVENT)
    zlog_debug ("%s: register VRF %s(%u) to zebra", __func__, vrf->name,
                vrf->vrf_id);
...
void
rip_zebra_vrf_deregister (struct vrf *vrf)
{
  if (vrf->vrf_id == VRF_DEFAULT)
    return;

  if (IS_RIP_DEBUG_EVENT)
    zlog_debug ("%s: deregister VRF %s(%u) from zebra.", __func__, vrf->name,
                vrf->vrf_id);
...
static void
rip_zebra_capabilities (struct zclient_capabilities *cap)
{
  zebra_ecmp_count = MIN (cap->ecmp, zebra_ecmp_count);
}

void
rip_zclient_init (struct event_loop *master)
{
