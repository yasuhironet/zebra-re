                 ...
                 VRF_CMD_HELP_STR)
{
  ...
  snprintf (xpath, sizeof (xpath), "/frr-ripd:ripd/instance[vrf='%s']", vrf);
...
void
cli_show_router_rip (struct vty *vty, const struct lyd_node *dnode,
                     bool show_defaults)
{
  const char *vrf_name;
...
void
cli_show_rip_allow_ecmp (struct vty *vty, const struct lyd_node *dnode,
                         bool show_defaults)
{
  uint8_t paths;
...
{
  nb_cli_enqueue_change (vty, "./default-information-originate", NB_OP_MODIFY,
                         no ? "false" : "true");
...
void
cli_show_rip_default_information_originate (struct vty *vty,
...
void
cli_show_rip_default_metric (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
...
void
cli_show_rip_distance (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
  ...
  if (! no)
    {
      ...
      nb_cli_enqueue_change (vty, "./distance", NB_OP_MODIFY, distance_str);
...
void
cli_show_rip_distance_source (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
  vty_out (vty, " distance %s %s", yang_dnode_get_string (dnode, "./distance"),
...
void
cli_show_rip_neighbor (struct vty *vty, const struct lyd_node *dnode,
                       bool show_defaults)
{
...
{
  nb_cli_enqueue_change (vty, "./network", no ? NB_OP_DESTROY : NB_OP_CREATE,
                         network_str);
...
void
cli_show_rip_network_prefix (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
...
DEFPY_YANG (rip_network_if,
            ...
            "Interface name\n")
{
  nb_cli_enqueue_change (vty, "./interface", no ? NB_OP_DESTROY : NB_OP_CREATE,
                         network);
...
void
cli_show_rip_network_interface (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
  vty_out (vty, " network %s\n", yang_dnode_get_string (dnode, NULL));
}
...
void
cli_show_rip_offset_list (struct vty *vty, const struct lyd_node *dnode,
                          bool show_defaults)
{
...
void
cli_show_rip_passive_default (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_rip_passive_interface (struct vty *vty, const struct lyd_node *dnode,
                                bool show_defaults)
{
...
void
cli_show_rip_non_passive_interface (struct vty *vty,
                                    const struct lyd_node *dnode,
                                    bool show_defaults)
{
            ...
            "Pointer to route-map entries\n")
{
  ...
  return nb_cli_apply_changes (vty, "./redistribute[protocol='%s']", protocol);
}

void
cli_show_rip_redistribute (struct vty *vty, const struct lyd_node *dnode,
                           bool show_defaults)
{
...
void
cli_show_rip_route (struct vty *vty, const struct lyd_node *dnode,
                    bool show_defaults)
{
  vty_out (vty, " route %s\n", yang_dnode_get_string (dnode, NULL));
}
...
{
  nb_cli_enqueue_change (vty, "./update-interval", NB_OP_MODIFY, update_str);
  nb_cli_enqueue_change (vty, "./holddown-interval", NB_OP_MODIFY,
                         timeout_str);
  nb_cli_enqueue_change (vty, "./flush-interval", NB_OP_MODIFY, garbage_str);
...
void
cli_show_rip_timers (struct vty *vty, const struct lyd_node *dnode,
                     bool show_defaults)
{
...
DEFPY_YANG (rip_version,
            ...
            "version\n")
{
  nb_cli_enqueue_change (vty, "./version/receive", NB_OP_MODIFY, version_str);
...
void
cli_show_rip_version (struct vty *vty, const struct lyd_node *dnode,
                      bool show_defaults)
{
  /*
...
DEFPY_YANG (rip_bfd_default_profile,
            ...
            "Profile name\n")
{
  nb_cli_enqueue_change (vty, "./default-bfd-profile", NB_OP_MODIFY, profile);
...
void
cli_show_ripd_instance_default_bfd_profile (struct vty *vty,
                                            const struct lyd_node *dnode,
                                            bool show_defaults)
{
...
void
cli_show_ip_rip_split_horizon (struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
  int value;
...
void
cli_show_ip_rip_v2_broadcast (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
  if (! yang_dnode_get_bool (dnode, NULL))
    vty_out (vty, " no");
...
void
cli_show_ip_rip_receive_version (struct vty *vty, const struct lyd_node *dnode,
                                 bool show_defaults)
{
...
void
cli_show_ip_rip_send_version (struct vty *vty, const struct lyd_node *dnode,
                              bool show_defaults)
{
  switch (yang_dnode_get_enum (dnode, NULL))
    {
...
void
cli_show_ip_rip_authentication_scheme (struct vty *vty,
                                       const struct lyd_node *dnode,
                                       bool show_defaults)
{
            ...
            "Authentication string\n")
{
  ...
  if (yang_dnode_existsf (vty->candidate_config->dnode, "%s%s", VTY_CURR_XPATH,
...
void
cli_show_ip_rip_authentication_string (struct vty *vty,
                                       const struct lyd_node *dnode,
                                       bool show_defaults)
{
...
void
cli_show_ip_rip_authentication_key_chain (struct vty *vty,
                                          const struct lyd_node *dnode,
                                          bool show_defaults)
{
...
void
cli_show_ip_rip_bfd_enable (struct vty *vty, const struct lyd_node *dnode,
                            bool show_defaults)
{
            ...
            "Profile name\n")
{
  if (no)
    ...
    nb_cli_enqueue_change (vty, "./bfd-monitoring/profile", NB_OP_MODIFY,
                           profile);

  return nb_cli_apply_changes (vty, "./frr-ripd:rip");
}

DEFPY_YANG (no_ip_rip_bfd_profile,
            ...
            "Use a pre-configured profile\n")
{
  nb_cli_enqueue_change (vty, "./bfd-monitoring/profile", NB_OP_DESTROY, NULL);
  return nb_cli_apply_changes (vty, "./frr-ripd:rip");
}

void
cli_show_ip_rip_bfd_profile (struct vty *vty, const struct lyd_node *dnode,
                             bool show_defaults)
{
  ...
  if (vrf)
    {
      ...
      yang_vrf = yang_data_new ("/frr-ripd:clear-rip-route/input/vrf", vrf);
       ...
       "Interface name\n")
{
  ...
  return distribute_list_no_parser (vty, prefix, true, argv[3 + prefix]->text,
