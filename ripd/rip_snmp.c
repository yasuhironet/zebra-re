...
static uint8_t *rip2IfConfAddress (struct variable *, oid[], size_t *, int,
                                   size_t *, WriteMethod **);
static uint8_t *rip2PeerTable (struct variable *, oid[], size_t *, int,
                               size_t *, WriteMethod **);
...
static uint8_t *
rip2Globals (struct variable *v, oid name[], size_t *length, int exact,
  ...
  if (exact)
    {
      ...
      return if_lookup_address_local ((void *) addr, AF_INET, VRF_DEFAULT);
    }
  else
    {
...
static uint8_t *
rip2IfStatEntry (struct variable *v, oid name[], size_t *length, int exact,
                 size_t *var_len, WriteMethod **write_method)
{
  struct interface *ifp;
...
static uint8_t *
rip2IfConfAddress (struct variable *v, oid name[], size_t *length, int exact,
...
static uint8_t *
rip2PeerTable (struct variable *v, oid name[], size_t *length, int exact,
               size_t *val_len, WriteMethod **write_method)
{
  static struct in_addr addr;
  static int domain = 0;
...
/* Register RIPv2-MIB. */
static int
rip_snmp_init (struct event_loop *master)
{
