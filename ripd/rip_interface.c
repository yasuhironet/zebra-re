                     ...
                     ifindex_t ifindex)
{
  ...
  ret = setsockopt_ipv4_multicast (sock, IP_ADD_MEMBERSHIP, ifa, group.s_addr,
                                   ifindex);
                      ...
                      ifindex_t ifindex)
{
  ...
  ret = setsockopt_ipv4_multicast (sock, IP_DROP_MEMBERSHIP, ifa, group.s_addr,
                                   ifindex);
...
static void
rip_request_interface_send (struct interface *ifp, uint8_t version)
{
      ...
      for (ALL_LIST_ELEMENTS (ifp->connected, cnode, cnnode, connected))
        {
          ...
          else if (connected->address->prefixlen < IPV4_MAX_BITLEN)
          ...
          if (IS_RIP_DEBUG_EVENT)
            zlog_debug ("SEND request to %pI4", &to.sin_addr);
...
rip_request_interface (struct interface *ifp)
{
  ...
  vsend =
      ((ri->ri_send == RI_RIP_UNSPEC) ? ri->rip->version_send : ri->ri_send);
  if (vsend & RIPv1)
...
int
if_check_address (struct rip *rip, struct in_addr addr)
{
...
static int
rip_ifp_down (struct interface *ifp)
{
...
static int
rip_ifp_up (struct interface *ifp)
{
  if (IS_RIP_DEBUG_ZEBRA)
    zlog_debug (
...
static int
rip_ifp_create (struct interface *ifp)
{
  rip_interface_sync (ifp);
...
static int
rip_ifp_destroy (struct interface *ifp)
{
  rip_interface_sync (ifp);
  if (if_is_up (ifp))
    {
...
int
rip_interface_vrf_update (ZAPI_CALLBACK_ARGS)
{
  struct interface *ifp;
  vrf_id_t new_vrf_id;
...
void
rip_interfaces_clean (struct rip *rip)
{
  struct interface *ifp;
...
{
  ri->auth_type =
      yang_get_default_enum ("%s/authentication-scheme/mode", RIP_IFACE);
  ...
  ri->split_horizon = yang_get_default_enum ("%s/split-horizon", RIP_IFACE);
...
rip_apply_address_add (struct connected *ifc)
{
  ...
  if ((rip_enable_if_lookup (rip, ifc->ifp->name) >= 0) ||
    ...
    rip_redistribute_add (rip, ZEBRA_ROUTE_CONNECT, RIP_ROUTE_INTERFACE,
                          &address, &nh, 0, 0, 0);
}

int
rip_interface_address_add (ZAPI_CALLBACK_ARGS)
{
  struct connected *ifc;
  struct prefix *p;
...
int
rip_interface_address_delete (ZAPI_CALLBACK_ARGS)
{
  struct connected *ifc;
  struct prefix *p;
  ...
  if (ifc)
    {
      ...
      if (p->family == AF_INET)
        {
          if (IS_RIP_DEBUG_ZEBRA)
            zlog_debug ("connected address %pFX is deleted", p);
  ...
  if (p->family == AF_INET)
    {
       ...
       * rip->enable_network */
      node =
          route_node_match (rip->enable_network, (struct prefix *) &address);
...
int
rip_enable_network_add (struct rip *rip, struct prefix *p)
{
  struct route_node *node;
...
int
rip_enable_network_delete (struct rip *rip, struct prefix *p)
{
  struct route_node *node;
...
static int
rip_enable_if_lookup (struct rip *rip, const char *ifname)
{
  unsigned int i;
  char *str;
...
int
rip_enable_if_delete (struct rip *rip, const char *ifname)
{
  int index;
  char *str;
...
static void
rip_interface_wakeup (struct event *t)
{
  struct interface *ifp;
  struct rip_interface *ri;
      ...
      if (set)
        {
          ...
          if ((rip_enable_if_lookup (rip, connected->ifp->name) >= 0) ||
            ...
            rip_redistribute_add (rip, ZEBRA_ROUTE_CONNECT,
                                  RIP_ROUTE_INTERFACE, &address, &nh, 0, 0, 0);
        }
      else
        {
          ...
          if (rip_redistribute_check (rip, ZEBRA_ROUTE_CONNECT))
            rip_redistribute_add (rip, ZEBRA_ROUTE_CONNECT,
                                  RIP_ROUTE_REDISTRIBUTE, &address, &nh, 0, 0,
                                  0);
        }
    }
}
  ...
  if (ri->enable_network || ri->enable_interface)
    {
      ...
      event_add_timer (master, rip_interface_wakeup, ifp, 1, &ri->t_wakeup);
...
static void
rip_enable_apply_all (struct rip *rip)
{
  struct interface *ifp;
...
int
rip_neighbor_lookup (struct rip *rip, struct sockaddr_in *from)
{
  struct prefix_ipv4 p;
  struct route_node *node;
...
int
rip_neighbor_add (struct rip *rip, struct prefix_ipv4 *p)
{
  struct route_node *node;
...
int
rip_neighbor_delete (struct rip *rip, struct prefix_ipv4 *p)
{
  struct route_node *node;
...
void
rip_clean_network (struct rip *rip)
{
  unsigned int i;
  char *str;
...
static int
rip_passive_nondefault_lookup (struct rip *rip, const char *ifname)
{
  unsigned int i;
  char *str;
...
static void
rip_passive_interface_apply_all (struct rip *rip)
{
  struct interface *ifp;
...
/* Passive interface. */
int
rip_passive_nondefault_set (struct rip *rip, const char *ifname)
{
...
int
rip_passive_nondefault_unset (struct rip *rip, const char *ifname)
{
  int i;
  char *str;
...
void
rip_passive_nondefault_clean (struct rip *rip)
{
  unsigned int i;
  char *str;
...
int
rip_show_network_config (struct vty *vty, struct rip *rip)
{
  unsigned int i;
  char *ifname;
  struct route_node *node;
  ...
  for (node = route_top (rip->enable_network); node; node = route_next (node))
    if (node->info)
      vty_out (vty, "    %pFX\n", &node->p);
...
void
rip_interface_sync (struct interface *ifp)
{
  struct rip_interface *ri;
...
rip_if_init (void)
{
  ...
  if_cmd_init_default ();
  if_zapi_callbacks (rip_ifp_create, rip_ifp_up, rip_ifp_down,
                     rip_ifp_destroy);
}
