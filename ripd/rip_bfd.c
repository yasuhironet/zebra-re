...
static const char *
rip_bfd_interface_profile (struct rip_interface *ri)
{
...
static void
rip_bfd_session_change (struct bfd_session_params *bsp,
                        const struct bfd_session_status *bss, void *arg)
{
  ...
  if (bss->state == BFD_STATUS_DOWN && bss->previous_state == BFD_STATUS_UP)
    {
      if (IS_RIP_DEBUG_EVENT)
        zlog_debug ("%s: peer %pI4: BFD Down", __func__, &rp->addr);
  ...
  if (bss->state == BSS_UP && bss->previous_state == BSS_DOWN)
    if (IS_RIP_DEBUG_EVENT)
      zlog_debug ("%s: peer %pI4: BFD Up", __func__, &rp->addr);
}

void
rip_bfd_session_update (struct rip_peer *rp)
{
  ...
  bfd_sess_set_timers (rp->bfd_session, BFD_DEF_DETECT_MULT, BFD_DEF_MIN_RX,
                       BFD_DEF_MIN_TX);
...
void
rip_bfd_interface_update (struct rip_interface *ri)
{
  struct rip *rip;
  struct rip_peer *rp;
...
void
rip_bfd_instance_update (struct rip *rip)
{
...
void
rip_bfd_init (struct event_loop *tm)
{
