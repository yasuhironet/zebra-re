...
route_match_interface (void *rule, const struct prefix *prefix, void *object)
{
  ...
  if (rinfo->ifindex_out == ifp->ifindex || rinfo->nh.ifindex == ifp->ifindex)
    return RMAP_MATCH;
  else
    return RMAP_NOMATCH;
}
...
route_match_ip_next_hop (void *rule, const struct prefix *prefix, void *object)
{
  ...
  rinfo = object;
  p.family = AF_INET;
  p.prefix = (rinfo->nh.gate.ipv4.s_addr != INADDR_ANY) ? rinfo->nh.gate.ipv4
                                                        : rinfo->from;
...
route_set_metric_compile (const char *arg)
{
  ...
  mod =
      XMALLOC (MTYPE_ROUTE_MAP_COMPILED, sizeof (struct rip_metric_modifier));
  mod->used = false;
...
static const struct route_map_rule_cmd route_set_tag_cmd = {
  "tag", route_set_tag, route_map_rule_tag_compile, route_map_rule_tag_free
};
