...
struct rip_peer *
rip_peer_lookup (struct rip *rip, struct in_addr *addr)
{
...
struct rip_peer *
rip_peer_lookup_next (struct rip *rip, struct in_addr *addr)
{
  struct rip_peer *peer;
  struct listnode *node, *nnode;
...
/* RIP peer is timeout. */
static void
rip_peer_timeout (struct event *t)
{
  struct rip_peer *peer;
...
static struct rip_peer *
rip_peer_get (struct rip *rip, struct rip_interface *ri, struct in_addr *addr)
{
  struct rip_peer *peer;
...
void
rip_peer_update (struct rip *rip, struct rip_interface *ri,
...
void
rip_peer_bad_route (struct rip *rip, struct rip_interface *ri,
...
void
rip_peer_bad_packet (struct rip *rip, struct rip_interface *ri,
                     struct sockaddr_in *from)
{
  struct rip_peer *peer;
  peer = rip_peer_get (rip, ri, &from->sin_addr);
...
void
rip_peer_display (struct vty *vty, struct rip *rip)
{
  struct rip_peer *peer;
  struct listnode *node, *nnode;
...
void
rip_peer_list_del (void *arg)
{
  rip_peer_free (arg);
}

void
rip_peer_delete_routes (const struct rip_peer *peer)
{
  ...
  for (route_node = route_top (peer->rip->table); route_node;
       route_node = route_next (route_node))
    {
      ...
      for (ALL_LIST_ELEMENTS (list, listnode, listnode_next, route_entry))
        {
          ...
          if (listcount (list) == 1)
            {
              ...
              if (list_isempty (list))
                {
                  list_delete ((struct list **) &route_node->info);
