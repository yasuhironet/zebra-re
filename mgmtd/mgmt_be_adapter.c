...
static void
mgmt_register_client_xpath (enum mgmt_be_client_id id, const char *xpath,
                            uint subscribed)
{
      ...
      for (; init < end; init++)
        {
          ...
          mgmt_register_client_xpath (id, init->xpath, init->subscribed);
        }
    }
...
static void
mgmt_be_xpath_map_cleanup (void)
{
  struct mgmt_be_xpath_map *map;
...
static int
mgmt_be_eval_regexp_match (const char *xpath_regexp, const char *xpath)
{
  ...
  for (re_indx = 0, xp_indx = 0;
       match && re_indx < rexp_len && xp_indx < xpath_len;)
    {
          ...
          (xpath_regexp[re_indx] == '*' || xpath[xp_indx] == '*'))
        {
          ...
          enter_wild_match =
              (xpath_regexp[re_indx - 1] == '/' ||
               xpath_regexp[re_indx - 1] == '\'' ||
               xpath[xp_indx - 1] == '/' || xpath[xp_indx - 1] == '\'');
          if (enter_wild_match)
            {
      ...
      if (enter_wild_match)
        {
          ...
          else if (xp_wild && xpath_regexp[re_indx] == wild_delim)
            {
              /*
      ...
      match = (xp_wild || re_wild || xpath_regexp[re_indx] == xpath[xp_indx]);

      /*
       * Check if we found a delimiter in both the Xpaths
       */
      if ((xpath_regexp[re_indx] == '/' && xpath[xp_indx] == '/') ||
          (xpath_regexp[re_indx] == ']' && xpath[xp_indx] == ']') ||
          (xpath_regexp[re_indx] == '[' && xpath[xp_indx] == '['))
        {
          /*
   ...
   */
  if (match && ! delim &&
      (xpath_regexp[re_indx] == '/' || xpath_regexp[re_indx] == ']'))
    match_len++;
...
static void
mgmt_be_adapter_delete (struct mgmt_be_client_adapter *adapter)
{
...
static int
mgmt_be_adapter_notify_disconnect (struct msg_conn *conn)
{
...
static int
mgmt_be_adapter_send_msg (struct mgmt_be_client_adapter *adapter,
...
static int
mgmt_be_send_subscr_reply (struct mgmt_be_client_adapter *adapter,
                           bool success)
{
  Mgmtd__BeMessage be_msg;
                            ...
                            Mgmtd__BeMessage *be_msg)
{
  ...
  switch ((int) be_msg->message_case)
    ...
    case MGMTD__BE_MESSAGE__MESSAGE_TXN_REPLY:
      MGMTD_BE_ADAPTER_DBG (
          "Got %s TXN_REPLY from '%s' txn-id %" PRIx64 " with '%s'",
       ...
       */
      mgmt_txn_notify_be_txn_reply (be_msg->txn_reply->txn_id,
                                    be_msg->txn_reply->create,
                                    be_msg->txn_reply->success, adapter);
      break;
    case MGMTD__BE_MESSAGE__MESSAGE_CFG_DATA_REPLY:
      MGMTD_BE_ADAPTER_DBG ("Got CFGDATA_REPLY from '%s' txn-id %" PRIx64
      ...
      mgmt_txn_notify_be_cfgdata_reply (
          be_msg->cfg_data_reply->txn_id, be_msg->cfg_data_reply->batch_id,
    ...
    case MGMTD__BE_MESSAGE__MESSAGE_CFG_APPLY_REPLY:
      MGMTD_BE_ADAPTER_DBG (
          ...
          be_msg->cfg_apply_reply
              ->batch_ids[be_msg->cfg_apply_reply->n_batch_ids - 1],
      ...
      mgmt_txn_notify_be_cfg_apply_reply (
          be_msg->cfg_apply_reply->txn_id, be_msg->cfg_apply_reply->success,
...
int
mgmt_be_send_txn_req (struct mgmt_be_client_adapter *adapter, uint64_t txn_id,
                      bool create)
{
  Mgmtd__BeMessage be_msg;
...
int
mgmt_be_send_cfgdata_req (struct mgmt_be_client_adapter *adapter,
                          ...
                          size_t num_reqs, bool end_of_data)
{
  ...
  MGMTD_BE_ADAPTER_DBG ("Sending CFGDATA_CREATE_REQ to '%s' txn-id: %" PRIu64
                        " batch-id: %" PRIu64,
...
int
mgmt_be_send_cfgapply_req (struct mgmt_be_client_adapter *adapter,
                           uint64_t txn_id)
{
  Mgmtd__BeMessage be_msg;
...
static void
mgmt_be_adapter_process_msg (uint8_t version, uint8_t *data, size_t len,
                             struct msg_conn *conn)
{
  ...
  if (! be_msg)
    {
      MGMTD_BE_ADAPTER_DBG ("Failed to decode %zu bytes for adapter: %s", len,
                            adapter->name);
      return;
    }
...
static void
mgmt_be_iter_and_get_cfg (const char *xpath, struct lyd_node *node,
                          struct nb_node *nb_node, void *ctx)
{
  ...
  subscr = mgmt_be_get_subscr_for_xpath_and_client (xpath, adapter->id,
 ...
 */
static void
mgmt_be_adapter_conn_init (struct event *thread)
{
  struct mgmt_be_client_adapter *adapter;
...
void
mgmt_be_adapter_lock (struct mgmt_be_client_adapter *adapter)
{
  adapter->refcount++;
}

extern void
mgmt_be_adapter_unlock (struct mgmt_be_client_adapter **adapter)
{
 ...
 */
void
mgmt_be_adapter_init (struct event_loop *tm)
{
  assert (! mgmt_loop);
  mgmt_loop = tm;
...
mgmt_be_create_adapter (int conn_fd, union sockunion *from)
{
  ...
  adapter =
      XCALLOC (MTYPE_MGMTD_BE_ADPATER, sizeof (struct mgmt_be_client_adapter));
  adapter->id = MGMTD_BE_CLIENT_ID_MAX;
  ...
  adapter->conn = msg_server_conn_create (
      ...
      MGMTD_BE_MAX_NUM_MSG_WRITE, MGMTD_BE_MSG_MAX_LEN, adapter, "BE-adapter");

  MGMTD_BE_ADAPTER_DBG ("Added new MGMTD Backend adapter '%s'", adapter->name);
...
void
mgmt_be_get_subscr_info_for_xpath (
 ...
 */
static uint
mgmt_be_get_subscr_for_xpath_and_client (const char *xpath,
                                         enum mgmt_be_client_id client_id,
                                         uint subscr_mask)
{
  ...
  for (i = 0; i < map->nxpaths; i++)
    {
      if (! mgmt_be_eval_regexp_match (map->xpaths[i].xpath, xpath))
        continue;
      MGMTD_BE_ADAPTER_DBG ("xpath: %s: matched: %s", map->xpaths[i].xpath,
                            xpath);
...
void
mgmt_be_adapter_status_write (struct vty *vty)
{
  struct mgmt_be_client_adapter *adapter;
...
void
mgmt_be_xpath_register_write (struct vty *vty)
{
  struct mgmt_be_xpath_map *map;
  enum mgmt_be_client_id id;
  struct mgmt_be_client_adapter *adapter;
      ...
      FOREACH_MGMTD_BE_CLIENT_ID (id)
        {
          ...
          if (! info)
            continue;
          vty_out (vty, "   -- Client: '%s'\tValidate:%d, Notify:%d, Own:%d\n",
                   mgmt_be_client_id2name (id),
...
void
mgmt_be_xpath_subscr_info_write (struct vty *vty, const char *xpath)
{
