                          ...
                          struct mgmt_be_client_adapter *be_adapter)
{
  ...
  mgmt_txn_batches_add_tail (
      &txn->commit_cfg_req->req.commit_cfg.curr_batches[id], batch);
...
static void
mgmt_txn_cfg_batch_free (struct mgmt_txn_be_cfg_batch **batch)
{
  size_t indx;
...
static unsigned int
mgmt_txn_cfgbatch_hash_key (const void *data)
{
...
static bool
mgmt_txn_cfgbatch_hash_cmp (const void *d1, const void *d2)
{
...
static void
mgmt_txn_cfgbatch_hash_free (void *data)
{
...
static void
mgmt_txn_cleanup_be_cfg_batches (struct mgmt_txn_ctx *txn,
...
mgmt_txn_req_alloc (struct mgmt_txn_ctx *txn, uint64_t req_id,
                    enum mgmt_txn_event req_event)
{
  ...
  switch (txn_req->req_event)
    ...
    case MGMTD_TXN_PROC_SETCFG:
      ...
      MGMTD_TXN_DBG ("Added a new SETCFG req-id: %" PRIu64 " txn-id: %" PRIu64
                     ", session-id: %" PRIu64,
    ...
    case MGMTD_TXN_PROC_COMMITCFG:
      ...
      FOREACH_MGMTD_BE_CLIENT_ID (id)
        {
          mgmt_txn_batches_init (&txn_req->req.commit_cfg.curr_batches[id]);
          mgmt_txn_batches_init (&txn_req->req.commit_cfg.next_batches[id]);
        }

      txn_req->req.commit_cfg.batches =
          hash_create (mgmt_txn_cfgbatch_hash_key, mgmt_txn_cfgbatch_hash_cmp,
                       "MGMT Config Batches");
      break;
    case MGMTD_TXN_PROC_GETCFG:
      txn_req->req.get_data = XCALLOC (MTYPE_MGMTD_TXN_GETDATA_REQ,
                                       sizeof (struct mgmt_get_data_req));
      assert (txn_req->req.get_data);
      mgmt_txn_reqs_add_tail (&txn->get_cfg_reqs, txn_req);
      MGMTD_TXN_DBG ("Added a new GETCFG req-id: %" PRIu64 " txn-id: %" PRIu64
                     " session-id: %" PRIu64,
                     txn_req->req_id, txn->txn_id, txn->session_id);
      break;
    case MGMTD_TXN_PROC_GETDATA:
      ...
      MGMTD_TXN_DBG ("Added a new GETDATA req-id: %" PRIu64 " txn-id: %" PRIu64
                     " session-id: %" PRIu64,
                     txn_req->req_id, txn->txn_id, txn->session_id);
      break;
...
static void
mgmt_txn_req_free (struct mgmt_txn_req **txn_req)
{
  int indx;
  ...
  switch ((*txn_req)->req_event)
    {
    case MGMTD_TXN_PROC_SETCFG:
      for (indx = 0; indx < (*txn_req)->req.set_cfg->num_cfg_changes; indx++)
        {
          if ((*txn_req)->req.set_cfg->cfg_changes[indx].value)
            {
              MGMTD_TXN_DBG ("Freeing value for %s at %p ==> '%s'",
                             (*txn_req)->req.set_cfg->cfg_changes[indx].xpath,
                             (*txn_req)->req.set_cfg->cfg_changes[indx].value,
                             (*txn_req)->req.set_cfg->cfg_changes[indx].value);
              free ((void *) (*txn_req)->req.set_cfg->cfg_changes[indx].value);
            }
        }
      req_list = &(*txn_req)->txn->set_cfg_reqs;
      MGMTD_TXN_DBG ("Deleting SETCFG req-id: %" PRIu64 " txn-id: %" PRIu64,
                     (*txn_req)->req_id, (*txn_req)->txn->txn_id);
      XFREE (MTYPE_MGMTD_TXN_SETCFG_REQ, (*txn_req)->req.set_cfg);
      break;
    case MGMTD_TXN_PROC_COMMITCFG:
      MGMTD_TXN_DBG ("Deleting COMMITCFG req-id: %" PRIu64 " txn-id: %" PRIu64,
                     (*txn_req)->req_id, (*txn_req)->txn->txn_id);
      ...
      FOREACH_MGMTD_BE_CLIENT_ID (id)
        {
          ...
          if (ccreq->batches)
            {
              hash_clean (ccreq->batches, mgmt_txn_cfgbatch_hash_free);
          ...
          if (adapter && cleanup && ccreq->subscr_info.xpath_subscr[id])
            mgmt_txn_send_be_txn_delete ((*txn_req)->txn, adapter);
        }
      break;
    case MGMTD_TXN_PROC_GETCFG:
      for (indx = 0; indx < (*txn_req)->req.get_data->num_xpaths; indx++)
        {
          if ((*txn_req)->req.get_data->xpaths[indx])
            free ((void *) (*txn_req)->req.get_data->xpaths[indx]);
        }
      req_list = &(*txn_req)->txn->get_cfg_reqs;
      MGMTD_TXN_DBG ("Deleting GETCFG req-id: %" PRIu64 " txn-id: %" PRIu64,
                     (*txn_req)->req_id, (*txn_req)->txn->txn_id);
      if ((*txn_req)->req.get_data->reply)
        XFREE (MTYPE_MGMTD_TXN_GETDATA_REPLY, (*txn_req)->req.get_data->reply);
    ...
    case MGMTD_TXN_PROC_GETDATA:
      ...
      MGMTD_TXN_DBG ("Deleting GETDATA req-id: %" PRIu64 " txn-id: %" PRIu64,
                     (*txn_req)->req_id, (*txn_req)->txn->txn_id);
      if ((*txn_req)->req.get_data->reply)
        XFREE (MTYPE_MGMTD_TXN_GETDATA_REPLY, (*txn_req)->req.get_data->reply);
      XFREE (MTYPE_MGMTD_TXN_GETDATA_REQ, (*txn_req)->req.get_data);
  ...
  if ((*txn_req)->pending_be_proc && pending_list)
    {
      ...
      MGMTD_TXN_DBG ("Removed req-id: %" PRIu64
                     ...
                     (*txn_req)->req_id, mgmt_txn_reqs_count (pending_list));
    }
  else if (req_list)
    {
...
static void
mgmt_txn_process_set_cfg (struct event *thread)
{
  struct mgmt_txn_ctx *txn;
  struct mgmt_txn_req *txn_req;
  struct mgmt_ds_ctx *ds_ctx;
  ...
  FOREACH_TXN_REQ_IN_LIST (&txn->set_cfg_reqs, txn_req)
    {
      ...
      if (! ds_ctx)
        {
          mgmt_fe_send_set_cfg_reply (
              ...
              txn_req->req_id, MGMTD_INTERNAL_ERROR, "No such datastore!",
      ...
      nb_candidate_edit_config_changes (
          ...
          (size_t) txn_req->req.set_cfg->num_cfg_changes, NULL, NULL, 0,
          err_buf, sizeof (err_buf), &error);
      if (error)
        {
          mgmt_fe_send_set_cfg_reply (
              txn->session_id, txn->txn_id, txn_req->req.set_cfg->ds_id,
              txn_req->req_id, MGMTD_INTERNAL_ERROR, err_buf,
              txn_req->req.set_cfg->implicit_commit);
          goto mgmt_txn_process_set_cfg_done;
        }
      ...
      if (txn_req->req.set_cfg->implicit_commit)
        {
          ...
          mgmt_txn_send_commit_config_req (
              txn->txn_id, txn_req->req_id, txn_req->req.set_cfg->ds_id,
              txn_req->req.set_cfg->ds_ctx, txn_req->req.set_cfg->dst_ds_id,
              txn_req->req.set_cfg->dst_ds_ctx, false, false, true);
          ...
          cmt_stats->commit_cnt++;
                   ...
                   txn->session_id, txn->txn_id, txn_req->req.set_cfg->ds_id,
                   txn_req->req_id, MGMTD_SUCCESS, NULL, false) != 0)
        {
...
static int
mgmt_txn_send_commit_cfg_reply (struct mgmt_txn_ctx *txn,
                                enum mgmt_result result,
                                const char *error_if_any)
{
  ...
  if (! txn->commit_cfg_req->req.commit_cfg.implicit && txn->session_id &&
      ...
      mgmt_fe_send_commit_cfg_reply (
          ...
          txn->commit_cfg_req->req.commit_cfg.validate_only, result,
          error_if_any) != 0)
    {
  ...
  if (txn->commit_cfg_req->req.commit_cfg.implicit && txn->session_id &&
      ...
      mgmt_fe_send_set_cfg_reply (
          ...
          txn->commit_cfg_req->req.commit_cfg.src_ds_id,
          txn->commit_cfg_req->req_id,
          success ? MGMTD_SUCCESS : MGMTD_INTERNAL_ERROR, error_if_any,
          true) != 0)
    {
  ...
  if (success)
    {
      ...
      create_cmt_info_rec = (result != MGMTD_NO_CFG_CHANGES &&
                             ! txn->commit_cfg_req->req.commit_cfg.rollback);
...
static void
mgmt_move_txn_cfg_batches (struct mgmt_txn_ctx *txn,
                           struct mgmt_commit_cfg_req *cmtcfg_req,
                           struct mgmt_txn_batches_head *src_list,
                           struct mgmt_txn_batches_head *dst_list,
                           bool update_commit_phase,
                           enum mgmt_commit_phase to_phase)
{
  ...
  FOREACH_TXN_CFG_BATCH_IN_LIST (src_list, batch)
    {
      mgmt_move_txn_cfg_batch_to_next (cmtcfg_req, batch, src_list, dst_list,
                                       update_commit_phase, to_phase);
    }
}
  ...
  FOREACH_MGMTD_BE_CLIENT_ID (id)
    {
      ...
      mgmt_move_txn_cfg_batches (txn, cmtcfg_req, next_list, curr_list, false,
                                 0);
    }
                                   ...
                                   struct mgmt_be_client_adapter *adapter)
{
  ...
  MGMTD_TXN_DBG (
      "Move txn-id: %" PRIu64 " for '%s' Phase(current: '%s' next:'%s')",
      txn->txn_id, adapter->name, mgmt_txn_commit_phase_str (txn, true),
      mgmt_txn_commit_phase_str (txn, false));
...
static int
mgmt_txn_create_config_batches (struct mgmt_txn_req *txn_req,
                                struct nb_config_cbs *changes)
{
  ...
  RB_FOREACH_SAFE (cb, nb_config_cbs, changes, nxt)
    {
      ...
      MGMTD_TXN_DBG ("XPATH: %s, Value: '%s'", xpath, value ? value : "NIL");
      ...
      FOREACH_MGMTD_BE_CLIENT_ID (id)
        {
          ...
          if (! batch ||
              (batch->num_cfg_data == MGMTD_MAX_CFG_CHANGES_IN_BATCH) ||
              (batch->buf_space_left < (xpath_len + value_len)))
            {
              /* Allocate a new config batch */
              batch = mgmt_txn_cfg_batch_alloc (txn_req->txn, id, adapter);
            }

          batch->buf_space_left -= (xpath_len + value_len);
          memcpy (&batch->xp_subscr[batch->num_cfg_data],
                  &subscr_info.xpath_subscr[id], sizeof (batch->xp_subscr[0]));

          mgmt_yang_cfg_data_req_init (&batch->cfg_data[batch->num_cfg_data]);
          ...
          mgmt_yang_data_value_init (&batch->value[batch->num_cfg_data]);
          ...
          batch->value[batch->num_cfg_data].encoded_str_val = value;
          value = NULL;

          if (subscr_info.xpath_subscr[id] & MGMT_SUBSCR_VALIDATE_CFG)
  ...
  if (! num_chgs)
    {
      (void) mgmt_txn_send_commit_cfg_reply (
          txn_req->txn, MGMTD_NO_CFG_CHANGES, "No changes found to commit!");
      return -1;
    }
...
static int
mgmt_txn_prepare_config (struct mgmt_txn_ctx *txn)
{
  ...
  nb_config =
      mgmt_ds_get_nb_config (txn->commit_cfg_req->req.commit_cfg.src_ds_ctx);
  if (! nb_config)
    {
      (void) mgmt_txn_send_commit_cfg_reply (
  ...
  if (RB_EMPTY (nb_config_cbs, cfg_chgs))
    {
       ...
       */
      (void) mgmt_txn_send_commit_cfg_reply (
          txn, MGMTD_NO_CFG_CHANGES, "No changes found to be committed!");
      ret = -1;
      goto mgmt_txn_prepare_config_done;
    }
  ...
  if (mm->perf_stats_en)
    gettimeofday (
        &txn->commit_cfg_req->req.commit_cfg.cmt_stats->validate_start, NULL);
  /*
...
mgmt_txn_prep_config_validation_done:
  ...
  if (mm->perf_stats_en)
    gettimeofday (
        &txn->commit_cfg_req->req.commit_cfg.cmt_stats->prep_cfg_start, NULL);
...
static int
mgmt_txn_send_be_txn_create (struct mgmt_txn_ctx *txn)
{
  enum mgmt_be_client_id id;
  struct mgmt_be_client_adapter *adapter;
  struct mgmt_commit_cfg_req *cmtcfg_req;
      ...
      if (cmtcfg_req->subscr_info.xpath_subscr[id])
        {
          ...
          FOREACH_TXN_CFG_BATCH_IN_LIST (
              &txn->commit_cfg_req->req.commit_cfg.curr_batches[id], batch)
            batch->comm_phase = MGMTD_COMMIT_PHASE_TXN_CREATE;
        }
    }

  txn->commit_cfg_req->req.commit_cfg.next_phase = MGMTD_COMMIT_PHASE_SEND_CFG;
                           ...
                           struct mgmt_be_client_adapter *adapter)
{
  ...
  FOREACH_TXN_CFG_BATCH_IN_LIST (&cmtcfg_req->curr_batches[adapter->id], batch)
    {
      ...
      indx++;
      if (mgmt_be_send_cfgdata_req (adapter, txn->txn_id, batch->batch_id,
                                    ...
                                    indx == num_batches ? true : false))
        {
          ...
          MGMTD_TXN_ERR ("Could not send CFGDATA_CREATE txn-id: %" PRIu64
                         ...
                         txn->txn_id, batch->batch_id, adapter->name);
          return -1;
        }
      ...
      mgmt_move_txn_cfg_batch_to_next (cmtcfg_req, batch,
                                       ...
                                       true, MGMTD_COMMIT_PHASE_SEND_CFG);
    }
...
static void
mgmt_txn_cfg_commit_timedout (struct event *thread)
{
  struct mgmt_txn_ctx *txn;
 ...
 */
static int
mgmt_txn_send_be_cfg_apply (struct mgmt_txn_ctx *txn)
{
  enum mgmt_be_client_id id;
  struct mgmt_be_client_adapter *adapter;
  struct mgmt_commit_cfg_req *cmtcfg_req;
  ...
  FOREACH_MGMTD_BE_CLIENT_ID (id)
    {
      if (cmtcfg_req->subscr_info.xpath_subscr[id] & MGMT_SUBSCR_NOTIFY_CFG)
        {
          adapter = mgmt_be_get_adapter_by_id (id);
          if (! adapter)
            return -1;
          ...
          UNSET_FLAG (adapter->flags, MGMTD_BE_ADAPTER_FLAGS_CFG_SYNCED);
...
static void
mgmt_txn_process_commit_cfg (struct event *thread)
{
  struct mgmt_txn_ctx *txn;
  struct mgmt_commit_cfg_req *cmtcfg_req;
  ...
  switch (cmtcfg_req->curr_phase)
    ...
    case MGMTD_COMMIT_PHASE_SEND_CFG:
      if (mm->perf_stats_en)
        gettimeofday (&cmtcfg_req->cmt_stats->send_cfg_start, NULL);
        /*
...
static void
mgmt_init_get_data_reply (struct mgmt_get_data_reply *get_reply)
{
  size_t indx;
...
static void
mgmt_reset_get_data_reply (struct mgmt_get_data_reply *get_reply)
{
  int indx;
...
static void
mgmt_reset_get_data_reply_buf (struct mgmt_get_data_req *get_data)
{
  if (get_data->reply)
...
static void
mgmt_txn_send_getcfg_reply_data (struct mgmt_txn_req *txn_req,
                                 struct mgmt_get_data_req *get_req)
{
  ...
  switch (txn_req->req_event)
    {
    case MGMTD_TXN_PROC_GETCFG:
      if (mgmt_fe_send_get_reply (
              txn_req->txn->session_id, txn_req->txn->txn_id, get_req->ds_id,
              txn_req->req_id, MGMTD_SUCCESS, data_reply, NULL) != 0)
        {
...
static void
mgmt_txn_iter_and_send_get_cfg_reply (const char *xpath, struct lyd_node *node,
...
static int
mgmt_txn_get_config (struct mgmt_txn_ctx *txn, struct mgmt_txn_req *txn_req,
  ...
  for (indx = 0; indx < get_data->num_xpaths; indx++)
    {
       ...
       * functionality.
       */
      if (mgmt_ds_iter_data (get_data->ds_id, root, get_data->xpaths[indx],
                             mgmt_txn_iter_and_send_get_cfg_reply,
                             (void *) txn_req) == -1)
        {
          MGMTD_TXN_DBG ("Invalid Xpath '%s", get_data->xpaths[indx]);
...
static void
mgmt_txn_process_get_cfg (struct event *thread)
{
  struct mgmt_txn_ctx *txn;
  struct mgmt_txn_req *txn_req;
  struct nb_config *cfg_root;
...
static void
mgmt_txn_process_get_data (struct event *thread)
{
  struct mgmt_txn_ctx *txn;
  struct mgmt_txn_req *txn_req;
  int num_processed = 0;
...
static struct mgmt_txn_ctx *
mgmt_txn_create_new (uint64_t session_id, enum mgmt_txn_type type)
{
...
static void
mgmt_txn_delete (struct mgmt_txn_ctx **txn)
{
  MGMTD_TXN_UNLOCK (txn);
}

static unsigned int
mgmt_txn_hash_key (const void *data)
{
...
static bool
mgmt_txn_hash_cmp (const void *d1, const void *d2)
{
...
static void
mgmt_txn_hash_init (void)
{
  if (! mgmt_txn_mm || mgmt_txn_mm->txn_hash)
    return;

  mgmt_txn_mm->txn_hash =
      hash_create (mgmt_txn_hash_key, mgmt_txn_hash_cmp, "MGMT Transactions");
}

static void
mgmt_txn_hash_destroy (void)
{
  if (! mgmt_txn_mm || ! mgmt_txn_mm->txn_hash)
    return;
...
static void
mgmt_txn_lock (struct mgmt_txn_ctx *txn, const char *file, int line)
{
  txn->refcount++;
...
static void
mgmt_txn_unlock (struct mgmt_txn_ctx **txn, const char *file, int line)
{
  assert (*txn && (*txn)->refcount);

  (*txn)->refcount--;
  MGMTD_TXN_DBG ("%s:%d --> Unlock %s txn-id: %" PRIu64 " refcnt: %d", file,
                 line, mgmt_txn_type2str ((*txn)->type), (*txn)->txn_id,
                 (*txn)->refcount);
  if (! (*txn)->refcount)
    {
      ...
      MGMTD_TXN_DBG ("Deleted %s txn-id: %" PRIu64 " session-id: %" PRIu64,
...
static void
mgmt_txn_cleanup_txn (struct mgmt_txn_ctx **txn)
{
...
static void
mgmt_txn_cleanup (struct event *thread)
{
  struct mgmt_txn_ctx *txn;
...
static void
mgmt_txn_register_event (struct mgmt_txn_ctx *txn, enum mgmt_txn_event event)
{
  ...
  switch (event)
    {
    case MGMTD_TXN_PROC_SETCFG:
      event_add_timer_tv (mgmt_txn_tm, mgmt_txn_process_set_cfg, txn, &tv,
                          &txn->proc_set_cfg);
      break;
    case MGMTD_TXN_PROC_COMMITCFG:
      event_add_timer_tv (mgmt_txn_tm, mgmt_txn_process_commit_cfg, txn, &tv,
                          &txn->proc_comm_cfg);
      break;
    case MGMTD_TXN_PROC_GETCFG:
      event_add_timer_tv (mgmt_txn_tm, mgmt_txn_process_get_cfg, txn, &tv,
                          &txn->proc_get_cfg);
      break;
    case MGMTD_TXN_PROC_GETDATA:
      event_add_timer_tv (mgmt_txn_tm, mgmt_txn_process_get_data, txn, &tv,
                          &txn->proc_get_data);
      break;
    case MGMTD_TXN_COMMITCFG_TIMEOUT:
      event_add_timer_msec (mgmt_txn_tm, mgmt_txn_cfg_commit_timedout, txn,
...
int
mgmt_txn_init (struct mgmt_master *mm, struct event_loop *tm)
{
...
uint64_t
mgmt_config_txn_in_progress (void)
{
...
uint64_t
mgmt_create_txn (uint64_t session_id, enum mgmt_txn_type type)
{
  struct mgmt_txn_ctx *txn;
...
void
mgmt_destroy_txn (uint64_t *txn_id)
{
  struct mgmt_txn_ctx *txn;
...
int
mgmt_txn_send_set_config_req (uint64_t txn_id, uint64_t req_id,
                              Mgmtd__DatastoreId ds_id,
                              struct mgmt_ds_ctx *ds_ctx,
                              Mgmtd__YangCfgDataReq **cfg_req, size_t num_req,
                              bool implicit_commit,
                              Mgmtd__DatastoreId dst_ds_id,
                              struct mgmt_ds_ctx *dst_ds_ctx)
{
  ...
  for (indx = 0; indx < num_req; indx++)
    {
      ...
      if (cfg_req[indx]->req_type == MGMTD__CFG_DATA_REQ_TYPE__DELETE_DATA)
        cfg_chg->operation = NB_OP_DESTROY;
      else if (cfg_req[indx]->req_type == MGMTD__CFG_DATA_REQ_TYPE__SET_DATA)
        cfg_chg->operation = mgmt_ds_find_data_node_by_xpath (
      ...
      MGMTD_TXN_DBG ("XPath: '%s', Value: '%s'", cfg_req[indx]->data->xpath,
      ...
      cfg_chg->value =
          (cfg_req[indx]->data->value &&
               ...
               : NULL);
      if (cfg_chg->value)
        MGMTD_TXN_DBG ("Allocated value at %p ==> '%s'", cfg_chg->value,
                       cfg_chg->value);
...
int
mgmt_txn_send_commit_config_req (uint64_t txn_id, uint64_t req_id,
                                 Mgmtd__DatastoreId src_ds_id,
                                 struct mgmt_ds_ctx *src_ds_ctx,
                                 Mgmtd__DatastoreId dst_ds_id,
                                 struct mgmt_ds_ctx *dst_ds_ctx,
  ...
  if (connect)
    {
      ...
      MGMTD_TXN_DBG ("Created initial txn-id: %" PRIu64 " for BE client '%s'",
          ...
          if (txn->type == MGMTD_TXN_TYPE_CONFIG)
            {
              cmtcfg_req = txn->commit_cfg_req
                               ...
                               : NULL;
              if (cmtcfg_req &&
                  cmtcfg_req->subscr_info.xpath_subscr[adapter->id])
                {
                  mgmt_txn_send_commit_cfg_reply (
                      txn, MGMTD_INTERNAL_ERROR,
...
mgmt_txn_notify_be_cfg_apply_reply (uint64_t txn_id, bool success,
                                    uint64_t batch_ids[], size_t num_batch_ids,
                                    char *error_if_any,
                                    struct mgmt_be_client_adapter *adapter)
{
  struct mgmt_txn_ctx *txn;
...
int
mgmt_txn_send_get_req (uint64_t txn_id, uint64_t req_id,
                       ...
                       Mgmtd__YangGetDataReq **data_req, size_t num_reqs)
{
  ...
  for (indx = 0; indx < num_reqs && indx < MGMTD_MAX_NUM_DATA_REPLY_IN_BATCH;
       indx++)
    {
...
void
mgmt_txn_status_write (struct vty *vty)
{
  struct mgmt_txn_ctx *txn;
