 ...
 */
static int
mgmt_ds_dump_in_memory (struct mgmt_ds_ctx *ds_ctx, const char *base_xpath,
...
static int
mgmt_ds_replace_dst_with_src_ds (struct mgmt_ds_ctx *src,
...
static int
mgmt_ds_merge_src_with_dst_ds (struct mgmt_ds_ctx *src,
                               struct mgmt_ds_ctx *dst)
{
  int ret;
  ...
  if (src->config_ds && dst->config_ds)
  ...
  else
    {
      assert (! src->config_ds && ! dst->config_ds);
      ret =
          lyd_merge_siblings (&dst->root.dnode_root, src->root.dnode_root, 0);
    }
  if (ret != 0)
    {
...
static int
mgmt_ds_load_cfg_from_file (const char *filepath, struct lyd_node **dnode)
{
  LY_ERR ret;
...
int
mgmt_ds_init (struct mgmt_master *mm)
{
...
struct mgmt_ds_ctx *
mgmt_ds_get_ctx_by_id (struct mgmt_master *mm, Mgmtd__DatastoreId ds_id)
{
  switch (ds_id)
    {
...
bool
mgmt_ds_is_config (struct mgmt_ds_ctx *ds_ctx)
{
  if (! ds_ctx)
    return false;
...
bool
mgmt_ds_is_locked (struct mgmt_ds_ctx *ds_ctx, uint64_t session_id)
{
  assert (ds_ctx);
...
int
mgmt_ds_lock (struct mgmt_ds_ctx *ds_ctx, uint64_t session_id)
{
  assert (ds_ctx);
...
void
mgmt_ds_unlock (struct mgmt_ds_ctx *ds_ctx)
{
  assert (ds_ctx);
  if (! ds_ctx->locked)
    zlog_warn (
        "%s: WARNING: unlock on unlocked in DS:%s last session-id %" PRIu64,
        __func__, mgmt_ds_id2name (ds_ctx->ds_id), ds_ctx->vty_session_id);
  ds_ctx->locked = 0;
}

int
mgmt_ds_copy_dss (struct mgmt_ds_ctx *src_ds_ctx,
...
int
mgmt_ds_dump_ds_to_file (char *file_name, struct mgmt_ds_ctx *ds_ctx)
{
  struct ly_out *out;
  int ret = 0;
...
struct nb_config *
mgmt_ds_get_nb_config (struct mgmt_ds_ctx *ds_ctx)
{
  if (! ds_ctx)
    return NULL;
...
static int
mgmt_walk_ds_nodes (struct nb_config *root, const char *base_xpath,
  ...
  LY_LIST_FOR (lyd_child (base_dnode), dnode)
    {
      ...
      ret = mgmt_walk_ds_nodes (root, xpath, dnode, mgmt_ds_node_iter_fn, ctx);
      if (ret != 0)
        break;
    }
...
int
mgmt_ds_delete_data_nodes (struct mgmt_ds_ctx *ds_ctx, const char *xpath)
{
  struct nb_node *nb_node;
  ...
  if (nb_node && nb_node->dep_cbs.get_dependant_xpath)
    {
      ...
      dep_dnode =
          yang_dnode_get (ds_ctx->config_ds ? ds_ctx->root.cfg_root->dnode
                                            : ds_ctx->root.dnode_root,
                          dep_xpath);
      if (dep_dnode)
...
int
mgmt_ds_load_config_from_file (struct mgmt_ds_ctx *dst, const char *file_path,
                               bool merge)
{
  ...
  if (xpath[0] == 0)
    {
      ...
      LY_LIST_FOR (base_dnode, node)
        {
          ret = mgmt_walk_ds_nodes (root, xpath, node, mgmt_ds_node_iter_fn,
                                    ctx);
        }
    }
  else
    ret = mgmt_walk_ds_nodes (root, xpath, base_dnode, mgmt_ds_node_iter_fn,
                              ctx);
...
void
mgmt_ds_dump_tree (struct vty *vty, struct mgmt_ds_ctx *ds_ctx,
...
void
mgmt_ds_status_write_one (struct vty *vty, struct mgmt_ds_ctx *ds_ctx)
{
  if (! ds_ctx)
    {
      vty_out (vty, "    >>>>> Datastore Not Initialized!\n");
...
void
mgmt_ds_status_write (struct vty *vty)
{
