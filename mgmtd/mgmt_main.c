...
static int
mgmt_vrf_delete (struct vrf *vrf)
{
...
static int
mgmt_vrf_enable (struct vrf *vrf)
{
...
static int
mgmt_vrf_disable (struct vrf *vrf)
{
...
static int
mgmt_vrf_config_write (struct vty *vty)
{
  return 0;
}
...
mgmt_vrf_init (void)
{
  vrf_init (mgmt_vrf_new, mgmt_vrf_enable, mgmt_vrf_disable, mgmt_vrf_delete);
  vrf_cmd_init (mgmt_vrf_config_write);
}

static void
mgmt_vrf_terminate (void)
{
  vrf_terminate ();
}
 ...
 */
#ifdef HAVE_STATICD
  &(struct frr_yang_module_info){ .name = "frr-staticd", .ignore_cbs = true },
#endif
};
...
main (int argc, char **argv)
{
  ...
  snprintf (backup_config_file, sizeof (backup_config_file), "%s/zebra.conf",
            frr_sysconfdir);
