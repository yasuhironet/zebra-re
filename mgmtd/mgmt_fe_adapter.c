                               ...
                               struct mgmt_fe_session_ctx *session)
{
  if (session->ds_locked[ds_id])
    zlog_warn ("multiple lock taken by session-id: %" PRIu64 " on DS:%s",
  ...
  else
    {
      if (mgmt_ds_lock (ds_ctx, session->session_id))
        {
          MGMTD_FE_ADAPTER_DBG (
              "Failed to lock the DS:%s for session-id: %" PRIu64 " from %s!",
      ...
      MGMTD_FE_ADAPTER_DBG ("Write-Locked the DS:%s for session-id: %" PRIu64
                            " from %s",
                            mgmt_ds_id2name (ds_id), session->session_id,
                            session->adapter->name);
    }
...
static void
mgmt_fe_session_unlock_ds (Mgmtd__DatastoreId ds_id,
                           struct mgmt_ds_ctx *ds_ctx,
                           struct mgmt_fe_session_ctx *session)
{
  if (! session->ds_locked[ds_id])
  ...
  MGMTD_FE_ADAPTER_DBG (
      "Unlocked DS:%s write-locked earlier by session-id: %" PRIu64 " from %s",
...
mgmt_fe_adapter_compute_set_cfg_timers (struct mgmt_setcfg_stats *setcfg_stats)
{
  setcfg_stats->last_exec_tm =
      timeval_elapsed (setcfg_stats->last_end, setcfg_stats->last_start);
...
static void
mgmt_fe_cleanup_session (struct mgmt_fe_session_ctx **sessionp)
{
...
static unsigned int
mgmt_fe_session_hash_key (const void *data)
{
...
static bool
mgmt_fe_session_hash_cmp (const void *d1, const void *d2)
{
                        ...
                        uint64_t client_id)
{
  ...
  session =
      XCALLOC (MTYPE_MGMTD_FE_SESSION, sizeof (struct mgmt_fe_session_ctx));
  assert (session);
...
static int
fe_adapter_send_msg (struct mgmt_fe_client_adapter *adapter,
                     Mgmtd__FeMessage *fe_msg, bool short_circuit_ok)
{
  return msg_conn_send_msg (
      ...
      (size_t (*) (void *, void *)) mgmtd__fe_message__pack, short_circuit_ok);
}

static int
fe_adapter_send_session_reply (struct mgmt_fe_client_adapter *adapter,
                               struct mgmt_fe_session_ctx *session,
...
static int
fe_adapter_send_lockds_reply (struct mgmt_fe_session_ctx *session,
...
static int
fe_adapter_send_set_cfg_reply (struct mgmt_fe_session_ctx *session,
                               Mgmtd__DatastoreId ds_id, uint64_t req_id,
...
static int
fe_adapter_send_commit_cfg_reply (struct mgmt_fe_session_ctx *session,
                                  ...
                                  bool validate_only, const char *error_if_any)
{
  ...
  commcfg_reply.success =
      (result == MGMTD_SUCCESS || result == MGMTD_NO_CFG_CHANGES) ? true
                                                                  : false;
   ...
   */
  if (session->cfg_txn_id && ((result == MGMTD_SUCCESS && ! validate_only) ||
...
static int
fe_adapter_send_get_reply (struct mgmt_fe_session_ctx *session,
                           Mgmtd__DatastoreId ds_id, uint64_t req_id,
                           ...
                           const char *error_if_any)
{
  ...
  MGMTD_FE_ADAPTER_DBG (
      "Sending GET_REPLY message to MGMTD Frontend client '%s'",
      session->adapter->name);
  ...
  if (session->txn_id && (! success || (data && data->next_indx < 0)))
    mgmt_fe_session_register_event (session, MGMTD_FE_SESSION_SHOW_TXN_CLNUP);

  return fe_adapter_send_msg (session->adapter, &fe_msg, false);
}

static void
mgmt_fe_session_cfg_txn_clnup (struct event *thread)
{
  struct mgmt_fe_session_ctx *session;
...
static void
mgmt_fe_session_show_txn_clnup (struct event *thread)
{
  struct mgmt_fe_session_ctx *session;
...
mgmt_fe_session_register_event (struct mgmt_fe_session_ctx *session,
                                enum mgmt_session_event event)
{
  struct timeval tv = { .tv_sec = 0, .tv_usec = MGMTD_FE_MSG_PROC_DELAY_USEC };

  switch (event)
    {
    case MGMTD_FE_SESSION_CFG_TXN_CLNUP:
      event_add_timer_tv (mgmt_loop, mgmt_fe_session_cfg_txn_clnup, session,
                          &tv, &session->proc_cfg_txn_clnp);
      break;
    case MGMTD_FE_SESSION_SHOW_TXN_CLNUP:
      event_add_timer_tv (mgmt_loop, mgmt_fe_session_show_txn_clnup, session,
...
static void
mgmt_fe_adapter_delete (struct mgmt_fe_client_adapter *adapter)
{
  struct mgmt_fe_session_ctx *session;
...
static int
mgmt_fe_adapter_notify_disconnect (struct msg_conn *conn)
{
                                       ...
                                       Mgmtd__FeLockDsReq *lockds_req)
{
  ...
  if (lockds_req->ds_id != MGMTD_DS_CANDIDATE &&
      lockds_req->ds_id != MGMTD_DS_RUNNING)
    {
      fe_adapter_send_lockds_reply (
          session, lockds_req->ds_id, lockds_req->req_id, lockds_req->lock,
          false,
  ...
  if (! ds_ctx)
    {
      fe_adapter_send_lockds_reply (
          session, lockds_req->ds_id, lockds_req->req_id, lockds_req->lock,
          false, "Failed to retrieve handle for DS!");
      return -1;
    }

  if (lockds_req->lock)
    {
      if (mgmt_fe_session_write_lock_ds (lockds_req->ds_id, ds_ctx, session))
        {
          fe_adapter_send_lockds_reply (
              session, lockds_req->ds_id, lockds_req->req_id, lockds_req->lock,
              false, "Lock already taken on DS by another session!");
          return -1;
        }
    }
  else
      ...
      if (! session->ds_locked[lockds_req->ds_id])
        {
          fe_adapter_send_lockds_reply (
              session, lockds_req->ds_id, lockds_req->req_id, lockds_req->lock,
              false, "Lock on DS was not taken by this session!");
          return 0;
        }
  ...
  if (fe_adapter_send_lockds_reply (session, lockds_req->ds_id,
                                    lockds_req->req_id, lockds_req->lock, true,
                                    NULL) != 0)
    {
      MGMTD_FE_ADAPTER_DBG (
 ...
 * probably need to just disconnect the client with a suitably loud log
 * message.
 */
static int
                                       ...
                                       Mgmtd__FeSetConfigReq *setcfg_req)
{
  ...
  if (! session->ds_locked[setcfg_req->ds_id])
    {
      fe_adapter_send_set_cfg_reply (
          session, setcfg_req->ds_id, setcfg_req->req_id, false,
          "Candidate DS is not locked", setcfg_req->implicit_commit);
      return 0;
    }
  ...
  if (session->cfg_txn_id == MGMTD_TXN_ID_NONE)
    {
      ...
      session->cfg_txn_id =
          mgmt_create_txn (session->session_id, MGMTD_TXN_TYPE_CONFIG);
  ...
  else
    {
      MGMTD_FE_ADAPTER_DBG ("Config txn-id: %" PRIu64
                            " for session-id: %" PRIu64 " already created",
                            session->cfg_txn_id, session->session_id);
      ...
      if (setcfg_req->implicit_commit)
        {
          ...
          fe_adapter_send_set_cfg_reply (
              session, setcfg_req->ds_id, setcfg_req->req_id, false,
              "A Configuration transaction is already in progress!", false);
          return 0;
        }
    }
  ...
  if (mgmt_txn_send_set_config_req (
          session->cfg_txn_id, setcfg_req->req_id, setcfg_req->ds_id, ds_ctx,
          setcfg_req->data, setcfg_req->n_data, setcfg_req->implicit_commit,
          setcfg_req->commit_ds_id, dst_ds_ctx) != 0)
    {
      fe_adapter_send_set_cfg_reply (
          session, setcfg_req->ds_id, setcfg_req->req_id, false,
...
static int
mgmt_fe_session_handle_get_req_msg (struct mgmt_fe_session_ctx *session,
  ...
  if (session->txn_id == MGMTD_TXN_ID_NONE)
    {
       ...
       */
      session->txn_id =
          mgmt_create_txn (session->session_id, MGMTD_TXN_TYPE_SHOW);
...
mgmt_fe_session_handle_commit_config_req_msg (
    struct mgmt_fe_session_ctx *session, Mgmtd__FeCommitConfigReq *commcfg_req)
{
  ...
  if (session->cfg_txn_id == MGMTD_TXN_ID_NONE)
    {
      ...
      MGMTD_FE_ADAPTER_DBG ("Created txn-id: %" PRIu64
                            " for session-id %" PRIu64 " for COMMIT-CFG-REQ",
                            session->cfg_txn_id, session->session_id);
    }
  ...
  if (mgmt_txn_send_commit_config_req (
          session->cfg_txn_id, commcfg_req->req_id, commcfg_req->src_ds_id,
          src_ds_ctx, commcfg_req->dst_ds_id, dst_ds_ctx,
          commcfg_req->validate_only, commcfg_req->abort, false) != 0)
    {
      fe_adapter_send_commit_cfg_reply (
          session, commcfg_req->src_ds_id, commcfg_req->dst_ds_id,
          commcfg_req->req_id, MGMTD_INTERNAL_ERROR,
  ...
  switch ((int) fe_msg->message_case)
    ...
    case MGMTD__FE_MESSAGE__MESSAGE_REGISTER_REQ:
      ...
      if (strlen (fe_msg->register_req->client_name))
        {
          strlcpy (adapter->name, fe_msg->register_req->client_name,
                   sizeof (adapter->name));
    ...
    case MGMTD__FE_MESSAGE__MESSAGE_SESSION_REQ:
      if (fe_msg->session_req->create &&
        ...
        {
          MGMTD_FE_ADAPTER_DBG (
              "Got SESSION_REQ (create) for client-id %" PRIu64 " from '%s'",
      ...
      else if (! fe_msg->session_req->create &&
               fe_msg->session_req->id_case ==
                   MGMTD__FE_SESSION_REQ__ID_SESSION_ID)
        {
          MGMTD_FE_ADAPTER_DBG (
              "Got SESSION_REQ (destroy) for session-id %" PRIu64 "from '%s'",
              fe_msg->session_req->session_id, adapter->name);

          session = mgmt_session_id2ctx (fe_msg->session_req->session_id);
          fe_adapter_send_session_reply (adapter, session, false, true);
          mgmt_fe_cleanup_session (&session);
        }
      break;
    case MGMTD__FE_MESSAGE__MESSAGE_LOCKDS_REQ:
      session = mgmt_session_id2ctx (fe_msg->lockds_req->session_id);
      MGMTD_FE_ADAPTER_DBG (
      ...
      mgmt_fe_session_handle_lockds_req_msg (session, fe_msg->lockds_req);
      break;
    case MGMTD__FE_MESSAGE__MESSAGE_SETCFG_REQ:
      session = mgmt_session_id2ctx (fe_msg->setcfg_req->session_id);
      ...
      mgmt_fe_session_handle_setcfg_req_msg (session, fe_msg->setcfg_req);
      break;
    case MGMTD__FE_MESSAGE__MESSAGE_COMMCFG_REQ:
      session = mgmt_session_id2ctx (fe_msg->commcfg_req->session_id);
      MGMTD_FE_ADAPTER_DBG (
      ...
      mgmt_fe_session_handle_commit_config_req_msg (session,
                                                    fe_msg->commcfg_req);
      break;
    case MGMTD__FE_MESSAGE__MESSAGE_GET_REQ:
      session = mgmt_session_id2ctx (fe_msg->get_req->session_id);
      MGMTD_FE_ADAPTER_DBG (
          "Got GET_REQ (iscfg %d) for DS:%s (xpaths: %d) on session-id %" PRIu64
          " from '%s'",
    ...
    case MGMTD__FE_MESSAGE__MESSAGE_REGNOTIFY_REQ:
      MGMTD_FE_ADAPTER_ERR ("Got unhandled message of type %u from '%s'",
...
static void
mgmt_fe_adapter_process_msg (uint8_t version, uint8_t *data, size_t len,
                             struct msg_conn *conn)
{
  ...
  if (! fe_msg)
    {
      MGMTD_FE_ADAPTER_DBG ("Failed to decode %zu bytes for adapter: %s", len,
                            adapter->name);
      return;
    }
  MGMTD_FE_ADAPTER_DBG ("Decoded %zu bytes of message: %u from adapter: %s",
                        len, fe_msg->message_case, adapter->name);
  (void) mgmt_fe_adapter_handle_msg (adapter, fe_msg);
  mgmtd__fe_message__free_unpacked (fe_msg, NULL);
}

void
mgmt_fe_adapter_lock (struct mgmt_fe_client_adapter *adapter)
{
  adapter->refcount++;
}
 ...
 */
void
mgmt_fe_adapter_init (struct event_loop *tm)
{
  assert (! mgmt_loop);
  mgmt_loop = tm;
...
static void
mgmt_fe_abort_if_session (void *data)
{
  ...
  if (! adapter)
    {
      ...
      adapter->conn = msg_server_conn_create (
          ...
          MGMTD_FE_MAX_NUM_MSG_WRITE, MGMTD_FE_MSG_MAX_LEN, adapter,
          "FE-adapter");
...
int
mgmt_fe_send_set_cfg_reply (uint64_t session_id, uint64_t txn_id,
                            Mgmtd__DatastoreId ds_id, uint64_t req_id,
                            ...
                            bool implicit_commit)
{
  ...
  return fe_adapter_send_set_cfg_reply (session, ds_id, req_id,
                                        result == MGMTD_SUCCESS ? true : false,
                                        error_if_any, implicit_commit);
}

int
mgmt_fe_send_commit_cfg_reply (uint64_t session_id, uint64_t txn_id,
                               Mgmtd__DatastoreId src_ds_id,
                               Mgmtd__DatastoreId dst_ds_id, uint64_t req_id,
                               bool validate_only, enum mgmt_result result,
                               const char *error_if_any)
{
  struct mgmt_fe_session_ctx *session;
...
int
mgmt_fe_send_get_reply (uint64_t session_id, uint64_t txn_id,
                        Mgmtd__DatastoreId ds_id, uint64_t req_id,
  ...
  if (adapter->cmt_stats.commit_cnt > 0)
    {
      ...
      if (mm->perf_stats_en)
        vty_out (vty, "    Last-Commit-Duration: \t\t%lu uSecs\n",
      ...
      if (mm->perf_stats_en)
        {
          ...
          vty_out (vty, "      Commit Start: \t\t\t%s\n",
                   mgmt_realtime_to_string (&adapter->cmt_stats.last_start,
                                            buf, sizeof (buf)));
#ifdef MGMTD_LOCAL_VALIDATIONS_ENABLED
          vty_out (vty, "        Config-Validate Start: \t\t%s\n",
                   mgmt_realtime_to_string (&adapter->cmt_stats.validate_start,
                                            buf, sizeof (buf)));
#endif
          vty_out (vty, "        Prep-Config Start: \t\t%s\n",
                   mgmt_realtime_to_string (&adapter->cmt_stats.prep_cfg_start,
                                            buf, sizeof (buf)));
          vty_out (
              vty, "        Txn-Create Start: \t\t%s\n",
              mgmt_realtime_to_string (&adapter->cmt_stats.txn_create_start,
                                       buf, sizeof (buf)));
          vty_out (vty,
#ifdef MGMTD_LOCAL_VALIDATIONS_ENABLED
                   "        Send-Config Start: \t\t%s\n",
...
#endif
                   mgmt_realtime_to_string (&adapter->cmt_stats.send_cfg_start,
                                            buf, sizeof (buf)));
          vty_out (
              vty, "        Apply-Config Start: \t\t%s\n",
              mgmt_realtime_to_string (&adapter->cmt_stats.apply_cfg_start,
                                       buf, sizeof (buf)));
          vty_out (vty, "        Apply-Config End: \t\t%s\n",
                   mgmt_realtime_to_string (&adapter->cmt_stats.apply_cfg_end,
                                            buf, sizeof (buf)));
          vty_out (vty, "        Txn-Delete Start: \t\t%s\n",
                   mgmt_realtime_to_string (&adapter->cmt_stats.txn_del_start,
                                            buf, sizeof (buf)));
          vty_out (vty, "      Commit End: \t\t\t%s\n",
                   mgmt_realtime_to_string (&adapter->cmt_stats.last_end, buf,
                                            sizeof (buf)));
        }
    }
}
  ...
  if (mm->perf_stats_en && adapter->setcfg_stats.set_cfg_count > 0)
    {
      ...
      vty_out (vty, "      Set-Cfg Start: \t\t\t%s\n",
               mgmt_realtime_to_string (&adapter->setcfg_stats.last_start, buf,
                                        sizeof (buf)));
      vty_out (vty, "      Set-Cfg End: \t\t\t%s\n",
               mgmt_realtime_to_string (&adapter->setcfg_stats.last_end, buf,
                                        sizeof (buf)));
    }
}

void
mgmt_fe_adapter_status_write (struct vty *vty, bool detail)
{
  struct mgmt_fe_client_adapter *adapter;
  struct mgmt_fe_session_ctx *session;
  Mgmtd__DatastoreId ds_id;
...
void
mgmt_fe_adapter_perf_measurement (struct vty *vty, bool config)
{
  mm->perf_stats_en = config;
}

void
mgmt_fe_adapter_reset_perf_stats (struct vty *vty)
{
  struct mgmt_fe_client_adapter *adapter;
  struct mgmt_fe_session_ctx *session;
