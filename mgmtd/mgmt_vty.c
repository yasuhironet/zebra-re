...
static int
write_mgmt_debug_helper (struct vty *vty, bool config)
{
...
int
config_write_mgmt_debug (struct vty *vty)
{
  return write_mgmt_debug_helper (vty, true);
}

DEFPY_NOSH (show_debugging_mgmt,
...
static void
mgmt_config_read_in (struct event *event)
{
