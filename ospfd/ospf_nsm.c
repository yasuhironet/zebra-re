...
static void
ospf_inactivity_timer (struct event *thread)
{
...
static void
ospf_db_desc_timer (struct event *thread)
{
  struct ospf_neighbor *nbr;
  ...
  if (IS_DEBUG_OSPF (nsm, NSM_TIMERS))
    zlog_debug ("NSM[%s:%pI4:%s]: Timer (DD Retransmit timer expire)",
                IF_NAME (nbr->oi), &nbr->src, ospf_get_name (nbr->oi->ospf));
...
nsm_notice_state_change (struct ospf_neighbor *nbr, int next_state, int event)
{
  ...
  if (CHECK_FLAG (nbr->oi->ospf->config, OSPF_LOG_ADJACENCY_CHANGES) &&
    ...
    zlog_notice ("AdjChg: Nbr %pI4, NbrIP %pI4 (%s) on %s: %s -> %s (%s)",
                 &nbr->router_id, &nbr->src, ospf_get_name (nbr->oi->ospf),
                 IF_NAME (nbr->oi),
                 lookup_msg (ospf_nsm_state_msg, nbr->state, NULL),
                 lookup_msg (ospf_nsm_state_msg, next_state, NULL),
                 ospf_nsm_event_str[event]);
      ...
      (old_state == NSM_Full && state != NSM_Full))
    {
      ...
      if (CHECK_FLAG (oi->ospf->config, OSPF_LOG_ADJACENCY_DETAIL))
        zlog_info (
            ...
            __func__, &nbr->router_id, ospf_get_name (oi->ospf),
...
void
ospf_nsm_event (struct event *thread)
{
  int event;
  int next_state;
  struct ospf_neighbor *nbr;
  ...
  if (IS_DEBUG_OSPF (nsm, NSM_EVENTS))
    zlog_debug ("NSM[%s:%pI4:%s]: %s (%s)", IF_NAME (nbr->oi), &nbr->router_id,
                ospf_get_name (nbr->oi->ospf),
                lookup_msg (ospf_nsm_state_msg, nbr->state, NULL),
                ospf_nsm_event_str[event]);
