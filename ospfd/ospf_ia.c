...
ospf_find_abr_route (struct route_table *rtrs, struct prefix_ipv4 *abr,
  ...
  if ((rn1 = route_node_lookup (rt, (struct prefix *) p)))
    {
      ...
      if ((or = rn1->info))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: Found a route to the same network", __func__);
  ...
  if (or)
    {
      ...
      /* Routes are the same */
        ...
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: merging the new route", __func__);
      ...
      /* New route is worse */
        ...
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: skipping the new route", __func__);
          ospf_route_free (new_or);
          return;
        }
    }
...
static void
ospf_examine_summaries (struct ospf_area *area, struct route_table *lsdb_rt,
                        struct route_table *rt, struct route_table *rtrs)
{
  struct ospf_lsa *lsa;
  struct route_node *rn;
...
static void
ospf_update_network_route (struct ospf *ospf, struct route_table *rt,
                           struct route_table *rtrs, struct summary_lsa *lsa,
                           struct prefix_ipv4 *p, struct ospf_area *area)
{
  struct route_node *rn;
  ...
  if (abr_or == NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: can't find a route to the ABR", __func__);
      return;
    }
  ...
  if (! rn)
    {
      if (ospf->abr_type != OSPF_ABR_SHORTCUT)
        return; /* Standard ABR can update only already
                   ...
                   backbone paths */
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: Allowing Shortcut ABR to add new route", __func__);
      new_or = ospf_route_new ();
      new_or->type = OSPF_DESTINATION_NETWORK;
  ...
  if (ospf->abr_type == OSPF_ABR_SHORTCUT)
  ...
  else /* Not Shortcut ABR */
    {
      if (! OSPF_IS_AREA_ID_BACKBONE (or->u.std.area_id))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: route is not BB-associated", __func__);
  ...
  if (or->cost > cost)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: new route is better, overriding nexthops", __func__);
...
static void
ospf_update_router_route (struct ospf *ospf, struct route_table *rtrs,
                          struct summary_lsa *lsa, struct prefix_ipv4 *p,
                          struct ospf_area *area)
{
  struct ospf_route * or, *abr_or, *new_or;
  struct prefix_ipv4 abr;
  ...
  if (! (or->u.std.flags & ROUTER_LSA_EXTERNAL))
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: the remote router is not an ASBR", __func__);
      return;
    }
...
void
ospf_ia_routing (struct ospf *ospf, struct route_table *rt,
                 struct route_table *rtrs)
{
  struct listnode *node;
  struct ospf_area *area;
  ...
  if (IS_OSPF_ABR (ospf))
      ...
      switch (ospf->abr_type)
        ...
        case OSPF_ABR_STAND:
          ...
          if ((area = ospf->backbone))
            ...
            zlog_debug ("%s:backbone area NOT found", __func__);
          break;
        case OSPF_ABR_IBM:
        case OSPF_ABR_CISCO:
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s:Alternative Cisco/IBM ABR", __func__);
          ...
          if (area && ospf_act_bb_connection (ospf))
            ...
            { /* No active BB connection--consider all areas
               */
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("%s: Active BB connection not found", __func__);
              for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
                OSPF_EXAMINE_SUMMARIES_ALL (area, rt, rtrs);
            }
          break;
    ...
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s:not ABR, considering all areas", __func__);
