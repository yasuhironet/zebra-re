...
struct ospf_route *
ospf_find_asbr_route (struct ospf *ospf, struct route_table *rtrs,
...
ospf_ase_calculate_route (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA))
    {
      zlog_debug (
          ...
          &al->header.id, ip_masklen (al->mask), &al->header.adv_router);
    }
  ...
  if ((lsa->data->type == OSPF_AS_EXTERNAL_LSA) &&
    ...
    {
      if (IS_DEBUG_OSPF (lsa, LSA))
        zlog_debug ("Route[External]: Ignore, If type-5 LSA from NSSA area.");
      return 0;
    }
  ...
  if ((rn =
           route_node_lookup (ospf->new_external_route, (struct prefix *) &p)))
    route_unlock_node (rn);
...
static int
ospf_ase_compare_tables (struct ospf *ospf,
...
static void
ospf_ase_calculate_timer (struct event *t)
{
  struct ospf *ospf;
  struct ospf_lsa *lsa;
  struct route_node *rn;
