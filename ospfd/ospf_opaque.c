...
static const char *
ospf_opaque_type_name (uint8_t opaque_type)
{
    ...
    int (*del_lsa_hook) (struct ospf_lsa *lsa))
{
  ...
  new =
      XCALLOC (MTYPE_OSPF_OPAQUE_FUNCTAB, sizeof (struct ospf_opaque_functab));
...
void
ospf_delete_opaque_functab (uint8_t lsa_type, uint8_t opaque_type)
{
  struct list *funclist;
  struct listnode *node, *nnode;
  struct ospf_opaque_functab *functab;
        ...
        if (functab->opaque_type == opaque_type)
          {
            ...
            if (functab->oipt != NULL)
              free_opaque_info_per_type (functab->oipt, true);
...
static void
free_opaque_info_per_type (struct opaque_info_per_type *oipt,
                           bool cleanup_owner)
{
  ...
  if (cleanup_owner)
    {
      ...
      switch (oipt->lsa_type)
        {
        case OSPF_OPAQUE_LINK_LSA:
          l = ((struct ospf_interface *) oipt->owner)->opaque_lsa_self;
          break;
        case OSPF_OPAQUE_AREA_LSA:
...
static void
free_opaque_info_per_type_del (void *val)
{
                             ...
                             struct ospf_lsa *new)
{
  ...
  oipi =
      XCALLOC (MTYPE_OPAQUE_INFO_PER_ID, sizeof (struct opaque_info_per_id));
...
int
ospf_opaque_is_owned (struct ospf_lsa *lsa)
{
       ...
       "Opaque LSA\n")
{
  ...
  if (ospf->vrf_id != VRF_DEFAULT)
    {
      vty_out (vty, "OSPF Opaque LSA is only supported in default VRF\n");
...
static void
opaque_lsa_config_write_if_callback (struct list *funclist, struct vty *vty,
  ...
  if (vty != NULL)
    {
      if (! json)
        ...
        {
          json_object_string_add (json, "opaqueType",
                                  ospf_opaque_type_name (opaque_type));
          ...
          json_object_boolean_add (json, "opaqueDataLengthValid", lenValid);
          if (lenValid)
            {
              bp = asnprintfrr (MTYPE_TMP, buf, sizeof (buf), "%*pHXn",
                                (int) len, (lsah + 1));
...
void
ospf_opaque_lsa_dump (struct stream *s, uint16_t length)
{
  struct ospf_lsa lsa = {};
...
static void ospf_opaque_lsa_reoriginate_resume (struct list *listtop,
                                                void *arg);
...
ospf_opaque_lsa_originate_schedule (struct ospf_interface *oi, int *delay0)
{
  ...
  if (! list_isempty (ospf_opaque_type9_funclist) &&
      list_isempty (oi->opaque_lsa_self) && oi->t_opaque_lsa_self == NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("Schedule Type-9 Opaque-LSA origination in %d ms later.",
                    delay);
      oi->t_opaque_lsa_self = NULL;
      event_add_timer_msec (master, ospf_opaque_type9_lsa_originate, oi, delay,
                            &oi->t_opaque_lsa_self);
      delay += top->min_ls_interval;
    }

  if (! list_isempty (ospf_opaque_type10_funclist) &&
      list_isempty (area->opaque_lsa_self) && area->t_opaque_lsa_self == NULL)
    {
      /*
       ...
       */
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("Schedule Type-10 Opaque-LSA origination in %d ms later.",
                    delay);
      area->t_opaque_lsa_self = NULL;
      event_add_timer_msec (master, ospf_opaque_type10_lsa_originate, area,
                            delay, &area->t_opaque_lsa_self);
      delay += top->min_ls_interval;
    }

  if (! list_isempty (ospf_opaque_type11_funclist) &&
      list_isempty (top->opaque_lsa_self) && top->t_opaque_lsa_self == NULL)
    {
      /*
       ...
       */
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("Schedule Type-11 Opaque-LSA origination in %d ms later.",
                    delay);
      top->t_opaque_lsa_self = NULL;
      event_add_timer_msec (master, ospf_opaque_type11_lsa_originate, top,
                            delay, &top->t_opaque_lsa_self);
      delay += top->min_ls_interval;
    }
      ...
      for (ALL_LIST_ELEMENTS (oi->opaque_lsa_self, node, nnode, oipt))
        {
          ...
          ospf_opaque_lsa_reoriginate_schedule (
              (void *) oi, OSPF_OPAQUE_LINK_LSA, oipt->opaque_type);
        }
    }
      ...
      for (ALL_LIST_ELEMENTS (area->opaque_lsa_self, node, nnode, oipt))
        {
          ...
          ospf_opaque_lsa_reoriginate_schedule (
              (void *) area, OSPF_OPAQUE_AREA_LSA, oipt->opaque_type);
        }
    }
      ...
      for (ALL_LIST_ELEMENTS (top->opaque_lsa_self, node, nnode, oipt))
        {
          ...
          ospf_opaque_lsa_reoriginate_schedule (
              (void *) top, OSPF_OPAQUE_AS_LSA, oipt->opaque_type);
        }
    }
...
static void
ospf_opaque_type9_lsa_originate (struct event *t)
{
  struct ospf_interface *oi;
...
static void
ospf_opaque_type10_lsa_originate (struct event *t)
{
  struct ospf_area *area;
...
static void
ospf_opaque_type11_lsa_originate (struct event *t)
{
  struct ospf *top;
  ...
  for (ALL_LIST_ELEMENTS (listtop, node, nnode, oipt))
    {
      ...
      if ((*functab->lsa_originator) (arg) != 0)
        {
          flog_warn (EC_OSPF_LSA, "%s: Failed (opaque-type=%u)", __func__,
                     oipt->opaque_type);
          continue;
        }
    }
...
ospf_opaque_lsa_install (struct ospf_lsa *lsa, int rt_recalc)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_INSTALL))
    zlog_debug ("Install Type-%u Opaque-LSA: [opaque-type=%u, opaque-id=%x]",
                lsa->data->type,
  ...
  switch (lsa_type)
    ...
    case OSPF_OPAQUE_LINK_LSA:
      if ((oi = (struct ospf_interface *) lsa_type_dependent) == NULL)
        {
          flog_warn (EC_OSPF_LSA, "%s: Type-9 Opaque-LSA: Invalid parameter?",
                     __func__);
          goto out;
        }
    ...
    case OSPF_OPAQUE_AREA_LSA:
      if ((area = (struct ospf_area *) lsa_type_dependent) == NULL)
        {
          flog_warn (EC_OSPF_LSA, "%s: Type-10 Opaque-LSA: Invalid parameter?",
                     __func__);
          goto out;
        }
      if ((top = area->ospf) == NULL)
        {
          flog_warn (EC_OSPF_LSA, "%s: AREA(%pI4) -> TOP?", __func__,
                     &area->area_id);
          goto out;
        }
      if (! list_isempty (ospf_opaque_type10_funclist) &&
    ...
    case OSPF_OPAQUE_AS_LSA:
      if ((top = (struct ospf *) lsa_type_dependent) == NULL)
        {
          flog_warn (EC_OSPF_LSA, "%s: Type-11 Opaque-LSA: Invalid parameter?",
                     __func__);
          goto out;
        }
      if (! list_isempty (ospf_opaque_type11_funclist) &&
...
static void
ospf_opaque_type9_lsa_reoriginate_timer (struct event *t)
{
  struct opaque_info_per_type *oipt;
  struct ospf_opaque_functab *functab;
  struct ospf *top;
  ...
  if (! CHECK_FLAG (top->config, OSPF_OPAQUE_CAPABLE) ||
      ! ospf_if_is_enable (oi) || ospf_nbr_count_opaque_capable (oi) == 0)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (
...
static void
ospf_opaque_type10_lsa_reoriginate_timer (struct event *t)
{
  struct opaque_info_per_type *oipt;
  struct ospf_opaque_functab *functab;
  struct listnode *node, *nnode;
...
static void
ospf_opaque_type11_lsa_reoriginate_timer (struct event *t)
{
  struct opaque_info_per_type *oipt;
  struct ospf_opaque_functab *functab;
  struct ospf *top;
...
ospf_opaque_lsa_refresh_schedule (struct ospf_lsa *lsa0)
{
  ...
  OSPF_OPAQUE_TIMER_ON (oipi->t_opaque_lsa_self, ospf_opaque_lsa_refresh_timer,
                        oipi, delay * 1000);
out:
  return;
}
...
static void
ospf_opaque_lsa_refresh_timer (struct event *t)
{
  struct opaque_info_per_id *oipi;
  struct ospf_opaque_functab *functab;
  struct ospf_lsa *lsa;
                                          ...
                                          struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("LSA[Type%d:%pI4]: processing self-originated Opaque-LSA",
                lsa->data->type, &lsa->data->id);
  ...
  lsa->data->ls_age = htons (OSPF_LSA_MAXAGE);
  ospf_lsa_install (
      top, (lsa->data->type == OSPF_OPAQUE_LINK_LSA) ? nbr->oi : NULL, lsa);
...
oi_to_top (struct ospf_interface *oi)
{
  ...
  if (oi == NULL || (area = oi->area) == NULL || (top = area->ospf) == NULL)
    flog_warn (EC_OSPF_LSA, "Broken relationship for \"OI -> AREA -> OSPF\"?");
