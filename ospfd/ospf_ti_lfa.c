...
void
ospf_print_protected_resource (struct protected_resource *protected_resource,
                               char *buf)
{
  ...
  switch (protected_resource->type)
    ...
    case OSPF_TI_LFA_LINK_PROTECTION:
      ...
      snprintfrr (buf, PROTECTED_RESOURCE_STRLEN, "protected link: %pI4 %pI4",
                  &link->link_id, &link->link_data);
      break;
    case OSPF_TI_LFA_NODE_PROTECTION:
      snprintfrr (buf, PROTECTED_RESOURCE_STRLEN, "protected node: %pI4",
      ...
      if (curr_node->next->next)
        {
          ...
          pc_vertex_parent = ospf_spf_vertex_parent_find (p_node_pc_parent->id,
                                                          pc_node_parent);
          q_space->p_node_info->nexthop = pc_vertex_parent->nexthop->router;
        }
      else
        {
          /*
...
static void
ospf_ti_lfa_find_q_node (struct vertex *pc_node, struct p_space *p_space,
                         struct q_space *q_space)
{
  ...
  pc_vertex_parent = ospf_spf_vertex_parent_find (pc_node_parent->id, pc_node);
  ...
  if (p_node && q_node)
    {
      ...
      q_space->q_node_info->nexthop = pc_vertex_parent->nexthop->router;
      return;
    }
   ...
   */
  q_space_parent = ospf_spf_vertex_find (pc_node_parent->id, q_node->children);
...
static void
ospf_ti_lfa_append_label_stack (struct mpls_label_stack *label_stack,
                                mpls_label_t labels[], uint32_t num_labels)
{
  int i, offset, limit;
...
static struct list *
ospf_ti_lfa_cut_out_pc_path (struct list *pc_vertex_list, struct list *pc_path,
                             struct vertex *p_node, struct vertex *q_node)
{
...
static void
ospf_ti_lfa_generate_inner_label_stack (
    ...
    struct ospf_ti_lfa_inner_backup_path_info *inner_backup_path_info)
{
  ...
  if (q_node_info->node->id.s_addr == q_space->root->id.s_addr)
    {
      pc_q_node =
          listnode_lookup (q_space->pc_path, listnode_head (q_space->pc_path));
      end_vertex = listgetdata (pc_q_node->next);
      end_label =
          ospf_sr_get_adj_sid_by_id (&end_vertex->id, &q_node_info->node->id);
    }
  ...
  if (start_vertex->id.s_addr == end_vertex->id.s_addr)
    {
      ...
      inner_backup_path_info->p_node_info.type = OSPF_TI_LFA_UNDEFINED_NODE;
      vertex_parent =
          ospf_spf_vertex_parent_find (p_space->root->id, end_vertex);
      inner_backup_path_info->p_node_info.nexthop =
          vertex_parent->nexthop->router;
      return;
    }

  inner_pc_path = ospf_ti_lfa_cut_out_pc_path (
      p_space->pc_vertex_list, q_space->pc_path, start_vertex, end_vertex);
  ...
  area->p_spaces = XCALLOC (MTYPE_OSPF_P_SPACE, sizeof (struct p_spaces_head));

  /* dry run true, root node false */
  ospf_spf_calculate (area, start_vertex->lsa_p, new_table, NULL, NULL, true,
                      false);
  ...
  if (start_label != MPLS_INVALID_LABEL)
    {
      ...
      vertex_parent =
          ospf_spf_vertex_parent_find (p_space->root->id, start_vertex);
      inner_backup_path_info->p_node_info.nexthop =
          vertex_parent->nexthop->router;
    }
  else
    {
      memcpy (inner_backup_path_info->label_stack, inner_q_space->label_stack,
              sizeof (struct mpls_label_stack) +
                  sizeof (mpls_label_t) *
                      inner_q_space->label_stack->num_labels);
      memcpy (&inner_backup_path_info->p_node_info, inner_q_space->p_node_info,
  ...
  if (end_label != MPLS_INVALID_LABEL)
    ...
    {
      memcpy (&inner_backup_path_info->q_node_info, inner_q_space->q_node_info,
              sizeof (struct ospf_ti_lfa_node_info));
    }
...
static void
ospf_ti_lfa_generate_label_stack (struct ospf_area *area,
  ...
  if (q_space->q_node_info->type == OSPF_TI_LFA_PQ_NODE)
    {
      ...
      if (ospf_spf_vertex_parent_find (p_space->root->id,
        ...
        labels[0] = ospf_sr_get_adj_sid_by_id (
            &p_space->root->id, &q_space->q_node_info->node->id);
      else
        labels[0] =
            ospf_sr_get_prefix_sid_by_id (&q_space->q_node_info->node->id);
  ...
  if (adjacency_result == OSPF_TI_LFA_P_Q_SPACE_ADJACENT)
    {
       ...
       * the Q node.
       */
      if (q_space->p_node_info->node->id.s_addr == p_space->root->id.s_addr)
        {
          labels[0] = ospf_sr_get_adj_sid_by_id (
              &p_space->root->id, &q_space->q_node_info->node->id);
          q_space->label_stack = ospf_ti_lfa_create_label_stack (labels, 1);
      ...
      if (ospf_spf_vertex_parent_find (p_space->root->id,
        ...
        labels[0] = ospf_sr_get_adj_sid_by_id (
            &p_space->root->id, &q_space->p_node_info->node->id);
      else
        labels[0] =
            ospf_sr_get_prefix_sid_by_id (&q_space->p_node_info->node->id);

      labels[1] = ospf_sr_get_adj_sid_by_id (&q_space->p_node_info->node->id,
  ...
  else
    {
      ...
      inner_backup_path_info.label_stack = XCALLOC (
          MTYPE_OSPF_PATH, sizeof (struct mpls_label_stack) +
      ...
      if (q_space->p_node_info->node->id.s_addr == p_space->root->id.s_addr)
        {
          ...
          if (inner_backup_path_info.p_node_info.type !=
            ...
            q_space->nexthop = inner_backup_path_info.p_node_info.nexthop;
          else
            q_space->nexthop = inner_backup_path_info.q_node_info.nexthop;
        }
                                            ...
                                            q_space->p_node_info->node))
        {
          ...
          ospf_ti_lfa_append_label_stack (
              q_space->label_stack, inner_backup_path_info.label_stack->label,
              inner_backup_path_info.label_stack->num_labels);
          q_space->nexthop = q_space->p_node_info->nexthop;
        }
      else
      ...
      if (ospf_spf_vertex_parent_find (
              ...
              q_space->q_node_info->node))
        {
          ...
          labels[0] = ospf_sr_get_adj_sid_by_id (
              &inner_backup_path_info.q_node_info.node->id,
              &q_space->q_node_info->node->id);
          ospf_ti_lfa_append_label_stack (q_space->label_stack, labels, 1);
        }
      else
        {
...
static void
ospf_ti_lfa_generate_q_spaces (struct ospf_area *area, struct p_space *p_space,
                               ...
                               struct list *pc_path)
{
  ...
  node_protected =
      ...
      dest->id.s_addr == p_space->protected_resource->router_id.s_addr;
      ...
      if (recursive)
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (dest->children, node, child))
            ospf_ti_lfa_generate_q_spaces (area, p_space, child, recursive,
                                           pc_path);
        }
      return;
    }
  ...
  q_space->p_node_info =
      XCALLOC (MTYPE_OSPF_Q_SPACE, sizeof (struct ospf_ti_lfa_node_info));
  q_space->q_node_info =
      XCALLOC (MTYPE_OSPF_Q_SPACE, sizeof (struct ospf_ti_lfa_node_info));
  ...
  ospf_spf_calculate (area, dest->lsa_p, new_table, NULL, NULL, true, false);
  ...
  if (pc_path)
    q_space->pc_path =
        ospf_ti_lfa_map_path_to_pc_vertices (pc_path, p_space->pc_vertex_list);
  else
  ...
  if (! q_space->pc_path)
    {
      zlog_info (
          ...
          __func__, &p_space->root->id, &q_space->root->id, res_buf);
  ...
  if (q_space->label_stack)
    {
      ...
      zlog_info (
          ...
          __func__, label_buf, &p_space->root->id, &q_space->root->id,
          res_buf);
    }
  else
    {
...
static void
ospf_ti_lfa_generate_post_convergence_spf (struct ospf_area *area,
                                           struct p_space *p_space)
{
  struct route_table *new_table;
   ...
   */
  ospf_spf_calculate (area, area->router_lsa_self, new_table, NULL, NULL, true,
                      false);
...
void
ospf_ti_lfa_generate_p_spaces (struct ospf_area *area,
          ...
          if (l->m[0].type == LSA_LINK_TYPE_POINTOPOINT)
            {
              protected_resource = XCALLOC (
                  MTYPE_OSPF_P_SPACE, sizeof (struct protected_resource));
              protected_resource->type = protection_type;
              protected_resource->router_id = l->link_id;
              child = ospf_spf_vertex_find (protected_resource->router_id,
                                            root->children);
              if (child)
                ospf_ti_lfa_generate_p_space (area, child, protected_resource,
                                              true, NULL);
            }
      ...
      for (ALL_LIST_ELEMENTS_RO (root->children, node, child))
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (child->parents, inner_node,
                                     vertex_parent))
            {
              ...
              child_prefix.u.prefix4 = vertex_parent->nexthop->router;
              ...
              if (prefix_match (&stub_prefix, &child_prefix))
                {
                  zlog_info ("%s: Generating P space for %pI4", __func__,
                             &l->link_id);
                  ...
                  ospf_ti_lfa_generate_p_space (
                      area, child, protected_resource, true, NULL);
                }
            }
        }
    }
...
ospf_ti_lfa_get_p_space_by_path (struct ospf_area *area,
  ...
  frr_each (p_spaces, area->p_spaces, p_space)
    {
      ...
      if (type == OSPF_TI_LFA_NODE_PROTECTION)
        {
          child = ospf_spf_vertex_by_nexthop (area->spf, &path->nexthop);
          if (child && p_space->protected_resource->router_id.s_addr ==
                           child->id.s_addr)
            return p_space;
        }
    }
...
void
ospf_ti_lfa_insert_backup_paths (struct ospf_area *area,
                                 struct route_table *new_table)
{
      ...
      for (ALL_LIST_ELEMENTS_RO (or->paths, node, path))
        {
          ...
          if (IS_DEBUG_OSPF_TI_LFA)
            zlog_debug (
                ...
                __func__, &rn->p, &path->adv_router, &path->nexthop);
          ...
          if (! p_space)
            {
              if (IS_DEBUG_OSPF_TI_LFA)
                zlog_debug (
                    ...
                    __func__, &path->adv_router, &path->nexthop);
              continue;
            }

          root_search.id = path->adv_router;
          q_space_search.root = &root_search;
          q_space = q_spaces_find (p_space->q_spaces, &q_space_search);
          if (! q_space)
            {
              if (IS_DEBUG_OSPF_TI_LFA)
                zlog_debug (
          ...
          if (path->srni.backup_label_stack)
            {
              mpls_label2str (path->srni.backup_label_stack->num_labels,
                              path->srni.backup_label_stack->label, label_buf,
                              MPLS_LABEL_STRLEN, 0, true);
              if (IS_DEBUG_OSPF_TI_LFA)
                zlog_debug (
                    "%s: inserted backup path %s for prefix %pFX, router id %pI4 and nexthop %pI4.",
                    __func__, label_buf, &rn->p, &path->adv_router,
                    &path->nexthop);
            }
          else
            {
...
void
ospf_ti_lfa_free_p_spaces (struct ospf_area *area)
{
  struct p_space *p_space;
  struct q_space *q_space;
...
void
ospf_ti_lfa_compute (struct ospf_area *area, struct route_table *new_table,
                     enum protection_type protection_type)
{
  /*
