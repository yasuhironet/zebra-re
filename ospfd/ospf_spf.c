 ...
 */
static int
vertex_cmp (const struct vertex *v1, const struct vertex *v2)
{
...
static void
lsdb_clean_stat (struct ospf_lsdb *lsdb)
{
...
int
vertex_parent_cmp (void *aa, void *bb)
{
...
ospf_vertex_new (struct ospf_area *area, struct ospf_lsa *lsa)
{
  struct vertex *new;
...
static void
ospf_vertex_dump (const char *msg, struct vertex *v, int print_parents,
                  int print_children)
{
  if (! IS_DEBUG_OSPF_EVENT)
    return;
          ...
          if (vp)
            {
              zlog_debug ("parent %pI4 backlink %d nexthop %pI4 lsa pos %d",
                          ...
                          &vp->nexthop->router, vp->nexthop->lsa_pos);
            }
        }
    }
...
struct vertex *
ospf_spf_vertex_find (struct in_addr id, struct list *vertex_list)
{
  struct listnode *node;
  struct vertex *found;
...
ospf_spf_vertex_parent_find (struct in_addr id, struct vertex *vertex)
{
  struct listnode *node;
...
struct vertex *
ospf_spf_vertex_by_nexthop (struct vertex *root, struct in_addr *nexthop)
{
  struct listnode *node;
  struct vertex *child;
...
static struct vertex *
ospf_spf_vertex_copy (struct vertex *vertex)
{
  struct vertex *copy;
...
void
ospf_spf_copy (struct vertex *vertex, struct list *vertex_list)
{
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (vertex->parents, node, vertex_parent))
    {
      parent_copy =
          ospf_spf_vertex_find (vertex_parent->parent->id, vertex_list);
      if (! parent_copy)
        {
          parent_copy = ospf_spf_vertex_copy (vertex_parent->parent);
...
static void
ospf_spf_remove_branch (struct vertex_parent *vertex_parent,
                        struct vertex *child, struct list *vertex_list)
{
  ...
  if (child->parents->count == 0)
    {
      for (ALL_LIST_ELEMENTS (child->children, node, nnode, grandchild))
        {
          for (ALL_LIST_ELEMENTS (grandchild->parents, inner_node, inner_nnode,
                                  vertex_parent_found))
            {
              ospf_spf_remove_branch (vertex_parent_found, grandchild,
                                      vertex_list);
            }
        }
...
static int
ospf_spf_remove_link (struct vertex *vertex, struct list *vertex_list,
                      struct router_lsa_link *link)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (vertex->children, node, child))
    {
      for (ALL_LIST_ELEMENTS_RO (child->parents, inner_node, vertex_parent))
        {
          if ((vertex_parent->local_nexthop->router.s_addr &
               link->link_data.s_addr) ==
              (link->link_id.s_addr & link->link_data.s_addr))
            {
              ospf_spf_remove_branch (vertex_parent, child, vertex_list);
              return 0;
            }
        }
    }
...
void
ospf_spf_remove_resource (struct vertex *vertex, struct list *vertex_list,
                          struct protected_resource *resource)
{
  ...
  switch (resource->type)
    ...
    case OSPF_TI_LFA_NODE_PROTECTION:
       ...
       */
      for (ALL_LIST_ELEMENTS (found->parents, node, nnode, vertex_parent))
        ospf_spf_remove_branch (vertex_parent, found, vertex_list);
...
static void
ospf_spf_init (struct ospf_area *area, struct ospf_lsa *root_lsa,
...
ospf_spf_add_parent (struct vertex *v, struct vertex *w,
                     ...
                     struct vertex_nexthop *newlhop, unsigned int distance)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: Adding %pI4 as parent of %pI4", __func__, &v->lsa->id,
                &w->lsa->id);
...
static int
match_stub_prefix (struct lsa_header *lsa, struct in_addr v_link_addr,
  ...
  while (p < lim)
    {
      ...
      masked_lsa_addr.s_addr = (l->link_id.s_addr & l->link_data.s_addr);
  ...
  if (v == area->spf)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: considering link type:%d link_id:%pI4 link_data:%pI4",
                    __func__, l->m[0].type, &l->link_id, &l->link_data);
          ...
          if (l->m[0].type == LSA_LINK_TYPE_POINTOPOINT)
            {
                  ...
                  if (! oi)
                    {
                      zlog_debug (
                          ...
                          __func__, lsa_pos, &l->link_id, &l->link_data);
                      return 0;
                    }
                }
              ...
              if (area->ospf->ti_lfa_enabled ||
                   ...
                   oi->address->prefixlen == IPV4_MAX_BITLEN))
                {
                  ...
                  if (area->spf_root_node)
                    {
                      nbr_w =
                          ospf_nbr_lookup_by_routerid (oi->nbrs, &l->link_id);
                      if (nbr_w)
                        {
                          added = 1;
                          nexthop = nbr_w->src;
                  ...
                  if (! added)
                    {
                      while ((l2 = ospf_get_next_link (w, v, l2)))
                        {
                          if (match_stub_prefix (v->lsa, l->link_data,
                                                 l2->link_data))
                            {
                              added = 1;
              ...
              else if (oi && oi->type == OSPF_IFTYPE_POINTOMULTIPOINT)
                {
          ...
          else if (l->m[0].type == LSA_LINK_TYPE_VIRTUALLINK)
            {
              ...
              if (vl_data &&
                ...
                zlog_info ("%s: vl_data for VL link not found", __func__);
...
static int
ospf_spf_is_protected_resource (struct ospf_area *area,
                                struct router_lsa_link *link,
 ...
 */
static uint16_t
get_reverse_distance (struct vertex *v, struct router_lsa_link *l,
      ...
      if (v->lsa->type == OSPF_ROUTER_LSA)
        {
          ...
          switch (type)
            ...
            case LSA_LINK_TYPE_POINTOPOINT:
            case LSA_LINK_TYPE_VIRTUALLINK:
              if (type == LSA_LINK_TYPE_VIRTUALLINK && IS_DEBUG_OSPF_EVENT)
                zlog_debug ("looking up LSA through VL: %pI4", &l->link_id);
              w_lsa = ospf_lsa_lookup (area->ospf, area, OSPF_ROUTER_LSA,
                                       l->link_id, l->link_id);
              if (w_lsa && IS_DEBUG_OSPF_EVENT)
                zlog_debug ("found Router LSA %pI4", &l->link_id);
              break;
            case LSA_LINK_TYPE_TRANSIT:
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("Looking up Network LSA, ID: %pI4", &l->link_id);
              w_lsa =
                  ospf_lsa_lookup_by_id (area, OSPF_NETWORK_LSA, l->link_id);
              if (w_lsa && IS_DEBUG_OSPF_EVENT)
                zlog_debug ("found the LSA");
              break;
            default:
              flog_warn (EC_OSPF_LSA, "Invalid LSA link type %d", type);
              continue;
            }
           ...
           */
          if (type == LSA_LINK_TYPE_POINTOPOINT && area->spf_reversed)
            link_distance = get_reverse_distance (v, l, w_lsa);
          else
      ...
      else
        {
          ...
          if (w_lsa && IS_DEBUG_OSPF_EVENT)
            zlog_debug ("found Router LSA %pI4", &w_lsa->data->id);
...
ospf_spf_dump (struct vertex *v, int i)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    for (ALL_LIST_ELEMENTS_RO (v->parents, nnode, parent))
      {
        zlog_debug (" nexthop %p %pI4 %d", (void *) parent->nexthop,
...
void
ospf_spf_print (struct vty *vty, struct vertex *v, int i)
{
  struct listnode *cnode;
  struct listnode *nnode;
  struct vertex_parent *parent;
  ...
  for (ALL_LIST_ELEMENTS_RO (v->parents, nnode, parent))
    {
      vty_out (vty,
               ...
               &parent->nexthop->router, parent->nexthop->lsa_pos,
               &parent->local_nexthop->router, parent->local_nexthop->lsa_pos);
    }
  ...
  if (v->type == OSPF_VERTEX_ROUTER)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: processing router LSA, id: %pI4", __func__,
                    &v->lsa->id);
...
void
ospf_spf_cleanup (struct vertex *spf, struct list *vertex_list)
{
  /*
...
void
ospf_spf_calculate (struct ospf_area *area, struct ospf_lsa *root_lsa,
                    ...
                    bool is_dry_run, bool is_root_node)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      zlog_debug ("%s: Start: running Dijkstra for area %pI4", __func__,
                  &area->area_id);
    }
...
void
ospf_spf_calculate_area (struct ospf *ospf, struct ospf_area *area,
                         struct route_table *new_table,
...
void
ospf_spf_calculate_areas (struct ospf *ospf, struct route_table *new_table,
                          struct route_table *all_rtrs,
                          struct route_table *new_rtrs)
{
  ...
  for (ALL_LIST_ELEMENTS (ospf->areas, node, nnode, area))
    {
      ...
      ospf_spf_calculate_area (ospf, area, new_table, all_rtrs, new_rtrs);
    }

  /* SPF for backbone, if required */
  if (ospf->backbone)
    ospf_spf_calculate_area (ospf, ospf->backbone, new_table, all_rtrs,
                             new_rtrs);
}

/* Worker for SPF calculation scheduler. */
static void
ospf_spf_calculate_schedule_worker (struct event *thread)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: ospf install new route, vrf %s id %u new_table count %lu",
                __func__, ospf_vrf_id_to_name (ospf->vrf_id), ospf->vrf_id,
                new_table->count);
...
void
ospf_restart_spf (struct ospf *ospf)
{
  if (IS_DEBUG_OSPF_EVENT)
