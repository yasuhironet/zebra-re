...
static int
ospf_router_id_update_zebra (ZAPI_CALLBACK_ARGS)
{
  ...
  if (IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
    zlog_debug ("Zebra rcvd: router id update %pFX vrf %s id %u", &router_id,
  ...
  else
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (
            ...
            __func__, ospf_vrf_id_to_name (vrf_id), vrf_id, &router_id);
    }
  return 0;
}

static int
ospf_interface_address_add (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
  struct ospf *ospf = NULL;
  ...
  if (IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
    zlog_debug ("Zebra: interface %s address add %pFX vrf %s id %u",
                c->ifp->name, c->address, ospf_vrf_id_to_name (vrf_id),
                vrf_id);
...
static int
ospf_interface_address_delete (ZAPI_CALLBACK_ARGS)
{
  struct connected *c;
  ...
  if (IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
    zlog_debug ("Zebra: interface %s address delete %pFX", c->ifp->name,
                c->address);
...
static int
ospf_interface_link_params (ZAPI_CALLBACK_ARGS)
{
  struct interface *ifp;
  bool changed = false;
...
static int
ospf_interface_vrf_update (ZAPI_CALLBACK_ARGS)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: Rx Interface %s VRF change vrf_id %u New vrf %s id %u",
                __func__, ifp->name, vrf_id, ospf_vrf_id_to_name (new_vrf_id),
                new_vrf_id);
...
static void
ospf_zebra_add_nexthop (struct ospf *ospf, struct ospf_path *path,
  ...
  if (path->srni.backup_label_stack)
    {
      ...
      api_nh_backup->label_num = path->srni.backup_label_stack->num_labels;
      memcpy (api_nh_backup->labels, path->srni.backup_label_stack->label,
...
static void
ospf_zebra_append_opaque_attr (struct ospf_route * or, struct zapi_route *api)
{
  ...
  switch (or->path_type)
    ...
    case OSPF_PATH_INTER_AREA:
      /* OSPF area ID */
      (void) inet_ntop (AF_INET, & or->u.std.area_id, ospf_opaque.area_id,
...
void
ospf_zebra_add (struct ospf *ospf, struct prefix_ipv4 *p,
  ...
  if (ospf->gr_info.restart_in_progress)
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug (
            "Zebra: Graceful Restart in progress -- not installing %pFX", p);
      return;
    }
      ...
      if (IS_DEBUG_OSPF (zebra, ZEBRA_REDISTRIBUTE))
        {
          ...
          zlog_debug ("Zebra: Route add %pFX nexthop %pI4, ifindex=%d %s", p,
                      &path->nexthop, path->ifindex, ifp ? ifp->name : " ");
        }
    }
...
void
ospf_zebra_delete (struct ospf *ospf, struct prefix_ipv4 *p,
                   struct ospf_route * or)
{
  struct zapi_route api;
  ...
  if (ospf->gr_info.restart_in_progress)
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug (
            "Zebra: Graceful Restart in progress -- not uninstalling %pFX", p);
      return;
    }
...
void
ospf_zebra_add_discard (struct ospf *ospf, struct prefix_ipv4 *p)
{
  struct zapi_route api;
...
void
ospf_zebra_delete_discard (struct ospf *ospf, struct prefix_ipv4 *p)
{
  struct zapi_route api;
...
ospf_external_lookup (struct ospf *ospf, uint8_t type, unsigned short instance)
{
...
struct ospf_external *
ospf_external_add (struct ospf *ospf, uint8_t type, unsigned short instance)
{
  struct list *ext_list;
  struct ospf_external *ext;
 ...
 */
bool
ospf_external_default_routemap_apply_walk (struct ospf *ospf,
                                           struct list *ext_list,
 ...
 */
static void
ospf_external_lsa_default_routemap_timer (struct event *thread)
{
  struct list *ext_list;
  ...
  default_ei =
      ospf_external_info_lookup (ospf, DEFAULT_ROUTE, ospf->instance, &p);
  if (! default_ei)
    {
...
void
ospf_external_del (struct ospf *ospf, uint8_t type, unsigned short instance)
{
  struct ospf_external *ext;
...
void
ospf_zebra_update_prefix_sid (const struct sr_prefix *srp)
{
  ...
  switch (srp->type)
    ...
    case LOCAL_SID:
      ...
      osr_debug ("SR (%s): Configure Prefix %pFX with labels %u/%u", __func__,
      ...
      for (ALL_LIST_ELEMENTS_RO (srp->route->paths, node, path))
        {
          ...
          if (path->srni.backup_label_stack)
            {
              znh_backup = &zl.backup_nexthops[zl.backup_nexthop_num++];
              znh_backup->type = NEXTHOP_TYPE_IPV4;
              znh_backup->gate.ipv4 = path->srni.backup_nexthop;

              memcpy (znh_backup->labels, path->srni.backup_label_stack->label,
                      sizeof (mpls_label_t) *
                          path->srni.backup_label_stack->num_labels);

              znh_backup->label_num =
                  path->srni.backup_label_stack->num_labels;
              if (path->srni.label_out != MPLS_LABEL_IPV4_EXPLICIT_NULL &&
          ...
          osr_debug ("  |- labels %u/%u", srp->label_in, path->srni.label_out);
...
void
ospf_zebra_delete_prefix_sid (const struct sr_prefix *srp)
{
  struct zapi_labels zl;
...
void
ospf_zebra_send_adjacency_sid (int cmd, struct sr_nhlfe nhlfe)
{
  struct zapi_labels zl;
  struct zapi_nexthop *znh;

  osr_debug ("SR (%s): %s Labels %u/%u for Adjacency via %u", __func__,
             cmd == ZEBRA_MPLS_LABELS_ADD ? "Add" : "Delete", nhlfe.label_in,
             nhlfe.label_out, nhlfe.ifindex);
...
struct ospf_redist *
ospf_redist_lookup (struct ospf *ospf, uint8_t type, unsigned short instance)
{
...
struct ospf_redist *
ospf_redist_add (struct ospf *ospf, uint8_t type, unsigned short instance)
{
  struct list *red_list;
  struct ospf_redist *red;
...
void
ospf_redist_del (struct ospf *ospf, uint8_t type, unsigned short instance)
{
  struct ospf_redist *red;
...
int
ospf_is_type_redistributed (struct ospf *ospf, int type,
                            unsigned short instance)
{
  return (
      DEFAULT_ROUTE_TYPE (type)
          ? vrf_bitmap_check (zclient->default_information[AFI_IP],
                              ospf->vrf_id)
          : ((instance && redist_check_instance (
                              &zclient->mi_redist[AFI_IP][type], instance)) ||
             (! instance && vrf_bitmap_check (zclient->redist[AFI_IP][type],
                                              ospf->vrf_id))));
}

int
ospf_redistribute_update (struct ospf *ospf, struct ospf_redist *red, int type,
...
int
ospf_redistribute_set (struct ospf *ospf, struct ospf_redist *red, int type,
                       unsigned short instance, int mtype, int mvalue)
{
  red->dmetric.type = mtype;
  red->dmetric.value = mvalue;
...
int
ospf_redistribute_unset (struct ospf *ospf, int type, unsigned short instance)
{
...
int
ospf_redistribute_default_set (struct ospf *ospf, int originate, int mtype,
                               int mvalue)
{
  struct prefix_ipv4 p;
  ...
  if (IS_DEBUG_OSPF (zebra, ZEBRA_REDISTRIBUTE))
    zlog_debug ("Redistribute[DEFAULT]: %s Type[%d], Metric[%d]", type_str,
                metric_type (ospf, DEFAULT_ROUTE, 0),
                metric_value (ospf, DEFAULT_ROUTE, 0));
...
static int
ospf_external_lsa_originate_check (struct ospf *ospf, struct external_info *ei)
{
...
int
ospf_external_info_apply_default_routemap (struct ospf *ospf,
 ...
 */
static bool
ospf_external_lsa_default_routemap_apply (struct ospf *ospf,
                                          struct external_info *ei, int cmd)
{
  struct external_info *default_ei;
  struct prefix_ipv4 p;
  struct ospf_lsa *lsa;
  ...
  if (cmd == ZEBRA_REDISTRIBUTE_ROUTE_ADD)
  ...
  else if (cmd == ZEBRA_REDISTRIBUTE_ROUTE_DEL)
    {
      ...
      if (! lsa)
        {
          if (IS_DEBUG_OSPF_DEFAULT_INFO)
            zlog_debug ("Default lsa not originated, not flushing");
          return true;
        }
...
int
ospf_redistribute_check (struct ospf *ospf, struct external_info *ei,
                         int *changed)
{
  struct route_map_set_values save_values;
  struct prefix_ipv4 *p = &ei->p;
  ...
  if (red && ROUTEMAP_NAME (red))
    {
      ...
      if (changed)
        {
          *changed =
              ! ospf_route_map_set_compare (&ei->route_map_set, &save_values);
...
static int
ospf_zebra_gr_update (struct ospf *ospf, int command, uint32_t stale_time)
{
  struct zapi_cap api;
  ...
  (void) zclient_capabilities_send (ZEBRA_CLIENT_CAPABILITIES, zclient, &api);

  return 0;
}

int
ospf_zebra_gr_enable (struct ospf *ospf, uint32_t stale_time)
{
  if (IS_DEBUG_OSPF_GR)
    zlog_debug ("Zebra enable GR [stale time %u]", stale_time);

  return ospf_zebra_gr_update (ospf, ZEBRA_CLIENT_GR_CAPABILITIES, stale_time);
}

int
ospf_zebra_gr_disable (struct ospf *ospf)
{
  if (IS_DEBUG_OSPF_GR)
...
static int
ospf_zebra_read_route (ZAPI_CALLBACK_ARGS)
{
  struct zapi_route api;
  struct prefix_ipv4 p;
  struct prefix pgen;
  ...
  if (cmd == ZEBRA_REDISTRIBUTE_ROUTE_ADD)
    {
      ...
      for (i = 0; i <= ZEBRA_ROUTE_MAX; i++)
        if (i != rt_type)
          ospf_external_info_delete (ospf, i, api.instance, p);

      ei = ospf_external_info_add (ospf, rt_type, api.instance, p, ifindex,
                                   nexthop, api.tag, api.metric);
      if (ei == NULL)
        {
              ...
              if (aggr)
                {
                  ...
                  if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
                    zlog_debug ("%s: Send Aggreate LSA (%pI4/%d)", __func__,
              ...
              else
                {
                  ...
                  if (! current)
                    ...
                    {
                      if (IS_DEBUG_OSPF (zebra, ZEBRA_REDISTRIBUTE))
                        zlog_debug ("%s: %pI4 refreshing LSA", __func__,
                                    &p.prefix);
    ...
    { /* if (cmd == ZEBRA_REDISTRIBUTE_ROUTE_DEL) */
      struct ospf_external_aggr_rt *aggr;
...
void
ospf_zebra_import_default_route (struct ospf *ospf, bool unreg)
{
  ...
  if (zclient_send_rnh (zclient, command, &prefix, SAFI_UNICAST, false, true,
                        ospf->vrf_id) == ZCLIENT_SEND_FAILURE)
    flog_err (EC_LIB_ZAPI_SOCKET, "%s: zclient_send_rnh() failed", __func__);
}

static int
ospf_zebra_import_check_update (ZAPI_CALLBACK_ARGS)
{
  struct ospf *ospf;
  ...
  if (! zapi_nexthop_update_decode (zclient->ibuf, &matched, &nhr))
    {
      zlog_err ("%s[%u]: Failure to decode route", __func__, ospf->vrf_id);
      return -1;
    }
...
static void
ospf_distribute_list_update_timer (struct event *thread)
{
  struct route_node *rn;
  struct external_info *ei;
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      zlog_debug ("%s: ospf distribute-list update vrf %s id %d", __func__,
              ...
              if (is_default_prefix4 (&ei->p))
              ...
              else
                {
                  ...
                  if (aggr)
                    ...
                    {
                      int force = LSA_REFRESH_IF_CHANGED;
                      /* If this is a MaxAge
                       * LSA, we need to
                       * force refresh it
                       * because distribute
      ...
      for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
        {
          ...
          if (PREFIX_NAME_IN (area) &&
              strcmp (PREFIX_NAME_IN (area), prefix_list_name (plist)) == 0)
            {
              PREFIX_LIST_IN (area) =
          ...
          if (PREFIX_NAME_OUT (area) &&
              strcmp (PREFIX_NAME_OUT (area), prefix_list_name (plist)) == 0)
            {
                     ...
                     struct ospf_route * or)
{
  ...
  if (ospf->distance_external && (or->path_type == OSPF_PATH_TYPE1_EXTERNAL ||
                                  or->path_type == OSPF_PATH_TYPE2_EXTERNAL))
...
void
ospf_zebra_vrf_register (struct ospf *ospf)
{
  if (! zclient || zclient->sock < 0 || ! ospf)
    return;
...
void
ospf_zebra_vrf_deregister (struct ospf *ospf)
{
  if (! zclient || zclient->sock < 0 || ! ospf)
    return;
  ...
  if (ospf->vrf_id != VRF_DEFAULT && ospf->vrf_id != VRF_UNKNOWN)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: De-Register VRF %s id %u to Zebra.", __func__,
                    ospf_vrf_id_to_name (ospf->vrf_id), ospf->vrf_id);
 ...
 */
bool
ospf_zebra_label_manager_ready (void)
{
 ...
 */
int
ospf_zebra_request_label_range (uint32_t base, uint32_t chunk_size)
{
  int ret;
  uint32_t start, end;
  ...
  ret = lm_get_label_chunk (zclient_sync, 0, base, chunk_size, &start, &end);
  if (ret < 0)
    {
 ...
 */
int
ospf_zebra_release_label_range (uint32_t start, uint32_t end)
{
  int ret;
 ...
 */
int
ospf_zebra_label_manager_connect (void)
{
  /* Connect to label manager. */
  if (zclient_socket_connect (zclient_sync) < 0)
    {
      zlog_warn ("%s: failed connecting synchronous zclient!", __func__);
      return -1;
    }
  ...
  osr_debug ("SR (%s): Successfully connected to the Label Manager", __func__);
 ...
 */
static int
ospf_opaque_msg_handler (ZAPI_CALLBACK_ARGS)
{
  struct stream *s;
...
static int
ospf_zebra_client_close_notify (ZAPI_CALLBACK_ARGS)
{
  int ret = 0;
...
void
ospf_zebra_init (struct event_loop *master, unsigned short instance)
{
...
void
ospf_zebra_send_arp (const struct interface *ifp, const struct prefix *p)
{
