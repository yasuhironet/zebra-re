...
int
ospf_interface_neighbor_count (struct ospf_interface *oi)
{
  int count = 0;
...
ospf_if_new (struct ospf *ospf, struct interface *ifp, struct prefix *p)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: ospf interface %s vrf %s id %u created", __func__,
                ifp->name, ospf_get_name (ospf), ospf->vrf_id);
...
ospf_if_free (struct ospf_interface *oi)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: ospf interface %s vrf %s id %u deleted", __func__,
                oi->ifp->name, oi->ifp->vrf->name, oi->ifp->vrf->vrf_id);
...
void
ospf_interface_fifo_flush (struct ospf_interface *oi)
{
...
static void
ospf_if_reset_stats (struct ospf_interface *oi)
{
...
static void
ospf_del_if_params (struct interface *ifp, struct ospf_if_params *oip)
{
  ...
  for (rn = route_top (ospf->new_table); rn; rn = route_next (rn))
    {
      ...
      for (nh = listhead (or->paths); nh; nh = listnextnode_unchecked (nh))
        {
          op = listgetdata (nh);
...
ospf_vl_new (struct ospf *ospf, struct ospf_vl_data *vl_data)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: creating pseudo zebra interface vrf id %u", __func__,
                ospf->vrf_id);
  ...
  if (voi == NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: Alarm: OSPF int structure is not created", __func__);
  ...
  vlink_count++;
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: Created name: %s set if->name to %s", __func__, ifname,
                vi->name);
 ...
 */
int
ospf_vl_count (struct ospf *ospf, struct ospf_area *area)
{
  int count = 0;
  struct ospf_vl_data *vl_data;
  struct listnode *node;
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      ...
      if (area)
        zlog_debug ("%s: in area %pI4", __func__, &area->area_id);
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->vlinks, node, vl_data))
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: VL %s, peer %pI4", __func__,
                    vl_data->vl_oi->ifp->name, &vl_data->vl_peer);
...
static int
ospf_vl_set_params (struct ospf_area *area, struct ospf_vl_data *vl_data,
                    struct vertex *v)
{
  int changed = 0;
  struct ospf_interface *voi;
  ...
  for (ALL_LIST_ELEMENTS_RO (v->parents, node, vp))
    {
      ...
      if (! area->spf_dry_run)
        {
          oi = ospf_if_lookup_by_lsa_pos (area, vl_data->nexthop.lsa_pos);
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: %s peer address: %pI4, cost: %d,%schanged", __func__,
...
void
ospf_vl_up_check (struct ospf_area *area, struct in_addr rid, struct vertex *v)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->vlinks, node, vl_data))
    {
      if (IS_DEBUG_OSPF_EVENT)
        {
          zlog_debug ("%s: considering VL, %s in area %pI4", __func__,
                      vl_data->vl_oi->ifp->name, &vl_data->vl_area_id);
          zlog_debug ("%s: peer ID: %pI4", __func__, &vl_data->vl_peer);
        }
      ...
      if (IPV4_ADDR_SAME (&vl_data->vl_peer, &rid) &&
          IPV4_ADDR_SAME (&vl_data->vl_area_id, &area->area_id))
        {
          ...
          if (oi->state == ISM_Down)
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("%s: VL is down, waking it up", __func__);
          ...
          if (ospf_vl_set_params (area, vl_data, v))
            {
              ...
              if (ospf->backbone)
                ...
                zlog_debug ("%s: VL cost change, no backbone!", __func__);
            }
        }
    }
}
...
struct crypt_key *
ospf_crypt_key_lookup (struct list *auth_crypt, uint8_t key_id)
{
  struct listnode *node;
  struct crypt_key *ck;
...
int
ospf_crypt_key_delete (struct list *auth_crypt, uint8_t key_id)
{
  struct listnode *node, *nnode;
  struct crypt_key *ck;
...
void
ospf_if_interface (struct interface *ifp)
{
  hook_call (ospf_if_update, ifp);
}

uint32_t
ospf_if_count_area_params (struct interface *ifp)
{
...
static int
ospf_ifp_create (struct interface *ifp)
{
  ...
  if (IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
    zlog_debug (
        ...
        (unsigned long long) ifp->flags, ifp->metric, ifp->mtu, ifp->speed,
        ifp->status);
...
static int
ospf_ifp_up (struct interface *ifp)
{
  struct ospf_interface *oi;
  struct route_node *rn;
...
static int
ospf_ifp_down (struct interface *ifp)
{
  struct ospf_interface *oi;
...
static int
ospf_ifp_destroy (struct interface *ifp)
{
  struct ospf *ospf;
  struct route_node *rn;
...
void
ospf_reset_hello_timer (struct interface *ifp, struct in_addr addr,
                        bool is_addr)
{
  struct route_node *rn;
