...
int
ospf_if_drop_alldrouters (struct ospf *top, struct prefix *p,
                          ifindex_t ifindex)
{
  int ret;
...
int
ospf_if_ipmulticast (int fd, struct prefix *p, ifindex_t ifindex)
{
  uint8_t val;
  int ret, len;
  ...
  if (ret < 0)
    flog_err (EC_LIB_SOCKET,
              "can't setsockopt IP_MULTICAST_LOOP(0) for fd %d: %s", fd,
              safe_strerror (errno));
  ...
  if (ret < 0)
    flog_err (EC_LIB_SOCKET,
              "can't setsockopt IP_MULTICAST_TTL(1) for fd %d: %s", fd,
              safe_strerror (errno));
#ifndef GNU_LINUX
 ...
 */
static int
sock_init_common (vrf_id_t vrf_id, const char *name, int *pfd)
{
  int ospf_sock;
  int ret, hincl = 1;
  ...
  frr_with_privs (&ospfd_privs)
    {
      ospf_sock =
          vrf_socket (AF_INET, SOCK_RAW, IPPROTO_OSPFIGP, vrf_id, name);
      if (ospf_sock < 0)
        {
      ...
      if (ret < 0)
        {
          flog_err (EC_LIB_SOCKET, "Can't set IP_HDRINCL option for fd %d: %s",
...
#warning "using IPTOS_PREC_INTERNETCONTROL"
      ...
      if (ret < 0)
        {
          flog_err (EC_LIB_SOCKET,
                    "can't set sockopt IP_TOS %d to socket %d: %s", tos,
                    ospf_sock, safe_strerror (errno));
          break;
        }
...
#warning "IP_HDRINCL not available, nor is IPTOS_PREC_INTERNETCONTROL"
      ...
      if (ret < 0)
        flog_err (EC_LIB_SOCKET, "Can't set pktinfo option for fd %d",
                  ospf_sock);
    }
 ...
 */
void
ospf_sock_bufsize_update (const struct ospf *ospf, int sock,
 ...
 */
int
ospf_ifp_sock_init (struct interface *ifp)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: ifp %s, oii %p, fd %d", __func__, ifp->name, oii,
                oii->oii_fd);
 ...
 */
int
ospf_ifp_sock_close (struct interface *ifp)
{
  struct ospf_if_info *oii;
