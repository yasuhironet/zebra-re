...
ospf_external_info_new (struct ospf *ospf, uint8_t type,
...
/* Add an External info for AS-external-LSA. */
struct external_info *
ospf_external_info_add (struct ospf *ospf, uint8_t type,
  ...
  if (rn && rn->info)
    {
      ...
      if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
        zlog_debug (
            ...
            ospf_redist_string (type), instance, ospf->vrf_id, &p,
            &nexthop.s_addr);
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug (
          ...
          ospf_redist_string (type), ospf->vrf_id, &p, &nexthop.s_addr,
          metric);
    }
  return new;
}
...
void
ospf_external_info_delete (struct ospf *ospf, uint8_t type,
                           unsigned short instance, struct prefix_ipv4 p)
{
  struct route_node *rn;
  struct ospf_external *ext;
...
struct external_info *
ospf_external_info_lookup (struct ospf *ospf, uint8_t type,
                           unsigned short instance, struct prefix_ipv4 *p)
{
  struct route_node *rn;
  struct ospf_external *ext;
...
ospf_external_info_find_lsa (struct ospf *ospf, struct prefix_ipv4 *p)
{
  ...
  lsa = ospf_lsdb_lookup_by_id (ospf->lsdb, OSPF_AS_EXTERNAL_LSA, p->prefix,
                                ospf->router_id);
...
/* Update ASBR status. */
void
ospf_asbr_status_update (struct ospf *ospf, uint8_t status)
{
  zlog_info ("ASBR[%s:Status:%d]: Update", ospf_get_name (ospf), status);
  ...
  if (status)
    {
      ...
      if (IS_OSPF_ASBR (ospf))
        {
          zlog_info ("ASBR[%s:Status:%d]: Already ASBR", ospf_get_name (ospf),
                     status);
          return;
        }
 ...
 */
static void
ospf_asbr_nssa_redist_update_timer (struct event *thread)
{
...
void
ospf_schedule_asbr_nssa_redist_update (struct ospf *ospf)
{
  if (IS_DEBUG_OSPF_EVENT)
...
void
ospf_redistribute_withdraw (struct ospf *ospf, uint8_t type,
                            unsigned short instance)
{
  struct route_node *rn;
...
void
ospf_asbr_external_aggregator_init (struct ospf *instance)
{
...
static unsigned int
ospf_external_rt_hash_key (const void *data)
{
...
static bool
ospf_external_rt_hash_cmp (const void *d1, const void *d2)
{
...
ospf_external_aggregator_new (struct prefix_ipv4 *p)
{
  ...
  aggr = (struct ospf_external_aggr_rt *) XCALLOC (
      MTYPE_OSPF_EXTERNAL_RT_AGGR, sizeof (struct ospf_external_aggr_rt));
  ...
  aggr->match_extnl_hash =
      hash_create (ospf_external_rt_hash_key, ospf_external_rt_hash_cmp,
                   "Ospf external route hash");
  return aggr;
}

static void
ospf_aggr_handle_external_info (void *data)
{
  ...
  if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
    zlog_debug ("%s: Handle extrenal route(%pI4/%d)", __func__, &ei->p.prefix,
                ei->p.prefixlen);
...
static void
ospf_aggr_unlink_external_info (void *data)
{
  struct external_info *ei = (struct external_info *) data;
...
void
ospf_external_aggregator_free (struct ospf_external_aggr_rt *aggr)
{
  hash_clean_and_free (&aggr->match_extnl_hash,
                       (void *) ospf_aggr_unlink_external_info);

  if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
    zlog_debug ("%s: Release the aggregator Address(%pI4/%d)", __func__,
...
static void
ospf_external_aggr_add (struct ospf *ospf, struct ospf_external_aggr_rt *aggr)
{
  struct route_node *rn;

  if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
    zlog_debug ("%s: Adding Aggregate route to Aggr table (%pI4/%d)", __func__,
                &aggr->p.prefix, aggr->p.prefixlen);
...
static void
ospf_external_aggr_delete (struct ospf *ospf, struct route_node *rn)
{
  ...
  if (CHECK_FLAG (aggr->flags, OSPF_EXTERNAL_AGGRT_ORIGINATED))
    {
      if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
        zlog_debug ("%s: Flushing Aggregate route (%pI4/%d)", __func__,
                    &aggr->p.prefix, aggr->p.prefixlen);
        ...
        if (node->info)
          {
            ...
            zlog_debug (
                ...
                __func__, &p->prefix, p->prefixlen, &ag->p.prefix,
                ag->p.prefixlen);
          }
...
void
ospf_unlink_ei_from_aggr (struct ospf *ospf,
                          struct ospf_external_aggr_rt *aggr,
                          struct external_info *ei)
{
  if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
    zlog_debug (
        ...
        __func__, &ei->p.prefix, ei->p.prefixlen, &aggr->p.prefix,
  ...
  if (! OSPF_EXTERNAL_RT_COUNT (aggr) &&
      CHECK_FLAG (aggr->flags, OSPF_EXTERNAL_AGGRT_ORIGINATED))
    {
      ...
      if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
        zlog_debug ("%s: Flushing the aggreagte route (%pI4/%d)", __func__,
                    &aggr->p.prefix, aggr->p.prefixlen);
...
static void
ospf_link_ei_to_aggr (struct ospf_external_aggr_rt *aggr,
                      struct external_info *ei)
{
  if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
    zlog_debug ("%s: Linking extrenal route(%pI4/%d) to aggregator(%pI4/%d)",
                __func__, &ei->p.prefix, ei->p.prefixlen, &aggr->p.prefix,
                aggr->p.prefixlen);
                            ...
                            struct external_info *ei)
{
  ...
  if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
    zlog_debug ("%s: Prepare to originate Summary route(%pI4/%d)", __func__,
                &aggr->p.prefix, aggr->p.prefixlen);
      ...
      if (! lsa)
        {
          flog_warn (EC_OSPF_LSA_MISSING,
                     "%s: Could not refresh/originate %pI4/%d", __func__,
                     &aggr->p.prefix, aggr->p.prefixlen);
          return NULL;
        }
      ...
      if (tag != ei_aggr.tag)
        {
          if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
            zlog_debug (
                ...
                __func__, tag, ei_aggr.tag, &aggr->p.prefix,
                aggr->p.prefixlen);

          ospf_external_lsa_refresh (ospf, lsa, &ei_aggr, LSA_REFRESH_FORCE,
                                     1);
        }
      return lsa;
    }
  ...
  if (lsa && IS_LSA_MAXAGE (lsa))
    {
       ...
       */
      if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
        zlog_debug ("%s: LSA is in MAX-AGE so refreshing LSA(%pI4/%d)",
                    __func__, &aggr->p.prefix, aggr->p.prefixlen);

      ospf_external_lsa_refresh (ospf, lsa, &ei_aggr, LSA_REFRESH_FORCE, 1);
   ...
   */
  if (lsa &&
      prefix_same ((struct prefix *) &ei_aggr.p, (struct prefix *) &ei->p))
    {
      if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
        zlog_debug (
...
void
ospf_unset_all_aggr_flag (struct ospf *ospf)
{
...
static void
ospf_delete_all_marked_aggregators (struct ospf *ospf)
{
  struct route_node *rn = NULL;
...
static void
ospf_handle_aggregated_exnl_rt (struct ospf *ospf,
                                struct ospf_external_aggr_rt *aggr,
                                struct external_info *ei)
{
  struct ospf_lsa *lsa;
...
static void
ospf_handle_exnl_rt_after_aggr_del (struct ospf *ospf,
                                    struct external_info *ei)
{
  struct ospf_lsa *lsa;
  ...
  if (lsa)
    ...
    {
      if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
        zlog_debug ("%s: Originate external route(%pI4/%d)", __func__,
...
static void
ospf_handle_external_aggr_add (struct ospf *ospf)
{
  struct external_info *ei;
  struct route_node *rn = NULL;
              ...
              if (aggr)
                {
                   ...
                   * are handled here.
                   */
                  ospf_handle_aggregated_exnl_rt (ospf, aggr, ei);
                  continue;
                }
  ...
  if (! CHECK_FLAG (aggr->flags, OSPF_EXTERNAL_AGGRT_ORIGINATED))
    {
      ...
      if (lsa && IS_LSA_MAXAGE (lsa))
      ...
      else
        {
          ...
          if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
            zlog_debug ("%s: Originate Aggregate LSA (%pI4/%d)", __func__,
                        &aggr->p.prefix, aggr->p.prefixlen);
...
static void
ospf_handle_external_aggr_update (struct ospf *ospf)
{
  struct route_node *rn = NULL;
...
static void
ospf_asbr_external_aggr_process (struct event *thread)
{
  struct ospf *ospf = EVENT_ARG (thread);
  int operation = 0;
...
static void
ospf_external_aggr_timer (struct ospf *ospf,
                          struct ospf_external_aggr_rt *aggr,
...
int
ospf_asbr_external_aggregator_set (struct ospf *ospf, struct prefix_ipv4 *p,
                                   route_tag_t tag)
{
  ...
  if (aggregator)
    {
      ...
      ospf_external_aggr_timer (ospf, aggregator, OSPF_ROUTE_AGGR_MODIFY);
    }
  else
    {
...
int
ospf_asbr_external_aggregator_unset (struct ospf *ospf, struct prefix_ipv4 *p,
                                     route_tag_t tag)
{
  struct route_node *rn;
  struct ospf_external_aggr_rt *aggr;
...
int
ospf_asbr_external_rt_no_advertise (struct ospf *ospf, struct prefix_ipv4 *p)
{
  struct ospf_external_aggr_rt *aggr;
  route_tag_t tag = 0;
...
int
ospf_asbr_external_rt_advertise (struct ospf *ospf, struct prefix_ipv4 *p)
{
  struct route_node *rn;
  struct ospf_external_aggr_rt *aggr;
...
int
ospf_external_aggregator_timer_set (struct ospf *ospf, uint16_t interval)
{
