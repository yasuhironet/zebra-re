...
static void
ospf_area_range_add (struct ospf_area *area, struct route_table *ranges,
                     struct ospf_area_range *range)
{
...
ospf_area_range_lookup (struct ospf_area *area, struct route_table *ranges,
                        struct prefix_ipv4 *p)
{
  struct route_node *rn;
...
static struct ospf_area_range *
ospf_area_range_match (struct ospf_area *area, struct route_table *ranges,
                       struct prefix_ipv4 *p)
{
...
int
ospf_area_range_set (struct ospf *ospf, struct ospf_area *area,
...
int
ospf_area_range_cost_set (struct ospf *ospf, struct ospf_area *area,
                          struct route_table *ranges, struct prefix_ipv4 *p,
                          uint32_t cost)
{
  struct ospf_area_range *range;
...
int
ospf_area_range_unset (struct ospf *ospf, struct ospf_area *area,
                       struct route_table *ranges, struct prefix_ipv4 *p)
{
  struct route_node *rn;
...
int
ospf_area_range_substitute_set (struct ospf *ospf, struct ospf_area *area,
...
int
ospf_area_range_substitute_unset (struct ospf *ospf, struct ospf_area *area,
                                  struct prefix_ipv4 *p)
{
  struct ospf_area_range *range;
      ...
      for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
        {
          ...
          if (nbr->state == NSM_Full || OSPF_GR_IS_ACTIVE_HELPER (nbr))
            full_nbrs++;
        }
    }
      ...
      if (IS_ROUTER_LSA_NT (rlsa))
        {
          if (IS_DEBUG_OSPF_NSSA)
            zlog_debug ("%s: router %pI4 asserts Nt", __func__,
                        &lsa->data->id);
          return 0;
        }
  ...
  for (ALL_LIST_ELEMENTS (ospf->areas, lnode, nnode, area))
    {
      ...
      if (IS_DEBUG_OSPF (nssa, NSSA))
        zlog_debug ("%s: checking area %pI4", __func__, &area->area_id);
      ...
      else
        {
          switch (area->NSSATranslatorRole)
            ...
            case OSPF_NSSA_ROLE_NEVER:
              ...
              if (IS_DEBUG_OSPF (nssa, NSSA))
                zlog_debug ("%s: never translate", __func__);
              area->NSSATranslatorState = OSPF_NSSA_TRANSLATE_DISABLED;
              break;
            ...
            case OSPF_NSSA_ROLE_ALWAYS:
               ...
               */
              if (IS_DEBUG_OSPF (nssa, NSSA))
                zlog_debug ("%s: translate always", __func__);
              ...
              if (ospf_abr_nssa_am_elected (area) > 0)
                {
                  area->NSSATranslatorState = OSPF_NSSA_TRANSLATE_ENABLED;
                  if (IS_DEBUG_OSPF (nssa, NSSA))
                    zlog_debug ("%s: elected translator", __func__);
                }
              else
                {
                  area->NSSATranslatorState = OSPF_NSSA_TRANSLATE_DISABLED;
                  if (IS_DEBUG_OSPF (nssa, NSSA))
                    zlog_debug ("%s: not elected", __func__);
                }
              break;
            }
        }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      ...
      zlog_debug ("%s: bb_act_attached: %d", __func__, bb_act_attached);
      zlog_debug ("%s: areas_configured: %d", __func__, areas_configured);
      zlog_debug ("%s: areas_act_attached: %d", __func__, areas_act_attached);
    }
  ...
  if (new_flags != ospf->flags)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: new router flags: %x", __func__, new_flags);
...
static void
ospf_abr_update_aggregate (struct ospf_area_range *range, uint32_t cost,
                           struct ospf_area *area)
{
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: Start", __func__);
  ...
  if (CHECK_FLAG (area->stub_router_state, OSPF_AREA_IS_STUB_ROUTED) &&
      (range->cost != OSPF_STUB_MAX_METRIC_SUMMARY_COST))
    {
      range->cost = OSPF_STUB_MAX_METRIC_SUMMARY_COST;
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: use summary max-metric 0x%08x", __func__,
                    range->cost);
    }
...
static void
set_metric (struct ospf_lsa *lsa, uint32_t metric)
{
...
ospf_abr_translate_nssa (struct ospf_area *area, struct ospf_lsa *lsa)
{
  ...
  if (! CHECK_FLAG (lsa->data->options, OSPF_OPTION_NP))
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: LSA Id %pI4, P-bit off, NO Translation", __func__,
                    &lsa->data->id);
      return 1;
    }
  ...
  if (ext7->e[0].fwd_addr.s_addr == OSPF_DEFAULT_DESTINATION)
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: LSA Id %pI4, Forward address is 0, NO Translation",
      ...
      if (old)
        {
          ...
          if (IS_DEBUG_OSPF_NSSA)
            zlog_debug ("%s: remove old translated LSA id %pI4", __func__,
                        &old->data->id);
        }
  ...
  if (range)
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("Suppressed by range %pI4/%u of area %pI4", &range->addr,
                    range->masklen, &area->area_id);

      ospf_abr_update_aggregate (range, GET_METRIC (ext7->e[0].metric), area);
      return 1;
    }
  ...
  if (old && CHECK_FLAG (old->flags, OSPF_LSA_APPROVED))
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: found old translated LSA Id %pI4, refreshing",
      ...
      if (! new)
        {
          if (IS_DEBUG_OSPF_NSSA)
            zlog_debug ("%s: could not refresh translated LSA Id %pI4",
                        __func__, &old->data->id);
        }
    }
  else
    {
      ...
      if (ospf_translated_nssa_originate (area->ospf, lsa, old) == NULL)
        {
          if (IS_DEBUG_OSPF_NSSA)
            zlog_debug ("%s: Could not translate Type-7 for %pI4 to Type-5",
                        __func__, &lsa->data->id);
          return 1;
        }
    }
...
static void
ospf_abr_translate_nssa_range (struct ospf *ospf, struct prefix_ipv4 *p,
                               uint32_t cost)
{
  ...
  if (lsa)
    lsa = ospf_external_lsa_refresh (ospf, lsa, &ei, LSA_REFRESH_FORCE, true);
  else
...
void
ospf_abr_announce_network_to_area (struct prefix_ipv4 *p, uint32_t cost,
                                   struct ospf_area *area)
{
  ...
  if (old)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: old metric: %d, new metric: %d", __func__,
          ...
          if (! lsa)
            {
              flog_warn (EC_OSPF_LSA_MISSING,
                         "%s: Could not refresh %pFX to %pI4", __func__,
  ...
  else
    {
      ...
      if (! lsa)
        {
          flog_warn (EC_OSPF_LSA_MISSING,
                     "%s: Could not originate %pFX to %pi4", __func__,
                     (struct prefix *) p, &area->area_id);
          return;
        }
      ...
      SET_FLAG (lsa->flags, OSPF_LSA_APPROVED);
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: flooding new version of summary", __func__);
    }
...
static void
ospf_abr_announce_network (struct ospf *ospf, struct prefix_ipv4 *p,
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      ...
      if (! ospf_abr_plist_in_check (area, or, p))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: prefix %pFX was denied by prefix-list", __func__,
                        p);
          continue;
        }
      ...
      if (area->external_routing != OSPF_AREA_DEFAULT && area->no_summary)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: area %pI4 is stub and no_summary", __func__,
                        &area->area_id);
          continue;
        }
      ...
      if (or->path_type == OSPF_PATH_INTER_AREA)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this is inter-area route to %pFX", __func__, p);
      ...
      if (or->path_type == OSPF_PATH_INTRA_AREA)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this is intra-area route to %pFX", __func__, p);
          if ((range = ospf_area_range_match (or_area, or_area->ranges, p)) &&
              ! ospf_area_is_transit (area))
            ospf_abr_update_aggregate (range, or->cost, area);
          else
            ospf_abr_announce_network_to_area (p, or->cost, area);
        }
    }
...
static int
ospf_abr_should_announce (struct ospf *ospf, struct prefix_ipv4 *p,
                          struct ospf_route * or)
{
  struct ospf_area *area;
...
static void
ospf_abr_process_network_rt (struct ospf *ospf, struct route_table *rt)
{
  struct ospf_area *area;
  struct ospf_route * or ;
  struct route_node *rn;
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
    {
      ...
      if (! (area = ospf_area_lookup_by_area_id (ospf, or->u.std.area_id)))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: area %pI4 no longer exists", __func__,
                        & or->u.std.area_id);
          continue;
        }

      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: this is a route to %pFX", __func__, &rn->p);
      if (or->path_type >= OSPF_PATH_TYPE1_EXTERNAL)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this is an External router, skipping", __func__);
          continue;
        }
      ...
      if (or->type == OSPF_DESTINATION_DISCARD)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this is a discard entry, skipping", __func__);
          continue;
        }

      if (or->path_type == OSPF_PATH_INTRA_AREA &&
                ! ospf_abr_should_announce (ospf,
                                            (struct prefix_ipv4 *) &rn->p, or))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: denied by export-list", __func__);
          continue;
        }

      if (or->path_type == OSPF_PATH_INTRA_AREA &&
                ! ospf_abr_plist_out_check (area, or,
                                            (struct prefix_ipv4 *) &rn->p))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: denied by prefix-list", __func__);
          continue;
        }
      ...
      if ((or->path_type == OSPF_PATH_INTER_AREA) &&
        ...
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this route is not backbone one, skipping",
                        __func__);
          continue;
        }
...
static void
ospf_abr_announce_rtr_to_area (struct prefix_ipv4 *p, uint32_t cost,
                               struct ospf_area *area)
{
  struct ospf_lsa *lsa, *old = NULL;
  ...
  if (old && (GET_METRIC (slsa->metric) == cost) &&
  ...
  else
    {
      ...
      if (! lsa)
        {
          flog_warn (EC_OSPF_LSA_MISSING,
                     "%s: Could not refresh/originate %pFX to %pI4", __func__,
                     (struct prefix *) p, &area->area_id);
          return;
        }
...
static void
ospf_abr_announce_rtr (struct ospf *ospf, struct prefix_ipv4 *p,
                       struct ospf_route * or)
{
  struct listnode *node;
  struct ospf_area *area;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      ...
      if (or->u.std.external_routing == OSPF_AREA_NSSA)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: do not generate LSA Type-4 %pI4 from NSSA",
                        __func__, &p->prefix);
          continue;
        }
      ...
      if (area->external_routing != OSPF_AREA_DEFAULT)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: area %pI4 doesn't support external routing",
      ...
      if (or->path_type == OSPF_PATH_INTER_AREA)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this is inter-area route to %pI4", __func__,
                        &p->prefix);
          if (! OSPF_IS_AREA_BACKBONE (area))
      ...
      if (or->path_type == OSPF_PATH_INTRA_AREA)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this is intra-area route to %pI4", __func__,
                        &p->prefix);
          ospf_abr_announce_rtr_to_area (p, or->cost, area);
        }
    }
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: this is a route to %pI4", __func__, &rn->p.u.prefix4);
      ...
      for (ALL_LIST_ELEMENTS (l, node, nnode, or))
        {
          ...
          if (! CHECK_FLAG (or->u.std.flags, ROUTER_LSA_EXTERNAL))
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("%s: This is not an ASBR, skipping", __func__);
              continue;
            }
          ...
          if (or->path_type == OSPF_PATH_INTER_AREA &&
            ...
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("%s: This route is not a backbone one, skipping",
                            __func__);
              continue;
            }
          ...
          if (or->cost >= OSPF_LS_INFINITY)
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("%s: This route has LS_INFINITY metric, skipping",
                            __func__);
              continue;
            }
    ...
    if (CHECK_FLAG (lsa->flags, OSPF_LSA_LOCAL_XLT))
      {
        ...
        if (IS_DEBUG_OSPF_NSSA)
          zlog_debug ("%s: approved unset on link id %pI4", __func__,
                      &lsa->data->id);
      }
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      ...
      LSDB_LOOP (SUMMARY_LSDB (area), rn, lsa)
        if (ospf_lsa_is_self_originated (ospf, lsa))
          {
            if (IS_DEBUG_OSPF_EVENT)
              zlog_debug ("%s: approved unset on summary link id %pI4",
                          __func__, &lsa->data->id);
            UNSET_FLAG (lsa->flags, OSPF_LSA_APPROVED);
          }
...
static void
ospf_abr_prepare_aggregates (struct ospf *ospf, bool nssa)
{
  struct listnode *node;
  struct route_node *rn;
  struct ospf_area_range *range;
        ...
        if ((range = rn->info))
          {
            if (! CHECK_FLAG (range->flags, OSPF_AREA_RANGE_ADVERTISE))
              {
                if (IS_DEBUG_OSPF_EVENT)
                  zlog_debug ("%s: discarding suppress-ranges", __func__);
                continue;
              }
            ...
            if (IS_DEBUG_OSPF_EVENT)
              zlog_debug ("%s: this is range: %pFX", __func__, &p);
            ...
            if (ospf_area_range_active (range))
              {
                if (IS_DEBUG_OSPF_EVENT)
                  zlog_debug ("%s: active range", __func__);
                ...
                for (ALL_LIST_ELEMENTS_RO (ospf->areas, n, ar))
                  {
                    ...
                    ospf_abr_announce_network_to_area (
                        (struct prefix_ipv4 *) &p, range->cost, ar);
                  }
              }
          }
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      ...
      for (rn = route_top (area->nssa_ranges); rn; rn = route_next (rn))
        {
          struct ospf_area_range *range;
          ...
          if (ospf_area_range_active (range) &&
              CHECK_FLAG (range->flags, OSPF_AREA_RANGE_ADVERTISE))
            {
               ...
               * Flood
               */
              ospf_abr_translate_nssa_range (ospf, &p, range->cost);
            }
        } /* all area ranges*/
    }     /* all areas */
 ...
 */
void
ospf_generate_indication_lsa (struct ospf *ospf, struct ospf_area *area)
{
  ...
  if (area_fr_not_supp && ! area->fr_info.area_ind_lsa_recvd &&
      ! area->fr_info.indication_lsa_self && ! area->fr_info.area_dc_clear)
    {
      ...
      new = ospf_summary_asbr_lsa_originate (&p, OSPF_LS_INFINITY, area);
      if (! new)
        {
          zlog_debug ("%s: Indication lsa originate failed", __func__);
          return;
        }
 ...
 */
void
ospf_recv_indication_lsa_flush (struct ospf_lsa *lsa)
{
  if (! IS_LSA_SELF (lsa) && IS_LSA_MAXAGE (lsa) &&
      ospf_check_indication_lsa (lsa))
    {
      ...
      OSPF_LOG_INFO ("%s: Received an ind lsa: %pI4 area %pI4", __func__,
                     &lsa->data->id, &lsa->area->area_id);
 ...
 */
void
ospf_abr_generate_indication_lsa (struct ospf *ospf,
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, o_area))
    {
      ...
      if (o_area->fr_info.indication_lsa_self ||
          o_area->fr_info.area_ind_lsa_recvd || o_area->fr_info.area_dc_clear)
        {
      ...
      else
        {
          ...
          new = ospf_summary_asbr_lsa_originate (&p, OSPF_LS_INFINITY, o_area);
          if (! new)
            {
              zlog_debug ("%s: Indication lsa originate Failed", __func__);
              return;
 ...
 */
void
ospf_flush_indication_lsas (struct ospf *ospf)
{
  struct ospf_area *area;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      if (area->fr_info.indication_lsa_self)
        {
          OSPF_LOG_INFO ("Flushing ind lsa: %pI4 area %pI4",
                         &area->fr_info.indication_lsa_self->data->id,
                         &area->area_id);
          ospf_schedule_lsa_flush_area (area,
 ...
 */
bool
ospf_check_fr_enabled_all (struct ospf *ospf)
{
  const struct ospf_area *area;
  struct listnode *node;
 ...
 */
static void
ospf_abr_announce_non_dna_routers (struct event *thread)
{
  struct ospf_area *area;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      OSPF_LOG_DEBUG (IS_DEBUG_OSPF_EVENT, "%s: Area %pI4 FR enabled: %d",
                      __func__, &area->area_id, area->fr_info.enabled);
      OSPF_LOG_DEBUG (IS_DEBUG_OSPF_EVENT,
      ...
      if (! OSPF_IS_AREA_BACKBONE (area) && area->fr_info.area_dc_clear)
        {
      ...
      if (OSPF_IS_AREA_BACKBONE (area) &&
          (area->fr_info.area_dc_clear || area->fr_info.area_ind_lsa_recvd))
        {
          /* rfc4136 rfc1793: Suppose if the abr is connected to
       ...
       */
      if (area->fr_info.area_dc_clear && area->fr_info.indication_lsa_self)
        {
          ospf_schedule_lsa_flush_area (area,
                                        area->fr_info.indication_lsa_self);
          area->fr_info.indication_lsa_self = NULL;
...
static void
ospf_abr_nssa_type7_default_create (struct ospf *ospf, struct ospf_area *area,
                                    struct ospf_lsa *lsa)
{
  struct external_info ei;

  if (IS_DEBUG_OSPF_NSSA)
    zlog_debug ("Announcing Type-7 default route into NSSA area %pI4",
                &area->area_id);
  ...
  if (area->nssa_default_originate.metric_value != -1)
    ei.route_map_set.metric = area->nssa_default_originate.metric_value;
  else
    ei.route_map_set.metric = DEFAULT_DEFAULT_ALWAYS_METRIC;
  if (area->nssa_default_originate.metric_type != -1)
    ei.route_map_set.metric_type = area->nssa_default_originate.metric_type;
  else
...
static void
ospf_abr_nssa_type7_default_delete (struct ospf *ospf, struct ospf_area *area,
                                    struct ospf_lsa *lsa)
{
  if (lsa && ! CHECK_FLAG (lsa->flags, OSPF_LSA_IN_MAXAGE))
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("Withdrawing Type-7 default route from area %pI4",
                    &area->area_id);
...
void
ospf_abr_nssa_type7_defaults (struct ospf *ospf)
{
  struct ospf_area *area;
  struct listnode *node;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      ...
      if (area->external_routing == OSPF_AREA_NSSA &&
          ...
          (IS_OSPF_ABR (ospf) ||
           (IS_OSPF_ASBR (ospf) && ospf->nssa_default_import_check.status)))
                                             ...
                                             struct ospf_lsa *lsa)
{
  if (CHECK_FLAG (lsa->flags, OSPF_LSA_LOCAL_XLT) &&
    ...
    {
      zlog_info ("%s: removing unapproved translates, ID: %pI4", __func__,
                 &lsa->data->id);
...
static void
ospf_abr_manage_discard_routes (struct ospf *ospf, bool nssa)
{
  struct listnode *node, *nnode;
  struct route_node *rn;
  struct ospf_area *area;
      ...
      for (rn = route_top (ranges); rn; rn = route_next (rn))
        {
          ...
          if (ospf_area_range_active (range) &&
              CHECK_FLAG (range->flags, OSPF_AREA_RANGE_ADVERTISE))
            ospf_add_discard_route (ospf, ospf->new_table, area,
                                    (struct prefix_ipv4 *) &rn->p, nssa);
          else
            ospf_delete_discard_route (ospf, ospf->new_table,
                                       (struct prefix_ipv4 *) &rn->p, nssa);
        }
    }
}
  ...
  if (IS_OSPF_ABR (ospf))
    {
      ...
      if (ospf->fr_configured)
        {
          OSPF_LOG_DEBUG (IS_DEBUG_OSPF_EVENT, "%s(): announce non-DNArouters",
                          __func__);
          /*
           ...
           * all the routers.
           */
          event_add_timer (master, ospf_abr_announce_non_dna_routers, ospf,
...
static void
ospf_abr_task_timer (struct event *thread)
{
  struct ospf *ospf = EVENT_ARG (thread);
