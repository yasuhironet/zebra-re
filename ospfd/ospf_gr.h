...
#define IS_GRACE_LSA(lsa)                                                     \
  ((lsa->data->type == OSPF_OPAQUE_LINK_LSA) &&                               \
   (GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)) == OPAQUE_TYPE_GRACE_LSA))

/* Check neighbour is in FULL state */
#define IS_NBR_STATE_FULL(nbr)                                                \
  (nsm_should_adj (nbr) && (nbr->state == NSM_Full))

/* Check neighbour is DR_OTHER and state is 2_WAY */
#define IS_NBR_STATE_2_WAY_WITH_DROTHER(nbr)                                  \
  ((ospf_get_nbr_ism_role (nbr) == ISM_DROther) && (nbr->state == NSM_TwoWay))
