...
static void
ospf_route_map_event (const char *name)
{
  struct ospf *ospf;
  int type;
  struct listnode *n1 = NULL;
...
static void *
route_set_min_metric_compile (const char *arg)
{
...
static void
route_set_min_metric_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
static void *
route_set_max_metric_compile (const char *arg)
{
...
static void
route_set_max_metric_free (void *rule)
{
  XFREE (MTYPE_ROUTE_MAP_COMPILED, rule);
}
