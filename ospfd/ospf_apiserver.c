...
ospf_apiserver_init (void)
{
  ...
  rc = ospf_register_opaque_functab (
      0 /* all LSAs */, 0 /* all opaque types */, ospf_apiserver_new_if,
      ospf_apiserver_del_if, ospf_apiserver_ism_change,
      ospf_apiserver_nsm_change, NULL, NULL, NULL,
...
lookup_apiserver (uint8_t lsa_type, uint8_t opaque_type)
{
...
ospf_apiserver_new (int fd_sync, int fd_async)
{
  ...
  new->filter =
      XMALLOC (MTYPE_APISERVER_MSGFILTER, sizeof (struct lsa_filter_type));
...
void
ospf_apiserver_event (enum ospf_apiserver_event event, int fd,
                      struct ospf_apiserver *apiserv)
{
  switch (event)
    {
    case OSPF_APISERVER_ACCEPT:
      (void) event_add_read (master, ospf_apiserver_accept, apiserv, fd, NULL);
      break;
...
ospf_apiserver_free (struct ospf_apiserver *apiserv)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("API: Delete apiserv(%p), total#(%d)", (void *) apiserv,
...
void
ospf_apiserver_read (struct event *thread)
{
  struct ospf_apiserver *apiserv;
  struct msg *msg;
  int fd;
  ...
  if (msg == NULL)
    {
      zlog_warn ("%s: read failed on fd=%d, closing connection", __func__, fd);
...
void
ospf_apiserver_sync_write (struct event *thread)
{
  struct ospf_apiserver *apiserv;
  struct msg *msg;
  int fd;
...
void
ospf_apiserver_async_write (struct event *thread)
{
  struct ospf_apiserver *apiserv;
  struct msg *msg;
  int fd;
...
void
ospf_apiserver_accept (struct event *thread)
{
  int accept_sock;
  int new_sync_sock;
  int new_async_sock;
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("API: %s: New peer: %pI4/%u", __func__, &peer_sync.sin_addr,
  ...
  if (ntohs (peer_async.sin_port) == ospf_apiserver_getport ())
    {
      zlog_warn ("API: %s: Peer(%pI4/%u): Invalid async port number?",
                 __func__, &peer_async.sin_addr, ntohs (peer_async.sin_port));
      close (new_sync_sock);
      return;
    }
...
ospf_apiserver_send_msg (struct ospf_apiserver *apiserv, struct msg *msg)
{
  ...
  switch (msg->hdr.msgtype)
    ...
    case MSG_ROUTER_ID_CHANGE:
      ...
      break;
    default:
      zlog_warn ("%s: Unknown message type %d", __func__, msg->hdr.msgtype);
      return -1;
    }
...
int
ospf_apiserver_send_reply (struct ospf_apiserver *apiserv, uint32_t seqnr,
                           uint8_t rc)
{
                                     ...
                                     uint8_t lsa_type, uint8_t opaque_type)
{
  ...
  rc = ospf_register_opaque_functab (
      ...
      ospf_apiserver_lsa_refresher, NULL, /* ospf_apiserver_lsa_update */
  ...
  regtype = XCALLOC (MTYPE_APISERVER, sizeof (struct registered_opaque_type));
  ...
  for (ALL_LIST_ELEMENTS (apiserv->opaque_types, node, nnode, regtype))
    {
      ...
      if (regtype->lsa_type == lsa_type && regtype->opaque_type == opaque_type)
        {
  ...
  for (ALL_LIST_ELEMENTS (apiserv->opaque_types, node, nnode, r))
    {
      ...
      if (r->lsa_type == OSPF_OPAQUE_AS_LSA)
        {
          /* Yes, this opaque type is ready */
          msg = new_msg_ready_notify (0, OSPF_OPAQUE_AS_LSA, r->opaque_type,
                                      noarea_id);
...
apiserver_sync_callback (struct ospf_lsa *lsa, void *p_arg, int int_arg)
{
  ...
  if ((param->filter->origin == ANY_ORIGIN) ||
      (param->filter->origin == (lsa->flags & OSPF_LSA_SELF)))
    {
      ...
      msg = new_msg_lsa_change_notify (MSG_LSA_UPDATE_NOTIFY, seqnum, ifaddr,
...
int
ospf_apiserver_handle_sync_reachable (struct ospf_apiserver *apiserv,
                                      struct msg *msg)
{
...
int
ospf_apiserver_handle_sync_ism (struct ospf_apiserver *apiserv,
                                struct msg *msg)
{
  struct ospf *ospf = ospf_lookup_by_vrf_id (VRF_DEFAULT);
...
int
ospf_apiserver_handle_sync_nsm (struct ospf_apiserver *apiserv,
                                struct msg *msg)
{
  struct ospf *ospf = ospf_lookup_by_vrf_id (VRF_DEFAULT);
  struct listnode *anode, *inode;
          ...
          for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
            {
              ...
              if (! nbr)
                continue;
              m = new_msg_nsm_change (seqnum, oi->address->u.prefix4, nbr->src,
...
int
ospf_apiserver_handle_sync_router_id (struct ospf_apiserver *apiserv,
                                      struct msg *msg)
{
  struct ospf *ospf = ospf_lookup_by_vrf_id (VRF_DEFAULT);
  uint32_t seqnum = msg_get_seq (msg);
                               ...
                               struct lsa_header *protolsa)
{
  ...
  lsa_header_set (s, options, protolsa->type, protolsa->id, ospf->router_id);
                                         ...
                                         struct msg *msg)
{
  ...
  switch (data->type)
    ...
    case OSPF_OPAQUE_LINK_LSA:
      ...
      if (! oi)
        {
          zlog_warn ("%s: unknown interface %pI4", __func__, &omsg->ifaddr);
    ...
    case OSPF_OPAQUE_AREA_LSA:
      ...
      if (! area)
        {
          zlog_warn ("%s: unknown area %pI4", __func__, &omsg->area_id);
    ...
    default:
      ...
      zlog_warn ("%s: Cannot originate non-opaque LSA type %d", __func__,
                 data->type);
  ...
  if (! apiserver_is_opaque_type_registered (apiserv, lsa_type, opaque_type))
    {
      zlog_warn ("%s: LSA-type(%d)/Opaque-type(%d): Not registered", __func__,
...
int
ospf_apiserver_originate1 (struct ospf_lsa *lsa, struct ospf_lsa *old)
{
  struct ospf *ospf;
  ...
  if (old)
    {
      ...
      if (IS_DEBUG_OSPF_CLIENT_API)
        zlog_debug (
            ...
            old->data->type, &old->data->id, ntohl (old->data->ls_seqnum),
            ntohl (old->data->ls_age));
      if (IS_LSA_MAX_SEQ (old))
        {
          flog_warn (EC_OSPF_LSA_INSTALL_FAILURE, "%s: old LSA at maxseq",
                     __func__);
          return -1;
        }
  ...
  if (ospf_lsa_install (ospf, lsa->oi, lsa) == NULL)
    {
      flog_warn (EC_OSPF_LSA_INSTALL_FAILURE, "%s: ospf_lsa_install failed",
                 __func__);
      return -1;
    }
...
ospf_apiserver_lsa_refresher (struct ospf_lsa *lsa)
{
  ...
  if (! apiserv)
    {
      zlog_warn ("%s: LSA[%s]: No apiserver?", __func__, dump_lsa_key (lsa));
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: Refresh Opaque LSA", new->data->type,
                  &new->data->id);
                                      ...
                                      struct msg *msg)
{
  ...
  switch (dmsg->lsa_type)
    ...
    case OSPF_OPAQUE_LINK_LSA:
      ...
      if (! oi)
        {
          zlog_warn ("%s: unknown interface %pI4", __func__, &dmsg->addr);
          rc = OSPF_API_NOSUCHINTERFACE;
          goto out;
        }
      area = oi->area;
  ...
  id.s_addr =
      htonl (SET_OPAQUE_LSID (dmsg->opaque_type, ntohl (dmsg->opaque_id)));
  ...
  if (! old)
    {
      zlog_warn ("%s: LSA[Type%d:%pI4] not in LSDB", __func__, dmsg->lsa_type,
                 &id);
...
static int
apiserver_flush_opaque_type_callback (struct ospf_lsa *lsa, void *p_arg,
                                      int int_arg)
{
  struct param_t
  {
  ...
  if (! oi)
    {
       ...
       */
      zlog_warn ("%s: interface %s not known to OSPFd?", __func__, ifp->name);
      return 0;
    }
...
void
ospf_apiserver_show_info (struct vty *vty, struct json_object *json,
                          struct ospf_lsa *lsa)
{
  struct opaque_lsa
  {
  ...
  if (json)
    json_object_string_addf (json, "opaqueData", "%*pHXn", (int) opaquelen,
                             olsa->data);
  else if (vty != NULL)
    {
      int i;
  ...
  else
    {
      int i;
      zlog_debug (
          "    Added using OSPF API: %u octets of opaque data %s", opaquelen,
          VALID_OPAQUE_INFO_LEN (lsa->data) ? "" : "(Invalid length?)");
          ...
          if (r->lsa_type == OSPF_OPAQUE_LINK_LSA)
            {
              ...
              if (! msg)
                {
                  zlog_warn ("%s: new_msg_ready_notify failed", __func__);
#ifdef NOTYET
                  /* Cannot allocate new message. What
                   * should we do? */
                  ospf_apiserver_free (apiserv);
          ...
          if (r->lsa_type == OSPF_OPAQUE_AREA_LSA)
            {
              ...
              if (! msg)
                {
                  zlog_warn ("%s: new_msg_ready_nofity failed", __func__);
#ifdef NOTYET
                  /* Cannot allocate new message. What
                   * should we do? */
                  ospf_apiserver_free (apiserv);
...
static int
apiserver_clients_lsa_change_notify (uint8_t msgtype, struct ospf_lsa *lsa)
{
  struct msg *msg;
  struct listnode *node, *nnode;
  struct ospf_apiserver *apiserv;
  ...
  msg = new_msg_lsa_change_notify (msgtype, 0L, /* no sequence number */
                                   ifaddr, area_id, lsa->flags & OSPF_LSA_SELF,
                                   lsa->data);
  if (! msg)
    {
      zlog_warn ("%s: msg_new failed", __func__);
...
static inline int
cmp_route_nodes (struct route_node *orn, struct route_node *nrn)
{
  if (! orn)
    return 1;
  else if (! nrn)
...
void
ospf_apiserver_notify_reachable (struct route_table *ort,
      ...
      if (! cmp)
        {
          ...
          if (IS_DEBUG_OSPF_CLIENT_API)
            zlog_debug ("keeping router id: %pI4", &orn->p.u.prefix4);
          orn = route_next (orn);
          nrn = route_next (nrn);
        }
      else if (cmp < 0)
        {
          ...
          *d++ = orn->p.u.prefix4;
          if (IS_DEBUG_OSPF_CLIENT_API)
            zlog_debug ("removing router id: %pI4", &orn->p.u.prefix4);
          orn = route_next (orn);
        }
      else
        {
          ...
          *a++ = nrn->p.u.prefix4;
          if (IS_DEBUG_OSPF_CLIENT_API)
            zlog_debug ("adding router id: %pI4", &nrn->p.u.prefix4);
          nrn = route_next (nrn);
        }
    }
