...
static void show_ospf_grace_lsa_info (struct vty *vty,
                                      struct json_object *json,
                                      struct ospf_lsa *lsa);
static bool ospf_check_change_in_rxmt_list (struct ospf_neighbor *nbr);

static unsigned int
ospf_enable_rtr_hash_key (const void *data)
{
...
static bool
ospf_enable_rtr_hash_cmp (const void *d1, const void *d2)
{
...
static void *
ospf_enable_rtr_hash_alloc (void *p)
{
  struct advRtr *rid;
...
static void
ospf_disable_rtr_hash_free (void *rtr)
{
  XFREE (MTYPE_OSPF_GR_HELPER, rtr);
}

static void
ospf_enable_rtr_hash_destroy (struct ospf *ospf)
{
 ...
 */
const char *
ospf_exit_reason2str (unsigned int reason)
{
 ...
 */
const char *
ospf_restart_reason2str (unsigned int reason)
{
 ...
 */
const char *
ospf_rejected_reason2str (unsigned int reason)
{
 ...
 */
void
ospf_gr_helper_instance_init (struct ospf *ospf)
{
  if (IS_DEBUG_OSPF_GR)
 ...
 */
void
ospf_gr_helper_instance_stop (struct ospf *ospf)
{
  if (IS_DEBUG_OSPF_GR)
...
ospf_gr_helper_init (void)
{
  ...
  rc = ospf_register_opaque_functab (
      OSPF_OPAQUE_LINK_LSA, OPAQUE_TYPE_GRACE_LSA, NULL, NULL, NULL, NULL,
      NULL, NULL, NULL, show_ospf_grace_lsa_info, NULL, NULL, NULL, NULL);
  if (rc != 0)
    {
      flog_warn (EC_OSPF_OPAQUE_REGISTRATION,
                 "%s: Failed to register Grace LSA functions", __func__);
    }
}
 ...
 */
static int
ospf_extract_grace_lsa_fields (struct ospf_lsa *lsa, uint32_t *interval,
                               struct in_addr *addr, uint8_t *reason)
{
  ...
  if (lsa->size <= OSPF_LSA_HEADER_SIZE)
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug ("%s: Malformed packet: Invalid LSA len:%d", __func__,
                    length);
  ...
  for (tlvh = TLV_HDR_TOP (lsah); sum < length && tlvh;
       tlvh = TLV_HDR_NEXT (tlvh))
    {
      ...
      if (sum + TLV_SIZE (tlvh) > length)
        {
          if (IS_DEBUG_OSPF_GR)
            zlog_debug ("%s: Malformed packet: Invalid TLV len:%u", __func__,
                        TLV_SIZE (tlvh));
          return OSPF_GR_FAILURE;
        }
      ...
      switch (ntohs (tlvh->type))
        ...
        case GRACE_PERIOD_TYPE:
          if (TLV_SIZE (tlvh) < sizeof (struct grace_tlv_graceperiod))
            {
        ...
        case RESTARTER_IP_ADDR_TYPE:
          if (TLV_SIZE (tlvh) < sizeof (struct grace_tlv_restart_addr))
            {
        ...
        default:
          if (IS_DEBUG_OSPF_GR)
            zlog_debug ("%s, Malformed packet.Invalid TLV type:%d", __func__,
                        ntohs (tlvh->type));
          return OSPF_GR_FAILURE;
        }
    }
 ...
 */
static void
ospf_handle_grace_timer_expiry (struct event *thread)
{
 ...
 */
int
ospf_process_grace_lsa (struct ospf *ospf, struct ospf_lsa *lsa,
                        struct ospf_neighbor *nbr)
{
  ...
  if (oi->type != OSPF_IFTYPE_POINTOPOINT)
    {
      ...
      if (! restarter)
        {
          if (IS_DEBUG_OSPF_GR)
            zlog_debug ("%s, Restarter is not a nbr(%pI4) for this router.",
  ...
  if (! IS_NBR_STATE_FULL (restarter))
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug ("%s, This Neighbour %pI4 is not in FULL state.", __func__,
                    &restarter->src);
      restarter->gr_helper_info.rejected_reason =
  ...
  if (ntohs (lsa->data->ls_age) >= grace_interval)
    {
      ...
      restarter->gr_helper_info.rejected_reason = OSPF_HELPER_LSA_AGE_MORE;
      return OSPF_GR_NOT_HELPER;
    }
  ...
  if (ospf->gr_info.restart_in_progress)
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug ("%s: router is in the process of graceful restart",
                    __func__);
      restarter->gr_helper_info.rejected_reason = OSPF_HELPER_RESTARTING;
      return OSPF_GR_NOT_HELPER;
    }
  ...
  if (grace_interval > ospf->supported_grace_time)
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug (
            ...
            __func__, grace_interval, ospf->supported_grace_time);
  ...
  if (OSPF_GR_IS_ACTIVE_HELPER (restarter))
    ...
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug ("%s, This Router becomes a HELPER for the neighbour %pI4",
 ...
 */
static bool
ospf_check_change_in_rxmt_list (struct ospf_neighbor *nbr)
{
 ...
 */
void
ospf_helper_handle_topo_chg (struct ospf *ospf, struct ospf_lsa *lsa)
{
      ...
      for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
        {
          ...
          if (OSPF_GR_IS_ACTIVE_HELPER (nbr))
            ospf_gr_helper_exit (nbr, OSPF_GR_HELPER_TOPO_CHG);
        }
    }
}
 ...
 */
void
ospf_gr_helper_exit (struct ospf_neighbor *nbr,
                     enum ospf_helper_exit_reason reason)
{
  ...
  if (IS_DEBUG_OSPF_GR)
    zlog_debug ("%s, Exiting from HELPER support to %pI4, due to %s", __func__,
 ...
 */
void
ospf_process_maxage_grace_lsa (struct ospf *ospf, struct ospf_lsa *lsa,
                               struct ospf_neighbor *nbr)
{
  ...
  if (oi->type != OSPF_IFTYPE_POINTOPOINT)
    {
      ...
      if (! restarter)
        {
          if (IS_DEBUG_OSPF_GR)
            zlog_debug ("%s, Restarter is not a neighbour for this router.",
                        __func__);
          return;
        }
    }
 ...
 */
void
ospf_gr_helper_support_set (struct ospf *ospf, bool support)
{
  struct ospf_interface *oi;
  struct listnode *node;
  struct advRtr lookup;
...
void
ospf_gr_helper_support_set_per_routerid (struct ospf *ospf,
                                         struct in_addr *advrtr, bool support)
{
  struct advRtr temp;
  struct advRtr *rtr;
  struct ospf_interface *oi;
 ...
 */
void
ospf_gr_helper_lsa_check_set (struct ospf *ospf, bool enabled)
{
 ...
 */
void
ospf_gr_helper_supported_gracetime_set (struct ospf *ospf, uint32_t interval)
{
 ...
 */
void
ospf_gr_helper_set_supported_planned_only_restart (struct ospf *ospf,
                                                   bool planned_only)
{
 ...
 */
static void
show_ospf_grace_lsa_info (struct vty *vty, struct json_object *json,
                          struct ospf_lsa *lsa)
{
  struct lsa_header *lsah = NULL;
  struct tlv_header *tlvh = NULL;
      ...
      if (sum + TLV_SIZE (tlvh) > length)
        {
          if (vty)
            ...
            zlog_debug ("%% Invalid TLV length: %u", TLV_SIZE (tlvh));
          return;
        }
      ...
      switch (ntohs (tlvh->type))
          ...
          if (TLV_SIZE (tlvh) < sizeof (struct grace_tlv_graceperiod))
            {
              if (vty)
                ...
                zlog_debug ("%% Invalid grace TLV length %u", TLV_SIZE (tlvh));
              return;
            }
          ...
          if (TLV_SIZE (tlvh) < sizeof (struct grace_tlv_restart_reason))
            {
              if (vty)
                ...
                zlog_debug ("%% Invalid reason TLV length %u",
                            TLV_SIZE (tlvh));
              return;
            }
        ...
        case RESTARTER_IP_ADDR_TYPE:
          if (TLV_SIZE (tlvh) < sizeof (struct grace_tlv_restart_addr))
            {
              if (vty)
                ...
                zlog_debug ("%% Invalid addr TLV length %u", TLV_SIZE (tlvh));
              return;
            }
          ...
          if (vty)
            ...
            zlog_debug ("   Restarter address:%pI4", &restartAddr->addr);
          break;
        default:
          if (vty)
            vty_out (vty, "   Unknown TLV type %d\n", ntohs (tlvh->type));
          else
            zlog_debug ("   Unknown TLV type %d", ntohs (tlvh->type));
