...
static void
ospf_free_refresh_queue (struct ospf *ospf)
{
...
int
p_spaces_compare_func (const struct p_space *a, const struct p_space *b)
{
...
int
q_spaces_compare_func (const struct q_space *a, const struct q_space *b)
{
...
void
ospf_process_refresh_data (struct ospf *ospf, bool reset)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("Router-ID[OLD:%pI4]: Update", &ospf->router_id);
  ...
  */
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("Router-ID[OLD:%pI4]: Update to %pI4", &ospf->router_id,
                &router_id);
  ...
  if (rid_change || (reset))
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("Router-ID[NEW:%pI4]: Update", &ospf->router_id);
...
void
ospf_process_reset (struct ospf *ospf)
{
  ospf_process_refresh_data (ospf, true);
}

void
ospf_neighbor_reset (struct ospf *ospf, struct in_addr nbr_id,
                     const char *nbr_str)
{
  struct route_node *rn;
...
struct ospf *
ospf_new_alloc (unsigned short instance, const char *name)
{
  int i;
  struct vrf *vrf = NULL;
...
static struct ospf *
ospf_new (unsigned short instance, const char *name)
{
  struct ospf *new;
  ...
  if ((ospf_sock_init (new)) < 0)
    {
      flog_err (EC_LIB_SOCKET, "%s: ospf_sock_init is unable to open a socket",
                __func__);
      return new;
    }
...
struct ospf *
ospf_lookup_by_inst_name (unsigned short instance, const char *name)
{
...
struct ospf *
ospf_lookup (unsigned short instance, const char *name)
{
  struct ospf *ospf;
...
struct ospf *
ospf_get (unsigned short instance, const char *name, bool *created)
{
  struct ospf *ospf;
...
uint32_t
ospf_count_area_params (struct ospf *ospf)
{
  struct vrf *vrf;
  struct interface *ifp;
  uint32_t count = 0;
...
ospf_lookup_by_name (const char *vrf_name)
{
  ...
  for (ALL_LIST_ELEMENTS (om->ospf, node, nnode, ospf))
    if ((ospf->name == NULL && vrf_name == NULL) ||
        (ospf->name && vrf_name && strcmp (ospf->name, vrf_name) == 0))
      return ospf;
  return NULL;
}
...
static void
ospf_deferred_shutdown_timer (struct event *t)
{
...
void
ospf_area_lsdb_discard_delete (struct ospf_area *area)
{
  struct route_node *rn;
  struct ospf_lsa *lsa;
...
ospf_area_check_free (struct ospf *ospf, struct in_addr area_id)
{
  ...
  if (area && listcount (area->oiflist) == 0 && area->ranges->top == NULL &&
      ...
      area->external_routing == OSPF_AREA_DEFAULT && area->no_summary == 0 &&
      area->default_cost == 1 && EXPORT_NAME (area) == NULL &&
      IMPORT_NAME (area) == NULL && area->auth_type == OSPF_AUTH_NULL)
    {
      listnode_delete (ospf->areas, area);
      ospf_area_free (area);
    }
 ...
 */
void
ospf_interface_area_set (struct ospf *ospf, struct interface *ifp)
{
  if (! ospf)
    return;
...
void
ospf_interface_area_unset (struct ospf *ospf, struct interface *ifp)
{
...
ospf_if_update (struct ospf *ospf, struct interface *ifp)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug (
        ...
        ospf_vrf_id_to_name (ospf->vrf_id), ospf->vrf_id, &ospf->router_id);
...
ospf_area_type_set (struct ospf_area *area, int type)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("Area[%pI4]: Configured as %s", &area->area_id,
...
int
ospf_area_display_format_set (struct ospf *ospf, struct ospf_area *area,
                              int df)
{
  area->area_id_fmt = df;
...
int
ospf_area_nssa_no_summary_set (struct ospf *ospf, struct in_addr area_id)
{
  struct ospf_area *area;
...
int
ospf_area_nssa_suppress_fa_set (struct ospf *ospf, struct in_addr area_id)
{
  struct ospf_area *area;
...
int
ospf_area_nssa_suppress_fa_unset (struct ospf *ospf, struct in_addr area_id)
{
  struct ospf_area *area;
...
void
ospf_area_nssa_default_originate_set (struct ospf *ospf,
...
void
ospf_area_nssa_default_originate_unset (struct ospf *ospf,
                                        struct in_addr area_id)
{
  struct ospf_area *area;
...
int
ospf_area_export_list_set (struct ospf *ospf, struct ospf_area *area,
...
ospf_nbr_nbma_new (void)
{
  ...
  nbr_nbma =
      XCALLOC (MTYPE_OSPF_NEIGHBOR_STATIC, sizeof (struct ospf_nbr_nbma));
...
static void
ospf_nbr_nbma_add (struct ospf_nbr_nbma *nbr_nbma, struct ospf_interface *oi)
{
  struct ospf_neighbor *nbr;
  struct route_node *rn;
  struct prefix p;
 ...
 */
void
ospf_update_bufsize (struct ospf *ospf, uint32_t recvsize, uint32_t sendsize)
{
...
void
ospf_master_init (struct event_loop *master)
{
...
void
ospf_vrf_link (struct ospf *ospf, struct vrf *vrf)
{
...
void
ospf_vrf_unlink (struct ospf *ospf, struct vrf *vrf)
{
  if (vrf->info == (void *) ospf)
    vrf->info = NULL;
...
static void
ospf_set_redist_vrf_bitmaps (struct ospf *ospf, bool set)
{
  int type;
  struct list *red_list;
  ...
  for (type = 0; type < ZEBRA_ROUTE_MAX; type++)
    {
      ...
      if (set)
        ...
        vrf_bitmap_unset (zclient->redist[AFI_IP][type], ospf->vrf_id);
    }
  ...
  if (red_list)
    {
      if (set)
        vrf_bitmap_set (zclient->default_information[AFI_IP], ospf->vrf_id);
      else
        vrf_bitmap_unset (zclient->default_information[AFI_IP], ospf->vrf_id);
    }
}
  ...
  if (ospf)
    {
      ...
      ospf->oi_running = 0;
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: ospf old_vrf_id %d unlinked", __func__, old_vrf_id);
      EVENT_OFF (ospf->t_read);
      close (ospf->fd);
      ospf->fd = -1;
    }
...
const char *
ospf_get_name (const struct ospf *ospf)
{
  if (ospf->name)
    return ospf->name;
  else
