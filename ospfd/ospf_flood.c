 ...
 */
void
ospf_refresh_dna_type5_and_type7_lsas (struct ospf *ospf)
{
  struct route_node *rn;
  struct ospf_lsa *lsa = NULL;

  LSDB_LOOP (EXTERNAL_LSDB (ospf), rn, lsa)
    if (IS_LSA_SELF (lsa) && CHECK_FLAG (lsa->data->ls_age, DO_NOT_AGE))
 ...
 */
void
ospf_area_update_fr_state (struct ospf_area *area)
{
  ...
  count_router_lsas =
      (unsigned int) (ospf_lsdb_count (area->lsdb, OSPF_ROUTER_LSA) -
                      ospf_lsdb_count_self (area->lsdb, OSPF_ROUTER_LSA));

  if (count_router_lsas > (unsigned int) area->fr_info.router_lsas_recv_dc_bit)
    {
           ...
           (unsigned int) area->fr_info.router_lsas_recv_dc_bit)
    {
      ...
      OSPF_LOG_ERR (
          ...
          __func__, count_router_lsas, area->fr_info.router_lsas_recv_dc_bit);
      return;
    }
  ...
  for (type = 0; type < ZEBRA_ROUTE_MAX; type++)
    {
      ...
      redist_on = is_default_prefix4 (&p)
...
static void
ospf_process_self_originated_lsa (struct ospf *ospf, struct ospf_lsa *new,
                                  struct ospf_area *area)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s:LSA[Type%d:%pI4]: Process self-originated LSA seq 0x%x",
                ospf_get_name (ospf), new->data->type, &new->data->id,
    ...
    case OSPF_OPAQUE_LINK_LSA:
      ...
      for (ALL_LIST_ELEMENTS_RO (ospf->oiflist, node, oi))
        ...
        if (IPV4_ADDR_SAME (&oi->address->u.prefix4, &new->data->id))
          {
            if (oi->area != area || oi->type != OSPF_IFTYPE_BROADCAST ||
  ...
  if (current != NULL) /* -- endo. */
    {
      if (IS_LSA_SELF (current) &&
        ...
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s:LSA[Flooding]: LSA is received recently.",
                        ospf_get_name (ospf));
          return -1;
        }
    }
      ...
      if (IS_LSA_MAXAGE (new))
        {
          ...
          if (IS_DEBUG_OSPF_GR)
            zlog_debug ("%s, Received a maxage GRACE-LSA from router %pI4",
        ...
        {
          if (IS_DEBUG_OSPF_GR)
            zlog_debug ("%s, Received a GRACE-LSA from router %pI4", __func__,
                        &new->data->adv_router);

          if (ospf_process_grace_lsa (ospf, new, nbr) == OSPF_GR_NOT_HELPER)
            {
              if (IS_DEBUG_OSPF_GR)
                zlog_debug (
                              ...
                              struct ospf_neighbor *inbr, struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: considering int %s (%s), INBR(%pI4), LSA[%s] AGE %u",
  ...
  for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
    {
      ...
      onbr = rn->info;
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: considering nbr %pI4 via %s (%s), state: %s",
      ...
      if (onbr->state < NSM_Full)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: adj to onbr %pI4 is not Full (%s)", __func__,
                        &onbr->router_id,
                        lookup_msg (ospf_nsm_state_msg, onbr->state, NULL));
          ...
          if (! CHECK_FLAG (onbr->options, OSPF_OPTION_O))
            {
              if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                zlog_debug (
                    ...
                    __func__, IF_NAME (oi), &onbr->router_id);
              continue;
            }
        }
          ...
          if (IPV4_ADDR_SAME (&inbr->router_id, &onbr->router_id))
            {
              if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                zlog_debug (
                    ...
                    __func__, IF_NAME (oi), &inbr->router_id);
              continue;
            }
        }
      else
  ...
  if (inbr && (inbr->oi == oi))
    {
      ...
      if (NBR_IS_DR (inbr) || NBR_IS_BDR (inbr))
        {
          if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
            zlog_debug ("%s: DR/BDR NOT SEND to int %s (%s)", __func__,
      ...
      if (oi->state == ISM_Backup)
        {
          if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
            zlog_debug ("%s: ISM_Backup NOT SEND to int %s (%s)", __func__,
                        IF_NAME (oi), ospf_get_name (oi->ospf));
          return 1;
        }
    }
        ...
        (inbr != NULL) && (oi == inbr->oi))
      {
        if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
          zlog_debug (
              ...
              dump_lsa_key (lsa),
              inbr ? &(inbr->router_id) : &(oi->ospf->router_id),
              IF_NAME (oi));
      }
    else
...
int
ospf_flood_through (struct ospf *ospf, struct ospf_neighbor *inbr,
                    struct ospf_lsa *lsa)
{
  int lsa_ack_flag = 0;
...
ospf_ls_request_add (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
    zlog_debug ("RqstL(%lu)++, NBR(%pI4(%s)), LSA[%s]",
                ospf_ls_request_count (nbr), &nbr->router_id,
      ...
      if (old)
        {
          ...
          if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
            zlog_debug ("RXmtL(%lu)--, NBR(%pI4(%s)), LSA[%s]",
                        ...
                        ospf_get_name (nbr->oi->ospf), dump_lsa_key (old));
...
ospf_lsa_flush_area (struct ospf_lsa *lsa, struct ospf_area *area)
{
      ...
      ospf->gr_info.restart_in_progress)
    {
      if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
        zlog_debug (
            ...
            ospf_get_name (ospf), lsa->data->type, &lsa->data->id);
      return;
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("%s: MaxAge set to LSA[%s]", __func__, dump_lsa_key (lsa));
