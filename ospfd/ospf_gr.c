...
ospf_gr_lsa_lookup (struct ospf *ospf, struct ospf_area *area)
{
  struct ospf_lsa *lsa;
...
static void
ospf_gr_lsa_body_set (struct ospf_gr_info *gr_info, struct ospf_interface *oi,
...
static struct ospf_lsa *
ospf_gr_lsa_new (struct ospf_interface *oi, enum ospf_gr_restart_reason reason)
{
  struct stream *s;
  ...
  if (IS_DEBUG_OSPF_GR)
    zlog_debug ("LSA[Type%d:%pI4]: Create an Opaque-LSA/GR instance", lsa_type,
                &lsa_id);
...
static void
ospf_gr_lsa_originate (struct ospf_interface *oi,
...
static void
ospf_gr_flush_grace_lsas (struct ospf *ospf)
{
...
static void
ospf_gr_restart_exit (struct ospf *ospf, const char *reason)
{
  struct ospf_area *area;
...
void
ospf_gr_restart_enter (struct ospf *ospf, enum ospf_gr_restart_reason reason,
                       time_t timestamp)
{
...
static bool
ospf_router_lsa_contains_adj (struct ospf_lsa *lsa, struct in_addr *id)
{
...
static bool
ospf_gr_check_router_lsa_consistency (struct ospf *ospf,
                                      struct ospf_area *area,
                                      struct ospf_lsa *lsa)
{
  if (CHECK_FLAG (lsa->flags, OSPF_LSA_SELF))
  ...
  else
    {
      ...
      lsa_self =
          ospf_lsa_lookup_by_id (area, OSPF_ROUTER_LSA, ospf->router_id);
 ...
 */
void
ospf_gr_check_lsdb_consistency (struct ospf *ospf, struct ospf_area *area)
{
...
static bool
ospf_gr_check_adj_id (struct ospf_area *area, struct in_addr *nbr_id)
{
  struct ospf_neighbor *nbr;
...
static bool
ospf_gr_check_adjs_lsa_transit (struct ospf_area *area,
...
static bool
ospf_gr_check_adjs_lsa (struct ospf_area *area, struct ospf_lsa *lsa)
{
  struct router_lsa *rl = (struct router_lsa *) lsa->data;
 ...
 */
void
ospf_gr_check_adjs (struct ospf *ospf)
{
  struct ospf_area *area;
  struct listnode *node;
...
static void
ospf_gr_grace_period_expired (struct event *thread)
{
 ...
 */
static char *
ospf_gr_nvm_filepath (struct ospf *ospf)
{
...
void
ospf_gr_iface_send_grace_lsa (struct event *thread)
{
 ...
 */
static void
ospf_gr_nvm_update (struct ospf *ospf, bool prepare)
{
  char *filepath;
 ...
 */
void
ospf_gr_nvm_delete (struct ospf *ospf)
{
  char *filepath;
  const char *inst_name;
  json_object *json;
 ...
 */
void
ospf_gr_nvm_read (struct ospf *ospf)
{
  char *filepath;
  const char *inst_name;
  json_object *json;
...
DEFPY (graceful_restart_prepare,
