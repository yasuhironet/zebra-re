...
static uint8_t *ospfGeneralGroup (struct variable *, oid *, size_t *, int,
                                  size_t *, WriteMethod **);
static uint8_t *ospfAreaEntry (struct variable *, oid *, size_t *, int,
                               size_t *, WriteMethod **);
static uint8_t *ospfStubAreaEntry (struct variable *, oid *, size_t *, int,
                                   size_t *, WriteMethod **);
static uint8_t *ospfLsdbEntry (struct variable *, oid *, size_t *, int,
                               size_t *, WriteMethod **);
static uint8_t *ospfAreaRangeEntry (struct variable *, oid *, size_t *, int,
                                    size_t *, WriteMethod **);
static uint8_t *ospfHostEntry (struct variable *, oid *, size_t *, int,
                               size_t *, WriteMethod **);
...
static uint8_t *ospfVirtIfEntry (struct variable *, oid *, size_t *, int,
                                 size_t *, WriteMethod **);
static uint8_t *ospfNbrEntry (struct variable *, oid *, size_t *, int,
                              size_t *, WriteMethod **);
...
static uint8_t *ospfExtLsdbEntry (struct variable *, oid *, size_t *, int,
                                  size_t *, WriteMethod **);
static uint8_t *ospfAreaAggregateEntry (struct variable *, oid *, size_t *,
...
static uint8_t *
ospfGeneralGroup (struct variable *v, oid *name, size_t *length, int exact,
...
static uint8_t *
ospfStubAreaEntry (struct variable *v, oid *name, size_t *length, int exact,
                   size_t *var_len, WriteMethod **write_method)
{
  struct ospf_area *area;
  struct in_addr addr;
...
lsdb_lookup_next (struct ospf_area *area, uint8_t *type, int type_next,
...
ospfLsdbLookup (struct variable *v, oid *name, size_t *length,
                struct in_addr *area_id, uint8_t *type, struct in_addr *ls_id,
      ...
      do
        {
          ...
          range =
              ospf_area_range_lookup_next (area, range_net, len == 0 ? 1 : 0);
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf_snmp_iflist, node, osif))
    {
      if (addr)
      ...
      else
        {
           ...
           * interface indexes */
          if (osif->addr.s_addr != INADDR_ANY || osif->ifindex > ifindex)
            break;
        }
      pn = node;
    }
      ...
      if (vl_data)
        {
          *length = v->namelen + IN_ADDR_SIZE + IN_ADDR_SIZE;
          ...
          oid_copy_in_addr (name + v->namelen + IN_ADDR_SIZE, neighbor);
          return vl_data;
        }
    }
  return NULL;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->oiflist, nn, oi))
    {
      for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
        if ((nbr = rn->info) != NULL && nbr != oi->nbr_self &&
            nbr->state != NSM_Down && nbr->src.s_addr != INADDR_ANY)
          {
            if (first)
              {
                if (! min)
...
*/
static int32_t
ospf_snmp_neighbor_state (uint8_t nst)
{
  switch (nst)
    {
    case NSM_Attempt:
...
static struct ospf_lsa *
ospfExtLsdbLookup (struct variable *v, oid *name, size_t *length,
                   ...
                   struct in_addr *router_id, int exact)
{
  ...
  if (exact)
  ...
  else
    {
      ...
      lsa = ospf_lsdb_lookup_by_id_next (ospf->lsdb, *type, *ls_id, *router_id,
                                         first);
...
ospfTrapNbrStateChange (struct ospf_neighbor *on)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_info ("%s: trap sent: %pI4 now %s", __func__, &on->address.u.prefix4,
               msgbuf);
...
static int
ospf_snmp_nsm_change (struct ospf_neighbor *nbr, int next_state, int old_state)
{
...
ospfTrapIfStateChange (struct ospf_interface *oi)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_info ("%s: trap sent: %pI4 now %s", __func__, &oi->address->u.prefix4,
...
static int
ospf_snmp_ism_change (struct ospf_interface *oi, int state, int old_state)
{
...
/* Register OSPF2-MIB. */
static int
ospf_snmp_init (struct event_loop *tm)
{
