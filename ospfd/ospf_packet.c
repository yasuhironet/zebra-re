...
ospf_packet_dup (struct ospf_packet *op)
{
  ...
  if (stream_get_endp (op->s) != op->length)
    /* XXX size_t */
    zlog_debug ("ospf_packet_dup stream %lu ospf_packet %u size mismatch",
...
ospf_make_md5_digest (struct ospf_interface *oi, struct ospf_packet *op)
{
  ...
  if (stream_get_endp (op->s) != op->length)
    ...
    flog_warn (EC_OSPF_MD5,
               ...
               __func__, (unsigned long) stream_get_endp (op->s), op->length,
               &ospfh->router_id);
...
static void
ospf_ls_req_timer (struct event *thread)
{
  struct ospf_neighbor *nbr;
...
/* Cyclic timer function.  Fist registered in ospf_nbr_new () in
   ospf_neighbor.c  */
void
ospf_ls_upd_timer (struct event *thread)
{
  struct ospf_neighbor *nbr;
...
void
ospf_ls_ack_timer (struct event *thread)
{
  ...
  while ((stream_get_endp (op->s) - stream_get_getp (op->s)) > maxdatasize)
    {
      ...
      if (ret < 0)
        flog_err (
            ...
            __func__, &iph->ip_dst, iph->ip_id, iph->ip_off, iph->ip_len, mtu,
            safe_strerror (errno));

      if (IS_DEBUG_OSPF_PACKET (type - 1, SEND))
        {
          zlog_debug ("%s: sent id %d, off %d, len %d to %pI4", __func__,
                      iph->ip_id, iph->ip_off, iph->ip_len, &iph->ip_dst);
        }
...
static void
ospf_write (struct event *thread)
{
...
#define OSPF_WRITE_IPHL_SHIFT 2
  ...
  if (fd < 0 || ospf->oi_running == 0)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s failed to send, fd %d, instance %u", __func__, fd,
                    ospf->oi_running);
      return;
    }
  ...
  while ((pkt_count < ospf->write_oi_count) && oi)
    {
      ...
      if (ospf->intf_socket_enabled && (IF_OSPF_IF_INFO (oi->ifp))->oii_fd > 0)
      ...
      if (op->length > maxdatasize)
        ospf_write_frags (fd, op, &iph, &msg, maxdatasize, oi->ifp->mtu, flags,
                          type);
#endif /* WANT_OSPF_WRITE_FRAGMENT */
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s to %pI4, id %d, off %d, len %d, interface %s, mtu %u:",
                    __func__, &iph.ip_dst, iph.ip_id, iph.ip_off, iph.ip_len,
      ...
      if (IS_DEBUG_OSPF_PACKET (type - 1, SEND))
        {
          ...
          zlog_debug ("%s sent to [%pI4] via [%s].",
                      lookup_msg (ospf_packet_type_str, type, NULL), &op->dst,
                      IF_NAME (oi));
  ...
  if (! list_isempty (ospf->oi_write_q))
    event_add_write (master, ospf_write, ospf, ospf->fd, &ospf->t_write);
}

/* OSPF Hello message read -- RFC2328 Section 10.5. */
static void
ospf_hello (struct ip *iph, struct ospf_header *ospfh, struct stream *s,
    ...
    if (oi->address->prefixlen != p.prefixlen)
      {
        flog_warn (
            ...
            &ospfh->router_id, IF_NAME (oi), (int) oi->address->prefixlen,
            (int) p.prefixlen);
        return;
      }
  ...
  if (OSPF_IF_PARAM (oi, v_wait) != ntohl (hello->dead_interval))
    {
      flog_warn (
          ...
          &ospfh->router_id, IF_NAME (oi), OSPF_IF_PARAM (oi, v_wait),
      ...
      if (OSPF_IF_PARAM (oi, v_hello) != ntohs (hello->hello_interval))
        {
          flog_warn (
              ...
              &ospfh->router_id, IF_NAME (oi), OSPF_IF_PARAM (oi, v_hello),
      ...
      if (! (CHECK_FLAG (OPTIONS (oi), OSPF_OPTION_NP) &&
             ...
             ! CHECK_FLAG (hello->options, OSPF_OPTION_E)))
        {
          flog_warn (
              ...
              &ospfh->router_id, OPTIONS (oi), hello->options);
          return;
        }
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("NSSA-Hello:RECV:Packet from %pI4:", &ospfh->router_id);
    }
  else
  ...
  if (OSPF_GR_IS_ACTIVE_HELPER (nbr))
  ...
  else
    {
      ...
      if (IPV4_ADDR_SAME (&nbr->address.u.prefix4, &hello->d_router))
        if (hello->bd_router.s_addr == INADDR_ANY && oi->state == ISM_Waiting)
...
static void
ospf_db_desc_save_current (struct ospf_neighbor *nbr, struct ospf_db_desc *dd)
{
  ...
  for (size -= OSPF_DB_DESC_MIN_SIZE; size >= OSPF_LSA_HEADER_SIZE;
       size -= OSPF_LSA_HEADER_SIZE)
    {
      ...
      if (lsah->type < OSPF_MIN_LSA || lsah->type >= OSPF_MAX_LSA)
        {
          flog_warn (EC_OSPF_PACKET, "Packet [DD:RECV]: Unknown LS type %d.",
                     lsah->type);
...
static void
ospf_db_desc (struct ip *iph, struct ospf_header *ospfh, struct stream *s,
  ...
  if ((OSPF_IF_PARAM (oi, mtu_ignore) == 0) &&
      (ntohs (dd->mtu) > oi->ifp->mtu))
    {
      flog_warn (
          ...
          &nbr->router_id, ntohs (dd->mtu), IF_NAME (oi), oi->ifp->mtu);
      return;
    }
  ...
  switch (nbr->state)
    ...
    case NSM_ExStart:
      ...
      if (CHECK_FLAG (oi->ospf->config, OSPF_OPAQUE_CAPABLE))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("Neighbor[%pI4] is %sOpaque-capable.", &nbr->router_id,
                        CHECK_FLAG (nbr->options, OSPF_OPTION_O) ? ""
                                                                 : "NOT ");
    ...
    case NSM_Exchange:
      ...
      if (IS_SET_DD_I (dd->flags))
        {
          zlog_info ("Packet[DD]: Neighbor %pI4 I-bit set.", &nbr->router_id);
          OSPF_NSM_EVENT_SCHEDULE (nbr, NSM_SeqNumberMismatch);
          break;
        }
...
static void
ospf_ls_req (struct ip *iph, struct ospf_header *ospfh, struct stream *s,
             struct ospf_interface *oi, uint16_t size)
{
  struct ospf_neighbor *nbr;
  uint32_t ls_type;
  ...
  if (nbr == NULL)
    {
      flog_warn (EC_OSPF_PACKET, "Link State Request: Unknown Neighbor %pI4",
                 &ospfh->router_id);
      return;
    }
      ...
      if (length + ntohs (find->data->length) > ospf_packet_max (oi))
        {
          if (oi->type == OSPF_IFTYPE_NBMA)
            ...
            ospf_ls_upd_send (nbr, ls_upd, OSPF_SEND_PACKET_INDIRECT, 0);
                      ...
                      struct ospf_interface *oi, size_t size)
{
  ...
  for (; size >= OSPF_LSA_HEADER_SIZE && count > 0;
       size -= length, stream_forward_getp (s, length), count--)
    {
      ...
      if (! ospf_lsa_checksum_valid (lsah))
        {
          ...
          flog_warn (
              ...
              sum, lsah->checksum, &lsah->id, &nbr->src, &nbr->router_id,
              &lsah->adv_router);
          continue;
        }
      ...
      if (lsah->type < OSPF_MIN_LSA || lsah->type >= OSPF_MAX_LSA)
        {
          flog_warn (EC_OSPF_PACKET, "Link State Update: Unknown LS type %d",
                     lsah->type);
          continue;
        }
      ...
      if (CHECK_FLAG (nbr->options, OSPF_OPTION_O))
        {
          ...
          if ((IS_OPAQUE_LSA (lsah->type) &&
               ...
               CHECK_FLAG (lsah->options, OSPF_OPTION_O)))
            {
               ...
               * only.
               */
              flog_warn (EC_OSPF_PACKET, "LSA[Type%d:%pI4]: O-bit abuse?",
                         lsah->type, &lsah->id);
              continue;
            }
#endif /* STRICT_OBIT_USAGE_CHECK */
...
ospf_ls_upd (struct ospf *ospf, struct ip *iph, struct ospf_header *ospfh,
             struct stream *s, struct ospf_interface *oi, uint16_t size)
{
  struct ospf_neighbor *nbr;
  struct list *lsas;
  struct listnode *node, *nnode;
  ...
  for (ALL_LIST_ELEMENTS (lsas, node, nnode, lsa))
    {
      ...
      if (IS_DEBUG_OSPF (lsa, LSA))
        zlog_debug ("LSA Type-%d from %pI4, ID: %pI4, ADV: %pI4",
                    lsa->data->type, &ospfh->router_id, &lsa->data->id,
                    &lsa->data->adv_router);
        ...
        if (! IPV4_ADDR_SAME (&lsa->data->id, &lsa->data->adv_router))
          {
            flog_err (
                ...
                &ospfh->router_id, &lsa->data->id, &lsa->data->adv_router);
            flog_err (
      ...
      if (IS_OPAQUE_LSA (lsa->data->type) &&
          IPV4_ADDR_SAME (&lsa->data->adv_router, &oi->ospf->router_id))
        {
          ...
          if (IS_LSA_MAXAGE (lsa))
            {
              zlog_info ("LSA[%s]: Boomerang effect?", dump_lsa_key (lsa));
              ospf_ls_ack_send (nbr, lsa);
              ospf_lsa_discard (lsa);
...
static void
ospf_ls_ack (struct ip *iph, struct ospf_header *ospfh, struct stream *s,
             struct ospf_interface *oi, uint16_t size)
{
  struct ospf_neighbor *nbr;
...
ospf_recv_packet (struct ospf *ospf, int fd, struct interface **ifp,
                  struct stream *ibuf)
{
  int ret;
  struct ip *iph;
  ...
  ret =
      stream_recvmsg (ibuf, fd, &msgh, MSG_DONTWAIT, OSPF_MAX_PACKET_SIZE + 1);
  if (ret < 0)
    {
      ...
      if (OSPF_AUTH_NULL != (iface_auth_type = ospf_auth_type (oi)))
        {
          if (IS_DEBUG_OSPF_PACKET (ospfh->type - 1, RECV))
            flog_warn (
                ...
                IF_NAME (oi),
                lookup_msg (ospf_auth_type_str, iface_auth_type, NULL),
                &ospfh->router_id);
          return 0;
        }
...
ospf_check_sum (struct ospf_header *ospfh)
{
  ...
  if (ret != sum)
    {
      zlog_info ("%s: checksum mismatch, my %X, his %X", __func__, ret, sum);
      return 0;
    }
...
static unsigned
ospf_router_lsa_links_examin (struct router_lsa_link *link, uint16_t linkbytes,
                              const uint16_t num_links)
{
  ...
  while (linkbytes >= OSPF_ROUTER_LSA_LINK_SIZE)
    {
      ...
      if (thislinklen > linkbytes)
        {
          if (IS_DEBUG_OSPF_PACKET (0, RECV))
            zlog_debug ("%s: length error in link block #%u", __func__,
                        counted_links);
          return MSG_NG;
        }
  ...
  if (counted_links != num_links)
    {
      if (IS_DEBUG_OSPF_PACKET (0, RECV))
        zlog_debug ("%s: %u link blocks declared, %u present", __func__,
...
/* Verify, that the given LSA is properly sized/aligned (including
   type-specific minimum length constraint). */
static unsigned
ospf_lsa_examin (struct lsa_header *lsah, const uint16_t lsalen,
                 const uint8_t headeronly)
{
  ...
  switch (lsah->type)
    ...
    case OSPF_ROUTER_LSA:
      {
         ...
         * (12+)-byte link blocks
         */
        size_t linkbytes_len =
            lsalen - OSPF_LSA_HEADER_SIZE - OSPF_ROUTER_LSA_MIN_SIZE;
        ...
        ret = ospf_router_lsa_links_examin (
            (struct router_lsa_link *) rlsa->link, linkbytes_len,
            ntohs (rlsa->links));
        break;
      }
    case OSPF_AS_EXTERNAL_LSA:
    ...
    case OSPF_AS_NSSA_LSA:
      /* RFC3101 C, idem */
      ret =
          (lsalen - OSPF_LSA_HEADER_SIZE - OSPF_AS_EXTERNAL_LSA_MIN_SIZE) % 12
              ? MSG_NG
              : MSG_OK;
      break;
      ...
      if (headeronly)
        {
          ...
          if (MSG_OK != ospf_lsa_examin (lsah, lsalen, 1))
            {
              if (IS_DEBUG_OSPF_PACKET (0, RECV))
                zlog_debug ("%s: malformed header-only LSA #%u", __func__,
                            counted_lsas);
              return MSG_NG;
            }
...
ospf_packet_examin (struct ospf_header *oh, const unsigned bytesonwire)
{
  ...
  if (oh->version != OSPF_VERSION)
    {
      if (IS_DEBUG_OSPF_PACKET (0, RECV))
        zlog_debug ("%s: invalid (%u) protocol version", __func__,
                    oh->version);
      return MSG_NG;
    }
  ...
  if (ntohs (oh->auth_type) != OSPF_AUTH_CRYPTOGRAPHIC)
  ...
  else
    {
      if (oh->u.crypt.auth_data_len != OSPF_AUTH_MD5_SIZE)
        {
          if (IS_DEBUG_OSPF_PACKET (0, RECV))
            zlog_debug ("%s: unsupported crypto auth length (%u B)", __func__,
  ...
  if (oh->type >= OSPF_MSG_HELLO && oh->type <= OSPF_MSG_LS_ACK &&
    ...
    {
      if (IS_DEBUG_OSPF_PACKET (0, RECV))
        zlog_debug ("%s: undersized (%u B) %s packet", __func__, bytesdeclared,
  ...
  switch (oh->type)
    ...
    case OSPF_MSG_HELLO:
      /* RFC2328 A.3.2, packet header + OSPF_HELLO_MIN_SIZE bytes
         followed
         by N>=0 router-IDs. */
      ret = (bytesdeclared - OSPF_HEADER_SIZE - OSPF_HELLO_MIN_SIZE) % 4
                ? MSG_NG
                : MSG_OK;
      break;
    case OSPF_MSG_DB_DESC:
                    ...
                    struct ip *iph, struct ospf_header *ospfh)
{
  ...
  if (! ospf_check_area_id (oi, ospfh))
    {
      flog_warn (EC_OSPF_PACKET,
                 "interface %s: ospf_read invalid Area ID %pI4", IF_NAME (oi),
                 &ospfh->area_id);
      return -1;
    }
...
static enum ospf_read_return_enum
ospf_read_helper (struct ospf *ospf)
{
  int ret;
  struct stream *ibuf;
  struct ospf_interface *oi;
      ...
      if (ifp == NULL)
        {
          if (IS_DEBUG_OSPF_PACKET (0, RECV))
            zlog_debug (
                ...
                __func__, &iph->ip_src, ospf_get_name (ospf));
      ...
      if ((unsigned int) (iph->ip_hl << 2) == STREAM_READABLE (ibuf))
      ...
      else
        {
          flog_warn (
              ...
              (unsigned int) (iph->ip_hl << 2), STREAM_READABLE (ibuf));
        }
  ...
  if (MSG_OK != ospf_packet_examin (ospfh, stream_get_endp (ibuf) -
  ...
  if (oi && OSPF_IF_PASSIVE_STATUS (oi) == OSPF_IF_PASSIVE)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug (
            ...
            &ospfh->router_id, &iph->ip_dst, &oi->address->u.prefix4);
   ...
   */
  ...
  else if (oi->state == ISM_Down)
    {
      flog_warn (
          ...
          &iph->ip_src, &iph->ip_dst, ifp->name, if_flag_dump (ifp->flags));
...
static void
ospf_fill_header (struct ospf_interface *oi, struct stream *s, uint16_t length)
{
  struct ospf_header *ospfh;
...
static int
ospf_make_ls_req_func (struct stream *s, uint16_t *length, unsigned long delta,
                       struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
  struct ospf_interface *oi;
...
static int
ospf_make_ls_upd (struct ospf_interface *oi, struct list *update,
                  struct stream *s)
{
  struct ospf_lsa *lsa;
  struct listnode *node;
...
static int
ospf_make_ls_ack (struct ospf_interface *oi, struct list *ack,
                  struct stream *s)
{
  struct listnode *node, *nnode;
...
ospf_hello_send_sub (struct ospf_interface *oi, in_addr_t addr)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      if (oi->ospf->vrf_id)
        zlog_debug ("%s: Hello Tx interface %s ospf vrf %s id %u", __func__,
                    oi->ifp->name, ospf_vrf_id_to_name (oi->ospf->vrf_id),
                    oi->ospf->vrf_id);
    }
...
void
ospf_poll_timer (struct event *thread)
{
...
void
ospf_hello_reply_timer (struct event *thread)
{
  struct ospf_neighbor *nbr;
...
ospf_db_desc_send (struct ospf_neighbor *nbr)
{
  ...
  if (CHECK_FLAG (oi->ospf->config, OSPF_LOG_ADJACENCY_DETAIL))
    zlog_info ("%s:Packet[DD]: %pI4 DB Desc send with seqnum:%x , flags:%x",
               ospf_get_name (oi->ospf), &nbr->router_id, nbr->dd_seqnum,
               nbr->dd_flags);
}
  ...
  if ((OSPF_LS_UPD_MIN_SIZE + ntohs (lsa->data->length)) >
      ospf_packet_max (oi))
    {
      ...
      size = ntohs (lsa->data->length) +
             (oi->ifp->mtu - ospf_packet_max (oi)) + OSPF_LS_UPD_MIN_SIZE;
    }
  else
    size = oi->ifp->mtu;
...
static void
ospf_ls_upd_send_queue_event (struct event *thread)
{
  ...
  if (oi->type == OSPF_IFTYPE_NBMA)
    {
      ...
      if (IPV4_ADDR_SAME (&oi->address->u.prefix4, &p.prefix))
        flog_warn (EC_OSPF_PACKET, "* LS-Update is sent to myself.");
    }
...
static void
ospf_ls_ack_send_event (struct event *thread)
{
  struct ospf_interface *oi = EVENT_ARG (thread);
...
ospf_ls_ack_send (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
  ...
  if (IS_GRACE_LSA (lsa))
    {
      if (IS_DEBUG_OSPF_GR)
        zlog_debug ("%s, Sending GRACE ACK to Restarter.", __func__);
    }
