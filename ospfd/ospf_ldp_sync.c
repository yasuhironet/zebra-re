 ...
 */
int
ospf_ldp_sync_state_update (struct ldp_igp_sync_if_state state)
{
  struct ospf *ospf;
...
int
ospf_ldp_sync_announce_update (struct ldp_igp_sync_announce announce)
{
  struct ospf *ospf;
  struct vrf *vrf;
  struct interface *ifp;
...
void
ospf_ldp_sync_state_req_msg (struct interface *ifp)
{
 ...
 */
void
ospf_ldp_sync_if_init (struct ospf_interface *oi)
{
...
void
ospf_ldp_sync_if_start (struct interface *ifp, bool send_state_req)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
...
void
ospf_ldp_sync_if_complete (struct interface *ifp)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
...
void
ospf_ldp_sync_ldp_fail (struct interface *ifp)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
...
void
ospf_ldp_sync_if_down (struct interface *ifp)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
...
void
ospf_ldp_sync_if_remove (struct interface *ifp, bool remove)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
...
static int
ospf_ldp_sync_ism_change (struct ospf_interface *oi, int state, int old_state)
{
 ...
 */
static void
ospf_ldp_sync_holddown_timer (struct event *thread)
{
  struct interface *ifp;
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
  ...
  if (params->ldp_sync_info)
    {
      ...
      ols_debug ("%s: holddown timer expired for %s state: %s", __func__,
...
void
ospf_ldp_sync_holddown_timer_add (struct interface *ifp)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
  ...
  ols_debug ("%s: start holddown timer for %s time %d", __func__, ifp->name,
 ...
 */
void
ospf_ldp_sync_gbl_exit (struct ospf *ospf, bool remove)
{
  struct interface *ifp;
  struct vrf *vrf;
 ...
 */
void
ospf_if_set_ldp_sync_enable (struct ospf *ospf, struct interface *ifp)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
  ...
  if (params->type == OSPF_IFTYPE_POINTOPOINT || if_is_pointopoint (ifp))
    ...
    {
      ldp_sync_info->state = LDP_IGP_SYNC_STATE_NOT_REQUIRED;
      zlog_debug ("%s: Sync only runs on P2P links %s", __func__, ifp->name);
    }
}

void
ospf_if_set_ldp_sync_holddown (struct ospf *ospf, struct interface *ifp)
{
  struct ospf_if_params *params;
  struct ldp_sync_info *ldp_sync_info;
...
void
ospf_ldp_sync_show_info (struct vty *vty, struct ospf *ospf,
  ...
  if (CHECK_FLAG (ospf->ldp_sync_cmd.flags, LDP_SYNC_FLAG_ENABLE))
    {
      if (use_json)
        {
          json_object_boolean_true_add (json_vrf, "mplsLdpIgpSyncEnabled");
...
static void
show_ip_ospf_mpls_ldp_interface_sub (struct vty *vty,
                                     ...
                                     bool use_json)
{
  ...
  if (use_json)
    {
      if (ldp_sync_info->enabled == LDP_IGP_SYNC_ENABLED)
        json_object_boolean_true_add (json_interface_sub, "ldpIgpSyncEnabled");
      else
  ...
  else
    {
      ...
      vty_out (vty, "  LDP-IGP Synchronization enabled: %s\n",
               ldp_sync_info->enabled == LDP_IGP_SYNC_ENABLED ? "yes" : "no");
  ...
  switch (ldp_sync_info->state)
    ...
    case LDP_IGP_SYNC_STATE_REQUIRED_UP:
      if (use_json)
        json_object_string_add (json_interface_sub, "ldpIgpSyncState",
                                "Sync achieved");
      else
          ...
          if (use_json)
            {
              ...
              time_store =
                  monotime_until (&ldp_sync_info->t_holddown->u.sands, NULL) /
                  1000LL;

              json_object_int_add (json_interface_sub,
                                   "ldpIgpSyncTimeRemainInMsec", time_store);
          ...
          else
            {
              vty_out (vty, "  Holddown timer is running %s remaining\n",
                       ospf_timer_dump (ldp_sync_info->t_holddown, timebuf,
                                        sizeof (timebuf)));
...
static int
show_ip_ospf_mpls_ldp_interface_common (struct vty *vty, struct ospf *ospf,
          ...
          for (rn = route_top (IF_OIFS (ifp)); rn; rn = route_next (rn))
            {
              ...
              show_ip_ospf_mpls_ldp_interface_sub (
                  vty, oi, ifp, json_interface_sub, use_json);
 ...
 */
void
ospf_ldp_sync_write_config (struct vty *vty, struct ospf *ospf)
{
  if (CHECK_FLAG (ospf->ldp_sync_cmd.flags, LDP_SYNC_FLAG_ENABLE))
 ...
 */
void
ospf_ldp_sync_if_write_config (struct vty *vty, struct ospf_if_params *params)
  ...
  if (CHECK_FLAG (ldp_sync_info->flags, LDP_SYNC_FLAG_HOLDDOWN))
    {
      ...
      ospf = ospf_lookup_by_vrf_id (VRF_DEFAULT);
      if (ospf &&
          CHECK_FLAG (ospf->ldp_sync_cmd.flags, LDP_SYNC_FLAG_HOLDDOWN))
        ldp_sync_info->holddown = ospf->ldp_sync_cmd.holddown;
      else
        ldp_sync_info->holddown = LDP_IGP_SYNC_HOLDDOWN_DEFAULT;
    }
       ...
       JSON_STR)
{
  ...
  ret =
      show_ip_ospf_mpls_ldp_interface_common (vty, ospf, intf_name, json, uj);
  if (uj)
    vty_json (vty, json);
