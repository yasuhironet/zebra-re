...
ospf_handle_summarylsa_lsId_chg (struct ospf_area *area, struct prefix_ipv4 *p,
                                 uint8_t type, uint32_t metric,
...
static struct ospf_lsa *ospf_summary_lsa_refresh (struct ospf *ospf,
                                                  struct ospf_lsa *lsa);
static struct ospf_lsa *ospf_asbr_summary_lsa_prepare_and_flood (
    struct prefix_ipv4 *p, uint32_t metric, struct ospf_area *area,
    struct in_addr id);
static struct ospf_lsa *ospf_summary_asbr_lsa_refresh (struct ospf *ospf,
                                                       struct ospf_lsa *lsa);
static struct ospf_lsa *
ospf_handle_exnl_lsa_lsId_chg (struct ospf *ospf, struct external_info *ei,
                               struct in_addr id);
static struct ospf_lsa *
 ...
 */
bool
ospf_check_dna_lsa (const struct ospf_lsa *lsa)
{
...
{
  if (IS_DEBUG_OSPF (lsa, LSA))
    zlog_debug ("LSA[Type%d:%pI4]: data freed %p", lsah->type, &lsah->id,
                (void *) lsah);
...
{
  static char
      buf[sizeof ("Type255,id(255.255.255.255),ar(255.255.255.255)") + 1];
  ...
  if (lsa != NULL && (lsah = lsa->data) != NULL)
    {
      ...
      snprintf (buf, sizeof (buf), "Type%d,id(%s),ar(%s)", lsah->type, id, ar);
    }
  else
...
void
lsa_header_set (struct stream *s, uint8_t options, uint8_t type,
...
char
link_info_set (struct stream **s, struct in_addr id, struct in_addr data,
      ...
      if (STREAM_SIZE (*s) == OSPF_MAX_LSA_SIZE)
        {
           ...
           */
          ret = stream_resize_inplace (s, OSPF_MAX_PACKET_SIZE -
                                              OSPF_MAX_LSA_SIZE);
        }
...
lsa_link_ptop_set (struct stream **s, struct ospf_interface *oi)
{
  ...
  if (oi->ptp_dmvpn ||
      ! CHECK_FLAG (oi->connected->flags, ZEBRA_IFA_UNNUMBERED))
    {
      ...
      id.s_addr =
          CONNECTED_PREFIX (oi->connected)->u.prefix4.s_addr & mask.s_addr;
      links +=
...
static void
ospf_stub_router_timer (struct event *t)
{
...
ospf_router_lsa_originate (struct ospf_area *area)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: Originate router-LSA %p", new->data->type,
                  &new->data->id, (void *) new);
...
ospf_router_lsa_refresh (struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: router-LSA refresh", new->data->type,
                  &new->data->id);
      ospf_lsa_header_dump (new->data);
    }
      ...
      else if (! IPV4_ADDR_SAME (&lsa->data->id, &ospf->router_id))
        {
          if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
            zlog_debug ("LSA[Type%d:%pI4]: Refresh router-LSA for Area %s",
                        lsa->data->type, &lsa->data->id, area_str);
...
ospf_network_lsa_refresh (struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: network-LSA refresh", new->data->type,
                  &new->data->id);
      ospf_lsa_header_dump (new->data);
    }
...
static void
stream_put_ospf_metric (struct stream *s, uint32_t metric_value)
{
  uint32_t metric;
  char *mp;
...
static void
ospf_summary_lsa_body_set (struct stream *s, struct prefix *p, uint32_t metric)
{
  struct in_addr mask;
...
ospf_summary_lsa_new (struct ospf_area *area, struct prefix *p,
                      uint32_t metric, struct in_addr id)
{
  struct stream *s;
  struct ospf_lsa *new;
  struct lsa_header *lsah;
                                 ...
                                 struct in_addr old_id)
{
  ...
  lsa = ospf_lsdb_lookup_by_id (area->lsdb, type, p->prefix, ospf->router_id);
  ...
  if (type == OSPF_SUMMARY_LSA)
    {
      ...
      ospf_summary_lsa_prepare_and_flood (&old_prefix, old_metric, old_area,
                                          old_id);
    }
  else
    {
...
/* Originate Summary-LSA. */
struct ospf_lsa *
ospf_summary_lsa_originate (struct prefix_ipv4 *p, uint32_t metric,
                            struct ospf_area *area)
{
  struct in_addr id;
  ...
  status =
      ospf_lsa_unique_id (area->ospf, area->lsdb, OSPF_SUMMARY_LSA, p, &id);
  ...
  if (status == LSID_CHANGE)
    {
      ...
      new = ospf_handle_summarylsa_lsId_chg (area, p, OSPF_SUMMARY_LSA, metric,
                                             id);
      return new;
    }
...
ospf_summary_lsa_refresh (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: summary-LSA refresh", new->data->type,
                  &new->data->id);
      ospf_lsa_header_dump (new->data);
    }
...
static struct ospf_lsa *
ospf_asbr_summary_lsa_prepare_and_flood (struct prefix_ipv4 *p,
                                         uint32_t metric,
                                         struct ospf_area *area,
                                         struct in_addr id)
{
...
struct ospf_lsa *
ospf_summary_asbr_lsa_originate (struct prefix_ipv4 *p, uint32_t metric,
                                 struct ospf_area *area)
{
  struct ospf_lsa *new;
  struct in_addr id;
  ...
  status = ospf_lsa_unique_id (area->ospf, area->lsdb, OSPF_ASBR_SUMMARY_LSA,
                               p, &id);
  ...
  if (status == LSID_CHANGE)
    {
      ...
      new = ospf_handle_summarylsa_lsId_chg (area, p, OSPF_ASBR_SUMMARY_LSA,
                                             metric, id);
      return new;
    }
  else if (status == LSID_NOT_AVAILABLE)
...
int
metric_type (struct ospf *ospf, uint8_t src, unsigned short instance)
{
...
int
metric_value (struct ospf *ospf, uint8_t src, unsigned short instance)
{
  struct ospf_redist *red;
                            ...
                            struct ospf *ospf)
{
  ...
  mvalue = (ROUTEMAP_METRIC (ei) != -1) ? ROUTEMAP_METRIC (ei)
                               ...
                               struct in_addr id)
{
  ...
  lsa = ospf_lsdb_lookup_by_id (ospf->lsdb, OSPF_AS_EXTERNAL_LSA, ei->p.prefix,
                                ospf->router_id);
...
static struct ospf_lsa *
ospf_external_lsa_new (struct ospf *ospf, struct external_info *ei,
                       struct in_addr *old_id)
{
  struct ospf_lsa *new;
  struct in_addr id;
  ...
  /* Get Link State with unique ID. */
    ...
    {
      status = ospf_lsa_unique_id (ospf, ospf->lsdb, OSPF_AS_EXTERNAL_LSA,
                                   &ei->p, &id);
...
/* As Type-7 */
static void
ospf_install_flood_nssa (struct ospf *ospf, struct ospf_lsa *lsa)
{
  struct ospf_lsa *new;
  ...
  for (ALL_LIST_ELEMENTS (ospf->areas, node, nnode, area))
    {
      ...
      if (! IS_OSPF_ABR (ospf))
        {
          ...
          if (extlsa->e[0].fwd_addr.s_addr == INADDR_ANY)
            extlsa->e[0].fwd_addr =
                ospf_get_nssa_ip (area); /* this NSSA area in ifp */
      ...
      ospf_flood_through_as (ospf, NULL,
                             new); /* all attached NSSA's, no AS/STUBs */
    }
}

static struct ospf_lsa *
ospf_lsa_translated_nssa_new (struct ospf *ospf, struct ospf_lsa *type7)
{
  ...
  if ((new = ospf_external_lsa_new (ospf, &ei, &type7->data->id)) == NULL)
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: Could not originate Translated Type-5 for %pI4",
  ...
  if (type7->area->suppress_fa)
    {
      ...
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: Suppress forwarding address for %pI4", __func__,
                    &ei.p.prefix);
    }
  else
                                ...
                                struct ospf_lsa *type5)
{
  ...
  if ((translated_lsa = ospf_lsa_translated_nssa_new (ospf, type7)) == NULL)
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: Could not translate Type-7, Id %pI4, to Type-5",
  ...
  if (IS_DEBUG_OSPF_NSSA)
    {
      ...
      zlog_debug ("   Forward addr: %pI4", &extnew->e[0].fwd_addr);
    }
                              ...
                              struct ospf_lsa *type5)
{
  ...
  if (! type7)
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: no Type-7 found for Type-5 LSA Id %pI4", __func__,
                    &type5->data->id);
      return NULL;
    }
  ...
  if (type5 == NULL || ! CHECK_FLAG (type5->flags, OSPF_LSA_LOCAL_XLT))
    {
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("%s: No translated Type-5 found for Type-7 with Id %pI4",
                    __func__, &type7->data->id);
      return NULL;
    }
  ...
  if (! (new = ospf_lsa_install (ospf, NULL, translated_lsa)))
    {
      flog_warn (EC_OSPF_LSA_INSTALL_FAILURE,
                 "%s: Could not install translated LSA, Id %pI4", __func__,
                 &type7->data->id);
      ospf_lsa_free (translated_lsa);
      return NULL;
    }
...
struct ospf_lsa *
ospf_nssa_lsa_originate (struct ospf_area *area, struct external_info *ei)
{
  struct ospf *ospf = area->ospf;
  struct ospf_lsa *new;
  ...
  if ((new = ospf_external_lsa_new (ospf, ei, NULL)) == NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("LSA[Type7:%pI4]: Could not originate NSSA-LSA",
                    &ei->p.prefix);
      return NULL;
    }
  new->data->type = OSPF_AS_NSSA_LSA;
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: Originate NSSA-LSA %p", new->data->type,
                  &new->data->id, (void *) new);
      ospf_lsa_header_dump (new->data);
    }
...
/* Refresh NSSA-LSA. */
struct ospf_lsa *
ospf_nssa_lsa_refresh (struct ospf_area *area, struct ospf_lsa *lsa,
                       struct external_info *ei)
{
  struct ospf *ospf = area->ospf;
  struct ospf_lsa *new;
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: NSSA-LSA refresh", new->data->type,
                  &new->data->id);
      ospf_lsa_header_dump (new->data);
    }
...
void
ospf_external_lsa_rid_change (struct ospf *ospf)
{
  struct external_info *ei;
          ...
          for (rn = route_top (rt); rn; rn = route_next (rn))
            {
              ...
              if (aggr)
              ...
              else if (lsa)
                {
                  ...
                  ospf_external_lsa_refresh (ospf, lsa, ei, force, 0);
                }
              else
                {
                  if (! ospf_redistribute_check (ospf, ei, NULL))
  ...
  for (ALL_LIST_ELEMENTS (ospf->areas, node, nnode, area))
    {
      if (area->external_routing == OSPF_AREA_NSSA)
        {
          lsa = ospf_lsa_lookup (ospf, area, OSPF_AS_NSSA_LSA, p->prefix,
                                 ospf->router_id);
          if (! lsa)
            {
...
void
ospf_external_lsa_flush (struct ospf *ospf, uint8_t type,
                         struct prefix_ipv4 *p,
                         ifindex_t ifindex /*, struct in_addr nexthop */)
{
  ...
  if (! (lsa = ospf_external_info_find_lsa (ospf, p)))
    {
      if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
        zlog_debug ("LSA: There is no such AS-external-LSA %pFX in LSDB", p);
      return;
    }
  ...
  if (ei && lsa)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("LSA[Type5:0.0.0.0]: Refresh AS-external-LSA %p",
                    (void *) lsa);
      ospf_external_lsa_refresh (ospf, lsa, ei, LSA_REFRESH_FORCE, false);
    }
  else if (ei && ! lsa)
    {
      if (IS_DEBUG_OSPF_EVENT)
...
void
ospf_external_lsa_refresh_type (struct ospf *ospf, uint8_t type,
                  ...
                  if (aggr)
                    {
                      ...
                      if (IS_DEBUG_OSPF (lsa, EXTNL_LSA_AGGR))
                        zlog_debug ("%s: Send Aggreate LSA (%pFX)", __func__,
                                    &aggr->p);
...
ospf_external_lsa_refresh (struct ospf *ospf, struct ospf_lsa *lsa,
                           struct external_info *ei, int force, bool is_aggr)
{
  struct ospf_lsa *new;
  int changed = 0;
  ...
  if (ospf->anyNSSA && ! (CHECK_FLAG (new->flags, OSPF_LSA_LOCAL_XLT)))
    ospf_install_flood_nssa (ospf, new); /* Install/Flood per new rules */
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    {
      zlog_debug ("LSA[Type%d:%pI4]: AS-external-LSA refresh", new->data->type,
                  &new->data->id);
      ospf_lsa_header_dump (new->data);
    }
...
static struct ospf_lsa *
ospf_network_lsa_install (struct ospf *ospf, struct ospf_interface *oi,
...
void
ospf_discard_from_db (struct ospf *ospf, struct ospf_lsdb *lsdb,
                      struct ospf_lsa *lsa)
{
  struct ospf_lsa *old;
                  ...
                  struct ospf_lsa *lsa)
{
  ...
  */
      ...
      if (ospf_lsa_is_self_originated (ospf, lsa))
        {
          ...
          if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
            {
              zlog_debug ("%s() Premature Aging lsa %p, seqnum 0x%x", __func__,
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_INSTALL))
    {
      switch (lsa->data->type)
        ...
        case OSPF_AS_NSSA_LSA:
          ...
          break;
        default:
          zlog_debug ("LSA[%s]: Install %s to Area %pI4", dump_lsa_key (new),
                      lookup_msg (ospf_lsa_type_msg, new->data->type, NULL),
                      &new->area->area_id);
          break;
        }
  ...
  if (IS_LSA_MAXAGE (new))
    {
      if (IS_DEBUG_OSPF (lsa, LSA_INSTALL))
        zlog_debug ("LSA[%s]: Install LSA %p, MaxAge", dump_lsa_key (new),
                    lsa);
...
void
ospf_maxage_lsa_remover (struct event *thread)
{
    ...
    for (rn = route_top (ospf->maxage_lsa); rn; rn = route_next (rn))
      {
        ...
        if (IS_LSA_SELF (lsa))
          if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
            zlog_debug ("LSA[Type%d:%pI4]: LSA 0x%lx is self-originated: ",
                        lsa->data->type, &lsa->data->id, (unsigned long) lsa);

        if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
          zlog_debug ("LSA[%s]: MaxAge LSA removed from list",
                      dump_lsa_key (lsa));

        if (CHECK_FLAG (lsa->flags, OSPF_LSA_PREMATURE_AGE))
          {
            if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
              zlog_debug ("originating new lsa for lsa %p", lsa);
        ...
        if (lsa->lsdb)
          {
            ...
            if (old != lsa)
              {
                flog_err (EC_OSPF_LSA_MISSING, "%s: LSA[%s]: LSA not in LSDB",
          ...
          {
            if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
              zlog_debug ("%s: LSA[%s]: No associated LSDB!", __func__,
                          dump_lsa_key (lsa));
          }
      }
 ...
 */
void
ospf_check_and_gen_init_seq_lsa (struct ospf_interface *oi,
                                 struct ospf_lsa *recv_lsa)
{
  ...
  if ((lsa == NULL) || (! CHECK_FLAG (lsa->flags, OSPF_LSA_PREMATURE_AGE)) ||
    ...
    {
      if (IS_DEBUG_OSPF (lsa, LSA))
        zlog_debug ("Do not generate LSA with initial seqence number.");
      return;
    }
...
ospf_lsa_maxage_delete (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if ((rn = route_node_lookup (ospf->maxage_lsa, &lsa_prefix)))
    ...
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: lsa %s is not found in maxage db.", __func__,
                    dump_lsa_key (lsa));
    }
}
...
ospf_lsa_maxage (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if (CHECK_FLAG (lsa->flags, OSPF_LSA_IN_MAXAGE))
    {
      if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
        zlog_debug ("LSA[%s]: %p already exists on MaxAge LSA list",
...
void
ospf_lsa_maxage_walker (struct event *thread)
{
  struct ospf *ospf = EVENT_ARG (thread);
  struct route_node *rn;
  struct ospf_lsa *lsa;
...
struct ospf_lsa *
ospf_lsa_lookup_by_prefix (struct ospf_lsdb *lsdb, uint8_t type,
...
struct ospf_lsa *
ospf_lsa_lookup (struct ospf *ospf, struct ospf_area *area, uint32_t type,
...
struct ospf_lsa *
ospf_lsa_lookup_by_id (struct ospf_area *area, uint32_t type,
                       struct in_addr id)
{
  struct ospf_lsa *lsa;
  struct route_node *rn;
...
ospf_lsa_lookup_by_header (struct ospf_area *area, struct lsa_header *lsah)
{
  ...
  if (match == NULL)
    if (IS_DEBUG_OSPF (lsa, LSA) == OSPF_DEBUG_LSA)
      zlog_debug ("LSA[Type%d:%pI4]: Lookup by header, NO MATCH", lsah->type,
                  &lsah->id);
                    ...
                    bool ignore_rcvd_flag)
{
  ...
  if (memcmp (p1 + OSPF_LSA_HEADER_SIZE, p2 + OSPF_LSA_HEADER_SIZE,
              l1->size - OSPF_LSA_HEADER_SIZE) != 0)
    return 1;
  ...
  for (ALL_LIST_ELEMENTS (ospf->areas, node, nnode, area))
    {
      ...
      for (ALL_LIST_ELEMENTS (area->oiflist, node2, nnode2, oi))
        {
          if ((lsa = oi->network_lsa_self) != NULL && oi->state == ISM_DR &&
              oi->full_nbrs > 0)
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug (
 ...
 */
void
ospf_refresh_area_self_lsas (struct ospf_area *area)
{
...
enum lsid_status
ospf_lsa_unique_id (struct ospf *ospf, struct ospf_lsdb *lsdb, uint8_t type,
                    struct prefix_ipv4 *p, struct in_addr *id)
{
  struct ospf_lsa *lsa;
  struct in_addr mask;
  ...
  if (lsa)
    {
      ...
      if (ip_masklen (al->mask) == p->prefixlen)
        {
          if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
            zlog_debug ("%s: Can't get Link State ID for %pFX", __func__, p);
          /*	  id.s_addr = 0; */
      ...
      else if (ip_masklen (al->mask) < p->prefixlen)
        {
          ...
          id->s_addr = p->prefix.s_addr | (~mask.s_addr);
          lsa =
              ospf_lsdb_lookup_by_id (ospf->lsdb, type, *id, ospf->router_id);
          if (lsa)
            {
              if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
                zlog_debug ("%s: Can't get Link State ID for %pFX", __func__,
                            p);
              id->s_addr = 0xffffffff;
              return LSID_NOT_AVAILABLE;
            }
...
static void
ospf_lsa_action (struct event *t)
{
...
ospf_refresher_register_lsa (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if (lsa->refresh_list < 0)
    {
      int delay;
      int min_delay = ospf->lsa_refresh_timer - (2 * OSPF_LS_REFRESH_JITTER);
      int max_delay = ospf->lsa_refresh_timer - OSPF_LS_REFRESH_JITTER;
       ...
       * and 1740s.
       */
      delay = (frr_weak_random () % (max_delay - min_delay)) + min_delay;
      ...
      if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
        zlog_debug ("LSA[Refresh:Type%d:%pI4]: age %d, added to index %d",
                    lsa->data->type, &lsa->data->id, LS_AGE (lsa), index);
      ...
      if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
        zlog_debug (
            ...
            lsa->data->type, &lsa->data->id, __func__, (void *) lsa, index);
    }
}
...
void
ospf_lsa_refresh_walker (struct event *t)
{
  struct list *refresh_list;
  struct listnode *node, *nnode;
  ...
  for (; i != ospf->lsa_refresh_queue.index;
    ...
    {
      if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
        zlog_debug ("LSA[Refresh]: %s: refresh index %d", __func__, i);
          ...
          for (ALL_LIST_ELEMENTS (refresh_list, node, nnode, lsa))
            {
              if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
                zlog_debug (
                    ...
                    lsa->data->type, &lsa->data->id, __func__, (void *) lsa,
                    i);
...
void
ospf_flush_lsa_from_area (struct ospf *ospf, struct in_addr area_id, int type)
{
  struct ospf_area *area;
  struct route_node *rn;
  struct ospf_lsa *lsa;
