...
void
api_opaque_lsa_print (struct ospf_lsa *lsa)
{
  struct opaque_lsa
  {
...
struct msg *
msg_new (uint8_t msgtype, void *msgbody, uint32_t seqnum, uint16_t msglen)
{
  struct msg *new;
...
void
msg_set_seq (struct msg *msg, uint32_t seqnr)
{
  assert (msg);
...
struct msg *
new_msg_register_opaque_type (uint32_t seqnum, uint8_t ltype, uint8_t otype)
{
...
struct msg *
new_msg_register_event (uint32_t seqnum, struct lsa_filter_type *filter)
{
  uint8_t buf[OSPF_API_MAX_MSG_SIZE];
...
struct msg *
new_msg_sync_lsdb (uint32_t seqnum, struct lsa_filter_type *filter)
{
  uint8_t buf[OSPF_API_MAX_MSG_SIZE];
...
struct msg *
new_msg_originate_request (uint32_t seqnum, struct in_addr ifaddr,
                           struct in_addr area_id, struct lsa_header *data)
{
  ...
  omsglen +=
      sizeof (struct msg_originate_request) - sizeof (struct lsa_header);

  return msg_new (MSG_ORIGINATE_REQUEST, omsg, seqnum, omsglen);
}

struct msg *
new_msg_delete_request (uint32_t seqnum, struct in_addr addr, uint8_t lsa_type,
                        uint8_t opaque_type, uint32_t opaque_id, uint8_t flags)
{
...
struct msg *
new_msg_reply (uint32_t seqnr, uint8_t rc)
{
  struct msg *msg;
...
struct msg *
new_msg_ready_notify (uint32_t seqnr, uint8_t lsa_type, uint8_t opaque_type,
                      struct in_addr addr)
{
...
struct msg *
new_msg_new_if (uint32_t seqnr, struct in_addr ifaddr, struct in_addr area_id)
{
...
struct msg *
new_msg_del_if (uint32_t seqnr, struct in_addr ifaddr)
{
...
struct msg *
new_msg_ism_change (uint32_t seqnr, struct in_addr ifaddr,
...
struct msg *
new_msg_nsm_change (uint32_t seqnr, struct in_addr ifaddr,
...
struct msg *
new_msg_lsa_change_notify (uint8_t msgtype, uint32_t seqnum,
...
struct msg *
new_msg_reachable_change (uint32_t seqnum, uint16_t nadd, struct in_addr *add,
                          uint16_t nremove, struct in_addr *remove)
{
  uint8_t buf[OSPF_API_MAX_MSG_SIZE];
...
struct msg *
new_msg_router_id_change (uint32_t seqnum, struct in_addr router_id)
{
