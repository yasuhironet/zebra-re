...
#define LS_AGE(x)                                                             \
  (OSPF_LSA_MAXAGE < get_age (x) ? OSPF_LSA_MAXAGE : get_age (x))
...
extern void ospf_router_lsa_body_set (struct stream **s,
                                      struct ospf_area *area);
...
ospf_lsa_install (struct ospf *, struct ospf_interface *, struct ospf_lsa *);
...
extern struct ospf_lsa *ospf_external_lsa_refresh (struct ospf *,
                                                   ...
                                                   bool aggr);
extern enum lsid_status
ospf_lsa_unique_id (struct ospf *ospf, struct ospf_lsdb *lsdb, uint8_t type,
                    struct prefix_ipv4 *p, struct in_addr *addr);
extern void ospf_schedule_lsa_flood_area (struct ospf_area *,
                                          struct ospf_lsa *);
extern void ospf_schedule_lsa_flush_area (struct ospf_area *,
                                          struct ospf_lsa *);
...
extern void ospf_check_and_gen_init_seq_lsa (struct ospf_interface *oi,
                                             struct ospf_lsa *lsa);
extern void ospf_flush_lsa_from_area (struct ospf *ospf,
                                      struct in_addr area_id, int type);
 ...
 */
static inline bool
ospf_check_indication_lsa (struct ospf_lsa *lsa)
{
