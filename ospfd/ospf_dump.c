...
const char *
ospf_redist_string (unsigned int route_type)
{
...
ospf_area_desc_string (struct ospf_area *area)
{
  ...
  switch (type)
    {
    case OSPF_AREA_NSSA:
      snprintf (buf, sizeof (buf), "%s [NSSA]", ospf_area_name_string (area));
      break;
    case OSPF_AREA_STUB:
      snprintf (buf, sizeof (buf), "%s [Stub]", ospf_area_name_string (area));
      break;
    default:
...
ospf_if_name_string (struct ospf_interface *oi)
{
  ...
  snprintf (buf, sizeof (buf), "%s:%d.%d.%d.%d", oi->ifp->name,
            (ifaddr >> 24) & 0xff, (ifaddr >> 16) & 0xff, (ifaddr >> 8) & 0xff,
            ifaddr & 0xff);
  return buf;
}

int
ospf_nbr_ism_state (struct ospf_neighbor *nbr)
{
  int state;
...
void
ospf_nbr_ism_state_message (struct ospf_neighbor *nbr, char *buf, size_t size)
{
  int state;
  struct ospf_interface *oi = nbr->oi;
...
const char *
ospf_timer_dump (struct event *t, char *buf, size_t size)
{
...
static void
ospf_packet_hello_dump (struct stream *s, uint16_t length)
{
...
static char *
ospf_dd_flags_dump (uint8_t flags, char *buf, size_t size)
{
...
static char *
ospf_router_lsa_flags_dump (uint8_t flags, char *buf, size_t size)
{
  snprintf (buf, size, "%s|%s|%s", (flags & ROUTER_LSA_VIRTUAL) ? "V" : "-",
...
static void
ospf_router_lsa_dump (struct stream *s, uint16_t length)
{
  char buf[BUFSIZ];
...
static void
ospf_network_lsa_dump (struct stream *s, uint16_t length)
{
  ...
  */
  ...
  for (i = 0; i < cnt; i++)
    zlog_debug ("      Attached Router %pI4", &nl->routers[i]);
}

static void
ospf_summary_lsa_dump (struct stream *s, uint16_t length)
{
...
static void
ospf_as_external_lsa_dump (struct stream *s, uint16_t length)
{
  ...
  for (i = 0; sum < size && asr; sum += 12, asr = &al->e[++i])
    {
      zlog_debug ("    bit %s TOS=%d metric %d",
                  IS_EXTERNAL_METRIC (asr->tos) ? "E" : "-", asr->tos & 0x7f,
                  GET_METRIC (asr->metric));
...
static void
ospf_lsa_header_list_dump (struct stream *s, uint16_t length)
{
...
static void
ospf_packet_db_desc_dump (struct stream *s, uint16_t length)
{
...
static void
ospf_packet_ls_req_dump (struct stream *s, uint16_t length)
{
  uint32_t sp;
  uint32_t ls_type;
...
static void
ospf_packet_ls_upd_dump (struct stream *s, uint16_t length)
{
  uint32_t sp;
  struct lsa_header *lsa;
  int lsa_len, len;
  ...
  while (len > 0 && count > 0)
    {
      if ((uint16_t) len < OSPF_LSA_HEADER_SIZE || len % 4 != 0)
        {
          zlog_debug ("  Remaining %d bytes; Incorrect length.", len);
          break;
        }
...
static void
ospf_packet_ls_ack_dump (struct stream *s, uint16_t length)
{
  uint32_t sp;
...
static int
debug_ospf_nsm_common (struct vty *vty, int arg_base, int argc,
...
static int
no_debug_ospf_nsm_common (struct vty *vty, int arg_base, int argc,
                          struct cmd_token **argv)
{
  /* XXX qlyoung */
  if (vty->node == CONFIG_NODE)
...
static int
debug_ospf_lsa_common (struct vty *vty, int arg_base, int argc,
                       struct cmd_token **argv)
{
  if (vty->node == CONFIG_NODE)
    {
...
static int
no_debug_ospf_lsa_common (struct vty *vty, int arg_base, int argc,
                          struct cmd_token **argv)
{
  if (vty->node == CONFIG_NODE)
    {
...
static int
debug_ospf_zebra_common (struct vty *vty, int arg_base, int argc,
                         struct cmd_token **argv)
{
  if (vty->node == CONFIG_NODE)
    {
...
static int
show_debugging_ospf_common (struct vty *vty)
{
  int i;
  ...
  if (IS_DEBUG_OSPF (bfd, BFD_LIB) == OSPF_DEBUG_BFD_LIB)
    vty_out (vty, "  OSPF BFD integration library debugging is on\n");
...
config_write_debug (struct vty *vty)
{
  ...
  for (i = 0; i < 5; i++)
    r &=
        conf_debug_ospf_packet[i] & (OSPF_DEBUG_SEND_RECV | OSPF_DEBUG_DETAIL);
  ...
  if (IS_CONF_DEBUG_OSPF (client_api, CLIENT_API) == OSPF_DEBUG_CLIENT_API)
    {
  ...
  if (IS_CONF_DEBUG_OSPF (defaultinfo, DEFAULTINFO) == OSPF_DEBUG_DEFAULTINFO)
    {
