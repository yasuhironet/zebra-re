...
const char *
ospf_path_type_name (int path_type)
{
  switch (path_type)
    {
  ...
  if (path->srni.backup_label_stack)
    {
      memsize =
          ...
          (sizeof (mpls_label_t) * path->srni.backup_label_stack->num_labels);
      new->srni.backup_label_stack = XCALLOC (MTYPE_OSPF_PATH, memsize);
      memcpy (new->srni.backup_label_stack, path->srni.backup_label_stack,
              memsize);
    }
...
void
ospf_route_delete (struct ospf *ospf, struct route_table *rt)
{
    ...
    if ((or = rn->info) != NULL)
      {
        if (or->type == OSPF_DESTINATION_NETWORK)
          ...
          ospf_zebra_delete_discard (ospf, (struct prefix_ipv4 *) &rn->p);
      }
}
...
static int
ospf_route_backup_path_same (struct sr_nexthop_info *srni1,
      ...
      if (or->type == OSPF_DESTINATION_NETWORK)
        {
          ...
          for (n1 = listhead (or->paths), n2 = listhead (newor->paths);
              ...
              n2 = listnextnode_unchecked (n2))
            {
              ...
              if (! ospf_route_backup_path_same (&op->srni, &newop->srni))
                return 0;
            }
          return 1;
        }
 ...
 */
static void
ospf_route_delete_same_ext (struct ospf *ospf,
      ...
      if (rn && rn->info)
        {
          ...
          if ((ext_rn =
                   route_node_lookup (external_routes, (struct prefix *) p)))
            {
              if (ext_rn->info)
                {
                  ospf_zebra_delete (ospf, p, ext_rn->info);
...
/* rt: Old, cmprt: New */
static void
ospf_route_delete_uniq (struct ospf *ospf, struct route_table *rt,
  ...
  if (! add_only)
    {
      ...
      if (! IS_ROUTER_LSA_BORDER (lsa) && ! IS_ROUTER_LSA_EXTERNAL (lsa))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: this router is neither ASBR nor ABR, skipping it",
                        __func__);
          return;
        }
                     ...
                     int parent_is_root, int lsa_pos)
{
  ...
  if (parent_is_root && link->link_data.s_addr == 0xffffffff &&
    ...
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: ignoring host route %pI4/32 to self.", __func__,
                    &link->link_id);
      return;
    }
  ...
  if (rn->info)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: another route to the same prefix found with cost %u",
      ...
      if (cost > cur_or->cost)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: old route is better, exit", __func__);
          return;
        }
      ...
      if (cost == cur_or->cost)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: routes are equal, merge", __func__);
      ...
      if (cost < cur_or->cost)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: new route is better, set it", __func__);
  ...
  if (v != area->spf)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: this network is on remote router", __func__);
      ospf_route_copy_nexthops_from_vertex (area, or, v);
    }
  else
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s: this network is on this router", __func__);
      ...
      if (oi || area->spf_dry_run)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("%s: the lsa pos is %d", __func__, lsa_pos);
...
static const char *const ospf_path_type_str[] = { "unknown-type", "intra-area",
                                                  "inter-area",
                                                  "type1-external",
                                                  "type2-external" };
    ...
    if ((or = rn->info) != NULL)
      {
        if (or->type == OSPF_DESTINATION_NETWORK)
          {
            zlog_debug ("N %-18pFX %-15pI4 %s %d", &rn->p, & or->u.std.area_id,
                        ospf_path_type_str[or->path_type], or->cost);
            for (ALL_LIST_ELEMENTS_RO (or->paths, pnode, path))
              zlog_debug ("  -> %pI4", &path->nexthop);
          }
        else
          zlog_debug ("R %-18pI4 %-15pI4 %s %d", &rn->p.u.prefix4,
                      & or->u.std.area_id, ospf_path_type_str[or->path_type],
                      or->cost);
      }
  zlog_debug ("========================================");
}
...
void
ospf_router_route_table_dump (struct route_table *rt)
{
  struct route_node *rn;
  struct ospf_route * or ;
...
void
ospf_route_copy_nexthops_from_vertex (struct ospf_area *area,
                                      struct ospf_route *to, struct vertex *v)
{
  struct listnode *node;
  struct ospf_path *path;
      ...
      if (rn->info != NULL)
        {
          ...
          if (listcount (or->paths) == 0)
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("Pruning route to %pFX", &rn->p);
  ...
  for (rn = route_top (rtrs); rn; rn = next)
    {
          ...
          if (listcount (or->paths) == 0)
            {
              if (IS_DEBUG_OSPF_EVENT)
                {
                  zlog_debug ("Pruning route to rtr %pI4", &rn->p.u.prefix4);
               ...
               * reachabele.
               */
              (CHECK_FLAG (or->u.std.origin->ls_age, DO_NOT_AGE))
                  ? UNSET_FLAG (or->u.std.origin->ls_age, DO_NOT_AGE)
                  : 0;
      ...
      if (listcount (paths) == 0)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("Pruning router node %pI4", &rn->p.u.prefix4);
...
void
ospf_delete_discard_route (struct ospf *ospf, struct route_table *rt,
