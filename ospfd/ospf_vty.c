...
area_id2str (char *buf, int length, struct in_addr *area_id, int area_id_fmt)
{
  if (area_id_fmt == OSPF_AREA_ID_FMT_DOTTEDQUAD)
    ...
    snprintf (buf, length, "%lu", (unsigned long) ntohl (area_id->s_addr));
}
...
static int
ospf_router_cmd_parse (struct vty *vty, struct cmd_token *argv[],
...
static void
ospf_show_vrf_name (struct ospf *ospf, struct vty *vty, json_object *json,
                    uint8_t use_vrf)
{
  if (use_vrf)
    {
      if (json)
        {
          json_object_string_add (json, "vrfName", ospf_get_name (ospf));
            ...
            VRF_CMD_HELP_STR)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug (
        ...
        ospf->instance, ospf_get_name (ospf), ospf->vrf_id, ospf->oi_running);
...
static void
ospf_passive_interface_default_update (struct ospf *ospf, uint8_t newval)
{
  struct listnode *ln;
              ...
              "IPv4 address\n")
{
  ...
  vty_out (vty, "This command is deprecated, because it is not VRF-aware.\n");
  ...
  if (count > 0)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("%s ospf vrf %s num of %u ip ospf area x config", __func__,
       ...
       "Advertised metric for this range\n")
{
  ...
  ospf_area_range_set (ospf, area, area->ranges, &p, OSPF_AREA_RANGE_ADVERTISE,
                       false);
  if (argc > 5)
    {
  ...
  if (vl_config->auth_key)
    {
      ...
      strlcpy ((char *) IF_DEF_PARAMS (ifp)->auth_simple, vl_config->auth_key,
  ...
  if (dflt_originate)
    {
      ...
      ospf_area_nssa_default_originate_set (ospf, area_id, mval, metric_type);
    }
  else
       ...
       "Use message-digest authentication\n")
{
  ...
  area = ospf_area_get (ospf, area_id);
  ospf_area_display_format_set (ospf, area, format);
  area->auth_type = strmatch (argv[0]->text, "no") ? OSPF_AUTH_NULL
...
static void
ospf_table_reinstall_routes (struct ospf *ospf, struct route_table *rt)
{
  struct route_node *rn;
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
    {
      ...
      if (or->type == OSPF_DESTINATION_NETWORK)
        ...
        ospf_zebra_add_discard (ospf, (struct prefix_ipv4 *) &rn->p);
    }
}

static void
ospf_reinstall_routes (struct ospf *ospf)
{
...
static int
ospf_timers_spf_set (struct vty *vty, unsigned int delay, unsigned int hold,
                     unsigned int max)
{
  VTY_DECLVAR_INSTANCE_CONTEXT (ospf, ospf);
...
static void
ospf_maxpath_set (struct vty *vty, struct ospf *ospf, uint16_t paths)
{
...
static const char *const ospf_abr_type_descr_str[] = {
  "Unknown", "Standard (RFC2328)", "Alternative IBM", "Alternative Cisco",
  "Alternative Shortcut"
};

static const char *const ospf_shortcut_mode_descr_str[] = { "Default",
                                                            "Enabled",
                                                            "Disabled" };
                   ...
                   json_object *json_areas, bool use_json)
{
          ...
          if (area->t_stub_router)
            {
              long time_store;
              time_store =
                  monotime_until (&area->t_stub_router->u.sands, NULL) /
                  1000LL;
  ...
  if (use_json)
    {
      ...
      if (! OSPF_IS_AREA_BACKBONE (area))
        json_object_int_add (json_area, "virtualAdjacenciesPassingCounter",
                             area->full_vls);
      ...
      json_object_int_add (json_area, "lsaRouterNumber",
                           ospf_lsdb_count (area->lsdb, OSPF_ROUTER_LSA));
      json_object_int_add (json_area, "lsaRouterChecksum",
                           ospf_lsdb_checksum (area->lsdb, OSPF_ROUTER_LSA));
      json_object_int_add (json_area, "lsaNetworkNumber",
                           ospf_lsdb_count (area->lsdb, OSPF_NETWORK_LSA));
      json_object_int_add (json_area, "lsaNetworkChecksum",
                           ospf_lsdb_checksum (area->lsdb, OSPF_NETWORK_LSA));
      json_object_int_add (json_area, "lsaSummaryNumber",
                           ospf_lsdb_count (area->lsdb, OSPF_SUMMARY_LSA));
      json_object_int_add (json_area, "lsaSummaryChecksum",
      ...
      json_object_int_add (json_area, "lsaNssaNumber",
                           ospf_lsdb_count (area->lsdb, OSPF_AS_NSSA_LSA));
      json_object_int_add (json_area, "lsaNssaChecksum",
  ...
  if (use_json)
    {
      json_object_int_add (json_area, "lsaOpaqueLinkNumber",
  ...
  if (area->fr_info.configured)
    {
      if (use_json)
        json_object_string_add (json_area, "areaFloodReduction", "configured");
      else
  ...
  if (area->fr_info.enabled)
    {
      if (use_json)
        {
          json_object_boolean_true_add (json_area,
                                        "areaFloodReductionEnabled");
          if (area->fr_info.router_lsas_recv_dc_bit)
            json_object_boolean_true_add (json_area, "lsasRecvDCbitSet");
          if (area->fr_info.area_ind_lsa_recvd)
            json_object_string_add (json_area, "areaIndicationLsaRecv",
                                    "received");
  ...
  if (use_json)
    json_object_object_add (
        json_areas, inet_ntop (AF_INET, &area->area_id, buf, sizeof (buf)),
        json_area);
  else
    vty_out (vty, "\n");
}
...
static int
show_ip_ospf_common (struct vty *vty, struct ospf *ospf, json_object *json,
                     uint8_t use_vrf)
{
  ...
  if (json)
    {
      json_object_string_addf (json_vrf, "routerId", "%pI4", &ospf->router_id);
    }
  else
    {
      ...
      if (json)
        {
          long time_store;
          time_store =
              monotime_until (&ospf->t_deferred_shutdown->u.sands, NULL) /
              1000LL;
          json_object_int_add (json_vrf, "deferredShutdownMsecs", time_store);
        }
      else
        {
          vty_out (vty, " Deferred shutdown in progress, %s remaining\n",
                   ospf_timer_dump (ospf->t_deferred_shutdown, timebuf,
                                    sizeof (timebuf)));
        }
    }
  ...
  if (json)
    {
      ...
      if (CHECK_FLAG (ospf->config, OSPF_RFC1583_COMPATIBLE))
        {
          json_object_boolean_true_add (json_vrf, "rfc1583Compatibility");
        }
    }
  else
    {
  ...
  if (ospf->stub_router_startup_time != OSPF_STUB_ROUTER_UNCONFIGURED ||
      ospf->stub_router_shutdown_time != OSPF_STUB_ROUTER_UNCONFIGURED)
    {
      if (json)
        {
          json_object_boolean_true_add (json_vrf, "stubAdvertisement");
          if (ospf->stub_router_startup_time != OSPF_STUB_ROUTER_UNCONFIGURED)
            json_object_int_add (json_vrf, "postStartEnabledSecs",
                                 ospf->stub_router_startup_time);
          if (ospf->stub_router_shutdown_time != OSPF_STUB_ROUTER_UNCONFIGURED)
            json_object_int_add (json_vrf, "preShutdownEnabledSecs",
  ...
  if (json)
    {
      json_object_int_add (json_vrf, "spfScheduleDelayMsecs", ospf->spf_delay);
      json_object_int_add (json_vrf, "holdtimeMinMsecs", ospf->spf_holdtime);
  ...
  else
    {
      vty_out (vty,
               ...
               ospf->spf_delay, ospf->spf_holdtime, ospf->spf_max_holdtime,
               ospf->spf_hold_multiplier);
    }
      ...
      if (ospf->ts_spf.tv_sec || ospf->ts_spf.tv_usec)
        {
          ...
          json_object_int_add (json_vrf, "spfLastExecutedMsecs", time_store);

          time_store = (1000 * ospf->ts_spf_duration.tv_sec) +
                       (ospf->ts_spf_duration.tv_usec / 1000);
          json_object_int_add (json_vrf, "spfLastDurationMsecs", time_store);
        }
      else
      ...
      if (ospf->ts_spf.tv_sec || ospf->ts_spf.tv_usec)
        {
          ...
          vty_out (vty, " Last SPF duration %s\n",
                   ospf_timeval_dump (&ospf->ts_spf_duration, timebuf,
                                      sizeof (timebuf)));
        }
      else
  ...
  if (json)
    {
      if (ospf->t_spf_calc)
        {
          long time_store;
          time_store =
              monotime_until (&ospf->t_spf_calc->u.sands, NULL) / 1000LL;
          json_object_int_add (json_vrf, "spfTimerDueInMsecs", time_store);
        }
      ...
      json_object_int_add (json_vrf, "writeMultiplier", ospf->write_oi_count);
      /* Show refresh parameters. */
      json_object_int_add (json_vrf, "refreshTimerMsecs",
                           ospf->lsa_refresh_interval * 1000);

      /* show max multipath */
      json_object_int_add (json_vrf, "maximumPaths", ospf->max_multipath);
  ...
  if (ospf->fr_configured)
    {
      if (json)
        json_object_string_add (json_vrf, "floodReduction", "configured");
      else
        vty_out (vty, " Flood Reduction is configured.\n");
    }
  ...
  if (CHECK_FLAG (ospf->flags, OSPF_FLAG_ABR))
    {
      if (json)
        json_object_string_add (json_vrf, "abrType",
  ...
  if (CHECK_FLAG (ospf->flags, OSPF_FLAG_ASBR))
    {
      if (json)
        json_object_string_add (json_vrf, "asbrRouter",
  ...
  if (json)
    {
      json_object_int_add (json_vrf, "lsaExternalCounter",
  ...
  if (json)
    {
      json_object_int_add (json_vrf, "lsaAsopaqueCounter",
  ...
  if (CHECK_FLAG (ospf->config, OSPF_LOG_ADJACENCY_CHANGES))
    {
      if (CHECK_FLAG (ospf->config, OSPF_LOG_ADJACENCY_DETAIL))
        ...
        {
          if (json)
            json_object_boolean_true_add (json_vrf, "adjacencyChangesLogged");
          else
      ...
      if (use_vrf)
        {
          ...
          json_object_object_add (json, ospf_get_name (ospf), json_vrf);
        }
      else
        {
          ...
          for (ALL_LIST_ELEMENTS_RO (om->ospf, node, ospf))
            {
              if (! ospf->oi_running)
                continue;
              ospf_output = true;
              ret = show_ip_ospf_common (vty, ospf, json, use_vrf);
            }
          if (uj)
            vty_json (vty, json);
          else if (! ospf_output)
...
static void
ospf_interface_auth_show (struct vty *vty, struct ospf_interface *oi,
                          json_object *json, bool use_json)
{
  ...
  switch (auth_type)
    ...
    case OSPF_AUTH_NULL:
      if (use_json)
        json_object_string_add (json, "authentication", "authenticationNone");
      else
    ...
    case OSPF_AUTH_SIMPLE:
      {
        if (use_json)
          ...
          vty_out (vty, "  Simple password authentication enabled\n");
        break;
      }
        ...
        if (ckey)
          {
            if (use_json)
              ...
              {
                vty_out (vty, "  Cryptographic authentication enabled\n");
  ...
  for (rn = route_top (IF_OIFS (ifp)); rn; rn = route_next (rn))
    {
      ...
      if (CHECK_FLAG (oi->connected->flags, ZEBRA_IFA_UNNUMBERED))
      ...
      else
        {
          ...
          if (use_json)
            {
              json_object_string_addf (json_interface_sub, "ipAddress", "%pI4",
                                       &oi->address->u.prefix4);
              json_object_int_add (json_interface_sub, "ipAddressPrefixlen",
                                   oi->address->prefixlen);
            }
          else
            vty_out (vty, "  Internet Address %pFX,", oi->address);
          ...
          if (use_json)
            {
              ...
              if (oi->type == OSPF_IFTYPE_VIRTUALLINK)
                json_object_string_addf (json_interface_sub, "vlinkPeer",
                                         "%pI4", &dest);
              else
                json_object_string_addf (json_interface_sub, "localIfUsed",
                                         "%pI4", &dest);
            }
          else
            vty_out (vty, " %s %pI4,", dstr, &dest);
        }
      if (use_json)
        {
          ...
          if (OSPF_IF_PARAM (oi, mtu_ignore))
            json_object_boolean_true_add (json_interface_sub,
                                          "mtuMismatchDetect");
          json_object_string_addf (json_interface_sub, "routerId", "%pI4",
                                   &ospf->router_id);
          json_object_string_add (json_interface_sub, "networkType",
                                  ospf_network_type_str[oi->type]);
          json_object_int_add (json_interface_sub, "cost", oi->output_cost);
          json_object_int_add (json_interface_sub, "transmitDelaySecs",
                               OSPF_IF_PARAM (oi, transmit_delay));
          json_object_string_add (
              json_interface_sub, "state",
      ...
      else
        {
          ...
          vty_out (vty, "  Router ID %pI4, Network Type %s, Cost: %d\n",
                   &ospf->router_id, ospf_network_type_str[oi->type],
                   oi->output_cost);
      ...
      else
        {
          ...
          if (nbr)
            {
              if (use_json)
                {
                  json_object_string_addf (json_interface_sub, "drId", "%pI4",
                                           &nbr->router_id);
                  json_object_string_addf (json_interface_sub, "drAddress",
                                           "%pI4", &nbr->address.u.prefix4);
                }
              else
                {
                  vty_out (vty, "  Designated Router (ID) %pI4",
                           &nbr->router_id);
                  vty_out (vty, " Interface Address %pFX\n", &nbr->address);
                }
            }
          nbr = NULL;
          ...
          if (nbr == NULL)
          ...
          else
            {
              if (use_json)
                {
                  json_object_string_addf (json_interface_sub, "bdrId", "%pI4",
                                           &nbr->router_id);
                  json_object_string_addf (json_interface_sub, "bdrAddress",
                                           "%pI4", &nbr->address.u.prefix4);
                }
              else
                {
                  vty_out (vty, "  Backup Designated Router (ID) %pI4,",
                           &nbr->router_id);
      ...
      if (oi->params && ntohl (oi->params->network_lsa_seqnum) !=
        ...
        {
          if (use_json)
            json_object_int_add (json_interface_sub, "networkLsaSequence",
              ...
              OI_MEMBER_CHECK (oi, MEMBER_DROUTERS))
            {
              ...
              if (OI_MEMBER_CHECK (oi, MEMBER_DROUTERS))
                json_object_boolean_true_add (
                    json_interface_sub, "mcastMemberOspfDesignatedRouters");
            }
        }
      else
        {
      ...
      if (use_json)
        {
          if (OSPF_IF_PARAM (oi, fast_hello) == 0)
            json_object_int_add (json_interface_sub, "timerMsecs",
          ...
          json_object_int_add (json_interface_sub, "timerWaitSecs",
                               OSPF_IF_PARAM (oi, v_wait));
          json_object_int_add (json_interface_sub, "timerRetransmitSecs",
          ...
          if (use_json)
            {
              ...
              if (oi->t_hello)
                time_store =
                    monotime_until (&oi->t_hello->u.sands, NULL) / 1000LL;
              json_object_int_add (json_interface_sub, "timerHelloInMsecs",
                                   time_store);
            }
          else
      ...
      if (use_json)
        {
          json_object_int_add (json_interface_sub, "nbrCount",
                               ospf_nbr_count (oi, 0));
          json_object_int_add (json_interface_sub, "nbrAdjacentCount",
      ...
      params = IF_DEF_PARAMS (ifp);
      if (params && OSPF_IF_PARAM_CONFIGURED (params, v_gr_hello_delay))
        {
          if (use_json)
            {
              json_object_int_add (json_interface_sub, "grHelloDelaySecs",
      ...
      if (oi->type == OSPF_IFTYPE_POINTOMULTIPOINT)
        {
          if (use_json)
            json_object_boolean_add (json_interface_sub, "p2mpDelayReflood",
...
static int
show_ip_ospf_interface_common (struct vty *vty, struct ospf *ospf,
          ...
          if (ospf_oi_count (ifp))
            {
              ...
              show_ip_ospf_interface_sub (vty, ospf, ifp, json_interface_sub,
                                          use_json);

              if (use_json)
                {
                  json_object_object_add (json_interface, ifp->name,
                                          json_interface_sub);
                }
            }
        }
      if (use_json)
        json_object_object_add (json_vrf, "interfaces", json_interface);
    }
  else
    {
...
static void
show_ip_ospf_interface_traffic_sub (struct vty *vty, struct ospf_interface *oi,
                                    json_object *json_interface_sub,
                                    bool use_json)
{
  if (use_json)
    {
      json_object_int_add (json_interface_sub, "ifIndex", oi->ifp->ifindex);
      json_object_int_add (json_interface_sub, "helloIn", oi->hello_in);
      json_object_int_add (json_interface_sub, "helloOut", oi->hello_out);
      json_object_int_add (json_interface_sub, "dbDescIn", oi->db_desc_in);
      json_object_int_add (json_interface_sub, "dbDescOut", oi->db_desc_out);
      json_object_int_add (json_interface_sub, "lsReqIn", oi->ls_req_in);
      json_object_int_add (json_interface_sub, "lsReqOut", oi->ls_req_out);
      json_object_int_add (json_interface_sub, "lsUpdIn", oi->ls_upd_in);
      json_object_int_add (json_interface_sub, "lsUpdOut", oi->ls_upd_out);
      json_object_int_add (json_interface_sub, "lsAckIn", oi->ls_ack_in);
      json_object_int_add (json_interface_sub, "lsAckOut", oi->ls_ack_out);
    ...
    {
      vty_out (
          vty, "%-10s %8u/%-8u %7u/%-7u %7u/%-7u %7u/%-7u %7u/%-7u %12lu\n",
          oi->ifp->name, oi->hello_in, oi->hello_out, oi->db_desc_in,
          oi->db_desc_out, oi->ls_req_in, oi->ls_req_out, oi->ls_upd_in,
          oi->ls_upd_out, oi->ls_ack_in, oi->ls_ack_out, listcount (oi->obuf));
    }
}

/* OSPFv2 Packet Counters */
static int
show_ip_ospf_interface_traffic_common (struct vty *vty, struct ospf *ospf,
                                       char *intf_name, json_object *json,
                                       int display_once, uint8_t use_vrf,
                                       bool use_json)
{
  struct vrf *vrf = NULL;
  struct interface *ifp = NULL;
          ...
          for (rn = route_top (IF_OIFS (ifp)); rn; rn = route_next (rn))
            {
              ...
              show_ip_ospf_interface_traffic_sub (vty, oi, json_interface_sub,
                                                  use_json);
              if (use_json)
                {
  ...
  if (vrf_name)
    {
      ...
      ret = show_ip_ospf_interface_common (vty, ospf, intf_name, use_vrf, json,
                                           uj);
    }
  else
    {
  ...
  if (vrf_name)
    {
      ...
      ret = show_ip_ospf_interface_traffic_common (vty, ospf, intf_name, json,
...
static void
show_ip_ospf_neighbour_brief (struct vty *vty, struct ospf_neighbor *nbr,
                              ...
                              json_object *json, bool use_json)
{
  ...
  if (nbr->ts_last_progress.tv_sec || nbr->ts_last_progress.tv_usec)
    time_val = monotime_since (&nbr->ts_last_progress, &res) / 1000LL;
  ...
  if (use_json)
    {
      ...
      if (nbr->state == NSM_Attempt && nbr->router_id.s_addr == INADDR_ANY)
        strlcpy (neigh_str, "neighbor", sizeof (neigh_str));
      else
        inet_ntop (AF_INET, &nbr->router_id, neigh_str, sizeof (neigh_str));
      ...
      if (! json_neigh_array)
        {
          ...
          json_object_object_add (json, neigh_str, json_neigh_array);
        }
      ...
      json_object_int_add (json_neighbor, "nbrPriority", nbr->priority);
      ...
      json_object_string_add (
          json_neighbor, "role",
          lookup_msg (ospf_ism_state_msg, ospf_nbr_ism_state (nbr), NULL));
      if (nbr->t_inactivity)
        {
          long time_store;

          time_store =
              monotime_until (&nbr->t_inactivity->u.sands, NULL) / 1000LL;
          json_object_int_add (json_neighbor, "upTimeInMsec", time_val);
          json_object_int_add (json_neighbor, "routerDeadIntervalTimerDueMsec",
                               time_store);
          json_object_string_add (
        ...
        {
          json_object_string_add (json_neighbor, "deadTimeMsecs", "inactive");
          json_object_string_add (
              json_neighbor, "routerDeadIntervalTimerDueMsec", "inactive");
        }
      json_object_string_addf (json_neighbor, "ifaceAddress", "%pI4",
                               &nbr->src);
      json_object_string_add (json_neighbor, "ifaceName", IF_NAME (nbr->oi));
      json_object_int_add (json_neighbor, "linkStateRetransmissionListCounter",
                           ospf_ls_retransmit_count (nbr));
      json_object_int_add (json_neighbor, "linkStateRequestListCounter",
  ...
  else
    {
      ...
      if (nbr->state == NSM_Attempt && nbr->router_id.s_addr == INADDR_ANY)
        ...
        vty_out (vty, "%-15pI4 %3d %-15s ", &nbr->router_id, nbr->priority,
                 msgbuf);
      ...
      vty_out (vty, "%-32s %5ld %5ld %5d\n", IF_NAME (nbr->oi),
               ospf_ls_retransmit_count (nbr), ospf_ls_request_count (nbr),
               ospf_db_summary_count (nbr));
    }
}
  ...
  for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
    {
      ...
      show_ip_ospf_neighbour_brief (vty, nbr, prev_nbr, json, use_json);
    }
}
       ...
       JSON_STR)
{
          ...
          for (ALL_LIST_ELEMENTS_RO (om->ospf, node, ospf))
            {
              if (! ospf->oi_running)
                continue;
              ret =
                  show_ip_ospf_neighbor_common (vty, ospf, json, uj, use_vrf);
            }
  ...
  if (ospf)
    {
      ret = show_ip_ospf_neighbor_common (vty, ospf, json, uj, use_vrf);
                                  ...
                                  uint8_t use_vrf)
{
  ...
  if (ospf->instance)
    {
      if (use_json)
        json_object_int_add (json_vrf, "ospfInstance", ospf->instance);
      else
        vty_out (vty, "\nOSPF Instance: %d\n\n", ospf->instance);
    }
              ...
              if (use_json)
                {
                  ...
                  json_object_int_add (json_neighbor_sub, "nbrNbmaPriority",
                                       nbr_nbma->priority);
                  json_object_boolean_true_add (json_neighbor_sub,
                                                "nbrNbmaDown");
                  json_object_string_add (json_neighbor_sub,
                  ...
                  json_object_object_add (
                      json_vrf,
                      inet_ntop (AF_INET, &nbr_nbma->addr, buf, sizeof (buf)),
                      json_neighbor_sub);
                }
              else
                {
                  ...
                  vty_out (vty, "%-32pI4 %-20s %5d %5d %5d\n", &nbr_nbma->addr,
                           IF_NAME (oi), 0, 0, 0);
                }
            }
        }
       ...
       JSON_STR)
{
  ...
  if (ospf)
    {
      ret = show_ip_ospf_neighbor_all_common (vty, ospf, json, uj, use_vrf);
      if (uj)
        {
          vty_out (
              vty, "%s\n",
...
static int
show_ip_ospf_neighbor_int_common (struct vty *vty, struct ospf *ospf,
      ...
      if (use_json)
        {
          long time_store;
          time_store =
              monotime_until (&nbr_nbma->t_poll->u.sands, NULL) / 1000LL;
          json_object_int_add (json_sub, "pollIntervalTimerDueMsec",
                               time_store);
        }
      else
        vty_out (
            vty, "    Poll timer due in %s\n",
                                  ...
                                  json_object *json, bool use_json)
{
  ...
  if (use_json)
  ...
  else
    {
      ...
      if (nbr->state == NSM_Attempt && nbr->router_id.s_addr == INADDR_ANY)
        ...
        vty_out (vty, " Neighbor %pI4,", &nbr->router_id);
    }
  ...
  if (use_json)
    {
      ...
      if (oi->address)
        json_object_string_addf (json_neigh, "localIfaceAddress", "%pI4",
                                 &oi->address->u.prefix4);
    }
  else
    {
  ...
  if (use_json)
    ...
    {
      vty_out (
          vty, "    Neighbor priority is %d, State is %s, Role is %s,",
          nbr->priority, neigh_state,
          lookup_msg (ospf_ism_state_msg, ospf_nbr_ism_state (nbr), NULL));
    }
  /* Show state changes. */
  if (use_json)
    json_object_int_add (json_neigh, "stateChangeCounter", nbr->state_change);
  else
    vty_out (vty, " %d state changes\n", nbr->state_change);
  ...
  if (nbr->ts_last_progress.tv_sec || nbr->ts_last_progress.tv_usec)
    {
      ...
      if (use_json)
        {
          json_object_int_add (json_neigh, "lastPrgrsvChangeMsec", time_store);
        }
      else
        {
  ...
  if (nbr->ts_last_regress.tv_sec || nbr->ts_last_regress.tv_usec)
    {
      ...
      if (use_json)
        {
          json_object_int_add (json_neigh, "lastRegressiveChangeMsec",
                               time_store);
          if (nbr->last_regress_str)
            json_object_string_add (json_neigh, "lastRegressiveChangeReason",
  ...
  if (DR (oi).s_addr == INADDR_ANY)
  ...
  else
    {
      ...
      if (nbr_dr)
        {
          if (use_json)
            json_object_string_addf (json_neigh, "routerDesignatedId", "%pI4",
                                     &nbr_dr->router_id);
          else
            vty_out (vty, "    DR is %pI4,", &nbr_dr->router_id);
        }
    }
      ...
      if (nbr->t_inactivity)
        {
          ...
          time_store =
              monotime_until (&nbr->t_inactivity->u.sands, NULL) / 1000LL;
          json_object_int_add (json_neigh, "routerDeadIntervalTimerDueMsec",
                               time_store);
        }
      else
        json_object_int_add (json_neigh, "routerDeadIntervalTimerDueMsec", -1);
    }
  else
  ...
  if (use_json)
    json_object_int_add (json_neigh, "linkStateRetransmissionListCounter",
                         ospf_ls_retransmit_count (nbr));
  else
  ...
  if (use_json)
    {
      if (nbr->t_inactivity != NULL)
        json_object_string_add (json_neigh, "threadInactivityTimer", "on");
    }
  else
  ...
  if (use_json)
    {
      if (nbr->t_db_desc != NULL)
        json_object_string_add (
            json_neigh, "threadDatabaseDescriptionRetransmission", "on");
    }
  else
  ...
  if (! use_json)
      ...
      if (OSPF_GR_IS_ACTIVE_HELPER (nbr))
        {
          ...
          vty_out (
              vty, "      Graceful Restart reason: %s.\n",
              ospf_restart_reason2str (nbr->gr_helper_info.gr_restart_reason));
        }
      else
        {
          vty_out (vty, "      Graceful Restart HELPER Status : None\n");
        }

      if (nbr->gr_helper_info.rejected_reason != OSPF_HELPER_REJECTED_NONE)
        vty_out (
            vty, "      Helper rejected reason: %s.\n",
            ospf_rejected_reason2str (nbr->gr_helper_info.rejected_reason));

      if (nbr->gr_helper_info.helper_exit_reason != OSPF_GR_HELPER_EXIT_NONE)
        vty_out (
            vty, "      Last helper exit reason: %s.\n\n",
            ospf_exit_reason2str (nbr->gr_helper_info.helper_exit_reason));
      else
        vty_out (vty, "\n");
    }
  else
    {
      json_object_string_add (json_neigh, "grHelperStatus",
                              OSPF_GR_IS_ACTIVE_HELPER (nbr) ? "Inprogress"
                                                             : "None");
      if (OSPF_GR_IS_ACTIVE_HELPER (nbr))
        {
          json_object_int_add (json_neigh, "graceInterval",
                               nbr->gr_helper_info.recvd_grace_period);
          json_object_string_add (
...
show_ip_ospf_neighbor_id_common (struct vty *vty, struct ospf *ospf,
                                 struct in_addr *router_id, bool use_json,
                                 uint8_t use_vrf, bool is_detail,
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->oiflist, node, oi))
    {
      ...
      if (is_detail)
        show_ip_ospf_neighbor_detail_sub (vty, oi, nbr, NULL, json, use_json);
      else
        show_ip_ospf_neighbour_brief (vty, nbr, NULL, json, use_json);
    }

  if (json_vrf && use_json)
    {
      json_object_object_add (
          json_vrf, (ospf->vrf_id == VRF_DEFAULT) ? "default" : ospf->name,
          json);
      return CMD_SUCCESS;
    }
  ...
  if (vrf_name && ! strmatch (vrf_name, "all"))
    {
      ...
      ret = show_ip_ospf_neighbor_id_common (vty, ospf, &router_id, ! ! json,
                                             0, ! ! detail, NULL);
    }
  else
    {
      ...
      for (ALL_LIST_ELEMENTS_RO (om->ospf, node, ospf))
        {
          ...
          ret = show_ip_ospf_neighbor_id_common (
              vty, ospf, &router_id, ! ! json, 0, ! ! detail, json_vrf);
        }
      if (json)
          ...
          if (nbr != oi->nbr_self)
            {
              if (nbr->state != NSM_Down)
                {
                  show_ip_ospf_neighbor_detail_sub (vty, oi, nbr, prev_nbr,
       ...
       JSON_STR)
{
  ...
  if (ospf)
    ret = show_ip_ospf_neighbor_detail_common (vty, ospf, json_vrf, ! ! json,
                                               use_vrf);
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->oiflist, node, oi))
    {
      ...
      for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
        {
          ...
          if (nbr != oi->nbr_self)
            if (nbr->state != NSM_Down)
              show_ip_ospf_neighbor_detail_sub (vty, oi, rn->info, prev_nbr,
                                                json_vrf, use_json);
          prev_nbr = nbr;
        }
      ...
      for (ALL_LIST_ELEMENTS_RO (oi->nbr_nbma, nd, nbr_nbma))
        {
          if (nbr_nbma->nbr == NULL || nbr_nbma->nbr->state == NSM_Down)
            show_ip_ospf_nbr_nbma_detail_sub (vty, oi, nbr_nbma, use_json,
                                              json_vrf);
        }
    }
       ...
       JSON_STR)
{
  ...
  if (ospf)
    {
      ret = show_ip_ospf_neighbor_detail_all_common (vty, ospf, json, uj,
                                                     use_vrf);
      if (uj)
        {
          vty_out (
      ...
      for (nrn = route_top (oi->nbrs); nrn; nrn = route_next (nrn))
        {
          ...
          show_ip_ospf_neighbor_detail_sub (vty, oi, nbr, NULL, json,
                                            use_json);
        }
    }
       ...
       JSON_STR)
{
  ...
  return show_ip_ospf_neighbor_int_detail_common (vty, ospf, ifname, ! ! json,
                                                  NULL);
}
      ...
      if (! json_lsa)
        {
          ...
          vty_out (vty, "%-15pI4 %4d 0x%08lx 0x%04x", &lsa->data->adv_router,
                   LS_AGE (lsa), (unsigned long) ntohl (lsa->data->ls_seqnum),
      ...
      else
        {
          ...
          snprintf (checksum, sizeof (checksum), "%x",
                    ntohs (lsa->data->checksum));
          json_object_string_addf (json_lsa, "lsId", "%pI4", &lsa->data->id);
          json_object_string_addf (json_lsa, "advertisedRouter", "%pI4",
                                   &lsa->data->adv_router);
          json_object_int_add (json_lsa, "lsaAge", LS_AGE (lsa));
          json_object_string_add (json_lsa, "sequenceNumber", seqnum);
      ...
      switch (lsa->data->type)
        ...
        case OSPF_ROUTER_LSA:
          ...
          if (! json_lsa)
            ...
            json_object_int_add (json_lsa, "numOfRouterLinks",
                                 ntohs (rl->links));
          break;
        case OSPF_SUMMARY_LSA:
          ...
          if (! json_lsa)
            ...
            {
              json_object_string_addf (json_lsa, "summaryAddress", "%pFX", &p);
            }
          break;
        ...
        case OSPF_AS_NSSA_LSA:
          ...
          if (! json_lsa)
            vty_out (vty, " %s %pFX [0x%lx]",
                     IS_EXTERNAL_METRIC (asel->e[0].tos) ? "E2" : "E1", &p,
                     (unsigned long) ntohl (asel->e[0].route_tag));
          else
            {
              json_object_string_add (
                  json_lsa, "metricType",
                  IS_EXTERNAL_METRIC (asel->e[0].tos) ? "E2" : "E1");
              json_object_string_addf (json_lsa, "route", "%pFX", &p);
              json_object_int_add (
                  json_lsa, "tag",
                  (unsigned long) ntohl (asel->e[0].route_tag));
            }
  ...
  if (! json)
    {
      if (IS_LSA_SELF (lsa))
        vty_out (vty, "  LS age: %d%s\n", LS_AGE (lsa),
                 CHECK_FLAG (lsa->data->ls_age, DO_NOT_AGE) ? "(S-DNA)" : "");
      else
        vty_out (vty, "  LS age: %d%s\n", LS_AGE (lsa),
                 CHECK_FLAG (lsa->data->ls_age, DO_NOT_AGE) ? "(DNA)" : "");
  ...
  else
    {
      ...
      if (lsa->flags & OSPF_LSA_LOCAL_XLT)
        json_object_boolean_true_add (json, "translatedFromType7");
          ...
          if (rlsa->flags)
            {
              if (IS_ROUTER_LSA_BORDER (rlsa))
                json_object_boolean_true_add (json, "abr");
              if (IS_ROUTER_LSA_EXTERNAL (rlsa))
                json_object_boolean_true_add (json, "asbr");
              if (IS_ROUTER_LSA_VIRTUAL (rlsa))
                json_object_boolean_true_add (json, "vlEndpoint");
              if (IS_ROUTER_LSA_SHORTCUT (rlsa))
                json_object_boolean_true_add (json, "shortcut");
            }
        }
      ...
      json_object_string_add (
          json, "lsaType",
          lookup_msg (ospf_lsa_type_msg, lsa->data->type, NULL));
      json_object_string_addf (json, "linkStateId", "%pI4", &lsa->data->id);
...
static void
show_ip_ospf_database_router_links (struct vty *vty, struct router_lsa *rl,
                                    json_object *json)
{
  int len, type;
  unsigned short i;
      ...
      if (json)
        {
          ...
          json_object_string_add (json_link, "linkType", link_type_desc[type]);
          json_object_string_add (
              json_link, link_id_desc_json[type],
              inet_ntop (AF_INET, &rl->link[i].link_id, buf, sizeof (buf)));
          json_object_string_add (
              json_link, link_data_desc_json[type],
              inet_ntop (AF_INET, &rl->link[i].link_data, buf, sizeof (buf)));
          json_object_int_add (json_link, "numOfTosMetrics", metric);
  ...
  if (lsa != NULL)
    {
      ...
      if (! json)
        ...
        json_object_int_add (json, "numOfLinks", ntohs (rl->links));
  ...
  if (lsa != NULL)
    {
      ...
      addr = &nl->routers[0];
      for (i = 0; length > 0 && addr; length -= 4, addr = &nl->routers[++i])
        if (! json)
          {
        ...
        else
          {
            ...
            json_object_object_add (
                json_attached_rt, inet_ntop (AF_INET, addr, buf, sizeof (buf)),
                json_router);
          }
    }

  if (json)
    json_object_object_add (json, "attchedRouters", json_attached_rt);
  ...
  if (lsa != NULL)
    {
      ...
      if (! json)
      ...
      else
        {
          ...
          json_object_int_add (json, "tos0Metric", GET_METRIC (sl->metric));
        }
    }
  ...
  if (lsa != NULL)
    {
      ...
      if (! json)
      ...
      else
        {
          ...
          json_object_string_add (json, "metricType",
                                  IS_EXTERNAL_METRIC (al->e[0].tos)
          ...
          json_object_string_addf (json, "forwardAddress", "%pI4",
                                   &(al->e[0].fwd_addr));
          json_object_int_add (json, "externalRouteTag",
                               (route_tag_t) ntohl (al->e[0].route_tag));
        }
    }
  ...
  if (lsa != NULL)
    {
      ...
      if (! json)
      ...
      else
        {
          ...
          json_object_int_add (json, "tos", tos);
          json_object_int_add (json, "metric", GET_METRIC (al->e[0].metric));
          json_object_string_addf (json, "nssaForwardAddress", "%pI4",
                                   &al->e[0].fwd_addr);
          json_object_int_add (json, "externalRouteTag",
                               (route_tag_t) ntohl (al->e[0].route_tag));
        }
...
static int
show_func_dummy (struct vty *vty, struct ospf_lsa *lsa, json_object *json)
{
  return 0;
}
            ...
            if (show_function[lsa->data->type] != NULL)
              {
                ...
                show_function[lsa->data->type](vty, lsa, json_lsa);
              }
          }
  ...
  switch (type)
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      show_lsa_detail_proc (vty, AS_LSDB (ospf, type), id, adv_router,
                            json_lsa_array);
      if (json)
        json_object_object_add (json, show_database_desc_json[type],
                                json_lsa_array);
      ...
      for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
        {
          if (! json)
            {
              vty_out (vty, "\n                %s (Area %s)\n\n",
                       show_database_desc[type], ospf_area_desc_string (area));
            }
          else
            {
              json_lsa_array = json_object_new_array ();
      ...
      if (json)
        {
          ...
          json_object_object_add (json_lsa_type, "areas", json_areas);
          json_object_object_add (json, show_database_desc_json[type],
                                  json_lsa_type);
        }
      break;
...
show_lsa_detail_adv_router_proc (struct vty *vty, struct route_table *rt,
                                 struct in_addr *adv_router, json_object *json)
{
  struct route_node *rn;
  struct ospf_lsa *lsa;
  json_object *json_lsa = NULL;
          ...
          if (ospf_lsdb_count_self (area->lsdb, type) > 0 ||
              (! self && ospf_lsdb_count (area->lsdb, type) > 0))
            {
              ...
              if (! json)
                ...
                {
                  json_lsa_array = json_object_new_array ();
                  json_object_object_add (json_area,
              ...
              LSDB_LOOP (AREA_LSDB (area, type), rn, lsa)
                {
                  if (json)
                    {
                      json_lsa = json_object_new_object ();
                      json_object_array_add (json_lsa_array, json_lsa);
                    }

                  count += show_lsa_summary (vty, lsa, self, json_lsa);
                }
      ...
      if (ospf_lsdb_count_self (ospf->lsdb, type) ||
          (! self && ospf_lsdb_count (ospf->lsdb, type)))
        {
          ...
          if (! json)
            ...
            json_object_int_add (json, show_database_desc_count_json[type],
                                 count);
        }
    }
      ...
      if ((lsa = rn->info) != NULL)
        {
          if (! json)
            ...
            {
              json_lsa = json_object_new_object ();
              json_object_int_add (json_lsa, "linkType", lsa->data->type);
              json_object_string_addf (json_lsa, "linkStateId", "%pI4",
                                       &lsa->data->id);
              json_object_string_addf (json_lsa, "advertisingRouter", "%pI4",
                                       &lsa->data->adv_router);
              json_object_int_add (json_lsa, "lsaLockCount", lsa->lock);
              json_object_object_add (
                  json_maxage,
                  inet_ntop (AF_INET, &lsa->data->id, buf, sizeof (buf)),
                  json_lsa);
            }
        }
    }
          ...
          if (use_vrf)
            {
              if (ospf->vrf_id == VRF_DEFAULT)
                ...
                json_object_object_add (json, ospf->name, json_vrf);
            }
        }
      return CMD_SUCCESS;
    }
  ...
  if (! type_name)
    {
      if (detail)
        {
          for (int i = OSPF_ROUTER_LSA; i <= OSPF_OPAQUE_AS_LSA; i++)
            {
              switch (i)
                {
              ...
              if (adv_router && ! lsid)
                show_lsa_detail_adv_router (vty, ospf, i, adv_router,
                                            json_vrf);
              else
                show_lsa_detail (vty, ospf, i, lsid, adv_router, json_vrf);
            }
        }
      else
       ...
       JSON_STR)
{
  ...
  if (vrf_name && strmatch (vrf_name, "all"))
  ...
  else
    {
      ...
      ret = (show_ip_ospf_database_common (vty, ospf, ! ! maxage, ! ! selforig,
                                           ! ! detail, type_name, lsid_p,
...
ospf_vty_dead_interval_set (struct vty *vty, const char *interval_str,
                            const char *nbr_str, const char *fast_hello_str)
{
  VTY_DECLVAR_CONTEXT (interface, ifp);
  uint32_t seconds;
  uint8_t hellomult;
       ...
       "Address of interface\n")
{
  int idx = 0;
  char *interval =
      argv_find (argv, argc, "(1-65535)", &idx) ? argv[idx]->arg : NULL;
  char *ifaddr =
       ...
       "Address of interface\n")
{
  ...
  int idx_ipv4 = 6;
  if (argc == 7)
    return ospf_vty_dead_interval_set (vty, NULL, argv[idx_ipv4]->arg,
                                       argv[idx_number]->arg);
  else
       ...
       "Pointer to route-map entries\n")
{
  ...
  if (update)
    return ospf_redistribute_update (ospf, red, source, 0, type, metric);
  else
    return ospf_redistribute_set (ospf, red, source, 0, type, metric);
}
       ...
       "Pointer to route-map entries\n")
{
  ...
  if (update)
    return ospf_redistribute_update (ospf, red, source, instance, type,
                                     metric);
  else
    return ospf_redistribute_set (ospf, red, source, instance, type, metric);
}
...
DEFUN (ospf_distribute_list_out,
       ...
       "distribute-list ACCESSLIST4_NAME out "
       FRR_REDIST_STR_OSPFD,
...
DEFUN (no_ospf_distribute_list_out,
       ...
       "no distribute-list ACCESSLIST4_NAME out "
       FRR_REDIST_STR_OSPFD,
       NO_STR
       "Filter networks in routing updates\n"
       "Access-list name\n"
       ...
       "Pointer to route-map entries\n")
{
  ...
  if ((sameRtmap) && (red->dmetric.type == type) &&
      (red->dmetric.value == metric) && (cur_originate == default_originate))
    return CMD_SUCCESS;
       ...
       "Pointer to route-map entries\n")
{
  ...
  return ospf_redistribute_default_set (ospf, DEFAULT_ORIGINATE_NONE, 0, 0);
}
       ...
       "Advertising Router-ID\n")
{
  ...
  if (address_str)
    {
      ospf_gr_helper_support_set_per_routerid (ospf, &address, OSPF_GR_TRUE);
      return CMD_SUCCESS;
    }
       ...
       "Advertising Router-ID\n")
{
  ...
  if (address_str)
    {
      ospf_gr_helper_support_set_per_routerid (ospf, &address, OSPF_GR_FALSE);
      return CMD_SUCCESS;
    }
...
static int
ospf_print_vty_helper_dis_rtr_walkcb (struct hash_bucket *bucket, void *arg)
{
...
static int
ospf_print_json_helper_enabled_rtr_walkcb (struct hash_bucket *bucket,
                                           void *arg)
{
  struct advRtr *rtr = bucket->data;
...
static int
ospf_show_gr_helper_details (struct vty *vty, struct ospf *ospf,
                             uint8_t use_vrf, json_object *json, bool uj,
                             bool detail)
{
  struct listnode *node;
  struct ospf_interface *oi;
  ...
  if (uj)
    {
      json_object_string_add (
          json_vrf, "routerId",
          inet_ntop (AF_INET, &ospf->router_id, buf, sizeof (buf)));
    }
  else
    {
      vty_out (vty, "\n       OSPF Router with ID (%pI4)\n\n",
  ...
  if (! uj)
    {
      ...
      if (ospf->active_restarter_cnt)
        vty_out (vty, " Number of Active neighbours in graceful restart: %d\n",
    ...
    {
      json_object_string_add (json_vrf, "helperSupport",
                              (ospf->is_helper_supported) ? "Enabled"
                                                          : "Disabled");
...
#endif
      json_object_string_add (json_vrf, "restartSupoort",
                              (ospf->only_planned_restart)
                                  ? "Planned Restart only"
                                  : "Planned and Unplanned Restarts");

      json_object_string_add (json_vrf, "restartSupport",
                              (ospf->only_planned_restart)
                                  ? "Planned Restart only"
                                  : "Planned and Unplanned Restarts");
      ...
      if (ospf->last_exit_reason != OSPF_GR_HELPER_EXIT_NONE)
        json_object_string_add (json_vrf, "lastExitReason",
                                ospf_exit_reason2str (ospf->last_exit_reason));
              ...
              if (! uj)
              ...
              else
                {
                  ...
                  json_object_int_add (json_neigh, "recvdGraceInterval",
                                       nbr->gr_helper_info.recvd_grace_period);
                  json_object_int_add (
                      json_neigh, "actualGraceInterval",
                      nbr->gr_helper_info.actual_grace_period);
          ...
          for (ALL_LIST_ELEMENTS_RO (om->ospf, node, ospf))
            {
              ...
              ret = ospf_show_gr_helper_details (vty, ospf, use_vrf, json, uj,
                                                 detail);
            }
...
static void
show_ip_ospf_route_network (struct vty *vty, struct ospf *ospf,
                            struct route_table *rt, json_object *json)
{
  struct route_node *rn;
  struct ospf_route * or ;
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
    {
      ...
      switch (or->path_type)
          ...
          if (or->type == OSPF_DESTINATION_NETWORK)
            {
              if (json)
                {
                  json_object_string_add (json_route, "routeType", "N IA");
                  json_object_int_add (json_route, "cost", or->cost);
                  json_object_string_addf (json_route, "area", "%pI4",
                                           & or->u.std.area_id);
                }
              else
                {
                  vty_out (vty, "N IA %-18s    [%d] area: %pI4\n", buf1,
          ...
          else if (or->type == OSPF_DESTINATION_DISCARD)
            {
              if (json)
                {
                  json_object_string_add (json_route, "routeType", "D IA");
                }
              else
                {
        ...
        case OSPF_PATH_INTRA_AREA:
          if (json)
            {
              json_object_string_add (json_route, "routeType", "N");
              json_object_int_add (json_route, "cost", or->cost);
              json_object_string_addf (json_route, "area", "%pI4",
                                       & or->u.std.area_id);
            }
                      ...
                      if (json)
                        {
                          ...
                          json_object_string_add (
                              json_nexthop, "directlyAttachedTo",
                              ifindex2ifname (path->ifindex, ospf->vrf_id));
                        }
                      else
                        {
                          vty_out (
                              vty, "%24s   directly attached to %s\n", "",
                              ifindex2ifname (path->ifindex, ospf->vrf_id));
                        }
                    }
                  else
...
static void
show_ip_ospf_route_router (struct vty *vty, struct ospf *ospf,
                           struct route_table *rtrs, json_object *json)
{
  struct route_node *rn;
  struct ospf_route * or ;
  ...
  if (! json)
    vty_out (vty, "============ OSPF %s table =============\n",
             ospf->all_rtrs == rtrs ? "reachable routers" : "router routing");
      ...
      if (json)
        {
          json_route = json_object_new_object ();
          json_object_object_add (
              json, inet_ntop (AF_INET, &rn->p.u.prefix4, buf, sizeof (buf)),
              json_route);
          json_object_string_add (json_route, "routeType", "R ");
        }
      else
        {
          vty_out (vty, "R    %-15pI4    ", &rn->p.u.prefix4);
        }
          ...
          if (json)
            {
              ...
              if (or->path_type == OSPF_PATH_INTER_AREA)
                {
                  json_object_boolean_true_add (json_route, "IA");
                  json_object_boolean_true_add (json_route, "ia");
                }
              if (or->u.std.flags & ROUTER_LSA_BORDER)
                json_object_string_add (json_route, "routerType", "abr");
              else if (or->u.std.flags & ROUTER_LSA_EXTERNAL)
                json_object_string_add (json_route, "routerType", "asbr");
            }
          else
            {
...
static void
show_ip_ospf_route_external (struct vty *vty, struct ospf *ospf,
                             struct route_table *rt, json_object *json)
{
  struct route_node *rn;
  ...
  for (rn = route_top (rt); rn; rn = route_next (rn))
    {
      ...
      switch (er->path_type)
        ...
        case OSPF_PATH_TYPE1_EXTERNAL:
          if (json)
            {
              json_object_string_add (json_route, "routeType", "N E1");
              json_object_int_add (json_route, "cost", er->cost);
              json_object_int_add (json_route, "tag", er->u.ext.tag);
            }
          else
            {
        ...
        case OSPF_PATH_TYPE2_EXTERNAL:
          if (json)
            {
              json_object_string_add (json_route, "routeType", "N E2");
              json_object_int_add (json_route, "cost", er->cost);
          ...
          else
            {
              vty_out (vty, "N E2 %-18s    [%d/%d] tag: %" ROUTE_TAG_PRI "\n",
                       buf1, er->cost, er->u.ext.type2_cost, er->u.ext.tag);
            }
          break;
        }
      ...
      if (json)
        {
          json_nexthop_array = json_object_new_array ();
          json_object_object_add (json_route, "nexthops", json_nexthop_array);
        }
...
static int
show_ip_ospf_reachable_routers_common (struct vty *vty, struct ospf *ospf,
                                       uint8_t use_vrf)
{
  if (ospf->instance)
    vty_out (vty, "\nOSPF Instance: %d\n\n", ospf->instance);
       ...
       JSON_STR)
{
  ...
  if (ospf)
    {
      ret = show_ip_ospf_border_routers_common (vty, ospf, use_vrf, json);
      if (uj)
        vty_json (vty, json);
    }
                           ...
                           json_object *json, uint8_t use_vrf)
{
  ...
  show_ip_ospf_route_external (vty, ospf, ospf->old_external_route, json_vrf);
...
static const char *const ospf_abr_type_str[] = { "unknown", "standard", "ibm",
                                                 "cisco", "shortcut" };

static const char *const ospf_shortcut_mode_str[] = { "default", "enable",
                                                      "disable" };
static int
ospf_vty_external_rt_walkcb (struct hash_bucket *bucket, void *arg)
{
...
static int
ospf_json_external_rt_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct external_info *ei = bucket->data;
...
static int
ospf_show_summary_address (struct vty *vty, struct ospf *ospf, uint8_t use_vrf,
        ...
        if (uj)
          {
            ...
            json_object_object_add (json_vrf, buf, json_aggr);
            json_object_string_add (json_aggr, "summaryAddress", buf);
            json_object_string_add (json_aggr, "metricType",
                                    (mtype == EXTERNAL_METRIC_TYPE_1) ? "E1"
                                                                      : "E2");

            json_object_int_add (json_aggr, "metric", mval);
            json_object_int_add (json_aggr, "tag", aggr->tag);
            ...
            if (OSPF_EXTERNAL_RT_COUNT (aggr) && detail)
              {
                hash_walk (aggr->match_extnl_hash,
                           ospf_json_external_rt_walkcb, json_aggr);
              }
          }
        else
          {
            ...
            (mtype == EXTERNAL_METRIC_TYPE_1) ? vty_out (vty, "%-16s", "E1")
                                              : vty_out (vty, "%-16s", "E2");
            ...
            if (OSPF_EXTERNAL_RT_COUNT (aggr) && detail)
              {
                vty_out (vty, "Matched External routes:\n");
                hash_walk (aggr->match_extnl_hash, ospf_vty_external_rt_walkcb,
                           vty);
                vty_out (vty, "\n");
              }
...
static const char *
interface_config_auth_str (struct ospf_if_params *params)
{
      ...
      do
        {
              ...
              if (params->type != ospf_default_iftype (ifp))
                {
                  ...
                  if (params->type == OSPF_IFTYPE_POINTOPOINT &&
                      params->ptp_dmvpn)
                    vty_out (vty, " dmvpn");
                  if (params->type == OSPF_IFTYPE_POINTOMULTIPOINT &&
          ...
          if (OSPF_IF_PARAM_CONFIGURED (params, v_gr_hello_delay) &&
              params->v_gr_hello_delay != OSPF_HELLO_DELAY_DEFAULT)
            vty_out (vty, " ip ospf graceful-restart hello-delay %u\n",
                     params->v_gr_hello_delay);
          ...
          if (OSPF_IF_PARAM_CONFIGURED (params, if_area))
            {
              if (ospf_instance)
                vty_out (vty, " ip ospf %d", ospf_instance);
              else
                vty_out (vty, " ip ospf");
    ...
    if (rn->info)
      {
        ...
        if (n->area_id_fmt == OSPF_AREA_ID_FMT_DOTTEDQUAD)
          inet_ntop (AF_INET, &n->area_id, buf, sizeof (buf));
        else
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      ...
      area_id2str (buf, sizeof (buf), &area->area_id, area->area_id_fmt);
              ...
              if (area->nssa_default_originate.enabled)
                {
                  ...
                  if (area->nssa_default_originate.metric_type !=
                      DEFAULT_METRIC_TYPE)
        ...
        if (rn1->info)
          {
            ...
            vty_out (vty, " area %s range %pFX", buf, &rn1->p);
            ...
            if (CHECK_FLAG (range->flags, OSPF_AREA_RANGE_SUBSTITUTE))
              vty_out (vty, " substitute %pI4/%d", &range->subst_addr,
                       range->subst_masklen);
      ...
      if (vl_data != NULL)
        {
          ...
          if (OSPF_IF_PARAM (oi, v_hello) != OSPF_HELLO_INTERVAL_DEFAULT ||
            ...
            vty_out (
                ...
                buf, &vl_data->vl_peer, OSPF_IF_PARAM (oi, v_hello),
                OSPF_IF_PARAM (oi, retransmit_interval),
                OSPF_IF_PARAM (oi, transmit_delay),
                OSPF_IF_PARAM (oi, v_wait));
          else
            vty_out (vty, " area %s virtual-link %pI4\n", buf,
                     &vl_data->vl_peer);
          /* Auth type */
          auth_str = interface_config_auth_str (IF_DEF_PARAMS (oi->ifp));
          if (auth_str)
            vty_out (vty, " area %s virtual-link %pI4 authentication%s\n", buf,
                     &vl_data->vl_peer, auth_str);
          /* Auth key */
          if (IF_DEF_PARAMS (vl_data->vl_oi->ifp)->auth_simple[0] != '\0')
            vty_out (vty, " area %s virtual-link %pI4 authentication-key %s\n",
                     buf, &vl_data->vl_peer,
                     IF_DEF_PARAMS (vl_data->vl_oi->ifp)->auth_simple);
          /* md5 keys */
          for (ALL_LIST_ELEMENTS_RO (
                   IF_DEF_PARAMS (vl_data->vl_oi->ifp)->auth_crypt, n2, ck))
            vty_out (
                vty,
                " area %s virtual-link %pI4 message-digest-key %d md5 %s\n",
                buf, &vl_data->vl_peer, ck->key_id, ck->auth_key);
        }
    }
...
static int
ospf_cfg_write_helper_dis_rtr_walkcb (struct hash_bucket *bucket, void *arg)
{
  struct advRtr *rtr = bucket->data;
  struct vty *vty = (struct vty *) arg;
...
static void
config_write_ospf_gr (struct vty *vty, struct ospf *ospf)
{
  if (! ospf->gr_info.restart_support)
    return;
...
static int
config_write_ospf_gr_helper (struct vty *vty, struct ospf *ospf)
{
  if (ospf->is_helper_supported)
  ...
  if (OSPF_HELPER_ENABLE_RTR_COUNT (ospf))
    {
      hash_walk (ospf->enable_rtr_list, ospf_cfg_write_helper_dis_rtr_walkcb,
                 vty);
    }
  return 0;
}
...
static int
config_write_ospf_external_aggregator (struct vty *vty, struct ospf *ospf)
{
  struct route_node *rn;
    ...
    if (rn->info)
      {
        ...
        vty_out (vty, " summary-address %pI4/%d", &aggr->p.prefix,
                 aggr->p.prefixlen);
        if (aggr->tag)
      ...
      for (type = 0; type < ZEBRA_ROUTE_MAX; type++)
        if (DISTRIBUTE_NAME (ospf, type))
          vty_out (vty, " distribute-list %s out %s\n",
                   DISTRIBUTE_NAME (ospf, type), zebra_route_string (type));
...
static int
ospf_config_write_one (struct vty *vty, struct ospf *ospf)
{
  int write = 0;
...
ospf_vty_show_init (void)
{
  ...
  install_element (VIEW_NODE, &show_ip_ospf_instance_reachable_routers_cmd);
...
DEFPY_HIDDEN (ospf_lsa_refresh_timer,
...
DEFPY_HIDDEN (ospf_maxage_delay_timer,
  ...
  if (! ospf->fr_configured)
    {
      ...
      OSPF_LOG_DEBUG (IS_DEBUG_OSPF_EVENT, "Flood Reduction: OFF -> ON");
      for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
        {
          if (area)
            {
...
DEFPY (flood_reduction_area,
       flood_reduction_area_cmd,
  ...
  if (! oa->fr_info.configured)
    {
      ...
      OSPF_LOG_DEBUG (IS_DEBUG_OSPF_EVENT,
                      "Flood Reduction area %pI4 : OFF -> ON", &oa->area_id);
...
DEFPY (no_flood_reduction,
       no_flood_reduction_cmd,
       "no flood-reduction",
       NO_STR
       "flood reduction feature\n")
{
  ...
  if (ospf->fr_configured)
    {
      ...
      OSPF_LOG_DEBUG (IS_DEBUG_OSPF_EVENT, "Flood Reduction: ON -> OFF");
      for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
        {
          if (area)
            {
  ...
  if (oa->fr_info.configured)
    {
      ...
      OSPF_LOG_DEBUG (IS_DEBUG_OSPF_EVENT,
                      "Flood Reduction area %pI4 : ON -> OFF", &oa->area_id);
      ospf_area_update_fr_state (oa);
      ospf_refresh_area_self_lsas (oa);
    }
