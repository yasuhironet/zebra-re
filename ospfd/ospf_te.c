...
static void ospf_mpls_te_ism_change (struct ospf_interface *oi,
                                     int old_status);
static void ospf_mpls_te_nsm_change (struct ospf_neighbor *nbr,
                                     int old_status);
...
ospf_mpls_te_init (void)
{
  ...
  rc = ospf_register_opaque_functab (
      ...
      ospf_mpls_te_new_if, ospf_mpls_te_del_if, ospf_mpls_te_ism_change,
      ospf_mpls_te_nsm_change, ospf_mpls_te_config_write_router,
   ...
   */
  rc = ospf_register_opaque_functab (
      OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_INTER_AS_LSA, NULL, NULL, NULL, NULL,
      NULL, NULL, NULL, ospf_mpls_te_show_info, ospf_mpls_te_lsa_inter_as_area,
  ...
  rc = ospf_register_opaque_functab (
      OSPF_OPAQUE_AS_LSA, OPAQUE_TYPE_INTER_AS_LSA, NULL, NULL, NULL, NULL,
      NULL, NULL, NULL, ospf_mpls_te_show_info, ospf_mpls_te_lsa_inter_as_as,
      ospf_mpls_te_lsa_refresh, NULL, NULL);
  if (rc != 0)
    {
      flog_warn (EC_OSPF_OPAQUE_REGISTRATION,
...
ospf_mpls_te_term (void)
{
  ...
  ospf_delete_opaque_functab (OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_INTER_AS_LSA);
  ospf_delete_opaque_functab (OSPF_OPAQUE_AS_LSA, OPAQUE_TYPE_INTER_AS_LSA);
...
static void
ospf_mpls_te_foreach_area (void (*func) (struct mpls_te_link *lp,
...
static void
set_linkparams_link_id (struct mpls_te_link *lp, struct in_addr link_id)
{
...
static void
set_linkparams_te_metric (struct mpls_te_link *lp, uint32_t te_metric)
{
...
static void
set_linkparams_rsc_clsclr (struct mpls_te_link *lp, uint32_t classcolor)
{
...
void
set_linkparams_llri (struct mpls_te_link *lp, uint32_t local, uint32_t remote)
{
...
static void
set_linkparams_av_delay (struct mpls_te_link *lp, uint32_t delay,
                         uint8_t anormal)
{
  uint32_t tmp;
  /* Note that TLV-length field is the size of array. */
...
static void
set_linkparams_mm_delay (struct mpls_te_link *lp, uint32_t low, uint32_t high,
                         uint8_t anormal)
{
  uint32_t tmp;
  /* Note that TLV-length field is the size of array. */
...
static void
set_linkparams_delay_var (struct mpls_te_link *lp, uint32_t jitter)
{
  /* Note that TLV-length field is the size of array. */
...
static void
set_linkparams_pkt_loss (struct mpls_te_link *lp, uint32_t loss,
                         uint8_t anormal)
{
  uint32_t tmp;
  /* Note that TLV-length field is the size of array. */
...
is_mandated_params_set (struct mpls_te_link *lp)
{
  ...
  if (ntohs (OspfMplsTE.router_addr.header.type) == 0)
    {
      flog_warn (EC_OSPF_TE_UNEXPECTED, "MPLS-TE (%s): Missing Router Address",
                 __func__);
      return rc;
    }
  ...
  if (ntohs (lp->link_type.header.type) == 0)
    {
      flog_warn (EC_OSPF_TE_UNEXPECTED, "MPLS-TE (%s): Missing Link Type",
                 __func__);
      return rc;
    }
...
ospf_mpls_te_new_if (struct interface *ifp)
{
  ...
  ote_debug ("MPLS-TE (%s): Add new %s interface %s to MPLS-TE list", __func__,
...
ospf_mpls_te_ism_change (struct ospf_interface *oi, int old_state)
{
  ...
  if (lp == NULL)
    {
      flog_warn (EC_OSPF_TE_UNEXPECTED,
                 "MPLS-TE (%s): Cannot get linkparams from OI(%s)?", __func__,
                 IF_NAME (oi));
      return;
    }
  ...
  if (oi->area == NULL || oi->area->ospf == NULL)
    {
      flog_warn (EC_OSPF_TE_UNEXPECTED,
                 "MPLS-TE (%s): Cannot refer to OSPF from OI(%s)?", __func__,
                 IF_NAME (oi));
      return;
    }
  ...
  switch (oi->state)
    ...
    case ISM_Down:
      ...
      if (CHECK_FLAG (lp->flags, LPFLG_LSA_ENGAGED))
        {
          ote_debug ("MPLS-TE (%s): Interface %s goes down: flush LSA",
                     __func__, IF_NAME (oi));
          ospf_mpls_te_lsa_schedule (lp, FLUSH_THIS_LSA);
          return;
        }
  ...
  ote_debug ("MPLS-TE (%s): Update Link parameters for interface %s", __func__,
             IF_NAME (oi));
...
ospf_mpls_te_nsm_change (struct ospf_neighbor *nbr, int old_state)
{
  ...
  switch (oi->state)
    ...
    case ISM_Down:
      ...
      if (OspfMplsTE.enabled && CHECK_FLAG (lp->flags, LPFLG_LSA_ENGAGED))
        {
          ote_debug ("MPLS-TE (%s): Interface %s goes down: flush LSA",
                     __func__, IF_NAME (oi));
          ospf_mpls_te_lsa_schedule (lp, FLUSH_THIS_LSA);
...
static void
build_tlv_header (struct stream *s, struct tlv_header *tlvh)
{
...
static void
build_link_subtlv (struct stream *s, struct tlv_header *tlvh)
{
...
ospf_mpls_te_lsa_new (struct ospf *ospf, struct ospf_area *area,
                      struct mpls_te_link *lp)
{
  struct stream *s;
  ...
  if (IS_INTER_AS (lp->type))
  ...
  else
    {
      ...
      lsa_type = OSPF_OPAQUE_AREA_LSA;
      tmp =
          SET_OPAQUE_LSID (OPAQUE_TYPE_TRAFFIC_ENGINEERING_LSA, lp->instance);
      lsa_id.s_addr = htonl (tmp);
          ...
          if (CHECK_FLAG (lp->flags, LPFLG_LSA_FORCED_REFRESH))
            {
              ...
              ote_debug ("MPLS-TE (%s): Refresh instead of Originate",
                         __func__);
              ospf_mpls_te_lsa_schedule (lp, REFRESH_THIS_LSA);
            }
          continue;
  ...
  if (new == NULL)
    {
      flog_warn (EC_OSPF_LSA_UNEXPECTED,
                 "MPLS-TE (%s): ospf_router_info_lsa_new() ?", __func__);
      return rc;
    }
...
ospf_mpls_te_lsa_originate_as (void *arg)
{
  ...
  if ((! OspfMplsTE.enabled) || (OspfMplsTE.inter_as == Off))
    {
      ote_debug ("MPLS-TE (%s): Inter-AS is disabled for now", __func__);
      rc = 0; /* This is not an error case. */
      return rc;
    }
  ...
  for (ALL_LIST_ELEMENTS (OspfMplsTE.iflist, node, nnode, lp))
    {
      ...
      ote_debug (
          ...
          __func__, lp->instance, IS_FLOOD_AS (lp->flags) ? "AS" : "Area",
 ...
 * scope is associated with the LSA origination as parameter is of type void
 * and must be cast to struct *ospf for AS flooding and to struct *ospf_area
 * for Area flooding.
 */
static int
ospf_mpls_te_lsa_inter_as_as (void *arg)
{
  if (OspfMplsTE.inter_as == AS)
...
static int
ospf_mpls_te_lsa_inter_as_area (void *arg)
{
...
ospf_mpls_te_lsa_refresh (struct ospf_lsa *lsa)
{
  ...
  if ((lp = lookup_linkparams_by_instance (lsa)) == NULL)
    {
      flog_warn (EC_OSPF_TE_UNEXPECTED, "MPLS-TE (%s): Invalid parameter?",
                 __func__);
      lsa->data->ls_age = htons (OSPF_LSA_MAXAGE); /* Flush it anyway. */
...
void
ospf_mpls_te_lsa_schedule (struct mpls_te_link *lp, enum lsa_opcode opcode)
{
  struct ospf_lsa lsa;
  struct lsa_header lsah;
  struct ospf *top;
      ...
      if (IS_INTER_AS (lp->type))
        {
          ...
          if (lp->area == NULL)
            lp->area =
                ospf_area_lookup_by_area_id (top, OspfMplsTE.interas_areaid);
  ...
  switch (opcode)
      ...
      if (IS_FLOOD_AS (lp->flags))
        {
          ospf_opaque_lsa_reoriginate_schedule (
              (void *) top, OSPF_OPAQUE_AS_LSA, OPAQUE_TYPE_INTER_AS_LSA);
        }
      else
        {
          if (IS_INTER_AS (lp->type))
    ...
    case FLUSH_THIS_LSA:
      ...
      break;
    default:
      flog_warn (EC_OSPF_TE_UNEXPECTED, "MPLS-TE (%s): Unknown opcode (%u)",
                 __func__, opcode);
      break;
    }
}
 ...
 */
static struct ls_vertex *
get_vertex (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  ...
  if (! vertex)
    {
      ...
      snprintfrr (lnode->name, MAX_NAME_LENGTH, "%pI4", &lnid.id.ip.addr);
 ...
 */
static struct ls_edge *
get_edge (struct ls_ted *ted, struct ls_node_id adv, struct in_addr link_id)
{
 ...
 */
static int
ospf_te_export (uint8_t type, void *link_state)
{
 ...
 */
static void
ospf_te_update_link (struct ls_ted *ted, struct ls_vertex *vertex,
                     struct in_addr link_data, uint8_t metric)
{
  ...
  if ((CHECK_FLAG (attr->flags, LS_ATTR_METRIC) && (attr->metric == metric)))
    {
      edge->status = SYNC;
      return;
    }
 ...
 */
static void
ospf_te_update_subnet (struct ls_ted *ted, struct ls_vertex *vertex,
 ...
 */
static void
ospf_te_delete_subnet (struct ls_ted *ted, struct in_addr addr)
{
  struct prefix p;
  struct ls_subnet *subnet;
 ...
 */
static int
ospf_te_parse_router_lsa (struct ls_ted *ted, struct ospf_lsa *lsa)
{
 ...
 */
static int
ospf_te_delete_router_lsa (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_node_id lnid;
  struct ls_vertex *vertex;
 ...
 */
static void
ospf_te_update_remote_asbr (struct ls_ted *ted, struct ls_edge *edge)
{
  struct ls_node_id lnid;
  struct ls_vertex *vertex;
  struct ls_node *lnode;
  ...
  if (vertex->status == NEW || vertex->status == UPDATE)
    {
      ote_debug ("  |- %s Remote Vertex %pI4 for AS %u",
                 vertex->status == NEW ? "Add" : "Update", &lnode->router_id,
                 lnode->as_number);
      ospf_te_export (LS_MSG_TYPE_NODE, vertex);
      vertex->status = SYNC;
    }
 ...
 */
static int
ospf_te_delete_te (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_edge *edge;
  struct ls_attributes *attr;
  struct tlv_header *tlvh;
 ...
 */
static int
ospf_te_parse_ri (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_vertex *vertex;
  struct ls_node *node;
      ...
      switch (ntohs (tlvh->type))
        ...
        case RI_SR_TLV_SR_ALGORITHM:
          ...
          for (int i = ntohs (algo->header.length); i < 2; i++)
            {
              if (vertex->status != NEW && node->algo[i] != SR_ALGORITHM_UNSET)
                vertex->status = UPDATE;
  ...
  if (vertex->status == NEW || vertex->status == UPDATE)
    {
      ote_debug ("  |- %s SR info - SRGB[%d/%d] for Vertex %pI4",
                 vertex->status == NEW ? "Add" : "Update",
                 vertex->node->srgb.lower_bound, vertex->node->srgb.range_size,
                 &vertex->node->router_id);
 ...
 */
static int
ospf_te_delete_ri (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_node_id lnid;
  struct ls_vertex *vertex;
  struct ls_node *node;
  ...
  ote_debug ("  |- Delete SR info. for Vertex %pI4", &vertex->node->router_id);
 ...
 */
static int
ospf_te_parse_ext_pref (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_node_id lnid;
  struct ls_subnet *subnet;
  struct ls_prefix *ls_pref;
  ...
  if (CHECK_FLAG (ls_pref->flags, LS_PREF_SR) &&
      ...
      ls_pref->sr.sid_flag == pref_sid->flags && ls_pref->sr.sid == label)
    return 0;
  ...
  if (subnet->status == NEW || subnet->status == UPDATE)
    {
      ote_debug ("  |- %s SID %d to subnet %pFX",
                 subnet->status == NEW ? "Add" : "Update", ls_pref->sr.sid,
                 &ls_pref->pref);
 ...
 */
static int
ospf_te_delete_ext_pref (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_subnet *subnet;
  struct ls_prefix *ls_pref;
  struct prefix pref;
 ...
 */
static int
ospf_te_parse_ext_link (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_node_id lnid;
  struct tlv_header *tlvh;
  ...
  for (; sum < len; tlvh = TLV_HDR_NEXT (tlvh))
    {
      ...
      switch (ntohs (tlvh->type))
        ...
        case EXT_SUBTLV_ADJ_SID:
          ...
          label = CHECK_FLAG (adj->flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG)
                      ? GET_LABEL (ntohl (adj->value))
                      : ntohl (adj->value);
          i = CHECK_FLAG (adj->flags, EXT_SUBTLV_LINK_ADJ_SID_BFLG) ? 1 : 0;
        ...
        case EXT_SUBTLV_LAN_ADJ_SID:
          ...
          label = CHECK_FLAG (ladj->flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG)
                      ? GET_LABEL (ntohl (ladj->value))
                      : ntohl (ladj->value);
          i = CHECK_FLAG (ladj->flags, EXT_SUBTLV_LINK_ADJ_SID_BFLG) ? 1 : 0;
          if (((i && CHECK_FLAG (atr->flags, LS_ATTR_BCK_ADJ_SID)) ||
               (! i && CHECK_FLAG (atr->flags, LS_ATTR_ADJ_SID))) &&
        ...
        case EXT_SUBTLV_RMT_ITF_ADDR:
          ...
          if (CHECK_FLAG (atr->flags, LS_ATTR_NEIGH_ADDR) &&
              IPV4_ADDR_SAME (&atr->standard.remote, &rmt->value))
            break;
 ...
 */
static int
ospf_te_delete_ext_link (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  struct ls_edge *edge;
  struct ls_attributes *atr;
  struct ext_tlv_link *ext;
 ...
 */
static int
ospf_te_parse_opaque_lsa (struct ls_ted *ted, struct ospf_lsa *lsa)
{
 ...
 */
static int
ospf_te_delete_opaque_lsa (struct ls_ted *ted, struct ospf_lsa *lsa)
{
  uint8_t key = GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr));
  int rc = -1;
 ...
 */
static int
ospf_mpls_te_lsa_update (struct ospf_lsa *lsa)
{
  ...
  if (lsa == NULL)
    {
      flog_warn (EC_OSPF_LSA_NULL, "TE (%s): Abort! LSA is NULL", __func__);
      return -1;
    }
 ...
 */
static int
ospf_mpls_te_lsa_delete (struct ospf_lsa *lsa)
{
 ...
 */
int
ospf_te_sync_ted (struct zapi_opaque_reg_info dst)
{
  int rc = -1;
 ...
 */
static void
ospf_te_init_ted (struct ls_ted *ted, struct ospf *ospf)
{
  struct listnode *node, *nnode;
  struct route_node *rn;
  struct ospf_area *area;
      ...
      if (ntohs (tlvh->length) > size)                                        \
        {                                                                     \
          if (vty != NULL)                                                    \
            ...
            zlog_debug ("    Wrong %s TLV size: %d(%d)", msg,                 \
                        ntohs (tlvh->length), size);                          \
...
static uint16_t
show_vty_router_addr (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_header (struct vty *vty, struct tlv_header *tlvh,
                      size_t buf_size)
{
  ...
  if (TLV_SIZE (tlvh) > buf_size)
    {
      if (vty != NULL)
        ...
        zlog_debug ("    TLV size %d exceeds buffer size. Abort!",
                    TLV_SIZE (tlvh));
      return buf_size;
    }
...
static uint16_t
show_vty_link_subtlv_link_type (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_link_id (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_lclif_ipaddr (struct vty *vty, struct tlv_header *tlvh,
                                   size_t buf_size)
{
...
static uint16_t
show_vty_link_subtlv_rmtif_ipaddr (struct vty *vty, struct tlv_header *tlvh,
                                   size_t buf_size)
{
...
static uint16_t
show_vty_link_subtlv_te_metric (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_max_bw (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_max_rsv_bw (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_unrsv_bw (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  for (i = 0; i < MAX_CLASS_TYPE; i += 2)
    {
      ...
      if (vty != NULL)
        ...
        zlog_debug ("      [%d]: %g (Bytes/sec),  [%d]: %g (Bytes/sec)", i,
                    fval1, i + 1, fval2);
    }
...
static uint16_t
show_vty_link_subtlv_rsc_clsclr (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_lrrid (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (vty != NULL)
    ...
    {
      zlog_debug ("    Local  TE Router ID: %pI4", &top->local);
      zlog_debug ("    Remote TE Router ID: %pI4", &top->remote);
    }
...
static uint16_t
show_vty_link_subtlv_llri (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (vty != NULL)
    ...
    {
      zlog_debug ("    Link Local  ID: %d", (uint32_t) ntohl (top->local));
      zlog_debug ("    Link Remote ID: %d", (uint32_t) ntohl (top->remote));
    }
...
static uint16_t
show_vty_link_subtlv_rip (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (vty != NULL)
    ...
    zlog_debug ("    Inter-AS TE Remote ASBR IP address: %pI4", &top->value);

  return TLV_SIZE (tlvh);
}

static uint16_t
show_vty_link_subtlv_ras (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_av_delay (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_mm_delay (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_delay_var (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_pkt_loss (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_res_bw (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_ava_bw (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_link_subtlv_use_bw (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_unknown_tlv (struct vty *vty, struct tlv_header *tlvh,
                      size_t buf_size)
{
  if (TLV_SIZE (tlvh) > buf_size)
    {
...
static uint16_t
ospf_mpls_te_show_link_subtlv (struct vty *vty, struct tlv_header *tlvh0,
  ...
  for (tlvh = tlvh0; sum < total; tlvh = TLV_HDR_NEXT (tlvh))
    {
      switch (ntohs (tlvh->type))
        ...
        case TE_LINK_SUBTLV_LINK_ID:
          ...
          break;
        case TE_LINK_SUBTLV_LCLIF_IPADDR:
          sum += show_vty_link_subtlv_lclif_ipaddr (vty, tlvh, total - sum);
          break;
        case TE_LINK_SUBTLV_RMTIF_IPADDR:
          sum += show_vty_link_subtlv_rmtif_ipaddr (vty, tlvh, total - sum);
          break;
        case TE_LINK_SUBTLV_TE_METRIC:
...
static void
ospf_mpls_te_show_info (struct vty *vty, struct json_object *json,
                        struct ospf_lsa *lsa)
{
  struct lsa_header *lsah = lsa->data;
...
static int
set_inter_as_mode (struct vty *vty, const char *mode_name, const char *area_id)
{
  ...
  if (OspfMplsTE.enabled)
    {
      ...
      if ((OspfMplsTE.inter_as == Off) && (mode != OspfMplsTE.inter_as))
        {
