  ...
  for (rn = route_top (nbrs); rn; rn = route_next (rn))
    {
       ...
       * Ignore myself
       */
      if (nbr->router_id.s_addr != INADDR_ANY && nbr->state >= NSM_TwoWay &&
...
void
ospf_hello_timer (struct event *thread)
{
...
static void
ospf_wait_timer (struct event *thread)
{
  struct ospf_interface *oi;
...
void
ospf_ism_event (struct event *thread)
{
  int event;
  int next_state;
  struct ospf_interface *oi;
