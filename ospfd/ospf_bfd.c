...
static void
ospf_bfd_session_change (struct bfd_session_params *bsp,
  ...
  if (bss->state == BFD_STATUS_DOWN && bss->previous_state == BFD_STATUS_UP)
    {
      if (IS_DEBUG_OSPF (bfd, BFD_LIB))
        zlog_debug ("%s: NSM[%s:%pI4]: BFD Down", __func__, IF_NAME (nbr->oi),
...
void
ospf_neighbor_bfd_apply (struct ospf_neighbor *nbr)
{
  ...
  bfd_sess_set_timers (nbr->bfd_session, oip->bfd_config->detection_multiplier,
...
static void
ospf_interface_bfd_apply (struct interface *ifp)
{
...
static void
ospf_interface_enable_bfd (struct interface *ifp)
{
...
void
ospf_interface_disable_bfd (struct interface *ifp, struct ospf_if_params *oip)
{
 ...
 */
void
ospf_bfd_write_config (struct vty *vty, const struct ospf_if_params *params
...
void
ospf_interface_bfd_show (struct vty *vty, const struct interface *ifp,
  ...
  if (json)
    {
      ...
      json_object_int_add (json_bfd, "rxMinInterval", bfd_config->min_rx);
      json_object_int_add (json_bfd, "txMinInterval", bfd_config->min_tx);
...
void
ospf_bfd_init (struct event_loop *tm)
{
