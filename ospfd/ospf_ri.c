...
static int
ospf_router_info_register (uint8_t scope)
{
  int rc = 0;
  ...
  zlog_info ("RI (%s): Register Router Information with scope %s(%d)",
             __func__, scope == OSPF_OPAQUE_AREA_LSA ? "Area" : "AS", scope);
  rc = ospf_register_opaque_functab (
      scope, OPAQUE_TYPE_ROUTER_INFORMATION_LSA, NULL, /* new interface */
      NULL,                                            /* del interface */
      ospf_router_info_ism_change, NULL,               /* NSM change */
      ospf_router_info_config_write_router, NULL, /* Config. write interface */
...
static struct ospf_ri_area_info *
lookup_by_area (struct ospf_area *area)
{
  struct listnode *node, *nnode;
  struct ospf_ri_area_info *ai;
...
static void
set_router_info_capabilities (struct ri_tlv_router_cap *ric, uint32_t cap)
{
...
static void
set_pce_path_scope (uint32_t scope, struct ospf_pce_info *pce)
{
...
static void
set_pce_domain (uint16_t type, uint32_t domain, struct ospf_pce_info *pce)
{
...
static void
unset_pce_domain (uint16_t type, uint32_t domain, struct ospf_pce_info *pce)
{
  struct listnode *node;
...
static void
set_pce_neighbor (uint16_t type, uint32_t domain, struct ospf_pce_info *pce)
{
  ...
  new =
      XCALLOC (MTYPE_OSPF_PCE_PARAMS, sizeof (struct ri_pce_subtlv_neighbor));
...
static void
unset_pce_neighbor (uint16_t type, uint32_t domain, struct ospf_pce_info *pce)
{
  struct listnode *node;
...
static void
set_pce_cap_flag (uint32_t cap, struct ospf_pce_info *pce)
{
...
static void
unset_sr_algorithm (uint8_t algo)
{
...
static void
set_sr_global_label_range (struct sr_block srgb)
{
  /* Set Header */
...
static void
unset_sr_global_label_range (void)
{
...
static void
set_sr_local_label_range (struct sr_block srlb)
{
  /* Set Header */
...
static void
unset_sr_local_label_range (void)
{
  ...
  if (OspfRI.scope == OSPF_OPAQUE_AREA_LSA)
    {
      ...
      for (ALL_LIST_ELEMENTS (top->areas, node, nnode, area))
        {
          zlog_debug ("RI (%s): Add area %pI4 to Router Information", __func__,
                      &area->area_id);
 ...
 */
void
ospf_router_info_update_sr (bool enable, struct sr_node *srn)
{
  struct listnode *node, *nnode;
  struct ospf_ri_area_info *ai;
  ...
  for (ALL_LIST_ELEMENTS (OspfRI.area_info, node, nnode, ai))
    {
      if (CHECK_FLAG (ai->flags, RIFLG_LSA_ENGAGED))
        ...
        ospf_router_info_lsa_schedule (ai, REORIGINATE_THIS_LSA);
    }
}
...
ospf_router_info_lsa_new (struct ospf_area *area)
{
  struct ospf *top;
  struct stream *s;
...
static int
ospf_router_info_lsa_originate_as (void *arg)
{
  struct ospf_lsa *new;
  struct ospf *top;
  int rc = -1;
  ...
  if (top == NULL)
    {
      zlog_debug ("RI (%s): ospf instance not found for vrf id %u", __func__,
                  VRF_DEFAULT);
      ospf_lsa_unlock (&new);
      return rc;
    }
  ...
  if (ospf_lsa_install (top, NULL /*oi */, new) == NULL)
    {
      flog_warn (EC_OSPF_LSA_INSTALL_FAILURE, "RI (%s): ospf_lsa_install() ?",
                 __func__);
      ospf_lsa_unlock (&new);
      return rc;
    }
...
static int
ospf_router_info_lsa_originate_area (void *arg)
{
  struct ospf_lsa *new;
  struct ospf *top;
...
ospf_router_info_lsa_originate (void *arg)
{
  ...
  if (! OspfRI.enabled)
    {
      zlog_info ("RI (%s): ROUTER INFORMATION is disabled now.", __func__);
  ...
  if (OspfRI.scope == OSPF_OPAQUE_AS_LSA)
    {
      if ((CHECK_FLAG (OspfRI.as_flags, RIFLG_LSA_ENGAGED)) &&
          (CHECK_FLAG (OspfRI.as_flags, RIFLG_LSA_FORCED_REFRESH)))
        {
  ...
  else
    {
      ...
      if (ai == NULL)
        {
          flog_warn (EC_OSPF_LSA, "RI (%s): Missing area information",
                     __func__);
          return rc;
        }
      if ((CHECK_FLAG (ai->flags, RIFLG_LSA_ENGAGED)) &&
...
ospf_router_info_lsa_refresh (struct ospf_lsa *lsa)
{
  ...
  if (GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)) != 0)
    {
      flog_warn (EC_OSPF_LSA, "RI (%s): Unsupported Router Information ID",
                 __func__);
      return NULL;
    }
  ...
  if (OspfRI.scope == OSPF_OPAQUE_AREA_LSA)
    {
      ...
      if (ai == NULL)
        {
          flog_warn (EC_OSPF_LSA, "RI (%s): No associated Area", __func__);
          return NULL;
        }
...
static void
ospf_router_info_lsa_schedule (struct ospf_ri_area_info *ai,
                               enum lsa_opcode opcode)
{
  struct ospf_lsa lsa;
  struct lsa_header lsah;
...
static int
ospf_router_info_lsa_update (struct ospf_lsa *lsa)
{

  /* Sanity Check */
  if (lsa == NULL)
    {
      flog_warn (EC_OSPF_LSA, "RI (%s): Abort! LSA is NULL", __func__);
      return -1;
    }
...
static uint16_t
show_vty_router_cap (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_pce_subtlv_address (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (ntohs (top->address.type) == PCE_ADDRESS_IPV4)
    {
      ...
      if (vty != NULL)
        ...
        zlog_debug ("    PCE Address: %pI4", &top->address.value);
    }
...
static uint16_t
show_vty_pce_subtlv_path_scope (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_pce_subtlv_domain (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (ntohs (top->type) == PCE_DOMAIN_TYPE_AREA)
  ...
  else
    {
      if (vty != NULL)
        ...
        zlog_debug ("    Wrong PCE Domain type: %d", ntohl (top->type));
    }
...
static uint16_t
show_vty_pce_subtlv_neighbor (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (ntohs (top->type) == PCE_DOMAIN_TYPE_AREA)
  ...
  else
    {
      if (vty != NULL)
        ...
        zlog_debug ("    Wrong PCE Neighbor type: %d", ntohl (top->type));
    }
...
static uint16_t
show_vty_pce_subtlv_cap_flag (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_pce_info (struct vty *vty, struct tlv_header *ri, size_t buf_size)
{
  ...
  if (length > buf_size)
    {
      vty_out (vty, "  PCE Info TLV size %d exceeds buffer size. Abort!\n",
               length);
      return buf_size;
    }
...
static uint16_t
show_vty_sr_algorithm (struct vty *vty, struct tlv_header *tlvh)
{
  struct ri_sr_tlv_sr_algorithm *algo = (struct ri_sr_tlv_sr_algorithm *) tlvh;
  int i;
      ...
      for (i = 0; i < ntohs (algo->header.length); i++)
        {
          switch (algo->value[i])
            ...
            case 1:
              ...
              break;
            default:
              vty_out (vty, "  Algorithm %d: Unknown value %d\n", i,
                       algo->value[i]);
              break;
            }
        }
      ...
      for (i = 0; i < ntohs (algo->header.length); i++)
        switch (algo->value[i])
          ...
          case 1:
            ...
            break;
          default:
            zlog_debug ("    Algorithm %d: Unknown value %d", i,
                        algo->value[i]);
            break;
          }
    }
...
static uint16_t
show_vty_sr_range (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (vty != NULL)
  ...
  else
    {
      zlog_debug (
          ...
          ntohs (range->header.type) == RI_SR_TLV_SRGB_LABEL_RANGE ? "Global"
                                                                   : "Local",
          GET_RANGE_SIZE (ntohl (range->size)),
          GET_LABEL (ntohl (range->lower.value)));
    }
...
static uint16_t
show_vty_sr_msd (struct vty *vty, struct tlv_header *tlvh)
{
  ...
  if (vty != NULL)
    ...
    {
      zlog_debug ("  Segment Routing MSD TLV:  Node Maximum Stack Depth = %d",
                  msd->value);
    }
...
static void
ospf_router_info_show_info (struct vty *vty, struct json_object *json,
                            struct ospf_lsa *lsa)
{
...
static void
ospf_router_info_schedule (enum lsa_opcode opcode)
{
  struct listnode *node, *nnode;
  struct ospf_ri_area_info *ai;
       ...
       "OSPF area ID in IP format (deprecated)\n")
{
  ...
  if (ospf->vrf_id != VRF_DEFAULT)
    {
      vty_out (vty, "Router Information is only supported in default VRF\n");
  ...
  if (! OspfRI.registered && (ospf_router_info_register (scope)) != 0)
    {
      vty_out (vty, "%% Unable to register Router Information callbacks.");
      flog_err (
          EC_OSPF_INIT_FAIL,
