...
static bool
sr_cmp (const void *p1, const void *p2)
{
...
sr_node_new (struct in_addr *rid)
{
...
struct sr_node *
ospf_sr_node_create (struct in_addr *rid)
{
  struct sr_node *srn;
...
static void
sr_node_del (struct sr_node *srn)
{
  /* Sanity Check */
  if (srn == NULL)
    return;
...
static struct sr_node *
get_sr_node_by_nexthop (struct ospf *ospf, struct in_addr nexthop)
{
...
/**
 * It is necessary to known which label is already allocated to manage the
 * range of SRLB. This is particular useful when an interface flap (goes up /
 * down frequently). Here, SR will release and then allocate label for the
 * Adjacency for each concerned interface. If we don't care, there is a risk to
 * run out of label.
 *
 ...
 * (bit unset). Its size is equal to the number of label of the SRLB range
 * round up to 64 bits.
 *
 ...
 */
static int
sr_local_block_init (uint32_t lower_bound, uint32_t upper_bound)
{
  ...
  if (ospf_zebra_request_label_range (lower_bound, size))
    {
      zlog_err ("SR: Error reserving SRLB [%u/%u] %u labels", lower_bound,
                upper_bound, size);
      return -1;
    }

  osr_debug ("SR: Got new SRLB [%u/%u], %u labels", lower_bound, upper_bound,
             size);
  ...
  if ((size % SRLB_BLOCK_SIZE) != 0)
    srlb->max_block++;
  srlb->used_mark =
      XCALLOC (MTYPE_OSPF_SR_PARAMS, srlb->max_block * SRLB_BLOCK_SIZE);
  srlb->reserved = true;
...
static int
sr_global_block_init (uint32_t start, uint32_t size)
{
  ...
  if (ospf_zebra_request_label_range (start, size) < 0)
    {
      zlog_err ("SR: Error reserving SRGB [%u/%u], %u labels", start, end,
                size);
      return -1;
    }
...
sr_local_block_delete (void)
{
  ...
  osr_debug ("SR (%s): Remove SRLB [%u/%u]", __func__, srlb->start, srlb->end);
...
sr_global_block_delete (void)
{
  ...
  ospf_zebra_release_label_range (srgb->start, srgb->start + srgb->size - 1);
 ...
 */
int
ospf_sr_local_block_release_label (mpls_label_t label)
{
  struct sr_local_block *srlb = &OspfSR.srlb;
  uint32_t index;
  uint32_t pos;
  ...
  if ((label < srlb->start) || (label > srlb->end))
    {
      flog_warn (EC_OSPF_SR_SID_OVERFLOW,
                 "%s: Returning label %u is outside SRLB [%u/%u]", __func__,
                 label, srlb->start, srlb->end);
      return -1;
    }
 ...
 */
static void
sr_start_label_manager (struct event *start)
{
  struct ospf *ospf;
...
static int
ospf_sr_start (struct ospf *ospf)
{
  struct route_node *rn;
  struct ospf_lsa *lsa;
  struct sr_node *srn;
      ...
      LSDB_LOOP (OPAQUE_AREA_LSDB (ospf->backbone), rn, lsa)
        {
          if (IS_LSA_MAXAGE (lsa) || IS_LSA_SELF (lsa))
            continue;
          int lsa_id = GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr));
          switch (lsa_id)
            {
...
static mpls_label_t
index2label (uint32_t index, struct sr_block srgb)
{
  mpls_label_t label;
  ...
  if (label > (srgb.lower_bound + srgb.range_size))
    {
      flog_warn (EC_OSPF_SR_SID_OVERFLOW,
                 "%s: SID index %u falls outside SRGB range", __func__, index);
      return MPLS_INVALID_LABEL;
    }
  else
    return label;
...
mpls_label_t
ospf_sr_get_prefix_sid_by_id (struct in_addr *id)
{
  struct sr_node *srn;
...
mpls_label_t
ospf_sr_get_adj_sid_by_id (struct in_addr *root_id,
...
get_neighbor_by_addr (struct ospf *top, struct in_addr addr)
{
...
get_nexthop_by_addr (struct ospf *top, struct prefix_ipv4 p)
{
  struct route_node *rn;
  ...
  osr_debug ("      |-  Search Nexthop for prefix %pFX", (struct prefix *) &p);
...
static int
compute_link_nhlfe (struct sr_link *srl)
{
  ...
  if (CHECK_FLAG (srl->flags[0], EXT_SUBTLV_LINK_ADJ_SID_VFLG))
    ...
    srl->nhlfe[0].label_in = index2label (srl->sid[0], srl->srn->srgb);
  if (CHECK_FLAG (srl->flags[1], EXT_SUBTLV_LINK_ADJ_SID_VFLG))
    srl->nhlfe[1].label_in = srl->sid[1];
  else
    srl->nhlfe[1].label_in = index2label (srl->sid[1], srl->srn->srgb);
 ...
 */
static mpls_label_t
sr_prefix_out_label (const struct sr_prefix *srp, const struct sr_node *srnext)
{
  ...
  if (CHECK_FLAG (srp->flags,
                  EXT_SUBTLV_PREFIX_SID_VFLG | EXT_SUBTLV_PREFIX_SID_LFLG))
    {
      /*
 ...
 */
static int
compute_prefix_nhlfe (struct sr_prefix *srp)
{
  struct ospf *top = ospf_lookup_by_vrf_id (VRF_DEFAULT);
  ...
  for (ALL_LIST_ELEMENTS_RO (srp->route->paths, node, path))
    {
      ...
      if (srnext == NULL || srnext->srgb.lower_bound == 0 ||
        ...
        {
          osr_debug ("    |-  SR-Node %pI4 not ready. Stop process",
                     &srnext->adv_router);
      ...
      osr_debug ("    |-  Computed new labels in: %u out: %u", srp->label_in,
                 path->srni.label_out);
      rc = 1;
    }
  return rc;
...
static inline void
add_adj_sid (struct sr_nhlfe nhlfe)
{
  if (nhlfe.label_in != 0)
...
static inline void
del_adj_sid (struct sr_nhlfe nhlfe)
{
  if (nhlfe.label_in != 0)
...
static inline void
update_adj_sid (struct sr_nhlfe n1, struct sr_nhlfe n2)
{
  del_adj_sid (n1);
  add_adj_sid (n2);
}
...
static struct sr_link *
get_ext_link_sid (struct tlv_header *tlvh, size_t size)
{
  ...
  for (; sum < length && sub_tlvh; sub_tlvh = TLV_HDR_NEXT (sub_tlvh))
    {
      switch (ntohs (sub_tlvh->type))
        ...
        case EXT_SUBTLV_ADJ_SID:
          ...
          srl->type = ADJ_SID;
          i = CHECK_FLAG (adj_sid->flags, EXT_SUBTLV_LINK_ADJ_SID_BFLG) ? 1
                                                                        : 0;
          srl->flags[i] = adj_sid->flags;
          if (CHECK_FLAG (adj_sid->flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG))
        ...
        case EXT_SUBTLV_LAN_ADJ_SID:
          ...
          i = CHECK_FLAG (lan_sid->flags, EXT_SUBTLV_LINK_ADJ_SID_BFLG) ? 1
                                                                        : 0;
          srl->flags[i] = lan_sid->flags;
          if (CHECK_FLAG (lan_sid->flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG))
            srl->sid[i] = GET_LABEL (ntohl (lan_sid->value));
          else
            srl->sid[i] = ntohl (lan_sid->value);
          IPV4_ADDR_COPY (&srl->nhlfe[i].nexthop, &lan_sid->neighbor_id);
          break;
...
static struct sr_prefix *
get_ext_prefix_sid (struct tlv_header *tlvh, size_t size)
{
          ...
          if (psid->algorithm != SR_ALGORITHM_SPF)
            {
              flog_err (EC_OSPF_INVALID_ALGORITHM,
                        "SR (%s): Unsupported Algorithm", __func__);
...
static inline int
sr_link_cmp (struct sr_link *srl1, struct sr_link *srl2)
{
...
static inline int
sr_prefix_cmp (struct sr_prefix *srp1, struct sr_prefix *srp2)
{
...
static void
update_ext_link_sid (struct sr_node *srn, struct sr_link *srl,
                     uint8_t lsa_flags)
{
  struct listnode *node;
  struct sr_link *lk;
...
static void
update_ext_prefix_sid (struct sr_node *srn, struct sr_prefix *srp)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (srn->ext_prefix, node, pref))
    if (pref->instance == srp->instance &&
        prefix_same ((struct prefix *) &srp->prefv4, &pref->prefv4))
      {
        found = true;
        break;
      }
 ...
 */
static void
update_in_nhlfe (struct hash_bucket *bucket, void *args)
{
  struct listnode *node;
 ...
 */
static void
update_out_nhlfe (struct hash_bucket *bucket, void *args)
{
  ...
  osr_debug ("SR (%s): Update Out NHLFE for neighbor SR-Node %pI4", __func__,
             &srn->adv_router);
...
void
ospf_sr_ri_lsa_update (struct ospf_lsa *lsa)
{
  struct sr_node *srn;
  ...
  if (OspfSR.neighbors == NULL)
    {
      flog_err (EC_OSPF_SR_INVALID_DB, "SR (%s): Abort! no valid SR DataBase",
                __func__);
      return;
    }

  /* Search SR Node in hash table from Router ID */
  srn = (struct sr_node *) hash_lookup (OspfSR.neighbors, &lsah->adv_router);
  ...
  if (srn == NULL)
    {
      ...
      srn = (struct sr_node *) hash_get (OspfSR.neighbors, &lsah->adv_router,
                                         (void *) sr_node_new);
      /* update LSA ID */
      srn->instance = ntohl (lsah->id.s_addr);
      /* Copy SRGB */
  ...
  osr_debug (
      "  |- Update SR-Node[%pI4], SRGB[%u/%u], SRLB[%u/%u], Algo[%u], MSD[%u]",
      &srn->adv_router, srn->srgb.lower_bound, srn->srgb.range_size,
      srn->srlb.lower_bound, srn->srlb.range_size, srn->algo[0], srn->msd);
 ...
 */
void
ospf_sr_ri_lsa_delete (struct ospf_lsa *lsa)
{
  struct sr_node *srn;
  struct lsa_header *lsah = lsa->data;
  ...
  if (OspfSR.neighbors == NULL)
    {
      flog_err (EC_OSPF_SR_INVALID_DB, "SR (%s): Abort! no valid SR Data Base",
                __func__);
      return;
    }
  ...
  if (srn == NULL)
    {
      flog_err (EC_OSPF_SR_NODE_CREATE,
                "SR (%s): Abort! no entry in SRDB for SR Node %pI4", __func__,
                &lsah->adv_router);
      return;
    }
...
void
ospf_sr_ext_link_lsa_update (struct ospf_lsa *lsa)
{
  struct sr_node *srn;
  struct tlv_header *tlvh;
  struct lsa_header *lsah = lsa->data;
  ...
  osr_debug ("SR (%s): Process Extended Link LSA 8.0.0.%u from %pI4", __func__,
  ...
  srn = (struct sr_node *) hash_get (
      OspfSR.neighbors, (void *) &(lsah->adv_router), (void *) sr_node_new);
...
void
ospf_sr_ext_link_lsa_delete (struct ospf_lsa *lsa)
{
  struct listnode *node;
  struct sr_link *srl;
  struct sr_node *srn;
  ...
  osr_debug ("SR (%s): Remove Extended Link LSA 8.0.0.%u from %pI4", __func__,
             GET_OPAQUE_ID (ntohl (lsah->id.s_addr)), &lsah->adv_router);
  ...
  if (srn == NULL)
    {
      flog_err (EC_OSPF_SR_INVALID_DB,
                "SR (%s): Stop! no entry in SRDB for SR Node %pI4", __func__,
                &lsah->adv_router);
      return;
    }
...
void
ospf_sr_ext_itf_add (struct ext_itf *exti)
{
  ...
  switch (exti->stype)
    ...
    case ADJ_SID:
      ...
      if (CHECK_FLAG (exti->adj_sid[0].flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG))
      ...
      if (CHECK_FLAG (exti->adj_sid[1].flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG))
    ...
    case LAN_ADJ_SID:
      ...
      if (CHECK_FLAG (exti->lan_sid[0].flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG))
      ...
      if (CHECK_FLAG (exti->lan_sid[1].flags, EXT_SUBTLV_LINK_ADJ_SID_VFLG))
...
ospf_sr_ext_itf_delete (struct ext_itf *exti)
{
  ...
  osr_debug ("SR (%s): Remove Extended LSA %u.0.0.%u from self", __func__,
  ...
  if (exti->stype == PREF_SID)
    {
      instance =
          SET_OPAQUE_LSID (OPAQUE_TYPE_EXTENDED_PREFIX_LSA, exti->instance);
      for (ALL_LIST_ELEMENTS_RO (srn->ext_prefix, node, srp))
...
void
ospf_sr_ext_prefix_lsa_update (struct ospf_lsa *lsa)
{
  struct sr_node *srn;
  struct tlv_header *tlvh;
...
void
ospf_sr_ext_prefix_lsa_delete (struct ospf_lsa *lsa)
{
  struct listnode *node;
  struct sr_prefix *srp;
  struct sr_node *srn;
 ...
 */
void
ospf_sr_update_local_prefix (struct interface *ifp, struct prefix *p)
{
  struct listnode *node;
  struct sr_prefix *srp;
...
static void
ospf_sr_nhlfe_update (struct hash_bucket *bucket, void *args)
{
...
void
ospf_sr_update_task (struct ospf *ospf)
{
  ...
  hash_iterate (OspfSR.neighbors,
                (void (*) (struct hash_bucket *, void *)) ospf_sr_nhlfe_update,
                NULL);
 ...
 */
void
ospf_sr_config_write_router (struct vty *vty)
{
  struct listnode *node;
  struct sr_prefix *srp;
  uint32_t upper;
  ...
  if ((OspfSR.srgb.start != DEFAULT_SRGB_LABEL) ||
      (OspfSR.srgb.size != DEFAULT_SRGB_SIZE))
    vty_out (vty, " segment-routing global-block %u %u", OspfSR.srgb.start,
             upper);
  ...
  if (OspfSR.self != NULL)
    {
      for (ALL_LIST_ELEMENTS_RO (OspfSR.self->ext_prefix, node, srp))
        {
          vty_out (vty, " segment-routing prefix %pFX index %u", &srp->prefv4,
                   srp->sid);
          if (CHECK_FLAG (srp->flags, EXT_SUBTLV_PREFIX_SID_EFLG))
            vty_out (vty, " explicit-null\n");
          else if (CHECK_FLAG (srp->flags, EXT_SUBTLV_PREFIX_SID_NPFLG))
...
static int
ospf_sr_enabled (struct vty *vty)
{
  if (OspfSR.status != SR_OFF)
    return 1;
...
static bool
ranges_overlap (uint32_t r1_lower, uint32_t r1_upper, uint32_t r2_lower,
                uint32_t r2_upper)
{
...
static bool
sr_range_is_valid (uint32_t lower, uint32_t upper, uint32_t min_size)
{
 ...
 */
static int
update_sr_blocks (uint32_t gb_lower, uint32_t gb_upper, uint32_t lb_lower,
                  uint32_t lb_upper)
{
  ...
  gb_changed = (OspfSR.srgb.size != gb_size || OspfSR.srgb.start != gb_lower);
  lb_changed = (OspfSR.srlb.end != lb_upper || OspfSR.srlb.start != lb_lower);
  ...
  if (OspfSR.self != NULL)
    {
      ...
      if (gb_changed)
        hash_iterate (
            OspfSR.neighbors,
            (void (*) (struct hash_bucket *, void *)) update_in_nhlfe, NULL);
       ...
       "Upper-bound range in decimal (16-1048575)\n")
{
  if (update_sr_blocks (DEFAULT_SRGB_LABEL, DEFAULT_SRGB_END,
                        DEFAULT_SRLB_LABEL, DEFAULT_SRLB_END) < 0)
    return CMD_WARNING_CONFIG_FAILED;
  else
    return CMD_SUCCESS;
}
       ...
       "Upstream neighbor must replace prefix-sid with explicit null label\n")
{
  ...
  if (ifp == NULL)
    {
      ...
      zlog_info ("Interface for prefix %pFX not found. Deferred LSA flooding",
                 &p);
      return CMD_SUCCESS;
    }
  ...
  srp->instance =
      ospf_ext_schedule_prefix_index (ifp, srp->sid, &srp->prefv4, srp->flags);
  if (srp->instance == 0)
    {
...
static char *
sr_op2str (char *buf, size_t size, mpls_label_t label_in,
...
static void
show_sr_prefix (struct sbuf *sbuf, struct json_object *json,
                struct sr_prefix *srp)
{
  ...
  if (srp->type == LOCAL_SID)
    {
      ...
      if (json)
          ...
          if (! json_route)
            {
              ...
              json_object_object_add (json, "prefixRoute", json_route);
            }
          json_obj = json_object_new_object ();
          json_object_int_add (json_obj, "outputLabel", srp->nhlfe.label_out);
      ...
      else
        {
          sbuf_push (
              sbuf, 0, "%20s  %9s  %15s\n",
              sr_op2str (op, 32, srp->label_in, srp->nhlfe.label_out),
              itf ? itf->name : "-",
              inet_ntop (AF_INET, &srp->nhlfe.nexthop, buf, sizeof (buf)));
        }
      return;
    }
      ...
      if (json)
        {
          ...
          json_obj = json_object_new_object ();
          json_object_int_add (json_obj, "outputLabel", path->srni.label_out);
          json_object_string_add (json_obj, "interface",
                                  itf ? itf->name : "-");
          json_object_string_addf (json_obj, "nexthop", "%pI4",
                                   &path->nexthop);
      ...
      else
        {
          sbuf_push (sbuf, indent, "%20s  %9s  %15s\n",
                     sr_op2str (op, 32, srp->label_in, path->srni.label_out),
                     itf ? itf->name : "-",
                     inet_ntop (AF_INET, &path->nexthop, buf, sizeof (buf)));
...
static void
show_sr_node (struct vty *vty, struct json_object *json, struct sr_node *srn)
{
  ...
  if (json)
    {
      ...
      json_object_string_addf (json_node, "routerID", "%pI4",
                               &srn->adv_router);
      json_object_int_add (json_node, "srgbSize", srn->srgb.range_size);
      json_object_int_add (json_node, "srgbLabel", srn->srgb.lower_bound);
      json_object_int_add (json_node, "srlbSize", srn->srlb.range_size);
      json_object_int_add (json_node, "srlbLabel", srn->srlb.lower_bound);
  ...
  else
    {
      ...
      sbuf_push (&sbuf, 0, "\tSRGB: [%u/%u]", srn->srgb.lower_bound, upper);
      upper = srn->srlb.lower_bound + srn->srlb.range_size - 1;
      sbuf_push (&sbuf, 0, "\tSRLB: [%u/%u]", srn->srlb.lower_bound, upper);
          ...
          if (! json_prefix)
            {
              ...
              json_object_object_add (json_node, "extendedPrefix",
                                      json_prefix);
            }
          json_obj = json_object_new_object ();
      ...
      if (json)
        {
          if (! json_link)
            {
              ...
              json_object_object_add (json_node, "extendedLink", json_link);
            }
          /* Primary Link */
          json_obj = json_object_new_object ();
          json_object_string_add (json_obj, "prefix", pref);
          json_object_int_add (json_obj, "sid", srl->sid[0]);
          json_object_int_add (json_obj, "inputLabel", srl->nhlfe[0].label_in);
          ...
          json_object_int_add (json_obj, "inputLabel", srl->nhlfe[1].label_in);
          json_object_int_add (json_obj, "outputLabel",
      ...
      else
        {
          sbuf_push (
              ...
              sr_op2str (op, 32, srl->nhlfe[0].label_in,
                         srl->nhlfe[0].label_out),
              itf ? itf->name : "-",
              inet_ntop (AF_INET, &srl->nhlfe[0].nexthop, buf, sizeof (buf)));
          snprintf (sid, 22, "SR Adj. (lbl %u)", srl->sid[1]);
          sbuf_push (
              &sbuf, 0, "%18s  %21s  %20s  %9s  %15s\n", pref, sid,
              sr_op2str (op, 32, srl->nhlfe[1].label_in,
                         srl->nhlfe[1].label_out),
              itf ? itf->name : "-",
              inet_ntop (AF_INET, &srl->nhlfe[1].nexthop, buf, sizeof (buf)));
        }
    }
  if (json)
...
static void
show_vty_srdb (struct hash_bucket *bucket, void *args)
{
...
static void
show_json_srdb (struct hash_bucket *bucket, void *args)
{
       ...
       JSON_STR)
{
  ...
  if (argv_find (argv, argc, "A.B.C.D", &idx))
    {
      ...
      srn = (struct sr_node *) hash_lookup (OspfSR.neighbors, (void *) &rid);
      show_sr_node (vty, json_node_array, srn);
      if (uj)
        vty_json (vty, json);
      return CMD_SUCCESS;
  ...
  if (uj)
    {
      hash_iterate (OspfSR.neighbors,
                    (void (*) (struct hash_bucket *, void *)) show_json_srdb,
    ...
    {
      hash_iterate (OspfSR.neighbors,
                    (void (*) (struct hash_bucket *, void *)) show_vty_srdb,
                    (void *) vty);
    }
  return CMD_SUCCESS;
}
