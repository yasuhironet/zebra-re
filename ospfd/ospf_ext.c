...
static void ospf_ext_link_nsm_change (struct ospf_neighbor *nbr,
                                      int old_status);
...
ospf_ext_init (void)
{
  ...
  if (rc != 0)
    {
      flog_warn (EC_OSPF_OPAQUE_REGISTRATION,
                 "EXT (%s): Failed to register Extended Link LSA", __func__);
      return rc;
    }
  ...
  if (rc != 0)
    {
      flog_warn (EC_OSPF_OPAQUE_REGISTRATION,
                 "EXT (%s): Failed to register Extended Prefix LSA", __func__);
      return rc;
    }
...
static struct ext_itf *
lookup_ext_by_ifp (struct interface *ifp)
{
  struct listnode *node;
  struct ext_itf *exti;
...
static struct ext_itf *
lookup_ext_by_instance (struct ospf_lsa *lsa)
{
  struct listnode *node;
  struct ext_itf *exti;
...
static void
set_ext_prefix (struct ext_itf *exti, uint8_t route_type, uint8_t flags,
...
static void
set_ext_link (struct ext_itf *exti, uint8_t type, struct in_addr id,
              struct in_addr data)
{
...
static void
set_prefix_sid (struct ext_itf *exti, uint8_t algorithm, uint32_t value,
                int value_type, uint8_t flags)
{
...
static void
set_adj_sid (struct ext_itf *exti, bool backup, uint32_t value, int value_type)
{
  int index;
  uint8_t flags;
...
static void
set_lan_adj_sid (struct ext_itf *exti, bool backup, uint32_t value,
...
static void
unset_adjacency_sid (struct ext_itf *exti)
{
  /* Reset Adjacency TLV */
...
static void
set_rmt_itf_addr (struct ext_itf *exti, struct in_addr rmtif)
{
...
/* Delete Extended LSA */
static void
ospf_extended_lsa_delete (struct ext_itf *exti)
{
 ...
 */
uint32_t
ospf_ext_schedule_prefix_index (struct interface *ifp, uint32_t index,
                                struct prefix_ipv4 *p, uint8_t flags)
{
  ...
  if (p != NULL)
    {
      osr_debug (
          "EXT (%s): Schedule new prefix %pFX with index %u on interface %s",
          __func__, p, index, ifp->name);

      /* Set first Extended Prefix then the Prefix SID information */
      set_ext_prefix (exti, OSPF_PATH_INTRA_AREA, EXT_TLV_PREF_NFLG, *p);
 ...
 */
static void
ospf_ext_link_update_adj_sid (struct ext_itf *exti)
{
  mpls_label_t label;
 ...
 */
static void
ospf_ext_link_delete_adj_sid (struct ext_itf *exti)
{
  /* Process only (LAN)Adjacency-SID Type */
  if (exti->stype != ADJ_SID && exti->stype != LAN_ADJ_SID)
    return;
 ...
 */
void
ospf_ext_link_srlb_update (void)
{
  struct listnode *node;
  struct ext_itf *exti;
 ...
 */
void
ospf_ext_update_sr (bool enable)
{
  struct listnode *node;
  struct ext_itf *exti;
      ...
      for (ALL_LIST_ELEMENTS_RO (OspfEXT.iflist, node, exti))
        {
          ...
          if (CHECK_FLAG (exti->flags, EXT_LPFLG_LSA_ENGAGED))
            ...
            ospf_ext_lsa_schedule (exti, REORIGINATE_THIS_LSA);
        }
    }
  else
    {
...
static int
ospf_ext_link_new_if (struct interface *ifp)
{
  struct ext_itf *new;
  int rc = -1;
...
static int
ospf_ext_link_del_if (struct interface *ifp)
{
  struct ext_itf *exti;
  int rc = -1;
 ...
 */
static void
ospf_ext_ism_change (struct ospf_interface *oi, int old_status)
{
  struct ext_itf *exti;
  ...
  if (exti == NULL)
    {
      flog_warn (EC_OSPF_EXT_LSA_UNEXPECTED,
                 "EXT (%s): Cannot get Extended info. from OI(%s)", __func__,
                 IF_NAME (oi));
      return;
    }
  ...
  if (oi->type == OSPF_IFTYPE_LOOPBACK)
    {
      ...
      if (OspfEXT.enabled)
        {
          osr_debug ("EXT (%s): Set Prefix SID to interface %s ", __func__,
                     oi->ifp->name);
  ...
  else
    {
       ...
       */
      if (OspfEXT.enabled)
        osr_debug ("EXT (%s): Set %sAdjacency SID for interface %s ", __func__,
 ...
 */
static void
ospf_ext_link_nsm_change (struct ospf_neighbor *nbr, int old_status)
{
  ...
  if (oi->area == NULL || oi->area->ospf == NULL)
    {
      flog_warn (EC_OSPF_EXT_LSA_UNEXPECTED,
                 "EXT (%s): Cannot refer to OSPF from OI(%s)", __func__,
                 IF_NAME (oi));
      return;
    }

  /* Remove Extended Link if Neighbor State goes Down or Deleted */
  if (OspfEXT.enabled && (nbr->state == NSM_Down || nbr->state == NSM_Deleted))
    {
      ospf_ext_link_delete_adj_sid (exti);
      if (CHECK_FLAG (exti->flags, EXT_LPFLG_LSA_ENGAGED))
...
static int
ospf_ext_link_lsa_update (struct ospf_lsa *lsa)
{
  /* Sanity Check */
  if (lsa == NULL)
    {
      flog_warn (EC_OSPF_LSA_NULL, "EXT (%s): Abort! LSA is NULL", __func__);
      return -1;
    }
...
static int
ospf_ext_pref_lsa_update (struct ospf_lsa *lsa)
{
...
static void
build_tlv (struct stream *s, struct tlv_header *tlvh)
{
...
static void
ospf_ext_pref_lsa_body_set (struct stream *s, struct ext_itf *exti)
{
...
static void
ospf_ext_link_lsa_body_set (struct stream *s, struct ext_itf *exti)
{
  size_t size;
  ...
  if (exti->stype == ADJ_SID)
    {
      ...
      size =
          EXT_TLV_LINK_SIZE + 2 * EXT_SUBTLV_ADJ_SID_SIZE + 2 * TLV_HDR_SIZE;
      if (ntohs (TLV_TYPE (exti->rmt_itf_addr)) != 0)
        size = size + EXT_SUBTLV_RMT_ITF_ADDR_SIZE + TLV_HDR_SIZE;
  ...
  else
    {
      ...
      size =
          EXT_TLV_LINK_SIZE + 2 * (EXT_SUBTLV_LAN_ADJ_SID_SIZE + TLV_HDR_SIZE);
      TLV_LEN (exti->link) = htons (size);
...
ospf_ext_pref_lsa_new (struct ospf_area *area, struct ext_itf *exti)
{
  struct stream *s;
...
static struct ospf_lsa *
ospf_ext_link_lsa_new (struct ospf_area *area, struct ext_itf *exti)
{
  struct stream *s;
  struct lsa_header *lsah;
  struct ospf_lsa *new = NULL;
 ...
 */
static int
ospf_ext_link_lsa_originate1 (struct ospf_area *area, struct ext_itf *exti)
{
  struct ospf_lsa *new;
  int rc = -1;
...
static int
ospf_ext_pref_lsa_originate (void *arg)
{
  ...
  if (! OspfEXT.enabled)
    {
      zlog_info ("EXT (%s): Segment Routing functionality is Disabled now",
                 __func__);
      rc = 0; /* This is not an error case. */
      return rc;
    }
  osr_debug ("EXT (%s): Start Originate Prefix LSA for area %pI4", __func__,
             &area->area_id);
  ...
  for (ALL_LIST_ELEMENTS_RO (OspfEXT.iflist, node, exti))
    {
      ...
      if (CHECK_FLAG (exti->flags, EXT_LPFLG_LSA_ENGAGED))
        {
          if (CHECK_FLAG (exti->flags, EXT_LPFLG_LSA_FORCED_REFRESH))
            {
              flog_warn (EC_OSPF_EXT_LSA_UNEXPECTED,
                         "EXT (%s): Refresh instead of Originate", __func__);
              UNSET_FLAG (exti->flags, EXT_LPFLG_LSA_FORCED_REFRESH);
              ospf_ext_pref_lsa_schedule (exti, REFRESH_THIS_LSA);
            }
          continue;
        }
      ...
      osr_debug (
          "EXT (%s): Let's finally re-originate the LSA 7.0.0.%u for Itf %s",
          __func__, exti->instance, exti->ifp ? exti->ifp->name : "");
...
static int
ospf_ext_link_lsa_originate (void *arg)
{
  struct ospf_area *area = (struct ospf_area *) arg;
  struct listnode *node;
  struct ext_itf *exti;
...
static struct ospf_lsa *
ospf_ext_pref_lsa_refresh (struct ospf_lsa *lsa)
{
  struct ospf_lsa *new = NULL;
  ...
  if ((exti != NULL) && ! CHECK_FLAG (exti->flags, EXT_LPFLG_LSA_ACTIVE))
    {
      flog_warn (EC_OSPF_EXT_LSA_UNEXPECTED,
                 "EXT (%s): Interface was Disabled: Flush it!", __func__);
      /* Flush it anyway. */
      lsa->data->ls_age = htons (OSPF_LSA_MAXAGE);
    }
...
static struct ospf_lsa *
ospf_ext_link_lsa_refresh (struct ospf_lsa *lsa)
{
  struct ext_itf *exti;
  struct ospf_area *area = lsa->area;
  ...
  /* Debug logging. */
  osr_debug ("EXT (%s): LSA[Type%u:%pI4]: Refresh Extended Link LSA", __func__,
...
static void
ospf_ext_pref_lsa_schedule (struct ext_itf *exti, enum lsa_opcode opcode)
{
  struct ospf_lsa lsa;
  struct lsa_header lsah;
  struct ospf *top;
  ...
  if (exti->area == NULL)
    {
      osr_debug ("EXT (%s): Area is not yet set. Try to use Backbone Area",
                 __func__);
  ...
  switch (opcode)
    ...
    case REORIGINATE_THIS_LSA:
      ospf_opaque_lsa_reoriginate_schedule ((void *) exti->area,
...
static void
ospf_ext_link_lsa_schedule (struct ext_itf *exti, enum lsa_opcode opcode)
{
  struct ospf_lsa lsa;
  struct lsa_header lsah;
  struct ospf *top;
...
static void
ospf_ext_lsa_schedule (struct ext_itf *exti, enum lsa_opcode op)
{
...
static uint16_t
show_vty_ext_link_rmt_itf_addr (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_ext_link_adj_sid (struct vty *vty, struct tlv_header *tlvh)
{
...
static uint16_t
show_vty_ext_link_lan_adj_sid (struct vty *vty, struct tlv_header *tlvh)
{
  struct ext_subtlv_lan_adj_sid *top = (struct ext_subtlv_lan_adj_sid *) tlvh;
...
static uint16_t
show_vty_link_info (struct vty *vty, struct tlv_header *ext, size_t buf_size)
{
...
/* Extended Link TLVs */
static void
ospf_ext_link_show_info (struct vty *vty, struct json_object *json,
                         struct ospf_lsa *lsa)
{
...
/* Prefix SID SubTLV */
static uint16_t
show_vty_ext_pref_pref_sid (struct vty *vty, struct tlv_header *tlvh)
{
  struct ext_subtlv_prefix_sid *top = (struct ext_subtlv_prefix_sid *) tlvh;
  ...
  vty_out (
      ...
      CHECK_FLAG (top->flags, EXT_SUBTLV_PREFIX_SID_VFLG) ? "Label" : "Index",
...
static uint16_t
show_vty_pref_info (struct vty *vty, struct tlv_header *ext, size_t buf_size)
{
...
static void
ospf_ext_pref_show_info (struct vty *vty, struct json_object *json,
                         struct ospf_lsa *lsa)
{
  struct lsa_header *lsah = lsa->data;
  struct tlv_header *tlvh;
