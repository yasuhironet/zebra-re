...
void
create_session_counters (pcep_session *session)
{
  /*
   * Message RX and TX counters
   */
  struct counters_subgroup *rx_msg_subgroup = create_counters_subgroup (
      "RX Message counters", COUNTER_SUBGROUP_ID_RX_MSG, PCEP_TYPE_MAX + 1);
  ...
  struct counters_subgroup *tx_msg_subgroup = clone_counters_subgroup (
      rx_msg_subgroup, "TX Message counters", COUNTER_SUBGROUP_ID_TX_MSG);
  ...
  struct counters_subgroup *tx_obj_subgroup = clone_counters_subgroup (
  ...
  struct counters_subgroup *tx_subobj_subgroup =
      clone_counters_subgroup (rx_subobj_subgroup, "TX RO Sub-Object counters",
  ...
  struct counters_subgroup *rx_tlv_subgroup =
      create_counters_subgroup ("RX TLV counters", COUNTER_SUBGROUP_ID_RX_TLV,
                                PCEP_OBJ_TLV_TYPE_UNKNOWN + 1);
  create_subgroup_counter (rx_tlv_subgroup, PCEP_OBJ_TLV_TYPE_NO_PATH_VECTOR,
                           "TLV No Path Vector");
  ...
  create_subgroup_counter (rx_tlv_subgroup, PCEP_OBJ_TLV_TYPE_LSP_ERROR_CODE,
                           "TLV LSP Error Code");
  create_subgroup_counter (rx_tlv_subgroup, PCEP_OBJ_TLV_TYPE_RSVP_ERROR_SPEC,
                           "TLV RSVP Error Spec");
  create_subgroup_counter (rx_tlv_subgroup, PCEP_OBJ_TLV_TYPE_LSP_DB_VERSION,
                           "TLV LSP DB Version");
  create_subgroup_counter (rx_tlv_subgroup,
  ...
  create_subgroup_counter (rx_tlv_subgroup, PCEP_OBJ_TLV_TYPE_PATH_SETUP_TYPE,
                           "TLV Path Setup Type");
  create_subgroup_counter (rx_tlv_subgroup,
                           PCEP_OBJ_TLV_TYPE_PATH_SETUP_TYPE_CAPABILITY,
                           "TLV Path Setup Type Capability");
  create_subgroup_counter (rx_tlv_subgroup, PCEP_OBJ_TLV_TYPE_SRPOLICY_POL_ID,
                           "TLV SR Policy PolId");
  create_subgroup_counter (rx_tlv_subgroup,
  ...
  create_subgroup_counter (events_subgroup, PCEP_EVENT_COUNTER_ID_PCC_CONNECT,
                           "PCC connect");
  create_subgroup_counter (events_subgroup, PCEP_EVENT_COUNTER_ID_PCE_CONNECT,
                           "PCE connect");
  create_subgroup_counter (
      events_subgroup, PCEP_EVENT_COUNTER_ID_PCC_DISCONNECT, "PCC disconnect");
  create_subgroup_counter (
      events_subgroup, PCEP_EVENT_COUNTER_ID_PCE_DISCONNECT, "PCE disconnect");
...
void
increment_message_counters (pcep_session *session,
                            struct pcep_message *message, bool is_rx)
{
  uint16_t counter_subgroup_id_msg =
      (is_rx ? COUNTER_SUBGROUP_ID_RX_MSG : COUNTER_SUBGROUP_ID_TX_MSG);
  uint16_t counter_subgroup_id_obj =
      (is_rx ? COUNTER_SUBGROUP_ID_RX_OBJ : COUNTER_SUBGROUP_ID_TX_OBJ);
  uint16_t counter_subgroup_id_subobj =
      (is_rx ? COUNTER_SUBGROUP_ID_RX_SUBOBJ : COUNTER_SUBGROUP_ID_TX_SUBOBJ);
  uint16_t counter_subgroup_id_ro_sr_subobj =
      (is_rx ? COUNTER_SUBGROUP_ID_RX_RO_SR_SUBOBJ
             : COUNTER_SUBGROUP_ID_TX_RO_SR_SUBOBJ);
  uint16_t counter_subgroup_id_tlv =
      (is_rx ? COUNTER_SUBGROUP_ID_RX_TLV : COUNTER_SUBGROUP_ID_TX_TLV);

  increment_counter (session->pcep_session_counters, counter_subgroup_id_msg,
                     message->msg_header->type);
          ...
          for (; ro_subobj_node != NULL;
               ro_subobj_node = ro_subobj_node->next_node)
            {
              struct pcep_object_ro_subobj *ro_subobj =
                  (struct pcep_object_ro_subobj *) ro_subobj_node->data;
              increment_counter (session->pcep_session_counters,
...
void
increment_message_rx_counters (pcep_session *session,
...
void
increment_message_tx_counters (pcep_session *session,
                               struct pcep_message *message)
{
...
void
increment_event_counters (pcep_session *session,
                          pcep_session_counters_event_counter_ids counter_id)
{
  increment_counter (session->pcep_session_counters, COUNTER_SUBGROUP_ID_EVENT,
                     counter_id);
}
