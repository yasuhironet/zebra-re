...
extern "C"
{
   ...
   * When creating objects, any objects passed into these APIs will be free'd
   * when the enclosing pcep_message is free'd. That includes the
   * double_linked_list's. So, just create the objects and TLVs, put them in
   ...
   * pcep_obj_free_object() and pcep_obj_free_tlv(). For received messages with
   * objects, call pcep_msg_free_message() to free them.
   */
...
#define OBJECT_SWITCH_LAYER_FLAG_I 0x01
  ...
  {
    struct pcep_object_header header;
    double_linked_list *switch_layer_rows; /* list of struct
 ...
 * 0                   1                   2                   3
   ...
   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ Entry
 */
#define ENCODE_SR_ERO_SID(label_20bits, tc_3bits, stack_bottom_bit,           \
                          ttl_8bits)                                          \
  ((((label_20bits) << 12) & 0xfffff000) | (((tc_3bits) << 9) & 0x00000e00) | \
...
#define GET_SR_ERO_SID_TTL(SID)   ((SID & 0x000000ff))
   ...
   * The message containing the objects should be converted to Network byte
   * order with pcep_encode_msg_header() before sending, which will also
   * convert the Objects, TLVs, and sub-objects.
   */
  ...
  struct pcep_object_metric *
  pcep_obj_create_metric (enum pcep_metric_types type, bool flag_b,
                          bool flag_c, float value);
  struct pcep_object_lspa *
  ...
  struct pcep_object_srp *pcep_obj_create_srp (bool lsp_remove,
                                               ...
                                               double_linked_list *tlv_list);
  struct pcep_object_lsp *
  pcep_obj_create_lsp (uint32_t plsp_id,
                       enum pcep_lsp_operational_status status, bool c_flag,
                       bool a_flag, bool r_flag, bool s_flag, bool d_flag,
                       double_linked_list *tlv_list);
  struct pcep_object_vendor_info *
  ...
  struct pcep_ro_subobj_ipv4 *
  pcep_obj_create_ro_subobj_ipv4 (bool loose_hop,
                                  const struct in_addr *ro_ipv4,
                                  uint8_t prefix_len, bool flag_local_prot);
  struct pcep_ro_subobj_ipv6 *
  pcep_obj_create_ro_subobj_ipv6 (bool loose_hop,
                                  const struct in6_addr *ro_ipv6,
                                  uint8_t prefix_len, bool flag_local_prot);
  struct pcep_ro_subobj_unnum *
  pcep_obj_create_ro_subobj_unnum (struct in_addr *router_id, uint32_t if_id);
  struct pcep_ro_subobj_32label *
  pcep_obj_create_ro_subobj_32label (bool flag_global_label,
                                     uint8_t class_type, uint32_t label);
  struct pcep_ro_subobj_asn *pcep_obj_create_ro_subobj_asn (uint16_t asn);

  /* SR ERO and SR RRO creation functions for different NAI (Node/Adj ID)
   * types.
   *  - The loose_hop is only used for sr ero and must always be false for sr
   * rro.
   *  - The NAI value will be set internally, depending on which function is
   * used. m_flag:
