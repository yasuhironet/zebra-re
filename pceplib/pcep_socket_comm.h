...
typedef int (*message_ready_to_read_handler) (void *session_data,
                                              int socket_fd);
 ...
 * If an internal pthread infrastructure is to be used, then it is not
 * necessary to explicitly call initialize_socket_comm_loop() as it will be
 * called internally when a socket comm session is initialized. */
...
/* Sets a flag to close the TCP connection either after all the pending
 * messages are written, or if there are no pending messages, the next time the
 * socket is checked to be writeable. */
