...
bool handle_pcep_initiate (pcep_session *session,
                           struct pcep_message *init_msg);
...
void
send_keep_alive (pcep_session *session)
{
...
void
send_pcep_error_with_object (pcep_session *session,
                             enum pcep_error_type error_type,
                             enum pcep_error_value error_value,
                             struct pcep_object_header *object)
{
  ...
  struct pcep_message *error_msg =
      pcep_msg_create_error_with_objects (error_type, error_value, obj_list);
...
void
send_pcep_error (pcep_session *session, enum pcep_error_type error_type,
                 enum pcep_error_value error_value)
{
  struct pcep_message *error_msg =
...
void
reset_dead_timer (pcep_session *session)
{
  ...
  if (session->timer_id_dead_timer == TIMER_ID_NOT_SET)
    {
      ...
      pcep_log (
          ...
          __func__, time (NULL), pthread_self (), dead_timer_seconds,
...
void
enqueue_event (pcep_session *session, pcep_event_type event_type,
 ...
 * back to the sender. */
bool
verify_pcep_open_object (pcep_session *session,
                         struct pcep_object_open *open_object)
{
  int retval = true;
  ...
  if (open_object->open_deadtimer < session->pcc_config.min_dead_timer_seconds)
    {
      ...
      open_object->open_deadtimer = session->pcc_config.min_dead_timer_seconds;
      retval = false;
    }
           ...
           session->pcc_config.max_dead_timer_seconds)
    {
      pcep_log (LOG_INFO,
                "%s: Rejecting unsupported Open Dead Timer value [%d]",
                __func__, open_object->open_deadtimer);
      open_object->open_deadtimer = session->pcc_config.max_dead_timer_seconds;
      retval = false;
    }
...
bool
handle_pcep_open (pcep_session *session, struct pcep_message *open_msg)
{
  ...
  if (session->pce_open_received == true &&
    ...
    {
      pcep_log (LOG_INFO, "%s: Received duplicate OPEN, replying with error",
                __func__);
      send_pcep_error (session,
                       PCEP_ERRT_ATTEMPT_TO_ESTABLISH_2ND_PCEP_SESSION,
                       PCEP_ERRV_RECVD_INVALID_OPEN_MSG);
  ...
  if (verify_pcep_open_object (session, open_object) == false)
    {
      ...
      if (session->pce_open_rejected)
      ...
      else
        {
          ...
          struct pcep_object_open *cloned_open_object = pceplib_malloc (
              PCEPLIB_MESSAGES, sizeof (struct pcep_object_open));
...
void
send_reconciled_pcep_open (pcep_session *session,
                           struct pcep_message *error_msg)
{
      ...
      if (error_open_obj->open_deadtimer >=
              ...
              session->pce_config.max_dead_timer_seconds)
        {
          ...
          pcep_log (
              ...
              __func__, session->pcc_config.dead_timer_seconds,
              session->pcc_config.dead_timer_pce_negotiated_seconds);
      ...
      if (error_open_obj->open_keepalive >=
              ...
              session->pce_config.max_keep_alive_seconds)
        {
          ...
          pcep_log (
              ...
              __func__, session->pcc_config.keep_alive_seconds,
              session->pcc_config.keep_alive_pce_negotiated_timer_seconds);
...
bool
handle_pcep_update (pcep_session *session, struct pcep_message *upd_msg)
{
  ...
  if (upd_msg->obj_list == NULL)
    {
      pcep_log (LOG_INFO, "%s: Invalid PcUpd message: Message has no objects",
                __func__);
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_INFO, "%s: Invalid PcUpd message: Missing SRP object",
                __func__);
      send_pcep_error (session, PCEP_ERRT_MANDATORY_OBJECT_MISSING,
                       PCEP_ERRV_SRP_OBJECT_MISSING);
      return false;
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_INFO, "%s: Invalid PcUpd message: Missing LSP object",
                __func__);
      send_pcep_error (session, PCEP_ERRT_MANDATORY_OBJECT_MISSING,
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_INFO, "%s: Invalid PcUpd message: Missing ERO object",
                __func__);
      send_pcep_error (session, PCEP_ERRT_MANDATORY_OBJECT_MISSING,
...
bool
handle_pcep_initiate (pcep_session *session, struct pcep_message *init_msg)
{
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_INFO, "%s: Invalid PcInitiate message: Missing SRP object",
                __func__);
      send_pcep_error (session, PCEP_ERRT_MANDATORY_OBJECT_MISSING,
                       PCEP_ERRV_SRP_OBJECT_MISSING);
      return false;
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_INFO, "%s: Invalid PcInitiate message: Missing LSP object",
                __func__);
      send_pcep_error (session, PCEP_ERRT_MANDATORY_OBJECT_MISSING,
                       PCEP_ERRV_LSP_OBJECT_MISSING);
      return false;
...
void
increment_unknown_message (pcep_session *session)
{
      ...
      if (*((time_t *) time_node->data) > expire_time)
        {
          pceplib_free (
              PCEPLIB_INFRA,
              queue_dequeue (session->num_unknown_messages_time_queue));
  ...
  if ((int) session->num_unknown_messages_time_queue->num_entries >=
      session->pcc_config.max_unknown_messages)
    {
      pcep_log (
          ...
          session->pcc_config.max_unknown_messages, session->session_id);
...
bool
check_and_send_open_keep_alive (pcep_session *session)
{
  if (session->pce_open_received == true &&
      session->pce_open_rejected == false &&
...
void
log_pcc_pce_connection (pcep_session *session)
{
  ...
  pcep_log (
      ...
      __func__, time (NULL), pthread_self (), src_ip_buf, src_port, dst_ip_buf,
      dst_port, session->session_id, session->socket_comm_session->socket_fd);
}
...
void
handle_timer_event (pcep_session_event *event)
{
  if (event == NULL)
    {
      pcep_log (LOG_INFO, "%s: handle_timer_event NULL event", __func__);
      return;
    }
  ...
  switch (session->session_state)
    ...
    case SESSION_STATE_UNKNOWN:
      pcep_log (
          ...
          __func__, event->expired_timer_id, session->session_state,
          session->session_id);
      break;
    }
}
...
void
handle_socket_comm_event (pcep_session_event *event)
{
  if (event == NULL)
    {
      pcep_log (LOG_INFO, "%s: handle_socket_comm_event NULL event", __func__);
      return;
    }
       ...
       msg_node = msg_node->next_node)
    {
      ...
      switch (msg->msg_header->type)
        ...
        case PCEP_TYPE_PCREQ:
          ...
          send_pcep_error (session, PCEP_ERRT_CAPABILITY_NOT_SUPPORTED,
              ...
              if (obj_hdr != NULL)
                {
                  ...
                  pcep_log (
                      LOG_DEBUG, "%s: Error object [type, value] = [%s, %s]",
                      __func__, get_error_type_str (error_obj->error_type),
                      get_error_value_str (error_obj->error_type,
          ...
          if (session->session_state == SESSION_STATE_PCEP_CONNECTING)
            {
               ...
               */
              pcep_log (LOG_INFO, "%s: PCC Open message rejected by PCE",
                        __func__);
        ...
        case PCEP_TYPE_MAX:
          pcep_log (LOG_INFO, "%s: \t UnSupported message", __func__);
          send_pcep_error (session, PCEP_ERRT_CAPABILITY_NOT_SUPPORTED,
                           PCEP_ERRV_UNASSIGNED);
