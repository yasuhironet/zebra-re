...
#define PCEP_MESSAGE_HEADER_VERSION 1
  ...
  /* The obj_list is a double_linked_list of struct pcep_object_header
   * pointers.
   */
  struct pcep_message
  {
   ...
   * When creating messages, any objects and tlvs passed into these APIs will
   * be free'd when the pcep_message is free'd. That includes the
   ...
   * message will be deleted by pcep_msg_free_message() or
   * pcep_msg_free_message_list() which, in turn will call one of:
   * pcep_obj_free_object() and pcep_obj_free_tlv(). For received messages,
   * call pcep_msg_free_message() to free them.
   */

  struct pcep_message *pcep_msg_create_open (uint8_t keepalive,
  ...
  struct pcep_message *
  pcep_msg_create_notify (struct pcep_object_notify *notify,
                          double_linked_list *object_list);

  /* Message defined in RFC 8231 section 6.1. Expecting double_linked_list of
   * struct pcep_object_header* objects of type SRP, LSP, or path (ERO,
   * Bandwidth, metrics, and RRO objects). */
  struct pcep_message *
   ...
   * struct pcep_object_header* objects of type SRP and LSP for LSP deletion,
   * and may also contain Endpoints, ERO and an attribute list for LSP
   * creation. */
  struct pcep_message *
