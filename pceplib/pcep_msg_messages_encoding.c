 ...
 */
static const enum pcep_object_classes
    MANDATORY_MESSAGE_OBJECT_CLASSES[13][4] = {
 ...
 */
void
pcep_encode_message (struct pcep_message *message,
                     struct pcep_versioning *versioning)
{
  ...
  for (; node != NULL; node = node->next_node)
    {
      message_length += pcep_encode_object (node->data, versioning,
...
static bool
validate_msg_header (uint8_t msg_version, uint8_t msg_flags, uint8_t msg_type,
                     uint16_t msg_length)
{
  ...
  if (msg_length < MESSAGE_HEADER_LENGTH || (msg_length % 4) != 0)
    {
      pcep_log (LOG_INFO, "%s: Invalid PCEP message header length [%d]",
                __func__, msg_length);
      return false;
    }
  ...
  if (msg_flags != 0)
    {
      pcep_log (LOG_INFO, "%s: Invalid PCEP message header flags [0x%x]",
                __func__, msg_flags);
      return false;
    }
...
static uint16_t
pcep_decode_msg_header (const uint8_t *msg_buf, uint8_t *msg_version,
                        uint8_t *msg_flags, uint8_t *msg_type)
{
...
int32_t
pcep_decode_validate_msg_header (const uint8_t *msg_buf)
{
  uint8_t msg_version;
  uint8_t msg_flags;
  uint8_t msg_type;
  ...
  msg_length =
      pcep_decode_msg_header (msg_buf, &msg_version, &msg_flags, &msg_type);
...
bool
validate_message_objects (struct pcep_message *msg)
{
  ...
  for (node = (msg->obj_list == NULL ? NULL : msg->obj_list->head), index = 0;
       ...
       index++, (node = (node == NULL ? NULL : node->next_node)))
    {
      struct pcep_object_header *obj =
          ((node == NULL) ? NULL : (struct pcep_object_header *) node->data);
...
void
destroy_pcep_versioning (struct pcep_versioning *versioning)
{
