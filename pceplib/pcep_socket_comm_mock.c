...
void
setup_mock_socket_comm_info (void)
{
...
void
teardown_mock_socket_comm_info (void)
{
  dll_destroy (mock_socket_metadata.sent_message_list);
}

void
reset_mock_socket_comm_info (void)
{
...
void
verify_socket_comm_times_called (int initialized, int teardown, int connect,
                                 int send_message, int close_tcp_after_write,
                                 int close_tcp, int destroy)
{
  CU_ASSERT_EQUAL (
      initialized,
...
socket_comm_session_initialize (
    ...
    connection_except_notifier notifier, struct in_addr *dst_ip,
    ...
    void *session_data)
{
  ...
  comm_session->dest_sock_addr.dest_sock_addr_ipv6.sin6_port =
      htons (dst_port);
  memcpy (&comm_session->dest_sock_addr.dest_sock_addr_ipv6.sin6_addr, dst_ip,
    ...
    bool is_tcp_auth_md5, void *session_data)
{
  ...
  if (src_ip == NULL)
    ...
    {
      memcpy (&comm_session->src_sock_addr.src_sock_addr_ipv6.sin6_addr,
...
socket_comm_session_send_message (
    ...
    unsigned int msg_length, bool delete_after_send)
{
  (void) socket_comm_session;
  (void) msg_length;
