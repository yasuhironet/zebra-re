...
static pcep_session_event *
create_session_event (pcep_session *session)
{
 ...
 * state machine. */
int
session_logic_msg_ready_handler (void *data, int socket_fd)
{
  if (data == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot handle msg_ready with NULL data",
                __func__);
      return -1;
    }
  ...
  if (msg_list == NULL)
    {
      ...
      pcep_log (LOG_INFO, "%s: PCEP connection closed for session [%d]",
  ...
  else
    {
      ...
      pcep_log (
          ...
          __func__, time (NULL), pthread_self (), msg->msg_header->type,
          msg->encoded_message_length, session->session_id);
...
void
session_logic_message_sent_handler (void *data, int socket_fd)
{
  (void) socket_fd;

  if (data == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot handle msg_sent with NULL data",
                __func__);
      return;
    }
...
void
session_logic_conn_except_notifier (void *data, int socket_fd)
{
  if (data == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot handle conn_except with NULL data",
                __func__);
      return;
    }
  ...
  pcep_log (
      ...
      __func__, time (NULL), pthread_self (), socket_fd, session->session_id);
 ...
 */
void
session_logic_timer_expire_handler (void *data, int timer_id)
{
  if (data == NULL)
    {
  ...
  while (session_logic_handle->active)
    {
      ...
      pcep_session_event *event =
          queue_dequeue (session_logic_handle->session_event_queue);
      while (event != NULL)
        {
          if (event->session == NULL)
            {
              pcep_log (
                  ...
                  __func__, time (NULL), pthread_self (),
                  (event->expired_timer_id != TIMER_ID_NOT_SET) ? "timer"
                                                                : "message");
          ...
          if (ordered_list_find (session_logic_handle->session_list,
                                 event->session) == NULL)
            {
              pcep_log (
                  LOG_INFO,
          ...
          pthread_mutex_unlock (&(session_logic_handle->session_list_mutex));
        }
