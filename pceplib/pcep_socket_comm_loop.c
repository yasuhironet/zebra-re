...
void write_message (int socket_fd, const char *message,
...
bool
comm_session_exists (pcep_socket_comm_handle *socket_comm_handle,
...
bool
comm_session_exists_locking (pcep_socket_comm_handle *socket_comm_handle,
                             pcep_socket_comm_session *socket_comm_session)
{
  if (socket_comm_handle == NULL)
    {
...
void
write_message (int socket_fd, const char *message, unsigned int msg_length)
{
  ...
  while ((uint32_t) bytes_sent < msg_length)
    {
      ...
      pcep_log (
          ...
          __func__, time (NULL), pthread_self (), socket_fd, msg_length,
          bytes_sent);
...
unsigned int
read_message (int socket_fd, char *received_message,
              unsigned int max_message_size)
{
...
int
build_fd_sets (pcep_socket_comm_handle *socket_comm_handle)
{
  int max_fd = 0;
...
void
handle_reads (pcep_socket_comm_handle *socket_comm_handle)
{
      ...
      if (is_set)
        {
          ...
          if (comm_session->message_handler != NULL)
            {
              received_bytes = read_message (comm_session->socket_fd,
          ...
          else
            {
               ...
               * closed. */
              received_bytes = comm_session->message_ready_to_read_handler (
          ...
          if (received_bytes == 0)
            {
              if (comm_session_exists_locking (socket_comm_handle,
                                               comm_session))
                {
                  ...
                  ordered_list_remove_first_node_equals (
                      socket_comm_handle->read_list, comm_session);
          ...
          else if (received_bytes < 0)
            {
               ...
               * here ? */
              pcep_log (
                  LOG_WARNING, "%s: Error on socket fd [%d] : errno [%d][%s]",
                  __func__, comm_session->socket_fd, errno, strerror (errno));
            }
          else
            {
              comm_session->received_bytes = received_bytes;
...
void
handle_writes (pcep_socket_comm_handle *socket_comm_handle)
{
  pthread_mutex_lock (&(socket_comm_handle->socket_comm_mutex));
              ...
              if (queued_message->free_after_send)
                {
                  pceplib_free (PCEPLIB_MESSAGES,
                                (void *) queued_message->encoded_message);
                }
          ...
          if (comm_session->message_queue->num_entries == 0)
            {
               ...
               * problems. */
              pcep_log (LOG_DEBUG, "%s: handle_writes close() socket fd [%d]",
      ...
      if (comm_session->message_sent_handler != NULL && msg_written == true)
        {
          ...
          pthread_mutex_unlock (&(socket_comm_handle->socket_comm_mutex));
          comm_session->message_sent_handler (comm_session->session_data,
                                              comm_session->socket_fd);
          pthread_mutex_lock (&(socket_comm_handle->socket_comm_mutex));
        }
    }
...
void
handle_excepts (pcep_socket_comm_handle *socket_comm_handle)
{
  /* TODO finish this */
  ...
  while (socket_comm_handle->active)
    {
      ...
      if (select (max_fd, &(socket_comm_handle->read_master_set),
                  &(socket_comm_handle->write_master_set),
                  &(socket_comm_handle->except_master_set), &timer) < 0)
        {
...
int
pceplib_external_socket_read (int fd, void *payload)
{
  pcep_socket_comm_handle *socket_comm_handle =
  ...
  if (node != NULL)
    {
      socket_comm_handle->socket_read_func (
          ...
          &((pcep_socket_comm_session *) node)->external_socket_data, fd,
          socket_comm_handle);
    }
  pthread_mutex_unlock (&(socket_comm_handle->socket_comm_mutex));
...
int
pceplib_external_socket_write (int fd, void *payload)
{
  pcep_socket_comm_handle *socket_comm_handle =
      (pcep_socket_comm_handle *) payload;
  if (socket_comm_handle == NULL)
