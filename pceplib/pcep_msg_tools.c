...
struct pcep_message *
pcep_msg_get_next (double_linked_list *list, struct pcep_message *current,
                   uint8_t type)
{
...
struct pcep_object_header *
pcep_obj_get_next (double_linked_list *list,
                   struct pcep_object_header *current, uint8_t object_class)
{
  if (list == NULL || current == NULL)
    {
      return NULL;
  ...
  if (obj->tlv_list != NULL)
    {
      ...
      while ((tlv = (struct pcep_object_tlv_header *) dll_delete_first_node (
                  obj->tlv_list)) != NULL)
        {
    ...
    case PCEP_OBJ_CLASS_RRO:
      {
        if (((struct pcep_object_ro *) obj)->sub_objects != NULL)
          {
            double_linked_list_node *node =
                ((struct pcep_object_ro *) obj)->sub_objects->head;
            for (; node != NULL; node = node->next_node)
              {
                struct pcep_object_ro_subobj *ro_subobj =
                    (struct pcep_object_ro_subobj *) node->data;
                if (ro_subobj->ro_subobj_type == RO_SUBOBJ_TYPE_SR)
                  {
                    if (((struct pcep_ro_subobj_sr *) ro_subobj)->nai_list !=
                        NULL)
                      {
                        dll_destroy_with_data_memtype (
  ...
  if (message->obj_list != NULL)
    {
      ...
      while ((obj = (struct pcep_object_header *) dll_delete_first_node (
                  message->obj_list)) != NULL)
        {
...
const char *
get_message_type_str (uint8_t type)
{
  uint8_t msg_type =
...
const char *
get_object_class_str (uint8_t class)
{
...
void
pcep_msg_print (double_linked_list *msg_list)
{
  double_linked_list_node *node;
  for (node = msg_list->head; node != NULL; node = node->next_node)
    {
      ...
      for (; obj_node != NULL; obj_node = obj_node->next_node)
        {
          ...
          pcep_log (LOG_INFO, "%s: PCEP_OBJ %s", __func__,
