...
bool
initialize_pcc_infra (struct pceplib_infra_config *infra_config)
{
...
bool
initialize_pcc_wait_for_completion (void)
{
...
void
destroy_pcep_configuration (pcep_configuration *config)
{
...
pcep_session *
connect_pce (pcep_configuration *config, struct in_addr *pce_ip)
{
  return create_pcep_session (config, pce_ip);
}

pcep_session *
connect_pce_ipv6 (pcep_configuration *config, struct in6_addr *pce_ip)
{
  return create_pcep_session_ipv6 (config, pce_ip);
}

void
disconnect_pce (pcep_session *session)
{
...
void
send_message (pcep_session *session, struct pcep_message *msg,
              bool free_after_send)
{
  if (session == NULL || msg == NULL)
    {
      pcep_log (LOG_DEBUG,
                "%s: send_message NULL params session [%p] msg [%p]", __func__,
                session, msg);
...
uint32_t
event_queue_num_events_available (void)
{
  if (session_logic_event_queue_ == NULL)
    {
      pcep_log (
...
void
destroy_pcep_event (struct pcep_event *event)
{
  if (event == NULL)
    {
      pcep_log (LOG_WARNING,
                "%s: destroy_pcep_event cannot destroy NULL event", __func__);
      return;
    }
...
const char *
get_event_type_str (int event_type)
{
  switch (event_type)
    {
...
void
dump_pcep_session_counters (pcep_session *session)
{
  if (session_exists (session) == false)
    {
      pcep_log (
  ...
  strlcpy (session->pcep_session_counters->counters_group_name, counters_name,
...
void
reset_pcep_session_counters (pcep_session *session)
{
  if (session_exists (session) == false)
    {
      pcep_log (
