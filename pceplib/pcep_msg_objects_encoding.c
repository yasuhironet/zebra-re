...
uint16_t pcep_encode_obj_server_ind (struct pcep_object_header *obj,
                                     ...
                                     uint8_t *buf);
uint16_t
pcep_encode_obj_objective_function (struct pcep_object_header *obj,
                                    struct pcep_versioning *versioning,
                                    uint8_t *buf);
...
#define PCEP_ENCODERS_ARGS                                                    \
  struct pcep_object_header *, struct pcep_versioning *versioning, uint8_t *buf
...
pcep_decode_obj_open (struct pcep_object_header *hdr, const uint8_t *buf);
...
struct pcep_object_header *pcep_decode_obj_ro (struct pcep_object_header *hdr,
                                               const uint8_t *buf);
struct pcep_object_header *
pcep_decode_obj_lspa (struct pcep_object_header *hdr, const uint8_t *buf);
struct pcep_object_header *
pcep_decode_obj_svec (struct pcep_object_header *hdr, const uint8_t *buf);
struct pcep_object_header *
pcep_decode_obj_notify (struct pcep_object_header *hdr, const uint8_t *buf);
struct pcep_object_header *
pcep_decode_obj_error (struct pcep_object_header *hdr, const uint8_t *buf);
struct pcep_object_header *
pcep_decode_obj_close (struct pcep_object_header *hdr, const uint8_t *buf);
...
/*
 * The TLVs can have strange length values, since they do not include padding
 * in the TLV header length, but that extra padding must be taken into account
 * by the enclosing object by rounding up to the next 4 byte boundary. Example
 * returned lengths: normalize_length(4)  =  4, normalize_length(5)  =  8,
 * normalize_length(6) =  8, normalize_length(7)  =  8, normalize_length(8)  =
 * 8 normalize_length(9)  = 12, normalize_length(10) = 12, normalize_length(11)
 * = 12, normalize_length(12) = 12, normalize_length(13) = 13...
 */
uint16_t
normalize_pcep_tlv_length (uint16_t length)
{
 ...
 */
uint16_t
pcep_encode_object (struct pcep_object_header *object_hdr,
                    struct pcep_versioning *versioning, uint8_t *buf)
{

  if (object_hdr->object_class >= MAX_OBJECT_ENCODER_INDEX)
    {
      pcep_log (LOG_INFO, "%s: Cannot encode unknown Object class [%d]",
  ...
  if (obj_encoder == NULL)
    {
      pcep_log (LOG_INFO, "%s: No object encoder found for Object class [%d]",
                __func__, object_hdr->object_class);
      return 0;
    }
  ...
  uint16_t object_length =
      ...
      obj_encoder (object_hdr, versioning, buf + OBJECT_HEADER_LENGTH);
  double_linked_list_node *node =
      (object_hdr->tlv_list == NULL ? NULL : object_hdr->tlv_list->head);
  for (; node != NULL; node = node->next_node)
    {
      /* Returns the length of the TLV, including the TLV header */
      object_length +=
          pcep_encode_tlv ((struct pcep_object_tlv_header *) node->data,
                           versioning, buf + object_length);
    }
...
void
write_object_header (struct pcep_object_header *object_hdr,
                     uint16_t object_length, uint8_t *buf)
{
...
uint16_t
pcep_encode_obj_open (struct pcep_object_header *hdr,
                      struct pcep_versioning *versioning,
...
uint16_t
pcep_encode_obj_rp (struct pcep_object_header *hdr,
                    struct pcep_versioning *versioning, uint8_t *obj_body_buf)
{
  (void) versioning;
  struct pcep_object_rp *rp = (struct pcep_object_rp *) hdr;
  obj_body_buf[3] =
      ((rp->flag_strict ? OBJECT_RP_FLAG_O : 0x00) |
       (rp->flag_bidirectional ? OBJECT_RP_FLAG_B : 0x00) |
       (rp->flag_reoptimization ? OBJECT_RP_FLAG_R : 0x00) |
       (rp->flag_of ? OBJECT_RP_FLAG_OF : 0x00) | (rp->priority & 0x07));
...
uint16_t
pcep_encode_obj_notify (struct pcep_object_header *hdr,
                        struct pcep_versioning *versioning,
                        uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_nopath (struct pcep_object_header *hdr,
                        struct pcep_versioning *versioning,
                        uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_association (struct pcep_object_header *hdr,
                             struct pcep_versioning *versioning,
                             uint8_t *obj_body_buf)
{
  (void) versioning;
  ...
  if (hdr->object_type == PCEP_OBJ_TYPE_ASSOCIATION_IPV4)
    {
      ...
      obj_body_buf[3] = (ipv4->R_flag ? OBJECT_ASSOCIATION_FLAG_R : 0x00);
  ...
  else
    {
      ...
      obj_body_buf[3] = (ipv6->R_flag ? OBJECT_ASSOCIATION_FLAG_R : 0x00);
...
uint16_t
pcep_encode_obj_endpoints (struct pcep_object_header *hdr,
                           struct pcep_versioning *versioning,
                           uint8_t *obj_body_buf)
{
  (void) versioning;
  ...
  if (hdr->object_type == PCEP_OBJ_TYPE_ENDPOINT_IPV4)
  ...
  else
    {
      ...
      memcpy (&uint32_ptr[4], &ipv6->dst_ipv6, sizeof (struct in6_addr));
...
uint16_t
pcep_encode_obj_bandwidth (struct pcep_object_header *hdr,
                           struct pcep_versioning *versioning,
                           uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_metric (struct pcep_object_header *hdr,
                        struct pcep_versioning *versioning,
                        uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_lspa (struct pcep_object_header *hdr,
                      struct pcep_versioning *versioning,
                      uint8_t *obj_body_buf)
{
  (void) versioning;
  ...
  obj_body_buf[14] = (lspa->flag_local_protection ? OBJECT_LSPA_FLAG_L : 0x00);

  return LENGTH_4WORDS;
}

uint16_t
pcep_encode_obj_svec (struct pcep_object_header *hdr,
                      struct pcep_versioning *versioning,
                      uint8_t *obj_body_buf)
{
  (void) versioning;
  struct pcep_object_svec *svec = (struct pcep_object_svec *) hdr;
  obj_body_buf[3] = ((svec->flag_srlg_diverse ? OBJECT_SVEC_FLAG_S : 0x00) |
...
uint16_t
pcep_encode_obj_error (struct pcep_object_header *hdr,
                       struct pcep_versioning *versioning,
                       uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_close (struct pcep_object_header *hdr,
                       struct pcep_versioning *versioning,
                       uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_srp (struct pcep_object_header *hdr,
                     struct pcep_versioning *versioning, uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_lsp (struct pcep_object_header *hdr,
                     struct pcep_versioning *versioning, uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_vendor_info (struct pcep_object_header *hdr,
                             struct pcep_versioning *versioning,
                             uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_inter_layer (struct pcep_object_header *hdr,
                             struct pcep_versioning *versioning,
                             uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_switch_layer (struct pcep_object_header *hdr,
                              struct pcep_versioning *versioning,
                              uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_req_adap_cap (struct pcep_object_header *hdr,
                              struct pcep_versioning *versioning,
                              uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_server_ind (struct pcep_object_header *hdr,
                            struct pcep_versioning *versioning,
                            uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_objective_function (struct pcep_object_header *hdr,
                                    struct pcep_versioning *versioning,
                                    uint8_t *obj_body_buf)
{
  (void) versioning;
...
uint16_t
pcep_encode_obj_ro (struct pcep_object_header *hdr,
                    struct pcep_versioning *versioning, uint8_t *obj_body_buf)
{
  (void) versioning;
        ...
        case RO_SUBOBJ_TYPE_IPV4:
          {
            ...
            obj_body_buf[index++] =
                (ipv4->flag_local_protection ? OBJECT_SUBOBJ_IP_FLAG_LOCAL_PROT
                                             : 0x00);
        ...
        case RO_SUBOBJ_TYPE_IPV6:
          {
            ...
            obj_body_buf[index++] =
                (ipv6->flag_local_protection ? OBJECT_SUBOBJ_IP_FLAG_LOCAL_PROT
                                             : 0x00);
        ...
        case RO_SUBOBJ_TYPE_LABEL:
          {
            ...
            obj_body_buf[index++] =
                (label->flag_global_label ? OBJECT_SUBOBJ_LABEL_FLAG_GLOGAL
                                          : 0x00);
        ...
        case RO_SUBOBJ_TYPE_SR:
          {
            ...
            obj_body_buf[index++] =
                ((sr_subobj->flag_f ? OBJECT_SUBOBJ_SR_FLAG_F : 0x00) |
                 (sr_subobj->flag_s ? OBJECT_SUBOBJ_SR_FLAG_S : 0x00) |
                 (sr_subobj->flag_c ? OBJECT_SUBOBJ_SR_FLAG_C : 0x00) |
                 (sr_subobj->flag_m ? OBJECT_SUBOBJ_SR_FLAG_M : 0x00));
            uint32_ptr = (uint32_t *) (obj_body_buf + index);
...
void
encode_ipv6 (struct in6_addr *src_ipv6, uint32_t *dst)
{
...
void
pcep_decode_object_hdr (const uint8_t *obj_buf,
                        struct pcep_object_header *obj_hdr)
{
...
uint16_t
pcep_object_get_length (enum pcep_object_classes object_class,
...
uint16_t
pcep_object_get_length_by_hdr (struct pcep_object_header *object_hdr)
{
...
bool
pcep_object_has_tlvs (struct pcep_object_header *object_hdr)
{
...
pcep_decode_object (const uint8_t *obj_buf)
{
  ...
  if (object_hdr.object_class >= MAX_OBJECT_ENCODER_INDEX)
    {
      pcep_log (LOG_INFO, "%s: Cannot decode unknown Object class [%d]",
                __func__, object_hdr.object_class);
      return NULL;
    }
  ...
  if (obj_decoder == NULL)
    {
      pcep_log (LOG_INFO, "%s: No object decoder found for Object class [%d]",
                __func__, object_hdr.object_class);
      return NULL;
    }
  ...
  if (object == NULL)
    {
      pcep_log (LOG_INFO, "%s: Unable to decode Object class [%d].", __func__,
  ...
  if (pcep_object_has_tlvs (&object_hdr))
    {
      ...
      while ((object->encoded_object_length - tlv_index) > 0 &&
             num_iterations++ < MAX_ITERATIONS)
        {
          ...
          tlv_index += normalize_pcep_tlv_length (tlv->encoded_tlv_length +
                                                  TLV_HEADER_LENGTH);
...
struct pcep_object_header *
pcep_decode_obj_open (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
...
struct pcep_object_header *
pcep_decode_obj_rp (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
  struct pcep_object_rp *obj = (struct pcep_object_rp *) common_object_create (
  ...
  if (hdr->object_type == PCEP_OBJ_TYPE_ASSOCIATION_IPV4)
    {
      struct pcep_object_association_ipv4 *obj =
          (struct pcep_object_association_ipv4 *) common_object_create (
              hdr, sizeof (struct pcep_object_association_ipv4));
  ...
  else if (hdr->object_type == PCEP_OBJ_TYPE_ENDPOINT_IPV6)
    {
      struct pcep_object_association_ipv6 *obj =
          (struct pcep_object_association_ipv6 *) common_object_create (
              hdr, sizeof (struct pcep_object_association_ipv6));
  ...
  if (hdr->object_type == PCEP_OBJ_TYPE_ENDPOINT_IPV4)
    {
      struct pcep_object_endpoints_ipv4 *obj =
          (struct pcep_object_endpoints_ipv4 *) common_object_create (
              hdr, sizeof (struct pcep_object_endpoints_ipv4));
  ...
  else if (hdr->object_type == PCEP_OBJ_TYPE_ENDPOINT_IPV6)
    {
      struct pcep_object_endpoints_ipv6 *obj =
          (struct pcep_object_endpoints_ipv6 *) common_object_create (
              hdr, sizeof (struct pcep_object_endpoints_ipv6));
...
struct pcep_object_header *
pcep_decode_obj_lspa (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
...
struct pcep_object_header *
pcep_decode_obj_svec (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
  ...
  if (hdr->encoded_object_length > LENGTH_2WORDS)
    {
      ...
      uint16_t index = 1;
      uint32_t *uint32_ptr = (uint32_t *) obj_buf;
      for (; index < ((hdr->encoded_object_length - LENGTH_2WORDS) / 4);
           index++)
        {
          uint32_t *req_id_ptr =
...
struct pcep_object_header *
pcep_decode_obj_error (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
...
struct pcep_object_header *
pcep_decode_obj_close (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
...
struct pcep_object_header *
pcep_decode_obj_srp (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
...
struct pcep_object_header *
pcep_decode_obj_lsp (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
  ...
  for (; i < num_rows; i++)
    {
      struct pcep_object_switch_layer_row *row = pceplib_malloc (
          PCEPLIB_MESSAGES, sizeof (struct pcep_object_switch_layer_row));
...
void
set_ro_subobj_fields (struct pcep_object_ro_subobj *subobj, bool flag_l,
                      uint8_t subobj_type)
{
...
void
decode_ipv6 (const uint32_t *src, struct in6_addr *dst_ipv6)
{
  memcpy (dst_ipv6, src, sizeof (struct in6_addr));
}
struct pcep_object_header *
pcep_decode_obj_ro (struct pcep_object_header *hdr, const uint8_t *obj_buf)
{
  struct pcep_object_ro *obj = (struct pcep_object_ro *) common_object_create (
  ...
  while ((obj_body_length - read_count) > OBJECT_RO_SUBOBJ_HEADER_LENGTH &&
         num_sub_objects < MAX_ITERATIONS)
    {
      ...
      if (subobj_length <= OBJECT_RO_SUBOBJ_HEADER_LENGTH)
        {
          pcep_log (LOG_INFO, "%s: Invalid ro subobj type [%d] length [%d]",
        ...
        case RO_SUBOBJ_TYPE_IPV4:
          {
            struct pcep_ro_subobj_ipv4 *ipv4 = pceplib_malloc (
                PCEPLIB_MESSAGES, sizeof (struct pcep_ro_subobj_ipv4));
            ...
            ipv4->flag_local_protection =
                (obj_buf[read_count++] & OBJECT_SUBOBJ_IP_FLAG_LOCAL_PROT);
        ...
        case RO_SUBOBJ_TYPE_IPV6:
          {
            struct pcep_ro_subobj_ipv6 *ipv6 = pceplib_malloc (
                PCEPLIB_MESSAGES, sizeof (struct pcep_ro_subobj_ipv6));
        ...
        case RO_SUBOBJ_TYPE_UNNUM:
          {
            struct pcep_ro_subobj_unnum *unum = pceplib_malloc (
                PCEPLIB_MESSAGES, sizeof (struct pcep_ro_subobj_unnum));
            unum->ro_subobj.flag_subobj_loose_hop = flag_l;
            unum->ro_subobj.ro_subobj_type = subobj_type;
            set_ro_subobj_fields ((struct pcep_object_ro_subobj *) unum,
                                  flag_l, subobj_type);
            uint32_ptr = (uint32_t *) (obj_buf + read_count);
        ...
        case RO_SUBOBJ_TYPE_ASN:
          {
            struct pcep_ro_subobj_asn *asn = pceplib_malloc (
                PCEPLIB_MESSAGES, sizeof (struct pcep_ro_subobj_asn));
        ...
        case RO_SUBOBJ_TYPE_SR:
          {
            ...
            struct pcep_ro_subobj_sr *sr_subobj = pceplib_malloc (
                PCEPLIB_MESSAGES, sizeof (struct pcep_ro_subobj_sr));
            ...
            switch (sr_subobj->nai_type)
              ...
              case PCEP_SR_SUBOBJ_NAI_IPV4_NODE:
                {
                  struct in_addr *ipv4 = pceplib_malloc (
                      PCEPLIB_MESSAGES, sizeof (struct in_addr));
              ...
              case PCEP_SR_SUBOBJ_NAI_IPV6_NODE:
                {
                  struct in6_addr *ipv6 = pceplib_malloc (
                      PCEPLIB_MESSAGES, sizeof (struct in6_addr));
