...
extern "C"
{
   ...
   *
   * struct counters_group *cntrs_group = create_counters_group("PCEP
   * Counters", 3); add_counters_subgroup(cntrs_group, rx_subgroup);
...
#define MAX_COUNTERS           500
   ...
   * number of counters. The subgroup_id is 0-based. counter_ids are 0-based,
   * so take that into account when setting max_counters. Return true on
   * success or false if subgroup_name is NULL, subgroup_id >=
   * MAX_COUNTER_GROUPS, or max_counters >= MAX_COUNTERS.
   */
  struct counters_subgroup *
   ...
   * This is useful for RX and TX counters: just create the RX counters and
   * clone it for the TX counters.
   */
  struct counters_subgroup *
