...
struct cmd_line_args *
get_cmdline_args (int argc, char *argv[])
{
  ...
  strlcpy (cmd_line_args->dest_ip_str, DEFAULT_DEST_HOSTNAME, MAX_DST_IP_STR);
  ...
  for (; i < argc; ++i)
    {
      if (strcmp (argv[i], "-help") == 0 || strcmp (argv[i], "--help") == 0 ||
      ...
      else if (strcmp (argv[i], "-ipv6") == 0)
        {
          ...
          if (argc == 2)
            {
              strlcpy (cmd_line_args->dest_ip_str, DEFAULT_DEST_HOSTNAME_IPV6,
                       MAX_DST_IP_STR);
            }
        }
      ...
      else if (strcmp (argv[i], "-srcip") == 0)
        {
          if (argc >= i + 2)
            {
              strlcpy (cmd_line_args->src_ip_str, argv[++i], MAX_SRC_IP_STR);
            }
          else
            {
              pcep_log (LOG_ERR,
      ...
      else if (strcmp (argv[i], "-destip") == 0)
        {
          if (argc >= i + 2)
            {
              strlcpy (cmd_line_args->dest_ip_str, argv[++i], MAX_DST_IP_STR);
            }
          else
            {
              pcep_log (LOG_ERR,
        ...
        {
          pcep_log (LOG_ERR, "%s: Invalid cmd_line_arg[%d] = %s", __func__, i,
                    argv[i]);
          free (cmd_line_args);
          return NULL;
        }
...
void
send_pce_path_request_message (pcep_session *session)
{
...
void
send_pce_report_message (pcep_session *session)
{
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_WARNING, "%s: send_pce_report_message SRP object was NULL",
                __func__);
  ...
  struct pcep_object_tlv_ipv4_lsp_identifier *ipv4_lsp_id_tlv =
      pcep_tlv_create_ipv4_lsp_identifiers (
          &ipv4_tunnel_sender, &ipv4_tunnel_endpoint, 42, 1, NULL);
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_WARNING, "%s: send_pce_report_message LSP object was NULL",
                __func__);
      dll_destroy_with_data (report_list);
      return;
    }
  ...
  struct pcep_ro_subobj_sr *sr_subobj_ipv4node =
      pcep_obj_create_ro_subobj_sr_ipv4_node (false, false, false, true, 16060,
                                              &sr_subobj_ipv4);
  ...
  if (obj == NULL)
    {
      pcep_log (LOG_WARNING, "%s: send_pce_report_message ERO object was NULL",
                __func__);
      dll_destroy_with_data (report_list);
      return;
    }
...
void
print_queue_event (struct pcep_event *event)
{
  pcep_log (
      LOG_INFO,
      "%s: [%ld-%ld] Received Event: type [%s] on session [%d] occurred at [%ld]",
      __func__, time (NULL), pthread_self (),
      get_event_type_str (event->event_type), event->session->session_id,
      event->event_time);

  if (event->event_type == MESSAGE_RECEIVED)
    {
      pcep_log (LOG_INFO, "%s: \t Event message type [%s]", __func__,
...
void
pcep_event_callback (void *cb_data, pcep_event *e)
{
  (void) cb_data;
  ...
  if (cmd_line_args->eventpoll == false)
    {
      ...
      if (! initialize_pcc_infra (&infra_config))
        {
          pcep_log (LOG_ERR, "%s: Error initializing PCC with infra.",
                    __func__);
          return -1;
        }
    }
  else
    {
      if (! initialize_pcc ())
        {
          pcep_log (LOG_ERR, "%s: Error initializing PCC.", __func__);
          return -1;
        }
    }
