...
create_counters_group (const char *group_name, uint16_t max_subgroups)
{
  if (group_name == NULL)
    {
      pcep_log (LOG_INFO,
...
create_counters_subgroup (const char *subgroup_name, uint16_t subgroup_id,
  ...
  for (; i <= subgroup->max_counters; i++)
    {
      ...
      if (counter != NULL)
        {
          create_subgroup_counter (cloned_subgroup, counter->counter_id,
...
bool
add_counters_subgroup (struct counters_group *group,
                       struct counters_subgroup *subgroup)
{
  if (group == NULL)
    {
...
bool
create_subgroup_counter (struct counters_subgroup *subgroup,
...
bool
delete_counters_group (struct counters_group *group)
{
  if (group == NULL)
    {
      pcep_log (LOG_INFO,
...
bool
delete_counters_subgroup (struct counters_subgroup *subgroup)
{
...
bool
reset_group_counters (struct counters_group *group)
{
  if (group == NULL)
    {
      pcep_log (LOG_INFO,
...
bool
reset_subgroup_counters (struct counters_subgroup *subgroup)
{
  if (subgroup == NULL)
    {
      pcep_log (
...
bool
increment_counter (struct counters_group *group, uint16_t subgroup_id,
                   uint16_t counter_id)
{
  if (group == NULL)
    {
...
bool
increment_subgroup_counter (struct counters_subgroup *subgroup,
                            uint16_t counter_id)
{
  if (subgroup == NULL)
    {
...
bool
dump_counters_group_to_log (struct counters_group *group)
{
  if (group == NULL)
    {
      pcep_log (
...
bool
dump_counters_subgroup_to_log (struct counters_subgroup *subgroup)
{
  if (subgroup == NULL)
    {
      pcep_log (
  ...
  pcep_log (LOG_INFO, "%s: \tPCEP Counters sub-group [%s] with [%d] counters",
  ...
  for (; i <= subgroup->max_counters; i++)
    {
      ...
      if (counter != NULL)
        {
          pcep_log (LOG_INFO, "%s: \t\t%s %d", __func__, counter->counter_name,
...
struct counters_subgroup *
find_subgroup (const struct counters_group *group, uint16_t subgroup_id)
{
  int i = 0;
  for (; i <= group->max_subgroups; i++)
    {
...
uint32_t
subgroup_counters_total (struct counters_subgroup *subgroup)
{
  if (subgroup == NULL)
    {
      return 0;
