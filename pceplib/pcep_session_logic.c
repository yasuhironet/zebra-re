...
static bool
run_session_logic_common (void)
{
  if (session_logic_handle_ != NULL)
    {
      pcep_log (LOG_WARNING, "%s: Session Logic is already initialized.",
                __func__);
      return false;
    }

  session_logic_handle_ =
      pceplib_malloc (PCEPLIB_INFRA, sizeof (pcep_session_logic_handle));
...
bool
run_session_logic_with_infra (pceplib_infra_config *infra_config)
{
  ...
  if (infra_config->pceplib_infra_mt != NULL &&
      infra_config->pceplib_messages_mt != NULL)
    {
      pceplib_memory_initialize (
          infra_config->pceplib_infra_mt, infra_config->pceplib_messages_mt,
  ...
  if (infra_config->pthread_create_func != NULL)
    {
      if (infra_config->pthread_create_func (
              &(session_logic_handle_->session_logic_thread), NULL,
              session_logic_loop, session_logic_handle_, "pcep_session_logic"))
        {
          pcep_log (LOG_ERR,
  ...
  if (! initialize_timers_external_infra (
          ...
          infra_config->external_infra_data, infra_config->timer_create_func,
          infra_config->timer_cancel_func, infra_config->pthread_create_func))
    {
      pcep_log (LOG_ERR,
  ...
  if (infra_config->pthread_create_func != NULL)
           ...
           infra_config->socket_write_func != NULL)
    {
      if (! initialize_socket_comm_external_infra (
              infra_config->external_infra_data,
              infra_config->socket_read_func, infra_config->socket_write_func,
              NULL))
        {
          pcep_log (
              LOG_ERR,
...
bool
run_session_logic_wait_for_completion (void)
{
...
bool
stop_session_logic (void)
{
  if (session_logic_handle_ == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Session logic already stopped", __func__);
      return false;
    }
...
void
close_pcep_session (pcep_session *session)
{
  close_pcep_session_with_reason (session, PCEP_CLOSE_REASON_NO);
}

void
close_pcep_session_with_reason (pcep_session *session,
...
void
destroy_pcep_session (pcep_session *session)
{
  if (session == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot destroy NULL session", __func__);
      return;
    }
  ...
  ordered_list_remove_first_node_equals (session_logic_handle_->session_list,
                                         session);
  pcep_log (LOG_DEBUG,
...
void
pcep_session_cancel_timers (pcep_session *session)
{
  if (session == NULL)
    {
      return;
...
/* Internal util function */
static pcep_session *
create_pcep_session_pre_setup (pcep_configuration *config)
{
  if (config == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot create pcep session with NULL config",
                __func__);
      return NULL;
    }
...
/* Internal util function */
static bool
create_pcep_session_post_setup (pcep_session *session)
{
  if (! socket_comm_session_connect_tcp (session->socket_comm_session))
    {
      pcep_log (LOG_WARNING, "%s: Cannot establish TCP socket.", __func__);
...
pcep_session *
create_pcep_session (pcep_configuration *config, struct in_addr *pce_ip)
{
  if (pce_ip == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot create pcep session with NULL pce_ip",
                __func__);
      return NULL;
    }
  ...
  session->socket_comm_session = socket_comm_session_initialize_with_src (
      ...
      session_logic_message_sent_handler, session_logic_conn_except_notifier,
      ...
      config->socket_connect_timeout_millis, config->tcp_authentication_str,
      config->is_tcp_auth_md5, session);
  if (session->socket_comm_session == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot establish socket_comm_session.",
                __func__);
      destroy_pcep_session (session);
...
pcep_session *
create_pcep_session_ipv6 (pcep_configuration *config, struct in6_addr *pce_ip)
{
  if (pce_ip == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot create pcep session with NULL pce_ip",
  ...
  if (session->socket_comm_session == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot establish ipv6 socket_comm_session.",
                __func__);
      destroy_pcep_session (session);
...
void
session_send_message (pcep_session *session, struct pcep_message *message)
{
...
struct pcep_message *
create_pcep_open (pcep_session *session)
{
      ...
      session->pcc_config.support_pce_triggered_initial_sync)
    {
      ...
      dll_append (tlv_list,
                  pcep_tlv_create_stateful_pce_capability (
                      /* U flag */
                      session->pcc_config.support_stateful_pce_lsp_update,
                      /* S flag */
                      session->pcc_config.support_include_db_version,
                      /* I flag */
                      session->pcc_config.support_pce_lsp_instantiation,
  ...
  if (session->pcc_config.support_include_db_version)
    {
      if (session->pcc_config.lsp_db_version != 0)
        {
          dll_append (tlv_list, pcep_tlv_create_lsp_db_version (
  ...
  if (session->pcc_config.support_sr_te_pst)
    {
      ...
      if (session->pcc_config.pcep_msg_versioning
              ->draft_ietf_pce_segment_routing_07 == false)
        {
      ...
      if (session->pcc_config.pcep_msg_versioning
              ->draft_ietf_pce_segment_routing_07 == true)
        {
  ...
  struct pcep_message *open_msg = pcep_msg_create_open_with_tlvs (
      ...
      session->pcc_config.dead_timer_seconds, session->session_id, tlv_list);
...
void
send_pcep_open (pcep_session *session)
{
  session_send_message (session, create_pcep_open (session));
}

/* This is a blocking call, since it is synchronized with
 * destroy_pcep_session() and session_logic_loop(). It may be possible that the
 * session has been deleted but API users havent been informed yet.
 */
bool
session_exists (pcep_session *session)
{
  if (session_logic_handle_ == NULL)
    {
      pcep_log (LOG_DEBUG, "%s: session_exists session_logic_handle_ is NULL",
                __func__);
      return false;
    }

  pthread_mutex_lock (&(session_logic_handle_->session_list_mutex));
  bool retval = (ordered_list_find (session_logic_handle_->session_list,
                                    session) != NULL);
  pthread_mutex_unlock (&(session_logic_handle_->session_list_mutex));
