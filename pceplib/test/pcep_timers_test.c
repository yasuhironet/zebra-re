...
void
pcep_timers_test_teardown ()
{
  teardown_timers ();
}

static void
test_timer_expire_handler (void *data, int timerId)
{
  (void) data;
  (void) timerId;
}
...
void
test_double_initialization (void)
{
...
void
test_initialization_null_callback (void)
{
...
void
test_cancel_timer_invalid (void)
{
  CU_ASSERT_EQUAL (initialize_timers (test_timer_expire_handler), true);
