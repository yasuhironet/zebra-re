...
test_create_counters_subgroup ()
{
  ...
  subgroup = create_counters_subgroup (subgroup_name, MAX_COUNTER_GROUPS + 1,
                                       num_counters);
  CU_ASSERT_PTR_NULL (subgroup);

  subgroup =
      create_counters_subgroup (subgroup_name, subgroup_id, MAX_COUNTERS + 1);
  CU_ASSERT_PTR_NULL (subgroup);

  subgroup =
      create_counters_subgroup (subgroup_name, subgroup_id, num_counters);
...
test_create_subgroup_counter ()
{
  ...
  CU_ASSERT_FALSE (
      create_subgroup_counter (subgroup, counter_id + 1, counter_name));
