...
int
pcep_session_logic_test_suite_setup (void)
{
...
int
pcep_session_logic_test_suite_teardown (void)
{
  printf ("\n");
  pceplib_memory_dump ();
  return 0;
...
void
pcep_session_logic_test_teardown ()
{
  stop_session_logic ();
...
test_create_pcep_session_open_tlvs ()
{
  ...
  CU_ASSERT_EQUAL (
      ((struct pcep_object_tlv_header *) open_obj->tlv_list->head->data)->type,
