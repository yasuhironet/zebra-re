 ...
 */
static void
test_message_received_handler (void *session_data, const char *message_data,
...
static int
test_message_ready_to_read_handler (void *session_data, int socket_fd)
{
  (void) session_data;
  (void) socket_fd;
  return 1;
...
static void
test_message_sent_handler (void *session_data, int socket_fd)
{
  (void) session_data;
  (void) socket_fd;
  return;
...
static void
test_connection_except_notifier (void *session_data, int socket_fd)
{
  (void) session_data;
  (void) socket_fd;
}
...
void
pcep_socket_comm_test_teardown ()
{
...
test_pcep_socket_comm_initialize_with_src ()
{
  ...
  test_session = socket_comm_session_initialize_with_src (
      test_message_received_handler, NULL, NULL,
      test_connection_except_notifier, NULL, 0, &test_host_ip, test_port,
      connect_timeout_millis, NULL, false, NULL);
  CU_ASSERT_PTR_NOT_NULL (test_session);
  assert (test_session != NULL);
  CU_ASSERT_EQUAL (
  ...
  test_session = socket_comm_session_initialize_with_src (
      ...
      &test_host_ip, test_port, connect_timeout_millis, NULL, false, NULL);
  CU_ASSERT_PTR_NOT_NULL (test_session);
  assert (test_session != NULL);
  CU_ASSERT_EQUAL (
      test_session->src_sock_addr.src_sock_addr_ipv4.sin_addr.s_addr,
...
test_pcep_socket_comm_initialize_with_src_ipv6 ()
{
  ...
  test_session = socket_comm_session_initialize_with_src_ipv6 (
      test_message_received_handler, NULL, NULL,
      test_connection_except_notifier, NULL, 0, &test_host_ipv6, test_port,
      connect_timeout_millis, NULL, false, NULL);
  CU_ASSERT_PTR_NOT_NULL (test_session);
  assert (test_session != NULL);
  CU_ASSERT_EQUAL (
      memcmp (&test_session->src_sock_addr.src_sock_addr_ipv6.sin6_addr,
  ...
  test_session = socket_comm_session_initialize_with_src_ipv6 (
      ...
      &test_host_ipv6, test_port, connect_timeout_millis, NULL, false, NULL);
  CU_ASSERT_PTR_NOT_NULL (test_session);
  assert (test_session != NULL);
  CU_ASSERT_EQUAL (
      memcmp (&test_session->src_sock_addr.src_sock_addr_ipv6.sin6_addr,
      ...
      0);
  CU_ASSERT_EQUAL (test_session->src_sock_addr.src_sock_addr_ipv6.sin6_port,
                   ntohs (test_src_port));
  CU_ASSERT_TRUE (test_session->is_ipv6);
}
...
test_pcep_socket_comm_initialize_tcpmd5 ()
{
  ...
  assert (test_session != NULL);
  CU_ASSERT_EQUAL (0,
                   strncmp (tcp_md5_str, test_session->tcp_authentication_str,
                            tcp_md5_strlen));
...
test_pcep_socket_comm_initialize_handlers ()
{
  ...
  test_session = socket_comm_session_initialize (
      NULL, NULL, NULL, test_connection_except_notifier, &test_host_ip,
      test_port, connect_timeout_millis, NULL, false, NULL);
  CU_ASSERT_PTR_NULL (test_session);

  /* Both receive handlers cannot be set */
  test_session = socket_comm_session_initialize (
      test_message_received_handler, test_message_ready_to_read_handler,
  ...
  test_session = socket_comm_session_initialize (
      NULL, test_message_ready_to_read_handler, test_message_sent_handler,
      test_connection_except_notifier, &test_host_ip, test_port,
      connect_timeout_millis, NULL, false, NULL);
  CU_ASSERT_PTR_NOT_NULL (test_session);
}
