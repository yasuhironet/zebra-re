...
int
pcep_messages_test_suite_setup (void)
{
...
int
pcep_messages_test_suite_teardown (void)
{
  printf ("\n");
  pceplib_memory_dump ();
  return 0;
...
void
pcep_messages_test_setup ()
{
  versioning = create_default_pcep_versioning ();
}

void
pcep_messages_test_teardown ()
{
...
test_pcep_msg_create_open ()
{
  ...
  CU_ASSERT_EQUAL (
      ...
      MESSAGE_HEADER_LENGTH +
          pcep_object_get_length (PCEP_OBJ_CLASS_OPEN, PCEP_OBJ_TYPE_OPEN));
...
test_pcep_msg_create_reply_nopath ()
{
  struct pcep_object_rp *rp_obj =
      pcep_obj_create_rp (0, false, false, false, false, 10, NULL);
  struct pcep_object_nopath *nopath_obj =
      pcep_obj_create_nopath (false, false, PCEP_NOPATH_TLV_ERR_NO_TLV);
  double_linked_list *obj_list = dll_initialize ();
...
test_pcep_msg_create_reply ()
{
  ...
  struct pcep_object_ro_subobj *ero_subobj =
      (struct pcep_object_ro_subobj *) pcep_obj_create_ro_subobj_32label (
          true, 1, 10);
  ...
  CU_ASSERT_EQUAL (message->encoded_message_length,
                   MESSAGE_HEADER_LENGTH +
                       pcep_object_get_length_by_hdr (&rp_obj->header) +
                       OBJECT_HEADER_LENGTH + OBJECT_RO_SUBOBJ_HEADER_LENGTH +
...
test_pcep_msg_create_update ()
{
  ...
  CU_ASSERT_EQUAL (message->obj_list->num_entries, 3);
  CU_ASSERT_EQUAL (message->encoded_message_length,
                   MESSAGE_HEADER_LENGTH + srp->header.encoded_object_length +
...
void
test_pcep_msg_create_notify (void)
{
  struct pcep_object_notify *notify_obj =
      pcep_obj_create_notify (PCEP_NOTIFY_TYPE_PENDING_REQUEST_CANCELLED,
