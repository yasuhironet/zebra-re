...
int
pcep_tlvs_test_suite_setup (void)
{
...
int
pcep_tlvs_test_suite_teardown (void)
{
  printf ("\n");
  pceplib_memory_dump ();
  return 0;
...
test_pcep_tlv_create_path_setup_type_capability ()
{
  ...
  *pst3 = 3;
  ...
  struct pcep_object_tlv_header *sub_tlv =
      (struct pcep_object_tlv_header *) pcep_tlv_create_sr_pce_capability (
          true, true, 0);
...
test_pcep_tlv_create_symbolic_path_name ()
{
  ...
  CU_ASSERT_EQUAL (
      0, strncmp ((char *) &(tlv->header.encoded_tlv[4]), &path_name[0], 4));
  CU_ASSERT_EQUAL (
      0, strncmp ((char *) &(tlv->header.encoded_tlv[8]), &path_name[4], 4));
  CU_ASSERT_EQUAL (
      0, strncmp ((char *) &(tlv->header.encoded_tlv[12]), &path_name[8], 4));
...
test_pcep_tlv_create_ipv4_lsp_identifiers ()
{
  ...
  struct pcep_object_tlv_ipv4_lsp_identifier *tlv =
      pcep_tlv_create_ipv4_lsp_identifiers (NULL, &endpoint_ip, lsp_id,
                                            tunnel_id, &extended_tunnel_id);
  CU_ASSERT_PTR_NULL (tlv);

  tlv = pcep_tlv_create_ipv4_lsp_identifiers (&sender_ip, NULL, lsp_id,
                                              tunnel_id, &extended_tunnel_id);
  CU_ASSERT_PTR_NULL (tlv);

  tlv = pcep_tlv_create_ipv4_lsp_identifiers (NULL, NULL, lsp_id, tunnel_id,
                                              &extended_tunnel_id);
  CU_ASSERT_PTR_NULL (tlv);
  assert (tlv == NULL);

  tlv = pcep_tlv_create_ipv4_lsp_identifiers (&sender_ip, &endpoint_ip, lsp_id,
                                              tunnel_id, &extended_tunnel_id);
  CU_ASSERT_PTR_NOT_NULL (tlv);
...
test_pcep_tlv_create_srpag_pol_id_ipv4 ()
{
  ...
  CU_ASSERT_EQUAL (
      0, memcmp (&tlv_buf[0] + TLV_HEADER_LENGTH, &aux_color, sizeof (color)));
...
{
  struct pcep_object_tlv_lsp_error_code *tlv = pcep_tlv_create_lsp_error_code (
