...
int
timer_list_node_compare (void *list_entry, void *new_entry)
{
  /* return:
 ...
 * its timer_id. */
int
timer_list_node_timer_id_compare (void *list_entry, void *new_entry)
{
 ...
 * its address. */
int
timer_list_node_timer_ptr_compare (void *list_entry, void *new_entry)
{
  return ((char *) new_entry - (char *) list_entry);
}

/* internal util method */
static pcep_timers_context *
...
static bool
initialize_timers_common (timer_expire_handler expire_handler)
{
...
bool
initialize_timers (timer_expire_handler expire_handler)
{
  ...
  if (pthread_create (&(timers_context_->event_loop_thread), NULL, event_loop,
                      timers_context_))
    {
      pcep_log (LOG_ERR,
...
bool
initialize_timers_external_infra (
    ...
    ext_pthread_create_callback thread_create_func)
{
  ...
  if (thread_create_func != NULL)
    {
      if (thread_create_func (&(timers_context_->event_loop_thread), NULL,
                              event_loop, timers_context_, "pceplib_timers"))
        {
          pcep_log (LOG_ERR, "%s: Cannot initialize external timers thread.",
                    __func__);
          return false;
        }
    }
 ...
 */
void
free_all_timers (pcep_timers_context *timers_context)
{
...
int
create_timer (uint16_t sleep_seconds, void *data)
{
  if (timers_context_ == NULL)
    {
      pcep_log (
  ...
  if (timers_context_->timer_create_func)
    {
      timers_context_->timer_create_func (
          timers_context_->external_timer_infra_data, &timer->external_timer,
          sleep_seconds, timer);
    }
...
reset_timer (int timer_id)
{
  ...
  if (timer_to_reset == NULL)
    {
      pthread_mutex_unlock (&timers_context_->timer_list_lock);
      pcep_log (LOG_WARNING, "%s: Trying to reset a timer that does not exist",
                __func__);
  ...
  if (timers_context_->timer_cancel_func)
    {
       ...
       * long the it takes.*/
      pcep_log (LOG_DEBUG, "%s: Resetting timer [%d] with callback", __func__,
                timer_to_reset->timer_id);
  ...
  if (timers_context_->timer_create_func)
    {
      timers_context_->timer_create_func (
          ...
          &timer_to_reset->external_timer, timer_to_reset->sleep_seconds,
          timer_to_reset);
      /* Keeping this log for now, since in older versions of FRR the
       * timer cancellation was blocking. This allows us to see how
       * long the it takes.*/
      pcep_log (LOG_DEBUG, "%s: Reset timer [%d] with callback", __func__,
                timer_to_reset->timer_id);
    }
...
void
pceplib_external_timer_expire_handler (void *data)
{
  if (timers_context_ == NULL)
    {
      pcep_log (
  ...
  if (data == NULL)
    {
      pcep_log (LOG_WARNING, "%s: External timer expired with NULL data",
                __func__);
      return;
    }
  ...
  pthread_mutex_lock (&timers_context_->timer_list_lock);
  ordered_list_node *timer_node = ordered_list_find2 (
      timers_context_->timer_list, timer, timer_list_node_timer_ptr_compare);
