...
extern "C"
{
   ...
   * When creating TLVs, any TLVs passed into messages or objects with these
   * APIs will be free'd when the the enclosing pcep_message is free'd. That
   * includes the double_linked_list's. So, just create the objects and TLVs,
   * put them in their double_linked_list's, and everything will be managed
   * internally. The enclosing message will be deleted by
   * pcep_msg_free_message() or pcep_msg_free_message_list() which, * in turn
   * will call one of: pcep_obj_free_object() and pcep_obj_free_tlv(). For
   * received messages, call pcep_msg_free_message() to free them.
   */
...
#define MAX_ARBITRARY_SIZE 256
  ...
  struct pcep_object_tlv_speaker_entity_identifier *
  pcep_tlv_create_speaker_entity_id (
      double_linked_list *speaker_entity_id_list);
  struct pcep_object_tlv_path_setup_type *
  pcep_tlv_create_path_setup_type (uint8_t pst);
  struct pcep_object_tlv_path_setup_type_capability *
  pcep_tlv_create_path_setup_type_capability (
      double_linked_list *pst_list, double_linked_list *sub_tlv_list);
  struct pcep_object_tlv_sr_pce_capability *
  ...
  struct pcep_object_tlv_srpag_pol_name *
  pcep_tlv_create_srpag_pol_name (const char *pol_name,
