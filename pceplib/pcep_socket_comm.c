...
int
socket_fd_node_compare (void *list_entry, void *new_entry)
{
...
bool
initialize_socket_comm_pre (void)
{
  socket_comm_handle_ =
  ...
  if (pthread_mutex_init (&(socket_comm_handle_->socket_comm_mutex), NULL) !=
      0)
    {
      pcep_log (LOG_ERR, "%s: Cannot initialize socket_comm mutex.", __func__);
...
bool
initialize_socket_comm_external_infra (
    ...
    ext_socket_pthread_create_callback thread_create_func)
{
  ...
  if (thread_create_func != NULL)
    {
      if (thread_create_func (&(socket_comm_handle_->socket_comm_thread), NULL,
...
bool
initialize_socket_comm_loop (void)
{
  if (socket_comm_handle_ != NULL)
    {
      /* already initialized */
    ...
    void *session_data)
{
  ...
  if (! initialize_socket_comm_loop ())
    {
      pcep_log (LOG_WARNING, "%s: ERROR: cannot initialize socket_comm_loop.",
                __func__);
...
/* Internal common init function */
bool
socket_comm_session_initialize_post (
    pcep_socket_comm_session *socket_comm_session)
{
  ...
  struct sockaddr *src_sock_addr =
      (socket_comm_session->is_ipv6
           ...
           : (struct sockaddr *) &(
                 socket_comm_session->src_sock_addr.src_sock_addr_ipv4));
  int addr_len =
      (socket_comm_session->is_ipv6
  ...
  if (bind (socket_comm_session->socket_fd, src_sock_addr, addr_len) == -1)
    {
      pcep_log (LOG_WARNING,
                "%s: Cannot bind address to socket errno [%d %s].", __func__,
    ...
    void *session_data)
{
  return socket_comm_session_initialize_with_src (
      ...
      NULL, 0, dest_ip, dest_port, connect_timeout_millis,
    ...
    bool is_tcp_auth_md5, void *session_data)
{
  ...
  pcep_socket_comm_session *socket_comm_session =
      socket_comm_session_initialize_pre (
          message_handler, message_ready_handler, msg_sent_notifier, notifier,
          connect_timeout_millis, tcp_authentication_str, is_tcp_auth_md5,
          session_data);
  ...
  if (socket_comm_session->socket_fd == -1)
    {
      pcep_log (LOG_WARNING, "%s: Cannot create ipv4 socket errno [%d %s].",
                __func__, errno, strerror (errno));
  ...
  socket_comm_session->src_sock_addr.src_sock_addr_ipv4.sin_port =
      htons (src_port);
  socket_comm_session->dest_sock_addr.dest_sock_addr_ipv4.sin_addr.s_addr =
      dest_ip->s_addr;
  if (src_ip != NULL)
    {
      socket_comm_session->src_sock_addr.src_sock_addr_ipv4.sin_addr.s_addr =
          src_ip->s_addr;
    }
  else
    {
    ...
    bool is_tcp_auth_md5, void *session_data)
{
  ...
  if (socket_comm_session->socket_fd == -1)
    {
      pcep_log (LOG_WARNING, "%s: Cannot create ipv6 socket errno [%d %s].",
                __func__, errno, strerror (errno));
      socket_comm_session_teardown (
          socket_comm_session); // socket_comm_session freed
                                // inside fn so NOLINT next.
  ...
  socket_comm_session->src_sock_addr.src_sock_addr_ipv6.sin6_port =
      htons (src_port);
  memcpy (&socket_comm_session->dest_sock_addr.dest_sock_addr_ipv6.sin6_addr,
          dest_ip, sizeof (struct in6_addr));
  if (src_ip != NULL)
    {
      memcpy (&socket_comm_session->src_sock_addr.src_sock_addr_ipv6.sin6_addr,
              src_ip, sizeof (struct in6_addr));
    }
  else
    {
      socket_comm_session->src_sock_addr.src_sock_addr_ipv6.sin6_addr =
          in6addr_any;
    }
...
bool
socket_comm_session_connect_tcp (pcep_socket_comm_session *socket_comm_session)
{
  if (socket_comm_session == NULL)
    {
      pcep_log (
  ...
  if ((fcntl_arg = fcntl (socket_comm_session->socket_fd, F_GETFL, NULL)) < 0)
    {
      pcep_log (LOG_WARNING, "%s: Error fcntl(..., F_GETFL) [%d %s]", __func__,
                errno, strerror (errno));
      return false;
    }
  ...
  if (fcntl (socket_comm_session->socket_fd, F_SETFL, fcntl_arg) < 0)
    {
      pcep_log (LOG_WARNING, "%s: Error fcntl(..., F_SETFL) [%d %s]", __func__,
                errno, strerror (errno));
      return false;
    }
  ...
  if (socket_comm_session->tcp_authentication_str[0] != '\0')
    {
      ...
      memcpy (sig.tcpm_key, socket_comm_session->tcp_authentication_str,
              sig.tcpm_keylen);
#else
      int sig = 1;
#endif
      if (setsockopt (socket_comm_session->socket_fd, IPPROTO_TCP, TCP_MD5SIG,
                      &sig, sizeof (sig)) == -1)
        {
          pcep_log (LOG_ERR, "%s: Failed to setsockopt(): [%d %s]", __func__,
                    errno, strerror (errno));
          return false;
        }
    }
      ...
      if (errno == EINPROGRESS)
        {
          ...
          if (socket_comm_session->connect_timeout_millis > 1000)
            {
              ...
              tv.tv_usec = (socket_comm_session->connect_timeout_millis -
                            (tv.tv_sec * 1000)) *
                           1000;
            }
          else
          ...
          if (select (socket_comm_session->socket_fd + 1, NULL, &fdset, NULL,
                      &tv) > 0)
            {
              int so_error;
              socklen_t len = sizeof (so_error);
              getsockopt (socket_comm_session->socket_fd, SOL_SOCKET, SO_ERROR,
                          &so_error, &len);
              if (so_error)
                {
                  pcep_log (LOG_WARNING,
...
bool
socket_comm_session_close_tcp (pcep_socket_comm_session *socket_comm_session)
{
  if (socket_comm_session == NULL)
    {
      pcep_log (LOG_WARNING,
...
bool
socket_comm_session_teardown (pcep_socket_comm_session *socket_comm_session)
{
  if (socket_comm_handle_ == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot teardown NULL socket_comm_handle",
                __func__);
      return false;
    }
  ...
  if (socket_comm_session == NULL)
    {
      pcep_log (LOG_WARNING, "%s: Cannot teardown NULL session", __func__);
      return false;
    }

  if (comm_session_exists_locking (socket_comm_handle_, socket_comm_session) ==
      false)
    {
      pcep_log (LOG_WARNING, "%s: Cannot teardown session that does not exist",
                __func__);
      return false;
    }
  ...
  pcep_log (
      ...
      __func__, time (NULL), pthread_self (), socket_comm_session->socket_fd,
      socket_comm_handle_->num_active_sessions);
...
socket_comm_session_send_message (
    pcep_socket_comm_session *socket_comm_session, const char *encoded_message,
    unsigned int msg_length, bool free_after_send)
{
  if (socket_comm_session == NULL)
    {
      pcep_log (
