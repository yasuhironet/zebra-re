...
static void babel_distribute_update (struct distribute_ctx *ctx,
...
babel_config_write (struct vty *vty)
{
  ...
  lines +=
      config_write_distribute (vty, babel_routing_process->distribute_ctx);
...
babel_create_routing_process (void)
{
  ...
  babel_routing_process->distribute_ctx =
      distribute_list_ctx_create (vrf_lookup_by_id (VRF_DEFAULT));
  distribute_list_add_hook (babel_routing_process->distribute_ctx,
                            babel_distribute_update);
  distribute_list_delete_hook (babel_routing_process->distribute_ctx,
                               babel_distribute_update);
  return 0;
fail:
...
static void
babel_read_protocol (struct event *thread)
{
  int rc;
...
static void
babel_init_routing_process (struct event *thread)
{
...
static void
babel_main_loop (struct event *thread)
{
  struct timeval tv;
  struct vrf *vrf = vrf_lookup_by_id (VRF_DEFAULT);
  struct interface *ifp = NULL;
      ...
      FOR_ALL_INTERFACES (vrf, ifp)
        {
          ...
          if (timeval_compare (&babel_now, &babel_ifp->update_flush_timeout) >=
              0)
            flushupdates (ifp);
        }
...
printIfMin (struct timeval *tv, int cmd, const char *tag, const char *ifname)
{
  ...
  switch (cmd)
    ...
    case 1: /* take the min */
      ...
      if (tv->tv_sec < curr_tv.tv_sec ||
          (tv->tv_sec == curr_tv.tv_sec && tv->tv_usec < curr_tv.tv_usec))
        {
          curr_tv = *tv;
          if (ifname != NULL)
            {
...
babel_distribute_update (struct distribute_ctx *ctx, struct distribute *dist)
{
  ...
  for (type = 0; type < DISTRIBUTE_MAX; type++)
    {
      family = type == DISTRIBUTE_V4_IN || type == DISTRIBUTE_V4_OUT ? AFI_IP
                                                                     : AFI_IP6;
      if (dist->list[type])
        babel_ifp->list[type] = access_list_lookup (family, dist->list[type]);
      else
        babel_ifp->list[type] = NULL;
      if (dist->prefix[type])
        babel_ifp->prefix[type] =
            prefix_list_lookup (family, dist->prefix[type]);
      else
...
babel_distribute_update_interface (struct interface *ifp)
{
  ...
  if (babel_routing_process)
    dist =
        distribute_lookup (babel_routing_process->distribute_ctx, ifp->name);
  if (dist)
...
int
input_filter (const unsigned char *id, const unsigned char *prefix,
