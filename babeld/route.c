  ...
  while (i < route_slots)
    {
      ...
      while (r)
        {
          if (r->neigh->ifp == ifp && (! v4only || v4mapped (r->nexthop)))
            {
              flush_route (r);
              goto again;
            }
...
switch_routes (struct babel_route *old, struct babel_route *new)
{
  ...
  rc = kernel_route (ROUTE_MODIFY, old->src->prefix, old->src->plen,
                     ...
                     metric_to_kernel (route_metric (old)), new->nexthop,
  ...
  move_installed_route (
      new, find_route_slot (new->src->prefix, new->src->plen, NULL));
}

static void
change_route_metric (struct babel_route *route, unsigned refmetric,
                     unsigned cost, unsigned add)
{
  int old, new;
  ...
  if (route->installed && old != new)
    {
      ...
      rc = kernel_route (ROUTE_MODIFY, route->src->prefix, route->src->plen,
                         route->nexthop, route->neigh->ifp->ifindex, old,
                         route->nexthop, route->neigh->ifp->ifindex, new);
      if (rc < 0)
        {
...
int
update_feasible (struct source *src, unsigned short seqno,
...
update_route_metric (struct babel_route *route)
{
  ...
  if (route_expired (route))
  ...
  else
    {
      ...
      int add_metric =
          input_filter (route->src->id, route->src->prefix, route->src->plen,
...
struct babel_route *
update_route (const unsigned char *router_id, const unsigned char *prefix,
              ...
              const unsigned char *channels, int channels_len)
{
  ...
  add_metric = input_filter (router_id, prefix, plen, neigh->address,
                             neigh->ifp->ifindex);
  ...
  metric =
      MIN ((int) refmetric + neighbour_cost (neigh) + add_metric, INFINITY);
  ...
  if (route)
    {
      ...
      change_route_metric (route, refmetric, neighbour_cost (neigh),
                           add_metric);
  ...
  if (force || ! route || route_metric (route) >= metric + 512)
    {
      send_unicast_multihop_request (
          neigh, src->prefix, src->plen,
          src->metric >= INFINITY ? src->seqno : seqno_plus (src->seqno, 1),
          src->id, 127);
    }
}
...
/* A route has just changed.  Decide whether to switch to a different route or
   send an update. */
void
route_changed (struct babel_route *route, struct source *oldsrc,
               unsigned short oldmetric)
{
  if (route->installed)
    {
          ...
          if (r->installed && r->refmetric < INFINITY)
            {
              if (route_old (r))
                ...
                send_unicast_request (r->neigh, r->src->prefix, r->src->plen);
            }
          r = r->next;
        }
      i++;
