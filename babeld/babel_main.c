...
const unsigned char ones[16] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
/* babeld privileges */
static zebra_capabilities_t _caps_p[] = { ZCAP_NET_RAW, ZCAP_BIND };
...
FRR_DAEMON_INFO (babeld, BABELD, .vty_port = BABEL_VTY_PORT,
...
main (int argc, char **argv)
{
  ...
  snprintf (state_file, sizeof (state_file), "%s/%s", frr_vtydir,
            "babel-state");
              ...
              else
                {
                  ...
                  if (memcmp (sid, myid, 8) == 0)
                    ...
                    flog_err (EC_BABEL_CONFIG,
                              ...
                              format_eui64 (myid), format_eui64 (sid));
                }
            }
          else
            {
  ...
  if (fd < 0)
  ...
  else
    {
      ...
      rc = snprintf (buf, 100, "%s %d %ld\n", format_eui64 (myid),
                     (int) myseqno, (long) realnow.tv_sec);
      if (rc < 0 || rc >= 100)
        {
...
show_babel_main_configuration (struct vty *vty)
{
  vty_out (vty,
           ...
           "kernel_metric           = %d\n",
           state_file,
           babeld_di.config_file ? babeld_di.config_file
                                 : babel_config_default,
           format_address (protocol_group), protocol_port,
           babel_vty_addr ? babel_vty_addr : "None", babel_vty_port,
           format_eui64 (myid), kernel_metric);
}
