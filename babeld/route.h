...
{
  int m = (int) route->refmetric +
          (diversity_factor * route->cost + 128) / 256 + route->add_metric;
  m = MAX (m, route->refmetric + 1);
  return MIN (m, INFINITY);
}

struct babel_route *find_route (const unsigned char *prefix,
                                unsigned char plen, struct neighbour *neigh,
...
int update_feasible (struct source *src, unsigned short seqno,
                     unsigned short refmetric);
...
struct babel_route *find_best_route (const unsigned char *prefix,
                                     unsigned char plen, int feasible,
...
struct babel_route *
update_route (const unsigned char *id, const unsigned char *prefix,
...
void send_unfeasible_request (struct neighbour *neigh, int force,
                              ...
                              struct source *src);
void send_triggered_update (struct babel_route *route, struct source *oldsrc,
                            unsigned oldmetric);
void route_changed (struct babel_route *route, struct source *oldsrc,
