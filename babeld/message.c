...
static const unsigned char tlv_min_length[MESSAGE_MAX + 1] = {
  [MESSAGE_PAD1] = 0,       [MESSAGE_PADN] = 0,        [MESSAGE_ACK_REQ] = 6,
  [MESSAGE_ACK] = 2,        [MESSAGE_HELLO] = 6,       [MESSAGE_IHU] = 6,
  [MESSAGE_ROUTER_ID] = 10, [MESSAGE_NH] = 2,          [MESSAGE_UPDATE] = 10,
  [MESSAGE_REQUEST] = 2,    [MESSAGE_MH_REQUEST] = 14,
};
...
static int
network_prefix (int ae, int plen, unsigned int omitted, const unsigned char *p,
  ...
  while (i < alen)
    {
      ...
      if (type & SUBTLV_MANDATORY)
        ...
        {
          debugf (BABEL_DEBUG_COMMON, "Received unknown route attribute %d.",
                  type);
        }
  ...
  while (i < alen)
    {
      ...
      if (type == SUBTLV_PADN)
        ...
        {
          debugf (BABEL_DEBUG_COMMON, "Received unknown IHU sub-TLV type %d.",
                  type);
        }
      ...
      if (type == SUBTLV_PADN)
      ...
      else if (type == SUBTLV_SOURCE_PREFIX)
        {
          ...
          if (have_src_prefix != 0)
            goto fail;
          rc = network_prefix (ae, a[i + 2], 0, a + i + 3, NULL, len - 1,
                               src_prefix);
          if (rc < 0)
            goto fail;
          if (ae == 1)
              ...
              const unsigned char *packet, int packetlen)
{
  ...
  if (babel_packet_examin (packet, packetlen, &bodylen))
    {
      flog_err (EC_BABEL_PACKET, "Received malformed packet on %s from %s.",
  ...
  while (i < bodylen)
    {
      ...
      if (type == MESSAGE_PADN)
      ...
      else if (type == MESSAGE_ACK_REQ)
        {
          ...
          debugf (BABEL_DEBUG_COMMON,
                  "Received ack-req (%04X %d) from %s on %s.", nonce, interval,
                  format_address (from), ifp->name);
...
#define BABEL_UNICAST_HELLO 0x8000
      ...
      else if (type == MESSAGE_IHU)
        {
          ...
          if (rc < 0)
            goto fail;
          debugf (BABEL_DEBUG_COMMON,
                  "Received ihu %d (%d) from %s on %s for %s.", txcost,
      ...
      else if (type == MESSAGE_UPDATE)
        {
          ...
          if (message[5] == 0 ||
            ...
            rc = network_prefix (
                message[2], message[4], message[5], message + 12,
                message[2] == 1 ? v4_prefix : v6_prefix, len - 10, prefix);
          else
            rc = -1;
          if (rc < 0)
            {
          ...
          debugf (
              BABEL_DEBUG_COMMON, "Received update%s%s for %s from %s on %s.",
          ...
          else
            {
              ...
              if (parsed_len < len)
                ignore_update = parse_update_subtlv (
                    message + 2 + parsed_len, len - parsed_len, channels);
            }
      ...
      else if (type == MESSAGE_REQUEST)
        {
          ...
          rc = network_prefix (message[2], message[3], 0, message + 4, NULL,
                               len - 2, prefix);
          if (rc < 0)
            goto fail;
      ...
      else if (type == MESSAGE_MH_REQUEST)
        {
          ...
          debugf (BABEL_DEBUG_COMMON,
                  ...
                  format_address (from), ifp->name, format_eui64 (message + 8),
                  seqno);
        ...
        {
          debugf (BABEL_DEBUG_COMMON,
                  "Received unknown packet type %d from %s on %s.", type,
                  format_address (from), ifp->name);
        }
    done:
      i += len + 2;
    ...
    fail:
      flog_err (EC_BABEL_PACKET,
                "Couldn't parse packet (%d, %d) from %s on %s.", message[0],
                message[1], format_address (from), ifp->name);
      goto done;
    }
  ...
  if (babel_ifp->bucket == 0)
    {
      ...
      if (seconds > 0)
        {
          babel_ifp->bucket =
              MIN (BUCKET_TOKENS_MAX, seconds * BUCKET_TOKENS_PER_SEC);
        }
      ...
      if (check_bucket (ifp))
        {
          ...
          rc = babel_send (protocol_socket, packet_header,
                           sizeof (packet_header), babel_ifp->sendbuf,
                           babel_ifp->buffered, (struct sockaddr *) &sin6,
                           sizeof (sin6));
          if (rc < 0)
...
static void
accumulate_bytes (struct interface *ifp, const unsigned char *value,
                  unsigned len)
{
  babel_interface_nfo *babel_ifp = babel_get_if_nfo (ifp);
...
static void
accumulate_unicast_bytes (struct neighbour *neigh, const unsigned char *value,
                          unsigned len)
{
...
void
send_ack (struct neighbour *neigh, unsigned short nonce,
  ...
  if (babel_ifp->num_buffered_updates > 0)
    {
      ...
      debugf (BABEL_DEBUG_COMMON,
              "  (flushing %d buffered updates on %s (%d))", n, ifp->name,
              ifp->ifindex);
...
static void
buffer_update (struct interface *ifp, const unsigned char *prefix,
               unsigned char plen)
{
  babel_interface_nfo *babel_ifp = babel_get_if_nfo (ifp);
  if (babel_ifp->num_buffered_updates > 0 &&
...
void
send_update (struct interface *ifp, int urgent, const unsigned char *prefix,
             unsigned char plen)
{
  babel_interface_nfo *babel_ifp = NULL;
...
void
send_update_resend (struct interface *ifp, const unsigned char *prefix,
                    unsigned char plen)
{
  assert (prefix != NULL);
...
void
send_request (struct interface *ifp, const unsigned char *prefix,
              unsigned char plen)
{
  int v4, pb, len;
...
void
send_unicast_request (struct neighbour *neigh, const unsigned char *prefix,
                      unsigned char plen)
{
  int rc, v4, pb, len;
...
void
send_multihop_request (struct interface *ifp, const unsigned char *prefix,
                       ...
                       const unsigned char *id, unsigned short hop_count)
{
  ...
  debugf (BABEL_DEBUG_COMMON, "Sending request (%d) on %s for %s.", hop_count,
...
void
send_request_resend (struct neighbour *neigh, const unsigned char *prefix,
                     unsigned char plen, unsigned short seqno,
                     unsigned char *id)
{
  if (neigh)
