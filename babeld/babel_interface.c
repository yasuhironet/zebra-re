...
int
babel_ifp_up (struct interface *ifp)
{
...
int
babel_ifp_create (struct interface *ifp)
{
       ...
       "Milliseconds\n")
{
  ...
  babel_ifp->hello_interval =
      no ? BABEL_DEFAULT_HELLO_INTERVAL : hello_interval;
  return CMD_SUCCESS;
}
       ...
       "Milliseconds\n")
{
  ...
  babel_ifp->update_interval =
      no ? BABEL_DEFAULT_UPDATE_INTERVAL : update_interval;
  return CMD_SUCCESS;
}
       ...
       "Milliseconds\n")
{
  ...
  babel_ifp->max_rtt_penalty =
      no ? BABEL_DEFAULT_MAX_RTT_PENALTY : max_rtt_penalty;
  return CMD_SUCCESS;
}
  ...
  FOR_ALL_INTERFACES_ADDRESSES (ifp, connected, node)
    {
      if (connected->address->family == AF_INET6 &&
          memcmp (&connected->address->u.prefix6, address, IPV6_MAX_BYTELEN) ==
              0)
        return 1;
    }
                       ...
                       struct prefix *prefix)
{
  const unsigned char *nexthop =
      memcmp (route->nexthop, route->neigh->address, IPV6_MAX_BYTELEN) == 0
          ? NULL
          : route->nexthop;
  char channels[100];
  ...
  vty_out (
      ...
      route_metric (route), route->refmetric, format_eui64 (route->src->id),
      (int) route->seqno, channels, (int) (babel_now.tv_sec - route->time),
      route->neigh->ifp->name, format_address (route->neigh->address),
      nexthop ? " nexthop " : "", nexthop ? format_address (nexthop) : "",
