...
/* d = s1 - s2 */
void
timeval_minus (struct timeval *d, const struct timeval *s1,
...
unsigned char *
mask_prefix (unsigned char *restrict ret, const unsigned char *restrict prefix,
             unsigned char plen)
{
  ...
  if (plen % 8 != 0)
    ret[plen / 8] = (prefix[plen / 8] & ((0xFF << (8 - (plen % 8))) & 0xFF));
  return ret;
}
...
parse_eui64 (const char *eui, unsigned char *eui_r)
{
  ...
  n = sscanf (eui, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
              &eui_r[0], &eui_r[1], &eui_r[2], &eui_r[3], &eui_r[4], &eui_r[5],
              &eui_r[6], &eui_r[7]);
  if (n == 8)
    return 0;
